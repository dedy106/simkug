...
function doLogin(lokasi){	
	try{					
		...
		var param = "cmd=login&email="+data.email+"&lokasi="+data.office+"&pwd="+data.pwd;
...
function loadClass(){
	...
	addJs("classes/addOn/swfobject.js");
...
function showOffice(){		
	try{			
		...
		if (window.listOff  == null){					
			...
			window.listOff.style.cssText = "position:absolute;width:400;height:"+height+";left:10;top:30;z-index:9999";
		...
		if (msg.search("error") == -1 && msg != ""){										
			...
			msg = "";
			for (var i in data){
				msg += "<div id='"+data[i].kode+"' style='display:block;width:100%;height:20;cursor:pointer' ondblclick='getSelectOffice(event,\""+data[i].kode+"\",\""+data[i].nama+"\");' onmouseover='this.style.background=\"#ff9900\";'' onmouseout='this.style.background=\"transparent\";' >"+data[i].nama+"</div>";
			}
		}			
	...
	try		
		{						
			...
			if (ok){
				...
				window.system = new portalui_system({parentType:"external"});						
