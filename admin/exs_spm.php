...
$sql="select periode,kode_lokasi from periode where kode_lokasi='01' and periode>='201501' order by periode";
$rs = execute($sql,$error);
...
while ($row = $rs->FetchNextObject($toupper=false))
{
	$sql="select kode_fs from fs where kode_lokasi='01' and kode_fs='FS1' order by kode_fs";
	$rs1 = execute($sql,$error);
	while ($row1 = $rs1->FetchNextObject($toupper=false))
	{
		$sql2="exec sp_exs_neraca '$row1->kode_fs','$row->kode_lokasi','$row->periode'";
		//echo $sql2;
		$rs2 = execute($sql2,$error);
	}
}
