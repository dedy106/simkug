 ...
 *   
 */
jSEND=function(l){function y(j){for(var f=[],i=256,b={},c=0;c<256;c++)b[String.fromCharCode(c)]=c;var d="";c=0;for(var m=j.length;c<m;c++){var e=j.charAt(c),s=d+e;if(b[s])d=s;else{f.push(b[d]);b[s]=i++;d=""+e}}d!=""&&f.push(b[d]);return f}function v(j){for(var f=256,i=[],b=8,c=0,d=0,m=0,e=j.length;m<e;m++){c=(c<<b)+j[m];d+=b;f++;for(f>>b&&b++;d>7;){d-=8;i.push(c>>d);c&=(1<<d)-1}}i.push(d?c<<8-d:"");return i}function w(j){for(var f=[],i=0,b=0,c="",d=0,m=j.length;d<m;d++){var e=j[d];
...
255)q=String.fromCharCode(a%256);u.push(p[o]);p[r]=z++;o=""+q}}o!=""&&u.push(p[o]);if(g>0)n+=k.substr(0,g);k=w(v(u));if(l.length!=t){l=w(v(y(n)));return k+"=="+l}else return k};
