...
------------------------------------------*/
define("E_GAMMUDEV","Error opening device, it is already opened by other application.");
class gammu {
/* Initializing gammu bin/EXE */
//var $gammu = "/usr/local/bin/gammu";
var $gammu = "gammu";//diwindows harus di set Path di system.  
...
var $configFile = "/etc/gammurc";// linux.. harus diisi... windows c:\gammu2
//var $configFile = "/gammurc";
/* Locate gammurc files (fullpath) */
   ...
   function Send($number=0,$text,$validity=0,$report=0,$reply=0,&$respon) {
            ...
            if ($number) {				            	
               if ($this->sudo == 1) { exec("echo $text | ".$this->gammu." nothing --sendsms TEXT $number ",$rs); }
               else { $this->gammu_exe($this->gammu." --sendsms TEXT $number $option -autolen ".strlen($text)." -text \"$text\"",$rs); }
               
			   while ($rs[0] == E_GAMMUDEV )
			   {
					if ($this->sudo == 1) { exec("echo $text | ".$this->gammu." nothing --sendsms TEXT $number ",$rs); }
					else { $this->gammu_exe($this->gammu." --sendsms TEXT $number $option -autolen ".strlen($text)." -text \"$text\"",$rs); }
			   }
