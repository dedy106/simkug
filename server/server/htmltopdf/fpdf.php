...
function Image($file,$x,$y,$w=0,$h=0,$type='',$link='',$paint=true)
{
	...
	if(!isset($this->images[$file]))
	{
        ...
        if ($type == "png"  and $isMask){
            $info['cs']="DeviceGray"; // try to force grayscale (instead of indexed)
        } 
		set_magic_quotes_runtime($mqr);
		if ($type == "png" and $maskImg>0) $info['masked'] = $maskImg;
...
function Output($name='',$dest='',$response = '')
{
...
//			error_log($dest );			
			...
			global $separator;			
			$name = md5(uniqid(rand(), true)) .".pdf";					
			$save =  $serverPath ."/tmp/$name";
			file_put_contents($save, $this->buffer);
			return  $serverTempDir . $separator . "$name";
// 			echo $this->buffer;
//			readfile($name);
//			unlink($name);
			break;
...
function _putimages()
{
    ...
    while(list($file,$info)=each($this->images))
    {
        ...
        $this->_out('/LEFT '.$info['x']);
        $this->_out('/TOP '.$info['y']);
