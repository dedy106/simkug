...
if (!defined('RELATIVE_PATH')) define('RELATIVE_PATH','server/htmltopdf/');
if (!defined('FPDF_FONTPATH')) define('FPDF_FONTPATH','server/htmltopdf/font/');
require_once(RELATIVE_PATH.'fpdf.php');
require_once(RELATIVE_PATH.'htmltoolkit.php');
uses("server_htmltopdf_fpdf", false);
uses("server_htmltopdf_gifImg", false);
...
$count = 0;
while ($count < 4){
    $pos = strrpos($path, $dirSeparator);
	$path = substr($path, 0, $pos);
	$count++;
}
$rootPath = $path;
$dirSeparator = "/";
$rootPath = str_replace("\\","/",$path);
...
function HTML2FPDF($orientation='P',$unit='mm',$format='A4', $baseFontSize = 10, $lineHeight = 5)
{
//! @desc Constructor
//! @return An object (a class instance)
  ...
  $this->setBaseFontSize($baseFontSize); //Nanang --
	//Initialization of the attributes
	$this->SetFont('Arial','',$baseFontSize); // Changeable?(not yet...)
    ...
    case 'ADDRESS': 
			...
			if(isset($attr['SRC']))
      {
          $srcpath = $attr['SRC'];
            ...
            global $rootPath;
            global $dirSeparator;
            $srcpath = $rootPath. $dirSeparator . $srcpath;
...
//! @return void
  ...
  if($tag=='OUTLINE')
  {
   ...
   if ($this->buffer_on == false) $this->listnum--;//Adjust minor BUG (this happens when there are two </OL> together)
    ...
    $this->lineheight = 5;
 		$this->Ln($this->lineheight);
    $this->SetFontSize($this->baseFontSize); //Nanang -- default 10
	...
	foreach($arrayaux as $k => $v)
  {
  	switch($k){
   			case 'WIDTH':
		  			...
		  			break;
	  		case 'LEFT':
		   			$this->divleft = ConvertSize($v,$this->pgwidth);
		  			break;	  		
            case 'TOP':
		   			$this->divtop = ConvertSize($v,$this->pgwidth);
		  			break;	  		
            case 'POSITION':
		   			$this->divposition = $v;
		  			break;	  		            
			  ...
			  case 'FONT-SIZE': //Does not support: smaller, larger
			      if(is_numeric($v{0}))
			      {
  			      ...
  			      switch($v)
  			      {
  			         ...
  			         case 'XX-SMALL': $this->SetFontSize( (0.7)* $this->baseFontSize);
  			             break;
                 case 'X-SMALL': $this->SetFontSize( (0.77) * $this->baseFontSize);
			               break;
			           case 'SMALL': $this->SetFontSize( (0.86)* $this->baseFontSize);
  			             break;
  			         case 'MEDIUM': $this->SetFontSize($this->baseFontSize);
  			             break;
  			         case 'LARGE': $this->SetFontSize( (1.2)*$this->baseFontSize);
  			             break;
  			         case 'X-LARGE': $this->SetFontSize( (1.5)*$this->baseFontSize);
  			             break;
  			         case 'XX-LARGE': $this->SetFontSize( 2*$this->baseFontSize);
			               break;
              }
            }
			   	  break;
