...
{
	if(!isSet($gif) || (@get_class($gif) <> "gifImg") || !$gif->loaded() || ($lpszFileName == "")) {
		return false;
	}
...
{
	if(isSet($gif) && (@get_class($gif) == "gifImg") && $gif->loaded()) {
		$width  = $gif->width();
		$height = $gif->height();
	}
...
class gifImg
{
	...
	function loadFile($lpszFileName, $iIndex)
	{
		...
		error_log("laod file " . $lpszFileName);
        // READ FILE
...
//		$this->m_lpData = @fRead($fh, @fileSize($lpszFileName));
        ...
        error_log("reasd1 file " . $contents);
		do {
			if(!$this->m_img->load($this->m_lpData, $imgLen = 0)) {
				return false;
			}
			error_log("reasd111 file " . $this->m_lpData);
			$this->m_lpData = substr($this->m_lpData, $imgLen);
		}
		while($iIndex-- > 0);
        error_log("read2 file " . $contents);
		$this->m_bLoaded = true;
		return true;
	}
