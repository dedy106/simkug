...
class server_app_sc_dataProvider  extends server_ShareObject
{
	var $dbLib;
	var $config;
	...
	function alert($filter, $info = null){	
		...
		switch($info){
			case "01" :
				$sql->add("select count(no_kontrak) as tot from sc_kontrak_m where kode_lokasi='$lokasi' ");
				$sql->add("select no_kontrak as no_bukti, keterangan
					from sc_kontrak_m where  kode_lokasi='$lokasi'  ");
				$title = "Kontrak";
			break;
			case "02" :
				$sql->add("select count(no_akru) as tot from sc_akru_m where  kode_lokasi='$lokasi' ");
				$sql->add("select no_akru as no_bukti, keterangan
					from sc_akru_m where  kode_lokasi='$lokasi'   ");
				$title = "Akru Billing";
			break;
			case "03" :
				$sql->add("select count(no_ar) as tot from sc_ar_m where  kode_lokasi='$lokasi' ");
				$sql->add("select no_ar as no_bukti, keterangan
				from sc_ar_m where  kode_lokasi='$lokasi'   ");
				$title = "Invoice";
			break;
			case "04" :
