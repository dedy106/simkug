...
class server_report_investasi_SubBidang
{
	protected $caption;
	protected $filter;
	protected $filter2;
	...
	function getTotalPage()
	{
		...
		$sql="select count(*) from sub_bidang ".$this->filter;
		global $dbLib;
	...
	function getHtml()
	{
		/*$sql = "select a.kode, b.nama, a.so_awal, a.debet, a.kredit, a.so_akhir from glma a ".
				" inner join masakun b on b.kode = a.kode and a.kode_lokasi = b.kode_lokasi ". $this->filter .
				" order by a.kode";*/
		$sql = "select * from sub_bidang ".$this->filter." order by kode_sub_bidang";
		$start = (($this->page-1) * $this->rows);
		//$sql .= " limit ". $start . "," . $this->rows;
		
		$html = "<label style='{font-size:20px;}'><b>Data Sub Bidang Usaha</b></label><br>"; 		
		//$html .= "<label style='{font-size:12px}'>$this->lokasi</label><br>"; 					
		$html .= "<label style='{font-size:14px}'>$this->showFilter</label></br></br>"; 					
		$html .= "<table width='600' border='1' cellpadding='0' cellspacing='0' bordercolor='#000000'>".				
...
//		return "server/tmp/$name";
		...
		header ("Content-Disposition: attachment; filename=SubBidang.xls");
