...
class server_report_investasi_Produk
{
	protected $caption;
	protected $filter;
	protected $filter2;
	...
	function getTotalPage()
	{
		...
		$sql="select count(*) from produk ".$this->filter;
		global $dbLib;
	...
	function getHtml()
	{
		/*$sql = "select a.kode, b.nama, a.so_awal, a.debet, a.kredit, a.so_akhir from glma a ".
				" inner join masakun b on b.kode = a.kode and a.kode_lokasi = b.kode_lokasi ". $this->filter .
				" order by a.kode";*/
		$sql = "select * from produk ".$this->filter." order by kode_produk";
		$start = (($this->page-1) * $this->rows);
		//$sql .= " limit ". $start . "," . $this->rows;
		
		$html = "<label style='{font-size:20px;}'><b>Data Produk</b></label><br>"; 		
		...
		$html .= "<table width='900' border='1' cellpadding='0' cellspacing='0' bordercolor='#000000'>".				
				...
				"   <th width='150' nowrap='nowrap' scope='col'><font color='#ffffff'>Akun Investasi</font></th>".
