...
class server_report_investasi_BentukKKP
{
	protected $caption;
	protected $filter;
	protected $filter2;
	...
	function getTotalPage()
	{
		...
		$sql="select count(*) from kkp_bentuk ".$this->filter;
		global $dbLib;
	...
	function getHtml()
	{
		/*$sql = "select a.kode, b.nama, a.so_awal, a.debet, a.kredit, a.so_akhir from glma a ".
				" inner join masakun b on b.kode = a.kode and a.kode_lokasi = b.kode_lokasi ". $this->filter .
				" order by a.kode";*/
		$sql = "select * from kkp_bentuk ".$this->filter." order by kode_bentuk";
		$start = (($this->page-1) * $this->rows);
		//$sql .= " limit ". $start . "," . $this->rows;
		
		$html = "<label style='{font-size:20px;}'><b>Data Bentuk KKP</b></label><br>"; 		
