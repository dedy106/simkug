...
class server_report_investasi_Perihal
{
	protected $caption;
	protected $filter;
	protected $filter2;
	...
	function getTotalPage()
	{
		...
		$sql="select count(*) from nota_perihal ".$this->filter;
		global $dbLib;
	...
	function getHtml()
	{
		/*$sql = "select a.kode, b.nama, a.so_awal, a.debet, a.kredit, a.so_akhir from glma a ".
				" inner join masakun b on b.kode = a.kode and a.kode_lokasi = b.kode_lokasi ". $this->filter .
				" order by a.kode";*/
		$sql = "select * from nota_perihal ".$this->filter." order by kode_perihal";
		$start = (($this->page-1) * $this->rows);
		//$sql .= " limit ". $start . "," . $this->rows;
		
		$html = "<label style='{font-size:20px;}'><b>Data Nota Perihal</b></label><br>"; 		
		...
		$html .= "<table width='400' border='1' cellpadding='0' cellspacing='0' bordercolor='#000000'>".				
				...
				"   <th width='250' nowrap='nowrap' scope='col'><font color='#ffffff'>Keterangan</font></th>".
				//"   <th width='100' nowrap='nowrap' scope='col'><font color='#ffffff'>Jenis</font></th>".
