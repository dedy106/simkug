...
class server_report_dmt_rptTerima extends server_report_basic
{	
...
inner join cust c on a.kode_bank=c.kode_cust";
	...
	function getHtml()
	{
		...
		$tmp=explode("/",$this->filter2);
		$periode=$tmp[0];
		$sql="select a.no_kas, a.no_dokumen, a.akun_kb,b.nama as nama_akun,date_format(a.tanggal,'%d/%m/%Y') as tanggal, a.keterangan,a.nilai,  
       a.no_link,a.kode_bank,c.nama as nama_cust,a.nilai
from kas_m a
inner join masakun b on a.akun_kb=b.kode_akun and a.kode_lokasi=b.kode_lokasi
inner join cust c on a.kode_bank=c.kode_cust ";
		...
		echo $AddOnLib->judul_laporan("laporan penerimaan",$this->lokasi,$AddOnLib->ubah_periode($periode));
  ...
  <tr>
    <td colspan='11' style='padding:5px'><table width='100%' border='0' cellspacing='2' cellpadding='1'>
	  ...
	  <tr>
	   ...
	   <tr>
        <td class='header_laporan'>Nilai KasBank  </td>
	  ...
	  <tr>
        <td class='header_laporan'>No Invoice   </td>
        <td class='header_laporan'>:&nbsp;$row->no_link</td>
      </tr>
	<tr>
