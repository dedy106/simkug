...
class server_report_rra_rptPdrk3 extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			echo "<table width='2000' border='0' cellspacing='2' cellpadding='1'>
				  <tr>
    ...
    echo "</table></td>
			...
			left join (select a.kode_cc,a.kode_akun,a.kode_lokasi, isnull(a.kode_drk,'-') as kode_drk , case when a.versi = '000' or a.versi = '0'  then 'P' when a.versi = '7' or a.versi = '007' then 'C' else case when '".$row->jenis_agg."' = 'OPEX' then 'K' else a.versi end end as jenis, 
					   ...
					   from (select a.* from rra_pdrk_orgi a left outer join rra_rev_m b on b.no_pdrk = a.no_pdrk and b.kode_lokasi = a.kode_lokasi where b.no_rev is null and a.no_pdrk = '$row->no_pdrk' 
										union
										select a.* from rra_rev_orgi a left outer join rra_grev_m b on b.no_rev = a.no_rev and b.no_pdrk = a.no_pdrk  where b.no_grev is null and a.no_pdrk = '$row->no_pdrk' 
										union 
										select a.* from rra_grev_orgi a left outer join rra_mrev_m b on b.no_grev = a.no_grev and b.no_pdrk = a.no_pdrk  where b.no_mrev is null and a.no_pdrk = '$row->no_pdrk' 
										union 
										select * from rra_mrev_orgi where no_pdrk = '$row->no_pdrk'
					) a 				   
