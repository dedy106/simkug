...
class server_report_rra_rptWasGar extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$tmp=explode("/",$this->filter2);
		$nama_ver=$tmp[0];				
		$sql="select a.kode_ubis, c.nama as ubis, substr(a.periode, 0,4) as thn, b.kode_akun, d.nama as nmakun,
				sum(case substr(b.periode, 5,2) when '01' then case when dc='D' then b.nilai else 0 end else 0 end) as j1,
				sum(case substr(b.periode, 5,2) when '02' then case when dc='D' then b.nilai else 0 end else 0 end) as j2,
				sum(case substr(b.periode, 5,2) when '03' then case when dc='D' then b.nilai else 0 end else 0 end) as j3,
				sum(case substr(b.periode, 5,2) when '04' then case when dc='D' then b.nilai else 0 end else 0 end) as j4,
				sum(case substr(b.periode, 5,2) when '05' then case when dc='D' then b.nilai else 0 end else 0 end) as j5,
				sum(case substr(b.periode, 5,2) when '06' then case when dc='D' then b.nilai else 0 end else 0 end) as j6,
				sum(case substr(b.periode, 5,2) when '07' then case when dc='D' then b.nilai else 0 end else 0 end) as j7,
				sum(case substr(b.periode, 5,2) when '08' then case when dc='D' then b.nilai else 0 end else 0 end) as j8,
				sum(case substr(b.periode, 5,2) when '09' then case when dc='D' then b.nilai else 0 end else 0 end) as j9,
				sum(case substr(b.periode, 5,2) when '10' then case when dc='D' then b.nilai else 0 end else 0 end) as j10,
				sum(case substr(b.periode, 5,2) when '11' then case when dc='D' then b.nilai else 0 end else 0 end) as j11,
				sum(case substr(b.periode, 5,2) when '12' then case when dc='D' then b.nilai else 0 end else 0 end) as j12,							
				sum(case substr(b.periode, 5,2) when '01' then case when dc='C' then -b.nilai else 0 end else 0 end) as n1,
				sum(case substr(b.periode, 5,2) when '02' then case when dc='C' then -b.nilai else 0 end else 0 end) as n2,
				sum(case substr(b.periode, 5,2) when '03' then case when dc='C' then -b.nilai else 0 end else 0 end) as n3,
				sum(case substr(b.periode, 5,2) when '04' then case when dc='C' then -b.nilai else 0 end else 0 end) as n4,
				sum(case substr(b.periode, 5,2) when '05' then case when dc='C' then -b.nilai else 0 end else 0 end) as n5,
				sum(case substr(b.periode, 5,2) when '06' then case when dc='C' then -b.nilai else 0 end else 0 end) as n6,
				sum(case substr(b.periode, 5,2) when '07' then case when dc='C' then -b.nilai else 0 end else 0 end) as n7,
				sum(case substr(b.periode, 5,2) when '08' then case when dc='C' then -b.nilai else 0 end else 0 end) as n8,
				sum(case substr(b.periode, 5,2) when '09' then case when dc='C' then -b.nilai else 0 end else 0 end) as n9,
				sum(case substr(b.periode, 5,2) when '10' then case when dc='C' then -b.nilai else 0 end else 0 end) as n10,
				sum(case substr(b.periode, 5,2) when '11' then case when dc='C' then -b.nilai else 0 end else 0 end) as n11,
				sum(case substr(b.periode, 5,2) when '12' then case when dc='C' then -b.nilai else 0 end else 0 end) as n12
			from RRA_PDRK_M a
			inner join rra_anggaran b on b.no_bukti = a.no_pdrk and b.kode_lokasi = a.kode_lokasi
			inner join rra_ubis c on c.kode_ubis = a.kode_ubis and c.kode_lokasi = a.kode_lokasi 
			inner join rra_masakun d on d.kode_akun = b.kode_akun and d.kode_lokasi = a.kode_lokasi 
			" . $this->filter ." group by a.kode_ubis,c.nama, b.kode_akun, d.nama,substr(a.periode, 0,4) ";
		//error_log($sql);
		...
		{						
			if ($tmp != $row->ubis){
				...
				<tr>
					<td align='center' class='header_laporan'>RRA(+)</td>
					<td align='center' class='header_laporan'>RRA(-)</td>
					<td align='center' class='header_laporan'>RRA(+)</td>
					<td align='center' class='header_laporan'>RRA(-)</td>
					<td align='center' class='header_laporan'>RRA(+)</td>
			...
			echo "<tr>
				<td>$ix</td>
				<td>$row->kode_akun - $row->nmakun</td>
