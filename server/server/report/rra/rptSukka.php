...
class server_report_rra_rptSukka extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$tmp=explode("/",$this->filter2);
		$nama_ver=$tmp[0];				
		$sql="select a.no_pdrk, g.deskripsi, g.keterangan as sukka,a.tanggal,a.jenis_agg,b.nama as nama_ubis,c.nama as nama_gubis,
			   to_char(g.tanggal,'DD') as tgl,to_char(g.tanggal,'MM') as bulan,to_char(g.tanggal,'YYYY') as tahun,
			   g.nik_app,d.nama as nama_app, case when i.no_app is null then '-' else f.no_surat end as no_surat, d.jabatan,
			   h.nama as kota
			from rra_pdrk_m a
			inner join rra_sukka g on g.no_pdrk = a.no_pdrk and g.kode_lokasi = a.kode_lokasi 
			inner join rra_ubis b on a.kode_ubis=b.kode_ubis and a.kode_lokasi=b.kode_lokasi
			...
			left outer join rra_app_d i on i.catatan = a.no_pdrk and i.no_app like 'APPSK%' and i.kode_lokasi = a.kode_lokasi
			" . $this->filter;
		//error_log($sql);
		...
		{						
			...
			echo "<table width='800' border='0' cellspacing='2' cellpadding='1'>
				  <tr>
					<td align='center'><b>KEPUTUSAN $row->jabatan<br>
