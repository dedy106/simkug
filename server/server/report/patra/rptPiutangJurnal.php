...
class server_report_patra_rptPiutangJurnal extends server_report_basic
{
	function getTotalPage()
	{
		...
		$periode=$tmp[2];
		$filter2=$tmp[3];
		$sql="select count(a.no_ar) as jum
from patra_ar_m a
...
left join karyawan c on a.nik_buat=c.nik and a.kode_lokasi=c.kode_lokasi $filter2 $modul";
	...
	function getHtml()
	{
		...
		$sql="select a.no_ar,a.no_dokumen,a.kode_lokasi,a.periode,a.tanggal,date_format(a.tanggal,'%d/%m/%Y') as tanggal1,a.keterangan,a.kode_lokasi,
			  ...
			  <tr>
				<td><table border='0' cellspacing='2' cellpadding='1'>
				  ...
				  <tr>
					...
					<td width='496' class='header_laporan'>:&nbsp;$row->no_ar</td>
					</tr>
				  <tr>
