...
class server_report_kb_rptKbVer
{
	protected $caption;
	protected $filter;
	protected $filter2;
...
inner join karyawan c on a.nik_app=c.nik and a.kode_lokasi=c.kode_lokasi ".$this->filter;
	...
	function getHtml()
	{
		...
		global $dbLib;
		$sql="select a.periode,a.tanggal,a.kode_lokasi,a.no_ver,a.keterangan,a.modul,a.nik_buat,a.nik_app,
...
inner join karyawan c on a.nik_app=c.nik and a.kode_lokasi=c.kode_lokasi ".$this->filter." order by a.no_ver";
		...
		$html=$AddOnLib->judul_laporan("laporan verifikasi",$this->lokasi,$AddOnLib->ubah_periode($periode));
...
";
	  ...
	  $sql1="select a.status,a.catatan,a.modul,a.no_bukti
from ver_d a
