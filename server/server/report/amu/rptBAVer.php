...
class server_report_amu_rptBAVer extends server_report_basic
{	
	var $tblHeader;
	var $dbField;
	var $dbSQL;
		...
		if ($model == "BA")
			...
			$sql = "select count(*) from (select distinct bb.nama as nmarea, c.nama as nmubis, 			
				date_format(now(),'%d-%m-%Y') as tgl, bb.tempat, b.kode_lokfa, ifnull(upper(x.dcs_area), 'Kantor Regional') as area
					...
					".  $this->filter ." ". ($regdiva ? "and b.ref1 = '$divisi'" :"")."
					union 
					select distinct 'Kantor Regional' as nmarea, c.nama as nmubis, 			
	...
	function getHtml()
	{
				  ...
				  <tr>
					...
					<td width='90%' align='center' valign='bottom' class='lokasi_laporan2'>
							<table width='100%' border='0' cellspacing='2' cellpadding='1'>
								  <tr>
									<td colspan=2 align='center' class='lokasi_laporan2'>BERITA ACARA HASIL VERIFIKASI - $jns_proc </td>						
								  </tr>					
								  <tr>
		...
		if ($model == "BA")
			$sql = "select distinct b.no_fa, b.no_sn,  b.kode_klpfa,b.kode_lokfa, b.nama, b.nama2 as alamat, date_format(b.tgl_perolehan,'%d-%m-%Y') as cap_date, 
					b.nilai, b.nilai_ap, b.nilai_buku,b.ref2, b.ref1,k.jns_proc, i.no_evd, j.kesimpulan, ". $this->dbSQLField ." 					
					from amu_asset b					
					...
					inner join amu_alt_ver_d i on i.no_gabung = b.no_gabung 
					inner join amu_alt_baver_d j on j.no_gabung = b.no_gabung 
			...
			$sql = "select distinct b.no_fa, b.no_sn,  b.kode_klpfa,b.kode_lokfa, b.nama, b.nama2 as alamat, date_format(b.tgl_perolehan,'%d-%m-%Y') as cap_date, 
					...
					inner join amu_alt_ver_d i on i.no_gabung = b.no_gabung 
					inner join amu_alt_baver_d j on j.no_gabung = b.no_gabung 
					". $this->filter  . " and j.no_ba = '$row1->NO_BA'
					order by b.no_fa ";		
		...
		echo "<tr>
				<td><table  border='1' cellspacing='0' cellpadding='0' class='kotak' width='100%'>
				 <tr align='center' bgcolor='#CCCCCC'>
					...
					<td rowspan='2' bgcolor='#ff9900' class='header_laporan' style='color:#ffffff'>Kesimpulan</td>
					</tr>
				  <tr align='center' bgcolor='#CCCCCC'>
					". $this->tblHeader ."					
				  </tr>
				  ...
				  echo "<td class='header_laporan'>".(count($this->tblColHeader)+1)."</td>";
			...
			while (!$rs->EOF && $row = $rs->FetchNextObject(true))
			{
				$this->processRow($jns_proc, $row, $no);	
					echo "<td class='isi_laporan'>$row->NO_EVD</td> 
						<td class='isi_laporan'>$row->KESIMPULAN</td> 
						</tr>";
				$no++;			
			}
	...
	function createXls(){
		...
		$namafile = "BAVerifikasi.xls";
		$excel = new Spreadsheet_Excel_Writer();
		
		$excel->send($namafile);
		$excel->setCustomColor(14, 0,128,128); 
		$excel->setCustomColor(15, 251,180,127); 
		...
		$hdrBA =& $excel->addFormat(array('size' => 9,'bold' => true, 'border' => 1,'halign' => 'center','valign' => 'vcenter', 'bordercolor' => 'black','fgcolor' => 'orange','pattern' => 1, 'color' => 'black','textwrap' => 1	));
		...
		if ($model == "BA")
			...
			$sql1 = "select distinct bb.nama as nmarea, c.nama as nmubis, 			
					...
					".  $this->filter ." ". ($regdiva ? "and b.ref1 = '$divisi'" :"")."
					union 
					select distinct ".($diva ? "bb.nama":"'Kantor Regional'") ." as nmarea, c.nama as nmubis, 			
					date_format(now(),'%d-%m-%Y') as tgl, '-' as tempat, ".($regdiva ? "b.ref1":"b.kode_lokfa")." as kode_lokfa, b.ref1 as area
					from amu_asset b 				
					inner join amu_lokasi c on c.kode_lokfa = b.kode_lokfa and c.kode_lokasi = b.kode_lokasi and c.kode_induk = '00'										
					inner join amu_alt_baver_d k on k.no_gabung = b.no_gabung and k.jns_proc = '$jns_proc' and k.periode = '$periode' 						
		...
		while ($row1 = $rs1->FetchNextObject(true))
		{									
			$sheet =& $excel->addWorksheet('KK BA Verifikasi');			
			$sheet->setMerge(1,2,1,count($this->tblColHeader) + 2);
			$sheet->write(1,2,"KERTAS KERJA BA VERIFIKASI - ". $jns_proc, $header);
			...
			$sheet->setMerge(6,count($this->tblColHeader)+2,7,count($this->tblColHeader)+2);
			$sheet->setColumn(count($this->tblColHeader)+1, count($this->tblColHeader)+1, 20);
			$sheet->write(6,count($this->tblColHeader)+2,"Kesimpulan", $hdrBA);
			$sheet->write(7,count($this->tblColHeader)+2," ", $hdrBA);
			$sheet->setColumn(count($this->tblColHeader)+2, count($this->tblColHeader)+2, 20);
			...
			$sheet->write($rowStart + 1, count($this->tblColHeader) + 2, count($this->tblColHeader)+3, $normalFormat2);
			...
			{						
				...
				$sheet->writeString($rowStart,count($this->dbField) + $this->startField , $tmp["KESIMPULAN"],$normalFormat);
			...
			$sheet->writeString($rowStart,count($this->dbField) + $this->startField , " ",$normalFormat2);
