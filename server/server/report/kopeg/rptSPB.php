...
class server_report_kopeg_rptSPB
{
	protected $caption;
	protected $filter;
	protected $rows;
	...
	function getTotalPage()
	{
		...
		$sql = "select count(distinct a.no_spb) ".
			"from spb_m a ".$this->filter2." and a.modul='KP.SPB' and a.jenis='SIMP' and a.no_del='-' ";
	...
	function getHtml()
	{
		...
		while (!$page->EOF)
		{
			$sql = "select distinct a.no_spb,date_format(a.tanggal,'%e/%m/%Y') as tgl,upper(d.nama) as nsbh,upper(d.alamat) as almt,d.no_rek, ".
				"a.keterangan,upper(d.bank) as bank,upper(d.cabang) as cabang,upper(d.nama_rek) as nmrek,(a.nilai+a.nilai_pot) as nilai,a.nilai_pot,a.nilai as tot ".
				", j.nama as nama_buat ". //,e.nama as nama_setuju,d.jabatan as jabatan_buat,e.jabatan as jabatan_setuju,c.cabang,c.nama_rek,f.logo".
				", g.nama as gm, h.nama as setuju,  i.initial as nmunit, a.kode_pp ".
				"from spb_m a inner join kop_simp_spbbuk b on a.no_spb=b.no_bukti and a.kode_lokasi=b.kode_lokasi and a.modul='KP.SPB' and a.jenis='SIMP' and a.no_del='-' ".
