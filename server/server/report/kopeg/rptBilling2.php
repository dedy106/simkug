...
class server_report_kopeg_rptBilling2
{
	protected $caption;
	protected $filter;
	protected $rows;
	...
	function getTotalPage()
	{
		...
		$sql = "select count(distinct a.no_bill) ".
			"from kop_pinjbill_m a ".$this->filter2;
	...
	function getHtml()
	{
		...
		while (!$page->EOF)
		{
			$sql = "select a.no_bill,b.no_kontrak,c.kode_agg,upper(d.nama) as nm,c.nilai,c.lama_bayar,c.p_bunga,b.cicilan_ke,b.npokok,b.nbunga,b.npokok+b.nbunga as tghn 
				...
				" and a.no_bill='".$page->fields[0]."' order by d.kode_loker,c.no_pinj ";
			$invc=$dbLib->execute($sql);
			$rs = $invc->FetchNextObject($toupper=false);
			$i = $start+1;
			$AddOnLib=new server_util_AddOnLib();
			$html=$AddOnLib->judul_laporan("LAPORAN DAFTAR AKRU PINJAMAN<br />NO. AKRU : ".$rs->no_bill,$filter[1],$AddOnLib->ubah_periode($periode));
			$html.=	"<table width='810' border='1' align='center' cellpadding='1' cellspacing='0' class='kotak'>
					  <tr align='center' bgcolor='#CCCCCC'>
					    ...
					    <td class='istyle18'>Nilai Pinjaman</td>
