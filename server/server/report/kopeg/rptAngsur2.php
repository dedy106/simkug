...
class server_report_kopeg_rptAngsur2
{
	protected $caption;
	protected $filter;
	protected $rows;
	...
	function getTotalPage()
	{
		...
		$sql = "select count(distinct a.no_angs) 
			...
			inner join kop_pinj_m d on d.no_pinj=c.no_pinj and d.kode_lokasi=c.kode_lokasi
			inner join kop_agg e on e.kode_agg=d.kode_agg and e.kode_lokasi=d.kode_lokasi
			inner join kop_pinjbill_m f on f.no_bill=c.no_bill and f.kode_lokasi=c.kode_lokasi ".$this->filter;
	...
	function getHtml()
	{
		...
		while (!$page->EOF)
		{
			$sql = "select a.no_angs,e.nama,e.kode_agg,c.cicilan_ke,date_format(f.tanggal,'%d/%m/%Y') as tglakru,
				...
				from kop_pinjangs_m a inner join kop_pinjangs_d b on a.no_angs=b.no_angs and a.kode_lokasi=b.kode_lokasi and a.jenis='PINJANGS' 
				inner join kop_pinjbill_d c on b.no_pinj=c.no_pinj and b.kode_lokasi=c.kode_lokasi
				inner join kop_pinj_m d on d.no_pinj=c.no_pinj and d.kode_lokasi=c.kode_lokasi
				inner join kop_agg e on e.kode_agg=d.kode_agg and e.kode_lokasi=d.kode_lokasi
				inner join kop_pinjbill_m f on f.no_bill=c.no_bill and f.kode_lokasi=c.kode_lokasi
				left outer join (select no_kontrak,no_pinj,kode_lokasi,date_format(tanggal,'%Y%m') as per, sum(npokok+nbunga) as nangs 
						from kop_pinjangs_d group by no_kontrak,no_pinj,kode_lokasi,date_format(tanggal,'%Y%m')) aa 
						on aa.no_kontrak=d.no_kontrak and aa.no_pinj=c.no_pinj and aa.kode_lokasi=d.kode_lokasi and date_format(f.tanggal,'%Y%m') = aa.per ".$this->filter.
