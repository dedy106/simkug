...
class server_report_kopeg_rptSetorP
{
	protected $caption;
	protected $filter;
	protected $rows;
	...
	function getTotalPage()
	{
		...
		$sql = "select count(distinct a.no_setor) 
			from kop_proyeksetor_m a inner join kop_proyeksetor_d b on a.no_setor=b.no_setor and a.kode_lokasi=b.kode_lokasi ".$this->filter;
	...
	function getHtml()
	{
		...
		while (!$page->EOF)
		{
			$sql = "select a.no_setor,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.keterangan,a.nilai,c.no_bukti,
				...
				from kop_proyeksetor_m a inner join kop_proyeksetor_d b on a.no_setor=b.no_setor and a.kode_lokasi=b.kode_lokasi
				inner join kop_proyekbayar_m c on b.no_bukti=c.no_bukti and b.kode_lokasi=c.kode_lokasi
