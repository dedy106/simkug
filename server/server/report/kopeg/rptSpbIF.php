...
class server_report_kopeg_rptSpbIF
{
	protected $caption;
	protected $filter;
	protected $rows;
	...
	function getHtml()
	{
		...
		while (!$page->EOF)
		{
			$sql = "select a.no_ifptg as no_spb,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.keterangan,a.akun_ap as akun_hutang,a.nik_buat,a.nik_buat as kode_terima,a.nik_setuju,a.nilai as nilai,0 as nilai_pot,a.nilai as tot,c.alamat,c.bank,c.no_rek,
				...
				, g.nama as gm, h.nama as setuju, i.initial as nmunit, j.kode_pp
				from ifptg_m a
				inner join if_m j on a.no_if=j.no_if and a.kode_lokasi=j.kode_lokasi
				inner join masakun b on a.akun_ap=b.kode_akun and a.kode_lokasi=b.kode_lokasi
				...
				inner join pp i on j.kode_lokasi= i.kode_lokasi and i.kode_pp = j.kode_pp 
				 left outer join karyawan g on g.jabatan = 'GENERAL MANAGER' and g.kode_lokasi = a.kode_lokasi  
				 left outer join karyawan h on h.jabatan like 'MANAGER%' and h.kode_lokasi = a.kode_lokasi and substring(h.kode_pp,1,4) = substring(i.kode_pp, 1, 4) ".$this->filter.				
				" and a.no_ifptg='".$page->fields[0]."' order by a.no_ifptg ";
			...
			$html.=	"<table width='700' align='center' cellpadding='0' cellspacing='0'>
					...
					//	$html .= " <td colspan='2' style='border:1px solid #111111; border-bottom-width:0px; border-right-width:0px;'>GM</td>";
					//else {
						if ($rs->tot >= 5000000)
					...
					//if ($rs->kode_pp == "00080000")
					//	$html .= "<td colspan='2' style='border:1px solid #111111; border-bottom-width:0px; border-right-width:0px;'>&nbsp;</td>";
					//else {
						if ($rs->tot >= 5000000)
					...
					//if ($rs->kode_pp == "00080000")
					//	$html .= "<td colspan='2' width='25%' style='border:1px solid #111111; border-right-width:0px;'>&nbsp;$rs->gm</td>";
					//else{
						if ($rs->tot >= 5000000)
