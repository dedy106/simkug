...
class server_report_kopeg_rptSPBT2
{
	protected $caption;
	protected $filter;
	protected $rows;
	...
	function getTotalPage()
	{
		...
		$sql = "select count(a.no_spb) ".
			"from spb_m a inner join kop_pinj_spb b on b.no_spbtitip = a.no_spb and b.kode_lokasi = a.kode_lokasi
				inner join kop_pinj_m c on c.no_pinj = b.no_pinj and a.kode_lokasi=c.kode_lokasi and a.modul='KP.SPB' and a.jenis='PINJ_TK' and a.no_del='-' 
	...
	function getHtml()
	{
		$sql0="select distinct a.no_spb,d.nama ".
              "from spb_m a 
				inner join kop_pinj_spb b on b.no_spbtitip = a.no_spb and b.kode_lokasi = a.kode_lokasi
				inner join kop_pinj_m c on c.no_pinj = b.no_pinj and a.kode_lokasi=c.kode_lokasi and a.modul='KP.SPB' and a.jenis='PINJ_TK' and a.no_del='-' 
				inner join kop_agg d on c.kode_agg=d.kode_agg and c.kode_lokasi=d.kode_lokasi 
				inner join kop_loker e on d.kode_loker=e.kode_loker and d.kode_lokasi=e.kode_lokasi ".$this->filter;
		...
		while (!$page->EOF)
		{
			$sql = "select distinct a.no_spb,date_format(a.tanggal,'%e/%m/%Y') as tgl,upper(d.nama) as nsbh,upper(d.alamat) as almt,d.no_rek, 
				a.keterangan,upper(d.bank) as bank,upper(d.cabang) as cabang,upper(d.nama_rek) as nmrek,a.nilai as nilai,a.nilai_pot,a.nilai as tot 
				, j.nama as nama_buat,e.nama as nama_setuju, d.nama as nama_agg
				...
				" and a.no_spb='".$page->fields[0]."' and d.nama='".$page->fields[1]."' order by a.no_spb ";
