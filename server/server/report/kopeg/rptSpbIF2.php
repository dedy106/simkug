...
class server_report_kopeg_rptSpbIF2
{
	protected $caption;
	protected $filter;
	protected $rows;
	...
	function getHtml()
	{
		...
		while (!$page->EOF)
		{
			$sql = "select a.no_ifptg as spb,a.no_dokumen,a.keterangan as ket1,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.nilai as tot,
				b.kode_akun,c.nama as nmakun,b.keterangan as ket2,b.nilai,a.nik_setuju,upper(e.nama) as nmstju,b.dc,
				e.jabatan as jab1,a.nik_buat,upper(d.nama) as nmbuat,d.jabatan as jab2,xx.kode_pp,f.nama as nmpp,b.kode_pp as pp,
				0 as sisa,0 as anggthn,0 as anggbln
				from ifptg_m a inner join ifptg_j b on a.no_ifptg=b.no_ifptg and a.kode_lokasi=b.kode_lokasi
				...
				inner join if_m xx on xx.no_if = a.no_if and a.kode_lokasi=xx.kode_lokasi 
				left join pp f on xx.kode_pp=f.kode_pp and xx.kode_lokasi=f.kode_lokasi 
				".$this->filter.
				" and a.no_ifptg='".$page->fields[0]."' order by a.no_ifptg,b.no_urut ";
			...
			$html.=	"<table width='750' border='0' align='center' cellpadding='0' cellspacing='0'>
					  <tr>
					    <td class='istyle12' align='center' style='padding-bottom:4px;'>LAMPIRAN SPB</td>
					  </tr>
					  <tr>
					      ...
					      <tr>
					        <td height='149'><table width='100%' border='0' cellspacing='0' cellpadding='3'>
					          ...
					          <tr>
					            ...
					            <td class='istyle14'>Nilai SPB </td>
					            <td class='istyle14'>:</td>
					...
					while ($dt = $data->FetchNextObject($toupper=false))
					{
					  ...
					  <tr>
					    <td class='istyle14' style='padding-top:4px;'>&nbsp;&nbsp;&nbsp;&nbsp;Nilai SPB : ".$AddOnLib->terbilang($rs->tot)."</td>
					  </tr>
					  <tr>
					    <td>&nbsp;</td>
					  </tr>
