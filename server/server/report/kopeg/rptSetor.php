...
class server_report_kopeg_rptSetor
{
	protected $caption;
	protected $filter;
	protected $rows;
	...
	function getTotalPage()
	{
		...
		$sql = "select count(distinct a.no_setor) 
			from kop_pinjsetor_m a inner join kop_pinjsetor_d b on a.no_setor=b.no_setor and a.kode_lokasi=b.kode_lokasi ".$this->filter;
	...
	function getHtml()
	{
		...
		while (!$page->EOF)
		{
			$sql = "select a.no_setor,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.keterangan,a.nilai,c.no_angs,
				...
				from kop_pinjsetor_m a inner join kop_pinjsetor_d b on a.no_setor=b.no_setor and a.kode_lokasi=b.kode_lokasi
					inner join kop_pinjangs_m c on b.no_angs=c.no_angs and b.kode_lokasi=c.kode_lokasi
