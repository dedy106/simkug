...
class server_report_kopeg_rptSPBA2
{
	protected $caption;
	protected $filter;
	protected $rows;
	...
	function getTotalPage()
	{
		...
		$sql = "select count(a.no_spb) ".
			"from spb_m a inner join kop_pinj_m c on a.no_spb=c.no_spbasur and a.kode_lokasi=c.kode_lokasi and a.modul='KP.SPB' and a.jenis='PINJ_AS' and a.no_del='-' 
	...
	function getHtml()
	{
		...
		while (!$page->EOF)
		{
			$sql = "select distinct a.no_spb,date_format(a.tanggal,'%e/%m/%Y') as tgl,upper(d.nama) as nsbh,upper(d.alamat) as almt,d.no_rek, 
				a.keterangan,upper(d.bank) as bank,upper(d.cabang) as cabang,upper(d.nama_rek) as nmrek,c.nilai_asur as nilai,a.nilai_pot,c.nilai_asur as tot 
				from spb_m a inner join kop_pinj_m c on a.no_spb=c.no_spbasur and a.kode_lokasi=c.kode_lokasi and a.modul='KP.SPB' and a.jenis='PINJ_AS' and a.no_del='-' 
				inner join kop_agg d on c.kode_agg=d.kode_agg and c.kode_lokasi=d.kode_lokasi 
				inner join kop_loker e on d.kode_loker=e.kode_loker and d.kode_lokasi=e.kode_lokasi ".$this->filter.
					      ...
					      <tr>
					        <td height='127' colspan='6' align='right' style='padding:0px;' valign='top'><table width='98%' border='0' cellspacing='0' cellpadding='3'>
					          ...
					          <tr align='center' class='istyle15'>
					            ...
					            <td style='border:1px solid #111111; border-bottom-width:0px; border-right-width:0px;'>Man UNIT UPKP</td>
					          ...
					          <tr align='center' class='istyle15'>
					            ...
					            <td width='25%' style='border:1px solid #111111; border-right-width:0px;'>&nbsp;</td>
					            <td width='25%' style='border:1px solid #111111; border-right-width:0px;'>&nbsp;</td>
					            <td style='border:1px solid #111111; border-right-width:0px;'>&nbsp;</td>
					          </tr>
					        </table></td>
					      </tr>
					      <tr>
