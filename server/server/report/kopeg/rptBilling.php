...
class server_report_kopeg_rptBilling
{
	protected $caption;
	protected $filter;
	protected $rows;
	...
	{
		global $dbLib;
		$sql = "select count(distinct b.no_pinj) 
	...
	function getHtml()
	{
		$sql0="select distinct b.no_pinj 
			from kop_pinjbill_m a inner join kop_pinjbill_d b on a.no_bill=b.no_bill and a.kode_lokasi=b.kode_lokasi 
			inner join kop_pinj_m c on b.no_pinj=c.no_pinj and b.no_kontrak=c.no_kontrak and b.kode_lokasi=c.kode_lokasi 
			inner join kop_agg d on c.kode_agg=d.kode_agg and c.kode_lokasi=d.kode_lokasi
			inner join kop_loker e on d.kode_loker=e.kode_loker and d.kode_lokasi=e.kode_lokasi ".$this->filter;
		...
		while (!$page->EOF)
		{
			$sql = "select concat(b.no_pinj,' - ',b.no_kontrak,' / ',a.periode) as nobuk,c.kode_agg,upper(d.nama) as nm,
				...
				left join (select aa.kode_lokasi,bb.no_pinj,bb.no_kontrak,sum((ifnull(bb.npokok,0)+ifnull(bb.nbunga,0))-(ifnull(cc.npokok,0)+ifnull(cc.nbunga,0))) as utang
					from kop_pinjbill_m aa inner join kop_pinjbill_d bb on aa.no_bill=bb.no_bill and aa.kode_lokasi=bb.kode_lokasi
					left join kop_pinjangs_d cc on bb.no_pinj=cc.no_pinj and bb.no_kontrak=cc.no_kontrak and bb.kode_lokasi=cc.kode_lokasi
					where aa.periode<'".$filter2[0]."' group by bb.no_pinj) f on b.no_pinj=f.no_pinj and b.no_kontrak=f.no_kontrak and b.kode_lokasi=f.kode_lokasi ".$this->filter.
				" and b.no_pinj='".$page->fields[0]."' order by c.no_pinj ";
