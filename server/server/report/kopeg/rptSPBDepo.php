...
class server_report_kopeg_rptSPBDepo
{
	protected $caption;
	protected $filter;
	protected $rows;
	...
	function getTotalPage()
	{
		...
		$sql = "select count(distinct a.no_spb) ".
			"from spb_m a ".$this->filter2." and a.modul='KP.SPB' and a.jenis='SIMPDEPO' and a.no_del='-' ";
	...
	function getHtml()
	{
		...
		while (!$page->EOF)
		{
			$sql = "select distinct a.no_spb,date_format(a.tanggal,'%e/%m/%Y') as tgl,upper(c.nama) as nsbh,c.alamat,c.no_rek, ".
				"a.keterangan,c.bank,c.cabang,c.nama_rek,(a.nilai+a.nilai_pot) as nilai,a.nilai_pot,a.nilai as tot ".
				",j.nama as nama_buat, case when (a.nilai) >= 5000000 then g.nama else '-' end  as gm, h.nama as setuju,  i.initial as nmunit "
				"from spb_m a inner join kop_depo b on a.no_spb=b.no_depo and a.kode_lokasi=b.kode_lokasi and a.modul='KP.SPB' and a.jenis='SIMPDEPO' and a.no_del='-' ".
					      ...
					      <tr>
					        <td><table width='100%' border='0' cellspacing='0' cellpadding='2' style='padding-left:4px;'>
					          ...
					          <tr>
					            ...
					            <td>:</td>
					            <td>".$rs->bank."</td>
					            <td colspan='2' rowspan='3' valign='top' style='border:1px solid #111111; border-right-width:0px;'>Nanager Keuangan</td>
					            </tr>
					          <tr>
					            <td>Alamat Bank </td>
					            <td>:</td>
