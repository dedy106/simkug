...
class server_report_anggaran_rptAggTwRka
{
	protected $caption;
	protected $filter;
	protected $filter2;
	...
	function getSumPage()
	{
		...
		$tmp=explode("/",$this->filter2);
		$nik_user=$tmp[0];
		$sql = "select count(kode_akun) as jum,sum(n1) as n1,sum(n2) as n2,sum(n3) as n3,sum(n4) as n4,sum(n1+n2+n3+n4) as n5 from glma_drk_tmp where nik_user='$nik_user' ".$this->filter".$this->filter." order by kode_akun,kode_pp,kode_drk ";
		$rs = $dbLib->execute($sql);		
		$totPage = 0;
		if ($rs)
		{
