...
class server_report_budget_rptCf
{
	protected $caption;
	protected $filter;
	protected $filter2;
	...
	function getHtml()
	{
		...
		$nik_user=$tmp[0];
		$bentuk=$tmp[2];
		//$sql = "select kode_cf,nama_cf,n1,level_spasi from agg_cf_lap where nik_user='$nik_user' order by rowindex ";
		$sql = "select kode_cf,nama_cf,n1,level_spasi from agg_cf_lap order by rowindex ";
