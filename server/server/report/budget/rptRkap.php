...
class server_report_budget_rptRkap
{
	protected $caption;
	protected $filter;
	protected $filter2;
  ...
  <tr bgcolor='#CCCCCC'>
	...
	<td width='60' class='header_laporan' align='center'>GROWTH RKA.11 VS OUTLOOK.10</td>
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			if ($row->n7 > 110)
			{
				$warna="style='color:#FF0000' 0";
			}
			if ($row->n6 > 110)
			{
				$warna2="style='color:#FF0000' 0";
			}
	...
	left join (select a.kode_lokasi,c.kode_bidang,a.kode_akun,isnull(sum(a.nilai),0) as usulan
				from agg_d a
				inner join agg_relakungar b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
				inner join agg_pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi
				where (a.kode_lokasi between '$kode_lokasi1' and '$kode_lokasi2') and a.tahun='$tahun' and b.kode_neraca='$row->kode_neraca' $program and (c.kode_bidang between '$kode_bidang1' and '$kode_bidang2')
