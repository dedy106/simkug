...
class server_report_budget_rptJurnal3
{
	protected $caption;
	protected $filter;
	protected $filter2;
	...
	function getTotalPage()
	{
		...
		$filter=$tmp[0];
		$urut=$tmp[1];
		$sql="select count(a.no_bukti) from agg_gldt a 
inner join agg_masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi $this->filter";
	...
	function getHtml()
	{
		...
		global $dbLib;
		$sql="select a.no_bukti,b.nama,a.periode,a.kode_akun,a.kode_cf,a.keterangan,
case when a.dc='D' then a.nilai else 0 end as debet,case when a.dc='C' then a.nilai else 0 end as kredit 
from agg_gldt a 
inner join agg_masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi $this->filter order by a.periode,a.no_bukti,a.dc desc";
		error_log($sql);	
  ...
  <tr bgcolor='#CCCCCC'>
    ...
    <td width='70' class='header_laporan'><div align='center'>No Bukti</div></td>
	 <td width='50' class='header_laporan'><div align='center'>Periode</div></td>
    ...
    <td height='20' valign='middle' class='isi_laporan'>$row->periode</td>
    <td valign='middle' class='isi_laporan'>$row->kode_akun</td>
    <td valign='middle' class='isi_laporan'>$row->nama</td>
    <td valign='middle' class='isi_laporan'>$row->kode_cf</td>
    ...
    <td valign='middle' class='isi_laporan'><div align='right' >".number_format($row->kredit,0,',','.')."</div></td>
		...
		echo "<tr>
    <td height='20' colspan='7' valign='middle' class='isi_laporan'><div align='right'><strong>Total</strong></div></td>
