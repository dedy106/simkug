...
class server_report_saku2_aka_rptAkSaldoMhsTrail2 extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$periode=$tmp[1];
		$kode_jur=$tmp[2];
		$sql="select a.nim,a.nama
	   ,isnull(b.bpp,0)-isnull(d.bpp,0) as so_bpp,isnull(b.sdp2,0)-isnull(d.sdp2,0) as so_sdp2,isnull(b.up3,0)-isnull(d.up3,0) as so_up3,isnull(b.sks,0)-isnull(d.sks,0) as so_sks,
	   isnull(b.bpp,0)-isnull(d.bpp,0) + isnull(b.sdp2,0)-isnull(d.sdp2,0) + isnull(b.up3,0)-isnull(d.up3,0) + isnull(b.sks,0)-isnull(d.sks,0) as so_total,	
	   isnull(c.bpp,0) as debet_bpp,isnull(c.sdp2,0) as debet_sdp2,isnull(c.up3,0) as debet_up3,isnull(c.sks,0) as debet_sks,
	   isnull(c.bpp,0) + isnull(c.sdp2,0) + isnull(c.up3,0) +  isnull(c.sks,0)  as debet_total,
...
left join (select y.nim,y.kode_lokasi,  
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode < '$periode') and y.kode_jur='$kode_jur'	
...
left join (select y.nim,y.kode_lokasi,  
					sum(case when x.kode_produk in ('BPP','S2BPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as bpp, 
				  sum(case when x.kode_produk in ('SDP2','S2SDP2') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as sdp2, 
				  sum(case when x.kode_produk in ('UP3','S2UP3') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as up3,
				  sum(case when x.kode_produk in ('SKS','S2SKS') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as sks,
...
where a.kode_lokasi='$kode_lokasi' and a.kode_jur='$kode_jur'  
order by a.nim
 ";
  ...
  <tr bgcolor='#CCCCCC'>
   ...
   <td width='90' align='center' class='header_laporan'>Total</td>
		...
		$sa_bpp=0;$sa_sdp2=0;$sa_up3=0;$sa_sks=0;$sa_s2=0;$sa_total=0;
		$sak_bpp=0;$sak_sdp2=0;$sak_up3=0;$sak_sks=0;$sak_s2=0;$sak_total=0;
		$so_bpp=0;$so_sdp2=0;$so_up3=0;$so_sks=0;$so_s2=0;$so_total=0;
