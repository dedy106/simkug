...
class server_report_saku2_aka_rptAkSaldoMhsTrail extends server_report_basic
{	
...
left join (select y.nim,  sum(case x.kode_produk when 'BPP' then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as bpp, 
				  sum(case x.kode_produk when 'SDP2' then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as sdp2, 
				  sum(case x.kode_produk when 'UP3' then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as up3,sum(nilai) as total 			
			from aka_rekon_d x 			
			inner join aka_mahasiswa y on x.nim=y.nim and x.kode_lokasi=y.kode_lokasi 
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode <'$periode')	and y.kode_jur='$kode_jur'	
			group by y.nim 			
...
where a.kode_lokasi='$kode_lokasi' and a.kode_jur='$kode_jur' 
	  and (isnull(b.bpp,0)-isnull(d.bpp,0) + isnull(b.sdp2,0)-isnull(d.sdp2,0) + isnull(b.up3,0)-isnull(d.up3,0) <>0 or
		  isnull(c.bpp,0) + isnull(c.sdp2,0) + isnull(c.up3,0) <>0  or 
		  isnull(e.bpp,0) + isnull(e.sdp2,0)+ isnull(e.up3,0) <>0 )  
order by a.nim
 ";
