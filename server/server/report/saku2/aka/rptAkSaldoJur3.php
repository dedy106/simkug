...
class server_report_saku2_aka_rptAkSaldoJur3 extends server_report_basic
{	
...
left join (select y.kode_jur,y.kode_lokasi, 	
				   sum(case when x.kode_produk in ('BPP','S2BPP','D3BPP') then x.nilai else 0 end) as bpp, 
				   sum(case when x.kode_produk in ('SDP2','S2SDP2','D2SDP2') then x.nilai else 0 end) as sdp2, 
...
left join (select y.kode_jur,y.kode_lokasi, 
				  sum(case when x.kode_produk in ('BPP','S2BPP','D3BPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as bpp, 
			      sum(case when x.kode_produk in ('SDP2','S2SDP2','D2SDP2') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as sdp2, 
