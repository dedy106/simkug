...
class server_report_saku2_gaji_rptGajiInsPol extends server_report_basic
{
  ...
  <tr>
    <td align='center'  class='style16'>DAFTAR PEMBAYARAN INSENTIF PENDIDIKAN PEGAWAI YAYASAN PENDIDIKAN TELKOM DI LINGKUNGAN POLITEKNIK TELKOM</td>
  </tr>
  <tr>
  ...
  <tr>
    <td><table border='1' cellspacing='0' cellpadding='0' class='kotak' >
      ...
      <tr align='center' bgcolor='#CCCCCC'>
        ...
        <td width='80' class='header_laporan'>% INSENTIF PENDIDIKAN</td>
        <td width='80' class='header_laporan'>JUMLAH INSENTIF PENDIDIKAN</td>
	   ...
	   g.tanggal,g.tgl_transfer,isnull(f.idx_didik,0) as idx_didik
from hr_karyawan a
...
order by a.nik";
		$i=1;
		$rs = $dbLib->execute($sql);
		$gdas=0; $tdas=0; $tpos=0; $rapl=0; $tdpl=0; $tpph=0; $pdpt=0; $pdpl=0; $pgaj=0; $ppph=0; $pot3=0; $zmiw=0; $potongan=0; $total=0; $pjms=0; $tjms=0;$tot_tipd=0;$thp=0;
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			$tipd=($row->gdas + $row->tdas + $row->tpos)* ($row->idx_didik/100);
			$tot_tipd+=$tipd;
			$thp+=$tipd+$row->tpph-$row->potongan;
			$pdpt+=$tipd+$row->tpph;
      echo "<tr>
       ...
       <td class='isi_laporan' >$row->nama_grade</td>
        ...
        <td class='isi_laporan' align='right'>".number_format($row->idx_didik,2,',','.')."%</td>
        <td class='isi_laporan' align='right'>".number_format($tipd,0,',','.')."</td>
	   <td class='isi_laporan' align='right'>".number_format($row->tpph,0,',','.')."</td>
        <td class='isi_laporan' align='right'>".number_format($tipd+$row->tpph,0,',','.')."</td>
       ...
       <td class='isi_laporan' align='right'>".number_format($row->potongan,0,',','.')."</td>
	    <td class='isi_laporan' align='right'>".number_format($tipd+$row->tpph-$row->potongan,0,',','.')."</td>
      </tr>";
        ...
        <td class='header_laporan' align='right' >&nbsp;</td>
      	<td class='header_laporan' align='right'>".number_format($tot_tipd,0,',','.')."</td>
