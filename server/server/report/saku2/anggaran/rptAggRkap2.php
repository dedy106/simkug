...
class server_report_saku2_anggaran_rptAggRkap2 extends server_report_basic
{	
...
from agg_neraca_tmp where nik_user='$nik_user' order by rowindex ";
  ...
  <tr bgcolor='#CCCCCC'>
	...
	<td width='80' class='header_laporan' align='center'>USULAN RKA $tahun PROGRAM FIXED</td>
	<td width='80' class='header_laporan' align='center'>USULAN RKA $tahun PROGRAM VARIABEL</td>
	<td width='80' class='header_laporan' align='center'>TOTAL USULAN RKA $tahun</td>
	<td width='60' class='header_laporan' align='center'>GROWTH PROGRAM FIXED $tahun VS OUTLOOK $tahun_rev</td>
			...
			*/
			if ($bentuk=="Detail" && $row->tipe=="Posting")
			{
				...
				if ($jenis=="Akun" || $jenis=="")
				{
					...
					$sql1="select a.kode_lokasi, a.kode_akun as kode_akunlok,a.nama,isnull(d.rka,0) as rka,isnull(d.realisasi,0) as realisasi,isnull(d.outlook,0) as outlook,
	...
	from masakun a
	inner join relakungar e on a.kode_akun=e.kode_akun and a.kode_lokasi=e.kode_lokasi 
	left join (select a.kode_akun,isnull(sum(a.rka),0) as rka,isnull(sum(a.realisasi),0) as realisasi,isnull(sum(a.outlook),0) as outlook,sum(estimasi) as estimasi
				from agg_outlook a
				inner join relakungar b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi 
	...
	left join (select a.kode_akun,sum(case when a.modul in ('BTPKK','ASSET','BGAJI','BPMBN','BPP','BP','BO') then a.nilai else 0 end) as lama,
					sum(case when a.modul in ('BLEMBUR','BSPPD','ABAU','INV','BTPKU','CC','PKUNJ') then a.nilai else 0 end) as baru,
	...
	left join (select a.kode_lokasi,a.kode_akun,sum(case when a.modul in ('BTPKK','ASSET','BGAJI','BPMBN','BPP','BP','BO') then a.nilai else 0 end) as baru,
					sum(case when a.modul in ('BLEMBUR','BSPPD','ABAU','INV','BTPKU','CC','PKUNJ') then a.nilai else 0 end) as lama,
					sum(a.nilai) as usulan
				from agg_d a
				inner join relakungar b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
				inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi
	...
	left join (select a.kode_lokasi,c.kode_bidang,a.kode_akun,sum(case when a.modul in ('BTPKK','ASSET','BGAJI','BPMBN','BO','BPP') then a.nilai else 0 end) as baru,
					sum(case when a.modul in ('BLEMBUR','BSPPD','ABAU','INV','BTPKU','BP','CC','PKUNJ') then a.nilai else 0 end) as lama,
					sum(a.nilai) as usulan
				from agg_d a
				inner join relakungar b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
				inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi
	...
	left join (select a.kode_akun,a.kode_drk,sum(case when a.modul in ('BTPKK','ASSET','BGAJI','BPMBN','BPP') then a.nilai else 0 end) as baru,
					sum(case when a.modul in ('BO','BLEMBUR','BSPPD','ABAU','INV','BTPKU','BP','CC','PKUNJ') then a.nilai else 0 end) as lama,
					sum(a.nilai) as usulan
				from agg_d a
				inner join relakungar b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
				inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi
	...
	<td class='$css' align='left'>$row1->nama</td>
	<td height='20' class='$css' class='isi_laporan' align='right'>";
	echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenRka('$row1->kode_akunlok','$row1->kode_lokasi','$periode_out','rka','$kode_bidang1','$kode_bidang2');\">$n0</a>";
	echo "</td>
	<td height='20' class='$css' class='isi_laporan' align='right'>";
	echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenRka('$row1->kode_akunlok','$row1->kode_lokasi','$periode_out','realisasi','$kode_bidang1','$kode_bidang2');\">$n1</a>";
	echo "</td>
	<td height='20' class='$css' class='isi_laporan' align='right'>";
	echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenRka('$row1->kode_akunlok','$row1->kode_lokasi','$periode_out','estimasi','$kode_bidang1','$kode_bidang2');\">$n10</a>";
	echo "</td>
	<td height='20' class='$css' class='isi_laporan' align='right'>";
	echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenRka('$row1->kode_akunlok','$row1->kode_lokasi','$periode_out','outlook','$kode_bidang1','$kode_bidang2');\">$n2</a>";
	echo "</td>
	<td class='$css' align='right'>$n3</td>
	<td class='$css' align='right'>$n4</td>
	
	<td height='20' class='$css' class='isi_laporan' align='right'>";
	echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenAkun('$row1->kode_akunlok','$row1->kode_lokasi','$tahun','fix','$kode_bidang1','$kode_bidang2');\">$n5</a>";
	echo "</td>
	<td height='20' class='$css' class='isi_laporan' align='right'>";
	echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenAkun('$row1->kode_akunlok','$row1->kode_lokasi','$tahun','var','$kode_bidang1','$kode_bidang2');\">$n6</a>";
	echo "</td>
	<td class='$css' align='right'>$n7</td>
	<td class='$css' align='right' $warna2>$n8</td>
