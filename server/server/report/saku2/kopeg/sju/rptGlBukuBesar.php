...
class server_report_saku2_kopeg_sju_rptGlBukuBesar extends server_report_basic
{
	...
	function getHtml()
	{
		...
		echo $AddOnLib->judul_laporan("Accumulative General Ledger",$this->lokasi,"Periode ".$AddOnLib->ubah_periode($periode));
  ...
  <tr align='center' bgcolor='#dbeef3'>
    ...
    <td width='90' class='header_laporan'>Net Change</td>
			...
			echo " <tr>
    <td colspan='3'>&nbsp;</td>
    <td colspan='1' class='header_laporan'>Opening Balance</td>
    ...
    <td>&nbsp;</td>
	 ...
	 $sql1="select a.kode_akun,b.nama as nama_akun,a.nilai,a.keterangan,isnull(a.debet,0) as debet,isnull(a.kredit,0) as kredit,a.kode_curr,a.tgl,a.no_bukti,a.periode
					  from (select kode_lokasi,no_bukti,tanggal,kode_akun,dc,nilai,keterangan,periode,case when dc='D' then nilai else 0 end as debet,case when dc='C' then nilai else 0 end as kredit,kode_curr,date_format(tanggal,'%d/%m/%Y') as tgl
							from gldt_h
							where periode='$row->periode' and kode_lokasi='$row->kode_lokasi' and kode_akun='$row->kode_akun'
							union
							select kode_lokasi,no_bukti,tanggal,kode_akun,dc,nilai,keterangan,periode,case when dc='D' then nilai else 0 end as debet,case when dc='C' then nilai else 0 end as kredit,kode_curr,date_format(tanggal,'%d/%m/%Y') as tgl
							from gldt
							where periode='$row->periode' and kode_lokasi='$row->kode_lokasi' and kode_akun='$row->kode_akun'
							)a 
	  ...
	  echo " <tr>
    <td colspan='4' align='right' class='header_laporan'>Total Month</td>
  <td class='header_laporan' align='right'>".number_format($tot_debet,2,',','.')."</td>
	...
	<td class='header_laporan' align='right'>&nbsp;</td>
	  ...
	  echo " <tr>
    <td colspan='4' align='right' class='header_laporan'>Transaction</td>
  <td class='header_laporan' align='right'>".number_format($tot_debet,2,',','.')."</td>
	<td class='header_laporan' align='right'>".number_format($tot_kredit,2,',','.')."</td>
	<td class='header_laporan' align='right'>".number_format($tot_debet-$tot_kredit,2,',','.')."</td>
  </tr>";
	  echo " <tr>
    <td colspan='4' align='right' class='header_laporan'>Ending Balance</td>
