...
class server_report_saku2_kopeg_sju_rptPrRekapCustKlp extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$kode_lokasi=$tmp[0];
		$tahun=$tmp[1];
			$sql="select c.kode_klp,a.kode_lokasi,c.nama,sum(isnull(b.n01,0)) as n01,sum(isnull(b.n02,0)) as n02,sum(isnull(b.n03,0)) as n03,sum(isnull(b.n04,0)) as n04,
	   sum(isnull(b.n05,0)) as n05,sum(isnull(b.n06,0)) as n06,sum(isnull(b.n07,0)) as n07,sum(isnull(b.n08,0)) as n08,sum(isnull(b.n09,0)) as n09,
	   sum(isnull(b.n10,0)) as n10,sum(isnull(b.n11,0)) as n11,sum(isnull(b.n12,0)) as n12,sum(isnull(b.total,0)) as total
from sju_cust a
...
group by c.kode_klp,a.kode_lokasi,c.nama
order by c.kode_klp ";
   ...
   <tr bgcolor='#CCCCCC'>
     ...
     <td width='40'  align='center' class='header_laporan'>Kode</td>
	  <td width='200'  align='center' class='header_laporan'>Jenis Tertanggung</td>
     ...
     <td class='isi_laporan' align='center'>$i</td>";
      echo "<td class='isi_laporan'><a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenPolisList('$row->kode_klp','$row->kode_lokasi');\">$row->kode_klp</a></td>";
