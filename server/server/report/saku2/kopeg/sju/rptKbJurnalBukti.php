...
class server_report_saku2_kopeg_sju_rptKbJurnalBukti extends server_report_basic
{
	...
	function getHtml()
	{
		...
		global $dbLib;
		$sql="select a.no_kas,a.tanggal,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.no_dokumen,a.kode_lokasi,a.periode,b.nama as nama_lokasi 
				from kas_m a
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			if (substr($row->no_kas,0,2)=="MI") { $judul="BUKTI PEMBUKUAN MEMORIAL";}
			if (substr($row->no_kas,0,2)=="BT") { $judul="BUKTI PEMBUKUAN BANK TERIMA";}
			if (substr($row->no_kas,0,2)=="BK") { $judul="BUKTI PEMBUKUAN BANK KELUAR";}
			if (substr($row->no_kas,0,2)=="KT") { $judul="BUKTI PEMBUKUAN KAS TERIMA";}
			if (substr($row->no_kas,0,2)=="KK") { $judul="BUKTI PEMBUKUAN KAS KELUAR";}
  ...
  <tr>
    <td><table width='800' border='1' cellspacing='0' cellpadding='0' class='kotak'>
	 ...
	 $sql1="select a.kode_akun,b.nama as nama_akun,a.nilai,a.keterangan,isnull(a.debet,0) as debet,isnull(a.kredit,0) as kredit,a.kode_curr,a.no_dokumen
					  from (select kode_lokasi,kode_akun,dc,nilai,keterangan,no_dokumen,case when dc='D' then nilai else 0 end as debet,case when dc='C' then nilai else 0 end as kredit,kode_curr
							from kas_j
