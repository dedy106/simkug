...
class server_report_saku2_kopeg_sju_rptGlNeracaKonsol extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="exec sp_neraca_konsol_sju 'FS1','N','K','1','$periode','$periode_rev','$kode_lokasi','$kode_lokasi','$kode_lokasi2','$nik_user' ";
		$rs = $dbLib->execute($sql);		
  ...
  <tr align='center' bgcolor='#dbeef3'>
   ...
   <tr align='center' bgcolor='#dbeef3'>
    ...
    <td width='50' class='header_laporan'>(2/3)</td>
     <td width='250' class='header_laporan'>(1)</td>
    <td width='90' class='header_laporan'>(2)</td>
    <td width='90' class='header_laporan'>(3)</td>
    <td width='50' class='header_laporan'>(2/3)</td>
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			if ($row->tipe1!="Header" && $row->nama1!="." )
			{
				...
				if ($row->nilai3 > 0)
				{
					$persen1=($row->nilai2 *100 )/ $row->nilai3;
				}
			}
			...
			{ 
				...
				if ($row->nilai6 > 0)
				{
					$persen2=($row->nilai5 *100 )/ $row->nilai6;
				}
			}
			echo "<tr>
   ...
   <td class='isi_laporan'>";
			...
			echo "</td>
    <td valign='top' class='isi_laporan' align='right'>";
			if ($row->tipe1=="Posting" && $row->nilai5 <> 0)
			{
			...
			echo "</td>
	  <td valign='top' class='isi_laporan' align='right'>";
			if ($row->tipe1=="Posting" && $row->nilai6 <> 0)
			{
