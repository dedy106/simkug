...
class server_report_saku2_kopeg_sju_rptGlNeracaRincianKonsolSimulasi extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="call sp_produk_konsol_sju_sim 'FS2','N','S','1','$periode','$periode_rev','$kode_lokasi','$kode_lokasi','$kode_lokasi2','$nik_user' ";
	...
	<td width='50' class='header_laporan'>(2/1)</td>
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			if ($row->tipe!="Header" && $row->nama!="." && $row->nama!="")
			{
				...
				if ($row->n5 > 0)
				{
					$persen1=(($row->n4+(-$row->n5)) / $row->n5)*100;
				}
				if ($row->n5 < 0)
				{
					$persen1=(($row->n4+(-$row->n5)) / -$row->n5)*100;
