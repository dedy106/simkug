...
class server_report_saku2_kopeg_sju_rptAggPdptCabangGroup extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="exec sp_pdpt_cab '02','02','201304','sju_123'";
		$rs = $dbLib->execute($sql);
		
		$sql = "select kode_klp,nama from sju_tipe_klp where kode_lokasi='02'";
  ...
  <tr>
    <td  class='lokasi_laporan2' align='center'>PENDAPATAN USAHA CABANG</td>
  </tr>
  <tr>
  ...
  <tr align='center' bgcolor='#dbeef3'>
    ...
    <td width='300' rowspan='3' class='header_laporan'>JENIS ASURANSI</td>
  ...
  <tr align='center' bgcolor='#dbeef3'>
    ...
    <td width='40' class='header_laporan'>3/4</td>
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			$sql = "select b.kode_klp,c.kode_group,d.nama,sum(n1) as n1,sum(n2) as n2,sum(n3) as n3,sum(n4) as n4 
from sju_pdpt_cab a
...
inner join sju_cust_group d on c.kode_group=d.kode_group and c.kode_lokasi=d.kode_lokasi
where a.nik_user='sju_123' and b.kode_klp='$row->kode_klp'
group by b.kode_klp,c.kode_group,d.nama
order by c.kode_group";
