...
class server_report_saku2_kopeg_sju_rptGlJurnalSimulasi extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="select a.no_kas as no_bukti,dbo.fnDokumen(a.no_kas) as no_dokumen,c.nama,date_format(b.tanggal,'%d/%m/%Y') as tanggal,a.nik_user,
			...
			d.nama as nama_dok
		from ju_m a 
		inner join kas_j b on a.no_kas=b.no_kas and a.kode_lokasi=b.kode_lokasi 
		inner join masakun c on b.kode_akun=c.kode_akun and b.kode_lokasi=c.kode_lokasi 
		inner join sju_dokumen d on dbo.fnDokumen(a.no_kas)=d.no_dokumen and a.kode_lokasi=d.kode_lokasi
		$this->filter
		order by a.no_kas ";
		$rs = $dbLib->execute($sql);		
		...
		echo $AddOnLib->judul_laporan("DAFTAR JURNAL KASBANK",$this->lokasi,$nama_periode);
  ...
  <tr bgcolor='#dbeef3'>
	...
	<td width='50' class='header_laporan' align='center'>Jenis Dok</td>
	<td width='150' class='header_laporan' align='center'>Nama Dok</td>
	...
	<td  class='isi_laporan'>$row->nama_dok</td>
