...
class server_report_saku2_kopeg_sju_rptPlacing extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$periode=$tmp[0];
		$jenis=$tmp[1];
		$sql="select a.slip, a.no_quo,g.kota,a.no_placing,
...
inner join sju_placing_m h on a.no_placing=h.no_placing and a.kode_lokasi=h.kode_lokasi
			$this->filter 
  ...
  <tr>
    <td align='center'>PLACING SLIP</td>
  </tr>
  <tr>
    <td align='center'>$row->no_placing</td>
  </tr>
  <tr>
    <td align='center'>";
			$slip = urldecode($row->slip);
			$rsD = $dbLib->execute("select kunci, isi from sju_placing_draft  where no_quo='$row->no_quo'");
