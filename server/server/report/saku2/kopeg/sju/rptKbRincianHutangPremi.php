...
class server_report_saku2_kopeg_sju_rptKbRincianHutangPremi extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$kode_lokasi=$tmp[0];
		$periode=$tmp[1];
		$sql="select distinct a.no_kas as no_bukti,a.keterangan,a.tanggal,a.kode_lokasi,date_format(a.tanggal,'%d/%m/%Y') as tgl,c.nama as nama_lokasi
		from kas_m a 
		inner join lokasi c on a.kode_lokasi=c.kode_lokasi
		left join sju_polisbayar_d d on a.no_kas=d.no_kashut and a.kode_lokasi=d.kode_lokasi
        $this->filter and a.modul='KBHUTPOL' 
		order by a.no_kas ";
...
d.premi+d.p_cost+d.materai-d.fee-d.diskon-d.ppn as hutang,a.kode_curr,a.kurs,b.no_dok2,d.ke
from sju_polisbayar_d a
inner join sju_polis_m b on a.no_polis=b.no_polis and a.kode_lokasi=b.kode_lokasi
