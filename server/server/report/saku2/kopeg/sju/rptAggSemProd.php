...
class server_report_saku2_kopeg_sju_rptAggSemProd extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql=" select a.kode_lokasi,a.kode_tipe,b.nama as nama_tipe,a.kode_pic,c.nama as nama_pic,a.kode_cust,d.nama as nama_cust,
         isnull(e.agg_01,0) as n1,isnull(e.agg_02,0) as n2,isnull(e.total,0) as total
  ...
  left join (select x.kode_lokasi,x.kode_tipe,kode_pic,kode_cust
	  	              , sum(case when substring(x.periode,5,2) between '01' and '06' then x.nilai else 0 end ) as agg_01
                    , sum(case when substring(x.periode,5,2) between '07' and '12' then x.nilai else 0 end ) as agg_02
