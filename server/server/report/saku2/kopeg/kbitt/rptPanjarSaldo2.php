...
class server_report_saku2_kopeg_kbitt_rptPanjarSaldo2 extends server_report_basic
{
	...
	function getHtml()
	{
	   ...
	   isnull(d.nilai_kas,0) as nilai_kas,isnull(d.no_kas,'-') as no_kas_ptg,isnull(d.nilai_ptg,0) as nilai_ptg,date_format(a.tanggal,'%d/%m/%Y') as tgl
from it_aju_m a
...
left join (select a.no_ptg as no_aju,a.no_aju+' | '+a.no_kas as no_kas,c.nilai-a.nilai as nilai_kas,a.nilai as nilai_ptg
			from it_aju_m a
			...
			union 
			select a.no_ptg as no_aju,a.no_aju as no_kas,0 as nilai_kas,c.nilai
			from it_aju_m a
...
order by c.periode,a.no_aju";
  ...
  <tr bgcolor='#CCCCCC'>
	...
	<td width='95'  align='center' class='header_laporan'>No PTG Panjar</td>
	...
	<td width='90'  align='center' class='header_laporan'>Pengembalian Kas</td>
	<td width='90'  align='center' class='header_laporan'>Saldo Panjar</td>
  </tr>
 ";
		$nilai=0; $tot_nilai_ptg=0; $tot_nilai_kas=0; $nilai_kas=0; $saldo=0; $selisih=0;
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			$nilai+=$row->nilai;
			$tot_nilai_kas+=$row->nilai_kas;
			$tot_nilai_ptg+=$row->nilai_ptg;
			$saldo+=$row->nilai-$nilai_ptg-$nilai_kas;
		echo "<tr>
    ...
    <td class='isi_laporan' align='right'>".number_format($row->nilai,0,",",".")."</td>
	  ...
	  <td class='isi_laporan' align='right'>".number_format($row->nilai-$nilai_ptg-$nilai_kas,0,",",".")."</td>
  </tr>";
			$i=$i+1;
		}
		echo "<tr>
	...
	<td class='header_laporan' align='right'>".number_format($nilai,0,",",".")."</td>
	 ...
	 <td class='header_laporan' align='right'>".number_format($tot_nilai_kas,0,",",".")."</td>
