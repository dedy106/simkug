...
class server_report_saku2_kopeg_kbitt_rptInvestasiPos extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$fullId = $_GET["fullId"];
		$jenis=$tmp[1];
		$nama_file="investasi_".$periode.".xls";
	   ...
	   a.no_spb,date_format(l.tanggal,'%d/%m/%Y') as tgl_spb,isnull(n.jum_dok,0) as jum_dok
from it_aju_m a
...
$this->filter order by a.no_aju";
		...
		echo $AddOnLib->judul_laporan("posisi pertanggungan investasi",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
