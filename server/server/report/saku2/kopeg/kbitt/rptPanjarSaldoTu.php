...
class server_report_saku2_kopeg_kbitt_rptPanjarSaldoTu extends server_report_basic
{
...
$this->filter";
	...
	function getHtml()
	{
		...
		$fullId = $_GET["fullId"];
		$sql="select a.kode_lokasi,a.no_aju,date_format(a.tanggal,'%d/%m/%Y') as tgl_panjar,a.keterangan,a.kode_pp,b.nama as nama_pp,a.nilai,
	   ...
	   a.nilai-isnull(i.nilai,0) as selisih,a.nilai-(isnull(i.nilai,0)+isnull(j.nilai,0)) as saldo
from it_aju_m a
inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi
...
$this->filter order by a.no_aju";
		...
		echo $AddOnLib->judul_laporan("laporan saldo panjar",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
   ...
   <td class='isi_laporan' align='right'>".number_format($row->nilai,0,",",".")."</td>
    <td class='isi_laporan'>";
		echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenPanjar('$row->no_ptg','$row->kode_lokasi');\">$row->no_ptg</a>";
		echo "</td>
	<td class='isi_laporan'>$row->no_verptg</td>
	  <td class='isi_laporan'>$row->no_fiatptg</td>
     <td class='isi_laporan'>";
		echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenKas('$row->no_kasptg','$row->kode_lokasi');\">$row->no_kasptg</a>";
		echo "</td>
