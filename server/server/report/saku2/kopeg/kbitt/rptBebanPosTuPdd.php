...
class server_report_saku2_kopeg_kbitt_rptBebanPosTuPdd extends server_report_basic
{
	...
	function getHtml()
	{
	   ...
	   c.kode_bank,a.kode_akun,h.nama as nama_akun,
	   ...
	   case when isnull(i.no_ju,'-')<>'-' then isnull(i.no_ju,'-') else isnull(m.no_ju,'-') end as no_ju,
	   case when isnull(i.no_ju,'-')<>'-' then isnull(i.no_ju,'-') else isnull(m.no_ju,'-') end as no_ju,
	   isnull(n.jum_dok,0) as jum_dok,
	   isnull(p.no_bukti,'-') as no_pdd_app, date_format(p.tgl_input,'%d/%m/%Y') as tgl_pdd_app, isnull(p.catatan,'-') as ctt_pdd_app
from it_aju_m a
...
left join ju_m m on l.no_spb=m.no_dokumen and l.kode_lokasi=m.kode_lokasi and m.modul='JUSPB' 
...
left join it_pdd_app p on a.no_aju=p.no_aju and a.kode_lokasi=p.kode_lokasi and p.no_ref='-'
...
// echo $sql;
		...
		echo "<table border='1' cellspacing='0' cellpadding='0' class='kotak' width='2750'>
   <tr bgcolor='#CCCCCC'>
   ...
   <td width='200'  align='center' class='header_laporan'>Keterangan</td>
	 <td width='90'  align='center' class='header_laporan'>Nilai Bruto</td>
	  ...
	  <td width='100'  align='center' class='header_laporan'>Catatan App</td>
	  <td width='90'  align='center' class='header_laporan'>No Dok</td>
	 <td width='60'  align='center' class='header_laporan'>Tgl Dok</td>
	 <td width='60'  align='center' class='header_laporan'>Tgl Aju</td>
	 ...
	 <td class='isi_laporan'>$row->no_pdd_app</td>
	 <td class='isi_laporan'>$row->tgl_pdd_app</td>
	 <td class='isi_laporan'>$row->ctt_pdd_app</td>
	 <td class='isi_laporan'>$row->no_app</td>
	 <td class='isi_laporan'>$row->tgl_app1</td>
	 <td class='isi_laporan'>$row->tgl_app</td>
	 <td class='isi_laporan'><a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenVer('$row->no_aju','$row->kode_pp','$periode','$row->kode_lokasi');\">$row->no_ver</a></td>
	 ...
	 <td class='isi_laporan'>$row->tgl_kas</td>
	  ...
	  <td class='header_laporan' align='center' colspan='15'>&nbsp;</td>
    </tr>";
		echo "</table><br>";
		echo "</div>";
		return "";
