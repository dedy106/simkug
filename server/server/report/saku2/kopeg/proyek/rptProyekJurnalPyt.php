...
class server_report_saku2_kopeg_proyek_rptProyekJurnalPyt extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="select a.no_pdd,a.no_dokumen,a.kode_lokasi,a.periode,a.tanggal,date_format(a.tanggal,'%d/%m/%Y') as tanggal1,a.keterangan,a.kode_lokasi,
...
left join karyawan c on a.nik_app=c.nik and a.kode_lokasi=c.kode_lokasi $this->filter ";
			  ...
			  <tr>
				<td><table border='0' cellspacing='2' cellpadding='1'>
				  ...
				  <tr>
					...
					<td width='496' class='header_laporan'>:&nbsp;$row->no_pdd</td>
					</tr>
				  <tr>
	  ...
	  $sql1="select a.kode_akun,b.nama,a.keterangan,a.kode_pp,a.kode_drk,case dc when 'D' then nilai else 0 end as debet,case dc when 'C' then nilai else 0 end as kredit  
			...
			where a.no_pdd='$row->no_pdd' and a.kode_lokasi='$row->kode_lokasi'
			order by a.dc desc ";
		//error_log($sql1);
