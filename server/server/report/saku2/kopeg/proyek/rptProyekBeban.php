...
class server_report_saku2_kopeg_proyek_rptProyekBeban extends server_report_basic
{
...
inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi $this->filter";
	...
	function getHtml()
	{
		...
		$periode=$tmp[0];
		$nama_cab=$tmp[1];
		$sql="select a.no_aju,a.no_proyek,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.kode_pp,b.nama as nama_pp,a.keterangan,a.nilai,c.keterangan,c.jenis
from pr_beban_d a
...
$this->filter order by a.no_aju";
		...
		echo $AddOnLib->judul_laporan("beban proyek",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
