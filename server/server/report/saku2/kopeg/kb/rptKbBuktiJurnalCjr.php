...
class server_report_saku2_kopeg_kb_rptKbBuktiJurnalCjr extends server_report_basic
{
...
left join karyawan c on a.nik_buat=c.nik and a.kode_lokasi=c.kode_lokasi $filter2 $modul";
	...
	function getHtml()
	{
		...
		$periode=$tmp[2];
		$filter2=$tmp[3];
		$sql="select a.no_kas,a.no_dokumen,a.kode_lokasi,a.periode,a.tanggal,date_format(a.tanggal,'%d/%m/%Y') as tanggal1,a.keterangan,a.ref1,a.kode_lokasi,
	   ...
	   f.nik as nik_fiat,f.nama as nama_fiat,isnull(e.kode_spro,'-') as kode_spro
from kas_m a
...
left join karyawan f on e.flag=f.nik and f.kode_lokasi=e.kode_lokasi $this->filter2 $modul";
    ...
    <td align='left'><table border='0' cellspacing='2' cellpadding='1'>
      ...
      <tr>
        ...
        <td width='120' class='header_laporan'>Dibuat Oleh : </td>
		 <td width='120' class='header_laporan'>Diterima Oleh : </td>
      </tr>
       <tr>
        <td height='40'>&nbsp;</td>
        <td>&nbsp;</td>
      ...
      <tr>
        ...
        <td class='header_laporan'>$row->nama_buat</td>
		 <td class='header_laporan'>$row->ref1</td>
      </tr>
      <tr>
