...
class server_report_saku2_kopeg_fa_rptFaApTahun extends server_report_basic
{
    ...
    <td width='100' align='center' class='header_laporan'>Nilai Buku</td>
  	...
  	";
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			$sql="select a.kode_lokasi,substring(b.periode,1,4) as tahun,sum(case when b.dc='D' then b.nilai else -b.nilai end) as ap
           from gl_fa_asset a
		   inner join fasusut_d b on a.no_fa=b.no_fa and a.kode_lokasi=b.kode_lokasi
           where a.kode_lokasi='$kode_lokasi' and b.periode<='$periode' and a.kode_klpakun='$row->kode_klpakun' and substring(a.periode,1,4)='$row->tahun'
           group by a.kode_lokasi,substring(b.periode,1,4) ";
			...
			while ($row1 = $rs1->FetchNextObject($toupper=false))
			{
				...
				echo "<tr>
				   <td class='isi_laporan' align='center' >&nbsp;</td>
				    <td class='isi_laporan' align='center' >&nbsp;</td>
				  <td class='isi_laporan' align='center' >Akumulasi Penyusutan</td>
				  <td class='isi_laporan' align='center'>$row1->tahun</td>
				  <td class='isi_laporan' align='right'>0</td>
				   <td class='isi_laporan' align='right'>".number_format($row1->ap,0,",",".")."</td>
