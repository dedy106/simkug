...
class server_report_saku2_kopeg_lab_rptNeraca extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="select a.no_tugas,a.keterangan,a.kode_matkul,a.kode_dosen,a.kode_lokasi,a.nama,a.kode_lokasi,
...
inner join lab_mhs f on e.nik_user=f.nim and e.kode_lokasi=f.kode_lokasi ";
  ...
  <tr>
    <td align='center' class='style16'>NERACA</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
  </tr>
  ...
  <tr>
    <td align='center'>";
		$sql="exec sp_lab_neraca '$row->kode_lokasi','N','K',1,'$no_tugas','$nik_user','$userid'";
		
		$rs1 = $dbLib->execute($sql);		
		$sql = "select kode_neraca1,kode_neraca2,nama1,tipe1,nilai1,level_spasi1,nama2,tipe2,nilai2,level_spasi2
				from lab_neraca_skontro where userid='$userid' order by rowindex ";
  ...
  <tr align='center' bgcolor='#dbeef3'>
    ...
    <td width='90' class='header_laporan'>NILAI</td>
		...
		while ($row1 = $rs1->FetchNextObject($toupper=false))
		{
			$nilai1="";
			$nilai2="";
			if ($row1->tipe1!="Header" && $row1->nama1!="." && $row1->nama1!="")
			{
				$nilai1=number_format($row1->nilai1,0,",",".");
			}
			if ($row1->tipe2!="Header" && $row1->nama2!="." && $row1->nama2!="")
			{ 
				$nilai2=number_format($row1->nilai2,0,",",".");
			}
			echo "<tr>
    <td class='isi_laporan'>";
			echo fnSpasi($row1->level_spasi1);
			if ($row1->tipe1=="Posting" && $row1->nilai1 <> 0)
			{
				echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row1->kode_neraca1','$row->kode_lokasi','$no_tugas','$nik_user','$userid');\">$row1->nama1</a>";
			}
			else
			{
				echo "$row1->nama1";
     ...
     <td class='isi_laporan'>";
			echo fnSpasi($row1->level_spasi2);
			if ($row1->tipe2=="Posting" && $row1->nilai2 <> 0)
			{
				echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row1->kode_neraca2','$row->kode_lokasi');\">$row1->nama2</a>";
			}
			else
			{
				echo "$row1->nama2";
