...
class server_report_saku2_kopeg_panjar_rptPjSaldo extends server_report_basic
{
...
from panjar2_m a  $this->filter ";
	...
	function getHtml()
	{
		...
		$periode=$tmp[0];
		$kode_lokasi=$tmp[1];
		$sql="select a.no_panjar,a.kode_lokasi,date_format(a.tanggal,'%d/%m/%Y') as tgl_panjar,a.nik_buat,b.nama as nama_buat,a.kode_pp,c.nama as nama_pp,a.nilai,a.keterangan,
	   e.no_spb,date_format(e.tanggal,'%d/%m/%Y')  as tgl_spb,f.no_kas,date_format(f.tanggal,'%d/%m/%Y')  as tgl_kas,g.no_ptg,date_format(g.tanggal,'%d/%m/%Y')  as tgl_ptg,
	   i.no_ver,date_format(i.tanggal,'%d/%m/%Y')  as tgl_ver,isnull(g.nilai,0) as nilai_ptg,isnull(g.nilai_kas,0) as nilai_kas,a.nilai - isnull(g.nilai,0) - isnull(g.nilai_kas,0) as saldo,
	   g.no_final,date_format(j.tanggal,'%d/%m/%Y') as tgl_final,k.no_kas as no_kasptg
from panjar2_m a
...
left join kas_m k on j.no_kas=k.no_kas and j.kode_lokasi=k.kode_lokasi 
$this->filter
