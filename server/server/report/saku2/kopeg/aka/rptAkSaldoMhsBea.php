...
class server_report_saku2_kopeg_aka_rptAkSaldoMhsBea extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$kode_lokasi=$tmp[0];
		$periode=$tmp[1];
		$sql="select a.nim,a.nama,a.kode_lokasi
...
left join (select y.nim,y.kode_lokasi,  sum(case when x.kode_produk in ('BEABPP','BEAS2BPP','BEAD3BPP') then x.nilai else 0 end) as bpp, 
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode = '$periode')			
			group by y.nim,y.kode_lokasi 			
			)g on a.nim=g.nim and a.kode_lokasi=g.kode_lokasi	$this->filter
order by a.nim
 ";
		//echo $sql;
		$rs = $dbLib->execute($sql);
