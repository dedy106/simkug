...
class server_report_saku2_kopeg_aka_rptAkSaldoFakultasBea extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$kode_lokasi=$tmp[0];
		$periode=$tmp[1];
		$sql="select a.kode_fakultas,a.nama,a.kode_lokasi
...
from aka_fakultas a 
left join (select y.kode_fakultas,y.kode_lokasi,  sum(case when x.kode_produk in ('BEABPP','BEAS2BPP','BEAD3BPP') then x.nilai else 0 end) as bpp, 
			...
			group by y.kode_fakultas,y.kode_lokasi 			
...
left join (select y.kode_fakultas,y.kode_lokasi,  sum(case when x.kode_produk in ('BEABPP','BEAS2BPP','BEAD3BPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as bpp, 
...
order by a.kode_fakultas
 ";
		...
		echo $AddOnLib->judul_laporan("saldo beasiswa fakultas",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
   ...
   <td class='isi_laporan' align='center'>$i</td>
    ...
    <td class='isi_laporan'>";
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenSaldoJur('$row->kode_lokasi','$row->kode_fakultas');\">$row->nama</a>";
			echo "</td>
