...
class server_report_saku2_kopeg_aka_rptAkSaldoJalurMaba extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$kode_lokasi=$tmp[0];
		$periode=$tmp[1];
		$sql="select a.kode_jalur,a.nama,a.kode_lokasi
...
from aka_mabajalur a 
left join (select y.kode_jalur,y.kode_lokasi,  sum(case when x.kode_produk in ('BPP') then x.nilai else 0 end) as bpp, 
			...
			group by y.kode_jalur,y.kode_lokasi 			
			)b on a.kode_jalur=b.kode_jalur and a.kode_lokasi=b.kode_lokasi
left join (select y.kode_jalur,y.kode_lokasi,  sum(case when x.kode_produk in ('BPP') then x.nilai else 0 end) as bpp, 
				   sum(case when x.kode_produk in ('SDP2') then x.nilai else 0 end) as sdp2, 
				   sum(case when x.kode_produk in ('UP3') then x.nilai else 0 end) as up3,
				   sum(case when x.kode_produk in ('JPKM') then x.nilai else 0 end) as jpkm	
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode = '$periode')			
			group by y.kode_jalur,y.kode_lokasi 			
			)c on a.kode_jalur=c.kode_jalur and a.kode_lokasi=c.kode_lokasi
left join (select y.kode_jalur,y.kode_lokasi,  sum(case when x.kode_produk in ('BPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as bpp, 
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode <'$periode')			
			group by y.kode_jalur,y.kode_lokasi 			
			)d on a.kode_jalur=d.kode_jalur and a.kode_lokasi=d.kode_lokasi
left join (select y.kode_jalur,y.kode_lokasi,  sum(case when x.kode_produk in ('BPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as bpp, 
			      sum(case when x.kode_produk in ('SDP2') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as sdp2, 
			      sum(case when x.kode_produk in ('UP3') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as up3,
				  sum(case when x.kode_produk in ('JPKM') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as jpkm
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode ='$periode')			
			group by y.kode_jalur,y.kode_lokasi 			
			)e on a.kode_jalur=e.kode_jalur and a.kode_lokasi=e.kode_lokasi
left join (select y.kode_jalur,y.kode_lokasi,  sum(case when x.kode_produk in ('BPP') then x.nilai else 0 end) as bpp, 
				   sum(case when x.kode_produk in ('SDP2') then x.nilai else 0 end) as sdp2, 
				   sum(case when x.kode_produk in ('UP3') then x.nilai else 0 end) as up3,
				   sum(case when x.kode_produk in ('JPKM')  then x.nilai else 0 end) as jpkm		
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode < '$periode')			
			group by y.kode_jalur,y.kode_lokasi 			
			)f on a.kode_jalur=f.kode_jalur and a.kode_lokasi=f.kode_lokasi
left join (select y.kode_jalur,y.kode_lokasi,  sum(case when x.kode_produk in ('BPP') then x.nilai else 0 end) as bpp, 
				   sum(case when x.kode_produk in ('SDP2') then x.nilai else 0 end) as sdp2, 
				   sum(case when x.kode_produk in ('UP3') then x.nilai else 0 end) as up3,
				   sum(case when x.kode_produk in ('JPKM')  then x.nilai else 0 end) as jpkm		
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode = '$periode')			
			group by y.kode_jalur,y.kode_lokasi 			
			)g on a.kode_jalur=g.kode_jalur and a.kode_lokasi=g.kode_lokasi
where a.kode_lokasi='$kode_lokasi'
 ";
		...
		echo $AddOnLib->judul_laporan("saldo tagihan jalur camaba",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr align='center' bgcolor='#CCCCCC'>
    ...
    <td width='200' rowspan='2' class='header_laporan'>Nama Jalur </td>
  ...
  <tr bgcolor='#CCCCCC'>
   ...
   <td class='isi_laporan' align='center'>$i</td>
    ...
    <td class='isi_laporan'>";
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenSaldoMhs('$row->kode_jalur','$row->kode_lokasi');\">$row->nama</a>";
			echo "</td>
