...
class server_report_saku2_kopeg_aka_rptAkSaldoBillBea extends server_report_basic
{	
	...
	function getHtml()
	{
	   ...
	   isnull(e.bpp,0) + isnull(e.sdp2,0)+ isnull(e.up3,0) + isnull(e.sks,0) <>0 ) ";
		...
		$sql="select a.nim,a.nama,a.kode_lokasi,h.no_inv,a.kode_jur
...
inner join (select a.kode_lokasi,a.nim,a.no_inv
			from aka_bill_d a
			where a.kode_lokasi = '$kode_lokasi'
			group by a.kode_lokasi,a.nim,a.no_inv
			)h on a.nim=h.nim and a.kode_lokasi=h.kode_lokasi
left join (select y.nim,y.kode_lokasi,x.no_inv,  sum(case when x.kode_produk in ('BEABPP','BEAS2BPP','BEAD3BPP') then x.nilai else 0 end) as bpp, 
...
left join (select y.nim,y.kode_lokasi,x.no_inv,   sum(case when x.kode_produk in ('BEABPP','BEAS2BPP','BEAD3BPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as bpp, 
...
left join (select y.nim,y.kode_lokasi,x.no_inv,   sum(case when x.kode_produk in ('BEABPP','BEAS2BPP','BEAD3BPP') then x.nilai else 0 end) as bpp, 
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode = '$periode')			
			group by y.nim,y.kode_lokasi,x.no_inv			
			)g on h.nim=g.nim and h.kode_lokasi=g.kode_lokasi and h.no_inv=g.no_inv	$this->filter $tmp
order by a.nim
 ";
