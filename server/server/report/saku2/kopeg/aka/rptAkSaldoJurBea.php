...
class server_report_saku2_kopeg_aka_rptAkSaldoJurBea extends server_report_basic
{	
...
from aka_jurusan a 
left join (select y.kode_jur,y.kode_lokasi,  sum(case when x.kode_produk in ('BEABPP','BEAS2BPP','BEAD3BPP') then x.nilai else 0 end) as bpp, 
...
left join (select y.kode_jur,y.kode_lokasi,  sum(case when x.kode_produk in ('BEABPP','BEAS2BPP','BEAD3BPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as bpp, 
