...
class server_report_saku2_kopeg_aka_rptAkSaldoMhsBeaTu extends server_report_basic
{	
	...
	function getHtml()
	{
	   ...
	   isnull(e.bpp,0) + isnull(e.sdp2,0)+ isnull(e.up3,0) + isnull(e.sks,0) <>0 ) ";
		...
		$sql="select a.nim,a.nama,a.kode_lokasi,a.kode_jur,a.kode_fakultas,h.nama as nama_jur,i.nama as nama_fakultas
...
left join (select y.nim,y.kode_lokasi, sum(case when x.kode_produk in ('BEABPP','BEAS2BPP','BEAD3BPP') then x.nilai else 0 end) as bpp, 
...
left join (select y.nim,y.kode_lokasi, sum(case when x.kode_produk in ('BEABPP','BEAS2BPP','BEAD3BPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as bpp, 
...
inner join aka_fakultas i on a.kode_fakultas=i.kode_fakultas and a.kode_lokasi=i.kode_lokasi
$this->filter $tmp
order by a.nim
 ";
		...
		echo $AddOnLib->judul_laporan("saldo beasiswa mahasiswa",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
   ...
   <td class='isi_laporan' align='center'>$i</td>
    ...
    <td class='isi_laporan'>";
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenKartuMhs('$row->nim','$row->kode_lokasi','$row->kode_fakultas');\">$row->nama</a>";
			echo "</td>
