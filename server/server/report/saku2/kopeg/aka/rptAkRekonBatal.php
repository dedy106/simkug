...
class server_report_saku2_kopeg_aka_rptAkRekonBatal extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$tmp=explode("/",$this->filter2);
		$periode=$tmp[0];
		$sql="select a.no_rekon,a.keterangan,b.nim,c.nama,c.kode_jur,d.nama as nama_jur,
	   isnull(b.bpp,0) as bpp,isnull(b.sdp2,0) as sdp2,isnull(b.up3,0) as up3 ,isnull(b.sks,0) as sks,isnull(b.total,0) as total    
from aka_rekon_m a
inner join (select c.nim,a.no_rekon,c.kode_lokasi,
				   ...
				   sum(case when a.kode_produk in ('SKS','SDP2','UP3','SKS') then a.nilai else 0 end) as total		
			from aka_rekon_d a 
