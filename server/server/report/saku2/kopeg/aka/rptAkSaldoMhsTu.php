...
class server_report_saku2_kopeg_aka_rptAkSaldoMhsTu extends server_report_basic
{	
...
left join (select y.nim,y.kode_lokasi,  sum(case when x.kode_produk in ('BPP','S2BPP','D3BPP') then x.nilai else 0 end) as bpp, 
...
left join (select y.nim,y.kode_lokasi,  sum(case when x.kode_produk in ('BPP','S2BPP','D3BPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as bpp, 
