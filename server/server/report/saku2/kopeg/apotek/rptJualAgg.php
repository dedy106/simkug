...
class server_report_saku2_kopeg_apotek_rptJualAgg extends server_report_basic
{	
...
inner join cust b on a.kode_cust=b.kode_cust and a.kode_lokasi=b.kode_lokasi  $this->filter ";
	...
	function getHtml()
	{
		...
		a.pasien,d.nama as nama_pasien,d.nik,a.kode_icdx,e.nama as nama_icdx,isnull(f.ppn,0) as ppn,isnull(g.dok,0) as dok
from apo_brg_jual_m a
...
inner join apo_peserta d on a.pasien=d.nikes and a.kode_lokasi=d.kode_lokasi and a.kode_cust=d.kode_cust
inner join apo_icdx e on a.kode_icdx=e.kode_icdx
left join (select no_jual,kode_lokasi,sum(nilai) as ppn
		   from apo_brg_jual_j
		   where jenis='PPN'
		   group by no_jual,kode_lokasi
		   )f on a.no_jual=f.no_jual and a.kode_lokasi=f.kode_lokasi
left join (select no_jual,kode_lokasi,sum(nilai) as dok
		   from apo_brg_jual_j
		   where jenis='PDPTDOK'
		   group by no_jual,kode_lokasi
		   )g on a.no_jual=g.no_jual and a.kode_lokasi=g.kode_lokasi   $this->filter
order by a.no_jual";
  ...
  <tr>
    <td colspan='20' style='padding:5px'><table width='100%' border='0' cellspacing='2' cellpadding='1'>
      ...
      <tr>
        ...
        <td class='header_laporan'>:&nbsp;$row->pasien -&nbsp; $row->nik -&nbsp;$row->nama_pasien</td>
      </tr>
	   <tr>
        <td class='header_laporan'>ICDX </td>
        <td class='header_laporan'>:&nbsp;$row->kode_icdx -&nbsp; $row->nama_icdx</td>
      </tr>
	   <tr>
