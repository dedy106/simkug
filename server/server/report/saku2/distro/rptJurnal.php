...
class server_report_saku2_distro_rptJurnal extends server_report_basic
{
	function getTotalPage()
	{
		global $dbLib;
		$sql="select count(a.no_do) as jum
from ds_do_m a
			inner join lokasi d on d.kode_lokasi = a.kode_lokasi ".$this->filter;
	...
	function getHtml()
	{
		...
		$alamat=$row->alamat;
		$sql="select  a.no_do,a.kode_lokasi,a.tanggal,a.keterangan,a.kode_lokasi,d.kota 
			from ds_do_m a
			inner join lokasi d on d.kode_lokasi = a.kode_lokasi  ".$this->filter."
			order by a.no_do";
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			echo "<table border='0' cellspacing='2' cellpadding='1' class='kotak'>
			  ...
			  <tr>
				<td align='center' class='istyle15'>No. $row->no_do</td>
			  </tr>
	  ...
	  $sql1="select a.kode_akun,b.nama,a.keterangan,a.kode_pp,a.kode_drk,case dc when 'D' then nilai else 0 end as debet,case dc when 'C' then nilai else 0 end as kredit  
			...
			where a.no_do='$row->no_do' and a.kode_lokasi='$row->kode_lokasi'
			order by a.no_urut ";
