...
class server_report_saku2_distro_rptHutangSaldo extends server_report_basic
{
...
inner join vendor b on d.kode_vendor=b.kode_vendor and d.kode_lokasi=b.kode_lokasi 
$this->filter";
	...
	function getHtml()
	{
		...
		$periode=$tmp[0];
		$nama_cab=$tmp[1];
		$sql="select a.kode_lokasi,a.no_do,a.no_dokumen,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.keterangan,a.total,b.nama as nama_vendor,a.no_po,
...
$this->filter order by a.no_do";
   ...
   <tr bgcolor='#CCCCCC'>
	 ...
	 <td width='100' rowspan='2' align='center' class='header_laporan'>No Dokumen</td>
	  <td width='100' rowspan='2' align='center' class='header_laporan'>No PO</td>
   ...
   <tr bgcolor='#CCCCCC'>
    <td width='90'  align='center' class='header_laporan'>Bayar Kas</td>
     ...
     <td class='isi_laporan'>";
		echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenJurnal('$row->no_do','$row->kode_lokasi');\">$row->no_do</a>";
		echo "</td>
