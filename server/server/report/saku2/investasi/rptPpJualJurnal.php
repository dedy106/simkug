...
class server_report_saku2_investasi_rptPpJualJurnal extends server_report_basic
{
	function getTotalPage()
	{
		global $dbLib;
		$sql="select count(a.no_ppjual) as jum
from inv_ppjual_m a
...
left join karyawan c on a.nik_app=c.nik and a.kode_lokasi=c.kode_lokasi ".$this->filter;
	...
	function getHtml()
	{
		...
		$alamat=$row->alamat;
		$sql="select  a.no_ppjual,a.no_dokumen,a.kode_lokasi,a.tanggal,a.keterangan,a.kode_lokasi,
			  ...
			  <tr>
				<td><table border='0' cellspacing='2' cellpadding='1'>
				  ...
				  <tr>
					...
					<td width='496' class='header_laporan'>:&nbsp;$row->no_ppjual</td>
					</tr>
				  <tr>
	  ...
	  $sql1="select a.kode_akun,b.nama,a.keterangan,a.kode_pp,a.kode_drk,case dc when 'D' then nilai else 0 end as debet,case dc when 'C' then nilai else 0 end as kredit  
			...
			where a.no_ppjual='$row->no_ppjual' and a.kode_lokasi='$row->kode_lokasi'
			order by a.no_urut ";
		//error_log($sql1);
