...
class server_report_saku2_investasi_rptSahamBeliJurnal extends server_report_basic
{
	function getTotalPage()
	{
		global $dbLib;
		$sql="select count(a.no_shmbeli) as jum
...
left join karyawan c on a.nik_app=c.nik and a.kode_lokasi=c.kode_lokasi ".$this->filter;
	...
	function getHtml()
	{
		...
		$alamat=$row->alamat;
		$sql="select  a.no_shmbeli,a.no_dokumen,a.kode_lokasi,a.tanggal,a.keterangan,a.kode_lokasi,
	  ...
	  $sql1="select a.kode_akun,b.nama,a.keterangan,a.kode_pp,a.kode_drk,case dc when 'D' then nilai else 0 end as debet,case dc when 'C' then nilai else 0 end as kredit  
			...
			where a.no_shmbeli='$row->no_shmbeli' and a.kode_lokasi='$row->kode_lokasi'
			order by a.no_urut ";
		//error_log($sql1);
