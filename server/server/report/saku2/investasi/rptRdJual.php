...
class server_report_saku2_investasi_rptRdJual extends server_report_basic
{
...
$this->filter ";
	...
	function getHtml()
	{
		...
		$periode=$tmp[0];
		$nama_cab=$tmp[1];
			$sql="select a.no_rdjual,a.no_dokumen,date_format(a.tanggal,'%d/%m/%Y') as tanggal,date_format(a.tgl_set,'%d/%m/%Y') as tgl_set,a.keterangan,
       a.kode_rdkelola,b.nama as nama_kelola,isnull(c.jumlah,0) as jumlah,isnull(c.n_jual,0) as n_jual,
	   isnull(c.komisi,0) as komisi,isnull(c.vat,0) as vat,isnull(c.levi,0) as levi,isnull(c.gainlos,0) as gainlos
from inv_rdjual_m a
inner join inv_rdkelola b on a.kode_rdkelola=b.kode_rdkelola
left join (select a.no_rdjual,sum(a.jumlah) as jumlah,sum(a.n_jual) as n_jual,sum(a.komisi) as komisi,
			   sum(a.vat) as vat,sum(a.levi) as levi,sum(a.gainlos) as gainlos
		from inv_rdjual_d a 
		group by a.no_rdjual
		  )c on a.no_rdjual=c.no_rdjual
$this->filter order by a.no_rdjual ";
		...
		echo $AddOnLib->judul_laporan("penjualan reksadana",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
   ...
   <tr bgcolor='#CCCCCC'>
     ...
     <td class='isi_laporan'>$row->no_rdjual</td>
