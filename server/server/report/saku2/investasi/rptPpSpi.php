...
class server_report_saku2_investasi_rptPpSpi extends server_report_basic
{
...
from inv_ppspi_m a
inner join inv_ppspi_d b on a.no_spi=b.no_spi
$this->filter ";
	...
	function getHtml()
	{
		...
		$periode=$tmp[0];
		$nama_cab=$tmp[1];
		$sql="select a.no_spi,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.keterangan,b.kode_properti,c.nama as nama_properti,
	   b.h_oleh,b.h_buku,b.h_wajar,b.jumlah,b.dc
from inv_ppspi_m a
inner join inv_ppspi_d b on a.no_spi=b.no_spi
inner join inv_properti c on b.kode_properti=c.kode_properti
$this->filter order by a.no_spi";
		...
		echo $AddOnLib->judul_laporan("spi properti",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
   ...
   <tr bgcolor='#CCCCCC'>
	...
	<td width='90'  align='center' class='header_laporan'>Nilai Wajar</td>
	 ...
	 <td class='isi_laporan'>$row->kode_properti</td>
