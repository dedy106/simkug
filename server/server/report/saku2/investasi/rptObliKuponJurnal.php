...
class server_report_saku2_investasi_rptObliKuponJurnal extends server_report_basic
{
	function getTotalPage()
	{
		global $dbLib;
		$sql="select count(a.no_cair) as jum
...
left join karyawan c on a.nik_app=c.nik and a.kode_lokasi=c.kode_lokasi ".$this->filter;
	...
	function getHtml()
	{
		...
		$alamat=$row->alamat;
		$sql="select  a.no_cair,'-' as no_dokumen,a.kode_lokasi,a.tanggal,a.keterangan,a.kode_lokasi,
			  ...
			  <tr>
				<td><table border='0' cellspacing='2' cellpadding='1'>
				  ...
				  <tr>
					...
					<td width='496' class='header_laporan'>:&nbsp;$row->no_cair</td>
					</tr>
				  <tr>
	  ...
	  $sql1="select a.kode_akun,b.nama,a.keterangan,a.kode_pp,a.kode_drk,case dc when 'D' then nilai else 0 end as debet,case dc when 'C' then nilai else 0 end as kredit  
			...
			where a.no_cair='$row->no_cair' and a.kode_lokasi='$row->kode_lokasi'
			order by a.no_urut ";
		//error_log($sql1);
