...
class server_report_saku2_investasi_rptRdDevidenSaldo extends server_report_basic
{
...
left join (select a.kode_rdkelola,b.kode_rd,
		   sum(case when substring(a.periode,5,2)='01' then b.nilai_dev else 0 end) as n01,
			sum(case when substring(a.periode,5,2)='02' then b.nilai_dev else 0 end) as n02,
			sum(case when substring(a.periode,5,2)='03' then b.nilai_dev else 0 end) as n03,
			sum(case when substring(a.periode,5,2)='04' then b.nilai_dev else 0 end) as n04,
			sum(case when substring(a.periode,5,2)='05' then b.nilai_dev else 0 end) as n05,
			sum(case when substring(a.periode,5,2)='06' then b.nilai_dev else 0 end) as n06,
			sum(case when substring(a.periode,5,2)='07' then b.nilai_dev else 0 end) as n07,
			sum(case when substring(a.periode,5,2)='08' then b.nilai_dev else 0 end) as n08,
			sum(case when substring(a.periode,5,2)='09' then b.nilai_dev else 0 end) as n09,
			sum(case when substring(a.periode,5,2)='10' then b.nilai_dev else 0 end) as n10,
			sum(case when substring(a.periode,5,2)='11' then b.nilai_dev else 0 end) as n11,
			sum(case when substring(a.periode,5,2)='12' then b.nilai_dev else 0 end) as n12,
			sum(b.nilai_dev) as total
		from  inv_rddev_m a
		inner join  inv_rddev_d b on a.no_rddev=b.no_rddev
		where substring(a.periode,1,4)='$tahun'
		group by a.kode_rdkelola,b.kode_rd
...
";
		...
		echo $AddOnLib->judul_laporan("reksadana deviden setahun",$this->lokasi,"TAHUN $tahun");
