...
class server_report_saku2_fa_rptFaLokasi extends server_report_basic
{
...
from fa_lokasi a
left join (select a.kode_lokfa,a.kode_lokasi,sum(a.nilai) as so_awal
           from fa_asset a
           where a.kode_lokasi='$kode_lokasi' and a.periode<'$periode'
           group by a.kode_lokfa,a.kode_lokasi
          )b on a.kode_lokfa=b.kode_lokfa and a.kode_lokasi=b.kode_lokasi
left join (select a.kode_lokfa,a.kode_lokasi,sum(a.nilai) as debet
           from fa_asset a
		   where a.kode_lokasi='$kode_lokasi' and a.periode='$periode'
           group by a.kode_lokfa,a.kode_lokasi
          ) c on a.kode_lokfa=c.kode_lokfa and a.kode_lokasi=c.kode_lokasi
left join (select a.kode_lokfa,a.kode_lokasi,sum(b.nilai) as kredit
		   from fa_asset a
...
left join (select a.kode_lokfa,a.kode_lokasi,sum(b.nilai) as ap
           from fa_asset a
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
		    ...
		    echo "<tr>
				<td class='isi_laporan'>$i</td>
				  ...
				  echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenAsset('$row->kode_lokfa','$kode_lokasi','$periode');\">$row->nama</a>";				  
				  echo "</td>
