...
class server_report_saku2_tak_rptTakJurnal extends server_report_basic
{
...
from (select no_kirim as no_bukti,kode_lokasi,periode,no_dokumen,tanggal,kode_akun,kode_pp,kode_drk,modul,keterangan,dc,nilai
      from takkirim_j
      )a 
inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi $filter  ";
		...
		{
			$sql="select count(a.no_bukti) 
from (select no_terima as no_bukti,kode_lokasi,periode,no_dokumen,tanggal,kode_akun,kode_pp,kode_drk,modul,keterangan,dc,nilai
      from takterima_j
      )a 
inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi $filter ";
		}
	...
	function getHtml()
	{
		...
		echo $AddOnLib->judul_laporan("LAPORAN JURNAL TAK UMUM",$this->lokasi,"Periode ".$AddOnLib->ubah_periode($periode));
	...
	{						
		...
		$header = array("No","No Bukti","Tanggal","Kode","Nama Akun","PP","DRK","Keterangan","Debet","Kredit");
		$headerWidth = array(10,20,15,15,80,15,15,75,20,20);//dalam mm
		...
		while ($row = $rs->FetchNextObject($toupper=false)){			
			if ($nb != $row->no_bukti){
				if ($nb != ""){
					$pdf->Cell($w[0]+$w[1]+$w[2]+$w[3]+$w[4]+$w[5]+$w[6]+$w[7],$rowHeight,"Sub Total",1,0,'R',false);		
					$pdf->Cell($w[8],$rowHeight,number_format($debet,0,',','.'),1,0,'R',false);
					$pdf->Cell($w[9],$rowHeight,number_format($kredit,0,',','.'),1,1,'R',false);				
				}
				$ix = 1;
				$debet = 0;
				$kredit = 0;
			...
			$kredit=$kredit+$row->kredit;
			$y1 = $pdf->GetY();											
			$numrowNm = $pdf->WordWrap($row->nama,$w[4] - 2);
			$numrowKet = $pdf->WordWrap($row->keterangan, $w[7] - 2);			
			...
			$pdf->MultiCell($w[4],$numrow == $numrowNm ? $defHeight : $rowHeight,$row->nama,1);														
			$y2 = $pdf->GetY();
			$y = $y2 - $y1;			
			$pdf->SetXY(10 + $w[0] + $w[1] + $w[2] + $w[3] + $w[4],  $pdf->GetY() - $y);											
			$pdf->Cell($w[5],$rowHeight,$row->kode_pp,1,0,'C',$fill);																	
			$pdf->Cell($w[6],$rowHeight,$row->kode_drk,1,0,'C',$fill);														
			$pdf->MultiCell($w[7],$numrow == $numrowKet ? $defHeight : $rowHeight,$row->keterangan,1);
			$y2 = $pdf->GetY();
			$y = $y2 - $y1;			
			$pdf->SetXY(10 + $w[0] + $w[1] + $w[2] + $w[3] + $w[4] + $w[5] + $w[6] + $w[7],  $pdf->GetY() - $y);											
			$pdf->Cell($w[8],$rowHeight,number_format($row->debet,0,',','.'),1,0,'R',$fill);
			$pdf->Cell($w[9],$rowHeight,number_format($row->kredit,0,',','.'),1,0,'R',$fill);						
			$ix++;
			$pdf->Ln();					
			$fill= false;					
		}
