...
class server_report_saku2_kb_rptPjPtgBuktiJurnal extends server_report_basic
{
	function getTotalPage()
	{
		...
		$periode=$tmp[2];
		$filter2=$tmp[3];
		$sql="select count(a.no_ptg) as jum
from ptg_m a
left join karyawan b on a.nik_user=b.nik and a.kode_lokasi=b.kode_lokasi
left join karyawan c on a.nik_setuju=c.nik and a.kode_lokasi=c.kode_lokasi $filter2 $modul";
	...
	function getHtml()
	{
		...
		$periode=$tmp[2];
		$filter2=$tmp[3];
		$sql="select a.no_ptg,a.no_dokumen,a.kode_lokasi,a.periode,a.tanggal,date_format(a.tanggal,'%d/%m/%Y') as tanggal1,a.keterangan,a.kode_lokasi,
       a.nik_user,b.nama as nama_buat,b.jabatan as jabatan_buat,a.nik_setuju,c.nama as nama_setuju,c.jabatan as jabatan_setuju,d.kota
from ptg_m a
...
left join karyawan c on a.nik_setuju=c.nik and a.kode_lokasi=c.kode_lokasi $filter2 $modul";
			  ...
			  <tr>
				<td><table border='0' cellspacing='2' cellpadding='1'>
				  ...
				  <tr>
					...
					<td width='496' class='header_laporan'>:&nbsp;$row->no_ptg</td>
					</tr>
				  <tr>
  ...
  <tr>
	...
	function doPrintHeader($sender, $page){		
		...
		$sender->Cell(10,5,"",'L',0,'C');$sender->Cell(30,5,"No Bukti",0,0,'l');$sender->Cell(5,5,":",0,0,'C');$sender->Cell($w-  55,5,$sender->row->no_ptg,'R',1,'L');
