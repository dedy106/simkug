...
class server_report_saku2_kb_rptPjPtgJurnal extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$periode=$tmp[2];
		$filter2=$tmp[3];
		$sql="select a.no_ptg as no_bukti,a.no_dokumen,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.kode_akun,b.nama,a.kode_pp,a.kode_drk,a.modul,a.keterangan, 
...
order by a.periode,a.no_ptg,a.dc desc ";
		...
		echo $AddOnLib->judul_laporan("LAPORAN JURNAL PERTANGGUNGAN PANJAR",$this->lokasi,"Periode ".$AddOnLib->ubah_periode($periode));
  ...
  <td width='30' class='header_laporan'><div align='center'>Modul</div></td>
	...
	{						
		...
		$AddOnLib=new server_util_AddOnLib();						
		$sql="select a.no_ptg as no_bukti,a.no_dokumen,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.kode_akun,b.nama,a.kode_pp,a.kode_drk,'-' as kode_cf,a.modul,a.keterangan, 
		case when a.dc='D' then a.nilai else 0 end as debet,case when a.dc='C' then a.nilai else 0 end as kredit 
		from ptg_j a  
		inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi $filter $modul
		order by a.periode,a.no_ptg,a.dc desc ";
		...
		$title = array($this->lokasi,"Laporan Jurnal Pertanggungan Panjar",$AddOnLib->ubah_periode($periode));
