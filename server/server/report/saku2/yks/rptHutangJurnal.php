...
class server_report_saku2_yks_rptHutangJurnal extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$filter2=$tmp[3];
		$filter3=$tmp[4];
		$sql="select a.no_rekon as no_bukti,a.no_dokumen,a.kode_lokasi,a.periode,a.tanggal,date_format(a.tanggal,'%d/%m/%Y') as tanggal1,a.keterangan,a.kode_lokasi,
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
		 ...
		 $sql1="select a.kode_akun,b.nama as nama_akun,a.kode_pp,a.kode_drk, a.modul,a.keterangan,case when a.dc='D' then a.nilai else 0 end as debet,case when a.dc='C' then a.nilai else 0 end as kredit 
				from yk_rekon_j a
				inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
				inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi
				where a.no_rekon='$row->no_bukti' order by a.kode_akun";
		//error_log($sql1);
	...
	{		
		...
		while ($row = $rs->FetchNextObject($toupper=false)){
			...
			{							
				...
				$tot_kredit=$tot_kredit+$row1->kredit;
				$y1 = $pdf->GetY();											
				$numrowNm = $pdf->WordWrap($row1->nama_akun,$w[2] - 2);
				...
				$pdf->MultiCell($w[2],$numrow == $numrowNm ? $defHeight : $rowHeight,$row1->nama_akun,1);														
				$y2 = $pdf->GetY();
				$y = $y2 - $y1;			
