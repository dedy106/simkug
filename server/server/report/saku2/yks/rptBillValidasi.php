...
class server_report_saku2_yks_rptBillValidasi extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$no_bill=$tmp[3];
		$jenis=$tmp[4];
		$nama_file="billing_validasi_".$no_bill.".xls";
		$sql2="exec sp_yk_bill_app '$no_bill','$kode_lokasi','$nik_user' ";
		$rs2 = $dbLib->execute($sql2);
		
		$sql = "select a.no_valid,date_format(a.tanggal,'%d-%m-%Y') as tanggal,a.periode,a.keterangan,a.no_load,b.keterangan as ket_load
from yk_valid_m a
inner join yk_peserta_m b on a.no_load=b.no_load 
where a.modul='VALID' and a.kode_lokasi='$kode_lokasi' and a.no_valid='$no_bill'";	
		...
		echo $AddOnLib->judul_laporan("VALIDASI PIUTANG",$this->lokasi,"Periode ".$AddOnLib->ubah_periode($periode));
...
from yk_bill_lap
where kode_lokasi='$kode_lokasi' and nik_user='$nik_user'
