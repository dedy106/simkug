...
class server_report_saku2_yks_rptPiutangNik extends server_report_basic
{
...
inner join karyawan f on e.flag=f.nik and e.kode_lokasi=f.kode_lokasi $this->filter and a.modul='VALID' ";	
		echo $sql;
	...
	function getHtml()
	{
		...
		$no_bill=$tmp[3];
		$jenis=$tmp[4];
		$nama_file="piutang_validasi_".$no_bill.".xls";
		//error_log($bank);
...
inner join karyawan f on e.flag=f.nik and e.kode_lokasi=f.kode_lokasi $this->filter and a.modul='VALID'";	
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
							  ...
							  <tr>
							    <td align='center' class='header_laporan'><table border='1' cellspacing='0' cellpadding='0' class='kotak'>
                                  <tr bgcolor='#CCCCCC'>
                                    ...
                                    <td width='80' rowspan='2' align='center' class='header_laporan'>Jenis</td>
									...
									<td width='200' rowspan='2' align='center' class='header_laporan'>Vendor</td>
                                  </tr>
                                  <tr bgcolor='#CCCCCC'>
			...
			$sql1 = "select a.no_valid,a.kode_lokasi,a.nik,a.nama,a.nikkes+'-'+a.pasien as pasien,a.jenis,a.icdx,date_format(a.tgl_masuk,'%d-%m-%Y') as tgl_masuk,date_format(a.tgl_keluar,'%d-%m-%Y') as tgl_keluar,a.kode_produk,a.nama_produk,
                   a.kode_cust,a.nama_cust,a.nilai_bp,a.nilai_kunj,a.nilai_cs,a.nilai_bp+a.nilai_kunj-a.nilai_cs as total,a.vendor
			from (select a.no_valid,a.kode_lokasi,a.nik,a.nama,a.nikkes,a.pasien,b.jenis,a.icdx,a.tgl_masuk,a.tgl_keluar,a.kode_produk,c.nama as nama_produk,
                   b.kode_cust,b.nama as nama_cust,a.nilai as nilai_bp,0 as nilai_kunj,0 as nilai_cs,isnull(f.nama,a.kode_vendor) as vendor
	   from yk_bill_d a
	   ...
	   left join vendor f on a.kode_vendor=f.kode_vendor and a.kode_lokasi=f.kode_lokasi
	   where a.no_valid='$row->no_valid' and a.kode_lokasi='$row->kode_lokasi' and a.flag_aktif='1' and b.jenis<>'PENSIUN'
           union all
	   select a.no_valid,a.kode_lokasi,a.nik,a.nama,a.nikkes,a.pasien,b.jenis,'-' as icdx,a.tgl_masuk,a.tgl_masuk as tgl_keluar,a.kode_produk,c.nama as nama_produk,
                  b.kode_cust,b.nama as nama_cust,0 as bp,a.umum+a.gigi+a.kbia+a.matkes as kunj,a.cs,'-' as vendor
	   from yk_billkunj_d a 
	   inner join cust b on a.loker_bast=b.kode_cust and a.kode_lokasi=b.kode_lokasi
	   inner join yk_kunj c on a.kode_produk=c.kode_produk
	   where a.no_valid='$row->no_valid' and a.kode_lokasi='$row->kode_lokasi' and a.flag_aktif='1' and b.jenis<>'PENSIUN' ) a $filter3
	   order by a.kode_cust,a.nik
	  ";
			
			$rs1 = $dbLib->execute($sql1);
			$first = true;
			$i=1;$tmp="";$nama_cust="";
			$nilai_bp=0;$nilai_kunj=0;$nilai_cs=0;$total=0;
			$tot_bp=0;$tot_kunj=0;$tot_cs=0;$tot_total=0;
			while ($row1 = $rs1->FetchNextObject($toupper=false))
			{				
				$beda = $tmp!=$row1->kode_cust; 
				
				if ($tmp!=$row1->kode_cust)
				{
					$tmp=$row1->kode_cust;
					$first = true;
					if ($i>1)
					{
						echo "<tr align='center' bgcolor='#CCCCCC'>
						    <td align='right' colspan='7' class='isi_laporan'>Sub Total $nama_cust</td>
						    <td align='right' class='isi_laporan'>".number_format($nilai_bp,0,',','.')."</td>
					...
					<td align='right' class='isi_laporan'>".number_format($total,0,',','.')."</td>
						...
						$nilai_bp=0;$nilai_kunj=0;$nilai_cs=0;$total=0;$i=1;
					}
					
				}
				if ($nama_cust!=$row1->nama_cust)
				{
					$nama_cust=$row1->nama_cust;
				};
				$nilai_bp=$nilai_bp+$row1->nilai_bp;
				$nilai_kunj=$nilai_kunj+$row1->nilai_kunj;
				$nilai_cs=$nilai_cs+$row1->nilai_cs;
				$total=$total+$row1->total;
				$tot_bp=$tot_bp+$row1->nilai_bp;
				...
				echo "<tr>
					...
					<td class='isi_laporan'>$row1->vendor</td>
					</tr>";
				$first = true;
				$i=$i+1;
			}
			...
			$nilai_bp=$nilai_bp+$row1->nilai_bp;
			$nilai_kunj=$nilai_bp+$row1->nilai_kunj;
			$nilai_cs=$nilai_bp+$row1->nilai_cs;
			$total=$total+$row1->total;
			echo " <tr align='center' bgcolor='#CCCCCC'>
						    <td align='right' colspan='7' class='isi_laporan'>Total</td>
						    <td align='right' class='isi_laporan'>".number_format($tot_bp,0,',','.')."</td>
