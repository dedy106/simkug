...
class server_report_saku2_gl_rptTBJejerKopeg extends server_report_basic
{
	function getSumPage()
	{
		global $dbLib;
...
<td width='110' class='header_laporan' align='center'>Total</div></td>
      ...
      <td class='isi_laporan' align='right'>";
			if ($n1 <> 0)
			{
				echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_akun','21','$periode');\">$n1</a>";
			}
			else
			{
				echo "$n1";
			...
			if ($n2 <> 0)
			{
				echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_akun','22','$periode');\">$n2</a>";
			}
			else
			{
				echo "$n2";
