...
class server_report_saku2_gl_rptTBLajurTuTrail extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$jenis=$tmp[6];
		$kode_fs=$tmp[7];
		$sql="exec sp_glma_dw_tmp '$lokasi','$periode','$nik_user' ";
		
		$rs = $dbLib->execute($sql);
		if ($jenis=="tb")
		{
		$sql = "select a.kode_lokasi,a.kode_akun,a.nama,a.debet,a.kredit,case when a.so_awal>0 then a.so_awal else 0 end as so_awal_debet,
...
where a.nik_user='$nik_user' and b.akun_konsol='$kode_akun'  and b.kode_lokkonsol='$lokkonsol'
...
from glma_tmp a
inner join konsol_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
inner join relakun c on b.akun_konsol=c.kode_akun and b.kode_lokkonsol=c.kode_lokasi
where a.nik_user='$nik_user'  and b.kode_lokkonsol='$lokkonsol' and c.kode_neraca='$kode_neraca' and c.kode_fs='$kode_fs'
order by a.kode_akun
";	
		}
