...
class server_report_saku2_gl_rptTBLajurKonsolTrail extends server_report_basic
{
...
from glma_tmp where nik_user='$nik_user' and (so_awal<>0 or debet<>0 or kredit<>0 or so_akhir<>0) order by kode_akun ";
  ...
  <tr bgcolor='#CCCCCC'> 
	...
	echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTbLokasi('$row->kode_akun','$row->kode_lokasi','$periode');\">$row->nama</a>";
