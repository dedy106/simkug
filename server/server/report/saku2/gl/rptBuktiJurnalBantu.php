...
class server_report_saku2_gl_rptBuktiJurnalBantu extends server_report_basic
{
...
left join karyawan c on a.nik_setuju=c.nik and a.kode_lokasi=c.kode_lokasi ".$this->filter2;
	...
	function getHtml()
	{
		...
		global $dbLib;
		$sql="select distinct a.no_ju,a.no_dokumen,a.kode_lokasi,a.periode,a.tanggal,date_format(a.tanggal,'%d/%m/%Y') as tanggal1,a.keterangan,a.kode_lokasi,
       a.nik_buat,b.nama as nama_buat,b.jabatan as jabatan_buat,a.nik_setuju,c.nama as nama_setuju,c.jabatan as jabatan_setuju, d.kota 
			from ju_m a
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			echo "<table border='0' cellspacing='1' cellpadding='2' class='kotak'>
			  ...
			  <tr>
				...
				<td width='60' class='header_laporan'><div align='center'>Kode Vendor </div></td>
	  ...
	  $sql1="select a.kode_akun,b.nama,a.keterangan,a.kode_pp,a.kode_drk,a.kode_cust,a.kode_vendor,a.nik, case dc when 'D' then nilai else 0 end as debet,case dc when 'C' then nilai else 0 end as kredit  
			from ju_j a
  ...
  <tr>
	...
	function doPrintHeader($sender, $page){		
		...
		$sender->Cell($w,10,"MEMO JURNAL",'LTR',1,'C');			
		$w = $sender->w - $sender->rMargin;
		$sender->SetX(20);		
		$sender->SetFont('Arial','B', $sender->headerSize);
		$sender->Cell(10,5,"",'L',0,'C');$sender->Cell(30,5,"No Bukti",0,0,'l');$sender->Cell(5,5,":",0,0,'C');$sender->Cell($w-  65,5,$sender->row->no_ju,'R',1,'L');		
