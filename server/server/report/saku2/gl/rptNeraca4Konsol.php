...
class server_report_saku2_gl_rptNeraca4Konsol extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$level_lap=$tmp[6];
		$nama_form=$tmp[7];
		$sql = "select kode_neraca,kode_fs,kode_lokasi,nama,tipe,n1,n2,n3,n4,level_spasi from neraca_tmp where modul='A' and nik_user='$nik_user' and level_lap<=$level_lap order by rowindex ";
...
<td width='100' class='header_laporan'><div align='center'>".$AddOnLib->ubah_periode($periode4)."</div></td>
    ...
    <td class='isi_laporan' align='right'>";
			...
			{
				echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTbKonsol('$row->kode_neraca','$row->kode_lokasi','$periode3');\">$n3</a>";
			}
			else
			{
				echo "$n3";
			...
			{
				echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTbKonsol('$row->kode_neraca','$row->kode_lokasi','$periode4');\">$n4</a>";
			}
			else
			{
				echo "$n4";
...
<td class='detail_laporan'><div align='right'>$so_akhir</div></td>
		...
		echo "<tr><td height='25' colspan='5' class='isi_laporan'>&nbsp;</td></tr>";
		$sql = "select kode_neraca,kode_fs,kode_lokasi,nama,tipe,n1*-1 as n1,n2*-1 as n2,-n3 as n3,-n4 as n4,level_spasi from neraca_tmp where modul='P' and nik_user='$nik_user' and level_lap<=$level_lap order by rowindex  ";
