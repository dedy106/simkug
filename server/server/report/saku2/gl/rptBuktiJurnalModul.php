...
class server_report_saku2_gl_rptBuktiJurnalModul extends server_report_basic
{
	function getTotalPage()
	{
		...
		$filter2=$tmp[3];
		$mdl = $tmp[4];
		$modul = array("ju_j", "kas_j",  "takkirim_j", "takterima_j","fasusut_j");
		$modulm = array("ju_m", "kas_m", "takkirim_m", "takterima_m","fasusut_m");
		$nobukti = array("no_ju", "no_kas",  "no_kirim", "no_terima","no_fasusut");
		$sql = "";
		foreach ($modul as $key => $value){			
			if ($sql != "") $sql .= " union all ";
			$sql .= " select distinct modul,'".$modulm[$key]."' as tab from ".$value ." ".($mdl !="" ? " where modul = '".$mdl."' ":"");
		}
		$dataModul = new server_util_Map();
		$rs = $dbLib->execute($sql);
		while ($row = $rs->FetchNextObject(false)){
			$dataModul->set($row->modul, $row->tab);
		}
		$sql ="";
		foreach($modul as $key => $value){
			if ($sql != "") $sql .=" union all ";
			$sql .= "select c.".$nobukti[$key]." as no_bukti, c.no_dokumen, c.kode_lokasi, c.periode, c.tanggal,
					c.keterangan, c.nik_buat,  c.".($value =="kas_j"?"nik_app":" nik_setuju")."  from $value a inner join ".$modulm[$key]."  c on c.".$nobukti[$key]." = a.".$nobukti[$key]." and c.kode_lokasi = a.kode_lokasi 
					inner join relakun b on b.kode_akun = a.kode_akun and b.kode_lokasi = a.kode_lokasi
					$filter $modulfilter ";
		}		
	...
	function getHtml()
	{
		...
		$sql="select distinct a.no_bukti,a.no_dokumen,a.kode_lokasi,a.periode,a.tanggal,date_format(a.tanggal,'%d/%m/%Y') as tanggal1,a.keterangan,
		...
		foreach($modul as $key => $value){
			...
			where a.".$nobukti[$key]."='$row->no_bukti' and a.kode_lokasi='$row->kode_lokasi' ";
		}		
		//error_log($sql1);
	...
	function doPrintHeader($sender, $page){		
		//$sender->SetY(10);		
		$wd = array(10,20,55,$sender->w - $sender->rMargin - 155,13,13,20,20);//dalam mm		
		$w = 0;
		...
		$sender->Cell(10,5,"",'L',0,'C');$sender->Cell(30,5,"No Bukti",0,0,'l');$sender->Cell(5,5,":",0,0,'C');$sender->Cell($w-  49,5,$sender->row->no_bukti,'R',1,'L');
		$sender->Cell(10,5,"",'L',0,'C');$sender->Cell(30,5,"No Dokumen",0,0,'l');$sender->Cell(5,5,":",0,0,'C');$sender->Cell($w - 49,5,$sender->row->no_dokumen,'R',1,'L');
		$sender->Cell(10,5,"",'L',0,'C');$sender->Cell(30,5,"Periode",0,0,'l');$sender->Cell(5,5,":",0,0,'C');$sender->Cell($w - 49,5,$sender->row->periode,'R',1,'l');
		$sender->Cell(10,5,"",'L',0,'C');$sender->Cell(30,5,"Tanggal",0,0,'l');$sender->Cell(5,5,":",0,0,'C');$sender->Cell($w- 49,5,$sender->row->tanggal1,'R',1,'l');
		$sender->Cell(10,5,"",'L',0,'C');$sender->Cell(30,5,"Keterangan",0,0,'l');$sender->Cell(5,5,":",0,0,'C');$sender->Cell($w- 49,5,$sender->row->keterangan,'R',1,'l');
		...
		$sender->SetFont('Arial','B', $sender->headerSize);
		//Header		
		$w= array(10,20,55,$w - 155,13,13,20,20);//dalam mm
	...
	{		
		...
		$AddOnLib=new server_util_AddOnLib();
		$title = array($this->lokasi,"Laporan Bukti Jurnal Modul");
		$titleHeight= array(8);//tinggi row per baris judul laporan dalam mm
		$titleSize = array(14);//fontSize judul
		$header = array("No","Kode Akun","Nama","Keterangan","PP","DRK","Debet","Kredit");
		...
		$ix = 1;
		$tmp = "";
		$w= array(10,20,55,$pdf->w - $pdf->rMargin - 155,13,13,20,20);//dalam mm
		while ($row = $rs->FetchNextObject($toupper=false)){
			$pdf->row = $row;			
			$sql1 ="";
			foreach($modul as $key => $value){
