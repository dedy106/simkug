...
class server_report_saku2_gl_rptBukuBesarBidang extends server_report_basic
{
...
from (select a.kode_akun,a.kode_lokasi,c.kode_bidang
	  from gldt_h a
	  inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi
	  $filter2
	  union 
	  select a.kode_akun,a.kode_lokasi,c.kode_bidang
	  from gldt a
	  inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi
	  $filter2
	 )a
...
left join (select a.kode_akun,c.kode_bidang,a.kode_lokasi,
				  sum(case when a.dc='D' then a.nilai else -a.nilai end) so_awal
           from gldt_h a
		   inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi	
           $filter4
	...
	function getHtml()
	{
		...
		$filter4=$tmp[3];
		$periode=$tmp[4];
		$sql = "select a.kode_lokasi,a.kode_akun,b.nama as nama_akun,a.kode_bidang,c.nama as nama_bidang,isnull(d.so_awal,0) as so_awal
from (select a.kode_akun,a.kode_lokasi,c.kode_bidang
	  from gldt_h a
	  inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi
	  $filter2
...
$filter3 order by a.kode_akun";
   ...
   <tr >
    <td height='23' colspan='8' style='padding:5px'><table  border='0' cellspacing='2' cellpadding='1'>
                  <tr>
                    <td class='header_laporan' width='100'>Kode Bidang  </td>
                    <td class='header_laporan' >:&nbsp;$row->kode_bidang</td>
                  </tr>
                  <tr>
                    <td class='header_laporan'>Nama Bidang </td>
...
from (select a.kode_lokasi,a.no_bukti,a.tanggal,a.kode_akun,a.kode_pp,a.kode_drk,a.dc,a.nilai,a.keterangan 
	  from gldt_h a 
	  inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi		
	  where a.kode_lokasi='$row->kode_lokasi' and substring(a.periode,1,4)=substring('$periode',1,4) and a.kode_akun='$row->kode_akun' and c.kode_bidang='$row->kode_bidang' $this->filter 	  
	  union all 
