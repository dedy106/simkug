...
class server_report_saku2_gl_rptNeracaKonsol extends server_report_basic
{
	...
	function getHtml()
	{
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			{
				echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTbKonsol('$row->kode_neraca1','$row->kode_lokasi');\">$row->nama1</a>";
			}
			else
			{
				echo "$row->nama1";
			...
			{
				echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTbKonsol('$row->kode_neraca2','$row->kode_lokasi');\">$row->nama2</a>";
			}
			else
			{
				echo "$row->nama2";
