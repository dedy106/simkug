...
class server_report_saku2_gl_rptNeracaJejerYakes extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$periode=$tmp[1];
		$nama_form=$tmp[2];
		$sql = "select kode_neraca,kode_fs,tipe,kode_lokasi,nama,tipe,n1,n2,n3,n4,n5,n6,n7,n8,n1+n2+n3+n4+n5+n6+n7+n8 as total, level_spasi 
		from neraca_tmp where modul='A' and nik_user='$nik_user'  order by rowindex ";
...
<td class='isi_laporan' align='right'>";
			...
			{
				echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_neraca','99','$periode');\">$n1</a>";
			}
			else
			{
				echo "$n1";
			...
			{
				echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_neraca','07','$periode');\">$n8</a>";
			}
			else
			{
				echo "$n8";
			...
			{
				echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_neraca','07','$periode');\">$total</a>";
			}
			else
			{
				echo "$total";
		...
		$sql = "select kode_neraca,kode_fs,kode_lokasi,nama,tipe,n1*-1 as n1,n2*-1 as n2,n3*-1 as n3,n4*-1 as n4,n5*-1 as n5,n6*-1 as n6,n7*-1 as n7,n8*-1 as n8,
		level_spasi from neraca_tmp where modul='P' and nik_user='$nik_user'  order by rowindex ";
...
<td class='isi_laporan' align='right'>";
			...
			if ($n7 <> 0)
			{
				echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_neraca','05','$periode');\">$n7</a>";
			}
			else
			{
				echo "$n7";
			...
			if ($n8 <> 0)
			{
				echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_neraca','$row->kode_lokasi','$periode');\">$n8</a>";
			}
			else
			{
				echo "$n8";
