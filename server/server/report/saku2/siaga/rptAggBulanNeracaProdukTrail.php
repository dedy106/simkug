...
class server_report_saku2_siaga_rptAggBulanNeracaProdukTrail extends server_report_basic
{
  ...
  from (select x.kode_lokasi,x.kode_akun
		...
		inner join relakunproduk z on x.kode_akun=z.kode_akun and x.kode_lokasi=z.kode_lokasi $this->filter
  ...
  left join (select x.kode_lokasi,x.kode_akun
		...
		echo $AddOnLib->judul_laporan("laporan anggaran",$this->lokasi,"TAHUN ".$tahun);
