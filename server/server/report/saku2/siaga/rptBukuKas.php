...
class server_report_saku2_siaga_rptBukuKas extends server_report_basic
{
...
$this->filter and a.nik_user='$nik_user'  and (a.so_awal<>0 or a.debet<>0 or a.kredit<>0 or a.so_akhir<>0)  and b.kode_curr='IDR' ";
	...
	function getHtml()
	{
		...
		$periode2=$tmp[3];
		$jenis=$tmp[4];
		$nama_file="mutasi_".$periode.".xls";

		$sql="exec sp_glma_kas_tmp '$kode_lokasi','$kode_lokasi','$kode_lokasi','$periode','$nik_user' ";
		$rs = $dbLib->execute($sql);
		$sql = "select a.kode_akun,a.nama,b.kode_curr,a.periode,a.so_awal
from glma_tmp a
inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi='$kode_lokasi' 
$this->filter and a.nik_user='$nik_user'  and (a.so_awal<>0 or a.debet<>0 or a.kredit<>0 or a.so_akhir<>0) and b.kode_curr='IDR'
...
";
			...
			$sql="select a.kode_lokasi,a.periode,a.no_kas as no_bukti,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.kode_akun,a.modul,a.keterangan,a.ref1,
				...
				order by a.periode,substring(a.no_kas,3,3) ";
