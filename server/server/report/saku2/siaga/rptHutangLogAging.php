...
class server_report_saku2_siaga_rptHutangLogAging extends server_report_basic
{
...
$this->filter";
	...
	function getHtml()
	{
		...
		$tgl_aging=$tmp[0];
		$sql="select a.kode_lokasi,a.no_kontrak,a.no_dokumen,date_format(a.tanggal,'%d/%m/%Y') as tgl,b.nama as nama_cust,a.nilai as tagihan,
...
left join (select a.no_kontrak,a.kode_lokasi,sum(a.nilai) as nilai_kas,
	       ...
	       from gr_kontrak_d a
		   inner join gr_kontrak b on a.no_kontrak=b.no_kontrak and a.kode_lokasi=b.kode_lokasi
		   where b.tanggal<='$tgl_aging'  
		   group by a.no_kontrak,a.kode_lokasi
		   )c on a.no_kontrak=c.no_kontrak and a.kode_lokasi=b.kode_lokasi
$this->filter and a.tanggal<='$tgl_aging' order by a.no_kontrak";
   ...
   <tr bgcolor='#CCCCCC'>
     ...
     <td class='isi_laporan'>";
		echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenHutang('$row->no_kontrak','$row->kode_lokasi');\">$row->no_kontrak</a>";
		echo "</td>
	<td class='isi_laporan'>";
		echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenBayar('$row->no_kontrak','$row->kode_lokasi');\">$row->no_dokumen</a>";
		echo "</td>
