...
class server_report_saku2_siaga_rptGlJurnalAudit extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="select a.no_bukti,a.no_dokumen,b.nama,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.kode_drk,
...
from (select kode_lokasi,no_bukti,no_dokumen,tanggal,kode_akun,keterangan,dc,kode_pp,nilai,modul,kode_drk
	from gldt $this->filter
	union all
	select kode_lokasi,no_bukti,no_dokumen,tanggal,kode_akun,keterangan,dc,kode_pp,nilai,modul,kode_drk
...
where a.kode_lokasi='$lokasi'
  ...
  <td valign='middle' class='isi_laporan' align='center'>$row->modul</td>
    ...
    <td valign='middle' class='isi_laporan' align='right'>".number_format($row->debet,0,',','.')."</td>
    <td valign='middle' class='isi_laporan' align='right'>".number_format($row->kredit,0,',','.')."</td>
  </tr>";
			$first = true;
			$i=$i+1;
		}
    ...
    <td class='header_laporan' class='header_laporan' align='right'>".number_format($debet,0,',','.')."</td>
    <td class='header_laporan' class='header_laporan' align='right'>".number_format($kredit,0,',','.')."</td>
  </tr>";
		echo "</table></div>";
		return "";
