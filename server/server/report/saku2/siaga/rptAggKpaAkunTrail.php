...
class server_report_saku2_siaga_rptAggKpaAkunTrail extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$fullId = $_GET["fullId"];		
		$sql = "select a.kode_akun,a.kode_lokasi,b.nama as nama_akun,c.nama as nama_lokasi,sum(isnull(case when dc='D' then nilai else -nilai end,0)) as so_awal
from anggaran_d a
...
union all 
select * from gldt where kode_lokasi='$row->kode_lokasi' and substring(periode,1,4)='$tahun' and kode_akun='$row->kode_akun'  ) ";
