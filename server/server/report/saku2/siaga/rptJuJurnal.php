...
class server_report_saku2_siaga_rptJuJurnal extends server_report_basic
{
...
left join karyawan c on a.nik_setuju=c.nik and a.kode_lokasi=c.kode_lokasi ".$this->filter;
	...
	function getHtml()
	{
		...
		$sql="select a.no_ju,a.kode_lokasi,a.periode,a.tanggal,a.keterangan,a.kode_lokasi,e.no_ref,
       a.nik_buat,b.nama as nama_buat,a.nik_setuju,c.nama as nama_setuju,d.kota 
			...
			left join karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi
			left join ju_j e on a.no_ju=e.no_ju and a.kode_lokasi=e.kode_lokasi and e.modul='REKPIUM' and e.jenis='piu' and e.no_urut='1'
			left join karyawan c on a.nik_setuju=c.nik and a.kode_lokasi=c.kode_lokasi ".$this->filter."
			order by a.no_ju";
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			echo "<table border='0' cellspacing='2' cellpadding='1' class='kotak'>
			  ...
			  <tr>
				<td align='center' class='istyle15'>No. $row->no_ju</td>
			  </tr>
    ...
    <td class='isi_laporan' align='right'>$tot_kredit1</td>
              ...
              <tr>
                <td><span class='header_laporan'>$row->keterangan | $row->no_ref </span></td>
              </tr>
              <tr>
                <td>&nbsp;</td>
              </tr>
