...
class server_report_saku2_siaga_rptVccAging extends server_report_basic
{
...
$this->filter";
	...
	function getHtml()
	{
	   ...
	   dbo.fnGetBuktiKasVcc(b.no_invoice,$periode) as ket_kas
from gr_vcc_m a
inner join gr_vcc_d b on a.no_vcc=b.no_vcc and a.kode_lokasi=b.kode_lokasi
left join (select a.no_invoice,a.kode_lokasi,sum(a.nilai_lain+a.nilai_kas) as nilai_kas
		   ...
		   inner join (
		    ...
		    union all 
		    select no_ju,kode_lokasi from ju_m
		     )b 
		    on a.no_bukti=b.no_kas and a.kode_lokasi=b.kode_lokasi
...
$this->filter order by b.no_invoice";
		...
		echo $AddOnLib->judul_laporan("aging piutang VCOSS - cabang $nama_cab",$this->lokasi,"Tanggal Aging ".$tgl_aging);
