...
class server_report_saku2_siaga_rptBukuBesarTrail extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$kode_akun=$tmp[3];
		$sql="exec sp_glma_akun_tmp '$kode_akun','$kode_akun','$kode_lokasi','$kode_lokasi','$kode_lokasi','$periode','$nik_user' ";
		$rs = $dbLib->execute($sql);
...
order by a.kode_akun";
		...
		echo $AddOnLib->judul_laporan("laporan buku besar",$this->lokasi,$AddOnLib->ubah_periode($periode));
...
";
			...
			$sql="select a.kode_lokasi,a.no_bukti,a.no_dokumen,a.tanggal,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.kode_akun,a.keterangan,a.kode_pp,a.kode_drk,
				...
				union all
				select a.kode_lokasi,a.no_bukti,a.no_dokumen,a.tanggal,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.kode_akun,a.keterangan,a.kode_pp,a.kode_drk,
						 case when a.dc='D' then a.nilai else 0 end as debet,case when a.dc='C' then a.nilai else 0 end as kredit,a.modul
				from gldt_h a
				where a.kode_akun='$row->kode_akun' and kode_lokasi='$kode_lokasi' and periode='$periode'
				order by a.tanggal ";
	...
	echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenJurnal('$row1->no_bukti','$row1->kode_lokasi','$periode','$row1->modul');\">$row1->no_bukti</a>";
	echo "</td>
