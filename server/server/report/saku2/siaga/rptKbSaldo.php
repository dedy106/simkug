...
class server_report_saku2_siaga_rptKbSaldo extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$nik_user=$tmp[0];
		$periode=$tmp[1];
		$sql = "select distinct kode_akun,periode,nama,kode_lokasi,debet,kredit,so_awal,so_akhir, 
...
from glma_tmp where nik_user='$nik_user' and (so_awal<>0 or debet<>0 or kredit<>0 or so_akhir<>0) order by kode_akun ";
  ...
  <tr bgcolor='#CCCCCC'>
    ...
    <td width='70' rowspan='2' class='header_laporan' align='center'>Kode</td>
  ...
  <tr bgcolor='#CCCCCC'> 
	...
	echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenBb('$row->kode_akun','$row->kode_lokasi','$row->periode');\">$row->nama</a>";
