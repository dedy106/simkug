...
class server_report_saku2_siaga_rptBukuKasTrail extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$rs = $dbLib->execute($sql);
		$sql = "select a.kode_lokasi,a.kode_akun,a.nama,b.kode_curr,a.periode,a.so_awal
from glma_tmp a
inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi='$kode_lokasi' 
where a.kode_akun='$kode_akun' and a.nik_user='$nik_user'  
order by a.kode_akun";
		...
		echo $AddOnLib->judul_laporan("laporan mutasi",$this->lokasi,"");
...
";
			...
			$sql="select a.no_kas as no_bukti,a.kode_lokasi,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.kode_akun,a.modul,a.keterangan,a.ref1,
				...
				where a.kode_akun='$row->kode_akun' and a.periode='$periode'
				order by substring(a.no_kas,3,3) ";
