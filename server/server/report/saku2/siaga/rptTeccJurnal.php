...
class server_report_saku2_siaga_rptTeccJurnal extends server_report_basic
{
	function getTotalPage()
	{
		global $dbLib;
		$sql="select count(a.no_tecc) as jum
from gr_tecc_m a
...
left join karyawan c on a.nik_app=c.nik and a.kode_lokasi=c.kode_lokasi ".$this->filter;
	...
	function getHtml()
	{
		...
		global $dbLib;
		$sql="select distinct a.no_tecc,a.kode_lokasi,a.tanggal,a.keterangan,a.kode_lokasi,
       a.nik_buat,b.nama as nama_buat,a.nik_app,c.nama as nama_setuju, d.kota 
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			echo "<table border='0' cellspacing='2' cellpadding='1' class='kotak'>
			  ...
			  <tr>
				<td align='center' class='istyle15'>No. $row->no_tecc</td>
			  </tr>
	  ...
	  $sql1="select a.kode_akun,b.nama,a.keterangan,a.kode_pp,a.kode_drk,case dc when 'D' then nilai else 0 end as debet,case dc when 'C' then nilai else 0 end as kredit  
			...
			where a.no_tecc='$row->no_tecc' and a.kode_lokasi='$row->kode_lokasi'
			order by a.no_urut ";
		//error_log($sql1);
