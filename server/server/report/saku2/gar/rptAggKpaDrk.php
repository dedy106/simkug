...
class server_report_saku2_gar_rptAggKpaDrk extends server_report_basic
{
...
from anggaran_d a
inner join drk b on a.kode_drk=b.kode_drk and a.kode_lokasi=b.kode_lokasi and b.tahun='$tahun'
inner join lokasi c on a.kode_lokasi=c.kode_lokasi ".$this->filter." 
group by a.kode_drk,a.kode_lokasi,b.nama,c.nama 
order by a.kode_drk";
 ...
 <tr >
    <td height='23' colspan='9' class='header_laporan'><table width='800' border='0' cellspacing='2' cellpadding='1'>
      <tr>
        <td width='60' class='header_laporan'>Kode DRK</td>
        <td width='360' class='header_laporan'>: $row->kode_drk</td>
       </tr>
      <tr>
        <td class='header_laporan'>Nama DRK </td>
...
";
			...
			{
				$tabel ="(select * from angg_r where kode_lokasi='$row->kode_lokasi' and substring(periode1,1,4)='$tahun' and kode_drk='$row->kode_drk' ".$kode_pp.$kode_drk.$periode." )";
				$sql="select a.no_bukti,'-' as no_dokumen,a.kode_akun,a.kode_pp,a.kode_drk,'-' as keterangan,case when a.dc='D' then nilai else 0 end as debet,case when a.dc='C' then nilai else 0 end as kredit from $tabel a  ";
...
select * from gldt where kode_lokasi='$row->kode_lokasi' and substring(periode,1,4)='$tahun' and kode_drk='$row->kode_drk' ".$kode_pp.$kode_drk.$periode." ) ";
    ...
    <td valign='top' class='isi_laporan' align='right'>".number_format($saldo,0,',','.')."</td>
			...
			echo "<tr>
   <td height='23' colspan='6' valign='top' class='header_laporan' align='right'>Total&nbsp;</td>
