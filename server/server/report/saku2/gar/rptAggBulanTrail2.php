...
class server_report_saku2_gar_rptAggBulanTrail2 extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql=" select a.kode_akun,a.kode_pp,a.kode_drk,b.nama as nama_akun,c.nama as nama_pp,d.nama as nama_drk,
         ...
         isnull(e.agg_09,0) as n9,isnull(e.agg_10,0) as n10,isnull(e.agg_11,0) as n11,isnull(e.agg_12,0) as n12
...
<td width='135' class='header_laporan'><div align='center'>Nama PP</div></td>
  ...
  <td class='isi_laporan'>$row->nama_drk</td>";
			if ($tw=="tw1")
			{
				$total=$total+$row->n1+$row->n2+$row->n3;
			...
			if ($tw=="tw2")
			{
				$total=$total+$row->n4+$row->n5+$row->n6;
			...
			if ($tw=="tw3")
			{
				$total=$total+$row->n7+$row->n8+$row->n9;
			...
			if ($tw=="tw4")
			{
				$total=$total+$row->n10+$row->n11+$row->n12;
  ...
  echo "<td class='isi_laporan' align='right'>".number_format($total,0,',','.')."</td>
</tr>";
