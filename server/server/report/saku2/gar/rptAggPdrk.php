...
class server_report_saku2_gar_rptAggPdrk extends server_report_basic
{
...
inner join karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi ".$this->filter;
	...
	function getHtml()
	{
		...
		$ttd=$tmp[1];
		$lokasi=$tmp[2];
		$sql="select a.periode,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.tanggal,a.no_pdrk,a.kode_lokasi,a.keterangan,a.nik_buat,b.nama as nama_buat,c.kota
from rra_pdrk_m a
...
inner join lokasi c on a.kode_lokasi=c.kode_lokasi
 $this->filter order by a.no_pdrk";
		...
		echo $AddOnLib->judul_laporan("laporan pengajuan rra anggaran",$this->lokasi,$tahun);
...
order by a.dc,a.kode_akun ";
  ...
  <tr>
    <td align='right'>";
        ...
        <td width='200' class='header_laporan' align='center' colspan='3'>".$row->kota." , ".substr($row->tanggal,8,2)." ".$AddOnLib->ubah_periode(substr(str_replace("-","",$row->tanggal),0,6))."</td>
      </tr>
		<tr>
        <td width='200' class='header_laporan' align='center'>Menyetujui</td>
        <td width='200' class='header_laporan' align='center'>Pemberi</td>
		<td width='200' class='header_laporan' align='center'>Menyetujui</td>
        <td width='200' class='header_laporan' align='center'>Pemohon</td>
      </tr>
