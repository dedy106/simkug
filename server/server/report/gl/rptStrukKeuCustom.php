...
class server_report_gl_rptStrukKeuCustom
{
	protected $caption;
	protected $filter;
	protected $rows;
	...
	function getTotalPage()
	{
		...
		if ($this->filter2=='COA')
		{
			$sql = "select count(kode_custom) from custom ".$this->filter;
		}else
		{
			$sql = "select count(n.kode_custom) ".
					"from custom n left outer join relakuncustom r on n.kode_custom=r.kode_custom and n.kode_fs=r.kode_fs and n.kode_lokasi=r.kode_lokasi ".
	...
	function getHtml()
	{
		...
		if ($this->filter2=='COA')
		{
			$sql = "select kode_custom,nama,jenis_akun,modul,level_spasi from custom ".$this->filter." order by rowindex ";
		}else
		{
			$sql = "select n.kode_custom,n.nama as nm,m.nama as nmakun,r.kode_akun,m.modul,m.jenis,m.kode_curr,m.block,level_spasi ".
					"from custom n left outer join relakuncustom r on n.kode_custom=r.kode_custom and n.kode_fs=r.kode_fs and n.kode_lokasi=r.kode_lokasi  ".
		...
		$html=$AddOnLib->judul_laporan("laporan Struktur Keuangan Custom",$this->lokasi,$AddOnLib->ubah_periode($periode));
		if ($this->filter2=='COA')
		{
			...
			{
			    $html.="<tr>
						 <td height='20' class='isi_laporan'>".$row->kode_custom."</td>
		...
		}else
		{
			...
			while ($row = $rs->FetchNextObject($toupper=false))
			{
			    if ($row->kode_akun=="")
				{
					...
					$length=strlen($row->kode_custom);
					$kode=substr($row->kode_custom,0,$length);
					$cek=false;
					$firstrow=true;
				}else
				{
					
					if (substr($row->kode_custom,0,$length)==$kode)
					{						
						if ($firstrow)
						{
							...
							$html.="<tr>
								 ...
								 <td class='isi_laporan'>".$row->kode_custom."</td>
								 <td class='isi_laporan'>".$row->modul."</td>
	...
	function createCSV()
	{
		$sql = "select kode_custom,nama,tipe from custom ".$this->filter." order by rowindex ";
		global $dbLib;
		$rs = $dbLib->execute($sql);
		print rs2csv($rs);
	}
