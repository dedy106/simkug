...
class server_report_gl_rptLRJejer
{
	protected $caption;
	protected $filter;
	protected $filter2;
	...
	function getHtml()
	{
		...
		case jenis_akun when  'Pendapatan' then -n4 else n4 end as n4,case jenis_akun when  'Pendapatan' then -n5 else n5 end as n5,level_spasi 
from neraca_tmp where modul='L' and nik_user='$nik_user' and level_lap<=$level_lap order by rowindex ";	
		...
		$html=$AddOnLib->judul_laporan("laporan laba rugi jejer",$this->lokasi,$AddOnLib->ubah_periode($periode));
