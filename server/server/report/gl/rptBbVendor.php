...
class server_report_gl_rptBbVendor
{
	protected $caption;
	protected $filter;
	protected $filter2;
	...
	function getTotalPage()
	{
		...
		$sql="select count(a.kode_vendor)
					 	...
					 	left join (select y.kode_akun,y.kode_vendor,sum(case when y.dc='D' then y.nilai else -y.nilai end) so_awal 
					 			   from gldt_h y 
					 	...
					 	left join (select x.kode_akun,x.kode_vendor,sum(case when x.dc='D' then x.nilai else 0 end) as debet, 
					 					  sum(case when x.dc='C' then x.nilai else 0 end) as kredit 
					 			   from (select y.kode_akun,y.kode_vendor,y.dc,y.nilai 
					 					 from gldt y 
					 					 ...
					 					 union all 
					 					 select y.kode_akun,y.kode_vendor,y.dc,y.nilai 
					 					 from gldt_h y 
					 			   		 where y.periode='$periode' and y.kode_lokasi ='$kode_lokasi' and substring(y.periode,1,4)=substring('$periode',1,4) and y.kode_akun='$kode_akun' 
					 					) x 
					 ...
					 where a.kode_lokasi='$kode_lokasi' $kode_vendor and (so_awal<>0 or debet<>0 or kredit <>0) 
	...
	function getHtml()
	{
		...
		$kode_akun=$tmp[2];
		$kode_vendor=$tmp[3];
		$sql="select a.kode_vendor,a.nama as nama_vendor,d.kode_akun,d.nama as nama_akun,a.kode_lokasi,ifnull(b.so_awal,0) as so_awal,ifnull(c.debet,0) as debet,ifnull(c.kredit,0) as kredit, 
		...
		echo $AddOnLib->judul_laporan("laporan buku besar hutang" ,$this->lokasi,$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
    ...
    <td width='70' class='header_laporan' align='center'>Kode Vendor</td>
...
select * from gldt ".$this->filter." )";
			...
			$sql="select a.no_bukti,a.no_dokumen,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.kode_akun,a.kode_pp,a.kode_vendor,a.keterangan,
       case when a.dc='D' then nilai else 0 end as debet,
       case when a.dc='C' then nilai else 0 end as kredit
from $tabel a
where a.kode_akun='$row->kode_akun' and a.kode_lokasi='$row->kode_lokasi'  and a.kode_vendor='$row->kode_vendor'
order by a.tanggal ";
			error_log($sql);
    ...
    <td valign='top' class='isi_laporan'>".$row1->kode_vendor."</td>
