...
class server_report_gl_rptGlJurnal
{
	protected $caption;
	protected $filter;
	protected $filter2;
	...
	{
		global $dbLib;
		$tabel="( select kode_lokasi,no_bukti,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,kode_drk
from gldt $this->filter
union all
select kode_lokasi,no_bukti,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,kode_drk
...
select kode_lokasi,no_bukti,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,kode_drk
from gldt_h $this->filter )";
		$sql="select a.no_bukti as no_bukti,b.nama,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.kode_akun,a.kode_pp,a.kode_drk,a.keterangan,case when a.dc='D' then a.nilai else 0 end as debet,case when a.dc='C' then a.nilai else 0 end as kredit 
		      from $tabel a 
			  inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
			  order by a.no_bukti,a.dc ";
		...
		echo $AddOnLib->judul_laporan("laporan jurnal transaksi $modul",$this->lokasi,$AddOnLib->ubah_periode($periode));
