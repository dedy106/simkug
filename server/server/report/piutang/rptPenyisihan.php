...
class server_report_piutang_rptPenyisihan
{
	protected $caption;
	protected $filter;
	protected $rows;
	...
	function getTotalPage()
	{
		...
		$sql =  "select count(*) ".
				"from mhs m inner join armhs_m b on m.npm=b.ref1 and m.kode_lokasi=b.kode_lokasi ".
						   ...
						   "inner join ar_susut s on b.no_invoice=s.ref1 and b.kode_lokasi=s.kode_lokasi ".
	...
	{
		global $dbLib;
		$sql =  "select m.npm,m.nama_mhs,c.kode_ang,c.nama_ang,d.semester,m.kode_jur,s.ref1,s.p_susut,s.nilai ".
				"from mhs m inner join armhs_m b on m.npm=b.ref1 and m.kode_lokasi=b.kode_lokasi ".
						   "inner join armhs_d d on b.no_invoice=d.no_invoice and b.kode_lokasi=d.kode_lokasi ".
						   "inner join ar_susut s on b.no_invoice=s.ref1 and b.kode_lokasi=s.kode_lokasi ".
						   "inner join angkatan c on m.kode_ang=c.kode_ang and m.kode_lokasi=c.kode_lokasi and m.kode_jur=c.kode_jur ".$this->filter.
				" order by m.npm,s.no_susut ";
		...
		$html=$AddOnLib->judul_laporan("Laporan Penyisihan Piutang",$this->lokasi,$AddOnLib->ubah_periode($periode));
