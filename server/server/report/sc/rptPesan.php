...
class server_report_sc_rptPesan extends server_report_basic
{	
...
inner join sc_jenis d on a.kode_jenis=d.kode_jenis   ";
	...
	function getHtml()
	{
		...
		$sql="select a.no_pesan,a.kode_cc,b.nama as nama_cc,d.nama as nama_jenis,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.keterangan,a.nik_buat,c.nama as nama_buat 
from sc_pesan_m a
inner join sc_cc b on a.kode_cc=b.kode_cc 
inner join sc_karyawan c on a.nik_buat=c.nik 
inner join sc_jenis d on a.kode_jenis=d.kode_jenis 
order by a.no_pesan ";
		...
		echo $AddOnLib->judul_laporan("pesanan",$this->lokasi,$AddOnLib->ubah_periode($periode));
  ...
  <tr>
    <td colspan='20' style='padding:5px'><table width='100%' border='0' cellspacing='2' cellpadding='1'>
      <tr>
	    ...
	    <tr>
        <td class='header_laporan'>Cost Center </td>
        <td class='header_laporan'>:&nbsp;$row->kode_cc -&nbsp; $row->nama_cc</td>
      </tr>
      <tr>
