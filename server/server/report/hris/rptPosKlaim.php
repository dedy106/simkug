...
class server_report_hris_rptPosKlaim extends server_report_basic
{
...
left join gr_ver_m d on c.no_ver=d.no_ver and d.kode_lokasi=c.kode_lokasi and d.modul='KLAIM'
...
left join gr_ver_m f on e.no_ver=f.no_ver and e.kode_lokasi=f.kode_lokasi and f.modul='FINALKLAIM'  $this->filter ";
	...
	function getHtml()
	{
		...
		$footer=$row->nama;
		$sql="select a.no_klaim,date_format(a.tanggal,'%d/%m/%Y') as tgl_absen,a.nik_buat,b.nama,a.keterangan,d.no_ver,
...
order by a.no_klaim ";
		...
		echo $AddOnLib->judul_laporan("laporan posisi klaim kesehatan",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
   ...
   <tr bgcolor='#CCCCCC'>
     ...
     <td class='isi_laporan' align='center'>$i</td>
      <td class='isi_laporan'>";
		echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenBukti('$row->no_klaim','$row->kode_lokasi');\">$row->no_klaim</a>";
		echo "</td>
