...
class server_report_hris_rptPosCuti extends server_report_basic
{
...
left join gr_app_m f on e.no_app=f.no_app and e.kode_lokasi=f.kode_lokasi $this->filter ";
	...
	function getHtml()
	{
		...
		$footer=$row->nama;
		$sql="select a.no_cuti,date_format(a.tanggal,'%d/%m/%Y') as tgl_cuti,a.nik_buat,b.nama,a.alasan as keterangan,d.no_ver,
...
order by a.no_cuti ";
		...
		echo $AddOnLib->judul_laporan("posisi cuti",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
		echo "<table  border='0' cellspacing='2' cellpadding='1'>
  <tr>
    <td align='center'><table border='1' cellspacing='0' cellpadding='0' class='kotak'>
   <tr bgcolor='#CCCCCC'>
     <td width='30'  align='center' class='header_laporan'>No</td>
     <td width='80'  align='center' class='header_laporan'>No Cuti</td>
	 <td width='60'  align='center' class='header_laporan'>Tgl Cuti</td>
     ...
     <td class='isi_laporan' align='center'>$i</td>
      <td class='isi_laporan'>";
		echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenBukti('$row->no_cuti','$row->kode_lokasi');\">$row->no_cuti</a>";
		echo "</td>
