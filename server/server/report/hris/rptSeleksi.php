...
class server_report_hris_rptSeleksi extends server_report_basic
{	
...
inner join gr_rekrut_proses b on a.kode_proses=b.kode_proses and a.kode_lokasi=b.kode_lokasi $this->filter ";
	...
	function getHtml()
	{
		...
		$kode_bidang=$tmp[4];
		$ver=$tmp[1];
		$sql="select a.no_seleksi,date_format(a.tanggal,'%d/%m/%Y') as tanggal,b.nama as proses,a.keterangan,a.kode_lokasi 
from gr_rekrut_seleksi_m a
inner join gr_rekrut_proses b on a.kode_proses=b.kode_proses and a.kode_lokasi=b.kode_lokasi $this->filter order by a.no_seleksi";
		error_log($sql);
