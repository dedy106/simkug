...
class server_report_eclaim3_rptVerifikasi extends server_report_basic
{	
...
inner join tlk_ver_d h on a.no_klaim=h.no_bukti
inner join tlk_ver_m j on h.no_ver=j.no_ver
left join tlk_user i on j.nik_user=i.nik ".$this->filter;
		$totPage = 0;
		if ($rs)
		{
			$count = $rs->fields[0];
	...
	function getHtml()
	{
       ...
       a.nilai, a.kode_curr, a.tindakan, a.tgl_dokumen, a.nik_buat, a.tgl_input, a.kode_obyek, a.kode_sebab, date_format(a.tgl_input,'%d/%m/%Y') as tgl_input,
       a.kurs, a.kode_asuransi, a.kode_lok, a.periode, a.nik_user, a.pic, a.no_tel, a.no_fax,a.host,a.ip,
       b.nama as nama_ttg,c.nama as nama_obyek,d.nama as nama_sebab,e.nama as nama_lok,f.nama as nama_asuransi,g.nama as nama_curr,
		j.no_ver,date_format(j.tanggal,'%d/%m/%Y') as tgl_ver,h.catatan  as ket_ver,j.nik_user as nik_ver,j.no_dokumen as dok_ver,
		j.host as host_ver,j.ip as ip_ver,j.tgl_input as tgl_input_ver
from tlk_klaim a
inner join tlk_ttg b on a.kode_ttg=b.kode_ttg
inner join tlk_obyek c on a.kode_obyek=c.kode_obyek
inner join tlk_sebab d on a.kode_sebab=d.kode_sebab
...
left join tlk_user i on j.nik_user=i.nik ".$this->filter;
  ...
  <tr>
    <td><table width='600' border='1' align='center' cellpadding='2' cellspacing='0' bordercolor='#111111' style='border-collapse: collapse'>
      ...
      <tr>
        <td valign='middle' class='laporan_isi' >No Klaim </td>
      ...
      <tr>
        ...
        <td class='laporan_isi'>$row->no_tel</td>
      </tr>
      <tr>
