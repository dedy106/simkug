...
class server_report_gaji_rptHRSlip2
{
	protected $caption;
	protected $filter;
	protected $filter2;
...
INNER JOIN hr_loker f ON  c.kode_loker=f.kode_loker ".$this->filter;
	...
	function getHtml()
	{
		...
		$sql = "SELECT distinct a.periode,date_format(a.tanggal,'%d/%m/%Y') as tanggal,b.bank,b.cabang,b.no_rek,b.nama_rek,g.gadas,a.nik,b.nama,e.nama AS lokasi,f.nama AS loker,c.tingkat,m.jab_baru as jabatan,
		ifnull(h.pph_bln_lalu,0) as pph_bln_lalu,ifnull(i.pph_bln_sd,0) as pph_bln_sd,       
ifnull(j.pdpt_bln_lalu,0) as pdpt_bln_lalu,ifnull(k.pdpt_bln_sd,0) as pdpt_bln_sd,l.jml,x.logo
FROM gaji_m a 
INNER JOIN karyawan b ON a.nik=b.nik 
INNER JOIN hr_dinas2 c ON a.nik=c.nik 
left JOIN hr_jabatan d ON c.kode_jabs=d.kode_jab 
...
inner join hr_jabs m on a.nik=m.nik
inner join lokasi x on b.kode_lokasi=x.kode_lokasi
...
left join (select a.nik,sum(nilai) as pph_bln_lalu
 ...
 left join (select a.nik,sum(case dc when 'D' then nilai else -nilai end)  as pdpt_bln_lalu
	    from gaji_d a
	    inner join gaji_m b on a.no_gaji=b.no_gaji
	    inner join gaji_param_d c on a.kode_param=c.kode_param
 ...
 left join (select a.nik,sum(case dc when 'D' then nilai else -nilai end) as pdpt_bln_sd
		...
		{
			$pathfoto = $path . "image/banner/".$row->logo;
			$status_nol="";
			if ($status=="Tidak")
			{
  ...
  <tr>
    <td height='23' colspan='8' class='header_laporan'><table width='100%' border='0' cellpadding='1' cellspacing='2'>
      <tr>
        <td colspan='4' align='center' class='header_laporan'><img src=$pathfoto /></td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td class='header_laporan'>JML TANGGUNGAN </td>
        <td class='header_laporan'>&nbsp;$row->jml</td>
      </tr>
      <tr>
...
WHERE b.jenis='POT' AND a.nik='$row->nik' $status_nol order by b.nu";
  ...
  <tr>
    <td class='header_laporan'>Jumlah Penerimaan : </td>
    <td class='header_laporan'>Jumlah Potongan : </td>
    <td class='header_laporan'>Jumlah Yang Diterima : </td>
  </tr>
  <tr>
    <td class='header_laporan'>Rp. ".number_format($jum1,0,',','.')."</td>
    <td class='header_laporan'>Rp. ".number_format($jum2,0,',','.')."</td>
    <td class='header_laporan'>Rp. ".number_format($jum1-$jum2,0,',','.')."</td>
  </tr>
</table></td>
  </tr>
  <tr >
	 ...
	 left join (select a.nik,sum(case dc when 'D' then nilai else -nilai end) as pdpt_bln_sd
			...
			while ($row = $rs->FetchNextObject($toupper=false))
			{
				...
				$html = "<!DOCTYPE html PUBLIC '-//W3C//DTD XHTML 1.0 Transitional//EN' 'http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd'>".
					...
					"<title>Gaji</title>".
					"</head>".
				$html .= file_get_contents("server/util/laporan.css"); 
					"<body>";				 
			$html .="<table border='1' cellspacing='0' cellpadding='1' class='kotak' width='600'>
	  <tr>
	    <td height='23' colspan='8' class='header_laporan'><table width='100%' border='0' cellpadding='1' cellspacing='2'>
	...
	WHERE b.jenis='POT' AND a.nik='$row->nik' $status_nol order by b.nu";
	  ...
	  <tr>
	    ...
	    <td class='header_laporan'>Rp. ".number_format($row->pdpt_bln_sd,0,',','.')."</td>
				...
				file_put_contents("tmp/".$row->nik.".html", $html);
				  			
	  			$result->set($row->nik,$html);
	  			$html = "";
			}
		}catch(Exception $e){
			return $e->getMessage();
