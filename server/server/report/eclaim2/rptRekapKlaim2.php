...
class server_report_eclaim2_rptRekapKlaim2 extends server_report_basic
{	
  ...
  <tr align='center' bgcolor='#CCCCCC'>
    ...
    <td width='100' class='header_laporan'>NET CLAIM</td>
		...
		$sql = "select  count(a.no_klaim) as jml,sum(a.nilai) as nilai_est,sum(ifnull(b.nilai,0)) as nilai_adjust, 
	     sum(ifnull(b.nilai_ddct,0)) as nilai_ddct,sum(ifnull(b.nilai,0))-sum(ifnull(b.nilai_ddct,0)) as nilai_settle
from tlk_klaim a 
left join tlk_adjust b on a.no_klaim=b.no_klaim $this->filter and a.progress='13' and a.status='0' ";
		$rs = $dbLib->execute($sql);
   ...
   <td class='isi_laporan'>KLAIM SWAKELOLA</td>
    ...
    <td align='right' class='isi_laporan'>".number_format($row->nilai_settle,0,',','.')."</td>
		...
		$sql = "select  count(a.no_klaim) as jml,sum(a.nilai) as nilai_est,sum(ifnull(b.nilai,0)) as nilai_adjust, 
	     sum(ifnull(b.nilai_ddct,0)) as nilai_ddct,sum(ifnull(b.nilai,0))-sum(ifnull(b.nilai_ddct,0)) as nilai_settle
from tlk_klaim a 
left join tlk_adjust b on a.no_klaim=b.no_klaim $this->filter and a.progress='13'  and a.status='1' ";
		$rs = $dbLib->execute($sql);
		$row = $rs->FetchNextObject($toupper=false);
		$jml=$jml+$row->jml;
		$nilai_est=$nilai_est+$row->nilai_est;
...
from tlk_proses a
inner join (select  a.progress,count(a.no_klaim) as jml,sum(a.nilai) as nilai_est,sum(ifnull(b.nilai,0)) as nilai_adjust, 
	     sum(ifnull(b.nilai_ddct,0)) as nilai_ddct,sum(ifnull(b.nilai,0))-sum(ifnull(b.nilai_ddct,0)) as nilai_settle
from tlk_klaim a 
left join tlk_adjust b on a.no_klaim=b.no_klaim $this->filter and a.progress not in ('13','15')  
group by a.progress
...
order by a.kode_proses	
 ";
	   ...
	   <td class='isi_laporan'>".strtoupper($row->nama)."</td>
		<td align='center' class='isi_laporan'>".number_format($row->jml,0,',','.')."</td>
		<td align='right' class='isi_laporan'>".number_format($row->nilai_est,0,',','.')."</td>
		<td align='right' class='isi_laporan'>".number_format($row->nilai_adjust,0,',','.')."</td>
		<td align='right' class='isi_laporan'>".number_format($row->nilai_ddct,0,',','.')."</td>
  ...
  <tr align='center' bgcolor='#CCCCCC'>
    ...
    <td width='100' class='header_laporan'>NET CLAIM</td>
		...
		$sql = "select  count(a.no_klaim) as jml,sum(a.nilai) as nilai_est,sum(ifnull(b.nilai,0)) as nilai_adjust, 
	     sum(ifnull(b.nilai_ddct,0)) as nilai_ddct,sum(ifnull(b.nilai,0))-sum(ifnull(b.nilai_ddct,0)) as nilai_settle
from tlk_klaim a 
left join tlk_adjust b on a.no_klaim=b.no_klaim $this->filter and a.progress='15'   ";
		$rs = $dbLib->execute($sql);
		$row = $rs->FetchNextObject($toupper=false);
		$jml=$jml+$row->jml;
		$nilai_est=$nilai_est+$row->nilai_est;
