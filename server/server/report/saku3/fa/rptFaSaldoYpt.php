...
class server_report_saku3_fa_rptFaSaldoYpt extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql = "select a.kode_klpakun,a.nama,a.kode_lokasi,
...
left join (select a.kode_klpakun,a.kode_lokasi,sum(b.nilai) as so_awal
           ...
           where  a.periode<'$periode' and a.progress not in ('K','P') and a.jenis<>'I' $filterpp
...
left join (select a.kode_klpakun,a.kode_lokasi,sum(b.nilai) as kredit
		   ...
		   where  b.periode<'$periode' and a.progress not in ('K','P') and a.jenis<>'I' $filterpp
		   group by a.kode_klpakun,a.kode_lokasi
...
left join (select a.kode_klpakun,a.kode_lokasi,sum(case when b.dc='D' then b.nilai else -b.nilai end) as ap
           ...
           where b.periode<'$periode_susut' and a.progress not in ('K','P') and a.jenis<>'I' $filterpp
           group by a.kode_klpakun,a.kode_lokasi
...
$this->filter  and (isnull(b.so_awal,0)-isnull(dd.kredit,0)+isnull(c.debet,0)-isnull(d.kredit,0)<>0) and a.kode_lokasi not in ('05','07','08','09')
order by a.kode_klpakun,a.kode_lokasi ";
  ...
  <tr bgcolor='#CCCCCC'>
	...
	<td rowspan='4' align='center' class='header_laporan'>Lokasi</td>
