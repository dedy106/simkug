...
class server_report_saku3_fa_rptFaSaldoTahunTu extends server_report_basic
{
	...
	function getHtml()
	{
	   ...
	   isnull(e.ap,0) as ap ,
	   isnull(b.so_awal,0)-isnull(e.ap,0) - isnull(dd.wo,0) as nilai_buku,j.jenis
from fa_klpakun a
...
left join (select a.kode_klpakun,a.kode_lokasi,substring(a.periode,1,4) as tahun,sum(case when b.dc='D' then b.nilai else -b.nilai end) as ap
		   ...
		   $this->filter and b.periode<='$periode' and a.progress not in ('K','P') and a.jenis<>'I' and bb.no_fa is null 
           group by a.kode_klpakun,a.kode_lokasi,substring(a.periode,1,4)
...
left join (select a.kode_klpakun,a.kode_lokasi,substring(a.periode,1,4) as tahun,sum(a.nilai) as wo
		   ...
		   $this->filter and b.periode<='$periode' and a.progress not in ('K','P') and a.jenis<>'I'
		   group by a.kode_klpakun,a.kode_lokasi,substring(a.periode,1,4)
