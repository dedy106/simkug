<?php
uses("server_DBConnection_dbLib");
uses("server_report_basic");
uses("server_util_AddOnLib");
global $dbLib;
$dbLib = new server_DBConnection_dbLib("mssql");
function fnSpasi($level)
{
	$tmp="";
	for ($i=1; $i<=$level; $i++)
	{
		$tmp=$tmp."&nbsp;&nbsp;&nbsp;&nbsp;";
	}
	return $tmp;
}
class server_report_saku3_pbh_ypt_rptDokFisik extends server_report_basic
{
	function getTotalPage()
	{
		global $dbLib;
		$sql = "select 1 ";
		$rs = $dbLib->execute($sql);		
		$totPage = 0;
		if ($rs)
		{
			$count = $rs->fields[0];
			$totPage = ceil($count / $this->rows);		
		}
		//error_log($this->rows."/".$count."/".$totPage);
		return $totPage;
	}
	function getHtml()
	{
		
		global $dbLib;
		$tmp=explode("/",$this->filter2);
		$kode_lokasi=$tmp[0];
        $periode=$tmp[1];
        $kode_pp=$tmp[2];
        $nik=$tmp[3];
                
        session_start();
        $_SESSION['isLogedIn'] = true;				
        $_SESSION['userLog'] = $nik;
        $_SESSION['lokasi'] = $kode_lokasi;
        
        $link=$_SERVER['REQUEST_SCHEME']."s://".$_SERVER['SERVER_NAME'];
        $root_ser=$link."/web/server/pbh";
        $folderroot_js=$link."/server/bs/js";
        $root=$link;
        $path = $link."/";
		$resource = $_GET["resource"];
        $fullId = $_GET["fullId"];
      
    echo "
      <div id='loading-overlay' style='background: rgba(233, 233, 233, 0.34) none repeat scroll 0% 0%; display: none; position: absolute; top: 0; right: 0; bottom: 0; left: 0; z-index:5;'>
          <center>
              <img src='$link/image/stackspinner.gif' style='position:fixed; top: 50%; transform: translateY(-50%);'>
          </center>
      </div>";
      echo"
        <style>
        @import url('https://fonts.googleapis.com/css?family=Roboto&display=swap');
        body {
          font-family: 'Roboto', sans-serif !important;
        }
        h1, h2, h3, h4, h5, h6, .h1, .h2, .h3, .h4, .h5, .h6 {
          font-family: 'Roboto', sans-serif !important;
          font-weight: normal !important;
        }
        h3{
          margin-bottom: 5px;
          font-size:18px !important
        }
        h2{
          margin-bottom: 5px;
          margin-top:5px;
        }
        .judul-box{
          font-weight:bold;
          font-size:18px !important;
        }
        .inner{
          padding:5px !important;

        }
        
        .box-nil{
          margin-bottom: 20px !important;
        }
        
        .pad-more{
          padding-left:0px !important;
          padding-right:10px !important;
        }
        .mar-mor{
          margin-bottom:10px !important;
        }
        .box-wh{
          box-shadow: 0 3px 3px 3px rgba(0,0,0,.05);
        }
        #ajax-content-section{
          background: white !important;
        }
        th{
          vertical-align: middle !important; 
        }
        </style>
          <div id='sai_containerx'>
            <form id='sai_form_insert'>
                <div class='row' style='margin-bottom:10px'>
                    <div class='col-md-12' style='margin-bottom: 10px;box-shadow: 0px 0px 1px white;border-bottom: 1px solid #ece7e7;'>
                     <button type='submit' class='btn btn-primary' style='margin-bottom: 10px;' id='validate'><i class='fa fa-save' ></i> Simpan</button>
                    </div>
                      <div class='col-md-12'>
                        <div class='row'>
                            <div class='form-group'>
                                <label class='control-label col-sm-2'>No Bukti</label>
                                <div class='col-sm-3' style='margin-bottom:5px;'>
                                    <input name='no_aju' type='text' class='form-control' required id='no_aju' readonly>
                                    <input name='tanggal' type='hidden' class='form-control' required id='tanggal' readonly value='".date('Y-m-d')."'>
                                </div>
                                <label class='control-label col-sm-2'>Barcode</label>
                                <div class='col-sm-3' style='margin-bottom:5px;'>
                                    <input name='barcode' type='text' class='form-control' required id='barcode'>
                                </div>
                            </div>
                        </div>
                        <div class='row'>
                            <div class='form-group'>
                                <label class='control-label col-sm-2'>Nominal </label>
                                <div class='col-sm-3' style='margin-bottom:5px;'>
                                    <input name='nilai' type='text' class='form-control currency' required id='nilai' readonly>
                                </div>
                            </div>
                        </div>
                        <div class='row'>
                            <div class='form-group'>
                                <label class='control-label col-sm-2'>Modul</label>
                                <div class='col-sm-3' style='margin-bottom:5px;'>
                                    <input name='modul' type='text' class='form-control' required id='modul' readonly>
                                </div>
                            </div>
                        </div>
                        <div class='row'>
                            <div class='form-group'>
                                <label class='control-label col-sm-2'>Tgl Pengajuan </label>
                                <div class='col-sm-3' style='margin-bottom:5px;'>
                                    <input name='tgl_aju' type='text' class='form-control' required id='tgl_aju' readonly>
                                </div>
                            </div>
                        </div>
                        <div class='row'>
                            <div class='form-group'>
                                <label class='control-label col-sm-2'>Deskripsi</label>
                                <div class='col-sm-8' style='margin-bottom:5px;'>
                                    <input name='deskripsi' type='text' class='form-control' required id='deskripsi' readonly>
                                </div>
                            </div>
                        </div>
                        <div class='row'>
                            <div class='form-group'>
                                <label class='control-label col-sm-2'>NIK Penerima</label>
                                <div style='margin-bottom:5px;' class='col-sm-3'>
                                    <select class='form-control' id='nik_terima' name='nik_terima' required>
                                    <option value=''>Pilih NIK Penerima</option>
                                    
                                    </select>
                                </div>
                            </div>
                        </div>
                        <div class='row'>
                            <div class='form-group'>
                                <label class='control-label col-sm-2'>Di serahkan oleh</label>
                                <div class='col-sm-5' style='margin-bottom:5px;'>
                                    <input name='nik_serah' type='text' class='form-control' required id='nik_serah'>
                                </div>
                            </div>
                        </div>
                        <div class='row'>
                            <div class='form-group'>
                                <label class='control-label col-sm-2'>Catatan</label>
                                <div class='col-sm-8' style='margin-bottom:5px;'>
                                    <input name='catatan' type='text' class='form-control' required id='catatan'>
                                </div>
                            </div>
                        </div>
                        <div class='nav-tabs-custom'>
                          <ul class='nav nav-tabs'>
                            <li class='active'><a href='#tab_1' data-toggle='tab' aria-expanded='true'>Rekening</a></li>
                            <li class=''><a href='#tab_2' data-toggle='tab' aria-expanded='false'>Item Jurnal</a></li>
                          </ul>
                          <div class='tab-content'>
                            <div class='tab-pane active' id='tab_1'>
                                <div class='sai-container-overflow-x' style='75%'>
                                    <table class='table table-bordered table-striped Datatable' id='table-rek' style='font-size: 12px;' >
                                        <thead style='background: #ff9500;color: white;'>
                                            <tr>
                                                <th>Bank</th>
                                                <th>Atensi</th>
                                                <th>No Rek</th>
                                                <th>Nama Rek</th>
                                                <th>Bruto</th>
                                                <th>Potongan</th>
                                                <th>Keterangan</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                        </tbody>
                                    </table>
                                </div>
                            </div>
                            <div class='tab-pane' id='tab_2'>
                                <div class='sai-container-overflow-x' style='75%'>
                                    <table class='table table-bordered table-striped Datatable' id='table-ju' style='font-size: 12px;' >
                                        <thead style='background: #ff9500;color: white;'>
                                            <tr>
                                                <th>Kode Akun</th>
                                                <th>Nama Akun</th>
                                                <th>DC</th>
                                                <th>Keterangan</th>
                                                <th>Nilai</th>
                                                <th>Kode PP</th>
                                                <th>Nama PP</th>
                                                <th>Kode Drk</th>
                                                <th>Nama Drk</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                        </tbody>
                                    </table>
                                </div>
                            </div>
                          </div>
                        </div>
                    </div>
                </div>
            </form>
            <div id='sai_hasil' hidden>
                <a id='printButton' href='#' class='btn btn-primary'>
                <i class='fa fa-print'></i> Print
                </a>
                <a id='backButton' href='#' class='btn btn-default'>
                <i class='fa fa-arrow-left'></i> Back
                </a>
                <iframe id='hasil' name='hasil' style='margin-top:10px;border: none;
                width: 100%;min-height:480px'></iframe>
                <div id='hasil2' name='hasil' style='margin-top:10px;border: none;
                width: 100%;min-height:480px' hidden></div>
                
            </div>
          </div>
          
          <script src='$folderroot_js/jquery.scannerdetection.js'></script>
          <script type='text/javascript'>
         
          function getNIK(){

            $.ajax({
                type: 'GET',
                url: '$root_ser/DokFisik.php?fx=getNIKTerima',
                dataType: 'json',
                async: false,
                data: {'kode_lokasi':'$kode_lokasi','kode_pp':'$kode_pp','nik':'$nik'},
                success:function(result){    
                    if(result.status){
                        if(typeof result.daftar !== 'undefined' && result.daftar.length>0){
                            var select = $('#nik_terima').selectize();
                            var control = select[0].selectize;
                            control.clearOptions();
                            for(var i=0;i<result.daftar.length;i++){
                              control.addOption([{text:result.daftar[i].nik + ' - ' + result.daftar[i].nama, value:result.daftar[i].nik}]);
                            }
                            control.setValue('$nik');
                          
                        }
                    }
                }
            });
          }

          getNIK();

          function loadDok(){
              var no_aju = $('#barcode').val();
              $.ajax({
                type: 'GET',
                url: '$root_ser/DokFisik.php?fx=getDok',
                dataType: 'json',
                data: {'kode_lokasi':'$kode_lokasi','no_aju':no_aju},
                success:function(result){
                  if(result.status){
                      if(result.daftar.length > 0){
                        $('#no_aju').val(result.daftar[0].no_pb);
                        $('#nilai').val(result.daftar[0].nilai);
                        $('#modul').val(result.daftar[0].modul);
                        $('#tgl_aju').val(result.daftar[0].tgl);
                        $('#deskripsi').val(result.daftar[0].keterangan);

                        if(result.daftar2.length > 0 ){
                            var html=``;
                            for(var i=0;i<result.daftar2.length;i++){
                                var line = result.daftar2[i];
                                html+=`<tr>
                                    <td>`+line.bank+`</td>
                                    <td>`+line.nama+`</td>
                                    <td>`+line.no_rek+`</td>
                                    <td>`+line.nama_rek+`</td>
                                    <td>`+line.bruto+`</td>
                                    <td>`+line.pajak+`</td>
                                    <td>`+line.keterangan+`</td>
                                </tr>`;
                            }
                            $('#table-rek tbody').html(html);
                        }
                        
                        
                        if(result.daftar3.length > 0 ){
                            var html=``;
                            for(var i=0;i<result.daftar3.length;i++){
                                var line = result.daftar3[i];
                                html+=`<tr>
                                    <td>`+line.kode_akun+`</td>
                                    <td>`+line.nama_akun+`</td>
                                    <td>`+line.dc.toUpperCase()+`</td>
                                    <td>`+line.keterangan+`</td>
                                    <td>`+line.nilai+`</td>
                                    <td>`+line.kode_pp+`</td>
                                    <td>`+line.nama_pp+`</td>
                                    <td>`+line.kode_drk+`</td>
                                    <td>`+line.nama_drk+`</td>
                                </tr>`;
                            }
                            $('#table-ju tbody').html(html);
                        }
                        // alert(result.message);
                        $('#validate').removeClass('disabled');
                        $('#validate').prop('disabled', false);
                      }else{
                        alert(result.message);
                      }
                  }
                },
                fail: function(xhr, textStatus, errorThrown){
                  alert('request failed:'+textStatus);
                }
              });
          }

          var iconloading = $('#loading-overlay');
          $('#barcode').focus();
          
          $('#validate').addClass('disabled');
          $('#validate').prop('disabled', true);

          $('#sai_form_insert').submit(function(e){
            e.preventDefault();
            iconloading.show();
            var formData = new FormData(this);
            
            for(var pair of formData.entries()) {
              console.log(pair[0]+ ', '+ pair[1]); 
            }
            var kode_lokasi = '$kode_lokasi';
            var kode_pp = '$kode_pp';
            var nik_user = '$nik';
            formData.append('kode_lokasi',kode_lokasi);
            formData.append('kode_pp',kode_pp);
            formData.append('nik_user',nik_user);
            
            $.ajax({
              type: 'POST',
              url: '$root_ser/DokFisik.php?fx=simpanDokFisik',
              dataType: 'json',
              data: formData,
              contentType: false,
              cache: false,
              processData: false, 
              success:function(result){
                alert('Simpan data '+result.message);
                if(result.status){
                    iconloading.hide();
                    var myFrame = $('#hasil').contents().find('body');
                    myFrame.html(result.html);
                    var header = $('#hasil').contents().find('head');
                    header.html('<title>Preview</title>');
                    $('#hasil2').html(result.html);
                    $('#sai_hasil').show();
                    $('#sai_form_insert').hide();
                    setTimeout(function(){ 
                      window.frames['hasil'].focus();
                      window.frames['hasil'].print();
                    }, 1500);
                    
                    // var w=window.open();
                    // w.document.write($('#hasil2').html());
                    // w.print();
                    // w.close();
                    // $('#printButton').attr('data-periode', result.periode);
                    // $('#printButton').attr('data-no_aju', result.no_aju);
                    // window.parent.system.getResource(".$resource.").doOpen('server_report_saku2_kopeg_kbitt_rptBebanFormDok',result.filter,result.filter);
                }
              },
              fail: function(xhr, textStatus, errorThrown){
                alert('request failed:'+textStatus);
              }
            });
          });

          $('#sai_containerx').on('click', '#printButton', function(e){
            e.preventDefault();
            var periode = $(this).data('periode');
            var no_aju = $(this).data('no_aju');
            // window.open('$root/web/server/ypt/printPreview.php?kode_lokasi=$kode_lokasi&no_aju='+no_aju+'&periode='+periode,'_blank'); 
              // var w=window.open();
              // w.document.write($('#hasil').html());
              // w.print();
              // w.close();
              window.frames['hasil'].focus();
              window.frames['hasil'].print();
          });

          
          $('#sai_containerx').on('click', '#backButton', function(e){
            e.preventDefault();
            $('#sai_hasil').hide();
            $('#no_aju').val('');
            $('#nilai').val(0);
            $('#modul').val('');
            $('#tgl_aju').val('');
            $('#deskripsi').val('');
            $('#barcode').val('');
            $('#validate').addClass('disabled');
            $('#validate').prop('disabled', true);
            $('#sai_form_insert').show();
            $('#barcode').focus();
            // location.reload();
          });

          $('.currency').inputmask('numeric', {
              radixPoint: ',',
              groupSeparator: '.',
              digits: 2,
              autoGroup: true,
              rightAlign: true,
              oncleared: function () { self.Value(''); }
          });

          $('#barcode').scannerDetection({
        
              //https://github.com/kabachello/jQuery-Scanner-Detection
      
              timeBeforeScanTest: 200, // wait for the next character for upto 200ms
              avgTimeByChar: 40, // it's not a barcode if a character takes longer than 100ms
              preventDefault: true,
      
              endChar: [13],
              onComplete: function(barcode, qty){
              validScan = true;
                  $('#barcode').val(barcode);
                  loadDok();
              
              } // main callback function	,
              ,
              onError: function(string, qty) {
                  console.log('barcode-error');
              }	
          });
          </script>      
        ";
		return "";
	}
	
}
?>
