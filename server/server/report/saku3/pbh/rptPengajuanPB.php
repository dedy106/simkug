...
class server_report_saku3_pbh_rptPengajuanPB extends server_report_basic
{
	...
	function getHtml()
	{
        ...
        $status=$tmp[3];
				$jenis=$tmp[4];
				$kode_pp=$tmp[5];
		$sql="select a.periode,convert(varchar,a.tanggal,103) as tgl,a.no_pb,a.kode_lokasi,a.keterangan,a.nik_ver,b.nama as nama_ver,
		a.nik_app,c.nama as nama_app,substring(a.periode,1,4) as tahun,d.kota,a.tanggal,b.email
from pbh_pb_m a
...
order by a.no_pb";
		...
		echo "<body >"; 
		echo $AddOnLib->judul_laporan("laporan pengajuan",$this->lokasi,$tahun);
  ...
  <tr>
    <td colspan='10' style='padding:5px'><table width='622' border='0' cellspacing='2' cellpadding='1'>
      ...
      <tr>
        ...
        <td class='header_laporan'>:&nbsp;$row->no_pb</td>
        </tr>
...
<td width='90' align='center' class='header_laporan'>Debit</td>
...
order by a.dc desc ";
  ...
  <tr>
    <td align='right' colspan='10'><table border='0' cellspacing='2' cellpadding='1'>
      ...
      <tr>
        <td class='header_laporan'>$row->nama_ver</td>
        <td class='header_laporan'>$row->nama_app</td>
      </tr>
      <tr>
        <td class='header_laporan'>$row->nik_ver</td>
