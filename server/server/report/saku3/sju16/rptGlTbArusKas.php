...
class server_report_saku3_sju16_rptGlTbArusKas extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$trail=$tmp[3];
		$kode_fs=$tmp[4];
		$sql="exec sp_glma_aruskas '$kode_lokasi','$periode','$nik_user' ";
		...
		if ($trail=="1")
		{
			$sql = "select a.kode_akun,a.nama,a.debet,a.kredit,a.so_akhir,a.kode_lokasi
		from glma_tmp a
				inner join relakunkas b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_fs='$kode_fs'
...
";
    ...
    <td class='header_laporan' align='right'>".number_format($so_akhir,0,',','.')."</td>
		...
		$sql = "select count(a.kode_akun) as jum
			...
			where a.nik_user='$nik_user' and a.kode_lokasi='$kode_lokasi' and (a.so_awal<>0 or a.debet<>0 or a.kredit<>0 or a.so_akhir<>0) 
				  and b.kode_flag not in ('001','009') and a.kode_akun not in (select kode_akun from relakunkas where kode_lokasi='$kode_lokasi' and kode_fs='$kode_fs')
			";
