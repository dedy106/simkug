...
class server_report_saku3_sju16_rptJuRincianPremi extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$kode_lokasi=$tmp[0];
		$periode=$tmp[1];
		$sql="select a.no_bukti as no_bukti,a.keterangan,a.tanggal,a.kode_lokasi,convert(varchar(20),a.tanggal,103) as tgl,c.nama as nama_lokasi
		from trans_m a
  ...
  <tr>
    <td><table border='1' cellpadding='0' cellspacing='0' class='kotak'>
      <tr>
		...
		$sql3="select b.kode_tipe,c.nama 
...
group by b.kode_tipe,c.nama ";
		...
		$rs12 = $dbLib->execute($sql3);
		$str = "";
		while ($row12 = $rs12->FetchNextObject($toupper=false))
		{
			      echo "<tr>
        <td colspan='13' class='header_laporan'>$row12->kode_tipe - $row12->nama</td>
        </tr>";
				if($i = 0){$op = "AND";}else{$op = "OR";}
				$str .= " $op b.kode_tipe ='".$row12->kode_tipe."'";
	  			$i=$i+1;

		}
			$sql2="select distinct c.nama as nama_ttg,b.no_dok,a.no_polis,e.nama as nama_vendor,d.premi+d.materai+d.p_cost as premi,d.materai+d.p_cost as biaya,d.fee,d.ppn,d.pph,
...
where a.no_bukti='$row->no_bukti' and a.kode_lokasi='$row->kode_lokasi' $str
";
