...
class server_report_saku3_sju16_rptKbRincianPremi extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$kode_lokasi=$tmp[0];
		$periode=$tmp[1];
		$sql="select b.nama as input,d.nama as periksa,a.no_bukti as no_bukti,a.keterangan,a.tanggal,a.kode_lokasi,convert(varchar(20),a.tanggal,103) as tgl,c.nama as nama_lokasi
		from trans_m a
		inner join lokasi c on a.kode_lokasi=c.kode_lokasi
		inner join sju_ttd e on a.no_bukti=e.no_bukti
		left join karyawan b on e.nik_buat=b.nik
		left join karyawan d on e.nik_periksa=d.nik

        $this->filter and  a.form in ('TRM_PREMI','BYR_PREMI','FEE_PREMI')
		order by a.no_bukti ";
    ...
    <td><table width='400' border='1' cellspacing='0' cellpadding='0' class='kotak'>
      ...
      <tr>
        ...
        <td>&nbsp;</td>
				...
				<tr>
        <td align='center'>$row->input</td>
        <td align='center'>$row->periksa</td>
				</tr>
    </table></td>
  </tr>
  ...
  <tr>
    <td align='center' class='istyle17'>RINCIAN PENERIMAAN PREMI </td>
  </tr>
  <tr>
  ...
  <tr>
    <td><table border='1' cellpadding='0' cellspacing='0' class='kotak'>
      <tr>
		...
		<td width='50' align='center' class='header_laporan'>Kurs</td>
...
group by b.kode_tipe,c.nama ";
      ...
      <tr>";
			$sql="select distinct c.nama as nama_ttg,b.no_dok,b.no_dok2,a.no_polis,e.nama as nama_vendor,d.premi+d.materai+d.p_cost as premi,d.materai+d.p_cost as biaya,d.fee,d.ppn,d.pph,
...
where a.no_bukti='$row->no_bukti' and a.kode_lokasi='$row->kode_lokasi' and b.kode_tipe='$row1->kode_tipe'
";
