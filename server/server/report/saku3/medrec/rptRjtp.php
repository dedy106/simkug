...
class server_report_saku3_medrec_rptRjtp extends server_report_basic
{
	function getTotalPage()
	{
		...
		$periode=$tmp[2];
		$filter2=$tmp[3];
		$sql="select count(a.nik) from dbexs.dbo.exs_harian a 
...
$this->filter";
	...
	function getHtml()
	{
		...
		$periode=$tmp[0];
		$nama_cab=$tmp[1];
		$sql="SELECT     a.periode,a.nama, g.jenis,a.no_daftar, a.no_resep, a.no_rujukan, a.tanggal, a.nama_mitra, a.nama_dokter1, a.nama_dokter2, a.kode_tpk, a.nik, a.band_posisi, a.hr_area, a.loker, a.nik_kes, a.nama_pasien, 
					  ...
					  a.umum,a.gigi,a.kbia,a.matkes
FROM dbexs.dbo.exs_harian a
left join dbexs.dbo.exs_klp_biaya b on a.n22 between b.awal and b.akhir
left join dbexs.dbo.exs_ina_drg c on substring(a.kode_produk,1,1)=c.kode_biaya and a.icdx=c.icdx
left join yk_icdx_relasi d on a.icdx=d.kode_icdx 
left join yk_icdx e on a.icdx=e.kode_icdx
inner join dbsimkug.dbo.yk_loker f on a.hr_area=f.loker
inner join dbsimkug.dbo.cust g on f.kode_cust=g.kode_cust
$this->filter order by a.nik ";
		...
		echo $AddOnLib->judul_laporan("biaya pengobatan rjtp",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr align='center' bgcolor='#B9FFCF'>
	...
	<td class='header_laporan' width='150' rowspan='2'>JENIS</td>
    ...
    <td class='header_laporan' colspan='4' align='center'>BIAYA KUNJUNGAN</td>
    <td class='header_laporan' width='60' rowspan='2' align='center'>TOTAL KUNJUNGAN</td>
  ...
  <tr bgcolor='#B9FFCF'>
	...
	<td class='header_laporan' width='80' align='center'>UMUM</td>
	<td class='header_laporan' width='80' align='center'>GIGI</td>
	<td class='header_laporan' width='80' align='center'>KBIA</td>
	<td class='header_laporan' width='80' align='center'>MATKES</td>
  </tr> ";
		$i=1; $nilai=0;
		...
		$umum=0; $gigi=0; $kbia=0; $matkes=0; $gigi=0;
	...
	<td class='isi_laporan' align='right'>".number_format($row->umum,0,',','.')."</td>
	<td class='isi_laporan' align='right'>".number_format($row->gigi,0,',','.')."</td>
	<td class='isi_laporan' align='right'>".number_format($row->kbia,0,',','.')."</td>
	<td class='isi_laporan' align='right'>".number_format($row->matkes,0,',','.')."</td>
    ...
    <td class='isi_laporan' align='right'>".number_format($persen,2,',','.')."</td>
		...
		echo "<tr>
    <td class='isi_laporan' colspan='15'>Total</td>
