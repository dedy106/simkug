...
class server_report_saku3_medrec_rptRekapProdukLokasi extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$periode=$tmp[0];
		$nama_cab=$tmp[1];
		$sql="SELECT   a.kode_lokasi,h.nama as nama_lokasi,e.kode_produk,g.jenis,e.nama,
...
inner join dbexs.dbo.exs_produk e on substring(a.kode_produk,1,1)=e.kode_produk
inner join dbsimkug.dbo.yk_loker f on a.hr_area=f.loker
inner join dbsimkug.dbo.cust g on f.kode_cust=g.kode_cust
inner join dbsimkug.dbo.lokasi h on a.kode_lokasi=h.kode_lokasi
$this->filter
group by a.kode_lokasi,h.nama,e.kode_produk,g.jenis,e.nama
order by a.kode_lokasi,g.jenis,e.kode_produk ";
		
		$rs = $dbLib->execute($sql);
		$AddOnLib=new server_util_AddOnLib();	
		$i=1;
		echo "<div align='center'>"; 
		echo $AddOnLib->judul_laporan("rekap biaya pengobatan per layanan",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
		echo "<table border='1' cellspacing='0' cellpadding='0' class='kotak'>
  <tr align='center' bgcolor='#B9FFCF'>
    <td class='header_laporan' rowspan='2'>No</td>
	<td class='header_laporan' rowspan='2'>AREA</td>
	<td class='header_laporan' width='120' rowspan='2'>LAYANAN</td>
	<td class='header_laporan' rowspan='2'>JENIS</td>
    <td class='header_laporan' colspan='20'>REKAP BIAYA PENGOBATAN PER AREA</td>
