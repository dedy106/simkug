...
class server_report_saku3_tpcc_rptPiutangSaldoCustUnbill extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql2=""; $sql3="";
			...
			$sql2=" and a.modul='UNBILL' ";
			$sql3=" and b.modul='UNBILL' ";
			$nama_jenis="<br>Piutang UNBILLING";
		
			$sql="select a.kode_cust,a.nama,isnull(b.nilai,0) as tagihan,
		isnull(b.nilai_ppn,0) as nilai_ppn,isnull(b.nilai,0)+isnull(b.nilai_ppn,0) as total,isnull(b.nilai_kas,0) as nilai_kas,isnull(b.nilai,0)+isnull(b.nilai_ppn,0)-isnull(b.nilai_kas,0) as saldo
 from cust a
 inner join (select a.kode_cust,a.kode_lokasi,
				sum(a.nilai) as nilai,sum(a.nilai_ppn) as nilai_ppn,sum(isnull(b.nilai,0)) as nilai_kas
		 from piutang_d a 
		 left join (select a.no_dokumen as no_piutang,a.kode_lokasi,sum((a.nilai+a.nilai_ppn)*-1) as nilai
				 from piutang_d a 
				 where a.kode_lokasi='$kode_lokasi' and(a.periode <= '$periode') and a.modul in ('REVUBILL') 
				 group by a.no_dokumen,a.kode_lokasi
				 )b on a.no_piutang=b.no_piutang and a.kode_lokasi=b.kode_lokasi 
		 where(a.kode_lokasi = '$kode_lokasi')and(a.periode <= '$periode') and a.modul in ('UNBILL')
...
order by a.kode_cust";
		...
		echo $AddOnLib->judul_laporan("saldo piutang unbill per customer",$this->lokasi,"PERIODE SD ".$AddOnLib->ubah_periode($periode).$nama_jenis);
   ...
   <tr bgcolor='#CCCCCC'>
     ...
     <td width='100'  align='center' class='header_laporan'>Reverse</td>
	 <td width='100'  align='center' class='header_laporan'>Saldo</td>
   </tr>
   
   ";
		$total=0;$tagihan=0;$nilai_kas=0;$saldo=0;
