...
class server_report_saku3_tpcc_proyek_rptDetHutang extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$tahun=$tmp[1];
		$filter2=$tmp[2];
		$sql=" select a.no_bukti,a.periode,a.nilai,a.dc,convert(varchar,c.tanggal,103) as tgl,a.kode_lokasi
         from hutbayar_d a 
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
            ...
            <td class='isi_laporan'><a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenJurnal('$row->no_bukti','$row->kode_lokasi');\">$row->no_bukti</a></td>
