...
class server_report_saku3_tm_rptKbTbAkunTm extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$rs = $dbLib->execute($sql);
		$sql = "select a.kode_akun,b.nama,a.kode_lokasi,sum(a.debet) as debet,sum(a.kredit) as kredit,
       sum(case when a.so_awal>0 then so_awal else 0 end) as so_awal_debet,
       sum(case when a.so_awal<0 then -so_awal else 0 end) as so_awal_kredit, 
       sum(case when a.so_akhir>0 then so_akhir else 0 end) as so_akhir_debet,
       sum(case when a.so_akhir<0 then -so_akhir else 0 end) as so_akhir_kredit
		from glma_tmp a 
       ...
       case when a.so_akhir<0 then -so_akhir else 0 end as so_akhir_kredit
		...
		echo $AddOnLib->judul_laporan("saldo akun kasbank",$this->lokasi,"Periode ".$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'> 
	...
	echo "<td class='isi_laporan'><a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_akun','$row->kode_lokasi','$periode');\">$row->kode_akun</a></td>";
