...
class server_report_saku3_tm_rptPiutangAging extends server_report_basic
{
	...
	function getHtml()
	{
		...
		if($tgl_aging == ""){
			...
			$filta = " and periode <='".substr($tgl_aging,0,4).substr($tgl_aging,5,2)."' ";
		}
	// 	$sql="select a.kode_lokasi,a.no_dokumen,a.no_piutang,date_format(a.tanggal,'%d/%m/%Y') as tgl,b.nama as nama_cust,a.nilai+a.nilai_ppn as tagihan,
	...
	//    case when datediff(day,a.tanggal,'$tgl_aging') between 91 and 180 then a.nilai+a.nilai_ppn else 0 end as aging4,	   
    //    case when datediff(day,a.tanggal,'$tgl_aging') > 180 then a.nilai+a.nilai_ppn else 0 end as aging5,
	//    isnull(c.nilai_kas,0) as nilai_kas,a.nilai+a.nilai_ppn-isnull(c.nilai_kas,0)  as saldo
	//    from piutang_m a
	...
	// 	$this->filter  order by a.no_piutang";
		$sql="select a.kode_lokasi,a.no_dokumen,a.no_piutang,CONVERT(varchar, a.tanggal, 103) as tgl,c.nama as nama_cust,a.nilai+a.nilai_ppn as tagihan, isnull(b.bayar,0) as bayar,datediff(day,a.tanggal,'$tgl_aging') as umur
		,case when datediff(day,a.tanggal,'$tgl_aging')  between 0 and 30
			  then a.nilai+a.nilai_ppn - isnull(b.bayar,0)
		 else 0 end as aging1
		,case when datediff(day,a.tanggal,'$tgl_aging')  between 31 and 60
			  then a.nilai+a.nilai_ppn - isnull(b.bayar,0)
		 else 0 end as aging2
		 ,case when datediff(day,a.tanggal,'$tgl_aging')  between 61 and 90
			  then a.nilai+a.nilai_ppn - isnull(b.bayar,0)
		 else 0 end as aging3
		 ,case when datediff(day,a.tanggal,'$tgl_aging')  between 91 and 180
			  then a.nilai+a.nilai_ppn - isnull(b.bayar,0)
		 else 0 end as aging4
		 ,case when datediff(day,a.tanggal,'$tgl_aging') > 180
			  then a.nilai+a.nilai_ppn - isnull(b.bayar,0)
		 else 0 end as aging5,
		 a.nilai+a.nilai_ppn - isnull(b.bayar,0) as saldo
		from piutang_m a 
		inner join cust2 c on a.kode_cust=c.kode_cust and a.kode_lokasi=c.kode_lokasi
		left join (
		select no_piutang,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar 
		from piubayar_d
		where kode_lokasi='$kode_lokasi' $filta
   ...
   <tr bgcolor='#CCCCCC'>
	 ...
	 <td colspan='5'  align='center' class='header_laporan'>Aging</td>
   ...
   <tr bgcolor='#CCCCCC'>
     ...
     <td width='90'  align='center' class='header_laporan'>90-180 HARI</td>
     <td width='90'  align='center' class='header_laporan'> > 180 HARI</td>
   </tr>  ";
		$tagihan=0;$nilai_kas=0;$aging1=0;$aging2=0;$aging3=0;$aging4=0;$aging5=0;$saldo=0;
	 ...
	 echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenKartu('$row->no_piutang','$row->kode_lokasi','$periode');\">".number_format($row->bayar,0,",",".")."</a>";
		echo "</td>
