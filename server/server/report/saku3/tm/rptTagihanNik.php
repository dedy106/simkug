...
class server_report_saku3_tm_rptTagihanNik extends server_report_basic
{
	...
	function getHtml()
	{
		...
		//error_log($bank);
			$sql = "select a.no_selesai,a.keterangan,d.kota,a.tanggal,d.nama as lokasi,a.kode_lokasi,
  ...
  <tr align='center' bgcolor='#CCCCCC'>
    ...
    <td class='isi_laporan'>14</td>	
		...
		$sql1 = "select a.nik, a.nama as karyawan, a.kode_produk, d.nama as nmprod, date_format(tgl_masuk,'%d-%m-%Y') as tgl1, date_format(tgl_keluar,'%d-%m-%Y') as tgl2,icdx, nikkes+'/'+pasien as pasien, 
 e.nama as nama,a.kode_lokasi,a.kode_vendor,
...
where a.kode_lokasi='$kode_lokasi' and a.no_hutang='$row->no_hutang' $bank $vendor
  ...
  <tr>
	...
	function doPrintHeader($sender, $page){		
		if ($sender->changeHeader || $page == 1){
			...
			$sender->Cell(0,5,"NO ".$sender->row->no_hutang,0,1,'C');
		}
	...
	{		
		...
		$sql = "select a.no_hutang,a.keterangan,d.kota,a.tanggal,d.nama as lokasi,
				   b.nama as nama_buat,c.nama as nama_app,b.jabatan as jab_buat,c.jabatan as jab_app,
				   f.nama as nama_area,f.jabatan as jab_area
			from yk_hutang_m a 
		...
		while ($row = $rs->FetchNextObject($toupper=false)){			
			$pdf->changeHeader = $tmp != $row->no_hutang;
			$pdf->row = $row;
			if ($tmp == ""){
				 $pdf->AddPage();
			...
			/*$sql1 = "select a.kode_vendor,a.nama,a.bank,a.cabang,a.no_rek,a.nama_rek,e.nik,f.nama as karyawan,
				...
				from (select a.kode_vendor,a.kode_lokasi,a.nik
							from yk_bill_d a 
						where a.kode_lokasi='$kode_lokasi' and a.periode='$periode' and a.no_hutang='$row->no_hutang'
			...
			*/
			$sql1 = "select  a.nik, e.nama as karyawan, a.kode_produk, d.nama as nmprod, date_format(tgl_masuk,'%d-%m-%Y') as tgl1, date_format(tgl_keluar,'%d-%m-%Y') as tgl2,icdx, nikkes+' / '+pasien as pasien, a.kode_vendor+' / '+ b.nama as nama,a.kode_lokasi, a.kode_vendor, 
				   ...
				   where a.kode_lokasi='$kode_lokasi' and a.periode='$periode' and a.no_hutang='$row->no_hutang' $fbank $vendor order by a.kode_vendor";
			$rs1 = $dbLib->execute($sql1);
			$debet_bp=0;$debet_cc=0;$debet_total=0; $i=1; $pph = 0;
