...
class server_report_saku3_tm_rptPiutangSaldoCust2 extends server_report_basic
{
	...
	function getHtml()
	{
		...
		sum(a.nilai+a.nilai_ppn) as nilai,sum(a.nilai_ppn) as nilai_ppn,sum(a.nilai) as tagihan,
	   sum(isnull(c.nilai_kas,0)) as nilai_kas,sum(a.nilai+a.nilai_ppn-isnull(c.nilai_kas,0)) as saldo 
from piutang_m a
...
order by b.kode_cust";
		...
		echo $AddOnLib->judul_laporan("saldo piutang per customer",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
