...
class server_report_saku3_tm_rptTakKirim extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$tmp=explode("/",$this->filter2);
		$periode=$tmp[0];
		$sql="select a.no_kirim,a.no_dokumen,a.tanggal,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.kode_loktuj,b.nama as nama_lokasi,a.keterangan,b.kota,
...
left join  karyawan e on a.nik_setuju=e.nik and a.kode_lokasi=e.kode_lokasi
$this->filter
order by a.no_kirim";
		$rs = $dbLib->execute($sql);
...
where a.no_kirim='$row->no_kirim' order by a.dc desc";
