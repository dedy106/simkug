...
class server_report_saku3_sla_rptSaldoCocd extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$periode=$tmp[0];
		$nama_cab=$tmp[1];
		$sql="select a.cocd,a.company_name as nama,isnull(b.nilai,0) as nilai,isnull(b.biaya,0) as biaya,
...
from mysym_company_code a
left join (select a.kode_cocd,sum(a.nilai) as nilai,sum(a.biaya) as biaya
		   from sla_kkp_m a
		   group by a.kode_cocd
		   )b on a.cocd=b.kode_cocd 
left join (select b.kode_cocd,
		   ...
		   group by b.kode_cocd
		   )c on a.cocd=c.kode_cocd 
order by a.cocd";
		//echo $sql;
		$rs = $dbLib->execute($sql);
   ...
   <tr bgcolor='#CCCCCC'>
     <td width='30'  align='center' class='header_laporan'>No</td>
	  <td width='200'  align='center' class='header_laporan'>Company</td>
	...
	echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenSaldo('$row->cocd');\">$row->nama</a></td>
