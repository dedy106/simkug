...
class server_report_saku3_sla_rptSaldoClass extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$periode=$tmp[0];
		$nama_cab=$tmp[1];
		$sql="select a.kode_class,a.nama,isnull(b.nilai,0) as nilai,isnull(b.biaya,0) as biaya,
...
from sla_class a
left join (select a.kode_class,sum(a.nilai) as nilai,sum(a.biaya) as biaya
		   from sla_kkp_m a
		   group by a.kode_class
		   )b on a.kode_class=b.kode_class 
left join (select b.kode_class,
		   ...
		   group by b.kode_class
		   )c on a.kode_class=c.kode_class 
order by a.kode_class";
   ...
   <tr bgcolor='#CCCCCC'>
     <td width='30'  align='center' class='header_laporan'>No</td>
	  <td width='200'  align='center' class='header_laporan'>Classification</td>
	...
	echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenSaldo('$row->kode_class');\">$row->nama</a></td>
