...
class server_report_saku3_cianjur_rptBuktiBank extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$periode=$tmp[2];
		$filter2=$tmp[3];
		$sql="select a.no_bukti,a.no_dokumen,a.kode_lokasi,a.periode,a.tanggal,convert(varchar,a.tanggal,103) as tanggal1,a.keterangan,a.kode_lokasi,a.modul,
       a.nik1,b.nama as nama_buat,b.jabatan as jabatan_buat,a.nik1,c.nama as nama_setuju,c.jabatan as jabatan_setuju,d.kota
		...
		left join karyawan c on a.nik1=c.nik and a.kode_lokasi=c.kode_lokasi $this->filter ";
	  ...
	  $sql1="select a.kode_akun,b.nama,a.keterangan,a.kode_pp,a.kode_drk,case dc when 'D' then nilai else 0 end as debet,case dc when 'C' then nilai else 0 end as kredit  
			...
			where a.no_bukti='$row->no_bukti' and a.kode_lokasi='$row->kode_lokasi' and a.modul='$row->modul'
			order by a.nu ";
		//error_log($sql1);
