...
class server_report_saku3_dw_rptDwLabaRugiFakultasJejer extends server_report_basic
{
	...
	function getHtml()
	{
		...
		echo $AddOnLib->judul_laporan("LAPORAN REKAP ANGGARAN LABARUGI JEJER FAKULTAS",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
    <td rowspan='2' align='center' width='300' class='header_laporan'>Deskripsi</td>";
		$sql="select kode_fakultas,nama from exs_fakultas where kode_lokasi='$kode_lokasi' order by kode_fakultas";
		$rs = $dbLib->execute($sql);
		$tmp_sql="";$jum=0;$tmp_sql2="";
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			$fak[$jum]=$row->kode_fakultas;
			$pdpt[$jum]="n2".strtolower($row->kode_fakultas);
			$jum=$jum+1;
			$fak[$jum]=$row->kode_fakultas;
			$pdpt[$jum]="n4".strtolower($row->kode_fakultas);
			$jum=$jum+1;
			$tmp_sql.="sum(case when kode_fakultas='$row->kode_fakultas' then (case when jenis_akun='Pendapatan' then -n2 else n2 end) else 0 end) as 'n2$row->kode_fakultas',
					sum(case when kode_fakultas='$row->kode_fakultas' then (case when jenis_akun='Pendapatan' then -n4 else n4 end) else 0 end) as 'n4$row->kode_fakultas',";
			$tmp_sql2.="sum(case when f.kode_fakultas='$row->kode_fakultas' then (case when c.jenis='Pendapatan' then -n2 else n2 end) else 0 end) as 'n2$row->kode_fakultas',
					sum(case when f.kode_fakultas='$row->kode_fakultas' then (case when c.jenis='Pendapatan' then -n4 else n4 end) else 0 end) as 'n4$row->kode_fakultas',";

    echo "<td colspan='2' align='center' class='header_laporan'>$row->kode_fakultas - $row->nama</td>";
  ...
  echo "</tr>
  <tr bgcolor='#CCCCCC'>";
		$sql="select nama from exs_fakultas where kode_lokasi='$kode_lokasi' order by kode_fakultas";
		$rs = $dbLib->execute($sql);
		while ($row = $rs->FetchNextObject($toupper=false))
		{
...
group by kode_neraca";
  ...
  <tr class='info'>
    ...
    $content .= "<td colspan='2' align='center' class='header_laporan'>$row->kode_fakultas - $row->nama</td>";
