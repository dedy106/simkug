...
class server_report_saku3_dw_rptDwLabaRugiProdi extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$kode_prodi=$tmp[3];
		$jenis=$tmp[4];
		$nama_file="labarugi_prodi_".$periode.".xls";
		...
		$sql="select a.kode_prodi,a.nama from exs_prodi a $kode_prodi order by a.kode_prodi";
		$rs2 = $dbLib->execute($sql);
		while ($row2 = $rs2->FetchNextObject($toupper=false))
		{
			$nama=$row2->nama;
			$sql = "select a.kode_neraca,a.kode_fs,a.kode_lokasi,a.nama,a.tipe,a.level_spasi,a.kode_prodi,
				...
				$this->filter and a.modul='L' and a.kode_prodi='$row2->kode_prodi'
				order by a.rowindex ";
			$rs = $dbLib->execute($sql);		
			...
			echo $AddOnLib->judul_laporan("laporan laba rugi prodi"."<br>".$nama,$this->lokasi,"Periode ".$AddOnLib->ubah_periode($periode));
		...
		<tr bgcolor='#CCCCCC'>
		  ...
		  <td height='20' class='isi_laporan'>";
				...
				if ($row->kode_neraca!="OR")
				{
					...
					echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenGar('$row->kode_lokasi','$periode','$row->kode_neraca','$row->kode_prodi');\">".number_format($row->n1,0,',','.')."</a>";
					echo "</td>
				<td class='isi_laporan' align='right'>";
					echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenGarSd('$row->kode_lokasi','$periode','$row->kode_neraca','$row->kode_prodi');\">".number_format($row->n2,0,',','.')."</a>";
				echo "</td>
				<td class='isi_laporan' align='right'>";
					echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_lokasi','$periode','$row->kode_neraca','$row->kode_prodi');\">".number_format($row->n4,0,',','.')."</a>";
						echo "</td>
			...
			<td class='isi_laporan' align='center'>".number_format($persen2,2,',','.')."%</td>";
				...
				if ($bentuk=="Detail" && $row->tipe=="Posting")
				{
					...
					$sql1="	select a.kode_akun,c.nama,
							...
							where b.kode_fs='$kode_fs' and a.kode_lokasi='$kode_lokasi' and b.kode_neraca='$kode_neraca' and a.periode='$periode' and e.kode_prodi='$row2->kode_prodi'
	...
	function getHtml2()
	{
		...
		$kode_prodi=$tmp[3];
		$jenis=$tmp[4];
		$nama_file="labarugi_fakultas_".$periode.".xls";
		$tahun=substr($periode,0,4);
		$tahun_rev=substr($periode,0,4)-1;
		if ($jenis=="Excell")
		{
		...
		while ($row2 = $rs2->FetchNextObject($toupper=false))
		{
			...
			$content .= $AddOnLib->judul_laporan("laporan laba rugi fakultas"."<br>".$nama,$this->lokasi,"Periode ".$AddOnLib->ubah_periode($periode));
			...
			<td class='isi_laporan' align='center'>".number_format($persen2,2,',','.')."%</td>";
				...
				if ($bentuk=="Detail" && $row->tipe=="Posting")
				{
					...
					$sql1="	select a.kode_akun,c.nama,
							...
							where b.kode_fs='$kode_fs' and a.kode_lokasi='$kode_lokasi' and b.kode_neraca='$kode_neraca' and a.periode='$periode' and f.kode_prodi='$row2->kode_prodi'
							group by a.kode_akun,c.nama
							order by a.kode_akun ";
