...
class server_report_saku3_dw_rptGlTbPpTu extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql = "select a.kode_akun,c.nama,a.kode_lokasi,a.debet,a.kredit,a.so_awal,so_akhir,a.kode_pp,d.nama as nama_pp,
       ...
       case when a.so_akhir<0 then -so_akhir else 0 end as so_akhir_kredit
		...
		left join exs_rektor g on e.kode_rektor=g.kode_rektor and e.kode_lokasi=g.kode_lokasi
		...
		if ($trail=="1")
		{
		   ...
		   case when a.so_akhir<0 then -so_akhir else 0 end as so_akhir_kredit
			...
			inner join pp d on a.kode_pp=d.kode_pp and a.kode_lokasi=d.kode_lokasi
			left join exs_bidang e on d.kode_bidang=e.kode_bidang and d.kode_lokasi=e.kode_lokasi
			left join exs_fakultas_bidang f on e.kode_bidang=f.kode_bidang and e.kode_lokasi=f.kode_lokasi
			left join exs_rektor g on e.kode_rektor=g.kode_rektor and e.kode_lokasi=g.kode_lokasi
			$this->filter and (a.so_awal<>0 or a.debet<>0 or a.kredit<>0 or a.so_akhir<>0) 
			order by a.kode_akun,a.kode_pp";
		}
  ...
  <tr bgcolor='#CCCCCC'>
	...
	<td width='250' rowspan='2' class='header_laporan' align='center'>Nama PP</td>
...
<td class='isi_laporan' align='right'>".number_format($row->so_awal_debet,0,',','.')."</td>
	...
	echo "<tr>
    <td height='20' colspan='5' class='sum_laporan' align='right'>Total</td>
	...
	function getHtml2()
	{
       ...
       case when a.so_akhir<0 then -so_akhir else 0 end as so_akhir_kredit
		...
		$content .= "<table border='0' cellspacing='0' cellpadding='0' >
		  ...
		  <tr>
			<td align='center'>";
		$content .=  "<table border='1' cellspacing='0' cellpadding='2' style='border-collapse: collapse' bordercolor='#111111'>
		  <tr bgcolor='#CCCCCC'>
			<td width='30' rowspan='2'  class='header_laporan' align='center'>No</td>
			<td width='70' rowspan='2' class='header_laporan' align='center'>Kode Akun</td>
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			$content .=  "<tr>
				...
				<td class='isi_laporan'>$row->kode_akun</td>
				<td height='20' class='isi_laporan'>";
			$content .=  "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenBb('$row->kode_akun','$row->kode_lokasi','$periode');\">$row->nama</a>";
			$content .=  "</td>
