...
class server_report_saku3_dw_rptDwLabaRugiRektorKlp extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$kode_klp=$tmp[3];
		$jenis=$tmp[4];
		$nama_file="labarugi_rektor_".$periode.".xls";
		...
		$sql="select a.kode_klp,a.nama from exs_rektor_klp a $kode_klp order by a.kode_klp ";
		...
		while ($row2 = $rs2->FetchNextObject($toupper=false))
		{
			...
			$sql = "select a.kode_neraca,a.kode_fs,a.kode_lokasi,a.nama,a.tipe,a.level_spasi,a.kode_klp,
				...
				from exs_neraca_rektor_klp a
				$this->filter and a.modul='L' and a.kode_klp='$row2->kode_klp'
				order by a.rowindex ";
			$rs = $dbLib->execute($sql);		
			...
			echo $AddOnLib->judul_laporan("laporan laba rugi"."<br>".$nama,$this->lokasi,"Periode ".$AddOnLib->ubah_periode($periode));
		...
		<tr bgcolor='#CCCCCC'>
		  ...
		  <td height='20' class='isi_laporan'>";
				...
				if ($row->kode_neraca!="OR")
				{
					...
					echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenGar('$row->kode_lokasi','$periode','$row->kode_neraca','$row->kode_klp');\">".number_format($row->n1,0,',','.')."</a>";
					echo "</td>
				<td class='isi_laporan' align='right'>";
					echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenGarSd('$row->kode_lokasi','$periode','$row->kode_neraca','$row->kode_klp');\">".number_format($row->n2,0,',','.')."</a>";
				echo "</td>
				<td class='isi_laporan' align='right'>";
					echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_lokasi','$periode','$row->kode_neraca','$row->kode_klp');\">".number_format($row->n4,0,',','.')."</a>";
						echo "</td>
				...
				if ($bentuk=="Detail" && $row->tipe=="Posting")
				{
					...
					$kode_lokasi=$row->kode_lokasi;
						$sql1="	select a.kode_akun,c.nama,
								...
								sum(case c.jenis when  'Pendapatan' then -n5 else n5 end) as n5
							...
							where b.kode_fs='$kode_fs' and a.kode_lokasi='$kode_lokasi' and b.kode_neraca='$kode_neraca' and a.periode='$periode' and f.kode_klp='$row2->kode_klp'
	...
	function getHtml2()
	{
		...
		$sql="select a.kode_rektor,a.nama from exs_rektor a $kode_rektor order by a.kode_rektor ";
		...
		while ($row2 = $rs2->FetchNextObject($toupper=false))
		{
			...
			$sql = "select a.kode_neraca,a.kode_fs,a.kode_lokasi,a.nama,a.tipe,a.level_spasi,
				...
				$this->filter and a.modul='L' and a.kode_rektor='$row2->kode_rektor'
				order by a.rowindex ";
			$rs = $dbLib->execute($sql);		
			...
			$content .= $AddOnLib->judul_laporan("laporan laba rugi"."<br>".$nama,$this->lokasi,"Periode ".$AddOnLib->ubah_periode($periode));
				...
				if ($bentuk=="Detail" && $row->tipe=="Posting")
				{
					...
					$kode_lokasi=$row->kode_lokasi;
						$sql1="	select a.kode_akun,c.nama,
							...
							inner join masakun c on a.kode_akun=c.kode_akun and a.kode_lokasi=c.kode_lokasi
							inner join pp d on a.kode_pp=d.kode_pp and a.kode_lokasi=d.kode_lokasi
							inner join exs_bidang e on d.kode_rektor=e.kode_rektor and d.kode_lokasi=e.kode_lokasi
							where b.kode_fs='$kode_fs' and a.kode_lokasi='$kode_lokasi' and b.kode_neraca='$kode_neraca' and a.periode='$periode' and e.kode_rektor='$row2->kode_rektor'
							group by a.kode_akun,c.nama
							order by a.kode_akun ";
