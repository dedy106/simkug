...
class server_report_saku3_dw_rptDwLabaRugiPpJejer extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$bentuk=$tmp[3];
		$jenis=$tmp[4];
		$nama_file="labarugi_".$periode.".xls";
  ...
  <tr bgcolor='#CCCCCC'>
    <td rowspan='2' align='center' width='300' class='header_laporan'>Deskripsi</td>";
		$sql="select kode_pp,nama from pp where kode_lokasi='$kode_lokasi' and flag_aktif='1' order by kode_pp";
		$rs = $dbLib->execute($sql);
    ...
    echo "<td colspan='2' align='center' class='header_laporan'>$row->kode_pp - $row->nama</td>";
  ...
  echo "</tr>
  <tr bgcolor='#CCCCCC'>";
		$sql="select nama from pp where kode_lokasi='$kode_lokasi'  and flag_aktif='1' order by kode_pp";
		$rs = $dbLib->execute($sql);
		while ($row = $rs->FetchNextObject($toupper=false))
		{
