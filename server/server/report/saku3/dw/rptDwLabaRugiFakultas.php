...
class server_report_saku3_dw_rptDwLabaRugiFakultas extends server_report_basic
{
	...
	function getHtml()
	{
		...
		{
			$nama=$row2->nama;
			$sql = "select a.kode_neraca,a.kode_fs,a.kode_lokasi,a.nama,a.tipe,a.level_spasi,a.kode_fakultas,
				...
				from exs_neraca_fakultas a
				$this->filter and a.modul='L' and a.kode_fakultas='$row2->kode_fakultas'
				order by a.rowindex ";
			$rs = $dbLib->execute($sql);		
			...
			echo $AddOnLib->judul_laporan("laporan laba rugi fakultas"."<br>".$nama,$this->lokasi,"Periode ".$AddOnLib->ubah_periode($periode));
		...
		<tr bgcolor='#CCCCCC'>
		  ...
		  <td height='20' class='isi_laporan'>";
				...
				if ($row->kode_neraca!="OR")
				{
					...
					echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenGar('$row->kode_lokasi','$periode','$row->kode_neraca','$row->kode_fakultas');\">".number_format($row->n1,0,',','.')."</a>";
					echo "</td>
				<td class='isi_laporan' align='right'>";
					echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenGarSd('$row->kode_lokasi','$periode','$row->kode_neraca','$row->kode_fakultas');\">".number_format($row->n2,0,',','.')."</a>";
				echo "</td>
				<td class='isi_laporan' align='right'>";
					echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_lokasi','$periode','$row->kode_neraca','$row->kode_fakultas');\">".number_format($row->n4,0,',','.')."</a>";
						echo "</td>
			...
			<td class='isi_laporan' align='center'>".number_format($persen2,2,',','.')."%</td>";
				...
				if ($bentuk=="Detail" && $row->tipe=="Posting")
				{
					...
					$sql1="	select a.kode_akun,c.nama,
							...
							where b.kode_fs='$kode_fs' and a.kode_lokasi='$kode_lokasi' and b.kode_neraca='$kode_neraca' and a.periode='$periode' and f.kode_fakultas='$row2->kode_fakultas'
