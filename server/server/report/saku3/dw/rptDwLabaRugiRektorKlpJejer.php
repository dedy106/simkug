...
class server_report_saku3_dw_rptDwLabaRugiRektorKlpJejer extends server_report_basic
{
	...
	function getHtml()
	{
		...
		echo $AddOnLib->judul_laporan("LAPORAN REKAP ANGGARAN LABARUGI JEJER kelompok rektor",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
    <td rowspan='2' align='center' width='300' class='header_laporan'>Deskripsi</td>";
		$sql="select kode_klp,nama from exs_rektor_klp where kode_lokasi='$kode_lokasi' order by kode_klp";
		$rs = $dbLib->execute($sql);
		
		$tmp_sql="";$jum=0;$tmp_sql2="";
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			$fak[$jum]=$row->kode_klp;
			$pdpt[$jum]="n2".strtolower($row->kode_klp);
			$jum=$jum+1;
			$fak[$jum]=$row->kode_klp;
			$pdpt[$jum]="n4".strtolower($row->kode_klp);
			$jum=$jum+1;
			$tmp_sql.="sum(case when kode_klp='$row->kode_klp' then (case when jenis_akun='Pendapatan' then -n2 else n2 end) else 0 end) as 'n2$row->kode_klp',
					sum(case when kode_klp='$row->kode_klp' then (case when jenis_akun='Pendapatan' then -n4 else n4 end) else 0 end) as 'n4$row->kode_klp',";
			$tmp_sql2.="sum(case when f.kode_klp='$row->kode_klp' then (case when c.jenis='Pendapatan' then -n2 else n2 end) else 0 end) as 'n2$row->kode_klp',
					sum(case when f.kode_klp='$row->kode_klp' then (case when c.jenis='Pendapatan' then -n4 else n4 end) else 0 end) as 'n4$row->kode_klp',";

    echo "<td colspan='2' align='center' class='header_laporan'>$row->kode_klp - $row->nama</td>";
			
		}
		echo "<td colspan='2' align='center' class='header_laporan'>Telkom University</td>";
		$pdpt[$jum]="n2t";
		$jum=$jum+1;
		$pdpt[$jum]="n4t";
		$tmp_sql.="sum(case when jenis_akun='Pendapatan' then -n2 else n2 end) as 'n2t',
				sum(case when jenis_akun='Pendapatan' then -n4 else n4 end) as 'n4t',";
  ...
  echo "</tr>
  <tr bgcolor='#CCCCCC'>";
		$sql="select nama from exs_rektor_klp where kode_lokasi='$kode_lokasi' order by kode_klp";
		$rs = $dbLib->execute($sql);
		while ($row = $rs->FetchNextObject($toupper=false))
		{
    echo "<td align='center' width='80' class='header_laporan'>RKA s.d Bulan Berjalan</td>
...
group by kode_neraca";
			...
			while ($row1 = $rs1->FetchNextObject($toupper=false))
			{
				...
				if ($bentuk=="Detail" && $row->tipe=="Posting")
				{
					...
					$sql1="	select a.kode_akun,c.nama,$tmp_sql2
								...
								inner join exs_rektor f on e.kode_rektor=f.kode_rektor and e.kode_lokasi=f.kode_lokasi
	...
	function getHtml2()
	{
		...
		$content .= $AddOnLib->judul_laporan("LAPORAN REKAP ANGGARAN LABARUGI JEJER rektor",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr class='info'>
    <td rowspan='2' align='center' width='300' class='header_laporan'>Deskripsi</td>";
		$sql="select kode_klp,nama from exs_rektor where kode_lokasi='$kode_lokasi' order by kode_klp";
		$rs = $dbLib->execute($sql);
		$tmp_sql="";$jum=0;$tmp_sql2="";
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			$tmp_sql.="sum(case when kode_klp='$row->kode_klp' then (case when jenis_akun='Pendapatan' then -n2 else n2 end) else 0 end) as 'n2$row->kode_klp',
					sum(case when kode_klp='$row->kode_klp' then (case when jenis_akun='Pendapatan' then -n4 else n4 end) else 0 end) as 'n4$row->kode_klp',";
			$tmp_sql2.="sum(case when e.kode_klp='$row->kode_klp' then (case when c.jenis='Pendapatan' then -n2 else n2 end) else 0 end) as 'n2$row->kode_klp',
					sum(case when e.kode_klp='$row->kode_klp' then (case when c.jenis='Pendapatan' then -n4 else n4 end) else 0 end) as 'n4$row->kode_klp',";

    $content .= "<td colspan='2' align='center' class='header_laporan'>$row->kode_klp - $row->nama</td>";
			
		}
  $content .= "</tr>
  <tr class='info'>";
		$sql="select nama from exs_rektor where kode_lokasi='$kode_lokasi' order by kode_klp";
		$rs = $dbLib->execute($sql);
		while ($row = $rs->FetchNextObject($toupper=false))
		{
