...
class server_report_saku3_dw_rptDwProduktivitas extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql = "select a.kode_lap,a.kode_lokasi,a.nama,ISNULL(b.n2,0) as n2,ISNULL(c.n4,0) as n4
from exs_lap_m a
left join (select a.kode_lap,a.kode_lokasi,sum(case when b.jenis_akun='Pendapatan' then -b.n1 else b.n1 end) as n2
		   from exs_lap_d a
		   inner join exs_neraca b on a.kode_akun=b.kode_neraca and a.kode_lokasi=b.kode_lokasi and a.jenis=b.kode_fs
		   inner join exs_lap_m c on a.kode_lap=c.kode_lap and a.kode_lokasi=c.kode_lokasi
		   where a.kode_lokasi='$kode_lokasi' and c.kode_klp='K03' and b.periode='$periode'
		   group by a.kode_lap,a.kode_lokasi
		   )b on a.kode_lap=b.kode_lap and a.kode_lokasi=b.kode_lokasi
left join (select a.kode_lap,a.kode_lokasi,sum(case when b.jenis_akun='Pendapatan' then -b.n4 else b.n4 end) as n4
		   from exs_lap_d a
		   inner join exs_neraca b on a.kode_akun=b.kode_neraca and a.kode_lokasi=b.kode_lokasi and a.jenis=b.kode_fs
		   inner join exs_lap_m c on a.kode_lap=c.kode_lap and a.kode_lokasi=c.kode_lokasi
		   where a.kode_lokasi='$kode_lokasi' and c.kode_klp='K03' and b.periode='$periode'
...
order by a.kode_klp";
		...
		echo $AddOnLib->judul_laporan("laporan PRODUKTIVITAS PEGAWAI"."<br>".$nama_akun,$this->lokasi,"Periode ".$AddOnLib->ubah_periode($periode));
