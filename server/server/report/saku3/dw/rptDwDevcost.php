...
class server_report_saku3_dw_rptDwDevcost extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql = "select a.kode_lap,a.kode_lokasi,a.nama,ISNULL(b.n1*-1,0) as n2,ISNULL(c.n2,0) as n4
from exs_lap_m a
left join (select a.kode_lap,a.kode_lokasi,
				  sum(case when a.jenis='SA' then b.so_akhir else b.debet-b.kredit end) as n1
		   from exs_lap_d a
		   ...
		   where a.kode_lokasi='$kode_lokasi' and c.kode_klp='K01' and a.dc='D' and b.periode='$periode'
...
left join (select a.kode_lap,a.kode_lokasi,
		   ...
		   inner join exs_glma b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
		   inner join exs_lap_m c on a.kode_lap=c.kode_lap and a.kode_lokasi=c.kode_lokasi
		   where a.kode_lokasi='$kode_lokasi' and c.kode_klp='K01' and a.dc='C' and b.periode='$periode'
		   group by a.kode_lap,a.kode_lokasi
		   )c on a.kode_lap=c.kode_lap and a.kode_lokasi=c.kode_lokasi
where a.kode_klp='K01'
order by a.kode_klp";
		...
		echo $AddOnLib->judul_laporan("laporan FACULTY DEVELOPMENT COST"."<br>".$nama_akun,$this->lokasi,"Periode ".$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
    ...
    <td width='100' class='header_laporan' align='center'>PENDAPATAN OPRASIONAL</td>
    <td width='100' class='header_laporan' align='center'>BIAYA</td>
    <td width='60' class='header_laporan' align='center'>PROSENTASE</td>
  </tr>
  ";
		$deviasi=0; $n2=0; $n4=0;
		$i=1;
