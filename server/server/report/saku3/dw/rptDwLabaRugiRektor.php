...
class server_report_saku3_dw_rptDwLabaRugiRektor extends server_report_basic
{
	...
	function getHtml()
	{
		...
		while ($row2 = $rs2->FetchNextObject($toupper=false))
		{
			...
			$sql = "select a.kode_neraca,a.kode_fs,a.kode_lokasi,a.nama,a.tipe,a.level_spasi,a.kode_rektor,
		...
		<tr bgcolor='#CCCCCC'>
		  ...
		  <td height='20' class='isi_laporan'>";
				...
				if ($row->kode_neraca!="OR")
				{
					...
					echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenGar('$row->kode_lokasi','$periode','$row->kode_neraca','$row->kode_rektor');\">".number_format($row->n1,0,',','.')."</a>";
					echo "</td>
				<td class='isi_laporan' align='right'>";
					echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenGarSd('$row->kode_lokasi','$periode','$row->kode_neraca','$row->kode_rektor');\">".number_format($row->n2,0,',','.')."</a>";
				echo "</td>
				<td class='isi_laporan' align='right'>";
					echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_lokasi','$periode','$row->kode_neraca','$row->kode_rektor');\">".number_format($row->n4,0,',','.')."</a>";
						echo "</td>
