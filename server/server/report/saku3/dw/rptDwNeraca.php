...
class server_report_saku3_dw_rptDwNeraca extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$judul1="Posisi Neraca Per ".$AddOnLib->ubah_periode($periode);
		$judul2="Posisi Neraca Per ".$AddOnLib->ubah_periode($periode_rev);
		echo "<div align='center'>"; 
		echo $AddOnLib->judul_laporan("laporan neraca",$this->lokasi,"");
		echo "<table  border='1' cellpadding='0' cellspacing='0' class='kotak' width='700'>
    <tr bgcolor='#CCCCCC'>
    <td width='500' height='25'  class='header_laporan' align='center'>Keterangan</td>
    <td width='100' class='header_laporan' align='center'>$judul1</td>
	<td width='100' class='header_laporan' align='center'>$judul2</td>
    </tr>
	";
		$sql = "select a.kode_neraca,a.kode_fs,a.kode_lokasi,a.nama,a.tipe,a.level_spasi,a.n1,a.n2,a.n3,a.n4,a.n5
			from exs_neraca a
      ...
      <td height='20' class='isi_laporan'>";
			...
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_lokasi','$periode','$row->kode_neraca');\">".number_format($row->n4,0,',','.')."</a>";
			echo "</td>
		<td class='isi_laporan' align='right'>";
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_lokasi','$periode_rev','$row->kode_neraca');\">".number_format($row->n5,0,',','.')."</a>";
			echo "</td>";
			}
			else
			{
		...
		a.n1*-1 as n1,a.n2*-1 as n2,a.n3*-1 as n3,a.n4*-1 as n4,a.n5*-1 as n5
			from exs_neraca a
