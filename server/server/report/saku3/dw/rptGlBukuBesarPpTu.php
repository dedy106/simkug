...
class server_report_saku3_dw_rptGlBukuBesarPpTu extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql = "select a.kode_lokasi,a.kode_akun,a.kode_pp,b.nama,a.so_awal,a.periode,case when a.so_awal>=0 then a.so_awal else 0 end as so_debet,case when a.so_awal<0 then a.so_awal else 0 end as so_kredit
				from exs_glma_pp a 
...
from (select a.kode_lokasi,a.no_bukti,a.no_dokumen,a.periode,a.tanggal,a.kode_akun,a.kode_pp,a.kode_drk,a.dc,a.nilai,a.keterangan 
	  from gldt a 
	  where a.kode_lokasi='$row->kode_lokasi' and a.kode_akun='$row->kode_akun' and (a.periode between '$periode_awal' and '$periode') and a.kode_pp='$row->kode_pp'
	  union all
