...
class server_report_saku3_bpr_rptPbPos extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="select a.no_bukti,a.kode_lokasi,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.keterangan,a.posted,a.nilai1,a.progress,a.kode_pp,b.nama as nama_pp,
       a.no_ref2,date_format(c.tanggal,'%d/%m/%Y') as tgl_app,
       a.no_ref3,date_format(g.tanggal,'%d/%m/%Y') as tgl_kas
from trans_m a
inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi
left join pb_app_m c on a.no_ref2=c.no_app and a.kode_lokasi=c.kode_lokasi
left join kas_m g on a.no_ref3=g.no_kas and a.kode_lokasi=g.kode_lokasi
$this->filter order by a.no_bukti ";
   ...
   <tr bgcolor='#CCCCCC'>
	 ...
	 <td width='50'  align='center' class='header_laporan'>Nama PP</td>
   <td width='200'  align='center' class='header_laporan'>Keterangan</td>
	 <td width='40'  align='center' class='header_laporan'>Nilai</td>
	 <td width='20'  align='center' class='header_laporan'>Posting</td>
	 <td width='20'  align='center' class='header_laporan'>Status</td>
	 <td width='50'  align='center' class='header_laporan'>No Ver</td>
	 <td width='60'  align='center' class='header_laporan'>Tgl Ver</td>
	 <td width='50'  align='center' class='header_laporan'>No KasBank</td>
	 <td width='60'  align='center' class='header_laporan'>Tgl KasBank</td>
	  </tr> ";
		$bruto=0;$npajak=0;$nilai1=0;
	...
	<td class='isi_laporan'>$row->tgl</td>
	 ...
	 <td class='isi_laporan' align='left'>".number_format($row->nilai1,0,",",".")."</td>
	 <td class='isi_laporan'>$row->posted</td>
	 <td class='isi_laporan'>$row->progress</td>
	 <td class='isi_laporan'>$row->no_ref2</td>
	 <td class='isi_laporan'>$row->tgl_app</td>
	  <td class='isi_laporan'>$row->no_ref3</td>
	 <td class='isi_laporan'>$row->tgl_kas</td>
	  ...
	  <td class='header_laporan' align='left'>".number_format($nilai1,0,",",".")."</td>
