...
class server_report_saku3_aka2_rptAkBillJurRekon extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="select a.no_bill,a.no_dokumen,a.kode_lokasi,a.periode,a.tanggal,date_format(a.tanggal,'%d/%m/%Y') as tanggal1,a.keterangan,a.kode_lokasi
from aka_bill_m a $this->filter ";
// echo $sql;
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			echo "<table border='0' cellspacing='2' cellpadding='1'>
			  <tr>
				<td align='center' class='header_laporan'>JURNAL TAGIHAN</td>
			  </tr>
			  <tr>
				<td><table border='0' cellspacing='2' cellpadding='1'>
				  ...
				  <tr>
					...
					<td width='496' class='header_laporan'>:&nbsp;$row->no_bill</td>
					</tr>
				  <tr>
	  ...
	  // $sql1="select a.kode_akun,b.nama,a.keterangan,a.kode_pp,a.kode_drk,case dc when 'D' then nilai else 0 end as debet,case dc when 'C' then nilai else 0 end as kredit  
		...
		// 	order by a.dc desc ";
		$sql1="select a.kode_akun,b.nama,a.keterangan,a.kode_pp,a.kode_drk,a.jenis,sum(case a.dc when 'D' then nilai else 0 end) as debet,sum(case a.dc when 'C' then nilai else 0 end) as kredit  
		from aka_bill_j a
		inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
		where a.no_bill='$row->no_bill' and a.kode_lokasi='$row->kode_lokasi'
		group by a.kode_akun,b.nama,a.keterangan,a.kode_pp,a.kode_drk,a.jenis ";
    ...
    <td class='isi_laporan' align='right'>$tot_kredit1</td>
        ...
        $sql="select a.no_rekon,a.no_dokumen,a.kode_lokasi,a.periode,a.tanggal,date_format(a.tanggal,'%d/%m/%Y') as tanggal1,a.keterangan,a.kode_lokasi
from aka_rekon_m a $this->filter2 ";
// echo $sql;
		// $start = (($this->page-1) * $this->rows);
        // $rs2=$dbLib->LimitQuery($sql,$this->rows,$start);
		...
		while ($row = $rs2->FetchNextObject($toupper=false))
		{
			echo "<table border='0' cellspacing='2' cellpadding='1'>
			  <tr>
				<td align='center' class='header_laporan'>JURNAL REKON TAGIHAN</td>
			  </tr>
			  <tr>
				<td><table border='0' cellspacing='2' cellpadding='1'>
				  ...
				  <tr>
					<td width='100' class='header_laporan'>No Bukti </td>
					<td width='496' class='header_laporan'>:&nbsp;$row->no_rekon</td>
					</tr>
				  <tr>
					<td width='100' class='header_laporan'>No Dokumen </td>
					<td width='496' class='header_laporan'>:&nbsp;$row->no_dokumen</td>
	  ...
	  $sql1="select a.kode_akun,b.nama,a.keterangan,a.kode_pp,a.kode_drk,case dc when 'D' then nilai else 0 end as debet,case dc when 'C' then nilai else 0 end as kredit  
			from aka_rekon_j a
			inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
			where a.no_rekon='$row->no_rekon' and a.kode_lokasi='$row->kode_lokasi'
			order by a.dc desc";
		//error_log($sql1);
		$rs12 = $dbLib->execute($sql1);
		$i=1;
		$tot_debet=0;
		$tot_kredit=0;
		while ($row1 = $rs12->FetchNextObject($toupper=false))
		{
			$debet=number_format($row1->debet,0,',','.');
			$kredit=number_format($row1->kredit,0,',','.');
			$tot_debet=$tot_debet+$row1->debet;
