...
class server_report_saku3_aka2_rptAkPend3 extends server_report_basic
{	
	...
	function getHtml()
	{
        ...
        $sql=" select * from(
            ...
            c.kode_akt
            ,f.tahunaka,case when substring(f.tahunaka,1,2) = substring(c.kode_akt,3,2) then 'MABA' else 'MALA' end as sts_mhs,b.akun_pdpt as kode_akun  ,b.akun_pdd as kode_akun1 
            from aka_amor_m a 
...
c.kode_akt,f.tahunaka,case when substring(x.tahunaka,1,2) = substring(c.kode_akt,3,2) then 'MABA' else 'MALA' end as sts_mhs,b.akun_piutang as kode_akun ,b.akun_pdpt as kode_akun1
from aka_bill_m a 
...
select distinct a.periode,a.no_batal as no_bill,
		...
		c.kode_akt
		,f.tahunaka,case when substring(f.tahunaka,1,2) = substring(c.kode_akt,3,2) then 'MABA' else 'MALA' end as sts_mhs,b.akun_piutang as kode_akun ,b.akun_pdpt as kode_akun1 
		from aka_batal_m a 
		inner join (
			select a.kode_produk,a.nilai,a.kode_lokasi,a.periode,a.nim,a.no_inv,a.no_bill,a.akun_piutang ,a.akun_pdpt
			from aka_bill_d a
			where a.kode_produk not in ('BPPP','BPPNP') and a.modul='BATAL'
			union all
			select a.kode_produk,a.nilai,a.kode_lokasi,a.periode,a.nim,a.no_inv,a.no_amor,a.akun_pdpt ,a.akun_pdd
			from aka_amor_d a
...
//  echo $sql;
  ...
  <tr bgcolor='#CCCCCC'>
    ...
    <td width='60' align='center' class='header_laporan'>Kode Akun1</td>
	 <td width='100' align='center' class='header_laporan'>Total</td>
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
            ...
            <td class='isi_laporan' >".$row->kode_akun1."</td>
