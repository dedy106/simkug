...
class server_report_saku3_aka2_rptAkRekonTu extends server_report_basic
{	
...
// 	   isnull(b.bpp,0) as bpp,isnull(b.BPNP,0) as bpnp,isnull(b.sdp2,0) as sdp2,isnull(b.up3,0) as up3 ,isnull(b.sks,0) as sks,isnull(b.perpus,0) as perpus,
// 	   isnull(b.asur,0) as asur,isnull(b.nilai,0) as total,isnull(b.batal,0) as batal,isnull(b.denda,0) as denda,isnull(b.ustatus,0) as ustatus,c.kode_akt,f.tahunaka
// from aka_rekon_m a
...
// 					 sum(case when a.modul in ('BATAL') then a.nilai else 0 end) as batal,
...
// 			inner join aka_bill_d g on a.no_inv=g.no_inv and a.periode=g.periode and a.kode_lokasi=g.kode_lokasi	
// 			$this->filter
...
// inner join aka_tahunaka f on a.periode=f.periode and a.kode_lokasi=f.kode_lokasi
// $this->filter
// order by b.nim,a.no_rekon

//  ";

$sql="select a.no_rekon,a.keterangan,b.no_inv,b.nim,c.nama,c.kode_jur,d.nama as nama_jur,c.kode_fakultas,e.nama as nama_fakultas, isnull(b.bpp,0) as bpp,isnull(b.BPNP,0) as bpnp,isnull(b.sdp2,0) as sdp2,isnull(b.up3,0) as up3 ,isnull(b.sks,0) as sks,isnull(b.perpus,0) as perpus, isnull(b.asur,0) as asur,isnull(b.nilai,0) as total,isnull(b.batal,0) as batal,isnull(b.no_batal,'-') as no_batal,isnull(b.pddk_lain,0) as pddk_lain,isnull(b.denda,0) as denda,isnull(b.ustatus,0) as ustatus,c.kode_akt,f.tahunaka,g.tahunaka as tahunaka2 
from aka_rekon_m a 
inner join (select c.nim,a.no_inv,a.no_rekon,c.kode_lokasi, 
			...
			sum(case when a.kode_produk in ('USTATUS') then a.nilai else 0 end) as ustatus, 
			sum(case when a.kode_produk in ('DENDA') then a.nilai else 0 end) as denda, 			
			sum(case when a.kode_produk in ('BADM','DHER','REMD','WISD','BEANONBPP','KTM','LEG','TDKREG') then a.nilai else 0 end) as pddk_lain, isnull(h.no_rekon,'-') as no_batal,
...
inner join aka_fakultas e on d.kode_fakultas=e.kode_fakultas and d.kode_lokasi=e.kode_lokasi 
inner join aka_tahunaka f on a.periode=f.periode and a.kode_lokasi=f.kode_lokasi 
inner join (select distinct a.no_inv,a.kode_lokasi,a.tahunaka 
			from aka_bill_d a
			) g on b.no_inv=g.no_inv and b.kode_lokasi=g.kode_lokasi
$filter3 $filterbtl order by b.nim,a.no_rekon";
  ...
  <tr bgcolor='#CCCCCC'>
   ...
   <td class='isi_laporan' align='center'>$i</td>
  		...
  		<td class='isi_laporan'>$row->no_batal</td>
			...
			<td class='isi_laporan'>$row->tahunaka2</td>
