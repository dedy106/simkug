...
class server_report_saku3_aka2_rptAkAmorJurnalTu extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="select a.no_amor,a.no_dokumen,a.kode_lokasi,a.periode,a.tanggal,date_format(a.tanggal,'%d/%m/%Y') as tanggal1,a.keterangan,a.kode_lokasi,a.nik_user,b.nama as nama_buat
from aka_amor_m a 
...
$this->filter ";
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			echo "<table border='0' cellspacing='2' cellpadding='1'>
			  <tr>
				<td align='center' class='header_laporan'>Jurnal Akru Pendapatan</td>
			  </tr>
			  <tr>
	  ...
	  $sql1="select a.kode_akun,b.nama,a.keterangan,a.kode_pp,a.kode_drk,case dc when 'D' then nilai else 0 end as debet,case dc when 'C' then nilai else 0 end as kredit  
		...
		while ($row1 = $rs1->FetchNextObject($toupper=false))
		{
			...
			echo "<tr>
				...
				<td class='isi_laporan'><a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenDetAmor('$row1->kode_pp','$row->kode_lokasi','$row1->kode_akun','$row->no_amor','$row1->kode_drk','$periode');\">$row1->kode_akun</a></td>
