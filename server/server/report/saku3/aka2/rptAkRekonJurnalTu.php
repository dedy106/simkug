...
class server_report_saku3_aka2_rptAkRekonJurnalTu extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="select a.no_rekon,a.no_dokumen,a.kode_lokasi,a.periode,a.tanggal,date_format(a.tanggal,'%d/%m/%Y') as tanggal1,a.keterangan,a.kode_lokasi,a.nik_user,b.nama as nama_buat
from aka_rekon_m a 
	  ...
	  $sql1="select a.kode_akun,b.nama,a.keterangan,a.kode_pp,a.kode_drk,case dc when 'D' then nilai else 0 end as debet,case dc when 'C' then nilai else 0 end as kredit  
		...
		while ($row1 = $rs1->FetchNextObject($toupper=false))
		{
			...
			echo "<tr>
				...
				<td class='isi_laporan'><a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenDetRekon('$row1->kode_pp','$row->kode_lokasi','$row1->kode_akun','$row->no_rekon','$periode');\">$row1->kode_akun</a></td>
