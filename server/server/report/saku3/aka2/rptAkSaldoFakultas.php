...
class server_report_saku3_aka2_rptAkSaldoFakultas extends server_report_basic
{	
...
from aka_fakultas a 
left join (select y.kode_lokasi,z.kode_fakultas,sum(case when x.kode_produk in ('BPPP','BPPNP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as bpp, 
...
left join (select y.kode_lokasi,z.kode_fakultas,sum(case when x.kode_produk in ('BPPP','BPPNP') then x.nilai else 0 end) as bpp, 
