...
class server_report_saku3_aka2_rptAkSaldoJurTu2 extends server_report_basic
{	
...
left join (select y.kode_jur,y.kode_lokasi,
				   sum(case when x.kode_produk in ('BPPP','BPPNP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as bpp, 
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode < '$periode') $tagih			
...
left join (select y.kode_jur,y.kode_lokasi,
			...
			from aka_bill_d x 			
			inner join aka_mahasiswa y on x.nim=y.nim and x.kode_lokasi=y.kode_lokasi  
			where(x.kode_lokasi = '$kode_lokasi') ".$strperiode." $tagih		
			group by y.kode_jur,y.kode_lokasi 			
...
left join (select y.kode_jur,y.kode_lokasi,sum(case when x.kode_produk in ('BPPP','BPPNP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as bpp, 
				   sum(case when x.kode_produk in ('SDP2') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as sdp2, 
				   sum(case when x.kode_produk in ('UP3') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as up3,
				   sum(case when x.kode_produk in ('SKS') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as sks,
				   sum(case when x.kode_produk in ('PERPUS','DENDA','USTATUS','ASUR') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as perpus,
...
left join (select y.kode_jur,y.kode_lokasi,sum(case when x.kode_produk in ('BPPP','BPPNP') then x.nilai else 0 end) as bpp, 
...
order by a.kode_jur
 ";
		...
		echo $AddOnLib->judul_laporan("saldo piutang jurusan",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
   ...
   <td class='isi_laporan' align='center'>$i</td>
    ...
    <td class='isi_laporan'>";
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenSaldoMhs('$row->kode_jur','$row->kode_lokasi','$row->kode_fakultas');\">$row->nama</a>";
			echo "</td>
