...
class server_report_saku3_gar_rptGarRkap extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="exec sp_rkap_dw 'L','FS1','$periode','$kode_lokasi1','$nik_user','$bentuk','$kode_bidang1','$kode_bidang2'";
		
		$rs = $dbLib->execute($sql);
		
		$sql = "select kode_neraca,kode_fs,nama,level_spasi,tipe,abs(n1) as n1,abs(n2) as n2,abs(n3) as n3,abs(n4) as n4,abs(n9) as n9,
...
order by rowindex ";
  ...
  <tr bgcolor='#CCCCCC'>
	...
	<td rowspan='2' align='center' class='header_laporan'>RKA S.D $bln ORIGINAL</td>
  ...
  <tr bgcolor='#CCCCCC'>
    ...
    <td align='center' class='header_laporan'>%REAL (RKA)</td>
	 <td align='center' class='header_laporan'>%REAL (ORIGINAL)</td>
  ...
  <tr bgcolor='#CCCCCC'>
    ...
    <td width='90' align='center' class='header_laporan'>2a</td>
	 <td width='90' align='center' class='header_laporan'>2b</td>
	...
	<td width='60' align='center' class='header_laporan'>12=10/2b</td>
    <td width='60' align='center' class='header_laporan'>13=(10-9)/9</td>
  </tr>";
		while ($row = $rs->FetchNextObject($toupper=false))
		{
		
			$real=0;$realtw=0;$grow=0;$realrka=0;$realorg=0;
			echo "<tr>
			...
			if ($row->tipe!="Header" && $row->nama!="." && $row->nama!="")
			{
				...
				echo "<td align='right' class='isi_laporan'>".number_format($row->n1,0,",",".")."</td>
					...
					<td align='right' class='isi_laporan'>".number_format($row->n9,0,",",".")."</td>
					...
					<td align='right' class='isi_laporan'>".number_format($realorg,2,",",".")."</td>
			...
			if ($bentuk=="Detail" && $row->tipe=="Posting")
			{
				...
				$sql1="select kode_akun,nama,n1,n2,n3,n4,n9,
...
from neracagar_tmp
where nik_user='$nik_user' and kode_neraca='$row->kode_neraca' and (n1<>0 or n2<>0 or n3<>0 or n4<>0 or n5<>0 or n6<>0 or n7<>0 or n8<>0 or n9<>0)
order by kode_akun ";
				...
				<td class='detail_laporan'>$row1->nama</td>";
					...
					<td align='right' class='detail_laporan'>".number_format($row1->n9,0,",",".")."</td>
					...
					<td align='right' class='detail_laporan'>".number_format($realorg,2,",",".")."</td>
