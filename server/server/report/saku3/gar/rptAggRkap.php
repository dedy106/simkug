...
class server_report_saku3_gar_rptAggRkap extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="exec sp_agg_rkap_bulan_reset '$kode_fs','L','$periode','$lokasi','$injenis','$nik_user'";
		//echo $sql;
		$rs = $dbLib->execute($sql);
		...
		echo $AddOnLib->judul_laporan($nama_form,$this->lokasi,"TAHUN ".$periode);
...
<td width='90' class='header_laporan'><div align='center'>September</div></td>
  ...
  <td class='isi_laporan' align='right'>".number_format($row->n0,0,',','.')."</td>
			...
			where b.kode_fs='$kode_fs' and b.kode_lokasi='$kode_lokasi' and b.kode_neraca='$kode_neraca' and substring(a.periode,1,4)='$periode' $sql_jenis
