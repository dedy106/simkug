...
class server_report_saku3_gar_rptAggRkapPp extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$jenis=$tmp[7];
		$jenis_file=$tmp[8];
		$nama_file="labarugi_pp_".$tahun.".xls";
		...
		$sql="select a.kode_pp,a.nama from pp a $this->filter and a.flag_aktif='1' order by a.kode_pp";
		...
		{
			$sql="exec sp_agg_rkap_bulan_tu_pp_reset '$kode_fs','L','$tahun','$lokasi','$row2->kode_pp','$injenis','$nik_user'";
			...
			echo $AddOnLib->judul_laporan($nama_form."<br>TAHUN $tahun",$this->lokasi,"$row2->kode_pp - $row2->nama");
	...
	<td width='90' class='header_laporan'><div align='center'>September</div></td>
	  ...
	  <td class='isi_laporan' align='right'>".number_format($row->n0,0,',','.')."</td>
				...
				where b.kode_fs='$kode_fs' and b.kode_lokasi='$kode_lokasi' and b.kode_neraca='$kode_neraca' and substring(a.periode,1,4)='$tahun' and a.kode_pp='$row2->kode_pp' $sql_jenis
