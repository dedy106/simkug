...
class server_report_saku3_gar_rptAggTahunTw extends server_report_basic
{
	...
	function getHtml()
	{
	   ...
	   case when b.jenis='Pendapatan' then isnull(f.real_01,0)*-1 else isnull(f.real_01,0) end as rtw1,
	   case when b.jenis='Pendapatan' then isnull(f.real_02,0)*-1 else isnull(f.real_02,0) end as rtw2,
	   case when b.jenis='Pendapatan' then isnull(f.real_03,0)*-1 else isnull(f.real_03,0) end  as rtw3,
	   case when b.jenis='Pendapatan' then isnull(f.real_04,0)*-1 else isnull(f.real_04,0) end  as rtw4
from akundrk a
