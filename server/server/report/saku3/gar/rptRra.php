...
class server_report_saku3_gar_rptRra extends server_report_basic
{
...
order by a.kode_akun";
		...
		echo $AddOnLib->judul_laporan("kartu pengawasan anggaran (KPA) RRA",$this->lokasi,"TAHUN $tahun");
...
";	
			...
			{
				$sql="select b.no_agg as no_bukti,b.no_dokumen,date_format(b.tanggal,'%d/%m/%Y') as tgl,a.kode_akun,b.keterangan,b.kode_lokasi,a.periode,
...
select * from gldt where kode_lokasi='$row->kode_lokasi' and substring(periode,1,4)='$tahun' and kode_akun='$row->kode_akun' ".$kode_pp.$kode_drk.$periode." ) ";
			...
			while ($row1 = $rs1->FetchNextObject($toupper=false))
			{
				...
				if ($tmp!=$row1->tw)
				{
					...
					if ($i>1)
					{
						...
						echo " <tr>
   <td height='23' colspan='5' valign='top' class='header_laporan' align='right'>TOTAL TRIWULAN&nbsp;</td>
   ...
   <td valign='top' class='header_laporan' align='right'>".number_format($saldo,0,',','.')."</td>
					...
					if ($j==1)
					{
						...
						echo "<tr>
						<td height='23' colspan='7' class='header_laporan' align='right'>ANGGARAN TRIWULAN-I&nbsp;</td>
					...
					if ($j==2)
					{
						...
						echo "<tr>
						<td height='23' colspan='7' class='header_laporan' align='right'>ANGGARAN TRIWULAN-II&nbsp;</td>
						<td class='header_laporan' align='right'>".number_format($row->tw2,0,',','.')."</td>
					  </tr>";
					}
					if ($j==3)
					{
						...
						echo "<tr>
						<td height='23' colspan='7' class='header_laporan' align='right'>ANGGARAN TRIWULAN-III&nbsp;</td>
						<td class='header_laporan' align='right'>".number_format($row->tw3,0,',','.')."</td>
					  </tr>";
					}
					if ($j==4)
					{
						$saldo=$row->tw4;
						echo "<tr>
						<td height='23' colspan='7' class='header_laporan' align='right'>ANGGARAN TRIWULAN-IV&nbsp;</td>
						<td class='header_laporan' align='right'>".number_format($row->tw4,0,',','.')."</td>
					  </tr>";
					}
					$j=$j+1;
				...
				<td valign='top' class='isi_laporan'>";
						echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenRRA('$row1->no_bukti','$row->kode_lokasi');\">$row1->no_bukti</a>";
						echo "</td>
			<td valign='top' class='isi_laporan'>".$row1->no_dokumen."</td>
				...
				<td valign='top' class='isi_laporan'>".$row1->periode."</td>
