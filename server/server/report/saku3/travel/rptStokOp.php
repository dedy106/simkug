...
class server_report_saku3_travel_rptStokOp extends server_report_basic
{
	...
	function getHtml()
	{
		...
		if($jperiode == "Range"){
			...
			$str_periode = " and a.periode between '$periode' and '$periode2' ";
		}else{
			$per = $periode;
			$str_periode = " and a.periode = '$periode'";
		}
		
		// echo $per;
		// echo $str_periode;
		$sql= "exec sp_brg_stok_mutasi '$per','$kode_lokasi','$nik_user' ";
		$rs = $dbLib->execute($sql);
		
		$sql= "exec sp_brg_hpp '$per','$kode_lokasi','$nik_user' ";
		$rs = $dbLib->execute($sql);
...
// order by a.tgl_ed ";
			$sql1="select * from (select convert(varchar(20),a.tgl_ed,103) as tgl , a.no_bukti, b.keterangan, a.modul, a.stok,a.harga,b.param2,
			case when a.dc='D' then a.jumlah else 0 end as debet,	      
			case when a.dc='C' then a.jumlah else 0 end as kredit, a.tgl_ed
			from brg_trans_d a
			inner join trans_m b on a.no_bukti=b.no_bukti and a.kode_lokasi=b.kode_lokasi 
			inner join brg_barang c on a.kode_barang=c.kode_barang and a.kode_lokasi=c.kode_lokasi 
			where a.kode_barang='$row->kode_barang' and a.kode_lokasi='$row->kode_lokasi' and a.kode_gudang='$row->kode_gudang' $str_periode
			union all
