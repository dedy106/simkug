...
class server_report_saku3_travel_rptDwLabaRugiPpJejer extends server_report_basic
{
	...
	function getHtml()
	{
		...
		echo $AddOnLib->judul_laporan("LAPORAN REKAP ANGGARAN LABARUGI JEJER".$nama_periode,$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
    <td  align='center' width='300' class='header_laporan'>Deskripsi</td>";
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			$jum=$jum+1;
			$tmp_sql.="sum(case when kode_pp='$row->kode_pp' then (case when jenis_akun='Pendapatan' then -n4 else n4 end) else 0 end) as 'n4$row->kode_pp',";
