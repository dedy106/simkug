...
class server_report_saku3_travel_rptGlJurnalBuktiYks extends server_report_basic
{
	...
	function getHtml()
	{
		...
		// $sql ="select nama from lokasi where kode_lokasi='$lokasi' ";
		// $rs = $dbLib->execute($sql);
		// echo $rs->fields[0];
		
		$sql="select distinct a.no_bukti,a.tanggal,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.kode_lokasi,a.periode,b.nama,b.kota
				from gldt_h a
				inner join lokasi b on a.kode_lokasi=b.kode_lokasi
				$this->filter
				union 
				select distinct a.no_bukti,a.tanggal,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.kode_lokasi,a.periode,b.nama,b.kota
				from gldt a
				inner join lokasi b on a.kode_lokasi=b.kode_lokasi
				$this->filter
				order by a.no_bukti ";
  ...
  <tr>
    <td><table width='800' border='1' cellspacing='0' cellpadding='0' class='kotak'>
	  ...
	  $sql1="select a.kode_akun,b.nama as nama_akun,a.nilai,a.keterangan,isnull(a.debet,0) as debet,isnull(a.kredit,0) as kredit,a.kode_curr,a.kode_pp,a.kode_drk
					  from (select kode_lokasi,kode_akun,dc,nilai,kode_pp,kode_drk,keterangan,case when dc='D' then nilai else 0 end as debet,case when dc='C' then nilai else 0 end as kredit,kode_curr
							from gldt_h
							where periode='$row->periode' and kode_lokasi='$row->kode_lokasi' and no_bukti='$row->no_bukti'
							union all
							select kode_lokasi,kode_akun,dc,nilai,kode_pp,kode_drk,keterangan,case when dc='D' then nilai else 0 end as debet,case when dc='C' then nilai else 0 end as kredit,kode_curr
							from gldt
							where periode='$row->periode' and kode_lokasi='$row->kode_lokasi' and no_bukti='$row->no_bukti'
							)a 
