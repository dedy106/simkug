...
class server_report_saku3_travel_rptGlTbPln extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$rs = $dbLib->execute($sql);
		$sql = "select a.kode_akun,a.nama,a.kode_lokasi,b.modul,a.debet,a.kredit,a.so_awal,so_akhir, 
  ...
  <tr bgcolor='#CCCCCC'>
    ...
    <td colspan='2' class='header_laporan' align='center'>Mutasi + Adjustment</td>
    <td colspan='2' class='header_laporan' align='center'>Rugi / Laba </td>
	<td colspan='2' class='header_laporan' align='center'>Neraca </td>
  </tr>
  <tr bgcolor='#CCCCCC'> 
    ...
    <td width='90' class='header_laporan' align='center'>Kredit</td>
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			$nso_akhir_debet+=$row->so_akhir_debet;
			$nso_akhir_kredit+=$row->so_akhir_kredit;
			$nso_akhir_debet2+=$row->so_akhir_debet;
			$nso_akhir_kredit2+=$row->so_akhir_kredit;

			if ($row->modul=='L') {
				$so_akhir_debet=$row->so_akhir_debet;
				$so_akhir_kredit=$row->so_akhir_kredit;
			}
			else {
				$so_akhir_debet2=$row->so_akhir_debet;
				$so_akhir_kredit2=$row->so_akhir_kredit;
			}
...
<td class='isi_laporan' align='right'>".number_format($row->so_awal_debet,0,',','.')."</td>
    ...
    <td class='isi_laporan' align='right'>".number_format($so_akhir_debet,0,',','.')."</td>
    <td class='isi_laporan' align='right'>".number_format($so_akhir_kredit,0,',','.')."</td>
	<td class='isi_laporan' align='right'>".number_format($so_akhir_debet2,0,',','.')."</td>
    <td class='isi_laporan' align='right'>".number_format($so_akhir_kredit2,0,',','.')."</td>
  </tr>";
    ...
    <td class='sum_laporan' align='right'>".number_format($nso_akhir_debet,0,',','.')."</td>
    <td class='sum_laporan' align='right'>".number_format(abs($nso_akhir_kredit),0,',','.')."</td>
	<td class='sum_laporan' align='right'>".number_format($nso_akhir_debet2,0,',','.')."</td>
    <td class='sum_laporan' align='right'>".number_format(abs($nso_akhir_kredit2),0,',','.')."</td>
</tr>";
		echo "</td></tr>";
		echo "</table></div>";
		return "";
