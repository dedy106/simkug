...
class server_report_saku3_travel_rptFaSaldoTahun extends server_report_basic
{
...
left join (select a.kode_klpakun,a.kode_lokasi,substring(a.periode,1,4) as tahun,sum(case when b.dc='D' then b.nilai else -b.nilai end) as ap
		   ...
		   where a.kode_lokasi='$kode_lokasi' and a.progress not in ('K','P') and a.jenis<>'I' and bb.no_fa is null and b.periode <='$periode'
           group by a.kode_klpakun,a.kode_lokasi,substring(a.periode,1,4)
   ...
   <td width='100' align='center' class='header_laporan'>Akum. Susut</td>
    <td width='100' align='center' class='header_laporan'>Nilai Buku</td>
  	...
  	";
		...
		$so_awal_tot=0;$nilai_buku_tot=0;$ap_tot=0;$wo_tot=0;
		$first = true;
		$tmp = "";$nama="";
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			$so_awal_tot+=$row->so_awal;
			$ap_tot+=$row->ap;
			$nilai_buku_tot+=$row->nilai_buku;
			$wo_tot+=$row->wo;
		    echo "<tr>
		...
		echo " <tr>
					  ...
					  <td class='header_laporan' align='right'>".number_format($so_awal_tot,0,",",".")."</td>
					  <td class='header_laporan' align='right'>".number_format($wo_tot,0,",",".")."</td>
					  <td class='header_laporan' align='right'>".number_format($ap_tot,0,",",".")."</td>
					  <td class='header_laporan' align='right'>".number_format($nilai_buku_tot,0,",",".")."</td>
					</tr></table></div>";
		return "";
	}
