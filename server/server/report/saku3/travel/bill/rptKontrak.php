...
class server_report_saku3_travel_bill_rptKontrak extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$periode=$tmp[0];
		$nama_cab=$tmp[1];
		$sql="select a.no_kontrak,a.no_dok,convert(varchar,tgl_awal,103) as tgl_awal,convert(varchar,tgl_akhir,103) as tgl_akhir,a.keterangan,
		a.nilai+a.nilai_ppn as nilai,a.nilai_ppn,a.nilai as kontrak,b.nama as nama_cust
from kontrak_m a
...
$this->filter order by a.no_kontrak";
   ...
   <tr bgcolor='#CCCCCC'>
     ...
     <td width='90'  align='center' class='header_laporan'>Total Kontrak</td>
		...
		$nilai=0;$nilai_ppn=0;$kontrak=0;
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			$nilai=$nilai+$row->nilai;
			$nilai_ppn=$nilai_ppn+$row->nilai_ppn;
			$kontrak=$kontrak+$row->kontrak;
		echo "<tr >
