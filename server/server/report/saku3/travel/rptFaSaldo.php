...
class server_report_saku3_travel_rptFaSaldo extends server_report_basic
{
...
left join (select a.kode_klpakun,a.kode_lokasi,sum(b.nilai) as kredit
		   ...
		   where a.kode_lokasi='$kode_lokasi' and b.periode<'$periode' and a.progress not in ('K','P') and a.jenis<>'I'
		   group by a.kode_klpakun,a.kode_lokasi
...
left join (select a.kode_klpakun,a.kode_lokasi,sum(case when b.dc='D' then b.nilai else -b.nilai end) as ap
           ...
           where a.kode_lokasi='$kode_lokasi' and b.periode<'$periode_susut' and a.progress not in ('K','P') and a.jenis<>'I'
           group by a.kode_klpakun,a.kode_lokasi
...
left join (select a.kode_klpakun,a.kode_lokasi,sum(case when bb.dc='D' then bb.nilai else -bb.nilai end) as ap_wo
	...
	// echo $sql;
		...
		echo $AddOnLib->judul_laporan("laporan saldo assettttttttt",$this->lokasi,$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
    <td rowspan='2' align='center' class='header_laporan'>Debet</td>
    <td rowspan='2' align='center' class='header_laporan'>Kredit</td>
  </tr>
  <tr bgcolor='#CCCCCC'>
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
		    ...
		    echo "<tr>
				<td class='isi_laporan'>$i</td>
				  ...
				  echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenAsset('$row->kode_klpakun','$kode_lokasi','$periode');\">$row->nama</a>";				  
				  echo "</td>
