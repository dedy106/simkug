...
class server_report_saku3_travel_rptPiutangJurnal extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="select e.nik1,a.no_piutang,a.kode_lokasi,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.keterangan,a.kode_lokasi,
	  ...
	  $sql1="select a.kode_akun,b.nama,a.keterangan,a.kode_pp,a.kode_drk,case dc when 'D' then nilai else 0 end as debet,case dc when 'C' then nilai else 0 end as kredit  
			...
			where a.no_bukti='$row->no_piutang' and a.kode_lokasi='$row->kode_lokasi'
			order by a.nu ";
		//error_log($sql1);
