...
class server_report_saku3_travel_rptGlLabaRugiInv extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql = "select kode_neraca,kode_fs,kode_lokasi,nama,tipe,level_spasi,n1,n2,n3,n4,n5,n6,n7,n8,n9
			from exs_neraca 
			where modul='A' and kode_lokasi='$kode_lokasi' and kode_fs='$kode_fs' and periode='$periode'  and level_lap<=$lev
			order by rowindex ";
  ...
  <tr>
    <td  class='lokasi_laporan2' align='center'>LAPORAN INVESTASI</td>
  </tr>
  <tr>
  ...
  <tr bgcolor='#dbeef3'>
    <td width='200' rowspan='2' align='center' class='header_laporan'>ITEMS (in Rp.Bn)</td>
  ...
  <tr bgcolor='#dbeef3'>
    ...
    <td width='60' align='center' class='header_laporan'>YoY Growth</td>
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			if ($row->n7!=0)
			{
				$persen1=(($row->n2-$row->n3)/$row->n7)*100;
			}
			if ($row->n9!=0)
			{
				$persen2=(((($row->n2-$row->n3)-$row->n9)/abs($row->n9)))*100;
			}
			if ($row->n2!=0)
			{
			...
			if ($bentuk=="Detail" && $row->tipe=="Posting")
			{
				...
				$sql1="select a.kode_akun,c.nama,a.n1,a.n2,a.n3,a.n4,a.n5,a.n6,a.n7,a.n8,a.n9
					from exs_glma_gar a
