...
class server_report_saku3_travel_rptGlNeracaMutasi extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$format=$tmp[4];
		$kode_fs=$tmp[5];
		$sql="exec sp_neraca_dw '$kode_fs','N','S','$lev','$periode','$kode_lokasi','$nik_user' ";
		...
		echo $AddOnLib->judul_laporan($this->lokasi.$nama_periode,$AddOnLib->ubah_periode($periode));
		echo "<table border='1' cellspacing='0' cellpadding='0' class='kotak'>
  <tr bgcolor='#CCCCCC'>
    <td width='400' height='25'  class='header_laporan'><div align='center'>Deskripsi</div></td>
    <td width='100' class='header_laporan'><div align='center'>Saldo Awal</div></td>
<td width='100' class='header_laporan'><div align='center'>Mutasi</div></td>
<td width='100' class='header_laporan'><div align='center'>Saldo Akhir</div></td>
</tr>";
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			if ($row->tipe!="Header" && $row->nama!="." && $row->nama!="")
			{
				...
				$mutasi=number_format($row->n2-$row->n3,0,",",".");
...
<td class='isi_laporan'><div align='right'>$mutasi</div></td>
...
order by a.kode_akun ";
				...
				{	
					...
					$mutasi=number_format($row1->debet-$row1->kredit,0,",",".");
					$kredit=number_format($row1->kredit,0,",",".");
					$so_akhir=number_format($row1->so_akhir,0,",",".");
					$nama=$row1->kode_akun." - ".$row1->nama;
					echo "<tr><td height='20' class='detail_laporan'>";
					echo fnSpasi($row->level_spasi);
					if ($row->tipe=="Posting" && $row->so_akhir > 0)
					{
						echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_neraca','$row->kode_lokasi');\">$row->nama</a>";
					}
					else
...
ifnull(sum(a.so_akhir),0) as so_akhir  
				...
				$rs1 = $dbLib->execute($sql1);
				$so_awal=0;$kredit=0;$so_akhir=0;$debet=0;
				while ($row1 = $rs1->FetchNextObject($toupper=false))
				{	
					$so_awal=number_format($row1->so_awal,0,",",".");
					$mutasi=number_format($row1->debet-$row1->kredit,0,",",".");
