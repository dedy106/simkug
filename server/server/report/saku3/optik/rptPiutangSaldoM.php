...
class server_report_saku3_optik_rptPiutangSaldoM extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="select a.kode_lokasi,a.no_bukti,a.no_dokumen,convert(varchar,a.tanggal,103)  as tgl,a.pasien,a.kode_pp,b.nama as nama_cust,
		...
		left join (select a.no_tagih,a.kode_lokasi,sum(a.nilai) as nilai_kas 
				   from optik_tagih_bayar a 
				   ...
				   group by a.no_tagih,a.kode_lokasi ) c on a.no_bukti=c.no_tagih and a.kode_lokasi=b.kode_lokasi 
		$this->filter and (a.no_bukti NOT LIKE '%RVB%') and a.tanggal <= '$tgl_aging' $tmp and d.nilai <> 0 order by a.no_bukti
		";
		...
		echo $AddOnLib->judul_laporan("saldo piutang Mitra",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
   ...
   <tr bgcolor='#CCCCCC'>
	 ...
	 <td width='100' rowspan='2' align='center' class='header_laporan'>Pasien</td>
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			if (($row->saldo==0) AND ($row->tagihan <> 0))
			{
				$sql="select a.tgl_kas from (select MAX(b.tanggal) as tgl_kas
					...
					where a.no_tagih='$row->no_tagih' and a.kode_lokasi='$row->kode_lokasi') a
