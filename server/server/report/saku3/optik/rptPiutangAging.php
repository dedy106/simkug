...
class server_report_saku3_optik_rptPiutangAging extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$periode=$tmp[1];
		$tgl_aging=$tmp[2];
		$sql="select a.kode_lokasi,a.no_bukti,date_format(a.tanggal,'%d/%m/%Y') as tgl,d.nama as nama_cust,a.keterangan,
		e.nilai as tagihan,
...
from trans_m a
inner join optik_app_m b on a.no_bukti=b.no_app and a.kode_lokasi=b.kode_lokasi 
inner join optik_pesan_m c on c.no_bukti=b.no_bukti and c.kode_lokasi=b.kode_lokasi 
inner join optik_mitra d on c.kode_mitra=d.kode_mitra and c.kode_lokasi=d.kode_lokasi 
inner join trans_j e on a.no_bukti=e.no_bukti and a.kode_lokasi=e.kode_lokasi 

$this->filter and a.tanggal<='$tgl_aging' and e.jenis='PIUMITRA' ";
