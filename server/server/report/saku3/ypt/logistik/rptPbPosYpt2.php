...
class server_report_saku3_ypt_logistik_rptPbPosYpt2 extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="
        select * from (select a.periode,a.modul,a.no_pb,a.kode_lokasi,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.posted,a.nilai,d.no_dokumen as no_dpc,
        ...
        left join (select b.no_bukti, b.kode_lokasi,count(b.no_gambar) as jum_dok,a.kode_pp,a.periode 
                from pbh_pb_m a
                inner join pbh_dok b on a.no_pb=b.no_bukti and a.kode_lokasi=b.kode_lokasi
                group by b.no_bukti, b.kode_lokasi,a.kode_pp,a.periode
                )j on a.no_pb=j.no_bukti and a.kode_lokasi=j.kode_lokasi and a.kode_pp=j.kode_pp and a.periode=j.periode 
        union all
        select a.periode,a.modul,a.no_aju,a.kode_lokasi,convert(varchar,a.tanggal,103) as tgl,a.keterangan,'-' as posted,a.nilai,'-' as no_dpc,
		isnull(j.jum_dok,0) as jum_dok,
        case a.progress 
        when '0' then 'Pengajuan PB'
        when 'D' then 'Ver Dok'
        ...
        ,a.kode_pp,b.nama as nama_pp,
            e.no_app as no_ver,convert(varchar,e.tgl_input,103) as tgl_ver,
            a.no_ver as no_ver_dok,convert(varchar,c.tanggal,103) as tgl_ver_dok,
            a.no_kas,convert(varchar,g.tanggal,103) as tgl_kas,
            a.no_spb,convert(varchar,d.tanggal,103) as tgl_spb,
            a.no_fiat as no_fisik,convert(varchar,h.tanggal,103) as tgl_fisik,
            '-' as no_pajak,'-' as tgl_pajak
        from it_aju_m a
        inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi
        left join ver_m c on a.no_ver=c.no_ver and a.kode_lokasi=c.kode_lokasi
        left join it_spb_m d on a.no_spb=d.no_spb and a.kode_lokasi=d.kode_lokasi
        left join it_ajuapp_m e on a.no_aju=e.no_aju and a.kode_lokasi=e.kode_lokasi
        left join kas_m g on a.no_kas=g.no_kas and a.kode_lokasi=g.kode_lokasi
        left join fiat_m h on a.no_fiat=h.no_fiat and a.kode_lokasi=h.kode_lokasi
        left join (select b.no_bukti, b.kode_lokasi,count(b.no_gambar) as jum_dok,a.kode_pp,a.periode 
                from it_aju_m a
                inner join it_aju_dok b on a.no_aju=b.no_bukti and a.kode_lokasi=b.kode_lokasi
                group by b.no_bukti, b.kode_lokasi,a.kode_pp,a.periode
                )j on a.no_aju=j.no_bukti and a.kode_lokasi=j.kode_lokasi and a.kode_pp=j.kode_pp and a.periode=j.periode
        ) a
        $this->filter 
        order by a.no_pb 
        ";
