...
class server_report_saku3_ypt_logistik_rptBastSaldo extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$kode_lokasi=$tmp[0];
		$periode=$tmp[1];
		$sql="select a.kode_lokasi,a.no_hutang,a.no_dokumen,convert(varchar(20),a.tanggal,103) as tgl,a.keterangan,
	   a.nilai+a.nilai_ppn as nilai,a.nilai_ppn,a.nilai as tagihan,b.nama as nama_vendor,c.no_agenda,
	   isnull(c.nilai,0) as nilai_kas,a.nilai+a.nilai_ppn-isnull(c.nilai,0) as saldo
from hutang_m a
inner join vendor b on a.kode_vendor=b.kode_vendor and a.kode_lokasi=b.kode_lokasi 
left join yk_pb_m c on a.no_hutang=c.no_hutang and a.kode_lokasi=b.kode_lokasi
left join (select a.no_hutang,a.kode_lokasi,sum(c.nilai) as nilai
		   from yk_pb_m a
		   inner join it_aju_m b on a.no_agenda=b.no_aju and a.kode_lokasi=b.kode_lokasi
		   inner join kas_m c on b.no_kas=c.no_kas and b.kode_lokasi=c.kode_lokasi 
		   where a.kode_lokasi='$kode_lokasi' and c.periode<='$periode'
		   group by a.no_hutang,a.kode_lokasi
		  )d on a.no_hutang=d.no_hutang and a.kode_lokasi=d.kode_lokasi
$this->filter order by a.no_hutang";
		...
		echo $AddOnLib->judul_laporan("saldo hutang asset",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
		echo "<table border='1' cellspacing='0' cellpadding='0' class='kotak'>
   <tr bgcolor='#CCCCCC'>
     <td width='30' rowspan='2'  align='center' class='header_laporan'>No</td>
	 <td width='100' rowspan='2' align='center' class='header_laporan'>No Hutang</td>
	 <td width='100' rowspan='2' align='center' class='header_laporan'>No Agenda</td>
	 <td width='60' rowspan='2' align='center' class='header_laporan'>Tanggal</td>
     <td width='150' rowspan='2' align='center' class='header_laporan'>Vendor</td>
   ...
   <tr bgcolor='#CCCCCC'>
     ...
     <td class='isi_laporan'>";
		echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenPiutang('$row->no_hutang','$row->kode_lokasi');\">$row->no_hutang</a>";
		echo "</td>
	<td class='isi_laporan'>";
		echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenAgenda('$row->no_agenda','$row->kode_lokasi');\">$row->no_agenda</a>";
		echo "</td>	
	 ...
	 echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenKartu('$row->no_hutang','$row->kode_lokasi');\">".number_format($row->nilai_kas,0,",",".")."</a>";
		echo "</td>
