...
class server_report_saku3_ypt_rptKasTerima extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$tmp=explode("/",$this->filter2);
		$periode=$tmp[0];
		$sql="select a.no_kas,a.no_dokumen,a.tanggal,convert(varchar(20),a.tanggal,103) as tgl,a.no_link as kode_lokkirim,b.nama as nama_lokasi,a.ref1 as no_kirim,a.keterangan,c.kota,
...
order by a.no_kas";
		...
		echo $AddOnLib->judul_laporan("LAPORAN droping TERIMA",$this->lokasi,"Periode ".$AddOnLib->ubah_periode($periode));
     ...
     <tr >
	  ...
	  $sql1="select a.kode_akun,b.nama as nama_akun,a.kode_pp,a.kode_pp,a.kode_drk,a.keterangan,case when a.dc='D' then a.nilai else 0 end as debet,case when a.dc='C' then a.nilai else 0 end as kredit 
from kas_j a
