...
class server_report_saku3_ppbs_rptDwRkapBidangJejer extends server_report_basic
{
  ...
  <tr bgcolor='#CCCCCC'>
    <td align='center' width='300' class='header_laporan'>Deskripsi</td>";
		$sql="select kode_bidang,nama from agg_bidang where kode_lokasi='$kode_lokasi' and tahun='$tahun' order by kode_bidang";
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			$pdpt[$jum]="n13".strtolower($row->kode_bidang);
			$jum=$jum+1;
			
			$tmp_sql.="sum(case when kode_bidang='$row->kode_bidang' then (case when jenis_akun='Pendapatan' then -n13 else n13 end) else 0 end) as 'n13$row->kode_bidang',";
			$tmp_sql2.="sum(case when d.kode_bidang='$row->kode_bidang' then (case when c.jenis='Pendapatan' then -n13 else n13 end) else 0 end) as 'n13$row->kode_bidang',";
			...
			while ($row1 = $rs1->FetchNextObject($toupper=false))
			{
				...
				if ($bentuk=="Detail" && $row->tipe=="Posting")
				{
					...
					$sql1="	select a.kode_akun,c.nama,$tmp_sql2
								...
								inner join agg_pp d on a.kode_bidang=d.kode_bidang and a.kode_lokasi=d.kode_lokasi and d.tahun='$tahun'
