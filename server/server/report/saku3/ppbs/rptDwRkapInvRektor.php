...
class server_report_saku3_ppbs_rptDwRkapInvRektor extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$lokasi=$tmp[5];
		$kode_pp=$tmp[6];
		$sql="select a.kode_rektor,a.nama,a.kode_lokasi,a.tahun  
			  from agg_rektor a 
			  $this->filter
			  order by a.kode_rektor ";
		...
		while ($row2 = $rs2->FetchNextObject($toupper=false))
		{
			...
			$sql = "select c.kode_neraca,c.kode_fs,c.kode_lokasi,c.nama,c.tipe,c.level_spasi,
					c.n1,c.n2,c.n3,c.n4,c.n5,c.n6,c.n7,c.n8,c.n9,c.n10,c.n11,c.n12,c.n13
				from agg_neraca_rektor c
				inner join agg_rektor a on c.kode_rektor=a.kode_rektor and a.kode_lokasi=c.kode_lokasi and a.tahun=c.tahun 
				where c.kode_lokasi='$row2->kode_lokasi' and c.kode_rektor='$row2->kode_rektor' and c.tahun='$row2->tahun' and c.kode_fs='$kode_fs'
				order by c.rowindex ";
			$rs = $dbLib->execute($sql);
			$i = 1;
			...
			echo $AddOnLib->judul_laporan("LAPORAN REKAP ANGGARAN INVESTASI",$this->lokasi."<br>".$row2->nama,"TAHUN $tahun");
	  ...
	  <td class='isi_laporan' align='right'>".number_format($row->n13,0,',','.')."</td>
				...
				if ($bentuk=="Detail" && $row->tipe=="Posting")
				{
					...
					$sql1="select a.kode_akun,c.nama,
					   sum(case when substring(a.periode,5,2) between '01' and '12' then (case c.jenis when 'Pendapatan' then a.total else a.total end) else 0 end) as total, 	
					   sum(case when substring(a.periode,5,2)='01' then (case c.jenis when 'Pendapatan' then a.total else a.total end) else 0 end) as n1,
					   sum(case when substring(a.periode,5,2)='02' then (case c.jenis when 'Pendapatan' then a.total else a.total end) else 0 end) as n2, 
					   sum(case when substring(a.periode,5,2)='03' then (case c.jenis when 'Pendapatan' then a.total else a.total end) else 0 end) as n3, 
					   sum(case when substring(a.periode,5,2)='04' then (case c.jenis when 'Pendapatan' then a.total else a.total end) else 0 end) as n4, 
					   sum(case when substring(a.periode,5,2)='05' then (case c.jenis when 'Pendapatan' then a.total else a.total end) else 0 end) as n5, 
					   sum(case when substring(a.periode,5,2)='06' then (case c.jenis when 'Pendapatan' then a.total else a.total end) else 0 end) as n6, 
					   sum(case when substring(a.periode,5,2)='07' then (case c.jenis when 'Pendapatan' then a.total else a.total end) else 0 end) as n7, 
					   sum(case when substring(a.periode,5,2)='08' then (case c.jenis when 'Pendapatan' then a.total else a.total end) else 0 end) as n8, 
					   sum(case when substring(a.periode,5,2)='09' then (case c.jenis when 'Pendapatan' then a.total else a.total end) else 0 end) as n9, 
					   sum(case when substring(a.periode,5,2)='10' then (case c.jenis when 'Pendapatan' then a.total else a.total end) else 0 end) as n10, 
					   sum(case when substring(a.periode,5,2)='11' then (case c.jenis when 'Pendapatan' then a.total else a.total end) else 0 end) as n11, 
					   sum(case when substring(a.periode,5,2)='12' then (case c.jenis when 'Pendapatan' then a.total else a.total end) else 0 end) as n12 
				from dw_glma_pp a
				inner join relakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
				inner join masakun c on a.kode_akun=c.kode_akun and a.kode_lokasi=c.kode_lokasi
				inner join agg_pp d on a.kode_pp=d.kode_pp and a.kode_lokasi=d.kode_lokasi
				inner join agg_bidang e on d.kode_bidang=e.kode_bidang and d.kode_lokasi=e.kode_lokasi
				where b.kode_fs='$kode_fs' and b.kode_lokasi='$kode_lokasi' and b.kode_neraca='$kode_neraca' and a.nik_user='$nik_user' and e.kode_rektor='$row2->kode_rektor'
