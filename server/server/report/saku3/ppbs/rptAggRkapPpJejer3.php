...
class server_report_saku3_ppbs_rptAggRkapPpJejer3 extends server_report_basic
{
	...
	function getHtml()
	{
		...
		echo $AddOnLib->judul_laporan("LAPORAN REKAP ANGGARAN LABARUGI JEJER pp",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
    <td align='center' width='300' class='header_laporan'>Deskripsi</td>";
		$sql="select kode_pp,nama from agg_pp where kode_lokasi='$lokasi' and tahun='$tahun' and flag_aktif='1' order by kode_pp";
		//echo $sql;
		$rs = $dbLib->execute($sql);
...
from neraca 
where kode_lokasi='$lokasi' and kode_fs='$kode_fs' and modul='L'
order by rowindex";
		$rs = $dbLib->execute($sql);
		while ($row = $rs->FetchNextObject($toupper=false))
		{
...
from agg_neraca_pp
where kode_lokasi='$lokasi' and kode_fs='$kode_fs' and kode_neraca='$row->kode_neraca' and tahun='$tahun'
			...
			while ($row1 = $rs1->FetchNextObject($toupper=false))
			{
				...
				if ($bentuk=="Detail" && $row->tipe=="Posting")
				{
					...
					$sql1="	select a.kode_akun,c.nama,$tmp_sql2
								...
								where a.kode_lokasi='$lokasi' and b.kode_fs='$kode_fs' and b.kode_neraca='$row->kode_neraca' and a.tahun='$tahun' 
