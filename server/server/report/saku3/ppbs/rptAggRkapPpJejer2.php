...
class server_report_saku3_ppbs_rptAggRkapPpJejer2 extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="exec sp_exs_neraca_pp '$kode_fs','$lokasi','$tahun','$nik_user' ";
		//echo $sql;
		$rs = $dbLib->execute($sql);
...
from exs_neraca_pp
where nik_user='$nik_user'  and kode_neraca='$row->kode_neraca' 
			...
			while ($row1 = $rs1->FetchNextObject($toupper=false))
			{
				...
				if ($bentuk=="Detail" && $row->tipe=="Posting")
				{
					...
					$sql1="	select a.kode_akun,c.nama,$tmp_sql2
								...
								where a.nik_user='$nik_user' and b.kode_fs='$kode_fs' and b.kode_neraca='$row->kode_neraca'  
