...
class server_report_saku3_ppbs_rptAggRkapOutTu extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="exec sp_agg_rkap_out '$kode_fs','L','$tahun','$periode','$lokasi','$nik_user' ";
		
		$rs = $dbLib->execute($sql);
		$sql = "select kode_neraca,kode_fs,kode_lokasi,nama,tipe,level_spasi,tipe,
...
";
  ...
  <td class='isi_laporan' align='right'><a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_neraca','$row->kode_lokasi','$periode');\">".number_format($row->n4,$koma,',','.')."</a></td>
  <td class='isi_laporan' align='center'>".number_format($persen1,$koma,',','.')."</td>
  <td class='isi_laporan' align='center'>".number_format($persen2,$koma,',','.')."</td>
 			  </tr>";
