...
class server_report_saku3_ppbs_rptDwRkap extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="exec sp_agg_neraca_tmp 'L','$kode_fs','$kode_lokasi','$tahun','$nik_user'";
		$rs = $dbLib->execute($sql);  
		
		if ($bentuk=="Detail"){
			$sql="exec sp_agg_glma_tmp '$kode_lokasi','$tahun','$nik_user'";
			$rs = $dbLib->execute($sql);
...
<td width='90' class='header_laporan'><div align='center'>September</div></td>
  ...
  <td class='isi_laporan' align='right'>".number_format($row->n13,$koma,',','.')."</td>
			...
			where a.nik_user='$nik_user' and b.kode_fs='$kode_fs' and b.kode_lokasi='$kode_lokasi' and b.kode_neraca='$kode_neraca' 
