...
class server_report_saku3_ppbs_rptDwRkapRektorJejer extends server_report_basic
{
  ...
  <tr bgcolor='#CCCCCC'>
    <td align='center' width='300' class='header_laporan'>Deskripsi</td>";
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			$tmp_sql.="sum(case when kode_rektor='$row->kode_rektor' then (case when jenis_akun='Pendapatan' then -n13 else n13 end) else 0 end) as 'n13$row->kode_rektor',";
			$tmp_sql2.="sum(case when d.kode_rektor='$row->kode_rektor' then (case when c.jenis='Pendapatan' then -n13 else n13 end) else 0 end) as 'n13$row->kode_rektor',";
