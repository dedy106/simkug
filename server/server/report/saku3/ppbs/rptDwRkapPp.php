...
class server_report_saku3_ppbs_rptDwRkapPp extends server_report_basic
{
	...
	function getHtml()
	{
		...
		if ($jenis=="Excell")
		{
			...
			$sql="exec sp_agg_neraca_pp_tmp 'L','$kode_fs','$kode_lokasi','$tahun','$kode_pp','$nik_user'";
			$rs = $dbLib->execute($sql);  
			
			if ($bentuk=="Detail"){
				$sql="exec sp_agg_glma_pp_tmp '$kode_lokasi','$kode_pp','$tahun','$nik_user'";
				$rs = $dbLib->execute($sql);
