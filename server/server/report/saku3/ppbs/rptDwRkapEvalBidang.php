...
class server_report_saku3_ppbs_rptDwRkapEvalBidang extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="exec sp_agg_neraca_eval_bidang_tmp '$kode_fs','$kode_lokasi','$tahun','$kode_bidang','$nik_user'; ";
		$rs = $dbLib->execute($sql);  
		
		if ($bentuk=="Detail"){
			$sql="exec sp_agg_glma_eval_bidang_tmp '$kode_lokasi','$tahun','$kode_bidang','$nik_user'";
			$rs = $dbLib->execute($sql);
			
		}
		
		$sql="select nama from agg_bidang $this->filter  ";
		$rs = $dbLib->execute($sql);  
		$row = $rs->FetchNextObject($toupper=false);
		$nama_bidang=$row->nama;
		
		$sql = "select kode_neraca,kode_fs,kode_lokasi,nama,tipe,level_spasi,kode_bidang,
			...
			from agg_neraca_eval_bidang_tmp
  ...
  <tr>
    <td class='lokasi_laporan' align='center'>$this->lokasi<br>$nama_bidang</td>
  </tr>
  <tr>
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			if ($bentuk=="Detail" && $row->tipe=="Posting")
			{
				...
				$sql1="select a.kode_akun,c.nama,
					...
					from agg_glma_eval_bidang_tmp a
					inner join relakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
					inner join masakun c on a.kode_akun=c.kode_akun and a.kode_lokasi=c.kode_lokasi
					where a.nik_user='$nik_user' and b.kode_fs='$kode_fs' and b.kode_lokasi='$kode_lokasi' and b.kode_neraca='$kode_neraca' and a.tahun='$tahun' and a.kode_bidang='$kode_bidang' and (a.n1<>0 or a.n2<>0 or a.n4<>0 or a.n5<>0)
