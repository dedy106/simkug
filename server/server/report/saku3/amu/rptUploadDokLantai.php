...
class server_report_saku3_amu_rptUploadDokLantai extends server_report_basic
{
        ...
        .badge-success{
          ...
          <script type='text/javascript'>
            ...
            var dataTable = $('#table-data').DataTable({
                ...
                'serverSide': true,
                'ajax': {
                    'url': '$root_ser/UploadDokLantai.php?fx=getData',
            ...
            function getJenis(param,kode_jenis=null){
                $.ajax({
                    type: 'GET',
                    url: '$root_ser/UploadDokLantai.php?fx=getJenis',
                    dataType: 'json',
            ...
            $('#sai_container').on('click','#btn-upload',function(e){
                ...
                $.ajax({
                    type: 'GET',
                    url: '$root_ser/UploadDokLantai.php?fx=getUploadData',
                    dataType: 'json',
                    async:false,
                        ...
                        if(result.status){
                            ...
                            if(result.daftar.length > 0){
                                for(var i=0;i < result.daftar.length;i++){
                                    ...
                                    input += `<td ><a class='btn btn-danger btn-sm hapus-dok2' title='Hapus'><i class='fa fa-times fa-1'></i></a>&nbsp;<a class='btn btn-success btn-sm download-dok' style='' href='https://api.simkug.com/api/aset/storage/`+line.file_dok+`' target='_blank' title='Download'><i class='fa fa-download fa-1'></i></a></td>`;
                                    input += `</tr>`;
                                    no++;
                                }
            ...
            $('#input-dok').on('click', '.hapus-dok2', function(){
                if(confirm('Sistem akan menghapus file dari server. Apakah anda ingin menghapus data ini? ')){
                    ...
                    $.ajax({
                        type: 'DELETE',
                        url: '$root_ser/UploadDokLantai.php',
                        dataType: 'json',
            ...
            $('#saku-form').on('submit', '#form-tambah', function(e){
                ...
                $.ajax({
                    type: 'POST',
                    url: '$root_ser/UploadDokLantai.php?fx=simpanDok',
                    dataType: 'json',
                    data: formData,
                    contentType: false,
                    cache: false,
