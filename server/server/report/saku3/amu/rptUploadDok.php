...
class server_report_saku3_amu_rptUploadDok extends server_report_basic
{
        ...
        .badge-success{
                        ...
                        <form class='form' id='form-tambah' >
                            ...
                            <div class='box-body' style='height:400px;border:none;box-shadow:none' >
                                ...
                                <ul class='nav nav-tabs' role='tablist'>
                                    <li class='nav-item active'><a class='nav-link active' data-toggle='tab' href='#detDok' role='tab' aria-selected='true'><span class='hidden-xs-down'>Detail Dokumen</span></a> </li>
                                </ul>
          ...
          <script type='text/javascript'>
            ...
            var dataTable = $('#table-data').DataTable({
                ...
                'serverSide': true,
                'ajax': {
                    'url': '$root_ser/UploadDok.php?fx=getData',
            ...
            function getJenis(param,kode_jenis=null){
                $.ajax({
                    type: 'GET',
                    url: '$root_ser/UploadDok.php?fx=getJenis',
                    dataType: 'json',
            ...
            $('#sai_container').on('click','#btn-upload',function(e){
                ...
                $.ajax({
                    type: 'GET',
                    url: '$root_ser/UploadDok.php?fx=getUploadData',
                    dataType: 'json',
                    async:false,
                        ...
                        if(result.status){
                            ...
                            if(result.daftar.length > 0){
                                for(var i=0;i < result.daftar.length;i++){
                                    ...
                                    input += `<td ><a class='btn btn-danger btn-sm hapus-dok2' title='Hapus'><i class='fa fa-times fa-1'></i></a>&nbsp;<a class='btn btn-success btn-sm download-dok' title='Download' href='https://api.simkug.com/api/aset/storage/`+line.file_dok+`' target='_blank'><i class='fa fa-download fa-1'></i></a></td>`;
                                    input += `</tr>`;
                                    no++;
                                }
            ...
            $('#saku-form').on('click', '#add-row', function(){
                ...
                `</td>`;
                input += `<td ><a class='btn btn-danger btn-sm hapus-dok' title='Hapus'><i class='fa fa-times fa-1'></i></a></td>`;
                input += `</tr>`;
            ...
            $('#saku-form').on('submit', '#form-tambah', function(e){
                ...
                $.ajax({
                    type: 'POST',
                    url: '$root_ser/UploadDok.php?fx=simpanDok',
                    dataType: 'json',
                    data: formData,
                    contentType: false,
                    cache: false,
