...
class server_report_saku3_amu_rptAsetRuang extends server_report_basic
{
	...
	function getHtml()
	{
        ...
        $kode_lokasi=$tmp[0];
        $kode_pp=$tmp[1];
        $sql="select a.no_ruangan,a.kode_lokasi,a.nama_ruangan,a.id_gedung,isnull(b.jumlah,0) as jumlah,isnull(b.nilai_perolehan,0) as nilai_perolehan
from amu_ruangan a
...
order by a.no_ruangan";
   ...
   <tr bgcolor='#CCCCCC'>
     ...
     <td width='80'  align='center' class='header_laporan'>Kode</td>
	  <td width='300'  align='center' class='header_laporan'>Nama Ruangan</td>
     ...
     <td class='isi_laporan'>$row->no_ruangan</td>
	 <td class='isi_laporan'>";
		echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenBarang('$row->id_gedung','$row->no_ruangan','$row->kode_lokasi');\">$row->nama_ruangan</a>";
		echo "</td>
