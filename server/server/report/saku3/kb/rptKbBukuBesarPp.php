...
class server_report_saku3_kb_rptKbBukuBesarPp extends server_report_basic
{
...
order by a.kode_akun";
  ...
  <tr bgcolor='#CCCCCC'>
    ...
    <td width='90' class='header_laporan' align='center'>Balance</td>
	   ...
	   case when a.dc='C' then nilai else 0 end as kredit,a.tanggal 
from kas_j a 
where a.kode_lokasi='$row->kode_lokasi' and a.kode_akun='$row->kode_akun' and a.kode_pp='$row->kode_pp'
$sql2
union all
select a.no_rekon as no_kas,a.no_dokumen,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.kode_akun,a.kode_pp,a.kode_drk,a.keterangan,
case when a.dc='D' then nilai else 0 end as debet,
case when a.dc='C' then nilai else 0 end as kredit,a.tanggal 
from sis_rekon_j a 
where a.kode_lokasi='$row->kode_lokasi' and a.kode_akun='$row->kode_akun' and a.kode_pp='$row->kode_pp' 
