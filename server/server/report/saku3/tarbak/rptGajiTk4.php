...
class server_report_saku3_tarbak_rptGajiTk4 extends server_report_basic
{
	...
	function getHtml()
	{
		...
		echo $AddOnLib->judul_laporan("Daftar Tambahan Gaji Guru & Karyawan Tetap TK",$this->lokasi,"");
   ...
   <tr bgcolor='#CCCCCC'>
	 ...
	 <td width='90'  align='center' class='header_laporan'>Total</td>
		...
		{
			$pdpt_lbr=$row->jam_lembur*$row->lbr;
			$pdpt_inv=$row->jam_inval*$row->inv;
			$pdpt_mtap=$row->jam_mantap*$row->mtap;
			$pdpt_remed=$row->jam_remed*$row->rmed;
			$pdpt_eks=$row->jam_ekskul*$row->eks;
			$total=$pdpt_lbr+$pdpt_inv+$pdpt_mtap+$pdpt_remed+$pdpt_eks;
		echo "<tr >
	 ...
	 <td class='isi_laporan' align='right'>".number_format($pdpt_lbr,0,',','.')."</td>
	 <td class='isi_laporan' align='right'>$row->jam_inval</td>
	 <td class='isi_laporan' align='right'>".number_format($pdpt_inv,0,',','.')."</td>
	 <td class='isi_laporan' align='right'>$row->jam_mantap</td>
	 <td class='isi_laporan' align='right'>".number_format($pdpt_mtap,0,',','.')."</td>
	 <td class='isi_laporan' align='right'>$row->jam_remed</td>
	 <td class='isi_laporan' align='right'>".number_format($pdpt_remed,0,',','.')."</td>
	 <td class='isi_laporan' align='right'>$row->jam_ekskul</td>
	 <td class='isi_laporan' align='right'>".number_format($pdpt_eks,0,',','.')."</td>
