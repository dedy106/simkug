...
class server_report_saku3_tarbak_rptGajiSma1 extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$periode=$tmp[0];
		$nama_cab=$tmp[1];
		$sql=" select a.nik,a.nama as nip,b.periode,a.kode_sdm,b.prestasi,b.absen as absen1,a.kode_gol,a.mk_gol,a.mk_ytb,a.kode_pajak,
		...
		sum(case when c.kode_param='RPL' then c.nilai else 0 end) as rpl,
		...
		sum(case when c.kode_param in ('LBR','INV','MTAP','RMED','EKS') then c.nilai else 0 end) as jml
		 ...
		 $this->filter and a.kode_unit='SMA' and a.kode_sdm<>'GH'
		...
		echo $AddOnLib->judul_laporan("Daftar Gaji Guru & Karyawan Tetap SMA",$this->lokasi,"");
   ...
   <tr bgcolor='#CCCCCC'>
     ...
     <td width='80'  rowspan='2' align='center' class='header_laporan'>NIP</td>
	  ...
	  <td width='90' rowspan='2' align='center' class='header_laporan'>Rapel</td>
