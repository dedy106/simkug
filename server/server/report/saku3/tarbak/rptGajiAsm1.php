...
class server_report_saku3_tarbak_rptGajiAsm1 extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$periode=$tmp[0];
		$nama_cab=$tmp[1];
		$sql=" select a.nik,a.nama as nip,b.periode,a.kode_sdm,b.prestasi,b.absen,a.kode_gol,a.mk_gol,a.mk_ytb,a.kode_pajak,
		...
		sum(case when c.kode_param='KJM' then c.nilai else 0 end) as kjm,
		sum(case when c.kode_param='TRANS' then c.nilai else 0 end) as trans,
		sum(case when c.kode_param='ABSEN' then c.nilai else 0 end) as absen1,
		...
		sum(case when c.kode_param in ('LBR','INV','MTAP','RMED','EKS') then c.nilai else 0 end) as jml
		 from hr_karyawan a
		 ...
		 $this->filter and a.kode_unit='ASM' and a.kode_sdm<>'GH'
		...
		echo $AddOnLib->judul_laporan("Daftar Gaji Guru & Karyawan Tetap ASM",$this->lokasi,"");
   ...
   <tr bgcolor='#CCCCCC'>
     ...
     <td width='80'  rowspan='2' align='center' class='header_laporan'>NIP</td>
	  ...
	  <td width='90' rowspan='2' align='center' class='header_laporan'>KJM</td>
	  <td width='90' rowspan='2' align='center' class='header_laporan'>Honororium Kelas Sore</td>
	  ...
	  <tr bgcolor='#CCCCCC'>
		...
		{
			$bruto=$row->gaji+$row->ins+$row->tytb+$row->tabdi+$row->twali+$row->psp+$row->pumk+$row->kjm;
			...
			$jml1=$jml+$row->jml;
			$absen=$absen+$row->absen;
			$absen1=$absen1+$row->absen1;
			$ijht=$ijht+$row->ijht;
			$jp=$jp+$row->jp;
			$pot1=$pot+$row->pot;
			$bank1=$bank+$row->bank;
	 ...
	 <td class='isi_laporan' align='right'>".number_format($row->kjm,0,',','.')."</td>
	 <td class='isi_laporan' align='right'>".number_format($row->kjm,0,',','.')."</td>
	 ...
	 <td class='isi_laporan' align='right'>".number_format($row->absen1,0,',','.')."</td>
	 ...
	 <td class='isi_laporan' align='right'>".number_format($netto,0,',','.')."</td>
		...
		<td class='header_laporan' align='right'>".number_format($absen2,0,',','.')."</td>
