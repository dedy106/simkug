...
class server_report_saku3_tarbak_rptHrGajiRekap extends server_report_basic
{
...
order by a.no_urut ";
		...
		echo "</tr>";
		//data gaji
		$sql="select a.nik,c.nama,c.kode_lokasi,$pdpt1,$beban1,dbo.fnGetPph(a.nik,'$periode') as pph21
from (select distinct b.nik
	  from hr_gaji_d b
	  inner join hr_karyawan a on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi
	  $this->filter
	   ) a
inner join hr_karyawan c on a.nik=c.nik
inner join (select b.nik,$pdpt2,$beban2 
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenSlip('$row->nik','$row->kode_lokasi');\">$row->nik</a>";
			echo "</td>";
