...
class server_report_saku3_siswa_rptSisRekonSma extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		isnull(b.n11,0) as n11,isnull(b.total,0) as total 
from sis_rekon_m a
inner join (select c.nis,a.no_rekon,c.kode_lokasi,
			      sum(case when a.kode_param in ('USSMA') then a.nilai else 0 end) as n1, 
				   sum(case when a.kode_param in ('ULSMA') then a.nilai else 0 end) as n2, 
				   sum(case when a.kode_param in ('PRAKSMA') then a.nilai else 0 end) as n3,
				   sum(case when a.kode_param in ('KOMSMA') then a.nilai else 0 end) as n4,
				   sum(case when a.kode_param in ('UASSMA') then a.nilai else 0 end) as n5, 
				   sum(case when a.kode_param in ('EKSSMA') then a.nilai else 0 end) as n6, 
				   sum(case when a.kode_param in ('INTSMA') then a.nilai else 0 end) as n7,
				   sum(case when a.kode_param in ('KEAGSMA') then a.nilai else 0 end) as n8,
				   sum(case when a.kode_param in ('OUTSMA') then a.nilai else 0 end) as n9,
				   sum(case when a.kode_param in ('BINGSMA') then a.nilai else 0 end) as n10,
				   sum(case when a.kode_param in ('DDSMA') then a.nilai else 0 end) as n11,
...
order by b.nis
 ...
 ";
  ...
  <tr bgcolor='#CCCCCC'>
    ...
    <td width='80' align='center' class='header_laporan'>Ulangan Umum</td>
    <td width='80' align='center' class='header_laporan'>Praktikum</td>
	<td width='80' align='center' class='header_laporan'>Komputer</td>
	<td width='90' align='center' class='header_laporan'>UAS</td>
    <td width='80' align='center' class='header_laporan'>EKSKUL</td>
    <td width='80' align='center' class='header_laporan'>I-NET</td>
    <td width='80' align='center' class='header_laporan'>KE AG</td>
	<td width='80' align='center' class='header_laporan'>KBM L.S</td>
