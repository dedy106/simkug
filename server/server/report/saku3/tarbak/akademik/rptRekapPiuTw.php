...
class server_report_saku3_tarbak_akademik_rptRekapPiuTw extends server_report_basic
{	
...
left join (select y.nis,y.kode_lokasi,  
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode < '$periode_awal') and x.kode_pp='$kode_pp'			
...
left join (select y.nis,y.kode_lokasi,  
			...
			from sis_bill_d x 			
			inner join sis_siswa y on x.nis=y.nis and x.kode_lokasi=y.kode_lokasi and x.kode_pp=y.kode_pp
			where(x.kode_lokasi = '$kode_lokasi') and x.kode_pp='$kode_pp' $tw			
			group by y.nis,y.kode_lokasi 			
...
left join (select y.nis,y.kode_lokasi,  
			...
			from sis_rekon_d x 	
			inner join sis_siswa y on x.nis=y.nis and x.kode_lokasi=y.kode_lokasi and x.kode_pp=y.kode_pp
			where(x.kode_lokasi = '$kode_lokasi') and x.kode_pp='$kode_pp' and(x.periode < '$periode_awal')
			group by y.nis,y.kode_lokasi 			
...
left join (select y.nis,y.kode_lokasi, 
			...
			where(x.kode_lokasi = '$kode_lokasi') and x.kode_pp='$kode_pp' $dw		
			group by y.nis,y.kode_lokasi 			
...
left join (select y.nis,y.kode_lokasi, 
			...
			inner join sis_siswa y on x.nis=y.nis and x.kode_lokasi=y.kode_lokasi and x.kode_pp=y.kode_pp
			inner join sis_bill_m z on x.no_bill=z.no_bill and x.kode_lokasi=z.kode_lokasi
			where(x.kode_lokasi = '$kode_lokasi') and x.kode_pp='$kode_pp' and z.periode < '$periode_awal' $tw	
			group by y.nis,y.kode_lokasi 			
...
order by a.kode_kelas,a.nis
 ";
  ...
  <tr bgcolor='#CCCCCC'>
	...
	<td width='100' align='center' class='header_laporan' colspan='4'>Tagihan TW1</td>
    <td width='200' align='center' class='header_laporan' colspan='4'>Pembayaran TW1 Berjalan</td>
   ...
   echo
	 "<tr>
    <td class='header_laporan' align='center' colspan='7'></td>
