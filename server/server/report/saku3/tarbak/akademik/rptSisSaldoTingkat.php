...
class server_report_saku3_siswa_rptSisSaldoTingkat extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$kode_lokasi=$tmp[0];
		$periode=$tmp[1];
		$sql="select a.kode_tingkat,a.nama,a.kode_lokasi
...
from sis_tingkat a 
left join (select y.kode_tingkat,y.kode_lokasi,  
			...
			group by y.kode_tingkat,y.kode_lokasi 			
			)b on a.kode_tingkat=b.kode_tingkat and a.kode_lokasi=b.kode_lokasi
left join (select y.kode_tingkat,y.kode_lokasi,  
				sum(case when x.kode_param in ('USTK','USSMP','USSMA','USSD') then x.nilai else 0 end) as n1, 
				   sum(case when x.kode_param not in ('USTK','USSMP','USSMA','USSD','DDTK','DDSMP','DDSMA','DDSD') then x.nilai else 0 end) as n2, 
				   sum(case when x.kode_param in ('DDTK','DDSMP','DDSMA','DDSD') then x.nilai else 0 end) as n3,
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode = '$periode')			
			group by y.kode_tingkat,y.kode_lokasi 			
			)c on a.kode_tingkat=c.kode_tingkat and a.kode_lokasi=c.kode_lokasi
left join (select y.kode_tingkat,y.kode_lokasi,  
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode <'$periode')			
			group by y.kode_tingkat,y.kode_lokasi 			
			)d on a.kode_tingkat=d.kode_tingkat and a.kode_lokasi=d.kode_lokasi
left join (select y.kode_tingkat,y.kode_lokasi, 
			    sum(case when x.kode_param in ('USTK','USSMP','USSMA','USSD') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n1, 
				   sum(case when x.kode_param not in ('USTK','USSMP','USSMA','USSD','DDTK','DDSMP','DDSMA','DDSD') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n2, 
				   sum(case when x.kode_param in ('DDTK','DDSMP','DDSMA','DDSD') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n3,
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode ='$periode')			
			group by y.kode_tingkat,y.kode_lokasi 			
			)e on a.kode_tingkat=e.kode_tingkat and a.kode_lokasi=e.kode_lokasi
$this->filter
order by a.nu
 ";
		//echo $sql;
		...
		echo $AddOnLib->judul_laporan("saldo pembayaran per tingkat",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr align='center' bgcolor='#CCCCCC'>
    ...
    <td width='200' rowspan='2' class='header_laporan'>Nama Tingkat </td>
  ...
  <tr bgcolor='#CCCCCC'>
   ...
   <td class='isi_laporan' align='center'>$i</td>
    ...
    <td class='isi_laporan'>";
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTingkat('$row->kode_tingkat','$row->kode_lokasi');\">$row->nama</a>";
			echo "</td>
