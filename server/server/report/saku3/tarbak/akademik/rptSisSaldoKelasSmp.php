...
class server_report_saku3_siswa_rptSisSaldoKelasSmp extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$kode_lokasi=$tmp[0];
		$periode=$tmp[1];
		$sql="select a.kode_kelas,a.nama,a.kode_lokasi
...
left join (select y.kode_kelas,y.kode_lokasi,  
					sum(case when x.kode_param in ('USSMP') then x.nilai else 0 end) as n1, 
				   sum(case when x.kode_param in ('TPBSMP','UASSMP','KETSMP','BTESMP','KOMSMP','INTSMP','BINGSMP','EKSSMP','OUTSMP','KEAGSMP') then x.nilai else 0 end) as n2, 
				   sum(case when x.kode_param in ('DDSMP') then x.nilai else 0 end) as n3,
			...
			)b on a.kode_kelas=b.kode_kelas and a.kode_lokasi=b.kode_lokasi
left join (select y.kode_kelas,y.kode_lokasi,  
				sum(case when x.kode_param in ('USSMP') then x.nilai else 0 end) as n1, 
				   sum(case when x.kode_param in ('TPBSMP','UASSMP','KETSMP','BTESMP','KOMSMP','INTSMP','BINGSMP','EKSSMP','OUTSMP','KEAGSMP') then x.nilai else 0 end) as n2, 
				   sum(case when x.kode_param in ('DDSMP') then x.nilai else 0 end) as n3,
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode = '$periode')			
			group by y.kode_kelas,y.kode_lokasi 			
			)c on a.kode_kelas=c.kode_kelas and a.kode_lokasi=c.kode_lokasi
left join (select y.kode_kelas,y.kode_lokasi,  
				  sum(case when x.kode_param in ('USSMP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n1, 
				   sum(case when x.kode_param in ('TPBSMP','UASSMP','KETSMP','BTESMP','KOMSMP','INTSMP','BINGSMP','EKSSMP','OUTSMP','KEAGSMP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n2, 
				   sum(case when x.kode_param in ('DDSMP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n3,
...
order by a.kode_kelas
 ";
		...
		$i = 1;
		$jum=$rs->recordcount();
		$ADDTKOnLib=new server_util_AddOnLib();
		echo "<div align='center'>"; 
		echo $ADDTKOnLib->judul_laporan("saldo pembayaran per kelas tingkat smp",$this->lokasi,"PERIODE ".$ADDTKOnLib->ubah_periode($periode));
		$resource = $_GET["resource"];
		$fullId = $_GET["fullId"];
		echo "<table border='1' cellspacing='0' cellpaDDTKing='0' class='kotak' width='1500'>
  <tr align='center' bgcolor='#CCCCCC'>
    <td width='20' rowspan='2' class='header_laporan'>No</td>
    <td width='50' rowspan='2' class='header_laporan'>Kode </td>
    <td width='200' rowspan='2' class='header_laporan'>Nama Kelas </td>
  ...
  <tr bgcolor='#CCCCCC'>
   ...
   <td class='isi_laporan' align='center'>$i</td>
    ...
    <td class='isi_laporan'>";
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenKelas('$row->kode_kelas','$row->kode_lokasi');\">$row->nama</a>";
			echo "</td>
