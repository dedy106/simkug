...
class server_report_saku3_dash_rptDashPbyrDetail extends server_report_basic
{
                            ...
                            <div>
                                <table class='table no-margin' id='table-progress-tagihan'>
                                    ...
                                    if ($jenis=="all")
									{
                                        ...
                                        $sql = "select a.no_bayar,b.nama,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.periode,c.nilai
                                            from dev_bayar_m a
                                            inner join dev_siswa b on a.nim=b.nim and a.kode_lokasi=b.kode_lokasi
                                            inner join (select no_bayar,kode_lokasi,sum(nilai) as nilai from dev_bayar_d group by no_bayar,kode_lokasi) c on a.no_bayar=c.no_bayar and a.kode_lokasi=c.kode_lokasi
