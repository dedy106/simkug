...
class server_report_saku3_dash_rptDashYakesNotif extends server_report_basic
{
	...
	function getHtml()
	{
        ...
        <div class='panel' style='box-shadow:none'>
            <div class='panel-heading text-right' style='font-size:18px;padding-bottom:0'> 
                <button type='button' id='btn-refresh' style='border: 1px solid #d5d5d5;border-radius: 20px;padding: 5px 20px;background: white;'>Refresh
                </button>
            </div>
                ...
                <div class='row'>
                    <div style='background: #d1d1d6;height: 400px;border-top-right-radius: 30px;border-bottom-right-radius: 30px;' class='col-xs-1'>
                        <ul class='nav nav-tabs tabs-left sideways' style='margin: 50px auto;border: none;width:50px'>
                            <li role='presentation' class='active'><a data-toggle='tab' href='#home'><i class='fa fa-filter fa-n'></i></a></li>
        ...
        OneSignal.push(function() {
            OneSignal.init({
                appId: 'b0bfb1bb-6ab8-450e-8ccd-f0bfc4098ca4',
            });
            OneSignal.isPushNotificationsEnabled().then(function(isEnabled) {
                if (isEnabled){
                    OneSignal.getUserId().then(function(userId) {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: 'http://".$_SERVER['SERVER_NAME']."/web/server/yakes/Notif.php?fx=register',
                            dataType: 'json',
                            async:false,
        ...
        $('#formHapusPesan').submit(function(e){
            ...
            $.ajax({
                type: 'POST',
                url: 'http://".$_SERVER['SERVER_NAME']."/web/server/yakes/Notif.php?fx=delPesan',
                dataType: 'json',
                async:false,
                data: formData,
                contentType: false,
        ...
        $('#formSetting').submit(function(e){
            ...
            $.ajax({
                type: 'POST',
                url: 'http://".$_SERVER['SERVER_NAME']."/web/server/yakes/Notif.php?fx=updateSetting',
                dataType: 'json',
                async:false,
                data: formData,
                contentType: false,
        ...
        $('.panel').on('click', '.kelola', function(){
            ...
            loadService('Deposito','GET','$root_ser/dashYakesInves.php?fx=getDeposito','2019-08-25|1|5050|'+jenis+'|'+kelola); 
        });
        ...
        $('.panel').on('click', '.pesan', function(){
            ...
            var kode = $(this).find('p').text();
            $('#inp-pesan').text(kode);
            loadService('GetNotif','GET','$root_ser/Notif.php?fx=getNotif2','$kode_lokasi|$nik|'+kode+'|'+idUser); 
        });
        ...
        $('.panel').on('click', '.klik', function(){
            ...
            if(tmp[2]== 'boxAktif'){
                ...
                var kelola =   $('#inp-kelola').text();
                var jenis =   $('#inp-jenis').text();
                loadService('DetailDepo','GET','$root_ser/dashYakesInves.php?fx=getDeposito','2019-08-25|1|5050|'+jenis+'|'+kelola+'|'+kode);     
            }
        ...
        $('.panel').on('click', '.klik2', function(){
            ...
            if(tmp[3]== 'box2Aktif' || tmp[2] == 'box2Aktif' ){
                ...
                loadService('detailNotif','GET','$root_ser/Notif.php?fx=getDetailNotif','$kode_lokasi|$nik|'+sts+'|'+kode+'|'+idUser);     
            }
        ...
        function sepNum(x){
                            ...
                            switch(index){
                                ...
                                case 'Deposito':
                                    ...
                                    for(var i=0;i<result.daftar.length;i++){
                                        ...
                                        html+=`<div class='row klik boxNonAktif' style='padding: 5px;vertical-align: middle;background:white;border-radius: 15px;margin-bottom: 10px;'>
            ...
            function initDash(){
                    ...
                    loadService('Deposito','GET','$root_ser/dashYakesInves.php?fx=getDeposito','2019-08-25|1|5050|semua|semua'); 
                    loadService('GetNotif','GET','$root_ser/Notif.php?fx=getNotif','$kode_lokasi|$nik|'+idUser);
            }
