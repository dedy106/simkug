...
class server_report_saku3_dash_rptDashYakesCashOut extends server_report_basic
{
	...
	function getHtml()
	{
        ...
        <div class='panel' style=''>
            ...
            white;'> 
                ...
                Pendapatan dan Cash Out <span id='jplan'></span></h3>
        ...
        <div id='mySidepanel' class='sidepanel close'>
            ...
            <a href='#' data-id='cash_out' class='sidepanel-link' style='color:#4274FE'>Pendapatan dan Cash Out</a>
        ...
        <script>
            ...
            $('.modal-body').on('click','#btnOk',function(){
                ...
                $.ajax({
                    ...
                    success:function(result){  
                        ...
                        if(result.status){
                            loadService('planAset','GET','$root_ser/dashYakesInves.php?fx=getCashOut',filtertgl+'|'+plan+'|'+kode_klp);
                        ...
                        if(tglreverse > '$tglakhirx'){
                            ...
                            $.ajax({
                                ...
                                success:function(result){  
                                    alert(result.message);  
                                    if(result.status){
                                        loadService('planAset','GET','$root_ser/dashYakesInves.php?fx=getCashOut',tglreverse+'|'+plan+'|'+kode_klp);
                                        $('aside').removeClass('control-sidebar-open');
                                        $('.tab-content').hide();
                                    }
                                }
                ...
                $.ajax({
                    ...
                    success:function(result){    
                        if(result.status){
                            switch(index){
                                case 'planAset':
                                    var pdpt = parseFloat(result.daftar[0].pdpt);
                                    var binves = parseFloat(result.daftar[0].binves);
                                    var bkes = parseFloat(result.daftar[0].bkes);
                                    var cc = parseFloat(result.daftar[0].cc);
                                    var capex = parseFloat(result.daftar[0].capex);
                                    var opex = parseFloat(result.daftar[0].opex)-bkes-binves;
                                    var penambahan = pdpt-(binves+bkes+cc+capex+opex);
                                    var spi = parseFloat(result.daftar[0].spi);
            ...
            function initDash(){ 
                loadService('planAset','GET','$root_ser/dashYakesInves.php?fx=getCashOut','$tglakhir|$kode_plan|$kode_klp');
            }
