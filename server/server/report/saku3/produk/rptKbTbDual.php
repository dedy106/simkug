...
class server_report_saku3_produk_rptKbTbDual extends server_report_basic
{
  ...
  <tr align='center' bgcolor='#dbeef3'>
    ...
    <td width='90' class='header_laporan'>Penerimaan</td>
    ...
    <td width='100' class='header_laporan'>Saldo Akhir</td>
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			echo "<tr>";
	echo "<td class='isi_laporan'><a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenBb('$row->kode_akun','$row->kode_lokasi','$periode','$row->kode_pp');\">".$row->kode_akun."</a></td>";
   ...
   echo "<td height='20' class='isi_laporan' align='right'><a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenMasuk('$row->kode_akun','$row->kode_lokasi','$periode','$row->kode_pp');\">".number_format($row->debet,0,',','.')."</a></td>";
	...
	echo "<td valign='top' class='isi_laporan' align='right'>".number_format($row->so_akhir,2,',','.')."</td>
  </tr>";
			$i=$i+1;
		}
