...
class server_report_saku3_yakes_inves_rptUploadRdExcel extends server_report_basic
{
      ...
      echo"
        ...
        th{
                    ...
                    <div class='col-xs-12'>
                      <div class='nav-tabs-custom'>
                          <ul class='nav nav-tabs'>
                            <li class='active'><a href='#tab_1' data-toggle='tab' aria-expanded='true'>Harga Reksadana</a></li>
                          ...
                          <div class='tab-content'>
                            <div class='tab-pane active' id='tab_1'>
                              <div class='sai-container-overflow-x' style='75%'>
                                <table class='table table-bordered table-striped Datatable' id='table-rd' style='font-size: 12px;' >
                                    <thead style='background: #ff9500;color: white;'>
                                      <tr>
                                      <th>Kode Reksadana</th>
                                      <th>TGL-1</th>
                                      <th>TGL-2</th>
                                      <th>TGL-3</th>
                                      <th>TGL-4</th>
          ...
          function clearTmp(){
              $.ajax({
                  type: 'GET',
                  url: '$root_ser/UploadExcel.php?fx=clearTmpRd',
                  dataType: 'json',
                  data: {'kode_lokasi':'$kode_lokasi'},
                  success:function(result){  
                      if(result.status){
          ...
          // var dataTable = $('#table-rd').DataTable({
          //   'processing': true,
          //   'serverSide': true,
          //   'ajax': {
          //       'url': '$root_ser/UploadExcel.php?fx=getHargaRd',
          ...
          var dataTable = $('#table-rd').DataTable({
            'ajax': {
                'url': '$root_ser/UploadExcel.php?fx=getHargaRd2',
                'data': {'kode_lokasi':'$kode_lokasi','periode':'$periode'},
                'type': 'GET',
                'dataSrc' : function(json) {
                    return json.data;   
          ...
          $('#nw_container').on('submit', '#nw_form_insert', function(e){
            ...
            $.ajax({
              type: 'POST',
              url: '$root_ser/UploadExcel.php?fx=simpanRdTmp',
              dataType: 'json',
              data: formData,
              contentType: false,
              cache: false,
              ...
              success:function(result){
                ...
                if(result.status){
                  ...
                  for(var i=0;i< result.error_ins.length;i++){
                      html +=`<tr>
                        <td>`+result.error_ins[i].kode_rd+`</td>
          ...
          $('#nw_container').on('click', '#saveBtn', function(e){
            ...
            if(per ==''){
              ...
              $.ajax({
                type: 'POST',
                url: '$root_ser/UploadExcel.php?fx=simpanRdHarga',
                dataType: 'json',
