...
class server_report_saku3_yakes_inves_rptRekapSaham extends server_report_basic
{
	...
	function getHtml()
	{
        ...
        echo"<table style='border-collapse: collapse;margin:10px' cellspacing='2' cellpadding='1' width='1200' >
                <style>
                    td{
                ...
                <thead>
                    ...
                    <tr >
                        <th style='font-size:14px' colspan='14'>INSTRUMEN INVESTASI : SAHAM BURSA (SWAKELOLA)</th>
                    </tr>
                ...
                <tbody>
                    ...
                    isnull(e.jtotal,0)+isnull(h.htotal,0) revspi, 
                    (a.jumlah*a.h_buku) + isnull(e.jtotal,0)+isnull(h.htotal,0) as nbuku_revspi, 
                    (a.jumlah*a.h_buku) + isnull(e.jtotal,0)+isnull(h.htotal,0) + ((a.jumlah*a.h_wajar) - (a.jumlah*a.h_buku)) as nwajar_revspi 
                    ...
                    left join (	
                                ...
                                $this->filter $filter_periode2	
                                group by a.kode_kelola,a.kode_saham,a.kode_plan
                                ) h on a.kode_saham=h.kode_saham and a.kode_kelola=h.kode_kelola  
                    
                    $this->filter and a.tanggal = '".$tmp[1]."' $fil_mutasi
                    order by a.kode_saham,a.kode_kelola";
                            
                    $rs = $dbLib->execute($sql);
                    $noleh= 0;$nbuku=0;$nwajar=0;$spioleh=0;$spibuku=0;$nkomisi=0;$jumlah=0;$revspi=0;$nbuku_revspi=0;$nwajar_revspi=0;
                    while ($row = $rs->FetchNextObject($toupper=false))
		            {
                        $noleh+=+$row->noleh;
                        $nbuku+=+$row->nbuku;
