...
class server_report_saku3_yakes_inves_rptRoiSP extends server_report_basic
{
	...
	function getHtml()
	{
        ...
        $sql = "select distinct a.kode_mitra,b.nama
        from inv_sp_kkp a
        left join inv_mitra b on a.kode_mitra=b.kode_mitra
		$this->filter
        ";

        $res = $dbLib->execute($sql);
        if($res->recordCount() == 1){
            $nama_mitra = strtoupper($res->fields[1]);
        }else{
            $nama_mitra = "";
        }
        ...
        echo"<table style='border-collapse: collapse;margin:10px' cellspacing='2' cellpadding='1' width='985'>
                ...
                <thead>
                    <tr>
                        <th colspan='14' style='font-size:14px;text-align:left'>LAPORAN ROI PRODUK SAHAM PENYERTAAN $nama_mitra YAKES TELKOM </th>
                    </tr>
