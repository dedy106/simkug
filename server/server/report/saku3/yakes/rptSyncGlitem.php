...
class server_report_saku3_yakes_rptSyncGlitem extends server_report_basic
{
          ...
          $('#nw_container').on('click', '#saveBtn', function(e){
              ...
              if(per ==''){
                ...
                $.ajax({
                  ...
                  type: 'POST',
                  url: '$root_ser/SyncGlitem.php?fx=simpan4',
                  dataType: 'json',
                  data: {'kode_lokasi':'$kode_lokasi','periode':per},
                  success:function(result){
                    alert('Sync data '+result.message); 
                    $('#progress-load').hide();  
                    $('#isi-sql').html(result.sql[0].sql);                
                  },
