...
class server_report_saku3_klinik_rptJualRekapCust extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$sql="select a.kode_cust,b.nama,sum(a.nilai-a.nilai_ppn) as nilai_jual,sum(a.nilai_ppn) as nilai_ppn,
...
order by a.kode_cust ";
		...
		echo $AddOnLib->judul_laporan("rekap penjualan per customer",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
   ...
   <td class='isi_laporan'>";
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTiket('$row->kode_cust','$row->kode_lokasi');\">$row->kode_cust</a>";
			echo "</td>
