...
class server_report_saku3_klinik_rptJualRekap extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$sql="select a.no_jual,a.kode_lokasi,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.no_dokumen,a.keterangan,a.nilai-a.nilai_ppn as nilai_jual,a.nilai_ppn,a.nilai_diskon,(a.nilai+a.nilai_ppn-a.nilai_diskon) as total,
	   a.kode_cust,b.nama as nama_cust ,a.kode_pp,c.nama as nama_pp,a.kode_pic,d.nama as nama_pic
from kli_jual_m a
...
inner join kli_pic d on a.kode_pic=d.kode_pic and a.kode_lokasi=d.kode_lokasi
$this->filter
order by a.no_jual ";
		...
		echo $AddOnLib->judul_laporan("penjualan obat",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
    ...
    <td width='100' align='center' class='header_laporan'>No Dokumen</td>
	 ...
	 <td width='120' align='center' class='header_laporan'>Marketing</td>
	...
	<td width='80' align='center' class='header_laporan'>Total</td>
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTiket('$row->no_jual','$row->kode_lokasi');\">$row->no_jual</a>";
			echo "</td>
			<td class='isi_laporan'>$row->no_dokumen</td>
			<td class='isi_laporan'>$row->tgl</td>
			<td class='isi_laporan'>$row->kode_pp</td>
			<td class='isi_laporan'>$row->kode_pic - $row->nama_pic</td>
			<td class='isi_laporan'>$row->kode_cust - $row->nama_cust</td>
