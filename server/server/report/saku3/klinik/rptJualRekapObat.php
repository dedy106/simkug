...
class server_report_saku3_klinik_rptJualRekapObat extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$sql="select b.kode_obat,c.nama,sum(b.harga*b.jumlah) as total,sum(b.jumlah) as jumlah,sum(b.bonus) as bonus,sum(b.diskon) as diskon
from kli_jual_m a
inner join kli_jual_d b on a.no_jual=b.no_jual and a.kode_lokasi=b.kode_lokasi
inner join kli_obat c on b.kode_obat=c.kode_obat and b.kode_lokasi=c.kode_lokasi
$this->filter
group by b.kode_obat,c.nama ";
		
		$rs = $dbLib->execute($sql);
		$i = 1;
		$AddOnLib=new server_util_AddOnLib();
		echo "<div align='center'>"; 
		echo $AddOnLib->judul_laporan("rekap penjualan per obat",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
