...
class server_report_saku3_klinik_rptJualRekapPic extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$sql="select a.kode_pic,b.nama,sum(a.nilai-a.nilai_ppn) as nilai_jual,sum(a.nilai_ppn) as nilai_ppn,
					sum(a.nilai_diskon) as nilai_diskon,sum(a.nilai+a.nilai_ppn-a.nilai_diskon) as total
from kli_jual_m a
inner join kli_pic b on a.kode_pic=b.kode_pic and a.kode_lokasi=b.kode_lokasi
$this->filter
group by a.kode_pic,b.nama
order by a.kode_pic ";
		...
		echo $AddOnLib->judul_laporan("rekap penjualan per marketing",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
   ...
   <td class='isi_laporan'>";
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTiket('$row->kode_pic','$row->kode_lokasi');\">$row->kode_pic</a>";
			echo "</td>
