...
class server_report_saku3_klinik_rptHutangAging extends server_report_basic
{
...
$this->filter";
	...
	function getHtml()
	{
		...
		$tgl_aging=$tmp[0];
		$sql="select a.kode_lokasi,a.no_beli,a.no_dokumen,date_format(a.tanggal,'%d/%m/%Y') as tgl,b.nama as nama_cust,a.nilai+a.nilai_ppn as tagihan,
...
left join (select a.no_beli,a.kode_lokasi,sum(a.nilai_kas) as nilai_kas
	       from kli_belibayar_d a
		   inner join kas_m b on a.no_bukti=b.no_kas and a.kode_lokasi=b.kode_lokasi
		   where b.tanggal<='$tgl_aging'  
		   group by a.no_beli,a.kode_lokasi
		   )c on a.no_beli=c.no_beli and a.kode_lokasi=b.kode_lokasi
$this->filter and a.tanggal<='$tgl_aging' order by a.no_beli";
   ...
   <tr bgcolor='#CCCCCC'>
     ...
     <td class='isi_laporan'>";
		echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenHutang('$row->no_beli','$row->kode_lokasi');\">$row->no_beli</a>";
		echo "</td>
	<td class='isi_laporan'>";
		echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenBayar('$row->no_beli','$row->kode_lokasi');\">$row->no_dokumen</a>";
		echo "</td>
