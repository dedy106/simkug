...
class server_report_saku3_klinik_rptHutangKartu extends server_report_basic
{
...
$this->filter ";
	...
	function getHtml()
	{
		...
		global $dbLib;
		$sql="select a.kode_lokasi,a.no_beli,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.keterangan,a.nilai,b.nama as nama_vendor
from kli_beli_m a
inner join kli_vendor b on a.kode_vendor=b.kode_vendor and a.kode_lokasi=b.kode_lokasi 
$this->filter order by a.no_beli";
  ...
  <tr >
    <td height='23' colspan='7' class='header_laporan'><table  border='0' cellspacing='2' cellpadding='1'>
      <tr>
        ...
        <td width='360' class='header_laporan'>: $row->no_beli</td>
      </tr>
	  <tr>
        <td width='99' class='header_laporan'>No Invoice</td>
  ...
  <tr bgcolor='#CCCCCC'>
	<td width='70' class='header_laporan' align='center'>Tanggal</td>
...
";
			...
			$sql="select a.kode_lokasi,a.no_bukti,a.no_beli,a.nilai_kas,date_format(b.tanggal,'%d/%m/%Y') as tgl_kas,b.keterangan
from kli_belibayar_d a
inner join kas_m b on a.no_bukti=b.no_kas and a.kode_lokasi=b.kode_lokasi
where a.no_beli='$row->no_beli' and a.kode_lokasi='$row->kode_lokasi' ";
			...
			while ($row1 = $rs1->FetchNextObject($toupper=false))
			{
				...
				echo "<tr>
	 <td height='23' valign='top' class='isi_laporan'>".$row1->tgl_kas."</td>
