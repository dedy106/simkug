...
class server_report_saku3_klinik_rptPiutangAging extends server_report_basic
{
...
$this->filter";
	...
	function getHtml()
	{
		...
		$tgl_aging=$tmp[0];
		$sql="select a.kode_lokasi,a.no_jual,a.no_dokumen,date_format(a.tanggal,'%d/%m/%Y') as tgl,b.nama as nama_cust,a.nilai+a.nilai_ppn as tagihan,
...
$this->filter and a.tanggal<='$tgl_aging' order by a.no_jual";
   ...
   <tr bgcolor='#CCCCCC'>
     ...
     <td class='isi_laporan'>";
		echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenHutang('$row->no_jual','$row->kode_lokasi');\">$row->no_jual</a>";
		echo "</td>
	<td class='isi_laporan'>";
		echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenBayar('$row->no_jual','$row->kode_lokasi');\">$row->no_dokumen</a>";
		echo "</td>
