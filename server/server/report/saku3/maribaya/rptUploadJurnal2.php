...
class server_report_saku3_maribaya_rptUploadJurnal2 extends server_report_basic
{
    ...
    echo "
      ...
      echo"
        ...
        th{
                    ...
                    .nav-tabs-custom2.tab-default > .nav-tabs > li.active {
                      ...
                      <div class='nav-tabs-custom2'>
                          <ul class='nav nav-tabs'>
                            <li class='active'><a href='#tab_1' data-toggle='tab' aria-expanded='true'>Data Jurnal</a></li>
          ...
          var periode_select =  $('#per option:selected').val();

          // var dataTable = $('#table-xjan').DataTable({
          //   'processing': true,
          //   'serverSide': true,
          //   'ajax': {
          //       'url': '$root_ser/UploadExcel.php?fx=getJurnal2',
          //       'data': {'kode_lokasi':'$kode_lokasi','periode':periode_select},
          //       'type': 'GET',
          ...
          var table = $('#table-xjan').DataTable({
            data: [],
            columns: [
                { data: 'no_bukti' },
                { data: 'tanggal' },
          ...
          $('#nw_container').on('submit', '#nw_form_insert', function(e){
            ...
            $.ajax({
              type: 'POST',
              url: '$root_ser/UploadExcel.php?fx=simpanJurnal2Tmp',
              dataType: 'json',
              data: formData,
              contentType: false,
              cache: false,
              ...
              success:function(result){
                ...
                if(result.status){
                  // periode_select = $('#per')[0].selectize.getValue();
          ...
          $('#nw_container').on('click', '#saveBtn', function(e){
            ...
            if(per ==''){
              ...
              $.ajax({
                type: 'POST',
                url: '$root_ser/UploadExcel.php?fx=simpanJurnal2',
                dataType: 'json',
                data: {'kode_lokasi':'$kode_lokasi','periode':per,'cek':cek},
                success:function(result){
                  alert('Input data '+result.message);
                  if(result.status){
