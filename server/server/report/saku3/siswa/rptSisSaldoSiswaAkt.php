...
class server_report_saku3_siswa_rptSisSaldoSiswaAkt extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$sql="select a.kode_lokasi,a.kode_pp,a.kode_akt,a.nama
...
from sis_angkat a 
left join (select y.kode_akt,y.kode_lokasi,y.kode_pp, 
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode < '$periode') $filter_pp			
			group by y.kode_akt,y.kode_lokasi,y.kode_pp 			
			)b on a.kode_akt=b.kode_akt and a.kode_lokasi=b.kode_lokasi and a.kode_pp=b.kode_pp
left join (select y.kode_akt,y.kode_lokasi,y.kode_pp, 
				sum(case when x.kode_param in ('DSP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n1, 
				   sum(case when x.kode_param in ('SPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end)  as n2, 
				   sum(case when x.kode_param not in ('DSP','SPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end)  as n3,
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode = '$periode') $filter_pp			
			group by y.kode_akt,y.kode_lokasi,y.kode_pp 			
			)c on a.kode_akt=c.kode_akt and a.kode_lokasi=c.kode_lokasi and a.kode_pp=c.kode_pp
left join (select y.kode_akt,y.kode_lokasi,y.kode_pp,  
				sum(case when x.kode_param in ('DSP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n1, 
				   sum(case when x.kode_param in ('SPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end)  as n2, 
				   sum(case when x.kode_param not in ('DSP','SPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end)  as n3,
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode <'$periode')	$filter_pp		
			group by y.kode_akt,y.kode_lokasi,y.kode_pp		
			)d on a.kode_akt=d.kode_akt and a.kode_lokasi=d.kode_lokasi and a.kode_pp=d.kode_pp
left join (select y.kode_akt,y.kode_lokasi,y.kode_pp, 
			  sum(case when x.kode_param in ('DSP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n1, 
				   sum(case when x.kode_param in ('SPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end)  as n2, 
				   sum(case when x.kode_param not in ('DSP','SPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end)  as n3,
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode ='$periode') $filter_pp			
			group by y.kode_akt,y.kode_lokasi,y.kode_pp		
			)e on a.kode_akt=e.kode_akt and a.kode_lokasi=e.kode_lokasi and a.kode_pp=e.kode_pp
$this->filter
order by a.kode_akt
 ";
