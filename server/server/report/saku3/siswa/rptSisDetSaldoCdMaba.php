...
class server_report_saku3_siswa_rptSisDetSaldoCdMaba extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$sql="select distinct a.no_bukti,b.keterangan,a.kode_lokasi
             ...
             $this->filter
             and b.form in ('CDIN','CDMUTMABA','CDINMABA')
        ";
            ...
            <tr>
              <td>
                <table width='800' border='1' cellspacing='0' cellpadding='0' class='kotak'>
                    ...
                    <td width='100' class='header_laporan'>PENDONOR</td>
                    <td width='100' class='header_laporan'>NIM</td>
                    <td width='200' class='header_laporan'>NAMA </td>
                    <td width='100' class='header_laporan'>PENERIMA</td>
                    </tr>
            ";
    //         $sql="select a.no_tes as nim,a.kode_lokasi,a.nama,
    ...
    //                  from trans_m a where a.form in ('CDIN','CDMUTMABA','CDINMABA')
    ...
    //  where a.kode_lokasi='$row->kode_lokasi' --and (isnull(d.nilai,0) <> 0 or isnull(e.nilai,0) <> 0)
    //  order by a.no_tes
            // ";
            $sql = "select a.no_tes as nim,a.kode_lokasi,a.nama,g.nilai as nilai_donor,g.ref1
            ,d.nama as nama_penerima,isnull(e.nilai,0) as nilai_terima
          from aka_maba a 
          inner join (select a.nim,a.kode_lokasi,a.ref1,sum(a.nilai) as nilai
                      from aka_cd_d a
                      inner join ( 
                          select a.no_bukti,a.kode_lokasi 
                          from trans_m a where a.form in ('CDIN','CDMUTMABA','CDINMABA')
                          ) b on a.no_bukti=b.no_bukti and a.kode_lokasi=b.kode_lokasi
                      where a.kode_lokasi='$row->kode_lokasi' and a.no_bukti='$row->no_bukti' and a.dc='C'
                      group by a.nim,a.kode_lokasi,a.ref1
                      )g on a.no_tes=g.nim and a.kode_lokasi=g.kode_lokasi 
          left join (select a.nim,a.kode_lokasi,sum(a.nilai) as nilai
                  from aka_cd_d a			
                  inner join aka_maba b on a.nim=b.no_tes and a.kode_lokasi=b.kode_lokasi 
                  ...
                  where a.kode_lokasi='$row->kode_lokasi' and a.dc='D' and a.no_bukti='$row->no_bukti'
                  group by a.nim,a.kode_lokasi
                  )e on g.ref1=e.nim and a.kode_lokasi=e.kode_lokasi 
        left join aka_maba d on e.nim=d.no_tes and e.kode_lokasi=d.kode_lokasi
          where a.kode_lokasi='$row->kode_lokasi' --isnull(e.nilai,0) <> 0)
          order by a.no_tes
        ";
            // echo $sql;
            ...
            while ($row1 = $rs1->FetchNextObject($toupper=false))
            {
                ...
                <td class='isi_laporan' align='center'>$i</td>
                    ...
                    <td class='isi_laporan' align='right'>".number_format($row1->nilai_donor,0,",",".")."</td>
                    <td class='isi_laporan'>$row1->ref1</td>
                    <td class='isi_laporan'>$row1->nama_penerima</td>
                    <td class='isi_laporan' align='right'>".number_format($row1->nilai_terima,0,",",".")."</td>
                </tr>";	 
                $i=$i+1;
            }
    // 		echo "<tr>
