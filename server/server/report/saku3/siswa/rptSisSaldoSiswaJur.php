...
class server_report_saku3_siswa_rptSisSaldoSiswaJur extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$sql="select a.kode_lokasi,a.kode_pp,a.kode_jur,a.nama
...
from sis_jur a 
left join (select z.kode_jur,y.kode_lokasi,y.kode_pp, 
			...
			inner join sis_kelas z on y.kode_kelas=z.kode_kelas and y.kode_lokasi=z.kode_lokasi and y.kode_pp=z.kode_pp
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode < '$periode') and x.kode_pp='$kode_pp'			
			group by z.kode_jur,y.kode_lokasi,y.kode_pp 			
			)b on a.kode_jur=b.kode_jur and a.kode_lokasi=b.kode_lokasi and a.kode_pp=b.kode_pp
left join (select z.kode_jur,y.kode_lokasi,y.kode_pp, 
				sum(case when x.kode_param in ('DSP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n1, 
				   sum(case when x.kode_param in ('SPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end)  as n2, 
				   sum(case when x.kode_param not in ('DSP','SPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end)  as n3,
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode = '$periode') and x.kode_pp='$kode_pp'			
			group by z.kode_jur,y.kode_lokasi,y.kode_pp 			
			)c on a.kode_jur=c.kode_jur and a.kode_lokasi=c.kode_lokasi and a.kode_pp=c.kode_pp
left join (select z.kode_jur,y.kode_lokasi,y.kode_pp,  
				sum(case when x.kode_param in ('DSP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n1, 
				   sum(case when x.kode_param in ('SPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end)  as n2, 
				   sum(case when x.kode_param not in ('DSP','SPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end)  as n3,
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode <'$periode')	and x.kode_pp='$kode_pp'		
			group by z.kode_jur,y.kode_lokasi,y.kode_pp		
			)d on a.kode_jur=d.kode_jur and a.kode_lokasi=d.kode_lokasi and a.kode_pp=d.kode_pp
left join (select z.kode_jur,y.kode_lokasi,y.kode_pp, 
			  sum(case when x.kode_param in ('DSP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n1, 
				   sum(case when x.kode_param in ('SPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end)  as n2, 
				   sum(case when x.kode_param not in ('DSP','SPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end)  as n3,
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode ='$periode') and x.kode_pp='$kode_pp'			
			group by z.kode_jur,y.kode_lokasi,y.kode_pp		
			)e on a.kode_jur=e.kode_jur and a.kode_lokasi=e.kode_lokasi and a.kode_pp=e.kode_pp
$this->filter
order by a.kode_jur
 ";
		...
		echo $AddOnLib->judul_laporan("saldo piutang per jurusan",$this->lokasi."<br>".$nama_pp,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr align='center' bgcolor='#CCCCCC'>
	...
	<td width='80' rowspan='2' class='header_laporan'>Kode</td>
  ...
  <tr bgcolor='#CCCCCC'>
   ...
   <td class='isi_laporan' align='center'>$i</td>
    ...
    <td class='isi_laporan'>";
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenKartu('$row->kode_jur','$row->kode_lokasi','$kode_pp');\">$row->kode_jur</a>";
			echo "</td>
