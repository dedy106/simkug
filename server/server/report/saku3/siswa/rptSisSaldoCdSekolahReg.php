...
class server_report_saku3_siswa_rptSisSaldoCdSekolahReg extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$sql="select a.kode_lokasi,a.nama,a.kode_pp,
...
order by a.kode_pp
 ";
		...
		echo $AddOnLib->judul_laporan("saldo pdd calon siswa per sekolah",$this->lokasi."<br>".$nama_pp,"PERIODE ".$AddOnLib->ubah_periode($periode));
 ...
 <td class='isi_laporan'>";
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenKartu('$row->kode_lokasi','$row->kode_pp');\">$row->kode_pp</a>";
			echo "</td>
