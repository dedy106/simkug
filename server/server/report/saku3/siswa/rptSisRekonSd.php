...
class server_report_saku3_siswa_rptSisRekonSd extends server_report_basic
{	
...
inner join (select c.nis,a.no_rekon,c.kode_lokasi,
			      sum(case when a.kode_param in ('USSD') then a.nilai else 0 end) as n1, 
				   sum(case when a.kode_param in ('SEMSD') then a.nilai else 0 end) as n2, 
				   sum(case when a.kode_param in ('PRASD') then a.nilai else 0 end) as n3,
				   sum(case when a.kode_param in ('KOMSD') then a.nilai else 0 end) as n4,
				   sum(case when a.kode_param in ('EKSSD') then a.nilai else 0 end) as n5, 
				   sum(case when a.kode_param in ('UKGSD') then a.nilai else 0 end) as n6, 
				   sum(case when a.kode_param in ('UASD') then a.nilai else 0 end) as n7,
				   sum(case when a.kode_param in ('BINGSD') then a.nilai else 0 end) as n8,
				   sum(case when a.kode_param in ('INTSD') then a.nilai else 0 end) as n9,
				   sum(case when a.kode_param in ('KEAGSD') then a.nilai else 0 end) as n10,
				   sum(case when a.kode_param in ('OUTSD') then a.nilai else 0 end) as n11,
				   sum(case when a.kode_param in ('DDSD') then a.nilai else 0 end) as n12,
...
order by b.nis
 ...
 ";
  ...
  <tr bgcolor='#CCCCCC'>
    ...
    <td width='80' align='center' class='header_laporan'>Semester</td>
    <td width='80' align='center' class='header_laporan'>Prakarya</td>
	<td width='80' align='center' class='header_laporan'>Komputer</td>
	<td width='90' align='center' class='header_laporan'>EKSKUL</td>
	...
	<td width='80' align='center' class='header_laporan'>Keagamaan</td>
	<td width='90' align='center' class='header_laporan'>KBM LS</td>
