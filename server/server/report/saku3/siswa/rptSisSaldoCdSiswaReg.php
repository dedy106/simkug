...
class server_report_saku3_siswa_rptSisSaldoCdSiswaReg extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$sql="select ''''+a.no_reg as no_reg,a.kode_lokasi,a.nama,a.kode_pp,a.kode_ta,
...
left join (select a.nis,a.kode_lokasi,a.kode_pp,sum(case when a.dc='D' then nilai else -nilai end) as nilai
		...
		)c on a.no_reg=c.nis and a.kode_lokasi=c.kode_lokasi and a.kode_pp=c.kode_pp
left join (select a.nis,a.kode_lokasi,a.kode_pp,sum(a.nilai) as nilai
		from sis_cd_d a			
		inner join sis_siswareg b on a.nis=b.no_reg and a.kode_lokasi=b.kode_lokasi and a.kode_pp=b.kode_pp
		where a.kode_lokasi='$kode_lokasi' and a.dc='D' $strperiode and a.kode_pp='$kode_pp'
		group by a.nis,a.kode_lokasi,a.kode_pp
		)d on a.no_reg=d.nis and a.kode_lokasi=d.kode_lokasi and a.kode_pp=d.kode_pp
left join (select a.nis,a.kode_lokasi,a.kode_pp,sum(a.nilai) as nilai
		from sis_cd_d a			
		inner join sis_siswareg b on a.nis=b.no_reg and a.kode_lokasi=b.kode_lokasi and a.kode_pp=b.kode_pp
		where a.kode_lokasi='$kode_lokasi' $strperiode and a.dc='C' and a.kode_pp='$kode_pp'
		group by a.nis,a.kode_lokasi,a.kode_pp
		)e on a.no_reg=e.nis and a.kode_lokasi=e.kode_lokasi and a.kode_pp=e.kode_pp
$this->filter 
order by a.no_reg
 ";
  ...
  <tr align='center' bgcolor='#CCCCCC'>
	...
	<td width='60'  class='header_laporan'>Tahun Ajaran</td>
 ...
 <td class='isi_laporan'>";
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenKartu('$row->no_reg','$row->kode_lokasi','$kode_pp');\">$row->no_reg</a>";
			echo "</td>
