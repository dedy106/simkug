...
class server_report_saku3_siswa_rptSisBill extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$tmp=explode("/",$this->filter2);
		$periode=$tmp[0];
		$sql="select a.no_bill,a.keterangan,b.nis,c.kode_tingkat,c.nama,
	    isnull(b.n1,0) as n1,isnull(b.n2,0) as n2, isnull(b.n3,0) as n3, isnull(b.n4,0) as n4, isnull(b.n5,0) as n5, 
		isnull(b.n6,0) as n6, isnull(b.n7,0) as n7, isnull(b.n8,0) as n8, isnull(b.n9,0) as n9, isnull(b.n10,0) as n10,isnull(b.total,0) as total 
from sis_bill_m a
inner join (select c.nis,a.no_bill,c.kode_lokasi,
			       sum(case when a.kode_param in ('USTK') then a.nilai else 0 end) as n1, 
				   sum(case when a.kode_param in ('PRATK') then a.nilai else 0 end) as n2, 
				   sum(case when a.kode_param in ('UKGSTK') then a.nilai else 0 end) as n3,
				   sum(case when a.kode_param in ('EVTK') then a.nilai else 0 end) as n4,
				   sum(case when a.kode_param in ('KOMTK') then a.nilai else 0 end) as n5, 
				   sum(case when a.kode_param in ('BINGTK') then a.nilai else 0 end) as n6, 
				   sum(case when a.kode_param in ('ESTK') then a.nilai else 0 end) as n7,
				   sum(case when a.kode_param in ('OUTTK') then a.nilai else 0 end) as n8,
				   sum(case when a.kode_param in ('BPTK') then a.nilai else 0 end) as n9,
				   sum(case when a.kode_param in ('DDTK') then a.nilai else 0 end) as n10,
				   sum(a.nilai) as total
			from sis_bill_d a 
			inner join sis_siswa c on a.nis=c.nis and a.kode_lokasi=c.kode_lokasi 
			$this->filter
			group by c.nis,a.no_bill,c.kode_lokasi 
...
order by c.nis
 ...
 ";
		...
		echo $AddOnLib->judul_laporan("tagihan siswa tk",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
    ...
    <td width='80' align='center' class='header_laporan'>Pra karya</td>
    <td width='80' align='center' class='header_laporan'>UKGS</td>
	<td width='80' align='center' class='header_laporan'>Evaluasi</td>
	<td width='90' align='center' class='header_laporan'>Komputer</td>
    <td width='80' align='center' class='header_laporan'>Bahasa Inggris</td>
    <td width='80' align='center' class='header_laporan'>Ekskul</td>
    <td width='80' align='center' class='header_laporan'>Outhing</td>
	<td width='80' align='center' class='header_laporan'>Buku Pelajaran</td>
	<td width='80' align='center' class='header_laporan'>Denda</td>
	<td width='90' align='center' class='header_laporan'>Total</td>
  </tr>";
		$n1=0;$n2=0;$n3=0;$n4=0;$n5=0;$n6=0;$n7=0;$n8=0;$n9=0;$total=0;$n10=0;
