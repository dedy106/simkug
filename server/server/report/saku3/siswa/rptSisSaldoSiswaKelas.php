...
class server_report_saku3_siswa_rptSisSaldoSiswaKelas extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$sql="select a.kode_lokasi,a.kode_pp,a.kode_kelas,a.nama
...
from sis_kelas a 
left join (select y.kode_kelas,y.kode_lokasi,y.kode_pp, 
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode < '$periode') and x.kode_pp='$kode_pp' $filter_status			
			group by y.kode_kelas,y.kode_lokasi,y.kode_pp 			
			)b on a.kode_kelas=b.kode_kelas and a.kode_lokasi=b.kode_lokasi and a.kode_pp=b.kode_pp
left join (select y.kode_kelas,y.kode_lokasi,y.kode_pp, 
				sum(case when x.kode_param in ('DSP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n1, 
				   sum(case when x.kode_param in ('SPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end)  as n2, 
				   sum(case when x.kode_param not in ('DSP','SPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end)  as n3,
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode = '$periode') and x.kode_pp='$kode_pp' $filter_status		
			group by y.kode_kelas,y.kode_lokasi,y.kode_pp 			
			)c on a.kode_kelas=c.kode_kelas and a.kode_lokasi=c.kode_lokasi and a.kode_pp=c.kode_pp
left join (select y.kode_kelas,y.kode_lokasi,y.kode_pp,  
				sum(case when x.kode_param in ('DSP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n1, 
				   sum(case when x.kode_param in ('SPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end)  as n2, 
				   sum(case when x.kode_param not in ('DSP','SPP') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end)  as n3,
...
left join (select y.kode_kelas,y.kode_lokasi,y.kode_pp, 
			...
			where(x.kode_lokasi = '$kode_lokasi')and(x.periode ='$periode') and x.kode_pp='$kode_pp' $filter_status			
			group by y.kode_kelas,y.kode_lokasi,y.kode_pp		
			)e on a.kode_kelas=e.kode_kelas and a.kode_lokasi=e.kode_lokasi and a.kode_pp=e.kode_pp
$this->filter
order by a.kode_kelas
 ";
		...
		echo $AddOnLib->judul_laporan("saldo piutang per kelas",$this->lokasi."<br>".$nama_pp,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr align='center' bgcolor='#CCCCCC'>
    ...
    <td width='80' rowspan='2' class='header_laporan'>Kelas</td>
  ...
  <tr bgcolor='#CCCCCC'>
   ...
   <td class='isi_laporan' align='center'>$i</td>
    ...
    <td class='isi_laporan'>";
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenKartu('$row->kode_kelas','$row->kode_lokasi','$kode_pp');\">$row->kode_kelas</a>";
			echo "</td>
