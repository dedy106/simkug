...
class server_report_saku3_siswa_rptSisSaldoSiswaPp4 extends server_report_basic
{	
	...
	function getHtml()
	{
        ...
        $bulan = substr($periode_rekon,5,2);
        ...
        if($range == "="){
            $filter_1 = " where(x.kode_lokasi = '$kode_lokasi')and(x.periode < '$periode') "; //filter saldo awal
            $filter_2 = " where(x.kode_lokasi = '$kode_lokasi')and(x.periode = '$periode')  "; //filter tagihan berjalan
            $filter_3 = " where(x.kode_lokasi = '$kode_lokasi')and (x.periode_bill between '".$tahun."01' and '".$tahun.$bulanSeb."') and (x.periode = '".$periode_rekon."') "; //filter bayar bulan sebelum
            $filter_4 = " where(x.kode_lokasi = '$kode_lokasi')and (x.periode_bill <= '".$tahunSeb."12') and (x.periode = '".$periode_rekon."') "; //filter tahun sebelum
            $filter_5 = " where(x.kode_lokasi = '$kode_lokasi')and(x.periode_bill ='$periode') and(x.periode ='$periode_rekon')  "; // filter bayar berjalan
            $filter_6 = "where(x.kode_lokasi = '$kode_lokasi')and(x.periode < '$periode')";
        }else if($range == "<="){
            ...
            $filter_4 = " where(x.kode_lokasi = '$kode_lokasi')and (x.periode_bill <= '".$tahunSeb."12') and (x.periode = '".$periode_rekon."') ";
            $filter_5 = " where(x.kode_lokasi = '$kode_lokasi')and (x.periode_bill between '".$tahun."01' and '$periode' ) and(x.periode ='$periode_rekon' )  ";
            $filter_6 = "where(x.kode_lokasi = '$kode_lokasi')and(x.periode < '".$tahunSeb."12')";
        }

        if($mutasi == "Tidak"){
            $filter_mutasi = " and ( (isnull(b.total,0)-isnull(d.total,0) <> 0 ) or (isnull(c.total,0) <> 0) or (isnull(e.total,0) <> 0) or (isnull(sb.total,0) <> 0 ) or (isnull(tb.total,0) <> 0) or (isnull(b.total,0)-isnull(d.total,0)-isnull(sb.total,0)-isnull(tb.total,0)+isnull(c.total,0)-isnull(e.total,0) <> 0 ) ) ";
        }else{
            $filter_mutasi = "";
        }
		
		$sql="select a.kode_lokasi,a.kode_pp,a.nis,a.nama ,
        ...
        from sis_siswa a 
        left join (select y.kode_lokasi,y.kode_pp,x.nis, 
                    ...
                    $filter_1
                    group by y.kode_lokasi,y.kode_pp,x.nis )b on a.kode_lokasi=b.kode_lokasi and a.kode_pp=b.kode_pp and a.nis=b.nis 
        left join (select y.kode_lokasi,y.kode_pp,x.nis, 
                    sum(case when x.kode_param in ('DSP','DSP_TK') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n1, 
                    sum(case when x.kode_param in ('SPP','SPP_TK') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n2, 
                    sum(case when x.kode_param not in ('DSP','SPP','DSP_TK','SPP_TK') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n3, 
                    ...
                    inner join sis_siswa y on x.nis=y.nis and x.kode_lokasi=y.kode_lokasi and x.kode_pp=y.kode_pp 
                    $filter_2
                    group by y.kode_lokasi,y.kode_pp,x.nis )c on a.kode_lokasi=c.kode_lokasi and a.kode_pp=c.kode_pp and a.nis=c.nis
        left join (select y.kode_lokasi,y.kode_pp, x.nis,
                    sum(case when x.kode_param in ('DSP','DSP_TK') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n1, 
                    sum(case when x.kode_param in ('SPP','SPP_TK') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n2, 
                    sum(case when x.kode_param not in ('DSP','SPP','DSP_TK','SPP_TK') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n3, 
                    ...
                    inner join sis_siswa y on x.nis=y.nis and x.kode_lokasi=y.kode_lokasi and x.kode_pp=y.kode_pp 
                    $filter_3 
                    group by y.kode_lokasi,y.kode_pp,x.nis )sb on a.kode_lokasi=sb.kode_lokasi and a.kode_pp=sb.kode_pp and a.nis=sb.nis
        left join (select y.kode_lokasi,y.kode_pp, x.nis,
                    sum(case when x.kode_param in ('DSP','DSP_TK') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n1, 
                    sum(case when x.kode_param in ('SPP','SPP_TK') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n2, 
                    sum(case when x.kode_param not in ('DSP','SPP','DSP_TK','SPP_TK') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n3, 
                    ...
                    inner join sis_siswa y on x.nis=y.nis and x.kode_lokasi=y.kode_lokasi and x.kode_pp=y.kode_pp 
                    $filter_4
                    group by y.kode_lokasi,y.kode_pp, x.nis )tb on a.kode_lokasi=tb.kode_lokasi and a.kode_pp=tb.kode_pp and a.nis=tb.nis
        left join (select y.kode_lokasi,y.kode_pp, x.nis,
                    sum(case when x.kode_param in ('DSP','DSP_TK') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n1, 
                    sum(case when x.kode_param in ('SPP','SPP_TK') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n2, 
                    sum(case when x.kode_param not in ('DSP','SPP','DSP_TK','SPP_TK') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n3, 
                    ...
                    inner join sis_siswa y on x.nis=y.nis and x.kode_lokasi=y.kode_lokasi and x.kode_pp=y.kode_pp 
                    $filter_5
                    group by y.kode_lokasi,y.kode_pp,x.nis )e on a.kode_lokasi=e.kode_lokasi and a.kode_pp=e.kode_pp and a.nis=e.nis
        left join (select y.kode_lokasi,y.kode_pp, x.nis,
                    sum(case when x.kode_param in ('DSP','DSP_TK') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n1, 
                    sum(case when x.kode_param in ('SPP','SPP_TK') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n2, 
                    sum(case when x.kode_param not in ('DSP','SPP','DSP_TK','SPP_TK') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end) as n3, 
                    ...
                    inner join sis_siswa y on x.nis=y.nis and x.kode_lokasi=y.kode_lokasi and x.kode_pp=y.kode_pp 
                    $filter_6
                    group by y.kode_lokasi,y.kode_pp, x.nis )d on a.kode_lokasi=d.kode_lokasi and a.kode_pp=d.kode_pp and a.nis=d.nis
        left join (select a.kode_pp,a.kode_lokasi,a.nis,max(a.periode) as periode
                    from sis_bill_d a
                    where a.kode_lokasi='$kode_lokasi' and a.kode_param in ('DSP','DSP_TK')
                    group by a.kode_pp,a.kode_lokasi,a.nis) p on a.kode_lokasi=p.kode_lokasi and a.kode_pp=p.kode_pp and a.nis=p.nis
        left join (select a.kode_pp, a.kode_lokasi,a.nis,max(a.periode) as periode
                    from sis_bill_d a
                    where a.kode_lokasi='$kode_lokasi' and a.kode_param in ('SPP','SPP_TK')
                    group by a.kode_pp,a.kode_lokasi,a.nis) q on a.kode_lokasi=q.kode_lokasi and a.kode_pp=q.kode_pp and a.nis=q.nis
        left join (select a.kode_pp,a.kode_lokasi, a.nis,max(a.periode) as periode
                    from sis_bill_d a
                    where a.kode_lokasi='$kode_lokasi' and a.kode_param not in ('DSP','SPP','DSP_TK','SPP_TK')
                    group by a.kode_pp,a.kode_lokasi,a.nis) r on a.kode_lokasi=r.kode_lokasi and a.kode_pp=r.kode_pp and a.nis=r.nis
$this->filter $filter_mutasi
order by a.nis,a.kode_pp ";
...
//  echo $filter_mutasi;
		...
		echo $AddOnLib->judul_laporan("LAPORAN CCR ",$this->lokasi,"BULAN ".$AddOnLib->ubah_periode($periode));
  ...
  <tr align='center' bgcolor='#CCCCCC'>
	...
	<td width='100' rowspan='3' class='header_laporan'>NIS</td>
	<td width='200' rowspan='3' class='header_laporan'>Nama</td>
