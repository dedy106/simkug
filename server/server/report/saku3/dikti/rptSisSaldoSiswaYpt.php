...
class server_report_saku3_dikti_rptSisSaldoSiswaYpt extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		,isnull(b.n1,0)-isnull(d.n1,0) as saw_n1,isnull(b.n2,0)-isnull(d.n2,0) as saw_n2,isnull(b.n3,0)-isnull(d.n3,0) as saw_n3,isnull(b.n4,0)-isnull(d.n4,0) as saw_n4,
		isnull(b.total,0)-isnull(d.total,0) as saw_total
	   ,isnull(c.n1,0) as debet_n1,isnull(c.n2,0) as debet_n2,isnull(c.n3,0) as debet_n3,isnull(c.n4,0) as debet_n4,isnull(c.total,0) as debet_total
	   ,isnull(e.n1,0) as kredit_n1,isnull(e.n2,0) as kredit_n2,isnull(e.n3,0) as kredit_n3,isnull(e.n4,0) as kredit_n4,isnull(e.total,0) as kredit_total
	   ...
	   ,isnull(b.n4,0)-isnull(d.n4,0)+isnull(c.n4,0)-isnull(e.n4,0) as sak_n4
	   ,isnull(b.total,0)-isnull(d.total,0)+isnull(c.total,0)-isnull(e.total,0) as sak_total,
	   a.kode_kelas,a.kode_jur,f.nama as nama_jur,a.kode_status,h.nama as nama_status
from dikti_mhs a 
...
left join (select y.nim,y.kode_lokasi,  
				   ...
				   sum(case when x.kode_param not in ('UP3','BPP','PUL') then (case when x.dc='D' then x.nilai else -x.nilai end) else 0 end)  as n3,
...
order by a.kode_kelas,a.nim
 ";
	//echo $sql;
		...
		echo $AddOnLib->judul_laporan("saldo piutang mahasiswa",$this->lokasi."<br>".$nama_pp,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
    ...
    <td width='80' align='center' class='header_laporan'>PUL</td>
    ...
    <td width='90' align='center' class='header_laporan'>Total</td>
		...
		$saw_n1=0;$saw_n2=0;$saw_n3=0;$saw_n4=0;$saw_total=0;
		$sak_n1=0;$sak_n2=0;$sak_n3=0;$sak_n4=0;$sak_total=0;
		$debet_n1=0;$debet_n2=0;$debet_n3=0;$debet_n4=0;$debet_total=0;
		$kredit_n1=0;$kredit_n2=0;$kredit_n3=0;$kredit_n4=0;$kredit_total=0;
		$sak_bpp=0;$sak_sdp2=0;$sak_up3=0;$sak_pul=0;$sak_total=0;$sak_sks=0;
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			$saw_n1+=$row->saw_n1;$saw_n2+=$row->saw_n2; $saw_n3+=$row->saw_n3; $saw_n4+=$row->saw_n4; $saw_total+=$row->saw_total;
			$debet_n1+=$row->debet_n1; $debet_n2+=$row->debet_n2; $debet_n3+=$row->debet_n3;  $debet_n4+=$row->debet_n4; $debet_total+=$row->debet_total;
			$kredit_n1+=$row->kredit_n1; $kredit_n2+=$row->kredit_n2; $kredit_n3+=$row->kredit_n3; $kredit_n4+=$row->kredit_n4; $kredit_total+=$row->kredit_total;
			$sak_n1+=$row->sak_n1; $sak_n2+=$row->sak_n2; $sak_n3+=$row->sak_n3;$sak_n4+=$row->sak_n4; $sak_total+=$row->sak_total;
			echo "<tr>
   <td class='isi_laporan' align='center'>$i</td>
    
    <td class='isi_laporan'>";
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenKartuPiu('$row->nim','$row->kode_lokasi');\">$row->nim</a>";
