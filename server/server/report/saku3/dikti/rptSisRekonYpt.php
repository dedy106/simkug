...
class server_report_saku3_dikti_rptSisRekonYpt extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$periode=$tmp[1];
		$nama_file="rekon.xls";
		// $sql="select a.no_kas,a.keterangan,b.nim,c.nama,c.kode_jur,c.kode_kelas,a.periode,
		// substring(a.periode,5,2) as bulan,c.kode_akt, isnull(b.n1,0) as n1,isnull(b.n2,0) as n2,
		//  isnull(b.n3,0) as n3, isnull(b.total,0)+isnull(d.n4,0) as total,isnull(d.n4,0) as n4 
		// from kas_m a
		...
		// 		group by c.nim,a.no_rekon,c.kode_lokasi )b on a.no_kas=b.no_rekon and a.kode_lokasi=b.kode_lokasi
		...
		// left join (select a.nim,a.kode_lokasi,a.no_bukti,sum(a.nilai) as n4 
		// 			from dikti_cd a
		// 			group by a.nim,a.kode_lokasi,a.no_bukti ) d on b.nim=d.nim and a.no_kas=d.no_bukti and a.kode_lokasi=d.kode_lokasi

		// $this->filter
		// order by a.tanggal,c.nim ";

		$sql= "
		select a.no_kas,a.keterangan,a.periode,b.nim,c.nama,c.kode_jur,c.kode_kelas,a.periode,
				substring(a.periode,5,2) as bulan,c.kode_akt, isnull(d.n1,0) as n1,isnull(d.n2,0) as n2,
				 isnull(d.n3,0) as n3, isnull(d.total,0)+isnull(f.n4,0) as total,isnull(f.n4,0) as n4 
		from kas_m a
		inner join ( select distinct a.* from 
						(select a.no_rekon,a.kode_lokasi,a.nim
						 from dikti_bill_rekon a
						 group by a.no_rekon,a.kode_lokasi,a.nim
						 union all
						 select a.no_bukti,a.kode_lokasi,a.nim
						 from dikti_cd a
						 group by a.no_bukti,a.kode_lokasi,a.nim
						 ) a
					) b on a.no_kas=b.no_rekon and a.kode_lokasi=b.kode_lokasi
		inner join dikti_mhs c on b.nim=c.nim and a.kode_lokasi=c.kode_lokasi
		inner join dikti_kelas e on c.kode_kelas=e.kode_kelas and c.kode_lokasi=e.kode_lokasi 
		left join (select c.nim,a.no_rekon,c.kode_lokasi,
						sum(case when a.kode_param in ('BPP') then a.nilai else 0 end) as n1, 
						sum(case when a.kode_param in ('UP3') then a.nilai else 0 end) as n2,
						sum(case when a.kode_param in ('SDP2') then a.nilai else 0 end) as n3,
						sum(a.nilai) as total 
					...
					) d on b.nim=d.nim and a.kode_lokasi=d.kode_lokasi and a.no_kas=d.no_rekon
		left join (select a.nim,a.kode_lokasi,a.no_bukti,sum(a.nilai) as n4 
				   from dikti_cd a
				   group by a.nim,a.kode_lokasi,a.no_bukti ) f on b.nim=f.nim and a.no_kas=f.no_bukti and a.kode_lokasi=f.kode_lokasi
		$this->filter ";
