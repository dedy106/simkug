...
class server_report_saku3_spj_rptPdPosTu extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="select a.kode_lokasi,a.no_aju,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.keterangan,a.kode_pp,b.nama as nama_pp,a.nilai,a.kode_drk,j.nama as nama_drk,a.npajak,a.nilai+a.npajak as bruto,k.jenis,
	   ...
	   c.kode_bank,a.kode_akun,h.nama as nama_akun, a.no_app,date_format(k.tgl_input,'%d/%m/%Y') as tgl_app1,isnull(i.no_ju,'-') as no_ju,
	   a.no_app,date_format(k.tgl_input,'%d/%m/%Y') as tgl_app,
	   a.no_spb,date_format(l.tanggal,'%d/%m/%Y') as tgl_spb,
	   m.no_app as no_appspj,convert(varchar,m.tanggal,103) as tgl_appspj
	   ,
	   isnull(n.jum_dok,0) as jum_dok,isnull(c.posted,'-') as posted,
	   isnull(o.no_ju,'-') as no_bmhd, date_format(o.tanggal,'%d/%m/%Y') as tgl_bmhd
from it_aju_m a
...
left join tu_pdapp_m m on a.no_aju=m.no_aju and a.kode_lokasi=m.kode_lokasi 
left join (select b.no_bukti, b.kode_lokasi,count(b.no_gambar) as jum_dok 
from it_aju_m a
inner join it_aju_dok b on a.no_aju=b.no_bukti and a.kode_lokasi=b.kode_lokasi
$this->filter
group by b.no_bukti,b.kode_lokasi) n on a.kode_lokasi=n.kode_lokasi and a.no_aju=n.no_bukti
left join ju_m o on a.no_spb=o.no_dokumen and a.kode_lokasi=o.kode_lokasi
$this->filter order by a.no_aju";
		...
		echo $AddOnLib->judul_laporan("posisi perjalanan dinas",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
   ...
   <td width='200'  align='center' class='header_laporan'>Keterangan</td>
	 ...
	 <td width='90'  align='center' class='header_laporan'>No BMHD</td>
	 <td width='60'  align='center' class='header_laporan'>Tgl BMHD</td>
     ...
     <td class='isi_laporan' align='center'>$i</td>
      <td class='isi_laporan'>";
		...
		echo "</td>
	  <td class='isi_laporan'>";
		echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenSpj('$row->no_appspj','$row->kode_lokasi');\">$row->no_appspj</a>";
		echo "</td>
	 ...
	 <td class='isi_laporan'>$row->tgl_fiat</td>
	  ...
	  <td class='isi_laporan'>";
	   ...
	   <td class='isi_laporan'>";
		echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenKas('$row->no_bmhd','$row->kode_lokasi');\">$row->no_bmhd</a>";
		echo "</td>
	 <td class='isi_laporan'>$row->tgl_bmhd</td>
	  <td class='isi_laporan'>";
	 ...
	 <td class='isi_laporan'>$row->tgl_kas</td>
	  ...
	  <td class='header_laporan' align='center' colspan='14'>&nbsp;</td>
    </tr>";
		echo "</table><br>";
		echo "</div>";
		return "";
