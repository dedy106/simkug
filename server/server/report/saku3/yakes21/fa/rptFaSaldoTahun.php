...
class server_report_saku3_yakes21_fa_rptFaSaldoTahun extends server_report_basic
{
	...
	function getHtml()
	{
	   ...
	   isnull(e.ap,0) as ap ,
	   isnull(b.so_awal,0)-isnull(e.ap,0) - isnull(dd.wo,0) as nilai_buku,a.jenis
from fa_klpakun a
left join fa_klp j on a.kode_klpakun=j.kode_klpakun 
inner join (select a.kode_klpakun,substring(a.periode,1,4) as tahun
			from fa_asset a			
			$this->filter and a.progress not in ('K','P') and a.jenis<>'I'
			group by a.kode_klpakun,substring(a.periode,1,4)
			)g on a.kode_klpakun=g.kode_klpakun 

left join (select a.kode_klpakun,substring(a.periode,1,4) as tahun,sum(b.nilai) as so_awal
           from fa_asset a
		   inner join fa_nilai b on a.no_fa=b.no_fa and a.kode_lokasi=b.kode_lokasi
           $this->filter and a.progress not in ('K','P') and a.jenis<>'I'
           group by a.kode_klpakun,substring(a.periode,1,4)
          )b on g.kode_klpakun=b.kode_klpakun and g.tahun=b.tahun 
		  
left join (select a.kode_klpakun,substring(a.periode,1,4) as tahun,sum(case when b.dc='D' then b.nilai else -b.nilai end) as ap
           from fa_asset a
		   inner join fasusut_d b on a.no_fa=b.no_fa and a.kode_lokasi=b.kode_lokasi
		   left join fa_woaju_d bb on a.no_fa=bb.no_fa and a.kode_lokasi=bb.kode_lokasi	
		   inner join fa_woaju_m c on bb.no_aju=c.no_aju	   
		   $this->filter and c.periode<='$periode' and a.progress not in ('K','P') and a.jenis<>'I' and bb.no_fa is null 
           group by a.kode_klpakun,substring(a.periode,1,4)
          ) e on g.kode_klpakun=e.kode_klpakun and g.tahun=e.tahun 
		  
left join (select a.kode_klpakun,substring(a.periode,1,4) as tahun,sum(a.nilai) as wo
		   from fa_asset a
		   inner join fa_woaju_d b on a.no_fa=b.no_fa and a.kode_lokasi=b.kode_lokasi	
		   inner join fa_woaju_m c on b.no_aju=c.no_aju	   
		   $this->filter and c.periode<='$periode' and a.progress not in ('K','P') and a.jenis<>'I'
		   group by a.kode_klpakun,substring(a.periode,1,4)
		   )dd on g.kode_klpakun=dd.kode_klpakun and g.tahun=dd.tahun    
$filter3	   
