...
class server_report_saku3_yakes21_fa_rptFaSaldo extends server_report_basic
{
...
left join (select a.kode_klpakun,sum(b.nilai) as so_awal
           from fa_asset a inner join fa_nilai b on a.no_fa=b.no_fa and a.kode_lokasi=b.kode_lokasi
           where a.periode<'$periode' and a.progress not in ('K','P') and a.jenis<>'I' $filterpp $filter_lokasi
           group by a.kode_klpakun,a.kode_lokasi
          )b on a.kode_klpakun=b.kode_klpakun  

left join (select a.kode_klpakun,sum(b.nilai) as debet
           from fa_asset a inner join fa_nilai b on a.no_fa=b.no_fa and a.kode_lokasi=b.kode_lokasi
		   where a.periode='$periode' and a.progress not in ('K','P') and a.jenis<>'I' $filterpp $filter_lokasi
           group by a.kode_klpakun,a.kode_lokasi
          ) c on a.kode_klpakun=c.kode_klpakun 

left join (select a.kode_klpakun,a.kode_lokasi,sum(a.nilai) as kredit
		   from fa_asset a
		   inner join fa_woaju_d b on a.no_fa=b.no_fa and a.kode_lokasi=b.kode_lokasi
		   inner join fa_woaju_m c on b.no_aju=c.no_aju
		   where c.periode<'$periode' and a.progress not in ('K','P') and a.jenis<>'I' $filterpp $filter_lokasi
		   group by a.kode_klpakun,a.kode_lokasi
		   )dd on a.kode_klpakun=dd.kode_klpakun 
...
left join (select a.kode_klpakun,sum(case when b.dc='D' then b.nilai else -b.nilai end) as ap
           from fa_asset a
		   inner join fasusut_d b on a.no_fa=b.no_fa and a.kode_lokasi=b.kode_lokasi
           where b.periode<'$periode_susut' and a.progress not in ('K','P') and a.jenis<>'I' $filterpp $filter_lokasi
           group by a.kode_klpakun
          ) e on a.kode_klpakun=e.kode_klpakun 
...
left join (select a.kode_klpakun,sum(case when b.dc='D' then b.nilai else -b.nilai end) as ap
           ...
           where b.periode='$periode_susut' and a.progress not in ('K','P') and a.jenis<>'I' $filterpp $filter_lokasi
           group by a.kode_klpakun
          ) f on a.kode_klpakun=f.kode_klpakun 

left join (select a.kode_klpakun,sum(case when bb.dc='D' then bb.nilai else -bb.nilai end) as ap_wo
		   from fa_asset a
		   inner join fa_woaju_d b on a.no_fa=b.no_fa and a.kode_lokasi=b.kode_lokasi
		   ...
		   where c.periode<'$periode' and a.progress not in ('K','P') and a.jenis<>'I' $filterpp $filter_lokasi
		   group by a.kode_klpakun
		   )ee on a.kode_klpakun=ee.kode_klpakun 
...
left join (select a.kode_klpakun,sum(case when bb.dc='D' then bb.nilai else -bb.nilai end) as ap_wo
		   ...
		   where c.periode='$periode' and a.progress not in ('K','P') and a.jenis<>'I' $filterpp $filter_lokasi
		   group by a.kode_klpakun
		   )ff on a.kode_klpakun=ff.kode_klpakun 	
$this->filter  
