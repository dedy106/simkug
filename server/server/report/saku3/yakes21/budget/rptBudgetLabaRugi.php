...
class server_report_saku3_yakes21_budget_rptBudgetLabaRugi extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="exec sp_rkap_yk '$kode_fs','A','S','$lev','$periode','$kode_lokasi','$nik_user' ";
		//echo $sql;
		$rs = $dbLib->execute($sql);
		if ($bentuk=="Detail") {
			$sql = "exec sp_glma_rkap_yk_tmp '$kode_lokasi','$periode','$nik_user' ";
			//echo $bentuk.$sql;
			$rs = $dbLib->execute($sql);
		}
		$sql = "select kode_neraca,kode_fs,kode_lokasi,nama,tipe,level_spasi,
			...
			from neraca_tmp 
			where nik_user='$nik_user' and level_lap<=$lev
			order by rowindex ";
  ...
  <tr>
    <td  class='lokasi_laporan2' align='center'>LAPORAN RKAP</td>
  </tr>
  <tr>
  ...
  <tr bgcolor='#CCCCCC'>
    <td width='350' rowspan='2' align='center' class='header_laporan'>Deskripsi</td>
    <td width='90' rowspan='2' align='center' class='header_laporan'>Budget $tahun</td>
    <td colspan='3' align='center' class='header_laporan'>$bulan $tahun</td>
  ...
  <tr bgcolor='#CCCCCC'>
    ...
    <td width='60' align='center' class='header_laporan'>YoY Growth</td>
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			if ($row->n6!=0)
			{
				$persen3=($row->n8/$row->n6)*100;
			}
			if ($row->n9!=0)
			{
				$persen4=(($row->n8-$row->n9)/$row->n9)*100;
			}
			echo "<tr >
			...
			if ($bentuk=="Detail" && $row->tipe=="Posting")
			{
				...
				$sql1="select a.kode_akun,c.nama,
					...
					from glma_gar_tmp a
					inner join relakun b on a.kode_akun=b.kode_akun and b.kode_lokasi='00'
					inner join masakun c on a.kode_akun=c.kode_akun and a.kode_lokasi=c.kode_lokasi
					where b.kode_fs='$kode_fs' and b.kode_neraca='$kode_neraca' and a.periode='$periode' and a.nik_user='$nik_user'
				...
				{	
					...
					if ($row1->n6!=0)
					{
						$persen3=($row1->n8/$row1->n6)*100;
					}
					if ($row1->n9!=0)
					{
						$persen4=(($row1->n8-$row1->n9)/$row1->n9)*100;
					}
					echo "<tr>
