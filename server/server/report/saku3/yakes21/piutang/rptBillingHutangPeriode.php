...
class server_report_saku3_yakes21_piutang_rptBillingHutangPeriode extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$bpcc=$tmp[3];
		$jenis=$tmp[4];
		$nama_file="billing_".$periode.".xls";
		$loker = "";
		if ($bpcc == "BP") {
		...
		$sql2="exec sp_yk_bill_lap_periode2 '$periode','$kode_lokasi','$nik_user' ";
		
		
		if ($bpcc == "BPTLKM") {
			$sql2="exec sp_yk_bill_lap_periode_nonakru '$periode','$kode_lokasi','$nik_user' ";
		}
...
where a.kode_lokasi='$kode_lokasi' and a.nik_user='$nik_user' and a.no_hutang<>'-' $loker
