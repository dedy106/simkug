...
class server_report_saku3_yakes21_piutang_rptHutangKesSaldo extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$rs = $dbLib->execute($sql);	
		$sql="select a.no_hutang,a.kode_lokasi,date_format(b.tanggal,'%d/%m/%Y') as tgl,b.keterangan,c.nama as nama_vendor,
...
order by no_hutang ";
  ...
  <tr bgcolor='#CCCCCC'>
    ...
    <td class='isi_laporan' class='isi_laporan'>$row->no_hutang</td>
	 <td class='isi_laporan' class='isi_laporan'>$row->tgl</td>
	 <td class='isi_laporan' class='isi_laporan'>$row->nama_vendor</td>
	 <td class='isi_laporan' class='isi_laporan'>$row->keterangan</td>
