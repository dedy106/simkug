...
class server_report_saku3_yakes21_panjar_rptPjPtgForm extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$periode=$tmp[2];
		$filter2=$tmp[3];
		$sql="select a.no_ptg,h.nik_buat,h.nik_app as nik_tahu,a.nik_app as nik_setuju,f.flag as nik_bdh,i.flag as nik_manbdh,h.kode_pp,h.nilai as nilai_pj,a.nilai as nilai_ptg,a.nilai_kas,
...
left join karyawan d on h.nik_app=d.nik and h.kode_lokasi=d.kode_lokasi
...
//echo $sql1;
  ...
  <tr>
    <td><table border='0' cellspacing='2' cellpadding='1'>
      ...
      $sql1="select sum(a.nilai) as nilai
from panjarptg2_j a
where a.no_ptg='$row->no_ptg' and  a.modul='PJPTG' and a.jenis='PAJAK'  ";

		$rs1 = $dbLib->execute($sql1);
    $row1 = $rs1->FetchNextObject($toupper=false);
    $nilai_pph=$row1->nilai;
      echo "<tr>
        <td>Titipan PPH </td>
        <td>Rp</td>
        <td align='right'>".number_format($nilai_pph,0,",",".")."</td>
        <td>&nbsp;</td>
      </tr>
    </table></td>
  </tr>
