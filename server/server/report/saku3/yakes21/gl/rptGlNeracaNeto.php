...
class server_report_saku3_yakes21_gl_rptGlNeracaNeto extends server_report_basic
{
  ...
  <tr>
    <td  class='lokasi_laporan2' align='center'>LAPORAN ASET NETTO</td>
  </tr>
  <tr>
  ...
  <tr>
    <td><table border='1' cellspacing='0' cellpadding='0' class='kotak' width='800'>
		...
		$sql = "select kode_neraca,kode_fs,kode_lokasi,nama,tipe,level_spasi,n1,n3
			from neraca_tmp 
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			{
				echo "<td height='20' class='isi_laporan' align='right'><a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_neraca','$row->kode_lokasi','$periode_rev');\">$n3</a></td>";
			}
			else
			{
				echo "<td height='20' class='isi_laporan' align='right'>$n3</td>";
			}
			echo "</tr>";
			
			$i=$i+1;
		}
		$sql = "select kode_neraca,kode_fs,kode_lokasi,nama,tipe,level_spasi,-n1 as n1,-n3 as n3
			from neraca_tmp 
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			if ($row->tipe=="Posting" )
			{
				echo "<td height='20' class='isi_laporan' align='right'><a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_neraca','$row->kode_lokasi','$periode12');\">$n3</a></td>";
			}
			else
			{
				echo "<td height='20' class='isi_laporan' align='right'>$n3</td>";
