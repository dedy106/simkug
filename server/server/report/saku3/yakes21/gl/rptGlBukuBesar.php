...
class server_report_saku3_yakes21_gl_rptGlBukuBesar extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="exec sp_glma_yk_tmp '$kode_lokasi','$periode','$nik_user' ";
		//echo $sql;
		$rs = $dbLib->execute($sql);		
   ...
   <tr >
    <td height='23' colspan='10' style='padding:5px'><table  border='0' cellspacing='2' cellpadding='1'>
                    <tr>
  ...
  <tr bgcolor='#CCCCCC'>
  	<td width='60' height='23' class='header_laporan' align='center'>Lokasi</td>
    ...
    <td width='90' class='header_laporan' align='center'>Balance</td>
		...
		if ($kode_lokasi=='00') {
			$sql="select a.kode_lokasi,a.no_bukti,a.no_dokumen,a.periode,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.kode_akun,a.kode_pp,a.kode_drk,a.keterangan,
					case when a.dc='D' then nilai else 0 end as debet,
					case when a.dc='C' then nilai else 0 end as kredit 
				from gldt a
				inner join lokasi_konsol b on a.kode_lokasi=b.kode_lokasi
				where a.nilai <> 0 and b.kode_konsol='$kode_lokasi' and a.kode_akun='$row->kode_akun' $var_periode 
			...
			$sql="select a.kode_lokasi,a.no_bukti,a.no_dokumen,a.periode,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.kode_akun,a.kode_pp,a.kode_drk,a.keterangan,
					...
					case when a.dc='C' then nilai else 0 end as kredit 
				from gldt a
				where a.nilai <> 0 and a.kode_lokasi='$kode_lokasi' and a.kode_akun='$row->kode_akun' $var_periode 
				order by a.tanggal,a.dc desc  ";
		}
			//echo $sql;
			...
			while ($row1 = $rs1->FetchNextObject($toupper=false))
			{
				...
				echo "<tr><td valign='top' class='isi_laporan'>".$row1->kode_lokasi."</td>
