...
class server_report_saku3_pajak_rptHonor extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$periode=$tmp[0];
		$nama_cab=$tmp[1];
		$sql="select a.no_aju,a.kode_lokasi,a.no_kas,a.kode_pp,a.tanggal,c.nilai,c.pajak,c.nama_rek,
		c.keterangan,date_format(b.tanggal,'%d/%m/%Y') as tgl_kas,
		case when d.flag_dosen='1' then pajak else 0 end as tunj_pajak,
		case when d.flag_dosen='1' then c.nilai else c.nilai-c.pajak end as netto,
