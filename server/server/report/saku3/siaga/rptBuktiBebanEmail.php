...
class server_report_saku3_siaga_rptBuktiBebanEmail extends server_report_basic
{
	...
	function getHtml()
	{
		...
		if($no_pb != ""){
			$sql = "select a.no_pb,c.nik as nik_kirim, c.email as email_kirim from gr_pb_m a
			left join apv_flow b on a.no_pb=b.no_bukti and a.kode_lokasi=b.kode_lokasi and b.status=1
			left join karyawan c on b.nik=c.nik and a.kode_lokasi=c.kode_lokasi
			where a.no_pb='$no_pb'";
			$res2 = $dbLib->execute($sql);
			$row2 = $res2->FetchNextObject($toupper=false);
			if(isset($row2->nik_kirim)){
				$email_kirim = ($email == "" ? $row2->email_kirim : $email);
				if($email_kirim != ""){
					$content = 'no_aju='.$row2->no_pb.'&email_kirim='.$email_kirim.'&nik_kirim='.$row2->nik_kirim.'&jenis=PB&judul=Pengajuan PB Nomor: '.$row2->no_pb.' telah direturn oleh '.$nik.', menunggu approval anda';
