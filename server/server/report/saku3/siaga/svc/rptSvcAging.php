...
class server_report_saku3_siaga_svc_rptSvcAging extends server_report_basic
{
...
$this->filter";
	...
	function getHtml()
	{
		...
		$jenis=$tmp[1];
		$nama_cab=$tmp[2];
		$nama_file="aging_svc_$periode_$nama_cab.xls";
	   ...
	   dbo.fnGetBuktiKasSVC(b.no_invoice,$periode) as ket_kas
from gr_svc_m a
inner join gr_svc_d b on a.no_svc=b.no_svc and a.kode_lokasi=b.kode_lokasi
...
$this->filter order by b.no_invoice";
		...
		echo $AddOnLib->judul_laporan("aging piutang SVC - cabang $nama_cab",$this->lokasi,"Tanggal Aging ".$tgl_aging);
