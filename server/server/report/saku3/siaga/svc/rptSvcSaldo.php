...
class server_report_saku3_siaga_svc_rptSvcSaldo extends server_report_basic
{
...
from gr_svc_m a
inner join gr_svc_d b on a.no_svc=b.no_svc and a.kode_lokasi=b.kode_lokasi
$this->filter";
	...
	function getHtml()
	{
	   ...
	   b.total-isnull(c.nilai_kas,0) as saldo,dbo.fnGetBuktiKasSVC(b.no_invoice,$periode) as ket_kas
from gr_svc_m a
inner join gr_svc_d b on a.no_svc=b.no_svc and a.kode_lokasi=b.kode_lokasi
left join (select a.no_invoice,a.kode_lokasi,sum(a.nilai_lain+a.nilai_kas) as nilai_kas				  
           from gr_svcbayar_d a
...
$this->filter order by b.no_invoice ";
		...
		echo $AddOnLib->judul_laporan("tagihan svc - cabang $nama_cab",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
