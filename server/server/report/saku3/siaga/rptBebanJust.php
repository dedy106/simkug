...
class server_report_saku3_siaga_rptBebanJust extends server_report_basic
{
...
select f.ref1,c.no_bukti,c.kode_akun,b.nama as nama_akun,sum(case a.dc when 'D' then a.nilai else -a.nilai end) as nilai,isnull(d.kode_flag,'-') as kode_flag, 
  ...
  inner join masakun b on c.kode_akun=b.kode_akun and c.kode_lokasi=b.kode_lokasi
  left join flag_relasi d on c.kode_akun=d.kode_akun and c.kode_lokasi=d.kode_lokasi and d.kode_flag='006'
where a.no_pb='$row->no_pb' and a.kode_lokasi='$row->kode_lokasi' and c.modul in ('AJU','BMHD','BYRBMHD','KES','SPJ')
...
union all
select c.no_pb,a.kode_lokasi,b.nama as nama_brg,'-' as satuan,1 as jumlah, a.nilai as harga from gr_kes_d a
      inner join gr_kes_m c on a.no_kes=c.no_kes and a.kode_lokasi=c.kode_lokasi
      inner join gr_kes_jenis b on a.kode_jenis=b.kode_jenis and a.kode_lokasi=b.kode_lokasi
      where c.no_pb='$row->no_pb' and a.kode_lokasi='$row->kode_lokasi'  ";
	//echo $sql;
    $rs1 = $dbLib->execute($sql);
	$i=1; $nilai=0; 
	while ($row1 = $rs1->FetchNextObject($toupper=false))
