...
class server_report_saku3_siaga_hris_rptGajiRekapBulan extends server_report_basic
{
  ...
  left join (select x.kode_lokasi,x.nik
	  	            , sum(case when substring(x.periode,5,2) between '01' and '01' then case when z.dc='D' then nilai else -nilai end else 0 end ) as agg_01
                    , sum(case when substring(x.periode,5,2) between '02' and '02' then case when z.dc='D' then nilai else -nilai end else 0 end ) as agg_02
                    , sum(case when substring(x.periode,5,2) between '03' and '03' then case when z.dc='D' then nilai else -nilai end else 0 end ) as agg_03
                    , sum(case when substring(x.periode,5,2) between '04' and '04' then case when z.dc='D' then nilai else -nilai end else 0 end ) as agg_04
                    , sum(case when substring(x.periode,5,2) between '05' and '05' then case when z.dc='D' then nilai else -nilai end else 0 end ) as agg_05
                    , sum(case when substring(x.periode,5,2) between '06' and '06' then case when z.dc='D' then nilai else -nilai end else 0 end ) as agg_06
                    , sum(case when substring(x.periode,5,2) between '07' and '07' then case when z.dc='D' then nilai else -nilai end else 0 end ) as agg_07
                    , sum(case when substring(x.periode,5,2) between '08' and '08' then case when z.dc='D' then nilai else -nilai end else 0 end ) as agg_08
                    , sum(case when substring(x.periode,5,2) between '09' and '09' then case when z.dc='D' then nilai else -nilai end else 0 end ) as agg_09
                    , sum(case when substring(x.periode,5,2) between '10' and '10' then case when z.dc='D' then nilai else -nilai end else 0 end ) as agg_10
                    , sum(case when substring(x.periode,5,2) between '11' and '11' then case when z.dc='D' then nilai else -nilai end else 0 end ) as agg_11
                    , sum(case when substring(x.periode,5,2) between '12' and '12' then case when z.dc='D' then nilai else -nilai end else 0 end ) as agg_12
					, sum(case when substring(x.periode,5,2) between '01' and '12' then case when z.dc='D' then nilai else -nilai end else 0 end ) as total
             from gr_gaji_d x
	         inner join gr_karyawan y on x.nik=y.nik and x.kode_lokasi=y.kode_lokasi 
			 inner join gr_gaji_param z on x.kode_param=z.kode_param and x.kode_lokasi=z.kode_lokasi 
			 $this->filter and z.jenis='T'
