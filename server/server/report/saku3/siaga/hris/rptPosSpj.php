...
class server_report_saku3_siaga_hris_rptPosSpj extends server_report_basic
{	
	...
	function getHtml()
	{
		...
		$sql="select a.no_spj,date_format(a.tanggal,'%d/%m/%Y') as tgl_spj,a.nik_buat,b.nama,a.keterangan,d.no_ver,
		date_format(d.tanggal,'%d/%m/%Y') as tgl_ver,f.no_app,date_format(f.tanggal,'%d/%m/%Y') as tgl_app,a.kode_lokasi,
		date_format(i.tgl_mulai,'%d/%m/%Y') as tgl_mulai,date_format(g.tgl_selesai,'%d/%m/%Y') as tgl_selesai,
		a.no_pb,date_format(h.tanggal,'%d/%m/%Y') as tgl_pb
		from gr_spj_m a 
		 inner join gr_karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi 
		left join gr_ver_d c on a.no_spj=c.no_bukti and a.kode_lokasi=c.kode_lokasi 
		left join gr_ver_m d on c.no_ver=d.no_ver and d.kode_lokasi=c.kode_lokasi 
		left join gr_app_d e on a.no_spj=e.no_bukti and a.kode_lokasi=e.kode_lokasi 
		left join gr_app_m f on e.no_app=f.no_app and e.kode_lokasi=f.kode_lokasi 
		left join (select no_spj,min(tgl_mulai) as tgl_mulai from
				gr_spj_dh 
				group by no_spj
				) i on a.no_spj=i.no_spj
		left join (select no_spj,max(tgl_selesai) as tgl_selesai from
				gr_spj_dh 
				group by no_spj
				) g on a.no_spj=g.no_spj
		...
		echo $AddOnLib->judul_laporan("data posisi pengajuan perjalanan dinas",$this->lokasi);
		echo "<table border='1' cellspacing='0' cellpadding='0' class='kotak' width='1200'>
  <tr bgcolor='#CCCCCC'>
		<td width='30' align='center' class='header_laporan'>No</td>
		<td width='100' align='center' class='header_laporan'>No BPD</td>
		<td width='60' align='center' class='header_laporan'>Tgl BPD</td>
