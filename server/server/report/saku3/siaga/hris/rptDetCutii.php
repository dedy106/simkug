...
class server_report_saku3_siaga_hris_rptDetCutii extends server_report_basic
{
...
inner join gr_jab b on a.kode_jab=b.kode_jab $this->filter ";
	...
	function getHtml()
	{
		...
		$periode=$tmp[3];
		$bulan=$tmp[4];
		$sql = "select c.periode,c.sts_cuti,a.nik,a.nama,date_format(a.tgl_masuk,'%d/%m/%Y') as tgl_masuk,b.nama as jabatan,isnull(c.so_awal_lalu,0) as so_awal_lalu,
...
left join (select periode,sts_cuti,nik,sum(jumlah) as so_awal,sum(kurang) as kurang,sum(tambah) as so_awal_lalu
		   from gr_cuti_karyawan
           
           group by nik,sts_cuti,periode
		   )c on a.nik=c.nik
...
left join (select nik,sum(jumlah) as so_awal
 ...
 $this->filter ";
		...
		echo $AddOnLib->judul_laporan("rincian cuti tahunan".$nama_cuti,$this->lokasi,"");
...
";
	...
	from gr_cuti a
	where a.nik_buat='$row->nik' and a.sts_cuti='$row->sts_cuti' and a.periode='$row->periode'
	union all
	select a.no_cuti,a.tanggal,a.tgl_mulai,date_format(a.tanggal,'%d/%m/%Y') as tgl,date_format(a.tgl_mulai,'%d/%m/%Y') as tgl_mulai1,date_format(a.tgl_selesai,'%d/%m/%Y') as tgl_selesai,a.alasan,
			a.lama,case when a.sts_cuti='$row->sts_cuti' then a.lama else 0 end as lama1,'-' as alamat
	from gr_cuti_d a
	where a.nik='$row->nik' and a.sts_cuti='$row->sts_cuti' and a.periode='$row->periode'
