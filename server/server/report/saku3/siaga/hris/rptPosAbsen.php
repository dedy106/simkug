...
class server_report_saku3_siaga_hris_rptPosAbsen extends server_report_basic
{
...
left join gr_app_m f on e.no_app=f.no_app and e.kode_lokasi=f.kode_lokasi $this->filter ";
	...
	function getHtml()
	{
		...
		$footer=$row->nama;
		$sql="select a.no_absen,date_format(a.tanggal,'%d/%m/%Y') as tanggal,date_format(a.tgl_absen,'%d/%m/%Y') as tgl_absen,a.nik_buat,b.nama,a.keterangan,d.no_ver,
...
echo $sql;
		...
		echo $AddOnLib->judul_laporan("posisi absen",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
		echo "<table  border='0' cellspacing='2' cellpadding='1'>
  <tr>
    <td align='center'><table border='1' cellspacing='0' cellpadding='0' class='kotak'>
   <tr bgcolor='#CCCCCC'>
     <td width='30'  align='center' class='header_laporan'>No</td>
     <td width='80'  align='center' class='header_laporan'>tgl Absen</td>
	 <td width='60'  align='center' class='header_laporan'>No Absen</td>
     ...
     <td class='isi_laporan' align='center'>$i</td>
      <td class='isi_laporan'>";
		echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenBukti('$row->no_absen','$row->kode_lokasi');\">$row->no_absen</a>";
		echo "</td>
