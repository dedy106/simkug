...
class server_report_saku3_siaga_hris_rptSurat extends server_report_basic
{	
...
inner join gr_jab e on a.kode_jab=e.kode_jab and a.kode_lokasi=e.kode_lokasi $this->filter ";
    ...
    select a.no_surat,datepart(day,a.tanggal) as tgl_surat,datename(weekday,a.tanggal) as hari,datepart(month,a.tanggal) as bulan,
		datepart(year,a.tanggal) as tahun,a.tanggal,a.nik_buat,c.nama,a.keterangan,a.progress,
a.nik_app,d.nama as nama_app,f.nama as jabatan_app,a.kepada,a.kepada,CONVERT(varchar, getdate(), 120) as tgl_input,d.email 
from gr_surat a
inner join gr_karyawan c on a.nik_buat=c.nik and a.kode_lokasi=c.kode_lokasi
inner join gr_karyawan d on a.nik_app=d.nik and a.kode_lokasi=d.kode_lokasi
inner join gr_so f on c.kode_so=f.kode_so and a.kode_lokasi=f.kode_lokasi
 $this->filter order by a.no_surat";
		error_log($sql);
