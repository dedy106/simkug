...
class server_report_saku3_siaga_hris_rptRekapLoker extends server_report_basic
{	
...
from gr_loker_tmp where nik_user='$nik_user' and level_lap<=$level order by rowindex";
  ...
  <tr bgcolor='#CCCCCC'>
	...
	{
		echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenDoc('$row->kode_loker','$row->nama','status','');\">$row->nama</a>";
	}
	else
	{
		echo "$row->nama";
