...
class server_report_saku3_siaga_hris_rptApp extends server_report_basic
{	
...
inner join gr_karyawan b on a.nik_app=b.nik and a.kode_lokasi=b.kode_lokasi $this->filter ";
	...
	function getHtml()
	{
		...
		$kode_bidang=$tmp[4];
		$ver=$tmp[1];
		$sql="select a.no_app,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.nik_app,b.nama,a.keterangan,a.periode,a.kode_lokasi
from gr_app_m a
inner join gr_karyawan b on a.nik_app=b.nik and a.kode_lokasi=b.kode_lokasi $this->filter order by a.no_app";
		...
		echo $AddOnLib->judul_laporan("laporan aproval",$this->lokasi,$AddOnLib->ubah_periode($periode));
