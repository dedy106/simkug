...
class server_report_saku3_siaga_hris_rptGajiRekap extends server_report_basic
{
...
where a.kode_lokasi='01' and a.dc='D' and a.jenis='T'
order by a.no_urut ";
...
where a.kode_lokasi='01' and a.dc='C' and a.jenis='T'
order by a.no_urut ";
		$rs = $dbLib->execute($sql);
		...
		echo "<td width='90'  align='center' class='header_laporan'>AKDHK</td>";
		echo "<td width='90'  align='center' class='header_laporan'>DPLK</td>";
		echo "<td width='90'  align='center' class='header_laporan'>Jamsostek</td>";
		echo "</tr>";
		//data gaji
		$sql="select a.nik,c.nama,c.kode_lokasi,$pdpt1,$beban1,
		dbo.fnGetPph(a.nik,$periode) as pph21,dbo.fnGetAkdhk(a.nik,$periode) as akdhk,dbo.fnGetDplk(a.nik,$periode) as dplk,dbo.fnGetJamsostek(a.nik,$periode) as jamsos
from (select distinct b.nik
	  from gr_gaji_d b
	  inner join gr_karyawan a on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi
	  $this->filter
	   ) a
inner join gr_karyawan c on a.nik=c.nik
inner join (select b.nik,$pdpt2,$beban2 
			from gr_gaji_d b
			inner join gr_karyawan a on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi
			$this->filter
			group by b.nik
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			echo "</td>";
			echo "<td class='isi_laporan' align='right'>";
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenAkdhk('$row->nik','$row->kode_lokasi');\">".number_format($row->akdhk,0,',','.')."</a>";
			echo "</td>";
			echo "<td class='isi_laporan' align='right'>";
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenDplk('$row->nik','$row->kode_lokasi');\">".number_format($row->dplk,0,',','.')."</a>";
			echo "</td>";
			echo "<td class='isi_laporan' align='right'>";
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenJamsos('$row->nik','$row->kode_lokasi');\">".number_format($row->jamsos,0,',','.')."</a>";
			echo "</td>";
			echo "</tr>";
			$i=$i+1;
		}
