...
class server_report_saku3_siaga_hris_rptAppKlaim extends server_report_basic
{	
...
from gr_ver_m a
inner join gr_karyawan b on a.nik_app=b.nik and a.kode_lokasi=b.kode_lokasi $this->filter and a.modul='FINALKLAIM' ";
...
from gr_ver_m a
inner join gr_karyawan b on a.nik_app=b.nik and a.kode_lokasi=b.kode_lokasi $this->filter and a.modul='FINALKLAIM' order by a.no_ver";
		error_log($sql);
		...
		echo $AddOnLib->judul_laporan("approval reimburse",$this->lokasi,$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
	...
	<td width='80' align='center' class='header_laporan'>Nilai Final</td>
  </tr>
";
		
	   $sql1="select  b.no_klaim as no_bukti,a.catatan,convert(varchar,b.tanggal,103) as tanggal,b.keterangan,b.kode_asur+' - '+e.nama as nama_asur,d.kode_loker+' - '+d.nama as loker,
				b.nik_buat+' - '+c.nama as karyawan,convert(varchar,b.tgl_kuitansi,103) as tgl_kuitansi,isnull(x.nilai,0) as nilai,
				convert(varchar,b.tgl_terima,103) as tgl_terima,convert(varchar,b.tgl_final,103) as tgl_final,b.nilai_final
from gr_ver_d a
inner join gr_klaim_m b on a.no_bukti=b.no_klaim and a.kode_lokasi=b.kode_lokasi
...
inner join (select no_klaim,kode_lokasi,sum(nilai) as nilai from gr_klaim_d group by no_klaim,kode_lokasi) x on x.no_klaim=a.no_bukti and a.kode_lokasi=x.kode_lokasi
inner join gr_asur e on e.kode_asur=b.kode_asur and e.kode_lokasi=b.kode_lokasi 
...
order by b.no_klaim ";
	...
	<td align='left' class='isi_laporan'>$row1->tgl_final</td>
	...
	<td align='right' class='isi_laporan'>".number_format($row1->nilai_final)."</td>
