...
{	
	...
	function getHtml()
	{
	   ...
	   datepart(month,a.tanggal) as bulan,datepart(year,a.tanggal) as tahun,c.flag_gaji,c.sts_bank,c.cabang,c.no_rek,c.nama_rek
from gr_gaji_m a
...
from gr_gaji_d a
inner join gr_gaji_param b on a.kode_param=b.kode_param and a.kode_lokasi=b.kode_lokasi
where a.no_gaji='$row->no_gaji'  and a.nik='$row->nik' and a.nilai<0 and a.kode_param<>'PPPH'
order by b.no_urut";
		$rs1 = $dbLib->execute($sql1);
		$tot_pot=0;
		while ($row1 = $rs1->FetchNextObject($toupper=false))
      ...
      <tr >
        <td height='23' colspan='2' align='left' class='header_laporan'><span class='isi_laporan'>Total pendapatan ditransfer  sebesar : Rp $total ke (Bank $row->sts_bank) No Rekening $row->no_rek</span></td>
      </tr>
    </table></td>
  </tr>";
