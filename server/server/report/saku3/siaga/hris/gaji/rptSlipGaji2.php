...
class server_report_saku3_siaga_hris_gaji_rptSlipGaji extends server_report_basic
{	
...
$this->filter ";
	...
	function getHtml()
	{
		...
		$kode_lokasi=$tmp[0];
		$periode=$tmp[1];
		$sql="select  f.pdpt,f.pot,b.nama,b.kode_lokasi,d.no_gaji,d.periode,e.tanggal,e.keterangan,
    datepart(month,e.tanggal) as bulan,datepart(year,e.tanggal) as tahun,b.flag_gaji,e.pesan,e.flag_penilaian,
  a.nik,a.kode_param,a.kode_lokasi,x.kode_loker,y.akun_gaji as kode_akun,x.kode_so, 
  case when a.jenis_gaji = 'BULANAN' then a.nilai else a.nilai * isnull(z.jml_hari,0) end as nilai
from gr_karyawan b
        inner join gr_gaji_nik a on b.nik=a.nik and b.kode_lokasi=a.kode_lokasi
        ...
        inner join gr_gaji_akun y on a.nik=y.nik and a.kode_lokasi=y.kode_lokasi 	
           left join ( 
            ...
            where flag_form='HARI' group by nik 		
          ) z on a.nik=z.nik
left join (select distinct kode_param,nik,no_gaji,kode_lokasi,periode
     from gr_gaji_d 
     )d on a.nik=d.nik and a.kode_lokasi=d.kode_lokasi
inner join gr_gaji_m e on d.no_gaji=e.no_gaji and d.kode_lokasi=e.kode_lokasi
LEFT JOIN (select kode_param,no_load,nik,sum(case when kode_param in ('TMBH','GAPOK','KONJ','TGRD','TUJAB','TUSUS','LBR','REM','RPL','BNS','TTRANS') then nilai else 0 end) as pdpt,
sum(case when kode_param in ('IKOP','PKOP','PASU') then nilai else 0 end) as pot,
periode 
          from gr_gajiload_d 
          where flag_form='GAJIVAR' 
          group by nik,kode_param,periode,nilai,no_load) f on d.nik=f.nik and d.kode_param=f.kode_param and d.periode=f.periode
