...
class server_report_saku3_siaga_hris_rptCvSk extends server_report_basic
{	
...
inner join gr_jur m on a.kode_jur=m.kode_jur and a.kode_lokasi=m.kode_lokasi $this->filter ";
    ...
    convert(varchar,a.tgl_lahir,103) as tgl_lahir,a.alamat,k.nama as nama_kota,a.kodepos,a.no_telp,a.no_hp,a.sex,a.email,a.gol_darah, 
    a.nik2,a.npwp,a.suku,a.sts_agama,c.nama as nama_agama,a.sts_didik,d.nama as nama_didik,
    a.foto,a.sts_pajak, 
	case when a.sts_pajak<>'TK0' then convert(varchar,a.tgl_nikah,103) else '' end as tgl_nikah , 
	convert(varchar,a.tgl_masuk,103) as tgl_masuk,a.kode_jab,
    ...
    e.nama as nama_jab,f.nama as nama_grade,g.nama as nama_loker,h.nama as nama_dir,
     a.lembaga,a.asal_lamaran,a.cabang,a.no_rek,a.nama_rek,a.nip,a.flag_gaji,i.nama as nama_dept,j.nama as nama_sdm,l.nama as nama_jur
     from gr_karyawan a 
    ...
    left join gr_dept i on a.kode_dept=i.kode_dept and a.kode_lokasi = i.kode_lokasi 
    left join gr_status_sdm j on a.sts_sdm=j.sts_sdm and a.kode_lokasi = j.kode_lokasi 
    left join gr_kota k on a.kode_kota=k.kode_kota and a.kode_lokasi=k.kode_lokasi
    left join gr_jur l on a.kode_jur=l.kode_jur and a.kode_lokasi=l.kode_lokasi
$this->filter
order by a.nik ";
	...
	<tr>
	   ...
	   <tr>
        <td><table width='100%' border='1' cellspacing='0' cellpadding='0' class='kotak'>
          <tr align='center' bgcolor='#CCCCCC'>
            ...
            <td width='120' class='isi_bukti'>No SK </td>
          	<td width='400' class='isi_bukti'>Keterangan</td>
          </tr>";

          $sql1="select a.nik,a.no_dok,h.nama as ket_sk,a.tgl_awal,b.nama as nama_jab
				from gr_dinas a 
        left join gr_reason h on a.kode_reason=h.kode_reason and a.kode_lokasi=h.kode_lokasi 
        left join gr_jab b on a.kode_jab=b.kode_jab and a.kode_lokasi=b.kode_lokasi
				where a.nik = '$row->nik'  and a.flag_form not in ('NONDINAS') and a.kode_lokasi='$row->kode_lokasi' order by a.tgl_awal desc";
		...
		{
         echo "<tr >
            <td align='center' class='isi_bukti'>$row1->tgl_awal</td>
            <td class='isi_bukti'>$row1->no_dok</td>
            <td class='isi_bukti'>$row1->nama_jab</td>
          </tr>";
		} 
		  echo "</table></td>
      </tr>
      ...
      <tr>
        <td><table width='100%' border='1' cellspacing='0' cellpadding='0' class='kotak'>
		  ...
		  $sql1="select a.nama as latih,a.nik,b.nama,d.nama as sts_latih,a.nama as lth,a.lama,date_format(a.tgl_mulai,'%d/%m/%Y') as tgl_mulai,date_format(a.tgl_selesai,'%d/%m/%Y') as tgl_selesai,a.panitia,f.nama as kota,g.nama as dana, 
		...
		{
         echo "<tr>
            <td class='isi_bukti'>$row1->latih</td>
...
where a.nik = '$row->nik' and a.kode_lokasi='$row->kode_lokasi' order by a.tanggal desc";
      ...
      <tr>
        <td><table width='100%' border='1' cellspacing='0' cellpadding='0' class='kotak'>
          <tr align='center' bgcolor='#CCCCCC'>
            <td width='150' class='isi_bukti'>No SK</td>
            <td width='200' class='isi_bukti'>Sanksi</td>
...
where a.nik = '$row->nik' and a.kode_lokasi='$row->kode_lokasi' order by a.tgl_mulai desc";
		...
		while ($row1 = $rs1->FetchNextObject($toupper=false))
		{
         echo "<tr>
            <td class='isi_bukti'>$row1->no_sk</td>
			<td class='isi_bukti'>$row1->snk</td>
