...
class server_report_saku3_siaga_rptPermintaanByr extends server_report_basic
{
...
$this->filter order by a.no_pb ";
  ...
  from gr_pb_j a inner join gr_pb_m f on a.no_pb=f.no_pb and a.kode_lokasi=f.kode_lokasi 
  inner join angg_r c on f.ref1=c.no_bukti 
...
where a.no_pb='$row->no_pb' and a.kode_lokasi='$row->kode_lokasi' and c.dc='D'
order by c.no_bukti ";
// echo"$sql";
...
// echo"$sql";
  ...
  <tr>
	...
	$sql="select a.item,a.jumlah,a.harga,(a.jumlah*a.harga) as jumlah_harga
  from log_spk_d a inner join gr_pb_m b on a.kode_lokasi=b.kode_lokasi and a.no_spk=b.ref1
where b.no_pb='$row->no_pb' and a.kode_lokasi='$row->kode_lokasi'";
// echo"$sql";
    $rs1 = $dbLib->execute($sql);
	$i=1; $nilai=0; 
	while ($row1 = $rs1->FetchNextObject($toupper=false))
	{
      echo "<tr>
        ...
        <td class='isi_laporan' align='right'>".number_format($row1->jumlah_harga,0,",",".")."</td>
      </tr>";
		$i=$i+1;
	}  
    echo "</table></td>
