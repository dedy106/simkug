...
class server_report_saku3_gl_rptYptLabaRugiGarBidang extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="select nama from bidang where kode_lokasi='$kode_lokasi' and kode_bidang='$kode_bidang'";
		$rs = $dbLib->execute($sql);
		...
		$sql = "select a.kode_neraca,a.kode_fs,a.kode_lokasi,a.nama,a.tipe,a.level_spasi,
			...
			where a.modul='L'  and a.kode_fs='$kode_fs' and a.periode='$periode'  and a.level_lap<=$lev and a.kode_bidang='$kode_bidang'
			order by a.rowindex ";
  ...
  <tr>
    <td class='lokasi_laporan' align='center'>$this->lokasi <br>$kode_bidang</td>
  </tr>
  <tr>
  ...
  <tr bgcolor='#dbeef3'>
    ...
    <td width='60' align='center' class='header_laporan'>YoY Growth</td>
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			if ($row->n7!=0)
			{
				$persen1=$row->n6/$row->n7;
			}
			if ($row->n3!=0)
			{
				$persen2=(($row->n6/$row->n3)-1)*100;
			}
			if ($row->n2!=0)
			{
				$persen3=$row->n4/$row->n2;
			}
			if ($row->n5!=0)
			{
				$persen4=(($row->n4/$row->n5)-1)*100;
			}
			echo "<tr >
			...
			if ($bentuk=="Detail" && $row->tipe=="Posting")
			{
				...
				$sql1="select a.kode_akun,c.nama,a.n1,a.n2,a.n3,a.n4,a.n5,a.n6,a.n7,a.n8
					from exs_glma_gar a
