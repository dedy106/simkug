...
class server_report_saku3_gl_rptGlTbBidang extends server_report_basic
{
	...
	function getHtml()
	{
       ...
       case when a.so_akhir<0 then -so_akhir else 0 end as so_akhir_kredit
		...
		if ($trail=="1")
		{
			$sql = "select a.kode_akun,a.kode_pp,d.nama,a.kode_lokasi,a.debet,a.kredit,a.so_awal,so_akhir, 
       case when a.so_awal>0 then so_awal else 0 end as so_awal_debet,
       case when a.so_awal<0 then -so_awal else 0 end as so_awal_kredit, 
       case when a.so_akhir>0 then so_akhir else 0 end as so_akhir_debet,
       case when a.so_akhir<0 then -so_akhir else 0 end as so_akhir_kredit
		...
		echo $AddOnLib->judul_laporan("NERACA LAJUR",$this->lokasi."<br>".$nama_pp,"Periode ".$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
	...
	<td width='50' rowspan='2' class='header_laporan' align='center'>Kode PP</td>
