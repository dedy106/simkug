...
class server_report_saku3_gl_rptGlNeracaKonsolDetailTm extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$rs = $dbLib->execute($sql);
		$sql = "select kode_neraca,kode_fs,kode_lokasi,nama,tipe,level_spasi,tipe,n4
			from neraca_tmp 
  ...
  <tr>
    <td><table border='1' cellspacing='0' cellpadding='0' class='kotak'>
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			{
				echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTbJejer('$row->kode_neraca','$row->kode_lokasi','$periode');\">$row->nama</a>";
			}
			else
			{
				echo "$row->nama";
			...
			if ($bentuk=="Detail" && $row->tipe=="Posting")
			{
				...
				{	
					...
					<td class='detail_laporan' align='right'>".number_format($row1->so_akhir,0,",",".")."</td>
				  </tr>";
				}
		...
		$sql = "select kode_neraca,kode_fs,kode_lokasi,nama,tipe,level_spasi,tipe,-n4 as n4
			from neraca_tmp 
