...
class server_report_saku3_gl_rptGlLabaRugiKonsol2Ypt extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$periode2=$tmp[3];
		$lev=$tmp[4];
		$sql="exec sp_neraca_konsol2 'FS1','L','K','$lev','$periode','$periode2','$kode_lokasi','$nik_user' ";
		$rs = $dbLib->execute($sql);
  ...
  <tr>
    ...
    <td valign='top' class='isi_laporan' align='right'>";
			...
			{
				echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_neraca','$kode_lokasi','$periode');\">$nilai1</a>";
			}
			else
			{
				echo "$nilai1";
    ...
    <td valign='top' class='isi_laporan' align='right'>";
			...
			{
				echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_neraca','$kode_lokasi','$periode2');\">$nilai2</a>";
			}
			else
			{
				echo "$nilai2";
