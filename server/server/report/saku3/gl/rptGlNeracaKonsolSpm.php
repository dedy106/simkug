...
class server_report_saku3_gl_rptGlNeracaKonsolSpm extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$kode_lokasi=$tmp[2];
		$kode_fs=$tmp[3];
		$sql="exec sp_neraca_konsol '$kode_fs','N','K','5','$periode','$kode_lokasi','$nik_user' ";
  ...
  <tr>
    <td><table border='1' cellspacing='0' cellpadding='0' class='kotak'>
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			{
				echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTbJejer('$row->kode_neraca1','$row->kode_lokasi','$periode');\">$row->nama1</a>";
			}
			else
			{
				echo "$row->nama1";
			...
			{
				echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTbJejer('$row->kode_neraca2','$row->kode_lokasi','$periode');\">$row->nama2</a>";
			}
			else
			{
				echo "$row->nama2";
