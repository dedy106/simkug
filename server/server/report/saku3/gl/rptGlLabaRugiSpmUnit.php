...
class server_report_saku3_gl_rptGlLabaRugiSpmUnit extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="exec sp_neraca_unit_dw 'FS2','L','S','$lev','$periode','$kode_lokasi','$nik_user' ";
  ...
  <tr>
    <td  class='lokasi_laporan2' align='center'>LAPORAN LABA RUGI UNIT</td>
  </tr>
  <tr>
  ...
  <tr>
    <td><table border='1' cellspacing='0' cellpadding='0' class='kotak'>
		  <tr bgcolor='#CCCCCC'>
			...
			<td width='100' class='header_laporan' align='center'>Pendapatan</td>
			<td width='100' class='header_laporan' align='center'>Harga Pokok Penjualan</td>
			<td width='100' class='header_laporan' align='center'>Laba (Rugi) Kotor</td>
		</tr>";
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			$n1=""; $n2=""; $n3="";
			if ($row->tipe!="Header" && $row->n1!=0 )
			{
				$n1=number_format($row->n1,0,",",".");
			}
			if ($row->tipe!="Header" && $row->n2!=0 )
			{
				$n2=number_format($row->n2,0,",",".");
			}
			if ($row->tipe!="Header" && $row->n3!=0 )
			{
