...
class server_report_saku3_gl_rptGlLabaRugiPp extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$bentuk=$tmp[5];
		$kode_pp=$tmp[6];
		$sql="exec sp_neraca_pp_dw '$kode_fs','L','S',$lev,'$periode','$kode_lokasi','$nik_user','$kode_pp' ";
		...
		$sql = "select kode_neraca,kode_fs,kode_lokasi,nama,tipe,level_spasi,
			...
			from neraca_tmp 
			where modul='L' and nik_user='$nik_user' and level_lap<=$lev
			order by rowindex ";
    ...
    <td><table border='1' cellspacing='0' cellpadding='0' class='kotak'>
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			if ($bentuk=="Detail" && $row->tipe=="Posting")
			{
				...
				$sql1="
						select a.kode_akun,d.nama,case c.jenis_akun when 'Pendapatan' then -a.so_akhir else a.so_akhir end as so_akhir 
						from glma_pp_tmp a
						...
						where b.kode_fs='$kode_fs' and b.kode_lokasi='$kode_lokasi' and b.kode_neraca='$kode_neraca' and a.nik_user='$nik_user' and a.so_akhir<>0 and a.kode_pp='$kode_pp'
