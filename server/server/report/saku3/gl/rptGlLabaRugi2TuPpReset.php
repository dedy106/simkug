...
class server_report_saku3_gl_rptGlLabaRugi2TuPpReset extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="exec sp_neraca2_gar_pp_dw_reset '$kode_fs','L','S','1','$periode','$periode2','$kode_lokasi','$nik_user','$kode_pp','$injenis' ";
		//echo $sql;
		$rs = $dbLib->execute($sql);
		...
		echo $AddOnLib->judul_laporan("laporan sisa hasil usaha kendali",$this->lokasi."<br>".$nama,"Periode yang Berakhir $totime");
		...
		<td class='isi_laporan' align='center'>".number_format($persen3,2,',','.')."%</td>
			...
			if ($bentuk=="Detail" && $row->tipe=="Posting")
			{
				...
				$sql1="	select a.kode_akun,c.nama,
						...
						where b.kode_fs='$kode_fs' and b.kode_lokasi='$kode_lokasi' and b.kode_neraca='$kode_neraca' and a.nik_user='$nik_user' and a.kode_pp='$kode_pp' and (a.n1<>0 or a.n2<>0 or a.n3<>0 or a.n4<>0) 
