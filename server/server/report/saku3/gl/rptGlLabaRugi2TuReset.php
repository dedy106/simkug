...
class server_report_saku3_gl_rptGlLabaRugi2TuReset extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="exec sp_neraca2_gar_dw_reset '$kode_fs','L','S','1','$periode','$periode2','$kode_lokasi','$injenis','$nik_user' ";
		//echo $sql;
		$rs = $dbLib->execute($sql);		
		...
		echo $AddOnLib->judul_laporan("laporan sisa hasil usaha",$this->lokasi,"Periode yang Berakhir $totime");
		echo "<table  border='1' cellpadding='0' cellspacing='0' class='kotak' width='1100'>
    <tr bgcolor='#CCCCCC'>
		...
		<td class='isi_laporan' align='center'>".number_format($persen3,2,',','.')."%</td>
			...
			if ($bentuk=="Detail" && $row->tipe=="Posting")
			{
				...
				{	
					...
					if ($row1->n3!=0)
					{
						$persen1=($row1->n1/$row1->n3)*100;
					}
					if ($row1->n4!=0)
					{
						$persen2=($row1->n1/$row1->n4)*100;
					}
					if ($row1->n2!=0)
					{
						$persen3=($row1->n1-$row1->n2)/$row1->n2*100;
					}
