...
class server_report_saku3_gl_rptGlJurnalBuktiBidang extends server_report_basic
{
  ...
  <tr>
    <td><table width='800' border='1' cellspacing='0' cellpadding='0' class='kotak'>
	  ...
	  $sql1="select a.kode_akun,b.nama as nama_akun,a.nilai,a.no_dokumen,a.keterangan,isnull(a.debet,0) as debet,isnull(a.kredit,0) as kredit,a.kode_curr,a.kode_pp,a.kode_drk
					  from (select kode_lokasi,no_urut,kode_akun,no_dokumen,dc,nilai,kode_pp,kode_drk,keterangan,case when dc='D' then nilai else 0 end as debet,case when dc='C' then nilai else 0 end as kredit,kode_curr
							from gldt_h
							where periode='$row->periode' and kode_lokasi='$row->kode_lokasi' and no_bukti='$row->no_bukti'
							union all
							select kode_lokasi,no_urut,kode_akun,no_dokumen,dc,nilai,kode_pp,kode_drk,keterangan,case when dc='D' then nilai else 0 end as debet,case when dc='C' then nilai else 0 end as kredit,kode_curr
							from gldt
							where periode='$row->periode' and kode_lokasi='$row->kode_lokasi' and no_bukti='$row->no_bukti'
							)a 
