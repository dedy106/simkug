...
class server_report_saku3_gl_rptGlLabaRugi2TuFakultasReset extends server_report_basic
{
	...
	function getHtml()
	{
		...
		if ($status=="Reseting") {
			$injenis="ABT";
			$sql_jenis=" and a.modul in ('ORGI','ABT') ";
		}
		if ($status=="Original") {
			$injenis="ORGI";
			$sql_jenis=" and a.modul in ('ORGI') ";
		}
		if ($status=="Deviasi") {
			$injenis="DEV";
			$sql_jenis=" and a.modul in ('ABT') ";
		}
		
		if ($jenis=="All")
		{
			$sql="select kode_fakultas,nama from aka_fakultas order by kode_fakultas";
		}
		else
		{
			$sql="select kode_fakultas,nama from aka_fakultas where kode_fakultas='$kode_fak' order by kode_fakultas";
		}
		$rs2 = $dbLib->execute($sql);
		
		while ($row2 = $rs2->FetchNextObject($toupper=false))
		{
			$sql="exec sp_neraca2_gar_fak_dw_reset '$kode_fs','L','S','1','$periode','$periode2','$kode_lokasi','$nik_user','$row2->kode_fakultas','$injenis' ";
			//echo $sql;
			$rs = $dbLib->execute($sql);	
			...
			echo $AddOnLib->judul_laporan("LAPORAN LABA RUGI ANGGARAN FAKULTAS",$this->lokasi."<br>".$row2->nama,"Periode yang Berakhir $totime");
			...
			<td class='isi_laporan' align='center'>".number_format($persen3,2,',','.')."%</td>
				...
				if ($bentuk=="Detail" && $row->tipe=="Posting")
				{
					...
					$sql1="	select a.kode_akun,c.nama,
							...
							where b.kode_fs='$kode_fs' and b.kode_lokasi='$kode_lokasi' and b.kode_neraca='$kode_neraca' and a.nik_user='$nik_user' and d.kode_fakultas='$kode_fak' 
