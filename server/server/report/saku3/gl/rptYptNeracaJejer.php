...
class server_report_saku3_gl_rptYptNeracaJejer extends server_report_basic
{
	...
	function getHtml()
	{
		...
		echo $AddOnLib->judul_laporan("LAPORAN NERACA JEJER",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
    <td  align='center' width='300' class='header_laporan'>Deskripsi</td>";
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			$jum=$jum+1;
			$tmp_sql.="sum(case when kode_lokasi='$row->kode_lokasi' then (case when modul='P' then -n4 else n4 end) else 0 end) as 'n4$row->kode_lokasi',";
    ...
    echo "<td  align='center' class='header_laporan'>$row->kode_lokasi - $row->nama</td>";
		...
		$pdpt[$jum]="n4t";
		$tmp_sql.="sum(case when modul='P' then -n4 else n4 end) as 'n4t',";
...
from neraca 
where kode_lokasi='$kode_lokasi' and kode_fs='$kode_fs' and modul='A' and level_lap<=$lev
order by rowindex";
...
group by kode_neraca";
		...
		$sql="select kode_neraca,nama ,level_spasi,tipe
from neraca 
where kode_lokasi='$kode_lokasi' and kode_fs='$kode_fs' and modul='P' and level_lap<=$lev
order by rowindex";
