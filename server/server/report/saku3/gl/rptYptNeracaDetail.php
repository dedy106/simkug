...
class server_report_saku3_gl_rptYptNeracaDetail extends server_report_basic
{
	...
	function getHtml()
	{
		...
		from exs_neraca 
		where kode_lokasi='$kode_lokasi' and kode_fs='$kode_fs' and periode='$periode' and level_lap<=$lev 
		order by rowindex ";
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			if ($bentuk=="Detail" && $row->tipe=="Posting")
			{
				...
				$sql1="select a.kode_akun,c.nama,a.so_akhir
					...
					inner join relakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
					inner join masakun c on a.kode_akun=c.kode_akun and a.kode_lokasi=c.kode_lokasi
					where b.kode_fs='$kode_fs' and a.kode_lokasi='$kode_lokasi' and b.kode_neraca='$kode_neraca' and nik_user='$nik_user' and so_akhir<>0
					order by a.kode_akun ";
