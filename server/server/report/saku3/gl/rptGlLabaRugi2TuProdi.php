...
class server_report_saku3_gl_rptGlLabaRugi2TuProdi extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="exec sp_neraca2_gar_prodi_dw 'FS2','L','S','1','$periode','$periode2','$kode_lokasi','$nik_user','$kode_prodi' ";
		$rs = $dbLib->execute($sql);	
		...
		echo $AddOnLib->judul_laporan("laporan sisa hasil usaha prodi",$this->lokasi."<br>".$nama,"Periode yang Berakhir $totime");
		...
		<td class='isi_laporan' align='center'>".number_format($persen3,2,',','.')."%</td>
			...
			if ($bentuk=="Detail" && $row->tipe=="Posting")
			{
				...
				$sql1="	select a.kode_akun,c.nama,
						...
						inner join prodi_persen d on a.kode_pp=d.kode_pp and a.kode_akun=d.kode_akun and a.kode_lokasi=d.kode_lokasi
						where b.kode_fs='$kode_fs' and b.kode_lokasi='$kode_lokasi' and b.kode_neraca='$kode_neraca' and a.nik_user='$nik_user' and d.kode_prodi='$kode_prodi' 
