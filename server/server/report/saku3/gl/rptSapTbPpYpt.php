...
class server_report_saku3_gl_rptSapTbPpYpt extends server_report_basic
{
	...
	function getHtml()
	{
	  ...
	  <tr >
		...
		{
			$sql="select a.no_bukti,a.kode_akun,b.nama,a.kode_lokasi,abs(a.nilai) as nilai,substring(a.periode,5,2) as bulan,a.kode_pp,d.kode_pc,
       a.periode,substring(a.periode,1,5) as tahun, case when dc='D' then '40' else '50' end as dc,substring(a.kode_akun,1,1) as klp,d.kode_ba
from sap_tb_d a 
inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
inner join sap_tb_m c on a.no_bukti=c.no_bukti and a.kode_lokasi=c.kode_lokasi
...
$this->filter and a.nilai<>0 ";
			...
			while ($row = $rs->FetchNextObject($toupper=false))
			{
				...
				for ($x = 1; $x <= 2; $x++) 
				{
					...
					<td >IDR</td>
					<td >$no_ref</td>
					<td >Mutasi Periode $row->periode</td>
    ...
    function createCSV(){
		...
		$tgl=$d.".".$bulan.".".$tahun;
		$sql="select kode_lokasi from lokasi where kode_lokasi in ('12') ";
		$rs2 = $dbLib->execute($sql);
        ...
        while ($row2 = $rs2->FetchNextObject($toupper=false))
		{
			...
			$sql="select a.kode_akun,b.nama,a.kode_lokasi,abs(a.nilai) as so_akhir,substring(a.periode,5,2) as bulan,
       a.periode,substring(a.periode,1,5) as tahun, case when dc='D' then '40' else '50' end as dc,substring(a.kode_akun,1,1) as klp,d.kode_ba
from sap_tb_d a 
inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
inner join sap_tb_m c on a.no_bukti=c.no_bukti and a.kode_lokasi=c.kode_lokasi
inner join pp d on a.kode_pp=d.kode_pp and a.kode_lokasi=d.kode_lokasi
where a.kode_lokasi='12' and a.periode='201601' and c.no_bukti='12-SAP1601.001'";
