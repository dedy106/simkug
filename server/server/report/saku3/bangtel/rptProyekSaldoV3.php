...
class server_report_saku3_bangtel_rptProyekSaldoV3 extends server_report_basic
{
	...
	function getHtml()
	{
	   ...
	   isnull(c.nilai_kas,0) as nilai_kas,a.nilai+a.nilai_ppn-a.nilai_pph-isnull(c.nilai_kas,0) as saldo,a.tanggal,x.nilai as pdpt,x.kode_akun 
from spm_piutang_m a
inner join spm_piutang_j x on a.no_piutang=x.no_piutang and a.kode_lokasi=x.kode_lokasi and x.jenis='PDPT'
...
select a.kode_lokasi,a.no_piutang,a.no_dokumen,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.keterangan,a.akun_piutang,a.kode_pp,
	   ...
	   isnull(c.nilai,0) as nilai_kas,a.nilai+a.nilai_ppn-a.nilai_pph-isnull(c.nilai,0) as saldo,a.tanggal,x.nilai as pdpt,x.kode_akun
from piutang_m a
inner join piutang_j x on a.no_piutang=x.no_piutang and a.kode_lokasi=x.kode_lokasi and x.jenis='PDPT'
inner join cust b on a.kode_cust=b.kode_cust and a.kode_lokasi=b.kode_lokasi 
...
order by a.tanggal ";
   ...
   <tr bgcolor='#CCCCCC'>
     ...
     <td align='center' class='header_laporan'>Pendapatan</td>
   </tr>";
