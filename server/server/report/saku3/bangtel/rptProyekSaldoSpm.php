...
class server_report_saku3_bangtel_rptProyekSaldoSpm extends server_report_basic
{
	...
	isnull(i.piutang,0) as piutang,isnull(j.kas_piutang,0) as kas_piutang
from spm_proyek a
...
left join (select kode_proyek,kode_lokasi,sum(nilai) as bymhd 
		   from yk_pb_m 
		   where kode_lokasi='$kode_lokasi' 
		   group by kode_proyek,kode_lokasi
...
left join (select a.kode_proyek,a.kode_lokasi,sum(a.nilai) as piutang 
		   from spm_piutang_d a
		   where a.kode_lokasi='$kode_lokasi' 
		   group by a.kode_proyek,a.kode_lokasi
		  )i on a.kode_proyek=i.kode_proyek and a.kode_lokasi=h.kode_lokasi
...
order by a.kode_proyek";
   ...
   <tr bgcolor='#CCCCCC'>
	 ...
	 <td class='isi_laporan'><a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenBdd('$row->kode_proyek','$row->kode_lokasi');\">".number_format($row->bdd,0,",",".")."</a></td>
	 <td class='isi_laporan'><a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenBeban('$row->kode_proyek','$row->kode_lokasi');\">".number_format($row->beban,0,",",".")."</a></td>
	 <td class='isi_laporan' align='right'>".number_format($row->bdd-$row->beban,0,",",".")."</td>
	 <td class='isi_laporan'><a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenBymhd('$row->kode_proyek','$row->kode_lokasi');\">".number_format($row->bymhd,0,",",".")."</a></td>
     <td class='isi_laporan'><a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenBymhdKas('$row->kode_proyek','$row->kode_lokasi');\">".number_format($row->kas_bymhd,0,",",".")."</a></td>
	<td class='isi_laporan' align='right'>".number_format($row->bymhd-$row->kas_bymhd,0,",",".")."</td>
	<td class='isi_laporan'><a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenPiutang('$row->kode_proyek','$row->kode_lokasi');\">".number_format($row->piutang,0,",",".")."</a></td>
	<td class='isi_laporan'><a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenPiutangKas('$row->kode_proyek','$row->kode_lokasi');\">".number_format($row->kas_piutang,0,",",".")."</a></td>
	<td class='isi_laporan' align='right'>".number_format($row->piutang-$row->kas_piutang,0,",",".")."</td>
	  ...
	  <td class='header_laporan' align='right'>".number_format($bdd-$beban,0,",",".")."</td>
