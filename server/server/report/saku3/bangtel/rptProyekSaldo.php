...
class server_report_saku3_bangtel_rptProyekSaldo extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="select a.kode_lokasi,a.no_piutang,a.no_dokumen,a.status,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.keterangan,a.akun_piutang,a.kode_pp,
...
left join (select a.no_piutang,a.kode_lokasi,sum(a.nilai_kas) as nilai_kas
		   ...
		   where a.kode_lokasi<='$kode_lokasi' and b.tanggal <= '$tgl_aging'  -- a.periode<='$periode'
		   group by a.no_piutang,a.kode_lokasi
		  )c on a.no_piutang=c.no_piutang and a.kode_lokasi=b.kode_lokasi
$this->filter and (a.no_piutang NOT LIKE '%RVB%') and a.tanggal <= '$tgl_aging' $tmp order by a.no_piutang";
   ...
   <tr bgcolor='#CCCCCC'>
	 ...
	 <td width='60' rowspan='2' align='center' class='header_laporan'>Status</td>
