...
class server_report_saku3_fitnes_rptJualKupon extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$tmp=explode("/",$this->filter2);
		$periode=$tmp[0];
		$sql="select a.no_kas,a.kode_lokasi,a.keterangan,date_format(a.tanggal,'%d/%m/%Y') as tgl 
from kas_m a
$this->filter
order by a.no_kas ";
		...
		echo $AddOnLib->judul_laporan("penjualan voucher",$this->lokasi,"Periode ".$AddOnLib->ubah_periode($periode));
      ...
      <tr>
        ...
        <td width='380' class='header_laporan'>: $row->no_kas </td>
      </tr>
	  ...
	  $sql1="select a.no_kupon,date_format(a.tgl_awal,'%d/%m/%Y') as tgl_awal,date_format(a.tgl_akhir,'%d/%m/%Y') as tgl_akhir,a.harga,a.modul
from fi_kupon_d a 
