...
class server_report_saku3_tu_proyek_rptProyekApp extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="select a.no_app,a.kode_lokasi,case when b.nilai < 100000000 then '-' else b.kode_proyek end as kode_proyek,b.nama,b.no_pks,b.kode_cust,d.nama as nama_cust,b.nilai,
	   ...
	   e.kode_drkb,e.kode_drkp,f.nama nama_drkb,g.nama as nama_drkp,a.nik_user,h.nama as nama_app,b.nama,
	   datediff(month,b.tgl_mulai,b.tgl_selesai)+1 as bulan
from prb_proyek_app a
inner join prb_proyek b on a.no_bukti=b.kode_proyek and a.kode_lokasi=b.kode_lokasi
inner join pp c on b.kode_pp=c.kode_pp and b.kode_lokasi=c.kode_lokasi
inner join prb_cust d on b.kode_cust=d.kode_cust and b.kode_lokasi=d.kode_lokasi
inner join prb_proyek_jenis e on b.kode_jenis=e.kode_jenis and b.kode_lokasi=e.kode_lokasi
left join drk f on e.kode_drkp=f.kode_drk and e.kode_lokasi=f.kode_lokasi and substring(a.periode,1,4)=f.tahun
left join drk g on e.kode_drkb=g.kode_drk and e.kode_lokasi=g.kode_lokasi and substring(a.periode,1,4)=g.tahun
...
";
  ...
  <tr>
    <td colspan='2'>Bandung, ".substr($row->tgl_input,8,2)." ".$AddOnLib->ubah_periode(substr(str_replace("-","",$row->tgl_input),0,6))." </td>
  </tr>
