...
class server_report_saku3_tu_proyek_rptProyekKartuBymhd extends server_report_basic
{
...
$this->filter order by a.kode_proyek";
		...
		echo $AddOnLib->judul_laporan("Kartu BYMHD",$this->lokasi,$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
	...
	<td width='90' class='header_laporan' align='center'>Saldo</td>
			...
			$sql="select a.no_bukti,convert(varchar,b.tanggal,103) as tgl,b.keterangan,a.nilai,b.tanggal,a.modul,
			case when a.dc='D' then a.nilai else 0 end as debet, case when a.dc='C' then a.nilai else 0 end as kredit 
from prb_prbeban_d a
inner join it_aju_m b on a.no_bukti=b.no_bukti and a.kode_lokasi=b.kode_lokasi
