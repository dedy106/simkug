...
class server_report_saku3_tu_proyek_rptProyekKartuPiutang extends server_report_basic
{
...
$this->filter order by a.kode_proyek";
		...
		echo $AddOnLib->judul_laporan("Kartu Piutang",$this->lokasi,$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
	...
	<td width='90' class='header_laporan' align='center'>Saldo</td>
			...
			$sql="select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai,a.tanggal,a.modul,a.no_dokumen,
	   case when a.dc='D' then a.nilai else 0 end as debet, case when a.dc='C' then a.nilai else 0 end as kredit 
from trans_j a
where a.modul='PRPIU' and jenis='PIU' and a.kode_lokasi='$kode_lokasi' and a.no_dokumen='$row->kode_proyek'
order by a.tanggal
 ...
 ";
			...
			while ($row1 = $rs1->FetchNextObject($toupper=false))
			{
				...
				echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenBukti('$row1->no_bukti','$row->kode_lokasi');\">$row1->no_bukti</a>";
				echo "</td>
