...
class server_report_saku3_tu_proyek_rptRabPos extends server_report_basic
{
	...
	function getHtml()
	{
	    ...
	    convert(varchar(20),a.tgl_mulai,103) as tgl_mulai,convert(varchar(20),a.tgl_selesai,103) as tgl_selesai,convert(varchar(20),a.tanggal,103) as tgl_rab,i.no_app as verrab,convert(varchar(20),k.tanggal,103) as tgl_verrab,k.status as status_verrab, 
		a.nilai,a.nilai_or,a.p_or,a.kode_jenis,d.nama as nama_jenis,
		a.no_app_proyek as no_ver,convert(varchar(20),e.tanggal,103) as tgl_ver,e.status as status_ver,
		g.kode_proyek,isnull(g.nilai,0) as nilai_proyek,isnull(g.nilai_or,0) as or_proyek,
		h.no_app,convert(varchar(20),h.tanggal,103) as tgl_app,h.status as status_app
from prb_rab_m a
inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi
inner join prb_cust c on a.kode_cust=c.kode_cust and a.kode_lokasi=c.kode_lokasi
left join prb_rabapp_m i on a.no_rab=i.no_rab and a.kode_lokasi=i.kode_lokasi
left join prb_rabapp_m j on a.no_rab=j.no_rab and a.kode_lokasi=j.kode_lokasi
left join prb_proyek_jenis d on j.kode_jenis=d.kode_jenis and j.kode_lokasi=d.kode_lokasi
left join prb_proyek_app k on k.no_app=i.no_app and k.kode_lokasi=i.kode_lokasi
left join prb_proyek_app e on j.no_app=e.no_app and j.kode_lokasi=e.kode_lokasi
left join prb_rabapp_m f on e.no_app=f.no_app and e.kode_lokasi=f.kode_lokasi
left join prb_proyek g on j.kode_proyek=g.kode_proyek and j.kode_lokasi=g.kode_lokasi
left join prb_proyek_app h on g.no_app=h.no_app and g.kode_lokasi=h.kode_lokasi
...
// echo $sql;
   ...
   <tr bgcolor='#CCCCCC'>
	 ...
	 <td width='60'  align='center' class='header_laporan'>Tgl Ver RAB</td>
	 ...
	 <td class='isi_laporan'>$row->verrab</td>
	 <td class='isi_laporan'>$row->tgl_verrab</td>
	  <td class='isi_laporan'>$row->nama_jenis</td>
	  <td class='isi_laporan' align='center'>$row->status_verrab</td>
