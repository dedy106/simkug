...
class server_report_saku3_tu_pajak_rptHonorPjk2 extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$periode=$tmp[0];
		$nama_cab=$tmp[1];
		$sql="select bulan,tahun,keterangan,form,nama_rek,alamat,npwp,sts_pjk,wni,wni2,jenis_sdm,kode_pajak,sum(pajak) as pajak,sum(nilai) as nilai,sum(netto) as netto 
		from(
			select substring(b.periode,5,2) as bulan,substring(b.periode,1,4) as tahun,c.keterangan, a.form,
			case when a.form='NONPEG' then c.berita else c.nama_rek end as nama_rek, sum(c.pajak) as pajak,d.alamat,c.npwp,e.nama as sts_pjk, 
			case when d.sts_wni='wni' then upper('N') else upper('Y') end as wni, 
			case when c.kode_pajak='5A' then upper('Y') else upper('N') end as wni2, c.kode_pajak,
			...
			$this->filter
			GROUP BY c.kode_pajak,e.nama,b.periode,d.sts_wni,d.alamat,c.npwp,c.keterangan,c.berita,a.form,c.nama_rek,d.flag_dosen 
		) a
		group by bulan,tahun,keterangan,form,nama_rek,alamat,npwp,sts_pjk,wni,wni2,jenis_sdm,kode_pajak
