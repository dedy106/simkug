...
class server_report_saku3_tu_bmhd_rptBuktiJurnalHU extends server_report_basic
{
	function getTotalPage()
	{
		global $dbLib;
		$sql="select count(a.no_bmhd) as jum
from bmhd_m a
left join karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi
left join karyawan c on a.nik_app=c.nik and a.kode_lokasi=c.kode_lokasi ".$this->filter2;
	...
	function getHtml()
	{
		...
		global $dbLib;
		$sql="select distinct a.no_bmhd,a.no_dokumen,a.kode_lokasi,a.periode,a.tanggal,date_format(a.tanggal,'%d/%m/%Y') as tanggal1,a.keterangan,a.kode_lokasi,
       a.nik_buat,b.nama as nama_buat,b.jabatan as jabatan_buat,a.nik_app,c.nama as nama_setuju,c.jabatan as jabatan_setuju, d.kota,a.kode_vendor 
			from bmhd_m a
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			echo "<table border='0' cellspacing='1' cellpadding='2' class='kotak'>
			  <tr>
				<td align='center' class='header_laporan'>MEMO JURNAL AKRU BMHD </td>
			  </tr>
			  <tr>
				<td><table border='0' cellspacing='2' cellpadding='1'>
				  ...
				  <tr>
					...
					<td width='496' class='header_laporan'>:&nbsp;$row->no_bmhd</td>
					</tr>
				  <tr>
				  ...
				  <tr>
					<td class='header_laporan'>Kode Vendor</td>
	  ...
	  $sql1="select a.kode_akun,b.nama,a.keterangan,a.kode_pp,a.kode_drk,case dc when 'D' then nilai else 0 end as debet,case dc when 'C' then nilai else 0 end as kredit  
			...
			where a.no_bmhd='$row->no_bmhd' and a.kode_lokasi='$row->kode_lokasi'
			order by a.no_urut ";
		//error_log($sql1);
  ...
  <tr>
	...
	function doPrintHeader($sender, $page){		
		$wd = array(20,40,$sender->w - $sender->rMargin - 156,13,13,25,25);//dalam mm		
		$w = 0;
		foreach ($wd as $value) $w+=$value;
		$sender->SetX(20);		
		$sender->SetFont('Arial','B',$sender->titleSize[0]);
		$sender->Cell($w,10,"MEMO JURNAL AKRU BMHD",'LTR',1,'C');			
		$w = $sender->w - $sender->rMargin;
		$sender->SetX(20);		
		$sender->SetFont('Arial','B', $sender->headerSize);
		$sender->Cell(10,5,"",'L',0,'C');$sender->Cell(30,5,"No Bukti",0,0,'l');$sender->Cell(5,5,":",0,0,'C');$sender->Cell($w-  65,5,$sender->row->no_bmhd,'R',1,'L');		
		$sender->SetX(20);$sender->Cell(10,5,"",'L',0,'C');$sender->Cell(30,5,"No Dokumen",0,0,'l');$sender->Cell(5,5,":",0,0,'C');$sender->Cell($w - 65,5,$sender->row->no_dokumen,'R',1,'L');
		$sender->SetX(20);$sender->Cell(10,5,"",'L',0,'C');$sender->Cell(30,5,"Periode",0,0,'l');$sender->Cell(5,5,":",0,0,'C');$sender->Cell($w - 65,5,$sender->row->periode,'R',1,'l');
		$sender->SetX(20);$sender->Cell(10,5,"",'L',0,'C');$sender->Cell(30,5,"Tanggal",0,0,'l');$sender->Cell(5,5,":",0,0,'C');$sender->Cell($w- 65,5,$sender->row->tanggal1,'R',1,'l');
		$sender->SetX(20);$sender->Cell(10,5,"",'L',0,'C');$sender->Cell(30,5,"Keterangan",0,0,'l');$sender->Cell(5,5,":",0,0,'C');$sender->Cell($w- 65,5,$sender->row->keterangan,'R',1,'l');
		...
		$sender->SetFont('Arial','B', $sender->headerSize);
		//Header				
		$w= array(20,40,$w - 156,13,13,25,25);//dalam mm		
	...
	{		
		...
		$AddOnLib=new server_util_AddOnLib();
		$title = array($this->lokasi,"Laporan Bukti Jurnal TAK");
		$titleHeight= array(5);//tinggi row per baris judul laporan dalam mm
		$titleSize = array(8);//fontSize judul
		$header = array("Kode Akun","Nama","Keterangan","PP","DRK","Debet","Kredit");
		...
		$pdf = new server_util_PdfLib($title, $titleHeight, $titleSize, "P", "A4", 8, $header,8, 8, $headerWidth, 10);		
		$pdf->AliasNbPages();			
		...
		$start = (($this->page-1) * $this->rows);
		$rs=$dbLib->LimitQuery($sql,$this->rows,$start);	
		$namafile = "Bukti-Jurnal.pdf";				
		$defHeight = 5;
		$rowHeight = 5;
		$fill = false;		
		$ix = 1;
		$tmp = "";
		$w= array(20,40,$pdf->w - $pdf->rMargin - 156,13,13,25,25);//dalam mm
		while ($row = $rs->FetchNextObject($toupper=false)){
			$pdf->row = $row;			
			$sql1="select a.kode_akun,b.nama,a.keterangan,a.kode_pp,a.kode_drk,case dc when 'D' then nilai else 0 end as debet,case dc when 'C' then nilai else 0 end as kredit  
				from bmhd_j a
			...
			{							
				...
				$numrowNm = $pdf->WordWrap($row1->nama,$w[1] - 2);
				$numrowKet = $pdf->WordWrap($row1->keterangan, $w[2] - 2);
				...
				$pdf->Cell($w[0],$rowHeight,$row1->kode_akun,1,0,'L',$fill);
				if ($y1 != $pdf->GetY()) $y1 = $pdf->GetY();
				$x = $pdf->GetX();
				$y = $pdf->GetY();
				$pdf->Cell($w[1],$rowHeight,'',1,0,'L',$fill);//buat kotak dulu
				$pdf->SetXY($pdf->GetX() - $w[1],$y);
				$pdf->MultiCell($w[1],$defHeight,$row1->nama,0);														
				$y2 = $pdf->GetY();
				$y = $y2 - $y1;			
				...
				$pdf->MultiCell($w[2],$defHeight,$row1->keterangan,0);
				$y2 = $pdf->GetY();
				$y = $y2 - $y1;			
				$pdf->SetXY(20 + $w[0] + $w[1] + $w[2],  $pdf->GetY() - $y);											
				
				$pdf->Cell($w[3],$rowHeight,$row1->kode_pp,1,0,'C',$fill);																	
				$pdf->Cell($w[4],$rowHeight,$row1->kode_drk,1,0,'C',$fill);														
				$pdf->Cell($w[5],$rowHeight,number_format($row1->debet,0,',','.'),1,0,'R',$fill);
				$pdf->Cell($w[6],$rowHeight,number_format($row1->kredit,0,',','.'),1,0,'R',$fill);				
				$pdf->Ln();
				$ix++;				
				$fill=false;					
			}
			...
			$pdf->Cell($w[0]+$w[1]+$w[2]+$w[3]+$w[4],$defHeight,"Total",1,0,'R',false);		
			$pdf->Cell($w[5],$defHeight,number_format($tot_debet,0,',','.'),1,0,'R',false);
			$pdf->Cell($w[6],$defHeight,number_format($tot_kredit,0,',','.'),1,1,'R',false);			
			$y = $pdf->GetY();
