...
class server_report_saku3_tu_bmhd_rptJurnalHU extends server_report_basic
{
	function getTotalPage()
	{
		...
		$sql="select count(a.no_bmhd) as jum ". 
		         "from bmhd_m a ".
				 "inner join bmhd_j b on a.no_bmhd=b.no_bmhd and a.kode_lokasi=b.kode_lokasi ".
	...
	function getHtml()
	{
		...
		$urut=$tmp[1];
		$periode=$tmp[2];
		$sql="select a.no_bmhd as no_bukti,a.no_dokumen,c.nama,date_format(b.tanggal,'%d/%m/%Y') as tanggal,b.kode_akun,b.kode_pp,b.kode_drk,a.posted,b.keterangan,case when b.dc='D' then b.nilai else 0 end as debet,case when b.dc='C' then b.nilai else 0 end as kredit ". 
		         "from bmhd_m a ".
				 "inner join bmhd_j b on a.no_bmhd=b.no_bmhd and a.kode_lokasi=b.kode_lokasi ".
			     "inner join masakun c on b.kode_akun=c.kode_akun and b.kode_lokasi=c.kode_lokasi ".$filter.
				 "order by $urut ";
		...
		echo $AddOnLib->judul_laporan("LAPORAN TRANSAKSI JURNAL AKRU BMHD",$this->lokasi,"Periode ".$AddOnLib->ubah_periode($periode));
		echo "<table border='1' cellspacing='0' cellpadding='2' class='kotak'>
  <tr bgcolor='#CCCCCC'>
    <td width='20'  class='header_laporan'><div align='center'>No</div></td>
    <td width='100' class='header_laporan'><div align='center'>No Bukti</div></td>
  ...
  <td width='30' class='header_laporan'><div align='center'>Posted</div></td>
	...
	{						
		...
		$AddOnLib=new server_util_AddOnLib();						
		$sql="select a.no_bmhd as no_bukti,a.no_dokumen,c.nama,date_format(b.tanggal,'%d/%m/%Y') as tanggal,b.kode_akun,b.kode_pp,b.kode_drk,a.posted,b.keterangan,case when b.dc='D' then b.nilai else 0 end as debet,case when b.dc='C' then b.nilai else 0 end as kredit, a.kode_lokasi ". 
		         "from bmhd_m a ".
				 "inner join bmhd_j b on a.no_bmhd=b.no_bmhd and a.kode_lokasi=b.kode_lokasi ".
			     "inner join masakun c on b.kode_akun=c.kode_akun and b.kode_lokasi=c.kode_lokasi ".$filter.
				 "order by $urut ";
