...
class server_report_saku3_tu_rptProyekSaldo2 extends server_report_basic
{
...
$this->filter order by a.kode_proyek";
		...
		echo $AddOnLib->judul_laporan("saldo proyek baru",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
   ...
   <tr bgcolor='#CCCCCC'>
	 ...
	 <td width='90'  align='center' class='header_laporan'>% TOTAL BEBAN THD TOTAL PENDAPATAN </td>
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			if ($row->pdpt <>0)
			{
				$persen=(($row->beban+$row->bmhd+$row->bdd)/$row->pdpt)*100;
			}
			if ($row->nilai <>0)
			{
				$persen2=(($row->beban+$row->bmhd+$row->bdd)/$row->nilai)*100;
			}
	...
	<td class='isi_laporan' align='right'>".number_format($row->beban+$row->bmhd+$row->bdd,0,",",".")."</td>
	<td class='isi_laporan' align='center'>".number_format($persen,2,",",".")."</td>
	<td class='isi_laporan' align='right'>".number_format($row->nilai-($row->beban+$row->bmhd+$row->bdd),0,",",".")."</td>
	...
	<td class='isi_laporan' align='right'>".number_format($beban+$bymhd+$bdd,0,",",".")."</td>
	<td class='isi_laporan' align='center'>&nbsp;</td>
	<td class='isi_laporan' align='right'>".number_format($nilai-($beban+$bmhd+$bdd),0,",",".")."</td>
	<td class='isi_laporan' align='center'>&nbsp;</td>
     </tr>";
		echo "</table><br>";
		echo "</div>";
