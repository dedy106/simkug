...
class server_report_saku3_tu_kegiatan_rptTakPos extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$tmp=explode("/",$this->filter2);
		$periode=$tmp[0];
		$sql="select a.no_kirim,a.modul,a.kode_lokasi,a.kode_lokasi,a.keterangan,a.nilai,date_format(a.tanggal,'%d/%m/%Y') as tgl_kirim,a.kode_loktuj,
b.no_terima,date_format(b.tanggal,'%d/%m/%Y')  as tgl_terima,d.nama as nama_lokasi,e.nama as nama_loktuj,a.kode_lokasi 
from takkirim_m a
...
";
		...
		echo $AddOnLib->judul_laporan("posisi tak",$this->lokasi,"Periode ".$AddOnLib->ubah_periode($periode));
   ...
   <tr bgcolor='#CCCCCC'>
     ...
     <td class='isi_laporan'>";
		...
		echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenKirim('$row->no_kirim','$row->kode_lokasi');\">$row->no_kirim</a>";
