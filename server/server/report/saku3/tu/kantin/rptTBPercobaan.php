...
class server_report_saku3_tu_kantin_rptTBPercobaan extends server_report_basic
{
	function getSumPage()
	{
		global $dbLib;
  ...
  <tr>
	...
	function doPrintHeader($sender, $page){
		...
		if ($page == 1){
			...
			$sender->Cell(15,10,"Kode",'LTR',0,'C',false);			
			$sender->Cell($w - 130,10,"Nama",'LTR',0,'C',false);		
			$sender->Cell(15,10,"Lokasi",'LTR',0,'C',false);			
			$sender->Cell(20,10,"Saldo Awal",'LTR',0,'C',false);			
			$y = $sender->GetY();
			$sender->Cell(40,5,"Mutasi",1,1,'C',false);
			$sender->SetXY(($w - 130)+70,$sender->GetY());
			$sender->Cell(20,5,"Debet",1,0,'C',false);			
			$sender->Cell(20,5,"Kredit",1,0,'C',false);						
			$sender->SetXY(($w - 130)+110,$y);
			$sender->Cell(20,10,"Saldo Akhir",'LTR',1,'C',false);						
		}else {
			$sender->SetFillColor(128,128,128);				
			$sender->SetLineWidth(.3);
			$sender->SetFont('Arial','B', $sender->headerSize);
	...
	{				
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			$y1 = $pdf->GetY();											
			$numrow = $pdf->WordWrap($row->nama,$w[2] - 2);			
			...
			$pdf->MultiCell($w[2],$defHeight,$row->nama,1);														
			$y2 = $pdf->GetY();
			$y = $y2 - $y1;			
