...
class server_report_saku3_tu_rptProyekSaldo extends server_report_basic
{
	...
	function getHtml()
	{
	   ...
	   isnull(h.nilai,0) as bdd,isnull(i.nilai,0) as bymhd,a.p_or,a.nilai_or
from tu_proyek a
...
left join (select b.kode_proyek,a.kode_lokasi,sum(case a.dc when 'C' then a.nilai else -a.nilai end) as nilai 
		   ...
		   where a.kode_lokasi='$kode_lokasi' and a.jenis='PIU' and a.periode<='$periode2'
...
left join (select kode_proyek,kode_lokasi,sum(nilai) as nilai
		   from tu_prbdd_d
		   where kode_lokasi='$kode_lokasi' and dc='D' and modul in ('AJUBDD','JCOST') and periode<='$periode2'
...
left join (select a.kode_proyek,a.kode_lokasi,sum(case when a.dc='D' then a.nilai else -a.nilai end) as nilai
		   ...
		   where a.kode_lokasi='$kode_lokasi' and a.modul in ('REVBDD') and a.periode<='$periode2'
		   ...
		   from tu_prbdd_d a
		   inner join it_aju_m b on a.no_bukti=b.no_aju and a.kode_lokasi=b.kode_lokasi
		   where a.kode_lokasi='$kode_lokasi' and a.modul in ('REVBMHD') and a.periode<='$periode2'
		   group by a.kode_proyek,a.kode_lokasi
   ...
   <tr bgcolor='#CCCCCC'>
	 ...
	 <td width='90'  align='center' class='header_laporan'>% TOTAL BEBAN THD TOTAL PENDAPATAN </td>
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			$kas+=$row->kas;
			$bmhd+=$row->bmhd;
			$saldo_proyek+=$row->nilai_or-($row->bdd+$row->bmhd);
			...
			if ($row->pdpt <>0)
			{
				$persen=(($row->bmhd+$row->bdd)/$row->pdpt)*100;
			}
			if ($row->nilai <>0)
			{
				$persen2=(($row->bmhd+$row->bdd)/$row->nilai)*100;
			}
...
<td class='isi_laporan'>$row->no_pks</td>
	...
	<td class='isi_laporan' align='right'>".number_format($row->bmhd+$row->bdd,0,",",".")."</td>
	<td class='isi_laporan' align='center'>".number_format($persen,2,",",".")."</td>
	<td class='isi_laporan' align='right'>".number_format($row->nilai-($row->bmhd+$row->bdd),0,",",".")."</td>
	...
	<td class='isi_laporan' align='right'>".number_format($bymhd+$bdd,0,",",".")."</td>
	<td class='isi_laporan' align='center'>&nbsp;</td>
	<td class='isi_laporan' align='right'>".number_format($nilai-($bmhd+$bdd),0,",",".")."</td>
	<td class='isi_laporan' align='center'>&nbsp;</td>
     </tr>";
		echo "</table><br>";
		echo "</div>";
