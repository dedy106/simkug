...
class server_report_saku3_tu_rptProyekKartu extends server_report_basic
{
...
$this->filter order by a.kode_proyek";
  ...
  <tr bgcolor='#CCCCCC'>
	...
	<td width='90' class='header_laporan' align='center'>Piutang</td>
	<td width='90' class='header_laporan' align='center'>Rekon Pelunasan</td>
	<td width='90' class='header_laporan' align='center'>Pengakuan Pendapatan</td>
	...
	<td width='90' class='header_laporan' align='center'>BYMHD</td>
			...
			case a.modul when 'PRPIU' then a.nilai else 0 end as piutang,
			case a.modul when 'GENPYT' then a.nilai else 0 end as pdpt,
			case a.modul when 'PIUPRO' then a.nilai else 0 end as rekon,
			case when a.modul in ('REVBDD')  then a.nilai else 0 end as beban,
			case a.modul when 'REVBMHD' then -a.nilai else 0 end as bymhd,
			case a.modul when 'JCOSTINI' then a.nilai else 0 end as join2
from (
select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai,a.tanggal,a.modul
from tu_prpiutang_m a
where a.kode_lokasi='$kode_lokasi' and a.no_dokumen='$row->kode_proyek'
union all
select a.no_bukti,convert(varchar,b.tanggal,103) as tgl,b.keterangan,a.nilai,b.tanggal,b.modul
from tu_prpyt_d  a
inner join tu_prpyt_m b on a.no_bukti=b.no_bukti and a.kode_lokasi=b.kode_lokasi
where a.kode_lokasi='$kode_lokasi' and a.kode_proyek='$row->kode_proyek'
union all
select b.no_bill as no_bukti,convert(varchar,b.tanggal,103) as tgl,b.keterangan,case when a.dc='D' then a.nilai else -a.nilai end as nilai,b.tanggal,b.modul
from ju_j a
inner join tu_prbill_m b on a.no_dokumen=b.no_bill and a.kode_lokasi=b.kode_lokasi
where a.kode_lokasi='$kode_lokasi' and a.jenis='PIU' and a.kode_proyek='$row->kode_proyek'
union all
select a.no_ju as no_bukti,convert(varchar,b.tanggal,103) as tgl,a.keterangan,case when a.dc='C' then a.nilai else -a.nilai end as nilai,b.tanggal,a.modul
from ju_j a
inner join tu_prbill_m b on a.no_dokumen=b.no_bill and a.kode_lokasi=b.kode_lokasi
where a.kode_lokasi='$kode_lokasi' and a.Modul='PIUPRO' and b.kode_proyek='$row->kode_proyek'
union all
...
where a.kode_lokasi='$kode_lokasi' and a.modul in ('REVBDD') and a.kode_proyek='$row->kode_proyek'
union all	 
select a.no_bukti,convert(varchar,b.tanggal,103) as tgl,b.keterangan,a.nilai,b.tanggal,a.modul
from tu_prbdd_d a
inner join tu_prbeban_m b on a.no_bukti=b.no_bukti and a.kode_lokasi=b.kode_lokasi
...
from tu_prbdd_d a
inner join tu_prbeban_m b on a.no_bukti=b.no_bukti and a.kode_lokasi=b.kode_lokasi
where a.kode_lokasi='$kode_lokasi' and a.dc='D' and b.modul='JCOSTINI' and a.kode_proyek='$row->kode_proyek'
	)a 
order by a.tanggal asc
 ...
 <td height='23'  class='isi_laporan'>".$row1->tgl."</td>
    ...
    <td class='isi_laporan' align='right'>".number_format($row1->piutang,0,',','.')."</td>
	<td class='isi_laporan' align='right'>".number_format($row1->rekon,0,',','.')."</td>
	<td class='isi_laporan' align='right'>".number_format($row1->pdpt,0,',','.')."</td>
