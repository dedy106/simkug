...
class server_report_saku3_spm_rptDwLabaRugiRegionalPpJejer extends server_report_basic
{
	...
	function getHtml()
	{
		...
		echo $AddOnLib->judul_laporan("LAPORAN REKAP ANGGARAN LABARUGI JEJER unit",$this->lokasi,"PERIODE ".$AddOnLib->ubah_periode($periode));
  ...
  <tr bgcolor='#CCCCCC'>
    <td  align='center' width='300' class='header_laporan'>Deskripsi</td>";
		$sql="select a.kode_pp,a.nama from pp a where a.kode_lokasi='$kode_lokasi' and a.kode_regional='$kode_regional' order by a.kode_pp";
		...
		while ($row = $rs->FetchNextObject($toupper=false))
		{
			...
			$jum=$jum+1;
			$tmp_sql.="sum(case when a.kode_pp='$row->kode_pp' then (case when a.jenis_akun='Pendapatan' then -a.n4 else a.n4 end) else 0 end) as 'n4$row->kode_pp',";
			$tmp_sql2.="sum(case when d.kode_pp='$row->kode_pp' then (case when c.jenis='Pendapatan' then -a.so_akhir else a.so_akhir  end) else 0 end) as 'n4$row->kode_pp',";
    ...
    echo "<td  align='center' class='header_laporan'>$row->kode_pp - $row->nama</td>";
		...
		$pdpt[$jum]="n4t";
		$tmp_sql.="sum(case when a.jenis_akun='Pendapatan' then -a.n4 else a.n4 end) as 'n4t',";
...
where a.kode_lokasi='$kode_lokasi' and a.periode='$periode' and a.kode_neraca='$row->kode_neraca' and a.kode_fs='$kode_fs' and b.kode_regional='$kode_regional'
group by a.kode_neraca";
  ...
  $content .= "</tr>
  <tr class='info'>";
		$sql="select nama from pp where kode_lokasi='$kode_lokasi' order by kode_pp";
		$rs = $dbLib->execute($sql);
		while ($row = $rs->FetchNextObject($toupper=false))
		{
