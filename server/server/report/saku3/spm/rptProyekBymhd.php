...
class server_report_saku3_spm_rptProyekBymhd extends server_report_basic
{
...
order by a.kode_proyek ";
  ...
  <tr bgcolor='#CCCCCC'>
    ...
    <td  width='100' class='header_laporan' align='center'>BYMHD </td>
	<td  width='100' class='header_laporan' align='center'>Reverse</td>	
    
	";
			
	$sql="select a.no_pb,a.keterangan,convert(varchar,a.tanggal,103) as tgl,a.kode_pp,case modul when 'REVPB' then 0 else a.nilai end as bymhd,case modul when 'REVPB' then a.nilai else 0 end as rev
	from yk_pb_m a
	...
	order by a.tanggal";
			...
			while ($row1 = $rs1->FetchNextObject($toupper=false))
			{
				...
				$rev+=$row1->rev;
				echo "<tr>
	<td class='isi_laporan'><a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenJurnal('$row1->no_pb','$row->kode_lokasi');\">$row1->no_pb</a></td>
