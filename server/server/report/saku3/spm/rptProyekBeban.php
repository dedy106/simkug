...
class server_report_saku3_spm_rptProyekBeban extends server_report_basic
{
...
order by a.kode_proyek ";
  ...
  <tr bgcolor='#CCCCCC'>
	...
	$sql="select a.no_reklas,b.keterangan,convert(varchar,b.tanggal,103) as tgl,b.kode_pp,
	   case when a.dc='D'then a.nilai else 0 end as bdd,
	   case when a.dc='C'then a.nilai else 0 end as rev
from spm_proyek_reklas_d a
inner join spm_proyek_reklas_m b on a.no_reklas=b.no_reklas and a.kode_lokasi=b.kode_lokasi
...
order by b.tanggal";
			...
			while ($row1 = $rs1->FetchNextObject($toupper=false))
			{
				...
				$rev+=$row1->rev;
				echo "<tr>
	<td class='isi_laporan'><a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenJurnal('$row1->no_reklas','$row->kode_lokasi');\">$row1->no_reklas</a></td>
