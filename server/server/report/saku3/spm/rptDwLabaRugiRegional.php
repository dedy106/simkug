...
class server_report_saku3_spm_rptDwLabaRugiRegional extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$kode_regional=$tmp[3];
		$jenis=$tmp[4];
		$nama_file="labarugi_regional_".$periode.".xls";
		...
		$sql="select a.kode_regional,a.nama from regional a $kode_regional order by a.kode_regional";
		$rs2 = $dbLib->execute($sql);
		while ($row2 = $rs2->FetchNextObject($toupper=false))
		{
			$nama=$row2->nama;
			$sql = "select a.kode_neraca,a.kode_fs,a.kode_lokasi,a.nama,a.tipe,a.level_spasi,a.kode_regional,
				...
				from exs_neraca_regional a
				$this->filter and a.modul='L' and a.kode_regional='$row2->kode_regional'
				order by a.rowindex ";
			$rs = $dbLib->execute($sql);		
			...
			echo $AddOnLib->judul_laporan("laporan laba rugi regional"."<br>".$nama,$this->lokasi,"Periode ".$AddOnLib->ubah_periode($periode));
		...
		";
		  ...
		  <td height='20' class='isi_laporan'>";
			...
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_lokasi','$periode','$row->kode_neraca','$row->kode_regional');\">".number_format($row->n4,0,',','.')."</a>";
				echo "</td>";
				}
				else
				{
				...
				if ($bentuk=="Detail" && $row->tipe=="Posting")
				{
					...
					$sql1="	select a.kode_akun,c.nama,
							...
							where b.kode_fs='$kode_fs' and a.kode_lokasi='$kode_lokasi' and b.kode_neraca='$kode_neraca' and a.periode='$periode' and d.kode_regional='$row2->kode_regional'
	...
	function getHtml2()
	{
		...
		$sql="select a.kode_regional,a.nama from exs_regional a $kode_regional order by a.kode_regional";
		$rs2 = $dbLib->execute($sql);
		while ($row2 = $rs2->FetchNextObject($toupper=false))
		{
			$nama=$row2->nama;
			...
			$content .= $AddOnLib->judul_laporan("laporan laba rugi regional"."<br>".$nama,$this->lokasi,"Periode ".$AddOnLib->ubah_periode($periode));
