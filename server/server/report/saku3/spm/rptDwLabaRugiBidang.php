...
class server_report_saku3_spm_rptDwLabaRugiBidang extends server_report_basic
{
	...
	function getHtml()
	{
		...
		$sql="select a.kode_bidang,a.nama from bidang a $kode_bidang and kode_lokasi='$kode_lokasi' order by a.kode_bidang";
		...
		while ($row2 = $rs2->FetchNextObject($toupper=false))
		{
			...
			$sql = "select a.kode_neraca,a.kode_fs,a.kode_lokasi,a.nama,a.tipe,a.level_spasi,a.kode_bidang,
				...
				$this->filter and a.modul='L' and a.kode_bidang='$row2->kode_bidang' and a.kode_fs='$kode_fs'
				order by a.rowindex ";
			$rs = $dbLib->execute($sql);		
		...
		";
		  ...
		  <td height='20' class='isi_laporan'>";
				...
				echo $row->nama;
				echo "</td>";
				if ($row->kode_neraca!="OR" && $row->kode_neraca!="OL")
				{
			...
			echo "<a style='cursor:pointer;color:blue' onclick=\"window.parent.system.getResource(".$resource.").doOpenTb('$row->kode_lokasi','$periode','$row->kode_neraca','$row->kode_bidang');\">".number_format($row->n4,0,',','.')."</a>";
				...
				{
				echo "<td class='isi_laporan' align='center'>".number_format($row->n4,2,',','.')."</td>";
				}
