...
class server_report_fa_rptFaKlaim
{
	protected $caption;
	protected $filter;
	protected $filter2;
...
inner join karyawan c on a.nik_setuju=c.nik and a.kode_lokasi=c.kode_lokasi ".$this->filter;
	...
	function getHtml()
	{
		...
		$tmp=explode("/",$this->filter2);
		$tahun=substr($tmp[0],0,4);
		$sql="select a.no_klaim,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.kode_vendor,a.periode,a.kode_lokasi,a.keterangan,a.no_dokumen,a.nik_buat,a.nik_setuju,a.kode_curr,a.kurs,
...
inner join vendor d on a.kode_vendor=d.kode_vendor and a.kode_lokasi=d.kode_lokasi
 ".$this->filter." order by a.no_klaim";
		...
		$html=$AddOnLib->judul_laporan("laporan klaim asuransi",$this->lokasi,$AddOnLib->ubah_periode($periode));
