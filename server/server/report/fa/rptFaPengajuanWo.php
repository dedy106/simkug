...
class server_report_fa_rptFaPengajuanWo
{
	protected $caption;
	protected $filter;
	protected $filter2;
...
inner join pp e on a.kode_pp=e.kode_pp and a.kode_lokasi=e.kode_lokasi ".$this->filter;
	...
	function getHtml()
	{
		...
		$tmp=explode("/",$this->filter2);
		$tahun=substr($tmp[0],0,4);
		$sql="select a.no_wo,a.tanggal,a.periode,a.kode_lokasi,a.keterangan,a.no_dokumen,a.nik_buat,a.nik_setuju,a.kode_curr,a.kurs,
       b.nama as nama_buat,c.nama as nama_setuju
from fawo_m a
inner join karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi
inner join karyawan c on a.nik_setuju=c.nik and a.kode_lokasi=c.kode_lokasi
 ".$this->filter." order by a.no_wo";
		error_log($sql);
		...
		$html=$AddOnLib->judul_laporan("laporan Pengajuan write off asset",$this->lokasi,$AddOnLib->ubah_periode($periode));
