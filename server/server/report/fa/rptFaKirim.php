...
class server_report_fa_rptFaKirim
{
	protected $caption;
	protected $filter;
	protected $filter2;
...
inner join pp e on a.kode_pp=e.kode_pp and a.kode_lokasi=e.kode_lokasi ".$this->filter;
	...
	function getHtml()
	{
		...
		$tmp=explode("/",$this->filter2);
		$tahun=substr($tmp[0],0,4);
		$sql="select a.no_kirim,a.kode_lokasi,a.periode,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.no_dokumen,a.keterangan,a.nik_buat,a.nik_setuju,a.kode_vendor,a.lokasi_tuj,a.lokfa_tuj,
...
inner join fa_lokasi f on a.lokfa_tuj=f.kode_lokfa and a.lokasi_tuj=f.kode_lokasi
 ".$this->filter." order by a.no_kirim";
		...
		$html=$AddOnLib->judul_laporan("laporan mutasi kirim asset",$this->lokasi,$AddOnLib->ubah_periode($periode));
  ...
  <tr>
    <td colspan='10' style='padding:5px'><table width='622' border='0' cellspacing='2' cellpadding='1'>
      ...
      <tr>
        ...
        <td class='header_laporan'>:&nbsp;$row->lokasi_tuj -&nbsp; $row->nama_lokasi_tuj</td>
      </tr>
      <tr>
        <td class='header_laporan'>Lokasi Asset </td>
        <td class='header_laporan'>:&nbsp;$row->lokfa_tuj -&nbsp; $row->nama_lokfa_tuj</td>
      </tr>
