...
class server_report_fa_rptFaAppWo
{
	protected $caption;
	protected $filter;
	protected $filter2;
...
inner join karyawan c on a.nik_setuju=c.nik and a.kode_lokasi=c.kode_lokasi
 ".$this->filter." order by a.no_woapp";
	...
	function getHtml()
	{
		...
		$tmp=explode("/",$this->filter2);
		$tahun=substr($tmp[0],0,4);
		$sql="select a.no_woapp,a.no_wo,a.tanggal,a.periode,a.kode_lokasi,a.keterangan,a.no_dokumen,a.nik_buat,a.nik_setuju,a.kode_curr,a.kurs,
...
inner join karyawan c on a.nik_setuju=c.nik and a.kode_lokasi=c.kode_lokasi
 ".$this->filter." order by a.no_woapp";
		...
		$html=$AddOnLib->judul_laporan("laporan approve write off asset",$this->lokasi,$AddOnLib->ubah_periode($periode));
  ...
  <tr>
    <td colspan='10' style='padding:5px'><table width='622' border='0' cellspacing='2' cellpadding='1'>
      ...
      <tr>
        <td class='header_laporan'>No Approve WO</td>
        <td class='header_laporan'>:&nbsp;$row->no_woapp</td>
        </tr>
	<tr>
  ...
  <tr bgcolor='#CCCCCC'>
    ...
    <td width='100' align='center' class='header_laporan'>Nilai residu</td>
	  ...
	  $sql1="select a.no_fa,b.barcode,b.nama,a.kode_pp,c.nama as nama_lokfa,date_format(b.tgl_perolehan,'%d/%m/%Y') as tgl_perolehan,a.nilai_buku,a.nilai,b.nilai_residu
from fawo_d a
...
where a.no_wo='$row->no_wo' and a.kode_lokasi='$row->kode_lokasi'
order by a.no_fa ";
		error_log($sql1);
