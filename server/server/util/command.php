...
function execCommand($cmd, &$retcode) {  
  ...
  if ($serverIsWindows && version_compare(PHP_VERSION, '5.3.0alpha') === -1) {
    $cmd = "\"$cmd\"";
  }
...
function runCommand($cmd, $path) {
  ...
  $resource = proc_open($c, $descriptorspec, $pipes, $path);
  $error = "";
  if (!is_resource($resource)) {
    return "<p>BADCMD: <code>".$cmd."</code></p>";
    exit;
  }
  ...
  if (!$err) {
    ...
    return"<p>BADCMD: <code>".$cmd."</code></p><p>$error</p>";
  }
}
