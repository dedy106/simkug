...
//-------------- error_log8
	...
	error_reporting (E_ALL & ~E_NOTICE & ~E_DEPRECATED & ~E_USER_DEPRECATED & ~E_STRICT & ~E_WARNING);
		...
		$sql="select kode_lokasi,kode_akun, kode_pp,kode_ba,round(nilai,0) as nilai,substring(periode,5,2) as bulan,substring(periode,1,4) as tahun 
		from ypt_gar_pp where kode_lokasi='$lokasi' and substring(periode,1,4)='$tahun' and (nilai<>0) order by kode_akun";
		if ($lokasi=="")
		{
			$sql="select kode_lokasi,kode_akun, kode_pp,kode_ba,round(nilai,0) as nilai,substring(periode,5,2) as bulan,substring(periode,1,4) as tahun 
		from ypt_gar_pp where substring(periode,1,4)='$tahun' and (nilai<>0) order by kode_akun";
		}
