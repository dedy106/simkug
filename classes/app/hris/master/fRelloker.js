window.app_hris_master_fRelloker = function(owner,lokasi)
{
	if (owner)
	{
		window.app_hris_master_fRelloker.prototype.parent.constructor.call(this, owner);
		window.app_hris_master_fRelloker.prototype.parent.setWidth.call(this, 800);
		window.app_hris_master_fRelloker.prototype.parent.setHeight.call(this, 400);
		
		this.centerize();
		
		this.className = "app_hris_master_fRelloker";
		
		this.mouseX = 0;
		this.mouseY = 0;					
		
		this.p1 = new panel(this,{bound:[0,20,this.width - 40,this.height - 100],caption:"Relasi Loker"});				
		
		uses("saiGrid");
		this.eSearch = new saiLabelEdit(this.p1,{bound:[5,20,245,20], caption:"Loker"});
		this.bCari = new button(this.p1, {bound:[250,20,80,20],caption:"Cari", click:[this,"doCariClick"]});
		this.sg1 = new saiGrid(this.p1,{bound:[5,33,345,277], colCount:2,colTitle:["Kode Loker","Nama"], 
				colWidth:[[1,0],[200,80]],dblClick:[this,"doDblClick"],
				readOnly:true
			});				
		this.sg = new saiGrid(this.p1,{bound:[400,33,345,300],
				colCount:2,colTitle:["Kode Loker","Nama"], 
				colWidth:[[1,0],[200,80]],dblClick:[this,"doDblClick"],
				readOnly:true		
			});				
...
window.app_hris_master_fRelloker.extend(window.commonForm);
window.app_hris_master_fRelloker.implement({
	...
	setWidth: function(data){
		window.app_hris_master_fRelloker.prototype.parent.setWidth.call(this, data);
		this.header.style.width = data - 23;
		this.rBg.style.left = data - 23;
	},
	doSysMouseMove: function(x, y, button, buttonState){
		window.app_hris_master_fRelloker.prototype.parent.doSysMouseMove.call(this,x, y, button, buttonState);
		if (this.isClick)
		{		
	...
	doClick: function(sender){
		try{
			...
			for (var i =0 ;i < this.sg.rows.getLength();i++)
			{
				...
				line = new arrayMap();
				line.set("kode_loker",this.sg.cells(0,i));
				value.set(i, line);						
			}		
	...
	loadNeraca: function(){			
		...
		if (this.listAkun != undefined)
			for (var i in this.listAkun.objList){
				if ( i > 0) filter += ",";
				filter += "'"+this.listAkun.get(i).get("kode_loker") +"'";
			}
		filter += ")";		
		var nrc = this.dblib.runSQL("select a.kode_loker, a.nama from gr_loker a "+				
					"where a.kode_lokasi = '"+this.elokasi+"' "+ (filter != "()"? " and a.kode_loker in "+filter:"and a.kode_loker in ('')")); 
	...
	doCariClick: function(sender){
		try{
			...
			filter += ")";		
			var nrc = this.dblib.getDataProvider("select a.kode_loker, a.nama  from gr_loker a "+				
						"where a.kode_lokasi = '"+this.elokasi+"'  "+
						" and (kode_loker like '"+this.eSearch.getText()+"%' or a.nama like '"+this.eSearch.getText()+"%' ) "+ 				
						(filter != "()"? "and kode_loker not in "+filter:"") +
						(this.allSelectAkun != ""? "and kode_loker not in ("+this.allSelectAkun+")":"") +" order by a.kode_loker",true);		
			this.dataNRC = nrc;
			if (typeof nrc != "string"){		
				...
				for (var i in nrc.rs.rows){		
					line = nrc.rs.rows[i];	
					this.sg1.appendData([line.kode_loker, line.nama]);		
				}
