window.app_hris_master_fKlpJab = function(owner)
{
	if (owner)
	{
		window.app_hris_master_fKlpJab.prototype.parent.constructor.call(this,owner);
		this.className  = "app_hris_master_fKlpJab";
		...
		this.p1 = new panel(this,{bound:[10,23,600,433],caption:"Daftar Kelompok Jabatan"});
...
window.app_hris_master_fKlpJab.extend(window.childForm);
window.app_hris_master_fKlpJab.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update gr_klpjab set nama = '"+this.e_nama.getText()+"' where kode_klpjab = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
		...
		try{
			if (this.cb_kode.getText() != ""){
				var data = this.dbLib.getDataProvider("select nama "+
				           " from gr_klpjab where kode_klpjab ='"+this.cb_kode.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ",true);
	...
	doTampilClick: function(sender){
		try{			
			var temp = this.dbLib.runSQL("select kode_klpjab,nama from gr_klpjab where kode_lokasi='"+this.app._lokasi+"' order by kode_klpjab");
		...
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Kelompok Jabatan",sender,undefined, 
											  ...
											  ["kode_klpjab","nama"],"and",["Kode","Nama"],false);				
			}
		}catch(e){
			systemAPI.alert(e);
		}
