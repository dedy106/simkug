window.app_hris_master_fAsur = function(owner)
{
	if (owner)
	{
		window.app_hris_master_fAsur.prototype.parent.constructor.call(this,owner);
		this.className  = "app_hris_master_fAsur";
		...
		this.p1 = new panel(this,{bound:[10,23,900,433],caption:"Daftar Asuransi"});
...
window.app_hris_master_fAsur.extend(window.childForm);
window.app_hris_master_fAsur.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into gr_asur(kode_asur,nama,kode_lokasi,alamat,kota,kodepos,no_telp,no_fax,npwp,pic,bank,cabang,no_rek,nama_rek) values "+
	...
	doTampilClick: function(sender){
		try{			
			var temp = this.dbLib.runSQL("select kode_asur,nama,alamat,kota,kodepos,no_telp,no_fax,npwp,pic,bank,cabang,no_rek,nama_rek from gr_asur where kode_lokasi='"+this.app._lokasi+"' order by kode_asur");
	...
	doBtnClick: function(sender, event){
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Asuransi",sender,undefined, 
											  "select kode_asur, nama  from gr_asur where kode_lokasi = '"+this.app._lokasi+"' ",
											  "select count(kode_asur) from gr_asur where kode_lokasi = '"+this.app._lokasi+"' ",
											  ["kode_asur","nama"],"and",["Kode","Nama"],false);				
			}
		}catch(e){
			systemAPI.alert(e);
		}
