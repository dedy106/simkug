window.app_hris_master_karyawan_fSkalaMgr = function(owner)
{
	if (owner)
	{
		window.app_hris_master_karyawan_fSkalaMgr.prototype.parent.constructor.call(this,owner);
		this.className  = "app_hris_master_karyawan_fSkalaMgr";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Skala Manager", 0);	
		...
		this.p1 = new panel(this,{bound:[10,23,600,233],caption:"Daftar Skala Manager"});
...
window.app_hris_master_karyawan_fSkalaMgr.extend(window.childForm);
window.app_hris_master_karyawan_fSkalaMgr.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try{									
					...
					sql.add("insert into gr_skala_mgr(kode_skala,kode_kategori,jml_min,jml_max,kode_lokasi) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update gr_skala_mgr set kode_kategori= '"+this.cb_kategori.getText()+"',jml_min="+parseNilai(this.e_min.getText())+",jml_max="+parseNilai(this.e_max.getText())+" where kode_skala = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from gr_skala_mgr where kode_skala = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");			
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
		...
		try{
			if (this.cb_kode.getText() != ""){
				var data = this.dbLib.getDataProvider("select a.kode_kategori,b.nama,a.jml_min,a.jml_max "+
				           " from gr_skala_mgr a inner join gr_kategori b on a.kode_kategori=b.kode_kategori and a.kode_lokasi=b.kode_lokasi "+
	...
	doBtnClick: function(sender, event){
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Skala Manager",sender,undefined, 
											  "select kode_skala, kode_kategori  from gr_skala_mgr where kode_lokasi = '"+this.app._lokasi+"' ",
											  "select count(kode_skala) from gr_skala_mgr where kode_lokasi = '"+this.app._lokasi+"' ",
