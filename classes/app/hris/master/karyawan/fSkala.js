window.app_hris_master_karyawan_fSkala = function(owner)
{
	if (owner)
	{
		window.app_hris_master_karyawan_fSkala.prototype.parent.constructor.call(this,owner);
		this.className  = "app_hris_master_karyawan_fSkala";
		...
		this.c_jenis = new saiCB(this,{bound:[20,13,180,20],caption:"Jenis Jab",readOnly:true,items:["MGR","NON"],tag:2,change:[this,"doChange"]}); 
		...
		this.bTampil = new button(this,{bound:[729,13,80,18],caption:"Lihat Data",click:[this,"doTampilClick"]});			
		
		this.p1 = new panel(this,{bound:[10,23,800,333],caption:"Daftar Skala"});
		this.sg1 = new saiGrid(this.p1,{bound:[0,20,795,280],tag:9,readOnly:true,
				colTitle: ["Kode Skala","Kd Ktg","Nama Kategori","Jenis","Nilai Min","Nilai Maks"]});		
		this.sgn = new sgNavigator(this.p1,{bound:[0,308,799,25],buttonStyle:3, grid:this.sg1, pager:[this,"doPager"]});		
...
window.app_hris_master_karyawan_fSkala.extend(window.childForm);
window.app_hris_master_karyawan_fSkala.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update gr_skala set kode_kategori= '"+this.cb_kategori.getText()+"',jml_min="+parseNilai(this.e_min.getText())+",jml_max="+parseNilai(this.e_max.getText())+" where jenis = '"+this.c_jenis.getText()+"' and kode_skala = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
	...
	doTampilClick: function(sender){
		try{			
			var temp = this.dbLib.runSQL("select a.kode_skala,a.kode_kategori,a.jenis,b.nama,a.jml_min,a.jml_max "+
			                             "from gr_skala a inner join gr_kategori b on a.kode_kategori=b.kode_kategori and a.kode_lokasi=b.kode_lokasi "+
										 "where a.kode_lokasi='"+this.app._lokasi+"' order by a.jenis,a.kode_skala");
	...
	doBtnClick: function(sender, event){
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Skala",sender,undefined,
											  "select distinct kode_skala, kode_kategori  from gr_skala where kode_lokasi = '"+this.app._lokasi+"'",
											  "select count(distinct kode_skala) 	     from gr_skala where kode_lokasi = '"+this.app._lokasi+"'",
