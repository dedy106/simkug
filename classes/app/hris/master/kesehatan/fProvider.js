window.app_hris_master_kesehatan_fProvider = function(owner)
{
	if (owner)
	{
		window.app_hris_master_kesehatan_fProvider.prototype.parent.constructor.call(this,owner);
		this.className  = "app_hris_master_kesehatan_fProvider";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Provider Layanan Kesehatan", 0);	
		...
		this.p1 = new panel(this,{bound:[10,23,900,433],caption:"Daftar Provider Layanan Kesehatan"});
...
window.app_hris_master_kesehatan_fProvider.extend(window.childForm);
window.app_hris_master_kesehatan_fProvider.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into gr_provider(kode_provider,nama,kode_lokasi,alamat,kota,kodepos,no_telp,no_fax,npwp,pic,bank,cabang,no_rek,nama_rek) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from gr_provider where kode_provider = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");			
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
		...
		try{
			if (this.cb_kode.getText() != ""){
				var data = this.dbLib.getDataProvider("select nama,alamat,kota,kodepos,no_telp,no_fax,npwp,pic,bank,cabang,no_rek,nama_rek "+
				           " from gr_provider where kode_provider ='"+this.cb_kode.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ",true);
	...
	doTampilClick: function(sender){
		try{			
			var temp = this.dbLib.runSQL("select kode_provider,nama,alamat,kota,kodepos,no_telp,no_fax,npwp,pic,bank,cabang,no_rek,nama_rek from gr_provider where kode_lokasi='"+this.app._lokasi+"' order by kode_provider");
	...
	doBtnClick: function(sender, event){
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Provider Kesehatan",sender,undefined, 
											  "select kode_provider, nama  from gr_provider where kode_lokasi = '"+this.app._lokasi+"' ",
											  "select count(kode_provider) from gr_provider where kode_lokasi = '"+this.app._lokasi+"' ",
