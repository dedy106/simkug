window.app_hris_master_kesehatan_fJenis = function(owner)
{
	if (owner)
	{
		window.app_hris_master_kesehatan_fJenis.prototype.parent.constructor.call(this,owner);
		this.className  = "app_hris_master_kesehatan_fJenis";
		...
		this.p1 = new panel(this,{bound:[10,23,600,433],caption:"Daftar Jenis Plafon Kesehatan dan Bantuan"});
...
window.app_hris_master_kesehatan_fJenis.extend(window.childForm);
window.app_hris_master_kesehatan_fJenis.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try{									
					...
					sql.add("insert into gr_kes_jenis(kode_jenis,nama,kode_lokasi) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update gr_kes_jenis set nama = '"+this.e_nama.getText()+"' where kode_jenis = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
	...
	doTampilClick: function(sender){
		try{			
			var temp = this.dbLib.runSQL("select kode_jenis,nama from gr_kes_jenis where kode_lokasi='"+this.app._lokasi+"' order by kode_jenis");
	...
	doBtnClick: function(sender, event){
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Jenis Palfon",sender,undefined, 
