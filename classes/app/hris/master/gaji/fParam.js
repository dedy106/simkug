window.app_hris_master_gaji_fParam = function(owner)
{
	if (owner)
	{
		window.app_hris_master_gaji_fParam.prototype.parent.constructor.call(this,owner);
		this.className  = "app_hris_master_gaji_fParam";
		...
		try {
			...
			var data = this.dbLib.getDataProvider(
						...
						"from gr_gaji_param a inner join gr_masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
...
window.app_hris_master_gaji_fParam.extend(window.childForm);
window.app_hris_master_gaji_fParam.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from gr_gaji_param where kode_lokasi='"+this.app._lokasi+"'");			
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into gr_gaji_param(kode_param,nama,kode_lokasi,jenis,kode_akun,dc,no_urut) values "+
			...
			switch(col){
				case 3 :
						this.standarLib.showListDataForSG(this, "Daftar Akun",this.sg, this.sg.row, this.sg.col, 
														"select kode_akun, nama  from gr_masakun where kode_lokasi='"+this.app._lokasi+"' and jenis='Beban' and block='0'",
														"select count(kode_akun) from gr_masakun where kode_lokasi='"+this.app._lokasi+"' and jenis='Beban' and block='0'",
