window.app_hris_master_gaji_fNormaFix = function(owner)
{
	if (owner)
	{
		window.app_hris_master_gaji_fNormaFix.prototype.parent.constructor.call(this,owner);
		this.className  = "app_hris_master_gaji_fNormaFix";
		...
		this.p1 = new panel(this,{bound:[10,23,500,433],caption:"Daftar Tarif Norma Gaji per Karyawan"});
		...
		try {
			...
			this.cb_kode.setSQL("select nik, nama, kode_grade from gr_karyawan where kode_lokasi='"+this.app._lokasi+"'",["nik","nama","kode_grade"],false,["Kode","Nama","Grade"],"and","Data Karyawan",true);
...
window.app_hris_master_gaji_fNormaFix.extend(window.childForm);
window.app_hris_master_gaji_fNormaFix.implement({
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into gr_gaji_nik(nik,kode_param,kode_lokasi,nilai) values "+
		...
		try{
			if (this.cb_kode.getText() != ""){
				...
				var data = this.dbLib.getDataProvider(
				            ...
				            "from  gr_gaji_nik a inner join gr_gaji_param b on a.kode_param=b.kode_param and a.kode_lokasi=b.kode_lokasi where a.nik='"+this.cb_kode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
