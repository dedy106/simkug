window.app_hris_master_adm_fUHarianKlp = function(owner)
{
	if (owner)
	{
		window.app_hris_master_adm_fUHarianKlp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_hris_master_adm_fUHarianKlp";
		...
		this.p1 = new panel(this,{bound:[10,23,700,433],caption:"Daftar Tarif Uang Harian per Grade"});
...
window.app_hris_master_adm_fUHarianKlp.extend(window.childForm);
window.app_hris_master_adm_fUHarianKlp.implement({
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into gr_spj_harian(sts_spj,kode_grade,kode_klpjab,kode_lokasi,nilai) values "+
										"	('"+this.cb_kode.getText()+"','"+this.sg.cells(0,i)+"','"+this.sg.cells(2,i)+"','"+this.app._lokasi+"',"+parseNilai(this.sg.cells(4,i))+")");
							}
						}
					}					
					//setTipeButton(tbAllFalse);
		...
		try{
			if (this.cb_kode.getText() != ""){
				var data = this.dbLib.getDataProvider(
				            "select a.kode_grade,b.nama,a.nilai,a.kode_klpjab,c.nama as nama_jab "+
