window.app_hris_transaksi_gaji_fGajiLoad = function(owner)
{
	if (owner)
	{
		window.app_hris_transaksi_gaji_fGajiLoad.prototype.parent.constructor.call(this, owner);
		this.className = "app_hris_transaksi_gaji_fGajiLoad";
		...
		this.p1 = new portalui_panel(this,{bound:[20,189,900,400],caption:"Data Variable Gaji Hasil Load"});
		this.sg1 = new portalui_saiGrid(this.p1,{bound:[0,20,900,350],colCount:6,
				colTitle:["NIK","Nama","Kode Param","Keterangan","DC","Nilai"],
...
window.app_hris_transaksi_gaji_fGajiLoad.extend(window.portalui_childForm);
window.app_hris_transaksi_gaji_fGajiLoad.implement({
	...
	selectPage: function(sender,page){
		...
		for (var i=start; i < finish;i++){
			...
			this.sg1.appendData([line.nik, line.nama, line.kode_param, line.keterangan, line.dc, parseFloat(line.nilai)]);
		}
			...
			case "simpan" :					
					if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
						try{
							...
							sql.add("delete from gr_gajiload_m where periode = '"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and flag_form='GAJI'");
							sql.add("delete from gr_gajiload_d where periode = '"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and flag_form='GAJI'");
							...
							for (var i in this.dataUpload.rows){
								...
								sql.add("insert into gr_gajiload_d(no_load,kode_lokasi,nik,kode_param,dc,nilai,periode,flag_form) values "+
									" ('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+line.nik+"','"+line.kode_param+"','"+line.dc+"',"+line.nilai+",'"+this.e_periode.getText()+"','GAJI')");
							}
