window.app_hris_transaksi_adm_fAbsenHari2 = function(owner)
{
	if (owner)
	{
		window.app_hris_transaksi_adm_fAbsenHari2.prototype.parent.constructor.call(this, owner);
		this.className = "app_hris_transaksi_adm_fAbsenHari2";
		...
		this.sg1 = new portalui_saiGrid(this.p1,{bound:[0,20,this.p1.width-5,this.p1.height-50],colCount:5,
				colTitle:["NIK","Nama","Tanggal","Jam Masuk","Jam Keluar"],				
...
window.app_hris_transaksi_adm_fAbsenHari2.extend(window.portalui_childForm);
window.app_hris_transaksi_adm_fAbsenHari2.implement({
	...
	selectPage: function(sender,page){
		...
		for (var i=start; i < finish;i++){
			...
			this.sg1.appendData([line.nik,line.nama,line.tanggal,line.jam_masuk,line.jam_keluar]);
		}
						...
						try{
							...
							for (var i=0; i < this.dataUpload.rows.length;i++){
								...
								sql.add("insert into gr_absen_harian_d(no_load,kode_lokasi,nik,tanggal,jam,jenis,modul,no_bukti) values "+
										"	('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+line.nik+"','"+line.tanggal+"','"+line.tanggal+" "+line.jam_masuk+"','I','LOAD', '"+this.e_nb.getText()+"')");
								sql.add("insert into gr_absen_harian_d(no_load,kode_lokasi,nik,tanggal,jam,jenis,modul,no_bukti) values "+
										"	('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+line.nik+"','"+line.tanggal+"','"+line.tanggal+" "+line.jam_keluar+"','O','LOAD', '"+this.e_nb.getText()+"')");
							}
