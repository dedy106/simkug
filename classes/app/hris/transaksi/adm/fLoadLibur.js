window.app_hris_transaksi_adm_fLoadLibur = function(owner)
{
	if (owner)
	{
		window.app_hris_transaksi_adm_fLoadLibur.prototype.parent.constructor.call(this, owner);
		this.className = "app_hris_transaksi_adm_fLoadLibur";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Data Hari Libur: Load", 0);	
		...
		this.p1 = new portalui_panel(this,{bound:[20,189,700,520],caption:"Data Hari Libur"});
		this.sg1 = new portalui_saiGrid(this.p1,{bound:[0,20,this.p1.width-5,this.p1.height-50],colCount:4,tag:0,
				colTitle:["Kode Libur","Keterangan","Tgl Mulai","Tgl Selesai"],
				colWidth:[[3,2,1,0],[80,80,230,80]],readOnly:true, defaultRow:1});		
...
window.app_hris_transaksi_adm_fLoadLibur.extend(window.portalui_childForm);
window.app_hris_transaksi_adm_fLoadLibur.implement({	
	...
	selectPage: function(sender,page){
		...
		for (var i=start; i < finish;i++){
			...
			this.sg1.appendData([line.kode_libur,line.keterangan,line.tgl_mulai,line.tgl_selesai]);
		}
			...
			case "simpan" :											
					if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
						try{								
							...
							sql.add("delete from gr_libur where tahun='"+this.e_tahun.getText()+"'");							
							var line;														
							for (var i=0; i < this.dataUpload.rows.length;i++){
								line = this.dataUpload.rows[i];												
								sql.add("insert into gr_libur(kode_libur,kode_lokasi,tahun,nama,tgl_mulai,tgl_akhir) values "+
										"('"+line.kode_libur+"','"+this.app._lokasi+"','"+this.e_tahun.getText()+"','"+line.keterangan+"','"+line.tgl_mulai+"','"+line.tgl_selesai+"')");
							}												
