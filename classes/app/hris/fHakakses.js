...
window.app_eclaim_admin_fHakakses.implement({
	...
	doEditChange: function(sender){
		...
		if (sender == this.e0)
		{		
			var pass = this.dbLib.execSQL("select * from eclaim_hakakses where kode_lokasi = '"+this.app._lokasi+"' and nik = '"+this.e0.getText()+"' and kode_ttg = '"+this.e5.getText()+"'",1,0);								
		...
		switch (event)
		{
			...
			case "simpan" :
				if (modalResult == mrOk){
					...
					var values = "";
					var sqlValue = "";
					var sql2 = "insert into eclaim_hakakses (nik, nama, pass, status_admin, kode_lokasi, kode_klp_menu, klp_akses,email, kode_ttg) values ";				
					values = "'"+this.e0.getText()+"','"+this.e1.getText()+"','"+this.e2.getText()+"','"+status+"','"+this.app._lokasi+"','"+this.e8.getText()+"','ADMIN','"+this.e9.getText()+"' ,'"+this.e5.getText()+"'";
					sqlValue = "(" + values +")";
					sql2 += sqlValue;
			...
			case "ubah" :
				if (modalResult == mrOk){
					...
					var values = "";
					var sqlValue = "";
					var sql2 = "insert into eclaim_hakakses (nik, nama, pass, status_admin, kode_ttg, kode_klp_menu, klp_akses,email, kode_lokasi) values ";
					
					values = "'"+this.e0.getText()+"','"+this.e1.getText()+"','"+this.e2.getText()+"','"+status+"','"+this.e5.getText()+"','"+this.e8.getText()+"','ADMIN','"+this.e9.getText()+"','"+this.app._lokasi+"'";
					sqlValue = "(" + values +")";
					sql2 += sqlValue;
					uses("server_util_arrayList");
					sql = new server_util_arrayList();
					sql.add("delete from eclaim_hakakses where nik = '"+this.e0.getText()+"'");
					sql.add(sql2);
					this.dbLib.execArraySQL(sql);		
				}			
				break;
