window.app_hris_report_rekrut_flJob = function(owner)
{
	if (owner)
	{
		window.app_hris_report_rekrut_flJob.prototype.parent.constructor.call(this,owner);
		this.className = "app_hris_report_rekrut_flJob";
		this.maximize();
...
window.app_hris_report_rekrut_flJob.extend(window.childForm);
window.app_hris_report_rekrut_flJob.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				var sql = "select a.kode_job,a.nama,a.keterangan,date_format(a.tgl_mulai,'%d/%m/%Y') as tgl_mulai,date_format(a.tgl_selesai,'%d/%m/%Y') as tgl_selesai "+
						  "from gr_rekrut_job a "+this.filter
						  "order by a.kode_job ";
			...
			systemAPI.alert("[app_hris_report_rekrut_flJob]::mainButtonClick:"+e);
		}
	},
