window.app_hris_report_adm_flApp = function(owner)
{
	if (owner)
	{
		window.app_hris_report_adm_flApp.prototype.parent.constructor.call(this,owner);
		this.className = "app_hris_report_adm_flApp";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Approval", 2);
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_app_m where kode_lokasi='"+this.app._lokasi+"'");
	if (this.app._userStatus=="A")
	{
		...
		this.gridLib.SGEditData(this.sg1,5,[0,1,2],["Modul","=","ABSEN"]);
		this.gridLib.SGEditData(this.sg1,6,[0,1,2],["No Approval","All",""])
	}
	else
	{
		this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Periode","=",this.periode]);
...
window.app_hris_report_adm_flApp.extend(window.portalui_childForm);
window.app_hris_report_adm_flApp.prototype.doEllipseClick= function(sender, col, row)
{
	...
	if (row == 4)
	{
		...
		this.filterRep.ListDataSGFilter(this, "Data Karyawan",this.sg1, this.sg1.row, this.sg1.col,
			...
			if (row == 6)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Daftar Approval",this.sg1, this.sg1.row, this.sg1.col,
													  "select a.no_app, a.keterangan from gr_app_m a "+
													  "inner join gr_karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+this.filter,
													  "select count(a.no_app) from gr_app_m a "+
													  "inner join gr_karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+this.filter,
													  ["a.no_app","a.keterangan"],"and",["No Approval","Keterangan"], false, this.sg1.cells(1,row) == "in");
			}
			if (row == 6)
			{
				this.filterRep.ListDataSGFilter(this, "Daftar Approval",this.sg1, this.sg1.row, this.sg1.col,
															  "select no_app, keterangan from gr_app_m where kode_lokasi = '"+this.app._lokasi+"' ",
															  "select count(no_app) from gr_app_m where kode_lokasi = '"+this.app._lokasi+"' ",
															  ["no_app","nama"],"and",["No Approval","Nama"], false, this.sg1.cells(1,row) == "in");
			}
	
};
window.app_hris_report_adm_flApp.prototype.doSelectCell = function(sender, col, row)
{
	if (this.app._userStatus=="A")
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123i","123i","123i","123i","123i","3","123i"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[0,2,2,2,2,0,2]);
	}
	else
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["3","3","3","3","3","3","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[0,3,3,3,3,0,2]);
	}
	if (row == 0)
	{
		this.sg1.columns.get(2).pickList.clear();
		this.dbLib.setItemsFromSQL("select distinct periode from gr_app_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList]);
	}
	if (row == 5)
	{
		this.sg1.columns.get(2).pickList.clear();
		this.dbLib.setItemsFromSQL("select distinct modul from gr_app_m where kode_lokasi='"+this.app._lokasi+"' order by modul",[this.sg1.columns.get(2).pickList]);
	}
};
window.app_hris_report_adm_flApp.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		{ 
			if (this.app._userStatus=="A")
			{
				...
				this.gridLib.SGEditData(this.sg1,6,[0,1,2],["No Ver","All",""])
			}
			else
			{
				this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Periode","=",this.periode]);
		...
		else
		{
			...
			this.filter = this.filterRep.filterStr("a.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
					...
					this.filterRep.filterStr("b.kode_dept",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
					this.filterRep.filterStr("b.nik",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
					this.filterRep.filterStr("a.modul",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+
					this.filterRep.filterStr("a.no_app",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and");
			this.showFilter = this.filterRep.showFilter(this.sg1);
			this.nama_report = "server_report_hris_rptApp";
			if (this.sg1.getCell(2,5)=="ABSEN") {this.nama_report = "server_report_hris_rptAppAbsen";}
			if (this.sg1.getCell(2,5)=="CUTI") {this.nama_report = "server_report_hris_rptAppCuti";}
			if (this.sg1.getCell(2,5)=="LEMBUR") {this.nama_report = "server_report_hris_rptAppLembur";}
			if (this.sg1.getCell(2,5)=="SURAT") {this.nama_report = "server_report_hris_rptAppSurat";}
			if (this.sg1.getCell(2,5)=="SPPD") {this.nama_report = "server_report_hris_rptAppSppd";}
			if (this.sg1.getCell(2,5)=="IJIN") {this.nama_report = "server_report_hris_rptAppIjin";}
	...
	{
		alert("[app_hris_report_adm_flApp]::mainButtonClick:"+e);
	}
};
window.app_hris_report_adm_flApp.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_hris_report_adm_flApp.prototype.getStringHeader = function(){
...
window.app_hris_report_adm_flApp.prototype.doConfirmClick = function(sender){
	try{
...
window.app_hris_report_adm_flApp.prototype.doSelectedPage = function(sender, page)
{
	this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,this.filter, this.page,  this.pager, this.showFilter, this.lokasi,this.filter2));
	this.page = page;
	this.allBtn = false;
};
window.app_hris_report_adm_flApp.prototype.previewReport = function(dthtml)
{
...
window.app_hris_report_adm_flApp.prototype.doCloseReportClick = function(sender)
{
...
window.app_hris_report_adm_flApp.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_hris_report_adm_flApp.prototype.doRowPerPageChange = function(sender, rowperpage)
{
