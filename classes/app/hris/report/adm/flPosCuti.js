window.app_hris_report_adm_flPosCuti = function(owner)
{
	if (owner)
	{
		window.app_hris_report_adm_flPosCuti.prototype.parent.constructor.call(this,owner);
		this.className = "app_hris_report_adm_flPosCuti";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Posisi Cuti", 2);
	...
	if (this.app._userStatus=="A")
	{
		...
		this.gridLib.SGEditData(this.sg1,6,[0,1,2],["No Cuti","All",""])
...
window.app_hris_report_adm_flPosCuti.extend(window.childForm);
window.app_hris_report_adm_flPosCuti.implement({
	...
	doSelectCell: function(sender, col, row){		
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123","3","3","3","3","3","3"]);
		...
		try
		{
			...
			else
			{
				...
				else
				{
					...
					this.filterRep.filterStr("b.nik",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+
					this.filterRep.filterStr("a.no_cuti",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and");	
					this.filter2 = this.sg1.getCell(2,0)+"/";
					this.showFilter = this.filterRep.showFilter(this.sg1);
					var result  = new arrayMap();		
					this.lokasi=this.app._namalokasi;
					this.nama_report = "server_report_hris_rptPosCuti";
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_cuti='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
	...
	doOpenVer: function(no_bukti,kode_lokasi){
		...
		filter2=this.app._namaUser;
		nama_report="server_report_hris_rptVerCuti";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenApp: function(no_bukti,kode_lokasi){
		...
		filter2=this.app._namaUser;
		nama_report="server_report_hris_rptAppCuti";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
