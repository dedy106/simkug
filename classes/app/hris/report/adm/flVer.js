window.app_hris_report_adm_flVer = function(owner)
{
	if (owner)
	{
		window.app_hris_report_adm_flVer.prototype.parent.constructor.call(this,owner);
		this.className = "app_hris_report_adm_flVer";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Verifikasi", 2);
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_ver_m where kode_lokasi='"+this.app._lokasi+"'");
	if (this.app._userStatus=="A")
	{
		...
		this.gridLib.SGEditData(this.sg1,6,[0,1,2],["No Verifikasi","All",""])
	}
	else
	{
		this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Periode","=",this.periode]);
...
window.app_hris_report_adm_flVer.extend(window.portalui_childForm);
window.app_hris_report_adm_flVer.prototype.doEllipseClick= function(sender, col, row)
{
	...
	if (row == 4)
	{
		...
		this.filterRep.ListDataSGFilter(this, "Data Karyawan",this.sg1, this.sg1.row, this.sg1.col,
			...
			if (row == 6)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Daftar Verifikasi",this.sg1, this.sg1.row, this.sg1.col,
													  ...
													  "inner join gr_karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+this.filter,
													  ["a.no_ver","a.keterangan"],"and",["No Verifikasi","Keterangan"], false, this.sg1.cells(1,row) == "in");
			}
			if (row == 6)
			{
				this.filterRep.ListDataSGFilter(this, "Daftar Verifikasi",this.sg1, this.sg1.row, this.sg1.col,
															  "select no_ver, keterangan from gr_ver_m where kode_lokasi = '"+this.app._lokasi+"' ",
															  "select count(no_ver) from gr_ver_m where kode_lokasi = '"+this.app._lokasi+"' ",
															  ["no_ver","nama"],"and",["No Verifikasi","Nama"], false, this.sg1.cells(1,row) == "in");
			}
	
};
window.app_hris_report_adm_flVer.prototype.doSelectCell = function(sender, col, row)
{
	if (this.app._userStatus=="A")
	{
	...
	if (row == 0)
	{
		...
		this.dbLib.setItemsFromSQL("select distinct periode from gr_ver_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList]);
	}
	if (row == 5)
	{
		this.sg1.columns.get(2).pickList.clear();
		this.dbLib.setItemsFromSQL("select distinct modul from gr_ver_m where kode_lokasi='"+this.app._lokasi+"' order by modul",[this.sg1.columns.get(2).pickList]);
	}
};
window.app_hris_report_adm_flVer.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.filter = this.filterRep.filterStr("a.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
					...
					this.filterRep.filterStr("a.no_ver",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and");
			this.showFilter = this.filterRep.showFilter(this.sg1);
			this.nama_report = "server_report_hris_rptVer";
			if (this.sg1.getCell(2,5)=="ABSEN") {this.nama_report = "server_report_hris_rptVerAbsen";}
			if (this.sg1.getCell(2,5)=="CUTI") {this.nama_report = "server_report_hris_rptVerCuti";}
			if (this.sg1.getCell(2,5)=="LEMBUR") {this.nama_report = "server_report_hris_rptVerLembur";}
			if (this.sg1.getCell(2,5)=="SURAT") {this.nama_report = "server_report_hris_rptVerSurat";}
			if (this.sg1.getCell(2,5)=="SPPD") {this.nama_report = "server_report_hris_rptVerSppd";}
			if (this.sg1.getCell(2,5)=="IJIN") {this.nama_report = "server_report_hris_rptVerIjin";}
	...
	{
		alert("[app_hris_report_adm_flVer]::mainButtonClick:"+e);
	}
};
window.app_hris_report_adm_flVer.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_hris_report_adm_flVer.prototype.getStringHeader = function(){
...
window.app_hris_report_adm_flVer.prototype.doConfirmClick = function(sender){
	try{
...
window.app_hris_report_adm_flVer.prototype.doSelectedPage = function(sender, page)
{
	this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,this.filter, this.page,  this.pager, this.showFilter, this.lokasi,this.filter2));
	this.page = page;
	this.allBtn = false;
};
window.app_hris_report_adm_flVer.prototype.previewReport = function(dthtml)
{
...
window.app_hris_report_adm_flVer.prototype.doCloseReportClick = function(sender)
{
...
window.app_hris_report_adm_flVer.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_hris_report_adm_flVer.prototype.doRowPerPageChange = function(sender, rowperpage)
{
