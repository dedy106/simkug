window.app_hris_report_adm_flPosAbsen = function(owner)
{
	if (owner)
	{
		window.app_hris_report_adm_flPosAbsen.prototype.parent.constructor.call(this,owner);
		this.className = "app_hris_report_adm_flPosAbsen";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Posisi Absen", 2);
...
window.app_hris_report_adm_flPosAbsen.extend(window.childForm);
window.app_hris_report_adm_flPosAbsen.implement({
		...
		try
		{
			...
			else
			{
				...
				else
				{
					...
					this.nama_report = "server_report_hris_rptPosAbsen";
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_absen='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
	...
	doOpenVer: function(no_bukti,kode_lokasi){
		...
		filter2=this.app._namaUser;
		nama_report="server_report_hris_rptVerAbsen";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenApp: function(no_bukti,kode_lokasi){
		...
		filter2=this.app._namaUser;
		nama_report="server_report_hris_rptAppAbsen";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
