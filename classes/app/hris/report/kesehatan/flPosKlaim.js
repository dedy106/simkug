window.app_hris_report_kesehatan_flPosKlaim = function(owner)
{
	if (owner)
	{
		window.app_hris_report_kesehatan_flPosKlaim.prototype.parent.constructor.call(this,owner);
		this.className = "app_hris_report_kesehatan_flPosKlaim";
		this.maximize();
	...
	if (this.app._userStatus=="A")
	{
		...
		this.gridLib.SGEditData(this.sg1,5,[0,1,2],["No Klaim","All",""])
	}
	else
	{
		this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Periode","=",this.periode]);
...
window.app_hris_report_kesehatan_flPosKlaim.extend(window.childForm);
window.app_hris_report_kesehatan_flPosKlaim.implement({
		...
		try{			
			...
			if (row == 5)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Daftar Absen",this.sg1, this.sg1.row, this.sg1.col,
													  ...
													  "inner join gr_karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+this.filter,
													  ["a.no_klaim","a.keterangan"],"and",["No Klaim","Keterangan"], false, this.sg1.cells(1,row) == "in");
			}
		}catch(e){
			systemAPI.alert(e);
		}
	...
	doSelectCell: function(sender, col, row){		
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gr_klaim_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
	},
	...
	doOpenVer: function(no_bukti,kode_lokasi){
		...
		filter2=this.app._namaUser;
		nama_report="server_report_hris_rptVerKlaim";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenApp: function(no_bukti,kode_lokasi){
		...
		filter = "where a.no_ver='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._namaUser;
		nama_report="server_report_hris_rptAppKlaim";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
