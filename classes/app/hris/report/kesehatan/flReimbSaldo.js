window.app_hris_report_kesehatan_flReimbSaldo = function(owner)
{
	if (owner)
	{
		window.app_hris_report_kesehatan_flReimbSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_hris_report_kesehatan_flReimbSaldo";
		this.maximize();
...
window.app_hris_report_kesehatan_flReimbSaldo.extend(window.childForm);
window.app_hris_report_kesehatan_flReimbSaldo.implement({
		...
		try{			
			...
			if (row == 5)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Daftar Absen",this.sg1, this.sg1.row, this.sg1.col,
													  ...
													  "inner join gr_karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+this.filter,
													  ["a.no_kes","a.keterangan"],"and",["No Reimburse","Keterangan"], false, this.sg1.cells(1,row) == "in");
			}
		}catch(e){
			systemAPI.alert(e);
		}
	...
	doSelectCell: function(sender, col, row){		
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gr_kes_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
	},
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				if (this.app._userStatus=="A")
				{
					...
					this.gridLib.SGEditData(this.sg1,5,[0,1,2],["No Reimburse","All",""])
				}
				else
				{
					this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Periode","=",this.periode]);
			...
			else
			{
				...
				else
				{
					...
					this.filter2 = this.sg1.getCell(2,0)+"/"+this.app._lokasi;
					this.showFilter = this.filterRep.showFilter(this.sg1);
					var result  = new arrayMap();		
					this.lokasi=this.app._namalokasi;
					this.nama_report = "server_report_hris_rptReimbSaldo";
	...
	doOpenKartu: function(nik,kode_lokasi){
		this.link="1";
		filter = "where a.nik='"+nik+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.sg1.getCell(2,0)+"/"+this.app._lokasi;
		nama_report="server_report_hris_rptReimbKartu";
