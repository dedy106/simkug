window.app_hris_report_kesehatan_flKlaimAsur = function(owner)
{
	if (owner)
	{
		window.app_hris_report_kesehatan_flKlaimAsur.prototype.parent.constructor.call(this,owner);
		this.className = "app_hris_report_kesehatan_flKlaimAsur";
		this.maximize();
	...
	if (this.app._userStatus=="A")
	{
		...
		this.gridLib.SGEditData(this.sg1,9,[0,1,2],["Jenis Laporan","=","Asuransi"]);
	}
	else
	{
		this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Periode","=",this.periode]);
...
window.app_hris_report_kesehatan_flKlaimAsur.extend(window.childForm);
window.app_hris_report_kesehatan_flKlaimAsur.implement({
	...
	doSelectCell: function(sender, col, row){	
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Asuransi"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				if (this.sg1.getCell(2,9)=='Asuransi')
				{
					this.nama_report = "server_report_hris_rptKlaimAsuransi";
			...
			systemAPI.alert("[app_hris_report_kesehatan_flKlaimAsur]::mainButtonClick:"+e);
		}
	},
