window.app_hris_report_karyawan_flAbsen = function(owner)
{
	if (owner)
	{
		window.app_hris_report_karyawan_flAbsen.prototype.parent.constructor.call(this,owner);
		this.className = "app_hris_report_karyawan_flAbsen";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Absen", 2);
	...
	this.gridLib.SGEditData(this.sg1,7,[0,1,2],["No Absen","All",""]);	
	this.gridLib.SGEditData(this.sg1,8,[0,1,2],["Jenis Laporan","=","Absen"]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_hris_report_karyawan_flAbsen.extend(window.childForm);
window.app_hris_report_karyawan_flAbsen.implement({
	doEllipseClick: function(sender, col, row){
		try{	
			...
			if (row == 6)
			{
				this.filter = this.filterRep.filterStr("kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
				...
				this.filterRep.ListDataSGFilter(this, "Data Karyawan",this.sg1, this.sg1.row, this.sg1.col,
													"select nik, nama from gr_karyawan "+this.filter,
													"select count(*) from gr_karyawan "+this.filter,
													["nik","nama"],"and",["NIK","Nama"]);
			}	
			if (row == 7)
			{
				this.filterRep.ListDataSGFilter(this, "Data Absen",this.sg1, this.sg1.row, this.sg1.col,
													"select no_load,keterangan from gr_absen_harian_m where a.kode_lokasi = '"+this.app._lokasi+"' ",
													"select count(no_load) from gr_absen_harian_m where a.kode_lokasi = '"+this.app._lokasi+"' ",
													["no_load","keterangan"],"and",["No Bukti","Keterangan"]);
			}	
		}catch(e){
			systemAPI.alert(e);
		}
	...
	doSelectCell: function(sender, col, row){		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["3","123","123","123","123","123","123","123","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[0,2,2,2,2,2,2,2,0]);
		if (row == 0)
		{
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Absen"));
		}	
	},
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
							...
							this.filterRep.filterStr("c.kode_jab",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							this.filterRep.filterStr("c.sts_sdm",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+
							this.filterRep.filterStr("b.nik",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and")+
							this.filterRep.filterStr("a.no_load",this.sg1.getCell(1,7),this.sg1.getCell(2,7),this.sg1.getCell(3,7),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();
				if (this.sg1.getCell(2,8)=="Absen")
				{
					var sql = "select b.nik,c.nama,date_format(b.tanggal,'%d/%m/%Y') as tanggal,convert(varchar,b.jam_masuk, 8)  as jam_masuk,convert(varchar,b.jam_pulang, 8) as jam_pulang "+
							"from gr_absen_harian_m a "+
							"inner join gr_absen_harian_d b on a.no_load=b.no_load and a.kode_lokasi=b.kode_lokasi "+
							"inner join gr_karyawan c on c.nik=b.nik and c.kode_lokasi=b.kode_lokasi "+this.filter+
							"order by b.nik ";
					this.scriptSqlCount = "select count(b.nik) "+
							"from gr_absen_harian_m a "+
							"inner join gr_absen_harian_d b on a.no_load=b.no_load and a.kode_lokasi=b.kode_lokasi "+
							"inner join gr_karyawan c on c.nik=b.nik and c.kode_lokasi=b.kode_lokasi "+this.filter;
					
					this.title = new server_util_arrayList({items:["NIK","Nama","Tanggal","Jam Masuk","Jam Pulang"]});
					this.widthTable = new server_util_arrayList({items:[60,300,60,60,60]});
			...
			systemAPI.alert("[app_hris_report_karyawan_flAbsen]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN ABSEN "+this.sg1.getCell(2,1).toUpperCase()+"<br />";
		var d = new Date();
