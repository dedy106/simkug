window.app_hris_report_karyawan_flRekapSdm2 = function(owner)
{
	if (owner)
	{
		window.app_hris_report_karyawan_flRekapSdm2.prototype.parent.constructor.call(this,owner);
		this.className = "app_hris_report_karyawan_flRekapSdm2";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(dbo.fnPeriode(tanggal)) as periode from gr_karyawan_d ");
...
window.app_hris_report_karyawan_flRekapSdm2.extend(window.childForm);
window.app_hris_report_karyawan_flRekapSdm2.implement({
	...
	doSelectCell: function(sender, col, row){		
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct dbo.fnPeriode(tanggal) as periode from gr_karyawan_d where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.link=="2")
				{
					this.doOpenSdm(this.sts_sdm,this.kode_lokasi,this.periode,this.jenis);
				}
				if (this.link=="1")
				{
					this.nama_report = "server_report_hris_rptRekapSdm";
			...
			systemAPI.alert("[app_hris_report_karyawan_flRekapSdm2]::mainButtonClick:"+e);
		}
	},
	...
	doOpenSdm: function(sts_sdm,kode_lokasi,periode,jenis){
		
		this.link="1";
		
		this.sts_sdm=sts_sdm;
		this.kode_lokasi=kode_lokasi;
		this.periode=periode;
		this.jenis=jenis;
		this.filter = "";
		this.filter2=this.sts_sdm+"/"+this.kode_lokasi+"/"+this.periode+"/"+this.jenis;
		this.nama_report="server_report_hris_rptkaryawanTrail";
		this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,this.filter, this.page,  this.pager, this.showFilter, this.lokasi,this.filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
