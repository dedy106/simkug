window.app_hris_report_karyawan_flCv = function(owner)
{
	if (owner)
	{
		window.app_hris_report_karyawan_flCv.prototype.parent.constructor.call(this,owner);
		this.className = "app_hris_report_karyawan_flCv";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Curiculum Vitae (CV)", 2);
	...
	this.gridLib.SGEditData(this.sg1,6,[0,1,2],["Jenis Laporan","=","CV SK"]);	
	this.gridLib.SGEditData(this.sg1,7,[0,1,2],["Status Aktif","=","Ya"]);
	this.gridLib.SGEditData(this.sg1,8,[0,1,2],["SK Penghasilan","=","Tidak"]);
...
window.app_hris_report_karyawan_flCv.extend(window.childForm);
window.app_hris_report_karyawan_flCv.implement({
	...
	doSelectCell: function(sender, col, row){		
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["123i","123i","123i","123i","123i","123i","3","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[2,2,2,2,2,2,0,0,0]);
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["123i","123i","123i","123i","123i","3","3","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[3,3,3,3,3,3,0,0,0]);
		}
		if (row == 6)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("CV Kedinasan","CV SK"));
		}		
		if (row == 7)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.gridLib.SGEditData(this.sg1,6,[0,1,2],["Jenis Laporan","=","Kedinasan"]);				
			}
			else
			{
				...
				var flag_aktif=" and a.flag_aktif='0' ";
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
							...
							this.filterRep.filterStr("a.nik",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+flag_aktif;
				this.filter2=this.sg1.getCell(2,8)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();
				if (this.sg1.getCell(2,6)=="CV Kedinasan")
				{
					this.nama_report = "server_report_hris_rptCv";
			...
			systemAPI.alert("[app_hris_report_karyawan_flCv]::mainButtonClick:"+e);
		}
	},
		...
		try{
			if (sender === sender.owner.bConfirm){
				...
				if (to !== ""){
					...
					this.mail.send("hris@gratika.co.id",to,subject,pesan);
				}else{
