window.app_hris_konten_fKontenV = function(owner)
{
	if (owner)
	{
		window.app_hris_konten_fKontenV.prototype.parent.constructor.call(this,owner);
		this.className  = "app_hris_konten_fKontenV";
...
window.app_hris_konten_fKontenV.extend(window.childForm);
window.app_hris_konten_fKontenV.implement({
	...
	doEllipseClick:function(sender, col, row)
	{
		if (row == 1)
		{
			this.filterRep.ListDataSGFilter(this, "Data Kelompok",this.sg1, this.sg1.row, this.sg1.col,
													  "select kode_klp,nama from gr_konten where kode_lokasi='"+this.app._lokasi+"'",
													  "select count(kode_klp) from gr_konten  where kode_lokasi='"+this.app._lokasi+"' ",
		...
		if (row == 2)
		{
			this.filterRep.ListDataSGFilter(this, "Data Konten",this.sg1, this.sg1.row, this.sg1.col,
													  "select no_konten,nama from gr_konten where kode_lokasi='"+this.app._lokasi+"'",
													  "select count(no_konten) from gr_konten  where  kode_lokasi='"+this.app._lokasi+"'",
	...
	doSelectCell:function(sender, col, row)
	{
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from gr_konten order by periode desc",[this.sg1.columns.get(2).pickList]);
		}
		...
		try
		{
			...
			else
			{
				...
				var sql="select a.no_konten,b.nama as nama_klp,date_format(a.tanggal,'%d-%m-%Y') as tanggal,a.judul,a.file_gambar,a.file_dok "+
					" from gr_konten a "+
					" inner join gr_klpkonten b on b.kode_lokasi = a.kode_lokasi and b.kode_klp = a.kode_klp "+this.filter+
