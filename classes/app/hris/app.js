...
//* 			SAI, PT											
//***********************************************************************************************
window.app_hris_app = function(owner){
	window.app_hris_app.prototype.parent.constructor.call(this, owner);
	this.className = "app_hris_app";
	this.onTerminate.set(this,"doTerminate");
	this.form = undefined;
	this.setTitle("HRIS GRATIKA");
	this.showForm();
};
window.app_hris_app.extend(window.application);
window.app_hris_app.implement({
	showForm: function(){
		try
		{
			...
			this._mainForm = new app_hris_fMain(this);	
	...
	doKeyDown: function(charCode, buttonState, keyCode, event){
		try{			
			window.app_hris_app.prototype.parent.doKeyDown.call(this, charCode, buttonState, keyCode, event);
