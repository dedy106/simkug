window.app_budget_fKaryawan = function(owner)
{
	if (owner)
	{
		window.app_budget_fKaryawan.prototype.parent.constructor.call(this,owner);
		this.className  = "app_budget_fKaryawan";
		this.maximize();
...
window.app_budget_fKaryawan.extend(window.portalui_childForm);
window.app_budget_fKaryawan.prototype.mainButtonClick = function(sender)
{
...
window.app_budget_fKaryawan.prototype.doModalResult = function(event, modalResult)
{			
	switch (event)
	{
		case "clear" :
...
window.app_budget_fKaryawan.prototype.showClick = function(sender)
{
	if (this.ed_kode.getText() != "")
	{
		try
		{
			...
			var data = this.dbLib.runSQL(" select a.nama,a.alamat,a.jabatan,a.no_telp,a.email,a.npwp,a.bank,a.cabang,a.no_rek,a.nama_rek, a.kode_pp, b.nama as nmpp "+
										 " from karyawan a "+
										 " left outer join agg_pp b on b.kode_pp = a.kode_pp and b.kode_lokasi = a.kode_lokasi "+
...
window.app_budget_fKaryawan.prototype.FindBtnClick = function(sender, event)
{
	try
	{
		...
		if (sender == this.cb_pp) 
		{
			this.standarLib.showListData(this, "Daftar Departemen",this.cb_pp,undefined, 
										  "select kode_pp, nama  from agg_pp where kode_lokasi='"+this.cb_lokasi.getText()+"' and tipe= 'posting'",
										  "select count(kode_pp) from agg_pp where kode_lokasi='"+this.cb_lokasi.getText()+"' and tipe= 'posting'",
...
window.app_budget_fKaryawan.prototype.doRequestReady = function(sender, methodName, result)
{
