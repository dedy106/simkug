 ...
 */
window.app_budget_transaksi_fTpku = function(owner)
{
	if (owner)
	{
		window.app_budget_transaksi_fTpku.prototype.parent.constructor.call(this,owner);
		this.className  = "app_budget_transaksi_fTpku";
		this.setTop(60);
		this.setWidth(1280);
		this.setHeight(550);
		this.itemsValue = new portalui_arrayList();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data TPKU NON FFS", 0);	
...
//------------------------------------------------------------------------
		try{
			...
			this.p1 = new portalui_panel(this,{bound:[10,28,800,273],caption:"Daftar Dokter TPKU non FFS"});
...
window.app_budget_transaksi_fTpku.extend(window.portalui_childForm);
//------------------------------------------------------------------ event
window.app_budget_transaksi_fTpku.implement({
					...
					if (this.standarLib.checkEmptyByTag(this, [0,1]))
					{
						...
						for (var j=1; j <= 12; j++){
							sql.add("insert into agg_tpku (no_kontrak,kode_lokasi,dokter,alamat,kode_kota,kode_tpku,tahun,nilai,kode_pp,kode_rka,kode_akun,periode,jumlah,tarif,nilai_final,p_derita,p_kunjungan,jml_kunjungan,p_diskon,tarif_final) "+
									...
									this.eKota.getText()+"','"+this.eJenis.getText()+"','"+this.eTahun.getText()+"',"+parseNilai(this.eNilai.getText())+",'"+this.ePP.getText()+"','"+this.eRka.getText()+"','"+
									this.eAkun.getText()+"','"+this.eTahun.getText()+(j<10?"0":"")+j+"',"+parseNilai(this.eJml.getText())+","+parseNilai(this.eTarif.getText())+","+parseNilai(this.eFinal.getText())+","+parseNilai(this.ePderita.getText())+","+parseNilai(this.ePkunj.getText())+","+
									parseNilai(this.eKunj.getText())+","+parseNilai(this.eDiskon.getText())+","+parseNilai(this.eTarifFinal.getText())+")");
						}
	...
	doEditChange: function(sender){
		try {
			...
			if (sender == this.eJml || sender == this.eTarifFinal ) {
				if (this.eJml.getText()!="" && this.eTarifFinal.getText()!="") {
					this.eNilai.setText(floatToNilai(nilaiToFloat(this.eJml.getText()) * nilaiToFloat(this.eTarifFinal.getText())));
			...
			if (sender == this.eTahun || sender == this.eKota) {
				...
				this.eJenis.setSQL("select kode_tpku, nama, round(tarif*"+idx+"/100,0) as tarif, nilai from agg_bpcc_jenis_tpku where jenis = 'NONFFS' and tahun = '"+this.eTahun.getText()+"' ",["kode_tpku","nama","tarif","nilai"],undefined,["Kode","Nama","Tarif","Nilai"],"and","Data Jenis TPKU",false);
			}
			...
			if (sender == this.eJenis) {
				if (this.eJenis.getText() != ""){
					...
					this.eMin.setText(parseFloat(this.eJenis.dataFromList[3]);
				}
			}
