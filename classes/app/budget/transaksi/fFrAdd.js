 ...
 * @author mr
 */
window.app_budget_transaksi_fFrAdd = function(owner)
{
	if (owner)
	{
		window.app_budget_transaksi_fFrAdd.prototype.parent.constructor.call(this,owner);
		this.className  = "app_budget_transaksi_fFrAdd";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Aktiva Tetap Yang Direncanakan: Input/Koreksi",0);
...
window.app_budget_transaksi_fFrAdd.extend(window.portalui_childForm);
window.app_budget_transaksi_fFrAdd.implement({
	...
	simpan: function(){	
		if (this.standarLib.checkEmptyByTag(this, new Array("0","1","2"))){
			...
			try{
				...
				sql.add("delete from agg_fa_asset where tahun_agg = '"+this.eTahun.getText()+"' and jenis_agg = 'T' and kode_lokasi = '"+this.app._lokasi+"' ");
				...
				for (var i=0; i < this.sg1.getRowCount();i++){			
					if (this.sg1.rowValid(i)) {
						var tglOleh = this.eTahun.getText()+'-'+this.sg1.cells(2,i)+'-01';
						var tglSusut = this.eTahun.getText()+'-'+this.sg1.cells(9,i)+'-01';
						sql.add("insert into agg_fa_asset(no_fa,barcode,kode_lokasi,kode_pp,kode_klpfa,kode_klpakun,kode_lokfa,nik_pnj,kode_brg,nama,kode_curr,kurs,nilai,nilai_residu,kode_drk,catatan,"+
									"                     progress,tgl_perolehan,periode,tgl_susut,nik_user,tgl_input,umur,persen,periode_susut,merk,tipe,no_seri,kode_status,kode_akun,jenis,jenis_agg,status_aktif,tahun_agg,kode_rka2,kode_rka3,kode_pp2,kode_pp3,jumlah,kode_status) values "+ //field progress dipakai utk jurnal assetnya --->dr 0 ke 1
									"('"+
									...
									"0,'"+this.sg1.cells(15,i)+"','-','0','"+tglOleh+"','"+this.eTahun.getText()+this.sg1.cells(2,i)+"','"+tglSusut+"','"+
									this.app._userLog+"',now(),"+parseNilai(this.sg1.cells(13,i))+","+parseNilai(this.sg1.cells(14,i))+",'"+this.eTahun.getText()+this.sg1.cells(9,i)+"','-','"+
s									"-','-','-','"+this.sg1.cells(11,i)+"','-','T','1','"+this.eTahun.getText()+"','"+this.sg1.cells(17,i)+"','"+this.sg1.cells(19,i)+"','"+this.sg1.cells(21,i)+"','"+this.sg1.cells(23,i)+"',"+parseNilai(this.sg1.cells(25,i))+",'T')");
						lastId++;
	...
	doTampil: function(sender){	
		if (this.eTahun.getText() != "") {
			var data = this.dbLib.getDataProvider("select a.no_fa,a.nama,substring(periode,5,2) as tgl_perolehan,a.kode_pp,b.nama as nama_pp,a.kode_klpfa,c.nama as nama_klp,a.kode_klpakun,d.nama as nama_klpakun,substring(periode_susut,5,2) as tgl_susut,a.nilai,a.kode_akun,e.nama as nama_akun,a.umur,a.persen,a.kode_drk,f.nama as nama_rka,a.kode_rka2,ff.nama as nama_rka2,a.kode_rka3,fff.nama as nama_rka3,a.kode_pp2,a.kode_pp3,bb.nama as nama_pp2,bbb.nama as nama_pp3,a.jumlah "+
												  ...
												  "where a.tahun_agg = '"+this.eTahun.getText()+"' and a.jenis_agg = 'T' and a.kode_lokasi='"+this.app._lokasi+"'"); // and a.status_aktif ='1'
			eval("data = "+data+";");				
