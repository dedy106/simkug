window.app_saku_anggaran_transaksi_fGarorgiK = function(owner)
{  
	if (owner)
	{
		window.app_saku_anggaran_transaksi_fGarorgiK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_anggaran_transaksi_fGarorgiK";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Data Anggaran Original: Koreksi", 0);
		...
		this.lblTgl1.setCaption("Tgl Anggaran");
		...
		this.ed_nilai.setCaption("Total Pengajuan");
...
window.app_saku_anggaran_transaksi_fGarorgiK.extend(window.portalui_childForm);
window.app_saku_anggaran_transaksi_fGarorgiK.prototype.doSelect = function(sender, year, month, day)
{
	if (month < 10)
		month = "0"+month;
	this.ed_period.setText(year.toString());
};
window.app_saku_anggaran_transaksi_fGarorgiK.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_anggaran_transaksi_fGarorgiK.prototype.simpan = function(event)
{	
	if (this.posted == "T")
	{
		system.alert(this,"Transaksi sudah diclosing tidak dapat dibatalkan.","");
		return false;
	}
	if (this.standarLib.checkEmptyByTag(this, new Array("0","1")))
	{
		try
		{
			...
			sql.add("delete from anggorgi_d where no_agg ='"+this.ed_nb.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
...
window.app_saku_anggaran_transaksi_fGarorgiK.prototype.doModalResult = function(event, modalResult)
{
...
window.app_saku_anggaran_transaksi_fGarorgiK.prototype.genClick = function(sender)
{
	try
	{
...
window.app_saku_anggaran_transaksi_fGarorgiK.prototype.doEditChange = function(sender)
{
	...
	if (sender == this.ed_nb){
		...
		if (typeof(rs) == "object"){
			...
			this.posted = rs.posted;
			rs = this.dbLib.execSQL("select a.kode_akun, b.nama as nama_akum, a.kode_pp, c.nama as nama_dept, a.kode_drk, d.nama, substring(a.periode,5,2), a.volume, a.satuan, a.nilai_sat "+
...
window.app_saku_anggaran_transaksi_fGarorgiK.prototype.FindBtnClick = function(sender, event)
{
	try
	{				
		if (sender == this.ed_nb){			
			this.standarLib.showListData2(this, "Daftar Upload Anggaran",sender,undefined, 
										  "select distinct no_agg, keterangan  from anggaran_m where kode_lokasi ='"+this.app._lokasi+"' and jenis = 'ORGI' and posted = 'F' and tanggal <= '"+this.dp_tgl1.getDateString()+"' ",
										  "select count(distinct no_agg, keterangan)  from anggaran_m where kode_lokasi ='"+this.app._lokasi+"' and jenis = 'ORGI' and posted = 'F' and tanggal <= '"+this.dp_tgl1.getDateString()+"'",
										  ["no_agg","keterangan"],"where",["No Anggaran","Deskripsi"],false);
		}
		if (sender == this.cb_curr) 
		{
		    this.standarLib.showListData2(this, "Daftar Currency",sender,undefined, 
		...
		{   		    
			this.standarLib.showListData2(this, "Daftar Petugas",sender,undefined, 
		...
		{   		    
			this.standarLib.showListData2(this, "Daftar yang Menyetujui",sender,undefined, 
										  "select nik, nama  from karyawan where kode_lokasi ='"+this.app._lokasi+"'",
										  "select count(nik) from karyawan where kode_lokasi ='"+this.app._lokasi+"'",
										  ["nik","nama"],"and",["NIK","Nama"],false);
		}
...
window.app_saku_anggaran_transaksi_fGarorgiK.prototype.doFindBtnClick = function(sender, col, row) 
{
	try
	{
		switch(col)
	...
	{
		alert("[app_saku_anggaran_transaksi_fGarorgiK] : doFindBtnClick : " + e);
	}
};
window.app_saku_anggaran_transaksi_fGarorgiK.prototype.doNilaiChange = function()
{
	try
	{
		var tot = 0;
	...
	{
		alert("[app_saku_anggaran_transaksi_fGarorgiK]::doNilaiChange:"+e);
	}
};
window.app_saku_anggaran_transaksi_fGarorgiK.prototype.doCellExit = function()
{
	this.sg1.validasi();
};
window.app_saku_anggaran_transaksi_fGarorgiK.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_saku_anggaran_transaksi_fGarorgiK.prototype.doAfterLoad = function(sender, result,data)
{
	try{	
		if (result){			
...
window.app_saku_anggaran_transaksi_fGarorgiK.prototype.doLoadData = function(data, page)
{
	var rs, arr;
	this.sg1.clear();
	if (data instanceof portalui_arrayMap){		
...
window.app_saku_anggaran_transaksi_fGarorgiK.prototype.doSelectedPage = function(sender, page)
{
