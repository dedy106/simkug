 ...
 * @author mr
 */
window.app_budget_transaksi_fPostAsset = function(owner)
{
	if (owner)
	{
		window.app_budget_transaksi_fPostAsset.prototype.parent.constructor.call(this,owner);
		this.className  = "app_budget_transaksi_fPostAsset";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Posting Anggaran Investasi dan Biaya Aktap: Input", 0);
		try{	
			...
			this.cJenis = new portalui_saiCB(this,{bound:[20,13,180,20],caption:"Jenis",items:["INVESTASI","BP","BPP"],tag:2});
			...
			this.p0 = new portalui_panel(this,{bound: [20, 231, 900, 355],caption: "Daftar Jurnal Anggaran Investasi dan Biaya Aktap"});	    				
			...
			var prd = this.dbLib.getDataProvider("select distinct periode from agg_fa_asset where kode_lokasi = '"+this.app._lokasi+"' and posted = 'F'  and jenis_agg='T' "+
			                                     "union "+
												 "select distinct b.periode from agg_fasusut_m a inner join agg_fasusut_d b on a.no_fasusut=b.no_fasusut and a.kode_lokasi=b.kode_lokasi "+
												 "where a.kode_lokasi = '"+this.app._lokasi+"' and a.posted = 'F' "+
...
window.app_budget_transaksi_fPostAsset.extend(window.portalui_childForm);
window.app_budget_transaksi_fPostAsset.implement({
	...
	doModalResult: function(event, modalResult){
		...
		switch (event)
		{
			...
			case "simpan" :		
				...
				this.ed_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'agg_fapost_m','no_post',this.app._lokasi+"-POFA"+this.dp_tgl1.getThnBln().substr(2,2)+""+(this.dp_tgl1.getMonth()< 10 ?"0":"")+this.dp_tgl1.getMonth()+".",'0000'));
				var sql = new server_util_arrayList();				
				sql.add("insert into agg_fapost_m(no_post, kode_lokasi, tanggal, keterangan, tgl_input, nik_user, periode, jenis)"+
				...
				var idx=0;
				if (this.cJenis.getText() == "INVESTASI") {					
					sql.add("insert into agg_gldt(no_bukti,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,kode_curr,kurs,nilai_curr,tgl_input,nik_user,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik) "+
							"select '"+this.ed_nb.getText()+"',0,kode_lokasi,'ASSET','INVESTASI','-','"+this.dp_tgl1.getDateString()+"',kode_akun,'D',sum(nilai),'"+this.ed_desc.getText()+"',kode_pp,'"+this.cbPeriode.getText()+"',kode_drk,'IDR',1,sum(nilai),now(),'"+this.app._userLog+"','-','-','-','-','-','-' "+
							"from agg_fa_asset "+
							"where periode='"+this.cbPeriode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and posted='F' and jenis_agg='T' and tahun_agg = '"+this.cbPeriode.getText().substr(0,4)+"' "+
							"group by kode_lokasi,kode_drk,kode_akun,kode_pp,periode ");
					sql.add("update agg_fa_asset set posted = 'T' where periode='"+this.cbPeriode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and posted='F'");
				}
				if (this.cJenis.getText() == "BPP") {					
					sql.add("insert into agg_gldt(no_bukti,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,kode_curr,kurs,nilai_curr,tgl_input,nik_user,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik) "+
							"select '"+this.ed_nb.getText()+"',0,kode_lokasi,'ASSET','BPP','-','"+this.dp_tgl1.getDateString()+"',akun_bp,'D',sum(nilai),'"+this.ed_desc.getText()+"',kode_pp,'"+this.cbPeriode.getText()+"',kode_drk,'IDR',1,sum(nilai),now(),'"+this.app._userLog+"','-','-','-','-','-','-' "+							
							"from agg_fasusut_d "+
							"where periode='"+this.cbPeriode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and posted='F' and status='BPP' "+
							"group by kode_lokasi,kode_drk,akun_bp,kode_pp,periode ");
					sql.add("update agg_fasusut_d set posted = 'T' where periode='"+this.cbPeriode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and posted='F' and status='BPP'");
				}
				if (this.cJenis.getText() == "BP") {					
					sql.add("insert into agg_gldt(no_bukti,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,kode_curr,kurs,nilai_curr,tgl_input,nik_user,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik) "+
							"select '"+this.ed_nb.getText()+"',0,kode_lokasi,'ASSET','BP','-','"+this.dp_tgl1.getDateString()+"',akun_bp,'D',sum(nilai),'"+this.ed_desc.getText()+"',kode_pp,'"+this.cbPeriode.getText()+"',kode_drk,'IDR',1,sum(nilai),now(),'"+this.app._userLog+"','-','-','-','-','-','-' "+							
							"from agg_fasusut_d "+
							"where periode='"+this.cbPeriode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and posted='F' and status='BP' "+
							"group by kode_lokasi,kode_drk,akun_bp,kode_pp,periode "+
							"union "+
							"select '"+this.ed_nb.getText()+"',0,kode_lokasi,'ASSET','AP','-','"+this.dp_tgl1.getDateString()+"',akun_ap,'C',sum(nilai),'"+this.ed_desc.getText()+"',kode_pp,'"+this.cbPeriode.getText()+"',kode_drk,'IDR',1,sum(nilai),now(),'"+this.app._userLog+"','-','-','-','-','-','-' "+							
							"from agg_fasusut_d "+
							"where periode='"+this.cbPeriode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and posted='F' and status='BP' "+
							"group by kode_lokasi,kode_drk,akun_ap,kode_pp,periode ");
					sql.add("update agg_fasusut_d set posted = 'T' where periode='"+this.cbPeriode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and posted='F' and status='BP' ");
				}
				...
				for (var i=0;i< this.sg1.getRowCount();i++){
					if (this.sg1.rowValid(i)){
						...
						sql.add("insert into agg_gldt(no_bukti,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,kode_curr,kurs,nilai_curr,tgl_input,nik_user,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik) values "+
								"('"+this.ed_nb.getText()+"',"+idx+",'"+this.app._lokasi+"','ASSET','ADD','-','"+this.dp_tgl1.getDateString()+"','"+this.sg1.cells(0,i)+"','"+this.sg1.cells(3,i)+"',"+parseNilai(this.sg1.cells(4,i))+",'"+this.sg1.cells(2,i)+"','"+this.sg1.cells(5,i)+"','"+this.cbPeriode.getText()+"','"+this.sg1.cells(7,i)+"','IDR',1,"+parseNilai(this.sg1.cells(4,i))+",now(),'"+this.app._userLog+"','-','-','-','-','-','-')");
					}
				}
	...
	doLoadData: function(sender){	
		if (this.cbPeriode.getText() != "") {
			if (this.cJenis.getText() == "INVESTASI") {
				var data = this.dbLib.getDataProvider("select c.kode_rka,c.nama as nama_rka,d.kode_drk,d.nama as nama_drk,a.kode_akun,e.nama as nama_akun,a.kode_pp,f.nama as nama_pp,a.periode,sum(a.nilai) as total,'D' as dc "+
											"from agg_fa_asset a "+
											"                  inner join agg_rka c on a.kode_drk=c.kode_rka and a.kode_lokasi=c.kode_lokasi and a.tahun_agg=c.tahun "+
											...
											"where a.periode='"+this.cbPeriode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.posted='F' and a.jenis_agg='T' and a.tahun_agg = '"+this.cbPeriode.getText().substr(0,4)+"' "+
											"group by c.kode_rka,c.nama,d.kode_drk,d.nama,a.kode_akun,e.nama,a.kode_pp,f.nama,a.periode "+
											"order by c.kode_rka,a.kode_akun,a.kode_pp", true);			
			} else {
				if (this.cJenis.getText() == "BPP") {
					var data = this.dbLib.getDataProvider("select c.kode_rka,c.nama as nama_rka,d.kode_drk,d.nama as nama_drk,a.akun_bp as kode_akun,e.nama as nama_akun,a.kode_pp,f.nama as nama_pp,a.periode,sum(a.nilai) as total,'D' as dc  "+
											"from agg_fasusut_d a "+
											"                  inner join agg_rka c on a.kode_drk=c.kode_rka and a.kode_lokasi=c.kode_lokasi and substring(a.periode,1,4)=c.tahun "+
											"                  inner join agg_drk d on c.kode_drk=d.kode_drk and c.kode_lokasi=d.kode_lokasi and c.tahun=d.tahun "+
											"                  inner join agg_masakun e on a.akun_bp=e.kode_akun and a.kode_lokasi=e.kode_lokasi "+
											"                  inner join agg_pp f on a.kode_pp=f.kode_pp and a.kode_lokasi=f.kode_lokasi "+
											"where a.periode='"+this.cbPeriode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.posted='F' and a.status='BPP' "+
											"group by c.kode_rka,c.nama,d.kode_drk,d.nama,a.akun_bp,e.nama,a.kode_pp,f.nama,a.periode "+
											"order by c.kode_rka,a.akun_bp,a.kode_pp", true);			
				} else {
					var data = this.dbLib.getDataProvider("select c.kode_rka,c.nama as nama_rka,d.kode_drk,d.nama as nama_drk,a.akun_bp as kode_akun,e.nama as nama_akun,a.kode_pp,f.nama as nama_pp,a.periode,sum(a.nilai) as total,'D' as dc  "+
											"from agg_fasusut_d a "+
											"                  inner join agg_rka c on a.kode_drk=c.kode_rka and a.kode_lokasi=c.kode_lokasi and substring(a.periode,1,4)=c.tahun "+
											...
											"                  inner join agg_masakun e on a.akun_bp=e.kode_akun and a.kode_lokasi=e.kode_lokasi "+
											"                  inner join agg_pp f on a.kode_pp=f.kode_pp and a.kode_lokasi=f.kode_lokasi "+
											"where a.periode='"+this.cbPeriode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.posted='F' and a.status='BP' "+
											"group by c.kode_rka,c.nama,d.kode_drk,d.nama,a.akun_bp,e.nama,a.kode_pp,f.nama,a.periode "+
											"union "+
											"select c.kode_rka,c.nama as nama_rka,d.kode_drk,d.nama as nama_drk,a.akun_ap as kode_akun,e.nama as nama_akun,a.kode_pp,f.nama as nama_pp,a.periode,sum(a.nilai) as total,'C' as dc  "+
											"from agg_fasusut_d a "+
											"                  inner join agg_rka c on a.kode_drk=c.kode_rka and a.kode_lokasi=c.kode_lokasi and substring(a.periode,1,4)=c.tahun "+
											"                  inner join agg_drk d on c.kode_drk=d.kode_drk and c.kode_lokasi=d.kode_lokasi and c.tahun=d.tahun "+
											"                  inner join agg_masakun e on a.akun_ap=e.kode_akun and a.kode_lokasi=e.kode_lokasi "+
											"                  inner join agg_pp f on a.kode_pp=f.kode_pp and a.kode_lokasi=f.kode_lokasi "+
											"where a.periode='"+this.cbPeriode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.posted='F' and a.status='BP' "+
											"group by c.kode_rka,c.nama,d.kode_drk,d.nama,a.akun_ap,e.nama,a.kode_pp,f.nama,a.periode "+
											"order by dc desc,c.kode_rka,a.akun_bp,a.kode_pp", true);			
				}
			}
