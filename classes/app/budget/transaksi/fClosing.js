window.app_saku_anggaran_transaksi_fClosing = function(owner)
{
	if (owner)
	{
		window.app_saku_anggaran_transaksi_fClosing.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_anggaran_transaksi_fClosing";
		this.maximize();
...
window.app_saku_anggaran_transaksi_fClosing.extend(window.portalui_childForm);
window.app_saku_anggaran_transaksi_fClosing.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_anggaran_transaksi_fClosing.prototype.doModalResult = function(event, modalResult)
{
...
window.app_saku_anggaran_transaksi_fClosing.prototype.genClick = function(sender)
{
	try
	{
...
window.app_saku_anggaran_transaksi_fClosing.prototype.pAllClick = function(sender)
{
	for (var i=0; i < this.sg1.rows.getLength(); i++)
	{
...
window.app_saku_anggaran_transaksi_fClosing.prototype.doSelectedPage = function(sender, page)
{
	try
	{
...
window.app_saku_anggaran_transaksi_fClosing.prototype.showClick = function(sender)
{
	try
	{
		this.sg1.clear();
		...
		if (this.app._dbEng == "mysqlt")
		{
			this.scriptSql = " select 'INPROG' as status,a.no_agg,a.no_dokumen,a.tanggal,a.keterangan,a.kode_curr,b.nama as nama_buat,c.nama as nama_setuju,a.nilai "+
							 " from anggaran_m a inner join karyawan b on a.nik_buat = b.nik and a.kode_lokasi=b.kode_lokasi "+
...
window.app_saku_anggaran_transaksi_fClosing.prototype.doEditChange = function(sender)
{
...
window.app_saku_anggaran_transaksi_fClosing.prototype.sg1ondblclick = function(sender, col , row)
{
	this.sg2.clearAll();
	...
	{	
		if (this.app._dbEng == "mysqlt")
		{
			var temp = this.dbLib.runSQL(" select a.kode_akun,b.nama as nama_akun,a.kode_pp,c.nama as nama_pp,a.kode_drk,d.nama as nama_drk,a.dc,substring(a.periode,5,2) as periode,a.volume,a.satuan,a.nilai_sat,a.nilai "+
			                             " from anggorgi_d a inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
...
window.app_saku_anggaran_transaksi_fClosing.prototype.FindBtnClick = function(sender, event)
{
	try
	{
...
window.app_saku_anggaran_transaksi_fClosing.prototype.doRequestReady = function(sender, methodName, result)
{
