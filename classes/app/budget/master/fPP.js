window.app_budget_master_fPP = function(owner)
{
	if (owner)
	{
		window.app_budget_master_fPP.prototype.parent.constructor.call(this, owner);
		this.className = "app_budget_master_fPP";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Bisnis Unit", 0);	
		...
		this.pAdd.hide();
			...
			this.e5.setCaption("Bidang");					
			this.e5.setMultiSelection(false);
			this.e5.setSQL("select kode_bidang, nama from agg_bidang",["kode_bidang","nama"],false,["Kode Bidang","Nama"],"where","Daftar Bidang", true);
...
window.app_budget_master_fPP.extend(window.portalui_childForm);
window.app_budget_master_fPP.prototype.mainButtonClick = function(sender){
...
window.app_budget_master_fPP.prototype.doRequestReady = function(sender, methodName, result){
	switch (methodName)
		{
			case "listData" : 
...
window.app_budget_master_fPP.prototype.doModalResult = function(event, modalResult, value){
	try{
		switch (event){
			case "Add" :
			   if ( modalResult == mrOk){
					try{
						...
						data.set("rowindex",0);							
						data.set("kode_bidang",this.e5.getText());							
						node.data = data;
					}catch(e)
					{
						alert(e);
			...
			case "simpan" :
				if (modalResult == mrOk)
				{
					...
					try
					{						
						var sql2,sql1 = "delete from agg_pp where kode_lokasi = '"+this.elokasi.getText()+"'";					// kode_lokasi,						
						...
						if (value != ""){							
								...
								for (var i in data){
									sql2 = "insert into agg_pp(kode_pp, kode_lokasi,nama, initial,level_spasi, tipe, sum_header, kode_induk, rowindex, kode_bidang) values ";
									sql2 += data[i];
									sql.add(sql2);
								}							
						}
...
window.app_budget_master_fPP.prototype.loadMenu = function(daftarNrc){
	try{
...
//-------------------------------------
window.app_budget_master_fPP.prototype.doClick = function(sender){	
	var nrc = this.dblib.runSQL("select * from agg_pp where  kode_lokasi = '"+this.elokasi.getText()+"' order by rowindex",0,0);	
	this.loadMenu(nrc);	
};
window.app_budget_master_fPP.prototype.entriesClick = function(sender){
	try
	{ 
		...
		{			
			...
			}else if (sender == this.editBtn)
			{
				...
				this.e5.setText(item.data.get("kode_bidang") == "undefined" ?"":item.data.get("kode_bidang"));				
...
window.app_budget_master_fPP.prototype.treeClick = function(item){
};
window.app_budget_master_fPP.prototype.getTreeData = function(node){
	var result = "";
	var child = undefined;
...
window.app_budget_master_fPP.prototype.getSummary = function(node){
	var result = "";
	var child = undefined;
	if (node instanceof portalui_treeView)
	{
...
window.app_budget_master_fPP.prototype.doFindLokasi = function(sender)
{
	if (this.app._userStatus =="A")
...
window.app_budget_master_fPP.prototype.setSummaryItems = function(data){
	this.e4.clearItem();
	for (var i in data)
	{
...
window.app_budget_master_fPP.prototype.copyChilds = function(from, to){
	var temp = undefined;
...
window.app_budget_master_fPP.prototype.switchItem = function(from, to){
...
window.app_budget_master_fPP.prototype.moveItem = function(item, owner)
{	
