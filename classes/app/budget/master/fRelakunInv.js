window.app_budget_master_fRelakunInv = function(owner,lokasi)
{
	if (owner)
	{
		window.app_budget_master_fRelakunInv.prototype.parent.constructor.call(this, owner);
		window.app_budget_master_fRelakunInv.prototype.parent.setWidth.call(this, 800);
		window.app_budget_master_fRelakunInv.prototype.parent.setHeight.call(this, 400);
		
		this.centerize();
		
		this.className = "app_budget_master_fRelakunInv";
...
window.app_budget_master_fRelakunInv.extend(window.portalui_commonForm);
window.app_budget_master_fRelakunInv.prototype.doDraw = function(canvas){
...
window.app_budget_master_fRelakunInv.prototype.doAfterResize = function(event){
	this.p1.setWidth(this.width - 40);
...
window.app_budget_master_fRelakunInv.prototype.setWidth = function(data){
	window.app_budget_master_fRelakunInv.prototype.parent.setWidth.call(this, data);
	this.header.style.width = data - 23;
	this.rBg.style.left = data - 23;
};
window.app_budget_master_fRelakunInv.prototype.doSysMouseMove = function(x, y, button, buttonState){
	window.app_budget_master_fRelakunInv.prototype.parent.doSysMouseMove.call(this,x, y, button, buttonState);
	if (this.isClick)
	{		
...
window.app_budget_master_fRelakunInv.prototype.eventMouseDown = function(event){	
...
window.app_budget_master_fRelakunInv.prototype.eventMouseUp = function(event){
	this.isClick = false;
	this.blockElm.style.display = "none";
	this.frameElm.style.display = "";
};
window.app_budget_master_fRelakunInv.prototype.eventMouseMove = function(event){
	if (this.isClick)
	{
		var x = event.clientX;
		var y = event.clientY;
...
window.app_budget_master_fRelakunInv.prototype.doClick = function(sender){
	try{
		if (sender == this.b1)
...
//--------------------------------------------
window.app_budget_master_fRelakunInv.prototype.setCaption = function(data){
...
window.app_budget_master_fRelakunInv.prototype.setItemParent = function(data){
	this.itemParent = data;
};
window.app_budget_master_fRelakunInv.prototype.entriesClick = function(sender, col, row){
...
window.app_budget_master_fRelakunInv.prototype.show = function()
{
	this.showModal();
...
window.app_budget_master_fRelakunInv.prototype.hide = function()
{
...
window.app_budget_master_fRelakunInv.prototype.loadNeraca = function(){		
	var filter = "(";
...
window.app_budget_master_fRelakunInv.prototype.doClose = function(sender)
{
	system.delMouseListener(this);
};
window.app_budget_master_fRelakunInv.prototype.doCariClick = function(sender)
{
	try{
		this.sg1.clear();
		var filter = "(";
		...
		var nrc = this.dblib.loadQuery("select a.kode_akun, a.nama  from agg_masakun a "+				
					"where a.kode_lokasi = '"+this.elokasi+"' and a.kode_akun like '"+this.eSearch.getText()+"%' and modul like '" + this.modul +"' "+ 				
