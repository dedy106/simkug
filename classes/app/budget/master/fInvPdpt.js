 ...
 * @author dweexfuad/
 */
window.app_budget_master_fInvPdpt = function(owner)
{
	if (owner)
	{
		window.app_budget_master_fInvPdpt.prototype.parent.constructor.call(this,owner);
		this.className  = "app_budget_master_fInvPdpt";
		this.setTop(60);
		this.setWidth(1280);
		this.setHeight(550);
		this.itemsValue = new portalui_arrayList();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Referensi Pendapatan", 0);	
		
//------------------------------------------------------------------------
		try{
			uses("portalui_saiCBBL;util_standar;");
			this.e0 = new portalui_saiCBBL(this,{bound:[20,21,170,20],caption:"Kode Inv", multiSelection:false,change:[this,"doEditChange"]});					
			...
			this.e0.setSQL("select kode_inv, nama from agg_inv_jenis ",["kode_inv","nama"],undefined,["Kode Param","Nama"],"where","Data Jenis Investasi",false);
		}catch(e)
		{
			alert(e);
		}
...
window.app_budget_master_fInvPdpt.extend(window.portalui_childForm);
//------------------------------------------------------------------ event
window.app_budget_master_fInvPdpt.implement({
	...
	doTampilClick: function(sender){
		try{			
			var temp = this.dbLib.runSQL("select a.kode_inv,b.nama,a.periode,a.persen,a.nilai "+
										"from agg_inv_pdpt a "+
										"inner join agg_inv_jenis b on a.kode_inv=b.kode_inv "+
										"order by a.kode_inv ");
