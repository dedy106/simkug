window.app_budget_master_fFS = function(owner)
{
	if (owner)
	{
		window.app_budget_master_fFS.prototype.parent.constructor.call(this,owner);
		this.className  = "app_budget_master_fFS";
		this.setTop(60);
		this.setWidth(1280);
		this.setHeight(550);
...
//------------------------------------------------------------------------
		try{
			...
			this.e0.setSQL("select kode_fs, nama from agg_fs where kode_lokasi = '"+this.app._lokasi+"' ",["kode_fs","nama"],undefined,["Kode FS","Nama"],"and","Data Versi Laporan",false);
		}catch(e)
		{
			alert(e);
		}
...
window.app_budget_master_fFS.extend(window.portalui_childForm);
//------------------------------------------------------------------ event
window.app_budget_master_fFS.implement({
		...
		try{
			...
			switch (event)
			{
				...
				case "simpan" :
						...
						sql.add("insert into agg_fs (kode_fs, kode_lokasi,nama,tgl_awal, tgl_akhir, flag_status, tgl_input,nik_user ) values ('"+this.e0.getText()+"','"+this.app._lokasi+"','"+this.e1.getText()+"','"+this.d1.getDate()+"','"+this.d2.getDate()+"','"+status+"','"+(new Date).getDateStr()+"','"+this.app._userLog+"') ");
						this.dbLib.execArraySQL(sql);					
					break;
				case "ubah" :					
						uses("server_util_arrayList");
						sql = new server_util_arrayList();
						sql.add("update agg_fs set nama = '"+this.e1.getText()+"',flag_status = '"+status+"',tgl_awal='"+this.d1.getDate()+"',tgl_akhir='"+this.d2.getDate()+"',tgl_input='"+(new Date).getDateStr()+"',nik_user='"+this.app._userLog+"' where kode_fs = '"+this.e0.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
						this.dbLib.execArraySQL(sql);							
					break;
				case "hapus" :
				    uses("server_util_arrayList");
						...
						sql.add("delete from  agg_fs where kode_fs = '"+this.e0.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
						this.dbLib.execArraySQL(sql);											   
					break;
			}
			this.e0.setFocus();
	...
	doEditChange: function(sender){
		...
		if(sender == this.e0){
			if (this.e0.getText() != "")
			{
				try
				{			
					var data = this.dbLib.runSQL("select * from agg_fs where kode_fs = '"+this.e0.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'  ");
