window.app_budget_master_fLoker = function(owner)
{
	if (owner)
	{
		window.app_budget_master_fLoker.prototype.parent.constructor.call(this, owner);
		this.className = "app_budget_master_fLoker";
...
window.app_budget_master_fLoker.extend(window.portalui_childForm);
window.app_budget_master_fLoker.prototype.mainButtonClick = function(sender){
...
window.app_budget_master_fLoker.prototype.doRequestReady = function(sender, methodName, result){
	switch (methodName)
		{
			case "listData" : 
...
window.app_budget_master_fLoker.prototype.doModalResult = function(event, modalResult, value){
	try{
		switch (event){
			case "Add" :
			   if ( modalResult == mrOk){
			...
			case "simpan" :
				if (modalResult == mrOk)
				{
					...
					try
					{						
						var sql2,sql1 = "delete from agg_loker where kode_lokasi = '"+this.elokasi.getText()+"'";					// kode_lokasi,						
						...
						if (value != ""){							
								...
								for (var i in data){
									sql2 = "insert into agg_loker(kode_loker, kode_lokasi,nama, initial,level_spasi, tipe, sum_header, kode_induk, rowindex) values ";
									sql2 += data[i];
									sql.add(sql2);
								}							
						}
...
window.app_budget_master_fLoker.prototype.loadMenu = function(daftarNrc){
	try{
...
//-------------------------------------
window.app_budget_master_fLoker.prototype.doClick = function(sender){	
	var nrc = this.dblib.runSQL("select * from agg_loker where  kode_lokasi = '"+this.elokasi.getText()+"' order by rowindex",0,0);	
	this.loadMenu(nrc);	
};
window.app_budget_master_fLoker.prototype.entriesClick = function(sender){
	try
	{ 
...
window.app_budget_master_fLoker.prototype.treeClick = function(item){
};
window.app_budget_master_fLoker.prototype.getTreeData = function(node){
	var result = "";
	var child = undefined;
...
window.app_budget_master_fLoker.prototype.getSummary = function(node){
	var result = "";
	var child = undefined;
	if (node instanceof portalui_treeView)
	{
...
window.app_budget_master_fLoker.prototype.doFindLokasi = function(sender)
{
	if (this.app._userStatus =="A")
...
window.app_budget_master_fLoker.prototype.setSummaryItems = function(data){
	this.e4.clearItem();
	for (var i in data)
	{
...
window.app_budget_master_fLoker.prototype.copyChilds = function(from, to){
	var temp = undefined;
	...
	{			
		...
		chld.data.set("kode_loker",temp.getKode());					
...
window.app_budget_master_fLoker.prototype.switchItem = function(from, to){
	...
	child.data.set("kode_loker",item.getKode());					
...
window.app_budget_master_fLoker.prototype.moveItem = function(item, owner)
{	
