window.app_budget_master_fInvKlp = function(owner)
{
	if (owner)
	{
		window.app_budget_master_fInvKlp.prototype.parent.constructor.call(this, owner);
		this.className = "app_budget_master_fInvKlp";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Kelpmpok Investasi", 0);	
...
window.app_budget_master_fInvKlp.extend(window.portalui_childForm);
window.app_budget_master_fInvKlp.prototype.mainButtonClick = function(sender){
...
window.app_budget_master_fInvKlp.prototype.doRequestReady = function(sender, methodName, result){
	switch (methodName)
		{
			case "listData" : 
...
window.app_budget_master_fInvKlp.prototype.doModalResult = function(event, modalResult, value){
	try{
		switch (event){
			case "Add" :
			   if ( modalResult == mrOk){
					try{
						...
						data.set("kode_klpinv",this.e0.getText());
			...
			case "simpan" :
				if (modalResult == mrOk)
				{
					...
					try
					{						
						var sql2,sql1 = "delete from agg_inv_klp where kode_lokasi = '"+this.elokasi.getText()+"'";					// kode_lokasi,						
						...
						if (value != ""){							
								...
								for (var i in data){
									sql2 = "insert into agg_inv_klp(kode_klpinv, kode_lokasi,nama, initial,level_spasi, tipe, sum_header, kode_induk, rowindex) values ";
									sql2 += data[i];
									sql.add(sql2);
								}							
						}
...
window.app_budget_master_fInvKlp.prototype.loadMenu = function(daftarNrc){
	try{
		...
		for (var r in menu.objList)
		{
			...
			kode = itemValues.get("kode_klpinv");							
			if (kode != "")
			{
...
//-------------------------------------
window.app_budget_master_fInvKlp.prototype.doClick = function(sender){	
	var nrc = this.dblib.runSQL("select * from agg_inv_klp where  kode_lokasi = '"+this.elokasi.getText()+"' order by rowindex",0,0);	
	this.loadMenu(nrc);	
};
window.app_budget_master_fInvKlp.prototype.entriesClick = function(sender){
	try
	{ 
...
window.app_budget_master_fInvKlp.prototype.treeClick = function(item){
};
window.app_budget_master_fInvKlp.prototype.getTreeData = function(node){
	var result = "";
	var child = undefined;
...
window.app_budget_master_fInvKlp.prototype.getSummary = function(node){
	var result = "";
	var child = undefined;
	if (node instanceof portalui_treeView)
	{
...
window.app_budget_master_fInvKlp.prototype.doFindLokasi = function(sender)
{
	if (this.app._userStatus =="A")
...
window.app_budget_master_fInvKlp.prototype.setSummaryItems = function(data){
	this.e4.clearItem();
	for (var i in data)
	{
...
window.app_budget_master_fInvKlp.prototype.copyChilds = function(from, to){
	var temp = undefined;
	...
	{			
		...
		chld.data.set("kode_klpinv",temp.getKode());					
...
window.app_budget_master_fInvKlp.prototype.switchItem = function(from, to){
	...
	child.data.set("kode_klpinv",item.getKode());					
...
window.app_budget_master_fInvKlp.prototype.moveItem = function(item, owner)
{	
