window.app_budget_master_fNeracaInv = function(owner)
{
	if (owner)
	{
		window.app_budget_master_fNeracaInv.prototype.parent.constructor.call(this, owner);
		this.className = "app_budget_master_fNeracaInv";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Setting Aktifitas Investasi", 0);	
...
window.app_budget_master_fNeracaInv.extend(window.portalui_childForm);
window.app_budget_master_fNeracaInv.implement({
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
						...
						try
						{
							...
							var sql2,sql1 = "delete from agg_neracainv where modul = '"+modul+"'  and kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"'";					// kode_lokasi,
							var sql = new server_util_arrayList();
							sql.add("delete from agg_relakuninv  where kode_lokasi = '"+this.elokasi.getText()+"' and kode_fs = '"+this.eKlp.getText()+"' "+
								"and kode_neraca in (select kode_neraca from agg_neracainv where modul = '"+modul+"'  and kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"')");																		
							sql.add(sql1);							
							//if mysql langsung
							...
							if (driver.search("mssql") != -1){
								...
								for (var i in data){
									sql2 = "insert into agg_neracainv(kode_neraca, kode_lokasi, kode_fs, nama, level_spasi, level_lap, tipe, sum_header, jenis_akun, kode_induk, rowindex, modul) values ";
									sql2 += " "+ data[i] +" ";
									sql.add(sql2);
								}	
								if (this.value2 != ""){
									...
									for (var i in data){
										sql3 = "insert into agg_relakuninv(kode_neraca, kode_fs, kode_akun, kode_lokasi) values ";
										sql3 += " "+ data[i] +" ;";
										sql.add(sql3);															
									}													
								}
								...
								sql.add(sql2);
								if (this.value2 != ""){
										var sql3 = "insert into agg_relakuninv(kode_neraca, kode_fs, kode_akun,  kode_lokasi) values ";								
	...
	loadMenu: function(daftarNrc){
		try
		{
			...
			var item = undefined;
			var node = undefined;
			var line, ix = -1, nrcRls, lineData, nrc_d = this.dblib.loadQuery("select kode_neraca, kode_akun from agg_relakuninv where kode_lokasi ='"+this.elokasi.getText()+"' and kode_fs = '"+this.eKlp.getText()+"' order by kode_neraca");												
	...
	doClick: function(sender){
		if (this.e0.getText() != "")
		{
			...
			var nrc = this.dblib.runSQL("select * from agg_neracainv where Modul = '"+modul+"' and kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"'  order by rowindex",0,0);
			this.loadMenu(nrc);
		}	
	},
		...
		{ 
		 	if (sender == this.relBtn)
			{
				...
				if (this.selectedItem.data.get("tipe").toLowerCase() == "posting"){
					uses("app_budget_master_fRelakun",true);
					if (this.relakun != undefined)
						this.relakun.free();
					this.relakun = new app_budget_master_fRelakun(this.app);
			...
			}else
			{
				if (sender == this.addBtn || sender == this.editBtn){
					uses("app_budget_master_fNeracaInvDetail");
					if (this.entryMenu == undefined)
						this.entryMenu = new app_budget_master_fNeracaInvDetail(this.app,this.e0.getText());
