window.app_budget_master_fStatusFa = function(owner)
{
	if (owner)
	{
		window.app_budget_master_fStatusFa.prototype.parent.constructor.call(this,owner);
		this.className  = "app_budget_master_fStatusFa";
		this.maximize();
...
window.app_budget_master_fStatusFa.extend(window.portalui_childForm);
//------------------------------------------------------------------ -------------------------------------------------------------------------------------------------------------------------------------------------------------
window.app_budget_master_fStatusFa.prototype.mainButtonClick = function(sender)
{
...
window.app_budget_master_fStatusFa.prototype.doModalResult = function(event, modalResult)
{			
	switch (event)
	{
		case "clear" :
		...
		case "simpan" :
			if (modalResult == mrOk)
			{
				...
				{					
					...
					sql.add("insert into agg_fa_status (kode_status,nama,jenis)  values "+
		...
		case "ubah" :
			if (modalResult == mrOk)
			{
					...
					sql.add("update agg_fa_status set  "+
		...
		case "hapus" :
		   if (modalResult == mrOk)
		   {
				uses("server_util_arrayList");					
					var sql = new server_util_arrayList();
					sql.add("delete from agg_fa_status where kode_status='"+this.ed_kode.getText()+"' and jenis= '"+this.cb_jenis.getText().substr(0,1)+"'");
					this.dbLib.execArraySQL(sql);	
		   }
			break;
	}
...
window.app_budget_master_fStatusFa.prototype.showClick = function(sender)
{
	try 
	{
...
window.app_budget_master_fStatusFa.prototype.FindBtnClick = function(sender, event)
{
	try
	{
		if (sender == this.ed_kode) 
		{
			this.standarLib.showListData(this, "Daftar Status Asset",this.ed_kode,undefined, 
										  "select kode_status, nama   from agg_fa_status ",
										  "select count(kode_status)  from agg_fa_status ",
...
window.app_budget_master_fStatusFa.prototype.doRequestReady = function(sender, methodName, result)
{
