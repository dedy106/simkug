	...
	switch (event)
	{
		...
		case "simpan" :
			if (modalResult == mrOk)
			{
				...
				{					
					...
					sql.add("insert into agg_satuan (kode_sat,nama)  values "+
		...
		case "hapus" :
		   if (modalResult == mrOk)
		   {
				uses("server_util_arrayList");					
					var sql = new server_util_arrayList();
					sql.add("delete from agg_satuan where kode_sat='"+this.ed_kode.getText()+"'");
					this.dbLib.execArraySQL(sql);	
		   }
			break;
	}
	...
	if (sender == this.ed_kode) 
	{
		if (this.ed_kode.getText() != "")
		{
			try
			{
				...
				var data = this.dbLib.loadQuery("select a.kode_sat,a.nama from agg_satuan a where a.kode_sat = '"+this.ed_kode.getText()+"'");
			    if (data != undefined) 
			    {
		          if  (data != "") 
				  {
