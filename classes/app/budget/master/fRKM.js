window.app_budget_master_fRKM = function(owner)
{
	if (owner)
	{
		window.app_budget_master_fRKM.prototype.parent.constructor.call(this, owner);
		this.className = "app_budget_master_fRKM";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Setting RKM", 0);	
		this.maximize();
...
window.app_budget_master_fRKM.extend(window.portalui_childForm);
window.app_budget_master_fRKM.implement({
			...
			switch (event)
			{
				...
				case "Add" :
				   if ( modalResult == mrOk)
				   {
						...
						{   
							...
							data.tipe = valArray[2];
							//data.sum_header = valArray[3];	
							//data.jenis_akun = valArray[4];	
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
						...
						try
						{
							var sql1 = "delete from agg_drk2 where  kode_lokasi = '"+this.elokasi.getText()+"' and tahun ="+this.eThn.getText()+"";
							...
							for (var i in data) {
								sql2 = "insert into agg_drk2(kode_drk, nama, level_spasi,tipe, kode_induk, rowindex, kode_pp, tgl_input, nik_user, kode_lokasi, tahun) values ";
								sql2 += " " + data[i] + " ";
								sql.add(sql2);
							}																
	...
	doClick: function(sender){
		kode_drk, tahun, kode_lokasi, tipe, nama, level_spasi
	, kode_induk,rowindex,kode_pp, tgl_input, nik_user
		this.dblib.getDataProviderA("select kode_drk,  nama, level_spasi, tipe, kode_induk, rowindex, kode_pp, tgl_input, nik_user, kode_lokasi, tahun from agg_drk2 where kode_lokasi = '"+this.elokasi.getText()+"' and tahun = "+this.eThn.getText()+" order by rowindex",0,0);					
	},
		...
		{ 
			...
			}else
			{
				if (sender == this.addBtn || sender == this.editBtn){
					uses("app_budget_master_fRKMDetail");
					if (this.entryMenu == undefined)
						this.entryMenu = new app_budget_master_fRKMDetail(this.app);
				...
				}else if (sender == this.editBtn)
				{
					...
					this.entryMenu.e2.setText(item.data.tipe);															
					this.entryMenu.showModal();		
