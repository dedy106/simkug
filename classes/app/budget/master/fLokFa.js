window.app_budget_master_fLokFa = function(owner)
{
	if (owner)
	{
		window.app_budget_master_fLokFa.prototype.parent.constructor.call(this, owner);
		this.className = "app_budget_master_fLokFa";
...
window.app_budget_master_fLokFa.extend(window.portalui_childForm);
window.app_budget_master_fLokFa.implement({
			...
			switch (event)
			{
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
						...
						try
						{						
							var sql2,sql1 = "delete from agg_fa_lokasi where kode_lokasi = '"+this.elokasi.getText()+"'";					// kode_lokasi,						
							...
							if (driver.search("mssql") != -1){
								...
								for (var i in data){
									sql2 = "insert into agg_fa_lokasi(kode_lokfa, nama, kode_lokasi,tipe, rowindex, kode_induk,sum_header, level_lap,level_spasi,  nik_user, tgl_input) values ";
									sql2 += data[i];
									sql.add(sql2);
								}
	...
	doClick: function(sender){	
		var nrc = this.dblib.runSQL("select kode_lokfa, nama, kode_lokasi,tipe, rowindex, kode_induk,sum_header, level_lap,level_spasi from agg_fa_lokasi where  kode_lokasi = '"+this.elokasi.getText()+"' order by rowindex",0,0);	
		this.loadMenu(nrc);
	},
