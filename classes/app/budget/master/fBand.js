 ...
 */
window.app_budget_master_fBand = function(owner)
{
	if (owner)
	{
		window.app_budget_master_fBand.prototype.parent.constructor.call(this,owner);
		this.className  = "app_budget_master_fBand";
		this.setTop(60);
		this.setWidth(1280);
		this.setHeight(550);
...
//------------------------------------------------------------------------
		try{
			...
			this.e0 = new portalui_saiCBBL(this,{bound:[20,21,170,20],caption:"Kode Band", multiSelection:false,change:[this,"doEditChange"]});					
			...
			this.p1 = new portalui_panel(this,{bound:[10,23,500,433],caption:"Daftar Band"});
			this.sg1 = new portalui_saiGrid(this.p1,{bound:[0,20,495,380],tag:9,
					colTitle: ["Kode Band","Nama Band"]});		
			...
			this.e0.setSQL("select kode_band, nama from agg_band ",["kode_band","nama"],false,["Kode Band","Nama"],"where","Data Band",false);
		}catch(e)
		{
			alert(e);
		}
...
window.app_budget_master_fBand.extend(window.portalui_childForm);
//------------------------------------------------------------------ event
window.app_budget_master_fBand.implement({
	...
	doTampilClick: function(sender){
		try{			
			var temp = this.dbLib.runSQL("select kode_band,nama from agg_band order by kode_band");
