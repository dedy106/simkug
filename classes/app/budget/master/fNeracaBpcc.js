window.app_budget_master_fNeracaBpcc = function(owner)
{
	if (owner)
	{
		window.app_budget_master_fNeracaBpcc.prototype.parent.constructor.call(this, owner);
		this.className = "app_budget_master_fNeracaBpcc";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Setting BPCC", 0);	
		...
		this.eKlp.setSQL("select kode_fs, nama from agg_fsbpcc where kode_lokasi = '"+this.app._lokasi+"' ",["kode_fs","nama"],undefined,["Kode FS","Nama"],"and","Data Versi Neraca");	
	}
};
window.app_budget_master_fNeracaBpcc.extend(window.portalui_childForm);
window.app_budget_master_fNeracaBpcc.implement({
			...
			switch (event)
			{
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
						...
						{
							var modul = "B";//
							var sql2,sql1 = "delete from agg_neracabpcc where kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"'";					// kode_lokasi,
							var sql = new server_util_arrayList();
							sql.add("delete from agg_relakunbpcc  where kode_lokasi = '"+this.elokasi.getText()+"' and kode_fs = '"+this.eKlp.getText()+"' "+
								"and kode_neraca in (select kode_neraca from agg_neracabpcc where  kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"')");																		
							sql.add(sql1);							
							//if mysql langsung
							...
							if (driver.search("mssql") != -1){
								...
								for (var i in data){
									sql2 = "insert into agg_neracabpcc(kode_neraca, kode_lokasi, kode_fs, nama, level_spasi, level_lap, tipe, sum_header, jenis_akun, kode_induk, rowindex, modul) values ";
									sql2 += " "+ data[i] +" ";
									sql.add(sql2);
								}	
								if (this.value2 != ""){
									...
									for (var i in data){
										sql3 = "insert into agg_relakunbpcc(kode_neraca, kode_fs, kode_akun, kode_lokasi) values ";
										sql3 += " "+ data[i] +" ;";
										sql.add(sql3);															
									}													
								}
								...
								sql.add(sql2);
								if (this.value2 != ""){
										var sql3 = "insert into agg_relakunbpcc(kode_neraca, kode_fs, kode_akun,  kode_lokasi) values ";								
	...
	loadMenu: function(daftarNrc){
		try
		{
			...
			var item = undefined;
			var node = undefined;
			var line, ix = -1, nrcRls, lineData, nrc_d = this.dblib.loadQuery("select kode_neraca, kode_akun from agg_relakunbpcc where kode_lokasi ='"+this.elokasi.getText()+"' and kode_fs = '"+this.eKlp.getText()+"' order by kode_neraca");												
	...
	doClick: function(sender){
		var nrc = this.dblib.runSQL("select * from agg_neracabpcc where kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"'  order by rowindex",0,0);
		this.loadMenu(nrc);
		...
		{ 
		 	if (sender == this.relBtn)
			{
				...
				if (this.selectedItem.data.get("tipe").toLowerCase() == "posting"){
					uses("app_budget_master_fRelakunGar",true);
					if (this.relakun != undefined)
						this.relakun.free();
					this.relakun = new app_budget_master_fRelakunGar(this.app);
			...
			}else
			{
				if (sender == this.addBtn || sender == this.editBtn){
					uses("app_budget_master_fNeracaGarDetail");
					if (this.entryMenu == undefined)
						this.entryMenu = new app_budget_master_fNeracaGarDetail(this.app);
	...
	doEditChange: function(sender){
		if (sender == this.elokasi){			
			this.eKlp.setText("");
			this.treev.clear();
			this.eKlp.setSQL("select kode_fs, nama from agg_fsbpcc where kode_lokasi = '"+sender.getText()+"' ",["kode_fs","nama"],undefined,["Kode FS","Nama"],"and","Data Versi Neraca");	
