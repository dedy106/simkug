window.app_budget_report_flOutPp = function(owner)
{
	
	if (owner)
	{
		window.app_budget_report_flOutPp.prototype.parent.constructor.call(this,owner);
		this.className = "app_budget_report_flOutPp";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Outlook Per PP", 2);
...
window.app_budget_report_flOutPp.extend(window.portalui_childForm);
window.app_budget_report_flOutPp.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_budget_report_flOutPp.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_budget_report_flOutPp.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.sql =  "select a.kode_akun,b.nama,a.kode_pp,c.nama as nama_pp,sum(a.rka) as rka,sum(a.realisasi) as realisasi,sum(a.outlook) as outlook, "+
						...
						"inner join agg_pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+this.filter+
						"group by a.kode_akun,b.nama,a.kode_pp,c.nama "+
	...
	{
		alert("[app_budget_report_flOutPp]::mainButtonClick:"+e);
	}
};
window.app_budget_report_flOutPp.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_budget_report_flOutPp.prototype.getStringHeader = function(){
...
window.app_budget_report_flOutPp.prototype.doConfirmClick = function(sender){
	try{
...
window.app_budget_report_flOutPp.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_budget_report_flOutPp.prototype.previewReport = function(dthtml)
{
	var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>Laporan Outlook Per PP<br>";			
	var d = new Date();
...
window.app_budget_report_flOutPp.prototype.doCloseReportClick = function(sender)
{
...
window.app_budget_report_flOutPp.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_budget_report_flOutPp.prototype.doRowPerPageChange = function(sender, rowperpage)
{
