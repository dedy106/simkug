window.app_budget_report_flAggBp = function(owner)
{
	if (owner)
	{
		window.app_budget_report_flAggBp.prototype.parent.constructor.call(this,owner);
		this.className = "app_budget_report_flAggBp";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Jenis","=","Detail"));
...
window.app_budget_report_flAggBp.extend(window.portalui_childForm);
window.app_budget_report_flAggBp.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_budget_report_flAggBp.prototype.doSelectCell = function(sender, col, row)
{
	...
	else
	{
		...
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2],[3,0,3]);
	}
	if (row == 1)
	{
		this.sg1.columns.get(2).pickList.clear();
		this.dbLib.setItemsFromSQL("select distinct tahun from agg_bpcc_peserta where kode_lokasi='"+this.app._lokasi+"' order by tahun",[this.sg1.columns.get(2).pickList]);
	}
	if (row == 2)
	{
		this.sg1.columns.get(2).pickList.clear();
		this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Detail","Summary"));
	}
};
window.app_budget_report_flAggBp.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
						  ...
						  this.filterRep.filterStr("d.kode_bidang",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
			if (this.sg1.getCell(2,4)=='Original')
			{
				this.filter2 = this.sg1.getCell(2,4)+"/0"+"/"+this.sg1.getCell(2,1);
			}
			else
			{
				this.filter2 = this.sg1.getCell(2,4)+"/1"+"/"+this.sg1.getCell(2,1);
			}
	...
	{
		alert("[app_budget_report_flAggBp]::mainButtonClick:"+e);
	}
};
window.app_budget_report_flAggBp.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_budget_report_flAggBp.prototype.getStringHeader = function(){
...
window.app_budget_report_flAggBp.prototype.doConfirmClick = function(sender){
	try{
...
window.app_budget_report_flAggBp.prototype.doSelectedPage = function(sender, page)
{
	this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,this.filter, this.page,  this.pager, this.showFilter, this.lokasi,this.filter2));
	this.page = page;
	this.allBtn = false;
};
window.app_budget_report_flAggBp.prototype.previewReport = function(dthtml)
{
...
window.app_budget_report_flAggBp.prototype.doCloseReportClick = function(sender)
{
...
window.app_budget_report_flAggBp.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_budget_report_flAggBp.prototype.doRowPerPageChange = function(sender, rowperpage)
{
