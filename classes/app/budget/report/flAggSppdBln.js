window.app_budget_report_flAggSppdBln = function(owner)
{
	
	try{
	if (owner)
	{
		window.app_budget_report_flAggSppdBln.prototype.parent.constructor.call(this,owner);
		this.className = "app_budget_report_flAggSppdBln";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Rekapitulasi SPPD karyawan", 2);
...
window.app_budget_report_flAggSppdBln.extend(window.portalui_childForm);
window.app_budget_report_flAggSppdBln.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				if (this.app._userStatus=="A")
				{
					this.filter=this.filterRep.filterStr("e.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
								...
								this.filterRep.filterStr("e.kode_pp",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
				...
				if (this.jenisLap == "Bulanan")
				{
					var sql="select e.nik,e.nama,g.nama as nama_jab,e.kode_band,e.kode_pp,h.nama as nama_pp,i.nama as nama_kota,i.idx, "+
							...
							"	   sum(case when substring(a.periode,5,2) between '01' and '01' then a.nilai+a.nilai_harian else 0 end ) as g01, "+
							"	   sum(case when substring(a.periode,5,2) between '02' and '02' then a.nilai+a.nilai_harian else 0 end ) as g02, "+
							"	   sum(case when substring(a.periode,5,2) between '03' and '03' then a.nilai+a.nilai_harian else 0 end ) as g03, "+
							"	   sum(case when substring(a.periode,5,2) between '04' and '04' then a.nilai+a.nilai_harian else 0 end ) as g04, "+
							"	   sum(case when substring(a.periode,5,2) between '05' and '05' then a.nilai+a.nilai_harian else 0 end ) as g05, "+
							"	   sum(case when substring(a.periode,5,2) between '06' and '06' then a.nilai+a.nilai_harian else 0 end ) as g06, "+
							"	   sum(case when substring(a.periode,5,2) between '07' and '07' then a.nilai+a.nilai_harian else 0 end ) as g07, "+
							"	   sum(case when substring(a.periode,5,2) between '08' and '08' then a.nilai+a.nilai_harian else 0 end ) as g08, "+
							"	   sum(case when substring(a.periode,5,2) between '09' and '09' then a.nilai+a.nilai_harian else 0 end ) as g09, "+
							"	   sum(case when substring(a.periode,5,2) between '10' and '10' then a.nilai+a.nilai_harian else 0 end ) as g10, "+
							"	   sum(case when substring(a.periode,5,2) between '11' and '11' then a.nilai+a.nilai_harian else 0 end ) as g11, "+
							"	   sum(case when substring(a.periode,5,2) between '12' and '12' then a.nilai+a.nilai_harian else 0 end ) as g12, "+
							"	   sum(case when substring(a.periode,5,2) between '01' and '12' then a.nilai+a.nilai_harian else 0 end ) as total "+ 
							"	from agg_sppd_d a "+
							"	inner join agg_sppd_m b on a.no_gaji=b.no_gaji and a.kode_lokasi=b.kode_lokasi "+this.filter2+
							"	group by a.nik)f on e.nik=f.nik "+this.filter+" order by e.kode_band";
			...
			systemAPI.alert("[app_budget_report_flAggSppdBln]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		if (this.jenisLap === "Bulanan")
			var header = "LAPORAN REKAPITULASI SPPD KARYAWAN PER BULAN";
		else if (this.jenisLap === "Triwulan")
			var header = "LAPORAN EKAPITULASI SPPD KARYAWAN PER TRIWULAN";
		else if (this.jenisLap === "Semester")
			var header = "LAPORAN REKAPITULASI SPPD KARYAWAN PER SEMESTER";
