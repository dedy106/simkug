window.app_budget_report_flAggCcBln = function(owner)
{
	
	try{
	if (owner)
	{
		window.app_budget_report_flAggCcBln.prototype.parent.constructor.call(this,owner);
		this.className = "app_budget_report_flAggCcBln";
		this.maximize();
...
window.app_budget_report_flAggCcBln.extend(window.portalui_childForm);
window.app_budget_report_flAggCcBln.implement({
	...
	mainButtonClick: function(sender){
		try
		{
				...
				if (this.lap == "Parameter Layanan")
				{
					if (this.jenisLap == "Bulanan")
					{
						var sql="select e.nama, "+
								...
								"   inner join agg_bpcc_param b on a.kode_param=b.kode_param and b.modul='CC' "+this.filter2+
				...
				if (this.lap == "Jenis Layanan")
				{
					if (this.jenisLap == "Bulanan")
					{
						var sql="select e.nama, "+
								...
								"	   sum(case when substring(a.periode,5,2) between '01' and '12' then a.jumlah else 0 end ) as tot_jum "+ 
								"	from agg_bpcc_peserta a "+
								"   inner join agg_bpcc_param b on a.kode_param=b.kode_param and b.modul='CC' "+
			...
			systemAPI.alert("[app_budget_report_flAggCcBln]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var header = "LAPORAN CLAIM COST <br>"+this.lap.toUpperCase()+" "+this.jenisLap.toUpperCase()+" <br>TAHUN "+this.sg1.getCell(2,1);
