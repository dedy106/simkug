window.app_budget_report_flAggEvalAktap = function(owner)
{
	if (owner)
	{
		window.app_budget_report_flAggEvalAktap.prototype.parent.constructor.call(this,owner);
		this.className = "app_budget_report_flAggEvalAktap";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Evaluasi Aktiva Tetap", 2);
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Bisnis Unit","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Jenis ","=","Transaksi"));
...
window.app_budget_report_flAggEvalAktap.extend(window.portalui_childForm);
window.app_budget_report_flAggEvalAktap.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_budget_report_flAggEvalAktap.prototype.doSelectCell = function(sender, col, row)
{
	...
	if (row == 3)
	{
		...
		this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("All","RECURING","TAMBAHAN"));
	}
};
window.app_budget_report_flAggEvalAktap.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.nama_report = "server_report_budget_rptAggEvalAktap";
	...
	{
		alert("[app_budget_report_flAggEvalAktap]::mainButtonClick:"+e);
	}
};
window.app_budget_report_flAggEvalAktap.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_budget_report_flAggEvalAktap.prototype.getStringHeader = function(){
...
window.app_budget_report_flAggEvalAktap.prototype.doConfirmClick = function(sender){
	try{
...
window.app_budget_report_flAggEvalAktap.prototype.doSelectedPage = function(sender, page)
{
	this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,this.filter, this.page,  this.pager, this.showFilter, this.lokasi,this.filter2));
	this.page = page;
	this.allBtn = false;
};
window.app_budget_report_flAggEvalAktap.prototype.previewReport = function(dthtml)
{
...
window.app_budget_report_flAggEvalAktap.prototype.doCloseReportClick = function(sender)
{
...
window.app_budget_report_flAggEvalAktap.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_budget_report_flAggEvalAktap.prototype.doRowPerPageChange = function(sender, rowperpage)
{
