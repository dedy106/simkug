window.app_budget_report_flOutAkun = function(owner)
{
	
	if (owner)
	{
		window.app_budget_report_flOutAkun.prototype.parent.constructor.call(this,owner);
		this.className = "app_budget_report_flOutAkun";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Outlook Per Akun", 2);
...
window.app_budget_report_flOutAkun.extend(window.portalui_childForm);
window.app_budget_report_flOutAkun.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_budget_report_flOutAkun.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_budget_report_flOutAkun.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.filter2=this.filterRep.filterStr("a.tahun",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where")+
				  		this.filterRep.filterStr("a.kode_akun",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
			this.showFilter = this.filterRep.showFilter(this.sg1);
			if (this.sg1.getCell(2,0) == this.app._kodeLokasiKonsol)
			{
				this.sql =  "select a.kode_akun,b.nama,sum(a.rka) as rka,sum(a.realisasi) as realisasi,sum(a.outlook) as outlook, "+
							"		sum(case when a.rka<>0 then convert(decimal(5,2),(a.outlook/a.rka)*100,0) else 0 end) as persen	"+
							"from agg_outlook a "+
							"inner join agg_masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+this.filter2+
			...
			else
			{
				this.sql =  "select a.kode_akun,b.nama,sum(a.rka) as rka,sum(a.realisasi) as realisasi,sum(a.outlook) as outlook, "+
							"		sum(case when a.rka<>0 then convert(decimal(5,2),(a.outlook/a.rka)*100,0) else 0 end) as persen	"+
							"from agg_outlook a "+
							"inner join agg_masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+this.filter+
							"group by a.kode_akun,b.nama "+
							"order by a.kode_akun ";
				this.scriptSqlCount = "select count(a.kode_akun) "+
								"from agg_outlook a "+
			...
			title.add("Nama Akun");width.add(400);fieldType.add("S");
			...
			this.summary = new server_util_arrayList({items:["N","N","Y","Y","Y","N"]});
	...
	{
		alert("[app_budget_report_flOutAkun]::mainButtonClick:"+e);
	}
};
window.app_budget_report_flOutAkun.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_budget_report_flOutAkun.prototype.getStringHeader = function(){
...
window.app_budget_report_flOutAkun.prototype.doConfirmClick = function(sender){
	try{
...
window.app_budget_report_flOutAkun.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_budget_report_flOutAkun.prototype.previewReport = function(dthtml)
{
	var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>Laporan Outlook Per Akun<br>";			
	var d = new Date();
...
window.app_budget_report_flOutAkun.prototype.doCloseReportClick = function(sender)
{
...
window.app_budget_report_flOutAkun.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_budget_report_flOutAkun.prototype.doRowPerPageChange = function(sender, rowperpage)
{
