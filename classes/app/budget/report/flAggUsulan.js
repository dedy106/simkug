window.app_budget_report_flAggUsulan = function(owner)
{
	if (owner)
	{
		window.app_budget_report_flAggUsulan.prototype.parent.constructor.call(this,owner);
		this.className = "app_budget_report_flAggUsulan";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Rekapitulasi Usulan Anggaran", 2);
...
window.app_budget_report_flAggUsulan.extend(window.portalui_childForm);
window.app_budget_report_flAggUsulan.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_budget_report_flAggUsulan.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_budget_report_flAggUsulan.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.filter2 = this.app._nikUser+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,4);
			...
			sql="call sp_agg_serap ('"+this.sg1.getCell(2,3)+"','"+this.sg1.getCell(2,2)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.app._nikUser+"')";
			this.dbLib.execQuerySync(sql);	
			this.nama_report = "server_report_budget_rptAggUsulan";
	...
	{
		alert("[app_budget_report_flAggUsulan]::mainButtonClick:"+e);
	}
};
window.app_budget_report_flAggUsulan.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_budget_report_flAggUsulan.prototype.getStringHeader = function(){
...
window.app_budget_report_flAggUsulan.prototype.doConfirmClick = function(sender){
	try{
...
window.app_budget_report_flAggUsulan.prototype.doSelectedPage = function(sender, page)
{
	this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,this.filter, this.page,  this.pager, this.showFilter, this.lokasi,this.filter2));
	this.page = page;
	this.allBtn = false;
};
window.app_budget_report_flAggUsulan.prototype.previewReport = function(dthtml)
{
...
window.app_budget_report_flAggUsulan.prototype.doCloseReportClick = function(sender)
{
...
window.app_budget_report_flAggUsulan.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_budget_report_flAggUsulan.prototype.doRowPerPageChange = function(sender, rowperpage)
{
