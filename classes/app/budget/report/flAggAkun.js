window.app_budget_report_flAggAkun = function(owner)
{
	
	if (owner)
	{
		window.app_budget_report_flAggAkun.prototype.parent.constructor.call(this,owner);
		this.className = "app_budget_report_flAggAkun";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Rekapitulasi RKA Per Akun", 2);
...
window.app_budget_report_flAggAkun.extend(window.portalui_childForm);
window.app_budget_report_flAggAkun.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_budget_report_flAggAkun.prototype.doSelectCell = function(sender, col, row)
{
	...
	{
		if (this.app._lokasi == this.app._lokKonsol) {
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["23","3","123","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,2,0,0]);
		}
		else {
...
window.app_budget_report_flAggAkun.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
				  ...
				  this.filterRep.filterStr("a.kode_akun",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+this.filter2;
				  
			this.showFilter = this.filterRep.showFilter(this.sg1);
			if (this.sg1.getCell(2,3)=="Bulanan")
			{
				if (this.app._lokasi == this.app._lokKonsol) {
					this.sql =  "select a.kode_akun,f.nama as nama_akun,zz.kode_lokasi,zz.nama as nama_lokasi, "+
					...
					this.sql =  "select a.kode_akun,f.nama as nama_akun, "+
							"	   isnull(g.agg_01,0) as agg_01,isnull(g.agg_02,0) as agg_02,isnull(g.agg_03,0) as agg_03,isnull(g.agg_04,0) as agg_04,"+
							"	   isnull(g.agg_05,0) as agg_05,isnull(g.agg_06,0) as agg_06,isnull(g.agg_07,0) as agg_07,isnull(g.agg_08,0) as agg_08,"+
							"      isnull(g.agg_09,0) as agg_09,isnull(g.agg_10,0) as agg_10,isnull(g.agg_11,0) as agg_11,isnull(g.agg_12,0) as agg_12,isnull(g.agg_total,0) as agg_total "+
							"from (select a.kode_lokasi,a.tahun,a.kode_akun,a.progress "+	
							"	  from agg_d a "+this.filter+
							"group by a.kode_lokasi,a.tahun,a.kode_akun,a.progress ) a  "+
							"inner join agg_masakun f on a.kode_akun=f.kode_akun and a.kode_lokasi=f.kode_lokasi "+
				...
				if (this.app._lokasi == "00") this.summary = new server_util_arrayList({items:["N","N","N","N","Y","Y","Y","Y","Y","Y","Y","Y","Y","Y","Y","Y","Y"]});
	...
	{
		alert("[app_budget_report_flAggAkun]::mainButtonClick:"+e);
	}
};
window.app_budget_report_flAggAkun.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_budget_report_flAggAkun.prototype.getStringHeader = function(){
...
window.app_budget_report_flAggAkun.prototype.doConfirmClick = function(sender){
	try{
...
window.app_budget_report_flAggAkun.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_budget_report_flAggAkun.prototype.previewReport = function(dthtml)
{
	var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>Laporan Rekapitulasi RKA Per Akun<br>"+" Tahun "+this.sg1.getCell(2,1);			
	var d = new Date();
...
window.app_budget_report_flAggAkun.prototype.doCloseReportClick = function(sender)
{
...
window.app_budget_report_flAggAkun.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_budget_report_flAggAkun.prototype.doRowPerPageChange = function(sender, rowperpage)
{
