window.app_budget_report_flAggRka = function(owner)
{
	if (owner)
	{
		window.app_budget_report_flAggRka.prototype.parent.constructor.call(this,owner);
		this.className = "app_budget_report_flAggRka";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Rekapitulasi RKA Per RKA", 2);
	...
	if (this.app._lokasi == "00")  this.tahun='2011';		
	else this.tahun=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from agg_d where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode RKA","All",""));
	...
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Kode Bidang",this.tanda,this.bidang));
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Status program ","=","All"));
	if (this.app._lokasi == "00") this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2,3), new Array("Kode Lokasi","Range","01","99"));
...
window.app_budget_report_flAggRka.extend(window.portalui_childForm);
window.app_budget_report_flAggRka.prototype.doEllipseClick= function(sender, col, row)
{
		if (row == 0)
		{
		...
		if (row == 5)
		{
			if (this.app._userStatus=="A") {
				...
				this.filterRep.ListDataSGFilter(this, "Daftar Bidang",this.sg1, this.sg1.row, this.sg1.col,
													  "select kode_bidang, nama from agg_bidang where kode_bidang='"+this.bidang+"' ",
													  "select count(kode_bidang) from agg_bidang where kode_bidang='"+this.bidang+"'   ",
													  ["kode_bidang","nama"],"where",["Kode","Nama"]);
			}
		}
};
window.app_budget_report_flAggRka.prototype.doSelectCell = function(sender, col, row)
{
	if (this.app._userStatus=="A")
	{
	...
	{
		if (this.app._lokasi == "00") {
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["123","3","123","3","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,0,2,0,0,2]);
		}
		else {
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["3","3","123","3","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[3,0,2,0,0,2]);
		}
	}
	if (row == 1)
	{
...
window.app_budget_report_flAggRka.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			{
				this.jenis_agg=" and a.jenis_agg='T' ";
			}
			...
			this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
				  ...
				  this.filterRep.filterStr("a.kode_rka",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+this.filter2;
			this.filter3 = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
				  this.filterRep.filterStr("a.tahun",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
				  this.filterRep.filterStr("a.kode_rka",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
				  this.filterRep.filterStr("b.kode_bidang",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+this.jenis_agg+this.filter2;
	  
			this.showFilter = this.filterRep.showFilter(this.sg1);
			if (this.sg1.getCell(2,3)=="Bulanan")
			{
				if (this.app._lokasi == "00") {
					this.sql =  "select a.kode_rka,f.nama as nama_akun,zz.kode_lokasi,zz.nama as nama_lokasi, "+
							...
							"      isnull(sum(g.agg_09),0) as agg_09,isnull(sum(g.agg_10),0) as agg_10,isnull(sum(g.agg_11),0) as agg_11,isnull(sum(g.agg_12),0) as agg_12,"+
							"      isnull(sum(g.agg_total),0) as agg_total  "+
							"from (select a.kode_lokasi,a.tahun,a.kode_rka,a.progress "+	
							"	  from agg_d a "+
							"     inner join agg_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter3+
							"group by a.kode_lokasi,a.tahun,a.kode_rka,a.progress ) a  "+
							"inner join agg_rka f on a.kode_rka=f.kode_rka "+
							"inner join lokasi zz on a.kode_lokasi=zz.kode_lokasi "+
							"left join ( "+
							"	 select a.kode_lokasi,a.kode_rka, "+
							...
							"	 from agg_d a "+
							"     inner join agg_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter3+
							"	 group by a.kode_lokasi,a.kode_rka "+
							"		 )g on a.kode_lokasi=g.kode_lokasi and a.kode_rka=g.kode_rka "+this.filter+
							"group by a.kode_rka,f.nama,zz.kode_lokasi,zz.nama order by a.kode_rka,zz.kode_lokasi";						
				}
				else {
					this.sql =  "select a.kode_rka,f.nama as nama_akun, "+
				...
				title.add("Kode RKA");width.add(60);fieldType.add("S");
				title.add("Rincian Kegiatan");width.add(300);fieldType.add("S");
				
				if (this.app._lokasi == "00") {
					title.add("Kode Lokasi");width.add(60);fieldType.add("S");
					title.add("Nama Lokasi");width.add(200);fieldType.add("S");
				}
				...
				else this.summary = new server_util_arrayList({items:["N","N","Y","Y","Y","Y","Y","Y","Y","Y","Y","Y","Y","Y","Y"]});
			...
			if (this.sg1.getCell(2,3)=="Triwulan")
			{
				if (this.app._lokasi == "00") {
					this.sql =  "select a.kode_rka,f.nama as nama_akun,zz.kode_lokasi,zz.nama as nama_lokasi, "+
							"	  isnull(sum(g.agg_01),0) as agg_01,isnull(sum(g.agg_02),0) as agg_02,isnull(sum(g.agg_03),0) as agg_03,isnull(sum(g.agg_04),0) as agg_04,isnull(sum(g.agg_total),0) as agg_total "+
							"from (select a.kode_lokasi,a.tahun,a.kode_rka,a.progress "+	
							"	  from agg_d a "+
							"     inner join agg_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter3+
							"group by a.kode_lokasi,a.tahun,a.kode_rka,a.progress ) a  "+
				...
				var fieldType = new server_util_arrayList();
				title.add("Kode RKA");width.add(60);fieldType.add("S");
				title.add("Rincian Kegiatan");width.add(250);fieldType.add("S");
				if (this.app._lokasi == "00") {
					title.add("Kode Lokasi");width.add(60);fieldType.add("S");
					title.add("Nama Lokasi");width.add(200);fieldType.add("S");
				}
				...
				if (this.app._lokasi == "00") this.summary = new server_util_arrayList({items:["N","N","N","N","Y","Y","Y","Y","Y","Y"]});
				else this.summary = new server_util_arrayList({items:["N","N","Y","Y","Y","Y","Y","Y"]});

			}
			if (this.sg1.getCell(2,3)=="Semester")
			{
				if (this.app._lokasi == "00") {
					this.sql =  "select a.kode_rka,f.nama as nama_akun,zz.kode_lokasi,zz.nama as nama_lokasi, "+
							"	  isnull(sum(g.agg_01),0) as agg_01,isnull(sum(g.agg_02),0) as agg_02,isnull(sum(g.agg_total),0) as agg_total "+
							"from (select a.kode_lokasi,a.tahun,a.kode_rka,a.progress "+	
							"	  from agg_d a "+
							"     inner join agg_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter3+
							"group by a.kode_lokasi,a.tahun,a.kode_rka,a.progress ) a  "+
				...
				if (this.app._lokasi == "00") this.summary = new server_util_arrayList({items:["N","N","N","N","Y","Y","Y"]});
				else this.summary = new server_util_arrayList({items:["N","N","Y","Y","Y"]});

			}
			this.scriptSqlCount = "select count(a.kode_rka) "+
						"from (select a.kode_lokasi,a.tahun,a.kode_rka,a.progress "+	
						"	  from agg_d a "+
						"     inner join agg_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter3+
						"group by a.kode_lokasi,a.tahun,a.kode_rka,a.progress ) a  "+
			...
			if (this.sg1.getCell(2,2).toUpperCase() != "ALL")
				this.nama=this.dbLib.getPeriodeFromSQL("select nama as periode from agg_bidang where kode_bidang='"+this.sg1.getCell(2,5)+"'");
			else this.nama= "";
	...
	{
		alert("[app_budget_report_flAggRka]::mainButtonClick:"+e);
	}
};
window.app_budget_report_flAggRka.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_budget_report_flAggRka.prototype.getStringHeader = function(){
...
window.app_budget_report_flAggRka.prototype.doConfirmClick = function(sender){
	try{
...
window.app_budget_report_flAggRka.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_budget_report_flAggRka.prototype.previewReport = function(dthtml)
{
	var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'>"+this.app._namalokasi.toUpperCase()+"<br>Laporan Rekapitulasi Anggaran Per RKA<br>"+this.nama+" Tahun "+this.sg1.getCell(2,1)  ;			
	var d = new Date();
...
window.app_budget_report_flAggRka.prototype.doCloseReportClick = function(sender)
{
...
window.app_budget_report_flAggRka.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_budget_report_flAggRka.prototype.doRowPerPageChange = function(sender, rowperpage)
{
