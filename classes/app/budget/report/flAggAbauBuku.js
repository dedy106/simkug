window.app_budget_report_flAggAbauBuku = function(owner)
{
	if (owner)
	{
		window.app_budget_report_flAggAbauBuku.prototype.parent.constructor.call(this,owner);
		this.className = "app_budget_report_flAggAbauBuku";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Buku RKA", 2);
	...
	this.gridLib.SGEditData(this.sg1,7,new Array(0,1,2), new Array("Jenis","=","Buku RKA"));
	this.gridLib.SGEditData(this.sg1,8,new Array(0,1,2), new Array("Status program ","=","All"));
...
window.app_budget_report_flAggAbauBuku.extend(window.portalui_childForm);
window.app_budget_report_flAggAbauBuku.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_budget_report_flAggAbauBuku.prototype.doSelectCell = function(sender, col, row)
{
	if (this.app._userStatus=="A")
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["123","3","13","13","3","123","123","3","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[2,0,2,2,0,2,2,0,0]);
	}
	else
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["3","3","3","13","3","123","3","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[3,0,3,2,0,2,2,0,0]);
	}
	if (row == 1)
	{
	...
	{
		this.sg1.columns.get(2).pickList.clear();
		this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Buku RKA","Buku RKA Modul"));
	}
	if (row == 8)
	{
		this.sg1.columns.get(2).pickList.clear();
...
window.app_budget_report_flAggAbauBuku.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.jenis_agg="";
			if (this.sg1.getCell(2,8)=="Baru")
			{
				this.jenis_agg=" and a.jenis_agg='T' ";
			}
			if (this.sg1.getCell(2,8)=="Lama")
			{
				this.jenis_agg=" and a.jenis_agg<>'T' ";
			}
			this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
						  ...
						  this.filterRep.filterStr("a.kode_rka",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and")+
			...
			{
				this.nama_bidang=this.dbLib.getPeriodeFromSQL("select nama as periode from agg_bidang where kode_bidang='"+this.sg1.getCell(2,2)+"' ");	
			}
			if (this.sg1.getCell(2,4)=='Original')
			{
				this.filter2 = this.sg1.getCell(2,4)+"/0"+"/"+this.sg1.getCell(2,1)+"/"+this.nama_bidang;
			}
			else
			{
				this.filter2 = this.sg1.getCell(2,4)+"/1"+"/"+this.sg1.getCell(2,1)+"/"+this.nama_bidang;
			}
			
			this.showFilter = this.filterRep.showFilter(this.sg1);
			if (this.sg1.getCell(2,7)=='Buku RKA')
			{
				this.nama_report = "server_report_budget_rptAggAbauBuku";
			}
			else
			{
				this.nama_report = "server_report_budget_rptAggAbauBukuModul";
			}
	...
	{
		alert("[app_budget_report_flAggAbauBuku]::mainButtonClick:"+e);
	}
};
window.app_budget_report_flAggAbauBuku.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_budget_report_flAggAbauBuku.prototype.getStringHeader = function(){
...
window.app_budget_report_flAggAbauBuku.prototype.doConfirmClick = function(sender){
	try{
...
window.app_budget_report_flAggAbauBuku.prototype.doSelectedPage = function(sender, page)
{
	this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,this.filter, this.page,  this.pager, this.showFilter, this.lokasi,this.filter2));
	this.page = page;
	this.allBtn = false;
};
window.app_budget_report_flAggAbauBuku.prototype.previewReport = function(dthtml)
{
...
window.app_budget_report_flAggAbauBuku.prototype.doCloseReportClick = function(sender)
{
...
window.app_budget_report_flAggAbauBuku.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_budget_report_flAggAbauBuku.prototype.doRowPerPageChange = function(sender, rowperpage)
{
