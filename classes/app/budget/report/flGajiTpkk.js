window.app_budget_report_flGajiTpkk = function(owner)
{
	if (owner)
	{
		window.app_budget_report_flGajiTpkk.prototype.parent.constructor.call(this,owner);
		this.className = "app_budget_report_flGajiTpkk";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Gaji TPKK ", 2);
...
window.app_budget_report_flGajiTpkk.extend(window.portalui_childForm);
window.app_budget_report_flGajiTpkk.implement({
	...
	initColumn: function(){
		try{		
            var data = this.dbLib.getDataProvider("select kode_param, kode_param+'-'+nama as nama, jns_periode  from agg_param where tahun = '"+this.sg1.cells(2,1)+"' and jenis = 'TPKK' order by no_urut",true);
