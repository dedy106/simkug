window.app_budget_report_flAggBa = function(owner)
{
	if (owner)
	{
		window.app_budget_report_flAggBa.prototype.parent.constructor.call(this,owner);
		this.className = "app_budget_report_flAggBa";
		this.maximize();
	...
	this.tahun=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from agg_ba ");
...
window.app_budget_report_flAggBa.extend(window.portalui_childForm);
window.app_budget_report_flAggBa.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_budget_report_flAggBa.prototype.doSelectCell = function(sender, col, row)
{
	if (this.app._userStatus=="A")
	{
		if (this.app._lokKonsol==this.app._lokasi)
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,],["2","3"]);
	...
	if (row == 1)
	{
		...
		this.dbLib.setItemsFromSQL("select distinct tahun from agg_ba where kode_lokasi='"+this.app._lokasi+"' order by tahun",[this.sg1.columns.get(2).pickList]);
	}
	
};
window.app_budget_report_flAggBa.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.nama_report = "server_report_budget_rptBa";
	...
	{
		alert("[app_budget_report_flAggBa]::mainButtonClick:"+e);
	}
};
window.app_budget_report_flAggBa.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_budget_report_flAggBa.prototype.getStringHeader = function(){
...
window.app_budget_report_flAggBa.prototype.doConfirmClick = function(sender){
	try{
...
window.app_budget_report_flAggBa.prototype.doSelectedPage = function(sender, page)
{
	this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,this.filter, this.page,  this.pager, this.showFilter, this.lokasi,this.filter2));
	this.page = page;
	this.allBtn = false;
};
window.app_budget_report_flAggBa.prototype.previewReport = function(dthtml)
{
...
window.app_budget_report_flAggBa.prototype.doCloseReportClick = function(sender)
{
...
window.app_budget_report_flAggBa.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_budget_report_flAggBa.prototype.doRowPerPageChange = function(sender, rowperpage)
{
