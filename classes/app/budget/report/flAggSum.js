window.app_budget_report_flAggSum = function(owner)
{
	if (owner)
	{
		window.app_budget_report_flAggSum.prototype.parent.constructor.call(this,owner);
		this.className = "app_budget_report_flAggSum";
		this.maximize();
...
window.app_budget_report_flAggSum.extend(window.portalui_childForm);
window.app_budget_report_flAggSum.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_budget_report_flAggSum.prototype.doSelectCell = function(sender, col, row)
{
	if (this.app._userStatus=="A")
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["123","3","3","13","3","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,0,0,2,0,0]);
	}
	else
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["3","3","3","13","3","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[3,0,0,2,0,0]);
	}
	if (row == 1)
	{
...
window.app_budget_report_flAggSum.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.filter2 = this.app._nikUser+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(2,5)+"/"+this.ver;
			...
			sql="call sp_agg_rkap ('"+this.sg1.getCell(2,3)+"','"+this.ver+"','"+this.sg1.getCell(2,2)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.app._nikUser+"')";
	...
	{
		alert("[app_budget_report_flAggSum]::mainButtonClick:"+e);
	}
};
window.app_budget_report_flAggSum.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_budget_report_flAggSum.prototype.getStringHeader = function(){
...
window.app_budget_report_flAggSum.prototype.doConfirmClick = function(sender){
	try{
...
window.app_budget_report_flAggSum.prototype.doSelectedPage = function(sender, page)
{
	this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,this.filter, this.page,  this.pager, this.showFilter, this.lokasi,this.filter2));
	this.page = page;
	this.allBtn = false;
};
window.app_budget_report_flAggSum.prototype.previewReport = function(dthtml)
{
...
window.app_budget_report_flAggSum.prototype.doCloseReportClick = function(sender)
{
...
window.app_budget_report_flAggSum.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_budget_report_flAggSum.prototype.doRowPerPageChange = function(sender, rowperpage)
{
