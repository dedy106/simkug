window.app_budget_report_flOutLokasi = function(owner)
{
	if (owner)
	{
		window.app_budget_report_flOutLokasi.prototype.parent.constructor.call(this,owner);
		this.className = "app_budget_report_flOutLokasi";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Outlook Per Lokasi", 2);
	...
	{
		this.tahun=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from agg_outlook ");
	}
	else
	{
...
window.app_budget_report_flOutLokasi.extend(window.portalui_childForm);
window.app_budget_report_flOutLokasi.prototype.doEllipseClick= function(sender, col, row)
{
		...
		if (row == 2)
		{
			this.filterRep.ListDataSGFilter(this, "Daftar Versi Anggaran",this.sg1, this.sg1.row, this.sg1.col,
													  "select kode_fs, nama from agg_fs where kode_lokasi = '"+this.app._lokasi+"' ",
													  "select count(kode_fs) from agg_fs  where kode_lokasi = '"+this.app._lokasi+"' ",
													  ["kode_fs","nama"],"and",["Kode","Nama"]);
		}
		
};
window.app_budget_report_flOutLokasi.prototype.doSelectCell = function(sender, col, row)
{
	...
	if (row == 1)
	{
		...
		this.dbLib.setItemsFromSQL("select distinct tahun from agg_outlook where kode_lokasi='"+this.app._lokasi+"' order by tahun",[this.sg1.columns.get(2).pickList]);
	}
	if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
...
window.app_budget_report_flOutLokasi.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.filter2 = this.app._nikUser+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,3);
			...
			sql="call sp_agg_outlook ('"+this.sg1.getCell(2,2)+"','"+this.sg1.getCell(2,1)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.app._nikUser+"')";
			
			this.dbLib.execQuerySync(sql);	
			this.nama_report = "server_report_budget_rptOutlook";
	...
	{
		alert("[app_budget_report_flOutLokasi]::mainButtonClick:"+e);
	}
};
window.app_budget_report_flOutLokasi.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_budget_report_flOutLokasi.prototype.getStringHeader = function(){
...
window.app_budget_report_flOutLokasi.prototype.doConfirmClick = function(sender){
	try{
...
window.app_budget_report_flOutLokasi.prototype.doSelectedPage = function(sender, page)
{
	this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,this.filter, this.page,  this.pager, this.showFilter, this.lokasi,this.filter2));
	this.page = page;
	this.allBtn = false;
};
window.app_budget_report_flOutLokasi.prototype.previewReport = function(dthtml)
{
...
window.app_budget_report_flOutLokasi.prototype.doCloseReportClick = function(sender)
{
...
window.app_budget_report_flOutLokasi.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_budget_report_flOutLokasi.prototype.doRowPerPageChange = function(sender, rowperpage)
{
