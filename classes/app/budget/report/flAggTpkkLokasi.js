window.app_budget_report_flAggTpkkLokasi = function(owner)
{
	
	try{
	if (owner)
	{
		window.app_budget_report_flAggTpkkLokasi.prototype.parent.constructor.call(this,owner);
		this.className = "app_budget_report_flAggTpkkLokasi";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Rekapitulasi Gaji TPKK Per Lokasi", 2);
...
window.app_budget_report_flAggTpkkLokasi.extend(window.portalui_childForm);
window.app_budget_report_flAggTpkkLokasi.implement({
		...
		try
		{
			...
			else
			{
				...
				var sql="select e.kode_param,e.nama, "+
						...
						"	inner join agg_gajitpkk_m b on a.no_gaji=b.no_gaji "+this.filter2+
						"	group by a.kode_param)f on e.kode_param=f.kode_param "+this.filter+" and e.jenis='TPKK' "+
			...
			systemAPI.alert("[app_budget_report_flAggTpkkLokasi]::mainButtonClick:"+e);
		}
	},
