window.app_budget_report_flAggDrk = function(owner)
{
	if (owner)
	{
		window.app_budget_report_flAggDrk.prototype.parent.constructor.call(this,owner);
		this.className = "app_budget_report_flAggDrk";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Rekapitulasi RKA Per DRK", 2);
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode DRK","All",""));
...
window.app_budget_report_flAggDrk.extend(window.portalui_childForm);
window.app_budget_report_flAggDrk.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_budget_report_flAggDrk.prototype.doSelectCell = function(sender, col, row)
{
	if (this.app._userStatus=="A")
	{
	...
	else
	{
		...
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[3,0,2,0,0,3]);
	}
	if (row == 1)
	{
...
window.app_budget_report_flAggDrk.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		{ 
			this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Kode Lokasi","=",this.app._lokasi));
			this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Tahun","=",this.tahun));
			this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode PK","All",""));
			this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Periodik Laporan","=","Bulanan"));
		...
		else
		{
			...
			this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
				  ...
				  this.filterRep.filterStr("a.kode_drk",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+this.filter2;
			this.filter3 = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
				  ...
				  this.filterRep.filterStr("b.kode_bidang",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+this.filter2;
	  
			this.showFilter = this.filterRep.showFilter(this.sg1);
			if (this.sg1.getCell(2,3)=="Bulanan")
			{
				this.sql =  "select a.kode_drk,f.nama as nama_akun, "+
						...
						"from (select a.kode_lokasi,a.tahun,a.kode_drk,a.progress "+	
						"	  from agg_d a "+
						"     inner join agg_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter3+
						"group by a.kode_lokasi,a.tahun,a.kode_drk,a.progress ) a  "+
						"inner join agg_drk f on a.kode_drk=f.kode_drk "+
						"left join ( "+
						"	 select a.kode_lokasi,a.kode_drk, "+
						...
						"	 from agg_d a "+
						"     inner join agg_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter3+
						"	 group by a.kode_lokasi,a.kode_drk "+
						"		 )g on a.kode_lokasi=g.kode_lokasi and a.kode_drk=g.kode_drk "+this.filter+
						"order by a.kode_drk";	
				var title = new server_util_arrayList();			
				var width = new server_util_arrayList();
				var fieldType = new server_util_arrayList();
				title.add("Kode DRK");width.add(60);fieldType.add("S");
				title.add("Rencana Kegiatan");width.add(250);fieldType.add("S");
			...
			this.scriptSqlCount = "select count(a.kode_drk) "+
						"from (select a.kode_lokasi,a.tahun,a.kode_drk,a.progress "+	
						"	  from agg_d a "+
						"     inner join agg_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter3+
						"group by a.kode_lokasi,a.tahun,a.kode_drk,a.progress ) a  "+
	...
	{
		alert("[app_budget_report_flAggDrk]::mainButtonClick:"+e);
	}
};
window.app_budget_report_flAggDrk.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_budget_report_flAggDrk.prototype.getStringHeader = function(){
...
window.app_budget_report_flAggDrk.prototype.doConfirmClick = function(sender){
	try{
...
window.app_budget_report_flAggDrk.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_budget_report_flAggDrk.prototype.previewReport = function(dthtml)
{
	var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>Laporan Rekapitulasi RKA Per DRK <br>"+this.nama;			
	var d = new Date();
...
window.app_budget_report_flAggDrk.prototype.doCloseReportClick = function(sender)
{
...
window.app_budget_report_flAggDrk.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_budget_report_flAggDrk.prototype.doRowPerPageChange = function(sender, rowperpage)
{
