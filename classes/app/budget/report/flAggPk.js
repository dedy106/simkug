window.app_budget_report_flAggPk = function(owner)
{
	if (owner)
	{
		window.app_budget_report_flAggPk.prototype.parent.constructor.call(this,owner);
		this.className = "app_budget_report_flAggPk";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Rekapitulasi RKA Per PK", 2);
...
window.app_budget_report_flAggPk.extend(window.portalui_childForm);
window.app_budget_report_flAggPk.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_budget_report_flAggPk.prototype.doSelectCell = function(sender, col, row)
{
	if (this.app._userStatus=="A")
	{
...
window.app_budget_report_flAggPk.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
				  ...
				  this.filterRep.filterStr("a.kode_pk",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+this.filter2;
			this.filter3 = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
				  this.filterRep.filterStr("a.tahun",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
				  this.filterRep.filterStr("a.kode_pk",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
			...
			{
				this.sql =  "select a.kode_pk,f.nama as nama_akun, "+
						...
						"from (select a.kode_lokasi,a.tahun,a.kode_pk,a.progress "+	
						"	  from agg_d a "+
						"     inner join agg_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter3+
						"group by a.kode_lokasi,a.tahun,a.kode_pk,a.progress ) a  "+
						"inner join agg_pk f on a.kode_pk=f.kode_pk  "+
						"left join ( "+
						"	 select a.kode_lokasi,a.kode_pk, "+
						...
						"	 from agg_d a "+
						"     inner join agg_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter3+
						"	 group by a.kode_lokasi,a.kode_pk "+
						"		 )g on a.kode_lokasi=g.kode_lokasi and a.kode_pk=g.kode_pk "+this.filter+
						"order by a.kode_pk";	
				...
				title.add("Program Kerja");width.add(200);fieldType.add("S");
			...
			if (this.sg1.getCell(2,3)=="Triwulan")
			{
				...
				var fieldType = new server_util_arrayList();
				title.add("Kode PK");width.add(60);fieldType.add("S");
				title.add("Program Kerja");width.add(250);fieldType.add("S");
			...
			this.scriptSqlCount = "select coumt(a.kode_pk) "+
						"from (select a.kode_lokasi,a.tahun,a.kode_pk,a.progress "+	
						"	  from agg_d a "+
						"     inner join agg_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter3+
						"group by a.kode_lokasi,a.tahun,a.kode_pk,a.progress ) a  "+
	...
	{
		alert("[app_budget_report_flAggPk]::mainButtonClick:"+e);
	}
};
window.app_budget_report_flAggPk.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_budget_report_flAggPk.prototype.getStringHeader = function(){
...
window.app_budget_report_flAggPk.prototype.doConfirmClick = function(sender){
	try{
...
window.app_budget_report_flAggPk.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_budget_report_flAggPk.prototype.previewReport = function(dthtml)
{
	var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>Laporan Rekapitulasi RKA Per PK<br>"+this.nama+" Tahun "+this.sg1.getCell(2,1);			
	var d = new Date();
...
window.app_budget_report_flAggPk.prototype.doCloseReportClick = function(sender)
{
...
window.app_budget_report_flAggPk.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_budget_report_flAggPk.prototype.doRowPerPageChange = function(sender, rowperpage)
{
