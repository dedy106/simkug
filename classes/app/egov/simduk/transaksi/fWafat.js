window.app_egov_simduk_transaksi_fWafat = function(owner)
{
	if (owner)
	{
		window.app_egov_simduk_transaksi_fWafat.prototype.parent.constructor.call(this,owner);
		this.className  = "app_egov_simduk_transaksi_fWafat";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Akte Kematian: Input/Koreksi", 0);	
		...
		this.cb_wafat = new portalui_saiCBB(this,{bound:[20,19,300,20],caption:"No Akta Kematian",btnClick:[this,"doBtnClick"],btnRefreshClick:[this,"doLoadData"],tag:1,maxLength:50});		
		this.l_tgl1 = new portalui_label(this,{bound:[20,20,100,18],caption:"Tanggal Wafat", underline:true});
		this.dp_d1 = new portalui_datePicker(this,{bound:[120,20,100,18],date:new Date().getDateStr()});
		...
		this.e_ayah = new portalui_saiLabelEdit(this,{bound:[20,25,300,20],caption:"Nama Ayah", tag:4, readOnly:true});		
		this.e_ibu = new portalui_saiLabelEdit(this,{bound:[570,25,300,20],caption:"Nama Ibu", tag:4, readOnly:true});				
		this.e_sebab = new portalui_saiLabelEdit(this,{bound:[20,26,300,20],caption:"Sebab Kematian", maxLength:150, tag:4});		
		this.e_goldar = new portalui_saiLabelEdit(this,{bound:[570,26,180,20],caption:"Gol. Darah | Rhesus", tag:4, readOnly:true});						
		this.e_tempat2 = new portalui_saiLabelEdit(this,{bound:[20,27,300,20],caption:"Tempat Kejadian", maxLength:150, tag:4});		
		this.e_saksi1 = new portalui_saiLabelEdit(this,{bound:[20,28,300,20],caption:"Saksi 1", maxLength:50, tag:4});		
		this.e_saksi2 = new portalui_saiLabelEdit(this,{bound:[20,29,300,20],caption:"Saksi 2", maxLength:50, tag:4});		
...
window.app_egov_simduk_transaksi_fWafat.extend(window.portalui_childForm);
window.app_egov_simduk_transaksi_fWafat.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,3,4])){
				try{									
					...
					sql.add("insert into egov_wafat (no_wafat ,tanggal ,no_akte ,sebab ,tempat,saksi1,saksi2,tgl_input ,nik_user) values "+
							"                       ('"+this.cb_wafat.getText()+"' ,'"+this.dp_d1.getDateString()+"' ,'"+this.cb_akte.getText()+"' ,'"+this.e_sebab.getText()+"','"+this.e_tempat2.getText()+"','"+this.e_saksi1.getText()+"','"+this.e_saksi2.getText()+"',now(),'"+this.app._userLog+"')");
					sql.add("update egov_kk_d set no_wafat='"+this.cb_wafat.getText()+"',sts_aktif='W' where no_akte='"+this.cb_akte.getText()+"' and no_kk='"+this.cb_kode.getText()+"' ");
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,3,4])){
				try{									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update egov_kk_d set no_wafat='-',sts_aktif='1' where no_akte='"+this.akteLama+"' and no_kk='"+this.cb_kode.getText()+"' ");
					sql.add("delete from egov_wafat where no_wafat='"+this.cb_wafat.getText()+"'");
					sql.add("insert into egov_wafat (no_wafat ,tanggal ,no_akte ,sebab ,tempat,saksi1,saksi2,tgl_input ,nik_user) values "+
							"                       ('"+this.cb_wafat.getText()+"' ,'"+this.dp_d1.getDateString()+"' ,'"+this.cb_akte.getText()+"' ,'"+this.e_sebab.getText()+"','"+this.e_tempat2.getText()+"','"+this.e_saksi1.getText()+"','"+this.e_saksi2.getText()+"',now(),'"+this.app._userLog+"')");
					sql.add("update egov_kk_d set no_wafat='"+this.cb_wafat.getText()+"',sts_aktif='W' where no_akte='"+this.cb_akte.getText()+"' and no_kk='"+this.cb_kode.getText()+"' ");
					this.dbLib.execArraySQL(sql);
	...
	doLoadData: function(sender){
		try{
			...
			if (sender == this.cb_wafat) {
				if (this.cb_wafat.getText() != ""){
					var data = this.dbLib.getDataProvider("select date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.sebab,a.tempat as tempat2,a.saksi1,a.saksi2, "+
					           " b.no_akte,b.nama,date_format(b.tgl_lahir,'%d/%m/%Y') as tgl_lahir,b.tempat,b.nama_ayah,b.nama_ibu,b.gender,concat(b.gol_darah,' | ',b.rhesus) as goldar "+
							   " from egov_wafat a inner join egov_akte b on a.no_akte=b.no_akte "+
						       " where a.no_wafat ='"+this.cb_wafat.getText()+"'");
					eval("data = "+data+";");
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
						if (line != undefined){
							...
							this.e_tempat2.setText(line.tempat2);
							this.e_saksi1.setText(line.saksi1);
							this.e_saksi2.setText(line.saksi2);
	...
	doBtnClick: function(sender, event){
		try{
			...
			if (sender == this.cb_wafat) {   
			    this.standarLib.showListData(this, "Daftar Nomor Akta Kematian",sender,undefined, 
											  "select a.no_wafat, b.nama  from egov_wafat a inner join egov_akte b on a.no_akte=b.no_akte inner join egov_kk_d c on b.no_akte=c.no_akte and c.no_kk='"+this.cb_kode.getText()+"'",
											  "select count(a.no_wafat)   from egov_wafat a inner join egov_akte b on a.no_akte=b.no_akte inner join egov_kk_d c on b.no_akte=c.no_akte and c.no_kk='"+this.cb_kode.getText()+"'",
											  ["no_wafat","nama"],"and",["No Akta","Nama"],false);				
