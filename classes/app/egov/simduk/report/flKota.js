window.app_egov_simduk_report_flKota = function(owner)
{
	if (owner)
	{
		window.app_egov_simduk_report_flKota.prototype.parent.constructor.call(this,owner);
		this.className = "app_egov_simduk_report_flKota";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Kabupaten/Kotamadya", 2);
...
window.app_egov_simduk_report_flKota.extend(window.portalui_childForm);
window.app_egov_simduk_report_flKota.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2){
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_propinsi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.kode_kota",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new portalui_arrayMap();
				var sql = "select a.kode_kota,a.nama,b.nama "+
					" from egov_kota a inner join egov_propinsi b on a.kode_propinsi=b.kode_propinsi "+this.filter+
					" order by a.kode_kota";
				this.scriptSqlCount = "select count(*) "+
								" from egov_kota a inner join egov_propinsi b on a.kode_propinsi=b.kode_propinsi "+this.filter;
				this.title = new server_util_arrayList({items:["Kode","Kabupaten/Kotamadya","Provinsi"]});
				this.widthTable = new server_util_arrayList({items:[80,250,250]});
			...
			systemAPI.alert("[app_egov_simduk_report_flKota]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>DAFTAR KABUPATEN/KOTAMADYA<br>";
		var d = new Date();
	  ...
	  switch(sender.getName())
	  {
	    ...
	    case "pdfBtn" :      
		  var html = new server_util_arrayList();
			...
			html.add("pdf");			
			html.add(new Date().valueOf()+"_kabupaten");				
