window.app_egov_simduk_report_flKecamatan = function(owner)
{
	if (owner)
	{
		window.app_egov_simduk_report_flKecamatan.prototype.parent.constructor.call(this,owner);
		this.className = "app_egov_simduk_report_flKecamatan";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Kecamatan", 2);
...
window.app_egov_simduk_report_flKecamatan.extend(window.portalui_childForm);
window.app_egov_simduk_report_flKecamatan.implement({
		...
		try
		{
			if (sender === this.app._mainForm.bClear2){
		    	...
		    	this.filter = this.filterRep.filterStr("c.kode_propinsi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_kecamatan",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new portalui_arrayMap();
				var sql = "select a.kode_kecamatan,a.nama,b.nama,c.nama "+
					" from egov_kecamatan a inner join egov_kota b on a.kode_kota=b.kode_kota "+
					" inner join egov_propinsi c on b.kode_propinsi=c.kode_propinsi "+this.filter+
					" order by a.kode_kecamatan";
				this.scriptSqlCount = "select count(*) "+
								" from egov_kecamatan a inner join egov_kota b on a.kode_kota=b.kode_kota "+
								" inner join egov_propinsi c on b.kode_propinsi=c.kode_propinsi "+this.filter;
				this.title = new server_util_arrayList({items:["Kode","Kecamatan","Kabupaten/Kotamadya","Provinsi"]});
				this.widthTable = new server_util_arrayList({items:[80,200,200,200]});
			...
			systemAPI.alert("[app_egov_simduk_report_flKecamatan]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>DAFTAR KECAMATAN<br>";
		var d = new Date();
	  ...
	  switch(sender.getName())
	  {
	    ...
	    case "pdfBtn" :      
		  var html = new server_util_arrayList();
			...
			html.add("pdf");			
			html.add(new Date().valueOf()+"_kecamatan");				
