...
//* 			SAI, PT											
//***********************************************************************************************
window.app_egov_fUser = function(owner)
{
	if (owner)
	{
		try
		{
			window.app_egov_fUser.prototype.parent.constructor.call(this, owner);
			this.className = "app_egov_fUser";
		...
		{
			alert("[app_egov_fUser]::constructor:"+e);
		}
		
		this.setTabChildIndex();
	}
};
window.app_egov_fUser.extend(window.portalui_childForm);
window.app_egov_fUser.prototype.doClick = function(sender){
...
window.app_egov_fUser.prototype.doEditChange = function(sender)
{
	setTipeButton(tbSimpan);
	if (sender == this.e0)
	{		
...
window.app_egov_fUser.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_egov_fUser.prototype.doFindClick = function(sender)
{
	try{
		if (sender == this.e5) {
			if (this.app._userStatus != "A" ) 
...
window.app_egov_fUser.prototype.mainButtonClick = function(sender)
{
...
window.app_egov_fUser.prototype.doModalResult = function(event, modalResult, value)
{
