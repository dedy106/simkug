...
//* 			SAI, PT											
//***********************************************************************************************
window.app_egov_fMenu = function(owner){
	if (owner){
		try{
			window.app_egov_fMenu.prototype.parent.constructor.call(this, owner);
			this.className = "app_egov_fMenu";
...
window.app_egov_fMenu.extend(window.portalui_childForm);
window.app_egov_fMenu.implement({
	...
	entriesClick: function(sender){
		try{
			uses("app_egov_fMenuDetail");	
			if (this.entryMenu !== undefined) this.entryMenu.free();
			this.entryMenu = new app_egov_fMenuDetail(this.app);
