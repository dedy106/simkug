window.app_rasapala_fBayar = function(owner)
{
	if (owner)
	{
		window.app_rasapala_fBayar.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rasapala_fBayar";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pembayaran Hutang Apotek: Input", 0);	
		...
		this.cb_akun = new portalui_saiCBBL(this,{bound:[20,17,200,20],caption:"Akun KasBank",tag:2, multiSelection:false,
		...
		this.cb_pph = new portalui_saiCBBL(this,{bound:[20,16,200,20],caption:"Akun PPh",tag:2, multiSelection:false,
			sql:["select kode_akun, nama from masakun where kode_lokasi ='"+this.app._lokasi+"' ", ["kode_akun","nama"],false,["Kode","Nama"],"and","Daftar Akun",true]
		});		
...
window.app_rasapala_fBayar.extend(window.portalui_childForm);
window.app_rasapala_fBayar.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,"+
							...
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_desc.getText()+"','"+this.app._kodePP+"','KB_APT','"+this.cJenis.getText()+"','"+this.e_periode.getText()+
					...
					sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
								...
								"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDate()+"',0,'"+this.cb_akun.getText()+
					...
					if (this.e_pph.getText() != "0") {
						sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
								"kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+	
								"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDate()+"',1,'"+this.cb_pph.getText()+
					...
					for (var i=0; i < this.sg.rows.getLength(); i++){
						if (this.sg.cells(0,i)=="BAYAR" && this.sg.rowValid(i)) {
							sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
									"kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+	
									"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDate()+"',"+idx+",'"+this.sg.cells(5,i)+
							...
							sql.add("insert into kas_d(no_kas,no_bukti,modul,catatan,no_del,kode_lokasi,nilai) values "+
							  	    "                 ('"+this.e_nb.getText()+"','"+this.sg.cells(1,i)+"','SPBAPT','-','-','"+this.app._lokasi+"',"+parseNilai(this.sg.cells(6,i))+")");
							sql.add("update spb_m set progress='3' where no_spb='"+this.sg.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
							idx++;
						}
					}
					var idx =999;					
					if (this.sg2.getRowValidCount() > 0){
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
										"kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+	
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDate()+"',"+idx+",'"+this.sg2.cells(0,i)+
	...
	doTampilClick: function(sender){
		try{			
			if (this.e_periode.getText() != ""){
				var data = this.dbLib.getDataProvider("select a.no_spb,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.kode_terima+' - '+b.nama as penerima,a.keterangan,a.nilai,a.akun_hutang "+
													  "from  spb_m a inner join vendor b on a.kode_terima=b.kode_vendor and a.kode_lokasi=b.kode_lokasi "+
													  "where a.nilai<>0 and a.progress='2' and a.no_del='-' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'");				
				eval("data = "+data+";");
				if (typeof data == "object"){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg.appendData(["BELUM",line.no_spb,line.tanggal,line.penerima,line.keterangan,line.akun_hutang,floatToNilai(line.nilai),"0"]);
					}
					this.sg.validasi();
				}
			}
			else {
				system.alert(this,"Periode tidak valid.","Periode harus terisi dengan Tahun dan Bulan transaksi.");
			}
		}catch(e){
			systemAPI.alert(e);
		}
