window.app_rasapala_report_flSpbSaldo = function(owner)
{
	if (owner)
	{
		window.app_rasapala_report_flSpbSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_rasapala_report_flSpbSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from spb_m ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Akun Hutang","All",""));
	...
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Jumlah Hari","=",""));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_rasapala_report_flSpbSaldo.extend(window.childForm);
window.app_rasapala_report_flSpbSaldo.implement({
		...
		try{			
			...
			if (row == 3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Vendor",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_vendor, nama  from vendor a where kode_lokasi='"+this.app._lokasi+"'",
												"select kode_vendor, nama  from vendor a where kode_lokasi='"+this.app._lokasi+"'",
												new Array("a.kode_vendor","a.nama"),"and",new Array("Kode Vendor","Nama"),"and");
			}	
			if (row ==4)
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.akun_hutang",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.kode_terima",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filterRep.ListDataSGFilter(this, "Data Hutang",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_spb,a.keterangan from spb_m a "+this.filter,
												"select count(a.no_spb) from spb_m a "+this.filter,
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["3","23","123","123","123","3","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[2,0,2,2,2,1,0]);	
		if (row == 1)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				var sql = "select a.no_spb,date_format(a.tanggal,'%d/%m/%Y') as tanggal,date_format(a.due_date,'%d/%m/%Y') as due_date,b.nama as vendor,a.akun_hutang,d.nama as nama_akun,a.jenis,a.nilai,isnull(c.nilai,0) as nilai_bayar, "+
						"	   a.nilai-isnull(c.nilai,0) as saldo,datediff (day,date_format('"+tgl_aging+"','%d/%m/%Y'),a.due_date)  as tgl "+
						"from spb_m a "+
						"inner join vendor b on a.kode_terima=b.kode_vendor and a.kode_lokasi=b.kode_lokasi "+
						"inner join masakun d on a.akun_hutang=d.kode_akun and a.kode_lokasi=d.kode_lokasi "+
						"left join (select a.no_bukti,sum(a.nilai) as nilai "+
						"		   from kas_d a "+
						"		   inner join kas_m b on a.no_kas=b.no_kas "+
						"	   where b.tanggal<=date_format('"+tgl_aging+"','%d/%m/%Y') "+
						"		   group by a.no_bukti)c on a.no_spb=c.no_bukti "+
						this.filter+" order by a.no_spb ";
			
				this.scriptSqlCount = "select count(a.no_spb) "+
						"from spb_m a "+
						"inner join vendor b on a.kode_terima=b.kode_vendor and a.kode_lokasi=b.kode_lokasi "+
						"inner join masakun d on a.akun_hutang=d.kode_akun and a.kode_lokasi=d.kode_lokasi "+this.filter;
				
				this.title = new server_util_arrayList({items:["No SPB","Tgl Tagihan","Tgl Jatuh Tempo","Vendor","Akun Hutang","Nama Akun","Jenis","Nilai SPB","Nilai Bayar","Saldo","Hari"]});
				this.widthTable = new server_util_arrayList({items:[80,60,60,150,60,150,50,80,80,50]});
				this.fieldType = new server_util_arrayList({items:["S","S","S","S","S","S","S","N","N","N","N"]});																
				this.summary = new server_util_arrayList({items:["N","N","N","N","N","N","Y","Y","Y","N"]});
			...
			systemAPI.alert("[app_rasapala_report_flSpbSaldo]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN SALDO HUTANG<br>Posisi : "+this.sg1.getCell(2,1)+"<br />";
		var d = new Date();
