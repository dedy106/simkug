window.app_officeair_fFiling = function(owner, options){
	if (owner){
		window.app_officeair_fFiling.prototype.parent.constructor.call(this,owner, options);
		...
		this.tFile = new portalui_fileExplorer(this,{bound:[10,31,300,this.height - 60],showFile:true, select:[this,"doSelectFile"], dataReady:[this,"doDataReady"],dblClick:[this,"doTreeClick"]});
		this.lvFile = new portalui_listView(this,{bound:[310,31,this.width - 330,this.height - 60], showIcon:true, dblClick:[this,"doDblClick"]});
		this.fileUtil = new util_file(undefined);		
		this.fileUtil.addListener(this);
		this.dbLib = new util_dbLib();
		this.dbLib.addListener(this);
		this.rootDir = this.tFile.rootDir;		
		this.fileUtil.setFilename(this.rootDir + "/classes/app/officeair/document/" + window.dataLogin.email);		
		this.rootPath = this.rootDir + "/classes/app/officeair/document/" + window.dataLogin.email;		
		if (!this.fileUtil.isDir()){
			this.fileUtil.createDir();
		}else{
			this.tFile.setUsrRoot(window.dataLogin.email,"classes/app/officeair/document/" + window.dataLogin.email,"My Document");
		}		
		this.initload();
	}
};
window.app_officeair_fFiling.extend(portalui_roundPanel);
window.app_officeair_fFiling.implement({
	...
	doClick: function(sender){
		if (sender == this.bClose || sender == this.bMin){
			this.hide();
		}else {
		}
	},
			...
			switch(data){
				case "FOLDER":
					if (this.pNew === undefined){
						this.pNew = new app_officeair_fUpload(this.owner,{bound:[130,50,400,100],caption:"New Folder"},this);
				...
				case "FILE":
					if (this.pNew === undefined){
						this.pNew = new app_officeair_fUpload(this.owner,{bound:[130,50,400,100],caption:"New File"},this);					
					}
				...
				case "SHARE":					
					if (this.pShare === undefined){						
						this.pShare = new app_officeair_fShare(this.owner,{bound:[130,50,400,100],caption:"Share Folder"},this);	
					}					
...
window.app_officeair_fUpload = function(owner, options, parentPanel){
	if (owner){
		window.app_officeair_fUpload.prototype.parent.constructor.call(this,owner,options);
		this.setWidth(400);
		this.setHeight(100);
		this.centerize();
		...
		this.uploader = new portalui_uploader(this,{bound:[320,30,70,20],onChange:[this,"doChange"],param1:"uploadTo",param2:"classes/app/officeair/document/",afterUpload:[this,"doAfterUpload"]});
...
window.app_officeair_fUpload.extend(portalui_panel);
window.app_officeair_fUpload.implement({
	doClick: function(sender){
		try{
...
window.app_officeair_fShare = function(owner, options, parentPanel){
	if (owner){
		try{
			window.app_officeair_fShare.prototype.parent.constructor.call(this,owner,options);
			this.setWidth(400);
			this.setHeight(100);
			this.centerize();
...
window.app_officeair_fShare.extend(portalui_panel);
window.app_officeair_fShare.implement({
	doClick: function(sender){
		try{
			if (sender == this.bClose || sender == this.bMin || sender == this.bCancel){
				this.hide();
