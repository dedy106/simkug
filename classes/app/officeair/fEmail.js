window.app_officeair_fEmail = function(owner,options){
	if (owner){
		try{
			window.app_officeair_fEmail.prototype.parent.constructor.call(this,owner,options);
			this.className  = "app_officeair_fEmail";			
			this.addStyle("border:2px solid #ffffff");	
			uses("portalui_uploader;portalui_saiCBBL;portalui_saiCB;portalui_saiGrid;util_file;portalui_bevel;portalui_richTextArea;portalui_datePicker;server_util_mail");
			//---------------------- input
			this.pInput = new portalui_panel(this,{bound:[0,50,this.width - 20, this.height-90]});
			this.eSubject = new portalui_saiLabelEdit(this.pInput,{bound:[10,30,500,20],caption:"<font color='#ffffff'>Subject</font>"});			
			...
			//------------------------browse
			this.pBrowse = new portalui_panel(this,{bound:[0,50,this.width - 20, this.height-90], visible:false});
			...
			this.sgData = new portalui_saiGrid(this.pBrowse,{bound:[10,30,this.width - 40,this.height - 130],colCount:4,colTitle:["Dari","Subject","Tanggal","Content"],readOnly:true,
				colWidth:[[3,2,1,0],[450,100,250,250]], dblClick:[this,"doSGDblClick"]});
			...
			this.mail.setUser(window.dataLogin.email,"nankninknonk","tls");
...
window.app_officeair_fEmail.extend(window.portalui_roundPanel);
window.app_officeair_fEmail.implement({
	...
	show: function(){
		window.app_officeair_fEmail.prototype.parent.show.call(this);
		this.formEvent = "save";
		this.pInput.hide();
		this.pBrowse.show();
		this.reloadData();
					...
					case "inbox" :								
						if (result instanceof portalui_arrayMap){
							...
							for (var i in result.objList){
								line = result.get(i);
								this.sgData.appendData([line.get("From"),line.get("Subject"),line.get("Date"),line.get("Body")]);								
							}
