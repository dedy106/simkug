...
//* 			SAI, PT											
//***********************************************************************************************
window.app_builder_component_controls_rssReader = function(owner,options){
    if (owner)
    {
        this.caption = "";
		window.app_builder_component_controls_rssReader.prototype.parent.constructor.call(this, owner,options);		
        this.className = "portalui_rssReader";
        this.owner = owner;
		...
		this.addProperty({className:this.className, caption:this.caption,color:this.bgColor, borderStyle:this.border, site:""});	
		this.addEvent({change:""});
    }
};
window.app_builder_component_controls_rssReader.extend(window.app_builder_component_controls_containerControl);
//---------------------------- Function ----------------------------------------
window.app_builder_component_controls_rssReader.implement({
	draw: function(canvas){
		window.app_builder_component_controls_rssReader.prototype.parent.draw.call(this, canvas);
	    ...
	    if (document.all)
	        html =  "<div style='{position: absolute; left: 0; top: 0; width: 100%; height: 100%;}' >" +
					...
					"<div id='" + n + "_refresh' style='{cursor:pointer;position: absolute;left: 0;top: 2; width:20; height: 20;background-image:url(icon/"+system.getThemes()+"/refresh.png);background-position:top left;background-repeat:no-repeat;}'> </div>"+
					"<div id='" + n + "_content' frameborder='0' style='{position: absolute; left: 0; top: 20; width: 100%; height: 100%;overflow:auto}' ></div>"+
					"<div id='" + n + "_loading' style='{background-color:#4d7795;background-image:url(image/gridload.gif);background-position: center;background-repeat: no-repeat;position:absolute;left: 0;top: 0; width: 100%; height: 100%;color:#ff8a00}'></div>"+
	        ...
	        html =  "<div id='" + n + "_bottom' style='{background: url(icon/"+system.getThemes()+"/panelShadow.png) bottom left repeat-x; position: absolute; left: 0; top: 0; width:100%; height: 100%;display:none}' ></div>" +					
					...
					"<div id='" + n + "_icon' style='{background-image:url(icon/rss_16.png);background-position:top left;background-repeat:no-repeat;position: absolute;left: 5;top: 2; width:16; height: 16;display:'> </div>"+					
	                "<div id='" + n + "form' style='{position: absolute; left: 0; top: 0; width: 100%; height: 100%}' ></div>"+
					"<div id='" + n + "_refresh' style='{cursor:pointer;position: absolute;left: 0;top: 0; width:20; height: 20;background:url(icon/"+system.getThemes()+"/refresh.png)no-repeat}'> </div>"+
					"<div id='" + n + "_content' frameborder='0' style='{position: absolute; left: 0; top: 20; width: 100%; height: 100%;overflow:auto}' ></div>"+					
					"<div id='" + n + "_loading' style='{background-color:#4d7795;background:url(image/gridload.gif) center no-repeat;position:absolute;left: 0;top: 0; width: 100%; height: 100%;color:#ff8a00}'></div>"+						
	...
	setWidth: function(data){
		window.app_builder_component_controls_rssReader.prototype.parent.setWidth.call(this, data);		
		if (this.btnRefresh !== undefined) this.btnRefresh.style.left = data - 30;
	},
	setHeight: function(data){
		window.app_builder_component_controls_rssReader.prototype.parent.setHeight.call(this, data);		
