...
//* 			SAI, PT											
//***********************************************************************************************
window.app_builder_component_controls_richTextArea = function(owner,options){
    if (owner){        
		window.app_builder_component_controls_richTextArea.prototype.parent.constructor.call(this, owner,options);		
        this.className = "portalui_richTextArea";
        this.owner = owner;		
		...
		this.addEvent({keyPress:"", keyDown:""});
    }
};
window.app_builder_component_controls_richTextArea.extend(window.app_builder_component_controls_control);
//---------------------------- Function ----------------------------------------
window.app_builder_component_controls_richTextArea.implement({
	doDraw: function(canvas){
	    ...
	    var html =  "<div style='{position: absolute; left: 0; top: 0; width: 100%; height: 100%;border:1px solid #555}' >" +
						...
						"<div id='"+ n +"format3' class='rtedropdown7' style='border:1px solid #002D96;background-color:#FFFFFF;font-family:Arial, Helvetica, sans-serif;cursor:default;position:absolute;display:none;z-index:10'>"+
							...
							"</div>"+
						"<textarea id='" + n + "_hidden' style='{visibility:hidden;display:none;position: absolute; left: 0; top: 0; width: 100%; height: 100%;}' "+					
						"></textarea>"+				
	...
	eventKeyDown: function (event, res){
		try{					
			var obj = window.parent.system.getResource(res);
	...
	display: function (){
		try{
		    if(this.isSupported()){		        		
				this.wysiwyg_content.contentWindow.document.designMode='On';					
				if (document.all){									
					this.wysiwyg_content.contentWindow.document.onkeydown = new Function("return system.getResource("+this.resourceId+").eventKeyDown(event,"+this.resourceId+")");
					this.wysiwyg_content.contentWindow.document.onkeypress = new Function("return system.getResource("+this.resourceId+").eventKeyPress(event,"+this.resourceId+")");
				}else{												
					var thedoc = this.wysiwyg_content.contentWindow.document;					
					thedoc.open();						        
					thedoc.close();				
					this.wysiwyg_content.contentWindow.document.addEventListener("keydown", new Function("event","return system.getResource("+this.resourceId+").eventKeyDown(event,"+this.resourceId+")"), true);
					this.wysiwyg_content.contentWindow.document.addEventListener("keypress", new Function("event","return system.getResource("+this.resourceId+").eventKeyPress(event,"+this.resourceId+")"), true);
				}		        
		    }else{
	...
	setHeight: function(data){
		window.app_builder_component_controls_richTextArea.prototype.parent.setHeight.call(this, data);
		this.wysiwyg_content.style.height = data - (document.all ? 32: 30);	
	},
	setWidth: function(data){
		window.app_builder_component_controls_richTextArea.prototype.parent.setWidth.call(this, data);
	...
	rteGetOffsetTop: function (elm) { 
		...
		while(mOffsetParent && mOffsetParent != this.getContainer()) { 
