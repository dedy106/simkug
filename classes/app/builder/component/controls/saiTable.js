...
//* 			SAI, PT											
//***********************************************************************************************
window.app_builder_component_controls_saiTable = function(owner, options){
	if (owner){
		window.app_builder_component_controls_saiTable.prototype.parent.constructor.call(this, owner, options);	
		...
		this.addProperty({className:this.className, tableWidth:this.tableWidth, html:"", color:this.color});	
		this.addEvent({validasi:"", selectCell :"", dblClick:""});
	}
};
window.app_builder_component_controls_saiTable.extend(window.app_builder_component_controls_control);
window.app_builder_component_controls_saiTable.implement({
	...
	setWidth: function(data){
		window.app_builder_component_controls_saiTable.prototype.parent.setWidth.call(this, data);	
		var cnv = $(this.getFullId()+"_frame");
		cnv.style.width = data;
		this.topRight.style.left = data - 17;
		if (this.layout !== undefined)
			this.layout.setLeft((data / 2) - 150 + this.left);
	},
	setHeight: function(data){
		window.app_builder_component_controls_saiTable.prototype.parent.setHeight.call(this, data);	
		var cnv = $(this.getFullId()+"_frame");
	...
	setData: function(data,system, rowPersystem){		
		try{
			this.data = data;	
			if (this.data instanceof portalui_arrayMap){
				if (system == undefined)
					...
					var start = (system - 1) * rowPersystem;
					var last = start + rowPersystem;
	...
	setTop: function(data){
		window.app_builder_component_controls_saiTable.prototype.parent.setTop.call(this,data);
		if (this.layout !== undefined) this.layout.setTop(data);
	},
	setLeft: function(data){
		window.app_builder_component_controls_saiTable.prototype.parent.setLeft.call(this,data);
