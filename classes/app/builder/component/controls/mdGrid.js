...
//* 			SAI, PT											
//***********************************************************************************************
window.app_builder_component_controls_mdGrid = function(owner, options){
	try{
		this.selectedItem = undefined;			
		this.height = 200;
		this.widthColNo = 20;
		window.app_builder_component_controls_mdGrid.prototype.parent.constructor.call(this, owner, options);	
		this.className = "portalui_mdGrid";
		this.readOnly = true;
		this.colCount = 0;
		this.rowCount = 0;
		...
		this.onExpand = new portalui_eventHandler();
		...
		uses("portalui_mdGridItem");
		uses("portalui_column");
		//------------
		this.first = false; //eventKeyPress
		this.rightBtn = false;
		this.addProperty({className:this.className,colCount:0, rowCount:0, readOnly:true});	
		this.addEvent({dblClick:"",click:"",selectCell:"",keyDown:"",keyPress:"",cellExit:"",expand:"",nilaiChange:"",ellipsClick:""});
	}catch(e){
		alert("[mdGrid]::contructor:"+e);
	}
};
window.app_builder_component_controls_mdGrid.extend(window.app_builder_component_controls_containerControl);
window.app_builder_component_controls_mdGrid.implement({
	...
	doKeyDown: function(keyCode, buttonState){
		...
		var text = rowObj.getCellValue(this.col);
		if (keyCode == 9)
		{
			var col = undefined;
			if (this.col == this.columns.getLength() -1 )
				this.setRowIndex(this.row + 1);
			else if (rowObj.isLastNotReadOnly(this.col))			
			{
				if (this.row + 1  < this.rows.getLength())
					this.setRowIndex(this.row + 1);				
				else if (this.row + 1  == this.rows.getLength())
				{
					...
					if (len > 1)
					{
						var Obj = this.rows.get(len - 1);
						if ( Obj != undefined)
						{
							var input = $(Obj.getFullId()+"cell0_input");
							if ((input != undefined) && (input.value ==""))
							{
								rowObj.setFirstFocus();
								return false;
							}
						}
					}				
					...
					this.setRowIndex(0);								
				rowObj = this.rows.objList[this.row];
				rowObj.setFirstFocus();
			}else		
				for (var i = 0; i < this.columns.getLength();i++)
				{
					col = this.columns.get(i);
					if ((!col.readOnly) && (i > this.col))
					{
						var obj = $(rowObj.getFullId()+"cell"+i+"_input");
						if (obj != undefined)
							obj.focus();
						break;
					}
		...
		this.onKeyDown.call(this, keyCode, buttonState);
	},
	...
	addRowValues: function(parent, values){
		try{
			...
			step = "create rowobj";
			uses("portalui_mdGridItem");
			var rowObj = new portalui_mdGridItem(parent,this, len);		
			
			step = "cek parent";
			if (parent instanceof portalui_mdGridItem)
	...
	clear: function(){
		try{
			...
			for (var i in this.childs.objList)
			{
				...
				if (node != undefined)
					headerNo.removeChild(node);
				if (child instanceof portalui_mdGridItem)
					child.free();
				j++;
			}	
			this.rows.clear();
...
//-----------------------------------			
	...
	doSelectItem: function(item){
	    ...
	    if (oldSelected instanceof portalui_mdGridItem)
	        oldSelected.doDeselect();
	    if (item instanceof portalui_mdGridItem)
	...
	topMouseUp: function(event){
		try{
			...
			alert("[portalui_mdGrid]:topMouseUp:"+e);
		}
	},
	appendRow: function(parent){
		...
		this.rowCount++;
		this.setProperty("rowCount",this.rowCount);
	},
	setHeight: function(data){
		window.app_builder_component_controls_mdGrid.prototype.parent.setHeight.call(this,data);
		if (this.owner instanceof portalui_mdGridItem) this.owner.setDetailHeight(data+2);
	}
});