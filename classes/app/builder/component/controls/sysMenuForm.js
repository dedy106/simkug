...
//* 			SAI, PT											
//***********************************************************************************************
window.app_builder_component_controls_sysMenuForm = function(owner){
    if (owner){
		this.className = "portalui_sysMenuForm";
        window.app_builder_component_controls_sysMenuForm.prototype.parent.constructor.call(this, owner);        
        this.setWidth(200);
        this.setHeight(30);
...
window.app_builder_component_controls_sysMenuForm.extend(window.app_builder_component_controls_commonForm);
window.app_builder_component_controls_sysMenuForm.implement({
	...
	addItem: function(menuItem){
		try{
		    ...
		    var html = "<div id='" + n + "' style='{cursor: pointer;position: relative;  width: 100%; height: 20;overflow: visible}' "+
	...
	popUp: function(x, y, pointMode){
		try{
		    if (this.parentForm == undefined)
		    {
		        ...
		        if (!(form instanceof portalui_sysMenuForm))
		...
		try{
		    ...
		    if (item != undefined)
		    {
		        ...
		        {              
		            ...
		            if (node != undefined)
		                node.style.color = "#ffffff"; //window.system.getConfig("text.normalColor");
					step = "lastitem hide child "+lastItem;	
	...
	doDeactivate: function(){
		try{
			window.app_builder_component_controls_sysMenuForm.prototype.parent.doDeactivate.call(this);
		    var app = this.getApplication();
	...
	setParentForm: function(data){
	    if (data instanceof portalui_sysMenuForm)
