...
//* 			SAI, PT											
//***********************************************************************************************
window.app_builder_component_controls_treeGridItem = function(parent, owner, index){
	try{
		if (owner){
			this.parent = parent;
			this.grid = owner;
			if (parent instanceof portalui_treeGrid)		
				this.level = 1;
			else if (parent instanceof portalui_treeGridItem)
				this.level = parent.getLevel() + 1;
			this.parent = owner;
			this.dataCaption = "data";
			
			this.title = "treeGriditem";
			this.rowIndex = index;
			window.app_builder_component_controls_treeGridItem.prototype.parent.constructor.call(this, parent);	
			this.className = "portalui_treeGridItem";
...
window.app_builder_component_controls_treeGridItem.extend(window.app_builder_component_controls_containerControl);
window.app_builder_component_controls_treeGridItem.implement({
	doDraw: function(canvas){
		...
		var html = "<div style='{position: relative; top:0;width:100%; height: 20%;}' >" +
					"<div id='"+n+"_colCel' style='{position: absolute; top:0;width:100%; height: 20%;}'"+	
					"></div>" +				
					"<div id="+n+"_frameIcon' style='{cursor: pointer; position : relative; left: 0; top: 0; width: 50; height: 20;}' " +
					"</div>"+						
	                "<div id='" + n + "form' style='{display: none;position : relative; left: 0; top: 0; width: 100%; height: auto;}'></div>";
	...
	doFocusEdit: function(idx){
		try{
			...
			this.getForm().setActiveControl(this.parent);
		...
		try{	
			if ((window.dropDown == undefined) || (!window.dropDown.visible))
			{
				...
				if (window.dropDown == undefined){
					...
					window.dropDown = new dropDownBox(this.getForm());
				}
				...
				var scrHeight = this.getForm().getHeight();
...
//			nodeCel.style.background = "#F0F9EF";
	...
	addChild: function(child){
	    if (child instanceof portalui_treeGridItem)
	    {
	        ...
	        window.app_builder_component_controls_treeGridItem.prototype.parent.addChild.call(this, child);	
			
			node = $(child.getFullId() + "_icon");
			if (node != undefined)
				node.style.background = "url(image/themes/"+system.getThemes()+"/treeLast.png) no-repeat";
...
//		this.onSelect.call(this);
	...
	treeMouseUp: function(event){
		if (this.clicked){
			...
			{			
				if (selNode.owner instanceof portalui_treeGridItem)
	    ...
	    if ((target != undefined) && (target.id == (n + "_frame")))
	        window.app_builder_component_controls_treeGridItem.prototype.parent.eventMouseUp.call(this, event);
	},
	...
	getTree: function(){
	    ...
	    while ((result != undefined) && !(result instanceof portalui_treeGrid))	
	...
	getTreeGrid: function(){
		...
		while (!(parent instanceof portalui_treeGrid))	
			parent = parent.owner;
		return parent;
	},
	lastChild: function(){
