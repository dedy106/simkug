window.app_saku_piutang_transaksi_fkorWriteOfMhs = function(owner)
{
  if (owner)
	{
		window.app_saku_piutang_transaksi_fkorWriteOfMhs.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_piutang_transaksi_fkorWriteOfMhs";
		this.setTop(60);
		this.setWidth(1280);
		this.setHeight(550);
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this,"mainButtonClick","Write Off Piutang : Koreksi",0);
		...
		{
			alert("[app_saku_piutang_transaksi_fkorWriteOfMhs]->constructor : "+e);
		}
	}
};
window.app_saku_piutang_transaksi_fkorWriteOfMhs.extend(window.portalui_childForm);
window.app_saku_piutang_transaksi_fkorWriteOfMhs.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_piutang_transaksi_fkorWriteOfMhs.prototype.doModalResult = function(event, modalResult)
{
	switch (event)
	{
		case "clear" :
...
window.app_saku_piutang_transaksi_fkorWriteOfMhs.prototype.insertData = function(sql)
{	
	if (this.dataAsli != undefined){
		...
		if (this.dataAsli.periode < this.app._periode){
			...
			sql.add("delete from ar_wo where no_wo = '"+this.e0.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");			
	...
	if (!hapus){
		var line;	
		var script2, script1 = "insert into ar_wo(no_wo, tanggal, keterangan, periode, ref1, nilai, kode_lokasi, akun_piutang, akun_deprs, kode_pp, p_bd,status, posted) values";	
			...
			for (var i in this.sg1.data.objList)
			{
				if (this.sg1.cells(0,i) =="TRUE"){
					...
					script1 += "('"+this.e01.getText()+"','"+this.dpTgl.getDateString()+"','"+this.eKeterangan.getText()+"','"+this.ePeriode.getText()+"','"+line.get("no_invoice")+"',"+
							" '"+parseFloat(line.get("nilai_bd"))+"','"+this.app._lokasi+"','"+line.get("akun_piutang")+"','"+line.get("akun_deprs")+"','"+line.get("kode_pp")+"','"+line.get("persen")+"','-','F')";				
				}
			}		
			var urut=0;
...
window.app_saku_piutang_transaksi_fkorWriteOfMhs.prototype.FindBtnClick = function(sender, event)
{	
	switch(sender){
			case this.e0 :
			this.standarLib.showListData(this, "Data Penyisihan",sender,undefined, 
										  "select distinct no_wo, keterangan from ar_wo  where kode_lokasi = '"+this.app._lokasi+"' and posted = 'F' ",
										  "select count(distinct no_wo, keterangan) from ar_wo where kode_lokasi = '"+this.app._lokasi+"' and posted = 'F'",
										  new Array("no_wo","keterangan"),"and",new Array("No Bukti","Keterangan"));
			break;		
	}
};
window.app_saku_piutang_transaksi_fkorWriteOfMhs.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_saku_piutang_transaksi_fkorWriteOfMhs.prototype.doGenerate = function(sender)
{
	try{
		if(sender == this.bGenerate)
			this.e01.setText(this.standarLib.noBuktiOtomatis(this.dbLib, "ar_wo", "no_wo", "KWO"+this.ePeriode.getText().substr(2),"000", " and kode_lokasi = '"+this.app._lokasi+"' "));	
		if (sender == this.bLoad){
			...
			var rs = this.dbLib.runSQL("select 'FALSE' as flag, a.no_invoice,"+
								   ...
								   " "+isnull+"(g.total_byr,0) as total_bayar, (a.nilai * a.jumlah) - "+isnull+"(g.total_byr,0) as saldo,"+
								   ...
								   "	inner join (select "+limitmsSQL+" ref1, akun_deprs, nilai as nilai_bd from ar_susut where kode_lokasi = '"+this.app._lokasi+"' and periode<= '"+this.ePeriode.getText()+"' order by tanggal desc "+limitmySQL+")h on h.ref1 = a.no_invoice "+
								   " left outer join (select z.kode_lokasi, ref2, kode_produk, sum(x.nilai + x.disc) as total_byr from arbyrmhs_d x inner join arbyrmhs_m z on z.no_bukti = x.no_bukti and z.kode_lokasi = x.kode_lokasi and z.kode_lokasi = '"+this.app._lokasi+"' group by z.kode_lokasi, ref2, kode_produk) g on g.ref2 = a.no_invoice and g.kode_produk = b.kode_produk and g.kode_lokasi = a.kode_lokasi "+
	                               "where  a.kode_lokasi = '"+this.app._lokasi+"' and "+
	                               ""+isnull+"(g.total_byr,0) < a.nilai and c.flag_hapus='0' and c.periode <= '"+this.ePeriode.getText()+"' "+
...
window.app_saku_piutang_transaksi_fkorWriteOfMhs.prototype.doSelectDate = function(sender, y,m,d)
{
	this.ePeriode.setText(y+""+(m < 10?'0'+m:m));
	this.bGenerate.click();
};
window.app_saku_piutang_transaksi_fkorWriteOfMhs.prototype.createJurnal = function()
{
	try{
...
window.app_saku_piutang_transaksi_fkorWriteOfMhs.prototype.doSGChange = function(sender, col, row)
{
	if (col == 0){
		this.createJurnal();
	}
};
window.app_saku_piutang_transaksi_fkorWriteOfMhs.prototype.editChange = function(sender)
{
	try{
		var dt = this.dbLib.execSQL("select a.tanggal, a.periode, a.keterangan   "+
			"from ar_wo a   "+
			" where a.no_wo = '"+this.e0.getText()+"' and a.kode_lokasi ='"+this.app._lokasi+"' ");
		...
		if (typeof(dt) == "object"){		
			...
			var rs = this.dbLib.runSQL("select 'TRUE' as flag, a.no_invoice,"+
								   ...
								   " i.p_bd, "+
								   " i.nilai as nilai_bd,"+
								   " i.akun_piutang,i.akun_deprs,i.kode_drk as kode_drk_pdpt, "+                               
	                               " i.kode_pp  "+
	                               "from armhs_d a inner join produk b on a.kode_produk=b.kode_produk and b.jenis = 'BPP' and b.kode_lokasi = a.kode_lokasi "+
								   "	inner join armhs_m c on a.no_invoice=c.no_invoice and c.kode_lokasi = a.kode_lokasi "+
								   "	inner join mhs d on c.ref1=d.NPM and d.kode_lokasi = a.kode_lokasi "+
								   "	inner join jurusan e on d.kode_jur=e.kode_jur and e.kode_lokasi = a.kode_lokasi "+
								   "   	inner join param_bpp f on f.kode_produk=b.kode_produk and f.kode_ang=d.kode_ang and f.kode_jur=d.kode_jur and f.semester = a.semester and f.kode_lokasi = a.kode_lokasi "+
								   " 	inner join ar_wo i on i.kode_lokasi = a.kode_lokasi and i.ref1 = a.no_invoice "+								   								   
	                               "where  a.kode_lokasi = '"+this.app._lokasi+"' and "+
	                               " i.no_wo = '"+this.e0.getText()+"' "+								   
	                               "order by d.kode_ang,d.npm");					
			   if (rs instanceof portalui_arrayMap){
					this.sg1.clear();
					for (var i in rs.objList){
