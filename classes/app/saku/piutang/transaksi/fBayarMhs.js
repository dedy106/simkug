...
* 			SAI, PT											
*************************************************************************************************/
window.app_saku_piutang_transaksi_fBayarMhs = function(owner)
{
  if (owner)
	{
		window.app_saku_piutang_transaksi_fBayarMhs.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_piutang_transaksi_fBayarMhs";
		this.setTop(60);
		this.setWidth(1280);
		this.setHeight(550);
		...
		try
		{
			...
			//this.eDisc.setReadOnly(true); disc bisa telat.... setelah beberapa bulan dari kts.
		...
		{
			alert("[app_saku_piutang_transaksi_fBayarMhs]->constructor : "+e);
		}
	}
};
window.app_saku_piutang_transaksi_fBayarMhs.extend(window.portalui_childForm);
window.app_saku_piutang_transaksi_fBayarMhs.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_piutang_transaksi_fBayarMhs.prototype.doModalResult = function(event, modalResult)
{
	switch (event)
	{
		case "clear" :
...
window.app_saku_piutang_transaksi_fBayarMhs.prototype.insertData = function(sql)
{	
...
//------------------------------------- end buffering spro	
	...
	scriptARM += "('"+this.e0.getText()+"','"+this.dpTgl.getDateString()+"','"+this.eKeterangan.getText()+"',"+parseNilai(this.eNilai.getText())+", "+
				...
				" '"+this.app._lokasi+"','"+this.eMhs.getText()+"','"+this.eKts.getText()+"','0','0','"+parseNilai(this.eDisc.getText())+"','T','"+parseNilai(this.eCDPakai.getText())+"','"+parseNilai(this.eBaddebt.getText())+"','"+this.eAkun.getText()+"' )";			
	var nourut = 0;
...
//-------------------------------------------------
	if (this.app._dbEng == "mysqlt"){		
		...
		sql.add("update armhs_m set disc='"+parseNilai(this.eDisc.getText())+"' where kode_lokasi ='"+this.app._lokasi+"' and no_invoice = '"+this.eKts.getText()+"' ");
...
window.app_saku_piutang_transaksi_fBayarMhs.prototype.doLoadDataBPP = function(sender)
{
	try{
...
window.app_saku_piutang_transaksi_fBayarMhs.prototype.doEditExit = function(sender)
{
	sender.checkItem();	
};
window.app_saku_piutang_transaksi_fBayarMhs.prototype.doEditChange = function(sender)
{
...
window.app_saku_piutang_transaksi_fBayarMhs.prototype.FindBtnClick = function(sender, event)
{
	switch(sender){
		case this.eJurusan :
...
window.app_saku_piutang_transaksi_fBayarMhs.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_saku_piutang_transaksi_fBayarMhs.prototype.doFindBtnClick = function(sender, col, row) 
{
...
window.app_saku_piutang_transaksi_fBayarMhs.prototype.doNilaiChange = function()
{
	try
	{
		var total=0;
	...
	{
		alert("[app_saku_piutang_transaksi_fBayarMhs]::doNilaiChange:"+e);
	}
};
window.app_saku_piutang_transaksi_fBayarMhs.prototype.doGenerate = function(sender)
{
...
window.app_saku_piutang_transaksi_fBayarMhs.prototype.doSelectDate = function(sender, y,m,d)
{
	this.ePeriode.setText(y+""+(m < 10?'0'+m:m));
	this.bGenerate.click();
};
window.app_saku_piutang_transaksi_fBayarMhs.prototype.doChange = function(sender){	
	this.eCD.setText("0");
	this.eCDPakai.setText("0");
...
window.app_saku_piutang_transaksi_fBayarMhs.prototype.doKtsChange = function(sender)
{
	try{
...
window.app_saku_piutang_transaksi_fBayarMhs.prototype.doSGChange = function()
{
	try
	{
