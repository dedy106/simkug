window.app_saku_piutang_report_flRekapByr = function(owner)
{
	if (owner)
	{
		window.app_saku_piutang_report_flRekapByr.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_piutang_report_flRekapByr";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Rekap Pembayaran Piutang", 2);
	...
	this.filterRep.setSGFilterRowButtonStyle(this.sg1, 1,new Array(0,1),new  Array(0,2,2));
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("No. Invoice","All",""));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());	
};
window.app_saku_piutang_report_flRekapByr.extend(window.portalui_childForm);
window.app_saku_piutang_report_flRekapByr.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 1)
	{
	...
	if (row == 2)
	{
		this.standar.ListDataSGFilter2(this, "Data DRK",this.sg1, this.sg1.row, this.sg1.col,
										"select kode_drk,tahun,nama from drk "+this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where"),
										"select count(*) from drk "+this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where"),
										new Array("kode_drk","nama"),"and",new Array("Kode DRK","Tahun","Nama"));
	}
};
window.app_saku_piutang_report_flRekapByr.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_saku_piutang_report_flRekapByr.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		}else
		{
	    	...
	    	this.filter = this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						  ...
						  this.filterRep.filterStr("a.no_invoice",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
	...
	{
		alert("[app_saku_piutang_report_flRekapByr]::mainButtonClick:"+e);
	}
};
window.app_saku_piutang_report_flRekapByr.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_piutang_report_flRekapByr.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page, 30, this.showFilter, this.app._namalokasi);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_piutang_report_flRekapByr.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_piutang_report_flRekapByr.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_piutang_report_flRekapByr.prototype.doRowPerPageChange = function(sender, rowperpage)
{
