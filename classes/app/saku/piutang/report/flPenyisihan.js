window.app_saku_piutang_report_flPenyisihan = function(owner)
{
	if (owner)
	{
		window.app_saku_piutang_report_flPenyisihan.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_piutang_report_flPenyisihan";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Penyisihan Piutang", 2);
	...
	this.filterRep.setSGFilterRowTipe(this.sg1, 0,[0,1,2,3,4],["13","123","123","123","6"]);
	this.filterRep.setSGFilterRowButtonStyle(this.sg1, 1,[0,1,2,3,4],[2,2,2,2,1]);
	...
	this.gridLib.SGEditData(this.sg1,4,[0,1,2],["Tanggal Bayar","<=",""]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku_piutang_report_flPenyisihan.extend(window.portalui_childForm);
window.app_saku_piutang_report_flPenyisihan.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 0)
	{
	...
	if (row == 3)
	{
		this.standar.ListDataSGFilter2(this, "Data Mahasiswa",this.sg1, this.sg1.row, this.sg1.col,
									  ...
									  ["npm","nama_mhs"],"where",["Kode Angkatan","Nama","Lokasi"]);
	}
};
window.app_saku_piutang_report_flPenyisihan.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["13","123","123","123","6"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,2,2,2,1]);
	}else
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["3","123","123","123","6"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[3,2,2,2,1]);
	}
};
window.app_saku_piutang_report_flPenyisihan.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		}else
		{
	        ...
	        var dt=this.sg1.getCell(2,4).split("/");
	        var tmp=dt[2]+"-"+dt[1]+"-"+dt[0];
	    	...
	    	this.filter = this.filterRep.filterStr("m.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						  ...
						  this.filterRep.filterStr("s.tanggal",this.sg1.getCell(1,4),tmp,this.sg1.getCell(3,4),"and");
			
			this.nama_report="server_report_piutang_rptPenyisihan";
	...
	{
		alert("[app_saku_piutang_report_flPenyisihan]::mainButtonClick:"+e);
	}
};
window.app_saku_piutang_report_flPenyisihan.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_piutang_report_flPenyisihan.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.showFilter, this.app._namalokasi);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_piutang_report_flPenyisihan.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_piutang_report_flPenyisihan.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_piutang_report_flPenyisihan.prototype.doRowPerPageChange = function(sender, rowperpage)
{
