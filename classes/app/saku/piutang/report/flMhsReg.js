window.app_saku_piutang_report_flMhsReg = function(owner)
{
	if (owner)
	{
		window.app_saku_piutang_report_flMhsReg.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_piutang_report_flMhsReg";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Mahasiswa Registrasi", 2);
		...
		uses("portalui_reportViewer;server_report_report;util_filterRep;util_gridLib;util_standar");
	...
	this.filterRep.setSGFilterRowTipe(this.sg1, 0,[0,1,2,3,4,5],["13","123","123","123","123678","1236"]);
	...
	this.gridLib.SGEditData(this.sg1,4,[0,1,2], ["Periode Awal Amortisasi","=",this.app._periode]);
	this.gridLib.SGEditData(this.sg1,5,[0,1,2], ["Periode Invoice","All",""]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku_piutang_report_flMhsReg.extend(window.portalui_childForm);
window.app_saku_piutang_report_flMhsReg.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 0)
	{
	...
	if (row == 1)
	{
		this.standar.ListDataSGFilter2(this, "Data Jurusan",this.sg1, this.sg1.row, this.sg1.col,
								  "select kode_jur, nama_jur,kode_lokasi from jurusan "+this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where"),
								  ...
								  ["kode_jur","nama_jur"],"where",["Kode Jurusan","Nama","Lokasi"]);
	}
	if (row == 2)
	{
		this.standar.ListDataSGFilter2(this, "Data Angkatan",this.sg1, this.sg1.row, this.sg1.col,
								  "select kode_ang, nama_ang,kode_lokasi from angkatan "+this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where"),
								  ...
								  ["kode_ang","nama_ang"],"where",["Kode Angkatan","Nama","Lokasi"]);
	}
	if (row == 3)
	{
		this.standar.ListDataSGFilter2(this, "Data Mahasiswa",this.sg1, this.sg1.row, this.sg1.col,
							  ...
							  ["npm","nama_mhs"],"where",["Kode ","Nama Mahasiswa","Lokasi"]);
	}
};
window.app_saku_piutang_report_flMhsReg.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["13","123","123","123","123678","1236"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,2,2,2,0,0]);
	}else
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["3","123","123","123","123678","1236"]);
	...
	if (row == 4 || row==5)
	{
		...
		{
			var rs = this.dbLib.runSQL("select distinct periode from armhs_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' ");			
...
window.app_saku_piutang_report_flMhsReg.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
	    	...
	    	this.filter = this.filterRep.filterStr("m.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						  ...
						  this.filterRep.filterStr("b.periode_awal",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
						  this.filterRep.filterStr("a.periode",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");
	
			this.nama_report="server_report_piutang_rptMhsReg";
	...
	{
		alert("[app_saku_piutang_report_flMhsReg]::mainButtonClick:"+e);
	}
};
window.app_saku_piutang_report_flMhsReg.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_piutang_report_flMhsReg.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page, 30, this.showFilter, this.app._namalokasi);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_piutang_report_flMhsReg.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_piutang_report_flMhsReg.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_piutang_report_flMhsReg.prototype.doRowPerPageChange = function(sender, rowperpage)
{
