window.app_saku_piutang_report_flPembayaran = function(owner)
{
	if (owner)
	{
		window.app_saku_piutang_report_flPembayaran.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_piutang_report_flPembayaran";
		this.maximize();
...
window.app_saku_piutang_report_flPembayaran.extend(window.portalui_childForm);
window.app_saku_piutang_report_flPembayaran.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 1)
	{
	...
	if (row == 2)
	{
		this.standar.ListDataSGFilter2(this, "Data Pembayaran",this.sg1, this.sg1.row, this.sg1.col,
									"select no_bukti,keterangan,nilai from arbyrmhs_m "+this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where")+
									this.filterRep.filterStr("periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and"),
									"select count(*) from arbyrmhs_m "+this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where")+
									this.filterRep.filterStr("periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and"),
									["no_bukti","keterangan"],"and",["No Bukti","Keterangan","Nilai Bayar"]);
	}
};
window.app_saku_piutang_report_flPembayaran.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_saku_piutang_report_flPembayaran.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		}else
		{
			...
			this.nama_report="server_report_piutang_rptPembayaran";
	...
	{
		alert("[app_saku_piutang_report_flPembayaran]::mainButtonClick:"+e);
	}
};
window.app_saku_piutang_report_flPembayaran.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_piutang_report_flPembayaran.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.showFilter, this.app._namalokasi);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_piutang_report_flPembayaran.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_piutang_report_flPembayaran.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_piutang_report_flPembayaran.prototype.doRowPerPageChange = function(sender, rowperpage)
{
