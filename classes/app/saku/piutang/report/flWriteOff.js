window.app_saku_piutang_report_flWriteOff = function(owner)
{
	if (owner)
	{
		window.app_saku_piutang_report_flWriteOff.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_piutang_report_flWriteOff";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Write Off Piutang", 2);
	...
	this.gridLib.SGEditData(this.sg1,4,[0,1,2],["Tanggal Write Off","<=",""]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku_piutang_report_flWriteOff.extend(window.portalui_childForm);
window.app_saku_piutang_report_flWriteOff.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 0)
	{
...
window.app_saku_piutang_report_flWriteOff.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_saku_piutang_report_flWriteOff.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		}else
		{
			...
			this.nama_report="server_report_piutang_rptWriteOff";
	...
	{
		alert("[app_saku_piutang_report_flWriteOff]::mainButtonClick:"+e);
	}
};
window.app_saku_piutang_report_flWriteOff.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_piutang_report_flWriteOff.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.showFilter, this.app._namalokasi);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_piutang_report_flWriteOff.prototype.doCloseReportClick = function(sender)
{
  switch(sender.getName())
  {
	...
	case "PreviewBtn" :        
		...
		break;					 
	case "PrintBtn" :        
		this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,this.filter, this.page,30, this.showFilter,this.app._namalokasi));
      try
      {
...
window.app_saku_piutang_report_flWriteOff.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_piutang_report_flWriteOff.prototype.doRowPerPageChange = function(sender, rowperpage)
{
