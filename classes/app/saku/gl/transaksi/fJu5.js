window.app_saku_gl_transaksi_fJu5 = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_transaksi_fJu5.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_gl_transaksi_fJu5";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Jurnal Umum [Buku Bantu-v2]: Input", 0);	
		uses("util_addOnLib;portalui_saiCBBL;portalui_saiEdit;portalui_datePicker;portalui_saiGrid;portalui_sgNavigator;app_saku_fJurnalViewer");
		
		/*
		this.cb_bukti = new portalui_saiCBBL(this);
		this.cb_bukti.setLeft(775);
...
window.app_saku_gl_transaksi_fJu5.extend(window.portalui_childForm);
window.app_saku_gl_transaksi_fJu5.prototype.doSelect = function(sender, year, month, day)
{
	if (month < 10)
		month = "0"+month;
...
window.app_saku_gl_transaksi_fJu5.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_gl_transaksi_fJu5.prototype.simpan = function()
{	
	this.hitungGar();	
	...
	if (this.standarLib.checkEmptyByTag(this, new Array("0","1","2")))
	{
		try
		{
			...
			sql.add("insert into ju_m (no_ju,kode_lokasi,no_dokumen,tanggal,keterangan,kode_pp,modul,jenis,"+
					...
					"('"+this.ed_nb.getText()+"','"+this.app._lokasi+"','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDateString()+"','"+this.ed_desc.getText()+"','-','JU2','"+this.jenis+"','"+
			...
			for (var i=0;i < this.sg1.getRowCount();i++){
				if (this.sg1.rowValid(i)){
					...
					if (this.cb_status.getText() == "REGULER") {
						d += "('"+this.ed_nb.getText()+"','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDateString()+"',"+ix+",'"+this.sg1.getCell(0,i)+
...
window.app_saku_gl_transaksi_fJu5.prototype.doModalResult = function(event, modalResult)
{
...
window.app_saku_gl_transaksi_fJu5.prototype.genClick = function(sender)
{
	try{
...
window.app_saku_gl_transaksi_fJu5.prototype.doEditChange = function(sender)
{
...
window.app_saku_gl_transaksi_fJu5.prototype.FindBtnClick = function(sender, event)
{
	try{
		if (sender == this.cb_curr) {
...
window.app_saku_gl_transaksi_fJu5.prototype.doCellEnter = function(sender, col, row) 
{
	try{
		switch(col){
			case 2 : 
...
window.app_saku_gl_transaksi_fJu5.prototype.doSgChange= function(sender, col, row){
   try{
...
window.app_saku_gl_transaksi_fJu5.prototype.doFindBtnClick = function(sender, col, row) 
{
	try{
		switch(col){
			case 0 : 
		...
		alert("[app_saku_gl_transaksi_fJu5] : doFindBtnClick : " + e);
	}
};

window.app_saku_gl_transaksi_fJu5.prototype.doNilaiChange = function()
{
	try{
		var totD = totC = 0;
		...
		alert("[app_saku_gl_transaksi_fJu5]::doNilaiChange:"+e);
	}
};

window.app_saku_gl_transaksi_fJu5.prototype.doCellExit = function(sender, col, row) 
{
	try{
		switch(col){
			case 3 : 
...
window.app_saku_gl_transaksi_fJu5.prototype.hitungGar = function()
{
...
window.app_saku_gl_transaksi_fJu5.prototype.garClick = function(sender)
{
	try
	{
...
window.app_saku_gl_transaksi_fJu5.prototype.loadMasterData= function(){
...
window.app_saku_gl_transaksi_fJu5.prototype.doRequestReady = function(sender, methodName, result)
{
