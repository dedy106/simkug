window.app_saku_gl_transaksi_fJu4k = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_transaksi_fJu4k.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_gl_transaksi_fJu4k";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Jurnal Umum Lintas Bulan: Koreksi", 0);	
		...
		{		   
			...
			var val = this.dbLib.loadQuery("select periode from periode where kode_lokasi='"+this.app._lokasi+"' and periode like '"+this.app._periode.substr(0,4)+"%' and periode<>'"+this.app._periode+"'");
			var val = val.split("\r\n");
...
window.app_saku_gl_transaksi_fJu4k.extend(window.portalui_childForm);
window.app_saku_gl_transaksi_fJu4k.implement({
	...
	ubah: function(){	
		...
		*/
		//this.bGen.click(); pasti hapus semua sama GLDT_H
		if (this.standarLib.checkEmptyByTag(this, new Array("0","1","2")))
		{
			try
			{
				...
				sql.add("delete from gldt_h where no_bukti='"+this.cb_bukti.getText()+"' and periode='"+this.periodeLama+"' and kode_lokasi='"+this.app._lokasi+"'");
				sql.add("delete from ju_j where no_ju='"+this.cb_bukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and periode='"+this.periodeLama+"' ");
				sql.add("delete from ju_m where no_ju='"+this.cb_bukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and periode='"+this.periodeLama+"' ");
				sql.add("delete from angg_r where no_bukti ='"+this.cb_bukti.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' and modul = 'JU' and periode1='"+this.periodeLama+"'");									
				...
				//--------------POSTING-------------------
				sql.add("call sp_post_modul ('JULP','"+this.app._lokasi+"','"+this.nb+"')");
			...
			if (sender == this.cb_bukti) {
			    if (this.ed_period != "") {
				  this.standarLib.showListData(this, "Daftar Bukti JU Periode "+this.ed_period.getText(),this.cb_bukti,undefined, 
													 "select no_ju, keterangan from ju_m where modul = 'JU' and periode='"+this.cb_perlama.getText()+"' and no_del='-' and kode_lokasi='"+this.app._lokasi+"'", 
													 "select count(no_ju)      from ju_m where modul = 'JU' and periode='"+this.cb_perlama.getText()+"' and no_del='-' and kode_lokasi='"+this.app._lokasi+"'", 
													 new Array("no_ju","keterangan"),"and", new Array("No Bukti","Keterangan"),false);
				}
				this.standarLib.clearByTag(this, new Array("2"),undefined);				
				this.sg1.clear(); this.sg1.appendRow();
			}
	...
	doFindBtnClick: function(sender, col, row) {
		try
		{
			...
			systemAPI.alert("[app_saku_gl_transaksi_fJu4k] : doFindBtnClick : " + e);
		}
	},
	doNilaiChange: function(){
		try{
			...
			alert("[app_saku_gl_transaksi_fJu4k]::doNilaiChange:"+e);
		}
	},
