window.app_saku_gl_transaksi_fJu2k = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_transaksi_fJu2k.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_gl_transaksi_fJu2k";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Jurnal Umum: Koreksi", 0);	
		...
		{		   
			...
			var val = this.dbLib.loadQuery("select distinct periode from ju_m where kode_lokasi='"+this.app._lokasi+"' and not (periode < '"+this.app._periode.substr(0,4)+"01') order by periode");
			var val = val.split("\r\n");
			...
			this.cb_bukti.setSQL("select no_ju, no_dokumen from ju_m where modul = 'JU' and periode='"+this.cb_perlama.getText()+"' and no_del='-' and kode_lokasi='"+this.app._lokasi+"'",["no_ju","no_dokumen"],true,["No JU","No Dokumen"],"and","Daftar Transaksi",true);
...
window.app_saku_gl_transaksi_fJu2k.extend(window.portalui_childForm);
window.app_saku_gl_transaksi_fJu2k.implement({
		...
		if (this.cb_bukti.getText() != ""){
			if (this.cb_bukti != undefined) {
				try {
					...
					sql.add("select  a.tanggal,a.no_dokumen, a.keterangan, a.kode_curr, a.kurs, a.nik_buat, a.nik_setuju, b.nama as nama_pembuat, c.nama as nama_setuju,a.periode,a.posted "+
			...
			if (sender == this.cb_bukti) {
			    if (this.cb_perlama.getText() != "") {
				  this.standarLib.showListData(this, "Daftar Bukti JU Periode "+this.ed_period.getText(),this.cb_bukti,undefined, 
													 "select no_ju, no_dokumen from ju_m where modul = 'JU' and periode='"+this.cb_perlama.getText()+"' and no_del='-' and posted = 'F' and kode_lokasi='"+this.app._lokasi+"'", //posted = 'F' and 
													 "select count(no_ju)      from ju_m where modul = 'JU' and periode='"+this.cb_perlama.getText()+"' and no_del='-' and posted = 'F' and kode_lokasi='"+this.app._lokasi+"'", //posted = 'F' and 
	...
	doFindBtnClick: function(sender, col, row) {
		try{
			...
			systemAPI.alert("[app_saku_gl_transaksi_fJu2k] : doFindBtnClick : " + e);
		}
	},
	doNilaiChange: function(){
		try{
			...
			alert("[app_saku_gl_transaksi_fJu2k]::doNilaiChange:"+e);
		}
	},
							...
							if (typeof data != "string"){
								if (data.result[0].rs.rows[0] != undefined){									
									...
									for (var i in dataD.rs.rows){
										...
										this.sg1.appendData([line.kode_akun,line.nama_akun,line.keterangan,line.dc,floatToNilai(line.nilai),line.kode_pp,line.nama_pp,line.kode_drk,line.nama_drk]);
									} 
									this.sg1.validasi();
								} 
							}else throw result;
