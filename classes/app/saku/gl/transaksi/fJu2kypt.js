window.app_saku_gl_transaksi_fJu2kypt = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_transaksi_fJu2kypt.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_gl_transaksi_fJu2kypt";
		this.maximize();
		...
		*/
		...
		{		   
			...
			this.ed_nb.onExit.set(this,"doExit");
			...
			//reverse tidak fungsi jd tidak boleh pilih periode lalu..... var val = this.dbLib.loadQuery("select distinct periode from ju_m where kode_lokasi='"+this.app._lokasi+"' and not (periode < '"+this.app._periode.substr(0,4)+"01') order by periode");
			var val = this.dbLib.loadQuery("select distinct periode from ju_m where kode_lokasi='"+this.app._lokasi+"' and periode >= '"+this.app._periode+"' order by periode");
			var val = val.split("\r\n");
...
window.app_saku_gl_transaksi_fJu2kypt.extend(window.portalui_childForm);
window.app_saku_gl_transaksi_fJu2kypt.implement({
	...
	ubah: function(){	
		var line,data = this.dbLib.runSQL("select no_ju from ju_m "+
										  "where no_ju = '"+this.ed_nb.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' and no_ju <> '"+this.cb_bukti.getText()+"' and periode = '"+this.ed_period.getText()+"' ");
		if (data instanceof portalui_arrayMap){
			line = data.get(0);
			if (line != undefined){
				system.alert(this,"No Bukti sudah terpakai.","");
				return false;
			} 
		}
		...
		if (this.cb_bukti.getText() != "")
		{
			if (this.cb_bukti != undefined) {
				try 
				{
					...
					var line, data = this.dbLib.runSQL(" select  a.tanggal,a.no_dokumen, a.keterangan, a.kode_curr, a.kurs, a.nik_buat, a.nik_setuju, b.nama as nama_pembuat, c.nama as nama_setuju,a.periode,a.posted "+
					...
					if (data instanceof portalui_arrayMap)
					{
						...
						{									
							...
							this.ed_nb.setText(this.cb_bukti.getText());
					...
					var strSql = " select  a.kode_akun, b.nama as nama_akun, a.keterangan, a.dc,a.nilai,a.kode_pp, ifnull(c.nama,'-') as nama_pp, a.kode_drk,ifnull(d.nama,'-') as nama_drk "+
								 ...
								 " where a.no_ju = '"+this.cb_bukti.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut desc";
	...
	doExit: function(sender){
		if (this.ed_nb.getText() != "") {
			var line,data = this.dbLib.runSQL("select no_ju from ju_m "+
											  "where no_ju = '"+this.ed_nb.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' and periode = '"+this.ed_period.getText()+"' ");
			if (data instanceof portalui_arrayMap)
			{
				line = data.get(0);
				if (line != undefined){
	...
	doFindBtnClick: function(sender, col, row) {
		try
		{
			...
			systemAPI.alert("[app_saku_gl_transaksi_fJu2kypt] : doFindBtnClick : " + e);
		}
	},
	doNilaiChange: function(){
		try
		{
			...
			alert("[app_saku_gl_transaksi_fJu2kypt]::doNilaiChange:"+e);
		}
	},
