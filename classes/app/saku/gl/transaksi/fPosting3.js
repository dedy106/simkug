window.app_saku_gl_transaksi_fPosting3 = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_transaksi_fPosting3.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_gl_transaksi_fPosting3";
		this.maximize();
...
window.app_saku_gl_transaksi_fPosting3.extend(window.portalui_childForm);
window.app_saku_gl_transaksi_fPosting3.implement({
				...
				if (modalResult == mrOk && (this.standarLib.checkEmptyByTag(this, new Array("0","1"))))
				{
					try
					{
						...
						for (var i=0; i < this.sg1.rows.getLength(); i++)
						{
							if (this.sg1.getCell(0,i) == "POSTING")
							{
								sql.add("insert into posting_d (no_post,modul,no_bukti,status,catatan,no_del) values "+
			...
			if (this.cb_status.getText() != ""){
				if ((this.cb_status.getText() != "JUAL") && (this.cb_status.getText().substr(0,2) == "JU")) {
					var pageCount = this.dbLib.getRowCount("select count(a.no_ju) "+
														   ...
														   "' and a.kode_lokasi='"+this.app._lokasi+"' and a.jenis='"+this.cb_status.getText().substr(2,2)+"'", this.baris);							
					this.scriptSql = " select 'INPROG' as status,'JU' as modul,a.no_ju as no_bukti,a.no_dokumen,ifnull(b.nama,'-') as nama_pp,a.tanggal,"+
									 ...
									 " where a.posted = 'F' and a.periode='"+this.ed_period.getText()+
									 "' and a.kode_lokasi ='"+this.app._lokasi+"' and a.jenis='"+this.cb_status.getText().substr(2,2)+"'";				
				}
				else
				{
					if (this.cb_status.getText() == "JHPP"){
						var pageCount = this.dbLib.getRowCount("select count(aa.no_hpp) from ("+
															   ...
															   ")aa", this.baris);
						this.scriptSql = " select 'INPROG' as status,'JHPP' as modul,a.no_hpp as no_bukti,a.no_dokumen,ifnull(b.nama,'-') as nama_pp,a.tanggal,"+
					...
					if (this.cb_status.getText() == "JUAL"){
						var pageCount = this.dbLib.getRowCount("select count(aa.no_jual) from ("+
															   ...
															   "' and a.kode_lokasi='"+this.app._lokasi+"' "+
															   "union "+
															   "select a.no_retur as no_jual "+
															   "from inv_jualretur_m a "+
															   "where a.posted = 'F' and a.periode='"+this.ed_period.getText()+
															   "' and a.kode_lokasi='"+this.app._lokasi+"' "+
															   ")aa", this.baris);
						this.scriptSql = " select 'INPROG' as status,'JUAL' as modul,a.no_jual as no_bukti,a.no_dokumen,ifnull(b.nama,'-') as nama_pp,a.tanggal,"+
										 "       a.keterangan,a.kode_curr,a.nilai as nilai "+
										 " from inv_jual_m a left outer join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
										 " where a.posted = 'F' and a.periode='"+this.ed_period.getText()+
										 "' and a.kode_lokasi ='"+this.app._lokasi+"' " +
										 " union "+
										 " select 'INPROG' as status,'RETJUAL' as modul,a.no_retur as no_bukti,a.no_dokumen,ifnull(b.nama,'-') as nama_pp,a.tanggal,"+
										 "       a.keterangan,a.kode_curr,a.nilai "+
					...
					if (this.cb_status.getText() == "BELI"){
						var pageCount = this.dbLib.getRowCount("select count(aa.no_beli) from ("+
															   ...
															   "' and a.kode_lokasi='"+this.app._lokasi+"' "+
															   "union "+
															   "select a.no_retur as no_beli "+
															   "from inv_beliretur_m a "+
															   "where a.posted = 'F' and a.periode='"+this.ed_period.getText()+
															   "' and a.kode_lokasi='"+this.app._lokasi+"' "+
															   ")aa", this.baris);
						this.scriptSql = " select 'INPROG' as status,'BELI' as modul,a.no_beli as no_bukti,a.no_dokumen,ifnull(b.nama,'-') as nama_pp,a.tanggal,"+
										 "       a.keterangan,a.kode_curr,a.nilai "+
										 " from inv_beli_m a left outer join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
										 " where a.posted = 'F' and a.periode='"+this.ed_period.getText()+
										 "' and a.kode_lokasi ='"+this.app._lokasi+"' " +
										 " union "+
										 " select 'INPROG' as status,'RETBELI' as modul,a.no_retur as no_bukti,a.no_dokumen,ifnull(b.nama,'-') as nama_pp,a.tanggal,"+
										 "       a.keterangan,a.kode_curr,a.nilai "+
					...
					if (this.cb_status.getText() == "KBIV"){
						var pageCount = this.dbLib.getRowCount(" select count(a.no_kas) "+
														   ...
														   "       a.kode_lokasi='"+this.app._lokasi+"'", this.baris);
					...
					if (this.cb_status.getText() == "GBIV"){
							var pageCount = this.dbLib.getRowCount(" select count(a.no_gb) "+
															   " from gb_m a  "+
															   " where a.posted = 'F' and a.periode='"+this.ed_period.getText()+"' and "+
															   "       a.kode_lokasi='"+this.app._lokasi+"'", this.baris);
							this.scriptSql = " select 'INPROG' as status,'GB' as modul,a.no_gb as no_bukti,a.no_dokumen,ifnull(b.nama,'-') as nama_pp,a.tanggal,"+
										     "        a.keterangan,a.kode_curr,a.nilai "+
										     " from gb_m a "+
										     "              left outer join pp b on b.kode_pp=a.kode_pp and b.kode_lokasi=a.kode_lokasi "+
										     " where a.posted = 'F' and a.periode='"+this.ed_period.getText()+"' and "+
										     "       a.kode_lokasi ='"+this.app._lokasi+"' order by a.no_gb";			
					}
				}
				...
				if (this.cb_status.getText() == "JHPP"){
					if (this.sg1.cells(1,row) == "JHPP") {
						var temp = this.dbLib.runSQL("select a.kode_akun,b.nama as nama_akun,a.keterangan,a.dc,a.nilai,"+
													...
													"              left outer join pp c on a.kode_pp=c.kode_pp  and a.kode_lokasi=c.kode_lokasi  "+
													"              left outer join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and d.tahun=substring(a.periode,1,4) "+
													"where a.no_hpp = '"+this.sg1.getCell(2,row)+"' and a.kode_lokasi = '"+this.app._lokasi+"' order by a.dc desc");
					}
				}
				if (this.cb_status.getText() == "JUAL"){
					...
					if (this.sg1.cells(1,row) == "RETJUAL") {
						var temp = this.dbLib.runSQL("select a.kode_akun,b.nama as nama_akun,a.keterangan,a.dc,a.nilai,"+
													...
													"              left outer join pp c on a.kode_pp=c.kode_pp  and a.kode_lokasi=c.kode_lokasi  "+
													"              left outer join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and d.tahun=substring(a.periode,1,4) "+
													"where a.no_retur = '"+this.sg1.getCell(2,row)+"' and a.kode_lokasi = '"+this.app._lokasi+"' order by a.dc desc");
					}
				}
				if (this.cb_status.getText() == "BELI"){
					if (this.sg1.cells(1,row) == "BELI") {
						var temp = this.dbLib.runSQL("select a.kode_akun,b.nama as nama_akun,a.keterangan,a.dc,a.nilai,"+
													...
													"              left outer join pp c on a.kode_pp=c.kode_pp  and a.kode_lokasi=c.kode_lokasi  "+
													"              left outer join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and d.tahun=substring(a.periode,1,4) "+
													"where a.no_beli = '"+this.sg1.getCell(2,row)+"' and a.kode_lokasi = '"+this.app._lokasi+"' order by a.dc desc");
					}
