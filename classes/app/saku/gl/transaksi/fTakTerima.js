window.app_saku_gl_transaksi_fTakTerima = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_transaksi_fTakTerima.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_gl_transaksi_fTakTerima";
		this.maximize();
		...
		this.cb_perlama = new portalui_saiCB(this,{bound:[680,78,185,20],caption:"Periode Kirim"});		
		this.cb_bukti = new portalui_saiCBBL(this,{bound:[680,100,220,20],caption:"No TAK Kirim",multiSelection:false});		
		
		this.bShow = new portalui_imageButton(this);
		this.bShow.setLeft(902);
		this.bShow.setTop(100);
...
window.app_saku_gl_transaksi_fTakTerima.extend(window.portalui_childForm);
window.app_saku_gl_transaksi_fTakTerima.implement({
		...
		if (this.standarLib.checkEmptyByTag(this, new Array("0","1","2"))){
			try{
				...
				sql.add("insert into ju_m (no_ju,kode_lokasi,no_dokumen,tanggal,keterangan,kode_pp,modul,jenis, "+
						...
						"('"+this.nb+"','"+this.app._lokasi+"','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDate()+"','"+this.ed_desc.getText()+"','"+this.cb_pp.getText()+"','JU','"+this.cb_jenis.getText()+"','"+
				...
				sql.add("insert into ju_j (no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,"+
							...
							"','"+this.ed_desc.getText()+"','"+dc+"',"+Math.abs(nilaiTak)+",'"+this.cb_pp.getText()+"','-',"+
							"'-','-','-','-','-','-','"+this.app._lokasi+"','JU','"+this.jenis+"',"+
							"'"+this.ed_period.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+
							",'"+this.app._userLog+"',now())"); 				
				//------------------------------------------------------------------------------------------ ANGGARAN ------------------------------------------------------------------------------
	...
	showClick: function(sender){
		if (this.cb_bukti.getText() != "" && this.cb_pp.getText() != ""){
			...
			system.alert(this,"Bukti dan PP harus valid terisi.","");
		}
	},
	...
	doFindBtnClick: function(sender, col, row) {
		try{
			...
			systemAPI.alert("[app_saku_gl_transaksi_fTakTerima] : doFindBtnClick : " + e);
		}
	},
	doNilaiChange: function(){
		try{
			...
			alert("[app_saku_gl_transaksi_fTakTerima]::doNilaiChange:"+e);
		}
	},
							...
							if (typeof data != "string"){
								if (data.result[0].rs.rows[0] != undefined){									
									...
									for (var i in dataD.rs.rows){
										...
										if (line.kode_akun != lineM.ref1) this.sg1.appendData([line.kode_akun,line.nama_akun,line.keterangan,line.dc.toUpperCase(),floatToNilai(line.nilai),this.cb_pp.getText(),this.cb_pp.rightLabelCaption,line.kode_drk,line.nama_drk]);
