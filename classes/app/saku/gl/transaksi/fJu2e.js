window.app_saku_gl_transaksi_fJu2e = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_transaksi_fJu2e.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_gl_transaksi_fJu2e";
		this.maximize();
		...
		try{		    
			...
			this.cb_jenis.addItem(4,"REVERSE");
...
window.app_saku_gl_transaksi_fJu2e.extend(window.portalui_childForm);
window.app_saku_gl_transaksi_fJu2e.implement({
	doSelect: function(sender, year, month, day){
		...
		*/
		...
		if (parseFloat(this.app._periode.substr(4,2)) <= 12) this.ed_period.setText(year.toString()+""+month);
		else {
			if (month == "12") this.ed_period.setText(this.app._periode);
			else this.ed_period.setText(year.toString()+""+month);
		}		
		this.loadMasterData();
	},
	...
	simpan: function(){							
		...
		for (var i in this.gridJurnal.objList)
		{
			...
			{
				system.alert(this,"Nilai transaksi melebihi saldo anggaran.","Periksa kembali data anggaran."+line.get("kode_drk")+" : "+ line.get("nilai")+" : "+line.get("kode_akun")+":GAR = "+line.get("saldo_gar"));
				return false;
			}
		}
		...
		if (this.standarLib.checkEmptyByTag(this, new Array("0","1","2")))
		{
			try
			{
				...
				if (totD != totC){					
					this.ed_debet.setText(totD);
					this.ed_kredit.setText(totC);
					throw("Total debet tidak sama dengan total Kredit. Cek lagi item jurnal anda. (Tidak boleh ada cell yg kosong.)");
				}
				//------------------------------------------------------------------------------------------ ANGGARAN ------------------------------------------------------------------------------
	...
	doEditChange: function(sender){
		...
		if (sender == this.cb_jenis)
		{
			...
			if (this.cb_jenis.getText() == "REVERSE")
			{
				this.jenis = "JR";
			}
		}
	...
	doFindBtnClick: function(sender, col, row) {
		...
		{
			systemAPI.alert("[app_saku_gl_transaksi_fJu2e] : doFindBtnClick : " + e);
		}
	},
	doNilaiChange: function(){
		...
		{
			alert("[app_saku_gl_transaksi_fJu2e]::doNilaiChange:"+e);
		}
	},
	...
	hitungGar: function(){
		...
		for (var i in this.gridJurnal.objList)
		{
			line = this.gridJurnal.get(i);
			var baris,data;
			if (parseFloat(this.ed_period.getText().substr(4,2)) <= 12)//{
				data = this.dbLib.runSQL("select fn_cekagg2('"+line.get("kode_pp")+"','"+this.app._lokasi+"','"+line.get("kode_akun")+"','"+line.get("kode_drk")+"','"+this.ed_period.getText()+"') as gar ");
			else data = this.dbLib.runSQL("select fn_cekagg2('"+line.get("kode_pp")+"','"+this.app._lokasi+"','"+line.get("kode_akun")+"','"+line.get("kode_drk")+"','"+this.ed_period.getText().substr(0,4)+"12') as gar ");
			...
			if (data instanceof portalui_arrayMap)
			{
				...
				if (baris != undefined)
				{
					baris = baris.get("gar");
					if (baris == undefined){
						alert(line.get("kode_pp")+":"+line.get("kode_akun")+":"+ line.get("kode_drk")  +":"+baris);
						break;
					}
