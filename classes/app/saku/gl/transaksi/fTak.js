 ...
 * @author dweexfuad
 */
window.app_saku_gl_transaksi_fTak = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_transaksi_fTak.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_gl_transaksi_fTak";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form TAK: Input", 0);			
		try{		    
			...
			this.cb_pp.setTop(78);
			this.cb_pp.setWidth(185);
			this.cb_pp.setLabelWidth(100);
			this.cb_pp.setReadOnly(true);
			this.cb_pp.setRightLabelVisible(false);
			this.cb_pp.setCaption("PP / Unit");
			this.cb_pp.setTag("1");
			this.cb_pp.setSQL("select kode_pp, nama from pp where kode_lokasi = '"+this.app._lokasi+"' and tipe ='Posting'",["kode_pp","nama"],true,["Kode","Nama"],"and","Daftar PP",true);
			...
			this.ed_dok.setTipeText(ttHurufBesar);
			this.ed_dok.setCaption("No Dokumen");
			this.ed_dok.setReadOnly(false);
			this.ed_dok.setAutoComplete(true);
			...
			this.ed_desc.setAutoComplete(true);
			...
			this.cb_pembuat.setSQL("select nik, nama from karyawan where kode_lokasi = '"+this.app._lokasi+"'",["nik","nama"],true,["NIK","Nama"],"and","Daftar Karyawan",true);
		
			this.cb_setuju = new portalui_saiCBBL(this);
			this.cb_setuju.setLeft(20);
			this.cb_setuju.setTop(210);
			...
			this.cb_setuju.setSQL("select nik, nama from karyawan where kode_lokasi = '"+this.app._lokasi+"'",["nik","nama"],true,["NIK","Nama"],"and","Daftar Karyawan",true);
...
window.app_saku_gl_transaksi_fTak.extend(window.portalui_childForm);
window.app_saku_gl_transaksi_fTak.implement({
		...
		if (this.standarLib.checkEmptyByTag(this, new Array("0","1","2")))
		{
			try
			{
				...
				{			
					if (this.sg1.rowValid(i)){
						...
						sql.add("insert into ju_j (no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,"+
							...
							"('"+this.ed_nb.getText()+"','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDate()+"',"+idx+",'"+this.sg1.getCell(0,i)+
				...
				sql.add("insert into ju_j (no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,"+
							...
							"','"+this.ed_desc.getText()+"','"+dc+"',"+nilaiTak+",'"+this.cb_pp.getText()+"','-',"+
							"'-','-','-','-','-','-','"+this.app._lokasi+"','JU','"+this.jenis+"',"+
							"'"+this.ed_period.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+
							",'"+this.app._userLog+"',now())"); 				
				//------------------------------------------------------------------------------------------ ANGGARAN ------------------------------------------------------------------------------
		...
		try
		{
			if (sender == this.cb_pp) 
			{
			    this.standarLib.showListData(this, "Daftar PP",this.cb_pp,undefined, 
											  ...
											  new Array("kode_pp","nama"),"where", new Array("Kode","Deskripsi"),false);
			}
		...
		try
		{
			switch(col)
			{
				...
				/*case 3 : 
					...
					break;*/
				case 4 : 
							if ((this.sg1.getCell(4,row) == "") && (row > 0)) {
							var sls = nilaiToFloat(this.ed_debet.getText()) - nilaiToFloat(this.ed_kredit.getText());
							sls = Math.abs(sls); 
	...
	doFindBtnClick: function(sender, col, row) {
		...
		{
			systemAPI.alert("[app_saku_gl_transaksi_fTak] : doFindBtnClick : " + e);
		}
	},
	doNilaiChange: function(){
		...
		{
			alert("[app_saku_gl_transaksi_fTak]::doNilaiChange:"+e);
		}
	},
	...
	doRequestReady: function(sender, methodName, result){
		if (sender == this.dbLib)
		{
			...
			{   
				switch(methodName)
	    		{
					...
					if (result.toLowerCase().search("error") == -1)					
					{
						...
						if (this.cb1.isSelected()) {
							...
							this.filter2 = " where a.kode_lokasi='"+this.app._lokasi+"' and a.no_ju='"+this.ed_nb.getText()+"' ";			
							this.viewer.prepare();
				...
				systemAPI.alert("step : "+step+"; error = "+e,result);
			}
	    }
