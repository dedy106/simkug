window.app_saku_gl_transaksi_fJu0k = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_transaksi_fJu0k.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_gl_transaksi_fJu0k";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Jurnal Umum Original: Koreksi", 0);	
		...
		*/
		...
		this.sg1.setColTitle(new Array("Kode Akun","Nama Akun","Keterangan","DC","Nilai"));
		this.sg1.setColWidth(new Array(4,3,2,1,0),new Array(160,30,350,220,100));	
...
window.app_saku_gl_transaksi_fJu0k.extend(window.portalui_childForm);
window.app_saku_gl_transaksi_fJu0k.implement({
		...
		if (this.standarLib.checkEmptyByTag(this, new Array("0","1","2")))
		{
			try
			{
				...
				sql.add("insert into ju_m (no_ju,kode_lokasi,no_dokumen,tanggal,keterangan,kode_pp,modul,jenis,"+
						...
						"('"+this.nb+"','"+this.app._lokasi+"','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDate()+"','"+this.ed_desc.getText()+"','-','JU0','"+this.cb_jenis.getText()+"','"+
				...
				{			
					if (this.sg1.rowValid(i)){
						...
						sql.add("insert into ju_j (no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,"+
							...
							"','"+this.sg1.getCell(2,i)+"','"+this.sg1.getCell(3,i).toUpperCase()+"',"+parseNilai(this.sg1.getCell(4,i))+",'-','-',"+
							"'-','-','-','-','-','-','"+this.app._lokasi+"','JU0','"+this.jenis+"',"+
		...
		if (this.cb_bukti.getText() != "")
		{
			if (this.cb_bukti != undefined) {
				try 
				{
					...
					{
						var strSql = " select  a.kode_akun, b.nama as nama_akun, a.keterangan, a.dc,a.nilai "+
			...
			if (sender == this.cb_bukti) 
			{
			    if (this.ed_period != "") 
				{
				  this.standarLib.showListData(this, "Daftar Bukti JU Periode "+this.ed_period.getText(),this.cb_bukti,undefined, 
													 "select no_ju, no_dokumen from ju_m where modul = 'JU0' and periode='"+this.cb_perlama.getText()+"' and no_del='-' and kode_lokasi='"+this.app._lokasi+"'", //posted = 'F' and 
													 "select count(no_ju)      from ju_m where modul = 'JU0' and periode='"+this.cb_perlama.getText()+"' and no_del='-' and kode_lokasi='"+this.app._lokasi+"'", //posted = 'F' and 
	...
	doFindBtnClick: function(sender, col, row) {
		try
		{
			...
			systemAPI.alert("[app_saku_gl_transaksi_fJu0k] : doFindBtnClick : " + e);
		}
	},
	doNilaiChange: function(){
		try
		{
			...
			alert("[app_saku_gl_transaksi_fJu0k]::doNilaiChange:"+e);
		}
	},
