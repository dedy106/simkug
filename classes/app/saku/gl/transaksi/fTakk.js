window.app_saku_gl_transaksi_fTakk = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_transaksi_fTakk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_gl_transaksi_fTakk";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Jurnal TAK: Koreksi", 0);	
		...
		{		   
			...
			var val = this.dbLib.loadQuery("select distinct periode from ju_m where kode_lokasi='"+this.app._lokasi+"' and jenis = 'TAK' and not (periode < '"+this.app._periode.substr(0,4)+"01') order by periode");
			var val = val.split("\r\n");
...
window.app_saku_gl_transaksi_fTakk.extend(window.portalui_childForm);
window.app_saku_gl_transaksi_fTakk.implement({
			...
			try{
				...
				if (parseFloat(this.periodeLama) < parseFloat(this.app._periode)) {
					...
					sql.add(" insert into ju_m (no_ju,kode_lokasi,no_dokumen,tanggal,keterangan,kode_pp,modul,jenis,"+
							"                  periode,kode_curr,kurs,nilai,nik_buat,nik_setuju,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_lokasi2)"+
							" select concat(no_ju,'r'),kode_lokasi,no_dokumen,'"+this.dp_tgl1.getDate()+"',keterangan,kode_pp,modul,jenis,"+
							"       '"+this.ed_period.getText()+"',kode_curr,kurs,nilai,'"+this.cb_pembuat.getText()+"',nik_setuju,now(),'"+this.app._userLog+"','F',no_ju,'-',ref1, kode_lokasi2 "+
				...
				sql.add("insert into ju_m (no_ju,kode_lokasi,no_dokumen,tanggal,keterangan,kode_pp,modul,jenis,"+
						...
						"('"+this.nb+"','"+this.app._lokasi+"','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDate()+"','"+this.ed_desc.getText()+"','-','JU','"+this.cb_jenis.getText()+"','"+
						     this.ed_period.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+","+parseNilai(this.ed_debet.getText())+",'"+this.cb_pembuat.getText()+"','"+this.cb_setuju.getText()+"',now(),'"+this.app._userLog+"','F','-','-','"+this.ed_akun.getText()+"','"+this.ed_lokasi.getText()+"' )");
				var totD= 0, totC = 0, idx = 0;
				...
				for (var i=0; i < this.sg1.rows.getLength(); i++){			
					if (this.sg1.rowValid(i)){
						...
						sql.add("insert into ju_j (no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,"+
							...
							"('"+this.nb+"','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDate()+"',"+idx+",'"+this.sg1.getCell(0,i)+
				...
				sql.add("insert into ju_j (no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,"+
							"kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+	
							"('"+this.nb+"','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDate()+"',"+idx+",'"+this.ed_akun.getText()+
							"','"+this.ed_desc.getText()+"','"+dc+"',"+nilaiTak+",'"+this.app._kodePP+"','-',"+
							"'-','-','-','-','-','-','"+this.app._lokasi+"','JU','"+this.jenis+"',"+
							"'"+this.ed_period.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+
							",'"+this.app._userLog+"',now())"); 				
				//------------------------------------------------------------------------------------------ ANGGARAN ------------------------------------------------------------------------------
		...
		if (this.cb_bukti.getText() != ""){
			if (this.cb_bukti != undefined) {
				try {
					...
					sql.add("select  a.tanggal,a.no_dokumen, a.keterangan, a.kode_curr, a.kurs, a.nik_buat, a.nik_setuju, b.nama as nama_pembuat, c.nama as nama_setuju,a.periode,a.posted, a.ref1, a.kode_lokasi2 "+
					...
					sql.add("select  a.no_urut, a.kode_akun, b.nama as nama_akun, a.keterangan, a.dc,a.nilai,a.kode_pp, ifnull(c.nama,'-') as nama_pp, a.kode_drk,ifnull(d.nama,'-') as nama_drk "+
	...
	doEditChange: function(sender){
		...
		if (sender == this.cb_perlama){
		      this.cb_bukti.setSQL("select no_ju, no_dokumen from ju_m a where modul = 'JU' and periode='"+this.cb_perlama.getText()+"' and no_del='-' and kode_lokasi='"+this.app._lokasi+"'",["no_ju","no_dokumen"],true);
        }
	...
	doFindBtnClick: function(sender, col, row) {
		try{
			...
			systemAPI.alert("[app_saku_gl_transaksi_fTakk] : doFindBtnClick : " + e);
		}
	},
	doNilaiChange: function(){
		try{
			...
			alert("[app_saku_gl_transaksi_fTakk]::doNilaiChange:"+e);
		}
	},
						...
						if (this.loadMaster){
							...
							if (typeof data != "string"){
								if (data.result[0].rs.rows[0] != undefined){									
									...
									this.ed_lokasi.setText(line.kode_lokasi2);
									this.dp_tgl1.onSelect.set(this, "doSelect");
									this.sg1.clear(); 			
									var lineM = line;
									...
									for (var i in dataD.rs.rows){
										...
										if (line.kode_akun != lineM.ref1) this.sg1.appendData([line.kode_akun,line.nama_akun,line.keterangan,line.dc,floatToNilai(line.nilai),line.kode_pp,line.nama_pp,line.kode_drk,line.nama_drk]);
									} 
									this.sg1.validasi();
								} 
							}else throw result;
