window.app_saku_gl_transaksi_fJu0 = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_transaksi_fJu0.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_gl_transaksi_fJu0";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Jurnal Umum Original: Input", 0);			
...
window.app_saku_gl_transaksi_fJu0.extend(window.portalui_childForm);
window.app_saku_gl_transaksi_fJu0.implement({
		...
		if (this.standarLib.checkEmptyByTag(this, new Array("0","1")))
		{
			try
			{
				...
				sql.add("insert into ju_m (no_ju,kode_lokasi,no_dokumen,tanggal,keterangan,kode_pp,modul,jenis,"+
						...
						"('"+this.ed_nb.getText()+"','"+this.app._lokasi+"','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDate()+"','"+this.ed_desc.getText()+"','-','JU0','"+this.cb_jenis.getText()+"','"+
	...
	doFindBtnClick: function(sender, col, row) {
		...
		{
			systemAPI.alert("[app_saku_gl_transaksi_fJu0] : doFindBtnClick : " + e);
		}
	},
	doNilaiChange: function(){
		...
		{
			alert("[app_saku_gl_transaksi_fJu0]::doNilaiChange:"+e);
		}
	},
