// JavaScript Document
window.app_saku_gl_trail_fTrailNeracaJejer = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_trail_fTrailNeracaJejer.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_trail_fTrailNeracaJejer";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Audit Trail Neraca Jejer", 2);
...
window.app_saku_gl_trail_fTrailNeracaJejer.extend(window.portalui_childForm);
window.app_saku_gl_trail_fTrailNeracaJejer.prototype.doSg2DblClick = function(sender, colIdx, rowIdx)
{
	...
	this.sg3.setTop(100);
	this.sg3.setLeft(10);
	this.sg3.setWidth(880);
	this.sg3.setHeight(365);	
	this.sg3.setColTitle(new Array("No","Kode Akun","Nama Akun","Saldo Awal","Debet","Kredit","Saldo Akhir"));
	
	var sql = "call sp_glma_trail_tmp ('"+this.lokasi+"','"+this.sg2.getCell(1,rowIdx)+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
...
window.app_saku_gl_trail_fTrailNeracaJejer.prototype.doSg3DblClick = function(sender, colIdx, rowIdx)
{
	...
	{		
		...
		var tabel ="(select * from gldt where kode_akun='"+this.sg3.getCell(1,rowIdx)+"' and periode='"+this.sg1.getCell(2,0)+"' and kode_lokasi='"+this.lokasi+"' "+
      		       " union all "+
				   " select * from gldt_h where kode_akun='"+this.sg3.getCell(1,rowIdx)+"' and periode='"+this.sg1.getCell(2,0)+"' and kode_lokasi='"+this.lokasi+"')";
		...
		else var line,data = this.dbLib.runSQL("select a.no_bukti,a.tanggal,a.keterangan,a.kode_pp,a.kode_drk,case when a.dc='D' then nilai else 0 end as debet,case when a.dc='C' then nilai else 0 end as kredit,0.0 as balance from "+tabel+" a order by a.tanggal, a.no_bukti ");				
		if (data instanceof portalui_arrayMap)
		{			
...
window.app_saku_gl_trail_fTrailNeracaJejer.prototype.SetSg4Mutasi = function()
{
	try
	{
	...
	{
		alert("[app_saku_gl_trail_fTrailNeracaJejer]::SetSg4Mutasi:"+e);
	}
};
window.app_saku_gl_trail_fTrailNeracaJejer.prototype.doSg4DblClick = function(sender, colIdx, rowIdx)
{	
	try{
	...
	{		
		...
		var tabel ="(select * from gldt where no_bukti='"+this.sg4.getCell(1,rowIdx)+"' and periode='"+this.sg1.getCell(2,0)+"' and kode_lokasi='"+this.lokasi+"' "+
      		       " union all "+
				   " select * from gldt_h where no_bukti='"+this.sg4.getCell(1,rowIdx)+"' and periode='"+this.sg1.getCell(2,0)+"' and kode_lokasi='"+this.lokasi+"')";
...
window.app_saku_gl_trail_fTrailNeracaJejer.prototype.SetSg5Mutasi = function()
{
	try
	{
		var debet = kredit = 0;
...
window.app_saku_gl_trail_fTrailNeracaJejer.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_saku_gl_trail_fTrailNeracaJejer.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_saku_gl_trail_fTrailNeracaJejer.prototype.mainButtonClick = function(sender)
{	
	if (sender == this.app._mainForm.bPreview){
		try
		{
			...
			{
				sql="call sp_neraca_jejer ('"+this.sg1.getCell(2,2)+"','N','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,0)+"','"+this.kode_lokasi+"','"+this.nik_user+"')";
			}
			if (this.app._dbEng == "ado_mssql")
			{
				sql="call sp_neraca_jejer ('"+this.sg1.getCell(2,2)+"','A','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,0)+"','"+this.kode_lokasi+"','"+this.nik_user+"')";
			}
			this.dbLib.execQuerySync(sql);	
				...
				if (this.p1.visible){ 
					...
					this.sg2.setColTitle(new Array("No","Kode","Nama","ITT","IMT","LAKHAR","NTC","POLTEK","TOTAL"));
					this.sg2.setTag("2");
					...
					{		
						this.sg2.clearAll();
						var line,data = this.dbLib.runSQL("select kode_neraca,fn_spasi(nama,level_spasi) as nama,n1,n2,n3,n4,n5,(n1+n2+n3+n4+n5) as n6 from neraca_tmp where jenis_akun='Neraca' and nik_user='"+this.nik_user+"' order by modul,rowindex ");
						if (data instanceof portalui_arrayMap)
						{						
...
window.app_saku_gl_trail_fTrailNeracaJejer.prototype.doRequestReady = function(sender, methodName, result)
{
		switch (methodName)
		{
			case "preview" : 
...
window.app_saku_gl_trail_fTrailNeracaJejer.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page, 30, "YAKES", this.showFilter,this.filter2);	
};

window.app_saku_gl_trail_fTrailNeracaJejer.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_gl_trail_fTrailNeracaJejer.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
