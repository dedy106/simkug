// JavaScript Document
window.app_saku_gl_trail_fTrailCf = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_trail_fTrailCf.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_trail_fTrailCf";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Audit Trail Arus Kas", 2);
	...
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2,3), new Array("Periode","Range",this.app._periode,this.app._periode));
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Lokasi/Cabang","=",this.app._lokasi));
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Versi","=","V1"));
...
window.app_saku_gl_trail_fTrailCf.extend(window.portalui_childForm);
window.app_saku_gl_trail_fTrailCf.prototype.doSg2DblClick = function(sender, colIdx, rowIdx)
{
...
window.app_saku_gl_trail_fTrailCf.prototype.doSg3DblClick = function(sender, colIdx, rowIdx)
{
...
window.app_saku_gl_trail_fTrailCf.prototype.SetSg4Mutasi = function()
{
	try
	{
	...
	{
		alert("[app_saku_gl_trail_fTrailCf]::SetSg4Mutasi:"+e);
	}
};

window.app_saku_gl_trail_fTrailCf.prototype.doSg4DblClick = function(sender, colIdx, rowIdx)
{	
	try{
...
window.app_saku_gl_trail_fTrailCf.prototype.SetSg5Mutasi = function()
{
	try
	{
		var debet = kredit = 0;
...
window.app_saku_gl_trail_fTrailCf.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_saku_gl_trail_fTrailCf.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_saku_gl_trail_fTrailCf.prototype.mainButtonClick = function(sender)
{	
	if (sender == this.app._mainForm.bPreview){
		try
		{
			...
			{
				sql="call sp_cf ('"+this.sg1.getCell(2,2)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(3,0)+"','"+this.nik_user+"')";
			}
			if (this.app._dbEng == "ado_mssql")
			{
			...
			this.dbLib.execQuerySync(sql);	
				...
				if (this.p1.visible){ 
					...
					this.sg2.setColTitle(new Array("No","Kode","Nama","Nilai"));
					this.sg2.setTag("2");
					...
					{		
						this.sg2.clearAll();
						var line,data = this.dbLib.runSQL("select kode_cf,fn_spasi(nama_cf,level_spasi) as nama,n1 from cf_tmp where nik_user='"+this.nik_user+"' order by rowindex ");
						if (data instanceof portalui_arrayMap)
						{						
...
window.app_saku_gl_trail_fTrailCf.prototype.doRequestReady = function(sender, methodName, result)
{
		switch (methodName)
		{
			case "preview" : 
...
window.app_saku_gl_trail_fTrailCf.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page, 30, "YAKES", this.showFilter,this.filter2);	
};

window.app_saku_gl_trail_fTrailCf.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_gl_trail_fTrailCf.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
