// JavaScript Document
window.app_saku_gl_trail_fTrailLR = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_trail_fTrailLR.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_trail_fTrailLR";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Audit Trail Laba Rugi", 2);		
...
window.app_saku_gl_trail_fTrailLR.extend(window.portalui_childForm);
window.app_saku_gl_trail_fTrailLR.prototype.doSg2DblClick = function(sender, colIdx, rowIdx)
{
...
window.app_saku_gl_trail_fTrailLR.prototype.doSg3DblClick = function(sender, colIdx, rowIdx)
{
...
window.app_saku_gl_trail_fTrailLR.prototype.SetSg4Mutasi = function()
{
	try
	{
	...
	{
		alert("[app_saku_gl_trail_fTrailLR]::SetSg4Mutasi:"+e);
	}
};

window.app_saku_gl_trail_fTrailLR.prototype.doSg4DblClick = function(sender, colIdx, rowIdx)
{	
	try{
...
window.app_saku_gl_trail_fTrailLR.prototype.SetSg5Mutasi = function()
{
	try
	{
		var debet = kredit = 0;
...
window.app_saku_gl_trail_fTrailLR.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_saku_gl_trail_fTrailLR.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_saku_gl_trail_fTrailLR.prototype.mainButtonClick = function(sender)
{	
	if (sender == this.app._mainForm.bPreview){
		try
		{
			...
			sql="call sp_neraca ('"+this.sg1.getCell(2,2)+"','L','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,0)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.nik_user+"')";
			...
			this.dbLib.execQuerySync(sql);	
				...
				if (this.p1.visible){ 
					...
					{		
						this.sg2.clearAll();
						var line,data = this.dbLib.runSQL("select kode_neraca,fn_spasi(nama,level_spasi) as nama,n1,n2,n3,n4 from neraca_tmp where modul='L' and nik_user='"+this.nik_user+"' order by rowindex ");
						if (data instanceof portalui_arrayMap)
						{						
...
window.app_saku_gl_trail_fTrailLR.prototype.doRequestReady = function(sender, methodName, result)
{
		switch (methodName)
		{
			case "preview" : 
...
window.app_saku_gl_trail_fTrailLR.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page, 30, "YAKES", this.showFilter,this.filter2);	
};

window.app_saku_gl_trail_fTrailLR.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_gl_trail_fTrailLR.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
