// JavaScript Document
window.app_saku_gl_trail_fTrailTB = function(owner)
{
	try{
		if (owner)
		{		
			window.app_saku_gl_trail_fTrailTB.prototype.parent.constructor.call(this,owner);
			this.className = "app_saku_gl_trail_fTrailTB";
			this.maximize();
			this.app._mainForm.childFormConfig(this, "mainButtonClick","Audit Trail Trial Balance", 2);		
...
window.app_saku_gl_trail_fTrailTB.extend(window.portalui_childForm);
window.app_saku_gl_trail_fTrailTB.prototype.doSg3DblClick = function(sender, colIdx, rowIdx)
{  
	try
	{
	...
	this.sg4.setColTitle(new Array("No","No Bukti","No Dokumen","Tanggal","Keterangan","Kode PP","Kode DRK","Debet","Kredit","Balance"));
	...
	this.e10.setReadOnly(true);
		...
		var tabel ="(select * from gldt where kode_akun='"+this.sg2.getCell(1,rowIdx)+"' and periode='"+this.sg1.getCell(2,0)+"' and kode_lokasi='"+this.sg1.getCell(2,1)+"' "+
      		       " union all "+
				   " select * from gldt_h where kode_akun='"+this.sg2.getCell(1,rowIdx)+"' and periode='"+this.sg1.getCell(2,0)+"' and kode_lokasi='"+this.sg1.getCell(2,1)+"')";
...
window.app_saku_gl_trail_fTrailTB.prototype.SetSg4Mutasi = function()
{
	try
	{
		...
		systemAPI.alert("[app_saku_gl_trail_fTrailTB]::SetSg4Mutasi:"+e);
	}
};
window.app_saku_gl_trail_fTrailTB.prototype.doSg4DblClick = function(sender, colIdx, rowIdx)
{	
	try{
	...
	this.e11.setReadOnly(true);
	this.e11.setCaption("No Bukti");
	this.e11.setText(this.sg4.getCell(1,rowIdx));
	
    this.e111 = new portalui_saiLabelEdit(this.p5);
	this.e111.setTop(26);
	this.e111.setLeft(10);
	this.e111.setWidth(200);
	this.e111.setCaption("No Dokumen");
	this.e111.setReadOnly(true);
	this.e111.setText(this.sg4.getCell(2,rowIdx));
	...
	this.e12.setReadOnly(true);
	this.e12.setText(this.sg4.getCell(3,rowIdx));
	
	this.e13 = new portalui_saiLabelEdit(this.p5);
	this.e13.setTop(75);
	this.e13.setLeft(10);
	this.e13.setWidth(500);
	this.e13.setReadOnly(true);
	this.e13.setCaption("Keterangan");
	this.e13.setText(this.sg4.getCell(4,rowIdx));
	
	this.e14 = new portalui_saiLabelEdit(this.p5);
	this.e14.setTop(100);
	this.e14.setLeft(10);
	this.e14.setWidth(400);
	this.e14.setReadOnly(true);
	...
	this.e16.setText(this.sg4.getCell(7,rowIdx));
	...
	this.e17.setText(this.sg4.getCell(8,rowIdx));
...
window.app_saku_gl_trail_fTrailTB.prototype.SetSg5Mutasi = function()
{
	try
	{
		var debet = kredit = 0;
		...
		alert("[app_saku_gl_trail_fTrailTB]::SetSg4Mutasi:"+e);
	}
};
window.app_saku_gl_trail_fTrailTB.prototype.doEllipseClick= function(sender, col, row)
{
		...
		if (row == 2)
		{
			this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,
													  "select a.kode_akun, a.nama from masakun a inner join lokasi b on a.kode_lokasi=b.kode_lokasi where a.block= '0' "+
													   this.filterRep.filterStr("b.kode_lokasi",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," and ")+" order by a.kode_akun",
													  "select count(a.kode_akun) from masakun a inner join lokasi b on a.kode_lokasi=b.kode_lokasi where a.block= '0'"+
													  this.filterRep.filterStr("b.kode_lokasi",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," and "),
													  new Array("kode","nama"),"and");
		}
		
};
window.app_saku_gl_trail_fTrailTB.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_saku_gl_trail_fTrailTB.prototype.mainButtonClick = function(sender)
{	
	if (sender == this.app._mainForm.bPreview){
		try
		{
			...
			if (this.sg1.getCell(1,1) == "=")
			{
				...
				{
					var sql = "call sp_glma_tmp ('"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
				...
				{
					var sql = "call sp_glma12_tmp ('"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
			...
			this.dbLib.execQuerySync(sql);
				...
				if (this.p1.visible){ 
					...
					{		
						...
						if (this.sg1.getCell(2,3)=="Neraca Lajur")
						{
							this.sg2.setColTitle(new Array("No","Kode","Nama","S.Awal Debet","S.Awal Kredit","Debet","Kredit","S.Awal Debet","S.Awal Kredit"));
							var sql="select kode_akun,nama,"+ 
							        "case when so_awal>0 then so_awal else 0 end as so_awal_debet,"+
							        "case when so_awal<0 then -so_awal else 0 end as so_awal_kredit,debet,kredit,"+ 
									...
									"from glma_tmp where nik_user='"+this.nik_user+"' and (so_awal<>0 or debet<>0 or kredit<>0 or so_akhir<>0) order by kode_akun ";
						}
						if (this.sg1.getCell(2,3)=="Neraca Percobaan")
						{
							this.sg2.setColTitle(new Array("No","Kode","Nama","Saldo Awal","Debet","Kredit","Saldo Akhir"));
							var sql="select kode_akun,nama,so_awal,debet,kredit,so_akhir from glma_tmp where nik_user='"+this.nik_user+"' and (so_awal<>0 or debet<>0 or kredit<>0 or so_akhir<>0) order by kode_akun ";
						}
						if (this.sg1.getCell(2,3)=="12 Periode")
						{
							this.sg2.setColTitle(new Array("No","Kode","Nama","Januari","Februari","Maret","April","Mei","Juni","Juli","Agustus","September","Oktober","November","Desember"));
							var sql="select kode_akun,nama,n01,n02,n03,n04,n05,n06,n07,n08,n09,n10,n11,n12 from glma12_tmp where nik_user='"+this.nik_user+"' and (n01<>0 or n02<>0 or n03<>0 or n04<>0 or n05<>0 or n06<>0 or n07<>0 or n08<>0 or n09<>0 or n10<>0 or n11<>0 or n12<>0) order by kode_akun ";
						}
...
window.app_saku_gl_trail_fTrailTB.prototype.doRequestReady = function(sender, methodName, result)
{
		switch (methodName)
		{
			case "preview" : 
...
window.app_saku_gl_trail_fTrailTB.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page, 30, "YAKES", this.showFilter,this.filter2);	
};
window.app_saku_gl_trail_fTrailTB.prototype.doCloseReportClick = function(sender)
{
  ...
  switch(sender.getName())
  {
	...
	case "PreviewBtn" :		
		if (this.isReport)
			window.open(this.report.previewWithHeader(this.nama_report,this.filter, 1, this.viewer.getTotalPage() * 30, this.showFilter, this.app._namalokasi));			
		else{ 							
			var win = window.open("");
			win.document.write(loadCSS("server_util_laporan"));
			win.document.write(this.gridHTML);
			win.document.close();
		}      
      break; 	
	case "PrintBtn" :
...
window.app_saku_gl_trail_fTrailTB.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
