// JavaScript Document
window.app_saku_gl_trail_fTrailTBJejer = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_trail_fTrailTBJejer.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_trail_fTrailTBJejer";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Audit Trail Neraca Lajur Jejer", 2);		
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Mutasi Nol","=","Tidak"));
...
window.app_saku_gl_trail_fTrailTBJejer.extend(window.portalui_childForm);
window.app_saku_gl_trail_fTrailTBJejer.prototype.doSg3DblClick = function(sender, colIdx, rowIdx)
{  
	try
	{
	...
	this.e10.setReadOnly(true);
		...
		var tabel ="(select * from gldt where kode_akun='"+this.sg2.getCell(1,rowIdx)+"' and periode='"+this.sg1.getCell(2,0)+"' and kode_lokasi='"+this.lokasi+"' "+
      		       " union all "+
				   " select * from gldt_h where kode_akun='"+this.sg2.getCell(1,rowIdx)+"' and periode='"+this.sg1.getCell(2,0)+"' and kode_lokasi='"+this.lokasi+"')";
...
window.app_saku_gl_trail_fTrailTBJejer.prototype.SetSg4Mutasi = function()
{
	try
	{
		...
		systemAPI.alert("[app_saku_gl_trail_fTrailTBJejer]::SetSg4Mutasi:"+e);
	}
};
window.app_saku_gl_trail_fTrailTBJejer.prototype.doSg4DblClick = function(sender, colIdx, rowIdx)
{	
	try{
...
window.app_saku_gl_trail_fTrailTBJejer.prototype.SetSg5Mutasi = function()
{
	try
	{
		var debet = kredit = 0;
...
window.app_saku_gl_trail_fTrailTBJejer.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_saku_gl_trail_fTrailTBJejer.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
...
window.app_saku_gl_trail_fTrailTBJejer.prototype.mainButtonClick = function(sender)
{	
	if (sender == this.app._mainForm.bPreview){
		try
		{
			...
			var sql = "call sp_glma_jejer_tmp ('"+this.kode_lokasi+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
			...
			this.dbLib.execQuerySync(sql);
				...
				if (this.p1.visible){ 
					...
					{		
						this.sg2.clearAll();
						var line,data = this.dbLib.runSQL("select kode_akun,nama,n1,n2,n3,n4,n5,(n1+n2+n3+n4+n5) as n6 from glma_jejer_tmp where nik_user='"+this.nik_user+"' and (n1<>0 or n2<>0 or n3<>0 or n4<>0 or n5<>0) order by kode_akun ");
						if (data instanceof portalui_arrayMap)
						{						
...
window.app_saku_gl_trail_fTrailTBJejer.prototype.doRequestReady = function(sender, methodName, result)
{
		switch (methodName)
		{
			case "preview" : 
...
window.app_saku_gl_trail_fTrailTBJejer.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page, 30, "YAKES", this.showFilter,this.filter2);	
};
window.app_saku_gl_trail_fTrailTBJejer.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_gl_trail_fTrailTBJejer.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
