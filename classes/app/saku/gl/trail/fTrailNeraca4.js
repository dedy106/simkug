// JavaScript Document
window.app_saku_gl_trail_fTrailNeraca4 = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_trail_fTrailNeraca4.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_trail_fTrailNeraca4";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Audit Trail Neraca 4 Periode", 2);		
	...
	this.filterRep.setSGFilterRowTipe(this.sg1, 0,new Array(0,1,2,3,4,5,6),new  Array("3","123","123","3","3","3","3"));
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Periode Pembanding 2","=",this.app._periode));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Periode Pembanding 3","=",this.app._periode));
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Periode Pembanding 4","=",this.app._periode));
...
window.app_saku_gl_trail_fTrailNeraca4.extend(window.portalui_childForm);
window.app_saku_gl_trail_fTrailNeraca4.prototype.doSg2DblClick = function(sender, colIdx, rowIdx)
{
	...
	if (colIdx==4)
	{
		this.periode=this.sg1.getCell(2,4);
	}
	if (colIdx==5)
	{
		this.periode=this.sg1.getCell(2,5);
	}
	if (colIdx==6)
	{
		this.periode=this.sg1.getCell(2,6);
	}
	...
	{		
	    ...
	    var line,data = this.dbLib.runSQL("select kode_akun,nama,so_awal,debet,kredit,so_akhir from glma_tmp where nik_user='"+this.nik_user+"' and (so_awal<>0 or debet<>0 or kredit<>0 or so_akhir<>0) order by kode_akun ");
...
window.app_saku_gl_trail_fTrailNeraca4.prototype.doSg3DblClick = function(sender, colIdx, rowIdx)
{
	...
	{		
		...
		var tabel ="(select * from gldt where kode_akun='"+this.sg3.getCell(1,rowIdx)+"' and periode='"+this.periode+"' and kode_lokasi='"+this.sg1.getCell(2,1)+"' "+
      		       " union all "+
				   " select * from gldt_h where kode_akun='"+this.sg3.getCell(1,rowIdx)+"' and periode='"+this.periode+"' and kode_lokasi='"+this.sg1.getCell(2,1)+"')";
...
window.app_saku_gl_trail_fTrailNeraca4.prototype.SetSg4Mutasi = function()
{
	try
	{
	...
	{
		alert("[app_saku_gl_trail_fTrailNeraca4]::SetSg4Mutasi:"+e);
	}
};
window.app_saku_gl_trail_fTrailNeraca4.prototype.doSg4DblClick = function(sender, colIdx, rowIdx)
{	
	try{
	...
	{		
		...
		var tabel ="(select * from gldt where no_bukti='"+this.sg4.getCell(1,rowIdx)+"' and periode='"+this.periode+"' and kode_lokasi='"+this.sg1.getCell(2,1)+"' "+
      		       " union all "+
				   " select * from gldt_h where no_bukti='"+this.sg4.getCell(1,rowIdx)+"' and periode='"+this.periode+"' and kode_lokasi='"+this.sg1.getCell(2,1)+"')";
...
window.app_saku_gl_trail_fTrailNeraca4.prototype.SetSg5Mutasi = function()
{
	try
	{
		var debet = kredit = 0;
...
window.app_saku_gl_trail_fTrailNeraca4.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_saku_gl_trail_fTrailNeraca4.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5,6),new  Array("3","123","123","3","3","3","3"));
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5,6),new  Array(0,2,2,0,0,0,0));
	}
	else
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5,6),new  Array("3","3","123","3","3","3","3"));
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5,6),new  Array(0,3,2,0,0,0,0));
	}
	if (row == 0)
	{
...
window.app_saku_gl_trail_fTrailNeraca4.prototype.mainButtonClick = function(sender)
{	
	if (sender == this.app._mainForm.bPreview){
		try
		{
			...
			var in_lap="S";
			periode1="Periode "+this.sg1.getCell(2,0);
			periode2="Periode "+this.sg1.getCell(2,4);
			periode3="Periode "+this.sg1.getCell(2,5);
			periode4="Periode "+this.sg1.getCell(2,6);
			if (this.sg1.getCell(1,1) == "All")
			{
			...
			{
				sql="call sp_neraca4 ('"+this.sg1.getCell(2,2)+"','N','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,4)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,6)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.nik_user+"')";
			}
			if (this.app._dbEng == "ado_mssql")
			{
				sql="call sp_neraca4 ('"+this.sg1.getCell(2,2)+"','A','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,4)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,6)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.nik_user+"')";
			}
			this.dbLib.execQuerySync(sql);	
				...
				if (this.p1.visible){ 
			    	...
			    	this.sg2.setLeft(10);
					this.sg2.setTop(25);
			    	this.sg2.setWidth(860);
			    	this.sg2.setHeight(420);
					this.sg2.setColTitle(new Array("No","Kode","Nama",periode1,periode2,periode3,periode4));
					this.sg2.setTag("2");
					...
					{		
						this.sg2.clearAll();
						var sql="select kode_neraca,fn_spasi(nama,level_spasi) as nama,"+
								"       case modul when 'A' then n1 else -n1 end n1,"+
								"       case modul when 'A' then n2 else -n2 end n2, "+
								"       case modul when 'A' then n3 else -n3 end n3, "+
								"       case modul when 'A' then n4 else -n4 end n4 "+
								"from neraca_tmp where jenis_akun='Neraca' and nik_user='"+this.nik_user+"' order by modul,rowindex ";
...
window.app_saku_gl_trail_fTrailNeraca4.prototype.doRequestReady = function(sender, methodName, result)
{
		switch (methodName)
		{
			case "preview" : 
...
window.app_saku_gl_trail_fTrailNeraca4.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page, 30, "YAKES", this.showFilter,this.filter2);	
};

window.app_saku_gl_trail_fTrailNeraca4.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_gl_trail_fTrailNeraca4.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
