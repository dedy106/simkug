// JavaScript Document
window.app_saku_gl_trail_fTrailNeraca = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_trail_fTrailNeraca.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_trail_fTrailNeraca";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Audit Trail Neraca", 2);	
...
window.app_saku_gl_trail_fTrailNeraca.extend(window.portalui_childForm);
window.app_saku_gl_trail_fTrailNeraca.prototype.doSg2DblClick = function(sender, colIdx, rowIdx)
{
...
window.app_saku_gl_trail_fTrailNeraca.prototype.doSg3DblClick = function(sender, colIdx, rowIdx)
{
...
window.app_saku_gl_trail_fTrailNeraca.prototype.SetSg4Mutasi = function()
{
	try
	{
...
window.app_saku_gl_trail_fTrailNeraca.prototype.doSg4DblClick = function(sender, colIdx, rowIdx)
{	
	try{
...
window.app_saku_gl_trail_fTrailNeraca.prototype.SetSg5Mutasi = function()
{
	try
	{
		var debet = kredit = 0;
...
window.app_saku_gl_trail_fTrailNeraca.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_saku_gl_trail_fTrailNeraca.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_saku_gl_trail_fTrailNeraca.prototype.mainButtonClick = function(sender)
{	
	if (sender == this.app._mainForm.bPreview){
		try
		{
			...
			{
				sql="call sp_neraca ('"+this.sg1.getCell(2,2)+"','N','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,0)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.nik_user+"')";
			}
			if (this.app._dbEng == "ado_mssql")
			{
				sql="call sp_neraca ('"+this.sg1.getCell(2,2)+"','A','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,0)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.nik_user+"')";
			}
			this.dbLib.execQuerySync(sql);	
...
window.app_saku_gl_trail_fTrailNeraca.prototype.doRequestReady = function(sender, methodName, result)
{
		switch (methodName)
		{
			case "preview" : 
...
window.app_saku_gl_trail_fTrailNeraca.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page, 30, "YAKES", this.showFilter,this.filter2);	
};
window.app_saku_gl_trail_fTrailNeraca.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_gl_trail_fTrailNeraca.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
