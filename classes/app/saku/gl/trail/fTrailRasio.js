// JavaScript Document
window.app_saku_gl_trail_fTrailRasio = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_trail_fTrailRasio.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_trail_fTrailRasio";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Audit Trail Rasio Keuangan", 2);		
		...
		this.sg1.setCell(0,2,"Kode Rasio");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode Rasio","=","V1"));
...
window.app_saku_gl_trail_fTrailRasio.extend(window.portalui_childForm);
window.app_saku_gl_trail_fTrailRasio.prototype.doSg2DblClick = function(sender, colIdx, rowIdx)
{
...
window.app_saku_gl_trail_fTrailRasio.prototype.doSg3DblClick = function(sender, colIdx, rowIdx)
{
...
window.app_saku_gl_trail_fTrailRasio.prototype.SetSg4Mutasi = function()
{
	try
	{
	...
	{
		alert("[app_saku_gl_trail_fTrailRasio]::SetSg4Mutasi:"+e);
	}
};
window.app_saku_gl_trail_fTrailRasio.prototype.doSg4DblClick = function(sender, colIdx, rowIdx)
{	
	try{
...
window.app_saku_gl_trail_fTrailRasio.prototype.SetSg5Mutasi = function()
{
	try
	{
		var debet = kredit = 0;
...
window.app_saku_gl_trail_fTrailRasio.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_saku_gl_trail_fTrailRasio.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_saku_gl_trail_fTrailRasio.prototype.mainButtonClick = function(sender)
{	
	if (sender == this.app._mainForm.bPreview){
		try
		{
			...
			{
				sql="call sp_rasio ('"+this.sg1.getCell(2,2)+"','"+this.kode_fs+"','N','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,0)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.nik_user+"')";
			}
			if (this.app._dbEng == "ado_mssql")
			{
				sql="exec sp_neraca '"+this.sg1.getCell(2,2)+"','A','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"'";
			}
			...
			this.dbLib.execQuerySync(sql);	
				...
				if (this.p1.visible){ 
					...
					this.sg2.setColTitle(new Array("No","Kode","Nama","Rumus","Nilai"));
					this.sg2.setTag("2");
					...
					{		
						this.sg2.clearAll();
						var line,data = this.dbLib.runSQL("select kode_rasio,fn_spasi(nama,level_spasi) as nama,rumus,rumus2 from rasio_tmp where nik_user='"+this.nik_user+"' order by rowindex ");
						if (data instanceof portalui_arrayMap)
						{				
							var nilai;	
							for (var i in data.objList){
								rms = data.get(i).get("rumus2");
								if (rms != "")
									eval("nilai = " + rms+";");
								else nilai = 0;
								data.get(i).set("rumus2",Math.round(nilai,2));
							}
...
window.app_saku_gl_trail_fTrailRasio.prototype.doRequestReady = function(sender, methodName, result)
{
		switch (methodName)
		{
			case "preview" : 
...
window.app_saku_gl_trail_fTrailRasio.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page, 30, "YAKES", this.showFilter,this.filter2);	
};

window.app_saku_gl_trail_fTrailRasio.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_gl_trail_fTrailRasio.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
