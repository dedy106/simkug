// JavaScript Document
window.app_saku_gl_trail_fTrailLR2 = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_trail_fTrailLR2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_trail_fTrailLR2";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Audit Trail Laba Rugi Komparasi", 2);
...
window.app_saku_gl_trail_fTrailLR2.extend(window.portalui_childForm);
window.app_saku_gl_trail_fTrailLR2.prototype.doSg2DblClick = function(sender, colIdx, rowIdx)
{
...
window.app_saku_gl_trail_fTrailLR2.prototype.doSg3DblClick = function(sender, colIdx, rowIdx)
{
...
window.app_saku_gl_trail_fTrailLR2.prototype.SetSg4Mutasi = function()
{
	try
	{
	...
	{
		alert("[app_saku_gl_trail_fTrailLR2]::SetSg4Mutasi:"+e);
	}
};

window.app_saku_gl_trail_fTrailLR2.prototype.doSg4DblClick = function(sender, colIdx, rowIdx)
{	
	try{
...
window.app_saku_gl_trail_fTrailLR2.prototype.SetSg5Mutasi = function()
{
	try
	{
		var debet = kredit = 0;
...
window.app_saku_gl_trail_fTrailLR2.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_saku_gl_trail_fTrailLR2.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_saku_gl_trail_fTrailLR2.prototype.mainButtonClick = function(sender)
{	
	if (sender == this.app._mainForm.bPreview){
		try
		{
			...
			sql="call sp_neraca2 ('"+this.sg1.getCell(2,2)+"','L','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,4)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.nik_user+"')";
			...
			this.dbLib.execQuerySync(sql);	
				...
				if (this.p1.visible){ 
					...
					this.p2.setBorder(3);
					this.p2.setCaption("Audit Trail Laba Rugi");
...
window.app_saku_gl_trail_fTrailLR2.prototype.doRequestReady = function(sender, methodName, result)
{
		switch (methodName)
		{
			case "preview" : 
...
window.app_saku_gl_trail_fTrailLR2.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page, 30, "YAKES", this.showFilter,this.filter2);	
};

window.app_saku_gl_trail_fTrailLR2.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_gl_trail_fTrailLR2.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
