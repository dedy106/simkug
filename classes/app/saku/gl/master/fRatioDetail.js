window.app_saku_gl_master_fRatioDetail = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_master_fRatioDetail.prototype.parent.constructor.call(this, owner);
		window.app_saku_gl_master_fRatioDetail.prototype.parent.setWidth.call(this, 300);
		window.app_saku_gl_master_fRatioDetail.prototype.parent.setHeight.call(this, 400);
		
		this.centerize();
		
		this.className = "app_saku_gl_master_fRatioDetail";
...
//		this.p1.setColor(system.getConfig("app.color.panel"));
		...
		this.e5 = new portalui_saiMemo(this.p1);
		this.e5.setTop(126);
		this.e5.setLeft(20);
		this.e5.setWidth(320);
		this.e5.setHeight(60);
		this.e5.setCaption("Keterangan");
...
window.app_saku_gl_master_fRatioDetail.extend(window.portalui_commonForm);
window.app_saku_gl_master_fRatioDetail.prototype.doDraw = function(canvas)
{
...
window.app_saku_gl_master_fRatioDetail.prototype.doAfterResize = function(event)
{
	this.p1.setWidth(this.width - 40);
...
window.app_saku_gl_master_fRatioDetail.prototype.doSysMouseMove = function(x, y, button, buttonState)
{
	window.app_saku_gl_master_fRatioDetail.prototype.parent.doSysMouseMove.call(this,x, y, button, buttonState);
	if (this.isClick)
	{		
...
window.app_saku_gl_master_fRatioDetail.prototype.eventMouseDown = function(event)
{
...
window.app_saku_gl_master_fRatioDetail.prototype.eventMouseUp = function(event)
{
	this.isClick = false;
	this.blockElm.style.display = "none";
	this.frameElm.style.display = "";
};
window.app_saku_gl_master_fRatioDetail.prototype.eventMouseMove = function(event)
{
	if (this.isClick)
	{
		var x = event.clientX;
...
window.app_saku_gl_master_fRatioDetail.prototype.doClick = function(sender)
{
	if (sender == this.b1)
...
window.app_saku_gl_master_fRatioDetail.prototype.doRequestReady = function(sender, methodName, result)
{

};
window.app_saku_gl_master_fRatioDetail.prototype.findBtnClick = function(sender)
{
};
//--------------------------------------------
window.app_saku_gl_master_fRatioDetail.prototype.setCaption = function(data)
{
...
window.app_saku_gl_master_fRatioDetail.prototype.setItemParent = function(data)
{
	this.itemParent = data;
};
window.app_saku_gl_master_fRatioDetail.prototype.setSummaryItems = function(data)
{
	this.e4.clearItem();
	for (var i in data)
		this.e4.addItem(i, data[i]);		
};
window.app_saku_gl_master_fRatioDetail.prototype.doClose = function(sender)
{
