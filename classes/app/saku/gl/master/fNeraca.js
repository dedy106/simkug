window.app_saku_gl_master_fNeraca = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_master_fNeraca.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku_gl_master_fNeraca";
...
window.app_saku_gl_master_fNeraca.extend(window.portalui_childForm);
window.app_saku_gl_master_fNeraca.implement({
		...
		{ 
			...
			}else
			{
				if (sender == this.addBtn || sender == this.editBtn){
					uses("app_saku_gl_master_fNeracaDetail");
					if (this.entryMenu == undefined)
						this.entryMenu = new app_saku_gl_master_fNeracaDetail(this.app,this.e0.getText());
