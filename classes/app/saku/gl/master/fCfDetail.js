window.app_saku_gl_master_fCfDetail = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_master_fCfDetail.prototype.parent.constructor.call(this, owner);
		window.app_saku_gl_master_fCfDetail.prototype.parent.setWidth.call(this, 300);
		window.app_saku_gl_master_fCfDetail.prototype.parent.setHeight.call(this, 400);
		
		this.centerize();
		
		this.className = "app_saku_gl_master_fCfDetail";
...
//		this.p1.setColor(system.getConfig("app.color.panel"));
		...
		this.e3.addItem("Summary Posted","SummPosted");
		this.e3.addItem("Mutasi","Mutasi");
		this.e3.addItem("Sawal","Sawal");
		...
		this.e5.addItem("D","Debet");
		this.e5.addItem("C","Kredit");		
...
window.app_saku_gl_master_fCfDetail.extend(window.portalui_commonForm);
window.app_saku_gl_master_fCfDetail.prototype.doDraw = function(canvas)
{
	var n = this.getFullId();
//	canvas.style.background = "url(icon/"+system.getThemes()+"/background.png) repeat";
...
window.app_saku_gl_master_fCfDetail.prototype.doAfterResize = function(event)
{
	this.p1.setWidth(this.width - 40);
...
window.app_saku_gl_master_fCfDetail.prototype.doSysMouseMove = function(x, y, button, buttonState)
{
	window.app_saku_gl_master_fCfDetail.prototype.parent.doSysMouseMove.call(this,x, y, button, buttonState);
	if (this.isClick)
	{		
...
window.app_saku_gl_master_fCfDetail.prototype.eventMouseDown = function(event)
{
...
window.app_saku_gl_master_fCfDetail.prototype.eventMouseUp = function(event)
{
	this.isClick = false;
	this.blockElm.style.display = "none";
	this.frameElm.style.display = "";
};
window.app_saku_gl_master_fCfDetail.prototype.eventMouseMove = function(event)
{
	if (this.isClick)
	{
		var x = event.clientX;
...
window.app_saku_gl_master_fCfDetail.prototype.doClick = function(sender)
{
	if (sender == this.b1)
...
window.app_saku_gl_master_fCfDetail.prototype.doRequestReady = function(sender, methodName, result)
{
};
window.app_saku_gl_master_fCfDetail.prototype.findBtnClick = function(sender)
{
};

//--------------------------------------------
window.app_saku_gl_master_fCfDetail.prototype.setCaption = function(data)
{
...
window.app_saku_gl_master_fCfDetail.prototype.setItemParent = function(data)
{
	this.itemParent = data;
};
window.app_saku_gl_master_fCfDetail.prototype.setSummaryItems = function(data)
{
	this.e4.clearItem();
	for (var i in data)
		this.e4.addItem(i, data[i]);	
};
window.app_saku_gl_master_fCfDetail.prototype.doClose = function(sender)
{
