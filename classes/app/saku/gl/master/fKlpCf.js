window.app_saku_gl_master_fKlpCf = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_master_fKlpCf.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_gl_master_fKlpCf";
		this.setTop(60);
		this.setWidth(1280);
		this.setHeight(550);
...
window.app_saku_gl_master_fKlpCf.extend(window.portalui_childForm);
//------------------------------------------------------------------ event
window.app_saku_gl_master_fKlpCf.prototype.mainButtonClick = function(sender){
...
window.app_saku_gl_master_fKlpCf.prototype.doModalResult = function(event, modalResult){
	switch (event)
	{
		case "clear" :
			if (modalResult == mrOk)
		...
		case "simpan" :
			if (modalResult == mrOk)
			{
				try
				{
					...
					sql.add("insert into klp_cf (klp_cf, nama,tgl_awal, tgl_akhir, flag_status, tgl_input,nik_user, kode_lokasi ) values ('"+this.e0.getText()+"','"+this.e1.getText()+"','"+this.d1.getDate()+"','"+this.d2.getDate()+"','"+status+"','"+(new Date).getDateStr()+"','"+this.app._userLog+"','"+this.elokasi.getText()+"' ) ");
		...
		case "ubah" :
			if (modalResult == mrOk)
			{
				try
				{
					...
					uses("server_util_arrayList");
					sql = new server_util_arrayList();
					sql.add("update klp_cf set nama = '"+this.e1.getText()+"',flag_status = '"+status+"',tgl_awal='"+this.d1.getDate()+"',tgl_akhir='"+this.d2.getDate()+"',tgl_input='"+(new Date).getDateStr()+"',nik_user='"+this.app._userLog+"' where klp_cf = '"+this.e0.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"' ");
					this.dbLib.execArraySQL(sql);	
				}catch(e){
					system.alert(this, e,"");
				}
		...
		case "hapus" :
		   if (modalResult == mrOk)
		   {
			    try
				{
					uses("server_util_arrayList");
					sql = new server_util_arrayList();
					var rs = this.dbLib.runSQL("select klp_cf from cf_m where klp_cf = '"+this.e0.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"' ");
					if (rs instanceof portalui_arrayMap)
						if (rs.getTag1() != 0)
							throw("kode fs ini sudah terpakai. tidak dapat dihapus");
					sql.add("delete from  klp_cf where klp_cf = '"+this.e0.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"' ");
					this.dbLib.execArraySQL(sql);	
				}catch(e){
					system.alert(this, e,"");
				}
...
window.app_saku_gl_master_fKlpCf.prototype.keyPress = function(sender, charCode, buttonState ){
};
window.app_saku_gl_master_fKlpCf.prototype.doEditChange = function(sender){
	if (this.e0.getText() != "")
	{
		try
		{			
			var data = this.dbLib.runSQL("select * from klp_cf where klp_cf = '"+this.e0.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"' ");
...
window.app_saku_gl_master_fKlpCf.prototype.FindBtnClick = function(sender, event)
{
	try
	{
		if (sender == this.elokasi){
			...
			this.standarLib.showListData(this, "Data Versi Cash Flow",sender,this.e1, 
										  ...
										  new Array("klp_cf","nama"),"and",new Array("Kode FS Custom","Nama"));
	}catch(e){
		systemAPI.alert(e);
	}
};
window.app_saku_gl_master_fKlpCf.prototype.doRequestReady = function(sender, methodName, result)
{	
