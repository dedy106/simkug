window.app_saku_gl_report_flStrukturKeuangan = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_report_flStrukturKeuangan.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_report_flStrukturKeuangan";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Struktur Keuangan", 2);
	...
	this.filterRep.setSGFilterRowTipe(this.sg1, 0,new Array(0,1,2,3,4),new  Array("13","123","123","03","3"));
...
window.app_saku_gl_report_flStrukturKeuangan.extend(window.portalui_childForm);
window.app_saku_gl_report_flStrukturKeuangan.implement({
	...
	doSelectCell: function(sender, col, row){
		if (this.userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1,row,new Array(0,1,2,3,4),new  Array("13","123","123","03","3"));
			this.filterRep.setSGFilterRowButtonStyle(this.sg1,row,new Array(0,1,2,3,4),new  Array(2,2,2,0,0));
		}else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1,row,new Array(0,1,2,3,4),new  Array("3","123","123","03","3"));
			this.filterRep.setSGFilterRowButtonStyle(this.sg1,row,new Array(0,1,2,3,4),new  Array(3,2,2,0,0));
		}
		if (row == 3)
		{
		...
		try
		{
			...
			else
			{
	    		...
	    		if (this.sg1.getCell(2,4)=="COA")
				{
	    			...
	    			this.viewer.setTotalPage(this.report.getTotalPage("server_report_gl_rptStrukturKeuangan",this.filter,this.pager,this.sg1.getCell(2,4)));
					this.app._mainForm.reportNavigator.setTotalPage(this.viewer.getTotalPage());
					this.app._mainForm.reportNavigator.rearrange();
					this.showFilter = this.filterRep.showFilter(this.sg1);
					this.report.preview("server_report_gl_rptStrukturKeuangan",this.filter, 1, this.pager, this.showFilter, this.app._namalokasi,this.sg1.getCell(2,4));	
	    		}
				else
				{
			...
			systemAPI.alert("[app_saku_gl_report_flStrukturKeuangan]::mainButtonClick:"+e);
		}
	},
	...
	doSelectedPage: function(sender, page){
		this.report.preview("server_report_gl_rptStrukturKeuangan",this.filter, page, this.pager, this.showFilter, this.app._namalokasi,this.sg1.getCell(2,4));
		this.page=page;		
		this.allBtn = false;
	},
	doCloseReportClick: function(sender){		
	  switch(sender.getName())
	  {
	    case "allBtn" :
			this.page = 1;
	        this.report.preview("server_report_gl_rptStrukturKeuangan",this.filter, this.page, this.viewer.getTotalPage() * this.pager, this.showFilter,this.app._namalokasi,this.sg1.getCell(2,4)); 			
			this.allBtn = true;
	      break;
	    case "pdfBtn" :      
	      this.viewer.useIframe(this.report.createPdf("server_report_gl_rptStrukturKeuangan",this.filter, (this.allBtn ? 1 : this.page), (this.allBtn ? this.viewer.getTotalPage() * this.pager : this.pager), this.showFilter, this.app._namalokasi,this.sg1.getCell(2,4)));
	      break;
	    case "xlsBtn" :
	      this.viewer.useIframe(this.report.createXls("server_report_gl_rptStrukturKeuangan",this.filter, (this.allBtn ? 1 : this.page), (this.allBtn ? this.viewer.getTotalPage() * this.pager : this.pager), this.showFilter, this.app._namalokasi,this.sg1.getCell(2,4)));       
	      break; 
		case "PreviewBtn" :
			window.open(this.report.previewWithHeader("server_report_gl_rptStrukturKeuangan",this.filter, (this.allBtn ? 1 : this.page),(this.allBtn ? this.viewer.getTotalPage() * this.pager : this.pager), this.showFilter,this.app._namalokasi,this.sg1.getCell(2,4)));
		break;
		case "PrintBtn" :        
			this.viewer.useIframe(this.report.previewWithHeader("server_report_gl_rptStrukturKeuangan",this.filter, (this.allBtn ? 1 : this.page),(this.allBtn ? this.viewer.getTotalPage() * this.pager : this.pager), this.showFilter,this.app._namalokasi,this.sg1.getCell(2,4)));
	      try
	      {
		...
		try{
			if (sender === sender.owner.bConfirm){
				...
				if (to !== ""){
					sender.owner.free();
					var d = new Date();
					var subject = "Laporan Struktur Keuangan "+d.toLocaleString();
