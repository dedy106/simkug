// JavaScript Document
window.app_saku_gl_report_flModal = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_report_flModal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_report_flModal";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Perubahan Ekuitas", 2);
...
window.app_saku_gl_report_flModal.extend(window.portalui_childForm);
window.app_saku_gl_report_flModal.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.filter2 = this.nik_user+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,4);
			var in_lap="S";
			if (this.sg1.getCell(1,1) == "All")
			{
			...
			{
				sql="call sp_modal ('"+this.sg1.getCell(2,2)+"','M','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,0)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.nik_user+"')";
			}
			
			this.dbLib.execQuerySync(sql);	
			
			this.nama_report="server_report_gl_rptModal";
		...
		try{
			if (sender === sender.owner.bConfirm){
				...
				if (to !== ""){
					sender.owner.free();
					var d = new Date();
					var subject = "Laporan Perubahan Ekuitas "+d.toLocaleString();
