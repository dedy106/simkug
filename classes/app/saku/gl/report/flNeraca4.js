// JavaScript Document
window.app_saku_gl_report_flNeraca4 = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_report_flNeraca4.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_report_flNeraca4";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Neraca Empat Periode", 2);
...
window.app_saku_gl_report_flNeraca4.extend(window.portalui_childForm);
window.app_saku_gl_report_flNeraca4.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			{
				sql="call sp_neraca4 ('"+this.sg1.getCell(2,2)+"','N','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,4)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,6)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.nik_user+"')";
			}
			else
			{
			...
			this.nama_report="server_report_gl_rptNeraca4";
			this.viewer.setTotalPage(this.report.getTotalPage(this.nama_report,this.filter,this.pager));
			this.app._mainForm.reportNavigator.setTotalPage(this.viewer.getTotalPage());
			this.app._mainForm.reportNavigator.rearrange();
			this.filter2 = filter2;
			this.report.preview(this.nama_report,this.filter, 1, this.viewer.getTotalPage() * this.pager, this.showFilter, this.lokasi,filter2);
						
		}catch(e){
			systemAPI.alert("[flNeraca4]::mainButtonClick:"+e);
		}
	},
		...
		try{
			if (sender === sender.owner.bConfirm){
				...
				if (to !== ""){
					sender.owner.free();
					var d = new Date();
					var subject = "Laporan Neraca Empat Periode "+d.toLocaleString();
