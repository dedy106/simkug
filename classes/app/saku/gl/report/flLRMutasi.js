// JavaScript Document
window.app_saku_gl_report_flLRMutasi = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_report_flLRMutasi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_report_flLRMutasi";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Laba Rugi Mutasi", 2);
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Bentuk Mutasi","=","Mutasi"));
...
window.app_saku_gl_report_flLRMutasi.extend(window.portalui_childForm);
window.app_saku_gl_report_flLRMutasi.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.filter2 = this.nik_user+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(2,5)+"/"+this.sg1.getCell(2,3);;
			var in_lap="S";
			...
			{
				this.nama_report="server_report_gl_rptLRMutasi";
			}
			else
			{
				this.nama_report="server_report_gl_rptLRMutasiDC";
			}	
		...
		{
			systemAPI.alert("[flLRMutasi]::mainButtonClick:"+e);
		}
	},
	...
	doCloseReportClick: function(sender){
	  ...
	  switch(sender.getName())
	  {
	    ...
	    case "xlsBtn" :
	      ...
	      break; 
		case "PreviewBtn" :
			window.open(this.report.previewWithHeader(this.nama_report,this.filter, 1, 300, this.showFilter, this.lokasi,this.filter2));
			break;
		case "PrintBtn" :
	      try
	      {
		...
		try{
			if (sender === sender.owner.bConfirm){
				...
				if (to !== ""){
					sender.owner.free();
					var d = new Date();
					var subject = "Laporan Laba Rugi Mutasi "+d.toLocaleString();
