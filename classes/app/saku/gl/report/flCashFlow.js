// JavaScript Document
window.app_saku_gl_report_flCashFlow = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_report_flCashFlow.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_report_flCashFlow";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Struktur Arus Kas", 2);
...
window.app_saku_gl_report_flCashFlow.extend(window.portalui_childForm);
window.app_saku_gl_report_flCashFlow.implement({
	    ...
	    if (row ==0)
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Neraca",this.sg1, this.sg1.row, this.sg1.col,
													  ...
													  this.filterRep.filterStr("kode_fs",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," where "),
												["kode_neraca","nama"],"and",["Kode Neraca","Nama"]);
			}
	},
		...
		try
		{
			...
			else
			{
	    		...
	    		if (this.sg1.getCell(2,4)=="COA")
				{
	    			...
	    			this.viewer.setTotalPage(this.report.getTotalPage("server_report_gl_rptCashFlow",this.filter,this.pager,this.sg1.getCell(2,4)));
					this.app._mainForm.reportNavigator.setTotalPage(this.viewer.getTotalPage());
					this.app._mainForm.reportNavigator.rearrange();
					this.showFilter = this.filterRep.showFilter(this.sg1);
					this.report.preview("server_report_gl_rptCashFlow",this.filter, 1, this.pager, this.showFilter, this.app._namalokasi,this.sg1.getCell(2,4));	
	    		}
				else
				{
			...
			systemAPI.alert("[app_saku_gl_report_flCashFlow]::mainButtonClick:"+e);
		}
	},
	...
	doSelectedPage: function(sender, page){		
		this.report.preview("server_report_gl_rptCashFlow",this.filter, page, this.pager, this.showFilter, this.app._namalokasi,this.sg1.getCell(2,4));
		this.page=page;		
		this.allBtn = false;
	},
	doCloseReportClick: function(sender){	
	  ...
	  {
	    case "allBtn" :
			this.report.preview("server_report_gl_rptCashFlow",this.filter, 1, this.viewer.getTotalPage() * this.pager, this.showFilter, this.app._namalokasi,this.sg1.getCell(2,4));
			this.page=1;		
			this.allBtn = true;	
	      break;
	    case "pdfBtn" :      
	      this.viewer.useIframe(this.report.createPdf("server_report_gl_rptCashFlow",this.filter, (this.allBtn ? 1 : this.page), (this.allBtn ? this.viewer.getTotalPage() * this.pager : this.pager), this.showFilter, this.app._namalokasi,this.sg1.getCell(2,4)));
	      break;
	    case "xlsBtn" :
	      this.viewer.useIframe(this.report.createXls("server_report_gl_rptCashFlow",this.filter, (this.allBtn ? 1 : this.page), (this.allBtn ? this.viewer.getTotalPage() * this.pager : this.pager), this.showFilter, this.app._namalokasi,this.sg1.getCell(2,4)));       
	      break; 
		case "PreviewBtn" :
			window.open(this.report.previewWithHeader("server_report_gl_rptCashFlow",this.filter, (this.allBtn ? 1 : this.page), (this.allBtn ? this.viewer.getTotalPage() * this.pager : this.pager), this.showFilter, this.app._namalokasi,this.sg1.getCell(2,4)));
			break;
		case "PrintBtn" :        
			this.viewer.useIframe(this.report.previewWithHeader("server_report_gl_rptCashFlow",this.filter, (this.allBtn ? 1 : this.page),(this.allBtn ? this.viewer.getTotalPage() * this.pager : this.pager), this.showFilter,this.app._namalokasi,this.sg1.getCell(2,4)));
	      try
	      {
		...
		try{
			if (sender === sender.owner.bConfirm){
				...
				if (to !== ""){
					sender.owner.free();
					var d = new Date();
					var subject = "Laporan Struktur Arus Kas "+d.toLocaleString();
