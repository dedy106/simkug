window.app_saku_gl_report_flTAK = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_report_flTAK.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_report_flTAK";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan TAK Umum", 2);
...
window.app_saku_gl_report_flTAK.extend(window.portalui_childForm);
window.app_saku_gl_report_flTAK.implement({
		...
		try{			
			...
			if (row === 2)
			{
				this.filterRep.ListDataSGFilter(this, "Daftar TAK",this.sg1, this.sg1.row, this.sg1.col,
											  "select no_ju, keterangan from ju_m where kode_lokasi ='"+this.sg1.getCell(2,0)+"' ",
											  "select count(*) from ju_m where kode_lokasi ='"+this.sg1.getCell(2,0)+"' ",
											  ["no_ju","keterangan"],"and",["No Bukti","Keterangan"]);
			}			
		}catch(e){
			systemAPI.alert(e);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2){
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("b.no_ju",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new portalui_arrayMap();
				var sql = "select a.no_ju, date_format(a.tanggal,'%d/%m/%Y') as tgl, a.keterangan, a.ref1 as akun_tak, a.kode_lokasi2, a.no_bukti2, "+
						" b.kode_akun, c.nama, case when dc='d' then b.nilai else 0 end as debet, case when dc='c' then b.nilai else 0 end as kredit  "+
						" from ju_m a inner join ju_j b on b.no_ju = a.no_ju and b.kode_lokasi = a.kode_lokasi "+
						"	inner join masakun c on c.kode_akun = b.kode_akun and c.kode_lokasi = a.kode_lokasi "+
						this.filter +" and a.jenis = 'TAK' order by a.no_ju, a.tanggal" ;
				this.scriptSqlCount = "select count(distinct a.no_ju) "+						
						" from ju_m a inner join ju_j b on b.no_ju = a.no_ju and b.kode_lokasi = a.kode_lokasi "+
						"	inner join masakun c on c.kode_akun = b.kode_akun and c.kode_lokasi = a.kode_lokasi "+
						this.filter +" and a.jenis = 'TAK'" ;
				...
				this.groupBy = new server_util_arrayList({items:["no_ju","tgl","keterangan","akun_tak","kode_lokasi2","no_bukti2"]});
				this.summary = new server_util_arrayList({items:["N","N","N","N","N","N","N","N","Y","Y"]});
				this.groupHeader = new server_util_arrayList({items:["no_ju"]});
			...
			systemAPI.alert("[app_saku_gl_report_flTAK]::mainButtonClick:"+e);
		}
	},
