// JavaScript Document
window.app_saku_gl_report_flStrukKeuCustom = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_report_flStrukKeuCustom.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_report_flStrukKeuCustom";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Struktur Keuangan Custom", 2);
...
window.app_saku_gl_report_flStrukKeuCustom.extend(window.portalui_childForm);
window.app_saku_gl_report_flStrukKeuCustom.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 2)
		{
			this.filterRep.ListDataSGFilter(this, "Data Neraca",this.sg1, this.sg1.row, this.sg1.col,
												  "select kode_custom,nama from custom "+
												  this.filterRep.filterStr("kode_fs",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," where ")+
												  "order by kode_custom",
												  "select count(kode_custom) from custom "+
												  this.filterRep.filterStr("kode_fs",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," where "),
											new Array("kode_custom","nama"),"and");
		}
	},
	...
	mainButtonClick: function(sender){
		try
		{
	        ...
	        this.p1.setVisible(false);
	    		...
	    		if (this.sg1.getCell(2,4)=="COA")
				{
					this.filter = this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							 ...
							 this.filterRep.filterStr("kode_custom",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							 this.filterRep.filterStr("modul",this.sg1.getCell(1,3),this.sg1.getCell(2,3).charAt(0),this.sg1.getCell(3,3).charAt(0),"and");
	    			this.viewer.setTotalPage(this.report.getTotalPage("server_report_gl_rptStrukKeuCustom",this.filter,this.pager,this.sg1.getCell(2,4)));
					this.app._mainForm.reportNavigator.setTotalPage(this.viewer.getTotalPage());
					this.app._mainForm.reportNavigator.rearrange();
					this.showFilter = this.filterRep.showFilter(this.sg1);
					this.report.preview("server_report_gl_rptStrukKeuCustom",this.filter, 1, this.pager, this.showFilter, this.app._namalokasi,this.sg1.getCell(2,4));	
	    		}
				else
				{
					this.filter =this.filterRep.filterStr("n.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
								...
								this.filterRep.filterStr("n.kode_custom",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
			...
			systemAPI.alert("[app_saku_gl_report_flStrukKeuCustom]::mainButtonClick:"+e);
		}
	},
	...
	doSelectedPage: function(sender, page){			
		this.report.preview("server_report_gl_rptStrukKeuCustom",this.filter, page, this.pager, this.showFilter, this.app._namalokasi,this.sg1.getCell(2,4));
		this.page=page;	
		this.allBtn = false;
	},
	doCloseReportClick: function(sender){		
	  switch(sender.getName())
	  {
	    case "allBtn" :
			this.page = 1;
			this.report.preview("server_report_gl_rptStrukKeuCustom",this.filter, this.page, this.viewer.getTotalPage() * this.pager, this.showFilter, this.app._namalokasi,this.sg1.getCell(2,4));	
			this.allBtn = true;
	      break;
	    case "pdfBtn" :      
	      this.viewer.useIframe(this.report.createPdf("server_report_gl_rptStrukKeuCustom",this.filter, (this.allBtn ? 1 : this.page),(this.allBtn ? this.viewer.getTotalPage() * this.pager : this.pager), this.showFilter, this.app._namalokasi,this.sg1.getCell(2,4)));
	      break;
	    case "xlsBtn" :
	      this.viewer.useIframe(this.report.createXls("server_report_gl_rptStrukKeuCustom",this.filter, (this.allBtn ? 1 : this.page), (this.allBtn ? this.viewer.getTotalPage() * this.pager : this.pager), this.showFilter, this.app._namalokasi,this.sg1.getCell(2,4)));       
	      break; 
		case "PreviewBtn" :
			window.open(this.report.previewWithHeader("server_report_gl_rptStrukKeuCustom",this.filter, (this.allBtn ? 1 : this.page), (this.allBtn ? this.viewer.getTotalPage() * this.pager : this.pager), this.showFilter, this.app._namalokasi,this.sg1.getCell(2,4)));
			break;
		case "PrintBtn" :        
			this.viewer.useIframe(this.report.previewWithHeader("server_report_gl_rptStrukKeuCustom",this.filter, (this.allBtn ? 1 : this.page),(this.allBtn ? this.viewer.getTotalPage() * this.pager : this.pager), this.showFilter,this.app._namalokasi,this.sg1.getCell(2,4)));
	      try
	      {
		...
		try{
			if (sender === sender.owner.bConfirm){
				...
				if (to !== ""){
					sender.owner.free();
					var d = new Date();
					var subject = "Laporan Struktur Keuangan Custom "+d.toLocaleString();
