// JavaScript Document
window.app_saku_gl_report_flLRJejer = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_report_flLRJejer.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_report_flLRJejer";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Laba Rugi Jejer", 2);
...
window.app_saku_gl_report_flLRJejer.extend(window.portalui_childForm);
window.app_saku_gl_report_flLRJejer.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			{
				sql="call sp_neraca_jejer ('"+this.sg1.getCell(2,2)+"','L','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,1)+"','"+this.kode_lokasi+"','"+this.nik_user+"')";
			}
			this.dbLib.execQuerySync(sql);	
			
			this.nama_report="server_report_gl_rptLRJejer";
		...
		try{
			if (sender === sender.owner.bConfirm){
				...
				if (to !== ""){
					sender.owner.free();
					var d = new Date();
					var subject = "Laporan Laba Rugi Jejer "+d.toLocaleString();
