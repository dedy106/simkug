// JavaScript Document
window.app_saku_gl_report_flNeracaAgg = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_report_flNeracaAgg.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_report_flNeracaAgg";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Neraca Anggaran", 2);
...
window.app_saku_gl_report_flNeracaAgg.extend(window.portalui_childForm);
window.app_saku_gl_report_flNeracaAgg.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			//var filter2 = this.nik_user+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,3);
			...
			{
				sql="call sp_neraca_agg ('"+this.sg1.getCell(2,2)+"','N','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,1)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.nik_user+"')";
			}
			else
			{
				sql="call sp_neraca_agg ('"+this.sg1.getCell(2,2)+"','A','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,1)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.nik_user+"')";
			}
		...
		{
			systemPAI.alert("[flNeracaAgg]::mainButtonClick:"+e);
		}
	},
	...
	doCloseReportClick: function(sender){	
	  switch(sender.getName())
	  {
		...
		case "PrintBtn" :
	      try
	      {
			//this.report.preview("server_report_anggaran_rptRkap",this.filter, 1, 300, this.showFilter, this.lokasi,this.filter2);
			...
			//this.viewer.useIframe(this.report.previewWithHeader("server_report_GL_rptBuktiJurnal",this.filter, 1, this.viewer.getTotalPage() * 30, this.lokasi,this.showFilter));
		...
		try{
			if (sender === sender.owner.bConfirm){
				...
				if (to !== ""){
					sender.owner.free();
					var d = new Date();
					var subject = "Laporan Neraca Anggaran "+d.toLocaleString();
