// JavaScript Document
window.app_saku_gl_report_flJurnal2 = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_report_flJurnal2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_report_flJurnal2";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,8,new Array(0,1,2), new Array("Order by","=","No. Bukti"));
...
window.app_saku_gl_report_flJurnal2.extend(window.portalui_childForm);
window.app_saku_gl_report_flJurnal2.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 4)
		{
			this.filterRep.ListDataSGFilter(this, "Data Modul",this.sg1, this.sg1.row, this.sg1.col,
													  "select kode_jenis, nama from ju_jenis "+
													   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
													  "select count(kode_jenis) from ju_jenis "+
													  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
													  new Array("kode_jenis","nama"),"where");
		}
		if (row == 7)
		{
			...
			var filter=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
						...
						this.filterRep.filterStr("b.posted",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");
			
			var sql1="select distinct a.no_ju,a.no_dokumen from ju_j a "+
					 "inner join ju_m b on a.no_ju=b.no_ju and a.kode_lokasi=b.kode_lokasi "+filter;
			var sql2="select count(a.no_ju) from ju_j a "+
					 "inner join ju_m b on a.no_ju=b.no_ju and a.kode_lokasi=b.kode_lokasi "+filter;
			this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_ju","no_dokumen"),"and");
		}
	},
	doSelectCell: function(sender, col, row){
		if (this.userStatus=="A"){
			this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5,6,7,8),new  Array("123","3","123","123","123","13","3","123","3"));
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5,6,7,8),new  Array(2,0,2,2,2,0,0,2,0));
		}else{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5,6,7,8),new  Array("123","3","123","123","123","13","3","123","3"));
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5,6,7,8),new  Array(3,0,2,2,2,0,0,2,0));
		}
		if (row == 1)
		{
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Daftar Jurnal","Bukti Jurnal"));
		}
		if (row == 8)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("No. Bukti","No. Dokumen"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			{
				this.nama_report="server_report_gl_rptJurnal2";
				if (this.sg1.getCell(2,8) === "No. Bukti")
					this.filter += " order by a.no_ju,a.dc desc ";
				else this.filter += " order by a.no_dokumen,a.dc desc ";
			}
			else
			{
				this.nama_report="server_report_gl_rptBuktiJurnal2";
			}
