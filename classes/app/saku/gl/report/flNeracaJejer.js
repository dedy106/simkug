// JavaScript Document
window.app_saku_gl_report_flNeracaJejer = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_report_flNeracaJejer.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_report_flNeracaJejer";
		this.maximize();
...
window.app_saku_gl_report_flNeracaJejer.extend(window.portalui_childForm);
window.app_saku_gl_report_flNeracaJejer.implement({
	...
	mainButtonClick: function(sender){	
		if (sender == this.app._mainForm.bPreview){
			try
			{
				...
				this.nama_report="server_report_gl_rptNeracaJejer";
				sql="call sp_neraca_jejer ('"+this.sg1.getCell(2,2)+"','N','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,1)+"','"+this.kode_lokasi+"','"+this.nik_user+"')";
		...
		try{
			if (sender === sender.owner.bConfirm){
				...
				if (to !== ""){
					sender.owner.free();
					var d = new Date();
					var subject = "Laporan Neraca Jejer "+d.toLocaleString();
