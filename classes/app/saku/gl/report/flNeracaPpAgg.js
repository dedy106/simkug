// JavaScript Document
window.app_saku_gl_report_flNeracaPpAgg = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_report_flNeracaPpAgg.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_report_flNeracaPpAgg";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Neraca PP Anggaran", 2);
	...
	this.doSelectCell(this.sg1,3,5);
...
window.app_saku_gl_report_flNeracaPpAgg.extend(window.portalui_childForm);
window.app_saku_gl_report_flNeracaPpAgg.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5),new  Array("3","3","123","3","3","23"));
	...
	mainButtonClick: function(sender){
		try
		{
			...
			sql="call sp_neraca_pp_agg ('"+this.sg1.getCell(2,2)+"','N','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,1)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.nik_user+"')";
			
			this.dbLib.execQuerySync(sql);	
			
			this.nama_report="server_report_gl_rptNeracaAgg";
	...
	doCloseReportClick: function(sender){	
	  switch(sender.getName())
	  {
		...
		case "PrintBtn" :
	      try
	      {
			this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,this.filter, 1, this.pager * this.viewer.getTotalPage(), this.showFilter, this.lokasi,this.filter2));
		...
		try{
			if (sender === sender.owner.bConfirm){
				...
				if (to !== ""){
					sender.owner.free();
					var d = new Date();
					var subject = "Laporan Laba Rugi PP Anggaran "+d.toLocaleString();
