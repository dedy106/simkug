// JavaScript Document
window.app_saku_gl_report_flTBJejer = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_report_flTBJejer.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_report_flTBJejer";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Trial Balance Jejer", 2);
...
window.app_saku_gl_report_flTBJejer.extend(window.portalui_childForm);
window.app_saku_gl_report_flTBJejer.implement({
		...
		try
		{
			...
			if (this.app._dbEng == "mysqlt")
			{
				...
				if (this.sg1.getCell(2,3)=="Neraca Lajur")
				{
					...
					this.nama_report="server_report_gl_rptTBJejer";
				}
		...
		try{
			if (sender === sender.owner.bConfirm){
				...
				if (to !== ""){
					sender.owner.free();
					var d = new Date();
					var subject = "Laporan Neraca Lajur Jejer "+d.toLocaleString();
