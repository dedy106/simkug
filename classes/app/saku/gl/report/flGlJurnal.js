window.app_saku_gl_report_flGlJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_report_flGlJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_report_flGlJurnal";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Jurnal Transaksi Modul", 2);
	...
	var periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from periode where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Modul","=","Jurnal Umum"]);
...
window.app_saku_gl_report_flGlJurnal.extend(window.portalui_childForm);
window.app_saku_gl_report_flGlJurnal.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==0)
	{
	...
	if (row == 6)
	{
		var modul="";
			...
			{
				case "Jurnal Umum":
					modul=" and modul in ('JU','JP','00') ";
					break;
				case "KasBank Keluar":
					modul=" and modul in ('KBO_ALB','SPP','KBO_NP') ";
					break;
				case "KasBank Masuk":
					modul=" and modul in ('KBI_NP') ";
					break;
				case "Piutang":
					modul=" and modul in ('AR','ARIM','ARMHS','ARCD') ";
					break;
				case "Asset":
					modul=" and modul in ('FA_SST','05') ";
					break;
			}
		...
		var sql1="select distinct no_bukti,no_dokumen,tanggal from gldt "+filter;
		var sql2="select count(distinct no_bukti) from gldt "+filter;
		this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_bukti","no_dokumen","tanggal"),"and",["No Bukti","No Dokumen","Tanggal"]);
	}
};
window.app_saku_gl_report_flGlJurnal.prototype.doSelectCell = function(sender, col, row)
{
	...
	if (row == 1)
	{
		...
		this.dbLib.setItemsFromSQL("select periode from periode where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
	}
	if (row == 2)
	{
		this.sg1.columns.get(2).pickList.clear(); 
		this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["Jurnal Umum","KasBank Masuk","KasBank Keluar","Piutang","Asset"]);
	}
};
window.app_saku_gl_report_flGlJurnal.prototype.mainButtonClick = function(sender)
{
	try
	{
		if (sender === this.app._mainForm.bClear2)
		{
			...
			this.filter2 = this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,1);
			this.filter=this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
						...
						this.filterRep.filterStr("no_bukti",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and")+modul ;
			this.nama_report="server_report_gl_rptGlJurnal";
...
window.app_saku_gl_report_flGlJurnal.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_gl_report_flGlJurnal.prototype.doSelectedPage = function(sender, page)
{
...
window.app_saku_gl_report_flGlJurnal.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_gl_report_flGlJurnal.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_gl_report_flGlJurnal.prototype.doRowPerPageChange = function(sender, rowperpage)
{
