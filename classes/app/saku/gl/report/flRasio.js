// JavaScript Document
window.app_saku_gl_report_flRasio = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_report_flRasio.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_report_flRasio";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Rasio Keuangan", 2);
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Versi Rasio","=","V1"));
...
window.app_saku_gl_report_flRasio.extend(window.portalui_childForm);
window.app_saku_gl_report_flRasio.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 2)
		{
			this.filterRep.ListDataSGFilter(this, "Data FS",this.sg1, this.sg1.row, this.sg1.col,
													  "select klp_rasio,nama from klp_rasio "+
													  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," where ")+" order by klp_rasio",
													  "select count(klp_rasio) from klp_rasio "+
	...
	doSelectCell: function(sender, col, row){
		if (this.userStatus=="A")
		{
			...
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2),new  Array(0,2,2,0));
		}
		else
		{
	...
	mainButtonClick: function(sender){	
		if (sender == this.app._mainForm.bPreview){
			try
			{
				...
				this.nama_report="server_report_gl_rptRasio";
		...
		try{
			if (sender === sender.owner.bConfirm){
				...
				if (to !== ""){
					sender.owner.free();
					var d = new Date();
					var subject = "Laporan Rasio Keuangan "+d.toLocaleString();
