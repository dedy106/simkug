// JavaScript Document
window.app_saku_gl_report_flCf = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_report_flCf.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_report_flCf";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Lokasi",tanda,lokasi));
...
window.app_saku_gl_report_flCf.extend(window.portalui_childForm);
window.app_saku_gl_report_flCf.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 2)
		{
			this.filterRep.ListDataSGFilter(this, "Data FS",this.sg1, this.sg1.row, this.sg1.col,
													  "select klp_cf,nama from klp_cf "+
													  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," where ")+" order by klp_cf",
													  "select count(klp_cf) from klp_cf "+
	...
	doSelectCell: function(sender, col, row){
		if (this.userStatus=="A")
		{
			...
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2],[0,2,2,0]);
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["1234","3","123","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2],[0,2,2,0]);
		}
		if (row == 0)
		{
		...
		if (row == 0)
		{
			...
			else
			{
				...
				if (col==3)
				{
					this.standar.isiItemsWithPeriodeLok(this.sg1.getCell(2,1),this.sg1.columns.get(3).pickList);
				}
	...
	mainButtonClick: function(sender){	
		if (sender == this.app._mainForm.bPreview){
			try
			{
				...
				this.filter2 = this.nik_user+"/"+this.sg1.getCell(2,0);
				...
				this.nama_report="server_report_gl_rptCf";
		...
		try{
			if (sender === sender.owner.bConfirm){
				...
				if (to !== ""){
					sender.owner.free();
					var d = new Date();
					var subject = "Laporan Arus Kas "+d.toLocaleString();
