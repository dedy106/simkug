// JavaScript Document
window.app_saku_gl_report_flNeracaMutasi = function(owner)
{
	if (owner)
	{
		window.app_saku_gl_report_flNeracaMutasi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gl_report_flNeracaMutasi";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Neraca Mutasi", 2);
...
window.app_saku_gl_report_flNeracaMutasi.extend(window.portalui_childForm);
window.app_saku_gl_report_flNeracaMutasi.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[flNeracaMutasi]::mainButtonClick:"+e);
		}
	},
		...
		if (sender == this.dbLib){
			switch (methodName)
			{
				case "execQuery" : 
					...
					{
						this.nama_report="server_report_gl_rptNeracaMutasi";
					}
					else
					{
						this.nama_report="server_report_gl_rptNeracaMutasiDC";
					}	
		...
		try{
			if (sender === sender.owner.bConfirm){
				...
				if (to !== ""){
					sender.owner.free();
					var d = new Date();
					var subject = "Laporan Neraca Mutasi "+d.toLocaleString();
