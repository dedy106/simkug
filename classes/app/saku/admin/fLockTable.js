window.app_saku_admin_fLockTable = function(owner)
{
	if (owner)
	{
		window.app_saku_admin_fLockTable.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku_admin_fLockTable";
...
window.app_saku_admin_fLockTable.extend(window.portalui_childForm);
window.app_saku_admin_fLockTable.implement({
    				...
    				if (modalResult == mrOk)
    				{
    					try{
    						...
    						if (this.app._dbEng == "mysqlt"){
    							...
    							for (var i in this.sg1.rows.objList){							    								
    								sql.add("insert into locktable (nm_table, status, user_id, tgl_modified) values('"+this.sg1.cells(0,i)+"','"+this.sg1.cells(1,i)+"','"+this.app._userLog+"',getDate())");    								
    							}
    						}    						
