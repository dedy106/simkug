...
* 			SAI, PT											
***********************************************************************************************/
window.app_saku_sms_transaksi_fSendSMS2 = function(owner)
{
	if (owner)
	{
		window.app_saku_sms_transaksi_fSendSMS2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_sms_transaksi_fSendSMS2";
		this.setTop(60);
		this.maximize();
		this.itemsValue = new portalui_arrayList();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Kirim info gaji via SMS", 7);	
...
//------------------------------------------------------------------------
		...
		this.sg = new portalui_saiGrid(this,{bound:[20, 13, 595, 200], colCount:6, colTitle:["Send","NPM","Nama","No Tel.","THP","Email"], 
			colWidth:[[5,4,3,2,1,0],[100,100,100,400,150,60]], buttonStyle:[[1],[bsEllips]], colFormat:[[0,4],[cfBoolean, cfNilai]], ellipsClick:[this,"doEllipse"]});				
...
window.app_saku_sms_transaksi_fSendSMS2.extend(window.portalui_childForm);
//------------------------------------------------------------------ event
window.app_saku_sms_transaksi_fSendSMS2.implement({
		...
		if (sender == this.app._mainForm.bClear){
			...
			for (var i=0; i < this.sg.rows.getLength();i++){
				if (this.sg.cells(3, i) != "-" && this.sg.cells(0,i).toLowerCase() == "true") {
					...
					first = false;
					this.sms.sendSms(this.sg.cells(3, i),this.e1.getText()+" "+this.sg.cells(4, i)+". Email anda "+this.sg.cells(5, i));
				}
			}					
			try{				
	...
	doClick: function(sender){
		try {
			if (sender == this.bBrowse) {
				var data = this.dbLib.getDataProvider("select a.nik, a.nama, a.no_ponsel, a.email, ifnull(b.thp,0) as gaji from karyawan a "+
					" left outer join gaji_m b on b.nik = a.nik and b.kode_lokasi = a.kode_lokasi and b.periode = '"+this.ePeriode.getText()+"' "+					 
				...
				if (typeof data != "string") {
					...
					for (var i in data.rs.rows) {
						line = data.rs.rows[i];
						this.sg.appendData(['False', line.nik, line.nama, line.no_ponsel, floatToNilai(line.gaji), line.email]);
					}
				}
				else 
