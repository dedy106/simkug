...
* 			SAI, PT											
***********************************************************************************************/
window.app_saku_sms_transaksi_fInbox = function(owner)
{
	if (owner)
	{
		window.app_saku_sms_transaksi_fInbox.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_sms_transaksi_fInbox";
		this.setTop(60);
		this.maximize();
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.bSend.setCaption("Refresh");
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Inbox SMS", 7);	
		this.onClose.set(this,"doClose");
//------------------------------------------------------------------------
		uses("portalui_pageControl;server_sms_sms");
		this.tp = new portalui_pageControl(this,{bound:[10,10,200,this.height - 20], childPage:["",""]});
		this.tp.setTop(10);
		this.tp.setLeft(10);
		this.tp.setWidth(200);		
		this.tp.childPage(["Folder","Cust Group"]);
		...
		this.sg.setColTitle(["From","No Telp","Date","Message","SMSC"]);
		this.sg.setColWidth([0,1,2,3],[150,80,110,350,100]);
...
window.app_saku_sms_transaksi_fInbox.extend(window.portalui_childForm);
//------------------------------------------------------------------ event
window.app_saku_sms_transaksi_fInbox.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_sms_transaksi_fInbox.prototype.doAfterResize = function(event, height, width)
{
...
window.app_saku_sms_transaksi_fInbox.prototype.doModalResult = function(event, modalResult)
{
	switch (event)
	{
		case "clear" :
...
window.app_saku_sms_transaksi_fInbox.prototype.doClick = function(sender)
{
	try{
		this.timer.setEnabled(false);
		var dt = this.sms.inbox();
...
window.app_saku_sms_transaksi_fInbox.prototype.doRequestReady = function(sender, methodName, result)
{	
			...
			case "getDataProviderPage" : 	
				...
				if (typeof(dtInbox) == "object"){
					...
					for (var i in dtInbox.rs.rows){
						...
						this.sg.appendData([line.nama, line.dari, line.tanggal, line.pesan, line.smscenter]);
					}
				}
				break;	
		}
...
window.app_saku_sms_transaksi_fInbox.prototype.doClose = function(){
	this.app._mainForm.bSend.setCaption("Send");
};
window.app_saku_sms_transaksi_fInbox.prototype.loadMenu = function(daftarNrc, treev)
{
	try
	{		
		this.isStr = true;
...
window.app_saku_sms_transaksi_fInbox.prototype.treeClick = function(item)
{
	try{
		if (item.owner == this.treev){
			var cust = this.getPostingLevel(item);
			cust = cust.split(";");			
			this.sqlScript = "select ifnull(b.nama, '-') as nama, a.dari, a.tanggal, a.pesan, a.smscenter from sms_inbox a "+
				"	left outer join sms_cust b on case when substring(b.no_telp,1,1) = '+' then b.no_telp else concat('+62', substring(b.no_telp,2,length(b.no_telp))) end = a.dari "+
				"where b.kode_klpcust in ("+cust+")";							
			this.scriptSqlCount = "select count(*) as tot from sms_inbox a "+
				"	left outer join sms_cust b on case when substring(b.no_telp,1,1) = '+' then b.no_telp else concat('+62', substring(b.no_telp,2,length(b.no_telp))) end = a.dari "+
				"where b.kode_klpcust in ("+cust+")";							
		}else {
...
window.app_saku_sms_transaksi_fInbox.prototype.getPostingLevel = function(item)
{	
...
window.app_saku_sms_transaksi_fInbox.prototype.doSelectedPage = function(sender, page)
{
