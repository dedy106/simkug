...
* 			SAI, PT											
***********************************************************************************************/
window.app_saku_sms_transaksi_fOutbox = function(owner)
{
	if (owner)
	{
		window.app_saku_sms_transaksi_fOutbox.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_sms_transaksi_fOutbox";
		this.setTop(60);
		this.maximize();
...
//------------------------------------------------------------------------
		...
		this.tp = new portalui_FEtabPanel(this, {bound:[20, 10, 200, this.height - 50]});
...
window.app_saku_sms_transaksi_fOutbox.extend(window.portalui_childForm);
//------------------------------------------------------------------ event
window.app_saku_sms_transaksi_fOutbox.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_sms_transaksi_fOutbox.prototype.doAfterResize = function(event, height, width)
{
...
window.app_saku_sms_transaksi_fOutbox.prototype.doModalResult = function(event, modalResult)
{
	switch (event)
	{
		case "clear" :
...
window.app_saku_sms_transaksi_fOutbox.prototype.doClick = function(sender)
{
	try{
		this.timer.setEnabled(false);
...
window.app_saku_sms_transaksi_fOutbox.prototype.doRequestReady = function(sender, methodName, result)
{	
	try{
...
window.app_saku_sms_transaksi_fOutbox.prototype.doClose = function(){
	this.app._mainForm.bSend.setCaption("Send");
};
window.app_saku_sms_transaksi_fOutbox.prototype.loadMenu = function(daftarNrc, treev)
{
	try
	{		
		this.isStr = true;
...
window.app_saku_sms_transaksi_fOutbox.prototype.treeClick = function(item)
{
	try{
...
window.app_saku_sms_transaksi_fOutbox.prototype.getPostingLevel = function(item)
{	
...
window.app_saku_sms_transaksi_fOutbox.prototype.doSelectedPage = function(sender, page)
{
