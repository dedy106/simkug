window.app_saku_logistik_transaksi_fSpblogk = function(owner)
{
	if (owner)
	{
		window.app_saku_logistik_transaksi_fSpblogk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_logistik_transaksi_fSpblogk";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Permintaan Pembayaran : Koreksi", 0);	
		...
		this.ed_nbapp.hide();
		this.ed_nbapp.setTag("1");
		
		this.ed_nbapp2 = new portalui_saiLabelEdit(this);
		this.ed_nbapp2.setLeft(550);
		this.ed_nbapp2.setTop(1);
		this.ed_nbapp2.setWidth(200);
		this.ed_nbapp2.setCaption("No Approve Lama");
		this.ed_nbapp2.setText(""); 
		this.ed_nbapp2.setReadOnly(true);
		this.ed_nbapp2.hide();
		this.ed_nbapp2.setTag("1");
...
window.app_saku_logistik_transaksi_fSpblogk.extend(window.portalui_childForm);
window.app_saku_logistik_transaksi_fSpblogk.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_logistik_transaksi_fSpblogk.prototype.ubah = function()
{
	this.bGen.click();
	if (this.standarLib.checkEmptyByTag(this, new Array("0","2")))
	{
		try
		{
			...
			else
			{
				...
				sql.add(" delete from spb_d where no_spb ='"+this.cb_bukti.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");			
			}
			
			if (this.stsFinalLama == "T")
			{
				if (parseFloat(this.periodeLama) < parseFloat(this.app._periode))
				{
					sql.add(" update fa_app set no_del = 'DEL' where no_faapp ='"+this.ed_nbapp2.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");			
				}
				else
				{
					sql.add(" delete from fa_app where no_faapp ='"+this.ed_nbapp2.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");			
					sql.add(" delete from fa_j where no_faapp ='"+this.ed_nbapp2.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");			
					sql.add(" delete from fa_spb where no_faapp ='"+this.ed_nbapp2.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");			
				}
			}
			
			
			//---------------------------------- SPB
			this.progress = this.progressLama;
...
window.app_saku_logistik_transaksi_fSpblogk.prototype.doModalResult = function(event, modalResult)
{
	...
	switch (event)
	{
		...
		case "ubah" :		
			if (((this.progressLama == "1") || (this.progressLama == "9") || (this.progressLama == "J")) && (this.progressPO == "F"))
			{
				system.alert(this,"Progress PO sudah dilevel finalisasi.","SPB dengan status dibawahnya tidak dapat dihapus.");
				return false;
			}
			if (((this.progressLama == "1") || (this.progressLama == "9")) && (this.progressPO == "J"))
			{
				system.alert(this,"Progress PO sudah dilevel finalisasi.","SPB dengan status dibawahnya tidak dapat dihapus.");
				return false;
			}
			...
			if (parseFloat(this.app._periode) < parseFloat(this.app._periode)) 
			{
		...
		case "hapus" :		
			...
			{	
				...
				sql.add(" update po_m set progress = '"+this.progressLama+"' where no_po ='"+this.cb_po.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");			
				if (parseFloat(this.periodeLama) < parseFloat(this.app._periode))
				{
					sql.add(" update spb_m set no_del = 'DEL' where no_spb ='"+this.cb_bukti.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");			
				}
...
window.app_saku_logistik_transaksi_fSpblogk.prototype.genClick = function(sender)
{
	try
	{
		if (sender == this.bTutup)
		{
			...
			this.ed_nbapp.hide(); this.ed_nbapp2.hide(); this.ed_nbapp2.setTag("9");
...
window.app_saku_logistik_transaksi_fSpblogk.prototype.finalClick = function(sender)
{
	try	
	{	
		if (this.cb_po.getText() != "")
		{
			...
			this.ed_nbapp.show(); if (this.stsFinalLama == "T") {this.ed_nbapp2.show(); this.ed_nbapp2.setTag("1");}
			...
			var data = this.dbLib.runSQL(" select a.no_spb,a.keterangan as deskripsi, c.kode_akun, e.nama as nama_akun,c.keterangan,c.dc,"+
										 ...
										 " where a.no_spb <> '"+this.cb_bukti.getText()+"' and b.no_bukti = '"+this.cb_po.getText()+"' and b.modul='PO' and a.no_del='-' and c.dc='D' and d.kode_flag in ('013','014') and b.sts_kapital='F'");		
...
window.app_saku_logistik_transaksi_fSpblogk.prototype.loadClick = function(sender)
{
	try
	{
		this.ed_spb.setReadOnly(false); this.ed_ppn.setReadOnly(false);
		this.cb_akun.setCaption("Akun ADP / UM");
		var strSql = " select a.periode,a.no_dokumen,a.no_invoice,a.tgl_invoice,a.no_fpajak,a.catatan,a.due_date,a.kode_curr,a.kurs,a.keterangan,a.nik_buat,a.nik_setuju,a.kode_terima,a.kode_pp,a.jenis,a.nilai,a.nilai_ppn, "+
		             "        b.nama as nama_buat, c.nama as nama_setuju, d.nama as nama_terima, e.nama as nama_pp, f.no_bukti as no_po, f.sts_kapital, f.bukti_prog, "+
					 "		  g.nilai-ifnull(h.nilai_spb,0) as sisa_po,g.nilai_ppn-ifnull(h.ppn_spb,0) as sisa_ppn,"+										  
		             " 		  (case '"+this.ppnlog+"' when '1' then g.nilai+g.nilai_ppn else g.nilai end) as nilai_po,g.progress "+
					 ...
					 "              inner join spb_d f on a.no_spb=f.no_spb and a.kode_lokasi = f.kode_lokasi and f.modul = 'PO' "+
					 "              inner join po_m g on g.no_po=f.no_bukti and g.kode_lokasi = f.kode_lokasi and f.modul = 'PO' "+
					 ...
					 "                               where x.no_spb <> '"+this.cb_bukti.getText()+"' and x.no_del='-' and x.kode_lokasi='"+this.app._lokasi+"' group by x.kode_lokasi,y.no_bukti) h on g.no_po=h.no_bukti and g.kode_lokasi=h.kode_lokasi "+
		...
		if (data instanceof portalui_arrayMap)
		{
			...
			if (line != undefined)
			{
				...
				this.jenisLama = line.get("jenis");
				this.progressLama = line.get("bukti_prog");
				this.progressPO = line.get("progress");
				this.stsFinalLama = line.get("sts_kapital");
				...
				this.cb_terima.setText(line.get("kode_terima"));
				this.cb_terima.setRightLabelCaption(line.get("nama_vendor"));
				this.dp_tgl2.setText(line.get("due_date"));
				this.dp_tgl3.setText(line.get("tgl_invoice"));
				this.ed_ref1.setText(line.get("no_dokumen"));
				this.ed_ref2.setText(line.get("no_invoice"));
				this.ed_catat.setText(line.get("catatan"));
				this.ed_desc.setText(line.get("keterangan"));
				this.ed_fppn.setText(line.get("no_fpajak"));
				this.cb_po.setText(line.get("no_po"));
				this.ed_spb.setText(floatToNilai(parseFloat(line.get("nilai"))));
				this.ed_ppn.setText(floatToNilai(parseFloat(line.get("nilai_ppn"))));
				this.ed_npo.setText(floatToNilai(parseFloat(line.get("nilai_po"))));
		...
		if (this.stsFinalLama == "F")
		{
			this.bFinal.hide();
			this.bTutup.hide();
			this.bTutup.click();
			
			var strSql = " select a.kode_akun,b.nama as nama_akun,a.jenis "+
						 " from spb_j a inner join masakun b on a.kode_akun = b.kode_akun and a.kode_lokasi = b.kode_lokasi "+
						 " where a.no_spb='"+this.cb_bukti.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' and a.jenis in ('ADPUM','JAMIN','PPH') ";
			...
			if (data instanceof portalui_arrayMap)
			{
				if (data.get(0) != undefined)
				{
					...
					{
						line = data.get(i);
						if ((line.get("jenis") == "ADPUM") || (line.get("jenis") == "JAMIN"))
						{
							if (line.get("jenis") == "JAMIN") this.cb_akun.setCaption("Akun Jaminan");
							
							this.cb_akun.setText(line.get("kode_akun"));
							this.cb_akun.setRightLabelCaption(line.get("nama_akun"));
						}
						if (line.get("jenis") == "PPH")
						{
							this.cb_pph.setText(line.get("kode_akun"));
							this.cb_pph.setRightLabelCaption(line.get("nama_akun"));
							this.ed_pph.setText(floatToNilai(parseFloat(line.get("nilai"))));
						}
					}
				}
			}
			
			if (this.progressLama == '9')
			{
				this.bFinal.show();
				this.bTutup.show();
			}
			if (this.progressLama == 'J')
			{
				...
				var line2,data2 = this.dbLib.runSQL("select a.kode_akun, a.nama from masakun a inner join spb_j b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.jenis = 'JAMIN' and b.modul = 'SPPLOG' "+
													"where b.no_spb <> '"+this.cb_bukti.getText()+"' and b.no_dokumen = '"+this.cb_po.getText()+"' and b.kode_lokasi = '"+this.app._lokasi+"'");											 
		...
		} else
		{
			...
			var line2,data2 = this.dbLib.runSQL("select no_faapp from fa_app "+
												"where no_spb ='"+this.cb_bukti.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' and no_del='-'");
			...
			if (data2 instanceof portalui_arrayMap)
			{
				...
				if (line2 != undefined)
				{
					this.ed_nbapp2.setText(line2.get("no_faapp"));
				} else this.ed_nbapp2.hide();
			} 
...
window.app_saku_logistik_transaksi_fSpblogk.prototype.doSelect = function(sender, year, month, day)
{
	if (month < 10)
		month = "0" + month;
...
window.app_saku_logistik_transaksi_fSpblogk.prototype.doEditChange = function(sender)
{
...
window.app_saku_logistik_transaksi_fSpblogk.prototype.FindBtnClick = function(sender, event)
{
	try
	{
		if (sender == this.cb_bukti) 
		{
		    if (this.ed_period != "") 
			{
				...
				this.standarLib.showListData(this, "Daftar Bukti SPP Logistik",this.cb_bukti,undefined, 
				  								 "select no_spb, keterangan from spb_m where progress in ('0','X') and modul = 'SPPLOG' and periode<='"+this.ed_period.getText()+"' and no_del='-' and kode_lokasi='"+this.app._lokasi+"' " +sts,
												 "select count(no_spb)      from spb_m where progress in ('0','X') and modul = 'SPPLOG' and periode<='"+this.ed_period.getText()+"' and no_del='-' and kode_lokasi='"+this.app._lokasi+"' " +sts,
												 new Array("no_spb","keterangan"),"and", new Array("No SPP","Keterangan"),false);
			}
...
window.app_saku_logistik_transaksi_fSpblogk.prototype.doRequestReady = function(sender, methodName, result)
{
