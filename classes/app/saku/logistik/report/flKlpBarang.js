window.app_saku_logistik_report_flKlpBarang = function(owner)
{
	if (owner)
	{
		window.app_saku_logistik_report_flKlpBarang.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_logistik_report_flKlpBarang";
		this.maximize();
...
window.app_saku_logistik_report_flKlpBarang.extend(window.portalui_childForm);
window.app_saku_logistik_report_flKlpBarang.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==0)
	{
...
window.app_saku_logistik_report_flKlpBarang.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
...
window.app_saku_logistik_report_flKlpBarang.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.nama_report="server_report_logistik_rptKlpBarang";
	...
	{
		alert("[app_saku_logistik_report_flKlpBarang]::mainButtonClick:"+e);
	}
};
window.app_saku_logistik_report_flKlpBarang.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_logistik_report_flKlpBarang.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.showFilter,this.lokasi);
    this.page=page;
	this.allBtn = false;
};
window.app_saku_logistik_report_flKlpBarang.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_logistik_report_flKlpBarang.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_logistik_report_flKlpBarang.prototype.doRowPerPageChange = function(sender, rowperpage)
{
