window.app_saku_logistik_report_flPo = function(owner)
{
	if (owner)
	{
		window.app_saku_logistik_report_flPo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_logistik_report_flPo";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Purchase Order", 2);
...
window.app_saku_logistik_report_flPo.extend(window.portalui_childForm);
window.app_saku_logistik_report_flPo.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==0)
	{
	...
	if (row == 2)
	{
		var sql1="select no_po,keterangan from po_m "+
				 ...
				 " order by no_po ";
		var sql2="select count(no_po) from po_m "+
				 this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,0),"where") +
				 this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and");
		
		this.filterRep.ListDataSGFilter(this, "Data PO",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_po","keterangan"),"and");
	}
};
window.app_saku_logistik_report_flPo.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
...
window.app_saku_logistik_report_flPo.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.nama_report="server_report_logistik_rptPo";
...
window.app_saku_logistik_report_flPo.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_logistik_report_flPo.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter,page,this.pager,this.lokasi,this.showFilter);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_logistik_report_flPo.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_logistik_report_flPo.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_logistik_report_flPo.prototype.doRowPerPageChange = function(sender, rowperpage)
{
