window.app_saku_logistik_report_flSatuan = function(owner)
{
	if (owner)
	{
		window.app_saku_logistik_report_flSatuan.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_logistik_report_flSatuan";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Satuan", 2);
...
window.app_saku_logistik_report_flSatuan.extend(window.portalui_childForm);
window.app_saku_logistik_report_flSatuan.prototype.doEllipseClick= function(sender, col, row)
{
	this.standar.ListDataSGFilter2(this, "Data Satuan",this.sg1, this.sg1.row, this.sg1.col,
										  "select kode_sat, nama from satuan ",
										  "select count(*) from satuan ",
										  new Array("kode_sat","nama"),"where",new Array("Kode Satuan","Nama Satuan"));
};
window.app_saku_logistik_report_flSatuan.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_saku_logistik_report_flSatuan.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.nama_report="server_report_logistik_rptSatuan";
			this.showFilter = this.filterRep.showFilter(this.sg1);
    		this.filter = this.filterRep.filterStr("kode_sat",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where");
	...
	{
		alert("[app_saku_logistik_report_flSatuan]::mainButtonClick:"+e);
	}
};
window.app_saku_logistik_report_flSatuan.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_logistik_report_flSatuan.prototype.doSelectedPage = function(sender, page)
{	
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.showFilter,this.lokasi);
    this.page=page;
	this.allBtn = false;
};
window.app_saku_logistik_report_flSatuan.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_logistik_report_flSatuan.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_logistik_report_flSatuan.prototype.doRowPerPageChange = function(sender, rowperpage)
{
