window.app_saku_logistik_report_flTr = function(owner)
{
	if (owner)
	{
		window.app_saku_logistik_report_flTr.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_logistik_report_flTr";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Temporary Receive",2);
...
window.app_saku_logistik_report_flTr.extend(window.portalui_childForm);
window.app_saku_logistik_report_flTr.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==0)
	{
	...
	if (row == 2)
	{
		var sql1="select no_tr,keterangan from tr_m "+
				...
				" order by no_po ";
		var sql2="select count(no_tr) from tr_m "+
				 this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,0),"where") +
				 this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and");
		
		this.filterRep.ListDataSGFilter(this, "Data TR",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_tr","keterangan"),"and");
	}
};
window.app_saku_logistik_report_flTr.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
...
window.app_saku_logistik_report_flTr.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.filter=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
					...
					this.filterRep.filterStr("a.no_tr",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
		this.nama_report="server_report_logistik_rptTr";
...
window.app_saku_logistik_report_flTr.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_logistik_report_flTr.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager,this.lokasi,this.showFilter);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_logistik_report_flTr.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_logistik_report_flTr.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_logistik_report_flTr.prototype.doRowPerPageChange = function(sender, rowperpage)
{
