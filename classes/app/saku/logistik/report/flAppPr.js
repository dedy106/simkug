window.app_saku_logistik_report_flAppPr = function(owner)
{
	if (owner)
	{
		window.app_saku_logistik_report_flAppPr.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_logistik_report_flAppPr";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Approve Purchase Request", 2);
...
window.app_saku_logistik_report_flAppPr.extend(window.portalui_childForm);
window.app_saku_logistik_report_flAppPr.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==0)
	{
	...
	if (row == 2)
	{
		var sql1="select no_ver,keterangan from verpr_m "+
				 ...
				 " order by no_ver ";
		var sql2="select count(no_ver) from verpr_m "+
				 this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
				 this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and");
...
window.app_saku_logistik_report_flAppPr.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
...
window.app_saku_logistik_report_flAppPr.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.nama_report="server_report_logistik_rptAppPr";
...
window.app_saku_logistik_report_flAppPr.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_logistik_report_flAppPr.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager,this.lokasi,this.showFilter);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_logistik_report_flAppPr.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_logistik_report_flAppPr.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_logistik_report_flAppPr.prototype.doRowPerPageChange = function(sender, rowperpage)
{
