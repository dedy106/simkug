window.app_saku_logistik_report_flFaApp = function(owner)
{
	if (owner)
	{
		window.app_saku_logistik_report_flFaApp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_logistik_report_flFaApp";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Kapitalisasi Asset",2);
...
window.app_saku_logistik_report_flFaApp.extend(window.portalui_childForm);
window.app_saku_logistik_report_flFaApp.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==0)
	{
	...
	if (row == 2)
	{
		var sql1="select no_faapp,keterangan from fa_app "+
				 ...
				 " order by no_faapp ";
		var sql2="select count(no_faapp) from fa_app "+
				 this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,0),"where") +
				 this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and");
		
		this.filterRep.ListDataSGFilter(this, "Data Finalisasi",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_faapp","keterangan"),"and");
	}
};
window.app_saku_logistik_report_flFaApp.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
...
window.app_saku_logistik_report_flFaApp.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.filter=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
					...
					this.filterRep.filterStr("a.no_faapp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
		this.nama_report="server_report_logistik_rptFaApp";
...
window.app_saku_logistik_report_flFaApp.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_logistik_report_flFaApp.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter,page,this.pager,this.lokasi,this.showFilter);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_logistik_report_flFaApp.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_logistik_report_flFaApp.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_logistik_report_flFaApp.prototype.doRowPerPageChange = function(sender, rowperpage)
{
