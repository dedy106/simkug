window.app_saku_logistik_report_flPr = function(owner)
{
	if (owner)
	{
		window.app_saku_logistik_report_flPr.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_logistik_report_flPr";
		this.maximize();
...
window.app_saku_logistik_report_flPr.extend(window.portalui_childForm);
window.app_saku_logistik_report_flPr.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==0)
	{
	...
	if (row == 2)
	{
		var sql1="select no_pr,keterangan from pr_m "+
		...
		var sql2="select count(no_pr) from pr_m "+
				 this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
				 this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and");
		this.filterRep.ListDataSGFilter(this, "Data PR",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_pr","keterangan"),"and");
	}
};
window.app_saku_logistik_report_flPr.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
...
window.app_saku_logistik_report_flPr.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.filter=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
					...
					this.filterRep.filterStr("a.no_pr",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
		this.nama_report="server_report_logistik_rptPr";
...
window.app_saku_logistik_report_flPr.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_logistik_report_flPr.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.lokasi, this.showFilter,this.filter2);	
	this.page=page;
	this.allBtn = false;
};
window.app_saku_logistik_report_flPr.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_logistik_report_flPr.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_logistik_report_flPr.prototype.doRowPerPageChange = function(sender, rowperpage)
{
