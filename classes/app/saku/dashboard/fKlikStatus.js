window.app_saku_dashboard_fKlikStatus = function(owner)
{
	if (owner)
	{
		try{
			window.app_saku_dashboard_fKlikStatus.prototype.parent.constructor.call(this,owner);
			//window.app_saku_dashboard_fKlikStatus.prototype.parent.setBorder.call(this,0);
			//window.app_saku_dashboard_fKlikStatus.prototype.parent.setColor.call(this,"");
			this.className  = "app_saku_dashboard_fKlikStatus";
			...
			if (this.getForm().filter === "status"){
				...
				if (this.getForm().umur[0] === "20-35"){
					...
					"where b.kode_lokasi='"+this.getForm().lokasi+
					"' and cast(substring(date_sub(now(),interval substring(a.tgl_lahir,1,7) year_month),3,2) as decimal)>19 "+
					...
					"from karyawan a left outer join hr_dinas2 b on a.nik=b.nik and a.kode_lokkonsol=b.kode_lokkonsol "+
					"where b.kode_lokasi='"+this.getForm().lokasi+
					"' and cast(substring(date_sub(now(),interval substring(a.tgl_lahir,1,7) year_month),3,2) as decimal)>49 ";
				}
				...
				if (this.getForm().msKrj[0] === "<2"){
					...
					"from karyawan a left outer join hr_dinas2 b on a.nik=b.nik and a.kode_lokkonsol=b.kode_lokkonsol "+
					"where b.kode_lokasi='"+this.getForm().lokasi+
					"' and cast(substring(date_sub(now(),interval substring(a.tgl_masuk,1,7) year_month),3,2) as decimal)<2 ";
					...
					"from karyawan a left outer join hr_dinas2 b on a.nik=b.nik and a.kode_lokkonsol=b.kode_lokkonsol "+
					"where b.kode_lokasi='"+this.getForm().lokasi+
					"' and cast(substring(date_sub(now(),interval substring(a.tgl_masuk,1,7) year_month),3,2) as decimal)>1 "+
					...
					"from karyawan a left outer join hr_dinas2 b on a.nik=b.nik and a.kode_lokkonsol=b.kode_lokkonsol "+
					"where b.kode_lokasi='"+this.getForm().lokasi+
					"' and cast(substring(date_sub(now(),interval substring(a.tgl_masuk,1,7) year_month),3,2) as decimal)>5 "+
		...
		{
			alert("[app_saku_dashboard_fKlikStatus]->constructor : "+e);
		}
	}
};
window.app_saku_dashboard_fKlikStatus.extend(window.portalui_panel);
window.app_saku_dashboard_fKlikStatus.prototype.doImgClose = function(sender)
{
	this.getForm().doImgClose();
};
window.app_saku_dashboard_fKlikStatus.prototype.sg1onDblClick = function(sender, col, row)
{
	try
	{		
