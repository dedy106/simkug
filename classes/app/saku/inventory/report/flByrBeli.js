window.app_saku_inventory_report_flByrBeli = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_report_flByrBeli.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_inventory_report_flByrBeli";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Pembayaran Pembelian", 2);
...
window.app_saku_inventory_report_flByrBeli.extend(window.portalui_childForm);
window.app_saku_inventory_report_flByrBeli.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row === 5)
		{
			this.standar.ListDataSGFilter(this, "Data No. Bukti Pembayaran Pembelian",this.sg1, this.sg1.row, this.sg1.col,
	...
	mainButtonClick: function(sender){
		try
		{
			...
			}else
			{
				...
				var sql = "select a.no_kas,a.no_dokumen,a.keterangan,a.kode_bank,b.nama as nama_bank,a.nik_buat,c.nama as nama_buat,a.ref1,d.nama as nama_vendor,a.akun_kb,a.posted,a.periode,a.nilai ";
				if (this.jenisLap === "Transaksi")
					sql+=",f.no_beli,f.akun_hutang,date_format(f.tanggal,'%d/%m/%Y') as tanggal,f.keterangan as ket,f.nilai+nilai_ppn as nilai_d,ifnull(g.totbayar,0)+ifnull(h.totgb,0) as totbayar,(f.nilai+f.nilai_ppn - (ifnull(g.totbayar,0)+ifnull(h.totgb,0))) as sisahut, i.nbayar ";
				sql+="from kas_m a "+
					"inner join bank2 b on a.kode_bank=b.kode_bank and a.kode_lokasi=b.kode_lokasi "+
					...
					"  from kas_m x "+
					"  inner join kas_d y on x.no_kas=y.no_kas and x.kode_lokasi=y.kode_lokasi "+
					"  where x.kode_lokasi='"+this.sg1.getCell(2,0)+"' and y.modul='INVBELI' and x.no_del='-' and x.no_kas not like '%"+this.sg1.getCell(2,5)+"' "+
					"  group by y.no_bukti,x.kode_lokasi) "+
					"  g on f.no_beli=g.no_bukti and f.kode_lokasi=g.kode_lokasi "+
					"left outer join (select y.no_bukti,x.kode_lokasi,sum(y.nilai) as totgb "+
					"  from gb_m x "+
					"  inner join gb_d y on x.no_gb=y.no_gb and x.kode_lokasi=y.kode_lokasi "+
					"  where x.kode_lokasi='"+this.sg1.getCell(2,0)+"' and y.modul='GBBELI' and x.no_del='-' "+
					"  group by y.no_bukti,x.kode_lokasi) "+
					"  h on f.no_beli=h.no_bukti and f.kode_lokasi=h.kode_lokasi "+
					"left outer join (select y.no_bukti,x.kode_lokasi,sum(y.nilai) as nbayar "+
					"  from kas_m x "+
					"  inner join kas_d y on x.no_kas=y.no_kas and x.kode_lokasi=y.kode_lokasi "+
					"  where x.kode_lokasi='"+this.sg1.getCell(2,0)+"' and y.modul='INVBELI' and x.no_del='-' and x.no_kas like '%"+this.sg1.getCell(2,5)+"' "+
					"  group by y.no_bukti,x.kode_lokasi) "+
					"  i on f.no_beli=i.no_bukti and f.kode_lokasi=i.kode_lokasi "+this.filter+
				...
				if (this.jenisLap === "Daftar"){
					...
					this.title = new server_util_arrayList({items:["No. Faktur","Ak. Piutang","Tanggal","Keterangan","Netto","Total Bayar","Sisa Hutang","Nilai Bayar"]});
			...
			systemAPI.alert("[app_saku_inventory_report_flByrBeli]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		if (this.jenisLap === "Transaksi")
			var header = "LAPORAN PEMBAYARAN PEMBELIAN";
	...
	convertResultToHTML: function(result,data){		
		...
		for (var r in table.rs.rows){
			...
			if (first){
				...
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.ref1+"-"+line.nama_vendor+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
			...
			for (var c in line){
				if (c === "no_beli" || c === "akun_piutang" || c === "tanggal" || c === "ket" || c === "nilai_d" || c === "totbayar" || c === "sisahut" || c === "nbayar"){
					if (c === "nilai_d" || c === "totbayar" || c === "sisahut" || c === "nbayar")
