window.app_saku_inventory_report_flPK = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_report_flPK.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_inventory_report_flPK";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Packing List", 2);
	...
	this.getPeriode="select distinct periode from inv_pl_m where kode_lokasi ='"+this.lokasi+"'";
...
window.app_saku_inventory_report_flPK.extend(window.portalui_childForm);
window.app_saku_inventory_report_flPK.implement({
	...
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			...
			this.dbLib.setItemsFromSQL("select distinct periode from inv_pl_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",[this.sg1.columns.get(2).pickList]);
		}
	},
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter2= this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.no_pl",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.nama_report="server_report_inventory_rptPL";
		...
		try{
			if (sender === this.confirm.bConfirm){
				...
				if (to !== ""){
					this.confirm.free();
					var d = new Date();
					var subject = "Daftar Packing List "+d.toLocaleString();
