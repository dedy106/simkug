window.app_saku_inventory_report_flInvoice = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_report_flInvoice.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_inventory_report_flInvoice";
		this.maximize();
	...
	this.getPeriode="select distinct periode from inv_invoice_m where kode_lokasi ='"+this.lokasi+"'";
...
window.app_saku_inventory_report_flInvoice.extend(window.portalui_childForm);
window.app_saku_inventory_report_flInvoice.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row === 3)
		{
			this.filterRep.ListDataSGFilter(this, "Data Faktur",this.sg1, this.sg1.row, this.sg1.col,
										  "select no_faktur from inv_invoice_m where kode_lokasi='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"' and kode_cust like '%"+this.sg1.getCell(2,2)+"' ",
										  "select count(*) from inv_invoice_m where kode_lokasi ='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"' and kode_cust like '%"+this.sg1.getCell(2,2)+"' ",
										  ["no_faktur"],"and",["No. Faktur"]);
		}
		if (row === 4)
		{
			this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
										  "select no_inv,date_format(tgl_jt,'%d/%m/%Y'),term,nilai from inv_invoice_m where kode_lokasi='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"' and kode_cust like '%"+this.sg1.getCell(2,2)+"' and no_faktur like '%"+this.sg1.getCell(2,3)+"'",
										  "select count(*) from inv_invoice_m where kode_lokasi ='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"' and kode_cust like '%"+this.sg1.getCell(2,2)+"' and no_faktur like '%"+this.sg1.getCell(2,3)+"'",
										  ["no_inv","tgl_jt","term","nilai"],"and",["Kode","Jatuh Tempo","Term","Total Nilai"]);
		}
	},
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			...
			this.dbLib.setItemsFromSQL("select distinct periode from inv_invoice_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",[this.sg1.columns.get(2).pickList]);
		}
	},
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter2= this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.no_inv",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.nama_report="server_report_inventory_rptInv";
		...
		try{
			if (sender === this.confirm.bConfirm){
				...
				if (to !== ""){
					this.confirm.free();
					var d = new Date();
					var subject = "Laporan Invoice "+d.toLocaleString();
