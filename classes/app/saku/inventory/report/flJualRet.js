window.app_saku_inventory_report_flJualRet = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_report_flJualRet.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_inventory_report_flJualRet";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Retur Penjualan", 2);
	...
	this.getPeriode="select distinct periode from inv_jualretur_m where kode_lokasi ='"+this.lokasi+"'";
...
window.app_saku_inventory_report_flJualRet.extend(window.portalui_childForm);
window.app_saku_inventory_report_flJualRet.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 4)
		{
			this.standar.ListDataSGFilter2(this, "Data Retur Penjualan",this.sg1, this.sg1.row, this.sg1.col,
										  "select no_retur, keterangan from inv_jualretur_m "+
										  ...
										  "select count(*) from inv_jualretur_m "+
										  this.filterRep.filterStr("kode_gudang",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"where")+
										  this.filterRep.filterStr("kode_cust",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
										  this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
										  " and kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",
	...
	doSelectCell: function(sender, col, row){
		...
		if (row === 1){
			this.dbLib.setItemsFromSQL("select distinct periode from inv_jualretur_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 5)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku_inventory_report_flJualRet]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		if (this.jenisLap === "Transaksi")
			var header = "LAPORAN RETUR PENJUALAN";
