window.app_saku_inventory_report_flSaldoPC = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_report_flSaldoPC.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_inventory_report_flSaldoPC";
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Saldo Piutang Per Customer", 2);
...
window.app_saku_inventory_report_flSaldoPC.extend(window.portalui_childForm);
window.app_saku_inventory_report_flSaldoPC.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			}else
			{
				...
				var sql = "select a.kode_cust,a.nama,ifnull(b.nilai,0)-ifnull(d.nilai_byr,0)-ifnull(e.nilai_byr,0) as so_awal, "+
						...
						"  from inv_jual_m "+
						"	group by kode_cust) z on a.kode_cust=z.kode_cust and a.kode_lokasi=z.kode_lokasi "+
						"left join (select kode_cust,kode_lokasi,sum(case when substring(no_jual,-1)<>'r' then nilai+nilai_ppn else -(nilai+nilai_ppn) end) as nilai "+
						"	from inv_jual_m "+
						"	where periode<'"+periode+"' "+
						"	group by kode_cust) b on a.kode_cust=b.kode_cust and a.kode_lokasi=b.kode_lokasi "+
						"left join (select a.kode_cust,a.kode_lokasi,sum(case when substring(a.no_gb,-1)<>'r' then b.nilai else -b.nilai end) as nilai_byr "+
						"  from gb_m a inner join gb_d b on a.no_gb=b.no_gb and a.kode_lokasi=b.kode_lokasi "+
						"  where a.periode<'"+periode+"' and a.jenis='JUAL' "+
						"  group by a.kode_cust) d on a.kode_cust=d.kode_cust and a.kode_lokasi=d.kode_lokasi "+
						"left join (select c.kode_cust,a.kode_lokasi,sum(case when substring(c.no_jual,-1)<>'r' then b.nilai else -b.nilai end) as nilai_byr "+
						...
						"  group by  c.kode_cust) e on a.kode_cust=e.kode_cust and a.kode_lokasi=e.kode_lokasi "+
						"left join (select kode_cust,kode_lokasi,sum(case when substring(no_jual,-1)<>'r' then nilai+nilai_ppn else -(nilai+nilai_ppn) end) as nilai "+
						"	from inv_jual_m "+
						"	where periode='"+periode+"' "+
						...
						"  from gb_m a inner join gb_d b on a.no_gb=b.no_gb and a.kode_lokasi=b.kode_lokasi "+
						"  where a.periode='"+periode+"' and a.jenis='JUAL' "+
						"  group by a.kode_cust) h on a.kode_cust=h.kode_cust and a.kode_lokasi=h.kode_lokasi "+
						"left join (select c.kode_cust,a.kode_lokasi,sum(case when substring(c.no_jual,-1)<>'r' then b.nilai else -b.nilai end) as nilai_byr "+
						"  from kas_m a inner join kas_d b on a.no_kas=b.no_kas and a.kode_lokasi=b.kode_lokasi "+
						"  inner join inv_jual_m c on b.no_bukti=c.no_jual and b.kode_lokasi=c.kode_lokasi "+
						"  where a.periode='"+periode+"' "+
						"  group by  c.kode_cust) i on a.kode_cust=i.kode_cust and a.kode_lokasi=i.kode_lokasi "+this.filter+
						" order by a.kode_cust ";
				this.scriptSqlCount = "select count(a.kode_cust) "+
						"from cust a "+
						"inner join (select kode_cust,kode_lokasi "+
						"  from inv_jual_m "+
						"	group by kode_cust) z on a.kode_cust=z.kode_cust and a.kode_lokasi=z.kode_lokasi "+this.filter;
			...
			systemAPI.alert("[app_saku_inventory_report_flSaldoPC]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var header = "DAFTAR SALDO PIUTANG PER CUSTOMER";
