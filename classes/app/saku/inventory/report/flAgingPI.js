window.app_saku_inventory_report_flAgingPI = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_report_flAgingPI.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_inventory_report_flAgingPI";
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Aging Piutang Per Invoice", 2);
...
window.app_saku_inventory_report_flAgingPI.extend(window.portalui_childForm);
window.app_saku_inventory_report_flAgingPI.implement({
		...
		try{			
			...
			if (row === 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data No. Invoice",this.sg1, this.sg1.row, this.sg1.col,
											  "select no_jual,keterangan from inv_jual_m where kode_lokasi ='"+this.sg1.getCell(2,0)+"' and kode_cust like '%"+this.sg1.getCell(2,1)+"'",
											  "select count(*) from inv_jual_m where kode_lokasi ='"+this.sg1.getCell(2,0)+"' and kode_cust like '%"+this.sg1.getCell(2,1)+"'",
	...
	mainButtonClick: function(sender){
		try
		{
			...
			}else
			{
				...
				var sql = "select a.no_jual,a.keterangan,c.nmcust,sum(b.aging1) as aging1,sum(b.aging2) as aging2,sum(b.aging3) as aging3,sum(b.aging4) as aging4 "+
						"	from inv_jual_m a "+
						...
						"				group by a.no_jual) c on a.no_jual=c.no_jual and a.kode_lokasi=c.kode_lokasi "+
						"	left join (select a.kode_lokasi,a.no_jual, "+
						...
						"		   )b on a.no_jual=b.no_jual and a.kode_lokasi=b.kode_lokasi "+this.filter+
						"	group by a.no_jual order by a.no_jual ";
				this.scriptSqlCount = "select count(a.no_jual) "+
						"	from inv_jual_m a "+
						"	inner join (select no_jual,kode_lokasi "+
						"				from inv_jual_m"+
						"				group by no_jual) c on a.no_jual=c.no_jual and a.kode_lokasi=c.kode_lokasi "+this.filter;
				
				this.title = new server_util_arrayList({items:["No. Invoice","Keterangan","Customer","0 - 30 Hari","31 - 60 Hari","61 - 90 Hari","> 90 Hari"]});
			...
			systemAPI.alert("[app_saku_inventory_report_flAgingPI]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var header = "AGING PIUTANG PER INVOICE";
