window.app_saku_inventory_report_flSO = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_report_flSO.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_inventory_report_flSO";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Sales Order", 2);
	...
	this.getPeriode="select distinct periode from inv_so_m where kode_lokasi ='"+this.lokasi+"'";
	...
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["No. Order","All",""]);
	this.pager = 1;
...
window.app_saku_inventory_report_flSO.extend(window.portalui_childForm);
window.app_saku_inventory_report_flSO.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row === 3)
		{
			this.filterRep.ListDataSGFilter(this, "Data Order",this.sg1, this.sg1.row, this.sg1.col,
										  "select no_so,keterangan from inv_so_m where kode_lokasi='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"' and kode_cust like '%"+this.sg1.getCell(2,2)+"' ",
										  "select count(*) from inv_so_m where kode_lokasi ='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"' and kode_cust like '%"+this.sg1.getCell(2,2)+"' ",
										  ["no_so","keterangan"],"and",["Kode","Keterangan"]);
		}
	},
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			...
			this.dbLib.setItemsFromSQL("select distinct periode from inv_so_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",[this.sg1.columns.get(2).pickList]);
		}
	},
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						...
						this.filterRep.filterStr("a.no_so",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2= this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.no_so",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.nama_report="server_report_inventory_rptSO";
		...
		try{
			if (sender === this.confirm.bConfirm){
				...
				if (to !== ""){
					this.confirm.free();
					var d = new Date();
					var subject = "Laporan Saldo Order "+d.toLocaleString();
