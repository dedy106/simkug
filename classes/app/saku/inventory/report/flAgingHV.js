window.app_saku_inventory_report_flAgingHV = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_report_flAgingHV.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_inventory_report_flAgingHV";
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Aging Hutang Per Vendor", 2);
...
window.app_saku_inventory_report_flAgingHV.extend(window.portalui_childForm);
window.app_saku_inventory_report_flAgingHV.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			}else
			{
				...
				var sql = "select a.kode_vendor,a.nama,sum(b.aging1) as aging1,sum(b.aging2) as aging2,sum(b.aging3) as aging3,sum(b.aging4) as aging4 "+
						"	from vendor a "+
						"	inner join (select kode_vendor,kode_lokasi,tanggal "+
						"				from inv_beli_m "+
						"				group by kode_vendor) c on a.kode_vendor=c.kode_vendor and a.kode_lokasi=c.kode_lokasi "+
						"	left join (select a.kode_lokasi,a.kode_vendor, "+
						...
						"	group by a.kode_vendor order by a.kode_vendor ";
				this.scriptSqlCount = "select count(a.kode_vendor) "+
						"	from vendor a "+
						"	inner join (select kode_vendor,kode_lokasi,tanggal "+
						"				from inv_beli_m"+
						"				group by kode_vendor) c on a.kode_vendor=c.kode_vendor and a.kode_lokasi=c.kode_lokasi "+this.filter;
				
				this.title = new server_util_arrayList({items:["Kode Vendor","Nama Vendor","0 - 30 Hari","31 - 60 Hari","61 - 90 Hari","> 90 Hari"]});
			...
			systemAPI.alert("[app_saku_inventory_report_flAgingHV]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var header = "AGING HUTANG PER VENDOR";
