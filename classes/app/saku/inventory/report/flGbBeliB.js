window.app_saku_inventory_report_flGbBeliB = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_report_flGbBeliB.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_inventory_report_flGbBeliB";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Pembayaran GB Pembelian", 2);
	...
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Akun Hutang GB","All",""]);
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Kode Bank","All",""]);
	this.gridLib.SGEditData(this.sg1,4,[0,1,2],["Kode Vendor","All",""]);
...
window.app_saku_inventory_report_flGbBeliB.extend(window.portalui_childForm);
window.app_saku_inventory_report_flGbBeliB.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row === 2)
		{
			this.filterRep.ListDataSGFilter(this, "Data Akun Hutang GB",this.sg1, this.sg1.row, this.sg1.col,
									  "select a.kode_akun, a.nama from masakun a inner join flag_relasi b on a.kode_akun = b.kode_akun and a.kode_lokasi=b.kode_lokasi where a.block= '0' and a.kode_lokasi = '"+this.sg1.getCell(2,0)+"' and b.kode_flag='025' ",
									  "select count(a.kode_akun)  from masakun a inner join flag_relasi b on a.kode_akun = b.kode_akun and a.kode_lokasi=b.kode_lokasi where a.block= '0' and a.kode_lokasi = '"+this.sg1.getCell(2,0)+"' and b.kode_flag='025' ",
		...
		if (row === 5)
		{
			this.standar.ListDataSGFilter(this, "Data No. Bukti Pembayaran GB Pembelian",this.sg1, this.sg1.row, this.sg1.col,
									  ...
									  this.filterRep.filterStr("kode_vendor",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
		...
		try
		{
			...
			}else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_vendor",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
				...
				if (this.jenisLap === "Transaksi")
					sql+=",f.no_beli,f.akun_hutang,date_format(f.tanggal,'%d/%m/%Y') as tanggal,f.keterangan as ket,f.nilai+f.nilai_ppn as nilai_d,ifnull(g.totgb,0)+ifnull(h.totbayar,0) as totbayar,(f.nilai+f.nilai_ppn - (ifnull(g.totgb,0)+ifnull(h.totbayar,0))) as sisahut, i.ngb ";
				sql+="from gb_m a inner join bank2 b on a.kode_bank=b.kode_bank and a.kode_lokasi=b.kode_lokasi "+
					...
					"left outer join (select y.no_bukti,x.kode_lokasi,sum(y.nilai) as totgb "+
					"  from gb_m x inner join gb_d y on x.no_gb=y.no_gb and x.kode_lokasi=y.kode_lokasi "+
					"  where x.kode_lokasi='"+this.sg1.getCell(2,0)+"' and y.modul='GBBELI' and x.no_del='-' and x.no_gb not like '%"+this.sg1.getCell(2,5)+"' "+
					"  group by y.no_bukti,x.kode_lokasi) g on f.no_beli=g.no_bukti and f.kode_lokasi=g.kode_lokasi "+
					"left outer join (select y.no_bukti,x.kode_lokasi,sum(y.nilai) as totbayar "+
					"  from kas_m x inner join kas_d y on x.no_kas=y.no_kas and x.kode_lokasi=y.kode_lokasi "+
					"  where x.kode_lokasi='"+this.sg1.getCell(2,0)+"' and y.modul='INVBELI' and x.no_del='-' "+
					"  group by y.no_bukti,x.kode_lokasi) h on f.no_beli=h.no_bukti and f.kode_lokasi=h.kode_lokasi "+
					"left outer join (select y.no_bukti,x.kode_lokasi,sum(y.nilai) as ngb "+
					"  from gb_m x inner join gb_d y on x.no_gb=y.no_gb and x.kode_lokasi=y.kode_lokasi "+
					"  where x.kode_lokasi='"+this.sg1.getCell(2,0)+"' and y.modul='GBBELI' and x.no_del='-' and x.no_gb like '%"+this.sg1.getCell(2,5)+"' "+
					"  group by y.no_bukti,x.kode_lokasi) i on f.no_beli=i.no_bukti and f.kode_lokasi=i.kode_lokasi "+this.filter+
				...
				if (this.jenisLap === "Daftar"){
					...
					this.title = new server_util_arrayList({items:["No. Faktur","Ak. Hutang","Tanggal","Keterangan","Netto","Total Bayar","Sisa Hutang","Nilai Bayar"]});
			...
			systemAPI.alert("[app_saku_inventory_report_flGbBeliB]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		if (this.jenisLap === "Transaksi")
			var header = "LAPORAN PEMBAYARAN GB PEMBELIAN";
	...
	convertResultToHTML: function(result,data){		
		...
		for (var r in table.rs.rows){
			...
			for (var c in line){
				if (c === "no_beli" || c === "akun_hutang" || c === "tanggal" || c === "ket" || c === "nilai_d" || c === "totbayar" || c === "sisahut" || c === "ngb"){
