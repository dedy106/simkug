window.app_saku_inventory_report_flMutkrm = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_report_flMutkrm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_inventory_report_flMutkrm";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Mutasi Kirim", 2);
...
window.app_saku_inventory_report_flMutkrm.extend(window.portalui_childForm);
window.app_saku_inventory_report_flMutkrm.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row === 4)
		{
			this.standar.ListDataSGFilter(this, "Data No. Bukti Mutasi Kirim",this.sg1, this.sg1.row, this.sg1.col,
									  "select no_kirim, keterangan from inv_mutkrm_m "+
									  ...
									  "select count(*) from inv_mutkrm_m "+
									  this.filterRep.filterStr("kode_gudangt",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"where")+
									  this.filterRep.filterStr("kode_gudanga",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
									  this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
									  " and kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",
									  ["no_kirim","keterangan"],"and",["No. Bukti","Keterangan"]);
		}
	},
		...
		try
		{
			...
			}else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_gudanga",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.kode_gudangt",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.no_kirim",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new portalui_arrayMap();
				this.jenisLap = "Transaksi";/*this.sg1.getCell(2,5);*/
				var sql = "select a.no_kirim,a.no_dokumen,a.keterangan,concat(a.nik_buat,'-',i.nama) as nik,concat(a.kode_gudanga,'-',b.nama) as gudasl,concat(a.kode_gudangt,'-',c.nama) as gudtuj,concat(a.kode_vendor,'-',d.nama) as vendor ";
				...
				sql+="from inv_mutkrm_m a "+
					...
					"  from inv_dt where kode_gudang =(select kode_gudanga from inv_mutkrm_m where no_kirim like '%"+this.sg1.getCell(2,4)+"') and kode_lokasi='"+this.sg1.getCell(2,0)+"' and periode<='"+this.sg1.getCell(2,1)+"' "+
				...
				if (this.jenisLap === "Daftar"){
					this.title = new server_util_arrayList({items:["No Bukti","No Dokumen","Keterangan","NIK ","Gudang Asal","Gudang Tujuan","Vendor"]});			
					this.widthTable = new server_util_arrayList({items:[100,120,150,100,120,120]});
			...
			systemAPI.alert("[app_saku_inventory_report_flMutkrm]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		if (this.jenisLap === "Transaksi")
			var header = "LAPORAN MUTASI KIRIM";
		else var header = "DAFTAR MUTASI KIRIM";
