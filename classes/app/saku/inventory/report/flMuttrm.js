window.app_saku_inventory_report_flMuttrm = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_report_flMuttrm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_inventory_report_flMuttrm";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Mutasi Terima", 2);
	...
	this.getPeriode="select distinct periode from inv_mutkrm_m where kode_lokasi ='"+this.lokasi+"'";
	this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Kode Lokasi",this.tanda,this.lokasi]);
	this.gridLib.SGEditData(this.sg1,1,[0,1,2],["Periode","=",this.dbLib.getPeriodeFromSQL(this.getPeriode)]);
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Kode Gudang Asal","All",""]);
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Kode Gudang Tujuan","All",""]);
...
window.app_saku_inventory_report_flMuttrm.extend(window.portalui_childForm);
window.app_saku_inventory_report_flMuttrm.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row === 2)
		{
			this.filterRep.ListDataSGFilter(this, "Daftar Gudang Asal",this.sg1, this.sg1.row, this.sg1.col,
		...
		if (row === 3)
		{
			this.filterRep.ListDataSGFilter(this, "Daftar Gudang Tujuan",this.sg1, this.sg1.row, this.sg1.col,
									  "select kode_gudang, nama  from inv_gudang where kode_lokasi='"+this.sg1.getCell(2,0)+"' and kode_gudang <> '"+this.sg1.getCell(2,2)+"' ",
									  "select count(kode_gudang) from inv_gudang where kode_lokasi='"+this.sg1.getCell(2,0)+"' and kode_gudang <> '"+this.sg1.getCell(2,2)+"' ",
									  ["kode_gudang","nama"],"and",["Kode","Nama"],false);
		}
		if (row === 4)
		{
			this.standar.ListDataSGFilter(this, "Data No. Bukti Mutasi Kirim",this.sg1, this.sg1.row, this.sg1.col,
									  "select no_terima, keterangan from inv_muttrm_m "+
									  this.filterRep.filterStr("kode_gudangt",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"where")+
									  this.filterRep.filterStr("kode_gudanga",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
									  ...
									  "select count(*) from inv_muttrm_m "+
									  this.filterRep.filterStr("kode_gudangt",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"where")+
									  this.filterRep.filterStr("kode_gudanga",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
									  this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
									  " and kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",
									  ["no_terima","keterangan"],"and",["No. Bukti","Keterangan"]);
		}
	},
	doSelectCell: function(sender, col, row){
		...
		if (row === 1){
			this.dbLib.setItemsFromSQL("select distinct periode from inv_mutkrm_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 5)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			...
			}else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("z.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("z.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("z.kode_gudanga",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("z.kode_gudangt",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("z.no_terima",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new portalui_arrayMap();
				this.jenisLap = "Transaksi";/*this.sg1.getCell(2,5);*/
				var sql = "select z.no_terima,z.no_dokumen,z.keterangan,concat(z.nik_buat,'-',i.nama) as nik,a.no_kirim,a.keterangan as ket_kirim,concat(a.kode_gudanga,'-',b.nama) as gudasl,concat(a.kode_gudangt,'-',c.nama) as gudtuj,concat(a.kode_vendor,'-',d.nama) as nama_vendor ";
				if (this.jenisLap === "Transaksi")
					sql+=",f.kode_brg,g.nama as nama_brg,g.tipe,f.sat,ifnull(h.stok,0) as stok,f.jumlah ";
				sql+="from inv_mutkrm_m a "+
					...
					"  from inv_dt "+
					"  where kode_gudang=(select kode_gudangt from inv_muttrm_m where no_terima like '%"+this.sg1.getCell(2,4)+"') and kode_lokasi='"+this.sg1.getCell(2,0)+"' and periode<='"+this.sg1.getCell(2,1)+"' "+
					"  group by kode_brg,kode_lokasi) "+
					"  h on g.kode_brg=h.kode_brg and g.kode_lokasi=h.kode_lokasi "+this.filter+
					" order by z.no_terima ";
				...
				if (this.jenisLap === "Daftar"){
					this.title = new server_util_arrayList({items:["No Bukti","No Dokumen","Keterangan","NIK ","No Kirim","Keterangan Kirim","Gudang Asal","Gudang Tujuan","Vendor"]});			
					this.widthTable = new server_util_arrayList({items:[100,120,150,100,100,150,100,100,100]});
					...
					this.title = new server_util_arrayList({items:["Kode","Nama","Tipe","Satuan","Stok","Jumlah"]});
					this.widthTable = new server_util_arrayList({items:[100,120,120,70,80,100]});
			...
			systemAPI.alert("[app_saku_inventory_report_flMuttrm]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		if (this.jenisLap === "Transaksi")
			var header = "LAPORAN MUTASI TERIMA";
		else var header = "DAFTAR MUTASI TERIMA";
	...
	convertResultToHTML: function(result,data){		
		...
		for (var r in table.rs.rows){
			...
			if (no_bill != line.no_kirim){
				first = true;
				no_bill = line.no_kirim;
				if (htmlHeader !== ""){
			...
			if (first){
				...
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.no_kirim+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				...
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>No. Kirim</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.no_kirim+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>Ket. Kirim</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.ket_kirim+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>Gudang Asal</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.gudasl+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>Gudang Tujuan</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.gudtuj+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>Vendor</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.vendor+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "</table>";
			}
			urut+=1;
			...
			for (var c in line){
				if (c === "kode_brg" || c === "nama_brg" || c === "tipe" || c === "sat" || c === "stok" || c === "jumlah"){
					if (c === "jumlah" || c === "stok")
