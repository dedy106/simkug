window.app_saku_inventory_report_flGbBeliC = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_report_flGbBeliC.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_inventory_report_flGbBeliC";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Pencairan GB Pembelian", 2);
...
window.app_saku_inventory_report_flGbBeliC.extend(window.portalui_childForm);
window.app_saku_inventory_report_flGbBeliC.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row === 3)
		{
			this.standar.ListDataSGFilter(this, "Data No. Bukti Pencairan GB Pembelian",this.sg1, this.sg1.row, this.sg1.col,
		...
		try
		{
			...
			}else
			{
				...
				sql+="from kas_m a inner join bank2 b on a.kode_bank=b.kode_bank and a.kode_lokasi=b.kode_lokasi "+
					...
					"inner join vendor g on f.kode_vendor=g.kode_vendor and f.kode_lokasi=g.kode_lokasi "+this.filter+
				...
				if (this.jenisLap === "Daftar"){
					...
					this.title = new server_util_arrayList({items:["No. Bukti","No. GB","Tgl Terbit","Tgl Jth Tempo","Hutang GB","Keterangan","Vendor","Nilai"]});
			...
			systemAPI.alert("[app_saku_inventory_report_flGbBeliC]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		if (this.jenisLap === "Transaksi")
			var header = "LAPORAN PENCAIRAN GB PEMBELIAN";
		else var header = "DAFTAR PENCAIRAN GB PEMBELIAN";
