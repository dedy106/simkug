window.app_saku_inventory_report_flSop = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_report_flSop.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_inventory_report_flSop";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Stok Opname [SOP]", 2);
	...
	this.getPeriode="select distinct periode from inv_sop_m where kode_lokasi ='"+this.lokasi+"'";
...
window.app_saku_inventory_report_flSop.extend(window.portalui_childForm);
window.app_saku_inventory_report_flSop.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row === 3)
		{
			this.standar.ListDataSGFilter(this, "Data No. Bukti Stok Opname",this.sg1, this.sg1.row, this.sg1.col,
									  "select no_sop, keterangan from inv_sop_m "+
									  ...
									  "select count(*) from inv_sop_m "+
									  this.filterRep.filterStr("kode_gudang",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"where")+
									  this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
									  " and kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",
									  ["no_sop","keterangan"],"and",["No. Bukti","Keterangan"]);
		}
	},
	doSelectCell: function(sender, col, row){
		...
		if (row === 1){
			this.dbLib.setItemsFromSQL("select distinct periode from inv_sop_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 4)
		{
		...
		try
		{
			...
			}else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_sop",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new portalui_arrayMap();
				this.jenisLap = "Transaksi";/*this.sg1.getCell(2,4);*/
				var sql = "select a.no_sop,a.no_dokumen,a.keterangan,concat(a.nik_buat,'-',b.nama) as nik,concat(a.kode_gudang,'-',c.nama) as gudang ";
				if (this.jenisLap === "Transaksi")
					sql+=",e.kode_brg,e.nama,e.tipe,e.sat,ifnull(sum(d.stok),0) as stok,ifnull(sum(d.sop),0) as jml,  ifnull(sum(d.stok),0) - ifnull(sum(d.sop),0) as sls ";
				sql+="from inv_sop_m a "+
					"inner join karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+
					"inner join inv_gudang c on a.kode_gudang=c.kode_gudang and a.kode_lokasi=c.kode_lokasi "+
					"inner join inv_sop_d d on a.no_sop=d.no_sop and a.kode_gudang=d.kode_gudang and a.kode_lokasi=d.kode_lokasi "+
					"inner join inv_brg e on d.kode_brg=e.kode_brg and d.kode_lokasi=e.kode_lokasi "+this.filter+
					" group by d.no_sop order by a.no_sop ";
				...
				if (this.jenisLap === "Daftar"){
					...
					this.title = new server_util_arrayList({items:["Kode","Nama","Tipe","Satuan","Stok","Jml Opname","Selisih"]});
			...
			systemAPI.alert("[app_saku_inventory_report_flSop]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		if (this.jenisLap === "Transaksi")
			var header = "LAPORAN STOK OPNAME";
		else var header = "DAFTAR STOK OPNAME";
	...
	convertResultToHTML: function(result,data){		
		...
		for (var r in table.rs.rows){
			...
			if (first){
				...
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.no_sop+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
			...
			for (var c in line){
				if (c === "kode_brg" || c === "nama" || c === "tipe" || c === "sat" || c === "stok" || c === "jml" || c === "sls"){
					if (c === "stok" || c === "jml" || c === "sls")
