window.app_saku_inventory_report_flTrmJual = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_report_flTrmJual.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_inventory_report_flTrmJual";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Penerimaan Penjualan", 2);
...
window.app_saku_inventory_report_flTrmJual.extend(window.portalui_childForm);
window.app_saku_inventory_report_flTrmJual.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row === 5)
		{
			this.standar.ListDataSGFilter(this, "Data No. Bukti Penerimaan Penjualan",this.sg1, this.sg1.row, this.sg1.col,
	...
	mainButtonClick: function(sender){
		try
		{
			...
			}else
			{
				...
				var sql = "select a.no_kas,a.no_dokumen,a.keterangan,a.kode_bank,b.nama as nama_bank,a.nik_buat,c.nama as nama_buat,a.ref1,d.nama as nama_cust,a.akun_kb,a.posted,a.periode,a.nilai ";
				if (this.jenisLap === "Transaksi")
					sql+=",f.no_jual,f.akun_piutang,date_format(f.tanggal,'%d/%m/%Y') as tanggal,f.keterangan as ket,f.nilai+nilai_ppn as nilai_d,ifnull(g.totbayar,0)+ifnull(h.totgb,0) as totbayar,(f.nilai+f.nilai_ppn - (ifnull(g.totbayar,0)+ifnull(h.totgb,0))) as sisapiu, i.nbayar ";
				sql+="from kas_m a "+
					...
					"  where x.kode_lokasi='"+this.sg1.getCell(2,0)+"' and y.modul='INVJUAL' and x.no_del='-' and x.no_kas not like '%"+this.sg1.getCell(2,5)+"' "+
					...
					"  where x.kode_lokasi='"+this.sg1.getCell(2,0)+"' and y.modul='GBJUAL' and x.no_del='-' "+
					"  group by y.no_bukti,x.kode_lokasi) "+
					...
					"  from kas_m x "+
					"  inner join kas_d y on x.no_kas=y.no_kas and x.kode_lokasi=y.kode_lokasi "+
					"  where x.kode_lokasi='"+this.sg1.getCell(2,0)+"' and y.modul='INVJUAL' and x.no_del='-' and x.no_kas like '%"+this.sg1.getCell(2,5)+"' "+
					"  group by y.no_bukti,x.kode_lokasi) "+
			...
			systemAPI.alert("[app_saku_inventory_report_flTrmJual]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		if (this.jenisLap === "Transaksi")
			var header = "LAPORAN PENERIMAAN PENJUALAN";
	...
	convertResultToHTML: function(result,data){		
		...
		for (var r in table.rs.rows){
			...
			if (first){
				...
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.ref1+"-"+line.nama_cust+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
			...
			for (var c in line){
				if (c === "no_jual" || c === "akun_piutang" || c === "tanggal" || c === "ket" || c === "nilai_d" || c === "totbayar" || c === "sisapiu" || c === "nbayar"){
					if (c === "nilai_d" || c === "totbayar" || c === "sisapiu" || c === "nbayar")
