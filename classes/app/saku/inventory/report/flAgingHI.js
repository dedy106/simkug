window.app_saku_inventory_report_flAgingHI = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_report_flAgingHI.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_inventory_report_flAgingHI";
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Aging Hutang Per Invoice", 2);
	...
	this.gridLib.SGEditData(this.sg1,1,[0,1,2],["Vendor","All",""]);
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Invoice","All",""]);
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Posisi Tanggal","<=",new Date().lclFormat()]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku_inventory_report_flAgingHI.extend(window.portalui_childForm);
window.app_saku_inventory_report_flAgingHI.implement({
		...
		try{			
			...
			if (row === 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data No. Invoice",this.sg1, this.sg1.row, this.sg1.col,
											  "select no_beli,keterangan from inv_beli_m where kode_lokasi ='"+this.sg1.getCell(2,0)+"' and kode_vendor like '%"+this.sg1.getCell(2,2)+"'",
											  "select count(*) from inv_beli_m where kode_lokasi ='"+this.sg1.getCell(2,0)+"' and kode_vendor like '%"+this.sg1.getCell(2,2)+"'",
	...
	doSelectCell: function(sender, col, row){
		if (this.userStatus==="A"){
			this.filterRep.setSGFilterRowTipe(this.sg1,row,[0,1,2,3],["3","13","13","26"]);
		...
		try
		{
			...
			}else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.no_beli",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.tanggal",this.sg1.getCell(1,3),this.sg1.getCellDateValue(2,3),this.sg1.getCellDateValue(3,3),"and");
				this.filter2 = this.filterRep.filterStr("c.tanggal",this.sg1.getCell(1,3),this.sg1.getCellDateValue(2,3),this.sg1.getCellDateValue(3,3),"and");
				this.filter3 = this.filterRep.filterStr("e.tanggal",this.sg1.getCell(1,3),this.sg1.getCellDateValue(2,3),this.sg1.getCellDateValue(3,3),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new portalui_arrayMap();
				var tanggal = this.sg1.getCellDateValue(2,3);
				var sql = "select a.no_beli,a.keterangan,c.nmvend,sum(b.aging1) as aging1,sum(b.aging2) as aging2,sum(b.aging3) as aging3,sum(b.aging4) as aging4 "+
						"	from inv_beli_m a "+
						...
						"	group by a.no_beli) c on a.no_beli=c.no_beli and a.kode_lokasi=c.kode_lokasi "+
						"	left join (select a.kode_lokasi,a.no_beli, "+
						...
						"		   from inv_beli_m a  "+
						"		   left join kas_d b on a.no_beli=b.no_bukti and a.kode_lokasi=b.kode_lokasi  "+
						"		   left join kas_m c on b.no_kas=c.no_kas and b.kode_lokasi=c.kode_lokasi "+this.filter2+
						"		   left join gb_d d on a.no_beli=d.no_bukti and a.kode_lokasi=d.kode_lokasi  "+
						...
						"		   )b on a.no_beli=b.no_beli and a.kode_lokasi=b.kode_lokasi "+this.filter+
						"	group by a.no_beli order by a.no_beli ";
				this.scriptSqlCount = "select count(a.no_beli) "+
						"	from inv_beli_m a "+
						"	inner join (select no_beli,kode_lokasi "+
						"				from inv_beli_m"+
						"				group by no_beli) c on a.no_beli=c.no_beli and a.kode_lokasi=c.kode_lokasi "+this.filter;
				
				this.title = new server_util_arrayList({items:["No. Invoice","Keterangan","Vendor","0 - 30 Hari","31 - 60 Hari","61 - 90 Hari","> 90 Hari"]});
				this.widthTable = new server_util_arrayList({items:[60,200,200,90,90,90,90]});
			...
			systemAPI.alert("[app_saku_inventory_report_flAgingHI]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var header = "AGING HUTANG PER INVOICE";
