window.app_saku_inventory_report_flBeli = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_report_flBeli.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_inventory_report_flBeli";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Pembelian", 2);
...
window.app_saku_inventory_report_flBeli.extend(window.portalui_childForm);
window.app_saku_inventory_report_flBeli.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 4)
		{
			this.standar.ListDataSGFilter2(this, "Data No. Bukti Pembelian",this.sg1, this.sg1.row, this.sg1.col,
										  "select no_beli, keterangan  from inv_beli_m "+
										  ...
										  "select count(*) from inv_beli_m "+
										  this.filterRep.filterStr("kode_gudang",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"where")+
										  this.filterRep.filterStr("kode_vendor",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
										  this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
										  " and kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",
										  ["no_beli","keterangan"],"and",["No. Bukti","Keterangan"]);
		}
	},
		...
		try
		{
			...
			}else
			{
				...
				if (this.jenisLap === "Transaksi")
					sql+=",a.brg,a.nmbrg,a.tipe,a.sat,a.harga,a.jumlah,a.bonus,a.subtot ";
				sql+=" from ( "+
					"(select a.no_beli as id,b.nama as nmvend,c.nama as nmgud,date_format(a.tanggal,'%d-%m-%Y') as tgl,d.nama as jenis,a.no_dokumen as nodok,a.keterangan as ket,a.nilai as nilai,a.nilai_ppn as ppn,f.kode_brg as brg,f.nama as nmbrg,f.tipe,f.sat,e.harga,e.jumlah,e.bonus,(e.harga*e.jumlah) as subtot "+
						"from inv_beli_m a "+
							"inner join vendor b on a.kode_vendor=b.kode_vendor and a.kode_lokasi=b.kode_lokasi "+
							"inner join inv_gudang c on a.kode_gudang=c.kode_gudang and a.kode_lokasi=c.kode_lokasi "+
							"inner join inv_beli_jenis d on a.kode_jenis=d.kode_jenis and a.kode_lokasi=d.kode_lokasi "+
					...
					"(select a.no_beli as id,b.nama as nmvend,c.nama as nmgud,date_format(a.tanggal,'%d-%m-%Y') as tgl,d.nama as jenis,a.no_dokumen as nodok,a.keterangan as ket,a.nilai as nilai,a.nilai_ppn as ppn,'brg' as brg,e.kode_akun as nmbrg,f.nama as tipe,e.keterangan as harga,case e.dc when 'D' then '+' else '-' end as sat,-1 as jumlah,-1 as bonus,e.nilai as subtot "+
						"from inv_beli_m a "+
							"inner join vendor b on a.kode_vendor=b.kode_vendor and a.kode_lokasi=b.kode_lokasi "+
							"inner join inv_gudang c on a.kode_gudang=c.kode_gudang and a.kode_lokasi=c.kode_lokasi "+
							"inner join inv_beli_jenis d on a.kode_jenis=d.kode_jenis and a.kode_lokasi=d.kode_lokasi "+
							"inner join inv_beli_j e on a.no_beli=e.no_beli and a.kode_lokasi=e.kode_lokasi and e.modul='INVBELI' and e.jenis='BBNBELI' "+
				...
				if (this.jenisLap === "Daftar"){
					...
					this.widthTable2 = new server_util_arrayList({items:[121,250,250,100,100]});
			...
			systemAPI.alert("[app_saku_inventory_report_flBeli]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		if (this.jenisLap === "Transaksi")
			var header = "DAFTAR DETAIL PEMBELIAN";
	...
	convertResultToHTML: function(result,data){		
		...
		for (var r in table.rs.rows){
			...
			if (no_bill !== line.id){
				...
				no_bill = line.id;
				if (htmlHeader !== ""){
					html+="<tr align='right' bgcolor='#EEEEEE'><td class='isi_laporan' colspan='8'>Sub Total</td>"+
			...
			for (var c in line){
				if (line["brg"] === "brg"){
					...
					if (c === "brg" || c === "nmbrg" || c === "tipe" || c === "sat" || c === "harga" || c === "jumlah" || c === "bonus" || c === "subtot"){
