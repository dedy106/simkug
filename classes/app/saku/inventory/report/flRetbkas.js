window.app_saku_inventory_report_flRetbkas = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_report_flRetbkas.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_inventory_report_flRetbkas";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Penerimaan Retur Pembelian", 2);
...
window.app_saku_inventory_report_flRetbkas.extend(window.portalui_childForm);
window.app_saku_inventory_report_flRetbkas.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row === 5)
		{
			this.standar.ListDataSGFilter(this, "Data No. Bukti Penerimaan Retur Pembelian",this.sg1, this.sg1.row, this.sg1.col,
										  ...
										  " and kode_lokasi = '"+this.sg1.getCell(2,0)+"' and modul='KBI_RETB' and no_del='-' ",
	...
	mainButtonClick: function(sender){
		try
		{
			...
			}else
			{
				...
				var sql = "select a.no_kas,a.no_dokumen,a.keterangan,concat(a.kode_bank,'-',b.nama) as bank,concat(a.nik_buat,'-',c.nama) as nik,concat(a.ref1,'-',d.nama) as vendor,a.nilai as tot ";
				if (this.jenisLap === "Transaksi")
					sql+=",y.no_retur,y.akun_piutang,date_format(y.tanggal,'%d/%m/%Y') as tanggal,y.keterangan as ket,y.nilai ";
				sql+="from kas_d x inner join inv_beliretur_m y on y.no_retur=x.no_bukti and x.kode_lokasi=y.kode_lokasi "+
					...
					" and x.modul='KBIRETB' and y.no_del='-' order by a.no_kas";
				...
				if (this.jenisLap === "Daftar"){
					...
					this.title = new server_util_arrayList({items:["No. Retur","Ak. Piutang","Tanggal","Keterangan","Nilai"]});
			...
			systemAPI.alert("[app_saku_inventory_report_flRetbkas]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		if (this.jenisLap === "Transaksi")
			var header = "LAPORAN PENERIMAAN RETUR PEMBELIAN";
		else var header = "DAFTAR PENERIMAAN RETUR PEMBELIAN";
	...
	convertResultToHTML: function(result,data){		
		...
		for (var r in table.rs.rows){
			...
			for (var c in line){
				if (c === "no_retur" || c === "akun_piutang" || c === "tanggal" || c === "ket" || c === "nilai"){
					if (c === "nilai")
