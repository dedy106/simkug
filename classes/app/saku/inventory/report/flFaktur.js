window.app_saku_inventory_report_flFaktur = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_report_flFaktur.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_inventory_report_flFaktur";
		this.maximize();
		this.app._mainForm.childFormConfig(this,"mainButtonClick","Laporan Faktur Pajak Standar",2);
	...
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["No. Faktur","All",""]);
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Lembar Faktur","=","Semua"]);
	this.pager = 1;
...
window.app_saku_inventory_report_flFaktur.extend(window.portalui_childForm);
window.app_saku_inventory_report_flFaktur.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row === 2)
		{
			this.filterRep.ListDataSGFilter(this, "Data Faktur",this.sg1, this.sg1.row, this.sg1.col,
										  "select no_faktur from inv_invoice_m where kode_lokasi='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"' ",
										  "select count(*) from inv_invoice_m where kode_lokasi ='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"' ",
	...
	doSelectCell: function(sender, col, row)
	{
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["Semua","Lembar-1","Lembar-2"]);
		}
	},
	mainButtonClick: function(sender)
	{
		try
		{
			...
			}else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  ...
							  this.filterRep.filterStr("a.no_faktur",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2=this.sg1.getCell(2,3);
				this.nama_report="server_report_inventory_rptFaktur";
		...
		{
			alert("[app_saku_inventory_report_flFaktur]::mainButtonClick:"+e);
		}
	},
		...
		try{
			if (sender === this.confirm.bConfirm){
				...
				if (to !== ""){
					this.confirm.free();
					var d = new Date();
					var subject = "Laporan Faktur Pajak "+d.toLocaleString();
