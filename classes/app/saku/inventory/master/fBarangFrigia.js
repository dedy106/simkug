window.app_saku_inventory_master_fBarangFrigia = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_master_fBarangFrigia.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_inventory_master_fBarangFrigia";
		...
		this.e_harga = new portalui_saiLabelEdit(this,{bound:[20,42,180,20],caption:"Harga", tipeText:ttNilai});		
...
window.app_saku_inventory_master_fBarangFrigia.extend(window.portalui_childForm);
window.app_saku_inventory_master_fBarangFrigia.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into fri_barang_m(kode_brg,nama,kode_kbm,kode_jenis,kode_klp,kode_lokasi,satuan,harga) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.cb_kbm.getText()+"','"+this.cb_jenis.getText()+"','"+this.cb_klp.getText()+"','"+this.app._lokasi+"','"+this.e_sat.getText()+"',"+parseNilai(this.e_harga.getText())+")");
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update fri_barang_m set harga ="+parseNilai(this.e_harga.getText())+",satuan='"+this.e_sat.getText()+"',nama = '"+this.e_nama.getText()+"', kode_kbm='"+this.cb_kbm.getText()+"',kode_jenis='"+this.cb_jenis.getText()+"',kode_klp='"+this.cb_klp.getText()+"' "+
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var data = this.dbLib.getDataProvider("select a.nama,a.satuan,a.harga,b.kode_klp,b.nama as nama_klp,c.kode_jenis,c.nama as nama_jenis,d.kode_kbm,d.nama as nama_kbm "+
