window.app_saku_inventory_master_fBrg2 = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_master_fBrg2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_inventory_master_fBrg2";
		...
		this.cb_sat = new portalui_saiCB(this,{bound:[20,14,200,20],caption:"Satuan 1",mustCheck: false});
		this.cb_sat2 = new portalui_saiCB(this,{bound:[20,16,200,20],caption:"Satuan 2",mustCheck: false});
		...
		{			
			...
			var prd = this.dbLib.getDataProvider("select kode_sat from inv_sat where kode_lokasi = '"+this.app._lokasi+"'",true);
			if (typeof prd == "object"){						
				var items = [];
				for (var i in prd.rs.rows) items.push(prd.rs.rows[i].kode_sat);			
				this.cb_sat.setItem(new portalui_arrayMap({items:items}));
				this.cb_sat2.setItem(new portalui_arrayMap({items:items}));
			}
...
window.app_saku_inventory_master_fBrg2.extend(window.portalui_childForm);
window.app_saku_inventory_master_fBrg2.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0]))
			{
				...
				{									
					...
					sql.add("insert into inv_brg(kode_brg,nama,tipe,kode_klpbrg,sat,sat2,harga,kode_lokasi) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.e_tipe.getText()+"','"+this.cb_klp.getText()+"','"+this.cb_sat.getText()+"','"+this.cb_sat2.getText()+"',"+parseNilai(this.e_harga.getText())+",'"+this.app._lokasi+"')");
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0]))
			{
				...
				{									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update inv_brg set nama='"+this.e_nama.getText()+"',tipe='"+this.e_tipe.getText()+"',kode_klpbrg='"+this.cb_klp.getText()+"',sat='"+this.cb_sat.getText()+"',sat2='"+this.cb_sat2.getText()+"',harga="+parseNilai(this.e_harga.getText())+" "+
	...
	doTampilClick: function(sender){
		try{			
			this.sg1.setColTitle(new Array("No","Kode Brg","Nama","Tipe","Kelompok","Satuan","Satuan2","Harga Jual"));				
			var data = this.dbLib.runSQL(" select a.kode_brg,a.nama,a.tipe,b.nama as nama_klp,a.sat,a.sat2,a.harga from inv_brg a "+
