window.app_saku_inventory_master_fKlpbeli = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_master_fKlpbeli.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_inventory_master_fKlpbeli";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Parameter Pembelian", 0);	
		...
		this.cb_ap = new portalui_saiCBBL(this,{bound:[20,12,200,20],caption:"Akun Hutang",btnClick:[this,"doBtnClick"]});
		this.bTampil = new portalui_button(this,{bound:[829,12,80,18],caption:"Tampil",click:[this,"doTampilClick"]});			
		this.p1 = new portalui_panel(this,{bound:[10,14,900,380],caption:"Daftar Jenis Pembelian"});
...
window.app_saku_inventory_master_fKlpbeli.extend(window.portalui_childForm);
window.app_saku_inventory_master_fKlpbeli.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try{									
					...
					sql.add("insert into inv_beli_jenis(kode_jenis,nama,akun_hutang,kode_lokasi) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.cb_ap.getText()+"','"+this.app._lokasi+"')");
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try{									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update inv_beli_jenis set nama = '"+this.e_nama.getText()+"',akun_hutang='"+this.cb_ap.getText()+"' "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try{									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from inv_beli_jenis "+
						    "where kode_jenis = '"+this.cb_kode.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
	...
	doLoadClick: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var data = this.dbLib.getDataProvider(" select a.nama,a.akun_hutang,b.nama as nama_ap "+
				           " from inv_beli_jenis a "+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){
						...
						this.cb_ap.setText(line.akun_hutang,line.nama_ap);
	...
	doTampilClick: function(sender){
		try{			
			this.sg1.setColTitle(new Array("No","Kode Jenis","Nama Jenis","Akun Hutang","Nama Akun"));				
			var data = this.dbLib.runSQL(" select a.kode_jenis, a.nama, a.akun_hutang, b.nama as nama_ap "+
			           " from inv_beli_jenis a "+
					   "         inner join masakun b on a.akun_hutang=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
