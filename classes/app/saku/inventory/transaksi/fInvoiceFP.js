...
//* 			SAI, PT											
//***********************************************************************************************
window.app_saku_inventory_transaksi_fInvoiceFP = function(owner,options){
	try{
		if (owner)
		{
			window.app_saku_inventory_transaksi_fInvoiceFP.prototype.parent.constructor.call(this, owner,options);			
			this.className = "app_saku_inventory_transaksi_fInvoiceFP";
			this.maximize();
			this.app._mainForm.childFormConfig(this, "mainButtonClick","Invoice Faktur Pajak",0);				
            this.initComponent();		
	...
	{
		alert("[app_saku_inventory_transaksi_fInvoiceFP]::contruct:"+e,"");
	}
};
window.app_saku_inventory_transaksi_fInvoiceFP.extend(window.portalui_childForm);
window.app_saku_inventory_transaksi_fInvoiceFP.implement({
	initComponent: function(){		
		try{
			...
			this.dp_order = new portalui_datePicker(this,{bound:[110,32,82,20],selectDate:[this,"doSelect"]});
			this.e0ip = new portalui_saiCBBL(this,{bound:[10,54,250,20],caption:"No. Invoice",btnVisible:false});
			...
			this.eTerm = new portalui_saiLabelEdit(this,{bound:[10,75,180,20],caption:"Terms"});
			this.lTgl = new portalui_label(this,{bound:[10,32,100,18],underline:true,caption:"Due Date"});
			this.dp_duedate = new portalui_datePicker(this,{bound:[110,32,82,20],selectDate:[this,"doSelect"]});
			
            this.eCust = new portalui_saiCBBL(this,{bound:[10,75,200,20],caption:"Tujuan",btnClick:[this,"FindBtnClick"]});
			this.eSJ = new portalui_saiCBBL(this,{bound:[10,78,200,20],caption:"Surat Jalan",btnClick:[this,"FindBtnClick"]});
			
			this.p1ip = new portalui_panel(this,{bound:[10,186,800,250],border:3,name:"P1",caption:"Data Order"});
			this.sg1ip = new portalui_saiGrid(this.p1ip,{bound:[1,20,this.p1ip.width - 4,203],name:"saiSG1",colCount:6,colTitle:["Kode Barang","Nama Barang","Quantity","Satuan","Harga Satuan","Total"],
                colWidth:[[5,4,3,2,1,0],[100,100,100,100,200,100]],colFormat:[[2,4,5],[cfNilai,cfNilai,cfNilai]],
			...
			this.dp_order.setDateString(new Date().getDateStr());
			...
			if (sender == this.bGen){
				...
				if (this.sg1ip.allRowValid()){
					...
					sql.add("insert into portal_order_m (no_order,kode_cust,tanggal,keterangan,status,cabang, kota,nama,alamat,sales,periode,kode_lokasi,nik_user,tgl_input,status_bayar) values "+
							"('"+this.e0ip.getText()+"','"+this.ecustip.getText()+"','"+this.dp_order.getDate()+"','"+this.e1ip.getText()+"','C','"+this.cab.getText()+"','"+this.kotaip.getText()+"','-','-','"+this.salesip.getText()+"','"+this.getForm().getPeriodeNow()+"','"+this.app._lokasi+"','"+this.app.userlog+"','"+(new Date()).getDateStr()+"','0') ");
					...
					this.tglOrder = this.dp_order.getDate();
