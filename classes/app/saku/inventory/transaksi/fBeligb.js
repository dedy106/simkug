window.app_saku_inventory_transaksi_fBeligb = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_transaksi_fBeligb.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_inventory_transaksi_fBeligb";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pembayaran Pembelian [GB]: Input", 0);	
		...
		this.cb_gb = new portalui_saiCBBL(this,{bound:[20,12,200,20],caption:"Akun Hutang GB",btnClick:[this,"doBtnClick"],tag:2,change:[this,"doChange"]});
		this.e_tothut = new portalui_saiLabelEdit(this,{bound:[723,12,200,20],caption:"Total Hutang",tipeText:ttNilai,readOnly:true,text:"0"});
...
window.app_saku_inventory_transaksi_fBeligb.extend(window.portalui_childForm);
window.app_saku_inventory_transaksi_fBeligb.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,2])){
				try{									
					...
					sql.add("insert into gb_m (no_gb,kode_lokasi,no_dokumen,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,"+
							...
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.akunkb+"','"+this.dp_d1.getDateString()+"','"+this.e_desc.getText()+"','"+this.app._kodePP+"','GB','BELI','"+this.e_periode.getText()+
						...
						if (this.sg.rowValid(i)){
							if (nilaiToFloat(this.sg.getCell(7,i)) != 0){
								...
								sql.add("insert into gb_d(no_gb,no_bukti,modul,catatan,no_del,kode_lokasi,nilai) values "+
										"                ('"+this.e_nb.getText()+"','"+this.sg.cells(0,i)+"','GBBELI','-','-','"+this.app._lokasi+"',"+parseNilai(this.sg.cells(7,i))+")");
							}
						}
					}
		...
		try{			
			if (this.cb_vend.getText() != ""){
				...
				var data = this.dbLib.getDataProvider("select no_retur, akun_piutang, date_format(tanggal,'%d/%m/%Y') as tanggal, keterangan, nilai "+
				           "from inv_beliretur_m where progress='0' and kode_vendor='"+this.cb_vend.getText()+"' and periode<= '"+this.e_periode.getText()+"' ");
				eval("data = "+data+";");
				if (typeof data == "object"){
					var line;
					this.sg2.clear();
