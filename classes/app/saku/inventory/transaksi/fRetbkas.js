window.app_saku_inventory_transaksi_fRetbkas = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_transaksi_fRetbkas.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_inventory_transaksi_fRetbkas";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penerimaan Retur Pembelian: Input", 0);	
		...
		this.p1 = new portalui_panel(this,{bound:[20,19,900,290],caption:"Item Faktur Retur Pembelian Barang"});
...
window.app_saku_inventory_transaksi_fRetbkas.extend(window.portalui_childForm);
window.app_saku_inventory_transaksi_fRetbkas.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,2])){
				try{									
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,"+
							...
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.akunkb+"','"+this.dp_d1.getDateString()+"','"+this.e_desc.getText()+"','"+this.app._kodePP+"','KBI_RETB','"+this.jenis+"','"+this.e_periode.getText()+
						...
						if (this.sg.rowValid(i)){
							if (this.sg.getCell(0,i) == "BAYAR"){
								...
								sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
										...
										"','Bayar retur pembelian','C',"+parseNilai(this.sg.cells(5,i))+",'"+this.app._kodePP+"','-','-',"+
										"'"+this.app._lokasi+"','KBI_RETB','RETBELI',"+
										"'"+this.e_periode.getText()+"','IDR',1"+
										",'"+this.app._userLog+"',now(),'-')");
								sql.add("update inv_beliretur_m set progress='1', no_kas='"+this.e_nb.getText()+"' where no_retur='"+this.sg.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
								sql.add("insert into kas_d(no_kas,no_bukti,modul,catatan,no_del,kode_lokasi,nilai) values "+
										"                 ('"+this.e_nb.getText()+"','"+this.sg.cells(1,i)+"','KBIRETB','-','-','"+this.app._lokasi+"',"+parseNilai(this.sg.cells(5,i))+")");
							}
						}
					}
			...
			if (this.cb_vend.getText() != ""){
				...
				if (typeof data == "object"){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg.appendData(["INPROG",line.no_retur,line.akun_piutang,line.tanggal,line.keterangan,floatToNilai(line.nilai)]);
					}
					this.sg.validasi();
				}
			}
		...
		try{
			...
			if (sender == this.cb_vend) {   
			    this.standarLib.showListData(this, "Daftar Vendor",sender,undefined, 
											  ...
											  ["kode_vendor","nama"],"and",["Kode Vend","Nama"],false);				
				this.sg.clear(1);
			}
