window.app_saku_inventory_transaksi_fJualret = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_transaksi_fJualret.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_inventory_transaksi_fJualret";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Faktur Retur Penjualan Barang: Input", 0);	
...
window.app_saku_inventory_transaksi_fJualret.extend(window.portalui_childForm);
window.app_saku_inventory_transaksi_fJualret.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,2])){
				try{									
					...
					sql.add("insert into inv_jualretur_m(no_retur,kode_lokasi,tanggal,no_dokumen,keterangan,kode_cust,nik_buat,nilai,periode,nik_user,tgl_input,no_link,no_del,posted,kode_pp,kode_curr,akun_hutang,kode_gudang, progress,no_kas) values" +
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_desc.getText()+"','"+this.cb_cust.getText()+"','"+this.cb_app.getText()+"',"+
					...
					if (this.sg.getRowValidCount() > 0){
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								if (i > 0) d+= ",";
								d += "('"+this.e_nb.getText()+"','"+this.app._lokasi+"',"+i+",'"+this.sg.cells(0,i)+"',"+parseNilai(this.sg.cells(4,i))+","+parseNilai(this.sg.cells(5,i))+","+parseNilai(this.sg.cells(6,i))+",'RETJUAL','D','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_gudang.getText()+"',0)";
							}
						}						
						sql.add(d);
					}						
					sql.add("insert into inv_jualretur_j(no_retur,no_dokumen,tanggal,nu,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values" +
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.akunRet+"','"+this.e_desc.getText()+"','D',"+parseNilai(this.e_total.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','RETJUAL','RETUR','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())");
					sql.add("insert into inv_jualretur_j(no_retur,no_dokumen,tanggal,nu,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values" +
						    "('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',1,'"+this.akunHut+"','"+this.e_desc.getText()+"','C',"+parseNilai(this.e_total.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','RETJUAL','HUT','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())");
