window.app_saku_inventory_transaksi_fBeliretk = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_transaksi_fBeliretk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_inventory_transaksi_fBeliretk";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Faktur Retur Pembelian Barang: Koreksi", 0);	
		...
		try{			
			...
			var prd = this.dbLib.getDataProvider("select distinct periode from inv_beliretur_m where kode_lokasi = '"+this.app._lokasi+"'",true);
...
window.app_saku_inventory_transaksi_fBeliretk.extend(window.portalui_childForm);
window.app_saku_inventory_transaksi_fBeliretk.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					if (parseFloat(this.perLama) < parseFloat(this.app._periode)) {
						...
						sql.add(" update inv_beliretur_m set no_link='"+this.e_nb.getText()+"',no_del = concat(no_retur,'r') where no_retur ='"+this.cb_nbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
						sql.add(" insert into inv_beliretur_m (no_retur,kode_lokasi,tanggal,no_dokumen,keterangan,kode_vendor,nik_buat,nilai,periode,nik_user,tgl_input,no_link,no_del,posted,kode_pp,kode_curr,akun_piutang,kode_gudang, progress,no_kas)"+
								" select concat(no_retur,'r'),kode_lokasi,'"+this.dp_d1.getDateString()+"',no_dokumen,keterangan,kode_vendor,'"+this.cb_app.getText()+"',nilai,'"+this.e_periode.getText()+"','"+this.app.userLog+"',now(),no_link,no_retur,'F',kode_pp,kode_curr,akun_piutang,kode_gudang, 0,'-' "+ 
								" from inv_beliretur_m where no_retur = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");							
						sql.add(" insert into inv_beliretur_j (no_retur,no_dokumen,tanggal,nu,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input)"+
								...
								" from inv_beliretur_j where no_retur = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");																					
						sql.add(" insert into inv_dt (no_bukti,kode_lokasi,nu,kode_brg,harga,jumlah,bonus,modul,dc,periode,tanggal,kode_gudang,pdisk)"+
								...
								" from inv_dt where no_bukti = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' and modul = 'RETBELI'");
						this.nb = this.e_nb.getText();
					}
					else{
						sql.add("delete from inv_beliretur_m where no_retur='"+this.cb_nbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from inv_beliretur_j where no_retur='"+this.cb_nbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from inv_dt where no_bukti ='"+this.cb_nbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' and modul = 'RETBELI'");			
					...
					sql.add("insert into inv_beliretur_m(no_retur,kode_lokasi,tanggal,no_dokumen,keterangan,kode_vendor,nik_buat,nilai,periode,nik_user,tgl_input,no_link,no_del,posted,kode_pp,kode_curr,akun_piutang,kode_gudang, progress,no_kas) values" +
						    "('"+this.nb+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_desc.getText()+"','"+this.cb_vend.getText()+"','"+this.cb_app.getText()+"',"+
					...
					if (this.sg.getRowValidCount() > 0){
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								if (i > 0) d+= ",";
								d += "('"+this.nb+"','"+this.app._lokasi+"',"+i+",'"+this.sg.cells(0,i)+"',"+parseNilai(this.sg.cells(5,i))+","+parseNilai(this.sg.cells(6,i))+",0,'RETBELI','C','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_gudang.getText()+"',0)";
							}
						}						
						sql.add(d);
					}	
					sql.add("insert into inv_beliretur_j(no_retur,no_dokumen,tanggal,nu,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values" +
						    "('"+this.nb+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.akunPiut+"','"+this.e_desc.getText()+"','D',"+parseNilai(this.e_total.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','RETBELI','PIUT','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())");
					sql.add("insert into inv_beliretur_j(no_retur,no_dokumen,tanggal,nu,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values" +
							"('"+this.nb+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',1,'"+this.akunRet+"','"+this.e_desc.getText()+"','C',"+parseNilai(this.e_total.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','RETBELI','RETUR','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())");
		...
		switch (event){
			...
			case "hapus" : 
				...
				try{
					...
					var sql = new server_util_arrayList();
					if (parseFloat(this.perLama) < parseFloat(this.app._periode)) {
						sql.add(" update inv_beliretur_m set no_del = concat(no_retur,'r') where no_retur ='"+this.cb_nbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
						sql.add(" insert into inv_beliretur_m (no_retur,kode_lokasi,tanggal,no_dokumen,keterangan,kode_vendor,nik_buat,nilai,periode,nik_user,tgl_input,no_link,no_del,posted,kode_pp,kode_curr,akun_piutang,kode_gudang, progress,no_kas)"+
								" select concat(no_retur,'r'),kode_lokasi,'"+this.dp_d1.getDateString()+"',no_dokumen,keterangan,kode_vendor,'"+this.cb_app.getText()+"',nilai,'"+this.e_periode.getText()+"','"+this.app.userLog+"',now(),no_link,no_retur,'F',kode_pp,kode_curr,akun_piutang,kode_gudang, 0,'-' "+ 
								" from inv_beliretur_m where no_retur = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");							
						sql.add(" insert into inv_beliretur_j (no_retur,no_dokumen,tanggal,nu,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input)"+
	...
	doLoadData:function(sender){
		if (this.cb_nbLama.getText() != "") {
			var data = this.dbLib.getDataProvider("select a.no_dokumen,a.keterangan,a.kode_gudang,b.nama as nama_gudang,a.kode_vendor,c.nama as nama_vendor,a.nik_buat,d.nama as nama_buat,a.nilai,a.posted,a.periode,a.progress, "+
												  "       e.kode_brg,f.nama as nama_brg,f.tipe,f.sat,(ifnull(x.jml,0) + e.jumlah) as stok,e.harga,e.jumlah,(e.jumlah * e.harga) as subttl "+
												  "from inv_beliretur_m a inner join inv_gudang  b on a.kode_gudang=b.kode_gudang and a.kode_lokasi=b.kode_lokasi "+
												  ...
												  "                  inner join inv_dt e on a.no_retur=e.no_bukti and a.kode_lokasi=e.kode_lokasi and e.modul='RETBELI' "+
												  "                  inner join inv_brg f on e.kode_brg=f.kode_brg and e.kode_lokasi=f.kode_lokasi  "+
												  "                  inner join   (select kode_brg,kode_gudang,kode_lokasi,sum(case dc when 'D' then jumlah+bonus else -(jumlah+bonus) end) as jml "+
												  "                                from inv_dt where kode_lokasi='"+this.app._lokasi+"' and periode<='"+this.e_periode.getText()+"' group by kode_brg,kode_gudang,kode_lokasi) x on e.kode_brg=x.kode_brg and e.kode_lokasi=x.kode_lokasi and a.kode_gudang=x.kode_gudang and a.kode_lokasi=x.kode_lokasi "+ 
			...
			if (typeof data == "object"){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData([line.kode_brg,line.nama_brg,line.tipe,line.sat,floatToNilai(line.stok),floatToNilai(line.harga),floatToNilai(line.jumlah),floatToNilai(line.subttl)]);
				}
				this.sg.validasi();
				if (line !== undefined){
	...
	doBtnClick: function(sender, event){
		try{
			if (sender == this.cb_nbLama) {   
			    this.standarLib.showListData(this, "Daftar Retur Pembelian",sender,undefined, 
											  "select no_retur, no_dokumen  from inv_beliretur_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and no_del='-'", 
											  "select count(no_retur) from inv_beliretur_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and no_del='-'",
