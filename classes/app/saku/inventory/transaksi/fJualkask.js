window.app_saku_inventory_transaksi_fJualkask = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_transaksi_fJualkask.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_inventory_transaksi_fJualkask";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penerimaan Penjualan [KB]: Koreksi", 0);	
		...
		this.e_totpiu = new portalui_saiLabelEdit(this,{bound:[723,15,200,20],caption:"Total Piutang",tipeText:ttNilai,readOnly:true,text:"0", tag:1});
		...
		this.p1 = new portalui_panel(this,{bound:[20,18,900,295],caption:"Item Faktur Penjualan Barang"});
		this.sg = new portalui_saiGrid(this.p1,{bound:[0,20,895,270],colCount:8,tag:2,colTitle:["No Faktur","Ak. Piutang","Tanggal","Keterangan","Netto","Total Bayar","Sisa Piutang","Nilai Pelunasan"],
		...
		try{			
			...
			var prd = this.dbLib.getDataProvider("select distinct periode from kas_m where modul = 'KBI_JUAL' and kode_lokasi = '"+this.app._lokasi+"'",true);
...
window.app_saku_inventory_transaksi_fJualkask.extend(window.portalui_childForm);
window.app_saku_inventory_transaksi_fJualkask.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,"+
							...
							"('"+this.nb+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.akunkb+"','"+this.dp_d1.getDateString()+"','"+this.e_desc.getText()+"','"+this.app._kodePP+"','KBI_JUAL','"+this.jenis+"','"+this.e_periode.getText()+
					...
					sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
							...
							"','"+this.e_desc.getText()+"','D',"+parseNilai(this.e_totkas.getText())+",'"+this.app._kodePP+"','-','-',"+
							"'"+this.app._lokasi+"','KBI_JUAL','KAS',"+
						...
						if (this.sg.rowValid(i)){
							if (nilaiToFloat(this.sg.getCell(7,i)) != 0){
								...
								sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
										...
										"'"+this.app._lokasi+"','KBI_JUAL','PIUJUAL',"+
										"'"+this.e_periode.getText()+"','IDR',1"+
										",'"+this.app._userLog+"',now(),'-')");
									
								sql.add("insert into kas_d(no_kas,no_bukti,modul,catatan,no_del,kode_lokasi,nilai) values "+
										"                 ('"+this.nb+"','"+this.sg.cells(0,i)+"','INVJUAL','-','-','"+this.app._lokasi+"',"+parseNilai(this.sg.cells(7,i))+")");
							}
						}
					}
							...
							if (this.sg2.rowValid(i)){
								if (this.sg2.getCell(0,i) == "OFFSET"){
									...
									sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
											...
											"'"+this.app._lokasi+"','KBI_JUAL','RETJUAL',"+
											"'"+this.e_periode.getText()+"','IDR',1"+
											",'"+this.app._userLog+"',now(),'-')");									
									sql.add("update inv_jualretur_m set progress='1', no_kas='"+this.nb+"' where no_retur='"+this.sg2.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
									sql.add("insert into kas_d(no_kas,no_bukti,modul,catatan,no_del,kode_lokasi,nilai) values "+
											"                 ('"+this.nb+"','"+this.sg2.cells(1,i)+"','RETJUAL','-','-','"+this.app._lokasi+"',"+parseNilai(this.sg2.cells(5,i))+")");
								}
							}
						}
					}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :	
				...
				if ((nilaiToFloat(this.e_totkas.getText()) <= 0) || ((nilaiToFloat(this.e_totkas.getText())+nilaiToFloat(this.e_totret.getText())) > nilaiToFloat(this.e_totpiu.getText()))){
					system.alert(this,"Transaksi tidak valid.","Nilai kasbank dan atau retur tidak boleh kurang dari atau sama dengan nol / melebihi sisa piutang.");
					return false;
				}
				for (var i=0;i < this.sg.getRowCount();i++){
					if (this.sg.rowValid(i)){
	...
	doChange: function(sender){
		try{			
			...
			if (sender == this.cb_jenis) {
				if (this.cb_jenis.getText().toUpperCase() == "KAS") this.jenis = "KM";
				if (this.cb_jenis.getText().toUpperCase() == "BANK") this.jenis = "BM";
			}
		}catch(e){
			systemAPI.alert(e);
		}
	...
	doLoadData:function(sender){
		if (this.cb_nbLama.getText() != "") {
			var data = this.dbLib.getDataProvider("select a.no_dokumen,a.keterangan,a.kode_bank,b.nama as nama_bank,a.nik_buat,c.nama as nama_buat,a.ref1,d.nama as nama_cust,a.akun_kb,a.posted,a.periode,a.nilai, "+
												  "       f.no_jual,f.akun_piutang,date_format(f.tanggal,'%d/%m/%Y') as tanggal,f.keterangan,f.nilai+nilai_ppn as nilai_d,ifnull(g.totbayar,0)+ifnull(h.totgb,0) as totbayar,(f.nilai+f.nilai_ppn - (ifnull(g.totbayar,0)+ifnull(h.totgb,0))) as sisapiu, i.nbayar "+
												  ...
												  "                  inner join kas_d e on a.no_kas=e.no_kas and a.kode_lokasi=e.kode_lokasi and e.modul='INVJUAL' "+
												  ...
												  "                      where x.kode_lokasi='"+this.app._lokasi+"' and y.modul='INVJUAL' and x.no_del='-' and x.no_kas <> '"+this.cb_nbLama.getText()+"' "+
			...
			if (typeof data == "object"){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData([line.no_jual,line.akun_piutang,line.tanggal,line.keterangan,floatToNilai(line.nilai_d),floatToNilai(line.totbayar),floatToNilai(line.sisapiu),floatToNilai(line.nbayar)]);
				}
				this.sg.validasi();
				if (line !== undefined){				
					...
					this.cb_cust.setText(line.ref1, line.nama_cust);
			...
			var data = this.dbLib.getDataProvider("select y.no_retur, date_format(y.tanggal,'%d/%m/%Y') as tanggal,y.akun_hutang,y.keterangan,y.nilai "+
												  ...
												  "where x.kode_lokasi='"+this.app._lokasi+"' and x.modul='RETJUAL' and y.no_del='-' and x.no_kas = '"+this.cb_nbLama.getText()+"'");
			eval("data = "+data+";");
			if (typeof data == "object"){
		...
		try{
			if (sender == this.cb_nbLama) {   
			    this.standarLib.showListData(this, "Daftar Bukti KasBank",sender,undefined, 
											  "select no_kas, no_dokumen  from kas_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and modul='KBI_JUAL' and no_del='-'", 
											  "select count(no_kas) from kas_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and modul='KBI_JUAL' and no_del='-'",
