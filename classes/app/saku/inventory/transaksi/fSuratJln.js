...
//* 			SAI, PT											
//***********************************************************************************************
window.app_saku_inventory_transaksi_fSuratJln = function(owner,options){
	try{
		if (owner)
		{
			window.app_saku_inventory_transaksi_fSuratJln.prototype.parent.constructor.call(this, owner,options);			
			this.className = "app_saku_inventory_transaksi_fSuratJln";
			this.maximize();
			this.app._mainForm.childFormConfig(this, "mainButtonClick","Surat Jalan",0);				
            this.initComponent();		
	...
	{
		alert("[app_saku_inventory_transaksi_fSuratJln]::contruct:"+e,"");
	}
};
window.app_saku_inventory_transaksi_fSuratJln.extend(window.portalui_childForm);
window.app_saku_inventory_transaksi_fSuratJln.implement({
	initComponent: function(){		
		try{
			...
			this.e0ip = new portalui_saiCBBL(this,{bound:[10,54,250,20],caption:"No. Surat Jalan",btnVisible:false});
			...
			this.ePacking = new portalui_saiCBBL(this,{bound:[10,75,200,20],caption:"Packing List",btnClick:[this,"FindBtnClick"]});
			this.eMobil = new portalui_saiLabelEdit(this,{bound:[10,71,200,20],caption:"No Mobil", readOnly:true});
			this.eSopir = new portalui_saiCBBL(this,{bound:[10,72,200,20],caption:"Sopir", readOnly:true, btnVisible:false});
			
			this.p1ip = new portalui_panel(this,{bound:[10,186,800,250],border:3,name:"P1",caption:"Sales Order"});
			this.sg1ip = new portalui_saiGrid(this.p1ip,{bound:[1,20,this.p1ip.width - 4,203],name:"saiSG1",colCount:6,colTitle:["Kode Barang","Nama Barang","Jumlah","Satuan","Jumlah(sat.kcl)","Satuan Kecil"],
                colWidth:[[5,4,3,2,1,0],[100,100,100,100,300,100]],buttonStyle:[[0],[bsEllips]],colFormat:[[2,4],[cfNilai,cfNilai]],
                change:[this,"sg1onChange"],ellipsClick:[this,"doFindBtnClick"],colReadOnly:[true,[1],[]],rowCount:1});
	...
	doBtnClick: function(sender){
		try{
			if (sender == this.bGen){
				this.e0ip.setText(this.standarLib.noBuktiOtomatis(this.dbLib, "inv_sj_m", "no_sj", "SJ/"+this.ePrd.getText()+"/","0000"));
			}else{
