window.app_saku_inventory_transaksi_fJualk = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_transaksi_fJualk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_inventory_transaksi_fJualk";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Faktur Penjualan Barang: Koreksi", 0);	
		...
		this.cb_jenis = new portalui_saiCBBL(this,{bound:[20,18,200,20],caption:"Jenis Penjualan",btnClick:[this,"doBtnClick"],tag:2});		
		...
		this.sg = new portalui_saiGrid(this.p1,{bound:[0,20,895,245],colCount:11,tag:2,colTitle:["Kode","Nama","Tipe","Satuan","Harga","Stok","%Disc","Jumlah","Bonus","SubTtl","Akun Penjualan"],
		...
		try{			
			...
			var prd = this.dbLib.getDataProvider("select distinct periode from inv_jual_m where kode_lokasi = '"+this.app._lokasi+"'",true);
...
window.app_saku_inventory_transaksi_fJualk.extend(window.portalui_childForm);
window.app_saku_inventory_transaksi_fJualk.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					if (parseFloat(this.perLama) < parseFloat(this.app._periode)) {
						...
						sql.add(" update inv_jual_m set no_link='"+this.e_nb.getText()+"',no_del = concat(no_jual,'r') where no_jual ='"+this.cb_nbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
						sql.add(" insert into inv_jual_m (no_jual,kode_lokasi,tanggal,no_dokumen,keterangan,kode_cust,nik_buat,nilai,nilai_ppn,periode,nik_user,tgl_input,no_link,no_del,posted,kode_pp,kode_curr,akun_piutang,kode_gudang,kode_jenis)"+
								" select concat(no_jual,'r'),kode_lokasi,'"+this.dp_d1.getDateString()+"',no_dokumen,keterangan,kode_cust,'"+this.cb_app.getText()+"',nilai,nilai_ppn,'"+this.e_periode.getText()+"','"+this.app._userLog+"',now(),'-',no_jual,'F',kode_pp,kode_curr,akun_piutang,kode_gudang,kode_jenis "+ 
								" from inv_jual_m where no_jual = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");
						sql.add(" insert into inv_jual_j (no_jual,no_dokumen,tanggal,nu,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input)"+
								" select concat(no_jual,'r'),no_dokumen,'"+this.dp_d1.getDateString()+"',nu,kode_akun,keterangan,case dc when 'D' then 'C' else 'D' end as dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,'"+this.e_periode.getText()+"',kode_curr,kurs,'"+this.app._userLog+"',now() "+ 
								" from inv_jual_j where no_jual = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");
						sql.add(" insert into inv_dt (no_bukti,kode_lokasi,nu,kode_brg,harga,jumlah,bonus,modul,dc,periode,tanggal,kode_gudang,pdisk)"+
								...
								" from inv_dt where no_bukti = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' and modul = 'JUAL'");
						this.nb = this.e_nb.getText();
					}
					else{
						sql.add("delete from inv_jual_m where no_jual='"+this.cb_nbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from inv_jual_j where no_jual='"+this.cb_nbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from inv_dt where no_bukti ='"+this.cb_nbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' and modul = 'JUAL'");			
					...
					sql.add("insert into inv_jual_m(no_jual,kode_lokasi,tanggal,no_dokumen,keterangan,kode_cust,nik_buat,nilai,nilai_ppn,periode,nik_user,tgl_input,no_link,no_del,posted,kode_pp,kode_curr,akun_piutang,kode_gudang,kode_jenis) values" +
						    "('"+this.nb+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_desc.getText()+"','"+this.cb_cust.getText()+"','"+this.cb_app.getText()+"',"+parseNilai(this.e_total2.getText())+","+
							     parseNilai(this.e_ppn.getText())+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',now(),'-','-','F','"+this.app._kodePP+"','IDR','"+this.akunar+"','"+this.cb_gudang.getText()+"','"+this.cb_jenis.getText()+"')");
					...
					sql.add("insert into inv_jual_j(no_jual,no_dokumen,tanggal,nu,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values" +
							"('"+this.nb+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.akunar+"','"+this.e_desc.getText()+"','D',"+this.nilaipiu+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','INVJUAL','PIUJUAL','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())");
					this.createJurnal();
					...
					for (var i in this.dataJurnal.rs.rows){
						...
						if (line.nilai != 0) {
							if (s >0) d+=",";
							s++;
							d+="('"+this.nb+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+s+",'"+line.kode_akun+"','"+this.e_desc.getText()+"','"+line.dc+"',"+line.nilai+",'"+line.kode_pp+"','"+line.kode_drk+"','"+this.app._lokasi+"','INVJUAL','JUAL','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
						}
					}
					sql.add(d);			
					if (nilaiToFloat(this.e_ppn.getText()) != 0) {
						s++;
						sql.add("insert into inv_jual_j(no_jual,no_dokumen,tanggal,nu,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values" +
								"('"+this.nb+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+s+",'"+this.akunPPN+"','PPN','C',"+parseNilai(this.e_ppn.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','INVJUAL','PPNJUAL','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())");
					}
						...
						if (this.sg2.getRowValidCount() > 0){
							...
							for (var i=0;i < this.sg2.getRowCount();i++){
								if (this.sg2.rowValid(i)){
									...
									d += "('"+this.nb+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+idx+",'"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+DC+"',"+parseNilai(this.sg2.cells(4,i))+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','INVJUAL','BBNJUAL','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
									idx++;
								}
							}						
							sql.add(d);
		...
		switch (event){
			...
			case "hapus" : 
				...
				try{
					...
					var sql = new server_util_arrayList();
					if (parseFloat(this.perLama) < parseFloat(this.app._periode)) {
						sql.add(" update inv_jual_m set no_del = concat(no_jual,'r') where no_jual ='"+this.cb_nbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
						sql.add(" insert into inv_jual_m (no_jual,kode_lokasi,tanggal,no_dokumen,keterangan,kode_cust,nik_buat,nilai,nilai_ppn,periode,nik_user,tgl_input,no_link,no_del,posted,kode_pp,kode_curr,akun_piutang,kode_gudang,kode_jenis)"+
								" select concat(no_jual,'r'),kode_lokasi,'"+this.dp_d1.getDateString()+"',no_dokumen,keterangan,kode_cust,'"+this.cb_app.getText()+"',nilai,nilai_ppn,'"+this.e_periode.getText()+"','"+this.app._userLog+"',now(),'-',no_jual,'F',kode_pp,kode_curr,akun_piutang,kode_gudang,kode_jenis "+ 
								" from inv_jual_m where no_jual = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");
						sql.add(" insert into inv_jual_j (no_jual,no_dokumen,tanggal,nu,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input)"+
	...
	doLoadData:function(sender){
		if (this.cb_nbLama.getText() != "") {
			var data = this.dbLib.getDataProvider("select a.no_dokumen,a.keterangan,a.kode_gudang,b.nama as nama_gudang,a.kode_cust,c.nama as nama_cust,a.nik_buat,d.nama as nama_buat,a.nilai,a.nilai_ppn,a.posted,a.periode,a.kode_jenis,x.nama as nama_jenis,a.akun_piutang,z.akun_jual,"+
												  "       e.kode_brg,f.nama as nama_brg,f.tipe,f.sat,e.harga,(ifnull(y.jml,0)+e.jumlah+e.bonus) as stok,e.pdisk,e.jumlah,e.bonus,round(e.jumlah * (e.harga-(e.harga*e.pdisk/100))) as subttl "+
												  "from inv_jual_m a inner join inv_gudang  b on a.kode_gudang=b.kode_gudang and a.kode_lokasi=b.kode_lokasi "+
												  ...
												  "                  inner join inv_jual_jenis x on a.kode_jenis=x.kode_jenis and a.kode_lokasi=x.kode_lokasi  "+
												  ...
												  "                                   from inv_dt where kode_lokasi='"+this.app._lokasi+"' and periode<='"+this.e_periode.getText()+"' group by kode_gudang,kode_brg,kode_lokasi) y on e.kode_gudang=y.kode_gudang and e.kode_brg=y.kode_brg and e.kode_lokasi=y.kode_lokasi "+
												  "where a.no_jual = '"+this.cb_nbLama.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_del='-'");
			eval("data = "+data+";");
			if (typeof data == "object"){
				var line;
				this.sg.clear();
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData([line.kode_brg,line.nama_brg,line.tipe,line.sat,floatToNilai(line.harga),floatToNilai(line.stok),floatToNilai(line.pdisk),floatToNilai(line.jumlah),floatToNilai(line.bonus),floatToNilai(line.subttl),line.akun_jual]);
				}
				this.sg.validasi();
				if (line !== undefined){
			...
			var data = this.dbLib.getDataProvider("select g.nu,g.kode_akun,h.nama as nama_akun,g.keterangan,case g.dc when 'C' then '+' else '-' end as dc,g.nilai "+
												  "from inv_jual_j g inner join masakun h on g.kode_akun=h.kode_akun and g.kode_lokasi=h.kode_lokasi "+
												  "where g.modul='INVJUAL' and g.jenis='BBNJUAL' and "+
												  "      g.no_jual = '"+this.cb_nbLama.getText()+"' and g.kode_lokasi='"+this.app._lokasi+"' order by g.nu");
			eval("data = "+data+";");
			if (typeof data == "object"){
	...
	doBtnClick: function(sender, event){
		try
		{
			if (sender == this.cb_nbLama) {   
			    this.standarLib.showListData(this, "Daftar Faktur Penjualan",sender,undefined, 
											  "select no_jual, no_dokumen  from inv_jual_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and no_del='-'", 
											  "select count(no_jual) from inv_jual_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and no_del='-'",
											  ["no_jual","no_dokumen"],"and",["No Faktur","No Dokumen"],false);				
