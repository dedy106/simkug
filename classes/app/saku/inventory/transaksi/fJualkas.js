window.app_saku_inventory_transaksi_fJualkas = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_transaksi_fJualkas.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_inventory_transaksi_fJualkas";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penerimaan Penjualan [KB]: Input", 0);	
		...
		this.e_totpiu = new portalui_saiLabelEdit(this,{bound:[723,15,200,20],caption:"Total Piutang",tipeText:ttNilai,readOnly:true,text:"0"});
...
window.app_saku_inventory_transaksi_fJualkas.extend(window.portalui_childForm);
window.app_saku_inventory_transaksi_fJualkas.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,2])){
				try{									
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,"+
							...
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.akunkb+"','"+this.dp_d1.getDateString()+"','"+this.e_desc.getText()+"','"+this.app._kodePP+"','KBI_JUAL','"+this.jenis+"','"+this.e_periode.getText()+
						...
						if (this.sg.rowValid(i)){
							if (nilaiToFloat(this.sg.getCell(7,i)) != 0){
								...
								sql.add("insert into kas_d(no_kas,no_bukti,modul,catatan,no_del,kode_lokasi,nilai) values "+
										"                 ('"+this.e_nb.getText()+"','"+this.sg.cells(0,i)+"','INVJUAL','-','-','"+this.app._lokasi+"',"+parseNilai(this.sg.cells(7,i))+")");
							}
						}
					}
