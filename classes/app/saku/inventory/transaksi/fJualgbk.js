window.app_saku_inventory_transaksi_fJualgbk = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_transaksi_fJualgbk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_inventory_transaksi_fJualgbk";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penerimaan Penjualan [GB]: Koreksi", 0);	
		...
		this.cb_gb = new portalui_saiCBBL(this,{bound:[20,12,200,20],caption:"Akun Piutang GB",btnClick:[this,"doBtnClick"],change:[this,"doChange"], tag:1});
		...
		try{			
			...
			var prd = this.dbLib.getDataProvider("select distinct periode from gb_m where kode_lokasi = '"+this.app._lokasi+"' and jenis='JUAL' ",true);
...
window.app_saku_inventory_transaksi_fJualgbk.extend(window.portalui_childForm);
window.app_saku_inventory_transaksi_fJualgbk.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into gb_m (no_gb,kode_lokasi,no_dokumen,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,"+
							...
							"('"+this.nb+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.akunkb+"','"+this.dp_d1.getDateString()+"','"+this.e_desc.getText()+"','"+this.app._kodePP+"','GB','JUAL','"+this.e_periode.getText()+
						...
						if (this.sg.rowValid(i)){
							if (nilaiToFloat(this.sg.getCell(7,i)) != 0){
								...
								sql.add("insert into gb_d(no_gb,no_bukti,modul,catatan,no_del,kode_lokasi,nilai) values "+
										"                ('"+this.nb+"','"+this.sg.cells(0,i)+"','GBJUAL','-','-','"+this.app._lokasi+"',"+parseNilai(this.sg.cells(7,i))+")");
							}
						}
					}
		...
		if (this.cb_nbLama.getText() != "") {
			...
			if (typeof data == "object"){
				...
				var data = this.dbLib.getDataProvider("select y.no_retur, date_format(y.tanggal,'%d/%m/%Y') as tanggal,y.akun_hutang,y.keterangan,y.nilai "+
												      ...
												      "where x.kode_lokasi='"+this.app._lokasi+"' and x.modul='RETJUAL' and y.no_del='-' and x.no_gb = '"+this.cb_nbLama.getText()+"'");
				eval("data = "+data+";");
				if (typeof data == "object"){
		...
		try
		{
			if (sender == this.cb_nbLama) {   
			    this.standarLib.showListData(this, "Daftar Bukti GB",sender,undefined, 
											  "select no_gb, no_dokumen  from gb_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and jenis='JUAL' and no_del='-'", 
											  "select count(no_gb) from gb_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and jenis='JUAL' and no_del='-'",
