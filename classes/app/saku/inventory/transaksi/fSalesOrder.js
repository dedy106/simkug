...
//* 			SAI, PT											
//***********************************************************************************************
window.app_saku_inventory_transaksi_fSalesOrder = function(owner,options){
	try{
		if (owner)
		{
			window.app_saku_inventory_transaksi_fSalesOrder.prototype.parent.constructor.call(this, owner,options);			
			this.className = "app_saku_inventory_transaksi_fSalesOrder";
			this.maximize();
			this.app._mainForm.childFormConfig(this, "mainButtonClick","Sales Order",0);				
            this.initComponent();		
	...
	{
		alert("[app_saku_inventory_transaksi_fSalesOrder]::contruct:"+e,"");
	}
};
window.app_saku_inventory_transaksi_fSalesOrder.extend(window.portalui_childForm);
window.app_saku_inventory_transaksi_fSalesOrder.implement({
	initComponent: function(){		
		try{
			...
			this.e0ip = new portalui_saiCBBL(this,{bound:[10,54,250,20],caption:"No. Order",btnVisible:false});
			...
			this.ecustip = new portalui_saiCBBL(this,{bound:[10,120,200,20],caption:"Customer",btnClick:[this,"FindBtnClick"]});
			this.p1ip = new portalui_panel(this,{bound:[10,186,800,250],border:3,name:"P1",caption:"Barang"});
			this.sg1ip = new portalui_saiGrid(this.p1ip,{bound:[1,20,this.p1ip.width - 4,203],name:"saiSG1",colCount:7,colTitle:["Kode Barang","Nama","Satuan","Harga per Item","Jumlah","Diskon","Sub Total"],
                colWidth:[[6,5,4,3,2,1,0],[100,100,76,100,100,100,100]],buttonStyle:[[0,2],[bsEllips,bsAuto]],colFormat:[[3,4,5,6],[cfNilai, cfNilai, cfNilai, cfNilai]],
                change:[this,"sg1onChange"],ellipsClick:[this,"doFindBtnClick"],colReadOnly:[true,[0,1,2,3,6],[]],rowCount:1,picklist:[[2],[new portalui_arrayMap({items:["PCS","BALL","Roll","Yard","Kg"]})]]});
