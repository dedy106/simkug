window.app_saku_inventory_transaksi_fRetjkask = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_transaksi_fRetjkask.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_inventory_transaksi_fRetjkask";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengeluaran Retur Penjualan: Koreksi", 0);	
		...
		this.cb_cust = new portalui_saiCBBL(this,{bound:[20,18,200,20],caption:"Customer",btnClick:[this,"doBtnClick"],tag:1});
		...
		try{			
			...
			var prd = this.dbLib.getDataProvider("select distinct periode from kas_m where modul = 'KBO_RETJ' and kode_lokasi = '"+this.app._lokasi+"'",true);
...
window.app_saku_inventory_transaksi_fRetjkask.extend(window.portalui_childForm);
window.app_saku_inventory_transaksi_fRetjkask.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,"+
							...
							"('"+this.nb+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.akunkb+"','"+this.dp_d1.getDateString()+"','"+this.e_desc.getText()+"','"+this.app._kodePP+"','KBO_RETJ','"+this.jenis+"','"+this.e_periode.getText()+
					...
					for (var i=0;i < this.sg.getRowCount();i++){
						if (this.sg.rowValid(i)){
							if (this.sg.getCell(0,i) == "BAYAR"){
								...
								sql.add("update inv_jualretur_m set progress='1', no_kas='"+this.nb+"' where no_retur='"+this.sg.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
								sql.add("insert into kas_d(no_kas,no_bukti,modul,catatan,no_del,kode_lokasi,nilai) values "+
										"                 ('"+this.nb+"','"+this.sg.cells(1,i)+"','KBORETJ','-','-','"+this.app._lokasi+"',"+parseNilai(this.sg.cells(5,i))+")");
							}
						}
					}
	...
	doLoadData:function(sender){
		if (this.cb_nbLama.getText() != "") {
			var data = this.dbLib.getDataProvider("select a.no_dokumen,a.keterangan,a.kode_bank,b.nama as nama_bank,a.nik_buat,c.nama as nama_buat,a.ref1,d.nama as nama_cust,a.akun_kb,a.posted,a.periode,a.nilai,"+ 
			                                      "       y.no_retur, date_format(y.tanggal,'%d/%m/%Y') as tanggal,y.akun_hutang,y.keterangan,y.nilai "+
												  ...
												  "where x.kode_lokasi='"+this.app._lokasi+"' and x.modul='KBORETJ' and y.no_del='-' and x.no_kas = '"+this.cb_nbLama.getText()+"'");
			eval("data = "+data+";");
			if (typeof data == "object"){
				if (data.rs.rows[0] !== undefined) {
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg.appendData(["BAYAR",line.no_retur,line.akun_hutang,line.tanggal,line.keterangan,floatToNilai(line.nilai)]);
					}
					this.sg.validasi();
					if (line !== undefined){				
		...
		try{
			if (sender == this.cb_nbLama) {   
			    this.standarLib.showListData(this, "Daftar Bukti KasBank",sender,undefined, 
											  "select no_kas, no_dokumen  from kas_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and modul='KBO_RETJ' and no_del='-'", 
											  "select count(no_kas) from kas_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and modul='KBO_RETJ' and no_del='-'",
