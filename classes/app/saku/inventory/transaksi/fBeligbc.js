window.app_saku_inventory_transaksi_fBeligbc = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_transaksi_fBeligbc.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_inventory_transaksi_fBeligbc";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pencairan Hutang GB: Input", 0);	
		...
		this.e_tot2 = new portalui_saiLabelEdit(this,{bound:[723,16,200,20],caption:"Total Hutang",tipeText:ttNilai,readOnly:true,text:"0",tag:9});
...
window.app_saku_inventory_transaksi_fBeligbc.extend(window.portalui_childForm);
window.app_saku_inventory_transaksi_fBeligbc.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,2])){
				try{									
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,"+
							...
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.akunkb+"','"+this.dp_d1.getDateString()+"','"+this.e_desc.getText()+"','"+this.app._kodePP+"','KBO_GBC','BK','"+this.e_periode.getText()+
						...
						if (this.sg.rowValid(i)){
							if (this.sg.getCell(0,i) == "CAIR"){
								...
								sql.add("insert into kas_d(no_kas,no_bukti,modul,catatan,no_del,kode_lokasi,nilai) values "+
										"                 ('"+this.e_nb.getText()+"','"+this.sg.cells(1,i)+"','KBO_GBC','-','-','"+this.app._lokasi+"',"+parseNilai(this.sg.cells(8,i))+")");
		...
		try{			
			if (this.cb_akun.getText() != ""){
				var data = this.dbLib.getDataProvider("select a.no_gb,a.no_dokumen,date_format(a.tgl_terbit,'%d/%m/%Y') as tanggal, date_format(a.due_date,'%d/%m/%Y') as due_date,a.akun_gb,a.keterangan,b.nama,a.nilai "+
				                                      "from gb_m a inner join vendor b on a.kode_vendor=b.kode_vendor and a.kode_lokasi=b.kode_lokasi "+
													  "where a.kode_bank='"+this.cb_akun.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.periode<='"+this.e_periode.getText()+"' and progress='0' and a.jenis ='BELI' and a.no_del='-' and a.modul='GB' ");
				eval("data = "+data+";");
