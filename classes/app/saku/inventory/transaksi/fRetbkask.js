window.app_saku_inventory_transaksi_fRetbkask = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_transaksi_fRetbkask.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_inventory_transaksi_fRetbkask";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penerimaan Retur Pembelian: Koreksi", 0);	
		...
		this.cb_vend = new portalui_saiCBBL(this,{bound:[20,18,200,20],caption:"Vendor",btnClick:[this,"doBtnClick"],tag:1});
		...
		try{			
			...
			var prd = this.dbLib.getDataProvider("select distinct periode from kas_m where modul = 'KBI_RETB' and kode_lokasi = '"+this.app._lokasi+"'",true);
...
window.app_saku_inventory_transaksi_fRetbkask.extend(window.portalui_childForm);
window.app_saku_inventory_transaksi_fRetbkask.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,"+
							...
							"('"+this.nb+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.akunkb+"','"+this.dp_d1.getDateString()+"','"+this.e_desc.getText()+"','"+this.app._kodePP+"','KBI_RETB','"+this.jenis+"','"+this.e_periode.getText()+
					...
					for (var i=0;i < this.sg.getRowCount();i++){
						if (this.sg.rowValid(i)){
							if (this.sg.getCell(0,i) == "BAYAR"){
								...
								sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
										"kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+	
										"('"+this.nb+"','"+this.sg.cells(1,i)+"','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sg.cells(2,i)+
										"','Bayar retur penjualan','C',"+parseNilai(this.sg.cells(5,i))+",'"+this.app._kodePP+"','-','-',"+
										"'"+this.app._lokasi+"','KBI_RETB','RETBELI',"+
										"'"+this.e_periode.getText()+"','IDR',1"+
										",'"+this.app._userLog+"',now(),'-')");
								sql.add("update inv_beliretur_m set progress='1', no_kas='"+this.nb+"' where no_retur='"+this.sg.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
								sql.add("insert into kas_d(no_kas,no_bukti,modul,catatan,no_del,kode_lokasi,nilai) values "+
										"                 ('"+this.nb+"','"+this.sg.cells(1,i)+"','KBIRETB','-','-','"+this.app._lokasi+"',"+parseNilai(this.sg.cells(5,i))+")");
							}
						}
					}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :	
				...
				var temu = false;
				for (var i=0;i < this.sg.getRowCount();i++){
					if (this.sg.cells(0,i) == "BAYAR") temu = true; 
				...
				if (!temu) {
					system.alert(this,"Data tidak valid.","Harus ada record yang berstatus -BAYAR-");
					return false;   
				}
	...
	doLoadData:function(sender){
		if (this.cb_nbLama.getText() != "") {
			var data = this.dbLib.getDataProvider("select a.no_dokumen,a.keterangan,a.kode_bank,b.nama as nama_bank,a.nik_buat,c.nama as nama_buat,a.ref1,d.nama as nama_vendor,a.akun_kb,a.posted,a.periode,a.nilai,"+ 
			                                      "       y.no_retur, date_format(y.tanggal,'%d/%m/%Y') as tanggal,y.akun_piutang,y.keterangan,y.nilai "+
												  ...
												  "where x.kode_lokasi='"+this.app._lokasi+"' and x.modul='KBIRETB' and y.no_del='-' and x.no_kas = '"+this.cb_nbLama.getText()+"'");
			eval("data = "+data+";");
			if (typeof data == "object"){
				if (data.rs.rows[0] !== undefined) {
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg.appendData(["BAYAR",line.no_retur,line.akun_piutang,line.tanggal,line.keterangan,floatToNilai(line.nilai)]);
					}
					this.sg.validasi();
					if (line !== undefined){				
		...
		try{
			if (sender == this.cb_nbLama) {   
			    this.standarLib.showListData(this, "Daftar Bukti KasBank",sender,undefined, 
											  "select no_kas, no_dokumen  from kas_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and modul='KBI_RETB' and no_del='-'", 
											  "select count(no_kas) from kas_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and modul='KBI_RETB' and no_del='-'",
