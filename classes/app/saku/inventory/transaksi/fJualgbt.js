window.app_saku_inventory_transaksi_fJualgbt = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_transaksi_fJualgbt.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_inventory_transaksi_fJualgbt";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Tolakan GB Penjualan : Input", 0);	
		...
		this.cb_nbLama = new portalui_saiCBB(this,{bound:[20,19,270,20],caption:"No Bukti",readOnly:true,btnClick:[this,"doBtnClick"],btnRefreshClick:[this,"doLoadData"]});		
...
window.app_saku_inventory_transaksi_fJualgbt.extend(window.portalui_childForm);
window.app_saku_inventory_transaksi_fJualgbt.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into gb_m (no_gb,kode_lokasi,no_dokumen,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,"+
							...
							"      select '"+this.e_nb.getText()+"',kode_lokasi,no_dokumen,akun_kb,'"+this.dp_d1.getDateString()+"',keterangan,kode_pp,'GBT',concat('T',jenis) as jenis,"+
							"             '"+this.e_periode.getText()+"',kode_curr,kurs,nilai,'"+this.cb_app.getText()+"','-',now(),'"+this.app._userLog+"','F','-','-',akun_gb,kode_bank,progress,due_date,kode_cust,tgl_terbit,kode_vendor "+
		...
		try
		{
			if (sender == this.cb_nbLama) {   
			    this.standarLib.showListData(this, "Daftar Bukti GB",sender,undefined, 
											  "select no_gb, no_dokumen  from gb_m where kode_lokasi='"+this.app._lokasi+"' and periode<='"+this.e_periode.getText()+"' and jenis='JUAL' and no_del='-'", 
											  "select count(no_gb) from gb_m where kode_lokasi='"+this.app._lokasi+"' and periode<='"+this.e_periode.getText()+"' and jenis='JUAL' and no_del='-'",
