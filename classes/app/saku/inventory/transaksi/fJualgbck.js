window.app_saku_inventory_transaksi_fJualgbck = function(owner)
{
	if (owner)
	{
		window.app_saku_inventory_transaksi_fJualgbck.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_inventory_transaksi_fJualgbck";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pencairan Piutang GB: Koreksi", 0);	
		...
		this.e_tot2 = new portalui_saiLabelEdit(this,{bound:[723,16,200,20],caption:"Total Piutang",tipeText:ttNilai,readOnly:true,text:"0",tag:1});
		...
		try{			
			...
			var prd = this.dbLib.getDataProvider("select distinct periode from kas_m where modul = 'KBI_GBC' and kode_lokasi = '"+this.app._lokasi+"'",true);
...
window.app_saku_inventory_transaksi_fJualgbck.extend(window.portalui_childForm);
window.app_saku_inventory_transaksi_fJualgbck.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,"+
							...
							"('"+this.nb+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.akunkb+"','"+this.dp_d1.getDateString()+"','"+this.e_desc.getText()+"','"+this.app._kodePP+"','KBI_GBC','BM','"+this.e_periode.getText()+
						...
						if (this.sg.rowValid(i)){
							if (this.sg.getCell(0,i) == "CAIR"){
								...
								sql.add("insert into kas_d(no_kas,no_bukti,modul,catatan,no_del,kode_lokasi,nilai) values "+
										"                 ('"+this.nb+"','"+this.sg.cells(1,i)+"','KBI_GBC','-','-','"+this.app._lokasi+"',"+parseNilai(this.sg.cells(8,i))+")");
	...
	doLoadData:function(sender){
		if (this.cb_nbLama.getText() != "") {
			var data = this.dbLib.getDataProvider("select a.no_dokumen,a.keterangan,a.kode_bank,b.nama as nama_bank,a.nik_buat,c.nama as nama_buat,a.akun_kb,a.posted,a.periode,a.nilai, "+
												  ...
												  "                  inner join cust g on f.kode_cust=g.kode_cust and f.kode_lokasi=g.kode_lokasi  "+
		...
		try
		{
			if (sender == this.cb_nbLama) {   
			    this.standarLib.showListData(this, "Daftar Bukti KasBank",sender,undefined, 
											  "select no_kas, no_dokumen  from kas_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and modul='KBI_GBC' and no_del='-'", 
											  "select count(no_kas) from kas_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and modul='KBI_GBC' and no_del='-'",
