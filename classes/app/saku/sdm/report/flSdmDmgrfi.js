window.app_saku_sdm_report_flSdmDmgrfi = function(owner)
{
	if (owner)
	{
		window.app_saku_sdm_report_flSdmDmgrfi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_sdm_report_flSdmDmgrfi";
		this.maximize();
...
window.app_saku_sdm_report_flSdmDmgrfi.extend(window.portalui_childForm);
window.app_saku_sdm_report_flSdmDmgrfi.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku_sdm_report_flSdmDmgrfi]::mainButtonClick:"+e);
		}
	},
	...
	getSQL: function(jnsLap,filter,tipe){
		...
		if (jnsLap === "Pribadi"){
			if (tipe === 0){
                ...
                "        and cast(substring(date_sub(now(),interval substring(a.tgl_lahir,1,7) year_month),3,2) as decimal)<35 "+
                ...
                "      , count(sex) as col2 "+
                "    from karyawan a "+
                "        where cast(substring(date_sub(now(),interval substring(a.tgl_lahir,1,7) year_month),3,2) as decimal)>34 "+
                ...
                "        cast(substring(date_sub(now(),interval substring(a.tgl_masuk,1,7) year_month),3,2) as decimal)<=5 and a.kode_lokkonsol = '"+this.app._lokKonsol+"' and kode_lokasi = '"+this.sg1.getCell(2,0)+"' "+
                "    group by col1) union "+
                ...
                "        cast(substring(date_sub(now(),interval substring(a.tgl_masuk,1,7) year_month),3,2) as decimal)<=10 and a.kode_lokkonsol = '"+this.app._lokKonsol+"' and kode_lokasi = '"+this.sg1.getCell(2,0)+"' "+
                "    group by col1) union "+
                ...
                "      , count(sex) as col2 "+
                "    from karyawan a "+
                "        where cast(substring(date_sub(now(),interval substring(a.tgl_masuk,1,7) year_month),3,2) as decimal)>10 and a.kode_lokkonsol = '"+this.app._lokKonsol+"' and kode_lokasi = '"+this.sg1.getCell(2,0)+"' "+
                "    group by col1) union "+
                ...
                "    , sum(case when b.sex='laki-laki' then 1 else 0 end) as male "+
                "    , sum(case when b.sex='perempuan' then 1 else 0 end) as female "+
                "    , count(sex) as col2 "+
                "	from karyawan b "+
			...
			if (tipe === 0){
				sql="select a.nmcol1,a.col1,a.male,a.female,a.col2 from ( "+
				"	(select 'Status Pegawai' as nmcol1, c.nama as col1 "+
                "    , sum(case when b.sex='laki-laki' then 1 else 0 end) as male "+
                "    , sum(case when b.sex='perempuan' then 1 else 0 end) as female "+
                "    , count(sex) as col2 "+
                "    from karyawan b "+
				...
				"	(select 'Tenaga Perbantuan' as nmcol1, c.nama as col1 "+
                "    , sum(case when b.sex='laki-laki' then 1 else 0 end) as male "+
                "    , sum(case when b.sex='perempuan' then 1 else 0 end) as female "+
                "    , count(sex) as col2 "+
                "    from karyawan b "+
                ...
                "   where b.kode_lokkonsol = '"+this.app._lokKonsol+"' and b.kode_lokasi = '"+this.sg1.getCell(2,0)+"' group by col1) union "+
				"	(select 'Jabatan Struktural', c.nama as col1 "+
                "    , sum(case when b.sex='laki-laki' then 1 else 0 end) as male "+
                "    , sum(case when b.sex='perempuan' then 1 else 0 end) as female "+
                "    , count(sex) as col2 "+
                "   from karyawan b "+
                ...
                "   where b.kode_lokkonsol = '"+this.app._lokKonsol+"' and b.kode_lokasi = '"+this.sg1.getCell(2,0)+"' group by col1) union "+
				"	(select 'Jabatan Fungsional', c.nama as col1 "+
                "    , sum(case when b.sex='laki-laki' then 1 else 0 end) as male "+
                "    , sum(case when b.sex='perempuan' then 1 else 0 end) as female "+
                "    , count(sex) as col2 "+
                "   from karyawan b "+
			...
			if (tipe === 0){
				sql="select a.nmcol1,a.col1,a.male,a.female,a.col2 from ( "+
				"	(select 'Lokasi Kerja' as nmcol1,c.nama as col1 "+
                "      , sum(case when b.sex = 'laki-laki' then 1 else 0 end) as male "+
                "      , sum(case when b.sex = 'perempuan' then 1 else 0 end) as female "+
                "      , count(sex) as col2 "+
                "    from karyawan b "+
				"    left outer join hr_lokasi c on b.kode_lokasi=c.kode_lokasi "+
				"   where b.kode_lokkonsol = '"+this.app._lokKonsol+"' and b.kode_lokasi = '"+this.sg1.getCell(2,0)+"' group by col1) union "+
				"	(select 'Unit Kerja' as nmcol1,c.nama as col1 "+
                "      , sum(case when b.sex = 'laki-laki' then 1 else 0 end) as male "+
                "      , sum(case when b.sex = 'perempuan' then 1 else 0 end) as female "+
                "      , count(sex) as col2 "+
                "    from karyawan b "+
				"    left outer join hr_loker c on b.kode_loker=c.kode_loker and b.kode_lokasi=c.kode_lokasi "+
				"   where b.kode_lokkonsol = '"+this.app._lokKonsol+"' and b.kode_lokasi = '"+this.sg1.getCell(2,0)+"' group by col1) "+
				"	) as a";
			}else if (tipe ===1){
				sql="select 1 ";
