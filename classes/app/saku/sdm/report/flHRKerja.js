window.app_saku_sdm_report_flHRKerja = function(owner)
{
	if (owner)
	{
		window.app_saku_sdm_report_flHRKerja.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_sdm_report_flHRKerja";
		this.maximize();
...
window.app_saku_sdm_report_flHRKerja.extend(window.portalui_childForm);

window.app_saku_sdm_report_flHRKerja.prototype.doEllipseClick= function(sender, col, row)
{
		if (row == 0)
		{
...
window.app_saku_sdm_report_flHRKerja.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
...
window.app_saku_sdm_report_flHRKerja.prototype.mainButtonClick = function(sender)
{
	try
	{
	...
	{
		alert("[app_saku_sdm_report_flHRKerja]::mainButtonClick:"+e);
	}
}

window.app_saku_sdm_report_flHRKerja.prototype.doRequestReady = function(sender, methodName, result)
{
		switch (methodName)
		{
			case "preview" : 
...
window.app_saku_sdm_report_flHRKerja.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview("server_report_sdm_rptKerja",this.filter, page, 1, this.showFilter, this.app._namalokasi);		
	this.page=page;
}

window.app_saku_sdm_report_flHRKerja.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_sdm_report_flHRKerja.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
