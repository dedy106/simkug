window.app_saku_sdm_report_flDetBKes = function(owner)
{
	if (owner)
	{
		window.app_saku_sdm_report_flDetBKes.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_sdm_report_flDetBKes";
		this.maximize();		
...
window.app_saku_sdm_report_flDetBKes.extend(window.portalui_childForm);
window.app_saku_sdm_report_flDetBKes.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			}else
			{
				...
				if (this.sg1.getCell(2,4) === 'Detail')
					var sql = "select c.nama as nmprov,d.nama as nmpoli,e.nik,e.nama as nmpeg, "+
							...
							"b.status,date_format(b.tgl_masuk,'%e/%m/%Y') as tgl,b.nilai "+
							...
							"inner join karyawan e on b.nik=e.nik  "+
			...
			systemAPI.alert("[app_saku_sdm_report_flDetBKes]::mainButtonClick:"+e);
		}
	},
