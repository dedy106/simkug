window.app_saku_sdm_report_flSdmUker = function(owner)
{
	if (owner)
	{
		window.app_saku_sdm_report_flSdmUker.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_sdm_report_flSdmUker";
		this.maximize();
...
window.app_saku_sdm_report_flSdmUker.extend(window.portalui_childForm);
window.app_saku_sdm_report_flSdmUker.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 0)
	{
...
window.app_saku_sdm_report_flSdmUker.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
...
window.app_saku_sdm_report_flSdmUker.prototype.mainButtonClick = function(sender)
{
	try
	{
	...
	{
		alert("[app_saku_sdm_report_flSdmUker]::mainButtonClick:"+e);
	}
};
window.app_saku_sdm_report_flSdmUker.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_sdm_report_flSdmUker.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_saku_sdm_report_flSdmUker.prototype.previewReport = function(dthtml)
{
...
window.app_saku_sdm_report_flSdmUker.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_sdm_report_flSdmUker.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_sdm_report_flSdmUker.prototype.doRowPerPageChange = function(sender, rowperpage)
{
