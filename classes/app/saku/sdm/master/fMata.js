window.app_saku_sdm_master_fMata = function(owner)
{
	if (owner)
	{
		window.app_saku_sdm_master_fMata.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_sdm_master_fMata";
		this.maximize();
...
window.app_saku_sdm_master_fMata.extend(window.portalui_childForm);
window.app_saku_sdm_master_fMata.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_sdm_master_fMata.prototype.doModalResult = function(event, modalResult)
{			
	switch (event)
	{
		case "clear" :
		...
		case "simpan" :
			if (modalResult == mrOk)
			{
				...
				{					
					...
					var scr1 = "insert into kes_mata (nu,tingkat1,tingkat2,tarif,jenis,status,kode_lokkonsol) values ";
					var baris1 = true;
					for (var i=0; i < this.sg1.rows.getLength(); i++)
					{
						if (!baris1) { scr1 += ",";}	
						scr1 += "("+i+",'"+this.sg1.getCell(0,i)+"','"+parseNilai(this.sg1.getCell(1,i))+"',"+parseNilai(this.sg1.getCell(2,i))+",'"+this.sg1.getCell(3,i)+"','"+this.sg1.getCell(4,i)+"','"+this.lokkonsol+"')";
						baris1 = false;
					}	
					sql.add(scr1);
...
window.app_saku_sdm_master_fMata.prototype.doRequestReady = function(sender, methodName, result)
{
