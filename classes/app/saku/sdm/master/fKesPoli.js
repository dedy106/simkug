window.app_saku_sdm_master_fKesPoli = function(owner)
{
	if (owner)
	{
		window.app_saku_sdm_master_fKesPoli.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_sdm_master_fKesPoli";
...
window.app_saku_sdm_master_fKesPoli.extend(window.portalui_childForm);
window.app_saku_sdm_master_fKesPoli.implement({
			...
			if (this.standarLib.checkEmptyByTag(this,[0]))
			{
				try{
					...
					sql.add("update kes_poli set nama = '"+this.e_nama.getText()+"' "+
						    "where kode_lokasi='"+this.app._lokasi+"' and kode_poli = '"+this.cb_kode.getText()+"'");
					this.dbLib.execArraySQL(sql);
				}catch(e){
					system.alert(this, e,"");
				}
	...
	doTampilClick: function(sender){
		try{			
			...
			var data = this.dbLib.runSQL(" select kode_poli,nama from kes_poli where kode_lokasi='"+this.app._lokasi+"' ");
			this.sg1.clearAll();
		...
		try{
			if (sender == this.cb_kode){   
			    this.standarLib.showListData(this, "Daftar Poli",sender,undefined, 
											  "select kode_poli, nama  from kes_poli where kode_lokasi='"+this.app._lokasi+"' ",
											  "select count(kode_poli) from kes_poli where kode_lokasi='"+this.app._lokasi+"' ",
