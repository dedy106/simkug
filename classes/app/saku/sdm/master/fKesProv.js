window.app_saku_sdm_master_fKesProv = function(owner)
{
	if (owner)
	{
		window.app_saku_sdm_master_fKesProv.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_sdm_master_fKesProv";
...
window.app_saku_sdm_master_fKesProv.extend(window.portalui_childForm);
window.app_saku_sdm_master_fKesProv.implement({
			...
			if (this.standarLib.checkEmptyByTag(this,[0])){
				try{
					...
					sql.add("insert into kes_provider(kode_provider,kode_klp,kode_lokasi,nama,alamat,bank,cabang,no_rek,nama_rek,kota,no_telp,no_fax) values "+
						    "('"+this.cb_kode.getText()+"','"+this.cb_klp.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+
			...
			if (this.standarLib.checkEmptyByTag(this,[0]))
			{
				try{
					...
					sql.add("update kes_provider set nama = '"+this.e_nama.getText()+"', "+
						    ...
						    "' where kode_lokasi='"+this.app._lokasi+"' and kode_provider = '"+this.cb_kode.getText()+"'");
					this.dbLib.execArraySQL(sql);
				}catch(e){
					system.alert(this, e,"");
				}
		...
		try{
			if (this.cb_kode.getText() != ""){
				var data = this.dbLib.getDataProvider(" select a.*,b.nama as nmklp from kes_provider a inner join kes_klp_provider b on a.kode_klp=b.kode_klp and a.kode_lokasi=b.kode_lokasi "+
					       " where a.kode_lokasi='"+this.app._lokasi+"' and a.kode_provider ='"+this.cb_kode.getText()+"'");
				eval("data = "+data+";");
	...
	doTampilClick: function(sender){
		try{			
			...
			var data = this.dbLib.runSQL(" select kode_provider,kode_klp,nama,alamat,bank,cabang,no_rek,nama_rek,kota,no_telp,no_fax from kes_provider where kode_lokasi='"+this.app._lokasi+"' ");
			this.sg1.clearAll();
		...
		try{
			...
			if (sender == this.cb_kode){   
			    this.standarLib.showListData(this, "Daftar Provider",sender,undefined, 
											  ...
											  "select count(kode_provider) from kes_provider where kode_lokasi='"+this.app._lokasi+"' ",
