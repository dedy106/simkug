window.app_saku_sdm_transaksi_fHRrwytingkat = function(owner)
{
	if (owner)
	{
		window.app_saku_sdm_transaksi_fHRrwytingkat.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_sdm_transaksi_fHRrwytingkat";
		this.maximize();
...
window.app_saku_sdm_transaksi_fHRrwytingkat.extend(window.portalui_childForm);
window.app_saku_sdm_transaksi_fHRrwytingkat.prototype.sg1init = function(sg)
{
	sg.setColWidth([9,8,7,6,5,4,3,2,1,0],[80,190,110,80,80,80,80,80,80,120]);
};
window.app_saku_sdm_transaksi_fHRrwytingkat.prototype.sg1onDblClick = function(sender, col, row)
{
	try
	{		
...
window.app_saku_sdm_transaksi_fHRrwytingkat.prototype.doEditChange = function(sender)
{
...
window.app_saku_sdm_transaksi_fHRrwytingkat.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_sdm_transaksi_fHRrwytingkat.prototype.doModalResult = function(event, modalResult)
{			
	var stsAktif = "0";
...
window.app_saku_sdm_transaksi_fHRrwytingkat.prototype.show2Click = function(sender)
{
	if (this.cb_nik.getText() != "")
	{
		try
...
window.app_saku_sdm_transaksi_fHRrwytingkat.prototype.showClick = function(sender)
{
...
window.app_saku_sdm_transaksi_fHRrwytingkat.prototype.hitungClick = function(sender)
{
	try
	{
		if (this.cb_nik.getText() != "")
...
window.app_saku_sdm_transaksi_fHRrwytingkat.prototype.FindBtnClick = function(sender, event)
{
	try
	{
...
window.app_saku_sdm_transaksi_fHRrwytingkat.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_saku_sdm_transaksi_fHRrwytingkat.implement({
	doChange: function(sender){
		if (sender == this.ed_nosk){
			try{
				var data = this.dbLib.getDataProvider("select tgl_sk, tgl_mulai, tgl_berlaku, datediff(now(),tgl_mulai) as jmlhari  from hr_sk where kode_lokkonsol = '"+this.app._lokKonsol+"' and no_sk = '"+sender.getText()+"' and nik = '"+this.cb_nik.getText()+"' ",true);			
