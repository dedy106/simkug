window.app_saku_sdm_transaksi_fHRSk = function(owner)
{
	if (owner)
	{
		window.app_saku_sdm_transaksi_fHRSk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_sdm_transaksi_fHRSk";
		this.maximize();
...
window.app_saku_sdm_transaksi_fHRSk.extend(window.portalui_childForm);
window.app_saku_sdm_transaksi_fHRSk.implement({
			...
			case "simpan" :
				if (modalResult == mrOk)
				{
					...
					{					
						...
						var script = "insert into hr_sk_d(no_sk, nik, kode_param, value1, value2, kode_lokkonsol, no_dok)values ";	
						var first = true;
						for (var i=0; i < this.sg1.rows.getLength();i++){							
							if ( this.sg1.cells(0,i) != ""){
								if (!first) script += ",";
								script += "('"+this.ed_nosk.getText()+"','"+this.cb_nik.getText()+"','"+this.sg1.cells(0,i)+"','"+nilaiToFloat(this.sg1.cells(2,i))+"','"+nilaiToFloat(this.sg1.cells(3,i))+"'  ,'"+this.app._lokKonsol+"','"+this.cb_Dok.getText()+"' )";
								first = false;
							}
						}
						if (!first) sql.add(script);
						var script = "";
						if (this.cbAngkat.isSelected()) script+= "('"+this.ed_nosk.getText()+"','"+this.cb_nik.getText()+"','ANGKAT','"+this.app._lokasi+"','"+this.app._lokKonsol+"' )";					
						if (this.cbJabs.isSelected()) script+= (script != ""? ",":"") + "('"+this.ed_nosk.getText()+"','"+this.cb_nik.getText()+"','JABS','"+this.app._lokasi+"','"+this.app._lokKonsol+"' )";
						if (this.cbJabf.isSelected()) script+= (script != ""? ",":"") + "('"+this.ed_nosk.getText()+"','"+this.cb_nik.getText()+"','JABF','"+this.app._lokasi+"','"+this.app._lokKonsol+"' )";
						if (this.cbKenti.isSelected()) script+= (script != ""? ",":"") + "('"+this.ed_nosk.getText()+"','"+this.cb_nik.getText()+"','KENTI','"+this.app._lokasi+"','"+this.app._lokKonsol+"' )";
						if (this.cbPangti.isSelected()) script+= (script != ""? ",":"") + "('"+this.ed_nosk.getText()+"','"+this.cb_nik.getText()+"','PANGTI','"+this.app._lokasi+"','"+this.app._lokKonsol+"' )";
						if (this.cbGadas.isSelected()) script+= (script != ""? ",":"") + "('"+this.ed_nosk.getText()+"','"+this.cb_nik.getText()+"','GADAS','"+this.app._lokasi+"','"+this.app._lokKonsol+"' )";
						if (this.cbStatus.isSelected()) script+= (script != ""? ",":"") + "('"+this.ed_nosk.getText()+"','"+this.cb_nik.getText()+"','STATUS','"+this.app._lokasi+"','"+this.app._lokKonsol+"' )";
						if (this.cbProf.isSelected()) script+= (script != ""? ",":"") + "('"+this.ed_nosk.getText()+"','"+this.cb_nik.getText()+"','PROF','"+this.app._lokasi+"','"+this.app._lokKonsol+"' )";
						if (this.cbMutasi.isSelected()) script+= (script != ""? ",":"") + "('"+this.ed_nosk.getText()+"','"+this.cb_nik.getText()+"','MUTASI','"+this.app._lokasi+"','"+this.app._lokKonsol+"' )";
						if (this.cbBelajar.isSelected()) script+= (script != ""? ",":"") + "('"+this.ed_nosk.getText()+"','"+this.cb_nik.getText()+"','BELAJAR','"+this.app._lokasi+"','"+this.app._lokKonsol+"' )";
                        if (script != "") sql.add("insert into hr_sk_jenis(no_sk, nik, jenis, kode_lokasi, kode_lokkonsol)values"+script);
	...
	doChange: function(sender){				
	   if (sender == this.cb_Dok){
	       this.sg1.clear();
	       var data = this.dbLib.getDataProvider("select a.kode_param, b.nama, a.value1, a.value2 from hr_sk_d a inner join gaji_param_d b on b.kode_param = a.kode_param and b.kode_lokkonsol = a.kode_lokkonsol and b.no_dok = a.no_dok "+
				"	where a.nik = '"+this.cb_nik.getText()+"' and a.no_sk = '"+this.ed_nosk.getText()+"' and a.kode_lokkonsol = '"+this.app._lokKonsol+"' and b.no_dok = '"+sender.getText()+"'",true);
	...
	doClick: function(sender){
	   try{
    		var data = this.dbLib.getDataProvider("select tgl_sk, tgl_berlaku, tgl_mulai ,aktif from hr_sk where no_sk = '"+this.ed_nosk.getText()+"' and nik = '"+this.cb_nik.getText()+"' ",true);		
    		this.sg1.clear();
           ...
           this.cbBelajar.setSelected(false);
    		if (typeof data != "string" && data.rs.rows[0] !== undefined){
    			if (data.rs.rows[0] != "") {
    				...
    				sql.add("select a.kode_param, b.nama, a.value1, a.value2, a.no_dok from hr_sk_d a inner join gaji_param_d b on b.kode_param = a.kode_param and b.kode_lokkonsol = a.kode_lokkonsol and b.no_dok = a.no_dok "+
    				...
    				if (typeof data != "string"){	
						...
						this.cb_Dok.onChange.set(this,"doChange");
	...
	doEditChange: function(sender){
        if (sender == this.cb_nik){
            var data = this.dbLib.getDataProvider("select tgl_masuk from karyawan where nik='"+sender.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ",true);	   
            if (typeof data != "string"){
                if (data.rs.rows[0]){
