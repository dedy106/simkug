window.app_saku_sdm_transaksi_fHRKes = function(owner)
{
	if (owner)
	{
		window.app_saku_sdm_transaksi_fHRKes.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku_sdm_transaksi_fHRKes";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Input Kesehatan", 0);			
		this.maximize();		
		...
		this.sg1 = new portalui_saiGrid(this.p1,{bound:[0,20,900,200],colCount:6,
				colTitle:["NIK","Nama","Lembur","Tunj. Khusus","Rapel","K.Beban Kerj"],
				colFormat:[[2,3,4,5],[cfNilai, cfNilai,cfNilai,cfNilai]], colWidth:[[0,1,2,3,4,5],[100,100,100,100,100,100]],
				buttonStyle:[[0],[bsEllips]],
				ellipsClick:[this,"doFindBtnClick"], change:[this,"doSgChange"]});		
...
window.app_saku_sdm_transaksi_fHRKes.extend(window.portalui_childForm);
window.app_saku_sdm_transaksi_fHRKes.implement({
	doAfterUpload: function(sender, result, data){		
	    try{   
    		...
    		for (var i in data.rows){
    			...
    			for (var c in line) {
    			    field = c.toLowerCase(); 
    				if (field != "nik" && field != "nama" && field != "nama_pasien" && field != "status" && field != "tgl_masuk" && field != "tgl_keluar" && field != "provider"){
    					var value = line[c];
			...
			case "simpan" :
					try{
						...
						for (var c = 7;c < this.sg1.getColCount();c++){
							sqlArray[sqlArray.length]="insert into kes_trans_d(no_trans,kode_lokasi, nu,nik, status,nama,tgl_masuk,tgl_keluar,kode_provider, kode_poli,nilai, diskon, tgl_input, nik_user,periode,jenis )values";
							first.push(true);
						}
						var ix = 1;
						for (var i=0;i < this.sg1.getRowCount();i++){														
                            for (var c =7;c < this.sg1.getColCount();c++){
                                if (nilaiToFloat(this.sg1.cells(c,i)) > 0){
    								...
    								sqlArray[c - 7] += "('"+this.ed_nb.getText()+"','"+this.app._lokasi+"','"+ix+"','"+this.sg1.cells(0,i)+"',"+
                                        "'"+this.sg1.cells(2,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.getCellDateValue(4,i)+"','"+this.sg1.getCellDateValue(5,i)+"',"+
                                        "'"+this.sg1.cells(6,i)+"','"+fieldname[c]+"','"+parseNilai(this.sg1.cells(c,i))+"',0,now(),'"+this.app._usrLog+"','"+this.ed_period.getText()+"','"+this.cb_jenis.getText()+"') ";
    								first[c-7] = false;
    								ix++;
    							}														        							
							}
						...
						for (var i in sqlArray){						
                            if (sqlArray[i] != "insert into kes_trans_d(no_trans,kode_lokasi, nu,nik, status,nama,tgl_masuk,tgl_keluar,kode_provider, kode_poli,nilai, diskon, tgl_input, nik_user,periode,jenis )values")
                                sql.add(sqlArray[i]);
						}
	...
	doFindBtnClick: function(sender, col, row){
		...
		if (sender == this.cb_jenis){
	       this.standarLib.showListData(this,"Jenis Biaya Kesehatan",sender, undefined,
    			"select kode_klp, nama from kes_klp_provider where kode_lokasi ='"+this.app._lokasi+"' ",
    			"select count(*) from kes_klp_provider where kode_lokasi ='"+this.app._lokasi+"' ",
    			["kode_klp","nama"],"and",["Kode Jenis","Nama"],false);   
        }
		if (sender == this.sg1){
			switch (col){
				case 0 : this.standarLib.showListDataForSG(this, "Daftar Karyawan",this.sg1, this.sg1.row, this.sg1.col, 
													  "select nik, nama from karyawan where kode_lokkonsol = '"+this.app._lokKonsol+"' ",
													  "select count(*) from karyawan where kode_lokkonsol = '"+this.app._lokKonsol+"'",
													  ["nik","nama"],"and",["NIK","Nama"],false);
				break;
				case 2:
					this.standarLib.showListDataForSG(this, "Daftar Pasien",this.sg1, this.sg1.row, this.sg1.col, 					
										  "select status_family,nama       from hr_keluarga where kode_lokkonsol='"+this.app._lokKonsol+"' and nik = '"+this.sg1.cells(0,row)+"' and (status_family in ('Istri','Suami') or (status_family = 'Anak' and status_tanggungan = 'tanggungan'))" + " union " +
										  "select 'Pegawai' as status,nama from karyawan where kode_lokkonsol='"+this.app._lokKonsol+"' and nik = '"+this.sg1.cells(0,row)+"' ",
										  "select count(nama)         from karyawan where kode_lokkonsol='"+this.app._lokKonsol+"' and nik = '"+this.sg1.cells(0,row)+"'",
										  ["status_family","nama"],"and",["Status","Nama"],false);										  
				break;			
				case 6:
					this.standarLib.ListDataSGFilter(this, "Daftar Provider",this.sg1, this.sg1.row, this.sg1.col, 
										  "select kode_provider,nama from kes_provider where kode_lokasi ='"+this.app._lokasi+"' ",
										  "select count(*) from kes_provider where kode_lokasi ='"+this.app._lokasi+"' ",
										  ["kode_provider","nama"],"and",["Kode","Nama Provider"],false);
				break;
			}
		}
	},
	...
	initColumn: function(){
		try{
			var data = this.dbLib.getDataProvider("select distinct a.kode_poli, a.nama from kes_poli a "+
                ...
                " where a.kode_lokasi = '"+this.app._lokasi+"' ",true);//and c.kode_klp = '"+this.cb_jenis.getText()+"' 
			if (typeof data != "string"){			
				this.sg1.setColCount(data.rs.rows.length + 7);
				var title = ["nik","nama","status","nama_pasien","tgl_Masuk","tgl_keluar","provider"];	
                var hint = ["NIK","Nama","Status Pasien","Nama Pasien","Tgl Masuk","Tgl Keluar","Provider"];
				...
				this.sg1.setButtonStyle([0,2,6],[bsEllips, bsEllips, bsEllips]);
	...
	doSgChange: function(sender, col, row){
		if (col == 1){
			var dataProvider = this.dbLib.getDataProvider("select nik, nama from karyawan where kode_lokasi = '"+this.app._lokasi+"' and nik = '"+this.sg1.cells(0,row)+"' ",true);
			if (typeof dataProvider != "string"){
				var line,data = ["-",(new Date()).idFormat(),(new Date()).idFormat(),'-'];
				line = dataProvider.rs.rows[0];				
				var column = [3,4,5,6]; var ix = 0;
				for (var c in this.colTitle) {
					if (ix > 6){
						data.push(0);					
						column[column.length] = ix;
					}
					ix++;
				}
				this.sg1.editData(row,data,column);				
			}
		}
	}
});
