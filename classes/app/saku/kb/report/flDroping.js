window.app_saku_kb_report_flDroping = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flDroping.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flDroping";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Alokasi Droping", 2);
...
window.app_saku_kb_report_flDroping.extend(window.portalui_childForm);
window.app_saku_kb_report_flDroping.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
	...
	if (row == 2)
	{
		var sql1="select no_alok,keterangan from alokasi_m "+
		...
		var sql2="select count(no_alok) from alokasi_m "+
				 this.filterRep.filterStr("periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
				 this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
		this.filterRep.ListDataSGFilter(this, "Data Alokasi",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_alok","keterangan"),"and");
	}
};
window.app_saku_kb_report_flDroping.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
...
window.app_saku_kb_report_flDroping.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.filter=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					...
					this.filterRep.filterStr("a.no_alok",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
		this.nama_report="server_report_kb_rptDroping";
...
window.app_saku_kb_report_flDroping.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_report_flDroping.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter,page,this.pager,this.lokasi,this.showFilter);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_kb_report_flDroping.prototype.doCloseReportClick = function(sender)
{
  switch(sender.getName())
  {
    case "allBtn" :
	  this.allBtn = true;
	  this.page = 1;
	  this.report.preview(this.nama_report,this.filter,this.page, this.viewer.getTotalPage() * this.page, this.showFilter,this.lokasi);       
      break;
    case "pdfBtn" :
...
window.app_saku_kb_report_flDroping.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_kb_report_flDroping.prototype.doRowPerPageChange = function(sender, rowperpage)
{
