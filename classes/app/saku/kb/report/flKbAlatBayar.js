window.app_saku_kb_report_flKbAlatBayar = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flKbAlatBayar.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flKbAlatBayar";
		this.maximize();
...
window.app_saku_kb_report_flKbAlatBayar.extend(window.portalui_childForm);
window.app_saku_kb_report_flKbAlatBayar.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==0)
	{
	...
	if (row == 2)
	{
		var sql1="select no_kas,keterangan from kas_m "+
				 ...
				 " and modul='KBO_ALB' order by no_kas ";
		var sql2="select count(no_kas) from kas_m "+
				 this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
				 this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
				 " and modul='KBO_ALB' ";
		
		this.filterRep.ListDataSGFilter(this, "Data Bukti Kas",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_kas","keterangan"),"and");
	}
};
window.app_saku_kb_report_flKbAlatBayar.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_saku_kb_report_flKbAlatBayar.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.nama_report="server_report_kb_rptKbAlatBayar";
...
window.app_saku_kb_report_flKbAlatBayar.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_report_flKbAlatBayar.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.lokasi, this.showFilter);	
	this.page=page;
	this.allBtn = false;
};
window.app_saku_kb_report_flKbAlatBayar.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_kb_report_flKbAlatBayar.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_kb_report_flKbAlatBayar.prototype.doRowPerPageChange = function(sender, rowperpage)
{
