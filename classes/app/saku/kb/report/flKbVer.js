window.app_saku_kb_report_flKbVer = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flKbVer.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flKbVer";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Verifikasi KasBank Keluar", 2);
	...
	var periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from ver_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku_kb_report_flKbVer.extend(window.portalui_childForm);
window.app_saku_kb_report_flKbVer.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==0)
	{
	...
	if (row == 2)
	{
		var sql1="select no_ver,keterangan from ver_m "+
				 this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
				 this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
				 " order by no_ver ";
		var sql2="select count(no_ver) from ver_m "+
				 this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
				 this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
		
		this.filterRep.ListDataSGFilter(this, "Data Verifikasi",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_ver","keterangan"),"and");
	}
};
window.app_saku_kb_report_flKbVer.prototype.doSelectCell = function(sender, col, row)
{
	...
	if (row == 1)
	{
		...
		this.dbLib.setItemsFromSQL("select distinct periode from ver_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
	}
};
window.app_saku_kb_report_flKbVer.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					...
					this.filterRep.filterStr("a.no_ver",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
		this.nama_report="server_report_kb_rptKbVer";
...
window.app_saku_kb_report_flKbVer.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_report_flKbVer.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.lokasi, this.showFilter);	
	this.page=page;
	this.allBtn = false;
};
window.app_saku_kb_report_flKbVer.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_kb_report_flKbVer.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_kb_report_flKbVer.prototype.doRowPerPageChange = function(sender, rowperpage)
{
