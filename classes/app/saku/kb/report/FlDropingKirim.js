window.app_saku_kb_report_flDropingKirim = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flDropingKirim.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flDropingKirim";
		this.maximize();
		this.app._mainForm.childFormConfig(this,"mainButtonClick","Laporan Droping Dana Kirim", 2);
		alert('aaaa');
...
window.app_saku_kb_report_flDropingKirim.extend(window.portalui_childForm);
window.app_saku_kb_report_flDropingKirim.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
...
window.app_saku_kb_report_flDropingKirim.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
...
window.app_saku_kb_report_flDropingKirim.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.nama_report="server_report_kb_rptDropingKirim";
...
window.app_saku_kb_report_flDropingKirim.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_report_flDropingKirim.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter,page,this.pager,this.lokasi,this.showFilter);	
	this.page=page;
	this.allBtn = false;
};
window.app_saku_kb_report_flDropingKirim.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_kb_report_flDropingKirim.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_kb_report_flDropingKirim.prototype.doRowPerPageChange = function(sender, rowperpage)
{
