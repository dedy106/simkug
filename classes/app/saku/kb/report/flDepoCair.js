window.app_saku_kb_report_flDepoCair = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flDepoCair.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flDepoCair";
		this.maximize();
...
window.app_saku_kb_report_flDepoCair.extend(window.portalui_childForm);
window.app_saku_kb_report_flDepoCair.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
...
window.app_saku_kb_report_flDepoCair.prototype.doSelectCell = function(sender, col, row)
{	
...
window.app_saku_kb_report_flDepoCair.prototype.mainButtonClick = function(sender)
{
	try
	{
...
window.app_saku_kb_report_flDepoCair.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_report_flDepoCair.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.lokasi, this.showFilter,this.filter2);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_kb_report_flDepoCair.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_kb_report_flDepoCair.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_kb_report_flDepoCair.prototype.doRowPerPageChange = function(sender, rowperpage)
{
