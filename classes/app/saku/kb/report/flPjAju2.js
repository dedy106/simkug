window.app_saku_kb_report_flPjAju2 = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flPjAju2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flPjAju2";
		this.maximize();
	...
	this.getPeriode="select distinct periode from panjar_m where kode_lokasi ='"+this.lokasi+"' order by periode desc ";
	this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Kode Lokasi",this.tanda,this.lokasi]);
	this.gridLib.SGEditData(this.sg1,1,[0,1,2],["Periode","=",this.app._periode/*this.dbLib.getPeriodeFromSQL(this.getPeriode)*/]);
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Akun Panjar","All",""]);
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["No. Bukti","All",""]);
	this.pager = 1;
};
window.app_saku_kb_report_flPjAju2.extend(window.portalui_childForm);
window.app_saku_kb_report_flPjAju2.implement({
	...
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			...
			this.dbLib.setItemsFromSQL("select distinct periode from panjar_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' order by periode desc ",[this.sg1.columns.get(2).pickList]);
		}
	},
	mainButtonClick: function(sender)
	{
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_kb_rptPjAjuG";
