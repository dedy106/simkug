window.app_saku_kb_report_flIfAju = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flIfAju.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flIfAju";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Pengajuan Imprest Fund", 2);
	...
	this.getPeriode="select distinct periode from if_m where kode_lokasi ='"+this.lokasi+"'";
	...
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Jenis Laporan","=","All"]);
	this.pager = 1;
};
window.app_saku_kb_report_flIfAju.extend(window.portalui_childForm);
window.app_saku_kb_report_flIfAju.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row === 2)
		{
			...
			var sql2="select count(no_if) from if_m "+
					 this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where")+
					 this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and");
			this.filterRep.ListDataSGFilter(this, "Data Imprest Fund",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_if","keterangan"),"and",["No. Bukti","Keterangan"]);
		}
	},
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			...
			this.dbLib.setItemsFromSQL("select distinct periode from if_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 3)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender)
	{
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,3) === "All"){
					this.nama_report="server_report_kb_rptIfAju";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.no_if",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
					this.filter2="";
					this.status=true;
				}else if (this.sg1.getCell(2,3) === "SPP"){
					this.nama_report="server_report_kb_rptIfAju";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.no_if",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
					...
					this.nama_report="server_report_kb_rptIfAju2";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.no_if",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
					this.filter2="";
			...
			case "preview" : 
				if (this.sg1.getCell(2,3) === "All"){
					if (this.status){
						...
						this.report.preview("server_report_kb_rptIfAju2",this.filter,this.page,1, this.showFilter,this.app._lokasi,this.filter2);
