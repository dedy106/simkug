window.app_saku_kb_report_flKbMasuk = function(owner)
{
	try{
		if (owner)
		{
			window.app_saku_kb_report_flKbMasuk.prototype.parent.constructor.call(this,owner);
			this.className = "app_saku_kb_report_flKbMasuk";
			this.maximize();
			this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan KasBank Masuk", 2);
	...
	{
		alert("app_saku_kb_report_flKbMasuk : "+e);
	}
};
window.app_saku_kb_report_flKbMasuk.extend(window.portalui_childForm);
window.app_saku_kb_report_flKbMasuk.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==0)
	{
	...
	if (row == 2)
	{
		this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,
						  ...
						  "where a.block= '0' and (b.kode_flag='001' or b.kode_flag='009') "+
						  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," and "),
						  new Array("a.kode_akun","a.nama"),"and",new Array("Kode Akun","Nama Akun"));
	}
	if (row == 3)
	{
		...
		var sql1="select no_kas,keterangan from kas_m "+filter+" and modul='KBI_NP' order by no_kas";
		var sql2="select count(no_kas) from kas_m  "+filter+" and modul='KBI_NP' ";
		alert(sql1);
...
window.app_saku_kb_report_flKbMasuk.prototype.doSelectCell = function(sender, col, row)
{	
	...
	if (this.userStatus=="A"){
		...
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["3","3","123","123"]);
...
window.app_saku_kb_report_flKbMasuk.prototype.mainButtonClick = function(sender)
{
	try
	{
		if (sender === this.app._mainForm.bClear2)
		{
			...
			this.nama_report="server_report_kb_rptKbMasuk";
...
window.app_saku_kb_report_flKbMasuk.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_report_flKbMasuk.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.lokasi, this.showFilter,this.filter2);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_kb_report_flKbMasuk.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_kb_report_flKbMasuk.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_kb_report_flKbMasuk.prototype.doRowPerPageChange = function(sender, rowperpage)
{
