window.app_saku_kb_report_flKBIn = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flKBIn.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flKBIn";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Jenis Laporan","=","Bank Masuk"]);
	this.pager = 1;
};
window.app_saku_kb_report_flKBIn.extend(window.portalui_childForm);
window.app_saku_kb_report_flKBIn.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row === 2)
		{
			if (this.sg1.getCell(2,3) === "Bank Masuk") 
				var jns="BANK";
			else var jns="KAS";
			var sql1="select no_kas,keterangan from kas_m "+
					 ...
					 " and modul='KBO_ALL' and jenis='"+jns+"' ";
	...
	doSelectCell: function(sender, col, row)
	{
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["Bank Masuk","Kas Masuk"]);
		}
	},
	mainButtonClick: function(sender)
	{
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,3) === "Bank Masuk"){
					this.nama_report="server_report_kb_rptBankIn";
					...
					this.nama_report="server_report_kb_rptKasIn";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.no_kas",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
					this.filter2="";
