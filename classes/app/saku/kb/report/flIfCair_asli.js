window.app_saku_kb_report_flIfCair = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flIfCair.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flIfCair";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Pencairan Imprest Fund", 2);
...
window.app_saku_kb_report_flIfCair.extend(window.portalui_childForm);
window.app_saku_kb_report_flIfCair.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
	...
	if (row == 3)
	{
		...
		var sql1="select no_kas,keterangan from kas_m "+filter+" and modul='IFD_NP' order by no_kas";
...
window.app_saku_kb_report_flIfCair.prototype.doSelectCell = function(sender, col, row)
{	
...
window.app_saku_kb_report_flIfCair.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.filter=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					...
					" and a.modul='IFD_NP' ";
		this.nama_report="server_report_kb_rptIfCair";
...
window.app_saku_kb_report_flIfCair.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_report_flIfCair.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.lokasi, this.showFilter,this.filter2);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_kb_report_flIfCair.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_kb_report_flIfCair.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_kb_report_flIfCair.prototype.doRowPerPageChange = function(sender, rowperpage)
{
