window.app_saku_kb_report_flPjCair = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flPjCair.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flPjCair";
		this.maximize();
	...
	this.filterRep.setSGFilterRowTipe(this.sg1, 0,new Array(0,1,2,3,4,5),new  Array("123","123","123","123","123","123"));
...
window.app_saku_kb_report_flPjCair.extend(window.portalui_childForm);
window.app_saku_kb_report_flPjCair.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
	...
	if (row == 3)
	{
		...
		var sql1="select no_kas,keterangan from kas_m "+filter+" and modul='PJR_NP' order by no_kas";
		var sql2="select count(no_kas) from kas_m  "+filter+" and modul='PJR_NP'";
...
window.app_saku_kb_report_flPjCair.prototype.doSelectCell = function(sender, col, row)
{	
...
window.app_saku_kb_report_flPjCair.prototype.mainButtonClick = function(sender)
{
	try
	{
...
window.app_saku_kb_report_flPjCair.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_report_flPjCair.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.lokasi, this.showFilter,this.filter2);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_kb_report_flPjCair.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_kb_report_flPjCair.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_kb_report_flPjCair.prototype.doRowPerPageChange = function(sender, rowperpage)
{
