window.app_saku_kb_report_flSpbIF = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flSpbIF.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flSpbIF";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan SPB Imprest Fund", 2);
...
window.app_saku_kb_report_flSpbIF.extend(window.portalui_childForm);
window.app_saku_kb_report_flSpbIF.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row === 2)
		{
			...
			var sql2="select count(no_ifptg) from ifptg_m "+
					 ...
					 this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and")+
					 " and modul='IFPTG_P' ";
			this.filterRep.ListDataSGFilter(this, "Data Reimburs",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_ifptg","keterangan"),"and",["No. Bukti","Keterangan"]);
		}
	},
	doSelectCell: function(sender, col, row)
	{
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["All","Reimburs","Lampiran Reimburs"]);
		}
	},
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,3) === "All"){
					...
					this.filter2="";
					this.status=true;
				}else if (this.sg1.getCell(2,3) === "Reimburs"){
					this.nama_report="server_report_kopeg_rptSpbIF";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.no_ifptg",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
					...
					this.nama_report="server_report_kopeg_rptSpbIF2";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.no_ifptg",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
					this.filter2="";
			...
			case "preview" : 
				if (this.sg1.getCell(2,3) === "All"){
					if (this.status){
						...
						this.report.preview("server_report_kopeg_rptSpbIF2",this.filter,this.page,1, this.showFilter,this.app._lokasi,this.filter2);
