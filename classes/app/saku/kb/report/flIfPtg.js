window.app_saku_kb_report_flIfPtg = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flIfPtg.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flIfPtg";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Pertanggungan Imprest Fund", 2);
	...
	this.filterRep.setSGFilterRowTipe(this.sg1, 0,[0,1,2,3,4],["123","123","123","3","123"]);
	this.filterRep.setSGFilterRowButtonStyle(this.sg1, 0,[0,1,2,3,4],[0,2,2,0,2]);
	...
	this.gridLib.SGEditData(this.sg1,5,[0,1,2],["Unit/Bagian","All",""]);
...
window.app_saku_kb_report_flIfPtg.extend(window.portalui_childForm);
window.app_saku_kb_report_flIfPtg.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==0)
	{
	...
	if (row == 4)
	{
		if (this.sg1.getCell(2,3)=="CLOSING")
		{
			...
			var sql1="select no_kas,keterangan from kas_m "+filter+" and modul='IFPTG_P' order by no_kas";
		...
		else
		{
			...
			var sql1="select no_ifptg,keterangan from ifptg_m "+filter+" and modul='IFPTG_P' order by no_ifptg";
			var sql2="select count(no_ifptg) from ifptg_m  "+filter;
			this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_ifptg","keterangan"),"and",new Array("No Bukti","Keterangan"));
		}
	}
};
window.app_saku_kb_report_flIfPtg.prototype.doSelectCell = function(sender, col, row)
{	
	this.sg1.columns.get(col).setReadOnly(false);
	if (this.userStatus=="A")
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5),new  Array("123","3","123","3","123","123"));
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5),new  Array(2,0,2,0,2,2));
	}
	else
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5),new  Array("3","3","123","3","123","123"));
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5),new  Array(3,0,2,0,2,2));
...
window.app_saku_kb_report_flIfPtg.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		if (this.sg1.getCell(2,3)=="CLOSING")
		{
			this.filter=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
						...
						this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and")+
						" and a.modul='IFPTG_P' ";
		}else
		{
			this.filter=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
					this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and")+
						...
						this.filterRep.filterStr("a.no_ifptg",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
						this.filterRep.filterStr("d.kode_pp",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and")+
						" and a.modul='IFPTG_P' ";
		}
		this.nama_report="server_report_kb_rptIfPtg";
...
window.app_saku_kb_report_flIfPtg.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_report_flIfPtg.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.lokasi, this.showFilter,this.filter2);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_kb_report_flIfPtg.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_kb_report_flIfPtg.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_kb_report_flIfPtg.prototype.doRowPerPageChange = function(sender, rowperpage)
{
