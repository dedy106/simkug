window.app_saku_kb_report_flDepoBunga = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flDepoBunga.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flDepoBunga";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Penerimaan Bunga Deposito", 2);
		...
		this.sg1.setColCount(4);
			...
			this.sg1.setCell(0,2,"Akun KasBank");
			this.sg1.setCell(0,3,"No Bukti");
	...
	this.filterRep.setSGFilterRowTipe(this.sg1, 0,new Array(0,1,2,3),new  Array("123","123","123","123"));
	this.filterRep.setSGFilterRowButtonStyle(this.sg1, 0,new Array(0,1,2,3),new  Array(0,2,2,2));
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Akun KasBank","All",""));
...
window.app_saku_kb_report_flDepoBunga.extend(window.portalui_childForm);
window.app_saku_kb_report_flDepoBunga.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
	...
	if (row == 3)
	{
		...
		var sql1="select no_kas,keterangan from kas_m "+filter+" and modul='DEPC_NP' order by no_kas";
		var sql2="select count(no_kas) from kas_m  "+filter;
	
		this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_kas","keterangan"),"and",new Array("No Kas","Keterangan"));
	}
};
window.app_saku_kb_report_flDepoBunga.prototype.doSelectCell = function(sender, col, row)
{	
	this.sg1.columns.get(col).setReadOnly(false);
	if (this.userStatus=="A")
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3),new  Array("23","123","123","123"));
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3),new  Array(0,2,2,2));
	}
	else
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3),new  Array("23","3","123","123"));
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3),new  Array(0,3,2,2));
...
window.app_saku_kb_report_flDepoBunga.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.filter=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					...
					" and a.modul='DEPC_NP' ";
		this.nama_report="server_report_kb_rptDepoCair";
...
window.app_saku_kb_report_flDepoBunga.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_report_flDepoBunga.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.lokasi, this.showFilter,this.filter2);	
	this.page=page;
	this.allBtn = false;
};
window.app_saku_kb_report_flDepoBunga.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_kb_report_flDepoBunga.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_kb_report_flDepoBunga.prototype.doRowPerPageChange = function(sender, rowperpage)
{
