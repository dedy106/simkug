window.app_saku_kb_report_flPjPtg = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flPjPtg.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flPjPtg";
		this.maximize();
		this.app._mainForm.childFormConfig(this,"mainButtonClick","Laporan Pertanggungan Panjar",2);
...
window.app_saku_kb_report_flPjPtg.extend(window.portalui_childForm);
window.app_saku_kb_report_flPjPtg.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==0)
	{
	...
	if (row == 3)
	{
		...
		var sql1="select no_ptg,keterangan from ptg_m "+filter+" and modul='PTG_P' order by no_ptg";
		var sql2="select count(no_ptg) from ptg_m  "+filter+" and modul='PTG_P'";
		
		this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_ptg","keterangan"),"and",new Array("No Kas","Keterangan"));
	}
};
window.app_saku_kb_report_flPjPtg.prototype.doSelectCell = function(sender, col, row)
{	
	this.sg1.columns.get(col).setReadOnly(false);
	if (this.userStatus=="A")
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4),new  Array("123","23","123","123","3"));
...
window.app_saku_kb_report_flPjPtg.prototype.mainButtonClick = function(sender)
{
	try
	{
...
window.app_saku_kb_report_flPjPtg.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_report_flPjPtg.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.lokasi, this.showFilter,this.filter2);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_kb_report_flPjPtg.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_kb_report_flPjPtg.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_kb_report_flPjPtg.prototype.doRowPerPageChange = function(sender, rowperpage)
{
