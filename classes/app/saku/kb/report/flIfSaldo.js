window.app_saku_kb_report_flIfSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flIfSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flIfSaldo";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Saldo Imprest Fund", 2);
...
window.app_saku_kb_report_flIfSaldo.extend(window.portalui_childForm);
window.app_saku_kb_report_flIfSaldo.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
...
window.app_saku_kb_report_flIfSaldo.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_saku_kb_report_flIfSaldo.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		if (this.sg1.getCell(2,3)=="Per Akun")
		{
			...
			this.nama_report="server_report_kb_rptIfSaldo";
		}
...
window.app_saku_kb_report_flIfSaldo.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_report_flIfSaldo.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter,page,this.pager, this.showFilter,this.lokasi, this.filter2);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_kb_report_flIfSaldo.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_kb_report_flIfSaldo.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_kb_report_flIfSaldo.prototype.doRowPerPageChange = function(sender, rowperpage)
{
