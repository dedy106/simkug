window.app_saku_kb_report_flDepoTempat = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flDepoTempat.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flDepoTempat";
		this.maximize();
...
window.app_saku_kb_report_flDepoTempat.extend(window.portalui_childForm);
window.app_saku_kb_report_flDepoTempat.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
	...
	if (row == 3)
	{
		...
		var sql1="select no_kas,keterangan from kas_m "+filter+" and modul='DEPO_NP' order by no_kas";
...
window.app_saku_kb_report_flDepoTempat.prototype.doSelectCell = function(sender, col, row)
{	
...
window.app_saku_kb_report_flDepoTempat.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.filter=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					...
					" and a.modul='DEPO_NP' ";
		this.nama_report="server_report_kb_rptDepoTempat";
...
window.app_saku_kb_report_flDepoTempat.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_report_flDepoTempat.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.lokasi, this.showFilter,this.filter2);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_kb_report_flDepoTempat.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_kb_report_flDepoTempat.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_kb_report_flDepoTempat.prototype.doRowPerPageChange = function(sender, rowperpage)
{
