window.app_saku_kb_report_flDepoAkru = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flDepoAkru.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flDepoAkru";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Pengakuan Bunga Deposito", 2);
	...
	this.filterRep.setSGFilterRowTipe(this.sg1, 0,new Array(0,1,2),new  Array("123","123","123"));
...
window.app_saku_kb_report_flDepoAkru.extend(window.portalui_childForm);
window.app_saku_kb_report_flDepoAkru.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
	...
	if (row == 2)
	{
		...
		var sql1="select no_akru,keterangan from depo_akru_m "+filter+" and modul='DEPO_AKRU' order by no_akru";
		var sql2="select count(no_akru) from depo_akru_m "+filter+" and modul='DEPO_AKRU'";
	
		this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_akru","keterangan"),"and",new Array("No Akru","Keterangan"));
	}
};
window.app_saku_kb_report_flDepoAkru.prototype.doSelectCell = function(sender, col, row)
{	
	this.sg1.columns.get(col).setReadOnly(false);
	if (this.userStatus=="A")
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2),new  Array("23","123","123"));
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2),new  Array(0,2,2));
	}
	else
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2),new  Array("23","3","123"));
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2),new  Array(0,3,2));
...
window.app_saku_kb_report_flDepoAkru.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.filter=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and") +
					this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
					this.filterRep.filterStr("a.no_akru",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
		this.nama_report="server_report_kb_rptDepoAkru";
...
window.app_saku_kb_report_flDepoAkru.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_report_flDepoAkru.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.lokasi, this.showFilter,this.filter2);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_kb_report_flDepoAkru.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_kb_report_flDepoAkru.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_kb_report_flDepoAkru.prototype.doRowPerPageChange = function(sender, rowperpage)
{
