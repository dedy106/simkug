window.app_saku_kb_report_flDropingTerima = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flDropingTerima.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flDropingTerima";
		this.maximize();
		this.app._mainForm.childFormConfig(this,"mainButtonClick","Laporan Droping Dana Terima",2);
...
window.app_saku_kb_report_flDropingTerima.extend(window.portalui_childForm);
window.app_saku_kb_report_flDropingTerima.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
	...
	if (row == 2)
	{
		var sql1="select no_terima,keterangan from droptrm_m "+
		...
		var sql2="select count(no_terima) from droptrm_m "+
				 this.filterRep.filterStr("periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
				 this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
		this.filterRep.ListDataSGFilter(this, "Data Droping Kirim",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_terima","keterangan"),"and");
	}
};
window.app_saku_kb_report_flDropingTerima.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
...
window.app_saku_kb_report_flDropingTerima.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.filter=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					...
					this.filterRep.filterStr("a.no_terima",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
		this.nama_report="server_report_kb_rptDropingTerima";
...
window.app_saku_kb_report_flDropingTerima.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_report_flDropingTerima.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter,page,this.pager,this.lokasi,this.showFilter);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_kb_report_flDropingTerima.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_kb_report_flDropingTerima.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_kb_report_flDropingTerima.prototype.doRowPerPageChange = function(sender, rowperpage)
{
