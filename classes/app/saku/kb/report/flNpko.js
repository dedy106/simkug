window.app_saku_kb_report_flNpko = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flNpko.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flNpko";
		this.maximize();
		...
		this.sg1.onEllipsClick.set(this, "doEllipseClick");
			...
			this.sg1.setCell(0,5,"Nomor NPKO");
	...
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1), new Array("Nomor NPKO","All"));
	this.sg1.onChange.set(this, "sg1onChange");
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku_kb_report_flNpko.extend(window.portalui_childForm);
window.app_saku_kb_report_flNpko.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
	...
	if (row == 5)
	{
		this.filterRep.ListDataSGFilter(this, "Data NPKO",this.sg1, this.sg1.row, this.sg1.col,
												  "select no_npko,uraian from npko order by no_npko ",								   
												  "select count(no_npko) from npko ",									 
												  new Array("no_npko","nama"),"where");
	}
};
window.app_saku_kb_report_flNpko.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_saku_kb_report_flNpko.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.filter = this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					  ...
					  this.filterRep.filterStr("a.kode_unit",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
					  this.filterRep.filterStr("a.kode_drk",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,5),"and")+
					  this.filterRep.filterStr("a.no_npko",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");
		this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(2,5);
		this.showFilter = this.filterRep.showFilter(this.sg1);
		this.nama_report="server_report_kb_rptNpko";
	...
	{
		alert("[flNpko]::mainButtonClick:"+e);
	}
};
window.app_saku_kb_report_flNpko.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_report_flNpko.prototype.doSelectedPage = function(sender, page)
{
	...
	{
		this.report.preview("server_report_kb_rptNpkoStafel",this.filter, page,this.pager, "YAKES", this.showFilter,this.filter2);	
	}
	this.page=page;
	this.allBtn = false;
};
window.app_saku_kb_report_flNpko.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_kb_report_flNpko.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_kb_report_flNpko.prototype.doRowPerPageChange = function(sender, rowperpage)
{
