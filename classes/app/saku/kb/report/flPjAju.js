window.app_saku_kb_report_flPjAju = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flPjAju.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flPjAju";
		this.maximize();
	...
	this.getPeriode="select distinct periode from panjar_m where kode_lokasi ='"+this.lokasi+"'";
	...
	this.gridLib.SGEditData(this.sg1,4,[0,1,2],["Jenis Laporan","=","All"]);
	this.pager = 1;
};
window.app_saku_kb_report_flPjAju.extend(window.portalui_childForm);
window.app_saku_kb_report_flPjAju.implement({
	...
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			...
			this.dbLib.setItemsFromSQL("select distinct periode from panjar_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 4)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["All","SPP","Lampiran SPP"]);
		}
	},
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,4) === "All"){
					...
					this.filter2="";
					this.status=true;
				}else if (this.sg1.getCell(2,4) === "SPP"){
					this.nama_report="server_report_kb_rptPjAju";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.akun_pj",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
					...
					this.nama_report="server_report_kb_rptPjAju2";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.akun_pj",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
						this.filterRep.filterStr("a.no_pj",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
			...
			case "preview" : 
				if (this.sg1.getCell(2,4) === "All"){
					if (this.status){
						...
						this.report.preview("server_report_kb_rptPjAju2",this.filter,this.page,1, this.showFilter,this.app._lokasi,this.filter2);
					}else this.viewer.preview("<br /><br /><br /><br /><br /><br /><br />"+result,true);
