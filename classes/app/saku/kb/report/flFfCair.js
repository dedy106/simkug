window.app_saku_kb_report_flFfCair = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flFfCair.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flFfCair";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Pencairan Fluktuatif Fund", 2);
	...
	this.filterRep.setSGFilterRowTipe(this.sg1, 0,[0,1,2,3],["123","123","123","123"]);
	this.filterRep.setSGFilterRowButtonStyle(this.sg1, 0,[0,1,2,3],[0,2,2,2]);
...
window.app_saku_kb_report_flFfCair.extend(window.portalui_childForm);
window.app_saku_kb_report_flFfCair.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
	...
	if (row == 3)
	{
		...
		var sql1="select no_kas,keterangan from kas_m "+filter+" and modul='FFD_NP' order by no_kas";
...
window.app_saku_kb_report_flFfCair.prototype.doSelectCell = function(sender, col, row)
{	
...
window.app_saku_kb_report_flFfCair.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.filter=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					...
					" and a.modul='FFD_NP' ";
		this.nama_report="server_report_kb_rptFfCair";
...
window.app_saku_kb_report_flFfCair.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_report_flFfCair.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.lokasi, this.showFilter,this.filter2);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_kb_report_flFfCair.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_kb_report_flFfCair.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_kb_report_flFfCair.prototype.doRowPerPageChange = function(sender, rowperpage)
{
