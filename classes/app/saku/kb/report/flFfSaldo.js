window.app_saku_kb_report_flFfSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flFfSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flFfSaldo";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Saldo Fluktuatif Fund", 2);
...
window.app_saku_kb_report_flFfSaldo.extend(window.portalui_childForm);
window.app_saku_kb_report_flFfSaldo.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
...
window.app_saku_kb_report_flFfSaldo.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_saku_kb_report_flFfSaldo.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		if (this.dbname=="mysqlt")
		{
			...
			if (this.sg1.getCell(2,3)=="Per Akun")
			{
				...
				this.nama_report="server_report_kb_rptFfSaldo";
			}
...
window.app_saku_kb_report_flFfSaldo.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_report_flFfSaldo.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.showFilter,this.lokasi, this.filter2);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_kb_report_flFfSaldo.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_kb_report_flFfSaldo.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_kb_report_flFfSaldo.prototype.doRowPerPageChange = function(sender, rowperpage)
{
