window.app_saku_kb_report_flDepoSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flDepoSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flDepoSaldo";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Saldo Deposito", 2);
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Bentuk Saldo","=","Per Akun"));
...
window.app_saku_kb_report_flDepoSaldo.extend(window.portalui_childForm);
window.app_saku_kb_report_flDepoSaldo.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
...
window.app_saku_kb_report_flDepoSaldo.prototype.doSelectCell = function(sender, col, row)
{
	...
	else
	{
		...
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4),new  Array(0,3,2,0,0));
...
window.app_saku_kb_report_flDepoSaldo.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.filter2 = this.nik_user+"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(2,0);
		...
		if (this.dbname=="mysqlt")
		{
			...
			if (this.sg1.getCell(2,3)=="Per Akun")
			{
				...
				this.nama_report="server_report_kb_rptDepoSaldo";
			}
		...
		else
		{
			if (this.sg1.getCell(2,3)=="Neraca Percobaan")
			{
				var sql = "sp_glma_tmp '"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"'";
				this.nama_report="server_report_gl_rptTBPercobaan";
			}
			...
			if (this.sg1.getCell(2,3)=="12 Periode")
			{
				var sql = "sp_glma12_tmp '"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"'";
				this.nama_report="server_report_gl_rptTB12";
			}
		}
...
window.app_saku_kb_report_flDepoSaldo.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_report_flDepoSaldo.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.showFilter,this.lokasi, this.filter2);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_kb_report_flDepoSaldo.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_kb_report_flDepoSaldo.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_kb_report_flDepoSaldo.prototype.doRowPerPageChange = function(sender, rowperpage)
{
