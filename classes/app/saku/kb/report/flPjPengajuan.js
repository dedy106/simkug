window.app_saku_kb_report_flPjPengajuan = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_report_flPjPengajuan.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_report_flPjPengajuan";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Pengajuan Panjar", 2);
	...
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Akun Panjar","All",""]);
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["No Bukti","All",""]);
	this.gridLib.SGEditData(this.sg1,4,[0,1,2],["Bentuk","=","Transaksi"]);
...
window.app_saku_kb_report_flPjPengajuan.extend(window.portalui_childForm);
window.app_saku_kb_report_flPjPengajuan.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==0)
	{
	...
	if (row == 3)
	{
		var filter= this.filter=this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					...
					this.filterRep.filterStr("no_pj",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
		var sql1="select no_pj,keterangan from panjar_m "+filter+" and modul='PJR_P' ";
		var sql2="select count(no_pj) from panjar_m  "+filter+" and modul='PJR_P'";
		
		this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_pj","keterangan"),"and",new Array("No Panjar","Keterangan"));
	}
};
window.app_saku_kb_report_flPjPengajuan.prototype.doSelectCell = function(sender, col, row)
{	
	this.sg1.columns.get(col).setReadOnly(false);
	if (this.userStatus=="A")
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4),new  Array("123","12","123","123","3"));
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4),new  Array(2,0,2,2,0));
	}else
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4),new  Array("3","23","123","123","3"));
	...
	{
		this.sg1.columns.get(2).pickList.clear();
		this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Transaksi","Formulir"));
	}
};
window.app_saku_kb_report_flPjPengajuan.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					...
					this.filterRep.filterStr("a.akun_pj",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
					this.filterRep.filterStr("a.no_pj",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
					" and a.modul='PJR_P' ";
		if (this.sg1.getCell(2,4) == "Formulir")
		{
			this.nama_report="server_report_kb_rptPjPengajuan";
		}
		else
		{
			this.nama_report="server_report_kb_rptPjCair";
		}
	...
	{
		alert("[app_saku_kb_report_flPjPengajuan]::mainButtonClick:"+e);
	}
};
window.app_saku_kb_report_flPjPengajuan.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_report_flPjPengajuan.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.lokasi, this.showFilter,this.filter2);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_kb_report_flPjPengajuan.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_kb_report_flPjPengajuan.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_kb_report_flPjPengajuan.prototype.doRowPerPageChange = function(sender, rowperpage)
{
