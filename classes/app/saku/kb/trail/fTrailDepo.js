window.app_saku_kb_trail_fTrailDepo = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_trail_fTrailDepo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_trail_fTrailDepo";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Audit Trail Deposito", 2);
		uses("portalui_panel");
	...
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Bentuk Saldo","=","Per Akun"]);
...
window.app_saku_kb_trail_fTrailDepo.extend(window.portalui_childForm);
window.app_saku_kb_trail_fTrailDepo.prototype.doSg3DblClick = function(sender, colIdx, rowIdx)
{  
	try
	{
...
window.app_saku_kb_trail_fTrailDepo.prototype.SetSg4Mutasi = function()
{
	try
	{
	...
	{
		alert("[app_saku_kb_trail_fTrailDepo]::SetSg4Mutasi:"+e);
	}
};
window.app_saku_kb_trail_fTrailDepo.prototype.doSg4DblClick = function(sender, colIdx, rowIdx)
{	
	try{
...
window.app_saku_kb_trail_fTrailDepo.prototype.SetSg5Mutasi = function()
{
	try
	{
		var debet = kredit = 0;
...
window.app_saku_kb_trail_fTrailDepo.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
...
window.app_saku_kb_trail_fTrailDepo.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
	...
	if (row == 3)
	{
		...
		this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Per Akun","Per PP","Per Karyawan"));
	}
	if (row == 4)
	{
		this.sg1.columns.get(2).pickList.clear();
...
window.app_saku_kb_trail_fTrailDepo.prototype.mainButtonClick = function(sender)
{	
	if (sender == this.app._mainForm.bPreview){
		try
		{
			...
			if (this.dbname=="mysqlt")
			{
				...
				if (this.sg1.getCell(2,3)=="Per Akun")
				{
					
					var sql = "call sp_glma_depo_tmp ('"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
...
window.app_saku_kb_trail_fTrailDepo.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_trail_fTrailDepo.prototype.doCloseReportClick = function(sender)
{
