window.app_saku_kb_trail_fTrailIf = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_trail_fTrailIf.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_trail_fTrailIf";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Audit Trail Imprest Fund", 2);
		uses("portalui_panel");
...
window.app_saku_kb_trail_fTrailIf.extend(window.portalui_childForm);
window.app_saku_kb_trail_fTrailIf.prototype.doSg3DblClick = function(sender, colIdx, rowIdx)
{  
	try
	{
...
window.app_saku_kb_trail_fTrailIf.prototype.SetSg4Mutasi = function()
{
	try
	{
	...
	{
		alert("[app_saku_kb_trail_fTrailIf]::SetSg4Mutasi:"+e);
	}
};
window.app_saku_kb_trail_fTrailIf.prototype.doSg4DblClick = function(sender, colIdx, rowIdx)
{	
	try{
...
window.app_saku_kb_trail_fTrailIf.prototype.SetSg5Mutasi = function()
{
	try
	{
		var debet = kredit = 0;
...
window.app_saku_kb_trail_fTrailIf.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
...
window.app_saku_kb_trail_fTrailIf.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
...
window.app_saku_kb_trail_fTrailIf.prototype.mainButtonClick = function(sender)
{	
		...
		try
		{
			...
			if (this.dbname=="mysqlt")
			{
				...
				if (this.sg1.getCell(2,3)=="Per Akun")
				{
					...
					var sql = "call sp_glma_if_tmp ('"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
...
window.app_saku_kb_trail_fTrailIf.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_kb_trail_fTrailIf.prototype.doCloseReportClick = function(sender)
{
