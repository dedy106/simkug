window.app_saku_kb_trail_fTrailKb = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_trail_fTrailKb.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_kb_trail_fTrailKb";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Audit Trail KasBank", 2);
	...
	this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Periode","=",this.app._mainForm._periode]);
...
window.app_saku_kb_trail_fTrailKb.extend(window.portalui_childForm);
window.app_saku_kb_trail_fTrailKb.prototype.doSg3DblClick = function(sender, colIdx, rowIdx)
{  
	try
	{
	...
	this.sg4.setColTitle(["No","No Bukti","Tanggal","Keterangan","Kode PP","Kode DRK","Debet","Kredit","Balance"]);
...
window.app_saku_kb_trail_fTrailKb.prototype.SetSg4Mutasi = function()
{
	try
	{
	...
	{
		alert("[app_saku_kb_trail_fTrailKb]::SetSg4Mutasi:"+e);
	}
};
window.app_saku_kb_trail_fTrailKb.prototype.doSg4DblClick = function(sender, colIdx, rowIdx)
{	
	try{
	...
	this.sg5.setColTitle(["No","Kode Akun","Nama Akun","Keterangan","Kode PP","Kode DRK","Debet","Kredit"]);
	}catch(e){
		alert(e);
	}
	try
...
window.app_saku_kb_trail_fTrailKb.prototype.SetSg5Mutasi = function()
{
	try
	{
		var debet = kredit = 0;
...
window.app_saku_kb_trail_fTrailKb.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
...
window.app_saku_kb_trail_fTrailKb.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
...
window.app_saku_kb_trail_fTrailKb.prototype.mainButtonClick = function(sender)
{	
		...
		try
		{
			...
			if (this.dbname=="mysqlt")
			{
				...
				if (this.sg1.getCell(2,3)=="Neraca Lajur")
				{
					...
					var sql = "call sp_glma_kas_tmp ('"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
			...
			this.dbLib.execQuerySync(sql);
				...
				if (this.p1.visible){ 
					...
					{		
						this.sg2.clearAll();
						var line,data = this.dbLib.runSQL("select kode_akun,nama,so_awal,debet,kredit,so_akhir from glma_tmp where nik_user='"+this.nik_user+"' order by kode_akun ");
						if (data instanceof portalui_arrayMap)
						{						
...
window.app_saku_kb_trail_fTrailKb.prototype.doRequestReady = function(sender, methodName, result)
{
		switch (methodName)
		{
			case "preview" : 
...
window.app_saku_kb_trail_fTrailKb.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page, 30, "YAKES", this.showFilter,this.filter2);	
};
window.app_saku_kb_trail_fTrailKb.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_kb_trail_fTrailKb.prototype.doAllPageClick = function(sender)
{
	this.report.preview(this.nama_report,this.filter, 1, this.viewer.getTotalPage() * 30, this.app._namaLokasi,this.showFilter);	
};
window.app_saku_kb_trail_fTrailKb.prototype.doPdfClick = function(sender)
{
...
window.app_saku_kb_trail_fTrailKb.prototype.doXlsClick = function()
{
