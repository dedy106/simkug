window.app_saku_kb_transaksi_fTakTerimaK = function(owner){
	if (owner)
	{
		window.app_saku_kb_transaksi_fTakTerimaK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_kb_transaksi_fTakTerimaK";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form TAK Kas Bank Terima : Koreksi", 0);
		...
		uses("portalui_datePicker;portalui_saiGrid;portalui_sgNavigator;app_saku_fJurnalViewer;util_dbLarge;util_standar;util_addOnLib");	
		...
		this.cb_periode = new portalui_saiCB(this,{bound:[700,56,185,20],caption:"Periode Lama", change:[this,"doChange"]});			
		...
		this.cb_bukti = new portalui_saiCBBL(this,{bound:[700,78,185,20],caption:"No TAK Lama",multiSelection:false});	
		this.bShow2 = new portalui_imageButton(this,{bound:[900,78,22,22],hint:"Load Data", image:"icon/"+system.getThemes()+"/reload.png", click:[this,"doLoadData"]});		
		
		this.ed_desc = new portalui_saiLabelEdit(this,{bound:[20,100,500,20], caption:"Deskripsi",maxLength:150});				
		this.cb_lokasi = new portalui_saiCBBL(this,{bound:[700,100,185,20],caption:"Lokasi Kirim",multiSelection:false,change:[this,"doChange"],
		...
		this.cb_nbkirim = new portalui_saiCBBL(this,{bound:[700,122,185,20],caption:"No TAK Kirim",multiSelection:false});	
		this.bShow = new portalui_imageButton(this,{bound:[900,122,22,22],hint:"Load Data", image:"icon/"+system.getThemes()+"/reload.png", click:[this,"doLoadData"]});		
		...
		try
		{
			...
			var val = this.dbLib.loadQuery("select distinct periode from kas_m where kode_lokasi2='"+this.app._lokasi+"' and jenis like 'T%' and not (periode < '"+this.app._periode.substr(0,4)+"01') order by periode");
			var val = val.split("\r\n");
...
window.app_saku_kb_transaksi_fTakTerimaK.extend(window.portalui_childForm);
window.app_saku_kb_transaksi_fTakTerimaK.implement({
	...
	simpan: function(){		
		...
		if (this.standarLib.checkEmptyByTag(this, new Array("0")))
		{
			try
			{
				...
				sql.add("update kas_m set no_bukti2 = '-' where kode_lokasi2 ='"+this.app._lokasi+"' and kode_lokasi = '"+this.cb_lokasi.getText()+"' and no_kas = '"+this.no_bukti2+"' ");				
				sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,"+
						...
						"('"+this.nb+"','"+this.app._lokasi+"','"+this.ed_dok.getText()+"','"+this.ed_giro.getText()+"','"+this.cb_akun.getText()+"','"+this.dp_tgl1.getDate()+"','"+this.ed_desc.getText()+"','-','KBI_NP','"+this.jenis+"','"+
						     this.ed_period.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+","+parseNilai(this.ed_nilai.getText())+",'"+this.cb_pembuat.getText()+"','-',now(),'"+this.app._userLog+"','F','-','-','-','"+this.cb_akun.getText()+"','"+this.cb_lokasi.getText()+"','"+this.cb_nbkirim.getText()+"')");
				...
				sql.add("update kas_m set no_bukti2 = '"+this.nb+"' where kode_lokasi2 ='"+this.app._lokasi+"' and kode_lokasi = '"+this.cb_lokasi.getText()+"' and no_kas = '"+this.cb_nbkirim.getText()+"' ");
			...
			case "hapus":
				...
				if (parseFloat(this.cb_periode.getText()) < parseFloat(this.app._periode)) 
				{
					...
					sql.add("insert into angg_r (no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) "+	
							"   select concat(no_bukti,'r'),modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,'"+this.ed_period.getText()+"',case dc when 'D' then 'C' else 'D' end as dc,0,nilai "+
							"   from angg_r where no_bukti = '"+this.cb_bukti.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");										
					this.nb = this.cb_bukti.getText()+"r";
					this.formEvent = "ubah";
				}
				else
				{
	...
	doChange: function(sender){
		if (sender == this.cb_lokasi){
			this.cb_nbkirim.setText("");
			this.cb_nbkirim.setSQL("select no_kas, keterangan from kas_m where kode_lokasi2 = '"+this.app._lokasi+"' and kode_lokasi = '"+this.cb_lokasi.getText()+"' and no_bukti2 = '-'  and jenis like 'T%' and posted = 'F'",["no_kas","keterangan"],false,["No KAS","Keterangan"],"and","Daftar TAK Kirim",false);
			
		}
		if (sender == this.cb_periode){
			this.cb_bukti.setText("");
			this.cb_bukti.setSQL("select no_kas, keterangan from kas_m where kode_lokasi = '"+this.app._lokasi+"' and periode = '"+this.cb_periode.getText()+"' and no_bukti2 <> '-' and jenis like 'T%' and posted = 'F'",["no_kas","keterangan"],false,["No KAS","Keterangan"],"and","Daftar TAK Terima",true);
	...
	doFindBtnClick: function(sender, col, row){
		...
		{
			alert("[app_saku_kb_transaksi_fTakTerimaK] : doFindBtnClick : " + e);
		}
	},
	doNilaiChange: function(){
		...
		{
			alert("[app_saku_kb_transaksi_fTakTerimaK]::doNilaiChange:"+e);
		}
	},
	doCellEnter: function(sender, col, row){
		...
		{
			alert("app_saku_kb_transaksi_fTakTerimaK::doCellEnter : " + e);
			}	
	},
	...
	doLoadData: function(sender){
		try{
			if (sender == this.bShow){
				var data = this.dbLib.getDataProvider("select a.kode_bank as akuntak, b.kode_akun, c.nama, b.keterangan, case when b.dc = 'D' then 'C' else 'D' end as dc, b.nilai, b.kode_pp, ifnull(d.nama,'-') as nmpp, b.kode_drk, ifnull(e.nama,'-') as nmdrk, b.kode_bank, ifnull(f.nama,'-') as nmbank "+
								...
								"where a.no_kas = '"+this.cb_nbkirim.getText()+"' and a.kode_lokasi2 = '"+this.app._lokasi+"' ",true);
				if (typeof data != "string"){
					...
					if (data.rs.rows[0]){
						...
						for (var i in data.rs.rows){
							line = data.rs.rows[i];
							this.sg1.appendData([line.kode_akun, line.nama,line.keterangan, line.dc.toUpperCase(), floatToNilai(line.nilai), this.app._kodePP, "-", line.kode_drk, line.nmdrk, line.kode_bank, line.nmbank]);
						}
					}
				}
			}
			if (sender == this.bShow2){				
				var data = this.dbLib.getDataProvider("select a.jenis, a.keterangan as ket, a.tanggal,a.kode_curr, a.kurs, a.no_dokumen, a.kode_bank as akuntak, b.kode_akun, c.nama, b.keterangan, b.dc, b.nilai, b.kode_pp, ifnull(d.nama,'-') as nmpp, b.kode_drk, ifnull(e.nama,'-') as nmdrk, b.kode_bank, ifnull(f.nama,'-') as nmbank, a.kode_lokasi2, a.nik_buat, a.no_bukti2 "+
								" from kas_m a inner join kas_j b on b.no_kas = a.no_kas and b.kode_lokasi = a.kode_lokasi and b.kode_akun <> a.kode_bank "+
								" inner join masakun c on c.kode_akun = b.kode_akun and c.kode_lokasi = a.kode_lokasi "+
								" left outer join pp d on d.kode_pp = b.kode_pp and d.kode_lokasi = b.kode_lokasi "+
								" left outer join drk e on e.kode_drk = b.kode_drk and e.kode_lokasi = b.kode_lokasi "+
				...
				if (typeof data != "string"){
					...
					if (data.rs.rows[0]){
						...
						this.cb_nbkirim.setText(line.no_bukti2 == "-" ? line.no_dokumen : line.no_bukti2);
						this.no_bukti2 = line.no_bukti2;
						for (var i in data.rs.rows){
							line = data.rs.rows[i];
