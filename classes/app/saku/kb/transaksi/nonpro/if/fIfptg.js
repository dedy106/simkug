window.app_saku_kb_transaksi_nonpro_if_fIfptg = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_transaksi_nonpro_if_fIfptg.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_kb_transaksi_nonpro_if_fIfptg";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Reimburse [Closing] ImprestFund [KB Masuk/Keluar] NP: Input", 0);	
		...
		this.cb_noif.setTop(166);
		this.cb_noif.setWidth(250);
		...
		this.cb_akunif.setLeft(700);
		this.cb_akunif.setTop(188);
		this.cb_akunif.setWidth(175);
		...
		try
		{
			...
			this.cb_curr.setText("IDR"); this.ed_kurs.setText("1"); this.cb_status.setText("REIMBURSE");
			this.sg1.clear();
			this.sg1.appendRow();
			
			data = this.dbLib.runSQL("select flag from spro where kode_spro = 'BYMHDIF' and kode_lokasi='"+this.app._lokasi+"'");
			var row = undefined;
			row = data.get(0);
			this.akunBYMHD = row.get("flag");
...
window.app_saku_kb_transaksi_nonpro_if_fIfptg.extend(window.portalui_childForm);
window.app_saku_kb_transaksi_nonpro_if_fIfptg.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_kb_transaksi_nonpro_if_fIfptg.prototype.simpan = function()
{
	if (nilaiToFloat(this.ed_nilai.getText()) > nilaiToFloat(this.ed_nilaiif.getText()))
	{
		system.alert(this,"Nilai pertanggungan tidak valid.","Nilai pertanggungan tidak boleh melebihi nilai imprestfund.");
		return false;
	}
	if ((nilaiToFloat(this.ed_nilai.getText()) <= 0) && (this.cb_status.getText() != "CLOSING"))
	{
		system.alert(this,"Nilai pertanggungan reimburse tidak valid.","Nilai pertanggungan reimburse tidak boleh nol atau kurang.");
		return false;
	}
	this.bGen.click();
	if (this.standarLib.checkEmptyByTag(this,["0","1"]))
	{
		try
		{
			...
			sql.add("insert into ifptg_m (no_ifptg,no_if,no_kas,no_dokumen,tanggal,"+
					"keterangan,catatan,kode_curr,kurs,akun_ap,akun_kas,nik_buat,nik_setuju,kode_lokasi,"+
					"modul,status,nilai,nilai_kas,progress,posted,periode,no_del,no_link,nik_user,tgl_input)  values "+
					"('"+this.ed_noptg.getText()+"','"+this.cb_noif.getText()+"','"+this.ed_nb.getText()+"','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDate()+"','"+
					this.ed_desc.getText()+"','-','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+",'"+this.akunBYMHD+"','"+this.cb_akun.getText()+"','"+this.cb_buat.getText()+"','"+this.cb_setuju.getText()+"','"+this.app._lokasi+"',"+
					"'IFPTG_NP','"+this.cb_status.getText()+"',"+parseNilai(this.ed_nilai.getText())+","+nilaiKB+",'2','F','"+this.ed_period.getText()+"','-','-','"+this.app._userLog+"',now())");
					
			if (nilaiToFloat(this.ed_nilai.getText()) != 0)
			{
				for (var i=0; i < this.sg1.rows.getLength(); i++)
				{
					sql.add("insert into ifptg_j (no_ifptg,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,"+
							...
							"'"+this.app._lokasi+"','IFPTG_NP','BBN','"+this.ed_period.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+
			...
			sql.add("insert into ifptg_j (no_ifptg,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,"+
						"kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+	
						"('"+this.ed_noptg.getText()+"','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDate()+"',"+i+",'"+this.akunBYMHD+
						"','"+this.ed_desc.getText()+"','C',"+parseNilai(this.ed_nilai.getText())+",'-','-',"+
						"'"+this.app._lokasi+"','IFPTG_NP','BYMHD','"+this.ed_period.getText()+
			...
			if (this.cb_status.getText() == "CLOSING") 
			{
				if (nilaiToFloat(this.ed_nilai.getText()) != nilaiToFloat(this.ed_nilaiif.getText()))
				{
					var z = nilaiToFloat(this.ed_nilaiif.getText()) -  nilaiToFloat(this.ed_nilai.getText());
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,"+
							...
							"('"+this.ed_nb.getText()+"','"+this.app._lokasi+"','"+this.ed_dok.getText()+"','"+this.ed_giro.getText()+"','"+this.cb_akun.getText()+"','"+this.dp_tgl1.getDate()+"','"+this.ed_desc.getText()+"','-','IFPTG_NP','"+this.cb_jenis.getText()+"','"+
					...
					sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
							...
							"'"+this.ed_noptg.getText()+"','"+this.app._lokasi+"','IFPTG_NP','KAS',"+
							"'"+this.ed_period.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+
							",'"+this.app._userLog+"',now())");
					
					sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
							"kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+	
							"('"+this.ed_nb.getText()+"','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDate()+"',1,'"+this.akunBYMHD+
							"','"+this.ed_desc.getText()+"','D',"+parseNilai(this.ed_nilai.getText())+",'-','-',"+
							"'"+this.ed_noptg.getText()+"','"+this.app._lokasi+"','IFPTG_NP','BYMHD',"+
							"'"+this.ed_period.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+
							",'"+this.app._userLog+"',now())");
					
					sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
							"kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+	
							"('"+this.ed_nb.getText()+"','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDate()+"',2,'"+this.cb_akunif.getText()+
							"','"+this.ed_desc.getText()+"','C',"+parseNilai(this.ed_nilaiif.getText())+",'-','-',"+
							"'"+this.ed_noptg.getText()+"','"+this.app._lokasi+"','IFPTG_NP','IF',"+
							"'"+this.ed_period.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+
							",'"+this.app._userLog+"',now())");		
				}
			} 
			...
			{
				var z = nilaiToFloat(this.ed_nilai.getText());
...
window.app_saku_kb_transaksi_nonpro_if_fIfptg.prototype.doModalResult = function(event, modalResult)
{
...
window.app_saku_kb_transaksi_nonpro_if_fIfptg.prototype.genClick = function(sender)
{
	try
	{
		this.sg1.validasi();
		if (this.cb_status.getText() == 'CLOSING')
		{
			if (nilaiToFloat(this.ed_nilaiif.getText()) != nilaiToFloat(this.ed_nilai.getText()))
			{
				var stsKB = "T";
			}
			else
		...
		if (this.ed_period.getText() != "")
		{
			...
			this.ed_noptg.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'ifptg_m','no_ifptg',this.app._lokasi+"-IFP"+this.ed_period.getText().substr(2,4)+".",'0000'));
...
window.app_saku_kb_transaksi_nonpro_if_fIfptg.prototype.doSelect = function(sender, year, month, day)
{
	if (month < 10)
		month = "0"+month;
	this.ed_period.setText(year.toString()+month);
};
window.app_saku_kb_transaksi_nonpro_if_fIfptg.prototype.showClick = function(sender)
{
	try 
	{
		var line,data = this.dbLib.runSQL(" select a.akun_if,a.nik_pengaju,b.nama as nama_pengaju,a.kode_curr,a.kurs,a.nilai "+
...
window.app_saku_kb_transaksi_nonpro_if_fIfptg.prototype.doEditChange = function(sender)
{
...
window.app_saku_kb_transaksi_nonpro_if_fIfptg.prototype.FindBtnClick = function(sender, event)
{
	try
	{
		if (sender == this.cb_noif) 
		{
			this.standarLib.showListData(this, "Daftar ImprestFund",this.cb_noif,undefined, 
										  "select no_if, keterangan  from if_m where kode_lokasi = '"+this.app._lokasi+"' and progress='2' and modul='IFD_NP' and periode<='"+this.ed_period.getText()+"'",
										  "select count(no_if)       from if_m where kode_lokasi = '"+this.app._lokasi+"' and progress='2' and modul='IFD_NP' and periode<='"+this.ed_period.getText()+"'",
										  ["no_if","keterangan"],"and",["No ImprestFund","Keterangan"],false);
			this.standarLib.clearByTag(this, new Array("1"),undefined);  this.cb_status.setText("REIMBURSE");				
		}
...
window.app_saku_kb_transaksi_nonpro_if_fIfptg.prototype.doFindBtnClick = function(sender, col, row) 
{
	try
	{
		switch(col)
		{
			...
			case 5 : 
				this.standarLib.showListDataForSG(this, "Daftar Departemen",this.sg1, this.sg1.row, this.sg1.col,
												  ...
												  ["kode_pp","nama"],"and",["Kode Departemen","Deskripsi"],false);
				break;
			case 7 : 
	...
	{
		alert("[app_saku_kb_transaksi_nonpro_if_fIfptg] : doFindBtnClick : " + e);
	}
};
window.app_saku_kb_transaksi_nonpro_if_fIfptg.prototype.doNilaiChange = function()
{
	try
	{
	...
	{
		alert("[app_saku_kb_transaksi_nonpro_if_fIfptg]::doNilaiChange:"+e);
	}
};
window.app_saku_kb_transaksi_nonpro_if_fIfptg.prototype.doCellExit = function()
{
	this.sg1.validasi();
};
window.app_saku_kb_transaksi_nonpro_if_fIfptg.prototype.doRequestReady = function(sender, methodName, result)
{
