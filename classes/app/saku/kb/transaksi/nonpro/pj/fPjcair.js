window.app_saku_kb_transaksi_nonpro_pj_fPjcair = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_transaksi_nonpro_pj_fPjcair.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_kb_transaksi_nonpro_pj_fPjcair";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pencairan Panjar [Kas Bank Keluar] Non Proses: Input", 0);
		...
		this.ed_nopj = new portalui_saiLabelEdit(this);
		this.ed_nopj.setLeft(20);
		this.ed_nopj.setTop(100);
		this.ed_nopj.setWidth(230);
		this.ed_nopj.setCaption("No Panjar");
		this.ed_nopj.setReadOnly(true);
		this.ed_nopj.setLength(50);
		...
		try
		{
			...
			this.cb_akunpj.onBtnClick.set(this, "FindBtnClick");
...
window.app_saku_kb_transaksi_nonpro_pj_fPjcair.extend(window.portalui_childForm);
window.app_saku_kb_transaksi_nonpro_pj_fPjcair.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_kb_transaksi_nonpro_pj_fPjcair.prototype.simpan = function()
{
	if  (nilaiToFloat(this.ed_nilai.getText()) <= 0)
	{
		system.alert(this,"Nilai Panjar tidak valid.","Nilai tidak boleh nol atau kurang");
		return false;
	}
	if ( (new Date()).strToDate(this.dp_tgl1.getDate())  > (new Date()).strToDate(this.dp_tgl2.getDate()))
	{
		system.alert(this,"Tanggal panjar tidak valid.","Tanggal pencairan melebihi maksimal tgl pertanggungan");
		return false;
	}	
	this.bGen.click();
	if (this.standarLib.checkEmptyByTag(this, new Array("0")))
	{
		try
		{
			...
			sql.add("insert into panjar_m (no_pj,no_kas,no_dokumen,tanggal,due_date,"+
					...
					"('"+this.ed_nopj.getText()+"','"+this.ed_nb.getText()+"','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDate()+"','"+this.dp_tgl2.getDate()+
					"','"+this.ed_desc.getText()+"','-','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+",'"+this.cb_akunpj.getText()+"','"+this.cb_pengaju.getText()+"','"+this.cb_setuju.getText()+"','"+this.app._lokasi+"','"+this.cb_pp.getText()+
					"','PJR_NP',"+parseNilai(this.ed_nilai.getText())+",0,'"+this.cb_drk.getText()+"','2','"+this.ed_period.getText()+"','-','-','"+this.app._userLog+"',now())");
			...
			sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,"+
					...
					"('"+this.ed_nb.getText()+"','"+this.app._lokasi+"','"+this.ed_dok.getText()+"','"+this.ed_giro.getText()+"','"+this.cb_akun.getText()+"','"+this.dp_tgl1.getDate()+"','"+this.ed_desc.getText()+"','-','PJR_NP','"+this.cb_jenis.getText()+"','"+
					     this.ed_period.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+","+parseNilai(this.ed_nilai.getText())+",'-','-',now(),'"+this.app._userLog+"','F','-','-','"+this.ed_nopj.getText()+"')");
			...
			sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
					...
					this.cb_drk.getText()+"','"+this.ed_nopj.getText()+"','"+this.app._lokasi+"','PJR_NP','KAS',"+
					"'"+this.ed_period.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+
					",'"+this.app._userLog+"',now())");
			
			sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
					"kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+	
					"('"+this.ed_nb.getText()+"','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDate()+"',1,'"+this.cb_akunpj.getText()+
					"','"+this.ed_desc.getText()+"','D',"+parseNilai(this.ed_nilai.getText())+",'"+this.cb_pp.getText()+"','"+
					this.cb_drk.getText()+"','"+this.ed_nopj.getText()+"','"+this.app._lokasi+"','PJR_NP','PJR',"+
					"'"+this.ed_period.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+
					",'"+this.app._userLog+"',now())");
...
window.app_saku_kb_transaksi_nonpro_pj_fPjcair.prototype.doModalResult = function(event, modalResult)
{
...
window.app_saku_kb_transaksi_nonpro_pj_fPjcair.prototype.genClick = function(sender)
{
	try
	{
		if ((this.ed_period.getText() != "") && (this.jenis != undefined))
		{
			...
			this.ed_nopj.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'panjar_m','no_pj',this.app._lokasi+"-PJR"+this.ed_period.getText().substr(2,4)+".",'0000'));
...
window.app_saku_kb_transaksi_nonpro_pj_fPjcair.prototype.doSelect = function(sender, year, month, day)
{
	if (month < 10)
		month = "0"+month;
	this.ed_period.setText(year.toString()+month);
};
window.app_saku_kb_transaksi_nonpro_pj_fPjcair.prototype.doEditChange = function(sender)
{

	if (sender == this.ed_period)
	{
		this.ed_nb.setText("");
		this.ed_nopj.setText("");
...
window.app_saku_kb_transaksi_nonpro_pj_fPjcair.prototype.FindBtnClick = function(sender, event)
{
	try
	{
		...
		if (sender == this.cb_drk) 
		{
			this.standarLib.showListData(this, "Daftar RKM",this.cb_drk,undefined, 
										  "select distinct a.kode_drk, a.nama from drk a inner join anggaran_d b on a.kode_drk=b.kode_drk where a.tahun=substring(b.periode,1,4) and b.periode like '"+this.ed_period.getText().substr(0,4)+"%' and b.kode_pp = '"+this.cb_pp.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",
										  "select count(distinct a.kode_drk)  from drk a inner join anggaran_d b on a.kode_drk=b.kode_drk where a.tahun=substring(b.periode,1,4) and b.periode like '"+this.ed_period.getText().substr(0,4)+"%' and b.kode_pp = '"+this.cb_pp.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",
										  ["a.kode_drk","a.nama"],"and",["Kode RKM","Nama"],false);
		}
		if (sender == this.cb_akunpj) 
		{
			this.standarLib.showListData(this, "Daftar Akun Panjar",this.cb_akunpj,undefined, 
...
window.app_saku_kb_transaksi_nonpro_pj_fPjcair.prototype.doRequestReady = function(sender, methodName, result)
{
