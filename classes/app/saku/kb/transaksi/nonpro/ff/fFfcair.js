window.app_saku_kb_transaksi_nonpro_ff_fFfcair = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_transaksi_nonpro_ff_fFfcair.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_kb_transaksi_nonpro_ff_fFfcair";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pencairan Fluktuatif Fund [KB Keluar] Non Proses: Input", 0);	
		...
		this.ed_noff = new portalui_saiLabelEdit(this);
		this.ed_noff.setLeft(20);
		this.ed_noff.setTop(100);
		this.ed_noff.setWidth(230);
		this.ed_noff.setCaption("No Fluktuatif Fund");
		this.ed_noff.setReadOnly(true);
		this.ed_noff.setLength(50);
		...
		this.cb_akunff.setLeft(20);
		this.cb_akunff.setTop(188);
		this.cb_akunff.setWidth(185);
		this.cb_akunff.setCaption("Akun Fluktuatif Fund");
		...
		this.ed_nilai.setCaption("Nilai Fluktuatif Fund");
		...
		try
		{
			...
			this.cb_akunff.onBtnClick.set(this, "FindBtnClick");
...
window.app_saku_kb_transaksi_nonpro_ff_fFfcair.extend(window.portalui_childForm);
window.app_saku_kb_transaksi_nonpro_ff_fFfcair.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_kb_transaksi_nonpro_ff_fFfcair.prototype.simpan = function()
{
	if  (nilaiToFloat(this.ed_nilai.getText()) <= 0)
	{
		system.alert(this,"Nilai fluktuatif fund tidak valid.","Nilai tidak boleh nol atau kurang");
		return false;
	}
	if ( (new Date()).strToDate(this.dp_tgl1.getDate())  > (new Date()).strToDate(this.dp_tgl2.getDate()))
	{
		system.alert(this,"Tanggal fluktuatif fund tidak valid.","Tanggal pencairan melebihi tanggal closing");
		return false;
	}	
	
	var data = this.dbLib.runSQL("select nik_pengaju from ff_m where progress <> '9' and nik_pengaju = '"+this.cb_pengaju.getText()+"'");
	if (data.get(0) != undefined)
	{
		system.alert(this,"NIK Pengaju tidak valid.","NIK Pengaju masih mempunyai Fluktuatif Fund yang masih aktif.");
		return false;
	}
	...
	if (this.standarLib.checkEmptyByTag(this, new Array("0")))
	{
		try
		{
			...
			sql.add("insert into ff_m (no_ff,no_kas,no_dokumen,tanggal,due_date,"+
					"keterangan,catatan,kode_curr,kurs,akun_ff,nik_pengaju,nik_setuju,kode_lokasi,kode_pp,"+
					"modul,nilai,progress,periode,no_del,no_link,nik_user,tgl_input)  values "+
					"('"+this.ed_noff.getText()+"','"+this.ed_nb.getText()+"','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDate()+"','"+this.dp_tgl2.getDate()+
					"','"+this.ed_desc.getText()+"','-','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+",'"+this.cb_akunff.getText()+"','"+this.cb_pengaju.getText()+"','"+this.cb_setuju.getText()+"','"+this.app._lokasi+"','"+this.cb_pp.getText()+
					"','FFD_NP',"+parseNilai(this.ed_nilai.getText())+",'2','"+this.ed_period.getText()+"','-','-','"+this.app._userLog+"',now())");
			...
			sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,"+
					...
					"('"+this.ed_nb.getText()+"','"+this.app._lokasi+"','"+this.ed_dok.getText()+"','"+this.ed_giro.getText()+"','"+this.cb_akun.getText()+"','"+this.dp_tgl1.getDate()+"','"+this.ed_desc.getText()+"','-','FFD_NP','"+this.cb_jenis.getText()+"','"+
					     this.ed_period.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+","+parseNilai(this.ed_nilai.getText())+",'-','-',now(),'"+this.app._userLog+"','F','-','-','"+this.ed_noff.getText()+"')");
			...
			sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
					...
					"'"+this.ed_noff.getText()+"','"+this.app._lokasi+"','FFD_NP','KAS',"+
					"'"+this.ed_period.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+
					",'"+this.app._userLog+"',now())");
			
			sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
					"kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+	
					"('"+this.ed_nb.getText()+"','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDate()+"',1,'"+this.cb_akunff.getText()+
					"','"+this.ed_desc.getText()+"','D',"+parseNilai(this.ed_nilai.getText())+",'"+this.cb_pp.getText()+"','-',"+
					"'"+this.ed_noff.getText()+"','"+this.app._lokasi+"','FFD_NP','FFD',"+
					"'"+this.ed_period.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+
					",'"+this.app._userLog+"',now())");
...
window.app_saku_kb_transaksi_nonpro_ff_fFfcair.prototype.doModalResult = function(event, modalResult)
{
...
window.app_saku_kb_transaksi_nonpro_ff_fFfcair.prototype.genClick = function(sender)
{
	try
	{
		if ((this.ed_period.getText() != "") && (this.jenis != undefined))
		{
			...
			this.ed_noff.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'ff_m','no_ff',this.app._lokasi+"-FFD"+this.ed_period.getText().substr(2,4)+".",'0000'));
...
window.app_saku_kb_transaksi_nonpro_ff_fFfcair.prototype.doSelect = function(sender, year, month, day)
{
	if (month < 10)
		month = "0"+month;
	this.ed_period.setText(year.toString()+month);
};
window.app_saku_kb_transaksi_nonpro_ff_fFfcair.prototype.doEditChange = function(sender)
{

	if (sender == this.ed_period)
	{
		this.ed_nb.setText("");
		this.ed_noff.setText("");
...
window.app_saku_kb_transaksi_nonpro_ff_fFfcair.prototype.FindBtnClick = function(sender, event)
{
	try
	{
		if (sender == this.cb_pp) 
		{
			this.standarLib.showListData(this, "Daftar Departemen",this.cb_pp,undefined, 
										  ...
										  new Array("kode_pp","nama"),"and",new Array("Kode Departemen","Nama"),false);
		}
		if (sender == this.cb_akunff) 
		{
			this.standarLib.showListData(this, "Daftar Akun Fluktuatif Fund",this.cb_akunff,undefined, 
										  "select a.kode_akun, a.nama from masakun a inner join flag_relasi b on a.kode_akun = b.kode_akun and a.kode_lokasi=b.kode_lokasi where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_flag='011' ",
										  "select count(a.kode_akun)  from masakun a inner join flag_relasi b on a.kode_akun = b.kode_akun and a.kode_lokasi=b.kode_lokasi where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_flag='011' ",
...
window.app_saku_kb_transaksi_nonpro_ff_fFfcair.prototype.doRequestReady = function(sender, methodName, result)
{
