window.app_saku_kb_transaksi_nonpro_kb_fKbin = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_transaksi_nonpro_kb_fKbin.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_kb_transaksi_nonpro_kb_fKbin";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Kas Bank Masuk Non Proses : Input", 0);
...
window.app_saku_kb_transaksi_nonpro_kb_fKbin.extend(window.portalui_childForm);
window.app_saku_kb_transaksi_nonpro_kb_fKbin.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_kb_transaksi_nonpro_kb_fKbin.prototype.simpan = function()
{
	...
	if (this.standarLib.checkEmptyByTag(this, new Array("0")))
	{
		try
		{
			...
			sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,"+
					...
					"('"+this.ed_nb.getText()+"','"+this.app._lokasi+"','"+this.ed_dok.getText()+"','"+this.ed_giro.getText()+"','"+this.akunkb+"','"+this.dp_tgl1.getDate()+"','"+this.ed_desc.getText()+"','-','KBI_NP','"+this.cb_jenis.getText()+"','"+
...
window.app_saku_kb_transaksi_nonpro_kb_fKbin.prototype.doModalResult = function(event, modalResult)
{
...
window.app_saku_kb_transaksi_nonpro_kb_fKbin.prototype.genClick = function(sender)
{
	try
	{
...
window.app_saku_kb_transaksi_nonpro_kb_fKbin.prototype.doSelect = function(sender, year, month, day)
{
	if (month < 10)
		month = "0"+month;
	this.ed_period.setText(year.toString()+month);
};
window.app_saku_kb_transaksi_nonpro_kb_fKbin.prototype.doEditChange = function(sender)
{
...
window.app_saku_kb_transaksi_nonpro_kb_fKbin.prototype.FindBtnClick = function(sender, event)
{
	try
	{
...
window.app_saku_kb_transaksi_nonpro_kb_fKbin.prototype.doFindBtnClick = function(sender, col, row) 
{
	try
	{
		switch(col)
...
window.app_saku_kb_transaksi_nonpro_kb_fKbin.prototype.doNilaiChange = function()
{
	try
	{
		var totD = totC = nKB = 0; 
	...
	{
		alert("[app_saku_kb_transaksi_nonpro_kb_fKbin]::doNilaiChange:"+e);
	}
};
window.app_saku_kb_transaksi_nonpro_kb_fKbin.prototype.doCellEnter = function(sender, col, row) 
{
	try
	{
		switch(col)
	...
	{
		alert("app_saku_kb_transaksi_nonpro_kb_fKbin::doCellEnter : " + e);
	}	
};
window.app_saku_kb_transaksi_nonpro_kb_fKbin.prototype.doCellExit = function(sender, col, row) 
{
	try
	{
		switch(col)
...
window.app_saku_kb_transaksi_nonpro_kb_fKbin.prototype.doRequestReady = function(sender, methodName, result)
{
