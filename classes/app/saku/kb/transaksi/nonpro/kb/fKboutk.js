window.app_saku_kb_transaksi_nonpro_kb_fKboutk = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_transaksi_nonpro_kb_fKboutk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_kb_transaksi_nonpro_kb_fKboutk";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Kas Bank Keluar Non Proses : Koreksi", 0);	
...
window.app_saku_kb_transaksi_nonpro_kb_fKboutk.extend(window.portalui_childForm);
window.app_saku_kb_transaksi_nonpro_kb_fKboutk.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_kb_transaksi_nonpro_kb_fKboutk.prototype.ubah = function()
{
	...
	if (this.standarLib.checkEmptyByTag(this, new Array("0","2")))
	{
		try
		{
			...
			else
			{
				...
				sql.add("delete from angg_r where no_bukti ='"+this.cb_bukti.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' and modul = 'KBONP'");			
			...
			sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,"+
					...
					"('"+this.nb+"','"+this.app._lokasi+"','"+this.ed_dok.getText()+"','"+this.ed_giro.getText()+"','"+this.akunkb+"','"+this.dp_tgl1.getDate()+"','"+this.ed_desc.getText()+"','-','KBO_NP','"+this.cb_jenis.getText()+"','"+
			...
			sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
					...
					"('"+this.nb+"','"+this.ed_giro.getText()+"','"+this.dp_tgl1.getDate()+"',0,'"+this.akunkb+
			...
			if (this.gridJurnal.getLength() > 0)
			{
				...
				for (var i in this.gridJurnal.objList)
				{
					...
					scr1 += "('"+this.nb+"','KBONP','"+this.app._lokasi+"','"+line.get("kode_akun")+"','"+line.get("kode_pp")+"','"+line.get("kode_drk")+
...
window.app_saku_kb_transaksi_nonpro_kb_fKboutk.prototype.doModalResult = function(event, modalResult)
{
...
window.app_saku_kb_transaksi_nonpro_kb_fKboutk.prototype.genClick = function(sender)
{
	try
	{
...
window.app_saku_kb_transaksi_nonpro_kb_fKboutk.prototype.doSelect = function(sender, year, month, day)
{
	if (month < 10)
		month = "0"+month;
	this.ed_period.setText(year.toString()+month);
};
window.app_saku_kb_transaksi_nonpro_kb_fKboutk.prototype.doEditChange = function(sender)
{
...
window.app_saku_kb_transaksi_nonpro_kb_fKboutk.prototype.showClick = function(sender)
{
	if (this.cb_bukti.getText() != "")
	{
		if (this.cb_bukti != undefined) {
			try 
			{
				...
				var line, data = this.dbLib.runSQL(" select  a.jenis,a.no_dokumen, a.keterangan, a.kode_curr, a.kurs, a.nik_buat, b.nama as nama_pembuat, a.akun_kb,a.no_bg,a.periode,d.nama as nama_akun,a.posted,a.kode_bank,cc.nama as nama_bank,a.tanggal "+
...
window.app_saku_kb_transaksi_nonpro_kb_fKboutk.prototype.FindBtnClick = function(sender, event)
{
	try
	{
		if (sender == this.cb_bukti) 
		{
		    if (this.ed_period != "") 
			{
			  this.standarLib.showListData(this, "Daftar Bukti KasBank Periode "+this.ed_period.getText(),this.cb_bukti,undefined, 
												 "select no_kas, keterangan from kas_m where modul = 'KBO_NP' and posted = 'F' and periode='"+this.cb_perlama.getText()+"' and no_del='-' and kode_lokasi='"+this.app._lokasi+"'",
												 "select count(no_kas)      from kas_m where modul = 'KBO_NP' and posted = 'F' and periode='"+this.cb_perlama.getText()+"' and no_del='-' and kode_lokasi='"+this.app._lokasi+"'",
...
window.app_saku_kb_transaksi_nonpro_kb_fKboutk.prototype.doFindBtnClick = function(sender, col, row) 
{
	try
	{
		switch(col)
	...
	{
		alert("[app_saku_kb_transaksi_nonpro_kb_fKboutk] : doFindBtnClick : " + e);
	}
};
window.app_saku_kb_transaksi_nonpro_kb_fKboutk.prototype.doNilaiChange = function()
{
	try
	{
		var totD = totC = nKB = 0; 
	...
	{
		alert("[app_saku_kb_transaksi_nonpro_kb_fKboutk]::doNilaiChange:"+e);
	}
};
window.app_saku_kb_transaksi_nonpro_kb_fKboutk.prototype.doCellExit = function(sender, col, row) 
{
	try
	{
		switch(col)
...
window.app_saku_kb_transaksi_nonpro_kb_fKboutk.prototype.doCellEnter = function(sender, col, row) 
{
	try
	{
		switch(col)
	...
	{
		alert("app_saku_kb_transaksi_nonpro_kb_fKboutk::doCellEnter : " + e);
	}
};
window.app_saku_kb_transaksi_nonpro_kb_fKboutk.prototype.hitungGar = function()
{
...
window.app_saku_kb_transaksi_nonpro_kb_fKboutk.prototype.garClick = function(sender)
{
	try
	{
...
window.app_saku_kb_transaksi_nonpro_kb_fKboutk.prototype.doRequestReady = function(sender, methodName, result)
{
