window.app_saku_kb_transaksi_nonpro_depo_fDepotc = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_transaksi_nonpro_depo_fDepotc.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_kb_transaksi_nonpro_depo_fDepotc";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pencairan dan Penempatan Deposito Sebagian Non Proses: Input", 0);	
...
window.app_saku_kb_transaksi_nonpro_depo_fDepotc.extend(window.portalui_childForm);
window.app_saku_kb_transaksi_nonpro_depo_fDepotc.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_kb_transaksi_nonpro_depo_fDepotc.prototype.simpan = function()
{
	...
	if (this.standarLib.checkEmptyByTag(this, new Array("0","1","2","3")))
	{
		try
		{
			...
			sql.add("update depo_m set no_depolink='"+this.ed_nodepo.getText()+"',cnilai="+parseNilai(this.ed_ncair.getText())+",ctgl_just='"+this.dp_tgl3.getDate()+"',cjam='"+this.ed_jam.getText()+"',cpic_bank='"+this.ed_pic1.getText()+"',cpic_karyawan='"+this.ed_pic2.getText()+"',csurat='"+this.ed_nomor.getText()+"',cnik_setuju='"+this.cb_setuju.getText()+"',progress='9' where no_depo='"+this.cb_depolama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");			
			
			sql.add("insert into depo_m (no_depo,no_kas,no_dokumen,tanggal,due_date,tgl_akru,bank,cabang,rate,basis,"+
					"keterangan,catatan,kode_curr,kurs,akun_depo,akun_cbunga,nik_buat,nik_setuju,kode_lokasi,kode_pp,"+
					"modul,nilai,progress,periode,no_del,no_link,nik_user,tgl_input,tgl_just,jam,pic_bank,pic_karyawan,jk_waktu,akun_ar,akun_pdpt)  values "+
					...
					"'"+this.ed_desc.getText()+"','"+this.ed_nomor.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+",'"+this.cb_akundepo.getText()+"','"+this.cb_cbunga.getText()+"','"+this.cb_buat.getText()+"','"+this.cb_setuju.getText()+"','"+this.app._lokasi+"','-',"+
					"'DEPO_NP',"+parseNilai(this.ed_ntmp.getText())+",'2','"+this.ed_period.getText()+"','-','-','"+this.app._userLog+"',now(),'"+
					this.dp_tgl2.getDate()+"','"+this.ed_jam.getText()+"','"+this.ed_pic1.getText()+"','"+this.ed_pic2.getText()+"','"+this.ed_waktu.getText()+"','"+this.cb_ar.getText()+"','"+this.cb_pdpt.getText()+"')");
			...
			sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,"+
					...
					"('"+this.ed_nb.getText()+"','"+this.app._lokasi+"','"+this.ed_dok.getText()+"','"+this.ed_giro.getText()+"','"+this.cb_akun.getText()+"','"+this.dp_tgl1.getDate()+"','"+this.ed_desc.getText()+"','-','DEPCSB_NP','"+this.cb_jenis.getText()+"','"+
					     this.ed_period.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+","+parseNilai(this.ed_ncair.getText())+",'-','-',now(),'"+this.app._userLog+"','F','-','-','"+this.cb_depolama.getText()+"')");
			...
			sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
					...
					"'"+this.ed_nodepo.getText()+"','"+this.app._lokasi+"','DEPCSB_NP','KAS',"+
			...
			sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
					...
					"'"+this.ed_nodepo.getText()+"','"+this.app._lokasi+"','DEPCSB_NP','DEPO',"+
					"'"+this.ed_period.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+
					",'"+this.app._userLog+"',now())");
...
window.app_saku_kb_transaksi_nonpro_depo_fDepotc.prototype.doModalResult = function(event, modalResult)
{
...
window.app_saku_kb_transaksi_nonpro_depo_fDepotc.prototype.genClick = function(sender)
{
	try
	{
...
window.app_saku_kb_transaksi_nonpro_depo_fDepotc.prototype.doSelect = function(sender, year, month, day)
{
	if (month < 10)
		month = "0"+month;
	this.ed_period.setText(year.toString()+month);
};
window.app_saku_kb_transaksi_nonpro_depo_fDepotc.prototype.showClick2 = function(sender)
{
	try 
	{
		var line,data = this.dbLib.runSQL(" select a.bank,a.cabang,a.due_date,a.rate,a.basis,a.kode_curr,a.kurs,a.akun_depo,a.nilai,a.tanggal,a.no_dokumen,a.akun_cbunga,b.nama as nama_depo, c.nama as nama_cbunga, "+
										  "        a.akun_ar,a.akun_pdpt, d.nama as nama_ar, e.nama as nama_pdpt "+
		...
		if (data instanceof portalui_arrayMap)
		{
			...
			if (line != undefined)
			{
				...
				this.cb_cbunga.setRightLabelCaption(line.get("nama_cbunga"));
...
window.app_saku_kb_transaksi_nonpro_depo_fDepotc.prototype.doEditChange = function(sender)
{
...
window.app_saku_kb_transaksi_nonpro_depo_fDepotc.prototype.showClick = function(sender)
{
	try
	{
		if (this.cb_akun.getText() != "")
		{
			var line,data = this.dbLib.runSQL(" select nama,no_rek,nama_rek from bank2 where kode_akun = '"+this.cb_akun.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
			if (data instanceof portalui_arrayMap)
			{
				line = data.get(0);
				if (line != undefined)
		...
		{
			var line,data = this.dbLib.runSQL(" select nama,no_rek,nama_rek from bank2 where kode_akun = '"+this.cb_cbunga.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
			if (data instanceof portalui_arrayMap)
			{
				line = data.get(0);
				if (line != undefined)
...
window.app_saku_kb_transaksi_nonpro_depo_fDepotc.prototype.FindBtnClick = function(sender, event)
{
	try
	{
...
window.app_saku_kb_transaksi_nonpro_depo_fDepotc.prototype.doRequestReady = function(sender, methodName, result)
{
