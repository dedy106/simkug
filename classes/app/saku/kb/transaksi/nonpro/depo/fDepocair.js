window.app_saku_kb_transaksi_nonpro_depo_fDepocair = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_transaksi_nonpro_depo_fDepocair.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_kb_transaksi_nonpro_depo_fDepocair";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pencairan Deposito [KB Masuk] Non Proses: Input", 0);	
		...
		this.ed_ncair.setLeft(300);
		this.ed_ncair.setTop(364);
		...
		try
		{
			...
			this.standarLib.clearByTag(this,["0","1","2","9"],this.dp_tgl1);				
...
window.app_saku_kb_transaksi_nonpro_depo_fDepocair.extend(window.portalui_childForm);
window.app_saku_kb_transaksi_nonpro_depo_fDepocair.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_kb_transaksi_nonpro_depo_fDepocair.prototype.simpan = function()
{
	if ( (new Date()).strToDate(this.dp_tgl1.getDate())  < (new Date()).strToDate(this.dp_tgl3.getDate()))
	{
		system.alert(this,"Tanggal pencairan tidak valid.","Tanggal pencairan kurang tanggal penempatan.");
		return false;
	}
	this.bGen.click();
	if (this.standarLib.checkEmptyByTag(this, new Array("0","1")))
	{
		try
		{
			...
			{var stsFinal = "9";} else {var stsFinal = "2";}
			
			sql.add("update depo_m set cnilai=nilai,ctgl_just='"+this.dp_tgl3.getDate()+"',cjam='"+this.ed_jam.getText()+"',cpic_bank='"+this.ed_pic1.getText()+"',cpic_karyawan='"+this.ed_pic2.getText()+"',csurat='"+this.ed_nomor.getText()+"',cnik_setuju='"+this.cb_setuju.getText()+"',progress='"+stsFinal+"' where no_depo='"+this.cb_nodepo.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");			
			sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,"+
					...
					"('"+this.ed_nb.getText()+"','"+this.app._lokasi+"','"+this.ed_dok.getText()+"','"+this.ed_giro.getText()+"','"+this.cb_akun.getText()+"','"+this.dp_tgl1.getDate()+"','"+this.ed_desc.getText()+"','-','DEPC_NP','"+this.cb_jenis.getText()+"','"+
					     this.ed_period.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+","+parseNilai(this.ed_ncair.getText())+",'-','-',now(),'"+this.app._userLog+"','F','-','-','"+this.cb_nodepo.getText()+"')");
			...
			sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
					...
					"'"+this.cb_nodepo.getText()+"','"+this.app._lokasi+"','DEPC_NP','KAS',"+
			...
			sql.add("insert into kas_j (no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
					...
					"'"+this.cb_nodepo.getText()+"','"+this.app._lokasi+"','DEPC_NP','DEPO',"+
					"'"+this.ed_period.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+
					",'"+this.app._userLog+"',now())");
...
window.app_saku_kb_transaksi_nonpro_depo_fDepocair.prototype.showClick2 = function(sender)
{
	try
	{
...
window.app_saku_kb_transaksi_nonpro_depo_fDepocair.prototype.doModalResult = function(event, modalResult)
{
	...
	switch (event)
	{
		case "clear" :
			if (modalResult == mrOk)
			{
				this.standarLib.clearByTag(this,["0","1","2","9"],undefined);				
				this.doSelect(this.dp_tgl1,this.dp_tgl1.year,this.dp_tgl1.month,this.dp_tgl1.day);
				this.cb_curr.setText("IDR"); this.ed_kurs.setText("1");
			}
			break;
		...
		case "simpan" :
			...
			if (nilaiToFloat(this.ed_nilai.getText()) < nilaiToFloat(this.ed_ncair.getText())) 
			{
				system.alert(this,"Nilai pencairan tidak valid.","Nilai pencairan melebihi nilai deposito.");
				return false;
			}
			if (parseFloat(this.app._periode) < parseFloat(this.ed_period.getText())) 
			{
...
window.app_saku_kb_transaksi_nonpro_depo_fDepocair.prototype.genClick = function(sender)
{
	try
	{
...
window.app_saku_kb_transaksi_nonpro_depo_fDepocair.prototype.doSelect = function(sender, year, month, day)
{
	if (month < 10)
		month = "0"+month;
	this.ed_period.setText(year.toString()+month);
};
window.app_saku_kb_transaksi_nonpro_depo_fDepocair.prototype.showClick = function(sender)
{
	try 
	{
		var line,data = this.dbLib.runSQL(" select a.bank,a.cabang,a.due_date,a.rate,a.basis,a.kode_curr,a.kurs,a.akun_depo,a.nilai,a.tanggal,a.no_dokumen,a.akun_cbunga "+
										  " from depo_m a "+
...
window.app_saku_kb_transaksi_nonpro_depo_fDepocair.prototype.doEditChange = function(sender)
{
...
window.app_saku_kb_transaksi_nonpro_depo_fDepocair.prototype.FindBtnClick = function(sender, event)
{
	try
	{
...
window.app_saku_kb_transaksi_nonpro_depo_fDepocair.prototype.doRequestReady = function(sender, methodName, result)
{
