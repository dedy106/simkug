window.app_saku_kb_transaksi_proses_pj_fPtgaju= function(owner){
	if (owner){
		window.app_saku_kb_transaksi_proses_pj_fPtgaju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_kb_transaksi_proses_pj_fPtgaju";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan Pertanggungan: Input", 0);	
		uses("portalui_datePicker;portalui_checkBox;portalui_saiGrid;portalui_sgNavigator;util_addOnLib;util_gridLib;app_saku_fJurnalViewer;server_report_report;portalui_reportViewer");
		...
		this.cb_nopj.setSQL("select no_pj, keterangan  from panjar_m where  catatan ='-' and kode_lokasi = '"+this.app._lokasi+"' and progress='2' and modul='PJR_P'",["no_pj","keterangan"],true);		
		...
		*/
		...
		this.sgn.setGrid(this.sg1);
		this.sgn.setButtonStyle(2);
		this.cb_preview = new portalui_checkBox(this.sgn,{bound:[this.p1.width - 100,2,16],caption:"Preview",checked:true});
...
window.app_saku_kb_transaksi_proses_pj_fPtgaju.extend(window.portalui_childForm);
window.app_saku_kb_transaksi_proses_pj_fPtgaju.prototype.mainButtonClick = function(sender){
...
window.app_saku_kb_transaksi_proses_pj_fPtgaju.prototype.simpan = function(){
	...
	if (this.standarLib.checkEmptyByTag(this, new Array("0"))){
		try{				
			...
			sql.add("insert into ptg_m (no_ptg,no_pj,no_kas,no_dokumen,tanggal,"+
					...
					this.ed_desc.getText()+"','-','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+",'"+this.akunpj+"','-','"+this.cb_pemohon.getText()+"','"+this.cb_setuju.getText()+"','"+this.app._lokasi+"','"+this.cb_pp.getText()+
			...
			for (var i=0; i < this.sg1.rows.getLength(); i++){
				if (this.sg1.rowValid(i)) {
					scr1 = "insert into ptg_j (no_ptg,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,"+
...
window.app_saku_kb_transaksi_proses_pj_fPtgaju.prototype.doModalResult = function(event, modalResult){
...
window.app_saku_kb_transaksi_proses_pj_fPtgaju.prototype.genClick = function(sender){
	try{
...
window.app_saku_kb_transaksi_proses_pj_fPtgaju.prototype.doSelect = function(sender, year, month, day){
	if (month < 10) month = "0"+month;
	this.ed_period.setText(year.toString()+month);
	this.loadMasterData();
};
window.app_saku_kb_transaksi_proses_pj_fPtgaju.prototype.doEditChange = function(sender){
...
window.app_saku_kb_transaksi_proses_pj_fPtgaju.prototype.showClick = function(sender){
	try {
		if (this.cb_nopj.getText() != ""){
			var line,data = this.dbLib.runSQL(" select a.keterangan,a.nik_pengaju,b.nama as nama_pengaju,a.kode_curr,a.kurs,a.nilai,a.periode,a.akun_pj "+
...
window.app_saku_kb_transaksi_proses_pj_fPtgaju.prototype.doCellExit= function(sender, col, row) {
	try{
		switch(col){
			case 3 : 
			case 4 : 
...
window.app_saku_kb_transaksi_proses_pj_fPtgaju.prototype.doCellEnter= function(sender, col, row) {
	try{
		switch(col){
			case 2 : 
...
window.app_saku_kb_transaksi_proses_pj_fPtgaju.prototype.FindBtnClick = function(sender, event){
	try{
...
*/
window.app_saku_kb_transaksi_proses_pj_fPtgaju.prototype.doFindBtnClick = function(sender, col, row) {
	try{
		switch(col){
			case 0 : 
...
window.app_saku_kb_transaksi_proses_pj_fPtgaju.prototype.doNilaiChange = function(){
	try{
		var totD = totC = nKB = 0; 
		for (var i = 0; i < this.sg1.rows.getLength();i++){
			if (this.sg1.rowValid(i)) {
	...
	{
		alert("[app_saku_kb_transaksi_proses_pj_fPtgaju]::doNilaiChange:"+e);
	}
};
window.app_saku_kb_transaksi_proses_pj_fPtgaju.prototype.doSgChange = function(sender, col, row){
   try{
...
window.app_saku_kb_transaksi_proses_pj_fPtgaju.prototype.hitungGar = function(){
...
window.app_saku_kb_transaksi_proses_pj_fPtgaju.prototype.garClick = function(sender){
	try{
...
window.app_saku_kb_transaksi_proses_pj_fPtgaju.prototype.loadMasterData = function(){
...
window.app_saku_kb_transaksi_proses_pj_fPtgaju.prototype.doRequestReady = function(sender, methodName, result){
			...
			switch(methodName){
				...
				if (result.toLowerCase().search("error") == -1)					{
					...
					if (this.cb_preview.isChecked()) {
								this.nama_report="server_report_kb_rptPjPtg";
								this.filter1 = " where a.kode_lokasi='"+this.app._lokasi+"' and a.no_ptg='"+this.ed_nb.getText()+"' ";
								this.filter2 = "";			
								this.viewer.prepare();
