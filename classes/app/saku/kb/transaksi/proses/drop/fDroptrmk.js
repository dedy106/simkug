window.app_saku_kb_transaksi_proses_drop_fDroptrmk = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_transaksi_proses_drop_fDroptrmk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_kb_transaksi_proses_drop_fDroptrmk";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penerimaan Droping Dana: Koreksi", 0);
		...
		this.ed_nilai.setCaption("Nilai Terima");
...
window.app_saku_kb_transaksi_proses_drop_fDroptrmk.extend(window.portalui_childForm);
window.app_saku_kb_transaksi_proses_drop_fDroptrmk.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_kb_transaksi_proses_drop_fDroptrmk.prototype.ubah = function()
{
	this.bGen.click();
	if (this.standarLib.checkEmptyByTag(this,["0","1"]))
	{
		try
		{
			...
			{
				sql.add(" update droptrm_m set no_del = 'DEL' where no_terima ='"+this.cb_bukti.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");			
				this.nb = this.ed_nb.getText();
			}
			else
			{
				sql.add(" delete from droptrm_m where no_terima ='"+this.cb_bukti.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");			
				sql.add(" delete from droptrm_j where no_terima ='"+this.cb_bukti.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");	
			...
			sql.add("insert into droptrm_m (no_terima,no_kirim,no_dokumen,no_kas,tanggal,akun_tak,"+
					...
					"('"+this.nb+"','"+this.cb_nokirim.getText()+"','"+this.ed_dok.getText()+"','-','"+this.dp_tgl1.getDate()+"',"+
...
window.app_saku_kb_transaksi_proses_drop_fDroptrmk.prototype.doModalResult = function(event, modalResult)
{
	...
	switch (event)
	{
		...
		case "hapus" :
			...
			{	
				...
				sql.add("update dropkrm_m set progress='2' where no_kirim = '"+this.cb_nokirim.getText()+"' and kode_lokasi='"+this.cb_lokasi.getText()+"' ");
				if (parseFloat(this.periodeLama) < parseFloat(this.app._periode))
				{
					sql.add(" update droptrm_m set no_del = 'DEL' where no_terima ='"+this.cb_bukti.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");			
				}
...
window.app_saku_kb_transaksi_proses_drop_fDroptrmk.prototype.genClick = function(sender)
{
	try
	{
...
window.app_saku_kb_transaksi_proses_drop_fDroptrmk.prototype.doSelect = function(sender, year, month, day)
{
	if (month < 10)
		month = "0"+month;
	this.ed_period.setText(year.toString()+month);
};
window.app_saku_kb_transaksi_proses_drop_fDroptrmk.prototype.doEditChange = function(sender)
{
...
window.app_saku_kb_transaksi_proses_drop_fDroptrmk.prototype.showClick = function(sender)
{
	try 
	{
		if (this.cb_bukti.getText() != "")
		{
			var line,data = this.dbLib.runSQL(" select a.no_kirim,x.periode,a.keterangan as ket,x.kode_curr,x.kurs,a.kode_lokasi,d.nama as nama_lokasi,"+
											  "        x.no_dokumen,x.keterangan,x.nik_buat,x.nik_setuju,y.nama as nama_buat,z.nama as nama_app,x.nilai,x.akun_tak,w.nama as nama_akun "+
											  " from dropkrm_m a inner join lokasi d on a.kode_lokasi=d.kode_lokasi "+
											  "                  inner join droptrm_m x on a.no_kirim=x.no_kirim and a.kode_lokasi=x.lok_asal "+
											  "                  inner join masakun w on x.akun_tak=w.kode_akun and x.kode_lokasi=w.kode_lokasi "+
											  "                  inner join karyawan y on y.nik=x.nik_buat and y.kode_lokasi=x.kode_lokasi "+
											  "                  inner join karyawan z on z.nik=x.nik_setuju and z.kode_lokasi=x.kode_lokasi "+
											  " where x.no_terima = '"+this.cb_bukti.getText()+"' and x.kode_lokasi='"+this.app._lokasi+"' and x.no_del='-'");
			...
			if (data instanceof portalui_arrayMap)
			{
				...
				if (line != undefined)
				{
					...
					this.cb_nokirim.setText(line.get("no_kirim"));
...
window.app_saku_kb_transaksi_proses_drop_fDroptrmk.prototype.FindBtnClick = function(sender, event)
{
	try
	{
		if (sender == this.cb_bukti) 
		{
		    if (this.ed_period != "") 
			{
				this.standarLib.showListData(this, "Daftar Bukti Terima",this.cb_bukti,undefined, 
				  								 "select no_terima, keterangan from droptrm_m where progress = '0' and periode='"+this.cb_perlama.getText()+"' and no_del='-' and kode_lokasi='"+this.app._lokasi+"' ",
												 "select count(no_terima)      from droptrm_m where progress = '0' and periode='"+this.cb_perlama.getText()+"' and no_del='-' and kode_lokasi='"+this.app._lokasi+"' ",
												 ["no_terima","keterangan"],"and",["No Terima","Keterangan"],false);
			}
...
window.app_saku_kb_transaksi_proses_drop_fDroptrmk.prototype.doRequestReady = function(sender, methodName, result)
{
