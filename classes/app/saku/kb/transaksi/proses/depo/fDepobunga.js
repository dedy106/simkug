window.app_saku_kb_transaksi_proses_depo_fDepobunga = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_transaksi_proses_depo_fDepobunga.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_kb_transaksi_proses_depo_fDepobunga";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penerimaan Bunga Deposito: Input", 0);	
...
window.app_saku_kb_transaksi_proses_depo_fDepobunga.extend(window.portalui_childForm);
window.app_saku_kb_transaksi_proses_depo_fDepobunga.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_kb_transaksi_proses_depo_fDepobunga.prototype.simpan = function()
{
	this.bGen.click();
	if (this.standarLib.checkEmptyByTag(this, ["0","1"]))
	{
		try
		{
			...
			for (var i=1; i <= this.sg1.getRowCount(); i++)
			{
				...
				sql.add("insert into depo_d_d (no_terima,no_depo,no_akru,kode_lokasi,progress) values ('"+this.ed_nb.getText()+"','"+this.cb_nodepo.getText()+"','"+this.sg1.getCell(1,i)+"','"+this.app._lokasi+"','1')");			
			}
			...
			if (this.ed_nsls.getText != '0') 
			{
				...
				sql.add("insert into depo_j (no_terima,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
						"kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+	
						"('"+this.ed_nb.getText()+"','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDate()+"',"+j+",'"+this.cb_sls.getText()+
			...
			{	
				...
				sql.add("insert into depo_j (no_terima,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
						"kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+	
						"('"+this.ed_nb.getText()+"','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDate()+"',"+j+",'"+akun_badm+
			...
			if (this.ed_npph.getText != '0') 
			{
				...
				sql.add("insert into depo_j (no_terima,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,ref1,"+
						"kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+	
						"('"+this.ed_nb.getText()+"','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDate()+"',"+j+",'"+akun_bpph+
...
window.app_saku_kb_transaksi_proses_depo_fDepobunga.prototype.doModalResult = function(event, modalResult)
{
...
window.app_saku_kb_transaksi_proses_depo_fDepobunga.prototype.genClick = function(sender)
{
	try
	{
...
window.app_saku_kb_transaksi_proses_depo_fDepobunga.prototype.doSelect = function(sender, year, month, day)
{
	if (month < 10)
		month = "0"+month;
	this.ed_period.setText(year.toString()+month);
};
window.app_saku_kb_transaksi_proses_depo_fDepobunga.prototype.showClick = function(sender)
{
	try 
	{
		var line,data = this.dbLib.runSQL(" select a.bank,a.cabang,a.rate,a.basis,a.kode_curr,a.kurs,a.akun_cbunga,a.nilai,a.no_dokumen,b.nama as nama_bankcbunga,a.kode_bankcbunga "+
										  " from depo_m a inner join bank2 b on a.kode_bankcbunga=b.kode_bank and a.kode_lokasi=b.kode_lokasi "+
...
window.app_saku_kb_transaksi_proses_depo_fDepobunga.prototype.doEditChange2 = function(sender)
{		
	try
	{
		if ( (this.ed_ncair.getText() != undefined) && (this.ed_nbunga.getText() != undefined) && (this.ed_nadm.getText() != undefined) && (this.ed_npph.getText() != undefined) )
			...
			//this.ed_nsls.setText(format_number(parseFloat(nilaiToFloat(this.ed_ncair.getText()) + nilaiToFloat(this.ed_nadm.getText()) + nilaiToFloat(this.ed_npph.getText()) - nilaiToFloat(this.ed_nbunga.getText())),2,',','.')); koma dibuang !!!			
	} catch(e)
	{
		alert(e);
	}
};
window.app_saku_kb_transaksi_proses_depo_fDepobunga.prototype.doEditChange = function(sender)
{
...
window.app_saku_kb_transaksi_proses_depo_fDepobunga.prototype.FindBtnClick = function(sender, event)
{
	try
	{
...
window.app_saku_kb_transaksi_proses_depo_fDepobunga.prototype.doRequestReady = function(sender, methodName, result)
{
