window.app_saku_kb_transaksi_proses_depo_fDepocairk = function(owner)
{
	if (owner)
	{
		window.app_saku_kb_transaksi_proses_depo_fDepocairk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_kb_transaksi_proses_depo_fDepocairk";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pencairan Deposito: Koreksi", 0);	
...
window.app_saku_kb_transaksi_proses_depo_fDepocairk.extend(window.portalui_childForm);
window.app_saku_kb_transaksi_proses_depo_fDepocairk.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_kb_transaksi_proses_depo_fDepocairk.prototype.ubah = function()
{
	...
	if (this.standarLib.checkEmptyByTag(this,["0","1","2","3"]))
	{
		try
		{
			...
			sql.add("insert into depocair_m (no_cair,no_kas,no_depo,kode_lokasi,no_dokumen,akun_kb,tanggal,keterangan,modul,progress,"+
					...
					"('"+this.nb+"','-','"+this.cb_nodepo.getText()+"','"+this.app._lokasi+"','"+this.ed_dok.getText()+"','"+this.akunkb+"','"+this.dp_tgl1.getDate()+"','"+this.ed_desc.getText()+"','DEPC_P','0',"+
...
window.app_saku_kb_transaksi_proses_depo_fDepocairk.prototype.showClick2 = function(sender)
{
	try
	{
...
window.app_saku_kb_transaksi_proses_depo_fDepocairk.prototype.doModalResult = function(event, modalResult)
{
	...
	switch (event)
	{
		...
		case "hapus" :
			...
			{	
				...
				sql.add("update depo_m set cnilai=0,cjam='-',cpic_bank='-',cpic_karyawan='-',csurat='-',cnik_setuju='-',progress='2',cakun_kb='-',kode_bankcair='-' "+
				        "where no_depo='"+this.cb_nodepo.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");						
				if (parseFloat(this.periodeLama) < parseFloat(this.app._periode))
				{
					sql.add(" update depocair_m set no_del = 'DEL' where no_cair ='"+this.cb_bukti.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");			
				}
...
window.app_saku_kb_transaksi_proses_depo_fDepocairk.prototype.genClick = function(sender)
{
	try
	{
...
window.app_saku_kb_transaksi_proses_depo_fDepocairk.prototype.doSelect = function(sender, year, month, day)
{
	if (month < 10)
		month = "0"+month;
	this.ed_period.setText(year.toString()+month);
};
window.app_saku_kb_transaksi_proses_depo_fDepocairk.prototype.loadClick = function(sender)
{
	try 
	{		
		var line,data = this.dbLib.runSQL(" select a.bank,a.cabang,a.tanggal as tgl_depo,a.due_date,a.rate,a.basis,a.akun_depo,a.nilai as nilai_depo,a.no_dokumen as bilyet,a.akun_cbunga,a.kode_bankcbunga, "+
										  "        b.no_dokumen,b.keterangan,b.no_depo,b.nik_buat,b.nik_app,b.akun_kb,b.kode_bank,b.tgl_just,b.jam,b.pic_bank,b.pic_karyawan,b.no_surat,b.periode,b.kode_curr,b.kurs,b.nilai, "+
										  "        c.nama as nama_akun,g.nama as nama_buat, h.nama as nama_app,cc.nama as nama_bank "+
										  ...
										  "               inner join bank2 cc on b.kode_bank = cc.kode_bank and b.kode_lokasi=cc.kode_lokasi "+
...
window.app_saku_kb_transaksi_proses_depo_fDepocairk.prototype.doEditChange = function(sender)
{
...
window.app_saku_kb_transaksi_proses_depo_fDepocairk.prototype.FindBtnClick = function(sender, event)
{
	try
	{
		if (sender == this.cb_bukti) 
		{
		    if (this.ed_period != "") 
			{
				this.standarLib.showListData(this, "Daftar Bukti Pencairan Deposito",this.cb_bukti,undefined, 
				  								 "select no_cair, keterangan from depocair_m where modul = 'DEPC_P' and progress = '0' and periode='"+this.cb_perlama.getText()+"' and no_del='-' and kode_lokasi='"+this.app._lokasi+"'",
												 "select count(no_cair)      from depocair_m where modul = 'DEPC_P' and progress = '0' and periode='"+this.cb_perlama.getText()+"' and no_del='-' and kode_lokasi='"+this.app._lokasi+"'",
...
window.app_saku_kb_transaksi_proses_depo_fDepocairk.prototype.doRequestReady = function(sender, methodName, result)
{
