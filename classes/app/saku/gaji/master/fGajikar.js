window.app_saku_gaji_master_fGajikar = function(owner)
{
	if (owner)
	{
		window.app_saku_gaji_master_fGajikar.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_gaji_master_fGajikar";
		this.maximize();
...
window.app_saku_gaji_master_fGajikar.extend(window.portalui_childForm);
window.app_saku_gaji_master_fGajikar.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_gaji_master_fGajikar.prototype.doModalResult = function(event, modalResult)
{
	switch (event)
	{
		case "clear" :
		...
		*/
		case "ubah" :
			if (modalResult == mrOk)
			{
					...
					var scr1 = "insert into gaji_kar_m (no_dok,kode_param,kode_lokkonsol,nik,nilai) values ";
					var baris1 = true;
					for (var i=0; i < this.sg1.rows.getLength(); i++)
					{
						if (!baris1) { scr1 += ",";}	
						scr1 += "('"+this.cb_dok.getText()+"','"+this.sg1.getCell(0,i)+"','"+this.lokkonsol+"','"+this.cb_nik.getText()+"',"+parseNilai(this.sg1.getCell(2,i))+")";
						baris1 = false;
					}
					for (var i=0; i < this.sg2.rows.getLength(); i++)
					{
						if (!baris1) { scr1 += ",";}	
						scr1 += "('"+this.cb_dok.getText()+"','"+this.sg2.getCell(0,i)+"','"+this.lokkonsol+"','"+this.cb_nik.getText()+"',"+parseNilai(this.sg2.getCell(2,i))+")";
						baris1 = false;
					}					
					sql.add(scr1);			
					this.dbLib.execArraySQL(sql);	
...
window.app_saku_gaji_master_fGajikar.prototype.showClick = function(sender)
{	
...
window.app_saku_gaji_master_fGajikar.prototype.doEditChange = function(sender)
{	
...
window.app_saku_gaji_master_fGajikar.prototype.FindBtnClick = function(sender, event)
{
	try
	{
...
window.app_saku_gaji_master_fGajikar.prototype.doRequestReady = function(sender, methodName, result)
{
