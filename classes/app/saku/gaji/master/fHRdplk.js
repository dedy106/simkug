window.app_saku_gaji_master_fHRdplk = function(owner)
{
	if (owner)
	{
		window.app_saku_gaji_master_fHRdplk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_gaji_master_fHRdplk";
		this.maximize();
...
window.app_saku_gaji_master_fHRdplk.extend(window.portalui_childForm);
window.app_saku_gaji_master_fHRdplk.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_gaji_master_fHRdplk.prototype.doModalResult = function(event, modalResult)
{
	switch (event)
	{
		case "clear" :
		...
		case "simpan" :
			if (modalResult == mrOk)
			{
				...
				{					
					...
					var scr1 = "insert into hr_dplk (nu,batasmin,batasmax,persen,jenis,kode_lokkonsol) values ";
					var baris1 = true;
					for (var i=0; i < this.sg1.rows.getLength(); i++)
					{
						if (!baris1) { scr1 += ",";}	
						scr1 += "("+i+","+parseNilai(this.sg1.getCell(0,i))+","+parseNilai(this.sg1.getCell(1,i))+","+parseNilai(this.sg1.getCell(2,i))+",'"+this.cb_jenis.getText()+"','"+this.lokkonsol+"')";
						baris1 = false;
					}	
					sql.add(scr1);
...
window.app_saku_gaji_master_fHRdplk.prototype.showClick = function(sender)
{
	this.sg1.clear(); 
...
window.app_saku_gaji_master_fHRdplk.prototype.doRequestReady = function(sender, methodName, result)
{
