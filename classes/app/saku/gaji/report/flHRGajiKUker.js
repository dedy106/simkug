window.app_saku_gaji_report_flHRGajiKUker = function(owner)
{
	try{
	if (owner)
	{
		window.app_saku_gaji_report_flHRGajiKUker.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gaji_report_flHRGajiKUker";
		this.maximize();
...
window.app_saku_gaji_report_flHRGajiKUker.extend(window.portalui_childForm);
window.app_saku_gaji_report_flHRGajiKUker.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			}else
			{
				...
				this.title = new server_util_arrayList({items:["Kode","Unit Kerja","Periode "+this.sg1.getCell(2,1),"Periode "+this.sg1.getCell(2,2),"Deviasi"]});
			...
			systemAPI.alert("[app_saku_gaji_report_flHRGajiKUker]::mainButtonClick:"+e);
		}
	},
