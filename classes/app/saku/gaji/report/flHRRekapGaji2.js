window.app_saku_gaji_report_flHRRekapGaji2 = function(owner)
{
	if (owner)
	{
		window.app_saku_gaji_report_flHRRekapGaji2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gaji_report_flHRRekapGaji2";
		this.maximize();
...
window.app_saku_gaji_report_flHRRekapGaji2.extend(window.portalui_childForm);
window.app_saku_gaji_report_flHRRekapGaji2.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 0)
	{
...
window.app_saku_gaji_report_flHRRekapGaji2.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_saku_gaji_report_flHRRekapGaji2.prototype.mainButtonClick = function(sender)
{
	try
	{
	...
	{
		alert("[app_saku_gaji_report_flHRRekapGaji2]::mainButtonClick:"+e);
	}
};
window.app_saku_gaji_report_flHRRekapGaji2.prototype.jenisReport = function(jenis,filter)
{
	try
	{
		...
		if (jenis === "Pendapatan")
		{
				var sql = "select a.nik,a.nama,sum(b.gdas) as gdas,sum(b.tyay) as tyay,sum(tjab) as tjab,sum(b.tprof) as tprof,sum(b.pdas) as pdas,sum(b.thrt) as thrt, "+
						  ...
						  "			sum(b.gdas+b.tyay+b.tjab+b.tprof+b.thrt+b.tran+b.tkhs+b.bpls+b.lmbr+b.kbkt+b.ph21) as total "+
						  "	from karyawan a "+
		...
		}else if (jenis === "Semua")
		{
				var sql = "select a.nik,a.nama,sum(b.gdas) as gdas,sum(b.tyay) as tyay,sum(tjab) as tjab,sum(b.tprof) as tprof,sum(b.pdas) as pdas,sum(b.thrt) as thrt, "+
						  "		   sum(b.tran) as tran,sum(b.tkhs) as tkhs,sum(b.bpls) as bpls,sum(b.lmbr) as lmbr,sum(b.kbkt) as kbkt,sum(b.ph21) as ph21, "+
						  "			sum(b.gdas+b.tyay+b.tjab+b.tprof+b.thrt+b.tran+b.tkhs+b.bpls+b.lmbr+b.kbkt+b.ph21) as total_pdpt, "+
						  ...
						  "			sum(b.gdas+b.tyay+b.tjab+b.tprof+b.thrt+b.tran+b.tkhs+b.bpls+b.lmbr+b.kbkt+b.ph21) -sum(b.ppph+b.ptht+b.pinl+b.pkes+b.kosb+b.koct+b.kopt+b.byd1) as total_bersih"+ 
						  "	from karyawan a "+
						  "	inner join (select a.nik,case a.kode_param when 'GDAS' then nilai else 0 end as gdas, "+
						  "		   case a.kode_param when 'TYAY' then nilai else 0 end as tyay, "+
						  "		   case a.kode_param when 'TJAB' then nilai else 0 end as tjab, "+
	...
	{
		alert("[app_saku_gaji_report_flHRRekapGaji2]::jenisReport:"+e);
	}
};
window.app_saku_gaji_report_flHRRekapGaji2.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_gaji_report_flHRRekapGaji2.prototype.doSelectedPage = function(sender, page)
{
...
window.app_saku_gaji_report_flHRRekapGaji2.prototype.previewReport = function(dthtml,jenis)
{
...
window.app_saku_gaji_report_flHRRekapGaji2.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_gaji_report_flHRRekapGaji2.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_gaji_report_flHRRekapGaji2.prototype.doRowPerPageChange = function(sender, rowperpage)
{
