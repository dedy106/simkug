window.app_saku_gaji_report_flHRSlipPoltek = function(owner)
{
	if (owner)
	{
		window.app_saku_gaji_report_flHRSlipPoltek.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gaji_report_flHRSlipPoltek";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gaji_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku_gaji_report_flHRSlipPoltek.extend(window.portalui_childForm);
window.app_saku_gaji_report_flHRSlipPoltek.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 0)
	{
...
window.app_saku_gaji_report_flHRSlipPoltek.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_saku_gaji_report_flHRSlipPoltek.prototype.mainButtonClick = function(sender)
{
	try
	{
	...
	{
		alert("[app_saku_gaji_report_flHRSlipPoltek]::mainButtonClick:"+e);
	}
};
window.app_saku_gaji_report_flHRSlipPoltek.prototype.jenisReport = function(jenis,filter)
{
	try
	{
		...
		else
		{
			this.filter = this.filterRep.filterStr("b.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							  this.filterRep.filterStr("b.kode_loker",this.sg1.getCell(1,2),this.sg1.getCell(2,3),this.sg1.getCell(3,2),"and")+
							  this.filterRep.filterStr("b.nik",this.tanda,this.nik,this.nik,"and");		

		}
		
		this.showFilter = this.filterRep.showFilter(this.sg1);
		this.filter2 = this.sg1.getCell(2,4)+"/"+this.sg1.getCell(2,1);
		this.nama_report="server_report_gaji_rptHRSlipPoltek";
...
window.app_saku_gaji_report_flHRSlipPoltek.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_gaji_report_flHRSlipPoltek.prototype.doSelectedPage = function(sender, page)
{
...
window.app_saku_gaji_report_flHRSlipPoltek.prototype.previewReport = function(dthtml,jenis)
{
	
	this.viewer.preview(html);
	this.allHtml = html;
};
window.app_saku_gaji_report_flHRSlipPoltek.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_gaji_report_flHRSlipPoltek.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_gaji_report_flHRSlipPoltek.prototype.doRowPerPageChange = function(sender, rowperpage)
{
...
window.app_saku_gaji_report_flHRSlipPoltek.prototype.doConfirmClick = function(sender){
	try{		
