window.app_saku_gaji_report_flHRRekapGaji = function(owner)
{
	if (owner)
	{
		window.app_saku_gaji_report_flHRRekapGaji.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gaji_report_flHRRekapGaji";
		this.maximize();
...
window.app_saku_gaji_report_flHRRekapGaji.extend(window.portalui_childForm);
window.app_saku_gaji_report_flHRRekapGaji.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 0)
	{
...
window.app_saku_gaji_report_flHRRekapGaji.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_saku_gaji_report_flHRRekapGaji.prototype.mainButtonClick = function(sender)
{
	try
	{
	...
	{
		alert("[app_saku_gaji_report_flHRRekapGaji]::mainButtonClick:"+e);
	}
};
window.app_saku_gaji_report_flHRRekapGaji.prototype.jenisReport = function(jenis,filter)
{
	try
	{
		...
		this.scriptSqlCount = "select count(a.nik) "+
				...
				var sql = "select a.nik,a.nama,sum(b.gdas) as gdas,sum(b.tyay) as tyay,sum(tjab) as tjab,sum(b.tprof) as tprof,sum(b.pdas) as pdas,sum(b.thrt) as thrt, "+
						  ...
						  "		   sum(b.koct) as koct,sum(b.kopt) as kopt,sum(b.byd1) as byd1,sum(b.zakat) as zakat,sum(b.infaq) as infaq,sum(b.wakaf) as wakaf,sum(b.pll) as pll, "+
						  "			sum(b.ppph+b.ptht+b.pinl+b.pkes+b.kosb+b.koct+b.kopt+b.byd1+b.zakat+b.infaq+b.wakaf+b.pll) as total_pot,sum(b.insentif) as insentif, "+
						  "			sum(b.gdas+b.tyay+b.tjab+b.tprof+b.pdas+b.thrt+b.tran+b.tkhs+b.bpls+b.lmbr+b.kbkt+b.tcut+b.rapl+b.ipddk+b.cuta+b.ph21+b.insentif) -sum(b.ppph+b.ptht+b.pinl+b.pkes+b.kosb+b.koct+b.kopt+b.byd1+b.zakat+b.infaq+b.wakaf+b.pll) as total_bersih "+ 
						  "	from karyawan a "+
						  ...
						  "		   case a.kode_param when 'ZAKAT' then nilai else 0 end as zakat, "+
						  "		   case a.kode_param when 'INFAQ' then nilai else 0 end as infaq, "+
						  "		   case a.kode_param when 'WAKAF' then nilai else 0 end as wakaf, "+
				...
				title.add("Infaq");width.add(80);fieldType.add("N");
				title.add("Wakaf");width.add(80);fieldType.add("N");
	...
	{
		alert("[app_saku_gaji_report_flHRRekapGaji]::jenisReport:"+e);
	}
};
window.app_saku_gaji_report_flHRRekapGaji.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_gaji_report_flHRRekapGaji.prototype.doSelectedPage = function(sender, page)
{
...
window.app_saku_gaji_report_flHRRekapGaji.prototype.previewReport = function(dthtml,jenis)
{
...
window.app_saku_gaji_report_flHRRekapGaji.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_gaji_report_flHRRekapGaji.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_gaji_report_flHRRekapGaji.prototype.doRowPerPageChange = function(sender, rowperpage)
{
...
window.app_saku_gaji_report_flHRRekapGaji.prototype.initColumn = function()
{
	try
	{
