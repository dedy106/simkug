window.app_saku_gaji_report_flHRSppGaji = function(owner)
{
	if (owner)
	{
		window.app_saku_gaji_report_flHRSppGaji.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_gaji_report_flHRSppGaji";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan SPP Gaji", 2);
	...
	strSql="select max(periode) as periode from spb_m where jenis='SPPGJ' and kode_lokasi='"+this.app._lokasi+"'"; 
...
window.app_saku_gaji_report_flHRSppGaji.extend(window.portalui_childForm);
window.app_saku_gaji_report_flHRSppGaji.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==0)
	{
...
window.app_saku_gaji_report_flHRSppGaji.prototype.doSelectCell = function(sender, col, row)
{	
...
window.app_saku_gaji_report_flHRSppGaji.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					...
					" and a.jenis='SPPGJ' ";
		this.nama_report="server_report_gaji_rptSppGaji";
...
window.app_saku_gaji_report_flHRSppGaji.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_gaji_report_flHRSppGaji.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.lokasi, this.showFilter,this.filter2);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_gaji_report_flHRSppGaji.prototype.doCloseReportClick = function(sender)
{
  switch(sender.getName())
  {
    ...
    case "pdfBtn" :
      ...
      break;
    case "xlsBtn" :
      this.viewer.useIframe(this.report.createXls(this.nama_report,this.filter,(this.allBtn ? 1 : this.page), (this.allBtn ? this.viewer.getTotalPage() * this.pager : this.pager), this.lokasi,this.showFilter,this.pager));       
      break; 
	case "PreviewBtn" :
...
window.app_saku_gaji_report_flHRSppGaji.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_gaji_report_flHRSppGaji.prototype.doRowPerPageChange = function(sender, rowperpage)
{
