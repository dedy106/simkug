window.app_saku_gaji_transaksi_fEditUploadPot = function(owner)
{
	if (owner)
	{
		window.app_saku_gaji_transaksi_fEditUploadPot.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku_gaji_transaksi_fEditUploadPot";
...
window.app_saku_gaji_transaksi_fEditUploadPot.extend(window.portalui_childForm);
window.app_saku_gaji_transaksi_fEditUploadPot.implement({
			...
			case "simpan" :
					try{
						...
						for (var c = 2;c < this.sg1.getColCount();c++){
							...
							sqlText[sqlText.length] = "insert into gaji_load_pot_d (no_bukti, nik,kode_param, nilai,kode_lokasi,periode)values";
							first.push(true);
						}
						...
						for (var i in sqlArray){						
                            if (sqlText[i] != "insert into gaji_load_pot_d (no_bukti, nik,kode_param, nilai,kode_lokasi,periode)values")
                                sql.add(sqlText[i]);  
							if (sqlArray[i] != "insert into gaji_pot(nik, kode_param, periode1, periode2, nilai, kode_lokkonsol)values") 
                                sql.add(sqlArray[i]);
						}
