window.app_saku_gaji_transaksi_fUploadAbsen = function(owner)
{
	if (owner)
	{
		window.app_saku_gaji_transaksi_fUploadAbsen.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku_gaji_transaksi_fUploadAbsen";
...
window.app_saku_gaji_transaksi_fUploadAbsen.extend(window.portalui_childForm);
window.app_saku_gaji_transaksi_fUploadAbsen.implement({
		...
		switch (event){
			...
			case "simpan" :
					try{
						...
						var sqlText = "insert into gaji_load_absen_d (no_bukti, no_urut,nik,jml,kode_lokasi,periode)values";
						var sqlAbs= "insert into hr_absen(kode_lokkonsol, periode, nik, jml)values";		
						for (var i=0;i < this.sg1.getRowCount();i++){
							...
							sqlAbs += "('"+this.app._lokKonsol+"','"+this.ed_period.getText()+"','"+this.sg1.cells(0,i)+"', '"+this.sg1.cells(2,i)+"' )";
						}						
						sql.add(sqlText);
						sql.add(sqlAbs);
