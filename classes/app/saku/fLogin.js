...
window.app_saku_fLogin = function(owner){
	if (owner){
		try{		
			...
			systemAPI.alert("[app_saku_fLogin]::oncreate lib : ",e);
		}
	}
};
window.app_saku_fLogin.extend(window.portalui_childForm);
window.app_saku_fLogin.implement({
    mainButtonClick: function(sender){
		try{
			...
			this.e1.blur();
			this.owner.block();		
			this.dbLib.loginAndLoad("select a.*,b.nama from hakakses a inner join lokasi b on b.kode_lokasi = a.kode_lokasi where nik= '"+this.e0.getText()+"' and pass = '"+this.e1.getText()+"'\r\n"+
						...
						"select kode_lokkonsol from lokasi where kode_lokasi = 'table1_kode_lokasi'",this.e0.getText(),this.e1.getText(),false);											
		}catch(e){
			system.hideProgress();
			system.alert(this,"[fLogin2]::mainButtonClick:" +step,e);
		}		
	},
	...
	doRequestReady: function(sender, methodName, result){
		try{
			...
			//------------------------------------------------------------------- locking transaction													
								...
								this.app._dbEng = data[4];//dbEngine
