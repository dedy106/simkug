window.app_saku_addon_fp_report_flDaftarFaktur = function(owner)
{
	if (owner)
	{
		window.app_saku_addon_fp_report_flDaftarFaktur.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_addon_fp_report_flDaftarFaktur";
		this.maximize();
...
window.app_saku_addon_fp_report_flDaftarFaktur.extend(window.portalui_childForm);
window.app_saku_addon_fp_report_flDaftarFaktur.implement({
	...
	doSelectCell: function(sender, col, row)
	{
		...
		if (row == 1)
		{
			...
			var rs = this.dbLib.runSQL("select distinct periode from fp_invoice where kode_lokasi like '%"+this.sg1.getCell(2,0)+"' ");			
	...
	mainButtonClick: function(sender)
	{
		try
		{
			...
			{ 
				this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Lokasi","=",this.app._periode]);
				this.gridLib.SGEditData(this.sg1,1,[0,1,2],["Periode",this.tanda,this.lokasi]);
			...
			}else
			{
				...
				{
					var sql = "select a.kode_cust,b.nama as nama_cust,b.npwp,a.no_faktur,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.no_invoice,a.keterangan,a.nilai_dpp,a.nilai_ppn "+
						  "from fp_invoice a "+
						  "inner join cust b on a.kode_cust=b.kode_cust and a.kode_lokasi=b.kode_lokasi "+this.filter ;
					this.scriptSqlCount = "select count(*) "+
								"from fp_invoice a "+
							     "inner join cust b on a.kode_cust=b.kode_cust and a.kode_lokasi=b.kode_lokasi "+this.filter ;
					this.title = new server_util_arrayList({items:["Kode Cust","Nama Customer","NPWP","No Faktur","Tanggal","No Invoice","Keterangan","Nilai DPP","Nilai PPN"]});			
					this.widthTable = new server_util_arrayList({items:[50, 200,110,100,60,100,150,80,80]});
					this.fieldType = new server_util_arrayList({items:["S","S","S","S","S","S","S","N","N"]});		
				}
				else
				{
					var sql = "select a.kode_cust,b.nama as nama_cust,b.npwp,sum(a.nilai_dpp) as nilai_dpp,sum(a.nilai_ppn) as nilai_ppn "+
						  "from fp_invoice a "+
						  "inner join cust b on a.kode_cust=b.kode_cust and a.kode_lokasi=b.kode_lokasi "+this.filter+
		...
		{
			alert("[app_saku_addon_fp_report_flDaftarFaktur]::mainButtonClick:"+e);
		}
	},
