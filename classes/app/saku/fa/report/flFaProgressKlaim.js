// JavaScript Document
window.app_saku_fa_report_flFaProgressKlaim = function(owner)
{
	if (owner)
	{
		window.app_saku_fa_report_flFaProgressKlaim.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_fa_report_flFaProgressKlaim";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Progress Klaim Asuransi", 2);
...
window.app_saku_fa_report_flFaProgressKlaim.extend(window.portalui_childForm);
window.app_saku_fa_report_flFaProgressKlaim.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 2)
		{
			var sql1="select no_klaimsts,keterangan from faklaim_sts "+
					 ...
					 " ";
			var sql2="select count(no_klaimsts) from faklaim_sts "+
			         this.filterRep.filterStr("periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					 this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
			
			this.filterRep.ListDataSGFilter(this, "Data Status Klaim",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_klaimsts","keterangan"),"and");
		}
	},
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.filter=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
						...
						this.filterRep.filterStr("a.no_klaimsts",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
			this.nama_report="server_report_fa_rptFaProgressKlaim";
	...
	doCloseReportClick: function(sender){	
	  switch(sender.getName())
	  {
		...
		case "PrintBtn" :
	      try
	      {
			this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,this.filter,this.pager, (this.allBtn ? this.viewer.getTotalPage() * this.pager : this.pager), this.showFilter, this.lokasi,this.filter2));
