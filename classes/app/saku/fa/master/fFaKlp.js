window.app_saku_fa_master_fFaKlp = function(owner)
{
	if (owner)
	{
		window.app_saku_fa_master_fFaKlp.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku_fa_master_fFaKlp";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Kelompok Assest", 0);	
...
window.app_saku_fa_master_fFaKlp.extend(window.portalui_childForm);
window.app_saku_fa_master_fFaKlp.implement({
			...
			switch (event)
			{
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
						...
						try
						{						
							var sql2,sql1 = "delete from fa_klp where kode_lokasi = '"+this.elokasi.getText()+"'";					// kode_lokasi,						
							...
							if (driver.search("mssql") != -1){
								...
								for (var i in data){
									sql2 = "insert into fa_klp(kode_klpfa, nama, kode_lokasi,tipe, rowindex, kode_induk,sum_header, level_lap,level_spasi,  nik_user, tgl_input) values ";
									sql2 += data[i];
									sql.add(sql2);
								}
	...
	doClick: function(sender){	
		var nrc = this.dblib.runSQL("select kode_klpfa, nama, kode_lokasi,tipe, rowindex, kode_induk,sum_header, level_lap,level_spasi from fa_klp where  kode_lokasi = '"+this.elokasi.getText()+"' order by rowindex",0,0);	
		this.loadMenu(nrc);
	},
