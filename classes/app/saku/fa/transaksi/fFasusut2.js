window.app_saku_fa_transaksi_fFasusut2 = function(owner)
{
	if (owner)
	{
		window.app_saku_fa_transaksi_fFasusut2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_fa_transaksi_fFasusut2";
		this.maximize();
	    ...
	    this.p1.setName('p1');
	    this.p1.setCaption('Item Data Jurnal Penyusutan Asset');
		...
		this.sg1.setTag("2");
		this.sg1.setColTitle(new Array("No","Akun Asset","Nama Akun","Akun BP","Nama Akun","Akun AP","Nama Akun","Kode PP","Nama PP","Nilai Susut"));
		...
		{		    
			...
			this.cb_buat.setSQL("select nik,nama   from karyawan where kode_pp='"+this.app._kodePP+"' and kode_lokasi='"+this.app._lokasi+"'",["nik","nama"]);
...
window.app_saku_fa_transaksi_fFasusut2.extend(window.portalui_childForm);
window.app_saku_fa_transaksi_fFasusut2.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_fa_transaksi_fFasusut2.prototype.simpan = function()
{	
	this.bGen.click();
	if (this.standarLib.checkEmptyByTag(this, new Array("0","1"))){
		try{
			...
			sql.add("insert into fasusut_d (no_fasusut,no_fa,periode,nilai,kode_lokasi,akun_bp,akun_ap,kode_pp,kode_drk,kode_akun,dc,no_del) "+								
					"select '"+this.ed_nb.getText()+"',a.no_fa,'"+this.ed_period.getText()+"', "+				
				    "     case when (a.nilai*a.persen/12/100) < (a.nilai-ifnull(c.total_susut,0)-a.nilai_residu) "+
					"          then round(a.nilai*a.persen/12/100,0) "+
					"          else round(a.nilai-ifnull(c.total_susut,0)-a.nilai_residu,0) end  as nilai_susut,'"+this.app._lokasi+"',"+
					"     b.akun_bp,b.akun_deprs,a.kode_pp,'"+this.cb_drk.getText()+"',b.kode_akun,'D','-' "+
					"from fa_asset a "+
					...
					"                        where y.no_del='-' and y.kode_lokasi='"+this.app._lokasi+"' and x.dc='D' "+
					"                        group by x.kode_lokasi,x.no_fa) c on a.no_fa=c.no_fa and a.kode_lokasi=c.kode_lokasi "+ 
					"where ((a.nilai-a.nilai_residu) > ifnull(c.total_susut,0)) and a.progress='1' and a.kode_lokasi = '"+this.app._lokasi+"' and periode_susut = '"+this.ed_period.getText()+"' and a.jenis='ASSET' and a.persen<>0");
			
			sql.add("update fa_asset a, fasusut_d b set a.periode_susut = '"+nextNPeriode(this.ed_period.getText(),1)+"' "+
			        "where a.no_fa=b.no_fa and a.kode_lokasi=b.kode_lokasi and b.no_fasusut='"+this.ed_nb.getText()+"' and b.kode_lokasi = '"+this.app._lokasi+"'");	
...
window.app_saku_fa_transaksi_fFasusut2.prototype.doModalResult = function(event, modalResult)
{
...
window.app_saku_fa_transaksi_fFasusut2.prototype.genClick = function(sender)
{
	try
	{
		if (sender == this.bGen)
		...
		if (sender == this.bHitung){
			this.sg1.clearAll();
			var data = this.dbLib.runSQL("select b.kode_akun,aa.nama as nama_1,b.akun_bp,ab.nama as nama_2,b.akun_deprs,ac.nama as nama_3, a.kode_pp,xx.nama as nama_pp, "+
										"sum(case when (a.nilai*a.persen/12/100) < (a.nilai-ifnull(c.total_susut,0)-a.nilai_residu) "+
										"	 then round(a.nilai*a.persen/12/100,0) "+
										"	 else round(a.nilai-ifnull(c.total_susut,0)-a.nilai_residu,0) end ) as nilai_susut  "+
										"from fa_asset a "+
										"	   inner join fa_klpakun b on a.kode_klpakun=b.kode_klpakun and a.kode_lokasi=b.kode_lokasi "+
										"	   inner join pp xx on xx.kode_pp = a.kode_pp and xx.kode_lokasi = a.kode_lokasi "+
										"	   inner join masakun aa on aa.kode_akun = b.kode_akun and aa.kode_lokasi = b.kode_lokasi "+
										"	   inner join masakun ab on ab.kode_akun = b.akun_bp and ab.kode_lokasi = b.kode_lokasi "+
										"	   inner join masakun ac on ac.kode_akun = b.akun_deprs and ac.kode_lokasi = b.kode_lokasi "+ 
										"	   left outer join (select x.kode_lokasi,x.no_fa,sum(x.nilai) as total_susut "+
										"						from fasusut_d x inner join fasusut_m y on x.no_fasusut=y.no_fasusut and x.kode_lokasi=y.kode_lokasi "+
										"						where y.no_del='-' and y.kode_lokasi='"+this.app._lokasi+"' and x.dc='D' "+
										"						group by x.kode_lokasi,x.no_fa) c on a.no_fa=c.no_fa and a.kode_lokasi=c.kode_lokasi  "+
										"where ((a.nilai-a.nilai_residu) > ifnull(c.total_susut,0)) and a.progress='1' and a.kode_lokasi = '"+this.app._lokasi+"' and periode_susut = '"+this.ed_period.getText()+"' and a.jenis='ASSET' and a.persen<>0 "+
										"group by b.kode_akun,aa.nama,b.akun_bp,ab.nama,b.akun_deprs,ac.nama,a.kode_pp,xx.nama "+
										"order by b.kode_akun,a.kode_pp");		
...
window.app_saku_fa_transaksi_fFasusut2.prototype.doSelect = function(sender, year, month, day)
{
	if (month < 10)
		month = "0"+month;
	this.ed_period.setText(year.toString()+month);
};
window.app_saku_fa_transaksi_fFasusut2.prototype.doEditChange = function(sender)
{
	if (sender == this.ed_period)
	{
		this.ed_nb.setText("");
		this.cb_drk.setSQL("select kode_drk,nama   from drk where kode_lokasi='"+this.app._lokasi+"' and tipe='posting' and tahun='"+this.ed_period.getText().substr(0,4)+"'",["kode_drk","nama"]);
		//if (this.ed_period.getText() != "") this.bGen.click();
	}
};											  
window.app_saku_fa_transaksi_fFasusut2.prototype.FindBtnClick = function(sender, event)
{
	try
	{
...
/*
window.app_saku_fa_transaksi_fFasusut2.prototype.hitungGar = function()
{
...
window.app_saku_fa_transaksi_fFasusut2.prototype.garClick = function(sender)
{
	try
	{
...
*/
window.app_saku_fa_transaksi_fFasusut2.prototype.doRequestReady = function(sender, methodName, result)
{
