window.app_saku_fa_transaksi_fFaklaim = function(owner)
{
	if (owner)
	{
		window.app_saku_fa_transaksi_fFaklaim.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_fa_transaksi_fFaklaim";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Klaim Asuransi: Input", 0);	
		...
		{		
			...
			this.cb_polis.onBtnClick.set(this, "FindBtnClick");
...
window.app_saku_fa_transaksi_fFaklaim.extend(window.portalui_childForm);
window.app_saku_fa_transaksi_fFaklaim.prototype.mainButtonClick = function(sender){
...
window.app_saku_fa_transaksi_fFaklaim.prototype.simpan = function()
{	
...
window.app_saku_fa_transaksi_fFaklaim.prototype.doModalResult = function(event, modalResult)
{
...
window.app_saku_fa_transaksi_fFaklaim.prototype.genClick = function(sender)
{
	try
	{
		if (sender == this.bGen)
...
window.app_saku_fa_transaksi_fFaklaim.prototype.doSelect = function(sender, year, month, day)
{
	if (month < 10)
		month = "0"+month;
	this.ed_period.setText(year.toString()+month);
};
window.app_saku_fa_transaksi_fFaklaim.prototype.doEditChange = function(sender)
{
...
window.app_saku_fa_transaksi_fFaklaim.prototype.FindBtnClick = function(sender, event)
{
	try
	{
		...
		if (sender == this.cb_polis) 
		{  
			this.standarLib.showListData(this, "Daftar Polis Asuransi",this.cb_polis,undefined, 
										  "select no_polis,keterangan from fapolis_m where kode_lokasi='"+this.app._lokasi+"' and tgl_selesai>='"+this.dp_tgl1.getDate()+"'",
										  "select count(no_polis)     from fapolis_m where kode_lokasi='"+this.app._lokasi+"' and tgl_selesai>='"+this.dp_tgl1.getDate()+"'",
										  new Array("no_polis","keterangan"),"and",new Array("No Polis","Keterangan"),false);
...
window.app_saku_fa_transaksi_fFaklaim.prototype.showClick = function(sender)
{
	try
	{	
		this.sg1.showLoading();
		var line,data = this.dbLib.runSQL(" select a.kode_vendor,b.nama as nama_vendor,a.nilai "+
		...
		if (data instanceof portalui_arrayMap)
		{
			...
			if (line != undefined)
			{
				...
				this.ed_npolis.setText(floatToNilai(parseFloat(line.get("nilai"))));
			} 
		}
		
		var line,data = this.dbLib.runSQL(" select a.no_fa,a.barcode,a.nama,a.merk,a.tipe,a.no_seri,b.nama as lokfa,c.nama as pp,d.nama as pnj,f.kode_akun,a.nilai "+
													  ...
													  " where a.kode_lokasi='"+this.app._lokasi+"' and g.no_polis='"+this.cb_polis.getText()+"'");					
		if (data instanceof portalui_arrayMap)
		{
			...
			{									
				...
				for (var i in data.objList)
				{
					...
					new Array("INPROG","-","0",line.get("no_fa"),line.get("barcode"),line.get("nama"),line.get("merk"),line.get("tipe"),line.get("no_seri"),line.get("lokfa"),line.get("pp"),line.get("pnj"),
...
window.app_saku_fa_transaksi_fFaklaim.prototype.doCellExit = function(sender, col, row) 
{
	try
	{
		switch(col)
...
window.app_saku_fa_transaksi_fFaklaim.prototype.doNilaiChange = function()
{
	try
	{
		var tot = tot2 = 0;  
...
window.app_saku_fa_transaksi_fFaklaim.prototype.doRequestReady = function(sender, methodName, result)
{
