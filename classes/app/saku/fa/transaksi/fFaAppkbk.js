window.app_saku_fa_transaksi_fFaAppkbk = function(owner)
{
	if (owner)
	{
		window.app_saku_fa_transaksi_fFaAppkbk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_fa_transaksi_fFaAppkbk";
		this.maximize();
		...
		this.cb_perJU = new portalui_saiCB(this,{bound:[720,6,200,20],caption:"Periode KB",mustCheck: true, change:[this,"doEditChange"],tag:2});
		this.cb_buat = new portalui_saiCBBL(this,{bound:[20,7,185,20],caption:"Dibuat Oleh"});
		this.cb_ju = new portalui_saiCBBL(this,{bound:[720,7,200,20],caption:"No Bukti KB",rightLabelVisible:false,tag:1});
		...
		this.ed_ju = new portalui_saiLabelEdit(this,{bound:[720,10,200,20],caption:"Nilai KB", tipeText:ttNilai,readOnly:true,text:"0",tag:1});		
		...
		{		    
			...
			var prd = this.dbLib.getDataProvider("select distinct periode from fa_app where kode_lokasi = '"+this.app._lokasi+"' and modul='KB' order by periode desc",true);
...
window.app_saku_fa_transaksi_fFaAppkbk.extend(window.portalui_childForm);
window.app_saku_fa_transaksi_fFaAppkbk.implement({
			...
			try{
				...
				for (var i=0;i < this.sg0.getRowCount();i++){
					if (this.sg0.cells(0,i) != "-" && this.sg0.cells(0,i) != ""){
						...
						faScript.push("('"+this.nb+"','"+this.sg0.cells(1,i)+"','"+this.cb_ju.getText()+"','"+this.sg0.getCell(4,i)+"','"+this.app._lokasi+"',"+parseNilai(this.sg0.getCell(5,i))+",'KB')"); 
					}
				}
	...
	doEditChange: function(sender){
		...
		if (sender == this.cb_perLama){
		      this.cb_nbLama.setSQL("select no_faapp, no_dokumen from fa_app where modul = 'KB' and periode='"+this.cb_perLama.getText()+"' and no_del='-' and kode_lokasi='"+this.app._lokasi+"'",["no_faapp","no_dokumen"],true);
        }
	},
			...
			if (sender == this.cb_nbLama) {
			    if (this.cb_perLama.getText() != "") {
				  this.standarLib.showListData(this, "Daftar Bukti Approve Periode "+this.cb_perLama.getText(),this.cb_nbLama,undefined, 
													 "select no_faapp, no_dokumen from fa_app where modul = 'KB' and periode='"+this.cb_perJU.getText()+"' and no_del='-' and kode_lokasi='"+this.app._lokasi+"'", 
													 "select count(no_faapp)      from fa_app where modul = 'KB' and periode='"+this.cb_perJU.getText()+"' and no_del='-' and kode_lokasi='"+this.app._lokasi+"'", 
			...
			if (sender == this.cb_ju) {
			    if (this.cb_perJU.getText() != "") {
				  this.standarLib.showListData(this, "Daftar Bukti KB Periode "+this.cb_perJU.getText(),this.cb_ju,undefined, 
													 ...
													 new Array("no_kas","no_dokumen"),"and", new Array("No Bukti","No Dokumen"),false);
				}
				this.sg.clear(1);
				for (var i=0; i < this.sg0.getRowCount();i++){
					this.sg0.cells(0,i,'-');
	...
	doFindBtnClick: function(sender, col, row){
		...
		{
			alert("[app_saku_fa_transaksi_fFaAppkbk] : doFindBtnClick : " + e);
		}
	},
		...
		try {
			if (sender == this.iShow) {
				...
				var data = this.dbLib.getDataProvider("select a.no_spb,c.no_fa,c.nama,c.barcode,c.kode_klpfa,c.kode_akun,c.kode_lokfa,c.nilai,a.periode,"+
													  ...
													  "              inner join kas_m h on a.no_spb=h.no_kas and a.kode_lokasi=h.kode_lokasi "+
