// JavaScript Document
window.app_saku_fa_trail_fTrailFaRekap = function(owner)
{
	if (owner)
	{
		window.app_saku_fa_trail_fTrailFaRekap.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_fa_trail_fTrailFaRekap";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Audit Trail Rekap Asset", 2);
...
window.app_saku_fa_trail_fTrailFaRekap.extend(window.portalui_childForm);
window.app_saku_fa_trail_fTrailFaRekap.prototype.doSg3DblClick = function(sender, colIdx, rowIdx)
{  
	try
	{
	...
	this.e10.setReadOnly(true);
		...
		var sql="select a.kode_klpakun,b.nama as nama_klpakun,a.kode_pp,c.nama as nama_pp,d.nama as nama_lokfa,a.no_fa,a.nama,a.tgl_perolehan,"+
				...
				"where a.kode_klpakun='"+this.sg2.getCell(1,rowIdx)+"' and a.kode_lokasi='"+this.sg1.getCell(2,1)+"' "+
				"order by a.no_fa ";
...
window.app_saku_fa_trail_fTrailFaRekap.prototype.SetSg4Mutasi = function()
{
	try
	{
	...
	{
		alert("[app_saku_fa_trail_fTrailFaRekap]::SetSg4Mutasi:"+e);
	}
};
window.app_saku_fa_trail_fTrailFaRekap.prototype.doSg4DblClick = function(sender, colIdx, rowIdx)
{	
	try{
...
window.app_saku_fa_trail_fTrailFaRekap.prototype.SetSg5Mutasi = function()
{
	try
	{
		var debet = kredit = 0;
...
window.app_saku_fa_trail_fTrailFaRekap.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_saku_fa_trail_fTrailFaRekap.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
...
window.app_saku_fa_trail_fTrailFaRekap.prototype.mainButtonClick = function(sender)
{	
	if (sender == this.app._mainForm.bPreview){
		try
		{
			...
			this.nik_user=this.app._nikUser;
			sql="call sp_fa_rekap ('"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
			...
			this.dbLib.execQuerySync(sql);
				...
				if (this.p1.visible){ 
					...
					{		
						this.sg2.clearAll();
						var line,data = this.dbLib.runSQL("select kode_fa,nama,n1,round(n2),round(n3),round(n4),round(n5) from fa_tmp where nik_user='"+this.nik_user+"' order by kode_fa ");
						if (data instanceof portalui_arrayMap)
						{						
...
window.app_saku_fa_trail_fTrailFaRekap.prototype.doRequestReady = function(sender, methodName, result)
{
		switch (methodName)
		{
			case "preview" : 
...
window.app_saku_fa_trail_fTrailFaRekap.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page, 30, "YAKES", this.showFilter,this.filter2);	
};
window.app_saku_fa_trail_fTrailFaRekap.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_fa_trail_fTrailFaRekap.prototype.doAllPageClick = function(sender)
{
	this.report.preview(this.nama_report,this.filter, 1, this.viewer.getTotalPage() * 30, this.app._namaLokasi,this.showFilter);	
};
window.app_saku_fa_trail_fTrailFaRekap.prototype.doPdfClick = function(sender)
{
...
window.app_saku_fa_trail_fTrailFaRekap.prototype.doXlsClick = function()
{
