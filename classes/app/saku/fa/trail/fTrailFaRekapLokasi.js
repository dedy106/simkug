// JavaScript Document
window.app_saku_fa_trail_fTrailFaRekapLokasi = function(owner)
{
	if (owner)
	{
		window.app_saku_fa_trail_fTrailFaRekapLokasi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_fa_trail_fTrailFaRekapLokasi";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Audit Trail Rekap Lokasi Asset", 2);
...
window.app_saku_fa_trail_fTrailFaRekapLokasi.extend(window.portalui_childForm);
window.app_saku_fa_trail_fTrailFaRekapLokasi.prototype.doSg3DblClick = function(sender, colIdx, rowIdx)
{  
	try
	{
	...
	this.p4.setBorder(3);
	this.p4.setCaption("Audit Trail Detail Asset");
	
	this.e3 = new portalui_saiLabelEdit(this.p4);
	this.e3.setTop(25);
	this.e3.setLeft(10);
	this.e3.setWidth(500);
	this.e3.setCaption("Lokasi");
	this.e3.setText(this.sg2.getCell(1,rowIdx)+" - "+trim(this.sg2.getCell(2,rowIdx).replace(/&nbsp;/g,"")));
	...
	this.e6.setCaption("Nilai Perolehan");
	...
	this.sg4.setColTitle(new Array("No","Klp Asset","Nama Klp Asset","Kode PP","Nama PP","Lokasi","No Asset","Keterangan","Tanggal","Bulan","%","No Approval","Kondisi","Nilai Perolehan","Deprs SD Bulan Lalu","Deprs Bulan Ini","Deprs Sd Bulan Ini","Nilai Buku"));
	
	this.e7 = new portalui_saiLabelEdit(this.p4);
	this.e7.setTop(450);
	this.e7.setLeft(30);
	this.e7.setWidth(200);
	this.e7.setCaption("Deprs SD Bulan Lalu");
	...
	this.e8.setCaption("Deprs Bulan Ini");
	...
	this.e9.setCaption("Deprs SD Bulan Ini");
	this.e9.setText(this.sg2.getCell(5,rowIdx));
	...
	this.e10.setCaption("Nilai Buku");
	this.e10.setText(this.sg2.getCell(6,rowIdx));
	this.e10.setTipeText(ttNilai);
	this.e10.setAlignment(alRight);
	this.e10.setReadOnly(true);
			
		this.sg4.clearAll();
		var sql = "call sp_fa_susut_tmp('"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
		
		this.dbLib.execQuerySync(sql);	
		var sql="select a.kode_klpakun,b.nama as nama_klpakun,a.kode_pp,c.nama as nama_pp,d.nama as nama_lokfa,a.no_fa,a.nama,a.tgl_perolehan,"+
				"       a.umur,a.persen,a.nilai,e.n1,e.n2,e.n3,a.nilai-(e.n1+e.n2+e.n3) as nilai_buku "+
				"from fa_asset a "+
				...
				"inner join fa_susut_tmp e on a.no_fa=e.no_fa and a.kode_lokasi=e.kode_lokasi and e.nik_user='"+this.nik_user+"' "+
				"where a.kode_lokfa='"+this.sg2.getCell(1,rowIdx)+"' and a.kode_lokasi='"+this.sg1.getCell(2,1)+"' "+
				"order by a.no_fa ";
...
window.app_saku_fa_trail_fTrailFaRekapLokasi.prototype.SetSg4Mutasi = function()
{
	try
	{
	...
	{
		alert("[app_saku_fa_trail_fTrailFaRekapLokasi]::SetSg4Mutasi:"+e);
	}
};
window.app_saku_fa_trail_fTrailFaRekapLokasi.prototype.doSg4DblClick = function(sender, colIdx, rowIdx)
{	
	try{
...
window.app_saku_fa_trail_fTrailFaRekapLokasi.prototype.SetSg5Mutasi = function()
{
	try
	{
		var debet = kredit = 0;
...
window.app_saku_fa_trail_fTrailFaRekapLokasi.prototype.doEllipseClick= function(sender, col, row)
{
...
window.app_saku_fa_trail_fTrailFaRekapLokasi.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
...
window.app_saku_fa_trail_fTrailFaRekapLokasi.prototype.mainButtonClick = function(sender)
{	
	if (sender == this.app._mainForm.bPreview){
		try
		{
			...
			this.nik_user=this.app._nikUser;
			sql="call sp_fa_lokasi ('"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
			...
			this.dbLib.execQuerySync(sql);
				...
				if (this.p1.visible){ 
					...
					this.p2.setBorder(3);
					this.p2.setCaption("Audit Trail Rekap Asset");
					...
					this.sg2.setColTitle(new Array("No","Kode","Nama","Nilai Perolehan","Deprs Bulan Lalu","Deprs Bulan Ini","Deprs SD Bulan Ini","Nilai Buku"));
					this.sg2.setTag("2");
					...
					{		
						this.sg2.clearAll();
						var line,data = this.dbLib.runSQL("select kode_fa,fn_spasi(nama,level_spasi),n1,round(n2),round(n3),round(n4),round(n5) from fa_tmp where nik_user='"+this.nik_user+"' order by kode_fa ");
						if (data instanceof portalui_arrayMap)
						{						
...
window.app_saku_fa_trail_fTrailFaRekapLokasi.prototype.doRequestReady = function(sender, methodName, result)
{
		switch (methodName)
		{
			case "preview" : 
...
window.app_saku_fa_trail_fTrailFaRekapLokasi.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page, 30, "YAKES", this.showFilter,this.filter2);	
};
window.app_saku_fa_trail_fTrailFaRekapLokasi.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_fa_trail_fTrailFaRekapLokasi.prototype.doAllPageClick = function(sender)
{
	this.report.preview(this.nama_report,this.filter, 1, this.viewer.getTotalPage() * 30, this.app._namaLokasi,this.showFilter);	
};
window.app_saku_fa_trail_fTrailFaRekapLokasi.prototype.doPdfClick = function(sender)
{
...
window.app_saku_fa_trail_fTrailFaRekapLokasi.prototype.doXlsClick = function()
{
