window.app_saku_dmt_report_flKB = function(owner)
{
	if (owner)
	{
		window.app_saku_dmt_report_flKB.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_dmt_report_flKB";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Pembayaran Piutang",2);
...
window.app_saku_dmt_report_flKB.extend(window.portalui_childForm);
window.app_saku_dmt_report_flKB.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 4)
		{
			this.standar.ListDataSGFilter2(this, "Data Kas Bank",this.sg1, this.sg1.row, this.sg1.col,
										  "select a.no_kb, a.keterangan  from dmt_kb a "+
										  "	inner join dmt_kontrak b on b.no_kontrak = a.no_kontrak and a.kode_lokasi = b.kode_lokasi "+
										  this.filterRep.filterStr("b.kode_cust",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"where")+
										  ...
										  " and a.kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",
										  "select count(*) from dmt_kb a "+
										  "	inner join dmt_kontrak b on b.no_kontrak = a.no_kontrak and a.kode_lokasi = b.kode_lokasi "+
										  this.filterRep.filterStr("b.kode_cust",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
										  this.filterRep.filterStr("b.no_kontrak",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
											this.filterRep.filterStr("a.periode",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+	
										  " and a.kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",
										  ["a.no_kb","a.keterangan"],"and",["No Buktikas","Keterangan"]);
		}
	},
	doSelectCell: function(sender, col, row){
		...
		if (row === 1){
			this.dbLib.setItemsFromSQL("select distinct periode from dmt_kb where kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 5)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			...
			}else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_cust",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							  ...
							  this.filterRep.filterStr("a.no_kb",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new portalui_arrayMap();
				this.jenisLap = this.sg1.getCell(2,5);
				var sql = "select a.no_kb,a.no_kontrak,concat(a.kode_cust,'-',b.nama) as customer,date_format(a.tanggal,'%d-%m-%Y') as tgl,c.no_invoice,d.keterangan,a.nilai ";
				if (this.jenisLap === "Transaksi") 
					sql+=",a.no_dokumen,a.keterangan,a.akun_kb,a.akun_pph,a.nilai_pph ";
				sql+="from dmt_kb a inner join dmt_cust b on b.kode_cust=a.kode_cust and b.kode_lokasi=a.kode_lokasi "+
					"inner join dmt_kb_d c on a.no_kb=c.no_kb "+
					"inner join dmt_invoice d on c.no_invoice=d.no_invoice "+this.filter+
					" order by a.no_kb,a.no_kontrak,customer,tgl ";
				this.scriptSqlCount = "select count(*) "+
					"from dmt_kb a inner join dmt_cust b on b.kode_cust=a.kode_cust and b.kode_lokasi=a.kode_lokasi "+
					"inner join dmt_kb_d c on a.no_kb=c.no_kb "+
					"inner join dmt_invoice d on c.no_invoice=d.no_invoice "+this.filter;
				if (this.jenisLap === "Daftar"){
					this.title = new server_util_arrayList({items:["No Bukti","No Kontrak","Customer","Tanggal","No Invoice","Keterangan","Nilai"]});
					this.widthTable = new server_util_arrayList({items:[120,120,150,80,120,250,100]});
					...
					this.title = new server_util_arrayList({items:["No Invoice","Keterangan","Nilai"]});
			...
			systemAPI.alert("[app_saku_dmt_report_flKB]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		if (this.jenisLap === "Transaksi")
			var header = "DAFTAR DETAIL PEMBAYARAN PIUTANG";
		else var header = "DAFTAR PEMBAYARAN PIUTANG";
	...
	convertResultToHTML: function(result,data){		
		...
		for (var r in table.rs.rows){
			...
			if (first){
				...
				htmlHeader += "<tr>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>No. Kas Bank</td>";
				...
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.akun_kb+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
			...
			for (var c in line){
				if (c === "no_invoice" || c === "keterangan" || c === "nilai"){
					if (c === "nilai")
