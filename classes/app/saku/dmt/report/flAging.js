window.app_saku_dmt_report_flAging = function(owner)
{
	if (owner)
	{
		window.app_saku_dmt_report_flAging.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_dmt_report_flAging";
		this.maximize();		
	...
	this.filterRep.setSGFilterRowTipe(this.sg1, 0,[0,1,2,3,4,5],["3","13","13","13","3","3"]);
	...
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["No. No Invoice","All",""]);	
	this.gridLib.SGEditData(this.sg1,4,[0,1,2],["Jenis Saldo","=","Customer"]);
	this.gridLib.SGEditData(this.sg1,5,[0,1,2],["Posisi Tanggal","=",new Date().lclFormat()]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku_dmt_report_flAging.extend(window.portalui_childForm);
window.app_saku_dmt_report_flAging.implement({
	...
	doSelectCell: function(sender, col, row){
		if (this.userStatus==="A"){
			this.filterRep.setSGFilterRowTipe(this.sg1,row,[0,1,2,3,4,5],["3","13","13","13","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1,row,[0,1,2,3,4,5],[2,2,2,2,0,1]);
		}else{
			this.filterRep.setSGFilterRowTipe(this.sg1,row,[0,1,2,3,4,5],["3","13","13","13","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1,row,[0,1,2,3,4,5],[3,2,2,2,0,1]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			}else
			{
				...
				var tanggal = this.sg1.getCellDateValue(2,5);
				if (this.jenisLap === "Customer") 
				{
					...
					var sql = "select a.kode_cust,a.nama,sum(b.aging1) as aging1,sum(b.aging2) as aging2,sum(b.aging3) as aging3,sum(b.aging4) as aging4 "+
							...
							"				  case when datediff('"+tanggal+"',a.tanggal)<=30 then ifnull(a.nilai+a.nilai_ppn,0)-ifnull(b.nilai,0) else 0 end as aging1, "+
							"				  case when (datediff('"+tanggal+"',a.tanggal)>30 and datediff('"+tanggal+"',a.tanggal)<=60) then ifnull(a.nilai+a.nilai_ppn,0)-ifnull(b.nilai,0) else 0 end as aging2, "+
							"				  case when (datediff('"+tanggal+"',a.tanggal)>60 and datediff('"+tanggal+"',a.tanggal)<=90) then ifnull(a.nilai+a.nilai_ppn,0)-ifnull(b.nilai,0) else 0 end as aging3, "+
							"				  case when datediff('"+tanggal+"',a.tanggal)>90 then ifnull(a.nilai+a.nilai_ppn,0)-ifnull(b.nilai,0) else 0 end as aging4 "+
							"		   from dmt_invoice a  "+
							"		   left join dmt_kb_d b on a.no_invoice=b.no_invoice and a.kode_lokasi=b.kode_lokasi "+
							"		   left join dmt_kb c on b.no_kb=c.no_kb and b.kode_lokasi=c.kode_lokasi and c.tanggal<='"+tanggal+"' "+
							"		   where ifnull(a.nilai+a.nilai_ppn,0)-ifnull(b.nilai,0)>0 "+
			...
			systemAPI.alert("[app_saku_dmt_report_flAging]::mainButtonClick:"+e);
		}
	},
