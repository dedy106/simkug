window.app_saku_dmt_report_flSaldoPdd = function(owner)
{
	if (owner)
	{
		window.app_saku_dmt_report_flSaldoPdd.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_dmt_report_flSaldoPdd";
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Saldo PDD", 2);
...
window.app_saku_dmt_report_flSaldoPdd.extend(window.portalui_childForm);
window.app_saku_dmt_report_flSaldoPdd.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		if (row === 1){
			this.dbLib.setItemsFromSQL("select distinct periode from dmt_kb where kode_lokasi = '"+this.sg1.getCell(2,0)+"' "+
									   "union select distinct periode from dmt_pdd_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"'" ,[this.sg1.columns.get(2).pickList]);
		}
		if (row === 5)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			...
			}else
			{
				...
				if (this.jenisLap === "Customer") 
				{
					...
					var sql = "select a.kode_cust,a.nama,ifnull(d.nilai,0)-ifnull(e.nilai_bayar,0) as so_awal, "+   		   
							"		  ifnull(b.nilai,0) as kredit, ifnull(c.nilai_bayar,0) as debet, "+
							...
							"	left join (select b.kode_cust,b.kode_lokasi,sum(a.nilai) as nilai "+		   
							"			   from dmt_pdd_d a "+		   
							"			   inner join dmt_kontrak b on a.no_kontrak=b.no_kontrak and a.kode_lokasi=b.kode_lokasi "+		   
							"			   where a.periode='"+periode+"' "+		   
							"			   group by b.kode_cust)b on a.kode_cust=b.kode_cust and a.kode_lokasi=b.kode_lokasi "+		
							"	left join (select c.kode_cust,a.kode_lokasi,sum(a.nilai*a.jumlah) as nilai_bayar "+			   
							"			   from dmt_kb_d a "+			   
							"			   inner join dmt_kb c on a.no_kb=c.no_kb and a.kode_lokasi=c.kode_lokasi "+			   
							"			   where c.periode='"+periode+"' and a.jenis='PDD' "+			   
							"			   group by c.kode_cust )c on a.kode_cust=c.kode_cust and a.kode_lokasi=c.kode_lokasi "+		
							"	left join (select b.kode_cust,b.kode_lokasi,sum(a.nilai) as nilai "+			   
							"			   from dmt_pdd_d a "+			   
							"			   inner join dmt_kontrak b on a.no_kontrak=b.no_kontrak and a.kode_lokasi=b.kode_lokasi "+			   
							"			   where a.periode<'"+periode+"' "+			   
							"			   group by b.kode_cust)d on a.kode_cust=d.kode_cust and a.kode_lokasi=d.kode_lokasi "+		
							"	left join (select c.kode_cust,a.kode_lokasi,sum(a.nilai*a.jumlah) as nilai_bayar "+			   
							"			   from dmt_kb_d a "+			   
							"			   inner join dmt_kb c on a.no_kb=c.no_kb and a.kode_lokasi=c.kode_lokasi "+			   
							"			   where c.periode<'"+periode+"' and a.jenis='PDD' "+			   
							"			   group by c.kode_cust )e on a.kode_cust=e.kode_cust and a.kode_lokasi=e.kode_lokasi "+this.filter +		
			...
			systemAPI.alert("[app_saku_dmt_report_flSaldoPdd]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		if (this.jenisLap === "Transaksi")
			var header = "SALDO PDD PER CUSTOMER";
		else var header = "DAFTAR PDD PER CUSTOMER";
