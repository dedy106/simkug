window.app_saku_dmt_report_flBilling = function(owner)
{
	if (owner)
	{
		window.app_saku_dmt_report_flBilling.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_dmt_report_flBilling";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Billing", 2);
	...
	this.getPeriode="select distinct periode from dmt_billing where kode_lokasi ='"+this.lokasi+"'";
	...
	this.gridLib.SGEditData(this.sg1,4,[0,1,2],["Billing","All",""]);
	this.gridLib.SGEditData(this.sg1,5,[0,1,2],["Jenis Laporan","=","Daftar"]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku_dmt_report_flBilling.extend(window.portalui_childForm);
window.app_saku_dmt_report_flBilling.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 4)
		{
			this.standar.ListDataSGFilter2(this, "Data Billing",this.sg1, this.sg1.row, this.sg1.col,
										  "select no_bill, keterangan  from dmt_billing "+
										  this.filterRep.filterStr("kode_cust",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"where")+
										  this.filterRep.filterStr("no_kontrak",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
											this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+	
										  " and kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",
										  "select count(*) from dmt_billing "+
										  ...
										  " and kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",
										  ["no_bill","keterangan"],"and",["No Bill","Keterangan"]);
		}
	},
	doSelectCell: function(sender, col, row){
		...
		if (row === 1){
			this.dbLib.setItemsFromSQL("select distinct periode from dmt_billing where kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 5)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			...
			}else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_cust",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							  ...
							  this.filterRep.filterStr("a.no_bill",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.jenisLap = this.sg1.getCell(2,5);
				var result  = new portalui_arrayMap();
				var sql = "select  a.no_bill,a.no_kontrak,concat(a.kode_cust,'-',b.nama)as customer, date_format(a.tanggal,'%d-%m-%Y') as tgl,  c.no_fa, d.nama as nmsite, e.alamat, c.nilai ";
				if (this.jenisLap === "Transaksi") sql+=",a.keterangan ";
				sql+=" from dmt_billing a "+
					"	inner join dmt_cust b on b.kode_cust = a.kode_cust and b.kode_lokasi = a.kode_lokasi "+
					"	inner join dmt_billing_d c on c.no_bill = a.no_bill and c.kode_lokasi = a.kode_lokasi "+
					"	inner join fa_asset d on d.no_fa = c.no_fa and d.kode_lokasi = a.kode_lokasi "+
					"	inner join dmt_tower e on e.no_fa = c.no_fa and e.kode_lokasi= a.kode_lokasi "+
					"	"+this.filter +
					"	order by no_bill,no_kontrak,customer,tgl";
				...
				if (this.jenisLap === "Daftar"){
					this.title = new server_util_arrayList({items:["No Bill","No Kontrak","Customer","Tanggal","Site","Nama Site","Alamat","Nilai"]});			
					this.widthTable = new server_util_arrayList({items:[120,120,100,80,120,150,400,100]});
					this.fieldType = new server_util_arrayList({items:["S","S","S","D","S","S","S","S","N"]});			
					//this.groupBy = new server_util_arrayList({items:["no_bill"]});			
			...
			systemAPI.alert("[app_saku_dmt_report_flBilling]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		if (this.jenisLap === "Transaksi")
			var header = "DAFTAR DETAIL BILLING";
		else var header = "DAFTAR BILLING";
	  ...
	  switch(sender.getName())
	  {
	    ...
	    case "pdfBtn" :      
		  var html = new server_util_arrayList();
			...
			html.add("pdf");			
			html.add(new Date().valueOf()+"_billing");				
	...
	convertResultToHTML: function(result,data){		
		...
		for (var r in table.rs.rows){
			...
			if (first){
				...
				htmlHeader += "<tr>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>No. Billing</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.no_bill+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
			...
			for (var c in line){
				if (c === "no_fa" || c === "nmsite" || c === "alamat" || c === "nilai"){
					if (c === "nilai")
