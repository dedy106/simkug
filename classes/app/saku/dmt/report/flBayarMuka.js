window.app_saku_dmt_report_flBayarMuka = function(owner)
{
	if (owner)
	{
		window.app_saku_dmt_report_flBayarMuka.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_dmt_report_flBayarMuka";
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Pembayaran Di Muka", 2);
	...
	this.filterRep.setSGFilterRowTipe(this.sg1, 0,[0,1,2,3,4],["3","13","13","13","3"]);
	...
	this.getPeriode="select distinct periode from dmt_kb where kode_lokasi ='"+this.lokasi+"'";
...
window.app_saku_dmt_report_flBayarMuka.extend(window.portalui_childForm);
window.app_saku_dmt_report_flBayarMuka.implement({
		...
		try{			
			...
			if (row == 3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.no_kb,a.keterangan from dmt_kb a where a.kode_lokasi ='"+this.sg1.getCell(2,0)+"' "+
														  this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and"),
														  "select count(*) from dmt_kb a where a.kode_lokasi ='"+this.sg1.getCell(2,0)+"' "+
														  this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and"),
														  ["a.no_kb","a.keterangan"],"and",["No Bukti","Keterangan"]);
			}			
		}catch(e){
			systemAPI.alert(e);
		}
		...
		try
		{
			...
			}else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and")+
							...
							this.filterRep.filterStr("a.no_kb",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				...
				{
					var sql = "select a.periode,date_format(a.tanggal,'%d-%m-%Y') as tanggal,a.no_kb,a.no_dokumen,a.keterangan,concat(a.akun_kb,'-',e.nama) as nama_kb,concat(a.akun_pph,'-',f.nama) as nama_pph,concat(b.akun_pdd,'-',g.nama) as nama_pdd,a.kode_cust, "+
							  "	   c.nama as nama_cust, "+
							  "	   h.no_kontrak,date_format(h.tanggal,'%d-%m-%Y')as tgl_kontrak,h.no_dokumen as dok_kontrak,h.no_po,b.jumlah,b.nilai "+
							  "from dmt_kb a "+
							  "inner join dmt_kb_d b on a.no_kb=b.no_kb and a.kode_lokasi=b.kode_lokasi "+
							  "inner join dmt_cust c on a.kode_cust=c.kode_cust and a.kode_lokasi=c.kode_lokasi "+
							  "left join masakun e on a.akun_kb=e.kode_akun and a.kode_lokasi=e.kode_lokasi "+
							  "left join masakun f on a.akun_pph=f.kode_akun and a.kode_lokasi=f.kode_lokasi "+
							  "left join masakun g on b.akun_pdd=g.kode_akun and b.kode_lokasi=g.kode_lokasi "+
							  "inner join dmt_kontrak h on b.no_invoice=h.no_kontrak and b.kode_lokasi=h.kode_lokasi "+
							  "where b.jenis='PDD' "+this.filter +" order by a.no_kb ";
					this.scriptSqlCount = "select count(a.no_kb) "+
							  "	from dmt_kb a "+this.filter;
				}		
				else
				{
				...
				if (this.jenisLap === "Daftar"){
					...
					this.title = new server_util_arrayList({items:["No Kontrak","Tanggal","No Dokumen","No PO","Jml Bulan","Nilai",]});
					this.widthTable = new server_util_arrayList({items:[100,80,100,120,80,100]});
			...
			systemAPI.alert("[app_saku_dmt_report_flBayarMuka]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		if (this.jenisLap === "Transaksi")
			var header = "PEMBAYARAN DIMUKA";
		else var header = "DAFTAR PEMBAYARAN DIMUKA";
	...
	convertResultToHTML: function(result,data){		
		...
		for (var r in table.rs.rows){
			...
			if (first){
				...
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.no_kb+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				...
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.nama_cust+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>Akun KasBank</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.nama_kb+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>Akun PPH</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.nama_pph+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>Akun PDD</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.nama_pdd+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "</table>";
			}
			urut+=1;
			...
			for (var c in line){
				if (c === "no_kontrak" || c === "tgl_kontrak" || c === "dok_kontrak" || c === "no_po" || c === "nilai" || c === "jumlah"){
					if (c === "nilai")
