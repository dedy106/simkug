...
//* 			SAI, PT											
//***********************************************************************************************
window.app_saku_fKlpAkses = function(owner){
	if (owner){
		try{
			window.app_saku_fKlpAkses.prototype.parent.constructor.call(this, owner);
			this.className = "app_saku_fKlpAkses";
		...
		{
			systemAPI.alert("[app_saku_fKlpAkses]::constructor:"+e);
		}
		
		this.setTabChildIndex();
	}
};
window.app_saku_fKlpAkses.extend(window.portalui_childForm);
window.app_saku_fKlpAkses.implement({
		...
		switch (event)
		{
			...
			case "simpan" :
				if (modalResult == mrOk)
				{
					...
					{				
						sql.add(sql2 + " ('"+this.e0.getText()+"','"+this.sg2.getCell(0,i)+"','"+this.app._userLog+"',now()) ");						
					}														
