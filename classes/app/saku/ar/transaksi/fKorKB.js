...
* 			SAI, PT											
*************************************************************************************************/
window.app_saku_ar_transaksi_fKorKB = function(owner)
{
  if (owner)
	{
		window.app_saku_ar_transaksi_fKorKB.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_ar_transaksi_fKorKB";
		this.setTop(60);
		this.setWidth(1280);
		this.setHeight(550);
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Setoran Kas Bank Piutang : Koreksi", 0);
		...
		try
		{
			...
			this.jurnal.sg.setColTitle(["Kode Akun","Nama","DC","Keterangan","Nilai","Kode PP","Kode DRK"]);			
			
			this.sg1.onChange.set(this,"sgChange");
			this.rearrangeChild(10,23);
		}catch(e)
		{
			alert("[app_saku_ar_transaksi_fKorKB]->constructor : "+e);
		}
	}
};
window.app_saku_ar_transaksi_fKorKB.extend(window.portalui_childForm);
window.app_saku_ar_transaksi_fKorKB.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_ar_transaksi_fKorKB.prototype.doModalResult = function(event, modalResult)
{
	switch (event)
	{
		case "clear" :
...
window.app_saku_ar_transaksi_fKorKB.prototype.insertData = function(sql)
{	
	var line;	
...
window.app_saku_ar_transaksi_fKorKB.prototype.FindBtnClick = function(sender, event)
{
	switch(sender){
		case this.eAkun :
...
window.app_saku_ar_transaksi_fKorKB.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_saku_ar_transaksi_fKorKB.prototype.doGenerate = function(sender)
{
	...
	if (sender == this.bLoad){
		...
		var rs = this.dbLib.runSQL(
				...
				" where a.progress = '0' and a.jenis = '"+this.eJenis.getText()+"' and a.kode_lokasi= '"+this.app._lokasi+"' "+
				"  ");		
...
window.app_saku_ar_transaksi_fKorKB.prototype.doSelectDate = function(sender, y,m,d)
{
	this.ePeriode.setText(y+""+(m < 10?'0'+m:m));
	this.bGenerate.click();
};
window.app_saku_ar_transaksi_fKorKB.prototype.createJurnal = function()
{
	try{
...
window.app_saku_ar_transaksi_fKorKB.prototype.sgChange = function(sender, col, row)
{
	if (col == 0){
		this.createJurnal();
	}
