window.app_saku_ar_transaksi_fKorTagihan = function(owner)
{
  if (owner)
	{
		try
		{
			window.app_saku_ar_transaksi_fKorTagihan.prototype.parent.constructor.call(this,owner);
			this.className  = "app_saku_ar_transaksi_fKorTagihan";
			this.setTop(60);
			this.setWidth(1280);
			this.setHeight(550);
			this.itemsValue = new portalui_arrayList();
			this.app._mainForm.childFormConfig(this,"mainButtonClick","Data Tagihan : Koreksi",0);
			...
			this.e_nb1.setCaption("No Tagihan");
			...
			this.e_nb.setCaption("No Tagihan Rev");
			...
			this.jurnal.sg.setColTitle(["Kode Akun","DC","Keterangan","Nilai","Kode PP","Kode DRK"]);						
		}catch(e)
		{
			alert("[app_saku_ar_transaksi_fKorTagihan]->constructor : "+e);
		}
	}
};
window.app_saku_ar_transaksi_fKorTagihan.extend(window.portalui_childForm);
window.app_saku_ar_transaksi_fKorTagihan.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_ar_transaksi_fKorTagihan.prototype.doModalResult = function(event, modalResult)
{
	switch (event)
	{
		case "clear" :
				...
				try
				{
					...
					sql.add("insert into ar_inv (no_invoice, kode_lokasi, periode, tanggal, no_dokumen, keterangan,nilai, kode_curr, kurs, nilai_ppn,no_kontrak, kode_pp, flag_hapus, progress, nilai_bd) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_tgl.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"' "+
								","+parseNilai(this.e_nilai.getText())+",'"+this.e_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+","+parseNilai(this.e_ppn.getText())+",'"+this.e_kontrak.getText()+"','"+this.app._kodePP+"','"+this.e_nb1.getText()+"','0',0) ");
					sql.add("update ar_kontrak set progress = '0' where no_kontrak = '"+this.e_kontrak.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
					sql.add("update ar_inv set progress = '0', flag_hapus='"+this.e_nb.getText()+"' where no_invoice = '"+this.e_nb1.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
					var urut = 0;
...
window.app_saku_ar_transaksi_fKorTagihan.prototype.doGenerate = function(sender)
{
	this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib, "ar_inv","no_invoice","KINV."+this.e_periode.getText().substr(2 ),"00000","and kode_lokasi ='"+this.app._lokasi+"' "));
};
window.app_saku_ar_transaksi_fKorTagihan.prototype.doChange = function(sender)
{
	if (sender.getText() != "")
	{
		if (sender == this.e_nb1){
			try
			{
				var rs = this.dbLib.execSQL("select a.no_dokumen,b.kode_cust, c.nama,a.no_kontrak, b.keterangan, a.keterangan as ket, a.kode_curr, d.nama as nmcurr, "+
						"  a.nilai, a.nilai_ppn, a.kurs, b.akun_pdpt, b.akun_piutang, b.akun_ppn "+
						" from ar_inv a "+
						"  inner join ar_kontrak b on b.no_kontrak = a.no_kontrak and b.kode_lokasi = a.kode_lokasi "+
						"  inner join cust c on c.kode_cust = b.kode_cust and b.kode_lokasi = c.kode_lokasi "+
						"  inner join curr d on d.kode_curr = a.kode_curr "+						 
						" where a.kode_lokasi = '"+this.app._lokasi +"' and a.no_invoice = '"+sender.getText()+"' ");
				if (typeof(rs) == "object"){
					this.e_cust.setText(rs.rs.rows[0].kode_cust);this.e_cust.setRightLabelCaption(rs.rs.rows[0].nama);
					this.e_dok.setText(rs.rs.rows[0].no_dokumen);					
					this.e_kontrak.setText(rs.rs.rows[0].no_kontrak);					
					...
					this.e_curr.setRightLabelCaption(rs.rs.rows[0].nmcurr);
...
window.app_saku_ar_transaksi_fKorTagihan.prototype.FindBtnClick = function(sender, event)
{
	try
	{
		if (sender == this.e_nb1)
			this.standarLib.showListData(this, "Data Kontrak",sender,undefined, 
										  "select no_invoice, keterangan from ar_inv where kode_lokasi='"+this.app._lokasi+"' and periode = '"+this.e_periode.getText()+"' ",
										  "select count(*) from ar_inv where kode_lokasi='"+this.app._lokasi+"' and periode = '"+this.e_periode.getText()+"' ",
		...
		if (sender == this.e_kontrak)
			this.standarLib.showListData(this, "Data Kontrak",sender,undefined, 
										  "select no_kontrak, keterangan from ar_kontrak where kode_lokasi = '"+this.app._lokasi+"' and kode_cust = '"+this.e_cust.getText()+"' and progress = '0' ",
										  "select count(*) from ar_kontrak where kode_lokasi = '"+this.app._lokasi+"' and kode_cust = '"+this.e_cust.getText()+"' and progress = '0'",
...
window.app_saku_ar_transaksi_fKorTagihan.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_saku_ar_transaksi_fKorTagihan.prototype.doSelectDate = function(sender, y, m, d){	
	this.e_periode.setText(y +""+ (m < 10 ? "0"+m:m));
	this.bGen.click();
};
window.app_saku_ar_transaksi_fKorTagihan.prototype.PreviewJurnal = function(sender){	
	this.createJurnal();
...
window.app_saku_ar_transaksi_fKorTagihan.prototype.createJurnal = function(sender){	
