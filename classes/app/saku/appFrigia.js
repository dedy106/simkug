...
//* 			SAI, PT											
//***********************************************************************************************
window.app_saku_appFrigia = function(owner){
	window.app_saku_appFrigia.prototype.parent.constructor.call(this, owner);
	this.className = "app_saku_appFrigia";
	this.onTerminate.set(this,"doTerminate");
	this.form = undefined;
	this.title = "Frigia";
	this.showForm();
};
window.app_saku_appFrigia.extend(window.portalui_application);
window.app_saku_appFrigia.implement({
	showForm: function(){
		try
		{
			var step = "load";			
			uses("util_dbLib;portalui_form;portalui_childForm");
			uses("app_saku_fMainFrigia");							
			step = "create";		
	...
	doKeyDown: function(charCode, buttonState, keyCode, event){
		try{			
			window.app_saku_appFrigia.prototype.parent.doKeyDown.call(this, charCode, buttonState, keyCode, event);
