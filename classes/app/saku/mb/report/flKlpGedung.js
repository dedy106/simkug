window.app_saku_mb_report_flKlpGedung = function(owner)
{
	if (owner)
	{
		window.app_saku_mb_report_flKlpGedung.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_mb_report_flKlpGedung";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Kelompok Gedung", 2);
...
window.app_saku_mb_report_flKlpGedung.extend(window.portalui_childForm);
window.app_saku_mb_report_flKlpGedung.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				var sql = "select kode_klpgedung,nama from mb_gedung_klp "+this.filter ;
				this.scriptSqlCount = "select count(*) from mb_gedung_klp "+this.filter ;
			...
			systemAPI.alert("[app_saku_mb_report_flKlpGedung]::mainButtonClick:"+e);
		}
	},
