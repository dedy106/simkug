window.app_saku_mb_report_flGedung = function(owner)
{
	if (owner)
	{
		window.app_saku_mb_report_flGedung.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_mb_report_flGedung";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Gedung", 2);
...
window.app_saku_mb_report_flGedung.extend(window.portalui_childForm);
window.app_saku_mb_report_flGedung.implement({
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
							  this.filterRep.filterStr("a.kode_klpgedung",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and")+
							  this.filterRep.filterStr("a.kode_gedung",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new portalui_arrayMap();
				var sql = "select a.kode_gedung,a.nama,a.alamat,b.nama as nama_kota,c.nama as nama_klp,a.kelas"+
						  "		from mb_gedung a "+
						  "		inner join mb_kota b on a.kode_kota=b.kode_kota and a.kode_lokasi=b.kode_lokasi "+
						  "		inner join mb_gedung_klp c on a.kode_klpgedung = c.kode_klpgedung and a.kode_lokasi=c.kode_lokasi "+this.filter ;
				this.scriptSqlCount = "select count(*) from mb_gedung "+this.filter ;
				this.title = new server_util_arrayList({items:["Kode Gedung","Nama","Alamat","Nama Kota","Nama Klp","Kelas"]});			
				this.widthTable = new server_util_arrayList({items:[80, 200,200,150,100,50]});
			...
			systemAPI.alert("[app_saku_mb_report_flGedung]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>KELOMPOK GEDUNG<br>";			
		var d = new Date();
