window.app_saku_mb_report_flJabatan = function(owner)
{
	if (owner)
	{
		window.app_saku_mb_report_flJabatan.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_mb_report_flJabatan";
		this.maximize();
...
window.app_saku_mb_report_flJabatan.extend(window.portalui_childForm);
window.app_saku_mb_report_flJabatan.implement({
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
							  this.filterRep.filterStr("a.kode_klpjob",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and")+
							  this.filterRep.filterStr("a.kode_job",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new portalui_arrayMap();
				var sql = "select a.kode_job,a.nama,b.nama as nama_klp,a.bmin,a.bmax,a.bindex "+
						  "	from mb_job a "+
						  "	inner join mb_job_klp b on a.kode_klpjob=b.kode_klpjob "+this.filter ;
				this.scriptSqlCount = "select count(*) "+
						  "	from mb_job a "+
						  "	inner join mb_job_klp b on a.kode_klpjob=b.kode_klpjob "+this.filter ;
				this.title = new server_util_arrayList({items:["Kode Jabatan","Nama","Kelompok","Min","Max","Indek"]});			
				this.widthTable = new server_util_arrayList({items:[80, 200,150,80,80,50]});
			...
			systemAPI.alert("[app_saku_mb_report_flJabatan]::mainButtonClick:"+e);
		}
	},
