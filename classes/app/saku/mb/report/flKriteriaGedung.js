window.app_saku_mb_report_flKriteriaGedung = function(owner)
{
	if (owner)
	{
		window.app_saku_mb_report_flKriteriaGedung.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_mb_report_flKriteriaGedung";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Kriteria Gedung", 2);
...
window.app_saku_mb_report_flKriteriaGedung.extend(window.portalui_childForm);
window.app_saku_mb_report_flKriteriaGedung.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				var sql = "select a.kode_gedung,b.nama as nama_gedung,a.kode_kriteria,c.nama as nama_kriteria,a.nilai,a.kode_indek "+
						  "	from mb_kriteria_gedung a "+
						  "	inner join mb_gedung b on a.kode_gedung=b.kode_gedung "+
						  "	inner join mb_kriteria c on a.kode_kriteria=c.kode_kriteria "+this.filter ;
				this.scriptSqlCount = "select count(*) "+
						  "	from mb_kriteria_gedung a "+
						  "	inner join mb_gedung b on a.kode_gedung=b.kode_gedung "+
						  "	inner join mb_kriteria c on a.kode_kriteria=c.kode_kriteria "+this.filter ;
				this.title = new server_util_arrayList({items:["Kode Gedung","Nama Gedung","Kode Kriteria","Nama Kriteria","Nilai","Kode Indek"]});			
			...
			systemAPI.alert("[app_saku_mb_report_flKriteriaGedung]::mainButtonClick:"+e);
		}
	},
