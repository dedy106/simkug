window.app_saku_mb_grafik_flCostCenter = function(owner)
{
	if (owner)
	{
		window.app_saku_mb_grafik_flCostCenter.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_mb_grafik_flCostCenter";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Grafik Cost Center", 2);
	...
	this.gridLib.SGEditData(this.sg1,2,[0,1,2], ["Chart Style","=","Bar"]);	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku_mb_grafik_flCostCenter.extend(window.portalui_childForm);
window.app_saku_mb_grafik_flCostCenter.implement({
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
							  ...
							  this.filterRep.filterStr("d.kode_gedung",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new portalui_arrayMap();
				var sql="select h.nama,sum(ifnull(case when d.nilai>bmin then round(d.nilai/bmin)*b.jumlah end,0)) as sdm, "+
						...
						"inner join mb_job_klp_d h on a.kode_klpjob=h.kode_klpjob "+this.filter
				...
				this.scriptSqlCount = "select count(*) "+
										 ...
										 " group by a.kode_klpjob";
				this.title = new server_util_arrayList({items:["Kelomok Jabatan","SDM","Nilai SDM","SDM Efisien","Nilai Efisien"]});			
				this.widthTable = new server_util_arrayList({items:[150, 50,80,50,80]});
			...
			systemAPI.alert("[app_saku_mb_grafik_flCostCenter]::mainButtonClick:"+e);
		}
	},
	...
	previewGrafik: function(sql){		
		...
		for (var i in data.rs.rows){
			...
			sdm += line.nama.replace(" ","_") +":"+line.sdm;
			ef += line.nama.replace(" ","_") +":"+line.sdm_efisien;
		}
		this.chart1.setMaxValue(maxValue);
		this.chart1.setMinValue(minValue);
		sdm += "}";
		ef += "}";
		series = "{sdm : "+sdm+",sdm_efisien :"+ef +"}";			
		this.chart1.setDataProvider(series);		
		this.chart1.setAnimate(true);
		if (this.cb_cetak.isSelected()){
