window.app_saku_mb_master_fKlpGedung = function(owner)
{
	if (owner)
	{
		window.app_saku_mb_master_fKlpGedung.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_mb_master_fKlpGedung";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Kelompok Gedung", 0);	
...
window.app_saku_mb_master_fKlpGedung.extend(window.portalui_childForm);
window.app_saku_mb_master_fKlpGedung.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0]))
			{
				...
				{									
					...
					sql.add("insert into mb_gedung_klp(kode_klpgedung,nama,kode_lokasi) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0]))
			{
				...
				{									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update mb_gedung_klp set nama = '"+this.e_nama.getText()+"' "+
	...
	doLoadClick: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var data = this.dbLib.getDataProvider(" select nama from mb_gedung_klp "+
					       " where kode_lokasi = '"+this.app._lokasi+"' and kode_klpgedung ='"+this.cb_kode.getText()+"'");
				eval("data = "+data+";");
	...
	doTampilClick: function(sender){
		try{			
			...
			var data = this.dbLib.runSQL(" select kode_klpgedung, nama from mb_gedung_klp "+
