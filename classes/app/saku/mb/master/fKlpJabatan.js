window.app_saku_mb_master_fKlpJabatan = function(owner)
{
	if (owner)
	{
		window.app_saku_mb_master_fKlpJabatan.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku_mb_master_fKlpJabatan";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Kelompok Jabatan", 0);	
...
window.app_saku_mb_master_fKlpJabatan.extend(window.portalui_childForm);
window.app_saku_mb_master_fKlpJabatan.prototype.mainButtonClick = function(sender){
...
window.app_saku_mb_master_fKlpJabatan.prototype.doRequestReady = function(sender, methodName, result){
	switch (methodName)
	{
		case "listData" : 
...
window.app_saku_mb_master_fKlpJabatan.prototype.doModalResult = function(event, modalResult, value){
	try{
		switch (event){
			case "Add" :
			   if ( modalResult == mrOk){
					try{
						...
						data.set("kode_klpjob",this.e0.getText());
			...
			case "simpan" :
				if (modalResult == mrOk)
				{
					...
					try
					{						
						var sql2,sql1 = "delete from mb_job_klp where kode_lokasi = '"+this.elokasi.getText()+"'";
						// kode_lokasi,						
						...
						if (value != ""){
							if (driver.search("mssql") != -1){
								...
								for (var i in data){
									sql2 = "insert into mb_job_klp (kode_klpjob, kode_lokasi,nama, level_spasi,level_lap, tipe, sum_header, kode_induk, rowindex) values ";
									sql2 += data[i];
									sql.add(sql2);
								}
		...
		systemAPI.alert("[app_saku_mb_master_fKlpJabatan]::doModalResult:"+e);
	}
};
window.app_saku_mb_master_fKlpJabatan.prototype.loadMenu = function(daftarNrc){
	try{
		...
		for (var r in menu.objList)
		{
			...
			kode = itemValues.get("kode_klpjob");							
			if (kode != "")
			{
...
//-------------------------------------
window.app_saku_mb_master_fKlpJabatan.prototype.doClick = function(sender){	
	var nrc = this.dblib.runSQL("select * from mb_job_klp where  kode_lokasi = '"+this.elokasi.getText()+"' order by rowindex",0,0);
	this.loadMenu(nrc);	
};
window.app_saku_mb_master_fKlpJabatan.prototype.entriesClick = function(sender){
	try
	{ 
		...
		{			
			...
			}else if (sender == this.delBtn)	
			{
				var item = this.treev.getSelectedItem();	
				this.selectedItem = item;
				system.confirm(this, "Remove","Yakin Kelompok Jabatan "+item.getCaption()+" akan dihapus?");
...
window.app_saku_mb_master_fKlpJabatan.prototype.treeClick = function(item){
};
window.app_saku_mb_master_fKlpJabatan.prototype.getTreeData = function(node){
	var result = "";
	var child = undefined;
...
window.app_saku_mb_master_fKlpJabatan.prototype.getSummary = function(node){
	var result = "";
	var child = undefined;
	if (node instanceof portalui_treeView)
	{
...
window.app_saku_mb_master_fKlpJabatan.prototype.doFindLokasi = function(sender)
{
	if (this.app._userStatus =="A")
...
window.app_saku_mb_master_fKlpJabatan.prototype.setSummaryItems = function(data){
	this.e4.clearItem();
	for (var i in data)
	{
...
window.app_saku_mb_master_fKlpJabatan.prototype.copyChilds = function(from, to){
	var temp = undefined;
	...
	{			
		...
		chld.data.set("kode_klpjob",temp.getKode());					
...
window.app_saku_mb_master_fKlpJabatan.prototype.switchItem = function(from, to){
	...
	child.data.set("kode_klpjob",item.getKode());					
...
window.app_saku_mb_master_fKlpJabatan.prototype.moveItem = function(item, owner)
{	
