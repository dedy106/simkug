window.app_saku_anggaran_master_fFSRKM = function(owner)
{
	if (owner)
	{
		window.app_saku_anggaran_master_fFSRKM.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_anggaran_master_fFSRKM";
		this.setTop(60);
		this.setWidth(1280);
		this.setHeight(550);
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Versi RKM", 0);	
		...
		try{
			...
			this.e0.setSQL("select kode_fsrkm, nama from fsrkm where kode_lokasi = '"+this.app._lokasi+"' ",["kode_fsrkm","nama"],undefined,["Kode FS","Nama"],"and","Data RKM",false);
		}catch(e)
		{
			alert(e);
		}
...
window.app_saku_anggaran_master_fFSRKM.extend(window.portalui_childForm);
window.app_saku_anggaran_master_fFSRKM.implement({
		...
		try{
			...
			switch (event)
			{
				...
				case "simpan" :									
					...
					sql.add("insert into fsrkm (kode_fsrkm, nama,tgl_awal, tgl_akhir, flag_status, tgl_input,nik_user, kode_lokasi ) values ('"+this.e0.getText()+"','"+this.e1.getText()+"','"+this.d1.getDate()+"','"+this.d2.getDate()+"','"+status+"','"+(new Date).getDateStr()+"','"+this.app._userLog+"','"+this.elokasi.getText()+"' ) ");
					this.dbLib.execArraySQL(sql);						
				break;
				case "ubah" :				
					uses("server_util_arrayList");
					sql = new server_util_arrayList();
					sql.add("update fsrkm set nama = '"+this.e1.getText()+"',flag_status = '"+status+"',tgl_awal='"+this.d1.getDate()+"',tgl_akhir='"+this.d2.getDate()+"',tgl_input='"+(new Date).getDateStr()+"',nik_user='"+this.app._userLog+"' where kode_fsrkm = '"+this.e0.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"' ");
					this.dbLib.execArraySQL(sql);						
				break;
				case "hapus" :			   
					uses("server_util_arrayList");
					sql = new server_util_arrayList();
					var rs = this.dbLib.runSQL("select kode_fsrkm from drk where kode_fsrkm = '"+this.e0.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"' ");
					if (rs instanceof portalui_arrayMap)
						if (rs.getTag1() != 0)
							throw("kode fs ini sudah terpakai. tidak dapat dihapus");
					sql.add("delete from  fsrkm where kode_fsrkm = '"+this.e0.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"' ");
					this.dbLib.execArraySQL(sql);				
				break;
			}
			this.e0.setFocus();
	...
	doEditChange: function(sender){		
		if (sender === this.elokasi){
			this.e0.setSQL("select kode_fsrkm, nama from fsrkm where kode_lokasi = '"+this.elokasi.getText()+"' ",["kode_fsrkm","nama"],undefined,["Kode FS","Nama"],"and","Data RKM",false);			
		}
		if (sender == this.e0){
			if (this.e0.getText() != ""){
				try{
					var data = this.dbLib.runSQL("select * from fsrkm where kode_fsrkm = '"+this.e0.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"' ");
