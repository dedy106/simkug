window.app_saku_anggaran_master_fNeraca = function(owner)
{
	if (owner)
	{
		window.app_saku_anggaran_master_fNeraca.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku_anggaran_master_fNeraca";
		...
		this.eKlp.setSQL("select kode_fs, nama from angg_fs where kode_lokasi = '"+this.app._lokasi+"' ",["kode_fs","nama"],undefined,["Kode FS","Nama"],"and","Data Versi Neraca");	
	}
};
window.app_saku_anggaran_master_fNeraca.extend(window.portalui_childForm);
window.app_saku_anggaran_master_fNeraca.implement({
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
						...
						try
						{
							...
							var sql2,sql1 = "delete from angg_neraca where modul = '"+modul+"'  and kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"'";					// kode_lokasi,
							var sql = new server_util_arrayList();
							sql.add("delete from relakun  where kode_lokasi = '"+this.elokasi.getText()+"' and kode_fs = '"+this.eKlp.getText()+"' "+
								"and kode_neraca in (select kode_neraca from angg_neraca where modul = '"+modul+"'  and kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"')");																		
							sql.add(sql1);							
							//if mysql langsung
							...
							if (driver.search("mssql") != -1){
								...
								for (var i in data){
									sql2 = "insert into angg_neraca(kode_neraca, kode_lokasi, kode_fs, nama, level_spasi, level_lap, tipe, sum_header, jenis_akun, kode_induk, rowindex, modul) values ";
									sql2 += " "+ data[i] +" ";
									sql.add(sql2);
								}	
								if (this.value2 != ""){
								...
								sql.add(sql2);
								if (this.value2 != ""){
										var sql3 = "insert into relakun(kode_neraca, kode_fs, kode_akun,  kode_lokasi) values ";								
	...
	doClick: function(sender){
		if (this.e0.getText() != "")
		{
			...
			var nrc = this.dblib.runSQL("select * from angg_neraca where Modul = '"+modul+"' and kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"'  order by rowindex",0,0);
			this.loadMenu(nrc);
		}	
	},
		...
		{ 
			...
			}else
			{
				if (sender == this.addBtn || sender == this.editBtn){
					uses("app_saku_anggaran_master_fNeracaDetail");
					if (this.entryMenu == undefined)
						this.entryMenu = new app_saku_anggaran_master_fNeracaDetail(this.app,this.e0.getText());
	...
	doEditChange: function(sender){
		if (sender == this.elokasi){			
			this.eKlp.setText("");
			this.treev.clear();
			this.eKlp.setSQL("select kode_fs, nama from angg_fs where kode_lokasi = '"+sender.getText()+"' ",["kode_fs","nama"],undefined,["Kode FS","Nama"],"and","Data Versi Neraca");	
