window.app_saku_anggaran_report_flAgg = function(owner)
{
	if (owner)
	{
		window.app_saku_anggaran_report_flAgg.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_anggaran_report_flAgg";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Anggaran", 2);
	...
	this.gridLib.SGEditData(this.sg1,7,[0,1,2],["Status Anggaran","=","Closing"]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku_anggaran_report_flAgg.extend(window.portalui_childForm);
window.app_saku_anggaran_report_flAgg.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
	...
	if (row == 4)
	{
		this.filterRep.ListDataSGFilter(this, "Data RKM",this.sg1, this.sg1.row, this.sg1.col,
									  "select kode_drk, nama from drk where tipe='Posting' "+
									   this.filterRep.filterStr("tahun",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," and ")+
...
window.app_saku_anggaran_report_flAgg.prototype.doSelectCell = function(sender, col, row)
{
	if (this.app._userStatus=="A")
	{
...
window.app_saku_anggaran_report_flAgg.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		{			
			if (this.sg1.getCell(2,7)=="Closing")
			{
				var sql = "call sp_agg_tw ('"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
			}
			else
			{
				var sql = "call sp_agg_tw_org ('"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
			}			
		...
		if (this.sg1.getCell(2,6)=="Semester")
		{
			...
			{
				var sql = "call sp_agg_sem_org ('"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
			}
		...
		if (this.sg1.getCell(2,6)=="Bulanan")
		{
			...
			{
				var sql = "call sp_agg_bulan_org ('"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
			}
...
window.app_saku_anggaran_report_flAgg.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_anggaran_report_flAgg.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page, this.pager, this.showFilter,this.lokasi, this.filter2);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_anggaran_report_flAgg.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_anggaran_report_flAgg.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_anggaran_report_flAgg.prototype.doRowPerPageChange = function(sender, rowperpage)
{
