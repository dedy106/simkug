window.app_saku_anggaran_report_flAggDis = function(owner)
{
	if (owner)
	{
		window.app_saku_anggaran_report_flAggDis.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_anggaran_report_flAggDis";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Anggaran Relokasi", 2);
...
window.app_saku_anggaran_report_flAggDis.extend(window.portalui_childForm);
window.app_saku_anggaran_report_flAggDis.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
	...
	if (row == 2)
	{
		var sql1="select no_agg,keterangan from anggaran_m "+
				 ...
				 " and jenis='DIST'  order by no_agg ";
		var sql2="select count(no_agg) from anggaran_m "+
				 this.filterRep.filterStr("tahun",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
				 this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
				 " and jenis='DIST'";
		this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,["no_agg","keterangan"],"and",["No Agg","Keterangan"]);
	}
};
window.app_saku_anggaran_report_flAggDis.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
...
window.app_saku_anggaran_report_flAggDis.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.nama_report="server_report_anggaran_rptAggDis";
...
window.app_saku_anggaran_report_flAggDis.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_anggaran_report_flAggDis.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.lokasi, this.showFilter);	
	this.page=page;
	this.allBtn = false;
};
window.app_saku_anggaran_report_flAggDis.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_anggaran_report_flAggDis.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_anggaran_report_flAggDis.prototype.doRowPerPageChange = function(sender, rowperpage)
{
