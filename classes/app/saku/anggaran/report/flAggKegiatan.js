window.app_saku_anggaran_report_flAggKegiatan = function(owner)
{
	if (owner)
	{
		window.app_saku_anggaran_report_flAggKegiatan.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_anggaran_report_flAggKegiatan";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Anggaran Kegiatan", 2);
	...
	this.gridLib.SGEditData(this.sg1,6,[0,1,2],["Realisasi","=","General Ledger"]);
...
window.app_saku_anggaran_report_flAggKegiatan.extend(window.portalui_childForm);
window.app_saku_anggaran_report_flAggKegiatan.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
...
window.app_saku_anggaran_report_flAggKegiatan.prototype.doSelectCell = function(sender, col, row)
{
	if (this.app._userStatus=="A")
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["1234","123","123","123","123","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[0,2,2,2,2,0]);
	}
	else
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["1234","2","123","2","123","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[0,3,2,3,2,0]);
	}
	if (row == 0)
	{
...
window.app_saku_anggaran_report_flAggKegiatan.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		{ 
			var sql = "call sp_agg_pakai ('"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
		}
		else
		{
			var sql = "call sp_agg_pakai_gl ('"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
		}
		this.dbLib.execQuerySync(sql);	
		this.nama_report="server_report_anggaran_rptAggKegiatan";
...
window.app_saku_anggaran_report_flAggKegiatan.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_anggaran_report_flAggKegiatan.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page, this.pager, this.showFilter,this.lokasi, this.filter2);	
	this.page=page;
	this.allBtn = false;
};
window.app_saku_anggaran_report_flAggKegiatan.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_anggaran_report_flAggKegiatan.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_anggaran_report_flAggKegiatan.prototype.doRowPerPageChange = function(sender, rowperpage)
{
