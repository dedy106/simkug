window.app_saku_anggaran_report_flDrk = function(owner)
{
	if (owner)
	{
		window.app_saku_anggaran_report_flDrk.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_anggaran_report_flDrk";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan DRK", 2);
	...
	this.gridLib.SGEditData(this.sg1,1,[0,1,2],["Kode RKM","All",""]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku_anggaran_report_flDrk.extend(window.portalui_childForm);
window.app_saku_anggaran_report_flDrk.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 0)
	{
...
window.app_saku_anggaran_report_flDrk.prototype.doSelectCell = function(sender, col, row)
{
	this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1],["13","13"]);
	this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1],[2,2]);
};
window.app_saku_anggaran_report_flDrk.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		{ 
			this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Lokasi","=",this.app._lokasi]);
			this.gridLib.SGEditData(this.sg1,1,[0,1,2],["Kode DRK","All",""]);
		}else
		{
			...
			this.nama_report="server_report_anggaran_rptDrk";
	...
	{
		alert("[app_saku_anggaran_report_flDrk]::mainButtonClick:"+e);
	}
};
window.app_saku_anggaran_report_flDrk.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_anggaran_report_flDrk.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page, this.pager, this.showFilter, this.app._namalokasi);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_anggaran_report_flDrk.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_anggaran_report_flDrk.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_anggaran_report_flDrk.prototype.doRowPerPageChange = function(sender, rowperpage)
{
