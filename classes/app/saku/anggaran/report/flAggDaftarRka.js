window.app_saku_anggaran_report_flAggDaftarRka = function(owner)
{
	if (owner)
	{
		window.app_saku_anggaran_report_flAggDaftarRka.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_anggaran_report_flAggDaftarRka";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Daftar Rencana Kerja & Anggaran", 2);
...
window.app_saku_anggaran_report_flAggDaftarRka.extend(window.portalui_childForm);
window.app_saku_anggaran_report_flAggDaftarRka.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
...
window.app_saku_anggaran_report_flAggDaftarRka.prototype.doSelectCell = function(sender, col, row)
{
	this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["1234","123","123","123","123","3","3","3"]);
...
window.app_saku_anggaran_report_flAggDaftarRka.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		{
			var sql = "call sp_agg_tw_rka ('"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
			this.nama_report="server_report_anggaran_rptAggTwRka";
		}
		if (this.sg1.getCell(2,6)=="Semester")
		{
			var sql = "call sp_agg_sem_rka ('"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
			this.nama_report="server_report_anggaran_rptAggSemRka";
		}
		if (this.sg1.getCell(2,6)=="Bulanan")
		{
			var sql = "call sp_agg_bulan_rka ('"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
			this.nama_report="server_report_anggaran_rptAggBulanRka";
		}
...
window.app_saku_anggaran_report_flAggDaftarRka.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_anggaran_report_flAggDaftarRka.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page, this.pager, this.showFilter,this.lokasi, this.filter2);
	this.page=page;
	this.allBtn = false;
};
window.app_saku_anggaran_report_flAggDaftarRka.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_anggaran_report_flAggDaftarRka.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku_anggaran_report_flAggDaftarRka.prototype.doRowPerPageChange = function(sender, rowperpage)
{
