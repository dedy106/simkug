window.app_saku_anggaran_trail_fTrailAggRkap = function(owner)
{
	if (owner)
	{
		window.app_saku_anggaran_trail_fTrailAggRkap.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_anggaran_trail_fTrailAggRkap";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Audit Trail Anggaran RKAP", 2);
	...
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Kode FS","=","FS001"]);
	...
	this.compArray = ["this.p1","this.p2","this.p3","this.p4","this.p5"];
	this.gridArray = ["this.sg1","this.sg2","this.sg3","this.sg4","this.sg5"];
	this.actComp = 1;
};
window.app_saku_anggaran_trail_fTrailAggRkap.extend(window.portalui_childForm);
window.app_saku_anggaran_trail_fTrailAggRkap.prototype.doSg2DblClick = function(sender, colIdx, rowIdx)
{
	...
	var sql = "call sp_glma_trail_tmp ('"+this.sg1.getCell(2,1)+"','"+this.sg2.getCell(1,rowIdx)+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
...
window.app_saku_anggaran_trail_fTrailAggRkap.prototype.doSg3DblClick = function(sender, colIdx, rowIdx)
{
...
window.app_saku_anggaran_trail_fTrailAggRkap.prototype.SetSg4Mutasi = function()
{
	try
	{
	...
	{
		alert("[app_saku_anggaran_trail_fTrailAggRkap]::SetSg4Mutasi:"+e);
	}
};
window.app_saku_anggaran_trail_fTrailAggRkap.prototype.doSg4DblClick = function(sender, colIdx, rowIdx)
{	
	try{
...
window.app_saku_anggaran_trail_fTrailAggRkap.prototype.SetSg5Mutasi = function()
{
	try
	{
		var debet = kredit = 0;
...
window.app_saku_anggaran_trail_fTrailAggRkap.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
...
window.app_saku_anggaran_trail_fTrailAggRkap.prototype.doSelectCell = function(sender, col, row)
{
	this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["1234","123","123","3"]);
...
window.app_saku_anggaran_trail_fTrailAggRkap.prototype.mainButtonClick = function(sender)
{	
	if (sender == this.app._mainForm.bPreview){
		try
		{
			...
			if (this.app._dbEng == "ado_mssql")
			{
				var sql="exec sp_rkap '"+this.sg1.getCell(2,2)+"','L','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,1)+"','admin'";
			}
			this.dbLib.execQuerySync(sql);	
				...
				if (this.p1.visible){ 
					...
					this.sg2.setColTitle(new Array("No","Kode","Nama","Triwulan I","Triwulan II","Triwulan III","Triwulan IV","Total"));
					this.sg2.setTag("2");
					...
					{		
						this.sg2.clearAll();
						var line,data = this.dbLib.runSQL("select kode_neraca,fn_spasi(nama,level_spasi) as nama,n1,n2,n3,n4,(n1+n2+n3+n4) as n5 from neraca_tmp where modul='L' and nik_user='"+this.nik_user+"' order by rowindex ");
						if (data instanceof portalui_arrayMap)
						{						
...
window.app_saku_anggaran_trail_fTrailAggRkap.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_anggaran_trail_fTrailAggRkap.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page, 30, "YAKES", this.showFilter,this.filter2);	
};
window.app_saku_anggaran_trail_fTrailAggRkap.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_anggaran_trail_fTrailAggRkap.prototype.doAllPageClick = function(sender)
{
	this.report.preview(this.nama_report,this.filter, 1, this.viewer.getTotalPage() * 30, this.app._namaLokasi,this.showFilter);	
};
window.app_saku_anggaran_trail_fTrailAggRkap.prototype.doPdfClick = function(sender)
{
...
window.app_saku_anggaran_trail_fTrailAggRkap.prototype.doXlsClick = function()
{
