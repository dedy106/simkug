window.app_saku_anggaran_trail_fTrailAgg = function(owner)
{
	if (owner)
	{
		window.app_saku_anggaran_trail_fTrailAgg.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku_anggaran_trail_fTrailAgg";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Audit Trail Anggaran", 2);
		...
		this.sg1.onEllipsClick.set(this, "doEllipseClick");
			...
			this.sg1.setCell(0,5,"Jenis Akun");
			this.sg1.setCell(0,6,"Periodik laporan");
	...
	this.filterRep.setSGFilterRowTipe(this.sg1, 0,[0,1,2,3,4,5,6],["1234","123","123","123","123","3","3"]);
	this.filterRep.setSGFilterRowButtonStyle(this.sg1, 0,[0,1,2,3,4,5,6],[0,2,2,2,2,0,0]);
	...
	this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Tahun","=","2008"]);
...
window.app_saku_anggaran_trail_fTrailAgg.extend(window.portalui_childForm);
window.app_saku_anggaran_trail_fTrailAgg.prototype.doSg3DblClick = function(sender, colIdx, rowIdx)
{  
	try
	{
...
window.app_saku_anggaran_trail_fTrailAgg.prototype.SetSg4Mutasi = function()
{
	try
	{
	...
	{
		alert("[app_saku_anggaran_trail_fTrailAgg]::SetSg4Mutasi:"+e);
	}
};
window.app_saku_anggaran_trail_fTrailAgg.prototype.doSg4DblClick = function(sender, colIdx, rowIdx)
{	
	try{
...
window.app_saku_anggaran_trail_fTrailAgg.prototype.SetSg5Mutasi = function()
{
	try
	{
		var debet = kredit = 0;
...
window.app_saku_anggaran_trail_fTrailAgg.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==1)
	{
...
window.app_saku_anggaran_trail_fTrailAgg.prototype.doSelectCell = function(sender, col, row)
{
	...
	if (row == 0)
	{
		...
		this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["2007","2008"]);
	}
	if (row == 5)
	{
		this.sg1.columns.get(2).pickList.clear();
...
window.app_saku_anggaran_trail_fTrailAgg.prototype.mainButtonClick = function(sender)
{	
	if (sender == this.app._mainForm.bPreview){
		try
		{
			...
			if (this.sg1.getCell(2,6)=="Triwulan")
			{
				...
				{
					var sql = "call sp_agg_tw ('"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
				}
				if (this.app._dbEng == "ado_mssql")
				{
					var sql = "sp_agg_tw '"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,0)+"','admin'";
				}
				this.nama_report="server_report_anggaran_rptAggTw";
			}
			if (this.sg1.getCell(2,6)=="Semester")
			{
				if (this.app._dbEng == "mysqlt")
				{
					var sql = "call sp_agg_sem ('"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
				}
				if (this.app._dbEng == "ado_mssql")
				{
					var sql = "sp_agg_sem '"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,0)+"','admin'";
				}			
				this.nama_report="server_report_anggaran_rptAggSem";
			}
			if (this.sg1.getCell(2,6)=="Bulanan")
			{
				if (this.app._dbEng == "mysqlt")
				{
					var sql = "call sp_agg_bulan ('"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,0)+"','"+this.nik_user+"')";
				}
				if (this.app._dbEng == "ado_mssql")
				{
					var sql = "sp_agg_bulan '"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,0)+"','admin'";
				}			
				this.nama_report="server_report_anggaran_rptAggBulan";
			}
			this.dbLib.execQuerySync(sql);
				...
				if (this.p1.visible){ 
					...
					this.sg2.setColTitle(new Array("No","Kode Akun","Nama Akun","Kode PP","Nama PP","Kode DRK","Nama DRK","Januari","Februari","Maret","April","Mei","Juni","Juli","Agustus","September","Oktober","November","Desember"));
					this.sg2.setTag("2");
					...
					{		
						this.sg2.clearAll();
						var line,data = this.dbLib.runSQL("select kode_akun,nama_akun,kode_pp,nama_pp,kode_drk,nama_drk,n1,n2,n3,n4,n5,n6,n7,n8,n9,n10,n11,n12 from glma_drk_tmp where nik_user='"+this.nik_user+"' order by kode_akun ");
						if (data instanceof portalui_arrayMap)
						{						
...
window.app_saku_anggaran_trail_fTrailAgg.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_saku_anggaran_trail_fTrailAgg.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page, 30, "YAKES", this.showFilter,this.filter2);	
};
window.app_saku_anggaran_trail_fTrailAgg.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku_anggaran_trail_fTrailAgg.prototype.doAllPageClick = function(sender)
{
	this.report.preview(this.nama_report,this.filter, 1, this.viewer.getTotalPage() * 30, this.app._namaLokasi,this.showFilter);	
};
window.app_saku_anggaran_trail_fTrailAgg.prototype.doPdfClick = function(sender)
{
...
window.app_saku_anggaran_trail_fTrailAgg.prototype.doXlsClick = function()
{
