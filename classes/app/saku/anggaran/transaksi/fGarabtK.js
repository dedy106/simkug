window.app_saku_anggaran_transaksi_fGarabtK = function(owner)
{  
	if (owner)
	{
		window.app_saku_anggaran_transaksi_fGarabtK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_anggaran_transaksi_fGarabtK";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Data Anggaran Tambahan (ABT): Koreksi", 0);
...
window.app_saku_anggaran_transaksi_fGarabtK.extend(window.portalui_childForm);
window.app_saku_anggaran_transaksi_fGarabtK.prototype.doSelect = function(sender, year, month, day)
{
	if (month < 10)
		month = "0"+month;
	this.ed_period.setText(year.toString());
};
window.app_saku_anggaran_transaksi_fGarabtK.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_anggaran_transaksi_fGarabtK.prototype.simpan = function(event)
{		
...
window.app_saku_anggaran_transaksi_fGarabtK.prototype.doModalResult = function(event, modalResult)
{
...
window.app_saku_anggaran_transaksi_fGarabtK.prototype.genClick = function(sender)
{
	try
	{
...
window.app_saku_anggaran_transaksi_fGarabtK.prototype.doEditChange = function(sender)
{
	if (sender == this.ed_nb){
		...
		if (typeof(rs) == "object"){
			...
			rs = this.dbLib.execSQL("select a.kode_akun, b.nama as nama_akum, a.kode_pp, c.nama as nama_dept, a.kode_drk, d.nama, substring(a.periode,4,2), a.volume, a.satuan, a.nilai_sat "+
...
window.app_saku_anggaran_transaksi_fGarabtK.prototype.FindBtnClick = function(sender, event)
{
	try
	{
		if (this.ed_nb){			
			this.standarLib.showListData(this, "Daftar Upload Anggaran",sender,undefined, 
										  "select distinct no_agg, keterangan  from anggaran_m where kode_lokasi ='"+this.app._lokasi+"' and jenis = 'ABT' and tanggal <= '"+this.dp_tgl1.getDateString()+"' ",
										  "select count(distinct no_agg, keterangan)  from anggaran_m where kode_lokasi ='"+this.app._lokasi+"' and jenis = 'ABT' and tanggal <= '"+this.dp_tgl1.getDateString()+"'",
...
window.app_saku_anggaran_transaksi_fGarabtK.prototype.doFindBtnClick = function(sender, col, row) 
{
	try
	{
		switch(col)
	...
	{
		alert("[app_saku_anggaran_transaksi_fGarabtK] : doFindBtnClick : " + e);
	}
};
window.app_saku_anggaran_transaksi_fGarabtK.prototype.doNilaiChange = function()
{
	try
	{
		var tot = 0;
	...
	{
		alert("[app_saku_anggaran_transaksi_fGarabtK]::doNilaiChange:"+e);
	}
};
window.app_saku_anggaran_transaksi_fGarabtK.prototype.doCellExit = function()
{
	this.sg1.validasi();
};
window.app_saku_anggaran_transaksi_fGarabtK.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_saku_anggaran_transaksi_fGarabtK.prototype.doAfterLoad = function(sender, result,data)
{
	try{	
		if (result){				
			var rs, arr;
