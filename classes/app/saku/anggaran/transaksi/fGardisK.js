window.app_saku_anggaran_transaksi_fGardisK = function(owner)
{  
	if (owner)
	{
		window.app_saku_anggaran_transaksi_fGardisK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku_anggaran_transaksi_fGardisK";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Perubahan Data Anggaran: Koreksi", 0);
...
window.app_saku_anggaran_transaksi_fGardisK.extend(window.portalui_childForm);
window.app_saku_anggaran_transaksi_fGardisK.prototype.doSelect = function(sender, year, month, day)
{
	if (month < 10)
		month = "0"+month;
	this.ed_period.setText(year.toString());
};
window.app_saku_anggaran_transaksi_fGardisK.prototype.mainButtonClick = function(sender)
{
...
window.app_saku_anggaran_transaksi_fGardisK.prototype.simpan = function(event)
{		
	if (this.standarLib.checkEmptyByTag(this, new Array("0","1")))
	{
		try
		{
			...
			sql.add("delete from anggaran_m where kode_lokasi ='"+this.app._lokasi+"' and no_agg = '"+this.ed_nb.getText()+"' ");
			sql.add("delete from anggdist_d where kode_lokasi ='"+this.app._lokasi+"' and no_agg = '"+this.ed_nb.getText()+"' ");
			sql.add("delete from anggaran_d where kode_lokasi ='"+this.app._lokasi+"' and no_agg = '"+this.ed_nb.getText()+"' ");
...
window.app_saku_anggaran_transaksi_fGardisK.prototype.doModalResult = function(event, modalResult)
{
...
window.app_saku_anggaran_transaksi_fGardisK.prototype.genClick = function(sender)
{
	try
	{
...
window.app_saku_anggaran_transaksi_fGardisK.prototype.doEditChange = function(sender)
{
	if (sender == this.ed_nb){	
		...
		if (typeof(rs) == "object"){
			...
			rs = this.dbLib.execSQL("select a.kode_pp1,a.kode_akun1,a.kode_drk1,substring(a.periode1,5,2) as periode1,a.nilai,a.kode_pp2,a.kode_akun2,a.kode_drk2,substring(a.periode2,4,2) as periode2 "+
				"	from anggdist_d a "+
...
window.app_saku_anggaran_transaksi_fGardisK.prototype.showClick = function(sender)
{
...
window.app_saku_anggaran_transaksi_fGardisK.prototype.okClick = function(sender)
{
...
window.app_saku_anggaran_transaksi_fGardisK.prototype.FindBtnClick = function(sender, event)
{
	try
	{
		if (sender == this.ed_nb) 
		{
		    this.standarLib.showListData(this, "Daftar Anggaran",sender,undefined, 
										  "select distinct no_agg, keterangan  from anggaran_m where kode_lokasi ='"+this.app._lokasi+"' and jenis = 'DIST' and posted = 'F' and tanggal <= '"+this.dp_tgl1.getDateString()+"' ",
										  "select count(distinct no_agg, keterangan)  from anggaran_m where kode_lokasi ='"+this.app._lokasi+"' and jenis = 'DIST' and posted = 'F' and tanggal <= '"+this.dp_tgl1.getDateString()+"'",
										  ["no_agg","keterangan"],"and",["No Anggaran.","Deskripsi"],false);
		}
		...
		if (sender == this.cb_akun) 
		{
			this.standarLib.showListData(this, "Daftar Mata Anggaran",this.cb_akun,undefined, 
										  ...
										  "       inner join drk d on b.kode_lokasi=d.kode_lokasi and b.kode_drk=d.kode_drk and d.tahun ='"+this.ed_period.getText()+"' "+
										  "where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"' and c.kode_pp='"+this.cb_pp.getText()+"' and d.kode_drk='"+this.cb_drk.getText()+"' ",
										  new Array("a.kode_akun","a.nama"),"and", new Array("Kode MTA","Deskripsi"),false);
			this.standarLib.clearByTag(this,["3"],undefined);				
		}
...
window.app_saku_anggaran_transaksi_fGardisK.prototype.doNilaiChange = function()
{
	try
	{
		var tot = 0;
	...
	{
		alert("[app_saku_anggaran_transaksi_fGardisK]::doNilaiChange:"+e);
	}
};
window.app_saku_anggaran_transaksi_fGardisK.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_saku_anggaran_transaksi_fGardisK.prototype.doAfterLoad = function(sender, result,data)
{
	try{	
		if (result){
			var rs, arr;
