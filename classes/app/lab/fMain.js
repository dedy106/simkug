...
//* 			SAI, PT											
//***********************************************************************************************
window.app_lab_fMain = function(owner)
{
	if (owner)
	{	
		try
		{
			window.app_lab_fMain.prototype.parent.constructor.call(this, owner);			
...
window.app_lab_fMain.extend(window.form);
window.app_lab_fMain.implement({
	createToolbar: function(){
		this.pTool = new panel(this,{bound:[0,0,this.width,30],border:0,color:"#EEEEEE",shadow:true});//DCEBF9
			...
			this.bRun = new image(this.pTool,{bound:[281,3,22,22],showHint:true,hint:"Execute",image:"icon/"+system.getThemes()+"/ok.png",click:[this,"doExecuteMenu"]});									
			...
			this.bBack = new image(this.pTool,{bound:[310,3,22,22],showHint:true,hint:"Back", image:"icon/"+system.getThemes()+"/Back.png",click:[this,"doExecuteMenu"]});									
			this.bBack.setCursor("pointer");
			this.bRecall = new image(this.pTool,{bound:[335,3,22,22],showHint:true,hint:"Refresh",image:"icon/"+system.getThemes()+"/bRefresh.png", click:[this,"doRecall"]});				
			this.bRecall.setCursor("pointer");
			this.bExit = new image(this.pTool,{bound:[360,3,22,22],showHint:true, hint:"Close",click:[this,"bTutupClick"],image:"icon/"+system.getThemes()+"/bExit.png"});			
			this.bExit.setCursor("pointer");
			
			this.separator1 = new control(this.pTool,{bound:[385,0,1,30]});
            this.separator1.getCanvas().style.background = system.getConfig("form.button.color");
            this.bClose = new image(this.pTool,{bound:[389,3,22,22],showHint:true,hint:"End Session",image:"icon/"+system.getThemes()+"/bEndSession.png",click:[this,"doClose"]});			
			this.bGenLocal = new image(this.pTool,{bound:[411,3,22,22],showHint:true,hint:"Open New Window",image:"icon/"+system.getThemes()+"/bGenLocal.png",click:[this,"openNewWindow2"]});
...
////------------------------------------ panel button -------------------------		
			...
			this.refMenu = new image(this.pButton,{bound:[10,2,23,23],hint:"Refresh Menu",image:"icon/"+system.getThemes()+"/bRefresh.png",click:[this,"getUserMenu"], cursor:"Pointer", showHint: true});
...
//----------------------------------------------------------------- tree View-----------------------------------						
			this.treev = new treeView(this,{bound:[10,95,400,this.height - 130],childLength:500,dblClick:[this,"treeClick"]});			
...
//--------------------------------`------------------
	...
	gotoLogin: function(){
		uses("app_lab_fLogin");
		this.form = new app_lab_fLogin(this);
	...
	initDashboard: function(){		
		try{
			uses("saiCBBL;saiEdit;saiGrid;sgNavigator;app_lab_dashboard;pageControl;util_ajaxCaller;listView;util_file;app_lab_remote_dataProvider;tinymceCtrl;reportViewer;server_report_report;server_util_Map");
			this.dashboard = new app_lab_dashboard(this,{bound:[this.treev.width + 20, this.treev.top, this.width - this.treev.width - 30, this.treev.height]});		
	...
	doAfterLogin: function(){
		try{		    
			...
			this.lUser = new label(this,{bound:[this.width - 120,37,80,20],caption:"System Info",color:"#299bcf",click:[this,"doBtnList"]});
	...
	bTutupClick: function(sender){	
		try{			
			this.pReport.hide();
			this.bDraft.hide();
			if (this.form instanceof app_lab_fLogin && sender == this.bExit){
				return false;
			}			
			if (this.form == undefined){
