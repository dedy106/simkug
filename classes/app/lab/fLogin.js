...
//* 			SAI, PT											
//***********************************************************************************************
window.app_lab_fLogin = function(owner){
	if (owner){
		try{		
			window.app_lab_fLogin.prototype.parent.constructor.call(this, owner);
			this.className  = "app_lab_fLogin";			
			...
			this.iLogo = new image(this.p2, {bound:[50,20,400,225], image:window.app_logo});
			...
			this.bLogin = new button(this.p3, {bound:[20,250,80,20],caption:"Login",icon:"url(icon/"+system.getThemes()+"/login.png)", click:[this,"doClick"]});
			...
			systemAPI.alert("[app_lab_fLogin]::oncreate lib : ",e);
			//alert(e);
		}
	}
};
window.app_lab_fLogin.extend(window.childForm);
window.app_lab_fLogin.implement({
	doClose: function(){
