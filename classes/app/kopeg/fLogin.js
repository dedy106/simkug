...
//* 			SAI, PT											
//***********************************************************************************************
window.app_kopeg_fLogin = function(owner){
	if (owner){
		try{		
			window.app_kopeg_fLogin.prototype.parent.constructor.call(this, owner);
			this.className  = "app_kopeg_fLogin";
			
			this.setBound(0,60,400,200);
			var step = "init child";
			this.app._isLogedIn = false;
			owner.childFormConfig(this, "mainButtonClick","Login eKoperasi", 1);			
			//------------------------ login data ------------------------			
			...
			systemAPI.alert("[app_kopeg_fLogin]::oncreate lib : ",e);
		}
	}
};
window.app_kopeg_fLogin.extend(window.portalui_childForm);
window.app_kopeg_fLogin.implement({
