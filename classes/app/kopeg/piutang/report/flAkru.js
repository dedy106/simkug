window.app_kopeg_piutang_report_flAkru = function(owner)
{
	try{	
		if (owner)
		{
			window.app_kopeg_piutang_report_flAkru.prototype.parent.constructor.call(this,owner);
			this.className = "app_kopeg_piutang_report_flAkru";
			this.maximize();
			this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Akru Piutang Umum", 2);
		...
		this.getPeriode="select distinct periode as periode from kop_ar_m where kode_lokasi ='"+this.lokasi+"' order by periode desc ";
		this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Kode Lokasi",this.tanda,this.lokasi]);
		this.gridLib.SGEditData(this.sg1,1,[0,1,2],["Periode","=",this.dbLib.getPeriodeFromSQL(this.getPeriode)]);
		this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Kode Bukti Akru","All",""]);
		this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Kode Akun","All",""]);	
		this.gridLib.SGEditData(this.sg1,4,[0,1,2],["Unit Kerja","All",""]);	
		this.gridLib.SGEditData(this.sg1,5,[0,1,2],["Customer","All",""]);	
		this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	}catch(e){
		systemAPI.alert(e);
	}
};
window.app_kopeg_piutang_report_flAkru.extend(window.portalui_childForm);
window.app_kopeg_piutang_report_flAkru.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row === 2)
		{
			this.filterRep.ListDataSGFilter(this, "Data Piutang",this.sg1, this.sg1.row, this.sg1.col,
										  "select no_ar,keterangan from kop_ar_m where kode_lokasi ='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"'",
										  "select count(*) from kop_ar_m where kode_lokasi ='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"'",
		...
		if (row === 3)
		{
			this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,
										  "select kode_akun, nama from masakun where kode_lokasi ='"+this.sg1.getCell(2,0)+"' ",
										  "select count(*) from masakun where kode_lokasi ='"+this.sg1.getCell(2,0)+"' ",
										  ["kode_akun","nama"],"and",["Kode Akun","Nama"]);
		}
		if (row === 4)
		{
			this.filterRep.ListDataSGFilter(this, "Data Unit Kerja",this.sg1, this.sg1.row, this.sg1.col,
										  "select kode_pp, nama from pp where kode_lokasi ='"+this.sg1.getCell(2,0)+"' ",
										  "select count(*) from pp where kode_lokasi ='"+this.sg1.getCell(2,0)+"' ",
	...
	doSelectCell: function(sender, col, row)
	{
		...
		if (this.userStatus==="A"){
			this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5),new Array("3","3","13","13","13","123"));
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5),new  Array(2,0,2,2,2,2,2));
		}else{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5),new Array("3","3","13","13","13","123"));
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5),new  Array(3,0,2,2,2));
	...
	mainButtonClick: function(sender){
		try
		{
			...
			{ 
				...
				var sql = "select a.kode_cust, d.nama as nmcust, b.kode_akun,c.nama as nmakun,a.no_ar,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.no_dokumen,a.keterangan,a.nilai_pph,a.nilai_ppn,a.nilai "+
						"from kop_ar_m a left outer join cust d on d.kode_cust = a.kode_cust and d.kode_lokasi = a.kode_lokasi "+
						" left outer join kop_arref_j b on a.kode_ref=b.kode_ref and a.kode_lokasi=b.kode_lokasi "+
						this.filterRep.filterStr("b.kode_akun",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
						"left outer join masakun c on b.kode_akun=c.kode_akun and b.kode_lokasi=c.kode_lokasi "+
						this.filter+
						" order by b.kode_akun, a.no_ar ";
				this.scriptSqlCount = "select count(*) "+
						...
						"left outer join masakun c on b.kode_akun=c.kode_akun and b.kode_lokasi=c.kode_lokasi "+
						this.filter;				
				this.title = new server_util_arrayList({items:["Kode Cust","Nama Customer","Kode Akun","Nama Akun","No. Bukti Akru","Tanggal","No. Dokumen","Keterangan","PPh","PPN","Nilai Tagihan"]});
				this.widthTable = new server_util_arrayList({items:[]});
				this.fieldType = new server_util_arrayList({items:["S","S","S","S","S","S","S","S","N","N","N"]});
				this.groupBy = new server_util_arrayList({items:["kode_akun","nmakun"]});
				this.groupHeader = new server_util_arrayList({items:["kode_akun"]});
			...
			systemAPI.alert("[app_kopeg_piutang_report_flAkru]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var header = "DAFTAR PENGAKUAN PIUTANG UMUM";
