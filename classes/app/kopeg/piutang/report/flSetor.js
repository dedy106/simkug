window.app_kopeg_piutang_report_flSetor = function(owner)
{
	if (owner)
	{
		window.app_kopeg_piutang_report_flSetor.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_piutang_report_flSetor";
		this.maximize();
	...
	this.getPeriode="select distinct periode from kop_arsetor_m where kode_lokasi ='"+this.lokasi+"' order by periode desc ";
	this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Kode Lokasi",this.tanda,this.lokasi]);
	this.gridLib.SGEditData(this.sg1,1,[0,1,2],["Periode","=",this.dbLib.getPeriodeFromSQL(this.getPeriode)]);
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["No. Pembayaran","All",""]);
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["No. Setoran","All",""]);
	this.pager = 1;
};
window.app_kopeg_piutang_report_flSetor.extend(window.portalui_childForm);
window.app_kopeg_piutang_report_flSetor.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row === 2)
		{
			this.filterRep.ListDataSGFilter(this, "Data Pembayaran",this.sg1, this.sg1.row, this.sg1.col,
										  "select no_bukti,keterangan from kop_arbayar_m where kode_lokasi='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"' ",
										  "select count(*) from kop_arbayar_m where kode_lokasi='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"' ",
		...
		if (row === 3)
		{
			this.filterRep.ListDataSGFilter(this, "Data Setoran",this.sg1, this.sg1.row, this.sg1.col,
										  "select no_setor,keterangan from kop_arsetor_m where kode_lokasi ='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"'",
										  "select count(*)            from kop_arsetor_m where kode_lokasi ='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"'",
										  ["no_setor","keterangan"],"and",["No. Setoran","Keterangan"]);
		}
	},
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_arsetor_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' order by periode desc ",[this.sg1.columns.get(2).pickList]);
		}
	},
	mainButtonClick: function(sender)
	{
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_kopeg_rptSetor3";
