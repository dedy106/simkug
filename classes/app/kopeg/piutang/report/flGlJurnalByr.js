// JavaScript Document
window.app_kopeg_piutang_report_flGlJurnalByr = function(owner)
{
	if (owner)
	{
		window.app_kopeg_piutang_report_flGlJurnalByr.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_piutang_report_flGlJurnalByr";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kop_arbayar_m where POSTED='T' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_kopeg_piutang_report_flGlJurnalByr.extend(window.portalui_childForm);
window.app_kopeg_piutang_report_flGlJurnalByr.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			...
			var sql1="select a.no_bukti,a.keterangan from kop_arbayar_m a "+filter; 
			var sql2="select count(a.no_bukti) from kop_arbayar_m a "+filter;
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_arbayar_m where posted='T' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
