window.app_kopeg_piutang_report_flBayar = function(owner)
{
	if (owner)
	{
		window.app_kopeg_piutang_report_flBayar.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_piutang_report_flBayar";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Penerimaan Pembayaran Piutang", 2);
	...
	this.getPeriode="select distinct periode from kop_arbayar_m where kode_lokasi ='"+this.lokasi+"' order by periode desc ";
	...
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Jenis Laporan","=","Kwitansi"]);
	this.pager = 1;
};
window.app_kopeg_piutang_report_flBayar.extend(window.portalui_childForm);
window.app_kopeg_piutang_report_flBayar.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row === 2)
		{
			this.standar.ListDataSGFilter(this, "Data Pembayaran Piutang",this.sg1, this.sg1.row, this.sg1.col,
									  "select no_bukti,keterangan from kop_arbayar_m "+
									  ...
									  "select count(*) from kop_arbayar_m "+
									  this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where")+
									  " and kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",
	...
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_arbayar_m where kode_lokasi ='"+this.lokasi+"' order by periode desc ",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 3)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,3) === "Kwitansi"){
					this.nama_report="server_report_kopeg_rptByrAR";
					...
					this.nama_report="server_report_kopeg_rptByrAR2";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.no_bukti",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
