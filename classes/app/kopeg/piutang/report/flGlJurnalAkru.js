// JavaScript Document
window.app_kopeg_piutang_report_flGlJurnalAkru = function(owner)
{
	if (owner)
	{
		window.app_kopeg_piutang_report_flGlJurnalAkru.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_piutang_report_flGlJurnalAkru";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kop_ar_m where POSTED='T' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_kopeg_piutang_report_flGlJurnalAkru.extend(window.portalui_childForm);
window.app_kopeg_piutang_report_flGlJurnalAkru.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			...
			var sql1="select a.no_ar,a.keterangan from kop_ar_m a "+filter; 
			var sql2="select count(a.no_Ar) from kop_ar_m a "+filter;
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_ar_m where posted='T' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			this.nama_report="server_report_saku3_rptGlJurnalArum";
