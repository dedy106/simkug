window.app_kopeg_piutang_fSetor = function(owner)
{
	if (owner)
	{
		window.app_kopeg_piutang_fSetor.prototype.parent.constructor.call(this,owner);
		this.className  = "app_kopeg_piutang_fSetor";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Setoran Pembayaran Piutang: Input", 0);	
		...
		this.bApp = new portalui_button(this,{bound:[440,17,80,18],caption:"Setor All",click:[this,"doTampilClick"]});		
		this.bNon = new portalui_button(this,{bound:[530,17,80,18],caption:"Belum All",click:[this,"doTampilClick"]});		
...
window.app_kopeg_piutang_fSetor.extend(window.portalui_childForm);
window.app_kopeg_piutang_fSetor.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into kop_arsetor_j (no_setor,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+	
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+idx+",'"+this.akunTAK+"','"+this.e_desc.getText()+"','D',"+parseNilai(this.e_tot.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','ARUM','TAK','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())");
					*/
					this.createJurnal();
					...
					for (var i in this.dataJurnal.rs.rows){
						...
						if (line.nilai != 0) {
							if (z >0) d+=",";
							idx++;
							d+="('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+idx+",'"+line.kode_akun+"','"+this.e_desc.getText()+"','"+line.dc+"',"+line.nilai+",'"+line.kode_pp+"','"+line.kode_drk+"','"+this.app._lokasi+"','ARUM','ARUM_AR','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
							z++;
						}
					}
					sql.add(d);
		...
		try{			
			if (sender == this.bTampil) {
				if (this.e_periode.getText() != ""){
					...
					alert("select distinct a.no_bukti,date_format(a.tanggal,'%d/%m/%Y') as tanggal,b.nama,a.keterangan,a.nilai,a.akun_ar,a.periode, "+
					...
					var data = this.dbLib.getDataProvider("select distinct a.no_bukti,date_format(a.tanggal,'%d/%m/%Y') as tanggal,b.nama,a.keterangan,a.nilai,a.akun_ar,a.periode, "+
														  ...
														  "                      inner join kop_arbayar_d e on a.no_bukti=e.no_bukti and a.kode_lokasi=e.kode_lokasi "+
														  "                      inner join kop_ar_m f on e.no_ar=f.no_ar and e.kode_lokasi=f.kode_lokasi "+
														  "where f.kode_pp='"+this.app._kodePP+"' and a.nilai<> 0 and a.progress='0' and a.no_del='-' and a.periode <= '"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' "+vFilter);
					eval("data = "+data+";");
					if (typeof data == "object"){
						var line;
						this.sg.clear();
