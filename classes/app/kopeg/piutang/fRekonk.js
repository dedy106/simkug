window.app_kopeg_piutang_fRekonk = function(owner)
{
	if (owner)
	{
		window.app_kopeg_piutang_fRekonk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_kopeg_piutang_fRekonk";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Rekonsiliasi Hutang-Piutang : Koreksi", 0);	
...
window.app_kopeg_piutang_fRekonk.extend(window.portalui_childForm);
window.app_kopeg_piutang_fRekonk.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					if (parseFloat(this.perLama) < parseFloat(this.app._periode)) {
						...
						sql.add(" update kop_rekon_m set no_link='"+this.e_kb.getText()+"',no_del = concat(no_rekon,'r') where no_rekon ='"+this.cb_nbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");						
						sql.add(" insert into kop_rekon_m (no_rekon,no_dokumen,keterangan,tanggal,nilai,periode,kode_lokasi,kode_lokasi2,akun_tak1,akun_tak2,posted,kurs,kode_curr,kode_ppar,kode_ppap,no_del,no_link,nik_ar,nik_ap,nik_user,tgl_input) "+
								...
								" from kop_rekon_m where no_rekon = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"'");
						sql.add(" insert into kop_rekon_j (no_rekon,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input)"+
								...
								" from kop_rekon_j where no_bukti = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");						
						sql.add(" insert into kop_rekon_d (no_rekon,no_ar,periode,nilai,kode_lokasi,kode_lokasi2,dc,no_spb)"+
								...
								" from kop_rekon_d where no_rekon = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");					
						sql.add("update spb_m a,kop_rekon_d b set a.progress='1' where a.no_spb=b.no_spb and a.kode_lokasi=b.kode_lokasi2 and b.no_rekon = '"+this.cb_nbLama.getText()+"' and b.kode_lokasi= '"+this.app._lokasi+"'");
						this.nb = this.e_kb.getText();
					}
					else{
						sql.add("delete from kop_rekon_m where no_rekon='"+this.cb_nbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from kop_rekon_j where no_rekon='"+this.cb_nbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("update spb_m a,kop_rekon_d b set a.progress='1' where a.no_spb=b.no_spb and a.kode_lokasi=b.kode_lokasi2 and b.no_rekon='"+this.cb_nbLama.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from kop_rekon_d where no_rekon='"+this.cb_nbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");								
					...
					sql.add("insert into kop_rekon_m(no_rekon,no_dokumen,keterangan,tanggal,nilai,periode,kode_lokasi,kode_lokasi2,akun_tak1,akun_tak2,posted,kurs,kode_curr,kode_ppar,kode_ppap,no_del,no_link,nik_ar,nik_ap,nik_user,tgl_input) values  "+
							"('"+this.nb+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.dp_d1.getDateString()+"',"+parseNilai(this.e_nilai.getText())+",'"+this.e_periode.getText()+"','"+this.app._lokasi+"','"+this.app._lokasi+"','-','-','F',1,'IDR','"+this.cb_pp1.getText()+"','"+this.cb_pp2.getText()+"','-','-','"+this.cb_nik1.getText()+"','"+this.cb_nik2.getText()+"','"+this.app._userLog+"',now())");					
					var nospb = [];
						...
						if (this.sg2.rowValid(i)){
							if (this.sg2.cells(7,i) != "-"){
								sql.add("insert into kop_rekon_d (no_rekon,no_ar,periode,nilai,kode_lokasi,kode_lokasi2,dc,no_spb) values "+
								        "('"+this.nb+"','"+this.sg2.cells(7,i)+"','"+this.e_periode.getText()+"',"+parseNilai(this.sg2.cells(6,i))+",'"+this.app._lokasi+"','"+this.app._lokasi+"','D','"+this.sg2.cells(0,i)+"')");
		...
		switch (event)
		{
			...
			case "hapus" : 
				...
				try{
					...
					var sql = new server_util_arrayList();
					if (parseFloat(this.perLama) < parseFloat(this.app._periode)) {
						sql.add(" update kop_rekon_m set no_del = concat(no_rekon,'r') where no_rekon ='"+this.cb_nbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");						
						sql.add(" insert into kop_rekon_m (no_rekon,no_dokumen,keterangan,tanggal,nilai,periode,kode_lokasi,kode_lokasi2,akun_tak1,akun_tak2,posted,kurs,kode_curr,kode_ppar,kode_ppap,no_del,no_link,nik_ar,nik_ap,nik_user,tgl_input) "+
							    " select concat(no_rekon,'r'),no_dokumen,keterangan,'"+this.dp_d1.getDateString()+"',nilai,'"+this.e_periode.getText()+"',kode_lokasi,kode_lokasi2,akun_tak1,akun_tak2,'F',kurs,kode_curr,kode_ppar,kode_ppap,no_rekon,'-',nik_ar,nik_ap,'"+this.app._userLog+"',now() "+
								" from kop_rekon_m where no_rekon = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"'");
						sql.add(" insert into kop_rekon_j (no_rekon,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input)"+
		...
		try{
			if (sender == this.cb_nbLama) {   
			    this.standarLib.showListData(this, "Daftar Bukti Rekon",sender,undefined, 
											  "select no_rekon,no_dokumen from kop_rekon_m where kode_lokasi=kode_lokasi2 and kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and no_del='-'", 
											  "select count(no_rekon)     from kop_rekon_m where kode_lokasi=kode_lokasi2 and kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and no_del='-'", 
											  ["no_rekon","no_dokumen"],"and",["No Bukti","No Dokumen"],false);				
	...
	doLoadData:function(sender){
		if (this.cb_nbLama.getText() != "") {
			var data = this.dbLib.getDataProvider("select  a.no_dokumen,a.tanggal,a.periode,a.posted,a.keterangan,a.kode_ppar,a.kode_ppap,a.nik_ar,a.nik_ap,b.nama as nama_ppar,c.nama as nama_ppap,d.nama as nama_ar,e.nama as nama_ap "+						
												  " 	  from kop_rekon_m a inner join pp b on a.kode_ppar=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
												  " 	                     inner join pp c on a.kode_ppap=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
												  " 	                     inner join karyawan d on a.nik_ar=d.nik and a.kode_lokasi=d.kode_lokasi "+
												  " 	                     inner join karyawan e on a.nik_ap=e.nik and a.kode_lokasi=e.kode_lokasi "+
												  " where  a.kode_lokasi = '"+this.app._lokasi+"' and a.no_rekon='"+this.cb_nbLama.getText()+"'");
			eval("data = "+data+";");
			...
			var data = this.dbLib.getDataProvider("select f.no_ar,date_format(f.tanggal,'%d/%m/%Y')as tanggal,concat(a.kode_cust,'-',a.nama) as cust, f.keterangan,f.akun_ar,(f.nilai+f.nilai_ppn)-ifnull(y.totbayar,0)-ifnull(z.totrekon,0) as sisa,j.nama as nama_akun,bb.no_spb from "+						
												  ...
												  " where bb.kode_lokasi = '"+this.app._lokasi+"' and bb.no_rekon='"+this.cb_nbLama.getText()+"' order by f.no_ar");
			eval("data = "+data+";");
			if (typeof data == "object"){
				var line;
				this.sg.clear();
			...
			var data = this.dbLib.getDataProvider("select f.no_spb,date_format(f.tanggal,'%d/%m/%Y')as tanggal,concat(a.kode_vendor,'-',a.nama) as vendor, f.keterangan,f.akun_hutang,(f.nilai)-ifnull(y.totrekon,0) as sisa,j.nama as nama_akun,bb.no_ar,bb.nilai from "+
												  ...
												  "	          inner join kop_rekon_d bb on f.no_spb = bb.no_spb and f.kode_lokasi = bb.kode_lokasi "+					
												  "	          inner join masakun j on f.akun_hutang = j.kode_akun and f.kode_lokasi = j.kode_lokasi "+					
												  "           left outer join (select no_spb,kode_lokasi, sum(case dc when 'D' then nilai else -nilai end) as totrekon from kop_rekon_d where no_rekon <> '"+this.cb_nbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"' group by no_spb,kode_lokasi) y on y.no_spb=f.no_spb and y.kode_lokasi=f.kode_lokasi "+
												  " where bb.kode_lokasi = '"+this.app._lokasi+"' and bb.no_rekon='"+this.cb_nbLama.getText()+"' order by f.no_spb");
			eval("data = "+data+";");
			if (typeof data == "object"){
				var line;
				this.sg2.clear();
