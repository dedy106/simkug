window.app_kopeg_piutang_fBayarGab = function(owner)
{
	if (owner)
	{
		window.app_kopeg_piutang_fBayarGab.prototype.parent.constructor.call(this,owner);
		this.className  = "app_kopeg_piutang_fBayarGab";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penerimaan Pembayaran Piutang Umum & Proyek: Input", 0);	
		
		uses("saiCBB;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;app_saku_fJurnalViewer");
		this.e_periode = new portalui_saiLabelEdit(this,{bound:[20,10,200,20],caption:"Periode", readOnly:true, tag: 2});
		this.l_tgl = new portalui_label(this,{bound:[20,11,100,18],caption:"Tanggal", underline:true});
		this.dp_d1 = new portalui_datePicker(this,{bound:[120,11,100,18],selectDate:[this,"doSelectDate"],date:new Date().getDateStr()});
		this.cb_modul = new portalui_saiCB(this,{bound:[20,26,200,20],caption:"Modul",items:["ARUM","ARPRO"],tag:9});
		...
		this.sg = new portalui_saiGrid(this.p1,{bound:[0,20,895,240],colCount:8,
		...
		this.sgn = new portalui_sgNavigator(this.p1,{bound:[0,260, 895, 25], buttonStyle:3, grid: this.sg, pager:[this,"doPager"]});
		this.eCari = new portalui_saiLabelEdit(this.sgn, {bound:[500,3,200,20], caption:"Cari No Invoice"});
		this.bCari = new portalui_button(this.sgn, {bound:[720, 3, 80, 20], caption:"Cari", click:"doCari"});
		...
		{		    
			...
			var data = this.dbLib.getDataProvider("select a.kode_spro,a.flag,b.nama from spro a inner join masakun b on a.flag=b.kode_akun "+
			                                      "where a.kode_spro in ('ARUMAD','PROADM') and a.kode_lokasi = '"+this.app._lokasi+"' "); 
			eval("data = "+data+";");
			if (typeof data == "object"){
				...
				for (var i in data.rs.rows){
					...
					if (line.kode_spro == "PROADM") {this.akunADPro = line.flag; this.namaADPro = line.nama;}
				}
			}
...
window.app_kopeg_piutang_fBayarGab.extend(window.portalui_childForm);
window.app_kopeg_piutang_fBayarGab.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					this.modul = this.cb_modul.getText();
					if (this.modul == "ARUM") {
						...
						sql.add("insert into kop_arbayar_m(no_bukti,no_dokumen,keterangan,tanggal,nilai,jenis,progress,akun_ar,periode,kode_lokasi,posted,kurs,kode_curr,kode_pp,no_del,no_link,nik_app,nik_user,tgl_input,nilai_pph,akun_pph,nilai_adm) values  "+
						...
						for (var i in this.dataAR.rs.rows){
							line = this.dataAR.rs.rows[i];
							//if (this.sg.rowValid(i))
							{
								if (parseFloat(line.pelunasan) != 0){
									sql.add("insert into kop_arbayar_d (no_bukti,no_ar,periode,akun_ar,nilai,nilai_pph,kode_lokasi,dc) values "+
											"('"+this.e_kb.getText()+"','"+line.no_ar+"','"+this.e_periode.getText()+"','"+line.akun_ar+"',"+(line.pelunasan)+","+(line.nilai_pph)+",'"+this.app._lokasi+"','D')");
								}
							}
						}
						this.createJurnal();					
		...
		switch (event)
		{
			...
			case "simpan" :	
				...
				for (var i in this.dataAR.rs.rows){
					...
					//if (this.sg.rowValid(i))
					{
						if ((this.dp_d1.toSysDate() < new Date().strToDate(line.tanggal)) && (parseFloat(line.pelunasan) != 0)) {
							system.alert(this,"Tanggal tidak valid.","Tanggal kurang dari tgl invoice. Baris["+i+"]");
							return false;   
						}
						if (parseFloat(line.pelunasan) > parseFloat(line.sisa)){
	...
	doLoadData: function(sender){
		try{			
			if (this.cb_cust.getText() != "" && this.cb_pp.getText() != "" ) {
				this.sg.clear(1);
				if (this.cb_modul.getText() == "ARUM") {
					var data = this.dbLib.getDataProvider("select f.no_ar,date_format(f.tanggal,'%d/%m/%Y')as tanggal,f.keterangan,f.akun_ar,(f.nilai+f.nilai_ppn)-ifnull(y.totbayar,0)-ifnull(z.totrekon,0) as sisa,j.nama as nama_akun,f.nilai_pph, 0 as pelunasan from "+						
														  " 	  kop_ar_m f "+
														  ...
														  " where f.no_del = '-' and (f.nilai+f.nilai_ppn)-ifnull(y.totbayar,0)-ifnull(z.totrekon,0) > 0 and f.kode_lokasi = '"+this.app._lokasi+"' and f.kode_cust ='"+this.cb_cust.getText()+"' and f.kode_pp='"+this.cb_pp.getText()+"' "+
														  "order by f.no_ar");
				}
				else {
					var data = this.dbLib.getDataProvider("select f.no_ar,date_format(f.tanggal,'%d/%m/%Y')as tanggal,f.keterangan,f.akun_ar,(f.nilai+f.nilai_ppn)-ifnull(y.totbayar,0) as sisa,j.nama as nama_akun,f.nilai_pph, 0 as pelunasan from "+						
														  " 	  kop_arproyek_m f "+
														  "	          inner join masakun j on f.akun_ar = j.kode_akun and f.kode_lokasi = j.kode_lokasi "+					
														  ...
														  " where (f.nilai+f.nilai_ppn)-ifnull(y.totbayar,0) >0 and f.kode_lokasi = '"+this.app._lokasi+"' and f.kode_cust ='"+this.cb_cust.getText()+"' and f.kode_pp='"+this.cb_pp.getText()+"' "+
														  "order by f.no_ar");
				}
				eval("data = "+data+";");
				if (typeof data == "object"){
	...
	doSgChange: function(sender, col, row){				
		switch (col){
				...
				break;
				case 6:
					this.dataAR.rs.rows[(this.page - 1) * 20 + row].pelunasan = nilaiToFloat(sender.cells(col, row));
				break;
				case 7:
					this.dataAR.rs.rows[(this.page - 1) * 20 + row].nilai_pph = nilaiToFloat(sender.cells(col, row));
				break;
		}
		if ((col == 5)||(col == 6)||(col == 7)){			
			...
			for (var i in this.dataAR.rs.rows){
				line = this.dataAR.rs.rows[i];
				tot += parseFloat(line.sisa);
				tot2 += parseFloat(line.pelunasan);
				tot3 += parseFloat(line.nilai_pph);
			}
	...
	createJurnal: function(){				
		try{
			...
			if (this.e_adm.getText() != "0") {
				if (this.cb_modul.getText() == "ARUM") 
					rows[rows.length] = {kode_akun:this.akunAD, nama: this.namaAD,dc:"D", keterangan:vKet, nilai: nilaiToFloat(this.e_adm.getText()),kode_pp:this.app._kodePP, kode_drk:'ARADM'};
				else rows[rows.length] = {kode_akun:this.akunADPro, nama: this.namaADPro,dc:"D", keterangan:vKet, nilai: nilaiToFloat(this.e_adm.getText()),kode_pp:this.app._kodePP, kode_drk:'ARADM'};
			}			
			...
			for (var i in this.dataAR.rs.rows){
				...
				if (line.pelunasan != 0){
					var temu = false;
					for (var j in rows){
						if (rows[j].kode_akun == line.akun_ar && rows[j].dc == "C"){
							rows[j].nilai += parseFloat(line.pelunasan);
							temu = true;
						}
					}
					if (!temu){
						rows[rows.length] = {kode_akun:line.akun_ar,nama:line.nama_akun,dc:"C", keterangan:vKet, nilai: parseFloat(line.pelunasan),kode_pp:this.app._kodePP, kode_drk:'ARAKRU'};
					}
				}
			} 
	...
	doClick:function(sender){
		if (sender == this.b_gen) {
			if (this.cb_modul.getText() == "ARUM") 
				this.e_kb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"kop_arbayar_m","no_bukti",this.app._lokasi+"-AAR"+this.e_periode.getText().substr(2,4)+".","00000"));		
			else this.e_kb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"kop_proyekbayar_m","no_bukti",this.app._lokasi+"-APR"+this.e_periode.getText().substr(2,4)+".","00000"));		
			this.e_dok.setFocus();
		}
		...
		if (sender == this.i_sort){
			var vFilter = " order by no_ar";
			if (this.cb_sort.getText() == "N. KECIL") vFilter = " order by saldo ";
			if (this.cb_sort.getText() == "N. BESAR") vFilter = " order by saldo desc ";
			if (this.cb_modul.getText() == "ARUM") {
				var data = this.dbLib.getDataProvider("select f.no_ar,date_format(f.tanggal,'%d/%m/%Y')as tanggal,f.keterangan,f.akun_ar,(f.nilai+f.nilai_ppn)-ifnull(y.totbayar,0)-ifnull(z.totrekon,0) as sisa,j.nama as nama_akun,f.nilai_pph,(f.nilai+f.nilai_ppn)-ifnull(y.totbayar,0)-ifnull(z.totrekon,0) as saldo, 0 as pelunasan from "+
													  " 	  kop_ar_m f "+
													  "	          inner join masakun j on f.akun_ar = j.kode_akun and f.kode_lokasi = j.kode_lokasi "+					
													  "           left outer join (select no_ar,kode_lokasi, sum(case dc when 'D' then nilai else -nilai end) as totbayar from kop_arbayar_d where kode_lokasi='"+this.app._lokasi+"' group by no_ar,kode_lokasi) y on y.no_ar=f.no_ar and y.kode_lokasi=f.kode_lokasi "+
													  "           left outer join (select no_ar,kode_lokasi, sum(case dc when 'D' then nilai else -nilai end) as totrekon from kop_rekon_d where kode_lokasi='"+this.app._lokasi+"' group by no_ar,kode_lokasi) z on z.no_ar=f.no_ar and z.kode_lokasi=f.kode_lokasi "+
													  " where (f.nilai+f.nilai_ppn)-ifnull(y.totbayar,0)-ifnull(z.totrekon,0) > 0 and f.kode_lokasi = '"+this.app._lokasi+"' and f.kode_cust ='"+this.cb_cust.getText()+"' and f.kode_pp='"+this.cb_pp.getText()+"' "+
													  vFilter, true);
			}
			else {
				var data = this.dbLib.getDataProvider("select f.no_ar,date_format(f.tanggal,'%d/%m/%Y')as tanggal,f.keterangan,f.akun_ar,(f.nilai+f.nilai_ppn)-ifnull(y.totbayar,0) as sisa,j.nama as nama_akun,f.nilai_pph,(f.nilai+f.nilai_ppn)-ifnull(y.totbayar,0) as saldo , 0 as pelunasan from "+
													  " 	  kop_arproyek_m f "+
													  "	          inner join masakun j on f.akun_ar = j.kode_akun and f.kode_lokasi = j.kode_lokasi "+					
													  "           left outer join (select no_ar,kode_lokasi, sum(case dc when 'D' then nilai else -nilai end) as totbayar from kop_proyekbayar_d where kode_lokasi='"+this.app._lokasi+"' group by no_ar,kode_lokasi) y on y.no_ar=f.no_ar and y.kode_lokasi=f.kode_lokasi "+
													  " where (f.nilai+f.nilai_ppn)-ifnull(y.totbayar,0) >0 and f.kode_lokasi = '"+this.app._lokasi+"' and f.kode_cust ='"+this.cb_cust.getText()+"' and f.kode_pp='"+this.cb_pp.getText()+"' "+
			...
			if (typeof data == "object"){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					if (line !== undefined)
						this.sg.appendData([line.no_ar, line.tanggal, line.keterangan, line.akun_ar, line.nama_akun, floatToNilai(line.sisa),"0",floatToNilai(line.nilai_pph),line.modul]);
				}
				this.doSgChange(this.sg, 5, 0);
				* */
			}
	...
	doSelectPage: function(page){
		this.page = page;
		var start = (page - 1) * 20;
		var finish = (start + 20 > this.dataAR.rs.rows.length ? this.dataAR.rs.rows.length : start + 20);
		this.sg.clear();
		var line;
		for (var i=start;i < finish; i++){
			line = this.dataAR.rs.rows[i];
			//this.sg.appendData([line.no_ar, line.tanggal, line.keterangan, line.akun_ar, line.nama_akun, floatToNilai(line.sisa),"0",floatToNilai(line.nilai_pph),line.modul]);
			this.sg.appendData([line.no_ar, line.tanggal, line.keterangan, line.akun_ar, line.nama_akun, floatToNilai(line.sisa),floatToNilai(line.pelunasan),floatToNilai(line.nilai_pph)]);
		}
	...
	doCari: function(sender){
		...
		for (var i in this.dataAR.rs.rows){
			line = this.dataAR.rs.rows[i];
			if (line.no_ar == this.eCari.getText()){
				this.doSelectPage( Math.ceil( i / 20));
				this.sgn.activePage = Math.ceil( i / 20);
				for (var r=0;r < this.sg.getRowCount();r++){
					if (this.sg.cells(0,r) == this.eCari.getText()){
						this.sg.goToRow(r);
					}
				}
				return;
