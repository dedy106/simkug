window.app_kopeg_piutang_fRekon2 = function(owner)
{
	if (owner)
	{
		window.app_kopeg_piutang_fRekon2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_kopeg_piutang_fRekon2";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Rekonsiliasi Hutang-Piutang via TAK: Input", 0);	
		...
		this.cb_lok1 = new portalui_saiCBBL(this,{bound:[20,31,200,20],caption:"Lokasi Piutang",btnClick:[this,"doBtnClick"],change:[this,"doChange"]});
		this.cb_lok2 = new portalui_saiCBBL(this,{bound:[20,32,200,20],caption:"Lokasi Hutang",btnClick:[this,"doBtnClick"],change:[this,"doChange"]});
		...
		{		    
			...
			this.cb_lok1.setSQL("select a.kode_lokasi,a.nama   from lokasi a inner join lokasi b on a.kode_lokkonsol=b.kode_lokkonsol where b.kode_lokasi='"+this.app._lokasi+"'",["a.kode_lokasi","a.nama"],true);
			this.cb_lok2.setSQL("select a.kode_lokasi,a.nama   from lokasi a inner join lokasi b on a.kode_lokkonsol=b.kode_lokkonsol where b.kode_lokasi='"+this.app._lokasi+"'",["a.kode_lokasi","a.nama"],true);
		}catch(e){
			systemAPI.alert(e);
		}
	}
};
window.app_kopeg_piutang_fRekon2.extend(window.portalui_childForm);
window.app_kopeg_piutang_fRekon2.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into kop_rekon_m(no_rekon,no_dokumen,keterangan,tanggal,nilai,periode,kode_lokasi,kode_lokasi2,akun_tak1,akun_tak2,posted,kurs,kode_curr,kode_ppar,kode_ppap,no_del,no_link,nik_ar,nik_ap,nik_user,tgl_input) values  "+
							"('"+this.e_kb.getText()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.dp_d1.getDateString()+"',"+parseNilai(this.e_nilai.getText())+",'"+this.e_periode.getText()+"','"+this.cb_lok1.getText()+"','"+this.cb_lok2.getText()+"','"+this.cb_tak1.getText()+"','"+this.cb_tak2.getText()+"','F',1,'IDR','"+this.cb_pp1.getText()+"','"+this.cb_pp2.getText()+"','-','-','"+this.cb_nik1.getText()+"','"+this.cb_nik2.getText()+"','"+this.app._userLog+"',now())");
					//hutang
					sql.add("insert into kop_rekon_m(no_rekon,no_dokumen,keterangan,tanggal,nilai,periode,kode_lokasi,kode_lokasi2,akun_tak1,akun_tak2,posted,kurs,kode_curr,kode_ppar,kode_ppap,no_del,no_link,nik_ar,nik_ap,nik_user,tgl_input) values  "+
							"('"+this.e_dok.getText()+"','"+this.e_kb.getText()+"','"+this.e_ket.getText()+"','"+this.dp_d1.getDateString()+"',"+parseNilai(this.e_nilai.getText())+",'"+this.e_periode.getText()+"','"+this.cb_lok2.getText()+"','"+this.cb_lok1.getText()+"','"+this.cb_tak1.getText()+"','"+this.cb_tak2.getText()+"','F',1,'IDR','"+this.cb_pp1.getText()+"','"+this.cb_pp2.getText()+"','-','-','"+this.cb_nik1.getText()+"','"+this.cb_nik2.getText()+"','"+this.app._userLog+"',now())");					
					var nospb = [];
						...
						if (this.sg2.rowValid(i)){
							if (this.sg2.cells(7,i) != "-"){
								sql.add("insert into kop_rekon_d (no_rekon,no_ar,periode,nilai,kode_lokasi,kode_lokasi2,dc,no_spb) values "+
								        "('"+this.e_kb.getText()+"','"+this.sg2.cells(7,i)+"','"+this.e_periode.getText()+"',"+parseNilai(this.sg2.cells(6,i))+",'"+this.cb_lok1.getText()+"','"+this.cb_lok2.getText()+"','D','"+this.sg2.cells(0,i)+"')");
					...
					for (var i in this.dataJurnal.rs.rows){
						...
						if (line.dc == "C") {
							...
							var norekon = this.e_kb.getText();
							var nodok = this.e_dok.getText();
						}
						else {
							var kdlok = this.cb_lok2.getText();
							var norekon = this.e_dok.getText();
							var nodok = this.e_kb.getText();
						}
					...
					d+=","+"('"+this.e_kb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+998+",'"+this.cb_tak1.getText()+"','"+this.e_ket.getText()+"','D',"+parseNilai(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','"+this.cb_lok1.getText()+"','ARUM','-','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
					d+=","+"('"+this.e_dok.getText()+"','"+this.e_kb.getText()+"','"+this.dp_d1.getDateString()+"',"+999+",'"+this.cb_tak2.getText()+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','"+this.cb_lok2.getText()+"','APUM','-','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
	...
	doModalResult: function(event, modalResult){
		...
		switch (event)
		{
			case "clear" :
				...
				break;			
			case "simpan" :	
				if (this.cb_lok1.getText() == this.cb_lok2.getText()) { 
					system.alert(this,"Transaksi tidak valid.","Lokasi Piutang dan Lokasi Hutang harus berbeda.");
					return false;
				}
				this.sg2.validasi();
		...
		try{
			...
			if (sender == this.cb_pp2) {   
			    this.standarLib.showListData(this, "Daftar PP/Unit Kerja Hutang",sender,undefined, 
											  "select kode_pp, nama   from pp where kode_lokasi='"+this.cb_lok2.getText()+"' and tipe='posting'",
											  "select count(kode_pp)  from pp where kode_lokasi='"+this.cb_lok2.getText()+"' and tipe='posting'",
			...
			if (sender == this.cb_pp1) {   
			    this.standarLib.showListData(this, "Daftar PP/Unit Kerja Piutang",sender,undefined, 
											  "select kode_pp, nama   from pp where kode_lokasi='"+this.cb_lok1.getText()+"' and tipe='posting'",
											  "select count(kode_pp)  from pp where kode_lokasi='"+this.cb_lok1.getText()+"' and tipe='posting'",
											  ["kode_pp","nama"],"and",["Kode PP","Nama "],false);				
				this.sg.clear(1);
			}
			if (sender == this.cb_lok1) {   
			    this.standarLib.showListData(this, "Daftar Lokasi Piutang",sender,undefined, 
				...
				this.sg.clear(1);
				this.cb_pp1.setText("","");
				this.cb_nik1.setText("","");
			}
			if (sender == this.cb_lok2) {   
			    this.standarLib.showListData(this, "Daftar Lokasi Hutang",sender,undefined, 
											  "select a.kode_lokasi,a.nama   from lokasi a inner join lokasi b on a.kode_lokkonsol=b.kode_lokkonsol where b.kode_lokasi='"+this.app._lokasi+"'",
											  "select count(a.kode_lokasi)   from lokasi a inner join lokasi b on a.kode_lokkonsol=b.kode_lokkonsol where b.kode_lokasi='"+this.app._lokasi+"'",
											  ["kode_lokasi","nama"],"and",["Kode Lokasi","Nama "],false);				
				this.sg2.clear(1);
				this.cb_pp2.setText("","");
				this.cb_nik2.setText("","");
			}
	...
	doChange:function(sender){
		if (sender == this.cb_lok1){
			this.cb_pp1.setSQL("select kode_pp, nama  from pp where tipe='posting' and kode_lokasi='"+this.cb_lok1.getText()+"'",["kode_pp","nama"],true);
		...
		if (sender == this.cb_lok2){
			this.cb_pp2.setSQL("select kode_pp, nama  from pp where tipe='posting' and kode_lokasi='"+this.cb_lok2.getText()+"'",["kode_pp","nama"],true);
		...
		var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro = 'ARIBJS' and kode_lokasi = '"+this.cb_lok1.getText()+"' "); 
		eval("data = "+data+";");
		...
		var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro = 'APIBJS' and kode_lokasi = '"+this.cb_lok2.getText()+"' "); 
		eval("data = "+data+";");
		if (typeof data == "object"){
			var line;
			for (var i in data.rs.rows){
			...
			if (sender == this.b_tampil){
				if (this.cb_pp1.getText() != "") {
					...
					var data = this.dbLib.getDataProvider("select f.no_ar,date_format(f.tanggal,'%d/%m/%Y')as tanggal,concat(a.kode_cust,'-',a.nama) as cust, f.keterangan,f.akun_ar,(f.nilai+f.nilai_ppn)-ifnull(y.totbayar,0)-ifnull(z.totrekon,0) as sisa,j.nama as nama_akun from "+
														  ...
														  "           left outer join (select no_ar,kode_lokasi, sum(case dc when 'D' then nilai else -nilai end) as totbayar from kop_arbayar_d where kode_lokasi='"+this.cb_lok1.getText()+"' group by no_ar,kode_lokasi) y on y.no_ar=f.no_ar and y.kode_lokasi=f.kode_lokasi "+
														  "           left outer join (select no_ar,kode_lokasi, sum(case dc when 'D' then nilai else -nilai end) as totrekon from kop_rekon_d where kode_lokasi='"+this.cb_lok1.getText()+"' group by no_ar,kode_lokasi) z on z.no_ar=f.no_ar and z.kode_lokasi=f.kode_lokasi "+
														  " where f.akun_ar = '"+this.akunARIJ+"' and f.periode<='"+this.e_periode.getText()+"' and (f.nilai+f.nilai_ppn)-ifnull(y.totbayar,0)-ifnull(z.totrekon,0) >0 and f.kode_lokasi = '"+this.cb_lok1.getText()+"' and f.kode_pp='"+this.cb_pp1.getText()+"' order by f.no_ar");
					eval("data = "+data+";");
					if (typeof data == "object"){
						var line;
						this.sg.clear();
				...
				if (this.cb_pp2.getText() != "") {
					...
					var data = this.dbLib.getDataProvider("select f.no_spb,date_format(f.tanggal,'%d/%m/%Y')as tanggal,concat(a.kode_vendor,'-',a.nama) as vendor, f.keterangan,f.akun_hutang,(f.nilai)-ifnull(y.totrekon,0) as sisa,j.nama as nama_akun from "+
														  ...
														  "           left outer join (select no_spb,kode_lokasi2, sum(case dc when 'D' then nilai else -nilai end) as totrekon from kop_rekon_d where kode_lokasi2='"+this.cb_lok2.getText()+"' group by no_spb,kode_lokasi2) y on y.no_spb=f.no_spb and y.kode_lokasi2=f.kode_lokasi "+
														  " where f.akun_hutang = '"+this.akunAPIJ+"' and f.progress = '1' and f.periode<='"+this.e_periode.getText()+"' and (f.nilai)-ifnull(y.totrekon,0) >0 and f.kode_lokasi = '"+this.cb_lok2.getText()+"' and f.kode_pp='"+this.cb_pp2.getText()+"' order by f.no_spb");
					eval("data = "+data+";");
					if (typeof data == "object"){
						var line;
						this.sg2.clear();
