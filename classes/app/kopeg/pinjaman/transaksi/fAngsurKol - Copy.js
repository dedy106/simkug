window.app_kopeg_pinjaman_transaksi_fAngsurKol = function(owner){
	if (owner){
		try{		
			...
			this.eSearch = new portalui_saiLabelEdit(this.sgn, {bound:[500,2,300,20], caption:"Cari Kontrak"});
			this.bSearch = new portalui_button(this.sgn, {bound:[810,2,80,20], caption:"Cari", click:[this,"doClick"]});
...
window.app_kopeg_pinjaman_transaksi_fAngsurKol.implement({
	...
	doClick: function(sender){
		...
		if (sender == this.bSearch){
			for (var i in this.dataPinjaman.rs.rows){
				var line = this.dataPinjaman.rs.rows[i];
				if (line.no_kontrak == this.eSearch.getText()){
					var pg = Match.ceil(parseFloat(i) / 30);//cari halaman
					this.selectPage(pg);
					var row = (i - ((pg - 1) * 30));
					this.sg1.setRowIndex(row, 0);
					break;
				}
			}
