window.app_kopeg_pinjaman_transaksi_fSpbtitip = function(owner){
	if (owner){
		try{		
			window.app_kopeg_pinjaman_transaksi_fSpbtitip.prototype.parent.constructor.call(this, owner);
			this.className  = "app_kopeg_pinjaman_transaksi_fSpbtitip";
			owner.childFormConfig(this, "mainButtonClick","Form SPB Titipan Kompensasi: Input", 0);
			this.maximize();
			//------------------------ login data ------------------------	
			...
			this.cbbVendor = new portalui_saiCBBL(this,{bound:[20,17,200,20],caption:"Penerima", btnClick:[this,"FindBtnClick"],readOnly:true});
			...
			this.p1 = new portalui_panel(this,{bound:[20,35,900,315],caption:"Daftar Kompensasi Titipan"});
			...
			systemAPI.alert("[app_kopeg_pinjaman_transaksi_fSpbtitip]::oncreate lib : ",e);
		}
	}
};
window.app_kopeg_pinjaman_transaksi_fSpbtitip.extend(window.portalui_childForm);
window.app_kopeg_pinjaman_transaksi_fSpbtitip.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into spb_m (no_spb,no_dokumen,tanggal,due_date,akun_hutang,keterangan,catatan,kode_curr,kurs,nik_buat,nik_setuju,kode_terima,kode_lokasi,kode_pp,"+
							...
							"('"+this.eSPB.getText()+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"','"+this.dTgl2.getDateString()+"','"+this.akunAP+"','"+this.eKet.getText()+"','-','IDR',1,'"+this.cbbPemohon.getText()+"','-','"+this.cbbVendor.getText()+"','"+this.app._lokasi+"','"+this.app._kodePP+
					...
					d+=",";
					idx++;
					d+="('"+this.eSPB.getText()+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.akunFee+"','"+this.eKet.getText()+"','C',"+parseNilai(this.ePot.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PINJ','FEE','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
					d+=",";
					idx++;
					...
					for (var i=0; i <= this.sg1.getRowCount(); i++){
						if (this.sg1.cells(0,i) == "APP") {
							sql.add("update kop_pinj_spb set no_spbtitip='"+this.eSPB.getText()+"' where no_pinj='"+this.sg1.cells(1,i)+"' and no_spb = '"+this.sg1.cells(2,i)+"' and kode_lokasi = '"+this.app._lokasi+"'");
						}
					}
	...
	try{			
			if (this.ePeriode.getText() != ""){
				var data = this.dbLib.getDataProvider("select a.no_pinj,c.no_spb,e.kode_agg,e.nama as nama_agg,c.ntitip,b.kode_akun  "+
						   ...
						   "where a.kode_lokasi='"+this.app._lokasi+"' and c.no_spbtitip='-' and c.ntitip<> 0 and a.periode<= '"+this.ePeriode.getText()+"' ");
				eval("data = "+data+";");
				if (typeof data == "object"){
					var line;
					this.sg1.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData(["INPROG",line.no_pinj,line.no_spb,line.kode_agg,line.nama_agg,floatToNilai(line.ntitip),line.kode_akun]);
					}
					this.sg1.validasi();
				}
			}
