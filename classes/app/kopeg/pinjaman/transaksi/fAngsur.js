window.app_kopeg_pinjaman_transaksi_fAngsur = function(owner){
	if (owner){
		try{		
			window.app_kopeg_pinjaman_transaksi_fAngsur.prototype.parent.constructor.call(this, owner);
			this.className  = "app_kopeg_pinjaman_transaksi_fAngsur";						
			owner.childFormConfig(this, "mainButtonClick","Form Angsuran Pinjaman: Input", 0);			
			this.maximize();
			...
			this.eBeban = new portalui_saiLabelEdit(this,{bound:[540,31,180,20],caption:"Beban Lunas",change:[this,"doChange"], tipeText:ttNilai, change:[this,"doChange"],text:"0",labelWidth:80});
			...
			systemAPI.alert("[app_kopeg_pinjaman_transaksi_fAngsur]::oncreate lib : ",e);
		}
	}
};
window.app_kopeg_pinjaman_transaksi_fAngsur.extend(window.portalui_childForm);
window.app_kopeg_pinjaman_transaksi_fAngsur.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					if (this.eDeposit.getText() != "0") {
						d+= ",";
						idx++;
						d += "('"+this.eBukti.getText()+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.akunTP+"','"+this.eKet.getText()+"','D',"+parseNilai(this.eDeposit.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PINJ','PK_DEPO','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
						sql.add("insert into kop_depo(no_depo,tanggal,keterangan,dc,nilai,kode_agg,modul,jenis,periode,kode_lokasi,kode_pp,nik_app,no_del,no_link,kurs,kode_curr,nik_user,tgl_input) values "+
							    "('"+this.eBukti.getText()+"','"+this.dTgl.getDateString()+"','"+this.eKet.getText()+"','C',"+parseNilai(this.eDeposit.getText())+",'"+this.cbbAgg.getText()+"','PINJ','ANGS_SLS','"+this.ePeriode.getText()+"','"+this.app._lokasi+"','"+this.app._kodePP+"','"+this.cbbPenerima.getText()+"','-','-',1,'IDR','"+this.app._userLog+"',now())");
					}					
					if (this.eSls.getText() != "0") { //<<------------ sls lebih akan diangap denda pelunasan,,hingga langsung masuk ke pdpt jasa angsuran
						d+= ",";
						idx++;
						d += "('"+this.eBukti.getText()+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.akunPdpt+"','"+this.eKet.getText()+"','C',"+parseNilai(this.eSls.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PINJ','SL_PDPT','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";					
					}
					if (this.eBeban.getText() != "0") { 
						d+= ",";
						idx++;
						d += "('"+this.eBukti.getText()+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.akunBeban+"','"+this.eKet.getText()+"','D',"+parseNilai(this.eBeban.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PINJ','BEBAN','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
					}
	...
	doChange: function(sender){
		...
		if (sender == this.cbbAgg) {
			var data = this.dbLib.getDataProvider("select ifnull(sum(case dc when 'D' then nilai else -nilai end),0) as saldo from kop_depo where kode_agg ='"+this.cbbAgg.getText()+"' and modul in ('SIMP','PINJ') and jenis <> 'UDP' and kode_lokasi = '"+this.app._lokasi+"'");
			eval("data = "+data+";");
			if (typeof data == "object"){
	...
	doLoadData: function(sender){
		if (this.cbbKontrak.getText() != "" && (this.ePeriode.getText() != "")){
			var data = this.dbLib.getDataProvider("select 'BILL' as status,a.no_bill as no_bukti,date_format(ab.tanggal,'%Y%m') as periode,date_format(ab.tanggal,'%d/%m/%Y') as tanggal,0 as npokok,sum(case a.dc when 'D' then a.nbunga else -a.nbunga end) as nbunga,b.nilai as baki,b.akun_piutang,b.akun_pjasa,a.dc,b.nilai_tagihan "+
												  ...
												  "where a.no_kontrak='"+this.cbbKontrak.getText()+"' and a.no_pinj='"+this.cbbKontrak.dataFromList[1]+"' and a.kode_lokasi = '"+this.app._lokasi+"' "+
												  ...
												  "where a.no_kontrak='"+this.cbbKontrak.getText()+"' and a.no_pinj='"+this.cbbKontrak.dataFromList[1]+"' and a.kode_lokasi = '"+this.app._lokasi+"' "+
												  "order by periode,status desc,no_bukti "); 
			eval("data = "+data+";");
			var periode = ""; 
			if (typeof data == "object"){
				var line;
	...
	FindBtnClick: function(sender){
		if (sender == this.cbbSpb) {   
			this.standarLib.showListData(this, "Daftar Bukti SPB/TAK",sender,undefined, 
									  ...
									  " union "+
									  " select no_ju as no_spb,keterangan,'TAK' as jenis from ju_m where kode_lokasi = '"+this.app._lokasi+"'  and periode ='"+this.cbbPerLama.getText()+"' "+ //and jenis='TAK'
									  " ) as A ", 										  
									  ...
									  "union "+
									  "select no_ju as no_spb, keterangan, 'TAK' as jenis from ju_m where kode_lokasi = '"+this.app._lokasi+"' and periode ='"+this.cbbPerLama.getText()+"' ) as b ", //and jenis='TAK' 
