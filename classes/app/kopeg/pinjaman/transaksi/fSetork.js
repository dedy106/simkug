window.app_kopeg_pinjaman_transaksi_fSetork = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjaman_transaksi_fSetork.prototype.parent.constructor.call(this,owner);
		this.className  = "app_kopeg_pinjaman_transaksi_fSetork";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Setoran Angsuran Pinjaman: Koreksi", 0);	
		...
		try{			
			...
			*/
			var prd = this.dbLib.getDataProvider("select distinct periode from kop_pinjsetor_m where kode_lokasi = '"+this.app._lokasi+"'",true);
...
window.app_kopeg_pinjaman_transaksi_fSetork.extend(window.portalui_childForm);
window.app_kopeg_pinjaman_transaksi_fSetork.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					if (parseFloat(this.perLama) < parseFloat(this.app._periode)) {
						...
						sql.add(" update kop_pinjsetor_m set no_link = '"+this.e_nb.getText()+"',no_del = concat(no_setor,'r') where no_setor ='"+this.cb_nbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
						sql.add(" insert into kop_pinjsetor_m (no_setor,no_dokumen,keterangan,tanggal,nilai,modul,periode,kode_lokasi,nik_app,kode_curr,kurs,kode_pp,progress,no_del,no_link,nik_user,tgl_input,akun_tak,posted) "+
								...
								" from kop_pinjsetor_m where no_setor = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"'");						
						sql.add(" insert into kop_pinjsetor_j (no_setor,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input)"+
								...
								" from kop_pinjsetor_j where no_setor = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");
						sql.add(" insert into kop_pinjsetor_d (no_setor,no_angs,akun_ar,jenis,nilai_sls,nilai,kode_lokasi,dc)"+
								" select concat(no_setor,'r'),no_angs,akun_ar,jenis,nilai_sls,nilai,kode_lokasi,'C' "+ 
								" from kop_pinjsetor_d where no_setor = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");
						sql.add("update kop_pinjangs_m a, kop_pinjsetor_d b set a.progress ='0' where b.no_angs = a.no_angs and a.kode_lokasi=b.kode_lokasi and b.kode_lokasi = '"+this.app._lokasi+"' and b.no_setor='"+this.cb_nbLama.getText()+"' ");
						sql.add("update kop_pinjinv_m a, kop_pinjsetor_d b set a.progress ='0' where b.no_angs = a.no_inv and a.kode_lokasi=b.kode_lokasi and b.kode_lokasi = '"+this.app._lokasi+"' and b.no_setor='"+this.cb_nbLama.getText()+"' ");
						this.nb = this.e_nb.getText();
					}
					else{
						sql.add("delete from kop_pinjsetor_m where no_setor='"+this.cb_nbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from kop_pinjsetor_j where no_setor='"+this.cb_nbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
						sql.add("update kop_pinjangs_m a, kop_pinjsetor_d b set a.progress ='0' where b.no_angs = a.no_angs and a.kode_lokasi=b.kode_lokasi and b.kode_lokasi = '"+this.app._lokasi+"' and b.no_setor='"+this.cb_nbLama.getText()+"' ");
						sql.add("update kop_pinjinv_m a, kop_pinjsetor_d b set a.progress ='0' where b.no_angs = a.no_inv and a.kode_lokasi=b.kode_lokasi and b.kode_lokasi = '"+this.app._lokasi+"' and b.no_setor='"+this.cb_nbLama.getText()+"' ");
						sql.add("delete from kop_pinjsetor_d where no_setor='"+this.cb_nbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");			
					...
					sql.add("insert into kop_pinjsetor_m(no_setor,no_dokumen,keterangan,tanggal,nilai,modul,periode,kode_lokasi,nik_app,kode_curr,kurs,kode_pp,progress,no_del,no_link,nik_user,tgl_input,akun_tak,posted) values  "+
							"('"+this.nb+"','"+this.e_dok.getText()+"','"+this.e_desc.getText()+"','"+this.dp_d1.getDateString()+"',"+parseNilai(this.e_tot.getText())+",'PINJ','"+this.e_periode.getText()+"','"+this.app._lokasi+"','"+this.cb_app.getText()+"','IDR',1,'"+this.app._kodePP+"','0','-','-','"+this.app._userLog+"',now(),'-','X')"); //akun tak  <-------- "+this.akunTAK+"
					var idx = 0;
					/*
					sql.add("insert into kop_pinjsetor_j (no_setor,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+	
							"('"+this.nb+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+idx+",'"+this.akunTAK+"','"+this.e_desc.getText()+"','D',"+parseNilai(this.e_tot.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PINJ','TAK','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())");
					*/
					this.createJurnal();
					...
					for (var i in this.dataJurnal.rs.rows){
						...
						if (line.nilai != 0) {
							if (z >0) d+=",";
							idx++;
							d+="('"+this.nb+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+idx+",'"+line.kode_akun+"','"+this.e_desc.getText()+"','"+line.dc+"',"+line.nilai+",'"+line.kode_pp+"','"+line.kode_drk+"','"+this.app._lokasi+"','PINJ','ANGS_AR','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
							z++;
						}
					}
					sql.add(d);
		...
		switch (event){
			...
			case "hapus" : 
				try{
					...
					var sql = new server_util_arrayList();
					if (parseFloat(this.perLama) < parseFloat(this.app._periode)) {
						sql.add(" update kop_pinjsetor_m set no_del = concat(no_setor,'r') where no_setor ='"+this.cb_nbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
						sql.add(" insert into kop_pinjsetor_m (no_setor,no_dokumen,keterangan,tanggal,nilai,modul,periode,kode_lokasi,nik_app,kode_curr,kurs,kode_pp,progress,no_del,no_link,nik_user,tgl_input,akun_tak,posted) "+
							    " select concat(no_setor,'r'),no_dokumen,keterangan,'"+this.dp_d1.getDateString()+"',nilai,modul,'"+this.e_periode.getText()+"',kode_lokasi,nik_app,kode_curr,kurs,kode_pp,'X',no_setor,'-','"+this.app._userLog+"',now(),akun_tak,'X' "+
								" from kop_pinjsetor_m where no_setor = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"'");						
						sql.add(" insert into kop_pinjsetor_j (no_setor,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input)"+
		...
		try{			
			if (this.cb_nbLama.getText() != ""){
				...
				var data = this.dbLib.getDataProvider("select a.no_angs,date_format(a.tanggal,'%d/%m/%Y') as tanggal,b.nama,a.keterangan,a.nilai,a.akun_ar,a.periode, "+
													  ...
													  "                       inner join kop_pinjsetor_d c on a.no_angs=c.no_angs and a.kode_lokasi=c.kode_lokasi and c.jenis='ANS' "+
													  ...
													  "                      inner join kop_pinjsetor_d c on x.no_inv=c.no_angs and x.kode_lokasi=c.kode_lokasi and c.jenis='INV' "+
													  "where c.no_setor = '"+this.cb_nbLama.getText()+"' and c.kode_lokasi='"+this.app._lokasi+"' ");
				eval("data = "+data+";");
				if (typeof data == "object"){
					var line;
		...
		try{
			if (sender == this.cb_nbLama) {   
			    this.standarLib.showListData(this, "Daftar Bukti Setoran",sender,undefined, 
											  "select no_setor, keterangan  from kop_pinjsetor_m where progress= '0' and kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and no_del='-'", 
											  "select count(no_setor)       from kop_pinjsetor_m where progress= '0' and kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and no_del='-'",
