window.app_kopeg_pinjaman_transaksi_fBilling = function(owner){
	if (owner){
		try{		
			window.app_kopeg_pinjaman_transaksi_fBilling.prototype.parent.constructor.call(this, owner);
			this.className  = "app_kopeg_pinjaman_transaksi_fBilling";						
			this.maximize();
			owner.childFormConfig(this, "mainButtonClick","Form Akrual Jasa Pinjaman : Input", 0);			
			//------------------------ login data ------------------------	
			...
			systemAPI.alert("[app_kopeg_pinjaman_transaksi_fBilling]::oncreate lib : ",e);
		}
	}
};
window.app_kopeg_pinjaman_transaksi_fBilling.extend(window.portalui_childForm);
window.app_kopeg_pinjaman_transaksi_fBilling.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into kop_pinjbill_d(no_bill,no_pinj,no_kontrak,kode_lokasi,cicilan_ke,npokok,nbunga,akun_pjasa,akun_pdpt,dc,no_del) "+
							"select '"+this.eBukti.getText()+"',a.no_pinj,a.no_kontrak,a.kode_lokasi,g.cicilan_ke, "+
						    ...
						    "                                   from kop_pinjangs_d x                             "+  //<----- no_del tidak dipakai karena pake DC 
						    ...
						    "      (a.nilai+ifnull(bb.nbill,0)-ifnull(aa.nangs,0)) > 0 and a.kode_lokasi = '"+this.app._lokasi+"' and g.tgl_angs between '"+
							this.ePeriode.getText().substr(0,4)+'-'+this.ePeriode.getText().substr(4,2)+'-01'+"' and '"+this.ePeriode.getText().substr(0,4)+'-'+this.ePeriode.getText().substr(4,2)+'-31'+"' ");
										
					sql.add("update kop_pinj_sch a, kop_pinjbill_d b set a.status_gen='1', a.no_bill='"+this.eBukti.getText()+
					        "' where a.no_pinj=b.no_pinj and a.no_kontrak=b.no_kontrak and a.cicilan_ke=b.cicilan_ke and a.kode_lokasi=b.kode_lokasi and b.kode_lokasi='"+this.app._lokasi+"' and b.no_bill='"+this.eBukti.getText()+"'");					
	...
	doDblClick: function(sender, col, row){
		if (this.sg2.getCell(0,row) != "") {
			var data = this.dbLib.getDataProvider("select a.no_kontrak,b.kode_agg,b.nama as nama_agg,date_format(g.tgl_angs,'%d/%m/%Y')as tanggal,a.keterangan,a.nilai,(a.nilai+ifnull(bb.nbill,0)-ifnull(aa.nangs,0)) as baki, "+
												  ...
												  "                                   group by x.no_kontrak,x.no_pinj,x.kode_lokasi) bb "+
												  "                                   on bb.no_kontrak=a.no_kontrak and bb.no_pinj=a.no_pinj and bb.kode_lokasi=a.kode_lokasi "+
												  "where b.kode_loker='"+this.sg2.getCell(0,row)+"' and a.akun_pjasa='"+this.sg2.getCell(2,row)+"' and a.progress = '1' and a.status_aktif='1' and d.no_del='-' and f.no_del='-' and g.status_gen='0' and "+
