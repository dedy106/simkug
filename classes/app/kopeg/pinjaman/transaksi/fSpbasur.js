window.app_kopeg_pinjaman_transaksi_fSpbasur = function(owner){
	if (owner){
		try{		
			window.app_kopeg_pinjaman_transaksi_fSpbasur.prototype.parent.constructor.call(this, owner);
			this.className  = "app_kopeg_pinjaman_transaksi_fSpbasur";
			...
			this.cbbPemohon = new portalui_saiCBBL(this,{bound:[20,16,200,20],caption:"Pemohon", btnClick:[this,"FindBtnClick"],readOnly:true,tag:2});
			this.ePot = new portalui_saiLabelEdit(this, {bound:[720,16,200,20],caption:"Nilai Tambah",readOnly:true,tipeText:ttNilai, text:"0", change:[this,"doChange"]});        
			this.cbbVendor = new portalui_saiCBBL(this,{bound:[20,17,200,20],caption:"Asuradur", btnClick:[this,"FindBtnClick"],readOnly:true});
			...
			systemAPI.alert("[app_kopeg_pinjaman_transaksi_fSpbasur]::oncreate lib : ",e);
		}
	}
};
window.app_kopeg_pinjaman_transaksi_fSpbasur.extend(window.portalui_childForm);
window.app_kopeg_pinjaman_transaksi_fSpbasur.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					for (var i in this.gridJurnal.objList){
						...
						d+= "('"+this.eSPB.getText()+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+line.get("kode_akun")+"','"+this.eKet.getText()+"','D',"+parseFloat(line.get("nilai"))+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PINJ','"+line.get("kode_drk")+"','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
						idx++;
					}
					...
					for (var i=0; i < this.sg2.rows.getLength(); i++){
						if (this.sg2.rowValid(i)) {
							d+=",";
							idx++;
							d+="('"+this.eSPB.getText()+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(3,i)+"',"+parseNilai(this.sg2.cells(4,i))+",'"+this.sg2.cells(5,i)+"','-','"+this.app._lokasi+"','PINJ','ADD','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
					...
					for (var i=0; i <= this.sg1.getRowCount(); i++){
						if (this.sg1.cells(0,i) == "APP") {
							sql.add("update kop_pinj_m set no_spbasur='"+this.eSPB.getText()+"' where no_pinj='"+this.sg1.cells(1,i)+"' and no_kontrak = '"+this.sg1.cells(2,i)+"' and kode_lokasi = '"+this.app._lokasi+"'");
						}
					}
	...
	try{			
			if (this.ePeriode.getText() != ""){
				...
				if (typeof data == "object"){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData(["INPROG",line.no_pinj,line.no_kontrak,line.kode_agg,line.nama_agg,floatToNilai(line.nilai_asur),line.kode_akun]);
					}
					this.sg1.validasi();
				}
			}
