window.app_kopeg_pinjaman_transaksi_fSpbasurk = function(owner){
	if (owner){
		try{		
			window.app_kopeg_pinjaman_transaksi_fSpbasurk.prototype.parent.constructor.call(this, owner);
			this.className  = "app_kopeg_pinjaman_transaksi_fSpbasurk";
			owner.childFormConfig(this, "mainButtonClick","Form SPB Asuransi: Koreksi", 0);
			this.maximize();
			//------------------------ login data ------------------------	
			...
			this.ePot = new portalui_saiLabelEdit(this, {bound:[720,16,200,20],caption:"Nilai Tambah",readOnly:true,tipeText:ttNilai, text:"0", change:[this,"doChange"],tag:1});        
			this.cbbVendor = new portalui_saiCBBL(this,{bound:[20,17,200,20],caption:"Asuradur", btnClick:[this,"FindBtnClick"],readOnly:true,tag:1});
			...
			this.p1 = new portalui_panel(this,{bound:[20,35,900,290],caption:"Daftar Kontrak BerAsuransi"});
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro ='PINJSP' and kode_lokasi = '"+this.app._lokasi+"'");
			eval("data = "+data+";");
			...
			var prd = this.dbLib.getDataProvider("select distinct periode from spb_m where modul = 'KP.SPB' and jenis = 'PINJ_AS' and kode_lokasi = '"+this.app._lokasi+"'",true);
			...
			systemAPI.alert("[app_kopeg_pinjaman_transaksi_fSpbasurk]::oncreate lib : ",e);
		}
	}
};
window.app_kopeg_pinjaman_transaksi_fSpbasurk.extend(window.portalui_childForm);
window.app_kopeg_pinjaman_transaksi_fSpbasurk.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					if (parseFloat(this.perLama) < parseFloat(this.app._periode)) {
						...
						sql.add(" update kop_pinj_m set no_spbasur = '-' where no_spbasur='"+this.cbbNbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add("insert into spb_m (no_spb,no_dokumen,tanggal,due_date,akun_hutang,"+
							...
							"','KP.SPB','PINJ_AS',"+parseNilai(this.eNilaiSpb.getText())+",0,0,'F','0','"+this.ePeriode.getText()+"','-','-','"+this.app._userLog+"',now())");
					var idx = 0;
					...
					for (var i=0; i < this.sg2.rows.getLength(); i++){
						if (this.sg2.rowValid(i)) {
							d+=",";
							idx++;
							d+="('"+this.nb+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(3,i)+"',"+parseNilai(this.sg2.cells(4,i))+",'"+this.sg2.cells(5,i)+"','-','"+this.app._lokasi+"','PINJ','ADD','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
					...
					for (var i=0; i <= this.sg1.getRowCount(); i++){
						if (this.sg1.cells(0,i) == "APP") {
							sql.add("update kop_pinj_m set no_spbasur='"+this.nb+"' where no_pinj='"+this.sg1.cells(1,i)+"' and no_kontrak = '"+this.sg1.cells(2,i)+"' and kode_lokasi = '"+this.app._lokasi+"'");
						}
					}
	...
	doLoadData: function(sender){
		try{			
			if (this.cbbNbLama.getText() != ""){
				var data = this.dbLib.getDataProvider("select a.no_pinj,a.no_kontrak,e.kode_agg,e.nama as nama_agg,a.nilai_asur,b.kode_akun, "+
													  "       xx.no_dokumen,xx.keterangan,xx.due_date,xx.tanggal,xx.periode,xx.posted,xx.nik_buat,yy.nama as nama_buat,xx.kode_terima,zz.nama as nama_vendor "+
						   ...
						   "                  inner join spb_j b on d.no_spb=b.no_spb and d.kode_lokasi=b.kode_lokasi and b.jenis='ASUR' "+
						   "                  inner join kop_agg e on a.kode_agg=e.kode_agg and a.kode_lokasi=e.kode_lokasi "+
						   "                  inner join spb_m xx on a.no_spbasur=xx.no_spb and a.kode_lokasi = xx.kode_lokasi "+
						   ...
						   "where a.kode_lokasi='"+this.app._lokasi+"' and a.no_spbasur='"+this.cbbNbLama.getText()+"' and a.nilai_asur<> 0 ");
				eval("data = "+data+";");
				if (typeof data == "object"){
					var line;
					this.sg1.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData(["APP",line.no_pinj,line.no_kontrak,line.kode_agg,line.nama_agg,floatToNilai(line.nilai_asur),line.kode_akun]);
					}
					this.sg1.validasi();
				...
				var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.keterangan,a.dc,a.nilai,a.kode_pp,c.nama as nama_pp "+
						   ...
						   "where a.jenis='ADD' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_spb='"+this.cbbNbLama.getText()+"' order by a.dc desc");
				eval("data = "+data+";");
				if (typeof data == "object"){
					var line;
					this.sg2.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg2.appendData([line.kode_akun,line.nama_akun,line.keterangan,line.dc,floatToNilai(line.nilai),line.kode_pp,line.nama_pp]);
					}
					this.sg2.validasi();
				}
			}
	...
	FindBtnClick: function(sender){
		if (sender == this.cbbNbLama) {   
			this.standarLib.showListData(this, "Daftar Bukti SPB",sender,undefined, 
										  "select no_spb, keterangan  from spb_m where modul = 'KP.SPB' and jenis = 'PINJ_AS' and progress= '0' and kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cbbPerLama.getText()+"' and no_del='-'", 
										  "select count(no_spb)       from spb_m where modul = 'KP.SPB' and jenis = 'PINJ_AS' and progress= '0' and kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cbbPerLama.getText()+"' and no_del='-'",
