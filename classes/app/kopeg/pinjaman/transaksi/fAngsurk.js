window.app_kopeg_pinjaman_transaksi_fAngsurk = function(owner){
	if (owner){
		try{		
			window.app_kopeg_pinjaman_transaksi_fAngsurk.prototype.parent.constructor.call(this, owner);
			this.className  = "app_kopeg_pinjaman_transaksi_fAngsurk";						
			owner.childFormConfig(this, "mainButtonClick","Form Angsuran Pinjaman: Koreksi", 0);			
			this.maximize();
			...
			this.eUdp = new portalui_saiLabelEdit(this, {bound:[340,28,180,20],change:[this,"doChange"],caption:"Nilai UDP",tipeText:ttNilai, text:"0",labelWidth:80,tag:1});
			...
			this.p2 = new portalui_panel(this,{bound:[20,36,900,240],caption:"Data SPB"});
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag,value1 from spro where kode_spro in ('PINJIM','PINJTP','PINJUD','PINJPD','PINJOE') and kode_lokasi = '"+this.app._lokasi+"' ");
			eval("data = "+data+";");
			...
			var prd = this.dbLib.getDataProvider("select distinct periode from kop_pinjangs_m where jenis = 'PINJANGS' and kode_lokasi = '"+this.app._lokasi+"'",true);
			...
			systemAPI.alert("[app_kopeg_pinjaman_transaksi_fAngsurk]::oncreate lib : ",e);
		}
	}
};
window.app_kopeg_pinjaman_transaksi_fAngsurk.extend(window.portalui_childForm);
window.app_kopeg_pinjaman_transaksi_fAngsurk.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					if (parseFloat(this.perLama) < parseFloat(this.app._periode)) {
						...
						sql.add(" insert into kop_pinjangs_m (no_angs,no_dokumen,keterangan,tanggal,nilai,nilai_depo,nilai_udp,nilai_sls,jenis,progress,akun_ar,periode,kode_lokasi,posted,kurs,kode_curr,kode_pp,no_del,no_link,nik_app,nik_user,tgl_input,p_fee,nilai_fee,no_spb,nilai_beban) "+
						...
						sql.add("insert into kop_pinjangs_d (no_angs,no_pinj,no_kontrak,tanggal,akun_ar,npokok,nbunga,kode_lokasi,dc,no_del)"+
								" select concat(no_angs,'r'),no_pinj,no_kontrak,tanggal,akun_ar,npokok,nbunga,kode_lokasi,'C','BATAL' "+ 
					...
					if (this.eDeposit.getText() != "0") {
						d+= ",";
						idx++;
						d += "('"+this.nb+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.akunTP+"','"+this.eKet.getText()+"','D',"+parseNilai(this.eDeposit.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PINJ','PK_DEPO','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
						sql.add("insert into kop_depo(no_depo,tanggal,keterangan,dc,nilai,kode_agg,modul,jenis,periode,kode_lokasi,kode_pp,nik_app,no_del,no_link,kurs,kode_curr,nik_user,tgl_input) values "+
							    "('"+this.nb+"','"+this.dTgl.getDateString()+"','"+this.eKet.getText()+"','C',"+parseNilai(this.eDeposit.getText())+",'"+this.cbbAgg.getText()+"','PINJ','ANGS_SLS','"+this.ePeriode.getText()+"','"+this.app._lokasi+"','"+this.app._kodePP+"','"+this.cbbPenerima.getText()+"','-','-',1,'IDR','"+this.app._userLog+"',now())");
					}					
					if (this.eSls.getText() != "0") { //<<------------ sls lebih akan diangap denda pelunasan,,hingga langsung masuk ke pdpt jasa angsuran
						d+= ",";
						idx++;
						d += "('"+this.nb+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.akunPdpt+"','"+this.eKet.getText()+"','C',"+parseNilai(this.eSls.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PINJ','SL_PDPT','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";					
					}
					if (this.eBeban.getText() != "0") { 
						d+= ",";
						idx++;
						d += "('"+this.nb+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.akunBeban+"','"+this.eKet.getText()+"','D',"+parseNilai(this.eBeban.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PINJ','BEBAN','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
					}
					if (this.eUdp.getText() != "0") {
						...
						sql.add("insert into kop_depo(no_depo,tanggal,keterangan,dc,nilai,kode_agg,modul,jenis,periode,kode_lokasi,kode_pp,nik_app,no_del,no_link,kurs,kode_curr,nik_user,tgl_input) values "+
							    "('"+this.nb+"','"+this.dTgl.getDateString()+"','"+this.eKet.getText()+"','C',"+parseNilai(this.eUdp.getText())+",'"+this.cbbAgg.getText()+"','PINJ','UDP','"+this.ePeriode.getText()+"','"+this.app._lokasi+"','"+this.app._kodePP+"','"+this.cbbPenerima.getText()+"','-','-',1,'IDR','"+this.app._userLog+"',now())");
					}
					d+= ",";
					idx++;
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :	
				...
				if (tot <= 0 || tot >= nilaiToFloat(this.eBaki.getText())){
	...
	doChange: function(sender){
		...
		if (sender == this.cbbAgg) {
			var data = this.dbLib.getDataProvider("select ifnull(sum(case dc when 'D' then nilai else -nilai end),0) as saldo from kop_depo where kode_agg ='"+this.cbbAgg.getText()+"' and modul in ('SIMP','PINJ') and jenis <> 'UDP' and kode_lokasi = '"+this.app._lokasi+"' and no_depo<>'"+this.cbbNbLama.getText()+"'");
			eval("data = "+data+";");
			if (typeof data == "object"){
	...
	doLoadData: function(sender){
		if (this.cbbNbLama.getText() != "") {
			var data = this.dbLib.getDataProvider("select yy.nilai_beban,yy.akun_ar,yy.no_spb,b.no_kontrak as nokont,b.no_pinj as nopinj,b.kode_agg,d.nama as nama_agg,yy.no_dokumen,yy.keterangan,yy.nilai,yy.nilai_depo,yy.nilai_udp,yy.nilai_sls,yy.nik_app,zz.nama as nama_app,yy.posted,yy.periode "+
			                                      "from kop_pinj_m b "+
												  "                      inner join kop_pinjangs_d xx on b.no_kontrak=xx.no_kontrak and xx.no_pinj=b.no_pinj and xx.kode_lokasi=b.kode_lokasi "+
												  "                      inner join kop_agg d on b.kode_agg=d.kode_agg and b.kode_lokasi=d.kode_lokasi "+
												  "                      inner join kop_pinjangs_m yy on xx.no_angs=yy.no_angs and xx.kode_lokasi=yy.kode_lokasi "+
			...
			var data = this.dbLib.getDataProvider("select 'BILL' as status,a.no_bill as no_bukti,date_format(ab.tanggal,'%Y%m') as periode,date_format(ab.tanggal,'%d/%m/%Y') as tanggal,0 as npokok,sum(case a.dc when 'D' then a.nbunga else -a.nbunga end) as nbunga,b.nilai as baki,b.akun_piutang,b.akun_pjasa,a.dc,b.nilai_tagihan "+
												  ...
												  "where a.no_kontrak='"+line.nokont+"' and a.no_pinj='"+line.nopinj+"' and a.kode_lokasi = '"+this.app._lokasi+"' "+
			                                      ...
			                                      "from (select a.no_angs,a.no_kontrak,a.no_pinj,a.kode_lokasi,a.tanggal,a.dc,case a.dc when 'D' then sum(a.npokok) else -sum(npokok) end as npokok,"+
												  "      case a.dc when 'D' then sum(a.nbunga) else -sum(a.nbunga) end as nbunga from kop_pinjangs_d a group by a.no_kontrak,a.no_pinj,a.kode_lokasi,a.no_angs,a.tanggal,a.dc "+
												  ") a inner join kop_pinj_m b on a.no_kontrak=b.no_kontrak and a.no_pinj=b.no_pinj and a.kode_lokasi=b.kode_lokasi "+												  
												  "where a.no_angs <> '"+this.cbbNbLama.getText()+"' and a.no_kontrak='"+line.nokont+"' and a.no_pinj='"+line.nopinj+"' and a.kode_lokasi = '"+this.app._lokasi+"' "+
	...
	FindBtnClick: function(sender){
		...
		if (sender == this.cbbNbLama) {   
			this.standarLib.showListData(this, "Daftar Bukti Angsuran",sender,undefined, 
										  "select no_angs, keterangan  from kop_pinjangs_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cbbPerLama.getText()+"' and no_del='-' and jenis='PINJANGS' and progress in ('0','3')", 
										  "select count(no_angs)       from kop_pinjangs_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cbbPerLama.getText()+"' and no_del='-' and jenis='PINJANGS' and progress in ('0','3')",
