window.app_kopeg_pinjaman_report_flLAngs = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjaman_report_flLAngs.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_pinjaman_report_flLAngs";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan List Schedule Angsuran", 2);
	...
	this.tanda="=";
	this.lokasi=this.app._lokasi;this.getPeriode="select distinct periode from kop_pinjbill_m where kode_lokasi ='"+this.lokasi+"' ";
...
window.app_kopeg_pinjaman_report_flLAngs.extend(window.portalui_childForm);
window.app_kopeg_pinjaman_report_flLAngs.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		if (row === 1){
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_pinjbill_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",[this.sg1.columns.get(2).pickList]);
		}		
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2){
				...
				var sql = "select c.kode_agg,upper(e.nama) as nm,c.no_kontrak,b.cicilan_ke, "+
						"date_format(d.tgl_angs,'%d/%m/%Y') as tglangs,b.npokok+b.nbunga as angs,b.npokok,b.nbunga "+
						"from kop_pinjbill_m a inner join kop_pinjbill_d b on a.no_bill=b.no_bill and a.kode_lokasi=b.kode_lokasi "+
						"inner join kop_pinj_m c on b.no_pinj=c.no_pinj and b.kode_lokasi=c.kode_lokasi "+
						"inner join kop_pinj_sch d on b.no_bill=d.no_bill and b.no_pinj=d.no_pinj and b.kode_lokasi=d.kode_lokasi and d.status_gen='1' "+
						"inner join kop_agg e on c.kode_agg=e.kode_agg and c.kode_lokasi=e.kode_lokasi "+
						this.filter+
						" order by c.kode_agg ";
				this.scriptSqlCount = "select count(*) "+
						...
						"inner join kop_agg e on c.kode_agg=e.kode_agg and c.kode_lokasi=e.kode_lokasi "+
						this.filter;
				this.title = new server_util_arrayList({items:["Anggota","Nama","No. Kontrak","Cicil Ke","Tgl. Angs.","Angsuran","Pokok","Jasa"]});
			...
			systemAPI.alert("[app_kopeg_pinjaman_report_flLAngs]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br />LIST SCHEDULE ANGSURAN<br />";
		var d = new Date();
