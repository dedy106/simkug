window.app_kopeg_pinjaman_report_flBilling = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjaman_report_flBilling.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_pinjaman_report_flBilling";
		this.maximize();
	...
	this.getPeriode="select distinct periode from kop_pinjbill_m where kode_lokasi ='"+this.lokasi+"'";
	this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Kode Lokasi",this.tanda,this.lokasi]);
	this.gridLib.SGEditData(this.sg1,1,[0,1,2],["Periode","=",this.dbLib.getPeriodeFromSQL(this.getPeriode)]);
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Kode Lokasi Kerja","All",""]);
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Kode Nasabah","All",""]);
	this.gridLib.SGEditData(this.sg1,4,[0,1,2],["Jenis Laporan","=","Per Kontrak"]);
	this.pager = 1;
};
window.app_kopeg_pinjaman_report_flBilling.extend(window.portalui_childForm);
window.app_kopeg_pinjaman_report_flBilling.implement({
	...
	doSelectCell: function(sender, col, row)
	{
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["Per Kontrak","Per Lokasi Kerja"]);
		}
	},
	mainButtonClick: function(sender)
	{
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,4) === "Per Kontrak"){
					this.nama_report="server_report_kopeg_rptBilling";
					...
					this.filter2=this.sg1.getCell(2,1)+";;"+(new Date()).toLocaleString();
				}else{
					this.nama_report="server_report_kopeg_rptBilling2";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("e.kode_loker",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
