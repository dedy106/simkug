window.app_kopeg_pinjaman_report_flSpbPinj = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjaman_report_flSpbPinj.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_pinjaman_report_flSpbPinj";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan SPB Pinjaman", 2);
...
window.app_kopeg_pinjaman_report_flSpbPinj.extend(window.portalui_childForm);
window.app_kopeg_pinjaman_report_flSpbPinj.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row === 4)
		{
			this.filterRep.ListDataSGFilter(this, "Data No. SPB Pinjaman",this.sg1, this.sg1.row, this.sg1.col,
										  "select no_spb,keterangan from spb_m where kode_lokasi='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"' and modul='KP.SPB' and jenis='PINJ' and no_del='-'",
										  "select count(*) from spb_m where kode_lokasi='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"' and modul='KP.SPB' and jenis='PINJ' and no_del='-'",
	...
	doSelectCell: function(sender, col, row)
	{
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["All","SPB Pinjaman","Daftar Pengajuan Pinjaman"]);
		}
	},
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,5) === "All"){
					this.nama_report="server_report_kopeg_rptSPBPinj";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						...
						this.filterRep.filterStr("a.no_spb",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
					this.status=true;
				}else if (this.sg1.getCell(2,5) === "SPB Pinjaman"){
					this.nama_report="server_report_kopeg_rptSPBPinj";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("e.kode_loker",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
					...
					this.nama_report="server_report_kopeg_rptSPBPinj2";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
			...
			case "preview" : 
				if (this.sg1.getCell(2,5) === "All"){
					if (this.status){
						...
						this.report.preview("server_report_kopeg_rptSPBPinj2",this.filter1,this.page,1, this.showFilter,this.app._lokasi,this.filter2);
