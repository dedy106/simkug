window.app_kopeg_pinjaman_report_flAngsur = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjaman_report_flAngsur.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_pinjaman_report_flAngsur";
		this.maximize();
	...
	this.getPeriode="select distinct periode from kop_pinjangs_m where kode_lokasi ='"+this.lokasi+"'";
...
window.app_kopeg_pinjaman_report_flAngsur.extend(window.portalui_childForm);
window.app_kopeg_pinjaman_report_flAngsur.implement({
	...
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_pinjangs_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 5)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,5) === "Kwitansi"){
					this.nama_report="server_report_kopeg_rptAngsur";
					...
					this.nama_report="server_report_kopeg_rptAngsur2";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
