// JavaScript Document
window.app_kopeg_pinjaman_report_flJurnalkB = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjaman_report_flJurnalkB.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_pinjaman_report_flJurnalkB";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from KAS_M where POSTED='T' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_kopeg_pinjaman_report_flJurnalkB.extend(window.portalui_childForm);
window.app_kopeg_pinjaman_report_flJurnalkB.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			...
			var sql1="select a.no_kas,a.keterangan from kas_m a where a.POSTED='T' "+filter; 
			var sql2="select count(a.no_kas) from kas_m a where a.POSTED='T' "+filter;
			this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
			                                new Array("a.no_kas","a.keterangan"),"and",new Array("No Bukti","Keterangan"),"and");
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where  posted='T' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			this.nama_report="server_report_saku3_rptJurnalkB";  
