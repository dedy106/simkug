window.app_kopeg_pinjaman_report_flAngsKol = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjaman_report_flAngsKol.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_pinjaman_report_flAngsKol";
		this.maximize();
	...
	this.getPeriode="select distinct periode as periode from kop_pinjangs_m where kode_lokasi ='"+this.lokasi+"' and jenis='PINJKOL' order by periode desc ";
...
window.app_kopeg_pinjaman_report_flAngsKol.extend(window.portalui_childForm);
window.app_kopeg_pinjaman_report_flAngsKol.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row === 4)
		{
			this.filterRep.ListDataSGFilter(this, "Data Kwitansi",this.sg1, this.sg1.row, this.sg1.col,
										  "select distinct a.no_angs,a.keterangan from kop_pinjangs_m a inner join kop_pinjangs_d b on a.no_angs=b.no_angs and a.kode_lokasi=b.kode_lokasi "+
										  ...
										  "select count(distinct a.no_angs,a.keterangan) from kop_pinjangs_m a inner join kop_pinjangs_d b on a.no_angs=b.no_angs and a.kode_lokasi=b.kode_lokasi "+
										  "inner join kop_pinj_m c on b.no_pinj=c.no_pinj and b.kode_lokasi=c.kode_lokasi "+
										  "inner join kop_agg d on c.kode_agg=d.kode_agg and c.kode_lokasi=d.kode_lokasi "+
										  "where a.kode_lokasi='"+this.sg1.getCell(2,0)+"' and a.periode='"+this.sg1.getCell(2,1)+"' and d.kode_loker like '%"+this.sg1.getCell(2,2)+"' and c.kode_agg like '%"+this.sg1.getCell(2,3)+"' and a.no_angs like '%"+this.sg1.getCell(2,4)+"' ",
	...
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_pinjangs_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' and jenis='PINJKOL' order by periode desc ",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 5)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			...
			{ 
				...
				if (this.jenisLap === "Transaksi"){
					var sql = "select a.no_angs,date_format(a.tanggal,'%d/%m/%Y') as tglangs,a.no_dokumen,a.keterangan,a.nilai,d.no_kontrak, "+
							"concat(e.nama,'-',d.kode_agg) as nm,c.cicilan_ke,c.npokok,c.nbunga,c.npokok+c.nbunga as tghn,c.npokok+c.nbunga as angs "+
							"from kop_pinjangs_m a inner join kop_pinjangs_d b on a.no_angs=b.no_angs and a.kode_lokasi=b.kode_lokasi and a.jenis='PINJKOL' "+
							"inner join kop_pinjbill_d c on b.no_pinj=c.no_pinj and b.kode_lokasi=c.kode_lokasi "+
							"inner join kop_pinj_m d on b.no_pinj=d.no_pinj and b.kode_lokasi=d.kode_lokasi "+
							"inner join kop_agg e on d.kode_agg=e.kode_agg and d.kode_lokasi=e.kode_lokasi "+
							"inner join kop_pinjbill_m f on f.no_bill = c.no_bill and f.tanggal = b.tanggal and b.kode_lokasi=c.kode_lokasi "+
							this.filter+
							" order by a.no_angs ";
					...
					var sql = "select a.no_angs,date_format(a.tanggal,'%d/%m/%Y') as tglangs,a.no_dokumen,a.keterangan,a.nilai "+
							"from kop_pinjangs_m a inner join kop_pinjangs_d b on a.no_angs=b.no_angs and a.kode_lokasi=b.kode_lokasi and a.jenis='PINJKOL' "+
							"inner join kop_pinjbill_d c on b.no_pinj=c.no_pinj and b.kode_lokasi=c.kode_lokasi "+
							"inner join kop_pinj_m d on b.no_pinj=d.no_pinj and b.kode_lokasi=d.kode_lokasi "+
							"inner join kop_agg e on d.kode_agg=e.kode_agg and d.kode_lokasi=e.kode_lokasi "+
							"inner join kop_pinjbill_m f on f.no_bill = c.no_bill and date_format(f.tanggal,'%Y%m') = date_format(b.tanggal,'%Y%m') and b.kode_lokasi=c.kode_lokasi "+
							this.filter+
			...
			systemAPI.alert("[app_kopeg_pinjaman_report_flAngsKol]::mainButtonClick:"+e);
		}
	},
