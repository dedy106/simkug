window.app_kopeg_pinjaman_report_flRSched = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjaman_report_flRSched.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_pinjaman_report_flRSched";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,4,[0,1,2],["No. Pinjaman","All",""]);
	this.pager = 1;
};
window.app_kopeg_pinjaman_report_flRSched.extend(window.portalui_childForm);
window.app_kopeg_pinjaman_report_flRSched.implement({
	...
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_pinj_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' order by periode ",[this.sg1.columns.get(2).pickList]);
		}
	},
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter2= this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.no_pinj",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.nama_report="server_report_kopeg_rptRSched";
				this.viewer.setTotalPage(this.report.getTotalPage(this.nama_report,this.filter,this.pager,this.filter2));
				this.app._mainForm.reportNavigator.setTotalPage(this.viewer.getTotalPage());
				this.app._mainForm.reportNavigator.rearrange();
				this.report.preview(this.nama_report,this.filter, 1,this.pager, this.showFilter, this.lokasi,this.filter2, undefined, "array");
				this.page = 1;
				this.allBtn = false;
			}
		}catch(e)
		...
		switch (methodName)
		{
			case "preview" : 
				if (result instanceof portalui_arrayList){						
						this.viewer.previewMultiPage(result.objList, false, ["Realisasi Angsuran","Schedule Angsuran"]);
