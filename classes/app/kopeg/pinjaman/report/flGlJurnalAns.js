// JavaScript Document
window.app_kopeg_pinjaman_report_flGlJurnalAns = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjaman_report_flGlJurnalAns.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_pinjaman_report_flGlJurnalAns";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kop_pinjangs_m where jenis='PINJANGS' and POSTED='T' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_kopeg_pinjaman_report_flGlJurnalAns.extend(window.portalui_childForm);
window.app_kopeg_pinjaman_report_flGlJurnalAns.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			...
			var sql1="select a.no_angs,a.keterangan from kop_pinjangs_m a where a.jenis='PINJANGS' "+filter; 
			var sql2="select count(a.no_angs) from kop_pinjangs_m a where a.jenis='PINJANGS' "+filter;
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_pinjangs_m where  jenis='PINJANGS' and posted='T' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			this.nama_report="server_report_saku3_rptGlJurnalAnsPinj";
