// JavaScript Document
window.app_kopeg_proyek_report_flGlJurnalStrPry = function(owner)
{
	if (owner)
	{
		window.app_kopeg_proyek_report_flGlJurnalStrPry.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_proyek_report_flGlJurnalStrPry";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(a.periode) as periode from kop_proyeksetor_m a inner join kop_proyeksetor_j b on a.no_setor=b.no_setor and a.kode_lokasi=b.kode_lokasi  where  b.modul='ARPR' and b.jenis='ARPR_AR' and a.POSTED='T' and a.kode_lokasi='"+this.app._lokasi+"'");
...
window.app_kopeg_proyek_report_flGlJurnalStrPry.extend(window.portalui_childForm);
window.app_kopeg_proyek_report_flGlJurnalStrPry.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			...
			var filter=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and") +
						this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and");
			var sql1="select a.no_setor,a.keterangan from kop_proyeksetor_m a inner join kop_proyeksetor_j b on a.no_setor=b.no_setor and a.kode_lokasi=b.kode_lokasi  where  b.modul='ARPR' and b.jenis='ARPR_AR' and a.POSTED='T'  "+filter; 
			var sql2="select count(a.no_setor) from kop_proyeksetor_m a where inner join kop_proyeksetor_j b on a.no_setor=b.no_setor and a.kode_lokasi=b.kode_lokasi  where  b.modul='ARPR' and b.jenis='ARPR_AR' and a.POSTED='T' "+filter;
			this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
			                                new Array("a.no_setor","a.keterangan"),"and",new Array("No Bukti","Keterangan"),"and");
		
		}
	},
	doSelectCell: function(sender, col, row){
		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3),new  Array("123","123","123","123"));
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3),new  Array(2,0,0,2));
		
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from kop_proyeksetor_m where inner join kop_proyeksetor_j b on a.no_setor=b.no_setor and a.kode_lokasi=b.kode_lokasi  where  b.modul='ARPR' and b.jenis='ARPR_AR' and a.POSTED='T' ='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			this.filter=this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
						...
						this.filterRep.filterStr("modul",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
						this.filterRep.filterStr("no_bukti",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
			this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(1,1)+"/"+this.sg1.getCell(3,1)+"/"+this.sg1.getCell(2,0);
			this.nama_report="server_report_saku3_rptGlJurnalStrPry";
