window.app_kopeg_proyek_report_flPosisiBillPro = function(owner)
{
	if (owner)
	{
		window.app_kopeg_proyek_report_flPosisiBillPro.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_proyek_report_flPosisiBillPro";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Posisi Invoice Piutang Proyek", 2);
...
window.app_kopeg_proyek_report_flPosisiBillPro.extend(window.portalui_childForm);
window.app_kopeg_proyek_report_flPosisiBillPro.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==0)
	{
	...
	if (row == 3)
	{
		var sql1="select no_ar,keterangan from kop_arproyek_m "+
		...
		var sql2="select count(no_ar) from kop_arproyek_m "+
				 this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
				 this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and")+
				 this.filterRep.filterStr("kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
	...
	if (row == 5)
	{
		var sql1="select no_bukti,keterangan from kop_proyekbayar_m "+
				 this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
				 this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and")+
				 this.filterRep.filterStr("kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
		var sql2="select count(no_bukti) from kop_proyekbayar_m "+
				 this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
				 this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and")+
				 this.filterRep.filterStr("kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
...
window.app_kopeg_proyek_report_flPosisiBillPro.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_kopeg_proyek_report_flPosisiBillPro.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.sql =  "select a.kode_pp,f.nama as nama_pp,a.no_ar,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.keterangan,a.nilai+a.nilai_ppn as nilai,b.no_bukti,date_format(c.tanggal,'%d/%m/%Y') as tgl_bayar,g.no_setor,date_format(h.tanggal,'%d/%m/%Y') as tgl_setor,d.no_kas,date_format(e.tanggal,'%d/%m/%Y') as tgl_kas,a.posted,a.no_del,a.no_link "+
						...
						"left join kop_proyekbayar_d b on a.no_ar=b.no_ar and a.kode_lokasi=b.kode_lokasi "+
						"left join kop_proyekbayar_m c on b.no_bukti=c.no_bukti and b.kode_lokasi=c.kode_lokasi "+
						"left join kop_proyeksetor_d g on c.no_bukti=g.no_bukti and c.kode_lokasi=g.kode_lokasi "+
						"left join kop_proyeksetor_m h on g.no_setor=h.no_setor and g.kode_lokasi=h.kode_lokasi "+
			...
			this.scriptSqlCount =   "select count(*) "+
						"from kop_arproyek_m a "+this.filter;
	...
	{
		alert("[app_kopeg_proyek_report_flPosisiBillPro]::mainButtonClick:"+e);
	}
};
window.app_kopeg_proyek_report_flPosisiBillPro.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_kopeg_proyek_report_flPosisiBillPro.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_kopeg_proyek_report_flPosisiBillPro.prototype.previewReport = function(dthtml)
{
	var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN POSISI INVOICE PIUTANG PROYEK<br>";			
	var d = new Date();
...
window.app_kopeg_proyek_report_flPosisiBillPro.prototype.doCloseReportClick = function(sender)
{
...
window.app_kopeg_proyek_report_flPosisiBillPro.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_kopeg_proyek_report_flPosisiBillPro.prototype.doRowPerPageChange = function(sender, rowperpage)
{
