window.app_kopeg_proyek_report_flByr = function(owner)
{
	if (owner)
	{
		window.app_kopeg_proyek_report_flByr.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_proyek_report_flByr";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Pembayaran Tagihan", 2);
	...
	this.getPeriode="select distinct periode from kop_proyekbayar_m where kode_lokasi ='"+this.lokasi+"' order by periode desc ";
...
window.app_kopeg_proyek_report_flByr.extend(window.portalui_childForm);
window.app_kopeg_proyek_report_flByr.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row === 2)
		{
			this.standar.ListDataSGFilter(this, "Data Pembayaran Tagihan",this.sg1, this.sg1.row, this.sg1.col,
	...
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_proyekbayar_m where kode_lokasi ='"+this.lokasi+"' order by periode desc ",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 3)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,3) === "Kwitansi"){
					this.nama_report="server_report_kopeg_rptByrPry";
					...
					this.nama_report="server_report_kopeg_rptByrPry2";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.no_bukti",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
