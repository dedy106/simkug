window.app_kopeg_proyek_report_flSpb = function(owner)
{
	if (owner)
	{
		window.app_kopeg_proyek_report_flSpb.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_proyek_report_flSpb";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan SPB Proyek", 2);
...
window.app_kopeg_proyek_report_flSpb.extend(window.portalui_childForm);
window.app_kopeg_proyek_report_flSpb.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row === 2)
		{
			...
			var sql2="select count(no_spb) from spb_m "+
					 ...
					 this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and")+
					 " and modul='SPB' and jenis='APPROYEK' ";
			this.filterRep.ListDataSGFilter(this, "Data SPB",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_spb","keterangan"),"and",["No. Bukti","Keterangan"]);
		}
	},
	doSelectCell: function(sender, col, row)
	{
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["All","SPB","Lampiran SPB"]);
		}
	},
	mainButtonClick: function(sender)
	{
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.gridLib.SGEditData(this.sg1,1,[0,1,2],["Periode","=",this.dbLib.getPeriodeFromSQL(this.getPeriode)]);
				this.gridLib.SGEditData(this.sg1,2,[0,1,2],["No. Bukti","All",""]);
				this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Jenis Laporan","=","SPB"]);
			}else{
				...
				if (this.sg1.getCell(2,3) === "All"){
					this.nama_report="server_report_kopeg_rptSpbP";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						...
						" and a.jenis = 'APPROYEK'";
					this.filter2="";
					this.status=true;
				}else if (this.sg1.getCell(2,3) === "SPB"){
					this.nama_report="server_report_kopeg_rptSpbP";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.no_spb",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
					...
					this.nama_report="server_report_kopeg_rptSpbP2";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.no_spb",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
						" and a.jenis = 'APPROYEK'";
			...
			case "preview" : 
				if (this.sg1.getCell(2,3) === "All"){
					if (this.status){
						...
						this.report.preview("server_report_kopeg_rptSpbP2",this.filter,this.page,1, this.showFilter,this.app._lokasi,this.filter2);
