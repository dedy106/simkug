// JavaScript Document
window.app_kopeg_proyek_report_flGlJurnalAkruArPry = function(owner)
{
	if (owner)
	{
		window.app_kopeg_proyek_report_flGlJurnalAkruArPry.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_proyek_report_flGlJurnalAkruArPry";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kop_arproyek_m where jenis='ARPR' and POSTED='T' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_kopeg_proyek_report_flGlJurnalAkruArPry.extend(window.portalui_childForm);
window.app_kopeg_proyek_report_flGlJurnalAkruArPry.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			...
			var sql1="select a.no_ar,a.keterangan from kop_arproyek_m a where a.jenis='ARPR' "+filter; 
			var sql2="select count(a.no_Ar) from kop_arproyek_m a where a.jenis='ARPR' "+filter;
			this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
			                                new Array("a.no_ar","a.keterangan"),"and",new Array("No Bukti","Keterangan"),"and");
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_arproyek_m where jenis='ARPR' and posted='T' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			this.filter=this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
						...
						this.filterRep.filterStr("no_ar",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
			this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(1,1)+"/"+this.sg1.getCell(3,1)+"/"+this.sg1.getCell(2,0);
			this.nama_report="server_report_saku3_rptGlJurnalArPry";
