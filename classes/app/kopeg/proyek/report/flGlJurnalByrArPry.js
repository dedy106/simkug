// JavaScript Document
window.app_kopeg_proyek_report_flGlJurnalByrArPry = function(owner)
{
	if (owner)
	{
		window.app_kopeg_proyek_report_flGlJurnalByrArPry.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_proyek_report_flGlJurnalByrArPry";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kop_proyekbayar_m where jenis='ARBYR' and POSTED='T' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_kopeg_proyek_report_flGlJurnalByrArPry.extend(window.portalui_childForm);
window.app_kopeg_proyek_report_flGlJurnalByrArPry.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			...
			var sql1="select a.no_bukti,a.keterangan from kop_proyekbayar_m a where a.jenis='ARBYR' "+filter; 
			var sql2="select count(a.no_bukti) from kop_proyekbayar_m a where a.jenis='ARBYR' "+filter;
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_proyekbayar_m where jenis='ARBYR' and posted='T' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			this.nama_report="server_report_saku3_rptGlJurnalByrArPry";
