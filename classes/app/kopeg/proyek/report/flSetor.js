window.app_kopeg_proyek_report_flSetor = function(owner)
{
	if (owner)
	{
		window.app_kopeg_proyek_report_flSetor.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_proyek_report_flSetor";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Setoran Pembayaran", 2);
	...
	this.getPeriode="select distinct periode from kop_proyeksetor_m where kode_lokasi ='"+this.lokasi+"' order by periode desc ";
	this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Kode Lokasi",this.tanda,this.lokasi]);
	this.gridLib.SGEditData(this.sg1,1,[0,1,2],["Periode","=",this.dbLib.getPeriodeFromSQL(this.getPeriode)]);
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["No. Kwitansi","All",""]);
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["No. Setoran","All",""]);
	this.pager = 1;
};
window.app_kopeg_proyek_report_flSetor.extend(window.portalui_childForm);
window.app_kopeg_proyek_report_flSetor.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row === 2)
		{
			this.standar.ListDataSGFilter(this, "Data Kwitansi",this.sg1, this.sg1.row, this.sg1.col,
									  "select no_bukti,keterangan from kop_proyekbayar_m "+
									  ...
									  "select count(*) from kop_proyekbayar_m "+
									  this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where")+
									  " and kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",
									  ["no_bukti","keterangan"],"and",["No. Bukti","Keterangan"]);
		}
		if (row === 3)
		{
			this.filterRep.ListDataSGFilter(this, "Data Setoran",this.sg1, this.sg1.row, this.sg1.col,
										  "select a.no_setor,a.keterangan from kop_proyeksetor_m a inner join kop_proyeksetor_d b on a.no_setor=b.no_setor and a.kode_lokasi=b.kode_lokasi "+
										  "where a.kode_lokasi ='"+this.sg1.getCell(2,0)+"' and a.periode='"+this.sg1.getCell(2,1)+"' and b.no_bukti like '%"+this.sg1.getCell(2,2)+"' ",
										  "select count(*) from kop_proyeksetor_m a inner join kop_proyeksetor_d b on a.no_setor=b.no_setor and a.kode_lokasi=b.kode_lokasi "+
										  "where a.kode_lokasi ='"+this.sg1.getCell(2,0)+"' and a.periode='"+this.sg1.getCell(2,1)+"' and b.no_bukti like '%"+this.sg1.getCell(2,2)+"' ",
	...
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_proyeksetor_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' order by periode desc ",[this.sg1.columns.get(2).pickList]);
		}
	},
	mainButtonClick: function(sender)
	{
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_kopeg_rptSetorP";
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
					this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
					this.filterRep.filterStr("b.no_bukti",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
					this.filterRep.filterStr("a.no_setor",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
