window.app_kopeg_proyek_report_flPjPtg = function(owner)
{
	if (owner)
	{
		window.app_kopeg_proyek_report_flPjPtg.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_proyek_report_flPjPtg";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Pertanggungan Panjar Proyek", 2);
	...
	this.getPeriode="select distinct periode from ptg_m where kode_lokasi ='"+this.lokasi+"' order by periode desc";
...
window.app_kopeg_proyek_report_flPjPtg.extend(window.portalui_childForm);
window.app_kopeg_proyek_report_flPjPtg.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		if (row === 1){
			this.dbLib.setItemsFromSQL("select distinct periode from ptg_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' order by periode desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 3)
		{
		...
		try
		{
			...
			}else
			{
				...
				if (this.jenisLap === "Daftar"){
					var sql = "select a.no_ptg,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.no_dokumen,a.keterangan,concat(a.kode_pp,' - ',c.nama) as nmpp, "+
						"a.no_pj,concat(a.catatan,' - ',d.keterangan) as pryk,f.nilai as npjr,d.nilai as npry,a.nilai as nptg "+
						"from ptg_m a inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
						"inner join kop_proyek_m d on a.catatan=d.no_proyek and a.kode_lokasi=d.kode_lokasi "+
						"inner join panjar_m f on a.no_pj=f.no_pj and a.kode_lokasi=f.kode_lokasi "+
						this.filter+
						" order by a.no_ptg ";
					this.scriptSqlCount = "select count(*) "+
						"from ptg_m a inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
						"inner join kop_proyek_m d on a.catatan=d.no_proyek and a.kode_lokasi=d.kode_lokasi "+
						"inner join panjar_m f on a.no_pj=f.no_pj and a.kode_lokasi=f.kode_lokasi "+this.filter;
					this.title = new server_util_arrayList({items:["No Bukti","Tanggal","No Dokumen","Keterangan","PP/Unit Kerja","No. Panjar","Proyek","Nilai Panjar","Nilai Proyek","Nilai Ptg"]});
					...
					var sql = "select a.no_ptg,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.no_dokumen,a.keterangan,a.kode_pp,c.nama as nmpp,a.catatan as pryk, "+
						"d.keterangan as ketpry,d.nilai as npry,a.no_pj,f.nilai as npjr,a.nik_buat,a.nik_setuju,g.nama as nama_pengaju,h.nama as nama_setuju, "+
						"g.jabatan as jabatan_buat,h.jabatan as jabatan_setuju,b.kode_akun,e.nama as nmakun,b.keterangan as ket,b.dc,b.nilai,a.nilai as nptg "+
						"from ptg_m a inner join ptg_j b on a.no_ptg=b.no_ptg and a.kode_lokasi=b.kode_lokasi "+
					...
					this.scriptSqlCount = "select count(*) "+
						...
						"inner join panjar_m f on a.no_pj=f.no_pj and a.kode_lokasi=f.kode_lokasi "+
						"inner join karyawan g on a.nik_buat=g.nik and a.kode_lokasi=g.kode_lokasi "+
						"inner join karyawan h on a.nik_setuju=h.nik and a.kode_lokasi=h.kode_lokasi "+this.filter;
			...
			systemAPI.alert("[app_kopeg_proyek_report_flPjPtg]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		if (this.jenisLap === "Transaksi")
			var header = "LAPORAN DETAIL PERTANGGUNGAN PANJAR PROYEK";
		else var header = "DAFTAR PERTANGGUNGAN PANJAR PROYEK";
	...
	convertResultToHTML: function(result,data){		
		...
		for (var r in table.rs.rows){
			...
			if (no_bill != line.no_ptg){
				first = true;
				no_bill = line.no_ptg;
				if (htmlHeader !== ""){
					html+="<tr align='right' bgcolor='#EEEEEE'><td class='isi_laporan' colspan='5'>Total Ptg Panjar Proyek</td>"+
						"<td class='isi_laporan'>"+floatToNilai(line.nptg)+"</td></tr>";
					html += "</table>";		 
			...
			if (first){
				...
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.no_ptg+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				...
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>No. Panjar</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.no_pj+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>Nilai Panjar</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+floatToNilai(line.npjr)+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>Proyek</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.pryk+" - "+line.ketpry+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				...
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>Pemegang Panjar</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.nik_buat+" - "+line.nama_pengaju+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>Disetujui Oleh</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.nik_setuju+" - "+line.nama_setuju+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "</table>";
			}
			urut+=1;
