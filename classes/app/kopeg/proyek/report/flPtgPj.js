window.app_kopeg_proyek_report_flPtgPj = function(owner)
{
	if (owner)
	{
		window.app_kopeg_proyek_report_flPtgPj.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_proyek_report_flPtgPj";
		this.maximize();
	...
	this.filterRep.setSGFilterRowTipe(this.sg1, 0,[0,1,2],["1234","123","123"]);
...
window.app_kopeg_proyek_report_flPtgPj.extend(window.portalui_childForm);
window.app_kopeg_proyek_report_flPtgPj.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row === 2)
		{
			...
			var sql2="select count(no_ptg) from ptg_m "+
					 ...
					 this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and")+
					 " ";
			this.filterRep.ListDataSGFilter(this, "Data Pertanggungan Panjar",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_ptg","keterangan"),"and",["No. Bukti","Keterangan"]);
		}
	},
	...
	mainButtonClick: function(sender)
	{
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_kopeg_rptPtgProyek";
		...
		switch (methodName)
		{
			case "preview" : 
				if (this.sg1.getCell(2,3) === "All"){
					this.viewer.preview("<br /><br /><br /><br /><br /><br /><br />"+result,true);
