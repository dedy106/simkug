window.app_kopeg_proyek_fPjajuk= function(owner){
	if (owner){
		window.app_kopeg_proyek_fPjajuk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_kopeg_proyek_fPjajuk";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this,"mainButtonClick","Form Pengajuan Panjar Kontrak: Koreksi",0);
		...
		this.cb_proyek = new portalui_saiCBBL(this);
		this.cb_proyek.setLeft(20);
		this.cb_proyek.setTop(232);
		this.cb_proyek.setWidth(185);
		this.cb_proyek.setLabelWidth(100);
		this.cb_proyek.setReadOnly(false);
		this.cb_proyek.setRightLabelVisible(true);
		this.cb_proyek.setCaption("Kontrak");
		this.cb_proyek.setText(""); 
		this.cb_proyek.setRightLabelCaption(" ");
		this.cb_proyek.setSQL("select no_proyek, no_dokumen from kop_proyek_m where kode_lokasi = '"+this.app._lokasi+"'",["no_proyek","no_dokumen"],true);
		
		this.ed_nilai = new portalui_saiLabelEdit(this);
		this.ed_nilai.setLeft(680);
		this.ed_nilai.setTop(232);
...
window.app_kopeg_proyek_fPjajuk.extend(window.portalui_childForm);
window.app_kopeg_proyek_fPjajuk.prototype.mainButtonClick = function(sender){
...
window.app_kopeg_proyek_fPjajuk.prototype.ubah = function(){	
	...
	if (this.standarLib.checkEmptyByTag(this, new Array("0"))){
		try{				
			...
			sql.add("insert into panjar_m (no_pj,no_kas,no_dokumen,tanggal,due_date,"+
					...
					"('"+this.nb+"','-','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDate()+"','"+this.dp_tgl2.getDate()+"','"+this.ed_desc.getText()+"','"+this.cb_proyek.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+",'"+this.cb_akun.getText()+"','"+this.cb_pemohon.getText()+"','"+this.cb_setuju.getText()+"','"+this.app._lokasi+"','"+this.cb_pp.getText()+
...
window.app_kopeg_proyek_fPjajuk.prototype.doModalResult = function(event, modalResult){
...
window.app_kopeg_proyek_fPjajuk.prototype.genClick = function(sender){
	try{
...
window.app_kopeg_proyek_fPjajuk.prototype.doSelect = function(sender, year, month, day){
	if (month < 10) month = "0"+month;
	this.ed_period.setText(year.toString()+month);
	this.loadMasterData();
};
window.app_kopeg_proyek_fPjajuk.prototype.doEditChange = function(sender){
...
window.app_kopeg_proyek_fPjajuk.prototype.showClick = function(sender){
		...
		if (this.cb_bukti != undefined) {
			try {
				...
				var line,data = this.dbLib.runSQL(" select a.no_dokumen, a.keterangan, a.kode_curr, a.kurs, a.nik_pengaju, a.nik_setuju, b.nama as nama_aju, c.nama as nama_setuju, "+
												  "        a.due_date,a.periode,a.akun_pj,d.nama as nama_akun,a.tanggal,a.catatan,zz.no_dokumen as nodok "+
												  ...
												  "                 inner join kop_proyek_m zz on a.catatan = zz.no_proyek and a.kode_lokasi = zz.kode_lokasi "+
				...
				if (data instanceof portalui_arrayMap){
					if (data.get(0) != undefined){									
						...
						this.cb_proyek.setText(line.get("catatan"));
						this.cb_proyek.setRightLabelCaption(line.get("nodok"));
					} 
				}
				this.sg1.clear(); 
...
window.app_kopeg_proyek_fPjajuk.prototype.FindBtnClick = function(sender, event){
	try{
...
window.app_kopeg_proyek_fPjajuk.prototype.doCellExit = function(sender, col, row) {
	try{
		switch(col){
			case 3 : 
			case 4 : 
...
window.app_kopeg_proyek_fPjajuk.prototype.doCellEnter = function(sender, col, row) {
	try{
		switch(col){
			case 2 : 
...
window.app_kopeg_proyek_fPjajuk.prototype.doFindBtnClick = function(sender, col, row) {
	try{
		switch(col){
			case 0 : 
...
window.app_kopeg_proyek_fPjajuk.prototype.doNilaiChange = function(){
	try{
		var totD = totC = nKB = 0; 
		for (var i = 0; i < this.sg1.rows.getLength();i++){
...
window.app_kopeg_proyek_fPjajuk.prototype.doSgChange= function(sender, col, row){
   try{
...
window.app_kopeg_proyek_fPjajuk.prototype.hitungGar = function(){
...
window.app_kopeg_proyek_fPjajuk.prototype.garClick = function(sender){
	try{
...
window.app_kopeg_proyek_fPjajuk.prototype.loadMasterData= function(){
...
window.app_kopeg_proyek_fPjajuk.prototype.doRequestReady = function(sender, methodName, result){
