window.app_kopeg_proyek_fBayark = function(owner)
{
	if (owner)
	{
		window.app_kopeg_proyek_fBayark.prototype.parent.constructor.call(this,owner);
		this.className  = "app_kopeg_proyek_fBayark";
		this.maximize();
		...
		{		    
			...
			var data = this.dbLib.getDataProvider("select a.kode_spro,a.flag,b.nama from spro a inner join masakun b on a.flag=b.kode_akun "+
			                                      "where a.kode_spro in ('PROADM') and a.kode_lokasi = '"+this.app._lokasi+"' "); //'PROIM0',
			eval("data = "+data+";");
			if (typeof data == "object"){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];												
					//if (line.kode_spro == "PROIM0") {this.akunIM = line.flag; this.namaIM = line.nama;}
					if (line.kode_spro == "PROADM") {this.akunAD = line.flag; this.namaAD = line.nama;}
				}
			}
			var prd = this.dbLib.getDataProvider("select distinct periode from kop_proyekbayar_m where kode_lokasi = '"+this.app._lokasi+"'",true);
...
window.app_kopeg_proyek_fBayark.extend(window.portalui_childForm);
window.app_kopeg_proyek_fBayark.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					if (parseFloat(this.perLama) < parseFloat(this.app._periode)) {
						...
						sql.add(" update kop_proyekbayar_m set no_link='"+this.e_kb.getText()+"',no_del = concat(no_bukti,'r') where no_bukti ='"+this.cb_nbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
						sql.add(" insert into kop_proyekbayar_m (no_bukti,no_dokumen,keterangan,tanggal,nilai,jenis,progress,akun_ar,periode,kode_lokasi,posted,kurs,kode_curr,kode_pp,no_del,no_link,nik_app,nik_user,tgl_input,nilai_pph,akun_pph,nilai_adm) "+
							    " select concat(no_bukti,'r'),no_dokumen,keterangan,'"+this.dp_d1.getDateString()+"',nilai,jenis,'X',akun_ar,'"+this.e_periode.getText()+"',kode_lokasi,'F',kurs,kode_curr,kode_pp,no_bukti,'-',nik_app,'"+this.app._userLog+"',now(),nilai_pph,akun_pph,nilai_adm "+
								" from kop_proyekbayar_m where no_bukti = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"'");
						sql.add(" insert into kop_proyekbayar_j (no_bukti,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input)"+
								...
								" from kop_proyekbayar_j where no_bukti = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");
						sql.add(" insert into kop_proyekbayar_d (no_bukti,no_ar,periode,akun_ar,nilai,nilai_pph,kode_lokasi,dc)"+
								" select concat(no_bukti,'r'),no_ar,'"+this.e_periode.getText()+"',akun_ar,nilai,nilai_pph,kode_lokasi,'C' "+ 
								" from kop_proyekbayar_d where no_bukti = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");						
						this.nb = this.e_kb.getText();
					}
					else{
						sql.add("delete from kop_proyekbayar_m where no_bukti='"+this.cb_nbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from kop_proyekbayar_j where no_bukti='"+this.cb_nbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from kop_proyekbayar_d where no_bukti ='"+this.cb_nbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");									
					...
					sql.add("insert into kop_proyekbayar_m(no_bukti,no_dokumen,keterangan,tanggal,nilai,jenis,progress,akun_ar,periode,kode_lokasi,posted,kurs,kode_curr,kode_pp,no_del,no_link,nik_app,nik_user,tgl_input,nilai_pph,akun_pph,nilai_adm) values  "+
							"('"+this.nb+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.dp_d1.getDateString()+"',"+parseNilai(this.e_kas.getText())+",'ARBYR','0','"+this.akunIM+"','"+this.e_periode.getText()+"','"+this.app._lokasi+"','F',1,'IDR','"+this.app._kodePP+"','-','-','"+this.cb_app.getText()+"','"+this.app._userLog+"',now(),"+parseNilai(this.e_pph.getText())+",'"+this.cb_pph.getText()+"',"+parseNilai(this.e_adm.getText())+")");
		...
		switch (event){
			...
			case "hapus" : 
				...
				try{
					...
					var sql = new server_util_arrayList();
					if (parseFloat(this.perLama) < parseFloat(this.app._periode)) {
						sql.add(" update kop_proyekbayar_m set no_del = concat(no_bukti,'r') where no_bukti ='"+this.cb_nbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
						sql.add(" insert into kop_proyekbayar_m (no_bukti,no_dokumen,keterangan,tanggal,nilai,jenis,progress,akun_ar,periode,kode_lokasi,posted,kurs,kode_curr,kode_pp,no_del,no_link,nik_app,nik_user,tgl_input,nilai_pph,akun_pph,nilai_adm) "+
							    " select concat(no_bukti,'r'),no_dokumen,keterangan,'"+this.dp_d1.getDateString()+"',nilai,jenis,'X',akun_ar,'"+this.e_periode.getText()+"',kode_lokasi,'F',kurs,kode_curr,kode_pp,no_bukti,'-',nik_app,'"+this.app._userLog+"',now(),nilai_pph,akun_pph,nilai_adm "+
								" from kop_proyekbayar_m where no_bukti = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"'");
						sql.add(" insert into kop_proyekbayar_j (no_bukti,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input)"+
	...
	doLoadData: function(sender){
		try{			
			if (this.cb_nbLama.getText() != "") {
				this.sg.clear(1);
				var data = this.dbLib.getDataProvider("select a.no_dokumen,a.keterangan,a.periode,a.posted,a.kode_pp,a.nik_app,a.nilai_pph,a.nilai_adm,a.akun_pph,a.tanggal,a.akun_ar as akun_im,aa.nama as nama_im,"+
				                                      "       b.nama as nama_app,ifnull(c.nama,'-') as nama_pph,d.nama as nama_pp,f.kode_cust, "+
													  "       f.no_ar,date_format(f.tanggal,'%d/%m/%Y')as tanggal,f.keterangan as ket,f.akun_ar,(f.nilai+f.nilai_ppn)-ifnull(y.totbayar,0) as sisa,j.nama as nama_akun,e.nilai,e.nilai_pph "+
		...
		try{
			if (sender == this.cb_nbLama) {   
			    this.standarLib.showListData(this, "Daftar Bukti",sender,undefined, 
											  "select no_bukti,no_dokumen from kop_proyekbayar_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and no_del='-' and progress = '0'", 
											  "select count(no_bukti)     from kop_proyekbayar_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and no_del='-' and progress = '0'", 
