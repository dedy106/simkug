window.app_kopeg_proyek_fSpb = function(owner){
	if (owner){
		window.app_kopeg_proyek_fSpb.prototype.parent.constructor.call(this,owner);
		this.className  = "app_kopeg_proyek_fSpb";
...
window.app_kopeg_proyek_fSpb.extend(window.portalui_childForm);
window.app_kopeg_proyek_fSpb.implement({
				...
				try{									
					...
					if (this.gridJurnal.getLength() > 0)
					{
						...
						{						
							...
							scr1 += "('"+this.e_nb.getText()+"','SPBPRY','"+this.app._lokasi+"','"+line.get("kode_akun")+"','"+line.get("kode_pp")+"','"+line.get("kode_drk")+
