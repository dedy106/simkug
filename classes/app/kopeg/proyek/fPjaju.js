window.app_kopeg_proyek_fPjaju= function(owner)
{
	if (owner)
	{
		window.app_kopeg_proyek_fPjaju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_kopeg_proyek_fPjaju";
		this.maximize();
		this.itemsValue = new portalui_arrayList();	
		this.app._mainForm.childFormConfig(this,"mainButtonClick","Form Pengajuan Panjar Kontrak: Input",0);
		...
		try
		{
			...
			this.cb_proyek.onBtnClick.set(this, "FindBtnClick");			
...
window.app_kopeg_proyek_fPjaju.extend(window.portalui_childForm);
window.app_kopeg_proyek_fPjaju.prototype.mainButtonClick = function(sender){
...
window.app_kopeg_proyek_fPjaju.prototype.simpan = function(){
	...
	if (this.standarLib.checkEmptyByTag(this, new Array("0"))){
		try{				
			...
			sql.add("insert into panjar_m (no_pj,no_kas,no_dokumen,tanggal,due_date,keterangan,catatan,kode_curr,kurs,akun_pj,nik_pengaju,nik_setuju,kode_lokasi,kode_pp,"+
					...
					"('"+this.ed_nb.getText()+"','-','"+this.ed_dok.getText()+"','"+this.dp_tgl1.getDate()+"','"+this.dp_tgl2.getDate()+"','"+this.ed_desc.getText()+"','"+this.cb_proyek.getText()+"','"+this.cb_curr.getText()+"',"+parseNilai(this.ed_kurs.getText())+",'"+this.cb_akun.getText()+"','"+this.cb_pemohon.getText()+"','"+this.cb_setuju.getText()+"','"+this.app._lokasi+"','"+this.cb_pp.getText()+
...
window.app_kopeg_proyek_fPjaju.prototype.doModalResult = function(event, modalResult){
...
window.app_kopeg_proyek_fPjaju.prototype.genClick = function(sender){
	try{
...
window.app_kopeg_proyek_fPjaju.prototype.doSelect = function(sender, year, month, day){
	if (month < 10)
		month = "0"+month;
	this.ed_period.setText(year.toString()+month);
	this.loadMasterData();
};
window.app_kopeg_proyek_fPjaju.prototype.doEditChange = function(sender){
...
window.app_kopeg_proyek_fPjaju.prototype.FindBtnClick = function(sender, event){
	try{
...
window.app_kopeg_proyek_fPjaju.prototype.doCellExit = function(sender, col, row) {
	try{
		switch(col){
			case 3 : 
			case 4 : 
...
window.app_kopeg_proyek_fPjaju.prototype.doCellEnter = function(sender, col, row) {
	try{
		switch(col){
			case 2 : 
...
window.app_kopeg_proyek_fPjaju.prototype.doFindBtnClick = function(sender, col, row) {
	try{
		switch(col){
			case 0 : 
...
window.app_kopeg_proyek_fPjaju.prototype.doNilaiChange = function(){
	try{
		var totD = totC = nKB = 0; 
		for (var i = 0; i < this.sg1.rows.getLength();i++){
		...
		alert("[app_kopeg_proyek_fPjaju]::doNilaiChange:"+e);
	}
};
window.app_kopeg_proyek_fPjaju.prototype.doSgChange= function(sender, col, row){
   try{
...
window.app_kopeg_proyek_fPjaju.prototype.hitungGar = function(){
...
window.app_kopeg_proyek_fPjaju.prototype.garClick = function(sender){
	try{
...
window.app_kopeg_proyek_fPjaju.prototype.loadMasterData= function(){
...
window.app_kopeg_proyek_fPjaju.prototype.doRequestReady = function(sender, methodName, result){
