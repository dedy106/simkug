window.app_kopeg_proyek_fSpbk = function(owner)
{
	if (owner)
	{
		window.app_kopeg_proyek_fSpbk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_kopeg_proyek_fSpbk";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form SPB Proyek: Koreksi", 0);	
		
		uses("portalui_saiCBB;portalui_checkBox;portalui_saiCBB;portalui_saiCBBL;portalui_saiEdit;portalui_datePicker;portalui_saiGrid;portalui_saiTable");
		...
		this.cb_proyek = new portalui_saiCBBL(this,{bound:[20,18,200,20],caption:"No Proyek",btnClick:[this,"doBtnClick"],change:[this,"doChange"],tag:1});
		...
		try{			
			...
			this.cb_proyek.setSQL("select no_proyek,no_dokumen from kop_proyek_m where kode_pp = '"+this.app._kodePP+"' and kode_lokasi = '"+this.app._lokasi+"'",["no_proyek","no_dokumen"],true);
			...
			var prd = this.dbLib.getDataProvider("select distinct periode from spb_m where kode_lokasi = '"+this.app._lokasi+"' and modul='SPB' and jenis = 'APPROYEK' and progress_fp = '0' order by periode desc",true);
...
window.app_kopeg_proyek_fSpbk.extend(window.portalui_childForm);
window.app_kopeg_proyek_fSpbk.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					if (parseFloat(this.perLama) < parseFloat(this.app._periode)) {
						...
						sql.add("delete from angg_r where no_bukti ='"+this.cb_nbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' and modul = 'SPBPRY'");			
					...
					sql.add("insert into spb_m (no_spb,no_dokumen,tanggal,due_date,akun_hutang,"+
							"keterangan,no_invoice,kode_curr,kurs,nik_buat,nik_setuju,kode_terima,kode_lokasi,kode_pp,"+
							...
							"','"+this.akunAP+"','"+this.e_desc.getText()+"','"+this.cb_proyek.getText()+"','IDR',1,'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.cb_vendor.getText()+"','"+this.app._lokasi+"','"+this.app._kodePP+
					...
					if (this.gridJurnal.getLength() > 0)
					{
						...
						{						
							...
							scr1 += "('"+this.nb+"','SPB','"+this.app._lokasi+"','"+line.get("kode_akun")+"','"+line.get("kode_pp")+"','"+line.get("kode_drk")+
	...
	doLoadData:function(sender){ 
		if (this.cb_nbLama.getText() != ""){
			var data = this.dbLib.getDataProvider("select a.no_dokumen,a.keterangan,a.kode_curr,a.kurs,a.nik_buat,a.nik_setuju,b.nama as nama_buat,c.nama as nama_setuju,a.due_date,a.kode_terima,d.nama as nama_terima,a.periode,a.posted,a.tanggal,a.akun_hutang,aa.nama as nama_akun,a.no_invoice,cc.no_dokumen as nodok,a.no_fpajak, "+
												  ...
												  "             inner join kop_proyek_m cc on a.no_invoice=cc.no_proyek and a.kode_lokasi=cc.kode_lokasi "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg2.appendData([line.kode_akun,line.nama_akun,line.ket,line.dc.toUpperCase(),floatToNilai(line.nilai),line.kode_pp,line.nama_pp,line.kode_drk,line.nama_drk]);
				}
				...
				this.cb_proyek.setText(line.no_invoice,line.nodok);
		...
		try{
			if (sender == this.cb_nbLama) {   
			    this.standarLib.showListData(this, "Daftar SPB",sender,undefined, 
											  "select no_spb,no_dokumen from spb_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and no_del='-' and progress = '0' and modul='SPB' and jenis='APPROYEK' and progress_fp = '0'", 
											  "select count(no_spb)     from spb_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and no_del='-' and progress = '0' and modul='SPB' and jenis='APPROYEK' and progress_fp = '0'", 
			...
			if (sender == this.cb_proyek) {   
			    this.standarLib.showListData(this, "Daftar Proyek",sender,undefined, 
											  ...
											  ["no_proyek","no_dokumen"],"and",["No Proyek","No Dokumen"],false);
			}
