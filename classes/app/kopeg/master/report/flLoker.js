window.app_kopeg_master_report_flLoker = function(owner)
{
	if (owner)
	{
		window.app_kopeg_master_report_flLoker.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_master_report_flLoker";
		this.maximize();
...
window.app_kopeg_master_report_flLoker.extend(window.portalui_childForm);
window.app_kopeg_master_report_flLoker.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2){
		    	...
		    	this.filter = this.filterRep.filterStr("kode_loker",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new portalui_arrayMap();
				var sql = "select kode_loker,fn_spasi(nama,level_spasi) as nama,initial,tipe "+
						"from kop_loker "+this.filter+" and kode_lokasi='"+this.app._lokasi+
						"' order by kode_loker ";
				this.scriptSqlCount = "select count(*) "+
						"from kop_loker "+this.filter+" and kode_lokasi='"+this.app._lokasi+"'";
				this.title = new server_util_arrayList({items:["Kode","Nama","Inisial","Tipe"]});
			...
			systemAPI.alert("[app_kopeg_master_report_flLoker]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>DATA LOKASI KERJA<br>";
		var d = new Date();
	  ...
	  switch(sender.getName())
	  {
	    ...
	    case "pdfBtn" :      
		  var html = new server_util_arrayList();
			...
			html.add("pdf");			
			html.add(new Date().valueOf()+"_loker");				
