...
//* 			SAI, PT											
//***********************************************************************************************
window.app_kopeg_fMenu = function(owner){
	if (owner){
		try{
			window.app_kopeg_fMenu.prototype.parent.constructor.call(this, owner);
			this.className = "app_kopeg_fMenu";
			...
			this.treev.setBound(20,68,700,this.height - 50);
...
window.app_kopeg_fMenu.extend(window.portalui_childForm);
window.app_kopeg_fMenu.implement({
	...
	doClick: function(sender){		
		this.dblib.getDataProviderA("select * from menu where kode_klp = '"+this.e0.getText()+"' order by rowindex",0,0);		
	},
	entriesClick: function(sender){
		try{
			uses("app_kopeg_fMenuDetail");	
			if (this.entryMenu !== undefined) this.entryMenu.free();
			this.entryMenu = new app_kopeg_fMenuDetail(this.app);
	...
	getTreeData: function(node){
		...
		if (node instanceof portalui_treeView){
			...
			result += "','"+(node.getLevel() - 1)+"','"+node.getKodeForm()+"','"+this.rowIndex+"','"+this.e0.getText()+"');";
			if (node.isHasChild())
				for (var i=0;i < node.childsIndex.length;i++)
				{
					child = system.getResource(node.childsIndex[i]);
