window.app_kopeg_hutang_fSpbpotk = function(owner)
{
	if (owner)
	{
		window.app_kopeg_hutang_fSpbpotk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_kopeg_hutang_fSpbpotk";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form SPB Potongan Gaji: Koreksi", 0);	
		
		uses("portalui_saiCBB;portalui_checkBox;portalui_saiCBBL;portalui_saiEdit;portalui_datePicker;portalui_saiGrid;portalui_saiTable");
		...
		this.cb_spb = new portalui_saiCBBL(this,{bound:[20,19,200,20],caption:"No SPB Potongan",tag:2});		
		...
		try{			
			...
			var prd = this.dbLib.getDataProvider("select distinct periode from spb_m where kode_lokasi = '"+this.app._lokasi+"' and modul='SPB' and jenis = 'APPOT' order by periode desc",true);
...
window.app_kopeg_hutang_fSpbpotk.extend(window.portalui_childForm);
window.app_kopeg_hutang_fSpbpotk.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					if (parseFloat(this.perLama) < parseFloat(this.app._periode)) {
						...
						sql.add("insert into kop_gajispb_d (no_spb,no_gaji,periode,kode_akun,nilai,kode_lokasi,dc) "+
								" select concat(no_spb,'r'),no_gaji,'"+this.e_periode.getText()+"',kode_akun,nilai,kode_lokasi,'C' "+ 
								" from kop_gajispb_d where no_spb = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"'");						
						...
						sql.add("delete from kop_gajispb_d where no_spb='"+this.cb_nbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from angg_r where no_bukti ='"+this.cb_nbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' and modul = 'SPBPOT'");			
					...
					for (var i=0; i < this.sg2.rows.getLength(); i++){
						if (this.sg2.rowValid(i) && this.sg2.cells(0,i)=="APP") {
							sql.add("insert into kop_gajispb_d (no_spb,no_gaji,periode,kode_akun,nilai,kode_lokasi,dc) values "+
								    "('"+this.nb+"','"+this.cb_spb.getText()+"','"+this.e_periode.getText()+"','"+this.sg2.cells(1,i)+"',"+parseNilai(this.sg2.cells(5,i))+",'"+this.app._lokasi+"','D')");
										
							scr1 = "insert into spb_j (no_spb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,"+
								"kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values ";
							scr1 += "('"+this.nb+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDate()+"',"+idx+",'"+this.sg2.getCell(1,i)+
					...
					if (this.gridJurnal.getLength() > 0)
					{
						...
						{						
							...
							scr1 += "('"+this.nb+"','SPBPOT','"+this.app._lokasi+"','"+line.get("kode_akun")+"','"+line.get("kode_pp")+"','"+line.get("kode_drk")+
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :
				...
				var cekData = "F";
				for (var i=0; i < this.sg2.rows.getLength(); i++){
					if (this.sg2.getCell(0,i) == "APP") cekData = "T";
				}
				if (cekData == "F"){
	...
	doLoadData: function(sender){
		try{			
			if (this.cb_nbLama.getText() != ""){
				...
				var data = this.dbLib.getDataProvider("select x.kode_akun,y.nama as nama_akun,x.keterangan as ket,x.dc,x.nilai-ifnull(c.totpakai,0) as sisa,x.kode_pp,ifnull(z.nama,'-') as nama_pp,x.kode_drk,ifnull(zz.nama,'-') as nama_drk,x.no_spb as nospb,aa.keterangan "+
												  "from spb_j x inner join kop_gajispb_d xx on x.kode_akun=xx.kode_akun and x.no_spb=xx.no_gaji and x.kode_lokasi=xx.kode_lokasi "+
												  "             inner join masakun y on x.kode_akun=y.kode_akun and x.kode_lokasi=y.kode_lokasi "+
												  "             inner join spb_m aa on x.no_spb=aa.no_spb and x.kode_lokasi=aa.kode_lokasi "+
												  ...
												  "                              from kop_gajispb_d where no_spb <> '"+this.cb_nbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"' group by no_gaji,kode_akun,kode_lokasi) c on c.kode_akun=x.kode_akun and c.no_gaji=x.no_spb and x.kode_lokasi=c.kode_lokasi "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg2.appendData(["APP",line.kode_akun,line.nama_akun,line.ket,line.dc.toUpperCase(),floatToNilai(line.sisa),line.kode_pp,line.nama_pp,line.kode_drk,line.nama_drk]);
					}
					this.cb_spb.setText(line.nospb,line.keterangan);
				}
				this.sg2.validasi();
			}
			else {
				system.alert(this,"SPB Lama tidak valid.","No SPB Lama harus dipilih.");
			}
		}catch(e){
			systemAPI.alert(e);
		}
...
alert("select no_spb,no_dokumen from spb_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and no_del='-' and progress = '0' and modul='SPB' and jenis='APPOT'");			
			    this.standarLib.showListData(this, "Daftar SPB",sender,undefined, 
											  "select no_spb,no_dokumen from spb_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and no_del='-' and progress = '0' and modul='SPB' and jenis='APPOT'", 
											  "select count(no_spb)     from spb_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and no_del='-' and progress = '0' and modul='SPB' and jenis='APPOT'", 
	...
	hitungGar: function(){
		...
		{
			line = this.gridJurnal.get(i);
			var baris,data = this.dbLib.runSQL("select fn_cekagg3('"+line.get("kode_pp")+"','"+this.app._lokasi+"','"+line.get("kode_akun")+"','"+line.get("kode_drk")+"','"+this.e_periode.getText()+"','"+this.cb_nbLama.getText()+"') as gar ");
