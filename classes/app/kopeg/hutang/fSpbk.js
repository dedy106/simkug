window.app_kopeg_hutang_fSpbk = function(owner)
{
	if (owner)
	{
		window.app_kopeg_hutang_fSpbk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_kopeg_hutang_fSpbk";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form SPB Hutang Umum: Koreksi", 0);	
		...
		try{			
			...
			var prd = this.dbLib.getDataProvider("select distinct periode from spb_m where kode_lokasi = '"+this.app._lokasi+"' and modul='SPB' and jenis = 'APUMUM' and progress_fp = '0' order by periode desc",true);
...
window.app_kopeg_hutang_fSpbk.extend(window.portalui_childForm);
window.app_kopeg_hutang_fSpbk.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					if (parseFloat(this.perLama) < parseFloat(this.app._periode)) {
						...
						sql.add("delete from angg_r where no_bukti ='"+this.cb_nbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' and modul = 'SPB'");			
	...
	doLoadData:function(sender){ 
		if (this.cb_nbLama.getText() != ""){
			var data = this.dbLib.getDataProvider("select a.no_dokumen,a.keterangan,a.kode_curr,a.kurs,a.nik_buat,a.nik_setuju,b.nama as nama_buat,c.nama as nama_setuju,a.due_date,a.kode_terima,d.nama as nama_terima,a.periode,a.posted,a.tanggal,a.akun_hutang,aa.nama as nama_akun,no_fpajak,  "+
		...
		try{
			if (sender == this.cb_nbLama) {   
			    this.standarLib.showListData(this, "Daftar SPB",sender,undefined, 
											  "select no_spb,no_dokumen from spb_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and no_del='-' and progress = '0' and modul='SPB' and jenis='APUMUM' and progress_fp='0' ", 
											  "select count(no_spb)     from spb_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and no_del='-' and progress = '0' and modul='SPB' and jenis='APUMUM' and progress_fp='0' ", 
