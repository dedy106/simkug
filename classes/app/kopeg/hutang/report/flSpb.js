window.app_kopeg_hutang_report_flSpb = function(owner)
{
	if (owner)
	{
		window.app_kopeg_hutang_report_flSpb.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_hutang_report_flSpb";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan SPB Hutang Umum", 2);
...
window.app_kopeg_hutang_report_flSpb.extend(window.portalui_childForm);
window.app_kopeg_hutang_report_flSpb.implement({
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,3) === "All"){
					...
					this.nama_report="server_report_kopeg_rptSpbH2";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.no_spb",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
					this.filter2="";
			...
			case "preview" : 
				if (this.sg1.getCell(2,3) === "All"){
					if (this.status){
						...
						this.report.preview("server_report_kopeg_rptSpbH2",this.filter,this.page,1, this.showFilter,this.app._lokasi,this.filter2);
