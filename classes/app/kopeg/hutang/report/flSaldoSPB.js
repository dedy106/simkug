window.app_kopeg_hutang_report_flSaldoSPB = function(owner)
{
	if (owner)
	{
		window.app_kopeg_hutang_report_flSaldoSPB.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_hutang_report_flSaldoSPB";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Daftar Saldo Hutang Umum", 2);
	...
	this.tanda="=";
	this.lokasi=this.app._lokasi;this.getPeriode="select distinct periode from spb_m where jenis = 'APUMUM' and kode_lokasi ='"+this.lokasi+"' order by periode desc";
...
window.app_kopeg_hutang_report_flSaldoSPB.extend(window.portalui_childForm);
window.app_kopeg_hutang_report_flSaldoSPB.implement({
		...
		try
		{
			if (sender === this.app._mainForm.bClear2){
				...
				var sql = "select b.kode_pp,b.nama as nama_pp,c.kode_vendor,c.nama as nama_vendor,a.no_spb,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.keterangan,a.akun_hutang, "+ 
						  ...
						  this.filter+"  and (case when a.periode <= '"+this.sg1.getCell(2,1)+"' then a.nilai-ifnull(y.bayarawal,0)-ifnull(z.rekonawal,0) else 0 end) > 0 and a.no_del = '-' order by b.kode_pp,a.akun_hutang,a.no_spb ";   
				this.scriptSqlCount = "select count(*) "+
						  ...
						  "           left outer join (select no_spb,kode_lokasi2, sum(case dc when 'D' then nilai else -nilai end) as rekonnow "+
						  "                            from kop_rekon_d where kode_lokasi2='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"' group by no_spb,kode_lokasi2) zz on zz.no_spb=a.no_spb and zz.kode_lokasi2=a.kode_lokasi "+
						  this.filter+"  and case when a.periode <= '"+this.sg1.getCell(2,1)+"' then a.nilai-ifnull(y.bayarawal,0)-ifnull(z.rekonawal,0) else 0 end > 0 and a.no_del = '-' ";   
			...
			systemAPI.alert("[app_kopeg_hutang_report_flSaldoSPB]::mainButtonClick:"+e);
		}
	},
