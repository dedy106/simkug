window.app_kopeg_hutang_report_flHB = function(owner)
{
	if (owner)
	{
		window.app_kopeg_hutang_report_flHB.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_hutang_report_flHB";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Hutang Bank", 2);
	...
	this.getPeriode="select distinct periode as periode from kop_ap_m where kode_lokasi ='"+this.lokasi+"' order by periode desc ";
...
window.app_kopeg_hutang_report_flHB.extend(window.portalui_childForm);
window.app_kopeg_hutang_report_flHB.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row === 2)
		{
			this.filterRep.ListDataSGFilter(this, "Data Hutang Bank",this.sg1, this.sg1.row, this.sg1.col,
									  "select no_ap,keterangan from kop_ap_m where kode_lokasi='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"'",
									  "select count(*) from kop_ap_m where kode_lokasi='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"'",
									  ["no_ap","keterangan"],"where",["Kode","Keterangan"]);
		}
	},
	doSelectCell: function(sender, col, row){
		...
		if (row === 1){
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_ap_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' order by periode desc ",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 3)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			...
			{ 
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_ap",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.jenisLap = this.sg1.getCell(2,3);
				var result  = new portalui_arrayMap();
				if (this.jenisLap === "Transaksi"){
					var sql = "select a.no_ap,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.no_dokumen,a.keterangan,c.nama as nmvendor,a.nilai,a.nilai_kjpe,a.nilai_kjpa, "+
							"b.kode_akun,d.nama as nmakun,b.keterangan as ket,b.kode_pp,b.dc,b.nilai as nilai2 "+
							"from kop_ap_m a inner join kop_ap_j b on a.no_ap=b.no_ap and a.kode_lokasi=b.kode_lokasi "+
					...
					this.scriptSqlCount = "select count(*) "+
							...
							"inner join masakun d on b.kode_akun=d.kode_akun and b.kode_lokasi=d.kode_lokasi "+
							this.filter;
					this.title = new server_util_arrayList({items:["Kode Akun","Nama Akun","Keterangan","Kode PP","DC","Nilai"]});
					this.widthTable = new server_util_arrayList({items:[80,150,200,80,50,100]});
					...
					var sql = "select a.no_ap,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.no_dokumen,a.keterangan,c.nama as nmvendor,a.nilai,a.nilai_kjpe,a.nilai_kjpa "+
							"from kop_ap_m a inner join vendor c on a.kode_vendor=c.kode_vendor and a.kode_lokasi=c.kode_lokasi "+
							this.filter+
							" order by a.no_ap ";
					this.scriptSqlCount = "select count(*) "+
							"from kop_ap_m a inner join vendor c on a.kode_vendor=c.kode_vendor and a.kode_lokasi=c.kode_lokasi "+
							this.filter;
					this.title = new server_util_arrayList({items:["Kode","Tanggal","No. Dokumen","Keterangan","Vendor","Nilai","Nilai KJPe","Nilai KJPa"]});
			...
			systemAPI.alert("[app_kopeg_hutang_report_flHB]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		if (this.jenisLap === "Transaksi")
			var header = "LAPORAN DETAIL HUTANG BANK";
		else var header = "LAPORAN HUTANG BANK";
	...
	convertResultToHTML: function(result,data){		
		...
		for (var r in table.rs.rows){
			...
			if (no_bill != line.no_ap){
				first = true;
				no_bill = line.no_ap;
				if (htmlHeader !== ""){
					html+="<tr align='right' bgcolor='#EEEEEE'><td class='isi_laporan' colspan='6'>Total Hutang</td>"+
			...
			if (first){
				...
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.no_ap+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				...
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.nmvendor+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				...
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>Nilai KJPe</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+floatToNilai(line.nilai_kjpe)+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>Nilai KJPa</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+floatToNilai(line.nilai_kjpa)+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "</table>";
			}
			urut+=1;
			...
			for (var c in line){
				if (c === "kode_akun" || c === "nmakun" || c === "ket" || c === "kode_pp" || c === "dc" || c === "nilai2"){
					if (c === "nilai2")
					...
					if (c === "dc")
						if (line[c] === "D") total+=parseInt(line["nilai2"]);
				}
			}
			//total+= parseInt(line.nilai2);
			html += "</tr>";
			first = false;
		}
		html+="<tr align='right' bgcolor='#EEEEEE'><td class='isi_laporan' colspan='6'>Total Hutang</td>"+
