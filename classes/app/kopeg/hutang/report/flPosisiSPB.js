window.app_kopeg_hutang_report_flPosisiSPB = function(owner)
{
	if (owner)
	{
		window.app_kopeg_hutang_report_flPosisiSPB.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_hutang_report_flPosisiSPB";
		this.maximize();
	...
	this.getPeriode="select distinct periode from spb_m where kode_lokasi ='"+this.lokasi+"' and jenis = 'APUMUM' order by periode desc ";
...
window.app_kopeg_hutang_report_flPosisiSPB.extend(window.portalui_childForm);
window.app_kopeg_hutang_report_flPosisiSPB.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			var sql1="select no_spb,keterangan from spb_m "+
					 ...
					 " and modul='SPB' and jenis='APUMUM' ";
	...
	doSelectCell: function(sender, col, row){
		try{
			...
			if (row === 1){
				...
				this.dbLib.setItemsFromSQL("select distinct periode from spb_m where jenis = 'APUMUM' and kode_lokasi = '"+this.sg1.getCell(2,0)+"' order by periode desc ",[this.sg1.columns.get(2).pickList]);
			}
		}catch(e){
			alert(e);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.sql =  "select a.kode_pp,f.nama as nama_pp,a.akun_hutang, g.nama, a.no_spb,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.keterangan,a.nilai,b.no_ver,date_format(c.tanggal,'%d/%m/%Y') as tgl_ver,d.no_kas,date_format(e.tanggal,'%d/%m/%Y') as tgl_kas,a.posted,a.no_del,a.no_link "+
							...
							"left join pp f on a.kode_pp=f.kode_pp and a.kode_lokasi=f.kode_lokasi "+this.filter+" and a.modul='SPB' and a.jenis='APUMUM' order by a.kode_pp,a.no_spb";
				this.scriptSqlCount =   "select count(a.no_spb) "+
										...
										"left join ver_m c on b.no_ver=c.no_ver and b.kode_lokasi=c.kode_lokasi "+
										"left join kas_d d on a.no_spb=d.no_bukti and a.kode_lokasi=d.kode_lokasi "+
										"left join kas_m e on d.no_kas=e.no_kas and d.kode_lokasi=e.kode_lokasi "+this.filter+"  and a.modul='SPB' and a.jenis='APUMUM' ";
		...
		{
			alert("[app_kopeg_hutang_report_flPosisiSPB]::mainButtonClick:"+e);
		}
	},
