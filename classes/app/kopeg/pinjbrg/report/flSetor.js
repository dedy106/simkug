window.app_kopeg_pinjbrg_report_flSetor = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjbrg_report_flSetor.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_pinjbrg_report_flSetor";
		this.maximize();
	...
	this.getPeriode="select distinct periode from kop_pbrgsetor_m where kode_lokasi ='"+this.lokasi+"'";
...
window.app_kopeg_pinjbrg_report_flSetor.extend(window.portalui_childForm);
window.app_kopeg_pinjbrg_report_flSetor.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row === 5)
		{
			this.filterRep.ListDataSGFilter(this, "Data Setoran",this.sg1, this.sg1.row, this.sg1.col,
										  "select a.no_setor,a.keterangan from kop_pbrgsetor_m a inner join kop_pbrgsetor_d b on a.no_setor=b.no_setor and a.kode_lokasi=b.kode_lokasi "+
										  "where a.kode_lokasi ='"+this.sg1.getCell(2,0)+"' and a.periode='"+this.sg1.getCell(2,1)+"' and b.no_angs like '%"+this.sg1.getCell(2,4)+"' ",
										  "select count(*) from kop_pbrgsetor_m a inner join kop_pbrgsetor_d b on a.no_setor=b.no_setor and a.kode_lokasi=b.kode_lokasi "+
										  "where a.kode_lokasi ='"+this.sg1.getCell(2,0)+"' and a.periode='"+this.sg1.getCell(2,1)+"' and b.no_angs like '%"+this.sg1.getCell(2,4)+"' ",
	...
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_pbrgsetor_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",[this.sg1.columns.get(2).pickList]);
		}
	},
	mainButtonClick: function(sender)
	{
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_kopeg_rptSetor2";
