window.app_kopeg_pinjbrg_report_flBrg = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjbrg_report_flBrg.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_pinjbrg_report_flBrg";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Data Barang", 2);
...
window.app_kopeg_pinjbrg_report_flBrg.extend(window.portalui_childForm);
window.app_kopeg_pinjbrg_report_flBrg.implement({
		...
		try{			
			...
			if (row === 1)
			{
				this.filterRep.ListDataSGFilter(this, "Data Barang",this.sg1, this.sg1.row, this.sg1.col,
									  "select kode_brg,nama from kop_brg where kode_lokasi='"+this.app._lokasi+"' and kode_klpbrg like '%"+this.sg1.getCell(2,0)+"'",
									  "select count(*) from kop_brg where kode_lokasi='"+this.app._lokasi+"' and kode_klpbrg like '%"+this.sg1.getCell(2,0)+"'",
									  ["kode_brg","nama"],"where",["Kode","Nama Barang"]);
			}
		}catch(e){
			systemAPI.alert(e);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2){
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_klpbrg",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.kode_brg",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new portalui_arrayMap();
				var sql = "select a.kode_brg,a.nama,b.nama,a.tipe,a.sat,a.harga "+
						"from kop_brg a inner join kop_brg_klp b on a.kode_klpbrg=b.kode_klpbrg and a.kode_lokasi=b.kode_lokasi "+
						this.filter+" and a.kode_lokasi='"+this.app._lokasi+
						"' order by a.kode_brg,b.kode_klpbrg ";
				this.scriptSqlCount = "select count(*) "+
						"from kop_brg a inner join kop_brg_klp b on a.kode_klpbrg=b.kode_klpbrg and a.kode_lokasi=b.kode_lokasi "+
						this.filter+" and a.kode_lokasi='"+this.app._lokasi+"'";
				this.title = new server_util_arrayList({items:["Kode","Nama","Klp Barang","Tipe","Satuan","Harga"]});
			...
			systemAPI.alert("[app_kopeg_pinjbrg_report_flBrg]::mainButtonClick:"+e);
		}
	},
