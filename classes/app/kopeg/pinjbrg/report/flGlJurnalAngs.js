// JavaScript Document
window.app_kopeg_pinjbrg_report_flGlJurnalAngs = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjbrg_report_flGlJurnalAngs.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_pinjbrg_report_flGlJurnalAngs";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kop_pbrgangs_m where jenis='PBRGANGS' and POSTED='T' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_kopeg_pinjbrg_report_flGlJurnalAngs.extend(window.portalui_childForm);
window.app_kopeg_pinjbrg_report_flGlJurnalAngs.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			...
			var sql1="select a.no_angs,a.keterangan from kop_pbrgangs_m a where a.jenis='PBRGANGS'  "+filter; 
			var sql2="select count(a.no_angs) from kop_pbrgangs_m a  where a.jenis='PBRGANGS' "+filter;
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_pbrgangs_m where jenis='PBRGANGS' and posted='T' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			this.nama_report="server_report_saku3_rptGlAngsPinjbrg";
