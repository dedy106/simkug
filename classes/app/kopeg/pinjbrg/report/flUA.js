window.app_kopeg_pinjbrg_report_flUA = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjbrg_report_flUA.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_pinjbrg_report_flUA";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Kuitansi Uang Muka dan Asuransi", 2);
	...
	this.getPeriode="select distinct periode from kop_pbrgangs_m where kode_lokasi ='"+this.lokasi+"'";
	...
	this.gridLib.SGEditData(this.sg1,4,[0,1,2],["No. Kontrak","All",""]);
	this.gridLib.SGEditData(this.sg1,5,[0,1,2],["Jenis Laporan","=","Kwitansi"]);
	this.pager = 1;
};
window.app_kopeg_pinjbrg_report_flUA.extend(window.portalui_childForm);
window.app_kopeg_pinjbrg_report_flUA.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row === 4)
		{
			this.filterRep.ListDataSGFilter(this, "Data Kontrak",this.sg1, this.sg1.row, this.sg1.col,
										  "select a.no_pbrg,a.keterangan from kop_pbrg_m a "+
										  "inner join kop_agg b on b.kode_agg=a.kode_agg and b.kode_lokasi=a.kode_lokasi "+
											...
											" and a.nilai_adm + a.nilai_um <> 0",
										  "select count(*) from kop_pbrg_m a "+
										  "inner join kop_agg b on b.kode_agg=a.kode_agg and b.kode_lokasi=a.kode_lokasi "+
											this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
											this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
											this.filterRep.filterStr("b.kode_loker",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
											this.filterRep.filterStr("a.kode_agg",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
											" and a.nilai_adm + a.nilai_um <> 0",
										  ["a.no_pbrg","a.keterangan"],"and",["No. Kontrak","Keterangan"]);
		}
	},
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_pbrgangs_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 5)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["Kwitansi","Daftar Per Loker"]);
		}
	},
	mainButtonClick: function(sender)
	{
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,5) === "Kwitansi"){
					...
					this.nama_report="server_report_kopeg_rptAngs4";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
