// JavaScript Document
window.app_kopeg_pinjbrg_report_flGlJurnalAkru = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjbrg_report_flGlJurnalAkru.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_pinjbrg_report_flGlJurnalAkru";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kop_pbrgakru_m where  POSTED='T' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_kopeg_pinjbrg_report_flGlJurnalAkru.extend(window.portalui_childForm);
window.app_kopeg_pinjbrg_report_flGlJurnalAkru.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			...
			var sql1="select a.no_akru,a.keterangan from kop_pbrgakru_m a   "+filter; 
			var sql2="select count(a.no_akru) from kop_pbrgakru_m a   "+filter;
			this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
			                                new Array("a.no_akru","a.keterangan"),"and",new Array("No Bukti","Keterangan"),"and");
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_pbrgakru_m where  posted='T' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			this.filter=this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
						...
						this.filterRep.filterStr("no_akru",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
			this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(1,1)+"/"+this.sg1.getCell(3,1)+"/"+this.sg1.getCell(2,0);
			this.nama_report="server_report_saku3_rptGlAkruPinjbrg";
