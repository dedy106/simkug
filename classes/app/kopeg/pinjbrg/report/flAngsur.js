window.app_kopeg_pinjbrg_report_flAngsur = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjbrg_report_flAngsur.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_pinjbrg_report_flAngsur";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Angsuran", 2);
...
window.app_kopeg_pinjbrg_report_flAngsur.extend(window.portalui_childForm);
window.app_kopeg_pinjbrg_report_flAngsur.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row === 4)
		{
			this.filterRep.ListDataSGFilter(this, "Data Kwitansi",this.sg1, this.sg1.row, this.sg1.col,
										  "select a.no_angs,a.keterangan from kop_pbrgangs_m a inner join kop_pbrgangs_d b on a.no_angs=b.no_angs and a.kode_lokasi=b.kode_lokasi "+
										  ...
										  "select count(*) from kop_pbrgangs_m a inner join kop_pbrgangs_d b on a.no_angs=b.no_angs and a.kode_lokasi=b.kode_lokasi "+
										  "inner join kop_pbrg_m c on b.no_pbrg=c.no_pbrg and b.kode_lokasi=c.kode_lokasi "+
										  "inner join kop_agg d on c.kode_agg=d.kode_agg and c.kode_lokasi=d.kode_lokasi "+
										  "where a.kode_lokasi='"+this.sg1.getCell(2,0)+"' and a.periode='"+this.sg1.getCell(2,1)+"' and d.kode_loker like '%"+this.sg1.getCell(2,2)+"' and c.kode_agg like '%"+this.sg1.getCell(2,3)+"' ",
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,5) === "Kwitansi"){
					this.nama_report="server_report_kopeg_rptAngs";
					...
					this.nama_report="server_report_kopeg_rptAngs2";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
