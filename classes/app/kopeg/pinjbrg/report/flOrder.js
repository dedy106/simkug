window.app_kopeg_pinjbrg_report_flOrder = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjbrg_report_flOrder.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_pinjbrg_report_flOrder";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Order Barang", 2);
	...
	this.getPeriode="select distinct periode from kop_jual_m where kode_lokasi ='"+this.lokasi+"'";
...
window.app_kopeg_pinjbrg_report_flOrder.extend(window.portalui_childForm);
window.app_kopeg_pinjbrg_report_flOrder.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row === 2)
		{
			this.standar.ListDataSGFilter(this, "Data Order Barang",this.sg1, this.sg1.row, this.sg1.col,
									  "select no_jual, keterangan from kop_jual_m "+
	...
	doSelectCell: function(sender, col, row){
		...
		if (row === 1){
			this.dbLib.setItemsFromSQL("select distinct periode from kop_jual_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 3)
		{
		...
		try
		{
			...
			}else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_jual",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new portalui_arrayMap();
				this.jenisLap = this.sg1.getCell(2,3);
				var sql = "select a.no_jual,date_format(a.tanggal,'%d/%m/%Y') as tgl,a.no_dokumen,a.keterangan,concat(a.kode_agg,'-',c.nama) as nmagg,concat(a.nik_buat,'-',d.nama) as nmbuat,a.nilai_um,a.nilai ";
				if (this.jenisLap === "Transaksi")
					sql+=",b.kode_brg,e.nama as nmbrg,e.tipe,e.sat,e.harga,b.jumlah,e.harga*b.jumlah as tot ";
				sql+="from kop_jual_m a inner join kop_jual_d b on a.no_jual=b.no_jual and a.kode_lokasi=b.kode_lokasi "+
					"inner join kop_agg c on a.kode_agg=c.kode_agg and a.kode_lokasi=b.kode_lokasi "+
					"inner join karyawan d on a.nik_buat=d.nik and a.kode_lokasi=d.kode_lokasi "+
					"inner join kop_brg e on b.kode_brg=e.kode_brg and b.kode_lokasi=e.kode_lokasi "+this.filter+
					" order by a.no_jual ";
				this.scriptSqlCount = "select count(*) "+
					"from kop_jual_m a inner join kop_jual_d b on a.no_jual=b.no_jual and a.kode_lokasi=b.kode_lokasi "+
					"inner join kop_agg c on a.kode_agg=c.kode_agg and a.kode_lokasi=b.kode_lokasi "+
					"inner join karyawan d on a.nik_buat=d.nik and a.kode_lokasi=d.kode_lokasi "+
					"inner join kop_brg e on b.kode_brg=e.kode_brg and b.kode_lokasi=e.kode_lokasi "+this.filter;
				if (this.jenisLap === "Daftar"){
					this.title = new server_util_arrayList({items:["No Bukti","Tanggal","No Dokumen","Keterangan","Nasabah","Pembuat","Uang Muka","Total Harga"]});
					...
					this.title = new server_util_arrayList({items:["Kode Barang","Nama Barang","Tipe","Satuan","Harga Satuan","Jumlah","Sub Total"]});
					this.widthTable = new server_util_arrayList({items:[100,120,100,70,100,60,100]});
			...
			systemAPI.alert("[app_kopeg_pinjbrg_report_flOrder]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		if (this.jenisLap === "Transaksi")
			var header = "LAPORAN ORDER BARANG";
		else var header = "DAFTAR ORDER BARANG";
	...
	convertResultToHTML: function(result,data){		
		...
		for (var r in table.rs.rows){
			...
			if (no_bill != line.no_jual){
				first = true;
				no_bill = line.no_jual;
				if (htmlHeader !== ""){
					html+="<tr align='right' bgcolor='#EEEEEE'><td class='isi_laporan' colspan='6'>Total</td>"+
						"<td class='isi_laporan'>"+floatToNilai(tot)+"</td>"+
			...
			if (first){
				...
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.no_jual+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				...
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.nmagg+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>Pembuat</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+line.nmbuat+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>Uang Muka</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>"+floatToNilai(line.nilai_um)+"</td>";
				htmlHeader += "</tr>";
				htmlHeader += "<tr>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>Total Harga</td>";
				htmlHeader += "<td height='20' class='isi_laporan' valign='top'>:</td>";
			...
			html += "<tr><td height='20' class='isi_laporan' valign='top'>"+urut+".</td>";
				for (var c in line){
					if (c === "kode_brg" || c === "nmbrg" || c === "tipe" || c === "sat" || c === "harga" || c === "jumlah" || c === "tot"){
						if (c === "harga" || c === "jumlah" || c === "tot")
