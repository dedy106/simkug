window.app_kopeg_pinjbrg_report_flPosisiSpb = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjbrg_report_flPosisiSpb.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_pinjbrg_report_flPosisiSpb";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Posisi Surat Perintah Bayar", 2);
	...
	this.getPeriode="select distinct periode from spb_m where kode_lokasi ='"+this.lokasi+"' and jenis = 'PBRG' order by periode desc ";
...
window.app_kopeg_pinjbrg_report_flPosisiSpb.extend(window.portalui_childForm);
window.app_kopeg_pinjbrg_report_flPosisiSpb.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==0)
	{
	...
	if (row == 3)
	{
		var sql1="select no_spb,keterangan from spb_m "+
				 ...
				 " and modul='KP.SPB' and jenis='PBRG' ";
...
window.app_kopeg_pinjbrg_report_flPosisiSpb.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3),new  Array("3","3","13","13"));
	...
	if (row === 1){
		...
		this.dbLib.setItemsFromSQL("select distinct periode from spb_m where jenis = 'PBRG' and kode_lokasi = '"+this.sg1.getCell(2,0)+"' order by periode desc ",[this.sg1.columns.get(2).pickList]);
	}
};
window.app_kopeg_pinjbrg_report_flPosisiSpb.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.sql =  "select a.kode_pp,f.nama as nama_pp,a.no_spb,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.keterangan,a.nilai,b.no_ver,date_format(c.tanggal,'%d/%m/%Y') as tgl_ver,d.no_kas,date_format(e.tanggal,'%d/%m/%Y') as tgl_kas,a.posted,a.no_del,a.no_link "+
						"from spb_m a "+
						...
						"left join pp f on a.kode_pp=f.kode_pp and a.kode_lokasi=f.kode_lokasi "+this.filter+" and a.modul='KP.SPB' and a.jenis='PBRG' order by a.kode_pp,a.no_spb";
			this.scriptSqlCount =   "select count(a.no_spb) "+
									"from spb_m a "+
									"left join ver_d b on a.no_spb=b.no_bukti and a.kode_lokasi=b.kode_lokasi "+
									"left join ver_m c on b.no_ver=c.no_ver and b.kode_lokasi=c.kode_lokasi "+
									"left join kas_d d on a.no_spb=d.no_bukti and a.kode_lokasi=d.kode_lokasi "+
									"left join kas_m e on d.no_kas=e.no_kas and d.kode_lokasi=e.kode_lokasi "+this.filter+"  and a.modul='KP.SPB' and a.jenis='PBRG' ";
	...
	{
		alert("[app_kopeg_pinjbrg_report_flPosisiSpb]::mainButtonClick:"+e);
	}
};
window.app_kopeg_pinjbrg_report_flPosisiSpb.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_kopeg_pinjbrg_report_flPosisiSpb.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_kopeg_pinjbrg_report_flPosisiSpb.prototype.previewReport = function(dthtml)
{
	var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN POSISI SPB<br>";			
	var d = new Date();
...
window.app_kopeg_pinjbrg_report_flPosisiSpb.prototype.doCloseReportClick = function(sender)
{
...
window.app_kopeg_pinjbrg_report_flPosisiSpb.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_kopeg_pinjbrg_report_flPosisiSpb.prototype.doRowPerPageChange = function(sender, rowperpage)
{
