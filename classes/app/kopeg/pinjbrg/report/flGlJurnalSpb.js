// JavaScript Document
window.app_kopeg_pinjbrg_report_flGlJurnalSpb = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjbrg_report_flGlJurnalSpb.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_pinjbrg_report_flGlJurnalSpb";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from spb_m where  modul='KP.SPB' and jenis='PBRG' and POSTED='T' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_kopeg_pinjbrg_report_flGlJurnalSpb.extend(window.portalui_childForm);
window.app_kopeg_pinjbrg_report_flGlJurnalSpb.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			...
			var sql1="select a.no_spb,a.keterangan from spb_m a where a.modul='KP.SPB' and a.jenis='PBRG'  "+filter; 
			var sql2="select count(a.no_spb) from spb_m a where a.modul='KP.SPB' and a.jenis='PBRG' "+filter;
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from spb_m where modul='KP.SPB' and jenis='PBRG' and posted='T' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			this.nama_report="server_report_saku3_rptGlSpbPinjbrg";
