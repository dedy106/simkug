window.app_kopeg_pinjbrg_report_flBilling = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjbrg_report_flBilling.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_pinjbrg_report_flBilling";
		this.maximize();
	...
	this.getPeriode="select distinct periode from kop_pbrgbill_m where kode_lokasi ='"+this.lokasi+"'";
...
window.app_kopeg_pinjbrg_report_flBilling.extend(window.portalui_childForm);
window.app_kopeg_pinjbrg_report_flBilling.implement({
	...
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_pbrgbill_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 4)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,4) === "Per Kontrak"){
					this.nama_report="server_report_kopeg_rptBill";
					...
					this.nama_report="server_report_kopeg_rptBill2";
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("e.kode_loker",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
