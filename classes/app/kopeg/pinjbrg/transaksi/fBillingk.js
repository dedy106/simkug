window.app_kopeg_pinjbrg_transaksi_fBillingk = function(owner){
	if (owner){
		try{		
			window.app_kopeg_pinjbrg_transaksi_fBillingk.prototype.parent.constructor.call(this, owner);
			this.className  = "app_kopeg_pinjbrg_transaksi_fBillingk";						
			this.maximize();
			owner.childFormConfig(this, "mainButtonClick","Form Akrual Jasa Kredit Barang : Koreksi", 0);			
			//------------------------ login data ------------------------	
			...
			this.p1 = new portalui_panel(this,{bound:[20,36,900,310],caption:"Daftar Kredit untuk diAkru"});
			this.sg1 = new portalui_saiGrid(this.p1,{bound:[1,20,898,290],colCount:14,
				...
				colTitle:["No Kontrak","Kd Nasabah","Nama Nasabah","Tanggal","Keterangan","Nilai Kredit","Jk Waktu","Persen","Cicil Ke","Nilai Pokok",
			...
			var prd = this.dbLib.getDataProvider("select distinct periode from kop_pbrgbill_m where kode_lokasi = '"+this.app._lokasi+"'",true);
			...
			systemAPI.alert("[app_kopeg_pinjbrg_transaksi_fBillingk]::oncreate lib : ",e);
		}
	}
};
window.app_kopeg_pinjbrg_transaksi_fBillingk.extend(window.portalui_childForm);
window.app_kopeg_pinjbrg_transaksi_fBillingk.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					if (parseFloat(this.perLama) < parseFloat(this.app._periode)) {
						...
						sql.add(" update kop_pbrgbill_m set no_link='"+this.eBukti.getText()+"',no_del = concat(no_bill,'r') where no_bill ='"+this.cbbNbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
						sql.add(" insert into kop_pbrgbill_m (no_bill,no_dokumen,keterangan,tanggal,nilai,periode,kode_pp,kode_lokasi,nik_app,nik_user,tgl_input,posted,kode_curr,kurs,no_del,no_link,no_ref,akun_pdpt) "+
								...
								" from kop_pbrgbill_m where no_bill = '"+this.cbbNbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"'");																		
						sql.add(" insert into kop_pbrgbill_j (no_bill,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input)"+
								...
								" from kop_pbrgbill_j where no_bill = '"+this.cbbNbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");
						sql.add("insert into kop_pbrgbill_d (no_bill,no_pbrg,no_kontrak,kode_lokasi,cicilan_ke,npokok,nbunga,akun_pjasa,akun_pdpt,dc,no_del)"+
								" select concat(no_bill,'r'),no_pbrg,no_kontrak,kode_lokasi,cicilan_ke,npokok,nbunga,akun_pjasa,akun_pdpt,'C','BATAL' "+ 
								" from kop_pbrgbill_d where no_bill = '"+this.cbbNbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"'");
						sql.add(" update kop_pbrg_sch set status_gen='0', no_bill='-' where no_bill='"+this.cbbNbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						this.nb = this.eBukti.getText();
					}
					else{
						sql.add(" delete from kop_pbrgbill_m where no_bill='"+this.cbbNbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add(" delete from kop_pbrgbill_j where no_bill='"+this.cbbNbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add(" delete from kop_pbrgbill_d where no_bill='"+this.cbbNbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add(" update kop_pbrg_sch set status_gen='0', no_bill='-' where no_bill='"+this.cbbNbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add(scr1);
					sql.add("insert into kop_pbrgbill_d(no_bill,no_pbrg,no_kontrak,kode_lokasi,cicilan_ke,npokok,nbunga,akun_pjasa,akun_pdpt,dc,no_del) "+
							"select '"+this.nb+"',a.no_pbrg,a.no_kontrak,a.kode_lokasi,g.cicilan_ke, "+
					...
					sql.add("update kop_pbrg_sch a, kop_pbrgbill_d b set a.status_gen='1', a.no_bill='"+this.nb+
					        "' where a.no_pbrg=b.no_pbrg and a.no_kontrak=b.no_kontrak and a.cicilan_ke=b.cicilan_ke and a.kode_lokasi=b.kode_lokasi and b.kode_lokasi='"+this.app._lokasi+"' and b.no_bill='"+this.nb+"'");					
		...
		switch (event){
			...
			case "hapus" :
				...
				try{	
					...
					if (parseFloat(this.perLama) < parseFloat(this.app._periode)) {
						sql.add(" update kop_pbrgbill_m set no_del = concat(no_bill,'r') where no_bill ='"+this.cbbNbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
						sql.add(" insert into kop_pbrgbill_m (no_bill,no_dokumen,keterangan,tanggal,nilai,periode,kode_pp,kode_lokasi,nik_app,nik_user,tgl_input,posted,kode_curr,kurs,no_del,no_link,no_ref,akun_pdpt) "+
							    " select concat(no_bill,'r'),no_dokumen,keterangan,'"+this.dTgl.getDateString()+"',nilai,'"+this.ePeriode.getText()+"',kode_pp,kode_lokasi,nik_app,'"+this.app._userLog+"',now(),'F',kode_curr,kurs,no_bill,'-','-',akun_pdpt "+
								" from kop_pbrgbill_m where no_bill = '"+this.cbbNbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"'");																		
						sql.add(" insert into kop_pbrgbill_j (no_bill,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input)"+
		...
		try{			
			if (this.cbbNbLama.getText() != ""){
				...
				var data = this.dbLib.getDataProvider("select m.kode_loker,m.nama as nama_loker,a.akun_pjasa,n.nama as nama_akun,sum(a.nilai) as totnilai,"+
													  ...
													  "from kop_pbrg_m a inner join kop_agg b on a.kode_agg=b.kode_agg and a.kode_lokasi=b.kode_lokasi "+
													  "                  inner join kop_pbrgbill_d c on c.no_kontrak=a.no_kontrak and c.no_pbrg=a.no_pbrg and a.kode_lokasi=c.kode_lokasi "+
													  "                  inner join kop_pbrgbill_m d on c.no_bill=d.no_bill and d.kode_lokasi=c.kode_lokasi "+
	...
	FindBtnClick: function(sender){
		if (sender == this.cbbNbLama) {   
			this.standarLib.showListData(this, "Daftar Bukti Akru",sender,undefined, 
										  "select no_bill, keterangan  from kop_pbrgbill_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cbbPerLama.getText()+"' and no_del='-'", 
										  "select count(no_bill)       from kop_pbrgbill_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cbbPerLama.getText()+"' and no_del='-'",
	...
	doDblClick: function(sender, col, row){
		if (this.sg2.getCell(0,row) != "") {
			var data = this.dbLib.getDataProvider("select a.no_kontrak,b.kode_agg,b.nama as nama_agg,date_format(g.tgl_angs,'%d/%m/%Y')as tanggal,a.keterangan,a.nilai,(a.nilai+ifnull(bb.nbill,0)-ifnull(aa.nangs,0)) as baki, "+
												  ...
												  "from kop_pbrg_m a inner join kop_agg b on a.kode_agg=b.kode_agg and a.kode_lokasi=b.kode_lokasi "+
												  "                  inner join kop_pbrgbill_d c on c.no_kontrak=a.no_kontrak and c.no_pbrg=a.no_pbrg and a.kode_lokasi=c.kode_lokasi "+
												  "					 inner join kop_pbrg_sch g on g.no_kontrak=a.no_kontrak and a.no_pbrg=g.no_pbrg and a.kode_lokasi=g.kode_lokasi and g.no_bill=c.no_bill "+
												  "                  left outer join (select x.no_kontrak,x.no_pbrg,x.kode_lokasi,sum(case x.dc when 'D' then x.npokok+x.nbunga else -(x.npokok+x.nbunga) end) as nangs "+
												  "                                   from kop_pbrgangs_d x "+
												  "                                   group by x.no_kontrak,x.no_pbrg,x.kode_lokasi) aa "+
												  "                                   on aa.no_kontrak=a.no_kontrak and aa.no_pbrg=a.no_pbrg and aa.kode_lokasi=a.kode_lokasi  "+ 
