window.app_kopeg_pinjbrg_transaksi_fBillingBatal = function(owner){
	if (owner){
		try{		
			window.app_kopeg_pinjbrg_transaksi_fBillingBatal.prototype.parent.constructor.call(this, owner);
			this.className  = "app_kopeg_pinjbrg_transaksi_fBillingBatal";						
			owner.childFormConfig(this, "mainButtonClick","Form Pembatalan Akrual Jasa Kredit Barang: Input", 0);			
			this.maximize();
			//------------------------ login data ------------------------	
			...
			systemAPI.alert("[app_kopeg_pinjbrg_transaksi_fBillingBatal]::oncreate lib : ",e);
		}
	}
};
window.app_kopeg_pinjbrg_transaksi_fBillingBatal.extend(window.portalui_childForm);
window.app_kopeg_pinjbrg_transaksi_fBillingBatal.implement({
		...
		try{				
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					var idx=0;
					var d="insert into kop_pbrgbill_j (no_bill,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values ";		
					for (var i=0;i< this.sg1.getRowCount();i++){
						if (this.sg1.cells(0,i) == "BATAL") {
							sql.add("update kop_pbrg_sch set status_gen='0',no_bill='-' where cicilan_ke ="+parseNilai(this.sg1.cells(3,i))+" and no_kontrak='"+this.cbbKontrak.getText()+"' and no_pbrg='"+this.cbbKontrak.dataFromList[1]+"' and no_bill='"+this.sg1.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"'");							
							sql.add("update kop_pbrgbill_d set no_del='"+this.eBukti.getText()+"' where cicilan_ke ="+parseNilai(this.sg1.cells(3,i))+" and no_kontrak='"+this.cbbKontrak.getText()+"' and no_pbrg='"+this.cbbKontrak.dataFromList[1]+"' and no_bill='"+this.sg1.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"'");							
							sql.add("insert into kop_pbrgbill_d (no_bill,no_pbrg,no_kontrak,kode_lokasi,cicilan_ke,npokok,nbunga,akun_pjasa,akun_pdpt,dc,no_del) values "+
							...
							if (idx > 0) d+= ",";
							idx++;
							d += "('"+this.eBukti.getText()+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.sg1.cells(6,i)+"','"+this.eKet.getText()+"','C',"+parseNilai(this.sg1.cells(8,i))+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBRG','ARJS','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
							d+= ",";
							idx++;
							d += "('"+this.eBukti.getText()+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.sg1.cells(7,i)+"','"+this.eKet.getText()+"','D',"+parseNilai(this.sg1.cells(8,i))+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBRG','PDPT','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
						}
					}
					sql.add(d);					
	...
	doLoadData: function(sender){
		if (this.cbbKontrak.getText() != "" && (this.ePeriode.getText() != "")){
			var data = this.dbLib.getDataProvider("select a.no_bill,c.keterangan,a.cicilan_ke,date_format(c.tanggal,'%Y%m') as periode,date_format(c.tanggal,'%d/%m/%Y') as tanggal,case a.dc when 'D' then a.nbunga else -a.nbunga end as nbunga,a.akun_pjasa,a.akun_pdpt "+
			                                      "from kop_pbrgbill_d a inner join kop_pbrgbill_m c on a.no_bill=c.no_bill and a.kode_lokasi=c.kode_lokasi "+
