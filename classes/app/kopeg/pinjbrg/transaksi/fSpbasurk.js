window.app_kopeg_pinjbrg_transaksi_fSpbasurk = function(owner){
	if (owner){
		try{		
			window.app_kopeg_pinjbrg_transaksi_fSpbasurk.prototype.parent.constructor.call(this, owner);
			this.className  = "app_kopeg_pinjbrg_transaksi_fSpbasurk";
			...
			this.cbbVendor = new portalui_saiCBBL(this,{bound:[20,17,200,20],caption:"Asuradur", btnClick:[this,"FindBtnClick"],tag:1});
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('PBRGPF','PBRGSP') and kode_lokasi = '"+this.app._lokasi+"'");
			eval("data = "+data+";");
			if (typeof data == "object"){
				...
				for (var i in data.rs.rows){
					...
					if (line.kode_spro == "PBRGPF") this.akunFee = line.flag;
				}
			}
			var prd = this.dbLib.getDataProvider("select distinct periode from spb_m where modul = 'KP.SPB' and jenis = 'PBRG_AS' and kode_lokasi = '"+this.app._lokasi+"'",true);
			...
			systemAPI.alert("[app_kopeg_pinjbrg_transaksi_fSpbasurk]::oncreate lib : ",e);
		}
	}
};
window.app_kopeg_pinjbrg_transaksi_fSpbasurk.extend(window.portalui_childForm);
window.app_kopeg_pinjbrg_transaksi_fSpbasurk.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					if (parseFloat(this.perLama) < parseFloat(this.app._periode)) {
						...
						sql.add(" update kop_pbrg_m set no_spbasur = '-' where no_spbasur='"+this.cbbNbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					for (var i=0; i <= this.sg1.getRowCount(); i++){
						if (this.sg1.cells(0,i) == "APP") {
							sql.add("update kop_pbrg_m set no_spbasur='"+this.nb+"' where no_pbrg='"+this.sg1.cells(1,i)+"' and no_kontrak = '"+this.sg1.cells(2,i)+"' and kode_lokasi = '"+this.app._lokasi+"'");
						}
					}
	...
	doLoadData: function(sender){
		try{			
			if (this.cbbNbLama.getText() != ""){
				var data = this.dbLib.getDataProvider("select a.no_pbrg,a.no_kontrak,e.kode_agg,e.nama as nama_agg,a.nilai_asur,b.kode_akun,  "+
				           ...
				           "from kop_pbrg_m a inner join kop_pbrg_spb c on c.no_kontrak=a.no_kontrak and a.kode_lokasi=c.kode_lokasi  "+
				...
				if (typeof data == "object"){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData(["APP",line.no_pbrg,line.no_kontrak,line.kode_agg,line.nama_agg,floatToNilai(line.nilai_asur),line.kode_akun]);
					}
					this.sg1.validasi();
				...
				system.alert(this,"No SPB Lama tidak valid.","Bukti Lama harus diisi.");
			}
		}catch(e){
			systemAPI.alert(e);
		}
	...
	FindBtnClick: function(sender){
		if (sender == this.cbbNbLama) {   
			this.standarLib.showListData(this, "Daftar Bukti SPB",sender,undefined, 
										  "select no_spb, keterangan  from spb_m where modul = 'KP.SPB' and jenis = 'PBRG_AS' and progress= '0' and kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cbbPerLama.getText()+"' and no_del='-'", 
										  "select count(no_spb)       from spb_m where modul = 'KP.SPB' and jenis = 'PBRG_AS' and progress= '0' and kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cbbPerLama.getText()+"' and no_del='-'",
