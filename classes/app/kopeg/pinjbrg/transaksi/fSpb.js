window.app_kopeg_pinjbrg_transaksi_fSpb = function(owner){
	if (owner){
		try{		
			window.app_kopeg_pinjbrg_transaksi_fSpb.prototype.parent.constructor.call(this, owner);
			this.className  = "app_kopeg_pinjbrg_transaksi_fSpb";
			owner.childFormConfig(this, "mainButtonClick","Form SPB Vendor: Input", 0);
			this.maximize();
			//------------------------ login data ------------------------	
			...
			this.cbbVendor = new portalui_saiCBBL(this,{bound:[20,17,200,20],caption:"Vendor", btnClick:[this,"FindBtnClick"]});
			...
			systemAPI.alert("[app_kopeg_pinjbrg_transaksi_fSpb]::oncreate lib : ",e);
		}
	}
};
window.app_kopeg_pinjbrg_transaksi_fSpb.extend(window.portalui_childForm);
window.app_kopeg_pinjbrg_transaksi_fSpb.implement({
				...
				try{									
					...
					for (var i=0; i <= this.sg1.getRowCount(); i++){
						if (this.sg1.cells(0,i) == "APP") {
							sql.add("update kop_jual_d set no_spb='"+this.eSPB.getText()+"',status ='2' where no_jual='"+this.sg1.cells(11,i)+"' and kode_brg = '"+this.sg1.cells(5,i)+"' and kode_vendor='"+this.cbbVendor.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
							sql.add("update kop_pbrg_m set progress='1' where progress = '0' and no_pbrg='"+this.sg1.cells(1,i)+"' and no_kontrak = '"+this.sg1.cells(2,i)+"' and kode_lokasi = '"+this.app._lokasi+"'");
						}
					}
	...
	try{			
			if (this.cbbVendor.getText() != ""){
				var data = this.dbLib.getDataProvider("select a.no_pbrg,a.no_kontrak,e.kode_agg,e.nama as nama_agg,d.kode_brg,d.nama as nama_brg,c.harga_kont as harga,c.jumlah_kont as jumlah,c.harga_kont*c.jumlah_kont as subttl,f.akun_hpp,c.no_jual "+
						   ...
						   "where c.kode_vendor = '"+this.cbbVendor.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' and c.status='1' ");
				eval("data = "+data+";");
				if (typeof data == "object"){
					var line;
					this.sg1.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData(["INPROG",line.no_pbrg,line.no_kontrak,line.kode_agg,line.nama_agg,line.kode_brg,line.nama_brg,floatToNilai(line.harga),floatToNilai(line.jumlah),floatToNilai(line.subttl),line.akun_hpp,line.no_jual]);
					}
					this.sg1.validasi();
				}
			}
