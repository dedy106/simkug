window.app_kopeg_pinjbrg_transaksi_fAngsurKolk = function(owner){
	if (owner){
		try{		
			window.app_kopeg_pinjbrg_transaksi_fAngsurKolk.prototype.parent.constructor.call(this, owner);
			this.className  = "app_kopeg_pinjbrg_transaksi_fAngsurKolk";						
			owner.childFormConfig(this, "mainButtonClick","Form Angsuran Kredit Kolektif : Koreksi", 0);			
			this.maximize();
			//------------------------ login data ------------------------	
			...
			var prd = this.dbLib.getDataProvider("select distinct periode from kop_pbrgangs_m where jenis = 'PBRGKOL' and kode_lokasi = '"+this.app._lokasi+"'",true);
			...
			systemAPI.alert("[app_kopeg_pinjbrg_transaksi_fAngsurKolk]::oncreate lib : ",e);
		}
	}
};
window.app_kopeg_pinjbrg_transaksi_fAngsurKolk.extend(window.portalui_childForm);
window.app_kopeg_pinjbrg_transaksi_fAngsurKolk.implement({
		...
		try{
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					d += "('"+this.nb+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.akunIM+"','"+this.eKet.getText()+"','D',"+nPiuIM+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBRG','AR','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
					if (nilaiToFloat(this.eFeeNilai.getText()) != 0) {
						d+= ",";
						idx++;
						d += "('"+this.nb+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.akunBF+"','"+this.eKet.getText()+"','D',"+parseNilai(this.eFeeNilai.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBRG','BYFEE','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
					}
					if (nilaiToFloat(this.eSls.getText()) < 0) {
						d+= ",";
						idx++;
						var vSls = nilaiToFloat(this.eSls.getText()) * -1;
						d += "('"+this.nb+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.akunOE+"','"+this.eKet.getText()+"','D',"+vSls+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBRG','SLS_ANGS','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
					} else {
						if (nilaiToFloat(this.eSls.getText()) > 0) {
						d+= ",";
						idx++;
						var vSls = nilaiToFloat(this.eSls.getText());
						d += "('"+this.nb+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.akunOI+"','"+this.eKet.getText()+"','C',"+vSls+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBRG','SLS_ANGS','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
						}
					}
					this.createJurnal();
					var line = undefined;
					...
					for (var i in this.gridJurnal.objList){
						...
						d+= "('"+this.nb+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+line.get("kode_akun")+"','"+this.eKet.getText()+"','C',"+parseFloat(line.get("nilai"))+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBRG','"+line.get("kode_drk")+"','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
						idx++;
					}
					sql.add(d);					
	...
	doLoadData: function(sender){
		try {
			if (this.cbbNbLama.getText() != ""){
				var data = this.dbLib.getDataProvider("select b.no_kontrak,b.kode_agg,c.nama,a.cicilan_ke,date_format(ab.tanggal,'%d/%m/%Y') as tgl_angs,a.npokok,a.nbunga,(a.npokok+a.nbunga) as ntagih,b.akun_piutang,b.akun_pjasa,b.no_pbrg, "+
													  "       e.tanggal as tgl_angs,e.akun_ar,e.no_spb,f.kode_loker,f.nama as nama_loker,e.posted,e.periode,e.no_dokumen,e.keterangan,e.nik_app,g.nama as nama_app,e.p_fee,e.nilai_fee,e.nilai_sls,e.no_spb, e.nilai, d.npokok + d.nbunga as nangs "+
													  ...
													  "                      inner join kop_pbrgangs_d d on a.no_kontrak=d.no_kontrak and a.no_pbrg=d.no_pbrg and a.kode_lokasi=d.kode_lokasi "+
													  "                      inner join kop_pbrgangs_m e on d.no_angs=e.no_angs and e.kode_lokasi=d.kode_lokasi "+
													  ...
													  "                                     from kop_pbrgangs_d where no_angs<>'"+this.cbbNbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"' group by no_kontrak,no_pbrg,kode_lokasi,date_format(tanggal,'%Y%m')) aa "+
				...
				if (typeof data == "object"){
					var line;
					line = data.rs.rows[0];
					this.sg1.data = data.rs.rows;
					this.sg1.onChange.set(this,undefined);
					this.sg1.onNilaiChange.set(this,undefined);					
					this.sg1.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];				
						this.sg1.appendData([line.no_kontrak,line.kode_agg,line.nama,line.cicilan_ke,line.tgl_angs,floatToNilai(line.npokok),floatToNilai(line.nbunga),floatToNilai(line.ntagih),floatToNilai(line.nangs),line.akun_piutang,line.akun_pjasa,line.no_pbrg]);
					}
					this.sg1.onNilaiChange.set(this,"doSgChange");
					this.sg1.validasi();					
	...
	doLoadPage: function(page){
		...
		if (finish > this.sg1.data.length) finish = this.sg1.data.length;
		for (var i=start ; i < finish; i++){
			line = this.sg1.data[i];				
			this.sg1.appendData([line.no_kontrak,line.kode_agg,line.nama,line.cicilan_ke,line.tgl_angs,floatToNilai(line.npokok),floatToNilai(line.nbunga),floatToNilai(line.ntagih),floatToNilai(line.nangs),line.akun_piutang,line.akun_pjasa,line.no_pbrg]);
		}
	},
	FindBtnClick: function(sender){
		...
		if (sender == this.cbbNbLama) {   
			this.standarLib.showListData(this, "Daftar Bukti Angsuran",sender,undefined, 
										  "select no_angs, keterangan  from kop_pbrgangs_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cbbPerLama.getText()+"' and no_del='-' and jenis='PBRGKOL' and progress='0'", 
										  "select count(no_angs)       from kop_pbrgangs_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cbbPerLama.getText()+"' and no_del='-' and jenis='PBRGKOL' and progress='0'",
