window.app_kopeg_pinjbrg_transaksi_fAngsurBatal = function(owner){
	if (owner){
		try{		
			window.app_kopeg_pinjbrg_transaksi_fAngsurBatal.prototype.parent.constructor.call(this, owner);
			this.className  = "app_kopeg_pinjbrg_transaksi_fAngsurBatal";						
			owner.childFormConfig(this, "mainButtonClick","Form Pembatalan Angsuran Kredit Barang: Input", 0);			
			this.maximize();
			//------------------------ login data ------------------------	
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag,value1 from spro where kode_spro in ('PBRGCD') and kode_lokasi = '"+this.app._lokasi+"' ");
			eval("data = "+data+";");
			if (typeof data == "object"){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];												
					if (line.kode_spro == "PBRGCD") this.akunCD = line.flag;
				}
			}
			...
			systemAPI.alert("[app_kopeg_pinjbrg_transaksi_fAngsurBatal]::oncreate lib : ",e);
		}
	}
};
window.app_kopeg_pinjbrg_transaksi_fAngsurBatal.extend(window.portalui_childForm);
window.app_kopeg_pinjbrg_transaksi_fAngsurBatal.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into kop_pbrgangs_m(no_angs,no_dokumen,keterangan,tanggal,nilai,nilai_depo,nilai_udp,nilai_sls,jenis,progress,akun_ar,periode,kode_lokasi,posted,kurs,kode_curr,kode_pp,no_del,no_link,nik_app,nik_user,tgl_input,p_fee,nilai_fee,no_spb,nilai_beban) values  "+
							"('"+this.eBukti.getText()+"','"+this.eDok.getText()+"','"+this.eKet.getText()+"','"+this.dTgl.getDateString()+"',"+nilai+",0,0,0,'PBRGBTL','1','"+this.akunCD+"','"+this.ePeriode.getText()+"','"+this.app._lokasi+"','F',1,'IDR','"+this.app._kodePP+"','BATAL','-','"+this.cbbPenerima.getText()+"','"+this.app._userLog+"',now(),0,0,'-',0)");
					...
					d += "('"+this.eBukti.getText()+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.akunCD+"','"+this.eKet.getText()+"','C',"+nilai+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBRG','CD','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
					for (var i=0;i< this.sg1.getRowCount();i++){
						if (this.sg1.cells(0,i) == "BATAL") {
							sql.add("update kop_pbrgangs_d set no_del='"+this.eBukti.getText()+"' where no_kontrak='"+this.cbbKontrak.getText()+"' and no_pbrg='"+this.cbbKontrak.dataFromList[1]+"' and no_angs='"+this.sg1.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"'");							
							d+= ",";
							idx++;
							d += "('"+this.eBukti.getText()+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.sg1.cells(6,i)+"','"+this.eKet.getText()+"','D',"+parseNilai(this.sg1.cells(8,i))+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBRG','ARPK','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
							d+= ",";
							idx++;
							d += "('"+this.eBukti.getText()+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.sg1.cells(7,i)+"','"+this.eKet.getText()+"','D',"+parseNilai(this.sg1.cells(9,i))+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBRG','ARJS','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
						}
					}
					sql.add("insert into kop_depo(no_depo,tanggal,keterangan,dc,nilai,kode_agg,modul,jenis,periode,kode_lokasi,kode_pp,nik_app,no_del,no_link,kurs,kode_curr,nik_user,tgl_input) values "+
							"('"+this.eBukti.getText()+"','"+this.dTgl.getDateString()+"','"+this.eKet.getText()+"','D',"+nilai+",'"+this.cbbAgg.getText()+"','PBRG','ANGS_BTL','"+this.ePeriode.getText()+"','"+this.app._lokasi+"','"+this.app._kodePP+"','"+this.cbbPenerima.getText()+"','-','-',1,'IDR','"+this.app._userLog+"',now())");
					sql.add(d);					
	...
	doLoadData: function(sender){
		if (this.cbbKontrak.getText() != "" && (this.ePeriode.getText() != "")){
			var data = this.dbLib.getDataProvider("select a.no_angs,c.jenis,c.keterangan,date_format(a.tanggal,'%Y%m') as periode,date_format(a.tanggal,'%d/%m/%Y') as tanggal,case a.dc when 'D' then a.npokok else -npokok end as npokok,case a.dc when 'D' then a.nbunga else -a.nbunga end as nbunga,b.akun_piutang,b.akun_pjasa "+
			                                      "from kop_pbrgangs_d a inner join kop_pbrgangs_m c on a.no_angs=c.no_angs and a.kode_lokasi=c.kode_lokasi "+												  
												  "						 inner join kop_pbrg_m b on a.no_kontrak=b.no_kontrak and a.no_pbrg=b.no_pbrg and a.kode_lokasi=b.kode_lokasi "+ 
												  "where a.no_del='-' and a.dc<>'C' and a.no_kontrak='"+this.cbbKontrak.getText()+"' and a.no_pbrg='"+this.cbbKontrak.dataFromList[1]+"' and a.kode_lokasi = '"+this.app._lokasi+"' "+
												  "order by periode"); 
			eval("data = "+data+";");
			var periode = ""; 
			if (typeof data == "object"){
