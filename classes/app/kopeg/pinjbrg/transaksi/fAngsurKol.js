window.app_kopeg_pinjbrg_transaksi_fAngsurKol = function(owner){
	if (owner){
		try{		
			window.app_kopeg_pinjbrg_transaksi_fAngsurKol.prototype.parent.constructor.call(this, owner);
			this.className  = "app_kopeg_pinjbrg_transaksi_fAngsurKol";						
			owner.childFormConfig(this, "mainButtonClick","Form Angsuran Kredit Kolektif : Input", 0);			
			this.maximize();
			//------------------------ login data ------------------------	
			uses("portalui_saiCBB;portalui_datePicker;portalui_radioButton;portalui_uploader;portalui_datePicker;portalui_saiGrid;portalui_sgNavigator;server_report_report;portalui_reportViewer");
			uses("uploader",true);
			...
			this.cStatus = new portalui_saiCB(this,{
				bound:[20,19,200,20],
				caption:"Status Bayar",
				items:["AUTODEBET","CASH"],
				tag:2
			});			
			this.cbbLoker = new portalui_saiCBBL(this, {
				bound: [20, 28, 200, 20],
				caption: "Loker",
			...
			this.i_load = new portalui_imageButton(this,{bound:[220,28,20,20],hint:"Tampil Data",image:"icon/"+system.getThemes()+"/refresh.png",click:[this,"doLoadData"]});
			...
			this.cbbPerLama = new portalui_saiCB(this,{bound:[720,29,200,20],caption:"Periode SPB/TAK",mustCheck: false, tag:9, change:[this,"doChange"]});
			...
			this.cbbSpb = new portalui_saiCBB(this,{bound:[720,30,200,20],caption:"No SPB/TAK Pot.", btnClick:[this,"FindBtnClick"], tag:9,btnRefreshClick:[this,"doLoadDataSPB"]});
			...
			this.bLoadInv = new portalui_button(this,{bound:[240,32,100,18],caption:"Upload tak Match",click:[this,"doVisible"],visible:false});	
			this.bHitung = new portalui_button(this,{bound:[545,32,80,18],caption:"Hitung Load",click:[this,"doMatching"]});				
			this.bInvalid = new portalui_button(this,{bound:[345,32,100,18],caption:"Tagihan Tak Match",click:[this,"doVisible"],visible:false});	
			...
			this.sg1 = new portalui_saiGrid(this.p1,{bound:[1,20,898,265],colCount:12,
			    colTitle:["No Kontrak","Kode Agg","Nasabah","Cicilan Ke","Tgl Angs","Nilai Pokok","Nilai Jasa","Nilai Tagihan","Nilai Angsur","AR Pokok","AR Jasa","No Kredit"],
			...
			this.sgn = new portalui_sgNavigator(this.p1,{bound:[1,288,895,25],grid:this.sg1, buttonStyle:bsViewExt, pager:[this,"doPager"]});
			...
			this.p4.sg = new portalui_saiGrid(this.p4,{bound:[0,20,895,265],colCount:5,tag:9,
						...
						readOnly:true,
						defaultRow:1});				
			this.p4.sgn = new portalui_sgNavigator(this.p4,{bound:[1,288,895,25],grid:this.p4.sg, buttonStyle:bsView, pager:[this,"doPager"]});
			...
			this.p5.sg = new portalui_saiGrid(this.p5,{bound:[0,20,895,265],colCount:5,tag:9,
						...
						readOnly:true,
						defaultRow:1});				
			this.p5.sgn = new portalui_sgNavigator(this.p5,{bound:[1,288,895,25],grid:this.p4.sg, buttonStyle:bsView, pager:[this,"doPager"]});
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag,value1 from spro where kode_spro in ('PBRGIM','PBRGOI','PBRGOE','PBRGNE','PBRGFE') and kode_lokasi = '"+this.app._lokasi+"' ");
			eval("data = "+data+";");
			if (typeof data == "object"){
				...
				for (var i in data.rs.rows){
					...
					if (line.kode_spro == "PBRGOE") this.akunOE = line.flag;
					if (line.kode_spro == "PBRGFE") this.akunBF = line.flag;
					if (line.kode_spro == "PBRGNE") this.nilaiOE = parseFloat(line.value1);
				}
			}
			...
			//this.cbbSpb.setSQL("select no_spb,keterangan,'SPB' as jenis from spb_m where periode = '"+this.app._periode+"' and kode_lokasi = '"+this.app._lokasi+"' union select no_ju,keterangan,'TAK' as jenis from ju_m where kode_lokasi = '"+this.app._lokasi+"' and jenis='TAK' and periode ='"+this.app._periode+"'",["no_spb","keterangan"],false,["No SPB","Keterangan"],"and","Daftar SPB/TAK", true);
		}catch(e){
			systemAPI.alert("[app_kopeg_pinjbrg_transaksi_fAngsurKol]::oncreate lib : ",e);
		}
	}
};
window.app_kopeg_pinjbrg_transaksi_fAngsurKol.extend(window.portalui_childForm);
window.app_kopeg_pinjbrg_transaksi_fAngsurKol.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into kop_pbrgangs_m(no_angs,no_dokumen,keterangan,tanggal,nilai,nilai_depo,nilai_udp,nilai_sls,jenis,progress,akun_ar,periode,kode_lokasi,posted,kurs,kode_curr,kode_pp,no_del,no_link,nik_app,nik_user,tgl_input,p_fee,nilai_fee,no_spb,nilai_beban) values  "+
							"('"+this.eBukti.getText()+"','"+this.eDok.getText()+"','"+this.eKet.getText()+"','"+this.dTgl.getDateString()+"',"+nPiuIM+",0,0,"+parseNilai(this.eSls.getText())+",'PBRGKOL','"+vProg+"','"+this.akunIM+"','"+this.ePeriode.getText()+"','"+this.app._lokasi+"','F',1,'IDR','"+this.app._kodePP+"','-','-','"+this.cbbPenerima.getText()+"','"+this.app._userLog+"',now(),"+parseNilai(this.eFee.getText())+","+parseNilai(this.eFeeNilai.getText())+",'"+noSPB+"',0)");
					var z = 0;
					var vPokok = vJasa = 0,line;
					var d="insert into kop_pbrgangs_d (no_angs,no_pbrg,no_kontrak,tanggal,akun_ar,npokok,nbunga,kode_lokasi,dc,no_del) values ";
					//for (var i=0;i < this.sg1.getRowCount();i++){
					for (var i in this.dataKontrak.rs.rows){
						line = this.dataKontrak.rs.rows[i];
						if (parseFloat(line.angsuran) != 0){//this.sg1.rowValid(i) && this.sg1.cells(8,i) != "0"
							if (z > 0) d+= ",";
							...
							//d += "('"+this.eBukti.getText()+"','"+this.sg1.cells(11,i)+"','"+this.sg1.cells(0,i)+"','"+this.dTgl.getDateString()+"','"+this.akunIM+"',"+vPokok+","+vJasa+",'"+this.app._lokasi+"','D')";
							d += "('"+this.eBukti.getText()+"','"+line.no_pbrg+"','"+line.no_kontrak+"','"+this.dTgl.getDateString()+"','"+this.akunIM+"',"+vPokok+","+vJasa+",'"+this.app._lokasi+"','D','-')";
							z++;
						}
					}
					sql.add(d);						
					...
					d += "('"+this.eBukti.getText()+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.akunIM+"','"+this.eKet.getText()+"','D',"+nPiuIM+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBRG','AR','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
					if (nilaiToFloat(this.eFeeNilai.getText()) != 0) {
						d+= ",";
						idx++;
						d += "('"+this.eBukti.getText()+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.akunBF+"','"+this.eKet.getText()+"','D',"+parseNilai(this.eFeeNilai.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBRG','BYFEE','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
					}
					if (nilaiToFloat(this.eSls.getText()) < 0) {
						d+= ",";
						idx++;
						var vSls = nilaiToFloat(this.eSls.getText()) * -1;
						d += "('"+this.eBukti.getText()+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.akunOE+"','"+this.eKet.getText()+"','D',"+vSls+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBRG','SLS_ANGS','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
					} else {
						if (nilaiToFloat(this.eSls.getText()) > 0) {
						d+= ",";
						idx++;
						var vSls = nilaiToFloat(this.eSls.getText());
						d += "('"+this.eBukti.getText()+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+this.akunOI+"','"+this.eKet.getText()+"','C',"+vSls+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBRG','SLS_ANGS','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
						}
					}
					this.createJurnal();
					var line = undefined;
					...
					for (var i in this.gridJurnal.objList){
						...
						d+= "('"+this.eBukti.getText()+"','"+this.eDok.getText()+"','"+this.dTgl.getDateString()+"',"+idx+",'"+line.get("kode_akun")+"','"+this.eKet.getText()+"','C',"+parseFloat(line.get("nilai"))+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBRG','"+line.get("kode_drk")+"','"+this.ePeriode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
						idx++;
					}
					sql.add(d);					
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk) {					
					...
					this.eAngs.clear(),this.eTagihan.clear(), this.eFeeNilai.clear(), this.eFee.clear(), this.eTotPokok.clear(), this.eTotJasa.clear();
					this.sg1.clear(1);
					this.sg2.setTag(9);
					this.sg2.clear(1);					
				}
	...
	doChange: function(sender){
		...
		if (sender == this.cbbSpb){
			//this.cbbSpb.setSQL("select no_spb,keterangan,'SPB' as jenis from spb_m where periode = '"+this.cbbSpb.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' union select no_ju,keterangan,'TAK' as jenis from ju_m where kode_lokasi = '"+this.app._lokasi+"' and jenis='TAK' and periode ='"+this.cbbSpb.getText()+"'",["no_spb","keterangan"],false,["No SPB","Keterangan"],"and","Daftar SPB/TAK", true);
		}
	},
	...
	doLoadData: function(sender){
		if (this.cbbLoker.getText() != "" && this.ePeriode.getText() != ""){
			...
			var data = this.dbLib.getDataProvider("select b.no_kontrak,b.kode_agg,c.nama,a.cicilan_ke,date_format(ab.tanggal,'%d/%m/%Y') as tgl_angs,a.npokok,a.nbunga,(a.npokok+a.nbunga) as ntagih,b.akun_piutang,b.akun_pjasa,b.no_pbrg,0 as angsuran, ab.no_bill "+
												  ...
												  "                      left outer join (select no_kontrak,no_pbrg,kode_lokasi,date_format(tanggal,'%Y%m') as per, sum(case dc when 'D' then npokok+nbunga else -(npokok+nbunga) end) as nangs,sum(case dc when 'D' then npokok else -npokok end) as angspokok, sum(case dc when 'D' then nbunga else -nbunga end) as angsbunga "+
												  ...
												  "where b.status_bayar='"+this.cStatus.getText().substr(0,1)+"' and ab.no_del='-' and c.kode_lokasi = '"+this.app._lokasi+"' "+filterLoker+" and b.status_aktif='1' and date_format(ab.tanggal,'%Y%m') "+fltperiode+" '"+this.ePeriode.getText()+"' and aa.per is null "+
												  "order by c.kode_agg,b.no_kontrak,a.cicilan_ke",true);						
			if (typeof data != "string"){
				var line;
				this.dataKontrak = data;
				/*
				line = data.rs.rows[0];
				this.sg1.clear();
				for (var i in data.rs.rows){
					line = data.rs.rows[i];				
					this.sg1.appendData([line.no_kontrak,line.kode_agg,line.nama,line.cicilan_ke,line.tgl_angs,floatToNilai(line.npokok),floatToNilai(line.nbunga),floatToNilai(line.ntagih),floatToNilai(line.ntagih),line.akun_piutang,line.akun_pjasa,line.no_pbrg]);
				}
				this.sg1.validasi();
				* */
				var tot = 0;
				for (var i in data.rs.rows){					
					tot += parseFloat(data.rs.rows[i].ntagih);
				}
	...
	doChangeCell: function(sender, col, row){		
		try{		
			if (col == 8) {				
				this.dataKontrak.rs.rows[(this.page - 1)* 30+row].angsuran = nilaiToFloat(this.sg1.cells(8,row));			
				this.sg1.validasi();
			}
		}catch(e){
			systemAPI.alert(e);
	...
	createJurnal : function() {
		var row,line,dtJurnal = new portalui_arrayMap();
		var nemu = false;
		var nreal,ix,dtJrnl = 0;
		//[line.no_kontrak,line.kode_agg,line.nama,line.cicilan_ke,line.tgl_angs,floatToNilai(line.npokok),floatToNilai(line.nbunga),floatToNilai(line.ntagih),floatToNilai(line.angsuran),line.akun_piutang,line.akun_pjasa,line.no_pbrg];
		for (var i in this.dataKontrak.rs.rows){
			line = this.dataKontrak.rs.rows[i];
	...
	selectPage: function(page){
		...
		if (finish > this.dataKontrak.rs.rows.length) finish = this.dataKontrak.rs.rows.length;
		for (var i = start;i < finish;i++){
			line = this.dataKontrak.rs.rows[i];			
			this.sg1.appendData([line.no_kontrak,line.kode_agg,line.nama,line.cicilan_ke,line.tgl_angs,floatToNilai(line.npokok),floatToNilai(line.nbunga),floatToNilai(line.ntagih),floatToNilai(line.angsuran),line.akun_piutang,line.akun_pjasa,line.no_pbrg]);
		}
	...
	doAfterLoad: function(sender, result, data, filename){
		try{
			if (result) {
				...
				//this.eAngs.setText(floatToNilai(tot));
	...
	doMatching: function(sender){
		try{
			...
			if (this.dataUpload instanceof portalui_arrayMap) {
				...
				var total = 0, Agg = [], tot1 = 0, tot2 =0, tot3 = 0, tot4 = 0,saldo = 0, pokok, bunga;
				for (var i in this.dataKontrak.rs.rows){
					data = this.dataKontrak.rs.rows[i];
					angs = this.dataUpload.get(data.kode_agg); 					
					this.dataKontrak.rs.rows[i].angsuran = 0;
					if (angs){
						/*if (angs.nilai_angs - angs.terpakai < parseFloat(data.ntagih)){							
						}else{
							this.dataKontrak.rs.rows[i].angsuran = parseFloat(data.ntagih);
							angs.terpakai += parseFloat(data.ntagih);							
							this.dataUpload.set(data.kode_agg,angs);
							total += parseFloat(data.ntagih);
						}*/						
						...
						this.dataKontrak.rs.rows[i].angsuran = pokok + bunga;						
						tot4 += parseFloat(pokok + bunga);
						this.dataUpload.set(data.kode_agg,angs);
					}else {
						dataInvalid.set(data.kode_agg, {kode_agg:data.kode_agg,ntagih:data.ntagih, nbayar:0, nama:data.nama, kontrak:data.no_pbrg });
					}		
				}												
				this.eAngs.setText(floatToNilai(tot4));
				this.eTotPokok.setText(floatToNilai(tot1));
