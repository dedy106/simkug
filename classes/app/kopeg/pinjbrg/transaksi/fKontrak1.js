window.app_kopeg_pinjbrg_transaksi_fKontrak = function(owner){
	if (owner){
		try{		
			window.app_kopeg_pinjbrg_transaksi_fKontrak.prototype.parent.constructor.call(this, owner);
			this.className  = "app_kopeg_pinjbrg_transaksi_fKontrak";						
			owner.childFormConfig(this, "mainButtonClick","Kontrak Kredit Barang: Input", 0);
			this.maximize();
			//------------------------ login data ------------------------	
			...
			//load sawal
			//this.bSawalSch = new portalui_button(this,{bound:[20,23,80,18],caption:"Sawal Sch", click:[this,"doSawal"]}); //liat dorequestready
			//this.bSawalAngsur = new portalui_button(this,{bound:[120,23,80,18],caption:"Sawal Angs", click:[this,"doSawalAngs"]}); //brg - uang
			...
			systemAPI.alert("[app_kopeg_pinjbrg_transaksi_fKontrak]::oncreate lib : ",e);
		}
	}
};
window.app_kopeg_pinjbrg_transaksi_fKontrak.extend(window.portalui_childForm);
window.app_kopeg_pinjbrg_transaksi_fKontrak.implement({
				...
				try{									
					...
					for (var i=0; i < this.sg2.rows.getLength(); i++){
					  if (this.sg2.cells(0,i) == "APP") {
						sql.add("update kop_jual_d set no_pbrg = '"+this.eKontrak.getText()+"', harga_kont="+parseNilai(this.sg2.cells(7,i))+",jumlah_kont="+parseNilai(this.sg2.cells(8,i))+",status='1',kode_vendor='"+this.sg2.cells(1,i)+"' where no_jual='"+this.cbbOrder.getText()+"' and kode_brg='"+this.sg2.cells(3,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
						}
					}
					var j = 0;
				...
				var sql = new server_util_arrayList();
					...
					for (var j = 0;j < lm;j++){
						...
						if (line.jenis_angs == "F"){								
							this.dataAngsuran=[floatToNilai(so),floatToNilai(pokok),floatToNilai(margin),floatToNilai(so - pokok),tglNext,floatToNilai(k),line.no_pbrg];
							so = so - pokok;
							if (so < pokok) pokok = so;
							...
							eval("value = "+value+";");
							this.dataAngsuran=[floatToNilai(value.totPayment),floatToNilai(value.pokok),floatToNilai(value.margin),floatToNilai(value.totPayment - value.pokok),tglNext,floatToNilai(k),line.no_pbrg];
						...
						perAwal = getNextPeriode(perAwal);
						sql.add("insert into kop_pbrg_sch(no_pbrg,no_kontrak,kode_lokasi,cicilan_ke,npokok,nbunga,saldo,tgl_angs,status_gen,no_bill) values "+
								"('"+line.no_pbrg+"','"+line.no_pbrg+"','"+this.app._lokasi+"',"+k+","+parseNilai(this.dataAngsuran[1])+","+parseNilai(this.dataAngsuran[2])+","+parseNilai(this.dataAngsuran[3])+",'"+this.dataAngsuran[4].substr(6,4)+'/'+this.dataAngsuran[4].substr(3,2)+'/'+this.dataAngsuran[4].substr(0,2)+"',0,'-')");
					}	
					this.dbLib.execArraySQL(sql);
	},
	doSawal: function(sender){
		try {
			//var data = this.dbLib.getDataProvider("select *,date_format(tgl_tagih,'%Y%m') as perawal,date_format(tgl_tagih,'%d') as tgl from kop_pbrg_m where kode_lokasi='"+this.app._lokasi+"'");
			var data = this.dbLib.getDataProvider("select *,date_format(tgl_tagih,'%Y%m') as perawal,date_format(tgl_tagih,'%d') as tgl from kop_pbrg_m where no_pbrg='K/C/0704/038' and kode_lokasi='"+this.app._lokasi+"'");
			eval("data = "+data+";");
			if (typeof data == "object"){
				var line;
	...
	genDataPinjAngs: function(index){
				var sql = new server_util_arrayList();
					line = this.dataPinj[index];									
					var lm = parseFloat(line.gen_bayar);
					...
					for (var j = 0;j < lm;j++){
						...
						sql.add("insert into kop_pbrgangs_d (no_angs,no_pbrg,no_kontrak,tanggal,akun_ar,npokok,nbunga,kode_lokasi,dc) values "+
							    "('21-ANSPB-"+perAwal+"','"+line.no_pbrg+"','"+line.no_pbrg+"','"+perAwal.substr(0,4)+'/'+perAwal.substr(4,2)+'/'+tgl+"','"+line.akun_piutang+"',"+line.nilai_pokok+","+line.nilai_bunga+",'"+this.app._lokasi+"','D')");
						//SP
						var data = this.dbLib.getDataProvider("select npokok,nbunga from kop_pinj_sch "+
						                                      "where date_format(tgl_angs,'%Y%m')='"+perAwal+"' and no_pinj = '"+line.no_pinj+"' and kode_lokasi='"+this.app._lokasi+"'",true);
						if (typeof data != "string"){
							if (data.rs.rows[0] != undefined){
								line.nilai_pokok = data.rs.rows[0].npokok;
								line.nilai_bunga = data.rs.rows[0].nbunga;
							}
							sql.add("insert into kop_pinjangs_d (no_angs,no_pinj,no_kontrak,tanggal,akun_ar,npokok,nbunga,kode_lokasi,dc) values "+
							        "('22-ANSPJ-"+perAwal+"','"+line.no_pinj+"','"+line.no_pinj+"','"+perAwal.substr(0,4)+'/'+perAwal.substr(4,2)+'/'+tgl+"','"+line.akun_piutang+"',"+line.nilai_pokok+","+line.nilai_bunga+",'"+this.app._lokasi+"','D')");
						}
						*/
						perAwal = getNextPeriode(perAwal);
					}	
	...
	doSawalAngs: function(sender){
		try {
			//var data = this.dbLib.getDataProvider("select a.*,b.gen_bayar,date_format(a.tgl_tagih,'%Y%m') as perawal,date_format(a.tgl_tagih,'%d') as tgl from kop_pbrg_m a inner join xxx_upkp_m b on a.no_pbrg=b.no_kontrak where a.kode_lokasi='"+this.app._lokasi+"'");
			//var data = this.dbLib.getDataProvider("select a.*,b.gen_bayar,date_format(a.tgl_tagih,'%Y%m') as perawal,date_format(a.tgl_tagih,'%d') as tgl from kop_pbrg_m a inner join xxx_upkp_m b on a.no_pbrg=b.no_kontrak where a.no_pbrg='K/C/0704/038' and a.kode_lokasi='"+this.app._lokasi+"'");
			
			//var data = this.dbLib.getDataProvider("select a.*,b.z_ke as gen_bayar,date_format(a.tgl_tagih,'%Y%m') as perawal,date_format(a.tgl_tagih,'%d') as tgl from kop_pinj_m a inner join xxx_pinj_valid5 b on a.no_pinj=b.z_no_pinj where b.z_ke <> 0 and a.periode<='201003' and a.kode_lokasi='"+this.app._lokasi+"'");
			
			var data = this.dbLib.getDataProvider("select a.*,b.z_ke as gen_bayar,date_format(a.tgl_tagih,'%Y%m') as perawal,date_format(a.tgl_tagih,'%d') as tgl from kop_pinj_m a inner join xxx_pinj_valid5 b on a.no_pinj=b.z_no_pinj where b.z_ke <> 0 and a.periode<='201003' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_pinj in "+
			...
			if (typeof data == "object"){
				...
				this.dataPinj = data.rs.rows;
				this.procId = 0;
				systemAPI.alert(this.procId);
				this.genDataPinjSawalAngs(this.procId);
			}									  
		}
		catch(e) {
			alert(e);
	...
	genDataPinjSawalAngs: function(index){
				var sql = new server_util_arrayList();
					line = this.dataPinj[index];									
					var lm = parseFloat(line.lama_bayar);
					var so = parseFloat(line.nilai);
					...
					for (var j = 0;j < lm;j++){
						...
						line.nilai_pokok = pokok;
						line.nilai_bunga = margin;
						
						sql.add("insert into kop_pinjangs_d (no_angs,no_pinj,no_kontrak,tanggal,akun_ar,npokok,nbunga,kode_lokasi,dc) values "+
							    "('22-ANSPJ-"+perAwal+"','"+line.no_pinj+"','"+line.no_pinj+"','"+perAwal.substr(0,4)+'/'+perAwal.substr(4,2)+'/'+tgl+"','"+line.akun_piutang+"',"+line.nilai_pokok+","+line.nilai_bunga+",'"+this.app._lokasi+"','D')");
						perAwal = getNextPeriode(perAwal);						
						if (perAwal == '201004') perAwal = "201003";
						if (line.gen_bayar == (j+1)) break;
					}	
					this.dbLib.execArraySQL(sql);
	},
	    			...
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){							 
							...
							if (this.dataPinj.length > this.procId){
								systemAPI.alert("Kontrak "+this.dataPinj[this.procId].no_pbrg+" Selesai.");
								this.procId++;
								this.genDataPinjSawal(this.procId);
							}
							//utk sawal angsuran
							if (this.dataPinj.length > this.procId){
								systemAPI.alert("Kontrak "+this.dataPinj[this.procId].no_pinj+" Selesai.");
								this.procId++;
								systemAPI.alert(this.procId);
								this.genDataPinjSawalAngs(this.procId);
							}
