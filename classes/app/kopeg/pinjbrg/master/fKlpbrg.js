window.app_kopeg_pinjbrg_master_fKlpbrg = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjbrg_master_fKlpbrg.prototype.parent.constructor.call(this,owner);
		this.className  = "app_kopeg_pinjbrg_master_fKlpbrg";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Kelompok Barang : Input/Koreksi", 0);	
		...
		this.p1 = new portalui_panel(this,{bound:[10,17,900,350],caption:"Daftar Kelompok Barang"});
		this.sg1 = new portalui_saiTable(this.p1,{bound:[1,20,895,324],tag:"9"});		
...
window.app_kopeg_pinjbrg_master_fKlpbrg.extend(window.portalui_childForm);
window.app_kopeg_pinjbrg_master_fKlpbrg.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0]))
			{
				...
				{									
					...
					sql.add("insert into kop_brg_klp(kode_klpbrg,nama,kode_akun,akun_hpp,kode_lokasi,akun_jual,akun_beli) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0]))
			{
				...
				{									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update kop_brg_klp set nama = '"+this.e_nama.getText()+"',kode_akun='"+this.cb_akun.getText()+"',akun_hpp='"+this.cb_hpp.getText()+"', "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0]))
			{
				...
				{									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from kop_brg_klp "+
						    "where kode_klpbrg = '"+this.cb_kode.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
	...
	doModalResult: function(event, modalResult){
		...
		switch (event)
		{
			case "clear" :
				...
				break;
			case "simpan" :	
			    var data = this.dbLib.getDataProvider("select kode_akun from kop_brg_klp where kode_lokasi = '"+this.app._lokasi+"' and akun_hpp<>'"+this.cb_hpp.getText()+"' ");
				eval("data = "+data+";");
		...
		try{
			if (this.cb_kode.getText() != ""){
				var data = this.dbLib.getDataProvider(" select a.nama,a.kode_akun,b.nama as nama_akun,a.akun_hpp,c.nama as nama_hpp,a.akun_jual,a.akun_beli,e.nama as nama_jual,g.nama as nama_beli "+
				           "from kop_brg_klp a inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
