window.app_kopeg_pinjbrg_master_fBrg = function(owner)
{
	if (owner)
	{
		window.app_kopeg_pinjbrg_master_fBrg.prototype.parent.constructor.call(this,owner);
		this.className  = "app_kopeg_pinjbrg_master_fBrg";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Master Barang : Input/Koreksi", 0);	
		...
		{			
			...
			var data = this.dbLib.runSQL("select distinct sat from kop_brg where kode_lokasi = '"+this.app._lokasi+"' ");
			this.cb_sat.setItem(data);
			
			var prd = this.dbLib.getDataProvider("select distinct sat from kop_brg where kode_lokasi = '"+this.app._lokasi+"'",true);
...
window.app_kopeg_pinjbrg_master_fBrg.extend(window.portalui_childForm);
window.app_kopeg_pinjbrg_master_fBrg.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0]))
			{
				...
				{									
					...
					sql.add("insert into kop_brg(kode_brg,nama,tipe,kode_klpbrg,sat,harga,kode_lokasi) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0]))
			{
				...
				{									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update kop_brg set nama='"+this.e_nama.getText()+"',tipe='"+this.e_tipe.getText()+"',kode_klpbrg='"+this.cb_klp.getText()+"',sat='"+this.cb_sat.getText()+"',harga="+parseNilai(this.e_harga.getText())+" "+
		...
		try{
			if (this.cb_kode.getText() != ""){
				var data = this.dbLib.getDataProvider(" select a.nama,a.tipe,b.nama as nama_klp,a.sat,a.harga,a.kode_klpbrg "+
				           " from kop_brg a inner join kop_brg_klp b on a.kode_klpbrg = b.kode_klpbrg and a.kode_lokasi=b.kode_lokasi "+
	...
	doTampilClick: function(sender){
		try{			
			...
			var data = this.dbLib.runSQL(" select a.kode_brg,a.nama,a.tipe,b.nama as nama_klp,a.sat,a.harga from kop_brg a "+
					   " inner join kop_brg_klp b on a.kode_klpbrg=b.kode_klpbrg and a.kode_lokasi = b.kode_lokasi "+
		...
		try
		{
			...
			{   
			    this.standarLib.showListData(this, "Daftar Barang",sender,undefined, 
											  "select kode_brg, nama  from kop_brg where kode_lokasi='"+this.app._lokasi+"'",
			...
			{   
			    this.standarLib.showListData(this, "Daftar Kelompok Barang",sender,undefined, 
											  ...
											  "select count(kode_klpbrg) from kop_brg_klp where kode_lokasi='"+this.app._lokasi+"'",
