window.app_kopeg_simpanan_report_flSPB = function(owner)
{
	if (owner)
	{
		window.app_kopeg_simpanan_report_flSPB.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_simpanan_report_flSPB";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan SPB Simpanan", 2);
...
window.app_kopeg_simpanan_report_flSPB.extend(window.portalui_childForm);
window.app_kopeg_simpanan_report_flSPB.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row === 4)
		{
			this.filterRep.ListDataSGFilter(this, "Data No. SPB Simpanan",this.sg1, this.sg1.row, this.sg1.col,
										  "select no_spb,keterangan from spb_m where kode_lokasi='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"' and modul='KP.SPB' and jenis='SIMP' and no_del='-'",
										  "select count(*) from spb_m where kode_lokasi='"+this.sg1.getCell(2,0)+"' and periode='"+this.sg1.getCell(2,1)+"' and modul='KP.SPB' and jenis='SIMP' and no_del='-'",
