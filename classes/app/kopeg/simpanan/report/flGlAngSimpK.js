// JavaScript Document
window.app_kopeg_simpanan_report_flGlAngSimpK = function(owner)
{
	if (owner)
	{
		window.app_kopeg_simpanan_report_flGlAngSimpK.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_simpanan_report_flGlAngSimpK";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kop_simpangs_m where jenis='ANGS_KOL' and POSTED='T' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_kopeg_simpanan_report_flGlAngSimpK.extend(window.portalui_childForm);
window.app_kopeg_simpanan_report_flGlAngSimpK.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			...
			var sql1="select a.no_angs,a.keterangan from kop_simpangs_m a where a.jenis='ANGS_KOL' "+filter; 
			var sql2="select count(a.no_angs) from kop_simpangs_m a where a.jenis='ANGS_KOL' "+filter;
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_simpangs_m where  jenis='ANGS_KOL' and posted='T' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			this.nama_report="server_report_saku3_rptGlAngSimpK";
