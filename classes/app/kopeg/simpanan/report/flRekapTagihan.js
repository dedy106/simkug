window.app_kopeg_simpanan_report_flRekapTagihan = function(owner)
{
	if (owner)
	{
		window.app_kopeg_simpanan_report_flRekapTagihan.prototype.parent.constructor.call(this,owner);
		this.className = "app_kopeg_simpanan_report_flRekapTagihan";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Rekapitulasi Tunggakan", 2);
	...
	this.getPeriode="select distinct periode from kop_simpbill_m where kode_lokasi ='"+this.lokasi+"'";
...
window.app_kopeg_simpanan_report_flRekapTagihan.extend(window.portalui_childForm);
window.app_kopeg_simpanan_report_flRekapTagihan.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2){
				...
				var sql = "select a.kode_agg,a.nama as nsbh,c.nama as loker,sum(b.sp) as sp,sum(b.sw) as sw,sum(b.ss) as ss,sum(b.sp)+sum(b.sw)+sum(b.ss) as total "+
						"from kop_agg a inner join kop_loker c on a.kode_loker=c.kode_loker and a.kode_lokasi=c.kode_lokasi "+
						"inner join (select t.kode_agg,z.kode_lokasi,z.periode, "+
						"	case t.jenis when 'SP' then z.nilai else 0 end as sp, "+
						"	case t.jenis when 'SW' then z.nilai else 0 end as sw, "+
						"	case t.jenis when 'SS' then z.nilai else 0 end as ss "+
						"	from kop_simp_d z inner join kop_simpbill_m q on z.no_bill=q.no_bill and z.kode_lokasi=q.kode_lokasi "+						
						"		inner join kop_simp_m t on z.no_simp=t.no_simp and z.kode_lokasi=t.kode_lokasi "+
						"		left outer join kop_simpangs_d y on y.no_simp = z.no_simp "+					
						"	where y.no_simp is null "+
						"	) b on a.kode_agg=b.kode_agg and a.kode_lokasi=b.kode_lokasi  "+this.filter+
						" group by a.kode_agg having sum(b.sp)+sum(b.sw)+sum(b.ss) > 0";
				this.scriptSqlCount = "select count(distinct kode_agg) "+
						"from kop_agg a inner join kop_loker c on a.kode_loker=c.kode_loker and a.kode_lokasi=c.kode_lokasi "+
						"inner join (select t.kode_agg,z.kode_lokasi,z.periode "+						
						"	from kop_simp_d z inner join kop_simpbill_m q on z.no_bill=q.no_bill and z.kode_lokasi=q.kode_lokasi "+
						"		left outer join kop_simpangs_d y on y.no_simp = z.no_simp "+						
						"	inner join kop_simp_m t on z.no_simp=t.no_simp and z.kode_lokasi=t.kode_lokasi "+
						"	where y.no_simp is null "+
						"	) b on a.kode_agg=b.kode_agg and a.kode_lokasi=b.kode_lokasi group by a.kode_agg having sum(b.sp)+sum(b.sw)+sum(b.ss) > 0 "+this.filter;
				this.title = new server_util_arrayList({items:["Kode","Nasabah","Lokasi Kerja","Simp. Pokok","Simp. Wajib","Simp. Sukarela","Sub Total"]});
			...
			systemAPI.alert("[app_kopeg_simpanan_report_flRekapTagihan]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>DATA REKAPITULASI TUNGGAKAN<br>";
		var d = new Date();
	  ...
	  switch(sender.getName())
	  {
	    ...
	    case "pdfBtn" :      
		  var html = new server_util_arrayList();
			...
			html.add("pdf");			
			html.add(new Date().valueOf()+"_tunggakan");				
