window.app_kopeg_simpanan_transaksi_fInvk = function(owner)
{
	if (owner)
	{
		window.app_kopeg_simpanan_transaksi_fInvk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_kopeg_simpanan_transaksi_fInvk";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penerimaan [UDP]: Koreksi", 0);	
		...
		try{			
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('SIMPIM','KOPUDP') and kode_lokasi = '"+this.app._lokasi+"' ");
			eval("data = "+data+";");
			if (typeof data == "object"){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];												
					if (line.kode_spro == "SIMPIM") this.akunIM = line.flag;
					if (line.kode_spro == "KOPUDP") this.akunUDP = line.flag;
				}
			}			
			var prd = this.dbLib.getDataProvider("select distinct periode from kop_simpinv_m where jenis = 'INVSIMP' and kode_lokasi = '"+this.app._lokasi+"'",true);
...
window.app_kopeg_simpanan_transaksi_fInvk.extend(window.portalui_childForm);
window.app_kopeg_simpanan_transaksi_fInvk.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					if (parseFloat(this.perLama) < parseFloat(this.app._periode)) {
						this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'kop_simpinv_m','no_inv',this.app._lokasi+"-INV"+this.e_periode.getText().substr(2,4)+".",'0000'));
						sql.add(" update kop_simpinv_m set no_link='"+this.e_nb.getText()+"',no_del = concat(no_inv,'r') where no_inv ='"+this.cb_nbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
						sql.add(" insert into kop_simpinv_m (no_inv,no_dokumen,keterangan,tanggal,nilai,jenis,periode,kode_pp,kode_lokasi,nik_app,posted,kurs,kode_curr,akun_ar,progress,no_del,no_link,nik_user,tgl_input) "+
								...
								" from kop_simpinv_m where no_inv = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"'");
						sql.add(" insert into kop_simpinv_j (no_inv,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input)"+
								...
								" from kop_simpinv_j where no_inv = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");
						...
						sql.add("delete from kop_simpinv_m where no_inv='"+this.cb_nbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from kop_simpinv_j where no_inv='"+this.cb_nbLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add("insert into kop_simpinv_m (no_inv,no_dokumen,keterangan,tanggal,nilai,jenis,periode,kode_pp,kode_lokasi,nik_app,posted,kurs,kode_curr,akun_ar,progress,no_del,no_link,nik_user,tgl_input)  values "+                              
							"('"+this.nb+"','"+this.e_dok.getText()+"','"+this.e_desc.getText()+"','"+this.dp_d1.getDateString()+"',"+parseNilai(this.e_nilai.getText())+",'INVSIMP','"+this.e_periode.getText()+"','"+this.app._kodePP+"','"+this.app._lokasi+"','"+this.cb_app.getText()+"','F',1,'IDR','"+this.akunIM+"','0','-','-','"+this.app._userLog+"',now())");
					sql.add("insert into kop_simpinv_j (no_inv,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+	
							"('"+this.nb+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.akunIM+"','"+this.e_desc.getText()+"','D',"+parseNilai(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','SIMP','IM','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())");
					sql.add("insert into kop_simpinv_j (no_inv,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+	
							"('"+this.nb+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',1,'"+this.akunUDP+"','"+this.e_desc.getText()+"','C',"+parseNilai(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','SIMP','UDP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())");
					sql.add("insert into kop_depo(no_depo,tanggal,keterangan,dc,nilai,kode_agg,modul,jenis,periode,kode_lokasi,kode_pp,nik_app,no_del,no_link,kurs,kode_curr,nik_user,tgl_input) values "+
				            "('"+this.nb+"','"+this.dp_d1.getDateString()+"','"+this.e_desc.getText()+"','D',"+parseNilai(this.e_nilai.getText())+",'"+this.cb_agg.getText()+"','SIMP','UDP','"+this.e_periode.getText()+"','"+this.app._lokasi+"','"+this.app._kodePP+"','"+this.cb_app.getText()+"','-','-',1,'IDR','"+this.app._userLog+"',now())");											
		...
		switch (event){
			...
			case "hapus" : 
				...
				try{
					...
					var sql = new server_util_arrayList();
					if (parseFloat(this.perLama) < parseFloat(this.app._periode)) {
						sql.add(" update kop_simpinv_m set no_del = concat(no_inv,'r') where no_inv ='"+this.cb_nbLama.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
						sql.add(" insert into kop_simpinv_m (no_inv,no_dokumen,keterangan,tanggal,nilai,jenis,periode,kode_pp,kode_lokasi,nik_app,posted,kurs,kode_curr,akun_ar,progress,no_del,no_link,nik_user,tgl_input) "+
							    " select concat(no_inv,'r'),no_dokumen,keterangan,'"+this.dp_d1.getDateString()+"',nilai,jenis,'"+this.e_periode.getText()+"',kode_pp,kode_lokasi,nik_app,'F',kurs,kode_curr,akun_ar,'X',no_inv,'-','"+this.app._userLog+"',now() "+
								" from kop_simpinv_m where no_inv = '"+this.cb_nbLama.getText()+"' and kode_lokasi ='"+this.app._lokasi+"'");
						sql.add(" insert into kop_simpinv_j (no_inv,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input)"+
		...
		try{			
			if (this.cb_nbLama.getText() != ""){
				...
				var data = this.dbLib.runSQL("select no_depo,date_format(tanggal,'%d/%m/%Y')as tanggal,keterangan,case dc when 'D' then 'IN' else 'OUT' end as dc,nilai "+
											 "	from kop_depo "+
											 "	where kode_agg='"+this.cb_nbLama.dataFromList[5]+"' and no_del='-' and modul = 'SIMP' and jenis='UDP' "+
		...
		try{
			if (sender == this.cb_nbLama) {   
			    this.standarLib.showListData(this, "Daftar Bukti Invoice",sender,undefined, 
											  "select a.no_inv,a.no_dokumen,a.keterangan,a.nik_app,b.nama,cc.kode_agg,cc.nama as nama_agg from kop_simpinv_m a "+
											  ...
											  "where a.kode_lokasi='"+this.app._lokasi+"' and a.periode='"+this.cb_perLama.getText()+"' and a.no_del='-' and a.progress = '0' ", 
											  
											  "select count(no_inv)       from kop_simpinv_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and no_del='-' and progress = '0'",
											  ["no_inv","no_dokumen","keterangan","nik_app","nama","kode_agg","nama_agg"],"and",
											  ["No Bukti","No Dokumen","Deskripsi","NIK App","Nama","Kode Agg","Nama Agg"],false);				
