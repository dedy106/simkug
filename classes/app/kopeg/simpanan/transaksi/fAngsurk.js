window.app_kopeg_simpanan_transaksi_fAngsurk = function(owner)
{
	if (owner)
	{
		window.app_kopeg_simpanan_transaksi_fAngsurk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_kopeg_simpanan_transaksi_fAngsurk";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penerimaan Angsuran Simpanan: Koreksi", 0);	
		...
		this.cb_nbLama = new portalui_saiCBB(this,{bound:[720,13,200,20],caption:"No Angs. Lama",readOnly:true,btnClick:[this,"doBtnClick"],btnRefreshClick:[this,"doLoadData"]});
		...
		this.cbbSpb = new portalui_saiCBB(this,{bound:[720,15,200,20],caption:"No SPB/TAK", btnClick:[this,"doBtnClick"], tag:9,btnRefreshClick:[this,"doLoadDataSPB"]});
		this.cb_app = new portalui_saiCBBL(this,{bound:[20,16,200,20],caption:"Penerima",btnClick:[this,"doBtnClick"],tag:1});		
		this.cb_agg = new portalui_saiCBBL(this,{bound:[20,17,200,20],caption:"Nasabah",btnClick:[this,"doBtnClick"],tag:1,change:[this,"doChange"],readOnly:true});
		this.cb_simp = new portalui_saiCBBL(this,{bound:[20,18,250,20],caption:"No Kartu",btnClick:[this,"doBtnClick"],tag:1,rightLabelVisible:false,readOnly:true});
		...
		try{			
			...
			var prd = this.dbLib.getDataProvider("select distinct periode from kop_simpangs_m where jenis = 'SIMPANGS' and kode_lokasi = '"+this.app._lokasi+"'",true);//
...
window.app_kopeg_simpanan_transaksi_fAngsurk.extend(window.portalui_childForm);
window.app_kopeg_simpanan_transaksi_fAngsurk.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{	
					...
					sql.add("insert into kop_simpangs_m(no_angs,no_dokumen,keterangan,tanggal,nilai,nilai_lain,nilai_sls,jenis,progress,akun_ar,periode,kode_lokasi,posted,kurs,kode_curr,kode_pp,no_del,no_link,nik_app,nik_user,tgl_input,no_spb) values  "+
							"('"+this.nb+"','"+this.e_dok.getText()+"','"+this.e_desc.getText()+"','"+this.dp_d1.getDateString()+"',"+parseNilai(this.e_angsur.getText())+","+parseNilai(this.e_depo.getText())+","+parseNilai(this.e_sls.getText())+",'SIMPANGS','"+vProg+"','"+this.akunIM+"','"+this.e_periode.getText()+"','"+this.app._lokasi+"','F',1,'IDR','"+this.app._kodePP+"','-','-','"+this.cb_app.getText()+"','"+this.app._userLog+"',now(),'-')");
					...
					if (nilaiToFloat(this.e_sls.getText()) > 0){
						...
						sql.add("insert into kop_simpangs_j (no_angs,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+	
							    "('"+this.nb+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+idx+",'"+this.akunTP+"','"+this.e_desc.getText()+"','C',"+parseNilai(this.e_sls.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','SIMP','ANGS_TP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())");							
						sql.add("insert into kop_depo(no_depo,tanggal,keterangan,dc,nilai,kode_agg,modul,jenis,periode,kode_lokasi,kode_pp,nik_app,no_del,no_link,kurs,kode_curr,nik_user,tgl_input) values "+
							    "('"+this.nb+"','"+this.dp_d1.getDateString()+"','"+this.e_desc.getText()+"','D',"+parseNilai(this.e_sls.getText())+",'"+this.cb_agg.getText()+"','SIMP','ANGS_SLS','"+this.e_periode.getText()+"','"+this.app._lokasi+"','"+this.app._kodePP+"','"+this.cb_app.getText()+"','-','-',1,'IDR','"+this.app._userLog+"',now())");					
					} 
					else {
						if (nilaiToFloat(this.e_sls.getText()) < 0){
							idx++;
					...
					if (nilaiToFloat(this.e_depo.getText()) > 0){
						idx++;
						sql.add("insert into kop_simpangs_j (no_angs,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+	
							    "('"+this.nb+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+idx+",'"+this.akunTP+"','"+this.e_desc.getText()+"','D',"+parseNilai(this.e_depo.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','SIMP','ANGS_DP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())");
						sql.add("insert into kop_depo(no_depo,tanggal,keterangan,dc,nilai,kode_agg,modul,jenis,periode,kode_lokasi,kode_pp,nik_app,no_del,no_link,kurs,kode_curr,nik_user,tgl_input) values "+
							    "('"+this.nb+"','"+this.dp_d1.getDateString()+"','"+this.e_desc.getText()+"','C',"+parseNilai(this.e_depo.getText())+",'"+this.cb_agg.getText()+"','SIMP','ANGS_PKI','"+this.e_periode.getText()+"','"+this.app._lokasi+"','"+this.app._kodePP+"','"+this.cb_app.getText()+"','-','-',1,'IDR','"+this.app._userLog+"',now())");											
					}
					idx++;
					sql.add("insert into kop_simpangs_j (no_angs,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+	
		...
		try{			
			if (sender == this.cb_agg){
				if (this.cb_agg.getText() != "") {
					var data = this.dbLib.getDataProvider("select ifnull(sum(case dc when 'D' then nilai else -nilai end),0) as saldo from kop_depo where kode_agg ='"+this.cb_agg.getText()+"' and modul in ('SIMP','PINJ') and jenis <> 'UDP' and kode_lokasi = '"+this.app._lokasi+"' and no_depo <> '"+this.cb_nbLama.getText()+"' ");
					eval("data = "+data+";");
					if (typeof data == "object"){
		...
		try{			
			if (this.cb_nbLama.getText() != ""){
				var data = this.dbLib.getDataProvider("select a.no_simp,a.jenis,b.nilai-ifnull(d.bayar,0) as saldo,b.akun_ar,b.periode,b.no_bill,c.keterangan as ket, "+
													  "       bb.akun_ar as akun_im,bb.no_spb,bb.tanggal as tgl_angs,bb.periode,bb.posted,bb.no_dokumen,bb.keterangan,bb.nik_app,cc.nama as nama_app,a.kode_agg,dd.nama as nama_agg,bb.nilai as n_angs,bb.nilai_lain as n_depo,bb.nilai_sls as n_sls "+
													  ...
													  "                   inner join kop_agg dd on a.kode_agg =dd.kode_agg and a.kode_lokasi=dd.kode_lokasi "+
				...
				if (typeof data == "object"){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg.appendData(["CAIR",line.no_simp,line.jenis,line.no_bill,line.ket,line.periode,line.akun_ar,floatToNilai(line.saldo)]);
					}
				}
				this.e_angsur.setText(floatToNilai(parseFloat(line.n_angs)));
				this.e_depo.setText(floatToNilai(parseFloat(line.n_depo)));
				...
				this.cb_simp.setText(line.no_simp,line.no_simp);
				...
				system.alert(this,"No Angsuran tidak valid.","No Angs. Lama harus dipilih.");
			}
		}catch(e){
			systemAPI.alert(e);
		}
		...
		try{
			if (sender == this.cb_nbLama) {   
			    this.standarLib.showListData(this, "Daftar Bukti Angsuran",sender,undefined, 
											  "select no_angs, keterangan  from kop_simpangs_m where jenis='SIMPANGS' and progress= '0' and kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and no_del='-'", 
											  "select count(no_angs)       from kop_simpangs_m where jenis='SIMPANGS' and progress= '0' and kode_lokasi='"+this.app._lokasi+"' and periode='"+this.cb_perLama.getText()+"' and no_del='-'",
