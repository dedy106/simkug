window.app_kopeg_simpanan_transaksi_fSpbdepo = function(owner)
{
	if (owner)
	{
		window.app_kopeg_simpanan_transaksi_fSpbdepo.prototype.parent.constructor.call(this,owner);
		this.className  = "app_kopeg_simpanan_transaksi_fSpbdepo";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form SPB Customer Deposit Simpanan: Input", 0);	
...
window.app_kopeg_simpanan_transaksi_fSpbdepo.extend(window.portalui_childForm);
window.app_kopeg_simpanan_transaksi_fSpbdepo.implement({
				...
				try{									
					...
					if (this.e_nilai.getText() != "0") {
						...
						sql.add("insert into kop_depo(no_depo,tanggal,keterangan,dc,nilai,kode_agg,modul,jenis,periode,kode_lokasi,kode_pp,nik_app,no_del,no_link,kurs,kode_curr,nik_user,tgl_input) values "+
								"('"+this.e_nb.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_desc.getText()+"','C',"+parseNilai(this.e_nilai.getText())+",'"+this.cb_agg.getText()+"','SIMP','AMBIL','"+this.e_periode.getText()+"','"+this.app._lokasi+"','"+this.app._kodePP+"','"+this.cb_app.getText()+"','-','-',1,'IDR','"+this.app._userLog+"',now())");
					}
					if (this.e_udp.getText() != "0") {
						idx++;
						sql.add("insert into spb_j (no_spb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+	
								"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+idx+",'"+this.akunUDP+"','"+this.e_desc.getText()+"','D',"+parseNilai(this.e_udp.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','SIMP','UDP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())");	
						sql.add("insert into kop_depo(no_depo,tanggal,keterangan,dc,nilai,kode_agg,modul,jenis,periode,kode_lokasi,kode_pp,nik_app,no_del,no_link,kurs,kode_curr,nik_user,tgl_input) values "+
