window.app_kopeg_simpanan_transaksi_fSpb = function(owner)
{
	if (owner)
	{
		window.app_kopeg_simpanan_transaksi_fSpb.prototype.parent.constructor.call(this,owner);
		this.className  = "app_kopeg_simpanan_transaksi_fSpb";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form SPB Pengambilan Simp. +UPD: Input", 0);	
		
		uses("portalui_saiCBBL;portalui_saiEdit;portalui_datePicker;portalui_saiGrid;portalui_checkBox;portalui_reportViewer;server_report_report");
...
window.app_kopeg_simpanan_transaksi_fSpb.extend(window.portalui_childForm);
window.app_kopeg_simpanan_transaksi_fSpb.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into spb_j (no_spb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+	
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+idx+",'"+this.akunAP+"','"+this.e_desc.getText()+"','C',"+total+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','SIMP','AP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())");	
					this.createJurnal();
					...
					if (this.e_udp.getText() != "0") {
						if (z >0) d+=",";
						idx++;
						d+="('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+idx+",'"+this.akunUDP+"','"+this.e_desc.getText()+"','D',"+parseNilai(this.e_udp.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','SIMP','UDP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',now())";
						sql.add("insert into kop_depo(no_depo,tanggal,keterangan,dc,nilai,kode_agg,modul,jenis,periode,kode_lokasi,kode_pp,nik_app,no_del,no_link,kurs,kode_curr,nik_user,tgl_input) values "+
							    "('"+this.e_nb.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_desc.getText()+"','C',"+parseNilai(this.e_udp.getText())+",'"+this.cb_agg.getText()+"','SIMP','UDP','"+this.e_periode.getText()+"','"+this.app._lokasi+"','"+this.app._kodePP+"','"+this.cb_app.getText()+"','-','-',1,'IDR','"+this.app._userLog+"',now())");											
					}
					sql.add(d);					
					...
					for (var i=0;i < this.sg.getRowCount();i++){
						if (this.sg.rowValid(i)){
							if (this.sg.cells(4,i) != "0") {
								if (z > 0) s+= ",";
								s+="('"+this.e_nb.getText()+"','SPB','"+this.sg.cells(0,i)+"','"+this.sg.cells(1,i)+"',"+parseNilai(this.sg.cells(4,i))+",'D','"+this.app._lokasi+"','"+this.e_desc.getText()+"')";
								z++;
							}
						}
					}						
		...
		try{			
			if (this.cb_agg.getText() != ""){
				var data = this.dbLib.getDataProvider("select c.no_simp,d.akun_simp,d.nama, "+
													  "     ifnull(a.nilai,0)+ifnull(b.ambil,0)+ifnull(e.pinbuk,0)+ifnull(f.bunga,0) as saldo "+
													  ...
													  " where c.kode_agg= '"+this.cb_agg.getText()+"' and c.kode_lokasi='"+this.app._lokasi+"' ");
