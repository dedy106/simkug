...
//* 			SAI, PT											
//***********************************************************************************************
window.app_kopeg_fUser = function(owner)
{
	if (owner)
	{
		try
		{
			window.app_kopeg_fUser.prototype.parent.constructor.call(this, owner);
			this.className = "app_kopeg_fUser";
		...
		{
			alert("[app_kopeg_fUser]::constructor:"+e);
		}
		
		this.setTabChildIndex();
	}
};
window.app_kopeg_fUser.extend(window.portalui_childForm);
window.app_kopeg_fUser.prototype.doClick = function(sender){
...
window.app_kopeg_fUser.prototype.doEditChange = function(sender)
{
	setTipeButton(tbSimpan);
	if (sender == this.e0)
	{		
...
window.app_kopeg_fUser.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_kopeg_fUser.prototype.doFindClick = function(sender)
{
	try{
		if (sender == this.e5) {
			if (this.app._userStatus != "A" ) 
...
window.app_kopeg_fUser.prototype.mainButtonClick = function(sender)
{
...
window.app_kopeg_fUser.prototype.doModalResult = function(event, modalResult, value)
{
	...
	switch (event)
	{
		...
		case "hapus" :
			if (modalResult == mrOk)
			{
				this.dbLib.execArraySQL("delete from hakakses where nik = '"+this.e0.getText()+"'");		
			}
			break;
	}
};
