...
window.app_rra_transaksi_fRRRrev = function(owner)
{
	if (owner)
	{
		...
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form PDRK Reschedule, Redistribusi, Realokasi Anggaran: Review", 0);	
		...
		this.pc1 = new pageControl(this,{bound:[20,16,900, 350], childPage:["Data PDRK","Data Donor","Data Penerima","Data Justifikasi","Dokumen Pendukung","View Dokumen","Nota Kajian (CAPEX)"], pageChange:[this,"doPageChange"]});
		...
		this.bOpen = new button(this.pc1.childPage[0], {bound:[520,2,80,20], caption:"Open", click:[this,"doOpenFile"]});
		...
		this.cb_buat = new saiCBBL(this.pc1.childPage[0],{bound:[20,12,200,20],caption:"NIK Pembuat", multiSelection:false, maxLength:10, tag:2, readOnly:true,  bufferOption: boHALF, bufferData: this.app._listNIK, text:this.app._userLog, rightLabelCaption:this.app._namaUser});
		...
		try {
			...
			this.cb_pdrk.setSQL("select no_pdrk,keterangan from rra_pdrk_m where progress = '0' and kode_lokasi='"+this.app._lokasi+"' and sts_pdrk in ('ABT','RRR') and nik_buat = '"+this.app._userLog+"' and kode_ubis='"+this.app._kodeUbis+"' and nik_buat= '"+this.app._userLog+"'",["no_pdrk","keterangan"],false,["No PDRK","Keterangan"],"and","Data PDRK",true);
			...
			this.cb_aju.setSQL("select nik, nama, jabatan from rra_karyawan where kode_lokasi='"+this.app._lokasi+"' ",["nik","nama","jabatan"],false,["NIK","Nama","Jabatan"],"and","Data Karyawan ",true);
...
window.app_rra_transaksi_fRRRrev.implement({
	...
	simpan: function(){			
		try{		
			if (this.showLoading) this.showLoading("Silahkan tunggu.<br>Sedang diproses....!!!<br> Generate SQL......");		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					var flag = (this.sub_jenis == "RSH" ? "-" : "-");
					var prog = this.e_modul.getText() == "ABT" ? "A" : "-";										
					if (this.cb_rev5.getText() != "-") prog = "05";
					if (this.cb_rev4.getText() != "-") prog = "04";
					if (this.cb_rev3.getText() != "-") prog = "03";
					if (this.cb_rev2.getText() != "-") prog = "02";
					...
					sql.add(new server_util_Map({items:{tipe:"clob",table:"rra_rev_m",field:"kajian", filter :" no_rev = '"+this.e_nb.getText()+"' ",value:urlencode(this.mDesk2.getCode())}})); 	
			...
			for (var i in this.data1Thn.objList){										
				...
				for (var l in row.objList){//rows = 16
					...
					for (var l2 in line.objList){//row																	
						...
						if (this.c_jenis.getText() == "OPEX"){						
							if (this.sg.cells(10,rowGrid) == "PAYMENT" ){
							...
							if (line.get(l2).get("VERSN") == "000" && this.sg.cells(10,rowGrid) == "PAYMENT" ){
...
/*if (this.c_jenis.getText() == "OPEX"){
										if (this.sub_jenis == "RSH"){
											line = { sawal : tmp2.get("TSL10") ,
														 pakai :  tmp2.get("TSL04") + dataRRR.get(i), //harusnya ke TSL04.. nunggu untuk transfer ke prod dari QA
														 sisa : tmp2.get("TSL10")- dataRRR.get(i) - tmp2.get("TSL04")
												};
										}else 
