window.app_rra_transaksi_fRevgubis = function(owner)
{
	if (owner)
	{
		window.app_rra_transaksi_fRevgubis.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fRevgubis";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form PDRK ABT Anggaran: Review Group Bisnis", 0);	
		...
		this.pc1 = new pageControl(this,{bound:[20,16,900, 350], childPage:["Data PDRK","Data Donor","Data Penerima","Data Justifikasi","Dokumen Pendukung","Nota Kajian"]});
		...
		this.mDesk2 = new tinymceCtrl(this.pc1.childPage[5],{bound:[1,10,this.pc1.width-2,this.pc1.height-15], withForm:false});		
		...
		try {
			...
			if (this.app._statusLokasiUser == "GUBIS" || this.app._statusLokasiUser == "FC" || this.app._statusLokasiUser == "MA"){			
				this.cb_pdrk.setSQL("select a.no_pdrk,a.keterangan,b.no_rev from rra_pdrk_m a inner join rra_rev_m b on a.no_pdrk=b.no_pdrk and a.kode_lokasi=b.kode_lokasi "+
									"where b.progress='1' and b.sts_pdrk = 'ABT' and b.kode_lokasi='"+this.app._lokasi+"' and  b.kode_gubis = '"+this.app._kodeGubis+"' ",["no_pdrk","keterangan","no_rev"],false,["No PDRK","Keterangan","No Review UBIS"],"and","Data PDRK",true);
			}else this.cb_pdrk.setSQL("select a.no_pdrk,a.keterangan,b.no_rev from rra_pdrk_m a inner join rra_rev_m b on a.no_pdrk=b.no_pdrk and a.kode_lokasi=b.kode_lokasi "+
									"where b.progress='XX' and b.sts_pdrk = 'ABT' and b.kode_lokasi='"+this.app._lokasi+"' and  b.kode_gubis = 'XX' ",["no_pdrk","keterangan","no_rev"],false,["No PDRK","Keterangan","No Review UBIS"],"and","Data PDRK",true);
			...
			this.cb_reviewer.setSQL("select nik, nama, jabatan from rra_karyawan where kode_lokasi='"+this.app._lokasi+"' and kode_gubis = '"+this.app._kodeGubis+"'",["nik","nama","jabatan"],false,["NIK","Nama","Jabatan"],"and","Data Karyawan ",true);
			this.cb_app1.setSQL("select nik, nama, jabatan from rra_karyawan where kode_lokasi='"+this.app._lokasi+"' and kode_gubis = '"+this.app._kodeGubis+"'",["nik","nama","jabatan"],false,["NIK","Nama","Jabatan"],"and","Data Karyawan ",true);
			this.cb_app2.setSQL("select nik, nama, jabatan from rra_karyawan where kode_lokasi='"+this.app._lokasi+"' and kode_gubis = '"+this.app._kodeGubis+"'",["nik","nama","jabatan"],false,["NIK","Nama","Jabatan"],"and","Data Karyawan",true);
			this.cb_app3.setSQL("select nik, nama, jabatan from rra_karyawan where kode_lokasi='"+this.app._lokasi+"' and kode_gubis = '"+this.app._kodeGubis+"' ",["nik","nama","jabatan"],false,["NIK","Nama","Jabatan"],"and","Data Karyawan",true);
			this.cb_buat.setSQL("select nik, nama, jabatan from rra_karyawan where kode_lokasi='"+this.app._lokasi+"' and kode_gubis = '"+this.app._kodeGubis+"'",["nik","nama","jabatan"],false,["NIK","Nama","Jabatan"],"and","Data Karyawan",true);
			this.cb_kota.setSQL("select kode_kota, nama from rra_kota ",["kode_kota","nama"],false,["Kode Kota","Nama Kota"],"and","Data Kota",true);
			this.cb_appj1.setSQL("select nik, nama, jabatan from rra_karyawan where kode_lokasi='"+this.app._lokasi+"' and kode_gubis = '"+this.app._kodeGubis+"'",["nik","nama","jabatan"],false,["NIK","Nama","Jabatan"],"and","Data Karyawan ",true);
			this.cb_appj2.setSQL("select nik, nama, jabatan from rra_karyawan where kode_lokasi='"+this.app._lokasi+"' and kode_gubis = '"+this.app._kodeGubis+"'",["nik","nama","jabatan"],false,["NIK","Nama","Jabatan"],"and","Data Karyawan",true);
			this.cb_appp3.setSQL("select nik, nama, jabatan from rra_karyawan where kode_lokasi='"+this.app._lokasi+"' and kode_gubis = '"+this.app._kodeGubis+"'",["nik","nama","jabatan"],false,["NIK","Nama","Jabatan"],"and","Data Karyawan",true);
			this.cb_appp32.setSQL("select nik, nama, jabatan from rra_karyawan where kode_lokasi='"+this.app._lokasi+"' and kode_gubis = '"+this.app._kodeGubis+"'",["nik","nama","jabatan"],false,["NIK","Nama","Jabatan"],"and","Data Karyawan",true);
...
window.app_rra_transaksi_fRevgubis.extend(window.childForm);
window.app_rra_transaksi_fRevgubis.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					var flag = this.c_modul.getText() == "RRR" ? (this.sub_jenis == "RSH" ? "-" : "-") : "-";
					sql.add("update rra_rev_m set progress='2' where no_rev='"+this.e_norev.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("update rra_pdrk_m set progress='4' where no_pdrk = '"+this.cb_pdrk.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
					sql.add("insert into rra_grev_m(no_grev,kode_lokasi,no_rev,no_pdrk,kode_ubis,kode_gubis,jenis_agg,tanggal,periode,nik_buat,nik_app1,nik_app2,nik_app3,sts_pdrk,justifikasi,nik_user,tgl_input,progress,keterangan, flag_rfc, kode_kota,no_nd, file_nd, nik_appjust, nik_appjust2, nik_apppdrk3, nik_apppdrk32, sts_appjust, sts_appjust2, sts_apppdrk3, sts_apppdrk32, sub_jenis, nik_review) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_norev.getText()+"','"+this.cb_pdrk.getText()+"','"+this.cb_ubis.getText()+"','"+this.cb_gubis.getText()+"','"+this.c_jenis.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_periode.getText()+"','"+this.app._userLog+"','"+this.cb_app1.getText()+"','"+this.cb_app2.getText()+"','"+this.cb_app3.getText()+"','"+this.c_modul.getText()+"',null,'"+this.app._userLog+"',now(),'0','"+this.e_ket.getText()+"','"+flag+"','"+this.cb_kota.getText()+"','"+this.ed_dok.getText()+"','"+this.e_file.getText()+"','"+this.cb_appj1.getText()+"','"+this.cb_appj2.getText()+"','"+this.cb_appp3.getText()+"','"+this.cb_appp32.getText()+"','0','0','0','0','"+this.sub_jenis+"','"+this.cb_reviewer.getText()+"')");
					sql.add(new server_util_Map({items:{tipe:"clob",table:"rra_grev_m",field:"justifikasi", filter :" no_grev = '"+this.e_nb.getText()+"' ",value:urlencode(this.mDesk.getCode())}})); 	
					sql.add(new server_util_Map({items:{tipe:"clob",table:"rra_grev_m",field:"kajian", filter :" no_grev = '"+this.e_nb.getText()+"' ",value:urlencode(this.mDesk2.getCode())}})); 	
		...
		switch (event){
			...
			case "simpan" :	
				...
				this.sg2.validasi();
					...
					{
						this.sg.validasi();
						if (nilaiToFloat(this.e_donor.getText()) != nilaiToFloat(this.e_terima.getText()) && (this.sub_jenis == "RRA" || this.sub_jenis == "RSH") )  {
						...
						if (this.app._sapOnline){									
							...
							for (var row=0; row < this.sg.getRowCount(); row++){
								...
								}else if (this.sub_jenis == "RSH" && nilaiToFloat(this.sg.cells(10,row)) > line.planBln){
			...
			if (sender == this.cb_pdrk && this.cb_pdrk.getText()!="") {
				...
				var data = this.dbLib.getMultiDataProvider(new server_util_arrayList({items:[
						   ...
						   "select file_dok, nama from rra_rev_dok where no_rev = '"+this.e_norev.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' order by no_urut",
							  "select distinct substring(a.periode,5,2) as bulan,a.kode_cc,b.nama as nama_cc,a.kode_drk,isnull(d.nama,'-') as nama_drk,a.kode_akun,c.nama as nama_akun,a.nilai_gar,a.nilai_pakai,a.saldo,a.nilai,a.dc,a.target, a.jenis "+
	...
	*/	
	...
	locateCC : function(sender, col, row, cc){
		var data = this.dbLib.getDataProvider("select a.nama from rra_cc a inner join rra_ubis b on b.kode_ubis = a.kode_ubis and b.kode_lokasi = a.kode_lokasi where a.kode_cc = '"+cc+"' and a.kode_gubis = '"+this.cb_gubis.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ",true);
		if (typeof data == "string"){
			error_log(data);
			return false;
		}
			...
			for (var i in this.data1Thn.objList){										
				...
				for (var l in row.objList){//rows = 16
					...
					for (var l2 in line.objList){//row																	
						...
						if (this.c_jenis.getText() == "OPEX"){						
							if (this.sg.cells(11,rowGrid) == "PAYMENT" ){
								var plan = line.get(l2).get("TSL09").split(".");
								var real = line.get(l2).get("TSL05").split(".");
								var act = line.get(l2).get("TSL03").split(".");												
							}else {
								var plan = line.get(l2).get("TSL10").split(".");
								var real = line.get(l2).get("TSL06").split(".");
								var act = line.get(l2).get("TSL04").split(".");		
							}
							if (act[1].indexOf("-") > -1) act[1] = parseFloat(act[1]) * -1;																										
							...
							if (line.get(l2).get("VERSN") == "000" && this.sg.cells(11,rowGrid) == "PAYMENT" ){
