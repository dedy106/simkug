window.app_rra_transaksi_fRRRsuka = function(owner)
{
	if (owner)
	{
		window.app_rra_transaksi_fRRRsuka.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fRRRsuka";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form PDRK Redistribusi, Realokasi, Reschedule Anggaran: SUKKA (UBIS)", 0);	
		...
		this.bLoad = new button(this.pc1.childPage[0], {bound:[this.pc1.width - 100, this.mDesk.top + 5, 80, 20], caption:"Load Template", click:"doCodeClick"});
		...
		try {
			...
			this.cb_pdrk.setSQL("select distinct a.no_pdrk,a.keterangan from rra_pdrk_m a "+
					"	inner join rra_rev_m b on a.no_pdrk=b.no_pdrk and a.kode_lokasi=b.kode_lokasi "+
					" inner join rra_frev_m c on c.no_pdrk = a.no_pdrk and c.kode_lokasi = a.kode_lokasi "+
					" where c.progress='1' and b.progress= '3' and b.kode_lokasi='"+this.app._lokasi+"' and b.kode_ubis = '"+this.app._kodeUbis+"' and b.nik_user = '"+this.app._userLog+"' "
					,["no_pdrk","keterangan"],false,["No PDRK","Keterangan"],"and","Data PDRK",true);
...
window.app_rra_transaksi_fRRRsuka.extend(window.childForm);
window.app_rra_transaksi_fRRRsuka.implement({
	...
	simpan: function(){			
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"rra_sukka","no_sukka","SUKKA-"+this.e_periode.getText().substr(2,4)+".","0000"));
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();								
					sql.add("update rra_rev_m set progress = '6' where no_pdrk in (select no_pdrk from rra_pdrk_m where no_pdrk='"+this.cb_pdrk.getText()+"' and kode_lokasi='"+this.app._lokasi+"') and kode_lokasi = '"+this.app._lokasi+"' ");
					...
					sql.add("insert into rra_sukka(no_sukka,periode,kode_lokasi,no_pdrk,tanggal,keterangan,nik_buat,nik_app,modul, deskripsi, justifikasi, kepada, nik_review) values "+
						    "('"+this.e_nb.getText()+"','"+this.e_periode.getText()+"','"+this.app._lokasi+"','"+this.cb_pdrk.getText()+"','"+this.dp_d1.getDateString()+"',null,'"+this.app._userLog+"','"+this.cb_app.getText()+"','SUKKA','"+this.e_ket.getText()+"',null,'"+this.m_kpd.getText()+"','"+this.cb_reviewer.getText()+"')");
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_pdrk && this.cb_pdrk.getText()!="") {
			...
			var strSQL = "select distinct bb.jenis_agg, substring(a.periode,5,2) as bulan,a.kode_cc,b.nama as nama_cc,a.kode_drk,isnull(d.nama,'-') as nama_drk,a.kode_akun,c.nama as nama_akun,a.nilai_gar,a.nilai_pakai,a.saldo,a.nilai,a.dc,a.target, bb.nik_app3, bb.nik_review "+
			             "from rra_frev_d a inner join rra_frev_m bb on a.no_frev=bb.no_frev and a.kode_lokasi=bb.kode_lokasi "+
