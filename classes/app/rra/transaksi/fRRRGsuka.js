window.app_rra_transaksi_fRRRGsuka = function(owner)
{
	if (owner)
	{
		window.app_rra_transaksi_fRRRGsuka.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fRRRGsuka";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form PRDRK Redistribusi, Realokasi, Reschedule Anggaran: SUKKA Group Bisnis", 0);	
		...
		this.e_nb = new portalui_saiLabelEdit(this,{bound:[20,12,200,20],caption:"No SUKKA",maxLength:30,readOnly:true});
		this.i_gen = new portalui_imageButton(this,{bound:[225,12,20,20],hint:"Generate",image:"icon/"+system.getThemes()+"/tabCont2.png",click:[this,"doClick"]});				
		this.e_ket = new saiLabelEdit(this,{bound:[20,14,450,20],caption:"Perihal", maxLength:150});		
		this.cb_pdrk = new saiCBBL(this,{bound:[20,13,220,20],caption:"No PDRK", multiSelection:false, tag:1, change:[this,"doChange"]});
		...
		this.cb_app = new saiCBBL(this,{bound:[20,15,200,20],caption:"NIK Approve", multiSelection:false, maxLength:10, bufferOption: boHALF, bufferData: this.app._listNIK, tag:1});		
		...
		this.mDesk2 = new tinymceCtrl(this.pc1.childPage[3],{bound:[1,10,this.pc1.width-2,this.pc1.height-125], withForm:false});
		...
		try {
			...
			this.cb_pdrk.setSQL("select a.no_pdrk,a.keterangan from rra_pdrk_m a "+
				"	inner join rra_grev_m b on a.no_pdrk=b.no_pdrk and a.kode_lokasi=b.kode_lokasi "+				
				" where b.progress='3' and b.kode_lokasi='"+this.app._lokasi+"' and b.kode_gubis = '"+this.app._kodeGubis+"' and b.nik_user = '"+this.app._userLog+"'",["no_pdrk","keterangan"],false,["No PDRK","Keterangan"],"and","Data PDRK",true);
			this.cb_app.setSQL("select nik, nama, jabatan from rra_karyawan where kode_lokasi='"+this.app._lokasi+"' and kode_gubis='"+this.app._kodeGubis+"'",["nik","nama","jabatan"],false,["NIK","Nama","Jabatan"],"and","Data Karyawan ",true);
...
window.app_rra_transaksi_fRRRGsuka.extend(window.childForm);
window.app_rra_transaksi_fRRRGsuka.implement({
	...
	simpan: function(){			
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"rra_sukka","no_sukka","GSUKKA-"+this.e_periode.getText().substr(2,4)+".","0000"));
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();								
					sql.add("update rra_grev_m set progress = '6' where no_pdrk in (select no_pdrk from rra_pdrk_m where no_pdrk='"+this.cb_pdrk.getText()+"' and kode_lokasi='"+this.app._lokasi+"') and kode_lokasi = '"+this.app._lokasi+"' ");					
					...
					sql.add("insert into rra_sukka(no_sukka,periode,kode_lokasi,no_pdrk,tanggal,keterangan,nik_buat,nik_app,modul,deskripsi, justifikasi, kepada, nik_review) values "+
						    "('"+this.e_nb.getText()+"','"+this.e_periode.getText()+"','"+this.app._lokasi+"','"+this.cb_pdrk.getText()+"','"+this.dp_d1.getDateString()+"',null,'"+this.app._userLog+"','"+this.cb_app.getText()+"','GSUKKA','"+this.e_ket.getText()+"',null,'"+this.m_kpd.getText()+"','"+this.cb_reviewer.getText()+"')");
	...
	doChange:function(sender){
		...
		if (sender == this.cb_pdrk && this.cb_pdrk.getText()!="") {
			...
			var strSQL = "select substring(a.periode,5,2) as bulan,a.kode_cc,b.nama as nama_cc,a.kode_drk,isnull(d.nama,'-') as nama_drk,a.kode_akun,c.nama as nama_akun,a.nilai_gar,a.nilai_pakai,a.saldo,a.nilai,a.dc,a.target, bb.nik_review, bb.nik_app3 "+
			             "from rra_grev_d a inner join rra_grev_m bb on a.no_grev=bb.no_grev and a.kode_lokasi=bb.kode_lokasi "+
			...
			this.e_ket.setText(sender.dataFromList[1]);
		}
	},
	...
	doRequestReady: function(sender, methodName, result, callbackObj){
		if (sender == this.dbLib && this == calbackObj){
			try{   
				switch(methodName){
	    			case "execArraySQL" :	    				
