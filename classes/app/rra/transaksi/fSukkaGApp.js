window.app_rra_transaksi_fSukkaGApp = function(owner)
{
	if (owner)
	{
		window.app_rra_transaksi_fSukkaGApp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fSukkaGApp";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form PRDRK Redistribusi, Realokasi, Reschedule Anggaran: Approve SUKKA Group Bisnis", 0);	
...
window.app_rra_transaksi_fSukkaGApp.extend(window.childForm);
window.app_rra_transaksi_fSukkaGApp.implement({
	...
	simpan: function(){			
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{				
					this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"rra_app_m","no_app","APPGSK-"+this.e_periode.getText().substr(2,4)+".","0000"));
					...
					sql.add("insert into rra_app_m(no_app, kode_lokasi,tanggal,keterangan,modul,periode,no_del,nik_buat,nik_app,nik_user,tgl_input,jenis_form) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.c_modul.getText()+"','"+this.e_periode.getText()+"','-','"+this.app._userLog+"','"+this.app._userLog+"','"+this.app._userLog+"',now(),'APPGSK')");
					var line;
					...
					sql.add("update rra_grev_m set progress = '5' where no_grev in (select no_bukti from  rra_app_d where no_app='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"') and kode_lokasi = '"+this.app._lokasi+"' ");
		...
		if (sender == this.c_modul) {
			if (this.c_modul.getText() != "") {
				...
				switch(this.c_modul.getText()){
					case "RRR" :
								strSQL = "select 'INPROG' as status,a.no_grev as no_bukti,a.no_pdrk as no_dokumen,date_format(b.tanggal,'%d-%m-%Y') as tanggal,b.keterangan,a.jenis_agg as jenis "+
										 "from rra_grev_m a inner join rra_pdrk_m b on a.no_pdrk=b.no_pdrk and a.kode_lokasi=b.kode_lokasi "+
										 "where a.progress='4' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
						break;
				}				
