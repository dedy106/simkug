window.app_rra_transaksi_fRRRGSukaRev = function(owner)
{
	if (owner)
	{
		window.app_rra_transaksi_fRRRGSukaRev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fRRRGSukaRev";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form PRDRK Redistribusi, Realokasi, Reschedule Anggaran: Review SUKKA", 0);	
		...
		this.e_nb = new saiCBBL(this,{bound:[20,13,220,20],caption:"No SUKKA",maxLength:30,readOnly:true,
			multiSelection:false, change:[this,"doChange"],
			sql:["select a.no_sukka, a.deskripsi from rra_sukka a inner join rra_grev_m b on b.no_pdrk = a.no_pdrk and b.kode_lokasi = a.kode_lokasi  "+
				 " where b.progress = '6' and b.kode_ubis = '"+this.app._kodeUbis+"' and a.kode_lokasi = '"+this.app._lokasi+"' ",["no_sukka","deskripsi"],false,["No SUKKA","Keterangan"],"and","Daftar Draft SUKKA",true]
		});		
		...
		this.e_donor = new saiLabelEdit(this,{bound:[720,13,220,20],caption:"Nilai Donor", readOnly:true, tipeText:ttNilai, text:"0"});		
		...
		this.mDesk = new tinymceCtrl(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-10], withForm:false});
		this.mDesk2 = new tinymceCtrl(this.pc1.childPage[3],{bound:[1,60,this.pc1.width-5,this.pc1.height-125], withForm:false});
		...
		try {
			...
			this.cb_pdrk.setSQL("select a.no_pdrk,a.keterangan from rra_pdrk_m a "+
				...
				" where b.progress='6' and b.kode_lokasi='"+this.app._lokasi+"' and ifnull(c.nik_review,b.nik_buat) = '"+this.app._userLog+"'",["no_pdrk","keterangan"],false,["No PDRK","Keterangan"],"and","Data PDRK",true);
...
window.app_rra_transaksi_fRRRGSukaRev.extend(window.childForm);
window.app_rra_transaksi_fRRRGSukaRev.implement({
	...
	simpan: function(){			
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					this.e_sukka.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"rra_nosukka","no_surat","/KU250/"+this.app._kodeBA+"/"+this.e_periode.getText().substr(0,4)+"","0000","",true));					
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();								
					sql.add("update rra_grev_m set progress = '4' where no_pdrk ='"+this.cb_pdrk.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");					
	...
	doChange:function(sender){		
		try{
			if (sender == this.e_nb){
				this.no_sukka = "";
				var data = this.dbLib.getDataProvider("select a.no_pdrk, a.no_sukka, date_format(a.tanggal, '%d-%m-%Y') as tgl, a.keterangan, a.deskripsi, a.nik_app, justifikasi, kepada  "+
