window.app_rra_transaksi_fCPXFCRev = function(owner)
{
	if (owner)
	{
		window.app_rra_transaksi_fCPXFCRev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fCPXFCRev";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form PDRK RRA Anggaran: Review FC Sebelum Keep CAPEX", 0);	
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:7,tag:0,
					...
					colWidth:[[6,5,4,3,2,1,0],[80,50,200,70,150,150,80]],
					columnReadOnly:[true,[0,1,2,3,4,5],[]],
					buttonStyle:[[0,6],[bsAuto, bsEllips]], ellipsClick:[this,"doFindBtn"],
...
window.app_rra_transaksi_fCPXFCRev.extend(window.childForm);
window.app_rra_transaksi_fCPXFCRev.implement({
				...
				switch(this.c_modul.getText()){
					case "RRR" :					
								strSQL = "select 'INPROG' as status,a.no_rev as no_bukti,a.no_pdrk as no_dokumen,date_format(b.tanggal,'%d-%m-%Y') as tanggal,b.keterangan,'REV' as jenis, '"+this.app._userLog+"' as nik_keep "+
										 ...
										 "where a.sts_pdrk='RRR' and a.progress in '0' and a.flag_rfc='-' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' "+
										 "union "+
										 ...
										 (this.app._statusLokasiUser == "FC" ? "" : " inner join rra_finop_ubis cc on cc.kode_ubis = a.kode_ubis and cc.kode_lokasi = a.kode_lokasi and cc.kode_cc = '"+this.app._kodeCC+"' ")+
										 "where a.sts_pdrk='RRR' and a.progress='0' and a.flag_rfc='-' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' "+
										 "union "+
	...
	doFindBtn: function(sender, col, row){
		if (col == 6)
