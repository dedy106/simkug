window.app_rra_transaksi_fRevFCOP = function(owner)
{
	if (owner)
	{
		window.app_rra_transaksi_fRevFCOP.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fRevFCOP";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form PDRK Redistribusi, Realokasi, Reschedule Anggaran: Review FC FINOP", 0);	
		...
		this.pc1 = new pageControl(this,{bound:[20,16,900, 350], childPage:["Data PDRK","Data Donor","Data Penerima","Data Justifikasi","Dokumen Pendukung","View Dokumen","Nota Kajian"], pageChange:[this,"doPageChange"]});
		...
		try {
			...
			this.cb_pdrk.setSQL("select a.no_pdrk, a.keterangan, b.no_rev "+
							 "from rra_pdrk_m a "+
							 " inner join rra_rev_m b on b.no_pdrk = a.no_pdrk and b.kode_lokasi = a.kode_lokasi "+
							 " where b.sts_pdrk in ('ABT') and b.progress='A' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' "
							 ,["no_pdrk","keterangan","b.no_rev"],false,["No PDRK","Keterangan"],"and","Data PDRK",true);
...
window.app_rra_transaksi_fRevFCOP.extend(window.childForm);
window.app_rra_transaksi_fRevFCOP.implement({
		...
		try{				
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{
					...
					this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"rra_frev_m","no_frev","FINOP-"+this.app._kodeUbis+"-"+this.e_periode.getText().substr(2,4)+".","0000"));					
					...
					sql.add("update rra_rev_m set progress='-' where no_pdrk='"+this.cb_pdrk.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
						...
						for (var l in row.objList){//rows = 16
							...
							{//row																	
								...
								if (this.c_jenis.getText() == "CAPEX")
									...
									sql.add("insert into rra_frev_orgi(no_frev, no_pdrk,periode, kode_akun, kode_cc, nilai,kode_lokasi)values ('"+this.e_nb.getText()+"','"+this.cb_pdrk.getText()+"','"+line.get("GJAHR")+prd+"','"+gridValue.get("akun")+"','"+gridValue.get("cc")+"','"+line.get("TSL06")+"','"+this.app._lokasi+"')");
							}								
						}
					}
					this.keepSAP = true;
		...
		try{
			...
			if (sender == this.cb_pdrk && this.cb_pdrk.getText()!="") {				
				...
				var data = this.dbLib.getMultiDataProvider(new server_util_arrayList({items:[
						   ...
						   "select distinct substring(a.periode,5,2) as bulan,a.kode_cc,b.nama as nama_cc,a.kode_drk,isnull(d.nama,'-') as nama_drk,a.kode_akun,c.nama as nama_akun,a.nilai_gar,a.nilai_pakai,a.saldo,a.nilai,a.dc,a.target, a.jenis, a.nilai_keep "+
	    			...
	    			case "execArraySQL" :	    	
						...
						if (result.toLowerCase().search("error") == -1)					
						{
							if (this.saveFiles != ""){//baru
                                ...
                                system.info(this,"transaksi telah sukses tersimpan","(ID : "+ this.e_nb.getText()+")");						
								this.standarLib.clearByTag(this, ["0","1"],this.e_nb, undefined, this);		
								this.sgUpld.clear(1);
								setTipeButton(tbSimpan);
							}
						}else {
