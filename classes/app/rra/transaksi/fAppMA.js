window.app_rra_transaksi_fAppMA = function(owner)
{
	if (owner)
	{
		window.app_rra_transaksi_fAppMA.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fAppMA";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form PRDRK ABT Anggaran: Approve MA", 0);	
		
		uses("saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;server_report_report;reportViewer");
		...
		this.pc1 = new pageControl(this,{bound:[20,12,900,287], childPage:["Data RRA","Detail Transaksi","PDRK","Dokumen Pendukung","Viewer"]});
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:7,tag:0,
					...
					colWidth:[[6,5,4,3,2,1,0],[50,290,70,200,150,150,80]],
...
window.app_rra_transaksi_fAppMA.extend(window.childForm);
window.app_rra_transaksi_fAppMA.implement({
	...
	simpan: function(){			
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{											
					this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"rra_app_m","no_app","MAPP-"+this.e_periode.getText().substr(2,4)+".","0000"));
					...
					sql.add("insert into rra_app_m(no_app, kode_lokasi,tanggal,keterangan,modul,periode,no_del,nik_buat,nik_app,nik_user,tgl_input,jenis_form) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.c_modul.getText()+"','"+this.e_periode.getText()+"','-','"+this.app._userLog+"','"+this.app._userLog+"','"+this.app._userLog+"',now(),'MAPP')");
					var line;
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						...
						if (line.status.toUpperCase() == "APP"){
							...
							sql.add("update rra_pdrk_m set progress='7' where no_pdrk = '"+line.no_dokumen+"' and kode_lokasi = '"+this.app._lokasi+"'");
							sql.add("update rra_mrev_m set progress = '1' where no_mrev = '"+line.no_bukti+"' and kode_lokasi='"+this.app._lokasi+"' ");					
						}else {
							switch(line.status.toUpperCase()){
								case "KEMBALI" : 
									sql.add("insert into rra_app_d(no_app,modul,no_bukti,kode_lokasi,sts_pdrk,catatan, keterangan) values "+
										"('"+this.e_nb.getText()+"','"+this.c_modul.getText()+"','"+line.no_bukti+"','"+this.app._lokasi+"','-','"+line.no_dokumen+"','"+line.catatan+"')");
									sql.add("update rra_mrev_m set progress='R',lastprogress='"+line.progress+"' where no_mrev = '"+line.no_bukti+"' and kode_lokasi = '"+this.app._lokasi+"'");
									...
									switch (line.progress){
										case "0":
											//kirim ke reviewer.											
											//sql.add("update rra_grev_m set progress='R' where no_rev = '"+line.no_bukti+"' and kode_lokasi = '"+this.app._lokasi+"'");
											//insert into rra_pesan
										break;
										case "0":
											//sql.add("update rra_grev_m set progress='0' where no_rev = '"+line.no_bukti+"' and kode_lokasi = '"+this.app._lokasi+"'");
											//insert into rra_pesan
								...
								case "BATAL" :
									sql.add("insert into rra_app_d(no_app,modul,no_bukti,kode_lokasi,sts_pdrk,catatan,keterangan) values "+
										"('"+this.e_nb.getText()+"','"+this.c_modul.getText()+"','"+line.no_bukti+"','"+this.app._lokasi+"','-','"+line.no_dokumen+"','"+line.catatan+"')");							
									sql.add("update rra_mrev_m set progress='D',lastprogress='"+line.progress+"' where no_mrev = '"+line.no_bukti+"' and kode_lokasi = '"+this.app._lokasi+"'");
									sql.add("insert into rra_pesan(no_pesan, kode_lokasi, tanggal, pengirim, penerima, judul, keterangan, periode, flag_email, nik_user, tgl_input, flag_read, flag_sms, no_dokumen) values "+
										"( '"+this.e_nb.getText()+"','"+this.app._lokasi+"',now(),'"+this.app._userLog+"','"+line.nik_user+"','Dokumen kembali "+line.no_dokumen+"','"+line.catatan+"','"+this.e_periode.getText()+"','0','"+this.app._userLog+"',now(),'0','0','"+line.no_dokumen+"')");
					...
					//sql.add("update rra_mrev_m set progress = '1' where no_mrev in (select no_bukti from rra_app_d where no_app='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"') and kode_lokasi='"+this.app._lokasi+"' ");														
		...
		if (sender == this.c_modul) {
			if (this.c_modul.getText() != "") {
				...
				switch(this.c_modul.getText()){
					case "RRR" :
								strSQL = "select 'INPROG' as status,a.no_mrev as no_bukti,a.no_pdrk as no_dokumen,date_format(b.tanggal,'%d-%m-%Y') as tanggal,b.keterangan,a.jenis_agg as jenis "+
					...
					case "ABT" :
								strSQL = "select 'INPROG' as status,a.no_mrev as no_bukti,a.no_pdrk as no_dokumen,date_format(b.tanggal,'%d-%m-%Y') as tanggal,b.keterangan,a.jenis_agg as jenis "+
										 "from rra_mrev_m a inner join rra_pdrk_m b on a.no_pdrk=b.no_pdrk and a.kode_lokasi=b.kode_lokasi "+
										 "where a.sts_pdrk = 'ABT' and a.progress='0' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
						break;
				}
	...
	doDoubleClick: function(sender, col , row) {
		if (this.sg.cells(1,row) != "") {
			...
			var data = this.dbLib.getMultiDataProvider(new server_util_arrayList({items:[strSQL,
						"select file_dok, nama from rra_mrev_dok where no_mrev = '"+this.sg.cells(2,row)+"' and kode_lokasi = '"+this.app._lokasi+"' order by no_urut"
					]}),true);
