window.app_rra_transaksi_fCPXRevFC = function(owner)
{
	if (owner)
	{
		window.app_rra_transaksi_fCPXRevFC.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fCPXRevFC";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form PDRK Redistribusi, Realokasi, Reschedule Anggaran: Review FC (CAPEX)", 0);	
		...
		this.pc1 = new pageControl(this,{bound:[20,16,900, 350], childPage:["Data PDRK","Data Donor","Data Penerima","Data Justifikasi","Dokumen Pendukung","View Dokumen","Dokumen PDRK"], pageChange:[this,"doPageChange"]});
		...
		try {
			...
			this.cb_pdrk.setSQL("select a.no_pdrk,b.keterangan,'REV' as modul, a.no_rev as no_review "+
										 ...
										 "where lower(a.jenis_agg) = 'capex' and a.sts_pdrk in ('RRR',"+(this.app._statusLokasiUser == "FC" ? "'STB'" : "' '")+") and a.progress='0' and a.flag_rfc='0' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' "+										 
										 "union "+
...
window.app_rra_transaksi_fCPXRevFC.extend(window.childForm);
window.app_rra_transaksi_fCPXRevFC.implement({
		...
		try{				
					...
					if (typeof data != "string"){
						...
						for (var i in data.rs.rows){
							...
							if (line.jns.toLowerCase() == "ma" && line.prd > this.app._periodeGAR)
