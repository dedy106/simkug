window.app_rra_transaksi_fPraRRRRev = function(owner)
{
	if (owner)
	{
		window.app_rra_transaksi_fPraRRRRev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fPraRRRRev";
		this.itemsValue = new arrayList();
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Draft PDRK Redistribusi, Realokasi, Reschedule Anggaran: Review", 0);	
		...
		this.e_nb = new saiLabelEdit(this,{bound:[20,12,222,20],caption:"No Approve",maxLength:30,readOnly:true, change:[this,"doChange"]});
...
window.app_rra_transaksi_fPraRRRRev.extend(window.childForm);
window.app_rra_transaksi_fPraRRRRev.implement({
	...
	doChange:function(sender){
		...
		if (sender == this.c_modul) {
			if (this.c_modul.getText() != "") {
				var strSQL = "select 'INPROG' as status,a.no_pdrk as no_bukti,a.no_pdrk as no_dokumen,date_format(a.tanggal,'%d-%m-%Y') as tanggal,a.keterangan,a.jenis_agg as jenis "+
										 "from rra_pra_m a "+
										 "where a.sts_pdrk = '"+this.c_modul.getText()+"' and a.progress='0' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and kode_ubis = '"+this.app._kodeUbis+"' and a.nik_appjust = '"+this.app._userLog+"'";				
		...
		if (this.sg.cells(1,row) != "") {
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				var url = [
					...
					this.report.previewWithHeader("server_report_rra_rptPdrk3Pra",this.filter, 1,  1, this.showFilter, this.app._namaLokasi,this.filter2)
					];
