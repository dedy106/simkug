window.app_rra_transaksi_fAppK = function(owner)
{
	if (owner)
	{
		window.app_rra_transaksi_fAppK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fAppK";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form PRDRK Redistribusi, Realokasi, Reschedule Anggaran: Approve (Koreksi)", 0);	
...
window.app_rra_transaksi_fAppK.extend(window.childForm);
window.app_rra_transaksi_fAppK.implement({
	...
	simpan: function(){			
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{											
					this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"rra_app_m","no_app","APP-"+this.e_periode.getText().substr(2,4)+".","0000"));
					...
					sql.add("update rra_rev_m set progress = '1' where no_rev in (select no_bukti from rra_app_d where no_app='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"') and kode_lokasi='"+this.app._lokasi+"' ");
	...
	doClick:function(sender){
		if (sender == this.i_gen) {
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"rra_app_m","no_app","APP-"+this.e_periode.getText().substr(2,4)+".","0000",true));
			//this.e_ket.setFocus();
		}
		...
		if (sender == this.c_modul) {
			if (this.c_modul.getText() != "") {
				...
				switch(this.c_modul.getText()){
					case "RRR" :
								strSQL = "select 'INPROG' as status,a.no_rev as no_bukti,a.no_pdrk as no_dokumen,date_format(b.tanggal,'%d-%m-%Y') as tanggal,b.keterangan,a.jenis_agg as jenis "+
