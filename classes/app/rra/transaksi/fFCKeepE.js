window.app_rra_transaksi_fFCKeepE = function(owner)
{
	if (owner)
	{
		window.app_rra_transaksi_fFCKeepE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fFCKeepE";
		this.itemsValue = new arrayList();
		if (this.showLoading) this.showLoading("Generate UI");
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form PDRK RRA Anggaran: Reverse Release Budget FC", 0);	
		
		uses("saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;util_rfc;server_report_report;util_rfcLib");
...
window.app_rra_transaksi_fFCKeepE.extend(window.childForm);
window.app_rra_transaksi_fFCKeepE.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					if (this.c_modul.getText() == "OPN"){//open hold
						...
						"select 'GB' as jns,a.no_grev as no_bukti,a.kode_akun, a.kode_cc, a.kode_drk,substr(a.periode,5,2) as periode, a.nilai, a.periode as prd, a.jenis as versi from rra_grev_d a where a.no_grev in ("+noRRR+") and dc='D' "+						
						") a order by versi,periode, kode_cc, kode_akun ",true);
					}else if (this.c_modul.getText() == "STB"){//standby budget
						var data = this.dbLib.getDataProvider(
						"select jns, no_bukti, kode_akun, kode_cc, kode_drk, periode, nilai, prd,versi from ("+						
						"select 'UB' as jns,a.no_rev as no_bukti,a.kode_akun, a.kode_cc, a.kode_drk,substr(a.periode,5,2) as periode, a.nilai, a.periode as prd, a.jenis as versi  from rra_rev_d a where a.no_rev in ("+noRRR+") and dc='D' "+						
						") a order by versi,periode, kode_cc, kode_akun ",true);
					...
					if (typeof data != "string"){
						...
						for (var i in data.rs.rows){
							line = data.rs.rows[i];							
							var nilai = (this.c_modul.getText() == "RRR" ? "":"-")+line.nilai.toString() ;
		...
		if (sender == this.c_modul || sender == this.c_jenis) {
			 {
				...
				var addFilter = this.c_modul.getText() == "RRR" ? "a.flag_rfc = '1'": this.c_modul.getText() == "STB" ? "a.flag_rfc = '2' and a.progress = '3' " :"a.flag_rfc = '2' and a.progress = '3' ";
				var strSQL = "select 'INPROG' as status,a.no_rev as no_bukti,a.no_pdrk as no_dokumen,date_format(a.tanggal,'%d-%m-%Y') as tanggal,a.keterangan,'REV' as jenis "+
						 "from  rra_rev_m a "+
						 (this.app._statusLokasiUser == "FC" ? "" : "	inner join rra_finop_ubis cc on cc.kode_ubis = a.kode_ubis and cc.kode_lokasi = a.kode_lokasi and cc.kode_cc = '"+this.app._kodeCC+"' ")+
						 "where a.sts_pdrk='"+this.c_modul.getText()+"' and a.jenis_agg = '"+this.c_jenis.getText()+"' and "+addFilter+" and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and ((a.nik_keep = '"+this.app._userLog+"' and a.sts_pdrk = 'RRR') or (a.sts_pdrk in ('STB') ) ) "+
						 " union "+
						 ...
						 "where a.sts_pdrk='"+this.c_modul.getText()+"' and a.jenis_agg = '"+this.c_jenis.getText()+"' and "+addFilter+" and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and ((a.nik_keep = '"+this.app._userLog+"' and a.sts_pdrk = 'RRR') or (a.sts_pdrk in ('OPN') ) ) "+
						 " union "+										 
