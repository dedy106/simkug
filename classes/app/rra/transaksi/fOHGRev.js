window.app_rra_transaksi_fOHGRev = function(owner)
{
	if (owner)
	{
		try{
			window.app_rra_transaksi_fOHGRev.prototype.parent.constructor.call(this,owner);
			this.className  = "app_rra_transaksi_fOHGRev";
			...
			this.cb_pdrk = new saiCBBL(this,{bound:[20,11,222,20],caption:"No Dokumen", multiSelection:false, tag:1, change:[this,"doChange"]});
            this.e_uid = new saiLabelEdit(this,{bound:[720,11,200,20], caption:"SAP User", tag:11});
            this.e_norev = new saiLabelEdit(this,{bound:[20,13,222,20],caption:"No Review", readOnly:true});		
			...
			this.cb_pdrk.setSQL("select a.no_pdrk,a.keterangan,b.no_rev from rra_pdrk_m a inner join rra_rev_m b on a.no_pdrk=b.no_pdrk and a.kode_lokasi=b.kode_lokasi "+
			                    "where b.progress='1' and b.sts_pdrk = 'OPN' and b.kode_lokasi='"+this.app._lokasi+"' and b.kode_gubis = '"+this.app._kodeGubis+"'",["no_pdrk","keterangan","no_rev"],false,["No PDRK","Keterangan","No Review UBIS"],"and","Data PDRK",true);
...
window.app_rra_transaksi_fOHGRev.extend(window.childForm);
window.app_rra_transaksi_fOHGRev.implement({
	...
	simpan: function(){			
		try{							
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"rra_grev_m","no_grev","GREV-"+this.app._kodeUbis+"-"+this.e_periode.getText().substr(2,4)+".","0000"));					
					...
					sql.add("update rra_pdrk_m set progress='2'  where no_pdrk='"+this.cb_pdrk.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add("delete from rra_grev_orgi where no_grev = '"+this.cb_pdrk.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");										
					if (this.sg2.getRowValidCount() > 0){
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								sql.add("insert into rra_grev_d(no_grev,no_pdrk,kode_lokasi,no_urut,kode_akun,kode_cc,kode_drk,periode,nilai_gar,nilai_pakai,saldo,nilai,dc,target,jenis) values "+
					...
					for (var i in this.data1Thn.objList){										
						...
						for (var l in row.objList){//rows = 16
							...
							for (var l2 in line.objList){//row																	
								...
								sql.add("insert into rra_grev_orgi(no_grev, no_pdrk,periode, kode_akun, kode_cc, nilai,kode_lokasi,versi)values "+
								...
								sql.add("insert into rra_grev_orgi(no_grev, no_pdrk,periode, kode_akun, kode_cc, nilai,kode_lokasi,versi)values ('"+this.e_nb.getText()+"','"+this.cb_pdrk.getText()+"','"+line.get(l2).get("GJAHR")+prd+"','"+gridValue.get("akun")+"','"+gridValue.get("cc")+"','"+line.get(l2).get("TSL06")+"','"+this.app._lokasi+"','R')");
							}								
						}
					}			
						...
						for (var l in row.objList){//rows = 16
							...
							for (var l2 in line.objList){//row									
								...
								if (this.c_jenis.getText() == "CAPEX")
									line.get(l2).set("PLAN", parseFloat(line.get(l2).get("PLAN")) - rrr);	
								else line.get(l2).set("TSL10", parseFloat(line.get(l2).get("TSL10")) - rrr);	
							}								
						}
					}
				}catch(e){
		...
		try{
			...
			if (sender == this.cb_pdrk && this.cb_pdrk.getText()!="") {
				...
				var data = this.dbLib.getMultiDataProvider(new server_util_arrayList({items:[
						   "select a.jenis_agg,a.kode_ubis,a.kode_gubis,a.nik_app1,a.nik_app2,a.nik_app3,b.nama as nama_ubis,c.nama as nama_gubis,d.nama as nama_app1,e.nama as nama_app2,f.nama as nama_app3,a.justifikasi,a.sts_pdrk, a.kode_kota, a.file_nd, a.no_nd, a.nik_appjust, a.nik_appjust2, a.nik_apppdrk3, a.nik_apppdrk32   "+
						   ...
						   "where a.no_pdrk='"+this.cb_pdrk.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",
                            "select file_dok, nama from rra_rev_dok a inner join rra_rev_m b on b.no_rev = a.no_rev and b.kode_lokasi= a.kode_lokasi   where no_pdrk = '"+this.cb_pdrk.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' order by no_urut",
