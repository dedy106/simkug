/***********************************
 * 15 - juli - 2011 
 * Penambahan Nota Pembatalan antar Group Bisnis dan Unit Bisnis, ditujukan ke KA UBIS
 * 
 *  
 */
window.app_rra_transaksi_fAppGubis = function(owner)
{
	if (owner)
	{
		window.app_rra_transaksi_fAppGubis.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fAppGubis";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form PDRK ABT Anggaran: Approve Group Bisnis", 0);	
		...
		this.c_modul = new saiCB(this,{bound:[20,22,200,20],caption:"Modul",items:["RRR","ABT","OPN"], readOnly:true,tag:2,change:[this,"doChange"]});		
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:7,tag:0,
					...
					picklist:[[0],[new portalui_arrayMap({items:["APP","KEMBALI","BATAL","INPROG"]})]],
...
window.app_rra_transaksi_fAppGubis.extend(window.childForm);
window.app_rra_transaksi_fAppGubis.implement({
	...
	simpan: function(){			
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{											
					this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"rra_app_m","no_app","GAPP-"+this.e_periode.getText().substr(2,4)+".","0000"));
					...
					sql.add("insert into rra_app_m(no_app, kode_lokasi,tanggal,keterangan,modul,periode,no_del,nik_buat,nik_app,nik_user,tgl_input,jenis_form) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.c_modul.getText()+"','"+this.e_periode.getText()+"','-','"+this.app._userLog+"','"+this.app._userLog+"','"+this.app._userLog+"',now(),'GAPP')");
					var line;
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						...
						if (line.status.toUpperCase() == "APP"){
							...
							sql.add("update rra_pdrk_m set progress='5' where no_pdrk = '"+line.no_dokumen+"' and kode_lokasi = '"+this.app._lokasi+"'");
							sql.add("update rra_grev_m set progress = '1' where no_grev = '"+line.no_bukti+"' and kode_lokasi='"+this.app._lokasi+"' ");					
						}else {
							switch(line.status.toUpperCase()){
								case "KEMBALI" : 
									sql.add("insert into rra_app_d(no_app,modul,no_bukti,kode_lokasi,sts_pdrk,catatan, keterangan) values "+
										"('"+this.e_nb.getText()+"','"+this.c_modul.getText()+"','"+line.no_bukti+"','"+this.app._lokasi+"','-','"+line.no_dokumen+"','"+line.catatan+"')");
									sql.add("update rra_grev_m set progress='R',lastprogress='"+line.progress+"' where no_grev = '"+line.no_bukti+"' and kode_lokasi = '"+this.app._lokasi+"'");
								...
								case "BATAL" :
									sql.add("insert into rra_app_d(no_app,modul,no_bukti,kode_lokasi,sts_pdrk,catatan, keterangan) values "+
										"('"+this.e_nb.getText()+"','"+this.c_modul.getText()+"','"+line.no_bukti+"','"+this.app._lokasi+"','-','"+line.no_dokumen+"','"+line.catatan+"')");							
									sql.add("update rra_grev_m set progress='D',lastprogress='"+line.progress+"' where no_grev = '"+line.no_bukti+"' and kode_lokasi = '"+this.app._lokasi+"'");
									sql.add("insert into rra_pesan(no_pesan, kode_lokasi, tanggal, pengirim, penerima, judul, keterangan, periode, flag_email, nik_user, tgl_input, flag_read, flag_sms, no_dokumen) values "+
										"( '"+this.e_nb.getText()+"','"+this.app._lokasi+"',now(),'"+this.app._userLog+"','"+line.nik_user+"','Dokumen kembali "+line.no_dokumen+"','"+line.catatan+"','"+this.e_periode.getText()+"','0','"+this.app._userLog+"',now(),'0','0','"+line.no_dokumen+"')");
	...
	doChange:function(sender){
		...
		if (sender == this.c_modul) {
			if (this.c_modul.getText() != "") {
				var strSQL = "select '-' as catatan,'INPROG' as status,a.no_grev as no_bukti,a.no_pdrk as no_dokumen,date_format(b.tanggal,'%d-%m-%Y') as tanggal,b.keterangan,a.jenis_agg as jenis "+
						"from rra_grev_m a inner join rra_pdrk_m b on a.no_pdrk=b.no_pdrk and a.kode_lokasi=b.kode_lokasi "+
						"where a.sts_pdrk = '"+this.c_modul.getText()+"' and a.progress='0' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
