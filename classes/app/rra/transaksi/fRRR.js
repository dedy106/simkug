 ...
 * 			-> 
 * */
window.app_rra_transaksi_fRRR = function(owner, options)
{
	if (owner)
	{
		window.app_rra_transaksi_fRRR.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fRRR";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form PDRK Redistribusi & Realokasi Anggaran: Input", 0);	
		...
		this.pc1 = new pageControl(this,{bound:[20,16,900, 350], childPage:["Data PDRK","Data Donor","Data Penerima","Data Justifikasi","Dokumen Pendukung","Viewer","Nota Kajian (CAPEX)"]});
		...
		this.bOpen = new button(this.pc1.childPage[0], {bound:[520,2,80,20], caption:"Open", click:[this,"doOpenFile"],visible:false});
		...
		this.cb_buat = new saiCBBL(this.pc1.childPage[0],{bound:[20,12,200,20],caption:"NIK Reviewer", multiSelection:false, maxLength:10, bufferOption: boHALF, bufferData: this.app._listNIK, tag:2, text:this.app._userLog, rightLabelCaption:this.app._namaUser});
		this.cb_kota = new saiCBBL(this.pc1.childPage[0],{bound:[20,17,200,20],caption:"Kota", multiSelection:false, bufferOption: boHALF, bufferData: this.app._dataKota, maxLength:10, tag:2});
		...
		this.mDesk2 = new tinymceCtrl(this.pc1.childPage[6],{bound:[1,10,this.pc1.width-5,this.pc1.height-15], withForm:false});
		...
		try {			
			...
			this.cb_rev2.setSQL("select nik, nama, jabatan from rra_karyawan where kode_lokasi='"+this.app._lokasi+"' and kode_ubis = '"+this.app._kodeUbis+"'",["nik","nama","jabatan"],false,["NIK","Nama","Jabatan"],"and","Data Karyawan ",true);
			this.cb_rev3.setSQL("select nik, nama, jabatan from rra_karyawan where kode_lokasi='"+this.app._lokasi+"' and kode_ubis = '"+this.app._kodeUbis+"'",["nik","nama","jabatan"],false,["NIK","Nama","Jabatan"],"and","Data Karyawan",true);
			this.cb_rev4.setSQL("select nik, nama, jabatan from rra_karyawan where kode_lokasi='"+this.app._lokasi+"' and kode_ubis = '"+this.app._kodeUbis+"'",["nik","nama","jabatan"],false,["NIK","Nama","Jabatan"],"and","Data Karyawan",true);
			this.cb_rev5.setSQL("select nik, nama, jabatan from rra_karyawan where kode_lokasi='"+this.app._lokasi+"' and kode_ubis = '"+this.app._kodeUbis+"'",["nik","nama","jabatan"],false,["NIK","Nama","Jabatan"],"and","Data Karyawan",true);
			...
			this.cb_rev2.setText("-");
			this.cb_rev3.setText("-");
			this.cb_rev4.setText("-");
			this.cb_rev5.setText("-");
...
window.app_rra_transaksi_fRRR.extend(window.childForm);
window.app_rra_transaksi_fRRR.implement({
	...
	mainButtonClick: function(sender){
		...
		if (sender == this.app._mainForm.bDraft)
			system.confirm(this, "simpandraft", "Apa anda yakin data akan disimpan di draft?","Data akan disimpan di draft.");			
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{
					...
					sql.add("insert into rra_pdrk_m(no_pdrk,kode_lokasi,keterangan,kode_ubis,kode_gubis,jenis_agg,tanggal,periode,nik_buat,nik_app1,nik_app2,nik_app3,sts_pdrk,justifikasi, nik_user, tgl_input,progress, kode_kota, no_nd, file_nd, flag_rfc, nik_appjust, nik_appjust2, nik_apppdrk3, nik_apppdrk32, sts_appjust, sts_appjust2, sts_apppdrk3, sts_apppdrk32, sub_jenis,no_draft, nik_review,nik_review2,nik_review3,nik_review4,nik_review5 ) values "+//
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_ket.getText()+"','"+this.cb_ubis.getText()+"','"+this.cb_gubis.getText()+"','"+this.c_jenis.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_periode.getText()+"','"+this.cb_buat.getText()+"','"+this.cb_app1.getText()+"','"+this.cb_app2.getText()+"','"+this.cb_app3.getText()+"','RRR',null, "+
								"'"+this.app._userLog+"',now(),'0','"+this.cb_kota.getText()+"','"+this.ed_dok.getText()+"','"+this.e_file.getText()+"','0','"+this.cb_appj1.getText()+"','"+this.cb_appj2.getText()+"','"+this.cb_appp3.getText()+"','"+this.cb_appp32.getText()+"','0','0','0','0','RRA','"+this.cbDraft.getText()+"','"+this.cb_reviewer.getText()+"','"+this.cb_rev2.getText()+"','"+this.cb_rev3.getText()+"','"+this.cb_rev4.getText()+"','"+this.cb_rev5.getText()+"')");
					...
					sql.add(new server_util_Map({items:{tipe:"clob",table:"rra_pdrk_m",field:"kajian", filter:" no_pdrk = '"+this.e_nb.getText()+"' ",value:urlencode(this.mDesk2.getCode())}})); 	
				...
				try{
					...
					for (var i=0;i < this.sg2.getRowCount();i++){
						...
						if (prd > this.app._periodeGAR){
							...
							// diperbolehkan untuk kewenangan Gubis.
							// bisa dicek waktu checking perbulan
						}
						var prd = this.sg2.cells(0,i) ;
						var prd2 = this.app._periodeGAR.substr(4,2);							
						if (this.app._TW.get(prd) > this.app._TW.get(prd2)) {
							throw("Transaksi Terima tidak valid. Periode lebih dari Triwulan berjalan [Baris: "+k+"]");													
						}				
						if ((this.app._TW.get(prd) < this.app._TW.get(prd2))){
							var k = i+1;
							throw("Transaksi Terima tidak valid. Periode kurang dari Triwulan berjalan [Baris: "+k+"]");							
						}
					...
					this.sub_jenis = 'RRA';	
					{
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								...
								if (prd > this.app._periodeGAR && this.c_jenis.getText().toUpperCase() != "CAPEX"){
									var k = i+1;
								...
								dataGrid.set(this.sg.cells(0,i)+":"+this.sg.cells(1,i)+":"+this.sg.cells(3,i)+":"+this.sg.cells(5,i),{index:i + 1, nilai: nilaiToFloat(this.sg.cells(10,i)), versi:this.sg.cells(11,i)});
						...
						for (var i in totalPerBulan.objList){
							if (totalPerBulan.get(i).totDonor != totalPerBulan.get(i).totTerima){
						...
						var data = this.dbLib.getDataProvider("select a.kode_lokasi,a.kode_akun,a.kode_cc,substr(a.periode,5,2) as periode, sum(a.nilai) as pakai "+
								...
								"		where a.kode_akun in ("+kodeAkun+")  and a.kode_cc in ("+kodeCc+") and a.periode like '"+this.e_periode.getText().substr(0,4)+"%'  and a.kode_lokasi= '"+this.app._lokasi+"' and lower(a.DC)= 'c' and b.flag_rfc='0' and b.no_pdrk <> '"+this.cbDraft.getText()+"' "+
					...
					*/
					if (this.app._sapOnline){
						if (this.e_terima.getText() == "0" || this.e_donor.getText() == "0" ){
							throw("Nilai terima tidak boleh Nol");							
						}
					}
		...
		try{
			...
			if (col == 0 || col == 1 || col == 3 || col == 5 || col == 11) {
				...
				if (this.sg.cells(0,row) != "" && this.sg.cells(1,row) != "" && this.sg.cells(3,row) != "" && this.sg.cells(5,row) != "") {
					...
					dataRRR.set(i, (dataRRR.get(i) == undefined ? 0 : dataRRR.get(i)) );
					rfcRet.sawal = rfcRet.sawal -  dataRRR.get(i);
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					...
					if (line.dc.toUpperCase() == "C")
						this.sg.appendData([line.bulan,line.kode_cc,line.nama_cc,line.kode_drk,line.nama_drk,line.kode_akun,line.nama_akun,floatToNilai(line.nilai_gar),floatToNilai(line.nilai_pakai),floatToNilai(line.saldo),floatToNilai(line.nilai), jenis,floatToNilai(line.nilai)]);
