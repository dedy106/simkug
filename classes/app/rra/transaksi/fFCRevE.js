window.app_rra_transaksi_fFCRevE = function(owner)
{
	if (owner)
	{
		window.app_rra_transaksi_fFCRevE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fFCRevE";
		...
		this.e_nb = new saiLabelEdit(this,{bound:[20,12,202,20],caption:"No Review",maxLength:30,readOnly:true});
...
window.app_rra_transaksi_fFCRevE.extend(window.childForm);
window.app_rra_transaksi_fFCRevE.implement({
	...
	simpan: function(){			
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{				
					this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"rra_app_m","no_app","FCRKE-"+this.e_periode.getText().substr(2,4)+".","0000"));
				...
				switch(this.c_modul.getText()){
					case "RRR" :					
						strSQL = "select 'INPROG' as status,a.no_rev as no_bukti,a.no_pdrk as no_dokumen,date_format(b.tanggal,'%d-%m-%Y') as tanggal,b.keterangan,'REV' as jenis, '"+this.app._userLog+"' as nik_keep "+
										 ...
										 "where a.sts_pdrk='RRR' and a.progress in ('0','1') and a.flag_rfc='0' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' "+
										 "union "+
										 ...
										 "where a.sts_pdrk='RRR' and a.progress in ('1','0') and a.flag_rfc='0' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' "+
										 "union "+
