window.app_rra_transaksi_fSB = function(owner)
{
	if (owner)
	{
		window.app_rra_transaksi_fSB.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fSB";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan Standby Budget: Input", 0);	
		
		uses("saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;wysiwyg;util_file;uploader;util_rfc;util_rfcLib");
		...
		this.e_nb = new portalui_saiLabelEdit(this,{bound:[20,12,202,20],caption:"No Standby",maxLength:30,readOnly:true});
		...
		this.cb_buat = new saiCBBL(this.pc1.childPage[0],{bound:[20,12,200,20],caption:"NIK Review", multiSelection:false, maxLength:10, tag:2, bufferOption: boHALF, bufferData: this.app._listNIK, btnVisible:false});
...
window.app_rra_transaksi_fSB.extend(window.childForm);
window.app_rra_transaksi_fSB.implement({
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into rra_anggaran(no_bukti,modul,kode_cc,kode_akun,kode_drk,kode_lokasi,periode,dc,nilai,nik_user,tgl_input) values "+
									"	('"+this.e_nb.getText()+"','STB','"+this.sg2.cells(1,i)+"','"+this.sg2.cells(5,i)+"','"+this.sg2.cells(3,i)+"','"+this.app._lokasi+"','"+this.e_periode.getText().substr(0,4)+this.sg2.cells(0,i)+"','D',"+parseNilai(this.sg2.cells(7,i))+",'"+this.app._userLog+"',now())");
							}
						}
					}
					if (files.length > 0){
