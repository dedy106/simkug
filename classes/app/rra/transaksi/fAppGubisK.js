window.app_rra_transaksi_fAppGubisK = function(owner)
{
	if (owner)
	{
		window.app_rra_transaksi_fAppGubisK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fAppGubisK";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form PRDRK ABT Anggaran: Approve Group Bisnis (Koreksi)", 0);	
...
window.app_rra_transaksi_fAppGubisK.extend(window.childForm);
window.app_rra_transaksi_fAppGubisK.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{											
					...
					sql.add("update rra_grev_m set progress = '1' where no_grev in (select no_bukti from rra_app_d where no_app='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"') and kode_lokasi='"+this.app._lokasi+"' ");					
