window.app_rra_transaksi_fCPXSukkaApp = function(owner)
{
	if (owner)
	{
		window.app_rra_transaksi_fCPXSukkaApp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fCPXSukkaApp";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form PDRK Redistribusi, Realokasi, Reschedule Anggaran: Approve SUKKA(UBIS)", 0);	
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:7,tag:0,
		            colTitle:["Status","No Bukti","No Dokumen","Tanggal","Keterangan","Jenis","No SUKKA"],
...
window.app_rra_transaksi_fCPXSukkaApp.extend(window.childForm);
window.app_rra_transaksi_fCPXSukkaApp.implement({
	...
	simpan: function(){			
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{				
					this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"rra_app_m","no_app","APPSK-"+this.e_periode.getText().substr(2,4)+".","0000"));					
					...
					sql.add("insert into rra_app_m(no_app, kode_lokasi,tanggal,keterangan,modul,periode,no_del,nik_buat,nik_app,nik_user,tgl_input,jenis_form) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.c_modul.getText()+"','"+this.e_periode.getText()+"','-','"+this.app._userLog+"','"+this.app._userLog+"','"+this.app._userLog+"',now(),'APPSK')");
					var line;
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						...
						if (line.status.toUpperCase() == "APP"){
							...
							var no_appsurat = line.no_surat;
							//no_appsurat = no_appsurat.substr(0,4) + no_appsurat.substr(5);
							sql.add("update rra_nosukka set no_surat = '"+no_appsurat+"', no_app='"+this.e_nb.getText()+"' where no_sukka = '"+line.no_sukka+"' and kode_lokasi = '"+this.app._lokasi+"' ");
							//sql.add("insert into rra_nosukka(no_sukka, kode_lokasi, no_app, no_surat, kode_ba, status) values  "+
							//	"('"+line.no_bukti+"','"+this.app._lokasi+"','"+this.e_nb.getText()+"','"+nosurat+"','"+this.app._kodeBA+"','0')");
							//this.standarLib.nextNoBuktiOtomatis(nosurat,"/KU250/"+this.app._kodeBA+"/"+this.e_periode.getText().substr(2,4)+,"0000",true);
							if (line.progress == '4') {								
								sql.add("update rra_rev_m  set progress = 'A' where no_pdrk = '"+line.no_dokumen+"' and kode_lokasi = '"+this.app._lokasi+"' ");
							}else if (line.progress == 'A') {								
								sql.add("update rra_rev_m  set progress = 'P' where no_pdrk = '"+line.no_dokumen+"' and kode_lokasi = '"+this.app._lokasi+"' ");
							}else if (line.progress = 'P'){
								sql.add("update rra_frev_m  set progress = '2' where no_frev like 'FREV%' and no_pdrk = '"+line.no_dokumen+"' and kode_lokasi = '"+this.app._lokasi+"' ");
								sql.add("update rra_rev_m  set progress = '5' where no_pdrk = '"+line.no_dokumen+"' and kode_lokasi = '"+this.app._lokasi+"' ");
							}
						}
					}				
		...
		if (sender == this.c_modul) {
			if (this.c_modul.getText() != "") {
				...
				switch(this.c_modul.getText()){
					case "RRR" :
								strSQL = "select 'INPROG' as status,a.no_rev as no_bukti,a.no_pdrk as no_dokumen,date_format(b.tanggal,'%d-%m-%Y') as tanggal,b.keterangan,a.jenis_agg as jenis, d.no_surat, c.no_sukka, a.nik_appjust, a.nik_appjust2, a.nik_apppdrk3, a.nik_apppdrk32, a.progress "+
										 ...
										 "where ((a.progress='4' and (b.nik_appjust = '"+this.app._userLog+"' or b.nik_apppdrk3 = '"+this.app._userLog+"') ) "+												
												" (a.progress='A' and (a.nik_app2 = '"+this.app._userLog+"' or b.nik_appjust2 = '"+this.app._userLog+"' or b.nik_apppdrk32 = '"+this.app._userLog+"') ) "+												
												" or (a.progress = 'P' and c.nik_app = '"+this.app._userLog+"') "+
	...
	doDoubleClick: function(sender, col , row) {
		if (this.sg.cells(1,row) != "") {
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				//this.writeToIframe(urldecode(line.justifikasi));
