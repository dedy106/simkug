window.app_rra_transaksi_fSukkaApp = function(owner)
{
	if (owner)
	{
		window.app_rra_transaksi_fSukkaApp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fSukkaApp";
...
window.app_rra_transaksi_fSukkaApp.extend(window.childForm);
window.app_rra_transaksi_fSukkaApp.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{				
					...
					sql.add("update rra_rev_m  set progress = '5' where no_pdrk in (select catatan from rra_app_d  where no_app='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"') and kode_lokasi = '"+this.app._lokasi+"' ");
		...
		if (sender == this.c_modul) {
			if (this.c_modul.getText() != "") {
				...
				switch(this.c_modul.getText()){
					case "RRR" :
								strSQL = "select 'INPROG' as status,a.no_rev as no_bukti,a.no_pdrk as no_dokumen,date_format(b.tanggal,'%d-%m-%Y') as tanggal,b.keterangan,a.jenis_agg as jenis, d.no_surat, c.no_sukka "+
										 ...
										 "where a.progress='4' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and c.nik_app = '"+this.app._userLog+"'";
						break;
				}				
