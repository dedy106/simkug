window.app_rra_transaksi_fCPXSukkaRev = function(owner)
{
	if (owner)
	{
		window.app_rra_transaksi_fCPXSukkaRev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fCPXSukkaRev";
		...
		this.mDesk = new tinymceCtrl(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-25,this.pc1.height-125], withForm:false});
		...
		try {
			...
			this.cb_pdrk.setSQL("select a.no_pdrk,a.keterangan from rra_pdrk_m a inner join rra_rev_m b on a.no_pdrk=b.no_pdrk and a.kode_lokasi=b.kode_lokasi where b.progress='6' and b.kode_lokasi='"+this.app._lokasi+"' and b.kode_ubis = '"+this.app._kodeUbis+"' and b.nik_review = '"+this.app._userLog+"'",["no_pdrk","keterangan"],false,["No PDRK","Keterangan"],"and","Data PDRK",true);
...
window.app_rra_transaksi_fCPXSukkaRev.extend(window.childForm);
window.app_rra_transaksi_fCPXSukkaRev.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{								
					...
					sql.add("insert into rra_nosukka(no_sukka, kode_lokasi, no_app, no_surat, kode_ba, status) values  "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_sukka.getText()+"','"+this.e_sukka.getText()+"','"+this.app._kodeBA+"','0')");
					sql.add("update rra_sukka set nik_app='"+this.cb_app.getText()+"',keterangan='"+urlencode(this.mDesk.getCode())+"', kepada='"+this.m_kpd.getText()+"',justifikasi='"+urlencode(this.mDesk2.getCode())+"' where no_sukka = '"+this.no_sukka+"' ");
