 ...
 * 			-> 
 * */
window.app_rra_transaksi_fRRRE = function(owner, options)
{
	if (owner)
	{
		window.app_rra_transaksi_fRRRE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fRRRE";
		...
		this.pc1 = new pageControl(this,{bound:[20,16,900, 350], childPage:["Data PDRK","Data Donor","Data Penerima","Data Justifikasi","Dokumen Pendukung","Nota Kajian (CAPEX)"]});
		...
		this.mDesk2 = new tinymceCtrl(this.pc1.childPage[5],{bound:[1,10,this.pc1.width-5,this.pc1.height-15], withForm:false});
		...
		try {			
			...
			this.e_nb.setSQL("select no_pdrk, keterangan from rra_pdrk_m where kode_lokasi= '"+this.app._lokasi+"' and progress in ('0','R') and kode_ubis = '"+this.app._kodeUbis+"' and sub_jenis = 'RRA'",["no_pdrk","keterangan"],false,["No PDRK","Keterangan"],"","Daftar PDRK",true);
...
window.app_rra_transaksi_fRRRE.extend(window.childForm);
window.app_rra_transaksi_fRRRE.implement({
	...
	locateCC : function(sender, col, row, cc){
		var data = this.dbLib.getDataProvider("select nama from rra_cc where kode_cc = '"+cc+"' and kode_ubis = '"+this.cb_ubis.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
		if (typeof data == "string"){
			error_log(data);
			return false;
		}
	...
	doChange: function(sender){
		...
		if (sender == this.e_nb){			
			var data = this.dbLib.getMultiDataProvider(new server_util_arrayList({items:[
					   "select date_format(a.tanggal, '%d-%m-%Y') as tanggal, a.keterangan, a.jenis_agg,a.kode_ubis,a.kode_gubis,a.nik_app1,a.nik_app2,a.nik_app3,a.nik_appjust, a.nik_appjust2, a.nik_apppdrk3, a.nik_apppdrk32, b.nama as nama_ubis,c.nama as nama_gubis,a.justifikasi,a.sts_pdrk, a.kode_kota, a.file_nd, a.no_nd, a.no_draft, a.nik_review, a.kajian, a.nik_review2, a.nik_review3, a.nik_review4, a.nik_review5   "+
					   ...
					   "select file_dok, nama from rra_pdrk_dok where no_pdrk = '"+sender.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' order by no_urut",
			...
			for (var i in this.data1Thn.objList){										
				...
				for (var l in row.objList){//rows = 16
					...
					for (var l2 in line.objList){//row																	
						...
						if (this.c_jenis.getText() == "OPEX"){						
							if (parseFloat(line.get(l2).get("VERSN")) == 0 && this.sg.cells(10,row) == "PAYMENT" ){
