window.app_rra_transaksi_fRRRMSukaRev = function(owner)
{
	if (owner)
	{
		window.app_rra_transaksi_fRRRMSukaRev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_transaksi_fRRRMSukaRev";
		...
		this.e_nb = new saiCBBL(this,{bound:[20,13,202,20],caption:"No SUKKA",maxLength:30,readOnly:true,
			multiSelection:false, change:[this,"doChange"],
			sql:["select a.no_sukka, a.deskripsi from rra_sukka a inner join rra_mrev_m b on b.no_pdrk = a.no_pdrk and b.kode_lokasi = a.kode_lokasi  "+
		...
		this.pc1 = new pageControl(this,{bound:[20,16,900, 330], childPage:["Data SUKKA","Data Donor","Data Penerima","Nota Pengantar SUKKA"]});
		...
		this.mDesk = new tinymceCtrl(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-25,this.pc1.height-15], withForm:false});
		...
		try {
			...
			this.cb_pdrk.setSQL("select a.no_pdrk,a.keterangan from rra_pdrk_m a "+
				"	inner join rra_mrev_m b on a.no_pdrk=b.no_pdrk and a.kode_lokasi=b.kode_lokasi "+
...
window.app_rra_transaksi_fRRRMSukaRev.extend(window.childForm);
window.app_rra_transaksi_fRRRMSukaRev.implement({
	...
	simpan: function(){			
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					this.e_sukka.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"rra_nosukka","no_surat","R/KU250/"+this.app._kodeBA+"/"+this.e_periode.getText().substr(0,4)+"","0000","",true));					
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();								
					sql.add("update rra_mrev_m set progress = '4' where no_pdrk ='"+this.cb_pdrk.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");					
	...
	doChange:function(sender){		
		if (sender == this.e_nb){
			this.no_sukka = "";
			var data = this.dbLib.getDataProvider("select a.no_pdrk, a.no_sukka, date_format(a.tanggal, '%d-%m-%Y') as tgl, a.keterangan, a.deskripsi, a.nik_app, a.justifikasi, a.kepada  "+
