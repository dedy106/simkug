...
***********************************************************************************************/
window.app_rra_dashboard_fDashboard = function(owner,options)
{
    if (owner)
    {
		try{
	        window.app_rra_dashboard_fDashboard.prototype.parent.constructor.call(this, owner,options);        
            this.className = "app_rra_dashboard_fDashboard";
			...
			this.tab = new pageControl(this,{bound:[0,0,this.width - 10, this.height - 30],
				childPage: ["PDRK UBIS","Review UBIS","Review GUBIS","Review MA"],
			...
			this.pFilter.area = new saiCBBL(this.pFilter,{bound:[20,30,200,20], caption:"UBIS",
				multiSelection: false,
				sql: ["select kode_ubis, nama from rra_ubis ",["kode_ubis","nama"],false,["Kode UBIS","Descp"],"and","Daftar UBIS",true]
			});			
...
window.app_rra_dashboard_fDashboard.extend(window.childForm);
window.app_rra_dashboard_fDashboard.implement({
	...
	doClick: function(sender){
        try{
            if (sender == this.bApply){               
				this.initComp(this.pFilter.area.getText(),"");
				this.pFilter.hide();
			}
       }catch(e){
            alert(e);
		...
		try{
		        ...
		        if (typeof lokfa != "string"){
					if (lokfa.dataSelection){																	
						lokfa = "and "+this.cbbLoker.convertFilter("a.kode_ubis");
					}else{
						lokfa ="'"+lokfa.getText()+"'"; 
						lokfa = "and a.kode_ubis in ("+loker+")";
					}
				}		        
				sql.add("select b.nama, sum(case when sts_pdrk='RRR' then 1 else 0 end ) as totrrr "+
					"	, sum(case when sts_pdrk='ABT' then 1 else 0 end ) as totabt "+
					"  from rra_pdrk_m a "+
				...
				sql.add("select b.nama, sum(case when sts_pdrk ='RRR' then c.nilai else 0 end) as nilairrr "+
						" , sum(case when sts_pdrk ='ABT' then c.nilai else 0 end) as nilaiabt "+
						"  from rra_pdrk_m a "+
						" inner join rra_ubis b on b.kode_ubis = a.kode_ubis and b.kode_lokasi = a.kode_lokasi "+
						" inner join rra_anggaran c on c.no_bukti = a.no_pdrk and c.kode_lokasi = a.kode_lokasi and c.dc = 'D' "+
						" where a.kode_lokasi = '"+this.app._lokasi+"' "+lokfa+" group by b.nama ");					
    	       ...
    	       switch(methodName){
    	           case "getMultiDataProvider":					
    	                ...
    	                if (typeof data != "string"){
            				...
            				this.fr1 = new panel(this.tab.childPage[0],{bound:[10,10,frameWidth,frameHeight],caption:"Permintaan RRR dan ABT UBIS(Nilai Usulan)"});
							...
							this.fr1.grid = new saiGrid(this.fr1,{bound:[5,30,this.fr1.getClientWidth() - 30,this.fr1.getClientHeight() - 50],colCount:3,colTitle:["Group By","RRR","ABT"],visible:systemAPI.browser.msie, readOnly:true, dblClick:[this,"doGridClick"],colWidth:[[1,0],[100,200]], colFormat:[[1,2],[cfNilai, cfNilai]]});		         	                        
							this.fr1.data = data.result[0];
            		        ...
            		        this.fr2 = new panel(this.tab.childPage[0],{bound:[this.fr1.left+this.fr1.width + 15,10,frameWidth,frameHeight],caption:"Permintaan RRR dan ABT UBIS(Nilai Usulan)"});
							...
							this.fr2.grid = new saiGrid(this.fr2,{bound:[5,30,this.fr2.getClientWidth() - 30,this.fr2.getClientHeight() - 50],colCount:3,colTitle:["Group By","Nilai RRA","Nilai ABT"],visible:systemAPI.browser.msie, readOnly:true, dblClick:[this,"doGridClick"],colWidth:[[1,0],[100,200]], colFormat:[[1,2],[cfNilai,cfNilai]]});  							
							...
							this.fr6 = new panel(this.tab.childPage[1],{bound:[10,10,frameWidth,frameHeight],caption:""});
							this.fr7 = new panel(this.tab.childPage[1],{bound:[this.fr6.left+this.fr6.width + 15,10,frameWidth,frameHeight],caption:" "});							
							var frameWidth = this.width / 2 - 30;						
							...
							this.fr11 = new panel(this.tab.childPage[3],{bound:[10,10,this.width - 30,this.height - 70],caption:"Schedule Monitoring"});													
							var frameWidth = this.width / 2 - 30;						
	...
	setData: function(grid, data, grid2){
		...
		if (grid == this.fr1.grid || grid == this.fr6.grid){			
			for (var i in data.rs.rows){
				grid.appendData([data.rs.rows[i].nama, floatToNilai(data.rs.rows[i].totrrr), floatToNilai(data.rs.rows[i].totabt)]);
			}			
		}
		...
		if (grid == this.fr2.grid || grid == this.fr7.grid){
			for (var i in data.rs.rows){
				grid.appendData([data.rs.rows[i].nama, floatToNilai(data.rs.rows[i].nilairrr), floatToNilai(data.rs.rows[i].nilaiabt)]);
			}				
		}
    ...
    doTabChange: function(sender, page){
        this.count = 0;
		if (page == this.tab.childPage[1] && this.fr6.chart === undefined){																	
		}
		if (page == this.tab.childPage[2] && this.fr10.chart === undefined){			
		}		
    },
		...
		try{			
			...
			if (swf == this.fr1.chart || swf == this.fr6.chart){
				...
				if (data.rs.rows[0]!==undefined)
				{
					for (var i in data.rs.rows){
						line = data.rs.rows[i];  						
						sap.values.push(Math.round(line.totrrr));						
						veat.values.push( Math.round(line.totabt));												
						chart.x_axis.labels.labels.push(line.nama);
						swf.labels.push(line.nama);
						if (parseFloat(line.totrrr) > maxValue ) maxValue = parseFloat(line.totrrr);
						if (parseFloat(line.totrrr) < minValue ) minValue = parseFloat(line.totrrr);						
						if (parseFloat(line.totabt) > maxValue ) maxValue = parseFloat(line.totabt);
						if (parseFloat(line.totabt) < minValue ) minValue = parseFloat(line.totabt);
				...
				if (data.rs.rows[0]!==undefined)
				{
					for (var i in data.rs.rows){
						line = data.rs.rows[i];  						
						sap.values.push(Math.round(line.nilairrr));						
						veat.values.push( Math.round(line.nilaiabt));												
						chart.x_axis.labels.labels.push(line.nama);
						swf.labels.push(line.nama);
						if (parseFloat(line.nilairrr) > maxValue ) maxValue = parseFloat(line.nilairrr);
						if (parseFloat(line.nilairrr) < minValue ) minValue = parseFloat(line.nilairrr);						
						if (parseFloat(line.nilaiabt) > maxValue ) maxValue = parseFloat(line.nilaiabt);
						if (parseFloat(line.nilaiabt) < minValue ) minValue = parseFloat(line.nilaiabt);						
						this.dataDivisi.add(line.nama);
					}									
				}else{
					var line,temp="", maxValue = 0,minValue = 0;			
