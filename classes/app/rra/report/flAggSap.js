window.app_rra_report_flAggSap = function(owner)
{
	if (owner)
	{
		window.app_rra_report_flAggSap.prototype.parent.constructor.call(this,owner);
		this.className = "app_rra_report_flAggSap";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Anggaran SAP", 2);
...
window.app_rra_report_flAggSap.extend(window.childForm);
window.app_rra_report_flAggSap.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				if (this.sg1.getCell(2,5)=="Bulanan")
				{
					var sql = "select a.kode_akun,c.nama as nama_akun,a.kode_cc,b.nama as nama_cc "+
							...
							"from (select distinct a.kode_cc,a.kode_akun,a.kode_lokasi,a.modul "+
							"	   from rra_anggaran a "+
					...
					this.widthTable = new server_util_arrayList({items:[60,200,60,200,80,80,80,80,80,80,80,80,80,80,80,80,100]});
			...
			systemAPI.alert("[app_rra_report_flAggSap]::mainButtonClick:"+e);
		}
	},
