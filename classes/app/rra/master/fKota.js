window.app_rra_master_fKota = function(owner)
{
	if (owner)
	{
		window.app_rra_master_fKota.prototype.parent.constructor.call(this,owner);
		this.className  = "app_rra_master_fKota";
...
window.app_rra_master_fKota.extend(window.childForm);
window.app_rra_master_fKota.implement({
	doClose: function(){
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try{									
					...
					sql.add("insert into rra_kota(kode_kota,nama) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update rra_kota set nama = '"+this.e_nama.getText()+"' where kode_kota = '"+this.cb_kode.getText()+"' ");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql, undefined, this);
				}
				catch(e){
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from rra_kota where kode_kota = '"+this.cb_kode.getText()+"' ");			
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql, undefined, this);
				}
				catch(e){
		...
		try{
			if (this.cb_kode.getText() != ""){
				var data = this.dbLib.getDataProvider("select nama "+
				           " from rra_kota where kode_kota ='"+this.cb_kode.getText()+"'  ",true);
	...
	doTampilClick: function(sender){
		try{			
			var temp = this.dbLib.runSQL("select kode_kota,nama from rra_kota  order by kode_kota");
		...
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Group Bisnis",sender,undefined, 
											  ...
											  "select count(kode_kota) from rra_kota  ",
	...
	doFind: function(){
		try{			
			var temp = this.dbLib.runSQL("select kode_kota,nama from rra_kota  "+
				" where ( upper(nama) like '%"+this.e_cari.getText().toUpperCase()+"%' or "+
				"   upper(kode_kota) like '%"+this.e_cari.getText().toUpperCase()+"%' "+
				" ) order by kode_kota ");
