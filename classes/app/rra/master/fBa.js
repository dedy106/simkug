 ...
 * @author dweexfuad
 */
window.app_rra_master_fBa = function(owner)
{
	if (owner)
	{
		window.app_rra_master_fBa.prototype.parent.constructor.call(this, owner);
		this.className = "app_rra_master_fBa";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Bisnis Area", 0);	
		...
		this.pAdd = new panel(this,{
			...
			this.e4 = new saiCBBL(this.pAdd,{
				bound:[20,23,200,20],multiSelection : false, 
				sql:["select kode_kota, nama from rra_kota ",["kode_kota","nama"],false,["Kode","Kota"],"and","Daftar Kota",true],				
				caption:"Tempat"
			});
			this.e5 = new saiCBBL(this.pAdd,{
				bound:[20,24,200,20], multiSelection : false, 
				sql:["select kode_ubis, nama from rra_ubis where kode_lokasi = '"+this.app._lokasi+"'",["kode_ubis","nama"],false,["Kode UBIS","Deskripsi"],"and","Daftar UBIS",true],
				caption:"UBIS"
			});	
			this.e6 = new saiCBBL(this.pAdd,{
				bound:[20,25,200,20], multiSelection : false, 
				sql:["select nik, nama from rra_karyawan where kode_lokasi = '"+this.app._lokasi+"' ",["nik","nama"],false,["NIK","Deskripsi"],"and","Daftar Karyawan",true],
				caption:"Karyawan"
			});	
...
window.app_rra_master_fBa.extend(window.childForm);
window.app_rra_master_fBa.implement({
	doClose: function(){
		...
		if (callObj ==  this){
			switch (methodName)
			{
				...
				case "execArraySQL" :
					...
					else system.alert(this,"Transaksi Gagal", result); 
					break;
			}
		}
	},
	doModalResult: function(event, modalResult, value){
		try
		{
			switch (event)
			{
				case "Add" :
				   if ( modalResult == mrOk)
				   {
						...
						{   						
							...
							data.set("kode_ba",this.e0.getText());						
							...
							data.set("kode_ubis",this.e5.getText());
							data.set("rowindex",0);		
							data.set("kode_induk",induk);														
							data.set("level_spasi",lvl);			
							data.set("nik_app",this.e6.getText());																					
							node.data = data;
						}catch(e)
						{
							system.alert(this,e,"");
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
						...
						try
						{						
							var sql2,sql1 = "delete from rra_ba where kode_lokasi = '"+this.elokasi.getText()+"'";					// kode_lokasi,						
							var sql = new server_util_arrayList();
							sql.add(sql1);
							var data = value.split("#");
							for (var i in data){
								sql2 = "insert into rra_ba(kode_ba, nama, kode_lokasi,tipe, tempat, kode_ubis,rowindex, kode_induk, level_spasi, nik_app,  nik_user, tgl_input) values ";
								sql2 += data[i];
								sql.add(sql2);
							}							
			...
			system.alert(this,"Error on doModalResult",e);
		}
	},
	loadMenu: function(daftarNrc){
		try
		{
			...
			system.alert(this,"Error on row "+ rowNo +" : ",e);
		}
	},
	doClick: function(sender){	
		var nrc = this.dblib.runSQL("select kode_ba, nama, kode_lokasi,tipe,tempat,kode_ubis,rowindex, kode_induk, level_spasi, nik_app from rra_ba where  kode_lokasi = '"+this.elokasi.getText()+"' order by rowindex",0,0);	
		this.loadMenu(nrc);
	},
		...
		{ 
			...
			{			
				...
				}else if (sender == this.editBtn)
				{
					...
					this.e4.setText(item.data.get("tempat"));								
					this.e5.setText(item.data.get("kode_ubis"));								
					this.e6.setText(item.data.get("nik_app"));	
	...
	copyChilds: function(from, to){
		...
		{			
			...
			chld.data.set("kode_ba",temp.getKode());					
	...
	switchItem: function(from, to){
		...
		child.data.set("kode_ba",item.getKode());					
