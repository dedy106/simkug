...
//* 			SAI, PT											
//***********************************************************************************************
window.app_rra_fHakakses = function(owner)
{
	if (owner)
	{
		try
		{
			window.app_rra_fHakakses.prototype.parent.constructor.call(this, owner);
			this.className = "app_rra_fHakakses";
			...
			this.e5 = new saiCBBL(this, {
				...
				sql:["select kode_ba, nama from rra_ba where kode_lokasi = '"+this.app._lokasi+"' ",["kode_ba","nama"],false, ["Kode","Nama"],"and","Daftar Bisnis Area",true]
			});									
			...
			this.e5.setText(this.app._kodeBA);
			this.e0.setText(this.app._userLog, this.app._namaUser);
			
		}catch(e)
		{
			alert("[app_rra_fHakakses]::constructor:"+e);
		}
		
		this.setTabChildIndex();
	}
};
window.app_rra_fHakakses.extend(window.childForm);
window.app_rra_fHakakses.implement({
		...
		if (sender == this.e0){	
			if (this.app._userStatus != "A")
				this.standar.showListData(this, "Data User", this.e0, this.e1,
												  "select nik, nama from rra_karyawan where kode_lokasi ='"+this.app._lokasi+"' and kode_ba='"+this.e5.getText()+"' and nik = '"+this.app._userLog+"' ",
												  "select count(*) from rra_karyawan where kode_lokasi ='"+this.app._lokasi+"' and kode_ba='"+this.e5.getText()+"' and nik = '"+this.app._userLog+"'",
												  ["nik","nama"],"and",["User Id","Nama"]);
			else this.standar.showListData(this, "Data User",this.e0, this.e1,
												  "select nik, nama from rra_karyawan where kode_lokasi ='"+this.app._lokasi+"' and kode_ba='"+this.e5.getText()+"' ",
												  "select count(*) from rra_karyawan where kode_lokasi ='"+this.app._lokasi+"' and kode_ba='"+this.e5.getText()+"'  ",
												  ["nik","nama"],"and",["User Id","Nama"]);
	...
	doEditChange: function(sender){
		try {
			setTipeButton(tbSimpan);
			if (sender == this.e0) {											
				var pass = this.dbLib.getDataProvider("select * from hakakses where kode_lokasi = '" + this.app._lokasi + "' and nik = '" + this.e0.getText() + "' ", true);
				...
				if (sender == this.e5) {
					this.e0.setSQL("select nik, nama from rra_karyawan where kode_lokasi = '" + this.app._lokasi + "' and kode_ba = '" + sender.getText() + "' ", ["nik", "nama"], false, ["NIK", "Nama"], "and", "Daftar User/Karyawan", false);
				}
		}catch(e){
			alert(e);
		}
		...
		switch (event)
		{
			...
			case "simpan" :
				if (modalResult == mrOk){
					...
					values = "'"+this.e0.getText()+"','"+this.e1.getText()+"','"+this.e2.getText()+"','"+status+"','"+this.app._lokasi+"','"+this.e8.getText()+"','"+this.e7.getText()+"' ";
					sqlValue = "(" + values +")";
					sql2 += sqlValue;
					uses("server_util_arrayList");
					sql = new server_util_arrayList();
					sql.add("delete from hakakses where nik='"+this.e0.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
					sql.add(sql2);
			...
			case "ubah" :
				if (modalResult == mrOk){
					...
					values = "'"+this.e0.getText()+"','"+this.e1.getText()+"','"+this.e2.getText()+"','"+status+"','"+this.e8.getText()+"','"+this.e7.getText()+"','"+this.app._lokasi+"'";
					sqlValue = "(" + values +")";
					sql2 += sqlValue;
					uses("server_util_arrayList");
					sql = new server_util_arrayList();
