window.app_saku3_report_tm_flPiutang = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tm_flPiutang.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tm_flPiutang";
		this.maximize();
...
window.app_saku3_report_tm_flPiutang.extend(window.childForm);
window.app_saku3_report_tm_flPiutang.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from piutang_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_tm_rptPiutang";
			...
			systemAPI.alert("[app_saku3_report_tm_flPiutang]::mainButtonClick:"+e);
		}
	},
