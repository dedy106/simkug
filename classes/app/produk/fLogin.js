...
//* 			SAI, PT											
//***********************************************************************************************
window.app_produk_fLogin = function(owner){
	if (owner){
		try{		
			window.app_produk_fLogin.prototype.parent.constructor.call(this, owner);
			this.className  = "app_produk_fLogin";			
			...
			systemAPI.alert("[app_produk_fLogin]::oncreate lib : ",e);
			//alert(e);
		}
	}
};
window.app_produk_fLogin.extend(window.childForm);
window.app_produk_fLogin.implement({
	doClose: function(){
