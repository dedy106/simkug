...
//* 			SAI, PT											
//***********************************************************************************************
window.app_example_app = function(owner){
	window.app_example_app.prototype.parent.constructor.call(this, owner);
	this.className = "app_example_app";
	this.onTerminate.set(this,"doTerminate");
	this.form = undefined;
	this.setTitle("roojax Example");
	this.showForm();
};
window.app_example_app.extend(window.portalui_application);
window.app_example_app.implement({
	showForm: function(){
		try
		{
			uses("util_dbLib;portalui_form;portalui_childForm");						
			this._mainForm = new app_example_fMain(this);	
	...
	//capture keyboard event keyDown and then send to active application
	doKeyDown: function(charCode, buttonState, keyCode, event){
		try{			
			window.app_example_app.prototype.parent.doKeyDown.call(this, charCode, buttonState, keyCode, event);
			...
			if (this.activeForm.activeChildForm !== undefined) {
				...
				//protected unwanted keystroke.. Spacebar. arrow. pageup, pagedown, backspace
