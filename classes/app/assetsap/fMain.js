...
//* 			SAI, PT											
//***********************************************************************************************
window.app_assetsap_fMain = function(owner)
{
	if (owner)
	{	
		try
		{
			window.app_assetsap_fMain.prototype.parent.constructor.call(this, owner);
			window.app_assetsap_fMain.prototype.parent.setWidth.call(this, 600);
			window.app_assetsap_fMain.prototype.parent.setHeight.call(this, 450);
			this._mainButtonClick = new portalui_eventHandler();			
			this.title = "Asset Management Unit";
			document.title = "AMU-VEAT";
			this._isLogedIn	= false;
			this.app._kodeMenu = "AMU";
			this._userLog = "";
			this._lokasi = "";
			this._pp = "";
			...
			this.centerize();							
			uses("saiCB;panelWidget;saiLabelEdit;image;imageButton;button;timer;label;saiCBBL;panel;reportNavigator;treeView;treeNode;statusBar;marquee;util_addOnLib;util_standar;imageButton;saiGrid");					
			this.className  = "fMain";			
			this.formCaption = "V E A T";	
...
//-------------------------------------caption		
			...
			this.lblCaption = new label(this.pCaption,{bound:[10,0,500,22],caption:this.formCaption,fontColor:system.getConfig("app.headerColor"),bold:true,fontSize:13});						
////------------------------------------ panel button -------------------------		
			this.pButton = new panel(this,{bound:[0,55,this.width,31],border:1,color:"url(image/whitegradblue.jpg)",shadow:true});//system.getConfig("app.color.panel")
...
//------------------------------------ panel transaksi -------------------------
			...
			this.bTutup = new button(this.pButton,{bound:[this.width-90,4,80,20],caption:"Tutup",icon:"url(icon/"+system.getThemes()+"/exit.png)",hint:"Exit Modul",click:[this,"bTutupClick"], visible:false});
			//this.bTutup.hide();			
			this.bSimpan = new button(this.pTrans,{bound:[5,3,80,20],caption:"<u>S</u>impan",click:[this,"mainButtonClick"],icon:"url(icon/"+system.getThemes()+"/save.png)",hint:"Simpan Transaksi",name:"bSimpan"});		
			this.bEdit = new button(this.pTrans,{bound:[90,3,80,20],caption:"<u>E</u>dit",click:[this,"mainButtonClick"],icon:"url(icon/"+system.getThemes()+"/edit.png)",name:"bEdit",hint:"Simpan Edit data Transaksi"});			
			this.bHapus = new button(this.pTrans,{bound:[175,3,80,20],caption:"<u>H</u>apus",click:[this,"mainButtonClick"],icon:"url(icon/"+system.getThemes()+"/delete.png)",hint:"Hapus data Transaksi",name:"bHapus"});		
			this.bClear = new button(this.pTrans,{bound:[300,3,80,20],caption:"<u>C</u>lear",click:[this,"mainButtonClick"],icon:"url(icon/"+system.getThemes()+"/refresh.png)",hint:"Clear Form",name:"bClear"});
//--------------- panel login -------------------------		
			this.pLogin = new panel(this.pButton,{bound:[2,1,500,26],border:0,color:"url(image/whitegradblue.jpg)",visible:false});
			this.bLogin = new button(this.pLogin,{bound:[10,3,80,20],caption:"Login",click:[this,"loginClick"],icon:"url(icon/"+system.getThemes()+"/login.png)",hint:"Login"});					
//------------------------------------ panel report -------------------------		
			this.pReport = new panel(this.pButton,{bound:[2,1,500,26],visible:false,border:0,color:"url(image/whitegradblue.jpg)"});
			this.bPreview = new button(this.pReport,{bound:[10,3,80,20],caption:"Tampil",click:[this,"mainButtonClick"],icon:"url(icon/"+system.getThemes()+"/execObj.png)",hint:"Preview Data"});
			this.bClear2 = new button(this.pReport,{bound:[120,3,80,20],caption:"Clear",click:[this,"mainButtonClick"],icon:"url(icon/"+system.getThemes()+"/refresh.png)",hint:"Clear Form"});						
//------------------------------------ panel Editor-------------------------		
			this.pEditor = new panel(this.pButton,{bound:[2,1,500,26],visible:false,border:0,color:"url(image/whitegradblue.jpg)"});
			this.bNew = new button(this.pEditor,{bound:[5,3,80,20],caption:"New",click:[this,"mainButtonClick"],icon:"url(icon/"+system.getThemes()+"/preview.png)",hint:"New Form"});
			this.bSaveEdit = new button(this.pEditor,{bound:[80,3,80,20],caption:"Save",click:[this,"mainButtonClick"],hint:"Save Form",icon:"url(icon/"+system.getThemes()+"/save.png)"});
			this.bOpen = new button(this.pEditor,{bound:[155,3,80,20],caption:"Open",click:[this,"mainButtonClick"],icon:"url(icon/"+system.getThemes()+"/preview.png)",hint:"Open Form"});		
			this.bRun = new button(this.pEditor,{bound:[239,3,80,20],caption:"Run",click:[this,"mainButtonClick"],icon:"url(icon/"+system.getThemes()+"/Execute.png)",hint:"Test Form"});			
//------------------------------------ panel Editor-------------------------		
...
//------------------------------------ panel Message-------------------------		
			...
			this.bSend = new button(this.pMsg,{bound:[10,3,80,20],caption:"Send",click:[this,"mainButtonClick"],hint:"Send Message",icon:"url(icon/"+system.getThemes()+"/send.png)"});
//---------------------------------------------- image
			this.cek=0;
			this.step=0;
...
//------------------------------------------------
		   this.maximize();		   		  
//------------------------------------------------ status Bar			
			this.sb = new statusBar(this,{bound:[0,this.height - 50,this.width,50],visible:true});			
//----------------------------------------------- Marquee		    
...
//--------------------------------------------------
			...
			this.setImage("url(image/telkomlogokcl.png)");//themes/"+system.getThemes()+"/telkom2
			this.infoPerusahaan = new control(this,{bound:[this.treev.width + 30,this.treev.top,this.width - this.treev.width - 40,this.treev.height - 70]});
			this.infoUnit = new control(this,{bound:[this.treev.width + 30,this.treev.top + this.treev.height - 80,this.width - this.treev.width - 40,60]});
			
			this.infoPerusahaan.getCanvas().innerHTML = "<table border=0 cellspacing=0 cellpadding=0 width='100%'>"+
					"<tr><td align='center'>"+
					"<img src='image/telkomindonesia.png' width='200' height='120'/>"+
					"</td></tr>"+
					"<tr><td height='50'></td></tr>"+
					"<tr><td align='center'>"+
					"<span style='font-size:40pt;font-weight:bold;color:#299bcf;font-family:Arial, Helvetica, Sans-serif;'> V E A T</span>"+
					"</td></tr>"+
					"<tr><td align='center'>"+
					"<span style='font-size:14pt;font-weight:bold;color:#ff9900;font-family:Arial, Helvetica, Sans-serif;'>(Verifikasi Eksistensi Aset Tetap)</span>"+
					"</td></tr>"+					
					" </table>";
			this.infoUnit.getCanvas().innerHTML = "<table border=0 cellspacing=0 cellpadding=0 width='100%'>"+					
					"<tr><td align='center' valign='middle' >"+
					"<span style='font-size:12pt;font-weight:normal;color:#299bcf;font-family:Arial, Helvetica, Sans-serif;'> Direktorat Keuangan </span>"+
					"</td></tr>"+										
					"<tr><td align='center' valign='middle' >"+
					"<span style='font-size:12pt;font-weight:normal;color:#299bcf;font-family:Arial, Helvetica, Sans-serif;'> Asset Management Unit (AMU) </span>"+
					"</td></tr>"+										
					" </table>";
		}catch(e){
...
window.app_assetsap_fMain.extend(window.form);
window.app_assetsap_fMain.implement({
	gotoLogin: function(){
		uses("app_assetsap_fLogin");
		this.form = new app_assetsap_fLogin(this);
		this.form.setTop(this.childTop);
		this.form.show();
		this.form.maximize();
		this.hideMainComp();		
		if (window.parent.hideLoading == undefined && getCookie("amuroojaxu")){			
			this.form.showLoading("Init Parameter");
			this.form.e0.hide();
			this.form.e1.hide();
			this.form.lblClient.hide();			
			this.form.e0.setText(getCookie("amuroojaxu"));
			this.form.e1.setText(getCookie("amuroojaxp"));			
			this.app._rfcSetting = getCookie("amurfcSetting");
			this.app._dbSetting = getCookie("amudbSetting");
			this.app._appState = getCookie("amuappState");								
	...
	doAfterLogin: function(){
		try{		    
			...
			this.btnList = new image(this,{bound:[this.width - 50,30,25,25],hint:"User Info",showHint:true, color:"",image:"image/userinfo.png",click:[this,"doBtnList"]});
			this.lUser = new label(this,{bound:[this.width - 360,37,300,20],caption:this.app._namaUser,alignment:alRight,color:"#299bcf"});
			this.btnList.addStyle("cursor:pointer");
						
			this.listInfo = new panel(this,{bound:[this.getWidth() - 425,55,400,200],caption:"Info",color:"url(icon/"+system.getThemes()+"/background.png)",visible:true});
			this.listInfo.rearrangeChild(25,23);
				//this.listInfo.bRefresh = new imageButton(this.listInfo,{bound:[this.listInfo.width - 30, 2, 16,16], image:"icon/dynpro/bRefresh.png"});
				this.listInfo.grid = new saiGrid(this.listInfo,{bound:[2,20,396,180], colCount:2,colWidth:[[0,1],[250,50]], colTitle:"Yang harus dikerjakan, Jumlah", readOnly:true,
									headerColor:[[0,1],["#a0beff","#a0beff"]],
									colAlign:[[1],[alCenter]]
									});			
			uses("util_ajaxCaller;saiMemo");
			this.listInfo.ajax = new util_ajaxCaller("server/serverApp.php");
			this.listInfo.ajax.setFrequency(5000);
			this.listInfo.ajax.addListener(this);
			this.listInfo.msg = new util_ajaxCaller("server/serverApp.php");
			this.listInfo.msg.setFrequency(5000);
			this.listInfo.msg.addListener(this);
			this.msgPanel = new control(this,{bound:[this.width - 425,260,400,100]});
			this.msgPanel.getCanvas().style.overflow = "auto";
			
			//------------------- Broadcast Msg
			if (this.app._userStatus == "N"){//hanya user nasional
				this.bBroadcast = new image(this,{bound:[this.width - 80,this.height - 100,50,50],hint:"Broadcast Message",showHint:true, color:"",image:"image/writemsg.png",click:[this,"doBtnList"]});			
				this.bBroadcast.addStyle("cursor:pointer");
				
				this.sendMsgPanel = new panel(this,{bound:[this.width - 425,this.height - 280,400,200],caption:"Message",color:"url(icon/"+system.getThemes()+"/background.png)",visible:false});
				this.sendMsgPanel.editor = new saiMemo(this.sendMsgPanel,{bound:[1,20,398,160], caption:"",labelWidth:0});
				this.sendMsgPanel.bSend = new button(this.sendMsgPanel,{bound:[1,180,398,20],caption:"Send", click:[this,"doSendMsg"]});
			}
			//-------------------------------------
			...
			//this.loadWidgets();
			/*this.listInfo.ajax.setRequester(this.dbLib2.getDataProviderR("select b.kode_lokfa2,a.nama, "+
					"	case when progress = '1' then 'Approve Area-Regional' "+//by pass approval area-> jika sudag terlanjur, gpp
					"		when progress = '0' then 'Approve Regional' "+
					"		when progress = '2' then 'Approve IC' "+
					"		when progress = '4' then 'Return Regional' "+
					"		when progress = '5' then 'Return Area' "+
					"		when progress = '6' then 'Return Officer' "+
					"		when progress = '3' then 'OK' "+
					"	end as progress, "+
					"	 count(progress) as totnka  from amu_lokasi a "+
					"	inner join (select distinct a.no_gabung, b.kode_lokfa2, b.progress, c.kode_lokfa "+
					" from amu_kkl_d a "+
					"	inner join amu_kkl_m b on b.no_inv = a.no_inv "+
					"	inner join amu_asset c on c.no_gabung = a.no_gabung "+
					"	where b.progress <> '3') b on b.kode_lokfa = a.kode_lokfa "+
					" "+
					" group by b.kode_lokfa2, a.nama,progress order by a.nama, b.kode_lokfa2,b.progress"));
			this.listInfo.ajax.connect();
			
			this.listInfo.msg.setRequester(this.dbLib2.getDataProviderR("select msg from amu_msg where msg_id = (select max(msg_id) from amu_msg)"));
			this.listInfo.msg.connect();
			*/
		}catch(e){
			systemAPI.alert(e);
		}
	...
	bTutupClick: function(sender){	
		try{
			...
			if (this.timer !== undefined) this.timer.setEnabled(false);
			//this.marquee.show();
			this.setFormCaption("V E A T");
			this.alertFormLisence = false;
			this.treev.setFocus();
			this.bBack.onClick.set(this,undefined);
			//if (this.listInfo) this.listInfo.ajax.connect();
		}catch(e){
			systemAPI.alert(e);
		}
	},
			...
			switch(kodeForm){
				...
				default :
					...
					{				    						
						...
						if (this.listInfo) {
							this.listInfo.ajax.disconnect();
							this.listInfo.msg.disconnect();
						}
		...
		if (this.initiated){
			try{
				...
				if (this.btnList){
					...
					this.listInfo.setLeft(this.getWidth() - 425);
					if (this.msgPanel) this.msgPanel.setLeft(this.getWidth() - 425);
					if (this.bBroadcast) {
						this.bBroadcast.setTop(this.heigth - 100);
						this.bBroadcast.setLeft(this.width - 80);
					}
				}
				...
				this.infoUnit.setLeft(20 + this.treev.width + 20);		
				this.infoUnit.setTop(this.treev.top + this.treev.height - 30);						
	...
	doRequestReady: function(sender, methodName, result, errCode, connection){
		if (sender == this.dbLib2 && connection == this.listInfo.msg){			
			try{			
				if (methodName == "getDataProvider"){
					if (errCode == 200){
						try{
							var line,result = JSON.parse(result);
						}catch(e){
							system.alert(this,e,"Stop autoupdate...");
							this.listInfo.msg.disconnect();
						}
						this.msgPanel.getCanvas().style.border = "1px solid #45b2f0" ;	
						this.msgPanel.getCanvas().innerHTML = "<div id='"+this.getFullId()+"_bg' style=';position:absolute;top:0;left:0;width:100%;height:100%;background:#a0beff;moz-opacity:0.8;opacity:0.8;filter:alpha(opacity:80)'></div>"+
									"<span id='"+this.getFullId()+"_msg' style='position:absolute;top:10;width:80%;left:10;height:auto;font-size:13'>"+result.rs.rows[0].msg+"</span>";
						var span = $(this.getFullId()+"_msg");
						if (parseFloat(span.offsetHeight) + this.msgPanel.top + 20> this.height) this.msgPanel.setHeight(this.msgPanel.top + (this.height - this.msgPanel.top - 80));
						else this.msgPanel.setHeight(parseFloat(span.offsetHeight) + 20);
					}else this.msgPanel.getCanvas().innerHTML = "<span>"+result+"</span>";					
				}
			}catch(e){
				alert(e);
			}
		}
		if (sender == this.dbLib2 && connection == this.listInfo.ajax){					
			if (methodName == "getDataProvider"){
				if (errCode == 200){
					var beda = true;
					try{
					...
					if (this.lastData === undefined) var beda = true;
					else var beda = this.lastData && this.lastData.getLength() != result.rs.rows.length;					
					if (!beda){
						var line;
						for (var i in result.rs.rows){
							line = result.rs.rows[i];
							beda = this.lastData.get(line.progress+"-"+line.nama +"("+line.kode_lokfa2+")") == undefined;
							if (beda) break;
						}
					}					
					if (beda){
						this.lastData = new arrayMap();						
						this.listInfo.grid.clear();
						for (var i in result.rs.rows){
							line = result.rs.rows[i];							
							this.listInfo.grid.appendData([line.progress+"-"+line.nama +"("+line.kode_lokfa2+")", line.totnka]);
							this.lastData.set(line.progress+"-"+line.nama +"("+line.kode_lokfa2+")", line.totnka);
						}
					}
				}
			}
				...
				switch (methodName){
					...
					case  "execArraySQL":
						if (result.search("error") > -1){
							system.alert(this,"Error send message",result)
						}else{
							this.sendMsgPanel.fadeOut();
						}
					break;
				}
			}			
	...
	doBtnList: function(sender){
		if (sender == this.btnList){
			if (!this.listInfo.visible)
			{
				...
				this.msgPanel.fadeOut();
			}
		}else {
			if (!this.sendMsgPanel.visible)
			{
				this.sendMsgPanel.show();		
				this.sendMsgPanel.fade();			
				this.sendMsgPanel.bringToFront();				
			}else {
				this.sendMsgPanel.fadeOut();								
			}
		}
...
//------------------------------------------------------------------- locking transaction		
	...
	doSendMsg: function(sender){		
		this.dbLib.execArraySQL(new server_util_arrayList({items:["insert into amu_msg(msg,nik_user, tgl_input)values('"+this.sendMsgPanel.editor.getText()+"','"+this.app._userLog+"',sysdate)"]}), undefined, this);
	}
});
