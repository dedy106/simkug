...
//* 			SAI, PT											
//***********************************************************************************************
window.app_assetsap_fLogin = function(owner){
	if (owner){
		try{		
			window.app_assetsap_fLogin.prototype.parent.constructor.call(this, owner);
			this.className  = "app_assetsap_fLogin";			
			...
			this.e0.setFocus();	
			this.app._rfcSetting = "veat/sap";
			this.onClose.set(this,"doClose");
		}catch(e){
			systemAPI.alert("[app_assetsap_fLogin]::oncreate lib : ",e);
		}
	}
};
window.app_assetsap_fLogin.extend(window.childForm);
window.app_assetsap_fLogin.implement({
    ...
    mainButtonClick: function(sender){		
		try{
			...
			this.dbLib.loginAndLoad("select  a.kode_klp_menu, a.nik, a.nama, a.pass, a.status_admin, a.kode_lokfa, a.klp_akses, a.kode_lokasi,b.nama as nmlok, c.nama as nmlokfa from amu_hakakses a inner join lokasi b on b.kode_lokasi = a.kode_lokasi inner join amu_lokasi c on a.kode_lokfa=c.kode_lokfa where nik= '"+this.e0.getText()+"' \r\n"+
						...
						"select 'a' as kode_form from dual \r\n"+						
						"select max(periode) as periode from amu_asset \r\n "+
						"select nik, nama from amu_karyawan where kode_lokasi = 'table1_kode_lokasi'"
		...
		try{
					...
					case "loginAndLoad":						
						...
						if ( (kodeMenu!= "") && (kodeMenu != undefined)){
							...
							if ( (lokasi!= "") && (lokasi != undefined) && (kodeMenu != lokasi)){
								setCookie("amuroojaxu",this.e0.getText(),1);		
								setCookie("amuroojaxp",this.e1.getText(),1);		
								setCookie("amudbSetting",this.app._dbSetting,1);
								setCookie("amurfcSetting",this.app._rfcSetting,1);
								setCookie("amuappState",this.app._appState,1);
			...
			//------------------------------------------------------------------- locking transaction													
								...
								for (var i=1; i < data.length;i++){
									...
									this.app._listNIK.set(line[0],line[1]);
								}
			...
			system.hideProgress();
			system.alert(this,"Login Gagal dilakukan.",e+"<br>"+result);			
		}
	},
