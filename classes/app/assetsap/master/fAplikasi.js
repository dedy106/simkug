window.app_assetsap_master_fAplikasi = function(owner)
{
	if (owner)
	{
		window.app_assetsap_master_fAplikasi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_assetsap_master_fAplikasi";
		this.maximize();
...
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		...
		{		
			...
			this.ed_kode = new saiCBBL(this, {
				bound: [20, 10, 200, 20],
				caption: "Kode Aplikasi",
				multiSelection:false, 
				rightLabelVisible:false,
				sql:["select kode_aplikasi, nama from amu_aplikasi ", ["kode_aplikasi", "nama"], false, ["Kode Aplikasi","Nama"], "and", "Daftar Aplikasi",false]
			});			
...
window.app_assetsap_master_fAplikasi.extend(window.childForm);
//------------------------------------------------------------------ event
window.app_assetsap_master_fAplikasi.implement({
			...
			switch (event)
			{
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
							...
							sql.add("insert into amu_aplikasi(kode_aplikasi, nama)  values "+
