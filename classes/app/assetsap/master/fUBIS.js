window.app_assetsap_master_fUBIS = function(owner)
{
	if (owner)
	{
		window.app_assetsap_master_fUBIS.prototype.parent.constructor.call(this,owner);
		this.className  = "app_assetsap_master_fUBIS";
		this.maximize();
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data UBIS", 0);	
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		try
		{		
...
window.app_assetsap_master_fUBIS.extend(window.childForm);
//------------------------------------------------------------------ event
window.app_assetsap_master_fUBIS.implement({
			...
			switch (event)
			{
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
							...
							sql.add("insert into amu_ubis(kode_ubis, nama)  values "+
				...
				case "ubah" :
					if (modalResult == mrOk)
					{
							...
							sql.add("update amu_ubis set  "+
									"nama = '"+this.ed_nama.getText()+"' "+
									"where kode_ubis = '"+this.ed_kode.getText()+"' ");
							this.dbLib.execArraySQL(sql);	
					}
					break;
				case "hapus" :
				   if (modalResult == mrOk)
				   {
						uses("server_util_arrayList");					
							var sql = new server_util_arrayList();
							sql.add("delete from amu_ubis where kode_ubis='"+this.ed_kode.getText()+"' ");
							this.dbLib.execArraySQL(sql);	
				   }
					break;
			}			
			...
			if (this.ed_kode.getText() != "")
			{
				...
				{					
					...
					var data = this.dbLib.getDataProvider("select nama "+
													"from amu_ubis a "+
													"where kode_ubis = '"+this.ed_kode.getText()+"' ",true);
