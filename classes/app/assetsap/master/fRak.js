window.app_assetsap_master_fRak = function(owner)
{
	if (owner)
	{
		window.app_assetsap_master_fRak.prototype.parent.constructor.call(this,owner);
		this.className  = "app_assetsap_master_fRak";
		this.maximize();
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Rak Ordner", 0);	
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		try
		{		
			...
			this.ed_awal = new saiCBBL(this, {
				bound: [20, 32, 200, 20],
				caption: "No.Lokasi Awal",
				multiSelection:false, 
				sql:["select kode_lokfa, alamat from amu_lokfa where kode_lokasi = '"+this.app._lokasi+"' ", ["kode_lokfa", "alamat"], false, ["Lokasi Aset","Alamat"], "and", "Daftar Lokasi",true]
			});							
			this.ed_akhir = new saiCBBL(this, {
				bound: [20, 33, 200, 20],
...
window.app_assetsap_master_fRak.extend(window.childForm);
//------------------------------------------------------------------ event
window.app_assetsap_master_fRak.implement({
				...
				try
				{
					...
					if (typeof data != "string"){
						if  (data.rs.rows[0]) {
							...
							this.ed_awal.setText("");
							this.ed_akhir.setText("");
							setTipeButton(tbSimpan);
						}
					}else{	
					  this.ed_awal.setText("");
			...
			switch	(methodName)
			{
				case "execArraySQL" :
					if (result.toLowerCase().search("error") == -1){
					  this.app._mainForm.pesan(2,"Transaksi Sukses ("+ this.ed_awal.getText()+")");
