window.app_assetsap_master_fPropinsi = function(owner)
{
	if (owner)
	{
		window.app_assetsap_master_fPropinsi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_assetsap_master_fPropinsi";
		this.maximize();
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Propinsi", 0);	
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
		try
		{		
...
window.app_assetsap_master_fPropinsi.extend(window.childForm);
//------------------------------------------------------------------ event
window.app_assetsap_master_fPropinsi.implement({
			...
			switch (event)
			{
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
							...
							sql.add("insert into amu_propinsi(kode_prop,nama,kode_lokasi,nik_user,tgl_input)  values "+
				...
				case "ubah" :
					if (modalResult == mrOk)
					{
							...
							sql.add("update amu_propinsi set  "+
									"nama = '"+this.ed_nama.getText()+"',nik_user='"+this.app._userLog+"',tgl_input = now() "+
									"where kode_prop = '"+this.ed_kode.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
							this.dbLib.execArraySQL(sql);	
					}
					break;
				case "hapus" :
				   if (modalResult == mrOk)
				   {
						uses("server_util_arrayList");					
							var sql = new server_util_arrayList();
							sql.add("delete from amu_propinsi where kode_prop='"+this.ed_kode.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
							this.dbLib.execArraySQL(sql);	
				   }
					break;
			}
			...
			if (this.ed_kode.getText() != "")
			{
				try
				{
					...
					var data = this.dbLib.getDataProvider("select nama  "+
													"from amu_propinsi "+
													"where kode_prop = '"+this.ed_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
