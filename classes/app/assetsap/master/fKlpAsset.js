 ...
 * @author dweexfuad
 */
window.app_assetsap_master_fKlpAsset = function(owner)
{
	if (owner)
	{
		window.app_assetsap_master_fKlpAsset.prototype.parent.constructor.call(this, owner);
		this.className = "app_assetsap_master_fKlpAsset";
		...
		this.relBtn = new imageButton(this,{
			bound: [87, 11, 22, 22],
			hint: "Root",
...
window.app_assetsap_master_fKlpAsset.extend(window.childForm);
window.app_assetsap_master_fKlpAsset.implement({
			...
			switch (event)
			{
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
						...
						try
						{						
							var sql2,sql1 = "delete from amu_klp where kode_lokasi = '"+this.elokasi.getText()+"'";					// kode_lokasi,						
							var sql = new server_util_arrayList();
							sql.add(sql1);													
							var data = value.split("#");
							for (var i in data){
								sql2 = "insert into amu_klp(kode_klpfa, nama, kode_lokasi,tipe, rowindex, kode_induk,level_spasi,  nik_user, tgl_input) values ";
								sql2 += data[i];
								sql.add(sql2);
							}							
	...
	doClick: function(sender){	
		var nrc = this.dblib.runSQL("select kode_klpfa, nama, kode_lokasi,tipe, rowindex, kode_induk,level_spasi from amu_klp where  kode_lokasi = '"+this.elokasi.getText()+"' order by rowindex",0,0);	
		this.loadMenu(nrc);
	},
	entriesClick: function(sender){
		...
		{ 
			...
			{
				this.treev.setSelectedItem(undefined);
