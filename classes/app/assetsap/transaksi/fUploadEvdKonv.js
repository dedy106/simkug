 ...
 * @author dweexfuad
 */
window.app_assetsap_transaksi_fUploadEvdKonv = function(owner, ba, klpfa, fa, formView) {
	if (owner){
		
		window.app_assetsap_transaksi_fUploadEvdKonv.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_assetsap_transaksi_fUploadEvdKonv";		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Upload Evidence KK Konversi", 0);	
		...
		this.ed_kode = new saiCBBL(this,{bound:[20,2,220,20],caption:"No Konversi",readOnly:false,
			multiSelection:false,
			change:[this,"doChange"]
		});						
...
window.app_assetsap_transaksi_fUploadEvdKonv.extend(window.childForm);
window.app_assetsap_transaksi_fUploadEvdKonv.implement({
				...
				case "simpan" :					
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){						
						if (this.ed_file.getText() != "-"){
							...
							var sql = new server_util_arrayList();
							sql.add("update amu_alt_konv_m set lampiran = '"+this.ed_file.getText()+"' where no_konv = '"+this.ed_kode.getText()+"' ");
	...
	doChange: function(sender){
		try{
			if (sender == this.ed_lokfa){						
				this.ed_kode.setSQL("select distinct no_konv, keterangan from amu_alt_konv_m "+
					"where kode_lokfa = '"+this.ed_lokfa.getText()+"'  ", ["no_konv","keterangan"],false, ["No Konversi","keterangan"], "and","Daftar Konversi",true);
			}	
			...
			if (sender == this.ed_kode){					
				this.no_inv = " ";
				var data = this.dbLib.getDataProvider("select distinct lampiran  from amu_alt_konv_m a inner join amu_alt_konv_d b on b.no_konv = a.no_konv where a.no_konv = '"+this.ed_kode.getText()+"' ",true);
				if (data.rs.rows[0]){
