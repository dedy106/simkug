 ...
 * @author dweexfuad
 */
window.app_assetsap_transaksi_fUploadEvdBA = function(owner, ba, klpfa, fa, formView) {
	if (owner){
		
		window.app_assetsap_transaksi_fUploadEvdBA.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_assetsap_transaksi_fUploadEvdBA";		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Upload Evidence KK BA Verifikasi", 0);	
		...
		this.ed_kode = new saiCBBL(this,{bound:[20,2,220,20],caption:"No BA Verifikasi",readOnly:false,
			multiSelection:false,
			change:[this,"doChange"]
		});						
...
window.app_assetsap_transaksi_fUploadEvdBA.extend(window.childForm);
window.app_assetsap_transaksi_fUploadEvdBA.implement({
				...
				case "simpan" :					
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){						
						if (this.ed_file.getText() != "-"){
							...
							var sql = new server_util_arrayList();
							sql.add("update amu_alt_baver_m set lampiran = '"+this.ed_file.getText()+"' where no_ba = '"+this.ed_kode.getText()+"' ");
	...
	doChange: function(sender){
		try{
			if (sender == this.ed_lokfa){						
				this.ed_kode.setSQL("select distinct no_ba, keterangan from amu_alt_baver_m "+
					"where kode_lokfa = '"+this.ed_lokfa.getText()+"'  ", ["no_ba","keterangan"],false, ["No BA Verifikasi","Keterangan"], "and","Daftar BA Verifikasi",true);
			}	
			...
			if (sender == this.ed_kode){					
				this.no_inv = " ";
				var data = this.dbLib.getDataProvider("select distinct lampiran  from amu_alt_baver_m a  where a.no_ba = '"+this.ed_kode.getText()+"' ",true);
				if (data.rs.rows[0]){
