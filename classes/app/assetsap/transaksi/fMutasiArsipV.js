...
//* 			SAI, PT											
//***********************************************************************************************
window.app_assetsap_transaksi_fMutasiArsipV = function(owner,options){
	try{
		if (owner)
		{
			window.app_assetsap_transaksi_fMutasiArsipV.prototype.parent.constructor.call(this, owner,options);
			this.className = "app_assetsap_transaksi_fMutasiArsipV";
			this.maximize();
			this.app._mainForm.childFormConfig(this, "mainButtonClick","Daftar Mutasi Arsip",0);
            this.initComponent();
	...
	{
		alert("[app_assetsap_transaksi_fMutasiArsipV]::contruct:"+e,"");
	}
};
window.app_assetsap_transaksi_fMutasiArsipV.extend(window.childForm);
window.app_assetsap_transaksi_fMutasiArsipV.implement({
    ...
    initComponent: function(){
		try{
			...
			this.sg1mp = new saiGrid(this.p1mp,{bound:[1,20,this.p1mp.width - 4,this.p1mp.height - 50],colCount:8,
				colTitle:"No Mutasi, No BA, Tanggal, Tgl Balik, Keterangan, Pembuat, Disetujui Oleh, Diterima Oleh ",
                colWidth:[[7,6,5,4,3,2,1,0],[100,100,100,150,80,80,100,100]],dblClick:[this,"sg1onDblClick"], 
                selectCell:[this,"doSgClick"], readOnly:true});
			...
			this.pHist = new panel(this,{bound:[10,300,this.width - 535,180],border:3,caption:"Data Arsip"});
			this.sgHist = new saiGrid(this.pHist,{bound:[1,20,this.pHist.width - 4,128],colCount:13,readOnly:true,
			...
			this.sgnHist = new sgNavigator(this.pHist,{bound:[1,this.pHist.height - 25,this.pHist.width - 2,25],buttonStyle:3, pager:[this,"doPager"],beforePrint:[this,"doBeforePrintSg2"], grid:this.sgHist});            
			
			this.p1mp2 = new panel(this,{bound:[this.pHist.width + 20,300,500,180],border:3,caption:"Detail File"});
			...
			this.pager = 20;
			var sql = "select count(a.no_mutasi)	   "+                         
					"from amu_mutasi_m a "+					
					"where a.kode_lokasi='"+this.app._lokasi+"' ";
			this.sqlCount = sql;
            ...
            //No BA, Tanggal, Tgl Balik, Keterangan, Pembuat, Disetujui Oleh, Diterima Oleh            
			this.sql =  "select distinct a.no_mutasi,a.no_ba, date_format (a.tanggal,'%d-%m-%Y') as tanggal, date_format (a.tgl_balik,'%d-%m-%Y') as tgl_balik, a.keterangan, a.nik_buat, a.nik_setuju, a.nik_terima, b.nama as nmbuat, c.nama as nmsetuju, d.nama as nmterima  "+
					" from amu_mutasi_m a "+
					" inner join amu_karyawan b on b.nik = a.nik_buat and b.kode_lokasi = a.kode_lokasi "+
					"	left outer join amu_karyawan c on c.nik = a.nik_setuju and c.kode_lokasi = a.kode_lokasi "+
					"	left outer join amu_karyawan d on d.nik = a.nik_terima and d.kode_lokasi = a.kode_lokasi "+
					"where a.kode_lokasi = '"+this.app._lokasi+"' ";				
			this.filter = "";
	...
	doPager: function(sender,page){
			...
			if (sender == this.sgn){
				...
				this.dbLib.getDataProviderPageA(this.sql+" "+this.filter+" order by a.no_mutasi",page, this.pager);   			 
			}
    },
					...
					if (data){			        
						...
						if (data.rs.rows[0]){
							...
							for (var i in data.rs.rows){
								line = data.rs.rows[i];
								//No BA, Tanggal, Tgl Balik, Keterangan, Pembuat, Disetujui Oleh, Diterima Oleh            
								this.sgHist.appendData([line.no_mutasi, line.no_ba, line.tanggal, line.tgl_balik, line.keterangan, line.nmbuat, line.nmsetuju, line.nmterima  ]);
								this.sgHist.rows.get(i).setCellColor(12,"#ff9900");
								this.sgHist.rows.get(i).setHint("Double Click untuk menampilkan detail.");						
							}
	...
	doSgClick:function(sender, col, row){				
		try{
			...
			if (sender == this.sg1mp){
				var data = this.dbLib.getDataProvider("select distinct a.no_arsip, a.no_surat, a.nama, a.kode_lokfa, b.alamat, b.kel, b.kec, b.kode_kodya as kodya, b.kode_prop as prop, b.tanah, b.bangun "+
				...
				if (typeof data != "string"){
					...
					for (var i in data.rs.rows){
						...
						this.sgHist.appendData([line.no_arsip, line.no_surat, line.nama, line.kode_lokfa, ordner.kode_rak,line.alamat, line.kel, line.kec, dt.nmkodya, dt.nmprop, line.tanah, line.bangun,"Preview"]);											
					}					
				}
			}
		}catch(e){
