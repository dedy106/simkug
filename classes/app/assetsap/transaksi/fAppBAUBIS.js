 ...
 * @author dweexfuad
 */
window.app_assetsap_transaksi_fAppBAUBIS = function(owner) {
	if (owner){
		window.app_assetsap_transaksi_fAppBAUBIS.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_assetsap_transaksi_fAppBAUBIS";		
		...
		this.p1 = new pageControl(this,{bound:[20,11,600,230],childPage:["Data Berita Acara","BA TB","BA NTB","Prosedur Alternatif"]});
		this.sg = new saiGrid(this.p1.childPage[0], {
			bound: [1, 20, 596, 180],
			colCount: 2,
			colTitle: ["No BA","Keterangan"],
...
window.app_assetsap_transaksi_fAppBAUBIS.extend(window.childForm);
window.app_assetsap_transaksi_fAppBAUBIS.implement({
			...
			switch(event){
				...
				case "simpan" :
					...
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						sql.add("insert into amu_baubis_m (no_ba,kode_lokasi, kode_lokfa, tgl_ba,tgl_inv_mulai, tgl_inv_selesai,  nik_user, tgl_input, keterangan, nik_app1, nik_app2, nik_app3, jenis, periode, jenis_ba, alamat, tempat, progress)"+
							...
							" '"+this.app._userLog+"',now(),'"+this.ed_ket.getText()+"','"+this.ed_nik1.getText()+"','"+this.ed_nik2.getText()+"','-' ,'ALL', '"+this.app._periode+"','ALL', '"+this.ed_alamat.getText()+"','"+this.ed_tempat.getText()+"','0')");
						var line;
						for (var i=0;i < this.sg.getRowCount();i++){													
							sql.add("insert into amu_baubis_d (no_ba, kode_lokasi, no_ba_lain) "+
								" values('"+this.ed_kode.getText()+"','"+this.app._lokasi+"','"+this.sg.cells(0,i)+"') ");
						}												
