...
window.app_assetsap_transaksi_fUpload = function(owner) {
	if (owner){
		try{
			...
			this.app._mainForm.childFormConfig(this, "mainButtonClick","Upload Data Asset SAP AM", 0);	
				...
				case "simpan" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						for (var i in this.dataUpload.rows ){
							...
							data = ["'"+this.app._lokasi+"'",line["Asset"]+line["SNo."]];
							for (var c in line){
							...
							data[data.length] = "1";
							sql.add("insert into amu_asset_d (kode_lokasi,no_gabung,no_fa, no_sn,  nama, nama2,kode_klpakun, kode_klpfa, kode_lokfa, ref1, ref2,  tgl_perolehan, nilai, nilai_ap, nilai_buku,jml_fisik )values("+ data+")");
							sql.add("insert into amu_load_d (no_load, kode_lokasi, no_gabung,no_fa, no_sn,  nama, nama2,kode_klpakun, kode_klpfa, kode_lokfa, ref1, ref2,  tgl_perolehan, nilai, nilai_ap, nilai_buku,jml_fisik )values('"+this.ed_kode.getText()+"',"+ data+")");
						...
						* */
						this.svrUpload.upload(this.ed_kode.getText(),this.app._periode,  this.dp_tgl.getDateString(), this.ed_ket.getText(), this.ed_nmFile.getText(), this.app._lokasi, this.app._userLog);
					}
				break;
				case "ubah" :
