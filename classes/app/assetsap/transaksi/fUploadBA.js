 ...
 * @author dweexfuad
 */
window.app_assetsap_transaksi_fUploadBA = function(owner, ba, klpfa, fa, formView) {
	if (owner){
		
		window.app_assetsap_transaksi_fUploadBA.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_assetsap_transaksi_fUploadBA";		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Upload Evidence BA Inventarisasi", 0);	
...
window.app_assetsap_transaksi_fUploadBA.extend(window.childForm);
window.app_assetsap_transaksi_fUploadBA.implement({
		...
		try{
			if (sender == this.ed_lokfa){						
				this.ed_kode.setSQL("select distinct a.no_ba from amu_ba_m a "+					
					"where a.kode_lokfa = '"+this.ed_lokfa.getText()+"' and a.jenis_ba = 'KKIL' ", ["no_ba"],false, ["NO Berita Acara"], "and","Daftar Berita Acara",true);
			}	
