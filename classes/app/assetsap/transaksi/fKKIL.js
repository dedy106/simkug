 ...
 * @author dweexfuad
 */
window.app_assetsap_transaksi_fKKIL = function(owner, ba, klpfa, fa, formView) {
	if (owner){
		try{
			window.app_assetsap_transaksi_fKKIL.prototype.parent.constructor.call(this,owner);
			this.maximize();
			this.className  = "app_assetsap_transaksi_fKKIL";		
			this.app._mainForm.childFormConfig(this, "mainButtonClick","Inventarisasi Tanah Bangunan", 0);	
			...
			this.ed_kode = new saiLabelEdit(this,{bound:[20,2,230,20],caption:"No Inventarisasi",readOnly:true});
			...
			//this.ed_area = new saiLabelEdit(this,{bound:[20,10,200,20], caption:"Area",text:""});
...
window.app_assetsap_transaksi_fKKIL.extend(window.childForm);
window.app_assetsap_transaksi_fKKIL.implement({
				...
				case "simpan" :
					...
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						if (this.dataUpload === undefined){							
							...
							sql.add("insert into amu_kkl_m (no_inv,  kode_lokasi, kode_lokfa, tanggal, nik_user, tgl_input, jenis, progress, nik_buat, lampiran, kode_lokfa2)values"+
								"('"+this.ed_kode.getText()+"', '"+this.app._lokasi+"','"+this.ed_lokfa.getText()+"','"+this.dp_tgl.getDateString()+"','"+this.app._userLog+"',now(), '"+this.ed_jenis.getText()+"','0', '"+this.ed_nik1.getText()+"','"+this.ed_file.getText()+"', '"+this.ed_area.getText()+"')");
							var status = 99;
	...
	doChange: function(sender){
		...
		try{			
			if (sender == this.ed_lokfa){
				...
				this.ed_klpfa.setSQL("select distinct b.kode_klpfa, b.nama, e.group_aset from amu_klp a "+
			...
			if (sender == this.ed_klpfa){
				...
				this.dbLib.getDataProviderA("select a.no_gabung, a.no_fa, a.no_sn, a.nama from amu_asset a "+
					...
					" where a.periode = '"+this.app._periode+"' and a.kode_lokasi = '"+this.app._lokasi+"' and a.kode_klpfa = '"+this.ed_klpfa.getText()+"' "+
					" and (a.kode_lokfa = '"+this.ed_lokfa.getText()+"' or a.ref1 = '"+( diva ? this.ed_lokfa.getText() :"" )+"' ) ");
			}	
			...
			if (col == 2){
				if (sender.cells(2,row) != "" && sender.cells(2,row).length != 18 && this.ed_klpfa.getText() != "101101"){
					switch (this.sg.cells(3,row)){
						...
						break;
						default:
							system.alert(this,"No Label harus 18 digit.(Abaikan pesan ini jika akan memasukkan status 2, 4, dan 5).","Cek Inputan Label Kembali");
				...
				for (var i=0; i < this.sg.getRowCount();i++){
					if (this.sg.cells(2,i) == this.sg.cells(2,row) && i != row && this.sg.cells(2,row) != "-" ) {
	...
	doCopy: function(sender){
		...
		if (sender == this.bLoad){
			var data = this.dbLib.getDataProvider("select no_gabung,no_fa,no_sn, alamat, jml_fisik, no_label, kode_status, nama, no_sertifikat, luas,ket_lokasi, keterangan from amu_kkl_d where no_gabung = '"+this.ed_nofa.getText()+"' and periode = '"+this.cb_periode.getText()+"'",true);
			if (typeof data == "string"){
	...
	doDownload: function(sender){
		downloadFile(this.fileUtil.getFileContents( this.rootDir+"/docs/template/tb.xls"));
	},
 ...
 * if (sender == this.ed_kode){
				var data = this.dbLib.getDataProvider("select d.kode_klpfa, a.nik_buat, date_format(a.tanggal,'%d-%m-%Y') as tgl, b.no_gabung, "+
					" b.alamat, b.jml_fisik, b.no_label, b.kode_status, c.nama as nm_status, b.no_sertifikat, b.luas, b.ket_lokasi, b.keterangan  "+
