 ...
 * @author dweexfuad
 */
window.app_assetsap_transaksi_fEvidenceK = function(owner) {
	if (owner){
		try{
			window.app_assetsap_transaksi_fEvidenceK.prototype.parent.constructor.call(this,owner);
			this.maximize();
			this.className  = "app_assetsap_transaksi_fEvidenceK";		
			this.app._mainForm.childFormConfig(this, "mainButtonClick","Evidence Data Aset (Koreksi)", 0);	
			...
			this.ed_kode = new saiCBBL(this,{bound:[20,2,200,20],caption:"No Evidence",readOnly:true, multiSelection:false, change:[this,"doChange"], rightLabelVisible:false});			
...
window.app_assetsap_transaksi_fEvidenceK.extend(window.childForm);
window.app_assetsap_transaksi_fEvidenceK.implement({
			...
			switch(event){
				...
				case "ubah" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						sql.add("delete from amu_evd_d where no_evd = '"+this.ed_kode.getText()+"' ");
						sql.add("delete from amu_evd_dok where no_evd = '"+this.ed_kode.getText()+"' ");
						sql.add("update amu_evd set  nama='"+this.ed_refevd.getText()+"', nik_buat ='"+this.ed_nik1.getText()+"', tgl_buat='"+this.dp_tgl.getText()+"', nik_review='"+ this.ed_nik2.getText()+"', tgl_review='"+this.dp_tgl2.getDateString()+"',kesimpulan='"+this.ed_kes.getText()+"' where no_evd = '"+this.ed_kode.getText()+"' ");
						...
						var files = [];
						for (var r in this.dataFileOld){
							for (var i=0;i < this.sgUpld.getRowCount();i++){
								if (this.sgUpld.rowValid(i)){
									var ada = this.dataFileOld[r].filedest == this.sgUpld.cells(1,i).filedest;							
									this.dataFileOld[r].ada = ada;
									if (ada) break; 									
								}
							}
						...
						for (var i=0;i < this.sgUpld.getRowCount();i++){
							if (this.sgUpld.rowValid(i)){
								...
								var ada = false;
								for (var r in this.dataFileOld){
									ada = this.dataFileOld[r].filedest == this.sgUpld.cells(1,i).filedest;
									if (ada) break;
								}
								if (!ada){
									if (this.saveFiles != "") {
				...
				case "hapus" :
					sql.add("delete from amu_evd_d where no_evd = '"+this.ed_kode.getText()+"' ");
					sql.add("delete from amu_evd_dok where no_evd = '"+this.ed_kode.getText()+"' ");
					sql.add("delete from amu_evd where no_evd ='"+ this.ed_kode.getText()+"' ");
					this.dbLib.execArraySQL(sql);
				break;
			}			
		}catch(e){
	...
	doChange: function(sender){
		if (sender == this.ed_kode){
			var data = this.dbLib.getMultiDataProvider(new server_util_arrayList({items:["select  nama, date_format(tgl_buat,'%d-%m-%Y') as tgl, nik_review, date_format(tgl_review,'%d-%m-%Y') as tgl2, nik_buat, kesimpulan from  amu_evd where no_evd = '"+this.ed_kode.getText()+"' ",				
				"select no_gambar, nu from amu_evd_dok where no_evd = '"+ this.ed_kode.getText()+"' order by nu",
				"select param, value,no_urut from amu_evd_d where no_evd = '"+ this.ed_kode.getText()+"' order by no_urut"
				]}), true);
			setTipeButton(tbAllFalse);
			if (typeof data != "string" && data.result[0].rs.rows[0]){
				try{
					var data1 = data.result[0];
					var line = data1.rs.rows[0];
					this.dp_tgl.setText(line.tgl);
					this.dp_tgl2.setText(line.tgl2);
					this.ed_kes.setText(line.kesimpulan);
					this.ed_nik2.setText(line.nik_review);
					this.ed_nik1.setText(line.nik_buat);
					this.ed_refevd.setText(line.nama);
					data1 = data.result[1];
					this.dataFileOld = [];
					this.sgUpld.clear();
					for (var i in data1.rs.rows){
						line = data1.rs.rows[i];
						this.sgUpld.appendData([line.no_gambar, {tmpfile:line.no_gambar, filedest:line.no_gambar}]);
						this.dataFileOld[this.dataFileOld.length] = {tmpfile:line.no_gambar, filedest:line.no_gambar};
					}
					data1 = data.result[2];
					this.sg.clear();					
					for (var i in data1.rs.rows){
						line = data1.rs.rows[i];
						this.sg.appendData([line.param, line.value]);
					}
		...
		if (sender == this.ed_lokfa){
			...
			this.ed_kode.setSQL("select no_evd, nama from amu_evd where kode_lokfa = '"+ this.ed_lokfa.getText() +"' ",["no_evd","nama"],false,["No Evidence","Referensi Evidence"],"where","Daftar Evidence",true);
		}
			...
			if (sender == this.dbLib)
			{
				switch	(methodName)
				{
					case "execArraySQL" :
						...
						{						  
						  if (this.sysEvent == "hapus"){
								var delFile = "";
								for (var i in this.dataFileOld){
									if (i > 0) delFile += ";";
									delFile += this.rootDir+"/server/media/" + this.delFileOld[i].filedest; 
								}
