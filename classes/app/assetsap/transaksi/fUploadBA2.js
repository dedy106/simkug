 ...
 * @author dweexfuad
 */
window.app_assetsap_transaksi_fUploadBA2 = function(owner, ba, klpfa, fa, formView) {
	if (owner){
		window.app_assetsap_transaksi_fUploadBA2.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_assetsap_transaksi_fUploadBA2";		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Upload Evidence BA Rekonsiliasi", 0);	
		uses("util_file;uploader;saiCBBL;");												
		...
		this.ed_kode = new saiCBBL(this,{bound:[20,2,220,20],caption:"No BA",readOnly:true,
			multiSelection:false,
			change: [this,"doChange"]
		});						
...
window.app_assetsap_transaksi_fUploadBA2.extend(window.childForm);
window.app_assetsap_transaksi_fUploadBA2.implement({
	...
	doChange: function(sender){
		try{
			if (sender == this.ed_lokfa){																						
				this.ed_kode.setSQL("select distinct a.no_ba from amu_ba_m a "+					
					"where a.kode_lokfa = '"+this.ed_lokfa.getText()+"'  and a.jenis_ba = 'REKON' ", ["no_ba"],false, ["NO Berita Acara"], "and","Daftar Berita Acara",true);				
			}	
			if (sender == this.ed_kode){					
				var data = this.dbLib.getDataProvider("select file_ba from amu_ba_m where no_ba = '"+this.ed_kode.getText()+"' ",true);
				if (data.rs.rows[0]){
					this.ed_file.setText(data.rs.rows[0].file_ba);
				}
			}	
	...
	doRequestReady: function(sender, methodName, result){		
		if (sender == this.dbLib)
		{
			...
			if (methodName == "copyFilesTo"){   
				var sql = new server_util_arrayList();
				sql.add("update amu_ba_m set file_ba = '"+this.ed_file.getText()+"' where no_ba = '"+this.ed_kode.getText()+"' ");
