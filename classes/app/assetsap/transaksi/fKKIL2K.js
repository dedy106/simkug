 ...
 * @author dweexfuad
 */
window.app_assetsap_transaksi_fKKIL2K = function(owner, ba, no_inv, formView) {
	if (owner){
		try{
			window.app_assetsap_transaksi_fKKIL2K.prototype.parent.constructor.call(this,owner);
			this.maximize();
			this.className  = "app_assetsap_transaksi_fKKIL2K";		
			this.app._mainForm.childFormConfig(this, "mainButtonClick","Inventarisasi Non Tanah Bangunan (Koreksi)", 0);	
			...
			this.ed_jenis = new saiLabelEdit(this,{bound:[20,10,150,20], caption:"Jenis",text:"NTB", readOnly:true, tag:9});
...
window.app_assetsap_transaksi_fKKIL2K.extend(window.childForm);
window.app_assetsap_transaksi_fKKIL2K.implement({
						...
						if (this.dataUpload === undefined){							
							...
							for (var row=0; row < this.sg.getRowCount();row++){
								if (this.sg.rowValid(row)){
									...
									if (this.sg.cells(2, row).length != 18 && parseFloat(this.sg.cells(3, row)) < 5 ) {
										system.alert(this,"Ada Label yg Panjangnya tidak 18 digit("+trim(this.sg.cells(2, row))+" baris "+row +")","Proses tidak dilanjutkan...");
										return;
									}else if (trim(this.sg.cells(2, row)).substr(0,4) != "TLKM" && parseFloat(this.sg.cells(3, row)) < 5 ) {
							...
							sql.add("update amu_kkl_m set kode_lokfa = '"+this.ed_lokfa.getText()+"' , tanggal = '"+this.dp_tgl.getDateString()+"' "+
								" , nik_modified='"+this.app._userLog+"', tgl_modified=now(),nik_buat='"+this.ed_nik1.getText()+"', nik_app='"+this.ed_nik2.getText()+"' "+
							...
							for (var i=0;i < this.sg.getRowCount();i++){
								if (this.sg.rowValid(i)){
									...
									"'"+this.sg.cells(1,i)+"','"+ this.sg.cells(2,i)+"','"+ this.sg.cells(3,i)+"','"+this.sg.cells(5,i)+"','-','-','"+ this.sg.cells(5,i)+"','"+ this.sg.cells(6,i)+"','"+this.app._periode+"' )");
								}
							}						
							this.dbLib.execArraySQL(sql);
							//this.svrUpload2.uploadAttachment();
						}else this.svrUpload.uploadKKIL(this.ed_kode.getText(), this.app._periode, this.dp_tgl.getDateString(), this.ed_jenis.getText(), this.app._lokasi, this.ed_lokfa.getText(), this.app._userLog,this.ed_nik1.getText(),this.filename);
		...
		try{
			if (sender == this.ed_kode){
				var data = this.dbLib.getDataProvider("select d.kode_klpfa, a.nik_buat, date_format(a.tanggal,'%d-%m-%Y') as tgl, b.no_gabung, "+
					" b.alamat, b.jml_fisik, b.no_label, b.kode_status, c.nama as nm_status, b.no_sertifikat, b.luas, b.ket_lokasi, b.keterangan, a.lampiran, a.kode_lokfa2, a.nik_app  "+
					...
					"	inner join amu_asset d on d.no_gabung = b.no_gabung and d.periode = b.periode "+
					" where a.no_inv = '"+this.ed_kode.getText()+"' ", true);
				this.ed_nofa.setSQL("select distinct d.no_gabung, d.no_fa, d.no_sn, d.nama "+
					" from amu_kkl_m a inner join amu_kkl_d b on b.no_inv = a.no_inv "+
					"	inner join amu_status c on c.kode_status = b.kode_status and c.jenis = 'NTB' "+
					"	inner join amu_asset d on d.no_gabung = b.no_gabung and d.periode  = b.periode "+
			   ...
			   if (data.rs.rows[0]){
					...
					this.ed_nik2.setText(line.nik_app);
					...
					for (var i in data.rs.rows){						
						this.sg.appendData([line.alamat, line.jml_fisik, line.no_label, line.kode_status, line.nm_status, line.ket_lokasi, line.keterangan]);				
			...
			if (sender == this.ed_lokfa){
				...
				this.ed_klpfa.setSQL("select distinct a.kode_klpfa, a.nama, e.group_aset from amu_klp a "+
					...
					" where a.kode_lokasi = '"+this.app._lokasi+"'  and not a.kode_klpfa like '101%'", ["a.kode_klpfa","a.nama","e.group_aset"],false, ["Class","Asset Class","Group"],"and","Data Kelompok Aset(Asset Class)",true);
				this.ed_kode.setSQL("select distinct a.no_inv, b.no_gabung, c.nama from amu_kkl_m a "+
					...
					"	inner join amu_asset c on c.no_gabung = b.no_gabung  and c.periode = b.periode "+
					" where a.progress in ('0','6') and a.kode_lokfa = '"+this.ed_lokfa.getText()+"' and a.jenis = 'NTB' and a.periode = '"+this.app._periode+"'",["no_inv", "no_gabung","nama"],false, ["No Inventarisasi","No NKA+SNo.","Deskripsi"],"where","Daftar Inventarisasi Aset Fisk",true);
			}	
			if (sender == this.ed_klpfa){
				...
				//	" where a.periode = '"+this.app._periode+"'  and a.kode_lokasi = '"+this.app._lokasi+"' and a.kode_klpfa = '"+this.ed_klpfa.getText()+"' and a.kode_lokfa = '"+this.ed_lokfa.getText()+"' ", ["a.no_gabung","a.no_fa","a.no_sn","a.nama"],false, ["No Gabung","NKA","SN","Deskripsi"],"and","Data SAP AM",true);
				this.cekStatus = 1;
	...
	doGridChange: function(sender, col, row,param1,result, data){	
		try{
			...
			if (col == 2){
				if (sender.cells(2,row) != "" && sender.cells(2,row).length != 18 && parseFloat(sender.cells(3, row)) < 5){
					system.alert(this,"No Label harus 18 digit","Cek Inputan Label Kembali");
