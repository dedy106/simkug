 ...
 * @author dweexfuad
 */
window.app_assetsap_transaksi_fJarEvidence = function(owner) {
	if (owner){
		try{
			window.app_assetsap_transaksi_fJarEvidence.prototype.parent.constructor.call(this,owner);
			this.maximize();
			this.className  = "app_assetsap_transaksi_fJarEvidence";		
			...
			this.ed_refevd = new saiCBBL(this,{bound:[20,4,300,20],caption:"Ref.Evidence/STO", maxLength:150, 
				multiSelection: false,			
				change:[this,"doChange"]
			});		
...
window.app_assetsap_transaksi_fJarEvidence.extend(window.childForm);
window.app_assetsap_transaksi_fJarEvidence.implement({
	...
	doChange: function(sender){
		if (sender == this.ed_jns){			
			this.ed_refevd.setSQL("select distinct a.kode_loksto, a.nama from amu_loksto a "+
				"	inner join amu_datajar_sample b on b.kode_loksto = a.kode_loksto and b.kode_regional = a.plant "+
				" where b.kode_lokasi = '"+this.app._lokasi+"' ", ["kode_loksto","nama"],false, ["STO","Nama"],"and","Referensi Evidence / STO",false);
		...
		if (sender == this.ed_refevd){		
			try{	
				switch (this.ed_jns.getText().toLowerCase()){
					...
					break;									
					case "jaringan":	
						if (sender.dataFromList[1])
							this.sg.cells(1,0, sender.dataFromList[1]);
						else this.sg.cells(1,0, sender.text);
				...
				switch	(methodName)
				{
					...
					case "getDataProvider":
						...
						for (var i in result.rs.rows) {
							...
							if (line.nama.toLowerCase() == "jaringan")
	    ...
	    try{
			if (sender == this.sg){
				if (sender.dataFromList){
					if (row == 1){
						sender.cells(1,2,sender.dataFromList[1]);				
					}
				}
			}
    ...
    doEllipsClick:function(sender, col ,row){		
			...
			{
				case 1 : 
					this.standarLib.ListDataSGFilter(this, "Daftar Jaringan STO",this.sg, this.sg.row, this.sg.col, 
													  "select no_rk, primer, kap from amu_loksto_d where kode_loksto = '"+this.ed_refevd.getText()+"' ",
													  "select count(*)  from amu_loksto_d where kode_loksto = '"+this.ed_refevd.getText()+"' ",
