 ...
 * @author dweexfuad
 */
window.app_assetsap_transaksi_fAppBAKKIL = function(owner) {
	if (owner){
		window.app_assetsap_transaksi_fAppBAKKIL.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_assetsap_transaksi_fAppBAKKIL";		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Approval BA KKIL ", 0);
...
window.app_assetsap_transaksi_fAppBAKKIL.extend(window.childForm);
window.app_assetsap_transaksi_fAppBAKKIL.implement({
			...
			switch(event){
				...
				case "simpan" :
					...
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){												
						sql.add("insert into amu_appbareg (no_app, tanggal, kode_lokfa, nik_app, periode, no_ba, kode_lokasi, tgl_input, nik_user)values"+
	...
	doChange: function(sender){
		try{
			...
			if (sender == this.ed_lokfa){
				...
				this.ed_ba.setSQL("select no_ba, keterangan from amu_ba_m where kode_lokasi = '"+this.app._lokasi+"' and progress = '0' and  kode_lokfa like '"+sender.getText()+"' ", ["no_ba","keterangan"],false, ["No BA","Keterangan"],"and","Data Berita Acara Regional",true);											
				this.doClick();
			}			
			if (sender == this.ed_ba && sender.getText() != ""){
	...
	doClick: function(sender){
		if (sender == this.bGen)
			this.ed_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'amu_appbareg','no_app',"APPBA"+this.ed_lokfa.getText()+"."+this.dp_tgl.getYear()+".",'0000'));
