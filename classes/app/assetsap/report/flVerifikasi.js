...
//* 			SAI, PT											
//***********************************************************************************************
window.app_assetsap_report_flVerifikasi = function(owner)
{
	if (owner)
	{
		window.app_assetsap_report_flVerifikasi.prototype.parent.constructor.call(this,owner);
		this.className = "app_assetsap_report_flVerifikasi";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Verifikasi Aset Prosedur Alternatif", 2);
		
		this.p1 = new panel(this,{bound:[10,10,720,250], caption:"Filter", border:3});		
		
		uses("saiGrid;reportViewer;server_report_simpleReport;server_report_report;util_filterRep;util_gridLib;util_standar");
		this.sg1 = new saiGrid(this.p1,{bound:[10,11,700,200],colCount:4, colTitle:"Filter, Type, From, To",
			...
			picklist:[[1],[new arrayMap({items:["All","=","Range","Like","<="]})]], defaultRow:4
		});			
...
window.app_assetsap_report_flVerifikasi.extend(window.childForm);
window.app_assetsap_report_flVerifikasi.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["1234","123","123","13","3","3"]);
	...
	mainButtonClick: function(sender){
		try{
		    ...
		    this.filter = this.filterRep.filterStr("b.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
		          this.filterRep.filterStr("b.kode_lokfa",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
			this.filter2 = this.sg1.getCell(2,2)+","+this.sg1.getCell(2,3);			
			this.kd_akun = "";
			this.pageHtml = new arrayMap();						
			this.nama_report = "server_report_amu_rptVerifikasi";			
