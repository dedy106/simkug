window.app_assetsap_report_flPropinsi = function(owner)
{
	if (owner)
	{
		window.app_assetsap_report_flPropinsi.prototype.parent.constructor.call(this,owner);
		this.className = "app_assetsap_report_flPropinsi";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Daftar Jumlah Lokasi per Propinsi", 2);
...
window.app_assetsap_report_flPropinsi.extend(window.childForm);
window.app_assetsap_report_flPropinsi.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2){
				...
				var sql = "select distinct a.kode_prop, a.nama, count(distinct b.kode_lokfa) as tot  "+
						" from amu_propinsi a  "+																	
						"	left outer join amu_lokfa b on b.kode_lokasi = a.kode_lokasi and b.kode_prop = a.kode_prop "+									
						this.filter +" group by a.kode_prop order by a.kode_prop" ;
				this.scriptSqlCount = "select count(distinct a.kode_prop) "+
						" from amu_propinsi a  "+																	
						this.filter;				
				this.title = new server_util_arrayList({items:["Kode Propinsi","Nama Propinsi","Jumlah"]});
			...
			systemAPI.alert("[app_assetsap_report_flPropinsi]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>Daftar Jumlah Lokasi per Propinsi<br>Posisi : "+this.sg1.getCell(2,1)+"<br />";
		var d = new Date();
