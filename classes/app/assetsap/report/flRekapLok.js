window.app_assetsap_report_flRekapLok = function(owner)
{
	if (owner)
	{
		window.app_assetsap_report_flRekapLok.prototype.parent.constructor.call(this,owner);
		this.className = "app_assetsap_report_flRekapLok";
		this.maximize();
		this.app._mainForm.childFormConfig(this,"mainButtonClick","Laporan Rekap Lokasi",2);
		uses("panel");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Tipe","=","UBIS"));
...
window.app_assetsap_report_flRekapLok.extend(window.childForm);
window.app_assetsap_report_flRekapLok.prototype.doEllipseClick= function(sender, col, row)
{
	if (row ==0)
		{
...
window.app_assetsap_report_flRekapLok.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_assetsap_report_flRekapLok.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.sql =  "select a.kode_lokfa,a.nama,b.jum,b.nilai,b.nilai_ap,b.nilai_buku "+
						"from amu_lokasi a "+
						"inner join (select kode_lokfa,count(kode_lokfa) as jum, "+
						"		   sum(nilai) as nilai,sum(nilai_ap) as nilai_ap,sum(nilai_buku) as nilai_buku "+
						"		from amu_asset "+
						"		group by kode_lokfa) b on a.kode_lokfa=b.kode_lokfa   "+this.filter+
						"order by a.kode_lokfa ";
			
			this.scriptSqlCount = "select count(a.kode_lokfa) "+
						"from amu_lokasi a "+
						"inner join (select kode_lokfa,count(kode_lokfa) as jum, "+
						"		   sum(nilai) as nilai,sum(nilai_ap) as nilai_ap,sum(nilai_buku) as nilai_buku "+
						"		from amu_asset "+
			...
			title.add("BA");width.add(50);fieldType.add("S");		
			title.add("Area Bisnis");width.add(200);fieldType.add("S");
	...
	{
		alert("[app_assetsap_report_flRekapLok]::mainButtonClick:"+e);
	}
};
window.app_assetsap_report_flRekapLok.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_assetsap_report_flRekapLok.prototype.getStringHeader = function(){
...
window.app_assetsap_report_flRekapLok.prototype.doConfirmClick = function(sender){
	try{
...
window.app_assetsap_report_flRekapLok.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_assetsap_report_flRekapLok.prototype.previewReport = function(dthtml)
{
...
window.app_assetsap_report_flRekapLok.prototype.doCloseReportClick = function(sender)
{
...
window.app_assetsap_report_flRekapLok.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_assetsap_report_flRekapLok.prototype.doRowPerPageChange = function(sender, rowperpage)
{
