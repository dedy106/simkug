window.app_assetsap_report_flSertifikatKodya = function(owner)
{
	if (owner)
	{
		window.app_assetsap_report_flSertifikatKodya.prototype.parent.constructor.call(this,owner);
		this.className = "app_assetsap_report_flSertifikatKodya";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Daftar Sertifikat Asli per Kodya", 2);
	...
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Jenis","All",""]);	
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["No Sertifikat","All",""]);	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_assetsap_report_flSertifikatKodya.extend(window.childForm);
window.app_assetsap_report_flSertifikatKodya.implement({
		...
		try
		{
			if (sender === this.app._mainForm.bClear2){
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_prop",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+							
							this.filterRep.filterStr("b.kode_kodya",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+							
							this.filterRep.filterStr("d.nama",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+							
							this.filterRep.filterStr("d.no_surat",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();				
				var sql = "select distinct a.nama as nmprop, b.nama as kodya, d.nama as jenis,d.no_surat,c.alamat, c.kel, c.kec, c.tanah, c.bangun,  c.kode_lokfa, e.nama as nmfisik, d.no_reg   "+
						" from amu_propinsi a  "+
						...
						this.filter +" order by a.nama, b.nama, d.nama" ;
				this.scriptSqlCount = "select count(distinct a.nama,d.nama, c.kode_lokfa, c.alamat, c.kel, c.kec, b.nama, c.tanah, c.bangun, d.no_surat, d.tgl_awal, e.nama, d.no_reg) "+
						...
						"	inner join amu_fisik e on e.kode_fisik = d.kode_fisik and e.kode_lokasi = a.kode_lokasi "+
						this.filter;				
				this.title = new server_util_arrayList({items:["Propinsi","Kodya","Jenis","Nomor","Alamat","Kelurahan","Kecamatan","Tanah","Bangunan","Lokasi","Fisik","No. Register"]});
				this.widthTable = new server_util_arrayList({items:[80,150,250,100,100,100,100,100,150,100,150]});
				this.fieldType = new server_util_arrayList({items:["S","S","S","S","S","S","S","S","S","S"]});												
				this.groupBy = new server_util_arrayList({items:["nmprop","kodya","jenis"]});
				this.summary = new server_util_arrayList({items:["N","N","N","N","N","N","N","N","N","N","N"]});
				this.groupHeader = new server_util_arrayList({items:["nmprop","kodya","jenis"]});
			...
			systemAPI.alert("[app_assetsap_report_flSertifikatKodya]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>SERTIFIKAT per JENIS ARSIP <br>Posisi : "+new Date().lclFormat()+"<br />";
		var d = new Date();
