...
//* 			SAI, PT											
//***********************************************************************************************
window.app_assetsap_report_flKKIL2 = function(owner)
{
	if (owner)
	{
		window.app_assetsap_report_flKKIL2.prototype.parent.constructor.call(this,owner);
		this.className = "app_assetsap_report_flKKIL2";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan KKIL Non Tanah Bangunan", 2);
	...
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Divisi","All",""));
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Bisnis Area","=",this.app._kodeLokfa));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Class Asset","All",""));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Status KKIL","All",""));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("No Inventarisasi","All",""));		
...
window.app_assetsap_report_flKKIL2.extend(window.childForm);
window.app_assetsap_report_flKKIL2.implement({
	doEllipseClick: function(sender, col, row){	
		...
		{					
			this.filterRep.ListDataSGFilter(this, "Class Asset",this.sg1, this.sg1.row, this.sg1.col,
													  "select distinct a.kode_klpfa, a.nama from amu_klp a where not kode_klpfa like '101%'",
													 "select count(*) from amu_klp a where not kode_klpfa like '101%' ",
		...
		if (row == 5)
		{
			this.filterRep.ListDataSGFilter(this, "Daftar Inventarisasi",this.sg1, this.sg1.row, this.sg1.col,
							...
							this.filterRep.filterStr("c.kode_lokfa",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"where")+" and a.jenis = 'NTB' "+							
							...
							"select distinct a.no_inv, b.no_gabung, c.nama from amu_kkl_m a "+
													  " inner join amu_kkl_d b on b.no_inv = a.no_inv "+
													  " inner join amu_asset c on c.no_gabung = b.no_gabung "+
							this.filterRep.filterStr("c.kode_lokfa",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where")+" and a.jenis = 'NTB' "+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and")+" ) a " ,							
	...
	doSelectCell: function(sender, col, row){
		if (this.userStatus=="N")
		{
			...
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[0,2,2,2,0,2]);
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["123","123","123","123","123","3"]);
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from amu_asset where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}		
	...
	mainButtonClick: function(sender){
		try{
				...
				var diva = this.sg1.cells(2,2).substr(0,3) == "TDV" || this.sg1.cells(2,2).substr(0,3) == "TCS";
				if (this.sg1.cells(2,4) == "All" || this.sg1.cells(1,4) == "All"){
					var sql = "select distinct c.no_gabung, c.no_fa, c.no_sn, c.kode_lokfa, c.kode_klpfa,c.nama, c.nama2, date_format(c.tgl_perolehan,'%d-%m-%Y') as tgl_perolehan, c.nilai, c.nilai_ap, c.nilai_buku, c.jml_fisik as qty, ifnull(e.bun,'-') as bun, "+
						" b.alamat, b.jml_fisik, b.no_label, b.kode_status, d.nama as nm_status ,b.ket_lokasi, b.keterangan, b.no_inv, f.no_bukti, case a.progress when '0' then 'KKIL' when '1' then 'APP Area' when '2' then 'APP Regional' when '3' then 'APP IC' when '4' then 'Ret. ke Regional' when '5' then 'Ret. ke Area' when '6' then 'Ret Officer' end as status_app, case when a.lampiran is null or a.lampiran = '-' then '-' else concat('<a href=''server/media/amu/',a.lampiran,'''','>',a.lampiran,'</a>') end as lampiran "+
						" from amu_asset c "+
						...
						"	inner join amu_bagiklp_d h on h.kode_klpfa = c.kode_klpfa and h.periode = c.periode and h.jenis_proc = 'FISIK' " +
						"   left outer join amu_kkl_d b on c.no_gabung = b.no_gabung and c.periode = b.periode "+
						"	left outer join amu_kkl_m  a on a.no_inv = b.no_inv and a.jenis = 'NTB' "+						
					this.filterRep.filterStr("a.no_inv",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and") +
					...
					this.filterRep.filterStr(diva ? "c.ref1" : "c.kode_lokfa",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"where")+
					this.filterRep.filterStr(diva ? "c.kode_lokfa" : "g.kode_induk",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
					this.filterRep.filterStr("c.kode_klpfa",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and") +
					this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and")+"  "+							
						" and not c.kode_klpfa like '101%' "+
					(this.sg1.cells(1,1).toLowerCase() == "all" && this.sg1.cells(1,2).toLowerCase() != "all" ? "":  
						" union "+
					...
					this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and")+"  "+							
						" and not c.kode_klpfa like '101%' ")+
						" order by no_gabung";									
				}else if (this.sg1.cells(2,4) == "Sudah"){
					var sql = "select distinct c.no_gabung, c.no_fa, c.no_sn, c.kode_lokfa, c.kode_klpfa,c.nama, c.nama2, date_format(c.tgl_perolehan,'%d-%m-%Y') as tgl_perolehan, c.nilai, c.nilai_ap, c.nilai_buku, c.jml_fisik as qty, ifnull(e.bun,'-') as bun, "+
						" b.alamat, b.jml_fisik, b.no_label, b.kode_status, d.nama as nm_status ,b.ket_lokasi, b.keterangan, b.no_inv, f.no_bukti, case a.progress when '0' then 'KKIL' when '1' then 'APP Area' when '2' then 'APP Regional' when '3' then 'APP IC' when '4' then 'Ret. ke Regional' when '5' then 'Ret. ke Area' when '6' then 'Ret Officer' end as status_app, case when a.lampiran is null or a.lampiran = '-' then '-' else concat('<a href=''server/media/amu/',a.lampiran,'''','>',a.lampiran,'</a>') end as lampiran "+
						" from amu_asset c "+
						" inner join amu_distaset_d f on f.no_gabung = c.no_gabung and f.periode = c.periode "+
						...
						"	inner join amu_kkl_m  a on a.no_inv = b.no_inv and a.jenis = 'NTB' "+
					this.filterRep.filterStr("a.no_inv",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and") +
					...
					this.filterRep.filterStr("c.kode_klpfa",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and") +
					this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and")+"  "+							
						" and not c.kode_klpfa like '101%' and b.no_gabung is null "+
					(this.sg1.cells(1,1).toLowerCase() == "all" && this.sg1.cells(1,2).toLowerCase() != "all" ? "":  
						" union "+
						" select distinct c.no_gabung, c.no_fa, c.no_sn, c.kode_lokfa, c.kode_klpfa, c.nama, c.nama2, date_format(c.tgl_perolehan,'%d-%m-%Y') as tgl_perolehan, c.nilai, c.nilai_ap, c.nilai_buku, c.jml_fisik as qty, ifnull(e.bun,'-') as bun, "+
						" b.alamat, b.jml_fisik, b.no_label, b.kode_status, d.nama as nm_status ,b.ket_lokasi, b.keterangan, b.no_inv, f.no_bukti, case a.progress when '0' then 'KKIL' when '1' then 'APP Area' when '2' then 'APP Regional' when '3' then 'APP IC' when '4' then 'Ret. ke Regional' when '5' then 'Ret. ke Area' when '6' then 'Ret Officer' end as status_app, case when a.lampiran is null or a.lampiran = '-' then '-' else concat('<a href=''server/media/amu/',a.lampiran,'''','>',a.lampiran,'</a>') end as lampiran "+
				...
				this.scriptSqlCount = "select count(*) as tot "+					
					" from ("+ sql +") a ";					
				this.title = new server_util_arrayList({items:["No Gabung","No Aset","SNo.","Bus.Area","Class","Asset Deskripsi","Deskripsi Alamat","Cap. Date","Acquis.Val","Accum.Val","Book.Val","Quantity SAP","BUN","Alamat","Jml Fisik","No Label","Status","Ket. Status","Update Deskripsi & Lokasi","Keterangan","No Inventarisasi","No KKIL","Progres","Lampiran"]});
				this.widthTable = new server_util_arrayList({items:[100,100,30,80,80,250,250,80,100,100,100,80,50,150,80,120,50,100,120,150,250,150,150,150]});
				this.fieldType = new server_util_arrayList({items:["S","S","S","S","S","S","S","S","N","N","N","N","S","S","N","S","S","S","S","S","S","S","S","S"]});																	
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'>"+
			...
			"			<td align='center'><span style='font-size:16;font-weight:bold;'>Daftar NKA Hasil Inventarisasi Lapangan</span><br><span style='font-size:14;font-weight:bold;'>Non Tanah Bangunan</span></td>"+
			"</tr>"+
	...
	doCloseReportClick: function(sender){
	  ...
	  switch(sender.getName())
	  {
	    ...
	    case "xlsBtn" :	
			...
			html.add(new Date().valueOf()+"_angsKol");				
			this.viewer.useIframe(upDownHtml(html));*/			
			var file = this.dbLib.sqlToXls2(this.sqlScript,this.title, "KKILNTB.xls");
			downloadFile(file);
			break;
		case "PreviewBtn" :        
			var win = window.open("");
