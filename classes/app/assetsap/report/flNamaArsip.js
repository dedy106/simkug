window.app_assetsap_report_flNamaArsip = function(owner)
{
	if (owner)
	{
		window.app_assetsap_report_flNamaArsip.prototype.parent.constructor.call(this,owner);
		this.className = "app_assetsap_report_flNamaArsip";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Daftar Nama Arsip", 2);
...
window.app_assetsap_report_flNamaArsip.extend(window.childForm);
window.app_assetsap_report_flNamaArsip.implement({
		...
		try
		{
			if (sender === this.app._mainForm.bClear2){
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_prop",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("d.kode_lokfa",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
				...
				var sql = "select distinct c.kode_lokfa, c.alamat, c.kel, c.kec, b.nama as kodya, a.nama as prop, c.tanah, c.bangun, d.nama, d.no_surat, date_format(d.tgl_awal,'%d-%m-%Y') as tgl_awal, e.nama as nmfisik, d.no_reg   "+
						" from amu_propinsi a  "+
						...
						this.filter +" order by c.kode_lokfa" ;
				this.scriptSqlCount = "select count(distinct c.kode_lokfa, c.alamat, c.kel, c.kec, b.nama, a.nama, c.tanah, c.bangun, d.nama, d.no_surat, d.tgl_awal,e.nama, d.no_reg) "+
						" from amu_propinsi a  "+
						"	inner join amu_kodya b on b.kode_prop = a.kode_prop and b.kode_lokasi = a.kode_lokasi "+
						"	inner join amu_lokfa c on c.kode_kodya = b.kode_kodya and c.kode_prop = b.kode_prop and c.kode_lokasi = b.kode_lokasi "+
						"	inner join amu_arsip d on d.kode_lokfa = c.kode_lokfa and d.kode_lokasi = a.kode_lokasi "+
						"	inner join amu_fisik e on e.kode_fisik = d.kode_fisik and e.kode_lokasi = a.kode_lokasi "+
						this.filter;				
				this.title = new server_util_arrayList({items:["Lokasi","Alamat","Kelurahan","Kecamatan","Kodya/Kabupaten","Propinsi","Luas Tanah","Luas Bangunan","Nama","Nomor","Tanggal","Fisik","No. Reg"]});
			...
			systemAPI.alert("[app_assetsap_report_flNamaArsip]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>DAFTAR NAMA ARSIP per LOKASI<br>Posisi : "+this.sg1.getCell(2,1)+"<br />";
		var d = new Date();
