...
//* 			SAI, PT											
//***********************************************************************************************
window.app_assetsap_report_flKKP = function(owner)
{
	if (owner)
	{
		window.app_assetsap_report_flKKP.prototype.parent.constructor.call(this,owner);
		this.className = "app_assetsap_report_flKKP";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Format KKP", 2);
...
window.app_assetsap_report_flKKP.extend(window.childForm);
window.app_assetsap_report_flKKP.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 2)
		{
			this.filterRep.ListDataSGFilter(this, "Daftar Rekon",this.sg1, this.sg1.row, this.sg1.col,
							...
							this.filterRep.filterStr("c.kode_lokfa",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.jenis",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and") +") a ",
													  ["no_rekon","no_gabung","nama"],"where",["No Rekonsiliasi","No Gabung(No FA + SN)","Deskripsi"]);
		}			
		if (row == 3)
		{
			this.filterRep.ListDataSGFilter(this, "Daftar Aset",this.sg1, this.sg1.row, this.sg1.col,
													  " select * from (select distinct b.no_gabung, c.nama from amu_rekon_m a "+
													  " inner join amu_rekon_d b on b.no_rekon = a.no_rekon "+
													  " inner join amu_asset c on c.no_gabung = b.no_gabung "+
							this.filterRep.filterStr("c.kode_lokfa",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.no_rekon",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.jenis",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and") +") a ",
													  " select count(*) from (select distinct b.no_gabung, c.nama from amu_rekon_m a "+
													  " inner join amu_rekon_d b on b.no_rekon = a.no_rekon "+
													  " inner join amu_asset c on c.no_gabung = b.no_gabung "+
							this.filterRep.filterStr("c.kode_lokfa",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.no_rekon",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
	...
	mainButtonClick: function(sender){
		try{
		    ...
		    this.filter =  this.filterRep.filterStr("c.kode_lokfa",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
				  this.filterRep.filterStr("rr.no_rekon",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
				  this.filterRep.filterStr("b.no_gabung",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
				  this.filterRep.filterStr("rr.jenis",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");				 				
								
			this.filter2 = "";			
			this.pageHtml = new arrayMap();						
			this.page = 1;
			this.nama_report = "server_report_amu_rptKKP";
