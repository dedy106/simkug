window.app_assetsap_report_flUpload = function(owner)
{
	if (owner)
	{
		window.app_assetsap_report_flUpload.prototype.parent.constructor.call(this,owner);
		this.className = "app_assetsap_report_flUpload";
		this.maximize();
...
window.app_assetsap_report_flUpload.extend(window.childForm);
window.app_assetsap_report_flUpload.implement({
		...
		try
		{
			if (sender === this.app._mainForm.bClear2){				
		    	...
		    	this.filter = this.filterRep.filterStr("b.kode_klpakun",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("nvl(b.ref1,'-')",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and")+
					...
					switch (this.sg1.cells(2,5).toLowerCase()){
						case "asset class":
							var sql = "select b.kode_klpfa,d.nama, count(b.no_gabung) as totalaset, sum(b.nilai) as nilai, sum(b.nilai_ap) as nilaiap, sum(b.nilai_buku) as nilai_buku "+
								...
								" "+(status == " "?"left outer":"inner")+" join (select distinct kode_klpfa, jenis_proc from amu_bagiklp_d "+ this.filterRep.filterStr("periode",this.sg1.getCell(1,7),this.sg1.getCell(2,7),this.sg1.getCell(3,7),"where") +") c on c.kode_klpfa = b.kode_klpfa   "+status+" "+								
						...
						case "class per apc":
							var sql = "select b.kode_klpakun,b.kode_klpfa,d.nama, count(b.no_gabung) as totalaset, sum(b.nilai) as nilai, sum(b.nilai_ap) as nilaiap, sum(b.nilai_buku) as nilai_buku "+
								" from amu_asset b "+
								" inner join (select distinct kode_klpfa, jenis_proc from amu_bagiklp_d "+ this.filterRep.filterStr("periode",this.sg1.getCell(1,7),this.sg1.getCell(2,7),this.sg1.getCell(3,7),"where") +" ) c on c.kode_klpfa = b.kode_klpfa   "+status+" "+
								" inner join amu_klp d on d.kode_klpfa = b.kode_klpfa "+
						...
						case "lokasi veat":
							...
							var sql = "select ubis, nama, jenis_proc,sum(totalaset), sum(nilai)as nilai, sum(nilaiap) as nilaiap, sum(nilai_buku) as nilaibuku from "+
								...
								" inner join amu_lokasi f on f.kode_lokfa = e.kode_induk "+
								" inner join amu_klp_alt k on k.kode_klpfa = b.kode_klpfa and k.periode = b.periode "+
								" inner join amu_bagiklp_d c on c.kode_klpfa = b.kode_klpfa and c.periode = b.periode  "+status+" "+
									this.filter +" group by f.kode_lokfa, k.jenis_proc "+
								" union all "+ 
								"select e.kode_lokfa, k.jenis_proc, count(b.no_gabung) as totalaset, sum(b.nilai) as nilai, sum(b.nilai_ap) as nilaiap, sum(b.nilai_buku) as nilai_buku "+
								" from amu_asset b "+								
			...
			systemAPI.alert("[app_assetsap_report_flUpload]::mainButtonClick:"+e);
		}
	},
