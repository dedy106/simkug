...
//* 			SAI, PT											
//***********************************************************************************************
window.app_assetsap_report_flRekapKKL = function(owner)
{
	if (owner)
	{
		window.app_assetsap_report_flRekapKKL.prototype.parent.constructor.call(this,owner);
		this.className = "app_assetsap_report_flRekapKKL";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Rekapitulasi Inventarisasi Fisik", 2);
		
		this.p1 = new panel(this,{bound:[10,10,720,250], caption:"Filter", border:3});		
		
		uses("saiGrid;reportViewer;server_report_simpleReport;server_report_report;util_filterRep;util_gridLib;util_standar;pageControl");
		this.sg1 = new saiGrid(this.p1,{bound:[10,11,700,200],colCount:4, colTitle:"Filter, Type, From, To",
			...
			picklist:[[1],[new arrayMap({items:["All","=","Range","Like","<="]})]], defaultRow:5
		});			
		...
		//this.pageControl = new pageControl(this,{bound:[20,280,600,300],childPage:["Test1","test2","test4"]});
	}
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Periode Inventarisasi","=",this.app._periode));	
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Model Report","=","DIVISI"));	
...
window.app_assetsap_report_flRekapKKL.extend(window.childForm);
window.app_assetsap_report_flRekapKKL.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["1234","123","3","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,2,0,0,0]);
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["1234","123","3","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[3,2,0,0,0]);
		}				
		if (row == 2)
		{
		...
		if (row == 3)
		{
			this.sg1.columns.get(2).setPicklist(new portalui_arrayMap({items:["DIVISI","REGIONAL","AREA"]}));			
		}		
	...
	mainButtonClick: function(sender){
		try{
		    ...
		    this.filter = this.filterRep.filterStr("c.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
		          this.filterRep.filterStr("c.kode_lokfa",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
			this.filter2 = this.sg1.getCell(2,2)+","+this.sg1.getCell(2,3)+","+this.sg1.getCell(1,1);
			this.kd_akun = "";
			this.pageHtml = new arrayMap();						
			this.nama_report = "server_report_amu_rptRekapKKL";
