...
//* 			SAI, PT											
//***********************************************************************************************
window.app_assetsap_report_flRekapAlt = function(owner)
{
	if (owner)
	{
		window.app_assetsap_report_flRekapAlt.prototype.parent.constructor.call(this,owner);
		this.className = "app_assetsap_report_flRekapAlt";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Rekapitulasi Prosedur Alternatif", 2);
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Model Report","=","DIVISI"));	
...
window.app_assetsap_report_flRekapAlt.extend(window.childForm);
window.app_assetsap_report_flRekapAlt.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["1234","123","123","3","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,2,0,0,0,0]);
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["1234","123","123","3","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[3,2,0,0,0,0]);
		}		
		if (row == 2)
		{
	...
	mainButtonClick: function(sender){
		try{
			...
			this.filter2 = this.sg1.getCell(2,2)+","+this.sg1.getCell(2,3)+","+this.sg1.getCell(2,4)+","+this.sg1.getCell(1,1);
			this.kd_akun = "";
			this.pageHtml = new arrayMap();						
			this.nama_report = "server_report_amu_rptRekapAlt";
