window.app_assetsap_report_flAsset = function(owner)
{
	if (owner)
	{
		window.app_assetsap_report_flAsset.prototype.parent.constructor.call(this,owner);
		this.className = "app_assetsap_report_flAsset";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Daftar Aset SAP-AM", 2);
	...
	this.gridLib.SGEditData(this.sg1,4,[0,1,2],["Group per","=","Asset Class"]);
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_assetsap_report_flAsset.extend(window.childForm);
window.app_assetsap_report_flAsset.implement({
	...
	doSelectCell: function(sender, col, row){		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","123","123","3","3"]);
		...
		if (row == 4) {
			this.sg1.columns.get(2).setPicklist(new portalui_arrayMap({items:["Asset Class","BA","BA Class Asset","Divisi","Divisi s/d Area","Divisi Class Asset","Prosedur/Divisi","Lokasi VEAT"]}));			
		}
	},
		...
		try
		{
			if (sender === this.app._mainForm.bClear2){				
				...
				if (this.sg1.cells(2,3) == "Rekap"){
					switch (this.sg1.cells(2,4).toLowerCase()){
						case "asset class":
							...
							this.scriptSqlCount = "select count(*) "+
									" from (select b.kode_klpfa, count(b.no_gabung) as totalaset, sum(b.nilai) as nilai, sum(b.nilai_ap) as nilaiap, sum(b.nilai_buku) as nilai_buku "+
								" from amu_asset b"+								
								" inner join amu_klp d on d.kode_klpfa = b.kode_klpfa "+
									this.filter +" group by b.kode_klpakun, b.kode_klpfa )";				
						...
						case "ba":
							...
							this.scriptSqlCount = "select count(*) "+
									" from (select b.kode_lokfa,count(b.no_gabung) as totalaset, sum(b.nilai) as nilai, sum(b.nilai_ap) as nilaiap, sum(b.nilai_buku) as nilai_buku "+
								" from amu_asset b"+								
									this.filter +" group by b.kode_lokfa )";
						...
						case "ba class asset":
							...
							this.scriptSqlCount = "select count(*) "+
									" from (select b.kode_lokfa, b.kode_klpfa, count(b.no_gabung) as totalaset, sum(b.nilai) as nilai, sum(b.nilai_ap) as nilaiap, sum(b.nilai_buku) as nilai_buku "+
								" from amu_asset b"+							
									this.filter +" group by b.kode_lokfa, b.kode_klpfa )";
						...
						case "divisi":						
							...
							this.scriptSqlCount = "select count(*) "+
									" from (select f.kode_lokfa as ubis, count(b.no_gabung) as totalaset, sum(b.nilai) as nilai, sum(b.nilai_ap) as nilaiap, sum(b.nilai_buku) as nilai_buku "+
								" from amu_asset b "+								
								" inner join amu_lokasi d on d.kode_lokfa = b.kode_lokfa "+
								" inner join amu_lokasi e on e.kode_lokfa = d.kode_induk "+
								" inner join amu_lokasi f on f.kode_lokfa = e.kode_induk "+								
								...
								" from amu_asset b "+								
								" inner join amu_lokasi d on d.kode_lokfa = b.kode_lokfa "+																
									this.filter +" group by b.kode_lokfa ) a ";				
						...
						case "divisi class asset":							
							...
							this.scriptSqlCount = "select count(*) "+
									" from (select f.kode_lokfa as ubis, b.kode_klpfa, count(b.no_gabung) as totalaset, sum(b.nilai) as nilai, sum(b.nilai_ap) as nilaiap, sum(b.nilai_buku) as nilai_buku "+
								" from amu_asset b "+								
								" inner join amu_lokasi d on d.kode_lokfa = b.kode_lokfa "+
								" inner join amu_lokasi e on e.kode_lokfa = d.kode_induk "+
								" inner join amu_lokasi f on f.kode_lokfa = e.kode_induk "+								
								...
								" from amu_asset b "+								
								" inner join amu_lokasi d on d.kode_lokfa = b.kode_lokfa "+																
									this.filter +" group by b.kode_lokfa, b.kode_klpfa ) a ";											
						...
						case "prosedur/divisi":							
							...
							this.scriptSqlCount = "select count(*) "+
									" from (select f.kode_lokfa as ubis, k.jenis_proc, count(b.no_gabung) as totalaset, sum(b.nilai) as nilai, sum(b.nilai_ap) as nilaiap, sum(b.nilai_buku) as nilai_buku "+
								" from amu_asset b "+								
								" inner join amu_lokasi d on d.kode_lokfa = b.kode_lokfa "+
								" inner join amu_lokasi e on e.kode_lokfa = d.kode_induk "+
								" inner join amu_lokasi f on f.kode_lokfa = e.kode_induk "+
								...
								" inner join amu_lokasi d on d.kode_lokfa = b.kode_lokfa "+								
								" inner join amu_klp_alt k on k.kode_klpfa = b.kode_klpfa and k.periode = b.periode "+								
									this.filter +" group by b.kode_lokfa, k.jenis_proc ) a ";							
						...
						case "lokasi veat":
							...
							var sql = "select ubis, nama, jenis_proc,sum(totalaset), sum(nilai)as nilai, sum(nilaiap) as nilaiap, sum(nilai_buku) as nilaibuku from "+
								...
								" inner join amu_lokasi f on f.kode_lokfa = e.kode_induk "+
								" inner join amu_klp_alt k on k.kode_klpfa = b.kode_klpfa and k.periode = b.periode "+
								" inner join amu_bagiklp_d c on c.kode_klpfa = b.kode_klpfa   "+status+" "+
									this.filter +" group by f.kode_lokfa, k.jenis_proc "+
								" union all "+ 
								"select e.kode_lokfa, k.jenis_proc, count(b.no_gabung) as totalaset, sum(b.nilai) as nilai, sum(b.nilai_ap) as nilaiap, sum(b.nilai_buku) as nilai_buku "+
								" from amu_asset b "+								
						...
						case "divisi s/d area":
							...
							this.scriptSqlCount = "select count(*) "+
									" from (select f.kode_lokfa as ubis, e.kode_lokfa as sbis, b.kode_lokfa, b.kode_klpfa, count(b.no_gabung) as totalaset, sum(b.nilai) as nilai, sum(b.nilai_ap) as nilaiap, sum(b.nilai_buku) as nilai_buku "+
								" from amu_asset b "+								
								" inner join amu_lokasi d on d.kode_lokfa = b.kode_lokfa "+
								" inner join amu_lokasi e on e.kode_lokfa = d.kode_induk "+
								" inner join amu_lokasi f on f.kode_lokfa = e.kode_induk "+								
								...
								" from amu_asset b "+								
								" inner join amu_lokasi d on d.kode_lokfa = b.kode_lokfa "+								
								" inner join (select distinct kode_klpfa, jenis_proc from amu_bagiklp_d ) c on c.kode_klpfa = b.kode_klpfa   "+status+" "+
									this.filter +" group by b.kode_lokfa, b.kode_klpfa) a ";				
					...
					this.scriptSqlCount = "select count(*) "+
							" from (select b.no_fa, b.no_sn, b.nama, b.nama2, b.kode_klpakun, b.kode_lokfa, b.ref1, b.ref2, b.tgl_perolehan, b.nilai, b.nilai_ap, b.nilai_buku "+
						" from amu_asset b "+					
							this.filter +" )";
			...
			systemAPI.alert("[app_assetsap_report_flAsset]::mainButtonClick:"+e);
		}
	},
	...
	doSelectedPage: function(sender, page){
		this.finish = this.pager;
		var dthtml = this.dbLib.sqlToHtml(this.sqlScript,1,this.pager, this.title, this.widthTable, this.fieldType,true, this.groupBy, this.summary, this.groupHeader);
		this.previewReport(dthtml);			
		this.page=page;
	},
	...
	doCloseReportClick: function(sender){
	  ...
	  switch(sender.getName())
	  {
	    ...
	    case "xlsBtn" :	
			...
			html.add(new Date().valueOf()+"_angsKol");				
			this.viewer.useIframe(upDownHtml(html));*/
			window.location = this.dbLib.sqlToXls(this.sqlScript,1,this.finish, this.title, this.widthTable, this.fieldType,true, this.groupBy, this.summary, this.groupHeader, undefined, undefined, undefined, undefined, "DataRekapAset.xls");
			break;
		case "PreviewBtn" :        
			var win = window.open("");
