...
//* 			SAI, PT											
//***********************************************************************************************
window.app_assetsap_report_flBAInv = function(owner)
{	
	try{
		if (owner)
		{
			window.app_assetsap_report_flBAInv.prototype.parent.constructor.call(this,owner);
			this.className = "app_assetsap_report_flBAInv";
			this.maximize();
			
			this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Berita Acara Inventarisasi", 2);		
		...
		this.tanda="=";	
		this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Bus. Area","=",this.app._kodeLokfa]);
		this.gridLib.SGEditData(this.sg1,1,[0,1,2],["No Berita Acara","All",""]);	
		this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Jenis BA","=","NTB"]);	
		this.pager = 1;		
	}catch(e){
		alert(e);
	}
};
window.app_assetsap_report_flBAInv.extend(window.childForm);
window.app_assetsap_report_flBAInv.implement({
	doEllipseClick: function(sender, col, row){		
		...
		if (row == 1)
		{
			this.filterRep.ListDataSGFilter(this, "Data BA",this.sg1, this.sg1.row, this.sg1.col,
													  "select no_ba, keterangan from amu_ba_m "+													   
													   this.filterRep.filterStr("kode_lokfa",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+" and jenis_ba = 'KKIL' order by no_ba",
													  "select count(no_ba) from amu_ba_m "+													  
													  this.filterRep.filterStr("kode_lokfa",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ") + "and jenis_ba = 'KKIL' ",
													  ["no_ba","keterangan"],"and",["No Berita Acara","Keterangan"]);
		}	
	},
	doSelectCell: function(sender, col, row){
		if (this.userStatus=="A")
		{
			...
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,2,0]);
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","123","123"]);
		...
		if (row == 2)
		{
			...
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["TB","NTB"]);
		}	
	...
	doCloseReportClick: function(sender){		
	  switch(sender.getName())
	  {
	    case "allBtn" :
			...
			break;
		case "xlsBtn" :				
			downloadFile(this.report.createExcel(this.nama_report,this.filter, this.page, this.viewer.getTotalPage() * this.pager, this.showFilter, this.lokasi,this.filter2));			
			break;			    
	    case "pdfBtn" :
