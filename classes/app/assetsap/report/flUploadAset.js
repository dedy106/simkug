window.app_assetsap_report_flUploadAset = function(owner)
{
	if (owner)
	{
		window.app_assetsap_report_flUploadAset.prototype.parent.constructor.call(this,owner);
		this.className = "app_assetsap_report_flUploadAset";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Jenis Report","=","Rekap"]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_assetsap_report_flUploadAset.extend(window.childForm);
window.app_assetsap_report_flUploadAset.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2){				
				...
				var sql = "select b.kode_klpakun, count(b.no_gabung) as totalaset, sum(b.nilai) as nilai, sum(b.nilai_ap) as nilaiap, sum(b.nilai_buku) as nilai_buku "+
					" from amu_load_m a inner join amu_load_d b on b.no_load = a.no_load and b.kode_lokasi = a.kode_lokasi "+
						this.filter +" group by b.kode_klpakun order by b.kode_klpakun" ;
				this.scriptSqlCount = "select count(*) "+
						" from (select b.kode_klpakun, count(b.no_gabung) as totalaset, sum(b.nilai) as nilai, sum(b.nilai_ap) as nilaiap, sum(b.nilai_buku) as nilai_buku "+
					" from amu_load_m a inner join amu_load_d b on b.no_load = a.no_load and b.kode_lokasi = a.kode_lokasi "+
						this.filter +" group by b.kode_klpakun )";				
				this.title = new server_util_arrayList({items:["APC","Count of Asset","Acquis. val","Accum.dep.","Book.val"]});
			...
			systemAPI.alert("[app_assetsap_report_flUploadAset]::mainButtonClick:"+e);
		}
	},
