window.app_assetsap_report_flBundel = function(owner)
{
	if (owner)
	{
		window.app_assetsap_report_flBundel.prototype.parent.constructor.call(this,owner);
		this.className = "app_assetsap_report_flBundel";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Daftar Bundel is Sertifikat", 2);
	...
	this.gridLib.SGEditData(this.sg1,1,[0,1,2],["Arsip","All",""]);		
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_assetsap_report_flBundel.extend(window.childForm);
window.app_assetsap_report_flBundel.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row === 1)
			{
				this.filterRep.ListDataSGFilter(this, "Data Arsip",this.sg1, this.sg1.row, this.sg1.col,
											  "select distinct a.no_arsip, a.nama from amu_arsip a "+
							...
							this.filterRep.filterStr("kode_prop",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and"),
											  "select count(*) from amu_arsip a "+
											  "	inner join amu_lokfa b on b.kode_lokfa = a.kode_lokfa and b.kode_lokasi = a.kode_lokasi "+
											  " where a.kode_lokasi = '"+this.app._lokasi+"' "+
							this.filterRep.filterStr("kode_prop",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and"),
											  ["no_arsip","nama"],"and",["No Arsip","Nama"]);
			}		
		}catch(e){
			systemAPI.alert(e);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2){				
		    	...
		    	this.filter = this.filterRep.filterStr("b.kode_prop",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.no_arsip",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");							
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();				
				var sql = "select distinct a.no_surat, c.nama as nmkodya, b.alamat, b.tanah "+
						" from amu_arsip a  "+																	
						"	inner join amu_lokfa b on b.kode_lokasi = a.kode_lokasi and b.kode_lokfa = a.kode_lokfa"+
						"	inner join amu_kodya c on c.kode_lokasi = a.kode_lokasi and c.kode_kodya = b.kode_kodya and b.kode_prop = c.kode_prop "+					
						this.filter +" order by b.kode_prop" ;
				this.scriptSqlCount = "select count(distinct c.kode_kodya) "+
						" from amu_arsip a  "+																	
						"	inner join amu_lokfa b on b.kode_lokasi = a.kode_lokasi and b.kode_lokfa = a.kode_lokfa"+
						"	inner join amu_kodya c on c.kode_lokasi = a.kode_lokasi and c.kode_kodya = b.kode_kodya and b.kode_prop = c.kode_prop "+					
						this.filter;				
				this.title = new server_util_arrayList({items:["Nomor","Kodya","Alamat","Tanah"]});
				this.widthTable = new server_util_arrayList({items:[80,250,150,120]});
				this.fieldType = new server_util_arrayList({items:["S","S","S","N"]});												
				this.groupBy = undefined;
				this.summary = new server_util_arrayList({items:["S","S","S","S"]});	
			...
			systemAPI.alert("[app_assetsap_report_flBundel]::mainButtonClick:"+e);
		}
	},
