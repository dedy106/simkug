window.app_patra_report_flPiutangKartu = function(owner)
{
	if (owner)
	{
		window.app_patra_report_flPiutangKartu.prototype.parent.constructor.call(this,owner);
		this.className = "app_patra_report_flPiutangKartu";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from patra_ar_m ");
...
window.app_patra_report_flPiutangKartu.extend(window.childForm);
window.app_patra_report_flPiutangKartu.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from patra_ar_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_patra_rptPiutangKartu";
			...
			systemAPI.alert("[app_patra_report_flPiutangKartu]::mainButtonClick:"+e);
		}
	},
