window.app_patra_report_flHutangAging = function(owner)
{
	if (owner)
	{
		window.app_patra_report_flHutangAging.prototype.parent.constructor.call(this,owner);
		this.className = "app_patra_report_flHutangAging";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from patra_hutang_m ");
...
window.app_patra_report_flHutangAging.extend(window.childForm);
window.app_patra_report_flHutangAging.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_hutang, a.keterangan from patra_hutang_m a "+filter,
												"select count(b.no_hutang) from patra_hutang_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from patra_hutang_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_patra_rptHutangAging";
			...
			systemAPI.alert("[app_patra_report_flHutangAging]::mainButtonClick:"+e);
		}
	},
