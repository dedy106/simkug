window.app_patra_report_flHutangKas = function(owner)
{
	if (owner)
	{
		window.app_patra_report_flHutangKas.prototype.parent.constructor.call(this,owner);
		this.className = "app_patra_report_flHutangKas";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBHUTANG'");
...
window.app_patra_report_flHutangKas.extend(window.childForm);
window.app_patra_report_flHutangKas.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBHUTANG' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							...
							" and a.modul='KBHUTANG' ";
				this.filter2=this.app._namaUser;
				this.nama_report="server_report_patra_rptKbJurnal";
