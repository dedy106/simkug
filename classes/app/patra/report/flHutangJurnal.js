window.app_patra_report_flHutangJurnal = function(owner)
{
	if (owner)
	{
		window.app_patra_report_flHutangJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_patra_report_flHutangJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from patra_hutang_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_patra_report_flHutangJurnal.extend(window.childForm);
window.app_patra_report_flHutangJurnal.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_hutang, keterangan from patra_hutang_m  "+filter ,
														  "select count(no_hutang) from patra_hutang_m "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from patra_hutang_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
