window.app_patra_report_flPiutangKas = function(owner)
{
	if (owner)
	{
		window.app_patra_report_flPiutangKas.prototype.parent.constructor.call(this,owner);
		this.className = "app_patra_report_flPiutangKas";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBAR'");
...
window.app_patra_report_flPiutangKas.extend(window.childForm);
window.app_patra_report_flPiutangKas.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBAR' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_patra_rptPiutangKbJurnal";
