window.app_patra_report_flPiutangJurnal = function(owner)
{
	if (owner)
	{
		window.app_patra_report_flPiutangJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_patra_report_flPiutangJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from patra_ar_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_patra_report_flPiutangJurnal.extend(window.childForm);
window.app_patra_report_flPiutangJurnal.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_ar, keterangan from patra_ar_m  "+filter ,
														  "select count(no_ar) from patra_ar_m "+filter,
														  new Array("no_ar","keterangan"),"and");
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from patra_ar_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_patra_rptPiutangJurnal";
