window.app_patra_report_flPiutangSaldo = function(owner)
{
	if (owner)
	{
		window.app_patra_report_flPiutangSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_patra_report_flPiutangSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select periode from (select max(periode) as periode from patra_ar_m "+
											"union "+
											"select max(periode) as periode from patra_kaspiutang_d )a ");
...
window.app_patra_report_flPiutangSaldo.extend(window.childForm);
window.app_patra_report_flPiutangSaldo.implement({
		...
		try{			
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Customer",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_cust, nama from patra_cust "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(kode_cust) from patra_cust "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_ar, a.keterangan from patra_ar_m a "+filter,
												"select count(b.no_ar) from patra_ar_m a "+filter,
												new Array("b.no_ar","b.keterangan"),"and",new Array("No Invoice","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select a.periode "+
									"from (select distinct periode from patra_ar_m "+
									"union "+
									"select distinct periode from patra_kaspiutang_d)a "+
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_patra_rptPiutangSaldo";
			...
			systemAPI.alert("[app_patra_report_flPiutangSaldo]::mainButtonClick:"+e);
		}
	},
	...
	doOpenJurnal: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_ar='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._namaUser;
		nama_report="server_report_patra_rptPiutangJurnal";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenKas: function(no_bukti,kode_lokasi){
		...
		filter2=this.app._namaUser;
		nama_report="server_report_patra_rptPiutangKbJurnal";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
