window.app_patra_hutang_fHutUmumE = function(owner)
{
	if (owner)
	{
		window.app_patra_hutang_fHutUmumE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_patra_hutang_fHutUmumE";
...
window.app_patra_hutang_fHutUmumE.extend(window.childForm);
window.app_patra_hutang_fHutUmumE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from patra_hutang_m where no_hutang = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from patra_hutang_j where no_hutang = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from angg_r where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='HUTUM'");
					
					sql.add("insert into patra_hutang_m(no_hutang,kode_lokasi,no_dokumen,tanggal,keterangan,kode_vendor,kode_curr,kurs,nik_buat,nik_app,kode_pp,nilai_curr,nilai,periode,nik_user,tgl_input,akun_hutang,posted,nilai_ppn) values  "+
					...
					sql.add("insert into patra_hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
	...
	doChange:function(sender){		
		if (sender == this.e_periode && this.e_periode.getText()!="") {	
			...
			this.e_nb.setSQL("select a.no_hutang, a.keterangan from patra_hutang_m a "+
			                 "       left join (select no_hutang,kode_lokasi,sum(nilai_kas+nilai_lain) as bayar from patra_hutangbayar_d group by no_hutang,kode_lokasi) b on a.no_hutang=b.no_hutang and a.kode_lokasi=b.kode_lokasi "+
			                 "where a.posted = 'F' and isnull(b.bayar,0) = 0 and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",["a.no_hutang","a.keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);					
		}	
