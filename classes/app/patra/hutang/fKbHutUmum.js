window.app_patra_hutang_fKbHutUmum = function(owner)
{
	if (owner)
	{
		window.app_patra_hutang_fKbHutUmum.prototype.parent.constructor.call(this,owner);
		this.className  = "app_patra_hutang_fKbHutUmum";
...
window.app_patra_hutang_fKbHutUmum.extend(window.childForm);
window.app_patra_hutang_fKbHutUmum.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','-','KBHUTANG','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+","+parseNilai(this.e_nilai.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_buat.getText()+"',getdate(),'"+this.app._userLog+"','F','-','"+this.cb_hutang.getText()+"','"+this.cb_vendor.getText()+"','-')");
	...
	doChange:function(sender){
		if (sender == this.e_periode || sender == this.cb_vendor) {
			if (this.cb_vendor.getText()!="" && this.e_periode.getText()!="" ) {
				this.cb_hutang.setSQL("select a.no_hutang, a.no_dokumen+ ' - '+a.keterangan as keterangan from patra_hutang_m a "+
								   "       left join (select no_hutang,kode_lokasi,sum(case dc when 'D' then (nilai_kas+nilai_lain) else -(nilai_kas+nilai_lain) end) as bayar from patra_hutangbayar_d group by no_hutang,kode_lokasi ) c on a.no_hutang=c.no_hutang and a.kode_lokasi=c.kode_lokasi "+
		...
		if (sender == this.cb_hutang && this.cb_hutang.getText()!="") {			
			var data = this.dbLib.getDataProvider("select a.nilai_curr-isnull(c.bayar,0) as total,a.akun_hutang,a.akun_hutang + ' - '+b.nama as nama,a.kode_curr,a.kurs "+
			           "from patra_hutang_m a inner join masakun b on a.akun_hutang=b.kode_akun and b.kode_lokasi = a.kode_lokasi "+
					   "                    left join (select no_hutang,kode_lokasi,sum(case dc when 'D' then (nilai_kas+nilai_lain) else -(nilai_kas+nilai_lain) end) as bayar from patra_hutangbayar_d group by no_hutang,kode_lokasi ) c on a.no_hutang=c.no_hutang and a.kode_lokasi=c.kode_lokasi "+
