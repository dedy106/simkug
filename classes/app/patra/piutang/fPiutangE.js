window.app_patra_piutang_fPiutangE = function(owner)
{
	if (owner)
	{
		window.app_patra_piutang_fPiutangE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_patra_piutang_fPiutangE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Akru Piutang: Edit", 0);	
...
window.app_patra_piutang_fPiutangE.extend(window.childForm);
window.app_patra_piutang_fPiutangE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from patra_ar_m where no_ar = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from patra_ar_j where no_ar = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!="") {
			this.e_nb.setSQL("select no_ar, (no_dokumen+'-'+keterangan) as keterangan from patra_ar_m "+
			                 "where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_ar","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti Piutang",true);
		}
		if (sender == this.e_nb && this.e_nb.getText()!="") {			
			this.e_dok.setFocus();
			var data = this.dbLib.getDataProvider(						
			           "select a.no_dokumen,a.tanggal,a.periode,a.keterangan,a.no_fp,a.tgl_invoice,a.link,a.kode_curr,a.kurs,a.kode_cust,a.akun_ar,a.jenis,a.nilai_curr,a.nilai,a.ppn_curr,a.ppn,b.nama as nama_cust,d.nama as nama_akun "+
			           "from patra_ar_m a "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){
					...
					this.dp_d2.setText(line.tgl_invoice);
					this.e_dok.setText(line.no_dokumen);					
					this.e_fp.setText(line.no_fp);
					this.e_link.setText(line.link);
					this.e_pakai.setText(line.keterangan);
			...
			var data2 = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai_curr as nilai "+
						"from patra_ar_j a inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+						
						"where a.jenis = 'PDPT' and a.no_ar = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);			
			if (typeof data2 == "object" && data.rs.rows[0] != undefined){
				var line2;
				this.sg.clear();
				for (var i in data2.rs.rows){
					line2 = data2.rs.rows[i];							
					this.sg.appendData([line2.kode_akun,line2.nama_akun,line2.dc,line2.keterangan,floatToNilai(line2.nilai)]);
				}
			} else this.sg.clear(1);				
			this.e_usd.setText(floatToNilai(line.nilai_curr));
			this.e_idr.setText(floatToNilai(line.nilai));			
			this.e_ppnusd.setText(floatToNilai(line.ppn_curr));
			this.e_ppnidr.setText(floatToNilai(line.ppn));
		}
