...
//* 			SAI, PT											
//***********************************************************************************************
window.app_eclaim3_app = function(owner){
	window.app_eclaim3_app.prototype.parent.constructor.call(this, owner);
	this.className = "app_eclaim3_app";
	this.onTerminate.set(this,"doTerminate");
	this.form = undefined;
	this.setTitle("e-Claim");
	this.showForm();
	document.title = "e-Claim";
};
window.app_eclaim3_app.extend(window.application);
window.app_eclaim3_app.implement({
	showForm: function(){
		try
		{
			uses("util_dbLib;form;childForm");		
			this._dbSetting = "mysql";			
			this._mainForm = new app_eclaim3_fMain(this);	
	...
	logout: function(){		
		try{
			if (this.dbLib) this.dbLib.logout(this._userSession);
	...
	doKeyDown: function(charCode, buttonState, keyCode, event){
		try{			
			window.app_eclaim3_app.prototype.parent.doKeyDown.call(this, charCode, buttonState, keyCode, event);
