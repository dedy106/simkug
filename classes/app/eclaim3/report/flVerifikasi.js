window.app_eclaim3_report_flVerifikasi = function(owner)
{
	if (owner)
	{
		window.app_eclaim3_report_flVerifikasi.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim3_report_flVerifikasi";
		this.maximize();
...
window.app_eclaim3_report_flVerifikasi.extend(window.portalui_childForm);
window.app_eclaim3_report_flVerifikasi.implement({
	doEllipseClick:function(sender, col, row)
	{
		...
		if (row == 6)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Klaim",this.sg1, this.sg1.row, this.sg1.col,
													  "select a.no_klaim,b.no_dokumen,date_format(b.tanggal,'%d/%m/%Y') as tanggal from tlk_ver_m a "+
													  "inner join tlk_klaim b on a.no_klaim=b.no_klaim "+filter,
													  "select count(a.no_klaim) from tlk_ver_m a "+
													  "inner join tlk_klaim b on a.no_klaim=b.no_klaim "+filter,
		...
		if (row == 7)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Verifikasi",this.sg1, this.sg1.row, this.sg1.col,
													  "select a.no_ver,a.keterangan from tlk_ver_m a "+
													  "inner join tlk_klaim b on a.no_klaim=b.no_klaim "+filter,
													  "select count(a.no_ver) from tlk_ver_m a "+
													  "inner join tlk_klaim b on a.no_klaim=b.no_klaim "+filter,
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from tlk_ver_m order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from tlk_ver_m order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			...
			else
			{
				...
				this.filter = this.filterRep.filterStr("a.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
					  this.filterRep.filterStr("b.kode_ttg","=",this.app._kodeTtg,this.app._kodeTtg,"and")+
					  this.filterRep.filterStr("substring(j.periode,1,4)",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and") +
					  this.filterRep.filterStr("j.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and") +
				...
				this.nama_report="server_report_eclaim3_rptVerifikasi";
