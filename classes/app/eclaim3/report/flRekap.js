window.app_eclaim3_report_flRekap = function(owner)
{
	if (owner)
	{
		window.app_eclaim3_report_flRekap.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim3_report_flRekap";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Versi Laporan ","=","Versi 2012"));
...
window.app_eclaim3_report_flRekap.extend(window.portalui_childForm);
window.app_eclaim3_report_flRekap.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from tlk_klaim ",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row === 2)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["Versi 2012","Versi 2011"]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				if (this.sg1.getCell(2,2)=="Versi 2011")
				{
					this.filter = this.filterRep.filterStr("a.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
						  ...
						  " and a.status='1' ";
					this.filter2=this.filterRep.filterStr("a.no_polis",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where")+"/"+this.sg1.getCell(2,0);			
					this.showFilter = this.filterRep.showFilter(this.sg1);
					this.nama_report="server_report_eclaim3_rptRekapKlaim";
				}
				if (this.sg1.getCell(2,2)=="Versi 2012")
				{
					this.filter = this.filterRep.filterStr("a.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
						  this.filterRep.filterStr("a.kode_ttg","=",this.app._kodeTtg,this.app._kodeTtg,"and")+
						  this.filterRep.filterStr("substring(a.periode,1,4)",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and") +
					...
					this.filter2=this.filterRep.filterStr("a.no_polis",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where")+"/"+this.sg1.getCell(2,0);			
					this.showFilter = this.filterRep.showFilter(this.sg1);
					this.nama_report="server_report_eclaim3_rptRekapKlaim2";
				}
