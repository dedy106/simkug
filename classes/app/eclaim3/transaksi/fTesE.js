window.app_eclaim3_transaksi_fTesE = function(owner)
{
	if (owner)
	{
		window.app_eclaim3_transaksi_fTesE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim3_transaksi_fTesE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Testing: Edit", 0);	
		...
		this.e_nb = new saiCBBL(this,{bound:[20,12,223,20],caption:"No Testing", multiSelection:false, maxLength:10, tag:1, readOnly:true,change:[this,"doChange"]});		
...
window.app_eclaim3_transaksi_fTesE.extend(window.childForm);
window.app_eclaim3_transaksi_fTesE.implement({
		...
		try{					
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from tlk_testing where no_testing='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from tlk_testing_dok where no_testing='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
					if (this.progLama == "5") var prog = "4"; else var prog = "3";
	...
	doChange:function(sender){	
		if (sender == this.e_periode && this.e_periode.getText() != "") {										 
			this.e_nb.setSQL("select max(b.no_testing) as no_testing, b.no_klaim from tlk_testing b inner join tlk_klaim a on a.no_klaim=b.no_klaim and a.kode_lokasi=b.kode_lokasi "+
			                 "where a.progress in ('4','5') and b.periode<='"+this.e_periode.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"' group by b.no_klaim",["no_testing","no_klaim"],false,["No Tes","No Klaim"],"and","Daftar Bukti",true);
		}
		if (sender == this.e_nb && this.e_nb.getText()!="") {										    
			var strSQL = "select a.no_klaim,date_format(a.tanggal,'%d/%m/%Y') as tanggal,case a.progress when '4' then 'TESTING' when '5' then 'FINAL' end as status,a.progress "+
						 ...
						 ",f.tanggal as tgl_testing,f.no_dokumen as dok,f.catatan "+
						 "from tlk_klaim a "+
						 ...
						 "inner join tlk_testing f on a.no_klaim=f.no_klaim and a.kode_lokasi=f.kode_lokasi "+						 
						 "where f.no_testing='"+this.e_nb.getText()+"' and f.kode_lokasi='"+this.app._lokasi+"'";			
			...
			if (typeof data == "object"){
				...
				if (line != undefined){																																												
					...
					if (this.progLama == "4") this.c_status.addItem(0,"TESTING");																
					else this.c_status.addItem(0,"FINAL");																		
				} 
			}			
			this.sgUpld.clear();
			this.deleteFiles = [];
			this.listFiles = new arrayMap();			
			var data2 = this.dbLib.getDataProvider("select no_file from tlk_testing_dok where no_testing = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' order by nu",true);
