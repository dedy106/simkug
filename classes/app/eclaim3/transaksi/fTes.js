window.app_eclaim3_transaksi_fTes = function(owner)
{
	if (owner)
	{
		window.app_eclaim3_transaksi_fTes.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim3_transaksi_fTes";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengetesan: Input", 0);	
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:14,tag:0,
		            colTitle:["No Klaim","Tgl Klaim","N Estimasi","No Polis","No Dokumen","Tgl Dok","Lokasi Kejadian","Loker","Penyebab","Object","PIC","No Telp","Status","No Dok SJU"],
		...
		this.c_status = new saiCB(this.pc1.childPage[1],{bound:[20,10,202,20],caption:"Status Approval",readOnly:true,tag:2});
		...
		this.c_status2 = new saiCB(this.pc1.childPage[2],{bound:[20,10,200,20],caption:"Status Approval",items:["-","SURVEY","TESTING"], readOnly:true,tag:9});
...
window.app_eclaim3_transaksi_fTes.extend(window.childForm);
window.app_eclaim3_transaksi_fTes.implement({
	...
	simpan: function(){			
		try{		
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'tlk_testing','no_testing',"TST/BDG/"+this.e_periode.getText().substring(2)+".",'000'));
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();										
					if (this.c_status.getText()=="TESTING") var prog = "4";																
					if (this.c_status.getText()=="FINAL") var prog = "5";						
					
					sql.add("update tlk_klaim set progress='"+prog+"' where no_klaim='"+this.e_noklaim.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
					sql.add("insert into tlk_testing (no_testing,tanggal,kode_lokasi,periode,nik_user,tgl_input,status,modul,host,ip,no_klaim,no_dokumen,catatan) values "+
						    "('"+this.e_nb.getText()+"','"+this.dp_d1.getDateString()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',now(),'"+this.c_status.getText()+"','SURVEY','"+this.app._hostname+"','"+this.app._iphost+"','"+this.e_noklaim.getText()+"','"+this.e_dok.getText()+"','"+this.e_memo.getText()+"')");
										
					var ix=0;
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i)){
							ix++;
							sql.add("insert into tlk_testing_dok (no_testing,no_file,nu,kode_lokasi) values ('"+this.e_nb.getText()+"','"+this.sgUpld.cells(1,i).filedest+"','"+ix+"','"+this.app._lokasi+"')");
						}	
					}
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg.cells(0,row) != "") {											
				...
				this.c_status.items.clear();
				/*
				if (this.sg.cells(12,row) == "SURVEY") {
					this.c_status.addItem(0,"TESTING");					
					this.c_status.setText("TESTING");
					
				}
				else if (this.sg.cells(12,row) == "TESTING") {
	...
	doLoad:function(sender){			
		var strSQL = "select a.no_klaim,date_format(a.tanggal,'%d/%m/%Y') as tanggal,case progress when '3' then 'SURVEY' end as status "+
					 ...
					 "inner join tlk_survey f on a.no_klaim=f.no_klaim and f.status='FINAL' and f.jenis='LIABLE TEST' "+
	...
	doCari:function(sender){				
		...
		if (this.c_status2.getText()=="-") {
			if (this.e_nobukti.getText()!="") filter = " where a.progress in ('3','4') and a.no_klaim='"+this.e_nobukti.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"'";
			if (nilaiToFloat(this.e_nominal.getText())!=0) filter = " where a.progress in ('3','4') and a.nilai="+nilaiToFloat(this.e_nominal.getText())+" and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"'";		
		}
		else {
			if (this.c_status2.getText()=="SURVEY") filter = " where a.progress = '3' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"'"; 
			if (this.c_status2.getText()=="TESTING") filter = " where a.progress = '4' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"'";			
		}
		var strSQL = "select a.no_klaim,date_format(a.tanggal,'%d/%m/%Y') as tanggal,case progress when '3' then 'SURVEY' when '4' then 'TESTING' end as status "+
					 ...
					 "inner join tlk_sebab d on a.kode_sebab=d.kode_sebab and a.kode_lokasi=d.kode_lokasi and d.kode_ttg='"+this.app._kodeTtg+"' "+					 
					 "inner join tlk_hakakses e on a.nik_buat=e.nik and a.kode_lokasi=e.kode_lokasi and e.kode_ttg='"+this.app._kodeTtg+"' "+
					 "inner join tlk_survey f on a.no_klaim=f.no_klaim and f.status='FINAL' and f.jenis='LIABLE TEST' "+filter;					
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
			this.sgn.setTotalPage(Math.ceil(data.rs.rows.length/20));
