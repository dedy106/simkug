window.app_eclaim3_transaksi_fKerja = function(owner)
{
	if (owner)
	{
		window.app_eclaim3_transaksi_fKerja.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim3_transaksi_fKerja";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Work In Progress: Input", 0);	
		...
		this.e_nb = new portalui_saiLabelEdit(this,{bound:[20,11,202,20],caption:"No WIP",maxLength:30,readOnly:true,visible:false});		
		...
		this.e_vendor = new saiLabelEdit(this.pc1.childPage[1],{bound:[520,20,450,20],caption:"Vendor", readOnly:true});		
		...
		this.c_status2 = new saiCB(this.pc1.childPage[2],{bound:[20,10,200,20],caption:"Status Approval",items:["-","NEGO","WIP"], readOnly:true,tag:9});
...
window.app_eclaim3_transaksi_fKerja.extend(window.childForm);
window.app_eclaim3_transaksi_fKerja.implement({
	...
	simpan: function(){			
		try{		
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'tlk_wip','no_wip',"WIP/BDG/"+this.e_periode.getText().substring(2)+".",'000'));
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg.cells(0,row) != "") {											
				...
				this.c_status.items.clear();
				/*
				if (this.sg.cells(12,row) == "NEGO") {
					this.c_status.addItem(0,"WIP");					
					this.c_status.setText("WIP");										
				}
				else if (this.sg.cells(12,row) == "WIP") {
	...
	doLoad:function(sender){			
		var strSQL = "select no_klaim,date_format(a.tanggal,'%d/%m/%Y') as tanggal,case progress when '8' then 'NEGO' end as status "+
					 ...
					 "where a.periode<='"+this.e_periode.getText()+"' and a.progress = '8' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"' ";
	...
	doCari:function(sender){				
		...
		if (this.c_status2.getText()=="-") {
			if (this.e_nobukti.getText()!="") filter = " where a.progress in ('8','9') and a.no_klaim='"+this.e_nobukti.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"'";
			if (nilaiToFloat(this.e_nominal.getText())!=0) filter = " where a.progress in ('8','9') and a.nilai="+nilaiToFloat(this.e_nominal.getText())+" and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"'";		
		}
		else {
			if (this.c_status2.getText()=="NEGO") filter = " where a.progress = '8' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"'"; 
			if (this.c_status2.getText()=="WIP") filter = " where a.progress = '9' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"'";			
		}
		var strSQL = "select a.no_klaim,date_format(a.tanggal,'%d/%m/%Y') as tanggal,case progress when '8' then 'NEGO' when '9' then 'WIP' end as status "+
