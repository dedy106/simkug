window.app_eclaim3_transaksi_fLapAwalV = function(owner)
{
	if (owner)
	{
		window.app_eclaim3_transaksi_fLapAwalV.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim3_transaksi_fLapAwalV";
		...
		this.pc1 = new pageControl(this,{bound:[15,12,1000,500], childPage:["Filter","Data Klaim","Riwayat Klaim","Laporan Awal"]});
...
window.app_eclaim3_transaksi_fLapAwalV.extend(window.childForm);
window.app_eclaim3_transaksi_fLapAwalV.implement({
	...
	doSelectCell:function(sender, col, row)
	{
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from tlk_klaim order by substring(periode,1,4) desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick:function(sender)
	{
		try
		{
			...
			else
			{
				...
				var sql="select a.no_klaim, a.no_dokumen, date_format(a.tanggal,'%d-%m-%Y') as tanggal, b.nama as nama_lok, c.nama as nama_obyek, d.nama as nama_sebab, a.nilai, e.nilai_nego, e.nilai_deduc,a.alamat "+
					" from tlk_klaim a "+
					...
					" left join tlk_nego e on e.kode_lokasi = a.kode_lokasi and e.no_klaim = a.no_klaim and e.status = 'FINAL' "+this.filter+user_lok+order;
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData([line.no_klaim,line.no_dokumen,line.tanggal,line.nama_lok,line.nama_obyek,line.nama_sebab,floatToNilai(line.nilai),floatToNilai(line.nilai_nego),floatToNilai(line.nilai_deduc),line.alamat]);
		}
	...
	doSgClick: function(sender, col , row) 
	{
		try{
			...
			this.docViewer2.src = this.report.previewWithHeader("server_report_eclaim3_rptLapAwal",this.filter, 1,1, this.showFilter, this.lokasi,this.filter2);
