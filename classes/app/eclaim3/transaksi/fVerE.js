window.app_eclaim3_transaksi_fVerE = function(owner)
{
	if (owner)
	{
		window.app_eclaim3_transaksi_fVerE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim3_transaksi_fVerE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Verifikasi Lap Awal: Edit", 0);	
		...
		this.pc1 = new pageControl(this,{bound:[20,18,980,480], childPage:["Detail Lap Awal"]});
		...
		this.l_tgl2 = new portalui_label(this.pc1.childPage[0],{bound:[20,12,100,18],caption:"Tgl Jasindo", underline:true});
...
window.app_eclaim3_transaksi_fVerE.extend(window.childForm);
window.app_eclaim3_transaksi_fVerE.implement({
		...
		try{					
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from tlk_ver_m where no_ver='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from tlk_ver_d where no_ver='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
					sql.add("delete from tlk_ver_dok where no_ver='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
					sql.add("update tlk_klaim set progress='0',no_ver='-' where no_klaim='"+this.e_noklaim.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText() != "") {										 
			this.e_nb.setSQL("select b.no_ver, b.no_bukti from tlk_ver_d b inner join tlk_klaim a on a.no_klaim=b.no_bukti and a.kode_lokasi=b.kode_lokasi and b.modul='LAPAWAL' "+
							 "       inner join tlk_ver_m c on b.no_ver=c.no_ver and a.kode_lokasi=c.kode_lokasi and c.no_verseb='-' "+ 							 
			                 "where c.status in ('APPROVE','REVISI') and a.progress in ('1','R') and c.periode<='"+this.e_periode.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"'",["b.no_ver","b.no_bukti"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
		}
		if (sender == this.e_nb && this.e_nb.getText()!="") {					
			var strSQL = "select no_klaim,date_format(a.tanggal,'%d/%m/%Y') as tanggal,case progress when '1' then 'APPROVE' when 'R' then 'REVISI' end as status "+
						 ...
						 ",f.tanggal as tgl_ver, f.tgl_png, f.no_dokumen as dok, g.catatan "+
						 "from tlk_klaim a "+
						 ...
						 "inner join tlk_ver_m f on a.no_ver=f.no_ver and a.kode_lokasi=f.kode_lokasi "+
						 "inner join tlk_ver_d g on f.no_ver=g.no_ver and g.kode_lokasi=f.kode_lokasi "+
						 "where f.no_ver='"+this.e_nb.getText()+"' and f.kode_lokasi='"+this.app._lokasi+"'";
			...
			if (typeof data == "object"){
				...
				if (line != undefined){																													
					...
					this.dp_d1.setText(line.tgl_ver);
					this.dp_d2.setText(line.tgl_png);										
			...
			var data = this.dbLib.getDataProvider("select no_file from tlk_ver_dok where no_ver = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' order by nu",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sgUpld.clear();
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.listFiles.set(line.no_file,line.no_file); 
