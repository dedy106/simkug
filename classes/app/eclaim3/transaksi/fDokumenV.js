window.app_eclaim3_transaksi_fDokumenV = function(owner)
{
	if (owner)
	{
		window.app_eclaim3_transaksi_fDokumenV.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim3_transaksi_fDokumenV";
		...
		this.pc1 = new pageControl(this,{bound:[15,12,1000,500], childPage:["Filter","Data Dokumen","Data Klaim","Riwayat Klaim","Laporan Awal"]});
...
window.app_eclaim3_transaksi_fDokumenV.extend(window.childForm);
window.app_eclaim3_transaksi_fDokumenV.implement({
		...
		try
		{
			...
			else
			{
				...
				var sql="select a.no_dok,a.no_klaim,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
						"a.catatan as keterangan,c.jml_dok    "+
						"from tlk_dok_m a "+
	...
	doSgClick:function(sender, col, row){	
		...
		if (col!=5){
			try{
				...
				this.lokasi = "";
				this.filter2 = "";
				this.docViewer.src = this.report.previewWithHeader("server_report_eclaim3_rptRwyKlaim",this.filter, 1,1, this.showFilter, this.lokasi,this.filter2);
