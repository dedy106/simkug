window.app_eclaim3_transaksi_fSurvey = function(owner)
{
	if (owner)
	{
		window.app_eclaim3_transaksi_fSurvey.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim3_transaksi_fSurvey";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Survey: Input", 0);	
		...
		this.e_nb = new portalui_saiLabelEdit(this,{bound:[20,11,202,20],caption:"No Survey",maxLength:30,readOnly:true,visible:false});		
		...
		this.cb_la = new saiCBBL(this.pc1.childPage[1],{bound:[20,12,220,20],caption:"Loss Adjuster", multiSelection:false, maxLength:10, tag:1});		
		this.e_dok = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,13,450,20],caption:"No Dokumen", readOnly:true});		
		this.c_jenis = new saiCB(this.pc1.childPage[1],{bound:[520,13,202,20],caption:"Status Survey",items:["LIABLE TEST","LIABLE NOTEST","NOT LIABLE"],readOnly:true,tag:9,visible:false});
		...
		this.c_status2 = new saiCB(this.pc1.childPage[2],{bound:[20,10,200,20],caption:"Status Approval",items:["-","VER","SURVEY"], readOnly:true,tag:9});
...
window.app_eclaim3_transaksi_fSurvey.extend(window.childForm);
window.app_eclaim3_transaksi_fSurvey.implement({
	...
	simpan: function(){			
		try{		
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'tlk_survey','no_survey',"SRV/BDG/"+this.e_periode.getText().substring(2)+".",'000'));
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg.cells(0,row) != "") {											
				...
				this.c_status.items.clear();
				if (this.sg.cells(12,row) == "VER") {
					this.c_status.addItem(0,"SURVEY");					
					this.c_status.setText("SURVEY");					
					this.c_jenis.hide(); this.c_jenis.setTag("9");
				}
				else if (this.sg.cells(12,row) == "SURVEY") {
					this.c_status.addItem(0,"FINAL");
					this.c_status.setText("FINAL");					
					this.c_jenis.show(); this.c_jenis.setTag("0");
					var data = this.dbLib.getDataProvider("select kode_la from tlk_survey where status='SURVEY' and no_klaim='"+this.e_noklaim.getText()+"' and kode_lokasi='"+this.app._lokasi+"'", true);
					if (typeof data == "object") {
						var line = data.rs.rows[0];
						if (line != undefined) this.cb_la.setText(line.kode_la);						
					}
				}
			}
		} catch(e) {alert(e);}
	...
	doLoad:function(sender){			
		var strSQL = "select no_klaim,date_format(a.tanggal,'%d/%m/%Y') as tanggal,case progress when '1' then 'VER' end as status "+
					 ...
					 "where a.periode<='"+this.e_periode.getText()+"' and a.progress in ('1') and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"' ";
	...
	doCari:function(sender){				
		...
		if (this.c_status2.getText()=="-") {
			if (this.e_nobukti.getText()!="") filter = " where a.progress in ('1','2') and a.no_klaim='"+this.e_nobukti.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"'";
			if (nilaiToFloat(this.e_nominal.getText())!=0) filter = " where a.progress in ('1','2') and a.nilai="+nilaiToFloat(this.e_nominal.getText())+" and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"'";		
		}
		else {
			if (this.c_status2.getText()=="VER") filter = " where a.progress = '1' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"'"; 
			if (this.c_status2.getText()=="SURVEY") filter = " where a.progress = '2' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"'";			
		}
		var strSQL = "select a.no_klaim,date_format(a.tanggal,'%d/%m/%Y') as tanggal,case progress when '1' then 'VER' when '2' then 'SURVEY' end as status "+
					 ",a.nilai,a.no_polis,a.no_dokumen,date_format(a.tgl_dokumen,'%d/%m/%Y') as tgl_dokumen,a.alamat,b.nama as loker,c.nama as sebab,d.nama as objek, e.nama as pic, a.no_tel,h.no_dokumen as no_sju "+ 
		             "from tlk_klaim a "+
		             "inner join tlk_lokasi b on a.kode_lok=b.kode_lok and a.kode_lokasi=b.kode_lokasi and b.kode_ttg='"+this.app._kodeTtg+"' "+					 
					 "inner join tlk_obyek c on a.kode_obyek=c.kode_obyek and a.kode_lokasi=c.kode_lokasi and c.kode_ttg='"+this.app._kodeTtg+"' "+					 
					 ...
					 "inner join tlk_hakakses e on a.nik_buat=e.nik and a.kode_lokasi=e.kode_lokasi and e.kode_ttg='"+this.app._kodeTtg+"' "+
					 "inner join tlk_ver_d g on a.no_klaim=g.no_bukti "+
					 "inner join tlk_ver_m h on g.no_ver=h.no_ver "+filter;					
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
			this.sgn.setTotalPage(Math.ceil(data.rs.rows.length/20));
