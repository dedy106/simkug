window.app_eclaim3_transaksi_fBA = function(owner)
{
	if (owner)
	{
		window.app_eclaim3_transaksi_fBA.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim3_transaksi_fBA";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form BAST: Input", 0);	
		...
		this.e_nb = new portalui_saiLabelEdit(this,{bound:[20,11,202,20],caption:"No BAST",maxLength:30,readOnly:true,visible:false});				
		...
		this.sgUpld = new saiGrid(this.pc1.childPage[1],{bound:[121,21,848,230],colCount:4,
		...
		this.sgUpld2 = new saiGrid(this.pc1.childPage[2],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:5,tag:9,
		...
		this.c_status2 = new saiCB(this.pc1.childPage[3],{bound:[20,10,200,20],caption:"Status Approval",items:["-","WIP","BAUT","BAST"], readOnly:true,tag:9});
...
window.app_eclaim3_transaksi_fBA.extend(window.childForm);
window.app_eclaim3_transaksi_fBA.implement({
	...
	simpan: function(){			
		try{		
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'tlk_ba','no_ba',"BA/BDG/"+this.e_periode.getText().substring(2)+".",'000'));
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();										
					if (this.progress == "A") { var prog = "B"; var status = "BAUT";}
					if (this.progress == "B") { var prog = "C"; var status = "BAST";}
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg.cells(0,row) != "") {											
				...
				if (this.sg.cells(12,row)=="WIP") { this.l_label.setCaption("Data BAUT"); this.progress = "A"; }
				if (this.sg.cells(12,row)=="BAUT") { this.l_label.setCaption("Data BAST"); this.progress = "B"; }
				if (this.sg.cells(12,row)=="BAST") { this.l_label.setCaption("Data DFORM"); this.progress = "C"; }
				
				var data = this.dbLib.getDataProvider("select c.status,a.no_dokumen,date_format(a.tgl_dok,'%d/%m/%Y') as tgl_dok,c.catatan,a.no_file "+ 
						   "from tlk_ba_dok a inner join tlk_ba c on a.no_ba=c.no_ba and a.kode_lokasi=c.kode_lokasi "+ 						   
	...
	doLoad:function(sender){			
		var strSQL = "select a.no_klaim,date_format(a.tanggal,'%d/%m/%Y') as tanggal,case progress when 'A' then 'WIP' when 'B' then 'BAUT' when 'C' then 'BAST' end as status,a.progress "+
					 ...
					 "where a.periode<='"+this.e_periode.getText()+"' and a.progress in ('A','B','C') and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"' "+
					 "order by a.progress";		
	...
	doCari:function(sender){				
		...
		if (this.c_status2.getText()=="-") {
			if (this.e_nobukti.getText()!="") filter = " where a.progress in ('A','B','C') and a.no_klaim='"+this.e_nobukti.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"'";
			if (nilaiToFloat(this.e_nominal.getText())!=0) filter = " where a.progress in ('A','B','C') and a.nilai="+nilaiToFloat(this.e_nominal.getText())+" and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"'";		
		}
		else {
			if (this.c_status2.getText()=="WIP") filter = " where a.progress = 'A' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"'"; 
			if (this.c_status2.getText()=="BAUT") filter = " where a.progress = 'B' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"'";			
			if (this.c_status2.getText()=="BAST") filter = " where a.progress = 'C' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"'";			
		}		
		var strSQL = "select a.no_klaim,date_format(a.tanggal,'%d/%m/%Y') as tanggal,case progress when 'A' then 'WIP' when 'B' then 'BAUT' when 'C' then 'BAST' end as status,a.progress "+
					 ",a.nilai,a.no_polis,a.no_dokumen,date_format(a.tgl_dokumen,'%d/%m/%Y') as tgl_dokumen,a.alamat,b.nama as loker,c.nama as sebab,d.nama as objek, e.nama as pic, a.no_tel "+ 
		             "from tlk_klaim a "+
