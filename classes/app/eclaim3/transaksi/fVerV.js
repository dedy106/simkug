window.app_eclaim3_transaksi_fVerV = function(owner)
{
	if (owner)
	{
		window.app_eclaim3_transaksi_fVerV.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim3_transaksi_fVerV";
		...
		this.pc1 = new pageControl(this,{bound:[15,12,1000,500], childPage:["Filter","Data Verifikasi","Data Verifikasi","Data Riwayat Klaim"]});
		...
		this.tahun=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from tlk_ver_m");
		...
		this.sg1mp = new saiGrid(this.pc1.childPage[1],{bound:[1,20,this.pc1.width - 4,this.pc1.height - 180],colCount:7,colTitle:["No. Verifikasi","No. Klaim","Tgl Verifikasi","No. Dokumen","Keterangan","Lokasi","View Dokumen"],
					colWidth:[[6,5,4,3,2,1,0],[100,200,200,150,100,100,100]],
...
window.app_eclaim3_transaksi_fVerV.extend(window.childForm);
window.app_eclaim3_transaksi_fVerV.implement({
	...
	doEllipseClick:function(sender, col, row)
	{
		if (row == 2)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Klaim",this.sg1, this.sg1.row, this.sg1.col,
												...
												"from tlk_klaim a "+
												"inner join tlk_ver b on a.no_klaim=b.no_klaim "+filter,
	...
	doSelectCell:function(sender, col, row)
	{
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from tlk_ver_m order by substring(periode,1,4) desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			...
			else
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
					  this.filterRep.filterStr("c.kode_ttg","=",this.app._kodeTtg,this.app._kodeTtg,"and")+
					  this.filterRep.filterStr("substring(a.periode,1,4)",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and") +
					  this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and") +
					  this.filterRep.filterStr("c.no_klaim",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
					  this.filterRep.filterStr("a.no_ver",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				...
				var sql="select a.no_ver,c.no_klaim,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
						"a.no_dokumen,b.catatan as keterangan,c.alamat    "+
						"from tlk_ver_m a "+
						"inner join tlk_ver_d b on a.no_ver=b.no_ver "+
						"inner join tlk_klaim c on b.no_bukti=c.no_klaim "+this.filter+user_lok+
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1mp.appendData([line.no_ver,line.no_klaim,line.tanggal, line.no_dokumen, line.keterangan,line.alamat,"Preview"]);
			this.sg1mp.rows.get(i).setCellColor(6,"#ff9900");
			this.sg1mp.rows.get(i).setHint("Click untuk menampilkan detail");
	...
	doSgClick:function(sender, col, row){	
		...
		if (col!=6){
			try{
				...
				this.lokasi = "";
				this.filter2 = "";
				this.docViewer.src = this.report.previewWithHeader("server_report_eclaim3_rptRwyKlaimSum",this.filter, 1,1, this.showFilter, this.lokasi,this.filter2);
				this.docViewer2.src = this.report.previewWithHeader("server_report_eclaim3_rptVerifikasi",this.filter, 1,1, this.showFilter, this.lokasi,this.filter2);
		...
		if (col==6){
			try{
				var data = this.dbLib.getDataProvider("select no_file as nm from tlk_ver_dok "+
