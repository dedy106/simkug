window.app_eclaim3_dashboard_fDbAging = function(owner)
{
	if (owner)
	{
		window.app_eclaim3_dashboard_fDbAging.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim3_dashboard_fDbAging";
		...
		this.pc1 = new pageControl(this,{bound:[15,12,1000,500], childPage:["Filter","Proses Klaim","Outstanding Divisi","Outstanding Regional","Outstanding Obyek","Outstanding Penyebab","Data Klaim ","Riwayat Klaim","Laporan Awal"]});
...
window.app_eclaim3_dashboard_fDbAging.extend(window.childForm);
window.app_eclaim3_dashboard_fDbAging.implement({
	...
	doSgClick2: function(sender, col , row) 
	{
		try{
			...
			var sql="select a.no_klaim, a.no_dokumen, date_format(a.tanggal,'%d-%m-%Y') as tanggal, b.nama as nama_lok, c.nama as nama_obyek, d.nama as nama_sebab, a.nilai, e.nilai_nego as nilai_adjust, e.nilai_deduc as nilai_bayar,a.alamat "+
					...
					" left join tlk_nego e on e.kode_lokasi = a.kode_lokasi and e.no_klaim = a.no_klaim and e.status = '1' "+this.filter;
			var data = this.dbLib.getDataProvider(sql,true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.dataJU = data;
