window.app_eclaim3_dashboard_fDbKlaim = function(owner)
{
	if (owner)
	{
		window.app_eclaim3_dashboard_fDbKlaim.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim3_dashboard_fDbKlaim";
		...
		this.pc1 = new pageControl(this,{bound:[15,12,1000,500], childPage:["Filter","Bulan Kejadian","Obyek Kerugian","Penyebab Kerugian","Divisi","Regional","Proses Klaim","Data Klaim","Riwayat Klaim","Laporan Awal"]});
		...
		*/
		...
		var cnv2 = this.pc1.childPage[9].getClientCanvas();
...
window.app_eclaim3_dashboard_fDbKlaim.extend(window.childForm);
window.app_eclaim3_dashboard_fDbKlaim.implement({
	...
	doChartClick: function(sender,index,title){	
		eval("sender="+sender+"");
		try{
			var sql="select a.no_klaim, a.no_dokumen, date_format(a.tanggal,'%d-%m-%Y') as tanggal, b.nama as nama_lok, c.nama as nama_obyek, d.nama as nama_sebab, a.nilai, e.nilai_nego as nilai_adjust, e.nilai_deduc as nilai_bayar "+
					" from tlk_klaim a "+
					...
					" left join tlk_nego e on e.kode_lokasi = a.kode_lokasi and e.no_klaim = a.no_klaim and e.status = '1' "+
	...
	doSgClick: function(sender, col , row) 
	{
		try{
			...
			this.lokasi = "";
			this.filter2 = "";
			this.docViewer.src = this.report.previewWithHeader("server_report_eclaim3_rptRwyKlaimSum",this.filter, 1,1, this.showFilter, this.lokasi,this.filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId();
