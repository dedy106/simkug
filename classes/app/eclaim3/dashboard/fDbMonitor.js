window.app_eclaim3_dashboard_fDbMonitor = function(owner)
{
	if (owner)
	{
		window.app_eclaim3_dashboard_fDbMonitor.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim3_dashboard_fDbMonitor";
		...
		this.pc1 = new pageControl(this,{bound:[15,12,980,500], childPage:["Filter","Data Monitoring","Rekapitulasi Klaim","Rekap Penanggung Jawab","Data Klaim","Riwayat Klaim","Laporan Awal"]});
		/*
		...
		*/
		...
		this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Tahun ","All",""));
		this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("No Polis ","=",this.polis));
		this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Judul Laporan ","=","Polis"));
		this.sg = new saiGrid(this.pc1.childPage[4],{bound:[1,11,this.pc1.width-5,this.pc1.height-50],colCount:10,tag:9,
		...
		this.pWidget4 = new panel(this.pc1.childPage[1],{bound:[330,300,300,240], caption:"Klaim Per Kelompok Lokasi"});
		...
		var cnv4 = this.pc1.childPage[6].getClientCanvas();
...
window.app_eclaim3_dashboard_fDbMonitor.extend(window.childForm);
window.app_eclaim3_dashboard_fDbMonitor.implement({
	...
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2),new  Array("123","123","3"));			
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2),new  Array(0,2,0));
		if (row == 0)
		{
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["Polis","Tahun","Gabung"]);
		}
	},
	mainButtonClick:function(sender)
	{
		try
		{
			...
			else
			{
				...
				this.doRekapKlaimPnj(this.filter,this.filter2);
			}
		}catch(e)
		{
	...
	doSgClick:function(sender, col, row){	
		if (col!=5){
			try{
				...
				this.filter2 = "";
				this.docViewer.src = this.report.previewWithHeader("server_report_eclaim2_rptRwyKlaim",this.filter, 1,1, this.showFilter, this.lokasi,this.filter2);
				this.docViewer2.src = this.report.previewWithHeader("server_report_eclaim2_rptVerifikasi",this.filter, 1,1, this.showFilter, this.lokasi,this.filter2);
		...
		if (col==5){
			try{
				var data = this.dbLib.getDataProvider("select nama as nm from tlk_ver_dok "+
	...
	doInformasi: function(filter){
		try{
			var sql="select count(a.no_klaim) as jum from tlk_klaim a "+filter;
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined)
			{
				...
				for (var i=0;i<finish;i++){
					...
					var jum_klaim=line.jum;
				}
			}
			var sql = "select a.kode_proses,a.nama,b.jml "+
								"from tlk_proses a "+
								"inner join (select a.progress,count(a.no_klaim) as jml "+
								"	  from tlk_klaim a "+filter+
								"	  group by a.progress "+
								"	 ) b on b.progress=a.kode_proses "+
								"order by a.no_urut";
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined)
			{
				...
				for (var i=0;i<finish;i++){
					line = this.dataJU.rs.rows[i];	
					var no_bukti=this.sg1.getCell(2,0)+"/1/"+line.kode_proses;
				...
				html=html+"<tr><td width='10'><img  src='image/check.png'></td><td width='150'>Jumlah Klaim</td><td width='30' align='center'>"+jum_klaim+"</td></tr>";
				html=html+"</table>";
			}
			this.cnv1.innerHTML = html;
			
			var sql = "select a.kode_obyek,b.nama,a.jml "+
								"from (select a.kode_obyek,count(a.no_klaim) as jml "+
								"	  from tlk_klaim a "+filter+
								"	  group by a.kode_obyek "+
								"	 )a "+
								"inner join tlk_obyek b on a.kode_obyek=b.kode_obyek "+
								"order by a.jml desc";
			var data = this.dbLib.getDataProvider(sql,true);
			var html="";
			if (typeof data == "object" && data.rs.rows[0] != undefined)
			...
			var sql = "select a.kode_sebab,b.nama,a.jml "+
								"from (select a.kode_sebab,count(a.no_klaim) as jml "+
								"	  from tlk_klaim a "+filter+
			...
			var sql = "select a.periode,case substring(a.periode,5,2) when '01' then 'Januari'  "+
						...
						"end as nama,count(a.no_klaim) as jml "+
						"from tlk_klaim a "+filter+
						"group by a.periode";
			var data = this.dbLib.getDataProvider(sql,true);
			var html="";
			...
			var sql = "select a.kode_klplok,b.nama,a.jml "+
								"from (select c.kode_klplok,count(a.no_klaim) as jml "+
								"	  from tlk_klaim a "+
								"	  inner join tlk_lokasi c on a.kode_lok=c.kode_lok "+filter+
								"	  group by c.kode_klplok "+
								"	 )a "+
								"inner join tlk_klplokasi b on a.kode_klplok=b.kode_klplok "+
								"order by a.jml desc";
			var data = this.dbLib.getDataProvider(sql,true);
			var html="";
			if (typeof data == "object" && data.rs.rows[0] != undefined)
			{
				...
				for (var i=0;i<finish;i++){
					line = this.dataJU.rs.rows[i];	
					var no_bukti=this.sg1.getCell(2,0)+"/5/"+line.kode_klplok;
					html=html+"<tr><td width='10'><img  src='image/check.png'></td><td width='150'><a style='cursor:pointer' onclick='$$("+this.resourceId +").doViewItem(event,\""+no_bukti+"\");'>"+line.nama+"</a></td><td width='30' align='center'>"+line.jml+"</td></tr>";
				}
				html=html+"</table>";
			}
			...
			var sql = "select a.kode_lok,b.nama,a.jml "+
								"from (select a.kode_lok,count(a.no_klaim) as jml "+
								"	  from tlk_klaim a "+filter+
								"	  group by a.kode_lok "+
								"	 )a "+
	...
	doViewItem: function(event,no_bukti){
		try{
			...
			if (modul=="5")
			{
				tmp=" and b.kode_klplok='"+no_bukti+"' ";
			}
			if (modul=="6")
			{
				tmp=" and a.kode_lok='"+no_bukti+"' ";
			}
			var sql="select a.no_klaim, a.no_dokumen, date_format(a.tanggal,'%d-%m-%Y') as tanggal, b.nama as nama_lok, c.nama as nama_obyek, d.nama as nama_sebab, a.nilai, e.nilai_nego as nilai_adjust, e.nilai_deduc as nilai_bayar,a.alamat "+
					" from tlk_klaim a "+
					...
					" left join tlk_nego e on e.kode_lokasi = a.kode_lokasi and e.no_klaim = a.no_klaim and e.status = '1' "+this.filter+tmp+
	...
	doSgClick: function(sender, col , row) 
	{
		try{
			...
			this.report = new server_report_report();
			filter = "where a.no_klaim='"+this.sg.cells(0,row)+"'";
			this.showFilter = "";
			lokasi = "";
			filter2 = "";
			this.docViewer.src = this.report.previewWithHeader("server_report_eclaim3_rptRwyKlaimSum",filter, 1,1, this.showFilter, lokasi,filter2);
			this.docViewer4.src = this.report.previewWithHeader("server_report_eclaim3_rptLapAwal",filter, 1,1, this.showFilter, lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId();
	...
	doOpenKlaim: function(kode_proses,status) 
	{
		try{
			filter="2013"+"/"+"1"+"/"+kode_proses;
			this.doViewItem("",filter);
			this.pc1.setActivePage(this.pc1.childPage[4]);
		}catch(e){
			alert(e);
		}
	...
	doRekapKlaim: function(filter,filter2) 
	{
		try{
			...
			this.docViewer2.src = this.report.previewWithHeader("server_report_eclaim3_rptRekapKlaim2",this.filter, 1,1, this.showFilter, this.lokasi,this.filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId();
			
		}catch(e){
			alert(e);
		}
	},
	doRekapKlaimPnj: function(filter,filter2) 
	{
		try{
			...
			this.lokasi = "";
			this.filter2 = filter2+"/"+this.sg1.getCell(2,0);
			this.docViewer3.src = this.report.previewWithHeader("server_report_eclaim3_rptRekapKlaimPnj",this.filter, 1,1, this.showFilter, this.lokasi,this.filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId();
