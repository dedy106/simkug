window.app_eclaim3_dashboard = function(owner,options)
{
	if (owner)
	{
		window.app_eclaim3_dashboard.prototype.parent.constructor.call(this,owner,options);
		this.className  = "app_eclaim3_dashboard";
		...
		try {
			...
			this.dataProvider = new app_eclaim3_remote_dataProvider(this.app._dbSetting);
...
window.app_eclaim3_dashboard.extend(window.panel);
window.app_eclaim3_dashboard.implement({	
	...
	doViewItem: function(event,noklaim){
		try{
			...
			this.docViewer2.src = this.report.previewWithHeader("server_report_eclaim3_rptRwyKlaim",this.filter, 1,1, this.showFilter, this.lokasi,this.filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId();
	...
	doSgClick: function(sender, col , row) 
	{
		try{
			...
			this.docViewer2.src = this.report.previewWithHeader("server_report_eclaim3_rptRwyKlaim",this.filter, 1,1, this.showFilter, this.lokasi,this.filter2);
			this.pg1.setActivePage(this.pg1.childPage[3]);
		}catch(e){
			alert(e);
		}
