window.app_eclaim_transaksi_fVerK = function(owner) {
	if (owner){
		window.app_eclaim_transaksi_fVerK.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim_transaksi_fVerK";		
		...
		this.ed_kode.setSQL("select a.no_ver, a.keterangan, a.no_klaim "+
							...
							"where a.kode_lokasi = '"+this.app._lokasi+"' and a.kode_ttg = '"+this.app._kodeTtg+"' and b.progress='1'  "
							,["a.no_ver","a.keterangan","a.no_klaim"],false,["No Ver","Keterangan","No Klaim"],"and","Data Verifikasi Klaim",true);

	}
};
window.app_eclaim_transaksi_fVerK.extend(window.childForm);
window.app_eclaim_transaksi_fVerK.implement({
			...
			switch(event){
				...
				case "hapus" :
					if (this.progress == 1){
						...
						sql.add("delete from eclaim_ver where no_ver = '"+this.ed_kode.getText()+"' and kode_ttg = '"+this.app._kodeTtg +"' ");
						sql.add("delete from eclaim_ver_dok where no_ver = '"+this.ed_kode.getText()+"' and kode_ttg = '"+this.app._kodeTtg +"' ");
						sql.add("update eclaim_klaim set progress = '0' where no_klaim = '"+this.cb_klaim.getText()+"' and kode_ttg ='"+this.app._kodeTtg+"' ");
				...
				case "ubah" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){						
						...
						sql.add("update eclaim_ver set no_klaim ='"+this.cb_klaim.getText()+"', tanggal='"+this.dp_tgl.getDateString()+"', no_dokumen = '"+this.ed_dok.getText()+
								"',keterangan='"+this.ed_ket.getText()+"',host='"+this.app._hostname+"',ip='"+this.app._iphost+"',nik_ver='"+this.cb_buat.getText()+"',tgl_input=now(),tgl_png='"+this.dp_tgl.getDateString()+"',kode_ttg='"+this.app._kodeTtg+"' "+
								" where no_ver = '"+this.ed_kode.getText()+"' and kode_ttg = '"+ this.app._kodeTtg +"'");
						sql.add("delete from eclaim_ver_dok where no_ver = '"+this.ed_kode.getText()+"' and kode_ttg = '"+this.app._kodeTtg +"' ");
						if (files.length > 0){
	...
	doClick: function(sender){
		this.ed_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'eclaim_ver','no_ver',"VER/"+this.ed_Periode.getText().substring(2)+"/",'0000'));
	},
		...
		try{
			if (sender == this.ed_kode){
				var data = this.dbLib.getDataProvider("select date_format(a.tanggal,'%d/%m/%Y')as tgl,date_format(a.tgl_png,'%d/%m/%Y')as tgl_png, a.no_dokumen, a.keterangan, a.no_klaim, b.penyebab,b.progress,  ifnull(c.no_file,'-') as no_file, nu,a.nik_ver,d.nama as nama_ver "+
					"from eclaim_ver a "+
					"inner join eclaim_klaim b on b.no_klaim = a.no_klaim and b.kode_ttg = a.kode_ttg  "+
					" left outer join eclaim_ver_dok c on c.no_ver = a.no_ver and c.kode_ttg = a.kode_ttg "+
					" left join eclaim_hakakses d on a.nik_ver=d.nik and c.kode_ttg = d.kode_ttg "+
					" where a.no_ver = '"+sender.getText()+"' and a.kode_ttg = '"+this.app._kodeTtg+"' "+
					" order by nu",true);
	...
	doMail: function(no_ver){
		try{
			...
			this.filter = " where h.no_ver='"+no_ver+"' and h.kode_ttg='"+this.app._kodeTtg+"'";
			this.filter2 = "";
			this.viewer.prepare();
