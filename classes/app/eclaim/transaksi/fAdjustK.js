window.app_eclaim_transaksi_fAdjustK = function(owner) {
	if (owner){
		window.app_eclaim_transaksi_fAdjustK.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim_transaksi_fAdjustK";		
...
window.app_eclaim_transaksi_fAdjustK.extend(window.portalui_childForm);
window.app_eclaim_transaksi_fAdjustK.implement({
			...
			switch(event){
				...
				case "ubah" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						sql.add("delete from eclaim_adjust where kode_ttg = '"+this.app._kodeTtg+"' and no_adjust = '"+this.ed_kode.getText()+"' ");
						sql.add("delete from eclaim_adjust_dok where kode_ttg = '"+this.app._kodeTtg+"' and no_adjust = '"+this.ed_kode.getText()+"' ");
						...
						{
							sql.add("update eclaim_klaim set progress = '3' where no_klaim = '"+this.cb_klaim.getText()+"' and kode_ttg ='"+this.app._kodeTtg+"' ");
						}
						if (files.length > 0){
				...
				case "hapus" :
					sql.add("delete from eclaim_adjust where kode_ttg = '"+this.app._kodeTtg+"' and no_adjust = '"+this.ed_kode.getText()+"' ");
					sql.add("delete from eclaim_adjust_dok where kode_ttg = '"+this.app._kodeTtg+"' and no_adjust = '"+this.ed_kode.getText()+"' ");
					sql.add("update eclaim_klaim a left outer join eclaim_adjust b on b.no_klaim = a.no_klaim and b.kode_lokasi = a.kode_lokasi "+
						" set a.progress = '3' "+						
						" where a.no_klaim = '"+this.cb_klaim.getText()+"' and a.kode_ttg ='"+this.app._kodeTtg+"' ");
					this.dbLib.execArraySQL(sql);
				break;
			}			
		}catch(e){
	...
	doFindBtnClick: function(sender){
		if (sender == this.cb_survey){
			this.standarLib.showListData(this, "Data Survey",sender,undefined, 
										  "select no_survey, keterangan from eclaim_survey where kode_ttg = '"+this.app._kodeTtg+"' and no_klaim='"+this.cb_klaim.getText()+"' ",
										  "select count(no_survey) from eclaim_survey where kode_ttg = '"+this.app._kodeTtg+"' and no_klaim='"+this.cb_klaim.getText()+"'  ",
		...
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, penyebab from eclaim_klaim where  kode_ttg='"+this.app._kodeTtg+"' and progress in ('4')   ",
										  "select count(no_klaim) from eclaim_klaim where  kode_ttg='"+this.app._kodeTtg+"' and progress in ('4') ",
		...
		if (sender == this.ed_kode){
			this.standarLib.showListData(this, "Data Adjustment",sender,undefined, 
										  ...
										  " where  b.kode_ttg='"+this.app._kodeTtg+"' and a.nik_user = '"+this.app._userLog+"'  and a.status='1' ",
		...
		try{			
			if (sender == this.ed_kode){
				if (sender.getText() != ""){	
					this.sgUpld.clear();
					var sql = new server_util_arrayList();
					sql.add("select a.*, b.penyebab, c.keterangan as ket_survey from eclaim_adjust a "+
						" inner join eclaim_klaim b on b.no_klaim = a.no_klaim and b.kode_lokasi = a.kode_lokasi "+
						" inner join eclaim_survey c on c.no_survey = a.no_survey and c.kode_lokasi = a.kode_lokasi "+
						" where a.kode_lokasi = '"+this.app._lokasi+"' and a.no_adjust = '"+sender.getText()+"' ");
					sql.add("select nama, nu from eclaim_adjust_dok where kode_lokasi =  '"+this.app._lokasi+"' and no_adjust = '"+sender.getText()+"' order by nu");
					...
					if (typeof data != "string" && data.result[0].rs.rows[0] !== undefined){
						...
						this.ed_tuntutan.setText(floatToNilai(data.nilai_tuntutan));
						var line;
						this.fileBfr = new portalui_arrayMap();
