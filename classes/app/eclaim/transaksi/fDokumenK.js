window.app_eclaim_transaksi_fDokumenK = function(owner) {
	if (owner){
		window.app_eclaim_transaksi_fDokumenK.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim_transaksi_fDokumenK";		
		...
		this.cStatus = new portalui_saiCB(this,{bound:[20,8,200,20],caption:"Status Dokumen",items:["LENGKAP","KURANG"],tag:2});
		this.ed_ket = new portalui_saiMemo(this,{bound:[20,4,600,50],caption:"Keterangan", maxLength:150});				
		
		this.p1 = new portalui_panel(this,{bound:[20,10,800,230],caption:"Upload Dokumen (Max 2 Mb)"});		
...
window.app_eclaim_transaksi_fDokumenK.extend(window.portalui_childForm);
window.app_eclaim_transaksi_fDokumenK.implement({
			...
			switch(event){
				...
				case "ubah" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						var status_dok="0";
						if (this.cStatus.getText() === "LENGKAP") 
						{
							status_dok = "1";
						}
						sql.add("delete from eclaim_dok_m where kode_ttg = '"+this.app._kodeTtg+"' and no_dok = '"+this.ed_kode.getText()+"' ");
						sql.add("delete from eclaim_dok_d where kode_ttg = '"+this.app._kodeTtg+"' and no_dok = '"+this.ed_kode.getText()+"' ");
						sql.add("insert into eclaim_dok_m (no_dok,no_klaim,kode_lokasi,tanggal,keterangan,periode,nik_user,tgl_input,kode_ttg,status_dok)values "+
							"('"+this.ed_kode.getText()+"','"+this.cb_klaim.getText()+"','"+this.app._lokasi+"','"+this.dp_tgl.getDateString()+"','"+this.ed_ket.getText()+"',"+
							" '"+this.ed_periode.getText()+"','"+this.app._userLog+"',now(),'"+this.app._kodeTtg+"','"+status_dok+"') ");
						sql.add("update eclaim_klaim set progress = '3',status_dok='"+status_dok+"' where no_klaim = '"+this.cb_klaim.getText()+"' and kode_ttg ='"+this.app._kodeTtg+"' ");
						if (files.length > 0){
							var scan = "insert into eclaim_dok_d (no_dok,kode_lokasi,kode_ref,no_file,nu,nik_user,tgl_input,ket_dok,kode_ttg) values ";						
							var first = true;
							var noUrut=1;
							for (var i in files){
								if (!first) scan +=",";
								scan += "('"+this.ed_kode.getText()+"','"+this.app._lokasi+"','"+this.sgUpld.cells(0,i)+"','"+files[i]+"',"+noUrut+",'"+this.app._userLog+"',now(),'"+this.sgUpld.cells(4,i)+"','"+this.app._kodeTtg+"') ";
								first = false;
								noUrut++;
							}
							sql.add(scan);
				...
				case "hapus" :
					sql.add("delete from eclaim_dok_m where kode_ttg = '"+this.app._kodeTtg+"' and no_dok = '"+this.ed_kode.getText()+"' ");
					sql.add("delete from eclaim_dok_d where kode_ttg = '"+this.app._kodeTtg+"' and no_dok = '"+this.ed_kode.getText()+"' ");
					sql.add("update eclaim_klaim set progress = '1' where no_klaim = '"+this.cb_klaim.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");
					this.dbLib.execArraySQL(sql);
				break;
			}			
		}catch(e){
	...
	doFindBtnClick: function(sender){
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, penyebab from eclaim_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and status='0' ",
										  "select count(no_klaim) from eclaim_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and status='0' ",
										  ["no_klaim","penyebab"],"and",["No Klaim","Keterangan"],false); 
		}
		if (sender == this.ed_kode){
			this.standarLib.showListData(this, "Data Dokumen Survey",sender,undefined, 
										  "select a.no_dok, a.keterangan from eclaim_dok_m a inner join eclaim_klaim b on b.no_klaim = a.no_klaim and b.kode_ttg = a.kode_ttg "+
										  " where a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_ttg='"+this.app._kodeTtg+"'  ",
										  "select count(a.no_dok) from eclaim_dok_m a inner join eclaim_klaim b on b.no_klaim = a.no_klaim and b.kode_ttg = a.kode_ttg "+
										  " where a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_ttg='"+this.app._kodeTtg+"'   ",
			...
			if (sender == this.sgUpld) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Kode Referensi",sender,undefined, 
												  "select a.kode_ref, a.nama from eclaim_ref a inner join eclaim_klaim b on a.kode_ttg=b.kode_ttg and a.kode_asuransi=b.kode_asuransi where a.kode_lokasi = '"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"' and b.no_klaim='"+this.cb_klaim.getText()+"'",
												  "select count(a.kode_ref)  from eclaim_ref a inner join eclaim_klaim b on a.kode_ttg=b.kode_ttg and a.kode_asuransi=b.kode_asuransi where a.kode_lokasi = '"+this.app._lokasi+"' and a.kode_ttg='"+this.app._kodeTtg+"' and b.no_klaim='"+this.cb_klaim.getText()+"'",
	...
	doChange: function(sender){	
		try{
			if (sender == this.ed_kode){
				if (sender.getText() != ""){	
					var sql = new server_util_arrayList();
					sql.add("select a.*, b.penyebab from eclaim_dok_m a "+
						" inner join eclaim_klaim b on b.no_klaim  = a.no_klaim and b.kode_ttg = a.kode_ttg "+
						" where a.kode_ttg = '"+this.app._kodeTtg+"' and a.no_dok = '"+sender.getText()+"' ");
					sql.add("select a.nu, a.kode_ref, d.nama as nm_ref, a.no_file as nama,a.ket_dok from eclaim_dok_d a "+
						...
						" inner join eclaim_ref d on d.kode_ref = a.kode_ref and a.kode_ttg = d.kode_ttg and c.kode_asuransi=d.kode_asuransi "+
						" where a.kode_ttg = '"+this.app._kodeTtg+"' and a.no_dok = '"+sender.getText()+"' order by a.nu");
	...
	doClick: function(sender){
		this.ed_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'eclaim_dok_m','no_dok',"DOK/"+this.ed_periode.getText().substring(2)+"/",'0000'));
	},
