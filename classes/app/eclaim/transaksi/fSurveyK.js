window.app_eclaim_transaksi_fSurveyK = function(owner) {
	if (owner){
		window.app_eclaim_transaksi_fSurveyK.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim_transaksi_fSurveyK";		
...
window.app_eclaim_transaksi_fSurveyK.extend(window.portalui_childForm);
window.app_eclaim_transaksi_fSurveyK.implement({
			...
			switch(event){
				...
				case "ubah" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						sql.add("delete from eclaim_survey_dok where kode_ttg = '"+this.app._kodeTtg+"' and no_survey = '"+this.ed_kode.getText()+"' ");
						sql.add("delete from eclaim_survey where kode_ttg = '"+this.app._kodeTtg+"' and no_survey = '"+this.ed_kode.getText()+"' ");
				...
				case "hapus" :
					sql.add("delete from eclaim_survey_dok where kode_ttg = '"+this.app._kodeTtg+"' and no_survey = '"+this.ed_kode.getText()+"' ");
					sql.add("delete from eclaim_survey where kode_ttg = '"+this.app._kodeTtg+"' and no_survey = '"+this.ed_kode.getText()+"' ");
					sql.add("update eclaim_klaim set progress = '1',status='0' where no_klaim = '"+this.cb_klaim.getText()+"' and kode_ttg ='"+this.app._kodeTtg+"' ");
					this.dbLib.execArraySQL(sql);
				break;
			}			
		}catch(e){
		...
		switch (sender){
			case this.cb_klaim : 
				this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, penyebab from eclaim_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('0','1') and status='0'  ",
										  "select count(no_klaim) from eclaim_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('0','1') and status='0' ",
			...
			case this.ed_kode :
				this.standarLib.showListData(this, "Data Survey",sender,undefined, 
										  "select a.no_survey, a.keterangan from eclaim_survey a inner join eclaim_klaim b on b.no_klaim = a.no_klaim and b.kode_lokasi = a.kode_lokasi where a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_ttg = '"+this.app._kodeTtg+"' ",
										  "select count(a.no_survey) from eclaim_survey a inner join eclaim_klaim b on b.no_klaim = a.no_klaim and b.kode_lokasi = a.kode_lokasi where a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_ttg = '"+this.app._kodeTtg+"' ",
	...
	doChange: function(sender){
		try{	
			if (sender == this.ed_kode){				
				if (sender.getText() != ""){	
					var data = this.dbLib.getDataProvider("select a.*, b.penyebab, c.nama as nm_la from eclaim_survey a "+
						" inner join eclaim_klaim b on b.no_klaim = a.no_klaim and b.kode_lokasi = a.kode_lokasi and b.kode_ttg= '"+this.app._kodeTtg+"' "+
						" inner join eclaim_la c on c.kode_la = a.kode_la and c.kode_lokasi = a.kode_lokasi "+
					...
					if (typeof data != "string" && data.rs.rows[0] !== undefined){
						...
						data = this.dbLib.getDataProvider("select nama,nu from eclaim_survey_dok where no_survey = '"+sender.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' order by nu",true);
						this.sgUpld.clear();
						this.fileBfr = new portalui_arrayMap();
						var line;
