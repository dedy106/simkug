window.app_eclaim_transaksi_fVerV = function(owner)
{
	if (owner)
	{
		window.app_eclaim_transaksi_fVerV.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim_transaksi_fVerV";
		...
		this.tahun=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from eclaim_ver");
...
window.app_eclaim_transaksi_fVerV.extend(window.childForm);
window.app_eclaim_transaksi_fVerV.implement({
	...
	doEllipseClick:function(sender, col, row)
	{
		if (row == 2)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Klaim",this.sg1, this.sg1.row, this.sg1.col,
												...
												"from eclaim_klaim a "+
												"inner join eclaim_ver b on a.no_klaim=b.no_klaim and a.kode_ttg=b.kode_ttg "+filter,
		...
		if (row == 4)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Verifikasi",this.sg1, this.sg1.row, this.sg1.col,
													  "select a.no_ver,a.keterangan from eclaim_ver a "+
													  "inner join eclaim_klaim b on a.no_klaim=b.no_klaim and a.kode_ttg=b.kode_ttg "+filter,
													  "select count(a.no_ver) from eclaim_ver a "+
													  "inner join eclaim_klaim b on a.no_klaim=b.no_klaim and a.kode_ttg=b.kode_ttg "+filter,
	...
	doSelectCell:function(sender, col, row)
	{
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from eclaim_ver order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from eclaim_ver order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	mainButtonClick:function(sender)
	{
		try
		{
			...
			else
			{
				...
				var sql="select a.no_ver,a.no_klaim,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1mp.appendData([line.no_ver,line.no_klaim,line.tanggal, line.no_dokumen, line.keterangan,"Preview"]);
	...
	doSgClick:function(sender, col, row){	
		...
		if (col==5){
			try{
				var data = this.dbLib.getDataProvider("select no_file as nm from eclaim_ver_dok "+
						"where no_ver='"+this.sg1mp.getCell(0,row)+"' and kode_ttg = '"+this.app._kodeTtg+"'  ",true);
				this.sg1mp2.clear();
