window.app_eclaim_transaksi_fBayar = function(owner) {
	if (owner){
		window.app_eclaim_transaksi_fBayar.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim_transaksi_fBayar";		
		...
		this.l_tgl = new portalui_label(this,{bound:[20,1,100,18],caption:"Tanggal",underline:true});
		this.dp_tgl = new portalui_datePicker(this,{bound:[120,1,100,18], selectDate:[this,"doSelectedDate"]});
		this.ed_kode = new portalui_saiLabelEdit(this,{bound:[20,2,200,20],caption:"No Bayar",readOnly:true});
...
window.app_eclaim_transaksi_fBayar.extend(window.portalui_childForm);
window.app_eclaim_transaksi_fBayar.implement({
	...
	doFindBtnClick: function(sender){
		if (sender == this.cb_adjust){
			this.standarLib.showListData(this, "Data Adjustment",sender,undefined, 
										  "select no_adjust, keterangan from eclaim_adjust where kode_lokasi = '"+this.app._lokasi+"' and no_klaim='"+this.cb_klaim.getText()+"' and status='1' and nik_user = '"+this.app._userLog+"'  ",
										  "select count(no_adjust) from eclaim_adjust where kode_lokasi = '"+this.app._lokasi+"' and no_klaim='"+this.cb_klaim.getText()+"' and status='1' and nik_user = '"+this.app._userLog+"'  ",
		...
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, penyebab from eclaim_klaim where  kode_ttg='"+this.app._kodeTtg+"' and progress='4'  ",
										  "select count(no_klaim) from eclaim_klaim where  kode_ttg='"+this.app._kodeTtg+"' and progress='4' ",
	...
	doChange: function(sender){
		...
		this.ed_skode.clear();
		if (sender.getText() != ""){	
			var data = this.dbLib.getDataProvider("select * from eclaim_bayar where kode_ttg = '"+this.app._kodeTtg+"' and no_survey = '"+sender.getText()+"' ",true);
