window.app_eclaim_transaksi_fDokumen = function(owner) {
	if (owner){
		window.app_eclaim_transaksi_fDokumen.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim_transaksi_fDokumen";		
		...
		this.ed_kode = new portalui_saiLabelEdit(this,{bound:[20,2,200,20],caption:"No Dokumen",readOnly:true, maxLength:45});
...
window.app_eclaim_transaksi_fDokumen.extend(window.portalui_childForm);
window.app_eclaim_transaksi_fDokumen.implement({
	...
	doFindBtnClick: function(sender){
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, penyebab from eclaim_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and status='0' and status='0' ",//
										  "select count(no_klaim) from eclaim_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and status='0' and status='0' ",//"+this.app._userLog+"
