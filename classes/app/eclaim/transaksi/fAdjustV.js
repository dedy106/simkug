window.app_eclaim_transaksi_fAdjustV = function(owner)
{
	if (owner)
	{
		window.app_eclaim_transaksi_fAdjustV.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim_transaksi_fAdjustV";
		...
		this.pc1 = new pageControl(this,{bound:[15,12,1000,500], childPage:["Filter","Data Adjustment","Data Klaim","Data Riwayat Klaim","History Adjustment"]});
		...
		this.tahun=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from eclaim_adjust");
		...
		this.gridLib.SGEditData(this.sg1,4,new Array(0,1), new Array("No Adjustment","All"));
		
		this.sg1mp = new saiGrid(this.pc1.childPage[1],{bound:[1,20,this.pc1.width - 4,this.pc1.height - 180],colCount:10,
					colTitle:["No. Adjustment","No. Klaim","Tanggal","No. Dokumen","Keterangan","Nilai Estimasi","Nilai Adjust","Nilai DDCT","File Dok"," "],
					colWidth:[[9,8,7,6,5,4,3,2,1,0],[0,60,80,80,80,200,100,60,100,100]],
					selectCell:[this,"doSgClick"], 
					colAlign:[[5,6,7,8,9],[alRight,alRight,alRight,alCenter,alCenter]],
					colFormat:[[5,6,7,8,9],[cfNilai,cfNilai,cfNilai,cfButton,cfButton]],readOnly:true});
		...
		this.sg1mp2 = new saiGrid(this.pc1.childPage[1],{bound:[1,this.pc1.height - 160,this.pc1.width - 4, 100],colCount:2,readOnly:true,colTitle:["Nama File","Download"],
		...
		this.sg3mp = new saiGrid(this.pc1.childPage[4],{bound:[1,20,this.pc1.width - 4,this.pc1.height - 180],colCount:9,
					colTitle:["No. Adjustment","No. Klaim","Tanggal","No. Dokumen","No Survey","Keterangan","Nilai Estimasi","Nilai Adjust","File Dok"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[70,100,100,100,120,80,80,80,100]],
					selectCell:[this,"doSgClick"], 
					colAlign:[[5,6,7,8],[alRight,alRight,alRight,alCenter,alCenter]],
					colFormat:[[5,6,7,8],[cfNilai,cfNilai,cfNilai,cfButton,cfButton]],readOnly:true});
        this.sgn3 = new sgNavigator(this.pc1.childPage[4],{bound:[1,this.pc1.height - 25,this.pc1.width - 4,25],buttonStyle:3, pager:[this,"doPager"], grid:this.sg1mp});
...
window.app_eclaim_transaksi_fAdjustV.extend(window.childForm);
window.app_eclaim_transaksi_fAdjustV.implement({
	...
	doEllipseClick:function(sender, col, row)
	{
		if (row == 2)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Klaim",this.sg1, this.sg1.row, this.sg1.col,
												...
												"from eclaim_klaim a "+
												"inner join eclaim_adjust b on a.no_klaim=b.no_klaim and a.kode_ttg=b.kode_ttg "+filter,
		...
		if (row == 4)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Verifikasi",this.sg1, this.sg1.row, this.sg1.col,
													  "select a.no_adjust,a.keterangan from eclaim_adjust a "+
													  "inner join eclaim_klaim b on a.no_klaim=b.no_klaim and a.kode_ttg=b.kode_ttg "+filter,
													  "select count(a.no_adjust) from eclaim_adjust a "+
													  "inner join eclaim_klaim b on a.no_klaim=b.no_klaim and a.kode_ttg=b.kode_ttg "+filter,
													  new Array("a.no_adjust","a.keterangan"),"and",new Array("kode","nama"));
		}
	...
	doSelectCell:function(sender, col, row)
	{
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from eclaim_adjust order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from eclaim_adjust order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
		...
		try
		{
			...
			else
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
					  ...
					  this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and") +
					  this.filterRep.filterStr("a.no_klaim",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
					  this.filterRep.filterStr("a.no_adjust",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				...
				var sql="select a.no_adjust,a.no_klaim,c.nama as nama_ttg,a.no_survey,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
						"a.no_dokumen,a.keterangan,b.nilai,a.nilai as nilai_adjust,a.nilai_dp,a.nilai_ddct	   "+                         
						"from eclaim_adjust a "+
						"inner join eclaim_klaim b on a.no_klaim=b.no_klaim and a.kode_ttg=b.kode_ttg "+
						"inner join eclaim_ttg c on b.kode_ttg=c.kode_ttg "+
						"inner join eclaim_survey d on a.no_survey=d.no_survey and a.kode_ttg=d.kode_ttg "+this.filter+user_lok+
	...
	doTampilData2: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1mp.appendData([line.no_adjust,line.no_klaim,  line.tanggal, line.no_dokumen, line.keterangan, floatToNilai(line.nilai), floatToNilai(line.nilai_adjust), floatToNilai(line.nilai_ddct),"View","View"]);
			this.sg1mp.rows.get(i).setCellColor(8,"#ff9900");
			this.sg1mp.rows.get(i).setCellColor(9,"#ff9900");
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU.rs.rows[i];							
			this.sg3mp.appendData([line.no_adjust,line.no_klaim,  line.tanggal, line.no_dokumen, line.keterangan, floatToNilai(line.nilai), floatToNilai(line.nilai_adjust),floatToNilai(line.nilai_ddct),"View","View"]);
			this.sg3mp.rows.get(i).setCellColor(8,"#ff9900");
			this.sg3mp.rows.get(i).setHint("Click untuk menampilkan detail klaim");
		}
		this.sg3mp.setNoUrut(start);
	},
	...
	doSgClick:function(sender, col, row){	
		...
		if (col<8){
			try{
				...
				this.filter2 = "";
				this.docViewer.src = this.report.previewWithHeader("server_report_eclaim_rptRwyKlaim",this.filter, 1,1, this.showFilter, this.lokasi,this.filter2);
				this.docViewer2.src = this.report.previewWithHeader("server_report_eclaim_rptRwyKlaimSum",this.filter, 1,1, this.showFilter, this.lokasi,this.filter2);
		...
		if (col==8){
			try{
				var data = this.dbLib.getDataProvider("select nama as nm from eclaim_adjust_dok "+
						"where no_adjust='"+this.sg1mp.getCell(0,row)+"' and kode_ttg = '"+this.app._kodeTtg+"'  ",true);
				this.sg1mp2.clear();
		...
		if (col==9){
			try{
				var sql="select a.no_adjust,a.no_klaim,c.nama as nama_ttg,a.no_survey,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
						"a.no_dokumen,a.keterangan,b.nilai,a.nilai as nilai_adjust,a.nilai_dp	   "+                         
						"from eclaim_adjust a "+
						"inner join eclaim_klaim b on a.no_klaim=b.no_klaim and a.kode_ttg=b.kode_ttg "+
						"inner join eclaim_ttg c on b.kode_ttg=c.kode_ttg "+
						"inner join eclaim_survey d on a.no_survey=d.no_survey and a.kode_ttg=d.kode_ttg "+
						"where a.no_klaim='"+this.sg1mp.cells(1,row)+"'";
