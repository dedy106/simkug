window.app_eclaim_transaksi_fDokumenV = function(owner)
{
	if (owner)
	{
		window.app_eclaim_transaksi_fDokumenV.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim_transaksi_fDokumenV";
		...
		this.tahun=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from eclaim_dok_m");
...
window.app_eclaim_transaksi_fDokumenV.extend(window.childForm);
window.app_eclaim_transaksi_fDokumenV.implement({
	...
	doEllipseClick:function(sender, col, row)
	{
		if (row == 2)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Klaim",this.sg1, this.sg1.row, this.sg1.col,
												...
												"from eclaim_klaim a "+
												"inner join eclaim_dok_m b on a.no_klaim=b.no_klaim and a.kode_ttg=b.kode_ttg "+filter,
		...
		if (row == 4)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Verifikasi",this.sg1, this.sg1.row, this.sg1.col,
													  "select a.no_dok,a.keterangan from eclaim_dok_m a "+
													  "inner join eclaim_klaim b on a.no_klaim=b.no_klaim  and a.kode_ttg=b.kode_ttg "+filter,
													  "select count(a.no_dok) from eclaim_dok_m a "+
													  "inner join eclaim_klaim b on a.no_klaim=b.no_klaim  and a.kode_ttg=b.kode_ttg "+filter,
	...
	doSelectCell:function(sender, col, row)
	{
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from eclaim_dok_m order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from eclaim_dok_m order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
		...
		try
		{
			...
			else
			{
				...
				var sql="select a.no_dok,a.no_klaim,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
						...
						"left join (select a.no_dok,a.kode_ttg,count(a.no_dok) as jml_dok "+
						"			from eclaim_dok_d a "+
						"			group by a.no_dok,a.kode_ttg "+
						"			)c on a.no_dok=c.no_dok  and a.kode_ttg=b.kode_ttg "+this.filter+user_lok+
	...
	doSgClick:function(sender, col, row){	
		...
		if (col==5){
			try{
				...
				var sql="select a.kode_ref,d.nama as nama_ref,a.ket_dok,a.no_file "+
						"from eclaim_dok_d a "+
						...
						"where a.no_dok='"+this.sg1mp.getCell(0,row)+"' and a.kode_ttg='"+this.app._kodeTtg+"'"+
						"order by a.kode_ref ";
				var data = this.dbLib.getDataProvider(sql,true);
				this.sg1mp2.clear();
