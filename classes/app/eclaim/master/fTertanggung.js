window.app_eclaim_master_fTertanggung = function(owner) {
	if (owner){
		window.app_eclaim_master_fTertanggung.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim_master_fTertanggung";		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Tertanggung", 0);	
		
		uses("portalui_checkBox;util_file;portalui_saiCBBL;portalui_uploader");
		this.ed_kode = new portalui_saiCBBL(this,{bound:[20,1,200,20], caption:"Kode ",rightLabelVisible:false, btnClick:[this,"doFindBtnClick"],change:[this,"doChange"]});
		...
		this.ed_file = new portalui_saiLabelEdit(this,{bound:[20,4,300,20],caption:"File Logo", readOnly:true, tag:1});
		this.up_file = new portalui_uploader(this,{bound:[330,4,80,20],param1:"uploadTo",param2:"server/media/tmp/",param3:"object",param4:"server/media/",autoSubmit:true,afterUpload:[this,"doAfterLoad"], tag:1});
...
window.app_eclaim_master_fTertanggung.extend(window.portalui_childForm);
window.app_eclaim_master_fTertanggung.implement({
	...
	doModalResult: function(event, result){				
		try{			
			...
			switch(event){
				...
				break;
				case "simpan" :
					sql.add("insert into eclaim_ttg (kode_ttg, nama, alamat, kota, kodepos, no_telp, no_fax, pic, email, web, npwp, skode, status,kode_lokasi, gambar, tgl_input, nik_user)values"+
				...
				break;
				case "ubah" :
					sql.add("update eclaim_ttg set nama = '"+this.ed_nama.getText()+"', skode='"+this.ed_skode.getText()+"' "+
				...
				break;
				case "hapus" :
					sql.add("delete from eclaim_ttg where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg ='"+this.ed_kode.getText()+"'  ");
				break;
			}			
	...
	doChange: function(sender){		
		if (sender.getText() != ""){	
			var kode = this.dbLib.getDataProvider("select nama, alamat, kota, kodepos, no_telp, no_fax, pic, email, web, npwp, skode, gambar,status from eclaim_ttg where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg = '"+sender.getText()+"' ",true);
	...
	doClick: function(sender){
		this.ed_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'eclaim_ttg','kode_ttg',"TG",'000'));
	},	
