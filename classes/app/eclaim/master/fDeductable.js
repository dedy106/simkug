window.app_eclaim_master_fDeductable = function(owner){
	if (owner)
	{
		window.app_eclaim_master_fDeductable.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim_master_fDeductable";
		this.setTop(60);
		this.setWidth(1280);
		this.setHeight(550);
		...
		this.cb_ttg = new portalui_saiCBBL(this,{bound:[20,10,185,20],caption:"Tertanggung", change:[this,"doChange"]});										
		this.bGen = new portalui_button(this,{bound:[620,11,80,20],caption:"Tampil Data",click:"doClick"});
		this.p1 = new portalui_panel(this,{bound:[20,12,700,400],caption:"Jenis Asuransi"});						
		this.sg1 = new portalui_saiGrid(this.p1,{bound:[1,20,698,350],colCount:8,
			...
			picklist:[[6],[new portalui_arrayMap({items:["A","B","C"]})]]});	
		this.sgn = new portalui_sgNavigator(this.p1,{bound:[1,373,698,25],buttonStyle:bsTrans, grid:this.sg1});							
		...
		{			
			...
			this.dbLib.getDataProviderA("select kode_asuransi, nama from eclaim_asuransi where kode_lokasi = '"+this.app._lokasi+"' ");
		}catch(e){
			systemAPI.alert(e);
		}
	}
};
window.app_eclaim_master_fDeductable.extend(window.portalui_childForm);
window.app_eclaim_master_fDeductable.implement({
		...
		switch (event)
		{
			...
			case "simpan" :
				try{
					...
					sql.add("delete from eclaim_ddct where kode_ttg = '"+this.cb_ttg.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					for (var i=0; i < this.sg1.rows.getLength(); i++){
						if (this.sg1.rowValid(i))
							sql.add("insert into eclaim_ddct(kode_ttg,kode_asuransi,no_polis, kode_obyek, kode_sebab, kode_curr, jenis, nilai,kode_lokasi,tgl_input, nik_user) values "+
			...
			case "hapus":
				uses("server_util_arrayList");
				sql = new server_util_arrayList();
				sql.add("delete from eclaim_ddct where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg = '"+this.cb_ttg.getText()+"' ");
				this.dbLib.execArraySQL(sql);	
			break;
		}
	...
	doClick: function(sender){
		try 
		{
			this.sg1.clear(); 
			setTipeButton(tbSimpan);
			var data = this.dbLib.getDataProvider(" select a.kode_asuransi,b.nama,  a.no_polis, a.kode_obyek , a.kode_sebab, a.kode_curr, a.jenis, nilai from eclaim_ddct a "+
				"	inner join eclaim_asuransi b on a.kode_asuransi =b.kode_asuransi and a.kode_lokasi=b.kode_lokasi "+
	...
	doFindBtnClick: function(sender, col, row){
			...
			switch(col)
			{
				case 0 : 
					this.standarLib.showListDataForSG(this, "Daftar Jenis Asuransi",sender, sender.row, sender.col, 
													  ...
													  " inner join eclaim_asuransi_ttg b on b.kode_asuransi = a.kode_asuransi and b.kode_lokasi = a.kode_lokasi"+
				...
				case 3 : 
					this.standarLib.ListDataSGFilter(this, "Daftar Obyek",sender, sender.row, sender.col, 
													  "select kode_obyek, nama  from eclaim_obyek where kode_lokasi='"+this.app._lokasi+"' and kode_ttg = '"+this.app._kodeTtg+"' ",
													  "select count(kode_obyek) from eclaim_obyek where kode_lokasi='"+this.app._lokasi+"' and kode_ttg = '"+this.app._kodeTtg+"'",
				...
				case 4 : 
					this.standarLib.ListDataSGFilter(this, "Daftar Penyebab",sender, sender.row, sender.col, 
													  "select kode_sebab, nama  from eclaim_sebab where kode_lokasi='"+this.app._lokasi+"' and kode_ttg = '"+this.app._kodeTtg+"' ",
													  "select count(kode_sebab) from eclaim_sebab where kode_lokasi='"+this.app._lokasi+"' and kode_ttg = '"+this.app._kodeTtg+"' ",
		...
		{
			systemAPI.alert("[app_eclaim_master_fAsuransiTtg] : doFindBtnClick : " + e);
		}	
	},
			...
			switch(methodName)
				{
					...
					case "getDataProvider":
						...
						this.dataAsuransi = new portalui_arrayMap();
