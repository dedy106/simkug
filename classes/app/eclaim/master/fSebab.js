window.app_eclaim_master_fSebab = function(owner)
{
	if (owner){
		window.app_eclaim_master_fSebab.prototype.parent.constructor.call(this, owner);
		this.className = "app_eclaim_master_fSebab";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Penyebab Kerugian", 0);	
...
window.app_eclaim_master_fSebab.extend(window.portalui_childForm);
window.app_eclaim_master_fSebab.implement({
			...
			switch (event){
				...
				case "simpan" :
					...
					try{
						...
						sql.add("delete from eclaim_sebab where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg = '"+this.cb_ttg.getText()+"' and kode_asuransi = '"+this.cb_ja.getText()+"' ");						
						var data = value.replace(/#/g,",");
						sql2 = "insert into eclaim_sebab(kode_sebab, kode_ttg, kode_asuransi, kode_lokasi,nama, level_lap,level_spasi, tipe, sum_header, kode_induk, rowindex, nik_user, tgl_input) values ";
						sql2 += " "+ data +" ";
						sql.add(sql2);						
	...
	doClick: function(sender){	
		var nrc = this.dblib.runSQL("select kode_sebab, kode_ttg, kode_asuransi, kode_lokasi,nama, level_lap,level_spasi, tipe, sum_header, kode_induk, rowindex, nik_user, tgl_input from eclaim_sebab where  kode_lokasi = '"+this.app._lokasi+"' order by rowindex",0,0);	
		this.loadMenu(nrc);	
	},	
	entriesClick: function(sender){
		...
		{ 
			if (sender == this.reloadBtn){				
				var nrc = this.dblib.getDataProvider("select kode_sebab, kode_ttg, kode_asuransi, kode_lokasi,nama, level_lap,level_spasi, tipe, sum_header, kode_induk, rowindex, nik_user, tgl_input from eclaim_sebab where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg = '"+this.cb_ttg.getText()+"' and kode_asuransi ='"+this.cb_ja.getText()+"' order by rowindex",true);
				this.loadMenu(nrc);		
				...
				if (sender == this.addBtn || sender == this.editBtn){
					if (this.frameAdd == undefined){
						this.frameAdd = new app_eclaimg_master_fFrameSebab(this.app, this);						
					}
				}
	...
	copyChilds : function(from, to){
		...
		{			
			...
			chld.data.kode_sebab = temp.getKode();					
	...
	switchItem : function(from, to){
		...
		child.data.kode_sebab = item.getKode();
...
//------------------------------------- Frame
window.app_eclaimg_master_fFrameSebab = function(owner,requester){
	if (owner){
		window.app_eclaimg_master_fFrameSebab.prototype.parent.constructor.call(this, owner);
		this.setHeight(150);
		this.setWidth(400);
...
window.app_eclaimg_master_fFrameSebab.extend(window.portalui_commonForm);
window.app_eclaimg_master_fFrameSebab.implement({
	...
	doSysMouseMove: function(x, y, button, buttonState){
		window.app_eclaimg_master_fFrameSebab.prototype.parent.doSysMouseMove.call(this,x, y, button, buttonState);
		if (this.isClick)
		{		
