window.app_eclaim_master_fObjek = function(owner)
{
	if (owner){
		window.app_eclaim_master_fObjek.prototype.parent.constructor.call(this, owner);
		this.className = "app_eclaim_master_fObjek";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Obyek Kerugian", 0);	
...
window.app_eclaim_master_fObjek.extend(window.portalui_childForm);
window.app_eclaim_master_fObjek.implement({
			...
			switch (event){
				...
				case "simpan" :
					...
					try{
						...
						sql.add("delete from eclaim_obyek where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg = '"+this.cb_ttg.getText()+"' and kode_asuransi = '"+this.cb_ja.getText()+"' ");						
						var data = value.replace(/#/g,",");
						sql2 = "insert into eclaim_obyek(kode_obyek,kode_ttg, kode_asuransi, kode_lokasi,nama, level_spasi,level_lap, tipe, sum_header, kode_induk, tgl_input, nik_user, rowindex) values ";
						sql2 += " "+ data +" ";
						sql.add(sql2);						
	...
	doClick: function(sender){	
		var nrc = this.dblib.runSQL("select kode_obyek,kode_ttg, kode_asuransi, kode_lokasi,nama, level_spasi,level_lap, tipe, sum_header, kode_induk, tgl_input, nik_user, rowindex from eclaim_obyek where  kode_lokasi = '"+this.app._lokasi+"' order by rowindex",0,0);	
		this.loadMenu(nrc);	
	},	
	entriesClick: function(sender){
		...
		{ 
			if (sender == this.reloadBtn){				
				var nrc = this.dblib.getDataProvider("select kode_obyek,kode_ttg, kode_asuransi, kode_lokasi,nama, level_spasi,level_lap, tipe, sum_header, kode_induk, tgl_input, nik_user, rowindex from eclaim_obyek where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg = '"+this.cb_ttg.getText()+"' and kode_asuransi ='"+this.cb_ja.getText()+"' order by rowindex",true);
				this.loadMenu(nrc);		
				...
				if (sender == this.addBtn || sender == this.editBtn){
					if (this.frameAdd == undefined){
						this.frameAdd = new app_eclaimg_master_fFrameObyek(this.app,this);						
					}
				}
	...
	copyChilds : function(from, to){
		...
		{			
			...
			chld.data.kode_obyek = temp.getKode();					
	...
	switchItem : function(from, to){
		...
		child.data.kode_obyek = item.getKode();
		...
		try
		{
			...
			if (sender == this.cb_ja) 
			{
				this.standarLib.showListData(this, "Data Asuransi",sender,undefined, 
											  ...
											  " inner join eclaim_asuransi_ttg b on b.kode_asuransi = a.kode_asuransi and b.kode_lokasi = a.kode_lokasi where a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_ttg = '"+this.app._kodeTtg+"'",
											  new Array("a.kode_asuransi","a.nama"),"where",new Array("Kode Asuransi","Deskripsi"),false);				
			}
		}catch(e){
			system.alert(this,e,"");
		}
...
//------------------------------------- Frame
window.app_eclaimg_master_fFrameObyek = function(owner,requester){
	if (owner){
		window.app_eclaimg_master_fFrameObyek.prototype.parent.constructor.call(this, owner);
		this.setHeight(150);
		this.setWidth(400);
...
window.app_eclaimg_master_fFrameObyek.extend(window.portalui_commonForm);
window.app_eclaimg_master_fFrameObyek.implement({
	...
	doSysMouseMove: function(x, y, button, buttonState){
		window.app_eclaimg_master_fFrameObyek.prototype.parent.doSysMouseMove.call(this,x, y, button, buttonState);
		if (this.isClick)
		{		
