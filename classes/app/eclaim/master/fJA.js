window.app_eclaim_master_fJA = function(owner) {
	if (owner){
		window.app_eclaim_master_fJA.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim_master_fJA";		
...
window.app_eclaim_master_fJA.extend(window.portalui_childForm);
window.app_eclaim_master_fJA.implement({
	...
	doModalResult: function(event, result){				
		try{
			...
			switch(event){
				case "simpan" :
					sql.add("insert into eclaim_asuransi (kode_asuransi, nama, skode,kode_lokasi, tgl_input, nik_user)values"+
						"('"+this.ed_kode.getText()+"','"+this.ed_nama.getText()+"','"+this.ed_skode.getText()+"','"+this.app._lokasi+"',now(),'"+this.app._userLog+"') ");
				break;
				case "ubah" :
					sql.add("update eclaim_asuransi set nama = '"+this.ed_nama.getText()+"',skode='"+this.ed_skode.getText()+"' where kode_lokasi = '"+this.app._lokasi+"' and kode_asuransi ='"+this.ed_kode.getText()+"'  ");
				break;
				case "hapus" :
					sql.add("delete from eclaim_asuransi where kode_lokasi = '"+this.app._lokasi+"' and kode_asuransi ='"+this.ed_kode.getText()+"'");
				break;
			}
	...
	doChange: function(sender){
		...
		this.ed_skode.clear();
		if (sender.getText() != ""){
			var kode = this.dbLib.getDataProvider("select nama,skode from eclaim_asuransi where kode_lokasi = '"+this.app._lokasi+"' and kode_asuransi = '"+sender.getText()+"' ",true);
	...
	doClick: function(sender){
		this.ed_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'eclaim_asuransi','kode_asuransi',this.app._lokasi+"-JA.",'000'));
	},
