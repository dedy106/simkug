window.app_eclaim_master_fAsuransiTtg = function(owner){
	if (owner)
	{
		window.app_eclaim_master_fAsuransiTtg.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim_master_fAsuransiTtg";
		this.setTop(60);
		this.setWidth(1280);
		this.setHeight(550);
		...
		this.cb_ttg = new portalui_saiCBBL(this,{bound:[20,10,185,20],caption:"Tertanggung"});										
		this.bGen = new portalui_button(this,{bound:[420,11,80,20],caption:"Tampil Data",click:"doClick"});
		this.p1 = new portalui_panel(this,{bound:[20,12,500,400],caption:"Jenis Asuransi"});						
		this.sg1 = new portalui_saiGrid(this.p1,{bound:[1,20,498,350],colCount:2,colTitle:["Kode Asuransi","Deskripsi"],
			colWidth:[[1,0],[365,100]],buttonStyle:[[0],[bsEllips]], colReadOnly:[true,[1],[]]});		
		this.sgn = new portalui_sgNavigator(this.p1,{bound:[1,373,498,25],buttonStyle:bsTrans, grid:this.sg1});								
...
window.app_eclaim_master_fAsuransiTtg.extend(window.portalui_childForm);
window.app_eclaim_master_fAsuransiTtg.implement({
			...
			case "simpan" :
				if (modalResult == mrOk)
				{
					try
					{
						...
						sql.add("delete from eclaim_asuransi_ttg where kode_ttg = '"+this.cb_ttg.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						for (var i=0; i < this.sg1.rows.getLength(); i++){
							if (this.sg1.rowValid(i))
								sql.add("insert into eclaim_asuransi_ttg (kode_ttg,kode_asuransi,nu,kode_lokasi) values ('"+this.cb_ttg.getText()+"','"+this.sg1.getCell(0,i)+"','"+i+"','"+this.app._lokasi+"')");
						}
	...
	genClick: function(sender){
		try 
		{
			this.sg1.clear(); 
			setTipeButton(tbSimpan);
			var data = this.dbLib.getDataProvider(" select a.kode_asuransi,a.nama from eclaim_asuransi a "+
				"	inner join eclaim_asuransi_ttg b on a.kode_asuransi =b.kode_asuransi and a.kode_lokasi=b.kode_lokasi "+
