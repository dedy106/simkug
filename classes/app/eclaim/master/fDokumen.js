window.app_eclaim_master_fDokumen = function(owner)
{
	if (owner){
		window.app_eclaim_master_fDokumen.prototype.parent.constructor.call(this, owner);
		this.className = "app_eclaim_master_fDokumen";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Referensi Dokumen", 0);	
		
		this.maximize();
		uses("portalui_saiCBBL");
		this.cb_ttg = new portalui_saiCBBL(this,{bound:[20,20,200,20],caption:"Tertanggung",btnClick:[this,"FindBtnClick"]});
		this.cb_ja = new portalui_saiCBBL(this,{bound:[20,30,200,20],caption:"Jenis Asuransi",btnClick:[this,"FindBtnClick"]});		
		...
		this.standarLib.clearByTag(this, [0], this.cb_ttg);		
		this.cb_ttg.setSQL("select kode_ttg, nama  from eclaim_ttg where kode_lokasi = '"+this.app._lokasi+"' ",["kode_ttg","nama"]);
...
window.app_eclaim_master_fDokumen.extend(window.portalui_childForm);
window.app_eclaim_master_fDokumen.implement({
			...
			switch (event){
				case "Add" :				   
					try{
						...
						var data = {};										
						data.kode_ref = value.kode;
						data.kode_ttg = this.cb_ttg.getText();
						data.kode_asuransi = this.cb_ja.getText();
				...
				case "simpan" :
					...
					try{
						...
						sql.add("delete from eclaim_ref where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg = '"+this.cb_ttg.getText()+"' and kode_asuransi = '"+this.cb_ja.getText()+"' ");												
						var data = value.replace(/#/g,",");
						sql2 = "insert into eclaim_ref(kode_ref,kode_ttg, kode_asuransi, kode_lokasi,nama, level_lap,level_spasi, tipe, sum_header, kode_induk, rowindex, nik_user, tgl_input) values ";
						sql2 += " "+ data +" ";
						sql.add(sql2);						
	...
	doClick: function(sender){	
		var nrc = this.dblib.runSQL("select kode_ref,kode_ttg, kode_asuransi, kode_lokasi,nama, level_lap,level_spasi, tipe, sum_header, kode_induk, rowindex, nik_user, tgl_input from eclaim_ref where  kode_lokasi = '"+this.app._lokasi+"' order by rowindex",0,0);	
		this.loadMenu(nrc);	
	},	
	entriesClick: function(sender){
		...
		{ 
			if (sender == this.reloadBtn){				
				var nrc = this.dblib.getDataProvider("select kode_ref,kode_ttg, kode_asuransi, kode_lokasi,nama, level_lap,level_spasi, tipe, sum_header, kode_induk, rowindex, nik_user, tgl_input from eclaim_ref where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg = '"+this.cb_ttg.getText()+"' and kode_asuransi ='"+this.cb_ja.getText()+"' order by rowindex",true);
				this.loadMenu(nrc);		
				...
				if (sender == this.addBtn || sender == this.editBtn){
					if (this.frameAdd == undefined){
						this.frameAdd = new app_eclaimg_master_fFrameDok(this.app, this);						
					}
				}
	...
	copyChilds : function(from, to){
		...
		{			
			...
			chld.data.kode_ref = temp.getKode();					
	...
	switchItem : function(from, to){
		...
		child.data.kode_ref = item.getKode();
	...
	FindBtnClick: function(sender, event){
		try
		{
			...
			if (sender == this.cb_ja) 
			{
				this.standarLib.showListData(this, "Data Asuransi",sender,undefined, 
											  ...
											  " inner join eclaim_asuransi_ttg b on b.kode_asuransi = a.kode_asuransi and b.kode_lokasi = a.kode_lokasi where a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_ttg = '"+this.app._kodeTtg+"'",
											  new Array("kode_asuransi","nama"),"where",new Array("Kode Asuransi","Deskripsi"),false);				
			}
		}catch(e){
			system.alert(this,e,"");
		}
...
//------------------------------------- Frame
window.app_eclaimg_master_fFrameDok = function(owner,requester){
	if (owner){
		window.app_eclaimg_master_fFrameDok.prototype.parent.constructor.call(this, owner);
		this.setHeight(150);
		this.setWidth(400);
		...
		this.cancelBtn = new portalui_imageButton(this,{bound:[42,90,22,22],image:"icon/"+system.getThemes()+"/bCancel.png",hint:"Cancel",click:"doClick"});				
		system.addMouseListener(this);
		this.formRequester = requester;
	}
};
window.app_eclaimg_master_fFrameDok.extend(window.portalui_commonForm);
window.app_eclaimg_master_fFrameDok.implement({
	...
	doSysMouseMove: function(x, y, button, buttonState){
		window.app_eclaimg_master_fFrameDok.prototype.parent.doSysMouseMove.call(this,x, y, button, buttonState);
		if (this.isClick)
		{		
