window.app_eclaim_report_flAdjust = function(owner)
{
	if (owner)
	{
		window.app_eclaim_report_flAdjust.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim_report_flAdjust";
		this.maximize();
		this.app._mainForm.childFormConfig(this,"mainButtonClick","Laporan Adjustment",2);
		uses("portalui_panel");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1), new Array("No Survey","All"));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1), new Array("No Adjust","All"));
...
window.app_eclaim_report_flAdjust.extend(window.portalui_childForm);
window.app_eclaim_report_flAdjust.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 1)
		{
...
window.app_eclaim_report_flAdjust.prototype.doSelectCell = function(sender, col, row)
{
	this.sg1.columns.get(2).setReadOnly(false);
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from eclaim_adjust where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
};
window.app_eclaim_report_flAdjust.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.filter = this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
				  ...
				  this.filterRep.filterStr("a.no_survey",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
				  this.filterRep.filterStr("a.no_adjust",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+user_lok;
			this.showFilter = this.filterRep.showFilter(this.sg1);
			this.sql =  "select a.no_adjust,a.no_klaim,c.nama as nama_ttg,a.no_survey,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
						"a.no_dokumen,a.keterangan,b.kode_curr,b.nilai,a.nilai as nilai_adjust,a.nilai_dp	   "+                         
						"from eclaim_adjust a "+
						"inner join eclaim_klaim b on a.no_klaim=b.no_klaim "+
						"inner join eclaim_ttg c on b.kode_ttg=c.kode_ttg "+
						"inner join eclaim_survey d on a.no_survey=d.no_survey "+
						this.filter+" order by a.no_adjust ";
			this.scriptSqlCount =   "select count(a.no_adjust)	   "+                         
						"from eclaim_adjust a "+
						"inner join eclaim_klaim b on a.no_klaim=b.no_klaim "+
						"inner join eclaim_ttg c on b.kode_ttg=c.kode_ttg "+
						"inner join eclaim_survey d on a.no_survey=d.no_survey "+this.filter;
			
			var title = new server_util_arrayList();			
			var width = new server_util_arrayList();
			var fieldType = new server_util_arrayList();
			title.add("No Adjust");width.add(100);fieldType.add("S");		
			title.add("No Klaim");width.add(100);fieldType.add("S");		
			title.add("Tertanggung");width.add(150);fieldType.add("S");		
			title.add("No Survey");width.add(150);fieldType.add("S");		
			...
			title.add("Nilai Adv Pay");width.add(80);fieldType.add("N");	
	...
	{
		alert("[app_eclaim_report_flAdjust]::mainButtonClick:"+e);
	}
};
window.app_eclaim_report_flAdjust.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_eclaim_report_flAdjust.prototype.getStringHeader = function(){
...
window.app_eclaim_report_flAdjust.prototype.doConfirmClick = function(sender){
	try{
		if (sender === sender.owner.bConfirm){
			...
			if (to !== ""){
				sender.owner.free();
				var d = new Date();
				var subject = "Laporan Adjustment "+d.toLocaleString();
...
window.app_eclaim_report_flAdjust.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_eclaim_report_flAdjust.prototype.previewReport = function(dthtml)
{
	var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN ADJUSTMENT<br>";			
	var d = new Date();
...
window.app_eclaim_report_flAdjust.prototype.doCloseReportClick = function(sender)
{
...
window.app_eclaim_report_flAdjust.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_eclaim_report_flAdjust.prototype.doRowPerPageChange = function(sender, rowperpage)
{
