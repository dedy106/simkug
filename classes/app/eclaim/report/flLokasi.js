window.app_eclaim_report_flLokasi = function(owner)
{
	if (owner)
	{
		window.app_eclaim_report_flLokasi.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim_report_flLokasi";
		this.maximize();
...
window.app_eclaim_report_flLokasi.extend(window.portalui_childForm);
window.app_eclaim_report_flLokasi.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 0)
	{
	...
	if (row == 1)
	{
		this.filterRep.ListDataSGFilter(this, "Data Lokasi",this.sg1, this.sg1.row, this.sg1.col,
											  ...
											  "select count(kode_lok) from eclaim_lokasi where kode_lokasi='"+this.app._lokasi+"'",
											  new Array("kode_lok","nama"),"and",new Array("kode","nama"));
	}	
};
window.app_eclaim_report_flLokasi.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_eclaim_report_flLokasi.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.filter = this.filterRep.filterStr("a.kode_ttg",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
						this.filterRep.filterStr("a.kode_lok",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,2),"and") +
						this.filterRep.filterStr("a.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"and");
			this.showFilter = this.filterRep.showFilter(this.sg1);
			this.sql =  "select c.nama as nama_ttg,a.kode_lok,a.nama "+
						"from eclaim_lokasi a "+
						"inner join eclaim_ttg c on a.kode_ttg=c.kode_ttg "+this.filter+
						" order by a.kode_lok ";
			this.scriptSqlCount =  "select count(a.kode_lok) "+
						"from eclaim_lokasi a "+
						"inner join eclaim_ttg c on a.kode_ttg=c.kode_ttg "+this.filter;
	...
	{
		alert("[app_eclaim_report_flLokasi]::mainButtonClick:"+e);
	}
};
window.app_eclaim_report_flLokasi.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_eclaim_report_flLokasi.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_eclaim_report_flLokasi.prototype.previewReport = function(dthtml)
{
	var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN LOKASI<br>";			
	var d = new Date();
...
window.app_eclaim_report_flLokasi.prototype.doCloseReportClick = function(sender)
{
...
window.app_eclaim_report_flLokasi.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_eclaim_report_flLokasi.prototype.doRowPerPageChange = function(sender, rowperpage)
{
