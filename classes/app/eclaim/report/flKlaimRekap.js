window.app_eclaim_report_flKlaimRekap = function(owner)
{
	if (owner)
	{
		window.app_eclaim_report_flKlaimRekap.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim_report_flKlaimRekap";
		this.maximize();
		this.app._mainForm.childFormConfig(this,"mainButtonClick","Laporan Rekap Progress Klaim",2);
		uses("portalui_panel");
...
window.app_eclaim_report_flKlaimRekap.extend(window.portalui_childForm);
window.app_eclaim_report_flKlaimRekap.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 1)
		{
...
window.app_eclaim_report_flKlaimRekap.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_eclaim_report_flKlaimRekap.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.filter2=this.filterRep.filterStr("a.kode_ttg",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where");
			this.showFilter = this.filterRep.showFilter(this.sg1);
			this.sql =  "select a.nama,ifnull(b.jml,0) as jml,ifnull(b.nilai_est,0) as nilai_est,ifnull(b.nilai_tuntutan,0) as nilai_tuntutan, "+
						...
						"			left join eclaim_bayar c on a.no_klaim=c.no_klaim and a.kode_ttg=c.kode_ttg and a.kode_lokasi=c.kode_lokasi "+this.filter+
						"			and a.status_dok='0'  "+
						...
						"			and a.status='1' "+
						"			group by kode_proses,a.kode_ttg "+
						")b on a.kode_proses=b.kode_proses and a.kode_ttg=b.kode_ttg "+this.filter2+
			...
			title.add("Nilai Tuntutan");width.add(100);fieldType.add("N");
			title.add("Nilai Adjust");width.add(100);fieldType.add("N");
			title.add("Nilai Bayar");width.add(100);fieldType.add("N");
	...
	{
		alert("[app_eclaim_report_flKlaimRekap]::mainButtonClick:"+e);
	}
};
window.app_eclaim_report_flKlaimRekap.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_eclaim_report_flKlaimRekap.prototype.getStringHeader = function(){
...
window.app_eclaim_report_flKlaimRekap.prototype.doConfirmClick = function(sender){
	try{
...
window.app_eclaim_report_flKlaimRekap.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_eclaim_report_flKlaimRekap.prototype.previewReport = function(dthtml)
{
	var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN REKAP PROGRES KLAIM<br>";			
	var d = new Date();
...
window.app_eclaim_report_flKlaimRekap.prototype.doCloseReportClick = function(sender)
{
...
window.app_eclaim_report_flKlaimRekap.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_eclaim_report_flKlaimRekap.prototype.doRowPerPageChange = function(sender, rowperpage)
{
