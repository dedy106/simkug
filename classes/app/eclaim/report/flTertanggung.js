window.app_eclaim_report_flTertanggung = function(owner)
{
	if (owner)
	{
		window.app_eclaim_report_flTertanggung.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim_report_flTertanggung";
		this.maximize();
...
window.app_eclaim_report_flTertanggung.extend(window.portalui_childForm);
window.app_eclaim_report_flTertanggung.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 0)
		{
...
window.app_eclaim_report_flTertanggung.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_eclaim_report_flTertanggung.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.sql =  "select kode_ttg,  nama, alamat, kota, kodepos,pic, no_telp, no_fax,  email, web "+
						"from eclaim_ttg "+this.filter+
						" order by kode_ttg ";
			this.scriptSqlCount =  "select count(kode_ttg) from eclaim_ttg "+this.filter;
	...
	{
		alert("[app_eclaim_report_flTertanggung]::mainButtonClick:"+e);
	}
};
window.app_eclaim_report_flTertanggung.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_eclaim_report_flTertanggung.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_eclaim_report_flTertanggung.prototype.previewReport = function(dthtml)
{
...
window.app_eclaim_report_flTertanggung.prototype.doCloseReportClick = function(sender)
{
...
window.app_eclaim_report_flTertanggung.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_eclaim_report_flTertanggung.prototype.doRowPerPageChange = function(sender, rowperpage)
{
