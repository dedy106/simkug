window.app_eclaim_report_flDokumen = function(owner)
{
	if (owner)
	{
		window.app_eclaim_report_flDokumen.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim_report_flDokumen";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from eclaim_dok_m where kode_lokasi='"+this.app._lokasi+"' ");
	this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Kode Tertanggung","=",this.app._kodeTtg]);
...
window.app_eclaim_report_flDokumen.extend(window.childForm);
window.app_eclaim_report_flDokumen.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 2)
			{
				...
				var sql1="select a.no_dok,a.keterangan from eclaim_dok_m a "+this.filter;
				var sql2="select count(a.no_dok) from eclaim_dok_m a "+this.filter;
				this.filterRep.ListDataSGFilter(this, "Data Approval",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,["a.no_dok","a.keterangan"],"and",["No Anggaran","Keterangan"]);
			}	
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2],["13","3","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2],[2,0,2]);
		
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct(periode) as periode from eclaim_dok_m "+
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter=this.filterRep.filterStr("a.kode_ttg",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.no_dok",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.nama_report="server_report_eclaim_rptDokumen";	
