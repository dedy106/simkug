window.app_eclaim_report_flPolis = function(owner)
{
	if (owner)
	{
		window.app_eclaim_report_flPolis.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim_report_flPolis";
		this.maximize();
...
window.app_eclaim_report_flPolis.extend(window.portalui_childForm);
window.app_eclaim_report_flPolis.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 0)
	{
...
window.app_eclaim_report_flPolis.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_eclaim_report_flPolis.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.sql =  "select d.nama as nama_ttg,a.no_polis,b.nama as nama_png,c.nama as nama_asuransi,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
						...
						"from eclaim_polis a "+
						"inner join eclaim_png b on a.kode_png=b.kode_png "+
						"inner join eclaim_asuransi c on a.kode_asuransi=c.kode_asuransi "+
						"inner join eclaim_ttg d on a.kode_ttg=d.kode_ttg "+this.filter;
	...
	{
		alert("[app_eclaim_report_flPolis]::mainButtonClick:"+e);
	}
};
window.app_eclaim_report_flPolis.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_eclaim_report_flPolis.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_eclaim_report_flPolis.prototype.previewReport = function(dthtml)
{
	var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN POLIS<br>";			
	var d = new Date();
...
window.app_eclaim_report_flPolis.prototype.doCloseReportClick = function(sender)
{
...
window.app_eclaim_report_flPolis.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_eclaim_report_flPolis.prototype.doRowPerPageChange = function(sender, rowperpage)
{
