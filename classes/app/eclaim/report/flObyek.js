window.app_eclaim_report_flObyek = function(owner)
{
	if (owner)
	{
		window.app_eclaim_report_flObyek.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim_report_flObyek";
		this.maximize();
...
window.app_eclaim_report_flObyek.extend(window.portalui_childForm);
window.app_eclaim_report_flObyek.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 0)
	{
	...
	if (row == 2)
	{
		this.filterRep.ListDataSGFilter(this, "Data Obyek Kerugian",this.sg1, this.sg1.row, this.sg1.col,
											  "select kode_obyek,nama from eclaim_obyek where kode_lokasi='"+this.app._lokasi+"'",
											  "select count(kode_obyek) from eclaim_obyek where kode_lokasi='"+this.app._lokasi+"'",
											  new Array("kode_obyek","nama"),"and",new Array("kode","nama"));
	}	
};
window.app_eclaim_report_flObyek.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_eclaim_report_flObyek.prototype.mainButtonClick = function(sender)
{
	try
	{
	...
	{
		alert("[app_eclaim_report_flObyek]::mainButtonClick:"+e);
	}
};
window.app_eclaim_report_flObyek.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_eclaim_report_flObyek.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_eclaim_report_flObyek.prototype.previewReport = function(dthtml)
{
	var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN OBYEK KERUGIAN<br>";			
	var d = new Date();
...
window.app_eclaim_report_flObyek.prototype.doCloseReportClick = function(sender)
{
...
window.app_eclaim_report_flObyek.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_eclaim_report_flObyek.prototype.doRowPerPageChange = function(sender, rowperpage)
{
