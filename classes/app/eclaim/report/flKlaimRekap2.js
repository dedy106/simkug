window.app_eclaim_report_flKlaimRekap2 = function(owner)
{
	if (owner)
	{
		window.app_eclaim_report_flKlaimRekap2.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim_report_flKlaimRekap2";
		this.maximize();
...
window.app_eclaim_report_flKlaimRekap2.extend(window.childForm);
window.app_eclaim_report_flKlaimRekap2.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 5)
		{
			this.filterRep.ListDataSGFilter(this, "Data Berkas",this.sg1, this.sg1.row, this.sg1.col,
													  "select no_klaim,no_dokumen,date_format(tanggal,'%d/%m/%Y') as tanggal from eclaim_klaim where kode_lokasi='"+this.app._lokasi+"' order by no_klaim desc",
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doKlaimDaftar(this.kode_proses);					
			...
			else
			{
				...
				this.nama_report = "server_report_eclaim_rptKlaimRekap2";
			...
			systemAPI.alert("[app_eclaim_report_flKlaimRekap2]::mainButtonClick:"+e);
		}
	},
	...
	doKlaimDaftar: function(kode_proses){
		...
		if (kode_proses=="P1")
		{
			filter=this.filter+" and a.status_dok='0' ";
		}
		if (kode_proses=="P2")
		{
			filter=this.filter+" and a.status_dok='1' ";
		}
		if (kode_proses=="P3")
		{
			filter=this.filter+" and a.status='0' ";
		}
		if (kode_proses=="P4")
		{
			filter=this.filter+" and a.status='1' ";
		}
		filter2=this.filter2;
		nama_report="server_report_eclaim_rptKlaimDaftar";
	...
	doKlaim: function(no_klaim){
		
		this.no_klaim=no_klaim;
		this.link="2";
		filter = "where a.no_klaim='"+no_klaim+"' and a.kode_ttg='"+this.sg1.getCell(2,1)+"' ";
		filter2=this.filter2;
		nama_report="server_report_eclaim_rptRwyKlaimSum";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
