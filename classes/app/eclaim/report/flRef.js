window.app_eclaim_report_flRef = function(owner)
{
	if (owner)
	{
		window.app_eclaim_report_flRef.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim_report_flRef";
		this.maximize();
		this.app._mainForm.childFormConfig(this,"mainButtonClick","Laporan Referensi Dokumen",2);
		uses("portalui_panel");
...
window.app_eclaim_report_flRef.extend(window.portalui_childForm);
window.app_eclaim_report_flRef.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 0)
	{
	...
	if (row == 2)
	{
		this.filterRep.ListDataSGFilter(this, "Data Dokumen",this.sg1, this.sg1.row, this.sg1.col,
											  "select kode_ref,nama from eclaim_ref where kode_lokasi='"+this.app._lokasi+"'",
											  "select count(kode_ref) from eclaim_ref where kode_lokasi='"+this.app._lokasi+"'",
											  new Array("kode_ref","nama"),"and",new Array("kode","nama"));
	}	
};
window.app_eclaim_report_flRef.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_eclaim_report_flRef.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.filter = this.filterRep.filterStr("a.kode_ttg",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
						...
						this.filterRep.filterStr("a.kode_ref",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and") +
						this.filterRep.filterStr("a.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"and");
			this.showFilter = this.filterRep.showFilter(this.sg1);
			this.sql =  "select c.nama as nama_ttg,b.nama as nama_asuransi,a.kode_ref,a.nama "+
						"from eclaim_ref a "+
			...
			title.add("Kode Ref");width.add(60);fieldType.add("S");		
			title.add("Nama Ref ");width.add(250);fieldType.add("S");
	...
	{
		alert("[app_eclaim_report_flRef]::mainButtonClick:"+e);
	}
};
window.app_eclaim_report_flRef.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_eclaim_report_flRef.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_eclaim_report_flRef.prototype.previewReport = function(dthtml)
{
	var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN DOKUMEN <br>";			
	var d = new Date();
...
window.app_eclaim_report_flRef.prototype.doCloseReportClick = function(sender)
{
...
window.app_eclaim_report_flRef.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_eclaim_report_flRef.prototype.doRowPerPageChange = function(sender, rowperpage)
{
