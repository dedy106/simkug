window.app_eclaim_dashboard = function(owner,options)
{
	if (owner)
	{
		window.app_eclaim_dashboard.prototype.parent.constructor.call(this,owner,options);
		this.className  = "app_eclaim_dashboard";
		...
		try {
			...
			var logo="server/media/"+this.dbLib.getPeriodeFromSQL("select gambar as periode from eclaim_ttg where kode_ttg='"+this.app._kodeTtg+"' ");
			...
			this.gridLib.SGEditData(this.sg1,7,new Array(0,1), new Array("No Dokumen","All"));
			...
			this.dataProvider = new app_eclaim_remote_dataProvider(this.app._dbSetting);
			this.dataProvider.addListener(this);
			this.standarLib = new util_standar();							
			this.filter=this.app._lokasi+"/"+this.app._periode+"/"+this.app._userLog+"/"+this.app._userStatus+"/"+this.app._kodeLok; 
...
window.app_eclaim_dashboard.extend(window.panel);
window.app_eclaim_dashboard.implement({	
	...
	addInfoItem: function(id, data, detail){
		...
		for (var i in detail.objList){
			line = detail.get(i);										
			table += "<tr><td padding=5><a style='cursor:pointer' onclick='$$("+this.resourceId +").doViewItem(event,\""+line.no_bukti+"\");'>"+line.no_bukti+"</div></td><td padding=5>"+line.keterangan+"</td></tr>";
	...
	doTampil: function() {
		...
		var sql="select a.no_klaim, a.no_dokumen, date_format(a.tanggal,'%d-%m-%Y') as tanggal, b.nama as nama_lok, c.nama as nama_obyek, d.nama as nama_sebab, a.nilai, e.nilai as nilai_adjust, e.nilai_ddct as nilai_bayar,a.alamat "+
			" from eclaim_klaim a "+
			" inner join eclaim_lokasi b on b.kode_lokasi = a.kode_lokasi and b.kode_lok = a.kode_lok and b.kode_ttg = a.kode_ttg "+
			" inner join eclaim_obyek c on c.kode_lokasi = a.kode_lokasi and c.kode_obyek = a.kode_obyek and c.kode_ttg = a.kode_ttg "+
			" inner join eclaim_sebab d on d.kode_lokasi = a.kode_lokasi and d.kode_sebab = a.kode_sebab and d.kode_ttg = a.kode_ttg "+
			" left join eclaim_adjust e on e.kode_lokasi = a.kode_lokasi and e.no_klaim = a.no_klaim and e.status = '1' "+this.filter+order;
