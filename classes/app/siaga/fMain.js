...
//* 			SAI, PT											
//***********************************************************************************************
window.app_siaga_fMain = function(owner)
{
	if (owner)
	{	
		try
		{
			window.app_siaga_fMain.prototype.parent.constructor.call(this, owner);			
			...
			uses("saiEdit;image;button;label;panel;util_addOnLib;util_standar;portalui_datePicker;saiMemo");					
...
window.app_siaga_fMain.extend(window.form);
window.app_siaga_fMain.implement({
...
//----------------------------------------------------------------- tree View-----------------------------------						
			this.treev = new treeView(this,{bound:[10,95,330,this.height - 130],childLength:400,click:[this,"treeClick"], visible:false});			
...
//--------------------------------`------------------
	...
	gotoLogin: function(){
		uses("app_siaga_fLogin");
		this.form = new app_siaga_fLogin(this);
		...
		try{
			...
			if (this.app._isLogedIn)
			{
				if (this.treev.isVisible()) {
					...
					this.dashboard.setWidth(this.width - 20);
	...
	initDashboard: function(){		
		try{
			uses("saiCBBL;saiEdit;saiGrid;sgNavigator;app_siaga_dashboard;pageControl;util_ajaxCaller;listView;util_file;app_lab_remote_dataProvider;tinymceCtrl;reportViewer;server_report_report;server_util_Map;webBrowser");
			//this.dashboard = new app_siaga_dashboard(this,{bound:[10, this.treev.top, this.width - this.treev.width - 30, this.treev.height]});		
			this.dashboard = new webBrowser(this,{bound:[10, this.treev.top, this.width - 30, this.treev.height]});
			
			//this.dashboardViewer.navigate("http://zembar.simkug.com?u="+this.app._userLog+"&p="+this.app._userPwd);
			//this.dashboardViewer.navigate("http://saife.simkug.com/index.php/akademik/CDashboard/home/"+this.app._userLog+"/"+this.app._userPwd);
			
			var rs = this.dbLib.getDataProvider("select server_fe,logo,db,controler from exs_server where kode_lokasi = '"+this.app._lokasi+"' ",true);
			var svr;
			var db;
			var logo;
			var controler;
			svr="http://saife.simkug.com/";
			if (rs.rs.rows[0])
			{
				svr = rs.rs.rows[0].server_fe;
				db = rs.rs.rows[0].db;
				logo = rs.rs.rows[0].logo;
				controler = rs.rs.rows[0].controler;
			}
			this.dashboard.navigate(svr+"index.php/dashboard/"+controler+"/main/"+this.app._userLog+"/"+this.app._lokasi+"/"+db);
	...
	bTutupClick: function(sender){	
		try{			
			this.pReport.hide();
			this.bDraft.hide();
			if (this.form instanceof app_siaga_fLogin && sender == this.bExit){
				return false;
			}			
			if (this.form == undefined){
