window.app_hrmis_referensi_report_flKaryawan = function(owner)
{
	if (owner)
	{
		window.app_hrmis_referensi_report_flKaryawan.prototype.parent.constructor.call(this,owner);
		this.className = "app_hrmis_referensi_report_flKaryawan";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,1,[0,1,2], ["Kode Karyawan","All",""]);		
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_hrmis_referensi_report_flKaryawan.extend(window.portalui_childForm);
window.app_hrmis_referensi_report_flKaryawan.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				var sql = "select k.nik,k.kode_lokasi,k.nama,k.alamat,k.jabatan,k.no_telp,k.email,k.kode_pp,k.npwp,"+
							"k.bank,k.cabang,k.no_rek,k.nama_rek "+
							"from karyawan k inner join lokasi l on k.kode_lokasi=l.kode_lokasi "+this.filter+
			...
			systemAPI.alert("[app_hrmis_referensi_report_flKaryawan]::mainButtonClick:"+e);
		}
	},
	  ...
	  switch(sender.getName())
	  {
	    ...
	    case "pdfBtn" :      
		  var html = new server_util_arrayList();
			...
			html.add("pdf");			
			html.add(new Date().valueOf()+"_Karyawan");				
