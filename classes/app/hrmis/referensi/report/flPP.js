window.app_hrmis_referensi_report_flPP = function(owner)
{
	if (owner)
	{
		window.app_hrmis_referensi_report_flPP.prototype.parent.constructor.call(this,owner);
		this.className = "app_hrmis_referensi_report_flPP";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Departemen", 2);
...
window.app_hrmis_referensi_report_flPP.extend(window.portalui_childForm);
window.app_hrmis_referensi_report_flPP.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("l.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
								this.filterRep.filterStr("p.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new portalui_arrayMap();
				var sql = "select p.kode_pp,p.kode_lokasi,fn_spasi(p.nama,p.level_spasi),p.initial,p.tipe,p.sum_header,p.kode_induk "+
							"from pp p inner join lokasi l on p.kode_lokasi=l.kode_lokasi "+this.filter+
							"order by p.kode_lokasi,p.kode_pp ";
				...
				title.add("Sum Header");width.add(100);fieldType.add("S");
		...
		{
			alert("[app_hrmis_referensi_report_flPP]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN DEPARTEMEN<br>";			
		var d = new Date();
