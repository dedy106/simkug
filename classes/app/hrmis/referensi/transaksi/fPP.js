window.app_hrmis_referensi_transaksi_fPP = function(owner)
{
	if (owner)
	{
		window.app_hrmis_referensi_transaksi_fPP.prototype.parent.constructor.call(this, owner);
		this.className = "app_hrmis_referensi_transaksi_fPP";
...
window.app_hrmis_referensi_transaksi_fPP.extend(window.portalui_childForm);
window.app_hrmis_referensi_transaksi_fPP.prototype.mainButtonClick = function(sender){
...
window.app_hrmis_referensi_transaksi_fPP.prototype.doRequestReady = function(sender, methodName, result){
	switch (methodName)
		{
			case "listData" : 
...
window.app_hrmis_referensi_transaksi_fPP.prototype.doModalResult = function(event, modalResult, value){
	try{
		switch (event){
			case "Add" :
			   if ( modalResult == mrOk){
					...
					{						
						...
						if (value != ""){
							if (driver.search("mssql") != -1){
								...
								for (var i in data){
									sql2 = "insert into pp(kode_pp, kode_lokasi,nama, initial,level_spasi, tipe, sum_header, kode_induk, rowindex) values ";
									sql2 += data[i];
									sql.add(sql2);
								}
...
window.app_hrmis_referensi_transaksi_fPP.prototype.loadMenu = function(daftarNrc){
	try{
...
//-------------------------------------
window.app_hrmis_referensi_transaksi_fPP.prototype.doClick = function(sender){	
	var nrc = this.dblib.runSQL("select * from pp where  kode_lokasi = '"+this.elokasi.getText()+"' order by rowindex",0,0);	
	this.loadMenu(nrc);	
};
window.app_hrmis_referensi_transaksi_fPP.prototype.entriesClick = function(sender){
	try
	{ 
...
window.app_hrmis_referensi_transaksi_fPP.prototype.treeClick = function(item){
};
window.app_hrmis_referensi_transaksi_fPP.prototype.getTreeData = function(node){
	var result = "";
	var child = undefined;
...
window.app_hrmis_referensi_transaksi_fPP.prototype.getSummary = function(node){
	var result = "";
	var child = undefined;
	if (node instanceof portalui_treeView)
	{
...
window.app_hrmis_referensi_transaksi_fPP.prototype.doFindLokasi = function(sender)
{
	if (this.app._userStatus =="A")
...
window.app_hrmis_referensi_transaksi_fPP.prototype.setSummaryItems = function(data){
	this.e4.clearItem();
	for (var i in data)
	{
...
window.app_hrmis_referensi_transaksi_fPP.prototype.copyChilds = function(from, to){
	var temp = undefined;
...
window.app_hrmis_referensi_transaksi_fPP.prototype.switchItem = function(from, to){
...
window.app_hrmis_referensi_transaksi_fPP.prototype.moveItem = function(item, owner)
{	
