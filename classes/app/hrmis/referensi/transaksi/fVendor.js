window.app_hrmis_referensi_transaksi_fVendor = function(owner){
	if (owner)
	{
		window.app_hrmis_referensi_transaksi_fVendor.prototype.parent.constructor.call(this,owner);
		this.className  = "app_hrmis_referensi_transaksi_fVendor";
		this.maximize();
		...
		this.ed_kode.setCaption("Kode Vendor");
		...
		this.cb_klp.setCaption("Kelompok Vendor");
		this.cb_klp.setText("");
...
window.app_hrmis_referensi_transaksi_fVendor.extend(window.portalui_childForm);
window.app_hrmis_referensi_transaksi_fVendor.prototype.mainButtonClick = function(sender)
{
...
window.app_hrmis_referensi_transaksi_fVendor.prototype.doModalResult = function(event, modalResult)
{			
	switch (event)
	{
		case "clear" :
		...
		case "simpan" :
			if (modalResult == mrOk)
			{
				...
				{					
					...
					sql.add("insert into vendor (kode_vendor,kode_klpvendor,nama,alamat,kota,kode_pos,no_telp,no_fax,npwp,pic,kode_lokasi,bank,cabang,no_rek,nama_rek,alamat2)  values "+
		...
		case "ubah" :
			if (modalResult == mrOk)
			{
					...
					sql.add("update vendor set  "+
							"kode_klpvendor='"+this.cb_klp.getText()+"',nama='"+this.ed_nama.getText()+"',alamat='"+this.ed_alamat.getText()+
		...
		case "hapus" :
		   if (modalResult == mrOk)
		   {
				uses("server_util_arrayList");					
					var sql = new server_util_arrayList();
					sql.add("delete from vendor where kode_vendor='"+this.ed_kode.getText()+"' and kode_lokasi='"+this.cb_lokasi.getText()+"'");
					this.dbLib.execArraySQL(sql);	
		   }
			break;
	}
};
window.app_hrmis_referensi_transaksi_fVendor.prototype.showClick = function(sender){
	if (this.ed_kode.getText() != ""){
		try
		{
			this.standarLib.clearByTag(this, [1],undefined);				
			uses("server_util_arrayMap");
			var data = this.dbLib.loadQuery("select a.kode_vendor,a.nama,a.alamat,a.kota,a.kode_pos,a.no_telp,a.no_fax,a.npwp,a.pic,a.kode_klpvendor,b.nama as nama_klp, "+
											"       a.bank,a.cabang,a.no_rek,a.nama_rek,a.alamat2 "+
											"from vendor a inner join vendor_klp b on a.kode_klpvendor = b.kode_klpvendor "+
...
window.app_hrmis_referensi_transaksi_fVendor.prototype.FindBtnClick = function(sender, event)
{
	try
	{
		...
		{
			this.standarLib.showListData(this, "Daftar Kelompok Vendor",this.cb_klp,undefined, 
...
window.app_hrmis_referensi_transaksi_fVendor.prototype.doRequestReady = function(sender, methodName, result)
{
