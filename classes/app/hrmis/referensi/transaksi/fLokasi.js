window.app_hrmis_referensi_transaksi_fLokasi = function(owner){
	if (owner){
		window.app_hrmis_referensi_transaksi_fLokasi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_hrmis_referensi_transaksi_fLokasi";
		this.setTop(60);
		this.setWidth(1280);
		this.setHeight(550);
		...
		try
		{		
			uses("portalui_saiCBBL;portalui_uploader;portalui_label;portalui_checkBox;util_standar;util_file");
			...
			//this.ed_kode.setMultiSelection(true);
			//this.ed_kode.setSQL("select kode_lokasi, nama from lokasi",["kode_lokasi","nama"],undefined,["Kode","Nama"],"where");
...
window.app_hrmis_referensi_transaksi_fLokasi.extend(window.portalui_childForm);
window.app_hrmis_referensi_transaksi_fLokasi.prototype.mainButtonClick = function(sender){
...
window.app_hrmis_referensi_transaksi_fLokasi.prototype.doModalResult = function(event, modalResult){
	switch (event)
	{
		case "clear" :
			if (modalResult == mrOk)
		...
		case "simpan" :
			if (modalResult == mrOk)
			{
				try
				{
					...
					sql.add("insert into lokasi (kode_lokasi, nama, alamat, kota, kodepos, no_telp, no_fax, flag_konsol, logo, email, website, npwp, pic, kode_lokkonsol, tgl_pkp) values "+
						    ...
						    "','"+this.ed_web.getText()+"','"+this.ed_npwp.getText()+"','"+this.ed_pic.getText()+"','-','"+this.dp_tgl1.getDateString()+"')");
			...
			if (modalResult == mrOk)
			{
				try
				{
					...
					sql.add("update lokasi set nama='"+this.ed_nama.getText()+"', alamat='"+this.ed_alamat.getText()+"', kota='"+
						 ...
						 this.ed_pic.getText()+"',tgl_pkp='"+this.dp_tgl1.getDateString()+"' where kode_lokasi='"+this.ed_kode.getText()+"'");
					this.dbLib.execArraySQL(sql);	
				}
				catch(e)
				{
...
window.app_hrmis_referensi_transaksi_fLokasi.prototype.showClick = function(sender){
	if (this.ed_kode.getText() != "")
	{
		...
		{ 			
			...
			var line,data = this.dbLib.runSQL("select nama, alamat, kota, kodepos, no_telp, no_fax, flag_konsol, logo, email, website, npwp, pic, date_format(tgl_pkp,'%d/%m/%Y') as tgl from lokasi "+
			...
			if (data instanceof portalui_arrayMap)
			{
				...
				if (line != undefined)
				{
					...
					this.fileBfr = "/server/image/"+line.get("logo");
...
window.app_hrmis_referensi_transaksi_fLokasi.prototype.FindBtnClick = function(sender, event){
	try
	{
		if (sender == this.ed_kode) 
		{
			this.standarLib.showListData(this, "Data Lokasi",this.ed_kode,this.ed_nama, 
										  "select kode_lokasi, nama from lokasi where kode_lokasi = '"+this.app._lokasi+"'","select count(kode_lokasi) from lokasi where kode_lokasi = '"+this.app._lokasi+"'",
...
window.app_hrmis_referensi_transaksi_fLokasi.prototype.doRequestReady = function(sender, methodName, result){
        ...
        switch(methodName)
    		{
    			case "execArraySQL" :    				
					...
					if (result.toLowerCase().search("error") == -1)					
		            {
		              ...
		              if (this.fileBfr && this.dataUpload) this.fileUtil.deleteFile(this.dataUpload.rootSvr+this.fileBfr);
                      if (this.dataUpload) this.fileUtil.copyFileTo(this.dataUpload.tmpfile,this.dataUpload.rootSvr+"/server/image/"+this.dataUpload.filename);
                      this.fileBfr = undefined;
...
window.app_hrmis_referensi_transaksi_fLokasi.prototype.doFileChange = function(sender, filename, allow,data){
	if (allow){
	    try{
