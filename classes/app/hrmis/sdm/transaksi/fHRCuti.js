window.GUI_sdm_transaksi_fHRCuti = function(owner)
{
	if (owner)
	{
		window.GUI_sdm_transaksi_fHRCuti.prototype.parent.constructor.call(this,owner);
		this.className  = "GUI_sdm_transaksi_fHRCuti";
		this.maximize();
		this.itemsValue = new controls_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Cuti : Input/Koreksi", 0);	
		...
		this.ed_jenis = new controls_saiLabelEdit(this);
		this.ed_jenis.setLeft(20);
		this.ed_jenis.setTop(120);
		this.ed_jenis.setWidth(250);
		this.ed_jenis.setCaption("Jenis Cuti");
		...
		this.sg1.setColTitle(new Array("Jenis Cuti","Tgl Mulai","Tgl Selesai","Jumlah Hari"));
...
window.GUI_sdm_transaksi_fHRCuti.extend(window.controls_childForm);

window.GUI_sdm_transaksi_fHRCuti.prototype.sg1init = function(sg)
{
	sg.setColWidth(new Array(3,2,1,0),
					new Array(100,80,80,150));	
}

window.GUI_sdm_transaksi_fHRCuti.prototype.sg1onDblClick = function(sender, col, row)
{
	try
	{			
		...
		dt=this.sg1.getCell(2,row).split("/");
		var dtakhir=dt[2]+"-"+dt[1]+"-"+dt[0];
		var data = this.dbLib.runSQL("select no_urut,tgl_mulai,tgl_selesai,jumlah "+
									 "from hr_cuti "+
									 "where nik='"+this.ed_kode.getText()+"' and tgl_mulai='"+dtawal+"' and tgl_selesai='"+dtakhir+
									 "' and jenis='"+this.sg1.cells(0,row)+"' and jumlah="+this.numLib.strToFloat(this.sg1.cells(3,row)));
...
window.GUI_sdm_transaksi_fHRCuti.prototype.mainButtonClick = function(sender)
{
...
window.GUI_sdm_transaksi_fHRCuti.prototype.doModalResult = function(event, modalResult)
{			
	switch (event)
	{
		case "clear" :
		...
		case "simpan" :
			if (modalResult == mrOk &&(this.standarLib.checkEmptyByTag(this, new Array("0","1"))))
			{
				try
				{					
					var data = this.dbLib.runSQL("select max(no_urut) as maks from hr_cuti where nik='"+this.ed_kode.getText()+"' ");
					if (data instanceof controls_arrayMap)
					{
						if (data.get(0) != undefined)
						{
		...
		case "ubah" :
			if (modalResult == mrOk &&(this.standarLib.checkEmptyByTag(this, new Array("0","1"))))
			{
				try
				{
					...
					sql.add("update hr_cuti set jenis='"+this.ed_jenis.getText()+"',jumlah="+this.numLib.strToFloat(this.ed_jumlah.getText())+
		...
		case "hapus" :
		   if (modalResult == mrOk &&(this.standarLib.checkEmptyByTag(this, new Array("0","1"))))
		   {
				uses("server_util_arrayList");					
				var sql = new server_util_arrayList();
				sql.add("delete from hr_cuti where nik='"+this.ed_kode.getText()+"' and no_urut="+this.nourut);
				this.dbLib.execArraySQL(sql);	
		   }
			break;
	}
...
window.GUI_sdm_transaksi_fHRCuti.prototype.showClick = function(sender)
{
	if (sender == this.bhistory)
	{
		if (this.ed_kode.getText() != "")
		{
			try
			{			
				var data = this.dbLib.runSQL("select jenis,tgl_mulai,tgl_selesai,jumlah "+
											 "from hr_cuti "+
...
window.GUI_sdm_transaksi_fHRCuti.prototype.FindBtnClick = function(sender, event)
{
	try
	{
...
window.GUI_sdm_transaksi_fHRCuti.prototype.doRequestReady = function(sender, methodName, result)
{
