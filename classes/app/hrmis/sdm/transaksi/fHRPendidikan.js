window.GUI_sdm_transaksi_fHRPendidikan = function(owner)
{
	if (owner)
	{
		window.GUI_sdm_transaksi_fHRPendidikan.prototype.parent.constructor.call(this,owner);
		this.className  = "GUI_sdm_transaksi_fHRPendidikan";
		this.maximize();
		this.itemsValue = new controls_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Pendidikan : Input/Koreksi", 0);	
		...
		this.ed_intitusi = new controls_saiLabelEdit(this);
		this.ed_intitusi.setLeft(20);
		this.ed_intitusi.setTop(120);
		this.ed_intitusi.setWidth(250);
		this.ed_intitusi.setCaption("Nama Institusi");
		this.ed_intitusi.setText("");
		this.ed_intitusi.setReadOnly(false);
		this.ed_intitusi.setLength(50);
		this.ed_intitusi.setTag("1");
		
		this.ed_jurusan = new controls_saiLabelEdit(this);
		this.ed_jurusan.setLeft(20);
		this.ed_jurusan.setTop(142);
		this.ed_jurusan.setWidth(250);
		this.ed_jurusan.setCaption("Jurusan/Falkutas");
		this.ed_jurusan.setText("");
		this.ed_jurusan.setReadOnly(false);
		this.ed_jurusan.setLength(100);
		this.ed_jurusan.setTag("1");
		
		this.ed_gelar = new controls_saiLabelEdit(this);
		this.ed_gelar.setLeft(20);
		this.ed_gelar.setTop(164);
		this.ed_gelar.setWidth(250);
		this.ed_gelar.setCaption("Gelar");
		this.ed_gelar.setText("");
		this.ed_gelar.setReadOnly(false);
		this.ed_gelar.setLength(100);
		...
		this.lbltgl1.setCaption("Tgl Selesai");
		this.lbltgl1.setUnderLine(true);
		
		uses("controls_datePicker");	
		this.dp_tglend = new controls_datePicker(this);
		this.dp_tglend.setTop(188);
		this.dp_tglend.setLeft(120);
		this.dp_tglend.setWidth(82);
		
		this.ed_biaya = new controls_saiLabelEdit(this);
		this.ed_biaya.setLeft(20);
		this.ed_biaya.setTop(208);
		this.ed_biaya.setWidth(250);
		this.ed_biaya.setCaption("Biaya");
		this.ed_biaya.setText("");
		this.ed_biaya.setReadOnly(false);
		this.ed_biaya.setLength(100);
		this.ed_biaya.setTag("1");
		...
		this.sg1.setTop(20);
		this.sg1.setLeft(1);
		this.sg1.setWidth(600);
		this.sg1.setHeight(194);			
		this.sg1.setColCount(5);
		this.sg1.setTag("2");
		this.sg1.setReadOnly(true);
		this.sg1.setColTitle(new Array("Nama Institusi","Jurusan/Falkutas","Gelar","Tgl Selesai","Biaya"));
...
window.GUI_sdm_transaksi_fHRPendidikan.extend(window.controls_childForm);

window.GUI_sdm_transaksi_fHRPendidikan.prototype.sg1init = function(sg)
{
	sg.setColWidth(new Array(4,3,2,1,0),
					new Array(100,80,80,150,150));
}

window.GUI_sdm_transaksi_fHRPendidikan.prototype.sg1onDblClick = function(sender, col, row)
{
	try
	{
		setTipeButton(tbUbahHapus);
		var dt=this.sg1.getCell(3,row).split("/");
		var dtend=dt[2]+"-"+dt[1]+"-"+dt[0];
		var data = this.dbLib.runSQL("select no_urut,tgl_selesai "+
									 "from hr_pendidikan "+
									 "where nik='"+this.ed_kode.getText()+"' and tgl_selesai='"+dtend+
									 "' and institusi='"+this.sg1.cells(0,row)+"' and jurusan='"+this.sg1.cells(1,row)+ 
									 "' and gelar='"+this.sg1.cells(2,row)+"' and biaya='"+this.sg1.cells(4,row)+"'");
		...
		if (data instanceof controls_arrayMap)
		{
			if (data.get(0) != undefined)
			{
				...
				this.ed_intitusi.setText(this.sg1.cells(0,row));
				this.ed_jurusan.setText(this.sg1.cells(1,row));
				this.ed_gelar.setText(this.sg1.cells(2,row));
				this.dp_tglend.setDateString(line.get("tgl_selesai"));
				this.ed_biaya.setText(this.sg1.cells(4,row));
			}
		}
	}catch(e)
	{
...
window.GUI_sdm_transaksi_fHRPendidikan.prototype.mainButtonClick = function(sender)
{
...
window.GUI_sdm_transaksi_fHRPendidikan.prototype.doModalResult = function(event, modalResult)
{			
	switch (event)
	{
		case "clear" :
		...
		case "simpan" :
			if (modalResult == mrOk &&(this.standarLib.checkEmptyByTag(this, new Array("0","1"))))
			{
				try
				{					
					var data = this.dbLib.runSQL("select max(no_urut) as maks from hr_pendidikan where nik='"+this.ed_kode.getText()+"' ");
					if (data instanceof controls_arrayMap)
					{
						if (data.get(0) != undefined)
						{
					...
					sql.add("insert into hr_pendidikan (nik,institusi,jurusan,gelar,tgl_selesai,biaya,user_id,tgl_input,no_urut) values ('"+this.ed_kode.getText()+"','"+this.ed_intitusi.getText()+"','"+this.ed_jurusan.getText()+"','"+this.ed_gelar.getText()+"','"+this.dp_tglend.getDate()+"','"+this.ed_biaya.getText()+"','"+this.app._userLog+"','"+tgl.getFullYear()+"-"+tgl.getBln()+"-"+tgl.getDate()+"',"+this.nourut+")");
		...
		case "ubah" :
			if (modalResult == mrOk &&(this.standarLib.checkEmptyByTag(this, new Array("0","1"))))
			{
				try
				{
					...
					sql.add("update hr_pendidikan set institusi='"+this.ed_intitusi.getText()+"',jurusan='"+this.ed_jurusan.getText()+
					"',gelar='"+this.ed_gelar.getText()+"',tgl_selesai='"+this.dp_tglend.getDate()+"',biaya='"+this.ed_biaya.getText()+
		...
		case "hapus" :
		   if (modalResult == mrOk &&(this.standarLib.checkEmptyByTag(this, new Array("0","1"))))
		   {
				uses("server_util_arrayList");					
				var sql = new server_util_arrayList();
				sql.add("delete from hr_pendidikan where nik='"+this.ed_kode.getText()+"' and no_urut="+this.nourut);
				this.dbLib.execArraySQL(sql);	
		   }
			break;
	}
...
window.GUI_sdm_transaksi_fHRPendidikan.prototype.showClick = function(sender)
{
	if (sender == this.bhistory)
	{
		if (this.ed_kode.getText() != "")
		{
			try
			{			
				var data = this.dbLib.runSQL("select institusi,jurusan,gelar,tgl_selesai,biaya "+
											 "from hr_pendidikan "+
											 "where nik='"+this.ed_kode.getText()+"' order by tgl_selesai desc");
...
window.GUI_sdm_transaksi_fHRPendidikan.prototype.FindBtnClick = function(sender, event)
{
	try
	{
...
window.GUI_sdm_transaksi_fHRPendidikan.prototype.doRequestReady = function(sender, methodName, result)
{
