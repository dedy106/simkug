window.app_hrmis_sdm_transaksi_fHRrwyjabf = function(owner)
{
	if (owner)
	{
		window.app_hrmis_sdm_transaksi_fHRrwyjabf.prototype.parent.constructor.call(this,owner);
		this.className  = "app_hrmis_sdm_transaksi_fHRrwyjabf";
		this.maximize();
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Riwayat Jabatan Fungsional: Input/Koreksi", 0);	
		...
		this.ed_jabflama = new portalui_saiLabelEdit(this);
		this.ed_jabflama.setLeft(20);
		this.ed_jabflama.setTop(186);
		this.ed_jabflama.setWidth(300);
		this.ed_jabflama.setLabelWidth(100);
		this.ed_jabflama.setLength(100);
		this.ed_jabflama.setReadOnly(false);
		this.ed_jabflama.setCaption("Jabatan Lama");
		this.ed_jabflama.setText("");
		this.ed_jabflama.setTag("1");
		
		this.ed_jabfbaru = new portalui_saiLabelEdit(this);
		this.ed_jabfbaru.setLeft(500);
		this.ed_jabfbaru.setTop(120);
		this.ed_jabfbaru.setWidth(300);
		this.ed_jabfbaru.setLabelWidth(120);
		this.ed_jabfbaru.setLength(100);
		this.ed_jabfbaru.setReadOnly(false);
		this.ed_jabfbaru.setCaption("Jabatan Baru");
		this.ed_jabfbaru.setText("");
		this.ed_jabfbaru.setTag("1");
	    ...
	    this.p1.setName('p1');
	    this.p1.setCaption('Riwayat Jabatan Fungsional');
		...
		try
		{
    		...
    		this.cb_jab.setSQL("select kode_jab, nama  from hr_jabatan where kode_lokkonsol='"+this.lokkonsol+"' and jenis = 'FUNGSIONAL'",["kode_jab","nama"]);
...
window.app_hrmis_sdm_transaksi_fHRrwyjabf.extend(window.portalui_childForm);
window.app_hrmis_sdm_transaksi_fHRrwyjabf.prototype.sg1init = function(sg)
{
	sg.setColWidth([9,8,7,6,5,4,3,2,1,0],
					[80,100,100,100,120,120,80,80,80,120]);
};
window.app_hrmis_sdm_transaksi_fHRrwyjabf.prototype.sg1onDblClick = function(sender, col, row)
{
	try
	{		
		var data = this.dbLib.runSQL("select a.no_sk,e.tgl_sk,e.tgl_berlaku as tgl_skmulai,a.tingkat,a.jab_lama,a.jab_baru,a.kode_jabf,"+
		                             "       a.kode_lokasi,a.kode_loker, "+
		                             "       b.nama as nama_lokasi, c.nama as nama_loker, ifnull(d.nama,'-') as nama_jabf, a.status_aktif "+
						"from hr_jabf a inner join hr_lokasi b on a.kode_lokasi=b.kode_lokasi and a.kode_lokkonsol = b.kode_lokkonsol "+
						...
						"	 inner join hr_sk e on e.no_sk = a.no_sk and e.kode_lokkonsol = a.kode_lokkonsol "+
									 "                 left outer join hr_jabatan d on a.kode_jabf=d.kode_jab and a.kode_lokkonsol = d.kode_lokkonsol "+
		...
		if (data instanceof portalui_arrayMap)
		{
			if (data.get(0) != undefined)
			{
				...
				this.cb_jab.setText(line.get("kode_jabf"));
				this.cb_jab.setRightLabelCaption(line.get("nama_jabf"));
				this.ed_jabflama.setText(line.get("jab_lama"));
				this.ed_jabfbaru.setText(line.get("jab_baru"));
...
window.app_hrmis_sdm_transaksi_fHRrwyjabf.prototype.doEditChange = function(sender)
{
...
window.app_hrmis_sdm_transaksi_fHRrwyjabf.prototype.mainButtonClick = function(sender)
{
...
window.app_hrmis_sdm_transaksi_fHRrwyjabf.prototype.doModalResult = function(event, modalResult)
{			
	var stsAktif = "0";
	...
	switch (event)
	{
		...
		case "simpan" :
			if (modalResult == mrOk &&(this.standarLib.checkEmptyByTag(this, new Array("0","1","9"))))
			{
				...
				{					
					...
					if (stsAktif == "1")
						sql.add("update hr_jabf set status_aktif = '0' where no_sk<>'"+this.ed_nosk.getText()+"' and nik = '"+this.cb_nik.getText()+"' and kode_lokkonsol = '"+this.lokkonsol+"'");
					sql.add("insert into hr_jabf (nik,no_sk,tgl_sk,tgl_skmulai,tingkat,jab_lama,jab_baru,kode_jabf,kode_lokasi,"+
					        ...
					        "values ('"+this.cb_nik.getText()+"','"+this.ed_nosk.getText()+"','"+this.dp_tglsk.getDate()+"','"+
										...
										this.ed_jabflama.getText()+"','"+this.ed_jabfbaru.getText()+"','"+this.cb_jab.getText()+"',"+
		...
		case "ubah" :
			if (modalResult == mrOk &&(this.standarLib.checkEmptyByTag(this, new Array("0","1"))))
			{
				try
				{
					...
					if (stsAktif == "1")
						sql.add("update hr_jabf set status_aktif = '0' where no_sk<>'"+this.ed_nosk.getText()+"' and nik = '"+this.cb_nik.getText()+"' and kode_lokkonsol = '"+this.lokkonsol+"'");
					sql.add("update hr_jabf set tgl_sk='"+this.dp_tglsk.getDate()+"',tgl_skmulai='"+this.dp_tglskmulai.getDate()+"',"+
					        "tingkat='"+this.ed_tingkat.getText()+"',jab_lama='"+this.ed_jabflama.getText()+"',jab_baru='"+this.ed_jabfbaru.getText()+"',kode_jabf='"+this.cb_jab.getText()+
		...
		case "hapus" :
		   if (modalResult == mrOk &&(this.standarLib.checkEmptyByTag(this, new Array("0","1"))))
		   {
				uses("server_util_arrayList");					
				var sql = new server_util_arrayList();
				sql.add("delete from hr_jabf where nik='"+this.cb_nik.getText()+"' and kode_lokkonsol='"+this.lokkonsol+"' and no_sk='"+this.ed_nosk.getText()+"'");
				this.dbLib.execArraySQL(sql);	
		   }
			break;
	}
};
window.app_hrmis_sdm_transaksi_fHRrwyjabf.prototype.show2Click = function(sender)
{
	if (this.cb_nik.getText() != "")
	{
		try
...
window.app_hrmis_sdm_transaksi_fHRrwyjabf.prototype.showClick = function(sender)
{
	if (sender == this.bhistory)
	{
		if (this.cb_nik.getText() != "")
		{
			...
			{			
				this.sg1.clear();									   
			    var data = this.dbLib.runSQL("select a.no_sk,a.tgl_sk,a.tgl_skmulai,a.tingkat,a.jab_lama,a.jab_baru,ifnull(d.nama,'-') as nama_jabf,"+
...
window.app_hrmis_sdm_transaksi_fHRrwyjabf.prototype.FindBtnClick = function(sender, event)
{
	try
	{
...
window.app_hrmis_sdm_transaksi_fHRrwyjabf.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_hrmis_sdm_transaksi_fHRrwyjabf.implement({
