window.app_hrmis_sdm_transaksi_fHRrwytingkat = function(owner)
{
	if (owner)
	{
		window.app_hrmis_sdm_transaksi_fHRrwytingkat.prototype.parent.constructor.call(this,owner);
		this.className  = "app_hrmis_sdm_transaksi_fHRrwytingkat";
		this.maximize();
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Riwayat Tingkat: Input/Koreksi", 0);	
		...
		this.ed_tingkat1.setLeft(20);
		this.ed_tingkat1.setTop(164);
		this.ed_tingkat1.setWidth(180);
		...
		this.ed_tingkat2.setLeft(20);
		this.ed_tingkat2.setTop(186);
		this.ed_tingkat2.setWidth(180);
	    ...
	    this.p1.setName('p1');
	    this.p1.setCaption('Riwayat Tingkat');
		...
		this.sg1.setColTitle(["Nomor SK","Tgl SK","Tgl Berlaku","Tingkat Lama","Tingkat Baru","Ms Kerja [Thn]","Ms Kerja [Bln]","Gaji Dasar","Jns Keterangan","Status Aktif"]);
...
window.app_hrmis_sdm_transaksi_fHRrwytingkat.extend(window.portalui_childForm);
window.app_hrmis_sdm_transaksi_fHRrwytingkat.prototype.sg1init = function(sg)
{
	sg.setColWidth([9,8,7,6,5,4,3,2,1,0],[80,190,110,80,80,80,80,80,80,120]);
};
window.app_hrmis_sdm_transaksi_fHRrwytingkat.prototype.sg1onDblClick = function(sender, col, row)
{
	try
	{		
		var data = this.dbLib.runSQL("select a.no_sk,a.tgl_sk,a.tgl_skmulai,a.tingkat1,a.tingkat2,a.mk_tahun,a.mk_bulan,a.gadas,a.keterangan,a.status_aktif "+
									 "from hr_tingkat a "+
...
window.app_hrmis_sdm_transaksi_fHRrwytingkat.prototype.doEditChange = function(sender)
{
...
window.app_hrmis_sdm_transaksi_fHRrwytingkat.prototype.mainButtonClick = function(sender)
{
...
window.app_hrmis_sdm_transaksi_fHRrwytingkat.prototype.doModalResult = function(event, modalResult)
{			
	var stsAktif = "0";
	...
	switch (event)
	{
		...
		case "simpan" :
			if (modalResult == mrOk &&(this.standarLib.checkEmptyByTag(this,["0","1","9"])))
			{
				...
				{					
					...
					if (stsAktif == "1")
						sql.add("update hr_tingkat set status_aktif = '0' where no_sk<>'"+this.ed_nosk.getText()+"' and nik = '"+this.cb_nik.getText()+"' and kode_lokkonsol = '"+this.lokkonsol+"'");
					sql.add("insert into hr_tingkat (nik,no_sk,tgl_sk,tgl_skmulai,tingkat1,tingkat2,mk_tahun,mk_bulan,gadas,keterangan,"+
					        "                        kode_lokkonsol,user_id,tgl_input,status_aktif) "+
					        "values ('"+this.cb_nik.getText()+"','"+this.ed_nosk.getText()+"','"+this.dp_tglsk.getDate()+"','"+
							            this.dp_tglskmulai.getDate()+"','"+this.ed_tingkat1.getText()+"','"+this.ed_tingkat2.getText()+
										"',"+parseNilai(this.ed_tahun.getText())+","+parseNilai(this.ed_bulan.getText())+","+
										parseNilai(this.ed_gaji.getText())+",'"+this.ed_desc.getText()+"','"+
		...
		case "ubah" :
			if (modalResult == mrOk &&(this.standarLib.checkEmptyByTag(this, new Array("0","1"))))
			{
				try
				{
					...
					if (stsAktif == "1")
						sql.add("update hr_tingkat set status_aktif = '0' where no_sk<>'"+this.ed_nosk.getText()+"' and nik = '"+this.cb_nik.getText()+"' and kode_lokkonsol = '"+this.lokkonsol+"'");					
					sql.add("update hr_tingkat set tgl_sk='"+this.dp_tglsk.getDate()+"',tgl_skmulai='"+this.dp_tglskmulai.getDate()+"',"+
					        "tingkat1='"+this.ed_tingkat1.getText()+"',tingkat2='"+this.ed_tingkat2.getText()+
							"',mk_tahun="+parseNilai(this.ed_tahun.getText())+",mk_bulan="+parseNilai(this.ed_bulan.getText())+",gadas="+parseNilai(this.ed_gaji.getText())+",keterangan='"+this.ed_desc.getText()+"',user_id='"+this.app._userLog+"',tgl_input=now(),status_aktif='"+stsAktif+"' "+
		...
		case "hapus" :
		   if (modalResult == mrOk &&(this.standarLib.checkEmptyByTag(this, new Array("0","1"))))
		   {
				uses("server_util_arrayList");					
				var sql = new server_util_arrayList();
				sql.add("delete from hr_tingkat where nik='"+this.cb_nik.getText()+"' and kode_lokkonsol='"+this.lokkonsol+"' and no_sk='"+this.ed_nosk.getText()+"'");
				this.dbLib.execArraySQL(sql);	
		   }
			break;
	}
};
window.app_hrmis_sdm_transaksi_fHRrwytingkat.prototype.show2Click = function(sender)
{
	if (this.cb_nik.getText() != "")
	{
		try
...
window.app_hrmis_sdm_transaksi_fHRrwytingkat.prototype.showClick = function(sender)
{
	if (sender == this.bhistory)
	{
		if (this.cb_nik.getText() != "")
		{
			...
			{			
			    ...
			    var data = this.dbLib.runSQL("select a.no_sk,a.tgl_sk,a.tgl_skmulai,a.tingkat1,a.tingkat2,a.mk_tahun,a.mk_bulan,a.gadas,a.keterangan,a.status_aktif "+
									 "from hr_tingkat a "+
									 "where a.kode_lokkonsol = '"+this.lokkonsol+"' and a.nik = '"+this.cb_nik.getText()+"' ");
...
window.app_hrmis_sdm_transaksi_fHRrwytingkat.prototype.hitungClick = function(sender)
{
	try
	{
		if (this.cb_nik.getText() != "")
...
window.app_hrmis_sdm_transaksi_fHRrwytingkat.prototype.FindBtnClick = function(sender, event)
{
	try
	{
...
window.app_hrmis_sdm_transaksi_fHRrwytingkat.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_hrmis_sdm_transaksi_fHRrwytingkat.implement({
	doChange: function(sender){
		if (sender == this.ed_nosk){
			try{
				var data = this.dbLib.getDataProvider("select tgl_sk, tgl_mulai, tgl_berlaku, datediff(day, tgl_mulai,getDate()) as jmlhari  from hr_sk where kode_lokkonsol = '"+this.app._lokKonsol+"' and no_sk = '"+sender.getText()+"' and nik = '"+this.cb_nik.getText()+"' ",true);			
