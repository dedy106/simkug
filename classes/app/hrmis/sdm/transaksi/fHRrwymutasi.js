window.app_hrmis_sdm_transaksi_fHRrwymutasi = function(owner)
{
	if (owner)
	{
		window.app_hrmis_sdm_transaksi_fHRrwymutasi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_hrmis_sdm_transaksi_fHRrwymutasi";
		this.maximize();
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Riwayat Mutasi Pegawai: Input/Koreksi", 0);	
		...
		this.cb_pp.setCaption("Loker Lama");
		this.cb_pp.setText("");
		this.cb_pp.setRightLabelCaption("");
		this.cb_pp.setTag("1");
		
		this.cb_pp2 = new portalui_saiCBBL(this);
		this.cb_pp2.setLeft(20);
		this.cb_pp2.setTop(186);
		this.cb_pp2.setWidth(185);
		this.cb_pp2.setLabelWidth(100);
		this.cb_pp2.setRightLabelVisible(true);		
		this.cb_pp2.setCaption("Loker Baru");
		this.cb_pp2.setText("");
		...
		this.cb_loker.setCaption("Unit Kerja Lama");
		this.cb_loker.setText("");
		this.cb_loker.setRightLabelCaption("");
		this.cb_loker.setTag("1");
		
		this.cb_loker2 = new portalui_saiCBBL(this);
		this.cb_loker2.setLeft(500);
		this.cb_loker2.setTop(164);
		this.cb_loker2.setWidth(205);
		this.cb_loker2.setLabelWidth(100);
		this.cb_loker2.setRightLabelVisible(true);		
		this.cb_loker2.setCaption("Unit Kerja Baru");
		this.cb_loker2.setText("");
		this.cb_loker2.setRightLabelCaption("");
		this.cb_loker2.setTag("1");
		...
		this.ed_desc.addItem(0,"REGULER");
		this.ed_desc.addItem(0,"ROTASI");
	    ...
	    this.p1.setName('p1');
	    this.p1.setCaption('Riwayat Mutasi Pegawai');
		...
		this.sg1.setColTitle(["No SK","Tgl SK","Tgl Berlaku","Loker Lama","Loker Baru","Unit Kerja Lama","Unit Kerja Baru","Keterangan","Status Aktif"]);
		...
		this.cb_pp2.onChange.set(this, "doEditChange");
		this.cb_loker.onBtnClick.set(this, "FindBtnClick");
		this.cb_loker2.onBtnClick.set(this, "FindBtnClick");
		...
		try
		{
			...
			this.cb_pp2.setSQL("select kode_lokasi, nama  from hr_lokasi where kode_lokkonsol='"+this.lokkonsol+"'  ",["kode_lokasi","nama"]);			
		}catch(e)
		{
			alert(e);
		}
...
window.app_hrmis_sdm_transaksi_fHRrwymutasi.extend(window.portalui_childForm);
window.app_hrmis_sdm_transaksi_fHRrwymutasi.prototype.sg1init = function(sg)
{
	sg.setColWidth([8,7,6,5,4,3,2,1,0],[80,190,100,100,120,110,80,80,120]);
};
window.app_hrmis_sdm_transaksi_fHRrwymutasi.prototype.sg1onDblClick = function(sender, col, row)
{
	try
	{		
		var data = this.dbLib.runSQL("select a.no_sk,f.tgl_sk,f.tgl_berlaku as tgl_skmulai,a.kode_lokasi,a.kode_lokasi2,a.kode_loker,a.kode_loker2,a.keterangan,b.nama as nama_lokasi,c.nama as nama_lokasi2,d.nama as nama_loker, e.nama as nama_loker2 ,a.status_aktif "+
									 "from hr_rwymutasi a inner join hr_lokasi b on a.kode_lokasi  = b.kode_lokasi and a.kode_lokkonsol = b.kode_lokkonsol "+
									 "                    inner join hr_lokasi c on a.kode_lokasi2 = c.kode_lokasi and a.kode_lokkonsol = c.kode_lokkonsol "+
									 "                    inner join hr_loker d on a.kode_loker = d.kode_loker and a.kode_lokkonsol = d.kode_lokkonsol "+
									 "                    inner join hr_loker e on a.kode_loker2 = e.kode_loker and a.kode_lokkonsol = e.kode_lokkonsol "+
		...
		if (data instanceof portalui_arrayMap)
		{
			if (data.get(0) != undefined)
			{
				...
				this.dp_tglsk.setText(line.get("tgl_sk"));
				this.dp_tglskmulai.setText(line.get("tgl_skmulai"));
				this.cb_pp.setText(line.get("kode_lokasi"));
				this.cb_pp.setRightLabelCaption(line.get("nama_lokasi"));
				this.cb_pp2.setText(line.get("kode_lokasi2"));
				this.cb_pp2.setRightLabelCaption(line.get("nama_lokasi2"));
				this.cb_loker.setText(line.get("kode_loker"));
				this.cb_loker.setRightLabelCaption(line.get("nama_loker"));
				this.cb_loker2.setText(line.get("kode_loker2"));
				this.cb_loker2.setRightLabelCaption(line.get("nama_loker2"));
...
window.app_hrmis_sdm_transaksi_fHRrwymutasi.prototype.doEditChange = function(sender)
{
	if (sender == this.cb_pp) this.cb_loker.setSQL("select kode_loker, nama  from hr_loker where kode_lokasi='"+this.cb_pp.getText()+"' and tipe= 'posting'",["kode_loker","nama"]);
	if (sender == this.cb_pp2) this.cb_loker2.setSQL("select kode_loker, nama  from hr_loker where kode_lokasi='"+this.cb_pp2.getText()+"' and tipe= 'posting'",["kode_loker","nama"]);
...
window.app_hrmis_sdm_transaksi_fHRrwymutasi.prototype.mainButtonClick = function(sender)
{
...
window.app_hrmis_sdm_transaksi_fHRrwymutasi.prototype.doModalResult = function(event, modalResult)
{			
	var stsAktif = "0";
	...
	switch (event)
	{
		...
		case "simpan" :
			if (modalResult == mrOk &&(this.standarLib.checkEmptyByTag(this,["0","1","9"])))
			{
				...
				{					
					...
					if (stsAktif == "1")
						sql.add("update hr_rwymutasi set status_aktif = '0' where no_sk<>'"+this.ed_nosk.getText()+"' and nik = '"+this.cb_nik.getText()+"' and kode_lokkonsol = '"+this.lokkonsol+"'");					
					sql.add("insert into hr_rwymutasi (nik,no_sk,tgl_sk,tgl_skmulai,kode_lokasi,kode_lokasi2,kode_loker,kode_loker2,keterangan,"+
					        "                          kode_lokkonsol,user_id,tgl_input,status_aktif) "+
					        "values ('"+this.cb_nik.getText()+"','"+this.ed_nosk.getText()+"','"+
							            this.dp_tglsk.getDate()+"','"+this.dp_tglskmulai.getDate()+"','"+
										this.cb_pp.getText()+"','"+this.cb_pp2.getText()+
										"','"+this.cb_loker.getText()+"','"+this.cb_loker2.getText()+"','"+this.ed_desc.getText()+"','"+this.lokkonsol+"','"+this.app._userLog+"',now(),'"+stsAktif+"')");
					sql.add("update karyawan set kode_loker='"+this.cb_loker2.getText()+"',kode_lokasi = '"+this.cb_pp2.getText()+"' where nik = '"+this.cb_nik.getText()+"'");
		...
		case "ubah" :
			if (modalResult == mrOk &&(this.standarLib.checkEmptyByTag(this, new Array("0","1"))))
			{
				try
				{
					...
					if (stsAktif == "1")
						sql.add("update hr_rwymutasi set status_aktif = '0' where no_sk<>'"+this.ed_nosk.getText()+"' and nik = '"+this.cb_nik.getText()+"' and kode_lokkonsol = '"+this.lokkonsol+"'");					
					sql.add("update hr_rwymutasi set "+
					        "tgl_sk='"+this.dp_tglsk.getDate()+"',"+
							"tgl_skmulai='"+this.dp_tglskmulai.getDate()+"',kode_lokasi='"+this.cb_pp.getText()+"',kode_lokasi2='"+this.cb_pp2.getText()+"',kode_loker='"+this.cb_loker.getText()+
							"',kode_loker2='"+this.cb_loker2.getText()+"',keterangan='"+this.ed_desc.getText()+"',user_id='"+this.app._userLog+"',tgl_input=now(),status_aktif='"+stsAktif+"' "+
		...
		case "hapus" :
		   if (modalResult == mrOk &&(this.standarLib.checkEmptyByTag(this, new Array("0","1"))))
		   {
				uses("server_util_arrayList");					
				var sql = new server_util_arrayList();
				sql.add("delete from hr_rwymutasi where nik='"+this.cb_nik.getText()+"' and kode_lokkonsol='"+this.lokkonsol+"' and no_sk='"+this.ed_nosk.getText()+"'");
				this.dbLib.execArraySQL(sql);	
		   }
			break;
	}
};
window.app_hrmis_sdm_transaksi_fHRrwymutasi.prototype.show2Click = function(sender)
{
	if (this.cb_nik.getText() != "")
	{
		try
...
window.app_hrmis_sdm_transaksi_fHRrwymutasi.prototype.showClick = function(sender)
{
	if (sender == this.bhistory)
	{
		if (this.cb_nik.getText() != "")
		{
			...
			{			
				this.sg1.clear();									   
			    var data = this.dbLib.runSQL("select a.no_sk,a.tgl_sk,a.tgl_skmulai, b.nama as nama_lokasi,c.nama as nama_lokasi2,d.nama as nama_loker, e.nama as nama_loker2,a.keterangan,a.status_aktif  "+
									 "from hr_rwymutasi a inner join hr_lokasi b on a.kode_lokasi  = b.kode_lokasi and a.kode_lokkonsol = b.kode_lokkonsol "+
									 "                    inner join hr_lokasi c on a.kode_lokasi2 = c.kode_lokasi and a.kode_lokkonsol = c.kode_lokkonsol "+
									 "                    inner join hr_loker d on a.kode_loker = d.kode_loker and a.kode_lokkonsol = d.kode_lokkonsol "+
									 "                    inner join hr_loker e on a.kode_loker2 = e.kode_loker and a.kode_lokkonsol = e.kode_lokkonsol "+
...
window.app_hrmis_sdm_transaksi_fHRrwymutasi.prototype.FindBtnClick = function(sender, event)
{
	try
	{
		if (sender == this.cb_nik) 
		...
		{
			this.standarLib.showListData(this, "Daftar Lokasi Kerja",this.cb_pp2,undefined, 
										  "select kode_lokasi, nama  from hr_lokasi where kode_lokkonsol='"+this.lokkonsol+"'  ",
										  "select count(kode_lokasi) from hr_lokasi where kode_lokkonsol='"+this.lokkonsol+"' ",
										  new Array("kode_lokasi","nama"),"and",new Array("Kode","Deskripsi"),false);
			this.cb_loker2.setText("");
		...
		if (sender == this.cb_loker2) 
		{
			this.standarLib.showListData(this, "Daftar Unit Kerja",this.cb_loker2,undefined, 
										  "select kode_loker, nama  from hr_loker where kode_lokasi='"+this.cb_pp2.getText()+"' and tipe= 'posting'",
										  "select count(kode_loker) from hr_loker where kode_lokasi='"+this.cb_pp2.getText()+"' and tipe= 'posting'",
										  new Array("kode_loker","nama"),"and",new Array("Kode","Deskripsi"),false);
		}
...
window.app_hrmis_sdm_transaksi_fHRrwymutasi.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_hrmis_sdm_transaksi_fHRrwymutasi.implement({
