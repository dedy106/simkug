window.app_hrmis_sdm_master_fRiop = function(owner)
{
	if (owner)
	{
		window.app_hrmis_sdm_master_fRiop.prototype.parent.constructor.call(this,owner);
		this.className  = "app_hrmis_sdm_master_fRiop";
		this.maximize();
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Parameter Rawat Inap Operasi", 0);	
...
window.app_hrmis_sdm_master_fRiop.extend(window.portalui_childForm);
window.app_hrmis_sdm_master_fRiop.prototype.mainButtonClick = function(sender)
{
...
window.app_hrmis_sdm_master_fRiop.prototype.doModalResult = function(event, modalResult)
{			
	switch (event)
	{
		case "clear" :
		...
		case "simpan" :
			if (modalResult == mrOk)
			{
				...
				{					
					...
					sql.add("insert into kes_riop (kode_jenis,nama,kode_lokkonsol,tarif,status)  values "+
		...
		case "hapus" :
		   if (modalResult == mrOk)
		   {
				uses("server_util_arrayList");					
					var sql = new server_util_arrayList();
					sql.add("delete from kes_riop where kode_jenis='"+this.ed_kode.getText()+"' and kode_lokkonsol='"+this.lokkonsol+"'");
					this.dbLib.execArraySQL(sql);	
		   }
			break;
	}
};
window.app_hrmis_sdm_master_fRiop.prototype.showClick = function(sender)
{	
	if (this.ed_kode.getText() != "")
	{
		try
		{
			...
			var data = this.dbLib.loadQuery("select a.kode_jenis,a.nama,a.tarif,a.status from kes_riop a where a.kode_jenis = '"+this.ed_kode.getText()+"' and a.kode_lokkonsol='"+this.lokkonsol+"'");
			if (data != undefined) 
			{
			  if  (data != "") 
			  {
...
window.app_hrmis_sdm_master_fRiop.prototype.tampilClick = function(sender)
{
	this.sg1.clearAll();
	var temp = this.dbLib.runSQL("select kode_jenis,nama,status,tarif from kes_riop where kode_lokkonsol = '"+this.lokkonsol+"'");
		
		if (temp instanceof portalui_arrayMap){
			this.sg1.setData(temp);
		}else alert(rs);
};
window.app_hrmis_sdm_master_fRiop.prototype.FindBtnClick = function(sender, event)
{
	try
	{
		if (sender == this.ed_kode) 
		{
			this.standarLib.showListData(this, "Daftar Jenis Rawat Inap",sender,undefined, 
										  "select kode_jenis, nama  from kes_riop where kode_lokkonsol='"+this.lokkonsol+"'",
										  "select count(kode_jenis) from kes_riop where kode_lokkonsol='"+this.lokkonsol+"'",
...
window.app_hrmis_sdm_master_fRiop.prototype.doRequestReady = function(sender, methodName, result)
{
