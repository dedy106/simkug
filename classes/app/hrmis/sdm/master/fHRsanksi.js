window.app_hrmis_sdm_master_fHRsanksi = function(owner)
{
	if (owner)
	{
		window.app_hrmis_sdm_master_fHRsanksi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_hrmis_sdm_master_fHRsanksi";
		this.maximize();
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Master Sanksi", 0);
		
		this.ed_kode = new portalui_saiCBBL(this);
		this.ed_kode.setLeft(20);
		this.ed_kode.setTop(10);
		this.ed_kode.setWidth(185);
		this.ed_kode.setCaption("Kode Sanksi");
	    ...
	    this.p1.setName('p1');
	    this.p1.setCaption('Daftar Master Sanksi');
		...
		this.sg1.setTag("8");
		this.sg1.setColTitle(["No","Kode Sanksi","Keterangan"]);
...
window.app_hrmis_sdm_master_fHRsanksi.extend(window.portalui_childForm);
window.app_hrmis_sdm_master_fHRsanksi.prototype.mainButtonClick = function(sender)
{
...
window.app_hrmis_sdm_master_fHRsanksi.prototype.doModalResult = function(event, modalResult)
{			
	switch (event)
	{
		case "clear" :
		...
		case "simpan" :
			if (modalResult == mrOk)
			{
				...
				{					
					...
					sql.add("insert into hr_sanksi (kode_sanksi,nama,kode_lokkonsol)  values "+
		...
		case "ubah" :
			if (modalResult == mrOk)
			{
					...
					sql.add(" update hr_sanksi set  "+
							" nama = '"+this.ed_nama.getText()+"' where kode_sanksi='"+this.ed_kode.getText()+"' and kode_lokkonsol='"+this.lokkonsol+"'");
					this.dbLib.execArraySQL(sql);	
			}
			break;
		case "hapus" :
		   if (modalResult == mrOk)
		   {
				uses("server_util_arrayList");					
					var sql = new server_util_arrayList();
					sql.add("delete from hr_sanksi where kode_sanksi='"+this.ed_kode.getText()+"' and kode_lokkonsol='"+this.lokkonsol+"'");
					this.dbLib.execArraySQL(sql);	
		   }
			break;
	}
};
window.app_hrmis_sdm_master_fHRsanksi.prototype.showClick = function(sender)
{	
	if (this.ed_kode.getText() != "")
	{
		try
		{
			...
			var data = this.dbLib.loadQuery("select a.kode_sanksi,a.nama from hr_sanksi a where a.kode_sanksi = '"+this.ed_kode.getText()+"' and a.kode_lokkonsol='"+this.lokkonsol+"'");
			if (data != undefined) 
			{
			  if  (data != "") 
			  {
...
window.app_hrmis_sdm_master_fHRsanksi.prototype.tampilClick = function(sender)
{
	this.sg1.clearAll();
	var temp = this.dbLib.runSQL("select kode_sanksi,nama from hr_sanksi where kode_lokkonsol = '"+this.lokkonsol+"'");
		
		if (temp instanceof portalui_arrayMap){
			this.sg1.setData(temp);
		}else alert(rs);
};
window.app_hrmis_sdm_master_fHRsanksi.prototype.FindBtnClick = function(sender, event)
{
	try
	{
...
window.app_hrmis_sdm_master_fHRsanksi.prototype.doRequestReady = function(sender, methodName, result)
{
