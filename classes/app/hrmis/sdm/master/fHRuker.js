window.app_hrmis_sdm_master_fHRuker = function(owner)
{
	if (owner)
	{
		window.app_hrmis_sdm_master_fHRuker.prototype.parent.constructor.call(this, owner);
		this.className = "app_hrmis_sdm_master_fHRuker";
		this.app._mainForm.childFormConfig(this,"mainButtonClick","Data Unit Kerja : Input/Koreksi",0);
		this.maximize();
		...
		this.pAdd.hide();
			...
			this.e5 = new portalui_saiLabelEdit(this.pAdd,{visible:false,text:'-'});
			this.e5.setTop(140);
			this.e5.setLeft(20);
			this.e5.setWidth(300);
			this.e5.setCaption("Alamat");	
			
			this.e6 = new portalui_saiLabelEdit(this.pAdd,{visible:false,text:'-'});
			this.e6.setTop(40);
			this.e6.setLeft(350);
			...
			this.e8 = new portalui_saiLabelEdit(this.pAdd,{visible:false,text:'-'});
			this.e8.setTop(90);
			this.e8.setLeft(350);
...
window.app_hrmis_sdm_master_fHRuker.extend(window.portalui_childForm);
window.app_hrmis_sdm_master_fHRuker.prototype.mainButtonClick = function(sender)
{
...
window.app_hrmis_sdm_master_fHRuker.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "listData" :
...
window.app_hrmis_sdm_master_fHRuker.prototype.doModalResult = function(event, modalResult, value)
{
	try
	{
		switch (event)
		{
			case "Add" :
			   if ( modalResult == mrOk)
			   {
					...
					{   						
						...
						data.set("kode_pos",this.e8.getText());
			...
			case "simpan" :
				if (modalResult == mrOk)
				{
					...
					try
					{						
						var sql2,sql1 = "delete from hr_loker where kode_lokasi = '"+this.elokasi.getText()+"' and kode_lokkonsol = '"+this.lokkonsol+"'";	
						...
						if (driver.search("mssql") != -1){
							...
							for (var i in data){
								sql2 = "insert into hr_loker (kode_loker, kode_lokasi,nama, initial,level_spasi, tipe, kode_induk, rowindex, alamat, kota, propinsi, kode_pos, telephone,kode_lokkonsol) values ";
								sql2 += data[i];
								sql.add(sql2);
							}
	...
	{
		alert("[app_hrmis_sdm_master_fHRuker]::doModalResult:"+e);
	}
};
window.app_hrmis_sdm_master_fHRuker.prototype.loadMenu = function(daftarNrc)
{
	try
	{
...
window.app_hrmis_sdm_master_fHRuker.prototype.doClick = function(sender)
{	
	var nrc = this.dblib.runSQL("select * from hr_loker where kode_lokasi = '"+this.elokasi.getText()+"' and kode_lokkonsol = '"+this.lokkonsol+"' order by rowindex",0,0);	
	this.loadMenu(nrc);
};
window.app_hrmis_sdm_master_fHRuker.prototype.entriesClick = function(sender)
{
	try
	{ 
	 	if (sender == this.okBtn)
		{					
			var value = this.e1.getText() +";" +this.e2.getText()+";"+this.e3.getText()+";"+this.e5.getText()+";"+this.e6.getText()+";"+this.e7.setText()+";"+this.e8.getText()+";"+this.e9.getText();
		...
		{			
			...
			if (sender == this.addBtn)
			{
				...
				this.e5.setText("-");this.e6.setText("-");this.e7.setText("-");this.e8.setText("-");this.e9.setText("-");
				this.EditStatus = "Add";
				this.pAdd.show();
			}else if (sender == this.editBtn)
			{
				...
				this.e8.setText(item.data.get("kode_pos"));
...
window.app_hrmis_sdm_master_fHRuker.prototype.treeClick = function(item)
{
};
window.app_hrmis_sdm_master_fHRuker.prototype.getTreeData = function(node)
{
	var result = "";
	var child = undefined;
...
window.app_hrmis_sdm_master_fHRuker.prototype.getSummary = function(node)
{
	var result = "";
	var child = undefined;
	if (node instanceof portalui_treeView)
...
window.app_hrmis_sdm_master_fHRuker.prototype.doFindLokasi = function(sender)
{
	this.standarLib.showListDataFromItems(this, "Data Lokasi Kerja",this.elokasi, 
											  "select a.kode_lokasi,a.nama   from hr_lokasi a where a.kode_lokkonsol = '"+this.lokkonsol+"' ",
											  "select count(a.kode_lokasi)   from hr_lokasi a where a.kode_lokkonsol = '"+this.lokkonsol+"' ",
										  new Array("a.kode_lokasi","a.nama"),"and",new Array("Kode Loker","Nama"),false);	
};
window.app_hrmis_sdm_master_fHRuker.prototype.copyChilds = function(from, to)
{
	var temp = undefined;
