window.app_hrmis_sdm_master_fHRprofesi = function(owner)
{
	if (owner)
	{
		window.app_hrmis_sdm_master_fHRprofesi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_hrmis_sdm_master_fHRprofesi";
		this.maximize();
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this,"mainButtonClick","Data Master Profesi Pegawai",0);
	    ...
	    this.p1.setName('p1');
	    this.p1.setCaption('Daftar Master Profesi Pegawai');
		...
		this.sg1.setTag("8");
		this.sg1.setColTitle(["No","Kode Profesi","Keterangan"]);
...
window.app_hrmis_sdm_master_fHRprofesi.extend(window.portalui_childForm);
window.app_hrmis_sdm_master_fHRprofesi.prototype.mainButtonClick = function(sender)
{
...
window.app_hrmis_sdm_master_fHRprofesi.prototype.doModalResult = function(event, modalResult)
{			
	switch (event)
	{
		case "clear" :
		...
		case "simpan" :
			if (modalResult == mrOk)
			{
				...
				{					
					...
					sql.add("insert into hr_profesi (kode_profesi,nama,kode_lokkonsol)  values "+
		...
		case "ubah" :
			if (modalResult == mrOk)
			{
					...
					sql.add(" update hr_profesi set  "+
							" nama = '"+this.ed_nama.getText()+"' where kode_profesi='"+this.ed_kode.getText()+"' and kode_lokkonsol='"+this.lokkonsol+"'");
					this.dbLib.execArraySQL(sql);	
			}
			break;
		case "hapus" :
		   if (modalResult == mrOk)
		   {
				uses("server_util_arrayList");					
					var sql = new server_util_arrayList();
					sql.add("delete from hr_profesi where kode_profesi='"+this.ed_kode.getText()+"' and kode_lokkonsol='"+this.lokkonsol+"'");
					this.dbLib.execArraySQL(sql);	
		   }
			break;
	}
};
window.app_hrmis_sdm_master_fHRprofesi.prototype.showClick = function(sender)
{	
	if (this.ed_kode.getText() != "")
	{
		try
		{
			...
			var data = this.dbLib.loadQuery("select a.kode_profesi,a.nama from hr_profesi a where a.kode_profesi = '"+this.ed_kode.getText()+"' and a.kode_lokkonsol='"+this.lokkonsol+"'");
			if (data != undefined) 
			{
			  if  (data != "") 
			  {
...
window.app_hrmis_sdm_master_fHRprofesi.prototype.tampilClick = function(sender)
{
	this.sg1.clearAll();
	var temp = this.dbLib.runSQL("select kode_profesi,nama from hr_profesi where kode_lokkonsol = '"+this.lokkonsol+"'");
		
		if (temp instanceof portalui_arrayMap){
			this.sg1.setData(temp);
		}else alert(rs);
};
window.app_hrmis_sdm_master_fHRprofesi.prototype.FindBtnClick = function(sender, event)
{
	try
	{
...
window.app_hrmis_sdm_master_fHRprofesi.prototype.doRequestReady = function(sender, methodName, result)
{
