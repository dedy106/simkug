window.app_hrmis_sdm_report_flKesRelPP = function(owner)
{
	if (owner)
	{
		window.app_hrmis_sdm_report_flKesRelPP.prototype.parent.constructor.call(this,owner);
		this.className = "app_hrmis_sdm_report_flKesRelPP";
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Relasi Provider Poli", 2);
	...
	this.gridLib.SGEditData(this.sg1,1,[0,1,2],["Kode Klp. Provider","All",""]);
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Kode Provider","All",""]);
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Kode Poli","All",""]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_hrmis_sdm_report_flKesRelPP.extend(window.portalui_childForm);
window.app_hrmis_sdm_report_flKesRelPP.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row === 1)
			{
				this.filterRep.ListDataSGFilter(this, "Data Kelompok Provider",this.sg1, this.sg1.row, this.sg1.col,
											  "select kode_klp,nama from kes_klp_provider where kode_lokasi ='"+this.sg1.getCell(2,0)+"' ",
											  "select count(*) from kes_klp_provider where kode_lokasi ='"+this.sg1.getCell(2,0)+"' ",
											  ["kode_klp","nama"],"and",["Kode","Nama Klp. Provider"]);
			}
			if (row === 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Provider",this.sg1, this.sg1.row, this.sg1.col,
											  "select kode_provider,nama from kes_provider where kode_klp ='"+this.sg1.getCell(2,1)+"' ",
											  "select count(*) from kes_provider where kode_klp ='"+this.sg1.getCell(2,1)+"' ",
		...
		try
		{
			...
			}else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.kode_provider",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.kode_poli",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new portalui_arrayMap();
				var sql = "select a.kode_provider,b.nama,a.kode_poli,c.nama "+
						"from kes_provider_poli a inner join kes_provider b on a.kode_provider=b.kode_provider and a.kode_lokasi=b.kode_lokasi "+
						"inner join kes_poli c on a.kode_poli=c.kode_poli and a.kode_lokasi=c.kode_lokasi "+this.filter+
						" order by a.kode_provider,a.nu";
				this.scriptSqlCount = "select count(*) "+
						"from kes_provider_poli a inner join kes_provider b on a.kode_provider=b.kode_provider and a.kode_lokasi=b.kode_lokasi "+
						"inner join kes_poli c on a.kode_poli=c.kode_poli and a.kode_lokasi=c.kode_lokasi "+this.filter;
				
				this.title = new server_util_arrayList({items:["Kode Provider","Nama Provider","Kode Poli","Nama Poli"]});
			...
			systemAPI.alert("[app_hrmis_sdm_report_flKesRelPP]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var header = "LAPORAN RELASI PROVIDER POLI";
