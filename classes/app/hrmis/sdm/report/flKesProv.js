window.app_hrmis_sdm_report_flKesProv = function(owner)
{
	if (owner)
	{
		window.app_hrmis_sdm_report_flKesProv.prototype.parent.constructor.call(this,owner);
		this.className = "app_hrmis_sdm_report_flKesProv";
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Provider", 2);
...
window.app_hrmis_sdm_report_flKesProv.extend(window.portalui_childForm);
window.app_hrmis_sdm_report_flKesProv.implement({
		...
		try
		{
			...
			}else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("kode_provider",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new portalui_arrayMap();
				var sql = "select kode_provider,nama,alamat,bank,cabang,no_rek,nama_rek,kota,no_telp,no_fax from kes_provider "+this.filter;
				this.scriptSqlCount = "select count(kode_provider) from kes_provider "+this.filter;
				
				this.title = new server_util_arrayList({items:["Kode Provider","Nama Provider","Alamat","Bank","Cabang","No Rekening","Nama Rekening","Kota","Telepon","Faximile"]});
			...
			systemAPI.alert("[app_hrmis_sdm_report_flKesProv]::mainButtonClick:"+e);
		}
	},
