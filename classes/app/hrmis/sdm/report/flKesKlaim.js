window.app_hrmis_sdm_report_flKesKlaim = function(owner)
{
	if (owner)
	{
		window.app_hrmis_sdm_report_flKesKlaim.prototype.parent.constructor.call(this,owner);
		this.className = "app_hrmis_sdm_report_flKesKlaim";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Klaim Kesehatan", 2);
	...
	this.filterRep.setSGFilterRowTipe(this.sg1, 0,[0,1,2,3,4],["13","3","13","13","13"]);
	...
	strSql="select max(periode) as periode from kes_klaim_m"; 
	...
	this.gridLib.SGEditData(this.sg1,4,[0,1,2],["No Klaim","All",""]);
...
window.app_hrmis_sdm_report_flKesKlaim.extend(window.portalui_childForm);
window.app_hrmis_sdm_report_flKesKlaim.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 0)
	{
	...
	if (row == 4)
	{
		...
		var sql1="select no_klaim,keterangan from kes_klaim_m "+filter;
		var sql2="select count(no_klaim) from kes_klaim_m "+filter;
		
		this.filterRep.ListDataSGFilter(this, "Daftar Klaim",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("no_klaim","keterangan"),"and",new Array("No Klaim","Keterangan"));
	}
};
window.app_hrmis_sdm_report_flKesKlaim.prototype.doSelectCell = function(sender, col, row)
{	
	...
	if (row == 1)
	{
		this.dbLib.setItemsFromSQL("select distinct periode from kes_klaim_m a "+this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where "),[this.sg1.columns.get(2).pickList]);		
	}
};
window.app_hrmis_sdm_report_flKesKlaim.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		this.filter=this.filterRep.filterStr("b.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					...
					this.filterRep.filterStr("a.no_klaim",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
		this.nama_report="server_report_sdm_rptKesKlaim";
...
window.app_hrmis_sdm_report_flKesKlaim.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_hrmis_sdm_report_flKesKlaim.prototype.doSelectedPage = function(sender, page)
{
	this.report.preview(this.nama_report,this.filter, page,this.pager, this.lokasi, this.showFilter,this.filter2);
	this.page=page;
	this.allBtn = false;
};
window.app_hrmis_sdm_report_flKesKlaim.prototype.doCloseReportClick = function(sender)
{
...
window.app_hrmis_sdm_report_flKesKlaim.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_hrmis_sdm_report_flKesKlaim.prototype.doRowPerPageChange = function(sender, rowperpage)
{
