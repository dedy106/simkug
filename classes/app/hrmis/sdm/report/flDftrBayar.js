window.app_hrmis_sdm_report_flDftrBayar = function(owner)
{
	if (owner)
	{
		window.app_hrmis_sdm_report_flDftrBayar.prototype.parent.constructor.call(this,owner);
		this.className = "app_hrmis_sdm_report_flDftrBayar";
		this.maximize();		
...
window.app_hrmis_sdm_report_flDftrBayar.extend(window.portalui_childForm);
window.app_hrmis_sdm_report_flDftrBayar.implement({
		...
		try
		{
			...
			}else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("c.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_provider",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new portalui_arrayMap();
				var sql = "select a.nama as nmprov,(a.bank+' Cab. '+a.cabang) as bank,a.no_rek,ifnull(sum(b.nilai),0) as byr "+
						"from kes_provider a left outer join kes_trans_d b on a.kode_provider=b.kode_provider "+
						"left outer join kes_trans_m c on b.no_trans=c.no_trans "+
						"left outer join kes_klp_provider d on a.kode_klp=d.kode_klp "+this.filter+
						" group by a.kode_provider,a.nama,a.bank,a.cabang,a.no_rek";
				this.scriptSqlCount = "select count(*) "+
						"from kes_provider a left outer join kes_trans_d b on a.kode_provider=b.kode_provider "+
						"left outer join kes_trans_m c on b.no_trans=c.no_trans "+
						"left outer join kes_klp_provider d on a.kode_klp=d.kode_klp "+this.filter;
				
				this.title = new server_util_arrayList({items:["Provider","Bank","No. Rekening","Jumlah"]});
			...
			systemAPI.alert("[app_hrmis_sdm_report_flDftrBayar]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var header = "DAFTAR PEMBAYARAN TRANSFER<br />MELALUI REKENING GIRO YPT";
