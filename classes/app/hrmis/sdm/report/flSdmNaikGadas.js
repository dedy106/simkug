window.app_hrmis_sdm_report_flSdmNaikGadas = function(owner)
{
	if (owner)
	{
		window.app_hrmis_sdm_report_flSdmNaikGadas.prototype.parent.constructor.call(this,owner);
		this.className = "app_hrmis_sdm_report_flSdmNaikGadas";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Kanaikan Gaji Dasar Pegawai", 2);
...
window.app_hrmis_sdm_report_flSdmNaikGadas.extend(window.portalui_childForm);
window.app_hrmis_sdm_report_flSdmNaikGadas.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 0)
	{
...
window.app_hrmis_sdm_report_flSdmNaikGadas.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
...
window.app_hrmis_sdm_report_flSdmNaikGadas.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			var sql = "select k.nik,k.nama,b.tingkat,b.gadas,concat(b.mk_tahun,' thn ',b.mk_bulan,' bln') as mskrj,b.keterangan "+
				"from karyawan k inner join hr_dinas2 a on k.nik=a.nik and k.kode_lokasi='-' "+
				"inner join hr_gadas b on a.nik=b.nik "+this.filter+
			...
			title.add("Tingkat");width.add(80);fieldType.add("S");
			title.add("Gaji Dasar");width.add(100);fieldType.add("N");
	...
	{
		alert("[app_hrmis_sdm_report_flSdmNaikGadas]::mainButtonClick:"+e);
	}
};
window.app_hrmis_sdm_report_flSdmNaikGadas.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_hrmis_sdm_report_flSdmNaikGadas.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_hrmis_sdm_report_flSdmNaikGadas.prototype.previewReport = function(dthtml)
{
	...
	html +="<br>LAPORAN KENAIKAN GAJI DASAR PEGAWAI YPT<br>";
	var d = new Date();
...
window.app_hrmis_sdm_report_flSdmNaikGadas.prototype.doCloseReportClick = function(sender)
{
...
window.app_hrmis_sdm_report_flSdmNaikGadas.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_hrmis_sdm_report_flSdmNaikGadas.prototype.doRowPerPageChange = function(sender, rowperpage)
{
