window.app_hrmis_sdm_report_flSdmUker = function(owner)
{
	if (owner)
	{
		window.app_hrmis_sdm_report_flSdmUker.prototype.parent.constructor.call(this,owner);
		this.className = "app_hrmis_sdm_report_flSdmUker";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Unit Kerja", 2);
...
window.app_hrmis_sdm_report_flSdmUker.extend(window.portalui_childForm);
window.app_hrmis_sdm_report_flSdmUker.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 0)
	{
...
window.app_hrmis_sdm_report_flSdmUker.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
...
window.app_hrmis_sdm_report_flSdmUker.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		}else
		{
			...
			var sql = "select p.kode_loker,p.kode_lokasi,fn_spasi(p.nama,p.level_spasi), p.initial,p.tipe,p.kode_induk "+
						"from hr_loker p inner join hr_lokasi l on p.kode_lokasi=l.kode_lokasi "+this.filter+
	...
	{
		alert("[app_hrmis_sdm_report_flSdmUker]::mainButtonClick:"+e);
	}
};
window.app_hrmis_sdm_report_flSdmUker.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_hrmis_sdm_report_flSdmUker.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_hrmis_sdm_report_flSdmUker.prototype.previewReport = function(dthtml)
{
	var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN UNIT KERJA<br>";
	var d = new Date();
...
window.app_hrmis_sdm_report_flSdmUker.prototype.doCloseReportClick = function(sender)
{
...
window.app_hrmis_sdm_report_flSdmUker.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_hrmis_sdm_report_flSdmUker.prototype.doRowPerPageChange = function(sender, rowperpage)
{
