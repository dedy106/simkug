window.app_hrmis_gaji_report_flHRGajiKomp = function(owner)
{
	try{
	if (owner)
	{
		window.app_hrmis_gaji_report_flHRGajiKomp.prototype.parent.constructor.call(this,owner);
		this.className = "app_hrmis_gaji_report_flHRGajiKomp";
		this.maximize();
...
window.app_hrmis_gaji_report_flHRGajiKomp.extend(window.portalui_childForm);
window.app_hrmis_gaji_report_flHRGajiKomp.implement({
		...
		try
		{
			...
			else
			{
				...
				this.filter3=this.filterRep.filterStr("y.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("y.nik",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.jenisLap = this.sg1.getCell(2,4);
				var result  = new portalui_arrayMap();
				var sql="select distinct a.nik,a.nama,ifnull(d.b1,0) as bln1,ifnull(d.b2,0) as bln2, "+
						...
						"from karyawan a inner join hr_dinas2 b on a.nik=b.nik and a.kode_lokkonsol=b.kode_lokkonsol "+
							...
							"left outer join (select z.nik, "+
								"(select sum(case a.dc when 'D' then a.nilai else -a.nilai end) from gaji_d a inner join gaji_m c on a.no_gaji=c.no_gaji and a.nik = z.nik "+this.filter2+" and(c.periode = '"+this.sg1.getCell(2,1)+"') group by a.nik) as b1, "+
								"(select sum(case a.dc when 'D' then a.nilai else -a.nilai end) from gaji_d a inner join gaji_m c on a.no_gaji=c.no_gaji and a.nik = z.nik "+this.filter2+" and(c.periode = '"+this.sg1.getCell(2,2)+"') group by a.nik) as b2 "+
							"from gaji_d z inner join gaji_m y on z.no_gaji=y.no_gaji "+this.filter3+
				...
				this.scriptSqlCount = "select count(distinct a.nik) "+
						"from karyawan a inner join hr_dinas2 b on a.nik=b.nik and a.kode_lokkonsol=b.kode_lokkonsol "+
							...
							"from gaji_d z inner join gaji_m y on z.no_gaji=y.no_gaji "+this.filter3+
							" ) d on a.nik=d.nik "+this.filter;
				this.title = new server_util_arrayList({items:["NIK","Nama","Periode "+this.sg1.getCell(2,1),"Periode "+this.sg1.getCell(2,2),"Total"]});
			...
			systemAPI.alert("[app_hrmis_gaji_report_flHRGajiKomp]::mainButtonClick:"+e);
		}
	},
