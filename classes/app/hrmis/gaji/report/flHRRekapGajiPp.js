window.app_hrmis_gaji_report_flHRRekapGajiPp = function(owner)
{
	if (owner)
	{
		window.app_hrmis_gaji_report_flHRRekapGajiPp.prototype.parent.constructor.call(this,owner);
		this.className = "app_hrmis_gaji_report_flHRRekapGajiPp";
		this.maximize();
...
window.app_hrmis_gaji_report_flHRRekapGajiPp.extend(window.portalui_childForm);
window.app_hrmis_gaji_report_flHRRekapGajiPp.prototype.doEllipseClick= function(sender, col, row)
{
	if (row == 0)
	{
...
window.app_hrmis_gaji_report_flHRRekapGajiPp.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_hrmis_gaji_report_flHRRekapGajiPp.prototype.mainButtonClick = function(sender)
{
	try
	{
	...
	{
		alert("[app_hrmis_gaji_report_flHRRekapGajiPp]::mainButtonClick:"+e);
	}
};
window.app_hrmis_gaji_report_flHRRekapGajiPp.prototype.jenisReport = function(jenis,filter)
{
	try
	{
		...
		if (jenis == "Pendapatan")
		{
			var sql = "select a.kode_loker,c.nama,sum(b.gdas) as gdas,sum(b.tyay) as tyay,sum(tjab) as tjab,sum(b.tprof) as tprof,sum(b.pdas) as pdas,sum(b.thrt) as thrt, "+
					  ...
					  "			sum(b.gdas+b.tyay+b.tjab+b.tprof+b.pdas+b.thrt+b.trans+b.tkhs+b.bpls+b.lmbr+b.kbkt+b.ph21) as total "+
					  "	from karyawan a "+
		...
		}else if (jenis == "Semua")
		{
			var sql = "select a.kode_loker,c.nama,sum(b.gdas) as gdas,sum(b.tyay) as tyay,sum(tjab) as tjab,sum(b.tprof) as tprof,sum(b.pdas) as pdas,sum(b.thrt) as thrt, "+
					  "		   sum(b.trans) as trans,sum(b.tkhs) as tkhs,sum(b.bpls) as bpls,sum(b.lmbr) as lmbr,sum(b.kbkt) as kbkt,sum(b.ph21) as ph21, "+
					  "			sum(b.gdas+b.tyay+b.tjab+b.tprof+b.thrt+b.pdas+b.trans+b.tkhs+b.bpls+b.lmbr+b.kbkt+b.ph21) as total_pdpt, "+
					  ...
					  "			sum(b.gdas+b.tyay+b.tjab+b.tprof+b.thrt+b.trans+b.pdas+b.tkhs+b.bpls+b.lmbr+b.kbkt+b.ph21) -sum(b.ppph+b.ptht+b.pinl+b.pkes+b.kosb+b.koct+b.kopt+b.byd1) as total_bersih"+ 
					  "	from karyawan a "+
					  " inner join hr_dinas2 d on a.nik=d.nik "+
					  " inner join hr_loker c on d.kode_loker=c.kode_loker and a.kode_lokasi=c.kode_lokasi and tipe='Posting' "+
					  "	inner join (select a.nik,case a.kode_param when 'GDAS' then nilai else 0 end as gdas, "+
	...
	{
		alert("[app_hrmis_gaji_report_flHRRekapGajiPp]::jenisReport:"+e);
	}
};
window.app_hrmis_gaji_report_flHRRekapGajiPp.prototype.doRequestReady = function(sender, methodName, result)
{
	switch (methodName)
	{
		case "preview" : 
...
window.app_hrmis_gaji_report_flHRRekapGajiPp.prototype.doSelectedPage = function(sender, page)
{
...
window.app_hrmis_gaji_report_flHRRekapGajiPp.prototype.previewReport = function(dthtml,jenis)
{
...
window.app_hrmis_gaji_report_flHRRekapGajiPp.prototype.doCloseReportClick = function(sender)
{
...
window.app_hrmis_gaji_report_flHRRekapGajiPp.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_hrmis_gaji_report_flHRRekapGajiPp.prototype.doRowPerPageChange = function(sender, rowperpage)
{
