		...
		{				
			...
			sql.add("insert into gaji_m (no_gaji,tanggal,keterangan,nik,no_dok,kode_lokkonsol,periode,progress,no_del,no_link,nik_user,tgl_input,kode_lokasi) values "+
					"('"+this.ed_nb.getText()+"','"+this.dp_tgl1.getDate()+"','Gaji periode "+this.ed_period.getText()+"','"+this.cb_nik.getText()+"','"+this.cb_dok.getText()+"','"+this.lokkonsol+"','"+
					     this.ed_period.getText()+"','0','-','-','"+this.app._userLog+"',now(),'"+this.app._lokasi+"')");
					
			var scr1 = "";			
			for (var i=0; i < this.sg1.rows.getLength(); i++){
				scr1 = "insert into gaji_d (no_gaji,nik,no_dok,kode_param,kode_lokkonsol,nilai,dc) values ('"+this.ed_nb.getText()+"','"+this.cb_nik.getText()+"','"+this.cb_dok.getText()+
		...
		try
		{
			if (this.cb_nik.getText() != "")
			{
				var line,data = runArraySQL("select a.nik, ifnull(a.jml,0) as jml "+
								...
								"select datediff (day,tgl_lahir,now()) as jmlhari,status "+
								...
								"select datediff (day,tgl_masuk, now()) as jmlhari "+
									"from karyawan where nik = '"+this.cb_nik.getText()+"' and kode_lokkonsol = '"+this.lokkonsol+"'\r\n"+
								...
								"select a.kode_pp+' - '+b.nama as nama_pp,a.alamat,a.grade "+
			...
			case 3 : 
					...
					{		
						  if (this.sg1.getCell(3,i) == "DEF")
						  {
							...
							if ((this.sg1.getCell(3,i) == "CAL") && (this.sg1.getCell(0,i) == "GDAS") && (this.kodestspeg != "1"))
							{																
                                var tjabs2 = this.getDataKontrak(this.sg1.getCell(0,i));// 14/10/2009 
                                if (tjabs2 !== undefined) {
                                    this.sg1.setCell(2,i,floatToNilai(tjabs2.nilai1));
                                    gdas = true;
                                }else this.sg1.setCell(2,i,"0");                                
							}else
							...
							{
								var tjabs , tjabs2 = 0;
								tjabs2 = this.getDataKontrak(this.sg1.getCell(0,i));
								if (tjabs2 === undefined){
									...
									if (this.dataTjabPddk !== undefined &&  !isNaN(this.dataTjabPddk) )//jika masih belajar ambil dari hr_belajar_m (tjab) 080909
										this.sg1.setCell(2,i,floatToNilai(tjabs * this.dataTjabPddk / 100));								
									else 
							...
							{				
								var tprof = this.getDataKontrak(this.sg1.getCell(0,i));
								if (tprof === undefined){
							...
							if ((this.sg1.getCell(3,i) == "CAL") && (this.sg1.getCell(0,i) == "THRT"))
							{
								...
								if (this.kodestspeg == "1") {	
                                    var ttht = this.getDataKontrak(this.sg1.getCell(0,i));								
                                    if (ttht){
                                        ttht = ttht.nilai1;
                                    }else{
    									var tyay = nilaiToFloat(this.ed_gadas.getText()) * parseFloat(this.nTyay);									
    									var tprof = this.getTunjProfesi();
							...
							{
								var data = this.getDataKontrak(this.sg1.getCell(0,i));
								if (data === undefined){
							...
							if ((this.sg1.getCell(3,i) == "CAL") && (this.sg1.getCell(0,i) == "BPLS"))
							{
								...
								if (data === undefined){		
									...
									if (this.masaKrj >= this.jmlMasakrj) pulsa = (ptmp === undefined ? 0 : ptmp);
								}else{
	...
	try{
		var data = runArraySQL("select kode_lokkonsol from lokasi where kode_lokasi = '"+this.app._lokasi+"'\r\n"+
							...
							"select kode_spro,value1,value2 "+
								      "from spro where (kode_lokasi = 'table1_kode_lokkonsol' or kode_lokasi = '"+this.app._lokasi+"' )and  kode_spro in ('TYAY','TPKS','TCUT','BYJAB','TPLS','TJAB')");			
...
window.app_hrmis_gaji_transaksi_fGaji.prototype.getTunjJabatanStruk= function(){
	try{
		var tjabs2 = 0;
...
window.app_hrmis_gaji_transaksi_fGaji.prototype.getTunjJabatanFung= function(){
	var tjabf;
	if (this.kodejabf == "-"){
	  tjabf = 0;
	}else{	
...
window.app_hrmis_gaji_transaksi_fGaji.prototype.getTunjProfesi= function(){
	var tprof = 0;
	if (this.kodestspeg == "1")
	{		
		if (this.kodeprof == "-")
...
window.app_hrmis_gaji_transaksi_fGaji.prototype.getTunjParam= function(kode){
...
window.app_hrmis_gaji_transaksi_fGaji.prototype.hitungPPh= function(){
	try{
		var rowPPh;
		var totbln = 0;
...
window.app_hrmis_gaji_transaksi_fGaji.prototype.getDataKontrak = function(kode){
