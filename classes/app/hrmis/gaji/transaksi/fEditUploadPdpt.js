window.app_hrmis_gaji_transaksi_fEditUploadPdpt = function(owner)
{
	if (owner)
	{
		window.app_hrmis_gaji_transaksi_fEditUploadPdpt.prototype.parent.constructor.call(this, owner);
		this.className = "app_hrmis_gaji_transaksi_fEditUploadPdpt";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Edit Upload Pendapatan", 0);	
		...
		this.bGen = new portalui_button(this,{bound:[276,78,80,20],caption:"Gen",icon:"url(icon/"+system.getThemes()+"/process.png)",click:[this,"doClick"]});		   		
...
window.app_hrmis_gaji_transaksi_fEditUploadPdpt.extend(window.portalui_childForm);
window.app_hrmis_gaji_transaksi_fEditUploadPdpt.implement({
						...
						for (var i=0;i < this.sg1.getRowCount();i++){														
                            for (var c =2;c < this.sg1.getColCount();c++){
                                if (nilaiToFloat(this.sg1.cells(c,i)) > 0){    							    
    								sqlText[c - 2] ="insert into gaji_load_pdpt_d (no_bukti, nik,kode_param, nilai,kode_lokasi,periode)values('"+this.ed_nb.getText()+"','"+this.sg1.cells(0,i)+"','"+fieldName[c]+"','"+ parseNilai(this.sg1.cells(c,i))+"','"+this.app._lokasi+"','"+this.ed_period.getText()+"')"; 							
    								sqlArray[c - 2] = "insert into gaji_pdpt(nik, kode_param, periode1, periode2, nilai, kode_lokkonsol)values('"+this.sg1.cells(0,i)+"','"+fieldName[c]+"','"+this.ed_period.getText()+"','"+this.ed_period.getText()+"','"+parseNilai(this.sg1.cells(c,i))+"','"+this.app._lokKonsol+"') ";    								
									sql.add(sqlText[c - 2]);  
	...
	doClick: function(sender){
		...
		if (sender == this.bLoad){
			try{
				var dataProvider = this.dbLib.getDataProvider("select distinct a.nik, b.nama, "+this.sqlField+" from gaji_load_pdpt_d a inner join karyawan b on b.nik = a.nik and b.kode_lokasi = a.kode_lokasi where a.kode_lokasi = '"+this.app._lokasi+"' and a.no_bukti ='"+this.cb_copy.getText()+"'  group by a.nik, b.nama",true);				
	...
	doFindBtnClick: function(sender){
	    ...
	    if (sender == this.cb_copy)
	       this.standarLib.showListData(this,"Data Upload Pendapatan Gaji",sender, undefined,
        			"select no_bukti, keterangan from gaji_load_pdpt_m where kode_lokasi ='"+this.app._lokasi+"' ",
        			"select count(*) from gaji_load_pdpt_m where kode_lokasi ='"+this.app._lokasi+"' ",
	...
	doEditChange: function(sender){
	    try{
			...
			var dataProvider = this.dbLib.getDataProvider("select distinct aa.periode, aa.tanggal, aa.keterangan, aa.nik_setuju, ifnull(c.nama,'-') as nmsetuju,a.nik, b.nama, "+this.sqlField+" from gaji_load_pdpt_m aa "+
                    " inner join gaji_load_pdpt_d a on a.no_bukti = aa.no_bukti and a.kode_lokasi = aa.kode_lokasi "+
	...
	initColumn: function(){
		try{
			var data = this.dbLib.getDataProvider("select distinct kode_param, nama  from gaji_param_d where kode_lokkonsol = '"+this.app._lokKonsol+"'  and not kode_param in ( 'GDAS','PDAS','PH21','THRT','TYAY','TJAB','TPROF','TRAN','BPLS') and jenis = 'PDPT' and kode_ref='LOD'",true);
