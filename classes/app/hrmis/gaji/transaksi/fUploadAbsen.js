window.app_hrmis_gaji_transaksi_fUploadAbsen = function(owner)
{
	if (owner)
	{
		window.app_hrmis_gaji_transaksi_fUploadAbsen.prototype.parent.constructor.call(this, owner);
		this.className = "app_hrmis_gaji_transaksi_fUploadAbsen";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Upload Absen", 0);	
		...
		this.sg1 = new portalui_saiGrid(this.p1,{bound:[0,20,900,200],colCount:3,
				colTitle:["NIK","Nama","Jumlah"],readOnly:true,
				colFormat:[[2],[cfNilai]], colWidth:[[0,1,2],[100,200,100]]});		
		...
		this.dp_tgl1.setText(new Date().lclFormat());
		this.ed_lokasi.setText(this.app._lokasi, this.app._namalokasi);
	}
};
window.app_hrmis_gaji_transaksi_fUploadAbsen.extend(window.portalui_childForm);
window.app_hrmis_gaji_transaksi_fUploadAbsen.implement({
		...
		switch (event){
			...
			case "simpan" :
					try{
						...
						sql.add("delete from gaji_load_absen_d where kode_lokasi = '"+this.app._lokasi+"' and periode = '"+this.ed_period.getText()+"' ");
						sql.add("delete from gaji_load_absen_m where kode_lokasi = '"+this.app._lokasi+"' and periode = '"+this.ed_period.getText()+"'");
						sql.add("delete from hr_absen where kode_lokkonsol = '"+this.app._lokKonsol+"' and periode = '"+this.ed_period.getText()+"' ");
						sql.add("insert into gaji_load_absen_m (no_bukti, periode, tanggal, keterangan, nik_setuju, kode_lokasi, user_id, tgl_input)values('"+this.ed_nb.getText()+"','"+this.ed_period.getText()+"','"+this.dp_tgl1.getDateString()+"','"+this.ed_ket.getText()+"','"+this.cb_setuju.getText()+"','"+this.app._lokasi+"','"+this.app._userLog+"',now())");
						var sqlText = "";
						var sqlAbs= "";
						for (var i=0;i < this.sg1.getRowCount();i++){
							sqlText ="insert into gaji_load_absen_d (no_bukti, no_urut,nik,jml,kode_lokasi,periode)values('"+this.ed_nb.getText()+"','"+i+"','"+this.sg1.cells(0,i)+"',";
							for (var c =2;c < this.sg1.getColCount();c++){
								sqlText += "'"+ parseNilai(this.sg1.cells(c,i))+"',";							
							}
							sqlText += "'"+this.app._lokasi+"','"+this.ed_period.getText()+"')"; 							
							sqlAbs = "insert into hr_absen(kode_lokkonsol, periode, nik, jml)values('"+this.app._lokKonsol+"','"+this.ed_period.getText()+"','"+this.sg1.cells(0,i)+"', '"+this.sg1.cells(2,i)+"' )";
							sql.add(sqlText);
							sql.add(sqlAbs);
						}																		
	...
	doClick: function(sender){
		this.ed_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"gaji_load_absen_m","no_bukti","UPL/"+this.ed_period.getText().substr(2,4),"00000"));
	},
