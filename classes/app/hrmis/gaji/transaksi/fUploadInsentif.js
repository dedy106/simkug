window.app_hrmis_gaji_transaksi_fUploadInsentif = function(owner)
{
	if (owner)
	{
		window.app_hrmis_gaji_transaksi_fUploadInsentif.prototype.parent.constructor.call(this, owner);
		this.className = "app_hrmis_gaji_transaksi_fUploadInsentif";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Upload Insentif", 0);	
		...
		this.sg1 = new portalui_saiGrid(this.p1,{bound:[0,20,900,200],colCount:5,
				colTitle:["NIK","Nama","Lembur","Tunj. Khusus","Rapel"],readOnly:true,
...
window.app_hrmis_gaji_transaksi_fUploadInsentif.extend(window.portalui_childForm);
window.app_hrmis_gaji_transaksi_fUploadInsentif.implement({
		...
		switch (event){
			...
			case "simpan" :
					try{
						...
						var sqlText = "insert into gaji_load_pdpt_d (no_bukti, no_urut,nik,lmbr,tkhs,rapl,kode_lokasi,periode)values";
						var sqlArray= [];		
						var fieldName = ["LMBR", "TKHS","RAPL"];
