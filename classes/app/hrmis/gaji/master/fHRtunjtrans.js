window.app_hrmis_gaji_master_fHRtunjtrans = function(owner)
{
	if (owner)
	{
		window.app_hrmis_gaji_master_fHRtunjtrans.prototype.parent.constructor.call(this,owner);
		this.className  = "app_hrmis_gaji_master_fHRtunjtrans";
		this.maximize();
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Tunjangan Transport", 0);
		...
		this.sg1.setTag("8");
		this.sg1.setColTitle(["No","Kode","Nama Jabatan","Tarif"]);
...
window.app_hrmis_gaji_master_fHRtunjtrans.extend(window.portalui_childForm);
window.app_hrmis_gaji_master_fHRtunjtrans.prototype.mainButtonClick = function(sender)
{
...
window.app_hrmis_gaji_master_fHRtunjtrans.prototype.doModalResult = function(event, modalResult)
{
	switch (event)
	{
		case "clear" :
			...
			if (modalResult == mrOk)
			{
				if (this.standarLib.checkEmptyByTag(this, new Array("0","2")))
				{
					...
					{					
						...
						sql.add("insert into hr_tunjtrans (kode_jab,tarif,kode_lokkonsol)  values "+
								"('"+this.ed_kode.getText()+"','"+parseNilai(this.ed_tarif.getText())+"','"+this.lokkonsol+"') ");
		...
		case "ubah" :
			if (modalResult == mrOk)
			{
					...
					sql.add(" update hr_tunjtrans set  "+
		...
		case "hapus" :
		   if (modalResult == mrOk)
		   {
				uses("server_util_arrayList");					
					var sql = new server_util_arrayList();
					sql.add("delete from hr_tunjtrans where kode_jab='"+this.ed_kode.getText()+"' and kode_lokkonsol='"+this.lokkonsol+"'");
					this.dbLib.execArraySQL(sql);	
		   }
			break;
	}
};
window.app_hrmis_gaji_master_fHRtunjtrans.prototype.showClick = function(sender)
{	
	if (this.ed_kode.getText() != "")
	{
		try
		{
			...
			var data = this.dbLib.loadQuery("select a.kode_jab,a.tarif "+
			                                "from hr_tunjtrans a "+
...
window.app_hrmis_gaji_master_fHRtunjtrans.prototype.tampilClick = function(sender)
{
	this.sg1.clearAll();
	var temp = this.dbLib.runSQL("select a.kode_jab,b.nama,a.tarif "+
	                             "from hr_tunjtrans a inner join hr_jabatan b on a.kode_jab=b.kode_jab and a.kode_lokkonsol = b.kode_lokkonsol "+
...
window.app_hrmis_gaji_master_fHRtunjtrans.prototype.FindBtnClick = function(sender, event)
{
	try
	{
		if (sender == this.ed_kode) 
		{
			this.standarLib.showListData(this, "Daftar Jabatan",sender,undefined, 
										  "select kode_jab, nama  from hr_jabatan where kode_lokkonsol='"+this.lokkonsol+"' and (jenis = 'STRUKTURAL' or kode_jab = 'TJF' ) ",
										  "select count(kode_jab) from hr_jabatan where kode_lokkonsol='"+this.lokkonsol+"' and (jenis = 'STRUKTURAL' or kode_jab = 'TJF' ) ",
...
window.app_hrmis_gaji_master_fHRtunjtrans.prototype.doRequestReady = function(sender, methodName, result)
{
