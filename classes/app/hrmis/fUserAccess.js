window.app_hrmis_fUserAccess = function(owner)
{
	if (owner)
	{
		window.app_hrmis_fUserAccess.prototype.parent.constructor.call(this,owner);
		this.className  = "app_hrmis_fUserAccess";
		this.maximize();
		...
		this.dbLib.getDataProviderPageA("select a.uid, ifnull(b.nama,a.uid) as nama, ifnull(c.nama, a.userloc) as lokasi, a.timein, a.session, a.timeout, a.host from userlog a "+
                          " left outer join lokasi c on c.kode_lokasi = a.userloc "+
                          " left outer join karyawan b on b.nik = a.uid and b.kode_lokasi = a.userloc "+
                          " where a.userloc like '"+this.app._lokasi+"%' order by a.timein desc, a.uid",1,this.pager);
 
        this.page = 1;                                  
        this.app._mainForm.pButton.show();
	}
};
window.app_hrmis_fUserAccess.extend(window.portalui_childForm);
window.app_hrmis_fUserAccess.implement({
    doSelectPage: function(sender, page){
        this.sg1.clear();
        this.dbLib.getDataProviderPageA("select a.uid, ifnull(b.nama,a.uid) as nama, ifnull(c.nama, a.userloc) as lokasi, a.timein, a.session, a.timeout, a.host from userlog a "+
                          " left outer join lokasi c on c.kode_lokasi = a.userloc "+
                          " left outer join karyawan b on b.nik = a.uid and b.kode_lokasi = a.userloc "+
                          " where a.userloc like '"+this.eLokasi.getText()+"%' order by a.timein desc, a.uid",page,this.pager);
        this.page = page;
    },
             ...
             if (sender === this.dbLib){
                switch(methodName){
                    case "getDataProviderPage" :
                       ...
                       this.sg1.setRowNumber((this.page - 1) * this.pager);
                    break;
                }
             }
         }catch(e){
    ...
    doFindBtnClick: function(sender){
        ...
        if (sender == this.eUser) 
           this.standarLib.showListData(this, "Daftar User",sender,undefined, 
											  "select a.uid, ifnull(b.nama,a.uid) as nama, a.host from userlog a  "+
    ...
    mainButtonClick: function(sender){
		...
		this.dbLib.getDataProviderPageA("select a.uid, ifnull(b.nama,a.uid) as nama, ifnull(c.nama, a.userloc) as lokasi, a.timein, a.session, a.timeout, a.host from userlog a "+
                          " left outer join lokasi c on c.kode_lokasi = a.userloc "+
                          " left outer join karyawan b on b.nik = a.uid and b.kode_lokasi = a.userloc "+
                          " where a.userloc like '"+this.eLokasi.getText()+"%' order by a.timein desc, a.uid",1,this.pager);
