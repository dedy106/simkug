...
//* 			SAI, PT											
//***********************************************************************************************
window.app_hrmis_fLogin = function(owner){
	if (owner){
		try{		
			window.app_hrmis_fLogin.prototype.parent.constructor.call(this, owner);
			this.className  = "app_hrmis_fLogin";			
			...
			systemAPI.alert("[app_hrmis_fLogin]::oncreate lib : ",e);
		}
	}
};
window.app_hrmis_fLogin.extend(window.portalui_childForm);
window.app_hrmis_fLogin.implement({
    mainButtonClick: function(sender){
		try{
			...
			var data = runArraySQL("select a.*,b.nama from hakakses a inner join lokasi b on b.kode_lokasi = a.kode_lokasi where nik= '"+this.e0.getText()+"' and pass = '"+this.e1.getText()+"'\r\n"+
						...
						"select kode_lokkonsol from lokasi where kode_lokasi = 'table1_kode_lokasi'","svrvar");			
