window.app_saku2_report_ak_flAkSaldoImt = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_ak_flAkSaldoImt.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_ak_flAkSaldoImt";
		this.maximize();
...
window.app_saku2_report_ak_flAkSaldoImt.extend(window.childForm);
window.app_saku2_report_ak_flAkSaldoImt.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["3","6","123","123","123","123","3","3"]);
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,6)=="Jurusan")
				{
					this.filter = this.filterRep.filterStr("x.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("z.kode_jenis",this.sg1.getCell(1,7),this.sg1.getCell(2,7),this.sg1.getCell(3,7),"and");
					...
					var sql = "select a.kode_jur,a.nama,"+param1+param2+
							...
							"			inner join aka_produk z on x.kode_produk=z.kode_produk and x.kode_lokasi=z.kode_lokasi "+this.filter+
							"			group by y.kode_jur "+
							"			)b on a.kode_jur=b.kode_jur "+
							"left join (select y.kode_jur, "+param3+
							"		    from aka_rekon_d x "+
							"			inner join aka_mahasiswa y on x.nim=y.nim and x.kode_lokasi=y.kode_lokasi "+
							"			inner join aka_produk z on x.kode_produk=z.kode_produk and x.kode_lokasi=z.kode_lokasi "+this.filter2+
							"		   group by y.kode_jur "+
			...
			systemAPI.alert("[app_saku2_report_ak_flAkSaldoImt]::mainButtonClick:"+e);
		}
	},
	initColumn: function(){
		try{		
