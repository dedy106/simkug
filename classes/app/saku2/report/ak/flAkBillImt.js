window.app_saku2_report_ak_flAkBillImt = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_ak_flAkBillImt.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_ak_flAkBillImt";
		this.maximize();
...
window.app_saku2_report_ak_flAkBillImt.extend(window.childForm);
window.app_saku2_report_ak_flAkBillImt.implement({
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,6)=="Tagihan")
				{
					...
					var sql = "select b.no_inv,b.nim,c.nama,c.kode_jur,c.kode_akt,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.keterangan, "+param1+
							"from aka_bill_m a "+
							"inner join (select distinct a.no_bill,a.no_inv,a.nim "+
							"			from aka_bill_d a "+
							"			inner join aka_produk b on a.kode_produk=b.kode_produk and a.kode_lokasi=b.kode_lokasi "+this.filter3+
							...
							"			from aka_bill_d a "+
							"			inner join aka_produk b on a.kode_produk=b.kode_produk and a.kode_lokasi=b.kode_lokasi "+this.filter3+
							"		   group by a.no_inv)d on b.no_inv=d.no_inv "+
					...
					this.title = new server_util_arrayList({items:["No Invoice","NIM","Nama","Jurusan","Angkatan","Tanggal","Keterangan"]});
					this.widthTable = new server_util_arrayList({items:[80,80,200,60,60,60,200]});
				...
				{
					var modul=" and a.modul='TAGIH' ";
					var sql = "select a.no_bill,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.kode_akun,b.nama,a.kode_pp,a.kode_drk,a.modul,a.keterangan, "+
						...
						"order by a.periode,a.no_bill,a.dc desc  ";
			...
			systemAPI.alert("[app_saku2_report_ak_flAkBillImt]::mainButtonClick:"+e);
		}
	},
	initColumn: function(){
		try{		
