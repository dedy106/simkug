window.app_saku2_report_ak_flAkRekon = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_ak_flAkRekon.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_ak_flAkRekon";
		this.maximize();
...
window.app_saku2_report_ak_flAkRekon.extend(window.childForm);
window.app_saku2_report_ak_flAkRekon.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,6)=="Rekon")
				{
					...
					var sql = "select a.no_rekon,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.keterangan,a.nim,b.nama,c.no_inv,"+param1+
							"from aka_rekon_m a "+
							"inner join (select distinct no_rekon,nim,no_inv "+
							"			from aka_rekon_d "+
			...
			systemAPI.alert("[app_saku2_report_ak_flAkRekon]::mainButtonClick:"+e);
		}
	},
	initColumn: function(){
		try{		
