window.app_saku2_report_ak_flAkTitipImt = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_ak_flAkTitipImt.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_ak_flAkTitipImt";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan KasBAnk Titipan", 2);
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from aka_titip_m ");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Bentuk Laporan","=","KasBank"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	this.doSelectCell(this.sg1,2,3);
};
window.app_saku2_report_ak_flAkTitipImt.extend(window.childForm);
window.app_saku2_report_ak_flAkTitipImt.implement({
		...
		try{			
			...
			if (row ==5)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Amortisasi",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_titip,a.keterangan from aka_titip_m a "+this.filter,
												"select count(a.no_titip) from aka_titip_m a "+this.filter,
												new Array("a.no_titip","a.keterangan"),"and",new Array("No Invoice","Periode"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from aka_titip_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_titip",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				...
				if (this.sg1.getCell(2,3)=="KasBank")
				{
					var sql = "select a.no_titip,a.no_dokumen,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.keterangan,a.akun_titip,c.nama as nama_titip, "+
							"	   b.akun_kb,d.nama as nama_kb,a.nilai "+
							"from aka_titip_m a "+
							...
							"inner join masakun d on b.akun_kb=d.kode_akun and b.kode_lokasi=d.kode_lokasi "+
							this.filter+" order by a.no_titip ";
					this.scriptSqlCount =  "select a.no_titip,a.no_dokumen,a.tanggal,a.keterangan,a.akun_titip,c.nama as nama_titip, "+
							"	   b.akun_kb,d.nama as nama_kb,a.nilai "+
							"from aka_titip_m a "+
							"inner join kas_m b on a.no_kas=b.no_kas and a.kode_lokasi=b.kode_lokasi "+
							"inner join masakun c on a.akun_titip=c.kode_akun and a.kode_lokasi=c.kode_lokasi "+
							"inner join masakun d on b.akun_kb=d.kode_akun and b.kode_lokasi=d.kode_lokasi "+this.filter;
					
					this.title = new server_util_arrayList({items:["No Bukti","No Dokumen","Tanggal","Keterangan","Akun Titipan","Nama Akun Titipan","Akun KasBank","Nama Akun KasBank","Nilai"]});
					this.widthTable = new server_util_arrayList({items:[80,80,60,200,60,200,60,200,90]});
				...
				{
					var modul=" and a.modul='KBTTP' ";
					var sql = "select a.no_kas,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.kode_akun,b.nama,a.kode_pp,a.kode_drk,a.modul,a.keterangan, "+
			...
			systemAPI.alert("[app_saku2_report_ak_flAkTitipImt]::mainButtonClick:"+e);
		}
	},
