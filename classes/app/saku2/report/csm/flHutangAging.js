window.app_saku2_report_csm_flHutangAging = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_csm_flHutangAging.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_csm_flHutangAging";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from csm_hutang_m ");
...
window.app_saku2_report_csm_flHutangAging.extend(window.childForm);
window.app_saku2_report_csm_flHutangAging.implement({
		...
		try{			
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Vendor",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_vendor, nama from csm_vendor "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(kode_vendor) from csm_vendor "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_hutang, a.keterangan from csm_hutang_m a "+filter,
												"select count(b.no_hutang) from csm_hutang_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from csm_hutang_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku2_csm_rptHutangAging";
			...
			systemAPI.alert("[app_saku2_report_csm_flHutangAging]::mainButtonClick:"+e);
		}
	},
