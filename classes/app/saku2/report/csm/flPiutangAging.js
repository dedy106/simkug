window.app_saku2_report_csm_flPiutangAging = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_csm_flPiutangAging.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_csm_flPiutangAging";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from csm_piutang_m ");
...
window.app_saku2_report_csm_flPiutangAging.extend(window.childForm);
window.app_saku2_report_csm_flPiutangAging.implement({
		...
		try{			
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Customer",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_cust, nama from csm_cust "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(kode_cust) from csm_cust "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_piutang, a.keterangan from csm_piutang_m a "+filter,
												"select count(b.no_piutang) from csm_piutang_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from csm_piutang_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku2_csm_rptPiutangAging";
			...
			systemAPI.alert("[app_saku2_report_csm_flPiutangAging]::mainButtonClick:"+e);
		}
	},
