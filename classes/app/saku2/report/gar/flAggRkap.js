window.app_saku2_report_gar_flAggRkap = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_gar_flAggRkap.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_gar_flAggRkap";
		this.maximize();
	...
	if (this.app._kodeLokasiKonsol==this.app._lokasi)
	{
		...
		this.bid1=this.dbLib.getPeriodeFromSQL("select min(kode_bidang) as periode from pp");
		this.bid2=this.dbLib.getPeriodeFromSQL("select max(kode_bidang) as periode from pp");
	}
	else
	{
		...
		this.bid1=this.dbLib.getPeriodeFromSQL("select min(kode_bidang) as periode from pp where kode_pp='"+this.app._kodePP+"'");
		this.bid2=this.dbLib.getPeriodeFromSQL("select max(kode_bidang) as periode from pp where kode_pp='"+this.app._kodePP+"'");
	}
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2,3), new Array("Kode Bidang ","Range",this.bid1,this.bid2));
	this.gridLib.SGEditData(this.sg1,5,[0,1,2],["Jenis Laporan","=","Realisasi"]);
...
window.app_saku2_report_gar_flAggRkap.extend(window.portalui_childForm);
window.app_saku2_report_gar_flAggRkap.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row ==4)
		{
			this.filterRep.ListDataSGFilter(this, "Data Bidang",this.sg1, this.sg1.row, this.sg1.col,
											...
											new Array("kode_bidang","nama"),"where",new Array("kode","nama"));
		}
	},
	doSelectCell: function(sender, col, row){
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["123","3","123","3","123","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,0,2,0,2,0]);
		}else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["2","3","3","3","123","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[3,0,2,0,2,0]);
		}
		if (row == 1)
		{
			if (this.app._kodeLokasiKonsol==this.app._lokasi)
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["Realisasi"]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			this.filter2 = this.nik_user+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,3)+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(3,0)+"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(3,4);
			if (this.app._kodeLokasiKonsol==this.app._lokasi)
			{
			...
			if (this.sg1.getCell(2,5)=="Realisasi")
			{				
				var sql="call sp_rkap ('"+this.sg1.getCell(2,2)+"','"+this.sg1.getCell(2,1)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.app._nikUser+"','"+this.sg1.getCell(2,3)+"','"+this.sg1.getCell(2,4)+"','"+this.sg1.getCell(3,4)+"')";
				this.nama_report="server_report_saku2_gar_rptAggRkap";
			}
			if (this.sg1.getCell(2,5)=="Anggaran")
			{
				var sql="call sp_rkap_gar ('"+this.sg1.getCell(2,2)+"','"+this.sg1.getCell(2,1)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.app._nikUser+"','"+this.sg1.getCell(2,3)+"','"+this.sg1.getCell(2,4)+"','"+this.sg1.getCell(3,4)+"')";
				this.nama_report="server_report_saku2_gar_rptAggRkap";
			}			
