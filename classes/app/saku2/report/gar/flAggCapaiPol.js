window.app_saku2_report_gar_flAggCapaiPol = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_gar_flAggCapaiPol.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_gar_flAggCapaiPol";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from agg_d where kode_lokasi='"+this.app._lokasi+"' ");
	...
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Periode 2","=",this.periode]);
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Kode Akun","All",""]);
	this.gridLib.SGEditData(this.sg1,4,[0,1,2],["Kode PP","=",this.app._kodePP]);
	this.gridLib.SGEditData(this.sg1,5,[0,1,2],["Kode DRK","All",""]);
	this.gridLib.SGEditData(this.sg1,6,[0,1,2],["Jenis Akun","=","Beban"]);
	this.gridLib.SGEditData(this.sg1,7,[0,1,2],["Jenis Mutasi","=","Saldo"]);
...
window.app_saku2_report_gar_flAggCapaiPol.extend(window.portalui_childForm);
window.app_saku2_report_gar_flAggCapaiPol.implement({
	...
	doSelectCell: function(sender, col, row){
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["3","3","3","123","123","123","3","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[2,0,0,2,2,2,0,0,0]);
		}
		if (this.app._userStatus=="U")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["3","3","3","123","3","123","3","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[3,0,0,2,2,3,0,0,0]);
		}
		if (this.app._userStatus=="P")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["3","3","3","123","23","123","3","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[3,0,0,2,2,2,0,0,0]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct(periode) as periode from agg_d "+
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2=this.nik_user+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2);		
				if (this.sg1.getCell(2,8)=="Anggaran")
				{
					var sql = "call sp_agg_capai ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,6)+"','"+this.sg1.getCell(2,7)+"','"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,2)+"','"+this.nik_user+"')";
				}
				else
				{
					var sql = "call sp_agg_capai_gl ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,6)+"','"+this.sg1.getCell(2,7)+"','"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,2)+"','"+this.nik_user+"')";
				}
			
				this.nama_report="server_report_saku2_gar_rptAggCapai2";
	...
	doOpenKartu: function(kode_akun,kode_lokasi,tahun,kode_pp,kode_drk){
		...
		if (this.sg1.getCell(2,8)=="Anggaran")
		{
			var sql = "call sp_angg_d ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,6)+"','"+tahun+"','"+this.nik_user+"')";
			this.dbLib.execQuerySync(sql);	
		}
		...
		filter2=tahun+"/"+this.sg1.getCell(2,8)+"/"+this.app._nikUser;
