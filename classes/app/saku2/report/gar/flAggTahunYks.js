window.app_saku2_report_gar_flAggTahunYks = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_gar_flAggTahunYks.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_gar_flAggTahunYks";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,6,[0,1,2],["Jenis Anggaran","=","Beban"]);
	this.gridLib.SGEditData(this.sg1,7,[0,1,2],["Periodik Laporan","=","Triwulan"]);
	this.gridLib.SGEditData(this.sg1,8,[0,1,2],["Status Anggaran","=","Anggaran"]);
...
window.app_saku2_report_gar_flAggTahunYks.extend(window.portalui_childForm);
window.app_saku2_report_gar_flAggTahunYks.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["123","123","123","123","123","123","3","3","3"]);
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["Anggaran","Generak Ledger"]);
		}
	},
		...
		try
		{
			...
			else
			{
				...
				if (this.sg1.getCell(2,7)=="Triwulan")
				{
					...
					{
						var sql = "call sp_agg_real_tw ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,6)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					}
					else
					{
						var sql = "call sp_agg_real_tw_gl ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,6)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					}
					this.nama_report="server_report_saku2_gar_rptAggRealTwYks";
				}
				if (this.sg1.getCell(2,7)=="Semester")
				{
					if (this.sg1.getCell(2,8)=="Anggaran")
					{
						var sql = "call sp_agg_real_sem ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,6)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					}
					else
					{
						var sql = "call sp_agg_real_sem_gl ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,6)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					}
					this.nama_report="server_report_saku2_gar_rptAggRealSemYks";
				}
				if (this.sg1.getCell(2,7)=="Bulanan")
				{
					if (this.sg1.getCell(2,8)=="Anggaran")
					{
						var sql = "call sp_agg_real_bulan ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,6)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					}
					else
					{
						var sql = "call sp_agg_real_bulan_gl ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,6)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					}
					this.nama_report="server_report_saku2_gar_rptAggRealBulanYks";
				}
	...
	doOpenKartu: function(kode_akun,kode_lokasi,tahun,kode_pp,kode_drk){
		...
		{
			var sql = "call sp_angg_d ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,6)+"','"+tahun+"','"+this.app._nikUser+"')";
			this.dbLib.execQuerySync(sql);	
		}
		...
		filter2=this.sg1.getCell(2,6)+"/"+tahun+"/"+kode_pp+"/"+kode_drk+"/"+tahun+"/"+this.app._nikUser+"/"+this.sg1.getCell(2,8)+"/"+this.sg1.getCell(2,3)+"/"+this.nama_bidang+"/"+"1";
