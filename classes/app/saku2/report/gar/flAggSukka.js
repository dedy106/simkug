window.app_saku2_report_gar_flAggSukka = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_gar_flAggSukka.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_gar_flAggSukka";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from rra_sukka ");
...
window.app_saku2_report_gar_flAggSukka.extend(window.portalui_childForm);
window.app_saku2_report_gar_flAggSukka.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			...
			var sql1="select a.no_sukka,a.keterangan from rra_sukka a "+this.filter;
			var sql2="select count(a.no_sukka) from rra_sukka a "+this.filter;
			this.filterRep.ListDataSGFilter(this, "Data Anggaran",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,["a.no_sukka","a.keterangan"],"and",["No SUKKA","Keterangan"]);
		}
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct(periode) as periode from rra_sukka "+
									   "where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct(jenis) as periode from anggaran_m "+
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
					this.filterRep.filterStr("a.no_sukka",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
			this.nama_report="server_report_saku2_gar_rptAggSukka";	
