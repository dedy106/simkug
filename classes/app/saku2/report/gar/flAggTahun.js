window.app_saku2_report_gar_flAggTahun = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_gar_flAggTahun.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_gar_flAggTahun";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,7,[0,1,2],["Realisasi","=","Anggaran"]);
...
window.app_saku2_report_gar_flAggTahun.extend(window.portalui_childForm);
window.app_saku2_report_gar_flAggTahun.implement({
	...
	doSelectCell: function(sender, col, row){
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["123","123","123","123","123","3","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[2,0,2,2,2,0,0,0]);
		}
		if (this.app._userStatus=="U")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["3","3","123","3","123","3","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[3,0,2,3,2,0,0,0]);
		}
		if (this.app._userStatus=="P")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["3","3","123","23","123","3","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[3,0,2,2,2,0,0,0]);
		}
		if (row == 1)
		{
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["Closing","Original"]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter=this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and")+
							this.filterRep.filterStr("substring(periode,1,4)",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and") +
				...
				if (this.sg1.getCell(2,6)=="Triwulan")
				{
					...
					{
						var sql = "call sp_agg_real_tw ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					}
					else
					{
						var sql = "call sp_agg_real_tw_gl ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					}
					this.nama_report="server_report_saku2_gar_rptAggRealTw";
				}
				if (this.sg1.getCell(2,6)=="Semester")
				{
					if (this.sg1.getCell(2,7)=="Anggaran")
					{
						var sql = "call sp_agg_real_sem ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					}
					else
					{
						var sql = "call sp_agg_real_sem_gl ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					}
					this.nama_report="server_report_saku2_gar_rptAggRealSem";
				}
				if (this.sg1.getCell(2,6)=="Bulanan")
				{
					if (this.sg1.getCell(2,7)=="Anggaran")
					{
						var sql = "call sp_agg_real_bulan ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					}
					else
					{
						var sql = "call sp_agg_real_bulan_gl ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					}
					this.nama_report="server_report_saku2_gar_rptAggRealBulan";
				}
	...
	doOpenGar: function(kode_akun,kode_lokasi,tahun,kode_pp,kode_drk,tw){
		...
		filter2=this.app._nikUser+"/"+tahun+"/"+tw;
		nama_report="server_report_saku2_gar_rptAggBulanTrail2";
	...
	doOpenKartu: function(kode_akun,kode_lokasi,tahun,kode_pp,kode_drk,tw){
		this.kode_akun=kode_akun;
		this.kode_lokasi=kode_lokasi;
		this.tahun=tahun;
		...
		if (tw=="tw1")
		{
			tmp=" and substring(a.periode,5,2) between '01' and '03' "
		}
		if (tw=="tw2")
		{
			tmp=" and substring(a.periode,5,2) between '04' and '06' "
		}
		if (tw=="tw3")
		{
			tmp=" and substring(a.periode,5,2) between '07' and '09' "
		}
		if (tw=="tw4")
		{
			tmp=" and substring(a.periode,5,2) between '10' and '12' "
		}
		if (this.sg1.getCell(2,7)=="Anggaran")
		{
		...
		if (tw=="tw4")
		{
			tmp=" and a.periode between '10' and '12' "
		}
		...
		filter2=tahun+"/"+this.sg1.getCell(2,7)+"/"+this.app._nikUser+"/"+tmp;
