window.app_saku2_report_gar_flAggKpaAkunBln = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_gar_flAggKpaAkunBln.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_gar_flAggKpaAkunBln";
		this.maximize();
...
window.app_saku2_report_gar_flAggKpaAkunBln.extend(window.portalui_childForm);
window.app_saku2_report_gar_flAggKpaAkunBln.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 2)
		{
			var filter=this.filterRep.filterStr("substring(periode,1,4)",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," where ")+
					this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," and ");
			var filter2=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," and ");		
			this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,
													...
													"			group by kode_akun,kode_lokasi "+
													"			)b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+filter2,
													  new Array("a.kode_akun","a.nama"),"where",new Array("kode","nama"),"where");
		}
		...
		if (row == 4)
		{
			...
			var filter2=this.filterRep.filterStr("a.tahun",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," where ")+
			...
			this.filterRep.ListDataSGFilter(this, "Data DRK",this.sg1, this.sg1.row, this.sg1.col,
													"select a.kode_drk,a.nama "+
													"from drk a "+
													"inner join (select kode_drk,kode_lokasi "+ 
													"			from anggaran_d "+filter+
													"			group by kode_drk,kode_lokasi "+
													"			)b on a.kode_drk=b.kode_drk and a.kode_lokasi=b.kode_lokasi "+filter2,
													"select count(a.kode_drk) "+
													"from drk a "+
													"inner join (select kode_drk,kode_lokasi "+ 
													"			from anggaran_d "+filter+
													"			group by kode_drk,kode_lokasi "+
													"			)b on a.kode_drk=b.kode_drk and a.kode_lokasi=b.kode_lokasi "+filter2,
													  new Array("a.kode_drk","a.nama"),"where",new Array("kode","nama"),"where");
		}
	...
	doSelectCell: function(sender, col, row){
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["123","123","123","123","123","3","13","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[2,0,2,2,2,0,0,0]);
		}
		if (this.app._userStatus=="U")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["3","3","123","3","123","3","13","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[3,0,2,3,2,0,0,0]);
		}
		if (this.app._userStatus=="P")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["3","3","123","23","123","3","13","3"]);
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2=this.sg1.getCell(2,5)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser+"/"+this.sg1.getCell(2,6)+"/";
				this.nama_report="server_report_saku2_gar_rptAggKpaAkunBln";
