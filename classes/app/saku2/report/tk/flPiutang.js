window.app_saku2_report_tk_flPiutang = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_tk_flPiutang.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_tk_flPiutang";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from tk_piutang_m ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode Tingkat","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Siswa","All",""));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("No Invoice","All",""));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Progress","=","Summary"));
...
window.app_saku2_report_tk_flPiutang.extend(window.childForm);
window.app_saku2_report_tk_flPiutang.implement({
		...
		try{			
			...
			if (row ==	2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Tingkat",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_tingkat, nama from tk_tingkat "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(kode_tingkat) from tk_tingkat "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  new Array("kode_tingkat","nama"),"and");
			}
			if (row == 3)
			{
				var filter=this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
						 this.filterRep.filterStr("kode_tingkat",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," and ");
				this.filterRep.ListDataSGFilter(this, "Data Siswa",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_siswa, nama from tk_siswa "+filter,
														  "select count(kode_siswa) from tk_siswa "+filter,
														  new Array("kode_siswa","nama"),"and");
			}
			...
			if (row ==4)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("b.tingkat",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.kode_siswa",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_piutang, a.keterangan from tk_piutang_m a "+
												"inner join tk_siswa b on a.kode_lokasi=b.kode_lokasi and a.kode_siswa=b.kode_siswa "+filter,
												"select count(a.no_piutang) from tk_piutang_m a "+
												"inner join tk_siswa b on a.kode_lokasi=b.kode_lokasi and a.kode_siswa=b.kode_siswa "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from tk_piutang_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 5)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
				this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Periode","=",this.periode));
				this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Siswa","All",""));
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," and ")+
							this.filterRep.filterStr("b.kode_tingkat",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.kode_siswa",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
				...
				if (this.sg1.getCell(2,5)=="Summary")
				{
					this.nama_report = "server_report_saku2_tk_rptPiutang";
				}
				else
				{
					this.nama_report = "server_report_saku2_tk_rptPiutangDetail";
				}
			...
			systemAPI.alert("[app_saku2_report_tk_flPiutang]::mainButtonClick:"+e);
		}
	},
