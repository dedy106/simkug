...
//* 			SAI, PT											
//***********************************************************************************************
window.app_saku2_report_siaga_flBukuBesar2 = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flBukuBesar2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flBukuBesar2";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Tampil Mutasi Nol","=","Ya"));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Tanggal","All",""));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Sort by","=","Tanggal, No. Bukti"));
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Versi Laporan","=","Standar"));
...
window.app_saku2_report_siaga_flBukuBesar2.extend(window.portalui_childForm);
window.app_saku2_report_siaga_flBukuBesar2.implement({
	...
	doSelectCell: function(sender, col, row){
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["1234","123","123","3","123","3","3","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[2,0,2,0,1,0,0,0]);
		...
		{			
			if (this.app._kodeLokasiKonsol==this.app._lokasi)
			{
				...
				this.dbLib.setItemsFromSQL("select distinct periode from periode order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
			}
			else
			{
				this.sg1.columns.get(2).pickList.clear();
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Standar","Audit","Atensi","Audit Ver 2"));
		}
		if (row === 7)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			...
			else
			{
				...
				this.filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," and ")+
							...
							this.filterRep.filterStr("a.kode_akun",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," and ")+tanggal;
				var mutasi="";
				if (this.sg1.getCell(2,3)=="Tidak")
				{
					mutasi=" and (a.so_awal<>0 or a.debet<>0 or a.kredit<>0 or a.so_akhir<>0) ";
				}
				var sort=" order by a.tanggal,a.no_bukti ";
				if (this.sg1.getCell(2,5)=="Tanggal, No. Dokumen")
				{
				...
				this.filter2 = this.app._namaForm+"/"+filter_akun+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0)+"/"+this.app._nikUser+"/"+sort+"/"+this.sg1.getCell(2,7);
				this.nama_report="server_report_saku2_siaga_rptBukuBesar2";
				if (this.sg1.getCell(2,6)=="Audit")
				{
					this.nama_report="server_report_saku2_siaga_rptBukuBesar2Audit";
				}
				if (this.sg1.getCell(2,6)=="Atensi")
				{
					this.nama_report="server_report_saku2_siaga_rptBukuBesar2Atensi";
				}
				if (this.sg1.getCell(2,6)=="Audit Ver 2")
				{
					this.nama_report="server_report_saku2_siaga_rptBukuBesar2AuditTmp";
				}
