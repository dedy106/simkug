window.app_saku2_report_siaga_flTeccKas = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flTeccKas.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flTeccKas";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('KBTECC','KBTECC2')");
...
window.app_saku2_report_siaga_flTeccKas.extend(window.childForm);
window.app_saku2_report_siaga_flTeccKas.implement({
