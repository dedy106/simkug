window.app_saku2_report_siaga_flSpbJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flSpbJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flSpbJurnal";
		this.maximize();
...
window.app_saku2_report_siaga_flSpbJurnal.extend(window.childForm);
window.app_saku2_report_siaga_flSpbJurnal.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBSPB' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
