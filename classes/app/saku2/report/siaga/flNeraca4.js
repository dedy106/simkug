// JavaScript Document
window.app_saku2_report_siaga_flNeraca4 = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flNeraca4.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flNeraca4";
		this.maximize();
...
window.app_saku2_report_siaga_flNeraca4.extend(window.portalui_childForm);
window.app_saku2_report_siaga_flNeraca4.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report="server_report_saku2_gl_rptNeraca4";
	...
	doOpenTb: function(kode_neraca,kode_lokasi,periode){
		...
		this.link="1";
		filter2 = this.app._nikUser+"/"+periode ;
		filter="";
		var sql = "call sp_glma_trail_tmp ('"+this.sg1.getCell(2,2)+"','"+kode_neraca+"','"+kode_lokasi+"','"+kode_lokasi+"','"+kode_lokasi+"','"+this.periode+"','"+this.app._nikUser+"')";
	...
	doOpenBb: function(kode_akun,kode_lokasi,periode){
		...
		filter="";
		nama_report="server_report_saku2_siaga_rptBukuBesarTrail";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenJurnal: function(no_bukti,kode_lokasi,periode,modul){
		...
		{
			filter = "where a.no_kas='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' and a.periode='"+periode+"'";
			nama_report="server_report_saku2_siaga_rptBuktiBank";
		}
		if (modul.substr(0,2)=="FA")
		{
			filter = "where a.no_fasusut='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' and a.periode='"+periode+"'";
			nama_report="server_report_saku2_siaga_rptFaJurnal";
		}
		if (modul.substr(0,2)=="JU")
		{
			filter = "where a.no_ju='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' and a.periode='"+periode+"'";
			nama_report="server_report_saku2_siaga_rptJuJurnal";
		}
		if (modul.substr(0,2)=="PI")
		{
			filter = "where a.no_piutang='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' and a.periode='"+periode+"'";
			nama_report="server_report_saku2_siaga_rptPiutangJurnal";
		}
		if (modul.substr(0,2)=="HU")
		{
			filter = "where a.no_hutang='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' and a.periode='"+periode+"'";
			nama_report="server_report_saku2_siaga_rptHutangJurnal";
		}
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
