window.app_saku2_report_siaga_flBatalPU = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flBatalPU.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flBatalPU";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from ju_m where modul='JUPIUM' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_siaga_flBatalPU.extend(window.childForm);
window.app_saku2_report_siaga_flBatalPU.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_ju, keterangan from ju_m where modul='JUPIUM' "+filter ,
														  "select count(no_ju) from ju_m where modul='JUPIUM' "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from ju_m where  modul='JUPIUM' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
