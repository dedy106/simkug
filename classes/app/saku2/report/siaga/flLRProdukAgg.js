// JavaScript Document
window.app_saku2_report_siaga_flLRProdukAgg = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flLRProdukAgg.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flLRProdukAgg";
		this.maximize();
...
window.app_saku2_report_siaga_flLRProdukAgg.extend(window.portalui_childForm);
window.app_saku2_report_siaga_flLRProdukAgg.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 2)
		{
			this.filterRep.ListDataSGFilter(this, "Data FS",this.sg1, this.sg1.row, this.sg1.col,
													  "select kode_fs,nama from fsproduk "+
													  ...
													  "select count(kode_fs) from fsproduk "+
	...
	doSelectCell: function(sender, col, row){
		...
		else
		{
			...
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4),new  Array(3,0,0,0));
		}
		if (row == 1)
		{
			if (this.app._kodeLokasiKonsol==this.app._lokasi)
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="1")
				{
					this.nama_report="server_report_saku2_siaga_rptLRAgg";
			...
			else
			{
				...
				//var filter2 = this.nik_user+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,3);
				this.filter2 = this.nik_user+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(2,3)+"/"+this.app._namaForm;
				var in_lap="S";
				...
				sql="call sp_neraca_produk_agg ('"+this.sg1.getCell(2,2)+"','A','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,1)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.nik_user+"')";
	...
	doOpenTb: function(kode_neraca,kode_lokasi){
		...
		filter="";
		var sql = "call sp_glma_produk_trail_tmp ('"+this.sg1.getCell(2,2)+"','"+kode_neraca+"','"+kode_lokasi+"','"+kode_lokasi+"','"+kode_lokasi+"','"+this.sg1.getCell(2,1)+"','"+this.app._nikUser+"')";
	...
	doOpenBb: function(kode_akun,kode_lokasi){
		...
		filter="";
		this.nama_report="server_report_saku2_siaga_rptBukuBesarTrail";
		this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenGar: function(kode_neraca,kode_lokasi,tahun){
		this.kode_neraca=kode_neraca;
		this.kode_lokasi=kode_lokasi;
		this.tahun=tahun;
		this.link="1";
		filter = "where z.kode_neraca='"+kode_neraca+"' and z.kode_lokasi='"+kode_lokasi+"' and substring(x.periode,1,4)='"+tahun+"' ";
		filter2="";
		nama_report="server_report_saku2_siaga_rptAggBulanNeracaProdukTrail";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	},
