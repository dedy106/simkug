window.app_saku2_report_siaga_flPjKurangJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flPjKurangJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flPjKurangJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBPJPTG'");
...
window.app_saku2_report_siaga_flPjKurangJurnal.extend(window.childForm);
window.app_saku2_report_siaga_flPjKurangJurnal.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"'  and modul='KBPJPTG' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
