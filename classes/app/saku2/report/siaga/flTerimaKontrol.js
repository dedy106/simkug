window.app_saku2_report_siaga_flTerimaKontrol = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flTerimaKontrol.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flTerimaKontrol";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_terima_m ");
...
window.app_saku2_report_siaga_flTerimaKontrol.extend(window.childForm);
window.app_saku2_report_siaga_flTerimaKontrol.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Penerimaan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_terima, a.keterangan from gr_terima_m a "+this.filter,
												"select count(*) from gr_terima_m a "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gr_terima_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_terima",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_siaga_rptTerimaKontrol";
			...
			systemAPI.alert("[app_saku2_report_siaga_flTerimaKontrol]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku2_siaga_rptBuktiTerima";
