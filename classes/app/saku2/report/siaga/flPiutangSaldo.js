window.app_saku2_report_siaga_flPiutangSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flPiutangSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flPiutangSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_piutang_m ");
	this.periode2=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_piutangbayar_d ");
...
window.app_saku2_report_siaga_flPiutangSaldo.extend(window.childForm);
window.app_saku2_report_siaga_flPiutangSaldo.implement({
		...
		try{			
			...
			if (row == 3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Customer",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_cust, nama from gr_cust "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(kode_cust) from gr_cust "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_piutang, a.keterangan from gr_piutang_m a "+filter,
												"select count(a.no_piutang) from gr_piutang_m a "+filter,
	...
	{		
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from gr_piutang_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2 = this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,5);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_siaga_rptPiutangSaldo";
			...
			systemAPI.alert("[app_saku2_report_siaga_flPiutangSaldo]::mainButtonClick:"+e);
		}
	},
