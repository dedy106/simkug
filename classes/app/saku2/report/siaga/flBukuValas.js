window.app_saku2_report_siaga_flBukuValas = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flBukuValas.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flBukuValas";
		this.maximize();
...
window.app_saku2_report_siaga_flBukuValas.extend(window.portalui_childForm);
window.app_saku2_report_siaga_flBukuValas.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 2)
		{
			this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,
													...
													"where a.block= '0' and a.kode_curr<>'IDR' and (b.kode_flag='001' or b.kode_flag='009') "+
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct(periode) as periode from kas_j where kode_curr<>'IDR' "+
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report="server_report_saku2_siaga_rptBukuValas";
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser;
				//var sql = "call sp_glma_kas_tmp ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,1)+"','"+this.app._nikUser+"')";
				//this.dbLib.execQuerySync(sql)
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser+"/"+this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
