// JavaScript Document
window.app_saku2_report_siaga_flTb = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flTb.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flTb";
		this.maximize();
...
window.app_saku2_report_siaga_flTb.extend(window.portalui_childForm);
window.app_saku2_report_siaga_flTb.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2 = this.nik_user+"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,5);
				...
				if (this.app._lokasi == this.app._kodeLokasiKonsol)
				{
					...
					this.filter = this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
						this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.app._lokasi,this.app._lokasi,"and")+
						this.filterRep.filterStr("kode_akun",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				...
				{
					this.sql = "call sp_glma_tmp ('"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					this.nama_report="server_report_saku2_gl_rptTBPercobaan";
				}
				if (this.sg1.getCell(2,3)=="Neraca Lajur")
				{
					this.sql = "call sp_glma_tmp ('"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";								
					this.nama_report="server_report_saku2_siaga_rptTBLajur";
					
				}
				if (this.sg1.getCell(2,3)=="12 Periode")
				{
					this.sql = "call sp_glma12_tmp ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
