window.app_saku2_report_siaga_flPjLebihJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flPjLebihJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flPjLebihJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBPJPTGL'");
...
window.app_saku2_report_siaga_flPjLebihJurnal.extend(window.childForm);
window.app_saku2_report_siaga_flPjLebihJurnal.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"'  and modul='KBPJPTGL' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
