window.app_saku2_report_siaga_flNpko2 = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flNpko2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flNpko2";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_npko_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_siaga_flNpko2.extend(window.childForm);
window.app_saku2_report_siaga_flNpko2.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gr_npko_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
