// JavaScript Document
window.app_saku2_report_siaga_flLRProduk = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flLRProduk.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flLRProduk";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Tanda Tangan","=","Tidak"));
...
window.app_saku2_report_siaga_flLRProduk.extend(window.portalui_childForm);
window.app_saku2_report_siaga_flLRProduk.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 2)
		{
			this.filterRep.ListDataSGFilter(this, "Data FS",this.sg1, this.sg1.row, this.sg1.col,
													  ...
													  new Array("kode_fs","nama"),"where",new Array("kode","nama"));
		}
	},
	doSelectCell: function(sender, col, row){
		...
		else
		{
			...
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5),new  Array(3,0,2,0,0,0));
		}
		if (row == 1)
		{
			if (this.app._kodeLokasiKonsol==this.app._lokasi)
		...
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="1")
				{
					...
					this.nama_report = "server_report_saku2_gl_rptLRProduk";
			...
			else
			{
				...
				this.judul=this.dbLib.getPeriodeFromSQL("select nama as periode from fsproduk where kode_fs='"+this.sg1.getCell(2,2)+"' ");
				this.filter2 = this.nik_user+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(2,3)+"/"+this.judul+"/"+this.kode_lokasi+"/"+this.sg1.getCell(2,5);
				
				this.nama_report="server_report_saku2_gl_rptLRProduk";
				sql="call sp_neraca_produk ('"+this.sg1.getCell(2,2)+"','A','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,1)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.nik_user+"')";
