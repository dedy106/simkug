window.app_saku2_report_siaga_flHutangJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flHutangJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flHutangJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_hutang_j where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_siaga_flHutangJurnal.extend(window.childForm);
window.app_saku2_report_siaga_flHutangJurnal.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_hutang, keterangan from gr_hutang_j  "+filter ,
														  "select count(no_hutang) from gr_hutang_j "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gr_hutang_j where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
