window.app_saku2_report_siaga_flHutangLogAging = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flHutangLogAging.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flHutangLogAging";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_kontrak ");
...
window.app_saku2_report_siaga_flHutangLogAging.extend(window.childForm);
window.app_saku2_report_siaga_flHutangLogAging.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_kontrak, a.keterangan from gr_kontrak a "+filter,
												"select count(b.no_kontrak) from gr_kontrak a "+filter,
												new Array("b.no_kontrak","b.keterangan"),"and",new Array("No Invoice","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gr_kontrak where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku2_siaga_rptHutangLogAging";
			...
			systemAPI.alert("[app_saku2_report_siaga_flHutangLogAging]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBayar: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter = "where a.no_kontrak='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
