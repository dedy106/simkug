window.app_saku2_report_siaga_flNppPanjar = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flNppPanjar.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flNppPanjar";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_npp_m where kode_lokasi='"+this.app._lokasi+"' and modul='PJNPKO' ");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("No NPP","All",""));
...
window.app_saku2_report_siaga_flNppPanjar.extend(window.childForm);
window.app_saku2_report_siaga_flNppPanjar.implement({
		...
		try{			
			...
			if (row == 3)
			{
				var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.modul='PJNPKO' ";
				var sql1="select a.no_npp,a.keterangan from gr_npp_m a "+filter; 
				var sql2="select count(a.no_npp) from gr_npp_m a "+filter;
				this.filterRep.ListDataSGFilter(this, "Data NPP",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
												["a.no_npp","a.keterangan"],"and",["No NPKO","Keterangan"], false, this.sg1.cells(1,row) == "in");
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["123i","123i","123i","123i"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3],[2,2,2,2]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from gr_npp_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
