window.app_saku2_report_siaga_flPjJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flPjJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flPjJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_panjarselesai_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_siaga_flPjJurnal.extend(window.childForm);
window.app_saku2_report_siaga_flPjJurnal.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_selesai, keterangan from gr_panjarselesai_m  "+filter ,
														  "select count(no_selesai) from gr_panjarselesai_m "+filter,
														  new Array("no_selesai","keterangan"),"and");
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gr_panjarselesai_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_saku2_siaga_rptPjJurnal";
