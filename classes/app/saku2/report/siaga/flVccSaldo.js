window.app_saku2_report_siaga_flVccSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flVccSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flVccSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_vcc_m ");
	this.periode2=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_vccbayar_d ");
...
window.app_saku2_report_siaga_flVccSaldo.extend(window.childForm);
window.app_saku2_report_siaga_flVccSaldo.implement({
		...
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select b.no_invoice, b.nama_cust from gr_vcc_m a "+
												"inner join gr_vcc_d b on a.no_vcc=b.no_vcc and a.kode_lokasi=b.kode_lokasi "+filter,
												"select count(b.no_invoice) from gr_vcc_m a "+
												"inner join gr_vcc_d b on a.no_vcc=b.no_vcc and a.kode_lokasi=b.kode_lokasi "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gr_vcc_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku2_siaga_rptVccSaldo";
			...
			systemAPI.alert("[app_saku2_report_siaga_flVccSaldo]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBayar: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku2_siaga_rptVccTrail";
	...
	doOpenPiutang: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_vcc='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._namaUser;
