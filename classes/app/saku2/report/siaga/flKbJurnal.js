window.app_saku2_report_siaga_flKbJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flKbJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flKbJurnal";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Kode Cabang","=",this.cab));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Kode KasBank","All",""));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1), new Array("No Bukti","All"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku2_report_siaga_flKbJurnal.extend(window.childForm);
window.app_saku2_report_siaga_flKbJurnal.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 4)
			{
				this.filterRep.ListDataSGFilter(this, "Data KasBank",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_bank, nama from gr_bank "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(kode_bank) from gr_bank "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
			...
			if (row == 5)
			{
				var filter=this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
					...
					this.filterRep.filterStr("kode_bank",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4)," and ")+
					" and modul='KBUMCUR' ";
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_kas, keterangan from kas_m  "+filter ,
														  "select count(no_kas) from kas_m "+filter,
														  new Array("no_kas","keterangan"),"and");
			}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["3","123","13","123","123","123"]);
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("CK","CD","BK","BD"));
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							...
							this.filterRep.filterStr("c.kode_pp",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3)," and ")+
							this.filterRep.filterStr("c.kode_bank",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4)," and ")+
							this.filterRep.filterStr("c.no_kas",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4)," and ");
				this.filter2=this.sg1.getCell(2,1);
				this.nama_report="server_report_saku2_siaga_rptKbJurnal";
