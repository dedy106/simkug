window.app_saku2_report_siaga_flPjPtg = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flPjPtg.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flPjPtg";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_panjarptg_m ");
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("No Pertanggungan","All",""));
...
window.app_saku2_report_siaga_flPjPtg.extend(window.childForm);
window.app_saku2_report_siaga_flPjPtg.implement({
		...
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Panjar",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_ptg, a.keterangan from gr_panjarptg_m a "+this.filter,
												"select count(*) from gr_panjarptg_m a "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gr_panjarptg_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku2_report_siaga_flPjPtg]::mainButtonClick:"+e);
		}
	},
