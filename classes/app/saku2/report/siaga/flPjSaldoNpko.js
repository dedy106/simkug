window.app_saku2_report_siaga_flPjSaldoNpko = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flPjSaldoNpko.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flPjSaldoNpko";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_panjar_m ");
...
window.app_saku2_report_siaga_flPjSaldoNpko.extend(window.childForm);
window.app_saku2_report_siaga_flPjSaldoNpko.implement({
		...
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Panjar",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_panjar, a.keterangan from gr_panjar_m a "+this.filter,
												"select count(*) from gr_panjar_m a "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gr_panjar_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku2_siaga_rptPjSaldoNpko";
			...
			systemAPI.alert("[app_saku2_report_siaga_flPjSaldoNpko]::mainButtonClick:"+e);
		}
	},
	...
	doOpenNpko: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_npko='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._namaUser;
		nama_report="server_report_saku2_siaga_rptNpko";
	...
	doOpenNpp: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_npp='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._namaUser;
		nama_report="server_report_saku2_siaga_rptNpp";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenSpb: function(no_bukti,kode_lokasi){
		...
		filter2=this.app._namaUser;
		nama_report="server_report_saku2_siaga_rptSpbPanjar";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenKasPtg: function(no_bukti,kode_lokasi){
		...
		{
			filter = "where a.no_selesai='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
			nama_report="server_report_saku2_siaga_rptPjJurnal";
		}
		filter2=this.app._namaUser;
