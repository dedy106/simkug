window.app_saku2_report_siaga_flPiutangJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_siaga_flPiutangJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_siaga_flPiutangJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_piutang_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_siaga_flPiutangJurnal.extend(window.childForm);
window.app_saku2_report_siaga_flPiutangJurnal.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_piutang, keterangan from gr_piutang_m  "+filter ,
														  "select count(no_piutang) from gr_piutang_m "+filter,
