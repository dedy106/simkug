window.app_saku2_report_gaji_flGajiInsPol = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_gaji_flGajiInsPol.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_gaji_flGajiInsPol";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from hr_gaji_m where kode_lokasi='"+this.app._lokasi+"' and modul='NONREG' ");
	this.no_gaji=this.dbLib.getPeriodeFromSQL("select max(no_gaji) as periode from hr_gaji_m where kode_lokasi='"+this.app._lokasi+"' and modul='NONREG' ");
...
window.app_saku2_report_gaji_flGajiInsPol.extend(window.childForm);
window.app_saku2_report_gaji_flGajiInsPol.implement({
	...
	doSelectCell: function(sender, col, row){		
		...
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["3","123i","123i","123i","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[0,0,2,2,2,0]);
		...
		try
		{
			...
			else
			{
				...
				else
				{
					...
					this.nama_report = "server_report_saku2_gaji_rptGajiInsPol";
