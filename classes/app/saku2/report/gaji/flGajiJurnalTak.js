window.app_saku2_report_gaji_flGajiJurnalTak = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_gaji_flGajiJurnalTak.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_gaji_flGajiJurnalTak";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from takterima_m  where kode_lokasi='"+this.app._lokasi+"' ");
	this.gaji=this.dbLib.getPeriodeFromSQL("select max(no_terima) as periode from takterima_m  where kode_lokasi='"+this.app._lokasi+"' ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("No Bukti","=",this.gaji));
...
window.app_saku2_report_gaji_flGajiJurnalTak.extend(window.childForm);
window.app_saku2_report_gaji_flGajiJurnalTak.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_terima, a.keterangan from takterima_m a "+filter,
												"select count(a.no_terima) from takterima_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from takterima_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku2_gaji_rptGajiJurnalTak";
			...
			systemAPI.alert("[app_saku2_report_gaji_flGajiJurnalTak]::mainButtonClick:"+e);
		}
	},
