window.app_saku2_report_gaji_flGajiDaftarPol = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_gaji_flGajiDaftarPol.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_gaji_flGajiDaftarPol";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Posisi Surat", 2);
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from hr_gaji_m where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,4,[0,1,2],["No Gaji","=",this.no_gaji]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku2_report_gaji_flGajiDaftarPol.extend(window.childForm);
window.app_saku2_report_gaji_flGajiDaftarPol.implement({
		...
		try{			
			...
			if (row == 3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Karyawan",this.sg1, this.sg1.row, this.sg1.col,
													"select a.nik, a.nama from hr_karyawan a where a.kode_lokasi = '"+this.app._lokasi+"' ",
													"select count(*) from hr_karyawan a where a.kode_lokasi = '"+this.app._lokasi+"' ",
			...
			if (row == 4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Gaji",this.sg1, this.sg1.row, this.sg1.col,
													"select a.no_gaji, a.keterangan from hr_gaji_m a "+filter,
													"select count(*) from hr_gaji_m a "+filter,
													["a.no_gaji","a.nama"],"and",["No Gaji","Keterangan"], false, this.sg1.cells(1,row) == "in");
			}	
		}catch(e){
			systemAPI.alert(e);
		}
	...
	doSelectCell: function(sender, col, row){		
		...
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["3","123i","123i","123i","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[0,0,2,2,2]);
		
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from hr_gaji_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
	},
		...
		try
		{
			...
			else
			{
				...
				else
				{
					...
					this.filter = this.filterRep.filterStr("g.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
								this.filterRep.filterStr("g.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
								this.filterRep.filterStr("a.sts_sdm",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
								this.filterRep.filterStr("a.nik",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
								this.filterRep.filterStr("g.no_gaji",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
					this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,4);
					this.showFilter = this.filterRep.showFilter(this.sg1);
					var result  = new arrayMap();		
					this.lokasi=this.app._namalokasi;
					this.nama_report = "server_report_saku2_gaji_rptGajiDaftarPol";
	...
	doOpenPph: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_hris_rptGajiPph";
	...
	doOpenAkdhk: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.nik='"+no_bukti+"' and a.periode='"+this.sg1.getCell(2,1)+"' ";
		filter2=this.sg1.getCell(2,1)+"/";
		nama_report="server_report_hris_rptGajiAkdhk";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenDplk: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.nik='"+no_bukti+"' and a.periode='"+this.sg1.getCell(2,1)+"' ";
		filter2=this.sg1.getCell(2,1)+"/";
		nama_report="server_report_hris_rptGajiDplk";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenJamsos: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.nik='"+no_bukti+"' and a.periode='"+this.sg1.getCell(2,1)+"' ";
		filter2=this.sg1.getCell(2,1)+"/";
		nama_report="server_report_hris_rptGajiJamsostek";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenSlip: function(no_gaji,nik,kode_lokasi){
		this.link="1";
		filter = "where a.nik='"+nik+"' and a.kode_lokasi='"+kode_lokasi+"'";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+no_gaji;
		nama_report="server_report_saku2_gaji_rptGajiSlipPol";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
