window.app_saku2_report_kopeg_bsm_flPrAwasPdpt = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_bsm_flPrAwasPdpt.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_bsm_flPrAwasPdpt";
		this.maximize();
	...
	var sql="select max(periode) as periode "+
			"from (select max(periode) as periode from sju_bill_m where kode_lokasi='"+this.app._lokasi+"' "+
			"union "+
			"select max(periode) as periode from sju_polisbayar_d where kode_lokasi='"+this.app._lokasi+"' "+
			"	)a ";
	...
	this.gridLib.SGEditData(this.sg1,7,new Array(0,1,2), new Array("No Polis","All",""));
...
window.app_saku2_report_kopeg_bsm_flPrAwasPdpt.extend(window.childForm);
window.app_saku2_report_kopeg_bsm_flPrAwasPdpt.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["123","123","123","123","123","123","123","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[2,0,0,2,2,2,2,2]);	
		
		if (row == 1)
		{
			var sql="select distinct periode from bsm_polis_m where kode_lokasi='"+this.app._lokasi+"' "+ 
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(1,1)+"/"+this.sg1.getCell(3,1)+"/"+this.sg1.getCell(2,2);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_kopeg_bsm_rptPrAwasPdpt";
			...
			systemAPI.alert("[app_saku2_report_kopeg_bsm_flPrAwasPdpt]::mainButtonClick:"+e);
		}
	},
