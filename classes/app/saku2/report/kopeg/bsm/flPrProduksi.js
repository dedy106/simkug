window.app_saku2_report_kopeg_bsm_flPrProduksi = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_bsm_flPrProduksi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_bsm_flPrProduksi";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from bsm_polis_m where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Penanggung","All",""));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Acc Exec","All",""));
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("No Polis","All",""));
	this.gridLib.SGEditData(this.sg1,7,new Array(0,1,2), new Array("Digit Koma","=","4"));
	this.gridLib.SGEditData(this.sg1,8,new Array(0,1,2), new Array("Output Laporan","=","Screen"));
...
window.app_saku2_report_kopeg_bsm_flPrProduksi.extend(window.childForm);
window.app_saku2_report_kopeg_bsm_flPrProduksi.implement({
		...
		try{			
			...
			if (row ==6)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Polis",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_polis, no_dok from bsm_polis_m a "+filter,
												"select count(a.no_polis) from bsm_polis_m a "+filter,
												new Array("a.no_polis","a.no_dok"),"and",new Array("No Polis","No Dok Polis"));
			}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["123","123","123","123","123","123","123","3","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[2,0,2,2,2,2,2,0,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from bsm_polis_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 7)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Screen","Excell"));
		}
	},
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("f.kode_klp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							...
							this.filterRep.filterStr("a.kode_pic",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+
							this.filterRep.filterStr("a.no_polis",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and");
				var klp = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.kode_klp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+klp+"/"+this.sg1.getCell(2,7)+"/"+this.sg1.getCell(2,8)+"/"+this.sg1.getCell(1,1)+"/"+this.sg1.getCell(3,1);
				
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_kopeg_bsm_rptPrProduksi";
			...
			systemAPI.alert("[app_saku2_report_kopeg_bsm_flPrProduksi]::mainButtonClick:"+e);
		}
	},
