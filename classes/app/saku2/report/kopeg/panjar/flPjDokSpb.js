window.app_saku2_report_kopeg_panjar_flPjDokSpb = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_panjar_flPjDokSpb.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_panjar_flPjDokSpb";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from spb_m where kode_lokasi='"+this.app._lokasi+"' and modul='PJ'");
...
window.app_saku2_report_kopeg_panjar_flPjDokSpb.extend(window.childForm);
window.app_saku2_report_kopeg_panjar_flPjDokSpb.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from spb_m where kode_lokasi='"+this.app._lokasi+"' and modul='PJ' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku2_report_kopeg_panjar_flPjDokSpb]::mainButtonClick:"+e);
		}
	},
