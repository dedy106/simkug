window.app_saku2_report_kopeg_kbitt_flHonorPos = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_kbitt_flHonorPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_kbitt_flHonorPos";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and form='HONOR' ");
	...
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Posted","All",""));
...
window.app_saku2_report_kopeg_kbitt_flHonorPos.extend(window.childForm);
window.app_saku2_report_kopeg_kbitt_flHonorPos.implement({
	...
	{		
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123","123","123","123","123","3","13"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[2,0,2,2,1,0,0]);	
		}
		else if (this.app._userStatus=="P")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123","123","23","123","123","3","13"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[3,0,2,2,1,0,0]);
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123","123","3","123","123","3","13"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[3,0,2,2,1,0,0]);
		}
		if (row == 1)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				{
					if(this.sg1.getCell(2,6) == "Posting"){
						var posted_from = "T";
					}else{
						var posted_from = "-";
					}
				...
				if (this.sg1.getCell(1,6)=="Range")
				{
					...
					if(this.sg1.getCell(3,6) == "Posting"){
						var posted_to = "T";
					}else{
						var posted_to = "-";
					}
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_aju",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+tanggal+posted+
							" and a.form in ('HONOR') ";
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,5);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_kopeg_kbitt_rptHonorPos";
			...
			systemAPI.alert("[app_saku2_report_kopeg_kbitt_flHonorPos]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku2_kopeg_kbitt_rptHonorTu";
