...
//* 			SAI, PT											
//***********************************************************************************************
window.app_saku2_report_kopeg_kbitt_flKbBbTu = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_kbitt_flKbBbTu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_kbitt_flKbBbTu";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Sort by","=","Tanggal, No. Dokumen"));
...
window.app_saku2_report_kopeg_kbitt_flKbBbTu.extend(window.portalui_childForm);
window.app_saku2_report_kopeg_kbitt_flKbBbTu.implement({
		...
		if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,
													...
													"inner join it_jeniskb b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
													"where a.block= '0'  "+
	...
	doSelectCell: function(sender, col, row){
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["1234","123","123","123","3"]);
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				var sort=" order by a.tanggal,a.no_kas ";
				if (this.sg1.getCell(2,4)=="Tanggal, No. Dokumen")
				{
				...
				var filter_akun=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							...
							" and a.nik_user='"+this.app._nikUser+"' ";
				
				this.filter2 = this.app._namaForm+"/"+filter_akun+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0)+"/"+this.app._nikUser+"/"+sort;
				this.nama_report="server_report_saku2_kopeg_kbitt_rptKbBukuBesar";
				this.sql = "call sp_glma_kasit_tmp ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,1)+"','"+this.app._nikUser+"')";	
				if (this.sg1.getCell(1,2)=="=")
				{
					this.sql = "call sp_glma_kas_akun_tmp ('"+this.sg1.getCell(2,2)+"','"+this.sg1.getCell(2,2)+"','"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,1)+"','"+this.app._nikUser+"')";	
				}
				if (this.sg1.getCell(1,2)=="Range")
				{
					this.sql = "call sp_glma_kas_akun_tmp ('"+this.sg1.getCell(2,2)+"','"+this.sg1.getCell(3,2)+"','"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,1)+"','"+this.app._nikUser+"')";	
				}
	...
	doOpenJurnal: function(no_bukti,kode_lokasi,periode){
		...
		filter2="";
		nama_report="server_report_saku2_kb_rptKasJurnal";
