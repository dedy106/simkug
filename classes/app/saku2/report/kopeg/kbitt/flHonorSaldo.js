window.app_saku2_report_kopeg_kbitt_flHonorSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_kbitt_flHonorSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_kbitt_flHonorSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and form='HONOR' ");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Kode dosen","All",""));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Jenis Laporan","=","Dosen"));
...
window.app_saku2_report_kopeg_kbitt_flHonorSaldo.extend(window.childForm);
window.app_saku2_report_kopeg_kbitt_flHonorSaldo.implement({
		...
		try{			
			...
			if (row ==	3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Dosen",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_dosen, nama from it_dosen "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(kode_dosen) from it_dosen "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
	...
	{		
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","123","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[3,0,2,0]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from it_aju_m where kode_lokasi='"+this.app._lokasi+"' order by substring(periode,1,4)",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Dosen","PP"));
		}
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("b.kode_dosen",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				if (this.sg1.getCell(2,4)=="Dosen")
				{
					this.nama_report = "server_report_saku2_kopeg_kbitt_rptHonorSaldo";
				}
				else
				{
					this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
					this.nama_report = "server_report_saku2_kopeg_kbitt_rptHonorSaldoPp";
				}
			...
			systemAPI.alert("[app_saku2_report_kopeg_kbitt_flHonorSaldo]::mainButtonClick:"+e);
		}
	},
