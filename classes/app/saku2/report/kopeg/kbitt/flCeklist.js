window.app_saku2_report_kopeg_kbitt_flCeklist = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_kbitt_flCeklist.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_kbitt_flCeklist";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from it_ajuapp_m where kode_lokasi='"+this.app._lokasi+"'  ");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Status","=","3.KasBank"));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("No Agenda","All",""));
...
window.app_saku2_report_kopeg_kbitt_flCeklist.extend(window.childForm);
window.app_saku2_report_kopeg_kbitt_flCeklist.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 2)
			{
				...
				if (this.app._userStatus=="A")
				{
					var sql="select a.kode_pp, a.nama "+
						"from pp a "+
						"inner join (select a.kode_pp,a.kode_lokasi from it_aju_m a "+
			...
			// 	var sql = "select distinct a.progress,case a.progress when '0' then 'Unit' "+
			// 			"		when 'A' then 'Unit' "+
			...
			// 	"from it_aju_m a "+
			// 	this.filter+" and progress in ('A','0','1','2','3','S','R') ";

			// 	var sql2 = "select count( distinct a.progress) as jum "+
			// 	"from it_aju_m a "+
			// 	this.filter+" and progress in ('A','0','1','2','3','S','R') ";
			...
			// 	this.filterRep.ListDataSGFilter(this, "Data Status",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("a.progress"),"and",new Array("Status","Keterangan"));
			// }
            ...
            if (row == 4)
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
                            " and b.jenis in ('ONLINE') ";
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_aju, a.keterangan from it_aju_m a inner join it_ajuapp_m b on a.no_aju=b.no_aju and a.kode_lokasi=b.kode_lokasi "+this.filter,
												"select count(a.no_aju) as jum from it_aju_m a inner join it_ajuapp_m b on a.no_aju=b.no_aju and a.kode_lokasi=b.kode_lokasi "+this.filter,
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["123","123","123","13","123","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,0,2,0,2,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from it_ajuapp_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		
		if (row === 3)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("A.Unit","0.Penyerahan Dokumen","1.Verifikasi Dokumen","2.Verifikasi Akun/Fiat","3.KasBank","S.SPB","R.Revisi"));
		}
		if (row === 5)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
                ...
                this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
                this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Periode","=",this.periode));
                this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("No Agenda","All",""));
                this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Output Laporan","=","Layar"));
			}else{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
                            this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+ 
                            this.filterRep.filterStr("a.progress",this.sg1.getCell(1,3),this.sg1.getCell(2,3).substr(0,1),this.sg1.getCell(3,3).substr(0,1),"and")+ 
                            this.filterRep.filterStr("a.no_aju",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+ 
                            " and b.jenis in ('ONLINE') ";
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,3);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_kopeg_kbitt_rptCeklist";
				// if (this.sg1.getCell(2,4)=="Detail")
				// {
				// 	this.nama_report = "server_report_saku2_kopeg_kbitt_rptHonorTransferDetail";
				// }
			...
			systemAPI.alert("[app_saku2_report_kopeg_kbitt_flCeklist]::mainButtonClick:"+e);
		}
	},
    ...
    doOpenDetail: function(jenis,kode_lokasi){
        this.link="1";
        filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
                this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+ " and a.jenis='"+jenis+"' ";
                            
		// filter = "where a.jenis='"+jenis+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=jenis;
		nama_report="server_report_saku2_kopeg_kbitt_rptCeklistDet";
