window.app_saku2_report_kopeg_kbitt_flPanjarPosisi = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_kbitt_flPanjarPosisi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_kbitt_flPanjarPosisi";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(a.periode) as periode "+
											"from (select max(periode) as periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' "+
											"union "+
											"select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"')a ");
...
window.app_saku2_report_kopeg_kbitt_flPanjarPosisi.extend(window.childForm);
window.app_saku2_report_kopeg_kbitt_flPanjarPosisi.implement({
	...
	{		
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["123","1236","3","123"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3],[2,0,3,2]);
		}	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			var sql="select a.periode "+
					"from (select distinct periode as periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' "+
					"union "+
					"select distinct periode as periode from kas_m where kode_lokasi='"+this.app._lokasi+"')a "+
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku2_kopeg_kbitt_rptPanjarPosisi";
			...
			systemAPI.alert("[app_saku2_report_kopeg_kbitt_flPanjarPosisi]::mainButtonClick:"+e);
		}
	},
