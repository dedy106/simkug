window.app_saku2_report_kopeg_kbitt_flIfReimPos = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_kbitt_flIfReimPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_kbitt_flIfReimPos";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and form in ('HUTIFNON') ");
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Tanggal","All",""));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Output Laporan","=","Layar"));
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Bukti Keluar","All",""));
...
window.app_saku2_report_kopeg_kbitt_flIfReimPos.extend(window.childForm);
window.app_saku2_report_kopeg_kbitt_flIfReimPos.implement({
		...
		try{			
			...
			if (row ==3)
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.form in ('HUTIFNON') ";
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select distinct a.no_aju, a.keterangan from it_aju_m a inner join it_ifreim_j b on a.no_aju=b.no_aju and a.kode_lokasi=b.kode_lokasi "+this.filter,
												"select count(distinct a.no_aju) from it_aju_m a inner join it_ifreim_j b on a.no_aju=b.no_aju and a.kode_lokasi=b.kode_lokasi "+this.filter,
			...
			{
				var tanggal="";
				if (this.sg1.getCell(1,4)=="=")
				{
					tanggal=" and a.tanggal='"+this.sg1.getCellDateValue(2,4)+"'";
				}
				if (this.sg1.getCell(1,4)=="Range")
				{
					tanggal=" and (a.tanggal between '"+this.sg1.getCellDateValue(2,4)+"' and '"+this.sg1.getCellDateValue(3,4)+"') ";
				}

				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_aju",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							" and a.modul in ('HUTIFNON') "+tanggal;
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
	...
	{		
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["123","123","123","123","123","3","123","13"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[2,0,2,2,1,0,2,0]);	
		}
		else if(this.app._userStatus=="P")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["123","123","23","123","123","3","123","13"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[2,0,2,2,1,0,2,0]);
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["123","123","3","123","123","3","123","13"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[3,0,2,2,1,0,2,0]);
		}
		if (row == 1)
		{
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Posting","Belum Posting"));
		}
	},
	cekpp: function(){
		alert('a');
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				{
					if(this.sg1.getCell(2,7) == "Posting"){
						var posted_from = "T";
					}else{
						var posted_from = "-";
					}
					posted=" and isnull(c.posted,'-')='"+posted_from+"' ";
				}
				if (this.sg1.getCell(1,7)=="Range")
				{
					...
					if(this.sg1.getCell(3,7) == "Posting"){
						var posted_to = "T";
					}else{
						var posted_to = "-";
					}
					posted=" and (isnull(c.posted,'-') between '"+posted_from+"' and '"+posted_to+"') ";
				}

		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_aju",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+tanggal+
							this.filterRep.filterStr("a.no_kas",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and")+posted+
							" and a.form in ('HUTIFNON') ";
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,5);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_kopeg_kbitt_rptIfReimPos";
			...
			systemAPI.alert("[app_saku2_report_kopeg_kbitt_flIfReimPos]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku2_kopeg_kbitt_rptIfPtgForm";
