window.app_saku2_report_kopeg_kbitt_flPtgPosTu2 = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_kbitt_flPtgPosTu2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_kbitt_flPtgPosTu2";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Bukti Keluar","All",""));
...
window.app_saku2_report_kopeg_kbitt_flPtgPosTu2.extend(window.childForm);
window.app_saku2_report_kopeg_kbitt_flPtgPosTu2.implement({
	...
	{		
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["123","123","123","123","123","123","3","13"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[2,0,2,2,1,2,0,0]);	
		}
		else if (this.app._userStatus=="P")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["123","123","23","123","123","123","3","13"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[2,0,2,2,1,2,0,0]);	
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["123","123","3","123","123","123","3","13"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[2,0,2,2,1,2,0,0]);
		}
		if (row == 1)
		{
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_kas",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+posted+
							" and a.modul='PJPTG' ";
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,6);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_kopeg_kbitt_rptPtgPosTu2";
			...
			systemAPI.alert("[app_saku2_report_kopeg_kbitt_flPtgPosTu2]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku2_kopeg_kbitt_rptPtgFormTu";
	...
	doOpenAju: function(no_bukti,kode_lokasi){
		...
		filter = "where a.no_aju='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.sg1.getCell(2,1);
		nama_report="server_report_saku2_kopeg_kbitt_rptPanjarFormTu";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
