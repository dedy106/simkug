window.app_saku2_report_kopeg_kbitt_flBymhdJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_kbitt_flBymhdJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_kbitt_flBymhdJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from ju_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('KBITT','JUSPB')");
...
window.app_saku2_report_kopeg_kbitt_flBymhdJurnal.extend(window.childForm);
window.app_saku2_report_kopeg_kbitt_flBymhdJurnal.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from ju_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('KBITT','JUSPB') order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_saku2_kopeg_kbitt_rptBymhdJurnal";
