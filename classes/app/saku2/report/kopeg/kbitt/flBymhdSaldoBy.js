// JavaScript Document
window.app_saku2_report_kopeg_kbitt_flBymhdSaldoBy = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_kbitt_flBymhdSaldoBy.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_kbitt_flBymhdSaldoBy";
		this.maximize();
	...
	var sql="select max(a.periode) as periode from (select distinct periode from ju_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('KBITT','JUSPB') "+
					"union "+
...
window.app_saku2_report_kopeg_kbitt_flBymhdSaldoBy.extend(window.portalui_childForm);
window.app_saku2_report_kopeg_kbitt_flBymhdSaldoBy.implement({
	...
	doSelectCell : function(sender, col, row){
		...
		if (row == 1)
		{
			...
			var sql="select distinct periode from ju_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('KBITT','JUSPB') "+
					"union "+
		...
		try
		{
			...
			else
			{
				...
				this.filter = this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
					  ...
					  this.filterRep.filterStr("a.no_ju",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," and ")+
							" and a.modul in ('KBITT','JUSPB') ";
				this.filter2 =this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0);
				
				this.nama_report="server_report_saku2_kopeg_kbitt_rptBymhdSaldoBy";
				this.dbLib.execQuerySync(this.sql);	
