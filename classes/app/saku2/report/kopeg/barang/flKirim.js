window.app_saku2_report_kopeg_barang_flKirim = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_barang_flKirim.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_barang_flKirim";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from brg_kirim_m ");
...
window.app_saku2_report_kopeg_barang_flKirim.extend(window.childForm);
window.app_saku2_report_kopeg_barang_flKirim.implement({
		...
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pemesanan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_kirim, a.keterangan from brg_kirim_m a "+filter,
												"select count(a.no_kirim) from brg_kirim_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from brg_kirim_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku2_kopeg_barang_rptKirim";
			...
			systemAPI.alert("[app_saku2_report_kopeg_barang_flKirim]::mainButtonClick:"+e);
		}
	},
