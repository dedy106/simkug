window.app_saku2_report_kopeg_barang_flSop = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_barang_flSop.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_barang_flSop";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from brg_sop_m ");
...
window.app_saku2_report_kopeg_barang_flSop.extend(window.childForm);
window.app_saku2_report_kopeg_barang_flSop.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data SOP",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_sop, a.keterangan from brg_sop_m a "+filter,
												"select count(a.no_sop) from brg_sop_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from brg_sop_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku2_report_kopeg_barang_flSop]::mainButtonClick:"+e);
		}
	},
