window.app_saku2_report_kopeg_logistik_flTerima = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_logistik_flTerima.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_logistik_flTerima";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from log_terima_m ");
...
window.app_saku2_report_kopeg_logistik_flTerima.extend(window.childForm);
window.app_saku2_report_kopeg_logistik_flTerima.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pemesanan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_terima, a.keterangan from log_terima_m a "+filter,
												"select count(a.no_terima) from log_terima_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from log_terima_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku2_report_kopeg_logistik_flTerima]::mainButtonClick:"+e);
		}
	},
