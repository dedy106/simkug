window.app_saku2_report_kopeg_logistik_flPesanView = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_logistik_flPesanView.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_report_kopeg_logistik_flPesanView";
		...
		this.pc1 = new pageControl(this,{bound:[15,12,1000,500], childPage:["Filter","Data Pemesanan","Detail Pemesanan","Riwayat Pemesanan"]});
		...
		this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from log_pesan_m");
		...
		this.sg1mp = new saiGrid(this.pc1.childPage[1],{bound:[1,20,this.pc1.width - 4,this.pc1.height - 180],colCount:8,
				    colTitle:["No Pemesanan","Tanggal","Kode PP","Nama PP","Sumber Dana","Keterangan","Jumlah","View Dokumen"],
...
window.app_saku2_report_kopeg_logistik_flPesanView.extend(window.childForm);
window.app_saku2_report_kopeg_logistik_flPesanView.implement({
	...
	mainButtonClick:function(sender)
	{
		try
		{
			...
			else
			{
				...
				var sql="select a.no_pesan,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.keterangan,a.kode_pp,b.nama as nama_pp,a.kode_dana,c.nama as nama_dana, "+
						...
						"left join  (select no_pesan,kode_lokasi,count(no_pesan) as jumlah "+
						"			from log_pesan_dok "+
						"			where kode_lokasi='05' "+
						"			group by no_pesan,kode_lokasi "+
						"			)d on a.no_pesan=d.no_pesan and a.kode_lokasi=d.kode_lokasi "+this.filter;
	...
	doTampilData2: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1mp.appendData([line.no_pesan,line.tanggal,line.kode_pp, line.nama_pp, line.nama_dana,line.keterangan,line.jumlah,"Preview"]);
