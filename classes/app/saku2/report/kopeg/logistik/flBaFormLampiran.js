window.app_saku2_report_kopeg_logistik_flBaFormLampiran = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_logistik_flBaFormLampiran.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_logistik_flBaFormLampiran";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from log_ba_m where modul<>'MTN' ");
...
window.app_saku2_report_kopeg_logistik_flBaFormLampiran.extend(window.childForm);
window.app_saku2_report_kopeg_logistik_flBaFormLampiran.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pemesanan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_ba, a.keterangan from log_ba_m a "+filter,
												"select count(a.no_ba) from log_ba_m a "+filter,
												new Array("a.no_ba","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from log_ba_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_ba",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							" and a.modul<>'MTN' "
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_kopeg_logistik_rptBaFormLampiran";
			...
			systemAPI.alert("[app_saku2_report_kopeg_logistik_flBaFormLampiran]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBayar: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter = "where a.no_ba='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
