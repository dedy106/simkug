window.app_saku2_report_kopeg_lab_flTugasJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_lab_flTugasJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_lab_flTugasJurnal";
		this.maximize();
	...
	this.dbLarge = new util_dbLarge();
		...
		this.no_tugas=this.dbLib.getPeriodeFromSQL("select max(no_tugas) as periode from lab_ju_m where kode_lokasi='"+this.app._lokasi+"' and nik_user='"+this.app._userLog+"'");
		...
		this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("No Jurnal","All",""));
...
window.app_saku2_report_kopeg_lab_flTugasJurnal.extend(window.childForm);
window.app_saku2_report_kopeg_lab_flTugasJurnal.implement({
		...
		try{			
			...
			if (row ==	3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.no_ju, a.keterangan,a.no_tugas from lab_ju_m a inner join lab_tugas b on a.no_tugas=b.no_tugas and a.kode_lokasi=b.kode_lokasi "+filter,														   
														  "select count(a.no_ju) from lab_ju_m a inner join lab_tugas b on a.no_tugas=b.no_tugas and a.kode_lokasi=b.kode_lokasi "+filter,
														  new Array("a.no_ju","a.nama","No Tugas"),"and");
			}
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.nik_user",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_tugas",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_ju",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_kopeg_lab_rptTugasJurnal";
			...
			systemAPI.alert("[app_saku2_report_kopeg_lab_flTugasJurnal]::mainButtonClick:"+e);
		}
	},
	...
	doOpenJurnal: function(no_tugas,kode_lokasi,id_soal,nik_user){
		this.no_tugas=no_tugas;
		this.kode_lokasi=kode_lokasi;
		this.id_soal=id_soal;
		this.nik_user=nik_user;
		this.link="1";
		filter = "where a.kode_lokasi='"+kode_lokasi+"' and a.no_tugas='"+no_tugas+"' and a.nik_user='"+nik_user+"' and a.id_soal="+id_soal;
		filter2="";
		nama_report="server_report_saku2_kopeg_lab_rptJurnal2";
