window.app_saku2_report_kopeg_lab_flNeraca = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_lab_flNeraca.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_lab_flNeraca";
		this.maximize();
	...
	this.dbLarge = new util_dbLarge();
		...
		this.no_tugas=this.dbLib.getPeriodeFromSQL("select max(no_tugas) as periode from lab_gldt where kode_lokasi='"+this.app._lokasi+"' and nik_user='"+this.app._userLog+"'");
		this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
		this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Mahasiswa","=",this.app._userLog));
		this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("No Tugas","=",this.no_tugas));
...
window.app_saku2_report_kopeg_lab_flNeraca.extend(window.childForm);
window.app_saku2_report_kopeg_lab_flNeraca.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==	1)
			{
				this.filterRep.ListDataSGFilter(this, "Data Mahasiswa",this.sg1, this.sg1.row, this.sg1.col,
														  "select nim, nama from lab_mhs "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(nim) from lab_mhs "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  new Array("nim","nama"),"and");
			}
			if (row ==	2)
			{
				
				this.filterRep.ListDataSGFilter(this, "Data Tugas",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_tugas, nama from lab_tugas "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(no_tugas) from lab_tugas "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  new Array("no_tugas","nama"),"and");
			}
			if (row ==	3)
			{
				filter= this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
						this.filterRep.filterStr("nik_user",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," where ")+
						this.filterRep.filterStr("no_tugas",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," and ");
				this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_akun, nama from lab_masakun "+filter,														   
														  "select count(kode_akun) from lab_masakun "+filter,
	...
	{		
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["123","2","123","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3],[2,3,2,0]);	
		}
		else
		{
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenTb(this.kode_neraca,this.kode_lokasi,this.no_tugas,this.nik_user,this.userid);
				}
				if (this.link=="3")
				{
					this.doOpenBb(this.kode_akun,this.kode_lokasi,this.no_tugas,this.nik_user);
				}
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.nik_user",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_tugas",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.app._nikUser+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_kopeg_lab_rptNeraca";
			...
			systemAPI.alert("[app_saku2_report_kopeg_lab_flNeraca]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBb: function(kode_akun,kode_lokasi,no_tugas,nik_user){
		this.kode_akun=kode_akun;
		this.kode_lokasi=kode_lokasi;
		this.no_tugas=no_tugas;
		this.nik_user=nik_user;
		this.link="2";
		filter = " where a.kode_akun='"+kode_akun+"' and a.kode_lokasi='"+kode_lokasi+"' and a.no_tugas='"+no_tugas+"' and a.nik_user='"+nik_user+"' ";
		filter2="";
	...
	doOpenJurnal: function(no_bukti,kode_lokasi,no_tugas,nik_user){
		this.no_bukti=no_bukti;
		this.kode_lokasi=kode_lokasi;
		this.link="3";
		filter = "where a.no_ju='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' and a.no_tugas='"+no_tugas+"' and a.nik_user='"+nik_user+"'";
		filter2="";
		nama_report="server_report_saku2_kopeg_lab_rptBuktiJurnal";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenTb: function(kode_neraca,kode_lokasi,no_tugas,nik_user,userid){
