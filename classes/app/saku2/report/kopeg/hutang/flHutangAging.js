window.app_saku2_report_kopeg_hutang_flHutangAging = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_hutang_flHutangAging.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_hutang_flHutangAging";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from hutang_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_kopeg_hutang_flHutangAging.extend(window.childForm);
window.app_saku2_report_kopeg_hutang_flHutangAging.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_hutang, a.keterangan from hutang_m a "+filter,
												"select count(b.no_hutang) from hutang_m a "+filter,
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku2_kopeg_hutang_rptHutangAging";
			...
			systemAPI.alert("[app_saku2_report_kopeg_hutang_flHutangAging]::mainButtonClick:"+e);
		}
	},
