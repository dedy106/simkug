window.app_saku2_report_kopeg_hutang_flHutangPb = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_hutang_flHutangPb.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_hutang_flHutangPb";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from pb_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_kopeg_hutang_flHutangPb.extend(window.childForm);
window.app_saku2_report_kopeg_hutang_flHutangPb.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Hutang",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_pb, a.keterangan from pb_m a "+filter,
												"select count(a.no_pb) from pb_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from pb_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku2_report_kopeg_hutang_flHutangPb]::mainButtonClick:"+e);
		}
	},
