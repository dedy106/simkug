window.app_saku2_report_kopeg_apotek_flJualAggDaftar = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_apotek_flJualAggDaftar.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_apotek_flJualAggDaftar";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from apo_brg_jual_m where modul='UMUM' ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Cusotmer","All",""));
...
window.app_saku2_report_kopeg_apotek_flJualAggDaftar.extend(window.childForm);
window.app_saku2_report_kopeg_apotek_flJualAggDaftar.implement({
		...
		try{			
			...
			if (row == 4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Penjualan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_jual, a.keterangan from apo_brg_jual_m a "+filter,
												"select count(a.no_jual) from apo_brg_jual_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from apo_brg_jual_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_gudang",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
				...
				this.nama_report = "server_report_saku2_kopeg_apotek_rptJualAggDaftar";
			...
			systemAPI.alert("[app_saku2_report_kopeg_apotek_flJualAggDaftar]::mainButtonClick:"+e);
		}
	},
