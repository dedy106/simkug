window.app_saku2_report_kopeg_apotek_flBeliTunaiDaftar = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_apotek_flBeliTunaiDaftar.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_apotek_flBeliTunaiDaftar";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from apo_brg_beli_m ");
...
window.app_saku2_report_kopeg_apotek_flBeliTunaiDaftar.extend(window.childForm);
window.app_saku2_report_kopeg_apotek_flBeliTunaiDaftar.implement({
		...
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pembelian",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_beli, a.keterangan from apo_brg_beli_m a "+filter,
												"select count(a.no_beli) from apo_brg_beli_m a "+filter,
												new Array("a.no_beli","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from apo_brg_beli_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_beli",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							" and a.modul='TUNAI' ";
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_kopeg_apotek_rptBeliTunaiDaftar";
			...
			systemAPI.alert("[app_saku2_report_kopeg_apotek_flBeliTunaiDaftar]::mainButtonClick:"+e);
		}
	},
