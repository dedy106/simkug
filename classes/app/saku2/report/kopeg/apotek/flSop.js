window.app_saku2_report_kopeg_apotek_flSop = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_apotek_flSop.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_apotek_flSop";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from apo_brg_sop_m ");
...
window.app_saku2_report_kopeg_apotek_flSop.extend(window.childForm);
window.app_saku2_report_kopeg_apotek_flSop.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data SOP",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_sop, a.keterangan from apo_brg_sop_m a "+filter,
												"select count(a.no_sop) from apo_brg_sop_m a "+filter,
												new Array("a.no_sop","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from apo_brg_sop_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku2_kopeg_apotek_rptSop";
			...
			systemAPI.alert("[app_saku2_report_kopeg_apotek_flSop]::mainButtonClick:"+e);
		}
	},
