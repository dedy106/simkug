window.app_saku2_report_kopeg_apotek_flTerima = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_apotek_flTerima.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_apotek_flTerima";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from apo_brg_terima_m ");
...
window.app_saku2_report_kopeg_apotek_flTerima.extend(window.childForm);
window.app_saku2_report_kopeg_apotek_flTerima.implement({
		...
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pemesanan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_terima, a.keterangan from apo_brg_terima_m a "+filter,
												"select count(a.no_terima) from apo_brg_terima_m a "+filter,
												new Array("a.no_terima","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from apo_brg_terima_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_tujuan",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
				...
				this.nama_report = "server_report_saku2_kopeg_apotek_rptTerima";
			...
			systemAPI.alert("[app_saku2_report_kopeg_apotek_flTerima]::mainButtonClick:"+e);
		}
	},
