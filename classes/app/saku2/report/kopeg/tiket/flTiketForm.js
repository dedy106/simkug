window.app_saku2_report_kopeg_tiket_flTiketForm = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_tiket_flTiketForm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_tiket_flTiketForm";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("User","=",this.app._userLog));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("No Tiket","All",""));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku2_report_kopeg_tiket_flTiketForm.extend(window.childForm);
window.app_saku2_report_kopeg_tiket_flTiketForm.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 2)
			{
				...
				var sql="select a.nik_user,b.nama "+
						"from (select a.nik_user,a.kode_lokasi "+
						"from sai_tiket_m a "+filter+
						"group by a.nik_user,a.kode_lokasi "+
						"	  )a  "+
						"inner join karyawan b on a.nik_user=b.nik and a.kode_lokasi=b.kode_lokasi ";
				var sql="select count(a.nik_user) "+
						"from (select a.nik_user,a.kode_lokasi "+
						"from sai_tiket_m a "+filter+
						"group by a.nik_user,a.kode_lokasi "+
						"	  )a  ";
				this.filterRep.ListDataSGFilter(this, "Data Jurusan",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,
														  ["a.nik_user","a.nama"],"and",["NIK","Nama"]);
			}
			if (row == 3)
			{
				...
				var sql1="select a.no_tiket,a.keterangan from sai_tiket_m a "+filter; 
				var sql2="select count(a.no_tiket) from sai_tiket_m a "+filter;
				this.filterRep.ListDataSGFilter(this, "Data Tiket",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
												new Array("a.no_tiket","a.keterangan"),"and",new Array("no_tiket","Nama"),"and");
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 2)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from sai_tiket_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("no_tiket","All",""));
			}else{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.nik_user",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_tiket",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku2_kopeg_tiket_rptTiketForm";
	...
	doOpenKartuMhs: function(no_tiket,kode_lokasi){
		
		this.no_tiket=no_tiket;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = "";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.kode_jur+"/"+no_tiket;;
