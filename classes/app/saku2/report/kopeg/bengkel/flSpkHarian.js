window.app_saku2_report_kopeg_bengkel_flSpkHarian = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_bengkel_flSpkHarian.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_bengkel_flSpkHarian";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from fri_spk_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_kopeg_bengkel_flSpkHarian.extend(window.childForm);
window.app_saku2_report_kopeg_bengkel_flSpkHarian.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==5)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data SPK",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_spk, a.no_polisi,a.cust from fri_spk_m a "+filter,
												"select count(a.no_spk) from fri_spk_m a "+filter,
												new Array("a.no_spk","a.no_polisi","a.cust"),"and",new Array("No Bukti","No Polisi","Customer"));
			}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123","123","123","13","13","123","13"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[2,0,2,0,0,2,1]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from fri_spk_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row === 3)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.status",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.jenis",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							this.filterRep.filterStr("a.no_spk",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+tanggal;
				this.filter2 = this.sg1.getCell(2,1)+"|"+this.sg1.getCellDateValue(2,6);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_kopeg_bengkel_rptSpkHarian";
			...
			systemAPI.alert("[app_saku2_report_kopeg_bengkel_flSpkHarian]::mainButtonClick:"+e);
		}
	},
