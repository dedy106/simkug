window.app_saku2_report_kopeg_bengkel_flSpkAkru = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_bengkel_flSpkAkru.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_bengkel_flSpkAkru";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Format Jurnal","=","Daftar Jurnal"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku2_report_kopeg_bengkel_flSpkAkru.extend(window.childForm);
window.app_saku2_report_kopeg_bengkel_flSpkAkru.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_jual, keterangan from fri_jual_m  "+filter ,
														  "select count(no_jual) from fri_jual_m "+filter,
														  new Array("no_jual","keterangan"),"and");
			}
			if (row == 3)
			{
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from fri_jual_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				var akun=this.filterRep.filterStr("b.kode_akun",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3)," and ")
				this.filter2=this.sg1.getCell(2,0)+"/"+akun;
			
				if (this.sg1.getCell(2,4)=="Daftar Jurnal")
				{
					this.nama_report="server_report_saku2_kopeg_bengkel_rptSpkAkruDaftar";
				}
				else
				{
					this.nama_report="server_report_saku2_kopeg_bengkel_rptSpkAkru";
				}
