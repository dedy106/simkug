window.app_saku2_report_kopeg_aka_flAkSaldoBillMaba = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_aka_flAkSaldoBillMaba.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_aka_flAkSaldoBillMaba";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(a.periode) as periode "+
									"from (select max(periode) as periode from aka_mabarekon_m where kode_lokasi='"+this.app._lokasi+"' "+
									"union "+
									"select max(periode) from aka_mababill_m where kode_lokasi='"+this.app._lokasi+"') a ");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Jalur","All",""));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("No Tes","All",""));
...
window.app_saku2_report_kopeg_aka_flAkSaldoBillMaba.extend(window.childForm);
window.app_saku2_report_kopeg_aka_flAkSaldoBillMaba.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Jalur",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.kode_jalur, a.nama from aka_mabajalur a   "+
														   this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(a.kode_jalur) from aka_mabajalur a "+
														  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  ["a.kode_jalur","a.nama"],"and",["Kode","Nama"]);
			}
			if (row == 4)
			{
				...
				var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_jalur",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				var sql1="select a.no_tes,a.nama from aka_maba a "+filter+modul; 
				var sql2="select count(distinct a.no_tes) from aka_maba a "+filter+modul;
				this.filterRep.ListDataSGFilter(this, "Data Mahasiswa",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
												new Array("a.no_tes","a.nama"),"and",new Array("no_tes","Nama"),"and");
			}		
			if (row ==4)
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_jur",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_tes",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_inv,a.periode "+
												"from (select distinct no_inv,no_tes,periode,kode_lokasi "+
												"	  from aka_mababill_d) a "+
												"inner join aka_maba b on a.no_tes=b.no_tes and a.kode_lokasi=b.kode_lokasi "+this.filter,
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["3","3","123","123","123","123","3"]);
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select a.periode "+
									"from (select distinct periode,kode_lokasi from aka_mabarekon_m where kode_lokasi='"+this.app._lokasi+"' "+
									"union "+
									"select periode,kode_lokasi from periode where kode_lokasi='"+this.app._lokasi+"' "+
									"union "+
									"select distinct periode,kode_lokasi from aka_mababill_m where kode_lokasi='"+this.app._lokasi+"') a "+
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,6);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku2_kopeg_aka_rptAkSaldoBillMaba";
	...
	doOpenKartuMhs: function(no_tes,kode_lokasi){
		this.link="1";
		this.no_tes=no_tes;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = " where a.kode_lokasi='"+this.kode_lokasi+"' and a.no_tes='"+this.no_tes+"'";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.kode_jur+"/"+no_tes;;
