window.app_saku2_report_kopeg_aka_flAkBillBatalJurnalTu = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_aka_flAkBillBatalJurnalTu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_aka_flAkBillBatalJurnalTu";
		this.maximize();
...
window.app_saku2_report_kopeg_aka_flAkBillBatalJurnalTu.extend(window.childForm);
window.app_saku2_report_kopeg_aka_flAkBillBatalJurnalTu.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_batal, a.keterangan from aka_batal_m a "+filter,
												"select count(a.no_batal) from aka_batal_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from aka_batal_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_batal",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_kopeg_aka_rptAkBillBatalJurnalTu";
			...
			systemAPI.alert("[app_saku2_report_kopeg_aka_flAkBillBatalJurnalTu]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBayar: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter = "where a.no_batal='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
