window.app_saku2_report_kopeg_aka_flAkBillMaba = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_aka_flAkBillMaba.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_aka_flAkBillMaba";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from aka_mababill_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku2_report_kopeg_aka_flAkBillMaba.extend(window.childForm);
window.app_saku2_report_kopeg_aka_flAkBillMaba.implement({
		...
		try{			
			...
			if (row ==5)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_bill,a.keterangan from aka_mababill_m a "+this.filter,
												"select count(a.no_bill) from aka_mababill_m a "+this.filter,
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["3","23","123","123","123","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,0,2,2,2,2]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from aka_mababill_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  ...
							  this.filterRep.filterStr("c.kode_jalur",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							  this.filterRep.filterStr("c.no_tes",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
				...
				this.nama_report = "server_report_saku2_kopeg_aka_rptAkBillMaba";
