window.app_saku2_report_kopeg_aka_flAkRekonTu = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_aka_flAkRekonTu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_aka_flAkRekonTu";
		this.maximize();
...
window.app_saku2_report_kopeg_aka_flAkRekonTu.extend(window.childForm);
window.app_saku2_report_kopeg_aka_flAkRekonTu.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from aka_rekon_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 6)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku2_kopeg_aka_rptAkRekonTu";
