window.app_saku2_report_kopeg_fa_flVaKkil = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_fa_flVaKkil.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_fa_flVaKkil";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from va_load_m ");
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Kelompok Asset","All",""));
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Lokasi Asset","All",""));
...
window.app_saku2_report_kopeg_fa_flVaKkil.extend(window.childForm);
window.app_saku2_report_kopeg_fa_flVaKkil.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Lokasi Asset",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_gedung, nama from va_gedung ",
												"select count(*) from va_gedung ",
												new Array("kode_gedung","nama"),"where",new Array("kode","nama"));
			}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["123","123","123","2"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3],[2,0,2,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from va_load_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_gedung",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
				...
				this.nama_report = "server_report_saku2_kopeg_fa_rptVaKkil";
			...
			systemAPI.alert("[app_saku2_report_kopeg_fa_flVaKkil]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBayar: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter = "where a.no_wo='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
