window.app_saku2_report_kopeg_fa_flFawoAjuApp = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_fa_flFawoAjuApp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_fa_flFawoAjuApp";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from fawo_m where progress='1' ");
...
window.app_saku2_report_kopeg_fa_flFawoAjuApp.extend(window.childForm);
window.app_saku2_report_kopeg_fa_flFawoAjuApp.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pemesanan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_wo, a.keterangan from fawo_m a "+filter,
												"select count(a.no_wo) from fawo_m a "+filter,
												new Array("a.no_wo","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from fawo_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_wo",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							" and a.progress='1' ";
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_kopeg_fa_rptFawoAjuApp";
			...
			systemAPI.alert("[app_saku2_report_kopeg_fa_flFawoAjuApp]::mainButtonClick:"+e);
		}
	},
