// JavaScript Document
window.app_saku2_report_kopeg_gl_flRefSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_gl_flRefSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_gl_flRefSaldo";
		this.maximize();
	...
	this.periode2=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('KBREF','KBADJ') ");
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Periode Akru","<=",this.periode));
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Periode Adjustment","<=",this.periode2));
	...
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Tampil Saldo Nol","=","Ya"));
...
window.app_saku2_report_kopeg_gl_flRefSaldo.extend(window.portalui_childForm);
window.app_saku2_report_kopeg_gl_flRefSaldo.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row ==3)
		{
			var sql="select a.kode_akun,a.nama "+
					"from masakun a "+
					"inner join (select kode_lokasi,kode_akun "+
					"			from ju_j "+
					"			where kode_lokasi='"+this.sg1.getCell(2,0)+"' and jenis='AKUNREF' "+
	...
	doSelectCell : function(sender, col, row){
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["123","236","236","123","123","3"]);
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("c.kode_akun",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.no_ju",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+modul;
				this.filter2 = this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,3)+"/"+this.sg1.getCell(2,5)+"/"+this.sg1.getCell(2,0);
				
				this.nama_report="server_report_saku2_kopeg_gl_rptRefSaldo";
