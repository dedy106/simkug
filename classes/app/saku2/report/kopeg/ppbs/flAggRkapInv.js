// JavaScript Document
window.app_saku2_report_kopeg_ppbs_flAggRkapInv = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_ppbs_flAggRkapInv.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_ppbs_flAggRkapInv";
		this.maximize();
...
window.app_saku2_report_kopeg_ppbs_flAggRkapInv.extend(window.portalui_childForm);
window.app_saku2_report_kopeg_ppbs_flAggRkapInv.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report="server_report_saku2_kopeg_ppbs_rptAggRkapInv";
				sql="call sp_agg_rkap_bulan ('"+this.sg1.getCell(2,2)+"','A','"+this.sg1.getCell(2,1)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.nik_user+"')";
	...
	doOpenDetail: function(kode_akun,kode_pp,kode_drk,kode_lokasi,tahun){
		...
		this.link="3";
		filter2 = tahun+"/";
		filter=" where a.kode_akun='"+kode_akun+"' and a.kode_lokasi='"+kode_lokasi+"' and substring(a.periode,1,4)='"+tahun+"' and a.kode_pp='"+kode_pp+"' and kode_drk='"+kode_drk+"'";
