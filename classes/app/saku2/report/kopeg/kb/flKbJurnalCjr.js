window.app_saku2_report_kopeg_kb_flKbJurnalCjr = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_kb_flKbJurnalCjr.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_kb_flKbJurnalCjr";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Kode Arus Kas","All",""));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Kode PP","All",""));
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Kode DRK","All",""));
	this.gridLib.SGEditData(this.sg1,7,new Array(0,1), new Array("No Bukti","All"));
	this.gridLib.SGEditData(this.sg1,8,new Array(0,1,2), new Array("Format Jurnal","=","Daftar Jurnal"));
	this.doSelectCell(this.sg1,2,8);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku2_report_kopeg_kb_flKbJurnalCjr.extend(window.childForm);
window.app_saku2_report_kopeg_kb_flKbJurnalCjr.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 4)
			{
				this.filterRep.ListDataSGFilter(this, "Data Arus Kas",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_cf, nama from neracacf where tipe='Posting' "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," and "),
														  "select count(kode_cf) from neracacf where tipe='Posting' "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," and "),
														  new Array("kode_cf","nama"),"and");
			}
			if (row == 5)
			{
			...
			if (row == 7)
			{
				...
				var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_cf",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["123","23","13","123","123","123","123","123","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[2,0,0,2,2,2,2,2,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from kas_j where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct modul from kas_j where kode_lokasi='"+this.app._lokasi+"' order by modul",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 8)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_saku2_kopeg_kb_rptKbJurnal";
				if (this.sg1.getCell(2,8)=="Memo Jurnal")
				{
					this.nama_report="server_report_saku2_kopeg_kb_rptKbBuktiJurnalCjr";
