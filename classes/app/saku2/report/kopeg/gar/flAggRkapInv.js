// JavaScript Document
window.app_saku2_report_kopeg_gar_flAggRkapInv = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_gar_flAggRkapInv.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_gar_flAggRkapInv";
		this.maximize();
...
window.app_saku2_report_kopeg_gar_flAggRkapInv.extend(window.portalui_childForm);
window.app_saku2_report_kopeg_gar_flAggRkapInv.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substrng(periode,1,4) from anggaran_d where kode_lokasi='"+this.app._lokasi+"' order by substrng(periode,1,4) desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="1")
				{
					...
					this.nama_report = "server_report_saku2_kopeg_gar_rptAggRkapInv";
			...
			else
			{
				...
				this.nama_report="server_report_saku2_kopeg_gar_rptAggRkapInv";
				sql="call sp_agg_rkap_bulan_tu ('"+this.sg1.getCell(2,2)+"','A','"+this.sg1.getCell(2,1)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.nik_user+"')";
