window.app_saku2_report_kopeg_sju_flRefJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_sju_flRefJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_sju_flRefJurnal";
		this.maximize();
...
window.app_saku2_report_kopeg_sju_flRefJurnal.extend(window.childForm);
window.app_saku2_report_kopeg_sju_flRefJurnal.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 3)
			{
				var modul=" and a.modul in ('JUREF','JUADJ') ";
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","23","13","123","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,0,2,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from ju_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('JUREF','JUADJ') order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct modul from ju_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('JUREF','JUADJ') order by modul",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				{
					this.nama_report="server_report_saku2_kopeg_sju_rptRefJurnal";
				}
				else
				{
					this.nama_report="server_report_saku2_kopeg_sju_rptRefJurnalBukti";
				}
