window.app_saku2_report_kopeg_sju_flPrPlacingList = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_sju_flPrPlacingList.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_sju_flPrPlacingList";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sju_placing_m where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("No Placing","All",""));
	this.gridLib.SGEditData(this.sg1,7,new Array(0,1,2), new Array("Digit Koma","=","4"));
	this.gridLib.SGEditData(this.sg1,8,new Array(0,1,2), new Array("Tanggal","All",""));
...
window.app_saku2_report_kopeg_sju_flPrPlacingList.extend(window.childForm);
window.app_saku2_report_kopeg_sju_flPrPlacingList.implement({
		...
		try{			
			...
			if (row ==6)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Polis",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_placing, no_dok from sju_placing_m a "+filter,
												"select count(a.no_placing) from sju_placing_m a "+filter,
												new Array("a.no_placing","a.no_dok"),"and",new Array("No Polis","No Dok Polis"));
			}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["123","123","123","123","123","123","123","3","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[2,0,2,2,2,2,2,0,1]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from sju_placing_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 7)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				{
					tanggal=" and a.tanggal='"+this.sg1.getCellDateValue(2,8)+"'";
				}
				if (this.sg1.getCell(1,8)=="Range")
				{
					tanggal=" and (a.tanggal between '"+this.sg1.getCellDateValue(2,8)+"' and '"+this.sg1.getCellDateValue(3,8)+"') ";
				}
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_tipe",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.kode_cust",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.kode_cust",this.sg1.getCell(1,4),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.kode_pic",this.sg1.getCell(1,5),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							this.filterRep.filterStr("a.no_placing",this.sg1.getCell(1,6),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+tanggal;
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,7);
				
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_kopeg_sju_rptPrPlacingList";
			...
			systemAPI.alert("[app_saku2_report_kopeg_sju_flPrPlacingList]::mainButtonClick:"+e);
		}
	},
