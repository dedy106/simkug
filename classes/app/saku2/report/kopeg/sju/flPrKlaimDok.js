window.app_saku2_report_kopeg_sju_flPrKlaimDok = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_sju_flPrKlaimDok.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_sju_flPrKlaimDok";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sju_klaim where kode_lokasi='"+this.app._lokasi+"'");
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Periode","All",this.periode));
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Customer","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("No Polis","All",""));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("No Klaim","All",""));
...
window.app_saku2_report_kopeg_sju_flPrKlaimDok.extend(window.childForm);
window.app_saku2_report_kopeg_sju_flPrKlaimDok.implement({
		...
		try{			
			...
			if (row ==	3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Polis",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_polis, no_dok from sju_polis_m "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(no_polis) from sju_polis_m "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  new Array("no_polis","no_dok"),"and");
			}
			if (row ==	4)
			{
				this.filterRep.ListDataSGFilter(this, "Data Klaim",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_klaim, keterangan from sju_klaim "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(no_klaim) from sju_klaim "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  new Array("no_klaim","keterangan"),"and");
			}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","123","123","!23","123"]);
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("e.kode_cust",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("e.no_polis",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
				...
				this.nama_report = "server_report_saku2_kopeg_sju_rptPrKlaimDok";
			...
			systemAPI.alert("[app_saku2_report_kopeg_sju_flPrKlaimDok]::mainButtonClick:"+e);
		}
	},
