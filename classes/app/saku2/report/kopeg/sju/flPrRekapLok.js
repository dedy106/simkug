window.app_saku2_report_kopeg_sju_flPrRekapLok = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_sju_flPrRekapLok.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_sju_flPrRekapLok";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from sju_polis_m where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Jenis Laporan","=","Marketing"));
...
window.app_saku2_report_kopeg_sju_flPrRekapLok.extend(window.childForm);
window.app_saku2_report_kopeg_sju_flPrRekapLok.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2],["123","3","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2],[2,0,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from sju_polis_m where kode_lokasi='"+this.app._lokasi+"' order by  substring(periode,1,4) desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Marketing","Tertanggung"));
		}	
	},
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenPolisList(this.kode_cust,this.kode_lokasi);
				}			
			}
			else
			{
				...
				if (this.sg1.getCell(2,2)=="Marketing")
				{
					this.nama_report = "server_report_saku2_kopeg_sju_rptPrRekapPic";
				}
				if (this.sg1.getCell(2,2)=="Cabang")
				{
					this.nama_report = "server_report_saku2_kopeg_sju_rptPrRekapLok";
				}
				if (this.sg1.getCell(2,2)=="Tertanggung")
				{
					this.nama_report = "server_report_saku2_kopeg_sju_rptPrRekapCust";
				}
				if (this.sg1.getCell(2,2)=="Jenis Tertanggung")
				{
					this.nama_report = "server_report_saku2_kopeg_sju_rptPrRekapCustKlp";
				}
			...
			systemAPI.alert("[app_saku2_report_kopeg_sju_flPrRekapLok]::mainButtonClick:"+e);
		}
	},
	...
	doOpenDaftarPolis: function(kode_pic,kode_lokasi,periode){
		this.kode_pic=kode_pic;
		this.kode_lokasi=kode_lokasi;
		this.periode=periode;
		this.link="1";
		filter = "where a.kode_lokasi='"+kode_lokasi+"' and d.kode_pic='"+kode_pic+"' and b.periode like '%"+periode+"%'";
		filter2="";
		nama_report="server_report_saku2_kopeg_sju_rptPrAwasPdpt";
	...
	doOpenPolisCust: function(kode_cust,kode_lokasi,periode){
		this.kode_cust=kode_cust;
		this.kode_lokasi=kode_lokasi;
		this.periode=periode;
		this.link="1";
		filter = "where a.kode_lokasi='"+kode_lokasi+"' and d.kode_cust='"+kode_cust+"' and b.periode like '%"+periode+"%'";
		filter2="";
		nama_report="server_report_saku2_kopeg_sju_rptPrAwasPdpt";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
