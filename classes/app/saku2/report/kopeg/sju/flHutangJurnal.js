window.app_saku2_report_kopeg_sju_flHutangJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_sju_flHutangJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_sju_flHutangJurnal";
		this.maximize();
...
window.app_saku2_report_kopeg_sju_flHutangJurnal.extend(window.childForm);
window.app_saku2_report_kopeg_sju_flHutangJurnal.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_bill, keterangan from sju_bill_m  "+filter ,
														  "select count(no_bill) from sju_bill_m "+filter,
														  new Array("no_bill","keterangan"),"and");
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from sju_bill_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_saku2_kopeg_sju_rptHutangJurnal";
