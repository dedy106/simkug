window.app_saku2_report_kopeg_sju_flJuRincianPremi = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_sju_flJuRincianPremi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_sju_flJuRincianPremi";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from ju_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('JUPOLIS','JUPOLISFEE') ");
...
window.app_saku2_report_kopeg_sju_flJuRincianPremi.extend(window.childForm);
window.app_saku2_report_kopeg_sju_flJuRincianPremi.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from ju_m where kode_lokasi='"+this.app._lokasi+"'  order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku2_report_kopeg_sju_flJuRincianPremi]::mainButtonClick:"+e);
		}
	},
