window.app_saku2_report_kopeg_sju_flGlLabaRugiKonsolSimulasi = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_sju_flGlLabaRugiKonsolSimulasi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_sju_flGlLabaRugiKonsolSimulasi";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gldt_tmp");
...
window.app_saku2_report_kopeg_sju_flGlLabaRugiKonsolSimulasi.extend(window.childForm);
window.app_saku2_report_kopeg_sju_flGlLabaRugiKonsolSimulasi.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data FS",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_fs,nama from fs "+lokasi+" and kode_fs in ('FS1','FS4','FS5','FS6','FS7')",
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["3","3","123","123","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[3,0,2,0,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode  as periode from gldt_tmp order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="4")
				{
					if (this.tw=="1")
					{
						this.doOpenGarTw(this.kode_neraca,this.kode_lokkonsol,this.periode);
					}
					else
					{
						this.doOpenGar(this.kode_neraca,this.kode_lokkonsol,this.periode);
					}
				}
			...
			else
			{
				...
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser+"/"+this.sg1.getCell(3,0)+"/"+this.sg1.getCell(2,2);
				this.nama_report="server_report_saku2_kopeg_sju_rptGlLabaRugiKonsolSimulasi";
				/*
				if (this.sg1.getCell(2,2)!="FS1")
				{
					this.nama_report="server_report_saku2_kopeg_sju_rptGlLabaRugiRincianKonsolSimulasi";
				}
				*/
	...
	doOpenGar: function(kode_neraca,kode_lokkonsol,periode){
		this.kode_neraca=kode_neraca;
		this.kode_lokkonsol=kode_lokkonsol;
		this.periode=periode;
		this.link="1";
		this.tw="0";
		filter2=this.sg1.getCell(2,1).substr(0,4)+"/1";
		filter="where z.kode_lokkonsol='"+kode_lokkonsol+"' and a.kode_neraca='"+kode_neraca+"' and substring(x.periode,1,4)='"+periode.substr(0,4)+"' and a.kode_fs='"+this.sg1.getCell(2,2)+"'";
		nama_report="server_report_saku2_kopeg_sju_rptAggBulanKonsolJejer";
	...
	doOpenGarTw: function(kode_neraca,kode_lokkonsol,periode){
		this.kode_neraca=kode_neraca;
		this.kode_lokkonsol=kode_lokkonsol;
		this.periode=periode;
		this.link="1";
		...
		filter2=this.sg1.getCell(2,1).substr(0,4)+"/1";
		filter="where z.kode_lokkonsol='"+kode_lokkonsol+"' and a.kode_neraca='"+kode_neraca+"' and substring(x.periode,1,4)='"+periode.substr(0,4)+"' and a.kode_fs='"+this.sg1.getCell(2,2)+"'";
		nama_report="server_report_saku2_kopeg_sju_rptAggTwKonsolJejer";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenGarBulan: function(kode_akun,kode_lokasi,tahun){
		
		this.kode_akun=kode_akun;
		this.kode_lokasi=kode_lokasi;
		this.tahun=tahun;
		this.link="4";
		filter2=this.sg1.getCell(2,1).substr(0,4)+"/3";
		filter="where x.kode_lokasi='"+kode_lokasi+"' and x.kode_akun='"+kode_akun+"' and substring(x.periode,1,4)='"+tahun+"' ";
		nama_report="server_report_saku2_kopeg_sju_rptAggBulan";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenTb: function(kode_neraca,kode_lokasi,periode){
		...
		nama_report="server_report_saku2_kopeg_sju_rptGlTbJejerSimulasi";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenBb: function(kode_akun,kode_lokasi,periode){
		...
		nama_report="server_report_saku2_kopeg_sju_rptGlBukuBesarSimulasi";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenJurnal: function(no_bukti,kode_lokasi){
		...
		filter2="";
		nama_report="server_report_saku2_kopeg_sju_rptGlJurnalBuktiSimulasi";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
