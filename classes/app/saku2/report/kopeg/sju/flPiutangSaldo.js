window.app_saku2_report_kopeg_sju_flPiutangSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_sju_flPiutangSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_sju_flPiutangSaldo";
		this.maximize();
	...
	var sql="select max(periode) as periode from sju_bill_m where kode_lokasi='"+this.app._lokasi+"'";
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Jenis Laporan","=","Outstanding Premi"));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Urut Berdasarkan","=","Nilai"));
...
window.app_saku2_report_kopeg_sju_flPiutangSaldo.extend(window.childForm);
window.app_saku2_report_kopeg_sju_flPiutangSaldo.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["123","123","3","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3],[2,0,0,0]);	
		if (row == 1)
		{
			var sql="select distinct periode from sju_bill_m where kode_lokasi='"+this.app._lokasi+"' "+ 
					"union "+
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Outstanding Premi","Summary Mutasi","Detail Mutasi"));
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.urut="a.n4 desc";
				if (this.sg1.getCell(2,3)=="Tertanggung")
				{
					this.urut="b.nama asc";
				}
				if (this.sg1.getCell(2,3)=="Kode Tertanggung")
				{
					this.urut="a.kode_cust asc";
				}
				if (this.sg1.getCell(2,3)=="No Register")
				{
					this.urut="a.no_polis asc";
				}
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.urut+"/"+this.app._nikUser;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();	
				this.nama_report = "server_report_saku2_kopeg_sju_rptPiutangSaldoOut";
				if (this.sg1.getCell(2,2)=="Summary Mutasi")
				{
					this.nama_report = "server_report_saku2_kopeg_sju_rptPiutangSaldo";
				}
				if (this.sg1.getCell(2,2)=="Detail Mutasi")
				{
					this.nama_report = "server_report_saku2_kopeg_sju_rptPiutangSaldoPolis";
				}
			...
			systemAPI.alert("[app_saku2_report_kopeg_sju_flPiutangSaldo]::mainButtonClick:"+e);
		}
	},
