window.app_saku2_report_kopeg_sju_flHutangKas = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_sju_flHutangKas.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_sju_flHutangKas";
		this.maximize();
...
window.app_saku2_report_kopeg_sju_flHutangKas.extend(window.childForm);
window.app_saku2_report_kopeg_sju_flHutangKas.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBHUTPOL' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
