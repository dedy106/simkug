window.app_saku2_report_kopeg_sju_flPiutangJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_sju_flPiutangJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_sju_flPiutangJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from ju_m where kode_lokasi='"+this.app._lokasi+"' and modul='BILLAKRU'");
...
window.app_saku2_report_kopeg_sju_flPiutangJurnal.extend(window.childForm);
window.app_saku2_report_kopeg_sju_flPiutangJurnal.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from ju_m where kode_lokasi='"+this.app._lokasi+"' and modul='BILLAKRU' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku2_report_kopeg_sju_flPiutangJurnal]::mainButtonClick:"+e);
		}
	},
