window.app_saku2_report_kopeg_sju_flAggProd = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_sju_flAggProd.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_sju_flAggProd";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Group COB ","All",""]);
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Tertanggung","All",""]);
	this.gridLib.SGEditData(this.sg1,4,[0,1,2],["PIC","All",""]);
	this.gridLib.SGEditData(this.sg1,5,[0,1,2],["Jenis Anggaran","=","Gross"]);
...
window.app_saku2_report_kopeg_sju_flAggProd.extend(window.portalui_childForm);
window.app_saku2_report_kopeg_sju_flAggProd.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["Gross","Net"]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.filter=this.filterRep.filterStr("x.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						...
						this.filterRep.filterStr("x.kode_tipe",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
						this.filterRep.filterStr("x.kode_cust",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
						this.filterRep.filterStr("x.kode_pic",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
			this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);	
			if (this.sg1.getCell(2,5)=="Gross")
			{
				this.nama_report="server_report_saku2_kopeg_sju_rptAggBulanProd";
			}
			else
			{
				this.nama_report="server_report_saku2_kopeg_sju_rptAggBulanProdNet";
			}
