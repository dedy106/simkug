window.app_saku2_report_kopeg_sju_flHutangAging = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_sju_flHutangAging.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_sju_flHutangAging";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sju_bill_m where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Penanggung","All",""));
...
window.app_saku2_report_kopeg_sju_flHutangAging.extend(window.childForm);
window.app_saku2_report_kopeg_sju_flHutangAging.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_bill, a.keterangan from sju_bill_m a "+filter,
												"select count(a.no_bill) from sju_bill_m a "+filter,
												new Array("a.no_bill","a.keterangan"),"and",new Array("No Invoice","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from sju_bill_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2 =this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCellDateValue(2,4);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_kopeg_sju_rptHutangAging";
			...
			systemAPI.alert("[app_saku2_report_kopeg_sju_flHutangAging]::mainButtonClick:"+e);
		}
	},
