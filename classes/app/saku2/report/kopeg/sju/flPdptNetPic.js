window.app_saku2_report_kopeg_sju_flPdptNetPic = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_sju_flPdptNetPic.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_sju_flPdptNetPic";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Jenis Laporan","=","GROSS"));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Acc Exec","All",""));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Tertanggung","All",""));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Jenis Laporan","=","Tertanggung"));
	this.doSelectCell(this.sg1,2,0);
...
window.app_saku2_report_kopeg_sju_flPdptNetPic.extend(window.childForm);
window.app_saku2_report_kopeg_sju_flPdptNetPic.implement({
	...
	{		
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["123","3","3","13","13","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,0,0,2,2,0]);	
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["3","3","3","13","13","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3],[3,0,0,2,2,0]);	
		}
		if (row == 1)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,5)=="Tertanggung")
				{
					if (this.sg1.getCell(2,2)=="NET")
					{
						this.nama_report = "server_report_saku2_kopeg_sju_rptPdptNet";
					}
					else
					{
						this.nama_report = "server_report_saku2_kopeg_sju_rptPdptGross";
					}
				}
				if (this.sg1.getCell(2,5)=="PIC")
				{
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.kode_cust",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							this.filterRep.filterStr("a.kode_pic",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
					if (this.sg1.getCell(2,2)=="NET")
					{
			...
			systemAPI.alert("[app_saku2_report_kopeg_sju_flPdptNetPic]::mainButtonClick:"+e);
		}
	},
