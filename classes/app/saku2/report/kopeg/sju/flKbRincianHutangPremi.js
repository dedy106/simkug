window.app_saku2_report_kopeg_sju_flKbRincianHutangPremi = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_sju_flKbRincianHutangPremi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_sju_flKbRincianHutangPremi";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBHUTPOL'");
...
window.app_saku2_report_kopeg_sju_flKbRincianHutangPremi.extend(window.childForm);
window.app_saku2_report_kopeg_sju_flKbRincianHutangPremi.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBHUTPOL' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku2_report_kopeg_sju_flKbRincianHutangPremi]::mainButtonClick:"+e);
		}
	},
