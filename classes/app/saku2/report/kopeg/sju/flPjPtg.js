window.app_saku2_report_kopeg_sju_flPjPtg = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_sju_flPjPtg.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_sju_flPjPtg";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from ptg_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_kopeg_sju_flPjPtg.extend(window.childForm);
window.app_saku2_report_kopeg_sju_flPjPtg.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from ptg_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("d.nik_pengaju",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
				...
				this.nama_report = "server_report_saku2_kopeg_sju_rptPjPtg";
			...
			systemAPI.alert("[app_saku2_report_kopeg_sju_flPjPtg]::mainButtonClick:"+e);
		}
	},
