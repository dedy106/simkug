window.app_saku2_report_kopeg_sju_flGlNeracaRincianSimulasi = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_sju_flGlNeracaRincianSimulasi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_sju_flGlNeracaRincianSimulasi";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode FS","=","FS2"));
...
window.app_saku2_report_kopeg_sju_flGlNeracaRincianSimulasi.extend(window.childForm);
window.app_saku2_report_kopeg_sju_flGlNeracaRincianSimulasi.implement({
		...
		try{			
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data FS",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_fs,nama from fs where kode_fs in ('FS2','FS3') "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," and "),
														  "select count(kode_fs) from fs where kode_fs in ('FS2','FS3')  "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," and "),
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				if (this.sg1.getCell(2,2)=="FS2")
				{
					this.nama_report="server_report_saku2_kopeg_sju_rptGlNeracaRincianSimulasi";
				}
				if (this.sg1.getCell(2,2)=="FS3")
				{
					this.nama_report="server_report_saku2_kopeg_sju_rptGlPasivaRincianSimulasi";
				}
	...
	doOpenTb: function(kode_neraca,kode_lokasi,periode){
		...
		nama_report="server_report_saku2_kopeg_sju_rptGlTbSimulasi";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenBb: function(kode_akun,kode_lokasi,periode){
		...
		this.periode=periode;
		this.link="2";
		filter2=kode_lokasi+"/"+periode+"/"+this.app._nikUser+"/"+"Tidak/1";
