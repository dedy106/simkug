window.app_saku2_report_kopeg_sju_flPrKwitansi = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_sju_flPrKwitansi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_sju_flPrKwitansi";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sju_tagih_m where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("No Polis","All",""));
...
window.app_saku2_report_kopeg_sju_flPrKwitansi.extend(window.childForm);
window.app_saku2_report_kopeg_sju_flPrKwitansi.implement({
		...
		try{			
			...
			if (row ==5)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Tagihan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_tagih, a.keterangan from sju_tagih_m a "+filter,
												"select count(a.no_tagih) from sju_tagih_m a "+filter,
												new Array("a.no_tagih","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}			
		}catch(e){O
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from sju_tagih_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							  this.filterRep.filterStr("a.no_tagih",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_kopeg_sju_rptPrKwitansi";
			...
			systemAPI.alert("[app_saku2_report_kopeg_sju_flPrKwitansi]::mainButtonClick:"+e);
		}
	},
