window.app_saku2_report_kopeg_sju_flJuBukti = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_sju_flJuBukti.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_sju_flJuBukti";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from ju_m where kode_lokasi='"+this.app._lokasi+"' and modul='JUASUR'");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1), new Array("Tanggal","All"));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1), new Array("No Bukti","All"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku2_report_kopeg_sju_flJuBukti.extend(window.childForm);
window.app_saku2_report_kopeg_sju_flJuBukti.implement({
		...
		try{			
			...
			if (row == 3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  ...
														  new Array("no_ju","keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		else
		{
			...
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3],[3,0,1,2]);	
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from ju_m where kode_lokasi='"+this.app._lokasi+"' and modul='JUASUR' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				{
					tanggal=" and a.tanggal='"+this.sg1.getCellDateValue(2,2)+"'";
				}
				if (this.sg1.getCell(1,2)=="Range")
				{
					tanggal=" and (a.tanggal between '"+this.sg1.getCellDateValue(2,2)+"' and '"+this.sg1.getCellDateValue(3,2)+"') ";
				}
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," and ")+
							this.filterRep.filterStr("a.no_ju",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3)," and ")+tanggal+
							" and a.modul='JUASUR' ";
