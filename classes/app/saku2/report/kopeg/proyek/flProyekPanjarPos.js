window.app_saku2_report_kopeg_proyek_flProyekPanjarPos = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_proyek_flProyekPanjarPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_proyek_flProyekPanjarPos";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from pr_beban_d where kode_lokasi='"+this.app._lokasi+"' and modul='PANJAR'");
...
window.app_saku2_report_kopeg_proyek_flProyekPanjarPos.extend(window.childForm);
window.app_saku2_report_kopeg_proyek_flProyekPanjarPos.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_aju, a.keterangan from pr_beban_d a "+this.filter,
												"select count(*) from pr_beban_d a "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from pr_beban_d where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku2_kopeg_proyek_rptProyekPanjarPos";
			...
			systemAPI.alert("[app_saku2_report_kopeg_proyek_flProyekPanjarPos]::mainButtonClick:"+e);
		}
	},
