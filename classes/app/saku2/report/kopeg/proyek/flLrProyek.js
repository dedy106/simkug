// JavaScript Document
window.app_saku2_report_kopeg_proyek_flLrProyek = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_proyek_flLrProyek.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_proyek_flLrProyek";
		this.maximize();
	...
	var tanda="=";
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where modul in ('KBUMPRO') and kode_lokasi='"+this.app._lokasi+"'");
	this.project=this.dbLib.getPeriodeFromSQL("select max(kode_project) as periode from project where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Kode Proyek","=",this.project ));
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Nilai Laporan","=","Saldo Akhir"));
...
window.app_saku2_report_kopeg_proyek_flLrProyek.extend(window.portalui_childForm);
window.app_saku2_report_kopeg_proyek_flLrProyek.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row ==5)
		{
			this.filterRep.ListDataSGFilter(this, "Data Proyek",this.sg1, this.sg1.row, this.sg1.col,
													  "select kode_project, keterangan from project "+
													  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
													  "select count(*) from project "+
													  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
													  new Array("kode_project","keterangan"),"and");
		}
	},
	doSelectCell: function(sender, col, row){
		...
		else
		{
			...
			this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5,6),new  Array("3","3","123","3","3","123","3"));			
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5,6),new  Array(3,0,2,0,0,2,0));
		}
		if (row == 1)
		{
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Saldo Akhir","Mutasi","DC"));
		}
		...
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="1")
				{
					...
					this.nama_report = "server_report_saku2_gl_rptLR";
			...
			else
			{
				...
				this.filter2 = this.nik_user+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(2,3)+"/"+this.app._namaForm+"/"+this.sg1.getCell(2,5)+"/"+this.sg1.getCell(3,5);
				var in_lap="S";
				if (this.sg1.getCell(1,5) == "=")
				{
					this.kode_lokasi1=this.sg1.getCell(2,5);
					this.kode_lokasi2=this.sg1.getCell(2,5);
				}
				if (this.sg1.getCell(1,5) == "Range")
				{
					this.kode_lokasi1=this.sg1.getCell(2,5);
					this.kode_lokasi2=this.sg1.getCell(3,5);
				}
				if (this.sg1.getCell(2,6) == "Saldo Akhir")
				{
					this.nama_report="server_report_saku2_gl_rptLR";
				}
				if (this.sg1.getCell(2,6) == "Mutasi")
				{
					this.nama_report="server_report_saku2_gl_rptLRMutasi";
				}
				if (this.sg1.getCell(2,6) == "DC")
				{
					this.nama_report="server_report_saku2_gl_rptLRMutasiDC";
				}
				//this.nama_report="server_report_saku2_gl_rptLRPp";
				this.nama_pp=this.dbLib.getPeriodeFromSQL("select nama as periode from pp where kode_lokasi='"+this.sg1.getCell(2,0)+"' and kode_pp='"+this.sg1.getCell(2,5)+"' ");
				sql="call sp_neraca_project ('"+this.sg1.getCell(2,2)+"','L','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,1)+"','"+this.app._lokasi+"','"+this.app._lokasi+"','"+this.app._lokasi+"','"+this.nik_user+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,5)+"')";
	...
	doOpenTb: function(kode_neraca,kode_lokasi){
		...
		filter="";
		var sql = "call sp_glma_pp_trail_tmp ('"+this.sg1.getCell(2,2)+"','"+kode_neraca+"','"+kode_lokasi+"','"+kode_lokasi+"','"+kode_lokasi+"','"+this.sg1.getCell(2,1)+"','"+this.app._nikUser+"','"+this.sg1.getCell(2,5)+"')";
		this.dbLib.execQuerySync(sql);	
		this.nama_report="server_report_saku2_gl_rptTBLajurTrail";
	...
	doOpenBb: function(kode_akun,kode_lokasi){
		...
		this.link="2";
		filter = "";
		filter2 = this.app._namaForm+"/"+this.app._nikUser+"/"+kode_akun+"/"+this.sg1.getCell(2,1)+"/"+kode_lokasi+"/"+this.sg1.getCell(2,5)+"/"+this.nama_pp;
		this.nama_report="server_report_saku2_gl_rptBukuBesarPpTrail";
		this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
