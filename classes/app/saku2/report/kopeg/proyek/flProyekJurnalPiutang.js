window.app_saku2_report_kopeg_proyek_flProyekJurnalPiutang = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_proyek_flProyekJurnalPiutang.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_proyek_flProyekJurnalPiutang";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from pr_piutang_m  where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku2_report_kopeg_proyek_flProyekJurnalPiutang.extend(window.childForm);
window.app_saku2_report_kopeg_proyek_flProyekJurnalPiutang.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_piutang, a.keterangan from pr_piutang_m a "+filter,
												"select count(a.no_piutang) from pr_piutang_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from pr_piutang_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku2_report_kopeg_proyek_flProyekJurnalPiutang]::mainButtonClick:"+e);
		}
	},
