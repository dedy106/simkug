window.app_saku2_report_kopeg_proyek_flProyekSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kopeg_proyek_flProyekSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kopeg_proyek_flProyekSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from pr_proyek_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_kopeg_proyek_flProyekSaldo.extend(window.childForm);
window.app_saku2_report_kopeg_proyek_flProyekSaldo.implement({
		...
		try{			
			...
			if (row ==4)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							his.filterRep.filterStr("a.jenis",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filterRep.ListDataSGFilter(this, "Data Proyek",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_proyek, a.keterangan from pr_proyek_m a "+filter,
												"select count(a.no_proyek) from pr_proyek_m a "+filter,
												new Array("a.no_proyek","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","123","123","13","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,2,0,2]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct tahun from pr_proyek_m where kode_lokasi='"+this.app._lokasi+"' order by tahun",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("PROYEK","PELATIHAN"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="1")
				{
					this.nama_report = "server_report_saku2_kopeg_proyek_rptProyekSaldo";
				...
				if (this.link=="2")
				{
					this.doOpenKartu(this.no_proyek,this.kode_lokasi);
				}
				if (this.link=="3")
				{
					this.doOpenBukti(this.no_aju,this.kode_lokasi);
				}
			...
			systemAPI.alert("[app_saku2_report_kopeg_proyek_flProyekSaldo]::mainButtonClick:"+e);
		}
	},
	...
	doOpenKartu: function(no_proyek,kode_lokasi){
		this.no_proyek=no_proyek;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter2 = "";
		filter =" where a.no_proyek='"+no_proyek+"' and a.kode_lokasi='"+kode_lokasi+"'";
		this.nama_report="server_report_saku2_kopeg_proyek_rptProyekKartu";
	...
	doOpenBukti: function(no_aju,kode_lokasi,modul){
		this.no_aju=no_aju;
		this.kode_lokasi=kode_lokasi;
		this.modul=modul;
		this.link="2";
		...
		{
			filter2="";
			filter = "where a.no_aju='"+no_aju+"' and a.kode_lokasi='"+kode_lokasi+"' ";
			nama_report="server_report_saku2_kopeg_proyek_rptProyekBebanForm";
		}
		if (modul=="JU")
		{
			filter="";
			filter2 = "where a.no_ju='"+no_aju+"' and a.kode_lokasi='"+kode_lokasi+"' ";
			nama_report="server_report_saku2_gl_rptBuktiJurnal";
		}
		if (modul=="KB")
		{
			filter="";
			filter2 = "where a.no_kas='"+no_aju+"' and a.kode_lokasi='"+kode_lokasi+"' ";
			nama_report="server_report_saku2_kb_rptKbBuktiJurnal";
		}
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	},
