window.app_saku2_report_fa_flFaKartu = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_fa_flFaKartu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_fa_flFaKartu";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(a.periode) as periode from fa_asset a "+
...
window.app_saku2_report_fa_flFaKartu.extend(window.childForm);
window.app_saku2_report_fa_flFaKartu.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from fa_asset where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			}else
			{
				...
				this.nama_report="server_report_saku2_fa_rptFaKartu";
