window.app_saku2_report_fa_flFaAssetYks = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_fa_flFaAssetYks.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_fa_flFaAssetYks";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,[0,1,2], ["Kelompok Akun","All",""]);
	this.gridLib.SGEditData(this.sg1,3,[0,1], ["No Asset","All"]);
	this.gridLib.SGEditData(this.sg1,4,[0,1,2], ["Jenis Laporan","=","Akun"]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku2_report_fa_flFaAssetYks.extend(window.childForm);
window.app_saku2_report_fa_flFaAssetYks.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select a.periode from (select distinct periode from fa_asset where kode_lokasi='"+this.app._lokasi+"' "+
									"union "+
									"select distinct periode from periode where kode_lokasi='"+this.app._lokasi+"')a order by a.periode desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["Akun","Asset","BDD"]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				var jenis=" and a.jenis<>'B' ";
				if (this.sg1.getCell(2,4)=="BDD")
				{
					jenis=" and a.jenis='B' ";
				}
				this.filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_klpakun",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_fa",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+jenis							;
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
				if (this.sg1.getCell(2,4)=="Akun")
				{
					this.nama_report="server_report_saku2_fa_rptFaAssetAkun";
				}
				else
				{
					this.nama_report="server_report_saku2_fa_rptFaAsset";
				}
