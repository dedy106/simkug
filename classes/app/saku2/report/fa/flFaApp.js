window.app_saku2_report_fa_flFaApp = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_fa_flFaApp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_fa_flFaApp";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from fa_baps_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku2_report_fa_flFaApp.extend(window.childForm);
window.app_saku2_report_fa_flFaApp.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 2)
			{
				...
				var sql1="select a.no_baps,a.keterangan from fa_baps_m a "+this.filter;
				var sql2="select count(a.no_baps) from fa_baps_m a "+this.filter;
				this.filterRep.ListDataSGFilter(this, "Data Approval",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,["a.no_baps","a.keterangan"],"and",["No Anggaran","Keterangan"]);
			}	
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct(periode) as periode from fa_baps_m "+
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
		    	...
		    	this.filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.no_baps",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.nama_report="server_report_saku2_fa_rptFaApp";	
				this.filter2="";
