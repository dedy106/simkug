// JavaScript Document
window.app_saku2_report_gl_flTbFakultas = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_gl_flTbFakultas.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_gl_flTbFakultas";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Bentuk Neraca","=","Neraca Lajur"));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Mutasi Nol","=","Tidak"));
...
window.app_saku2_report_gl_flTbFakultas.extend(window.portalui_childForm);
window.app_saku2_report_gl_flTbFakultas.implement({
	...
	doSelectCell : function(sender, col, row){
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5),new  Array("123","123","123","123","3","3"));
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5),new  Array(2,0,2,2,0,0));
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5),new  Array("3","123","123","123","3","3"));
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5),new  Array(3,0,2,2,0,0));
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_pp=this.dbLib.getPeriodeFromSQL("select nama as periode from pp where kode_lokasi='"+this.sg1.getCell(2,0)+"' and kode_pp='"+this.sg1.getCell(2,2)+"' ");
				this.filter2 = this.nik_user+"/"+this.sg1.getCell(2,5)+"/"+this.sg1.getCell(2,1)+"/0";
				...
				if (this.sg1.getCell(2,4)=="Neraca Percobaan")
				{
					this.sql = "call sp_glma_pp_tmp ('"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"','"+this.kode_pp1+"','"+this.kode_pp2+"')";
					this.nama_report="server_report_saku2_gl_rptTBPercobaan";
				}
				if (this.sg1.getCell(2,4)=="Neraca Lajur")
				{
					this.sql = "call sp_glma_pp_dw_tmp ('"+this.kode_lokasi+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";								
					this.nama_report="server_report_saku2_gl_rptTBLajurFakultas";
					
				}
				if (this.sg1.getCell(2,4)=="12 Periode")
				{
					this.sql = "call sp_glma12_pp_tmp ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"','"+this.kode_pp1+"','"+this.kode_pp2+"')";
					this.nama_report="server_report_saku2_gl_rptTB12";
				}
				alert(this.sql);
				this.dbLib.execQuerySync(this.sql);	
	...
	doOpenBb: function(kode_akun,kode_lokasi){
		...
		this.link="1";
		filter = "";
		filter2 = this.app._namaForm+"/"+this.app._nikUser+"/"+kode_akun+"/"+this.sg1.getCell(2,1)+"/"+kode_lokasi+"/"+this.sg1.getCell(2,2)+"/"+this.nama_pp;
		nama_report="server_report_saku2_gl_rptBukuBesarPpTrail";
