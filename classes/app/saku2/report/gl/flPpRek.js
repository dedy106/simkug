// JavaScript Document
window.app_saku2_report_gl_flPpRek = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_gl_flPpRek.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_gl_flPpRek";
		this.maximize();
...
window.app_saku2_report_gl_flPpRek.extend(window.portalui_childForm);
window.app_saku2_report_gl_flPpRek.implement({
	    ...
	    if (row ==0)
			...
			if (row == 1)
			{
				this.filterRep.ListDataSGFilter(this, "Data PP",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.kode_pp, a.nama from pp_rek a  "+
														   this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," and ")+" order by a.kode_pp",
														  "select count(a.kode_pp) from pp_rek a "+
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.nama_report="server_report_saku2_gl_rptPpRek";
