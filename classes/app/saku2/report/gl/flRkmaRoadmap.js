// JavaScript Document
window.app_saku2_report_gl_flRkmaRoadmap = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_gl_flRkmaRoadmap.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_gl_flRkmaRoadmap";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Tahun","=",this.app._periode.substr(0,4)));
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Perbandingan Tahun","=","5 Tahun"));
...
window.app_saku2_report_gl_flRkmaRoadmap.extend(window.portalui_childForm);
window.app_saku2_report_gl_flRkmaRoadmap.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) as periode from periode where kode_lokasi='"+this.app._lokasi+"' order by substring(periode,1,4) desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("5 Tahun","4 Tahun","3 Tahun","4 Tahun","1 Tahun"));
		}
	},
		...
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="1")
				{
					...
					this.nama_report = "server_report_saku2_gl_rptRkma";
			...
			else
			{
				...
				this.filter2 = this.nik_user+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,3)+"/"+this.app._periode+"/"+this.sg1.getCell(2,4);
				this.nama_report="server_report_saku2_gl_rptRkmaRoadmap";
				sql="call sp_neraca4 ('"+this.sg1.getCell(2,2)+"','L','"+in_lap+"',"+this.sg1.getCell(2,3)+",'201012','201112','201212','201307','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.nik_user+"')";
