// JavaScript Document
window.app_saku2_report_gl_flLRBidang = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_gl_flLRBidang.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_gl_flLRBidang";
		this.maximize();
...
window.app_saku2_report_gl_flLRBidang.extend(window.portalui_childForm);
window.app_saku2_report_gl_flLRBidang.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row ==5)
		{
			this.filterRep.ListDataSGFilter(this, "Data Bidang",this.sg1, this.sg1.row, this.sg1.col,
													  ...
													  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
													  ["kode_bidang","nama"],"and",["Kode PP","Nama"]);
		}
	},
	doSelectCell: function(sender, col, row){
		...
		else
		{
			...
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5),new  Array(3,0,2,0,0,2));
		}
		if (row == 1)
		{
			if (this.app._kodeLokasiKonsol==this.app._lokasi)
	...
	mainButtonClick: function(sender){
		try
		{
			...
			{
				this.nama_report="server_report_saku2_gl_rptLRPpMutasi";
			}
			if (this.sg1.getCell(2,6) == "DC")
			{
				this.nama_report="server_report_saku2_gl_rptLRPpMutasiDC";
			}
			//this.nama_report="server_report_saku2_gl_rptLRPp";
			sql="call sp_neraca_bidang ('"+this.sg1.getCell(2,2)+"','L','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,1)+"','"+this.app._lokasi+"','"+this.app._lokasi+"','"+this.app._lokasi+"','"+this.nik_user+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,5)+"')";
