// JavaScript Document
window.app_saku2_report_gl_flNeraca4Konsol = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_gl_flNeraca4Konsol.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_gl_flNeraca4Konsol";
		this.maximize();
...
window.app_saku2_report_gl_flNeraca4Konsol.extend(window.portalui_childForm);
window.app_saku2_report_gl_flNeraca4Konsol.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5,6,7),new  Array("123","3","123","3","3","3","3","3"));
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5,6,7),new  Array(2,0,2,0,0,0,0,0));
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5,6,7),new  Array("3","3","123","3","3","3","3","3"));
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5,6,7),new  Array(3,2,2,0,0,0,0,0));
		}
		if (row == 1)
		{
			if (this.app._kodeLokasiKonsol==this.app._lokasi)
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenTbKonsol(this.kode_neraca,this.kode_lokasi,this.periode);
				}
				if (this.link=="3")
				{
					this.doOpenTbLokasi(this.kode_akun,this.kode_lokasi,this.periode);
				}
				if (this.link=="4")
				{
					this.doOpenTb(this.kode_akun,this.kode_lokasi,this.periode);
				}
				if (this.link=="5")
				{
			...
			else
			{
				...
				this.filter2 = this.nik_user+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(2,5)+"/"+this.sg1.getCell(2,6)+"/"+this.sg1.getCell(2,3)+"/"+this.app._namaForm;
				var in_lap="S";
				if (this.app._kodeLokasiKonsol==this.app._lokasi)
				{
				...
				sql="call sp_neraca4 ('"+this.sg1.getCell(2,2)+"','A','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,4)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,6)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.nik_user+"')";
				this.nama_report="server_report_saku2_gl_rptNeraca4Konsol";
	...
	doOpenTbKonsol: function(kode_neraca,kode_lokasi,periode){
		this.kode_neraca=kode_neraca;
		this.link="1";
		filter2 = "trail1_"+this.app._nikUser+"/"+periode;
		filter="";
		var sql = "call sp_glma_konsol_trail_tmp ('"+this.sg1.getCell(2,2)+"','"+this.kode_neraca+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+periode+"','trail1_"+this.app._nikUser+"')";
		
		this.dbLib.execQuerySync(sql);	
		nama_report="server_report_saku2_gl_rptTBLajurKonsolTrail";
	...
	doOpenTbLokasi: function(kode_akun,kode_lokasi,periode){
		this.kode_akun=kode_akun;
		this.lok=kode_lokasi;
		this.link="2";
		filter2 = "trail2_"+this.app._nikUser+"/"+periode;
		filter="";
		var sql = "call sp_glma_lok_trail_tmp ('"+this.kode_akun+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+periode+"','trail2_"+this.app._nikUser+"')";
		
		this.dbLib.execQuerySync(sql);	
		nama_report="server_report_saku2_gl_rptTBLajurLokasiTrail";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenTb: function(kode_akun,kode_lokasi,periode){
		this.kode_akun=kode_akun;
		this.kode_lokasi=kode_lokasi;
		this.periode=periode;
		this.link="3";
		filter2 = "trail3_"+this.app._nikUser+"/"+periode ;
		filter="";
		var sql = "call sp_glma_lok_detail_trail_tmp ('"+this.kode_akun+"','"+this.kode_lokasi+"','"+this.kode_lokasi+"','"+this.kode_lokasi+"','"+periode+"','trail3_"+this.app._nikUser+"')";
	...
	doOpenBb: function(kode_akun,kode_lokasi,periode){
		...
		this.link="4";
		this.periode=periode;
		filter2 = kode_lokasi+"/"+periode+"/"+this.app._nikUser+"/"+kode_akun;
		filter="";
		nama_report="server_report_saku2_gl_rptBukuBesarTrail";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
