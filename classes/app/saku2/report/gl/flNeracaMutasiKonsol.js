// JavaScript Document
window.app_saku2_report_gl_flNeracaMutasiKonsol = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_gl_flNeracaMutasiKonsol.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_gl_flNeracaMutasiKonsol";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Bentuk Mutasi","=","DC"));
...
window.app_saku2_report_gl_flNeracaMutasiKonsol.extend(window.portalui_childForm);
window.app_saku2_report_gl_flNeracaMutasiKonsol.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Mutasi","DC"));
		}
		if (row == 5)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2 = this.nik_user+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(2,5)+"/"+this.sg1.getCell(2,3)+"/"+this.app._namaForm;
				var in_lap="S";
				//alert(this.app._kodeLokasiKonsol);
				if (this.app._kodeLokasiKonsol==this.app._lokasi)
				{
				...
				if (this.sg1.getCell(2,4)=="Mutasi")
				{
					this.nama_report="server_report_saku2_gl_rptNeracaMutasiKonsol";
				}
				else
				{
					this.nama_report="server_report_saku2_gl_rptNeracaMutasiDCKonsol";
				}	
	...
	doOpenTbKonsol: function(kode_neraca,kode_lokasi){
		this.kode_neraca=kode_neraca;
		this.link="1";
		filter2 = "trail1_"+this.app._nikUser+"/"+this.sg1.getCell(2,1) ;
		filter="";
		var sql = "call sp_glma_konsol_trail_tmp ('"+this.sg1.getCell(2,2)+"','"+this.kode_neraca+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.sg1.getCell(2,1)+"','trail1_"+this.app._nikUser+"')";
	...
	doOpenTbLokasi: function(kode_akun,kode_lokasi){
		this.kode_akun=kode_akun;
		this.lok=kode_lokasi;
		this.link="2";
		filter2 = "trail2_"+this.app._nikUser+"/"+this.sg1.getCell(2,1) ;
		filter="";
		var sql = "call sp_glma_lok_trail_tmp ('"+this.kode_akun+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.sg1.getCell(2,1)+"','trail2_"+this.app._nikUser+"')";
		this.dbLib.execQuerySync(sql);	
		nama_report="server_report_saku2_gl_rptTBLajurLokasiTrail";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenTb: function(kode_akun,kode_lokasi){
		this.kode_akun=kode_akun;
		this.lok=kode_lokasi;
		this.link="3";
		filter2 = "trail3_"+this.app._nikUser+"/"+this.sg1.getCell(2,1) ;
		filter="";
		var sql = "call sp_glma_lok_detail_trail_tmp ('"+this.kode_akun+"','"+this.lok+"','"+this.lok+"','"+this.lok+"','"+this.sg1.getCell(2,1)+"','trail3_"+this.app._nikUser+"')";
		this.dbLib.execQuerySync(sql);	
	...
	doOpenBb: function(kode_akun,kode_lokasi){
		...
		this.akun=kode_akun;
		this.link="4";
		filter2 = this.lok+"/"+this.sg1.getCell(2,1)+"/trail4_"+this.app._nikUser+"/"+kode_akun;
		filter="";
	...
	doOpenJurnal: function(no_bukti,kode_lokasi){
		...
		this.lok=kode_lokasi;
		this.link="5";
		filter = "where no_bukti='"+no_bukti+"' and kode_lokasi='"+this.lok+"' ";
		filter2="";
