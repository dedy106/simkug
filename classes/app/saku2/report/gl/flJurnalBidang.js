// JavaScript Document
window.app_saku2_report_gl_flJurnalBidang = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_gl_flJurnalBidang.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_gl_flJurnalBidang";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode Bidang","=",this.app._kodeBidang));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1), new Array("Kode PP","All"));
	...
	this.gridLib.SGEditData(this.sg1,7,new Array(0,1), new Array("Tanggal","All"));
	this.gridLib.SGEditData(this.sg1,8,new Array(0,1,2), new Array("Sort by","=","Tanggal, No. Bukti"));
...
window.app_saku2_report_gl_flJurnalBidang.extend(window.portalui_childForm);
window.app_saku2_report_gl_flJurnalBidang.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 2)
		{
			this.filterRep.ListDataSGFilter(this, "Data Bidang",this.sg1, this.sg1.row, this.sg1.col,
													  ...
													  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
													  ["kode_bidang","nama"],"and",["Kode Bidang","Nama"]);
		}
		if (row == 3)
		{
		...
		if (row == 4)
		{
			this.filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
						...
						this.filterRep.filterStr("a.kode_bidang",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," and ")+
		...
		if (row == 6)
		{
			...
			var filter =this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
						...
						this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
						this.filterRep.filterStr("a.kode_akun",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
			...
			var sql1="select a.no_bukti,a.tanggal "+
					"from (select distinct a.no_bukti,a.tanggal "+
					"	  from gldt a "+
					"     inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+ filter+
					"	  union  "+
					"	  select distinct a.no_bukti,a.tanggal "+
					"	  from gldt_h a "+
					"     inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+ filter+
					"	  )a "; 
			var sql2="select count(a.no_bukti) "+
					"from (select distinct a.no_bukti,a.tanggal "+
					"	  from gldt a "+
					"     inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+ filter+
					"	  union  "+
	...
	doSelectCell: function(sender, col, row){
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5,6,7,8),new  Array("123","3","123","123","123","123","13","3","123"));
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5,6,7,8),new  Array(2,2,2,2,2,2,2,1,0));
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5,6,7,8),new  Array("123","3","123","123","123","123","13","3","123"));
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5,6,7,8),new  Array(3,2,2,2,2,2,2,1,0));
		}
		if (row == 1)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.filter=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
						...
						this.filterRep.filterStr("a.tanggal",this.sg1.getCell(1,7),this.sg1.getCellDateValue(2,7),this.sg1.getCellDateValue(3,7),"and");
			this.filter2=
			this.urut="";
			if (this.sg1.getCell(2,8)=="Tanggal, No. Bukti")
			{ 
				this.urut=" b.no_bukti,b.tanggal,b.no_urut";
			}
			if (this.sg1.getCell(2,8)=="Tanggal, No. Dokumen")
			{ 
				this.urut=" b.no_dokumen,b.tanggal,b.no_urut";
			}
			if (this.sg1.getCell(2,8)=="No. Dokumen, Tanggal")
			{ 
				this.urut=" b.tanggal,b.no_dokumen,b.no_urut";
			}
			
			this.nama_report="server_report_saku2_gl_rptJurnalBidang";
