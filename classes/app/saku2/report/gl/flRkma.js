// JavaScript Document
window.app_saku2_report_gl_flRkma = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_gl_flRkma.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_gl_flRkma";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode FS","=","FP1"));
...
window.app_saku2_report_gl_flRkma.extend(window.portalui_childForm);
window.app_saku2_report_gl_flRkma.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.pp=this.dbLib.getPeriodeFromSQL("select nama as periode from pp where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.sg1.getCell(2,5)+"'");
				this.filter2 = this.nik_user+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(2,3)+"/"+this.pp+"/"+this.kode_lokasi+"/"+this.sg1.getCell(2,5);
				this.nama_report="server_report_saku2_gl_rptRkma";
				sql="call sp_neraca_produk_pp ('"+this.sg1.getCell(2,2)+"','L','"+in_lap+"',"+this.sg1.getCell(2,3)+",'"+this.sg1.getCell(2,1)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.nik_user+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,5)+"')";
