window.app_saku2_report_yks_flTakPiutang = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_yks_flTakPiutang.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_yks_flTakPiutang";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_valid_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('TAKKIRIM','TAKTERIMA')");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Jenis TAK","=","TAK Piutang Kirim"));
...
window.app_saku2_report_yks_flTakPiutang.extend(window.childForm);
window.app_saku2_report_yks_flTakPiutang.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			{
				var tak="TAKKIRIM";
				if (this.sg1.getCell(2,2)=="TAK Piutang Terima")
				{
					tak="TAKTERIMA";
				}
				...
				this.filterRep.ListDataSGFilter(this, "Data Droping",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_valid, a.keterangan from yk_valid_m a "+this.filter,
												"select count(*) from yk_valid_m a "+this.filter,
												new Array("a.no_valid","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","23","3","123","3"]);
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from yk_valid_m where modul in ('TAKKIRIM','TAKTERIMA') "+this.filter+" order by periode",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("TAK Piutang Kirim","TAK Piutang Terima"));
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Bentuk Laporan","=","TAK Piutang Kirim"));
				this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Jenis Laporan","=","Summary"));
			}else{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_valid",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							" and a.modul='"+tak+"' ";
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();	
				if (this.sg1.getCell(2,4)=="Summary")
				{
					this.nama_report = "server_report_saku2_yks_rptPiutangKirim";
					if (this.sg1.getCell(2,2)=="TAK Piutang Terima")
					{
						this.nama_report = "server_report_saku2_yks_rptPiutangTerima";
					}
				}
				else
				{
					this.nama_report = "server_report_saku2_yks_rptPiutangKirimNik";
					if (this.sg1.getCell(2,2)=="TAK Piutang Terima")
					{
						this.nama_report = "server_report_saku2_yks_rptPiutangTerimaNik";
					}
				}
			...
			systemAPI.alert("[app_saku2_report_yks_flTakPiutang]::mainButtonClick:"+e);
		}
	},
