window.app_saku2_report_yks_flHutangKartu = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_yks_flHutangKartu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_yks_flHutangKartu";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select substring(max(periode),1,4) as periode from yk_hutang_m a where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_yks_flHutangKartu.extend(window.childForm);
window.app_saku2_report_yks_flHutangKartu.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2],["3","6","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2],[2,0,2]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select a.tahun "+
									"from (select distinct substring(periode,1,4) as tahun,kode_lokasi from yk_hutang_m "+
									"union "+
									"select distinct substring(periode,1,4) as tahun,kode_lokasi from yk_rekon_m) a "+
									"where a.kode_lokasi='"+this.app._lokasi+"' order by a.tahun",[this.sg1.columns.get(2).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			}else
			{
				...
				this.nama_report="server_report_saku2_yks_rptHutangKartu";
