window.app_saku2_report_yks_flPiutangTagihan = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_yks_flPiutangTagihan.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_yks_flPiutangTagihan";
		this.maximize();
    ...
    this.no_valid=this.dbLib.getPeriodeFromSQL("select max(no_valid) as periode from yk_valid_m where kode_lokasi='"+this.app._lokasi+"' and modul='VALID'");	
	...
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("No Bukti","=",this.no_valid));
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Bentuk Laporan","=","Tagihan Mitra"));
	this.gridLib.SGEditData(this.sg1,7,new Array(0,1,2), new Array("Jenis Laporan","=","Layar"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	this.doSelectCell(this.sg1,2,7);
};
window.app_saku2_report_yks_flPiutangTagihan.extend(window.childForm);
window.app_saku2_report_yks_flPiutangTagihan.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["3","3","13","123","123","3","3","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[2,0,0,2,2,2,0,0]);	
		if (row == 1)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Mitra","All",""));
				this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Peserta","All",""));
				this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("No Validasi","=",this.no_valid));
				this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Bentuk Laporan","=","Tagihan Mitra"));
			}else
			{
				...
				this.nama_report="server_report_saku2_yks_rptPiutangTagihan";
				if (this.sg1.getCell(2,6)=="Tagihan NIK")
				{
					this.nama_report="server_report_saku2_yks_rptPiutangNik";
				}				
				this.filter2=this.filter2+"/"+this.sg1.getCell(2,1)+"/"+this.filter3+"/"+this.sg1.getCell(2,5)+"/"+this.sg1.getCell(2,7);
