window.app_saku2_report_yks_flPiutangJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_yks_flPiutangJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_yks_flPiutangJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_valid_j ");
...
window.app_saku2_report_yks_flPiutangJurnal.extend(window.childForm);
window.app_saku2_report_yks_flPiutangJurnal.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 5)
			{
				...
				var sql1="select distinct a.no_valid,a.no_dokumen from yk_valid_j a "+filter; 
				var sql2="select count(distinct a.no_valid) from yk_valid_j a "+filter;
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
												new Array("a.no_valid","a.no_dokumen"),"and",new Array("No Kas","No Dokumen"),"and");
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["3","3","123","123","123","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,0,2,2,2,2]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from yk_valid_j where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter=this.filter+"/"+"/"+this.sg1.getCell(2,1)+"/"+this.filter2;
				this.nama_report="server_report_saku2_yks_rptPiutangJurnal";
