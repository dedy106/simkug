window.app_saku2_report_yks_flIcdx = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_yks_flIcdx.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_yks_flIcdx";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan ICDX", 2);
	...
	{
		this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_bill_m ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode ICDX","All",""));
	//this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	this.pager=10;
	
};
window.app_saku2_report_yks_flIcdx.extend(window.childForm);
window.app_saku2_report_yks_flIcdx.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 3)
			{
				this.filterRep.ListDataSGFilter(this, "Data ICDX",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_icdx,nama from yk_icdx ",
														  "select count(kode_icdx) from yk_icdx ",
														  new Array("kode_icdx","nama"),"where");
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			if (this.app._kodeLokasiKonsol==this.app._lokasi)
			{
				...
				this.dbLib.setItemsFromSQL("select distinct periode from yk_bill_m order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
			}
			else
			{
				this.sg1.columns.get(2).pickList.clear();
				this.dbLib.setItemsFromSQL("select distinct periode from yk_bill_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc " ,[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
			}
		}
		if (row == 3)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode ICDX","=",this.app._kodeFs));
			...
			}else
			{
				...
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(3,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(3,1);
				...
				this.nama_report="server_report_saku2_yks_rptIcdx";
