window.app_saku2_report_yks_flHutangSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_yks_flHutangSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_yks_flHutangSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(a.periode) as periode "+
									"from (select distinct periode,kode_lokasi from yk_hutang_m "+
									"union "+
									"select distinct periode,kode_lokasi from yk_rekon_m) a "+
...
window.app_saku2_report_yks_flHutangSaldo.extend(window.childForm);
window.app_saku2_report_yks_flHutangSaldo.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			}else
			{
				...
				this.filter2 = this.app._nikUser+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,3);
				this.nama_report="server_report_saku2_yks_rptHutangSaldo";
