window.app_saku2_report_yks_flPiutangSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_yks_flPiutangSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_yks_flPiutangSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(a.periode) as periode "+
									"from (select distinct periode,kode_lokasi from yk_valid_m "+
									"union "+
...
window.app_saku2_report_yks_flPiutangSaldo.extend(window.childForm);
window.app_saku2_report_yks_flPiutangSaldo.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["3","3","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3],[2,0,2]);	
		if (row == 1)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			...
			}else
			{
				...
				this.nama_report="server_report_saku2_yks_rptPiutangSaldo";
