window.app_saku2_report_yks_flTakPiutangJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_yks_flTakPiutangJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_yks_flTakPiutangJurnal";
		this.maximize();
...
window.app_saku2_report_yks_flTakPiutangJurnal.extend(window.childForm);
window.app_saku2_report_yks_flTakPiutangJurnal.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 6)
			{
					...
					var sql1="select a.no_valid,a.keterangan from yk_valid_m a "+filter; 
					var sql2="select count(a.no_valid) from yk_valid_m a "+filter;
					this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
													new Array("a.no_valid","a.keterangan"),"and",new Array("No Jurnal","Keterangan"),"and");			
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["123","3","13","123","123","123","123","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[2,0,0,2,2,2,2,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select periode from yk_valid_j where kode_lokasi='"+this.sg1.getCell(2,0)+"' "+
									"order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct modul from yk_valid_m where kode_lokasi='"+this.sg1.getCell(2,0)+"' "+
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter2 = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_valid",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and")+
							this.filterRep.filterStr("a.modul",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+		
							this.filterRep.filterStr("a.modul",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+		
							this.filterRep.filterStr("a.no_valid",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and");
				this.filter2=this.filter+"/"+this.sg1.getCell(2,1)+"/"+this.filter2;
				this.nama_report="server_report_saku2_yks_rptTakPiutangJurnal";				
