window.app_saku2_report_kpa_flPjPtgForm = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kpa_flPjPtgForm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kpa_flPjPtgForm";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from itt_panjarptg_m ");
...
window.app_saku2_report_kpa_flPjPtgForm.extend(window.childForm);
window.app_saku2_report_kpa_flPjPtgForm.implement({
		...
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Panjar",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_ptg, a.keterangan from itt_panjarptg_m a "+this.filter,
												"select count(*) from itt_panjarptg_m a "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from itt_panjarptg_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku2_kpa_rptPjPtgForm";
			...
			systemAPI.alert("[app_saku2_report_kpa_flPjPtgForm]::mainButtonClick:"+e);
		}
	},
