window.app_saku2_report_kpa_flPjCair = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kpa_flPjCair.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kpa_flPjCair";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where modul='KBPJ' ");
...
window.app_saku2_report_kpa_flPjCair.extend(window.childForm);
window.app_saku2_report_kpa_flPjCair.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where modul='KBPJ' and kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku2_report_kpa_flPjCair]::mainButtonClick:"+e);
		}
	},
