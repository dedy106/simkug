window.app_saku2_report_kpa_flKpaSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kpa_flKpaSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kpa_flKpaSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from itt_kpa_m ");
...
window.app_saku2_report_kpa_flKpaSaldo.extend(window.childForm);
window.app_saku2_report_kpa_flKpaSaldo.implement({
		...
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Panjar",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_kpa, a.keterangan from itt_kpa_m a "+this.filter,
												"select count(*) from itt_kpa_m a "+this.filter,
												new Array("a.no_kpa","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from itt_kpa_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_kpa",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_kpa_rptKpaSaldo";
			...
			systemAPI.alert("[app_saku2_report_kpa_flKpaSaldo]::mainButtonClick:"+e);
		}
	},
