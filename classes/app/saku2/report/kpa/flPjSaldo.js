window.app_saku2_report_kpa_flPjSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kpa_flPjSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kpa_flPjSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from itt_panjar_m ");
...
window.app_saku2_report_kpa_flPjSaldo.extend(window.childForm);
window.app_saku2_report_kpa_flPjSaldo.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				if (this.app._userStatus=="A")
				{
					...
					var sql2="select count(a.kode_pp) from pp a where a.tipe='Posting' "+filter;
				}
				if (this.app._userStatus=="P")
				{
					var sql="select a.kode_pp,a.nama "+
							"from pp a "+
							"inner join karyawan_pp b on a.kode_lokasi=b.kode_lokasi and a.kode_pp=b.kode_pp "+
							"where b.nik='"+this.app._userLog+"' "+filter;
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Karyawan",this.sg1, this.sg1.row, this.sg1.col,
												"select nik, nama from karyawan "+filter,
												"select count(*) from karyawan "+filter,
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Panjar",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_panjar, a.keterangan from itt_panjar_m a "+filter,
												"select count(*) from itt_panjar_m a "+filter,
												new Array("a.no_panjar","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (this.app._userStatus=="P")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","123","23","123","123"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,2,2,2]);	
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from itt_panjar_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_panjar",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_kpa_rptPjSaldo";
			...
			systemAPI.alert("[app_saku2_report_kpa_flPjSaldo]::mainButtonClick:"+e);
		}
	},
	...
	doOpenPanjar: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_panjar='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._namaUser;
		nama_report="server_report_saku2_kpa_rptPjAjuForm";
	...
	doOpenPtg: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_ptg='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._namaUser;
		nama_report="server_report_saku2_kpa_rptPjPtgForm";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
