window.app_saku2_report_investasi_flPpSpi = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_investasi_flPpSpi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_investasi_flPpSpi";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_ppspi_m ");
...
window.app_saku2_report_investasi_flPpSpi.extend(window.childForm);
window.app_saku2_report_investasi_flPpSpi.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data SPI",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_spi, a.keterangan from inv_ppspi_m a "+filter,
												"select count(a.no_spi) from inv_ppspi_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from inv_ppspi_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku2_investasi_rptPpSpi";
			...
			systemAPI.alert("[app_saku2_report_investasi_flPpSpi]::mainButtonClick:"+e);
		}
	},
