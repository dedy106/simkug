window.app_saku2_report_investasi_flRdSpiBulan = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_investasi_flRdSpiBulan.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_investasi_flRdSpiBulan";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from inv_rdspi_m ");
...
window.app_saku2_report_investasi_flRdSpiBulan.extend(window.childForm);
window.app_saku2_report_investasi_flRdSpiBulan.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Kelola",this.sg1, this.sg1.row, this.sg1.col,
												"select a.kode_rd, a.nama from inv_rdkelola a "+filter,
												"select count(a.kode_rd) from inv_rdkelola a "+filter,
			...
			if (row ==4)
			{
				filter=this.filterRep.filterStr("c.jenis",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"where");
						this.filterRep.filterStr("b.kode_rd",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
				this.filterRep.ListDataSGFilter(this, "Data Reksadana",this.sg1, this.sg1.row, this.sg1.col,
												...
												"inner join inv_rdkelola c on b.kode_rd=c.kode_rd "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) as periode from inv_rdspi_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter = this.filterRep.filterStr("c.jenis",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"where")+
							this.filterRep.filterStr("a.kode_rd",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.kode_rd",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_investasi_rptRdSpiBulan";
			...
			systemAPI.alert("[app_saku2_report_investasi_flRdSpiBulan]::mainButtonClick:"+e);
		}
	},
