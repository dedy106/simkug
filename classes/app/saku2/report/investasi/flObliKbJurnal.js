window.app_saku2_report_investasi_flObliKbJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_investasi_flObliKbJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_investasi_flObliKbJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('KBOBLBELI','KBOBLJUAL','KBOBLCAIR') ");
...
window.app_saku2_report_investasi_flObliKbJurnal.extend(window.childForm);
window.app_saku2_report_investasi_flObliKbJurnal.implement({
		...
		try{			
			...
			if (row == 2)
			{
				var filter=this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
					...
					" and a.modul in ('KBOBLBELI','KBOBLJUAL','KBOBLCAIR') ";
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_saku2_investasi_rptObliKbJurnal";
