window.app_saku2_report_investasi_flPp = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_investasi_flPp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_investasi_flPp";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Kode Properti","All",""));
...
window.app_saku2_report_investasi_flPp.extend(window.childForm);
window.app_saku2_report_investasi_flPp.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			if (row == 0)
			{
				this.filterRep.ListDataSGFilter(this, "Data Properti",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_properti, nama from inv_saham "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(kode_properti) from inv_saham "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_properti",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where");
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_investasi_rptPp";
			...
			systemAPI.alert("[app_saku2_report_investasi_flPp]::mainButtonClick:"+e);
		}
	},
