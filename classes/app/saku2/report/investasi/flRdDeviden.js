window.app_saku2_report_investasi_flRdDeviden = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_investasi_flRdDeviden.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_investasi_flRdDeviden";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_rddev_m ");
...
window.app_saku2_report_investasi_flRdDeviden.extend(window.childForm);
window.app_saku2_report_investasi_flRdDeviden.implement({
		...
		try{			
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Broker / MI",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_rdkelola, nama from inv_rdkelola ",
														  "select count(kode_rdkelola) from inv_rdkelola ",
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Penjualan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_rddev, a.keterangan from inv_rddev_m a "+filter,
												"select count(a.no_rddev) from inv_rddev_m a "+filter,
												new Array("a.no_rddev","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from inv_rddev_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row === 4)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("b.kode_rdkelola",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_rddev",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_investasi_rptRdDeviden";
				if (this.sg1.getCell(2,4)=="Detail")
				{
					this.nama_report = "server_report_saku2_investasi_rptRdDevidenDetail";
				}
			...
			systemAPI.alert("[app_saku2_report_investasi_flRdDeviden]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBayar: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter = "where a.no_rddev='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
