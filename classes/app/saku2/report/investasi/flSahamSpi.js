window.app_saku2_report_investasi_flSahamSpi = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_investasi_flSahamSpi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_investasi_flSahamSpi";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_shmspi_m ");
...
window.app_saku2_report_investasi_flSahamSpi.extend(window.childForm);
window.app_saku2_report_investasi_flSahamSpi.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data SPI",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_spi, a.keterangan from inv_shmspi_m a "+filter,
												"select count(a.no_spi) from inv_shmspi_m a "+filter,
												new Array("a.no_spi","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from inv_shmspi_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_spi",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_investasi_rptSahamSpi";
			...
			systemAPI.alert("[app_saku2_report_investasi_flSahamSpi]::mainButtonClick:"+e);
		}
	},
