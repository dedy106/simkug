window.app_saku2_report_investasi_flDocAkru = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_investasi_flDocAkru.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_investasi_flDocAkru";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_depoakru_m ");
...
window.app_saku2_report_investasi_flDocAkru.extend(window.childForm);
window.app_saku2_report_investasi_flDocAkru.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data SPI",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_akru, a.keterangan from inv_depoakru_m a "+filter,
												"select count(a.no_akru) from inv_depoakru_m a "+filter,
												new Array("a.no_akru","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from inv_depoakru_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku2_investasi_rptDocAkru";
			...
			systemAPI.alert("[app_saku2_report_investasi_flDocAkru]::mainButtonClick:"+e);
		}
	},
