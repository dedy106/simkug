window.app_saku2_report_investasi_flRdKbJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_investasi_flRdKbJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_investasi_flRdKbJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('KBRDBELI','KBRDJUAL','KBRDDEV') ");
...
window.app_saku2_report_investasi_flRdKbJurnal.extend(window.childForm);
window.app_saku2_report_investasi_flRdKbJurnal.implement({
		...
		try{			
			...
			if (row == 2)
			{
				var filter=this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
					...
					" and a.modul in ('KBSHMBELI','KBSHMJUAL','KBSHMDEV') ";
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul like 'KBDOC%' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							...
							" and a.modul in ('KBRDBELI','KBRDJUAL','KBRDDEV') ";
				this.filter2=this.sg1.getCell(2,1);
				this.nama_report="server_report_saku2_investasi_rptRdKbJurnal";
