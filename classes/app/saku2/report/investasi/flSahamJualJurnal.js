window.app_saku2_report_investasi_flSahamJualJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_investasi_flSahamJualJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_investasi_flSahamJualJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_shmjual_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_investasi_flSahamJualJurnal.extend(window.childForm);
window.app_saku2_report_investasi_flSahamJualJurnal.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_shmjual, keterangan from inv_shmjual_m  "+filter ,
														  "select count(no_shmjual) from inv_shmjual_m "+filter,
														  new Array("no_shmjual","keterangan"),"and");
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from inv_shmjual_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							...
							this.filterRep.filterStr("a.no_shmjual",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," and ");
				this.filter2=this.sg1.getCell(2,1);
				this.nama_report="server_report_saku2_investasi_rptSahamJualJurnal";
