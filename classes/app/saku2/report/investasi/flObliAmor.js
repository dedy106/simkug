window.app_saku2_report_investasi_flObliAmor = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_investasi_flObliAmor.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_investasi_flObliAmor";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_obliamor_m ");
...
window.app_saku2_report_investasi_flObliAmor.extend(window.childForm);
window.app_saku2_report_investasi_flObliAmor.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Amortisasi",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_amor, a.keterangan from inv_obliamor_m a "+filter,
												"select count(a.no_amor) from inv_obliamor_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from inv_obliamor_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_amor",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_investasi_rptObliAmor";
			...
			systemAPI.alert("[app_saku2_report_investasi_flObliAmor]::mainButtonClick:"+e);
		}
	},
