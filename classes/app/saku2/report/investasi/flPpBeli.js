window.app_saku2_report_investasi_flPpBeli = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_investasi_flPpBeli.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_investasi_flPpBeli";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_ppbeli_m ");
...
window.app_saku2_report_investasi_flPpBeli.extend(window.childForm);
window.app_saku2_report_investasi_flPpBeli.implement({
		...
		try{			
			...
			if (row ==3)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_properti",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filterRep.ListDataSGFilter(this, "Data Pembelian",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_beli, a.keterangan from inv_ppbeli_m a "+filter,
												"select count(a.no_beli) from inv_ppbeli_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from inv_ppbeli_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku2_investasi_rptPpBeli";
			...
			systemAPI.alert("[app_saku2_report_investasi_flPpBeli]::mainButtonClick:"+e);
		}
	},
