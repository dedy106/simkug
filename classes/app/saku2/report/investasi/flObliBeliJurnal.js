window.app_saku2_report_investasi_flObliBeliJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_investasi_flObliBeliJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_investasi_flObliBeliJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_oblibeli_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_investasi_flObliBeliJurnal.extend(window.childForm);
window.app_saku2_report_investasi_flObliBeliJurnal.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_beli, keterangan from inv_oblibeli_m  "+filter ,
														  "select count(no_beli) from inv_oblibeli_m "+filter,
														  new Array("no_beli","keterangan"),"and");
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from inv_oblibeli_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_saku2_investasi_rptObliBeliJurnal";
