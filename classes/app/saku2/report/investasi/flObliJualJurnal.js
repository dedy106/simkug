window.app_saku2_report_investasi_flObliJualJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_investasi_flObliJualJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_investasi_flObliJualJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_oblijual_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_investasi_flObliJualJurnal.extend(window.childForm);
window.app_saku2_report_investasi_flObliJualJurnal.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_jual, keterangan from inv_oblijual_m  "+filter ,
														  "select count(no_jual) from inv_oblijual_m "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from inv_oblijual_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_saku2_investasi_rptObliJualJurnal";
