window.app_saku2_report_investasi_flDocKkp = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_investasi_flDocKkp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_investasi_flDocKkp";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_depo_m ");
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Progress","=","Aktif"));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("No Bukti","All",""));
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Tanggal Jatuh Tempo","All",""));
...
window.app_saku2_report_investasi_flDocKkp.extend(window.childForm);
window.app_saku2_report_investasi_flDocKkp.implement({
		...
		try{			
			...
			if (row == 3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Bank",this.sg1, this.sg1.row, this.sg1.col,
														  ...
														  new Array("kode_bank","nama"),"and");
			}
			
			if (row ==5)
			{
				var progress="";
				if (this.sg1.getCell(2,4)=="Aktif")
				{
					progress=" and a.progress<>'X' ";
				}
				if (this.sg1.getCell(2,4)=="Close")
				{
					progress=" and a.progress='X' ";
				}
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_bank",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+progress;
				this.filterRep.ListDataSGFilter(this, "Data Penempatan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_depo, a.keterangan from inv_depo_m a "+filter,
												"select count(a.no_depo) from inv_depo_m a "+filter,
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123","1236","13","123","13","123","1236"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[2,0,0,2,0,2,1]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from inv_depo_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Close","Aktif"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,4)=="Aktif")
				{
					progress=" and a.progress in ('0','1') ";
				}
				if (this.sg1.getCell(2,4)=="Close")
				{
					progress=" and a.progress in ('X','Z') ";
				}
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.modul",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.bsumber",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.no_depo",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+progress;
				this.filter2 = this.sg1.getCell(2,1)+";"+this.sg1.getCell(1,6)+";"+this.sg1.getCellDateValue(2,6)+";"+this.sg1.getCellDateValue(3,6);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_investasi_rptDocKkp";
			...
			systemAPI.alert("[app_saku2_report_investasi_flDocKkp]::mainButtonClick:"+e);
		}
	},
