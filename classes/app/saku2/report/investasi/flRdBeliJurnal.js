window.app_saku2_report_investasi_flRdBeliJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_investasi_flRdBeliJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_investasi_flRdBeliJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_rdbeli_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_investasi_flRdBeliJurnal.extend(window.childForm);
window.app_saku2_report_investasi_flRdBeliJurnal.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_rdbeli, keterangan from inv_rdbeli_m  "+filter ,
														  "select count(no_rdbeli) from inv_rdbeli_m "+filter,
														  new Array("no_rdbeli","keterangan"),"and");
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from inv_rdbeli_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							...
							this.filterRep.filterStr("a.no_rdbeli",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," and ");
				this.filter2=this.sg1.getCell(2,1);
				this.nama_report="server_report_saku2_investasi_rptRdBeliJurnal";
