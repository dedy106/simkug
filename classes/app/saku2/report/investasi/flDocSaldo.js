window.app_saku2_report_investasi_flDocSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_investasi_flDocSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_investasi_flDocSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(a.periode) as periode "+
											"from (select periode from inv_depoakru_d "+
											"	  union "+
											"	  select periode from inv_depocair_m) a ");
...
window.app_saku2_report_investasi_flDocSaldo.extend(window.childForm);
window.app_saku2_report_investasi_flDocSaldo.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Penempatan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_depo, a.keterangan from inv_depoakru_d a "+filter,
												"select count(a.no_depo) from inv_depoakru_d a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct a.periode as periode "+
									"from (select periode from inv_depoakru_d "+
									"	  union "+
									"	  select periode from inv_depocair_m) a "+
		...
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					...
					this.doOpenKartu(this.no_depo,this.kode_lokasi);					
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_bank",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_depo",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_investasi_rptDocSaldo";
			...
			systemAPI.alert("[app_saku2_report_investasi_flDocSaldo]::mainButtonClick:"+e);
		}
	},
	...
	doOpenKartu: function(no_depo,kode_lokasi){
		this.no_depo=no_depo;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = "where a.no_depo='"+no_depo+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
		nama_report="server_report_saku2_investasi_rptDocKartu";
	...
	doOpenJurnal: function(no_bukti,kode_lokasi){
		...
		this.kode_lokasi=kode_lokasi;
		this.link="2";
		if (no_bukti.substr(3,2)=="BM" || no_bukti.substr(3,2)=="BK")
		{
			filter = "where a.no_kas='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
			nama_report="server_report_saku2_investasi_rptDocKbJurnal";
		}
		if (no_bukti.substr(3,2)=="DA")
		{
			filter = "where a.no_akru='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
			nama_report="server_report_saku2_investasi_rptDocJurnal";
		}
		if (no_bukti.substr(3,2)=="DC")
		{
			filter = "where a.no_cair='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
			nama_report="server_report_saku2_investasi_rptDocRevJurnal";
		}
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
