window.app_saku2_report_anggaran_flAggLembur = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_anggaran_flAggLembur.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_anggaran_flAggLembur";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from agg_lembur_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_anggaran_flAggLembur.extend(window.childForm);
window.app_saku2_report_anggaran_flAggLembur.implement({
		...
		try{			
			...
			if (row == 3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_lembur, tahun from agg_lembur_m "+filter,
														  "select count(no_lembur) from agg_lembur_m "+filter,
														  ["no_lembur","tahun"],"and",["No Bukti","Tahun"]);
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct tahun from agg_lembur_m where kode_lokasi='"+this.app._lokasi+"' order by tahun",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku2_anggaran_rptAggLembur";
			...
			systemAPI.alert("[app_saku2_report_anggaran_flAggLembur]::mainButtonClick:"+e);
		}
	},
