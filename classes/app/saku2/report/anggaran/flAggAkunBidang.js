window.app_saku2_report_anggaran_flAggAkunBidang = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_anggaran_flAggAkunBidang.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_anggaran_flAggAkunBidang";
		this.maximize();
...
window.app_saku2_report_anggaran_flAggAkunBidang.extend(window.childForm);
window.app_saku2_report_anggaran_flAggAkunBidang.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[123,0,2,2,0]);
		if (row == 0)
		{
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
							  ...
							  this.filterRep.filterStr("h.kode_bidang",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				...
				{
					var sql =  "select a.kode_akun,f.nama as nama_akun,zz.kode_lokasi,zz.nama as nama_lokasi,h.kode_bidang,h.nama as nama_bidang, "+
							...
							"left join ( "+
							"	 select a.kode_lokasi,a.kode_akun,h.kode_bidang, "+
							...
							"	 from agg_d a "+
							"	  inner join pp h on a.kode_pp=h.kode_pp and a.kode_lokasi=h.kode_lokasi "+this.filter+
							"	 group by a.kode_lokasi,a.kode_akun,h.kode_bidang "+
							"		 )g on a.kode_lokasi=g.kode_lokasi and a.kode_akun=g.kode_akun and h.kode_bidang=g.kode_bidang "+this.filter+
							"group by a.kode_akun,f.nama,zz.kode_lokasi,zz.nama,h.kode_bidang,h.nama "+
							"order by zz.kode_lokasi,zz.nama,h.kode_bidang,h.nama,a.kode_akun,f.nama ";	
					this.scriptSqlCount = "select count(a.kode_akun) from (select a.kode_akun "+
							...
							"	 group by a.kode_lokasi,a.kode_akun,h.kode_bidang "+
							"		 )g on a.kode_lokasi=g.kode_lokasi and a.kode_akun=g.kode_akun and h.kode_bidang=g.kode_bidang "+this.filter+
							"group by a.kode_akun,f.nama,zz.kode_lokasi,zz.nama,h.kode_bidang,h.nama) a";
				
					this.title = new server_util_arrayList({items:["Kode Akun","Nama Akun","Lok","Nama Lokasi","Kode Bidang","Nama Bidang","Januari","Februari","Maret","April","Mei","Juni","Juli","Agustus","September","Oktober","November","Desember","Total"]});
				...
				if (this.sg1.getCell(2,4)=="Triwulan")
				{
					...
					this.scriptSqlCount = "select count(xx.kode_akun) from ("+
					        "select a.kode_akun,f.nama as nama_akun,zz.kode_lokasi,zz.nama as nama_lokasi,h.kode_bidang,h.nama as nama_bidang, "+
							"	   isnull(sum(g.agg_tw1),0) as agg_01,isnull(sum(g.agg_tw2),0) as agg_02,isnull(sum(g.agg_tw3),0) as agg_03,isnull(sum(g.agg_tw4),0) as agg_04,"+
							"      isnull(sum(g.agg_total),0) as agg_total "+
							"from (select a.kode_lokasi,a.tahun,a.kode_akun,h.kode_bidang "+	
							"	  from agg_d a "+
							...
							"	 group by a.kode_lokasi,a.kode_akun,h.kode_bidang "+
							"		 )g on a.kode_lokasi=g.kode_lokasi and a.kode_akun=g.kode_akun and h.kode_bidang=g.kode_bidang "+this.filter+
							"group by a.kode_akun,f.nama,zz.kode_lokasi,zz.nama,h.kode_bidang,h.nama) xx ";
							
							
					this.title = new server_util_arrayList({items:["Kode Akun","Nama Akun","Lok","Nama Lokasi","Kode Bidang","Nama Bidang","Triwulan I","Triwulan II","Triwulan III","Triwulan IV","Total"]});
			...
			systemAPI.alert("[app_saku2_report_anggaran_flAggAkunBidang]::mainButtonClick:"+e);
		}
	},
