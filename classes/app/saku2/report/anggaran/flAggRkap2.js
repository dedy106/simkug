window.app_saku2_report_anggaran_flAggRkap2 = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_anggaran_flAggRkap2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_anggaran_flAggRkap2";
		this.maximize();
	...
	if (this.app._lokasi== this.app._kodeLokasiKonsol)
	{
		...
		this.outlook=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from agg_outlook ");
	...
	else
	{
		...
		this.outlook=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from agg_outlook where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Periode Outlook","=",this.outlook));
	this.gridLib.SGEditData(this.sg1,7,new Array(0,1,2), new Array("Versi Laporan","=","Fixed Variabel"));
...
window.app_saku2_report_anggaran_flAggRkap2.extend(window.childForm);
window.app_saku2_report_anggaran_flAggRkap2.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["23","3","123","3","3","123","3","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[2,0,2,0,0,2,0,0]);	
		if (row == 1)
		{
		...
		if (row == 6)
		{
			...
			if (this.app._lokasi== this.app._kodeLokasiKonsol)
			{
				this.dbLib.setItemsFromSQL("select distinct periode from agg_outlook order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
			}
			else
			{
				this.dbLib.setItemsFromSQL("select distinct periode from agg_outlook where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
			}
		}
		if (row == 3)
		{
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Fixed Variabel","Program"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenAkun(this.kode_akun,this.kode_lokasi,this.tahun,this.jenis);
				}
			...
			else
			{
				...
				this.filter2 = this.app._nikUser+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,3)+"/"+this.sg1.getCell(2,4)+"/"+this.ver+"/"+this.kode_lokasi1+"/"+this.kode_lokasi2+"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(2,6)+"/"+this.sg1.getCell(2,7)+"/"+this.sg1.getCell(2,5)+"/"+this.sg1.getCell(3,5)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,6);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				
				this.ver="0";
				if (this.sg1.getCell(2,7)=="Fixed Variabel")
				{
					this.nama_report = "server_report_saku2_anggaran_rptAggRkap2";
					sql="call sp_agg_rkap_total_inv2 ('"+this.sg1.getCell(2,2)+"','"+this.ver+"','"+this.sg1.getCell(2,1)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(3,5)+"','"+this.app._nikUser+"','"+this.sg1.getCell(2,6)+"')";
				}
				else
				{
					this.nama_report = "server_report_saku2_anggaran_rptAggRkap";
					sql="call sp_agg_rkap_total_inv ('"+this.sg1.getCell(2,2)+"','"+this.ver+"','"+this.sg1.getCell(2,1)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(3,5)+"','"+this.app._nikUser+"','"+this.sg1.getCell(2,6)+"')";
				}
			...
			systemAPI.alert("[app_saku2_report_anggaran_flAggRkap2]::mainButtonClick:"+e);
		}
	},
	...
	doOpenRka: function(kode_akun,kode_lokasi,periode_out,jenis){
		this.kode_akun=kode_akun;
		this.kode_lokasi=kode_lokasi;
		this.periode_out=periode_out;
		this.link="1";
		filter2 = periode_out+"/"+jenis ;
		filter="where a.kode_akun='"+kode_akun+"' and a.kode_lokasi='"+kode_lokasi+"' and a.periode='"+periode_out+"'";
		this.lokasi=this.app._namalokasi;
		nama_report="server_report_saku2_anggaran_rptAggOutAkunTrail";
	...
	doOpenAkun: function(kode_akun,kode_lokasi,tahun,jenis){
		this.kode_akun=kode_akun;
		this.kode_lokasi=kode_lokasi;
		this.tahun=tahun;
		this.jenis=jenis;
		...
		if (jenis=="fix")
		{
			filter="where a.kode_akun='"+kode_akun+"' and a.kode_lokasi='"+kode_lokasi+"' and a.tahun='"+tahun+"' and a.modul in ('BTPKK','ASSET','BGAJI','BPMBN','BPP')";
		}
		else
		{
			filter="where a.kode_akun='"+kode_akun+"' and a.kode_lokasi='"+kode_lokasi+"' and a.tahun='"+tahun+"' and a.modul in ('BO','BLEMBUR','BSPPD','ABAU','INV','BTPKU','BP','CC','PKUNJ')";
		}
		filter2 = tahun+"/"+this.sg1.getCell(2,1) ;
		
		nama_report="server_report_saku2_anggaran_rptAggBulanTrail";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenAkunTrans: function(kode_akun,kode_lokasi,tahun){
		this.kode_akun=kode_akun;
		this.kode_lokasi=kode_lokasi;
		this.tahun=tahun;
		this.link="2";
		filter2 = tahun+"/"+this.sg1.getCell(2,1) ;
		filter="where a.kode_akun='"+kode_akun+"' and a.kode_lokasi='"+kode_lokasi+"' and a.tahun='"+tahun+"'";
		nama_report="server_report_saku2_anggaran_rptAggBulanTrans";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
