window.app_saku2_report_anggaran_flAggDrk = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_anggaran_flAggDrk.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_anggaran_flAggDrk";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Kode DRK","All",""]);
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Periodik Laporan","=","Bulanan"]);
	this.gridLib.SGEditData(this.sg1,4,[0,1,2],["Kode Bidang","All",""]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	
};
window.app_saku2_report_anggaran_flAggDrk.extend(window.childForm);
window.app_saku2_report_anggaran_flAggDrk.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 2)
		{
			this.filterRep.ListDataSGFilter(this, "Daftar DRK",this.sg1, this.sg1.row, this.sg1.col,
													  "select kode_drk, nama from drk where kode_lokasi = '"+this.app._lokasi+"' ",
													  "select count(kode_drk) from drk where kode_lokasi = '"+this.app._lokasi+"'  ",
	...
	doSelectCell: function(sender, col, row){
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","3","123","3","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[123,0,2,0,2]);
		if (row == 0)
		{
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
							  ...
							  this.filterRep.filterStr("a.kode_drk",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							  this.filterRep.filterStr("h.kode_bidang",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				...
				{
					var sql =  "select a.kode_drk,f.nama as nama_akun,zz.kode_lokasi,zz.nama as nama_lokasi,h.kode_bidang,h.nama as nama_bidang, "+
							...
							"from (select a.kode_lokasi,a.tahun,a.kode_drk,h.kode_bidang "+	
							"	  from agg_d a "+
							"	  inner join pp h on a.kode_pp=h.kode_pp and a.kode_lokasi=h.kode_lokasi "+this.filter+
							"group by a.kode_lokasi,a.tahun,a.kode_drk,h.kode_bidang ) a  "+
							"inner join agg_drk f on a.kode_drk=f.kode_drk  and f.tahun='"+this.sg1.getCell(2,1)+"' "+
							...
							"		 )g on a.kode_lokasi=g.kode_lokasi and a.kode_drk=g.kode_drk and h.kode_bidang=g.kode_bidang "+this.filter+
							"group by a.kode_drk,f.nama,zz.kode_lokasi,zz.nama,h.kode_bidang,h.nama order by a.kode_drk";	
					
					this.scriptSqlCount = "select count(a.kode_drk) from (select a.kode_drk "+
							"from (select a.kode_lokasi,a.tahun,a.kode_drk,h.kode_bidang "+	
							"	  from agg_d a "+
							"	  inner join pp h on a.kode_pp=h.kode_pp and a.kode_lokasi=h.kode_lokasi "+this.filter+
							"group by a.kode_lokasi,a.tahun,a.kode_drk,h.kode_bidang ) a  "+
							...
							"	 group by a.kode_lokasi,a.kode_drk,h.kode_bidang "+
							"		 )g on a.kode_lokasi=g.kode_lokasi and a.kode_drk=g.kode_drk and h.kode_bidang=g.kode_bidang "+this.filter+
							"group by a.kode_drk,f.nama,zz.kode_lokasi,zz.nama,h.kode_bidang,h.nama)a ";	
					
					
					this.title = new server_util_arrayList({items:["Kode DRK","Nama DRK","Lok","Nama Lokasi","Kode Bidang","Nama Bidang","Januari","Februari","Maret","April","Mei","Juni","Juli","Agustus","September","Oktober","November","Desember","Total"]});
					this.widthTable = new server_util_arrayList({items:[70,200,50,100,50,100,80,80,80,80,80,80,80,80,80,80,80,80,90]});
					this.fieldType = new server_util_arrayList({items:["S","S","S","S","S","S","N","N","N","N","N","N","N","N","N","N","N","N","N"]});																
				...
				if (this.sg1.getCell(2,3)=="Triwulan")
				{
					var sql =  "select a.kode_drk,f.nama as nama_akun,zz.kode_lokasi,zz.nama as nama_lokasi,h.kode_bidang,h.nama as nama_bidang, "+
							...
							"      sum(case when a.bulan between '04' and '06' then a.nilai else 0 end) as agg_tw2, "+
							"      sum(case when a.bulan between '07' and '09' then a.nilai else 0 end) as agg_tw3, "+
							"      sum(case when a.bulan between '10' and '12' then a.nilai else 0 end) as agg_tw4 "+							
							"	 from agg_d a "+
							"	  inner join pp h on a.kode_pp=h.kode_pp and a.kode_lokasi=h.kode_lokasi "+this.filter+
							"	 group by a.kode_lokasi,a.kode_drk,h.kode_bidang "+
							"		 )g on a.kode_lokasi=g.kode_lokasi and a.kode_drk=g.kode_drk and h.kode_bidang=g.kode_bidang "+this.filter+
							"group by a.kode_drk,f.nama,zz.kode_lokasi,zz.nama,h.kode_bidang,h.nama order by a.kode_drk,h.kode_bidang";
			
					
					this.scriptSqlCount = "select count(xx.kode_drk) from( "+
						    "select a.kode_drk,f.nama as nama_akun,zz.kode_lokasi,zz.nama as nama_lokasi,h.kode_bidang,h.nama as nama_bidang, "+
							"	   isnull(sum(g.agg_tw1),0) as agg_01,isnull(sum(g.agg_tw2),0) as agg_02,isnull(sum(g.agg_tw3),0) as agg_03,isnull(sum(g.agg_tw4),0) as agg_04,"+
							"      isnull(sum(g.agg_total),0) as agg_total "+
							"from (select a.kode_lokasi,a.tahun,a.kode_drk,h.kode_bidang "+	
							"	  from agg_d a "+
							...
							"	 group by a.kode_lokasi,a.kode_drk,h.kode_bidang "+
							"		 )g on a.kode_lokasi=g.kode_lokasi and a.kode_drk=g.kode_drk and h.kode_bidang=g.kode_bidang "+this.filter+
							"group by a.kode_drk,f.nama,zz.kode_lokasi,zz.nama,h.kode_bidang,h.nama order by a.kode_drk) xx ";
					this.title = new server_util_arrayList({items:["Kode DRK","Nama DRK","Lok","Nama Lokasi","Kode Bidang","Nama Bidang","Triwulan I","Triwulan II","Triwulan III","Triwulan IV","Total"]});
					this.widthTable = new server_util_arrayList({items:[70,200,50,100,50,100,80,80,80,80,90]});
			...
			systemAPI.alert("[app_saku2_report_anggaran_flAggDrk]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		this.namaLokasi=this.dbLib.getPeriodeFromSQL("select nama as periode from lokasi where kode_lokasi='"+this.sg1.getCell(2,0)+"' ");
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'>"+this.namaLokasi.toUpperCase()+"<br>"+this.app._namaForm+"<br>Posisi : "+this.sg1.getCell(2,1)+"<br />";
		var d = new Date();
