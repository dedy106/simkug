window.app_saku2_report_anggaran_flAggOutBidang = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_anggaran_flAggOutBidang.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_anggaran_flAggOutBidang";
		this.maximize();
...
window.app_saku2_report_anggaran_flAggOutBidang.extend(window.childForm);
window.app_saku2_report_anggaran_flAggOutBidang.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3],[123,0,2,2]);
		if (row == 0)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var sql =  "select a.kode_akun,b.nama,c.kode_bidang,d.nama,sum(a.rka) as rka,sum(a.realisasi) as realisasi,sum(a.outlook) as outlook, "+
						"		sum(case when a.rka<>0 then convert(decimal(5,2),(a.outlook/a.rka)*100,0) else 0 end) as persen	"+
						"from agg_outlook a "+
						"inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
						"inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
						"inner join bidang d on c.kode_bidang=d.kode_bidang "+this.filter+
						"group by a.kode_akun,b.nama,c.kode_bidang,d.nama "+
						"order by c.kode_bidang,d.nama,a.kode_akun,b.nama  ";	
				this.scriptSqlCount =  "select count(a.kode_akun) from (select a.kode_akun "+
						"from agg_outlook a "+
						"inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
						"inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
						"inner join bidang d on c.kode_bidang=d.kode_bidang "+this.filter+
						"group by a.kode_akun,b.nama,c.kode_bidang,d.nama)a ";
				
				this.title = new server_util_arrayList({items:["Kode Akun","Nama Akun","Bidang","Nama Bidang","RKA","Realisasi","Outlook","Persen"]});
				this.widthTable = new server_util_arrayList({items:[70,350,50,200,100,100,100,100,50]});
			...
			systemAPI.alert("[app_saku2_report_anggaran_flAggOutBidang]::mainButtonClick:"+e);
		}
	},
