window.app_saku2_report_anggaran_flAggPs = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_anggaran_flAggPs.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_anggaran_flAggPs";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Kode PS","All",""]);
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Periodik Laporan","=","Bulanan"]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	
};
window.app_saku2_report_anggaran_flAggPs.extend(window.childForm);
window.app_saku2_report_anggaran_flAggPs.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 2)
		{
			this.filterRep.ListDataSGFilter(this, "Daftar PS",this.sg1, this.sg1.row, this.sg1.col,
													  "select kode_ps, nama from agg_ps where kode_lokasi = '"+this.app._lokasi+"' ",
													  "select count(kode_ps) from agg_ps where kode_lokasi = '"+this.app._lokasi+"'  ",
													  ["kode_ps","nama"],"and",["Kode","Nama"]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
							  ...
							  this.filterRep.filterStr("a.kode_ps",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				...
				{
					var sql =  "select a.kode_ps,f.nama as nama_akun,zz.kode_lokasi,zz.nama as nama_lokasi, "+
							...
							"from (select a.kode_lokasi,a.tahun,a.kode_ps,a.progress "+	
							"	  from agg_d a "+this.filter+
							"group by a.kode_lokasi,a.tahun,a.kode_ps,a.progress ) a  "+
							"inner join agg_ps f on a.kode_ps=f.kode_ps  "+
							"inner join lokasi zz on a.kode_lokasi=zz.kode_lokasi "+
							"left join ( "+
							"	 select a.kode_lokasi,a.kode_ps, "+
							...
							"	 from agg_d a "+this.filter+
							"	 group by a.kode_lokasi,a.kode_ps "+
							"		 )g on a.kode_lokasi=g.kode_lokasi and a.kode_ps=g.kode_ps "+this.filter+
							"group by a.kode_ps,f.nama,zz.kode_lokasi,zz.nama order by a.kode_ps";	
			...
			systemAPI.alert("[app_saku2_report_anggaran_flAggPs]::mainButtonClick:"+e);
		}
	},
