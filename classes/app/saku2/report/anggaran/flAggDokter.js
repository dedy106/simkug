window.app_saku2_report_anggaran_flAggDokter = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_anggaran_flAggDokter.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_anggaran_flAggDokter";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from agg_dokter ");
	...
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Kode Dokter","All",""]);
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	
};
window.app_saku2_report_anggaran_flAggDokter.extend(window.childForm);
window.app_saku2_report_anggaran_flAggDokter.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row ==2)
		{
			this.filterRep.ListDataSGFilter(this, "Data Dokter",this.sg1, this.sg1.row, this.sg1.col,
											"select kode_dokter, nama from agg_dokter ",
											"select count(*) from agg_dokter ",
											new Array("kode_dokter","nama"),"where",new Array("kode","nama"));
		}
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct(tahun) as tahun from agg_dokter "+
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_dokter",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();	
			    var sql = "select a.kode_dokter,a.kode_lokasi, a.nama, a.keterangan, a.kode_band, a.kode_pp,b.nama as nama_pp, "+
							"	   a.volume, a.status "+
							"from agg_dokter a "+
							"inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter+
							"order by a.kode_lokasi,a.kode_dokter ";
				this.scriptSqlCount = "select count(a.kode_dokter) "+
							"from agg_dokter a "+
							"inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter;
				
				this.title = new server_util_arrayList({items:["Kode Dokter","Lokasi","Nama Dokter","Keterangan","Band","Kode PP","Nama PP","Volume","Status"]});
				this.widthTable = new server_util_arrayList({items:[60,30,200,200,40,50,150,50,50]});
			...
			systemAPI.alert("[app_saku2_report_anggaran_flAggDokter]::mainButtonClick:"+e);
		}
	},
