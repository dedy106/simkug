window.app_saku2_report_anggaran_flAggGajiFix = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_anggaran_flAggGajiFix.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_anggaran_flAggGajiFix";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Gaji Karyawan", 2);
	...
	{
		this.tahun=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from agg_gaji_m ");
	}
	else
	{
...
window.app_saku2_report_anggaran_flAggGajiFix.extend(window.portalui_childForm);
window.app_saku2_report_anggaran_flAggGajiFix.implement({
	...
	doSelectCell: function(sender, col, row){		
		...
		if (row == 3)
		{
			...
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("5. Organik TPKK","6. Pengurus","7. TKBW Paramedis","8. Organik","9. TKBW Administrasi"));
		}
	},
		...
		try
		{
			if (sender === this.app._mainForm.bClear2){
				...
				if (this.sg1.getCell(2,0)=="99")
				{
					...
					sql += ") as total";
					sql += "	from (select a.nik,a.nama,a.kode_band,a.kode_pp,a.kode_lokasi,a.tahun,a.status_org,a.kode_jab,a.kode_kota "+
							"		from agg_karyawan a "+
							"		where a.kode_lokasi='99' "+
							"		union "+
							"		select a.nik,a.nama,a.kode_band,b.kode_pp,b.kode_lokasi,a.tahun,a.status_org,a.kode_jab,a.kode_kota "+
							"		from agg_karyawan a "+
							"		inner join (select distinct a.nik,a.kode_lokasi,a.kode_pp"+
							"				from agg_gaji_d a "+
							"				inner join agg_karyawan b on a.nik=b.nik "+
							"				where a.kode_lokasi='99' and b.status_org in ('6','8') "+
							"					  and a.nik not in (select distinct a.nik  "+
							"				  from agg_karyawan a  "+
							"				  inner join agg_karyawan b on a.nik=b.nik "+ 
							"				  where a.kode_lokasi='99' and b.status_org in ('6','8')) "+
							"				)b on a.nik=b.nik) a "+
									...
									"inner join agg_kota f on a.kode_kota=f.kode_kota "+
									  " left outer join (select a.nik, a.kode_lokasi,a.periode ";
					for (var c in this.colTitle) sql += ", case when kode_param = '"+this.colTitle[c]+"' then nilai else 0 end as "+this.colTitle[c];
					sql +=	"		   from agg_gaji_d a "+
									  "		   inner join agg_gaji_m c on a.no_gaji=c.no_gaji and a.kode_lokasi=c.kode_lokasi "+this.filter+
									  "			  ) b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi "+this.filter2+
			...
			systemAPI.alert("[app_saku2_report_anggaran_flAggGajiFixs]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN GAJI KARYAWAN<br />";
		var d = new Date();
	...
	initColumn: function(){
		try{		
            //var data = this.dbLib.getDataProvider("select kode_param, kode_param+'-'+nama as nama, jns_periode  from agg_param where tahun = '"+this.sg1.cells(2,1)+"' and jenis = 'PDPT' order by no_urut",true);
			if (this.sg1.cells(2,3) == "") 
				var data = this.dbLib.getDataProvider("select kode_param, kode_param+'-'+nama as nama, jns_periode  from agg_param where tahun = '"+this.sg1.cells(2,1)+"' and jenis = 'PDPT' order by no_urut",true);
			else {
				if (this.sg1.cells(2,3).substr(0,1) == "5") 
					var data = this.dbLib.getDataProvider("select kode_param, kode_param+'-'+nama+'-'+kode_akun5 as nama, jns_periode  from agg_param where tahun = '"+this.sg1.cells(2,1)+"' and jenis = 'PDPT' order by no_urut",true);
				if (this.sg1.cells(2,3).substr(0,1) == "6") 
					var data = this.dbLib.getDataProvider("select kode_param, kode_param+'-'+nama+'-'+kode_akun4 as nama, jns_periode  from agg_param where tahun = '"+this.sg1.cells(2,1)+"' and jenis = 'PDPT' order by no_urut",true);
				if (this.sg1.cells(2,3).substr(0,1) == "7") 
					var data = this.dbLib.getDataProvider("select kode_param, kode_param+'-'+nama+'-'+kode_akun3 as nama, jns_periode  from agg_param where tahun = '"+this.sg1.cells(2,1)+"' and jenis = 'PDPT' order by no_urut",true);
				if (this.sg1.cells(2,3).substr(0,1) == "8") 
					var data = this.dbLib.getDataProvider("select kode_param, kode_param+'-'+nama+'-'+kode_akun as nama, jns_periode  from agg_param where tahun = '"+this.sg1.cells(2,1)+"' and jenis = 'PDPT' order by no_urut",true);
				if (this.sg1.cells(2,3).substr(0,1) == "9") 
					var data = this.dbLib.getDataProvider("select kode_param, kode_param+'-'+nama+'-'+kode_akun2 as nama, jns_periode  from agg_param where tahun = '"+this.sg1.cells(2,1)+"' and jenis = 'PDPT' order by no_urut",true);
			}			
