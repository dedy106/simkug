window.app_saku2_report_anggaran_flAggUhar = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_anggaran_flAggUhar.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_anggaran_flAggUhar";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from agg_pd_uhar ");
	this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Tahun","=",this.periode]);
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	
};
window.app_saku2_report_anggaran_flAggUhar.extend(window.childForm);
window.app_saku2_report_anggaran_flAggUhar.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct(tahun) as tahun from agg_pd_uhar "+
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				var sql = "select a.kode_band,a.tahun,a.nilai "+
						"from agg_pd_uhar a "+this.filter+
						"order by a.kode_band ";
				this.scriptSqlCount = "select count(a.kode_band) "+
						"from agg_pd_uhar a "+this.filter;
				this.title = new server_util_arrayList({items:["Kode Band","Tahun","Nilai"]});
				this.widthTable = new server_util_arrayList({items:[50,50,90]});
			...
			systemAPI.alert("[app_saku2_report_anggaran_flAggUhar]::mainButtonClick:"+e);
		}
	},
