window.app_saku2_report_anggaran_flAggOutPp = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_anggaran_flAggOutPp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_anggaran_flAggOutPp";
		this.maximize();
...
window.app_saku2_report_anggaran_flAggOutPp.extend(window.childForm);
window.app_saku2_report_anggaran_flAggOutPp.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var sql =  "select a.kode_akun,b.nama,c.kode_pp,c.nama,sum(a.rka) as rka,sum(a.realisasi) as realisasi,sum(a.outlook) as outlook, "+
						...
						"group by a.kode_akun,b.nama,c.kode_pp,c.nama "+
						"order by c.kode_pp,c.nama,a.kode_akun,b.nama  ";	
				this.scriptSqlCount =  "select count(a.kode_akun) from (select a.kode_akun	"+
						"from agg_outlook a "+
						"inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
						"inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+this.filter+
						"group by a.kode_akun,b.nama,c.kode_pp,c.nama)a ";
				
				this.title = new server_util_arrayList({items:["Kode Akun","Nama Akun","Kode PP","Nama PP","RKA","Realisasi","Outlook","Persen"]});
			...
			systemAPI.alert("[app_saku2_report_anggaran_flAggOutPp]::mainButtonClick:"+e);
		}
	},
