window.app_saku2_report_anggaran_flAggTpkuRekap = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_anggaran_flAggTpkuRekap.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_anggaran_flAggTpkuRekap";
		this.maximize();
	...
	this.tahun=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from agg_tpku_m where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Periodik Laporan","=","Bulanan"));
	this.doSelectCell(this.sg1,2,2);
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	
};
window.app_saku2_report_anggaran_flAggTpkuRekap.extend(window.childForm);
window.app_saku2_report_anggaran_flAggTpkuRekap.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct tahun from agg_tpku_m where kode_lokasi='"+this.app._lokasi+"' order by tahun",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,2)=="Bulanan")
				{
					...
					var sql =  "select a.no_kontrak,a.dokter "+ 
						...
						"		 )g on a.kode_lokasi=g.kode_lokasi and a.no_kontrak=g.no_kontrak and a.dokter=g.dokter "+this.filter+
						"order by a.no_kontrak,a.dokter";	
				
					this.scriptSqlCount = "select count(a.no_kontrak) "+
										"from (select distinct a.no_kontrak "+	
										"	  from agg_tpku_d a "+this.filter+") a  ";
					this.title = new server_util_arrayList({items:["No Kontrak","Dokter","Januari","Februari","Maret","April","Mei","Juni","Juli","Agustus","September","Oktober","November","Desember","Total"]});
					this.widthTable = new server_util_arrayList({items:[50,200,80,80,80,80,80,80,80,80,80,80,80,80,90]});
			...
			systemAPI.alert("[app_saku2_report_anggaran_flAggTpkuRekap]::mainButtonClick:"+e);
		}
	},
