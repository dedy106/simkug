window.app_saku2_report_anggaran_flAggLr = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_anggaran_flAggLr.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_anggaran_flAggLr";
		this.maximize();
	...
	this.est=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from agg_gldt  ");
	this.rka=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from agg_gldt  ");
...
window.app_saku2_report_anggaran_flAggLr.extend(window.childForm);
window.app_saku2_report_anggaran_flAggLr.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenTb(this.kode_neraca,this.kode_lokasi,periode);
				}
				if (this.link=="3")
				{
					this.doOpenBb(this.kode_akun,this.kode_lokasi,periode);
				}
			...
			else
			{
				...
				if (this.sg1.getCell(2,6)=="Tahunan")
				{
					...
					{
						this.nama_report="server_report_saku2_anggaran_rptAggLr";
					}
					else
					{
						this.nama_report="server_report_saku2_anggaran_rptAggLrDc";
					}
				...
				{
					this.nama_report="server_report_saku2_anggaran_rptAggLrTw";
			...
			systemAPI.alert("[app_saku2_report_anggaran_flAggLr]::mainButtonClick:"+e);
		}
	},
	...
	doOpenTb: function(kode_neraca,kode_lokasi,periode){
		...
		filter="";
		var sql = "call sp_agg_glma_trail_tmp ('FS1','"+kode_neraca+"','"+kode_lokasi+"','"+kode_lokasi+"','"+kode_lokasi+"','"+periode+"','"+this.app._nikUser+"')";
		this.dbLib.execQuerySync(sql);	
		nama_report="server_report_saku2_anggaran_rptTBLajurTrail";
	...
	doOpenBb: function(kode_akun,kode_lokasi,periode){
		...
		filter="";
		nama_report="server_report_saku2_anggaran_rptBukuBesarTrail";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenJurnal: function(no_bukti,kode_lokasi,periode){
		...
		nama_report="server_report_saku2_anggaran_rptGlJurnal";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
