window.app_saku2_report_anggaran_flAggGajiPmbn = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_anggaran_flAggGajiPmbn.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_anggaran_flAggGajiPmbn";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Biaya Penyelenggaraan YAKES", 2);
	...
	this.tanda="=";	
	this.tahun=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from agg_pembina_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_anggaran_flAggGajiPmbn.extend(window.portalui_childForm);
window.app_saku2_report_anggaran_flAggGajiPmbn.implement({
	...
	doSelectCell: function(sender, col, row){		
		...
		if (row == 1)
		{
			this.dbLib.setItemsFromSQL("select distinct tahun from agg_pembina_m a "+
		...
		try
		{
			if (sender === this.app._mainForm.bClear2){
				...
				this.scriptSqlCount = "select count(a.nik) "+
								"	from agg_karyawan a "+this.filter2;
				var sql = "select a.nik,a.nama,c.nama as nama_jab,a.kode_band  ";
				...
				sql += ") as total";
				sql += "	from agg_pembina a "+
								"inner join agg_jab c on a.jabatan=c.kode_jab "+
				...
				sql +=	"		   from agg_pembina_d a "+
								  "		   inner join agg_pembina_m c on a.no_pembina=c.no_pembina and a.kode_lokasi=c.kode_lokasi "+this.filter+
								  "			  ) b on a.nik=b.nik "+this.filter2+
								  "	group by a.nik, a.nama,c.nama,a.kode_band order by a.nik";									
				
				var s = [];
				title.add("NIK");width.add(50);fieldType.add("S"); s[s.length] = "N";
				title.add("Nama");width.add(200);fieldType.add("S");s[s.length] = "N";
				title.add("Jabatan");width.add(200);fieldType.add("S");s[s.length] = "N";
			...
			systemAPI.alert("[app_saku2_report_anggaran_flAggGajiPmbns]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN BIAYA PENYELENGGARAAN YAKES<br />";
		var d = new Date();
	...
	initColumn: function(){
		try{		
            var data = this.dbLib.getDataProvider("select kode_param, kode_param+'-'+nama as nama, jns_periode  from agg_param where tahun = '"+this.sg1.cells(2,1)+"' and jenis = 'PMBN' order by no_urut",true);
