window.app_saku2_report_anggaran_flAggPdRekap = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_anggaran_flAggPdRekap.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_anggaran_flAggPdRekap";
		this.maximize();
	...
	this.tahun=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from agg_pd_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_anggaran_flAggPdRekap.extend(window.childForm);
window.app_saku2_report_anggaran_flAggPdRekap.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct tahun from agg_pd_m where kode_lokasi='"+this.app._lokasi+"' order by tahun",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,4)=="Bulanan")
				{
					...
					var sql =  "select a.nik,b.nama,b.kode_pp,c.nama as nama_pp "+ 
						...
						"from (select distinct a.kode_lokasi,a.tahun,a.nik "+	
						"	  from agg_pd_d a "+this.filter+") a  "+
						...
						"	 from agg_pd_d a "+this.filter+
						"	 group by a.kode_lokasi,a.nik "+
						"		 )g on a.kode_lokasi=g.kode_lokasi and a.nik=g.nik  "+this.filter+
						"order by b.kode_pp,a.nik";	
					
					this.scriptSqlCount = "select count(a.nik) "+
										"from (select distinct a.kode_lokasi,a.kode_pp,a.tahun,a.nik "+	
										"	  from agg_pd_d a "+this.filter+") a  ";
			...
			systemAPI.alert("[app_saku2_report_anggaran_flAggPdRekap]::mainButtonClick:"+e);
		}
	},
