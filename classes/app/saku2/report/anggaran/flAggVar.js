window.app_saku2_report_anggaran_flAggVar = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_anggaran_flAggVar.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_anggaran_flAggVar";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from agg_var ");
...
window.app_saku2_report_anggaran_flAggVar.extend(window.childForm);
window.app_saku2_report_anggaran_flAggVar.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct(tahun) as tahun from agg_var "+
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				var result  = new arrayMap();	
					...
					var sql = "select a.kode_var,a.nama "+
							"from agg_var a "+this.filter+
							"order by a.kode_var ";
					this.scriptSqlCount = "select count(a.kode_var) "+
							"from agg_var a "+this.filter;
					this.title = new server_util_arrayList({items:["Kode Var","Nama Var"]});
					this.widthTable = new server_util_arrayList({items:[60,400]});
			...
			systemAPI.alert("[app_saku2_report_anggaran_flAggVar]::mainButtonClick:"+e);
		}
	},
