window.app_saku2_report_anggaran_flAggAbau = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_anggaran_flAggAbau.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_anggaran_flAggAbau";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from agg_abau_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_anggaran_flAggAbau.extend(window.childForm);
window.app_saku2_report_anggaran_flAggAbau.implement({
		...
		try{			
			...
			if (row == 3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_abau, tahun from agg_abau_m "+filter,
														  "select count(no_abau) from agg_abau_m "+filter,
														  ["no_abau","tahun"],"and",["No Bukti","Tahun"]);
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct tahun from agg_abau_m where kode_lokasi='"+this.app._lokasi+"' order by tahun",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
							  ...
							  this.filterRep.filterStr("a.no_abau",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");

				this.filter2 = this.sg1.getCell(2,0)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_anggaran_rptAggAbau";
			...
			systemAPI.alert("[app_saku2_report_anggaran_flAggAbau]::mainButtonClick:"+e);
		}
	},
