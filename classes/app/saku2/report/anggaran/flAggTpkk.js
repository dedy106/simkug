window.app_saku2_report_anggaran_flAggTpkk = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_anggaran_flAggTpkk.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_anggaran_flAggTpkk";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from agg_tpkk_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_anggaran_flAggTpkk.extend(window.childForm);
window.app_saku2_report_anggaran_flAggTpkk.implement({
		...
		try{			
			...
			if (row == 4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_tpkk, tahun from agg_tpkk_m "+filter,
														  "select count(kode_pp) from agg_tpkk_m"+filter,
														  ["no_tpkk","tahun"],"and",["No Bukti","Tahun"]);
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct tahun from agg_tpkk_m where kode_lokasi='"+this.app._lokasi+"' order by tahun",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku2_anggaran_rptAggTpkk";
			...
			systemAPI.alert("[app_saku2_report_anggaran_flAggTpkk]::mainButtonClick:"+e);
		}
	},
