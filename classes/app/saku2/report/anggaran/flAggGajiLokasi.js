window.app_saku2_report_anggaran_flAggGajiLokasi = function(owner)
{
	
	try{
	if (owner)
	{
		window.app_saku2_report_anggaran_flAggGajiLokasi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_anggaran_flAggGajiLokasi";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Rekapitulasi Gaji karyawan Per Lokasi", 2);
...
window.app_saku2_report_anggaran_flAggGajiLokasi.extend(window.portalui_childForm);
window.app_saku2_report_anggaran_flAggGajiLokasi.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				var sql="select e.kode_param,e.nama, "+
						...
						"	   sum(case when a.kode_lokasi between '99' and '99' then a.nilai else 0 end ) as g99, "+
						"	   sum(case when a.kode_lokasi between '01' and '01' then a.nilai else 0 end ) as g01, "+
						"	   sum(case when a.kode_lokasi between '02' and '02' then a.nilai else 0 end ) as g02, "+
						"	   sum(case when a.kode_lokasi between '03' and '03' then a.nilai else 0 end ) as g03, "+
						"	   sum(case when a.kode_lokasi between '04' and '04' then a.nilai else 0 end ) as g04, "+
						"	   sum(case when a.kode_lokasi between '05' and '05' then a.nilai else 0 end ) as g05, "+
						"	   sum(case when a.kode_lokasi between '06' and '06' then a.nilai else 0 end ) as g06, "+
						"	   sum(case when a.kode_lokasi between '07' and '07' then a.nilai else 0 end ) as g07, "+
						"	   sum(case when a.kode_lokasi between '01' and '99' then a.nilai else 0 end ) as total "+ 
						"	from agg_gaji_d a "+
						"	inner join agg_gaji_m b on a.no_gaji=b.no_gaji "+this.filter2+
						"	group by a.kode_param)f on e.kode_param=f.kode_param "+this.filter+" and e.jenis='PDPT' "+
						" order by e.kode_param";
				
				this.scriptSqlCount = "select count(*) "+
									"from agg_param e "+
									"left join (select akode_param "+ 
									"	from agg_gaji_d a "+
									"	inner join agg_gaji_m b on a.no_gaji=b.no_gaji and a.kode_lokasi=b.kode_lokasi "+this.filter2+
									"	group by a.kode_param)f on e.nik=f.nik "+this.filter;
						
				this.title = new server_util_arrayList({items:["Kode","Nama","Pusat","Area 1","Area 2","Area 3","Area 4","Area 5","Area 6","Area 7","Total"]});
				this.widthTable = new server_util_arrayList({items:[60,200,80,80,80,80,80,80,80,80,80,80,80,80,100]});
			...
			systemAPI.alert("[app_saku2_report_anggaran_flAggGajiLokasi]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var header = "LAPORAN REKAPITULASI GAJI KARYAWAN PER LOKASI";
