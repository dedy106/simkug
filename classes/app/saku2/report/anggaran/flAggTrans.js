window.app_saku2_report_anggaran_flAggTrans = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_anggaran_flAggTrans.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_anggaran_flAggTrans";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from agg_pd_trans ");
	this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Tahun","=",this.periode]);
	this.gridLib.SGEditData(this.sg1,1,[0,1,2],["Kode Rute","All",""]);
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	
};
window.app_saku2_report_anggaran_flAggTrans.extend(window.childForm);
window.app_saku2_report_anggaran_flAggTrans.implement({
	doEllipseClick: function(sender, col, row){
		if (row ==1)
		{
			this.filterRep.ListDataSGFilter(this, "Data Rute",this.sg1, this.sg1.row, this.sg1.col,
											"select kode_rute, nama from agg_pd_rute ",
											"select count(*) from agg_pd_rute ",
											new Array("kode_rute","nama"),"where",new Array("kode","nama"));
		}
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct(tahun) as tahun from agg_pd_trans "+
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.tahun",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.kode_rute",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();	
				var sql = "select a.kode_rute,a.nama,a.nilai "+
							"from agg_pd_trans a "+this.filter+
							"order by a.kode_rute ";
				this.scriptSqlCount = "select count(a.kode_rute) "+
							"from agg_pd_trans a "+this.filter;
				this.title = new server_util_arrayList({items:["Kode Rute","Nama Rute","Nilai"]});
				this.widthTable = new server_util_arrayList({items:[60,300,90]});
			...
			systemAPI.alert("[app_saku2_report_anggaran_flAggTrans]::mainButtonClick:"+e);
		}
	},
