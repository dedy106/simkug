window.app_saku2_report_anggaran_flAggNormaVar = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_anggaran_flAggNormaVar.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_anggaran_flAggNormaVar";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from agg_norma_var ");
	this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Tahun","=",this.periode]);
	this.gridLib.SGEditData(this.sg1,1,[0,1,2],["Kode Var","All",""]);
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Kode Akun","All",""]);
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Jenis Laporan","=","Norma"]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	
};
window.app_saku2_report_anggaran_flAggNormaVar.extend(window.childForm);
window.app_saku2_report_anggaran_flAggNormaVar.implement({
	doEllipseClick: function(sender, col, row){
		if (row ==1)
		{
			this.filterRep.ListDataSGFilter(this, "Data Variabel",this.sg1, this.sg1.row, this.sg1.col,
											"select kode_var, nama from agg_var ",
											"select count(*) from agg_ps ",
											new Array("kode_var","nama"),"where",new Array("kode","nama"));
		}
		if (row ==2)
		{
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct(tahun) as tahun from agg_norma_var "+
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,3)=="Norma")
				{
					var sql = "select a.kode_var,b.nama as nama_var,a.kode_akun,c.nama as nama_akun,a.tahun,a.tarif "+
							"from agg_norma_var a "+
							"inner join agg_var b on a.kode_var=b.kode_var and a.tahun=b.tahun "+
							"inner join masakun c on a.kode_akun=c.kode_akun and c.kode_lokasi='99'  "+this.filter+
							"order by a.kode_var,a.kode_akun ";
					this.scriptSqlCount = "select count(a.kode_var) "+
							"from agg_norma_var a "+
							"inner join agg_var b on a.kode_var=b.kode_var "+
							"inner join masakun c on a.kode_akun=c.kode_akun and c.kode_lokasi='99'  "+this.filter;
					this.title = new server_util_arrayList({items:["Kode Var","Nama Var","Kode Akun","Nama Akun","Tahun","Tarif"]});
					this.widthTable = new server_util_arrayList({items:[50,200,70,300,50,100]});
			...
			systemAPI.alert("[app_saku2_report_anggaran_flAggNormaVar]::mainButtonClick:"+e);
		}
	},
