window.app_saku2_report_kb_flKbDropingSaldoOut = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kb_flKbDropingSaldoOut.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kb_flKbDropingSaldoOut";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_kasdrop_d where  progress='0' ");
...
window.app_saku2_report_kb_flKbDropingSaldoOut.extend(window.childForm);
window.app_saku2_report_kb_flKbDropingSaldoOut.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from yk_kasdrop_d where progress='0'  order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Droping Kirim","Droping Terima","Nasional"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_loktuj",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							" and a.progress='0' ";
				...
				var result  = new arrayMap();		
					...
					{
						var sql = "select a.kode_lokasi,a.no_kas,a.no_dokumen,date_format(c.tanggal,'%d/%m/%Y') as tanggal,a.akun_tak,b.nama as nama_akun,a.keterangan,a.nilai "+
								"from yk_kasdrop_d a "+
						...
						this.title = new server_util_arrayList({items:["Lokasi Asal","No Kas","No Dokumen","Tanggal","Akun","Nama Akun","Keterangan","Nilai Kirim"]});
						this.widthTable = new server_util_arrayList({items:[50,80,120,60,60,200,200,90]});
			...
			systemAPI.alert("[app_saku2_report_kb_flKbDropingSaldoOut]::mainButtonClick:"+e);
		}
	},
