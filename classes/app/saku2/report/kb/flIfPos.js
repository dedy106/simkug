window.app_saku2_report_kb_flIfPos = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kb_flIfPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kb_flIfPos";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_ifptg_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_kb_flIfPos.extend(window.childForm);
window.app_saku2_report_kb_flIfPos.implement({
		...
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan Pembayaran",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_ifptg, a.keterangan from yk_ifptg_m a "+
												"inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter,
												"select count(*) from yk_ifptg_m a "+
												"inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter,
												new Array("a.no_ifptg","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from yk_ifptg_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku2_kb_rptIfPos";
			...
			systemAPI.alert("[app_saku2_report_kb_flIfPos]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter2 = "";
		filter=" where a.no_ifptg='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"'";
	...
	doOpenKas: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku2_kb_rptIfJurnal";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
