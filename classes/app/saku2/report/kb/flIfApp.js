window.app_saku2_report_kb_flIfApp = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kb_flIfApp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kb_flIfApp";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_ifapp_m where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("No Approval IF","All",""));
...
window.app_saku2_report_kb_flIfApp.extend(window.childForm);
window.app_saku2_report_kb_flIfApp.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Approval",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_app, a.keterangan from yk_ifapp_m a "+this.filter,
												"select count(*) from yk_ifapp_m a "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from yk_ifapp_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku2_kb_rptIfAppJurnal";
			...
			systemAPI.alert("[app_saku2_report_kb_flIfApp]::mainButtonClick:"+e);
		}
	},
