window.app_saku2_report_kb_flKbDropingAjuApp = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kb_flKbDropingAjuApp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kb_flKbDropingAjuApp";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_ver_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBAJU' ");
...
window.app_saku2_report_kb_flKbDropingAjuApp.extend(window.childForm);
window.app_saku2_report_kb_flKbDropingAjuApp.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from yk_ver_m where kode_lokasi='"+this.app._lokasi+"'  nd modul='KBAJU' order by periode desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Droping Kirim"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku2_kb_rptKbDropingAjuApp";
			...
			systemAPI.alert("[app_saku2_report_kb_flKbDropingAjuApp]::mainButtonClick:"+e);
		}
	},
