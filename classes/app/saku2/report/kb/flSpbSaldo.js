window.app_saku2_report_kb_flSpbSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kb_flSpbSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kb_flSpbSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_spb_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_kb_flSpbSaldo.extend(window.childForm);
window.app_saku2_report_kb_flSpbSaldo.implement({
		...
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan Pembayaran",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_spb, a.keterangan from yk_spb_m a "+
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from yk_spb_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku2_kb_rptSpbSaldo";
			...
			systemAPI.alert("[app_saku2_report_kb_flSpbSaldo]::mainButtonClick:"+e);
		}
	},
