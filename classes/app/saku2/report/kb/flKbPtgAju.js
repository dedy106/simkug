window.app_saku2_report_kb_flKbPtgAju = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kb_flKbPtgAju.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kb_flKbPtgAju";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_pjaju_m ");
...
window.app_saku2_report_kb_flKbPtgAju.extend(window.childForm);
window.app_saku2_report_kb_flKbPtgAju.implement({
		...
		try{			
			...
			if (row ==4)
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.modul='PJPTGAJU'";
				this.filterRep.ListDataSGFilter(this, "Data Pertanggungan",this.sg1, this.sg1.row, this.sg1.col,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from ptg_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku2_kb_rptKbPtgAju";
			...
			systemAPI.alert("[app_saku2_report_kb_flKbPtgAju]::mainButtonClick:"+e);
		}
	},
