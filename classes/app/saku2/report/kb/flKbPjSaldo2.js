window.app_saku2_report_kb_flKbPjSaldo2 = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kb_flKbPjSaldo2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kb_flKbPjSaldo2";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(a.periode) as periode "+
											"from (select max(periode) as periode from panjar_m where kode_lokasi='"+this.app._lokasi+"' "+
											"union "+
											"select max(periode) as periode from ptg_m where kode_lokasi='"+this.app._lokasi+"')a ");
...
window.app_saku2_report_kb_flKbPjSaldo2.extend(window.childForm);
window.app_saku2_report_kb_flKbPjSaldo2.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			var sql="select a.periode "+
					"from (select distinct periode as periode from panjar_m where kode_lokasi='"+this.app._lokasi+"' "+
					"union "+
					"select distinct periode as periode from ptg_m where kode_lokasi='"+this.app._lokasi+"')a "+
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_saku2_kb_rptKbPjSaldo";
