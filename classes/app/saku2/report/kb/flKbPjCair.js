window.app_saku2_report_kb_flKbPjCair = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kb_flKbPjCair.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kb_flKbPjCair";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Pencairan Panjar", 2);
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from panjar_m ");
...
window.app_saku2_report_kb_flKbPjCair.extend(window.childForm);
window.app_saku2_report_kb_flKbPjCair.implement({
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter2 = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_dokumen",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				if (this.sg1.getCell(2,5)=="Transaksi")
				{
					var sql = "select a.no_pj, a.no_kas, a.no_dokumen, date_format(a.tanggal,'%d/%m/%Y') as tanggal, date_format(a.due_date,'%d/%m/%Y') as due_date, a.keterangan, a.catatan, "+ 
					...
					this.title = new server_util_arrayList({items:["No Panjar","No Kas","No Dokumen","Tanggal","Due Date","Keterangan","Catatan","Akun Panjar","Nama Akun","Nik Pengaju","Nama Pengaju","Nik Setuju","Nama Setuju","Kode PP","Nama PP","Kode DRK","Nilai","Potongan"]});
					this.widthTable = new server_util_arrayList({items:[80,80,80,60,60,200,150,80,150,60,150,60,150,60,150,60,80,80]});
					this.fieldType = new server_util_arrayList({items:["S","S","S","S","S","S","S","S","S","S","S","S","S","S","S","S","N","N"]});																
					this.summary = new server_util_arrayList({items:["N","N","N","N","N","N","N","N","N","N","N","N","N","N","N","N","Y","Y"]});
				}
				else
				{
					var modul=" and a.modul='KBPJ' ";
					var sql = "select a.no_kas,a.no_dokumen,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.kode_akun,b.nama,a.kode_pp,a.kode_drk,a.kode_cf,a.modul,a.keterangan, "+
					...
					this.title = new server_util_arrayList({items:["No Kas","No Dokumen","Tanggal","Kode Akun","Nama Akun","Kode PP","Kode DRK","Kode CF","Modul","Keterangan","Debet","Kredit"]});
					this.widthTable = new server_util_arrayList({items:[80,120,50,80,200,60,60,60,60,250,90,90]});
					this.fieldType = new server_util_arrayList({items:["S","S","S","S","S","S","S","S","S","S","N","N"]});																
					this.summary = new server_util_arrayList({items:["N","N","N","N","N","N","N","N","N","N","Y","Y"]});
				}
			...
			systemAPI.alert("[app_saku2_report_kb_flKbPjCair]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN PENCAIRAN PANJAR<br>Posisi : "+this.sg1.getCell(2,1)+"<br />";
		var d = new Date();
