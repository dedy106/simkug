window.app_saku2_report_kb_flPjCair = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kb_flPjCair.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kb_flPjCair";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where modul='KBPJAJU' ");
...
window.app_saku2_report_kb_flPjCair.extend(window.childForm);
window.app_saku2_report_kb_flPjCair.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where modul='KBPJAJU' and kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku2_kb_rptPjCair";
			...
			systemAPI.alert("[app_saku2_report_kb_flPjCair]::mainButtonClick:"+e);
		}
	},
