window.app_saku2_report_kb_flKbPjPtgStisi = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kb_flKbPjPtgStisi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kb_flKbPjPtgStisi";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Bentuk Laporan","=","Jurnal"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	this.doSelectCell(this.sg1,2,5);
};
window.app_saku2_report_kb_flKbPjPtgStisi.extend(window.childForm);
window.app_saku2_report_kb_flKbPjPtgStisi.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Ptg",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_ptg, a.keterangan from ptg_m a "+this.filter,
												"select count(*) from ptg_m a "+this.filter,
												new Array("a.no_ptg","a.keterangan"),"and",new Array("No Ptg","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from ptg_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 5)
		{
			this.sg1.columns.get(1).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(1).pickList,new Array("Jurnal"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode PP","All",""));
				this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Nik Pengaju","All",""));
				this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("No Ptg","All",""));
				this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Bentuk Laporan","=","Jurnal"));
			}else{
				...
				if (this.sg1.getCell(2,5)=="Jurnal")
				{
			...
			systemAPI.alert("[app_saku2_report_kb_flKbPjPtgStisi]::mainButtonClick:"+e);
		}
	},
