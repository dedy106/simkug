window.app_saku2_report_kb_flKbSpb = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kb_flKbSpb.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kb_flKbSpb";
		this.maximize();
...
window.app_saku2_report_kb_flKbSpb.extend(window.childForm);
window.app_saku2_report_kb_flKbSpb.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from yk_spb_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku2_kb_rptKbSpb";
			...
			systemAPI.alert("[app_saku2_report_kb_flKbSpb]::mainButtonClick:"+e);
		}
	},
