window.app_saku2_report_kb_flKbPjPtg = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kb_flKbPjPtg.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kb_flKbPjPtg";
		this.maximize();
...
window.app_saku2_report_kb_flKbPjPtg.extend(window.childForm);
window.app_saku2_report_kb_flKbPjPtg.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				{
					var sql = "SELECT a.no_ptg, a.no_pj, a.no_kas, a.no_dokumen, date_format(a.tanggal,'%d/%m/%Y') as tanggal, a.keterangan, a.catatan, "+
					...
					this.scriptSqlCount = "SELECT count(a.no_ptg) "+
							...
							"inner join pp e on a.kode_pp=e.kode_pp and a.kode_lokasi=e.kode_lokasi "+
							"left join masakun f on a.akun_kas=f.kode_akun and a.kode_lokasi=f.kode_lokasi "+
							"inner join ptg_m g on a.no_pj=g.no_pj and a.kode_lokasi=g.kode_lokasi "+this.filter;
					this.title = new server_util_arrayList({items:["No PTG","No Panjar","No Kas","No Dokumen","Tanggal","Keterangan","Catatan","Akun Panjar","Nama Akun Panjar","Akun Kas","Nama Akun Kas","Nik Pembuat","Nama Pembuat","Nik Setuju","Nama Setuju","Kode PP","Nama PP","Kode DRK","Nilai Panjar","Nilai Ptg","Nilai Kas"]});
					this.widthTable = new server_util_arrayList({items:[80,80,80,80,60,200,150,80,150,80,150,60,150,60,150,60,150,60,80,80,80]});
					this.fieldType = new server_util_arrayList({items:["S","S","S","S","S","S","S","S","S","S","S","S","S","S","S","S","S","S","N","N","N"]});																
					this.summary = new server_util_arrayList({items:["N","N","N","N","N","N","N","N","N","N","N","N","N","N","N","N","N","N","Y","Y","Y"]});
				}
				else
				{
					var sql = "select a.no_ptg,a.no_dokumen,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.kode_akun,b.nama,a.kode_pp,a.kode_drk,a.modul,a.keterangan, "+
						...
						"order by a.periode,a.no_ptg,a.dc desc  ";
					this.scriptSqlCount = "select count(a.no_ptg) "+
							"from ptg_j a  "+
							"inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+this.filter2;
					
					this.title = new server_util_arrayList({items:["No PTG","No Dokumen","Tanggal","Kode Akun","Nama Akun","Kode PP","Kode DRK","Modul","Keterangan","Debet","Kredit"]});
			...
			systemAPI.alert("[app_saku2_report_kb_flKbPjPtg]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN PERTANGGUNGAN PANJAR<br>Posisi : "+this.sg1.getCell(2,1)+"<br />";
		var d = new Date();
