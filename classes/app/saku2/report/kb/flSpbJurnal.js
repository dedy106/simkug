window.app_saku2_report_kb_flSpbJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kb_flSpbJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kb_flSpbJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where modul='KBSPB' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_kb_flSpbJurnal.extend(window.childForm);
window.app_saku2_report_kb_flSpbJurnal.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where modul='KBSPB' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_saku2_kb_rptSpbJurnal";
