window.app_saku2_report_kb_flKbIf = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kb_flKbIf.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kb_flKbIf";
		this.maximize();
...
window.app_saku2_report_kb_flKbIf.extend(window.childForm);
window.app_saku2_report_kb_flKbIf.implement({
		...
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Imprest Fund",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_ifptg, a.keterangan from yk_ifptg_m a "+this.filter,
												"select count(*) from yk_ifptg_m a "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from yk_ifptg_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku2_report_kb_flKbIf]::mainButtonClick:"+e);
		}
	},
