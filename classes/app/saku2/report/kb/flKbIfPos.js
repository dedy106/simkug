window.app_saku2_report_kb_flKbIfPos = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kb_flKbIfPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kb_flKbIfPos";
		this.maximize();
...
window.app_saku2_report_kb_flKbIfPos.extend(window.childForm);
window.app_saku2_report_kb_flKbIfPos.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Imprest Fund",this.sg1, this.sg1.row, this.sg1.col,
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				{
					var sql = "select a.no_ifptg,a.no_dokumen,date_format(a.tanggal,'%d/%m/%Y') as tanggal,a.keterangan,a.nik_buat,b.nama,b.kode_pp,f.nama as nama_pp, "+
							...
							"left join yk_ver_d c on a.no_ifptg=c.no_bukti and a.kode_lokasi=c.kode_lokasi "+
							"left join yk_ver_m d on c.no_ver=d.no_ver and c.kode_lokasi=d.kode_lokasi "+
							"left join kas_m e on a.no_kas=e.no_kas and a.kode_lokasi=e.kode_lokasi "+this.filter+
							" order by a.no_ifptg";
					this.scriptSqlCount = "select count(a.no_ifptg) "+
							...
							"left join yk_ver_m d on c.no_ver=d.no_ver and c.kode_lokasi=d.kode_lokasi "+
							"left join kas_m e on a.no_kas=e.no_kas and a.kode_lokasi=e.kode_lokasi "+this.filter;
					this.title = new server_util_arrayList({items:["No IF","No Dokumen","Tanggal","Keterangan","NIK","Nama","PP","Nama PP","No Ver","Tgl Ver","No Kas","Tgl Kas","Nilai"]});
					this.widthTable = new server_util_arrayList({items:[80,80,60,200,60,150,60,150,80,60,80,60,90]});
					this.fieldType = new server_util_arrayList({items:["S","S","S","S","S","S","S","S","S","S","S","S","N"]});																
					this.summary = new server_util_arrayList({items:["N","N","N","N","N","N","N","N","N","N","N","N","Y"]});
				}
			...
			systemAPI.alert("[app_saku2_report_kb_flKbIfPos]::mainButtonClick:"+e);
		}
	},
