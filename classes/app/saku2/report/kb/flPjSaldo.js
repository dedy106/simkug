window.app_saku2_report_kb_flPjSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_kb_flPjSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_kb_flPjSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_pjaju_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_kb_flPjSaldo.extend(window.childForm);
window.app_saku2_report_kb_flPjSaldo.implement({
		...
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Panjar",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_pjaju, a.keterangan from yk_pjaju_m a "+this.filter,
												"select count(*) from yk_pjaju_m a "+this.filter,
												new Array("a.no_pjaju","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_pjaju",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_kb_rptPjSaldo";
			...
			systemAPI.alert("[app_saku2_report_kb_flPjSaldo]::mainButtonClick:"+e);
		}
	},
