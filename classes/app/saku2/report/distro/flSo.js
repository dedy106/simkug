window.app_saku2_report_distro_flSo = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_distro_flSo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_distro_flSo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from ds_so_m ");
...
window.app_saku2_report_distro_flSo.extend(window.childForm);
window.app_saku2_report_distro_flSo.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data DO",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_so, a.keterangan from ds_so_m a "+filter,
												"select count(b.no_so) from ds_so_m a "+filter,
												new Array("b.no_so","b.keterangan"),"and",new Array("No Invoice","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from ds_so_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku2_distro_rptSo";
			...
			systemAPI.alert("[app_saku2_report_distro_flSo]::mainButtonClick:"+e);
		}
	},
