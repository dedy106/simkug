window.app_saku2_report_distro_flHutangSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_distro_flHutangSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_distro_flHutangSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from ds_do_m ");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("No DO","All",""));
...
window.app_saku2_report_distro_flHutangSaldo.extend(window.childForm);
window.app_saku2_report_distro_flHutangSaldo.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data DO",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_do, a.keterangan from ds_do_m a "+filter,
												"select count(b.no_do) from ds_do_m a "+filter,
												new Array("b.no_do","b.keterangan"),"and",new Array("No Invoice","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from ds_do_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_do",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku2_distro_rptHutangSaldo";
			...
			systemAPI.alert("[app_saku2_report_distro_flHutangSaldo]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBayar: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku2_distro_rptHutangTrail";
	...
	doOpenJurnal: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_do='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._namaUser;
		nama_report="server_report_saku2_distro_rptJurnal";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenKas: function(no_bukti,kode_lokasi){
		...
		filter2=this.app._namaUser;
		nama_report="server_report_saku2_distro_rptBuktiBank";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
