window.app_saku2_report_distro_flBa = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_distro_flBa.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_distro_flBa";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from ds_ba_m ");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("No BAs","All",""));
...
window.app_saku2_report_distro_flBa.extend(window.childForm);
window.app_saku2_report_distro_flBa.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data BA",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_ba, a.keterangan from ds_ba_m a "+filter,
												"select count(b.no_ba) from ds_ba_m a "+filter,
												new Array("b.no_ba","b.keterangan"),"and",new Array("No Invoice","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from ds_ba_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku2_distro_rptBa";
			...
			systemAPI.alert("[app_saku2_report_distro_flBa]::mainButtonClick:"+e);
		}
	},
