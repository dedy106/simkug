window.app_saku2_report_sppd_flSppdVer = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_sppd_flSppdVer.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_sppd_flSppdVer";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_ver_m where modul='SPPD' ");
...
window.app_saku2_report_sppd_flSppdVer.extend(window.childForm);
window.app_saku2_report_sppd_flSppdVer.implement({
		...
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data SPJ",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_ver, a.keterangan from yk_ver_m a "+this.filter,
												"select count(*) from yk_ver_m a "+this.filter,
												new Array("a.no_ver","a.keterangan"),"and",new Array("No SPB","Keterangan"));
			}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				{
					this.nama_report = "server_report_saku2_sppd_rptSppdTerbit";
				}
			...
			systemAPI.alert("[app_saku2_report_sppd_flSppdVer]::mainButtonClick:"+e);
		}
	},
