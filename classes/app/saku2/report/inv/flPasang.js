window.app_saku2_report_inv_flPasang = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_inv_flPasang.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_inv_flPasang";
		this.maximize();
...
window.app_saku2_report_inv_flPasang.extend(window.childForm);
window.app_saku2_report_inv_flPasang.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 2)
			{
				...
				var sql1="select a.no_pasang,a.keterangan from inv_pasang_m a "+this.filter;
				var sql2="select count(a.no_pasang) from inv_pasang_m a "+this.filter;
				this.filterRep.ListDataSGFilter(this, "Data Approval",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,["a.no_pasang","a.keterangan"],"and",["No Anggaran","Keterangan"]);
			}	
		}catch(e){
			systemAPI.alert(e);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
		    	...
		    	this.filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
						this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.no_pasang",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.nama_report="server_report_saku2_inv_rptPasang";	
