window.app_saku2_report_inv_flTerima = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_inv_flTerima.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_inv_flTerima";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_terima_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku2_report_inv_flTerima.extend(window.childForm);
window.app_saku2_report_inv_flTerima.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 2)
			{
				...
				var sql1="select a.no_terima,a.keterangan from inv_terima_m a "+this.filter;
				var sql2="select count(a.no_terima) from inv_terima_m a "+this.filter;
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct(periode) as periode from inv_terima_m "+
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report="server_report_saku2_inv_rptTerima";	
