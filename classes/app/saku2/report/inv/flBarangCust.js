window.app_saku2_report_inv_flBarangCust = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_inv_flBarangCust.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_inv_flBarangCust";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_pasang_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku2_report_inv_flBarangCust.extend(window.childForm);
window.app_saku2_report_inv_flBarangCust.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 2)
			{
				...
				var sql1="select a.no_terima,a.keterangan from inv_pasang_m a "+this.filter;
				var sql2="select count(a.no_terima) from inv_pasang_m a "+this.filter;
				this.filterRep.ListDataSGFilter(this, "Data Approval",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,["a.no_terima","a.keterangan"],"and",["No Anggaran","Keterangan"]);
			}	
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct(periode) as periode from inv_pasang_m "+
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report="server_report_saku2_inv_rptBarangCust";	
