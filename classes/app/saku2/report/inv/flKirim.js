window.app_saku2_report_inv_flKirim = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_inv_flKirim.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_inv_flKirim";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_kirim_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku2_report_inv_flKirim.extend(window.childForm);
window.app_saku2_report_inv_flKirim.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 2)
			{
				...
				var sql1="select a.no_kirim,a.keterangan from inv_kirim_m a "+this.filter;
				var sql2="select count(a.no_kirim) from inv_kirim_m a "+this.filter;
				this.filterRep.ListDataSGFilter(this, "Data Approval",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,["a.no_kirim","a.keterangan"],"and",["No Anggaran","Keterangan"]);
			}	
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct(periode) as periode from inv_kirim_m "+
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report="server_report_saku2_inv_rptKirim";	
