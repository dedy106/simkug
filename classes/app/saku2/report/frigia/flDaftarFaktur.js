window.app_saku2_report_frigia_flDaftarFaktur = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_frigia_flDaftarFaktur.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_frigia_flDaftarFaktur";
		this.maximize();
...
window.app_saku2_report_frigia_flDaftarFaktur.extend(window.childForm);
window.app_saku2_report_frigia_flDaftarFaktur.implement({
	...
	{		
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["13","123","13","13","3"]);
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from fp_invoice where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				{
					this.nama_report = "server_report_saku2_frigia_rptDaftarFaktur";
				}
				else
				{
					this.nama_report = "server_report_saku2_frigia_rptDaftarFakturSum";
				}
			...
			systemAPI.alert("[app_saku2_report_frigia_flDaftarFaktur]::mainButtonClick:"+e);
		}
	},
