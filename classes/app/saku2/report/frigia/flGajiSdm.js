window.app_saku2_report_frigia_flGajiSdm = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_frigia_flGajiSdm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_frigia_flGajiSdm";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from fri_gaji_m where kode_lokasi='"+this.app._lokasi+"'");
	this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Lokasi","=",this.app._lokasi]);
	this.gridLib.SGEditData(this.sg1,1,[0,1,2],["Cabang","=",this.periode]);
...
window.app_saku2_report_frigia_flGajiSdm.extend(window.childForm);
window.app_saku2_report_frigia_flGajiSdm.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2],["3","123i","123i"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2],[0,2,2]);
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from fri_gaji_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku2_frigia_rptGajiSdm";
			...
			systemAPI.alert("[app_saku2_report_frigia_flGajiSdm]::mainButtonClick:"+e);
		}
	},
