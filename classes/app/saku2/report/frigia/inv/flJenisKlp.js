window.app_saku2_report_frigia_inv_flJenisKlp = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_frigia_inv_flJenisKlp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_frigia_inv_flJenisKlp";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Barang Per Kelompok Barang", 2);
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Kelompok Barang","=",""));
...
window.app_saku2_report_frigia_inv_flJenisKlp.extend(window.portalui_childForm);
window.app_saku2_report_frigia_inv_flJenisKlp.prototype.doEllipseClick= function(sender, col, row)
{
		if (row == 0){
		...
		if (row == 3)
		{
			this.filterRep.ListDataSGFilter(this, "Daftar Kelompok Barang",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_klp, nama  from fri_barang_klp where kode_lokasi='"+this.app._lokasi+"' ",
												"select count(kode_klp) from fri_barang_klp where kode_lokasi='"+this.app._lokasi+"' ",
												["kode_klp","nama"],"where",["Kode","Nama"]);
		}
};
window.app_saku2_report_frigia_inv_flJenisKlp.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_saku2_report_frigia_inv_flJenisKlp.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			if (this.sg1.getCell(2,3) == "") {
				...
				this.sql =  "select a.kode_brg,a.nama as nama_brg,a.satuan,isnull(b.sawal,0) as sawal,isnull(c.debet,0) as debet,isnull(d.kredit,0) as kredit,isnull(b.sawal,0) + isnull(c.debet,0) - isnull(d.kredit,0) as sakhir "+
							...
							"where a.kode_klp = '"+this.sg1.getCell(2,3)+"' ";						
				...
				this.scriptSqlCount = "select count(kode_brg) from fri_barang_m  where kode_klp = '"+this.sg1.getCell(2,3)+"' ";
				
				var dthtml = this.dbLib.sqlToHtml(this.sql,1,this.pager, title, width,fieldType,true,undefined,this.summary);
				this.nama=this.dbLib.getPeriodeFromSQL("select nama as periode from fri_barang_klp where kode_klp='"+this.sg1.getCell(2,3)+"'");
	...
	{
		alert("[app_saku2_report_frigia_inv_flJenisKlp]::mainButtonClick:"+e);
	}
};
window.app_saku2_report_frigia_inv_flJenisKlp.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_saku2_report_frigia_inv_flJenisKlp.prototype.getStringHeader = function(){
...
window.app_saku2_report_frigia_inv_flJenisKlp.prototype.doConfirmClick = function(sender){
	try{
...
window.app_saku2_report_frigia_inv_flJenisKlp.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_saku2_report_frigia_inv_flJenisKlp.prototype.previewReport = function(dthtml)
{
...
window.app_saku2_report_frigia_inv_flJenisKlp.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku2_report_frigia_inv_flJenisKlp.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku2_report_frigia_inv_flJenisKlp.prototype.doRowPerPageChange = function(sender, rowperpage)
{
