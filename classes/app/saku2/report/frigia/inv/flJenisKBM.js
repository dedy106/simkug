window.app_saku2_report_frigia_inv_flJenisKBM = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_frigia_inv_flJenisKBM.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_frigia_inv_flJenisKBM";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Barang Per Jenis KBM", 2);
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Jenis KBM","=",""));
...
window.app_saku2_report_frigia_inv_flJenisKBM.extend(window.portalui_childForm);
window.app_saku2_report_frigia_inv_flJenisKBM.prototype.doEllipseClick= function(sender, col, row)
{
		if (row == 0){
		...
		if (row == 3)
		{
			this.filterRep.ListDataSGFilter(this, "Daftar Jenis Kendaraan",this.sg1, this.sg1.row, this.sg1.col,
												...
												["kode_kbm","nama"],"where",["Kode","Nama"]);
		}
};
window.app_saku2_report_frigia_inv_flJenisKBM.prototype.doSelectCell = function(sender, col, row)
{
...
window.app_saku2_report_frigia_inv_flJenisKBM.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			if (this.sg1.getCell(2,3) == "") {
				system.alert(this,"Jenis KBM tidak valid.","Data Jenis KBM harus dipilih.");
				return false;
			}
			else {
				...
				this.sql =  "select a.kode_brg,a.nama as nama_brg,a.satuan,isnull(b.sawal,0) as sawal,isnull(c.debet,0) as debet,isnull(d.kredit,0) as kredit,isnull(b.sawal,0) + isnull(c.debet,0) - isnull(d.kredit,0) as sakhir "+
							...
							"where a.kode_kbm = '"+this.sg1.getCell(2,3)+"' ";						
				...
				this.scriptSqlCount = "select count(kode_brg) from fri_barang_m  where kode_kbm = '"+this.sg1.getCell(2,3)+"' ";
				
				var dthtml = this.dbLib.sqlToHtml(this.sql,1,this.pager, title, width,fieldType,true,undefined,this.summary);
				this.nama=this.dbLib.getPeriodeFromSQL("select nama as periode from fri_barang_kbm where kode_kbm='"+this.sg1.getCell(2,3)+"'");
	...
	{
		alert("[app_saku2_report_frigia_inv_flJenisKBM]::mainButtonClick:"+e);
	}
};
window.app_saku2_report_frigia_inv_flJenisKBM.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_saku2_report_frigia_inv_flJenisKBM.prototype.getStringHeader = function(){
...
window.app_saku2_report_frigia_inv_flJenisKBM.prototype.doConfirmClick = function(sender){
	try{
...
window.app_saku2_report_frigia_inv_flJenisKBM.prototype.doSelectedPage = function(sender, page)
{
	var dthtml = this.dbLib.sqlToHtml(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false);			
	this.previewReport(dthtml);			
	this.page=page;
};
window.app_saku2_report_frigia_inv_flJenisKBM.prototype.previewReport = function(dthtml)
{
	var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>Laporan Barang Per Jenis KBM<br>"+this.nama+" Periode "+this.sg1.getCell(2,1)  ;			
	var d = new Date();
...
window.app_saku2_report_frigia_inv_flJenisKBM.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku2_report_frigia_inv_flJenisKBM.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku2_report_frigia_inv_flJenisKBM.prototype.doRowPerPageChange = function(sender, rowperpage)
{
