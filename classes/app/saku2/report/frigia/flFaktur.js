window.app_saku2_report_frigia_flFaktur = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_frigia_flFaktur.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_frigia_flFaktur";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from fp_invoice where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku2_report_frigia_flFaktur.extend(window.childForm);
window.app_saku2_report_frigia_flFaktur.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from fp_invoice where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku2_report_frigia_flFaktur]::mainButtonClick:"+e);
		}
	},
