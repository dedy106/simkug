// JavaScript Document
window.app_saku2_report_frigia_flTbCust = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_frigia_flTbCust.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_frigia_flTbCust";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Akun Piutang ","=","115001"));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Kode Customer","All",""));
...
window.app_saku2_report_frigia_flTbCust.extend(window.portalui_childForm);
window.app_saku2_report_frigia_flTbCust.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 2)
		{
			this.filterRep.ListDataSGFilter(this, "Data Akun Piutang",this.sg1, this.sg1.row, this.sg1.col,
													  "select a.kode_akun, a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag='003' "+
													   this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
													  "select count(a.kode_akun) from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag='003' "+
													  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
													  new Array("a.kode_akun","a.nama"),"where");
		}
		if (row == 3)
		{
			this.filterRep.ListDataSGFilter(this, "Data Customer",this.sg1, this.sg1.row, this.sg1.col,
													  ...
													  "INNER JOIN flag_relasi b ON a.kode_lokasi=b.kode_lokasi AND b.kode_flag='003' AND b.kode_akun='"+this.sg1.getCell(2,2)+"' "+
													   this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
													  "select count(a.kode_cust) from cust a "+
													  "INNER JOIN flag_relasi b ON a.kode_lokasi=b.kode_lokasi AND b.kode_flag='003' AND b.kode_akun='"+this.sg1.getCell(2,2)+"' "+
													  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
													  new Array("a.kode_cust","a.nama"),"and");
		}
	},
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.kode_cust=this.sg1.getCell(2,3);
			...
			this.sql=" select a.kode_cust,a.nama,isnull(b.so_awal,0) as so_awal,isnull(c.debet,0) as debet,isnull(c.kredit,0) as kredit, "+
					 "	   isnull(b.so_awal,0)+isnull(c.debet,0)-isnull(c.kredit,0) as so_akhir "+
					 "	from cust a "+
					 "	left join (select y.kode_cust,sum(case when y.dc='D' then y.nilai else -y.nilai end) so_awal "+
					 "			   from gldt_h y "+
					 "			   inner join flag_relasi z on y.kode_lokasi=z.kode_lokasi and z.kode_flag='003' and y.kode_akun=z.kode_akun "+	
					 ...
					 "	left join (select x.kode_cust,sum(case when x.dc='D' then x.nilai else 0 end) as debet, "+
					 "					  sum(case when x.dc='C' then x.nilai else 0 end) as kredit "+
					 "			   from (select y.kode_cust,y.dc,y.nilai "+
					 "					 from gldt y "+
					 "			   		 inner join flag_relasi z on y.kode_lokasi=z.kode_lokasi and z.kode_flag='003' and y.kode_akun=z.kode_akun "+	
					 ...
					 "					 union all "+
					 "					 select y.kode_cust,y.dc,y.nilai "+
					 "					 from gldt_h y "+
					 "			   		 inner join flag_relasi z on y.kode_lokasi=z.kode_lokasi and z.kode_flag='003' and y.kode_akun=z.kode_akun "+	
					 "					 where y.periode='"+this.periode+"' and y.kode_lokasi ='"+this.kode_lokasi+"' and substring(y.periode,1,4)=substring('"+this.periode+"',1,4) and y.kode_akun='"+this.kode_akun+"' "+
					 "					) x "+
					 "			   group by x.kode_cust "+
					 "			  ) c on a.kode_cust=c.kode_cust "+this.filter+
			...
			title.add("Kode Cust");width.add(70);fieldType.add("S");		
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'>"+this.app._namalokasi.toUpperCase()+"<br>Laporan Saldo Piutang<br>";			
		var d = new Date();
