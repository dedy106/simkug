window.app_saku2_report_tak_flTakJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_tak_flTakJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_tak_flTakJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(a.periode) as periode "+
											"from (select max(periode) as periode from takkirim_m where kode_lokasi='"+this.app._lokasi+"' "+
											"	  union "+
											"	  select max(periode) as periode from takterima_m where kode_lokasi='"+this.app._lokasi+"' "+
											"	  )a ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Modul","=","TAK Kirim"));
...
window.app_saku2_report_tak_flTakJurnal.extend(window.childForm);
window.app_saku2_report_tak_flTakJurnal.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 6)
			{
				...
				if (this.sg1.getCell(2,2)=="TAK Kirim")
				{
					var sql1="select a.no_kirim,a.keterangan from takkirim_m a "+filter; 
					var sql2="select count(a.no_kirim) from takkirim_m a "+filter;
					this.filterRep.ListDataSGFilter(this, "Data TAK Kirim",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
													new Array("a.no_kirim","a.keterangan"),"and",new Array("No Kirim","Keterangan"),"and");
				}
				if (this.sg1.getCell(2,2)=="TAK Terima")
				{
					var sql1="select a.no_terima,a.keterangan from takterima_m a "+filter; 
					var sql2="select count(a.no_terima) from takterima_m a "+filter;
					this.filterRep.ListDataSGFilter(this, "Data TAK Terima",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
													new Array("a.no_terima","a.keterangan"),"and",new Array("No Terima","Keterangan"),"and");
				}
			}			
		}catch(e){
			systemAPI.alert(e);
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["123","123","3","123","123","123","123","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[2,0,0,2,2,2,2,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select periode "+
									"from (select periode from takkirim_m where kode_lokasi='"+this.app._lokasi+"' "+
									"	  union "+
									"	  select periode from takterima_m where kode_lokasi='"+this.app._lokasi+"' "+
									"	  )a "+
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("TAK Kirim","TAK Terima"));
		}
		if (row == 7)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				var modul=this.sg1.getCell(2,2);
				...
				this.filter3 = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+							
							this.filterRep.filterStr("a.no_terima",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and");
				
				this.filter=this.filter+"/"+modul+"/"+this.sg1.getCell(2,1)+"/"+this.filter2+"/"+this.filter3;
				this.nama_report="server_report_saku2_tak_rptTakJurnal";
				if (this.sg1.getCell(2,7)=="Memo Jurnal")
				{
					this.nama_report="server_report_saku2_tak_rptTakBuktiJurnal";
				}
