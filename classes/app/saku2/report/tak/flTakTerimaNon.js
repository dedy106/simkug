window.app_saku2_report_tak_flTakTerimaNon = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_tak_flTakTerimaNon.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_tak_flTakTerimaNon";
		this.maximize();
...
window.app_saku2_report_tak_flTakTerimaNon.extend(window.childForm);
window.app_saku2_report_tak_flTakTerimaNon.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data TAK Terima",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_rekon, a.keterangan from yk_rekon_m a "+this.filter,
												"select count(*) from yk_rekon_m a "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from yk_rekon_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,3)=="TAK Terima")
				{
					this.nama_report = "server_report_saku2_tak_rptTakTerimaNon";
			...
			systemAPI.alert("[app_saku2_report_tak_flTakTerimaNon]::mainButtonClick:"+e);
		}
	},
