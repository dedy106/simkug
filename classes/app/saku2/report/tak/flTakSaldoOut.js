window.app_saku2_report_tak_flTakSaldoOut = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_tak_flTakSaldoOut.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_tak_flTakSaldoOut";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from takkirim_m where kode_loktuj='"+this.app._lokasi+"' and progress='0'");
...
window.app_saku2_report_tak_flTakSaldoOut.extend(window.childForm);
window.app_saku2_report_tak_flTakSaldoOut.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from takkirim_m where progress='0' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("TAK Kirim","Akun","Nasional"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,4)=="TAK Kirim")
				{
					...
					var sql = "select a.no_kirim,a.kode_lokasi,a.kode_loktuj,date_format(a.tanggal,'%d/%m/%Y') as tgl_kirim,a.keterangan as ket_kirim,a.nilai "+
							"from takkirim_m a "+
							"left join takterima_m b on a.no_kirim=b.no_kirim "+this.filter+" order by a.kode_lokasi,a.tanggal";
					this.scriptSqlCount = "select count(a.no_kirim) "+
							"from takkirim_m a "+
							"left join takterima_m b on a.no_kirim=b.no_kirim "+this.filter;
					
					this.title = new server_util_arrayList({items:["No Kirim","Lok Awal","Lok Tujuan","Tgl Kirim","Keterangan Kirim","Nilai Kirim"]});
					this.widthTable = new server_util_arrayList({items:[80,40,40,60,400,100]});
			...
			systemAPI.alert("[app_saku2_report_tak_flTakSaldoOut]::mainButtonClick:"+e);
		}
	},
