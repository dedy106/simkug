window.app_saku2_report_tak_flTakBayar = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_tak_flTakBayar.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_tak_flTakBayar";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from takkirim_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('TAKKB','TAKKBLOAD') ");
...
window.app_saku2_report_tak_flTakBayar.extend(window.childForm);
window.app_saku2_report_tak_flTakBayar.implement({
		...
		try{			
			...
			if (row ==2)
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.modul  in ('TAKKB','TAKKBLOAD')";
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from takkirim_m "+this.filter+" order by periode desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku2_tak_rptTakBayar";
			...
			systemAPI.alert("[app_saku2_report_tak_flTakBayar]::mainButtonClick:"+e);
		}
	},
