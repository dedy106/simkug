window.app_saku2_report_tak_flTakBuktiTerimaNon = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_tak_flTakBuktiTerimaNon.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_tak_flTakBuktiTerimaNon";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_rekon_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku2_report_tak_flTakBuktiTerimaNon.extend(window.childForm);
window.app_saku2_report_tak_flTakBuktiTerimaNon.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 6)
			{
					...
					var sql1="select a.no_rekon,a.keterangan from yk_rekon_m a "+filter; 
					var sql2="select count(a.no_rekon) from yk_rekon_m a "+filter;
					this.filterRep.ListDataSGFilter(this, "Data TAK Kewajiban",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
													new Array("a.no_rekon","a.keterangan"),"and",new Array("No Rekon","Keterangan"),"and");			
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select periode from yk_rekon_m where kode_lokasi='"+this.app._lokasi+"' "+
									"order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct modul from yk_rekon_j where kode_lokasi='"+this.app._lokasi+"' "+
									"order by modul desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}		
	},
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+							
							this.filterRep.filterStr("a.no_rekon",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and");
				this.filter2=this.filter+"/"+modul+"/"+this.sg1.getCell(2,1)+"/"+this.filter2;
				this.nama_report="server_report_saku2_tak_rptTakBuktiTerimaNon";				
