window.app_saku2_dashboard = function(owner,options)
{
	if (owner)
	{
		window.app_saku2_dashboard.prototype.parent.constructor.call(this,owner,options);
		this.className  = "app_saku2_dashboard";
		...
		try {
			...
			this.dataProvider = new app_saku2_remote_dataProvider(this.app._dbSetting);
...
window.app_saku2_dashboard.extend(window.panel);
window.app_saku2_dashboard.implement({	
	...
	doInformasi: function(event,no_konten){
		try{
			...
			var nama_report="server_report_saku2_konten_rptInformasi";
	...
	doRequestReady: function(sender, methodName, result, errCode, connection){
		...
		if (sender == this.dataProvider){
			...
			try{
				...
				setTimeout("window.system.getResource("+this.resourceId+").updateInfo()",this.frequency);
			}	
			try{
				...
				if (result.get("state") != "DONE" ){
					this.prosesId++;
					clearTimeout(3000);
					setTimeout("window.system.getResource("+this.resourceId+").updateInfo()",3000);
				}else {
					this.prosesId = 1;
					this.beda = false;
					clearTimeout(this.frequency);
