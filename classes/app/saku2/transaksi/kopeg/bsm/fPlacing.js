window.app_saku2_transaksi_kopeg_bsm_fPlacing = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_bsm_fPlacing.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_bsm_fPlacing";
		...
		this.pc2 = new pageControl(this,{bound:[10,11,1000,450], childPage:["Data Polis","Daftar Polis","Cari Data","Desk Quotation"]});		
		this.sg2 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-30],colCount:11,tag:9,		            
					colTitle:["No Placing","Status","Tanggal","Penanggung","Tertanggung","Curr","Sum Insured","Premi","Tgl Mulai","Tgl Selesai","Remark"],
		...
		this.cb_quot = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,11,220,20],caption:"No Quotation",tag:2,multiSelection:false,change:[this,"doChange"]}); 		
		this.e_nb = new portalui_saiLabelEdit(this.pc2.childPage[0],{bound:[20,14,200,20],caption:"No Placing",maxLength:30,readOnly:true});		
		...
		this.cb_tipe = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,11,220,20],caption:"COB",tag:2,multiSelection:false,change:[this,"doChange"],readOnly:true}); 		
		...
		this.cb_cust = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,13,220,20],caption:"Tertanggung",tag:1,multiSelection:false,readOnly:true}); 		
		this.cb_pic = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,14,220,20],caption:"Acc Exec",tag:2,multiSelection:false,readOnly:true}); 		
		...
		this.e_ppremi = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,18,200,20],caption:"Premi % - Nilai", tag:2, tipeText:ttNilai, text:"0",change:[this,"doChange"]});		
		...
		this.e_pfee = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,19,200,20],caption:"Brokerage % - Nilai", tag:2, tipeText:ttNilai, text:"0",change:[this,"doChange"]});		
		this.e_nfee = new saiLabelEdit(this.pc2.childPage[0],{bound:[260,19,100,20],caption:"", labelWidth:0, tag:1, tipeText:ttNilai, text:"0",readOnly:true,change:[this,"doChange"]});										
		...
		this.mDesk = new tinymceCtrl(this.pc2.childPage[3],{bound:[5,13,990,430], withForm:false, tag:9});
		...
		try {
			...
			this.cb_quot.setSQL("select a.no_quot, b.nama from bsm_quot_m a inner join sju_cust b on a.kode_cust=b.kode_cust and a.kode_lokasi=b.kode_lokasi where a.progress='0' and a.kode_lokasi='"+this.app._lokasi+"'",["a.no_quot","b.nama"],false,["No Bukti","Nama Cust"],"and","Data Quotation",true);						
...
window.app_saku2_transaksi_kopeg_bsm_fPlacing.extend(window.childForm);
window.app_saku2_transaksi_kopeg_bsm_fPlacing.implement({		
	...
	doChange:function(sender){
		try {
			if (sender == this.cb_quot && this.cb_quot.getText()!="") {			
				var data = this.dbLib.getDataProvider("select * from bsm_quot_m where no_quot='"+this.cb_quot.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){												
						this.cb_tipe.setSQL("select kode_tipe, nama from sju_tipe where kode_tipe ='"+line.kode_tipe+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_tipe","nama"],false,["Kode","Nama"],"and","Data Tipe",true);
						this.cb_cust.setSQL("select kode_cust, nama from sju_cust where kode_cust ='"+line.kode_cust+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_cust","nama"],false,["Kode","Nama"],"and","Data Tertanggung",true);					
						this.cb_pic.setSQL("select kode_pic, nama from sju_pic where kode_pic ='"+line.kode_pic+"' and flag_aktif='1' and kode_lokasi='"+this.app._lokasi+"'",["kode_pic","nama"],false,["Kode","Nama"],"and","Data Acc Exec",true);
	...
	doLoad2:function(sender){												
		var strSQL = "select a.no_polis, "+
					 ...
					 "where a.modul='QUOT' and a.periode<='"+this.e_periode.getText()+"' and a.progress in ('0','R') and a.kode_lokasi='"+this.app._lokasi+"' ";
	...
	doCari:function(sender){										
		var strSQL = "select a.no_polis, "+
		             ...
		             "inner join sju_cust b on a.kode_cust=b.kode_cust and a.kode_lokasi=b.kode_lokasi "+
					 "inner join sju_vendor c on a.kode_vendor=c.kode_vendor and a.kode_lokasi=c.kode_lokasi "+
					 "where a.modul='QUOT' and a.kode_cust like '%"+this.cb_cust3.getText()+"%' and a.no_polis like '%"+this.e_quo3.getText()+"%' and a.periode<='"+this.e_periode.getText()+"' and a.progress in ('0','R') and a.kode_lokasi='"+this.app._lokasi+"' ";					 		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
			this.sgn2.setTotalPage(Math.ceil(data.rs.rows.length/20));
			...
			if (this.sg2.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){													
						...
						this.cb_quot.setText(line.no_placing);
						this.cb_tipe.setText(line.kode_tipe);
						this.cb_pic.setText(line.kode_pic);
