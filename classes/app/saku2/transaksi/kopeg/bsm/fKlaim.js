window.app_saku2_transaksi_kopeg_bsm_fKlaim = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_bsm_fKlaim.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_bsm_fKlaim";
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:10,tag:0,
		            colTitle:["No Dokumen","No Polis","Tgl Polis","Unit","Penanggung","Tertanggung","Curr","Nilai","Premi","COB"],
					colWidth:[[9,8,7,6,5,4,3,2,1,0],[200,100,100,70,200,200,150,80,150,100]],
		...
		this.e_nopolis = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,19,450,20],caption:"No Polis", readOnly:true,visible:false});
		this.e_nodok = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,19,450,20],caption:"No Polis", readOnly:true});
...
window.app_saku2_transaksi_kopeg_bsm_fKlaim.extend(window.childForm);
window.app_saku2_transaksi_kopeg_bsm_fKlaim.implement({
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			case "clear" :
				...
				break;
			case "simpan" :		
				var strSQL = "select a.no_polis from bsm_polis_m a "+
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg.cells(0,row) != "") {			
				...
				this.e_nodok.setText(this.sg.cells(1,row));
				this.e_tipe.setText(this.sg.cells(9,row));
				this.e_tglpolis.setText(this.sg.cells(2,row));
				this.e_unit.setText(this.sg.cells(3,row));
				this.e_curr.setText(this.sg.cells(6,row));
				this.e_penanggung.setText(this.sg.cells(4,row));
				this.e_tertanggung.setText(this.sg.cells(5,row));
				this.e_nilai.setText(this.sg.cells(7,row));
				this.e_premi.setText(this.sg.cells(8,row));				
				this.e_memo.setText("-");													
				
				var strSQL = "select d.kode_sebab,d.nama from bsm_polis_m b "+
	...
	doLoad:function(sender){				
		var strSQL = "select a.no_polis,a.no_dok,convert(varchar,a.tanggal,103) as tgl, a.kode_pp+' - '+f.nama as pp, c.kode_cust +'-'+c.nama as cust, d.kode_vendor +'-'+d.nama as vendor,e.kode_tipe+'-'+e.nama as tipe,a.kode_curr,a.total,a.n_premi "+ 
		             "from bsm_polis_m a inner join sju_cust c on a.kode_cust = c.kode_cust and a.kode_lokasi=c.kode_lokasi "+
