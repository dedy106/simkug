window.app_saku2_transaksi_kopeg_bsm_fPolisApp = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_bsm_fPolisApp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_bsm_fPolisApp";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Approve Pengajuan Polis : Input", 0);	
		...
		this.c_status2 = new saiCB(this.pc1.childPage[2],{bound:[20,10,202,20],caption:"Status",items:["AJU","APPROVE","REVISI"], readOnly:true,tag:2});
...
window.app_saku2_transaksi_kopeg_bsm_fPolisApp.extend(window.childForm);
window.app_saku2_transaksi_kopeg_bsm_fPolisApp.implement({	
	...
	doLoad:function(sender){												
		var strSQL = "select a.no_polis, "+
					 ...
					 "where a.modul='POLIS' and a.periode<='"+this.e_periode.getText()+"' and a.progress = '0' and a.kode_lokasi='"+this.app._lokasi+"' ";
	...
	doCari:function(sender){						
		...
		var strSQL = "select a.no_polis, "+
		             ...
		             "inner join sju_cust b on a.kode_cust=b.kode_cust and a.kode_lokasi=b.kode_lokasi "+
					 "inner join sju_vendor c on a.kode_vendor=c.kode_vendor and a.kode_lokasi=c.kode_lokasi "+
					 "where a.modul='POLIS' and a.kode_lokasi='"+this.app._lokasi+"' "+filter;
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
			this.sgn.setTotalPage(Math.ceil(data.rs.rows.length/20));
