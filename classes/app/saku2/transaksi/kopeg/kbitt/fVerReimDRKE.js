window.app_saku2_transaksi_kopeg_kbitt_fVerReimDRKE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fVerReimDRKE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fVerReimDRKE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Verifikasi IF (Penggantian Uang): Edit", 0);	
		...
		this.e_nb = new saiCBBL(this,{bound:[20,11,222,20],caption:"No Agenda", multiSelection:false, maxLength:10, tag:0, readOnly:true,change:[this,"doChange"]});				
...
window.app_saku2_transaksi_kopeg_kbitt_fVerReimDRKE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fVerReimDRKE.implement({	
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from it_ifver_m where no_ver='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from it_ifver_j where no_ver='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from angg_r where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='IFSUBVER'");
					sql.add("update it_ifaju_m set no_ver='-' where no_ver='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doChange:function(sender){		
		if (sender == this.e_periode && this.e_periode.getText()!="") {						
				if (this.app._userStatus == "A") this.e_nb.setSQL("select a.no_ver, a.keterangan from it_ifver_m a where a.no_aju='-' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' ",["a.no_ver","a.keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);		
				else this.e_nb.setSQL("select a.no_ver, a.keterangan from it_ifver_m a where a.kode_pp='"+this.app._kodePP+"' and a.no_aju='-' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' ",["a.no_ver","a.keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
		}				
		if (sender == this.e_nb && this.e_nb.getText()!="") {				
			var data = this.dbLib.getDataProvider("select kode_pp,nik,keterangan,periode,tanggal "+
					   "from it_ifver_m  "+ 
					   "where no_ver='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object"){
		...
		if (sender == this.cb_nik && this.cb_nik.getText()!="") {
			var strSQL = "select d.kode_bidang,a.kode_pp,a.nilai-isnull(b.ganti,0)-isnull(c.subif,0)-isnull(e.reimburse,0) as saldo "+
						 ...
						 "								where a.no_ver<> '"+this.e_nb.getText()+"' and a.nik='"+this.cb_nik.getText()+"' and a.kode_lokasi ='"+this.app._lokasi+"' "+
	...
	doLoad:function(sender){		
		if (this.cb_nik.getText() != "" && this.e_periode.getText() != "") {
			var data = this.dbLib.getDataProvider("select 'GANTI' as status,a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.kode_akun,a.kode_pp,a.kode_drk,a.keterangan,a.nilai,a.nik+' - '+e.nama as niksub,isnull(z.jenis,'NON') as jenis,isnull(z.nilai,0) as npajak "+
					   ...
					   "                  left join (select no_ver,no_bukti,jenis,kode_lokasi,nilai from it_ifver_j where dc='D' and kode_lokasi='"+this.app._lokasi+"') z on a.no_ver = z.no_ver and a.kode_lokasi=z.kode_lokasi "+
					   "where a.no_ver = '"+this.e_nb.getText()+"' and a.no_aju='-' and a.kode_lokasi='"+this.app._lokasi+"' and x.nik_if='"+this.cb_nik.getText()+"' and a.periode<='"+this.e_periode.getText()+"'",true);					   
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
				for (var i in data.rs.rows){
					line = data.rs.rows[i];																		
					this.sg.appendData([line.status.toUpperCase(),line.tgl,line.keterangan,floatToNilai(line.nilai),line.jenis.toUpperCase(),floatToNilai(line.npajak),line.no_bukti,line.kode_akun,line.kode_drk,line.kode_pp,line.niksub]);
				}
