window.app_saku2_transaksi_kopeg_kbitt_fVerKBDRKE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fVerKBDRKE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fVerKBDRKE";
...
window.app_saku2_transaksi_kopeg_kbitt_fVerKBDRKE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fVerKBDRKE.implement({
	...
	doChange:function(sender){
		...
		if (sender == this.e_nb && this.e_nb.getText()!="") {					
			var strSQL = "select a.tanggal as tgl,b.catatan,case c.progress when '1' then 'APPROVE' when 'D' then 'REVISI' end as progress,c.npajak,"+
						 "c.no_aju,convert(varchar,c.tanggal,103) as tanggal,c.modul,bb.kode_pp+' - '+bb.nama as pp,cc.kode_akun+' - '+isnull(cc.nama,'-') as akun,zz.kode_drk +' - '+zz.nama as drk,c.keterangan,c.nilai,convert(varchar,c.tgl_input,103) as tgl_input,c.user_input as nik_user, c.no_app  "+
