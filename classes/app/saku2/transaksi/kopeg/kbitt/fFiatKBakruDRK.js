window.app_saku2_transaksi_kopeg_kbitt_fFiatKBakruDRK = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fFiatKBakruDRK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fFiatKBakruDRK";
		...
		this.e_memo2 = new saiMemo(this.pc1.childPage[1],{bound:[20,13,450,80],caption:"Cat. Ver. Dok",tag:9,readOnly:true});		
		this.e_memo = new saiMemo(this.pc1.childPage[1],{bound:[20,12,450,80],caption:"Cat. Ver. Akun",tag:9,readOnly:true});
		this.cb_cf = new saiCBBL(this.pc1.childPage[1],{bound:[20,14,220,20],caption:"Kode CashFlow", multiSelection:false, maxLength:10, tag:2});
...
window.app_saku2_transaksi_kopeg_kbitt_fFiatKBakruDRK.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fFiatKBakruDRK.implement({
	...
	doDoubleClick: function(sender, col , row) {
		if (this.sg.cells(0,row) != "") {			
			...
			if (this.e_modul.getText().substr(0,5) == "HUTIF") {
				...
				if (vForm == "BMULTI" || vForm == "PTGMULTI" || vForm == "SPPD") {
					...
					var strAJU = "select a.kode_akun,isnull(b.nama,'-') as nama_akun,'D' as dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk,a.nik_panjar+' - '+isnull(e.nama,'-') as nik_panjar "+
								 "from it_aju_m a  "+
								 "                inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
