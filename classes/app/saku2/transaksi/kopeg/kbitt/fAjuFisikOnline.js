window.app_saku2_transaksi_kopeg_kbitt_fAjuFisikOnline = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fAjuFisikOnline.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fAjuFisikOnline";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Approve Pengajuan/upload dokumen fisik Online", 0);	
...
window.app_saku2_transaksi_kopeg_kbitt_fAjuFisikOnline.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fAjuFisikOnline.implement({
	isiCBagenda: function() {
		this.e_noaju.setSQL("select a.no_aju,a.keterangan "+
							...
							"where a.progress in ('3','4') and e.no_aju is null and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' "
	...
	simpan: function(){			
		try{						
			var nb = this.standarLib.noBuktiOtomatis(this.dbLib,"it_aju_fisik","no_bukti",this.app._lokasi+"-FOL"+this.e_periode.getText().substr(2,4)+".","00000");		
	...
	doClick:function(sender){
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"it_aju_fisik","no_bukti",this.app._lokasi+"-FOL"+this.e_periode.getText().substr(2,4)+".","00000"));		
		...
		if (sender==this.e_noaju) {			
			if (this.e_noaju.getText() != "") {
				var strSQL = "select a.form,a.nilai,a.kode_pp,a.keterangan,a.tanggal,a.kode_pp+' - '+b.nama as pp,a.kode_akun+' - '+c.nama as akun,a.kode_drk+' - '+isnull(d.nama,'-') as drk "+
							 ...
							 "where a.no_aju = '"+this.e_noaju.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress in ('3','4') ";				
