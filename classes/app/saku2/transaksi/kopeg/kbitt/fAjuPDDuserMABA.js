window.app_saku2_transaksi_kopeg_kbitt_fAjuPDDuserMABA = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fAjuPDDuserMABA.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fAjuPDDuserMABA";
		...
		this.c_jenis = new saiCB(this.pc2.childPage[0],{bound:[20,12,200,20],caption:"Jenis",items:["MABA"], readOnly:true,tag:2,change:[this,"doChange"]});
		...
		try {
			...
			this.c_jenis.setText("MABA");
			this.akunPDD = "2141104";
...
window.app_saku2_transaksi_kopeg_kbitt_fAjuPDDuserMABA.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fAjuPDDuserMABA.implement({	
	doLoad: function() {
		this.stsSimpan = 0;
		var data = this.dbLib.getDataProvider(
					...
					"where a.no_aju='"+this.cb_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_ref1='MABA' ",true);
	...
	isiCBnb: function() {
		...
		this.cb_nb.setSQL("select no_aju, keterangan from it_aju_m where form = 'PDDOUTAJU' and progress in ('J','L') and periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and no_ref1='MABA' ",["no_aju","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);				
	},
	doAfterPaste: function(sender,totalRow){
		try {
			...
			////pecah menu
			//if (this.c_jenis.getText() == "MALA") var strSQL = "select nim from aka_mahasiswa where kode_lokasi ='"+this.app._lokasi+"'";			
			//else 
			...
			if (this.inValid == false) {
				...
				for (var i=0; i < this.sg1.getRowCount();i++) {
					if (this.sg1.cells(0,i).substr(0,7) == "INVALID") {
						var j = i+1;
						this.sg2.appendData([j,this.sg1.cells(13,i),"DUPLIKASI DATA"]);						
					}
				}
			}
	...
	doChange:function(sender){
		if (sender == this.c_jenis && this.c_jenis.getText()!="") {
			//if (this.c_jenis.getText() == "MALA") this.akunPDD = "2141101";
			//else 
			this.this.akunPDD = "2141104";
		}
	},	
