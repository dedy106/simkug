window.app_saku2_transaksi_kopeg_kbitt_fAjuHonor = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fAjuHonor.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fAjuHonor";
		...
		this.e_nb = new portalui_saiLabelEdit(this.pc2.childPage[0],{bound:[20,11,202,20],caption:"No Bukti",maxLength:30,readOnly:true,visible:true});		
		...
		this.cb_akun = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,12,222,20],caption:"MTA",tag:2,multiSelection:false,change:[this,"doChange"]});         		
		this.cb_drk = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,13,222,20],caption:"DRK",tag:1,multiSelection:false,change:[this,"doChange"]});         				
		this.e_saldo = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,13,202,20],caption:"Saldo Budget TW", tag:1, tipeText:ttNilai, text:"0", readOnly:true});						
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,20,990,262], childPage:["Detail Honor","Err Dosen"]});		
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:8,tag:0,
				colTitle:["Kd Dosen","Bruto","Pot. Pajak","Netto","Berita","Nama Dosen","Gelar","Rekening"],
				colWidth:[[7,6,5,4,3,2,1,0],[350,80,200,200,80,80,80,60]],
		...
		this.e_memo = new saiMemo(this.pc1.childPage[1],{bound:[5,5,980,260],labelWidth:0,tag:9});
...
window.app_saku2_transaksi_kopeg_kbitt_fAjuHonor.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fAjuHonor.implement({	
		...
		try {						
			...
			for (var i=0;i < this.sg1.getRowCount();i++){			
				if (this.sg1.rowValid(i)){
					var data = this.dbLib.getDataProvider("select nama,gelar,bank+' - '+cabang+' - '+no_rek+' - '+nama_rek as rekening from it_dosen where kode_dosen='"+this.sg1.cells(0,i)+"' and kode_lokasi='"+this.app._lokasi+"'",true);
						...
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){
								sql.add("insert into it_aju_rek(no_aju,kode_lokasi,bank,no_rek,nama_rek,bank_trans,nilai,keterangan,pajak,berita) values "+
								        "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','-','-','-','-',"+nilaiToFloat(this.sg1.cells(3,i))+",'"+this.sg1.cells(0,i)+"',"+nilaiToFloat(this.sg1.cells(2,i))+",'"+this.sg1.cells(4,i)+"')");
							}
						}
					}					
	...
	doLoad3:function(sender){																
		var strSQL = "select no_aju, convert(varchar,tanggal,103) as tgl, keterangan, nilai "+
					 "from it_aju_m "+
					 "where kode_pp='"+this.cb_pp.getText()+"' and form = 'HONOR' and modul='UMUM' and progress in ('A','K','R') and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];												
						this.sg1.appendData([line.kode_dosen,floatToNilai(line.bruto),floatToNilai(line.pajak),floatToNilai(line.nilai),line.berita,line.nama,line.gelar,line.rek]);
					}
