window.app_saku2_transaksi_kopeg_kbitt_fSpbKB = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fSpbKB.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fSpbKB";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,20,990,310], childPage:["Peruntukan","Data Agenda","Detail Agenda","File Dok"]});		
		...
		this.sgUpld = new saiGrid(this.pc1.childPage[3],{bound:[1,5,this.pc1.width-5,this.pc1.height-10],colCount:5, tag:9,
				colTitle:["KdDok","Jenis Dokumen","Nama File","DownLoad","Jenis"],
				colWidth:[[4,3,2,1,0],[60,80,300,200,80]], 
...
window.app_saku2_transaksi_kopeg_kbitt_fSpbKB.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fSpbKB.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into it_spb_m (no_spb,kode_lokasi,nik_user,tgl_input,periode,tanggal,nama,alamat,keterangan,nik_bdh,nik_ver,nik_fiat,nilai,no_kas,form,no_juspb) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.app._userLog+"',getdate(),'"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_nama.getText()+"','"+this.e_alamat.getText()+"','"+this.e_ket.getText()+"','"+this.cb_bdh.getText()+"','"+this.cb_ver.getText()+"','"+this.cb_fiat.getText()+"',"+nilaiToFloat(this.e_total.getText())+",'-','PBH','-')");					
			...
			case "clear" :
				if (modalResult == mrOk)
					...
					sql.add("select a.no_aju,a.keterangan from it_aju_m a "+
							...
							") b on a.no_aju=b.no_aju and a.kode_lokasi=b.kode_lokasi and b.total > 0 "+
	...
	doEllipsClick1: function(sender, col, row){
		try{			
			if (col == 0){				
				strSQL = "select a.no_aju,a.keterangan,c.jenis "+
						 "from it_aju_m a inner join it_ajuapp_m c on a.no_app=c.no_app and a.kode_lokasi=c.kode_lokasi "+
				...
				strROW = "select count(a.no_aju) from (select a.no_aju,a.keterangan,a.kode_lokasi from it_aju_m a "+						 
						 "inner join it_ajuapp_m c on a.no_app=c.no_app and a.kode_lokasi=c.kode_lokasi  "+
				...
				this.standarLib.showListData(this, "Daftar Agenda",sender,undefined,strSQL,strROW,
						["a.no_aju","a.keterangan","c.jenis"],"and",["No Agenda","Deskripsi","Status"],false);				
			}				
		}catch(e){
			systemAPI.alert(e);
		}
	...
	doDoubleClick1: function(sender, col , row) {
		...
		var data = this.dbLib.getDataProvider(							 
						...
						"from dok_jenis a left join it_aju_dok b on a.kode_jenis='DOK' and a.kode_lokasi=b.kode_lokasi and b.no_bukti='"+this.sg1.cells(0,row)+"' "+
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_spb,convert(varchar,a.tanggal,103) as tgl,a.nama,a.keterangan "+
		             "from it_spb_m a "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_juspb='-' and a.no_kas ='-' and form='PBH' order by a.no_spb ";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.no_aju,convert(varchar,a.tanggal,103) as tanggal,a.modul,b.kode_pp+' - '+b.nama as pp,a.keterangan,xx.nilai_fiat as nilai,g.catatan,convert(varchar,a.tgl_input,103) as tgl_input,a.user_input as nik_user,e.no_ver,convert(varchar,e.tgl_input,103) as tgl_ver,f.no_fiat,convert(varchar,f.tgl_input,103) as tgl_fiat  "+
							 ...
							 "			  from it_aju_d where kode_lokasi='"+this.app._lokasi+"' "+
							 "			  group by no_aju,kode_lokasi) xx on a.no_aju=xx.no_aju and a.kode_lokasi=xx.kode_lokasi "+
