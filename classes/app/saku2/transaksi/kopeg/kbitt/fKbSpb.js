window.app_saku2_transaksi_kopeg_kbitt_fKbSpb = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fKbSpb.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fKbSpb";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pembayaran KasBank Operasional", 0);	
		...
		this.cb_jenis = new saiCBBL(this.pc2.childPage[0],{bound:[20,12,222,20],caption:"Jenis Bayar", multiSelection:false, maxLength:10, tag:2});		
		this.e_giro = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,16,202,20],caption:"No Bilyet", maxLength:50});
		...
		this.e_total = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,19,200,20],caption:"Nilai KasBank", readOnly:true, tipeText:ttNilai, text:"0", tag:9});				
...
window.app_saku2_transaksi_kopeg_kbitt_fKbSpb.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fKbSpb.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) "+
							"select '"+this.e_nb.getText()+"',a.no_aju,'"+this.dp_d1.getDateString()+"',a.no_urut,a.kode_akun,a.keterangan,a.dc,a.nilai,a.kode_pp,a.kode_drk,'-','-',a.kode_lokasi,'KBITSPB','FIAT','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-' "+
							"from it_aju_d a inner join it_aju_m b on a.no_aju=b.no_aju and a.kode_lokasi=b.kode_lokasi  "+
							"where b.no_spb='"+this.cb_spb.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"' and a.nilai>0");

									
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_spb.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.akunKB+"','"+this.cb_spb.rightLabelCaption+"','"+this.dc+"',"+Math.abs(nilaiToFloat(this.e_nilainet.getText()))+",'"+this.app._kodePP+"','-','"+this.e_cf.getText()+"','-','"+this.app._lokasi+"','KBITSPB','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'"+this.cb_jenis.getText()+"')");
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai "+
		             "from kas_m a "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.posted ='F' and a.modul='KBITSPB' ";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
