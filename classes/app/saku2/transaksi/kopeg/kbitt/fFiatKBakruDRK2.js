window.app_saku2_transaksi_kopeg_kbitt_fFiatKBakruDRK2 = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fFiatKBakruDRK2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fFiatKBakruDRK2";
		...
		this.pc1 = new pageControl(this,{bound:[10,18,1000,500], childPage:["Data Pengajuan","Detail Pengajuan","Item Jurnal","Data KPA","Filter Data","File Dok"]});
		...
		this.e_jenis = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,14,200,20],caption:"Jenis Dok", readOnly:true});								
		this.cb_cf = new saiCBBL(this.pc1.childPage[1],{bound:[20,14,220,20],caption:"Kode CashFlow", multiSelection:false, visible: false, maxLength:10, tag:2, text:"1"});
		this.e_totRek = new saiLabelEdit(this.pc1.childPage[1],{bound:[520,14,200,20],caption:"Tot Rekening", tag:9, readOnly:true, tipeText:ttNilai, text:"0"});		
		
		this.sgRek = new saiGrid(this.pc1.childPage[1],{bound:[5,5,970,220],colCount:10,tag:0,
		...
		this.cb_hutang = new saiCBBL(this.pc1.childPage[2],{bound:[20,11,220,20],caption:"Akun BMHD", multiSelection:false, maxLength:10, tag:9,change:[this,"doChange"]});				
		...
		this.e_noju = new saiLabelEdit(this.pc1.childPage[2],{bound:[20,12,200,20],caption:"Bukti Akru", readOnly:true, tag:9});								
		...
		this.e_nilaikb = new saiLabelEdit(this.pc1.childPage[2],{bound:[790,1,200,20],caption:"Nilai KasBank", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		
		this.sg1 = new saiGrid(this.pc1.childPage[2],{bound:[1,10,this.pc1.width-5,390],colCount:9,tag:0, 
		...
		this.sgUpld = new saiGrid(this.pc1.childPage[5],{bound:[1,5,this.pc1.width-5,this.pc1.height-10],colCount:5, tag:9,
...
window.app_saku2_transaksi_kopeg_kbitt_fFiatKBakruDRK2.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fFiatKBakruDRK2.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.c_status.getText()=="APPROVE") {
						...
						//BMHD
						//if (this.e_noju.getText()!="" && this.e_noju.getText()!="-") {
						if (this.cb_hutang.getText()!="" && this.cb_hutang.getText()!="-") {	
							...
							sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
									"	('"+this.e_noju.getText()+"','"+this.e_nb.getText()+"','"+this.dp_d1.getDateString()+"',99,'"+this.cb_hutang.getText()+"','"+this.e_noaju.getText()+"|"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_nilaikb.getText())+",'"+this.app._kodePP+"','-','-','-','-','-','-','-','"+this.app._lokasi+"','JU','BMHD','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','-')");																
							if (this.sg1.getRowValidCount() > 0){
								for (var i=0;i < this.sg1.getRowCount();i++){
									if (this.sg1.rowValid(i)){
										var j = i+1;
										sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
												"	('"+this.e_noju.getText()+"','"+this.e_nb.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(0,i)+"','"+this.e_noaju.getText()+"|"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"','"+this.sg1.cells(7,i)+"','-','-','-','-','-','-','"+this.app._lokasi+"','JU','KBITT','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','-')");
									}
								}
							}											
						}
						...
						for (var j=0;j<this.dataAkunGar.rs.rows.length;j++){
							...
							if (line.kode_akun == this.sg2.cells(0, i)) {
								if (nilaiToFloat(this.sg2.cells(7, i)) > 0 && nilaiToFloat(this.sg2.cells(6, i)) < nilaiToFloat(this.sg2.cells(7, i))) {
									...
									if (!temu && this.formInput != "INISIASI" && this.formInput == "PRPTG" && this.formInput == "ISPJPTG" && this.formInput == "PRPJPTG" && this.formInput == "PRAGENDA" && this.formInput == "PRBEBAN") {
										var k =i+1;
	...
	doChange:function(sender){
		if (sender == this.cb_hutang) {
			if (this.cb_hutang.getText()=="" || this.cb_hutang.getText()=="-") {
				this.cb_hutang.setTag(9); 
				this.e_noju.setTag(9);
				this.e_noju.setText("-");
			}
			else {
				this.cb_hutang.setTag(0); 
				this.e_noju.setTag(0);
				this.e_noju.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"ju_m","no_ju",this.app._lokasi+"-JU"+this.e_periode.getText().substr(2,4)+".","0000"));
			}
		}
	},
		...
		if (this.sg.cells(0,row) != "") {					
			...
			if (this.e_modul.getText().substr(0,5) == "HUTIF") {
				...
				if (vForm == "KANTIN" || vForm == "BMULTIREK" || vForm == "BMULTI" || vForm == "PTGMULTI" || vForm == "PTGMULTIBD" || vForm == "SPPD2" ) {
					...
					if (vForm == "PRPTG" || this.formInput == "ISPJPTG" || vForm == "PRPJPTG" || vForm == "PRAGENDA" || vForm == "PRBEBAN" ) {						
					...
					else 
					{
						var strAJU = "select a.kode_akun,isnull(b.nama,'-') as nama_akun,'D' as dc,a.keterangan,"+
									...
									/*
									"union "+
									"select a.kode_akun,isnull(b.nama,'-') as nama_akun,'D' as dc,a.keterangan,a.npajak as nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk,a.nik_panjar+' - '+isnull(e.nama,'-') as nik_panjar "+
									"from it_aju_m a  "+
									"                inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
									"                left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and d.tahun=substring(a.periode,1,4) "+						
									"                left join karyawan e on a.nik_panjar=e.nik and a.kode_lokasi=e.kode_lokasi "+						
									"				  left join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
									"where a.sts_pajak <> 'NON' and a.no_aju = '"+this.e_noaju.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' ";				
									*/
					}
				}
			}					
	...
	doCari:function(sender){				
		...
		var strSQL = "select no_aju,convert(varchar,a.tanggal,103) as tanggal,a.modul,b.kode_pp+' - '+b.nama as pp,c.kode_akun+' - '+isnull(c.nama,'-') as akun,g.kode_drk+' - '+isnull(g.nama,'-') as drk,a.keterangan,a.nilai,convert(varchar,a.tgl_input,103) as tgl_input,a.user_input as nik_user,e.no_ver,convert(varchar,e.tgl_input,103) as tgl_ver,f.catatan "+
					 ...
					 "inner join ver_d f on f.no_ver=e.no_ver and f.kode_lokasi=e.kode_lokasi "+
					 "left join masakun c on a.kode_akun=c.kode_akun and a.kode_lokasi=c.kode_lokasi "+
					 "left join drk g on a.kode_drk=g.kode_drk and a.kode_lokasi=g.kode_lokasi and substring(a.periode,1,4)=g.tahun "+filter;					 							
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
			this.sgn.setTotalPage(Math.ceil(data.rs.rows.length/20));
	...
	doNilaiChange1: function(){
		try{			
			...
			if (this.e_modul.getText()!="PJPTG") this.e_nilaikb.setText(floatToNilai(tot));						
				...
				2019
				var data = this.dbLib.getDataProvider(
					   "select sum(case a.dc when 'D' then a.nilai else -a.nilai end) as nilai_pj "+
				...
				*/
				...
				var data = this.dbLib.getDataProvider("select case when b.no_ref1='CLOSE' then a.nilai else a.nilai+a.npajak end as nilai_pj "+
				...
				//this.e_nilaikb.setText(floatToNilai(nilaiToFloat(this.e_nilaipj.getText()) - tot ));	28/01-2019	-- gk jadi(harus minus)		
			}
	...
	doHitungGar: function(){
		...
		var sls = 0;
		for (var i=0;i < this.sg2.getRowCount();i++){
			//alert("select fn_cekagg3('"+this.sg2.cells(2,i)+"','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(4,i)+"','"+this.periodeAju+"','"+this.e_noaju.getText()+"') as gar ");
