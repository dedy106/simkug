window.app_saku2_transaksi_kopeg_kbitt_fReimDRKNonSimpleE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fReimDRKNonSimpleE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fReimDRKNonSimpleE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan Reimburse IF Non Sub Simple: Edit", 0);	
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('ITHUTIF') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku2_transaksi_kopeg_kbitt_fReimDRKNonSimpleE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fReimDRKNonSimpleE.implement({	
	...
	doChange:function(sender){				
		...
		if (sender == this.e_periode && this.e_periode.getText()!="") {						
			this.e_nb.setSQL("select a.no_aju, a.keterangan from it_aju_m a where a.nik_panjar='"+this.app._userLog+"' and a.form = 'HUTIFNONSP' and a.modul='HUTIF' and a.progress in ('A','K','R','0') and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' ",["a.no_aju","a.keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
		}		
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			var data = this.dbLib.getDataProvider(
			           "select a.tanggal,a.periode,a.keterangan,a.nilai,a.kode_pp,b.kode_akun,b.kode_drk,a.user_input,a.sts_pajak,a.npajak,isnull(c.no_gambar,'') as no_gambar  "+
					   "from it_aju_m a inner join it_ifreim_j b on a.no_aju=b.no_aju and a.kode_lokasi=b.kode_lokasi and b.jenis='BEBAN' "+
			...
			var strSQL = "select d.kode_bidang,a.kode_pp,a.nilai-isnull(b.ganti,0)-isnull(c.subif,0)-isnull(e.reimburse,0) as saldo "+
					     ...
					     "                              from it_aju_m where no_aju <> '"+this.e_nb.getText()+"' and no_kas='-' and modul='HUTIF' and kode_lokasi='"+this.app._lokasi+"' and nik_panjar='"+this.app._userLog+"' "+
