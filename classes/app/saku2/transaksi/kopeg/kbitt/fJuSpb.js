...
window.app_saku2_transaksi_kopeg_kbitt_fJuSpb.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("update a set a.kode_akun=a.akun_seb, a.akun_seb='-' "+
								"from it_aju_d a inner join it_aju_m b on a.no_aju=b.no_aju and a.kode_lokasi=b.kode_lokasi "+
								"where b.no_spb='"+this.cb_spb.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"'");		
						
						sql.add("update it_spb_m set no_juspb='-' "+
								"where no_juspb='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");																						
					}
					...
					sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref)  "+
							...
							"from it_aju_d a inner join it_aju_m b on a.no_aju=b.no_aju and a.kode_lokasi=b.kode_lokasi "+
							"where b.no_spb='"+this.cb_spb.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"' and a.nilai>0 and a.kode_akun not in (select kode_akun from flag_relasi where kode_lokasi='11' and kode_flag='060') ");
					...
					sql.add("update a set a.akun_seb=a.kode_akun "+
							"from it_aju_d a inner join it_aju_m b on a.no_aju=b.no_aju and a.kode_lokasi=b.kode_lokasi "+
							"where b.no_spb='"+this.cb_spb.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"'");										
					
					sql.add("update a set a.kode_akun='"+this.cb_jenis.getText()+"' "+
							"from it_aju_d a inner join it_aju_m b on a.no_aju=b.no_aju and a.kode_lokasi=b.kode_lokasi and a.kode_akun not in (select kode_akun from flag_relasi where kode_lokasi='11' and kode_flag='060') "+
							"where b.no_spb='"+this.cb_spb.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"'");										

					
					sql.add("update it_aju_m set no_juspb='"+this.e_nb.getText()+"' "+
							"where no_spb='"+this.cb_spb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");												
					
					sql.add("update it_spb_m set no_juspb='"+this.e_nb.getText()+"' "+
							"where no_spb='"+this.cb_spb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");														
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					setTipeButton(tbSimpan);
					this.stsSimpan = 1;
					this.cb_spb.setSQL("select a.no_spb, a.keterangan from it_spb_m a "+
									   " left join ju_m b on a.no_spb=b.no_dokumen and a.kode_lokasi=b.kode_lokasi and b.modul='JUSPB' "+
									   "where b.no_ju is null and a.no_kas='-' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",["no_spb","keterangan"],false,["No SPB","Deskripsi"],"and","Data SPB",true);									
				break;
			case "simpan" :					
			case "ubah" :					
				this.preView = "1";
			...
			case "hapus" :	
				...
				if (parseFloat(this.app._periode) > parseFloat(this.e_periode.getText())){
					...
					sql.add("update it_aju_m set no_juspb='-' "+
							"where no_spb='"+this.cb_spb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");	
					sql.add("update it_spb_m set no_juspb='-' "+
							"where no_juspb='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");																																			
					setTipeButton(tbAllFalse);	
	...
	doDoubleClick: function(sender, col , row) {
		if (this.sg.cells(0,row) != "") {			
			...
			var data = this.dbLib.getDataProvider(
						...
						"where a.no_aju = '"+this.e_noaju.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_akun not in (select kode_akun from flag_relasi where kode_lokasi='11' and kode_flag='060')  order by a.no_urut",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg1.clear();
				for (var i in data.rs.rows){
	...
	doLoad:function(sender){
		if (this.cb_spb.getText() != "") {
			...
			var strSQL = "select isnull(sum(case a.dc when 'D' then a.nilai else -a.nilai end),0) as total "+
			             "from it_aju_d a "+
						 "inner join it_aju_m b on a.no_aju=b.no_aju and a.kode_lokasi=b.kode_lokasi "+
						 "where b.no_spb = '"+this.cb_spb.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"' and a.kode_akun not in (select kode_akun from flag_relasi where kode_lokasi='11' and kode_flag='060') ";
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){					
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){
						...
						this.cb_spb.setSQL("select no_spb, keterangan from it_spb_m where no_spb='"+line.no_dokumen+"' and kode_lokasi='"+this.app._lokasi+"'",["no_spb","keterangan"],false,["No SPB","Deskripsi"],"and","Data SPB",true);									
