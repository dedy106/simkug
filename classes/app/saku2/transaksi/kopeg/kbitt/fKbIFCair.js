window.app_saku2_transaksi_kopeg_kbitt_fKbIFCair = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fKbIFCair.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fKbIFCair";
		...
		this.c_jenis = new saiCB(this.pc2.childPage[0],{bound:[20,12,200,20],caption:"Jenis",items:["BK","KK"], readOnly:true,tag:2,change:[this,"doChange"]});
...
window.app_saku2_transaksi_kopeg_kbitt_fKbIFCair.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fKbIFCair.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from it_if where no_kas='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("update it_if set no_flag='-',flag_aktif='1' where nik='"+this.cb_nik.getText()+"' and no_flag='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}
					...
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akunif.getText()+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBIFCAIR','IF','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");										
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',1,'"+this.cb_akunkb.getText()+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBIFCAIR','KREDIT','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");					
							
					sql.add("insert into it_if(nik,kode_lokasi,kode_pp,nilai,flag_aktif,periode,no_kas,akun_if,no_flag) values "+
					...
					sql.add("update it_if set no_flag='"+this.e_nb.getText()+"',flag_aktif='0' where no_kas<>'"+this.e_nb.getText()+"' and no_flag='-' and nik='"+this.cb_nik.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
