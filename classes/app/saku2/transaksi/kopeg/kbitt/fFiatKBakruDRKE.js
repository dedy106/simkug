window.app_saku2_transaksi_kopeg_kbitt_fFiatKBakruDRKE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fFiatKBakruDRKE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fFiatKBakruDRKE";
		...
		this.e_memo2 = new saiMemo(this.pc1.childPage[0],{bound:[20,13,450,80],caption:"Cat. Ver. Dok",tag:9,readOnly:true});
		this.e_memo = new saiMemo(this.pc1.childPage[0],{bound:[20,12,450,80],caption:"Cat. Ver. Akun",tag:9,readOnly:true});
...
window.app_saku2_transaksi_kopeg_kbitt_fFiatKBakruDRKE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fFiatKBakruDRKE.implement({
		...
		if (sender == this.e_nb && this.e_nb.getText()!="") {					
			...
			if (this.statusBMHD == "-") {				
				...
				var strSQL = "select dd.kode_akun,isnull(b.nama,'-') as nama_akun,dd.dc,dd.keterangan,dd.nilai,dd.kode_pp,c.nama as nama_pp,dd.kode_drk,isnull(d.nama,'-') as nama_drk,aa.nik_panjar+' - '+isnull(e.nama,'-') as nik_panjar,a.kode_akun as akun_bmhd,dd.no_ju "+
						...
						"                left join karyawan e on aa.nik_panjar=e.nik and aa.kode_lokasi=e.kode_lokasi "+													
						"where dd.jenis <> 'BMHD' and a.jenis<> 'PJ' a.no_aju = '"+this.e_noaju.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut";
			}			
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
