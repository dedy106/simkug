window.app_saku2_transaksi_kopeg_kbitt_fFiatKBakruDRK2E = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fFiatKBakruDRK2E.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fFiatKBakruDRK2E";
		...
		this.pc1 = new pageControl(this,{bound:[20,18,1000,460], childPage:["Detail Pengajuan","Item Jurnal","Data KPA"]});										
		...
		this.e_memo2 = new saiMemo(this.pc1.childPage[0],{bound:[20,13,450,50],caption:"Cat. Ver. Dok",tag:9,readOnly:true});
		this.e_memo = new saiMemo(this.pc1.childPage[0],{bound:[520,13,450,50],caption:"Cat. Ver. Akun",tag:9,readOnly:true});
		this.cb_cf = new saiCBBL(this.pc1.childPage[0],{bound:[20,14,220,20],caption:"Kode CashFlow", multiSelection:false, maxLength:10, tag:2,text:"1",visible:false});
		this.e_jenis = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,14,200,20],caption:"Jenis Dok", readOnly:true});								
		this.e_totRek = new saiLabelEdit(this.pc1.childPage[0],{bound:[520,14,200,20],caption:"Tot Rekening", tag:9, readOnly:true, tipeText:ttNilai, text:"0"});		
	
		this.sgRek = new saiGrid(this.pc1.childPage[0],{bound:[5,5,970,180],colCount:10,tag:0,
		...
		this.cb_hutang = new saiCBBL(this.pc1.childPage[1],{bound:[20,11,220,20],caption:"Akun BMHD", multiSelection:false, maxLength:10, tag:9,change:[this,"doChange"]});				
		...
		this.e_nilaikb = new saiLabelEdit(this.pc1.childPage[1],{bound:[790,1,200,20],caption:"Nilai KasBank", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		
		this.sg1 = new saiGrid(this.pc1.childPage[1],{bound:[1,10,this.pc1.width-5,345],colCount:9,tag:0, 
...
window.app_saku2_transaksi_kopeg_kbitt_fFiatKBakruDRK2E.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fFiatKBakruDRK2E.implement({
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
				...
				if (this.flagGarFree == "0") {
					...
					for (var i=0;i < this.sg2.getRowCount();i++){
						if (nilaiToFloat(this.sg2.cells(7,i))>0 && nilaiToFloat(this.sg2.cells(6,i)) < nilaiToFloat(this.sg2.cells(7,i))) {
							...
							if (!temu && this.formInput != "INISIASI" && this.formInput != "PRBBN") {
								var k =i+1;
			...
			case "hapus" :					
				...
				if (this.formInput != "PRPTG" && this.formInput != "ISPJPTG" && this.formInput != "PRPJPTG" && this.formInput != "PRAGENDA" && this.formInput != "PRBEBAN" ) {
					sql.add("delete from it_aju_d where no_aju='"+this.e_noaju.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");	
				}
	...
	doChange:function(sender){		
		...
		if (sender == this.e_nb && this.e_nb.getText()!="") {					
			var strSQL = "select a.tanggal as tgl,b.catatan,case c.progress when '2' then 'APPROVE' when '4' then 'APPROVE' when 'F' then 'REVISI' end as progress, "+
						 "c.no_aju,convert(varchar,c.tanggal,103) as tanggal,c.modul,bb.kode_pp+' - '+bb.nama as pp,cc.kode_akun+' - '+isnull(cc.nama,'-') as akun,zz.kode_drk+' - '+isnull(zz.nama,'-') as drk,c.keterangan,c.nilai,convert(varchar,c.tgl_input,103) as tgl_input,c.user_input as nik_user,ee.no_ver,ee.catatan as catatan_ver,convert(varchar,ff.tgl_input,103) as tanggal_ver, isnull(dd.modul,'-') as status_bmhd,a.kode_cf, c.form "+
			...
			this.periodeAju = "";
			//var data = this.dbLib.getDataProvider("select periode1 from angg_r where no_bukti='"+this.e_noaju.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
