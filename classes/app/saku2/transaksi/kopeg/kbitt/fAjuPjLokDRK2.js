window.app_saku2_transaksi_kopeg_kbitt_fAjuPjLokDRK2 = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fAjuPjLokDRK2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fAjuPjLokDRK2";
		...
		this.pc1 = new pageControl(this,{bound:[20,12,900,208], childPage:["Data Rekening","List Panjar OutStanding","File Dok"]});
		...
		this.e_file = new saiLabelEdit(this.pc1.childPage[2],{bound:[20,15,450,20],caption:"File", readOnly:true, tag:0});		
...
window.app_saku2_transaksi_kopeg_kbitt_fAjuPjLokDRK2.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fAjuPjLokDRK2.implement({	
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "simpan" :			
				...
				var data = this.dbLib.getDataProvider("select count(no_aju) as jml from it_aju_m where progress <>'4' and nik_panjar ='"+this.cb_nik.getText()+"' and modul = 'PANJAR' and no_ptg='-' and kode_lokasi='"+this.app._lokasi+"'",true);
