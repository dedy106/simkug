window.app_saku2_transaksi_kopeg_kbitt_fSPBMutKB = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fSPBMutKB.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fSPBMutKB";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form SPB Droping Dana", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,20,995,315], childPage:["Peruntukan","Data Transfer"]});		
		...
		this.cb_lokasi = new saiCBBL(this.pc1.childPage[0],{bound:[20,12,220,20],caption:"Lokasi Penerima", multiSelection:false, maxLength:10, tag:2});
		this.cb_tak = new saiCBBL(this.pc1.childPage[0],{bound:[20,13,220,20],caption:"Akun TAK", multiSelection:false, maxLength:10, tag:2});
...
window.app_saku2_transaksi_kopeg_kbitt_fSPBMutKB.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fSPBMutKB.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into it_spb_m (no_spb,kode_lokasi,nik_user,tgl_input,periode,tanggal,nama,alamat,keterangan,nik_bdh,nik_ver,nik_fiat,nilai,no_kas,form) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.app._userLog+"',getdate(),'"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_nama.getText()+"','"+this.e_alamat.getText()+"','"+this.e_ket.getText()+"','"+this.cb_bdh.getText()+"','"+this.cb_ver.getText()+"','"+this.cb_fiat.getText()+"',"+nilaiToFloat(this.e_total.getText())+",'-','TAK')");											
					...
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i)){
							sql.add("insert into it_spb_rek(no_spb,kode_lokasi,form,nu,kode_rek,bank,cabang,no_rek,nama_rek,nilai,param1) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','TAK',"+i+",'"+this.sg1.cells(0,i)+"', '"+this.sg1.cells(4,i)+"','"+this.sg1.cells(5,i)+"','"+this.sg1.cells(2,i)+"','"+this.sg1.cells(3,i)+"',"+nilaiToFloat(this.sg1.cells(7,i))+",'"+this.sg1.cells(6,i)+"')");																	
						}						
					}	
					
					//posting = Z (posting dr kas); progress=Z supaya tidak bisa diload langsung dari penerima
					sql.add("insert into takkirim_m(no_kirim,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user,kode_loktuj,progress,no_terima,due_date) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','-','"+this.e_ket.getText()+"','"+this.app._kodePP+"','MUTKB','KIRIM','IDR',1,"+nilaiToFloat(this.e_total.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"','Z','-','-','"+this.cb_tak.getText()+"',getdate(),'"+this.app._userLog+"','"+this.cb_lokasi.getText()+"','Z','-','"+this.dp_d1.getDateString()+"')");
					sql.add("insert into takkirim_j(no_kirim,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.cb_tak.getText()+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_total.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','MUTKB','TAK','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_spb,convert(varchar,a.tanggal,103) as tgl,a.nama,a.keterangan "+
		             "from it_spb_m a "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_kas ='-' and form='TAK' order by a.no_spb ";
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.*,b.kode_loktuj,b.ref1 from it_spb_m a "+
							 "inner join takkirim_m b on a.no_spb=b.no_kirim and a.kode_lokasi=b.kode_lokasi "+
