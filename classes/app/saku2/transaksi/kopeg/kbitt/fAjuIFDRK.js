window.app_saku2_transaksi_kopeg_kbitt_fAjuIFDRK = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fAjuIFDRK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fAjuIFDRK";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan Beban untuk I/F: Input", 0);	
		...
		try {
			...
			var strSQL = "select a.nik,a.nilai-isnull(b.pakai,0) as saldo "+
						 ...
						 "                 			  from it_ifaju_m where no_ver='-' and kode_lokasi='"+this.app._lokasi+"' and nik='"+this.app._userLog+"' "+
...
window.app_saku2_transaksi_kopeg_kbitt_fAjuIFDRK.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fAjuIFDRK.implement({	
	...
	doClick:function(sender){
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"it_ifaju_m","no_bukti",this.app._lokasi+"-IF"+this.e_periode.getText().substr(2,2)+".","00000"));
		this.cb_pp.setFocus();
		setTipeButton(tbSimpan);
	},	
