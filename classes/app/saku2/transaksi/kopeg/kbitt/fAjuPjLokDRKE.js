window.app_saku2_transaksi_kopeg_kbitt_fAjuPjLokDRKE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fAjuPjLokDRKE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fAjuPjLokDRKE";
...
window.app_saku2_transaksi_kopeg_kbitt_fAjuPjLokDRKE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fAjuPjLokDRKE.implement({	
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{				
					...
					sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_kpa,no_app,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,sts_pajak,npajak,due_date,dasar,form) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.c_jenis.getText()+"','"+this.akunPJ+"','"+this.cb_pp.getText()+"','-','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",getdate(),'"+this.app._userLog+"','-','-','-','-','-','A','"+this.cb_nik.getText()+"','-','NON',0,'"+this.dp_d2.getDateString()+"','"+this.e_dasar.getText()+"','SINGLE')");					
					if (this.stsGar == "1") {
	...
	doChange:function(sender){						
		...
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			var data = this.dbLib.getDataProvider("select a.progress,a.modul,a.tanggal,a.periode,a.keterangan,a.nilai,a.kode_pp,a.nik_panjar,a.kode_drk,isnull(x.no_kpa,'-') as no_ver,isnull(x.catatan,'-') as catatan, "+
