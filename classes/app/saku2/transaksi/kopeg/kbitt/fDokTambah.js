window.app_saku2_transaksi_kopeg_kbitt_fDokTambah = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fDokTambah.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fDokTambah";
...
window.app_saku2_transaksi_kopeg_kbitt_fDokTambah.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fDokTambah.implement({
	isiCBagenda: function() {
		this.e_noaju.setSQL("select distinct a.no_aju, a.keterangan "+
							"from it_aju_m a "+
							"where a.progress in ('1','2','3','S') and a.kode_lokasi='"+this.app._lokasi+"' and a.periode >= '202003' ",["a.no_aju","a.keterangan"],false,["No Agenda","Deskripsi"],"and","Data Agenda",true);
	},	
					...
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i) && this.sgUpld.cells(2,i)!="-") {							
							...
							sql.add("insert into it_aju_dok(no_bukti,no_gambar,kode_lokasi,modul) values "+
									"('"+this.e_noaju.getText()+"','"+this.sgUpld.cells(3,i).tmpfile+"','"+this.app._lokasi+"','"+this.modul+"')");															
						}	
					}	
