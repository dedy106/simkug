window.app_saku2_transaksi_kopeg_kbitt_fAjuPtg = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fAjuPtg.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fAjuPtg";
...
window.app_saku2_transaksi_kopeg_kbitt_fAjuPtg.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fAjuPtg.implement({	
	...
	doChange:function(sender){
		if (sender == this.cb_nik || this.cb_nik.getText()!="") {			
			this.cb_panjar.setSQL("select a.no_aju,a.keterangan from it_aju_m a left join pr_beban_d c on a.no_aju=c.no_aju and a.kode_lokasi=c.kode_lokasi and c.modul='PANJAR' "+
								  "where c.no_aju is null and a.progress='3' and a.nik_panjar ='"+this.cb_nik.getText()+"' and a.modul = 'PANJAR' and a.no_ptg='-' and a.kode_lokasi='"+this.app._lokasi+"'",["a.no_aju","a.keterangan"],false,["No Bukti","Keterangan"],"and","Data Panjar",true);
		}
