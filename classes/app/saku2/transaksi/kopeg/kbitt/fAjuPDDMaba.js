window.app_saku2_transaksi_kopeg_kbitt_fAjuPDDMaba = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fAjuPDDMaba.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fAjuPDDMaba";
		...
		this.pc2 = new pageControl(this,{bound:[10,10,1000,440], childPage:["Transaksi","List Data"]});				
		this.c_jenis = new saiCB(this.pc2.childPage[0],{bound:[20,12,200,20],caption:"Jenis",items:["CAMABA"], readOnly:true,tag:2,change:[this,"doChange"]});
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,996,223], childPage:["Daftar Rekening","Error"]});
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-33],colCount:8,tag:0,
				colTitle:["No Peserta","Nama","Bank","No Rekening","Nama Rekening","Nilai","Thn Akademik","No Duplikasi"],
				colWidth:[[7,6,5,4,3,2,1,0],[100,100,100,200,120,120,200,100]],
				readOnly:true,
				colFormat:[[5],[cfNilai]],												
		...
		this.sg2 = new portalui_saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:3,tag:9,
				...
				colWidth:[[2,1,0],[100,100,100]],autoAppend:false,
		...
		try {
			...
			this.c_jenis.setText("CAMABA");
		}catch(e){
			systemAPI.alert(e);
		}
	}
};
window.app_saku2_transaksi_kopeg_kbitt_fAjuPDDMaba.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fAjuPDDMaba.implement({	
	doLoad: function() {
		this.stsSimpan = 0;
		var data = this.dbLib.getDataProvider(
					...
					"where a.no_aju='"+this.cb_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_ref1='CAMABA' ",true);
		...
		var strSQL = "select top 20 '-'  as bank,'-' as no_rek,'-' as nama_rek,b.nilai,b.nim,c.nama "+
					"from aka_cd_d b "+
					"inner join aka_maba c on b.nim=c.no_tes and b.kode_lokasi=c.kode_lokasi "+
		...
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			...
			for (var i in data.rs.rows){
				line = data.rs.rows[i];												
				this.sg1.appendData([line.nim,line.nama, line.bank,line.no_rek,line.nama_rek,floatToNilai(line.nilai)]);
			}
	...
	isiCBnb: function() {
		this.cb_nb.setSQL("select no_aju, keterangan from it_aju_m where form = 'PDDOUT' and progress in ('A','K','R') and periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and no_ref1='CAMABA' ",["no_aju","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);				
	},
		...
		try {
			...
			for (var i=0; i < this.sg1.getRowCount();i++){
				for (var j=0; j < this.sg1.getRowCount();j++){
					if (this.sg1.cells(0,i) == this.sg1.cells(0,j) && this.sg1.cells(6,i) == this.sg1.cells(6,j) && (i != j) ) {
						this.inValid = true;
						var k = i+1;
						var l = j+1;						
				...
				if (this.dataNIS.rs.rows.length > 0) {
					for (var j=0;j < this.dataNIS.rs.rows.length;j++){				
						if (this.sg1.cells(0,i).substr(10,30) == this.dataNIS.rs.rows[j].nim) {
							...
							//------------ cek duplikasi tabel ------------
							var temu = false;
							var data = this.dbLib.getDataProvider("select no_bukti from aka_cd_d where nim='"+this.dataNIS.rs.rows[j].nim+"' and tahunaka='"+this.sg1.cells(6,i)+"'",true);
							if (typeof data == "object"){
								var line = data.rs.rows[0];							
								if (line != undefined){
									this.sg1.cells(7,i,line.no_bukti);
									temu = true;		
								} 							
							}	
							//------------ cek duplikasi tabel ------------
			...
			if (this.inValid == false) setTipeButton(tbSimpan);	
				...
				for (var i=0; i < this.sg1.getRowCount();i++) {
					if (this.sg1.cells(0,i).substr(0,7) == "INVALID") {
						var j = i+1;
						this.sg2.appendData([j,this.sg1.cells(7,i),"DUPLIKASI DATA"]);											
					}
				}
			}
	...
	doChange:function(sender){
		if (sender == this.c_jenis && this.c_jenis.getText()!="") {
			if (this.c_jenis.getText() == "CAMABA") this.cb_akun.setText("2141104");
