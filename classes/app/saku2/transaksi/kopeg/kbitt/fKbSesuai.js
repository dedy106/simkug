window.app_saku2_transaksi_kopeg_kbitt_fKbSesuai = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fKbSesuai.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fKbSesuai";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Jurnal Adjustment : Input", 0);	
		...
		this.pc2 = new pageControl(this,{bound:[10,10,1000,450], childPage:["List KasBank","Data Jurnal","Cari Data"]});		
		this.sg3 = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-30],colCount:7,tag:9,
		            colTitle:["No KasBank","Tanggal","Modul","No Agenda","Deskripsi","Posted","Nilai"],
		...
		this.c_jenis = new saiCB(this.pc2.childPage[1],{bound:[20,22,202,20],caption:"Jenis",items:["ADJUST"], readOnly:true,tag:2});
		this.e_nb = new portalui_saiLabelEdit(this.pc2.childPage[1],{bound:[20,12,202,20],caption:"No JU",maxLength:30,readOnly:true});
		...
		this.e_dok = new saiLabelEdit(this.pc2.childPage[1],{bound:[20,16,450,20],caption:"No KasBank", readOnly:true});		
		this.e_ket = new saiLabelEdit(this.pc2.childPage[1],{bound:[20,17,450,20],caption:"Deskripsi", maxLength:150});		
		this.cb_buat = new saiCBBL(this.pc2.childPage[1],{bound:[20,16,200,20],caption:"NIK Pembuat", multiSelection:false, maxLength:10, tag:2});
		this.e_debet = new saiLabelEdit(this.pc2.childPage[1],{bound:[700,16,220,20],caption:"Total Debet", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		this.cb_app = new saiCBBL(this.pc2.childPage[1],{bound:[20,17,200,20],caption:"NIK Approve", multiSelection:false, maxLength:10, tag:2});
		this.e_kredit = new saiLabelEdit(this.pc2.childPage[1],{bound:[700,17,220,20],caption:"Total Kredit", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		
		this.pc1 = new pageControl(this.pc2.childPage[1],{bound:[20,12,900,280], childPage:["Jurnal KB","Jurnal Adjust","Data Anggaran"]});
		this.sg4 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-40],colCount:9,tag:0,
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-40],colCount:9,tag:0,
		            colTitle:["Kode Akun","Nama Akun","DC","Keterangan","Nilai","Kode PP","Nama PP","Kode DRK","Nama DRK"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[150,80,150,80,100,200,50,150,80]],					
		...
		this.sg2 = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-40],colCount:9,tag:9,
		...
		this.e_nobukti = new saiLabelEdit(this.pc2.childPage[2],{bound:[20,11,200,20],caption:"No Agenda",tag:9});
		this.e_nokas = new saiLabelEdit(this.pc2.childPage[2],{bound:[20,12,200,20],caption:"No KasBank",tag:9});
		this.bCari = new button(this.pc2.childPage[2],{bound:[230,12,80,18],caption:"Cari Data",click:[this,"doCari"]});			
		
		this.rearrangeChild(10, 23);
		this.pc2.childPage[1].rearrangeChild(10, 22);
		...
		try {
			...
			this.flagAkunKB = "0"; this.flagGarFree = "0"; this.flagDokFree = "0"; this.flagFAGL = "0"; this.ppFA = "-";
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('FAGL','FAPP','JUAKUNKB','GARFREE','DOKFREE') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku2_transaksi_kopeg_kbitt_fKbSesuai.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fKbSesuai.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','-','JU','"+this.c_jenis.getText()+"','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','F','-','-','-',getdate(),'"+this.app._userLog+"')");
	...
	doLoad3:function(sender){																				
		var strSQL = "select a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai,b.modul,a.posted,b.no_aju "+
		             "from kas_m a inner join it_aju_m b on a.no_dokumen=b.no_aju and a.kode_lokasi=b.kode_lokasi "+					 
					 "             left join ju_m c on a.no_kas=c.no_dokumen and a.kode_lokasi=c.kode_lokasi "+
					 "where c.no_ju is null and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' ";				
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_kas,line.tgl,line.modul,line.no_aju,line.keterangan,line.posted,floatToNilai(line.nilai)]); 
		}
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {				
				...
				this.pc1.setActivePage(this.pc1.childPage[0]);
				this.e_dok.setText(this.sg3.cells(0,row));
				var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk "+
							...
							"where a.no_kas = '"+this.sg3.cells(0,row)+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg4.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];													
						this.sg4.appendData([line.kode_akun,line.nama_akun,line.dc,line.keterangan,floatToNilai(line.nilai),line.kode_pp,line.nama_pp,line.kode_drk,line.nama_drk]);
					}
	...
	doCari:function(sender){				
		var filter = "";		
		if (this.e_nobukti.getText()!="") filter = " where a.no_dokumen='"+this.e_nobukti.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";			
		else filter = " where a.no_kas='"+this.e_nokas.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";			
		
		var strSQL = "select a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai,b.modul,a.posted,b.no_aju "+
		             "from kas_m a inner join it_aju_m b on a.no_dokumen=b.no_aju and a.kode_lokasi=b.kode_lokasi "+filter;		
		var data = this.dbLib.getDataProvider(strSQL,true);		
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
			this.sgn3.setTotalPage(Math.ceil(data.rs.rows.length/20));
