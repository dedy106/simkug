window.app_saku2_transaksi_kopeg_kbitt_fAjuPDD = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fAjuPDD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fAjuPDD";
		...
		this.c_jenis = new saiCB(this.pc2.childPage[0],{bound:[20,12,200,20],caption:"Jenis",items:["MALA","MABA"], readOnly:true,tag:2,change:[this,"doChange"]});
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-33],colCount:6,tag:0,
				colTitle:["NIM","Nama","Bank","No Rekening","Nama Rekening","Nilai"],
				colWidth:[[5,4,3,2,1,0],[100,200,120,120,200,100]],
				readOnly:true,
				colFormat:[[5],[cfNilai]],												
...
window.app_saku2_transaksi_kopeg_kbitt_fAjuPDD.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fAjuPDD.implement({	
	doLoad: function() {
		this.stsSimpan = 0;
		var data = this.dbLib.getDataProvider(
					...
					"where a.no_aju='"+this.cb_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
	...
	isiCBnb: function() {
		this.cb_nb.setSQL("select no_aju, keterangan from it_aju_m where form = 'PDDOUT' and progress in ('A','K','R') and periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ",["no_aju","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);				
	},
	doAfterPaste: function(sender,totalRow){
		try {
			...
			this.inValid = false;
			var strSQL = "select nim from aka_mahasiswa where flag_status ='AKTIF'";			
				...
				if (this.dataNIS.rs.rows.length > 0) {
					for (var j=0;j < this.dataNIS.rs.rows.length;j++){				
						if (this.sg1.cells(0,i).substr(10,30) == this.dataNIS.rs.rows[j].nim) {
							this.sg1.cells(0,i,this.dataNIS.rs.rows[j].nim);
						}						
					}	
					...
					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){								
								sql.add("insert into aka_cd_d (no_bukti,kode_lokasi,periode,nim,kode_akun,dc,nilai,ref1) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.sg1.cells(0,i)+"','"+this.cb_akun.getText()+"','C',"+nilaiToFloat(this.sg1.cells(5,i))+",'-')");
