window.app_saku2_transaksi_kopeg_kbitt_fFiatKBE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fFiatKBE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fFiatKBE";
...
window.app_saku2_transaksi_kopeg_kbitt_fFiatKBE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fFiatKBE.implement({
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText() != "") {										 							 
			this.e_nb.setSQL("select b.no_fiat, b.no_bukti "+
			                 ...
			                 "where (x.posted is null or x.posted = 'F') and c.status in ('APPROVE','REVISI') and a.progress in ('2','F','4') and c.periode='"+this.e_periode.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"'",["b.no_fiat","b.no_bukti"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
		}
		if (sender == this.e_nb && this.e_nb.getText()!="") {					
			var strSQL = "select a.tanggal as tgl,b.catatan,case c.progress when '2' then 'APPROVE' when '4' then 'APPROVE' when 'F' then 'REVISI' end as progress, "+
						 "c.no_aju,convert(varchar,c.tanggal,103) as tanggal,c.modul,bb.kode_pp+' - '+bb.nama as pp,cc.kode_akun+' - '+isnull(cc.nama,'-') as akun,'-' as drk,c.keterangan,c.nilai,convert(varchar,c.tgl_input,103) as tgl_input,c.user_input as nik_user,ee.no_ver,ee.catatan as catatan_ver,convert(varchar,ff.tgl_input,103) as tanggal_ver "+
			...
			var data = this.dbLib.getDataProvider(
						"select a.kode_akun,isnull(b.nama,'-') as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk,aa.nik_panjar+' - '+isnull(e.nama,'-') as nik_panjar "+
						"from it_aju_d a "+
						...
						"where a.jenis<> 'PJ' and a.no_aju = '"+this.e_noaju.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg1.clear();
				for (var i in data.rs.rows){
