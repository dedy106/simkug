window.app_saku2_transaksi_kopeg_kbitt_fPDEdit = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fPDEdit.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fPDEdit";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Edit Pengajuan PD", 0);	
		...
		try {
			...
			else this.cb_pp.setSQL("select a.kode_pp, a.nama from pp a  "+
...
window.app_saku2_transaksi_kopeg_kbitt_fPDEdit.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fPDEdit.implement({	
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{				
					...
					sql.add("update it_aju_m set nilai="+nilaiToFloat(this.e_nilai.getText())+" where no_aju='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");
	...
	doChange:function(sender){				
		if (sender == this.e_periode && this.e_periode.getText()!="") {									
			this.e_nb.setSQL("select no_aju, keterangan from it_aju_m where form = 'SPPD' and modul='-' and progress in ('A','K','R','1','2') and periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ",["no_aju","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);					
		}		
