window.app_saku2_transaksi_kopeg_kbitt_fDokAppv2 = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fDokAppv2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fDokAppv2";
...
window.app_saku2_transaksi_kopeg_kbitt_fDokAppv2.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fDokAppv2.implement({
	...
	doLoad:function(sender){			
		...
		var strSQL = "select 'INPROG' as status,a.no_aju,convert(varchar,g.tgl_input,103) as tanggal,a.modul,b.kode_pp+' - '+b.nama as pp,c.kode_akun+' - '+isnull(c.nama,'-') as akun,a.kode_drk+' - '+isnull(d.nama,'-') as drk,a.keterangan,a.nilai,convert(varchar,a.tgl_input,103) as tgl_input,a.user_input as nik_user,a.tanggal as tgl,a.no_spb,a.no_kas "+
					 ...
					 "where a.progress in ('3','4') and e.no_aju is null and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.tanggal ";			
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
	...
	doCari: function (sender) {
		try {
			...
			var strSQL = "select 'INPROG' as status,a.no_aju,convert(varchar,g.tgl_input,103) as tanggal,a.modul,b.kode_pp+' - '+b.nama as pp,c.kode_akun+' - '+isnull(c.nama,'-') as akun,a.kode_drk+' - '+isnull(d.nama,'-') as drk,a.keterangan,a.nilai,convert(varchar,a.tgl_input,103) as tgl_input,a.user_input as nik_user,a.tanggal as tgl,a.no_spb,a.no_kas "+
						 ...
						 "left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and substring(a.periode,1,4)=d.tahun "+
						 "left join it_dok_app e on a.no_aju=e.no_aju and a.kode_lokasi=e.kode_lokasi "+
						 "where (" + filter_string + ") and a.progress in ('3','4') and e.no_aju is null and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.tanggal ";			
