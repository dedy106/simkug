window.app_saku2_transaksi_kopeg_kbitt_fAjuPtgLokDRK2 = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fAjuPtgLokDRK2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fAjuPtgLokDRK2";
...
window.app_saku2_transaksi_kopeg_kbitt_fAjuPtgLokDRK2.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fAjuPtgLokDRK2.implement({	
	...
	doChange:function(sender){
		try {
			if (sender == this.cb_nik && this.cb_nik.getText()!="") {			
				this.cb_panjar.setSQL("select no_aju,keterangan from it_aju_m where form not in ('PMULTI','PJPROYEK','PMULTIBDD') and progress='3' and nik_panjar ='"+this.cb_nik.getText()+"' and modul = 'PANJAR' and no_ptg='-' and kode_lokasi='"+this.app._lokasi+"'",["no_aju","keterangan"],false,["No Bukti","Keterangan"],"and","Data Panjar",true);
			}
			if (sender == this.cb_panjar && this.cb_panjar.getText()!="") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){										
						...
						this.cb_pp.setSQL("select a.kode_pp, a.nama from pp a inner join karyawan_pp d on a.kode_pp = d.kode_pp and a.kode_lokasi=d.kode_lokasi and d.nik='"+this.cb_nik.getText()+"' "+
