window.app_saku2_transaksi_kopeg_kbitt_fAjuKBLokMultiDRKE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fAjuKBLokMultiDRKE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fAjuKBLokMultiDRKE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan Kas Operasional Multi Transfer DRK: Edit", 0);	
		...
		this.e_saldo = new saiLabelEdit(this,{bound:[20,18,200,20],caption:"Saldo Budget", tag:1, tipeText:ttNilai, text:"0", readOnly:true});
		...
		this.e_nilai = new saiLabelEdit(this,{bound:[720,12,200,20],caption:"Total", tag:1, tipeText:ttNilai, text:"0", readOnly:true});
		
		this.p1 = new portalui_panel(this,{bound:[20,18,900,280],caption:"Data Transfer"});
		this.sg1 = new portalui_saiGrid(this.p1,{bound:[0,20,this.p1.width-5,this.p1.height-45],colCount:6,
				...
				nilaiChange:[this,"doNilaiChange"],colFormat:[[1],[cfNilai]],				
...
window.app_saku2_transaksi_kopeg_kbitt_fAjuKBLokMultiDRKE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fAjuKBLokMultiDRKE.implement({	
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{				
					...
					sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_kpa,no_app,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,form) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.c_jenis.getText()+"','"+this.cb_akun.getText()+"','"+this.cb_pp.getText()+"','"+this.cb_drk.getText()+"','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",getdate(),'"+this.app._userLog+"','-','-','-','-','-','A','-','-','SINGLE')");					
	...
	doChange:function(sender){				
		try{
			if (sender == this.e_periode && this.e_periode.getText()!="") {			
				if (this.app._userStatus == "A") this.e_nb.setSQL("select no_aju, keterangan from it_aju_m where form = 'MULTI' and modul='UMUM' and progress in ('A','K','R') and periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ",["no_aju","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);		
				else this.e_nb.setSQL("select no_aju, keterangan from it_aju_m where kode_pp='"+this.app._kodePP+"' and form = 'MULTI' and modul='UMUM' and progress in ('A','K','R') and periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ",["no_aju","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);		
			}		
			if (sender == this.e_nb && this.e_nb.getText()!="") {
				var data = this.dbLib.getDataProvider(
				           "select a.progress,a.modul,a.tanggal,a.periode,a.keterangan,a.nilai,a.kode_pp,a.kode_akun,a.kode_drk,isnull(x.no_kpa,'-') as no_ver,isnull(x.catatan,'-') as catatan, r.bank,r.no_rek,r.nama_rek "+
