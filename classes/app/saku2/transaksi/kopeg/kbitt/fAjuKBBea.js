window.app_saku2_transaksi_kopeg_kbitt_fAjuKBBea = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fAjuKBBea.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fAjuKBBea";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan Penggaian Beasiswa: Input", 0);	
		
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		this.e_periode = new portalui_saiLabelEdit(this,{bound:[20,14,202,20],caption:"Periode",tag:2,readOnly:true});
		this.l_tgl1 = new portalui_label(this,{bound:[20,11,100,18],caption:"Tanggal", underline:true});
		this.dp_d1 = new portalui_datePicker(this,{bound:[120,11,100,18],selectDate:[this,"doSelectDate"]}); 
		this.c_jenis = new saiCB(this,{bound:[20,22,202,20],caption:"Kode Transaksi",items:["BEA"], readOnly:true,tag:2});
		...
		this.e_nilai = new saiLabelEdit(this,{bound:[20,17,200,20],caption:"Total", tag:1, tipeText:ttNilai, text:"0", readOnly:true});
...
window.app_saku2_transaksi_kopeg_kbitt_fAjuKBBea.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fAjuKBBea.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,user_input) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.c_jenis.getText()+"','"+this.cb_akun.getText()+"','"+this.cb_pp.getText()+"','-','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",getdate(),'"+this.app._userLog+"','-','-','-','0','-','-','"+this.e_user.getText()+"')");
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "simpan" :									
				for (var i = 0; i < this.sg1.getRowCount();i++){ 
					if (this.sg1.rowValid(i)){						
						var strSQL = "select kode_bea from aka_bea_jenis where kode_bea='"+this.sg1.cells(3,i)+"' and kode_lokasi='"+this.app._lokasi+"'"
						var data = this.dbLib.getDataProvider(strSQL,true);
						if (typeof data == "object"){
							var line = data.rs.rows[0];							
							if (line == undefined){						
								system.alert(this,"Data Beasiswa tidak valid.","Kode Beasiswa "+this.sg1.cells(3,i)+" tidak terdaftar.");
								return false;
							}
						}
					}
