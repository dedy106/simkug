window.app_saku2_transaksi_kopeg_kbitt_fAjuBebanME = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fAjuBebanME.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fAjuBebanME";
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,247], childPage:["Item Pertanggungan","Data Anggaran"]});		
...
window.app_saku2_transaksi_kopeg_kbitt_fAjuBebanME.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fAjuBebanME.implement({		
	...
	doChange:function(sender){						
		try {
			if (sender == this.e_periode && this.e_periode.getText()!="") {						
				if (this.app._userStatus == "A") this.e_nb.setSQL("select no_aju, keterangan from it_aju_m where form = 'BMULTI' and modul='UMUM' and progress in ('A','K','R') and periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ",["no_aju","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);		
				else this.e_nb.setSQL("select a.no_aju, a.keterangan from it_aju_m a inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi = b.kode_lokasi and b.nik='"+this.app._userLog+"' "+
									  "where a.form = 'BMULTI' and a.modul='UMUM' and a.progress in ('A','K','R') and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' ",["no_aju","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);		
				//this.e_nb.setSQL("select no_aju, keterangan from it_aju_m where kode_pp ='"+this.app._kodePP+"' and form = 'BMULTI' and modul='UMUM' and progress in ('A','K','R') and periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ",["no_aju","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);		
			}		
			if (sender == this.e_nb && this.e_nb.getText()!="") {
				var data = this.dbLib.getDataProvider("select a.nik_app,a.kode_akun,a.progress,a.modul,a.tanggal,a.periode,a.keterangan,a.nilai,r.bank,r.no_rek,r.nama_rek,isnull(x.no_kpa,'-') as no_ver,isnull(x.catatan,'-') as catatan "+
						   "from it_aju_m a "+
