window.app_saku2_transaksi_kopeg_kbitt_fDokRevisi = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fDokRevisi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fDokRevisi";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Tambahan Dokumen untuk Revisi", 0);	
...
window.app_saku2_transaksi_kopeg_kbitt_fDokRevisi.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fDokRevisi.implement({
	isiCBagenda: function() {
		this.e_noaju.setSQL("select distinct a.no_aju, a.keterangan "+
							"from it_aju_m a "+
							"where a.progress in ('R') and a.no_ref1 in ('maba','mala','camaba') and a.kode_lokasi='"+this.app._lokasi+"' and a.periode >= '202003' ",["a.no_aju","a.keterangan"],false,["No Agenda","Deskripsi"],"and","Data Agenda",true);
	},	
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.progAwal == "R") {
						sql.add("update it_aju_m set progress='A' where no_aju = '"+this.e_noaju.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");
					}
	...
	doChange:function(sender){		
		if (sender==this.e_noaju) {			
			if (this.e_noaju.getText() != "") {
				var strSQL = "select a.progress,a.form,a.nilai,a.kode_pp,a.keterangan,a.tanggal,a.kode_pp+' - '+b.nama as pp,a.kode_akun+' - '+c.nama as akun,a.kode_drk+' - '+isnull(d.nama,'-') as drk "+
							 "from it_aju_m a "+
