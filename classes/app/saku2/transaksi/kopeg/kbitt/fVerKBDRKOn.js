window.app_saku2_transaksi_kopeg_kbitt_fVerKBDRKOn = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fVerKBDRKOn.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fVerKBDRKOn";
...
window.app_saku2_transaksi_kopeg_kbitt_fVerKBDRKOn.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fVerKBDRKOn.implement({
	...
	doLoad:function(sender){				
		var strSQL = "select a.no_aju,convert(varchar,a.tanggal,103) as tanggal,a.modul,b.kode_pp+' - '+b.nama as pp,c.kode_akun+' - '+isnull(c.nama,'-') as akun,a.kode_drk+' - '+isnull(d.nama,'-') as drk,a.keterangan,a.nilai,convert(varchar,a.tgl_input,103) as tgl_input,a.user_input as nik_user, case progress when '0' then 'AJU' when 'R' then 'REVISI' end as status "+
					 ...
					 "where a.periode<='"+this.e_periode.getText()+"' and a.progress in ('0','F') and a.kode_lokasi='"+this.app._lokasi+"' and e.jenis='ONLINE' ";			
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
	...
	doCari:function(sender){				
		...
		var strSQL = "select a.no_aju,convert(varchar,a.tanggal,103) as tanggal,a.modul,b.kode_pp+' - '+b.nama as pp,c.kode_akun+' - '+isnull(c.nama,'-') as akun,a.kode_drk+' - '+isnull(d.nama,'-') as drk,a.keterangan,a.nilai,convert(varchar,a.tgl_input,103) as tgl_input,a.user_input as nik_user, case progress when '0' then 'AJU' when 'R' then 'REVISI' end as status "+
					 ...
					 "left join masakun c on a.kode_akun=c.kode_akun and a.kode_lokasi=c.kode_lokasi "+
					 "left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and substring(a.periode,1,4)=d.tahun "+filter+ " and e.jenis='ONLINE' ";					
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
			this.sgn.setTotalPage(Math.ceil(data.rs.rows.length/20));
