window.app_saku2_transaksi_kopeg_kbitt_fAjuKBBeaE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fAjuKBBeaE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fAjuKBBeaE";
		...
		this.p1 = new portalui_panel(this,{bound:[20,189,900,320],caption:"Data Mahasiswa"});
		this.sg1 = new portalui_saiGrid(this.p1,{bound:[0,20,this.p1.width-5,this.p1.height-50],colCount:4,
				colTitle:["NIM","Nama","Nilai","Kode Beasiswa"],
...
window.app_saku2_transaksi_kopeg_kbitt_fAjuKBBeaE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fAjuKBBeaE.implement({	
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{				
					...
					sql.add("delete from aka_bea_mhs where no_aju = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");										
					...
					sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.c_jenis.getText()+"','"+this.cb_akun.getText()+"','"+this.cb_pp.getText()+"','-','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",getdate(),'"+this.app._userLog+"','-','-','-','0','-','-')");
					
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i)){
							sql.add("insert into aka_bea_mhs(no_aju,kode_lokasi,kode_bea,nim,nama,nilai,periode,no_urut) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(0,i)+"','"+this.sg1.cells(1,i)+"',"+nilaiToFloat(this.sg1.cells(2,i))+",'"+this.e_periode.getText()+"',"+i+")");
						}
					}					
	...
	doChange:function(sender){				
		if (sender == this.e_periode && this.e_periode.getText()!="") {			
			this.e_nb.setSQL("select no_aju, keterangan from it_aju_m where modul='BEA' and progress in ('0','R') and periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ",["no_aju","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);		
		}		
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			var data = this.dbLib.getDataProvider("select a.progress,a.modul,a.tanggal,a.periode,a.keterangan,a.nilai,a.kode_pp,a.kode_akun,a.kode_drk,isnull(x.no_ver,'-') as no_ver,isnull(x.catatan,'-') as catatan "+
					   "from it_aju_m a left join ver_d x on a.no_aju=x.no_bukti and a.kode_lokasi=x.kode_lokasi "+					   
			...
			var strSQL = "select * from aka_bea_mhs where no_aju='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' order by no_urut";
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg1.appendData([line.nim,line.nama,floatToNilai(line.nilai),line.kode_bea]);
				}
			}
			this.sg1.validasi();
