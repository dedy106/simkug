window.app_saku2_transaksi_kopeg_kbitt_fReimIF2 = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kbitt_fReimIF2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kbitt_fReimIF2";
		...
		this.c_status = new saiCB(this,{bound:[20,14,200,20],caption:"Status",items:["REIMBURSE","CLOSE"], readOnly:true,tag:2});
		this.cb_app = new portalui_saiCBBL(this,{bound:[20,15,220,20],caption:"NIK Approve",tag:1,multiSelection:false});         				
		...
		try {
			...
			this.cb_nik.setSQL("select a.nik, a.nama from karyawan a inner join it_if b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi where a.nik='"+this.app._userLog+"' and a.kode_lokasi='"+this.app._lokasi+"' and b.flag_aktif='1'",["a.nik","a.nama"],false,["Kode","Nama"],"and","Data Pemegang",true);
			...
			sql.add("select distinct a.kode_akun, a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
					...
					"union all "+
					"select a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag='060' where a.kode_lokasi='"+this.app._lokasi+"'");				
...
window.app_saku2_transaksi_kopeg_kbitt_fReimIF2.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kbitt_fReimIF2.implement({	
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			case "clear" :
				...
				break;
			case "simpan" :					
				if (this.c_status.getText() == "CLOSE") this.sg1.setTag("9");
				else this.sg1.setTag("0");
	...
	doEllipsClick1: function(sender, col, row){
		try{						
			if (col == 0){
				var str1  = "select x.kode_akun,x.nama from ("+
				...
				/*
						...
						"where b.kode_flag in ('041') and a.kode_lokasi='"+this.app._lokasi+"' "+
						"union all "+
						"select a.kode_akun from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag='060' where a.kode_lokasi='"+this.app._lokasi+"' "+
						") a "
				*/
				...
				this.standarLib.showListData(this, "Daftar Akun",sender,undefined, 
						str1,						
						str2,
						["x.kode_akun","x.nama"],"where",["Kode","Nama"],false);				
			}
			if (col == 5){
			...
			if (col == 7){					
				...
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){
						if (line.status_gar != "1") {							
							var vUnion = " union select '-','-' "; 
						}
						else {
							//cek apakah budget di open by nik..?
							var str = "select flag_aktif as free_budget from tu_free_gar where flag_aktif='1' and nik='"+this.app._userLog+"' and kode_akun='"+sender.cells(0,row)+"' and kode_lokasi='"+this.app._lokasi+"'";
							var data = this.dbLib.getDataProvider(str,true);
							if (typeof data == "object"){
								var line = data.rs.rows[0];							
								if (line != undefined){					
									if (line.free_budget == "1") {
										this.stsGar = "0"; 
										var vUnion = " union select '-','-' "; 
									}
								} 								
