window.app_saku2_transaksi_kopeg_bengkel_fKeluar = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_bengkel_fKeluar.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_bengkel_fKeluar";
...
window.app_saku2_transaksi_kopeg_bengkel_fKeluar.extend(window.portalui_childForm);
window.app_saku2_transaksi_kopeg_bengkel_fKeluar.implement({
							...
							if (this.sg.rowValid(i)){
								if (this.sg.cells(4,i) != "0") {									
									sql.add("insert into fri_keluar_d (no_keluar,kode_lokasi,periode,nu,kode_brg,jumlah,bonus,harga,stok,kode_gudang2) values "+  
										   "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"',"+i+",'"+this.sg.cells(0,i)+"',"+parseNilai(this.sg.cells(5,i))+",0,0,"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(6,i)+"')");
								}
							}
						}						
					}
		...
		try{			
			if (col == 0){		
				this.standarLib.showListData(this, "Daftar Item Barang",sender,undefined, 
											  ...
											  ["a.kode_brg","a.nama","keterangan","a.satuan","stok"],"and",["Kode","Nama","Keterangan","Satuan","Stok"],false);				
			}
			if (col == 6){		
