window.app_saku2_transaksi_kopeg_bengkel_fInspeksiE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_bengkel_fInspeksiE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_bengkel_fInspeksiE";
		...
		try{			
			...
			this.e_nb.setSQL("select no_spk, no_polisi from fri_spk_m where progress in ('0','R') and kode_lokasi='"+this.app._lokasi+"'",["no_spk","no_polisi"],false,["No SPK","No Polisi"],"and","Daftar SPK",true);
...
window.app_saku2_transaksi_kopeg_bengkel_fInspeksiE.extend(window.portalui_childForm);
window.app_saku2_transaksi_kopeg_bengkel_fInspeksiE.implement({
