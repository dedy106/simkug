window.app_saku2_transaksi_kopeg_bengkel_fKeluarE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_bengkel_fKeluarE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_bengkel_fKeluarE";
		...
		this.sg = new portalui_saiGrid(this.p1,{bound:[0,20,895,280],colCount:8,tag:2,
		            colTitle:["Kode","Nama","Kelompok - Jenis - KBM - Tipe","Satuan","Stok","Jumlah","Kode Gudang","Nama Gudang"],
					colWidth:[[0,1,2,3,4,5,6,7],[100,180,290,50,80,80,80,100]],
...
window.app_saku2_transaksi_kopeg_bengkel_fKeluarE.extend(window.portalui_childForm);
window.app_saku2_transaksi_kopeg_bengkel_fKeluarE.implement({
	...
	doChange:function(sender){		
		...
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			...
			var data = this.dbLib.getDataProvider(
						"select a.kode_brg,c.nama, d.nama+' - '+e.nama+' - '+f.nama+' - '+c.tipe as keterangan,c.satuan,a.stok,a.jumlah,a.kode_gudang2,isnull(b.nama,'-') as nama_gudang "+
						"from fri_keluar_d a "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData([line.kode_brg,line.nama,line.keterangan,line.satuan,floatToNilai(line.stok),floatToNilai(line.jumlah),line.kode_gudang2,line.nama_gudang]);
				}
