window.app_saku2_transaksi_kopeg_bengkel_fKbFinalE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_bengkel_fKbFinalE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_bengkel_fKbFinalE";
		...
		this.cb_jual = new saiCBBL(this,{bound:[20,17,220,20],caption:"No SPK", readOnly:true});				
...
window.app_saku2_transaksi_kopeg_bengkel_fKbFinalE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_bengkel_fKbFinalE.implement({
	...
	doChange:function(sender){
		...
		if (sender == this.e_nb && this.e_nb.getText()!= "") {
			var data = this.dbLib.getDataProvider("select a.*,c.no_jual,c.keterangan as ket_jual from kas_m a inner join fri_jualbayar_d b on a.no_kas=b.no_bukti and a.kode_lokasi=b.kode_lokasi "+
					   "      inner join fri_jual_m c on b.no_jual=c.no_jual and c.kode_lokasi=b.kode_lokasi "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){		
					...
					this.cb_jual.setText(line.no_jual,line.ket_jual);
					var strSQL = "select (a.nilai+a.nilai_service-a.nilai_diskon+a.nilai_ppn - isnull(b.bayar,0)) as saldo, '-' as akun_piutang,a.no_dokumen+' - '+a.keterangan as ket "+			             
								 ...
								 "             from fri_jualbayar_d where no_bukti <> '"+this.e_nb.getText()+"' group by no_jual,kode_lokasi"+
								 "           ) b "+
