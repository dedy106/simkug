window.app_saku2_transaksi_kopeg_piutang_fPiuUmumE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_piutang_fPiuUmumE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_piutang_fPiuUmumE";
...
window.app_saku2_transaksi_kopeg_piutang_fPiuUmumE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_piutang_fPiuUmumE.implement({
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!= "") {
			this.e_nb.setSQL("select no_piutang, keterangan from piutang_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
			                 "and no_piutang not in (select distinct no_piutang from piubayar_d)",["no_piutang","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);			
		}		
		if (sender == this.e_nb && this.e_nb.getText()!="") {			
			var strSQL = "select a.tanggal,a.periode,a.no_dokumen,a.keterangan,a.kode_pp,a.akun_piutang,a.nik_app,a.kode_project,b.keterangan as ket_project,a.kode_cust,c.nama as nama_cust,nilai_ppn "+						 
						 "from piutang_m a inner join project b on a.kode_project=b.kode_project and a.kode_lokasi=b.kode_lokasi "+
