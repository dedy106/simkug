window.app_saku2_transaksi_kopeg_piutang_fKbPiuE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_piutang_fKbPiuE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_piutang_fKbPiuE";
...
window.app_saku2_transaksi_kopeg_piutang_fKbPiuE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_piutang_fKbPiuE.implement({
	...
	doChange:function(sender){
		...
		if (sender == this.e_nb && this.e_nb.getText()!= "") {			
			var strSQL = "select (a.nilai+a.nilai_ppn) -  isnull(b.bayar,0) as saldo, a.akun_piutang,a.kode_project+' - '+c.no_dokumen+' - '+c.keterangan as ket, "+
						 ...
						 "from piutang_m a inner join project c on a.kode_project=c.kode_project and a.kode_lokasi=c.kode_lokasi "+
						 ...
						 "left join (select no_piutang,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar from piubayar_d where no_bukti <> '"+this.e_nb.getText()+"' group by no_piutang,kode_lokasi) b "+
