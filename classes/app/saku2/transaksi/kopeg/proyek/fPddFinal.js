window.app_saku2_transaksi_kopeg_proyek_fPddFinal = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_proyek_fPddFinal.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_proyek_fPddFinal";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Finalisasi PDD: Input", 0);	
...
window.app_saku2_transaksi_kopeg_proyek_fPddFinal.extend(window.childForm);
window.app_saku2_transaksi_kopeg_proyek_fPddFinal.implement({
	...
	doChange:function(sender){		
		if (sender==this.cb_pp && this.cb_pp.getText()!="") {			
			this.cb_id.setText("","");
			this.cb_id.setSQL("select no_proyek, keterangan from pr_proyek_m where kode_pp='"+this.cb_pp.getText()+"' and progress ='1' and kode_lokasi='"+this.app._lokasi+"'",["no_proyek","keterangan"],false,["ID Project","Deskripsi"],"and","Data Project",true);									
			//this.cb_id.setSQL("select kode_project, keterangan from project where kode_lokasi='"+this.app._lokasi+"'",["kode_project","keterangan"],false,["Kode","Deskripsi"],"and","Data Project",true);			
		...
		if (sender==this.cb_id && this.cb_id.getText()!="") {
			...
			var strSQL = "select a.no_piutang,a.periode,a.akun_pdd,a.keterangan,a.nilai-isnull(b.pdd,0) as saldo "+						
					"from pr_piutang_m a "+
