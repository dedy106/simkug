window.app_saku2_transaksi_kopeg_proyek_fPytE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_proyek_fPytE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_proyek_fPytE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Finalisasi PYT: Edit", 0);	
		...
		this.e_pdd = new saiLabelEdit(this,{bound:[710,14,210,20],caption:"Total PYT", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		this.e_uraian = new saiLabelEdit(this,{bound:[20,16,450,20],caption:"Uraian", readOnly:true});						
		this.e_total = new saiLabelEdit(this,{bound:[710,16,210,20],caption:"Total Final", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		
		this.pc1 = new pageControl(this,{bound:[20,12,900,240], childPage:["Data PYT","Item Jurnal Pdpt"]});
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:6,tag:0,
		            colTitle:["No Piutang","Periode","Akun PYT","Keterangan","Nilai PYT","Nilai Final"],
					colWidth:[[5,4,3,2,1,0],[100,100,320,80,80,120]],					
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:0,
					...
					colFormat:[[4],[cfNilai]],picklist:[[2],[new portalui_arrayMap({items:["C"]})]],checkItem:true,
		...
		this.e_nilai = new saiLabelEdit(this.sgn,{bound:[660,1,210,20],caption:"Total Jurnal", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		...
		try {
			...
			this.cb_id.setSQL("select kode_project, keterangan from project where kode_lokasi='"+this.app._lokasi+"'",["kode_project","keterangan"],false,["Kode","Deskripsi"],"and","Data Project",true);			
...
window.app_saku2_transaksi_kopeg_proyek_fPytE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_proyek_fPytE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from pr_pdd_m where no_pdd='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from pr_pdd_d where no_pdd='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from pr_pdd_j where no_pdd='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					
					var total = nilaiToFloat(this.e_total.getText());
					sql.add("insert into pr_pdd_m(no_pdd,kode_lokasi,no_dokumen,tanggal,keterangan,no_proyek,kode_cust,kode_curr,kurs,nik_app,kode_pp,nilai,periode,nik_user,tgl_input,posted) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_id.getText()+"','"+this.kodeCust+"','IDR',1,'"+this.app._userLog+"','"+this.cb_pp.getText()+"',"+total+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'F')");
									
					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i) && this.sg1.cells(5,i) != "0"){
								var j = i+1;
								sql.add("insert into pr_pdd_j(no_pdd,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sg1.cells(2,i)+"','"+this.sg1.cells(3,i)+"','D','IDR',1,"+parseNilai(this.sg1.cells(5,i))+","+parseNilai(this.sg1.cells(5,i))+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','PDDPROJECT','PDD','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");								
								sql.add("insert into pr_pdd_d(no_pdd,kode_lokasi,no_piutang,no_proyek,nilai,periode) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.sg1.cells(0,i)+"','"+this.cb_id.getText()+"',"+nilaiToFloat(this.sg1.cells(5,i))+",'"+this.e_periode.getText()+"')");								
							}
						}
					}					
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :																						
				...
				this.sg.validasi();								
				this.sg1.validasi();								
				if (nilaiToFloat(this.e_total.getText()) != nilaiToFloat(this.e_nilai.getText())) {
					system.alert(this,"Transaksi tidak valid.","Total Final harus sama dengan Total Jurnal.");
					return false;						
				}
				if (nilaiToFloat(this.e_pdd.getText()) < nilaiToFloat(this.e_total.getText())) {
	...
	doChange:function(sender){		
		if (sender == this.e_periode && this.e_periode.getText()!="") {			
			this.e_nb.setSQL("select no_pdd, keterangan from pr_pdd_m where posted='F' and periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ",["no_pdd","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);		
		}		
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			var data = this.dbLib.getDataProvider("select no_dokumen,periode,tanggal,keterangan,no_proyek,kode_pp "+
					   "from pr_pdd_m a "+					   
					   "where a.no_pdd='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object"){
			...
			var strSQL = "select a.keterangan,kode_cust "+
			             "from pr_proyek_m a "+
						 "  left join (select no_proyek,kode_lokasi,sum(nilai_pdd) as piutang from pr_piutang_m where no_piutang<>'"+this.e_nb.getText()+"' and no_proyek='"+this.cb_id.getText()+"' and kode_lokasi='"+this.app._lokasi+"' group by no_proyek,kode_lokasi) b on a.no_proyek=b.no_proyek and a.kode_lokasi=b.kode_lokasi "+
			...
			var strSQL = "select a.no_piutang,a.periode,a.akun_pdd,a.keterangan,a.nilai_pdd-isnull(b.pdd,0) as saldo,c.nilai "+						
					"from pr_piutang_m a "+
					"     inner join pr_pdd_d c on a.no_piutang=c.no_piutang and a.kode_lokasi=c.kode_lokasi and c.no_pdd='"+this.e_nb.getText()+"' "+
					"     left join (select no_piutang,kode_lokasi,sum(nilai) as pdd from pr_pdd_d where no_pdd <> '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and no_proyek='"+this.cb_id.getText()+"' group by no_piutang,kode_lokasi) b on a.no_piutang=b.no_piutang and a.kode_lokasi=b.kode_lokasi "+
					"where a.no_proyek = '"+this.cb_id.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_piutang";			
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;				
				this.sg1.clear();
				for (var i in data.rs.rows){
					line = data.rs.rows[i];												
					this.sg1.appendData([line.no_piutang,line.periode,line.akun_pdd,line.keterangan,floatToNilai(line.saldo),floatToNilai(line.nilai)]);					
				}
			...
			var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk "+						
						...
						"where a.jenis='PDPT' and a.no_pdd = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
				for (var i in data.rs.rows){
	...
	doNilaiChange1: function(){
		try{			
			...
			for (var i = 0; i < this.sg1.rows.getLength();i++){
				if (this.sg1.rowValid(i) && this.sg1.cells(5,i) != "" && this.sg1.cells(4,i) != ""){										
					tot += nilaiToFloat(this.sg1.cells(5,i));									
					pdd += nilaiToFloat(this.sg1.cells(4,i));									
				}
			}						
			this.e_total.setText(floatToNilai(tot));		
			this.e_pdd.setText(floatToNilai(pdd));			
		}catch(e)
		{
					...
					case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.cb1.isSelected()) {								
								this.nama_report="server_report_saku2_kopeg_proyek_rptProyekJurnalPyt";
								this.filter2 = " where a.kode_lokasi='"+this.app._lokasi+"' and a.no_pdd='"+this.e_nb.getText()+"' ";
