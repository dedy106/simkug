window.app_saku2_transaksi_kopeg_proyek_fAkruPiu = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_proyek_fAkruPiu.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_proyek_fAkruPiu";
		...
		this.e_nsaldo = new saiLabelEdit(this,{bound:[710,18,210,20],caption:"Saldo  Project", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});								
		this.cb_pp = new saiCBBL(this,{bound:[20,16,200,20],caption:"PP", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});						
		this.e_pdd = new saiLabelEdit(this,{bound:[710,16,210,20],caption:"Nilai PDD", tag:1, tipeText:ttNilai, text:"0",change:[this,"doChange"]});				
		this.cb_akun = new saiCBBL(this,{bound:[20,13,200,20],caption:"Akun Piutang", multiSelection:false, maxLength:10, tag:2});										
		this.e_nilai = new saiLabelEdit(this,{bound:[710,13,210,20],caption:"Nilai Pdpt", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});								
...
window.app_saku2_transaksi_kopeg_proyek_fAkruPiu.extend(window.childForm);
window.app_saku2_transaksi_kopeg_proyek_fAkruPiu.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					var total = nilaiToFloat(this.e_nilai.getText()) + nilaiToFloat(this.e_pdd.getText());
					sql.add("insert into pr_piutang_m(no_piutang,kode_lokasi,no_dokumen,tanggal,keterangan,no_proyek,kode_cust,kode_curr,kurs,nik_app,kode_pp,nilai,periode,nik_user,tgl_input,akun_piutang,posted,nilai_ppn,nilai_pdd,akun_pdd) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_id.getText()+"','"+this.kodeCust+"','IDR',1,'"+this.app._userLog+"','"+this.cb_pp.getText()+"',"+total+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.cb_akun.getText()+"','F',"+parseNilai(this.e_ppn.getText())+","+parseNilai(this.e_pdd.getText())+",'"+this.akunPDD+"')");
					...
					sql.add("insert into pr_piutang_j(no_piutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','D','IDR',1,"+parseNilai(this.e_total.getText())+","+parseNilai(this.e_total.getText())+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','PIUPROJECT','PIU','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
					
					if (nilaiToFloat(this.e_ppn.getText()) != 0) {
						sql.add("insert into pr_piutang_j(no_piutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
								"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',99,'"+this.akunPPN+"','"+this.e_ket.getText()+"','C','IDR',1,"+parseNilai(this.e_ppn.getText())+","+parseNilai(this.e_ppn.getText())+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','PIUPROJECT','HUTPPN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
					}
					sql.add("insert into pr_piutang_j(no_piutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',97,'"+this.akunPDD+"','"+this.e_ket.getText()+"','C','IDR',1,"+parseNilai(this.e_pdd.getText())+","+parseNilai(this.e_pdd.getText())+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','PIUPROJECT','PDD','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','PIUPROJECT','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(6,i))+","+nilai+")");
							}
						}
					}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "simpan" :																						
				...
				*/
				var total = nilaiToFloat(this.e_nilai.getText()) + nilaiToFloat(this.e_pdd.getText());
				if (nilaiToFloat(this.e_nsaldo.getText()) < total) {
	...
	doChange:function(sender){		
		if (sender==this.cb_pp && this.cb_pp.getText()!="") {			
			this.cb_id.setText("","");
			this.cb_id.setSQL("select no_proyek, no_dokumen from pr_proyek_m where kode_pp='"+this.cb_pp.getText()+"' and progress ='1' and kode_lokasi='"+this.app._lokasi+"'",["no_proyek","no_dokumen"],false,["ID Project","No Dokumen"],"and","Data Project",true);									
		...
		if (sender==this.cb_id && this.cb_id.getText()!="") {
			var strSQL = "select a.keterangan,kode_cust,a.nilai-isnull(b.piutang,0) as saldo "+
			             "from pr_proyek_m a "+
						 "  left join (select no_proyek,kode_lokasi,sum(nilai) as piutang from pr_piutang_m where no_proyek='"+this.cb_id.getText()+"' and kode_lokasi='"+this.app._lokasi+"' group by no_proyek,kode_lokasi) b on a.no_proyek=b.no_proyek and a.kode_lokasi=b.kode_lokasi "+
		...
		if ((sender == this.e_ppn || sender == this.e_pdd) && this.e_ppn.getText()!="" && this.e_pdd.getText()!="") {			
			this.e_total.setText(floatToNilai(nilaiToFloat(this.e_nilai.getText()) + nilaiToFloat(this.e_ppn.getText()) + nilaiToFloat(this.e_pdd.getText())));
		}
	},	
	...
	doNilaiChange: function(){
		try{			
			...
			this.e_total.setText(floatToNilai(tot + nilaiToFloat(this.e_pdd.getText()) + nilaiToFloat(this.e_ppn.getText())));
		}catch(e)
		{
