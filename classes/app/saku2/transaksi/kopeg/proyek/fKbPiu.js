window.app_saku2_transaksi_kopeg_proyek_fKbPiu = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_proyek_fKbPiu.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_proyek_fKbPiu";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Piutang Project: Input", 0);	
		...
		this.cb_cust = new saiCBBL(this,{bound:[20,14,220,20],caption:"Customer", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});				
...
window.app_saku2_transaksi_kopeg_proyek_fKbPiu.extend(window.childForm);
window.app_saku2_transaksi_kopeg_proyek_fKbPiu.implement({
	...
	doChange:function(sender){		
		if ((sender == this.e_periode || sender == this.cb_cust) && (this.e_periode.getText()!= "" && this.cb_cust.getText()!= "")) {			
			var strSQL = "select a.no_piutang,a.keterangan,a.kode_lokasi from pr_piutang_m a "+
						 "left join (select no_piutang,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar from pr_piubayar_d group by no_piutang,kode_lokasi) b "+
						 "on a.no_piutang=b.no_piutang and a.kode_lokasi=b.kode_lokasi "+
						 "where a.no_app<>'-' and a.kode_cust='"+this.cb_cust.getText()+"' and a.nilai_pdd+a.nilai_ppn>isnull(b.bayar,0) and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
		...
		if (sender == this.cb_piutang && this.cb_piutang.getText()!= "") {
			var strSQL = "select (a.nilai_pdd+a.nilai_ppn) -  isnull(b.bayar,0) as saldo, a.akun_piutang,a.no_proyek as ket "+			             
