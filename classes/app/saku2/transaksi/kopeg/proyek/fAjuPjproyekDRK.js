window.app_saku2_transaksi_kopeg_proyek_fAjuPjproyekDRK = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_proyek_fAjuPjproyekDRK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_proyek_fAjuPjproyekDRK";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan Panjar Proyek: Input", 0);	
		...
		this.c_jenis = new saiCB(this,{bound:[20,11,202,20],caption:"Kode Transaksi",items:["PANJAR"], readOnly:true,tag:2,visible:false});
		this.e_nb = new portalui_saiLabelEdit(this,{bound:[20,11,200,20],caption:"No Bukti",maxLength:30,readOnly:true,visible:false});						
		this.cb_pp = new portalui_saiCBBL(this,{bound:[20,20,220,20],caption:"Bagian / Unit",tag:2,multiSelection:false,change:[this,"doChange"]}); 
...
window.app_saku2_transaksi_kopeg_proyek_fAjuPjproyekDRK.extend(window.childForm);
window.app_saku2_transaksi_kopeg_proyek_fAjuPjproyekDRK.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,user_input,sts_pajak) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.c_jenis.getText()+"','"+this.akunPJ+"','"+this.cb_pp.getText()+"','-','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",getdate(),'"+this.app._userLog+"','-','-','-','A','"+this.cb_nik.getText()+"','-','"+this.e_user.getText()+"','NON')");					
	...
	doChange:function(sender){
		if (sender==this.cb_pp && this.cb_pp.getText()!="") {			
			this.cb_id.setText("","");
			//this.cb_id.setSQL("select no_proyek, keterangan from pr_proyek_m where flag_aktif in ('1','2') and tahun = '"+this.e_periode.getText().substr(0,4)+"' and kode_pp='"+this.cb_pp.getText()+"' and progress ='1' and kode_lokasi='"+this.app._lokasi+"'",["no_proyek","keterangan"],false,["ID Project","Keterangan"],"and","Data Project",true);
			this.cb_id.setSQL("select no_proyek, keterangan from pr_proyek_m where flag_aktif in ('1','2')  and progress ='1' and kode_lokasi='"+this.app._lokasi+"'",["no_proyek","keterangan"],false,["ID Project","Keterangan"],"and","Data Project",true);
		...
		if ((sender == this.cb_pp ||sender == this.e_periode)  && this.cb_pp.getText() !="" && this.e_periode.getText() !="") {
				this.cb_akun.setSQL("select distinct a.kode_akun, a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
									...
									"where b.kode_flag in ('045') and a.kode_lokasi='"+this.app._lokasi+"'",["kode_akun","nama"],false,["Kode","Nama"],"and","Data Akun",true);													
		}
