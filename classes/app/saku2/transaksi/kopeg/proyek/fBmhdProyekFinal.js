window.app_saku2_transaksi_kopeg_proyek_fBmhdProyekFinal = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_proyek_fBmhdProyekFinal.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_proyek_fBmhdProyekFinal";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan Penyelesaian BMHD Project: Input", 0);	
		...
		this.cb_bmhd = new portalui_saiCBBL(this,{bound:[20,17,220,20],caption:"Bukti BMHD",tag:1,multiSelection:false,change:[this,"doChange"]}); 						
...
window.app_saku2_transaksi_kopeg_proyek_fBmhdProyekFinal.extend(window.childForm);
window.app_saku2_transaksi_kopeg_proyek_fBmhdProyekFinal.implement({	
	...
	doChange:function(sender){
		if (sender==this.cb_pp && this.cb_pp.getText()!="") {			
			...
			this.cb_bmhd.setText("","");				
		}
		if (sender==this.cb_id && this.cb_id.getText()!="") {
			var strSQL = "select a.keterangan from pr_proyek_m a "+						 
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				this.cb_bmhd.setSQL("select no_bmhd, keterangan from pr_bmhd_m where no_proyek ='"+this.cb_id.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_bmhd","keterangan"],false,["No BMHD","Keterangan"],"and","Data BMHD",true);									
			}
		}
		if (sender==this.cb_bmhd && this.cb_bmhd.getText()!="") {
			var strSQL = "select a.kode_akun,a.nilai-isnull(b.selesai,0) as saldo "+
			             "from pr_bmhd_m a "+
						 "  left join (select no_bmhd,kode_lokasi,sum(nilai) as selesai from pr_bmhdfinal_d where no_bmhd='"+this.cb_bmhd.getText()+"' and kode_lokasi='"+this.app._lokasi+"' group by no_bmhd,kode_lokasi) b on a.no_bmhd=b.no_bmhd and a.kode_lokasi=b.kode_lokasi "+
