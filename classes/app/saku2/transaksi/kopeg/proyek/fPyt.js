window.app_saku2_transaksi_kopeg_proyek_fPyt = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_proyek_fPyt.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_proyek_fPyt";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Finalisasi PYT: Input", 0);	
		...
		this.cb_id = new saiCBBL(this,{bound:[20,14,220,20],caption:"ID Project", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});								
...
window.app_saku2_transaksi_kopeg_proyek_fPyt.extend(window.childForm);
window.app_saku2_transaksi_kopeg_proyek_fPyt.implement({
	...
	doChange:function(sender){		
		if (sender==this.cb_pp && this.cb_pp.getText()!="") {			
			this.cb_id.setText("","");
			this.cb_id.setSQL("select no_proyek, keterangan from pr_proyek_m where progress ='1' and kode_lokasi='"+this.app._lokasi+"'",["no_proyek","keterangan"],false,["ID Project","Deskripsi"],"and","Data Project",true);									
		...
		if (sender==this.cb_id && this.cb_id.getText()!="") {
			var strSQL = "select a.no_dokumen,a.kode_cust "+
			             "from pr_proyek_m a "+						 
						 "where a.no_proyek = '"+this.cb_id.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";			
			var data = this.dbLib.getDataProvider(strSQL,true);	
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line = data.rs.rows[0];								
				this.e_uraian.setText(line.no_dokumen);				
				this.kodeCust = line.kode_cust;
			}
			var strSQL = "select a.no_piutang,a.periode,a.akun_pdd,a.keterangan,a.nilai_pdd-isnull(b.pdd,0) as saldo "+						
					"from pr_piutang_m a "+
					"left join (select no_piutang,kode_lokasi,sum(nilai) as pdd from pr_pdd_d where kode_lokasi='"+this.app._lokasi+"' and no_proyek='"+this.cb_id.getText()+"' group by no_piutang,kode_lokasi) b on a.no_piutang=b.no_piutang and a.kode_lokasi=b.kode_lokasi "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];												
					this.sg1.appendData([line.no_piutang,line.periode,line.akun_pdd,line.keterangan,floatToNilai(line.saldo),"0"]);					
				}
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "" ) {				
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"pr_pdd_m","no_pdd",this.app._lokasi+"-PDD"+this.e_periode.getText().substr(2,4)+".","000"));
			this.e_dok.setFocus();
		}
	},	
			...
			if (sender == this.sg) {
				...
				if (col == 7){
					...
					this.standarLib.showListData(this, "Daftar DRK",sender,undefined, 
												  "select kode_drk, nama  from drk where kode_lokasi = '"+this.app._lokasi+"' and tahun='"+this.e_periode.getText().substr(0,4)+"' and ",
