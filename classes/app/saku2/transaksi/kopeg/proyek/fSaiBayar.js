window.app_saku2_transaksi_kopeg_proyek_fSaiBayar = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_proyek_fSaiBayar.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_proyek_fSaiBayar";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Penerimaan Invoice: Input", 0);	
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:11,tag:0,
				colTitle:["Status","No Bill","No Invoice","Keterangan","Akun AR","Kurs","Tagihan","Nilai Bayar","Sisa","Sls Kurs","ID"],
...
window.app_saku2_transaksi_kopeg_proyek_fSaiBayar.extend(window.childForm);
window.app_saku2_transaksi_kopeg_proyek_fSaiBayar.implement({
				...
				try{														
					...
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && this.sg1.cells(0,i) == "BAYAR"){							
							...
							sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
									"('"+this.e_nb.getText()+"','"+this.sg1.cells(1,i)+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(4,i)+"','"+this.sg1.cells(2,i)+' - '+this.e_ket.getText()+"','C',"+nilaiPiuIDR+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBPOLIS','AR','"+this.e_periode.getText()+"','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+",'"+this.app._userLog+"',getdate(),'-',"+nilaiPiu+")");														
							sql.add("insert into sai_billbayar_d(no_bukti,kode_lokasi,no_bill,no_inv,nu,akun_piutang,nilai_kas,nilai_lain,nilai_sk,periode,dc,modul,kode_curr,kurs) values "+
							...
							sql.add("update sai_bill_d set no_kas='"+this.e_nb.getText()+"' where no_inv='"+this.sg1.cells(2,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
						}
					}																																		
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_cust && this.cb_cust.getText()!="") {			
			var strSQL = "select *,nilai+nilai_ppn as total, 1 as kurs from sai_bill_d "+						 						 
						 "where kode_cust = '"+this.cb_cust.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and no_kas='-'";
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg1.appendData(["INPROG",line.no_bill,line.no_inv,line.keterangan,'-',floatToNilai(line.kurs),floatToNilai(line.total),"0","0","0","0"]);
				}
			}
			this.sg1.validasi();
		}
	...
	doNilaiChange1: function(){
		try{
			if (this.stsSGJurnal == "1") this.sg.clear();
			this.nilaiSK = 0;
	...
	doChangeCell1: function(sender, col, row){				
		if (col == 7) {	
			if (this.sg1.cells(7,row) != "0" && this.sg1.cells(6,row)!= "" && this.sg1.cells(7,row)!= "" ) {
				...
				var sls = nilaiToFloat(this.sg1.cells(6,row)) - nilaiToFloat(this.sg1.cells(7,row));
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {							
				this.sg1.cells(0,row,"BAYAR");
				this.sg1.cells(7,row,this.sg1.cells(6,row));				
			}
		} catch(e) {alert(e);}
	},
