window.app_saku2_transaksi_kopeg_gar_fRRAjuTelU = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_gar_fRRAjuTelU.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_gar_fRRAjuTelU";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Reprogramming Approve: Input", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[1],{bound:[1,5,995,305], childPage:["Penerima","Pemberi","Cek Anggaran","File Dok"]});				
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[0,5,this.pc1.width-5,this.pc1.height-33],colCount:9,tag:0,
		...
		this.sgG = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:10,tag:9,
...
window.app_saku2_transaksi_kopeg_gar_fRRAjuTelU.extend(window.childForm);
window.app_saku2_transaksi_kopeg_gar_fRRAjuTelU.implement({
		...
		try{
			if (sender == this.c_bulanT || sender == this.cb_drkT || sender == this.cb_ppT || sender == this.cb_akunT) {				
				...
				if (this.cb_akunT.getText()!="" && this.cb_ppT.getText()!="" && this.cb_drkT.getText()!="" && this.c_bulanT.getText()!="") {
					var data = this.dbLib.getDataProvider("select fn_cekaggBulan('"+this.cb_ppT.getText()+"','"+this.app._lokasi+"','"+this.cb_akunT.getText()+"','"+this.cb_drkT.getText()+"','"+this.e_periode.getText().substr(0,4)+this.c_bulanT.getText()+"','"+this.e_nb.getText()+"') as gar ",true);
					if (typeof data == "object" && data.rs.rows[0] != undefined){
						var line = data.rs.rows[0];
						data = line.gar.split(";");
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider(							 
							 ...
							 "from dok_jenis a left join it_aju_dok b on a.kode_jenis='DOK' and a.kode_lokasi=b.kode_lokasi and b.no_bukti='"+this.e_nb.getText()+"' "+
