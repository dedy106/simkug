window.app_saku2_transaksi_kopeg_gar_fRelBuffer = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_gar_fRelBuffer.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_gar_fRelBuffer";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Release Buffer Anggaran: Input", 0);	
		...
		this.e_total = new saiLabelEdit(this,{bound:[700,17,220,20],caption:"Total Release", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		
		this.pc1 = new pageControl(this,{bound:[20,16,900,300], childPage:["Data Anggaran Buffer"]});
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:0,
		            colTitle:["Kode Akun","Nama Akun","Kode PP","Nama PP","Kode DRK","Nama DRK","Periode","Saldo Buf","Nilai"],
...
window.app_saku2_transaksi_kopeg_gar_fRelBuffer.extend(window.childForm);
window.app_saku2_transaksi_kopeg_gar_fRelBuffer.implement({
	...
	simpan: function(){			
		try{						
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"anggaran_m","no_agg",this.app._lokasi+"-BUF"+this.e_periode.getText().substr(2,4)+".","000"));
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into anggaran_m (no_agg,kode_lokasi,no_dokumen,tanggal,keterangan,tahun,kode_curr,nilai,tgl_input,nik_user,posted,no_del,nik_buat,nik_setuju,jenis) values  "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.e_periode.getText().substr(0,4)+"','IDR',"+parseNilai(this.e_total.getText())+",getdate(),'"+this.app._userLog+"','T','-','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','BUFFER')");
					...
					sql.add("insert into rra_pdrk_m(no_pdrk,kode_lokasi,keterangan,kode_pp,kode_bidang,jenis_agg,tanggal,periode,nik_buat,nik_app1,nik_app2,nik_app3,sts_pdrk,justifikasi, nik_user, tgl_input,progress) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_periode.getText()+"','"+this.app._userLog+"','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.cb_app.getText()+"','BUFFER','-','"+this.app._userLog+"',now(),'0')");
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								...
								sql.add("update anggaran_d set buffer=buffer-"+nilaiToFloat(this.sg.cells(8,i))+" where kode_akun='"+this.sg.cells(0,i)+"' and kode_pp='"+this.sg.cells(2,i)+"' and kode_drk='"+this.sg.cells(4,i)+"' and periode='"+periode+"' and kode_lokasi='"+this.app._lokasi+"'");
								sql.add("insert into anggaran_d (no_agg,kode_lokasi,no_urut,kode_pp,kode_akun,kode_drk,volume,periode,nilai_sat,nilai,dc,satuan,nik_user,tgl_input,modul,buffer) values "+		
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',"+i+",'"+this.sg.cells(2,i)+"','"+this.sg.cells(0,i)+"','"+this.sg.cells(4,i)+"',1,'"+periode+"',"+parseNilai(this.sg.cells(8,i))+","+parseNilai(this.sg.cells(8,i))+",'D','-','"+this.app._userLog+"',getdate(),'BUFFER',0)");
							}
						}
					}					
	...
	doChangeCell: function(sender, col, row){
		...
		if (col == 0 || col == 2 || col == 4 || col == 6) {
			if (this.sg.cells(0,row) !="") {
				var vPeriode = this.e_periode.getText().substr(0,4)+this.sg.cells(6,row);
				var data = this.dbLib.getDataProvider("select buffer from anggaran_d where kode_akun='"+this.sg.cells(0,row)+"' and kode_pp='"+this.sg.cells(2,row)+"' and kode_drk='"+this.sg.cells(4,row)+"' and periode='"+vPeriode+"' and kode_lokasi='"+this.app._lokasi+"'",true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){		
						this.sg.cells(7,row,floatToNilai(line.buffer));			
					} 
				}						
			}
		}		
