window.app_saku2_transaksi_kopeg_logistik_fBA = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_logistik_fBA.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_logistik_fBA";
...
window.app_saku2_transaksi_kopeg_logistik_fBA.extend(window.childForm);
window.app_saku2_transaksi_kopeg_logistik_fBA.implement({	
	...
	doClick:function(sender){
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"log_ba_m","no_ba",this.app._lokasi+"-LOGBA"+this.e_periode.getText().substr(2,4)+".","000"));
		this.e_dok.setFocus();
		setTipeButton(tbSimpan);
	},
	doChange:function(sender){		
		if (sender == this.cb_vendor && this.cb_vendor.getText()!="") {			
			this.cb_po.setSQL("select distinct a.no_po, a.keterangan from log_po_m a inner join log_po_termin b on a.no_po=b.no_po and a.kode_lokasi=b.kode_lokasi and b.no_ba='-' where a.kode_vendor='"+this.cb_vendor.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.periode<='"+this.e_periode.getText()+"'",["a.no_po","a.keterangan"],false,["No PO","Nama"],"and","Data PO",true);
		}
		if (sender == this.cb_po && this.cb_po.getText()!="") {														
			...
			this.cb_termin.setSQL("select top 1 nu, termin from log_po_termin where no_ba='-' and no_po='"+this.cb_po.getText()+"' and kode_lokasi ='"+this.app._lokasi+"'",["nu","termin"],false,["Ke","Termin"],"and","Data Termin",true);
		}		
		if (sender == this.cb_termin && this.cb_termin.getText()!="") {											
			var data = this.dbLib.getDataProvider("select nilai,nilai_ppn,nilai+nilai_ppn as total,jenis from log_po_termin where nu='"+this.cb_termin.getText()+"' and no_po='"+this.cb_po.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){
			...
			if (this.jenisTermin == "FINAL") {
				//mtn di jurnal hutangnya supaya nilai asetnya balance,,jadi finalisasi tingal ppn (jika ada)
