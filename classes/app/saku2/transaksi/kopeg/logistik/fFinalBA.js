window.app_saku2_transaksi_kopeg_logistik_fFinalBA = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_logistik_fFinalBA.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_logistik_fFinalBA";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Finalisasi BAST : Input", 0);	
		...
		this.cb_po = new saiCBBL(this,{bound:[20,16,220,20],caption:"No PO", multiSelection:false, maxLength:10, tag:1, change:[this,"doChange"]});		
		this.e_nilai = new saiLabelEdit(this,{bound:[700,16,220,20],caption:"Nilai Maintenance", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});						
		this.cb_termin = new saiCBBL(this,{bound:[20,15,200,20],caption:"Termin", multiSelection:false, maxLength:10, tag:1, change:[this,"doChange"]});				
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('LOGADK','GARFREE','DOKFREE','PPNM') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku2_transaksi_kopeg_logistik_fFinalBA.extend(window.childForm);
window.app_saku2_transaksi_kopeg_logistik_fFinalBA.implement({	
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into log_ba_m(no_ba,kode_lokasi,tgl_input,nik_user,periode,tanggal,no_dokumen,keterangan,nilai,nilai_ppn,kode_vendor,nik_buat,nik_app,no_po,akun_hutang,nilai_tambah,nilai_mtn,nu,akun_adk,modul,kode_pp) values "+
						   "('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+","+nilaiToFloat(this.e_ppn.getText())+",'"+this.cb_vendor.getText()+"','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.cb_po.getText()+"','"+this.akunHutMtn+"',"+nilaiToFloat(this.e_tambah.getText())+",0,'"+this.cb_termin.getText()+"','-','MTN','"+this.app._kodePP+"')"); 
					...
					if (nilaiToFloat(this.e_ppn.getText()) != 0) {
						sql.add("insert into log_ba_j(no_ba,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
								"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',100,'"+this.akunPPN+"','"+this.e_ket.getText()+"','D','IDR',1,"+nilaiToFloat(this.e_ppn.getText())+","+nilaiToFloat(this.e_ppn.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','LOGFNLBA','PPN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");													
					}		
					
					var nilaiHutSPB = nilaiToFloat(this.e_ppn.getText()) + nilaiToFloat(this.e_tambah.getText());
					sql.add("insert into log_ba_j(no_ba,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',99,'"+this.akunHutMtn+"','"+this.e_ket.getText()+"','C','IDR',1,"+nilaiHutSPB+","+nilaiHutSPB+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','LOGFNLBA','HUTMTN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
					
					if (this.sg2.getRowValidCount() > 0){
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i) && (nilaiToFloat(this.sg2.cells(4,i)) != 0)){
								sql.add("insert into log_ba_j(no_ba,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg2.cells(0,i)+"','"+this.sg2.cells(3,i)+"','"+this.sg2.cells(2,i).toUpperCase()+"','IDR',1,"+nilaiToFloat(this.sg2.cells(4,i))+","+nilaiToFloat(this.sg2.cells(4,i))+",'"+this.sg2.cells(5,i)+"','"+this.sg2.cells(7,i)+"','"+this.app._lokasi+"','LOGFNLBA','TAMBAH','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");																
							}
						}
					}					
					sql.add("update log_po_termin set no_final='"+this.e_nb.getText()+"' where nu='"+this.cb_termin.getText()+"' and no_po='"+this.cb_po.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
		...
		switch (event){
			...
			case "simpan" :	
				...
				if (this.flagDokFree == "1") {				
					var data = this.dbLib.getDataProvider("select no_po from log_ba_m where no_dokumen='"+this.e_dok.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
						if (line != undefined){
	...
	doClick:function(sender){
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"log_ba_m","no_ba",this.app._lokasi+"-FNLBA"+this.e_periode.getText().substr(2,4)+".","000"));
		this.e_dok.setFocus();
		setTipeButton(tbSimpan);
	},
	doChange:function(sender){		
		if (sender == this.cb_vendor && this.cb_vendor.getText()!="") {			
			this.cb_po.setSQL("select distinct a.no_po, a.keterangan from log_po_m a inner join log_po_termin b on a.no_po=b.no_po and a.kode_lokasi=b.kode_lokasi and b.no_ba<>'-' and b.jenis='MTN' and b.no_final='-' where a.kode_vendor='"+this.cb_vendor.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.periode<='"+this.e_periode.getText()+"'",["a.no_po","a.keterangan"],false,["No PO","Nama"],"and","Data PO",true);
		}
		if (sender == this.cb_po && this.cb_po.getText()!="") {														
			...
			this.cb_termin.setSQL("select top 1 nu, termin from log_po_termin where jenis='MTN' and no_final='-' and no_ba<>'-' and no_po='"+this.cb_po.getText()+"' and kode_lokasi ='"+this.app._lokasi+"'",["nu","termin"],false,["Ke","Termin"],"and","Data Termin",true);
		}		
		if (sender == this.cb_termin && this.cb_termin.getText()!="") {											
			var data = this.dbLib.getDataProvider("select nilai,nilai_ppn,nilai+nilai_ppn as total,akun_hutang from log_po_termin where nu='"+this.cb_termin.getText()+"' and no_po='"+this.cb_po.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){
					...
					this.akunHutMtn = line.akun_hutang;
				} 
			}			
		}		
	},		
	    			...
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.cb1.isSelected()) {								
								this.nama_report="server_report_saku2_kopeg_logistik_rptFinal";
								this.filter2 = " where a.kode_lokasi='"+this.app._lokasi+"' and a.no_final='"+this.e_nb.getText()+"' ";
