window.app_saku2_transaksi_kopeg_logistik_fKbSpbE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_logistik_fKbSpbE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_logistik_fKbSpbE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank SPB Logistik: Edit", 0);	
...
window.app_saku2_transaksi_kopeg_logistik_fKbSpbE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_logistik_fKbSpbE.implement({
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!="") {
			this.sg.clear(1); this.sg2.clear(1);
			this.e_nb.setSQL("select no_kas, keterangan from kas_m where no_del='-' and modul = 'KBSPBLOG' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kas","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);			
		}
		if (sender == this.e_nb && this.e_nb.getText()!= "") {			
			var strSQL = "select jenis,periode,tanggal,no_dokumen,keterangan,akun_kb,nik_app "+
						 "from kas_m where no_kas='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";
			...
			var strSQL = "select 'APP' as status,a.no_spb,convert(varchar,a.tanggal,103) as tanggal,convert(varchar,a.due_date,103) as due_date,a.keterangan,a.nik_buat+' - '+c.nama as buat,a.nilai "+
						 ...
						 "where a.modul = 'LOG' and a.no_kas='"+this.e_nb.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"'";						
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU = data;
