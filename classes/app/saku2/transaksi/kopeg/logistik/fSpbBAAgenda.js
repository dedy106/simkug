window.app_saku2_transaksi_kopeg_logistik_fSpbBAAgenda = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_logistik_fSpbBAAgenda.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_logistik_fSpbBAAgenda";
...
window.app_saku2_transaksi_kopeg_logistik_fSpbBAAgenda.extend(window.childForm);
window.app_saku2_transaksi_kopeg_logistik_fSpbBAAgenda.implement({
	...
	simpan: function(){			
		try{						
			...
			//this.e_agenda.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"it_aju_m","no_aju",this.e_periode.getText().substr(2,2)+".","00000"));
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "simpan" :					
				...
				if (jml > 1) {
					system.alert(this,"Transaksi tidak valid.","Pilihan status SPB hanya untuk satu Tagihan.");
					return false;						
				}
	...
	doLoad:function(sender){
		if (this.e_periode.getText()!="") {
			var strSQL = "select 'INPROG' as status,a.no_ba,a.no_po,convert(varchar,b.tanggal,103) as tanggal,b.keterangan,d.kode_pp+' - '+d.nama as pp,b.nik_app+' - '+e.nama as approve,f.kode_vendor+' - '+f.nama as vendor,f.bank,f.cabang,f.no_rek,f.nama_rek,b.nilai+b.nilai_ppn+b.nilai_tambah as nilai,a.akun_hutang,'NONMTN' as jenis "+
						 ...
						 "     inner join (select distinct no_po,kode_dana,kode_lokasi from log_pesan_d where kode_lokasi='"+this.app._lokasi+"') g on a.no_po=g.no_po and a.kode_lokasi=g.kode_lokasi "+
						 "where g.kode_dana <> '2' and a.jenis<>'MTN' and a.no_spb = '-' and b.periode <='"+this.e_periode.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' "+
						 "union "+
						 ...
						 "     inner join vendor f on c.kode_vendor=f.kode_vendor and c.kode_lokasi=f.kode_lokasi "+						 
						 "     inner join (select distinct no_po,kode_dana,kode_lokasi from log_pesan_d where kode_lokasi='"+this.app._lokasi+"') g on a.no_po=g.no_po and a.kode_lokasi=g.kode_lokasi "+
						 "where g.kode_dana <> '2' and a.jenis='MTN' and no_final <> '-' and a.no_spb = '-' and b.periode <='"+this.e_periode.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' "+
						 "order by tanggal";
