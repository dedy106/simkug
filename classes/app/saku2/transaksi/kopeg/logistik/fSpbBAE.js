window.app_saku2_transaksi_kopeg_logistik_fSpbBAE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_logistik_fSpbBAE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_logistik_fSpbBAE";
...
window.app_saku2_transaksi_kopeg_logistik_fSpbBAE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_logistik_fSpbBAE.implement({
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!="") {
			this.sg.clear(1); this.sg2.clear(1);
			this.e_nb.setSQL("select no_spb, keterangan from spb_m where no_kas ='-' and modul = 'LOG' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_spb","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);			
		}
		if (sender == this.e_nb && this.e_nb.getText()!= "") {			
			var strSQL = "select tanggal,periode,due_date,keterangan,nik_buat,nik_tahu,nik_fiat,nik_bdh "+
