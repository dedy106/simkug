window.app_saku2_transaksi_kopeg_barang_fBeli = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_barang_fBeli.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_barang_fBeli";
		...
		try{			
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('PPNM','BRGHUT','BRGINV','BRGDIS') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku2_transaksi_kopeg_barang_fBeli.extend(window.portalui_childForm);
window.app_saku2_transaksi_kopeg_barang_fBeli.implement({	
	...
	simpan: function(){			
		try{					
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"brg_beli_m","no_beli",this.app._lokasi+"-BELI"+this.e_periode.getText().substr(2,4)+".","000"));
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();					
					var totDiskon = nilaiToFloat(this.e_diskon.getText()) + this.diskon;
					
					sql.add("insert into brg_beli_m(no_beli,kode_lokasi,tanggal,no_dokumen,keterangan,kode_gudang,periode,nik_user,tgl_input,nilai,nilai_ppn,nilai_diskon,kode_vendor,no_kas,posted) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.cb_gudang.getText()+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),"+nilaiToFloat(this.e_total.getText())+","+nilaiToFloat(this.e_ppn.getText())+","+nilaiToFloat(this.e_diskon.getText())+",'"+this.cb_vendor.getText()+"','-','F')");					
					sql.add("insert into brg_beli_j(no_beli,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){								
								sql.add("insert into brg_beli_d (no_beli,kode_lokasi,periode,nu,kode_brg,kode_gudang,satuan,jumlah,bonus,harga,diskon) values "+  
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.cb_gudang.getText()+"','"+this.sg.cells(3,i)+"',"+nilaiToFloat(this.sg.cells(6,i))+","+nilaiToFloat(this.sg.cells(7,i))+","+nilaiToFloat(this.sg.cells(4,i))+","+nilaiToFloat(this.sg.cells(5,i))+")");								
							}
						}						
					}					
					sql.add("update a set a.harga=b.harga "+
					        "from brg_m a inner join brg_beli_d b on a.kode_brg=b.kode_brg and a.kode_lokasi=b.kode_lokasi "+
	...
	doChangeCell: function(sender, col, row){
		if (col == 0 && this.sg.cells(0,row)!="") {
			...
			var strSQL = "select merk+' - '+tipe as ket,satuan,harga from brg_m where kode_brg='"+this.sg.cells(0,row)+"' and kode_lokasi='"+this.app._lokasi+"'";
		...
		if (col == 4 || col == 5 || col == 6) {
			if (this.sg.cells(4,row) != "" && this.sg.cells(5,row) != "" && this.sg.cells(6,row) != "") {
				this.sg.cells(8,row,floatToNilai(Math.round((nilaiToFloat(this.sg.cells(4,row))-nilaiToFloat(this.sg.cells(5,row))) * nilaiToFloat(this.sg.cells(6,row)))));
			}
		}
		this.sg.validasi();
	},
		...
		try{
			...
			for (var i = 0; i < this.sg.rows.getLength();i++){
				if (this.sg.rowValid(i) && this.sg.cells(8,i) != ""){
					...
					this.barang += Math.round( nilaiToFloat(this.sg.cells(4,i)) * (nilaiToFloat(this.sg.cells(6,i))+nilaiToFloat(this.sg.cells(7,i))) );
					this.diskon += Math.round(nilaiToFloat(this.sg.cells(5,i)) * nilaiToFloat(this.sg.cells(6,i)));
					this.bonus += Math.round(nilaiToFloat(this.sg.cells(4,i)) * nilaiToFloat(this.sg.cells(7,i)));
				}
			}
