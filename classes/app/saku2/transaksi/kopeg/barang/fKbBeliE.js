window.app_saku2_transaksi_kopeg_barang_fKbBeliE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_barang_fKbBeliE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_barang_fKbBeliE";
...
window.app_saku2_transaksi_kopeg_barang_fKbBeliE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_barang_fKbBeliE.implement({
	...
	doChange:function(sender){		
		if (sender == this.e_periode && this.e_periode.getText()!= "") {
			this.e_nb.setSQL("select no_kas, keterangan from kas_m where no_del='-' and modul = 'KBBELI' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kas","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);			
		}
		if (sender == this.e_nb && this.e_nb.getText()!= "") {			
			var strSQL = "select (a.nilai -  isnull(b.bayar,0)) as saldo, c.kode_akun as akun_hutang,a.no_dokumen+' - '+a.keterangan as ket,a.no_beli, "+
						 ...
						 "left join (select no_beli,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar from brg_belibayar_d where no_bukti <> '"+this.e_nb.getText()+"' group by no_beli,kode_lokasi) b "+
