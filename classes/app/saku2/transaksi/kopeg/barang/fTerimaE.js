window.app_saku2_transaksi_kopeg_barang_fTerimaE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_barang_fTerimaE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_barang_fTerimaE";
...
window.app_saku2_transaksi_kopeg_barang_fTerimaE.extend(window.portalui_childForm);
window.app_saku2_transaksi_kopeg_barang_fTerimaE.implement({
		...
		try{					
			if (this.standarLib.checkEmptyByTag(this, [0,2])){
				try{																			
					...
					sql.add("update brg_kirim_m set no_terima='-' where no_terima='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from brg_terima_m where no_terima='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from brg_terima_d where no_terima='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doChange:function(sender){		
		if (sender == this.e_periode && this.e_periode.getText() != "") {									
			this.e_nb.setSQL("select no_terima, keterangan from brg_terima_m where periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_terima","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
		}	
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			var strSQL = "select e.no_kirim,e.keterangan as ket,a.no_dokumen,a.keterangan,a.tanggal,a.periode,a.nik_app,d.nama as nama_app,a.kode_tujuan,c.nama as nama_tujuan,a.kode_asal,b.nama as nama_asal "+
			             "from brg_terima_m a inner join brg_gudang b on a.kode_asal=b.kode_gudang and a.kode_lokasi=b.kode_lokasi "+						 
						 "				      inner join brg_gudang c on a.kode_tujuan=c.kode_gudang and a.kode_lokasi=c.kode_lokasi "+						 
						 "				      inner join karyawan d on a.nik_app=d.nik and a.kode_lokasi=d.kode_lokasi "+						 
						 "                    inner join brg_kirim_m e on a.no_terima=e.no_terima and a.kode_lokasi=e.kode_lokasi "+
