window.app_saku2_transaksi_kopeg_barang_fKirimE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_barang_fKirimE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_barang_fKirimE";
...
window.app_saku2_transaksi_kopeg_barang_fKirimE.extend(window.portalui_childForm);
window.app_saku2_transaksi_kopeg_barang_fKirimE.implement({
		...
		try{					
			if (this.standarLib.checkEmptyByTag(this, [0,2])){
				try{																			
					...
					var sql = new server_util_arrayList();										
					sql.add("delete from brg_kirim_m where no_kirim='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from brg_kirim_d where no_kirim='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doChange:function(sender){		
		if (sender == this.e_periode && this.e_periode.getText() != "") {									
			this.e_nb.setSQL("select no_kirim, keterangan from brg_kirim_m where no_terima ='-' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kirim","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
		}		
	...
	doChangeCell: function(sender, col, row){
		if (col == 0 && this.sg.cells(0,row)!="" && this.cb_asal.getText()!="") {
			...
			var strSQL = "select a.merk+' - '+a.tipe as ket,a.satuan,b.stok+isnull(c.jumlah,0) as stok  "+
						 ...
						 "             left join brg_kirim_d c on a.kode_brg=c.kode_brg and a.kode_lokasi=c.kode_lokasi and c.no_kirim='"+this.e_nb.getText()+"' "+
