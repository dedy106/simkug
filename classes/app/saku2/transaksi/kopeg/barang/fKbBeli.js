window.app_saku2_transaksi_kopeg_barang_fKbBeli = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_barang_fKbBeli.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_barang_fKbBeli";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Pembelian: Input", 0);	
		...
		this.cb_vendor = new saiCBBL(this,{bound:[20,14,220,20],caption:"Kode Vendor", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});				
		this.cb_beli = new saiCBBL(this,{bound:[20,17,220,20],caption:"No Pembelian", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});				
...
window.app_saku2_transaksi_kopeg_barang_fKbBeli.extend(window.childForm);
window.app_saku2_transaksi_kopeg_barang_fKbBeli.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBBELI','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.cb_app.getText()+"',getdate(),'"+this.app._userLog+"','F','-','"+this.cb_beli.getText()+"','"+this.cb_vendor.getText()+"','-')");
					
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.akunHutang+"','"+this.e_ket.getText()+"','D',"+parseNilai(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBBELI','HUT','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");					
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){																							
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i)+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','-','-','"+this.app._lokasi+"','KBBELI','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");					
							}
						}
					}
					sql.add("insert into brg_belibayar_d(no_bukti,no_beli,kode_lokasi,modul,periode,nik_user,tgl_input,dc,nilai) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_beli.getText()+"','"+this.app._lokasi+"','KBBELI','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'D',"+parseNilai(this.e_nilai.getText())+")");					
	...
	doChange:function(sender){		
		if ((sender == this.e_periode || sender == this.cb_vendor) && (this.e_periode.getText()!= "" && this.cb_vendor.getText()!= "")) {
			var strSQL = "select a.no_beli,a.keterangan,a.kode_lokasi from brg_beli_m a "+
						 "left join (select no_beli,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar from brg_belibayar_d group by no_beli,kode_lokasi) b "+
						 "on a.no_beli=b.no_beli and a.kode_lokasi=b.kode_lokasi where a.nilai>isnull(b.bayar,0) and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
			this.cb_beli.setSQL(strSQL,["a.no_beli","a.keterangan"],false,["No Bukti","Keterangan"],"and","Data Pembelian",true);											
		}
		if (sender == this.cb_beli && this.cb_beli.getText()!= "") {
			var strSQL = "select (a.nilai -  isnull(b.bayar,0)) as saldo, c.kode_akun as akun_hutang,a.no_dokumen+' - '+a.keterangan as ket "+
						 "from brg_beli_m a inner join brg_beli_j c on a.no_beli=c.no_beli and a.kode_lokasi=c.kode_lokasi and c.jenis='HUT' "+
						 "left join (select no_beli,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar from brg_belibayar_d group by no_beli,kode_lokasi) b "+
						 "on a.no_beli=b.no_beli and a.kode_lokasi=b.kode_lokasi "+
						 "where a.no_beli='"+this.cb_beli.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
