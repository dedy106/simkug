window.app_saku2_transaksi_kopeg_barang_fKbJualE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_barang_fKbJualE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_barang_fKbJualE";
...
window.app_saku2_transaksi_kopeg_barang_fKbJualE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_barang_fKbJualE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into brg_jualbayar_d(no_bukti,no_jual,kode_lokasi,modul,periode,nik_user,tgl_input,dc,nilai) values "+
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!= "") {
			this.e_nb.setSQL("select no_kas, keterangan from kas_m where no_del='-' and modul = 'KBJUAL' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kas","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);			
		}
		if (sender == this.e_nb && this.e_nb.getText()!= "") {			
			var strSQL = "select (a.nilai -  isnull(b.bayar,0)) as saldo, c.kode_akun as akun_piutang,a.no_dokumen+' - '+a.keterangan as ket,a.no_jual, "+
						 ...
						 "from brg_jual_m a inner join brg_jual_j c on a.no_jual=c.no_jual and a.kode_lokasi=c.kode_lokasi and c.jenis='PIU' "+
						 ...
						 "left join (select no_jual,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar from brg_jualbayar_d where no_bukti <> '"+this.e_nb.getText()+"' group by no_jual,kode_lokasi) b "+
