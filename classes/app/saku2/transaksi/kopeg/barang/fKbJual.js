window.app_saku2_transaksi_kopeg_barang_fKbJual = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_barang_fKbJual.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_barang_fKbJual";
...
window.app_saku2_transaksi_kopeg_barang_fKbJual.extend(window.childForm);
window.app_saku2_transaksi_kopeg_barang_fKbJual.implement({
	...
	doChange:function(sender){
		if ((sender == this.e_periode || sender == this.cb_cust) && (this.e_periode.getText()!= "" && this.cb_cust.getText()!= "")) {
			var strSQL = "select a.no_jual,a.keterangan,a.kode_lokasi from brg_jual_m a "+
						 "left join (select no_jual,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar from brg_jualbayar_d group by no_jual,kode_lokasi) b "+
						 "on a.no_jual=b.no_jual and a.kode_lokasi=b.kode_lokasi where a.kode_cust='"+this.cb_cust.getText()+"' and a.nilai>isnull(b.bayar,0) and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
