window.app_saku2_transaksi_kopeg_sju_sju2_fKbPiutangReturIE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_sju_sju2_fKbPiutangReturIE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_sju_sju2_fKbPiutangReturIE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Pengembalian Piutang Premi", 0);	
		...
		this.cb_cust = new saiCBBL(this.pc2.childPage[0],{bound:[20,15,220,20],caption:"Tertanggung", multiSelection:false, maxLength:10, tag:0,change:[this,"doChange"]});							
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:12,tag:0,
				colTitle:["No Bill","No Register","No Polis | Sertifikat","Akun AR","Curr","Kurs","Tagihan","N Retur Curr KB","ID","Brokerage","PPN","PPh"],
...
window.app_saku2_transaksi_kopeg_sju_sju2_fKbPiutangReturIE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_sju_sju2_fKbPiutangReturIE.implement({
				...
				try{														
					...
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && nilaiToFloat(this.sg1.cells(7,i)) != 0){
							if (this.c_jcurr.getText() == "CURR") var nilaiPiu = nilaiToFloat(this.sg1.cells(7,i));															
							else var nilaiPiu = Math.round(nilaiToFloat(this.sg1.cells(7,i)) / nilaiToFloat(this.sg1.cells(5,i)) * 100) / 100;								
							...
							sql.add("insert into sju_polisbayar_d(no_bukti,kode_lokasi,no_bill,no_polis,nu,akun_piutang,nilai_kas,nilai_lain,nilai_sk,periode,dc,modul,kode_curr,kurs,no_kashut,kode_vendor,ke)  "+
									"select '"+this.e_nb.getText()+"','"+this.app._lokasi+"',no_bill,no_polis,nu,akun_piutang,"+nilaiPiu+",0,0,'"+this.e_periode.getText()+"','C','RETUR',kode_curr,kurs,'-',kode_vendor,ke "+
					...
					if (this.sg.getRowValidCount() > 0) {
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								...
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+nilaiSlsIDR+",'"+this.sg.cells(5,i)+"','-','-','-','"+this.app._lokasi+"','KBRETUR','SLS','"+this.e_periode.getText()+"','"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+",'"+this.app._userLog+"',getdate(),'-','"+nilaiToFloat(this.sg.cells(4,i))+"')");
							}
						}
					}										
					setTipeButton(tbAllFalse);					
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :														
				...
				if ((nilaiToFloat(this.e_total.getText()) + nilaiToFloat(this.e_kredit.getText())) != nilaiToFloat(this.e_debet.getText())) {
					system.alert(this,"Transaksi tidak valid.","Total Debet tidak sama dengan Nilai KasBank+Total Kredit.");
					return false;						
				}					
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_cust && this.cb_cust.getText()!="" && this.stsSimpan==1) {			
			var strSQL = "select b.no_polis,b.no_dok "+
						 "from (select no_polis,kode_lokasi, "+
						 "	   sum(case dc when 'D' then (nilai_kas+nilai_lain) else -(nilai_kas+nilai_lain) end) as total "+
						 "	   from sju_polisbayar_d where kode_lokasi='"+this.app._lokasi+"' group by no_polis,kode_lokasi) a "+
						 "inner join sju_polis_m b on a.no_polis=b.no_polis and a.kode_lokasi=b.kode_lokasi "+
						 "where b.kode_cust='"+this.cb_cust.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"' and a.total<>0 ";
		...
		if (sender == this.cb_polis && this.cb_polis.getText()!="" && this.stsSimpan==1 && this.c_curr.getText()!="") {			
			var strSQL = "select a.nu,a.kode_lokasi,a.no_bill,a.no_polis,a.keterangan,a.akun_piutang,a.kode_curr,a.kurs,round(a.total,2) as premi,round(a.fee,2) as fee,round(a.ppn,2) as ppn,round(a.pph,2) as pph "+
			             "from ( "+
						 ...
						 "where a.no_polis='"+this.cb_polis.getText()+"' and aa.kode_cust='"+this.cb_cust.getText()+"' and (a.premi-a.diskon+a.p_cost+a.materai)>0  and a.kode_lokasi='"+this.app._lokasi+"' and no_bill <> '-' "+
						 ...
						 "    from sju_polisbayar_d where no_polis='"+this.cb_polis.getText()+"' and kode_lokasi='"+this.app._lokasi+"' group by nu,no_bill,no_polis,kode_lokasi) b "+
						 "on a.nu=b.nu and a.no_bill=b.no_bill and a.no_polis=b.no_polis and a.kode_lokasi=b.kode_lokasi "+
						 "where a.no_polis='"+this.cb_polis.getText()+"' and a.kode_cust='"+this.cb_cust.getText()+"' and b.bayar>0 and a.kode_lokasi ='"+this.app._lokasi+"' order by a.no_polis,a.nu";
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg1.appendData([line.no_bill,line.no_polis,line.keterangan,line.akun_piutang,line.kode_curr,floatToNilai(line.kurs),floatToNilai(line.premi),"0",line.nu,floatToNilai(line.fee),floatToNilai(line.ppn),floatToNilai(line.pph)]);
				}
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						var strSQL = "select distinct b.no_polis,b.no_dok "+
						 ...
						 "	   from sju_polisbayar_d where kode_lokasi='"+this.app._lokasi+"' group by no_polis,kode_lokasi) a "+
						 "inner join sju_polis_m b on a.no_polis=b.no_polis and a.kode_lokasi=b.kode_lokasi "+
						 "inner join sju_polisbayar_d c on b.no_polis=c.no_polis and b.kode_lokasi=c.kode_lokasi "+
						 "where b.kode_cust='"+this.cb_cust.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"' and a.total<>0 and c.no_bukti='"+this.e_nb.getText()+"'";
						this.cb_polis.setSQL(strSQL,["no_polis","no_dok"],false,["No Register","No Polis"],"and","Data Polis",true);			
						this.cb_polis.setText(line.no_del);				
					}
				}
				var strSQL = "select a.nu,a.kode_lokasi,a.no_bill,a.no_polis,a.keterangan,a.akun_piutang,a.kode_curr,a.kurs,round(a.total,2) as premi,round(a.fee,2) as fee,round(a.ppn,2) as ppn,round(a.pph,2) as pph,c.retur "+
							 "from ( "+
							 "select a.nu,aa.kode_cust,a.kode_curr,a.kode_lokasi,a.no_bill,a.no_polis,aa.no_dok+' | '+aa.no_dok2 as keterangan,a.akun_piutang,a.kurs, "+
							 "sum((a.premi-a.diskon+a.p_cost+a.materai)) as total,sum(a.fee) as fee,sum(a.ppn) as ppn,sum(a.pph) as pph  "+ 
							 "from sju_polis_termin a inner join sju_polis_m aa on a.no_polis=aa.no_polis and a.kode_lokasi=aa.kode_lokasi and aa.flag_aktif='1' "+
							 ...
							 "inner join ( "+
							 "    select nu,no_bill,no_polis,kode_lokasi,sum(case dc when 'D' then (nilai_kas+nilai_lain) else -(nilai_kas+nilai_lain) end) as bayar  "+
							 "    from sju_polisbayar_d where no_bukti <> '"+this.e_nb.getText()+"' and no_polis='"+this.cb_polis.getText()+"' and kode_lokasi='"+this.app._lokasi+"' group by nu,no_bill,no_polis,kode_lokasi) b "+
							 "on a.nu=b.nu and a.no_bill=b.no_bill and a.no_polis=b.no_polis and a.kode_lokasi=b.kode_lokasi "+
							 "inner join ( "+
							 "    select nu,no_bill,no_polis,kode_lokasi,nilai_kas+nilai_lain as retur  "+
							 "    from sju_polisbayar_d where no_bukti = '"+this.e_nb.getText()+"' and no_polis='"+this.cb_polis.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ) c "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData([line.no_bill,line.no_polis,line.keterangan,line.akun_piutang,line.kode_curr,floatToNilai(line.kurs),floatToNilai(line.premi),floatToNilai(line.retur),line.nu,floatToNilai(line.fee),floatToNilai(line.ppn),floatToNilai(line.pph)]);
					}
				} else this.sg1.clear(1);
				this.sg1.validasi();	
