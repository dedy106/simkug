window.app_saku2_transaksi_kopeg_sju_fKbHutangIE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_sju_fKbHutangIE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_sju_fKbHutangIE";
		...
		this.e_kompen = new saiLabelEdit(this.pc2.childPage[1],{bound:[790,18,200,20],caption:"Total Kompensasi", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});			
...
window.app_saku2_transaksi_kopeg_sju_fKbHutangIE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_sju_fKbHutangIE.implement({
					...
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && this.sg1.cells(0,i) == "BAYAR"){
							...
							sql.add("insert into sju_hutbayar_d(no_bukti,kode_lokasi,no_bill,no_polis,nu,akun_hutang,nilai_kas,nilai_lain,nilai_sk,periode,dc,modul,kode_curr,kurs,kode_vendor,ke) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.sg1.cells(1,i)+"','"+this.sg1.cells(2,i)+"',"+this.sg1.cells(11,i)+",'"+this.sg1.cells(4,i)+"',"+parseNilai(this.sg1.cells(8,i))+","+parseNilai(this.sg1.cells(9,i))+","+parseNilai(this.sg1.cells(11,i))+",'"+this.e_periode.getText()+"','D','KB','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+",'"+this.cb_vendor.getText()+"','"+this.sg1.cells(11,i)+"')");
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_vendor && this.cb_vendor.getText()!="" && this.c_curr.getText()!="" && this.stsSimpan == 1) {
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg1.appendData(["BAYAR",line.no_bill,line.no_polis,line.keterangan,line.akun_hutang,floatToNilai(line.kurs),floatToNilai(line.nilai_hut),floatToNilai(line.pph),floatToNilai(line.nilai_kas),"0","0",line.nu]);
				}
