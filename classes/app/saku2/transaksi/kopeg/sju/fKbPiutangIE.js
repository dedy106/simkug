window.app_saku2_transaksi_kopeg_sju_fKbPiutangIE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_sju_fKbPiutangIE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_sju_fKbPiutangIE";
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:10,tag:0,
				colTitle:["Status","No Bill","No Register","No Polis","Akun AR","Kurs","Tagihan","Nilai Bayar","Sisa","Sls Kurs"],
				colWidth:[[9,8,7,6,5,4,3,2,1,0],[80,80,100,100,60,60,150,150,80,80]],
...
window.app_saku2_transaksi_kopeg_sju_fKbPiutangIE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_sju_fKbPiutangIE.implement({
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_cust && this.cb_cust.getText()!="" && this.stsSimpan==1) {
			var strSQL = "select a.kode_lokasi,a.no_bill,a.no_polis,a.keterangan,a.akun_piutang,a.kurs,a.total-isnull(b.bayar,0) as saldo "+
			             "from ( "+
						 "select aa.kode_cust,a.kode_curr,a.kode_lokasi,a.no_bill,a.no_polis,aa.no_dok as keterangan,a.akun_piutang,a.kurs, "+
						 ...
						 "group by aa.kode_cust,a.kode_curr,a.kode_lokasi,a.no_bill,a.no_polis,aa.no_dok,a.akun_piutang,a.kurs "+
						 ")a  "+
						 "left join ( "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg1.appendData(["INPROG",line.no_bill,line.no_polis,line.keterangan,line.akun_piutang,floatToNilai(line.kurs),floatToNilai(line.saldo),"0","0","0"]);
				}
	...
	doDoubleClick: function(sender, col , row) {
		try{			
			if (this.sg1.cells(0,row) != "") {							
				...
				this.sg1.cells(7,row,parseFloat(nilai));				
			}
		} catch(e) {alert(e);}
	},
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				this.stsSGJurnal = "0";				
				var strSQL = "select a.kode_lokasi,a.no_bill,a.no_polis,a.keterangan,a.akun_piutang,a.kurs,a.total-isnull(b.bayar,0) as saldo,c.nilai_kas,c.nilai_lain,c.nilai_sk "+
			             "from ( "+
						 "select aa.kode_cust,a.kode_curr,a.kode_lokasi,a.no_bill,a.no_polis,aa.no_dok as keterangan,a.akun_piutang,a.kurs, "+
						 "sum((a.premi-a.diskon+a.p_cost+a.materai)) as total  "+ 
						 "from sju_polis_termin a inner join sju_polis_m aa on a.no_polis=aa.no_polis and a.kode_lokasi=aa.kode_lokasi and aa.flag_aktif='1' "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData(["BAYAR",line.no_bill,line.no_polis,line.keterangan,line.akun_piutang,floatToNilai(line.kurs),floatToNilai(line.saldo),floatToNilai(line.nilai_kas),floatToNilai(line.nilai_lain),floatToNilai(line.nilai_sk)]);
					}
				} else this.sg1.clear(1);				
