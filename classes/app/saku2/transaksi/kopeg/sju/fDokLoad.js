window.app_saku2_transaksi_kopeg_sju_fDokLoad = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_sju_fDokLoad.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_sju_fDokLoad";
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:9,tag:0,
		            colTitle:["No Klaim","Tgl Klaim","Nilai Klaim","No Polis","Keterangan","Penanggung","Tertanggung","Tipe","Status"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[80,200,200,200,200,150,100,70,100]],
					readOnly:true,colFormat:[[2],[cfNilai]],
					dblClick:[this,"doDoubleClick"],autoAppend:false,defaultRow:1});		
		this.sgn = new portalui_sgNavigator(this.pc1.childPage[0],{bound:[1,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:3,grid:this.sg,pager:[this,"doPager"]});
		
		this.e_periode = new portalui_saiLabelEdit(this,{bound:[20,11,202,20],caption:"Periode",tag:2,readOnly:true,visible:false});
		this.e_nb = new portalui_saiLabelEdit(this,{bound:[20,11,202,20],caption:"No Dok",maxLength:30,readOnly:true,visible:false});		
		...
		this.e_tipe = new saiLabelEdit(this.pc1.childPage[1],{bound:[520,12,450,20],caption:"Tipe", readOnly:true});
		this.e_nilai = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,13,200,20],caption:"Nilai Klaim", readOnly:true, tipeText:ttNilai, text:"0"});		
		...
		this.e_memo = new saiMemo(this.pc1.childPage[1],{bound:[20,20,450,80],caption:"Uraian Klaim",tag:0});
...
window.app_saku2_transaksi_kopeg_sju_fDokLoad.extend(window.childForm);
window.app_saku2_transaksi_kopeg_sju_fDokLoad.implement({
		...
		try{		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into sju_dok_m(no_dok,tanggal,kode_lokasi,periode,nik_user,tgl_input,host,ip,catatan,no_klaim) values "+
					...
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i)){
							ix++;
							sql.add("insert into sju_dok_d(no_dok,kode_lokasi,kode_ref,no_file,nu,nik_user,tgl_input,ket_dok) values "+
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg.cells(0,row) != "") {			
				...
				var data = this.dbLib.getDataProvider("select kode_tipe from sju_quo_m where no_polis='"+this.e_nopolis.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line = data.rs.rows[0];							
					this.kodeCOB = line.kode_tipe;
				}

				var data = this.dbLib.getDataProvider("select a.kode_ref,a.nama,isnull(b.no_dok,'-') as no_dok,isnull(convert(varchar,b.tanggal,103),'-') as tanggal,isnull(b.ket_dok,'-') as keterangan,isnull(b.no_file,'-') as no_file "+  
				           "from sju_ref a left join (select x.no_klaim,x.tanggal,y.* from sju_dok_m x inner join sju_dok_d y on x.no_dok=y.no_dok and x.kode_lokasi=y.kode_lokasi  where x.kode_lokasi='"+this.app._lokasi+"') b on a.kode_ref=b.kode_ref and a.kode_lokasi=b.kode_lokasi "+
	...
	doLoad:function(sender){					
		var strSQL = "select no_klaim,convert(varchar,a.tanggal,103) as tanggal,a.nilai,a.no_polis,a.progress,a.keterangan,c.kode_cust +'-'+c.nama as cust, f.kode_vendor +'-'+d.nama as vendor,"+
					"d.kode_tipe+'-'+d.nama as tipe "+					 					 					 
					"from sju_klaim a "+
					...
					"inner join sju_polis_vendor e on b.no_polis = e.no_polis and b.kode_lokasi=e.kode_lokasi "+
					"inner join sju_vendor f on e.kode_vendor = f.kode_vendor and e.kode_lokasi=f.kode_lokasi "+					 
		             "where a.periode<='"+this.e_periode.getText()+"' and a.progress not in ('FINAL') and a.kode_lokasi='"+this.app._lokasi+"' ";
	...
	doCari:function(sender){				
		...
		var strSQL = "select no_klaim,convert(varchar,a.tanggal,103) as tanggal"+
	...
	*/
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData([line.no_klaim,line.tanggal,floatToNilai(line.nilai),line.no_polis,line.keterangan,line.vendor,line.cust,line.tipe,line.progress]);  
		}
		...
		try{			
			if (sender == this.sgUpld) {
				if (col == 0){
					...
					"select kode_ref, nama from sju_ref where kode_lokasi = '"+this.app._lokasi+"' and kode_tipe='"+this.kodeCOB+"'",
					"select count(kode_ref) from sju_ref where kode_lokasi = '"+this.app._lokasi+"' and kode_tipe='"+this.kodeCOB+"'",
