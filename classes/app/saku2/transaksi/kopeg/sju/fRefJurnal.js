window.app_saku2_transaksi_kopeg_sju_fRefJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_sju_fRefJurnal.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_sju_fRefJurnal";
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,10,this.pc1.width-5,this.pc1.height-135],colCount:13,tag:0,
		            colTitle:["Kode Akun","Nama Akun","DC","Keterangan","Nilai","Kode PP","Nama PP","Penanggung","Nama","Tertanggung","Nama","COB","Nama"],
...
window.app_saku2_transaksi_kopeg_sju_fRefJurnal.extend(window.childForm);
window.app_saku2_transaksi_kopeg_sju_fRefJurnal.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == "1") {
						this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"sju_jurnalref_m","no_bukti",this.app._lokasi+"-REF.","000"));
					}
					else {
						sql.add("delete from sju_jurnalref_m where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from sju_jurnalref_j where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}					
					sql.add("insert into sju_jurnalref_m (no_bukti,kode_lokasi,keterangan,kode_curr,modul) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_ket.getText()+"','"+this.c_curr.getText()+"','"+this.c_modul.getText()+"')");					
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){								
 								sql.add("insert into sju_jurnalref_j(no_bukti,no_urut,kode_akun,keterangan,dc,nilai,kode_lokasi,kode_pp,kode_drk,kode_cf,kode_cust,kode_vendor,kode_tipe) values "+
										"('"+this.e_nb.getText()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg.cells(4,i))+",'"+this.cb_lokasi.getText()+"','"+this.sg.cells(5,i)+"','-','-','"+this.sg.cells(9,i)+"','"+this.sg.cells(7,i)+"','"+this.sg.cells(11,i)+"')");
							}
						}
					}					
					setTipeButton(tbAllFalse);					
	...
	doChange:function(sender){
		if ((sender == this.cb_lokasi || sender == this.c_modul) && this.cb_lokasi.getText()!="" && this.c_modul.getText()!="") {
			...
			sql.add("select kode_vendor,nama from sju_vendor where kode_lokasi = '"+this.cb_lokasi.getText()+"' union select '-','-' ");
			sql.add("select kode_cust,nama from sju_cust where kode_lokasi = '"+this.cb_lokasi.getText()+"' union select '-','-' ");
			sql.add("select kode_tipe,nama from sju_tipe where kode_lokasi = '"+this.cb_lokasi.getText()+"' union select '-','-' ");
	...
	doLoad:function(sender){						
		if (this.cb_lokasi.getText()!="") {
			var strSQL = "select * from sju_jurnalref_m where kode_lokasi='"+this.cb_lokasi.getText()+"' ";		
	...
	doCari:function(sender){								
		...
		var strSQL = "select * from sju_jurnalref_m where kode_lokasi='"+this.cb_lokasi.getText()+"' "+filter;
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
			this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length/20));
