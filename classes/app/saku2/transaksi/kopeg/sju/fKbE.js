window.app_saku2_transaksi_kopeg_sju_fKbE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_sju_fKbE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_sju_fKbE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Produksi: Edit", 0);	
		...
		this.pc1 = new pageControl(this,{bound:[20,12,900,320], childPage:["Data Item Jurnal"]});
		...
		try {
			...
			sql.add("select kode_cust,nama from sju_cust where kode_lokasi = '"+this.app._lokasi+"'");
			sql.add("select kode_tipe,nama from sju_tipe where kode_lokasi = '"+this.app._lokasi+"'");
...
window.app_saku2_transaksi_kopeg_sju_fKbE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_sju_fKbE.implement({
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								...
								sql.add("insert into sju_kas_j(no_kas,kode_lokasi,no_urut,kode_vendor,kode_cust,kode_tipe) values "+
										"	('"+this.e_nb.getText()+"','"+this.app._lokasi+"',"+i+",'"+this.sg.cells(7,i)+"','"+this.sg.cells(9,i)+"','"+this.sg.cells(11,i)+"')");
							}
						}
					}																				
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!="") {
			this.e_nb.setSQL("select no_kas, keterangan from kas_m where modul = 'KBASUR' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kas","keterangan"],false,["No KB","Deskripsi"],"and","Daftar Bukti",true);
		}
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			...
			var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp, "+
						"aa.kode_cust,aa.kode_tipe,aa.kode_vendor,isnull(e.nama ,'-') as nama_cust,isnull(f.nama,'-') as nama_vendor,isnull(g.nama,'-') as nama_tipe "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData([line.kode_akun,line.nama_akun,line.dc,line.keterangan,floatToNilai(line.nilai),line.kode_pp,line.nama_pp,line.kode_cust,line.nama_cust,line.kode_vendor,line.nama_vendor,line.kode_proyek,line.nama_tipe]);
				}
