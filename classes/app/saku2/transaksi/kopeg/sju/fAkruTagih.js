window.app_saku2_transaksi_kopeg_sju_fAkruTagih = function(owner)
{
	if (owner)
	{		
		window.app_saku2_transaksi_kopeg_sju_fAkruTagih.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_sju_fAkruTagih";
		...
		this.bJurnal = new button(this,{bound:[708,17,80,18],caption:"Tampil",click:[this,"doJurnal"]});			
		
		this.pc1 = new pageControl(this,{bound:[20,20,990,350], childPage:["Data Kurs","Jurnal Akru","Data Invoice"]});		
		...
		this.sg = new saiGrid(this.pc1.childPage[2],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:14,tag:0,
...
window.app_saku2_transaksi_kopeg_sju_fAkruTagih.extend(window.childForm);
window.app_saku2_transaksi_kopeg_sju_fAkruTagih.implement({	
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{				
					...
					sql.add("insert into sju_bill_d (no_polis,kode_lokasi,nu,keterangan,kode_curr,premi,fee,diskon,p_cost,tipe,tgl_bill,no_bill,kurs,akun_piutang,akun_hutang,ppn,pph,due_date,materai,no_tagih,dc) "+
							"select no_polis,kode_lokasi,nu,keterangan,kode_curr,premi,fee,diskon,p_cost,tipe,tgl_bill,no_bill,kurs,akun_piutang,akun_hutang,ppn,pph,due_date,materai,no_tagih,'D' "+
		...
		try {			
			if (this.e_periode.getText() != "") {
				...
				var strSQL = "select a.no_polis,b.no_dok,convert(varchar,a.tgl_bill,103) as tgl_bill,d.kode_pp+'-'+d.nama as pp,e.kode_cust+'-'+e.nama as cust,f.kode_vendor+'-'+f.nama as vendor, "+
							 ...
							 "where a.kode_lokasi='"+this.app._lokasi+"' and a.no_tagih<>'-' and a.no_bill='-' and a.tgl_bill between '"+this.e_periode.getText().substr(0,4)+"-"+this.e_periode.getText().substr(4,2)+"-01' and (dateadd(s,-1,dateadd(mm, datediff(m,0,'"+this.dp_d1.getDateString()+"')+1,0)))";
				...
				var strSQL = "select a.kode_akun,b.nama as nama_akun,a.dc,a.nilai,a.kode_pp,c.nama as nama_pp, a.keterangan,a.kode_cust,a.nama_cust,a.kode_vendor,a.nama_vendor "+
							 "from "+
							 "( "+
							 "select 1 as nu,a.kode_lokasi,d.akun_piutang as kode_akun,'D' as dc,b.kode_pp, f.kode_cust,f.nama as nama_cust,e.kode_vendor,e.nama as nama_vendor, "+
							 ...
							 "inner join sju_curr_tmp c on b.kode_curr=c.kode_curr and b.kode_lokasi=c.kode_lokasi and c.nik_user='"+this.app._userLog+"' "+
							 ...
							 "where a.kode_lokasi='"+this.app._lokasi+"' and a.no_tagih<>'-' and a.no_bill='-' and a.tgl_bill between '"+this.e_periode.getText().substr(0,4)+"-"+this.e_periode.getText().substr(4,2)+"-01' and (dateadd(s,-1,dateadd(mm, datediff(m,0,'"+this.dp_d1.getDateString()+"')+1,0))) "+
							 "group by a.kode_lokasi,b.kode_pp,d.akun_piutang,f.nama,aa.no_dok, f.kode_cust,e.kode_vendor,e.nama "+
							 "union all "+
							 							 
							 "select 2 as nu,a.kode_lokasi,d.akun_fee as kode_akun,'C' as dc,b.kode_pp, f.kode_cust,f.nama as nama_cust,e.kode_vendor,e.nama as nama_vendor, "+
							 ...
							 "inner join sju_vendor e on b.kode_vendor=e.kode_vendor and b.kode_lokasi=e.kode_lokasi "+
							 "where a.kode_lokasi='"+this.app._lokasi+"' and a.no_tagih<>'-' and a.no_bill='-' and a.tgl_bill between '"+this.e_periode.getText().substr(0,4)+"-"+this.e_periode.getText().substr(4,2)+"-01' and (dateadd(s,-1,dateadd(mm, datediff(m,0,'"+this.dp_d1.getDateString()+"')+1,0))) "+
							 "group by a.kode_lokasi,b.kode_pp,d.akun_fee,f.nama,aa.no_dok, f.kode_cust,e.kode_vendor, e.nama "+						 							 
							 "union all "+
							 
							 "select 3 as nu, a.kode_lokasi,'"+this.akunPPN+"' as kode_akun,'C' as dc,b.kode_pp, f.kode_cust,f.nama as nama_cust,e.kode_vendor,e.nama as nama_vendor, "+						 
							 ...
							 "inner join sju_vendor e on b.kode_vendor=e.kode_vendor and b.kode_lokasi=e.kode_lokasi "+
							 "where a.kode_lokasi='"+this.app._lokasi+"' and a.no_tagih<>'-' and a.no_bill='-' and a.tgl_bill between '"+this.e_periode.getText().substr(0,4)+"-"+this.e_periode.getText().substr(4,2)+"-01' and (dateadd(s,-1,dateadd(mm, datediff(m,0,'"+this.dp_d1.getDateString()+"')+1,0))) "+ 
							 "group by a.kode_lokasi,b.kode_pp,f.nama,aa.no_dok,f.kode_cust,e.kode_vendor,e.nama "+							 
							 "union all "+						 
							 
							 "select 4 as nu,a.kode_lokasi,e.akun_hutang as kode_akun,'C' as dc,b.kode_pp, f.kode_cust,f.nama as nama_cust,e.kode_vendor,e.nama as nama_vendor, "+						 
							 ...
							 "inner join sju_cust f on aa.kode_cust=f.kode_cust and aa.kode_lokasi=f.kode_lokasi "+
							 "where a.kode_lokasi='"+this.app._lokasi+"' and a.no_tagih<>'-' and a.no_bill='-' and a.tgl_bill between '"+this.e_periode.getText().substr(0,4)+"-"+this.e_periode.getText().substr(4,2)+"-01' and (dateadd(s,-1,dateadd(mm, datediff(m,0,'"+this.dp_d1.getDateString()+"')+1,0))) "+
							 "group by a.kode_lokasi,b.kode_pp,e.akun_hutang,e.nama,aa.no_dok,f.kode_cust,f.nama,e.kode_vendor,e.nama "+
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU.rs.rows[i];													
			this.sg.appendData([line.no_polis,line.no_dok,line.tgl_bill,line.pp,line.vendor,line.cust,line.kode_curr,floatToNilai(line.premi),floatToNilai(line.fee),floatToNilai(line.diskon),floatToNilai(line.p_cost),floatToNilai(line.ppn),floatToNilai(line.materai),floatToNilai(line.total)]);
		}
