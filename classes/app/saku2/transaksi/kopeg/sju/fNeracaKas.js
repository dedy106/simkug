window.app_saku2_transaksi_kopeg_sju_fNeracaKas = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_sju_fNeracaKas.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku2_transaksi_kopeg_sju_fNeracaKas";
		...
		this.e0 = new portalui_saiCB(this,{bound:[20,12,200,20], caption:"Tipe Neraca", items:["CF"]});						
...
window.app_saku2_transaksi_kopeg_sju_fNeracaKas.extend(window.portalui_childForm);
window.app_saku2_transaksi_kopeg_sju_fNeracaKas.implement({
		...
		{ 
		 	if (sender == this.relBtn)
			{
				...
				if (this.selectedItem.data.get("tipe").toLowerCase() == "posting"){
					uses("app_saku2_transaksi_kopeg_sju_fRelakunKas",true);
					if (this.relakun != undefined)
						this.relakun.free();
					this.relakun = new app_saku2_transaksi_kopeg_sju_fRelakunKas(this.app);
