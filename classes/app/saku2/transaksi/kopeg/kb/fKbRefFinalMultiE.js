window.app_saku2_transaksi_kopeg_kb_fKbRefFinalMultiE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_kb_fKbRefFinalMultiE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_kb_fKbRefFinalMultiE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Finalisasi Ref Multi: Edit", 0);	
		...
		this.c_jenis = new saiCB(this,{bound:[20,22,202,20],caption:"Jenis",items:["KK","BK","KM","BM"], readOnly:true,tag:2});		
		...
		this.sg3 = new saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:8,tag:0,
		            colTitle:["No Referensi","Deskripsi","Kode Akun","Nama Akun","DC","Nilai","Sisa","Adjustment"],
					colWidth:[[7,6,5,4,3,2,1,0],[80,80,80,60,150,80,220,100]],					
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:11,tag:0,
...
window.app_saku2_transaksi_kopeg_kb_fKbRefFinalMultiE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_kb_fKbRefFinalMultiE.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','-','KBADJ','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"',getdate(),'"+this.app._userLog+"','F','-','-','MULTI','-')");
					
					if (this.sg3.getRowValidCount() > 0){
						for (var i=0;i < this.sg3.getRowCount();i++){
							if (this.sg3.rowValid(i)){
								if (this.sg3.cells(4,i) == "D") var dcRef = "C"; else var dcRef = "D";										
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
										"	('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.sg3.cells(2,i)+"','"+this.e_ket.getText()+"','"+dcRef+"',"+parseNilai(this.sg3.cells(7,i))+",'"+this.app._kodePP+"','-','-','"+this.sg3.cells(0,i)+"','"+this.app._lokasi+"','KBADJ','AKUNREF','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");					
							}
						}
					}
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
										"	('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','"+this.sg.cells(9,i)+"','MULTI','"+this.app._lokasi+"','KBADJ','UMUM','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");								
							}
						}
					}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :	
				...
				for (var i=0;i < this.sg3.getRowCount();i++){
					if (this.sg3.rowValid(i)){
						if (nilaiToFloat(this.sg3.cells(6,i)) < nilaiToFloat(this.sg3.cells(7,i))) {
							system.alert(this,"Transaksi tidak valid.","Adjustment melebihi Sisa untuk baris ["+k+"]");
							return false;
						}						
						for (var j=i;j < this.sg3.getRowCount();j++){
							if (this.sg3.cells(0,j) == this.sg3.cells(0,i) && (i != j)) {
							    var k = i+1;
								system.alert(this,"Transaksi tidak valid.","Duplikasi No Referensi untuk baris ["+k+"]");
								return false;
							}
						}
					}
	...
	doChange:function(sender){								
		...
		if (sender == this.e_nb && this.e_nb.getText()!="") {							   
			var strSQL = "select a.no_dokumen,convert(varchar,a.tanggal,103) as tanggal,a.periode,a.keterangan,a.jenis,a.nik_buat,b.nama as nama_buat,a.nik_app,c.nama as nama_app, "+
					   ...
					   "             inner join kas_m d on g.ref1=d.no_kas and g.kode_lokasi=d.kode_lokasi "+
			           ...
			           "			 inner join karyawan c on a.nik_app=c.nik and a.kode_lokasi=c.kode_lokasi "+
					   "             inner join kas_j g on a.no_kas=g.no_kas and a.kode_lokasi=g.kode_lokasi and g.jenis='AKUNREF' "+
					   "             inner join ju_m d on g.ref1=d.no_ju and g.kode_lokasi=d.kode_lokasi "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];												
					this.sg3.appendData([line.no_ref,line.ket_ref,line.kode_akun,line.nama_akun,line.dc,floatToNilai(line.nilai),floatToNilai(line.sisa),floatToNilai(line.pakai)]); 
				}
			...
			var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk,a.kode_cf,isnull(e.nama,'-') as nama_cf "+
		...
		try{
			...
			for (var i = 0; i < this.sg3.rows.getLength();i++){
				if (this.sg3.rowValid(i) && this.sg3.cells(7,i) != ""){
					if (this.sg3.cells(4,i).toUpperCase() == "D") totC += nilaiToFloat(this.sg3.cells(7,i));
					if (this.sg3.cells(4,i).toUpperCase() == "C") totD += nilaiToFloat(this.sg3.cells(7,i));
				}
			}			
	...
	clearLayar : function(){
		try {
			this.standarLib.clearByTag(this, new Array("0","1"),this.e_nb);
			this.sg3.clear(1); this.sg.clear(1); this.sg2.clear(1); this.sgUpld.clear(1);
			this.pc1.setActivePage(this.pc1.childPage[0]);
			...
			if (sender == this.sg3) {
				if (col == 0){
					var strSQL = "select a.no_kas,a.keterangan "+
								 "from kas_m a "+
								 "     inner join kas_j c on a.no_kas=c.no_kas and a.kode_lokasi=c.kode_lokasi and c.jenis='AKUNREF' "+
								 "     left join ( "+
								 "			select ref1,kode_akun,kode_lokasi,sum(nilai) as pakai "+
								 "          from kas_j where modul='KBADJ' and jenis='AKUNREF' and kode_lokasi='"+this.app._lokasi+"' "+
								 "          group by ref1,kode_akun,kode_lokasi "+
								 "                ) b on c.no_kas=b.ref1 and c.kode_lokasi=b.kode_lokasi and c.kode_akun=b.kode_akun "+						 
								 "     left join ( "+
								 "			select no_ref,kode_akun,kode_lokasi,sum(nilai) as pakai "+
								 "          from ju_j where modul='JUADJ' and jenis='AKUNREF' and kode_lokasi='"+this.app._lokasi+"' "+
								 "          group by no_ref,kode_akun,kode_lokasi "+
								 "                ) d on c.no_kas=d.no_ref and c.kode_lokasi=d.kode_lokasi and c.kode_akun=d.kode_akun "+						 
								 "where c.nilai > (isnull(b.pakai,0)+isnull(d.pakai,0)) and a.modul = 'KBREF' and a.kode_lokasi ='"+this.app._lokasi+"' "+
								 "union "+
								 "select a.no_ju,a.keterangan "+
								 "from ju_m a "+
								 "     inner join ju_j c on a.no_ju=c.no_ju and a.kode_lokasi=c.kode_lokasi and c.jenis='AKUNREF' "+
								 "     left join ( "+
								 "			select no_ref,kode_akun,kode_lokasi,sum(nilai) as pakai "+
								 "          from ju_j where modul='JUADJ' and jenis='AKUNREF' and kode_lokasi='"+this.app._lokasi+"' "+
								 "          group by no_ref,kode_akun,kode_lokasi "+
								 "                ) b on c.no_ju=b.no_ref and c.kode_lokasi=b.kode_lokasi and c.kode_akun=b.kode_akun "+
								 "     left join ( "+
								 "			select ref1,kode_akun,kode_lokasi,sum(nilai) as pakai "+
								 "          from kas_j where modul='KBADJ' and jenis='AKUNREF' and kode_lokasi='"+this.app._lokasi+"' "+
								 "          group by ref1,kode_akun,kode_lokasi "+
								 "                ) d on c.no_ju=d.ref1 and c.kode_lokasi=d.kode_lokasi and c.kode_akun=d.kode_akun "+						 
								 "where c.nilai > isnull(b.pakai,0)+isnull(d.pakai,0) and a.modul = 'JUREF' and a.kode_lokasi ='"+this.app._lokasi+"' ";
					
					var strSQLCount = "select count(zz.no_kas) from ("+
								 "select a.no_kas "+
								 "from kas_m a "+
								 "     inner join kas_j c on a.no_kas=c.no_kas and a.kode_lokasi=c.kode_lokasi and c.jenis='AKUNREF' "+
								 "     left join ( "+
								 ...
								 "where c.nilai > isnull(b.pakai,0)+isnull(d.pakai,0) and a.modul = 'JUREF' and a.kode_lokasi ='"+this.app._lokasi+"' "+
								 ") zz";								 					
					this.standarLib.showListData(this, "Daftar Referensi",sender,undefined,strSQL,strSQLCount,["a.no_kas","a.keterangan"],"and",["No Ref","Keterangan"],false);				
				}				
			}
		}catch(e){
			systemAPI.alert(e);
	...
	doChangeCell3: function(sender, col, row){
		if (col == 7 && this.sg3.cells(7,row) != "") this.sg3.validasi();		
	    if (col == 0) {
			var strSQL = "select c.kode_akun,d.nama as nama_akun,c.dc,c.nilai,c.nilai-isnull(b.pakai,0)-isnull(e.pakai,0) as sisa "+
			             "from kas_m a "+
						 "     inner join kas_j c on a.no_kas=c.no_kas and a.kode_lokasi=c.kode_lokasi and c.jenis='AKUNREF' "+
						 "     inner join masakun d on c.kode_akun=d.kode_akun and d.kode_lokasi=c.kode_lokasi "+
						 "     left join ( "+
						 "			select ref1,kode_akun,kode_lokasi,sum(nilai) as pakai "+
						 "          from kas_j where ref1='"+this.sg3.cells(0,row)+"' and modul='KBADJ' and jenis='AKUNREF' and kode_lokasi='"+this.app._lokasi+"' "+
						 "          group by ref1,kode_akun,kode_lokasi "+
						 "                ) b on c.no_kas=b.ref1 and c.kode_lokasi=b.kode_lokasi and c.kode_akun=b.kode_akun "+						 
						 "     left join ( "+
						 "			select no_ref,kode_akun,kode_lokasi,sum(nilai) as pakai "+
						 "          from ju_j where no_ref='"+this.sg3.cells(0,row)+"' and modul='JUADJ' and jenis='AKUNREF' and kode_lokasi='"+this.app._lokasi+"' "+
						 "          group by no_ref,kode_akun,kode_lokasi "+
						 "                ) e on c.no_kas=e.no_ref and c.kode_lokasi=e.kode_lokasi and c.kode_akun=e.kode_akun "+						 
						 "where a.no_kas='"+this.sg3.cells(0,row)+"' and a.modul = 'KBREF' and a.kode_lokasi ='"+this.app._lokasi+"' "+
						 "union "+
						 "select c.kode_akun,d.nama as nama_akun,c.dc,c.nilai,c.nilai-isnull(b.pakai,0)-isnull(e.pakai,0) as sisa "+
			             "from ju_m a "+
						 "     inner join ju_j c on a.no_ju=c.no_ju and a.kode_lokasi=c.kode_lokasi and c.jenis='AKUNREF' "+
						 "     inner join masakun d on c.kode_akun=d.kode_akun and d.kode_lokasi=c.kode_lokasi "+
						 "     left join ( "+
						 ...
						 "          from kas_j where ref1='"+this.sg3.cells(0,row)+"' and modul='KBADJ' and jenis='AKUNREF' and kode_lokasi='"+this.app._lokasi+"' "+
						 "          group by ref1,kode_akun,kode_lokasi "+
						 "                ) e on c.no_ju=e.ref1 and c.kode_lokasi=e.kode_lokasi and c.kode_akun=e.kode_akun "+
						 "where a.no_ju='"+this.sg3.cells(0,row)+"' and a.modul = 'JUREF' and a.kode_lokasi ='"+this.app._lokasi+"' ";
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){
					this.sg3.cells(2,row,line.kode_akun);
					this.sg3.cells(3,row,line.nama_akun);
					this.sg3.cells(4,row,line.dc);
					this.sg3.cells(5,row,floatToNilai(line.nilai));
					this.sg3.cells(6,row,floatToNilai(line.sisa));					
					this.sg3.cells(7,row,"0");					
				} 
			}						
		}			
	}
