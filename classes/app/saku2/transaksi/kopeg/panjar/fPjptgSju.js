...
*/
window.app_saku2_transaksi_kopeg_panjar_fPjptgSju = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_panjar_fPjptgSju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_panjar_fPjptgSju";
		...
		this.c_jenis2 = new saiCBBL(this,{bound:[20,17,222,20],caption:"Jenis Dok", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});
		...
		this.e_nilaiptg = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,15,200,20],caption:"Ptg Non Kas", tag:1, readOnly:true, tipeText:ttNilai, text:"0",change:[this,"doChange"]});				
		this.e_nilaikb = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,16,200,20],caption:"Nilai KasBank", tag:9, tipeText:ttNilai, text:"0", readOnly:true});				
		this.c_jenis = new saiCBBL(this.pc1.childPage[0],{bound:[20,17,222,20],caption:"Jenis Dok", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});
		...
		try {
			...
			this.c_jenis2.setSQL("select no_dokumen, nama from sju_dokumen where modul = 'PJ' and kode_lokasi='"+this.app._lokasi+"'",["no_dokumen","nama"],false,["Format","Nama"],"and","Data Format Dokumen",true);
...
window.app_saku2_transaksi_kopeg_panjar_fPjptgSju.extend(window.childForm);
window.app_saku2_transaksi_kopeg_panjar_fPjptgSju.implement({
	...
	simpan: function(){			
		try{						
			...
			if (nilaiToFloat(this.e_nilaikb.getText()) != 0) {				
				var AddFormat = this.c_jenis.getText().substr(0,2)+"/"+this.e_periode.getText().substr(2,4)+"/___%";//+this.c_jenis.getText().substr(2,2);
				...
				if (typeof data == "object"){
					...
					if (line != undefined){
						if (line.no_kas == "0") this.e_kb.setText(this.c_jenis.getText().substr(0,2)+"/"+this.e_periode.getText().substr(2,4)+"/001"+this.c_jenis.getText().substr(2,3));
						else {
							...
							this.e_kb.setText(this.c_jenis.getText().substr(0,2)+"/"+this.e_periode.getText().substr(2,4)+"/"+nu+this.c_jenis.getText().substr(2,3));						
						}
					} 
				}			
			}
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (nilaiToFloat(this.e_nilaikb.getText()) != 0) {
						var posted = "X"; 
						this.e_nb.setText(this.e_kb.getText());
					}
					else {
						var posted = "F";
						sql.add("insert into ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user) values "+
						        "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','-','"+this.e_ket.getText()+"','-','PTGPJ','"+this.c_jenis2.getText()+"','IDR',1,"+parseNilai(this.e_nilaiptg.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"','X','-','-','-',getdate(),'"+this.app._userLog+"')");
					}
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into ptg_j(no_ptg,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
										"	('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','"+this.app._lokasi+"','PJPTG','BEBAN','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");
							}
						}
					}
					if (nilaiToFloat(this.e_nilaikb.getText()) != 0) {
						if (nilaiToFloat(this.e_nilaikb.getText())<0) var DC = "C"; else var DC = "D"; 
						sql.add("insert into ptg_j(no_ptg,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
								"	('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',98,'"+this.cb_akunkb.getText()+"','"+this.e_ket.getText()+"','"+DC+"',"+Math.abs(nilaiToFloat(this.e_nilaikb.getText()))+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','PJPTG','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");
					}
					sql.add("insert into ptg_j(no_ptg,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
							"	('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',99,'"+this.cb_akunpj.getText()+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_nilaipj.getText())+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','PJPTG','PJ','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");								
					...
					if (nilaiToFloat(this.e_nilaikb.getText()) != 0) {
						...
						sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
								"('"+this.e_kb.getText()+"','"+this.app._lokasi+"','-','-','"+this.cb_akunkb.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','-','KBPTG','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilaikb.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"',getdate(),'"+this.app._userLog+"','F','-','-','"+this.e_nb.getText()+"','-')");
						if (this.sg.getRowValidCount() > 0){
							for (var i=0;i < this.sg.getRowCount();i++){
								if (this.sg.rowValid(i)){
									sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
											"	('"+this.e_kb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','-','-','"+this.app._lokasi+"','KBPTG','BEBAN','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");
								}
							}
						}
						if (nilaiToFloat(this.e_nilaikb.getText())<0) var DC = "C"; else var DC = "D"; 
						sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
								"	('"+this.e_kb.getText()+"','-','"+this.dp_d1.getDateString()+"',98,'"+this.cb_akunkb.getText()+"','"+this.e_ket.getText()+"','"+DC+"',"+Math.abs(nilaiToFloat(this.e_nilaikb.getText()))+",'"+this.cb_pp.getText()+"','-','-','-','"+this.app._lokasi+"','KBPTG','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");
						sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
								"	('"+this.e_kb.getText()+"','-','"+this.dp_d1.getDateString()+"',99,'"+this.cb_akunpj.getText()+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_nilaipj.getText())+",'"+this.cb_pp.getText()+"','-','-','-','"+this.app._lokasi+"','KBPTG','PJ','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");
	...
	doChange:function(sender){
		if (sender == this.e_periode || sender == this.c_jenis2) this.doClick();
		...
		if (sender == this.e_nilaipj || sender == this.e_nilaiptg) {
			if (this.e_nilaipj.getText()!="" && this.e_nilaiptg.getText()!="") {
				this.e_nilaikb.setText(floatToNilai(nilaiToFloat(this.e_nilaipj.getText()) - nilaiToFloat(this.e_nilaiptg.getText())));
			}
		}
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "" && this.c_jenis2.getText()!= "") {
			var AddFormat = this.c_jenis2.getText().substr(0,2)+"/"+this.e_periode.getText().substr(2,4)+"/___%";//+this.c_jenis2.getText().substr(2,2);
			...
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){
					if (line.no_ju == "0") this.e_nb.setText(this.c_jenis2.getText().substr(0,2)+"/"+this.e_periode.getText().substr(2,4)+"/001"+this.c_jenis2.getText().substr(2,3));
					else {
						...
						if (idx.length == 2) var nu = "0"+idx;
						if (idx.length == 3) var nu = idx;
						this.e_nb.setText(this.c_jenis2.getText().substr(0,2)+"/"+this.e_periode.getText().substr(2,4)+"/"+nu+this.c_jenis2.getText().substr(2,3));						
					}
				} 
			}
			this.e_ket.setFocus();
