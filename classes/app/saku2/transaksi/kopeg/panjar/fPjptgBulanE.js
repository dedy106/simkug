...
*/
window.app_saku2_transaksi_kopeg_panjar_fPjptgBulanE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_panjar_fPjptgBulanE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_panjar_fPjptgBulanE";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pertanggungan Panjar Gar Bulanan: Edit", 0);	
		...
		this.pc1 = new pageControl(this,{bound:[20,12,900,307], childPage:["Data Panjar","Item Jurnal Pertanggungan","Data Anggaran","Data Pendukung"]});
...
window.app_saku2_transaksi_kopeg_panjar_fPjptgBulanE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_panjar_fPjptgBulanE.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from dok_load where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul ='PJPTG'");
	...
	doChange:function(sender){
		...
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			...
			var data = this.dbLib.getDataProvider("select b.kode_jenis,b.nama,a.no_gambar from dok_load a inner join dok_jenis b on a.kode_jenis=b.kode_jenis and a.kode_lokasi=b.kode_lokasi "+
					   "where a.no_bukti = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='PJPTG' order by a.nu",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sgUpld.clear();
				for (var i in data.rs.rows){
	...
	doHitungGar: function(){
		...
		var sls = 0;
		for (var i=0;i < this.sg2.getRowCount();i++){
			var data = this.dbLib.getDataProvider("select fn_cekaggBulanRev('"+this.sg2.cells(2,i)+"','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.nb+"') as gar ",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line = data.rs.rows[0];
				data = line.gar.split(";");
