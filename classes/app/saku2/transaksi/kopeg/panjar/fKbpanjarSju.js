window.app_saku2_transaksi_kopeg_panjar_fKbpanjarSju = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_panjar_fKbpanjarSju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_panjar_fKbpanjarSju";
		...
		this.cb_buat = new saiCBBL(this,{bound:[20,16,200,20],caption:"NIK Pemegang", multiSelection:false, maxLength:10, tag:2});
...
window.app_saku2_transaksi_kopeg_panjar_fKbpanjarSju.extend(window.childForm);
window.app_saku2_transaksi_kopeg_panjar_fKbpanjarSju.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','-','-','"+this.cb_kas.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_pp.getText()+"','KBPJ','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"',getdate(),'"+this.app._userLog+"','F','-','-','"+this.e_pj.getText()+"','-')");					
	...
	doSelectDate: function(sender, y,m,d){
		...
		var data = this.dbLib.getDataProvider("select convert(varchar,dateadd(day, 14, '"+this.dp_d1.getDateString()+"'),103) as due_date ",true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			var line = data.rs.rows[0];							
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "" && this.c_jenis.getText()!= "") {
			...
			this.e_pj.setText(this.e_nb.getText());
			this.dp_d2.setFocus();
			setTipeButton(tbSimpan);
		}		
	},
