...
*/
window.app_saku2_transaksi_kopeg_panjar_fPjptgSjuE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_panjar_fPjptgSjuE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_panjar_fPjptgSjuE";
		...
		this.c_jenis2 = new saiLabelEdit(this,{bound:[20,15,202,20],caption:"Jenis Dok", readOnly:true});				
		...
		this.cb_pj = new saiCBBL(this,{bound:[20,17,220,20],caption:"No Panjar",  readOnly:true, tag:1});
		...
		this.c_jenis = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,15,202,20],caption:"Jenis Dok", readOnly:true});				
...
window.app_saku2_transaksi_kopeg_panjar_fPjptgSjuE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_panjar_fPjptgSjuE.implement({
	...
	simpan: function(){			
		try{						
			...
			if (this.nokas == "-" && nilaiToFloat(this.e_nilaikb.getText()) != 0) {
			...
			if (this.nokas != "-" && nilaiToFloat(this.e_nilaikb.getText()) != 0 && this.jenis != this.c_jenis.getText()) {
				var AddFormat = this.c_jenis.getText().substr(0,2)+"/"+this.e_periode.getText().substr(2,4)+"/___%";//+this.c_jenis.getText().substr(2,3);
				var data = this.dbLib.getDataProvider("select isnull(max(no_kas),0) as no_kas from kas_m where no_kas like '"+AddFormat+"' and kode_lokasi='"+this.app._lokasi+"'",true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (nilaiToFloat(this.e_nilaikb.getText()) != 0) {
						...
						sql.add("insert into ptg_j(no_ptg,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
								"	('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',98,'"+this.cb_akunkb.getText()+"','"+this.e_ket.getText()+"','"+DC+"',"+parseNilai(this.e_nilaikb.getText())+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','PJPTG','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");
					}
					sql.add("insert into ptg_j(no_ptg,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
					...
					if (nilaiToFloat(this.e_nilaikb.getText()) != 0) {
						...
						if (this.c_jenis.getText() == "KK" || this.c_jenis.getText() == "BK") var DC = "C"; else var DC = "D"; 
						sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
								"	('"+this.e_kb.getText()+"','-','"+this.dp_d1.getDateString()+"',98,'"+this.cb_akunkb.getText()+"','"+this.e_ket.getText()+"','"+DC+"',"+parseNilai(this.e_nilaikb.getText())+",'"+this.cb_pp.getText()+"','-','-','-','"+this.app._lokasi+"','KBPTG','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");
						sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
			...
			case "ubah" :
				...
				*/
				...
				if (nilaiToFloat(this.e_nilaikb.getText()) != 0) {
					...
					if (this.c_jenis.getText()=="-" || this.cb_akunkb.getText()=="-") {
	...
	doChange:function(sender){		
		...
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			...
			var data = this.dbLib.getDataProvider(
					   "select a.no_dokumen,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,a.nik_buat,b.nama as nama_buat,a.nik_setuju,c.nama as nama_setuju,a.nilai as nilai_ptg,abs(a.nilai_kas) as nilai_kas,a.no_link, "+ 
			...
			if (typeof data == "object"){
				...
				if (line != undefined){	
					...
					this.c_jenis2.setText(line.no_link);
