window.app_saku2_transaksi_kopeg_panjar_proses_fSpbPjE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_panjar_proses_fSpbPjE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_panjar_proses_fSpbPjE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pembuatan SPB (Panjar Proses): Edit", 0);	
...
window.app_saku2_transaksi_kopeg_panjar_proses_fSpbPjE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_panjar_proses_fSpbPjE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("update a set a.progress='0' from panjar2_m a inner join spb_d b on a.no_panjar=b.no_bukti and a.kode_lokasi=b.kode_lokasi "+
					        "where b.no_spb = '"+this.e_nb.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"'");
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!= "") {
			this.e_nb.setSQL("select distinct a.no_spb, a.keterangan from spb_m a inner join spb_d b on a.no_spb=b.no_spb and a.kode_lokasi=b.kode_lokasi and b.modul='PJ2' "+
			                 "where a.no_kas='-' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",["a.no_spb","a.keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar SPB",true);			
		}
		if (sender == this.e_nb && this.e_nb.getText()!= "") {
			var data = this.dbLib.getDataProvider("select * from spb_m where no_spb='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
	...
	doLoad:function(sender){
		if (this.e_nb.getText()!="") {
			var strSQL = "select 'SPB' as status,a.no_panjar,a.no_dokumen,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,a.kode_pp+' - '+b.nama as pp,a.nik_setuju+' - '+c.nama as approve,a.nik_buat+' - '+d.nama as pemegang,d.bank,d.cabang,d.no_rek,d.nama_rek,a.nilai,a.akun_panjar "+
						 "from panjar2_m a "+
						 ...
						 "where e.no_spb ='"+this.e_nb.getText()+"' and e.kode_lokasi = '"+this.app._lokasi+"'";						
