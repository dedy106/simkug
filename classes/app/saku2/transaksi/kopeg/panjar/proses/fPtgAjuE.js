window.app_saku2_transaksi_kopeg_panjar_proses_fPtgAjuE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_panjar_proses_fPtgAjuE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_panjar_proses_fPtgAjuE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Input Pertanggungan Panjar (Proses): Edit", 0);	
		...
		this.pc1 = new pageControl(this,{bound:[20,12,980,247], childPage:["Item Mata Anggaran","KPA Anggaran","Verifikasi"]});
...
window.app_saku2_transaksi_kopeg_panjar_proses_fPtgAjuE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_panjar_proses_fPtgAjuE.implement({
	...
	doChange:function(sender){
		if (sender == this.e_periode || sender == this.cb_pp) {
			if (this.e_periode.getText()!="" && this.cb_pp.getText()!="") {				
				this.e_nb.setSQL("select no_ptg, keterangan from panjarptg2_m where modul = 'PJPTG' and kode_pp='"+this.cb_pp.getText()+"' and progress in ('0','R')  and periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_ptg","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);				
			}
		}
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			this.cb1.setSelected(true);		
			var strSQL = "select a.nilai,a.akun_panjar,a.no_panjar,a.keterangan, "+
						 " b.no_dokumen,b.keterangan as ket,b.nik_buat,c.nama as nama_buat,b.nik_app,d.nama as nama_app,b.tanggal,isnull(x.no_ver,'-') as no_ver,isnull(x.catatan,'-') as catatan  "+
						 ...
						 "       		   left join ver_d x on b.no_ptg=x.no_bukti and b.kode_lokasi=x.kode_lokasi "+
