window.app_saku2_transaksi_kopeg_lab_fJuE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_lab_fJuE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_lab_fJuE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Jurnal Umum : Edit", 0);	
		
		uses("saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox;wysiwyg");
		...
		this.c_jenis = new saiCB(this,{bound:[20,22,202,20],caption:"Jenis",items:["UMUM","KOREKSI","ADJUST","REVERSE"], readOnly:true, change:[this,"doChange"], tag:2});		
		this.e_dok = new saiLabelEdit(this,{bound:[20,13,450,20],caption:"No Dokumen", maxLength:50});				
		this.e_debet = new saiLabelEdit(this,{bound:[700,13,220,20],caption:"Total Debet", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		...
		this.pc1 = new pageControl(this,{bound:[20,12,900,320], childPage:["Data Item Jurnal","Catatan",,"Dokumen Tugas"]});
		...
		this.mCatat = new wysiwyg(this.pc1.childPage[1],{bound:[1,10,this.pc1.width-25,this.pc1.height-125], withForm:false});
		this.mCatat.display();
		this.mCatat.enable();
...
window.app_saku2_transaksi_kopeg_lab_fJuE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_lab_fJuE.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into lab_ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_tugas,tgl_input,nik_user,catatan) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','-','JU','"+this.c_jenis.getText()+"','IDR',1,"+parseNilai(this.e_debet.getText())+",'-','-','F','"+this.cb_tugas.getText()+"',getdate(),'"+this.app._userLog+"','"+urlencode(this.mCatat.getCode())+"')");
	...
	doChange:function(sender){
		if (sender == this.cb_tugas) {
			if (this.cb_tugas.getText()!="") {
				this.e_nb.setSQL("select no_ju, keterangan from lab_ju_m where no_tugas='"+this.cb_tugas.getText()+"' and nik_user = '"+this.app._userLog+"' and posted='F' and kode_lokasi='"+this.app._lokasi+"'",["no_ju","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
				var sql = new server_util_arrayList();
		...
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			var data = this.dbLib.getDataProvider("select a.catatan,a.no_dokumen,convert(varchar,a.tanggal,103) as tanggal,a.periode,a.keterangan,a.jenis,no_tugas "+
