window.app_saku2_transaksi_kopeg_lab_fKontenE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_lab_fKontenE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_lab_fKontenE";
		...
		try {
			...
			this.e_nb.setSQL("select no_konten, judul from lab_konten where  kode_lokasi='"+this.app._lokasi+"'",["no_konten","judul"],false,["No Konten","Keterangan"],"and","Data Konten",true);					
			...
			this.cb_buat.setSQL("select kode_dosen, nama from lab_dosen where kode_lokasi='"+this.app._lokasi+"'",["kode_dosen","nama"],false,["Kode","Nama"],"and","Data NIK Pembuat",true);
...
window.app_saku2_transaksi_kopeg_lab_fKontenE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_lab_fKontenE.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("delete from lab_konten where no_konten='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doChange:function(sender){
		...
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			var data = this.dbLib.getDataProvider(
					   "select a.kode_klp,b.nama as nama_klp,a.judul,a.keterangan,a.tanggal,a.file_gambar,a.file_dok,a.flag_aktif,a.kode_dosen,e.nama as nama_dosen "+					   
					   "from lab_konten a "+
					   "inner join lab_klpkonten b on a.kode_klp=b.kode_klp and a.kode_lokasi=b.kode_lokasi "+	  
					   "inner join lab_dosen e on a.kode_dosen=e.kode_dosen and a.kode_lokasi=e.kode_lokasi "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){			
					...
					this.cb_buat.setText(line.kode_dosen,line.nama_dosen);
