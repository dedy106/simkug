window.app_saku2_transaksi_kopeg_lab_fNeraca = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_lab_fNeraca.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku2_transaksi_kopeg_lab_fNeraca";
		...
		this.cb1 = new portalui_checkBox(this,{bound:[450,25,200,20],caption:"Show Referensi",selected:false,click:[this,"doClick4"]});

		if (this.app._userStatus !="A"){
			this.elokasi.setReadOnly(true);			
		}
		this.eNik = new portalui_saiCBBL(this,{bound:[20, 10, 220, 20], caption:"Dosen", multiSelection:false, 
			sql:["select kode_dosen, nama from lab_dosen where kode_lokasi = '"+this.app._lokasi+"' and kode_dosen='"+this.app._userLog+"'", ["kode_dosen","nama"], false, ["Kode","Nama"],"and","Daftar Dosen",true]
		});	
		this.eNik2 = new saiCBBL(this,{bound:[450,10,222,20],caption:"Dosen",multiSelection:false, maxLength:10, tag:2,visible:false,change:[this,"doClick3"]});
		
		this.eKlp = new portalui_saiCBBL(this,{bound:[20, 11, 220, 20], caption:"No Tugas", multiSelection:false, 
			sql:["select no_tugas, keterangan from lab_tugas where kode_lokasi = '"+this.app._lokasi+"' and kode_dosen='"+this.app._userLog+"'", ["no_tugas","keterangan"], false, ["No Tugas","Keterangan"],"and","Daftar Tugas",true]
		});	
		this.eKlp2 = new saiCBBL(this,{bound:[450,11,222,20],caption:"Referensi No Tugas",multiSelection:false,visible:false, maxLength:10, tag:2});
		...
		this.btn2 = new portalui_imageButton(this,{bound:[550,12,22,22], hint:"Reload", image:"icon/"+system.getThemes()+"/reload.png",visible:false, click:"doClick2"});
		...
		this.treev = new portalui_treeView(this,{bound:[20,14,700,this.getHeight() - 150], dblClick:[this,"treeClick"]});
		...
		this.eNik2.setSQL("select kode_dosen, nama from lab_dosen where  kode_lokasi='"+this.app._lokasi+"'",["kode_dosen","nama"],false,["Kode","Nama"],"and","Daftar Dosen",true);

	}
};
window.app_saku2_transaksi_kopeg_lab_fNeraca.extend(window.portalui_childForm);
window.app_saku2_transaksi_kopeg_lab_fNeraca.implement({
				...
				case "relakun":
					if (modalResult == mrOk)
					{
						...
						for (var i in value.objList){
							value.get(i).set("no_tugas",this.eKlp.getText());
						}
						node.data2 = value;
					}
					break;
				case "Add" :
				   if ( modalResult == mrOk)
				   {
						...
						{   
							...
							data.set("no_tugas",this.eKlp.getText());	
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
						...
						try
						{
							...
							var sql2,sql1 = "delete from lab_neraca where modul = '"+modul+"'  and no_tugas = '"+this.eKlp.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";					// kode_lokasi,
							var sql = new server_util_arrayList();
							sql.add("delete from lab_relakun  where  no_tugas = '"+this.eKlp.getText()+"' "+
								"and kode_neraca in (select distinct kode_neraca from lab_neraca where modul = '"+modul+"'  and no_tugas = '"+this.eKlp.getText()+"' )");
							sql.add(sql1);							
							//if mysql langsung														
							var data = value.split("#");
							for (var i in data){
								sql2 = "insert into lab_neraca(kode_neraca, kode_lokasi, no_tugas, nama, level_spasi, level_lap, tipe, sum_header, jenis_akun, kode_induk, rowindex, modul,nik_user) values ";
								sql2 += " "+ data[i] +" ";
								sql.add(sql2);
							}	
							data = this.value2.split("#");
							for (var i in data){
								sql3 = "insert into lab_relakun(kode_neraca, no_tugas, kode_akun, kode_lokasi,nik_user) values ";
								if (data[i]) {
									sql3 += " "+ data[i] +" ;";
									sql.add(sql3);					
								}
							...
							sql.add("insert into lab_neraca(kode_neraca, kode_lokasi, no_tugas, nama, level_spasi, level_lap, tipe, sum_header, jenis_akun, kode_induk, rowindex, modul,nik_user) "+
								" select distinct a.kode_neraca, a.kode_lokasi, a.no_tugas, a.nama, a.level_spasi, a.level_lap, a.tipe, a.sum_header, a.jenis_akun, a.kode_induk, a.rowindex, a.modul,b.nim "+
								"from lab_neraca a "+
								"cross join (select a.nim "+
								"			from lab_mhs a  "+
								"			inner join lab_kelas_mhs b on a.nim=b.nim and a.kode_lokasi=b.kode_lokasi "+
								"			inner join lab_tugas c on b.kode_lokasi=c.kode_lokasi and b.kode_kelas=c.kode_kelas "+
								"			where c.no_tugas='"+this.eKlp.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and c.kode_dosen='"+this.app._userLog+"' and b.nim<>'"+this.app._userLog+"') b "+  
								"where a.kode_lokasi = '"+this.app._lokasi+"' and a.modul = '"+modul+"' and a.no_tugas='"+this.eKlp.getText()+"' and a.nik_user='"+this.app._userLog+"'");
							
							sql.add("insert into lab_relakun(kode_neraca, no_tugas, kode_akun, kode_lokasi,nik_user) "+
								" select distinct a.kode_neraca, a.no_tugas, a.kode_akun, a.kode_lokasi,b.nim "+
								" from lab_relakun a "+
								"cross join (select a.nim "+
								"			from lab_mhs a  "+
								"			inner join lab_kelas_mhs b on a.nim=b.nim and a.kode_lokasi=b.kode_lokasi "+
								"			inner join lab_tugas c on b.kode_lokasi=c.kode_lokasi and b.kode_kelas=c.kode_kelas "+
								"			where c.no_tugas='"+this.eKlp.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and c.kode_dosen='"+this.app._userLog+"' and b.nim<>'"+this.app._userLog+"') b "+  
								"where a.kode_lokasi = '"+this.app._lokasi+"' and a.no_tugas='"+this.eKlp.getText()+"' and a.nik_user='"+this.app._userLog+"' "+
								"and a.kode_neraca in (select distinct kode_neraca from lab_neraca where modul = '"+modul+"'  and no_tugas = '"+this.eKlp.getText()+"' )");
	...
	loadMenu: function(daftarNrc){
		try
		{
			...
			var item = undefined;
			var node = undefined;
			var line, ix = -1, nrcRls, lineData, nrc_d = this.dblib.loadQuery("select kode_neraca, kode_akun from lab_relakun where kode_lokasi ='"+this.elokasi.getText()+"' and no_tugas = '"+this.eKlp.getText()+"' and nik_user='"+this.eNik.getText()+"' order by kode_neraca");															
	...
	loadMenu2: function(daftarNrc){
		try
		{
			...
			var item = undefined;
			var node = undefined;
			var line, ix = -1, nrcRls, lineData, nrc_d = this.dblib.loadQuery("select kode_neraca, kode_akun from lab_relakun where kode_lokasi ='"+this.elokasi.getText()+"' and no_tugas = '"+this.eKlp2.getText()+"' and nik_user='"+this.eNik2.getText()+"' order by kode_neraca");															
			var kdNrc = '-', dataRelakun = new portalui_arrayMap();
			nrcRls = new portalui_arrayMap();
			if (nrc_d != ""){					
				nrc_d = nrc_d.split("\r\n");							
	...
	doClick: function(sender){
		if (this.e0.getText() != "")
		{
			...
			var nrc = this.dblib.runSQL("select * from lab_neraca where Modul = '"+modul+"' and no_tugas = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"' and nik_user='"+this.eNik.getText()+"' order by rowindex",0,0);
			this.loadMenu(nrc);
		}	
	},
	doClick2: function(sender){
		if (this.e0.getText() != "")
		{
			var modul = this.e0.getText();
			modul = modul.substr(0,1);//
			var nrc = this.dblib.runSQL("select * from lab_neraca where Modul = '"+modul+"' and no_tugas = '"+this.eKlp2.getText()+"' and nik_user='"+this.eNik2.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"'  order by rowindex",0,0);
			this.loadMenu2(nrc);
		}	
	},
	doClick3: function(sender){
		
		this.eKlp2.setSQL("select no_tugas, nama from lab_tugas where kode_dosen='"+this.eNik2.getText()+"' and flag_aktif ='1' and kode_lokasi='"+this.app._lokasi+"'",["no_tugas","nama"],false,["Kode","Nama"],"and","Daftar Tugas",true);
	},
	doClick4: function(sender){
		if (this.cb1.isSelected()) 
		{
			this.eKlp2.setVisible(true);
			this.eNik2.setVisible(true);
			this.btn2.setVisible(true);
		}
		else
		{
			this.eKlp2.setVisible(false);
			this.eNik2.setVisible(false);
		...
		{ 
		 	if (sender == this.relBtn)
			{
				...
				if (this.selectedItem.data.get("tipe").toLowerCase() == "posting"){
					uses("app_saku2_transaksi_kopeg_lab_fRelakun",true);
					if (this.relakun != undefined)
						this.relakun.free();
					this.relakun = new app_saku2_transaksi_kopeg_lab_fRelakun(this.app);
					this.relakun.elokasi = this.elokasi.getText();
					this.relakun.eTugas = this.eKlp.getText();
					this.relakun.eNik = this.eNik.getText();
			...
			}else
			{
				if (sender == this.addBtn || sender == this.editBtn){
					uses("app_saku2_transaksi_kopeg_lab_fNeracaDetail");
					if (this.entryMenu == undefined)
						this.entryMenu = new app_saku2_transaksi_kopeg_lab_fNeracaDetail(this.app,this.e0.getText());
	...
	getTreeData: function(node){
		...
		}else
		{
			...
			if (value != undefined && value != ""){
				...
				for (var i in value.objList){				
					this.value2 += "#('"+node.getKode()+"','"+this.eKlp.getText()+"','" + value.get(i).get("kode_akun") +"','"+this.elokasi.getText()+"','"+this.app._userLog+"')";		
