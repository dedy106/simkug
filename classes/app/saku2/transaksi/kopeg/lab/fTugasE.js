window.app_saku2_transaksi_kopeg_lab_fTugasE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_lab_fTugasE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_lab_fTugasE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Tugas: Edit", 0);	
		...
		this.e_nb = new saiCBBL(this,{bound:[20,12,223,20],caption:"No Tugas", multiSelection:false, maxLength:10, tag:1, readOnly:true,change:[this,"doChange"]});		
		...
		this.sg = new saiGrid(this.pc1.childPage[3],{bound:[0,10,this.pc1.width-5,this.pc1.height-40],colCount:3,tag:0,
					...
					pasteEnable:true,autoAppend:true,defaultRow:1});
...
window.app_saku2_transaksi_kopeg_lab_fTugasE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_lab_fTugasE.implement({	
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from lab_tugas where no_tugas='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from lab_tugas_d where no_tugas='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doSelectDate: function(sender, y,m,d){
		...
		this.e_nb.setSQL("select no_tugas,nama from lab_tugas where flag_aktif='1' and kode_dosen='"+this.app._userLog+"' and kode_lokasi='"+this.app._lokasi+"'",["no_tugas","nama"],false,["No Tugas","Deskripsi"],"and","Daftar Tugas",true);
	},							
	doChange:function(sender){
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			var strSQL = "select * from lab_tugas where no_tugas ='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
			...
			if (typeof data == "object"){
				...
				if (line != undefined){												
					...
					this.dp_d4.setText(line.tgl_awal);
					this.dp_d5.setText(line.tgl_akhir);
					this.e_tahun.setText(line.tahun);
					this.c_sem.setText(line.semester);					
			...
			var data = this.dbLib.getDataProvider("select tanggal,keterangan,jenis,nu from lab_tugas_d where no_tugas='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' order by nu",true); 
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData([line.tanggal,line.keterangan,line.jenis]);
				}
