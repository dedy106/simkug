window.app_saku2_transaksi_kopeg_lab_fKontenV = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_lab_fKontenV.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_lab_fKontenV";
...
window.app_saku2_transaksi_kopeg_lab_fKontenV.extend(window.childForm);
window.app_saku2_transaksi_kopeg_lab_fKontenV.implement({
	...
	doEllipseClick:function(sender, col, row)
	{
		...
		if (row == 2)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Konten",this.sg1, this.sg1.row, this.sg1.col,
													  "select a.no_konten,a.judul from lab_konten a "+filter,
													  "select count(a.no_konten) from lab_konten a "+filter,
													  new Array("a.no_konten","a.judul"),"and",new Array("No Konten","Judul"));
		}
	...
	doSelectCell:function(sender, col, row)
	{
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from lab_konten order by periode desc",[this.sg1.columns.get(2).pickList]);
		}
		...
		try
		{
			...
			else
			{
				...
				var sql="select a.no_konten,b.nama as nama_klp,date_format(a.tanggal,'%d-%m-%Y') as tanggal,a.judul,a.file_gambar,a.file_dok "+
					" from lab_konten a "+
					" inner join lab_klpkonten b on b.kode_lokasi = a.kode_lokasi and b.kode_klp = a.kode_klp "+this.filter+
	...
	doDoubleClick: function(sender, col , row) 
	{
		try{
			...
			this.lokasi = "";
			this.filter2 = "";
			this.docViewer.src = this.report.previewWithHeader("server_report_saku2_kopeg_lab_rptInformasi",this.filter, 1,1, this.showFilter, this.lokasi,this.filter2);
