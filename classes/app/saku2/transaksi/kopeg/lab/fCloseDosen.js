window.app_saku2_transaksi_kopeg_lab_fCloseDosen = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_lab_fCloseDosen.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_lab_fCloseDosen";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Closing Tugas: Input", 0);	
		...
		this.e_nb = new portalui_saiLabelEdit(this,{bound:[20,10,202,20],caption:"No Close",maxLength:30,readOnly:true,visible:false});		
		
		this.cb_tugas = new saiCBBL(this,{bound:[20,10,222,20],caption:"No Tugas", multiSelection:false, maxLength:10, tag:0,change:[this,"doChange"]});		
		...
		this.pc1 = new pageControl(this,{bound:[10,18,1000,370], childPage:["Deskripsi Tugas","Rincian Tugas","Data Tugas MHS","Filter Data"]});
		...
		this.e_npm = new saiLabelEdit(this.pc1.childPage[3],{bound:[20,11,200,20],caption:"N I M",tag:9});		
		this.bCari = new button(this.pc1.childPage[3],{bound:[230,11,80,18],caption:"Cari Data",click:[this,"doCari"]});			
...
window.app_saku2_transaksi_kopeg_lab_fCloseDosen.extend(window.childForm);
window.app_saku2_transaksi_kopeg_lab_fCloseDosen.implement({
	...
	simpan: function(){			
		try{		
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"lab_closetugas","no_close",this.app._lokasi+"-CTG.","000000"));
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();										
					sql.add("update lab_tugas set flag_aktif='0' where no_tugas='"+this.cb_tugas.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");							
					sql.add("insert into lab_closetugas(no_close,kode_lokasi,tanggal,no_tugas,nik_user,catatan) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.cb_tugas.getText()+"','"+this.app._userLog+"','-')");
