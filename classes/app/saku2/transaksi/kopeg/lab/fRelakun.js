window.app_saku2_transaksi_kopeg_lab_fRelakun = function(owner,lokasi)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_lab_fRelakun.prototype.parent.constructor.call(this, owner);
		window.app_saku2_transaksi_kopeg_lab_fRelakun.prototype.parent.setWidth.call(this, 800);
		window.app_saku2_transaksi_kopeg_lab_fRelakun.prototype.parent.setHeight.call(this, 400);
		
		this.centerize();
		
		this.className = "app_saku2_transaksi_kopeg_lab_fRelakun";
...
window.app_saku2_transaksi_kopeg_lab_fRelakun.extend(window.commonForm);
window.app_saku2_transaksi_kopeg_lab_fRelakun.implement({
	...
	setWidth: function(data){
		window.app_saku2_transaksi_kopeg_lab_fRelakun.prototype.parent.setWidth.call(this, data);
		this.header.style.width = data - 23;
		this.rBg.style.left = data - 23;
	},
	doSysMouseMove: function(x, y, button, buttonState){
		window.app_saku2_transaksi_kopeg_lab_fRelakun.prototype.parent.doSysMouseMove.call(this,x, y, button, buttonState);
		if (this.isClick)
		{		
	...
	loadNeraca: function(){			
		...
		var nrc = this.dblib.runSQL("select a.kode_akun, a.nama from lab_masakun a "+				
					"where a.kode_lokasi = '"+this.elokasi+"' and a.no_tugas='"+this.eTugas+"' and a.nik_user='"+this.eNik+"' "+ (filter != "()"? " and a.kode_akun in "+filter:"and a.kode_akun in ('')")); 
	...
	doCariClick: function(sender){
		try{
			...
			var nrc = this.dblib.getDataProvider("select a.kode_akun, a.nama  from lab_masakun a "+				
						"where a.kode_lokasi = '"+this.elokasi+"' and modul like '" + this.modul +"%' and a.no_tugas='"+this.eTugas+"' and a.nik_user='"+this.eNik+"' "+
