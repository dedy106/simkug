window.app_saku2_transaksi_kopeg_tiket_fTiketAjuD = function(owner,options)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_tiket_fTiketAjuD.prototype.parent.constructor.call(this,owner, options);
		this.className  = "app_saku2_transaksi_kopeg_tiket_fTiketAjuD";
		this.itemsValue = new arrayList();
		uses("saiCB;saiMemo;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");		
		this.pc1 = new pageControl(this,{bound:[10,18,1000,500], childPage:["Daftar Tiket","Tiket Baru","Histori Tiket","Filter Data"]});
		...
		this.e_nb = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,13,450,20],caption:"No Tiket", readOnly:true,tag:4});				
		this.e_judul = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,12,450,20],caption:"Judul", maxLength:100,tag:4});
		this.e_form = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,13,450,20],caption:"Kode Menu", maxLength:100,tag:4});
		this.e_user = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,14,450,20],caption:"User", readOnly:true,tag:4});
		this.e_memo = new saiMemo(this.pc1.childPage[1],{bound:[20,13,600,160],caption:"Deskripsi",tag:4});
		this.p1 = new panel(this.pc1.childPage[1],{bound:[10,11,610,200],border:3, caption:"Lampiran Tiket"});		
...
window.app_saku2_transaksi_kopeg_tiket_fTiketAjuD.extend(window.panel);
window.app_saku2_transaksi_kopeg_tiket_fTiketAjuD.implement({	
					...
					if (this.e_memo3.getText() != "") {
						...
						for (var i=0;i < this.sgUpld2.getRowCount();i++){
							if (this.sgUpld2.rowValid(i)){
								ix++;
								sql.add("insert into sai_tiket_dok(no_tiket,no_gambar,nu,kode_lokasi,no_urut) values('"+this.e_nb2.getText()+"','"+this.sgUpld2.cells(1,i).filedest+"','"+ix+"','"+this.app._lokasi+"',"+nu+")");								
							}	
						}
					}
					else {
						sql.add("insert into sai_tiket_m (no_tiket,tanggal,kode_lokasi,nik_user,form,progress,judul,keterangan,no_close,mandays,periode) values "+
								"('"+this.e_nb.getText()+"',getdate(),'"+this.app._lokasi+"','"+this.app._userLog+"','"+this.e_form.getText()+"','"+this.c_status.getText()+"','"+this.e_judul.getText()+"','"+this.e_memo.getText()+"','-',0,'"+this.e_periode.getText()+"')");					
						sql.add("insert into sai_tiket_d (no_tiket,kode_lokasi,nik_user,no_urut,keterangan,tanggal,sts_user) values "+
						...
						for (var i=0;i < this.sgUpld.getRowCount();i++){
							if (this.sgUpld.rowValid(i)){
								ix++;
								sql.add("insert into sai_tiket_dok(no_tiket,no_gambar,nu,kode_lokasi,no_urut) values('"+this.e_nb.getText()+"','"+this.sgUpld.cells(1,i).filedest+"','"+ix+"','"+this.app._lokasi+"',0)");								
							}	
						}
					}
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){							
							...
							this.nama_report="server_report_saku2_kopeg_tiket_rptTiketJawabMail";
