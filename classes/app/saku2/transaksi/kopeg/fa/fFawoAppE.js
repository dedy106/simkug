window.app_saku2_transaksi_kopeg_fa_fFawoAppE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_fa_fFawoAppE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_fa_fFawoAppE";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Approve WriteOff Aktiva Tetap : Edit", 0);	
		...
		this.cb_aju = new saiCBBL(this,{bound:[20,12,220,20],caption:"No Pengajuan", readOnly:true});
...
window.app_saku2_transaksi_kopeg_fa_fFawoAppE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_fa_fFawoAppE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,3])){
				try{													
					...
					sql.add("update a set a.progress='9' from fa_asset a inner join fawo_d b on a.no_fa=b.no_fa and a.kode_lokasi=b.kode_lokasi "+
							"                                            inner join fawo_m c on b.no_wo=c.no_wo and b.kode_lokasi=c.kode_lokasi "+
							"where c.no_link='"+this.e_nb.getText()+"' and c.kode_lokasi='"+this.app._lokasi+"'");
					sql.add("update fawo_m set progress='0', no_link='-' where no_link='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
		...
		switch (event){
			...
			case "ubah" :	
				...
				if (this.flagDokFree == "1") {				
					var data = this.dbLib.getDataProvider("select no_woapp from fawoapp_m where no_woapp <> '"+this.e_nb.getText()+"' and no_dokumen='"+this.e_dok.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
						if (line != undefined){
	...
	doChange:function(sender){				
		if (sender == this.e_periode && this.e_periode.getText()!="") {
			this.sg.clear(1);
			this.e_nb.setSQL("select no_woapp, keterangan from fawoapp_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_woapp","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);						
		}
		if (sender == this.e_nb && this.e_nb.getText()!= "") {									
			var strSQL = "select a.periode,a.tanggal,a.no_dokumen,a.keterangan,a.kode_drk,a.nik_buat,a.nik_setuju,b.kode_akun,c.no_wo,c.keterangan as ket_wo,c.kode_pp,d.nama as nama_akun "+
						 "from fawoapp_m a inner join fawoapp_j b on a.no_woapp=b.no_woapp and a.kode_lokasi=b.kode_lokasi and b.jenis='BEBAN' "+
						 "                 inner join fawo_m c on a.no_woapp=c.no_link and a.kode_lokasi=c.kode_lokasi "+
						 "                 inner join masakun d on b.kode_akun=d.kode_akun and b.kode_lokasi=d.kode_lokasi "+
						 "where a.no_woapp='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
			...
			if (typeof data == "object"){
				...
				if (line != undefined){		
					...
					this.cb_beban.setText(line.kode_akun,line.nama_akun);
					this.cb_buat.setText(line.nik_buat);					
					this.cb_app.setText(line.nik_setuju);							
					this.cb_aju.setText(line.no_wo,line.ket_wo);					
