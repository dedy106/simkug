window.app_saku2_transaksi_kopeg_fa_fFawoAju = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_fa_fFawoAju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_fa_fFawoAju";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan WriteOff Aktiva Tetap : Input", 0);	
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-35],colCount:14,tag:0,
					...
					picklist:[[0],[new portalui_arrayMap({items:["WO","AKTIF"]})]],
...
window.app_saku2_transaksi_kopeg_fa_fFawoAju.extend(window.childForm);
window.app_saku2_transaksi_kopeg_fa_fFawoAju.implement({
	...
	simpan: function(){			
		try{						
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"fawo_m","no_wo",this.app._lokasi+"-RQWO"+this.e_periode.getText().substr(2,4)+".","000"));
		...
		switch (event){
			...
			case "simpan" :				
				...
				if (this.flagDokFree == "1") {				
					var data = this.dbLib.getDataProvider("select no_wo from fawo_m where no_dokumen='"+this.e_dok.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
						if (line != undefined){
	...
	doLoad:function(sender){		
		if(this.e_periode.getText()!="") {
			var strSQL = "select 'AKTIF' as status,a.no_fa,a.barcode,a.no_seri,a.nama,a.merk,a.tipe,c.kode_klpfa+'-'+c.nama as klp_fa,b.kode_klpakun+'-'+b.nama as klp_akun,a.tgl_perolehan,a.nilai,isnull(d.tot_susut,0) as tot_susut,(a.nilai-a.nilai_residu-isnull(d.tot_susut,0)) as nilai_buku,a.nilai_residu,b.kode_akun,b.akun_deprs "+						 
						 "from fa_asset a  "+
						 ...
						 "where a.periode<='"+this.e_periode.getText()+"' and (a.nilai-isnull(d.tot_susut,0)) = a.nilai_residu and a.progress = '2' and a.kode_lokasi='"+this.app._lokasi+"'";				
