window.app_saku2_transaksi_kopeg_apotek_fSoP = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_apotek_fSoP.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_apotek_fSoP";
		...
		this.sgn = new portalui_sgNavigator(this.p1,{bound:[0,333,900,25],buttonStyle:bsAll,grid:this.sg,pager:[this,"doPager"]});
		...
		try{						
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('OBTPDPT','OBTHPP','OBTINV') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku2_transaksi_kopeg_apotek_fSoP.extend(window.portalui_childForm);
window.app_saku2_transaksi_kopeg_apotek_fSoP.implement({	
	...
	simpan: function(){			
		try{					
			if (this.standarLib.checkEmptyByTag(this, [0,2])){
				try{														
					this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"apo_brg_sop_m","no_sop",this.app._lokasi+"-OPNM"+this.e_periode.getText().substr(2,4)+".","000"));
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();										
					sql.add("insert into apo_brg_sop_m(no_sop,kode_lokasi,tanggal,no_dokumen,keterangan,kode_gudang,periode,nik_user,tgl_input,nilai,posted) values "+						    
					...
					sql.add("insert into apo_brg_sop_j(no_sop,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
					...
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						...
						sql.add("insert into apo_brg_sop_d(no_sop,kode_lokasi,periode,kode_brg,kode_gudang,satuan,harga,stok,jumlah,selisih,total) values "+
	...
	doLoad:	function(sender){
		if (this.cb_gudang.getText()!="") {	
			var strSQL = "select a.kode_brg,a.nama,a.merk+' - '+a.tipe as ket,a.satuan,b.h_avg,isnull(c.stok,0) as stok, 0 as jumlah,(0-isnull(c.stok,0)) as selisih,round((0-isnull(c.stok,0))*b.h_avg,0) as total "+
			             "from apo_brg_m a inner join apo_brg_tmp b on a.kode_brg=b.kode_brg and a.kode_lokasi=b.kode_lokasi and b.nik_user ='"+this.app._userLog+"' "+
						 "                 left join apo_brg_stok c on a.kode_brg=c.kode_brg and a.kode_lokasi=c.kode_lokasi and c.kode_gudang='"+this.cb_gudang.getText()+"' and c.nik_user ='"+this.app._userLog+"' "+
