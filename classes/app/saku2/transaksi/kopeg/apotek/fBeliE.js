...
window.app_saku2_transaksi_kopeg_apotek_fBeliE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_apotek_fBeliE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_apotek_fBeliE";
...
window.app_saku2_transaksi_kopeg_apotek_fBeliE.extend(window.portalui_childForm);
window.app_saku2_transaksi_kopeg_apotek_fBeliE.implement({	
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								...
								var j = i+1;
								if ((nilaiToFloat(this.sg.cells(6,i))+nilaiToFloat(this.sg.cells(7,i))) > nilaiToFloat(this.sg.cells(9,i))) {
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText() != "") {						
			this.e_nb.setSQL("select a.no_beli, a.keterangan from apo_brg_beli_m a left join (select distinct no_beli,kode_lokasi from apo_brg_belibayar_d) b on a.no_beli=b.no_beli and a.kode_lokasi=b.kode_lokasi "+
							 "where a.modul='NON' and b.no_beli is null and a.posted='F' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",["no_beli","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
		}		
