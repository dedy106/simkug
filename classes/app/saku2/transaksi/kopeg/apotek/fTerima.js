window.app_saku2_transaksi_kopeg_apotek_fTerima = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_apotek_fTerima.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_apotek_fTerima";
...
window.app_saku2_transaksi_kopeg_apotek_fTerima.extend(window.portalui_childForm);
window.app_saku2_transaksi_kopeg_apotek_fTerima.implement({
	...
	simpan: function(){			
		try{					
			if (this.standarLib.checkEmptyByTag(this, [0,2])){
				try{														
					this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"apo_brg_terima_m","no_terima",this.app._lokasi+"-TRM"+this.e_periode.getText().substr(2,4)+".","000"));
	...
	doChange:function(sender){		
		if (sender == this.cb_tujuan) {
			...
			this.sg.clear(1);
			if (this.cb_tujuan.getText()!="") this.cb_kirim.setSQL("select no_kirim, keterangan from apo_brg_kirim_m where kode_tujuan='"+this.cb_tujuan.getText()+"' and no_terima ='-' and periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kirim","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
		}	
