window.app_saku2_transaksi_kopeg_apotek_fBeliPtg = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_apotek_fBeliPtg.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_apotek_fBeliPtg";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pembelian Barang via Panjar: Input", 0);	
		...
		this.e_pj = new saiLabelEdit(this,{bound:[720,13,200,20],caption:"Nilai PJ", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		this.e_ket = new portalui_saiLabelEdit(this,{bound:[20,14,503,20],caption:"Keterangan", maxLength:150});				
		this.e_nilai = new saiLabelEdit(this,{bound:[720,14,200,20],caption:"Total", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		this.cb_nik = new portalui_saiCBBL(this,{bound:[20,19,200,20],caption:"NIK Pemegang",multiSelection:false,tag:1,change:[this,"doChange"]});
		this.e_diskon = new saiLabelEdit(this,{bound:[720,19,200,20],caption:"Diskon", tag:1, tipeText:ttNilai, text:"0",change:[this,"doChange"]});		
		this.cb_pj = new portalui_saiCBBL(this,{bound:[20,18,220,20],caption:"No Panjar",multiSelection:false,tag:1,change:[this,"doChange"]});
		this.e_ppn = new saiLabelEdit(this,{bound:[720,18,200,20],caption:"Nilai PPN", tag:1, tipeText:ttNilai, text:"0",change:[this,"doChange"]});		
...
window.app_saku2_transaksi_kopeg_apotek_fBeliPtg.extend(window.portalui_childForm);
window.app_saku2_transaksi_kopeg_apotek_fBeliPtg.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into apo_brg_belibayar_d(no_bukti,no_beli,kode_lokasi,modul,periode,nik_user,tgl_input,dc,nilai) values "+
							"('"+this.e_nb.getText()+"','"+this.e_nb.getText()+"','"+this.app._lokasi+"','PJPTGBELI','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'D',"+parseNilai(this.e_total.getText())+")");
//------------------------------------------------- MODUL PANJAR ---------------------------------------------------------------
					var sls = nilaiToFloat(this.e_pj.getText()) - nilaiToFloat(this.e_total.getText());
					...
					sql.add("insert into panjarptg2_m (no_ptg,no_panjar,no_final,no_ver,no_dokumen,tanggal,keterangan,akun_panjar,nik_buat,nik_app,kode_lokasi,kode_pp,nilai_pj,nilai,nilai_kas,progress,periode,nik_user,tgl_input,modul) values "+
						    "('"+this.e_nb.getText()+"','"+this.cb_pj.getText()+"','-','-','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.akunpj+"','"+this.cb_nik.getText()+"','"+this.cb_nik.getText()+"','"+this.app._lokasi+"','"+this.app._kodePP+"',"+parseNilai(this.e_pj.getText())+","+parseNilai(this.e_total.getText())+","+sls+",'0','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'BELI')");
					...
					sql.add("insert into panjarptg2_j(no_ptg,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.akunHutang+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_total.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PJPTG','HUT','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
					sql.add("insert into panjarptg2_j(no_ptg,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',99,'"+this.akunpj+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_pj.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PJPTG','PANJAR','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");					
	...
	doChangeCell: function(sender, col, row){
		...
		if (col == 8 || col == 5 || col == 7) {
			if (this.sg.cells(8,row) != "" && this.sg.cells(7,row) != "" && this.sg.cells(5,row) != "" && this.sg.cells(5,row) != "0" && this.sg.cells(8,row) != "0") {
				this.sg.cells(4,row,floatToNilai((nilaiToFloat(this.sg.cells(8,row))+nilaiToFloat(this.sg.cells(7,row))) / nilaiToFloat(this.sg.cells(5,row))));
			}
		}		
		this.sg.validasi();
	},
	...
	doChange:function(sender){
		if (sender == this.cb_nik && this.cb_nik.getText()!="") {
			this.cb_pj.setText("","");
			this.cb_pj.setSQL("select no_panjar, keterangan from panjar2_m where nik_buat='"+this.cb_nik.getText()+"' and progress='2' and periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_panjar","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
		}
		if (sender == this.cb_pj && this.cb_pj.getText()!="") {
			...
			if (typeof data == "object"){
				...
				if (line != undefined){																				
					...
					this.e_pj.setText(floatToNilai(line.nilai));															
				} 
			}
		}
