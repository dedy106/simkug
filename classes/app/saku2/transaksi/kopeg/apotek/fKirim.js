window.app_saku2_transaksi_kopeg_apotek_fKirim = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_apotek_fKirim.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_apotek_fKirim";
		...
		try{			
			...
			this.cb_asal.setSQL("select kode_gudang, nama from apo_brg_gudang where kode_lokasi = '"+this.app._lokasi+"'",["kode_gudang","nama"],false,["Kode","Nama"],"and","Data Gudang Asal",true);
...
window.app_saku2_transaksi_kopeg_apotek_fKirim.extend(window.portalui_childForm);
window.app_saku2_transaksi_kopeg_apotek_fKirim.implement({
	...
	simpan: function(){			
		try{					
			if (this.standarLib.checkEmptyByTag(this, [0,2])){
				try{														
					this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"apo_brg_kirim_m","no_kirim",this.app._lokasi+"-KRM"+this.e_periode.getText().substr(2,4)+".","000"));
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "simpan" :	
				...
				for (var i=0;i < this.sg.getRowCount();i++){
					if (this.sg.rowValid(i)){
						var data = this.dbLib.getDataProvider("select stok from apo_brg_stok where kode_brg='"+this.sg.cells(0,i)+"'  and kode_gudang='"+this.cb_asal.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and nik_user='"+this.nik_user+"'",true);
