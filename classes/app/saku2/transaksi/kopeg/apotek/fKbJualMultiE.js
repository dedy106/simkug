window.app_saku2_transaksi_kopeg_apotek_fKbJualMultiE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_apotek_fKbJualMultiE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_apotek_fKbJualMultiE";
...
window.app_saku2_transaksi_kopeg_apotek_fKbJualMultiE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_apotek_fKbJualMultiE.implement({
	...
	doChange:function(sender){		
		if (sender == this.e_periode && this.e_periode.getText()!= "") {
			this.e_nb.setSQL("select no_kas, keterangan from kas_m where no_del='-' and modul = 'KBJUALAPOM' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kas","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);			
		}
		if (sender == this.e_nb && this.e_nb.getText()!= "") {			
			...
			strSQL = "select c.kode_cust,c.nama,b.no_jual,b.no_dokumen+' - '+b.keterangan as ket,d.kode_akun,a.nilai,a.nilai+a.sisa as bayar,a.sisa "+
			         "from apo_brg_jualbayar_d a inner join apo_brg_jual_m b on a.no_jual=b.no_jual and a.kode_lokasi=b.kode_lokasi "+
					 "                           inner join cust c on b.kode_cust=c.kode_cust and b.kode_lokasi=c.kode_lokasi "+
					 "							 inner join apo_brg_jual_j d on b.no_jual=d.no_jual and d.kode_lokasi=b.kode_lokasi and d.jenis='PIU' "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData([line.kode_cust,line.nama,line.no_jual,line.ket,line.kode_akun,floatToNilai(line.nilai),floatToNilai(line.bayar),floatToNilai(line.sisa)]);
				}
