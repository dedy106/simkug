window.app_saku2_transaksi_kopeg_apotek_fKbBeli = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_apotek_fKbBeli.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_apotek_fKbBeli";
...
window.app_saku2_transaksi_kopeg_apotek_fKbBeli.extend(window.childForm);
window.app_saku2_transaksi_kopeg_apotek_fKbBeli.implement({
	...
	doChange:function(sender){		
		if ((sender == this.e_periode || sender == this.cb_vendor) && (this.e_periode.getText()!= "" && this.cb_vendor.getText()!= "")) {
			var strSQL = "select a.no_beli,a.keterangan,a.kode_lokasi from apo_brg_beli_m a "+
						 "left join (select no_beli,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar from apo_brg_belibayar_d group by no_beli,kode_lokasi) b "+
						 "on a.no_beli=b.no_beli and a.kode_lokasi=b.kode_lokasi where a.modul='NON' and a.kode_vendor ='"+this.cb_vendor.getText()+"' and a.nilai>isnull(b.bayar,0) and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
