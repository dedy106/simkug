window.app_saku2_transaksi_kopeg_apotek_fBeliTunai = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_apotek_fBeliTunai.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_apotek_fBeliTunai";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pembelian Barang Tunai: Input", 0);	
		...
		this.e_nbkas = new portalui_saiLabelEdit(this,{bound:[20,14,200,20],caption:"No Bukti KB",maxLength:30,readOnly:true});		
		...
		try{			
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('OBTPOLI','OBTCAS','OBTMAT','PPNM','OBTHUT','OBTINV','OBTDIS') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku2_transaksi_kopeg_apotek_fBeliTunai.extend(window.portalui_childForm);
window.app_saku2_transaksi_kopeg_apotek_fBeliTunai.implement({	
