window.app_saku2_transaksi_kopeg_apotek_fBeli2 = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_apotek_fBeli2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_apotek_fBeli2";
		...
		this.e_nilai = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,12,200,20],caption:"Total", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,10,996,292], childPage:["Detail Item"]});
...
window.app_saku2_transaksi_kopeg_apotek_fBeli2.extend(window.portalui_childForm);
window.app_saku2_transaksi_kopeg_apotek_fBeli2.implement({		
	...
	simpan: function(){			
		try{					
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					if (this.stsSimpan == 1) this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"apo_brg_beli_m","no_beli",this.app._lokasi+"-BELI"+this.e_periode.getText().substr(2,4)+".","000"));
	...
	*/
	...
	doLoad3:function(sender){																				
		var strSQL = "select a.no_beli, convert(varchar,a.tanggal,103) as tgl, a.keterangan, c.nama as vendor, a.nilai "+
					 ...
					 "left join (select distinct no_beli,kode_lokasi from apo_brg_belibayar_d) b on a.no_beli=b.no_beli and a.kode_lokasi=b.kode_lokasi "+
					 "where a.modul='NON' and b.no_beli is null and a.posted='F' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' "+
