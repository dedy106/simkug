...
window.app_saku2_transaksi_kopeg_apotek_fBeliTunaiE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_apotek_fBeliTunaiE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_apotek_fBeliTunaiE";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pembatalan Pembelian Barang Tunai: Edit", 0);	
		...
		this.e_nbkas = new portalui_saiLabelEdit(this,{bound:[20,14,202,20],caption:"No Bukti KB",maxLength:30,readOnly:true});		
		...
		this.e_diskon = new saiLabelEdit(this,{bound:[720,13,200,20],caption:"Diskon", tag:1, tipeText:ttNilai, text:"0",readOnly:true});		
		this.e_ket = new portalui_saiLabelEdit(this,{bound:[20,14,503,20],caption:"Keterangan", maxLength:150});				
		this.e_ppn = new saiLabelEdit(this,{bound:[720,14,200,20],caption:"Nilai PPN", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
...
window.app_saku2_transaksi_kopeg_apotek_fBeliTunaiE.extend(window.portalui_childForm);
window.app_saku2_transaksi_kopeg_apotek_fBeliTunaiE.implement({	
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								var data = this.dbLib.getDataProvider("select round(stok,0) as stok from apo_brg_stok where kode_brg='"+this.sg.cells(0,i)+"'  and kode_gudang='"+this.cb_gudang.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and nik_user='"+this.nik_user+"'",true);
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText() != "") {						
			this.e_nb.setSQL("select a.no_beli, a.keterangan from apo_brg_beli_m a "+
							 "      inner join kas_m c on a.no_beli=c.no_link and a.kode_lokasi=c.kode_lokasi and c.posted ='F' "+							 
							 "where a.modul = 'TUNAI' and a.posted='F' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",["no_beli","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
		}		
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			...
			if (typeof data1 == "object" && data1.rs.rows[0] != undefined){
				...
				for (var i in data1.rs.rows){
					...
					this.sg.appendData([line1.kode_brg,line1.nama,line1.ket,line1.satuan,floatToNilai(line1.harga),floatToNilai(line1.diskon),floatToNilai(line1.jumlah),floatToNilai(line1.bonus),floatToNilai(line1.total),floatToNilai(line1.stok)]);
				}
