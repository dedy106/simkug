window.app_saku2_transaksi_kopeg_apotek_fKbBeliMulti = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_apotek_fKbBeliMulti.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_apotek_fKbBeliMulti";
...
window.app_saku2_transaksi_kopeg_apotek_fKbBeliMulti.extend(window.childForm);
window.app_saku2_transaksi_kopeg_apotek_fKbBeliMulti.implement({
	...
	doChangeCell: function(sender, col, row){
		if (col == 2 && this.sg.cells(2,i) != "") {
			var strSQL = "select (a.nilai -  isnull(b.bayar,0)) as saldo, c.kode_akun as akun_hutang,a.no_dokumen+' - '+a.keterangan as ket "+
						 ...
						 "where a.no_beli='"+this.sg.cells(2,row)+"' and a.kode_lokasi='"+this.app._lokasi+"'";
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){		
					this.sg.cells(4,row,line.akun_hutang);
			...
			if (sender == this.sg) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Vendor",sender,undefined, 
												  ...
												  "select count(*) from vendor where kode_lokasi = '"+this.app._lokasi+"'",
												  ["kode_vendor","nama"],"and",["Kode","Nama"],false);				
				}	
				if (col == 2 && sender.cells(0,row) != ""){
					var strSQL1 = "select a.no_beli,a.keterangan from apo_brg_beli_m a "+
								 "left join (select no_beli,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar from apo_brg_belibayar_d group by no_beli,kode_lokasi) b "+
								 "on a.no_beli=b.no_beli and a.kode_lokasi=b.kode_lokasi where a.modul = 'NON' and a.kode_vendor='"+this.sg.cells(0,row)+"' and a.nilai>isnull(b.bayar,0) and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
					var strSQL2 = "select count(*) from apo_brg_beli_m a "+
								 "left join (select no_beli,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar from apo_brg_belibayar_d group by no_beli,kode_lokasi) b "+
								 "on a.no_beli=b.no_beli and a.kode_lokasi=b.kode_lokasi where a.modul = 'NON' and a.kode_vendor='"+this.sg.cells(0,row)+"' and a.nilai>isnull(b.bayar,0) and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
								 
					this.standarLib.showListData(this, "Daftar Pembelian",sender,undefined, 
												  strSQL1,
												  strSQL2,
												  ["a.no_beli","a.keterangan"],"and",["Bukti","Deskripsi"],false);				
				}							
			}
		}catch(e){
			systemAPI.alert(e);
