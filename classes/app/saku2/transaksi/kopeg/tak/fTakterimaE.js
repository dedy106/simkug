window.app_saku2_transaksi_kopeg_tak_fTakterimaE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_tak_fTakterimaE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_tak_fTakterimaE";
		this.itemsValue = new arrayList();
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form TAK Umum Terima : Edit", 0);	
		...
		this.cb_asal = new saiCBBL(this,{bound:[20,18,220,20],caption:"Lokasi Kirim", multiSelection:false, maxLength:10, tag:1,readOnly:true});
		this.cb_kirim = new saiCBBL(this,{bound:[20,19,220,20],caption:"No TAK Kirim", multiSelection:false, maxLength:10, tag:1,readOnly:true});
		...
		this.pc1 = new pageControl(this,{bound:[20,12,900,260], childPage:["Akun TAK","Data Item Jurnal","Data Anggaran"]});
		...
		try {
			...
			this.cb_asal.setSQL("select kode_lokasi, nama from lokasi where kode_lokkonsol = '"+this.app._kodeLokasiKonsol+"' and flag_konsol='0' and kode_lokasi<>'"+this.app._lokasi+"'",["kode_lokasi","nama"],false,["Kode","Nama"],"and","Data Lokasi Asal",true);
...
window.app_saku2_transaksi_kopeg_tak_fTakterimaE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_tak_fTakterimaE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from angg_r where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul ='TUTERIMA'");
					...
					sql.add("insert into takterima_m(no_terima,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,tgl_input,nik_user,kode_lokkirim,no_kirim) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','-','TUTERIMA','TERIMA','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','F','-','-',getdate(),'"+this.app._userLog+"','"+this.cb_asal.getText()+"','"+this.cb_kirim.getText()+"')");					
		...
		switch (event){
			...
			case "ubah" :	
				...
				if (this.flagDokFree == "1") {				
					var data = this.dbLib.getDataProvider("select no_terima from takterima_m where no_terima <> '"+this.e_nb.getText()+"' and no_dokumen='"+this.e_dok.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
					if (typeof data == "object"){
			...
			case "hapus" :	
				...
				if (parseFloat(this.app._periode) > parseFloat(this.e_periode.getText())){
					...
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update takkirim_m set progress='0' where no_kirim='"+this.cb_kirim.getText()+"' and kode_lokasi='"+this.cb_asal.getText()+"'");
					sql.add("delete from takterima_m where no_terima = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from takterima_j where no_terima = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from angg_r where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul ='TUTERIMA'");
					setTipeButton(tbAllFalse);	
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!="") {
			this.e_nb.setSQL("select no_terima, keterangan from takterima_m where modul = 'TUTERIMA' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_terima","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti TAK",true);
		}
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			var data = this.dbLib.getDataProvider(
					   "select convert(varchar,a.tanggal,103) as tanggal,a.periode,a.no_dokumen,a.keterangan,a.kode_lokkirim,a.nik_buat,a.nik_setuju,b.nama as nama_buat,c.nama as nama_app,d.nama as lokkirim,a.no_kirim,e.keterangan as ket "+
					   "from takterima_m a inner join karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+
					   "                   inner join karyawan c on a.nik_setuju=c.nik and a.kode_lokasi=c.kode_lokasi "+
					   "                   inner join lokasi d on a.kode_lokkirim=d.kode_lokasi "+
					   "                   inner join takkirim_m e on a.no_kirim=e.no_kirim and a.kode_lokkirim=e.kode_lokasi "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){
					...
					this.cb_asal.setText(line.kode_lokkirim,line.lokkirim);
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					...
					else this.sg3.appendData([line.kode_akun,line.nama_akun,line.dc,line.keterangan,floatToNilai(line.nilai)]);
				}
			} 
			else {
				this.sg.clear(1);
			...
			var data = this.dbLib.getDataProvider(
						...
						"              inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
						"              left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and d.tahun='"+this.e_periode.getText().substr(0,4)+"' "+
						"where a.no_bukti = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='TUTERIMA' order by a.kode_akun",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg2.clear();
				for (var i in data.rs.rows){
