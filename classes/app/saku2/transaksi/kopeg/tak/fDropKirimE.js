window.app_saku2_transaksi_kopeg_tak_fDropKirimE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_tak_fDropKirimE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_tak_fDropKirimE";
		...
		this.e_nilai = new saiLabelEdit(this,{bound:[720,17,202,20],caption:"Total Droping", readOnly:true,tipeText:ttNilai, text:"0"});		
		
		this.pc1 = new pageControl(this,{bound:[20,12,900,230], childPage:["Lokasi Penerima","Dokumen Pendukung"]});
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:4,tag:0,
					...
					colWidth:[[3,2,1,0],[100,400,250,80]],columnReadOnly:[true,[1],[0,2,3]],
					buttonStyle:[[0],[bsEllips]], colFormat:[[3],[cfNilai]],
		...
		try {
			...
			sql.add("select kode_lokasi, nama from lokasi where kode_lokasi <> '"+this.app._kodeLokasiKonsol+"' and flag_konsol='0' and kode_lokasi<>'"+this.app._lokasi+"'");
...
window.app_saku2_transaksi_kopeg_tak_fDropKirimE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_tak_fDropKirimE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from tak_kasdrop_d where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.cb_kas.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBDROP','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.cb_app.getText()+"',getdate(),'"+this.app._userLog+"','F','-','-','-','-')");
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into tak_kasdrop_d(no_kas,no_urut,no_dokumen,kode_lokasi,periode,kode_loktuj,keterangan,nilai,progress,akun_tak,no_kasterima) values "+
										"	('"+this.e_nb.getText()+"',"+i+",'"+this.e_dok.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.sg.cells(0,i)+"','"+this.sg.cells(2,i)+"','"+parseNilai(this.sg.cells(3,i))+"','0','"+this.cb_tak.getText()+"','-')");
							}
						}
					}
					var ix=0;
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i)){
							ix++;
							sql.add("insert into dok_load(no_bukti,modul,periode,no_gambar,nu,kode_jenis,kode_lokasi)values('"+this.e_nb.getText()+"','KBDROP','"+this.e_periode.getText()+"','"+this.sgUpld.cells(3,i).filedest+"','"+ix+"','"+this.sgUpld.cells(0,i)+"','"+this.app._lokasi+"')");								
						}	
					}	
	...
	doChange:function(sender){	
		if (sender == this.e_periode && this.e_periode.getText()!="") {
			this.e_nb.setSQL("select distinct a.no_kas, a.keterangan from kas_m a left join (select no_kas,kode_lokasi,no_kasterima from tak_kasdrop_d where progress='1') b on a.no_kas=b.no_kas and a.kode_lokasi=b.kode_lokasi  "+
		...
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			...
			var data = this.dbLib.getDataProvider(
						...
						"from tak_kasdrop_d a inner join lokasi b on a.kode_loktuj=b.kode_lokasi "+
						"where a.no_kas = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
			...
			var data = this.dbLib.getDataProvider("select b.kode_jenis,b.nama,a.no_gambar from dok_load a inner join dok_jenis b on a.kode_jenis=b.kode_jenis and a.kode_lokasi=b.kode_lokasi "+
					   "where a.no_bukti = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='KBDROP' order by a.nu",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sgUpld.clear();
				for (var i in data.rs.rows){
			...
			if (sender == this.sg) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Lokasi Tujuan",sender,undefined, 					
												  "select kode_lokasi, nama from lokasi where kode_lokasi <> '"+this.app._kodeLokasiKonsol+"' and flag_konsol='0' and kode_lokasi<>'"+this.app._lokasi+"'",
												  "select count(kode_lokasi) from lokasi where kode_lokasi <> '"+this.app._kodeLokasiKonsol+"' and flag_konsol='0' and kode_lokasi<>'"+this.app._lokasi+"'",
