window.app_saku2_transaksi_kopeg_tak_fTakterima = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_tak_fTakterima.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_tak_fTakterima";
		...
		this.e_debet = new saiLabelEdit(this,{bound:[800,14,220,20],caption:"Total Debet", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		this.l_tgl1 = new portalui_label(this,{bound:[20,11,100,18],caption:"Tanggal", underline:true});
		this.dp_d1 = new portalui_datePicker(this,{bound:[120,11,100,18],selectDate:[this,"doSelectDate"]}); 
		this.e_kredit = new saiLabelEdit(this,{bound:[800,11,220,20],caption:"Total Kredit", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		
		this.pc1 = new pageControl(this,{bound:[20,12,1000,420], childPage:["Daftar TAK Kirim","Detail TAK","Data Item Jurnal","Data Anggaran"]});
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:7,tag:0,
		            colTitle:["Lok Kirim","Nama Lokasi","No Kirim","Tanggal","No Dokumen","Keterangan","Pengirim"],
					colWidth:[[6,5,4,3,2,1,0],[200,300,150,80,100,150,80]],readOnly:true,dblClick:[this,"doDoubleClick"],autoAppend:false,defaultRow:1});		
		...
		this.e_nodok = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,11,450,20],caption:"No Dokumen", readOnly:true});		
		this.e_ket2 = new saiLabelEdit(this.pc1.childPage[1],{bound:[520,11,450,20],caption:"Deskripsi",readOnly:true});				
		this.e_lokasal = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,18,450,20],caption:"Lokasi Asal", readOnly:true});		
		this.e_pengirim = new saiLabelEdit(this.pc1.childPage[1],{bound:[520,18,450,20],caption:"Pengirim",readOnly:true});		
		this.e_nokirim = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,19,450,20],caption:"No Kirim",readOnly:true});		
		this.e_tgl = new saiLabelEdit(this.pc1.childPage[1],{bound:[520,19,450,20],caption:"Tgl Kirim",readOnly:true});		
		
		this.sg3 = new saiGrid(this.pc1.childPage[1],{bound:[1,10,this.pc1.width-5,this.pc1.height-235],colCount:5,tag:0,
		            colTitle:["Kode Akun","Nama Akun","DC","Keterangan","Nilai"],
					colWidth:[[4,3,2,1,0],[100,400,50,300,80]],
					readOnly : true,colFormat:[[4],[cfNilai]],nilaiChange:[this,"doNilaiChange"],
					autoAppend:false,defaultRow:1});
		
		this.sg4 = new saiGrid(this.pc1.childPage[1],{bound:[1,20,this.pc1.width-5,this.pc1.height-300],colCount:2,tag:0,
		            colTitle:["Jenis Dokumen","File Dokumen"],
					colWidth:[[1,0],[500,430]],readOnly : true,autoAppend:false,defaultRow:1});
							
		this.e_nb = new portalui_saiLabelEdit(this.pc1.childPage[2],{bound:[20,12,202,20],caption:"No Terima",maxLength:30,readOnly:true});
		this.i_gen = new portalui_imageButton(this.pc1.childPage[2],{bound:[225,12,20,20],hint:"Generate",image:"icon/"+system.getThemes()+"/tabCont2.png",click:[this,"doClick"]});		
		this.e_ket = new saiLabelEdit(this.pc1.childPage[2],{bound:[20,17,450,20],caption:"Deskripsi", maxLength:150});				
		this.e_dok = new saiLabelEdit(this.pc1.childPage[2],{bound:[520,17,450,20],caption:"No Dokumen", maxLength:50});				
		
		this.sg = new saiGrid(this.pc1.childPage[2],{bound:[1,10,this.pc1.width-5,this.pc1.height-90],colCount:9,tag:0, 
		...
		try {
			...
			sql.add("select a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '043' where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'");
...
window.app_saku2_transaksi_kopeg_tak_fTakterima.extend(window.childForm);
window.app_saku2_transaksi_kopeg_tak_fTakterima.implement({
		...
		try{									
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					var sql = new server_util_arrayList();
					sql.add("update takkirim_m set progress='1' where no_kirim='"+this.e_nokirim.getText()+"' and kode_lokasi='"+this.lokasal+"'");
					sql.add("insert into takterima_m(no_terima,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,tgl_input,nik_user,kode_lokkirim,no_kirim) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','-','TUTERIMA','TERIMA','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"','F','-','-',getdate(),'"+this.app._userLog+"','"+this.lokasal+"','"+this.e_nokirim.getText()+"')");					
						...
						for (var i=0;i < this.sg3.getRowCount();i++){
							if (this.sg3.rowValid(i)){
								sql.add("insert into takterima_j(no_terima,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"	('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg3.cells(0,i)+"','"+this.sg3.cells(3,i)+"','"+this.sg3.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg3.cells(4,i))+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','TUTERIMA','TAK','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");
							}
						}
					}
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into takterima_j(no_terima,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"	('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','"+this.app._lokasi+"','TUTERIMA','TERIMA','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");
							}
						}
					}
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','TUTERIMA','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(6,i))+","+nilai+")");
							}
						}
					}					
		...
		switch (event){
			...
			case "simpan" :	
				if (this.flagDokFree == "1") {				
					var data = this.dbLib.getDataProvider("select no_terima from takterima_m where no_dokumen='"+this.e_dok.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
						if (line != undefined){
							system.alert(this,"No Dokumen sudah terpakai.","Terpakai di no bukti : "+line.no_terima);
							return false;
						} 
					}
				}
					...
					if (this.sg.rowValid(j)){
						for (var i=0;i<this.dataJU.rs.rows.length;i++){
							...
							if (line.kode_akun == this.sg.cells(0,j)) {
								system.alert(this,"Transaksi tidak valid.","Akun KasBank tidak boleh dipakai di form ini.");
								return false;						
							}
						}
					}
			...
			if (sender == this.sg) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Akun",sender,undefined, 
							  "select a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '043' where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'",
							  "select count(a.kode_akun) from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '043' where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'",
	...
	doLoad:function(sender){						
		var strSQL = "select a.kode_lokasi,b.nama as nama_lokasi,a.no_kirim,convert(varchar,tanggal,103) as tanggal,a.no_dokumen,a.keterangan,a.nik_buat+'-'+c.nama as pengirim "+
		             "from takkirim_m a inner join lokasi b on a.kode_lokasi=b.kode_lokasi "+
					 "                  inner join karyawan c on a.nik_buat = c.nik and a.kode_lokasi=c.kode_lokasi "+
					 "where a.modul = 'TUKIRIM' and a.periode<='"+this.e_periode.getText()+"' and a.progress = '0' and a.kode_loktuj='"+this.app._lokasi+"' order by a.kode_lokasi,a.no_kirim";					
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU.rs.rows[i];													
			this.sg1.appendData([line.kode_lokasi,line.nama_lokasi,line.no_kirim,line.tanggal,line.no_dokumen,line.keterangan,line.pengirim]);
		}
	...
	doDoubleClick: function(sender, col , row) {
		try{
			...
			if (this.sg1.cells(0,row) != "") {							
				...
				this.e_lokasal.setText(this.sg1.cells(0,row) + " - "+this.sg1.cells(1,row));			
				this.e_pengirim.setText(this.sg1.cells(6,row));			
				this.e_nokirim.setText(this.sg1.cells(2,row));			
				this.e_tgl.setText(this.sg1.cells(3,row));			
				this.e_ket2.setText(this.sg1.cells(5,row));			
				this.e_nodok.setText(this.sg1.cells(4,row));			
				this.lokasal = this.sg1.cells(0,row);
				this.e_ket.setText(this.sg1.cells(5,row));			
				
				var strSQL = "select a.kode_akun,d.nama,case a.dc when 'D' then 'C' else 'D' end as dc,a.keterangan,a.nilai "+
			             "from takkirim_j a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
						 "                  inner join takkirim_m c on a.no_kirim=c.no_kirim and a.kode_lokasi=c.kode_lokasi "+
						 "                  inner join masakun d on a.kode_akun=d.kode_akun and a.kode_lokasi=d.kode_lokasi "+
						 "where a.no_kirim='"+this.e_nokirim.getText()+"' and c.kode_loktuj = '"+this.app._lokasi+"' and b.kode_flag ='016' ";
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg3.clear();
				...
				var data = this.dbLib.getDataProvider("select b.kode_jenis+' - '+b.nama as jenis,a.no_gambar from dok_load a inner join dok_jenis b on a.kode_jenis=b.kode_jenis and a.kode_lokasi=b.kode_lokasi "+
						   "where a.no_bukti = '"+this.e_nokirim.getText()+"' and a.kode_lokasi='"+this.lokasal+"' and a.modul='TUKIRIM' order by a.nu",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg4.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];													
						this.sg4.appendData([line.jenis,line.no_gambar]);
					}
