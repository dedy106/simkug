window.app_saku2_transaksi_kopeg_gl_fClosingThnSju = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_gl_fClosingThnSju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_gl_fClosingThnSju";
...
window.app_saku2_transaksi_kopeg_gl_fClosingThnSju.extend(window.childForm);
window.app_saku2_transaksi_kopeg_gl_fClosingThnSju.implement({
				...
				try{														
					...
					if (this.isPostedAll(this.app._periode)){						
						sql.add("insert into glma(kode_akun, kode_lokasi, periode, so_akhir, tgl_input,nik_user)  "+
								"select kode_akun,kode_lokasi,'"+closePeriode(this.app._periode,this.maxPeriode)+"',round(so_akhir,2),getdate(),'"+this.app._userLog+"' "+
	...
	isPostedAll: function(periode){
		...
		var msg  = "";
		//--------------------------- control modul		
		var sqlSUSUT = "select count(a.no_fa) from gl_fa_asset a "+
					   "   left join "+
					   ...
					   "where a.nilai_susut<>0 and a.progress = '2' and a.kode_lokasi='"+this.app._lokasi+"' and (a.nilai-a.nilai_residu) > isnull(d.tot_susut,0) and a.periode_susut = '"+periode+"'";
		...
		var sqlPROD = "select count(x.no_bukti) from ("+
				     "  select no_ptg as no_bukti from ptg_m where posted = 'F' and periode ='"+periode+"' and kode_lokasi='"+this.app._lokasi+"' "+
				     ") x";					   
		var data,ck = this.dbLib.loadQuery(sqlPROD);
		if (ck != "") { 
			data = ck.split("\r\n"); 
			if (data[1] != "0"){
				result = result && false;
				msg+= "Data Modul PRODUKSI [BILL] masih ada yang belum diposting.("+data[1]+" data)\n";
			}
		}
