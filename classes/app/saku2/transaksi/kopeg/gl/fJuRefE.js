window.app_saku2_transaksi_kopeg_gl_fJuRefE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_gl_fJuRefE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_gl_fJuRefE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Jurnal Umum Gar Bulanan: Edit", 0);	
		...
		this.c_jenis = new saiCB(this,{bound:[20,22,202,20],caption:"Jenis",items:["REF"], readOnly:true, change:[this,"doChange"], tag:2});		
...
window.app_saku2_transaksi_kopeg_gl_fJuRefE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_gl_fJuRefE.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from dok_load where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul ='JU'");
					...
					sql.add("insert into ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','-','JUREF','"+this.c_jenis.getText()+"','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','F','-','-','-',getdate(),'"+this.app._userLog+"')");
					...
					sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
							"	('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','"+this.c_dc.getText()+"',"+parseNilai(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','-','-','-','-','-','-','"+this.app._lokasi+"','JU','AKUNREF','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','-')");									
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					this.c_jenis.setText("REF");
					setTipeButton(tbUbahHapus);
				break;
			case "ubah" :					
				this.cb1.setSelected(true);
				if (this.flagDokFree == "1") {				
					var data = this.dbLib.getDataProvider("select no_ju from ju_m where no_dokumen='"+this.e_dok.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and no_ju<>'"+this.e_nb.getText()+"'",true);
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
						if (line != undefined){
	...
	doChange:function(sender){
		...
		if (sender == this.e_periode && this.e_periode.getText()!="") {
			this.e_nb.setSQL("select no_ju, keterangan from ju_m where modul = 'JUREF' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_ju","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
		}
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			var data = this.dbLib.getDataProvider("select d.dc,d.nilai,d.kode_akun,a.no_dokumen,convert(varchar,a.tanggal,103) as tanggal,a.periode,a.keterangan,a.jenis,a.nik_buat,b.nama as nama_buat,a.nik_setuju,c.nama as nama_setuju "+
					   ...
					   "			inner join ju_j d on a.no_ju=d.no_ju and a.kode_lokasi=d.kode_lokasi and d.jenis='AKUNREF' "+
			...
			var data = this.dbLib.getDataProvider(
						...
						"from ju_j a inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and a.jenis <> 'AKUNREF' "+
