window.app_saku2_transaksi_kopeg_gl_fClosingSju = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_gl_fClosingSju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_gl_fClosingSju";
...
window.app_saku2_transaksi_kopeg_gl_fClosingSju.extend(window.childForm);
window.app_saku2_transaksi_kopeg_gl_fClosingSju.implement({
	...
	isPostedAll: function(periode){
		...
		var sqlPROD = "select count(x.no_bukti) from ("+
				     "  select no_bill as no_bukti from sju_bill_m where posted = 'F' and periode ='"+periode+"' and kode_lokasi='"+this.app._lokasi+"' "+
				     ") x";					   
