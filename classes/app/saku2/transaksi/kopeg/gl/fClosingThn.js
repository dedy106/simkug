window.app_saku2_transaksi_kopeg_gl_fClosingThn = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_gl_fClosingThn.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_gl_fClosingThn";
...
window.app_saku2_transaksi_kopeg_gl_fClosingThn.extend(window.childForm);
window.app_saku2_transaksi_kopeg_gl_fClosingThn.implement({
	...
	isPostedAll: function(periode){
		...
		if (ck != "") { 
			...
			if (data[1] != "0"){
				result = result && false;
				msg+= "Data Modul PIUTANG [PIUUM,SISIHPIUM] masih ada yang belum diposting.("+data[1]+" data)\n";
			}
		}
