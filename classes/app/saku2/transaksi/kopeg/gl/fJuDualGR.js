window.app_saku2_transaksi_kopeg_gl_fJuDualGR = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_gl_fJuDualGR.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_gl_fJuDualGR";
		...
		try {
			...
			this.cb_jurnal.setSQL("select kode_jurnal, nama from refju_dual where modul = 'GL' and kode_lokasi='"+this.app._lokasi+"'",["kode_jurnal","nama"],false,["Kode","Nama"],"and","Data Referensi",true);
...
window.app_saku2_transaksi_kopeg_gl_fJuDualGR.extend(window.childForm);
window.app_saku2_transaksi_kopeg_gl_fJuDualGR.implement({	
