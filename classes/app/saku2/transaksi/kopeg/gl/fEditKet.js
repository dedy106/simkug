window.app_saku2_transaksi_kopeg_gl_fEditKet = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_gl_fEditKet.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_gl_fEditKet";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Edit Keterangan After Closing: Input", 0);	
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:9,tag:0,
		            colTitle:["No Bukti","Kode DRK","Kode Akun","Uraian","Nominal","Kode PP","User","Periode","ID"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[50,70,150,70,100,250,150,80,100]],
		...
		this.e_ket = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,16,450,20],caption:"Deskripsi", maxLength:150});								
		this.cb_drk = new saiCBBL(this.pc1.childPage[1],{bound:[20,11,200,20],caption:"DRK",readOnly:true});		
		...
		this.e_total = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,17,200,20],caption:"Nilai", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
...
window.app_saku2_transaksi_kopeg_gl_fEditKet.extend(window.childForm);
window.app_saku2_transaksi_kopeg_gl_fEditKet.implement({
		...
		try{					
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("update gldt_h set keterangan='"+this.e_ket.getText()+"' where no_urut="+this.nu+" and no_bukti='"+this.e_noaju.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and kode_akun='"+this.e_akun.getText()+"' and kode_pp='"+this.e_pp.getText()+"' and kode_drk='"+this.cb_drk.getText()+"'");					
	...
	doDoubleClick: function(sender, col , row) {
		if (this.sg.cells(0,row) != "") {						
			...
			this.cb_drk.setText(this.sg.cells(1,row));						
			this.nu = this.sg.cells(8,row);			
		}
	},	
	doCari:function(sender){				
		...
		var strSQL = "select a.no_bukti,a.kode_drk,a.kode_akun as akun,a.keterangan,a.nilai,a.kode_pp as pp,d.nik+' - '+d.nama as nik_user,a.periode,a.no_urut "+
		             "from gldt_h a "+					 		             
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData([line.no_bukti,line.kode_drk,line.akun,line.keterangan,floatToNilai(line.nilai),line.pp,line.nik_user,line.periode,line.no_urut]);
		}
