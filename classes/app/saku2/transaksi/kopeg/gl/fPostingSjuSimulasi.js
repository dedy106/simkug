window.app_saku2_transaksi_kopeg_gl_fPostingSjuSimulasi = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_gl_fPostingSjuSimulasi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_gl_fPostingSjuSimulasi";
...
window.app_saku2_transaksi_kopeg_gl_fPostingSjuSimulasi.extend(window.childForm);
window.app_saku2_transaksi_kopeg_gl_fPostingSjuSimulasi.implement({
		...
		try{									
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("exec sp_post_modul_simulasi '"+this.e_periode.getText()+"'");
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							this.nama_report="server_report_saku2_kopeg_sju_rptGlPostingSimulasi";
