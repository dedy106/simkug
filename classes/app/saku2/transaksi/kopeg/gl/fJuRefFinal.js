window.app_saku2_transaksi_kopeg_gl_fJuRefFinal = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_gl_fJuRefFinal.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_gl_fJuRefFinal";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Jurnal Umum Finalisasi Ref: Input", 0);	
		
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		this.e_periode = new portalui_saiLabelEdit(this,{bound:[20,14,202,20],caption:"Periode",tag:2,readOnly:true});
		this.l_tgl1 = new portalui_label(this,{bound:[20,11,100,18],caption:"Tanggal", underline:true});
		this.dp_d1 = new portalui_datePicker(this,{bound:[120,11,100,18],selectDate:[this,"doSelectDate"]}); 
		this.c_jenis = new saiCB(this,{bound:[20,22,202,20],caption:"Jenis",items:["ADJ"], readOnly:true,tag:2});
		...
		this.cb_ref = new saiCBBL(this.pc1.childPage[0],{bound:[20,10,220,20],caption:"No Referensi", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});
		...
		try {
			...
			var strSQL = "select a.no_ju,a.keterangan "+
			             "from ju_m a "+
			...
			this.cb_ref.setSQL(strSQL,["a.no_ju","a.keterangan"],false,["No Bukti","Keterangan"],"and","Data Jurnal Ref",true);
...
window.app_saku2_transaksi_kopeg_gl_fJuRefFinal.extend(window.childForm);
window.app_saku2_transaksi_kopeg_gl_fJuRefFinal.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','-','JUADJ','"+this.c_jenis.getText()+"','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','F','-','-','"+this.cb_ref.getText()+"',getdate(),'"+this.app._userLog+"')");										
					sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
							"	('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','"+dcRef+"',"+parseNilai(this.e_pakai.getText())+",'"+this.app._kodePP+"','-','-','-','-','-','-','-','"+this.app._lokasi+"','JUADJ','AKUNREF','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'"+this.cb_ref.getText()+"','-')");
					
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
										"	('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','-','-','-','-','-','-','"+this.app._lokasi+"','JUADJ','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'"+this.cb_ref.getText()+"','-')");
							}
						}
					}
					...
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i)){
							ix++;
							sql.add("insert into dok_load(no_bukti,modul,periode,no_gambar,nu,kode_jenis,kode_lokasi)values('"+this.e_nb.getText()+"','JU','"+this.e_periode.getText()+"','"+this.sgUpld.cells(3,i).filedest+"','"+ix+"','"+this.sgUpld.cells(0,i)+"','"+this.app._lokasi+"')");								
						}	
					}
					if (this.flagFAGL == "1") {
	...
	doChange:function(sender){		
		if (sender == this.e_pakai && this.e_pakai.getText()!="") {
			...
			sender.onChange.set(this,"doChange");
			this.sg.validasi();
		}
		if (sender == this.cb_ref && this.cb_ref.getText()!="") {
			var strSQL = "select c.kode_akun,d.nama as nama_akun,c.dc,c.nilai,c.nilai-isnull(b.pakai,0)-isnull(e.pakai,0) as sisa "+
						 ...
						 "     left join ( "+
						 "			select no_ref,kode_akun,kode_lokasi,sum(nilai) as pakai "+
						 "          from ju_j where no_ref='"+this.cb_ref.getText()+"' and modul='JUADJ' and jenis='AKUNREF' and kode_lokasi='"+this.app._lokasi+"' "+
						 "          group by no_ref,kode_akun,kode_lokasi "+
						 "                ) b on c.no_ju=b.no_ref and c.kode_lokasi=b.kode_lokasi and c.kode_akun=b.kode_akun "+
						 "     left join ( "+
						 "			select ref1,kode_akun,kode_lokasi,sum(nilai) as pakai "+
						 "          from kas_j where ref1='"+this.cb_ref.getText()+"' and modul='KBADJ' and jenis='AKUNREF' and kode_lokasi='"+this.app._lokasi+"' "+
						 "          group by ref1,kode_akun,kode_lokasi "+
						 "                ) e on c.no_ju=e.ref1 and c.kode_lokasi=e.kode_lokasi and c.kode_akun=e.kode_akun "+
						 "where a.no_ju='"+this.cb_ref.getText()+"' and a.modul = 'JUREF' and a.kode_lokasi ='"+this.app._lokasi+"' "+
						 "union "+
						 ...
						 "where a.no_kas='"+this.cb_ref.getText()+"' and a.modul = 'KBREF' and a.kode_lokasi ='"+this.app._lokasi+"' ";
