window.app_saku2_transaksi_kopeg_gl_fPostingSju = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_gl_fPostingSju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_gl_fPostingSju";
		...
		this.bPostAll = new button(this,{bound:[700,17,100,18],caption:"Posting Semua",click:[this,"doClick"]});
		this.bLoad = new button(this,{bound:[830,17,80,18],caption:"Load Data",click:[this,"doLoad"]});			
		this.bPost = new button(this,{bound:[930,17,80,18],caption:" Flag Posting",click:[this,"doClick"]});			
		
		this.pc1 = new pageControl(this,{bound:[20,12,1000,357], childPage:["Modul","Data Transaksi Modul","Detail Transaksi","Filter Data","Keterangan"]});
		this.e_memo = new saiMemo(this.pc1.childPage[4],{bound:[5,24,690,280],labelWidth:0,tag:9,readOnly:true});
...
window.app_saku2_transaksi_kopeg_gl_fPostingSju.extend(window.childForm);
window.app_saku2_transaksi_kopeg_gl_fPostingSju.implement({
	...
	simpan_all: function(){			
		try{									
			...
			var sql = new server_util_arrayList();
			sql.add("insert into posting_m(no_post,kode_lokasi,periode,tanggal,modul,keterangan,nik_buat,nik_app,no_del,tgl_input,nik_user,nilai) values "+
				    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.app._periode+"','"+this.dp_d1.getDateString()+"','-','Posting Periode "+this.e_periode.getText()+"','"+this.app._userLog+"','"+this.app._userLog+"','-',getdate(),'"+this.app._userLog+"',0)");
			sql.add("call sp_post_all ('"+this.app._periode+"','"+this.app._lokasi+"','"+this.e_nb.getText()+"')");
			setTipeButton(tbAllFalse);					
			this.dbLib.execArraySQL(sql);
		...
		switch (event){
			...
			case "simpan" :	
				...
				var strSQL = "select no_bukti+' - '+periode as bukper from ("+
						 ...
						 "from sju_bill_j a inner join sju_bill_m b on a.no_bill=b.no_bill and a.kode_lokasi=b.kode_lokasi and b.posted='F' "+
			...
			if (this.sg1.rowValid(i) && this.sg1.cells(0,i)=="TRUE") {
				...
				if (this.sg1.cells(1,i) == "PROD") 
					strSQL += "union all "+
							  ...
							  "from sju_bill_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";
	...
	doChange:function(sender){
		...
		if (sender == this.c_modul && this.c_modul.getText()!="") {
			...
			if (this.c_modul.getText()=="PROD") this.cb_bukti.setSQL("select no_bill as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'BILL' as jenis from sju_bill_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ",["no_bukti","no_dokumen","tanggal","keterangan","jenis"],false,["No Bukti","No Dokumen","Tanggal","Keterangan","Jenis"],"and","Data Bukti",true);
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							this.nama_report="server_report_saku3_gl_rptGlPosting";
