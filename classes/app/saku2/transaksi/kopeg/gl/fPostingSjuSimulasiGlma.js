window.app_saku2_transaksi_kopeg_gl_fPostingSjuSimulasiGlma = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_gl_fPostingSjuSimulasiGlma.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_gl_fPostingSjuSimulasiGlma";
		...
		try {
			...
			this.periode=this.dbLib.getPeriodeFromSQL("select min(periode) as periode from periode where substring(periode,1,4)='"+this.app._periode.substr(0,4)+"'");
...
window.app_saku2_transaksi_kopeg_gl_fPostingSjuSimulasiGlma.extend(window.childForm);
window.app_saku2_transaksi_kopeg_gl_fPostingSjuSimulasiGlma.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					$sql="select a.periode "+
						"from (select distinct periode from dbsju.dbo.ju_j where periode>='"+this.periode+"' "+
						"union "+
						"select distinct periode from dbsju.dbo.kas_j where periode>='"+this.periode+"' "+
						"union "+
						"select distinct periode from dbsju.dbo.sju_bill_j where periode>='"+this.periode+"' "+
						"	)a";
