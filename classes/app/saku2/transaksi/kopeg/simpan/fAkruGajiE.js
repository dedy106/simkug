window.app_saku2_transaksi_kopeg_simpan_fAkruGajiE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_simpan_fAkruGajiE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_simpan_fAkruGajiE";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Akru Simpanan Status Gaji: Edit", 0);	
...
window.app_saku2_transaksi_kopeg_simpan_fAkruGajiE.extend(window.portalui_childForm);
window.app_saku2_transaksi_kopeg_simpan_fAkruGajiE.implement({
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText() != "") {
			this.e_nb.setSQL("select no_bill, keterangan from kop_simpbill_m where modul = 'PGAJI' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_bill","keterangan"],false,["No Bukti","Keterangan"],"and","Data Bukti",true);
		}
		if (sender == this.e_nb && this.e_nb.getText()!="") {			
			...
			var strSQL = "select xx.kode_loker,xx.nama as nama_loker,x.jenis,a.kode_simp+' - '+a.nama as nama_simp,a.akun_ar,b.nama as nama_ar,a.akun_simp,c.nama as nama_asimp,sum(x.nilai) as total "+
					   ...
					   "      inner join kop_simp_d zz on x.no_simp=zz.no_simp and x.kode_lokasi=zz.kode_lokasi "+
					   " where zz.no_bill ='"+this.e_nb.getText()+"' and x.kode_lokasi = '"+this.app._lokasi+"' "+			   
	...
	doDblClick: function(sender, col, row){
		if (this.sg2.cells(0,row) != "") {			
			var strSQL = "select x.no_simp,convert(varchar,x.tgl_tagih,103) as tgl_tagih,y.kode_agg,y.nama as nama_agg,x.jenis,a.kode_simp+' - '+a.nama as nama_simp,a.akun_ar,a.akun_simp,x.nilai "+
					     ...
					     "      ((x.jenis in ('SP','SW')) or (x.jenis='SS' and x.status_bayar='GAJI')) "+
