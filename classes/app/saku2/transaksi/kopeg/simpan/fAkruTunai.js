window.app_saku2_transaksi_kopeg_simpan_fAkruTunai = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_simpan_fAkruTunai.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_simpan_fAkruTunai";
		...
		this.cb_agg = new portalui_saiCBBL(this,{bound:[20,17,200,20],caption:"Anggota",tag:2,multiSelection:false,change:[this,"doChange"]});				
		this.cb_simp = new portalui_saiCBBL(this,{bound:[20,18,222,20],caption:"No Kartu",tag:2,multiSelection:false,change:[this,"doChange"]});				
		this.e_nilai = new portalui_saiLabelEdit(this,{bound:[710,18,200,20],caption:"Nilai Akru",tipeText:ttNilai,text:"0",readOnly: true});
		this.bTampil = new portalui_button(this,{bound:[610,18,80,18],caption:"Tampil Data",click:[this,"doTampilClick"]});		
		...
		try{			
			...
			this.cb_agg.setSQL("select kode_agg, nama from kop_agg where status = 'AKTIF' and kode_lokasi='"+this.app._lokasi+"'",["kode_agg","nama"],false,["Kode","Nama"],"and","Data Anggota",true);
...
window.app_saku2_transaksi_kopeg_simpan_fAkruTunai.extend(window.portalui_childForm);
window.app_saku2_transaksi_kopeg_simpan_fAkruTunai.implement({
	...
	doChange:function(sender){
		if (sender == this.cb_agg && this.cb_agg.getText()!="") {
			this.sg1.clear(1);
			this.cb_simp.setSQL("select a.no_simp, b.nama as kode_simp  from kop_simp_m a inner join kop_simp_jenis b on a.kode_simp=b.kode_simp and a.kode_lokasi=b.kode_lokasi where a.kode_lokasi='"+this.app._lokasi+"' and a.kode_agg='"+this.cb_agg.getText()+"'",["no_simp","kode_simp"],false,["No Simp","Jenis"],"and","Data Simpanan",true);
		}		
		if (sender == this.cb_simp) this.sg1.clear(1);
	},
	doTampilClick: function(sender){
		try{			
			 //if kosong data simpanan seluruhnya per anggota	
			 //data yang tampil seluruhnya, meskipun statusnya sudah nonaktif  (untuk SP pun tampil lagi, misal untuk yg kekurangannya)
			if (this.cb_agg.getText() != "") { 
				...
				system.alert(this,"Anggota tidak valid.","Anggota harus dipilih.");
				this.sg1.clear(1);
			}
		}catch(e){
			systemAPI.alert(e);
