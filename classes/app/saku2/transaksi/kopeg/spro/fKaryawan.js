window.app_saku2_transaksi_kopeg_spro_fKaryawan = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_spro_fKaryawan.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_spro_fKaryawan";
		...
		this.e_jab = new saiLabelEdit(this,{bound:[20,13,300,20],caption:"Jabatan", maxLength:50, tag:1});							
		this.e_grade = new saiLabelEdit(this,{bound:[20,14,200,20],caption:"Grade", maxLength:20, tag:1});							
		...
		this.e_cabang = new saiLabelEdit(this,{bound:[20,18,300,20],caption:"Cabang", maxLength:50, tag:1});			
		this.e_norek = new saiLabelEdit(this,{bound:[20,19,300,20],caption:"No Rekening", maxLength:50, tag:1});			
		this.e_namarek = new saiLabelEdit(this,{bound:[20,18,300,20],caption:"Nama Rekening", maxLength:50, tag:1});			
		this.cb_pp = new saiCBBL(this,{bound:[20,15,200,20],caption:"PP Induk", multiSelection:false, maxLength:10, tag:2});		
				
		this.p1 = new panel(this,{bound:[20,23,500,220],caption:"Daftar Pusat Pertanggungjawaban Lainnya"});
...
window.app_saku2_transaksi_kopeg_spro_fKaryawan.extend(window.childForm);
window.app_saku2_transaksi_kopeg_spro_fKaryawan.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into karyawan(nik,kode_lokasi,nama,alamat,no_tel,email,npwp,grade,jabatan,bank,cabang,no_rek,nama_rek,kode_pp) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.e_alamat.getText()+"','"+this.e_tel.getText()+"','"+this.e_mail.getText()+"','"+this.e_npwp.getText()+"','"+this.e_grade.getText()+"','"+this.e_jab.getText()+"','"+this.e_bank.getText()+"','"+this.e_cabang.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"','"+this.cb_pp.getText()+"')");
					...
					for (var i=0;i < this.sg.getRowCount();i++){
						if (this.sg.rowValid(i) && this.sg.cells(0,i) != this.cb_pp.getText()) 
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					...
					sql.add("update karyawan set nama='"+this.e_nama.getText()+"',alamat='"+this.e_alamat.getText()+"',no_tel='"+this.e_tel.getText()+"',email='"+this.e_mail.getText()+"',npwp='"+this.e_npwp.getText()+"',grade='"+this.e_grade.getText()+"',jabatan='"+this.e_jab.getText()+"',bank='"+this.e_bank.getText()+"',cabang='"+this.e_cabang.getText()+"',no_rek='"+this.e_norek.getText()+"',nama_rek='"+this.e_namarek.getText()+"',kode_pp='"+this.cb_pp.getText()+"' "+
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select nik,nama,alamat,no_tel,email,npwp,jabatan,grade,bank,cabang,no_rek,nama_rek,kode_pp from karyawan "+
						     "where nik ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
				...
				if (typeof data == "object"){
					...
					if (line != undefined){						
						...
						var strSQL = "select a.kode_pp,a.nama from pp a inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+						 
						 "where b.kode_pp<>'"+line.kode_pp+"' and b.nik='"+this.cb_kode.getText()+"' and b.kode_lokasi = '"+this.app._lokasi+"' order by b.kode_pp";						
						var data = this.dbLib.getDataProvider(strSQL,true);
