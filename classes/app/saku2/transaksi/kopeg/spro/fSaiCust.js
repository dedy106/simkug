window.app_saku2_transaksi_kopeg_spro_fSaiCust = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_spro_fSaiCust.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_spro_fSaiCust";
		...
		this.e_pic = new saiLabelEdit(this,{bound:[20,18,500,20],caption:"P I C", maxLength:150, tag:1});			
		this.e_jabatan = new saiLabelEdit(this,{bound:[20,19,500,20],caption:"Jabatan", maxLength:150, tag:1});			
...
window.app_saku2_transaksi_kopeg_spro_fSaiCust.extend(window.childForm);
window.app_saku2_transaksi_kopeg_spro_fSaiCust.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into cust(kode_cust,kode_lokasi,nama,alamat,no_tel,email,npwp,pic,alamat2,jabatan) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.e_alamat.getText()+"','"+this.e_tel.getText()+"','"+this.e_mail.getText()+"','"+this.e_npwp.getText()+"','"+this.e_pic.getText()+"','"+this.e_alamat2.getText()+"','"+this.e_jabatan.getText()+"')");
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update cust set nama='"+this.e_nama.getText()+"',alamat='"+this.e_alamat.getText()+"',no_tel='"+this.e_tel.getText()+"',email='"+this.e_mail.getText()+"',npwp='"+this.e_npwp.getText()+"',pic='"+this.e_pic.getText()+"',alamat2='"+this.e_alamat2.getText()+"',jabatan='"+this.e_jabatan.getText()+"' "+
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select kode_cust,nama,alamat,no_tel,email,npwp,alamat2,pic,jabatan from cust "+
						     "where kode_cust ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
