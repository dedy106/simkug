window.app_saku2_transaksi_kopeg_spro_fPP = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_spro_fPP.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_spro_fPP";
...
window.app_saku2_transaksi_kopeg_spro_fPP.extend(window.childForm);
window.app_saku2_transaksi_kopeg_spro_fPP.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into pp(kode_pp, kode_lokasi, nama, initial, level_spasi, tipe, sum_header, kode_induk, rowindex, kode_bidang, flag_aktif) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"',  '-', '0', 'Posting', '00', '00', 1, '1', '"+this.c_status.getText().substr(0,1)+"')");
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update pp set nama='"+this.e_nama.getText()+"', flag_aktif ='"+this.c_status.getText().substr(0,1)+"' "+
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select kode_pp,nama,flag_aktif from pp "+
						     "where kode_pp ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
