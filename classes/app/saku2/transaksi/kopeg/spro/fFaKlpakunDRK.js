window.app_saku2_transaksi_kopeg_spro_fFaKlpakunDRK = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_spro_fFaKlpakunDRK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_spro_fFaKlpakunDRK";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Kelompok Akun Aktap + DRK: Input/Edit", 0);	
...
window.app_saku2_transaksi_kopeg_spro_fFaKlpakunDRK.extend(window.childForm);
window.app_saku2_transaksi_kopeg_spro_fFaKlpakunDRK.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into fa_klpakun(kode_klpakun, kode_lokasi, nama, umur, persen, kode_akun, akun_bp, akun_deprs, kode_agg, tahun, kode_drk) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"',"+nilaiToFloat(this.e_umur.getText())+", "+nilaiToFloat(this.e_persen.getText())+", '"+this.cb_akun.getText()+"', '"+this.cb_beban.getText()+"', '"+this.cb_akum.getText()+"','"+this.e_agg.getText()+"','"+this.c_tahun.getText()+"','"+this.cb_drk.getText()+"')");
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update fa_klpakun set nama='"+this.e_nama.getText()+"', umur="+nilaiToFloat(this.e_umur.getText())+", persen="+nilaiToFloat(this.e_persen.getText())+", kode_akun='"+this.cb_akun.getText()+"', akun_bp='"+this.cb_beban.getText()+"', akun_deprs='"+this.cb_akum.getText()+"',kode_agg='"+this.e_agg.getText()+"',tahun='"+this.c_tahun.getText()+"',kode_drk='"+this.cb_drk.getText()+"' "+
	...
	doChange: function(sender){
		try{
			...
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select nama, umur, persen, kode_akun, akun_bp, akun_deprs, kode_agg, tahun, kode_drk from fa_klpakun "+
						     "where kode_klpakun ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
