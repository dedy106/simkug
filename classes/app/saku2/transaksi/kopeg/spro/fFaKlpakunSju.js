window.app_saku2_transaksi_kopeg_spro_fFaKlpakunSju = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_spro_fFaKlpakunSju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_spro_fFaKlpakunSju";
		...
		this.c_metod = new saiCB(this,{bound:[20,13,202,20],caption:"Metoda Susut",items:["SL","DD"], readOnly:true,tag:2});
...
window.app_saku2_transaksi_kopeg_spro_fFaKlpakunSju.extend(window.childForm);
window.app_saku2_transaksi_kopeg_spro_fFaKlpakunSju.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into fa_klpakun(kode_klpakun, kode_lokasi, nama, umur, persen, kode_akun, akun_bp, akun_deprs, kode_agg, rumus) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"',"+nilaiToFloat(this.e_umur.getText())+", "+nilaiToFloat(this.e_persen.getText())+", '"+this.cb_akun.getText()+"', '"+this.cb_beban.getText()+"', '"+this.cb_akum.getText()+"','-','"+this.c_metod.getText()+"')");
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update fa_klpakun set nama='"+this.e_nama.getText()+"', umur="+nilaiToFloat(this.e_umur.getText())+", persen="+nilaiToFloat(this.e_persen.getText())+", kode_akun='"+this.cb_akun.getText()+"', akun_bp='"+this.cb_beban.getText()+"', akun_deprs='"+this.cb_akum.getText()+"',rumus='"+this.c_metod.getText()+"' "+
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select nama, umur, persen, kode_akun, akun_bp, akun_deprs, rumus from fa_klpakun "+
						     "where kode_klpakun ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
				...
				if (typeof data == "object"){
					...
					if (line != undefined){						
						...
						this.c_metod.setText(line.rumus);												
