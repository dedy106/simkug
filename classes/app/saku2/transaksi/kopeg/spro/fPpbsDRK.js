window.app_saku2_transaksi_kopeg_spro_fPpbsDRK = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_spro_fPpbsDRK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_spro_fPpbsDRK";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Daftar Rencana Kegiatan (PPBS) : Input/Edit", 0);	
...
window.app_saku2_transaksi_kopeg_spro_fPpbsDRK.extend(window.childForm);
window.app_saku2_transaksi_kopeg_spro_fPpbsDRK.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into agg_drk(kode_drk, tahun, kode_lokasi, nama, block, tgl_input, nik_user, tipe, kode_fsrkm, level_spasi, level_lap, sum_header, jenis_akun, kode_induk, rowindex, modul) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update agg_drk set nama='"+this.e_nama.getText()+"', tahun ='"+this.c_tahun.getText()+"' "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from agg_drk where tahun = '"+this.c_tahun.getText()+"' and kode_drk = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");			
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != "" && this.c_tahun.getText() != ""){
				var strSQL = "select kode_drk,nama from agg_drk "+
						     "where tahun = '"+this.c_tahun.getText()+"' and kode_drk ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
