window.app_saku2_transaksi_kopeg_spro_fFlagRelasi = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_spro_fFlagRelasi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_spro_fFlagRelasi";
		...
		try {
			...
			this.cb_flag.setSQL("select kode_flag, nama from flag_akun",["kode_flag","nama"],false,["Kode","Nama"],"and","Data PP",true);
		}catch(e){
			systemAPI.alert(e);
		}
	}
};
window.app_saku2_transaksi_kopeg_spro_fFlagRelasi.extend(window.childForm);
window.app_saku2_transaksi_kopeg_spro_fFlagRelasi.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("delete from flag_relasi where kode_flag = '"+this.cb_flag.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");			
					
					for (var i=0;i < this.sg.getRowCount();i++){
						if (this.sg.rowValid(i)) 
							sql.add("insert into flag_relasi (kode_flag,kode_lokasi,kode_akun) values ('"+this.cb_flag.getText()+"','"+this.app._lokasi+"','"+this.sg.cells(0,i)+"')");
					}					
