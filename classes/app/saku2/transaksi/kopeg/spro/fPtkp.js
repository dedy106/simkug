window.app_saku2_transaksi_kopeg_spro_fPtkp = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_spro_fPtkp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_spro_fPtkp";
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select sts_pajak,nama,nilai,biaya_jab,jab_max from fri_status_pajak where kode_lokasi='"+this.app._lokasi+"' order by sts_pajak",true);
...
window.app_saku2_transaksi_kopeg_spro_fPtkp.extend(window.childForm);
window.app_saku2_transaksi_kopeg_spro_fPtkp.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from fri_status_pajak where kode_lokasi='"+this.app._lokasi+"'");			
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into fri_status_pajak(sts_pajak,nama,kode_lokasi,nilai,biaya_jab,jab_max) values "+
