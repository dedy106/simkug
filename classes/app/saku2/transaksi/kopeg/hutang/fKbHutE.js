window.app_saku2_transaksi_kopeg_hutang_fKbHutE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_hutang_fKbHutE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_hutang_fKbHutE";
...
window.app_saku2_transaksi_kopeg_hutang_fKbHutE.extend(window.childForm);
window.app_saku2_transaksi_kopeg_hutang_fKbHutE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into hutbayar_d(no_bukti,no_hutang,kode_lokasi,modul,periode,nik_user,tgl_input,dc,nilai) values "+
	...
	doChange:function(sender){
		...
		if (sender == this.e_nb && this.e_nb.getText()!= "") {			
			var strSQL = "select (a.nilai+a.nilai_ppn) -  isnull(b.bayar,0) as saldo, a.akun_hutang,a.kode_project+' - '+c.no_dokumen+' - '+c.keterangan as ket, "+
						 ...
						 "from hutang_m a inner join project c on a.kode_project=c.kode_project and a.kode_lokasi=c.kode_lokasi "+
						 ...
						 "left join (select no_hutang,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar from hutbayar_d where no_bukti <> '"+this.e_nb.getText()+"' group by no_hutang,kode_lokasi) b "+
