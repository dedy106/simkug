window.app_saku2_transaksi_kopeg_hutang_fKbHut = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_hutang_fKbHut.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_hutang_fKbHut";
...
window.app_saku2_transaksi_kopeg_hutang_fKbHut.extend(window.childForm);
window.app_saku2_transaksi_kopeg_hutang_fKbHut.implement({
	...
	doChange:function(sender){
		if ((sender == this.e_periode || sender == this.cb_vendor) && (this.e_periode.getText()!= "" && this.cb_vendor.getText()!= "")) {
			var strSQL = "select a.no_hutang,a.keterangan,a.kode_lokasi from hutang_m a "+
						 "left join (select no_hutang,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar from hutbayar_d group by no_hutang,kode_lokasi) b "+
