window.app_saku2_transaksi_kopeg_ppbs_fAppYpt = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_ppbs_fAppYpt.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_ppbs_fAppYpt";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Approve Yayasan", 0);	
...
window.app_saku2_transaksi_kopeg_ppbs_fAppYpt.extend(window.childForm);
window.app_saku2_transaksi_kopeg_ppbs_fAppYpt.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into agg_app_m (no_app,kode_lokasi,tanggal,periode,jenis,no_dokumen,keterangan,nik_user,tgl_input,nilai) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.c_tahun.getText()+"','YPT','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.app._userLog+"',getdate(),"+nilaiToFloat(this.e_total.getText())+")");
	...
	doLoad:function(sender){	
		if (this.c_tahun.getText() != "") {
			var strSQL = "select a.kode_pp,b.nama,round(sum(a.total),0) as nilai "+
						 ...
						 "						  where b.periode='"+this.c_tahun.getText()+"' and b.jenis ='TU' and b.kode_lokasi ='"+this.app._lokasi+"') c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
						 "             left join (select a.kode_pp,a.kode_lokasi "+
						 " 						  from agg_app_d a inner join agg_app_m b on a.no_app=b.no_app and a.kode_lokasi=b.kode_lokasi "+
						 "						  where b.periode='"+this.c_tahun.getText()+"' and b.jenis ='YPT' and b.kode_lokasi ='"+this.app._lokasi+"') d on a.kode_pp=d.kode_pp and a.kode_lokasi=d.kode_lokasi "+
						 "where d.kode_pp is null and a.periode like '"+this.c_tahun.getText()+"%' and a.kode_lokasi='"+this.app._lokasi+"' "+
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_app,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai "+
		             "from agg_app_m a "+
					 "where  a.kode_lokasi='"+this.app._lokasi+"' and a.jenis = 'YPT'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
