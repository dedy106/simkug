window.app_saku2_transaksi_kopeg_ppbs_fNeraca = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_ppbs_fNeraca.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku2_transaksi_kopeg_ppbs_fNeraca";
		...
		this.eKlp.setSQL("select kode_fs, nama from agg_fs where kode_lokasi = '"+this.app._lokasi+"' ",["kode_fs","nama"],undefined,["Kode FS","Nama"],"and","Data Versi Neraca");	
	}
};
window.app_saku2_transaksi_kopeg_ppbs_fNeraca.extend(window.portalui_childForm);
window.app_saku2_transaksi_kopeg_ppbs_fNeraca.implement({
		...
		{ 
		 	if (sender == this.relBtn)
			{
				...
				if (this.selectedItem.data.get("tipe").toLowerCase() == "posting"){
					uses("app_saku2_master_gl_fagg_relakungar",true);
					if (this.relakun != undefined)
						this.relakun.free();
					this.relakun = new app_saku2_master_gl_fagg_relakungar(this.app);
			...
			}else
			{
				if (sender == this.addBtn || sender == this.editBtn){
					uses("app_saku2_transaksi_kopeg_ppbs_fNeracaDetail");
					if (this.entryMenu == undefined)
						this.entryMenu = new app_saku2_transaksi_kopeg_ppbs_fNeracaDetail(this.app);
	...
	doEditChange: function(sender){
		if (sender == this.elokasi){			
			this.eKlp.setText("");
			this.treev.clear();
			this.eKlp.setSQL("select kode_fs, nama from agg_fs where kode_lokasi = '"+sender.getText()+"' ",["kode_fs","nama"],undefined,["Kode FS","Nama"],"and","Data Versi Neraca");	
