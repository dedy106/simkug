window.app_saku2_transaksi_kopeg_ppbs_fEvalAkunPeriode = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_ppbs_fEvalAkunPeriode.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_ppbs_fEvalAkunPeriode";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Evaluasi Usulan Anggaran Periode: Input", 0);	
		...
		this.c_bulan = new saiCB(this,{bound:[20,22,182,20],caption:"Bulan",items:["01","02","03","04","05","06","07","08","09","10","11","12"], readOnly:true,tag:2,change:[this,"doChange"]});
...
window.app_saku2_transaksi_kopeg_ppbs_fEvalAkunPeriode.extend(window.childForm);
window.app_saku2_transaksi_kopeg_ppbs_fEvalAkunPeriode.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into agg_eval_d(no_eval,kode_lokasi,kode_akun,persen,bulan) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_akun.getText()+"',"+nilaiToFloat(this.e_persen.getText())+",'"+this.c_bulan.getText()+"')");
					...
					sql.add("update agg_d set vol=vol+((vol * "+nilaiToFloat(this.e_persen.getText())+")/100), "+
							...
							"where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.c_tahun.getText()+this.c_bulan.getText()+"' and kode_akun='"+this.cb_akun.getText()+"' ");						 
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_akun || sender == this.c_bulan) {
			if (this.cb_akun.getText()!="" && this.c_bulan.getText()!="") {
				var strSQL = "select a.kode_drk,a.kode_norma,a.kode_akun,a.keterangan,a.satuan,substring(a.periode,5,2) as bulan,a.tarif,a.jumlah,a.vol,a.total, "+
							 "b.nama as nama_drk, isnull(c.nama,'-') as nama_norma,vol as vol_baru,total as total_baru "+
							 "from agg_d a inner join agg_drk b on a.kode_drk=b.kode_drk and a.kode_lokasi=b.kode_lokasi "+
							 "			   left join agg_norma c on a.kode_norma=c.kode_norma "+
							 "where a.kode_lokasi='"+this.app._lokasi+"' and a.periode = '"+this.c_tahun.getText()+this.c_bulan.getText()+"' and a.kode_akun='"+this.cb_akun.getText()+"' ";			
