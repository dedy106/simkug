window.app_saku2_transaksi_kopeg_ppbs_fEvalMulti = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_ppbs_fEvalMulti.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_ppbs_fEvalMulti";
		...
		this.sg = new saiGrid(this.p1,{bound:[0,20,this.p1.width-5,this.p1.height-45],colCount:3,tag:0,
		            colTitle:["Kode Akun","Nama Akun","Persentase(+/-)"],
...
window.app_saku2_transaksi_kopeg_ppbs_fEvalMulti.extend(window.childForm);
window.app_saku2_transaksi_kopeg_ppbs_fEvalMulti.implement({	
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								...
								sql.add("update agg_d set vol=vol+((vol * "+nilaiToFloat(this.sg.cells(2,i))+")/100), "+
										...
										"where kode_lokasi='"+this.app._lokasi+"' and periode like '"+this.c_tahun.getText()+"%' and kode_akun='"+this.sg.cells(0,i)+"' ");						 					
							}
						}
					}
