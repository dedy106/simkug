window.app_saku2_transaksi_kopeg_ppbs_fFs = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kopeg_ppbs_fFs.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kopeg_ppbs_fFs";
		this.setTop(60);
		this.setWidth(1280);
		this.setHeight(550);
...
window.app_saku2_transaksi_kopeg_ppbs_fFs.extend(window.portalui_childForm);
//------------------------------------------------------------------ event
window.app_saku2_transaksi_kopeg_ppbs_fFs.implement({
		...
		try{
			...
			switch (event)
			{
				...
				case "simpan" :
						...
						sql.add("insert into agg_fs (kode_fs, kode_lokasi,nama,flag_status, tgl_input,nik_user ) values ('"+this.e0.getText()+"','"+this.app._lokasi+"','"+this.e1.getText()+"','"+status+"','"+(new Date).getDateStr()+"','"+this.app._userLog+"') ");
						this.dbLib.execArraySQL(sql);					
					break;
				case "ubah" :					
						uses("server_util_arrayList");
						sql = new server_util_arrayList();
						sql.add("update agg_fs set nama = '"+this.e1.getText()+"',flag_status = '"+status+"',nik_user='"+this.app._userLog+"' where kode_fs = '"+this.e0.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
						this.dbLib.execArraySQL(sql);							
					break;
				case "hapus" :
				    uses("server_util_arrayList");
