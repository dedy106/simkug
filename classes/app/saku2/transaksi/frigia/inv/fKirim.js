window.app_saku2_transaksi_frigia_inv_transaksi_fKirim = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_frigia_inv_transaksi_fKirim.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_frigia_inv_transaksi_fKirim";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Kirim Barang: Input", 0);	
		...
		this.cb_asal = new portalui_saiCBBL(this,{bound:[20,16,200,20],caption:"Gudang Asal",multiSelection:false,tag:1,change:[this,"doChange"]});
...
window.app_saku2_transaksi_frigia_inv_transaksi_fKirim.extend(window.portalui_childForm);
window.app_saku2_transaksi_frigia_inv_transaksi_fKirim.implement({
	...
	simpan: function(){			
		try{					
			if (this.standarLib.checkEmptyByTag(this, [0,2])){
				try{									
					this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"fri_kirim_m","no_kirim",this.app._lokasi+"-KRM"+this.e_periode.getText().substr(2,4)+".","0000"));
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("insert into fri_kirim_m(no_kirim,kode_lokasi,tanggal,no_dokumen,keterangan,nik_buat,kode_asal,kode_tujuan,periode,nik_user,tgl_input,no_link,no_del,no_terima) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_desc.getText()+"','-','"+this.cb_asal.getText()+"','"+this.cb_tujuan.getText()+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',now(),'-','-','-')");
					
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								if (this.sg.cells(4,i) != "0") {									
									sql.add("insert into fri_kirim_d (no_kirim,kode_lokasi,periode,nu,kode_brg,jumlah,bonus,harga,stok) values "+  
										   "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"',"+i+",'"+this.sg.cells(0,i)+"',"+parseNilai(this.sg.cells(5,i))+",0,0,"+parseNilai(this.sg.cells(4,i))+")");
								}
							}
						}						
					}
					sql.add("insert into fri_barang_d(no_bukti,kode_lokasi,nu,kode_brg,harga,jumlah,bonus,modul,dc,periode,tanggal,kode_gudang,pdisk) "+
						    "select no_kirim,kode_lokasi,nu,kode_brg,harga,jumlah,bonus,'KIRIM','C',periode,'"+this.dp_d1.getDateString()+"','"+this.cb_asal.getText()+"',0 "+
							"from fri_kirim_d where no_kirim='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");
		...
		try{			
			if (col == 0){		
				this.standarLib.showListData(this, "Daftar Item Barang",sender,undefined, 
											  ...
											  "                        from fri_barang_d where kode_gudang='"+this.cb_asal.getText()+"' and kode_lokasi='"+this.app._lokasi+"' group by kode_lokasi,kode_brg ) e on a.kode_brg=e.kode_brg and a.kode_lokasi=e.kode_lokasi "+
											  "where a.kode_lokasi='"+this.app._lokasi+"'",
											  "select count(a.kode_brg) from fri_barang_m a where a.kode_lokasi='"+this.app._lokasi+"' ",
