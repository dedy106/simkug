window.app_saku2_transaksi_tk_fClosingThn = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_tk_fClosingThn.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_tk_fClosingThn";
...
window.app_saku2_transaksi_tk_fClosingThn.extend(window.childForm);
window.app_saku2_transaksi_tk_fClosingThn.implement({
	...
	isPostedAll: function(periode){
		...
		var sqlAR = "select count(x.no_bukti) from ("+
				    "  select no_piutang as no_bukti from tk_piutang_m where posted = 'F' and periode ='"+periode+"' and kode_lokasi='"+this.app._lokasi+"' "+				    
				    ") x";					   
		...
		if (ck != "") { 
			...
			if (data[1] != "0"){
				result = result && false;
				msg+= "Data Modul PIUTANG [PIUUM] masih ada yang belum diposting.("+data[1]+" data)\n";
			}
		}
		var sqlAP = "select count(x.no_bukti) from ("+
				    "  select no_hutang as no_bukti from csm_hutang_m where posted = 'F' and periode ='"+periode+"' and kode_lokasi='"+this.app._lokasi+"' "+				    
				    ") x";					   
