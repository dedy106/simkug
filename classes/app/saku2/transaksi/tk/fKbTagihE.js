window.app_saku2_transaksi_tk_fKbTagihE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_tk_fKbTagihE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_tk_fKbTagihE";
		...
		this.e_siswa = new saiLabelEdit(this,{bound:[20,18,470,20],caption:"Siswa", readOnly:true});												
...
window.app_saku2_transaksi_tk_fKbTagihE.extend(window.childForm);
window.app_saku2_transaksi_tk_fKbTagihE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from tk_piubayar_d where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add("insert into tk_piubayar_d(no_bukti,no_piutang,kode_lokasi,modul,periode,nik_user,tgl_input,dc,nilai) values "+
	...
	doChange:function(sender){
		...
		if (sender == this.e_nb && this.e_nb.getText()!= "") {			
			var strSQL = "select (a.nilai) -  isnull(b.bayar,0) as saldo, a.akun_piutang, "+
			             "d.nama+'-'+d.kode_siswa as siswa,"+
						 "e.jenis,e.periode,e.no_dokumen,e.tanggal,e.keterangan as ket_kas,e.nik_app,e.no_link "+
						 "from tk_piutang_m a "+
						 "					   inner join tk_siswa d on a.kode_siswa=d.kode_siswa and a.kode_lokasi=d.kode_lokasi "+
						 "                     inner join kas_m e on e.no_link = a.no_piutang and a.kode_lokasi=e.kode_lokasi "+
						 "left join (select no_piutang,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar from tk_piubayar_d where no_bukti <> '"+this.e_nb.getText()+"' group by no_piutang,kode_lokasi) b "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){		
					...
					this.e_siswa.setText(line.siswa);
