window.app_saku2_transaksi_tk_fTagih = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_tk_fTagih.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_tk_fTagih";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Tagihan Siswa: Input", 0);	
		...
		this.cb_siswa = new saiCBBL(this,{bound:[20,12,200,20],caption:"Siswa", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});												
		...
		this.p1 = new panel(this,{bound:[20,20,800,330],caption:"Data Item Biaya"});		
		...
		try {
			...
			this.cb_siswa.setSQL("select kode_siswa, nama from tk_siswa where kode_lokasi='"+this.app._lokasi+"'",["kode_siswa","nama"],false,["Kode","Nama"],"and","Data Siswa",true);			
...
window.app_saku2_transaksi_tk_fTagih.extend(window.childForm);
window.app_saku2_transaksi_tk_fTagih.implement({
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "" ) {				
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"tk_piutang_m","no_piutang",this.app._lokasi+"-PU"+this.e_periode.getText().substr(2,4)+".","000"));
			this.e_ket.setFocus();
		}
	},
	doChange:function(sender){
		if (sender == this.cb_siswa && this.cb_siswa.getText()!= "" ) {				
			var data = this.dbLib.getDataProvider("select a.kode_tingkat,a.kode_tingkat+' - '+a.nama as tingkat from tk_tingkat a inner join tk_siswa b on a.kode_tingkat=b.kode_tingkat and a.kode_lokasi=b.kode_lokasi where b.kode_siswa='"+this.cb_siswa.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"'",true);
			...
			var sql = new server_util_arrayList();			
			sql.add("select kode_tarif, nama from tk_tarif where kode_tingkat='"+this.kodeTingkat+"' and kode_lokasi='"+this.app._lokasi+"'");
			this.dbLib.getMultiDataProviderA(sql);		
			
			
			var strSQL = "select kode_tarif,dc,nilai as tarif,0 as pot,nilai,nama,kode_akun  "+
					     "from tk_tarif "+
					     "where kode_tingkat = '"+this.kodeTingkat+"' and kode_lokasi='"+this.app._lokasi+"'";
