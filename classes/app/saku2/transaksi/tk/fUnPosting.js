window.app_saku2_transaksi_tk_fUnPosting = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_tk_fUnPosting.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_tk_fUnPosting";
...
window.app_saku2_transaksi_tk_fUnPosting.extend(window.childForm);
window.app_saku2_transaksi_tk_fUnPosting.implement({
				...
				switch(this.c_modul.getText()){
					...
					case "AR" :
								strSQL = "select 'INPROG' as status,no_piutang as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'PIUTK' as jenis "+
										 "from tk_piutang_m "+
										 "where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";
								this.cb_bukti.setSQL("select no_piutang as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'PIUTK' as jenis from tk_piutang_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ",												     
