window.app_saku2_transaksi_tk_fKbTagih = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_tk_fKbTagih.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_tk_fKbTagih";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Tagihan: Input", 0);	
		...
		this.cb_piutang = new saiCBBL(this,{bound:[20,19,220,20],caption:"No Piutang", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});				
...
window.app_saku2_transaksi_tk_fKbTagih.extend(window.childForm);
window.app_saku2_transaksi_tk_fKbTagih.implement({
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					this.sg.clear(1); 
					setTipeButton(tbSimpan);
					var strSQL = "select a.no_piutang,a.keterangan,a.kode_lokasi from tk_piutang_m a "+
								 "left join (select no_piutang,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar from tk_piubayar_d group by no_piutang,kode_lokasi) b "+
								 "on a.no_piutang=b.no_piutang and a.kode_lokasi=b.kode_lokasi where a.nilai>isnull(b.bayar,0) and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
	...
	doChange:function(sender){
		...
		if (sender == this.cb_piutang && this.cb_piutang.getText()!= "") {
			var strSQL = "select (a.nilai) -  isnull(b.bayar,0) as saldo, a.akun_piutang,"+
						 ...
						 "from tk_piutang_m a inner join tk_siswa d on a.kode_siswa=d.kode_siswa and a.kode_lokasi=d.kode_lokasi "+
						 "left join (select no_piutang,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar from tk_piubayar_d group by no_piutang,kode_lokasi) b "+
