window.app_saku2_transaksi_konten_fKontenE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_konten_fKontenE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_konten_fKontenE";
		...
		this.cb_buat = new saiCBBL(this.pc1.childPage[0],{bound:[20,18,200,20],caption:"NIK Pembuat", multiSelection:false, maxLength:10, tag:2});
		...
		try {
			...
			this.e_nb.setSQL("select no_konten, judul from konten where  kode_lokasi='"+this.app._lokasi+"'",["no_konten","judul"],false,["No Konten","Keterangan"],"and","Data Konten",true);					
			this.cb_klp.setSQL("select kode_klp, nama from klpkonten where kode_lokasi='"+this.app._lokasi+"'",["kode_klp","nama"],false,["Kode","Nama"],"and","Data Kelompok",true);	
...
window.app_saku2_transaksi_konten_fKontenE.extend(window.childForm);
window.app_saku2_transaksi_konten_fKontenE.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("delete from konten where no_konten='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("insert into konten(no_konten,kode_lokasi,periode,tanggal,kode_klp,nik_buat,nik_app,keterangan,judul,file_gambar,file_dok,tgl_input,nik_user,flag_aktif) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_klp.getText()+"','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+urlencode(this.mDesk.getCode())+"','"+this.e_judul.getText()+"','"+this.e_gambar.getText()+"','"+this.e_file.getText()+"',getdate(),'"+this.app._userLog+"','"+flag_aktif+"')");
	...
	doChange:function(sender){
		...
		if (this.e_nb.getText()!="") {
			...
			var data = this.dbLib.getDataProvider(
					   ...
					   "from konten a "+
					   "inner join klpkonten b on a.kode_klp=b.kode_klp and a.kode_lokasi=b.kode_lokasi "+	  
	...
	doClick:function(sender){
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"konten","no_konten",this.app._lokasi+"-KTN"+this.e_periode.getText().substr(2,4)+".","000"));
		this.dp_d2.setFocus();
	},
