window.app_saku2_transaksi_gl_fClosingThnFrigia = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_gl_fClosingThnFrigia.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_gl_fClosingThnFrigia";
...
window.app_saku2_transaksi_gl_fClosingThnFrigia.extend(window.childForm);
window.app_saku2_transaksi_gl_fClosingThnFrigia.implement({
	...
	isPostedAll: function(periode){
		...
		var sqlAR = "select count(x.no_bukti) from ("+
					...
					"  union "+
				    "  select no_jual as no_bukti from fri_jual_m where posted = 'F' and periode ='"+periode+"' and kode_lokasi='"+this.app._lokasi+"' "+
				    ") x";					   
