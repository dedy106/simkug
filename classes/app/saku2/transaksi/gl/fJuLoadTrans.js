window.app_saku2_transaksi_gl_fJuLoadTrans = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_gl_fJuLoadTrans.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_gl_fJuLoadTrans";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Jurnal Umum : Load Transaksi", 0);	
...
window.app_saku2_transaksi_gl_fJuLoadTrans.extend(window.childForm);
window.app_saku2_transaksi_gl_fJuLoadTrans.implement({
	...
	selectPage: function(sender,page){
		...
		for (var i=start; i < finish;i++){
			...
			var pp = this.dataPP.get(line.kode_pp);
			if (!pp) {
				this.sg.clear(1);
				system.alert(this,"Data tidak valid.","PP '"+line.kode_pp+"' tidak terdaftar.");
				return false;				
			}						
			
			/*
			var isAda = false;
			var data2 = this.dbLib.getDataProvider("select count(distinct a.kode_drk) as jml from drk a inner join anggaran_d b on a.kode_drk=b.kode_drk where a.tahun=substring(b.periode,1,4) and b.periode like '"+this.e_periode.getText().substr(0,4)+"%' and b.kode_akun='"+line.kode_akun+"' and b.kode_pp = '"+line.kode_pp+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data2 == "object"){
				var line2 = data2.rs.rows[0];							
				if (line2 != undefined){
					if (line2.jml != 0) isAda = true;
				} 
			}			
			var data2 = this.dbLib.getDataProvider("select distinct a.nama from drk a inner join anggaran_d b on a.kode_drk=b.kode_drk where a.tahun=substring(b.periode,1,4) and b.periode like '"+this.e_periode.getText().substr(0,4)+"%' and b.kode_akun='"+line.kode_akun+"' and b.kode_pp = '"+line.kode_pp+"' and b.kode_drk = '"+line.kode_drk+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data2 == "object"){
				var line2 = data2.rs.rows[0];							
				if (line2 == undefined && isAda) {					
					this.sg.clear(1);
					system.alert(this,"Data tidak valid.","");
					return false;									
				}
			}
			*/
