window.app_saku2_transaksi_gl_fJPYakes = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_gl_fJPYakes.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku2_transaksi_gl_fJPYakes";
		...
		this.ed_debet = new portalui_saiLabelEdit(this,{bound:[700,13,220,20],caption:"Total Debet",tipeText:ttNilai, alignment:alRight,readOnly:true,text:"0"});
		this.cb_pembuat = new saiCBBL(this,{bound:[20,14,200,20],caption:"NIK Pembuat", multiSelection:false, maxLength:10, tag:2});
		this.ed_kredit = new portalui_saiLabelEdit(this,{bound:[700,14,220,20],caption:"Total Kredit",tipeText:ttNilai, alignment:alRight,readOnly:true,text:"0"});
		this.cb_setuju = new saiCBBL(this,{bound:[20,15,200,20],caption:"NIK Approve", multiSelection:false, maxLength:10, tag:2});
		this.ed_jp = new portalui_saiLabelEdit(this,{bound:[700,15,220,20],caption:"Total JP",tipeText:ttNilai, alignment:alRight, text:"0",readOnly:true});		
		this.bGar = new portalui_button(this,{bound:[580,15,100,20],caption:"Tampil",hint:"Load",image:"icon/"+system.getThemes()+"/tabCont2.png",click:[this,"doUploadTB"]});
		...
		sql.add("select m.kode_akun, m.nama from masakun m "+ //2310000000 [lawan dr : 3__3%,2313%]
				" inner join flag_relasi b on b.kode_akun = m.kode_akun and m.kode_lokasi = b.kode_lokasi "+
				" where b.kode_flag = '998' and m.kode_lokasi = '"+this.app._lokasi+"' ");
		sql.add("select m.kode_akun, m.nama from masakun m "+ //1602000001 [lawan dari : 15%]
				" inner join flag_relasi b on b.kode_akun = m.kode_akun and m.kode_lokasi = b.kode_lokasi "+
				" where b.kode_flag = '997' and m.kode_lokasi = '"+this.app._lokasi+"' ");		
		this.dbLib.getMultiDataProviderA(sql);				
		setTipeButton(tbSimpan);
	}
};
window.app_saku2_transaksi_gl_fJPYakes.extend(window.portalui_childForm);
window.app_saku2_transaksi_gl_fJPYakes.implement({
	doUploadTB: function(){		
		try{
			if (this.app._periode != this.app._periode.substr(0,4)+this.maxPeriode){
				systemAPI.alert("Periode sekarang ["+this.app._periode+"] masih belum akhir periode ["+(this.app._periode.substr(0,4) +this.maxPeriode)+"].","Load data tidak dapat dilanjutkan.");
				return;
			}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "simpan" :
					if (this.app._periode != this.app._periode.substr(0,4) +this.maxPeriode){
						systemAPI.alert("Periode sekarang ["+this.app._periode+"] masih belum akhir periode ["+(this.app._periode.substr(0,4) +this.maxPeriode)+"].","Process JP tidak bisa dilakukan.");
						return;
					}
					...
					try{						
						if (this.standarLib.checkEmptyByTag(this, new Array("0","1","2"))){
							...
							sql.add("insert into jp(no_bukti, tanggal, periode, keterangan, nik_pembuat, kode_lokasi, nilai, tgl_input, nik_user )values"+
									...
									"'"+this.cb_pembuat.getText()+"','"+this.app._lokasi+"',"+parseNilai(this.ed_jp.getText())+",now(), '"+this.app._userLog+"')");
							
							var line; 
							var nilai = jp2 = jp3 = jp4 = jp5 = 0; 
							var dc = dc2 = "";
							var tgl = this.ed_period.getText().substr(0,4)+'-12-31';
							for (var i=0;i < this.dataJU.rs.rows.length;i++){
								line = this.dataJU.rs.rows[i];
								if (line.kode_akun.substr(0,1) == "3" && line.kode_akun.substr(3,1) == "3") jp2 += parseFloat(line.so_akhir);
							...
							if (jp2 > 0) dc2 = "D"; else dc2 = "C";
							nilai = Math.abs(nilaiToFloat(this.ed_jp.getText()) - jp2);
							jp2 = Math.abs(jp2);
							sql.add("insert into gldt (no_bukti,no_urut,kode_lokasi,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,modul,jenis,periode,kode_curr,kurs,nilai_curr,tgl_input,nik_user) values "+
							...
							sql.add("insert into gldt (no_bukti,no_urut,kode_lokasi,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,modul,jenis,periode,kode_curr,kurs,nilai_curr,tgl_input,nik_user) values "+
									"('"+this.ed_nb.getText()+"',99998,'"+this.app._lokasi+"','-','"+tgl+"','"+this.akunJP2+"','"+dc2+"',"+jp2+",'Jurnal Penutup Periode "+this.ed_period.getText()+"','-','-','-','-','-','-','-','-','JP','JP2','"+this.ed_period.getText()+"','IDR',1,"+jp2+",getdate(),'"+this.app._userLog+"')");
							
							//DAKEM
							strSQL = "select kode_akun,round(so_akhir,0) as so_akhir "+
							  	     "from glma_tmp where substring(kode_akun,1,4) in ('2313','1513') and kode_lokasi ='"+this.app._lokasi+"' and nik_user = '"+this.nik_user+"' and round(so_akhir,0) <> 0 order by kode_akun";
							var data = this.dbLib.getDataProvider(strSQL,true);
							if (typeof data == "object" && data.rs.rows[0] != undefined){
								this.dataJU2 = data;
								for (var i=0;i < this.dataJU2.rs.rows.length;i++){
									line = this.dataJU2.rs.rows[i];				
									jp3 += parseFloat(line.so_akhir);									
									if (parseFloat(line.so_akhir) > 0) dc = "C"; else dc = "D";
									nilai = Math.abs(parseFloat(line.so_akhir));
									sql.add("insert into jp_d (no_bukti,kode_lokasi,kode_akun,dc,nilai) values ('"+this.ed_nb.getText()+"','"+this.app._lokasi+"','"+line.kode_akun+"','"+dc+"',"+nilai+")");
									sql.add("insert into gldt (no_bukti,no_urut,kode_lokasi,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,modul,jenis,periode,kode_curr,kurs,nilai_curr,tgl_input,nik_user) values "+
											"('"+this.ed_nb.getText()+"',"+i+",'"+this.app._lokasi+"','-','"+tgl+"','"+line.kode_akun+"','"+dc+"',"+nilai+",'Jurnal Penutup Periode "+this.ed_period.getText()+"','-','-','-','-','-','-','-','-','JP','DAKEM','"+this.ed_period.getText()+"','IDR',1,"+nilai+",getdate(),'"+this.app._userLog+"')");
								}
							}							
							if (jp3 > 0) dc2 = "D"; else dc2 = "C";
							jp3 = Math.abs(jp3);
							sql.add("insert into gldt (no_bukti,no_urut,kode_lokasi,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,modul,jenis,periode,kode_curr,kurs,nilai_curr,tgl_input,nik_user) values "+
									"('"+this.ed_nb.getText()+"',99997,'"+this.app._lokasi+"','-','"+tgl+"','"+this.akunJP2+"','"+dc2+"',"+jp3+",'Jurnal Penutup Periode "+this.ed_period.getText()+"','-','-','-','-','-','-','-','-','JP','JP3','"+this.ed_period.getText()+"','IDR',1,"+jp3+",getdate(),'"+this.app._userLog+"')");
							
							//MUTASI
							strSQL = "select kode_akun,round(so_akhir,0) as so_akhir "+
									 "from glma_tmp where substring(kode_akun,1,4) in ('1511','1512') and kode_lokasi ='"+this.app._lokasi+"' and nik_user = '"+this.nik_user+"' and round(so_akhir,0) <> 0 order by kode_akun";
							var data = this.dbLib.getDataProvider(strSQL,true);
							if (typeof data == "object" && data.rs.rows[0] != undefined){
								this.dataJU3 = data;
								for (var i=0;i < this.dataJU3.rs.rows.length;i++){
									line = this.dataJU3.rs.rows[i];				
									jp4 += parseFloat(line.so_akhir);									
									if (parseFloat(line.so_akhir) > 0) dc = "C"; else dc = "D";
									nilai = Math.abs(parseFloat(line.so_akhir));
									sql.add("insert into jp_d (no_bukti,kode_lokasi,kode_akun,dc,nilai) values ('"+this.ed_nb.getText()+"','"+this.app._lokasi+"','"+line.kode_akun+"','"+dc+"',"+nilai+")");
									sql.add("insert into gldt (no_bukti,no_urut,kode_lokasi,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,modul,jenis,periode,kode_curr,kurs,nilai_curr,tgl_input,nik_user) values "+
											"('"+this.ed_nb.getText()+"',"+i+",'"+this.app._lokasi+"','-','"+tgl+"','"+line.kode_akun+"','"+dc+"',"+nilai+",'Jurnal Penutup Periode "+this.ed_period.getText()+"','-','-','-','-','-','-','-','-','JP','MUTASI','"+this.ed_period.getText()+"','IDR',1,"+nilai+",getdate(),'"+this.app._userLog+"')");
								}
							}
							if (jp4 > 0) dc2 = "D"; else dc2 = "C";
							jp4 = Math.abs(jp4);
							sql.add("insert into gldt (no_bukti,no_urut,kode_lokasi,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,modul,jenis,periode,kode_curr,kurs,nilai_curr,tgl_input,nik_user) values "+
									"('"+this.ed_nb.getText()+"',99996,'"+this.app._lokasi+"','-','"+tgl+"','"+this.akunJP3+"','"+dc2+"',"+jp4+",'Jurnal Penutup Periode "+this.ed_period.getText()+"','-','-','-','-','-','-','-','-','JP','JP4','"+this.ed_period.getText()+"','IDR',1,"+jp4+",getdate(),'"+this.app._userLog+"')");

							//JAMKESPEN
							strSQL = "select kode_akun,round(so_akhir,0) as so_akhir "+
									 "from glma_tmp where kode_akun = '1712010002' and kode_lokasi ='"+this.app._lokasi+"' and nik_user = '"+this.nik_user+"' and round(so_akhir,0) <> 0 order by kode_akun";
							var data = this.dbLib.getDataProvider(strSQL,true);
							if (typeof data == "object" && data.rs.rows[0] != undefined){
								this.dataJU4 = data;								
								line = this.dataJU4.rs.rows[0];				
								jp5 = parseFloat(line.so_akhir);
								if (parseFloat(line.so_akhir) > 0) dc = "C"; else dc = "D";
								nilai = Math.abs(parseFloat(line.so_akhir));
								sql.add("insert into jp_d (no_bukti,kode_lokasi,kode_akun,dc,nilai) values ('"+this.ed_nb.getText()+"','"+this.app._lokasi+"','"+line.kode_akun+"','"+dc+"',"+nilai+")");
								sql.add("insert into gldt (no_bukti,no_urut,kode_lokasi,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,modul,jenis,periode,kode_curr,kurs,nilai_curr,tgl_input,nik_user) values "+
										"('"+this.ed_nb.getText()+"',"+i+",'"+this.app._lokasi+"','-','"+tgl+"','"+line.kode_akun+"','"+dc+"',"+nilai+",'Jurnal Penutup Periode "+this.ed_period.getText()+"','-','-','-','-','-','-','-','-','JP','JAMKPENS','"+this.ed_period.getText()+"','IDR',1,"+nilai+",getdate(),'"+this.app._userLog+"')");
								if (jp5 > 0) dc2 = "D"; else dc2 = "C";
								jp5 = Math.abs(jp5);
								sql.add("insert into gldt (no_bukti,no_urut,kode_lokasi,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,modul,jenis,periode,kode_curr,kurs,nilai_curr,tgl_input,nik_user) values "+
										"('"+this.ed_nb.getText()+"',99995,'"+this.app._lokasi+"','-','"+tgl+"','"+this.akunJP+"','"+dc2+"',"+jp5+",'Jurnal Penutup Periode "+this.ed_period.getText()+"','-','-','-','-','-','-','-','-','JP','JP5','"+this.ed_period.getText()+"','IDR',1,"+jp5+",getdate(),'"+this.app._userLog+"')");
							}
			...
			if (sender == this.dbLib){
				if (methodName == "getMultiDataProvider"){
					try{
						...
						if (result[2].rs.rows[0] !== undefined){
							this.akunJP2 = result[2].rs.rows[0].kode_akun;
						}else {
							systemAPI.alert("Akun JP DAKEM masih belum ada","Untuk menandai akun JP di form Relasi Flag Akun");
							return;
						}
						if (result[3].rs.rows[0] !== undefined){
							this.akunJP3 = result[3].rs.rows[0].kode_akun;
						}else {
							systemAPI.alert("Akun JP MUTASI masih belum ada","Untuk menandai akun JP di form Relasi Flag Akun");
							return;
						}
					}catch(e){
