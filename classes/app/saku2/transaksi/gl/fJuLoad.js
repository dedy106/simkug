window.app_saku2_transaksi_gl_fJuLoad = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_gl_fJuLoad.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_gl_fJuLoad";
		this.itemsValue = new arrayList();
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Jurnal Umum : Load Reverse", 0);	
		
		uses("saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		this.e_periode = new portalui_saiLabelEdit(this,{bound:[20,14,202,20],caption:"Periode",tag:2,readOnly:true});
		this.l_tgl1 = new portalui_label(this,{bound:[20,11,100,18],caption:"Tanggal", underline:true});
		this.dp_d1 = new portalui_datePicker(this,{bound:[120,11,100,18],selectDate:[this,"doSelectDate"]}); 
		this.c_jenis = new saiCB(this,{bound:[20,22,202,20],caption:"Jenis",items:["UMUM","KOREKSI","ADJUST","REVERSE"], readOnly:true,tag:2});
		this.c_perLama = new saiCB(this,{bound:[700,22,180,20],caption:"Periode Lama",readOnly:true,change:[this,"doChange"]}); 
		this.e_nb = new portalui_saiLabelEdit(this,{bound:[20,12,202,20],caption:"No JU",maxLength:30,readOnly:true});
		this.cb_nbLama = new saiCBBL(this,{bound:[700,12,200,20],caption:"No BuktiLama", multiSelection:false, maxLength:10, rightLabelVisible:false, tag:2,change:[this,"doChange"]});
		...
		try {
			...
			this.c_perLama.items.clear();
			var data = this.dbLib.getDataProvider(
						"select distinct periode as periode from ju_m where kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.c_perLama.addItem(i,line.periode);
				}
			}
			this.c_perLama.setText("");
...
window.app_saku2_transaksi_gl_fJuLoad.extend(window.childForm);
window.app_saku2_transaksi_gl_fJuLoad.implement({
	...
	doChange:function(sender){
		if (sender == this.c_perLama && this.c_perLama.getText()!="") {
			this.cb_nbLama.setText("","");
			this.sg.clear(1);
			this.cb_nbLama.setSQL("select no_ju,keterangan from ju_m where periode = '"+this.c_perLama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_ju","keterangan"],false,["No Bukti","Keterangan"],"and","Data Bukti Jurnal",true);
		}
		if (sender == this.cb_nbLama && this.cb_nbLama.getText()!="") {
			var data = this.dbLib.getDataProvider(
						"select a.kode_akun,b.nama as nama_akun,case a.dc when 'D' then 'C' else 'D' end as dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk "+
						"from ju_j a inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
						"            inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
						"            left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and d.tahun='"+this.c_perLama.getText().substr(0,4)+"' "+
						"where a.no_ju = '"+this.cb_nbLama.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
				for (var i in data.rs.rows){
