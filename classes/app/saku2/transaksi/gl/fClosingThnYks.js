window.app_saku2_transaksi_gl_fClosingThnYks = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_gl_fClosingThnYks.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_gl_fClosingThnYks";
...
window.app_saku2_transaksi_gl_fClosingThnYks.extend(window.childForm);
window.app_saku2_transaksi_gl_fClosingThnYks.implement({
	...
	isPostedAll: function(periode){
		...
		if (ck != "") { 
			...
			if (data[1] != "0"){
				result = result && false;
				msg+= "Data Modul PIUTANG [BILL,AMOR,REKON,BTL,SISIH,ARYKS,ARPATRA] masih ada yang belum diposting.("+data[1]+" data)\n";
			}
		}
		...
		if (ck != "") { 
			...
			if (data[1] != "0"){
				result = result && false;
				msg+= "Data Modul AP [HUTKES,REKKES] masih ada yang belum diposting.("+data[1]+" data)\n";
			}
		}
		...
		if (ck != "") { 
			...
			if (data[1] != "0"){
				result = result && false;
				msg+= "Data Modul Investasi [DEPOAKRU,DEPOCAIR,DEPOTUTUP,SHMBELI,SHMJUAL,SHMSPI,RDBELI,RDJUAL,RDSPI] masih ada yang belum diposting.("+data[1]+" data)\n";
			}
		}
