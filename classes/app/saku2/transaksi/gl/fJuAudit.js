window.app_saku2_transaksi_gl_fJuAudit = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_gl_fJuAudit.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_gl_fJuAudit";
...
window.app_saku2_transaksi_gl_fJuAudit.extend(window.childForm);
window.app_saku2_transaksi_gl_fJuAudit.implement({
							...
							if (this.sg.rowValid(i)){	
								...
								if (parseFloat(this.app._periode.substr(0,4)) - parseFloat(this.e_periode.getText().substr(0,4)) == 1) {
									...
									if (typeof data2 == "object" && data2.rs.rows[0] != undefined) {
										sql.add("update glma set so_akhir = so_akhir "+(this.sg.cells(2,i).toUpperCase() == "D" ? "+":"-")+" "+Math.abs(nilaiToFloat(this.sg.cells(4,i)))+" where kode_lokasi='"+this.app._lokasi+"' and kode_akun='"+this.sg.cells(0,i)+"' and periode='"+this.app._periode.substr(0,4)+"01'");
									}
									else {
									...
									if (this.cb2.isSelected()) {
										...
										if (typeof data3 == "object" && data3.rs.rows[0] != undefined) {
											...
											if (line3.modul == "L") {											
												...
												dc = (this.sg.cells(2,i).toUpperCase() == "D" ? "C" : "D");
												...
												sql.add("update glma set so_akhir = so_akhir "+(dc.toUpperCase() == "D" ? "+":"-")+" "+Math.abs(nilaiToFloat(this.sg.cells(4,i)))+" where kode_lokasi = '"+this.app._lokasi+"' and kode_akun = '"+this.sg.cells(0,i)+"' and periode = '"+this.app._periode.substr(0,4)+"01' ");
												sql.add("update glma set so_akhir = so_akhir "+(dc.toUpperCase() == "D" ? "-":"+")+" "+Math.abs(nilaiToFloat(this.sg.cells(4,i)))+" where kode_lokasi = '"+this.app._lokasi+"' and kode_akun = '"+this.akunJP+"' and periode = '"+this.app._periode.substr(0,4)+"01' ");
											}
										}
									}
								}								
	...
	doHitungGar: function(){
		...
		for (var i=0;i < this.sg.getRowCount();i++){
			if (this.sg.rowValid(i) && this.sg.cells(5,i) != "-" && this.sg.cells(7,i)!= "-"){
				...
				if (this.sg.cells(2,i).toUpperCase() == "D") nilai = nilaiToFloat(this.sg.cells(4,i));
