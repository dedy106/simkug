window.app_saku2_transaksi_investasi_fBankMitra = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_investasi_fBankMitra.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_investasi_fBankMitra";
		...
		this.bTampil = new button(this,{bound:[829,13,80,18],caption:"Lihat Data",click:[this,"doTampilClick"]});			
		
		this.p1 = new panel(this,{bound:[10,23,900,343],caption:"Daftar Bank"});
		this.sg1 = new saiGrid(this.p1,{bound:[0,20,895,355],tag:9,readOnly:true,colTitle: ["Kode","Nama","Alamat","Bank","Basis","Rate","Kode Akun","No Rekening","Nama Rekening"]});		
...
window.app_saku2_transaksi_investasi_fBankMitra.extend(window.childForm);
window.app_saku2_transaksi_investasi_fBankMitra.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into inv_bank(kode_bank,nama,alamat,kode_bankklp,jml_hari,p_bunga,kode_akun,no_rek,nama_rek) values "+
						    "('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.e_alamat.getText()+"','"+this.cb_klp.getText()+"',"+parseNilai(this.e_jml.getText())+","+parseNilai(this.e_pbunga.getText())+",'"+this.cb_akun.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"')");					
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update inv_bank set nama='"+this.e_nama.getText()+"',alamat='"+this.e_alamat.getText()+"',kode_bankklp='"+this.cb_klp.getText()+"',jml_hari="+parseNilai(this.e_jml.getText())+",p_bunga="+parseNilai(this.e_pbunga.getText())+",kode_akun='"+this.cb_akun.getText()+"',no_rek='"+this.e_norek.getText()+"',nama_rek='"+this.e_namarek.getText()+"' "+
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){				
				var data = this.dbLib.getDataProvider("select a.nama,a.alamat,a.kode_bankklp,a.jml_hari,a.p_bunga,a.kode_akun,a.no_rek,a.nama_rek from inv_bank a "+						   
