window.app_saku2_transaksi_investasi_fKbDOC = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_investasi_fKbDOC.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_investasi_fKbDOC";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Penempatan Deposito: Input", 0);	
...
window.app_saku2_transaksi_investasi_fKbDOC.extend(window.childForm);
window.app_saku2_transaksi_investasi_fKbDOC.implement({
	...
	doLoad:function(sender){
		if (this.e_periode.getText() != "") {
			var strSQL = "select no_depo,convert(varchar,tanggal,103) as tgl,keterangan,kode_bank,akun_doc,nilai from inv_depo_m where progress ='0' and periode<='"+this.e_periode.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ";
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData(["INPROG",line.no_depo,line.tgl,line.keterangan,line.kode_bank,line.akun_doc,floatToNilai(line.nilai)]);
				}
