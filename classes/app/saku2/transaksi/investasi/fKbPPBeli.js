window.app_saku2_transaksi_investasi_fKbPPBeli = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_investasi_fKbPPBeli.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_investasi_fKbPPBeli";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Pembelian Properti: Input", 0);	
		...
		this.p1 = new panel(this,{bound:[10,23,900,280],caption:"Daftar Penyertaan Properti"});
		this.sg = new saiGrid(this.p1,{bound:[0,20,this.p1.width-5,this.p1.height-45],colCount:7,tag:0,
				colTitle:["Status","No Pembelian","Tanggal","Keterangan","Properti","Akun Penyertaan","Nilai"],
				colWidth:[[6,5,4,3,2,1,0],[100,100,160,200,80,150,80]],
...
window.app_saku2_transaksi_investasi_fKbPPBeli.extend(window.childForm);
window.app_saku2_transaksi_investasi_fKbPPBeli.implement({
					...
					if (this.sg.getRowValidCount() > 0){
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i) && this.sg.cells(0,i) == "APP"){
								...
								sql.add("update inv_ppbeli_m set progress='1', no_kas ='"+this.e_nb.getText()+"' where no_beli='"+this.sg.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
							}
						}
					}					
	...
	doLoad:function(sender){
		if (this.e_periode.getText() != "") {
			var strSQL = "select a.no_beli,convert(varchar,a.tanggal,103) as tgl,a.keterangan,b.nama,a.akun_pp,a.jumlah*a.h_oleh as nilai "+
			             "from inv_ppbeli_m a inner join inv_properti b on a.kode_properti=b.kode_properti "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData(["INPROG",line.no_beli,line.tgl,line.keterangan,line.nama,line.akun_pp,floatToNilai(line.nilai)]);
				}
