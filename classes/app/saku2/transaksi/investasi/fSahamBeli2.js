window.app_saku2_transaksi_investasi_fSahamBeli2 = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_investasi_fSahamBeli2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_investasi_fSahamBeli2";
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('PPINV','SHMSWA','SHMMI','SHMBBN','SHMHUT') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku2_transaksi_investasi_fSahamBeli2.extend(window.childForm);
window.app_saku2_transaksi_investasi_fSahamBeli2.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into inv_shmbeli_m(no_shmbeli,kode_lokasi,periode,tanggal,nik_user,tgl_input,posted,no_kas,progress,nik_buat,  no_dokumen,keterangan,kode_drk,kode_kelola,tgl_set,nilai_komisi,nilai_ppn,nilai_levy,nilai_pph) values "+
					...
					for (var i = 0; i < this.sg.rows.getLength();i++){
						if (this.sg.rowValid(i) && this.sg.cells(7,i) != ""){
							sql.add("insert into inv_shmbeli_d (no_shmbeli,kode_kelola,kode_saham,h_oleh,h_buku,harga,jumlah,n_beli,  komisi,vat,levi,pph,kode_broker) values "+
								    "('"+this.e_nb.getText()+"','"+this.cb_kelola.getText()+"','"+this.sg.cells(0,i)+"',"+nilaiToFloat(this.sg.cells(3,i))+","+nilaiToFloat(this.sg.cells(4,i))+","+nilaiToFloat(this.sg.cells(5,i))+","+nilaiToFloat(this.sg.cells(6,i))+","+nilaiToFloat(this.sg.cells(7,i))+",  "+nilaiToFloat(this.sg.cells(8,i))+","+nilaiToFloat(this.sg.cells(9,i))+","+nilaiToFloat(this.sg.cells(10,i))+","+nilaiToFloat(this.sg.cells(11,i))+",'"+this.sg.cells(12,i)+"')");
						}						
					}
					...
					sql.add("insert into angg_r "+
							...
							"from inv_shmbeli_j where  kode_akun like '4%' and no_shmbeli='"+this.e_nb.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' "+
