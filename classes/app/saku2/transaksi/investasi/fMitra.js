window.app_saku2_transaksi_investasi_fMitra = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_investasi_fMitra.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_investasi_fMitra";
		...
		this.p1 = new panel(this,{bound:[10,23,800,343],caption:"Daftar Mitra"});
...
window.app_saku2_transaksi_investasi_fMitra.extend(window.childForm);
window.app_saku2_transaksi_investasi_fMitra.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into inv_mitra(kode_mitra,nama,akun_spi) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update inv_mitra set nama='"+this.e_nama.getText()+"',akun_spi='"+this.cb_spi.getText()+"' "+
		...
		try{
			if (this.cb_kode.getText() != ""){				
				var data = this.dbLib.getDataProvider("select a.nama,a.akun_spi "+
				           "from inv_mitra a "+						   
						   "where a.kode_mitra ='"+this.cb_kode.getText()+"' ",true);
	...
	doTampilClick: function(sender){
		try{					
			var temp = this.dbLib.runSQL("select kode_mitra,nama,akun_spi from inv_mitra order by kode_mitra");
		...
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Mitra",sender,undefined, 
											  ...
											  ["kode_mitra","nama"],"where",["Kode","Nama"],false);				
			}
		}catch(e){
			systemAPI.alert(e);
		}
