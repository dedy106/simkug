window.app_saku2_transaksi_tak_fTakterima = function(owner)
{
	if (owner)
	{
		...
		this.cb_asal = new saiCBBL(this,{bound:[20,18,200,20],caption:"Lokasi Kirim", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});
		this.cb_kirim = new saiCBBL(this,{bound:[20,19,200,20],caption:"No TAK Kirim", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});
...
window.app_saku2_transaksi_tak_fTakterima.implement({
		...
		try{									
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("update takkirim_m set progress='1' where no_kirim='"+this.cb_kirim.getText()+"' and kode_lokasi='"+this.cb_asal.getText()+"'");
	...
	doChange:function(sender){
		if (sender  == this.cb_asal && this.cb_asal.getText()!= "") {
			...
			this.cb_kirim.setSQL("select no_kirim, keterangan from takkirim_m where modul = 'TAK' and periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.cb_asal.getText()+"' and progress = '0' and kode_loktuj='"+this.app._lokasi+"'",["no_kirim","keterangan"],false,["No Bukti","Keterangan"],"and","Data TAK Kirim",true);			
		}
		if (sender  == this.cb_kirim && this.cb_kirim.getText()!= "") {
			var strSQL = "select a.kode_akun,d.nama,case a.dc when 'D' then 'C' else 'D' end as dc,a.keterangan,a.nilai "+
						 ...
						 "where a.no_kirim='"+this.cb_kirim.getText()+"' and c.kode_loktuj = '"+this.app._lokasi+"' and b.kode_flag ='016' ";
