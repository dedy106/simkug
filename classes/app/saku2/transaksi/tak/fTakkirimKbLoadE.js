window.app_saku2_transaksi_tak_fTakkirimKbLoadE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_tak_fTakkirimKbLoadE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_tak_fTakkirimKbLoadE";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form TAK Permintaan Pembayaran : Load", 0);	
		...
		this.cb_akun = new saiCBBL(this,{bound:[20,20,200,20],caption:"Akun TAK", multiSelection:false, maxLength:10, tag:2});
		this.cb_buat = new saiCBBL(this,{bound:[20,16,200,20],caption:"NIK Pembuat", multiSelection:false, maxLength:10, tag:2});						
		this.e_trans = new saiLabelEdit(this,{bound:[700,16,220,20],caption:"Total Transfer", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		...
		this.pc1 = new pageControl(this,{bound:[20,12,900,280], childPage:["Data Transfer","Data Item Jurnal","Data Anggaran"]});		
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:7,tag:0,
				colTitle:["Mitra","Bank","Cabang","No Rekening","Nama Rekening","Nilai","Keterangan"],				
				colWidth:[[6,5,4,3,2,1,0],[150,80,180,100,200,60,150]],colFormat:[[5],[cfNilai]],readOnly:true, defaultRow:1});
		this.sgn = new portalui_sgNavigator(this.pc1.childPage[0],{bound:[0,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:bsAll, grid:this.sg1});		
...
window.app_saku2_transaksi_tak_fTakkirimKbLoadE.extend(window.childForm);
window.app_saku2_transaksi_tak_fTakkirimKbLoadE.implement({	
		...
		switch (event){
			...
			case "hapus" :	
				...
				if (parseFloat(this.app._periode) > parseFloat(this.e_periode.getText())){
					...
					sql.add("delete from yk_takkirim_d where no_kirim = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from angg_r where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul ='TAKKIRIM'");
					setTipeButton(tbAllFalse);	
					this.dbLib.execArraySQL(sql);
				}
				break;				
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!="") {
			this.e_nb.setSQL("select no_kirim, keterangan from takkirim_m where progress ='0' and modul = 'TAKKBLOAD' and jenis = 'KIRIM' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kirim","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti TAK",true);
		}
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			var data = this.dbLib.getDataProvider(
					   "select convert(varchar,a.tanggal,103) as tanggal,a.periode,a.no_dokumen,a.keterangan,a.kode_loktuj,a.nik_buat,a.nik_setuju,b.nama as nama_buat,c.nama as nama_app,d.nama as loktuj "+
			...
			var total = 0;
			var strSQL = "select a.keterangan,a.nama_rek,a.no_rek,a.bank,a.cabang,a.akun_tak as kode_akun,a.jenis,a.nilai,b.nama as nama_akun "+
					     "from yk_takkirim_d a inner join masakun b on a.akun_tak=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg1.appendData([line.keterangan,line.bank,line.cabang,line.no_rek,line.nama_rek,floatToNilai(line.nilai),line.keterangan]);
					total += parseFloat(line.nilai);
				}
				this.cb_akun.setText(line.kode_akun,line.nama_akun);
				this.c_jenis.setText(line.jenis);
				this.e_trans.setText(floatToNilai(total));
			...
			var data = this.dbLib.getDataProvider(
						...
						"where a.jenis='KIRIM' and a.no_kirim = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
				for (var i in data.rs.rows){
			...
			var data = this.dbLib.getDataProvider(
						...
						"              inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
						"              left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and d.tahun='"+this.e_periode.getText().substr(0,4)+"' "+
						"where a.no_bukti = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='TAKKIRIM' order by a.kode_akun",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg2.clear();
				for (var i in data.rs.rows){
