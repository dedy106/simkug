window.app_saku2_transaksi_tak_fTakkirimKbE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_tak_fTakkirimKbE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_tak_fTakkirimKbE";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form TAK Permintaan Pembayaran : Edit/Hapus", 0);	
		...
		this.cb_tujuan = new saiCBBL(this,{bound:[20,18,200,20],caption:"Lokasi Tujuan", multiSelection:false, maxLength:10, tag:2});				
		...
		this.pc1 = new pageControl(this,{bound:[20,12,900,280], childPage:["Data Vendor","Data Item Jurnal","Data Anggaran"]});
		this.c_jenis = new saiCB(this.pc1.childPage[0],{bound:[20,22,180,20],caption:"Jenis",items:["OPEX","CAPEX"], readOnly:true,tag:2});
		this.cb_vendor = new saiCBBL(this.pc1.childPage[0],{bound:[20,18,200,20],caption:"Data Vendor", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});
		this.cb_akun = new saiCBBL(this.pc1.childPage[0],{bound:[20,20,200,20],caption:"Akun TAK", multiSelection:false, maxLength:10, tag:2});
		this.e_trans = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,18,200,20],caption:"Nilai Transfer", tag:1, tipeText:ttNilai, text:"0",change:[this,"doChange"]});		
		this.e_norek = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,19,450,20],caption:"No Rekening", readOnly:true});				
		this.e_bank = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,20,450,20],caption:"Bank", readOnly:true});		
		this.e_namarek = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,18,450,20],caption:"Nama Rekening", readOnly:true});				
		this.e_cabang = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,21,450,20],caption:"Cabang", readOnly:true});				
		...
		try {
			...
			this.cb_akun.setText("1512000001");
		}catch(e){
			systemAPI.alert(e);
		}
	}
};
window.app_saku2_transaksi_tak_fTakkirimKbE.extend(window.childForm);
window.app_saku2_transaksi_tak_fTakkirimKbE.implement({
	...
	simpan: function(){			
		try{						
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"takkirim_m","no_kirim",this.app._lokasi+"-KRM"+this.e_periode.getText().substr(2,4)+".","000"));
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into takkirim_m(no_kirim,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user,kode_loktuj,progress) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','-','TAKKB','KIRIM','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','F','-','-','"+this.cb_vendor.getText()+"',getdate(),'"+this.app._userLog+"','"+this.cb_tujuan.getText()+"','0')");
					
					sql.add("insert into yk_takkirim_d(no_kirim,kode_lokasi,no_kas,kode_lokkas,keterangan,no_rek,nama_rek,bank,cabang,nilai,jenis,kode_vendor,akun_tak) values "+
							"	('"+this.e_nb.getText()+"','"+this.app._lokasi+"','-','-','"+this.e_ket.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"','"+this.e_bank.getText()+"','"+this.e_cabang.getText()+"',"+parseNilai(this.e_trans.getText())+",'"+this.c_jenis.getText()+"','"+this.cb_vendor.getText()+"','"+this.cb_akun.getText()+"')");
					...
					sql.add("insert into takkirim_j(no_kirim,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_trans.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','TAKKB','TAK','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into takkirim_j(no_kirim,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','"+this.app._lokasi+"','TAKKB','KIRIM','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");
							}
						}
					}
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','TAKKIRIM','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(6,i))+","+nilai+")");
							}
						}
					}					
			...
			case "simpan" :					
				for (var i=0;i < this.sg.getRowCount();i++){					
					if (!this.sg.rowValid(i)){
						...
						if (!isKosong) {
							system.alert(this,"Transaksi tidak valid.","Terdapat kolom yang kosong di grid jurnal.");
							return false;
						}
					}
				}
	...
	doChange:function(sender){
		if (sender == this.cb_vendor && this.cb_vendor.getText()!="") {
			var data = this.dbLib.getDataProvider("select no_rek,nama_rek,bank,cabang from vendor "+					   			           
					   "where kode_vendor='"+this.cb_vendor.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
		...
		if (sender == this.e_trans && this.e_trans.getText()!="") {
			this.sg.validasi();
		}
	},	
	...
	doNilaiChange: function(){
		try{
			...
			totC += nilaiToFloat(this.e_trans.getText());
