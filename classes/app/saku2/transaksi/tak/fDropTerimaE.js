window.app_saku2_transaksi_tak_fDropTerimaE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_tak_fDropTerimaE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_tak_fDropTerimaE";
		...
		this.p1 = new portalui_panel(this,{bound:[20,189,800,240],caption:"Data Droping"});
		this.sg = new saiGrid(this.p1,{bound:[0,20,this.p1.width-5,this.p1.height-50],colCount:8,tag:0,
		            colTitle:["Status","No Bukti","No Dokumen","Lokasi Kirim","Akun TAK","Keterangan","Nilai","Tgl Kirim"],
					colWidth:[[7,6,5,4,3,2,1,0],[80,100,200,80,80,120,100,70]],
		...
		this.cb1 = new portalui_checkBox(this.sgn,{bound:[740,5,100,25],caption:"Preview",selected:true});
...
window.app_saku2_transaksi_tak_fDropTerimaE.extend(window.childForm);
window.app_saku2_transaksi_tak_fDropTerimaE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("update yk_kasdrop_d set progress='0',no_kasterima='-' where no_kasterima ='"+this.e_nb.getText()+"' and kode_loktuj='"+this.app._lokasi+"'");

					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.cb_kas.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBDROPTRM','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"',getdate(),'"+this.app._userLog+"','F','-','-','-','-')");
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i) && this.sg.cells(0,i) == "APP"){
								sql.add("update yk_kasdrop_d set progress='1',no_kasterima='"+this.e_nb.getText()+"' where keterangan ='"+this.sg.cells(5,i)+"' and no_kas ='"+this.sg.cells(1,i)+"' and kode_loktuj='"+this.app._lokasi+"' and kode_lokasi='"+this.sg.cells(3,i)+"'");
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :	
				...
				var temu = 0;
				for (var i=0;i < this.sg.getRowCount();i++){
					if (this.sg.cells(0,i) == "APP") temu += 1;
				}
				if (temu != 1) {
					system.alert(this,"Transaksi tidak valid.","APP hanya untuk satu transaksi per no bukti terima");
					return false;						
				}
				*/
	...
	doChange:function(sender){
		...
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			...
			var data = this.dbLib.getDataProvider(
			              "select a.no_kas,a.no_dokumen,a.kode_lokasi,a.akun_tak,a.keterangan,a.nilai,convert(varchar,b.tanggal,103) as tanggal "+
			              "from yk_kasdrop_d a inner join kas_m b on a.no_kas=b.no_kas and a.kode_lokasi=b.kode_lokasi where a.no_kasterima='"+this.e_nb.getText()+"' and a.kode_loktuj='"+this.app._lokasi+"' and a.progress = '1' and a.periode<='"+this.e_periode.getText()+"'",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData(["APP",line.no_kas,line.no_dokumen,line.kode_lokasi,line.akun_tak,line.keterangan,floatToNilai(line.nilai),line.tanggal]);
				}
	...
	doTampilClick:function(sender){		
		if (this.e_periode.getText() != "") {
			var data = this.dbLib.getDataProvider(
			              "select no_kas,no_dokumen,kode_lokasi,akun_tak,keterangan,nilai "+
			              "from yk_kasdrop_d where kode_loktuj='"+this.app._lokasi+"' and progress = '0' and periode<='"+this.e_periode.getText()+"'",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData(["INPROG",line.no_kas,line.no_dokumen,line.kode_lokasi,line.akun_tak,line.keterangan,floatToNilai(line.nilai)]);
				}
			} else this.sg.clear(1);
		}
		else {
