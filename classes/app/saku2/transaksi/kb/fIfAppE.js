window.app_saku2_transaksi_kb_fIfAppE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kb_fIfAppE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kb_fIfAppE";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Verifikasi Pengajuan I/F: Pembatalan", 0);	
		...
		this.pc1 = new pageControl(this,{bound:[20,12,980,367], childPage:["Data Imprest Fund","Detail Transaksi"]});		
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:10,tag:0,
		            colTitle:["Status","Catatan","No Bukti","No Dokumen","Tanggal","Keterangan","PP","Pengaju","Nilai","No Kas"],
					colWidth:[[9,8,7,6,5,4,3,2,1,0],[100,100,100,100,250,60,100,100,300,80]],
...
window.app_saku2_transaksi_kb_fIfAppE.extend(window.childForm);
window.app_saku2_transaksi_kb_fIfAppE.implement({
		...
		switch (event){
			...
			case "hapus" :	
				...
				for (var i=0;i < this.dataJU.rs.rows.length;i++){
					if (this.dataJU.rs.rows[i].no_kas != "-") isAda = true;
				}
				if (isAda){
					system.alert(this,"Transaksi tidak valid.","Ditemukan transaksi dengan no bukti kas aktif.");
					return false;
				}
				...
				if (parseFloat(this.app._periode) > parseFloat(this.e_periode.getText())){
					...
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update a set a.progress='0' from yk_ifptg_m a inner join yk_ver_d b on a.no_ifptg=b.no_bukti and a.kode_lokasi=b.kode_lokasi "+
	...
	doChange:function(sender){
		if (sender == this.e_periode) {
			this.e_nb.setSQL("select no_ver, keterangan from yk_ver_m where modul = 'IFPTG' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_ver","keterangan"],false,["No KasBank","Deskripsi"],"and","Daftar Bukti",true);
			this.dataJU.rs.rows = [];
			this.sg.clear(1); this.sg2.clear(1);
		}		
		if (sender == this.e_nb && this.e_nb.getText() != "") {
			var strSQL = "select a.tanggal,a.periode,a.keterangan,a.nik_buat,b.nama as nama_buat,a.nik_app,c.nama as nama_app "+					   
					     "from yk_ver_m a "+
	...
	doLoad:function(sender){
		var strSQL = "select e.status,e.catatan,a.no_ifptg,a.no_dokumen,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,d.kode_pp+' - '+b.nama as pp,a.nik_buat+' - '+c.nama as pengaju,a.nilai,a.no_kas "+
					 ...
					 "					inner join yk_ver_d e on a.no_ifptg=e.no_bukti and a.kode_lokasi=e.kode_lokasi "+
					 "where e.no_ver ='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData([line.status.toUpperCase(),line.catatan,line.no_ifptg,line.no_dokumen,line.tanggal,line.keterangan,line.pp,line.pengaju,floatToNilai(line.nilai),line.no_kas]);
		}
	...
	doDoubleClick: function(sender, col , row) {
		if (this.sg.cells(2,row) != "") {
			...
			var strSQL = "select a.kode_pp,b.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk,a.kode_akun,c.nama as nama_akun,a.nilai,a.dc,a.keterangan,convert(varchar,a.tanggal,103) as tanggal  "+
						 ...
						 "where a.no_ifptg = '"+this.sg.cells(2,row)+"' and a.kode_lokasi = '"+this.app._lokasi+"' order by a.dc desc ";			
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
