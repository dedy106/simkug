window.app_saku2_transaksi_kb_fSpbLoad = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kb_fSpbLoad.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kb_fSpbLoad";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Permintaan Pembayaran : Load", 0);	
		...
		this.cb_pp = new saiCBBL(this,{bound:[20,14,200,20],caption:"PP", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});				
		this.cb_buat = new saiCBBL(this,{bound:[20,16,200,20],caption:"NIK Pembuat", multiSelection:false, maxLength:10, tag:2});								
		this.cb_app = new saiCBBL(this,{bound:[20,17,200,20],caption:"NIK Approve", multiSelection:false, maxLength:10, tag:2});								
		this.e_trans = new saiLabelEdit(this,{bound:[700,17,220,20],caption:"Total Transfer", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		this.c_jenis = new saiCB(this,{bound:[20,22,180,20],caption:"Jenis",items:["OPEX","CAPEX"], readOnly:true,tag:2});
		this.e_total = new saiLabelEdit(this,{bound:[700,22,220,20],caption:"Total Jurnal", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		this.bUpload = new portalui_uploader(this,{bound:[600,22,80,18],param3:"object",param4:"gridupload",afterUpload:[this,"doAfterUpload"],autoSubmit:true});
		
		this.pc1 = new pageControl(this,{bound:[20,12,900,320], childPage:["Data Transfer","Data Item Jurnal","Data Anggaran"]});		
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:6,tag:0,
		...
		this.sgn1 = new portalui_sgNavigator(this.pc1.childPage[1],{bound:[1,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:bsAll,grid:this.sg});		
		...
		try {
			...
			this.cb_pp.setSQL("select kode_pp, nama from pp where kode_bidang='"+this.kodeBidang+"' and tipe='posting' and flag_aktif ='1' and kode_lokasi='"+this.app._lokasi+"'",["kode_pp","nama"],false,["Kode","Nama"],"and","Data PP",true);			
		}catch(e){
			systemAPI.alert(e);
		}
	}
};
window.app_saku2_transaksi_kb_fSpbLoad.extend(window.childForm);
window.app_saku2_transaksi_kb_fSpbLoad.implement({
	    ...
	    try{   			
			...
			if (result) {								
				...
				for (var j=0; j < this.dataUpload.rows.length;j++){
					...
					this.totTrans += parseFloat(line1.nilai);					
				}
				this.e_trans.setText(floatToNilai(this.totTrans));	
			}else throw(data);					
   		}catch(e){
	...
	selectPage: function(sender,page){
		...
		for (var i=start; i < finish;i++){
			...
			this.sg1.appendData([line.mitra,line.bank,line.cabang,line.no_rek,line.nama_rek,floatToNilai(line.nilai)]);
		}
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into yk_spb_m(no_spb,kode_lokasi,no_dokumen,tanggal,due_date,keterangan,nik_buat,nik_app,kode_vendor,kode_pp,nilai,periode,nik_user,tgl_input,no_ver,no_kas,modul) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','LOAD','"+this.app._kodePP+"',"+parseNilai(this.e_trans.getText())+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'-','-','LOAD')");
									
					var line;														
					for (var j=0; j < this.dataUpload.rows.length;j++){
						line = this.dataUpload.rows[j];
						sql.add("insert into yk_spb_d(no_spb,kode_lokasi,no_kas,keterangan,no_rek,nama_rek,bank,cabang,nilai,jenis) values "+
							    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','-','"+line.mitra+"','"+line.no_rek+"','"+line.nama_rek+"','"+line.bank+"','"+line.cabang+"',"+line.nilai+",'"+this.c_jenis.getText()+"')");
					}										
					
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){								
								sql.add("insert into yk_spb_j(no_spb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','"+this.app._lokasi+"','TAKKBLOAD','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
							}
						}
					}
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','SPBLOAD','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(6,i))+","+nilai+")");
							}
						}
					}					
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "simpan" :									
				...
				if (nilaiToFloat(this.e_total.getText()) != nilaiToFloat(this.e_trans.getText())) {
					system.alert(this,"Transaksi tidak valid.","Total Jurnal harus sama dengan Nilai Transfer.");
					return false;						
				}
	...
	doChange:function(sender){
		if (sender == this.cb_pp && this.cb_pp.getText()!= "") {
			this.cb_buat.setSQL("select nik, nama from karyawan where kode_pp = '"+this.cb_pp.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Data Karyawan",true);			
			this.cb_app.setSQL("select  nik, nama from karyawan where kode_pp = '"+this.cb_pp.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Data Karyawan",true);			
		}
	},
	doClick:function(sender){
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"yk_spb_m","no_spb",this.app._lokasi+"-LPB"+this.e_periode.getText().substr(2,4)+".","000"));
		this.e_dok.setFocus();
	},		
