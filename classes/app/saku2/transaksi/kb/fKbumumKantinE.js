window.app_saku2_transaksi_kb_fKbumumKantinE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kb_fKbumumKantinE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kb_fKbumumKantinE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Umum : Edit", 0);	
		...
		this.cb_jtrans = new saiCBBL(this,{bound:[20,17,200,20],caption:"Jenis Transaksi", multiSelection:false, maxLength:10, tag:2});		
		this.e_kredit = new saiLabelEdit(this,{bound:[700,17,220,20],caption:"Total Kredit", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		
		this.pc1 = new pageControl(this,{bound:[20,12,900,257], childPage:["Data Item Jurnal","Data Anggaran"]});
		...
		try {
			...
			this.cb_jtrans.setSQL("select kode_trans, nama from kt_jenis_trans where kode_lokasi='"+this.app._lokasi+"'",["kode_trans","nama"],false,["Kode","Nama"],"and","Data Jenis Transaksi",true);
			...
			this.flagGarFree = "0"; this.flagDokFree = "0"; this.flagDakemFree = "0"; this.flagFAGL = "0"; this.ppFA = "-";
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('FAGL','FAPP','DAKEMFREE','GARFREE','DOKFREE') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku2_transaksi_kb_fKbumumKantinE.extend(window.childForm);
window.app_saku2_transaksi_kb_fKbumumKantinE.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from angg_r where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul ='KBKT'");
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.e_gb.getText()+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','-','KBKT','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"',getdate(),'"+this.app._userLog+"','F','-','-','"+this.cb_jtrans.getText()+"','-')");
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
										"	('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','-','-','"+this.app._lokasi+"','KBKT','UMUM','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");
							}
						}
					}										
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','KBKT','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(6,i))+","+nilai+")");
							}
						}
					}					
					if (this.flagFAGL == "1") {
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			case "clear" :
				...
				break;
			case "ubah" :	
				if (this.flagDakemFree == "0") {
					var data = this.dbLib.getDataProvider("select kode_akun from flag_relasi where kode_flag = '033' and kode_lokasi='"+this.app._lokasi+"'",true);
					if (typeof data == "object" && data.rs.rows[0] != undefined){
						this.dataJU = data;
					} 				
					var k=0;
						...
						if (this.sg.rowValid(j)){
							for (var i=0;i<this.dataJU.rs.rows.length;i++){
								...
								if (line.kode_akun == this.sg.cells(0,j)) {
									system.alert(this,"Transaksi tidak valid.","Akun Dakem tidak diperkenankan dipakai di KB Umum.");
									return false;								
								}
							}
						}
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!="") {
			this.e_nb.setSQL("select no_kas, keterangan from kas_m where no_del='-' and jenis in ('KK','BK','KM','BM') and modul = 'KBKT' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kas","keterangan"],false,["No KB","Deskripsi"],"and","Daftar Bukti KB",true);
		}
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			var data = this.dbLib.getDataProvider("select a.no_dokumen,convert(varchar,a.tanggal,103) as tanggal,a.periode,a.no_bg,a.keterangan,a.jenis,a.nik_buat,b.nama as nama_buat,a.nik_app,c.nama as nama_app, a.ref1 "+
			           "from kas_m a "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){
					...
					this.cb_jtrans.setText(line.ref1);
			...
			if (sender == this.sg) {
				if (col == 0){
					var sql="select a.kode_akun,a.nama "+
						...
						"where a.kode_lokasi='"+this.app._lokasi+"' and b.kode_flag='051' ";
