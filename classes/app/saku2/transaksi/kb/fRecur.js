window.app_saku2_transaksi_kb_fRecur = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kb_fRecur.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kb_fRecur";
		this.itemsValue = new arrayList();
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Keluar BDD: Input", 0);	
...
window.app_saku2_transaksi_kb_fRecur.extend(window.childForm);
window.app_saku2_transaksi_kb_fRecur.implement({
	...
	simpan: function(){			
		try{						
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"kas_m","no_kas",this.app._lokasi+"-"+this.jenis+this.e_periode.getText().substr(2,4)+".","000"));
	...
	doChange:function(sender){
		if (sender == this.c_jenis) {
			if (this.c_jenis.getText() == "KAS") this.jenis = "KK"; else this.jenis = "BK"; 
			this.e_nb.setText("");
		}
	...
	doCellEnter2: function(sender, col, row){
		switch(col){
			case 3 : 
				...
				break;
			case 4 : 
					if (this.sg2.cells(4,row) == "" && row > 0) {
						this.sg2.setCell(4,row,this.sg2.cells(4,(row-1)));
					}
				break;
			case 5 : 
