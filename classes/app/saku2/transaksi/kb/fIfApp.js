window.app_saku2_transaksi_kb_fIfApp = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kb_fIfApp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kb_fIfApp";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Verifikasi Pengajuan I/F: Input", 0);	
...
window.app_saku2_transaksi_kb_fIfApp.extend(window.childForm);
window.app_saku2_transaksi_kb_fIfApp.implement({
	...
	simpan: function(){			
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{				
					this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"yk_ver_m","no_ver",this.app._lokasi+"-VIF"+this.e_periode.getText().substr(2,4)+".","000"));
					...
					sql.add("insert into yk_ver_m(no_ver, kode_lokasi,tanggal,keterangan,modul,periode,no_del,nik_buat,nik_app,nik_user,tgl_input,jenis_form) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','IFPTG','"+this.e_periode.getText()+"','-','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.app._userLog+"',now(),'APP')");
					...
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						...
						if (line.status.toUpperCase() == "APP"){
							sql.add("insert into yk_ver_d(no_ver,modul,kode_lokasi,no_bukti,jenis,catatan,status) values "+
									"('"+this.e_nb.getText()+"','IFPTG','"+this.app._lokasi+"','"+line.no_ifptg+"','-','"+line.catatan+"','"+line.status+"')");							
							sql.add("update yk_ifptg_m set progress='1' where no_ifptg='"+line.no_ifptg+"' and kode_lokasi='"+this.app._lokasi+"'");
						}						
					}				
	...
	doLoad:function(sender){
		var strSQL = "select 'INPROG' as status,'-' as catatan,a.no_ifptg,a.no_dokumen,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,d.kode_pp+' - '+b.nama as pp,a.nik_buat+' - '+c.nama as pengaju,a.nilai "+
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU.rs.rows[i];							
			this.sg.appendData([line.status.toUpperCase(),line.catatan,line.no_ifptg,line.no_dokumen,line.tanggal,line.keterangan,line.pp,line.pengaju,floatToNilai(line.nilai)]);
		}
