window.app_saku2_transaksi_kb_fSpbVer = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kb_fSpbVer.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kb_fSpbVer";
...
window.app_saku2_transaksi_kb_fSpbVer.extend(window.childForm);
window.app_saku2_transaksi_kb_fSpbVer.implement({
	...
	simpan: function(){			
		try{						
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"yk_spbver_m","no_ver",this.app._lokasi+"-VSPB"+this.e_periode.getText().substr(2,4)+".","0000"));
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();										
					sql.add("insert into yk_spbver_m (no_ver,tanggal,kode_lokasi,periode,keterangan,nik_buat,nik_user,tgl_input) values "+
					...
					sql.add("update yk_spb_m set nilai="+parseNilai(this.e_total.getText())+",no_ver='"+this.e_nb.getText()+"' where no_spb='"+this.cb_spb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into yk_spb_j(no_spb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
										"('"+this.cb_spb.getText()+"','"+this.e_nb.getText()+"','"+this.dp_d1.getDateString()+"',"+k+",'"+this.sg2.cells(0,i)+"','"+this.sg2.cells(6,i)+"','"+this.sg2.cells(7,i)+"',"+parseNilai(this.sg2.cells(8,i))+",'"+this.sg2.cells(2,i)+"','"+this.sg2.cells(4,i)+"','"+this.app._lokasi+"','SPB','VER','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
							}						
						}
					}									
	...
	doChange:function(sender){
		if (sender == this.e_periode ) {
			if (this.e_periode.getText()!="") {
				var strSQL = "select no_spb, keterangan from yk_spb_m where no_ver='-' and periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";
				this.cb_spb.setSQL(strSQL,["no_spb","keterangan"],false,["No SPB","Keterangan"],"and","Data SPB",true);			
			}
		}		
