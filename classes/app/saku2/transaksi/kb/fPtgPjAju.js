window.app_saku2_transaksi_kb_fPtgPjAju = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kb_fPtgPjAju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kb_fPtgPjAju";
		...
		this.sgn = new portalui_sgNavigator(this.pc1.childPage[1],{bound:[1,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:bsAll,grid:this.sg, pager:[this,"selectPage"]});		
		...
		this.bUpload = new portalui_uploader(this.sgn2,{bound:[840,2,80,18],param3:"object",param4:"gridupload",afterUpload:[this,"doAfterUpload"],autoSubmit:true});
...
window.app_saku2_transaksi_kb_fPtgPjAju.extend(window.childForm);
window.app_saku2_transaksi_kb_fPtgPjAju.implement({
	...
	simpan: function(){			
		try{									
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"ptg_m","no_ptg",this.app._lokasi+"-PJPG"+this.e_periode.getText().substr(2,4)+".","000"));
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','PJPTGAJU','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(4,i)+"','"+this.periodePJ+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(6,i))+","+nilai+")");
							}
						}
					}
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_buat && this.cb_buat.getText() != "") {
			this.cb_pj.setText("","");
			this.cb_pj.setSQL("select no_pjaju,keterangan from yk_pjaju_m where periode<='"+this.e_periode.getText()+"' and progress='3' and nik_buat='"+this.cb_buat.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_pjaju","keterangan"],false,["No Panjar","Keterangan"],"and","Data Panjar",true);						
		}
		if (sender == this.cb_pj && this.cb_pj.getText() != "") {
			this.sg.clear(1);
			var data = this.dbLib.getDataProvider("select akun_pj,nilai,periode from yk_pjaju_m where no_pjaju='"+this.cb_pj.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line = data.rs.rows[0];							
			...
			var data = this.dbLib.getDataProvider(
						"select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk,convert(varchar,getdate(),103) as tgl "+
	...
	doHitungGar: function(){
		...
		var sls = 0;
		for (var i=0;i < this.sg2.getRowCount();i++){
			//pake periode ambil panjar (panjar maret, realisasi april...pake gar maret)
			var data = this.dbLib.getDataProvider("select fn_cekagg3('"+this.sg2.cells(2,i)+"','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(4,i)+"','"+this.periodePJ+"','"+this.cb_pj.getText()+"') as gar ",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line = data.rs.rows[0];
				data = line.gar.split(";");
