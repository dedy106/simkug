window.app_saku2_transaksi_kb_fRecurE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kb_fRecurE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kb_fRecurE";
		this.itemsValue = new arrayList();
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Keluar BDD: Edit", 0);	
		
		uses("saiCBBL;saiEdit;saiGrid;sgNavigator;datePicker;pageControl;checkBox");
		...
		this.c_jenis = new saiCB(this,{bound:[20,22,202,20],caption:"Jenis",items:["KAS","BANK"], readOnly:true,tag:2, change:[this,"doChange"]});
		...
		this.e_debet = new saiLabelEdit(this,{bound:[700,16,220,20],caption:"Total Debet", tag:1, readOnly:true, tipeText:ttNilai, text:"0",change:[this,"doChange"]});		
		...
		this.pc1 = new pageControl(this,{bound:[20,12,900,260], childPage:["Data Item Jurnal","Data Transaksi Recurring","Data Anggaran"]});
		...
		this.c_tahun = new saiCB(this.pc1.childPage[1],{bound:[20,11,180,20],caption:"Periode Awal",change:[this,"doChange"]}); 
		this.c_tahun.addItem(0,this.app._periode.substr(0,4));
		this.c_tahun.addItem(1,"2"+floatToNilai(parseFloat(this.app._periode.substr(0,4)) + 1).substr(2,3));
		this.c_bulan = new saiCB(this.pc1.childPage[1],{bound:[200,11,50,20], labelWidth:0, caption:"Bulan",items:["01","02","03","04","05","06","07","08","09","10","11","12"],change:[this,"doChange"]}); 
		this.e_jml = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,12,180,20], caption:"Jumlah Bulan", tag:9, tipeText:ttNilai, text:"0",change:[this,"doChange"]});		
		this.e_bdd = new saiLabelEdit(this.pc1.childPage[1],{bound:[655,12,200,20], caption:"Nilai BDD", tag:1, tipeText:ttNilai, text:"0"});		
		this.e_perAkhir = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,13,180,20],caption:"Periode Akhir", tag:1, readOnly:true});		
		this.e_nilai = new saiLabelEdit(this.pc1.childPage[1],{bound:[655,13,200,20], caption:"Nilai Recurring", tag:1, tipeText:ttNilai, text:"0",readOnly:true});		
		
		this.sg2 = new saiGrid(this.pc1.childPage[1],{bound:[1,14,this.pc1.width-5,this.pc1.height-110],colCount:9,tag:0,
					...
					cellEnter:[this,"doCellEnter2"],ellipsClick:[this,"doEllipsClick"],change:[this,"doChangeCell"],nilaiChange:[this,"doNilaiChange2"],
					autoAppend:true,defaultRow:1});
...
window.app_saku2_transaksi_kb_fRecurE.extend(window.childForm);
window.app_saku2_transaksi_kb_fRecurE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from kas_bdd_m where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from kas_bdd_d where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from kas_m where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from kas_j where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from angg_r where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul ='KBBDD'");
					
					this.perAwal = this.c_tahun.getText() + this.c_bulan.getText();
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.e_gb.getText()+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','-','KBBDD','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"',getdate(),'"+this.app._userLog+"','F','-','-','-','-')");
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
										"	('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','"+this.sg.cells(9,i)+"','-','"+this.app._lokasi+"','KBBDD','REC','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");
							}
						}
					}					
					sql.add("insert into kas_bdd_m (no_kas,kode_lokasi,no_dokumen,keterangan,per_awal,per_akhir,flag_periode,jumlah,nilai_bdd,nilai_rec) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.perAwal+"','"+this.e_perAkhir.getText()+"','"+this.perAwal+"',"+parseNilai(this.e_jml.getText())+","+parseNilai(this.e_bdd.getText())+","+parseNilai(this.e_nilai.getText())+")");
					if (this.sg2.getRowValidCount() > 0){
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								sql.add("insert into kas_bdd_d(no_kas,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi) values "+
										"	('"+this.e_nb.getText()+"',"+i+",'"+this.sg2.cells(0,i)+"','"+this.sg2.cells(3,i)+"','"+this.sg2.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg2.cells(4,i))+",'"+this.sg2.cells(5,i)+"','"+this.sg2.cells(7,i)+"','"+this.app._lokasi+"')");
							}
						}
					}
						...
						for (var i=0;i < this.sg3.getRowCount();i++){
							if (this.sg3.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','KBBDD','"+this.app._lokasi+"','"+this.sg3.cells(0,i)+"','"+this.sg3.cells(2,i)+"','"+this.sg3.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg3.cells(6,i))+","+nilai+")");
							}
						}
					}
		...
		switch (event){
			...
			case "ubah" :	
				...
				if (this.debet != this.kredit) {
					system.alert(this,"Transaksi tidak valid.","Total Debet dan Kredit Setting Recuuring tidak sama.");
					return false;						
				}
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText() != "") {
				this.e_nb.setSQL("select no_kas, keterangan from kas_m where no_del='-' and modul = 'KBBDD' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kas","keterangan"],false,["No KasBank","Deskripsi"],"and","Daftar Bukti KB",true);
		}
		if (sender == this.e_nb && this.e_nb.getText() != "") {
			var data = this.dbLib.getDataProvider("select a.no_dokumen,a.no_bg,convert(varchar,a.tanggal,103) as tanggal,a.periode,a.keterangan,a.jenis,a.nik_buat,b.nama as nama_buat,a.nik_app,c.nama as nama_setuju "+					   
					   "from kas_m a "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){
					...
					this.cb_app.setText(line.nik_app,line.nama_setuju);
				} 
			}
			var data = this.dbLib.getDataProvider(
						"select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk,isnull(a.kode_cf,'-') as kode_cf,isnull(e.nama,'-') as nama_cf "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData([line.kode_akun,line.nama_akun,line.dc,line.keterangan,floatToNilai(line.nilai),line.kode_pp,line.nama_pp,line.kode_drk,line.nama_drk,line.kode_cf,line.nama_cf]);
				}
			...
			var data = this.dbLib.getDataProvider(
						...
						"	    aa.per_awal,aa.per_akhir,aa.jumlah,aa.nilai_bdd "+
						"from kas_bdd_d a "+
						"			  inner join kas_bdd_m aa on aa.no_kas=a.no_kas and a.kode_lokasi=aa.kode_lokasi "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				this.c_tahun.setText(line.per_awal.substr(0,4));
				this.c_bulan.setText(line.per_awal.substr(4,2));
				this.e_perAkhir.setText(line.per_akhir);
				this.e_jml.setText(floatToNilai(line.jumlah));
				this.e_bdd.setText(floatToNilai(line.nilai_bdd));
			...
			var data = this.dbLib.getDataProvider(
						...
						"              inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
						"              left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and d.tahun='"+this.e_periode.getText().substr(0,4)+"' "+
						"where a.no_bukti = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='KBBDD' order by a.kode_akun",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg3.clear();
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg3.appendData([line.kode_akun,line.nama_akun,line.kode_pp,line.nama_pp,line.kode_drk,line.nama_drk,floatToNilai(line.saldo),floatToNilai(line.nilai),floatToNilai(line.sakhir)]);
				}
			} else this.sg3.clear(1);
		}
		if (sender == this.c_tahun || sender == this.c_bulan || sender == this.e_jml) {
			if (this.c_tahun.getText() != "" && this.c_bulan.getText() != "" && this.e_jml.getText() != "") {
				this.e_perAkhir.setText(nextNPeriode(this.c_tahun.getText()+this.c_bulan.getText(),nilaiToFloat(this.e_jml.getText())));
			}
		}
	},
	    ...
	    if (col == 0) {
			if (sender.cells(0,row) != "") {
				...
				if (akun) sender.cells(1,row,akun);
					...
					if (trim(sender.cells(0,row)) != "") 
					system.alert(this,"Kode Akun "+sender.cells(0,row)+" tidak ditemukan","Inputkan kode lainnya.","checkAkun");                
					sender.cells(0,row,"");
					sender.cells(1,row,"");
				}
				//sender.cells(7,row,"");
		...
		if (col == 5) {
			if (sender.cells(5,row) != "") {
				...
				if (pp) sender.cells(6,row,pp);
					...
					if (trim(sender.cells(5,row)) != "") 
					system.alert(this,"Kode PP "+sender.cells(5,row)+" tidak ditemukan","Inputkan kode lainnya.","checkPP");                
					sender.cells(5,row,"");
					sender.cells(6,row,"");
				}
				//sender.cells(7,row,"");
	...
	doNilaiChange2: function(){
		...
		this.debet = totD; this.kredit = totC;
	},	
	doEllipsClick: function(sender, col, row){
		try{			
			if (sender == this.sg || sender == this.sg2) {
				if (col == 0){
				...
				if (col == 7){					
					...
					this.standarLib.showListData(this, "Daftar DRK",sender,undefined, 
													  "select distinct a.kode_drk, a.nama from drk a inner join anggaran_d b on a.kode_drk=b.kode_drk where a.tahun=substring(b.periode,1,4) and b.periode like '"+this.e_periode.getText().substr(0,4)+"%' and b.kode_akun='"+sender.cells(0,row)+"' and b.kode_pp = '"+sender.cells(5,row)+"' and a.kode_lokasi='"+this.app._lokasi+"'",
	...
	doHitungGar: function(){
		...
		for (var i=0;i < this.sg.getRowCount();i++){
			if (this.sg.rowValid(i) && this.sg.cells(5,i) != "-" && this.sg.cells(7,i)!= "-"){
				...
				for (var j=0;j < this.sg3.getRowCount();j++){
					if (this.sg.cells(0,i) == this.sg3.cells(0,j) && this.sg.cells(5,i) == this.sg3.cells(2,j) && this.sg.cells(7,i) == this.sg3.cells(4,j)) {
						isAda = true;
						idx = j;
						break;
					}
				...
				if (!isAda) {
					this.sg3.appendData([this.sg.cells(0,i),this.sg.cells(1,i),this.sg.cells(5,i),this.sg.cells(6,i),this.sg.cells(7,i),this.sg.cells(8,i),"0",floatToNilai(nilai),"0"]);
				} 
				else { 
		...
		var sls = 0;
		for (var i=0;i < this.sg3.getRowCount();i++){			
			var data = this.dbLib.getDataProvider("select fn_cekagg3('"+this.sg3.cells(2,i)+"','"+this.app._lokasi+"','"+this.sg3.cells(0,i)+"','"+this.sg3.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.e_nb.getText()+"') as gar ",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line = data.rs.rows[0];
				data = line.gar.split(";");
