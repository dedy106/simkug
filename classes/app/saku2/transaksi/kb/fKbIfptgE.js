window.app_saku2_transaksi_kb_fKbIfptgE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kb_fKbIfptgE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kb_fKbIfptgE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Pertanggungan IF: Edit", 0);	
		...
		this.cb_ptg = new saiCBBL(this,{bound:[20,15,220,20],caption:"No Pertanggungan", readOnly:true, maxLength:10, tag:1});
		...
		this.pc1 = new pageControl(this,{bound:[20,12,980,327], childPage:["Data Item Pertgg","Item Jurnal KasBank"]});
		this.sg3 = new saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:10,tag:0,
...
window.app_saku2_transaksi_kb_fKbIfptgE.extend(window.childForm);
window.app_saku2_transaksi_kb_fKbIfptgE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("update yk_ifapp_m set no_kas='-' where no_app='"+this.cb_ptg.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("update yk_ifptg_m set no_kas='-',progress='1' where no_app='"+this.cb_ptg.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					
					sql.add("update yk_ifapp_m set no_kas='"+this.e_nb.getText()+"' where no_app='"+this.cb_ptg.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("update yk_ifptg_m set no_kas='"+this.e_nb.getText()+"',progress='2' where no_app='"+this.cb_ptg.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','-','KBIF','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"',getdate(),'"+this.app._userLog+"','F','-','-','"+this.cb_ptg.getText()+"','-')");										
					...
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank)  "+					
							"select '"+this.e_nb.getText()+"',a.no_ifptg,'"+this.dp_d1.getDateString()+"',a.no_urut,a.kode_akun,a.keterangan,a.dc,a.nilai,a.kode_pp,a.kode_drk,'-','-',a.kode_lokasi,'KBIF','IFPTG','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-' "+
							"from yk_ifptg_j a inner join yk_ifptg_m b on a.no_ifptg=b.no_ifptg and a.kode_lokasi=b.kode_lokasi "+
							"where b.no_app='"+this.cb_ptg.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"' ");					
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								...
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
										"	('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','-','"+this.sg.cells(7,i)+"','-','"+this.app._lokasi+"','KBIF','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");
							}
						}
					}										
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText() != "") {
				this.e_nb.setSQL("select no_kas, keterangan from kas_m where no_del='-' and modul = 'KBIF' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kas","keterangan"],false,["No KasBank","Deskripsi"],"and","Daftar Bukti KB",true);
		}
		if (sender == this.e_nb && this.e_nb.getText() != "") {
			var data = this.dbLib.getDataProvider("select d.no_app,d.keterangan as ket,a.no_dokumen,convert(varchar,a.tanggal,103) as tanggal,a.periode,a.keterangan,a.jenis,a.nik_buat,b.nama as nama_buat,a.nik_app,c.nama as nama_setuju "+					   
					   "from kas_m a "+
			           "	inner join karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+
			           "	inner join karyawan c on a.nik_app=c.nik and a.kode_lokasi=c.kode_lokasi "+
					   "	inner join yk_ifapp_m d on a.no_kas=d.no_kas and a.kode_lokasi=d.kode_lokasi "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){
					...
					this.cb_ptg.setText(line.no_app,line.ket);
				} 
			}			
			var data = this.dbLib.getDataProvider(
						...
						"from yk_ifptg_j a inner join yk_ifptg_m xx on a.no_ifptg=xx.no_ifptg and a.kode_lokasi=xx.kode_lokasi "+
						...
						"where xx.no_app = '"+this.cb_ptg.getText()+"' and xx.kode_lokasi='"+this.app._lokasi+"' order by a.tanggal",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg3.clear();
