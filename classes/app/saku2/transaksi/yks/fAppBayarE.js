window.app_saku2_transaksi_yks_fAppBayarE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_yks_fAppBayarE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_yks_fAppBayarE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Approve Pembayaran Hutang: Edit", 0);	
		...
		this.cb_hutang = new saiCBBL(this,{bound:[20,22,200,20],caption:"No Hutang", readOnly:true});
...
window.app_saku2_transaksi_yks_fAppBayarE.extend(window.childForm);
window.app_saku2_transaksi_yks_fAppBayarE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{																
					...
					sql.add("update yk_hutang_d set no_app='-' where no_app = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!="") {			
			this.e_nb.setSQL("select a.no_app, a.keterangan from yk_app_m a "+
			                 "       left join yk_kashutang_d b on a.no_app=b.no_app and a.kode_lokasi=b.kode_lokasi "+
							 "where b.no_app is null and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",["a.no_app","a.keterangan"],false,["No Approve","Deskripsi"],"and","Daftar Bukti Approval",true);
		}
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			var data = this.dbLib.getDataProvider(
					   "select a.no_hutang,d.keterangan as ket,convert(varchar,a.tanggal,103) as tanggal,a.periode,a.keterangan,a.nik_buat,b.nama as nama_buat,a.nik_app,c.nama as nama_app "+
					   "from yk_app_m a "+
			           "	inner join karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+
			           "	inner join karyawan c on a.nik_app=c.nik and a.kode_lokasi=c.kode_lokasi "+
					   "	inner join yk_hutang_m d on a.no_hutang=d.no_hutang and a.kode_lokasi=d.kode_lokasi "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){
					...
					this.cb_hutang.setText(line.no_hutang,line.ket);
				} 
			}
			var data = this.dbLib.getDataProvider(
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData(["APP",line.kode_vendor,line.nama_vendor,line.no_inv,floatToNilai(line.nilai_bp),floatToNilai(line.nilai_cc),floatToNilai(line.total)]);
				}
