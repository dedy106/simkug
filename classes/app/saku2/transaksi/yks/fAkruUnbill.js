window.app_saku2_transaksi_yks_fAkruUnbill = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_yks_fAkruUnbill.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_yks_fAkruUnbill";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Akru Hutang: Input", 0);	
		...
		this.e_nb = new portalui_saiLabelEdit(this,{bound:[20,12,202,20],caption:"No Hutang",maxLength:30,readOnly:true});
...
window.app_saku2_transaksi_yks_fAkruUnbill.extend(window.childForm);
window.app_saku2_transaksi_yks_fAkruUnbill.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into yk_hutang_m(no_hutang,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,tgl_input,nik_user) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','-','HUTKES','BILL','IDR',1,"+parseNilai(this.e_kredit.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','F',getdate(),'"+this.app._userLog+"')");
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i) && this.sg2.cells(4,i) != "0"){
								sql.add("insert into yk_hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg2.cells(0,i)+"','"+this.sg2.cells(3,i)+"','"+this.sg2.cells(2,i)+"',"+parseNilai(this.sg2.cells(4,i))+",'"+this.kodepp+"','"+this.sg2.cells(6,i)+"','"+this.app._lokasi+"','HUTKES','"+this.sg2.cells(5,i)+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");
								if (this.sg2.cells(6,i) != "-") {
									sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
											"	('"+this.e_nb.getText()+"','HUTKES','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.kodepp+"','"+this.sg2.cells(6,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','D',0,"+parseNilai(this.sg2.cells(4,i))+")");
								}
							}
						}
					}
					...
					sql.add("insert into yk_hutang_d(no_hutang,kode_lokasi,no_inv,periode,kode_vendor,bank,cabang,no_rek,nama_rek,nilai_bp,nilai_cc,bank_trans,no_kas,kode_lokkas,no_rekon,kode_lokrek,no_app,lok_bayar) "+
							...
							" where a.no_bill in ("+nobukti+") and d.jenis = 'NONRESTITUSI' and d.progress='0' and d.periode<='"+this.e_periode.getText()+"' and d.kode_lokasi='"+this.app._lokasi+"' "+
	...
	doTampilClick:function(sender){		
		if (this.e_periode.getText() != "") {
			var data = this.dbLib.getDataProvider(
			              "select no_bill,convert(varchar,tanggal,103) as tanggal,keterangan,nilai "+
			              "from yk_bill_m where jenis = 'NONRESTITUSI' and progress = '0' and periode<='"+this.e_periode.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
	...
	doJurnalClick:function(sender){		
		...
		var data = this.dbLib.getDataProvider(
							...
							"where a.no_bill in ("+nobukti+") and c.jenis = 'NONRESTITUSI' and c.progress ='0' and c.periode<='"+this.e_periode.getText()+"' and c.kode_lokasi='"+this.app._lokasi+"' "+
							...
							"				  inner join yk_bill_m d on a.no_bill=d.no_bill and a.kode_lokasi=d.kode_lokasi "+
							"				  cross join ( "+
							"        					select a.flag as kode_akun,b.nama "+
							"        					from spro a inner join masakun b on a.flag=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
							"        					where a.kode_spro = 'HUTPPH21' and a.kode_lokasi='"+this.app._lokasi+"') cc "+
							"where a.no_bill in ("+nobukti+") and d.jenis = 'NONRESTITUSI' and d.progress='0' and d.periode<='"+this.e_periode.getText()+"' and d.kode_lokasi='"+this.app._lokasi+"' "+
							"group by cc.kode_akun,cc.nama "+
							
							"order by dc desc,kode_akun",true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			var line;
			this.sg2.clear();
			for (var i in data.rs.rows){
		...
		this.sg2.validasi();
		var data = this.dbLib.getDataProvider(
					"select b.nama_rek,b.no_rek,b.bank,b.cabang,a.kode_vendor,b.nama as vendor,b.bank_trans,"+
					...
					"    inner join yk_bill_m d on a.no_bill=d.no_bill and a.kode_lokasi=d.kode_lokasi "+
					" where a.no_bill in ("+nobukti+") and d.jenis = 'NONRESTITUSI' and d.progress='0' and d.periode<='"+this.e_periode.getText()+"' and d.kode_lokasi='"+this.app._lokasi+"' "+
					" group by b.nama_rek,b.no_rek,b.bank,b.cabang,a.kode_vendor,b.nama,b.bank_trans order by b.bank_trans,a.kode_vendor+' - '+b.nama",true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			var line;
			this.sg3.clear();
			for (var i in data.rs.rows){
