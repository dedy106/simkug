window.app_saku2_transaksi_yks_hutpiu_fKBHutangAreaE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_yks_hutpiu_fKBHutangAreaE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_yks_hutpiu_fKBHutangAreaE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Pembayaran Hutang Mitra Area : Edit", 0);	
...
window.app_saku2_transaksi_yks_hutpiu_fKBHutangAreaE.extend(window.childForm);
window.app_saku2_transaksi_yks_hutpiu_fKBHutangAreaE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("update yk_hutang_d set no_rekon='-',kode_lokrek='-',no_kas ='-',kode_lokkas='-' where no_kas='"+this.e_nb.getText()+"' and kode_lokkas='"+this.app._lokasi+"'");
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!="") {
			this.e_nb.setSQL("select a.no_kas, a.keterangan from kas_m a  "+
			                 "where a.no_del='-' and a.modul = 'KBHUTAREA' and a.posted='F' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",["no_kas","keterangan"],false,["No KB","Deskripsi"],"and","Daftar Bukti KB",true);
		}
	...
	doDoubleClick: function(sender, col , row) {
		if (this.sg2.cells(2,row) != "" && this.sg2.cells(3,row) != "") {
			var data = this.dbLib.getDataProvider("select a.no_inv,a.nama_rek,a.no_rek,a.bank,a.cabang,a.nilai_bp,a.nilai_cc,(a.nilai_bp+a.nilai_cc) as total, a.kode_vendor,b.nama as vendor,a.kode_lokasi+a.no_hutang as no_hutang "+
												  ...
												  "where a.no_kas='"+this.e_nb.getText()+"' and a.bank_trans='"+this.c_bank.getText()+"' and a.no_hutang='"+this.sg2.cells(2,row)+"' and a.no_app='"+this.sg2.cells(3,row)+"' and a.kode_lokasi='"+this.sg2.cells(1,row)+"' order by a.kode_vendor+' - '+b.nama",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg3.clear();
				for (var i in data.rs.rows){
