window.app_saku2_transaksi_yks_hutpiu_fRekon = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_yks_hutpiu_fRekon.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_yks_hutpiu_fRekon";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Rekon Pembayaran Hutang: Input", 0);	
...
window.app_saku2_transaksi_yks_hutpiu_fRekon.extend(window.childForm);
window.app_saku2_transaksi_yks_hutpiu_fRekon.implement({
	...
	doTampilClick:function(sender){		
		if (this.e_periode.getText() != "") {
			var data = this.dbLib.getDataProvider(
			           ...
			           "where a.kode_lokkas<>'"+this.app._lokasi+"' and a.no_app<>'X' and c.periode<='"+this.e_periode.getText()+"' and a.kode_lokhut='"+this.app._lokasi+"' and a.no_rekon='-'",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
	...
	doJurnalClick:function(sender){		
		...
		var strSQL = "select c.flag as kode_akun,d.nama as nama_akun,'C' as dc,'MUTASI KEWAJIBAN' as ket,sum(a.nilai_bp+a.nilai_cc) as nilai,'TAK' as jenis "+
					 ...
					 "where a.no_rekon='-' and a.bank_trans+a.kode_lokasi+a.no_hutang in ("+nobukti+") and a.kode_lokasi+a.no_app in ("+noapp+") and a.kode_lokkas+a.no_kas in ("+nokas+") "+
					...
					"   end "+
					"end) "+
					"where  a.nilai >= 0 and a.pph >=0 and y.bank_trans+a.kode_lokasi+a.no_hutang in ("+nobukti+") and a.kode_lokasi+a.no_app in ("+noapp+") "+
					"group by b.nama,"+
					"case f.jenis when 'PENSIUN' then "+
					"   case when substring(a.kode_produk,1,1) = '1' then bb.cc_rjtp "+
					"		 when substring(a.kode_produk,1,1) = '2' then bb.cc_rjtl "+
