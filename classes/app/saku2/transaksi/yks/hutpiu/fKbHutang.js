window.app_saku2_transaksi_yks_hutpiu_fKbHutang = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_yks_hutpiu_fKbHutang.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_yks_hutpiu_fKbHutang";
		...
		this.sgn2 = new portalui_sgNavigator(this.pc1.childPage[0],{bound:[0,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:bsAll,grid:this.sg2});
...
window.app_saku2_transaksi_yks_hutpiu_fKbHutang.extend(window.childForm);
window.app_saku2_transaksi_yks_hutpiu_fKbHutang.implement({
	...
	doLoadData:function(sender){
		if (this.c_bank.getText()!="") {
			var strSQL = "select a.no_hutang,a.kode_lokasi,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,isnull(b.bp,0) as bp,isnull(b.cc,0) as cc,isnull(b.nilai,0) as nilai,isnull(b.no_app,'-') as no_app "+
						 ...
						 "where isnull(b.nilai,0) <> 0 and  a.periode<='"+this.e_periode.getText()+"' order by a.kode_lokasi,a.no_hutang";
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg2.appendData(["INPROG",line.kode_lokasi,line.no_hutang,line.no_app,line.tanggal,line.keterangan,floatToNilai(line.bp),floatToNilai(line.cc),floatToNilai(line.nilai)]);
				}
	...
	doDoubleClick: function(sender, col , row) {
		if (this.sg2.cells(2,row) != "" && this.sg2.cells(3,row) != "") {
			var data = this.dbLib.getDataProvider("select a.no_inv,a.nama_rek,a.no_rek,a.bank,a.cabang,a.nilai_bp,a.nilai_cc,(a.nilai_bp+a.nilai_cc) as total, a.kode_vendor,b.nama as vendor,a.kode_lokasi+a.no_hutang as no_hutang "+
												  ...
												  "where a.no_kas='-' and b.bank_trans='"+this.c_bank.getText()+"' and a.no_hutang='"+this.sg2.cells(2,row)+"' and a.no_app='"+this.sg2.cells(3,row)+"' and a.kode_lokasi='"+this.sg2.cells(1,row)+"' order by a.kode_vendor+' - '+b.nama",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg3.clear();
				for (var i in data.rs.rows){
