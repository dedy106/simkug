window.app_saku2_transaksi_yks_hutpiu_fKbHutangE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_yks_hutpiu_fKbHutangE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_yks_hutpiu_fKbHutangE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Pembayaran Hutang Mitra : Edit", 0);			
		...
		this.e_debet = new saiLabelEdit(this,{bound:[790,17,220,20],caption:"Total Debet", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		this.c_bank = new saiLabelEdit(this,{bound:[20,22,182,20],caption:"Jenis Bank", readOnly:true,tag:2});
		this.e_kredit = new saiLabelEdit(this,{bound:[790,22,220,20],caption:"Total Kredit", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		
		this.bJurnal = new button(this,{bound:[690,22,80,18],caption:"Jurnal",click:[this,"doJurnal"]});
				
		this.pc1 = new pageControl(this,{bound:[20,12,990,288], childPage:["Daftar Hutang","Data Detail Transfer","Data Billing","Data Item Jurnal"]});
		this.sg2 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:0,
				...
				colFormat:[[6,7,8],[cfNilai,cfNilai,cfNilai]],checkItem:true,
				change:[this,"doChangeCell2"],dblClick:[this,"doDoubleClick"],defaultRow:1,autoAppend:false});
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select flag from spro where kode_lokasi='"+this.app._lokasi+"' and kode_spro = 'TAKHUT'",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line = data.rs.rows[0];							
				this.akuntak = line.flag;
			} else this.akuntak = "-";			
			this.c_bank.setText("");						
		}catch(e){
			systemAPI.alert(e);
...
window.app_saku2_transaksi_yks_hutpiu_fKbHutangE.extend(window.childForm);
window.app_saku2_transaksi_yks_hutpiu_fKbHutangE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from yk_kashutang_d where no_kas = '"+this.e_nb.getText()+"' and kode_lokkas='"+this.app._lokasi+"'");
					sql.add("update yk_hutang_d set no_kas ='-',kode_lokkas='-' where no_kas='"+this.e_nb.getText()+"' and kode_lokkas='"+this.app._lokasi+"'");
										
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.e_gb.getText()+"','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBHUT','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"',getdate(),'"+this.app._userLog+"','F','-','-','"+this.c_bank.getText()+"','-')");					
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
										"	('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','"+this.sg.cells(9,i)+"','-','"+this.app._lokasi+"','KBHUT','HUT','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");
							}
						}
					}					
					var nobukti = noapp = "";
					for (var i=0;i < this.sg2.getRowCount();i++){
						if (this.sg2.rowValid(i) && this.sg2.cells(0,i) == "APP") {
							...
							sql.add("insert into yk_kashutang_d(no_kas,kode_lokkas,bank_trans,bp,cc,total,no_hutang,no_app,kode_lokhut,no_rekon,kode_lokrek,akun_tak,keterangan,periode) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.c_bank.getText()+"',"+parseNilai(this.sg2.cells(6,i))+","+parseNilai(this.sg2.cells(7,i))+","+parseNilai(this.sg2.cells(8,i))+",'"+this.sg2.cells(2,i)+"','"+this.sg2.cells(3,i)+"','"+this.sg2.cells(1,i)+"','-','-','"+this.akuntak+"','"+this.sg2.cells(5,i)+"','"+this.e_periode.getText()+"')");
						}
					}
					nobukti = nobukti.substr(1);
					noapp = noapp.substr(1);
					sql.add("update yk_hutang_d set no_kas ='"+this.e_nb.getText()+"',kode_lokkas='"+this.app._lokasi+"' where bank_trans='"+this.c_bank.getText()+"' and kode_lokasi+no_hutang in ("+nobukti+") and kode_lokasi+no_app in ("+noapp+") and no_kas='-' ");										  										
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!="") {
			this.e_nb.setSQL("select a.no_kas, a.keterangan from kas_m a left join (select distinct no_kas,kode_lokkas from yk_kashutang_d where no_rekon<>'-') b on a.no_kas=b.no_kas and a.kode_lokasi=b.kode_lokkas "+
			                 "where a.no_del='-' and b.no_kas is null and a.modul = 'KBHUT' and a.posted='F' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",["no_kas","keterangan"],false,["No KB","Deskripsi"],"and","Daftar Bukti KB",true);
		}
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			var data = this.dbLib.getDataProvider("select a.akun_kb,d.nama as nama_kb,a.no_dokumen,convert(varchar,a.tanggal,103) as tanggal,a.periode,a.no_bg,a.keterangan,a.jenis,a.nik_buat,b.nama as nama_buat,a.nik_app,c.nama as nama_app,a.ref1 "+
			           "from kas_m a "+
			...
			var data = this.dbLib.getDataProvider(
						...
						"           (select no_app,no_hutang,kode_lokasi,sum(nilai_bp) as bp,sum(nilai_cc) as cc,sum(nilai_bp+nilai_cc) as nilai "+
						"            from yk_hutang_d "+
						"            where no_app<>'-' and no_kas ='"+this.e_nb.getText()+"' and kode_lokkas='"+this.app._lokasi+"' group by no_app,no_hutang,kode_lokasi) b "+
	...
	doJurnal:function(sender){
		if (this.cb_akun.getText() != "") {
			...
			nobukti = nobukti.substr(1);
			noapp = noapp.substr(1);
			var strSQL = "select c.flag as kode_akun,d.nama as nama_akun,'D' as dc,'MUTASI KEWAJIBAN' as ket,sum(a.nilai_bp+a.nilai_cc) as total,'"+this.app._kodePP+"' as kode_pp,e.nama as nama_pp,'-' as kode_drk,'-' as nama_drk,'-' as kode_cf,'-' as nama_cf "+
						 "from yk_hutang_d a "+						 
						 "	   inner join spro c on c.kode_lokasi='"+this.app._lokasi+"' and c.kode_spro = 'TAKHUT' "+
						 "	   inner join masakun d on d.kode_lokasi='"+this.app._lokasi+"' and c.flag = d.kode_akun "+
						 "	   inner join pp e on e.kode_lokasi='"+this.app._lokasi+"' and e.kode_pp = '"+this.app._kodePP+"' "+
						 "where a.no_kas = '"+this.e_nb.getText()+"' and a.bank_trans='"+this.c_bank.getText()+"' and a.kode_lokasi+a.no_hutang in ("+nobukti+") and a.kode_lokasi+a.no_app in ("+noapp+") "+
						 "group by c.flag,d.nama,e.nama "+
						 "union "+
	...
	doDoubleClick: function(sender, col , row) {
		if (this.sg2.cells(2,row) != "" && this.sg2.cells(3,row) != "") {
			var data = this.dbLib.getDataProvider("select a.no_inv,a.nama_rek,a.no_rek,a.bank,a.cabang,a.nilai_bp,a.nilai_cc,(a.nilai_bp+a.nilai_cc) as total, a.kode_vendor,b.nama as vendor,a.kode_lokasi+a.no_hutang as no_hutang "+
												  ...
												  "where a.no_kas='"+this.e_nb.getText()+"' and b.bank_trans='"+this.c_bank.getText()+"' and a.no_hutang='"+this.sg2.cells(2,row)+"' and a.no_app='"+this.sg2.cells(3,row)+"' and a.kode_lokasi='"+this.sg2.cells(1,row)+"' order by a.kode_vendor+' - '+b.nama",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg3.clear();
				for (var i in data.rs.rows){
