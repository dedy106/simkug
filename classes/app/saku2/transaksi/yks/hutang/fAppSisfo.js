window.app_saku2_transaksi_yks_hutang_fAppSisfo = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_yks_hutang_fAppSisfo.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_yks_hutang_fAppSisfo";
		...
		this.e_total = new saiLabelEdit(this,{bound:[710,11,200,20],caption:"Total", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
...
window.app_saku2_transaksi_yks_hutang_fAppSisfo.extend(window.childForm);
window.app_saku2_transaksi_yks_hutang_fAppSisfo.implement({
	...
	doClick:function(sender){
		...
		if (sender == this.i_load) {
			var data = this.dbLib.getDataProvider(
						...
						"where  x.tanggal between '"+this.dp_d2.getDateString()+"' and '"+this.dp_d3.getDateString()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_app='-' order by a.kode_vendor,a.no_inv",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
