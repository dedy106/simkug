window.app_saku2_transaksi_yks_hutang_fLoadOpta = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_yks_hutang_fLoadOpta.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku2_transaksi_yks_hutang_fLoadOpta";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Load Data Jurnal Akru Sisfo Yankesta", 0);	
		...
		this.p1 = new portalui_panel(this,{bound:[20,189,900,304],caption:"Data Jurnal"});
...
window.app_saku2_transaksi_yks_hutang_fLoadOpta.extend(window.portalui_childForm);
window.app_saku2_transaksi_yks_hutang_fLoadOpta.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("update yk_bill_temp set no_hutang='"+this.e_nb.getText()+"',kode_lokasi='"+this.app._lokasi+"',kode_vendor='XXX' "+
					        "where ('0'+kdarea)='"+this.app._lokasi+"' and tgl_periksa between '"+this.dp_d2.getDateString()+" 00:00:00' and '"+this.dp_d3.getDateString()+" 23:59:59' and no_hutang='-'");
					
					sql.add("insert into yk_hutang_d "+
							"(no_hutang,kode_lokasi,no_inv,periode,kode_vendor,bank,cabang,no_rek,nama_rek,nilai_bp,nilai_cc,bank_trans,no_kas,kode_lokkas,no_rekon,kode_lokrek,no_app) "+
							"select "+
							"'"+this.e_nb.getText()+"',a.kode_lokasi,a.kd_mitra,'"+this.e_periode.getText()+"',a.kd_mitra,'bank','cabang','no_rek','nama_rek', "+
							"round(sum(case when substring(loker,1,3)<>'PEN' then a.biaya else 0 end),0) as nilai_bp, "+
							"round(sum(case when substring(loker,1,3)='PEN' then a.biaya else 0 end),0) as nilai_cc, "+
							"'MANDIRI','-','-','-','-','-' "+
							"from yk_bill_temp a "+
							"   	inner join yk_peserta_d c on a.nik=c.nik "+
							"where a.no_hutang='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.biaya <> 0 "+
							"group by a.kode_lokasi,a.kd_mitra");
					
					sql.add("insert into yk_valid_d "+
							"(no_valid,kode_lokasi,no_bill,nilai,modul,keterangan,periode,dc,progress,no_kirim,no_reklas,kode_cust,kode_produk,kode_loktuj) "+
							"select "+
							"'"+this.e_nb.getText()+"',a.kode_lokasi,'SISFO',round(sum(biaya),0),'SISFO','SISFO', "+
							"'"+this.e_periode.getText()+"','D','0','-','-',c.loker,a.kode_produk,d.kode_lokasi "+
							"from yk_bill_temp a "+
							"     inner join yk_peserta_d c on a.nik=c.nik "+
							"     inner join cust d on c.loker=d.kode_cust "+
							"where  a.no_hutang='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.biaya <> 0 "+
							"group by a.kode_lokasi,c.loker,a.kode_produk,d.kode_lokasi");
	...
	doClick: function(sender){
		...
		if (sender == this.i_load) {
			this.sg.clear();			
			var data = this.dbLib.getDataProvider("select kd_mitra,no_resep,nik,nm_pegawai,band_posisi,convert(varchar,tgl_periksa,103) as tanggal,nikes,nm_peserta,icdx,round(biaya,0) as biaya "+
			                                      "from yk_bill_temp where ('0'+kdarea)='"+this.app._lokasi+"' and tgl_periksa between '"+this.dp_d2.getDateString()+" 00:00:00' and '"+this.dp_d3.getDateString()+" 23:59:59' and no_hutang='-' ",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU = data;
				var total = 0;
