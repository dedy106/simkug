window.app_saku2_transaksi_yks_fKbDakem = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_yks_fKbDakem.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_yks_fKbDakem";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Dana Kematian: Input", 0);	
		...
		this.bTampil = new button(this,{bound:[680,17,80,18],caption:"Tampil Data",click:[this,"doLoad"]});					
...
window.app_saku2_transaksi_yks_fKbDakem.extend(window.childForm);
window.app_saku2_transaksi_yks_fKbDakem.implement({
	...
	doLoad:function(sender){		
		if (this.e_periode.getText() != "") {				  
			...
			var data = this.dbLib.getDataProvider("select a.kdtrans,'-' as kdloker,a.nik,a.nikkes,a.namaaw,a.statusaw,a.nominal,convert(varchar,a.tglmeninggal,103) as tanggal,a.namabank,a.norek,a.cabang,isnull(b.no_gambar,'-') as no_gambar "+
						...
						"where a.no_kas='-' and a.kode_lokasi = '"+this.app._lokasi+"' and periode<='"+this.e_periode.getText()+"'",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData(["INPROG",line.kdtrans,line.kdloker,line.nik,line.nikkes,line.namaaw,line.statusaw,floatToNilai(line.nominal),line.tanggal,line.namabank,line.norek,line.cabang,line.no_gambar]);
				}
