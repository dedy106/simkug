window.app_saku2_transaksi_anggaran_fEvalAkun = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_anggaran_fEvalAkun.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_anggaran_fEvalAkun";
...
window.app_saku2_transaksi_anggaran_fEvalAkun.extend(window.childForm);
window.app_saku2_transaksi_anggaran_fEvalAkun.implement({	
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into agg_eval_d(no_eval,kode_lokasi,kode_akun,persen) values "+
					...
					sql.add("update agg_d set vol=vol+(vol * "+nilaiToFloat(this.e_persen.getText())+"/100), "+
							...
							"where kode_lokasi='"+this.app._lokasi+"' and periode like '"+this.c_tahun.getText()+"%' and kode_akun='"+this.cb_akun.getText()+"' ");						 
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_akun && this.cb_akun.getText()!="") {
			var strSQL = "select a.kode_drk,a.kode_norma,a.kode_akun,a.keterangan,a.satuan,substring(a.periode,5,2) as bulan,a.tarif,a.jumlah,a.vol,a.total, "+
						 "b.nama as nama_drk, c.nama as nama_norma,vol as vol_baru,total as total_baru "+
						 ...
						 "where a.kode_lokasi='"+this.app._lokasi+"' and a.periode like '"+this.c_tahun.getText()+"%' and a.kode_akun='"+this.cb_akun.getText()+"' ";
