window.app_saku2_transaksi_anggaran_fLoadLemburB = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_anggaran_fLoadLemburB.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku2_transaksi_anggaran_fLoadLemburB";
		...
		this.cb_drk = new saiCBBL(this,{bound:[20,13,200,20],caption:"Kode DRK", readOnly:true});		
...
window.app_saku2_transaksi_anggaran_fLoadLemburB.extend(window.portalui_childForm);
window.app_saku2_transaksi_anggaran_fLoadLemburB.implement({			
			...
			case "hapus" :							
					...
					if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
						try{														
							...
							sql.add("delete from agg_lembur_m where no_lembur ='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and tahun= '"+this.e_tahun.getText()+"'");
							sql.add("delete from agg_lembur_d where no_lembur ='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and substring(periode,1,4)= '"+this.e_tahun.getText()+"'");
							sql.add("delete from agg_d where no_bukti ='"+this.e_nb.getText()+"' and modul='BLEMBUR' and kode_lokasi='"+this.app._lokasi+"' and tahun= '"+this.e_tahun.getText()+"'");
	...
	doChange: function(sender){	
		if (sender == this.e_tahun && this.e_tahun.getText()!= "") {						
			...
			this.e_nb.setSQL("select a.no_lembur, a.kode_drk from agg_lembur_m a inner join agg_drk b on a.kode_drk=b.kode_drk where a.tahun='"+this.e_tahun.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",["a.no_lembur","a.kode_drk"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
		}
		if (sender == this.e_nb && this.e_nb.getText()!= "") {			
			var data = this.dbLib.getDataProvider("select kode_drk,nilai from agg_lembur_m where no_lembur='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object"){
