window.app_saku2_transaksi_anggaran_fLoadOutLook = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_anggaran_fLoadOutLook.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku2_transaksi_anggaran_fLoadOutLook";
...
window.app_saku2_transaksi_anggaran_fLoadOutLook.extend(window.portalui_childForm);
window.app_saku2_transaksi_anggaran_fLoadOutLook.implement({	
						...
						try{
							...
							for (var i=0; i < this.dataUpload.rows.length;i++){
								...
								sql.add("insert into agg_outlook(kode_lokasi,kode_bidang,kode_akun,kode_pp,rka,realisasi,outlook,tahun,dc,gar_sem1,gar_sem2,rseb,rnow,estimasi,periode,persen) values "+
										"('"+this.app._lokasi+"','"+this.cb_bidang.getText()+"','"+line.kode_akun+"','"+line.kode_pp+"',"+rka+","+realisasi+","+outlook+",'"+this.e_tahun.getText()+"','"+dc+"',"+rka+",0,"+realisasi+",0,"+estimasi+",'"+this.e_tahun.getText()+this.c_bulan.getText()+"',10)");
							}															
