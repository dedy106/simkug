window.app_saku2_transaksi_anggaran_fLoadTpku = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_anggaran_fLoadTpku.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku2_transaksi_anggaran_fLoadTpku";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Biaya Dokter TPKU (Cek Outlook): Load", 0);	
		...
		this.cb_akun = new saiCBBL(this,{bound:[20,10,200,20],caption:"Akun TPKU", multiSelection:false, maxLength:10, tag:2});			
		...
		this.pc1 = new pageControl(this,{bound:[20,20,900,410], childPage:["Data Honor Dokter TPKU (per Bulan)","Pesan Kesalahan","Eval Outlook"]});				
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:9,tag:0,				
				colTitle:["No Kontrak","Dokter","Alamat","Nilai","Pensiun","Pegawai","Jenis","Status","Kode PP"],
				colWidth:[[8,7,6,5,4,3,2,1,0],[60,60,60,60,60,80,200,150,150]],
				colFormat:[[3,4,5],[cfNilai,cfNilai,cfNilai]],readOnly:true, defaultRow:1});				
		...
		this.cb_akun.setSQL("select kode_akun, nama from masakun where kode_akun in ('4212010002') and kode_lokasi='"+this.app._lokasi+"'",["kode_akun","nama"],false,["Kode","Nama"],"and","Daftar Akun",true);
		this.cb_akun.setText("4212010002");		
...
window.app_saku2_transaksi_anggaran_fLoadTpku.extend(window.portalui_childForm);
window.app_saku2_transaksi_anggaran_fLoadTpku.implement({	
	...
	selectPage: function(sender,page){
		...
		for (var i=start; i < finish;i++){
			...
			this.sg1.appendData([line.no_kontrak,line.dokter,line.alamat,floatToNilai(line.nilai),floatToNilai(line.pensiun),floatToNilai(line.pegawai),line.jenis,line.status,line.kode_pp]);
		}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "simpan" :	
					this.stsSimpan = "1";
					if (this.prog != "0") {
						system.alert(this,"Transaksi tidak valid.","Transaksi Honor TPKU telah di Close.");
						return false;
					}
					...
					if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
						try{	
							this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'agg_tpku_m','no_tpku',this.app._lokasi+"-TPKU"+this.e_tahun.getText()+".",'000'));
							uses("server_util_arrayList");
							var sql = new server_util_arrayList();															
							sql.add("insert into agg_tpku_m(no_tpku,tahun,kode_akun,kode_drk,nilai,kode_lokasi,tgl_input,nik_user) values "+
									"('"+this.e_nb.getText()+"','"+this.e_tahun.getText()+"','"+this.cb_akun.getText()+"','"+this.cb_drk.getText()+"',"+parseNilai(this.e_total.getText())+",'"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"')");
							sql.add("insert into agg_tpku_d(no_tpku,kode_lokasi,tahun,no_kontrak,dokter,alamat,nilai,pensiun,pegawai,jenis,status,periode,kode_pp) "+							
									"select no_tpku,kode_lokasi,tahun,no_kontrak,dokter,alamat,nilai,pensiun,pegawai,jenis,status,periode,kode_pp from agg_tpku_tmp "+
							...
							sql.add("insert into agg_d (kode_lokasi,kode_pk,kode_drk,kode_rka,kode_akun,kode_pp,periode,bulan,jumlah,volume,nilai,tahun,no_bukti,modul,progress,jenis_agg,keterangan) "+
									"select a.kode_lokasi,'-',b.kode_drk,a.no_kontrak,b.kode_akun,a.kode_pp,a.periode,substring(a.periode,5,2),1,1,a.nilai,a.tahun,a.no_tpku,'BTPKU','0',a.status,'"+this.cb_drk.rightLabelCaption+"' "+
									"from agg_tpku_d a inner join agg_tpku_m b on a.no_tpku=b.no_tpku and a.kode_lokasi=b.kode_lokasi "+
									"where b.no_tpku ='"+this.e_nb.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"'");
							sql.add("delete from agg_tpku_tmp where nik_user = '"+this.nik_user+"'");					
	...
	doChange: function(sender){			
		if (sender == this.e_tahun && this.e_tahun.getText()!= "") {						
			var data = this.dbLib.getDataProvider("select progress from agg_close where kode_lokasi = '"+this.app._lokasi+"' and modul = 'YANKESTA' and tahun = '"+this.e_tahun.getText()+"'",true);
			...
			this.cb_drk.setSQL("select distinct kode_drk, kegiatan from agg_rkm where kode_akun='4212010002' and tahun='"+this.e_tahun.getText()+"'",["kode_drk","kegiatan"],false,["Kode","Nama"],"and","Daftar DRK",true);		
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'agg_tpku_m','no_tpku',this.app._lokasi+"-TPKU"+this.e_tahun.getText()+".",'000'));
		}		
	},
	...
	doHitung:function(sender){						
		try {	
			...
			for (var i=0; i < this.dataUpload.rows.length;i++){
				line = this.dataUpload.rows[i];				
				for (var j=1; j < 13;j++){
					sql.add("insert into agg_tpku_tmp(no_tpku,kode_lokasi,tahun,no_kontrak,dokter,alamat,nilai,pensiun,pegawai,jenis,status,periode,kode_pp,nik_user) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_tahun.getText()+"','"+line.no_kontrak+"','"+line.dokter+"','"+line.alamat+"',"+line.nilai+","+line.pensiun+","+line.pegawai+",'"+line.jenis+"','"+line.status+"','"+this.e_tahun.getText()+(j<10?"0":"")+j+"','"+line.kode_pp+"','"+this.nik_user+"')");
				}
			}									
			this.dbLib.execArraySQL(sql);			
						
			strSQL = "select sum(a.nilai) as total from agg_tpku_tmp a where a.nik_user = '"+this.nik_user+"'";
			...
			var msg  = "";
			this.e_memo.setText("");			
			strSQL = "select distinct a.kode_pp from agg_tpku_tmp a left join pp b on a.kode_pp = b.kode_pp and b.kode_lokasi='"+this.app._lokasi+"' and b.initial='TPKU' "+
					 "where b.nama is null and a.nik_user = '"+this.nik_user+"'";
			var data = this.dbLib.getDataProvider(strSQL,true);	
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];																													
					temu = true;
					msg+= "Kode PP tidak terdaftar sebagai PP TPKU. [kode : "+line.kode_pp+"]\n";
				}
			}
			strSQL = "select distinct jenis from agg_tpku_tmp where jenis not in ('FFS','KAPITASI') and nik_user = '"+this.nik_user+"'";
			var data = this.dbLib.getDataProvider(strSQL,true);	
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;				
				for (var i in data.rs.rows){
					line = data.rs.rows[i];																													
					temu = true;
					msg+= "Jenis tidak terdaftar [FFS,KAPITASI]. [kode : "+line.jenis+"]\n";
				}
			}
			if (!temu) {														
				...
				strSQL = "select distinct a.kode_lokasi,'"+this.cb_akun.rightLabelCaption+"' as nama_akun, '"+this.cb_akun.getText()+"' as kode_akun,substring(a.kode_pp,3,1) as kode_pp,b.usul,isnull(c.usul_total,0) as usul_total,isnull(d.nilai_max,0) as nilai_max,case when isnull(d.outlook,0) = 0 then 0 else round(isnull(d.nilai_max,0)/isnull(d.outlook,0),2)*100 end as growth,isnull(d.outlook,0) as outlook,b.usul+isnull(c.usul_total,0)-isnull(d.nilai_max,0) as kelebihan "+
						"from agg_tpku_tmp a "+						
						"inner join "+
						"( "+
						"select kode_lokasi,substring(kode_pp,3,1) as kode_pp,sum(nilai) as usul "+
						"from agg_tpku_tmp  "+
						"where nik_user='"+this.nik_user+"' "+
