window.app_saku2_transaksi_anggaran_fEvalMulti = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_anggaran_fEvalMulti.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_anggaran_fEvalMulti";
		...
		this.p1 = new panel(this,{bound:[20,23,700,354],caption:"Daftar Akun Evaluasi"});							
		this.sg = new saiGrid(this.p1,{bound:[0,20,this.p1.width-5,this.p1.height-45],colCount:6,tag:0,
		            colTitle:["Kode Akun","Nama Akun","Persentase(+/-)","Kode Lokasi","Kode CF","Nama CF"],
					colWidth:[[5,4,3,2,1,0],[150,80,80,100,150,80]],					
					columnReadOnly:[true,[1,3,4],[0,2]],
					buttonStyle:[[0,3,4],[bsEllips,bsAuto,bsEllips]], colFormat:[[2],[cfNilai]],
					picklist:[[3],[new portalui_arrayMap({items:["01","02","03","04","05","06","07","99"]})]],
		...
		if (typeof data == "object" && data.rs.rows[0] != undefined) this.dataAkun = data;
		try {
...
window.app_saku2_transaksi_anggaran_fEvalMulti.extend(window.childForm);
window.app_saku2_transaksi_anggaran_fEvalMulti.implement({	
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into agg_eval_d(no_eval,kode_lokasi,kode_akun,persen,kode_lokasi_edit,kode_cf) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.sg.cells(0,i)+"',"+nilaiToFloat(this.sg.cells(2,i))+",'"+this.sg.cells(3,i)+"','"+this.sg.cells(4,i)+"')");
								sql.add("update agg_d set vol=vol+(vol * "+nilaiToFloat(this.sg.cells(2,i))+"/100), "+
										...
										"where kode_lokasi='"+this.sg.cells(3,i)+"' and periode like '"+this.c_tahun.getText()+"%' and kode_akun='"+this.sg.cells(0,i)+"' ");						 					
							
								/* tidak bisa multi lokasi
								if (this.sg.cells(4,i) != "-") {
									sql.add("update agg_gldt set nilai=round(nilai+(nilai * "+nilaiToFloat(this.sg.cells(2,i))+"/100),0)  "+
											"where modul='POST' and periode like '"+this.c_tahun.getText()+"%' and kode_akun='"+this.sg.cells(0,i)+"' ");
									sql.add("update agg_gldt set nilai=round(nilai+(nilai * "+nilaiToFloat(this.sg.cells(2,i))+"/100),0)  "+
											"where modul='POST' and kode_lokasi='00' and periode like '"+this.c_tahun.getText()+"%' and kode_cf='"+this.sg.cells(4,i)+"' ");
								}
								*/
							}
						}
			...
			if (sender == this.sg) {
				...
				if (col == 4){
					this.standarLib.showListData(this, "Daftar CF",sender,undefined, 
						    "select kode_cf,nama from agg_neracacf",
							"select count(kode_cf) from agg_neracacf",
							["kode_cf","nama"],"where",["Kode","Nama"],false);
				}				
			}
		}catch(e){
			systemAPI.alert(e);
