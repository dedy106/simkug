window.app_saku2_transaksi_anggaran_fFSGar = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_anggaran_fFSGar.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_anggaran_fFSGar";
		this.setTop(60);
		this.setWidth(1280);
		this.setHeight(550);
...
//------------------------------------------------------------------------
		try{
			...
			this.e0.setSQL("select kode_fs, nama from agg_fsgar where kode_lokasi = '"+this.app._lokasi+"' ",["kode_fs","nama"],undefined,["Kode FS","Nama"],"and","Data Versi Laporan",false);
		}catch(e)
		{
			alert(e);
		}
...
window.app_saku2_transaksi_anggaran_fFSGar.extend(window.portalui_childForm);
//------------------------------------------------------------------ event
window.app_saku2_transaksi_anggaran_fFSGar.implement({
		...
		try{
			...
			switch (event)
			{
				...
				case "simpan" :
						...
						sql.add("insert into agg_fsgar (kode_fs, kode_lokasi,nama,tgl_awal, tgl_akhir, flag_status, tgl_input,nik_user ) values ('"+this.e0.getText()+"','"+this.app._lokasi+"','"+this.e1.getText()+"','"+this.d1.getDate()+"','"+this.d2.getDate()+"','"+status+"','"+(new Date).getDateStr()+"','"+this.app._userLog+"') ");
						this.dbLib.execArraySQL(sql);					
					break;
				case "ubah" :					
						uses("server_util_arrayList");
						sql = new server_util_arrayList();
						sql.add("update agg_fsgar set nama = '"+this.e1.getText()+"',flag_status = '"+status+"',tgl_awal='"+this.d1.getDate()+"',tgl_akhir='"+this.d2.getDate()+"',tgl_input='"+(new Date).getDateStr()+"',nik_user='"+this.app._userLog+"' where kode_fs = '"+this.e0.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
						this.dbLib.execArraySQL(sql);							
					break;
				case "hapus" :
				    uses("server_util_arrayList");
						...
						sql.add("delete from  agg_fsgar where kode_fs = '"+this.e0.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
						this.dbLib.execArraySQL(sql);											   
					break;
			}
			this.e0.setFocus();
	...
	doEditChange: function(sender){
		...
		if(sender == this.e0){
			if (this.e0.getText() != "")
			{
				try
				{			
					var data = this.dbLib.runSQL("select * from agg_fsgar where kode_fs = '"+this.e0.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'  ");
