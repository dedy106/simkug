window.app_saku2_transaksi_aka_fMhsLoadxml = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_aka_fMhsLoadxml.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku2_transaksi_aka_fMhsLoadxml";
		...
		this.bGet = new button(this,{bound:[500,14,80,20],caption:"Get Data", click:[this,"doGetFile"]});
		this.bUpload = new portalui_uploader(this,{bound:[620,14,100,20],param3:"json",param4:"gridupload",afterUpload:[this,"doAfterUpload"],autoSubmit:true});		
		
		this.p1 = new portalui_panel(this,{bound:[20,189,700,304],caption:"Data Mahasiswa"});
		this.sg1 = new portalui_saiGrid(this.p1,{bound:[0,20,this.p1.width-5,this.p1.height-45],colCount:7,tag:9,
				colTitle:["NIM","Nama","Jalur","Beasiswa","Status","Prodi","Angkatan"],
				colWidth:[[6,5,4,3,2,1,0],[70,70,70,70,70,220,70]],
		...
		var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('XMLMHS') and kode_lokasi = '"+this.app._lokasi+"'",true);			
		if (typeof data == "object"){
			var line;
			for (var i in data.rs.rows){
				line = data.rs.rows[i];																	
				if (line.kode_spro == "XMLMHS") this.alamatXml = line.flag;							
			}
		}
...
window.app_saku2_transaksi_aka_fMhsLoadxml.extend(window.portalui_childForm);
window.app_saku2_transaksi_aka_fMhsLoadxml.implement({
	...
	selectPage: function(sender,page){
		...
		for (var i=start; i < finish;i++){
			...
			this.sg1.appendData([line.nim,line.nama,line.jalur,line.beasiswa,line.status,line.prodi,line.angkatan]);
		}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "simpan" :					
					...
					if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
						try{
							...
							if (this.c_modul.getText() == "INPUT") {
								sql.add("insert into aka_mahasiswa(nim,nama,kode_lokasi,kode_jur,kode_akt,kode_jalur,flag_status,flag_bea) values "+
										"	('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.cb_jur.getText()+"','"+this.cb_akt.getText()+"','"+this.cb_jalur.getText()+"','"+this.c_status.getText()+"','"+this.c_bea.getText()+"')");
							}
							else {							
								var line;
								for (var i=0; i < this.dataUpload.rows.length;i++){
									line = this.dataUpload.rows[i];
									sql.add("insert into aka_mahasiswa(nim,nama,kode_lokasi,kode_jur,kode_akt,kode_jalur,flag_status,flag_bea) values "+
											"('"+line.nim+"','"+line.nama+"','"+this.app._lokasi+"','"+line.prodi+"','"+line.angkatan+"','"+line.jalur+"','"+line.status+"','"+line.beasiswa+"')");
								}
							}
			...
			case "ubah" :		
					...
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();	
					sql.add("update aka_mahasiswa set nama='"+this.e_nama.getText()+"',kode_jur='"+this.cb_jur.getText()+"',kode_akt='"+this.cb_akt.getText()+"',kode_jalur='"+this.cb_jalur.getText()+"',flag_status='"+this.c_status.getText()+"',flag_bea='"+this.c_bea.getText()+"' where nim = '"+this.cb_kode.getText()+"'  and kode_lokasi = '"+this.app._lokasi+"' ");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				break;
			case "hapus" :	
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var data = this.dbLib.getDataProvider("select a.nama,a.kode_jur,a.kode_akt,a.kode_jalur,a.flag_status,a.flag_bea,b.nama as nama_jur,c.nama as nama_akt,d.nama as nama_jalur "+
