window.app_saku2_transaksi_aka_fTagihBatalInv = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_aka_fTagihBatalInv.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_aka_fTagihBatalInv";
		...
		this.cb_nim = new saiCBBL(this,{bound:[20,19,200,20],caption:"Mahasiswa", multiSelection:false, maxLength:10, tag:1, change:[this,"doChange"]});
...
window.app_saku2_transaksi_aka_fTagihBatalInv.extend(window.childForm);
window.app_saku2_transaksi_aka_fTagihBatalInv.implement({
	...
	doChange: function(sender){
		...
		if (sender == this.cb_nim && this.cb_nim.getText()!="") {
			...
			var strSQL = "select distinct a.no_inv,a.periode "+
						 "from aka_bill_d a "+
						 ...
						 "where a.nim = '"+this.cb_nim.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' and a.nilai-isnull(x.tot_batal,0) > 0 ";						 
			this.cb_tagihan.setSQL(strSQL,["no_inv","periode"],false,["Invoice","Periode"],"and","Data Invoice",true);
		}
		
		if (sender == this.cb_tagihan && this.cb_tagihan.getText()!="") {			
