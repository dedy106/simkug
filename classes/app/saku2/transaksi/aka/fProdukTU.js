window.app_saku2_transaksi_aka_fProdukTU = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_aka_fProdukTU.prototype.parent.constructor.call(this,owner);
		this.className  = "s";
		...
		this.cb_fak = new saiCBBL(this,{bound:[20,15,200,20],caption:"Fakultas", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});		
		this.p1 = new panel(this,{bound:[10,23,900,430],caption:"Daftar Produk Piutang"});
...
window.app_saku2_transaksi_aka_fProdukTU.extend(window.childForm);
window.app_saku2_transaksi_aka_fProdukTU.implement({
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into aka_produk(kode_produk,nama,kode_lokasi,akun_piutang,akun_pdpt,akun_pdd,akun_bp,akun_ap,no_urut,kode_fakultas) values "+
										"	('"+this.sg.cells(0,i)+"','"+this.sg.cells(1,i)+"','"+this.app._lokasi+"','"+this.sg.cells(2,i)+"','"+this.sg.cells(4,i)+"','"+this.sg.cells(6,i)+"','"+this.sg.cells(8,i)+"','"+this.sg.cells(10,i)+"',"+parseNilai(this.sg.cells(12,i))+",'"+this.cb_fak.getText()+"')");
							}
						}
					}					
					setTipeButton(tbAllFalse);
	...
	doChange: function(sender){	
		if (sender == this.cb_fak && this.cb_fak.getText()!="") {
			var data = this.dbLib.getDataProvider(
						...
						"where a.kode_fakultas='"+this.cb_fak.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by kode_produk",true);
