window.app_saku2_transaksi_aka_fAmorBatal = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_aka_fAmorBatal.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_aka_fAmorBatal";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pembatalan Amortisasi Tagihan", 0);	
		...
		this.e_pdd = new saiLabelEdit(this,{bound:[660,17,200,20],caption:"Total Amortisasi", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		...
		this.p1 = new panel(this,{bound:[20,23,840,303],caption:"Data Amortisasi"});
		this.sg = new saiGrid(this.p1,{bound:[1,20,this.p1.width-5,this.p1.height-50],colCount:7,tag:0,
		            colTitle:["No Invoice","Periode","Kode Produk","Nama Produk","Akun PDD","Total Amortisasi","Nilai Pembatalan"],
...
window.app_saku2_transaksi_aka_fAmorBatal.extend(window.childForm);
window.app_saku2_transaksi_aka_fAmorBatal.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into aka_amor_m(no_amor,no_dokumen,tanggal,keterangan,nilai,kode_pp,kode_drk,posted,modul,nik_buat,nik_app,kode_lokasi,periode,nik_user,tgl_input) values "+
						    "('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"',"+parseNilai(this.e_nilai.getText())+",'"+this.pp+"','"+this.cb_drk.getText()+"','F','AMORBTL','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
					sql.add("insert into aka_amor_j(no_amor,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
							"	('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.cb_pdpt.getText()+"','"+this.e_ket.getText()+"','D',"+parseNilai(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','AMORBTL','PDPT','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");							
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i) && this.sg.cells(6,i) != "0"){
								sql.add("insert into aka_amor_j(no_amor,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
										"	('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(4,i)+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.sg.cells(6,i))+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','AMORBTL','PDD','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");								
								sql.add("insert into aka_amor_d(no_amor,nim,no_inv,periode,nilai,kode_lokasi,akun_pdpt,akun_pdd,kode_produk,kode_pp,kode_drk,dc,no_del) values "+
										"	('"+this.e_nb.getText()+"','"+this.cb_nim.getText()+"','"+this.sg.cells(0,i)+"','"+this.e_periode.getText()+"',"+parseNilai(this.sg.cells(6,i))+",'"+this.app._lokasi+"','"+this.cb_pdpt.getText()+"','"+this.sg.cells(4,i)+"','"+this.sg.cells(2,i)+"','"+this.pp+"','"+this.cb_drk.getText()+"','C','AMORBTL')");
							}
						}
					}			
	...
	doChange: function(sender){
		...
		if (sender == this.cb_nim && this.cb_nim.getText()!="") {
			...
			var strSQL = "select a.no_inv,a.periode,a.kode_produk,c.nama,a.akun_pdd,isnull(b.tot_amor,0) as total "+
						 "from aka_bill_d a "+
						 ...
						 "where a.kode_produk = 'BPP' and isnull(b.tot_amor,0) > 0 and a.nim = '"+this.cb_nim.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' ";
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData([line.no_inv,line.periode,line.kode_produk,line.nama,line.akun_pdd,floatToNilai(line.total),"0"]);
				}
