window.app_saku2_transaksi_aka_fRekonLoadNIM = function(owner)
{
	if (owner)
	{
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('XMLRKN') and kode_lokasi = '"+this.app._lokasi+"'",true);			
			if (typeof data == "object"){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];																	
					if (line.kode_spro == "XMLRKN") this.alamatXml = line.flag;							
				}
			}
