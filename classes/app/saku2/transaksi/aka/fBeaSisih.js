window.app_saku2_transaksi_aka_fBeaSisih = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_aka_fBeaSisih.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_aka_fBeaSisih";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Beban Penyisihan Beasiswa : Proses", 0);	
		...
		this.cb_drk = new saiCBBL(this,{bound:[20,18,200,20],caption:"DRK Beasiswa", multiSelection:false, maxLength:10, tag:2 });
...
window.app_saku2_transaksi_aka_fBeaSisih.extend(window.childForm);
window.app_saku2_transaksi_aka_fBeaSisih.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into aka_beasisih_m(no_sisih,no_dokumen,tanggal,keterangan,nilai,kode_pp,kode_drk,posted,modul,nik_buat,nik_app,kode_lokasi,periode,nik_user,tgl_input) values "+
						    "('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"',"+parseNilai(this.e_nilai.getText())+",'-','"+this.cb_drk.getText()+"','F','BEASISIH','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");					
					
					var line;
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						line = this.dataJU.rs.rows[i];
						sql.add("insert into aka_beasisih_j(no_sisih,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
								"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+line.akun_bp+"','"+this.e_ket.getText()+"','D',"+line.nilai_sisih+",'"+line.kode_pp+"','"+this.cb_drk.getText()+"','"+this.app._lokasi+"','BEASISIH','BEBAN','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");
						sql.add("insert into aka_beasisih_j(no_sisih,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
								"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',1,'"+line.akun_piutang+"','"+this.e_ket.getText()+"','C',"+line.nilai_sisih+",'"+line.kode_pp+"','"+this.cb_drk.getText()+"','"+this.app._lokasi+"','BEASISIH','PIU','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");
					}			
					var perNext = nextNPeriode(this.e_periode.getText(),1);
					sql.add("call sp_ar_beasisih ('"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+perNext+"','"+this.e_nb.getText()+"','"+this.cb_drk.getText()+"')");
	...
	doLoadData: function(sender){
		...
		var strSQL = "select bb.akun_bp,x.nama as nama_bp,a.akun_piutang,y.nama as nama_piutang,e.kode_pp,e.nama as nama_pp, "+
					 ...
					 "		  from aka_beasisih_d where kode_lokasi = '"+this.app._lokasi+"' group by no_inv,kode_produk,kode_lokasi) d on  "+
					 ...
					 " 	and a.nilai-isnull(xx.tot_batal,0) > isnull(d.tot_sisih,0) and a.periode_sisih = '"+this.e_periode.getText()+"' "+
