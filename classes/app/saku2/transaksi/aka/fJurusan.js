window.app_saku2_transaksi_aka_fJurusan = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_aka_fJurusan.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_aka_fJurusan";
		...
		this.cb_drk = new saiCBBL(this,{bound:[20,21,200,20],caption:"DRK Amortisasi", multiSelection:false, maxLength:10, tag:2});
		...
		this.sg1 = new saiGrid(this.p1,{bound:[0,20,795,357],tag:9,readOnly:true,colTitle: ["Kode","Nama","Kode PP","Nama PP","DRK Tagih","Nama DRK","DRK Amor","Nama DRK","Tahun"]});		
...
window.app_saku2_transaksi_aka_fJurusan.extend(window.childForm);
window.app_saku2_transaksi_aka_fJurusan.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try{									
					...
					sql.add("insert into aka_jurusan(kode_jur,nama,kode_lokasi,kode_pp,kode_drktagih,kode_drk,tahun) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.cb_pp.getText()+"','"+this.cb_drktagih.getText()+"','"+this.cb_drk.getText()+"','"+this.c_tahun.getText()+"')");
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update aka_jurusan set kode_pp='"+this.cb_pp.getText()+"',nama = '"+this.e_nama.getText()+"',kode_drktagih='"+this.cb_drktagih.getText()+"',kode_drk='"+this.cb_drk.getText()+"',tahun='"+this.c_tahun.getText()+"' where kode_jur = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
		...
		try{
			if (sender == this.cb_kode) {
				if (this.cb_kode.getText() != ""){
					var data = this.dbLib.getDataProvider("select a.nama,a.kode_pp,b.nama as nama_pp,a.tahun,a.kode_drk,c.nama as nama_drk,a.kode_drktagih,d.nama as nama_drktagih "+
	...
	doTampilClick: function(sender){
		try{			
			var temp = this.dbLib.runSQL("select a.kode_jur,a.nama,a.kode_pp,b.nama as nama_pp,a.kode_drktagih,d.nama as nama_drktagih,a.kode_drk,c.nama as nama_drk,a.tahun "+
			           "from aka_jurusan a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi = b.kode_lokasi "+
					   "                   inner join drk c on a.kode_drk=c.kode_drk and a.kode_lokasi=c.kode_lokasi and a.tahun=c.tahun "+
