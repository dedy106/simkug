window.app_saku2_transaksi_aka_fBillLoadBeaH = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_aka_fBillLoadBeaH.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku2_transaksi_aka_fBillLoadBeaH";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Load Data Tagihan Beasiswa: Pembatalan", 0);	
		...
		this.cb_pp = new saiCBBL(this,{bound:[20,14,200,20],caption:"PP", readOnly:true});
		this.cb_drk = new saiCBBL(this,{bound:[20,15,200,20],caption:"DRK", readOnly:true});
		
		this.p1 = new portalui_panel(this,{bound:[20,189,900,354],caption:"Data Billing Mahasiswa"});
		this.sg = new portalui_saiGrid(this.p1,{bound:[0,20,this.p1.width-5,this.p1.height-45],colCount:16,
...
window.app_saku2_transaksi_aka_fBillLoadBeaH.extend(window.portalui_childForm);
window.app_saku2_transaksi_aka_fBillLoadBeaH.implement({
	...
	doChange: function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!="") {
			this.e_nb.setSQL("select no_bill, keterangan from aka_bill_m where posted ='F' and jenis = 'BEA' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_bill","keterangan"],false,["No Load","Deskripsi"],"and","Daftar Bukti Load Billing",true);			
		}
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			var data = this.dbLib.getDataProvider(
					   "select convert(varchar,tanggal,103) as tanggal,a.keterangan,a.kode_pp,a.kode_drk,b.nama as nama_pp,c.nama as nama_drk "+
					   "from aka_bill_m a "+
			...
			var data = this.dbLib.getDataProvider("select nim,nama,kode_jalur,flag_bea,flag_status,kode_jur,kode_akt,bpp,up3,sdp2,sks,s2bpp,s2up3,s2sdp2,s2sks,bpp+up3+sdp2+sks+s2bpp+s2up3+s2sdp2+s2sks as total "+
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData([line.nim,line.nama,line.kode_jalur,line.flag_bea,line.flag_status,line.kode_jur,line.kode_akt,floatToNilai(line.bpp),floatToNilai(line.up3),floatToNilai(line.sdp2),floatToNilai(line.sks),floatToNilai(line.s2bpp),floatToNilai(line.s2up3),floatToNilai(line.s2sdp2),floatToNilai(line.s2sks),floatToNilai(line.total)]);
		}
