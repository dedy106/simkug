window.app_saku2_transaksi_aka_fAmorPddH2 = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_aka_fAmorPddH2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_aka_fAmorPddH2";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Amortisasi Pendapatan Diterima Dimuka : Pembatalan", 0);	
		...
		this.cb_drk = new saiCBBL(this,{bound:[20,18,200,20],caption:"DRK Amortisasi", readOnly:true});
		...
		this.e_nilai = new saiLabelEdit(this,{bound:[660,17,200,20],caption:"Total Amortisasi", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
...
window.app_saku2_transaksi_aka_fAmorPddH2.extend(window.childForm);
window.app_saku2_transaksi_aka_fAmorPddH2.implement({
	...
	doLoadData: function(sender){
		...
		var strSQL = "select a.akun_pdd,x.nama as nama_pdd,a.akun_pdpt,y.nama as nama_pdpt,e.kode_pp,e.nama as nama_pp, "+
					 ...
					 "		 else a.nilai-isnull(xx.tot_batal,0)-isnull(d.tot_amor,0) end) as nilai_amor "+
					 "	from aka_bill_d a  "+
					 ...
					 "       inner join aka_amor_d ab on a.no_inv=ab.no_inv and a.kode_produk=ab.kode_produk and a.kode_lokasi=ab.kode_lokasi "+
					 "	   left join  "+
					 "		 (select no_inv,kode_produk,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as tot_amor  "+
					 "		  from aka_amor_d where kode_lokasi = '"+this.app._lokasi+"' and no_amor <> '"+this.e_nb.getText()+"' group by no_inv,kode_produk,kode_lokasi) d on  "+
					 ...
					 "	where a.kode_produk in ('BPP','S2BPP') and a.kode_lokasi = '"+this.app._lokasi+"' "+
					 " 	and a.nilai-isnull(xx.tot_batal,0) > isnull(d.tot_amor,0) and ab.no_amor = '"+this.e_nb.getText()+"' "+
					 "	group by a.akun_pdd,x.nama,a.akun_pdpt,y.nama,e.kode_pp,e.nama order by a.akun_pdd";
	...
	doChange:function(sender){
		if (sender == this.e_periode) {
			if (this.e_periode.getText()!="")  {
				...
				this.e_nb.setSQL("select no_amor, keterangan from aka_amor_m where modul = 'AMOLOAD' and posted ='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_amor","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti Amortisasi",true);
			}
			this.dataJU.rs.rows = [];
			this.sg.clear(1);			
		}
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU.rs.rows[i];							
			this.sg.appendData([line.akun_pdd,line.nama_pdd,line.akun_pdpt,line.nama_pdpt,line.kode_pp,line.nama_pp,floatToNilai(line.nilai_amor)]);
		}
