window.app_saku2_transaksi_aka_fAmorPdd2 = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_aka_fAmorPdd2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_aka_fAmorPdd2";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Amortisasi Pendapatan Diterima Dimuka : Proses", 0);	
...
window.app_saku2_transaksi_aka_fAmorPdd2.extend(window.childForm);
window.app_saku2_transaksi_aka_fAmorPdd2.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into aka_amor_m(no_amor,no_dokumen,tanggal,keterangan,nilai,kode_pp,kode_drk,posted,modul,nik_buat,nik_app,kode_lokasi,periode,nik_user,tgl_input) values "+
						    "('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"',"+parseNilai(this.e_nilai.getText())+",'-','"+this.cb_drk.getText()+"','F','AMOLOAD','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");					
					...
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						...
						sql.add("insert into aka_amor_j(no_amor,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
								"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+line.akun_pdd+"','"+this.e_ket.getText()+"','D',"+line.nilai_amor+",'"+line.kode_pp+"','"+this.cb_drk.getText()+"','"+this.app._lokasi+"','AMOLOAD','PDD','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");
						sql.add("insert into aka_amor_j(no_amor,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
								"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',1,'"+line.akun_pdpt+"','"+this.e_ket.getText()+"','C',"+line.nilai_amor+",'"+line.kode_pp+"','"+this.cb_drk.getText()+"','"+this.app._lokasi+"','AMOLOAD','PDPT','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");
					}			
					var perNext = nextNPeriode(this.e_periode.getText(),1);
					sql.add("call sp_ar_amor ('"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+perNext+"','"+this.e_nb.getText()+"','"+this.cb_drk.getText()+"')");
