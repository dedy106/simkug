window.app_saku2_transaksi_aka_fBillLoad2 = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_aka_fBillLoad2.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku2_transaksi_aka_fBillLoad2";
		...
		this.sg1 = new portalui_saiGrid(this.p1,{bound:[0,20,this.p1.width-5,this.p1.height-45],colCount:16,
...
window.app_saku2_transaksi_aka_fBillLoad2.extend(window.portalui_childForm);
window.app_saku2_transaksi_aka_fBillLoad2.implement({
						...
						try{
							...
							for (var i=0; i < this.dataUpload.rows.length;i++){
								...
								sql.add("insert into aka_bill_load(no_bill,jenis,kode_lokasi,periode,nim,nama,kode_jalur,flag_bea,flag_status,kode_jur,kode_akt,bpp,up3,sdp2,sks,s2bpp,s2up3,s2sdp2,s2sks) values "+
										"	('"+this.e_nb.getText()+"','BLOAD','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+line.nim+"','"+line.nama+"','"+line.jalur+"','"+line.beasiswa+"','"+line.status+"','"+line.prodi+"','"+line.angkatan+"',"+parseNilai(line.bpp)+","+parseNilai(line.up3)+","+parseNilai(line.sdp2)+","+parseNilai(line.sks)+","+parseNilai(line.s2bpp)+","+parseNilai(line.s2up3)+","+parseNilai(line.s2sdp2)+","+parseNilai(line.s2sks)+")");
							}							
							...
							sql.add("insert into aka_bill_d (no_bill,kode_lokasi,no_inv,nim,periode,kode_produk,akun_piutang,akun_pdpt,akun_pdd,nilai,periode_susut) "+
									...
									"union "+
									"select b.no_bill,b.kode_lokasi,b.no_bill+'-'+b.nim,b.nim,'"+this.e_periode.getText()+"','S2BPP', a.akun_piutang,a.akun_pdpt,a.akun_pdd,b.s2bpp as nilai,'"+this.e_periode.getText()+"' "+
									"from aka_bill_load b inner join aka_produk a on 'S2BPP' = a.kode_produk and b.kode_lokasi=a.kode_lokasi "+
									"where b.no_bill = '"+this.e_nb.getText()+"' and b.kode_lokasi = '"+this.app._lokasi+"' and b.s2bpp<>0"+
									"union "+
									"select b.no_bill,b.kode_lokasi,b.no_bill+'-'+b.nim,b.nim,'"+this.e_periode.getText()+"','S2UP3', a.akun_piutang,a.akun_pdpt,a.akun_pdd,b.s2up3 as nilai,'"+this.e_periode.getText()+"' "+
									"from aka_bill_load b inner join aka_produk a on 'S2UP3' = a.kode_produk and b.kode_lokasi=a.kode_lokasi "+
									"where b.no_bill = '"+this.e_nb.getText()+"' and b.kode_lokasi = '"+this.app._lokasi+"' and b.s2up3<>0"+
									"union "+
									"select b.no_bill,b.kode_lokasi,b.no_bill+'-'+b.nim,b.nim,'"+this.e_periode.getText()+"','S2SDP2', a.akun_piutang,a.akun_pdpt,a.akun_pdd,b.s2sdp2 as nilai,'"+this.e_periode.getText()+"' "+
									"from aka_bill_load b inner join aka_produk a on 'S2SDP2' = a.kode_produk and b.kode_lokasi=a.kode_lokasi "+
									"where b.no_bill = '"+this.e_nb.getText()+"' and b.kode_lokasi = '"+this.app._lokasi+"' and b.s2sdp2<>0"+
									"union "+
									"select b.no_bill,b.kode_lokasi,b.no_bill+'-'+b.nim,b.nim,'"+this.e_periode.getText()+"','S2SKS', a.akun_piutang,a.akun_pdpt,a.akun_pdd,b.s2sks as nilai,'"+this.e_periode.getText()+"' "+
									"from aka_bill_load b inner join aka_produk a on 'S2SKS' = a.kode_produk and b.kode_lokasi=a.kode_lokasi "+
							...
							if (typeof data == "object" && data.rs.rows[0] != undefined){
								...
								for (var i in data.rs.rows){
									...
									if (line.kode_produk.toUpperCase() == "SKS") {
										nilai = this.sks;
										akunCR = line.akun_pdpt;
										jenis = "SKS";
									}
									if (line.kode_produk.toUpperCase() == "S2BPP") {
										nilai = this.s2bpp;
										akunCR = line.akun_pdd;
										jenis = "S2BPP";
									} 
									if (line.kode_produk.toUpperCase() == "S2UP3") {
										nilai = this.s2up3;
										akunCR = line.akun_pdpt;
										jenis = "S2UP3";
									} 
									if (line.kode_produk.toUpperCase() == "S2SDP2") {
										nilai = this.s2sdp2;
										akunCR = line.akun_pdpt;
										jenis = "S2SDP2";
									}
									if (line.kode_produk.toUpperCase() == "S2SKS") {
										nilai = this.s2sks;
										akunCR = line.akun_pdpt;
										jenis = "S2SKS";
									}
