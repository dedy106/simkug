window.app_saku2_transaksi_aka_fRekonLoadTxtBea = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_aka_fRekonLoadTxtBea.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_aka_fRekonLoadTxtBea";
		...
		this.bTampil = new button(this,{bound:[420,22,80,20],caption:"Data Tagihan", click:[this,"doLoad"]});
...
window.app_saku2_transaksi_aka_fRekonLoadTxtBea.extend(window.childForm);
window.app_saku2_transaksi_aka_fRekonLoadTxtBea.implement({
	...
	doLoad: function(sender){		
		...
		if (this.c_jenis.getText() == "ALL") {
			...
			var strSQL = "select aa.nim,aa.nama as mhs,a.no_inv,a.periode,a.kode_produk,c.nama,a.akun_piutang,(a.nilai-isnull(x.tot_batal,0)-isnull(b.tot_lunas,0)) as saldo,0 as lunas, '-' as id_bank "+
						 ...
						 "      left join (select no_inv,nim,kode_produk,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as tot_batal "+
						 "                 from aka_batal_d group by nim,no_inv,kode_produk,kode_lokasi) x on a.nim=x.nim and a.no_inv=x.no_inv and a.kode_produk=x.kode_produk and a.kode_lokasi=x.kode_lokasi "+						 
						 "where a.periode = '"+this.c_periode2.getText()+"' and (a.nilai-isnull(x.tot_batal,0)-isnull(b.tot_lunas,0)) > 0 and a.kode_lokasi = '"+this.app._lokasi+"' and a.kode_produk like 'BEA%' order by aa.nim,a.periode,c.no_urut";		
		}
