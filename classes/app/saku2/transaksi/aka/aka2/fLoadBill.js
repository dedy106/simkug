window.app_saku2_transaksi_aka_aka2_fLoadBill = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_aka_aka2_fLoadBill.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku2_transaksi_aka_aka2_fLoadBill";
		...
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:14,tag:9,
		...
		this.sg2 = new portalui_saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:3,tag:9,
...
window.app_saku2_transaksi_aka_aka2_fLoadBill.extend(window.portalui_childForm);
window.app_saku2_transaksi_aka_aka2_fLoadBill.implement({	
	...
	doValid: function(sender){
		...
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			var line;			
			for (var i in data.rs.rows){
				//temu = true; ---> diloloskan just info
				line = data.rs.rows[i];							
		...
		var strSQL = "select a.nim+' | '+a.kode_produk as id_err "+
					 "from aka_bill_tmp a "+
					 "inner join aka_mahasiswa b on a.nim=b.nim and a.kode_lokasi=b.kode_lokasi "+
					 "inner join aka_produk c on a.kode_produk=c.kode_produk and a.kode_lokasi=c.kode_lokasi and b.kode_jur=c.kode_pp and b.kode_akt=c.kode_akt and b.kode_jalur=c.kode_jalur and c.tarif<>0 "+
					 "where a.nilai <> 0 and a.no_bill='"+this.nbTmp+"' and a.kode_lokasi ='"+this.app._lokasi+"' and a.nilai <> c.tarif ";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			var line;			
			for (var i in data.rs.rows){
				//temu = true; ---> diloloskan just info
				line = data.rs.rows[i];							
				this.sg2.appendData([line.id_err,"NIM | Kode Biaya","NIM untuk tarif Kode Biaya tidak sesuai"]);
			}
		}
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("update a set a.kode_akt=b.kode_akt,a.kode_pp=b.kode_jur,a.kode_jalur=b.kode_jalur "+
							...
							"where a.no_bill='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_produk in ('UP3','SDP2','BPPP','BPPNP','SKS','DENDA','USTATUS','PERPUS','ASUR') ");
							
					sql.add("update a set a.akun_piutang=b.akun_piutang, a.akun_pdpt=b.akun_pdpt, a.akun_pdd=b.akun_pdd, a.kode_drk=b.kode_drk "+
							"from aka_bill_d a inner join aka_produk b on a.kode_produk=b.kode_produk and a.kode_lokasi=b.kode_lokasi and a.kode_pp=b.kode_pp and a.kode_akt=b.kode_akt and a.kode_jalur=b.kode_jalur "+
					...
					sql.add("update a set a.periode_susut=b.periode "+
							...
							"	select top 1 kode_lokasi,tahunaka,periode "+
							"   from aka_tahunaka "+
					...
					*/
					...
					sql.add("insert into aka_bill_j(no_bill,kode_lokasi,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,modul,jenis,periode,no_dokumen,kode_curr,kurs,nik_user,tgl_input) "+
							"select '"+this.e_nb.getText()+"',kode_lokasi,'"+this.dp_d1.getDateString()+"',0,akun_piutang,'"+this.e_ket.getText()+"','D',sum(nilai),kode_pp,'-','BLOAD',kode_produk,'"+this.e_periode.getText()+"','-','IDR',1,'"+this.app._userLog+"',getdate() "+
							"from aka_bill_d "+									
							"where kode_produk in ('UP3','SDP2','BPPP','BPPNP','SKS','PERPUS','DENDA','USTATUS','ASUR') and no_bill='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' group by kode_lokasi,akun_piutang,kode_produk,kode_pp");
					...
					sql.add("insert into aka_bill_j(no_bill,kode_lokasi,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,modul,jenis,periode,no_dokumen,kode_curr,kurs,nik_user,tgl_input) "+
							"select '"+this.e_nb.getText()+"',a.kode_lokasi,'"+this.dp_d1.getDateString()+"',1,a.akun_pdpt,'"+this.e_ket.getText()+"','C',sum(a.nilai),b.kode_pp2,a.kode_drk,'BLOAD',a.kode_produk,'"+this.e_periode.getText()+"','-','IDR',1,'"+this.app._userLog+"',getdate() "+
							"from aka_bill_d a inner join aka_produk b on a.kode_produk=b.kode_produk and a.kode_lokasi=b.kode_lokasi and a.kode_pp=b.kode_pp and a.kode_akt=b.kode_akt and a.kode_jalur=b.kode_jalur "+
							"where a.kode_produk in ('PERPUS','ASUR') and a.no_bill='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' group by a.kode_lokasi,a.akun_pdpt,a.kode_produk,b.kode_pp2,a.kode_drk");
