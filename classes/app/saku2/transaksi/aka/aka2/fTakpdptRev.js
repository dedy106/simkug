window.app_saku2_transaksi_aka_aka2_fTakpdptRev = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_aka_aka2_fTakpdptRev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_aka_aka2_fTakpdptRev";
		...
		this.cb_bukti = new saiCBBL(this.pc2.childPage[0],{bound:[20,13,220,20],caption:"No TAK PDPT", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});										
...
window.app_saku2_transaksi_aka_aka2_fTakpdptRev.extend(window.childForm);
window.app_saku2_transaksi_aka_aka2_fTakpdptRev.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into takkirim_m(no_kirim,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user,kode_loktuj,progress,no_terima,due_date) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_bukti.getText()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','TAKPDREV','KIRIM','IDR',1,"+this.nilaiTAK+",'"+this.app._userLog+"','"+this.app._userLog+"','F','-','-','-',getdate(),'"+this.app._userLog+"','"+this.cb_lokasi.getText()+"','0','-','"+this.dp_d1.getDateString()+"')");
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){															           
								sql.add("insert into takkirim_j(no_kirim,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','"+this.cb_bukti.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','"+this.app._lokasi+"','TAKPDREV','JURNAL','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");
							}
						}
					}
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"('"+this.e_nb.getText()+"','TAKPDREV','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(6,i))+","+nilai+")");
							}
						}
					}
					...
					//nge-balik (reverse)
					sql.add("insert into aka_tak_d(no_tak,nim,no_inv,periode,nilai,kode_lokasi,akun_tak,akun_pdpt,kode_produk,dc,modul,kode_drk) "+
							"select '"+this.e_nb.getText()+"',nim,no_inv,'"+this.e_periode.getText()+"',nilai,kode_lokasi,akun_tak,akun_pdpt,kode_produk,'C',modul,kode_drk from aka_tak_d where no_tak ='"+this.cb_bukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");							
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					setTipeButton(tbAllFalse);
					this.cb_bukti.setSQL("select no_kirim, keterangan from takkirim_m where periode<='"+this.e_periode.getText()+"' and modul='TAKBILL' and kode_lokasi = '"+this.app._lokasi+"' "+
		                     			 "and no_kirim not in (select no_dokumen from takkirim_m where kode_lokasi='"+this.app._lokasi+"' and modul='TAKPDREV')",["no_kirim","keterangan"],false,["No Bukti","Deskripsi"],"and","Data TAK SDP2 - UP3",true);				
				break;
			case "simpan" :															
			case "ubah" :															
				this.preView = "1";						
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "") {
			if (this.stsSimpan == 0) {					
				...
				this.cb_bukti.setSQL("select no_kirim, keterangan from takkirim_m where periode<='"+this.e_periode.getText()+"' and modul='TAKBILL' and kode_lokasi = '"+this.app._lokasi+"' "+
		                     "and no_kirim not in (select no_dokumen from takkirim_m where kode_lokasi='"+this.app._lokasi+"' and modul='TAKPDREV')",["no_kirim","keterangan"],false,["No Bukti","Deskripsi"],"and","Data TAK PDPT",true);				
			}
			this.stsSimpan = 1;
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_kirim,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.kode_loktuj+' - '+b.nama as lokasi,a.nilai "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'TAKPDREV' and a.posted ='F' and a.no_terima='-' and progress='0' ";		
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){				
						this.cb_bukti.setSQL("select no_kirim, keterangan from takkirim_m where no_kirim='"+line.no_dokumen+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kirim","keterangan"],false,["No Bukti","Deskripsi"],"and","Data TAK PDPT",true);				
