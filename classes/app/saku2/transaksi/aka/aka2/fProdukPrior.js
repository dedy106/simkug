window.app_saku2_transaksi_aka_aka2_fProdukPrior = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_aka_aka2_fProdukPrior.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku2_transaksi_aka_aka2_fProdukPrior";
		...
		this.sg1 = new portalui_saiGrid(this.p1,{bound:[0,20,this.p1.width-5,this.p1.height-45],colCount:2,tag:9,
				colTitle:["Kode Produk","Urut Prioritas"],				
...
window.app_saku2_transaksi_aka_aka2_fProdukPrior.extend(window.portalui_childForm);
window.app_saku2_transaksi_aka_aka2_fProdukPrior.implement({	
			...
			case "simpan" :					
					...
					if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
						try{
							...
							sql.add("delete from aka_produk_prior where kode_lokasi='"+this.app._lokasi+"' and tahunaka='"+this.cb_kode.getText()+"'");
							for (var i=0;i < this.sg1.getRowCount();i++){									
								sql.add("insert into aka_produk_prior(tahunaka,kode_produk,kode_lokasi,no_urut) values "+
										"('"+this.cb_kode.getText()+"','"+this.sg1.cells(0,i)+"','"+this.app._lokasi+"',"+nilaiToFloat(this.sg1.cells(1,i))+")");
							}							
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var data = this.dbLib.getDataProvider("select kode_produk,no_urut from aka_produk_prior where kode_lokasi='"+this.app._lokasi+"' and tahunaka='"+this.cb_kode.getText()+"'",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg1.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData([line.kode_produk,floatToNilai(line.no_urut)]);
					}
