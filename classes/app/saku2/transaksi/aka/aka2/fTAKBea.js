window.app_saku2_transaksi_aka_aka2_fTAKBea = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_aka_aka2_fTAKBea.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_aka_aka2_fTAKBea";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form TAK Beasiswa", 0);	
		...
		this.cb_pdd = new saiCBBL(this.pc2.childPage[0],{bound:[20,19,220,20],caption:"Akun PDD", multiSelection:false, maxLength:10, tag:2 });
		...
		this.bTampil = new button(this.pc2.childPage[0],{bound:[670,18,80,20],caption:"Data Rekon", click:[this,"doLoad"]});
		
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,12,990,230], childPage:["Data Rekon"]});
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:4,tag:0,
		            colTitle:["Status","No Rekon","Keterangan","Nilai"],
					colWidth:[[3,2,1,0],[100,400,120,80]],
					colFormat:[[3],[cfNilai]],
					columnReadOnly:[true,[0,1,2,3],[]],								
					buttonStyle:[[0],[bsAuto]], picklist:[[0],[new portalui_arrayMap({items:["TAK","INPROG"]})]],										
		...
		try {
			...
			this.cb_pdd.setSQL("select a.kode_akun, a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag='034' "+
			                    "where a.kode_lokasi='"+this.app._lokasi+"'",["a.kode_akun","a.nama"],false,["Kode","Nama"],"and","Daftar Akun TAK",true);			
...
window.app_saku2_transaksi_aka_aka2_fTAKBea.extend(window.childForm);
window.app_saku2_transaksi_aka_aka2_fTAKBea.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("update aka_bill_bea set no_tak='-' where no_tak = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}
					...
					sql.add("insert into takkirim_m(no_kirim,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user,kode_loktuj,progress,no_terima,due_date) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.noRekon+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','TAKBEA','KIRIM','IDR',1,"+nilaiToFloat(this.e_nilai.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','F','-','"+this.cb_pdd.getText()+"','"+this.cb_akun.getText()+"',getdate(),'"+this.app._userLog+"','"+this.cb_lokasi.getText()+"','0','-','"+this.dp_d1.getDateString()+"')");
					sql.add("insert into takkirim_j(no_kirim,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','"+this.noRekon+"','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','TAKBEA','TAK','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");															
					sql.add("insert into takkirim_j(no_kirim,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','"+this.noRekon+"','"+this.dp_d1.getDateString()+"',1,'"+this.cb_pdd.getText()+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','TAKBEA','PDD','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");															
					
					for (var i=0;i < this.sg.getRowCount();i++){
						if (this.sg.rowValid(i)){
							if (this.sg.cells(0,i) == "TAK") {
								sql.add("update aka_bill_bea set no_tak='"+this.e_nb.getText()+"' where no_rekon = '"+this.sg.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
							}							
						}
					}
			...
			case "ubah" :
				...
				for (var i=0;i < this.sg.getRowCount();i++){
					if (this.sg.rowValid(i)){
						if (this.sg.cells(0,i) == "TAK") {
							temu ++;
							this.noRekon = this.sg.cells(1,i);
						}							
					}
				}
				if (temu != 1) {
					system.alert(this,"Transaksi tidak valid.","TAK hanya untuk satu Nomor Rekon.");
					return false;						
				}
	...
	doLoad: function(sender){		
		try {			
			...
			var strSQL = "select a.no_rekon,a.keterangan,b.nilai "+
						 "from aka_rekon_m a inner join ( "+
						 "	select kode_lokasi,no_rekon,sum(pakai) as nilai "+
						 "	from aka_bill_bea "+
						 "	where no_rekon<>'-' and no_tak = '-' and kode_lokasi='"+this.app._lokasi+"' "+
						 "	group by kode_lokasi,no_rekon "+
						 ") b on a.no_rekon=b.no_rekon and a.kode_lokasi=b.kode_lokasi where a.kode_lokasi='"+this.app._lokasi+"'";						
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];																		
					this.sg.appendData(["INPROG",line.no_rekon,line.keterangan,floatToNilai(line.nilai)]);
				}
	...
	doNilaiChange: function(){
		try{			
			...
			for (var i = 0; i < this.sg.rows.getLength();i++){
				if (this.sg.rowValid(i) && this.sg.cells(3,i) != "" && this.sg.cells(0,i) == "TAK"){										
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_kirim,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.kode_loktuj+' - '+b.nama as lokasi,a.nilai "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'TAKBEA' and a.posted ='F' and a.no_terima='-' and progress='0' ";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select keterangan,tanggal,kode_loktuj,ref1,nik_buat,nik_setuju,no_link "+
							 "from takkirim_m "+							 
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						this.cb_pdd.setText(line.no_link);									
				...
				var strSQL = "select a.no_rekon,a.keterangan,b.nilai "+
							 "from aka_rekon_m a inner join ( "+
							 "	select kode_lokasi,no_rekon,sum(nilai) as nilai "+
							 "	from aka_bill_bea "+
							 "	where no_rekon<>'-' and no_tak = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
							 "	group by kode_lokasi,no_rekon "+
							 ") b on a.no_rekon=b.no_rekon and a.kode_lokasi=b.kode_lokasi where a.kode_lokasi='"+this.app._lokasi+"'";				
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];																		
						this.sg.appendData(["TAK",line.no_rekon,line.keterangan,floatToNilai(line.nilai)]);
					}
				} else this.sg.clear(1);													
			}									
		} catch(e) {alert(e);}
