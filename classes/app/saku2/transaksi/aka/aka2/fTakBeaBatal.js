window.app_saku2_transaksi_aka_aka2_fTAKBeaBatal = function(owner)
{
	if (owner)
	{		
		window.app_saku2_transaksi_aka_aka2_fTAKBeaBatal.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_aka_aka2_fTAKBeaBatal";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pembatalan TAK Kirim Beasiswa", 0);	
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:13,tag:9,
		            colTitle:["Modul","No Bukti","Status","Tanggal","Due Date","PP","No Dokumen","Deskripsi","Nilai","Pembuat","No Flag","Lok. Tujuan","Tgl Input"],
		...
		this.c_status = new saiCB(this.pc1.childPage[1],{bound:[20,11,202,20],caption:"Status",items:["FINAL"], readOnly:true,tag:2,change:[this,"doChange"],visible:false});				
		...
		this.sg2 = new saiGrid(this.pc1.childPage[3],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:9,tag:9,
...
window.app_saku2_transaksi_aka_aka2_fTAKBeaBatal.extend(window.childForm);
window.app_saku2_transaksi_aka_aka2_fTAKBeaBatal.implement({	
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg.cells(0,row) != "") {							
				...
				this.kodeLokAsal = this.app._lokasi;//this.sg.cells(11,row);				
				...
				var strSQL = "select isnull(d.catatan,'-') as catatan "+
							 ...
							 "where a.no_kirim='"+this.e_nobukti.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' ";					 
				...
				if (this.e_modul.getText() == "TAKUMUM") {
					...
					if (this.stsProg == "INPROG") { //load data awal jurnal kirim
						var strSQLlawanSeb = "select a.kode_akun,b.nama as nama_akun,case a.dc when 'D' then 'C' else 'D' end as dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk "+
									"from takkirim_j a inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
									"                  inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+										
									"                  left join (select kode_akun,kode_lokasi from flag_relasi where kode_flag='016' and kode_lokasi='"+this.app._lokasi+"') e on b.kode_akun=e.kode_akun and b.kode_lokasi=e.kode_lokasi "+
									"                  left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and substring(a.periode,1,4)=d.tahun "+													
									"where a.no_kirim = '"+this.e_nobukti.getText()+"' and a.kode_lokasi='"+this.kodeLokAsal+"' and e.kode_akun is null order by a.no_urut";					
						var data = this.dbLib.getDataProvider(strSQLlawanSeb,true);
						if (typeof data == "object" && data.rs.rows[0] != undefined){
							var line;
							this.sgv.clear();
							for (var i in data.rs.rows){
	...
	doLoad:function(sender){																		
		var strSQL = "select a.no_kirim as no_bukti,'INPROG' as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_terima,a.kode_loktuj,convert(varchar,a.tgl_input,120) as tglinput "+
					 ...
					 "where a.progress='0' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul<> 'BILKIRIM' and a.periode<='"+this.e_periode.getText()+"' ";
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData([line.modul.toUpperCase(),line.no_bukti,line.status.toUpperCase(),line.tgl,line.tgl2,line.pp,line.no_dokumen,line.keterangan,floatToNilai(line.nilai),line.pembuat,line.no_terima,line.kode_loktuj,line.tglinput]); 
		}
