window.app_saku2_transaksi_aka_fAmorPddProdiTU = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_aka_fAmorPddProdiTU.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_aka_fAmorPddProdiTU";
		...
		this.cb_app = new saiCBBL(this,{bound:[20,15,220,20],caption:"NIK Approve", multiSelection:false, maxLength:10, tag:2});
		this.cb_fak = new saiCBBL(this,{bound:[20,17,220,20],caption:"Fakultas", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});		
		...
		this.p1 = new panel(this,{bound:[20,23,1000,333],caption:"Data Jurnal Amortisasi"});
...
window.app_saku2_transaksi_aka_fAmorPddProdiTU.extend(window.childForm);
window.app_saku2_transaksi_aka_fAmorPddProdiTU.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into aka_amor_m(no_amor,no_dokumen,tanggal,keterangan,nilai,kode_pp,kode_drk,posted,modul,nik_buat,nik_app,kode_lokasi,periode,nik_user,tgl_input,kode_fakultas) values "+
						    "('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"',"+parseNilai(this.e_nilai.getText())+",'-','-','F','AMOLOADPR','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.cb_fak.getText()+"')");					
					...
					sql.add("call sp_ar_amor_prodi_tu ('"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+perNext+"','"+this.e_nb.getText()+"')");
	...
	doLoadData: function(sender){
		...
		if (this.cb_fak.getText()!="") {
			var strSQL = "select a.akun_pdd,x.nama as nama_pdd,a.akun_pdpt,y.nama as nama_pdpt,e.kode_pp,e.nama as nama_pp, "+
						 ...
						 "	from aka_bill_d a  "+
						 "		 inner join aka_mahasiswa b on a.nim=b.nim and a.kode_lokasi=b.kode_lokasi and a.kode_fakultas=b.kode_fakultas "+
						 "		 inner join aka_jurusan c on b.kode_jur=c.kode_jur and b.kode_lokasi=c.kode_lokasi and b.kode_fakultas=c.kode_fakultas "+
						 ...
						 "		  from aka_amor_d where kode_lokasi = '"+this.app._lokasi+"' and periode='"+this.e_periode.getText()+"' group by no_inv,kode_produk,kode_lokasi) d on  "+
						 ...
						 "        from aka_batal_d where kode_lokasi = '"+this.app._lokasi+"' and periode='"+this.e_periode.getText()+"' group by no_inv,kode_produk,kode_lokasi) xx on "+
						 ...
						 " 	and a.nilai-isnull(xx.tot_batal,0) > isnull(d.tot_amor,0) and a.periode_susut = '"+this.e_periode.getText()+"' and a.kode_fakultas='"+this.cb_fak.getText()+"' "+
