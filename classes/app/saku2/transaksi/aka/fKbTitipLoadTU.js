window.app_saku2_transaksi_aka_fKBTitipLoadTU = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_aka_fKBTitipLoadTU.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_aka_fKBTitipLoadTU";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Titipan: Input", 0);	
		...
		this.cb_kas = new saiCBBL(this,{bound:[20,21,220,20],caption:"Akun KasBank", multiSelection:false, maxLength:10, tag:2});
		this.cb_titip = new saiCBBL(this,{bound:[20,22,220,20],caption:"Akun Titipan", multiSelection:false, maxLength:10, tag:2});
		this.cb_cf = new saiCBBL(this,{bound:[20,19,220,20],caption:"CashFlow", multiSelection:false, maxLength:10, tag:2});
		...
		this.e_nilai = new saiLabelEdit(this,{bound:[820,17,200,20],caption:"Nilai KB Titipan", tag:1, tipeText:ttNilai, text:"0",readOnly:true});		
		this.bUpload = new portalui_uploader(this,{bound:[700,17,80,20],param3:"object",param4:"gridupload",afterUpload:[this,"doAfterUpload"],autoSubmit:true});		
		
		this.pc1 = new pageControl(this,{bound:[20,12,1000,327], childPage:["Data Pelunasan"]});
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:5,
					colTitle:["NIM","Nama","Nilai","ID Bank","Keterangan"],
					colWidth:[[4,3,2,1,0],[200,200,100,300,150]],
					colFormat:[[2],[cfNilai]],
					readOnly:true, defaultRow:1
					});		
		this.sgn1 = new portalui_sgNavigator(this.pc1.childPage[0],{bound:[0,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:3, grid:this.sg1, pager:[this,"selectPage"]});		
...
window.app_saku2_transaksi_aka_fKBTitipLoadTU.extend(window.childForm);
window.app_saku2_transaksi_aka_fKBTitipLoadTU.implement({
	doAfterUpload: function(sender, result, data){		
	    try{   			
			...
			for (var i=0; i < this.dataUpload.rows.length;i++){
				...
				nilaiLoad += parseFloat(line.nilai);				
			}
			this.e_nilai.setText(floatToNilai(nilaiLoad));
   		}catch(e){
	...
	selectPage: function(sender,page){
		...
		for (var i=start; i < finish;i++){
			line = this.dataUpload.rows[i];
			this.sg1.appendData([line.nim,line.nama,floatToNilai(line.nilai),line.id_bank,line.keterangan]);
		}
				...
				try{														
					...
					for (var i=0; i < this.dataUpload.rows.length;i++){
						...
						sql.add("insert into aka_depo_d (no_bukti,kode_lokasi, nim,nim_seb,id_bank,periode,nilai,keterangan,modul,no_rekon,nama) values "+
						        "('"+this.e_nb.getText()+"','"+this.app._lokasi+"', '"+line.nim+"','-','"+line.id_bank+"','"+this.e_periode.getText()+"',"+parseFloat(line.nilai)+",'"+line.keterangan+"','KBTTP','-','"+line.nama.substr(0,50)+"')");
					}					
