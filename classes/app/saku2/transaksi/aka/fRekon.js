window.app_saku2_transaksi_aka_fRekon = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_aka_fRekon.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_aka_fRekon";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Rekonsiliasi Pelunasan Tagihan : Input", 0);	
...
window.app_saku2_transaksi_aka_fRekon.extend(window.childForm);
window.app_saku2_transaksi_aka_fRekon.implement({
	...
	doChange: function(sender){
		if (sender == this.cb_nim && this.cb_nim.getText()!="") {
			...
			var strSQL = "select a.no_inv,a.periode,a.kode_produk,c.nama,a.akun_piutang,(a.nilai-isnull(x.tot_batal,0)-isnull(b.tot_lunas,0)) as saldo "+
						 "from aka_bill_d a "+
						 ...
						 "                 from aka_batal_d where nim = '"+this.cb_nim.getText()+"' and kode_lokasi='"+this.app._lokasi+"' group by no_inv,kode_produk,kode_lokasi) x on a.no_inv=x.no_inv and a.kode_produk=x.kode_produk and a.kode_lokasi=x.kode_lokasi "+						 
						 "where (a.nilai-isnull(x.tot_batal,0)-isnull(b.tot_lunas,0)) > 0 and a.nim = '"+this.cb_nim.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' ";
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData([line.no_inv,line.periode,line.kode_produk,line.nama,line.akun_piutang,floatToNilai(line.saldo),"0","-"]);
				}
