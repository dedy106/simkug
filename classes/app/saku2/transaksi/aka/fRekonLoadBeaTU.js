window.app_saku2_transaksi_aka_fRekonLoadBeaTU = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_aka_fRekonLoadBeaTU.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_aka_fRekonLoadBeaTU";
		...
		this.cb_fak = new saiCBBL(this,{bound:[20,12,220,20],caption:"Fakultas", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});		
		...
		this.bUpload = new portalui_uploader(this,{bound:[620,22,80,20],param3:"object",param4:"gridupload",afterUpload:[this,"doAfterUpload"],autoSubmit:true});		
				
		this.pc1 = new pageControl(this,{bound:[20,12,900,320], childPage:["Data Tagihan","Data Pelunasan","Info Sisa"]});
		...
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[1],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:4,
					colTitle:["NIM","Nama","Nilai","ID Bank"],
		...
		try {
			...
			this.cb_titip.setSQL("select a.kode_akun, a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag not in ('001','009') where a.kode_lokasi='"+this.app._lokasi+"'",["a.kode_akun","a.nama"],false,["Kode","Nama"],"and","Daftar Akun Pelunasan",true);			
...
window.app_saku2_transaksi_aka_fRekonLoadBeaTU.extend(window.childForm);
window.app_saku2_transaksi_aka_fRekonLoadBeaTU.implement({
	...
	doCek:function(sender){										
		...
		for (var i=0; i < this.dataUpload.rows.length;i++){
			...
			var nilaiLoad = parseFloat(line.nilai);				
			for (var j=0;j < this.dataJU.rs.rows.length;j++){
				if (line.nim == this.dataJU.rs.rows[j].nim && nilaiLoad > 0) {						
					if (nilaiLoad > (this.dataJU.rs.rows[j].saldo-this.dataJU.rs.rows[j].lunas)) { 							
						nilaiLoad = nilaiLoad - (this.dataJU.rs.rows[j].saldo-this.dataJU.rs.rows[j].lunas);
			...
			if (nilaiLoad != 0) {					
				msg+= "NIM : "+line.nim+" - Sisa : "+floatToNilai(nilaiLoad)+"; \n";
			}
		}
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into aka_rekon_m(no_rekon,no_dokumen,tanggal,keterangan,nilai,posted,modul,akun_titip,nim,nik_buat,nik_app,kode_lokasi,periode,nik_user,tgl_input,kode_fakultas) values "+
						    "('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"',"+parseNilai(this.e_nilai.getText())+",'F','LOAD','"+this.cb_titip.getText()+"','-','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.cb_fak.getText()+"')");
					...
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						...
						if (parseFloat(line.lunas) != 0){
							sql.add("insert into aka_rekon_d(no_rekon,nim,no_inv,periode,nilai,kode_lokasi,akun_titip,akun_piutang,kode_produk,dc,modul,id_bank,kode_fakultas) values "+
									"	('"+this.e_nb.getText()+"','"+line.nim+"','"+line.no_inv+"','"+this.e_periode.getText()+"',"+line.lunas+",'"+this.app._lokasi+"','"+this.cb_titip.getText()+"','"+line.akun_piutang+"','"+line.kode_produk+"','D','LOAD','"+line.id_bank+"','"+this.cb_fak.getText()+"')");
						}
					}
	...
	doLoad: function(sender){		
		...
		if (this.cb_fak.getText()!="") {
			if (this.c_jenis.getText() == "ALL") {
				var strSQL = "select aa.nim,aa.nama as mhs,a.no_inv,a.periode,a.kode_produk,c.nama,a.akun_piutang,(a.nilai-isnull(x.tot_batal,0)-isnull(b.tot_lunas,0)) as saldo,0 as lunas, '-' as id_bank "+
							 ...
							 "where a.kode_fakultas ='"+this.cb_fak.getText()+"' and (a.nilai-isnull(x.tot_batal,0)-isnull(b.tot_lunas,0)) > 0 and a.kode_lokasi = '"+this.app._lokasi+"' order by aa.nim,a.periode,c.no_urut";		
			}
			else {
				var strSQL = "select aa.nim,aa.nama as mhs,a.no_inv,a.periode,a.kode_produk,c.nama,a.akun_piutang,(a.nilai-isnull(x.tot_batal,0)-isnull(b.tot_lunas,0)) as saldo,0 as lunas, '-' as id_bank "+
							 "from aka_bill_d a "+
							 ...
							 "      left join (select no_inv,nim,kode_produk,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as tot_batal "+
							 "                 from aka_batal_d group by nim,no_inv,kode_produk,kode_lokasi) x on a.nim=x.nim and a.no_inv=x.no_inv and a.kode_produk=x.kode_produk and a.kode_lokasi=x.kode_lokasi "+						 
							 "where a.kode_fakultas ='"+this.cb_fak.getText()+"' and a.periode = '"+this.c_periode2.getText()+"' and (a.nilai-isnull(x.tot_batal,0)-isnull(b.tot_lunas,0)) > 0 and a.kode_lokasi = '"+this.app._lokasi+"' and a.kode_produk like 'BEA%' order by aa.nim,a.periode,c.no_urut";		
			}
		...
		else system.alert(this,"Fakultas tidak valid.","Fakultas harus dipilih.");
	},
