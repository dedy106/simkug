window.app_saku2_transaksi_fa_fFaTak = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_fa_fFaTak.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_fa_fFaTak";
		...
		try {
			...
			this.cb_tak.setText("1512000003");
		}catch(e){
			systemAPI.alert(e);
		}
	}
};
window.app_saku2_transaksi_fa_fFaTak.extend(window.childForm);
window.app_saku2_transaksi_fa_fFaTak.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,3])){
				try{													
					...
					sql.add("insert into takkirim_m(no_kirim,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user,kode_loktuj,progress) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','-','TAKFA','KIRIM','IDR',1,0,'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','F','-','-','"+this.cb_tak.getText()+"',getdate(),'"+this.app._userLog+"','"+this.cb_lokasi.getText()+"','0')");
