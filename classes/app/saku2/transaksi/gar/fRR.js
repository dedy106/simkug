window.app_saku2_transaksi_gar_fRR = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_gar_fRR.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_gar_fRR";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Redistribusi, Reshedule dan Realokasi Anggaran: Input", 0);	
		...
		this.p1 = new panel(this,{bound:[20,23,900,333],caption:"Data Item Redistribusi, Reschedule dan Realokasi Anggaran"});
		this.sg = new saiGrid(this.p1,{bound:[1,20,this.p1.width-5,this.p1.height-45],colCount:9,tag:0,
		            colTitle:["Kode Akun","Nama Akun","Kode PP","Nama PP","Kode DRK","Nama DRK","Periode","DC","Nilai"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[100,60,80,150,80,150,80,150,80]],
					columnReadOnly:[true,[1,3,5,6],[0,2,4,7,8]],
					buttonStyle:[[0,2,4,6,7],[bsEllips,bsEllips,bsEllips,bsAuto,bsAuto]], 
					colFormat:[[8],[cfNilai]],
					picklist:[[6,7],[new portalui_arrayMap({items:["01","02","03","04","05","06","07","08","09","10","11","12"]}),new portalui_arrayMap({items:["D","C"]})]],
...
window.app_saku2_transaksi_gar_fRR.extend(window.childForm);
window.app_saku2_transaksi_gar_fRR.implement({
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								...
								sql.add("insert into anggaran_d (no_agg,kode_lokasi,no_urut,kode_pp,kode_akun,kode_drk,volume,periode,nilai_sat,nilai,dc,satuan,nik_user,tgl_input,modul) values "+		
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',"+i+",'"+this.sg.cells(2,i)+"','"+this.sg.cells(0,i)+"','"+this.sg.cells(4,i)+"',1,'"+periode+"',"+parseNilai(this.sg.cells(8,i))+","+parseNilai(this.sg.cells(8,i))+",'"+this.sg.cells(7,i)+"','-','"+this.app._userLog+"',getdate(),'RRA')");
							}
						}
					}
				...
				for (var i=0;i < this.sg.getRowCount();i++) {
					if (this.sg.rowValid(i)){
						if (this.sg.cells(6,i) != "01" && this.sg.cells(6,i) != "02" && this.sg.cells(6,i) != "03") {
							system.alert(this,"Transaksi tidak valid.","Hanya di TW1");
							return false;						
						}
					}
				}
	...
	doCopy: function(sender){
		try {
			if (this.sg.rowValid(this.sg.row)) {
				var data = [];
				for (var i=0;i<this.sg.getColCount();i++) 	
	...
	*/
	...
	doChangeCell: function(sender, col, row){
		if ((col == 7 || col == 8) && (this.sg.cells(8,row) != "")) this.sg.validasi();
		...
		try{
			...
			for (var i = 0; i < this.sg.rows.getLength();i++){
				if (this.sg.rowValid(i) && this.sg.cells(8,i) != ""){
					if (this.sg.cells(7,i) == "D") totD += nilaiToFloat(this.sg.cells(8,i));
					if (this.sg.cells(7,i) == "C") totC += nilaiToFloat(this.sg.cells(8,i));
				}
			}
