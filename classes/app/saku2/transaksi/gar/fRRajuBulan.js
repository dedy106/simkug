window.app_saku2_transaksi_gar_fRRajuBulan = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_gar_fRRajuBulan.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_gar_fRRajuBulan";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan Redis, Reshedule dan Realok Anggaran: Input", 0);	
...
window.app_saku2_transaksi_gar_fRRajuBulan.extend(window.childForm);
window.app_saku2_transaksi_gar_fRRajuBulan.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into rra_pdrk_m(no_pdrk,kode_lokasi,keterangan,kode_pp,kode_bidang,jenis_agg,tanggal,periode,nik_buat,nik_app1,nik_app2,nik_app3,sts_pdrk,justifikasi, nik_user, tgl_input,progress) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_periode.getText()+"','"+this.app._userLog+"','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.cb_app.getText()+"','RRR','-','"+this.app._userLog+"',now(),'0')");
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								...
								sql.add("insert into anggaran_d (no_agg,kode_lokasi,no_urut,kode_pp,kode_akun,kode_drk,volume,periode,nilai_sat,nilai,dc,satuan,nik_user,tgl_input,modul) values "+		
										"('"+this.e_nb.getText()+"','"+this.cb_lokdonor.getText()+"',"+i+",'"+this.sg.cells(2,i)+"','"+this.sg.cells(0,i)+"','"+this.sg.cells(4,i)+"',1,'"+periode+"',"+parseNilai(this.sg.cells(8,i))+","+parseNilai(this.sg.cells(8,i))+",'C','-','"+this.app._userLog+"',getdate(),'RRA')");
							}
						}
					}
		...
		switch (event){
			...
			case "simpan" :					
				...
				for (var i=0;i < this.sg.getRowCount();i++){
					if (this.sg.rowValid(i)){												
						/*
						if (this.app._userLog != "570304" || this.app._userLog != "680007") {
							if (this.sg.cells(6,i) == "01" || this.sg.cells(6,i) == "02" || this.sg.cells(6,i) == "03") {
								system.alert(this,"Transaksi tidak valid.","TW1 sudah di-Close");
								return false;						
							}						
							if (this.sg.cells(6,i) == "04" || this.sg.cells(6,i) == "05" || this.sg.cells(6,i) == "06") {
								system.alert(this,"Transaksi tidak valid.","TW II sudah di-Close");
								return false;						
							}
							if (this.sg.cells(6,i) == "07" || this.sg.cells(6,i) == "08" || this.sg.cells(6,i) == "09") {
								system.alert(this,"Transaksi tidak valid.","TW III sudah di-Close");
								return false;						
							}
						}
						*/
	...
	doChangeCell: function(sender, col, row){
		...
		if (col == 0 || col == 2 || col == 4 || col == 6) {
			var data = this.dbLib.getDataProvider("select fn_cekaggBulan('"+this.sg.cells(2,row)+"','"+this.cb_lokdonor.getText()+"','"+this.sg.cells(0,row)+"','"+this.sg.cells(4,row)+"','"+this.e_periode.getText().substr(0,4)+this.sg.cells(6,row)+"') as gar ",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line = data.rs.rows[0];
				data = line.gar.split(";");
