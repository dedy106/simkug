window.app_saku2_transaksi_gar_fRRajuBPCC = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_gar_fRRajuBPCC.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_gar_fRRajuBPCC";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Reprogramming BPCC TW: Input", 0);	
		...
		this.c_jenis = new saiCB(this,{bound:[20,22,182,20],caption:"Jenis",items:["BP","CC"], readOnly:true,tag:2,change:[this,"doChange"]});
		...
		try {
			...
			this.c_jenis.setText("BP");
		}catch(e){
			systemAPI.alert(e);
		}
	}
};
window.app_saku2_transaksi_gar_fRRajuBPCC.extend(window.childForm);
window.app_saku2_transaksi_gar_fRRajuBPCC.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into anggaran_m (no_agg,kode_lokasi,no_dokumen,tanggal,keterangan,tahun,kode_curr,nilai,tgl_input,nik_user,posted,no_del,nik_buat,nik_setuju,jenis) values  "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.e_periode.getText().substr(0,4)+"','IDR',"+parseNilai(this.e_debet.getText())+",getdate(),'"+this.app._userLog+"','T','-','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','RRBPCC')");
					...
					sql.add("insert into rra_pdrk_m(no_pdrk,kode_lokasi,keterangan,kode_pp,kode_bidang,jenis_agg,tanggal,periode,nik_buat,nik_app1,nik_app2,nik_app3,sts_pdrk,justifikasi, nik_user, tgl_input,progress) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_periode.getText()+"','"+this.app._userLog+"','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.cb_app.getText()+"','RRRBPCC','-','"+this.app._userLog+"',getdate(),'1')");
	...
	doChange:function(sender){				
		if (sender == this.c_jenis && this.c_jenis.getText()!="") {
			...
			var sql = new server_util_arrayList(); 
			if (this.c_jenis.getText() == "BP") sql.add("select kode_akun, nama from masakun where kode_akun in ('1151010001','1151020001','1151030001','1151040001') and block='0' and kode_lokasi='"+this.app._lokasi+"'");
			else sql.add("select kode_akun, nama from masakun where kode_akun in ('5302010001','5302020001','5302030001','5302040001') and block='0' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doChangeCell: function(sender, col, row){
		...
		if (col == 0 || col == 2 || col == 4 || col == 6) {
			if (this.sg.cells(0,row) != "" && this.sg.cells(2,row) != "" && this.sg.cells(4,row) != "" && this.sg.cells(6,row) != "" ) {
				var data = this.dbLib.getDataProvider("select fn_cekaggBulan('"+this.sg.cells(2,row)+"','"+this.app._lokasi+"','"+this.sg.cells(0,row)+"','"+this.sg.cells(4,row)+"','"+this.e_periode.getText().substr(0,4)+this.sg.cells(6,row)+"') as gar ",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line = data.rs.rows[0];
					data = line.gar.split(";");
	...
	doEllipsClick: function(sender, col, row){
		try{			
			if (sender == this.sg) {
				if (col == 0){
					if (this.c_jenis.getText() == "BP") {
						this.standarLib.showListData(this, "Daftar Akun",sender,undefined, 
							"select kode_akun, nama from masakun where kode_akun in ('1151010001','1151020001','1151030001','1151040001') and block='0' and kode_lokasi='"+this.app._lokasi+"'",
							"select count(kode_akun) from masakun where kode_akun in ('1151010001','1151020001','1151030001','1151040001') and block='0' and kode_lokasi='"+this.app._lokasi+"'",
							["kode_akun","nama"],"and",["Kode","Nama"],false);				
					}
					else {
						this.standarLib.showListData(this, "Daftar Akun",sender,undefined, 
							"select kode_akun, nama from masakun where kode_akun in ('5302010001','5302020001','5302030001','5302040001') and block='0' and kode_lokasi='"+this.app._lokasi+"'",
							"select count(kode_akun) from masakun where kode_akun in ('5302010001','5302020001','5302030001','5302040001') and block='0' and kode_lokasi='"+this.app._lokasi+"'",
							["kode_akun","nama"],"and",["Kode","Nama"],false);				
					}				
				}
				if (col == 2){
			...
			if (sender == this.sg2) {
				...
				if (col == 4){										
					this.standarLib.showListData(this, "Daftar DRK",sender,undefined, 
													  "select kode_drk, nama from drk where kode_drk like '900%' and tahun = '"+this.e_periode.getText().substr(0,4)+"' and kode_lokasi='"+this.app._lokasi+"'",
													  "select count(kode_drk) from drk where kode_drk like '900%' and  tahun = '"+this.e_periode.getText().substr(0,4)+"' and kode_lokasi='"+this.app._lokasi+"'",
