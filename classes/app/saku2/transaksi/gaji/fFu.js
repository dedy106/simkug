window.app_saku2_transaksi_gaji_fFu = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_gaji_fFu.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_gaji_fFu";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Batas Parameter PPh21", 0);	
		
		uses("saiCBBL;saiEdit;saiGrid;sgNavigator");
		this.p1 = new panel(this,{bound:[10,23,700,450],caption:"Daftar Faktor Usia"});
		this.sg = new saiGrid(this.p1,{bound:[1,20,695,400],colCount:3,tag:0,
		            colTitle:["Batas Bawah","Batas Atas","Persentase"],
					colWidth:[[2,1,0],[100,100,100,220,80]],
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select bawah,atas,persen from hr_dplk_fu where kode_lokasi='"+this.app._lokasi+"' order by bawah",true);
...
window.app_saku2_transaksi_gaji_fFu.extend(window.childForm);
window.app_saku2_transaksi_gaji_fFu.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from hr_dplk_fu where kode_lokasi='"+this.app._lokasi+"'");			
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into hr_dplk_fu(bawah,atas,persen,kode_lokasi) values "+
