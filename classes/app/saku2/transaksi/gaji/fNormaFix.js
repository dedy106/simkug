window.app_saku2_transaksi_gaji_fNormaFix = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_gaji_fNormaFix.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_gaji_fNormaFix";
		...
		try {
			...
			this.cb_kode.setSQL("select nik, nama from hr_karyawan where kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["Kode","Nama"],"and","Data Karyawan",true);
...
window.app_saku2_transaksi_gaji_fNormaFix.extend(window.childForm);
window.app_saku2_transaksi_gaji_fNormaFix.implement({
		...
		try{
			if (this.cb_kode.getText() != ""){
				...
				var data = this.dbLib.getDataProvider(
				            ...
				            "from  hr_gaji_nik a inner join hr_gaji_param b on a.kode_param=b.kode_param and a.kode_lokasi=b.kode_lokasi where a.nik='"+this.cb_kode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
