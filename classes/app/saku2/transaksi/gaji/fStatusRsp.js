window.app_saku2_transaksi_gaji_fStatusRsp = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_gaji_fStatusRsp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_gaji_fStatusRsp";
		...
		this.sg1 = new saiGrid(this.p1,{bound:[0,20,595,350],tag:9,readOnly:true,colTitle: ["Kode","Nama","Status"]});		
...
window.app_saku2_transaksi_gaji_fStatusRsp.extend(window.childForm);
window.app_saku2_transaksi_gaji_fStatusRsp.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try{									
					...
					sql.add("insert into hr_status_sdm(sts_sdm,nama,kode_lokasi) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();					
					sql.add("update hr_status_sdm set nama = '"+this.e_nama.getText()+"' where sts_sdm = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
	...
	doTampilClick: function(sender){
		try{			
			var temp = this.dbLib.runSQL("select sts_sdm,nama from hr_status_sdm where kode_lokasi='"+this.app._lokasi+"' order by sts_sdm");
