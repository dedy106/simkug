window.app_saku2_transaksi_gaji_fHitungRegB = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_gaji_fHitungRegB.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_gaji_fHitungRegB";
...
window.app_saku2_transaksi_gaji_fHitungRegB.extend(window.childForm);
window.app_saku2_transaksi_gaji_fHitungRegB.implement({
		...
		switch (event){
			...
			case "hapus" :	
				...
				if (parseFloat(this.app._periode) > parseFloat(this.e_periode.getText())){
					...
					sql.add("delete from hr_gaji_tambah where no_gaji='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!="") {									
			this.e_nb.setSQL("select no_gaji, keterangan from hr_gaji_m where modul = 'NONREG' and periode='"+this.e_periode.getText()+"' and no_tak='-' and posted='F' and kode_lokasi='"+this.app._lokasi+"'",["no_gaji","keterangan"],false,["No Bukti","Deskripsi"],"and","Data Bukti",true);
		}
