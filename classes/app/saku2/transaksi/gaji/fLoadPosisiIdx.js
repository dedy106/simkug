window.app_saku2_transaksi_gaji_fLoadPosisiIdx = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_gaji_fLoadPosisiIdx.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku2_transaksi_gaji_fLoadPosisiIdx";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Index dan Tunjangan Posisi : Load", 0);	
		...
		this.pc1 = new pageControl(this,{bound:[20,20,900,370], childPage:["Data Index Posisi","Pesan Kesalahan"]});		
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:9,tag:0,			
				colTitle:["NIK","Idx Posisi","Idx Kinerja","Idx Unit",  "Idx KML","Idx CUTI","Idx THR","Idx PAKSER","Idx DIDIK"],
				colWidth:[[8,7,6,5,4,3,2,1,0],[80,80,80,80,80, 80,80,80,80]],
				colFormat:[[1,2,3,4,5,6,7,8],[cfNilai,cfNilai,cfNilai,cfNilai,cfNilai,cfNilai,cfNilai,cfNilai]],readOnly:true, defaultRow:1});		
...
window.app_saku2_transaksi_gaji_fLoadPosisiIdx.extend(window.portalui_childForm);
window.app_saku2_transaksi_gaji_fLoadPosisiIdx.implement({
	...
	selectPage: function(sender,page){
		...
		for (var i=start; i < finish;i++){
			...
			this.sg1.appendData([line.nik,floatToNilai(line.idx_posisi),floatToNilai(line.idx_kinerja),floatToNilai(line.idx_unit),floatToNilai(line.idx_kml),floatToNilai(line.idx_cuti),floatToNilai(line.idx_thr),floatToNilai(line.idx_pakser),floatToNilai(line.idx_didik)]);
		}		
					...
					if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
						try{
							...
							sql.add("insert into hr_gajiload_m(no_load, kode_lokasi, tanggal, periode, keterangan, tgl_input, nik_user, flag_form)"+
									" values('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_periode.getText()+"','"+this.e_ket.getText()+"',getdate(),'"+this.app._userLog+"','GAJIPOS')");
							var line;												
							for (var i in this.dataUpload.rows){
								line = this.dataUpload.rows[i];																																
								sql.add("insert into hr_gaji_pos(no_gaji,kode_lokasi,nik,idx_posisi,idx_kinerja,idx_unit,idx_kml,idx_cuti,idx_thr,idx_pakser,idx_didik) values "+
									    " ('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+line.nik+"',"+line.idx_posisi+","+line.idx_kinerja+","+line.idx_unit+","+line.idx_kml+","+line.idx_cuti+","+line.idx_thr+","+line.idx_pakser+","+line.idx_didik+")");
							}							
