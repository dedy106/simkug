window.app_saku2_transaksi_kpa_fPtgAju = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kpa_fPtgAju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kpa_fPtgAju";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Input Permohonan Pertanggungan: Input", 0);	
...
window.app_saku2_transaksi_kpa_fPtgAju.extend(window.childForm);
window.app_saku2_transaksi_kpa_fPtgAju.implement({
	...
	doChange:function(sender){
		if (sender == this.e_periode || sender == this.cb_pp) {
			if (this.e_periode.getText()!="" && this.cb_pp.getText()!="") {				
				this.cb_pj.setSQL("select no_panjar, keterangan from itt_panjar_m where kode_pp='"+this.cb_pp.getText()+"' and progress='2'  and periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_panjar","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
			}
		}
		if (sender == this.cb_pj && this.cb_pj.getText()!="") {
			var strSQL = "select a.nilai,a.akun_panjar from itt_panjar_m a where a.no_panjar='"+this.cb_pj.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
	...
	doClick:function(sender){
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"itt_panjarptg_m","no_ptg",this.app._lokasi+"-PTG"+this.e_periode.getText().substr(2,4)+".","0000"));
		this.e_dok.setFocus();
	},
