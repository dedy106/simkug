window.app_saku2_transaksi_kpa_fKbPanjar = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kpa_fKbPanjar.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kpa_fKbPanjar";
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:10,tag:0,
		            colTitle:["No Bukti","Tanggal","Tgl Pakai","No Dokumen","Nilai","Keterangan","Kode Akun","PP","Pemohon","No Verifikasi"],
					colWidth:[[9,8,7,6,5,4,3,2,1,0],[100,100,100,80,200,100,150,80,80,100]],
		...
		this.e_akunpj = new saiLabelEdit(this.pc1.childPage[1],{bound:[520,12,450,20],caption:"Akun Panjar", readOnly:true});						
		this.e_nopj = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,13,450,20],caption:"No Bukti", readOnly:true});						
...
window.app_saku2_transaksi_kpa_fKbPanjar.extend(window.childForm);
window.app_saku2_transaksi_kpa_fKbPanjar.implement({
		...
		try{		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("update itt_panjar_m set progress='2',no_kas='"+this.e_nb.getText()+"' where no_panjar='"+this.e_nopj.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','-','KBPJ','BK','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.app._userLog+"','"+this.nikapp+"',getdate(),'"+this.app._userLog+"','F','-','-','"+this.e_nopj.getText()+"','-')");										
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.e_akunpj.getText()+"','"+this.e_ket.getText()+"','D',"+parseNilai(this.e_debet.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBPJ','PJ','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");
					if (this.sg4.getRowValidCount() > 0){
						...
						for (var i=0;i < this.sg4.getRowCount();i++){
							if (this.sg4.rowValid(i)){
								j = j+1;
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sg4.cells(0,i)+"','"+this.sg4.cells(3,i)+"','"+this.sg4.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg4.cells(4,i))+",'"+this.sg4.cells(5,i)+"','-','"+this.sg4.cells(7,i)+"','-','"+this.app._lokasi+"','KBPJ','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");
							}
						}
					}
		...
		switch (event){
			...
			case "simpan" :			
				if (this.flagDokFree == "1") {		
					//alert("select no_kas from kas_m where no_dokumen='"+this.e_dok.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");				
	...
	doDoubleClick: function(sender, col , row) {
		if (this.sg.cells(0,row) != "") {			
			...
			var data = this.dbLib.getDataProvider(
						"select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk,'-' as catatan "+
						"from itt_panjar_j a inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
						"            	     inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
						"                    inner join itt_panjar_m x on a.no_panjar=x.no_panjar and a.kode_lokasi=x.kode_lokasi "+						
						"                    left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and d.tahun=substring(a.periode,1,4) "+
						"where a.no_panjar = '"+this.e_nopj.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg3.clear();
				for (var i in data.rs.rows){
	...
	doLoad:function(sender){		
		var strSQL = "select no_panjar,convert(varchar,a.tanggal,103) as tanggal,convert(varchar,a.due_date,103) as due_date,a.no_dokumen,a.nilai,a.keterangan,b.kode_pp+' - '+b.nama as pp,c.nik+' - '+c.nama as pemohon,a.no_ver,a.akun_panjar "+
		             "from itt_panjar_m a "+
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU.rs.rows[i];							
			this.sg.appendData([line.no_panjar,line.tanggal,line.due_date,line.no_dokumen,floatToNilai(line.nilai),line.keterangan,line.akun_panjar,line.pp,line.pemohon,line.no_ver]);
		}
