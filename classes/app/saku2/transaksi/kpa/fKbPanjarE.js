window.app_saku2_transaksi_kpa_fKbPanjarE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kpa_fKbPanjarE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kpa_fKbPanjarE";
		...
		this.cb_pj = new saiCBBL(this,{bound:[20,15,220,20],caption:"No Permohonan", readOnly:true, maxLength:10, tag:1});		
		this.e_debet = new saiLabelEdit(this,{bound:[780,15,220,20],caption:"Total Debet", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		this.e_akunpj = new saiLabelEdit(this,{bound:[20,17,200,20],caption:"Akun Panjar", readOnly:true});		
		this.e_kredit = new saiLabelEdit(this,{bound:[780,17,220,20],caption:"Total Kredit", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		
		this.pc1 = new pageControl(this,{bound:[20,12,980,367], childPage:["Data Item Permohonan","Item Jurnal KasBank"]});
...
window.app_saku2_transaksi_kpa_fKbPanjarE.extend(window.childForm);
window.app_saku2_transaksi_kpa_fKbPanjarE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("update itt_panjar_m set progress='1',no_kas='-' where no_panjar='"+this.cb_pj.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					
					sql.add("update itt_panjar_m set progress='2',no_kas='"+this.e_nb.getText()+"' where no_panjar='"+this.cb_pj.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','-','KBPJ','BK','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.app._userLog+"','"+this.nikapp+"',getdate(),'"+this.app._userLog+"','F','-','-','"+this.cb_pj.getText()+"','-')");					
					...
					if (this.sg.getRowValidCount() > 0){
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								j = j+1;
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','-','"+this.sg.cells(7,i)+"','-','"+this.app._lokasi+"','KBPJ','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");
							}
						}
					}										
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText() != "") {
				this.e_nb.setSQL("select no_kas, keterangan from kas_m where no_del='-' and modul = 'KBPJ' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kas","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
		}
		if (sender == this.e_nb && this.e_nb.getText() != "") {
			this.cb1.setSelected(true);		
			var data = this.dbLib.getDataProvider("select d.akun_panjar,d.no_panjar,d.keterangan as ket,a.no_dokumen,convert(varchar,a.tanggal,103) as tanggal,a.periode,a.keterangan "+
					   "from kas_m a inner join itt_panjar_m d on a.no_kas=d.no_kas and a.kode_lokasi=d.kode_lokasi "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){
					...
					this.cb_pj.setText(line.no_panjar,line.ket);
