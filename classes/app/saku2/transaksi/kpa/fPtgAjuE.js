window.app_saku2_transaksi_kpa_fPtgAjuE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_kpa_fPtgAjuE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_kpa_fPtgAjuE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Input Pertanggungan Panjar: Edit", 0);	
		...
		this.cb_pp = new saiCBBL(this,{bound:[20,16,205,20],caption:"PP", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});								
		...
		this.cb_pj = new saiCBBL(this,{bound:[20,17,220,20],caption:"Bukti Panjar", readOnly:true, tag:1});		
		...
		try {
			...
			this.cb_pp.setSQL("select kode_pp, nama from pp where kode_lokasi='"+this.app._lokasi+"' and tipe='posting' and flag_aktif ='1' and kode_lokasi='"+this.app._lokasi+"'",["kode_pp","nama"],false,["Kode","Nama"],"and","Data PP",true);
...
window.app_saku2_transaksi_kpa_fPtgAjuE.extend(window.childForm);
window.app_saku2_transaksi_kpa_fPtgAjuE.implement({
		...
		try{												
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from itt_panjarptg_m where no_ptg ='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from itt_panjarptg_j where no_ptg ='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from angg_r where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul ='PJPTG'");					
					sql.add("delete from angg_r where no_bukti = '"+this.cb_pj.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul ='PJPTG'");					
					sql.add("update itt_panjar_m set progress = '2' where no_panjar='"+this.cb_pj.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
					
					sql.add("update itt_panjar_m set progress = '3' where no_panjar='"+this.cb_pj.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
					sql.add("insert into itt_panjarptg_m (no_ptg,no_panjar,no_km,no_kk,no_dokumen,tanggal,keterangan,akun_panjar,akun_km,akun_kk,nik_buat,nik_app,kode_lokasi,kode_pp,nilai_pj,nilai,nilai_kas,progress,periode,nik_user,tgl_input) values "+
						    "('"+this.e_nb.getText()+"','"+this.cb_pj.getText()+"','-','-','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.e_akunpj.getText()+"','-','-','"+this.cb_buat.getText()+"','"+this.cb_setuju.getText()+"','"+this.app._lokasi+"','"+this.cb_pp.getText()+"',"+parseNilai(this.e_nilaipj.getText())+","+parseNilai(this.e_total.getText())+","+parseNilai(this.e_sls.getText())+",'1','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
										
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){								
								sql.add("insert into itt_panjarptg_j(no_ptg,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
					...
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) "+
							"select no_bukti,'PJPTG',kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,'"+this.e_periode.getText()+"',case dc when 'D' then 'C' else 'D' end,0,nilai from angg_r where no_bukti='"+this.cb_pj.getText()+"' and modul='PJ'");
		...
		switch (event){
			...
			case "ubah" :		
				...
				if (this.flagDokFree == "1") {				
					var data = this.dbLib.getDataProvider("select no_ptg from itt_panjarptg_m where no_dokumen='"+this.e_dok.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
						if (line != undefined){
	...
	doChange:function(sender){
		if (sender == this.e_periode || sender == this.cb_pp) {
			if (this.e_periode.getText()!="" && this.cb_pp.getText()!="") {				
				this.e_nb.setSQL("select no_ptg, keterangan from itt_panjarptg_m where kode_pp='"+this.cb_pp.getText()+"' and progress='1'  and periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_ptg","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);				
			}
		}
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			this.cb1.setSelected(true);		
			var strSQL = "select a.nilai,a.akun_panjar,a.no_panjar,a.keterangan, "+
						 " b.no_dokumen,b.keterangan as ket,b.nik_buat,c.nama as nama_buat,b.nik_app,d.nama as nama_app,b.tanggal  "+
			             "from itt_panjar_m a inner join itt_panjarptg_m b on a.no_panjar=b.no_panjar and a.kode_lokasi=b.kode_lokasi "+
						 "                    inner join karyawan c on b.nik_buat=c.nik and b.kode_lokasi=c.kode_lokasi "+
