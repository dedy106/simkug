window.app_saku2_transaksi_siaga_fHutUmumE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fHutUmumE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fHutUmumE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Hutang Umum: Edit", 0);	
		...
		this.cb_pp = new saiCBBL(this,{bound:[20,16,200,20],caption:"PP", multiSelection:false, maxLength:10, tag:2,readOnly:true});								
		...
		this.cb_vendor = new saiCBBL(this,{bound:[20,19,200,20],caption:"Vendor", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});							
		...
		this.pc1 = new pageControl(this,{bound:[20,12,980,270], childPage:["Data Item Jurnal","Data Anggaran"]});
...
window.app_saku2_transaksi_siaga_fHutUmumE.extend(window.childForm);
window.app_saku2_transaksi_siaga_fHutUmumE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from gr_hutang_m where no_hutang = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from gr_hutang_j where no_hutang = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from angg_r where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='HUTUM'");
					
					sql.add("insert into gr_hutang_m(no_hutang,kode_lokasi,no_dokumen,tanggal,keterangan,kode_vendor,kode_curr,kurs,nik_buat,nik_app,kode_pp,nilai_curr,nilai,periode,nik_user,tgl_input,akun_hutang,posted,nilai_ppn) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_vendor.getText()+"','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.cb_pp.getText()+"',"+parseNilai(this.e_nilaiCurr.getText())+","+parseNilai(this.e_nilai.getText())+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.cb_akun.getText()+"','F',"+this.nilaiPPN+")");
					
					sql.add("insert into gr_hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','C','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+","+parseNilai(this.e_nilaiCurr.getText())+","+parseNilai(this.e_nilai.getText())+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','HUTANG','HUT','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){															
								var j = i+1;
								sql.add("insert into gr_hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i)+"','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+","+parseNilai(this.sg.cells(4,i))+","+parseNilai(this.sg.cells(5,i))+",'"+this.sg.cells(6,i)+"','"+this.sg.cells(8,i)+"','"+this.app._lokasi+"','HUTANG','BBN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");								
							}
						}
					}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			case "clear" :
				...
				break;
			case "ubah" :						
				var data = this.dbLib.getDataProvider("select kode_akun from flag_relasi where kode_flag = '029' and kode_lokasi='"+this.app._lokasi+"'",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					this.dataJU = data;
				}
				var k=0;
	...
	doChange:function(sender){		
		if (sender == this.e_periode && this.e_periode.getText()!="") {	
			...
			this.e_nb.setSQL("select a.no_hutang, a.keterangan from gr_hutang_m a "+
			                 "       left join (select no_hutang,kode_lokasi,sum(nilai_kas+nilai_lain) as bayar from gr_hutangbayar_d group by no_hutang,kode_lokasi) b on a.no_hutang=b.no_hutang and a.kode_lokasi=b.kode_lokasi "+
			                 "where isnull(b.bayar,0) = 0 and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",["a.no_hutang","a.keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);					
		}	
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			this.cb1.setSelected(true);			
			var data = this.dbLib.getDataProvider("select a.tanggal,a.periode,a.no_dokumen,a.keterangan,a.nik_buat,b.nama as nama_buat,a.kode_pp,c.nama as nama_pp,a.nik_app,d.nama as nama_app,a.akun_hutang,f.nama as nama_akun,a.kode_vendor,g.nama as nama_vendor,a.kode_curr,a.kurs "+
			           "from gr_hutang_m a "+
			           "	inner join karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+
			           "	inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
					   "	inner join karyawan d on a.nik_app=d.nik and a.kode_lokasi=d.kode_lokasi "+
					   "    inner join masakun f on a.akun_hutang=f.kode_akun and a.kode_lokasi=f.kode_lokasi "+					   
					   "    inner join gr_vendor g on a.kode_vendor=g.kode_vendor and a.kode_lokasi=g.kode_lokasi "+					   
			...
			if (typeof data == "object"){
				...
				if (line != undefined){
					...
					this.cb_akun.setText(line.akun_hutang,line.nama_akun);		
			...
			var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.nilai_curr,a.kode_pp,a.kode_drk,c.nama as nama_pp,isnull(d.nama,'-') as nama_drk "+
			...
			var data = this.dbLib.getDataProvider(
						...
						"where a.no_bukti = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='HUTUM' order by a.kode_akun",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg2.clear();
				for (var i in data.rs.rows){
	    			...
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.cb1.isSelected()) {								
								this.nama_report="server_report_saku2_siaga_rptHutangJurnal";
