window.app_saku2_transaksi_siaga_fKbPiutangE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fKbPiutangE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fKbPiutangE";
		...
		this.pc1 = new pageControl(this,{bound:[20,12,980,280], childPage:["Data Pelunasan","Data Jurnal Tambahan","Data Anggaran","NoBukti Baru"]});				
...
window.app_saku2_transaksi_siaga_fKbPiutangE.extend(window.childForm);
window.app_saku2_transaksi_siaga_fKbPiutangE.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.nb+"','"+this.app._lokasi+"','"+this.cb_inv.getText()+"','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_cabang.getText()+"','KBPIUTANG','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+","+parseNilai(this.e_nilai.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_buat.getText()+"',getdate(),'"+this.app._userLog+"','F','-','"+this.cb_inv.getText()+"','"+this.cb_cust.rightLabelCaption+"','"+this.cb_bank.getText()+"')");																				
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.nb+"','"+this.cb_inv.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_nilaiIDR.getText())+",'"+this.app._kodePP+"','-','-','"+this.cb_cust.getText()+"','"+this.app._lokasi+"','KBPIUTANG','KB','"+this.e_periode.getText()+"','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+",'"+this.app._userLog+"',getdate(),'"+this.cb_bank.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+")");										
					var totAR = Math.round(nilaiToFloat(this.e_total.getText()) * nilaiToFloat(this.e_kursar.getText()));
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.nb+"','"+this.cb_inv.getText()+"','"+this.dp_d1.getDateString()+"',1,'"+this.akunAR+"','"+this.e_ket.getText()+"','C',"+totAR+",'"+this.app._kodePP+"','-','-','"+this.cb_cust.getText()+"','"+this.app._lokasi+"','KBPIUTANG','AR','"+this.e_periode.getText()+"','"+this.c_curr.getText()+"',"+parseNilai(this.e_kursar.getText())+",'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.e_total.getText())+")");									
					if (this.sg.getRowValidCount() > 0){
					 	for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){	
								var j = i+99;
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
										"('"+this.nb+"','"+this.cb_inv.getText()+"','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg.cells(5,i))+",'"+this.sg.cells(6,i)+"','"+this.sg.cells(8,i)+"','-','"+this.cb_cust.getText()+"','"+this.app._lokasi+"','KBPIUTANG','BEBAN','"+this.e_periode.getText()+"','"+this.c_curr.getText()+"',"+parseNilai(this.e_kursar.getText())+",'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.sg.cells(4,i))+")");
							}
						}
					}					
					if (nilaiToFloat(this.e_sls.getText()) > 0) {						
						sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.nb+"','"+this.cb_inv.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.akunLSK+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_sls.getText())+",'"+this.app._kodePP+"','-','-','"+this.cb_cust.getText()+"','"+this.app._lokasi+"','KBPIUTANG','SKURS','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.e_sls.getText())+")");				
					}
					if (nilaiToFloat(this.e_sls.getText()) < 0) {						
						sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.nb+"','"+this.cb_inv.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.akunRSK+"','"+this.e_ket.getText()+"','D',"+Math.abs(nilaiToFloat(this.e_sls.getText()))+",'"+this.app._kodePP+"','-','-','"+this.cb_cust.getText()+"','"+this.app._lokasi+"','KBPIUTANG','SKURS','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.e_sls.getText())+")");				
					}										
					...
					sql.add("insert into gr_piutangbayar_d(no_bukti,kode_lokasi,no_piutang,akun_piutang,nilai_kas,nilai_lain,nilai_sk,periode,dc,modul,kode_curr,kurs) values "+
							"('"+this.nb+"','"+this.app._lokasi+"','"+this.cb_inv.getText()+"','"+this.akunAR+"',"+parseNilai(this.e_nilai.getText())+","+nilaiLain+","+parseNilai(this.e_sls.getText())+",'"+this.e_periode.getText()+"','D','KB','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+")");
	...
	doChange:function(sender){
		if (sender == this.e_periode || sender == this.cb_cabang) {			
			if (this.e_periode.getText()!="" && this.cb_cabang.getText()!="") {
				this.e_nb.setSQL("select no_kas, keterangan from kas_m where kode_pp ='"+this.cb_cabang.getText()+"' and modul = 'KBPIUTANG' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kas","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
			}			
		}		
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			this.cb1.setSelected(true);			
			var data = this.dbLib.getDataProvider("select a.tanggal,a.periode,a.keterangan,a.jenis,a.nik_buat,b.nama as nama_buat,a.kode_pp,c.nama as nama_cab,a.kode_bank,d.nama as nama_bank,a.akun_kb,f.nama as nama_akun,"+
			           "e.no_piutang,g.keterangan as ket_inv,e.nilai_kas,h.kode_cust,h.nama as nama_cust, "+
					   ...
					   "    inner join gr_piutangbayar_d e on a.no_kas=e.no_bukti and a.kode_lokasi=e.kode_lokasi "+
		...
		if (sender == this.e_nilai && this.e_nilai.getText()!="") {
			...
			if (nilaiToFloat(this.e_nilai.getText()) > nilaiToFloat(this.e_saldo.getText())) {
				...
				var OIidr = Math.abs((nilaiToFloat(this.e_nilai.getText()) - nilaiToFloat(this.e_saldo.getText())) * nilaiToFloat(this.e_kurs.getText()));
				this.sg.appendData([this.akunOI,this.namaOI,"C","Pendapatan Lain-Lain",floatToNilai(OI),floatToNilai(OIidr),this.app._kodePP,this.app._namaPP,"-","-"]);				
			} else this.sg.clear(1);
			this.sg.validasi();				
		}								
	},	
	doChangeCell: function(sender, col, row){
		if (col == 2 || col == 4) {			
			if (this.sg.cells(2,row) != "" && this.sg.cells(4,row) != "" && this.e_kursar.getText() != "") {				
				this.sg.cells(5,row,Math.round(nilaiToFloat(this.e_kursar.getText()) * nilaiToFloat(this.sg.cells(4,row))));
				this.sg.validasi();			
			}
		}
	...
	doNilaiChange: function(){
		try{
			...
			var totIDR = nilaiIDR + Math.round(totD * nilaiToFloat(this.e_kursar.getText()));
			this.e_nilaiIDR.setText(floatToNilai(nilaiIDR));
			this.e_totalIDR.setText(floatToNilai(totIDR));
			var nilaiIDRLama = Math.round(nilaiToFloat(this.e_nilai.getText()) * nilaiToFloat(this.e_kursar.getText()));
