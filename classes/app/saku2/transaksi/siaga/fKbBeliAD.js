window.app_saku2_transaksi_siaga_fKbBeliAD = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fKbBeliAD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fKbBeliAD";
		...
		this.c_jenis = new saiCB(this,{bound:[20,12,202,20],caption:"Jenis",items:["CK","BK"], readOnly:true,tag:2,change:[this,"doChange"]});
		...
		this.e_nilai = new saiLabelEdit(this,{bound:[700,16,220,20],caption:"Total", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});
		this.e_ket = new saiLabelEdit(this,{bound:[20,13,450,20],caption:"Keterangan", maxLength:150});						
		this.e_ppn = new saiLabelEdit(this,{bound:[700,13,220,20],caption:"PPN", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});
		...
		this.pc1 = new pageControl(this,{bound:[20,20,900,320], childPage:["Data Pembelian","Detail Pembelian"]});				
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:4,tag:0,
		            colTitle:["Status","ID","Deskripsi","Nilai"],
		...
		this.sg2 = new saiGrid(this.pc1.childPage[1],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:4,tag:9,
		...
		try {
			...
			this.flagDokFree = "0"; 
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('DOKFREE','BRGINV','PPNM') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku2_transaksi_siaga_fKbBeliAD.extend(window.childForm);
window.app_saku2_transaksi_siaga_fKbBeliAD.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_cabang.getText()+"','KBBELIAD','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_total.getText())+",'"+this.app._userLog+"','"+this.cb_buat.getText()+"',getdate(),'"+this.app._userLog+"','F','-','-','-','"+this.cb_bank.getText()+"')");
										
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.akunBarang+"','"+this.e_ket.getText()+"','D',"+parseNilai(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBBELIAD','BARANG','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+parseNilai(this.e_nilai.getText())+")");					
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',1,'"+this.akunPPN+"','"+this.e_ket.getText()+"','D',"+parseNilai(this.e_ppn.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBBELIAD','PPNM','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+parseNilai(this.e_ppn.getText())+")");												
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',99,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_total.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBBELIAD','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'"+this.cb_bank.getText()+"',"+parseNilai(this.e_total.getText())+")");					
					
					var line;
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						line = this.dataJU.rs.rows[i];
						if (line.status.toUpperCase() == "APP"){
							sql.add("update doa_tb set no_jurnal='"+this.e_nb.getText()+"' where id ='"+line.id+"'");							
						}
					}														
	...
	doLoad:function(sender){
		if (this.e_periode.getText()!="") {
			var strSQL = "select 'INPROG' as status,b.id,b.name,sum(cast(qty as float) * cast(price as float)) as nilai "+
						 "from doaitems_tb a inner join doa_tb b on a.id = b.id "+
						 "where b.no_jurnal ='-' group by b.id,b.name";									
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU.rs.rows[i];										
			this.sg.appendData([line.status.toUpperCase(),line.id,line.name,floatToNilai(line.nilai)]);
		}
	...
	doChangeCells: function(sender, col , row) {
		if (col == 0) {
			...
			this.e_total.setText(floatToNilai(Math.round(tot*1.1)));
		}
	},	
	doDoubleClick: function(sender, col , row) {	
		if (this.sg.cells(1,row) != "") {
			var strSQL = "select * from doaitems_tb where id ='"+this.sg.cells(1,row)+"'";
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg2.clear();
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg2.appendData([line.id,line.pid,floatToNilai(line.qty),floatToNilai(line.price)]);
				}
