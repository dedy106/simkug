window.app_saku2_transaksi_siaga_fKbPanjar = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fKbPanjar.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fKbPanjar";
		...
		this.cb_panjar = new saiCBBL(this,{bound:[20,13,240,20],caption:"Bukti Permohonan", multiSelection:false, maxLength:10, tag:0,change:[this,"doChange"]});					
		...
		this.pc1 = new pageControl(this,{bound:[20,12,980,235], childPage:["Data Item Jurnal","Item Permintaan"]});
...
window.app_saku2_transaksi_siaga_fKbPanjar.extend(window.childForm);
window.app_saku2_transaksi_siaga_fKbPanjar.implement({
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!="") {
			this.cb_panjar.setSQL("select no_panjar, keterangan from gr_panjar_m where progress='0' and no_kas='-' and periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_panjar","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
		}
		if (sender == this.c_jenis || sender == this.cb_cabang || sender == this.cb_bank) {
			this.doClick();				
		}								
		if (sender == this.cb_panjar && this.cb_panjar.getText()!="") {
			var data = this.dbLib.getDataProvider("select a.nik_buat+'-'+b.nama as atensi,a.keterangan,a.kode_curr,a.kurs,a.nilai_curr,convert(varchar,a.due_date,103) as due_date,a.akun_panjar  "+
			           "from gr_panjar_m a inner join karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi where a.no_panjar='"+this.cb_panjar.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){
		...
		if (sender == this.e_kurs || sender == this.e_nilaiCurr) {
			this.sg.validasi();
		}
	},
			...
			case 3 : 
					if (this.sg.cells(3,row) == ""){
						if (row == 0)
						{
							...
							this.sg.setCell(4,row,this.e_nilaiCurr.getText());
			...
			if (sender == this.sg) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Akun",sender,undefined, 
												  "select a.kode_akun,a.nama  from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi where b.kode_flag in ('001','009') and a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'",
												  "select count(a.kode_akun)  from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi where b.kode_flag in ('001','009') and a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'",
