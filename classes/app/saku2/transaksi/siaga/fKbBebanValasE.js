window.app_saku2_transaksi_siaga_fKbBebanValasE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fKbBebanValasE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fKbBebanValasE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Permintaan Bayar: Edit", 0);	
		...
		this.cb_beban = new saiCBBL(this,{bound:[20,13,240,20],caption:"Bukti Permintaan", readOnly:true, multiSelection:false, maxLength:10, tag:0,change:[this,"doChange"]});					
		...
		this.e_debet = new saiLabelEdit(this,{bound:[790,19,210,20],caption:"Total Debet (IDR)", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});								
		this.e_nilaiCurr = new saiLabelEdit(this,{bound:[20,18,220,20],caption:"Nilai Curr", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		this.e_nilaiKB = new saiLabelEdit(this,{bound:[250,18,220,20],caption:"Nilai KBCurr", tag:1, tipeText:ttNilai, text:"0",change:[this,"doChange"]});		
		...
		this.cb_akun = new saiCBBL(this,{bound:[20,20,240,20],caption:"Akun KasBank", multiSelection:false, maxLength:10, tag:0});					
		this.e_sls = new saiLabelEdit(this,{bound:[790,20,210,20],caption:"Selisih (IDR)", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		
		this.pc1 = new pageControl(this,{bound:[20,12,980,223], childPage:["Item Permintaan","Data Item Jurnal","Data Anggaran","NoBukti Baru"]});
		this.sg3 = new saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:10,tag:9,
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:9,tag:9,
		            colTitle:["Kode Akun","Nama Akun","DC","Keterangan","Nilai IDR","Kode PP","Nama PP","DRK","Nama DRK"],
		...
		this.c_jenis2 = new saiCB(this.pc1.childPage[3],{bound:[20,12,200,20],caption:"Jenis",items:["CK","BK"], readOnly:true,tag:4,change:[this,"doChange2"]});
...
window.app_saku2_transaksi_siaga_fKbBebanValasE.extend(window.childForm);
window.app_saku2_transaksi_siaga_fKbBebanValasE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("update gr_beban_m set progress='0',no_kas='-' where no_beban='"+this.cb_beban.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add("update gr_beban_m set progress='1', no_kas='"+this.nb+"' where no_beban='"+this.cb_beban.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.nb+"','"+this.app._lokasi+"','-','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_cabang.getText()+"','KBBEBAN','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','"+this.c_curr.getText()+"','"+parseNilai(this.e_kurs.getText())+"',"+parseNilai(this.e_nilaiKB.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_buat.getText()+"',getdate(),'"+this.app._userLog+"','F','-','"+this.cb_beban.getText()+"','"+this.e_atensi.getText()+"','"+this.cb_bank.getText()+"')");
					
					var nilaiIDR = Math.round(nilaiToFloat(this.e_nilaiKB.getText()) * nilaiToFloat(this.e_kurs.getText()));
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.nb+"','-','"+this.dp_d1.getDateString()+"',999,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','C',"+nilaiIDR+",'"+this.app._kodePP+"','-','-','"+this.e_atensi.getText()+"','"+this.app._lokasi+"','KBBEBAN','KB','"+this.e_periode.getText()+"','"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+",'"+this.app._userLog+"',getdate(),'-',"+parseNilai(this.e_nilaiKB.getText())+")");
										
					if (this.sg3.getRowValidCount() > 0){
						for (var i=0;i < this.sg3.getRowCount();i++){
							if (this.sg3.rowValid(i)){							
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
										"('"+this.nb+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg3.cells(0,i)+"','"+this.sg3.cells(3,i)+"','"+this.sg3.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg3.cells(5,i))+",'"+this.sg3.cells(6,i)+"','"+this.sg3.cells(8,i)+"','-','"+this.e_atensi.getText()+"','"+this.app._lokasi+"','KBBEBAN','BEBAN','"+this.e_periode.getText()+"','"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+",'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.sg3.cells(4,i))+")");
							}
						}
					}
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){							
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
										"('"+this.nb+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','-','"+this.e_atensi.getText()+"','"+this.app._lokasi+"','KBBEBAN','TAMBAH','"+this.e_periode.getText()+"','"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+",'"+this.app._userLog+"',getdate(),'"+this.cb_bank.getText()+"',"+nilaiToFloat(this.sg.cells(4,i))+")");
							}
						}
					}					
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					this.sg.clear(1); this.sg2.clear(1); this.sg.clear(3);
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!="") {			
			this.e_nb.setSQL("select no_kas, keterangan from kas_m where modul = 'KBBEBAN' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kas","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
		}	
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			this.cb1.setSelected(true);						
			var data = this.dbLib.getDataProvider("select a.no_beban,a.atensi,a.keterangan,a.kode_curr,a.kurs,a.nilai_curr  "+
			           "from gr_beban_m a where a.no_kas='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){
					this.cb_beban.setText(line.no_beban,line.keterangan);
			...
			var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.nilai_curr,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk  "+
						...
						"where a.no_beban = '"+this.cb_beban.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg3.clear();
				for (var i in data.rs.rows){
			...
			this.sg3.validasi();			
			var data = this.dbLib.getDataProvider("select a.akun_kb,e.nama as nama_akun,a.nilai,a.tanggal,a.periode,a.keterangan,a.jenis,a.nik_buat,b.nama as nama_buat,a.kode_pp,c.nama as nama_cab,a.kode_bank,d.nama as nama_bank,a.kode_curr,a.kurs,a.ref1 "+
			           "from kas_m a "+
					   ...
					   "	inner join masakun e on a.akun_kb=e.kode_akun and a.kode_lokasi=e.kode_lokasi "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){
					...
					this.cb_akun.setSQL("select a.kode_akun, a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
										"where a.kode_curr='"+this.c_curr.getText()+"' and b.kode_flag in ('001','009') and a.kode_lokasi='"+this.app._lokasi+"'",["a.kode_akun","a.nama"],false,["Kode","Nama"],"and","Data Akun KasBank",true);			
				} 
			}		
			var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.nilai_curr,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk  "+
		...
		if (sender == this.e_kurs || sender == this.e_nilaiKB) {
			this.sg.validasi();
		}
	},	
			...
			if (this.e_kurs.getText()!=0) {
				...
				for (var i = 0; i < this.sg3.rows.getLength();i++){
					if (this.sg3.rowValid(i) && this.sg3.cells(4,i) != "" && this.e_kurs.getText() != ""){
						//tidak dikonversi ke kurs baru supaya sama dgn anggaran, selisih kurs jurnal manual
				...
				if (this.e_nilaiKB.getText() != 0) 
					totC += Math.round(nilaiToFloat(this.e_nilaiKB.getText()) * nilaiToFloat(this.e_kurs.getText()));
