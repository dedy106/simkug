window.app_saku2_transaksi_siaga_fClosingThn = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fClosingThn.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fClosingThn";
...
window.app_saku2_transaksi_siaga_fClosingThn.extend(window.childForm);
window.app_saku2_transaksi_siaga_fClosingThn.implement({
	...
	isPostedAll: function(periode){
		...
		var sqlPTG = "select count(x.no_bukti) from ("+
					 ...
					 "  union "+
					 "  select no_selesai as no_bukti from gr_panjarselesai_m where posted = 'F' and periode ='"+periode+"' and kode_lokasi='"+this.app._lokasi+"' "+
				     ") x";					   
		...
		if (ck != "") { 
			...
			if (data[1] != "0"){
				result = result && false;
				msg+= "Data Modul PTG [PJ,GRPTG] masih ada yang belum diposting.("+data[1]+" data)\n";
			}
		}
		...
		var sqlAR = "select count(x.no_bukti) from ("+
					...
					"  union "+
				    "  select no_valid as no_bukti from yk_valid_m where posted = 'F' and periode ='"+periode+"' and kode_lokasi='"+this.app._lokasi+"' and modul <> 'VALID' "+
					"  union "+
				    "  select no_ar as no_bukti from patra_ar_m where posted = 'F' and periode ='"+periode+"' and kode_lokasi='"+this.app._lokasi+"' "+
				    "  union "+
				    "  select no_piutang as no_bukti from gr_piutang_m where posted = 'F' and periode ='"+periode+"' and kode_lokasi='"+this.app._lokasi+"' "+
					"  union "+
				    "  select no_tecc as no_bukti from gr_tecc_m where posted = 'F' and periode ='"+periode+"' and kode_lokasi='"+this.app._lokasi+"' "+
					") x";					   
		...
		if (ck != "") { 
			...
			if (data[1] != "0"){
				result = result && false;
				msg+= "Data Modul PIUTANG [BILL,AMOR,REKON,BTL,SISIH,ARYKS,ARPATRA,GRPIUTANG,GRTECC] masih ada yang belum diposting.("+data[1]+" data)\n";
			}
		}
		var sqlAP = "select count(x.no_bukti) from ("+
				    "  select no_hutang as no_bukti from yk_hutang_m where posted = 'F' and periode ='"+periode+"' and kode_lokasi='"+this.app._lokasi+"' "+
				    "  union "+
				    "  select no_rekon as no_bukti from yk_rekon_m where posted = 'F' and periode ='"+periode+"' and kode_lokasi='"+this.app._lokasi+"' "+
					"  union "+
				    "  select no_spb as no_bukti from spb_m where progress <> '0' and posted = 'F' and periode ='"+periode+"' and kode_lokasi='"+this.app._lokasi+"' "+
					"  union "+
				    "  select no_ppn as no_bukti from ras_ppn_m where posted = 'F' and periode ='"+periode+"' and kode_lokasi='"+this.app._lokasi+"' "+
				    "  union "+
				    "  select no_hutang as no_bukti from gr_hutang_m where posted = 'F' and periode ='"+periode+"' and kode_lokasi='"+this.app._lokasi+"' "+
					") x";					   
		...
		if (ck != "") { 
			...
			if (data[1] != "0"){
				result = result && false;
				msg+= "Data Modul AP [HUTKES,REKKES,GRHUT] masih ada yang belum diposting.("+data[1]+" data)\n";
			}
		}
...
//AREA 1 CEK DATA ANGGARAN DAN GL UNTUK BPCC