window.app_saku2_transaksi_siaga_fFasusut = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fFasusut.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fFasusut";
		...
		this.p1 = new panel(this,{bound:[20,23,840,333],caption:"Data Jurnal Penyusutan"});
...
window.app_saku2_transaksi_siaga_fFasusut.extend(window.childForm);
window.app_saku2_transaksi_siaga_fFasusut.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into fasusut_m(no_fasusut,no_dokumen,tanggal,keterangan,kode_curr,kurs,nilai,kode_pp,kode_drk,posted,modul,nik_buat,nik_setuju,kode_lokasi,periode,no_del,no_link,nik_user,tgl_input) values "+
						    "('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'-','"+this.cb_drk.getText()+"','F','FA','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','-','-','"+this.app._userLog+"',getdate())");					
					...
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						...
						sql.add("insert into fasusut_j(no_fasusut,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
								"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+line.akun_bp+"','"+this.e_ket.getText()+"','D',"+line.nilai_susut+",'"+line.kode_pp_susut+"','"+this.cb_drk.getText()+"','"+this.app._lokasi+"','FA','BP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");
						sql.add("insert into fasusut_j(no_fasusut,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
								"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',1,'"+line.akun_deprs+"','"+this.e_ket.getText()+"','C',"+line.nilai_susut+",'"+line.kode_pp_susut+"','"+this.cb_drk.getText()+"','"+this.app._lokasi+"','FA','AP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");
					}
					...
					sql.add("call sp_fa_susut ('"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+perNext+"','"+this.e_nb.getText()+"','"+this.cb_drk.getText()+"')");
