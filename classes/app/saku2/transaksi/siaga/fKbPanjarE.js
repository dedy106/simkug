window.app_saku2_transaksi_siaga_fKbPanjarE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fKbPanjarE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fKbPanjarE";
		...
		this.cb_panjar = new saiCBBL(this,{bound:[20,13,240,20],caption:"Bukti Permohonan", readOnly:true});
		this.e_atensi = new saiLabelEdit(this,{bound:[20,14,300,20],caption:"Atensi", readOnly:true});						
		this.e_tglpakai = new saiLabelEdit(this,{bound:[330,14,140,20],labelWidth:60,caption:"Tgl Pakai", readOnly:true});
		...
		this.e_akunpj = new saiLabelEdit(this,{bound:[330,19,140,20],caption:"Akun Panjar", labelWidth:60, tag:0, readOnly:true, text:""});				
		...
		this.e_nilaiCurr = new saiLabelEdit(this,{bound:[20,20,220,20],caption:"Nilai Panjar", tag:1, readOnly:true, tipeText:ttNilai, change:[this,"doChange"],text:"0"});		
		this.e_sls = new saiLabelEdit(this,{bound:[790,20,210,20],caption:"Selisih (IDR)", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		
		this.pc1 = new pageControl(this,{bound:[20,12,980,235], childPage:["Data Item Jurnal","Item Permintaan","NoBukti Baru"]});
		...
		this.sg3 = new saiGrid(this.pc1.childPage[1],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:10,tag:9,
		...
		//this.e_cabang2 = new saiLabelEdit(this.pc1.childPage[2],{bound:[20,13,180,20],caption:"Cabang", multiSelection:false, text:"",tag:4,change:[this,"doChange2"]});						
		this.e_cabang2 = new saiCBBL(this.pc1.childPage[2],{bound:[20,13,200,20],caption:"Cabang", multiSelection:false, maxLength:10, tag:4,change:[this,"doChange2"]});			
		...
		try {
			...
			this.e_cabang2.setSQL("select kode_cabang, nama from gr_cabang where kode_lokasi='"+this.app._lokasi+"'",["kode_cabang","nama"],false,["Kode","Nama"],"and","Data Cabang",true);
...
window.app_saku2_transaksi_siaga_fKbPanjarE.extend(window.childForm);
window.app_saku2_transaksi_siaga_fKbPanjarE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("update gr_panjar_m set progress='0', no_kas='-' where no_panjar='"+this.cb_panjar.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.nb+"','"+this.app._lokasi+"','-','-','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_cabang.getText()+"','KBPJ','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','"+this.c_curr.getText()+"','"+parseNilai(this.e_kurs.getText())+"',"+this.nilaiKB+",'"+this.cb_buat.getText()+"','"+this.cb_buat.getText()+"',getdate(),'"+this.app._userLog+"','F','-','"+this.cb_panjar.getText()+"','"+this.e_atensi.getText()+"','"+this.cb_bank.getText()+"')");
					sql.add("update gr_panjar_m set progress='1', no_kas='"+this.nb+"' where no_panjar='"+this.cb_panjar.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");										
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){							
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
										"('"+this.nb+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg.cells(5,i))+",'"+this.sg.cells(6,i)+"','-','-','"+this.e_atensi.getText()+"','"+this.app._lokasi+"','KBPJ','KBPJ','"+this.e_periode.getText()+"','"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+",'"+this.app._userLog+"',getdate(),'"+this.cb_bank.getText()+"',"+nilaiToFloat(this.sg.cells(4,i))+")");
							}
						}
					}
					var nilai = nilaiToFloat(this.e_nilaiCurr.getText()) * nilaiToFloat(this.e_kurs.getText());
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.nb+"','-','"+this.dp_d1.getDateString()+"',99,'"+this.e_akunpj.getText()+"','"+this.e_ket.getText()+"','D',"+nilai+",'"+this.app._kodePP+"','-','-','"+this.e_atensi.getText()+"','"+this.app._lokasi+"','KBPJ','PJ','"+this.e_periode.getText()+"','"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+",'"+this.app._userLog+"',getdate(),'"+this.cb_bank.getText()+"',"+nilaiToFloat(this.e_nilaiCurr.getText())+")");
		...
		switch (event){
			...
			case "ubah" :						
				...
				if (nilaiToFloat(this.e_nilaiCurr.getText()) != this.totCurr) {
					system.alert(this,"Transaksi tidak valid.","Total Debet atau Kredit (curr) tidak sama dengan Nilai Panjar.");
					return false;						
				}				
	...
	doChange:function(sender){	
		if (sender == this.e_periode && this.e_periode.getText()!="") {			
			this.e_nb.setSQL("select a.no_kas, a.keterangan from kas_m a inner join gr_panjar_m b on a.no_kas=b.no_kas and a.kode_lokasi=b.kode_lokasi "+
			                 "where b.progress ='1' and a.modul = 'KBPJ' and a.posted='F' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",["a.no_kas","a.keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
		}	
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			this.cb1.setSelected(true);						
			var data = this.dbLib.getDataProvider("select a.no_panjar,a.keterangan,a.nik_buat+'-'+b.nama as atensi,a.kode_curr,a.kurs,a.nilai_curr,convert(varchar,a.due_date,103) as due_date,a.akun_panjar  "+
			           "from gr_panjar_m a inner join karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi where a.no_kas='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){
					this.cb_panjar.setText(line.no_panjar,line.keterangan);
					this.e_atensi.setText(line.atensi);
					this.e_tglpakai.setText(line.due_date);
			...
			var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.nilai_curr,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk  "+
						...
						"where a.jenis = 'BEBAN' and a.no_panjar = '"+this.cb_panjar.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg3.clear();
				for (var i in data.rs.rows){
		...
		if (sender == this.e_kurs || this.e_nilaiCurr) {
			this.sg.validasi();
		}	
	},	
	...
	doNilaiChange: function(){
		try{
			this.totCurr = 0;
			var totD = totC = 0;			
			if (this.e_nilaiCurr.getText() != "" && this.e_kurs.getText() != "") totD = nilaiToFloat(this.e_nilaiCurr.getText()) * nilaiToFloat(this.e_kurs.getText());			
