window.app_saku2_transaksi_siaga_fFaEditM = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fFaEditM.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fFaEditM";
...
window.app_saku2_transaksi_siaga_fFaEditM.extend(window.childForm);
window.app_saku2_transaksi_siaga_fFaEditM.implement({
		...
		try{
			if (sender == this.cb_fa) {   
			    this.standarLib.showListData(this, "Daftar Aktiva Tetap",sender,undefined, 
											  "select no_fa, nama  from fa_asset where progress in ('1') and kode_lokasi='"+this.app._lokasi+"'",
											  "select count(no_fa) from fa_asset where progress in ('1') and kode_lokasi='"+this.app._lokasi+"'",
