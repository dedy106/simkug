window.app_saku2_transaksi_siaga_fPjPtgE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fPjPtgE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fPjPtgE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Input Petanggungan Panjar: Edit", 0);	
		...
		this.cb_panjar = new saiCBBL(this,{bound:[20,12,240,20],caption:"Bukti Panjar", readOnly:true, tag:0});
		...
		this.e_nilaiCurr = new saiLabelEdit(this,{bound:[780,20,220,20],caption:"Nilai Curr", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});						
...
window.app_saku2_transaksi_siaga_fPjPtgE.extend(window.childForm);
window.app_saku2_transaksi_siaga_fPjPtgE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("update gr_panjar_m set progress = '1' where no_panjar='"+this.cb_panjar.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from angg_r where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='PJPTG'");
					sql.add("delete from angg_r where no_bukti = '"+this.cb_panjar.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='PJPTG'");
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){								
								sql.add("insert into gr_panjarptg_j(no_ptg,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i)+"','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+","+parseNilai(this.sg.cells(4,i))+","+parseNilai(this.sg.cells(5,i))+",'"+this.sg.cells(6,i)+"','"+this.sg.cells(8,i)+"','"+this.app._lokasi+"','PANJAR','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
							}
						}
					}
					sql.add("insert into gr_panjarptg_j(no_ptg,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :									
				...
				/*
				if (nilaiToFloat(this.e_nilaiCurr.getText()) < 0 || nilaiToFloat(this.e_nilai.getText()) < 0 || nilaiToFloat(this.e_kas.getText()) < 0) {
					system.alert(this,"Transaksi tidak valid.","Nilai PTG dan atau KAS tidak boleh kurang dari nol.");
					return false;						
				}
				*/				
	...
	doChange:function(sender){		
		if (sender == this.e_periode && this.e_periode.getText()!="") {			
			this.e_nb.setSQL("select no_ptg, keterangan from gr_panjarptg_m where progress='0' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_ptg","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
		}		
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			this.cb1.setSelected(true);			
			var data = this.dbLib.getDataProvider(
			           "select a.no_panjar,a.tanggal,a.periode,a.no_dokumen,a.keterangan,a.nik_buat,b.nama as nama_buat,a.nik_app,bb.nama as nama_app,a.kode_curr,a.kurs,a.kode_pp,c.nama as nama_pp,a.nilai_kas, "+
					   "       aa.nilai_curr as nilai_pj,aa.keterangan as ket_pj,aa.akun_panjar "+
			           "from gr_panjarptg_m a "+
			           "    inner join gr_panjar_m aa on a.no_panjar=aa.no_panjar and a.kode_lokasi=aa.kode_lokasi "+
			...
			var data = this.dbLib.getDataProvider(
						...
						"where a.no_bukti = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='PJPTG' order by a.kode_akun",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg2.clear();
				for (var i in data.rs.rows){
