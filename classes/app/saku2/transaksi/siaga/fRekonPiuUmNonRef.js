window.app_saku2_transaksi_siaga_fRekonPiuUmNonRef = function (owner) {
	if (owner) {
		window.app_saku2_transaksi_siaga_fRekonPiuUmNonRef.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku2_transaksi_siaga_fRekonPiuUmNonRef";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick", "Form Rekon Piutang Umum", 0);
		...
		this.cb_cust = new saiCBBL(this.pc1.childPage[0], { bound: [20, 12, 240, 20], caption: "Customer", readOnly: true });
...
window.app_saku2_transaksi_siaga_fRekonPiuUmNonRef.extend(window.childForm);
window.app_saku2_transaksi_siaga_fRekonPiuUmNonRef.implement({
		...
		try {
			if (this.standarLib.checkEmptyByTag(this, [0, 1, 2])) {
				try {
					...
					//Get Nomor Dokumen
					if (this.cb_inv.getText() != "") {
						var data2 = this.dbLib.getDataProvider("select no_dokumen from gr_piutang_m where no_piutang='" + this.cb_inv.getText() + "'", true);
						if (typeof data2 == "object") {
							var line = data2.rs.rows[0];
							if (line != undefined) {
								var no_dok = line.no_dokumen;
							}
						}
					}
					//pph dijurnal ke ju		
					if (this.e_pph.getText() != "0" || this.e_lain.getText() != "0") {
						...
						sql.add("insert into gr_piutangbayar_d(no_bukti,kode_lokasi,no_piutang,akun_piutang,nilai_kas,nilai_lain,nilai_sk,periode,dc,modul,kode_curr,kurs,akun_pph,akun_lain) values " +
							"('" + this.e_nb.getText() + "','" + this.app._lokasi + "','" + this.cb_inv.getText() + "','" + this.akunAR + "',0," + pphLain + ",0,'" + this.e_periode.getText() + "','D','JUNONREF','" + this.c_curr.getText() + "'," + parseNilai(this.e_kursar.getText()) + ",'" + this.cb_pph.getText() + "','" + this.cb_lain.getText() + "')");
						...
						sql.add("insert into ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user) values " +
							"('" + this.e_nb.getText() + "','" + this.app._lokasi + "','" + this.e_periode.getText() + "','" + this.dp_d1.getDateString() + "','" + this.cb_inv.getText() + "','Penyelesaian atas Invoice : " + this.cb_inv.getText() + "','-','REKPIUM','JU','IDR',1," + pphLain + ",'" + this.app._userLog + "','" + this.app._userLog + "','F','-','-','" + this.cb_inv.getText() + "',getdate(),'" + this.app._userLog + "')");
						...
						if (this.e_pph.getText() != "0") {
							sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values " +
								"('" + this.e_nb.getText() + "','" + this.cb_inv.getText() + "','" + this.dp_d1.getDateString() + "',0,'" + this.cb_pph.getText() + "','PPh atas Invoice : " + this.cb_inv.getText() + "','D'," + parseNilai(this.e_pph.getText()) + ",'" + this.app._kodePP + "','-','-','-','-','-','-','-','" + this.app._lokasi + "','REKPIUM','PPH','" + this.e_periode.getText() + "','IDR',1,'" + this.app._userLog + "',getdate(),'" + no_dok + "','-')");
							sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values " +
								"('" + this.e_nb.getText() + "','" + this.cb_inv.getText() + "','" + this.dp_d1.getDateString() + "',1,'" + this.akunAR + "','Penyelesaian atas Invoice : " + this.cb_inv.getText() + "','C'," + parseNilai(this.e_pph.getText()) + ",'" + this.app._kodePP + "','-','-','-','-','-','-','-','" + this.app._lokasi + "','REKPIUM','PIU','" + this.e_periode.getText() + "','IDR',1,'" + this.app._userLog + "',getdate(),'" + no_dok + "','-')");
						}
						if (this.e_lain.getText() != "0") {
							sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values " +
								"('" + this.e_nb.getText() + "','" + this.cb_inv.getText() + "','" + this.dp_d1.getDateString() + "',2,'" + this.cb_lain.getText() + "','Penyelesaian atas Invoice : " + this.cb_inv.getText() + "','D'," + parseNilai(this.e_lain.getText()) + ",'" + this.app._kodePP + "','-','-','-','-','-','-','-','" + this.app._lokasi + "','REKPIUM','LAIN','" + this.e_periode.getText() + "','IDR',1,'" + this.app._userLog + "',getdate(),'" + no_dok + "','-')");
							sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values " +
								"('" + this.e_nb.getText() + "','" + this.cb_inv.getText() + "','" + this.dp_d1.getDateString() + "',3,'" + this.akunAR + "','Penyelesaian atas Invoice : " + this.cb_inv.getText() + "','C'," + parseNilai(this.e_lain.getText()) + ",'" + this.app._kodePP + "','-','-','-','-','-','-','-','" + this.app._lokasi + "','REKPIUM','PIU','" + this.e_periode.getText() + "','IDR',1,'" + this.app._userLog + "',getdate(),'" + no_dok + "','-')");
						}
	...
	doSelectDate: function (sender, y, m, d) {
		...
		this.cb_inv.setSQL("select a.no_piutang,a.no_dokumen, a.keterangan from gr_piutang_m a " +
			...
			"where a.nilai_curr>isnull(c.bayar,0) and a.periode<='" + this.e_periode.getText() + "' and a.kode_lokasi='" + this.app._lokasi + "'",
	...
	doChange: function (sender) {
		if (sender == this.cb_inv && this.cb_inv.getText() != "") {
			var data = this.dbLib.getDataProvider("select a.nilai_curr-isnull(c.bayar,0) as total,a.akun_piutang,a.akun_piutang + ' - '+b.nama as nama,a.kode_curr,a.kurs,a.kode_cust " +
		...
		if ((sender == this.e_pph || sender == this.e_lain) && this.e_pph.getText() != "" && this.e_lain.getText() != "") {
			var total = nilaiToFloat(this.e_pph.getText()) + nilaiToFloat(this.e_lain.getText());
