window.app_saku2_transaksi_siaga_fBillVcc = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fBillVcc.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fBillVcc";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Biling VCC: Input", 0);	
...
window.app_saku2_transaksi_siaga_fBillVcc.extend(window.childForm);
window.app_saku2_transaksi_siaga_fBillVcc.implement({	
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "" && this.cb_cabang.getText()!= "") {				
			...
			var data = this.dbLib.getDataProvider("select isnull(max(substring(no_vcc,4,20)),0) as no_vcc from gr_vcc_m where no_vcc like '______"+AddFormat+"%' and kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){
					if (line.no_vcc == "0") this.e_nb.setText("VCC001"+AddFormat);
					else {
						...
						this.e_nb.setText("VCC"+nu+AddFormat);
					}
				} 
			}
			this.e_ket.setFocus();
