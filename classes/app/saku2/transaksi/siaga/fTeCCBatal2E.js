window.app_saku2_transaksi_siaga_fTeCCBatal2E = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fTeCCBatal2E.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fTeCCBatal2E";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pembatalan Piutang TeCC: Edit", 0);	
...
window.app_saku2_transaksi_siaga_fTeCCBatal2E.extend(window.childForm);
window.app_saku2_transaksi_siaga_fTeCCBatal2E.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user) values "+
						    "('"+this.nb+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','-','"+this.e_ket.getText()+"','-','TECCBTL','TECCBTL','IDR',1,"+parseNilai(this.e_saldo.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','F','-','-','-',getdate(),'"+this.app._userLog+"')");							
					sql.add("insert into gr_teccbayar_d(no_bukti,kode_lokasi,no_invoice,akun_piutang,nilai_kas,nilai_lain,periode,dc,modul) values "+
							"('"+this.nb+"','"+this.app._lokasi+"','"+this.cb_inv.getText()+"','"+this.akunAR+"',0,"+parseNilai(this.e_saldo.getText())+",'"+this.e_periode.getText()+"','D','KB')");
	...
	doChange:function(sender){
		if (sender == this.e_periode) {			
			if (this.e_periode.getText()!="") {
				this.e_nb.setSQL("select no_ju, keterangan from ju_m where modul = 'TECCBTL' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_ju","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
			}			
		}		
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			var data = this.dbLib.getDataProvider("select a.tanggal,a.periode,a.keterangan,a.nik_buat,a.nik_setuju,b.nama as nama_buat,bb.nama as nama_setuju,a.kode_pp,e.no_invoice,g.nama_cust,e.nilai_lain "+
			           "from ju_m a "+
			           "	inner join karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+
					   "	inner join karyawan bb on a.nik_setuju=bb.nik and a.kode_lokasi=bb.kode_lokasi "+
			           "    inner join gr_teccbayar_d e on a.no_ju=e.no_bukti and a.kode_lokasi=e.kode_lokasi "+
