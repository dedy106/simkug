window.app_saku2_transaksi_siaga_npko2_fPanjarNPKO = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_npko2_fPanjarNPKO.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_npko2_fPanjarNPKO";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Input Permohonan Panjar NPKO: Input", 0);	
		...
		try {
			...
			//masih susah ngatur anggaran buat kursnya...(hanya keep)
...
window.app_saku2_transaksi_siaga_npko2_fPanjarNPKO.extend(window.childForm);
window.app_saku2_transaksi_siaga_npko2_fPanjarNPKO.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into gr_npp_m(no_npp,kode_lokasi,no_npko,no_dokumen,tanggal,due_date,keterangan,catatan,kode_curr,kurs,nik_buat,nik_setuju,nik_gar,nik_kas,kode_vendor,kode_pp,modul,jenis,nilai,nilai_ppn,periode,nik_user,tgl_input,no_spb,akun_ppn) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_npko.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.dp_d2.getDateString()+"','"+this.e_ket.getText()+"','-','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','-','-','"+this.cb_buat.getText()+"','"+this.cb_pp.getText()+"','PJNPKO','PJ',"+parseNilai(this.e_nilaiCurr.getText())+",0,'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'-','-')");
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){																																
								sql.add("insert into gr_npp_j(no_npp,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nilai_curr,kode_curr,kurs,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.cb_npko.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i)+"',"+parseNilai(this.sg.cells(5,i))+",'"+this.sg.cells(6,i)+"','"+this.sg.cells(8,i)+"','"+this.app._lokasi+"','PJNPKO','BEBAN','"+this.e_periode.getText()+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+",'"+this.app._userLog+"',getdate())");
							}
						}
					}
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_npko && this.cb_npko.getText()!="") {
			...
			if (this.e_ket.getText() == "") var keterangan = "-"; else keterangan = this.e_ket.getText();
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData([line.kode_akun,line.nama_akun,'D',keterangan,floatToNilai(line.nilai),floatToNilai(line.nilai),line.kode_pp,line.nama_pp,line.kode_drk,line.nama_drk]);
				}
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "" ) {							
			...
			var data = this.dbLib.getDataProvider("select isnull(max(substring(no_panjar,5,20)),0) as no_panjar from gr_panjar_m where no_panjar like '_______"+AddFormat+"%' and kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){
					if (line.no_panjar == "0") this.e_nb.setText("KASP001"+AddFormat+this.cb_pp.getText());
					else {
						var idx = parseFloat(line.no_panjar.substr(0,3)) + 1;
						idx = idx.toString();
						if (idx.length == 1) var nu = "00"+idx;
						if (idx.length == 2) var nu = "0"+idx;
						if (idx.length == 3) var nu = idx;
						this.e_nb.setText("KASP"+nu+AddFormat+this.cb_pp.getText());
					}
				} 
			}
			this.e_dok.setFocus();
