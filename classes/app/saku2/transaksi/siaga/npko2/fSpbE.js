window.app_saku2_transaksi_siaga_npko2_fSpbE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_npko2_fSpbE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_npko2_fSpbE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Input SPB via NPP: Edit", 0);	
		
		uses("saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		this.e_periode = new portalui_saiLabelEdit(this,{bound:[20,14,202,20],caption:"Periode",tag:2,readOnly:true,change:[this,"doChange"]});
		this.l_tgl1 = new portalui_label(this,{bound:[20,11,100,18],caption:"Tanggal", underline:true});
		this.dp_d1 = new portalui_datePicker(this,{bound:[120,11,100,18],selectDate:[this,"doSelectDate"]}); 				
		this.e_nb = new saiCBBL(this,{bound:[20,12,222,20],caption:"No SPB", multiSelection:false, maxLength:10, tag:1, readOnly:true,change:[this,"doChange"]});				
		...
		this.cb_app2 = new saiCBBL(this,{bound:[20,18,200,20],caption:"Disetujui Oleh", multiSelection:false, maxLength:10, tag:2});			
		...
		this.cb_npp = new saiCBBL(this,{bound:[20,17,220,20],caption:"No NPP", readOnly:true});
		this.cb_vendor = new saiCBBL(this,{bound:[20,14,220,20],caption:"Kepada (Mitra)", readOnly:true});						
		...
		this.pc1 = new pageControl(this,{bound:[20,12,900,260], childPage:["Daftar Item NPP","Jurnal Tambahan","Data Pendukung"]});						
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,10,this.pc1.width-5,this.pc1.height-40],colCount:10,tag:0,
		            colTitle:["Kode Akun","Nama Akun","Kode PP","Nama PP","Kode DRK","Nama DRK","DC","Keterangan","Nilai NPP","NPP Original"],
...
window.app_saku2_transaksi_siaga_npko2_fSpbE.extend(window.childForm);
window.app_saku2_transaksi_siaga_npko2_fSpbE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("update gr_npp_m set no_spb='-' where no_spb='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add("insert into gr_spb_m(no_spb,kode_lokasi,no_npko,no_dokumen,tanggal,due_date,keterangan,catatan,kode_curr,kurs,nik_buat,nik_setuju,nik_gar,nik_kas,kode_vendor,kode_pp,modul,jenis,nilai,periode,nik_user,tgl_input,no_ver,no_kaslist,no_kas,  no_po,tgl_po,tgl_ba,no_dok,no_ref,tgl_dok) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_npp.getText()+"','-','"+this.dp_d1.getDateString()+"','"+this.dp_d2.getDateString()+"','"+this.e_ket.getText()+"','-','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.cb_app2.getText()+"','-','"+this.cb_vendor.getText()+"','"+this.cb_pp.getText()+"','SPB','NPP',"+parseNilai(this.e_total.getText())+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'PASS','PASS','-',  '"+this.e_po.getText()+"','"+this.dp_d3.getDateString()+"','"+this.dp_d4.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ref.getText()+"','"+this.dp_d5.getDateString()+"')");
										
					sql.add("update gr_npp_m set no_spb='"+this.e_nb.getText()+"' where no_npp='"+this.cb_npp.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){								
								sql.add("insert into gr_spb_j(no_spb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nilai_curr,kode_curr,kurs,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.cb_npp.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(7,i)+"','"+this.sg.cells(6,i)+"',"+parseNilai(this.sg.cells(8,i))+",'"+this.sg.cells(2,i)+"','"+this.sg.cells(4,i)+"','"+this.app._lokasi+"','SPB','NPP','"+this.e_periode.getText()+"',"+parseNilai(this.sg.cells(9,i))+",'"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+",'"+this.app._userLog+"',getdate())");																	 															
							}						
						}
					}									
					if (this.sg2.getRowValidCount() > 0){
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){								
								var j = i+99;										                 
								sql.add("insert into gr_spb_j(no_spb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nilai_curr,kode_curr,kurs,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.cb_npp.getText()+"','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sg2.cells(0,i)+"','"+this.sg2.cells(3,i)+"','"+this.sg2.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg2.cells(5,i))+",'"+this.sg2.cells(6,i)+"','"+this.sg2.cells(8,i)+"','"+this.app._lokasi+"','SPB','TAMBAH','"+this.e_periode.getText()+"',"+parseNilai(this.sg2.cells(4,i))+",'"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+",'"+this.app._userLog+"',getdate())");
							}
						}
					}
	...
	doChange:function(sender){
		if (sender == this.e_periode) {			
			if (this.e_periode.getText()!="") {
				this.e_nb.setSQL("select no_spb, keterangan from gr_spb_m where jenis = 'NPP' and no_kas='-' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_spb","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
			}			
		}		
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			this.cb1.setSelected(true);			
			var strSQL = "select b.no_npp,b.keterangan as ket_npp,a.tanggal,a.due_date,a.keterangan,a.kode_curr,a.kurs,a.nik_buat,a.nik_setuju,a.nik_gar,a.kode_vendor,c.nama as nama_vendor,a.kode_pp,a.nilai,a.periode,  a.no_po,a.tgl_po,a.tgl_ba,a.no_dok,a.no_ref,a.tgl_dok "+
			             "from gr_spb_m a inner join gr_npp_m b on a.no_spb=b.no_spb and a.kode_lokasi=b.kode_lokasi "+
						 "                inner join gr_vendor c on a.kode_vendor=c.kode_vendor and a.kode_lokasi=c.kode_lokasi "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){
					...
					this.cb_npp.setText(line.no_npp,line.ket_npp);
					...
					this.dp_d3.setText(line.tgl_po);															
					this.dp_d4.setText(line.tgl_ba);										
					this.e_dok.setText(line.no_dok);										
					this.e_ref.setText(line.no_ref);															
					this.dp_d5.setText(line.tgl_dok);										
				} 
			}											
			var strSQL = "select a.kode_akun,a.kode_pp,a.kode_drk,a.nilai,a.nilai_curr,b.nama as nama_akun,c.nama as nama_pp,d.nama as nama_drk,a.periode,a.dc,a.keterangan,a.kode_curr,a.kurs,e.nilai_ppn "+
						 ...
						 "                 inner join gr_npp_m e on a.no_npp=e.no_npp and a.kode_lokasi=e.kode_lokasi "+						 
						 "where a.no_npp='"+this.cb_npp.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
				...
				this.periodeNPP = line.periode;				
				if (parseFloat(line.nilai_ppn) != 0){
					var strSQL = "select b.kode_akun,b.nama as nama_akun,c.nama as nama_pp,a.nilai_ppn,a.kode_pp,round(a.nilai_ppn * a.kurs,0) as ppn_idr,a.keterangan "+
								 "from  gr_npp_m a inner join masakun b on a.akun_ppn=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
								 "                 inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
								 "where a.no_npp='"+this.cb_npp.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";										
					var data1 = this.dbLib.getDataProvider(strSQL,true);
					if (typeof data1 == "object" && data1.rs.rows[0] != undefined){
						var line1 = data1.rs.rows[0];
						this.sg.appendData([line1.kode_akun,line1.nama_akun,line1.kode_pp,line1.nama_pp,'-','-','D',line1.keterangan,floatToNilai(line1.ppn_idr),floatToNilai(line1.nilai_ppn)]);
					}
				}				
			...
			var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai_curr,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk  "+
						...
						"where a.jenis='TAMBAH' and a.no_spb = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg2.clear();
				for (var i in data.rs.rows){
	    			...
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.cb1.isSelected()) {								
								this.nama_report="server_report_saku2_siaga_rptSpb2";
	...
	viewBukti : function(){
		try {
			this.flag="1";
			this.nama_report="server_report_saku2_siaga_rptSpbBukti";
			this.filter = " where a.kode_lokasi='"+this.app._lokasi+"' and a.no_spb='"+this.e_nb.getText()+"' ";
			this.filter2 = this.app._namaUser;
			this.viewer.prepare();
			this.viewer.setVisible(true);
