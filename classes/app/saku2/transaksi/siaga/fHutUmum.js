window.app_saku2_transaksi_siaga_fHutUmum = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fHutUmum.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fHutUmum";
...
window.app_saku2_transaksi_siaga_fHutUmum.extend(window.childForm);
window.app_saku2_transaksi_siaga_fHutUmum.implement({
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "" ) {				
			...
			var data = this.dbLib.getDataProvider("select isnull(max(substring(no_hutang,4,20)),0) as no_hutang from gr_hutang_m where no_hutang like '______"+AddFormat+"%' and kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){
					if (line.no_hutang == "0") this.e_nb.setText("HUT001"+AddFormat+this.cb_pp.getText());
					else {
						var idx = parseFloat(line.no_hutang.substr(0,3)) + 1;
						idx = idx.toString();
						if (idx.length == 1) var nu = "00"+idx;
						if (idx.length == 2) var nu = "0"+idx;
						if (idx.length == 3) var nu = idx;
						this.e_nb.setText("HUT"+nu+AddFormat+this.cb_pp.getText());
					}
				} 
			}
			this.e_dok.setFocus();
