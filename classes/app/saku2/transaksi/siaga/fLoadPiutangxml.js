window.app_saku2_transaksi_siaga_fLoadPiutangxml = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fLoadPiutangxml.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fLoadPiutangxml";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Piutang XML: Load", 0);	
		...
		try {			
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('XMLPIU') and kode_lokasi = '"+this.app._lokasi+"'",true);			
			if (typeof data == "object"){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];																	
					if (line.kode_spro == "XMLPIU") this.alamatXml = line.flag;							
				}
			}
		}catch(e){
			systemAPI.alert(e);
...
window.app_saku2_transaksi_siaga_fLoadPiutangxml.extend(window.childForm);
window.app_saku2_transaksi_siaga_fLoadPiutangxml.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					var data = this.dbLib.getDataProvider("select isnull(max(substring(no_piutang,4,20)),0) as no_piutang from gr_piutang_m where no_piutang like '______"+AddFormat+"%' and kode_lokasi='"+this.app._lokasi+"'",true);
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
						if (line != undefined){
							if (line.no_piutang == "0") var idx = 0;
							else var idx = parseFloat(line.no_piutang.substr(0,3));
						} 
					}					
					var line;					
					for (var i=0; i < this.dataUpload.rows.length;i++){
