window.app_saku2_transaksi_siaga_fMintaBayarKonE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fMintaBayarKonE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fMintaBayarKonE";
		...
		this.e_nilaiCurr = new saiLabelEdit(this,{bound:[780,16,220,20],caption:"Nilai Curr", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});						
		this.e_ket = new saiLabelEdit(this,{bound:[20,13,450,20],caption:"Untuk Pembayaran", maxLength:150});						
		this.e_nilai = new saiLabelEdit(this,{bound:[780,13,220,20],caption:"Nilai", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		
		this.pc1 = new pageControl(this,{bound:[20,12,980,287], childPage:["Data Kontrak","Item Mata Anggaran","KPA Anggaran"]});		
		this.cb_kontrak = new saiCBBL(this.pc1.childPage[0],{bound:[20,21,240,20],caption:"Kontrak", multiSelection:false, maxLength:10, tag:1, readOnly:true});		
		this.e_atensi = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,14,450,20],caption:"Atensi", maxLength:150, readOnly:true});						
		this.c_curr = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,20,160,20],caption:"Mt Uang - Kurs", tag:0, readOnly:true, text:"IDR",change:[this,"doChange"]});				
		...
		this.e_nilaiKon = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,13,220,20],caption:"Nilai Kontrak", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		this.e_sisa = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,14,220,20],caption:"Sisa Kontrak", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
...
window.app_saku2_transaksi_siaga_fMintaBayarKonE.extend(window.childForm);
window.app_saku2_transaksi_siaga_fMintaBayarKonE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from gr_beban_m where no_beban='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from gr_beban_j where no_beban='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
					sql.add("delete from angg_r where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='PB'");
					sql.add("delete from gr_kontrak_d where no_beban='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					
					sql.add("insert into gr_beban_m(no_beban,kode_lokasi,no_dokumen,tanggal,keterangan,kode_pp,nik_buat,nik_app,atensi,kode_curr,kurs,nilai,nilai_curr,periode,nik_user,tgl_input,no_kas,progress) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_pp.getText()+"','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.e_atensi.getText()+"','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+","+parseNilai(this.e_nilai.getText())+","+parseNilai(this.e_nilaiCurr.getText())+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'-','0')");															
					sql.add("insert into gr_kontrak_d(no_beban,no_kontrak,kode_lokasi,dc,nilai) values "+
						    "('"+this.e_nb.getText()+"','"+this.cb_kontrak.getText()+"','"+this.app._lokasi+"','D',"+parseNilai(this.e_nilaiCurr.getText())+")");
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){								
								sql.add("insert into gr_beban_j(no_beban,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i)+"','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+","+parseNilai(this.sg.cells(4,i))+","+parseNilai(this.sg.cells(5,i))+",'"+this.sg.cells(6,i)+"','"+this.sg.cells(8,i)+"','"+this.app._lokasi+"','PB','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
							}
						}
					}														
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','PB','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(6,i))+","+nilai+")");
							}
						}
					}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :		
				...
				if (nilaiToFloat(this.e_nilaiCurr.getText()) > nilaiToFloat(this.e_sisa.getText())) {
					system.alert(this,"Transaksi tidak valid.","Nilai tidak boleh melebihi sisa kontrak.");
					return false;						
				}
	...
	doChange:function(sender){		
		if (sender == this.e_periode && this.e_periode.getText()!="") {			
			this.e_nb.setSQL("select no_beban, keterangan from gr_beban_m where modul = 'BKON' and progress='0' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_beban","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
		}		
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			this.cb1.setSelected(true);			
			var data = this.dbLib.getDataProvider("select a.tanggal,a.periode,a.no_dokumen,a.atensi,a.keterangan,a.nik_buat,b.nama as nama_buat,a.nik_app,bb.nama as nama_app,a.kode_curr,a.kurs,a.kode_pp,c.nama as nama_pp,d.no_kontrak,e.keterangan as ket,e.nilai,e.nilai-isnull(f.tagih,0) as sisa  "+
			           "from gr_beban_m a "+
			           "	inner join karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+
					   "	inner join karyawan bb on a.nik_app=bb.nik and a.kode_lokasi=bb.kode_lokasi "+			           
					   "	inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+			           
					   "    inner join gr_kontrak_d d on a.no_beban=d.no_beban and a.kode_lokasi=d.kode_lokasi "+
					   "    inner join gr_kontrak e on d.no_kontrak=e.no_kontrak and d.kode_lokasi=e.kode_lokasi "+
					   "    left join (select no_kontrak,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as tagih from gr_kontrak_d where no_beban <> '"+this.e_nb.getText()+"' group by no_kontrak,kode_lokasi) f on e.no_kontrak=f.no_kontrak and e.kode_lokasi=f.kode_lokasi "+
					   "where a.no_beban='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){
					...
					this.cb_kontrak.setText(line.no_kontrak,line.ket);					
					
					this.e_nilaiKon.setText(floatToNilai(line.nilai));
			...
			var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.nilai_curr,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk  "+
						...
						"where a.no_beban = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
				for (var i in data.rs.rows){
