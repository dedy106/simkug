window.app_saku2_transaksi_siaga_fMintaBayarKon = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fMintaBayarKon.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fMintaBayarKon";
		...
		this.cb_kontrak = new saiCBBL(this.pc1.childPage[0],{bound:[20,21,240,20],caption:"Kontrak", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});		
		...
		try {
			...
			this.cb_kontrak.setSQL("select a.no_kontrak, a.no_dokumen from gr_kontrak a left join (select no_kontrak,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as tagih from gr_kontrak_d group by no_kontrak,kode_lokasi) b on a.no_kontrak=b.no_kontrak and a.kode_lokasi=b.kode_lokasi where a.nilai > isnull(b.tagih,0) and a.kode_lokasi='"+this.app._lokasi+"' ",["a.no_kontrak","a.no_dokumen"],false,["No Kontrak","No Dokumen"],"and","Data Kontrak",true);			
		}catch(e){
			systemAPI.alert(e);
		}
	}
};
window.app_saku2_transaksi_siaga_fMintaBayarKon.extend(window.childForm);
window.app_saku2_transaksi_siaga_fMintaBayarKon.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into gr_beban_m(no_beban,kode_lokasi,no_dokumen,tanggal,keterangan,kode_pp,nik_buat,nik_app,atensi,kode_curr,kurs,nilai,nilai_curr,periode,nik_user,tgl_input,no_kas,progress,modul) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_pp.getText()+"','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.e_atensi.getText()+"','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+","+parseNilai(this.e_nilai.getText())+","+parseNilai(this.e_nilaiCurr.getText())+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'-','0','BKON')");										
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_kontrak) {
			var strSQL = "select c.kode_vendor+' - '+c.nama as atensi,a.kode_curr,a.nilai,a.nilai-isnull(b.tagih,0) as sisa from gr_kontrak a "+
						 " inner join gr_vendor c on a.kode_vendor=c.kode_vendor and a.kode_lokasi=c.kode_lokasi "+
			             " left join (select no_kontrak,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as tagih from gr_kontrak_d group by no_kontrak,kode_lokasi) b on a.no_kontrak=b.no_kontrak and a.kode_lokasi=b.kode_lokasi where a.no_kontrak = '"+this.cb_kontrak.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' ";						
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
