window.app_saku2_transaksi_siaga_fApp = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fApp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fApp";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Approval Reprogramming Anggaran: Input", 0);	
		...
		this.c_modul = new saiCB(this,{bound:[20,22,200,20],caption:"Modul",items:["REPRO","ABT"], readOnly:true,tag:2,change:[this,"doChange"]});		
		...
		this.pc1 = new pageControl(this,{bound:[20,12,990,337], childPage:["Data RRA","Detail Transaksi"]});
...
window.app_saku2_transaksi_siaga_fApp.extend(window.childForm);
window.app_saku2_transaksi_siaga_fApp.implement({
					...
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						...
						if (line.status.toUpperCase() == "APP"){
							...
							sql.add("insert into anggaran_d (no_agg,kode_lokasi,no_urut,kode_pp,kode_akun,kode_drk,volume,periode,nilai_sat,nilai,dc,satuan,nik_user,tgl_input,modul)  "+		
									"select a.no_pdrk,a.kode_lokasi,a.no_urut,a.kode_pp,a.kode_akun,a.kode_drk,1,a.periode,a.nilai,a.nilai,a.dc,'-',b.nik_user,getdate(),'REPRO' "+
									"from rra_pdrk_d a inner join rra_pdrk_m b on a.no_pdrk=b.no_pdrk and a.kode_lokasi=b.kode_lokasi "+
									"where a.no_pdrk = '"+line.no_bukti+"' and a.kode_lokasi ='"+line.kode_lokasi+"' and a.dc ='D' ");
						}
				...
				switch(this.c_modul.getText()){
					case "REPRO" :
								strSQL = "select 'INPROG' as status,no_pdrk as no_bukti,'-' as catatan,a.kode_lokasi,a.kode_pp,b.no_dokumen,date_format(a.tanggal,'%d-%m-%Y') as tanggal,a.keterangan,a.sts_pdrk as jenis "+
										 ...
										 "where a.sts_pdrk = 'REPRO' and a.progress='0' and a.periode<='"+this.e_periode.getText()+"' ";
						break;
					/*
					case "ABT" :
								strSQL = "select 'INPROG' as status,a.no_mrev as no_bukti,a.no_pdrk as no_dokumen,date_format(b.tanggal,'%d-%m-%Y') as tanggal,b.keterangan,'MREV' as jenis "+
	...
	doDoubleClick: function(sender, col , row) {
		if (this.sg.cells(1,row) != "") {
			...
			var strSQL = "select substring(a.periode,5,2) as bulan,a.kode_pp,b.nama as nama_pp,a.kode_drk,d.nama as nama_drk,a.kode_akun,c.nama as nama_akun,a.nilai,case a.dc when 'D' then 'TERIMA' else 'DONOR' end as dc  "+
						 ...
						 "where a.no_pdrk = '"+this.sg.cells(4,row)+"' and a.kode_lokasi = '"+this.sg.cells(2,row)+"' order by a.dc";					
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
