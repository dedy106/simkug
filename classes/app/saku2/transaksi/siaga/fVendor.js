window.app_saku2_transaksi_siaga_fVendor = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fVendor.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fVendor";
...
window.app_saku2_transaksi_siaga_fVendor.extend(window.childForm);
window.app_saku2_transaksi_siaga_fVendor.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into gr_vendor(kode_vendor,kode_lokasi,nama,alamat,no_tel,email,npwp,pic,alamat2,bank,cabang,no_rek,nama_rek) values "+
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select kode_vendor,nama,alamat,no_tel,email,npwp,alamat2,pic,bank,cabang,no_rek,nama_rek from gr_vendor "+
						     "where kode_vendor ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
