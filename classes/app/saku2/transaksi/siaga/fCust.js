window.app_saku2_transaksi_siaga_fCust = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fCust.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fCust";
...
window.app_saku2_transaksi_siaga_fCust.extend(window.childForm);
window.app_saku2_transaksi_siaga_fCust.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into gr_cust(kode_cust,kode_lokasi,nama,alamat,no_tel,email,npwp,pic,alamat2) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update gr_cust set nama='"+this.e_nama.getText()+"',alamat='"+this.e_alamat.getText()+"',no_tel='"+this.e_tel.getText()+"',email='"+this.e_mail.getText()+"',npwp='"+this.e_npwp.getText()+"',pic='"+this.e_pic.getText()+"',alamat2='"+this.e_alamat2.getText()+"' "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from gr_cust where kode_cust = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");			
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select kode_cust,nama,alamat,no_tel,email,npwp,alamat2,pic from gr_cust "+
						     "where kode_cust ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
		...
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Customer",sender,undefined, 
											  "select kode_cust, nama  from gr_cust where kode_lokasi='"+this.app._lokasi+"'",
											  "select count(kode_cust) from gr_cust where kode_lokasi='"+this.app._lokasi+"'",
