window.app_saku2_transaksi_siaga_fJuTeCC = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fJuTeCC.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fJuTeCC";
...
window.app_saku2_transaksi_siaga_fJuTeCC.extend(window.childForm);
window.app_saku2_transaksi_siaga_fJuTeCC.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref)  "+
							...
							"from gr_tecc_d a "+
							"inner join gr_tecc_m b on a.no_tecc=b.no_tecc "+
							"inner join gr_tecc c on b.kode_cabang=c.kode_cabang "+
	...
	doChange:function(sender){
		...
		if (sender == this.cb_inv && this.cb_inv.getText()!="") {		
			var data = this.dbLib.getDataProvider("select a.nama_cust,a.total-isnull(c.bayar,0) as total,a.akun_piutang,a.akun_piutang + ' - '+b.nama as nama "+
					   ...
					   "                            from gr_teccbayar_d where no_bukti<>'"+this.e_nb.getText()+"' group by no_invoice,kode_lokasi ) c on a.no_invoice=c.no_invoice and a.kode_lokasi=c.kode_lokasi "+
