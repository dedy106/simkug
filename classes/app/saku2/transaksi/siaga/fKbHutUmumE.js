window.app_saku2_transaksi_siaga_fKbHutUmumE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_siaga_fKbHutUmumE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_siaga_fKbHutUmumE";
...
window.app_saku2_transaksi_siaga_fKbHutUmumE.extend(window.childForm);
window.app_saku2_transaksi_siaga_fKbHutUmumE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from gr_hutangbayar_d where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.nb+"','"+this.app._lokasi+"','"+this.cb_hutang.getText()+"','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_cabang.getText()+"','KBHUTANG','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+","+parseNilai(this.e_nilai.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_buat.getText()+"',getdate(),'"+this.app._userLog+"','F','-','"+this.cb_hutang.getText()+"','"+this.cb_vendor.getText()+"','"+this.cb_bank.getText()+"')");
	...
	doChange:function(sender){
		if (sender == this.e_periode || sender == this.cb_cabang) {			
			if (this.e_periode.getText()!="" && this.cb_cabang.getText()!="") {
				this.e_nb.setSQL("select no_kas, keterangan from kas_m where kode_pp ='"+this.cb_cabang.getText()+"' and modul = 'KBHUTANG' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kas","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
			}			
		}		
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			...
			var data = this.dbLib.getDataProvider("select a.tanggal,a.periode,a.keterangan,a.jenis,a.nik_buat,b.nama as nama_buat,a.kode_pp,c.nama as nama_cab,a.kode_bank,d.nama as nama_bank,a.akun_kb,f.nama as nama_akun,"+
					   ...
					   "    inner join gr_hutangbayar_d e on a.no_kas=e.no_bukti and a.kode_lokasi=e.kode_lokasi "+
					   "    inner join masakun f on a.akun_kb=f.kode_akun and a.kode_lokasi=f.kode_lokasi "+
					   "    inner join gr_hutang_m g on e.no_hutang=g.no_hutang and e.kode_lokasi=g.kode_lokasi "+
					   "    inner join gr_vendor h on g.kode_vendor=h.kode_vendor and h.kode_lokasi=g.kode_lokasi "+
					   "    inner join masakun i on g.akun_hutang=i.kode_akun and g.kode_lokasi=i.kode_lokasi "+
					   "    left join (select no_hutang,kode_lokasi,sum(case dc when 'D' then (nilai_kas+nilai_lain) else -(nilai_kas+nilai_lain) end) as bayar from gr_hutangbayar_d where no_bukti <> '"+this.e_nb.getText()+"' group by no_hutang,kode_lokasi ) cc on g.no_hutang=cc.no_hutang and g.kode_lokasi=cc.kode_lokasi "+
