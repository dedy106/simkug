window.app_saku2_transaksi_inv_fVendor = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_inv_fVendor.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_inv_fVendor";
		...
		this.p1 = new panel(this,{bound:[10,23,900,283],caption:"Daftar "+this.app._namaForm});
		this.sg1 = new saiGrid(this.p1,{bound:[0,20,895,230],tag:9,readOnly:true,
			colTitle: ["Kode","Nama","Alamat","Kota","Kode Pos","No Telpon","No Fax","NPWP","PIC"]});		
...
window.app_saku2_transaksi_inv_fVendor.extend(window.childForm);
window.app_saku2_transaksi_inv_fVendor.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into inv_vendor(kode_vendor,nama,kode_lokasi,alamat,kota,kode_pos,no_telp,no_fax,npwp,pic) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.e_alamat.getText()+"','"+this.e_kota.getText()+"','"+this.e_kodepos.getText()+"','"+this.e_telp.getText()+"','"+this.e_fax.getText()+"','"+this.e_npwp.getText()+"','"+this.e_pic.getText()+"')");
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from inv_vendor where kode_vendor = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");	
					sql.add("insert into inv_vendor(kode_vendor,nama,kode_lokasi,alamat,kota,kode_pos,no_telp,no_fax,npwp,pic) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.e_alamat.getText()+"','"+this.e_kota.getText()+"','"+this.e_kodepos.getText()+"','"+this.e_telp.getText()+"','"+this.e_fax.getText()+"','"+this.e_npwp.getText()+"','"+this.e_pic.getText()+"')");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var data = this.dbLib.getDataProvider("select a.nama,a.alamat,a.kota,a.kode_pos,a.no_telp,a.no_fax,a.npwp,a.pic "+
				           " from inv_vendor a "+
						   "where a.kode_vendor ='"+this.cb_kode.getText()+"' ",true);
	...
	doTampilClick: function(sender){
		try{			
			var temp = this.dbLib.runSQL("select a.kode_vendor,a.nama,a.alamat,a.kota,a.kode_pos,a.no_telp,a.no_fax,a.npwp,a.pic "+
			                             "from inv_vendor a where a.kode_lokasi = '"+this.app._lokasi+"' order by a.kode_vendor");
	...
	doBtnClick: function(sender, event){
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar vendoromer",sender,undefined, 
											  "select kode_vendor, nama  from inv_vendor where kode_lokasi = '"+this.app._lokasi+"' ",
											  "select count(kode_vendor) from inv_vendor where kode_lokasi = '"+this.app._lokasi+"' ",
