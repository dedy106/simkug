window.app_saku2_transaksi_inv_fStatus = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_inv_fStatus.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_inv_fStatus";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Perubahan Status Barang: Input", 0);	
		...
		this.sg = new saiGrid(this.p1,{bound:[1,20,this.p1.width-5,this.p1.height-45],colCount:10,tag:0,
		            colTitle:["Kondisi","Kode Brg","Nama Barang","Merk","Tipe","Serial","Modul","SN Modul","Voip","SN Voip"],
					colWidth:[[9,8,7,6,5,4,3,2,1,0],[80,80,80,80,80,100,100,150,80,70]],
					columnReadOnly:[true,[9,8,7,6,5,4,3,2,1,0],[]],
					buttonStyle:[[0,1],[bsAuto,bsEllips]], 										
					picklist:[[0],[new portalui_arrayMap({items:["BAIK","RUSAK","UNREPAIR"]})]],
...
window.app_saku2_transaksi_inv_fStatus.extend(window.childForm);
window.app_saku2_transaksi_inv_fStatus.implement({
	...
	simpan: function(){			
		try{						
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"inv_status_m","no_status",this.app._lokasi+"-STS"+this.e_periode.getText().substr(2,4)+".","000"));
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("insert into inv_status_m (no_status,kode_lokasi,no_dokumen,tanggal,keterangan,periode,tgl_input,nik_user,nik_buat,kode_cabang) values  "+
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into inv_status_d(no_status,kode_lokasi,kode_cabang,kode_barang,no_urut,status) values  "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_cabang.getText()+"','"+this.sg.cells(1,i)+"',"+i+",'"+this.sg.cells(0,i)+"')");								
								sql.add("update inv_barang set kondisi='"+this.sg.cells(0,i)+"' where kode_barang = '"+this.sg.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
							}
						}
					}					
