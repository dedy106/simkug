window.app_saku2_transaksi_inv_fCust = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_inv_fCust.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_inv_fCust";
...
window.app_saku2_transaksi_inv_fCust.extend(window.childForm);
window.app_saku2_transaksi_inv_fCust.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into inv_cust(kode_cust,nama,kode_lokasi,alamat,kota,kode_pos,no_telp,no_fax,npwp,pic) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from inv_cust where kode_cust = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");	
					sql.add("insert into inv_cust(kode_cust,nama,kode_lokasi,alamat,kota,kode_pos,no_telp,no_fax,npwp,pic) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.e_alamat.getText()+"','"+this.e_kota.getText()+"','"+this.e_kodepos.getText()+"','"+this.e_telp.getText()+"','"+this.e_fax.getText()+"','"+this.e_npwp.getText()+"','"+this.e_pic.getText()+"')");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
	...
	doTampilClick: function(sender){
		try{			
			var temp = this.dbLib.runSQL("select a.kode_cust,a.nama,a.alamat,a.kota,a.kode_pos,a.no_telp,a.no_fax,a.npwp,a.pic "+
			                             "from inv_cust a where a.kode_lokasi = '"+this.app._lokasi+"' order by a.kode_cust");
