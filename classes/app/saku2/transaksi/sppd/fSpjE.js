window.app_saku2_transaksi_sppd_fSpjE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_sppd_fSpjE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_sppd_fSpjE";
		...
		this.cb_perintah = new saiCBBL(this,{bound:[20,15,200,20],caption:"NIK Perintah", multiSelection:false, maxLength:10, tag:2});
		...
		try {
			...
			this.nilai_uh = 0;		
			this.cb_buat.setSQL("select nik, nama from karyawan ",["nik","nama"],false,["NIK","Nama"],"and","Data NIK",true);						
...
window.app_saku2_transaksi_sppd_fSpjE.extend(window.childForm);
window.app_saku2_transaksi_sppd_fSpjE.implement({
	...
	doChange:function(sender){		
		...
		if (sender == this.e_periode && this.e_periode.getText() != "") {			
			this.e_nb.setSQL("select a.no_spj, a.keterangan+' - '+x.nama as keterangan from yk_spj_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
							 "  inner join karyawan x on a.nik_buat=x.nik "+
			                 "where b.kode_bidang='"+this.kodeBidang+"' and a.progress='0' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",["a.no_spj","a.keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
		}
		if (sender == this.e_nb && this.e_nb.getText() != "") {			
			var data = this.dbLib.getDataProvider("select a.no_dokumen,convert(varchar,a.tanggal,103) as tanggal,a.periode,a.keterangan,a.nik_buat,b.nama as nama_buat,a.nik_app,c.nama as nama_app,a.nik_gar,d.nama as nama_gar,a.akun_uhar,a.kode_pp,a.kode_drk,e.nama as nama_akun,f.nama as nama_pp,g.nama as nama_drk,a.nik_perintah,dd.nama as nama_perintah,a.jabatan,a.nama_spj,a.tempat "+
					   "from yk_spj_m a "+
			           "	inner join karyawan b on a.nik_buat=b.nik "+ //and a.kode_lokasi=b.kode_lokasi 
			           "	inner join karyawan c on a.nik_app=c.nik "+ //and a.kode_lokasi=c.kode_lokasi 
					   "	inner join karyawan d on a.nik_gar=d.nik "+ //and a.kode_lokasi=d.kode_lokasi 					   
					   "	inner join karyawan dd on a.nik_perintah=dd.nik  "+ //and a.kode_lokasi=dd.kode_lokasi
					   "    inner join masakun e on a.akun_uhar=e.kode_akun and a.kode_lokasi=e.kode_lokasi "+
					   "    inner join pp f on a.kode_pp=f.kode_pp and a.kode_lokasi=f.kode_lokasi "+
					   "    inner join drk g on a.kode_drk=g.kode_drk and a.kode_lokasi=g.kode_lokasi and substring(a.periode,1,4)=g.tahun "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){
					...
					this.cb_akun.setText(line.akun_uhar,line.nama_akun);
					this.cb_pp.setText(line.kode_pp,line.nama_pp);
					this.cb_perintah.setText(line.nik_perintah,line.nama_perintah);						
					this.cb_drk.setText(line.kode_drk,line.nama_drk);					
					this.cb_buat.setText(line.nik_buat,line.nama_buat);
					this.cb_app.setText(line.nik_app,line.nama_app);					
					this.cb_gar.setText(line.nik_gar,line.nama_gar);						
		...
		if (sender == this.cb_pp || sender == this.cb_akun || sender == this.e_periode || sender == this.cb_drk) {
			if (sender == this.cb_pp) {
				if (this.cb_pp.getText()!="") {
					this.cb_gar.setSQL("select nik, nama from karyawan where kode_pp ='"+this.cb_pp.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Data NIK",true);								
				}
			}
		...
		if (sender == this.cb_buat) {			
			if (this.cb_buat.getText() != "") {				
				var data2 = this.dbLib.getDataProvider("select grade from karyawan where nik='"+this.cb_buat.getText()+"'",true);
				if (typeof data2 == "object" && data2.rs.rows[0] != undefined){
					var line2 = data2.rs.rows[0];							
