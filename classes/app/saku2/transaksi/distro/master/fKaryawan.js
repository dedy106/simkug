window.app_saku2_transaksi_distro_master_fKaryawan = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_distro_master_fKaryawan.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_distro_master_fKaryawan";
		...
		this.e_jab = new saiLabelEdit(this,{bound:[20,14,250,20],caption:"Jabatan", maxLength:50, tag:1});
		this.cb_pp = new saiCBBL(this,{bound:[20,17,200,20],caption:"Kode PP", multiSelection:false, maxLength:10, tag:2});		
...
window.app_saku2_transaksi_distro_master_fKaryawan.extend(window.childForm);
window.app_saku2_transaksi_distro_master_fKaryawan.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into karyawan(nik,kode_lokasi,nama,alamat,no_telp,jabatan,kode_pp) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.e_alamat.getText()+"','"+this.e_tel.getText()+"','"+this.e_jab.getText()+"','"+this.cb_pp.getText()+"')");
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update karyawan set nama='"+this.e_nama.getText()+"',alamat='"+this.e_alamat.getText()+"',no_telp='"+this.e_tel.getText()+"',jabatan='"+this.e_jab.getText()+"',kode_pp='"+this.cb_pp.getText()+"' "+
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select a.nik,a.nama,a.alamat,a.no_telp,a.jabatan,a.kode_pp,b.nama as nama_pp "+
