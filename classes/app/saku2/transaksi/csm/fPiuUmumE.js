window.app_saku2_transaksi_csm_fPiuUmumE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_csm_fPiuUmumE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_csm_fPiuUmumE";
		...
		this.e_nilai = new saiLabelEdit(this,{bound:[790,15,210,20],caption:"Nilai", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});								
		this.cb_project = new saiCBBL(this,{bound:[20,14,220,20],caption:"ID Project", readOnly:true});						
		this.e_ppn = new saiLabelEdit(this,{bound:[790,14,210,20],caption:"PPN", tag:1, tipeText:ttNilai, text:"0", change:[this,"doChange"]});		
		this.cb_cust = new saiCBBL(this,{bound:[20,19,220,20],caption:"Customer",readOnly:true});	
		this.e_total = new saiLabelEdit(this,{bound:[790,19,210,20],caption:"Total", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
...
window.app_saku2_transaksi_csm_fPiuUmumE.extend(window.childForm);
window.app_saku2_transaksi_csm_fPiuUmumE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from csm_piutang_m where no_piutang='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from csm_piutang_j where no_piutang='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from angg_r where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					
					sql.add("insert into csm_piutang_m(no_piutang,kode_lokasi,no_dokumen,tanggal,keterangan,kode_project,kode_cust,kode_curr,kurs,nik_app,kode_pp,nilai,periode,nik_user,tgl_input,akun_piutang,posted,nilai_ppn) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_project.getText()+"','"+this.cb_cust.getText()+"','IDR',1,'"+this.cb_app.getText()+"','"+this.cb_pp.getText()+"',"+parseNilai(this.e_nilai.getText())+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.cb_akun.getText()+"','F',"+parseNilai(this.e_ppn.getText())+")");
									
					sql.add("insert into csm_piutang_j(no_piutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!= "") {
			this.e_nb.setSQL("select no_piutang, keterangan from csm_piutang_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
			                 "and no_piutang not in (select distinct no_piutang from csm_piubayar_d)",["no_piutang","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);			
		}		
		if (sender == this.e_nb && this.e_nb.getText()!="") {			
			var strSQL = "select a.tanggal,a.periode,a.no_dokumen,a.keterangan,a.kode_pp,a.akun_piutang,a.nik_app,a.kode_project,b.keterangan as ket_project,a.kode_cust,c.nama as nama_cust "+						 
						 "from csm_piutang_m a inner join csm_project b on a.kode_project=b.kode_project and a.kode_lokasi=b.kode_lokasi "+
						 "                     inner join csm_cust c on a.kode_cust=c.kode_cust and a.kode_lokasi=c.kode_lokasi "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){		
					...
					this.cb_project.setText(line.kode_project,line.ket_project);
