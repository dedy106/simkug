window.app_saku2_transaksi_csm_fKbPiuE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_csm_fKbPiuE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_csm_fKbPiuE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Piutang Umum: Edit", 0);	
...
window.app_saku2_transaksi_csm_fKbPiuE.extend(window.childForm);
window.app_saku2_transaksi_csm_fKbPiuE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from csm_piubayar_d where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add("insert into csm_piubayar_d(no_bukti,no_piutang,kode_lokasi,modul,periode,nik_user,tgl_input,dc,nilai) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_piutang.getText()+"','"+this.app._lokasi+"','KBPIU','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'D',"+parseNilai(this.e_nilai.getText())+")");					
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!= "") {
			this.e_nb.setSQL("select no_kas, keterangan from kas_m where no_del='-' and modul = 'KBPIU' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kas","keterangan"],false,["No KasBank","Deskripsi"],"and","Daftar Bukti KB",true);			
		}
		if (sender == this.e_nb && this.e_nb.getText()!= "") {			
			var strSQL = "select (a.nilai+a.nilai_ppn) -  isnull(b.bayar,0) as saldo, a.akun_piutang,a.kode_project+' - '+c.no_dokumen+' - '+c.keterangan as ket, "+
			             "d.nama+'-'+d.kode_cust as cust,"+
						 "e.jenis,e.periode,e.no_dokumen,e.tanggal,e.keterangan as ket_kas,e.nik_app,e.no_link "+
						 "from csm_piutang_m a inner join csm_project c on a.kode_project=c.kode_project and a.kode_lokasi=c.kode_lokasi "+
						 "					   inner join csm_cust d on a.kode_cust=d.kode_cust and a.kode_lokasi=d.kode_lokasi "+
						 "                     inner join kas_m e on e.no_link = a.no_piutang and a.kode_lokasi=e.kode_lokasi "+
						 "left join (select no_piutang,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar from csm_piubayar_d where no_bukti <> '"+this.e_nb.getText()+"' group by no_piutang,kode_lokasi) b "+
