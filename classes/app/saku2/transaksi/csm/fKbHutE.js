window.app_saku2_transaksi_csm_fKbHutE = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_csm_fKbHutE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_csm_fKbHutE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Hutang Umum: Edit", 0);	
		...
		this.cb_hutang = new saiCBBL(this,{bound:[20,17,220,20],caption:"No Hutang", readOnly:true});				
		this.e_project = new saiLabelEdit(this,{bound:[20,19,470,20],caption:"Dokumen Project", readOnly:true});												
		this.e_saldo = new saiLabelEdit(this,{bound:[710,19,200,20],caption:"Saldo Hutang", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		this.e_vendor = new saiLabelEdit(this,{bound:[20,18,470,20],caption:"Vendor", readOnly:true});												
...
window.app_saku2_transaksi_csm_fKbHutE.extend(window.childForm);
window.app_saku2_transaksi_csm_fKbHutE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from csm_hutbayar_d where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBHUT','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.cb_app.getText()+"',getdate(),'"+this.app._userLog+"','F','-','"+this.cb_hutang.getText()+"','-','-')");
					
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.akunHutang+"','"+this.e_ket.getText()+"','D',"+parseNilai(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBHUT','HUT','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");					
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){																							
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i)+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','-','-','"+this.app._lokasi+"','KBHUT','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");					
							}
						}
					}
					sql.add("insert into csm_hutbayar_d(no_bukti,no_hutang,kode_lokasi,modul,periode,nik_user,tgl_input,dc,nilai) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_hutang.getText()+"','"+this.app._lokasi+"','KBHUT','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'D',"+parseNilai(this.e_nilai.getText())+")");					
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!= "") {
			this.e_nb.setSQL("select no_kas, keterangan from kas_m where no_del='-' and modul = 'KBHUT' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kas","keterangan"],false,["No KasBank","Deskripsi"],"and","Daftar Bukti KB",true);			
		}
		if (sender == this.e_nb && this.e_nb.getText()!= "") {			
			var strSQL = "select (a.nilai+a.nilai_ppn) -  isnull(b.bayar,0) as saldo, a.akun_hutang,a.kode_project+' - '+c.no_dokumen+' - '+c.keterangan as ket, "+
			             "d.nama+'-'+d.kode_vendor as vendor,"+
						 "e.jenis,e.periode,e.no_dokumen,e.tanggal,e.keterangan as ket_kas,e.nik_app,e.no_link "+
						 "from csm_hutang_m a inner join csm_project c on a.kode_project=c.kode_project and a.kode_lokasi=c.kode_lokasi "+
						 "					  inner join csm_vendor d on a.kode_vendor=d.kode_vendor and a.kode_lokasi=d.kode_lokasi "+
						 "                    inner join kas_m e on e.no_link = a.no_hutang and a.kode_lokasi=e.kode_lokasi "+
						 "left join (select no_hutang,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar from csm_hutbayar_d where no_bukti <> '"+this.e_nb.getText()+"' group by no_hutang,kode_lokasi) b "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){		
					...
					this.cb_hutang.setText(line.no_link);
					
					this.akunHutang = line.akun_hutang;
					this.e_project.setText(line.ket);
