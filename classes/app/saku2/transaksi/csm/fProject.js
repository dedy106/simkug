window.app_saku2_transaksi_csm_fProject = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_csm_fProject.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_csm_fProject";
		...
		try {
			...
			this.cb_cust.setSQL("select kode_cust, nama from csm_cust where kode_lokasi='"+this.app._lokasi+"'",["kode_cust","nama"],false,["Kode","Nama"],"and","Data Customer",true);			
		}catch(e){
			systemAPI.alert(e);
		}
	}
};
window.app_saku2_transaksi_csm_fProject.extend(window.childForm);
window.app_saku2_transaksi_csm_fProject.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into csm_project(kode_project,kode_lokasi,no_dokumen,keterangan,kode_cust,tgl_mulai,tgl_selesai,nik_user,tgl_input) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update csm_project set no_dokumen='"+this.e_dok.getText()+"',keterangan='"+this.e_ket.getText()+"',kode_cust='"+this.cb_cust.getText()+"',tgl_mulai='"+this.dp_d1.getDateString()+"',tgl_selesai='"+this.dp_d2.getDateString()+"',nik_user='"+this.app._userLog+"',tgl_input=getdate() "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from csm_project where kode_project = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");			
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select kode_cust,no_dokumen,keterangan,tgl_mulai,tgl_selesai from csm_project "+
						     "where kode_project ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
		...
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Project",sender,undefined, 
											  "select kode_project, keterangan from csm_project where kode_lokasi='"+this.app._lokasi+"'",
											  "select count(kode_project) from csm_project where kode_lokasi='"+this.app._lokasi+"'",
