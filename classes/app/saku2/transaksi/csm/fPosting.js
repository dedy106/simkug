window.app_saku2_transaksi_csm_fPosting = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_csm_fPosting.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_csm_fPosting";
...
window.app_saku2_transaksi_csm_fPosting.extend(window.childForm);
window.app_saku2_transaksi_csm_fPosting.implement({
				...
				switch(this.c_modul.getText()){
					...
					case "AR" :
								strSQL = "select 'INPROG' as status,no_piutang as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'PIUUM' as jenis "+
										 "from csm_piutang_m "+
										 "where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";								
								this.cb_bukti.setSQL("select no_piutang as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'PIUUM' as jenis from csm_piutang_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ",
