window.app_saku2_transaksi_csm_fKbHut = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_csm_fKbHut.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_csm_fKbHut";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Hutang Umum: Input", 0);	
		...
		this.cb_hutang = new saiCBBL(this,{bound:[20,17,220,20],caption:"No Hutang", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});				
...
window.app_saku2_transaksi_csm_fKbHut.extend(window.childForm);
window.app_saku2_transaksi_csm_fKbHut.implement({
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					this.sg.clear(1); 
					setTipeButton(tbSimpan);
					var strSQL = "select a.no_hutang,a.keterangan,a.kode_lokasi from csm_hutang_m a "+
								 "left join (select no_hutang,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar from csm_hutbayar_d group by no_hutang,kode_lokasi) b "+
								 "on a.no_hutang=b.no_hutang and a.kode_lokasi=b.kode_lokasi where a.nilai+a.nilai_ppn>isnull(b.bayar,0) and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
