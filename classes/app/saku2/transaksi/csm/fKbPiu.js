window.app_saku2_transaksi_csm_fKbPiu = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_csm_fKbPiu.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_csm_fKbPiu";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Piutang Umum: Input", 0);	
		...
		this.cb_piutang = new saiCBBL(this,{bound:[20,17,220,20],caption:"No Piutang", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});				
...
window.app_saku2_transaksi_csm_fKbPiu.extend(window.childForm);
window.app_saku2_transaksi_csm_fKbPiu.implement({
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					this.sg.clear(1); 
					setTipeButton(tbSimpan);
					var strSQL = "select a.no_piutang,a.keterangan,a.kode_lokasi from csm_piutang_m a "+
								 "left join (select no_piutang,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar from csm_piubayar_d group by no_piutang,kode_lokasi) b "+
								 "on a.no_piutang=b.no_piutang and a.kode_lokasi=b.kode_lokasi where a.nilai+a.nilai_ppn>isnull(b.bayar,0) and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
