window.app_saku2_transaksi_csm_fHutUmum2E = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_csm_fHutUmum2E.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_csm_fHutUmum2E";
		...
		try {
			...
			this.cb_vendor.setSQL("select kode_vendor, nama from csm_vendor where kode_lokasi='"+this.app._lokasi+"'",["kode_vendor","nama"],false,["Kode","Nama"],"and","Data Vendor",true);						
...
window.app_saku2_transaksi_csm_fHutUmum2E.extend(window.childForm);
window.app_saku2_transaksi_csm_fHutUmum2E.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from csm_hutang_m where no_hutang='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from csm_hutang_j where no_hutang='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from angg_r where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					
					sql.add("insert into csm_hutang_m(no_hutang,kode_lokasi,no_dokumen,tanggal,keterangan,kode_project,kode_vendor,kode_curr,kurs,nik_app,kode_pp,nilai,periode,nik_user,tgl_input,akun_hutang,posted,nilai_ppn) values  "+
					...
					sql.add("insert into csm_hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!= "") {
			this.e_nb.setSQL("select no_hutang, keterangan from csm_hutang_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
			                 "and no_hutang not in (select distinct no_hutang from csm_hutbayar_d)",["no_hutang","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);			
		}		
		if (sender == this.e_nb && this.e_nb.getText()!="") {			
			var strSQL = "select a.tanggal,a.periode,a.no_dokumen,a.keterangan,a.kode_pp,a.akun_hutang,a.nik_app,a.kode_vendor,c.nama as nama_vendor "+						 
						 "from csm_hutang_m a  "+
						 "                    inner join csm_vendor c on a.kode_vendor=c.kode_vendor and a.kode_lokasi=c.kode_lokasi "+
