window.app_saku2_transaksi_droping_fVer = function(owner)
{
	if (owner)
	{
		window.app_saku2_transaksi_droping_fVer.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_transaksi_droping_fVer";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Verifikasi Permintaan: Input", 0);	
		...
		this.cb_lokasi = new saiCBBL(this,{bound:[20,18,200,20],caption:"Lokasi Pengaju", multiSelection:false, maxLength:10, tag:2, change:[this,"doChange"]});				
		this.e_totalnon = new saiLabelEdit(this,{bound:[720,18,200,20],caption:"Total Non Approve", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		this.cb_bukti = new saiCBBL(this,{bound:[20,17,220,20],caption:"Bukti Pengaju", multiSelection:false, maxLength:10, tag:1});						
		this.e_totalapp = new saiLabelEdit(this,{bound:[720,17,200,20],caption:"Total Approve", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		...
		try {
			...
			this.cb_lokasi.setSQL("select kode_lokasi, nama from lokasi where kode_lokasi not in ('00')",["kode_lokasi","nama"],false,["Kode","Nama"],"and","Data Lokasi",true);												
			if (this.app._lokasi != "99") {
				system.alert(this,"Lokasi tidak valid.","Akses hanya untuk lokasi Pusat.");
...
window.app_saku2_transaksi_droping_fVer.extend(window.childForm);
window.app_saku2_transaksi_droping_fVer.implement({
	...
	simpan: function(){			
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{				
					this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"yk_ajuapp_m","no_app",this.app._lokasi+"-VER"+this.e_periode.getText().substr(2,4)+".","000"));
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();										
					sql.add("update yk_ajuapp_m set progress='1' where no_app='"+this.cb_bukti.getText()+"' and kode_lokasi='"+this.cb_lokasi.getText()+"'");									
					sql.add("insert into yk_ajuapp_m(no_app, kode_lokasi,tanggal,keterangan,modul,periode,no_del,nik_buat,nik_app,nik_user,tgl_input,progress) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','AJUVER','"+this.e_periode.getText()+"','"+this.cb_bukti.getText()+"','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.app._userLog+"',now(),'0')");
					var line;
					for (var i = 0; i < this.sg.rows.getLength();i++){
						if (this.sg.rowValid(i) && this.sg.cells(0,i)!="INPROG"){
							if (this.sg.cells(0,i)=="APP") var progress = "2"; 
							else {
								var progress = "X";  										 
								sql.add("insert into angg_cash(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) "+
										"select '"+this.sg.cells(2,i)+"','AJUVER',kode_lokasi,kode_akun,kode_pp,kode_drk,periode,periode,'C',0,nilai "+
										"from yk_aju_j where no_aju = '"+this.sg.cells(4,i)+"' and kode_lokasi='"+this.sg.cells(2,i)+"'");	
							}
							sql.add("insert into yk_ajuapp_d(no_app,modul,kode_lokasi,no_bukti,catatan,status) values "+
									"('"+this.e_nb.getText()+"','AJUVER','"+this.app._lokasi+"','"+this.sg.cells(4,i)+"','"+this.sg.cells(1,i)+"','"+this.sg.cells(0,i)+"')");
							sql.add("update yk_aju_m set progress='"+progress+"' where no_aju='"+this.sg.cells(4,i)+"' and kode_lokasi='"+this.sg.cells(2,i)+"'");
						}											
					}				
					setTipeButton(tbAllFalse);					
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "simpan" :			
				...
				var isAda = false;
				for (var i = 0; i < this.sg.rows.getLength();i++){
					if (this.sg.cells(0,i) == "APP" || this.sg.cells(0,i) == "NONAPP") isAda = true;
				}
				if (!isAda){
					system.alert(this,"Transaksi tidak valid.","Tidak ada transaksi dengan status APP/NONAPP.");
					return false;
				}
				if ((nilaiToFloat(this.e_totalapp.getText()) + nilaiToFloat(this.e_totalnon.getText())) <= 0) {
	...
	doChange:function(sender){
		if (sender == this.e_periode || sender == this.cb_lokasi) {
			if ( this.e_periode.getText()!="" && this.cb_lokasi.getText()!="") {
				this.cb_bukti.setSQL("select a.no_app, a.keterangan from yk_ajuapp_m a "+
							   	     "where a.progress='0' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.cb_lokasi.getText()+"'",["a.no_app","a.keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
			}
		}
	},
	doLoad:function(sender){				
		this.sg.clear(1); this.sg2.clear(1);
		var strSQL = "select 'INPROG' as status,a.no_aju as no_bukti,'-' as catatan,a.kode_lokasi,a.kode_pp,a.no_dokumen,date_format(a.tanggal,'%d-%m-%Y') as tanggal,a.keterangan,a.nilai "+
				     "from yk_aju_m a inner join yk_ajuapp_d b on a.no_aju=b.no_bukti and a.kode_lokasi=b.kode_lokasi "+
				     "where a.progress='1' and b.no_app='"+this.cb_bukti.getText()+"' and a.kode_lokasi='"+this.cb_lokasi.getText()+"' ";		
	...
	doNilaiChange: function(){
		try{
			...
			for (var i = 0; i < this.sg.rows.getLength();i++){
				...
				if (this.sg.rowValid(i) && this.sg.cells(8,i) != "" && this.sg.cells(0,i) == "NONAPP"){
					tot2 += nilaiToFloat(this.sg.cells(8,i));						
				}
			}							
			this.e_totalapp.setText(floatToNilai(tot1));
			this.e_totalnon.setText(floatToNilai(tot2));
		}catch(e)
		{
