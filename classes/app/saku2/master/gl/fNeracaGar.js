window.app_saku2_master_gl_fNeracaGar = function(owner)
{
	if (owner)
	{
		window.app_saku2_master_gl_fNeracaGar.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku2_master_gl_fNeracaGar";
...
window.app_saku2_master_gl_fNeracaGar.extend(window.portalui_childForm);
window.app_saku2_master_gl_fNeracaGar.implement({
			...
			switch (event)
			{
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
						...
						{
							var modul = "B";//
							var sql2,sql1 = "delete from neracagar where kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"'";					// kode_lokasi,
							var sql = new server_util_arrayList();
							sql.add("delete from relakungar  where kode_lokasi = '"+this.elokasi.getText()+"' and kode_fs = '"+this.eKlp.getText()+"' "+
								"and kode_neraca in (select kode_neraca from neracagar where  kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"')");																		
							sql.add(sql1);							
							//if mysql langsung
							...
							if (driver.search("mssql") != -1){
								...
								sql.add(sql2);
								if (this.value2 != ""){
										var sql3 = "insert into relakungar(kode_neraca, kode_fs, kode_akun,  kode_lokasi) values ";								
		...
		{ 
		 	if (sender == this.relBtn)
			{
				...
				if (this.selectedItem.data.get("tipe").toLowerCase() == "posting"){
					uses("app_saku2_master_gl_fRelakunGar",true);
					if (this.relakun != undefined)
						this.relakun.free();
					this.relakun = new app_saku2_master_gl_fRelakunGar(this.app);
			...
			}else
			{
				if (sender == this.addBtn || sender == this.editBtn){
					uses("app_saku2_master_gl_fNeracaGarDetail");
					if (this.entryMenu == undefined)
						this.entryMenu = new app_saku2_master_gl_fNeracaGarDetail(this.app);
