window.app_saku2_master_gl_fFsCf = function(owner)
{
	if (owner)
	{
		window.app_saku2_master_gl_fFsCf.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_master_gl_fFsCf";
		this.setTop(60);
		this.setWidth(1280);
		this.setHeight(550);
...
//------------------------------------------------------------------------
		try{
			...
			this.e0.setSQL("select kode_fs, nama from fscf where kode_lokasi = '"+this.app._lokasi+"' ",["kode_fs","nama"],undefined,["Kode FS","Nama"],"and","Data Versi Laporan",false);
		}catch(e)
		{
			alert(e);
		}
...
window.app_saku2_master_gl_fFsCf.extend(window.portalui_childForm);
//------------------------------------------------------------------ event
window.app_saku2_master_gl_fFsCf.implement({
		...
		try{
			...
			switch (event)
			{
				...
				case "simpan" :
						...
						sql.add("insert into fscf (kode_fs, kode_lokasi,nama,flag_status, tgl_input,nik_user ) values ('"+this.e0.getText()+"','"+this.app._lokasi+"','"+this.e1.getText()+"','"+status+"','"+(new Date).getDateStr()+"','"+this.app._userLog+"') ");
						this.dbLib.execArraySQL(sql);					
					break;
				case "ubah" :					
						uses("server_util_arrayList");
						sql = new server_util_arrayList();
						sql.add("update fscf set nama = '"+this.e1.getText()+"',flag_status = '"+status+"',nik_user='"+this.app._userLog+"' where kode_fs = '"+this.e0.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
						this.dbLib.execArraySQL(sql);							
					break;
				case "hapus" :
				    uses("server_util_arrayList");
						...
						sql.add("delete from  fscf where kode_fs = '"+this.e0.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
						this.dbLib.execArraySQL(sql);											   
					break;
			}
			this.e0.setFocus();
	...
	doEditChange: function(sender){
		...
		if(sender == this.e0){
			if (this.e0.getText() != "")
			{
				try
				{			
					var data = this.dbLib.runSQL("select * from fscf where kode_fs = '"+this.e0.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'  ");
