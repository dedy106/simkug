window.app_saku2_master_gl_fNeraca = function(owner)
{
	if (owner)
	{
		window.app_saku2_master_gl_fNeraca.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku2_master_gl_fNeraca";
...
window.app_saku2_master_gl_fNeraca.extend(window.portalui_childForm);
window.app_saku2_master_gl_fNeraca.implement({
		...
		{ 
			...
			}else
			{
				if (sender == this.addBtn || sender == this.editBtn){
					uses("app_saku2_master_gl_fNeracaDetail");
					if (this.entryMenu == undefined)
						this.entryMenu = new app_saku2_master_gl_fNeracaDetail(this.app,this.e0.getText());
