window.app_saku2_master_gl_fNeracaProduk = function(owner)
{
	if (owner)
	{
		window.app_saku2_master_gl_fNeracaProduk.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku2_master_gl_fNeracaProduk";
...
window.app_saku2_master_gl_fNeracaProduk.extend(window.portalui_childForm);
window.app_saku2_master_gl_fNeracaProduk.implement({
			...
			switch (event)
			{
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
						...
						{
							var modul = "L";//
							var sql2,sql1 = "delete from neracaproduk where kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"'";					// kode_lokasi,
							var sql = new server_util_arrayList();
							sql.add("delete from relakunproduk  where kode_lokasi = '"+this.elokasi.getText()+"' and kode_fs = '"+this.eKlp.getText()+"' "+
								"and kode_neraca in (select kode_neraca from neracaproduk where  kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"')");																		
							sql.add(sql1);							
							//if mysql langsung
							...
							if (driver.search("mssql") != -1){
								...
								for (var i in data){
									sql2 = "insert into neracaproduk(kode_neraca, kode_lokasi, kode_fs, nama, level_spasi, level_lap, tipe, sum_header, jenis_akun, kode_induk, rowindex, modul) values ";
									sql2 += " "+ data[i] +" ";
									sql.add(sql2);
								}	
								if (this.value2 != ""){
									...
									for (var i in data){
										sql3 = "insert into relakunproduk(kode_neraca, kode_fs, kode_akun, kode_lokasi) values ";
										sql3 += " "+ data[i] +" ;";
										sql.add(sql3);															
									}													
								}
								...
								sql.add(sql2);
								if (this.value2 != ""){
										var sql3 = "insert into relakunproduk(kode_neraca, kode_fs, kode_akun,  kode_lokasi) values ";								
	...
	loadMenu: function(daftarNrc){
		try
		{
			...
			var item = undefined;
			var node = undefined;
			var line, ix = -1, nrcRls, lineData, nrc_d = this.dblib.loadQuery("select kode_neraca, kode_akun from relakunproduk where kode_lokasi ='"+this.elokasi.getText()+"' and kode_fs = '"+this.eKlp.getText()+"' order by kode_neraca");												
	...
	doClick: function(sender){
		var nrc = this.dblib.runSQL("select * from neracaproduk where kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"'  order by rowindex",0,0);
		this.loadMenu(nrc);
		...
		{ 
			...
			}else
			{
				if (sender == this.addBtn || sender == this.editBtn){
					uses("app_saku2_master_gl_fNeracaProdukDetail");
					if (this.entryMenu == undefined)
						this.entryMenu = new app_saku2_master_gl_fNeracaProdukDetail(this.app);
