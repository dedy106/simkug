window.app_saku2_master_gl_fMasakunLokasi = function(owner)
{
	if (owner)
	{
		window.app_saku2_master_gl_fMasakunLokasi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_master_gl_fMasakunLokasi";
...
window.app_saku2_master_gl_fMasakunLokasi.extend(window.childForm);
window.app_saku2_master_gl_fMasakunLokasi.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					...
					sql.add("update masakun set nama='"+this.e_nama.getText()+"',modul='"+this.c_modul.getText().substr(0,1)+"',jenis='"+this.c_jenis.getText()+"',block='"+status+"' where kode_akun = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
