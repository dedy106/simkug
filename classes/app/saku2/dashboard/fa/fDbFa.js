window.app_saku2_dashboard_fa_fDbFa = function(owner)
{
	if (owner)
	{
		window.app_saku2_dashboard_fa_fDbFa.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_dashboard_fa_fDbFa";
		...
		this.pc1 = new pageControl(this,{bound:[15,12,1000,500], childPage:["Filter","Kelompok Asset","Data Trail"]});
        ...
        this.fr1 = new panel(this.pc1.childPage[1],{bound:[10,10,frameWidth,480],caption:"Kelompok Asset"});									            		        							
		...
		this.fr1.grid = new saiGrid(this.fr1,{bound:[5,30,this.fr1.getClientWidth() - 30,this.fr1.getClientHeight() - 50],colCount:4,colTitle:["Group By","Nilai Perolehan","Akumulasi","Nila Buku"],visible:false, readOnly:true, dblClick:[this,"doGridClick"],colWidth:[[1,0],[100,200]], colAlign:[[1,2,3],[alCenter,alCenter,alCenter]]});		         	                        
...
window.app_saku2_dashboard_fa_fDbFa.extend(window.childForm);
window.app_saku2_dashboard_fa_fDbFa.implement({
	...
	mainButtonClick:function(sender)
	{
		try
		{
			...
			else
			{
				...
				sql.add("select a.kode_klpakun+'-'+a.nama as nama, "+
						"	   round(b.so_awal/1000,0) as tot,round(c.ap/1000,0) as ap,round((b.so_awal-c.ap)/1000,0) as buku "+
						"from fa_klpakun a "+
						"left join (select a.kode_klpakun,a.kode_lokasi,sum(a.nilai) as so_awal "+
						"		   from fa_asset a "+
						...
						"left join (select a.kode_klpakun,a.kode_lokasi,sum(b.nilai) as ap "+
						"		   from fa_asset a "+
						...
						"where a.kode_lokasi='"+kode_lokasi+"' and b.so_awal is not null "+
	...
	getChart: function(data,swf,autoRefresh){
		...
		if (data.rs.rows[0]!==undefined)
		{
			...
			temp3 = {"type":"bar_3d","alpha":     1.8,"colour":    "#66FF33",
			...
			for (var i in data.rs.rows){
				...
				temp2.values.push(parseFloat(line.ap));
				temp3.values.push(parseFloat(line.buku));
	...
	doChartClick: function(sender,index,title){	
		...
		try{
			...
			var tmp=title.split('-');
			var kode_akun=tmp[0];
			this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+kode_akun;
			
			this.report = new server_report_report();
			var kode_lokasi=this.sg1.getCell(2,0);
			this.showFilter = "";
			this.lokasi = "";
			this.filter=" where a.kode_klpakun='"+kode_akun+"' and a.kode_lokasi='"+this.sg1.getCell(2,0)+"'";
			this.docViewer.src = this.report.previewWithHeader("server_report_saku2_fa_rptFaAssetTrail",this.filter, 1,1, this.showFilter, this.lokasi,this.filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId();
	...
	setData: function(grid, data){
		grid.clear();
		for (var i in data.rs.rows){
			grid.appendData([data.rs.rows[i].nama, data.rs.rows[i].tot,data.rs.rows[i].ap,data.rs.rows[i].buku]);
		}	
	...
	doOpenBbAsset: function(no_fa,kode_lokasi){
		...
		nama_report="server_report_saku2_fa_rptFaKartuTrail";
