window.app_saku2_dashboard_sju_fDbNeraca = function(owner)
{
	if (owner)
	{
		window.app_saku2_dashboard_sju_fDbNeraca.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_dashboard_sju_fDbNeraca";
		...
		this.pc1 = new pageControl(this,{bound:[15,12,1000,500], childPage:["Filter","Aktiva Lancar","Aktiva Tetap","Kewajiban","Pendapatan","Beban","Data Trail"]});
...
window.app_saku2_dashboard_sju_fDbNeraca.extend(window.childForm);
window.app_saku2_dashboard_sju_fDbNeraca.implement({
	...
	mainButtonClick:function(sender)
	{
		try
		{
			...
			else
			{
				...
				sql.add("select a.kode_neraca+'-'+b.nama as nama,abs(round(b.n2/1000,0)) as tot "+
						"from db_grafik_d a "+
	...
	doBtnClick: function(sender){
		try{
			if (sender == this.fr1.bXls ){				
				...
				this.xlsChart.setFilename("aktiva-lancar.xls");
