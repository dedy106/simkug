window.app_saku2_dashboard_gl_fDbTbBidangTrail = function(owner)
{
	if (owner)
	{
		window.app_saku2_dashboard_gl_fDbTbBidangTrail.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_dashboard_gl_fDbTbBidangTrail";
...
window.app_saku2_dashboard_gl_fDbTbBidangTrail.extend(window.childForm);
window.app_saku2_dashboard_gl_fDbTbBidangTrail.implement({
	doEllipseClick:function(sender, col, row)
	{
		...
		if (row == 3)
		{
			var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
						" and a.block='0' and a.modul='L' ";
			this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,
												"select a.kode_akun,a.nama from masakun a "+filter, 
												"select count(a.kode_akun)  from masakun a "+filter,
	...
	mainButtonClick:function(sender)
	{
		try
		{
			...
			else
			{
				...
				var sql = "call sp_glma_bidang_tmp ('"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.sg1.getCell(2,1)+"','"+this.app._nikUser+"','"+this.kode_pp1+"','"+this.kode_pp2+"')";
	...
	doDoubleClick: function(sender, col , row) 
	{
		if (this.sg.cells(4,row)!=0 || this.sg.cells(5,row)!=0)
		{
			...
			var sql = "call sp_bb_bidang ('"+this.sg.cells(0,row)+"','"+this.sg.cells(0,row)+"','"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,1)+"','"+this.sg1.getCell(2,1)+"','"+this.app._nikUser+"','0','"+this.sg1.cells(2,2)+"')";
