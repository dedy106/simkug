window.app_saku2_dashboard_gar_fDbGar = function(owner)
{
	if (owner)
	{
		window.app_saku2_dashboard_gar_fDbGar.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_dashboard_gar_fDbGar";
		...
		this.pc1 = new pageControl(this,{bound:[15,12,1000,500], childPage:["Filter","RKAP","Investasi","Pendapatan Operasional","Pendapatan Non Operasional","Beban Operasinal","Beban Non Operasional","Data Trail"]});
		...
		this.fr5 = new panel(this.pc1.childPage[5],{bound:[10,10,frameWidth,480],caption:"Beban Operasional"});									            		        							
		...
		this.fr5.bXls = new imageButton(this.fr5,{bound:[frameWidth - 75,2,18,16],image:"icon/dynpro/excel2.png",hint:"Save to excel",click:[this,"doBtnClick"], name:"xls"});
		...
		this.fr5.chart = new flashChart(this.fr5,{bound:[5,30,this.fr5.getClientWidth() - 30,this.fr5.getClientHeight() - 50],objectReady:[this,"doObjectReady"],title:"Lokasi"});		         	                        

		this.fr6 = new panel(this.pc1.childPage[6],{bound:[10,10,frameWidth,480],caption:"Pendapatan Pendidikan"});									            		        							
...
window.app_saku2_dashboard_gar_fDbGar.extend(window.childForm);
window.app_saku2_dashboard_gar_fDbGar.implement({
	...
	mainButtonClick:function(sender)
	{
		try
		{
			...
			else
			{
				...
				sql="call sp_neracagar_konsol ('"+this.sg1.getCell(2,2)+"','N','K',5,'"+this.sg1.getCell(2,1)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.app._nikUser+"')";
			
				this.dbLib.execQuerySync(sql);
				this.pc1.setActivePage(this.pc1.childPage[1]);
				var sql = new server_util_arrayList();
				sql.add("select a.kode_neraca+'-'+b.nama as nama,abs(round(b.n1/1000,0)) as tot "+
						"from db_grafik_d a "+
						"inner join neraca_tmp b on a.kode_lokasi=b.kode_lokasi and a.kode_neraca=b.kode_neraca "+
						"where b.nik_user='"+this.app._nikUser+"' and a.kode_grafik='GA01' order by a.no");
				sql.add("select a.kode_neraca+'-'+b.nama as nama,abs(round(b.n4/1000,0)) as tot "+
						"from db_grafik_d a "+
						"inner join neraca_tmp b on a.kode_lokasi=b.kode_lokasi and a.kode_neraca=b.kode_neraca "+
						"where b.nik_user='"+this.app._nikUser+"' and a.kode_grafik='GA02' order by a.no");
				sql.add("select a.kode_neraca+'-'+b.nama as nama,abs(round(b.n4/1000,0)) as tot "+
						"from db_grafik_d a "+
						"inner join neraca_tmp b on a.kode_lokasi=b.kode_lokasi and a.kode_neraca=b.kode_neraca "+
    	       ...
    	       switch(methodName){
    	           case "getMultiDataProvider":					
    	                ...
    	                if (typeof data != "string"){
            		        ...
            		        this.setData(this.fr6.grid,data.result[5]);
