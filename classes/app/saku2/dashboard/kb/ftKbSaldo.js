window.app_saku2_dashboard_kb_ftKbSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku2_dashboard_kb_ftKbSaldo.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_dashboard_kb_ftKbSaldo";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Trail Saldo KasBank", 2);	
...
window.app_saku2_dashboard_kb_ftKbSaldo.extend(window.childForm);
window.app_saku2_dashboard_kb_ftKbSaldo.implement({
	...
	mainButtonClick:function(sender)
	{
		try
		{
			...
			else
			{
				...
				var sql = "call sp_glma_kas_tmp ('"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.sg1.getCell(2,1)+"','"+this.app._nikUser+"')";
	...
	{
		var tabel = "(select * from kas_j where no_kas='"+this.sg2.cells(0,row)+"' and periode='"+this.sg1.getCell(2,1)+"' and kode_lokasi='"+this.sg1.getCell(2,0)+"')";
		var strSQL= "select a.no_kas,a.no_dokumen,date_format(tanggal,'%d/%m/%Y') as tanggal,a.kode_akun,b.nama as nama_akun,a.kode_pp,a.kode_drk,a.keterangan,case when a.dc='D' then nilai else 0 end as debet,"+
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU.rs.rows[i];							
			this.sg3.appendData([line.no_kas,line.no_dokumen,line.tanggal,line.kode_akun,line.nama_akun,line.kode_pp,line.kode_drk,line.keterangan,floatToNilai(line.debet),floatToNilai(line.kredit)]);
		}
