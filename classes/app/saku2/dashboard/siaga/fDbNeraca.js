window.app_saku2_dashboard_siaga_fDbNeraca = function(owner)
{
	if (owner)
	{
		window.app_saku2_dashboard_siaga_fDbNeraca.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku2_dashboard_siaga_fDbNeraca";
		...
		this.pc1 = new pageControl(this,{bound:[15,12,1000,500], childPage:["Filter","Aktiva Lancar","Aktiva Tetap","Kewajiban","Pendapatan Operasional","Beban Operasinal","Pendapatan Pendidikan","Beban Pendidikan","Data Trail"]});
		...
		this.fr7 = new panel(this.pc1.childPage[7],{bound:[10,10,frameWidth,480],caption:"Beban Pendidikan"});									            		        							
...
window.app_saku2_dashboard_siaga_fDbNeraca.extend(window.childForm);
window.app_saku2_dashboard_siaga_fDbNeraca.implement({
		...
		try
		{
			...
			else
			{
				...
				sql.add("select a.kode_neraca+'-'+b.nama as nama,abs(round(b.n4/1000,0)) as tot "+
						...
						"where b.nik_user='"+this.app._nikUser+"' and a.kode_grafik='DB01' order by a.no");
				sql.add("select a.kode_neraca+'-'+b.nama as nama,abs(round(b.n4/1000,0)) as tot "+
						"from db_grafik_d a "+
						"inner join neraca_tmp b on a.kode_lokasi=b.kode_lokasi and a.kode_neraca=b.kode_neraca "+
				...
				sql.add("select a.kode_neraca+'-'+b.nama as nama,abs(round(b.n4/1000,0)) as tot "+
						"from db_grafik_d a "+
						"inner join neraca_tmp b on a.kode_lokasi=b.kode_lokasi and a.kode_neraca=b.kode_neraca "+
						"where b.nik_user='"+this.app._nikUser+"' and a.kode_grafik='DB03' order by a.no");
				sql.add("select a.kode_neraca+'-'+b.nama as nama,abs(round(b.n4/1000,0)) as tot "+
						"from db_grafik_d a "+
						"inner join neraca_tmp b on a.kode_lokasi=b.kode_lokasi and a.kode_neraca=b.kode_neraca "+
    	       ...
    	       switch(methodName){
    	           case "getMultiDataProvider":					
    	                ...
    	                if (typeof data != "string"){
            		        ...
            		        this.setData(this.fr7.grid,data.result[6]);
							this.getChart(this.fr7.chart.data,this.fr7.chart);
							this.fr7.chart.refresh();
						}else throw result;
    	           break;
