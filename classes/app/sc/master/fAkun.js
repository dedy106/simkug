window.app_sc_master_fAkun = function(owner)
{
	if (owner)
	{
		window.app_sc_master_fAkun.prototype.parent.constructor.call(this,owner);
		this.className  = "app_sc_master_fAkun";
		...
		this.p1 = new panel(this,{bound:[10,23,600,403],caption:"Daftar Akun"});
...
window.app_sc_master_fAkun.extend(window.childForm);
window.app_sc_master_fAkun.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try{									
					...
					sql.add("insert into masakun(kode_akun,nama,kode_lokasi) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update masakun set nama = '"+this.e_nama.getText()+"' where kode_akun = '"+this.cb_kode.getText()+"'");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
		...
		try{
			if (this.cb_kode.getText() != ""){
				var data = this.dbLib.getDataProvider("select nama  "+
				           " from masakun where kode_akun ='"+this.cb_kode.getText()+"'",true);
	...
	doTampilClick: function(sender){
		try{			
			var temp = this.dbLib.runSQL("select kode_akun,nama from masakun order by kode_akun");
		...
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Akun",sender,undefined, 
											  ...
											  ["kode_akun","nama"],"where",["Kode","Nama"],false);				
			}
		}catch(e){
			systemAPI.alert(e);
		}
