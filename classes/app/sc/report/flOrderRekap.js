window.app_sc_report_flOrderRekap = function(owner)
{
	if (owner)
	{
		window.app_sc_report_flOrderRekap.prototype.parent.constructor.call(this,owner);
		this.className = "app_sc_report_flOrderRekap";
		this.maximize();
...
window.app_sc_report_flOrderRekap.extend(window.childForm);
window.app_sc_report_flOrderRekap.implement({
		...
		try{			
			...
			if (row == 3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_jurnal, keterangan from sc_jurnal_m "+filter,
														  "select count(no_jurnal) from sc_jurnal_m"+filter,
														  ["no_jurnal","keterangan"],"and",["No Bukti","Keterangan"]);
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from sc_jurnal_m order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.peride",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
							  ...
							  this.filterRep.filterStr("a.no_jurnal",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");

				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_sc_rptOrderRekap";
			...
			systemAPI.alert("[app_sc_report_flOrderRekap]::mainButtonClick:"+e);
		}
	},
