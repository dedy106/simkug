window.app_sc_report_flPesanTerima = function(owner)
{
	if (owner)
	{
		window.app_sc_report_flPesanTerima.prototype.parent.constructor.call(this,owner);
		this.className = "app_sc_report_flPesanTerima";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sc_terima_m ");
...
window.app_sc_report_flPesanTerima.extend(window.childForm);
window.app_sc_report_flPesanTerima.implement({
		...
		try{			
			...
			if (row == 3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_terima, keterangan from sc_terima_m "+filter,
														  "select count(no_terima) from sc_terima_m"+filter,
	...
	{		
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from sc_terima_m order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_sc_rptPesanTerima";
			...
			systemAPI.alert("[app_sc_report_flPesanTerima]::mainButtonClick:"+e);
		}
	},
