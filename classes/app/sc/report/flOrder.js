window.app_sc_report_flOrder = function(owner)
{
	if (owner)
	{
		window.app_sc_report_flOrder.prototype.parent.constructor.call(this,owner);
		this.className = "app_sc_report_flOrder";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sc_po_m ");
...
window.app_sc_report_flOrder.extend(window.childForm);
window.app_sc_report_flOrder.implement({
		...
		try{			
			...
			if (row == 3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_po, keterangan from sc_po_m "+filter,
														  "select count(no_po) from sc_po_m"+filter,
														  ["no_po","keterangan"],"and",["No Bukti","Keterangan"]);
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from sc_po_m order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_sc_rptOrder";
			...
			systemAPI.alert("[app_sc_report_flOrder]::mainButtonClick:"+e);
		}
	},
