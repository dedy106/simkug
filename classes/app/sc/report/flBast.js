window.app_sc_report_flBast = function(owner)
{
	if (owner)
	{
		window.app_sc_report_flBast.prototype.parent.constructor.call(this,owner);
		this.className = "app_sc_report_flBast";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sc_bast_m ");
...
window.app_sc_report_flBast.extend(window.childForm);
window.app_sc_report_flBast.implement({
		...
		try{			
			...
			if (row == 3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_bast, keterangan from sc_bast_m "+filter,
														  "select count(no_bast) from sc_bast_m"+filter,
														  ["no_bast","keterangan"],"and",["No Bukti","Keterangan"]);
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from sc_bast_m order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_sc_rptBast";
			...
			systemAPI.alert("[app_sc_report_flBast]::mainButtonClick:"+e);
		}
	},
