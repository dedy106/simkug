window.app_mina_dashboard = function(owner,options)
{
	if (owner)
	{
		window.app_mina_dashboard.prototype.parent.constructor.call(this,owner,options);
		this.className  = "app_mina_dashboard";
		...
		try {
			...
			this.fileUtil.setFilename(this.rootDir + "/server/media/documents/mina/" + this.app._userLog);		
			this.rootPath = this.rootDir + "/server/media/documents/mina/" + this.app._userLog;		
			this.prosesId = 1; 
			if (!this.fileUtil.isDir()){
				this.fileUtil.createDir();
			}else{
				this.tFile.setUsrRoot(this.app._userLog,"server/media/documents/mina/" + this.app._userLog,"My Document");
			}		
			this.initload();
			...
			this.dataProvider = new app_mina_remote_dataProvider(this.app._dbSetting);
...
window.app_mina_dashboard.extend(window.panel);
window.app_mina_dashboard.implement({	
