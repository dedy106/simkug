...
//* 			SAI, PT											
//***********************************************************************************************
window.app_mina_fMain = function(owner)
{
	if (owner)
	{	
		try
		{
			window.app_mina_fMain.prototype.parent.constructor.call(this, owner);			
...
window.app_mina_fMain.extend(window.form);
window.app_mina_fMain.implement({
	gotoLogin: function(){
		uses("app_mina_fLogin");
		this.form = new app_mina_fLogin(this);
	...
	initDashboard: function(){		
		uses("saiCBBL;saiEdit;saiGrid;;util_ajaxCaller;sgNavigator;pageControl;app_mina_dashboard;fileExplorer;listView;util_file;PopUpMenu;MenuItem;app_mina_remote_dataProvider");
		this.dashboard = new app_mina_dashboard(this,{bound:[this.treev.width + 20, this.treev.top, this.width - this.treev.width - 30, this.treev.height]});		
	},
				...
				if (child !== undefined){
					...
					if (child.grid){						
						...
						if (child.data.rs.rows[0]!== undefined){
							var line,temp="", maxValue = -999999999,mminaalue = 999999999;
							for (var i in child.data.rs.rows){
								...
								if (parseFloat(line.n1) < mminaalue ) mminaalue = parseFloat(line.n1);
