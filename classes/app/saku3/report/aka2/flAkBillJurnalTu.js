window.app_saku3_report_aka2_flAkBillJurnalTu = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_aka2_flAkBillJurnalTu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_aka2_flAkBillJurnalTu";
		this.maximize();
...
window.app_saku3_report_aka2_flAkBillJurnalTu.extend(window.childForm);
window.app_saku3_report_aka2_flAkBillJurnalTu.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_bill, a.keterangan from aka_bill_m a "+filter,
												"select count(a.no_bill) from aka_bill_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from aka_bill_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_bill",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_aka2_rptAkBillJurnalTu";
			...
			systemAPI.alert("[app_saku3_report_aka2_flAkBillJurnalTu]::mainButtonClick:"+e);
		}
	},
	...
	doOpenDetTagihan: function(kode_pp,kode_lokasi,kode_akun,no_bill,jenis,periode){
		this.link="2";
		filter = "where a.no_bill='"+no_bill+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		
		if(kode_pp.substr(0,1) == "5"){
			var filter3 = "where a.no_bill='"+no_bill+"' and (a.akun_piutang='"+kode_akun+"' or a.akun_pdd='"+kode_akun+"' or a.akun_pdpt='"+kode_akun+"' )  and a.kode_pp='"+kode_pp+"' and a.kode_produk='"+jenis+"' and a.kode_lokasi='"+kode_lokasi+"'";

		}else{
			var filter3 = "where a.no_bill='"+no_bill+"' and (a.akun_piutang='"+kode_akun+"' or a.akun_pdd='"+kode_akun+"' or a.akun_pdpt='"+kode_akun+"' ) and a.kode_produk='"+jenis+"' and a.kode_lokasi='"+kode_lokasi+"'";
		}
		filter2= periode+"/Layar//"+filter3+"/"+jenis;
		nama_report="server_report_saku3_aka2_rptAkBillTu2";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
