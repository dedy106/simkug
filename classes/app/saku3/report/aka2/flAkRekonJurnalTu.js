window.app_saku3_report_aka2_flAkRekonJurnalTu = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_aka2_flAkRekonJurnalTu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_aka2_flAkRekonJurnalTu";
		this.maximize();
...
window.app_saku3_report_aka2_flAkRekonJurnalTu.extend(window.childForm);
window.app_saku3_report_aka2_flAkRekonJurnalTu.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_rekon, a.keterangan from aka_rekon_m a "+filter,
												"select count(a.no_rekon) from aka_rekon_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from aka_rekon_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_aka2_rptAkRekonJurnalTu";
			...
			systemAPI.alert("[app_saku3_report_aka2_flAkRekonJurnalTu]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBayar: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter = "where a.no_rekon='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
	...
	doOpenDetRekon: function(kode_pp,kode_lokasi,kode_akun,no_rekon,periode){
		this.link="2";
		filter = "where a.no_rekon='"+no_rekon+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		if(kode_pp.substr(0,1) == "5"){
			var filter3 = "where a.no_rekon='"+no_rekon+"' and (a.akun_piutang='"+kode_akun+"' or a.akun_titip='"+kode_akun+"')  and a.kode_lokasi='"+kode_lokasi+"' and c.kode_jur='"+kode_pp+"'  ";

		}else{
			var filter3 = "where a.no_rekon='"+no_rekon+"' and (a.akun_piutang='"+kode_akun+"' or a.akun_titip='"+kode_akun+"')  and a.kode_lokasi='"+kode_lokasi+"' ";
		}
		filter2= periode+"/Layar/"+kode_lokasi+"//"+filter3;

		nama_report="server_report_saku3_aka2_rptAkRekonTu2";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
