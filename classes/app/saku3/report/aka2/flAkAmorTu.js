window.app_saku3_report_aka2_flAkAmorTu = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_aka2_flAkAmorTu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_aka2_flAkAmorTu";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from aka_amor_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_aka2_flAkAmorTu.extend(window.childForm);
window.app_saku3_report_aka2_flAkAmorTu.implement({
		...
		try{			
			...
			if (row ==5)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												...
												new Array("a.no_amor","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}		
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (this.app._userStatus=="A")
		{	
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["3","123","123","123","123","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[2,0,2,2,2,2,0]);
		}else{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["3","123","3","3","123","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[2,0,2,2,2,2,0]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from aka_amor_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 6)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  ...
							  this.filterRep.filterStr("a.no_amor",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,6);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_aka2_rptAkAmorTu";
