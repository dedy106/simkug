window.app_saku3_report_aka2_flAkKartuTu = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_aka2_flAkKartuTu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_aka2_flAkKartuTu";
		this.maximize();
	...
	if (this.app._userStatus=="A")
		{
			...
			this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Fakultas","All",""));
			this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Jurusan","All",""));
			this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Status Piutang","All",""));
			this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("NIM","All",""));
			this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Pembatalan","=","Tidak"));
		...
		this.fak = this.dbLib.getPeriodeFromSQL("select top 1 a.kode_fakultas as periode from aka_fakultas a left join pp b on a.kode_fakultas=b.kode_bidang and a.kode_lokasi=b.kode_lokasi left join karyawan_pp c on b.kode_pp=c.kode_pp and b.kode_lokasi=c.kode_lokasi where a.kode_lokasi='"+this.app._lokasi+"' and c.nik='"+this.app._userLog+"' ");
			this.jur = this.dbLib.getPeriodeFromSQL("select top 1 a.kode_jur as periode from aka_jurusan a left join pp b on a.kode_fakultas=b.kode_bidang and a.kode_lokasi=b.kode_lokasi left join karyawan_pp c on b.kode_pp=c.kode_pp and b.kode_lokasi=c.kode_lokasi where a.kode_lokasi='"+this.app._lokasi+"' and c.nik='"+this.app._userLog+"' ");
			this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
			this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Fakultas","=",this.fak));
			this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Jurusan","=",this.jur));
			this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Status Piutang","All",""));
			this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("NIM","All",""));
			this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Pembatalan","=","Tidak"));
	}
...
window.app_saku3_report_aka2_flAkKartuTu.extend(window.childForm);
window.app_saku3_report_aka2_flAkKartuTu.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				{	
					...
					this.filterRep.filterStr("a.kode_fakultas",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," and ");	
			...
			// 	this.filterRep.ListDataSGFilter(this, "Data Status Tagih",this.sg1, this.sg1.row, this.sg1.col,
			// 											  "select distinct a.sts_tagih from aka_mahasiswa a   "+filter,
			// 											  "select count(distinct a.sts_tagih) from aka_mahasiswa a "+filter,
			// 											  ["a.sts_tagih"],"and",["Status"]);
			// }
			if (row == 4)
			{
							 if(this.sg1.getCell(1,3) != "All"){
									var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
									this.filterRep.filterStr("b.kode_fakultas",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," and ")+
									this.filterRep.filterStr("a.kode_jur",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," and ")+
									this.filterRep.filterStr("a.sts_tagih",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3)," and ");
							 }else{
									var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
									this.filterRep.filterStr("b.kode_fakultas",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," and ")+
									this.filterRep.filterStr("a.kode_jur",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," and ");
	...
	{		
		...
		{	
			...
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,2,2,0,2,0]);
		}else{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["3","123","3","3","123","13"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,2,2,0,2,0]);
		}	
		if (row == 5)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("NIM","All",""));
			}else
			{
				...
				this.nama_report = "server_report_saku3_aka2_rptAkKartuTu";
