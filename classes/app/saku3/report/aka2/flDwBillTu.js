window.app_saku3_report_aka2_flDwBillTu = function (owner) {
	if (owner) {
		window.app_saku3_report_aka2_flDwBillTu.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_report_aka2_flDwBillTu";
		this.maximize();
		...
		this.sg1 = new saiGrid(this.p1, {
			bound: [1, 20, 700, 207], colCount: 4, cellExit: [this, "doCellExit"],
	...
	this.periode = this.dbLib.getPeriodeFromSQL("select max(periode) as periode from aka_bill_m where kode_lokasi='" + this.app._lokasi + "' ");
		...
		this.gridLib.SGEditData(this.sg1, 3, new Array(0, 1, 2), new Array("Jurusan","=",this.app._kodePP));
		...
		this.gridLib.SGEditData(this.sg1, 6, new Array(0, 1, 2), new Array("Tampilan Laporan", "=", "Layar"));
		this.gridLib.SGEditData(this.sg1, 7, new Array(0, 1, 2), new Array("Tahun Akademik ", "All", ""));
		this.gridLib.SGEditData(this.sg1, 8, new Array(0, 1, 2), new Array("Status Tagihan", "All", ""));
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_aka2_flDwBillTu.extend(window.childForm);
window.app_saku3_report_aka2_flDwBillTu.implement({
		...
		try {
			...
			if (row == 3) {
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurusan", this.sg1, this.sg1.row, this.sg1.col,
					 "select distinct a.kode_jur, a.nama from aka_jurusan a left join pp b on a.kode_fakultas=b.kode_bidang and a.kode_lokasi=b.kode_lokasi left join karyawan_pp c on b.kode_pp=c.kode_pp and b.kode_lokasi=c.kode_lokasi   "+filter+" and c.nik='"+this.app._userLog+"'",
														  "select count(a.kode_jur) from aka_jurusan a left join pp b on a.kode_fakultas=b.kode_bidang and a.kode_lokasi=b.kode_lokasi left join karyawan_pp c on b.kode_pp=c.kode_pp and b.kode_lokasi=c.kode_lokasi "+filter+" and c.nik='"+this.app._userLog+"' ",
			...
			if (row == 5) {
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice", this.sg1, this.sg1.row, this.sg1.col,
					"select a.no_bill,a.keterangan from aka_bill_m a " + this.filter,
					"select count(a.no_bill) from aka_bill_m a " + this.filter,
	...
	doSelectCell: function (sender, col, row) {
		if (this.app._userStatus=="A")
		{		
			this.filterRep.setSGFilterRowTipe(this.sg1, row, [0, 1, 2, 3, 4, 5, 6, 7, 8], ["3", "123", "123", "123", "123", "123", "3", "13", "13"]);			
		}else{
			this.filterRep.setSGFilterRowTipe(this.sg1, row, [0, 1, 2, 3, 4, 5, 6, 7, 8], ["3", "123", "3", "123", "123", "123", "3", "13", "13"]);
		}
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row, [0, 1, 2, 3, 4, 5, 6, 7, 8], [2, 0, 2, 2, 2, 2, 0, 0, 0]);
		if (row == 1) {
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from aka_bill_m where kode_lokasi='" + this.app._lokasi + "' order by periode desc", [this.sg1.columns.get(2).pickList]);
		}
		if (row == 6) {
			this.sg1.columns.get(2).pickList.clear();
		...
		if (row == 7) {
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct tahunaka from aka_tahunaka where kode_lokasi='" + this.app._lokasi + "' order by tahunaka desc", [this.sg1.columns.get(2).pickList]);
		}
		if (row == 8) {
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList, new Array("All", "Aktif", "Batal"));
		}
	},
	mainButtonClick: function (sender) {
		try {
			if (sender === this.app._mainForm.bClear2) {
				...
				this.filter = this.filterRep.filterStr("a.kode_lokasi", this.sg1.getCell(1, 0), this.sg1.getCell(2, 0), this.sg1.getCell(3, 0), "where") +
					this.filterRep.filterStr("a.periode", this.sg1.getCell(1, 1), this.sg1.getCell(2, 1), this.sg1.getCell(3, 1), "and") +
					this.filterRep.filterStr("c.kode_fakultas", this.sg1.getCell(1, 2), this.sg1.getCell(2, 2), this.sg1.getCell(3, 2), "and") +
					...
					this.filterRep.filterStr("a.no_bill", this.sg1.getCell(1, 5), this.sg1.getCell(2, 5), this.sg1.getCell(3, 5), "and") +
					this.filterRep.filterStr("a.tahunaka", this.sg1.getCell(1, 7), this.sg1.getCell(2, 7), this.sg1.getCell(3, 7), "and");
				
				var filter3 = this.filterRep.filterStr("a.kode_lokasi", this.sg1.getCell(1, 0), this.sg1.getCell(2, 0), this.sg1.getCell(3, 0), "where") +
					this.filterRep.filterStr("c.kode_fakultas", this.sg1.getCell(1, 2), this.sg1.getCell(2, 2), this.sg1.getCell(3, 2), "and") +
					this.filterRep.filterStr("c.kode_jur", this.sg1.getCell(1, 3), this.sg1.getCell(2, 3), this.sg1.getCell(3, 3), "and") +
					this.filterRep.filterStr("a.nim", this.sg1.getCell(1, 4), this.sg1.getCell(2, 4), this.sg1.getCell(3, 4), "and") ;
					
				this.filter2 = this.sg1.getCell(2, 0) +"/"+this.sg1.getCell(2, 1) + "/" + this.sg1.getCell(2, 6) + "/" + this.sg1.getCell(2, 8)+"/"+filter3;
				
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_aka2_rptDwBillTu";
