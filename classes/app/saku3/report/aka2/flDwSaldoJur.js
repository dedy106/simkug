window.app_saku3_report_aka2_flDwSaldoJur = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_aka2_flDwSaldoJur.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_aka2_flDwSaldoJur";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from exs_aka_saldo_jur where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_aka2_flDwSaldoJur.extend(window.childForm);
window.app_saku3_report_aka2_flDwSaldoJur.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from exs_aka_saldo_jur where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_aka2_rptDwSaldoJur";
	...
	doOpenSaldoMhs: function(kode_jur,kode_lokasi,kode_fakultas){
		this.link="1";
		this.kode_jur=kode_jur;
		this.kode_lokasi=kode_lokasi;
		this.kode_fakultas=kode_fakultas;
		this.link="1";
		filter = " where a.kode_lokasi='"+this.kode_lokasi+"' and a.kode_jur='"+this.kode_jur+"' and a.kode_fakultas='"+kode_fakultas+"'";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+kode_jur;
		nama_report="server_report_saku2_kopeg_aka_rptAkSaldoMhsTu";
