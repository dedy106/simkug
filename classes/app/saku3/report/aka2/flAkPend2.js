window.app_saku3_report_aka2_flAkPend2 = function (owner) {
	if (owner) {
		window.app_saku3_report_aka2_flAkPend2.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_report_aka2_flAkPend2";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick", "Laporan Pendapatan Pendidikan", 2);
...
window.app_saku3_report_aka2_flAkPend2.extend(window.childForm);
window.app_saku3_report_aka2_flAkPend2.implement({
		...
		try {
			...
			if (row == 5) {
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice", this.sg1, this.sg1.row, this.sg1.col,
					"select a.no_bill,a.keterangan from aka_bill_m a " + this.filter +" union all select a.no_batal as no_bill,a.keterangan from aka_batal_m a "+this.filter+" union all select a.no_amor as no_bill,a.keterangan from aka_amor_m a "+this.filter,
					"select count(a.no_bill) from (select a.no_bill from aka_bill_m a " + this.filter +" union all select a.no_batal as no_bill from aka_batal_m a "+ this.filter +"union all select a.no_amor as no_bill from aka_amor_m a "+ this.filter +") a  ",
	...
	doSelectCell: function (sender, col, row) {
		...
		if (row == 1) {
			...
			this.dbLib.setItemsFromSQL("select distinct periode from aka_bill_m where kode_lokasi='" + this.app._lokasi + "' order by periode desc", [this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 6) {
			this.sg1.columns.get(2).pickList.clear();
		...
		try {
			if (sender === this.app._mainForm.bClear2) {
				...
				this.filter3 = this.filterRep.filterStr("a.kode_lokasi", this.sg1.getCell(1, 0), this.sg1.getCell(2, 0), this.sg1.getCell(3, 0), "where") +
					...
					this.filterRep.filterStr("c.nim", this.sg1.getCell(1, 4), this.sg1.getCell(2, 4), this.sg1.getCell(3, 4), "and") +
                    this.filterRep.filterStr("f.tahunaka", this.sg1.getCell(1, 7), this.sg1.getCell(2, 7), this.sg1.getCell(3, 7), "and");
                this.filter4 = this.filterRep.filterStr("a.kode_lokasi", this.sg1.getCell(1, 0), this.sg1.getCell(2, 0), this.sg1.getCell(3, 0), "where") +
					this.filterRep.filterStr("a.periode", this.sg1.getCell(1, 1), this.sg1.getCell(2, 1), this.sg1.getCell(3, 1), "and") +
					this.filterRep.filterStr("d.kode_fakultas", this.sg1.getCell(1, 2), this.sg1.getCell(2, 2), this.sg1.getCell(3, 2), "and") +
					this.filterRep.filterStr("c.kode_jur", this.sg1.getCell(1, 3), this.sg1.getCell(2, 3), this.sg1.getCell(3, 3), "and") +
					this.filterRep.filterStr("c.nim", this.sg1.getCell(1, 4), this.sg1.getCell(2, 4), this.sg1.getCell(3, 4), "and") +
					this.filterRep.filterStr("b.no_amor", this.sg1.getCell(1, 5), this.sg1.getCell(2, 5), this.sg1.getCell(3, 5), "and") +
                    this.filterRep.filterStr("f.tahunaka", this.sg1.getCell(1, 7), this.sg1.getCell(2, 7), this.sg1.getCell(3, 7), "and");
                this.filter2 = this.sg1.getCell(2, 1) + "/" + this.sg1.getCell(2, 6) + "/" + this.sg1.getCell(2, 8)+"/"+this.filter4;
				
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_aka2_rptAkPend2";
