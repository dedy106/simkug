window.app_saku3_report_aka2_flAkRekonTu = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_aka2_flAkRekonTu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_aka2_flAkRekonTu";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from aka_rekon_m where kode_lokasi='"+this.app._lokasi+"' ");
	...
	{	
		...
		this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("No Batal","All",""));
		this.gridLib.SGEditData(this.sg1,7,new Array(0,1,2), new Array("Tampilan Laporan","=","Layar"));
		this.gridLib.SGEditData(this.sg1,8,new Array(0,1,2), new Array("Pembatalan","=","Tidak"));
	}else{
		this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
		this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Periode ","=",this.periode));
		...
		this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("No Batal","All",""));
		this.gridLib.SGEditData(this.sg1,7,new Array(0,1,2), new Array("Tampilan Laporan","=","Layar"));
		this.gridLib.SGEditData(this.sg1,8,new Array(0,1,2), new Array("Pembatalan","=","Ya"));
	}
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_aka2_flAkRekonTu.extend(window.childForm);
window.app_saku3_report_aka2_flAkRekonTu.implement({
		...
		try{			
			...
			if (row ==5)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Rekon",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_rekon,a.keterangan from aka_rekon_m a "+this.filter,
												"select count(a.no_rekon) from aka_rekon_m a "+this.filter,
												new Array("a.no_rekon","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}		
			if (row ==6)
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				this.filterRep.ListDataSGFilter(this, "Data Pembatalan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_batal,a.keterangan from aka_batal_m a "+this.filter,
												"select count(a.no_batal) from aka_batal_m a "+this.filter,
												new Array("a.no_batal","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}		
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["3","123","123","123","123","123","123","3","13"]);
		}else{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["3","123","3","123","123","123","123","3","13"]);
		}
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[2,0,2,2,2,2,2,0,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from aka_rekon_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 7)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter3 = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  ...
							  this.filterRep.filterStr("c.nim",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							  this.filterRep.filterStr("a.no_rekon",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+
							  this.filterRep.filterStr("b.no_batal",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,7)+"/"+this.app._lokasi+"/"+this.sg1.getCell(2,8)+"/"+this.filter3;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_aka2_rptAkRekonTu";
