window.app_saku3_report_aka2_flAkTakPdd = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_aka2_flAkTakPdd.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_aka2_flAkTakPdd";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from takkirim_m where modul='TAKBEA' and kode_lokasi='"+this.app._lokasi+"' and modul='TAKBEA' ");
...
window.app_saku3_report_aka2_flAkTakPdd.extend(window.childForm);
window.app_saku3_report_aka2_flAkTakPdd.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from takkirim_m where modul='TAKBEA' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 7)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  ...
							  "and a.modul='TAKBEA' ";
				this.filter3=this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
				this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
				this.filterRep.filterStr("d.kode_fakultas",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
				this.filterRep.filterStr("c.kode_jur",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
				this.filterRep.filterStr("c.kode_akt",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
				...
				this.nama_report = "server_report_saku3_aka2_rptAkTakPdd";
