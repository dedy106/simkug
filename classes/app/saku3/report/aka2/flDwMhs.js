window.app_saku3_report_aka2_flDwMhs = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_aka2_flDwMhs.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_aka2_flDwMhs";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from exs_aka_saldo_fak where kode_lokasi='"+this.app._lokasi+"' ");
	...
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Tahun Akademik 1 ","=",""));
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Tahun Akademik 2","=",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Fakultas","All",""));
	}else{
	
		this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
		this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Tahun Akademik 1 ","=",""));
		this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Tahun Akademik 2","=",""));
		this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Fakultas","=",this.fak));
	}
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	//this.pager=10;
	
};
window.app_saku3_report_aka2_flDwMhs.extend(window.childForm);
window.app_saku3_report_aka2_flDwMhs.implement({
	...
	{		
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["3","3","3","123"]);
		}else{
			
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["3","3","3","3"]);
		}
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,0,2]);	
		if (row == 1)
		{
			var sql="select distinct a.tahunaka "+
					"from aka_tahunaka a "+
					"inner join periode b on a.periode=b.periode and a.kode_lokasi=b.kode_lokasi "+
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_aka2_rptDwMhs";
	...
	doOpenSaldoJur: function(kode_lokasi,kode_fakultas){
		this.link="1";
		this.kode_fakultas=kode_fakultas;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = " where a.kode_lokasi='"+this.kode_lokasi+"' and a.kode_fakultas='"+this.kode_fakultas+"'";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+kode_fakultas;
		nama_report="server_report_saku2_kopeg_aka_rptAkSaldoJurTu";
