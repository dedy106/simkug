window.app_saku3_report_aka2_flAkRekonBatalTu = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_aka2_flAkRekonBatalTu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_aka2_flAkRekonBatalTu";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from aka_batal_m where kode_lokasi='"+this.app._lokasi+"' and modul='BATAL' ");
...
window.app_saku3_report_aka2_flAkRekonBatalTu.extend(window.childForm);
window.app_saku3_report_aka2_flAkRekonBatalTu.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from aka_batal_m where kode_lokasi='"+this.app._lokasi+"' and modul='BATAL' order by periode",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 6)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_aka2_rptAkRekonBatalTu";
