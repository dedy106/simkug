window.app_saku3_report_aka2_flDwSaldoInv = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_aka2_flDwSaldoInv.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_aka2_flDwSaldoInv";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from exs_aka_saldo_nim where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_aka2_flDwSaldoInv.extend(window.childForm);
window.app_saku3_report_aka2_flDwSaldoInv.implement({
		...
		try{			
			...
			if (row == 3)
			{
				if (this.app._userStatus=="A")
				{
					...
					var sql="select a.kode_jur,a.nama from aka_jurusan a "+filter;
					
					var sql2="select count(a.kode_jur) from aka_jurusan a "+filter;
				}
				else
				{
					var sql="select a.kode_jur,a.nama from aka_jurusan a "+
							"inner join karyawan_pp b on a.kode_lokasi=b.kode_lokasi and a.kode_jur=b.kode_pp "+
							"where a.kode_lokasi='"+this.app._lokasi+"' and b.nik='"+this.app._userLog+"' ";
					
					var sql2="select count(a.kode_jur) from aka_jurusan a "+
							"inner join karyawan_pp b on a.kode_lokasi=b.kode_lokasi and a.kode_jur=b.kode_pp "+
							"where a.kode_lokasi='"+this.app._lokasi+"' and b.nik='"+this.app._userLog+"'";
				}
				this.filterRep.ListDataSGFilter(this, "Data Prodi",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,new Array("a.kode_jur","a.nama"),"where");
			}
			if (row == 4)
			{
	...
	{		
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["3","3","1","123","123","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[2,0,3,2,2,0,0]);	
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from exs_aka_saldo_nim where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 5)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,5)+"/"+this.sg1.getCell(2,6);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_aka2_rptDwSaldoInv";
