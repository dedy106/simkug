window.app_saku3_report_aka2_flAkAmorJurnalTu = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_aka2_flAkAmorJurnalTu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_aka2_flAkAmorJurnalTu";
		this.maximize();
...
window.app_saku3_report_aka2_flAkAmorJurnalTu.extend(window.childForm);
window.app_saku3_report_aka2_flAkAmorJurnalTu.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_amor, a.keterangan from aka_amor_m a "+filter,
												"select count(a.no_amor) from aka_amor_m a "+filter,
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_aka2_rptAkAmorJurnalTu";
			...
			systemAPI.alert("[app_saku3_report_aka2_flAkAmorJurnalTu]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBayar: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter = "where a.no_amor='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
	...
	doOpenDetAmor: function(kode_pp,kode_lokasi,kode_akun,no_amor,kode_drk,periode){
		this.link="2";
		filter = "where a.no_amor='"+no_amor+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		var filter3 = "where a.no_amor='"+no_amor+"' and (a.akun_pdd='"+kode_akun+"' or a.akun_pdpt='"+kode_akun+"' )  and a.kode_pp='"+kode_pp+"' and a.kode_lokasi='"+kode_lokasi+"' and a.kode_drk='"+kode_drk+"'";
		filter2= periode+"/Layar/"+filter3;
		nama_report="server_report_saku3_aka2_rptAkAmorTu2";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
