window.app_saku3_report_uin_flCekList = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_uin_flCekList.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_uin_flCekList";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from uin_tahun where kode_lokasi='"+this.app._lokasi+"'");
	this.satker=this.dbLib.getPeriodeFromSQL("select kdsatker as periode from uin_user where kode_lokasi='"+this.app._lokasi+"' and nik='"+this.app._userLog+"' ");
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Satker","=",this.satker));
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Tahun Anggaran","=",this.periode));
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode Unit","=",this.app._kodePP));
...
window.app_saku3_report_uin_flCekList.extend(window.childForm);
window.app_saku3_report_uin_flCekList.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			if (row == 0)
			{
					this.filterRep.ListDataSGFilter(this, "Daftar Satker",this.sg1, this.sg1.row, this.sg1.col,
														  "select kdsatker, nmsatker from uin_satker where kdsatker='"+this.satker+"' ",
														  "select count(kdsatker) from uin_satker where kdsatker='"+this.satker+"' ",
														  ["kdsatker","nmsatker"],"and",["Kode","Nama"]);
			}	
			if (row == 2)
			{
			...
			if (row == 3)
			{
				var filter = this.filterRep.filterStr("substring(b.periode,1,4)",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where")+
				...
				this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.no_ver, a.keterangan from uin_ver_m a inner join uin_aju_m b on a.no_ver=b.no_ver and a.kode_lokasi=b.kode_lokasi "+filter,
														  "select count(*) from uin_ver_m a inner join uin_aju_m b on a.no_ver=b.no_ver and a.kode_lokasi=b.kode_lokasi "+filter,
														  ["no_ver","keterangan"],"and",["No Bukti","Keterangan"]);
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			this.dbLib.setItemsFromSQL("select tahun from uin_tahun  ",[this.sg1.columns.get(2).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	// this.filter = this.filterRep.filterStr("a.kdsatker",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
                ...
                this.filter = this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
                this.filterRep.filterStr("a.no_ver",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				...
				this.nama_report = "server_report_saku3_uin_rptCekList";
			...
			systemAPI.alert("[app_saku3_report_uin_flCekList]::mainButtonClick:"+e);
		}
	},
