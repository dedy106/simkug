window.app_saku3_report_uin_flBuktiSilabi = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_uin_flBuktiSilabi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_uin_flBuktiSilabi";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from uin_sppd_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_uin_flBuktiSilabi.extend(window.childForm);
window.app_saku3_report_uin_flBuktiSilabi.implement({
		...
		try{			
			...
			if (row == 3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.no_sppd, a.keterangan from uin_sppd_m a inner join uin_aju_m b on a.no_sppd=b.no_sppd and a.kode_lokasi=b.kode_lokasi "+filter,
														  "select count(a.no_sppd) from uin_sppd_m a inner join uin_aju_m b on a.no_sppd=b.no_sppd and a.kode_lokasi=b.kode_lokasi "+filter,
														  ["a.no_sppd","a.keterangan"],"and",["No Bukti","Keterangan"]);
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			this.dbLib.setItemsFromSQL("select distinct periode from uin_sppd_m  ",[this.sg1.columns.get(2).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
							  this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							  this.filterRep.filterStr("b.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							  this.filterRep.filterStr("a.no_sppd",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				...
				this.nama_report = "server_report_saku3_uin_rptBuktiSilabi";
			...
			systemAPI.alert("[app_saku3_report_uin_flBuktiSilabi]::mainButtonClick:"+e);
		}
	},
