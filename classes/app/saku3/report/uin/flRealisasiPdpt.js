window.app_saku3_report_uin_flRealisasiPdpt = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_uin_flRealisasiPdpt.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_uin_flRealisasiPdpt";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from uin_aju_m where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Jenis","=","PDPT"));
...
window.app_saku3_report_uin_flRealisasiPdpt.extend(window.childForm);
window.app_saku3_report_uin_flRealisasiPdpt.implement({
		...
		try{			
			...
			if (row == 4)
			{
				var filter = this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
							  ...
							  this.filterRep.filterStr("jenis",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_aju, keterangan from uin_aju_m "+filter,
														  "select count(no_aju) from uin_aju_m "+filter,
														  ["no_aju","keterangan"],"and",["No Bukti","Tahun"]);
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["3","123","123","3","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,2,0,2]);	
		
		if (row == 1)
		{
			this.dbLib.setItemsFromSQL("select distinct periode from uin_aju_m  ",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("BLU","RM"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
				this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Periode","=",this.periode));
				this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode Unit","All",""));
				this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Jenis","=","BLU"));
				this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("No Bukti","All",""));
			}else{
				...
				this.nama_report = "server_report_saku3_uin_rptRealisasiPdpt";
			...
			systemAPI.alert("[app_saku3_report_uin_flRealisasiPdpt]::mainButtonClick:"+e);
		}
	},
    ...
    doOpenBukti: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter2=this.app._userLog+"/"+this.app._lokasi;
		filter="where a.no_aju='"+no_bukti+"' ";
		nama_report="server_report_saku3_uin_rptRealisasi";
