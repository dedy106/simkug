window.app_saku3_report_uin_flNota = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_uin_flNota.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_uin_flNota";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from uin_nota_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_uin_flNota.extend(window.childForm);
window.app_saku3_report_uin_flNota.implement({
		...
		try{			
			...
			if (row == 3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_nota, keterangan from uin_nota_m "+filter,
														  "select count(no_nota) from uin_nota_m "+filter,
														  ["no_nota","keterangan"],"and",["No Bukti","Keterangan"]);
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			this.dbLib.setItemsFromSQL("select distinct periode from uin_nota_m  ",[this.sg1.columns.get(2).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
							  ...
							  this.filterRep.filterStr("a.no_nota",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				...
				this.nama_report = "server_report_saku3_uin_rptNota";
			...
			systemAPI.alert("[app_saku3_report_uin_flNota]::mainButtonClick:"+e);
		}
	},
