window.app_saku3_report_uin_flPBAgg = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_uin_flPBAgg.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_uin_flPBAgg";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from uin_aju_m where kode_lokasi='"+this.app._lokasi+"' and jenis='PDPT' ");
...
window.app_saku3_report_uin_flPBAgg.extend(window.childForm);
window.app_saku3_report_uin_flPBAgg.implement({
		...
		try{			
			...
			if (row == 2)
			{
					...
					this.filterRep.ListDataSGFilter(this, "Data FS",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_fs, nama from uin_fs "+
	...
	{		
		...
		if (row == 1)
		{
			this.dbLib.setItemsFromSQL("select distinct periode from periode where kode_lokasi='"+this.app._lokasi+"' ",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 3)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.app._nikUser+"/"+this.sg1.getCell(2,3);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_uin_rptPBAgg";
			...
			systemAPI.alert("[app_saku3_report_uin_flPBAgg]::mainButtonClick:"+e);
		}
	},
