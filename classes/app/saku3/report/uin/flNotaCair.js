window.app_saku3_report_uin_flNotaCair = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_uin_flNotaCair.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_uin_flNotaCair";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from uin_cair_m where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Rekening","All",""));
...
window.app_saku3_report_uin_flNotaCair.extend(window.childForm);
window.app_saku3_report_uin_flNotaCair.implement({
		...
		try{			
			...
			if (row == 2)
			{
					...
					this.filterRep.ListDataSGFilter(this, "Data Rekening",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_rek, nama from uin_rek "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(*) from uin_rek "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
			...
			if (row == 3)
			{
				var filter = this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
							  ...
							  this.filterRep.filterStr("kode_rek",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_cair, keterangan from uin_cair_m "+filter,
														  "select count(no_cair) from uin_cair_m "+filter,
														  ["no_cair","keterangan"],"and",["No Bukti","Keterangan"]);
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["123","3","123","123"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3],[0,3,2,2]);	
		}
		if (row == 1)
		{
			this.dbLib.setItemsFromSQL("select distinct periode from uin_cair_m  ",[this.sg1.columns.get(2).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
							  ...
							  this.filterRep.filterStr("a.kode_rek",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
				...
				this.nama_report = "server_report_saku3_uin_rptNotaCair";
			...
			systemAPI.alert("[app_saku3_report_uin_flNotaCair]::mainButtonClick:"+e);
		}
	},
