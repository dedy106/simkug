window.app_saku3_report_uin_flKbSilabi = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_uin_flKbSilabi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_uin_flKbSilabi";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and form='KBSILBI' ");
...
window.app_saku3_report_uin_flKbSilabi.extend(window.childForm);
window.app_saku3_report_uin_flKbSilabi.implement({
		...
		try{			
			...
			if (row == 3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.no_bukti, a.keterangan from trans_m a "+filter+" and form='KBSILBI' ",
														  "select count(a.no_bukti) from trans_m a  "+filter+" and form='KBSILBI' ",
	...
	{		
		...
		if (row == 1)
		{
			this.dbLib.setItemsFromSQL("select distinct periode from uin_trans_m where form='KBSILBI' ",[this.sg1.columns.get(2).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_uin_rptKbSilabi";
			...
			systemAPI.alert("[app_saku3_report_uin_flKbSilabi]::mainButtonClick:"+e);
		}
	},
