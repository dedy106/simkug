// JavaScript Document
window.app_saku3_report_sapyakes_flGlJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sapyakes_flGlJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sapyakes_flGlJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from glsap where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_sapyakes_flGlJurnal.extend(window.portalui_childForm);
window.app_saku3_report_sapyakes_flGlJurnal.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			...
			var sql1="select distinct a.no_bukti,a.no_doksap from glsap a "+filter; 
			var sql2="select count(a.no_bukti) "+
					"from (select distinct a.no_bukti from glsap a "+filter+")a ";
			this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
			                                new Array("a.no_bukti","a.no_doksap"),"and",new Array("No Bukti","No Dokumen"),"and");
		
		}
	},
	doSelectCell: function(sender, col, row){
		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3),new  Array("123","123","123","1235"));
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3),new  Array(2,0,0,2));
		
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from glsap where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct modul from glsap where kode_lokasi='"+this.app._lokasi+"' order by modul",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			this.nama_report="server_report_saku3_sapyakes_rptGlJurnal";
