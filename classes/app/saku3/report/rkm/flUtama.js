window.app_saku3_report_rkm_flUtama = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_rkm_flUtama.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_rkm_flUtama";
		this.maximize();
	...
	this.tahun=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from rkm_pu_h where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_rkm_flUtama.extend(window.childForm);
window.app_saku3_report_rkm_flUtama.implement({
		...
		try{			
			...
			if (row ==2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Bidang",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_bidang,nama from rkm_bidang ",
												"select count(*) from rkm_bidang ",
												new Array("kode_bidang","nama"),"where",new Array("Kode Bidang","Nama"));
			}			
	...
	{		
		...
		if (row == 1)
		{
			this.dbLib.setItemsFromSQL("select distinct tahun from rkm_pu_h a "+
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filterRep.filterStr("x.tahun",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
				this.filterRep.filterStr("x.kode_bidang",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
							
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_rkm_rptUtama";
			...
			systemAPI.alert("[app_saku3_report_rkm_flUtama]::mainButtonClick:"+e);
		}
	},
