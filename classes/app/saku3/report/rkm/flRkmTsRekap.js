window.app_saku3_report_rkm_flRkmTsRekap = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_rkm_flRkmTsRekap.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_rkm_flRkmTsRekap";
		this.maximize();
	...
	this.tahun=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from rkm_ts where kode_lokasi='"+this.app._lokasi+"' ");
	this.pp=this.dbLib.getPeriodeFromSQL("select nik_user as periode from rkm_ts where nik_user='"+this.app._userLog+"' and kode_lokasi='"+this.app._lokasi+"' ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("No Ref","All",""));
...
window.app_saku3_report_rkm_flRkmTsRekap.extend(window.childForm);
window.app_saku3_report_rkm_flRkmTsRekap.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Ref",this.sg1, this.sg1.row, this.sg1.col,
												"select distinct no_ref from rkm_ts ",
												"select count(*) from rkm_ts ",
												new Array("no_ref"),"where",new Array("No Ref"));
			}			
	...
	{		
		...
		if (row == 1)
		{
			this.dbLib.setItemsFromSQL("select distinct tahun from rkm_ts a "+
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenSs(this.kode_ts,this.kode_lokasi,this.tahun);
				}
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_ref",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
							
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_rkm_rptRkmTsRekap";
			...
			systemAPI.alert("[app_saku3_report_rkm_flRkmTsRekap]::mainButtonClick:"+e);
		}
	},
	...
	doOpenSs: function(kode_ts,kode_lokasi,tahun){
		
		this.kode_ts=kode_ts;
		this.kode_lokasi=kode_lokasi;
		this.tahun=tahun;
		this.link="1";
		filter2=kode_lokasi+"/"+tahun;
		filter="where a.kode_lokasi='"+kode_lokasi+"' and a.kode_ts='"+kode_ts+"' and a.tahun='"+tahun+"'";
		
		nama_report="server_report_saku3_rkm_rptRkmSsRekap";
	...
	doOpenDrk: function(kode_ts,kode_ss,kode_lokasi,tahun){
		...
		this.link="2";
		filter2=kode_lokasi+"/"+tahun;
		filter="where a.kode_lokasi='"+kode_lokasi+"' and a.kode_ss='"+kode_ss+"' and a.tahun='"+tahun+"'";
		
		nama_report="server_report_saku3_rkm_rptTargetBulan";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
