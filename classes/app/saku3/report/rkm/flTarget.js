window.app_saku3_report_rkm_flTarget = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_rkm_flTarget.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_rkm_flTarget";
		this.maximize();
	...
	this.tahun=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from rkm_target_m where kode_lokasi='"+this.app._lokasi+"' ");
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Kode IP","All",""));
	this.gridLib.SGEditData(this.sg1,5,[0,1,2],["Periode Laporan","=","Bulanan"]);
...
window.app_saku3_report_rkm_flTarget.extend(window.childForm);
window.app_saku3_report_rkm_flTarget.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				var sql="select a.kode_akun, a.nama "+
						"from masakun a "+
						"inner join rkm_akun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+filter;
			...
			if (row == 3)
			{
				...
				var sql="select a.kode_pp,a.nama "+
						...
						"			from rkm_target_d a "+filter+
						"			group by kode_pp,kode_lokasi "+
						"			)b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+filter;
			...
			{
				var filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.tahun",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,3),this.sg1.getCell(2,2),this.sg1.getCell(3,3),"and");
				var sql="select a.kode_ip,a.nama from rkm_ip a "+filter;
				var sql2="select a.kode_ip,a.nama from rkm_ip a "+filter;	
				this.filterRep.ListDataSGFilter(this, "Data IP",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,
												new Array("a.kode_ip","a.nama"),"where",new Array("Kode IP","Nama"));
			}			
	...
	{		
		...
		if (row == 1)
		{
			this.dbLib.setItemsFromSQL("select distinct tahun from rkm_target_m a "+
		...
		if (row == 5)
		{
			...
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["Bulanan","Triwulan"]);
		}
		if (row == 6)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("x.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("x.kode_ip",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
							
							
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,5)+"/"+this.sg1.getCell(2,6);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_rkm_rptTargetBulan";
			
				if (this.sg1.getCell(2,5)=="Triwulan")
				{
					this.nama_report = "server_report_saku3_rkm_rptTarget";
				}
			...
			systemAPI.alert("[app_saku3_report_rkm_flTarget]::mainButtonClick:"+e);
		}
	},
