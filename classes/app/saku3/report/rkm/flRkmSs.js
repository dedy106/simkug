window.app_saku3_report_rkm_flRkmSs = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_rkm_flRkmSs.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_rkm_flRkmSs";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Rektorat","All",""));
...
window.app_saku3_report_rkm_flRkmSs.extend(window.childForm);
window.app_saku3_report_rkm_flRkmSs.implement({
		...
		try{			
			...
			if (row ==2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Rektorat",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_rek,nama from rkm_rektorat ",
												"select count(*) from rkm_rektorat ",
												new Array("kode_rek","nama"),"where",new Array("Kode Rektorat","Nama"));
			}			
	...
	{		
		...
		if (row == 1)
		{
			this.dbLib.setItemsFromSQL("select distinct tahun from rkm_ss a "+
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_rkm_rptRkmSs";
			...
			systemAPI.alert("[app_saku3_report_rkm_flRkmSs]::mainButtonClick:"+e);
		}
	},
