window.app_saku3_report_belajar_flTagihandev = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_belajar_flTagihandev.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_belajar_flTagihandev";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Tagihan", 2);
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from dev_tagihan_m where kode_lokasi='"+this.app._lokasi+"'");

	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Periode","=",this.periode));
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("NIM","All",""));
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Nomor Tagihan","All",""));
	
	
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_belajar_flTagihandev.extend(window.childForm);
window.app_saku3_report_belajar_flTagihandev.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			if (row == 1)
			{
				this.filterRep.ListDataSGFilter(this, "Data Siswa",this.sg1, this.sg1.row, this.sg1.col,
												"select a.nim,a.nama from dev_siswa a ",
												"select count(*) from dev_siswa a ",
			...
			if (row == 2)
			{
				...
				this.filterRep.filterStr("a.nim",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");

				var sql1="select a.no_tagihan,a.keterangan from dev_tagihan_m a "+filter; 
				var sql2="select count(a.no_tagihan) from dev_tagihan_m a "+filter;
				this.filterRep.ListDataSGFilter(this, "Data Tagihan",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("a.no_tagihan","a.keterangan")," and ",new Array("No Tagihan","Keterangan")," and ");
			}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2],["123","123i","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2],[0,2,2]);
		if (row == 0)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from dev_tagihan_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}	
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("NIM","All",""));
				this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("No Tagihan","All",""));
			}else{
		    	...
		    	this.filter=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.nim",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.no_tagihan",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_belajar_rptTagihandev";
								
				this.viewer.setTotalPage(this.report.getTotalPage(this.nama_report,this.filter,this.pager,this.filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
			...
			systemAPI.alert("[app_saku3_report_belajar_flTagihandev]::mainButtonClick:"+e);
		}
	},
		...
		try{
			if (sender === sender.owner.bConfirm){
				...
				if (to !== ""){
					sender.owner.free();
					var d = new Date();
					var subject = "Laporan Tagihan "+d.toLocaleString();
	...
	// },
	doOpenTest: function(periode){
		window.open('http://dev.simkug.com','_blank'); 
	},
