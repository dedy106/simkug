window.app_saku3_report_belajar_flGajiKaryawan = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_belajar_flGajiKaryawan.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_belajar_flGajiKaryawan";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Data Gaji", 2);
	...
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Jenis Gaji","All",""));
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Nama","All",""));
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_belajar_flGajiKaryawan.extend(window.childForm);
window.app_saku3_report_belajar_flGajiKaryawan.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			if (row ==0)
			{
				this.filterRep.ListDataSGFilter(this, "Data Jenis Gaji",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_jns, nama_jns from jenisgajid",
												"select count(*) from jenisgajid",
												new Array("kode_jns","nama_jns"),"where",new Array("kode_jns","nama_jns"));
			}
			if (row ==1)
			{
				this.filterRep.ListDataSGFilter(this, "Data Siswa",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.no_gj, a.nama from gajid a  "+
														   this.filterRep.filterStr("a.kode_jns",this.sg1.getCell(1,0)," where "),
														  "select count(a.no_gajid) from gajid a "+
														  this.filterRep.filterStr("a.kode_jns",this.sg1.getCell(1,0)," where "),
														  ["a.no_gj","a.nama","a.kode_jns"],"and",["Kode","Nama","Jenis Gaji"]);
			}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_jns",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  this.filterRep.filterStr("a.no_gj",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_belajar_rptGajiKaryawan";
