window.app_saku3_report_belajar_flPbyr = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_belajar_flPbyr.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_belajar_flPbyr";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from dev_bayar_m where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Nomor Pembayaran","All",""));
	
	
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_belajar_flPbyr.extend(window.childForm);
window.app_saku3_report_belajar_flPbyr.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 2)
			{
				...
				var sql1="select a.no_bayar,a.keterangan from dev_bayar_m a "+filter; 
				var sql2="select count(a.no_bayar) from dev_bayar_m a "+filter;
				this.filterRep.ListDataSGFilter(this, "Data Pembayaran",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("a.no_bayar","a.keterangan")," and ",new Array("No Pembayaran","Keterangan")," and ");
			}
	...
	{		
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from dev_bayar_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}	
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("NIM","All",""));
				this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("No Pembayaran","All",""));
			}else{
		    	...
		    	this.filter=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.nim",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("a.no_bayar",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_belajar_rptPbyr";
			...
			systemAPI.alert("[app_saku3_report_belajar_flPbyr]::mainButtonClick:"+e);
		}
	},
