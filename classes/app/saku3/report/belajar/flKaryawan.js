window.app_saku3_report_belajar_flKaryawan = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_belajar_flKaryawan.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_belajar_flKaryawan";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Data Karyawan", 2);
	...
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Nip","All",""));
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_belajar_flKaryawan.extend(window.childForm);
window.app_saku3_report_belajar_flKaryawan.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			if (row ==0)
			{
				this.filterRep.ListDataSGFilter(this, "Data Jabatan",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_jab, nama_jab from jabatan",
												"select count(*) from jabatan",
												new Array("kode_jab","nama_jab"),"where",new Array("kode_jab","nama_jab"));
			}
			if (row ==1)
			{
				this.filterRep.ListDataSGFilter(this, "Data Karyawan",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.nip, a.nama from karyawand a  "+
														   this.filterRep.filterStr("a.kode_jab",this.sg1.getCell(1,0)," where "),
														  "select count(a.nip) from karyawand a "+
														  this.filterRep.filterStr("a.kode_jab",this.sg1.getCell(1,0)," where "),
														  ["a.nip","a.nama","a.kode_jab"],"and",["Kode","Nama","Jabatan"]);
			}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_belajar_rptKaryawan";
