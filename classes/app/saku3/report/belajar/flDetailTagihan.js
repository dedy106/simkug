window.app_saku3_report_belajar_flDetailTagihan = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_belajar_flDetailTagihan.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_belajar_flDetailTagihan";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Data Tagihan", 2);
	...
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Nomor Tagihan","All",""));
	//this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("No Tagihan","All",""));
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_belajar_flDetailTagihan.extend(window.childForm);
window.app_saku3_report_belajar_flDetailTagihan.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			if (row ==0)
			{
				this.filterRep.ListDataSGFilter(this, "Data Tagihan Siswa",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_tagihan, a.nim from tagihan2 a",
												"select count(*) from tagihan2 a",
												new Array("a.no_tagihan","a.nim"),"where",new Array("No_Tagihan","NIM"));
			}
			/*if (row ==1)
			{
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.no_tagihan",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where");
							 // this.filterRep.filterStr("a.nim",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_belajar_rptDetailTagihan";
