window.app_saku3_report_belajar_flTest = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_belajar_flTest.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_belajar_flTest";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Jurusan","All",""));
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Nim","All",""));
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_belajar_flTest.extend(window.childForm);
window.app_saku3_report_belajar_flTest.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			if (row ==0)
			{
				this.filterRep.ListDataSGFilter(this, "Data Jurusan",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_jur, nama_jur from jurusan",
												"select count(*) from jurusan",
												new Array("kode_jur","nama_jur"),"where",new Array("kode_jur","nama_jur"));
			}
			if (row ==1)
			{
				this.filterRep.ListDataSGFilter(this, "Data Siswa",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.nim, a.nama from siswa a  "+
														   this.filterRep.filterStr("a.kode_jur",this.sg1.getCell(1,0)," where "),
														  "select count(a.nim) from siswa a "+
														  this.filterRep.filterStr("a.kode_jur",this.sg1.getCell(1,0)," where "),
														  ["a.nim","a.nama","a.kode_jur"],"and",["Kode","Nama","Jurusan"]);
			}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	// this.filter = this.filterRep.filterStr("a.kode_jur",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
                // 			  this.filterRep.filterStr("a.nim",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
                this.filter2 = this.app._lokasi+"/"+this.app._userLog+"/01-RRA1905.0101/Approve/Approval TEST";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_siaga_rptAppRRA";
