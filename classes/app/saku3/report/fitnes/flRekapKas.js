window.app_saku3_report_fitnes_flRekapKas = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_fitnes_flRekapKas.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_fitnes_flRekapKas";
		this.maximize();
...
window.app_saku3_report_fitnes_flRekapKas.extend(window.childForm);
window.app_saku3_report_fitnes_flRekapKas.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenDetail(this.jenis,this.periode,this.kode_lokasi,this.tanggal);
				}
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_fitnes_rptRekapKas";
	...
	doOpenDetail: function(jenis,periode,kode_lokasi,tanggal){
		this.jenis=jenis;
		this.periode=periode;
		this.kode_lokasi=kode_lokasi;
		this.tanggal=tanggal;
		this.link="1";
		filter = "where a.kode_lokasi='"+kode_lokasi+"' and periode='"+periode+"' and a.tanggal='"+tanggal+"'";
		if (jenis!="10")
		{
			filter = "where a.kode_lokasi='"+kode_lokasi+"' and periode='"+periode+"' and d.jenis='"+jenis+"' and a.tanggal='"+tanggal+"'";
		}
		
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
		nama_report="server_report_saku3_fitnes_rptDetail";
	...
	doOpenKunjungan: function(no_kunj,kode_lokasi){
		this.no_kunj=no_kunj;
		this.kode_lokasi=kode_lokasi;
		this.link="2";
		filter = "where a.kode_lokasi='"+kode_lokasi+"' and a.no_kunj='"+no_kunj+"' ";
		
		
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
		nama_report="server_report_saku3_fitnes_rptKunjungan";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
