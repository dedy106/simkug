window.app_saku3_report_fitnes_flAktivasiKupon = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_fitnes_flAktivasiKupon.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_fitnes_flAktivasiKupon";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from fi_reg where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_fitnes_flAktivasiKupon.extend(window.childForm);
window.app_saku3_report_fitnes_flAktivasiKupon.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Aktivasi",this.sg1, this.sg1.row, this.sg1.col,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBVOC' order by periode",[this.sg1.columns.get(2).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_fitnes_rptAktivasiKupon";
