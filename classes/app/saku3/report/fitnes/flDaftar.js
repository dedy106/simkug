window.app_saku3_report_fitnes_flDaftar = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_fitnes_flDaftar.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_fitnes_flDaftar";
		this.maximize();
...
window.app_saku3_report_fitnes_flDaftar.extend(window.childForm);
window.app_saku3_report_fitnes_flDaftar.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pendaftaran",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_reg,a.kode_agg from fi_reg a "+this.filter,
												"select count(a.no_reg) from fi_reg a "+this.filter,
												new Array("a.no_reg","a.kode_agg"),"and",new Array("No Bukti","Keterangan"));
			}		
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from fi_reg where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_fitnes_rptDaftar";
