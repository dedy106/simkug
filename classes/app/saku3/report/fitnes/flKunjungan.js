window.app_saku3_report_fitnes_flKunjungan = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_fitnes_flKunjungan.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_fitnes_flKunjungan";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from fi_kunj_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_fitnes_flKunjungan.extend(window.childForm);
window.app_saku3_report_fitnes_flKunjungan.implement({
		...
		try{			
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Anggota",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.kode_agg, a.nama from fi_anggota a   "+
														   this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(a.kode_agg) from fi_anggota a "+
														  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  ["a.kode_agg","a.nama"],"and",["Kode","Nama"]);
			}
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Kunjungan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_kunj,a.kode_agg from fi_kunj_m a "+this.filter,
												"select count(a.no_kunj) from fi_kunj_m a "+this.filter,
												new Array("a.no_kunj","a.kode_agg"),"and",new Array("No Kunjungan","Keterangan"));
			}		
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from fi_kunj_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							  this.filterRep.filterStr("a.kode_agg",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							  this.filterRep.filterStr("a.no_kunj",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				
				this.nama_report = "server_report_saku3_fitnes_rptKunjungan";
