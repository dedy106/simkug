window.app_saku3_report_fitnes_flAnggota = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_fitnes_flAnggota.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_fitnes_flAnggota";
		this.maximize();
...
window.app_saku3_report_fitnes_flAnggota.extend(window.childForm);
window.app_saku3_report_fitnes_flAnggota.implement({
		...
		try{			
			...
			if (row == 1)
			{
				this.filterRep.ListDataSGFilter(this, "Data Kelompok",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.kode_klp, a.nama from fi_peserta_klp a   "+
														   this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(a.kode_klp) from fi_peserta_klp a "+
														  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  ["a.kode_klp","a.nama"],"and",["Kode","Nama"]);
			}
			...
			if (row == 2)
			{
				...
				var sql1="select a.kode_agg,a.nama from fi_anggota a "+filter;
				var sql2="select count(a.kode_agg) from fi_anggota a "+filter;
				this.filterRep.ListDataSGFilter(this, "Data Siswa",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
												new Array("a.kode_agg","a.nama"),"and",new Array("Kode","Nama"),"and");
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_fitnes_rptAnggota";
