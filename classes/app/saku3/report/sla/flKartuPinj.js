window.app_saku3_report_sla_flKartuPinj = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sla_flKartuPinj.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sla_flKartuPinj";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sla_kkp_m ");
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Perusahaan","All",""));
...
window.app_saku3_report_sla_flKartuPinj.extend(window.childForm);
window.app_saku3_report_sla_flKartuPinj.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Kartu",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_sla, a.keterangan from sla_kkp_m a "+filter,
												"select count(a.no_sla) from sla_kkp_m a "+filter,
												new Array("a.no_sla","a.keterangan"),"and",new Array("No Invoice","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from sla_kkp_m  order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_cocd",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_mitra",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_sla",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_sla_rptKartuPinj";
			...
			systemAPI.alert("[app_saku3_report_sla_flKartuPinj]::mainButtonClick:"+e);
		}
	},
