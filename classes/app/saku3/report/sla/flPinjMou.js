window.app_saku3_report_sla_flPinjMou = function(owner) {
    if (owner) {
        window.app_saku3_report_sla_flPinjMou.prototype.parent.constructor.call(this, owner);
        this.className = "app_saku3_report_sla_flPinjMou";
        this.maximize();
    ...
    //this.periode = this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sla_kkp_m where kode_lokasi='" + this.app._lokasi + "'");
    this.gridLib.SGEditData(this.sg1, 0, new Array(0, 1, 2), new Array("Cocd", "All", ""));
    this.gridLib.SGEditData(this.sg1, 1, new Array(0, 1, 2), new Array("Mitra", "All", ""));
    this.gridLib.SGEditData(this.sg1, 2, new Array(0, 1, 2), new Array("Classification", "All", ""));
...
window.app_saku3_report_sla_flPinjMou.extend(window.childForm);
window.app_saku3_report_sla_flPinjMou.implement({
    doEllipseClick: function(sender, col, row) {
        try {
            if (row == 0) {
                this.filterRep.ListDataSGFilter(this, "Data Lokasi", this.sg1, this.sg1.row, this.sg1.col,
                    "select cocd, company_name from mysym_company_code ",
                    "select count(*) from mysym_company_code ",
                    new Array("cocd", "company_name"), "where", new Array("kode", "nama"));
            }
            if (row == 1) {
            ...
            if (row == 2) {
                this.filterRep.ListDataSGFilter(this, "Data Classification", this.sg1, this.sg1.row, this.sg1.col,
                    "select kode_class, nama from sla_class ",
                    "select count(kode_class) from sla_class ",
                    new Array("kode_class", "nama"), "and");
            }
            if (row == 3) {
                filter = this.filterRep.filterStr("a.kode_cocd", this.sg1.getCell(1, 0), this.sg1.getCell(2, 0), this.sg1.getCell(3, 0), "where") +
                    this.filterRep.filterStr("a.kode_mitra", this.sg1.getCell(1, 1), this.sg1.getCell(2, 1), this.sg1.getCell(3, 1), "and");
                    this.filterRep.filterStr("a.kode_class", this.sg1.getCell(1, 2), this.sg1.getCell(2, 2), this.sg1.getCell(3, 2), "and");
                    this.filterRep.ListDataSGFilter(this, "Data MOU", this.sg1, this.sg1.row, this.sg1.col,
                    "select a.kode_mou, a.nama from sla_mou a " + filter,
                    "select count(a.kode_mou) from sla_mou a " + filter,
                    new Array("a.kode_mou", "a.nama"), "and", new Array("No MOU", "Keterangan"));
            }
    ...
    mainButtonClick: function(sender) {
        try {
            if (sender === this.app._mainForm.bClear2) {
                ...
                this.filter = this.filterRep.filterStr("a.kode_cocd", this.sg1.getCell(1, 0), this.sg1.getCell(2, 0), this.sg1.getCell(3, 0), "where") +
                    this.filterRep.filterStr("a.kode_mitra", this.sg1.getCell(1, 1), this.sg1.getCell(2, 1), this.sg1.getCell(3, 1), "and") +
                    this.filterRep.filterStr("a.kode_class", this.sg1.getCell(1, 2), this.sg1.getCell(2, 2), this.sg1.getCell(3, 2), "and") +
                    this.filterRep.filterStr("a.kode_mou", this.sg1.getCell(1, 3), this.sg1.getCell(2, 3), this.sg1.getCell(3, 3), "and");
                this.filter2 = this.sg1.getCell(2, 1);
                this.showFilter = this.filterRep.showFilter(this.sg1);
                var result = new arrayMap();
                this.nama_report = "server_report_saku3_sla_rptPinjMou";
            ...
            systemAPI.alert("[app_saku3_report_sla_flPinjMou]::mainButtonClick:" + e);
        }
    },
    ...
    doOpenKartu: function(no_bukti) {

        this.no_bukti = no_bukti;
        this.link = "1";
        filter = "where a.no_sla='" + no_bukti + "' ";
        filter2 = "";
        nama_report = "server_report_saku3_sla_rptKartuPinj";
