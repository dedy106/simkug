window.app_saku3_report_sla_flSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sla_flSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sla_flSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sla_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_sla_flSaldo.extend(window.childForm);
window.app_saku3_report_sla_flSaldo.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Kartu",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_sla, a.keterangan from sla_m a "+filter,
												"select count(a.no_sla) from sla_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from sla_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_sla_rptKartu";
			...
			systemAPI.alert("[app_saku3_report_sla_flSaldo]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBayar: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter = "where a.no_sla='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
