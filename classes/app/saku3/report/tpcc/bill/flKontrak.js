window.app_saku3_report_tpcc_bill_flKontrak = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tpcc_bill_flKontrak.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tpcc_bill_flKontrak";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(per_tagih) as periode from kontrak_m where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("No Kontrak","All",""));
...
window.app_saku3_report_tpcc_bill_flKontrak.extend(window.childForm);
window.app_saku3_report_tpcc_bill_flKontrak.implement({
		...
		try{			
			...
			if (row ==3)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.per_tagih",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				...
				this.filterRep.ListDataSGFilter(this, "Data Kontrak",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_kontrak, a.keterangan from kontrak_m a "+filter,
												"select count(a.no_kontrak) from kontrak_m a "+filter,
												new Array("a.no_kontrak","a.keterangan"),"and",new Array("No Kontrak","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct per_tagih from kontrak_m where kode_lokasi='"+this.app._lokasi+"' order by per_tagih ",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_tpcc_bill_rptKontrak";
			...
			systemAPI.alert("[app_saku3_report_tpcc_bill_flKontrak]::mainButtonClick:"+e);
		}
	},
	...
	doOpenKontrak: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_tpcc_bill_rptKontrakKartu";
