window.app_saku3_report_tpcc_bill_flBillDaftar = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tpcc_bill_flBillDaftar.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tpcc_bill_flBillDaftar";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from bill_m where kode_lokasi='"+this.app._lokasi+"' and modul='BILL' ");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("No Bill","All",""));
...
window.app_saku3_report_tpcc_bill_flBillDaftar.extend(window.childForm);
window.app_saku3_report_tpcc_bill_flBillDaftar.implement({
		...
		try{			
			...
			if (row ==3)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
                            ...
                            " and a.modul='BILL' ";
				this.filterRep.ListDataSGFilter(this, "Data Bill",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_bill, a.keterangan from bill_m a "+filter,
												"select count(a.no_bill) from bill_m a "+filter,
												new Array("a.no_bill","a.keterangan"),"and",new Array("No Bill","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from bill_m where kode_lokasi='"+this.app._lokasi+"' and modul='BILL' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_tpcc_bill_rptBillDaftar";
			...
			systemAPI.alert("[app_saku3_report_tpcc_bill_flBillDaftar]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBill: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_tpcc_bill_rptBillGabung";
