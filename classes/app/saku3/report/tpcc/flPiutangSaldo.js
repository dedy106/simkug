window.app_saku3_report_tpcc_flPiutangSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tpcc_flPiutangSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tpcc_flPiutangSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and modul='AR' and form='PIUBILL' ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Periode Bayar","<=",this.periode2));
...
window.app_saku3_report_tpcc_flPiutangSaldo.extend(window.childForm);
window.app_saku3_report_tpcc_flPiutangSaldo.implement({
		...
		try{			
			...
			if (row ==4)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.param1",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							" and a.modul='AR' and a.form='PIUBILL' ";
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["123","6","1236","123","123","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,0,0,2,2,0]);	
		if (row == 1)
		{
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.modul in  ('PIUBILL') ";
				var periode_bayar=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,5)+"/"+periode_bayar;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_tpcc_rptPiutangSaldo";
			...
			systemAPI.alert("[app_saku3_report_tpcc_flPiutangSaldo]::mainButtonClick:"+e);
		}
	},
