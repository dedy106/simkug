window.app_saku3_report_tpcc_proyek_flProyekPws = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tpcc_proyek_flProyekPws.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tpcc_proyek_flProyekPws";
		this.maximize();
	...
	var sql="select max(a.periode) as periode "+
			"from (select max(periode) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"' "+
			"union "+
			"select max(periode) as periode  from trans_j where kode_lokasi='"+this.app._lokasi+"' "+
			") a";
...
window.app_saku3_report_tpcc_proyek_flProyekPws.extend(window.childForm);
window.app_saku3_report_tpcc_proyek_flProyekPws.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Project",this.sg1, this.sg1.row, this.sg1.col,
												"select a.kode_proyek, a.nama from pr_proyek a "+filter,
												"select count(a.kode_proyek) from pr_proyek a "+filter,
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_tpcc_proyek_rptProyekPws";
			...
			systemAPI.alert("[app_saku3_report_tpcc_proyek_flProyekPws]::mainButtonClick:"+e);
		}
	},
