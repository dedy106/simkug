window.app_saku3_report_tpcc_proyek_flBymhdSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tpcc_proyek_flBymhdSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tpcc_proyek_flBymhdSaldo";
		this.maximize();
	...
	var sql="select max(periode) as periode from trans_j where modul='PR' and jenis='HUTANG' and kode_lokasi='"+this.app._lokasi+"'";
	this.periode=this.dbLib.getPeriodeFromSQL(sql);
	var sql="select max(periode) as periode from hutbayar_d where kode_lokasi='"+this.app._lokasi+"' ";
	...
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Periode Tagihan","<=",this.periode));
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Periode Bayar","<=",this.periode2));
...
window.app_saku3_report_tpcc_proyek_flBymhdSaldo.extend(window.childForm);
window.app_saku3_report_tpcc_proyek_flBymhdSaldo.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==3)
			{
							...
							" and a.modul='PR' and a.jenis='HUTANG' ";
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_bukti, a.keterangan from trans_j a "+filter,
												"select count(a.no_bukti) from trans_j a "+filter,
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","1236","123","123","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,2,2,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			var sql="select distinct periode from trans_j where modul='PR' and jenis='HUTANG' and kode_lokasi='"+this.app._lokasi+"' "+
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			var sql="select distinct periode from hutbayar_d where kode_lokasi='"+this.app._lokasi+"' "+
				"order by periode desc ";
			this.dbLib.setItemsFromSQL(sql,[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_hutang",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							" and x.form='BEBAN' ";
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,4);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_tpcc_proyek_rptBymhdSaldo";
			...
			systemAPI.alert("[app_saku3_report_tpcc_proyek_flBymhdSaldo]::mainButtonClick:"+e);
		}
	},
	...
	doOpenJurnal: function(no_bukti,kode_lokasi){
		...
		filter2=this.app._namaUser;
		nama_report="server_report_saku3_produk_rptProyekBebanKb";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenBayar: function(no_bukti,kode_lokasi){
		...
		filter2=this.app._namaUser;
		nama_report="server_report_saku3_tpcc_proyek_rptDetHutang";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
