window.app_saku3_report_tpcc_proyek_flBebanJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tpcc_proyek_flBebanJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tpcc_proyek_flBebanJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where form='BEBAN'  ");
...
window.app_saku3_report_tpcc_proyek_flBebanJurnal.extend(window.childForm);
window.app_saku3_report_tpcc_proyek_flBebanJurnal.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where form='BEBAN' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_tpcc_proyek_flBebanJurnal]::mainButtonClick:"+e);
		}
	},
