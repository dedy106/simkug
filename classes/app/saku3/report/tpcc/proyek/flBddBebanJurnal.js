window.app_saku3_report_tpcc_proyek_flBddBebanJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tpcc_proyek_flBddBebanJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tpcc_proyek_flBddBebanJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where form='BDDBEBAN'  ");
...
window.app_saku3_report_tpcc_proyek_flBddBebanJurnal.extend(window.childForm);
window.app_saku3_report_tpcc_proyek_flBddBebanJurnal.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where form='BDDBEBAN' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_tpcc_proyek_rptBddBebanJurnal";
			...
			systemAPI.alert("[app_saku3_report_tpcc_proyek_flBddBebanJurnal]::mainButtonClick:"+e);
		}
	},
