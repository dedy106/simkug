window.app_saku3_report_tpcc_proyek_flDetailPiutangNonBymhd = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tpcc_proyek_flDetailPiutangNonBymhd.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tpcc_proyek_flDetailPiutangNonBymhd";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(distinct '20'+substring(right(kode_proyek,9),1,2)) as periode from pr_proyek where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_tpcc_proyek_flDetailPiutangNonBymhd.extend(window.childForm);
window.app_saku3_report_tpcc_proyek_flDetailPiutangNonBymhd.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct '20'+substring(right(kode_proyek,9),1,2) as thn from pr_proyek where kode_lokasi='"+this.app._lokasi+"' ",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
                ...
                this.filterRep.filterStr("substring(convert(varchar,b.tgl_input,112),1,4)",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");

				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.filter3;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_tpcc_proyek_rptDetailPiutangNonBymhd";
			...
			systemAPI.alert("[app_saku3_report_tpcc_proyek_flDetailPiutangNonBymhd]::mainButtonClick:"+e);
		}
	},
	...
	doOpenPiutang: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where no_kontrak='"+no_bukti+"' and kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._namaUser;
		nama_report="server_report_saku3_tpcc_proyek_rptDetPiu";
	...
	doOpenBeban: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_tpcc_proyek_rptDetBeban";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
