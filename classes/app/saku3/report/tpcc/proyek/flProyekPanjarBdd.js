window.app_saku3_report_tpcc_proyek_flProyekPanjarBdd = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tpcc_proyek_flProyekPanjarBdd.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tpcc_proyek_flProyekPanjarBdd";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where form='PJPTGBDD' ");
...
window.app_saku3_report_tpcc_proyek_flProyekPanjarBdd.extend(window.childForm);
window.app_saku3_report_tpcc_proyek_flProyekPanjarBdd.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where form='PJPTGBDD' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_tpcc_proyek_rptProyekPanjarBdd";
			...
			systemAPI.alert("[app_saku3_report_tpcc_proyek_flProyekPanjarBdd]::mainButtonClick:"+e);
		}
	},
