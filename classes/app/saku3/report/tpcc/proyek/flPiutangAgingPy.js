window.app_saku3_report_tpcc_proyek_flPiutangAgingPy = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tpcc_proyek_flPiutangAgingPy.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tpcc_proyek_flPiutangAgingPy";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where  form='AKRUPIU' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_tpcc_proyek_flPiutangAgingPy.extend(window.childForm);
window.app_saku3_report_tpcc_proyek_flPiutangAgingPy.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where form='AKRUPIU' and kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_tpcc_proyek_rptPiutangAgingPy";
			...
			systemAPI.alert("[app_saku3_report_tpcc_proyek_flPiutangAgingPy]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBayar: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter = "where b.kode_project='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
		
		// nama_report="server_report_saku2_siaga_rptPiutangTrail";
		
		nama_report="server_report_saku3_tpcc_proyek_rptDetBayar2";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
