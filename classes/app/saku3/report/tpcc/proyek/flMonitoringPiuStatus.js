window.app_saku3_report_tpcc_proyek_flMonitoringPiuStatus = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tpcc_proyek_flMonitoringPiuStatus.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tpcc_proyek_flMonitoringPiuStatus";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from trans_m where  form='AKRUPIU' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_tpcc_proyek_flMonitoringPiuStatus.extend(window.childForm);
window.app_saku3_report_tpcc_proyek_flMonitoringPiuStatus.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) as periode from trans_m where form='AKRUPIU' and kode_lokasi='"+this.app._lokasi+"' order by substring(periode,1,4) ",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_tpcc_proyek_rptMonitoringPiuStatus";
			...
			systemAPI.alert("[app_saku3_report_tpcc_proyek_flMonitoringPiuStatus]::mainButtonClick:"+e);
		}
	},
