window.app_saku3_report_tpcc_proyek_flKbHutangJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tpcc_proyek_flKbHutangJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tpcc_proyek_flKbHutangJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where form='KBHUT'  ");
...
window.app_saku3_report_tpcc_proyek_flKbHutangJurnal.extend(window.childForm);
window.app_saku3_report_tpcc_proyek_flKbHutangJurnal.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where form='KBHUT' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_tpcc_proyek_flKbHutangJurnal]::mainButtonClick:"+e);
		}
	},
