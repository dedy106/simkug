window.app_saku3_report_tpcc_proyek_flProyekSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tpcc_proyek_flProyekSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tpcc_proyek_flProyekSaldo";
		this.maximize();
	...
	var sql="select max(periode) as periode from trans_m where form = 'AKRUPIU' and kode_lokasi='"+this.app._lokasi+"'";
	this.periode=this.dbLib.getPeriodeFromSQL(sql);
	var sql="select max(periode) as periode from piutang_bayar where kode_lokasi='"+this.app._lokasi+"' ";
...
window.app_saku3_report_tpcc_proyek_flProyekSaldo.extend(window.childForm);
window.app_saku3_report_tpcc_proyek_flProyekSaldo.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			var sql="select distinct periode from trans_m where form = 'AKRUPIU' and kode_lokasi='"+this.app._lokasi+"' "+
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			var sql="select distinct periode from piutang_bayar where kode_lokasi='"+this.app._lokasi+"' "+
				"order by periode desc ";
			this.dbLib.setItemsFromSQL(sql,[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_tpcc_proyek_rptProyekSaldo";
			...
			systemAPI.alert("[app_saku3_report_tpcc_proyek_flProyekSaldo]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBayar: function(no_bukti,kode_lokasi){
		...
		// nama_report="server_report_saku3_tpcc_proyek_rptDetBayar";
		nama_report="server_report_saku3_produk_rptProyekPiutangKb";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
