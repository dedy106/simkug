window.app_saku3_report_tpcc_proyek_flPiutangJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tpcc_proyek_flPiutangJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tpcc_proyek_flPiutangJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where form='AKRUPIU'  ");
...
window.app_saku3_report_tpcc_proyek_flPiutangJurnal.extend(window.childForm);
window.app_saku3_report_tpcc_proyek_flPiutangJurnal.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where form='AKRUPIU' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_tpcc_proyek_flPiutangJurnal]::mainButtonClick:"+e);
		}
	},
