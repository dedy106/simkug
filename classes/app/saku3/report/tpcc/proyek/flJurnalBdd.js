window.app_saku3_report_tpcc_proyek_flJurnalBdd = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tpcc_proyek_flJurnalBdd.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tpcc_proyek_flJurnalBdd";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where form='BDD'  ");
...
window.app_saku3_report_tpcc_proyek_flJurnalBdd.extend(window.childForm);
window.app_saku3_report_tpcc_proyek_flJurnalBdd.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where form='BDD' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_tpcc_proyek_flJurnalBdd]::mainButtonClick:"+e);
		}
	},
