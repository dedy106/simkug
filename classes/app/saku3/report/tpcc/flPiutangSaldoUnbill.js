window.app_saku3_report_tpcc_flPiutangSaldoUnbill = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tpcc_flPiutangSaldoUnbill.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tpcc_flPiutangSaldoUnbill";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and modul='AR' and form in ('UNBILL') ");
	var bulan=this.periode.substr(4,2);
	var tahun=this.periode.substr(0,4);
	this.periode2=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from piubayar_d where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Status","All",""));
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Periode Bayar","<=",this.periode2));
...
window.app_saku3_report_tpcc_flPiutangSaldoUnbill.extend(window.childForm);
window.app_saku3_report_tpcc_flPiutangSaldoUnbill.implement({
		...
		try{			
			...
			if (row ==3)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.param1",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							" and a.modul='AR' and a.form in ('UNBILL') ";
				var sql1 = "select distinct a.param2,b.nama "+ 
				"from trans_m a "+
				"inner join masakun b on a.param2=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+filter;

				var sql2 = "select count(distinct a.param2) "+
				"from trans_m a "+
				"inner join masakun b on a.param2=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+filter;
				...
				this.filterRep.ListDataSGFilter(this, "Data Panjar",this.sg1, this.sg1.row, this.sg1.col,
												sql1,
												sql2,
												new Array("a.param2","b.nama"),"and",new Array("Kode Akun","Nama"));
			}
			...
			if (row == 4)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.param1",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.param2",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							" and a.modul='AR' and a.form in ('UNBILL') ";
				this.filterRep.ListDataSGFilter(this, "Data Bukti",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_bukti, a.keterangan from trans_m a "+filter,
												"select count(a.no_bukti) from trans_m a "+filter,
												new Array("a.no_bukti","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123","63","123","123","123","3","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[2,0,2,2,2,0,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and modul='AR' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	
		if (row == 5)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Lunas","OutStanding"));
		}
		if (row == 6)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("d.param2",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.no_piutang",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							" and a.modul in  ('UNBILL') ";
				var periode="";
				var periode_bayar=this.sg1.getCell(2,6);
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,5)+"/"+periode_bayar;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_tpcc_rptPiutangSaldoUnbill";
			...
			systemAPI.alert("[app_saku3_report_tpcc_flPiutangSaldoUnbill]::mainButtonClick:"+e);
		}
	},
	...
	doOpenPiutang: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_tpcc_rptPiutangJurnal";
	...
	doOpenBill: function(no_bukti,kode_lokasi){
		...
		filter2=this.sg1.getCell(2,1);
		nama_report="server_report_saku3_tpcc_bill_rptBill";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenRekon: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_bukti='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.sg1.getCell(2,1);
		nama_report="server_report_saku3_tpcc_rptPiutangKasJurnal";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
