window.app_saku3_report_tpcc_flPiutangSaldoCust = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tpcc_flPiutangSaldoCust.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tpcc_flPiutangSaldoCust";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and modul='AR' ");
...
window.app_saku3_report_tpcc_flPiutangSaldoCust.extend(window.childForm);
window.app_saku3_report_tpcc_flPiutangSaldoCust.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2],["123","6","123"]);
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenSaldo(this.kode_cust,this.kode_lokasi);					
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_tpcc_rptPiutangSaldoCust";
			...
			systemAPI.alert("[app_saku3_report_tpcc_flPiutangSaldoCust]::mainButtonClick:"+e);
		}
	},
	...
	doOpenSaldo: function(kode_cust,kode_lokasi){
		...
		filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
				...
				this.filterRep.filterStr("a.kode_cust","=",kode_cust,kode_cust,"and")+
				" and a.modul in  ('PIUBILL') ";
		var periode_bayar=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
		filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"//"+periode_bayar;
		nama_report="server_report_saku3_tpcc_rptPiutangSaldo";
