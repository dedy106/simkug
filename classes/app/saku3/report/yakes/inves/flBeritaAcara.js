window.app_saku3_report_yakes_inves_flBeritaAcara = function (owner) {
    if (owner) {
        window.app_saku3_report_yakes_inves_flBeritaAcara.prototype.parent.constructor.call(this, owner);
        this.className = "app_saku3_report_yakes_inves_flBeritaAcara";
        this.maximize();
    ...
    this.periode = this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_deporekon_m where kode_lokasi='"+this.app._lokasi+"' ");
    ...
    this.gridLib.SGEditData(this.sg1, 2, new Array(0, 1, 2), new Array("No Rekon", "All", ""));
...
window.app_saku3_report_yakes_inves_flBeritaAcara.extend(window.childForm);
window.app_saku3_report_yakes_inves_flBeritaAcara.implement({
        ...
        try {
            ...
            if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Rekon",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_rekon, a.keterangan from inv_deporekon_m a "+filter,
												"select count(a.no_rekon) from inv_deporekon_m a "+filter,
    ...
    doSelectCell: function (sender, col, row) {
        ...
        if (row == 1) {
            ...
            this.dbLib.setItemsFromSQL("select distinct periode from inv_deporekon_m where kode_lokasi='" + this.app._lokasi + "' order by periode desc", [this.sg1.columns.get(2).pickList, this.sg1.columns.get(3).pickList]);
        }
    ...
    mainButtonClick: function (sender) {
        try {
            if (sender === this.app._mainForm.bClear2) {
                ...
                this.filterRep.filterStr("a.periode", this.sg1.getCell(1, 1), this.sg1.getCell(2, 1), this.sg1.getCell(3, 1), "and")+
                this.filterRep.filterStr("a.no_rekon", this.sg1.getCell(1, 2), this.sg1.getCell(2, 2), this.sg1.getCell(3, 2), "and");
                this.filter2 = this.sg1.getCell(2, 1);
                this.showFilter = this.filterRep.showFilter(this.sg1);
                var result = new arrayMap();
                this.nama_report = "server_report_saku3_yakes_inves_rptBeritaAcara";
            ...
            systemAPI.alert("[app_saku3_report_yakes_inves_flBeritaAcara]::mainButtonClick:" + e);
        }
    },
