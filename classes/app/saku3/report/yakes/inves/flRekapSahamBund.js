window.app_saku3_report_yakes_inves_flRekapSahamBund = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes_inves_flRekapSahamBund.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes_inves_flRekapSahamBund";
		this.maximize();
	...
	//this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_shmjual_m ");
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_shm_harga ");
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Tampil Nol","=","Tidak"));
...
window.app_saku3_report_yakes_inves_flRekapSahamBund.extend(window.childForm);
window.app_saku3_report_yakes_inves_flRekapSahamBund.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","3","3","123","13"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,1,2,0]);	
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from inv_shm_harga order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_plan",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"where");
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCellDateValue(2,2)+"/"+this.sg1.getCell(2,4);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_yakes_inves_rptRekapSahamBund";									
			...
			systemAPI.alert("[app_saku3_report_yakes_inves_flRekapSahamBund]::mainButtonClick:"+e);
		}
	},
