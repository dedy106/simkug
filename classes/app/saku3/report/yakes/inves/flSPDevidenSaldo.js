window.app_saku3_report_yakes_inves_flSPDevidenSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes_inves_flSPDevidenSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes_inves_flSPDevidenSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from  inv_spdev_m ");
...
window.app_saku3_report_yakes_inves_flSPDevidenSaldo.extend(window.childForm);
window.app_saku3_report_yakes_inves_flSPDevidenSaldo.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Mitra",this.sg1, this.sg1.row, this.sg1.col,
												"select a.kode_mitra, a.nama from inv_mitra a ",
												"select count(a.kode_mitra) from inv_mitra a ",
												new Array("a.kode_mitra","a.nama"),"and",new Array("Kode","Nama"));
			}					
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) as periode from  inv_spdev_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter = this.filterRep.filterStr("c.kode_mitra",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"where")+
				...
				this.nama_report = "server_report_saku3_yakes_inves_rptSPDevidenSaldo";
			...
			systemAPI.alert("[app_saku3_report_yakes_inves_flSPDevidenSaldo]::mainButtonClick:"+e);
		}
	},
