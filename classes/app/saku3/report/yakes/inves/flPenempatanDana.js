window.app_saku3_report_yakes_inves_flPenempatanDana = function (owner) {
    if (owner) {
        window.app_saku3_report_yakes_inves_flPenempatanDana.prototype.parent.constructor.call(this, owner);
        this.className = "app_saku3_report_yakes_inves_flPenempatanDana";
        this.maximize();
    ...
    this.periode = this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_shop_m where kode_lokasi='"+this.app._lokasi+"' ");

    // this.periode = this.app._periode;
    var bulan = this.periode.substr(4, 2);
    var tahun = this.periode.substr(0, 4);
    var strSQL = "select max(substring(convert(varchar,a.tanggal,103),1,2)) as tglakhir from inv_shop_m a where a.kode_lokasi='"+this.app._lokasi+"' and a.periode='"+this.periode+"' ";
    ...
    // var strSQL = "select max(tanggal) as tglakhir from inv_shop_m";
    // var data = this.dbLib.getDataProvider(strSQL, true);
    // if (typeof data == "object") {
    //     var line = data.rs.rows[0];
    //     if (line != undefined) {
    ...
    this.gridLib.SGEditData(this.sg1, 2, new Array(0, 1, 2), new Array("MI", "=", "YKT"));    
	this.gridLib.SGEditData(this.sg1, 3, new Array(0, 1, 2), new Array("No Bukti","=",""));
...
window.app_saku3_report_yakes_inves_flPenempatanDana.extend(window.childForm);
window.app_saku3_report_yakes_inves_flPenempatanDana.implement({
        ...
        try {
			...
			if (row == 3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data ",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_shop,a.keterangan from inv_shop_m a "+this.filter,
												"select count(a.no_shop) from inv_shop_m a "+this.filter,
												new Array("a.no_shop","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}	
    ...
    mainButtonClick: function (sender) {
        try {
            if (sender === this.app._mainForm.bClear2) {
                ...
                this.filterRep.filterStr("a.periode", this.sg1.getCell(1, 1), this.sg1.getCell(2, 1), this.sg1.getCell(3, 1), "and")+
                this.filterRep.filterStr("a.kode_kelola", this.sg1.getCell(1, 2), this.sg1.getCell(2, 2), this.sg1.getCell(3, 2), "and")+
                this.filterRep.filterStr("a.no_shop", this.sg1.getCell(1, 3), this.sg1.getCell(2, 3), this.sg1.getCell(3, 3), "and");
                this.filter2 = this.sg1.getCell(2, 1);
                this.showFilter = this.filterRep.showFilter(this.sg1);
                var result = new arrayMap();
                this.nama_report = "server_report_saku3_yakes_inves_rptPenempatanDana";
            ...
            systemAPI.alert("[app_saku3_report_yakes_inves_flPenempatanDana]::mainButtonClick:" + e);
        }
    },
