// JavaScript Document
window.app_saku3_report_yakes_inves_flAlokasi = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes_inves_flAlokasi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes_inves_flAlokasi";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Komposisi","=","5050"));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Kode Plan","=","1"));
...
window.app_saku3_report_yakes_inves_flAlokasi.extend(window.portalui_childForm);
window.app_saku3_report_yakes_inves_flAlokasi.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 5)
		{
			...
			"select kode_plan, nama from inv_plan where flag_konsol='0'",
			"select count(*) from inv_plan where flag_konsol='0'",
			new Array("kode_plan","nama"),"where",new Array("kode","nama"));
		}
	},
	doSelectCell: function(sender, col, row){
		...
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5),new  Array(2,0,2,1,0,0));
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct kode_klp from inv_persen where kode_lokasi='"+this.app._lokasi+"' order by kode_klp desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2 = this.app._nikUser+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCellDateValue(2,3)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(2,5);
				
                this.nama_report="server_report_saku3_yakes_inves_rptAlokasi";
