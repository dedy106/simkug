window.app_saku3_report_yakes_inves_flDepNDPindahBuku = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes_inves_flDepNDPindahBuku.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes_inves_flDepNDPindahBuku";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Bank Sumber","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("No Bukti","All",""));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_yakes_inves_flDepNDPindahBuku.extend(window.childForm);
window.app_saku3_report_yakes_inves_flDepNDPindahBuku.implement({
		...
		try{			
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Bank",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.kode_bank, a.nama from inv_bank a   ",
														  "select count(a.kode_bank) from inv_bank a ",
														  ["a.kode_bank","a.nama"],"and",["Kode","Nama"]);
			}
			...
			if (row ==3)
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+" and a.jenis='DOC' ";
				this.filterRep.ListDataSGFilter(this, "Data ",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_depo,a.keterangan from inv_depo2_m a "+this.filter,
												"select count(a.no_depo) from inv_depo2_m a "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from inv_shop_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  ...
							  this.filterRep.filterStr("d.bsumber",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							  this.filterRep.filterStr("a.no_depo",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+" and a.jenis='DOC' ";
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				
				this.nama_report = "server_report_saku3_yakes_inves_rptDepNDPindahBuku";
