window.app_saku3_report_yakes_inves_flRdGainLoss = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes_inves_flRdGainLoss.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes_inves_flRdGainLoss";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(substring(a.periode,1,4)) as periode from (select distinct periode from inv_rdjual_m union select periode from inv_rddev_m) a ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode Reksadana","All",""));
...
window.app_saku3_report_yakes_inves_flRdGainLoss.extend(window.childForm);
window.app_saku3_report_yakes_inves_flRdGainLoss.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==2)
			{				
				this.filterRep.ListDataSGFilter(this, "Data RD",this.sg1, this.sg1.row, this.sg1.col,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) as periode from inv_rdjual_m where kode_lokasi='"+this.app._lokasi+"' union select distinct substring(periode,1,4) as periode from inv_rddev_m  order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter = this.filterRep.filterStr("a.kode_rd",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,4),"where");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_yakes_inves_rptRdGainLoss";
			...
			systemAPI.alert("[app_saku3_report_yakes_inves_flRdGainLoss]::mainButtonClick:"+e);
		}
	},
