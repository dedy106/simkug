window.app_saku3_report_yakes_inves_flRekapSp = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes_inves_flRekapSp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes_inves_flRekapSp";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_sp_harga ");
...
window.app_saku3_report_yakes_inves_flRekapSp.extend(window.childForm);
window.app_saku3_report_yakes_inves_flRekapSp.implement({
	...
	{		
		...
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3],[2,0,2,1]);	
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from inv_sp_harga order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("b.kode_plan",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"where");
									this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCellDateValue(2,3)+"/"+this.sg1.getCell(2,2);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_yakes_inves_rptRekapSp";									
			...
			systemAPI.alert("[app_saku3_report_yakes_inves_flRekapSp]::mainButtonClick:"+e);
		}
	},
