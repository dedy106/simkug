window.app_saku3_report_yakes_inves_flRoiSimpleKelas = function (owner) {
    if (owner) {
        window.app_saku3_report_yakes_inves_flRoiSimpleKelas.prototype.parent.constructor.call(this, owner);
        this.className = "app_saku3_report_yakes_inves_flRoiSimpleKelas";
        this.maximize();
    ...
    this.periode = this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from periode ");
    ...
    this.gridLib.SGEditData(this.sg1, 3, new Array(0, 1, 2), new Array("Kode Kelas", "=", ""));
...
window.app_saku3_report_yakes_inves_flRoiSimpleKelas.extend(window.childForm);
window.app_saku3_report_yakes_inves_flRoiSimpleKelas.implement({
        ...
        try {
            ...
            if (row == 2) {
                this.filterRep.ListDataSGFilter(this, "Data Plan", this.sg1, this.sg1.row, this.sg1.col,
                    "select kode_plan, nama from inv_plan ",
                    "select count(kode_plan) from inv_plan ",
                    new Array("kode_plan", "nama"), "and");
            }

            if (row == 3) {
                this.filterRep.ListDataSGFilter(this, "Data Kelas Aset", this.sg1, this.sg1.row, this.sg1.col,
                    "select distinct kode_kelas, nama from inv_batas_alokasi ",
                    "select count(distinct kode_kelas) from inv_batas_alokasi ",
    ...
    doSelectCell: function (sender, col, row) {
        ...
        if (row == 1) {
            ...
            this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) as periode from periode where kode_lokasi='" + this.app._lokasi + "' order by periode desc", [this.sg1.columns.get(2).pickList, this.sg1.columns.get(3).pickList]);
        }
    ...
    mainButtonClick: function (sender) {
        try {
            if (sender === this.app._mainForm.bClear2) {
                ...
                this.gridLib.SGEditData(this.sg1, 1, new Array(0, 1, 2), new Array("Tahun", "=", this.periode));
                this.gridLib.SGEditData(this.sg1, 2, new Array(0, 1, 2), new Array("Kode Plan", "=", "1"));
                this.gridLib.SGEditData(this.sg1, 3, new Array(0, 1, 2), new Array("Kode Kelas", "All", ""));
            } else {
                ...
                // this.filterRep.filterStr("kode_lokasi", this.sg1.getCell(1, 0), this.sg1.getCell(2, 0), this.sg1.getCell(3, 0), "where")+
                            this.filterRep.filterStr("substring(periode,1,4)", this.sg1.getCell(1, 1), this.sg1.getCell(2, 1), this.sg1.getCell(3, 1), "where")+
                            this.filterRep.filterStr("kode_plan", this.sg1.getCell(1, 2), this.sg1.getCell(2, 2), this.sg1.getCell(3, 2), "and")+
                            this.filterRep.filterStr("kelas", this.sg1.getCell(1, 3), this.sg1.getCell(2, 3), this.sg1.getCell(3, 3), "and");
               
                this.filter2 = this.sg1.getCell(2, 1)+"/"+this.sg1.getCell(2,2);
                this.showFilter = this.filterRep.showFilter(this.sg1);
                var result = new arrayMap();
                this.nama_report = "server_report_saku3_yakes_inves_rptRoiSimpleKelas";
            ...
            systemAPI.alert("[app_saku3_report_yakes_inves_flRoiSimpleKelas]::mainButtonClick:" + e);
        }
    },
