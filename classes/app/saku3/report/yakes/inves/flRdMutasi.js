window.app_saku3_report_yakes_inves_flRdMutasi = function (owner) {
    if (owner) {
        window.app_saku3_report_yakes_inves_flRdMutasi.prototype.parent.constructor.call(this, owner);
        this.className = "app_saku3_report_yakes_inves_flRdMutasi";
        this.maximize();
    ...
    // var sqlper = "select max(a.periode) as periode from (select periode from inv_rdjual_m where kode_lokasi='" + this.app._lokasi + "' "+
    // "union all "+
    // "select periode from inv_rdbeli_m where kode_lokasi='" + this.app._lokasi + "' "+
    // "union all "+
    // "select substring(convert(varchar,getdate(),112),1,6) as periode "+
    // ") a ";
    var sqlper = "select max(periode) as periode from inv_rd_harga ";
    this.periode = this.dbLib.getPeriodeFromSQL(sqlper);
    ...
    this.gridLib.SGEditData(this.sg1, 1, new Array(0, 1, 2,3), new Array("Periode", "Range", this.periode,this.periode));

    this.gridLib.SGEditData(this.sg1, 2, new Array(0, 1, 2), new Array("Kelompok Reksadana", "=", ""));
...
window.app_saku3_report_yakes_inves_flRdMutasi.extend(window.childForm);
window.app_saku3_report_yakes_inves_flRdMutasi.implement({
    doEllipseClick: function (sender, col, row) {
        try {
            ...
            if (row == 2) {
                this.filterRep.ListDataSGFilter(this, "Data Kelompok Reksadana", this.sg1, this.sg1.row, this.sg1.col,
                    "select kode_rdklp, nama from inv_rdklp ",
                    "select count(kode_rdklp) from inv_rdklp ",
    ...
    doSelectCell: function (sender, col, row) {
        ...
        if (row == 1) {
            ...
            // var sql = "select distinct a.periode from (select periode from inv_rdjual_m where kode_lokasi='" + this.app._lokasi + "' "+
            // "union all "+
            // "select periode from inv_rdbeli_m where kode_lokasi='" + this.app._lokasi + "' "+
            // "union all "+
            // "select substring(convert(varchar,getdate(),112),1,6) as periode "+
            ...
            var sql = "select distinct periode from inv_rd_harga order by periode desc ";
    ...
    mainButtonClick: function (sender) {
        try {
            if (sender === this.app._mainForm.bClear2) {
                ...
                this.filter3 = this.filterRep.filterStr("b.kode_lokasi", this.sg1.getCell(1, 0), this.sg1.getCell(2, 0), this.sg1.getCell(3, 0), "where")+
                            this.filterRep.filterStr("b.periode", this.sg1.getCell(1, 1), this.sg1.getCell(2, 1), this.sg1.getCell(3, 1), "and");
               
                this.filter2 = this.sg1.getCell(2, 1)+"/"+this.sg1.getCell(2,2)+"/"+this.filter3;
                this.showFilter = this.filterRep.showFilter(this.sg1);
                var result = new arrayMap();
                this.nama_report = "server_report_saku3_yakes_inves_rptRdMutasi";
            ...
            systemAPI.alert("[app_saku3_report_yakes_inves_flRdMutasi]::mainButtonClick:" + e);
        }
    },
