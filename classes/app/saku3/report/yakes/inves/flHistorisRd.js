window.app_saku3_report_yakes_inves_flHistorisRd = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes_inves_flHistorisRd.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes_inves_flHistorisRd";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_rd_kkp ");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Kode Reksadana","=",""));
...
window.app_saku3_report_yakes_inves_flHistorisRd.extend(window.childForm);
window.app_saku3_report_yakes_inves_flHistorisRd.implement({
		...
		try{			
            ...
            if (row == 3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Reksadana",this.sg1, this.sg1.row, this.sg1.col,
												"select a.kode_rd, a.nama from inv_rd a ",
												"select count(a.kode_rd) from inv_rd a ",
												new Array("a.kode_rd","a.nama"),"and",new Array("Kode Reksadana","Nama"));
			}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["3","3","1","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3],[2,0,2,2]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from periode where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
                ...
                // this.filterRep.filterStr("kode_kelola",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
                this.filterRep.filterStr("kode_rd",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_yakes_inves_rptHistorisRd";									
			...
			systemAPI.alert("[app_saku3_report_yakes_inves_flHistorisRd]::mainButtonClick:"+e);
		}
	},
