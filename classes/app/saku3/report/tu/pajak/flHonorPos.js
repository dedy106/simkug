window.app_saku3_report_tu_pajak_flHonorPos = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_pajak_flHonorPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_pajak_flHonorPos";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and form in ('HONOR1','NONPEG','PEGAWAI')  ");
...
window.app_saku3_report_tu_pajak_flHonorPos.extend(window.childForm);
window.app_saku3_report_tu_pajak_flHonorPos.implement({
		...
		try{			
			...
			if (row ==3)
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.form in ('HONOR1','NONPEG','PEGAWAI') ";
			...
			if (row == 6)
			{
				...
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.form in ('HONOR1','NONPEG','PEGAWAI') "+tanggal;
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
	...
	{		
		if (this.app._userStatus=="A")
		{
			...
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[2,0,2,2,1,0,2]);	
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123","123","123","123","123","3","123"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[3,0,2,2,1,0,2]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from it_aju_m where form in ('HONOR1','NONPEG','PEGAWAI') and kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row === 5)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_aju",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+tanggal+this.filterRep.filterStr("a.no_kas",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and")+
							" and a.form in ('HONOR1','NONPEG','PEGAWAI') ";
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,5);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_tu_pajak_rptHonorPos";
			...
			systemAPI.alert("[app_saku3_report_tu_pajak_flHonorPos]::mainButtonClick:"+e);
		}
	},
