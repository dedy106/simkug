window.app_saku3_report_tu_pajak_flHonor = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_pajak_flHonor.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_pajak_flHonor";
		this.maximize();
...
window.app_saku3_report_tu_pajak_flHonor.extend(window.childForm);
window.app_saku3_report_tu_pajak_flHonor.implement({
		...
		try{			
			...
			if (row ==3)
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.form in ('HONOR1','PEGAWAI','NONPEG') ";
	...
	{		
		if (this.app._userStatus=="A")
		{
			...
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,0,2,2,1,0]);	
		}
		else
		{
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenKas(this.no_kas,this.kode_lokasi);					
					
				}
				if (this.link=="3")
				{
					this.doOpenJu(this.no_ju,this.kode_lokasi);					
			...
			else
			{
				...
				*/
				...
				this.nama_report = "server_report_saku3_tu_pajak_rptHonor";
			...
			systemAPI.alert("[app_saku3_report_tu_pajak_flHonor]::mainButtonClick:"+e);
		}
	},
	...
	doOpenKas: function(no_kas,kode_lokasi){
		...
		filter2=this.sg1.getCell(2,1);
		nama_report="server_report_saku2_kopeg_kbitt_rptKbJurnalTu";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},

	doOpenJu: function(no_ju,kode_lokasi){

		this.no_ju=no_ju;
		this.kode_lokasi=kode_lokasi;
		this.link="3";
		filter = "where a.no_aju='"+no_ju+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.sg1.getCell(2,1);
		nama_report="server_report_saku3_tu_pajak_rptHonorFormNP";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
