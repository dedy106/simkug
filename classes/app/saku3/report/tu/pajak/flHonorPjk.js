window.app_saku3_report_tu_pajak_flHonorPjk = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_pajak_flHonorPjk.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_pajak_flHonorPjk";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and form in ('PEGAWAI')  ");
...
window.app_saku3_report_tu_pajak_flHonorPjk.extend(window.childForm);
window.app_saku3_report_tu_pajak_flHonorPjk.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and form in ('PEGAWAI')  order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row === 5)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			...
			else
			{
				...
				*/
				...
				this.nama_report = "server_report_saku3_tu_pajak_rptHonorPjk";
			...
			systemAPI.alert("[app_saku3_report_tu_pajak_flHonorPjk]::mainButtonClick:"+e);
		}
	},
