window.app_saku3_report_tu_flProyekApp = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_flProyekApp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_flProyekApp";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from tu_proyek_app where kode_lokasi='"+this.app._lokasi+"'  ");
...
window.app_saku3_report_tu_flProyekApp.extend(window.childForm);
window.app_saku3_report_tu_flProyekApp.implement({
		...
		try{			
			...
			if (row ==5)
			{
				...
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("b.kode_jenis",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				var sql="select a.no_app,a.no_bukti "+
						"from tu_proyek_app a "+
						"inner join tu_proyek b on a.no_bukti=b.kode_proyek and a.kode_lokasi=b.kode_lokasi "+filter;
				var sql2="select count(a.no_app) "+
						"from tu_proyek_app a "+
						"inner join tu_proyek b on a.no_bukti=b.kode_proyek and a.kode_lokasi=b.kode_lokasi "+filter;
				this.filterRep.ListDataSGFilter(this, "Data RAB Proyek",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,
												new Array("a.no_app","a.no_bukti"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from tu_proyek_app where kode_lokasi='"+this.app._lokasi+"' ",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("b.kode_cust",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("b.kode_jenis",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							this.filterRep.filterStr("a.no_app",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");;
			...
			systemAPI.alert("[app_saku3_report_tu_flProyekApp]::mainButtonClick:"+e);
		}
	},
