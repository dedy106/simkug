window.app_saku3_report_tu_kantin_flRekon = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_kantin_flRekon.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_kantin_flRekon";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from it_aju_m where form='KANTIN'  ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Nomor Bukti","All",""));
...
window.app_saku3_report_tu_kantin_flRekon.extend(window.childForm);
window.app_saku3_report_tu_kantin_flRekon.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_aju, a.keterangan from it_aju_m a "+filter,
												"select count(a.no_aju) from it_aju_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and form = 'KANTIN' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_tu_kantin_flRekon]::mainButtonClick:"+e);
		}
	},
