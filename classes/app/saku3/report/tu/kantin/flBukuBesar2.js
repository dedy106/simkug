...
//* 			SAI, PT											
//***********************************************************************************************
window.app_saku3_report_tu_kantin_flBukuBesar2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_kantin_flBukuBesar2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_kantin_flBukuBesar2";
		this.maximize();
...
window.app_saku3_report_tu_kantin_flBukuBesar2.extend(window.portalui_childForm);
window.app_saku3_report_tu_kantin_flBukuBesar2.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["1234","123","123","123","123","3","123","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[2,0,2,2,2,0,1,0,0]);
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["1234","123","123","123","123","3","123","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[3,0,2,2,2,0,1,0,0]);
		}
		if (row == 1)
		{			
		...
		try
		{
			...
			else
			{
				...
				this.filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," and ")+
							...
							this.filterRep.filterStr("a.kode_drk",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4)," and ")+tanggal;
				var mutasi="";
				if (this.sg1.getCell(2,5)=="Tidak")
				{
					mutasi=" and (a.so_awal<>0 or a.debet<>0 or a.kredit<>0 or a.so_akhir<>0) ";
				}
				var sort=" order by a.tanggal,a.dc desc ";
				...
				this.filter2 = this.app._namaForm+"/"+filter_akun+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0)+"/"+this.app._nikUser+"/"+sort+"/"+this.sg1.getCell(2,8);
				this.nama_report="server_report_saku3_tu_kantin_rptBukuBesar2";
