// JavaScript Document
window.app_saku3_report_tu_kantin_flTb = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_kantin_flTb.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_kantin_flTb";
		this.maximize();
...
window.app_saku3_report_tu_kantin_flTb.extend(window.portalui_childForm);
window.app_saku3_report_tu_kantin_flTb.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 2)
		{
			this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,
													  "select distinct a.kode_akun, a.nama from masakun a inner join lokasi b on a.kode_lokasi=b.kode_lokasi inner join flag_relasi c on a.kode_akun=c.kode_akun and c.kode_flag='051' "+
													   this.filterRep.filterStr("b.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," and "),
													  "select count(a.kode_akun) from masakun a inner join lokasi b on a.kode_lokasi=b.kode_lokasi inner join flag_relasi c on a.kode_akun=c.kode_akun and c.kode_flag='051' and a.kode_lokasi=c.kode_lokasi "+
													  this.filterRep.filterStr("b.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," and "),
	...
	doSelectCell : function(sender, col, row){
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Neraca Lajur"));
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			...
			else
			{
				...
				if (this.app._lokasi == this.app._kodeLokasiKonsol)
				{
					...
					this.filter = this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
						this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.app._lokasi,this.app._lokasi,"and")+
						this.filterRep.filterStr("a.kode_akun",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				...
				if (this.sg1.getCell(2,3)=="Neraca Percobaan")
				{
					...
					this.nama_report="server_report_saku3_tu_kantin_rptTBPercobaan";
				}
				if (this.sg1.getCell(2,3)=="Neraca Lajur")
				{
					this.sql = "call sp_glma_tmp ('"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";								
					this.nama_report="server_report_saku3_tu_kantin_rptTBLajur";
					
				}
				if (this.sg1.getCell(2,3)=="12 Periode Mutasi")
				{
					this.sql = "call sp_glma12_tmp ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					this.nama_report="server_report_saku3_tu_kantin_rptTB12";
				}
				if (this.sg1.getCell(2,3)=="12 Periode Saldo Akhir")
				{
					this.sql = "call sp_glma12Akhir_tmp ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					this.nama_report="server_report_saku3_tu_kantin_rptTB12Akhir";
				}
