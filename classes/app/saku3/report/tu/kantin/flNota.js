window.app_saku3_report_tu_kantin_flNota = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_kantin_flNota.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_kantin_flNota";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from ktu_nota_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_tu_kantin_flNota.extend(window.childForm);
window.app_saku3_report_tu_kantin_flNota.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==3)
			{
					...
					this.filterRep.filterStr("c.kode_kantin",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
					
		this.filterRep.ListDataSGFilter(this, "Data Nota",this.sg1, this.sg1.row, this.sg1.col,
										"select a.no_bukti, a.ket from ktu_nota_m a inner join ktu_tenan b on a.kode_tenan=b.kode_tenan inner join ktu_kantin c on b.kode_kantin=c.kode_kantin "+filter,
										"select count(a.no_bukti) from ktu_nota_m a inner join ktu_tenan b on a.kode_tenan=b.kode_tenan inner join ktu_kantin c on b.kode_kantin=c.kode_kantin "+filter,
										new Array("a.no_bukti","a.ket"),"and",new Array("No Bukti","Keterangan"));
			}			
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from ktu_nota_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("b.kode_kantin",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
			...
			systemAPI.alert("[app_saku3_report_tu_kantin_flNota]::mainButtonClick:"+e);
		}
	},
