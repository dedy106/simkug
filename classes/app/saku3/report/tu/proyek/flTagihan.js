window.app_saku3_report_tu_proyek_flTagihan = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_proyek_flTagihan.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_proyek_flTagihan";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from prb_prbill_m where kode_lokasi='"+this.app._lokasi+"'  ");
...
window.app_saku3_report_tu_proyek_flTagihan.extend(window.childForm);
window.app_saku3_report_tu_proyek_flTagihan.implement({
		...
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Proyek",this.sg1, this.sg1.row, this.sg1.col,
												"select a.kode_proyek, a.nama from prb_proyek a inner join prb_prbill_m b on a.kode_proyek=b.kode_proyek and a.kode_lokasi=b.kode_lokasi "+filter,
												"select count(a.kode_proyek) from prb_proyek a inner join prb_prbill_m b on a.kode_proyek=b.kode_proyek and a.kode_lokasi=b.kode_lokasi "+filter,
												new Array("a.kode_proyek","a.nama"),"and",new Array("Kode Proyek","Nama"));
			}
			...
			if (row ==5)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Tagihan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_bill, a.keterangan from prb_prbill_m a "+filter,
												"select count(a.no_bill) from prb_prbill_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from prb_prbill_m where kode_lokasi='"+this.app._lokasi+"' ",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_tu_proyek_rptTagihan";
			...
			systemAPI.alert("[app_saku3_report_tu_proyek_flTagihan]::mainButtonClick:"+e);
		}
	},
