window.app_saku3_report_tu_proyek_flProyekApp = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_proyek_flProyekApp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_proyek_flProyekApp";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from prb_proyek_app where  kode_lokasi='"+this.app._lokasi+"'  ");
...
window.app_saku3_report_tu_proyek_flProyekApp.extend(window.childForm);
window.app_saku3_report_tu_proyek_flProyekApp.implement({
		...
		try{			
			...
			if (row ==4)
			{
				...
				var sql="select a.no_app,a.no_bukti "+
						...
						"inner join prb_proyek b on a.no_bukti=b.kode_proyek and a.kode_lokasi=b.kode_lokasi "+filter;
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from prb_proyek_app where kode_lokasi='"+this.app._lokasi+"' ",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_tu_proyek_flProyekApp]::mainButtonClick:"+e);
		}
	},
