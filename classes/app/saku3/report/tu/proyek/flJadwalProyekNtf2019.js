window.app_saku3_report_tu_proyek_flJadwalProyekNtf2019 = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_proyek_flJadwalProyekNtf2019.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_proyek_flJadwalProyekNtf2019";
		this.maximize();
    ...
    this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from prb_proyek_d where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_tu_proyek_flJadwalProyekNtf2019.extend(window.childForm);
window.app_saku3_report_tu_proyek_flJadwalProyekNtf2019.implement({
	doEllipseClick: function(sender, col, row){
		try{
			if (row ==	1)
			{
                var sql1 =  "select distinct a.kode_proyek, a.nama from prb_proyek a inner join prb_proyek_d b on a.kode_proyek=b.kode_proyek and a.kode_lokasi=b.kode_lokasi "+
                this.filterRep.filterStr("b.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ") + " and a.kode_lokasi='"+this.app._lokasi+"' ";

                var sql2 =  "select count(distinct a.kode_proyek) as jum from prb_proyek a inner join prb_proyek_d b on a.kode_proyek=b.kode_proyek and a.kode_lokasi=b.kode_lokasi "+
                this.filterRep.filterStr("b.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ") + " and a.kode_lokasi='"+this.app._lokasi+"' ";

                this.filterRep.ListDataSGFilter(this, "Data Proyek",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,new Array("kode_proyek","nama"),"and");
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1],["1239","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1],[0,2]);	
		if (row == 0)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from prb_proyek_d where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.kode_proyek",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_tu_proyek_rptJadwalProyekNtf";
			...
			systemAPI.alert("[app_saku3_report_tu_proyek_flJadwalProyekNtf2019]::mainButtonClick:"+e);
		}
	},
