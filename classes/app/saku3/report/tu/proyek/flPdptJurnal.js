window.app_saku3_report_tu_proyek_flPdptJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_proyek_flPdptJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_proyek_flPdptJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where form='GENPYT'  ");
...
window.app_saku3_report_tu_proyek_flPdptJurnal.extend(window.childForm);
window.app_saku3_report_tu_proyek_flPdptJurnal.implement({
		...
		try{			
			...
			if (row ==2)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							"and a.form='GENPYT'";
				this.filterRep.ListDataSGFilter(this, "Data Jurnal Akru Pendapatan",this.sg1, this.sg1.row, this.sg1.col,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where form='GENPYT' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_tu_proyek_flPdptJurnal]::mainButtonClick:"+e);
		}
	},
