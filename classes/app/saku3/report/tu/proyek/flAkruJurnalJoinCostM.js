window.app_saku3_report_tu_proyek_flAkruJurnalJoinCostM = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_proyek_flAkruJurnalJoinCostM.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_proyek_flAkruJurnalJoinCostM";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where modul in ('MJC','IJC') ");
...
window.app_saku3_report_tu_proyek_flAkruJurnalJoinCostM.extend(window.childForm);
window.app_saku3_report_tu_proyek_flAkruJurnalJoinCostM.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where modul in ('MJC','IJC')  and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_tu_proyek_flAkruJurnalJoinCostM]::mainButtonClick:"+e);
		}
	},
