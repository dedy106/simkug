window.app_saku3_report_tu_proyek_flAmandemenJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_proyek_flAmandemenJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_proyek_flAmandemenJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where form='AMD'  ");
...
window.app_saku3_report_tu_proyek_flAmandemenJurnal.extend(window.childForm);
window.app_saku3_report_tu_proyek_flAmandemenJurnal.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where modul in ('PRPIU','AR') and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_tu_proyek_rptAmandemenJurnal";
			...
			systemAPI.alert("[app_saku3_report_tu_proyek_flAmandemenJurnal]::mainButtonClick:"+e);
		}
	},
