window.app_saku3_report_tu_proyek_flPdptShare = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_proyek_flPdptShare.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_proyek_flPdptShare";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where form='PDSHARE'  ");
...
window.app_saku3_report_tu_proyek_flPdptShare.extend(window.childForm);
window.app_saku3_report_tu_proyek_flPdptShare.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where form='PDSHARE' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_tu_proyek_flPdptShare]::mainButtonClick:"+e);
		}
	},
