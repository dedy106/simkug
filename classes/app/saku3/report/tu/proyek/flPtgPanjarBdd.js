window.app_saku3_report_tu_proyek_flPtgPanjarBdd = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_proyek_flPtgPanjarBdd.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_proyek_flPtgPanjarBdd";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from it_aju_m where form='NTF21' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_tu_proyek_flPtgPanjarBdd.extend(window.childForm);
window.app_saku3_report_tu_proyek_flPtgPanjarBdd.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from it_aju_m where form='NTF21' and kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_tu_proyek_flPtgPanjarBdd]::mainButtonClick:"+e);
		}
	},
