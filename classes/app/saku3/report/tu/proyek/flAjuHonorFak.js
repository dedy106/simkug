window.app_saku3_report_tu_proyek_flAjuHonorFak = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_proyek_flAjuHonorFak.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_proyek_flAjuHonorFak";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from it_honorntf_m where kode_lokasi='"+this.app._lokasi+"'  ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode Fakultas","All",""));
...
window.app_saku3_report_tu_proyek_flAjuHonorFak.extend(window.childForm);
window.app_saku3_report_tu_proyek_flAjuHonorFak.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 2)
			{
				...
				if (this.app._userStatus=="A")
				{
					var sql="select a.kode_pp,a.nama "+
						...
						"where a.kode_lokasi='"+this.app._lokasi+"' and a.nama like 'Fakultas%' ";
					...
					this.filterRep.ListDataSGFilter(this, "Data PP",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,new Array("a.kode_pp","a.nama"),"and",new Array("kode","nama"));
			...
			// 	this.filterRep.ListDataSGFilter(this, "Data Honor",this.sg1, this.sg1.row, this.sg1.col,
			// 									"select a.no_bukti, a.nama from it_honorntf_m a "+filter,
			// 									"select count(a.no_bukti) from it_honorntf_m a "+filter,
			// 									new Array("a.no_bukti","a.nama"),"and",new Array("No Bukti","Keterangan"));
			// }
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode as periode from it_honorntf_m where kode_lokasi='"+this.app._lokasi+"'  order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
                ...
                this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.filter3;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_tu_proyek_rptAjuHonorFak";
			...
			systemAPI.alert("[app_saku3_report_tu_proyek_flAjuHonorFak]::mainButtonClick:"+e);
		}
	},
	...
	doOpenKartu: function(no_bukti,kode_lokasi){
		...
		filter = "where a.kode_proyek='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._namaUser;
		nama_report="server_report_saku3_tu_proyek_rptKartuNTF";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenTglHis: function(no_bukti,kode_lokasi){
		this.link="2";
		filter = "where a.kode_proyek='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._lokasi+"/"+this.app._periode;
		nama_report="server_report_saku3_tu_proyek_rptTglAdmHistory";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenKartu2: function(no_bukti,kode_lokasi){
		this.link="2";
		filter = "where a.kode_proyek='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._namaUser;
		nama_report="server_report_saku3_tu_proyek_rptKartuNTFNonBmhd";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
