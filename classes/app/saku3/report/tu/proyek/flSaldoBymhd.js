// JavaScript Document
window.app_saku3_report_tu_proyek_flSaldoBymhd = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_proyek_flSaldoBymhd.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_proyek_flSaldoBymhd";
		this.maximize();
	...
	var sql="select max(a.periode) as periode from (select distinct periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and form in ('PRPJPTG','PRAGENDA','PJPROYEK') "+
					"union "+
					"select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBITAJU')a ";
...
window.app_saku3_report_tu_proyek_flSaldoBymhd.extend(window.portalui_childForm);
window.app_saku3_report_tu_proyek_flSaldoBymhd.implement({
		...
		if (row ==0)
		{
			...
			if (row == 2)
			{
				var filter=this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							...
							" and form in ('PRPJPTG','PRAGENDA','PJPROYEK') ";
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_aju, keterangan from it_aju_m "+filter ,
														  "select count(no_aju) from it_aju_m "+filter,
														  new Array("no_aju","keterangan"),"and");
			}
	},
	doSelectCell : function(sender, col, row){
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2],["123","63","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2],[2,0,2]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			var sql="select distinct periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and form in ('PRPJPTG','PRAGENDA','PJPROYEK') "+
					"union "+
					"select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBITAJU' "+
		...
		try
		{
			...
			else
			{
				...
				this.filter = this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
					  ...
					  this.filterRep.filterStr("a.no_aju",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," and ")+
							" and a.form in ('PRPJPTG','PRAGENDA','PJPROYEK') ";
				this.filter2 =this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0);
				
				this.nama_report="server_report_saku3_tu_proyek_rptSaldoBymhd";
				this.dbLib.execQuerySync(this.sql);	
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku2_kopeg_kbitt_rptBebanFormTu";
	...
	doOpenJu: function(no_bukti,kode_lokasi){
		this.link="1";
		filter2 = "where a.no_aju='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter="";
