window.app_saku3_report_tu_proyek_flKartu = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_proyek_flKartu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_proyek_flKartu";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from it_aju_m where form='INISIAL' and kode_lokasi='"+this.app._lokasi+"'  ");
...
window.app_saku3_report_tu_proyek_flKartu.extend(window.childForm);
window.app_saku3_report_tu_proyek_flKartu.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and a.modul='UMUM' and a.form='INISIAL' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_tu_proyek_rptKartu";
			...
			systemAPI.alert("[app_saku3_report_tu_proyek_flKartu]::mainButtonClick:"+e);
		}
	},
