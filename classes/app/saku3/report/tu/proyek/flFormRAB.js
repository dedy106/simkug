window.app_saku3_report_tu_proyek_flFormRAB = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_proyek_flFormRAB.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_proyek_flFormRAB";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from prb_rabapp_m where kode_lokasi='"+this.app._lokasi+"'  ");
...
window.app_saku3_report_tu_proyek_flFormRAB.extend(window.childForm);
window.app_saku3_report_tu_proyek_flFormRAB.implement({
		...
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data RAB Proyek",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_rab, a.keterangan from prb_rabapp_m a "+filter,
												"select count(a.no_rab) from prb_rabapp_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from prb_rabapp_m where kode_lokasi='"+this.app._lokasi+"' ",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_tu_proyek_flFormRAB]::mainButtonClick:"+e);
		}
	},
