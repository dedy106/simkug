window.app_saku3_report_tu_proyek_flPtgPanjarBebanPos = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_proyek_flPtgPanjarBebanPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_proyek_flPtgPanjarBebanPos";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and modul='PJPTG' and form='ISPJPTG'  ");
...
window.app_saku3_report_tu_proyek_flPtgPanjarBebanPos.extend(window.childForm);
window.app_saku3_report_tu_proyek_flPtgPanjarBebanPos.implement({
		...
		try{			
			...
			if (row ==5)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.modul='PJPTG' and a.form='ISPJPTG' ";
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_tu_proyek_rptPtgPanjarBebanPos";
			...
			systemAPI.alert("[app_saku3_report_tu_proyek_flPtgPanjarBebanPos]::mainButtonClick:"+e);
		}
	},
