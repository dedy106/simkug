window.app_saku3_report_tu_proyek_flRab = function (owner) {
	if (owner) {
		window.app_saku3_report_tu_proyek_flRab.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_report_tu_proyek_flRab";
		this.maximize();
	...
	this.periode = this.dbLib.getPeriodeFromSQL("select max(periode) as periode from prb_rab_m where kode_lokasi='" + this.app._lokasi + "'  ");
...
window.app_saku3_report_tu_proyek_flRab.extend(window.childForm);
window.app_saku3_report_tu_proyek_flRab.implement({
		...
		try {
			...
			if (row == 4) {
				...
				this.filterRep.ListDataSGFilter(this, "Data RAB Proyek", this.sg1, this.sg1.row, this.sg1.col,
					"select a.no_rab, a.keterangan from prb_rab_m a " + filter,
					"select count(a.no_rab) from prb_rab_m a " + filter,
	...
	doSelectCell: function (sender, col, row) {
		if (this.app._userStatus == "A") {
			...
			if (row == 1) {
				...
				this.dbLib.setItemsFromSQL("select distinct periode from prb_rab_m where kode_lokasi='" + this.app._lokasi + "' ", [this.sg1.columns.get(2).pickList, this.sg1.columns.get(3).pickList]);
			}
		} else {
			this.filterRep.setSGFilterRowTipe(this.sg1, row, [0, 1, 2, 3, 4, 5], ["123", "123", "23", "123", "123", "123"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row, [0, 1, 2, 3, 4, 5], [2, 0, 2, 2, 2, 2]);
			if (row == 1) {
				this.sg1.columns.get(2).pickList.clear();
				this.dbLib.setItemsFromSQL("select distinct periode from prb_rab_m where kode_lokasi='" + this.app._lokasi + "' ", [this.sg1.columns.get(2).pickList, this.sg1.columns.get(3).pickList]);
		...
		try {
			if (sender === this.app._mainForm.bClear2) {
				...
				this.filter = this.filterRep.filterStr("a.kode_lokasi", this.sg1.getCell(1, 0), this.sg1.getCell(2, 0), this.sg1.getCell(3, 0), "where") +
					...
					this.filterRep.filterStr("a.kode_pp", this.sg1.getCell(1, 2), this.sg1.getCell(2, 2), this.sg1.getCell(3, 2), "and") +
					this.filterRep.filterStr("a.kode_cust", this.sg1.getCell(1, 3), this.sg1.getCell(2, 3), this.sg1.getCell(3, 3), "and") +
					this.filterRep.filterStr("a.no_rab", this.sg1.getCell(1, 4), this.sg1.getCell(2, 4), this.sg1.getCell(3, 4), "and");
			...
			systemAPI.alert("[app_saku3_report_tu_proyek_flRab]::mainButtonClick:" + e);
		}
	},
