window.app_saku3_report_tu_proyek_flPanjarBeban = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_proyek_flPanjarBeban.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_proyek_flPanjarBeban";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and modul='PANJAR' and form'PJPR' ");
...
window.app_saku3_report_tu_proyek_flPanjarBeban.extend(window.childForm);
window.app_saku3_report_tu_proyek_flPanjarBeban.implement({
		...
		try{			
			...
			if (row ==5)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.modul='PANJAR' and a.form='PJPR' ";
				var sql="select a.no_aju,a.keterangan "+
						"from it_aju_m a "+
						"inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
						"inner join prb_prbeban_d c on a.no_aju=c.no_bukti and a.kode_lokasi=c.kode_lokasi "+
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and modul='PANJAR' and form='PJPR' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_tu_proyek_flPanjarBeban]::mainButtonClick:"+e);
		}
	},
