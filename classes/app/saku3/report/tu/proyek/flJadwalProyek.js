window.app_saku3_report_tu_proyek_flJadwalProyek = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_proyek_flJadwalProyek.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_proyek_flJadwalProyek";
		this.maximize();
    ...
    this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from tu_proyek_d where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_tu_proyek_flJadwalProyek.extend(window.childForm);
window.app_saku3_report_tu_proyek_flJadwalProyek.implement({
	doEllipseClick: function(sender, col, row){
		try{
			if (row ==	1)
			{
                var sql1 =  "select distinct a.kode_proyek, a.nama from tu_proyek a inner join tu_proyek_d b on a.kode_proyek=b.kode_proyek and a.kode_lokasi=b.kode_lokasi "+
                ...
                var sql2 =  "select count(distinct a.kode_proyek) as jum from tu_proyek a inner join tu_proyek_d b on a.kode_proyek=b.kode_proyek and a.kode_lokasi=b.kode_lokasi "+
                this.filterRep.filterStr("b.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ") + " and a.kode_lokasi='"+this.app._lokasi+"' ";
	...
	{		
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from tu_proyek_d where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_tu_proyek_rptJadwalProyek";
			...
			systemAPI.alert("[app_saku3_report_tu_proyek_flJadwalProyek]::mainButtonClick:"+e);
		}
	},
