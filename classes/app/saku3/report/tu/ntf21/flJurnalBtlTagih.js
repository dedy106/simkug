window.app_saku3_report_tu_ntf21_flJurnalBtlTagih = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_ntf21_flJurnalBtlTagih.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_ntf21_flJurnalBtlTagih";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where modul='AR' and form='BTLBILL' ");
...
window.app_saku3_report_tu_ntf21_flJurnalBtlTagih.extend(window.childForm);
window.app_saku3_report_tu_ntf21_flJurnalBtlTagih.implement({
		...
		try{			
			...
			if (row ==2)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.modul='AR' and a.form='BTLBILL'";
				this.filterRep.ListDataSGFilter(this, "Data Jurnal Akru Beban",this.sg1, this.sg1.row, this.sg1.col,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where modul='AR' and form='BTLBILL' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_tu_ntf21_rptJurnalBtlTagih";
			...
			systemAPI.alert("[app_saku3_report_tu_ntf21_flJurnalBtlTagih]::mainButtonClick:"+e);
		}
	},
