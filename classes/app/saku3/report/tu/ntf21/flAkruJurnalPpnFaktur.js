window.app_saku3_report_tu_ntf21_flAkruJurnalPpnFaktur = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_ntf21_flAkruJurnalPpnFaktur.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_ntf21_flAkruJurnalPpnFaktur";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where modul='AR' and form='PRPIU' ");
...
window.app_saku3_report_tu_ntf21_flAkruJurnalPpnFaktur.extend(window.childForm);
window.app_saku3_report_tu_ntf21_flAkruJurnalPpnFaktur.implement({
		...
		try{			
			...
			if (row ==2)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.modul='AR' and a.form='PRPIU'";
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where modul='AR' and form='PRPIU' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_tu_ntf21_flAkruJurnalPpnFaktur]::mainButtonClick:"+e);
		}
	},
