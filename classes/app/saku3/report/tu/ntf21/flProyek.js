window.app_saku3_report_tu_ntf21_flProyek = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_ntf21_flProyek.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_ntf21_flProyek";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(dbo.fnPeriode(tgl_app)) as periode from prb_proyek where versi ='NTF21' and kode_lokasi='"+this.app._lokasi+"'  ");
...
window.app_saku3_report_tu_ntf21_flProyek.extend(window.childForm);
window.app_saku3_report_tu_ntf21_flProyek.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				else
				{
					var sql="select a.kode_pp,a.nama "+
						"from pp a "+ 
						"inner join karyawan_pp b on a.kode_pp=b.kode_pp and b.kode_lokasi=a.kode_lokasi and b.nik='"+this.app._userLog+"' "+
						"where a.kode_lokasi='"+this.app._lokasi+"' ";
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct dbo.fnPeriode(tgl_app) as periode from prb_proyek where versi ='NTF21' and kode_lokasi='"+this.app._lokasi+"'  order by dbo.fnPeriode(tgl_app)",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_tu_ntf21_rptProyek";
			...
			systemAPI.alert("[app_saku3_report_tu_ntf21_flProyek]::mainButtonClick:"+e);
		}
	},
