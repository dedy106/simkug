window.app_saku3_report_tu_flProyekSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_flProyekSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_flProyekSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(dbo.fnPeriode(tgl_app)) as periode from tu_proyek where kode_lokasi='"+this.app._lokasi+"'  ");
	
	this.periode2=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from tu_prbdd_d where kode_lokasi='"+this.app._lokasi+"'  ");
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Periode Proyek","<=",this.periode));
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Mitra","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Kode PP","=",this.app._kodePP));
	...
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Periode Rekon","=",this.periode2));
...
window.app_saku3_report_tu_flProyekSaldo.extend(window.childForm);
window.app_saku3_report_tu_flProyekSaldo.implement({
		...
		try{			
			...
			if (row ==5)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("dbo.fnPeriode(a.tgl_app)",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
	...
	{		
		if (this.app._userStatus=="A")
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123","1235","123","123","123","123","3"]);
		else if(this.app._userStatus=="P")  this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123","1235","123","23","123","123","3"]);
		else this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123","1235","123","3","123","123","3"]);
	
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[2,0,2,2,2,0,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct dbo.fnPeriode(tgl_app) as periode from tu_proyek where kode_lokasi='"+this.app._lokasi+"' order by dbo.fnPeriode(tgl_app) ",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 6)
		{
			this.sg1.columns.get(2).pickList.clear();
			var sql="select distinct periode as periode from tu_prbdd_d where kode_lokasi='"+this.app._lokasi+"'  union "+
					"select distinct periode as periode from tu_prpyt_d where kode_lokasi='"+this.app._lokasi+"' union "+
					"select distinct periode as periode from ju_j where kode_lokasi='"+this.app._lokasi+"' and jenis='PIU'  "+
					" order by periode";
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_proyek",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,6)+"/"+this.sg1.getCell(2,7);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_tu_rptProyekSaldo";
			...
			systemAPI.alert("[app_saku3_report_tu_flProyekSaldo]::mainButtonClick:"+e);
		}
	},
	...
	doOpenKartu: function(kode_proyek,kode_lokasi){
		...
		nama_report="server_report_saku3_tu_rptProyekKartu";
