// JavaScript Document
window.app_saku3_report_tu_bmhd_flAjuBhmdSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_bmhd_flAjuBhmdSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_bmhd_flAjuBhmdSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from bmhd_m where kode_lokasi='"+this.app._lokasi+"' ");
	...
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Status","=","Outstanding"));
...
window.app_saku3_report_tu_bmhd_flAjuBhmdSaldo.extend(window.portalui_childForm);
window.app_saku3_report_tu_bmhd_flAjuBhmdSaldo.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row ==3)
		{
			this.filterRep.ListDataSGFilter(this, "Data Vendor",this.sg1, this.sg1.row, this.sg1.col,
											"select kode_vendor, nama from it_vendor where kode_lokasi='"+this.app._lokasi+"'",
											"select count(*) from it_vendor where kode_lokasi='"+this.app._lokasi+"'",
		...
		if (row == 4)
		{
			...
			var sql1="select a.no_bmhd,a.keterangan from bmhd_m a "+filter; 
			var sql2="select count(a.no_bmhd) from bmhd_m a "+filter;
			this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
			                                new Array("a.no_bmhd","a.keterangan"),"and",new Array("No Bukti","Keterangan"),"and");
		
		}
	},
	doSelectCell: function(sender, col, row){
		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5),new  Array("123","123","123","123","123","3"));
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5),new  Array(2,0,2,2,2,0));
		
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from bmhd_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row === 5)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.filter=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
						...
						this.filterRep.filterStr("a.no_bmhd",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
			this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,5);
			this.nama_report="server_report_saku3_tu_bmhd_rptAjuBmhdSaldo";
	...
	doOpenKartu: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_bmhd='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
		nama_report="server_report_saku3_siswa_rptBmhdKartu";
	...
	doOpenJurnal: function(no_bukti,kode_lokasi,periode){
		this.link="1";
		var urut="b.no_bmhd,b.tanggal,b.no_urut";
		filter2 = "where a.no_bmhd='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' and a.periode='"+periode+"' ";
		filter=filter2+"/"+urut+"/"+this.sg1.getCell(2,1);
		nama_report="server_report_saku3_tu_bmhd_rptJurnalHU";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
