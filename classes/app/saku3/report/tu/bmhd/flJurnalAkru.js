// JavaScript Document
window.app_saku3_report_tu_bmhd_flJurnalAkru = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_bmhd_flJurnalAkru.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_bmhd_flJurnalAkru";
		this.maximize();
...
window.app_saku3_report_tu_bmhd_flJurnalAkru.extend(window.portalui_childForm);
window.app_saku3_report_tu_bmhd_flJurnalAkru.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 7)
		{
			...
			var sql1="select distinct a.no_ju,a.no_dokumen,a.keterangan from ju_m a where a.modul='BMHDREKLAS' "+filter; 
			var sql2="select count(distinct a.no_ju) from ju_m a where a.modul='BMHDREKLAS' "+filter;
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from ju_m where modul='BMHDREKLAS' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 5)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			{
				this.nama_report="server_report_saku3_tu_bmhd_rptJurnal";
			}
