window.app_saku3_report_tu_bmhd_flAjuBhmdPos = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_bmhd_flAjuBhmdPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_bmhd_flAjuBhmdPos";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and form='BMHD'");
...
window.app_saku3_report_tu_bmhd_flAjuBhmdPos.extend(window.childForm);
window.app_saku3_report_tu_bmhd_flAjuBhmdPos.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and form='BMHD' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_tu_bmhd_rptAjuBmhdPos";
			...
			systemAPI.alert("[app_saku3_report_tu_bmhd_flAjuBhmdPos]::mainButtonClick:"+e);
		}
	},
