window.app_saku3_report_tu_flProyekDistRevJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_flProyekDistRevJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_flProyekDistRevJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from tu_prbeban_m where modul='REVBMHD'  ");
...
window.app_saku3_report_tu_flProyekDistRevJurnal.extend(window.childForm);
window.app_saku3_report_tu_flProyekDistRevJurnal.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from tu_prbeban_m where  modul='REVBMHD' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_tu_flProyekDistRevJurnal]::mainButtonClick:"+e);
		}
	},
