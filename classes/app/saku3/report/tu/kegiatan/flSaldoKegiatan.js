window.app_saku3_report_tu_kegiatan_flSaldoKegiatan = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_kegiatan_flSaldoKegiatan.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_kegiatan_flSaldoKegiatan";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select substring(max(periode),1,4) as periode from keg_aju_m where kode_lokasi='"+this.app._lokasi+"' and progress <> 'X'");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode Kegiatan","All",""));
...
window.app_saku3_report_tu_kegiatan_flSaldoKegiatan.extend(window.childForm);
window.app_saku3_report_tu_kegiatan_flSaldoKegiatan.implement({
		...
		try{			
			...
			if (row ==	2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Kegiatan",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_aju, keterangan from keg_aju_m "+this.filter,
														  "select count(no_aju) from keg_aju_m "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from keg_aju_m where kode_lokasi='"+this.sg1.getCell(2,0)+"' and progress <> 'X' ",[this.sg1.columns.get(2).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_tu_kegiatan_rptKegiatanSaldo";
			...
			systemAPI.alert("[app_saku3_report_tu_kegiatan_flSaldoKegiatan]::mainButtonClick:"+e);
		}
	},
	...
	doOpenKartu: function(no_aju,kode_lokasi){
		...
		nama_report="server_report_saku3_tu_kegiatan_rptKegiatanKartu";
