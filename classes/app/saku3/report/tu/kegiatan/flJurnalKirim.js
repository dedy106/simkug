window.app_saku3_report_tu_kegiatan_flJurnalKirim = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_kegiatan_flJurnalKirim.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_kegiatan_flJurnalKirim";
		this.maximize();
...
window.app_saku3_report_tu_kegiatan_flJurnalKirim.extend(window.childForm);
window.app_saku3_report_tu_kegiatan_flJurnalKirim.implement({
		...
		try{			
			...
			if (row ==2)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.jenis = 'KIRIM' and a.modul='TAKPJPTG' ";
				this.filterRep.ListDataSGFilter(this, "Data TAK Kirim",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_kirim,a.keterangan from takkirim_m a "+filter,
												"select count(a.no_kirim) from takkirim_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from takkirim_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_tu_kegiatan_rptJurnalKirim";
			...
			systemAPI.alert("[app_saku3_report_tu_kegiatan_flJurnalKirim]::mainButtonClick:"+e);
		}
	},
