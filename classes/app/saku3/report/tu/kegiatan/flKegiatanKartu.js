window.app_saku3_report_tu_kegiatan_flKegiatanKartu = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_kegiatan_flKegiatanKartu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_kegiatan_flKegiatanKartu";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from keg_aju_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_tu_kegiatan_flKegiatanKartu.extend(window.childForm);
window.app_saku3_report_tu_kegiatan_flKegiatanKartu.implement({
		...
		try{			
			...
			if (row ==	2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Kegiatan",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.no_aju, a.keterangan from keg_aju_m a "+filter ,
														  "select count(a.no_aju) from keg_aju_m a "+filter,
															new Array("a.no_aju","a.keterangan"),"where",new Array("kode","nama"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from keg_aju_m where kode_lokasi='"+this.app._lokasi+"'",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_tu_kegiatan_rptKegiatanKartu";
			...
			systemAPI.alert("[app_saku3_report_tu_kegiatan_flKegiatanKartu]::mainButtonClick:"+e);
		}
	},
