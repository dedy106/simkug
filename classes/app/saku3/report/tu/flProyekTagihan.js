window.app_saku3_report_tu_flProyekTagihan = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_flProyekTagihan.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_flProyekTagihan";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from tu_prbill_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_tu_flProyekTagihan.extend(window.childForm);
window.app_saku3_report_tu_flProyekTagihan.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==5)
			{
				...
				var sql="select a.no_bill,a.keterangan "+
						"from tu_prbill_m a "+
						"inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
						"inner join tu_proyek d on a.kode_proyek=d.kode_proyek and a.kode_lokasi=d.kode_lokasi "+filter;
				var sql1="select count(a.no_bill) "+
						"from tu_prbill_m a "+
						"inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
						"inner join tu_proyek d on a.kode_proyek=d.kode_proyek and a.kode_lokasi=d.kode_lokasi "+filter;		
				this.filterRep.ListDataSGFilter(this, "Data Tagihan",this.sg1, this.sg1.row, this.sg1.col,sql,sql1,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from tu_prbill_m where kode_lokasi='"+this.app._lokasi+"'  order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("b.kode_jenis",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("b.kode_proyek",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
			...
			systemAPI.alert("[app_saku3_report_tu_flProyekTagihan]::mainButtonClick:"+e);
		}
	},
