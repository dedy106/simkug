window.app_saku3_report_tu_flProyekKasJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_flProyekKasJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_flProyekKasJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from ju_m where kode_lokasi='"+this.app._lokasi+"' and jenis='REKON'  ");
...
window.app_saku3_report_tu_flProyekKasJurnal.extend(window.childForm);
window.app_saku3_report_tu_flProyekKasJurnal.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and jenis='REKON' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_tu_rptProyekKasJurnal"; 
			...
			systemAPI.alert("[app_saku3_report_tu_flProyekKasJurnal]::mainButtonClick:"+e);
		}
	},
