window.app_saku3_report_tu_flProyekRabPos = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_flProyekRabPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_flProyekRabPos";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from tu_rab_m where kode_lokasi='"+this.app._lokasi+"'  ");
...
window.app_saku3_report_tu_flProyekRabPos.extend(window.childForm);
window.app_saku3_report_tu_flProyekRabPos.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==5)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data RAB Proyek",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_rab, a.keterangan from tu_rab_m a "+filter,
												"select count(a.no_rab) from tu_rab_m a "+filter,
												new Array("a.no_rab","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from tu_rab_m where kode_lokasi='"+this.app._lokasi+"'  order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_cust",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.kode_jenis",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							this.filterRep.filterStr("a.no_rab",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");;
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_tu_rptProyekRabPos";
			...
			systemAPI.alert("[app_saku3_report_tu_flProyekRabPos]::mainButtonClick:"+e);
		}
	},
	...
	doOpenRab: function(no_bukti,kode_lokasi){
		
		this.no_bukti=no_bukti;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = "where a.no_rab='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);;
		
		nama_report="server_report_saku3_tu_rptProyekRab";
