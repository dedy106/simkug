window.app_saku3_report_tu_flProduk = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_flProduk.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_flProduk";
		this.maximize();
...
window.app_saku3_report_tu_flProduk.extend(window.childForm);
window.app_saku3_report_tu_flProduk.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==	1)
			{
				this.filterRep.ListDataSGFilter(this, "Data Produk",this.sg1, this.sg1.row, this.sg1.col,
														  "select distinct kode_produk, nama from aka_produk "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(kode_produk) from aka_produk "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  new Array("kode_produk","nama"),"and");
			}
			if (row ==	2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Angkatan",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_akt, nama from aka_angkatan "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(kode_akt) from aka_angkatan "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  new Array("kode_akt","nama"),"and");
			}
			if (row ==	3)
			{
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filterRep.filterStr("kode_produk",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
				this.filterRep.filterStr("kode_akt",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
				this.filterRep.filterStr("akun_pdpt",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_tu_rptProduk";
			...
			systemAPI.alert("[app_saku3_report_tu_flProduk]::mainButtonClick:"+e);
		}
	},
