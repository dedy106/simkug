window.app_saku3_report_tu_flProyekBillJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_flProyekBillJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_flProyekBillJurnal";
		this.maximize();
...
window.app_saku3_report_tu_flProyekBillJurnal.extend(window.childForm);
window.app_saku3_report_tu_flProyekBillJurnal.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_bill, a.keterangan from tu_prbill_m a "+filter,
												"select count(a.no_bill) from tu_prbill_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from tu_prbill_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_tu_rptProyekBillJurnal";
			...
			systemAPI.alert("[app_saku3_report_tu_flProyekBillJurnal]::mainButtonClick:"+e);
		}
	},
