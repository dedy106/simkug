window.app_saku3_report_tu_flProyekPanjar = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_flProyekPanjar.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_flProyekPanjar";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and modul='PANJAR' and form='PJPROYEK' ");
...
window.app_saku3_report_tu_flProyekPanjar.extend(window.childForm);
window.app_saku3_report_tu_flProyekPanjar.implement({
		...
		try{			
			...
			if (row ==5)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.modul='PANJAR' and a.form='PJPROYEK' ";
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and modul='PANJAR' and form='PJPROYEK' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_tu_flProyekPanjar]::mainButtonClick:"+e);
		}
	},
