window.app_saku3_report_tu_flProyekBebanPos = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_flProyekBebanPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_flProyekBebanPos";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and form in ('PRAGENDA','PROYEK') ");
	...
	{
		this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Kode PP","=",""));
	}else{
	...
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("No Proyek","All",""));
...
window.app_saku3_report_tu_flProyekBebanPos.extend(window.childForm);
window.app_saku3_report_tu_flProyekBebanPos.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 3)
			{
				...
				// "select a.kode_pp, a.nama from pp a inner join karyawan_pp  b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.nik='"+this.app._userLog+"' "+
				// this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
				// "select count(a.kode_pp) from pp a inner join karyawan_pp  b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.nik='"+this.app._userLog+"' "+
				// this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
				// new Array("a.kode_pp","a.nama"),"and");
				...
				else
				{
					...
					"where c.nik='"+this.app._userLog+"' "+filter;
			...
			if (row == 4)
			{
				this.filterRep.ListDataSGFilter(this, "Data Jenis Proyek",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_jenis, nama from tu_proyek_jenis "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(kode_jenis) from tu_proyek_jenis "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
			...
			if (row ==5)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_jenis",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
				...
				this.filterRep.ListDataSGFilter(this, "Data Proyek",this.sg1, this.sg1.row, this.sg1.col,
												"select a.kode_proyek, a.nama from tu_proyek a "+filter,
												"select count(a.kode_proyek) from tu_proyek a "+filter,
			...
			if (row ==6)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("d.kode_pp",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("d.kode_jenis",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							this.filterRep.filterStr("d.kode_proyek",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+
							" and a.modul='UMUM' and a.form in ('PRAGENDA','PROYEK') ";
				var sql="select a.no_aju,a.keterangan "+
						"from it_aju_m a "+
						"inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
						"inner join tu_prbdd_d c on a.no_aju=c.no_bukti and a.kode_lokasi=c.kode_lokasi and a.kode_akun=c.kode_akun "+
						"inner join tu_proyek d on c.kode_proyek=d.kode_proyek and c.kode_lokasi=d.kode_lokasi "+filter;
	...
	{		
		if (this.app._userStatus=="U")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123","123","123","3","123","123","123"]);
		}else{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123","123","123","123","123","123","123"]);
		}
		
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[2,0,2,2,2,2,2]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and modul='UMUM' and form in ('PRAGENDA','PROYEK') order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("n.kode_jenis",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							this.filterRep.filterStr("n.kode_proyek",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+							
							this.filterRep.filterStr("a.no_aju",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and")+
							" and a.modul='UMUM' and a.form in ('PRAGENDA','PROYEK') ";
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_tu_rptProyekBebanPos";
			...
			systemAPI.alert("[app_saku3_report_tu_flProyekBebanPos]::mainButtonClick:"+e);
		}
	},
