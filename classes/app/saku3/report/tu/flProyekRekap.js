window.app_saku3_report_tu_flProyekRekap = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_flProyekRekap.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_flProyekRekap";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from tu_prpiutang_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_tu_flProyekRekap.extend(window.childForm);
window.app_saku3_report_tu_flProyekRekap.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from tu_proyek where kode_lokasi='"+this.app._lokasi+"'  order by substring(periode,1,4)",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_tu_rptProyekRekap";
			...
			systemAPI.alert("[app_saku3_report_tu_flProyekRekap]::mainButtonClick:"+e);
		}
	},
	...
	doOpenDetail: function(tahun,kode_lokasi){
		
		this.tahun=tahun;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = "where year(a.tgl_mulai)="+tahun+" and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=kode_lokasi+"/";
		
		nama_report="server_report_saku3_tu_rptProyekRinci";
