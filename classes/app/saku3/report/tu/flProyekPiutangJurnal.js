window.app_saku3_report_tu_flProyekPiutangJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_flProyekPiutangJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_flProyekPiutangJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from tu_prpiutang_m  ");
...
window.app_saku3_report_tu_flProyekPiutangJurnal.extend(window.childForm);
window.app_saku3_report_tu_flProyekPiutangJurnal.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pemesanan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_bukti, a.keterangan from tu_prpiutang_m a "+filter,
												"select count(a.no_bukti) from tu_prpiutang_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from tu_prpiutang_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_tu_flProyekPiutangJurnal]::mainButtonClick:"+e);
		}
	},
