window.app_saku3_report_tu_flProyekPtg = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_flProyekPtg.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_flProyekPtg";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and modul='PJPTG' and form='PRPJPTG' ");
...
window.app_saku3_report_tu_flProyekPtg.extend(window.childForm);
window.app_saku3_report_tu_flProyekPtg.implement({
		...
		try{			
			...
			if (row ==5)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.modul='PJPTG' and a.form='PRPJPTG' ";
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' and a.modul='UMUM' and a.form='PROYEK' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_tu_flProyekPtg]::mainButtonClick:"+e);
		}
	},
