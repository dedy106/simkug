window.app_saku3_report_tu_flProyekRekonJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tu_flProyekRekonJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tu_flProyekRekonJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from ju_m where modul='PIUPRO'  ");
...
window.app_saku3_report_tu_flProyekRekonJurnal.extend(window.childForm);
window.app_saku3_report_tu_flProyekRekonJurnal.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from ju_m where modul='PIUPRO' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_tu_flProyekRekonJurnal]::mainButtonClick:"+e);
		}
	},
