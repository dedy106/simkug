window.app_saku3_report_siswa_flSisSiswaYpt = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flSisSiswaYpt.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flSisSiswaYpt";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Status Siswa","=","1"));
	this.gridLib.SGEditData(this.sg1,7,new Array(0,1,2), new Array("Output Laporan","=","Layar"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_siswa_flSisSiswaYpt.extend(window.childForm);
window.app_saku3_report_siswa_flSisSiswaYpt.implement({
		...
		try{			
			...
			if (row == 4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Angkatan",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.kode_akt, a.nama from sis_angkat a   "+filter+" order by a.kode_akt desc",
			...
			if (row == 5)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Siswa",this.sg1, this.sg1.row, this.sg1.col,
												...
												new Array("nis","nama"),"where",new Array("nis","nama"));
			}	
			if (row == 6)
			{
				var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
				...
				this.filterRep.ListDataSGFilter(this, "Data Status",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.kode_ss, a.nama from sis_siswa_status a   "+filter,
														  "select count(a.kode_ss) from sis_siswa_status a "+filter,
														  ["a.kode_ss","a.nama"],"and",["Kode","Nama"]);
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_siswa_rptSisSiswaYpt";
