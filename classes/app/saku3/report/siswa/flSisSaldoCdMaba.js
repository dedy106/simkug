window.app_saku3_report_siswa_flSisSaldoCdMaba = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flSisSaldoCdMaba.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flSisSaldoCdMaba";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from aka_cd_d where kode_lokasi='"+this.app._lokasi+"' ");
	...
	this.gridLib.SGEditData(this.sg1,7,new Array(0,1,2), new Array("Jenis","All",""));
	this.gridLib.SGEditData(this.sg1,8,new Array(0,1,2), new Array("No Bukti","All",""));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_siswa_flSisSaldoCdMaba.extend(window.childForm);
window.app_saku3_report_siswa_flSisSaldoCdMaba.implement({
		...
		try{			
			...
			if (row == 3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Angkatan",this.sg1, this.sg1.row, this.sg1.col,
														  "select distinct a.tahunaka from aka_maba a   "+filter,
														  "select count(distinct a.tahunaka) from aka_maba a "+filter,
														  ["a.tahunaka"],"and",["Kode"]);
			}
			if (row == 4)
			{
				...
				var sql1="select a.no_tes,a.nama from aka_maba a "+filter;
				var sql2="select count(a.no_tes) from aka_maba a "+filter;
				this.filterRep.ListDataSGFilter(this, "Data Mahasiswa Baru",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
												new Array("a.no_tes","a.nama"),"and",new Array("NIM","Nama"),"and");
			}	
			...
			if (row == 8)
			{
				...
				var filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							// this.filterRep.filterStr("c.flag_status",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");
				var sql1="select distinct a.no_bukti,b.keterangan "+
						"	from aka_cd_d a "+
						"	inner join ( "+
						"		select a.no_bukti,a.kode_lokasi,a.keterangan "+
						"		from trans_m a "+
						"		where a.form in ('CDIN','CDMUTMABA') "+
						"		union all "+
						"		select a.no_aju as no_bukti,a.kode_lokasi,a.keterangan "+
						"		from it_aju_m a "+
						"		where a.no_ref1 in ('MABA','MALA','CAMABA') "+
						"	) b on a.no_bukti=b.no_bukti and a.kode_lokasi=b.kode_lokasi "+
						"   inner join aka_maba c on a.nim=c.no_tes and a.kode_lokasi=c.kode_lokasi "+
						filter;
				// var sql1 = "select distinct a.no_bukti "+
				// 		   "from aka_cd_d a "+
				// 		   "inner join aka_mahasiswa c on a.nim=c.nim and a.kode_lokasi=c.kode_lokasi "+filter;

				var sql2="select count(distinct a.no_bukti) "+
						"	from aka_cd_d a "+
						"	inner join ( "+
						"		select a.no_bukti,a.kode_lokasi,a.keterangan "+
						"		from trans_m a "+
				...
				// var sql2 = "select count(distinct a.no_bukti) as jum "+
				// 		   "from aka_cd_d a "+
				// 		   "inner join aka_mahasiswa c on a.nim=c.nim and a.kode_lokasi=c.kode_lokasi "+filter;

				this.filterRep.ListDataSGFilter(this, "Data No Bukti",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
												new Array("a.no_bukti","b.keterangan"),"and",new Array("No Bukti","Keterangan"),"and");
			}	
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["3","3","123","123","123","13","3","13","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[2,0,2,2,2,0,0,0,2]);	
		if (row == 1)
		{
			var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where");
			var sql="select distinct a.periode from aka_cd_d a "+filter+" order by a.periode desc"
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("AKTIF","NONAKTIF"));
		}
		if (row == 6)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Masuk","Keluar"));
		}
	},
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.kode_jur",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.tahunaka",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.no_tes",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
							// this.filterRep.filterStr("a.flag_status",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");
				var filterBukti = this.filterRep.filterStr("a.no_bukti",this.sg1.getCell(1,8),this.sg1.getCell(2,8),this.sg1.getCell(3,8),"and");
				this.filter2 =this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,6)+"/"+this.sg1.getCell(2,7)+"."+filterBukti;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				
				
				this.nama_report = "server_report_saku3_siswa_rptSisSaldoCdMaba";
