window.app_saku3_report_siswa_flSisBea = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flSisBea.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flSisBea";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sis_rekon_m where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"' and modul='LOADBEA' ");
...
window.app_saku3_report_siswa_flSisBea.extend(window.childForm);
window.app_saku3_report_siswa_flSisBea.implement({
	...
	{		
		...
		if (row == 2)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from sis_rekon_m where kode_lokasi='"+this.app._lokasi+"' and modul='LOADBEA' order by periode",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 7)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_siswa_rptSisBea";
