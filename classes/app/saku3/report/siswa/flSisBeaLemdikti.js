window.app_saku3_report_siswa_flSisBeaLemdikti = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flSisBeaLemdikti.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flSisBeaLemdikti";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(a.tahun) as periode from ( "+
        "select substring(periode,1,4) as tahun from aka_bill_bea where kode_lokasi='"+this.app._lokasi+"' "+
        "union all "+
        "select substring(periode,1,4) as tahun from aka_bill_bea_h where kode_lokasi='"+this.app._lokasi+"' "+
        ") a");
...
window.app_saku3_report_siswa_flSisBeaLemdikti.extend(window.childForm);
window.app_saku3_report_siswa_flSisBeaLemdikti.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct a.tahun from ( "+
                "select substring(periode,1,4) as tahun from aka_bill_bea where  kode_lokasi='"+this.app._lokasi+"' "+
                "union all "+
                "select substring(periode,1,4) as tahun from aka_bill_bea_h where kode_lokasi='"+this.app._lokasi+"' "+
                ") a "+
                "order by a.tahun desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
                ...
                this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," where ")+
                this.filterRep.filterStr("a.tahun",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3)," and ");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_siswa_rptSisBeaLemdikti";
