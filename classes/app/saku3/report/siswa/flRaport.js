window.app_saku3_report_siswa_flRaport = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flRaport.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flRaport";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Data Raport", 2);
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select kode_ta as periode from sis_ta where kode_lokasi='"+this.app._lokasi+"'");				
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Tahun Ajaran","=",this.periode));
...
window.app_saku3_report_siswa_flRaport.extend(window.childForm);
window.app_saku3_report_siswa_flRaport.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==1)
			{
				this.filterRep.ListDataSGFilter(this, "Data TA",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.kode_ta, a.nama from sis_ta a  "+
														   this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(2,0)," where "),
														  "select count(a.kode_ta) from sis_ta a "+
														  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(2,0)," where "),
			...
			if (row ==3)
			{
				this.filterRep.ListDataSGFilter(this, "NIS",this.sg1, this.sg1.row, this.sg1.col,
												"select nis, nama from sis_siswa",
												"select count(*) from sis_siswa",
												new Array("nis","nama"),"where",new Array("Kode","Nama"));
			}					
		}catch(e){
			systemAPI.alert(e);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  ...
							  this.filterRep.filterStr("a.nis",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");

							  this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_siswa_rptRaport";
