window.app_saku3_report_siswa_flBayarDaftarLoad = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flBayarDaftarLoad.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flBayarDaftarLoad";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"' and modul in ('CDINREG','CDINREGIB')");
...
window.app_saku3_report_siswa_flBayarDaftarLoad.extend(window.childForm);
window.app_saku3_report_siswa_flBayarDaftarLoad.implement({
		...
		try{			
			...
			if (row ==3)
			{
				var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
					...
					" and a.modul in ('CDINREG','CDINREGIB')  ";	
	...
	{		
		...
		if (row == 2)
		{
			...
			var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
			this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
			this.filterRep.filterStr("a.periode",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+" and a.modul in ('CDINREG','CDINREGIB')";
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_siswa_rptJurByr";
	...
	doOpenDetailByr: function(no_bukti,kode_lokasi){
		this.link="2";
		filter = "where a.no_bukti='"+no_bukti+"' and a.modul in ('CDINREG','CDINREGIB') and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
		nama_report="server_report_saku3_siswa_rptSisBayarDaftar";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
