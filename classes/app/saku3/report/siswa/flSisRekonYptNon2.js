window.app_saku3_report_siswa_flSisRekonYptNon2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flSisRekonYptNon2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flSisRekonYptNon2";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"' and modul='LOADSISKB' ");
	...
	this.gridLib.SGEditData(this.sg1,7,[0,1,2],["Output Laporan","=","Layar"]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_siswa_flSisRekonYptNon2.extend(window.childForm);
window.app_saku3_report_siswa_flSisRekonYptNon2.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 5)
			{
                ...
                var filter2=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
                            this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
                            
				var sql1="select a.nis,a.nama from sis_siswa a inner join sis_kelas b on a.kode_kelas=b.kode_kelas and a.kode_lokasi=b.kode_lokasi and a.kode_pp=b.kode_pp "+filter+" union all select a.no_reg as nis,a.nama from sis_siswareg a "+filter2;
                var sql2="select count(a.nis) from (select a.nis,a.nama from sis_siswa a inner join sis_kelas b on a.kode_kelas=b.kode_kelas and a.kode_lokasi=b.kode_lokasi and a.kode_pp=b.kode_pp "+filter+" union all select a.no_reg as nis,a.nama from sis_siswareg a "+filter2+" ) a";
			...
			if (row ==6)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pembayaran",this.sg1, this.sg1.row, this.sg1.col,
												...
												"select count(a.no_kas) from kas_m a "+this.filter,
												new Array("a.no_kas","a.keterangan"),"and",new Array("No Tagihan","Keterangan"));
			}		
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["3","123","3","123","123","123","123","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[2,2,0,2,2,2,2,0]);	
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 7)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter2 =this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  ...
							  this.filterRep.filterStr("c.nis",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+							  							  							  
							  this.filterRep.filterStr("a.no_kas",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and")+
							  "/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,7);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				
				
				this.nama_report = "server_report_saku3_siswa_rptSisRekonYptNon2";
