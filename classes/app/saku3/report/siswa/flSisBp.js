window.app_saku3_report_siswa_flSisBp = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flSisBp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flSisBp";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Buku Penghubung", 2);
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(substring(replace(tanggal,'-',''),1,6)) as periode from sis_bp where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"'");
...
window.app_saku3_report_siswa_flSisBp.extend(window.childForm);
window.app_saku3_report_siswa_flSisBp.implement({
	...
	{		
		...
		if (row == 2)
		{
			var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and"); 
			var sql="select distinct substring(replace(a.tanggal,'-',''),1,6) as periode from sis_bp a "+filter+" order by substring(replace(a.tanggal,'-',''),1,6) desc";
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_siswa_rptBukuP";
