window.app_saku3_report_siswa_flCalonJurnalNon = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flCalonJurnalNon.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flCalonJurnalNon";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from ju_m where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"' ");
...
window.app_saku3_report_siswa_flCalonJurnalNon.extend(window.childForm);
window.app_saku3_report_siswa_flCalonJurnalNon.implement({
		...
		try{			
			...
			if (row ==3)
			{
				var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
					...
					" and a.modul in ('JUCDOUT','JUCDOUTREG') ";	
				this.filterRep.ListDataSGFilter(this, "Data Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.no_ju, a.keterangan from ju_m a   "+filter,
														  "select count(a.no_ju) from ju_m a "+filter,
														  ["a.no_ju","a.keterangan"],"and",["Kode","Keterangan"]);
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["3","3","13","123"]);
		...
		if (row == 2)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct a.periode from ju_m a "+filter+" order by a.periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}	
	},
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.ref1",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.no_ju",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							" and a.modul in ('JUCDOUT','JUCDOUTREG')";
	...
	doOpenJurnal: function(no_bukti,kode_lokasi){
		this.link="2";
		filter = "where no_ju='"+no_bukti+"' and kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
		nama_report="server_report_saku2_siaga_rptKasJurnal";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenDetailByr: function(no_bukti,kode_lokasi,nis){
		this.link="2";
		filter = "where a.no_bukti='"+no_bukti+"' and a.modul in ('CDOUT','CDOUTREG') and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._lokasi+"/"+this.app._kodePP+"/"+this.app._periode;
		nama_report="server_report_saku3_siswa_rptSisByrCalonNon";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
