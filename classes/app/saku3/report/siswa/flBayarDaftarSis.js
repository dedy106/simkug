window.app_saku3_report_siswa_flBayarDaftarSis = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flBayarDaftarSis.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flBayarDaftarSis";
		this.maximize();
...
window.app_saku3_report_siswa_flBayarDaftarSis.extend(window.childForm);
window.app_saku3_report_siswa_flBayarDaftarSis.implement({
	...
	{		
		...
		if (row == 2)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct a.periode from sis_cd_d a  "+filter+" order by a.periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}		
	},
