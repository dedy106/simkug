window.app_saku3_report_siswa_flBayarDaftar2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flBayarDaftar2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flBayarDaftar2";
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"' and modul in ('KBREG','KBBILLREG')");
...
window.app_saku3_report_siswa_flBayarDaftar2.extend(window.childForm);
window.app_saku3_report_siswa_flBayarDaftar2.implement({
		...
		try{			
			...
			if (row ==3)
			{
				var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
					...
					" and a.modul in ('KBREG','KBBILLREG') ";	
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_siswa_rptSisBayarDaftar2";
