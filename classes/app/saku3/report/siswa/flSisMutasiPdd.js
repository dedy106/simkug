window.app_saku3_report_siswa_flSisMutasiPdd = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flSisMutasiPdd.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flSisMutasiPdd";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sis_rekon_m where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"' and modul='CDMUTMULTI'");
...
window.app_saku3_report_siswa_flSisMutasiPdd.extend(window.childForm);
window.app_saku3_report_siswa_flSisMutasiPdd.implement({
		...
		try{			
			...
			if (row ==3)
			{
				var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
					...
					" and a.modul='CDMUTMULTI' ";	
				this.filterRep.ListDataSGFilter(this, "Data Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.no_rekon from sis_rekon_m a   "+filter,
														  "select count(a.no_rekon) from sis_rekon_m a "+filter,
														  ["a.no_rekon","a.keterangan"],"and",["No Bukti","Keterangan"]);
	...
	{		
		...
		if (row == 2)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct a.periode from sis_rekon_m a  "+filter+" order by a.periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}		
	},
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.modul='CDMUTMULTI' and a.dc='C' ";
				this.filter2 =this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_siswa_rptSisMutasiPdd";
