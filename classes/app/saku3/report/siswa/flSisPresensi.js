window.app_saku3_report_siswa_flSisPresensi = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flSisPresensi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flSisPresensi";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Presensi Siswa", 2);
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(substring(replace(tanggal,'-',''),1,6)) as periode from sis_presensi where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"'");
...
window.app_saku3_report_siswa_flSisPresensi.extend(window.childForm);
window.app_saku3_report_siswa_flSisPresensi.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["3","123","3","123","123","123","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[2,2,0,2,2,2,2]);	
		if (row == 2)
		{
			var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and"); 
			var sql="select distinct substring(replace(a.tanggal,'-',''),1,6) as periode from sis_presensi a "+filter+" order by substring(replace(a.tanggal,'-',''),1,6) desc";
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter =this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							  this.filterRep.filterStr("substring(replace(a.tanggal,'-',''),1,6)",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							  ...
							  this.filterRep.filterStr("d.kode_akt",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+
							  this.filterRep.filterStr("d.nis",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and");
				...
				this.filter2 = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  ...
							  "/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2);				
		    	
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_siswa_rptSisPresensi";
	...
	doOpenAbsenHar: function(nis,kode_lokasi,periode){
		...
		this.link="1";
		filter2 = "";
		filter="where a.nis='"+nis+"' and a.kode_lokasi='"+kode_lokasi+"' and substring(convert(varchar,a.tanggal,112),1,6)='"+periode+"' ";
		nama_report="server_report_saku3_siswa_rptSisPresensiHar";
