window.app_saku3_report_siswa_flSisKwitansiMulti = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flSisKwitansiMulti.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flSisKwitansiMulti";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Kwitansi", 2);
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"' ");
...
window.app_saku3_report_siswa_flSisKwitansiMulti.extend(window.childForm);
window.app_saku3_report_siswa_flSisKwitansiMulti.implement({
		...
		try{			
            ...
            if (row == 7)
			{
				...
				var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
                            ...
                            this.filterRep.filterStr("a.ref1",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");
                            // " and a.modul in ('KBBILSIS','KBCDIN') ";
				var sql1="select a.no_kas,a.keterangan from kas_m a inner join sis_siswa b on a.ref1=b.nis and a.kode_lokasi=b.kode_lokasi and a.kode_pp=b.kode_pp "+filter;
				var sql2="select count(a.no_kas) from kas_m a inner join sis_siswa b on a.ref1=b.nis and a.kode_lokasi=b.kode_lokasi and a.kode_pp=b.kode_pp "+filter;
				this.filterRep.ListDataSGFilter(this, "Data No Bukti",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
                ["a.no_kas","a.keterangan"],"and",["Kode","Nama"]);
			}			
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["3","23","3","123","123","123","123","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[2,2,0,2,2,2,2,2]);	
		if (row == 2)
		{
			var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
                        this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
                        " and modul in ('KBBILSIS','KBCDIN') "; 
			var sql="select distinct a.periode from kas_m a "+filter+" order by a.periode desc"
