window.app_saku3_report_siswa_flSisSaldoSiswa = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flSisSaldoSiswa.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flSisSaldoSiswa";
		this.maximize();
...
window.app_saku3_report_siswa_flSisSaldoSiswa.extend(window.childForm);
window.app_saku3_report_siswa_flSisSaldoSiswa.implement({
		...
		try{			
			...
			if (row == 3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Siswa",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.nis, a.nama from sis_siswa a   "+
														   this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(a.nis) from sis_siswa a "+
														  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  ["a.nis","a.nama"],"and",["Kode","Nama"]);
			}	
		}catch(e){
			systemAPI.alert(e);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				{
					this.nama_report = "server_report_saku3_siswa_rptSisSaldoSiswa";
				}
				if (this.sg1.getCell(2,2)=="SMP")
				{
					this.nama_report = "server_report_saku3_siswa_rptSisSaldoSiswaSmp";
				}
	...
	doOpenKartu: function(nis,kode_lokasi){
		...
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.kode_jur+"/"+nis;;
