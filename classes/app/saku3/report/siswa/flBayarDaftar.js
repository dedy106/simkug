window.app_saku3_report_siswa_flBayarDaftar = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flBayarDaftar.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flBayarDaftar";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Data Pendaftaran", 2);
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"' and modul='KBBILLREG'");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("No Registrasi","All",""));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("No Bukti","All",""));
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_siswa_flBayarDaftar.extend(window.childForm);
window.app_saku3_report_siswa_flBayarDaftar.implement({
		...
		try{			
			...
			if (row ==1)
			{
				...
				if (this.app._userStatus=="A")
				{
					this.filterRep.ListDataSGFilter(this, "Data PP",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_pp,nama from pp "+filter,
														  "select count(kode_pp) from pp "+filter,
			...
			if (row ==3)
			{
				var filter=this.filterRep.filterStr("b.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
					this.filterRep.filterStr("b.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
					" and b.modul='KBBILLREG' ";	
				var sql="select a.no_reg,a.nama "+
						"from sis_siswareg a "+
						"inner join kas_m b on a.no_reg=b.ref1 and a.kode_lokasi=b.kode_lokasi "+filter;
				var sql2="select count(a.no_reg) "+
						"from sis_siswareg a "+
						"inner join kas_m b on a.no_reg=b.ref1 and a.kode_lokasi=b.kode_lokasi "+filter;
				this.filterRep.ListDataSGFilter(this, "Data Pendaftaran",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,
														  ["a.no_reg","a.nama"],"and",["No Registrasi","nama"]);
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  ...
														  ["a.no_kas","a.keterangan"],"and",["Kode","Keterangan"]);
	...
	{		
		...
		if (row == 2)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct a.periode from kas_m a  "+filter+" order by a.periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}		
	},
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("b.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("b.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("b.periode",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+							
							this.filterRep.filterStr("b.no_ref1",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("b.no_kas",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							" and b.modul='KBBILLREG' ";
				this.filter2 =this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_siswa_rptBayarDaftar";
