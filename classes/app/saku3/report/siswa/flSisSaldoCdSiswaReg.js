window.app_saku3_report_siswa_flSisSaldoCdSiswaReg = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flSisSaldoCdSiswaReg.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flSisSaldoCdSiswaReg";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sis_cd_d where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"'");
	...
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Tahun Akademik","All",""));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_siswa_flSisSaldoCdSiswaReg.extend(window.childForm);
window.app_saku3_report_siswa_flSisSaldoCdSiswaReg.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 3)
			{
				...
				var sql1="select a.no_reg,a.nama from sis_siswareg a  "+filter;
				var sql2="select count(a.no_reg) from sis_siswareg a  "+filter;
				this.filterRep.ListDataSGFilter(this, "Data Siswa",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
												new Array("a.no_reg","a.nama"),"and",new Array("No Registrasi","Nama"),"and");
			}			
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["3","23","23","123","3","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,2,0,2,0,0]);	
		if (row == 2)
		{
			var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and"); 
			var sql="select distinct a.periode from sis_cd_d a "+filter+" order by a.periode desc"
		...
		if (row == 5)
		{
			var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and"); 
			var sql="select distinct a.kode_ta from sis_siswareg a "+filter+" order by a.kode_ta desc"
			this.sg1.columns.get(2).pickList.clear();
			this.sg1.columns.get(3).pickList.clear();
			this.dbLib.setItemsFromSQL(sql,[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);		
		}
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_reg",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.kode_ta",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");

				if(this.sg1.getCell(1,2) == "Range"){
					this.filter3 = " and a.periode between "+this.sg1.getCell(2,2)+" and "+this.sg1.getCell(3,2); 
				}else{
					this.filter3 = " and a.periode = "+this.sg1.getCell(2,2);
				}
				this.filter2 =this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,4)+"/"+this.filter3;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				
				
				this.nama_report = "server_report_saku3_siswa_rptSisSaldoCdSiswaReg";
	...
	doOpenKartu: function(nis,kode_lokasi,kode_pp){
		...
		this.link="1";
		filter = "where a.no_reg='"+nis+"' and a.kode_lokasi='"+kode_lokasi+"' and a.kode_pp='"+kode_pp+"'";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2);
		nama_report="server_report_saku3_siswa_rptSisKartuCdReg";
