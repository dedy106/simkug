window.app_saku3_report_siswa_flJadwalUjian = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flJadwalUjian.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flJadwalUjian";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Tingkat","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Jenis Ujian","All",""));
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_siswa_flJadwalUjian.extend(window.childForm);
window.app_saku3_report_siswa_flJadwalUjian.implement({
		...
		try{			
			...
			
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Tingkat",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.kode_tingkat, a.nama from sis_tingkat a   "+filter,
														  "select count(a.kode_tingkat) from sis_tingkat a "+filter,
														  ["a.kode_tingkat","a.nama"],"and",["Kode","Nama"]);
			}	
			if (row ==3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Jenis Ujian",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_jenis, nama from sis_jenisnilai",
												"select count(*) from sis_jenisnilai",
												new Array("kode_jenis","nama"),"where",new Array("Kode","Nama"));
			}					
		}catch(e){
			systemAPI.alert(e);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_siswa_rptJadwalUjian";
