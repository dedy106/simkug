window.app_saku3_report_siswa_flSisSaldoSiswaAgingPP = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flSisSaldoSiswaAgingPP.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flSisSaldoSiswaAgingPP";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Status Siswa","All",""));
...
window.app_saku3_report_siswa_flSisSaldoSiswaAgingPP.extend(window.childForm);
window.app_saku3_report_siswa_flSisSaldoSiswaAgingPP.implement({
		...
		try{			
			...
			if (row == 4)
			{
				...
				this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
					var sql="select distinct a.kode_akt from sis_angkat a "+filter;
					var sql2="select count(distinct a.kode_kt) "+
							"from sis_angkat a "+filter;
					this.filterRep.ListDataSGFilter(this, "Data Angkatan",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,
													new Array("a.kode_akt"),"and",new Array("Kode Angkatan"));
			}
	...
	{	
		if(this.app._userStatus == "A"){
			...
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["3","123","3","13","123","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,2,0,0,2,0]);	
		}else{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["3","3","3","13","123","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,2,0,0,2,0]);	
		}	
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("b.kode_akt",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
							
				this.filtersis = this.filterRep.filterStr("y.flag_aktif",this.sg1.getCell(1,3),this.sg1.getCell(2,3).substr(0,1),this.sg1.getCell(3,3).substr(0,1),"and");

				this.filter2 =this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,5)+"/"+this.filtersis;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				
				this.nama_report = "server_report_saku3_siswa_rptSisSaldoSiswaAgingPP";
	...
	doOpenDetail: function(kode_pp,kode_lokasi,kode_akt){
		...
		filter2 =this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/Layar/"+filterpp;

		filter = "where a.kode_lokasi='"+kode_lokasi+"' and a.kode_pp='"+kode_pp+"' and a.kode_akt='"+kode_akt+"' "+this.filterRep.filterStr("a.flag_aktif",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");

		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2);
		nama_report="server_report_saku3_siswa_rptSisSaldoSiswaAging";
