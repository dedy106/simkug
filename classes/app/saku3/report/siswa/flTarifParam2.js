window.app_saku3_report_siswa_flTarifParam2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flTarifParam2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flTarifParam2";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Tarif", 2);
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(kode_ta) as periode from sis_ta where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"'");
	this.periode2=this.dbLib.getPeriodeFromSQL("select max(kode_akt) as periode from sis_param_tarif where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"'");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Tahun Akademik","=",this.periode));	
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Angkatan","=",this.periode2));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Output Laporan","=","Layar"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_siswa_flTarifParam2.extend(window.childForm);
window.app_saku3_report_siswa_flTarifParam2.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["3","3","3","123","3"]);
		...
		if (row == 2)
		{
			var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and"); 
			var sql="select distinct kode_ta from sis_ta a "+filter+" order by a.kode_ta desc"
	...
	mainButtonClick: function(sender){
		try
		{
			...
			{
				this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
				this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Kode Agen","All",""));
			}else{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
								...
								// this.filterRep.filterStr("b.kode_ta",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 =this.sg1.getCell(2,0)+"|"+this.sg1.getCell(2,1)+"|"+this.sg1.getCell(2,2)+"|"+this.sg1.getCell(2,3);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_siswa_rptTarifParam2";
	...
	doOpenDetTarif: function(kode_pp,kode_lokasi,kode_akt,kode_tingkat,tahunaka,kode_jur){
		this.link="2";
		filter = "where a.kode_lokasi='"+kode_lokasi+"' and a.kode_pp='"+kode_pp+"' and a.kode_akt='"+kode_akt+"' and b.kode_tingkat='"+kode_tingkat+"' and b.kode_jur='"+kode_jur+"' ";
		filter2 = kode_lokasi+"|"+kode_pp+"|"+tahunaka;
		nama_report="server_report_saku3_siswa_rptTarifParamDet";
	...
	doOpenTarifSiswa: function(kode_pp,kode_lokasi,kode_akt,kode_kelas,kode_jur){
		this.link="2";
		filter = "where a.kode_lokasi='"+kode_lokasi+"' and a.kode_pp='"+kode_pp+"' and a.kode_akt='"+kode_akt+"' and c.kode_kelas='"+kode_kelas+"' and c.kode_jur='"+kode_jur+"' ";
		filter2 = kode_lokasi+"/"+kode_pp+"/"+this.app._periode;
		nama_report="server_report_saku3_tarbak_akademik_rptSisTarif2";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
