window.app_saku3_report_siswa_flSisRekonYptTkPdd = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flSisRekonYptTkPdd.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flSisRekonYptTkPdd";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sis_rekon_m where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"'");
...
window.app_saku3_report_siswa_flSisRekonYptTkPdd.extend(window.childForm);
window.app_saku3_report_siswa_flSisRekonYptTkPdd.implement({
		...
		try{			
			...
			if (row ==6)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pembayaran",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_rekon,a.keterangan from sis_rekon_m a "+this.filter,
												"select count(a.no_rekon) from sis_rekon_m a "+this.filter,
												new Array("a.no_rekon","a.keterangan"),"and",new Array("No Tagihan","Keterangan"));
			}		
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 2)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from sis_rekon_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 7)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  ...
							  this.filterRep.filterStr("a.nis",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+
							  this.filterRep.filterStr("a.no_rekon",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and");
				this.filter2 =this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  ...
							  this.filterRep.filterStr("a.no_rekon",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and")+"/"+this.sg1.getCell(2,7)+"/"+this.sg1.getCell(2,2);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				
				
				this.nama_report = "server_report_saku3_siswa_rptSisRekonYptTkPdd";
