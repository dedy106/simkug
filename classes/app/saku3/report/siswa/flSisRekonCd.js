window.app_saku3_report_siswa_flSisRekonCd = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flSisRekonCd.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flSisRekonCd";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sis_rekon_m where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"' and modul in ('CDMUT','CDIN') ");
...
window.app_saku3_report_siswa_flSisRekonCd.extend(window.childForm);
window.app_saku3_report_siswa_flSisRekonCd.implement({
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter2 =this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  ...
							  this.filterRep.filterStr("a.periode",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							  this.filterRep.filterStr("a.no_rekon",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and")+
							  " and a.modul in ('CDMUT','CDIN')"+"/"+this.sg1.getCell(2,7)+"/"+this.sg1.getCell(2,2);
