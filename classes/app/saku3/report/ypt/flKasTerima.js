window.app_saku3_report_ypt_flKasTerima = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ypt_flKasTerima.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ypt_flKasTerima";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('KBTT','KBTAKDIR') ");
...
window.app_saku3_report_ypt_flKasTerima.extend(window.childForm);
window.app_saku3_report_ypt_flKasTerima.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data TAK Terima",this.sg1, this.sg1.row, this.sg1.col,
												...
												new Array("a.no_kas","a.keterangan"),"and",new Array("No Terima","Keterangan"));
			}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.lokasi=this.dbLib.getPeriodeFromSQL("select nama as periode from lokasi where kode_lokasi='"+this.sg1.getCell(2,0)+"'");
					...
					this.nama_report = "server_report_saku3_ypt_rptKasTerima";
			...
			systemAPI.alert("[app_saku3_report_ypt_flKasTerima]::mainButtonClick:"+e);
		}
	},
