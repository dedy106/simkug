window.app_saku3_report_ypt_flKasSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ypt_flKasSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ypt_flKasSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('KBTAK','KBTAKDIR') ");
...
window.app_saku3_report_ypt_flKasSaldo.extend(window.childForm);
window.app_saku3_report_ypt_flKasSaldo.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data TAK Kirim",this.sg1, this.sg1.row, this.sg1.col,
												...
												new Array("a.no_kas","a.keterangan"),"and",new Array("No Kirim","Keterangan"));
			}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_ypt_rptKasSaldo";
			...
			systemAPI.alert("[app_saku3_report_ypt_flKasSaldo]::mainButtonClick:"+e);
		}
	},
	...
	doOpenKirim: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_ypt_rptKasKirim";
	...
	doOpenTerima: function(no_bukti,kode_lokasi){
		...
		filter2 = "";
		filter=" where a.no_kas='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"'";
		nama_report="server_report_saku3_ypt_rptKasTerima";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
