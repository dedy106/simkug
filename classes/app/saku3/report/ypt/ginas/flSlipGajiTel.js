window.app_saku3_report_ypt_ginas_flSlipGajiTel = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ypt_ginas_flSlipGajiTel.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ypt_ginas_flSlipGajiTel";
		this.maximize();
...
window.app_saku3_report_ypt_ginas_flSlipGajiTel.extend(window.childForm);
window.app_saku3_report_ypt_ginas_flSlipGajiTel.implement({
		...
		try{			
			...
			if (row ==2)
			{
				this.filterRep.ListDataSGFilter(this, "KOde PP",this.sg1, this.sg1.row, this.sg1.col,
												"select distinct a.kode_pp , a.nama from pp a ",
												"select  distinct a.kode_pp , a.nama  from pp a ",
			...
			if (row ==3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Karyawan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.nik , a.nama from hr_karyawan a  inner join hr_gaji_loadtelu b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi where a.bm='-'  ",
												"select a.nik , a.nama  from hr_karyawan a  inner join hr_gaji_loadtelu b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi where a.bm='-' ",
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Gaji",this.sg1, this.sg1.row, this.sg1.col,
									"select distinct a.no_gaji, a.nik from hr_gaji_d a "+this.filter,
									"select distinct a.no_gaji, a.nik from hr_gaji_d a  "+this.filter,
									new Array("a.no_gaji","a.nik"),"and",new Array("No Gaji","NIK"));
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from hr_gaji_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filterRep.filterStr("c.no_gaji",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_ypt_ginas_rptSlipGajiT";
			...
			systemAPI.alert("[app_saku3_report_ypt_ginas_flSlipGajiTel]::mainButtonClick:"+e);
		}
	},
