window.app_saku3_report_ypt_flHonorTransfer2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ypt_flHonorTransfer2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ypt_flHonorTransfer2";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from it_spb_m where kode_lokasi='"+this.app._lokasi+"'  ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("No SPB","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Output Laporan","=","Layar"));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Jenis Laporan","=","SPB"));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("No Bilyet","All",""));
...
window.app_saku3_report_ypt_flHonorTransfer2.extend(window.childForm);
window.app_saku3_report_ypt_flHonorTransfer2.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_spb, a.keterangan from it_spb_m a "+this.filter,
												"select count(*) from it_spb_m a "+this.filter,
												new Array("a.no_spb","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
			if (row ==5)
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				this.filterRep.ListDataSGFilter(this, "Data Nomor Bilyet",this.sg1, this.sg1.row, this.sg1.col,
												"select distinct a.no_cek, '-' as keterangan from it_spb_m a "+this.filter,
												"select count(*) from it_spb_m a "+this.filter,
												new Array("a.no_cek","a.keterangan"),"and",new Array("No blyet","Keterangan"));
			}	
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from it_spb_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row === 3)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("SPB","CEK"));
		}
	},
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("x.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("x.no_spb",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("x.no_cek",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,3);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_ypt_rptHonorTransfer2Detail";
				// if (this.sg1.getCell(2,4)=="CEK")
				// {
				// 	this.nama_report = "server_report_saku3_ypt_rptHonorTransfer2DetailCek";
				// }
			...
			systemAPI.alert("[app_saku3_report_ypt_flHonorTransfer2]::mainButtonClick:"+e);
		}
	},
