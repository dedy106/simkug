window.app_saku3_report_ypt_logistik_flMonitoringPengadaan = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ypt_logistik_flMonitoringPengadaan.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ypt_logistik_flMonitoringPengadaan";
		this.maximize();
	...
	var strSQL = "select convert(varchar,getdate(),103) as tglakhir" ;
	var data = this.dbLib.getDataProvider(strSQL,true);
	if (typeof data == "object"){
		var line = data.rs.rows[0];							
		if (line != undefined){						
			var tgl = line.tglakhir;
		}
	}
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Tanggal","=",tgl));
...
window.app_saku3_report_ypt_logistik_flMonitoringPengadaan.extend(window.childForm);
window.app_saku3_report_ypt_logistik_flMonitoringPengadaan.implement({
	...
	{		
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1],["123","13"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1],[2,1]);	
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1],["123","13"]);
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
                ...
                if (typeof data == "object"){
                    var line = data.rs.rows[0];							
                    if (line != undefined){						
                        var tgl = parseInt(line.tglakhir);
                    }
                }
                this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
                this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Tanggal","=",tgl));
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.tanggal",this.sg1.getCell(1,1),this.sg1.getCellDateValue(2,1),this.sg1.getCellDateValue(3,1),"and");
				...
				this.nama_report = "server_report_saku3_ypt_logistik_rptMonitoringPengadaan";
			...
			systemAPI.alert("[app_saku3_report_ypt_logistik_flMonitoringPengadaan]::mainButtonClick:"+e);
		}
	},
	...
	doOpenDetail: function(no_bukti,kode_lokasi){
		...
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = "where c.no_spk='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
		
		nama_report="server_report_saku3_ypt_logistik_rptPengadaanBayarDet";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
