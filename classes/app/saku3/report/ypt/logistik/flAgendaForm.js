window.app_saku3_report_ypt_logistik_flAgendaForm = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ypt_logistik_flAgendaForm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ypt_logistik_flAgendaForm";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from it_aju_m where modul='LOG' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_ypt_logistik_flAgendaForm.extend(window.childForm);
window.app_saku3_report_ypt_logistik_flAgendaForm.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==4)
			{
				...
				// alert("select a.no_aju, a.keterangan from it_aju_m a "+this.filter);
				// console.log("select a.no_aju, a.keterangan from it_aju_m a "+this.filter);
			}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_ypt_logistik_flAgendaForm]::mainButtonClick:"+e);
		}
	},
