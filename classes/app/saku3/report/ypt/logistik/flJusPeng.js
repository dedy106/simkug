window.app_saku3_report_ypt_logistik_flJusPeng = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ypt_logistik_flJusPeng.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ypt_logistik_flJusPeng";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from log_justerima_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_ypt_logistik_flJusPeng.extend(window.childForm);
window.app_saku3_report_ypt_logistik_flJusPeng.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_terima, a.keterangan from log_justerima_m a "+this.filter,
												"select count(*) from log_justerima_m a "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from log_justerima_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_ypt_logistik_flJusPeng]::mainButtonClick:"+e);
		}
	},
