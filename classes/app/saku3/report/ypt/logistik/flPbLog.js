window.app_saku3_report_ypt_logistik_flPbLog = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ypt_logistik_flPbLog.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ypt_logistik_flPbLog";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from pbh_pb_m where modul in ('PBBA','LOGADK','PBADK') and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_ypt_logistik_flPbLog.extend(window.childForm);
window.app_saku3_report_ypt_logistik_flPbLog.implement({
		...
		try{			
			...
			if (row ==3)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
						...
						" and a.modul in ('PBBA','LOGADK','PBADK') ";
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from pbh_pb_m where modul in ('PBBA','LOGADK','PBADK') and kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_ypt_logistik_flPbLog]::mainButtonClick:"+e);
		}
	},
