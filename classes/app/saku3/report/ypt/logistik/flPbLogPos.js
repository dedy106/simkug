window.app_saku3_report_ypt_logistik_flPbLogPos = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ypt_logistik_flPbLogPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ypt_logistik_flPbLogPos";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from pbh_pb_m where modul in ('PBADK') and kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_ypt_logistik_flPbLogPos.extend(window.childForm);
window.app_saku3_report_ypt_logistik_flPbLogPos.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from pbh_pb_m where modul in ('PBADK') and kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_ypt_logistik_rptPbPosYpt";
			...
			systemAPI.alert("[app_saku3_report_ypt_logistik_flPbLogPos]::mainButtonClick:"+e);
		}
	},
	...
	doOpenPb: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_pbh_ypt_rptPengajuanPB";
