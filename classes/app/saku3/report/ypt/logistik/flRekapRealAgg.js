window.app_saku3_report_ypt_logistik_flRekapRealAgg = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ypt_logistik_flRekapRealAgg.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ypt_logistik_flRekapRealAgg";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(substring(periode1,1,4)) as periode from angg_r where kode_lokasi='"+this.app._lokasi+"' ");
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lembaga","=",this.app._lokasi));
...
window.app_saku3_report_ypt_logistik_flRekapRealAgg.extend(window.childForm);
window.app_saku3_report_ypt_logistik_flRekapRealAgg.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode1,1,4) from angg_r where kode_lokasi='"+this.app._lokasi+"' order by substring(periode1,1,4)",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_ypt_logistik_rptRekapRealAgg";
			...
			systemAPI.alert("[app_saku3_report_ypt_logistik_flRekapRealAgg]::mainButtonClick:"+e);
		}
	},
	...
	doOpenDetailSPK: function(kode_lokasi,kode_akun,kode_pp,tahun){
		
		this.link="1";
		filter = "and a.kode_lokasi='"+kode_lokasi+"' and a.periode like '"+tahun+"%' and a.kode_akun='"+kode_akun+"' and b.kode_pp='"+kode_pp+"' ";
		filter2=kode_lokasi+"/"+tahun;
		
		nama_report="server_report_saku3_ypt_logistik_rptRekapRealAggDet";
