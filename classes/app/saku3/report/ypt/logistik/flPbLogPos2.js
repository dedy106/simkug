window.app_saku3_report_ypt_logistik_flPbLogPos2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ypt_logistik_flPbLogPos2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ypt_logistik_flPbLogPos2";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from pbh_pb_m where modul in ('PBADK','PBBA') and kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_ypt_logistik_flPbLogPos2.extend(window.childForm);
window.app_saku3_report_ypt_logistik_flPbLogPos2.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==3)
			{
                ...
                var sql = "select a.no_pb, a.keterangan from pbh_pb_m a "+this.filter+" "+
                          " and a.modul in ('PBADK','PBBA') ";
                          "union all "+
                          "select a.no_aju, a.keterangan from it_aju_m a "+this.filter+" "+
                          " and a.modul in ('LOG') ";

                var sql2 =" select count(*) as jum ( select a.no_pb, a.keterangan from pbh_pb_m a "+this.filter+" "+
                          " and a.modul in ('PBADK','PBBA') ";
                          "union all "+
                          "select a.no_aju, a.keterangan from it_aju_m a "+this.filter+" "+
                          " and a.modul in ('LOG') ) a ";
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from ( select periode from pbh_pb_m where modul in ('PBADK','PBBA') and kode_lokasi='"+this.app._lokasi+"' union all select periode from it_aju_m where modul in ('LOG') and kode_lokasi='"+this.app._lokasi+"' ) a order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.modul in ('PBADK','PBBA','LOG') ";
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_ypt_logistik_rptPbPosYpt2";
			...
			systemAPI.alert("[app_saku3_report_ypt_logistik_flPbLogPos2]::mainButtonClick:"+e);
		}
	},
	...
	doOpenPb: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_ypt_logistik_rptPbLog";
	...
	doOpenVer: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_pbh_ypt_rptVer";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenSpb: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_pbh_ypt_rptSpb";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenDok: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_pbh_ypt_rptDetailDok";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenKas: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_pbh_ypt_rptKbPb";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
