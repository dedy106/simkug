window.app_saku3_report_ypt_logistik_flRealPO = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ypt_logistik_flRealPO.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ypt_logistik_flRealPO";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from log_SPK_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_ypt_logistik_flRealPO.extend(window.childForm);
window.app_saku3_report_ypt_logistik_flRealPO.implement({
		...
		try{			
			...
			if (row ==3)
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.modul in ('PBADK') ";
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_pb, a.keterangan from pbh_pb_m a "+this.filter,
												"select count(*) from pbh_pb_m a "+this.filter,
	...
	{		
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1],["123","2"]);
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from log_SPK_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							// " and a.modul in ('PBADK') ";
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(3,1)+"/"+this.app._userLog;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_ypt_logistik_rptRealPO";
			...
			systemAPI.alert("[app_saku3_report_ypt_logistik_flRealPO]::mainButtonClick:"+e);
		}
	},
