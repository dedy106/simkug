window.app_saku3_report_ypt_logistik_flAgendaLogPos = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ypt_logistik_flAgendaLogPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ypt_logistik_flAgendaLogPos";
		this.maximize();
...
window.app_saku3_report_ypt_logistik_flAgendaLogPos.extend(window.childForm);
window.app_saku3_report_ypt_logistik_flAgendaLogPos.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from it_aju_m where modul='LOG' and kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row === 5)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_ypt_logistik_rptAgendaLogPos";
			...
			systemAPI.alert("[app_saku3_report_ypt_logistik_flAgendaLogPos]::mainButtonClick:"+e);
		}
	},
	...
	doOpenVer: function(no_bukti,kode_pp,periode,kode_lokasi){
		this.link="1";
		filter = "where a.no_aju='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' and a.kode_pp='"+kode_pp+"' and a.modul in ('LOG') ";
