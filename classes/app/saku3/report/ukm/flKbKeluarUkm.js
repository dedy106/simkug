// JavaScript Document
window.app_saku3_report_ukm_flKbKeluarUkm = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ukm_flKbKeluarUkm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ukm_flKbKeluarUkm";
		this.maximize();
	...
	this.akun=this.dbLib.getPeriodeFromSQL("select kode_akun as periode from flag_relasi where kode_lokasi='"+this.app._lokasi+"' and kode_flag='049' ");
...
window.app_saku3_report_ukm_flKbKeluarUkm.extend(window.portalui_childForm);
window.app_saku3_report_ukm_flKbKeluarUkm.implement({
		...
		if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,
													...
													"where a.block= '0' and b.kode_flag in ('049') and a.kode_curr='IDR' "+
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report="server_report_saku3_ukm_rptKbKeluarUkm";
				this.dbLib.execQuerySync(this.sql);	
