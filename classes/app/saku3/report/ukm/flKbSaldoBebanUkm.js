// JavaScript Document
window.app_saku3_report_ukm_flKbSaldoBebanUkm = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ukm_flKbSaldoBebanUkm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ukm_flKbSaldoBebanUkm";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Kode Template","All",""));
...
window.app_saku3_report_ukm_flKbSaldoBebanUkm.extend(window.portalui_childForm);
window.app_saku3_report_ukm_flKbSaldoBebanUkm.implement({
		...
		if (row == 3)
			{
				var filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.jenis='BEBAN' ";
				var sql="select a.kode_jurnal, a.nama from refju_dual a "+filter;
				var sql2="select count(a.kode_jurnal) from refju_dual a "+filter;
				this.filterRep.ListDataSGFilter(this, "Data Template",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,
													["a.kode_jurnal","a.nama"],"and",["Kode","Nama"]);
			}
	},
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenBb(this.kode_jurnal,this.kode_lokasi);
				}
			}
			else
			{
				...
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
					  this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
					  this.filterRep.filterStr("a.kode_jurnal",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
				this.nama_report="server_report_saku3_ukm_rptKbSaldoBebanUkm";
				this.dbLib.execQuerySync(this.sql);	
	...
	doOpenBb: function(kode_jurnal,kode_lokasi,periode,kode_pp){
		this.kode_jurnal=kode_jurnal;
		this.kode_lokasi=kode_lokasi;
		this.periode=periode;
		this.kode_pp=kode_pp;
		this.link="1";
		filter2 = kode_lokasi+"/"+this.sg1.getCell(2,1);
		filter = "where a.kode_jurnal='"+kode_jurnal+"' and a.kode_lokasi='"+kode_lokasi+"' and a.kode_pp='"+kode_pp+"' ";
		nama_report="server_report_saku3_ukm_rptKbKartuKeluarUkm";
