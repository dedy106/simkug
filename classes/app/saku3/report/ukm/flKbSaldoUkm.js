// JavaScript Document
window.app_saku3_report_ukm_flKbSaldoUkm = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ukm_flKbSaldoUkm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ukm_flKbSaldoUkm";
		this.maximize();
...
window.app_saku3_report_ukm_flKbSaldoUkm.extend(window.portalui_childForm);
window.app_saku3_report_ukm_flKbSaldoUkm.implement({
	...
	doSelectCell : function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 3)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report="server_report_saku3_ukm_rptKbSaldoUkm";
				this.dbLib.execQuerySync(this.sql);	
