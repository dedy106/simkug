window.app_saku3_report_sju16_flPiutangSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sju16_flPiutangSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sju16_flPiutangSaldo";
		this.maximize();
	...
	var sql="select max(periode) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"'";
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Jenis Laporan","=","Outstanding Premi"));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Urut Berdasarkan","=","Nilai"));
...
window.app_saku3_report_sju16_flPiutangSaldo.extend(window.childForm);
window.app_saku3_report_sju16_flPiutangSaldo.implement({
	...
	{		
		...
		if (row == 2)
		{
			var sql="select distinct periode from trans_m where kode_lokasi='"+this.app._lokasi+"' "+ 
					"union "+
					"select distinct periode from sju_polisbayar_d where kode_lokasi='"+this.app._lokasi+"' "+
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Outstanding Premi","Summary Mutasi","Detail Mutasi","Piutang","Piutang Summary"));
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Nilai","Tertanggung","No Register","Kode Tertanggung"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.urut="a.n4 desc";
				if (this.sg1.getCell(2,4)=="Tertanggung")
				{
					this.urut="b.nama asc";
				}
				if (this.sg1.getCell(2,4)=="Kode Tertanggung")
				{
					this.urut="a.kode_cust asc";
				}
				if (this.sg1.getCell(2,4)=="No Register")
				{
					this.urut="a.no_polis asc";
				}
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,2)+"/"+this.urut+"/"+this.app._nikUser;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();	
				this.nama_report = "server_report_saku3_sju16_rptPiutangSaldoOut";
				if (this.sg1.getCell(2,3)=="Summary Mutasi")
				{
					this.nama_report = "server_report_saku3_sju16_rptPiutangSaldo";
				}
				if (this.sg1.getCell(2,3)=="Detail Mutasi")
				{
					this.nama_report = "server_report_saku3_sju16_rptPiutangSaldoPolis";
				}
				if (this.sg1.getCell(2,3)=="Piutang")
				{
					this.filter = this.filter+ this.filterRep.filterStr("b.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
					this.nama_report = "server_report_saku3_sju16_rptPiutang";
				}
				if (this.sg1.getCell(2,3)=="Piutang Summary")
				{
					this.filter = this.filter+ this.filterRep.filterStr("b.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
					this.nama_report = "server_report_saku3_sju16_rptPiutangSum";
				}
			...
			systemAPI.alert("[app_saku3_report_sju16_flPiutangSaldo]::mainButtonClick:"+e);
		}
	},
	...
	doOpenSaldo: function(kode_cust,kode_lokasi){
		this.kode_cust=kode_cust;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = "where a.kode_cust='"+kode_cust+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.urut+"/"+this.app._nikUser;
		
		nama_report="server_report_saku2_kopeg_sju_rptPiutangSaldoPolis";
