window.app_saku3_report_sju16_flPrRenewal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sju16_flPrRenewal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sju16_flPrRenewal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sju_renew_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_sju16_flPrRenewal.extend(window.childForm);
window.app_saku3_report_sju16_flPrRenewal.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_renew, keterangan from sju_renew_m  "+filter ,
														  "select count(no_renew) from sju_renew_m "+filter,
														  new Array("no_renew","keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
			if (row == 3)
			{
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from sju_renew_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_renew",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				var acc=this.filterRep.filterStr("c.kode_pic",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+acc;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_sju16_rptPrRenewal";
			...
			systemAPI.alert("[app_saku3_report_sju16_flPrRenewal]::mainButtonClick:"+e);
		}
	},
