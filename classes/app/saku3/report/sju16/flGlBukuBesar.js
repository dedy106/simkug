window.app_saku3_report_sju16_flGlBukuBesar = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sju16_flGlBukuBesar.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sju16_flGlBukuBesar";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Tampil Mutasi Nol","=","Tidak"));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Tanggal","All",""));
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Sort By","=","Tanggal"));
	this.gridLib.SGEditData(this.sg1,7,new Array(0,1,2), new Array("Output Laporan","=","Layar"));
	this.gridLib.SGEditData(this.sg1,8,new Array(0,1,2), new Array("Format Laporan","=","Buku Besar"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_sju16_flGlBukuBesar.extend(window.childForm);
window.app_saku3_report_sju16_flGlBukuBesar.implement({
	...
	{		
		...
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[2,0,2,2,0,1,0,0,0,0]);	
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Tanggal","No Bukti"));
		}
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Buku Besar","Produksi","Kegiatan"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				if (this.sg1.getCell(1,2)!="All")
				{
					this.filter=this.filter+this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," and ");
				}
				var var_periode=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," and ")
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser+"/"+this.sg1.getCell(2,4)+"/"+var_periode+"/"+this.sg1.getCell(2,7)+"/"+this.sg1.getCell(1,1)+"/"+this.sg1.getCell(3,1)+"/"+this.sg1.getCell(1,2);
				
				this.nama_report="server_report_saku3_sju16_rptGlBukuBesar";
				if (this.sg1.getCell(2,8)=="Produksi")
				{
					this.nama_report="server_report_saku3_sju16_rptGlBukuBesarProduksi";
				}
				if (this.sg1.getCell(2,8)=="Kegiatan")
				{
					this.nama_report="server_report_saku3_sju16_rptGlBukuBesarKeg";
				}
