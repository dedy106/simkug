window.app_saku3_report_sju16_flPbPos = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sju16_flPbPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sju16_flPbPos";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sju_pb_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_sju16_flPbPos.extend(window.childForm);
window.app_saku3_report_sju16_flPbPos.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_pb, a.keterangan from sju_pb_m a "+this.filter,
												"select count(*) from sju_pb_m a "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from sju_pb_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_sju16_rptPbPos";
			...
			systemAPI.alert("[app_saku3_report_sju16_flPbPos]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_sju16_rptPbForm";
