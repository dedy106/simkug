window.app_saku3_report_sju16_flAggProd = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sju16_flAggProd.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sju16_flAggProd";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from sju_gar_d where kode_lokasi='"+this.app._lokasi+"' ");
	this.gridLib.SGEditData(this.sg1,0,[0,1,2,3],["Lokasi","=",this.app._lokasi,""]);
	this.gridLib.SGEditData(this.sg1,1,[0,1,2],["Kode PP","All",""]);
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Tahun","=",this.periode]);
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Group COB ","All",""]);
	this.gridLib.SGEditData(this.sg1,4,[0,1,2],["Tertanggung","All",""]);
	this.gridLib.SGEditData(this.sg1,5,[0,1,2],["PIC","All",""]);
	this.gridLib.SGEditData(this.sg1,6,[0,1,2],["Jenis Anggaran","=","Gross"]);
...
window.app_saku3_report_sju16_flAggProd.extend(window.portalui_childForm);
window.app_saku3_report_sju16_flAggProd.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			this.filterRep.ListDataSGFilter(this, "Data Group COB",this.sg1, this.sg1.row, this.sg1.col,
													  "select a.kode_klp, a.nama from sju_tipe_klp a "+
													   this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
													  "select count(a.kode_klp) from sju_tipe_klp a "+
													  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
													  ["kode_klp","nama"],"and",["Kode","Nama"]);
		}
		if (row == 4)
		{
			this.filterRep.ListDataSGFilter(this, "Data Customer",this.sg1, this.sg1.row, this.sg1.col,
													  "select a.kode_cust, a.nama from sju_cust a "+
													   this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
													  "select count(a.kode_cust) from sju_cust a "+
													  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
		...
		{
			this.filterRep.ListDataSGFilter(this, "Data Group COB",this.sg1, this.sg1.row, this.sg1.col,
													  "select a.kode_pic, a.nama from sju_pic a "+
													   this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
													  "select count(a.kode_pic) from sju_pic a "+
													  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
													  ["kode_pic","nama"],"and",["Kode","Nama"]);
		}
	...
	doSelectCell: function(sender, col, row){
		...
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[2,2,0,2,2,2,0]);
		
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct(substring(periode,1,4)) as periode from sju_gar_d "+
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["Gross"]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.filter=this.filterRep.filterStr("x.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("x.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						this.filterRep.filterStr("substring(x.periode,1,4)",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
						this.filterRep.filterStr("x.kode_tipe",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
						this.filterRep.filterStr("x.kode_cust",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
						this.filterRep.filterStr("x.kode_pic",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");
			this.filter2=this.sg1.getCell(2,2)+"/";	
			this.nama_report="server_report_saku3_sju16_rptAggBulanProd";
