window.app_saku3_report_sju16_flGlArusKas = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sju16_flGlArusKas.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sju16_flGlArusKas";
		this.maximize();
...
window.app_saku3_report_sju16_flGlArusKas.extend(window.childForm);
window.app_saku3_report_sju16_flGlArusKas.implement({
		...
		try{			
			...
			if (row == 3)
			{
				this.filterRep.ListDataSGFilter(this, "Data FS",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_fs,nama from fskas "+
														  ...
														  "select count(kode_fs) from fskas "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
	...
	{		
		...
		else
		{
			...
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3],[3,2,0,2]);
		}
		if (row == 2)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,2)+"/"+this.app._nikUser+"/"+this.sg1.getCell(3,0)+"/"+this.sg1.getCell(2,3);
				this.nama_report="server_report_saku3_sju16_rptGlArusKas";
	...
	doOpenTb: function(kode_neraca,kode_lokasi){
		...
		this.link="1";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,2)+"/"+this.app._nikUser+"/1/"+this.sg1.getCell(2,3);
		filter="where a.kode_lokasi='"+kode_lokasi+"' and b.kode_neraca='"+kode_neraca+"' ";
		nama_report="server_report_saku3_sju16_rptGlTbArusKas";
	...
	doOpenBb: function(kode_akun,kode_lokasi,periode){
		...
		filter="where a.kode_lokasi='"+kode_lokasi+"' and b.kode_akun='"+kode_akun+"' ";
		nama_report="server_report_saku3_sju16_rptGlBbArusKas";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
