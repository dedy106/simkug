window.app_saku3_report_sju16_flGlJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sju16_flGlJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sju16_flGlJurnal";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Format Laporan","=","Bukti"));
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Output Laporan","=","Layar"));
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_sju16_flGlJurnal.extend(window.childForm);
window.app_saku3_report_sju16_flGlJurnal.implement({
	...
	{		
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123","123","123","123","123","3","3"]);
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_j where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct modul from trans_j where kode_lokasi='"+this.app._lokasi+"' order by modul desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 5)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Bukti","Daftar","Produksi"));
		}	
		if (row === 6)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	var filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							...
							this.filterRep.filterStr("a.modul",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3)," and ")+
							this.filterRep.filterStr("a.no_bukti",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4)," and ");
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(1,1)+"/"+this.sg1.getCell(3,1)+"/"+this.sg1.getCell(2,0)+"/"+this.app._nikUser+"/"+this.sg1.getCell(2,6);
				
				if (this.sg1.getCell(2,5)=="Daftar")
				{
					this.filter=filter;
					this.nama_report="server_report_saku3_sju16_rptGlJurnal";
				}
