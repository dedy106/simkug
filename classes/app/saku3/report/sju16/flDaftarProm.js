window.app_saku3_report_sju16_flDaftarProm = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sju16_flDaftarProm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sju16_flDaftarProm";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sju_dnp_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_sju16_flDaftarProm.extend(window.childForm);
window.app_saku3_report_sju16_flDaftarProm.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from sju_dnp_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_sju16_rptDaftarProm";
			...
			systemAPI.alert("[app_saku3_report_sju16_flDaftarProm]::mainButtonClick:"+e);
		}
	},
