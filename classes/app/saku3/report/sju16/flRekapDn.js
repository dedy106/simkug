window.app_saku3_report_sju16_flRekapDn = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sju16_flRekapDn.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sju16_flRekapDn";
		this.maximize();
...
window.app_saku3_report_sju16_flRekapDn.extend(window.childForm);
window.app_saku3_report_sju16_flRekapDn.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_dn, a.nama_aju from sju_dne_m a "+this.filter,
												"select count(*) from sju_dne_m a "+this.filter,
												new Array("a.no_dn","a.nama_aju"),"and",new Array("No Bukti","Keterangan"));
			}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_dn",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_sju16_rptRekapDn";
			...
			systemAPI.alert("[app_saku3_report_sju16_flRekapDn]::mainButtonClick:"+e);
		}
	},
