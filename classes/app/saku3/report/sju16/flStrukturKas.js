// JavaScript Document
window.app_saku3_report_sju16_flStrukturKas = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sju16_flStrukturKas.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sju16_flStrukturKas";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Kode FS","=",this.app._kodeFs));
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode Neraca","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Jenis Laporan","=","Struktur"));
...
window.app_saku3_report_sju16_flStrukturKas.extend(window.portalui_childForm);
window.app_saku3_report_sju16_flStrukturKas.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data FS",this.sg1, this.sg1.row, this.sg1.col,
												  "select kode_fs,kode_lokasi,nama from fskas "+filter,
												  "select count(kode_fs) from fskas "+filter,
		...
		if (row == 2)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Neraca",this.sg1, this.sg1.row, this.sg1.col,
												  "select kode_neraca,nama from neracakas "+filter,
												  "select count(kode_neraca) from neracakas "+filter,
											new Array("kode_neraca","nama"),"and");
		}
	},
	doSelectCell: function(sender, col, row){
		
		this.filterRep.setSGFilterRowTipe(this.sg1,row,new Array(0,1,2,3),new  Array("13","123","123","3"));
		this.filterRep.setSGFilterRowButtonStyle(this.sg1,row,new Array(0,1,2,3),new  Array(2,2,2,0));
		
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Struktur","Relasi Akun"));
		}	
	},
	mainButtonClick: function(sender){
		try
		{
			...
			this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						 this.filterRep.filterStr("a.kode_fs",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						 this.filterRep.filterStr("a.kode_neraca",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
	    		
			this.nama_report="server_report_saku3_sju16_rptStrukturKas";
			if (this.sg1.getCell(2,3)=="Relasi Akun")
			{
				this.nama_report="server_report_saku3_sju16_rptStrukturKasRelasi";
			}
