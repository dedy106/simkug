window.app_saku3_report_sju16_flGlLabaRugi = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sju16_flGlLabaRugi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sju16_flGlLabaRugi";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gldt where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Kode Divisi","All",""));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_sju16_flGlLabaRugi.extend(window.childForm);
window.app_saku3_report_sju16_flGlLabaRugi.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 3)
			{
				var lokasi=this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ");
				this.filterRep.ListDataSGFilter(this, "Data FS",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_fs,nama from fs "+lokasi+" and kode_fs in ('FS1','FS4','FS5','FS6','FS7','FS8','FS9')",
														  "select count(kode_fs) from fs "+lokasi,
			...
			if (row == 4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Divisi",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_bidang,nama from bidang "+lokasi,
														  "select count(kode_bidang) from bidang "+lokasi,
	...
	{		
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["3","13","3","3","13"]);
		...
		if (row == 2)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gldt where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenTb(this.kode_neraca,this.sg1.getCell(2,0),this.periode);
				}
				if (this.link=="3")
				{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							...
							this.filterRep.filterStr("a.kode_fs",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3)," and ");
				
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,2)+"/"+this.app._nikUser+"/"+this.sg1.getCell(2,3)+"/"+this.sg1.getCell(1,1)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(1,4)+"/"+this.sg1.getCell(2,4);
				
				this.nama_report="server_report_saku3_sju16_rptGlLabaRugi";
	...
	doOpenGar: function(kode_neraca,kode_lokasi,periode){
		...
		{
			filter2=this.sg1.getCell(2,1).substr(0,4)+"/1";
			filter="where x.kode_lokasi='"+kode_lokasi+"' and z.kode_neraca='"+kode_neraca+"' and substring(x.periode,1,4)='"+periode.substr(0,4)+"' ";
		}
		else
		{
			filter2=this.sg1.getCell(2,1).substr(0,4)+"/1";
			filter="where x.kode_lokasi='"+kode_lokasi+"' and z.kode_neraca='"+kode_neraca+"' and substring(x.periode,1,4)='"+periode.substr(0,4)+"' and x.kode_pp='"+this.sg1.getCell(2,1)+"'";
		}
		
		nama_report="server_report_saku3_sju16_rptAggBulan";
	...
	doOpenTb: function(kode_neraca,kode_lokasi,periode){
		...
		if (this.sg1.getCell(1,1)=="All")
		{
			filter2=this.sg1.getCell(2,0)+"/"+periode+"/"+this.app._nikUser+"/"+this.sg1.getCell(1,1)+"/Tidak/1";
			filter="where a.kode_lokasi='"+this.sg1.getCell(2,0)+"' and b.kode_neraca='"+kode_neraca+"' and b.kode_fs='"+this.sg1.getCell(2,3)+"'";
		}
		else
		{
			filter2=this.sg1.getCell(2,0)+"/"+periode+"/"+this.app._nikUser+"/"+this.sg1.getCell(1,1)+"/Tidak/1";
			filter="where a.kode_lokasi='"+this.sg1.getCell(2,0)+"' and b.kode_neraca='"+kode_neraca+"' and b.kode_fs='"+this.sg1.getCell(2,3)+"' and a.kode_pp='"+this.sg1.getCell(2,1)+"'";
		}
		
		nama_report="server_report_saku3_sju16_rptGlTb";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenBb: function(kode_akun,kode_lokasi,periode){
		...
		if (this.sg1.getCell(1,1)=="All")
		{
			...
			filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,2)+"/"+this.app._nikUser+"/"+this.sg1.getCell(1,1)+"/"+this.sg1.getCell(2,4);
		}
		else
		{
			filter="where a.kode_lokasi='"+kode_lokasi+"' and a.kode_akun='"+kode_akun+"' and a.kode_pp='"+this.sg1.getCell(2,1)+"'";
			filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,2)+"/"+this.app._nikUser+"/"+this.sg1.getCell(1,1)+"/"+this.sg1.getCell(2,4);
		}
		nama_report="server_report_saku3_sju16_rptGlBukuBesar";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
