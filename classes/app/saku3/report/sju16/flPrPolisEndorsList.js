window.app_saku3_report_kopeg_sju_flPrPolisEndorsList = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_kopeg_sju_flPrPolisEndorsList.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_kopeg_sju_flPrPolisEndorsList";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sju_polis_m_h where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("COB","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Tertanggung","All",""));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Acc Exec","All",""));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("No Register","All",""));
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Digit Koma","=","2"));
	this.gridLib.SGEditData(this.sg1,7,new Array(0,1,2), new Array("Tanggal","All",""));
...
window.app_saku3_report_kopeg_sju_flPrPolisEndorsList.extend(window.childForm);
window.app_saku3_report_kopeg_sju_flPrPolisEndorsList.implement({
		...
		try{			
			...
			if (row ==5)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Polis",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_polis, no_dok from sju_polis_m_h a "+filter,
												"select count(a.no_polis) from sju_polis_m_h a "+filter,
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["123","123","123","123","123","123","3","123","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[2,0,2,2,2,2,0,1,2]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from sju_polis_m_h where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 6)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				{
					tanggal=" and d.tanggal='"+this.sg1.getCellDateValue(2,7)+"'";
				}
				if (this.sg1.getCell(1,7)=="Range")
				{
					tanggal=" and (d.tanggal between '"+this.sg1.getCellDateValue(2,7)+"' and '"+this.sg1.getCellDateValue(3,7)+"') ";
				}
		    	this.filter = this.filterRep.filterStr("d.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("d.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("d.kode_tipe",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("d.kode_cust",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("d.kode_pic",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							this.filterRep.filterStr("d.no_polis",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+
							this.filterRep.filterStr("a.kode_vendor",this.sg1.getCell(1,8),this.sg1.getCell(2,8),this.sg1.getCell(3,8),"and")+tanggal;
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,6);
			...
			systemAPI.alert("[app_saku3_report_kopeg_sju_flPrPolisEndorsList]::mainButtonClick:"+e);
		}
	},
	...
	doOpenTagihan: function(no_bukti,kode_lokasi){
		this.no_bukti=no_bukti;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = "where a.no_tagih='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
		nama_report="server_report_saku2_kopeg_sju_rptPrTagihan";
	...
	doOpenQuo: function(no_bukti,kode_lokasi){
		this.no_bukti=no_bukti;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = "where a.no_quo='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
		nama_report="server_report_saku2_kopeg_sju_rptPrQuo";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenPlacing: function(no_bukti,kode_lokasi){
		this.no_bukti=no_bukti;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = "where a.no_placing='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
		nama_report="server_report_saku2_kopeg_sju_rptPrPlacing";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
