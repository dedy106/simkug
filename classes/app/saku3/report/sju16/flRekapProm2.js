window.app_saku3_report_sju16_flRekapProm2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sju16_flRekapProm2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sju16_flRekapProm2";
		this.maximize();
	...
	this.periode = this.dbLib.getPeriodeFromSQL("select max(tahun_pajak) as periode from sju_dnp_m where kode_lokasi='"+this.app._lokasi+"'");
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Tahun Pajak","=",this.periode));
...
window.app_saku3_report_sju16_flRekapProm2.extend(window.childForm);
window.app_saku3_report_sju16_flRekapProm2.implement({
		...
		try{			
			...
			if (row ==2)
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.tahun_pajak",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				this.filterRep.ListDataSGFilter(this, "Data Promosi",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_dn, a.nama_aju from sju_dnp_m a "+this.filter,
												"select count(*) from sju_dnp_m a "+this.filter,
												new Array("a.no_dn","a.nama_ju"),"and",new Array("No Bukti","Nama"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct tahun_pajak from sju_dnp_m where kode_lokasi='"+this.app._lokasi+"' order by tahun_pajak",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.tahun_pajak",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_dn",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_sju16_rptRekapProm2";
			...
			systemAPI.alert("[app_saku3_report_sju16_flRekapProm2]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.sju_dnp_m='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
