window.app_saku3_report_pbh_ypt_flHonorTransfer = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_pbh_ypt_flHonorTransfer.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_pbh_ypt_flHonorTransfer";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from spb_m where kode_lokasi='"+this.app._lokasi+"'  ");
...
window.app_saku3_report_pbh_ypt_flHonorTransfer.extend(window.childForm);
window.app_saku3_report_pbh_ypt_flHonorTransfer.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												...
												"select count(*) from spb_m a "+this.filter,
			...
			if (row ==5)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Nomor Bilyet",this.sg1, this.sg1.row, this.sg1.col,
												"select distinct a.no_cek, '-' as keterangan from spb_m a "+this.filter,
												"select count(*) from spb_m a "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from spb_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row === 3)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_pbh_ypt_rptHonorTransferDetail";
				if (this.sg1.getCell(2,4)=="CEK")
				{
					this.nama_report = "server_report_saku3_pbh_ypt_rptHonorTransferDetailCek";
				}
			...
			systemAPI.alert("[app_saku3_report_pbh_ypt_flHonorTransfer]::mainButtonClick:"+e);
		}
	},
