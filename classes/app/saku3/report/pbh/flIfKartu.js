window.app_saku3_report_pbh_flIfKartu = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_pbh_flIfKartu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_pbh_flIfKartu";
		this.maximize();
...
window.app_saku3_report_pbh_flIfKartu.extend(window.childForm);
window.app_saku3_report_pbh_flIfKartu.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 1)
			{
				...
				this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from pbh_pb_m where kode_lokasi='"+this.app._lokasi+"' order by substring(periode,1,4)",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
			}
			if (row == 2)
			{
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_pbh_rptIfKartu";
			...
			systemAPI.alert("[app_saku3_report_pbh_flIfKartu]::mainButtonClick:"+e);
		}
	},
