// JavaScript Document
window.app_saku3_report_kb_flTitipPtg = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_kb_flTitipPtg.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_kb_flTitipPtg";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_titip_d where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_kb_flTitipPtg.extend(window.portalui_childForm);
window.app_saku3_report_kb_flTitipPtg.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			...
			var sql1="select a.no_aju,a.keterangan from kas_titip_d a "+filter; 
			var sql2="select count(a.no_aju) from kas_titip_d a "+filter;
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_titip_d where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
