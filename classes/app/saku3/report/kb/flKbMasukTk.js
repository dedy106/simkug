// JavaScript Document
window.app_saku3_report_kb_flKbMasukTk = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_kb_flKbMasukTk.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_kb_flKbMasukTk";
		this.maximize();
	...
	this.akun=this.dbLib.getPeriodeFromSQL("select kode_akun as periode from flag_relasi where kode_lokasi='"+this.app._lokasi+"' and kode_flag='050' ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode Akun","=",this.akun));
...
window.app_saku3_report_kb_flKbMasukTk.extend(window.portalui_childForm);
window.app_saku3_report_kb_flKbMasukTk.implement({
		...
		if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,
													...
													"where a.block= '0' and b.kode_flag in ('050') and a.kode_curr='IDR' "+
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report="server_report_saku3_kb_rptKbMasukTk";
				this.dbLib.execQuerySync(this.sql);	
	...
	doOpenBb: function(kode_akun,kode_lokasi){
		...
		filter="";
		nama_report="server_report_saku2_kb_rptBukuKasTrail";
