window.app_saku3_report_kb_flKbBukuBesarYks = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_kb_flKbBukuBesarYks.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_kb_flKbBukuBesarYks";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Jenis Rekening","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Bank","All",""));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Kode Rekening","All",""));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Kode Akun","All",""));
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Tampil Mutasi Nol","=","Ya"));
	this.gridLib.SGEditData(this.sg1,7,new Array(0,1,2), new Array("Tanggal","All",""));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_kb_flKbBukuBesarYks.extend(window.childForm);
window.app_saku3_report_kb_flKbBukuBesarYks.implement({
		...
		try{			
			...
			if (row ==3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Bank",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_bank, nama from bank ",
												"select count(*) from bank ",
												new Array("kode_bank","nama"),"where",new Array("kode","nama"));
			}
			if (row == 4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Rekening",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_rek, nama from bank_rek  "+filter ,
														  "select count(kode_rek) from bank_rek "+filter,
														  new Array("kode_rek","nama"),"and");
			}
			
			if (row == 5)
			{
				var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							this.filterRep.filterStr("b.kode_rek",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3)," and ");
				this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.kode_akun,a.nama from masakun a inner join bank_rek b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi   "+filter ,
														  "select count(a.kode_akun) from masakun a inner join bank_rek b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+filter,
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["123","123","13","123","123","123","123","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[2,0,0,2,2,2,0,1]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("OPERASIONAL","DAKEM","RESTITUSI"));
		}
		if (row == 6)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							...
							this.filterRep.filterStr("b.kode_bank",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3)," and ")+
							this.filterRep.filterStr("a.kode_bank",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4)," and ")+
							this.filterRep.filterStr("a.kode_akun",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5)," and ");
				var var_periode=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," and ")
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser+"/"+this.sg1.getCell(2,3)+"/"+var_periode;
				this.nama_report="server_report_saku3_kb_rptKbBukuBesarYks";
	...
	doOpenJurnal: function(no_bukti,kode_lokasi){
		...
		filter2="";
		nama_report="server_report_saku3_kb_rptKbJurnalBuktiYks";
