// JavaScript Document
window.app_saku3_report_kb_flTitipSaldoLama = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_kb_flTitipSaldoLama.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_kb_flTitipSaldoLama";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_titip where modul='KBTITIP' and kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_kb_flTitipSaldoLama.extend(window.portalui_childForm);
window.app_saku3_report_kb_flTitipSaldoLama.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 2)
		{
			...
			var sql1="select a.no_kas,a.keterangan from kas_titip a "+filter; 
			var sql2="select count(a.no_kas) from kas_titip a "+filter;
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_titip where modul='KBTITIP' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row === 3)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.nama_report="server_report_saku3_kb_rptTitipSaldoLama";
