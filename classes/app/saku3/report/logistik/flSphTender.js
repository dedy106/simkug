window.app_saku3_report_logistik_flSphTender = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_logistik_flSphTender.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_logistik_flSphTender";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from log_sph_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_logistik_flSphTender.extend(window.childForm);
window.app_saku3_report_logistik_flSphTender.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_sph, a.keterangan from log_sph_m a "+this.filter,
												"select count(*) from log_sph_m a "+this.filter,
												new Array("a.no_sph","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from log_sph_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_sph",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_logistik_rptSphTender";
			...
			systemAPI.alert("[app_saku3_report_logistik_flSphTender]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.log_sph_m='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
