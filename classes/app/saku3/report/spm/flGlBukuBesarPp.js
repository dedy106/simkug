window.app_saku3_report_spm_flGlBukuBesarPp = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_spm_flGlBukuBesarPp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_spm_flGlBukuBesarPp";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,7,new Array(0,1,2), new Array("Jenis Laporan","=","Buku Besar"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_spm_flGlBukuBesarPp.extend(window.childForm);
window.app_saku3_report_spm_flGlBukuBesarPp.implement({
		...
		try{			
			...
			if (row == 4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_ju, keterangan from ju_m  "+filter ,
														  "select count(no_ju) from ju_m "+filter,
														  new Array("no_ju","keterangan"),"and");
			}
			if (row == 5)
			{
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["123","123","123","123","123","123","3","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[2,0,2,0,1,2,0,0]);	
		if (row == 1)
		{
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Buku Besar","Buku Besar Proyek"));
		}
	},
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(2,1)," and ")+
				...
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser+"/"+this.sg1.getCell(2,3)+"/"+var_periode+"/"+this.sg1.getCell(2,6)+"/"+this.sg1.getCell(1,1)+"/"+this.sg1.getCell(3,1);
				this.nama_report="server_report_saku3_spm_rptGlBukuBesarPp";
				if (this.sg1.getCell(2,7)=="Buku Besar Proyek")
				{
					this.nama_report="server_report_saku3_spm_rptGlBukuBesarPpProyek";
				}
	...
	doOpenJurnal: function(no_bukti,kode_lokasi){
		...
		filter2="";
		nama_report="server_report_saku3_gl_rptGlJurnalBuktiSpm";
