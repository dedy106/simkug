window.app_saku3_report_spm_flFaWoJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_spm_flFaWoJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_spm_flFaWoJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from fawoapp_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_spm_flFaWoJurnal.extend(window.childForm);
window.app_saku3_report_spm_flFaWoJurnal.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 2)
			{
				...
				var sql1="select a.no_woapp,a.keterangan from fawoapp_m a "+filter; 
				var sql2="select count( a.no_woapp) from fawoapp_m a "+filter;
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_saku3_spm_rptFaWoJurnal";
				if (this.sg1.getCell(2,3)=="Memo Jurnal")
				{
					this.nama_report="server_report_saku3_spm_rptFaWoBuktiJurnal";
				}
