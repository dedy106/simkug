window.app_saku3_report_spm_flIfKartu = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_spm_flIfKartu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_spm_flIfKartu";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from spm_if_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_spm_flIfKartu.extend(window.childForm);
window.app_saku3_report_spm_flIfKartu.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				var sql="select a.nik,b.nama "+
						"from (select a.nik from spm_if_m a "+filter+
						"	  group by a.nik "+
						"	  )a "+
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct susbtring(periode,1,4) from spm_if_m where kode_lokasi='"+this.app._lokasi+"'",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_spm_rptIfKartu";
			...
			systemAPI.alert("[app_saku3_report_spm_flIfKartu]::mainButtonClick:"+e);
		}
	},
