// JavaScript Document
window.app_saku3_report_spm_flDwLabaRugiRegional = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_spm_flDwLabaRugiRegional.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_spm_flDwLabaRugiRegional";
		this.maximize();
	...
	this.regional=this.dbLib.getPeriodeFromSQL("select kode_regional as periode from pp where  kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"'");
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Kode Regional","=",this.regional));
...
window.app_saku3_report_spm_flDwLabaRugiRegional.extend(window.portalui_childForm);
window.app_saku3_report_spm_flDwLabaRugiRegional.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 4)
		{
			this.filterRep.ListDataSGFilter(this, "Data Bidang",this.sg1, this.sg1.row, this.sg1.col,
													  "select kode_regional,nama from regional where flag_aktif='1' ",
													  "select count(kode_regional) from regional where flag_aktif='1' ",
													  new Array("kode_regional","nama"),"where");
		}
	},
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenTb(this.kode_lokasi,this.periode,this.kode_neraca,this.kode_regional);
				}
				if (this.link=="3")
				{
			...
			else
			{
				...
				var regional=	this.filterRep.filterStr("a.kode_regional",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"where");
				this.nama_report="server_report_saku3_spm_rptDwLabaRugiRegional";
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,3)+"/"+regional+"/"+this.sg1.getCell(2,5);
	...
	doOpenGar: function(kode_lokasi,periode,kode_neraca,kode_regional){
		...
		filter="where a.kode_lokasi='"+kode_lokasi+"' and e.kode_neraca='"+kode_neraca+"' and e.kode_fs='"+this.sg1.getCell(2,2)+"' and a.tahun='"+tahun+"' and c.kode_regional='"+kode_regional+"'";
	...
	doOpenGarSd: function(kode_lokasi,periode,kode_neraca,kode_regional){
		this.kode_neraca=kode_neraca;
		this.kode_lokasi=kode_lokasi;
		this.periode=periode;
		this.kode_regional=kode_regional;
	...
	doOpenTb: function(kode_lokasi,periode,kode_neraca,kode_regional){
		this.kode_neraca=kode_neraca;
		this.kode_lokasi=kode_lokasi;
		this.periode=periode;
		this.kode_regional=kode_regional;
		...
		filter2=this.sg1.getCell(2,0)+"/"+periode+"/"+kode_neraca+"/"+this.sg1.getCell(2,2);
		filter="where a.kode_lokasi='"+kode_lokasi+"' and b.kode_neraca='"+kode_neraca+"' and b.kode_fs='"+this.sg1.getCell(2,2)+"' and a.periode='"+periode+"' and d.kode_regional='"+kode_regional+"'";
