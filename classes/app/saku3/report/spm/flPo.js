window.app_saku3_report_spm_flPo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_spm_flPo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_spm_flPo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from log_po_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_spm_flPo.extend(window.childForm);
window.app_saku3_report_spm_flPo.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data PO",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_po, a.keterangan from log_po_m a "+this.filter,
												"select count(*) from log_po_m a "+this.filter,
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_spm_rptPo";
			...
			systemAPI.alert("[app_saku3_report_spm_flPo]::mainButtonClick:"+e);
		}
	},
