window.app_saku3_report_spm_flSppdAjuPos = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_spm_flSppdAjuPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_spm_flSppdAjuPos";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from pd_aju_m where kode_lokasi='"+this.app._lokasi+"' ");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("No SPPD","All",""));
...
window.app_saku3_report_spm_flSppdAjuPos.extend(window.childForm);
window.app_saku3_report_spm_flSppdAjuPos.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_aju, a.tujuan from pd_aju_m a "+this.filter,
												"select count(*) from pd_aju_m a "+this.filter,
												new Array("a.no_aju","a.tujuan"),"and",new Array("No Bukti","Tujuan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from pd_aju_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_spm_rptSppdAjuPos";
			...
			systemAPI.alert("[app_saku3_report_spm_flSppdAjuPos]::mainButtonClick:"+e);
		}
	},
