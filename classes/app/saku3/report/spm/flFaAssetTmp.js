window.app_saku3_report_spm_flFaAssetTmp = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_spm_flFaAssetTmp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_spm_flFaAssetTmp";
		this.maximize();
		...
		this.bTampil = new portalui_button(this,{bound:[20,220,150,18],caption:"Generate Simulasi",click:[this,"doClick"]});
	...
	this.period=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from periode where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,4,[0,1], ["No Asset","All"]);
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Output Laporan","=","Layar"));
	this.gridLib.SGEditData(this.sg1,6,[0,1,2], ["Kode PP","All",""]);
	this.gridLib.SGEditData(this.sg1,7,[0,1,2], ["No Hutang","All",""]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_spm_flFaAssetTmp.extend(window.childForm);
window.app_saku3_report_spm_flFaAssetTmp.implement({
		...
		try{			
			...
			if (row == 3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Kelompok Akun",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
												new Array("a.kode_klpakun","a.nama"),"and",new Array("Kode","Nama"),"and");
			}
			if (row == 4)
			{
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["3","236","236","123","123","3","123","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[2,0,0,2,2,0,2,2]);	
		if (row == 1)
		{
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and")+
							this.filterRep.filterStr("a.catatan",this.sg1.getCell(1,7),this.sg1.getCell(2,7),this.sg1.getCell(3,7),"and");
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,5);
				this.nama_report="server_report_saku3_spm_rptFaAssetTmp";
	...
	doClick:function(sender){
		...
		var bukti=this.app._lokasi+'-TMP'+this.period;
		var sql1="delete from fasusut_tmp where kode_lokasi='"+this.app._lokasi+"' and periode = '"+this.period+"'";
		var sql2="insert into fasusut_tmp(no_fasusut,no_fa,periode,nilai,kode_lokasi,akun_bp,akun_ap,kode_akun,kode_pp,kode_drk,dc,no_del,nilai_aset,umur) "+
		"select '"+bukti+"',a.no_fa,'"+this.period+"', "+
		...
		"		  a.kode_lokasi,c.akun_bp,c.akun_deprs,c.kode_akun,a.kode_pp_susut,'-','D','-',zz.nilai,a.umur "+
		"from fa_asset a "+
		...
		"			from fa_nilai "+
		"			where periode<='"+this.period+"' and kode_lokasi='"+this.app._lokasi+"' "+
		...
		"where progress = '2' and (abs(zz.nilai)-abs(a.nilai_residu)) > abs(isnull(b.tot_susut,0)) and a.kode_lokasi='"+this.app._lokasi+"' and a.periode_susut<='"+this.period+"' ";
		sql.add(sql1);
		sql.add(sql2);
