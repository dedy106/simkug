window.app_saku3_report_spm_flIfPtgJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_spm_flIfPtgJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_spm_flIfPtgJurnal";
		this.maximize();
...
window.app_saku3_report_spm_flIfPtgJurnal.extend(window.childForm);
window.app_saku3_report_spm_flIfPtgJurnal.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_reim, keterangan from spm_ifreim_m "+filter ,
														  "select count(no_reim) from spm_ifreim_m "+filter,
														  new Array("no_reim","keterangan"),"and");
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from spm_ifreim_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBITT' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							...
							this.filterRep.filterStr("a.no_reim",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," and ")+
							" and a.status_if<>'CLOSE' ";
				this.filter2=this.sg1.getCell(2,1);
				this.nama_report="server_report_saku3_spm_rptIfPtgJurnal";
