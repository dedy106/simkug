window.app_saku3_report_spm_flIfPtgForm = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_spm_flIfPtgForm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_spm_flIfPtgForm";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from spm_ifreim_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_spm_flIfPtgForm.extend(window.childForm);
window.app_saku3_report_spm_flIfPtgForm.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_reim, a.keterangan from spm_ifreim_m a "+this.filter,
												"select count(*) from spm_ifreim_m a "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from spm_ifreim_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_spm_rptIfPtgForm";
			...
			systemAPI.alert("[app_saku3_report_spm_flIfPtgForm]::mainButtonClick:"+e);
		}
	},
