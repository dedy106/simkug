window.app_saku3_report_spm_flPbPos = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_spm_flPbPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_spm_flPbPos";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("No Bukti PB","All",""));
...
window.app_saku3_report_spm_flPbPos.extend(window.childForm);
window.app_saku3_report_spm_flPbPos.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 2)
			{
				...
				// 	"select a.kode_pp, a.nama from pp a "+this.filter,
				// 	"select count(*) from pp a "+this.filter,
				...
				// else {
					...
					// filter+ " and c.nik='" + this.app._userLog + "' ";
					...
					// new Array("a.kode_pp", "a.nama"), "and", new Array("kode pp", "nama"));
					this.filterRep.ListDataSGFilter(this, "Data PP",this.sg1, this.sg1.row, this.sg1.col,
					"select a.kode_pp,a.nama from pp a inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter+" and b.nik='"+this.app._userLog+"'",
					"select count(a.kode_pp) from pp a inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter+" and b.nik='"+this.app._userLog+"'",
					new Array("a.kode_pp","a.nama"),"and",new Array("Kode PP","Nama"));
				// }
			}
			if (row ==3)
	...
	{		
		if (this.app._userStatus == "A") {
			...
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","123","23","123","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,2,2,0]);	
		}
		if (row == 1)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Periode","=",this.periode));
				this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode PP","All",""));
				this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("No KPA","All",""));
			}else{
				if(this.app._userStatus != "A" && (this.sg1.getCell(1,2) == "All" || this.sg1.getCell(1,2) == "all")){
					alert("Error: PP Anda tidak mempunyai akses untuk menampilkan data All");
					return false;
				}else{
					...
					this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(1,2)+"/"+this.app._userStatus;
					this.showFilter = this.filterRep.showFilter(this.sg1);
					var result  = new arrayMap();		

					this.nama_report = "server_report_saku3_spm_rptPbPos";
			...
			systemAPI.alert("[app_saku3_report_spm_flPbPos]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_hutang_rptPbForm";
