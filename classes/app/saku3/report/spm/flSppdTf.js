window.app_saku3_report_spm_flSppdTf = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_spm_flSppdTf.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_spm_flSppdTf";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from pd_spj_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_spm_flSppdTf.extend(window.childForm);
window.app_saku3_report_spm_flSppdTf.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data SPB",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_spj, a.no_dokumen from pd_spj_m a "+this.filter,
												"select count(*) from pd_spj_m a "+this.filter,
												new Array("a.no_spj","a.no_dokumen"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from pd_spj_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("b.no_spj",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_spm_rptSppdTf";
			...
			systemAPI.alert("[app_saku3_report_spm_flSppdTf]::mainButtonClick:"+e);
		}
	},
