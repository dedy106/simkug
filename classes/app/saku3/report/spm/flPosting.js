window.app_saku3_report_spm_flPosting = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_spm_flPosting.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_spm_flPosting";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from posting_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_spm_flPosting.extend(window.childForm);
window.app_saku3_report_spm_flPosting.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Bukti",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_post, a.keterangan from posting_m a "+filter,
												"select count(a.no_post) from posting_m a "+filter,
												new Array("a.no_post","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from posting_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_post",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_spm_rptPosting";
			...
			systemAPI.alert("[app_saku3_report_spm_flPosting]::mainButtonClick:"+e);
		}
	},
