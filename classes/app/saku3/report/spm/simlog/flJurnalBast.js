// JavaScript Document
window.app_saku3_report_spm_simlog_flJurnalBast = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_spm_simlog_flJurnalBast.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_spm_simlog_flJurnalBast";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from hutang_m where  modul='LOGBAST' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_spm_simlog_flJurnalBast.extend(window.portalui_childForm);
window.app_saku3_report_spm_simlog_flJurnalBast.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from hutang_m where modul='LOGBAST' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.nama_report="server_report_saku3_spm_simlog_rptJurnalBast";
