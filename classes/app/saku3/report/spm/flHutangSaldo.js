window.app_saku3_report_spm_flHutangSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_spm_flHutangSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_spm_flHutangSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from spm_proyek_bdd where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_spm_flHutangSaldo.extend(window.childForm);
window.app_saku3_report_spm_flHutangSaldo.implement({
		...
		try{			
			...
			if (row ==3)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("f.kode_vendor",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.kode_proyek, a.keterangan from spm_proyek_bdd a inner join yk_pb_d f on a.no_bukti=f.no_pb "+filter,
												"select count(a.kode_proyek) from spm_proyek_bdd a "+filter,
												new Array("a.kode_proyek","a.keterangan"),"and",new Array("No Invoice","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from spm_proyek_bdd where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_spm_rptHutangSaldo";
			...
			systemAPI.alert("[app_saku3_report_spm_flHutangSaldo]::mainButtonClick:"+e);
		}
	},
