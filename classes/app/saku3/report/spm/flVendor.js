window.app_saku3_report_spm_flVendor = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_spm_flVendor.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_spm_flVendor";
		this.maximize();
...
window.app_saku3_report_spm_flVendor.extend(window.childForm);
window.app_saku3_report_spm_flVendor.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==	1)
			{
				...
				{
					var filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+" and a.tipe='Posting' and c.nik='"+this.app._userLog+"' ";
					
					var sql="select a.kode_pp, a.nama "+
							"from pp a "+
							"inner join karyawan_pp c on a.kode_lokasi=c.kode_lokasi and a.kode_pp=c.kode_pp "+filter;
					var sql2="select count(a.kode_pp) "+
							"from pp a "+
							"inner join karyawan_pp c on a.kode_lokasi=c.kode_lokasi and a.kode_pp=c.kode_pp "+filter;
					this.filterRep.ListDataSGFilter(this, "Data PP",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,new Array("a.kode_pp","a.nama"),"and",new Array("kode pp","nama"));
				}
			}
			...
			if (row ==	2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Mitra",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_vendor, nama from vendor "+filter,
												"select count(kode_vendor) from vendor "+filter,
												new Array("kode_vendor","nama"),"and",new Array("No Kode Vendor","Nama"));
			}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_spm_rptVendor";
			...
			systemAPI.alert("[app_saku3_report_spm_flVendor]::mainButtonClick:"+e);
		}
	},
