window.app_saku3_report_amu_flSumurProduksi = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_amu_flSumurProduksi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_amu_flSumurProduksi";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("ID Lahan","All",""));
...
window.app_saku3_report_amu_flSumurProduksi.extend(window.childForm);
window.app_saku3_report_amu_flSumurProduksi.implement({
		...
		try{			
			...
			if (row == 1)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Lahan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.id_lahan, a.nama_lahan from amu_lahan a "+filter,
												"select count(a.id_lahan) from amu_lahan a "+filter,
												new Array("a.id_lahan","a.nama_lahan"),"and",new Array("ID Lahan","Keterangan"));
			}

            if (row == 2)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
                        this.filterRep.filterStr("a.id_lahan",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," and ");
				this.filterRep.ListDataSGFilter(this, "Data Sumur",this.sg1, this.sg1.row, this.sg1.col,
												"select a.id_sumur, a.nama_sumur from amu_sumur_bor a "+filter,
												"select count(a.id_sumur) from amu_sumur_bor a "+filter,
												new Array("a.id_sumur","a.nama_sumur"),"and",new Array("No Bukti","Keterangan"));
			}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							this.filterRep.filterStr("a.id_lahan",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," and ")+
							this.filterRep.filterStr("a.id_sumur",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," and ");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_amu_rptSumurProduksi";
			...
			systemAPI.alert("[app_saku3_report_amu_flSumurProduksi]::mainButtonClick:"+e);
		}
	},
