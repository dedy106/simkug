window.app_saku3_report_amu_flAsetDetail = function (owner) {
    if (owner) {
        window.app_saku3_report_amu_flAsetDetail.prototype.parent.constructor.call(this, owner);
        this.className = "app_saku3_report_amu_flAsetDetail";
        this.maximize();
        this.app._mainForm.childFormConfig(this, "mainButtonClick", "Laporan Aset", 2);
    ...
    this.gridLib.SGEditData(this.sg1, 1, new Array(0, 1, 2), new Array("Lembaga", "All", ""));
    this.gridLib.SGEditData(this.sg1, 2, new Array(0, 1, 2), new Array("Gedung", "All", ""));
	this.gridLib.SGEditData(this.sg1, 3, new Array(0, 1, 2), new Array("Lantai", "All", ""));
	this.gridLib.SGEditData(this.sg1, 4, new Array(0, 1, 2), new Array("Ruang", "All", ""));
	this.gridLib.SGEditData(this.sg1, 5, new Array(0, 1, 2), new Array("Kelompok Barang", "All", ""));
	this.gridLib.SGEditData(this.sg1, 6, new Array(0, 1, 2), new Array("Status Akhir", "All", ""));
	this.gridLib.SGEditData(this.sg1, 7, new Array(0, 1, 2), new Array("ID Barang", "All", ""));
    this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_amu_flAsetDetail.extend(window.childForm);
window.app_saku3_report_amu_flAsetDetail.implement({
    doEllipseClick: function (sender, col, row) {
        try {
			if (row == 1) {
                this.filterRep.ListDataSGFilter(this, "Data Lembaga", this.sg1, this.sg1.row, this.sg1.col,
                    "select id_lembaga,nama from amu_lembaga",
                    "select count(*) from amu_lembaga",
                    new Array("id_lembaga", "nama"), "where", new Array("ID", "Nama"));
            }
			if (row == 2) {
                this.filterRep.ListDataSGFilter(this, "Data Gedung", this.sg1, this.sg1.row, this.sg1.col,
                    "select id_gedung,nama from amu_gedung",
                    "select count(*) from amu_gedung",
                    new Array("id_gedung", "nama"), "where", new Array("ID", "Nama"));
            }
			if (row == 3) {
                this.filterRep.ListDataSGFilter(this, "Data Lantai", this.sg1, this.sg1.row, this.sg1.col,
                    "select id_lantai,nama from amu_lantai",
                    "select count(*) from amu_lantai",
                    new Array("id_lantai", "nama"), "where", new Array("ID", "Nama"));
            }
			if (row == 4) {
                this.filterRep.ListDataSGFilter(this, "Data Ruangan", this.sg1, this.sg1.row, this.sg1.col,
                    "select no_ruangan,nama from amu_ruangan",
                    "select count(*) from amu_ruangan",
                    new Array("no_ruangan", "nama"), "where", new Array("ID", "Nama"));
            }
            if (row == 5) {
                this.filterRep.ListDataSGFilter(this, "Data Kelompok Barang", this.sg1, this.sg1.row, this.sg1.col,
                    "select kode_klp,nama_klp from amu_klp_brg where kode_lokasi='" + this.app._lokasi + "'",
                    "select count(*) from amu_klp_brg where kode_lokasi='" + this.app._lokasi + "'",
                    new Array("kode_klp", "nama_klp"), "where", new Array("Kode", "Nama"));
            }
			if (row == 7) {
				var filter = this.filterRep.filterStr("a.kode_lokasi", this.sg1.getCell(1, 0), this.sg1.getCell(2, 0), this.sg1.getCell(3, 0), "where") +
							this.filterRep.filterStr("a.kode_klp", this.sg1.getCell(1, 5), this.sg1.getCell(2, 5), this.sg1.getCell(3, 5), "and");
                this.filterRep.ListDataSGFilter(this, "Data Barang", this.sg1, this.sg1.row, this.sg1.col,
                    "select a.no_bukti,a.deskripsi from amu_asset_bergerak a "+filter,
                    "select count(*) from amu_asset_bergerak a  "+filter,
                    new Array("a.no_bukti", "a.deskripsi"), "and", new Array("No Barang", "Deskripsi"));
            }
        } catch (e) {
            systemAPI.alert(e);
        }
    ...
    mainButtonClick: function (sender) {
        try {
            if (sender === this.app._mainForm.bClear2) {
				...
				this.gridLib.SGEditData(this.sg1, 2, new Array(0, 1, 2), new Array("Barang", "All", ""));
            } else {
                ...
                this.filter = this.filterRep.filterStr("a.kode_lokasi", this.sg1.getCell(1, 0), this.sg1.getCell(2, 0), this.sg1.getCell(3, 0), "where") +
                    this.filterRep.filterStr("a.kode_klp", this.sg1.getCell(1, 5), this.sg1.getCell(2, 5), this.sg1.getCell(3, 5), "and")+
					this.filterRep.filterStr("a.no_bukti", this.sg1.getCell(1, 7), this.sg1.getCell(2, 7), this.sg1.getCell(3, 7), "and");
                this.filter2 = this.sg1.getCell(2,0)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
                this.nama_report = "server_report_saku3_amu_rptAsetDetail";
    ...
    doOpenDetail: function (kd_asset) {
        this.id_lahan = kd_asset;
        this.link = "1";
        filter = "where a.kd_asset='" + kd_asset + "' and a.kode_lokasi='" + this.app._lokasi + "' and kode_pp='" + this.app._kodePP + "'";
        filter2 = "";
        nama_report = "server_report_saku3_amu_rptDetailAset";
        this.viewer.useIframe(this.report.previewWithHeader(nama_report, filter, this.page, this.pager, this.showFilter, this.lokasi, filter2));
    },
