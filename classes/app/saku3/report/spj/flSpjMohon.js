window.app_saku3_report_spj_flSpjMohon = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_spj_flSpjMohon.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_spj_flSpjMohon";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_spj_app where kode_lokasi='"+this.app._lokasi+"' ");
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("No SPJ","=",""));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Jenis SPPD","=","Surat Penerbitan"));
...
window.app_saku3_report_spj_flSpjMohon.extend(window.childForm);
window.app_saku3_report_spj_flSpjMohon.implement({
		...
		try{			
			...
			{	
				this.filterRep.ListDataSGFilter(this, "Data PP",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_pp, nama from pp where tipe='Posting' and kode_bidang='"+this.app._kodeBidang+"' "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," and "),
														  "select count(kode_pp) from pp where tipe='Posting' and kode_bidang='"+this.app._kodeBidang+"' "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," and "),
			...
			if (row ==4)
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_app",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							" ";
							
				this.filterRep.ListDataSGFilter(this, "Data SPJ",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_app, a.maksud from yk_spj_app a "+
												"	inner join yk_spj_m b on a.no_app=b.no_spj "+
												"	inner join pp c on b.kode_pp=c.kode_pp and b.kode_lokasi=c.kode_lokasi and c.kode_bidang='"+this.app._kodeBidang+"' "+this.filter, 
												
												"select count(*) from yk_spj_app a  "+
												
												"	inner join yk_spj_m b on a.no_app=b.no_spj "+
												"	inner join pp c on b.kode_pp=c.kode_pp and b.kode_lokasi=c.kode_lokasi and c.kode_bidang='"+this.app._kodeBidang+"' "+
												
												" "+this.filter,
												new Array("a.no_app","a.maksud"),"and",new Array("No APP","Keterangan"));
			}
	...
	{		
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Surat Penerbitan"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,5)=="Surat Penerbitan")
				{
					this.nama_report = "server_report_saku3_spj_rptSpjTerbit";
				}
			...
			systemAPI.alert("[app_saku3_report_spj_flSpjMohon]::mainButtonClick:"+e);
		}
	},
