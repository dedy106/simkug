window.app_saku3_report_spj_flPdPosTu = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_spj_flPdPosTu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_spj_flPdPosTu";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from tu_pdaju_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_spj_flPdPosTu.extend(window.childForm);
window.app_saku3_report_spj_flPdPosTu.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				var sql="select a.nik,a.nama "+
						"from karyawan a "+
						"inner join (select a.nik_spj,a.kode_lokasi "+
						"			from tu_pdaju_m a "+filter+
						"			group by a.nik_spj,a.kode_lokasi "+
						"			)b on a.nik=b.nik_spj and a.kode_lokasi=b.kode_lokasi "+filter;
				var sql2="select count(a.nik) "+
						...
						"			group by a.nik_spj,a.kode_lokasi "+
						"			)b on a.nik=b.nik_spj and a.kode_lokasi=b.kode_lokasi "+filter;
				this.filterRep.ListDataSGFilter(this, "Data Karyawan",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_spj, a.keterangan from tu_pdaju_m a "+this.filter,
												"select count(*) from tu_pdaju_m a "+this.filter,
												new Array("a.no_spj","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		else
		{
			...
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[3,0,2,2,1]);
		}	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from tu_pdaju_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_spj_rptPdPosTu";
			...
			systemAPI.alert("[app_saku3_report_spj_flPdPosTu]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku2_kopeg_kbitt_rptBebanFormTu3";
	...
	doOpenSpj: function(no_bukti,kode_lokasi){
		this.no_bukti=no_bukti;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = "where a.no_app='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
		nama_report="server_report_saku3_spj_rptPdAppFormTu";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	},
	doOpenSpjApp: function(no_bukti,kode_lokasi){
		this.no_bukti=no_bukti;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = "where a.no_app='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
