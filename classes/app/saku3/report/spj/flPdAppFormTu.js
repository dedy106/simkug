window.app_saku3_report_spj_flPdAppFormTu = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_spj_flPdAppFormTu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_spj_flPdAppFormTu";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from tu_pdapp_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_spj_flPdAppFormTu.extend(window.childForm);
window.app_saku3_report_spj_flPdAppFormTu.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_app, a.keterangan from tu_pdapp_m a "+this.filter,
												"select count(*) from tu_pdapp_m a "+this.filter,
												new Array("a.no_app","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from tu_pdapp_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_spj_flPdAppFormTu]::mainButtonClick:"+e);
		}
	},
