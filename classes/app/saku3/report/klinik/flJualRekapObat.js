window.app_saku3_report_klinik_flJualRekapObat = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_klinik_flJualRekapObat.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_klinik_flJualRekapObat";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode Obat","All",""));
...
window.app_saku3_report_klinik_flJualRekapObat.extend(window.childForm);
window.app_saku3_report_klinik_flJualRekapObat.implement({
		...
		try{			
			...
			if (row ==2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Obat",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_obat, nama from kli_obat ",
												"select count(*) from kli_obat ",
												new Array("kode_obat","nama"),"where",new Array("kode","nama"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kli_jual_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("b.kode_obat",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_klinik_rptJualRekapObat";
			...
			systemAPI.alert("[app_saku3_report_klinik_flJualRekapObat]::mainButtonClick:"+e);
		}
	},
