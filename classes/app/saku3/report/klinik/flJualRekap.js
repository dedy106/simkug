window.app_saku3_report_klinik_flJualRekap = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_klinik_flJualRekap.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_klinik_flJualRekap";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kli_jual_m where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Marketing","All",""));
...
window.app_saku3_report_klinik_flJualRekap.extend(window.childForm);
window.app_saku3_report_klinik_flJualRekap.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==2)
			{
				this.filterRep.ListDataSGFilter(this, "Data cust",this.sg1, this.sg1.row, this.sg1.col,
			...
			if (row ==3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Gudang",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_gudang, nama from apo_brg_gudang ",
												"select count(*) from apo_brg_gudang ",
												new Array("kode_gudang","nama"),"where",new Array("kode","nama"));
			}
			if (row ==4)
			{
				this.filterRep.ListDataSGFilter(this, "Data Marketing",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_gudang, nama from apo_brg_gudang ",
												"select count(*) from apo_brg_gudang ",
												new Array("kode_gudang","nama"),"where",new Array("kode","nama"));
			}
			if (row ==5)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Penjualan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_jual, a.keterangan from kli_jual_m a "+filter,
												"select count(a.no_jual) from kli_jual_m a "+filter,
												new Array("a.no_jual","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kli_jual_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_klinik_rptJualRekap";
			...
			systemAPI.alert("[app_saku3_report_klinik_flJualRekap]::mainButtonClick:"+e);
		}
	},
