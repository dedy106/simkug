window.app_saku3_report_klinik_flStok = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_klinik_flStok.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_klinik_flStok";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kli_beli_m where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Kode Obat","All",""));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Tampil Stok Nol","=","Ya"));
...
window.app_saku3_report_klinik_flStok.extend(window.childForm);
window.app_saku3_report_klinik_flStok.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Gudang",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_pp, nama from kli_pp "+filter,
												"select count(*) from kli_pp "+filter,
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Kelompok",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_klp, nama from kli_obat_klp "+filter,
												"select count(*) from kli_obat_klp "+filter,
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Obat",this.sg1, this.sg1.row, this.sg1.col,
												"select a.kode_obat, a.nama from kli_obat a "+filter,
												"select count(a.kode_obat) from kli_obat a "+filter,
												new Array("a.kode_obat","a.nama"),"and",new Array("Kode Obat","Nama"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kli_beli_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 5)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and")+
							...
							this.filterRep.filterStr("a.kode_obat",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0)+"/"+this.app._nikUser+"/"+this.sg1.getCell(2,5);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_klinik_rptStok";
			...
			systemAPI.alert("[app_saku3_report_klinik_flStok]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBayar: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter = "where a.kode_obat='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
