window.app_saku3_report_klinik_flBeli = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_klinik_flBeli.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_klinik_flBeli";
		this.maximize();
...
window.app_saku3_report_klinik_flBeli.extend(window.childForm);
window.app_saku3_report_klinik_flBeli.implement({
		...
		try{			
			...
			if (row ==2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Vendor",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_vendor, nama from kli_vendor where kode_lokasi='"+this.app._lokasi+"'",
												"select count(*) from kli_vendor where kode_lokasi='"+this.app._lokasi+"'",
			...
			if (row ==3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Gudang",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_gudang, nama from kli_pp where kode_lokasi='"+this.app._lokasi+"'",
												"select count(*) from kli_pp where kode_lokasi='"+this.app._lokasi+"'",
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pembelian",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_beli, a.keterangan from kli_beli_m a "+filter,
												"select count(a.no_beli) from kli_beli_m a "+filter,
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_klinik_flBeli]::mainButtonClick:"+e);
		}
	},
