window.app_saku3_report_klinik_flBeliRekapObat = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_klinik_flBeliRekapObat.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_klinik_flBeliRekapObat";
		this.maximize();
...
window.app_saku3_report_klinik_flBeliRekapObat.extend(window.childForm);
window.app_saku3_report_klinik_flBeliRekapObat.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kli_beli_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_klinik_rptBeliRekapObat";
			...
			systemAPI.alert("[app_saku3_report_klinik_flBeliRekapObat]::mainButtonClick:"+e);
		}
	},
