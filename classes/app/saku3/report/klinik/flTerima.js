window.app_saku3_report_klinik_flTerima = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_klinik_flTerima.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_klinik_flTerima";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kli_terima_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_klinik_flTerima.extend(window.childForm);
window.app_saku3_report_klinik_flTerima.implement({
		...
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pemesanan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_terima, a.keterangan from kli_terima_m a "+filter,
												"select count(a.no_terima) from kli_terima_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kli_terima_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_pp_asal",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.kode_pp_tujuan",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
				...
				this.nama_report = "server_report_saku3_klinik_rptTerima";
			...
			systemAPI.alert("[app_saku3_report_klinik_flTerima]::mainButtonClick:"+e);
		}
	},
