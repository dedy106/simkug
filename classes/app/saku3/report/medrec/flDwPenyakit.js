// JavaScript Document
window.app_saku3_report_medrec_flDwPenyakit = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_medrec_flDwPenyakit.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_medrec_flDwPenyakit";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from exs_yk_bp_jejer ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Jenis","=","PEGAWAI"));
...
window.app_saku3_report_medrec_flDwPenyakit.extend(window.portalui_childForm);
window.app_saku3_report_medrec_flDwPenyakit.implement({
	...
	doSelectCell: function(sender, col, row){
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4),new  Array("123","3","3","3","3"));
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4),new  Array(2,0,0,0,0));
		
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct tahun from exs_yk_bp_jejer  order by tahun desc",[this.sg1.columns.get(2).pickList]);
			
		}
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("PEGAWAI","GROUP","PENSIUN"));
		}	
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report="server_report_saku3_medrec_rptDwPenyakit";
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(3,0);
