// JavaScript Document
window.app_saku3_report_panjar_flPanjarCairJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_panjar_flPanjarCairJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_panjar_flPanjarCairJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBPJ' ");
...
window.app_saku3_report_panjar_flPanjarCairJurnal.extend(window.portalui_childForm);
window.app_saku3_report_panjar_flPanjarCairJurnal.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			...
			var filter=this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
						...
						" and a.modul='KBPJ' ";
			var sql1="select a.no_kas,a.keterangan from kas_m a "+filter; 
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBPJ' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.nama_report="server_report_saku3_kb_rptKbJurnalPp";
