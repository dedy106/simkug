window.app_saku3_report_bpr_flHutangSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bpr_flHutangSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bpr_flHutangSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from hu_bill_d where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Vendor","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Akun Hutang","All",""));
...
window.app_saku3_report_bpr_flHutangSaldo.extend(window.childForm);
window.app_saku3_report_bpr_flHutangSaldo.implement({
		...
		try{			
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Vendor",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_vendor, nama from vendor "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(kode_vendor) from vendor "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
			...
			if (row ==4)
			{
				this.filterRep.ListDataSGFilter(this, "KOde PP",this.sg1, this.sg1.row, this.sg1.col,
												"select distinct a.kode_pp , a.nama from pp a inner join hu_bill_d b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi where  a.kode_lokasi='01'",
												"select  distinct a.kode_pp , a.nama  from pp a inner join hu_bill_d b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi where  a.kode_lokasi='01'",
			...
			if (row ==5)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_hutang, a.keterangan from hu_bill_d a "+filter,
												"select count(a.no_hutang) from hu_bill_d a "+filter,
												new Array("a.no_hutang","a.keterangan"),"and",new Array("No Invoice","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from hu_bill_d where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.akun_hutang",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							this.filterRep.filterStr("a.no_hutang",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_bpr_rptHutangSaldo";
			...
			systemAPI.alert("[app_saku3_report_bpr_flHutangSaldo]::mainButtonClick:"+e);
		}
	},
