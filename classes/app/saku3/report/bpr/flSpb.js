window.app_saku3_report_bpr_flSpb = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bpr_flSpb.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bpr_flSpb";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where form='PBDIR' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_bpr_flSpb.extend(window.childForm);
window.app_saku3_report_bpr_flSpb.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where form='PBDIR' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_bpr_flSpb]::mainButtonClick:"+e);
		}
	},
