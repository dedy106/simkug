window.app_saku3_report_bpr_flPanjarPtgPos = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bpr_flPanjarPtgPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bpr_flPanjarPtgPos";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from panjarptg2_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_bpr_flPanjarPtgPos.extend(window.childForm);
window.app_saku3_report_bpr_flPanjarPtgPos.implement({
		...
		try{			
			...
			if (row ==2)
			{
				this.filterRep.ListDataSGFilter(this, "KOde PP",this.sg1, this.sg1.row, this.sg1.col,
												"select distinct a.kode_pp , a.nama from pp a inner join trans_m b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi ",
												"select  distinct a.kode_pp , a.nama  from pp a inner join trans_m b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi ",
			...
			if (row ==3)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.ListDataSGFilter(this, "Data PB",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_ptg,a.keterangan from panjarptg2_m a "+filter,
												"select count(a.no_ptg) from panjarptg2_m a "+filter,
												new Array("a.no_ptg","a.keterangan"),"and",new Array("No Invoice","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from panjarptg2_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_ptg",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_bpr_rptPanjarPtgPos";
			...
			systemAPI.alert("[app_saku3_report_bpr_flPanjarPtgPos]::mainButtonClick:"+e);
		}
	},
