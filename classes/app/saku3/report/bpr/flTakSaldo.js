window.app_saku3_report_bpr_flTakSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bpr_flTakSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bpr_flTakSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where form='TKUMUM' and kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_bpr_flTakSaldo.extend(window.childForm);
window.app_saku3_report_bpr_flTakSaldo.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data TAK Kirim",this.sg1, this.sg1.row, this.sg1.col,
												...
												new Array("a.no_bukti","a.keterangan"),"and",new Array("No Kirim","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where form='TKUMUM' and kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("TAK Kirim"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_bpr_rptTakSaldo";
			...
			systemAPI.alert("[app_saku3_report_bpr_flTakSaldo]::mainButtonClick:"+e);
		}
	},
