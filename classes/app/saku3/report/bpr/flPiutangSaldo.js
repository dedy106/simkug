window.app_saku3_report_bpr_flPiutangSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bpr_flPiutangSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bpr_flPiutangSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from pu_bill_d where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_bpr_flPiutangSaldo.extend(window.childForm);
window.app_saku3_report_bpr_flPiutangSaldo.implement({
		...
		try{			
			...
			if (row ==4)
			{
				this.filterRep.ListDataSGFilter(this, "KOde PP",this.sg1, this.sg1.row, this.sg1.col,
												"select distinct a.kode_pp , a.nama from pp a inner join pu_bill_d b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi where  a.kode_lokasi='01'",
												"select  distinct a.kode_pp , a.nama  from pp a inner join pu_bill_d b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi where  a.kode_lokasi='01'",
			...
			if (row ==5)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_piutang, a.keterangan from pu_bill_d a "+filter,
												"select count(a.no_piutang) from pu_bill_d a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from pu_bill_d where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_bpr_rptPiutangSaldo";
			...
			systemAPI.alert("[app_saku3_report_bpr_flPiutangSaldo]::mainButtonClick:"+e);
		}
	},
