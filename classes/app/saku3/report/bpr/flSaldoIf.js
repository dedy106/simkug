window.app_saku3_report_bpr_flSaldoIf = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bpr_flSaldoIf.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bpr_flSaldoIf";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where form='IFREIM' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_bpr_flSaldoIf.extend(window.childForm);
window.app_saku3_report_bpr_flSaldoIf.implement({
		...
		try{			
			...
			if (row ==3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Karyawan",this.sg1, this.sg1.row, this.sg1.col,
											  "select a.nik, a.nama from karyawan a inner join if_m b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi where a.kode_lokasi = '"+this.sg1.getCell(2,0)+"'",
											  "select count(*)      from karyawan a inner join if_m b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi where a.kode_lokasi = '"+this.sg1.getCell(2,0)+"'",
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where form='IFREIM' and  kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_bpr_rptSaldoIf";
			...
			systemAPI.alert("[app_saku3_report_bpr_flSaldoIf]::mainButtonClick:"+e);
		}
	},
