// JavaScript Document
window.app_saku3_report_bpr_flJurnalKb = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bpr_flJurnalKb.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bpr_flJurnalKb";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where Form='KB' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_bpr_flJurnalKb.extend(window.portalui_childForm);
window.app_saku3_report_bpr_flJurnalKb.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where Form='KB' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.nama_report="server_report_saku3_bpr_rptJurnalKb";
