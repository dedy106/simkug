window.app_saku3_report_bpr_flPanjarAjuPos = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bpr_flPanjarAjuPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bpr_flPanjarAjuPos";
		this.maximize();
...
window.app_saku3_report_bpr_flPanjarAjuPos.extend(window.childForm);
window.app_saku3_report_bpr_flPanjarAjuPos.implement({
			...
			if (row ==3)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.ListDataSGFilter(this, "Data PB",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_pj,a.keterangan from panjar_m a "+filter,
												"select count(a.no_pj) from panjar_m a "+filter,
												new Array("a.no_pj","a.keterangan"),"and",new Array("No Invoice","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from panjar_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_bpr_rptPanjarAjuPos";
			...
			systemAPI.alert("[app_saku3_report_bpr_flPanjarAjuPos]::mainButtonClick:"+e);
		}
	},
