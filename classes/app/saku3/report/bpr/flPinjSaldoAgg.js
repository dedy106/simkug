window.app_saku3_report_bpr_flPinjSaldoAgg = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bpr_flPinjSaldoAgg.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bpr_flPinjSaldoAgg";
		this.maximize();
...
window.app_saku3_report_bpr_flPinjSaldoAgg.extend(window.childForm);
window.app_saku3_report_bpr_flPinjSaldoAgg.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_pinjangs_d where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_bpr_rptPinjSaldoAgg";
