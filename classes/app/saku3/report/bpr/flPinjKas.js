window.app_saku3_report_bpr_flPinjKas = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bpr_flPinjKas.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bpr_flPinjKas";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('PINJCAIR','PJANGS')");
...
window.app_saku3_report_bpr_flPinjKas.extend(window.childForm);
window.app_saku3_report_bpr_flPinjKas.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBPIU'  order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_bpr_rptPinjKas";
			...
			systemAPI.alert("[app_saku3_report_bpr_flPinjKas]::mainButtonClick:"+e);
		}
	},
