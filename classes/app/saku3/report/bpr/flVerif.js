window.app_saku3_report_bpr_flVerif = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bpr_flVerif.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bpr_flVerif";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from pb_app_m where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("No Ver","All",""));
...
window.app_saku3_report_bpr_flVerif.extend(window.childForm);
window.app_saku3_report_bpr_flVerif.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Ver",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_app, a.catatan from pb_app_m a "+filter,
												"select count(a.no_app) from pb_app_m a "+filter,
												new Array("a.no_app","a.keterangan"),"and",new Array("No Ver","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from pb_app_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_bpr_flVerif]::mainButtonClick:"+e);
		}
	},
