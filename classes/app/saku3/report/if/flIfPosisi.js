window.app_saku3_report_if_flIfPosisi = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_if_flIfPosisi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_if_flIfPosisi";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from if_aju_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_if_flIfPosisi.extend(window.childForm);
window.app_saku3_report_if_flIfPosisi.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==3)
			{
				var sql="select b.nik,b.nama "+
						"from if_aju_m a "+
						"inner join karyawan b on a.user_input=b.nik and a.kode_lokasi=b.kode_lokasi "+
						"where a.kode_lokasi='"+this.sg1.getCell(2,0)+"' ";
				var sql2="select count(b.nik) "+
						"from if_aju_m a "+
						"inner join karyawan b on a.user_input=b.nik and a.kode_lokasi=b.kode_lokasi "+
						"where a.kode_lokasi='"+this.sg1.getCell(2,0)+"' ";		
				this.filterRep.ListDataSGFilter(this, "Data Karyawan",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,
												new Array("b.nik","b.nama"),"and",new Array("NIK","Nama"));
			}
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_aju, a.keterangan from if_aju_m a "+this.filter,
												"select count(*) from if_aju_m a "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from if_aju_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.user_input",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
				...
				this.nama_report = "server_report_saku3_if_rptIfPosisi";
			...
			systemAPI.alert("[app_saku3_report_if_flIfPosisi]::mainButtonClick:"+e);
		}
	},
