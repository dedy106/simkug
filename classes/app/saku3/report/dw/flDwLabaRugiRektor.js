// JavaScript Document
window.app_saku3_report_dw_flDwLabaRugiRektor = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dw_flDwLabaRugiRektor.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dw_flDwLabaRugiRektor";
		this.maximize();
	...
	var sql="select e.kode_rektor as periode "+
			"from pp d "+
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Kode Rektor","=",this.pp));
...
window.app_saku3_report_dw_flDwLabaRugiRektor.extend(window.portalui_childForm);
window.app_saku3_report_dw_flDwLabaRugiRektor.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 4)
		{
			this.filterRep.ListDataSGFilter(this, "Data Rektor",this.sg1, this.sg1.row, this.sg1.col,
													  "select kode_rektor,nama from exs_rektor "+
													  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
													  " and flag_aktif='1' ",
													  "select count(kode_rektor) from exs_rektor "+
													  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
													  " and flag_aktif='1' ",
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenTb(this.kode_lokasi,this.periode,this.kode_neraca,this.kode_rektor);
				}
				if (this.link=="3")
				{
			...
			else
			{
				...
				var rektor=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.kode_rektor",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.nama_report="server_report_saku3_dw_rptDwLabaRugiRektor";
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,3)+"/"+rektor+"/"+this.sg1.getCell(2,5);
	...
	doOpenGar: function(kode_lokasi,periode,kode_neraca,kode_rektor){
		this.kode_neraca=kode_neraca;
		this.kode_lokasi=kode_lokasi;
		this.periode=periode;
		this.kode_rektor=kode_rektor;
		this.link="1";
		var tahun=periode.substr(0,4);
		filter2=this.sg1.getCell(2,0)+"/"+tahun+"/"+kode_neraca+"/"+this.sg1.getCell(2,2);
		filter="where a.kode_lokasi='"+kode_lokasi+"' and e.kode_neraca='"+kode_neraca+"' and e.kode_fs='"+this.sg1.getCell(2,2)+"' and a.tahun='"+tahun+"' and f.kode_rektor='"+kode_rektor+"'";
	...
	doOpenGarSd: function(kode_lokasi,periode,kode_neraca,kode_rektor){
		this.kode_neraca=kode_neraca;
		this.kode_lokasi=kode_lokasi;
		this.periode=periode;
		this.kode_rektor=kode_rektor;
	...
	doOpenTb: function(kode_lokasi,periode,kode_neraca,kode_rektor){
		this.kode_neraca=kode_neraca;
		this.kode_lokasi=kode_lokasi;
		this.periode=periode;
		this.kode_rektor=kode_rektor;
		...
		filter2=this.sg1.getCell(2,0)+"/"+periode+"/"+kode_neraca+"/"+this.sg1.getCell(2,2);
		filter="where a.kode_lokasi='"+kode_lokasi+"' and b.kode_neraca='"+kode_neraca+"' and b.kode_fs='"+this.sg1.getCell(2,2)+"' and a.periode='"+periode+"' and e.kode_rektor='"+kode_rektor+"'";
