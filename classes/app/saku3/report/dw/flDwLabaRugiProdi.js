// JavaScript Document
window.app_saku3_report_dw_flDwLabaRugiProdi = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dw_flDwLabaRugiProdi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dw_flDwLabaRugiProdi";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Kode Prodi","=",this.app._kodePP));
	this.gridLib.SGEditData(this.sg1,5,[0,1,2],["Output Laporan","=","Layar"]);
...
window.app_saku3_report_dw_flDwLabaRugiProdi.extend(window.portalui_childForm);
window.app_saku3_report_dw_flDwLabaRugiProdi.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 4)
		{
			...
			{
				var sql="select a.kode_prodi,a.nama from exs_prodi a "+
						"where a.kode_lokasi='"+this.app._lokasi+"'  ";
				
				var sql2="select count(a.kode_prodi) from exs_prodi a "+
						"where a.kode_lokasi='"+this.app._lokasi+"' ";
			}
			else
			{
				var sql="select a.kode_prodi,a.nama from exs_prodi a "+
						"inner join karyawan_pp b on a.kode_lokasi=b.kode_lokasi and a.kode_prodi=b.kode_pp "+
			...
			this.filterRep.ListDataSGFilter(this, "Data Prodi",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,new Array("a.kode_prodi","a.nama"),"where");
		}
	},
	doSelectCell: function(sender, col, row){
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5),new  Array("3","3","2","3","123","3"));
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(3,1,2,3,4,5),new  Array(3,0,3,0,2,0));
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5),new  Array("3","3","2","3","123","3"));
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5),new  Array(3,0,3,0,2,0));
		}
		if (row == 1)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenTb(this.kode_lokasi,this.periode,this.kode_neraca,this.kode_fakultas);
				}
				if (this.link=="3")
				{
			...
			else
			{
				...
				var bidang=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.kode_prodi",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.nama_report="server_report_saku3_dw_rptDwLabaRugiProdi";
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,3)+"/"+bidang+"/"+this.sg1.getCell(2,5);
	...
	doOpenGar: function(kode_lokasi,periode,kode_neraca,kode_fakultas){
		...
		filter2=this.sg1.getCell(2,0)+"/"+tahun+"/1";
		filter="where a.kode_lokasi='"+kode_lokasi+"' and e.kode_neraca='"+kode_neraca+"' and e.kode_fs='"+this.sg1.getCell(2,2)+"' and a.tahun='"+tahun+"' and g.kode_fakultas='"+kode_fakultas+"'";
	...
	doOpenGarSd: function(kode_lokasi,periode,kode_neraca,kode_fakultas){
		this.kode_neraca=kode_neraca;
		this.kode_lokasi=kode_lokasi;
		this.periode=periode;
		this.kode_fakultas=kode_fakultas;
		this.link="1";
		var tahun=periode.substr(0,4);
		filter2=this.sg1.getCell(2,0)+"/"+periode+"/1";
		filter="where a.kode_lokasi='"+kode_lokasi+"' and e.kode_neraca='"+kode_neraca+"' and e.kode_fs='"+this.sg1.getCell(2,2)+"' and a.tahun='"+tahun+"' and g.kode_fakultas='"+kode_fakultas+"'";
		nama_report="server_report_saku3_dw_rptGarBulanSdTu";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenTb: function(kode_lokasi,periode,kode_neraca,kode_fakultas){
		this.kode_neraca=kode_neraca;
		this.kode_lokasi=kode_lokasi;
		this.periode=periode;
		this.kode_fakultas=kode_fakultas;
		...
		filter2=this.sg1.getCell(2,0)+"/"+periode+"/"+kode_neraca+"/"+this.sg1.getCell(2,2)+"/1";
		filter="where a.kode_lokasi='"+kode_lokasi+"' and b.kode_neraca='"+kode_neraca+"' and b.kode_fs='"+this.sg1.getCell(2,2)+"' and a.periode='"+periode+"' and f.kode_fakultas='"+kode_fakultas+"'";
