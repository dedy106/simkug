// JavaScript Document
window.app_saku3_report_dw_flDwLabaRugiPp = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dw_flDwLabaRugiPp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dw_flDwLabaRugiPp";
		this.maximize();
...
window.app_saku3_report_dw_flDwLabaRugiPp.extend(window.portalui_childForm);
window.app_saku3_report_dw_flDwLabaRugiPp.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 4)
		{
			...
			{
				var sql="select a.kode_pp,a.nama from pp a "+
						"where a.kode_lokasi='"+this.app._lokasi+"' and a.flag_aktif='1' ";
				
				var sql2="select count(a.kode_pp) from pp a "+
						"where a.kode_lokasi='"+this.app._lokasi+"' and a.flag_aktif='1' ";
			}
			else
			{
				var sql="select a.kode_pp,a.nama from pp a "+
						...
						"where a.kode_lokasi='"+this.app._lokasi+"' and b.nik='"+this.app._userLog+"' and a.flag_aktif='1' ";
				
				var sql2="select count(a.kode_pp) from pp a "+
						"inner join karyawan_pp b on a.kode_lokasi=b.kode_lokasi and a.kode_pp=b.kode_pp "+
						"where a.kode_lokasi='"+this.app._lokasi+"' and b.nik='"+this.app._userLog+"' and a.flag_aktif='1' ";
			}
			this.filterRep.ListDataSGFilter(this, "Data Prodi",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,new Array("a.kode_pp","a.nama"),"where");
		}
	},
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report="server_report_saku3_dw_rptDwLabaRugiPp";
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,3)+"/"+kode_pp+"/"+this.sg1.getCell(2,5);
