// JavaScript Document
window.app_saku3_report_dw_flDwPdptFakultasGrafik = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dw_flDwPdptFakultasGrafik.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dw_flDwPdptFakultasGrafik";
		this.maximize();
		...
		this.pg1 = new pageControl(this,{bound:[0,0,this.width - 20, this.height -30], childPage:["Filter","Grafik Fakultas","Grafik Prodi","Tabel","Trail"], pageChange:[this,"doPageChange"]});
		...
		this.viewer = new reportViewer(this.pg1.childPage[3],{bound:[0,0,this.width - 35, this.height - 35]});
		this.viewer2 = new reportViewer(this.pg1.childPage[4],{bound:[0,0,this.width - 35, this.height - 35]});
	...
	try{
		...
		this.fakultas=this.dbLib.getPeriodeFromSQL("select a.kode_fakultas as periode "+
													"from exs_prodi a "+
													"where a.kode_prodi='"+this.app._kodePP+"' and a.kode_lokasi='"+this.app._lokasi+"' ");
		...
		this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode Akun","=","4121102"));
...
window.app_saku3_report_dw_flDwPdptFakultasGrafik.extend(window.portalui_childForm);
window.app_saku3_report_dw_flDwPdptFakultasGrafik.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			this.filterRep.ListDataSGFilter(this, "Data Fakultas",this.sg1, this.sg1.row, this.sg1.col,
													  "select kode_fakultas,nama from exs_fakultas "+
													  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
													  " and flag_aktif='1' ",
													  "select count(kode_fakultas) from exs_fakultas "+
													  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
													  " and flag_aktif='1' ",
													  new Array("kode_fakultas","nama"),"where");
		}
	},
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.nama_report="server_report_saku3_dw_rptDwPdptFakultas";
			this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,3);
		...
		//	this.app._mainForm.reportNavigator.rearrange();
			...
			this.nama_fakultas=this.dbLib.getPeriodeFromSQL("select nama as periode from exs_fakultas where kode_fakultas='"+this.sg1.getCell(2,3)+"' and kode_lokasi='"+this.sg1.getCell(2,0)+"'");

			var sql = "select c.kode_fakultas,c.nama,sum(ISNULL(b.n2*-1,0)) as n2,sum(ISNULL(b.n4*-1,0)) as n4,sum(ISNULL(b.n4*-1,0)-ISNULL(b.n2*-1,0)) as deviasi "+
					"from aka_jurusan a "+
					"left join (select a.kode_pp,a.kode_lokasi,a.kode_akun,sum(n2) as n2,sum(n4) as n4 "+
					"from exs_glma_gar_pp a "+this.filter+
					"group by a.kode_pp,a.kode_lokasi,a.kode_akun "+
					"		  )b on a.kode_jur=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
					"inner join aka_fakultas c on a.kode_fakultas=c.kode_fakultas and a.kode_lokasi=c.kode_lokasi "+
					"where a.kode_lokasi='11' and (ISNULL(b.n2,0)<>0 or ISNULL(b.n4,0)<>0) and a.kode_fakultas='"+this.sg1.getCell(2,3)+"' "+
					"group by c.kode_fakultas,c.nama ";
			...
			var options = {
				...
				var sql = "select a.kode_pp,a.nama,ISNULL(b.n2*-1,0) as n2,ISNULL(b.n4*-1,0) as n4,ISNULL(b.n4*-1,0)-ISNULL(b.n2*-1,0) as deviasi "+
					"from exs_pp a "+
					"left join (select a.kode_pp,a.kode_lokasi,a.kode_akun,sum(n2) as n2,sum(n4) as n4 "+
					"from exs_glma_gar_pp a "+this.filter+
					"group by a.kode_pp,a.kode_lokasi,a.kode_akun "+
					"		  )b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
					"inner join aka_jurusan c on a.kode_pp=c.kode_jur and a.kode_lokasi=c.kode_lokasi "+
					"where a.kode_lokasi='11' and (ISNULL(b.n2,0)<>0 or ISNULL(b.n4,0)<>0) and c.kode_fakultas='"+this.sg1.getCell(2,3)+"' ";
				...
				for (var i = 0; i < data.rs.rows.length; i++){
					var line = data.rs.rows[i];
						...
						categories.push(line.nama);
						this.dataCategories.push(line.kode_pp);
						series1.data.push(parseFloat(line.n2));
						series2.data.push(parseFloat(line.n4));
