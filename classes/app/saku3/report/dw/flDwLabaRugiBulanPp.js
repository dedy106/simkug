// JavaScript Document
window.app_saku3_report_dw_flDwLabaRugiBulanPp = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dw_flDwLabaRugiBulanPp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dw_flDwLabaRugiBulanPp";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select substring(max(periode),1,4) as periode from exs_periode where  kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Jenis","=","Mutasi"));
...
window.app_saku3_report_dw_flDwLabaRugiBulanPp.extend(window.portalui_childForm);
window.app_saku3_report_dw_flDwLabaRugiBulanPp.implement({
	...
	doSelectCell: function(sender, col, row){
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5),new  Array("123","3","123","3","3","123"));
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5),new  Array(2,0,2,0,0,2));
		
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from exs_periode where kode_lokasi='"+this.app._lokasi+"' order by substring(periode,1,4) desc",[this.sg1.columns.get(2).pickList]);
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Mutasi","Saldo Akhir"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report="server_report_saku3_dw_rptDwLabaRugiBulanPp";
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,3)+"/"+this.sg1.getCell(2,4)+"/"+
								this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"where");
