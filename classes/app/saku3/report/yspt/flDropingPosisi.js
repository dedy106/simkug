window.app_saku3_report_yspt_flDropingPosisi = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yspt_flDropingPosisi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yspt_flDropingPosisi";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from ys_minta_m where kode_lokasi='"+this.app._lokasi+"'  ");
...
window.app_saku3_report_yspt_flDropingPosisi.extend(window.childForm);
window.app_saku3_report_yspt_flDropingPosisi.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Bukti",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_minta, a.keterangan from ys_minta_m a "+this.filter,
												"select count(*) from ys_minta_m a "+this.filter,
												new Array("a.no_minta","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["123","123","123","!23"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3],[2,0,2,2]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from ys_minta_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_minta",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_yspt_rptDropingPosisi";
			...
			systemAPI.alert("[app_saku3_report_yspt_flDropingPosisi]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter2 = "";
		filter=" where a.no_minta='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"'";
		nama_report="server_report_saku3_yspt_rptDropingKirim";
