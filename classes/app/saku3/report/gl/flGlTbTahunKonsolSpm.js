window.app_saku3_report_gl_flGlTbTahunKonsolSpm = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_gl_flGlTbTahunKonsolSpm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_gl_flGlTbTahunKonsolSpm";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select substring(max(periode),1,4) as periode from periode ");
...
window.app_saku3_report_gl_flGlTbTahunKonsolSpm.extend(window.childForm);
window.app_saku3_report_gl_flGlTbTahunKonsolSpm.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,
														  ...
														  new Array("kode_akun","keterangan"),"and");
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from periode  order by substring(periode,1,4) desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenTbJejer(this.kode_akun,this.kode_lokasi,this.periode);
				}
				if (this.link=="3")
				{
			...
			else
			{
				...
				this.nama_report="server_report_saku3_gl_rptGlTbTahunKonsolSpm";
	...
	doOpenTbJejer: function(kode_akun,kode_lokasi,periode){
		...
		nama_report="server_report_saku3_gl_rptGlTbJejerSpm";
	...
	doOpenTb: function(kode_akun,kode_lokasi,periode){
		...
		filter="where a.kode_lokasi='"+kode_lokasi+"' and b.akun_konsol='"+kode_akun+"' ";
		nama_report="server_report_saku3_gl_rptGlTbSpm";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
