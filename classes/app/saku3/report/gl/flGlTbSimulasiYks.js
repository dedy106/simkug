window.app_saku3_report_gl_flGlTbSimulasiYks = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_gl_flGlTbSimulasiYks.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_gl_flGlTbSimulasiYks";
		this.maximize();
	...
	this.periode_awal=this.dbLib.getPeriodeFromSQL("select min(periode) as periode from gldt_tmp where substring(periode,1,4)=substring('"+this.periode+"',1,4) and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_gl_flGlTbSimulasiYks.extend(window.childForm);
window.app_saku3_report_gl_flGlTbSimulasiYks.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser+"/"+this.sg1.getCell(2,3)+"//"+this.periode_awal;
				
				this.nama_report="server_report_saku3_gl_rptGlTbSimulasi";
				if (this.sg1.getCell(2,4)=="Tahunan")
				{
					this.nama_report="server_report_saku3_gl_rptGlTbSimulasiTahunYks";
				}
	...
	doOpenBb: function(kode_akun,kode_lokasi){
		...
		this.link="1";
		var var_periode=" and a.periode='"+this.sg1.getCell(2,1)+"'";
		...
		nama_report="server_report_saku3_gl_rptGlBukuBesarSimulasi";
	...
	doOpenBbTahun: function(kode_akun,kode_lokasi){
		this.kode_akun=kode_akun;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter2=this.sg1.getCell(2,0)+"/"+this.periode_awal+"/"+this.app._nikUser+"/"+"Tidak/1";
		filter="where a.kode_lokasi='"+kode_lokasi+"' and a.kode_akun='"+kode_akun+"' and a.periode between '"+this.periode_awal+"' and '"+this.sg1.getCell(2,1)+"'";
		nama_report="server_report_saku3_gl_rptGlBukuBesarSimulasi";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenJurnal: function(no_bukti,kode_lokasi){
		...
		filter2="";
		nama_report="server_report_saku3_gl_rptGlJurnalBuktiSimulasi";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
