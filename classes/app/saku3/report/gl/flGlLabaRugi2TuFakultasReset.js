// JavaScript Document
window.app_saku3_report_gl_flGlLabaRugi2TuFakultasReset = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_gl_flGlLabaRugi2TuFakultasReset.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_gl_flGlLabaRugi2TuFakultasReset";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Kode Fakultas","=","51"));
...
window.app_saku3_report_gl_flGlLabaRugi2TuFakultasReset.extend(window.portalui_childForm);
window.app_saku3_report_gl_flGlLabaRugi2TuFakultasReset.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			this.filterRep.ListDataSGFilter(this, "Data Fakultas",this.sg1, this.sg1.row, this.sg1.col,
													  "select kode_fakultas, nama from aka_fakultas "+
													   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
													  "select count(kode_fakultas) from aka_fakultas "+
													  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
													  ["kode_fakultas","nama"],"and",["Kode","Nama"]);
		}
	},
	doSelectCell: function(sender, col, row){
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5),new  Array("123","3","123","13","3","3"));
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.fakultas=this.dbLib.getPeriodeFromSQL("select nama as periode from aka_fakultas where kode_lokasi='"+this.sg1.getCell(2,0)+"' and kode_fakultas='"+this.sg1.getCell(2,3)+"'");
				this.nama_report="server_report_saku3_gl_rptGlLabaRugi2TuFakultasReset";
				this.filter2 = this.app._nikUser+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,3)+"/"+this.sg1.getCell(2,4)+"/"+this.fakultas+"/"+this.sg1.getCell(1,3)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,5);
	...
	doOpenTb: function(kode_neraca,kode_lokasi,periode){
		...
		filter="where a.kode_lokasi='"+kode_lokasi+"' and b.kode_neraca='"+kode_neraca+"' ";
