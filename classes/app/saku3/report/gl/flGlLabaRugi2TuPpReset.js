// JavaScript Document
window.app_saku3_report_gl_flGlLabaRugi2TuPpReset = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_gl_flGlLabaRugi2TuPpReset.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_gl_flGlLabaRugi2TuPpReset";
		this.maximize();
...
window.app_saku3_report_gl_flGlLabaRugi2TuPpReset.extend(window.portalui_childForm);
window.app_saku3_report_gl_flGlLabaRugi2TuPpReset.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			var filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and");
				...
				if (this.app._userStatus=="P" || this.app._userStatus=="U")
				{
	...
	doSelectCell: function(sender, col, row){
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5),new  Array("123","3","123","3","3","3"));
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.pp=this.dbLib.getPeriodeFromSQL("select nama as periode from pp where kode_lokasi='"+this.sg1.getCell(2,0)+"' and kode_pp='"+this.sg1.getCell(2,3)+"'");

				this.nama_report="server_report_saku3_gl_rptGlLabaRugi2TuPpReset";
				this.filter2 = this.app._nikUser+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,3)+"/"+this.sg1.getCell(2,4)+"/"+this.pp+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,5);
