// JavaScript Document
window.app_saku3_report_gl_flGlLabaRugi2TuBidangReset = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_gl_flGlLabaRugi2TuBidangReset.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_gl_flGlLabaRugi2TuBidangReset";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Kode Kendali","=",this.bidang));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Bentuk Laporan","=","Summary"));
	this.gridLib.SGEditData(this.sg1,5,[0,1,2],["Status Anggaran","=","Reseting"]);
...
window.app_saku3_report_gl_flGlLabaRugi2TuBidangReset.extend(window.portalui_childForm);
window.app_saku3_report_gl_flGlLabaRugi2TuBidangReset.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			this.filterRep.ListDataSGFilter(this, "Data Kendali",this.sg1, this.sg1.row, this.sg1.col,
	...
	doSelectCell: function(sender, col, row){
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5),new  Array("123","3","123","3","0","0"));
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5),new  Array(2,0,2,2,0,0));
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["Original","Reseting","Deviasi"]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.bidang=this.dbLib.getPeriodeFromSQL("select nama as periode from bidang where kode_lokasi='"+this.sg1.getCell(2,0)+"' and kode_bidang='"+this.sg1.getCell(2,3)+"'");
				this.nama_report="server_report_saku3_gl_rptGlLabaRugi2TuBidangReset";
				this.filter2 = this.app._nikUser+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,3)+"/"+this.sg1.getCell(2,4)+"/"+this.bidang+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,5);
	...
	doOpenTb: function(kode_neraca,kode_lokasi){
		...
		this.link="1";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser+"/"+"Tidak/1/"+this.sg1.getCell(2,3);
		filter="where a.kode_lokasi='"+kode_lokasi+"' and b.kode_neraca='"+kode_neraca+"' and e.kode_bidang='"+this.sg1.getCell(2,3)+"' and b.kode_fs='"+this.sg1.getCell(2,2)+"'";
		nama_report="server_report_saku3_gl_rptGlTbBidang";
	...
	doOpenBb: function(kode_akun,kode_lokasi,periode,kode_pp){
		...
		var periode_awal=periode.substr(0,4)+'01';
		var var_periode=" and a.periode between '"+periode_awal+"' and '"+periode+"'";
		this.link="2";
		filter2=this.sg1.getCell(2,0)+"/"+periode+"/"+this.app._nikUser+"/"+"Tidak/"+var_periode;
		filter="where a.kode_lokasi='"+kode_lokasi+"' and a.kode_akun='"+kode_akun+"' and a.kode_pp='"+kode_pp+"'";
		nama_report="server_report_saku3_gl_rptGlBukuBesarBidang";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenJurnal: function(no_bukti,kode_lokasi){
		...
		filter2="";
		nama_report="server_report_saku3_gl_rptGlJurnalBuktiBidang";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenModul: function(no_bukti,kode_lokasi){
		this.no_bukti=no_bukti;
		this.kode_lokasi=kode_lokasi;
		this.link="3";
		filter = "where a.kode_proyek='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,1)+"/"+"Layar";
		nama_report="server_report_saku3_tu_rptProyekSaldo";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
