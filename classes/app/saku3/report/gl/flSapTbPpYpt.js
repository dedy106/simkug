window.app_saku3_report_gl_flSapTbPpYpt = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_gl_flSapTbPpYpt.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_gl_flSapTbPpYpt";
		this.maximize();
	...
	this.bukti=this.dbLib.getPeriodeFromSQL("select max(no_bukti) as periode from sap_tb_m where kode_lokasi='"+this.app._lokasi+"' ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("No Bukti","=",this.bukti));
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Output Laporan","=","Layar"]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_gl_flSapTbPpYpt.extend(window.childForm);
window.app_saku3_report_gl_flSapTbPpYpt.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_bukti, keterangan from sap_tb_m  "+filter ,
														  "select count(no_bukti) from sap_tb_m "+filter,
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["123","3","3","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3],[2,0,2,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from sap_tb_m  order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Layar","Excell","Csv"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="1")
				{
					this.viewer.useIframe(this.report.previewWithHeader2(this.nama_report,this.filter, this.page,  this.pager, this.showFilter, this.lokasi,this.filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
                    this.link="";
				}
				if (this.link=="2")
				{
					this.doOpenTbJejer(this.kode_lokasi,this.periode);
				}
				if (this.link=="3")
				{
			...
			else
			{
				...
				this.filter = this.filterRep.filterStr("c.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							...
							this.filterRep.filterStr("c.no_bukti",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," and ");
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,3);
				this.nama_report="server_report_saku3_gl_rptSapTbPpYpt";
				this.viewer.setTotalPage(this.report.getTotalPage(this.nama_report,this.filter,this.pager,this.filter2));
				this.app._mainForm.reportNavigator.setTotalPage(this.viewer.getTotalPage());
				this.app._mainForm.reportNavigator.rearrange();
				if (this.sg1.getCell(2,3)=="Csv")
				{
					this.viewer.useIframe(this.report.previewWithHeader2(this.nama_report,this.filter, this.page,  this.pager, this.showFilter, this.lokasi,this.filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
                }
				else
	...
	doSelectedPage: function(sender, page){
		this.viewer.useIframe(this.report.previewWithHeader2(this.nama_report,this.filter,page,  this.pager, this.showFilter, this.lokasi,this.filter2));
		this.page = page;
		this.allBtn = false;
	},
	doCloseReportClick: function(sender){	
	    ...
	    case "xlsBtn" :
	      ...
	      break; 
		case "PreviewBtn" :
			window.open(this.report.previewWithHeader2(this.nama_report,this.filter, (this.allBtn ? 1 : this.page), (this.allBtn ? this.viewer.getTotalPage() * this.pager : this.pager), this.showFilter, this.lokasi,this.filter2));
			break;		  
		case "PrintBtn" :
	      try
	      {
			this.viewer.useIframe(this.report.previewWithHeader2(this.nama_report,this.filter, (this.allBtn ? 1 : this.page), (this.allBtn ? this.viewer.getTotalPage() * this.pager : this.pager), this.showFilter, this.lokasi,this.filter2));
	...
	doOpenTbJejer: function(kode_akun,kode_lokasi,periode){
		this.kode_akun=kode_akun;
		this.kode_lokasi=kode_lokasi;
		this.periode=periode;
		this.link="1";
		filter2=this.sg1.getCell(2,0)+"/"+periode+"/"+this.app._nikUser+"/"+"Tidak/";
		filter="where a.kode_lokasi='"+this.sg1.getCell(2,0)+"' and a.kode_akun='"+kode_akun+"'";
		nama_report="server_report_saku3_gl_rptGlTbJejerTm";
	...
	doOpenTb: function(kode_akun,kode_lokasi,periode){
		...
		this.periode=periode;
		this.link="2";
		filter2=kode_lokasi+"/"+periode+"/"+this.app._nikUser+"/"+"Tidak/";
		filter="where a.kode_lokasi='"+kode_lokasi+"' and a.kode_akun='"+kode_akun+"' ";
		nama_report="server_report_saku3_gl_rptGlTbYks";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenBb: function(kode_akun,kode_lokasi,periode){
		...
		filter="where a.kode_lokasi='"+kode_lokasi+"' and a.kode_akun='"+kode_akun+"' ";
		nama_report="server_report_saku3_gl_rptGlBukuBesarSpm";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
