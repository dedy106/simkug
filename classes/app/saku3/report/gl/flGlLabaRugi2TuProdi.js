// JavaScript Document
window.app_saku3_report_gl_flGlLabaRugi2TuProdi = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_gl_flGlLabaRugi2TuProdi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_gl_flGlLabaRugi2TuProdi";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Kode Prodi","=","01"));
...
window.app_saku3_report_gl_flGlLabaRugi2TuProdi.extend(window.portalui_childForm);
window.app_saku3_report_gl_flGlLabaRugi2TuProdi.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			this.filterRep.ListDataSGFilter(this, "Data Prodi",this.sg1, this.sg1.row, this.sg1.col,
													  "select kode_prodi, nama from prodi "+
													   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
													  "select count(kode_prodi) from prodi "+
													  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
													  ["kode_prodi","nama"],"and",["Kode","Nama"]);
		}
	},
	doSelectCell: function(sender, col, row){
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4),new  Array("123","3","123","3","3"));
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4),new  Array(2,0,2,3,0));
		}
		if (row == 1)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.pp=this.dbLib.getPeriodeFromSQL("select nama as periode from prodi where kode_lokasi='"+this.sg1.getCell(2,0)+"' and kode_prodi='"+this.sg1.getCell(2,3)+"'");

				this.nama_report="server_report_saku3_gl_rptGlLabaRugi2TuProdi";
				this.filter2 = this.app._nikUser+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,3)+"/"+this.sg1.getCell(2,4)+"/"+this.pp;
