// JavaScript Document
window.app_saku3_report_gl_flGlLabaRugiTahunPp = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_gl_flGlLabaRugiTahunPp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_gl_flGlLabaRugiTahunPp";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select substring(max(periode),1,4) as periode from periode where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_gl_flGlLabaRugiTahunPp.extend(window.portalui_childForm);
window.app_saku3_report_gl_flGlLabaRugiTahunPp.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from periode where kode_lokasi='"+this.app._lokasi+"' order by substring(periode,1,4) desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2 = this.app._nikUser+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,3)+"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(2,5);
				
				this.nama_report="server_report_saku3_gl_rptGlLabaRugiTahunPp";
	...
	doOpenTb: function(kode_neraca,kode_lokasi,periode){
		...
		filter="where a.kode_lokasi='"+kode_lokasi+"' and b.kode_neraca='"+kode_neraca+"' and a.kode_pp='"+this.sg1.getCell(2,5)+"' and a.periode='"+periode+"'";
	...
	doOpenBb: function(kode_akun,kode_lokasi,periode){
		...
		filter="where a.kode_lokasi='"+kode_lokasi+"' and a.kode_akun='"+kode_akun+"' and a.kode_pp='"+this.sg1.getCell(2,5)+"' and a.periode='"+periode+"'";
