window.app_saku3_report_gl_flGlLabaRugiSimulasi = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_gl_flGlLabaRugiSimulasi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_gl_flGlLabaRugiSimulasi";
		this.maximize();
...
window.app_saku3_report_gl_flGlLabaRugiSimulasi.extend(window.childForm);
window.app_saku3_report_gl_flGlLabaRugiSimulasi.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,3);
				this.nama_report="server_report_saku3_gl_rptGlLabaRugiSimulasi";
	...
	doOpenGar: function(kode_neraca,kode_lokasi,periode){
		...
		nama_report="server_report_saku3_gl_rptAggBulan";
	...
	doOpenTb: function(kode_neraca,kode_lokasi){
		...
		this.link="1";
		filter2=this.sg1.getCell(2,0)+"/"+this.periode+"/"+this.app._nikUser+"/"+"Tidak/1";
		filter="where a.kode_lokasi='"+kode_lokasi+"' and b.kode_neraca='"+kode_neraca+"' and b.kode_fs='"+this.sg1.getCell(2,2)+"'";
		nama_report="server_report_saku3_gl_rptGlTbSimulasi";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenBb: function(kode_akun,kode_lokasi,periode){
		...
		this.periode=periode;
		this.link="2";
		filter2=this.sg1.getCell(2,0)+"/"+periode+"/"+this.app._nikUser+"/"+"Tidak/and a.periode='"+periode+"'";
