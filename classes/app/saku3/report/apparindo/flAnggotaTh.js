window.app_saku3_report_apparindo_flAnggotaTh = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_apparindo_flAnggotaTh.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_apparindo_flAnggotaTh";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from ape_rel_properti ");
...
window.app_saku3_report_apparindo_flAnggotaTh.extend(window.childForm);
window.app_saku3_report_apparindo_flAnggotaTh.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct tahun from ape_rel_properti where kode_lokasi='"+this.app._lokasi+"' ",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_apparindo_rptAnggota";
			...
			systemAPI.alert("[app_saku3_report_apparindo_flAnggotaTh]::mainButtonClick:"+e);
		}
	},
