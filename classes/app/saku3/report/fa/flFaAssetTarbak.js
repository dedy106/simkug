window.app_saku3_report_fa_flFaAssetTarbak = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_fa_flFaAssetTarbak.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_fa_flFaAssetTarbak";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,7,[0,1,2], ["Jenis","All",""]);
	this.gridLib.SGEditData(this.sg1,8,[0,1,2], ["Versi Laporan","=","Versi 1"]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_fa_flFaAssetTarbak.extend(window.childForm);
window.app_saku3_report_fa_flFaAssetTarbak.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["3","236","236","123","123","3","123","123","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[2,0,0,2,2,0,2,0,0]);	
		if (row == 1)
		{
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct jenis from fa_asset where kode_lokasi='"+this.app._lokasi+"' ",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row === 8)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Versi 1","Versi 2"));
		}
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.jenis",this.sg1.getCell(1,7),this.sg1.getCell(2,7),this.sg1.getCell(3,7),"and");
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,5);
				this.nama_report="server_report_saku3_fa_rptFaAssetTarbak";
				if (this.sg1.getCell(2,8)=="Versi 2") {
					this.nama_report="server_report_saku3_fa_rptFaAssetTarbak2";
				}
