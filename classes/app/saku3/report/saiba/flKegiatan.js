window.app_saku3_report_saiba_flKegiatan = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_saiba_flKegiatan.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_saiba_flKegiatan";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Data Program", 2);
	...
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Departemen","All",""));
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Program","All",""));
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Fungsi","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Unit","All",""));
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_saiba_flKegiatan.extend(window.childForm);
window.app_saku3_report_saiba_flKegiatan.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			if (row ==0)
			{
				this.filterRep.ListDataSGFilter(this, "Data Departemen",this.sg1, this.sg1.row, this.sg1.col,
												"select kddept, nmdept from t_dept",
												"select count(*) from t_dept",
												new Array("kddept","nmdept"),"where",new Array("Kode","Nama"));
			}
			if (row ==1)
			{
				this.filterRep.ListDataSGFilter(this, "Data Program",this.sg1, this.sg1.row, this.sg1.col,
												"select kdprogram, nmprogram from t_program",
												"select count(*) from t_program",
												new Array("kdprogram","nmprogram"),"where",new Array("Kode","Nama"));
			}
			if (row ==2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Fungsi",this.sg1, this.sg1.row, this.sg1.col,
												"select kdfungsi, nmfungsi from t_fungsi",
												"select count(*) from t_fungsi",
												new Array("kdfungsi","nmfungsi"),"where",new Array("Kode","Nama"));
			}			
			if (row ==3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Unit",this.sg1, this.sg1.row, this.sg1.col,
												"select kdunit, nmunit from t_unit",
												"select count(*) from t_unit",
												new Array("kdunit","nmunit"),"where",new Array("Kode","Nama"));
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kdfungsi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  this.filterRep.filterStr("a.kdprogram",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							  this.filterRep.filterStr("a.kddept",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							  this.filterRep.filterStr("a.kdunit",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");

							  this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_saiba_rptKegiatan";
