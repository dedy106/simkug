window.app_saku3_report_saiba_flRealisasiBelanja = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_saiba_flRealisasiBelanja.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_saiba_flRealisasiBelanja";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Bulan","=",this.app._periode.substr(0,4)]);
	this.gridLib.SGEditData(this.sg1,1,[0,1,2],["Jenis Laporan","=","Bulanan"]);
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Pengeluaran Melalui","=","Kantor Pelayanan Perbendaharaan Negara (KPPN)"]);
...
window.app_saku3_report_saiba_flRealisasiBelanja.extend(window.portalui_childForm);
window.app_saku3_report_saiba_flRealisasiBelanja.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["Kantor Pelayanan Perbendaharaan Negara (KPPN)","Bendaharawan Umum Negara (BUN)","Gabungan Melalui KPPN dan BUN"]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.filter=this.filterRep.filterStr("x.tahun",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and")+
						this.filterRep.filterStr("x.kdperk1",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
			this.filter2=this.nik_user+"/"+this.sg1.getCell(2,1)+"/"+this.app._namaForm+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(3,0);	
			if (this.sg1.getCell(2,6)=="Kantor Pelayanan Perbendaharaan Negara (KPPN)")
			{			
				this.nama_report="server_report_saku3_saiba_rptOprTahun";
			}
			if (this.sg1.getCell(2,6)=="Bendaharawan Umum Negara (BUN)")
			{
				this.nama_report="server_report_saku3_saiba_rptOprSemester";
			}
			if (this.sg1.getCell(2,6)=="Gabungan Melalui KPPN dan BUN")
			{
				this.nama_report="server_report_saku3_saiba_rptOprBulan";
			}
