window.app_saku3_report_saiba_flLender = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_saiba_flLender.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_saiba_flLender";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Kode Lender","All",""));
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_saiba_flLender.extend(window.childForm);
window.app_saku3_report_saiba_flLender.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			if (row ==0)
			{
				this.filterRep.ListDataSGFilter(this, "Data Pinjam",this.sg1, this.sg1.row, this.sg1.col,
												"select kdlender,nmlender from t_lender",
												"select count(*) from t_lender",
												new Array("kdlender","nmlender"),"where",new Array("kode","nama"));
			}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kdlender",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where");

							  this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_saiba_rptLender";
