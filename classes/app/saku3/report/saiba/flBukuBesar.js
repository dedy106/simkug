window.app_saku3_report_saiba_flBukuBesar = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_saiba_flBukuBesar.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_saiba_flBukuBesar";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Tanggal","All",""));
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Kode Akun","All",""));
};
window.app_saku3_report_saiba_flBukuBesar.extend(window.childForm);
window.app_saku3_report_saiba_flBukuBesar.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			if (row ==0)
			{
				this.filterRep.ListDataSGFilter(this, "Data Tanggal",this.sg1, this.sg1.row, this.sg1.col,
												"select tgl_update from t_perk1",
												"select count(*) from t_perk1",
												new Array("tgl_update"),"where",new Array("tanggal"));
			}
			if (row ==1)
			{
				this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,
												"select kdperk1,nmperk1 from t_perk1",
												"select count(*) from t_perk1",
												new Array("kdperk1","nmperk1"),"where",new Array("kode","nama"));
			}
	...
	{		
		...
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,2,0,2,0,1]);	
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.tgl_update",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							this.filterRep.filterStr("a.kdperk1",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," and ");
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,2)+"/"+this.app._nikUser+"/"+this.sg1.getCell(1,1)+"/"+this.sg1.getCell(2,4);
				this.nama_report="server_report_saku3_saiba_rptBukuBesar";
