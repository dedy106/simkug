window.app_saku3_report_travel_flPiutangSaldo2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flPiutangSaldo2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flPiutangSaldo2";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from piutang_d where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_travel_flPiutangSaldo2.extend(window.childForm);
window.app_saku3_report_travel_flPiutangSaldo2.implement({
		...
		try{			
            ...
            if (row ==3)
            {
                ...
                this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,
                                                "select distinct a.akun_piutang,b.nama from piutang_d a inner join masakun b on a.akun_piutang=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+filter,
                                                "select count(distinct a.akun_piutang) from piutang_d a inner join masakun b on a.akun_piutang=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+filter,
			...
			if (row ==4)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
                            ...
                            this.filterRep.ListDataSGFilter(this, "Data Bukti",this.sg1, this.sg1.row, this.sg1.col,
												"select distinct a.no_piutang, a.keterangan from piutang_d a "+filter,
												"select count(distinct a.no_piutang) from piutang_d a "+filter,
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["123","1236","123","123","123","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,0,2,2,2,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from piutang_d where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			...
			else
			{
				...
				this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and"); 
							...
							this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.filter3+"/"+this.sg1.getCell(2,5);	
							this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_travel_rptPiutangSaldo2";
			...
			systemAPI.alert("[app_saku3_report_travel_flPiutangSaldo2]::mainButtonClick:"+e);
		}
	},
	...
	doOpenKartu: function(no_bukti,kode_lokasi){
		...
		filter2=this.app._namaUser;
		nama_report="server_report_saku3_travel_rptPiutangKartu";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
