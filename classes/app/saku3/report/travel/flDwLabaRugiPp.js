// JavaScript Document
window.app_saku3_report_travel_flDwLabaRugiPp = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flDwLabaRugiPp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flDwLabaRugiPp";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Kode PP","=",this.app._kodePP));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Nilai Laporan","=","Saldo Akhir"));
...
window.app_saku3_report_travel_flDwLabaRugiPp.extend(window.portalui_childForm);
window.app_saku3_report_travel_flDwLabaRugiPp.implement({
	...
	doSelectCell: function(sender, col, row){
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5,6),new  Array("123","3","123","3","123","3","3"));
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5,6),new  Array(2,0,2,0,2,0,0));
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5,6),new  Array("123","3","123","3","123","3","#"));
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5,6),new  Array(2,0,2,0,2,0,0));
		}
		if (row == 1)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report="server_report_saku3_travel_rptDwLabaRugiPp";
				if (this.sg1.getCell(2,5)=="Mutasi")
				{
					this.nama_report="server_report_saku3_travel_rptDwLabaRugiPpMutasi";
				}
				if (this.sg1.getCell(2,5)=="DC")
				{
					this.nama_report="server_report_saku3_travel_rptDwLabaRugiPpDc";
				}
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,3)+"/"+kode_pp+"/"+this.sg1.getCell(2,6);
	...
	doOpenTb: function(kode_lokasi,periode,kode_neraca,kode_pp){
		...
		filter2=this.sg1.getCell(2,0)+"/"+periode+"/-/Tidak/1/"+this.sg1.getCell(2,4);
		...
		nama_report="server_report_saku3_spm_rptGlTbPp";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenBb: function(kode_akun,kode_lokasi,periode,kode_pp){
		...
		nama_report="server_report_saku3_spm_rptGlBukuBesarPp";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
