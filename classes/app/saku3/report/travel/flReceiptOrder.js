window.app_saku3_report_travel_flReceiptOrder = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flReceiptOrder.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flReceiptOrder";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from mb_terima_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_travel_flReceiptOrder.extend(window.childForm);
window.app_saku3_report_travel_flReceiptOrder.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data RO",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_terima, a.keterangan from mb_terima_m a "+filter,
												"select count(a.no_terima) from mb_terima_m a "+filter,
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_travel_rptReceiptOrder";
			...
			systemAPI.alert("[app_saku3_report_travel_flReceiptOrder]::mainButtonClick:"+e);
		}
	},
