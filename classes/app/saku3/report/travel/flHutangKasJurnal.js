window.app_saku3_report_travel_flHutangKasJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flHutangKasJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flHutangKasJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and form in ('MIHUT','KBHUTMT') ");
...
window.app_saku3_report_travel_flHutangKasJurnal.extend(window.childForm);
window.app_saku3_report_travel_flHutangKasJurnal.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and form in ('MIHUT','KBHUTMT')  order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_travel_flHutangKasJurnal]::mainButtonClick:"+e);
		}
	},
