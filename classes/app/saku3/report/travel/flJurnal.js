...
//* 			SAI, PT											
//***********************************************************************************************
window.app_saku3_report_travel_flJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and form='KBDUALIN' ");
...
window.app_saku3_report_travel_flJurnal.extend(window.portalui_childForm);
window.app_saku3_report_travel_flJurnal.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		{			
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and form='KBDUALIN' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row === 5)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report="server_report_saku3_travel_rptJurnal";
