// JavaScript Document
window.app_saku3_report_travel_flKbJurnalBukti2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flKbJurnalBukti2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flKbJurnalBukti2";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and modul='KB'");
...
window.app_saku3_report_travel_flKbJurnalBukti2.extend(window.portalui_childForm);
window.app_saku3_report_travel_flKbJurnalBukti2.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and modul='KB' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.nama_report="server_report_saku3_travel_rptKbJurnalBukti2";
