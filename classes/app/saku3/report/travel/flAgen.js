window.app_saku3_report_travel_flAgen = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flAgen.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flAgen";
		this.maximize();
...
window.app_saku3_report_travel_flAgen.extend(window.childForm);
window.app_saku3_report_travel_flAgen.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==1)
			{
				this.filterRep.ListDataSGFilter(this, "Data Agen",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.no_agen, a.nama_agen from dgw_agent a  "+
														   this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(2,0)," where "),
														  "select count(a.no_agen) from dgw_agent a "+
														  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(2,0)," where "),
														  ["a.no_agen","a.nama_agen"],"and",["Kode","Nama"]);
			}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  this.filterRep.filterStr("no_agen",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_travel_rptAgen";
