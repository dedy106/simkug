window.app_saku3_report_travel_flJurnalTerima = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flJurnalTerima.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flJurnalTerima";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and form='BRGTRM' ");
...
window.app_saku3_report_travel_flJurnalTerima.extend(window.childForm);
window.app_saku3_report_travel_flJurnalTerima.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and form='BRGTRM' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_saku3_travel_rptJurnalTerima";
