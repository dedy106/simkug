window.app_saku3_report_travel_flSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from dgw_reg  ");
...
window.app_saku3_report_travel_flSaldo.extend(window.childForm);
window.app_saku3_report_travel_flSaldo.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Registrasi",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_reg, a.no_paket from dgw_reg a "+filter,
												"select count(a.no_reg) from dgw_reg a "+filter,
												new Array("a.no_reg","a.no_paket"),"and",new Array("No Bukti","No Paket"));
			}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_travel_rptSaldo";
			...
			systemAPI.alert("[app_saku3_report_travel_flSaldo]::mainButtonClick:"+e);
		}
	},
