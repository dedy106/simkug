// JavaScript Document
window.app_saku3_report_travel_flKbSaldo2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flKbSaldo2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flKbSaldo2";
		this.maximize();
...
window.app_saku3_report_travel_flKbSaldo2.extend(window.portalui_childForm);
window.app_saku3_report_travel_flKbSaldo2.implement({
	...
	doSelectCell : function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('KB','KBSPB','KBSPBPJ') order by periode desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 3)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report="server_report_saku3_travel_rptKbSaldo2";
				this.dbLib.execQuerySync(this.sql);	
	...
	doOpenBb: function(kode_akun,kode_lokasi){
		...
		filter="where a.kode_akun='"+kode_akun+"'";
		nama_report="server_report_saku3_travel_rptKbBukuBesar2";
