// JavaScript Document
window.app_saku3_report_travel_flDwLabaRugiBidang = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flDwLabaRugiBidang.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flDwLabaRugiBidang";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Kode Bidang","=",this.bidang));
...
window.app_saku3_report_travel_flDwLabaRugiBidang.extend(window.portalui_childForm);
window.app_saku3_report_travel_flDwLabaRugiBidang.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 4)
		{
			this.filterRep.ListDataSGFilter(this, "Data Bidang",this.sg1, this.sg1.row, this.sg1.col,
													  "select kode_bidang,nama from bidang where flag_aktif='1' ",
													  "select count(kode_bidang) from bidang where flag_aktif='1' ",
													  new Array("kode_bidang","nama"),"where");
		}
	},
	doSelectCell: function(sender, col, row){
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5),new  Array("123","3","123","3","123","3"));
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5),new  Array(2,0,2,0,2,0));
		
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from gldt where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList]);
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenTb(this.kode_lokasi,this.periode,this.kode_neraca,this.kode_bidang);
				}
				if (this.link=="3")
				{
			...
			else
			{
				...
				var bidang=	this.filterRep.filterStr("a.kode_bidang",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"where");
				this.nama_report="server_report_saku3_travel_rptDwLabaRugiBidang";
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,3)+"/"+bidang+"/"+this.sg1.getCell(2,5)+"/"+this.sg1.getCell(2,2);
	...
	doOpenGar: function(kode_lokasi,periode,kode_neraca,kode_bidang){
		...
		filter="where a.kode_lokasi='"+kode_lokasi+"' and e.kode_neraca='"+kode_neraca+"' and e.kode_fs='"+this.sg1.getCell(2,2)+"' and a.tahun='"+tahun+"' and c.kode_bidang='"+kode_bidang+"'";
	...
	doOpenGarSd: function(kode_lokasi,periode,kode_neraca,kode_bidang){
		this.kode_neraca=kode_neraca;
		this.kode_lokasi=kode_lokasi;
		this.periode=periode;
		this.kode_bidang=kode_bidang;
	...
	doOpenTb: function(kode_lokasi,periode,kode_neraca,kode_bidang){
		this.kode_neraca=kode_neraca;
		this.kode_lokasi=kode_lokasi;
		this.periode=periode;
		this.kode_bidang=kode_bidang;
		...
		filter2=this.sg1.getCell(2,0)+"/"+periode+"/"+kode_neraca+"/"+this.sg1.getCell(2,2);
		filter="where a.kode_lokasi='"+kode_lokasi+"' and b.kode_neraca='"+kode_neraca+"' and b.kode_fs='"+this.sg1.getCell(2,2)+"' and a.periode='"+periode+"' and d.kode_bidang='"+kode_bidang+"'";
		nama_report="server_report_saku3_dw_rptGlTbBidang";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
