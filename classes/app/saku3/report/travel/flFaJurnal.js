window.app_saku3_report_travel_flFaJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flFaJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flFaJurnal";
		this.maximize();
...
window.app_saku3_report_travel_flFaJurnal.extend(window.childForm);
window.app_saku3_report_travel_flFaJurnal.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.modul='AT' ";
				var sql1="select distinct a.no_bukti,a.keterangan from trans_m a "+filter; 
				var sql2="select count(distinct a.no_bukti) from trans_m a "+filter;
