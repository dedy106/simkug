window.app_saku3_report_travel_flStok = function (owner) {
	if (owner) {
		window.app_saku3_report_travel_flStok.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_report_travel_flStok";
		this.maximize();
	...
	var sql = "select max(a.periode) as periode " +
		"from ( " +
		"select max(periode) as periode from periode where kode_lokasi='" + this.app._lokasi + "' " +
		"union " +
		"select max(periode) as periode from brg_beli_m where kode_lokasi='" + this.app._lokasi + "' " +
		"	)a ";
...
window.app_saku3_report_travel_flStok.extend(window.childForm);
window.app_saku3_report_travel_flStok.implement({
	...
	mainButtonClick: function (sender) {
		try {
			if (sender === this.app._mainForm.bClear2) {
				...
				this.filter2 = this.sg1.getCell(2, 1) + "/" + this.sg1.getCell(2, 0) + "/" + this.app._nikUser + "/" + this.sg1.getCell(2, 3);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result = new arrayMap();
				this.nama_report = "server_report_saku3_travel_rptStok";
			...
			systemAPI.alert("[app_saku3_report_travel_flStok]::mainButtonClick:" + e);
		}
	},
	...
	doOpenBayar: function (no_bukti, kode_lokasi) {
		...
		this.link = "1";
		filter = "where a.no_sop='" + no_bukti + "' and a.kode_lokasi='" + kode_lokasi + "' ";
		filter2 = "";
