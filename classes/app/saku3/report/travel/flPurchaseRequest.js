window.app_saku3_report_travel_flPurchaseRequest = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flPurchaseRequest.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flPurchaseRequest";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from mb_pesan_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_travel_flPurchaseRequest.extend(window.childForm);
window.app_saku3_report_travel_flPurchaseRequest.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Purchase Request",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_pesan, a.keterangan from mb_pesan_m a "+filter,
												"select count(a.no_pesan) from mb_pesan_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from mb_pesan_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
						    this.filterRep.filterStr("a.no_pesan",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_travel_rptPurchaseRequest";
			...
			systemAPI.alert("[app_saku3_report_travel_flPurchaseRequest]::mainButtonClick:"+e);
		}
	},
