window.app_saku3_report_travel_flJurnalTagihan = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flJurnalTagihan.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flJurnalTagihan";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and form='BRGBA' ");
...
window.app_saku3_report_travel_flJurnalTagihan.extend(window.childForm);
window.app_saku3_report_travel_flJurnalTagihan.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  ...
														  new Array("no_bukti","keterangan"),"and");
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and form='BRGBA' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_saku3_travel_rptJurnalTagihan";
