window.app_saku3_report_travel_flFaSaldoTahun = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flFaSaldoTahun.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flFaSaldoTahun";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,3,[0,1,2], ["Periode Penyusutan","<=",this.periode]);
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_travel_flFaSaldoTahun.extend(window.childForm);
window.app_saku3_report_travel_flFaSaldoTahun.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["123","23","123","6"]);
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(a.periode,1,4) "+
									...
									"	 )a "+
									"order by substring(a.periode,1,4) desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
								this.filterRep.filterStr("g.tahun",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
								this.filterRep.filterStr("a.kode_klpakun",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,3);
				this.nama_report="server_report_saku3_travel_rptFaSaldoTahun";
	...
	doOpenAsset: function(kode_akun,kode_lokasi,tahun){
		...
		this.tahun=tahun;
		this.link="1";
		filter2 = kode_lokasi+"/"+this.sg1.getCell(2,3)+"/"+this.sg1.getCell(2,3);
		filter=" where a.kode_klpakun='"+kode_akun+"' and a.kode_lokasi='"+kode_lokasi+"' and substring(a.periode,1,4)='"+tahun+"'";
		nama_report="server_report_saku3_fa_rptFaAsset3";
