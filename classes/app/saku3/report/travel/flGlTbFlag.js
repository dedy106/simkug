window.app_saku3_report_travel_flGlTbFlag = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flGlTbFlag.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flGlTbFlag";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Flag Akun","All",""));
...
window.app_saku3_report_travel_flGlTbFlag.extend(window.childForm);
window.app_saku3_report_travel_flGlTbFlag.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Flag Akun",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_flag, nama from flag_akun  "+filter ,
														  "select count(kode_flag) from flag_akun "+filter,
														  new Array("kode_flag","nama"),"and");
			}
			if (row == 3)
			{
				var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							this.filterRep.filterStr("b.kode_flag",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," and ");
				var sql="select a.kode_akun, a.nama "+
						"from masakun a "+
						"inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+filter;
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","3","13","123","123"]);
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser+"/"+this.sg1.getCell(2,4)+"/0/"+this.sg1.getCell(2,2);
				this.nama_report="server_report_saku3_travel_rptGlTbFlag";
