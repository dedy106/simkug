window.app_saku3_report_travel_flPiutangAging = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flPiutangAging.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flPiutangAging";
		this.maximize();
...
window.app_saku3_report_travel_flPiutangAging.extend(window.childForm);
window.app_saku3_report_travel_flPiutangAging.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 3)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+ " and b.kode_flag='003' ";		
				this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,
				"select a.kode_akun, a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+filter,
				"select count(a.kode_akun) from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+filter,
			...
			if (row ==4)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_cust",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");				this.filterRep.ListDataSGFilter(this, "Data Bukti",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_bukti, a.keterangan from trans_m a "+filter,
												"select count(a.no_bukti) from trans_m a "+filter,
												new Array("a.no_bukti","a.keterangan"),"and",new Array("No Invoice","Keterangan"));
			}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123","1236","123","123","123","3","3"]);
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_piutang",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");				this.filter2 = this.sg1.getCell(2,1);
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCellDateValue(2,5)+"/"+this.sg1.getCell(2,6);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_travel_rptPiutangAging";
			...
			systemAPI.alert("[app_saku3_report_travel_flPiutangAging]::mainButtonClick:"+e);
		}
	},
