// JavaScript Document
window.app_saku3_report_travel_flBeliMutasi = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flBeliMutasi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flBeliMutasi";
		this.maximize();
...
window.app_saku3_report_travel_flBeliMutasi.extend(window.portalui_childForm);
window.app_saku3_report_travel_flBeliMutasi.implement({
	...
	doSelectCell : function(sender, col, row){
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","23","123","3","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,2,0,2]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and form='BRGBELI' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenBb(this.periode,this.kode_lokasi);
				}
			}
			else
			{
				...
				this.nama_report = "server_report_saku3_travel_rptBeliMutasi";
				this.dbLib.execQuerySync(this.sql);	
	...
	doOpenBb: function(periode,kode_lokasi){
		this.periode=periode;		
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = "where a.kode_lokasi='"+kode_lokasi+"' and a.periode='"+periode+"' and a.form='BRGBELI'";
		filter2="";		
		nama_report="server_report_saku3_produk_rptBeliRekap";
	...
	doOpenMasuk: function(kode_akun,kode_lokasi,periode,kode_pp){
		...
		filter = "where a.kode_akun='"+kode_akun+"' and a.kode_lokasi='"+kode_lokasi+"' and a.kode_pp='"+kode_pp+"' and a.periode='"+periode+"'";
		nama_report="server_report_saku3_ukm_rptKbMasukUkm";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenKeluar: function(kode_akun,kode_lokasi,periode,kode_pp){
		...
		filter2 = kode_lokasi+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser+"/"+kode_akun;
		filter = "where a.kode_akun='"+kode_akun+"' and a.kode_lokasi='"+kode_lokasi+"' and a.kode_pp='"+kode_pp+"' and a.periode='"+periode+"'";
		nama_report="server_report_saku3_ukm_rptKbKeluarUkm";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenJurnal: function(no_bukti,kode_lokasi){
		...
		filter2="";
		nama_report="server_report_saku3_ukm_rptKbJurnalBuktiUkm";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
