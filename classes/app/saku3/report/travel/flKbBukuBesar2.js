...
//* 			SAI, PT											
//***********************************************************************************************
window.app_saku3_report_travel_flKbBukuBesar2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flKbBukuBesar2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flKbBukuBesar2";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('KB','KBSPB','KBSPBPJ') ");
...
window.app_saku3_report_travel_flKbBukuBesar2.extend(window.portalui_childForm);
window.app_saku3_report_travel_flKbBukuBesar2.implement({
	...
	doSelectCell: function(sender, col, row){
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["1234","123","123","123","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,2,2,2]);
		
		if (row == 1)
		{			
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('KB','KBSPB','KBSPBPJ') order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row === 5)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser+"/"+this.sg1.getCell(2,3)+"/"+this.sg1.getCell(1,1)+"/"+this.sg1.getCell(3,1)+"/";

				
				this.nama_report="server_report_saku3_travel_rptKbBukuBesar2";
	...
	doOpenJurnal: function(no_bukti,kode_lokasi,periode){
		...
		filter2="";
		nama_report="server_report_saku3_travel_rptKbJurnalBukti2";
