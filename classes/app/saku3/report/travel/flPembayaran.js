window.app_saku3_report_travel_flPembayaran = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flPembayaran.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flPembayaran";
		this.maximize();
...
window.app_saku3_report_travel_flPembayaran.extend(window.childForm);
window.app_saku3_report_travel_flPembayaran.implement({
		...
		try{			
			...
			if (row ==1)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where");
							// "and a.form='KBPembayaran' ";
				this.filterRep.ListDataSGFilter(this, "Data Bukti",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_kwitansi, a.no_reg from dgw_pembayaran a "+filter,
												"select count(a.no_kwitansi) from dgw_pembayaran a "+filter,
												new Array("a.no_kwitansi","a.no_reg"),"and",new Array("No Invoice","Keterangan"));
			}
