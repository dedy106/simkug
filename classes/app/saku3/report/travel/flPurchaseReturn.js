window.app_saku3_report_travel_flPurchaseReturn = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_travel_flPurchaseReturn.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_travel_flPurchaseReturn";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from mb_po_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_travel_flPurchaseReturn.extend(window.childForm);
window.app_saku3_report_travel_flPurchaseReturn.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data PO",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_po, a.keterangan from mb_po_m a "+filter,
												"select count(a.no_po) from mb_po_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from mb_po_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_travel_rptPurchaseReturn";
			...
			systemAPI.alert("[app_saku3_report_travel_flPurchaseReturn]::mainButtonClick:"+e);
		}
	},
