window.app_saku3_report_ppbs_flAggUsulanPark = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ppbs_flAggUsulanPark.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ppbs_flAggUsulanPark";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from agg_usulpark_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_ppbs_flAggUsulanPark.extend(window.childForm);
window.app_saku3_report_ppbs_flAggUsulanPark.implement({
		...
		try{			
			...
			if (row == 4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_usul, keterangan from agg_usulpark_m "+filter,
														  "select count(no_usul) from agg_usulpark_m "+filter,
	...
	{		
		...
		if (row == 1)
		{
			this.dbLib.setItemsFromSQL("select distinct tahun from agg_usulpark_m a "+
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_ppbs_rptAggUsulanPark";
			...
			systemAPI.alert("[app_saku3_report_ppbs_flAggUsulanPark]::mainButtonClick:"+e);
		}
	},
