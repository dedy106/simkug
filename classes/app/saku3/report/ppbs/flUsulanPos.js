window.app_saku3_report_ppbs_flUsulanPos = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ppbs_flUsulanPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ppbs_flUsulanPos";
		this.maximize();
...
window.app_saku3_report_ppbs_flUsulanPos.extend(window.childForm);
window.app_saku3_report_ppbs_flUsulanPos.implement({
		...
		try{			
			...
			if (row == 3)
			{
				...
				var sql="select a.kode_akun, a.nama from masakun a "+
						"inner join log_akun_pp b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi  "+filter;
				var sql2="select count(a.kode_akun) from masakun a "+
						"inner join log_akun_pp b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi  "+filter;
				this.filterRep.ListDataSGFilter(this, "Daftar Akun",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_ppbs_rptUsulanPos";
			...
			systemAPI.alert("[app_saku3_report_ppbs_flUsulanPos]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_usul='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.sg1.getCell(2,1);
		nama_report="server_report_saku3_ppbs_rptUsulan";
    ...
    doOpenHis: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_usul='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.sg1.getCell(2,1);
		nama_report="server_report_saku3_ppbs_rptUsulanHis";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
