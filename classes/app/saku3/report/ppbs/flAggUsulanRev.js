window.app_saku3_report_ppbs_flAggUsulanRev = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ppbs_flAggUsulanRev.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ppbs_flAggUsulanRev";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from agg_rev_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_ppbs_flAggUsulanRev.extend(window.childForm);
window.app_saku3_report_ppbs_flAggUsulanRev.implement({
		...
		try{			
			...
			if (row == 3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_rev, keterangan from agg_rev_m "+filter,
														  "select count(no_rev) from agg_rev_m "+filter,
														  ["no_rev","keterangan"],"and",["No Bukti","Tahun"]);
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		else
		{
			...
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,3,2,0]);	
		}
		if (row == 1)
		{
			this.dbLib.setItemsFromSQL("select distinct tahun from agg_rev_m a "+
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
							  ...
							  this.filterRep.filterStr("a.no_rev",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				...
				this.nama_report = "server_report_saku3_ppbs_rptAggUsulanRev";
				if (this.sg1.getCell(2,4)=="Detail")
				{
					this.nama_report = "server_report_saku3_ppbs_rptAggUsulanRevDetail";
				}
			...
			systemAPI.alert("[app_saku3_report_ppbs_flAggUsulanRev]::mainButtonClick:"+e);
		}
	},
