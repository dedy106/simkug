window.app_saku3_report_ppbs_flProgramv2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ppbs_flProgramv2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ppbs_flProgramv2";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Tahun","=",this.periode]);
...
window.app_saku3_report_ppbs_flProgramv2.extend(window.childForm);
window.app_saku3_report_ppbs_flProgramv2.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==	2)
			{
				...
				"select count(*) from agg_program "+filter,
				new Array("kode_program","nama"),"where",new Array("kode","nama"));
	...
	{		
		...
		if (row == 3)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from agg_d where kode_lokasi='"+this.app._lokasi+"' order by substring(periode,1,4) desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("substring(a.periode,1,4)",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_ppbs_rptProgramv2";
			...
			systemAPI.alert("[app_saku3_report_ppbs_flProgramv2]::mainButtonClick:"+e);
		}
	},
