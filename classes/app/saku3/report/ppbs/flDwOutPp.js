window.app_saku3_report_ppbs_flDwOutPp = function(owner) {
    if (owner) {
        window.app_saku3_report_ppbs_flDwOutPp.prototype.parent.constructor.call(this, owner);
        this.className = "app_saku3_report_ppbs_flDwOutPp";
        this.maximize();
    ...
    this.gridLib.SGEditData(this.sg1, 3, [0, 1, 2], ["Kode PP", "=", this.pp]);
    this.gridLib.SGEditData(this.sg1, 4, [0, 1, 2], ["Jenis", "=", "Beban"]);
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Output Laporan","=","Layar"));
    this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());

};
window.app_saku3_report_ppbs_flDwOutPp.extend(window.childForm);
window.app_saku3_report_ppbs_flDwOutPp.implement({
    doEllipseClick: function(sender, col, row) {
        if (row == 0) {
            this.filterRep.ListDataSGFilter(this, "Daftar Lokasi", this.sg1, this.sg1.row, this.sg1.col,
                "select kode_lokasi, nama from lokasi  ",
                "select count(kode_lokasi) from lokasi  ", ["kode_lokasi", "nama"], "and", ["Kode", "Nama"]);
        }
        if (row == 2) {
            this.filterRep.ListDataSGFilter(this, "Daftar Akun", this.sg1, this.sg1.row, this.sg1.col,
                ...
                this.filterRep.filterStr("kode_lokasi", this.sg1.getCell(1, 0), this.sg1.getCell(2, 0), this.sg1.getCell(3, 0), "where"), ["kode_akun", "nama"], "and", ["Kode", "Nama"]);
        }
        if (row == 3) {
    ...
    doSelectCell: function(sender, col, row) {
        if (this.app._userStatus == "A") {
            this.filterRep.setSGFilterRowTipe(this.sg1, row, [0, 1, 2, 3, 4], ["123", "3", "123", "123", "13"]);
            this.filterRep.setSGFilterRowButtonStyle(this.sg1, row, [0, 1, 2, 3, 4], [2, 0, 2, 2, 0]);
        } else {
            this.filterRep.setSGFilterRowTipe(this.sg1, row, [0, 1, 2, 3, 4, 5], ["123", "3", "123", "3", "13","3"]);
            this.filterRep.setSGFilterRowButtonStyle(this.sg1, row, [0, 1, 2, 3, 4, 5], [2, 0, 2, 3, 0,0]);
        }
        if (row == 1) {
            this.sg1.columns.get(2).pickList.clear();
            this.dbLib.setItemsFromSQL("select tahun as periode from agg_tahun where kode_lokasi='" + this.app._lokasi + "'", [this.sg1.columns.get(2).pickList]);
        }
        if (row == 4) {
            this.sg1.columns.get(2).pickList.clear();
            this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList, new Array("Beban", "Pendapatan", "Investasi"));
        }
		if (row == 5)
		{
			this.sg1.columns.get(2).pickList.clear();
    ...
    mainButtonClick: function(sender) {
        try {
            ...
            if (sender == this.app._mainForm.bBack) {
                ...
                var jenis = "and b.jenis='" + this.sg1.getCell(2, 4) + "' ";
                if (this.sg1.getCell(2, 4) == "Investasi") {
                    var jenis = "and b.modul='A' ";
                }
				if (this.sg1.getCell(1, 4) == "All") {
                    var jenis = "";
                }
                this.filter = this.filterRep.filterStr("a.kode_lokasi", this.sg1.getCell(1, 0), this.sg1.getCell(2, 0), this.sg1.getCell(3, 0), "where") +
                    this.filterRep.filterStr("a.tahun_gar", this.sg1.getCell(1, 1), this.sg1.getCell(2, 1), this.sg1.getCell(3, 1), "and") +
                    this.filterRep.filterStr("a.kode_akun", this.sg1.getCell(1, 2), this.sg1.getCell(2, 2), this.sg1.getCell(3, 2), "and") +
                    this.filterRep.filterStr("a.kode_pp", this.sg1.getCell(1, 3), this.sg1.getCell(2, 3), this.sg1.getCell(3, 3), "and") + jenis;
                this.filter2 = this.sg1.getCell(2, 1) + "/" + this.sg1.getCell(2,3)+ "/" + this.sg1.getCell(2,5);
    ...
    doOpenUsulan: function(kode_akun, kode_pp, kode_drk, kode_lokasi, tahun) {
        ...
        this.link = "2";
        filter2 = tahun + "/";
        filter = "where a.kode_pp='" + kode_pp + "' and a.kode_drk='" + kode_drk + "' and a.tahun='" + tahun + "'";
