window.app_saku3_report_ppbs_flAggPk = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ppbs_flAggPk.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ppbs_flAggPk";
		this.maximize();
...
window.app_saku3_report_ppbs_flAggPk.extend(window.childForm);
window.app_saku3_report_ppbs_flAggPk.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 2)
		{
			this.filterRep.ListDataSGFilter(this, "Daftar DRK",this.sg1, this.sg1.row, this.sg1.col,
													  "select kode_pk, nama from drk where kode_lokasi = '"+this.app._lokasi+"' ",
													  "select count(kode_pk) from drk where kode_lokasi = '"+this.app._lokasi+"'  ",
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
							  ...
							  this.filterRep.filterStr("c.kode_pk",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				...
				{
					var sql =  "select a.kode_pk,f.nama as nama_akun,zz.kode_lokasi,zz.nama as nama_lokasi, "+
							...
							"from (select a.kode_lokasi,a.tahun,c.kode_pk,a.progress "+	
							"	  from agg_d a "+
							"	inner join agg_drk c on a.kode_drk=c.kode_drk and a.tahun=c.tahun "+
							"	inner join agg_pk b on c.kode_pk=b.kode_pk and c.tahun=b.tahun "+this.filter+
							"group by a.kode_lokasi,a.tahun,c.kode_pk,a.progress ) a  "+
							"inner join agg_pk c on a.kode_pk=c.kode_pk  "+
							"inner join lokasi zz on a.kode_lokasi=zz.kode_lokasi "+
							"left join ( "+
							"	 select a.kode_lokasi,c.kode_pk, "+
							...
							this.filter+
							"	 group by a.kode_lokasi,c.kode_pk "+
							"		 )g on a.kode_lokasi=g.kode_lokasi and a.kode_pk=g.kode_pk "+this.filter+
							"group by c.kode_pk,f.nama,zz.kode_lokasi,zz.nama order by c.kode_pk";	
					this.scriptSqlCount = "select count(a.kode_pk) "+
							...
							"		 )g on a.kode_lokasi=g.kode_lokasi and a.kode_pk=g.kode_pk "+this.filter;
					this.title = new server_util_arrayList({items:["Kode PK","Nama PK","Lok","Nama Lokasi","Januari","Februari","Maret","April","Mei","Juni","Juli","Agustus","September","Oktober","November","Desember","Total"]});
				...
				if (this.sg1.getCell(2,3)=="Triwulan")
				{
					var sql =  "select a.kode_pk,f.nama as nama_akun,zz.kode_lokasi,zz.nama as nama_lokasi, "+
							...
							"	 group by a.kode_lokasi,a.kode_pk "+
							"		 )g on a.kode_lokasi=g.kode_lokasi and a.kode_pk=g.kode_pk "+this.filter+
							"group by a.kode_pk,f.nama,zz.kode_lokasi,zz.nama order by a.kode_pk";	
					this.scriptSqlCount = "select count(a.kode_pk) "+
							"from (select a.kode_lokasi,a.tahun,a.kode_pk,a.progress "+	
							"	  from agg_d a "+this.filter+
							"group by a.kode_lokasi,a.tahun,a.kode_pk,a.progress ) a  "+
							...
							"	 group by a.kode_lokasi,a.kode_pk "+
							"		 )g on a.kode_lokasi=g.kode_lokasi and a.kode_pk=g.kode_pk "+this.filter;
					this.title = new server_util_arrayList({items:["Kode PK","Nama PK","Lok","Nama Lokasi","Triwulan I","Triwulan II","Triwulan III","Triwulan IV","Total"]});
			...
			systemAPI.alert("[app_saku3_report_ppbs_flAggPk]::mainButtonClick:"+e);
		}
	},
