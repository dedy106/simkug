// JavaScript Document
window.app_saku3_report_ppbs_flDwRkapInvOutPp = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ppbs_flDwRkapInvOutPp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ppbs_flDwRkapInvOutPp";
		this.maximize();
		...
		this.sg1 = new saiGrid(this.p1,{bound:[1,20,700,200],colCount:4,cellExit:[this,"doCellExit"],
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from agg_tahun where kode_lokasi='"+this.app._lokasi+"' ");
	...
	this.gridLib.SGEditData(this.sg1,5,[0,1,2],["Kode PP","=",this.pp]);
	this.gridLib.SGEditData(this.sg1,6,[0,1,2],["Output Laporan","=","Layar"]);
...
window.app_saku3_report_ppbs_flDwRkapInvOutPp.extend(window.portalui_childForm);
window.app_saku3_report_ppbs_flDwRkapInvOutPp.implement({
		...
		if (row ==5)
		{
			...
			if (this.app._userStatus=="A")
			{
				this.filterRep.ListDataSGFilter(this, "Data PP",this.sg1, this.sg1.row, this.sg1.col,
													  "select a.kode_pp, a.nama from agg_pp a "+
													  "inner join agg_bidang b on a.kode_bidang=b.kode_bidang and a.kode_lokasi=b.kode_lokasi and a.tahun=b.tahun "+filter,
													  "select count(*) from agg_pp a "+
													  "inner join agg_bidang b on a.kode_bidang=b.kode_bidang and a.kode_lokasi=b.kode_lokasi and a.tahun=b.tahun "+filter,
	...
	doSelectCell: function(sender, col, row){
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5,6),new  Array("123","123","123","3","3","123","3"));
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5,6),new  Array(2,0,2,0,0,2,0));
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5,6),new  Array("123","123","123","3","3","3","3"));
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5,6),new  Array(2,0,2,0,0,3,0));
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select tahun as periode from agg_tahun where kode_lokasi='"+this.app._lokasi+"' order by tahun desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="1")
				{
					...
					this.nama_report = "server_report_saku3_ppbs_rptDwRkapInvOutPp";
			...
			else
			{
				...
				this.filter2 = this.nik_user+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(2,3)+"/"+this.sg1.getCell(2,2)+"/"+this.kode_lokasi+"/"+this.sg1.getCell(2,6);
	...
	doOpenUsulan: function(kode_akun,kode_pp,kode_drk,kode_lokasi,tahun){
		...
		if (kode_lokasi=="11") {
			...
			filter="where a.kode_pp='"+kode_pp+"' and a.kode_drk='"+kode_drk+"' and a.tahun='"+tahun+"' and a.kode_pp='"+this.sg1.getCell(2,5)+"'";
			this.nama_report="server_report_saku3_ppbs_rptAggUsulanYpt";
		}
