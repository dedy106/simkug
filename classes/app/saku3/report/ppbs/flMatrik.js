window.app_saku3_report_ppbs_flMatrik = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ppbs_flMatrik.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ppbs_flMatrik";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode Program","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Kode RKM","All",""));
...
window.app_saku3_report_ppbs_flMatrik.extend(window.childForm);
window.app_saku3_report_ppbs_flMatrik.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==	2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Program",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_program, nama from agg_program "+filter,
														  "select count(kode_program) from agg_program "+filter,
														  new Array("kode_program","nama"),"and");
			}
			if (row ==	3)
			{
				var filter=this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
						this.filterRep.filterStr("tahun",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," and ")+
						this.filterRep.filterStr("kode_program",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," and ");
				this.filterRep.ListDataSGFilter(this, "Data RKM",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_rkm, nama from agg_rkm "+filter,
														  "select count(kode_rkm) from agg_rkm "+filter,
														  new Array("kode_rkm","nama"),"and");
			}
			if (row ==	4)
			{
				var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
						...
						this.filterRep.filterStr("b.kode_program",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," and ")+
						this.filterRep.filterStr("b.kode_rkm",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3)," and ");
				var sql="select a.kode_drk,a.nama "+
						"from agg_drk a "+
						"inner join agg_rkm b on a.kode_rkm=b.kode_rkm and a.kode_lokasi=b.kode_lokasi and a.tahun=b.tahun "+filter;
				var sql2="select count(a.kode_drk) "+
						"from agg_drk a "+
						"inner join agg_rkm b on a.kode_rkm=b.kode_rkm and a.kode_lokasi=b.kode_lokasi and a.tahun=b.tahun "+filter;
				this.filterRep.ListDataSGFilter(this, "Data DRK",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,new Array("a.kode_drk","a.nama"),"and");
			}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("c.kode_program",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," and ")+
							this.filterRep.filterStr("c.kode_rkm",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3)," and ")+
							this.filterRep.filterStr("a.kode_drk",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
							
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,5)+"/"+this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_ppbs_rptMatrik";
			...
			systemAPI.alert("[app_saku3_report_ppbs_flMatrik]::mainButtonClick:"+e);
		}
	},
