window.app_saku3_report_ppbs_flPpKuota = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ppbs_flPpKuota.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ppbs_flPpKuota";
		this.maximize();
...
window.app_saku3_report_ppbs_flPpKuota.extend(window.childForm);
window.app_saku3_report_ppbs_flPpKuota.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==	2)
			{
				...
				"select distinct a.kode_pp, a.nama from agg_pp a inner join agg_user b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
				this.filterRep.filterStr("a.tahun",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," where ")+
				" and b.nik='"+this.app._userLog+"' ",
				"select count(a.kode_pp) from agg_pp a  inner join agg_user b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
				this.filterRep.filterStr("a.tahun",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," where ")+
				" and b.nik='"+this.app._userLog+"' ",
				new Array("a.kode_pp","a.nama"),"where",new Array("kode","nama"));
			}
	...
	{		
		...
		if (row == 1)
		{
			this.dbLib.setItemsFromSQL("select distinct tahun from agg_tahun a "+
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and d.nik='"+this.app._userLog+"' ";

				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,2);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_ppbs_rptPpKuota";
			...
			systemAPI.alert("[app_saku3_report_ppbs_flPpKuota]::mainButtonClick:"+e);
		}
	},
