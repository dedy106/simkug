window.app_saku3_report_ppbs_flParam = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ppbs_flParam.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ppbs_flParam";
		this.maximize();
...
window.app_saku3_report_ppbs_flParam.extend(window.childForm);
window.app_saku3_report_ppbs_flParam.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==	2)
			{
				var sql="select a.kode_pp, a.nama from agg_pp a "+
				...
				var sql2="select count(a.kode_pp) from agg_pp a "+
					this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
					this.filterRep.filterStr("a.tahun",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct tahun from agg_tahun where kode_lokasi='"+this.app._lokasi+"' order by tahun desc",[this.sg1.columns.get(2).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_ppbs_rptParam";
			...
			systemAPI.alert("[app_saku3_report_ppbs_flParam]::mainButtonClick:"+e);
		}
	},
