// JavaScript Document
window.app_saku3_report_ppbs_flDwRkapInvBidang = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ppbs_flDwRkapInvBidang.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ppbs_flDwRkapInvBidang";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from agg_d where kode_lokasi='"+this.app._lokasi+"' ");
	this.pp=this.dbLib.getPeriodeFromSQL("select b.kode_bidang as periode from agg_user a "+
	...
	this.fs=this.dbLib.getPeriodeFromSQL("select kode_fs as periode from agg_fs_lokasi where kode_lokasi='"+this.app._lokasi+"' ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode FS","=","FS3"));
	...
	this.gridLib.SGEditData(this.sg1,5,[0,1,2],["Kode Rektor","All",""]);
	this.gridLib.SGEditData(this.sg1,6,[0,1,2],["Kode Bidang","=",this.pp]);
...
window.app_saku3_report_ppbs_flDwRkapInvBidang.extend(window.portalui_childForm);
window.app_saku3_report_ppbs_flDwRkapInvBidang.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 5)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Rektorat",this.sg1, this.sg1.row, this.sg1.col,
													  "select kode_rektor,nama from agg_rektor "+filter,
													  "select count(kode_rektor) from agg_rektor "+filter,
													  new Array("kode_rektor","nama"),"where");
		}
		if (row ==6)
		{
			var filter= this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
						this.filterRep.filterStr("tahun",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," and ")+
						this.filterRep.filterStr("kode_rektor",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5)," and ");
			this.filterRep.ListDataSGFilter(this, "Data Bidang",this.sg1, this.sg1.row, this.sg1.col,
													  "select kode_bidang,nama from agg_bidang "+filter,
													  "select count(kode_bidang) from agg_bidang "+filter,
													  new Array("kode_bidang","nama"),"where");
		}
	},
	doSelectCell: function(sender, col, row){
		this.sg1.columns.get(2).setReadOnly(false);
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5,6),new  Array("123","123","123","3","3","123","123"));
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5,6),new  Array(2,0,2,0,0,2,2));
		...
		try
		{
			...
			else
			{
				...
				this.filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.kode_rektor",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+
							this.filterRep.filterStr("a.kode_bidang",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and")+
							this.filterRep.filterStr("a.tahun",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				this.filter2 = this.nik_user+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(2,3)+"/"+this.sg1.getCell(2,2)+"/"+this.kode_lokasi+"/"+this.sg1.getCell(2,5);
				
				this.nama_report="server_report_saku3_ppbs_rptDwRkapInvBidang";
	...
	doOpenTb: function(kode_neraca,kode_lokasi){
		...
		filter=" where b.kode_neraca='"+kode_neraca+"' and b.kode_lokasi='"+kode_lokasi+"' and substring(a.periode,1,4)='"+this.sg1.getCell(2,1)+"' and b.kode_fs='"+this.sg1.getCell(2,2)+"' and a.kode_pp='"+this.sg1.getCell(2,5)+"'";
		this.nama_report="server_report_saku3_ppbs_rptAggAkunTu";
	...
	doOpenTbDrk: function(kode_akun,kode_lokasi){
		...
		filter=" where a.kode_akun='"+kode_akun+"' and a.kode_lokasi='"+kode_lokasi+"' and substring(a.periode,1,4)='"+this.sg1.getCell(2,1)+"' and a.kode_pp='"+this.sg1.getCell(2,5)+"'";
		this.nama_report="server_report_saku3_ppbs_rptAggAkunDrkTu";
		this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenUsulan: function(kode_akun,kode_pp,kode_drk,kode_lokasi,tahun){
		this.link="3";
		this.kode_akun=kode_akun;
		...
		this.link="2";
		filter2 = tahun+"/";
		filter="where a.kode_akun='"+kode_akun+"' and a.kode_pp='"+kode_pp+"' and a.kode_drk='"+kode_drk+"' and a.tahun='"+tahun+"' and a.kode_pp='"+this.sg1.getCell(2,5)+"'";
		this.nama_report="server_report_saku3_ppbs_rptAggUsulanRincian";
		this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
