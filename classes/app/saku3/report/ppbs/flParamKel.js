window.app_saku3_report_ppbs_flParamKel = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ppbs_flParamKel.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ppbs_flParamKel";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from agg_param_klp where kode_lokasi='"+this.app._lokasi+"' ");
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Kode Parameter","All",""));
...
window.app_saku3_report_ppbs_flParamKel.extend(window.childForm);
window.app_saku3_report_ppbs_flParamKel.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==	1)
			{
				this.filterRep.ListDataSGFilter(this, "Data Parameter",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_param, nama from agg_param_klp "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(kode_param) from agg_param_klp "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  new Array("kode_param","nama"),"and");
			}
	...
	{		
		...
		if (row == 2)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct tahun from agg_param_klp "+
									   "where kode_lokasi='"+this.app._lokasi+"' order by tahun desc",[this.sg1.columns.get(2).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.kode_param",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.tahun",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,2);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_ppbs_rptParamKel";
			...
			systemAPI.alert("[app_saku3_report_ppbs_flParamKel]::mainButtonClick:"+e);
		}
	},
