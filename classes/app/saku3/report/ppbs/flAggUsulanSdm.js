window.app_saku3_report_ppbs_flAggUsulanSdm = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_ppbs_flAggUsulanSdm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_ppbs_flAggUsulanSdm";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from agg_gaji_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_ppbs_flAggUsulanSdm.extend(window.childForm);
window.app_saku3_report_ppbs_flAggUsulanSdm.implement({
			...
			if (row == 2)
		{
				...
				if (this.app._userStatus=="P")
				{
					var sql="select a.kode_pp, a.nama "+
						...
						"where a.flag_aktif='1' and c.nik='"+this.app._userLog+"' "+filter;
					var sql2="select count(a.kode_pp) "+
						"from agg_pp a "+
						"inner join (select a.kode_pp,a.kode_lokasi from agg_d a "+
						"			where substring(a.periode,1,4)='"+tahun+"' "+filter+
			...
			if (row == 3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Daftar Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_gaji, keterangan from agg_gaji_m "+filter,
														  "select count(no_gaji) from agg_gaji_m "+filter,
														  ["no_gaji","keterangan"],"and",["No Bukti","Tahun"]);
			}			
		}catch(e){
			systemAPI.alert(e);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
							  ...
							  this.filterRep.filterStr("a.no_gaji",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.lokasi=this.dbLib.getPeriodeFromSQL("select nama as periode from lokasi where kode_lokasi='"+this.sg1.getCell(2,0)+"'");
				this.filter2 = this.sg1.getCell(2,4);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_ppbs_rptAggUsulanSdm";
			...
			systemAPI.alert("[app_saku3_report_ppbs_flAggUsulanSdm]::mainButtonClick:"+e);
		}
	},
