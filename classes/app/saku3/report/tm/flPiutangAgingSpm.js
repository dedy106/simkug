window.app_saku3_report_tm_flPiutangAgingSpm = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tm_flPiutangAgingSpm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tm_flPiutangAgingSpm";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from piutang_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_tm_flPiutangAgingSpm.extend(window.childForm);
window.app_saku3_report_tm_flPiutangAgingSpm.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["123","1236","123","123","123","123","3","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[2,0,2,2,2,2,1,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from piutang_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCellDateValue(2,6)+"/"+this.sg1.getCell(2,7);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_tm_rptPiutangAgingSpm";
			...
			systemAPI.alert("[app_saku3_report_tm_flPiutangAgingSpm]::mainButtonClick:"+e);
		}
	},
