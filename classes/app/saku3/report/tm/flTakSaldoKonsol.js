window.app_saku3_report_tm_flTakSaldoKonsol = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tm_flTakSaldoKonsol.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tm_flTakSaldoKonsol";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from takkirim_m ");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Status","=","Outstanding"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	this.doSelectCell(this.sg1,2,3);
};
window.app_saku3_report_tm_flTakSaldoKonsol.extend(window.childForm);
window.app_saku3_report_tm_flTakSaldoKonsol.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data TAK Kirim",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_kirim, a.keterangan from takkirim_m a "+this.filter,
												"select count(*) from takkirim_m a "+this.filter,
												new Array("a.no_kirim","a.keterangan"),"and",new Array("No Kirim","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from takkirim_m order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Outstanding","Selesai","Semua"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_tm_rptTakSaldoKonsol";
			...
			systemAPI.alert("[app_saku3_report_tm_flTakSaldoKonsol]::mainButtonClick:"+e);
		}
	},
	...
	doOpenKirim: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_tm_rptTakKirim";
	...
	doOpenTerima: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_tm_rptTakTerima";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
