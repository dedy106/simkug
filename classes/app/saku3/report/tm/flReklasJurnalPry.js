window.app_saku3_report_tm_flReklasJurnalPry = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tm_flReklasJurnalPry.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tm_flReklasJurnalPry";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from spm_proyek_reklas_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_tm_flReklasJurnalPry.extend(window.childForm);
window.app_saku3_report_tm_flReklasJurnalPry.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Bukti",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_reklas, a.keterangan from spm_proyek_reklas_m a "+filter,
												"select count(b.no_reklas) from spm_proyek_reklas_m a "+filter,
												new Array("a.no_reklas","a.keterangan"),"and",new Array("No Reklas","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from spm_proyek_reklas_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_reklas",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_tm_rptReklasJurnalPry";
			...
			systemAPI.alert("[app_saku3_report_tm_flReklasJurnalPry]::mainButtonClick:"+e);
		}
	},
