window.app_saku3_report_tm_flPiutangSaldoAppar = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tm_flPiutangSaldoAppar.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tm_flPiutangSaldoAppar";
		this.maximize();
...
window.app_saku3_report_tm_flPiutangSaldoAppar.extend(window.childForm);
window.app_saku3_report_tm_flPiutangSaldoAppar.implement({
		...
		try{			
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Customer",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_cust, nama from cust2 "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(kode_cust) from cust2 "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
            ...
            if (row ==3)
            {
                ...
                this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,
                                                "select distinct a.akun_piutang,b.nama from piutang_m a inner join masakun b on a.akun_piutang=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+filter,
                                                "select count(distinct a.akun_piutang) from piutang_m a inner join masakun b on a.akun_piutang=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+filter,
                                                new Array("a.akun_piutang","a.nama"),"and",new Array("Kode Akun","Nama"));
            }
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				{
					periode=this.sg1.getCell(3,1);
				}
				this.filter2 = this.sg1.getCell(2,0)+"/"+periode;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_tm_rptPiutangSaldoAppar";
			...
			systemAPI.alert("[app_saku3_report_tm_flPiutangSaldoAppar]::mainButtonClick:"+e);
		}
	},
	...
	doOpenKartu: function(no_bukti,kode_lokasi){
		...
		filter = "where a.no_piutang='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
		nama_report="server_report_saku3_tm_rptPiutangKartu";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
