window.app_saku3_report_tm_flDropingTerima = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tm_flDropingTerima.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tm_flDropingTerima";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBDROPTRM' ");
...
window.app_saku3_report_tm_flDropingTerima.extend(window.childForm);
window.app_saku3_report_tm_flDropingTerima.implement({
	...
	{		
		...
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,2]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBDROPTRM' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_tm_rptDropingTerima";
			...
			systemAPI.alert("[app_saku3_report_tm_flDropingTerima]::mainButtonClick:"+e);
		}
	},
