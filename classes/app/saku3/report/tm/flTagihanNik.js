window.app_saku3_report_tm_flTagihanNik = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tm_flTagihanNik.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tm_flTagihanNik";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_batm_m where kode_lokasi='"+this.app._lokasi+"'");
	this.nb=this.dbLib.getPeriodeFromSQL("select max(no_selesai) as periode from yk_batm_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_tm_flTagihanNik.extend(window.childForm);
window.app_saku3_report_tm_flTagihanNik.implement({
		...
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Approve Hutang",this.sg1, this.sg1.row, this.sg1.col,
												"select no_selesai,keterangan from yk_batm_m "+this.filter,
												"select count(no_selesai) from yk_batm_m "+this.filter,
												new Array("no_selesai","keterangan"),"and",new Array("No Approve","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct a.periode from yk_batm_m a "+
		...
		try
		{
			...
			}else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_selesai",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");		
				...
				this.nama_report="server_report_saku3_tm_rptTagihanNik";
