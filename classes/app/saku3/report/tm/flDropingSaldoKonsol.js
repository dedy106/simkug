window.app_saku3_report_tm_flDropingSaldoKonsol = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tm_flDropingSaldoKonsol.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tm_flDropingSaldoKonsol";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where modul='KBDROP' ");
...
window.app_saku3_report_tm_flDropingSaldoKonsol.extend(window.childForm);
window.app_saku3_report_tm_flDropingSaldoKonsol.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where  modul='KBDROP' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_tm_rptDropingSaldoKonsol";
			...
			systemAPI.alert("[app_saku3_report_tm_flDropingSaldoKonsol]::mainButtonClick:"+e);
		}
	},
	...
	doOpenKirim: function(no_bukti,kode_lokasi){
		...
		filter2 = "";
		filter=" where a.no_kas='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"'";
		nama_report="server_report_saku3_tm_rptDropingKirim";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenTerima: function(no_bukti,kode_lokasi){
		...
		filter2 = "";
		filter=" where a.no_kas='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"'";
		nama_report="server_report_saku3_tm_rptDropingTerima";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
