window.app_saku3_report_tm_flDropingSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tm_flDropingSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tm_flDropingSaldo";
		this.maximize();
...
window.app_saku3_report_tm_flDropingSaldo.extend(window.childForm);
window.app_saku3_report_tm_flDropingSaldo.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBDROP' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_tm_rptDropingSaldo";
			...
			systemAPI.alert("[app_saku3_report_tm_flDropingSaldo]::mainButtonClick:"+e);
		}
	},
