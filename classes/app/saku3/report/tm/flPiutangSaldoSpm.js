window.app_saku3_report_tm_flPiutangSaldoSpm = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tm_flPiutangSaldoSpm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tm_flPiutangSaldoSpm";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from piubayar_d where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_tm_flPiutangSaldoSpm.extend(window.childForm);
window.app_saku3_report_tm_flPiutangSaldoSpm.implement({
		...
		try{			
			...
			if (row ==4)
			{
				this.filterRep.ListDataSGFilter(this, "KOde PP",this.sg1, this.sg1.row, this.sg1.col,
												"select distinct a.kode_pp , a.nama from pp a inner join piutang_m b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi where  a.kode_lokasi='01'",
												"select  distinct a.kode_pp , a.nama  from pp a inner join piutang_m b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi where  a.kode_lokasi='01'",
			...
			if (row ==5)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_piutang, a.keterangan from piutang_m a "+filter,
												"select count(a.no_piutang) from piutang_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from piubayar_d where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 6)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_tm_rptPiutangSaldoSpm";
			...
			systemAPI.alert("[app_saku3_report_tm_flPiutangSaldoSpm]::mainButtonClick:"+e);
		}
	},
	...
	doOpenKas: function(no_kas,kode_lokasi){
		...
		filter2=this.app._namaUser;
		nama_report="server_report_saku3_tm_rptPiutangKasJurnal";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
