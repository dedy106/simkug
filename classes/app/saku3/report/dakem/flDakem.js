window.app_saku3_report_dakem_flDakem = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dakem_flDakem.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dakem_flDakem";
		this.maximize();
	...
	if (this.app._lokasi=="99") this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_dakem_m where kode_lokasi<>'00'");
	else this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_dakem_m where kode_lokasi='"+this.app._lokasi+"'");
	
	if (this.app._lokasi == "99") this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","All",""));
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Status Bayar","All",""));
...
window.app_saku3_report_dakem_flDakem.extend(window.childForm);
window.app_saku3_report_dakem_flDakem.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Penempatan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_dakem, a.keterangan from yk_dakem_m a "+filter,
												"select count(a.no_dakem) from yk_dakem_m a "+filter,
												new Array("a.no_dakem","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		if (this.app._lokasi == "99") {
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","1236","123","123","13"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,2,1,0]);	
		}
		else {
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["3","1236","123","123","1,3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,2,1,0]);	
		}
		if (row == 1)
		{
			...
			if (this.app._lokasi == "99") 
				this.dbLib.setItemsFromSQL("select distinct periode from yk_dakem_m where kode_lokasi<>'99' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);			
			else this.dbLib.setItemsFromSQL("select distinct periode from yk_dakem_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				if (this.sg1.getCell(1,4) == "All") var filterBayar = " ";
				else {
					if (this.sg1.getCell(2,4) == "BAYAR") var filterBayar = " and c.no_kas <>'-' ";
					else var filterBayar = " and (c.no_kas is null or c.no_kas ='-' ) ";
				}

				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_dakem",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and") + filterBayar;
				this.filter2 = this.sg1.getCell(2,1)+";"+this.sg1.getCell(1,3)+";"+this.sg1.getCellDateValue(2,3)+";"+this.sg1.getCellDateValue(3,3);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_dakem_rptDakem";
			...
			systemAPI.alert("[app_saku3_report_dakem_flDakem]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter2 = "";
		filter=" where a.no_dakem='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"'";
		nama_report="server_report_saku2_dakem_rptDakemForm";
