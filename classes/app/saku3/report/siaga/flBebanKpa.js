window.app_saku3_report_siaga_flBebanKpa = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_flBebanKpa.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_flBebanKpa";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_app_m ");
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Jenis Laporan","=","Daftar"));
...
window.app_saku3_report_siaga_flBebanKpa.extend(window.childForm);
window.app_saku3_report_siaga_flBebanKpa.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_app, a.catatan from gr_app_m a "+this.filter,
												"select count(*) from gr_app_m a "+this.filter,
												new Array("a.no_app","a.catatan"),"and",new Array("No Bukti","Keterangan"));
			}
			if (row ==3)
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")
				this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,
												"select a.kode_akun, a.nama from masakun a "+this.filter,
												"select count(*) from masakun a "+this.filter,
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["123","123","123","123","3","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,0,2,2,0,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from gr_app_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Daftar","Detail"));
		}
		if (row == 5)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_app",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							" and a.form='APPBUDGET' ";
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(3,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				if (this.sg1.getCell(2,4)=="Daftar")
				{
					this.nama_report="server_report_saku3_siaga_rptDaftarKpa";
				}
				else
				{
			...
			systemAPI.alert("[app_saku3_report_siaga_flBebanKpa]::mainButtonClick:"+e);
		}
	},
	...
	doOpenKpa: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_app='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		var periode2=" and b.periode='"+this.sg1.getCell(2,1)+"'";		
		filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser+"/"+periode2;
		nama_report="server_report_saku3_siaga_rptBebanKpa";
