window.app_saku3_report_siaga_flBebanAjuEmail = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_flBebanAjuEmail.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_flBebanAjuEmail";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Email","=",""));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("No Pesan","=",""));
...
window.app_saku3_report_siaga_flBebanAjuEmail.extend(window.childForm);
window.app_saku3_report_siaga_flBebanAjuEmail.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select*from gr_pb_m a "+
                                                "inner join apv_flow h on a.no_pb=h.no_bukti and a.kode_lokasi=h.kode_lokasi  and h.no_urut=1 "+this.filter,
												"select count(*) from gr_pb_m a "+
                                                "inner join apv_flow h on a.no_pb=h.no_bukti and a.kode_lokasi=h.kode_lokasi  and h.no_urut=1 "+this.filter,
	...
	{		
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["123","123","3","3","3","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,0,2,2,2,2]);
		}	
		if (row == 1)
		{
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Justifikasi","Kebutuhan"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter2 = this.app._lokasi + "/" + this.app._periode + "/" + this.app._kodePP + "/" + this.app._userLog + "/" + this.app._userPwd + "/" + this.sg1.getCell(2,3) +"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(2,5);
			...
			systemAPI.alert("[app_saku3_report_siaga_flBebanAjuEmail]::mainButtonClick:"+e);
		}
	},
