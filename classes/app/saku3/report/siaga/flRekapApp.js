window.app_saku3_report_siaga_flRekapApp = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_flRekapApp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_flRekapApp";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_pb_m ");
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("NIK Dir Unit","=",""));
...
window.app_saku3_report_siaga_flRekapApp.extend(window.childForm);
window.app_saku3_report_siaga_flRekapApp.implement({
		...
		try{			
			...
			if (row ==3)
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.modul<>'BMHD' ";
				this.filterRep.ListDataSGFilter(this, "Data Penerimaan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_pb, a.keterangan from gr_pb_m a "+this.filter,
												"select count(*) from gr_pb_m a "+this.filter,
			...
			if (row ==4)
			{
				...
				this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
				" and a.modul<>'BMHD' ";
				this.filterRep.ListDataSGFilter(this, "Data NIK Dir",this.sg1, this.sg1.row, this.sg1.col,
												"select distinct a.nik_dir, b.nama from gr_pb_m a left join karyawan b on a.nik_dir=b.nik and a.kode_lokasi=b.kode_lokasi "+this.filter,
												"select count(distinct a.nik_dir) from gr_pb_m a left join karyawan b on a.nik_dir=b.nik and a.kode_lokasi=b.kode_lokasi "+this.filter,
												new Array("a.nik_dir","b.nama"),"and",new Array("NIK","Nama"));
			}
	...
	{		
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["123","123","3","123","123","13"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,0,2,2,2,0]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from gr_pb_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 5)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_pb",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.nik_dir",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							" and a.modul<>'BMHD' ";
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,5);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_siaga_rptRekapApp";
			...
			systemAPI.alert("[app_saku3_report_siaga_flRekapApp]::mainButtonClick:"+e);
		}
	},
	...
	doOpenAju: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_siaga_rptBebanJust";
