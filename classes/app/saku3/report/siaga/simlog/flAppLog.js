window.app_saku3_report_siaga_simlog_flAppLog = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_simlog_flAppLog.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_simlog_flAppLog";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from app_m where kode_lokasi='"+this.app._lokasi+"' and modul='LOGREQ_TRM'");
...
window.app_saku3_report_siaga_simlog_flAppLog.extend(window.childForm);
window.app_saku3_report_siaga_simlog_flAppLog.implement({
		...
		try{			
			...
			if (row ==2)
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							"and a.modul='LOGREQ_TRM' ";
				this.filterRep.ListDataSGFilter(this, "Data App",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_app, a.status from app_m a "+this.filter,
												"select count(*) from app_m a "+this.filter,
												new Array("a.no_app","a.status"),"and",new Array("No Bukti","Status"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from app_m where kode_lokasi='"+this.app._lokasi+"'  and modul='LOGREQ_TRM' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_siaga_simlog_flAppLog]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.app_m='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
