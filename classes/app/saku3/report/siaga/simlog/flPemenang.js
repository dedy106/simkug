window.app_saku3_report_siaga_simlog_flPemenang = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_simlog_flPemenang.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_simlog_flPemenang";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from log_tap_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_siaga_simlog_flPemenang.extend(window.childForm);
window.app_saku3_report_siaga_simlog_flPemenang.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Penetapan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_tap, a.keterangan from log_tap_m a "+this.filter,
												"select count(*) from log_tap_m a "+this.filter,
												new Array("a.no_tap","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from log_tap_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_tap",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
			...
			systemAPI.alert("[app_saku3_report_siaga_simlog_flPemenang]::mainButtonClick:"+e);
		}
	},
