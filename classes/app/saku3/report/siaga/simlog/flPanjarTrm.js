window.app_saku3_report_siaga_simlog_flPanjarTrm = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_simlog_flPanjarTrm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_simlog_flPanjarTrm";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from fa_asset where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_siaga_simlog_flPanjarTrm.extend(window.childForm);
window.app_saku3_report_siaga_simlog_flPanjarTrm.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==2)
			{
				...
				var sql="select a.no_pb, a.keterangan from gr_pb_m a inner join fa_asset d on a.no_pb=d.no_po and a.kode_lokasi=d.kode_lokasi "+filter;
				var sql2="select count(a.no_pb) from gr_pb_m a inner join fa_asset d on a.no_pb=d.no_po and a.kode_lokasi=d.kode_lokasi "+filter;
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_siaga_simlog_rptPanjarTrm";
			...
			systemAPI.alert("[app_saku3_report_siaga_simlog_flPanjarTrm]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.log_pesan_m='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
