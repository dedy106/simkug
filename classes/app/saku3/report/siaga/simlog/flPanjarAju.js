window.app_saku3_report_siaga_simlog_flPanjarAju = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_simlog_flPanjarAju.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_simlog_flPanjarAju";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_pb_m where kode_lokasi='"+this.app._lokasi+"' and modul='PJAJULOG'");
...
window.app_saku3_report_siaga_simlog_flPanjarAju.extend(window.childForm);
window.app_saku3_report_siaga_simlog_flPanjarAju.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gr_pb_m where kode_lokasi='"+this.app._lokasi+"' and modul='PJAJULOG' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_siaga_simlog_flPanjarAju]::mainButtonClick:"+e);
		}
	},
