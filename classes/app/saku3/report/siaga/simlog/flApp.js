window.app_saku3_report_siaga_simlog_flApp = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_simlog_flApp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_simlog_flApp";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from app_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_siaga_simlog_flApp.extend(window.childForm);
window.app_saku3_report_siaga_simlog_flApp.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from app_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_siaga_simlog_flApp]::mainButtonClick:"+e);
		}
	},
