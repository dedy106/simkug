window.app_saku3_report_logistik_flSanggup = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_logistik_flSanggup.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_logistik_flSanggup";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from log_sanggup_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_logistik_flSanggup.extend(window.childForm);
window.app_saku3_report_logistik_flSanggup.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_sanggup, a.keterangan from log_sanggup_m a "+this.filter,
												"select count(*) from log_sanggup_m a "+this.filter,
												new Array("a.no_sanggup","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from log_sanggup_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("l.no_sanggup",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_logistik_rptSanggup";
			...
			systemAPI.alert("[app_saku3_report_logistik_flSanggup]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.log_sanggup_m='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
