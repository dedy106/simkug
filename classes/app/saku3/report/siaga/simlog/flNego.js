window.app_saku3_report_siaga_simlog_flNego = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_simlog_flNego.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_simlog_flNego";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from log_nego_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_siaga_simlog_flNego.extend(window.childForm);
window.app_saku3_report_siaga_simlog_flNego.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_nego, a.keterangan from log_nego_m a "+this.filter,
												"select count(*) from log_nego_m a "+this.filter,
												new Array("a.no_nego","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from log_nego_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("i.no_nego",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
			...
			systemAPI.alert("[app_saku3_report_siaga_simlog_flNego]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.log_nego_m='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
