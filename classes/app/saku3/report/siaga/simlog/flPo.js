window.app_saku3_report_siaga_simlog_flPo = function (owner) {
	if (owner) {
		window.app_saku3_report_siaga_simlog_flPo.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_report_siaga_simlog_flPo";
		this.maximize();
	...
	this.periode = this.dbLib.getPeriodeFromSQL("select max(periode) as periode from log_spk_m where kode_lokasi='" + this.app._lokasi + "' and modul='PO'");
...
window.app_saku3_report_siaga_simlog_flPo.extend(window.childForm);
window.app_saku3_report_siaga_simlog_flPo.implement({
		...
		try {
			...
			if (row == 2) {
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan", this.sg1, this.sg1.row, this.sg1.col,
					"select a.no_spk, a.keterangan from log_spk_m a " + this.filter + "and a.modul='PO'",
					"select count(*) from log_spk_m a " + this.filter + "and a.modul='PO'",
	...
	doSelectCell: function (sender, col, row) {
		...
		if (row == 1) {
			...
			this.dbLib.setItemsFromSQL("select distinct periode from log_spk_m where kode_lokasi='" + this.app._lokasi + "' and modul='PO' order by periode", [this.sg1.columns.get(2).pickList, this.sg1.columns.get(3).pickList]);
		}
		...
		try {
			if (sender === this.app._mainForm.bClear2) {
				...
				if (sender == this.app._mainForm.bBack) {
					...
					this.filter = this.filterRep.filterStr("a.kode_lokasi", this.sg1.getCell(1, 0), this.sg1.getCell(2, 0), this.sg1.getCell(3, 0), "where") +
						this.filterRep.filterStr("a.periode", this.sg1.getCell(1, 1), this.sg1.getCell(2, 1), this.sg1.getCell(3, 1), "and") +
						this.filterRep.filterStr("a.no_spk", this.sg1.getCell(1, 2), this.sg1.getCell(2, 2), this.sg1.getCell(3, 2), "and");
			...
			systemAPI.alert("[app_saku3_report_siaga_simlog_flPo]::mainButtonClick:" + e);
		}
	},
