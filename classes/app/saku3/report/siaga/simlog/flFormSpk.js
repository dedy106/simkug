window.app_saku3_report_siaga_simlog_flFormSpk = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_simlog_flFormSpk.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_simlog_flFormSpk";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from log_spk_m where kode_lokasi='"+this.app._lokasi+"' and modul='SPK'");
...
window.app_saku3_report_siaga_simlog_flFormSpk.extend(window.childForm);
window.app_saku3_report_siaga_simlog_flFormSpk.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from log_spk_m where kode_lokasi='"+this.app._lokasi+"' and modul='SPK' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_siaga_simlog_flFormSpk]::mainButtonClick:"+e);
		}
	},
