window.app_saku3_report_siaga_flAggPakai = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_flAggPakai.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_flAggPakai";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,4,[0,1,2],["Realisasi","=","Anggaran"]);
...
window.app_saku3_report_siaga_flAggPakai.extend(window.portalui_childForm);
window.app_saku3_report_siaga_flAggPakai.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				if (this.sg1.getCell(2,4)=="Anggaran")
				{ 
					var sql = "call sp_agg_pakai ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,3)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
				}else
				{
					var sql = "call sp_agg_pakai_gl ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,3)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
				}
				this.nama_report="server_report_saku3_siaga_rptAggPakai";
	...
	doOpenKartu: function(kode_akun,kode_lokasi,tahun,periode){
		this.kode_akun=kode_akun;
		this.kode_lokasi=kode_lokasi;
		this.tahun=tahun;
		this.periode=periode;
		this.link="1";
		var periode2=" and b.periode='"+periode+"'";
		...
		nama_report="server_report_saku3_siaga_rptAggKpaAkun";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	},
	doOpenKartuSd: function(kode_akun,kode_lokasi,tahun,periode){
		this.kode_akun=kode_akun;
		this.kode_lokasi=kode_lokasi;
		this.tahun=tahun;
		this.periode=periode;
