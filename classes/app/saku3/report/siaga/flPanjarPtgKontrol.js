window.app_saku3_report_siaga_flPanjarPtgKontrol = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_flPanjarPtgKontrol.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_flPanjarPtgKontrol";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_panjarptg_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_siaga_flPanjarPtgKontrol.extend(window.childForm);
window.app_saku3_report_siaga_flPanjarPtgKontrol.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==4)
			{
				...
				var sql="select a.no_ptg, a.keterangan from gr_panjarptg_m a "+filter;
				var sql2="select count(a.no_ptg) from gr_panjarptg_m a "+filter;
				
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,
												new Array("a.no_ptg","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gr_panjarptg_m where kode_lokasi='"+this.app._lokasi+"'  order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_ptg",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.filter2 = this.app._namaUser;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				
				
				this.nama_report = "server_report_saku3_siaga_rptPanjarPtgKontrol";
			...
			systemAPI.alert("[app_saku3_report_siaga_flPanjarPtgKontrol]::mainButtonClick:"+e);
		}
	},
