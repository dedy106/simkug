window.app_saku3_report_siaga_flBebanKontrol2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_flBebanKontrol2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_flBebanKontrol2";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_pb_m where modul='AJU2'");
...
window.app_saku3_report_siaga_flBebanKontrol2.extend(window.childForm);
window.app_saku3_report_siaga_flBebanKontrol2.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gr_pb_m where modul='AJU2' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.modul='AJU2' ";
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,4);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_siaga_rptBebanKontrol2";
			...
			systemAPI.alert("[app_saku3_report_siaga_flBebanKontrol2]::mainButtonClick:"+e);
		}
	},
	...
	doOpenAju: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_siaga_rptBebanJust2";
    ...
    doOpenDok: function(no_pb, kode_lokasi) {
        this.no_pb = no_pb;
        ...
        filter2 = "";
        nama_report = "server_report_saku3_siaga_rptBebanKontrolDok";
        this.viewer.useIframe(this.report.previewWithHeader(nama_report, filter, this.page, this.pager, this.showFilter, this.lokasi, filter2) + "&resource=" + this.resourceId + "&fullId=" + this.getFullId());

    },
    doOpenPdf: function(file_gambar) {

        window.open("media/" + file_gambar);
    },
