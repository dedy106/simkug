window.app_saku3_report_siaga_flKartuSch = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_flKartuSch.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_flKartuSch";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_kartu_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_siaga_flKartuSch.extend(window.childForm);
window.app_saku3_report_siaga_flKartuSch.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Sch",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_kartu, a.keterangan from gr_kartu_m a "+this.filter,
												"select count(*) from gr_kartu_m a "+this.filter,
												new Array("a.no_kartu","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gr_kartu_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_kartu",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_siaga_rptKartuSch";
			...
			systemAPI.alert("[app_saku3_report_siaga_flKartuSch]::mainButtonClick:"+e);
		}
	},
