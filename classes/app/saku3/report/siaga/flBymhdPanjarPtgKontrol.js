window.app_saku3_report_siaga_flBymhdPanjarPtgKontrol = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_flBymhdPanjarPtgKontrol.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_flBymhdPanjarPtgKontrol";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_pb_m where kode_lokasi='"+this.app._lokasi+"' and modul='PJAJUBMHD'");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Karyawan","All",""));
...
window.app_saku3_report_siaga_flBymhdPanjarPtgKontrol.extend(window.childForm);
window.app_saku3_report_siaga_flBymhdPanjarPtgKontrol.implement({
		...
		try{			
			...
			if (row == 3)
			{
				...
				var sql="select a.nik,a.nama "+
						"from karyawan a "+
						"inner join (select nik_buat,kode_lokasi "+
						"			from gr_pb_m "+
						"			where modul='PJAJUBMHD' "+
						"			group by nik_buat,kode_lokasi "+
						"			)b on a.nik=b.nik_buat and a.kode_lokasi=b.kode_lokasi "+filter;
				var sql2="select count(a.nik) "+
						"from karyawan a "+
						"inner join (select nik_buat,kode_lokasi "+
						"			from gr_pb_m "+
						"			where modul='PJAJUBMHD' "+
						"			group by nik_buat,kode_lokasi "+
						"			)b on a.nik=b.nik_buat and a.kode_lokasi=b.kode_lokasi "+filter;
				this.filterRep.ListDataSGFilter(this, "Data Karyawan",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,new Array("a.nik","a.nama"),"and",new Array("NIK","Nama"));
			}
			if (row ==4)
			{
				var filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.modul='PJAJUBMHD'";
				var sql="select a.no_pb, a.keterangan from gr_pb_m a "+filter;
				var sql2="select count(a.no_pb) from gr_pb_m a "+filter;
				
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,
	...
	{		
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","123","3","123","123"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,2,2,2]);	
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from gr_pb_m where kode_lokasi='"+this.app._lokasi+"' and modul='PJAJUBMHD' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("No Panjar","All",""));
			}else{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.modul='PJAJUBMHD'";
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(1,1)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(3,1);
				
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				
				
				this.nama_report = "server_report_saku3_siaga_rptPanjarKontrol";
			...
			systemAPI.alert("[app_saku3_report_siaga_flBymhdPanjarPtgKontrol]::mainButtonClick:"+e);
		}
	},
	...
	doOpenAju: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_pb='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._namaUser;
		nama_report="server_report_saku3_siaga_rptPanjarAju";
	...
	doOpenVer: function(no_bukti,kode_lokasi){
		...
		filter2=this.app._namaUser;
		nama_report="server_report_saku3_siaga_rptBuktiBeban";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenSpb: function(no_bukti,kode_lokasi){
		...
		filter2=this.app._namaUser;
		nama_report="server_report_saku3_siaga_rptSpbForm";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenKas: function(no_bukti,kode_lokasi){
		...
		filter2=this.app._namaUser;
		nama_report="server_report_saku3_siaga_rptBuktiBank";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenAjuPtg: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_ptg='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._namaUser;
		nama_report="server_report_saku3_siaga_rptPanjarPtg";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenVerPtg: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_beban='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._namaUser;
		nama_report="server_report_saku3_siaga_rptBuktiBeban";
	...
	doOpenSpbPtg: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_spb='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._namaUser;
		nama_report="server_report_saku3_siaga_rptSpbForm";
	...
	doOpenKasPtg: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_kas='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._namaUser;
		nama_report="server_report_saku3_siaga_rptBuktiBank";
