window.app_saku3_report_siaga_flSchJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_flSchJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_flSchJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from ju_m where modul='JUSCH' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_siaga_flSchJurnal.extend(window.childForm);
window.app_saku3_report_siaga_flSchJurnal.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from ju_m where modul='JUSCH' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
