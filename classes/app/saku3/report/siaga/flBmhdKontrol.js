window.app_saku3_report_siaga_flBmhdKontrol = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_flBmhdKontrol.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_flBmhdKontrol";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_pb_m where modul='BMHD' ");
...
window.app_saku3_report_siaga_flBmhdKontrol.extend(window.childForm);
window.app_saku3_report_siaga_flBmhdKontrol.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gr_pb_m where modul='BMHD' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_siaga_rptBmhdKontrol";
			...
			systemAPI.alert("[app_saku3_report_siaga_flBmhdKontrol]::mainButtonClick:"+e);
		}
	},
