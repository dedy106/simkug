window.app_saku3_report_siaga_hris_gajinew_flGajiSlip = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_hris_gajinew_flGajiSlip.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_hris_gajinew_flGajiSlip";
		this.maximize();
	...
	this.bukti=this.dbLib.getPeriodeFromSQL("select max(no_gaji) as periode from gr_gaji_m where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.periode+"'");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Status SDM","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("NIK","All",""));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("No Bukti","=",this.bukti));
	this.gridLib.SGEditData(this.sg1,5,[0,1,2],["Format Laporan","=","Bulanan"]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	this.doSelectCell(this.sg1,2,5);
};
window.app_saku3_report_siaga_hris_gajinew_flGajiSlip.extend(window.childForm);
window.app_saku3_report_siaga_hris_gajinew_flGajiSlip.implement({
	doEllipseClick: function(sender, col, row){
		try{	
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Status SDM",this.sg1, this.sg1.row, this.sg1.col,
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Gaji",this.sg1, this.sg1.row, this.sg1.col,
													"select a.no_gaji, a.keterangan from gr_gaji_m a "+filter,
													"select count(*) from gr_gaji_m a "+filter,
													new Array("a.no_gaji","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}	
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Bulanan","Harian"));
		}
	},
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("c.sts_sdm",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("c.nik",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.no_gaji",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
						
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				
				this.nama_report = "server_report_saku3_siaga_hris_gaji_rptSlipGaji";
				if (this.sg1.getCell(2,5)=="Harian")
				{
					this.nama_report = "server_report_saku3_siaga_hris_gaji_rptSlipGajiHarian";
				}
			...
			systemAPI.alert("[app_saku3_report_siaga_hris_gajinew_flGajiSlip]::mainButtonClick:"+e);
		}
	},
