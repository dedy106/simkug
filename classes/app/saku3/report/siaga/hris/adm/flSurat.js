window.app_saku3_report_siaga_hris_adm_flSurat = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_hris_adm_flSurat.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_hris_adm_flSurat";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Surat", 2);
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_surat where kode_lokasi='"+this.app._lokasi+"'");
	if (this.app._userStatus=="A")
	{
		...
		this.gridLib.SGEditData(this.sg1,5,[0,1,2],["No Surat","All",""])
	}
	else
	{
		this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Periode","=",this.periode]);
...
window.app_saku3_report_siaga_hris_adm_flSurat.extend(window.portalui_childForm);
window.app_saku3_report_siaga_hris_adm_flSurat.prototype.doEllipseClick= function(sender, col, row)
{
	...
	if (row == 5)
	{
		...
		this.filterRep.ListDataSGFilter(this, "Daftar Surat",this.sg1, this.sg1.row, this.sg1.col,
											  "select a.no_surat, a.keterangan from gr_surat a "+
											  "inner join gr_karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+this.filter,
											  "select count(a.no_surat) from gr_surat a "+
											  "inner join gr_karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+this.filter,
											  ["a.no_surat","a.keterangan"],"and",["No Surat","Keterangan"], false, this.sg1.cells(1,row) == "in");
	}
	
	
};
window.app_saku3_report_siaga_hris_adm_flSurat.prototype.doSelectCell = function(sender, col, row)
{
	if (this.app._userStatus=="A")
	{
	...
	if (row == 0)
	{
		...
		this.dbLib.setItemsFromSQL("select distinct periode from gr_surat where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
	}
};
window.app_saku3_report_siaga_hris_adm_flSurat.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		{ 
			if (this.app._userStatus=="A")
			{
				...
				this.gridLib.SGEditData(this.sg1,5,[0,1,2],["No SPPD","All",""])
			}
			else
			{
				this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Periode","=",this.periode]);
		...
		else
		{
			...
			this.filter = this.filterRep.filterStr("a.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
					...
					this.filterRep.filterStr("c.nik",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
					this.filterRep.filterStr("a.no_surat",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");
						
				
			this.showFilter = this.filterRep.showFilter(this.sg1);
			this.nama_report = "server_report_siaga_hris_rptSurat";
	...
	{
		alert("[app_saku3_report_siaga_hris_adm_flSurat]::mainButtonClick:"+e);
	}
};
window.app_saku3_report_siaga_hris_adm_flSurat.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_saku3_report_siaga_hris_adm_flSurat.prototype.getStringHeader = function(){
...
window.app_saku3_report_siaga_hris_adm_flSurat.prototype.doConfirmClick = function(sender){
	try{
...
window.app_saku3_report_siaga_hris_adm_flSurat.prototype.doSelectedPage = function(sender, page)
{
	this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,this.filter, this.page,  this.pager, this.showFilter, this.lokasi,this.filter2));
	this.page = page;
	this.allBtn = false;
};
window.app_saku3_report_siaga_hris_adm_flSurat.prototype.previewReport = function(dthtml)
{
...
window.app_saku3_report_siaga_hris_adm_flSurat.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku3_report_siaga_hris_adm_flSurat.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku3_report_siaga_hris_adm_flSurat.prototype.doRowPerPageChange = function(sender, rowperpage)
{
