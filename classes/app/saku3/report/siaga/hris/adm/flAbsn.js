window.app_saku3_report_siaga_hris_adm_flAbsn = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_hris_adm_flAbsn.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_hris_adm_flAbsn";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("No Absen","All",""));
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	this.doSelectCell(this.sg1,2,3);
};
window.app_saku3_report_siaga_hris_adm_flAbsn.extend(window.childForm);
window.app_saku3_report_siaga_hris_adm_flAbsn.implement({
		...
		try{	
				...
				if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Absen",this.sg1, this.sg1.row, this.sg1.col,
													...
													"inner join gr_absen_harian_d b on a.no_absen=b.no_load and a.kode_lokasi=b.kode_lokasi	"+filter,
													"select count(*) from gr_absen a "+
													"inner join gr_absen_harian_d b on a.no_absen=b.no_load and a.kode_lokasi=b.kode_lokasi "+filter,
													new Array("a.no_absen","a.keterangan"),"and",new Array("Absen","Keterangan"));
			}					
		}catch(e){
			systemAPI.alert(e);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("c.kode_so",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where")+
							this.filterRep.filterStr("a.nik_buat",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_absen",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
			...
			systemAPI.alert("[app_saku3_report_siaga_hris_adm_flAbsn]::mainButtonClick:"+e);
		}
	},
