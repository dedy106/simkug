window.app_saku3_report_siaga_hris_adm_flLembur = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_hris_adm_flLembur.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_hris_adm_flLembur";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("No Lembur","All",""));
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	this.doSelectCell(this.sg1,2,3);
};
window.app_saku3_report_siaga_hris_adm_flLembur.extend(window.childForm);
window.app_saku3_report_siaga_hris_adm_flLembur.implement({
	doEllipseClick: function(sender, col, row){
		try{	
				...
				if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Lembur",this.sg1, this.sg1.row, this.sg1.col,
													"select a.no_lembur, a.keterangan from gr_lembur a "+filter,
													"select count(*) from gr_lembur a "+filter,
													new Array("a.no_lembur","a.keterangan"),"and",new Array("Absen","Keterangan"));
			}					
		}catch(e){
			systemAPI.alert(e);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("c.kode_jab",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where")+
							this.filterRep.filterStr("a.nik_buat",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_lembur",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
							
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				
				this.nama_report = "server_report_saku3_siaga_hris_rptLembur";
			...
			systemAPI.alert("[app_saku3_report_siaga_hris_adm_flLembur]::mainButtonClick:"+e);
		}
	},
