window.app_saku3_report_siaga_hris_adm_flSaldoCuti = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_hris_adm_flSaldoCuti.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_hris_adm_flSaldoCuti";
		this.maximize();
	...
	"	select distinct periode from gr_cuti_karyawan where kode_lokasi='"+this.app._lokasi+"'"+
	"	union all "+
	"	select distinct periode from gr_cuti where kode_lokasi='"+this.app._lokasi+"'"+
	"	union all "+
	"	select distinct periode from gr_cuti where kode_lokasi='"+this.app._lokasi+"'"+
	"	) a ");
...
window.app_saku3_report_siaga_hris_adm_flSaldoCuti.extend(window.childForm);
window.app_saku3_report_siaga_hris_adm_flSaldoCuti.implement({
	doEllipseClick: function(sender, col, row){
		try{	
			...
			if (row ==4)
			{
				...
				// 								"select distinct sts_cuti, alasan from gr_cuti_d ",
				// 								"select count(*) from gr_cuti_d ",
				// 								new Array("sts_cuti","alasan"),"where",new Array("sts_cuti","alasan"));
				this.filterRep.ListDataSGFilter(this, "Data Status Cuti",this.sg1, this.sg1.row, this.sg1.col,
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","36","123","123","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,2,2,2]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			var sql = "select distinct a.periode as periode from ( "+
			"	select distinct periode from gr_cuti_karyawan where kode_lokasi='"+this.app._lokasi+"'"+
			"	union all "+
			"	select distinct periode from gr_cuti where kode_lokasi='"+this.app._lokasi+"'"+
			"	union all "+
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(1,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				
				this.nama_report = "server_report_saku3_siaga_hris_rptSaldoCuti";
			...
			systemAPI.alert("[app_saku3_report_siaga_hris_adm_flSaldoCuti]::mainButtonClick:"+e);
		}
	},
