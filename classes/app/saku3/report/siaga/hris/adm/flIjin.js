window.app_saku3_report_siaga_hris_adm_flIjin = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_hris_adm_flIjin.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_hris_adm_flIjin";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Ijin", 2);
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_ijin_m where kode_lokasi='"+this.app._lokasi+"'");
	if (this.app._userStatus=="A")
	{
		...
		this.gridLib.SGEditData(this.sg1,5,[0,1,2],["No Ijin","All",""])
	}
	else
	{
		this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Periode","=",this.periode]);
...
window.app_saku3_report_siaga_hris_adm_flIjin.extend(window.portalui_childForm);
window.app_saku3_report_siaga_hris_adm_flIjin.prototype.doEllipseClick= function(sender, col, row)
{
	...
	if (row == 5)
	{
		...
		this.filterRep.ListDataSGFilter(this, "Daftar Ijin",this.sg1, this.sg1.row, this.sg1.col,
											  "select a.no_ijin, a.keterangan from gr_ijin_m a "+
											  "inner join gr_karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+this.filter,
											  "select count(a.no_ijin) from gr_ijin_m a "+
											  "inner join gr_karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+this.filter,
											  ["a.no_ijin","a.keterangan"],"and",["No Ijin","Keterangan"], false, this.sg1.cells(1,row) == "in");
	}
	
	
};
window.app_saku3_report_siaga_hris_adm_flIjin.prototype.doSelectCell = function(sender, col, row)
{
	if (this.app._userStatus=="A")
	{
	...
	if (row == 0)
	{
		...
		this.dbLib.setItemsFromSQL("select distinct periode from gr_ijin_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
	}
};
window.app_saku3_report_siaga_hris_adm_flIjin.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.filter = this.filterRep.filterStr("a.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
					...
					this.filterRep.filterStr("a.no_ijin",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");
						
				
			this.showFilter = this.filterRep.showFilter(this.sg1);
			this.nama_report = "server_report_siaga_hris_rptIjin";
	...
	{
		alert("[app_saku3_report_siaga_hris_adm_flIjin]::mainButtonClick:"+e);
	}
};
window.app_saku3_report_siaga_hris_adm_flIjin.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_saku3_report_siaga_hris_adm_flIjin.prototype.getStringHeader = function(){
...
window.app_saku3_report_siaga_hris_adm_flIjin.prototype.doConfirmClick = function(sender){
	try{
...
window.app_saku3_report_siaga_hris_adm_flIjin.prototype.doSelectedPage = function(sender, page)
{
	this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,this.filter, this.page,  this.pager, this.showFilter, this.lokasi,this.filter2));
	this.page = page;
	this.allBtn = false;
};
window.app_saku3_report_siaga_hris_adm_flIjin.prototype.previewReport = function(dthtml)
{
...
window.app_saku3_report_siaga_hris_adm_flIjin.prototype.doCloseReportClick = function(sender)
{
...
window.app_saku3_report_siaga_hris_adm_flIjin.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku3_report_siaga_hris_adm_flIjin.prototype.doRowPerPageChange = function(sender, rowperpage)
{
