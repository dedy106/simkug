window.app_saku3_report_siaga_hris_adm_flSpj = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_hris_adm_flSpj.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_hris_adm_flSpj";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Perjalanan Dinas", 2);
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_spj_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_siaga_hris_adm_flSpj.extend(window.portalui_childForm);
window.app_saku3_report_siaga_hris_adm_flSpj.prototype.doEllipseClick= function(sender, col, row)
{
	...
	if (row == 5)
	{
		...
		this.filterRep.ListDataSGFilter(this, "Daftar SPPD",this.sg1, this.sg1.row, this.sg1.col,
											  "select a.no_spj, a.keterangan from gr_spj_m a "+
											  "inner join gr_karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+this.filter,
											  "select count(a.no_spj) from gr_spj_m a "+
											  "inner join gr_karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+this.filter,
											  ["a.no_spj","a.keterangan"],"and",["No SPPD","Keterangan"], false, this.sg1.cells(1,row) == "in");
	}

	
};
window.app_saku3_report_siaga_hris_adm_flSpj.prototype.doSelectCell = function(sender, col, row)
{
	...
	*/
	if (row == 0)
	{
		...
		this.dbLib.setItemsFromSQL("select distinct periode from gr_spj_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
	}
};
window.app_saku3_report_siaga_hris_adm_flSpj.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			this.filter = this.filterRep.filterStr("a.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
					...
					this.filterRep.filterStr("a.no_spj",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");
				
			this.showFilter = this.filterRep.showFilter(this.sg1);
			this.nama_report = "server_report_siaga_hris_rptSpj";
	...
	{
		alert("[app_saku3_report_siaga_hris_adm_flSpj]::mainButtonClick:"+e);
	}
};
window.app_saku3_report_siaga_hris_adm_flSpj.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_saku3_report_siaga_hris_adm_flSpj.prototype.getStringHeader = function(){
...
window.app_saku3_report_siaga_hris_adm_flSpj.prototype.doConfirmClick = function(sender){
	try{
...
window.app_saku3_report_siaga_hris_adm_flSpj.prototype.doSelectedPage = function(sender, page)
{
	this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,this.filter, this.page,  this.pager, this.showFilter, this.lokasi,this.filter2));
	this.page = page;
	this.allBtn = false;
};
window.app_saku3_report_siaga_hris_adm_flSpj.prototype.previewReport = function(dthtml)
{
...
window.app_saku3_report_siaga_hris_adm_flSpj.prototype.doCloseReportClick = function(sender)
{
  switch(sender.getName())
  {
    ...
    case "pdfBtn" :      
	  var html = new server_util_arrayList();
		html.add(this.viewer.getContent());			
		html.add("pdf");			
		html.add("PosisiSpp");				
...
window.app_saku3_report_siaga_hris_adm_flSpj.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_saku3_report_siaga_hris_adm_flSpj.prototype.doRowPerPageChange = function(sender, rowperpage)
{
