window.app_saku3_report_siaga_hris_adm_flCutii = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_hris_adm_flCutii.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_hris_adm_flCutii";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max (substring(periode,1,4)) as periode from gr_cuti ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Jabatan","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("NIK","All",""));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Jenis Cuti","All",""));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("No Cuti","All",""));
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	this.doSelectCell(this.sg1,2,3);
};
window.app_saku3_report_siaga_hris_adm_flCutii.extend(window.childForm);
window.app_saku3_report_siaga_hris_adm_flCutii.implement({
		...
		try{	
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Jabatan",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_so, nama from gr_so where tipe='posting' ",
												"select count(*) from gr_so where tipe='posting' ",
												new Array("kode_so","nama"),"where",new Array("kode","nama"));
			}			
			
			if (row ==3)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.kode_so",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
			...
			if (row ==4)
			{
				this.filterRep.ListDataSGFilter(this, "Data Status Cuti",this.sg1, this.sg1.row, this.sg1.col,
												"select sts_cuti, nama from gr_status_cuti ",
												"select count(*) from gr_status_cuti ",
												new Array("sts_cuti","nama"),"where",new Array("kode","nama"));
			}	
			if (row ==5)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
				this.filterRep.filterStr("a.nik_buat",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
				this.filterRep.filterStr("a.sts_cuti",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.filterRep.ListDataSGFilter(this, "Data Cuti",this.sg1, this.sg1.row, this.sg1.col,
													"select a.no_cuti, a.alasan from gr_cuti a "+filter,
													"select count(*) from gr_cuti a "+filter,
													new Array("a.no_cuti","a.alasan"),"and",new Array("Cuti","Keterangan"));
			}					
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[0,2,2,2,2]);	
		if (row == 0)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct (substring(periode,1,4)) from gr_cuti where kode_lokasi='"+this.app._lokasi+"' ",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.lokasi=this.dbLib.getPeriodeFromSQL("select nama as periode from lokasi where kode_lokasi='"+this.sg1.getCell(2,1)+"'");
				this.filter = this.filterRep.filterStr("f.kode_lokasi",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where")+
							this.filterRep.filterStr("substring(a.periode,1,4)",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and")+
							this.filterRep.filterStr("c.kode_so",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.nik_buat",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.sts_cuti",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							this.filterRep.filterStr("a.no_cuti",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");
					
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				
				this.nama_report = "server_report_saku3_siaga_hris_rptCuti";
			...
			systemAPI.alert("[app_saku3_report_siaga_hris_adm_flCutii]::mainButtonClick:"+e);
		}
	},
