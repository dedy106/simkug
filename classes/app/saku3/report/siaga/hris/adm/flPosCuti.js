window.app_saku3_report_siaga_hris_adm_flPosCuti = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_hris_adm_flPosCuti.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_hris_adm_flPosCuti";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max (periode) as periode from gr_cuti ");
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Jenis Cuti","=","1"));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Nomor Bukti","All",""));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	this.doSelectCell(this.sg1,2,3);
};
window.app_saku3_report_siaga_hris_adm_flPosCuti.extend(window.childForm);
window.app_saku3_report_siaga_hris_adm_flPosCuti.implement({
		...
		try{	
			...
			if (row ==5)
			{
				filter = this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						...
						this.filterRep.filterStr("nik_buat",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filterRep.ListDataSGFilter(this, "Data Cuti",this.sg1, this.sg1.row, this.sg1.col,
												"select no_cuti, alasan from gr_cuti "+filter,
												"select count(*) from gr_cuti "+filter,
												new Array("no_cuti","alasan"),"where",new Array("No Bukti","Alasan"));
			}	
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["123","123","123","123","3","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,2,2,2,2,2]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from gr_cuti where kode_lokasi='"+this.app._lokasi+"' ",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_siaga_hris_rptPosCuti";
			...
			systemAPI.alert("[app_saku3_report_siaga_hris_adm_flPosCuti]::mainButtonClick:"+e);
		}
	},
