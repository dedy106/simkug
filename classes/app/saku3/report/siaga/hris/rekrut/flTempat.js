window.app_saku3_report_siaga_hris_rekrut_flTempat = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_hris_rekrut_flTempat.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_hris_rekrut_flTempat";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Penempatan", 2);
...
window.app_saku3_report_siaga_hris_rekrut_flTempat.extend(window.childForm);
window.app_saku3_report_siaga_hris_rekrut_flTempat.implement({
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("b.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
							...
							this.filterRep.filterStr("a.flag_form","=","REKRUT","REKRUT","and");;
				...
				var sql = "select a.nik,b.nama,a.no_sk,date_format(c.tgl_awal_sk,'%d/%m/%Y') as tgl_awal_sk,date_format(c.tgl_akhir_sk,'%d/%m/%Y') as tgl_akhir_sk,a.kode_grade,d.nama as loker,e.nama as dir,f.nama as dept, "+
							"	   g.nama as jabatan,i.nama as status_sdm ,h.nama as pb, b.nip "+
							"from gr_dinas a "+
							"inner join gr_karyawan b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi "+
							"inner join gr_sk c on a.no_sk=c.no_sk and a.kode_lokasi=c.kode_lokasi "+
				...
				this.scriptSqlCount = "select count(a.nik) "+
							...
							"inner join gr_pb h on a.kode_pb=h.kode_pb and a.kode_lokasi=h.kode_lokasi "+
							"inner join gr_status_sdm i on a.sts_sdm=i.sts_sdm and a.kode_lokasi=i.kode_lokasi "+this.filter; 
				this.title = new server_util_arrayList({items:["NIK","Nama","No SK","Tgl Awal","Tgl Akhir","Grade","Lokasi Kerja","Direktorat","Departemen","Jabatan","Status","Perbantuan","NIP"]});
				this.widthTable = new server_util_arrayList({items:[60,150,80,60,60,40,120,120,120,120,80,120,70]});
			...
			systemAPI.alert("[app_saku3_report_siaga_hris_rekrut_flTempat]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		...
		var judul="LAPORAN PENEMPATAN "+this.sg1.getCell(2,1).toUpperCase()
