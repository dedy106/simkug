window.app_saku3_report_siaga_hris_rekrut_flJobPelamar2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_hris_rekrut_flJobPelamar2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_hris_rekrut_flJobPelamar2";
		this.maximize();
...
window.app_saku3_report_siaga_hris_rekrut_flJobPelamar2.extend(window.childForm);
window.app_saku3_report_siaga_hris_rekrut_flJobPelamar2.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			if (row ==0)
			{
				this.filterRep.ListDataSGFilter(this, "Data Lowongan",this.sg1, this.sg1.row, this.sg1.col,
													"select kode_job, nama from gr_rekrut_job where kode_lokasi = '"+this.app._lokasi+"' order by a.tanggal desc ",
			...
			if (row == 2)
			{
							...
							this.filter = this.filterRep.filterStr("a.kode_job",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and");
				this.filterRep.ListDataSGFilter(this, "Data Pelamar",this.sg1, this.sg1.row, this.sg1.col,
													...
													"inner join gr_rekrut_pelamar c on a.nip=c.nip and a.kode_lokasi=c.kode_lokasi "+this.filter,
													"select count(*) from gr_rekrut_job_pelamar a "+
													"inner join gr_rekrut_pelamar c on a.nip=c.nip and a.kode_lokasi=c.kode_lokasi "+this.filter,
													["a.nip","c.nama"],"and",["NIP","Nama"]);
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_job",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("substring(e.periode,0,7)",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
				...
				this.nama_report = "server_report_saku3_siaga_hris_rptJobPelamar";
			...
			systemAPI.alert("[app_saku3_report_siaga_hris_rekrut_flJobPelamar2]::mainButtonClick:"+e);
		}
	},
