window.app_saku3_report_siaga_hris_rekrut_flJobHis = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_hris_rekrut_flJobHis.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_hris_rekrut_flJobHis";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Kode Job","All",""));
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("NIP","All",""));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	this.doSelectCell(this.sg1,2,3);
};
window.app_saku3_report_siaga_hris_rekrut_flJobHis.extend(window.childForm);
window.app_saku3_report_siaga_hris_rekrut_flJobHis.implement({
	doEllipseClick: function(sender, col, row){
		try{	
			...
			if (row ==1)
			{
				this.filterRep.ListDataSGFilter(this, "Data Job",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_job, nama from gr_rekrut_job ",
												"select count(*) from gr_rekrut_job ",
												new Array("kode_job","nama"),"where",new Array("kode","nama"));
			}		
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pelamar",this.sg1, this.sg1.row, this.sg1.col,
													"select a.nip, c.nama from gr_rekrut_job_pelamar a "+
													"inner join gr_rekrut_pelamar c on a.nip=c.nip and a.kode_lokasi=c.kode_lokasi "+filter,
													"select count(*) from gr_rekrut_job_pelamar a "+
													"inner join gr_rekrut_pelamar c on a.nip=c.nip and a.kode_lokasi=c.kode_lokasi "+filter,
													new Array("a.nip","a.nama"),"and",new Array("NIP","Nama"));
			}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("b.kode_job",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where")+
							this.filterRep.filterStr("a.nip",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
							
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				
				this.nama_report = "server_report_saku3_siaga_hris_rptJobHis";
			...
			systemAPI.alert("[app_saku3_report_siaga_hris_rekrut_flJobHis]::mainButtonClick:"+e);
		}
	},
