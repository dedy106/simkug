window.app_saku3_report_siaga_hris_kesehatan_flReimb = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_hris_kesehatan_flReimb.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_hris_kesehatan_flReimb";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("No Reimb","All",""));
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	this.doSelectCell(this.sg1,2,3);
};
window.app_saku3_report_siaga_hris_kesehatan_flReimb.extend(window.childForm);
window.app_saku3_report_siaga_hris_kesehatan_flReimb.implement({
		...
		try{	
			...
			if (row ==2)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.kode_so",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				...
				if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Reimb",this.sg1, this.sg1.row, this.sg1.col,
											"select a.no_kes, a.keterangan from gr_kes_m a "+
											"inner join gr_karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+filter,
											"select count(a.no_kes) from gr_kes_m a "+
											"inner join gr_karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+filter,
				new Array("a.no_kes","a.keterangan"),"and",new Array("Surat","Keterangan"));
			}					
		}catch(e){
			systemAPI.alert(e);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("b.kode_so",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where")+
							this.filterRep.filterStr("b.nik",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_kes",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
							
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				
				this.nama_report = "server_report_saku3_siaga_hris_rptReimburse";
			...
			systemAPI.alert("[app_saku3_report_siaga_hris_kesehatan_flReimb]::mainButtonClick:"+e);
		}
	},
