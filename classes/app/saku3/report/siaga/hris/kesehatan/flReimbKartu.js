window.app_saku3_report_siaga_hris_kesehatan_flReimbKartu = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_hris_kesehatan_flReimbKartu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_hris_kesehatan_flReimbKartu";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select substring(max(periode),1,4) as periode from gr_kes_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_siaga_hris_kesehatan_flReimbKartu.extend(window.childForm);
window.app_saku3_report_siaga_hris_kesehatan_flReimbKartu.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2],["123","2","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2],[2,0,2]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from gr_kes_m where kode_lokasi='"+this.app._lokasi+"' order by substring(periode,1,4) desc",[this.sg1.columns.get(2).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_siaga_hris_rptReimbKartu";
			...
			systemAPI.alert("[app_saku3_report_siaga_hris_kesehatan_flReimbKartu]::mainButtonClick:"+e);
		}
	},
