window.app_hris_report_kesehatan_flPosKlaimUser = function(owner)
{
	if (owner)
	{
		window.app_hris_report_kesehatan_flPosKlaimUser.prototype.parent.constructor.call(this,owner);
		this.className = "app_hris_report_kesehatan_flPosKlaimUser";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from gr_klaim_m where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["No Klaim","All",""]);	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_hris_report_kesehatan_flPosKlaimUser.extend(window.childForm);
window.app_hris_report_kesehatan_flPosKlaimUser.implement({
		...
		try{			
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Reimburse",this.sg1, this.sg1.row, this.sg1.col,
													"select a.no_klaim, a.keterangan from gr_klaim_m a "+filter,
													"select count(a.no_klaim) from gr_klaim_m a "+filter,
													["a.no_klaim","a.nama"],"and",["Kode","Nama"]);
			}	
		}catch(e){
			systemAPI.alert(e);
		}
	...
	doSelectCell: function(sender, col, row){		
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from gr_klaim_m where kode_lokasi='"+this.app._lokasi+"' order by substring(periode,1,4)",[this.sg1.columns.get(2).pickList]);
		}
	},
		...
		try
		{
			...
			else
			{
				...
				else
				{
					...
					this.nama_report = "server_report_hris_rptPosKlaim";
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_klaim='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
