window.app_saku3_report_siaga_hris_karyawan_flSoSdm2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_hris_karyawan_flSoSdm2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_hris_karyawan_flSoSdm2";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_penilaian_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_siaga_hris_karyawan_flSoSdm2.extend(window.childForm);
window.app_saku3_report_siaga_hris_karyawan_flSoSdm2.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Penilaian",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_penilaian, a.keterangan from gr_penilaian_m a "+this.filter,
												"select count(*) from gr_penilaian_m a "+this.filter,
												new Array("a.no_penilaian","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gr_penilaian_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			...
			else
			{
				...
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
								...
								this.filterRep.filterStr("a.no_penilaian",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
								this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
								this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_siaga_hris_rptNilai";
			...
			systemAPI.alert("[app_saku3_report_siaga_hris_karyawan_flSoSdm2]::mainButtonClick:"+e);
		}
	},
