window.app_saku3_report_siaga_hris_karyawan_flRwyNilai = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_hris_karyawan_flRwyNilai.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_hris_karyawan_flRwyNilai";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Jabatan","All",""));
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("NIK","All",""));
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("No Penilaian","All",""));
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	this.doSelectCell(this.sg1,2,3);
};
window.app_saku3_report_siaga_hris_karyawan_flRwyNilai.extend(window.childForm);
window.app_saku3_report_siaga_hris_karyawan_flRwyNilai.implement({
	doEllipseClick: function(sender, col, row){
		try{	
			...
			if (row ==2)
			{
				filter = this.filterRep.filterStr("a.kode_so",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where");
				...
				if (row ==3)
			{
				filter = this.filterRep.filterStr("a.nik_buat",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where");

				this.filterRep.ListDataSGFilter(this, "Data SK",this.sg1, this.sg1.row, this.sg1.col,
													"select a.no_penilaian,a.keterangan from gr_penilaian_m a "+filter,
													"select count(*) from gr_penilaian_m a "+filter,
													new Array("a.no_penilaian","a.keterangan"),"and",new Array("Nilai","Keterangan"));
			}					
		}catch(e){
			systemAPI.alert(e);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("b.kode_so",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
				...
				this.nama_report = "server_report_saku3_siaga_hris_rptRwyNilai";
			...
			systemAPI.alert("[app_saku3_report_siaga_hris_karyawan_flRwyNilai]::mainButtonClick:"+e);
		}
	},
