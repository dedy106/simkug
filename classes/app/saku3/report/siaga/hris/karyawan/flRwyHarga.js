window.app_saku3_report_siaga_hris_karyawan_flRwyHarga = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_hris_karyawan_flRwyHarga.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_hris_karyawan_flRwyHarga";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Riwayat Penghargaan", 2);
...
window.app_saku3_report_siaga_hris_karyawan_flRwyHarga.extend(window.childForm);
window.app_saku3_report_siaga_hris_karyawan_flRwyHarga.implement({
		...
		try{	
			...
			if (row == 6)
			{
				this.filterRep.ListDataSGFilter(this, "Data SK",this.sg1, this.sg1.row, this.sg1.col,
													...
													this.filterRep.filterStr("b.sts_sdm",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
													this.filterRep.filterStr("b.nik",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and"),
													["a.no_sk","c.nama"],"and",["No SK","Nama"], false, this.sg1.cells(1,row) == "in");
			}		
		}catch(e){
			systemAPI.alert(e);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				var sql = "select a.nik,b.nama,a.no_sk,date_format(c.tgl_awal_sk,'%d/%m/%Y') as tgl_awal_sk,a.keterangan,a.nilai "+
						"from gr_rwyharga a "+
						"inner join gr_karyawan b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi "+
						"inner join gr_sk c on a.no_sk=c.no_sk and a.kode_lokasi=c.kode_lokasi "+this.filter
						"order by a.nik ";
				this.scriptSqlCount = "select count(*) "+
									"from gr_rwyharga a "+
									"inner join gr_karyawan b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi "+
									"inner join gr_sk c on a.no_sk=c.no_sk and a.kode_lokasi=c.kode_lokasi "+this.filter; 
				this.title = new server_util_arrayList({items:["NIK","Nama","No SK","Tanggal","Keterangan","Nilai"]});
				this.widthTable = new server_util_arrayList({items:[60,150,100,60,200,80]});
			...
			systemAPI.alert("[app_saku3_report_siaga_hris_karyawan_flRwyHarga]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		...
		var judul="LAPORAN RIWAYAT PENGHARGAAN";
