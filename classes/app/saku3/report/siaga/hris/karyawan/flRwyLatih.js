window.app_saku3_report_siaga_hris_karyawan_flRwyLatih = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_hris_karyawan_flRwyLatih.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_hris_karyawan_flRwyLatih";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Riwayat Pelatihan", 2);
	...
	this.gridLib.SGEditData(this.sg1,7,[0,1,2],["Periode Pelatihan","All",""]);	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_siaga_hris_karyawan_flRwyLatih.extend(window.childForm);
window.app_saku3_report_siaga_hris_karyawan_flRwyLatih.implement({
	...
	doSelectCell: function(sender, col, row){		
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["123i","123i","123i","123i","123i","123i","3","123"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[2,2,2,2,2,2,0,0]);
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["123i","123i","123i","123i","123i","3","3","123"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[3,3,3,3,3,3,0,0]);
		}
		if (row == 6)
		{
		...
		if (row == 7)
		{
			...
			this.dbLib.setItemsFromSQL("select a.periode "+
									"from (select distinct fn_periode(tgl_mulai) as periode from gr_rwylatih "+
									"	  union  "+
									"	  select distinct fn_periode(tgl_selesai) as periode from gr_rwylatih "+
									"	 )a "+
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("b.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
							...
							this.filterRep.filterStr("fn_periode(a.tgl_mulai)",this.sg1.getCell(1,7),this.sg1.getCell(2,7),this.sg1.getCell(3,7),"and")+flag_aktif;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();
				var sql = "select a.nik,b.nama,d.nama as sts_latih,a.nama,a.lama,date_format(a.tgl_mulai,'%d/%m/%Y') as tgl_mulai,date_format(a.tgl_selesai,'%d/%m/%Y') as tgl_selesai,a.panitia,f.nama as kota,g.nama as dana, "+
						...
						"inner join gr_status_dana g on a.sts_dana=g.sts_dana and a.kode_lokasi=g.kode_lokasi "+this.filter
						"order by a.nik ";
				this.scriptSqlCount = "select count(*) "+
									"from gr_rwylatih a "+
									"inner join gr_karyawan b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi "+
									"inner join gr_status_latih d on a.sts_latih=d.sts_latih and a.kode_lokasi=d.kode_lokasi "+
									"inner join gr_status_sertifikat e on a.sts_sertifikat=e.sts_sertifikat and a.kode_lokasi=e.kode_lokasi "+this.filter; 
				this.title = new server_util_arrayList({items:["NIK","Nama","Status","Pelatihan","Lama","Tgl Mulai","Tgl Selesai","Panitia","Kota","Dana","Sertifikat","Berlaku","Peserta"]});
				this.widthTable = new server_util_arrayList({items:[60,150,100,250,60,60,60,150,150,150,100,60]});
			...
			systemAPI.alert("[app_saku3_report_siaga_hris_karyawan_flRwyLatih]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		...
		var judul="LAPORAN RIWAYAT PELATIHAN";
