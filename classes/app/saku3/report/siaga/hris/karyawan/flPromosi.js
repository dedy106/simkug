window.app_saku3_report_siaga_hris_karyawan_flPromosi = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_hris_karyawan_flPromosi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_hris_karyawan_flPromosi";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Riwayat Kedinasan", 2);
...
window.app_saku3_report_siaga_hris_karyawan_flPromosi.extend(window.childForm);
window.app_saku3_report_siaga_hris_karyawan_flPromosi.implement({
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("b.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
							...
							this.filterRep.filterStr("a.flag_form","=","PROMOSI","PROMOSI","and")+flag_aktif;
				...
				var sql = "select a.nik,b.nama,a.no_sk,date_format(c.tgl_awal_sk,'%d/%m/%Y') as tgl_awal_sk,date_format(c.tgl_akhir_sk,'%d/%m/%Y') as tgl_akhir_sk,a.kode_grade,d.nama as loker,e.nama as dir,f.nama as dept, "+
						    ...
						    "order by a.nik,c.tgl_masuk desc ";
				...
				this.widthTable = new server_util_arrayList({items:[60,150,150,60,60,40,120,120,120,120,120,120]});
			...
			systemAPI.alert("[app_saku3_report_siaga_hris_karyawan_flPromosi]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		...
		var judul="LAPORAN RIWAYAT KEDINASAN";
