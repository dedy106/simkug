window.app_hris_report_gaji_flAkdhk = function(owner)
{
	if (owner)
	{
		window.app_hris_report_gaji_flAkdhk.prototype.parent.constructor.call(this,owner);
		this.className = "app_hris_report_gaji_flAkdhk";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Perhitungan AKDHK", 2);
...
window.app_hris_report_gaji_flAkdhk.extend(window.childForm);
window.app_hris_report_gaji_flAkdhk.implement({
	...
	doSelectCell: function(sender, col, row){		
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Karyawan","Sales"));
		}
	},
		...
		try
		{
			...
			else
			{
				...
				if (this.sg1.getCell(2,9)=="Karyawan")
				{
					...
					var sql = "select a.nik,a.nama,a.flag_gaji,b.nama as jabatan, "+
							"	   isnull(c.gdas,0) as gdas,isnull(c.tpos,0) as tpos,isnull(c.tpres,0) as tpres, "+
							"	   isnull(c.tsus,0) as tsus,isnull(c.trans,0) as trans, "+
			...
			systemAPI.alert("[app_hris_report_gaji_flAkdhk]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		...
		var judul="LAPORAN PERHITUNGAN AKDHK";
