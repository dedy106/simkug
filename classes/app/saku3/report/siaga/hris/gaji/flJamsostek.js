window.app_hris_report_gaji_flJamsostek = function(owner)
{
	if (owner)
	{
		window.app_hris_report_gaji_flJamsostek.prototype.parent.constructor.call(this,owner);
		this.className = "app_hris_report_gaji_flJamsostek";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Perhitungan Jamsostek", 2);
...
window.app_hris_report_gaji_flJamsostek.extend(window.childForm);
window.app_hris_report_gaji_flJamsostek.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				if (this.sg1.getCell(2,9)=="Karyawan")
				{
					...
					var sql = "select a.nik,a.nama,a.sts_pajak,b.nama as jabatan, "+
							...
							"	   isnull(c.tsus,0) as tsus,isnull(c.trans,0) as trans,isnull(c.ht,0) as ht,isnull(c.th,0) as th,"+
							"      case when a.flag_gaji='NON' then isnull(c.rem,0) else isnull(c.ht,0)+isnull(c.th,0) end as rem, "+
							"	   case when a.flag_gaji='NON' then (isnull(c.rem,0)-isnull(c.tpres,0)-isnull(c.tsus,0)) "+
							"           else round(((isnull(c.ht,0)+isnull(c.th,0))*0.75),0) end as upah_dasar, "+
					...
					this.scriptSqlCount = "select count(a.nik) "+
							...
							" ";
					this.title = new server_util_arrayList({items:["NIK","Nama","Status","Jabatan","Gaji Dasar","Tunjangan Posisi","Tunjangan Prestasi","Tunjangan Khusus","Tunjangan Transport","Honor Tetap","Tunjangan Harian (22 hk)","Remunerasi","Upah Dasar","Iuran"]});
					this.widthTable = new server_util_arrayList({items:[80,200,50,200,90,90,90,90,90,90,90,90,90,90]});
					this.fieldType = new server_util_arrayList({items:["S","S","S","S","N","N","N","N","N","N","N","N","N","N"]});																
					this.summary = new server_util_arrayList({items:["N","N","N","N","Y","Y","Y","Y","Y","Y","Y","Y","Y","Y"]});
			...
			systemAPI.alert("[app_hris_report_gaji_flJamsostek]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		...
		var judul="LAPORAN PERHITUNGAN JAMSOSTEK";
