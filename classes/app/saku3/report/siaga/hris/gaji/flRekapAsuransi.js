window.app_hris_report_gaji_flRekapAsuransi = function(owner)
{
	if (owner)
	{
		window.app_hris_report_gaji_flRekapAsuransi.prototype.parent.constructor.call(this,owner);
		this.className = "app_hris_report_gaji_flRekapAsuransi";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Rekap Asuransi", 2);
...
window.app_hris_report_gaji_flRekapAsuransi.extend(window.childForm);
window.app_hris_report_gaji_flRekapAsuransi.implement({
	...
	doSelectCell: function(sender, col, row){		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["3","123","123","123","123","3","123","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7],[0,2,2,2,2,0,2,0]);
		if (row == 0)
		{
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.initColumn();
				var flag_gaji="";
				if (this.sg1.getCell(2,5)=="Pegawai")
				{
					flag_gaji=" and a.flag_gaji='NON' ";
				}
				if (this.sg1.getCell(2,5)=="Sales")
				{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
							...
							this.filterRep.filterStr("a.nik",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and")+flag_gaji;
				...
				if (this.sg1.getCell(2,7)=="Karyawan")
				{
					...
					var sql = "select a.nik,a.nama,"+param1+
							...
							"			from gr_gaji_d b "+
							"		    inner join gr_gaji_param c on b.kode_param=c.kode_param and b.kode_lokasi=c.kode_lokasi "+this.filter2+
							"           and c.jenis='A' "+
							"			)d on a.nik=d.nik "+
					...
					this.title = new server_util_arrayList({items:["NIK","Nama"]});
				...
				if (this.sg1.getCell(2,7)=="Lokasi Kerja")
				{
				...
				if (this.sg1.getCell(2,7)=="Direktorat")
				{
				...
				if (this.sg1.getCell(2,7)=="Departemen")
				{
				...
				if (this.sg1.getCell(2,7)=="Akun")
				{
					
					var sql = "select a.kode_akun,a.nama,"+param1+
							"from gr_masakun a "+
							"inner join (select distinct b.kode_akun "+
							"			from gr_gaji_d b "+
							"		    inner join gr_gaji_param c on b.kode_param=c.kode_param and b.kode_lokasi=c.kode_lokasi "+
							"		    inner join gr_karyawan a on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi "+this.filter2+
							"           and c.jenis='A' "+
							"			)d on a.kode_akun=d.kode_akun "+
							"left join (select b.kode_akun,"+param3+
							"			from gr_karyawan a "+
							"			inner join gr_gaji_d b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi "+
							"		    inner join gr_gaji_param c on b.kode_param=c.kode_param and a.kode_lokasi=c.kode_lokasi "+this.filter2+
							"		    and c.dc='C' and c.jenis='A' "+
					...
					this.widthTable = new server_util_arrayList({items:[80,300]});
					this.fieldType = new server_util_arrayList({items:["S","S"]});																
					this.summary = new server_util_arrayList({items:["N","N"]});
			...
			systemAPI.alert("[app_hris_report_gaji_flRekapAsuransi]::mainButtonClick:"+e);
		}
	},
	initColumn: function(){
		try{		
			var sql="select a.kode_param,a.nama from gr_gaji_param a "+
					"inner join (select kode_param from gr_gaji_d group by kode_param having sum(nilai)>0) b on a.kode_param=b.kode_param "+
					"where a.kode_lokasi = '"+this.app._lokasi+"' and a.dc='C' and a.jenis='A' order by a.no_urut"
	...
	previewReport: function(dthtml){
		...
		var judul="LAPORAN REKAP ASURANSI";
