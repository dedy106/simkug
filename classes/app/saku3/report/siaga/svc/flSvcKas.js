window.app_saku3_report_siaga_svc_flSvcKas = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_svc_flSvcKas.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_svc_flSvcKas";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('KBSVC','KBSVC2')");
...
window.app_saku3_report_siaga_svc_flSvcKas.extend(window.childForm);
window.app_saku3_report_siaga_svc_flSvcKas.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gr_piutang_j where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
