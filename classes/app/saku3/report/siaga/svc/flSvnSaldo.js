window.app_saku3_report_siaga_svc_flSvnSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_svc_flSvnSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_svc_flSvnSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_svc_m ");
	this.periode2=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_svcbayar_d ");
	this.cab=this.dbLib.getPeriodeFromSQL("select kode_cabang as periode from gr_karyawan_cab where nik='"+this.app._userLog+"' and flag_aktif='1'");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Periode Pembayaran","<=",this.periode2));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Cabang","=",this.cab));
...
window.app_saku3_report_siaga_svc_flSvnSaldo.extend(window.childForm);
window.app_saku3_report_siaga_svc_flSvnSaldo.implement({
		...
		try{			
			...
			if (row == 3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Cabang",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_cabang, nama from gr_cabang "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(kode_cabang) from gr_cabang "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  new Array("kode_cabang","nama"),"and");
			}
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select b.no_invoice, b.nama_cust from gr_svc_m a "+
												"inner join gr_svc_d b on a.no_svc=b.no_svc and a.kode_lokasi=b.kode_lokasi "+filter,
												"select count(b.no_invoice) from gr_svc_m a "+
												"inner join gr_svc_d b on a.no_svc=b.no_svc and a.kode_lokasi=b.kode_lokasi "+filter,
												new Array("b.no_invoice","b.nama_cust"),"and",new Array("No Invoice","Customer"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gr_svc_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_cabang",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("b.no_invoice",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.nama_cab=this.dbLib.getPeriodeFromSQL("select nama as periode from gr_cabang where kode_cabang='"+this.sg1.getCell(2,3)+"' ");
				this.filter2 = this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,5)+"/"+this.nama_cab;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_siaga_svc_rptSvcSaldo";
			...
			systemAPI.alert("[app_saku3_report_siaga_svc_flSvnSaldo]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBayar: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter = "where b.no_invoice='"+no_bukti+"' and b.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
		
		nama_report="server_report_saku2_siaga_rptSvcTrail";
	...
	doOpenPiutang: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_svc='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._namaUser;
