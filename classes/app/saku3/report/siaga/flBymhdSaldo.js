// JavaScript Document
window.app_saku3_report_siaga_flBymhdSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_flBymhdSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_flBymhdSaldo";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Periode Penyelesaian","All",""));
...
window.app_saku3_report_siaga_flBymhdSaldo.extend(window.portalui_childForm);
window.app_saku3_report_siaga_flBymhdSaldo.implement({
	...
	doSelectCell : function(sender, col, row){
		...
		if (row == 1)
		{
			...
			var sql="select distinct periode from ju_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('BYMHD') "+
			...
			else
			{
				...
				this.filter = this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where") +
					  ...
					  this.filterRep.filterStr("a.no_ju",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," and ")+
							" and a.modul in ('BYMHD') ";
				this.filter2 =this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0);
				
				this.nama_report="server_report_saku3_siaga_rptBymhdSaldo";
				this.dbLib.execQuerySync(this.sql);	
