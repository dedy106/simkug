window.app_saku3_report_siaga_flAjuBMHD = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_flAjuBMHD.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_flAjuBMHD";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_pb_m where modul ='BYRBMHD' ");
...
window.app_saku3_report_siaga_flAjuBMHD.extend(window.childForm);
window.app_saku3_report_siaga_flAjuBMHD.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gr_pb_m where modul ='BYRBMHD' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_siaga_flAjuBMHD]::mainButtonClick:"+e);
		}
	},
