window.app_saku3_report_siaga_flSpbFormEmail = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siaga_flSpbFormEmail.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siaga_flSpbFormEmail";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from gr_spb2_m ");
...
window.app_saku3_report_siaga_flSpbFormEmail.extend(window.childForm);
window.app_saku3_report_siaga_flSpbFormEmail.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Penerimaan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_spb, a.keterangan from gr_spb2_m a "+this.filter,
												"select count(*) from gr_spb2_m a "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from gr_spb2_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter2 = this.app._namaUser+ "/" + this.app._userLog + "/" + this.app._userPwd + "/" + this.sg1.getCell(2,4)+ "/" + this.sg1.getCell(2,5);
			...
			systemAPI.alert("[app_saku3_report_siaga_flSpbFormEmail]::mainButtonClick:"+e);
		}
	},
