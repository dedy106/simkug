window.app_saku2_report_gar_flAggPos = function(owner)
{
	if (owner)
	{
		window.app_saku2_report_gar_flAggPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku2_report_gar_flAggPos";
		this.maximize();
...
window.app_saku2_report_gar_flAggPos.extend(window.childForm);
window.app_saku2_report_gar_flAggPos.implement({
	...
	doSelectCell: function(sender, col, row){
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["123","123","13","123"]);
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct(jenis) as periode from rra_pdrk_m "+
									   "where kode_lokasi='"+this.app._lokasi+"' order by jenis",[this.sg1.columns.get(2).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report="server_report_saku2_gar_rptAggPos";
				this.dbLib.execQuerySync(this.sql);	
