// JavaScript Document
window.app_saku3_report_gar_flAggRkapPp = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_gar_flAggRkapPp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_gar_flAggRkapPp";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,6,[0,1,2],["Status Anggaran","=","Reseting"]);
...
window.app_saku3_report_gar_flAggRkapPp.extend(window.portalui_childForm);
window.app_saku3_report_gar_flAggRkapPp.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5,6,7),new  Array("123","123","123","3","3","123","3","3"));
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5,6,7),new  Array(2,0,2,0,0,2,0,0));
		
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from anggaran_d where kode_lokasi='"+this.app._lokasi+"' order by substring(periode,1,4) desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2 = this.nik_user+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,4)+"/"+this.sg1.getCell(2,3)+"/"+this.app._namaForm+"/"+this.kode_lokasi+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,6)+"/"+this.sg1.getCell(2,7);
				
				this.nama_report="server_report_saku3_gar_rptAggRkapPp";
				//sql="call sp_agg_rkap_bulan_tu ('"+this.sg1.getCell(2,2)+"','L','"+this.sg1.getCell(2,1)+"','"+this.kode_lokasi+"','"+this.kode_lokasi1+"','"+this.kode_lokasi2+"','"+this.nik_user+"')";
	...
	doOpenTb: function(kode_neraca,kode_lokasi){
		...
		this.nama_report="server_report_saku2_kopeg_gar_rptAggAkun";
	...
	doOpenTbDrk: function(kode_akun,kode_lokasi){
		...
		this.nama_report="server_report_saku2_kopeg_gar_rptAggAkunDrk";
		this.viewer.useIframe(this.report.previewWithHeader(this.nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
