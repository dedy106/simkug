window.app_saku3_report_gar_flAggTahunPp = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_gar_flAggTahunPp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_gar_flAggTahunPp";
		this.maximize();
...
window.app_saku3_report_gar_flAggTahunPp.extend(window.portalui_childForm);
window.app_saku3_report_gar_flAggTahunPp.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["Bulanan"]);
		}
		if (row == 7)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			...
			else
			{
				...
				if (this.sg1.getCell(2,6)=="Bulanan")
				{
					...
					{
						var sql = "call sp_agg_real_bulan_pp ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					}
					else
					{
						var sql = "call sp_agg_real_bulan_pp_gl ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,5)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					}
					this.nama_report="server_report_saku3_gar_rptAggRealBulanPp";
				}
	...
	doOpenGar: function(kode_akun,kode_lokasi,tahun,kode_pp,kode_drk,tw){
		...
		nama_report="server_report_saku3_gar_rptAggBulanPp";
	...
	doOpenKartu: function(kode_akun,kode_lokasi,tahun,kode_pp,kode_drk,tw){
		...
		filter = "where a.kode_akun='"+kode_akun+"' and a.kode_lokasi='"+kode_lokasi+"' and substring(a.periode,1,4)='"+tahun+"' and a.kode_pp='"+kode_pp+"' ";
		filter2=this.sg1.getCell(2,5)+"/"+tahun+"////"+this.nik_user+"/"+this.sg1.getCell(2,7);
		//nama_report="server_report_saku2_gar_rptAggKpaAkunTrail";
		nama_report="server_report_saku3_gar_rptAggKpaAkunPp";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	},
