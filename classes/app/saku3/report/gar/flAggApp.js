window.app_saku3_report_gar_flAggApp = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_gar_flAggApp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_gar_flAggApp";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Modul","All",""]);
...
window.app_saku3_report_gar_flAggApp.extend(window.portalui_childForm);
window.app_saku3_report_gar_flAggApp.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			...
			var sql1="select a.no_app,a.keterangan from rra_app_m a "+this.filter;
			var sql2="select count(a.no_app) from rra_app_m a "+this.filter;
			this.filterRep.ListDataSGFilter(this, "Data Approval",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,["a.no_app","a.keterangan"],"and",["No Approval","Keterangan"]);
		}
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct(periode) as periode from rra_app_m "+
									   "where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct(modul) as periode from rra_app_m "+
