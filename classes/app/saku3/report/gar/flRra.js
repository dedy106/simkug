window.app_saku3_report_gar_flRra = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_gar_flRra.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_gar_flRra";
		this.maximize();
...
window.app_saku3_report_gar_flRra.extend(window.portalui_childForm);
window.app_saku3_report_gar_flRra.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report="server_report_saku3_gar_rptRra";
				/*
	...
	doOpenRRA: function(no_bukti,kode_lokasi){
		this.no_bukti=no_bukti;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = "where a.no_agg='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
		
		nama_report="server_report_saku2_gar_rptAggDis";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
