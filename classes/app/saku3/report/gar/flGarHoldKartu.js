window.app_saku3_report_gar_flGarHoldKartu = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_gar_flGarHoldKartu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_gar_flGarHoldKartu";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from anggaran_m where kode_lokasi='"+this.app._lokasi+"' and jenis='HOLD' ");
...
window.app_saku3_report_gar_flGarHoldKartu.extend(window.portalui_childForm);
window.app_saku3_report_gar_flGarHoldKartu.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 2)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Redis",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,["a.no_agg","a.keterangan"],"and",["No Anggaran","Keterangan"]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					...
					" and a.jenis='HOLD'";
			this.nama_report="server_report_saku3_gar_rptGarHoldKartu";	
