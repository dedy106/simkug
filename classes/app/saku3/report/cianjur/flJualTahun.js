window.app_saku3_report_cianjur_flJualTahun = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_cianjur_flJualTahun.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_cianjur_flJualTahun";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from apo_brg_jual_m  ");
...
window.app_saku3_report_cianjur_flJualTahun.extend(window.childForm);
window.app_saku3_report_cianjur_flJualTahun.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) as periode from apo_brg_jual_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_cianjur_rptJualTahun";
			...
			systemAPI.alert("[app_saku3_report_cianjur_flJualTahun]::mainButtonClick:"+e);
		}
	},
