window.app_saku3_report_cianjur_flKbJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_cianjur_flKbJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_cianjur_flKbJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_j where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_cianjur_flKbJurnal.extend(window.childForm);
window.app_saku3_report_cianjur_flKbJurnal.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 5)
			{
				...
				var sql1="select distinct a.no_bukti,a.no_dokumen from trans_j a "+filter+modul; 
				var sql2="select count(distinct a.no_bukti) from trans_j a "+filter+modul;
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
												new Array("a.no_bukti","a.no_dokumen"),"and",new Array("No Bukti","No Dokumen"),"and");
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123","23","13","123","123","123","3"]);
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct modul from trans_j where kode_lokasi='"+this.app._lokasi+"' order by modul",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 8)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				// this.nama_report="server_report_saku3_cianjur_rptKbJurnal";
