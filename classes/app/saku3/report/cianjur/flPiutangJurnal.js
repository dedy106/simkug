window.app_saku3_report_cianjur_flPiutangJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_cianjur_flPiutangJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_cianjur_flPiutangJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where no_bukti LIKE '%PU%'  ");
...
window.app_saku3_report_cianjur_flPiutangJurnal.extend(window.childForm);
window.app_saku3_report_cianjur_flPiutangJurnal.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pemesanan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_bukti, a.keterangan from trans_m a "+filter+" and a.no_bukti LIKE '%PU%' ",
												"select count(a.no_bukti) from trans_m a "+filter+" and a.no_bukti LIKE '%PU%' ",
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where no_bukti LIKE '%PU%' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_cianjur_flPiutangJurnal]::mainButtonClick:"+e);
		}
	},
