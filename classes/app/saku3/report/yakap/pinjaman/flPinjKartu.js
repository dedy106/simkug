window.app_saku3_report_yakap_pinjaman_flPinjKartu = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakap_pinjaman_flPinjKartu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakap_pinjaman_flPinjKartu";
		this.maximize();
...
window.app_saku3_report_yakap_pinjaman_flPinjKartu.extend(window.childForm);
window.app_saku3_report_yakap_pinjaman_flPinjKartu.implement({
		...
		try{			
			...
			if (row == 1)
			{
				this.filterRep.ListDataSGFilter(this, "Data Anggota",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.no_agg, a.nama from kop_agg a   ",
														  "select count(a.no_agg) from kop_agg a ",
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  ...
							  this.filterRep.filterStr("a.no_pinj",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				
				this.nama_report = "server_report_saku3_yakap_pinjaman_rptPinjKartu";
