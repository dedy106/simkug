window.app_saku3_report_optik_flPiutangAging = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_optik_flPiutangAging.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_optik_flPiutangAging";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from optik_app_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_optik_flPiutangAging.extend(window.childForm);
window.app_saku3_report_optik_flPiutangAging.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data optik_mitra",this.sg1, this.sg1.row, this.sg1.col,
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_app, a.catatan from optik_app_m a "+filter,
												"select count(a.no_app) from optik_app_m a "+filter,
												new Array("a.no_app","a.catatan"),"and",new Array("No Invoice","catatan"));
			}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123","1236","123","123","123","123","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[2,0,2,2,2,2,1]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from optik_app_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("d.kode_mitra",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_bukti",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCellDateValue(2,4);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_optik_rptPiutangAging";
			...
			systemAPI.alert("[app_saku3_report_optik_flPiutangAging]::mainButtonClick:"+e);
		}
	},
