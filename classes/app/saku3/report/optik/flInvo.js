window.app_saku3_report_optik_flInvo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_optik_flInvo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_optik_flInvo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from optik_tagih_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_optik_flInvo.extend(window.childForm);
window.app_saku3_report_optik_flInvo.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Bukti",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_tagih, a.keterangan from optik_tagih_m a "+this.filter,
												"select count(*) from optik_tagih_m a "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from optik_tagih_m where kode_lokasi='"+this.app._lokasi+"' and form='JMITRA' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_tagih",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_optik_rptInvo";
			...
			systemAPI.alert("[app_saku3_report_optik_flInvo]::mainButtonClick:"+e);
		}
	},
