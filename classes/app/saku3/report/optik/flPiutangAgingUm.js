window.app_saku3_report_optik_flPiutangAgingUm = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_optik_flPiutangAgingUm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_optik_flPiutangAgingUm";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where modul='AR' and form='JUMUM' and kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Tanggal","=",new Date().lclFormat()));
...
window.app_saku3_report_optik_flPiutangAgingUm.extend(window.childForm);
window.app_saku3_report_optik_flPiutangAgingUm.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where modul='AR' and form='JUMUM' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_optik_flPiutangAgingUm]::mainButtonClick:"+e);
		}
	},
