window.app_saku3_report_optik_flNotaUm = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_optik_flNotaUm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_optik_flNotaUm";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and form='JUMUM' ");
...
window.app_saku3_report_optik_flNotaUm.extend(window.childForm);
window.app_saku3_report_optik_flNotaUm.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and form='JUMUM' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_optik_rptNotaUm";
			...
			systemAPI.alert("[app_saku3_report_optik_flNotaUm]::mainButtonClick:"+e);
		}
	},
