window.app_saku3_report_optik_flNota = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_optik_flNota.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_optik_flNota";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and form='JMITRA' ");
...
window.app_saku3_report_optik_flNota.extend(window.childForm);
window.app_saku3_report_optik_flNota.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and form='JMITRA' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_optik_flNota]::mainButtonClick:"+e);
		}
	},
