window.app_saku3_report_kopeg_flSimpKartu = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_kopeg_flSimpKartu.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_kopeg_flSimpKartu";
		this.maximize();
...
window.app_saku3_report_kopeg_flSimpKartu.extend(window.childForm);
window.app_saku3_report_kopeg_flSimpKartu.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_simp,a.no_agg from kop_simp_d a "+this.filter,
												"select count(a.no_simp) from kop_simp_d a "+this.filter,
												new Array("a.no_simp","a.no_agg"),"and",new Array("No Bukti","Keterangan"));
			}		
		}catch(e){
			systemAPI.alert(e);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  ...
							  this.filterRep.filterStr("a.no_simp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				
				this.nama_report = "server_report_saku3_kopeg_rptSimpKartu";
