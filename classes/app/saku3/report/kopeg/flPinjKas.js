window.app_saku3_report_kopeg_flPinjKas = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_kopeg_flPinjKas.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_kopeg_flPinjKas";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul in ('PINJCAIR','PJANGS')");
...
window.app_saku3_report_kopeg_flPinjKas.extend(window.childForm);
window.app_saku3_report_kopeg_flPinjKas.implement({
		...
		try{			
			...
			if (row ==2)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							"and a.modul in ('PINJCAIR','PJANGS') ";
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_kopeg_rptPinjKas";
			...
			systemAPI.alert("[app_saku3_report_kopeg_flPinjKas]::mainButtonClick:"+e);
		}
	},
