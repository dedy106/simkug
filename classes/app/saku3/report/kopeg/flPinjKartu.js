window.app_saku3_report_kopeg_flPinjKartu = function(owner) {
    if (owner) {
        window.app_saku3_report_kopeg_flPinjKartu.prototype.parent.constructor.call(this, owner);
        this.className = "app_saku3_report_kopeg_flPinjKartu";
        this.maximize();
    ...
    this.periode = this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kop_pinj_m where kode_lokasi='" + this.app._lokasi + "' ");
    ...
    this.gridLib.SGEditData(this.sg1, 2, new Array(0, 1, 2), new Array("Anggota", "All", ""));
...
window.app_saku3_report_kopeg_flPinjKartu.extend(window.childForm);
window.app_saku3_report_kopeg_flPinjKartu.implement({
        ...
        try {
            ...
            if (row == 2) {
                ...
                this.filterRep.ListDataSGFilter(this, "Data Anggota", this.sg1, this.sg1.row, this.sg1.col,
                    "select a.no_agg, a.nama from kop_agg a " + filter,
                    "select count(a.no_agg) from kop_agg a " + filter, ["a.no_agg", "a.nama"], "and", ["Kode", "Nama"]);
            }
            ...
            if (row == 4) {
                this.filter = this.filterRep.filterStr("a.kode_lokasi", this.sg1.getCell(1, 0), this.sg1.getCell(2, 0), this.sg1.getCell(3, 0), "where") +
                    ...
                    this.filterRep.filterStr("a.no_agg", this.sg1.getCell(1, 2), this.sg1.getCell(2, 2), this.sg1.getCell(3, 2), "and") +
                    this.filterRep.filterStr("a.kode_param", this.sg1.getCell(1, 3), this.sg1.getCell(2, 3), this.sg1.getCell(3, 3), "and");
                this.filterRep.ListDataSGFilter(this, "Data", this.sg1, this.sg1.row, this.sg1.col,
                    "select a.no_pinj,a.no_agg from kop_pinj_m a " + this.filter,
                    "select count(a.no_pinj) from kop_pinj_m a " + this.filter,
                    new Array("a.no_pinj", "a.no_agg"), "and", new Array("No Bukti", "Keterangan"));
            }
        } catch (e) {
            systemAPI.alert(e);
        }
    ...
    doSelectCell: function(sender, col, row) {
        this.filterRep.setSGFilterRowTipe(this.sg1, row, [0, 1, 2, 3, 4], ["3", "23", "123", "13", "123"]);
        this.filterRep.setSGFilterRowButtonStyle(this.sg1, row, [0, 1, 2, 3, 4], [2, 0, 2, 0, 2]);
        if (row == 1) {
            this.sg1.columns.get(2).pickList.clear();
            this.dbLib.setItemsFromSQL("select distinct periode from kop_pinj_m where kode_lokasi='" + this.app._lokasi + "' order by periode", [this.sg1.columns.get(2).pickList]);
        }
        if (row == 3) {
            this.sg1.columns.get(2).pickList.clear();
            this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList, new Array("Y-Housing", "Y-Welfare"));
        }
    },
    mainButtonClick: function(sender) {
        try {
            ...
            if (sender == this.app._mainForm.bBack) {
                ...
                if (this.link == "2") {
                    this.doOpenKartu(this.no_pinj, this.kode_lokasi);
                    this.link = "1";
                }
                ...
                this.nama_report = "server_report_saku3_kopeg_rptPinjKartu";
    ...
    doOpenKartu: function(no_pinj, kode_lokasi) {

        this.no_pinj = no_pinj;
        this.kode_lokasi = kode_lokasi;
        this.link = "1";
        filter = " where a.no_pinj='" + no_pinj + "' and a.kode_lokasi='" + kode_lokasi + "'";
        filter2 = this.sg1.getCell(2, 0);
        nama_report = "server_report_saku3_kopeg_rptPinjKartu";
    ...
    doOpenBill: function(no_bill, kode_lokasi) {
        this.no_bill = no_bill;
        this.kode_lokasi = kode_lokasi;
        this.link = "2";
        filter = "where a.no_bill='" + no_bill + "' and a.kode_lokasi='" + kode_lokasi + "' ";
        filter2 = "";
        nama_report = "server_report_saku3_kopeg_rptPinjBill";
        this.viewer.useIframe(this.report.previewWithHeader(nama_report, filter, this.page, this.pager, this.showFilter, this.lokasi, filter2) + "&resource=" + this.resourceId + "&fullId=" + this.getFullId());
    ...
    doOpenKas: function(no_kas, kode_lokasi) {
        ...
        filter2 = "";
        nama_report = "server_report_saku3_kopeg_rptPinjKas";
        this.viewer.useIframe(this.report.previewWithHeader(nama_report, filter, this.page, this.pager, this.showFilter, this.lokasi, filter2) + "&resource=" + this.resourceId + "&fullId=" + this.getFullId());
