window.app_saku3_report_kopeg_flSimpBatal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_kopeg_flSimpBatal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_kopeg_flSimpBatal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kop_simpangs_m where kode_lokasi='"+this.app._lokasi+"' and modul='BTLBILL'");
...
window.app_saku3_report_kopeg_flSimpBatal.extend(window.childForm);
window.app_saku3_report_kopeg_flSimpBatal.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Bukti",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_angs, a.keterangan from kop_simpangs_m a "+filter,
												"select count(a.no_angs) from kop_simpangs_m a "+filter,
												new Array("a.no_angs","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kop_simpangs_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBPIU'  order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_kopeg_rptSimpBatal";
			...
			systemAPI.alert("[app_saku3_report_kopeg_flSimpBatal]::mainButtonClick:"+e);
		}
	},
