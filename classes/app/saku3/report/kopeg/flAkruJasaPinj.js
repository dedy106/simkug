window.app_saku3_report_kopeg_flAkruJasaPinj = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_kopeg_flAkruJasaPinj.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_kopeg_flAkruJasaPinj";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(a.periode) as periode from kop_pinjangs_d a "+
	"inner join kop_pinj_m b on a.no_pinj=b.no_pinj and a.kode_lokasi=b.kode_lokasi "+
	"where a.kode_lokasi='"+this.app._lokasi+"' and b.no_kas <>'-' and b.status_aktif='1'  ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,3), new Array("Jenis","All",""));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_kopeg_flAkruJasaPinj.extend(window.childForm);
window.app_saku3_report_kopeg_flAkruJasaPinj.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["3","23","13"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,3],[2,0,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct (a.periode) as periode from kop_pinjangs_d a "+
			"inner join kop_pinj_m b on a.no_pinj=b.no_pinj and a.kode_lokasi=b.kode_lokasi "+
			"where a.kode_lokasi='"+this.app._lokasi+"' and b.no_kas <>'-' and b.status_aktif='1'  order by a.periode desc",[this.sg1.columns.get(2).pickList]);
		}
		...
		if (row == 2)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct b.kode_param "+
			"from kop_pinjangs_d a "+
			"inner join kop_pinj_m b on a.no_pinj=b.no_pinj and a.kode_lokasi=b.kode_lokasi "+filter+" and b.no_kas <>'-' and b.status_aktif='1'  ",[this.sg1.columns.get(2).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							  this.filterRep.filterStr("a.kode_param",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				
				this.nama_report = "server_report_saku3_kopeg_rptAkruJasaPinj";
