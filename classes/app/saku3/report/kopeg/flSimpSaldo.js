window.app_saku3_report_kopeg_flSimpSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_kopeg_flSimpSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_kopeg_flSimpSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kop_simpbill_m where kode_lokasi='"+this.app._lokasi+"' ");
	var anggota="";
...
window.app_saku3_report_kopeg_flSimpSaldo.extend(window.childForm);
window.app_saku3_report_kopeg_flSimpSaldo.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2],["3","23","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2],[2,0,2]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from kop_simpbill_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenKartu(this.no_simp,this.kode_lokasi);
					this.link="1";
				}
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_kopeg_rptSimpSaldo";
	...
	doOpenKartu: function(no_simp,kode_lokasi){
		
		this.no_simp=no_simp;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = " where a.no_simp='"+no_simp+"' and a.kode_lokasi='"+kode_lokasi+"'";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
		nama_report="server_report_saku3_kopeg_rptSimpKartu";
	...
	doOpenBill: function(no_bill,kode_lokasi){
		...
		filter2="";
		nama_report="server_report_saku3_kopeg_rptSimpBill";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenKas: function(no_kas,kode_lokasi,modul){
		this.no_kas=no_kas;
		this.kode_lokasi=kode_lokasi;
		this.link="2";
		if (modul=="BTLBILL")
		{
			filter = "where a.no_angs='"+no_kas+"' and a.kode_lokasi='"+kode_lokasi+"' ";
			filter2="";
			nama_report="server_report_saku3_kopeg_rptSimpBatal";
		}
		else
		{
			filter = "where a.no_kas='"+no_kas+"' and a.kode_lokasi='"+kode_lokasi+"' ";
			filter2="";
			nama_report="server_report_saku3_kopeg_rptSimpKas";
		}
