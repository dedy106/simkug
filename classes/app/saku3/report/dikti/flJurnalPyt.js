window.app_saku3_report_dikti_flJurnalPyt = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dikti_flJurnalPyt.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dikti_flJurnalPyt";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m  where form='MHSDIST' and kode_lokasi='"+this.app._lokasi+"' ");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Jenis","=","Jurnal"));
...
window.app_saku3_report_dikti_flJurnalPyt.extend(window.childForm);
window.app_saku3_report_dikti_flJurnalPyt.implement({
	...
	{		
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Jurnal","Rincian"));
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,3)=="Rincian") {
					this.nama_report = "server_report_saku3_dikti_rptDetailPyt";
				}
			...
			systemAPI.alert("[app_saku3_report_dikti_flJurnalPyt]::mainButtonClick:"+e);
		}
	},
