window.app_saku3_report_dikti_flBayarDaftarLoad = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dikti_flBayarDaftarLoad.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dikti_flBayarDaftarLoad";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='CDINREG'");
...
window.app_saku3_report_dikti_flBayarDaftarLoad.extend(window.childForm);
window.app_saku3_report_dikti_flBayarDaftarLoad.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_dikti_rptJurByr";
	...
	doOpenDetailByr: function(no_bukti,kode_lokasi){
		this.link="2";
		filter = "where no_bukti='"+no_bukti+"' and modul='CDINREG' and kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
		nama_report="server_report_saku3_dikti_rptSisBayarDaftar";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
