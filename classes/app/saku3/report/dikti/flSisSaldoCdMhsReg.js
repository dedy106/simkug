window.app_saku3_report_dikti_flSisSaldoCdMhsReg = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dikti_flSisSaldoCdMhsReg.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dikti_flSisSaldoCdMhsReg";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from dikti_cd where kode_lokasi='"+this.app._lokasi+"' ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("NIS","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Output Laporan","=","Layar"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_dikti_flSisSaldoCdMhsReg.extend(window.childForm);
window.app_saku3_report_dikti_flSisSaldoCdMhsReg.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 2)
			{
				...
				var sql1="select a.no_reg,a.nama from dikti_camaba a  "+filter;
				var sql2="select count(a.no_reg) from dikti_camaba a  "+filter;
				this.filterRep.ListDataSGFilter(this, "Data Calon Mahasiswa",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
	...
	{		
		...
		{
			var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where");
			var sql="select distinct a.periode from dikti_cd a "+filter+" order by a.periode desc"
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenKartu(this.nis,this.kode_lokasi);					
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_dikti_rptSisSaldoCdMhsReg";
	...
	doOpenKartu: function(nis,kode_lokasi){
		...
		this.link="1";
		filter = "where a.no_reg='"+nis+"' and a.kode_lokasi='"+kode_lokasi+"' ";
