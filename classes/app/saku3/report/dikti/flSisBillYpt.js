window.app_saku3_report_dikti_flSisBillYpt = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dikti_flSisBillYpt.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dikti_flSisBillYpt";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and form='MHSBILL' ");
	...
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("NIS","All",""));
...
window.app_saku3_report_dikti_flSisBillYpt.extend(window.childForm);
window.app_saku3_report_dikti_flSisBillYpt.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 5)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Siswa",this.sg1, this.sg1.row, this.sg1.col,
												"select a.nim, a.nama from dikti_mhs a "+filter,
												"select count(*) from dikti_mhs a "+filter,
												new Array("a.nim","a.nama"),"where",new Array("nim","nama"));
			}	
			if (row == 6)
			{
				var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where");
			...
			if (row == 7)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  ...
														  ["a.no_bukti","a.keterangan"],"and",["Kode","Nama"]);
			}		
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["3","3","123","123","123","123","123","123","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[2,0,2,2,2,2,2,2,0]);
		if (row == 1)
		{
			var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+" and a.form='MHSBILL' ";
			var sql="select distinct a.periode from trans_m a "+filter+" order by a.periode desc"
	...
	mainButtonClick: function(sender){
		try
		{
			...
			{
				this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
				this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Tingkat","All",""));
				this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kelas","All",""));
				this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("NIS","All",""));
			}else{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("c.kode_kelas",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("c.kode_akt",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							this.filterRep.filterStr("b.nim",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+
							this.filterRep.filterStr("c.kode_status",this.sg1.getCell(1,6),this.sg1.getCell(2,6).substr(0,1),this.sg1.getCell(3,6).substr(0,1),"and")+
							this.filterRep.filterStr("b.no_bill",this.sg1.getCell(1,7),this.sg1.getCell(2,7),this.sg1.getCell(3,7),"and")+
							"and a.form='MHSBILL' ";
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,8);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_dikti_rptSisBillYpt";
