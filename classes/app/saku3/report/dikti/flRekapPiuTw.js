window.app_saku3_report_dikti_flRekapPiuTw = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dikti_flRekapPiuTw.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dikti_flRekapPiuTw";
		this.maximize();
	...
	// this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("NIS","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Status PP","All",""));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Output Laporan","=","Layar"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_dikti_flRekapPiuTw.extend(window.childForm);
window.app_saku3_report_dikti_flRekapPiuTw.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			// 	var sql1="select a.nis,a.nama from sis_dikti a inner join sis_kelas b on a.kode_kelas=b.kode_kelas and a.kode_lokasi=b.kode_lokasi and a.kode_pp=b.kode_pp "+filter;
			// 	var sql2="select count(a.nis) from sis_dikti a inner join sis_kelas b on a.kode_kelas=b.kode_kelas and a.kode_lokasi=b.kode_lokasi and a.kode_pp=b.kode_pp "+filter;
			// 	this.filterRep.ListDataSGFilter(this, "Data dikti",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["3","123","3","13","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,2,0,0,0]);	
		if (row == 2)
		{
			var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where");
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("1. AKTIF"));
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							// this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							// this.filterRep.filterStr("f.kode_jur",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							// this.filterRep.filterStr("a.kode_kelas",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							// this.filterRep.filterStr("a.kode_akt",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+
							// this.filterRep.filterStr("a.nis",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and")+
							this.filterRep.filterStr("a.flag_aktif",this.sg1.getCell(1,3),this.sg1.getCell(2,3).substr(0,1),this.sg1.getCell(3,3).substr(0,1),"and");
				
				if(this.sg1.getCell(1,1) == "Range"){
					this.filter2 =this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,4)+"/"+" and x.kode_pp between '"+this.sg1.getCell(2,1)+"' and '"+this.sg1.getCell(3,1)+"' ";
				}else if (this.sg1.getCell(1,1) == "="){
					this.filter2 =this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,4)+"/"+" and x.kode_pp = '"+this.sg1.getCell(2,1)+"' ";
				}else if (this.sg1.getCell(1,1) == "All") {
					this.filter2 =this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,4)+"/"+" and x.kode_pp LIKE '%' ";
				}
				this.showFilter = this.filterRep.showFilter(this.sg1);
				
				
				this.nama_report = "server_report_saku3_dikti_rptRekapPiuTwPp";
	...
	doOpenDet: function(kode_lokasi,kode_pp,periode){

		this.kode_lokasi=kode_lokasi;
		this.kode_pp=kode_pp;
		this.periode=periode;
		this.link="1";
		filter = "where a.kode_lokasi='"+kode_lokasi+"' and a.kode_pp='"+kode_pp+"'  ";
		filter2 =kode_lokasi+"/"+kode_pp+"/"+periode+"/"+this.sg1.getCell(2,4);
		nama_report="server_report_saku3_dikti_rptRekapPiuTw";
	...
	doOpenBill: function(no_bukti,kode_lokasi,kode_pp,modul){
		...
		if (modul=="bil")
		{
			...
			nama_report="server_report_saku3_dikti_rptSisBillYpt";
		}
		else
		{
			filter2="where a.no_rekon='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' and a.kode_pp='"+kode_pp+"'"+"/";
			nama_report="server_report_saku3_dikti_rptSisRekonYpt";
		}
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenJurnal: function(no_bukti,kode_lokasi,kode_pp,modul){
		...
		if (modul=="bil")
		{
			...
			nama_report="server_report_saku3_dikti_rptSisJurnalPiutangYpt";
		}
		else
		{
			filter="where a.no_rekon='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' and a.kode_pp='"+kode_pp+"'";
			nama_report="server_report_saku3_dikti_rptSisJurnalRekonYpt";
		}
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
