window.app_saku3_report_dikti_flSisSaldoAging = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dikti_flSisSaldoAging.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dikti_flSisSaldoAging";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from dikti_bill_d where kode_lokasi='"+this.app._lokasi+"' ");
	...
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("NIM","All",""));
...
window.app_saku3_report_dikti_flSisSaldoAging.extend(window.childForm);
window.app_saku3_report_dikti_flSisSaldoAging.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 6)
			{
				...
				var sql1="select a.nim,a.nama from dikti_mhs a inner join dikti_kelas b on a.kode_kelas=b.kode_kelas and a.kode_lokasi=b.kode_lokasi "+filter;
				var sql2="select count(a.nim) from dikti_mhs a inner join dikti_kelas b on a.kode_kelas=b.kode_kelas and a.kode_lokasi=b.kode_lokasi "+filter;
	...
	{		
		...
		if (row == 2)
		{
			var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where");
						// this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and"); 
			var sql="select distinct a.periode from dikti_bill_d a "+filter+" order by a.periode desc"
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_kelas",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							// this.filterRep.filterStr("a.kode_akt",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+
							this.filterRep.filterStr("a.nim",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and");
				...
				this.nama_report = "server_report_saku3_dikti_rptSisSaldoAging";
	...
	doOpenKartu: function(nis,kode_lokasi){
		...
		this.link="1";
		filter = "where a.nim='"+nis+"' and a.kode_lokasi='"+kode_lokasi+"' ";
