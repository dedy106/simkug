window.app_saku3_report_dikti_flBayarDaftarLoad2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dikti_flBayarDaftarLoad2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dikti_flBayarDaftarLoad2";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"' and modul in ('KBBILLREG','KBREG') ");
...
window.app_saku3_report_dikti_flBayarDaftarLoad2.extend(window.childForm);
window.app_saku3_report_dikti_flBayarDaftarLoad2.implement({
		...
		try{			
			...
			if (row ==3)
			{
				var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
					...
					" and a.modul in ('KBBILLREG','KBREG') ";	
	...
	{		
		...
		if (row == 2)
		{
			...
			var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
			this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
					" and a.modul in ('LOAD','KBREG') ";
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct a.periode from sis_reg_bayar a  "+filter+" order by a.periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}		
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_dikti_rptSisBayarDaftarLoad";
