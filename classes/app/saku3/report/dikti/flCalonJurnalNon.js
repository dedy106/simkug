window.app_saku3_report_dikti_flCalonJurnalNon = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dikti_flCalonJurnalNon.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dikti_flCalonJurnalNon";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Data dikti", 2);
...
window.app_saku3_report_dikti_flCalonJurnalNon.extend(window.childForm);
window.app_saku3_report_dikti_flCalonJurnalNon.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Bukti",this.sg1, this.sg1.row, this.sg1.col,
														  ...
														  ["a.no_bukti","a.keterangan"],"and",["Kode","Keterangan"]);
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2],["3","13","123"]);
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_dikti_rptCalonJurnalNon";
	...
	doOpenDetailByr: function(no_bukti,kode_lokasi,nis){
		this.link="2";
		filter = "where a.no_bukti='"+no_bukti+"' and a.modul='CDOUT' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._lokasi+"/"+this.app._kodePP+"/"+this.app._periode;
		nama_report="server_report_saku3_dikti_rptSisByrCalonNon";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
