window.app_saku3_report_dikti_flMhsTarif = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dikti_flMhsTarif.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dikti_flMhsTarif";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select kode_ta as periode from dikti_ta where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.app._periode+"' ");
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Tahun Akademik","=",this.periode));
...
window.app_saku3_report_dikti_flMhsTarif.extend(window.childForm);
window.app_saku3_report_dikti_flMhsTarif.implement({
	...
	{		
		...
		{
			var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where");
			var sql="select distinct a.kode_ta from dikti_ta a "+filter+" order by a.kode_ta desc"
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_dikti_rptSisTarif2";
