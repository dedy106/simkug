window.app_saku3_report_dikti_flTarifParam = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dikti_flTarifParam.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dikti_flTarifParam";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(kode_akt) as periode from dikti_param_tarif where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_dikti_flTarifParam.extend(window.childForm);
window.app_saku3_report_dikti_flTarifParam.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct kode_akt from dikti_param_tarif where kode_lokasi='"+this.app._lokasi+"' order by kode_akt",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}	
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_dikti_rptTarifParam";
