window.app_saku3_report_dikti_flSisJurnalRekonYpt = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dikti_flSisJurnalRekonYpt.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dikti_flSisJurnalRekonYpt";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where form in ('MHSREKON','REKONCD') and kode_lokasi='"+this.app._lokasi+"'  ");
...
window.app_saku3_report_dikti_flSisJurnalRekonYpt.extend(window.childForm);
window.app_saku3_report_dikti_flSisJurnalRekonYpt.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Bukti",this.sg1, this.sg1.row, this.sg1.col,
												...
												"select count(*) from trans_m a "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where form in ('MHSREKON','REKONCD') and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_dikti_rptSisJurnalRekonYpt";
			...
			systemAPI.alert("[app_saku3_report_dikti_flSisJurnalRekonYpt]::mainButtonClick:"+e);
		}
	},
