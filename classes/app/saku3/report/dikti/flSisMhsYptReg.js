window.app_saku3_report_dikti_flSisMhsYptReg = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dikti_flSisMhsYptReg.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dikti_flSisMhsYptReg";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Angkatan","All",""));
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("No Registrasi","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Output Laporan","=","Layar"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_dikti_flSisMhsYptReg.extend(window.childForm);
window.app_saku3_report_dikti_flSisMhsYptReg.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 1)
			{
				...
				// var sql="select distinct a.kode_ta,b.nama from sis_siswareg a "+
				// "inner join sis_angkat b on a.kode_ta=b.kode_akt and a.kode_lokasi=b.kode_lokasi and a.kode_pp=b.kode_pp "+filter+" order by a.kode_ta desc ";

				// alert(sql);
				this.filterRep.ListDataSGFilter(this, "Data Angkatan",this.sg1, this.sg1.row, this.sg1.col,
				"select distinct a.kode_ta,b.nama from dikti_camaba a "+
				"inner join dikti_ta b on a.kode_ta=b.kode_ta and a.kode_lokasi=b.kode_lokasi  "+filter+" order by a.kode_ta desc ",
				"select count (distinct a.kode_ta) from dikti_camaba a "+
															"inner join dikti_ta b on a.kode_ta=b.kode_ta and a.kode_lokasi=b.kode_lokasi "+filter,
														  ["a.kode_ta","a.nama"],"and",["Kode","Nama"]);
			}
			if (row == 2)
			{
				var filter=this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("kode_ta",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				this.filterRep.ListDataSGFilter(this, "Data Calon Mahasiswa",this.sg1, this.sg1.row, this.sg1.col,
												"select no_reg, nama from dikti_camaba "+filter,
												"select count(*) from dikti_camaba "+filter,
												new Array("no_reg","nama"),"where",new Array("no_reg","nama"));
			}					
		}catch(e){
			systemAPI.alert(e);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.kode_ta",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_reg",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,3);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_dikti_rptSisMhsYptReg";
