window.app_saku3_report_dikti_flSisMutasiPdd = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dikti_flSisMutasiPdd.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dikti_flSisMutasiPdd";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"' and form='CDMUTMULTI'");
...
window.app_saku3_report_dikti_flSisMutasiPdd.extend(window.childForm);
window.app_saku3_report_dikti_flSisMutasiPdd.implement({
	...
	{		
		...
		if (row == 2)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct a.periode from trans_m a  "+filter+" order by a.periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}		
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_dikti_rptSisMutasiPdd";
