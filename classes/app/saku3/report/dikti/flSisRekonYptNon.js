window.app_saku3_report_dikti_flSisRekonYptNon = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dikti_flSisRekonYptNon.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dikti_flSisRekonYptNon";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where modul='KBBILLOAD' and kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"'");
...
window.app_saku3_report_dikti_flSisRekonYptNon.extend(window.childForm);
window.app_saku3_report_dikti_flSisRekonYptNon.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 5)
			{
				...
				var sql1="select a.nis,a.nama from sis_siswa a inner join dikti_kelas b on a.kode_kelas=b.kode_kelas and a.kode_lokasi=b.kode_lokasi  "+filter;
				var sql2="select count(a.nis) from sis_siswa a inner join dikti_kelas b on a.kode_kelas=b.kode_kelas and a.kode_lokasi=b.kode_lokasi  "+filter;
	...
	{		
		...
		if (row == 2)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where modul='KBBILLOAD' and kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 7)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							  this.filterRep.filterStr("a.periode",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							  this.filterRep.filterStr("c.kode_kelas",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+							  							  
							  ...
							  this.filterRep.filterStr("a.no_kas",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and")+"/"+this.sg1.getCell(2,7)+"/"+this.sg1.getCell(2,2);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				
				
				this.nama_report = "server_report_saku3_dikti_rptSisRekonYptNon";
