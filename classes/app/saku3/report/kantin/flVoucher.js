window.app_saku3_report_kantin_flVoucher = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_kantin_flVoucher.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_kantin_flVoucher";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from ju_m where kode_lokasi='"+this.app._lokasi+"' and jenis='KUPON'");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Jenis Kupon","All",""));
...
window.app_saku3_report_kantin_flVoucher.extend(window.childForm);
window.app_saku3_report_kantin_flVoucher.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Jenis Kupon",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.kode_kupon, a.nama from kt_kupon_jenis a   "+
														   this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(a.kode_kupon) from kt_kupon_jenis a "+
														  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  ["a.kode_kupon","a.nama"],"and",["Kode","Nama"]);
			}
			...
			if (row ==3)
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.jenis='KUPON' ";
				this.filterRep.ListDataSGFilter(this, "Data Voucher",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_ju,a.keterangan from ju_m a "+this.filter,
												"select count(a.no_ju) from ju_m a "+this.filter,
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_kantin_rptVoucher";
