window.app_saku3_report_haji_flTitipSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_haji_flTitipSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_haji_flTitipSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from haj_reg where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Peserta","All",""));
...
window.app_saku3_report_haji_flTitipSaldo.extend(window.childForm);
window.app_saku3_report_haji_flTitipSaldo.implement({
		...
		try{			
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Peserta",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_peserta, nama from haj_peserta "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(no_peserta) from haj_peserta "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  new Array("no_peserta","nama"),"and");
			}
			...
			if (row ==3)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_peserta",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filterRep.ListDataSGFilter(this, "Data Bukti",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_reg, a.keterangan from haj_reg a "+filter,
												"select count(b.no_reg) from haj_reg a "+filter,
												new Array("a.no_reg","a.keterangan"),"and",new Array("No Invoice","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from haj_reg where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_haji_rptTitipSaldo";
			...
			systemAPI.alert("[app_saku3_report_haji_flTitipSaldo]::mainButtonClick:"+e);
		}
	},
	...
	doOpenPiutang: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_reg='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
