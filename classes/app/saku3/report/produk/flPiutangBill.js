window.app_saku3_report_produk_flPiutangBill = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_produk_flPiutangBill.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_produk_flPiutangBill";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"'");
	...
	this.gridLib.SGEditData(this.sg1,8,new Array(0,1,2), new Array("Status Mahasiswa","=","1. AKTIF"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	
};
window.app_saku3_report_produk_flPiutangBill.extend(window.childForm);
window.app_saku3_report_produk_flPiutangBill.implement({
		...
		try{			
			...
			if (row ==6)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_bukti,a.keterangan from trans_m a "+this.filter,
												"select count(a.no_bukti) from trans_m a "+this.filter,
												new Array("a.no_bukti","a.keterangan"),"and",new Array("No Tagihan","Keterangan"));
			}		
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["3","123","3","123","123","123","123","3","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[2,2,0,2,2,2,2,0,0]);	
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 7)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  ...
							  this.filterRep.filterStr("b.nis",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+
							  this.filterRep.filterStr("a.no_bill",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and");
				...
				this.nama_report = "server_report_saku3_produk_rptPiutangBill";
