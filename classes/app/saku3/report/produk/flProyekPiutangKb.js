window.app_saku3_report_produk_flProyekPiutangKb = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_produk_flProyekPiutangKb.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_produk_flProyekPiutangKb";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and form = 'KBPRPIU' ");
...
window.app_saku3_report_produk_flProyekPiutangKb.extend(window.childForm);
window.app_saku3_report_produk_flProyekPiutangKb.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and and form = 'KBPRPIU' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_produk_flProyekPiutangKb]::mainButtonClick:"+e);
		}
	},
