window.app_saku3_report_produk_tm_flJurnalReim = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_produk_tm_flJurnalReim.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_produk_tm_flJurnalReim";
		this.maximize();
...
window.app_saku3_report_produk_tm_flJurnalReim.extend(window.childForm);
window.app_saku3_report_produk_tm_flJurnalReim.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Reim",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_reim, a.keterangan from if_reim_m a "+this.filter,
												"select count(*) from if_reim_m a "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from if_reim_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_reim",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_produk_tm_rptJurnalReim";
			...
			systemAPI.alert("[app_saku3_report_produk_tm_flJurnalReim]::mainButtonClick:"+e);
		}
	},
