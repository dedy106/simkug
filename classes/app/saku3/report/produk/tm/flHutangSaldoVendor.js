window.app_saku3_report_produk_tm_flHutangSaldoVendor = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_produk_tm_flHutangSaldoVendor.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_produk_tm_flHutangSaldoVendor";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from tm_hutang_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_produk_tm_flHutangSaldoVendor.extend(window.childForm);
window.app_saku3_report_produk_tm_flHutangSaldoVendor.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from tm_hutang_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenSaldo(this.kode_vendor,this.kode_lokasi,this.periode);					
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_produk_tm_rptHutangSaldoVendor";
			...
			systemAPI.alert("[app_saku3_report_produk_tm_flHutangSaldoVendor]::mainButtonClick:"+e);
		}
	},
	...
	doOpenSaldo: function(kode_vendor,kode_lokasi,periode){
		this.kode_vendor=kode_vendor;
		this.kode_lokasi=kode_lokasi;
		this.periode=periode;
		this.link="1";
		filter = "where a.kode_vendor='"+kode_vendor+"' and a.kode_lokasi='"+kode_lokasi+"' and a.periode='"+periode+"'";
		filter2=kode_lokasi+"/"+periode+"/";
		nama_report="server_report_saku3_produk_tm_rptHutangSaldo";
	...
	doOpenPiutang: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_produk_tm_rptHutangJurnal";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
