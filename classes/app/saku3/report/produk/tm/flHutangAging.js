window.app_saku3_report_produk_tm_flHutangAging = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_produk_tm_flHutangAging.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_produk_tm_flHutangAging";
		this.maximize();
...
window.app_saku3_report_produk_tm_flHutangAging.extend(window.childForm);
window.app_saku3_report_produk_tm_flHutangAging.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_hutang, a.keterangan from tm_hutang_m a "+filter,
												"select count(a.no_hutang) from tm_hutang_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from tm_hutang_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_produk_tm_rptHutangAging";
			...
			systemAPI.alert("[app_saku3_report_produk_tm_flHutangAging]::mainButtonClick:"+e);
		}
	},
