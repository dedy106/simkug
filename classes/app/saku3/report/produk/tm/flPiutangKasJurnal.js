window.app_saku3_report_produk_tm_flPiutangKasJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_produk_tm_flPiutangKasJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_produk_tm_flPiutangKasJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBPIU'");
...
window.app_saku3_report_produk_tm_flPiutangKasJurnal.extend(window.childForm);
window.app_saku3_report_produk_tm_flPiutangKasJurnal.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBPIU'  order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_produk_tm_rptPiutangKasJurnal";
			...
			systemAPI.alert("[app_saku3_report_produk_tm_flPiutangKasJurnal]::mainButtonClick:"+e);
		}
	},
