window.app_saku3_report_produk_flProyekBebanKb = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_produk_flProyekBebanKb.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_produk_flProyekBebanKb";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and form = 'KBHUTPR' ");
...
window.app_saku3_report_produk_flProyekBebanKb.extend(window.childForm);
window.app_saku3_report_produk_flProyekBebanKb.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and form = 'KBHUTPR' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_produk_flProyekBebanKb]::mainButtonClick:"+e);
		}
	},
