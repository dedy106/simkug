window.app_saku3_report_produk_flJualMutasi = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_produk_flJualMutasi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_produk_flJualMutasi";
		this.maximize();
...
window.app_saku3_report_produk_flJualMutasi.extend(window.childForm);
window.app_saku3_report_produk_flJualMutasi.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and form='BRGJUAL' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_produk_rptJualMutasi";
			...
			systemAPI.alert("[app_saku3_report_produk_flJualMutasi]::mainButtonClick:"+e);
		}
	},
	...
	doOpenKas: function(periode,kode_lokasi){
		this.periode=periode;
		this.kode_lokasi=kode_lokasi;
		this.link="2";
		filter = "where a.periode='"+periode+"' and a.kode_lokasi='"+kode_lokasi+"' and a.form='BRGJUAL' ";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);;
		nama_report="server_report_saku3_produk_rptJualRekap";
