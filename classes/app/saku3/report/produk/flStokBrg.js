window.app_saku3_report_produk_flStokBrg = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_produk_flStokBrg.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_produk_flStokBrg";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Kelompok","All",""));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Kode Barang","All",""));
...
window.app_saku3_report_produk_flStokBrg.extend(window.childForm);
window.app_saku3_report_produk_flStokBrg.implement({
		...
		try{			
			...
			if (row ==2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Gudang",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_gudang,nama  from brg_gudang where kode_lokasi='"+this.app._lokasi+"'",
												"select count(*) from brg_gudang where kode_lokasi='"+this.app._lokasi+"'",
												new Array("kode_gudang","nama"),"where",new Array("kode_gudang","nama"));
			}
			if (row ==3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Kelompok",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_klp, nama from brg_barangklp ",
												"select count(*) from brg_barangklp ",
			...
			if (row ==4)
			{
				var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("c.kode_klp",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");							
				this.filterRep.ListDataSGFilter(this, "Data Barang",this.sg1, this.sg1.row, this.sg1.col,
														"select distinct a.kode_barang, c.nama from brg_stok a inner join brg_barang c on a.kode_barang=c.kode_barang "+filter,
														  "select count(a.kode_barang) from brg_stok a "+filter,
														  ["a.kode_barang","a.nama"],"and",["Kode","Nama"]);
			}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("c.kode_gudang",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("b.kode_klp",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.kode_barang",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_produk_rptStokBrg";
			...
			systemAPI.alert("[app_saku3_report_produk_flStokBrg]::mainButtonClick:"+e);
		}
	},
	...
	doOpenKartu: function(kode_barang,kode_gudang,kode_lokasi){
		
		this.kode_barang=kode_barang;
		this.kode_gudang=kode_gudang;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = "where a.kode_barang='"+kode_barang+"' and a.kode_lokasi='"+kode_lokasi+"' and a.kode_gudang='"+kode_gudang+"'";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser;;
		
		nama_report="server_report_saku3_produk_rptStokOp";
