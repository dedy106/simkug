window.app_saku3_report_produk_flGlJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_produk_flGlJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_produk_flGlJurnal";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Modul","All",""));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1), new Array("No Bukti","All"));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1), new Array("No Register","All"));
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Format Laporan","=","Daftar"));
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_produk_flGlJurnal.extend(window.childForm);
window.app_saku3_report_produk_flGlJurnal.implement({
		...
		try{			
			...
			if (row == 4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_bukti, keterangan from trans_m  "+filter ,
														  "select count(no_bukti) from trans_m "+filter,
	...
	{		
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123","123","123","123","123","123","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[2,0,2,0,2,2,0]);	
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["3","123","123","123","123","123","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[3,0,2,0,2,2,0]);	
		}
		if (row == 1)
		{
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct modul from gldt where kode_lokasi='"+this.app._lokasi+"' order by modul desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 6)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				var dokumen=this.filterRep.filterStr("a.no_dokumen",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5)," and ");
				...
				if (this.sg1.getCell(2,6)=="Daftar")
				{
					this.filter=filter+dokumen;
					this.nama_report="server_report_saku3_produk_rptGlJurnal";
				}
				if (this.sg1.getCell(2,6)=="Bukti")
				{
					this.filter=filter;
					this.nama_report="server_report_saku3_sju16_rptGlJurnalBukti";
				}
				if (this.sg1.getCell(2,6)=="Produksi")
				{
					this.filter=filter+dokumen;
					this.nama_report="server_report_saku3_sju16_rptGlJurnalProduksi";
				}
