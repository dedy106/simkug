window.app_saku3_report_produk_flKbTbDual = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_produk_flKbTbDual.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_produk_flKbTbDual";
		this.maximize();
	...
	this.akun=this.dbLib.getPeriodeFromSQL("select kode_akun as periode from flag_relasi where kode_lokasi='"+this.app._lokasi+"' and kode_flag='048' ");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Kode Akun","=",this.akun));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Tampil Mutasi Nol","=","Ya"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_produk_flKbTbDual.extend(window.childForm);
window.app_saku3_report_produk_flKbTbDual.implement({
		...
		try{			
			...
			if (row == 3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,
													...
													"where a.block= '0' and b.kode_flag='048' "+
	...
	{		
		...
		if (row == 2)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					...
					if (this.link2=="12")
					{
						this.doOpenMasuk(this.kode_akun,this.kode_lokasi,this.periode,this.kode_pp);
					}
					if (this.link2=="13")
					{
						this.doOpenKeluar(this.kode_akun,this.kode_lokasi,this.periode,this.kode_pp);
					}
				}
			...
			else
			{
				...
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,2)+"/"+this.app._nikUser+"/"+this.sg1.getCell(1,1)+"/"+this.sg1.getCell(2,4);
				this.nama_report="server_report_saku3_produk_rptKbTbDual";
	...
	doOpenBb: function(kode_akun,kode_lokasi,periode,kode_pp){
		...
		this.link="1";
		this.link2="11";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser+"/"+"Tidak/1";
		filter="where a.kode_lokasi='"+kode_lokasi+"' and a.kode_akun='"+kode_akun+"' and a.kode_pp='"+kode_pp+"'";
		nama_report="server_report_saku3_produk_rptKbBuku";
	...
	doOpenMasuk: function(kode_akun,kode_lokasi,periode,kode_pp){
		this.kode_akun=kode_akun;
		this.kode_lokasi=kode_lokasi;
		this.kode_pp=kode_pp;
		this.periode=periode;
		...
		this.link2="12";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser+"/"+"Tidak/1";
		filter="where a.kode_lokasi='"+kode_lokasi+"' and a.kode_akun='"+kode_akun+"' and c.kode_pp='"+kode_pp+"'";
		nama_report="server_report_saku3_produk_rptKbMasuk";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenKeluar: function(kode_akun,kode_lokasi,periode,kode_pp){
		this.kode_akun=kode_akun;
		this.kode_lokasi=kode_lokasi;
		this.kode_pp=kode_pp;
		this.periode=periode;
		...
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser+"/"+"Tidak/1";
		filter="where a.kode_lokasi='"+kode_lokasi+"' and a.kode_akun='"+kode_akun+"' and c.kode_pp='"+kode_pp+"'";
		nama_report="server_report_saku3_produk_rptKbKeluar";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenJurnal: function(no_bukti,kode_lokasi){
		...
		filter2="";
		nama_report="server_report_saku3_produk_rptKbJurnalBukti";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
