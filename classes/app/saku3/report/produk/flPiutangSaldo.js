window.app_saku3_report_produk_flPiutangSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_produk_flPiutangSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_produk_flPiutangSaldo";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Status Mahasiswa","=","1. AKTIF"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_produk_flPiutangSaldo.extend(window.childForm);
window.app_saku3_report_produk_flPiutangSaldo.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["3","23","3","123","123","123","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[2,2,0,2,2,2,0]);	
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from sis_bill_d where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 6)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_produk_rptPiutangSaldo";
