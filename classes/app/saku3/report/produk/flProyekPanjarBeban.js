window.app_saku3_report_produk_flProyekPanjarBeban = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_produk_flProyekPanjarBeban.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_produk_flProyekPanjarBeban";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from trans_m where form='PJKB' ");
...
window.app_saku3_report_produk_flProyekPanjarBeban.extend(window.childForm);
window.app_saku3_report_produk_flProyekPanjarBeban.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where form='PJKB' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_produk_flProyekPanjarBeban]::mainButtonClick:"+e);
		}
	},
