window.app_saku3_report_produk_flMapelGuru = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_produk_flMapelGuru.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_produk_flMapelGuru";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Mata Pelajaran","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Status","All",""));
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_produk_flMapelGuru.extend(window.childForm);
window.app_saku3_report_produk_flMapelGuru.implement({
		...
		try{			
			...
			if (row ==1)
			{
				this.filterRep.ListDataSGFilter(this, "Data Guru",this.sg1, this.sg1.row, this.sg1.col,
														  ...
														  "select count(a.nik) from karyawan a "+
														  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(2,0)," where "),
			...
			if (row ==2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Pelajaran",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.kode_matpel, a.nama from sis_matpel a  "+
														   this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(2,0)," where "),
														  "select count(a.kode_matpel) from sis_matpel a "+
														  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(2,0)," where "),
														  ["a.kode_matpel","a.nama"],"and",["Kode","Nama"]);
			}	
			if (row ==3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Status",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.kode_status, a.nama from sis_guru_status a  "+
														   this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(2,0)," where "),
														  "select count(a.kode_status) from sis_guru_status a "+
														  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(2,0)," where "),
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  ...
							  this.filterRep.filterStr("a.kode_matpel",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							  this.filterRep.filterStr("a.kode_status",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");

							  this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_produk_rptMapelGuru";
