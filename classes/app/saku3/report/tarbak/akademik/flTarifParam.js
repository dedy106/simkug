window.app_saku3_report_tarbak_akademik_flTarifParam = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tarbak_akademik_flTarifParam.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tarbak_akademik_flTarifParam";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Data Pajak", 2);
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(kode_akt) as periode from sis_param_tarif where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"'");
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Angkatan","=",this.periode));
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_tarbak_akademik_flTarifParam.extend(window.childForm);
window.app_saku3_report_tarbak_akademik_flTarifParam.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct kode_akt from sis_param_tarif where kode_lokasi='"+this.app._lokasi+"' order by kode_akt",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}	
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_tarbak_akademik_rptTarifParam";
