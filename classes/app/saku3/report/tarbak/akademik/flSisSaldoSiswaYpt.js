window.app_saku3_report_tarbak_akademik_flSisSaldoSiswaYpt = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tarbak_akademik_flSisSaldoSiswaYpt.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tarbak_akademik_flSisSaldoSiswaYpt";
		this.maximize();
	...
	var sql="select max(a.periode) as periode from ( "+
					"select distinct a.periode from sis_bill_m a where a.kode_lokasi='"+this.app._lokasi+"' "+
					" union "+
					"select distinct a.periode from sis_rekon_m a where a.kode_lokasi='"+this.app._lokasi+"' "+
					"	)a ";
...
window.app_saku3_report_tarbak_akademik_flSisSaldoSiswaYpt.extend(window.childForm);
window.app_saku3_report_tarbak_akademik_flSisSaldoSiswaYpt.implement({
	...
	{		
		...
		if (row == 2)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from sis_bill_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 6)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2 =this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,8)+"/"+this.app._nikUser;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				
				
				this.nama_report = "server_report_saku3_tarbak_akademik_rptSisSaldoSiswaYpt";
	...
	doOpenKartu: function(nis,kode_lokasi,kode_pp){
		...
		nama_report="server_report_saku3_tarbak_akademik_rptSisKartuYpt";
	...
	doOpenBill: function(no_bukti,kode_lokasi,kode_pp,modul){
		...
		if (modul=="bil")
		{
			...
			nama_report="server_report_saku3_tarbak_akademik_rptSisBillYpt";
		}
		else
		{
			filter2="where a.no_rekon='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' and a.kode_pp='"+kode_pp+"'"+"/";
			nama_report="server_report_saku3_tarbak_akademik_rptSisRekonYpt";
		}
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenJurnal: function(no_bukti,kode_lokasi,kode_pp,modul){
		...
		if (modul=="bil")
		{
			...
			nama_report="server_report_saku3_tarbak_akademik_rptSisJurnalPiutangYpt";
		}
		else
		{
			filter="where a.no_rekon='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' and a.kode_pp='"+kode_pp+"'";
			nama_report="server_report_saku3_tarbak_akademik_rptSisJurnalRekonYpt";
		}
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
