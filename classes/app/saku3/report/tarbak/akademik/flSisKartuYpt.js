window.app_saku3_report_tarbak_akademik_flSisKartuYpt = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tarbak_akademik_flSisKartuYpt.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tarbak_akademik_flSisKartuYpt";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Sis Kartu ", 2);
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sis_bill_d where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_tarbak_akademik_flSisKartuYpt.extend(window.childForm);
window.app_saku3_report_tarbak_akademik_flSisKartuYpt.implement({
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7,8],["3","23","123","123","123","123","123","13","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6,7,8],[2,2,0,2,2,2,0,2,0]);	
		if (row == 2)
		{
			var filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where");
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_kelas",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							this.filterRep.filterStr("a.kode_akt",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+
							this.filterRep.filterStr("a.flag_aktif",this.sg1.getCell(1,6),this.sg1.getCell(2,6).substr(0,1),this.sg1.getCell(3,6).substr(0,1),"and")+
							this.filterRep.filterStr("a.nis",this.sg1.getCell(1,7),this.sg1.getCell(2,7),this.sg1.getCell(3,7),"and");

				this.filter3 = this.filterRep.filterStr("a.periode",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"where");
				
				this.filter2 =this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,8)+"/"+this.filter3+"/"+this.sg1.getCell(3,2);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				
				
				this.nama_report = "server_report_saku3_tarbak_akademik_rptSisKartuYpt";
	...
	doOpenKartu: function(nis,kode_lokasi,kode_pp){
		...
		nama_report="server_report_saku3_siswa_rptSisKartuCd";
	...
	doOpenDetRekonBill: function(no_bukti,kode_lokasi,modul,nis,kode_pp){
		...
		if (modul=="BILL")
		{
			filter="where a.no_bill='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' and a.nis='"+nis+"' and a.kode_pp='"+kode_pp+"' ";
			filter2=modul+"/"+" ";
			nama_report="server_report_saku3_tarbak_akademik_rptSisDetKartuYpt";
		}
		else
		{
			filter="where a.no_rekon='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' and a.nis='"+nis+"' and a.kode_pp='"+kode_pp+"'";
			filter2=modul+"/"+"where a.no_bukti='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' and a.nis='"+nis+"' and a.kode_pp='"+kode_pp+"'";
			nama_report="server_report_saku3_tarbak_akademik_rptSisDetKartuYpt";
		}
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
