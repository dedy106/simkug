window.app_saku3_report_siswa_flMapelGuru = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flMapelGuru.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flMapelGuru";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Data Siswa", 2);
	...
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Kode PP","All",this.app._kodePP));
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("NIK","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Mata Pelajaran","All",""));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Status","All",""));
	
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_siswa_flMapelGuru.extend(window.childForm);
window.app_saku3_report_siswa_flMapelGuru.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Guru",this.sg1, this.sg1.row, this.sg1.col,
												"select a.nik, a.nama from karyawan a "+filter,
												"select count(a.nik) from karyawan a "+filter,
												new Array("a.nik","a.nama"),"and",new Array("Kode","Keterangan"));				
			...
			if (row ==3)
			{
				...
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				this.filterRep.ListDataSGFilter(this, "Data Pelajaran",this.sg1, this.sg1.row, this.sg1.col,
												"select a.kode_matpel, a.nama from sis_matpel a "+filter,
												"select count(a.kode_matpel) from sis_matpel a "+filter,
												new Array("a.kode_matpel","a.nama"),"and",new Array("Kode","Keterangan"));				
			}	
			if (row ==4)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
				...
				this.filterRep.ListDataSGFilter(this, "Data Status",this.sg1, this.sg1.row, this.sg1.col,
												"select a.kode_status, a.nama from sis_guru_status a "+filter,
												"select count(a.kode_status) from sis_guru_status a "+filter,
												new Array("a.kode_status","a.nama"),"and",new Array("Kode","Keterangan"));				
			}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  ...
							  this.filterRep.filterStr("a.kode_matpel",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							  this.filterRep.filterStr("a.kode_status",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");

							  this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_siswa_rptMapelGuru";
