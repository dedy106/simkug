window.app_saku3_report_siswa_flSisRekonJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_siswa_flSisRekonJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_siswa_flSisRekonJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sis_rekon_m  ");
...
window.app_saku3_report_siswa_flSisRekonJurnal.extend(window.childForm);
window.app_saku3_report_siswa_flSisRekonJurnal.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from sis_rekon_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_siswa_rptSisRekonJurnal";
			...
			systemAPI.alert("[app_saku3_report_siswa_flSisRekonJurnal]::mainButtonClick:"+e);
		}
	},
