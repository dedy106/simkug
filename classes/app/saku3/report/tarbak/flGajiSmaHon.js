window.  app_saku3_report_tarbak_flGajiSmaHon = function(owner)
{
	if (owner)
	{
		window.  app_saku3_report_tarbak_flGajiSmaHon.prototype.parent.constructor.call(this,owner);
		this.className = "  app_saku3_report_tarbak_flGajiSmaHon";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Jenis","=","Gaji"]);
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.  app_saku3_report_tarbak_flGajiSmaHon.extend(window.childForm);
window.  app_saku3_report_tarbak_flGajiSmaHon.implement({
		...
		try{			
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Karyawan",this.sg1, this.sg1.row, this.sg1.col,
													"select a.nik, a.nama from hr_karyawan a where a.kode_sdm='GH' and a.kode_unit='SMA' ",
													"select count(*) from hr_karyawan a where a.kode_sdm='GH' and a.kode_unit='SMA' ",
	...
	doSelectCell: function(sender, col, row){		
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Gaji","Tambahan"));
		}
		...
		try
		{
			...
			else
			{
				...
				else
				{
					...
					if (this.sg1.getCell(2,3)=="Gaji")
				{
					this.nama_report="server_report_saku3_tarbak_rptGajiSma3";
				}
				else
				{
					this.nama_report="server_report_saku3_tarbak_rptGajiSma4";
				}	
