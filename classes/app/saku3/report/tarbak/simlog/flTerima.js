window.app_saku3_report_tarbak_simlog_flTerima = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tarbak_simlog_flTerima.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tarbak_simlog_flTerima";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from log_terima_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_tarbak_simlog_flTerima.extend(window.childForm);
window.app_saku3_report_tarbak_simlog_flTerima.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_terima, a.keterangan from log_terima_m a "+this.filter,
												"select count(*) from log_terima_m a "+this.filter,
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_tarbak_simlog_rptTerima";
			...
			systemAPI.alert("[app_saku3_report_tarbak_simlog_flTerima]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.log_terima_m='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
