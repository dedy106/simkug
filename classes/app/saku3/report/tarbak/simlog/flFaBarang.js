// JavaScript Document
window.app_saku3_report_tarbak_simlog_flFaBarang = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tarbak_simlog_flFaBarang.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tarbak_simlog_flFaBarang";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Jenis","All",""));
...
window.app_saku3_report_tarbak_simlog_flFaBarang.extend(window.portalui_childForm);
window.app_saku3_report_tarbak_simlog_flFaBarang.implement({
	doEllipseClick: function(sender, col, row){
        ...
        if (row == 1)
        {
            this.filterRep.ListDataSGFilter(this, "Data Jenis Barang",this.sg1, this.sg1.row, this.sg1.col,
                                            "select kode_klpakun,nama from fa_klpakun "+
                                            this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+" order by kode_klpakun",
                                            "select count(kode_klpakun) from fa_klpakun "+
                                            this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
                                            new Array("kode_klpakun","nama"),"where");
        }	
		if (row == 2)
		{
            filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.kode_klpakun",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
                            
			this.filterRep.ListDataSGFilter(this, "Data Barang",this.sg1, this.sg1.row, this.sg1.col,
													  "select kode_klpfa,nama from fa_klp "+filter+" order by kode_klpfa",
													  "select count(kode_klpfa) from fa_klp "+filter,
                                                      new Array("kode_klpfa","nama"),"and",new Array("Kode","Nama"));
        }	
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.nama_report="server_report_saku3_tarbak_simlog_rptFaBarang";
