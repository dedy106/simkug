window.app_saku3_report_tarbak_simlog_flPbLogPBAdk = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tarbak_simlog_flPbLogPBAdk.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tarbak_simlog_flPbLogPBAdk";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from pbh_pb_m where kode_lokasi='"+this.app._lokasi+"' and modul='PBADK'");
...
window.app_saku3_report_tarbak_simlog_flPbLogPBAdk.extend(window.childForm);
window.app_saku3_report_tarbak_simlog_flPbLogPBAdk.implement({
	...
	{		
		...
		else
		{
			...
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,3,2,1]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from pbh_pb_m where modul='PBADK' and kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_pb",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+tanggal+
							" and a.modul='PBADK' ";
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_tarbak_simlog_rptPbLogPBAdk";
			...
			systemAPI.alert("[app_saku3_report_tarbak_simlog_flPbLogPBAdk]::mainButtonClick:"+e);
		}
	},
