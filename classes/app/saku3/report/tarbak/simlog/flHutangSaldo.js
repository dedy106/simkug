window.app_saku3_report_tarbak_simlog_flHutangSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tarbak_simlog_flHutangSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tarbak_simlog_flHutangSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from hutang_m where kode_lokasi='"+this.app._lokasi+"' and modul='LOGBAST'");
...
window.app_saku3_report_tarbak_simlog_flHutangSaldo.extend(window.childForm);
window.app_saku3_report_tarbak_simlog_flHutangSaldo.implement({
		...
		try{			
			...
			if (row ==3)
			{
                this.filterRep.ListDataSGFilter(this, "Data Vendor",this.sg1, this.sg1.row, this.sg1.col,
                                                ...
                                                this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+" order by kode_vendor",
            ...
            if(row == 4){
                this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.modul in ('LOGBAST','LOGADK') ";
				this.filterRep.ListDataSGFilter(this, "Data Hutang",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_hutang, a.keterangan from hutang_m a "+this.filter,
												"select count(*) from hutang_m a "+this.filter,
	...
	{		
		if (this.app._userStatus=="A")
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","36","123","123","123"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,2,2,2]);	
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","36","3","123","123"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,3,2,2]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from hutang_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							" and a.modul in ('LOGBAST','LOGADK') ";
				this.filter2 = this.app._lokasi+"/"+this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_tarbak_simlog_rptHutangSaldo";
			...
			systemAPI.alert("[app_saku3_report_tarbak_simlog_flHutangSaldo]::mainButtonClick:"+e);
		}
	},
