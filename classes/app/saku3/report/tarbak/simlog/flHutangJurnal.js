window.app_saku3_report_tarbak_simlog_flHutangJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tarbak_simlog_flHutangJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tarbak_simlog_flHutangJurnal";
		this.maximize();
...
window.app_saku3_report_tarbak_simlog_flHutangJurnal.extend(window.childForm);
window.app_saku3_report_tarbak_simlog_flHutangJurnal.implement({
		...
		try{			
			...
			if (row == 2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_hutang, keterangan from hutang_m  "+filter ,
														  "select count(no_hutang) from hutang_m "+filter,
														  new Array("no_hutang","keterangan"),"and");
            }
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from hutang_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_saku3_tarbak_simlog_rptHutangJurnal";
