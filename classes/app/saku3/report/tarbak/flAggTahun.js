window.app_saku3_report_tarbak_flAggTahun = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_tarbak_flAggTahun.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_tarbak_flAggTahun";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,4,[0,1,2],["Periodik Laporan","=","Triwulan"]);
	this.gridLib.SGEditData(this.sg1,5,[0,1,2],["Realisasi","=","General Ledger"]);
...
window.app_saku3_report_tarbak_flAggTahun.extend(window.portalui_childForm);
window.app_saku3_report_tarbak_flAggTahun.implement({
		...
		try
		{
			...
			else
			{
				...
				if (this.sg1.getCell(2,4)=="Triwulan")
				{
					...
					{
						var sql = "call sp_agg_real_tw ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,3)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					}
					else
					{
						var sql = "call sp_agg_real_tw_gl ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,3)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					}
					this.nama_report="server_report_saku3_tarbak_rptAggRealTw";
				}
				if (this.sg1.getCell(2,4)=="Semester")
				{
					if (this.sg1.getCell(2,5)=="Anggaran")
					{
						var sql = "call sp_agg_real_sem ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,3)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					}
					else
					{
						var sql = "call sp_agg_real_sem_gl ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,3)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					}
					this.nama_report="server_report_saku3_tarbak_rptAggRealSem";
				}
				if (this.sg1.getCell(2,4)=="Bulanan")
				{
					if (this.sg1.getCell(2,5)=="Anggaran")
					{
						var sql = "call sp_agg_real_bulan ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,3)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					}
					else
					{
						var sql = "call sp_agg_real_bulan_gl ('"+this.sg1.getCell(2,0)+"','"+this.sg1.getCell(2,3)+"','"+this.sg1.getCell(2,1)+"','"+this.nik_user+"')";
					}
					this.nama_report="server_report_saku3_tarbak_rptAggRealBulan";
				}
	...
	doOpenKartu: function(kode_akun,kode_lokasi,tahun){
		...
		this.tahun=tahun;
		this.link="1";
		var sql = "call sp_angg_d ('"+kode_lokasi+"','"+tahun+"','"+this.app._nikUser+"')";
		this.dbLib.execQuerySync(sql);	
		filter = "where a.kode_akun='"+kode_akun+"' and a.kode_lokasi='"+kode_lokasi+"' and substring(a.periode,1,4)='"+tahun+"' ";
		filter2=tahun+"/"+this.sg1.getCell(2,5)+"/"+this.app._nikUser;
		nama_report="server_report_saku2_siaga_rptAggKpaAkunTrail";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	},
