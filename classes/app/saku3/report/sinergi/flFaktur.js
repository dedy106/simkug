window.app_saku3_report_sinergi_flFaktur = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sinergi_flFaktur.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sinergi_flFaktur";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from si_so_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_sinergi_flFaktur.extend(window.childForm);
window.app_saku3_report_sinergi_flFaktur.implement({
		...
		try{			
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Customer",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_cust, nama from si_cust "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(kode_cust) from si_cust "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_so, a.keterangan from si_so_m a "+filter,
												"select count(a.no_so) from si_so_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from si_so_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_sinergi_rptFaktur";
			...
			systemAPI.alert("[app_saku3_report_sinergi_flFaktur]::mainButtonClick:"+e);
		}
	},
