window.app_saku3_report_sinergi_flPo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sinergi_flPo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sinergi_flPo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from si_po_m where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Suplier","All",""));
...
window.app_saku3_report_sinergi_flPo.extend(window.childForm);
window.app_saku3_report_sinergi_flPo.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Suplier",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_vendor, nama from si_vendor "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  "select count(kode_vendor) from si_vendor "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_so, a.keterangan from si_po_m a "+filter,
												"select count(a.no_so) from si_po_m a "+filter,
												new Array("a.no_so","a.keterangan"),"and",new Array("No Invoice","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from si_po_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_sinergi_rptPo";
			...
			systemAPI.alert("[app_saku3_report_sinergi_flPo]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBayar: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter = "where a.no_so='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
