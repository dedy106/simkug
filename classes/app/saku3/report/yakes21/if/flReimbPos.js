window.app_saku3_report_yakes21_if_flReimbPos = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_if_flReimbPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_if_flReimbPos";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from if_reim_m where kode_lokasi='"+this.app._lokasi+"'");
	
	if (this.app._userStatus=="A" && this.app._lokasi=="99") this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","All",""));
	else this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
	...
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Status Bayar","All",""));
...
window.app_saku3_report_yakes21_if_flReimbPos.extend(window.childForm);
window.app_saku3_report_yakes21_if_flReimbPos.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			if (row ==0)
			{
				if (this.app._userStatus=="A" && this.app._lokasi=="99") {
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data PP",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_pp, nama from pp where tipe='Posting' and kode_bidang='"+this.bidang+"' "+
														   this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," and "),
														  "select count(kode_pp) from pp where tipe='Posting' and kode_bidang='"+this.bidang+"' "+
														  this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," and "),
			...
			if (row == 3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Karyawan",this.sg1, this.sg1.row, this.sg1.col,
														  ...
														  "select count(a.nik_buat) "+
															"from if_reim_m a "+
															"inner join karyawan b on a.nik_buat=b.nik "+
														  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where "),
														  new Array("a.nik_buat","a,nama"),"and");
			}
			if (row ==4)
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.nik_user",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				
				this.filterRep.ListDataSGFilter(this, "Data Reimburse",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_reim, a.keterangan from if_reim_m a "+
												"inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter,
												"select count(*) from if_reim_m a "+
												"inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter,
												new Array("a.no_reim","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		if (this.app._userStatus=="A" && this.app._lokasi=="99") {
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["123","123","123","123","123","13"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,0,2,2,2,0]);	
		}
		else {
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["3","123","123","123","123","13"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,0,2,2,2,0]);	
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from if_reim_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 5)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("BAYAR","BELUM"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,5);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_yakes21_if_rptReimbPosisi";
			...
			systemAPI.alert("[app_saku3_report_yakes21_if_flReimbPos]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter2 = "";
		filter=" where a.no_reim='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"'";
		nama_report="server_report_saku3_sapyakes_rptReimbForm";
	...
	doOpenPb: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_yakes21_if_rptReimbForm";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenVer: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_yakes21_pbh_rptVer";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenSpb: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_yakes21_pbh_rptSpb";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenDok: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_yakes21_ypt_rptDetailDok";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenNota: function(no_bukti,kode_lokasi){
		this.link="1";
		filter = "where a.no_bukti='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._periode;
		nama_report="server_report_saku3_yakes21_ypt_rptKbTransfer";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
	...
	doOpenKas: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_yakes21_pbh_rptKbPb";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
