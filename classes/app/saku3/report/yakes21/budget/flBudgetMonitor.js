window.app_saku3_report_yakes21_budget_flBudgetMonitor = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_budget_flBudgetMonitor.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_budget_flBudgetMonitor";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from anggaran_d where kode_lokasi='"+this.app._lokasi+"'");
	...
	if (this.app._userStatus=="A") this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Kode PP","All",""]);
	else this.gridLib.SGEditData(this.sg1,3,[0,1,2],["Kode PP","=",this.app._kodePP]);
	this.gridLib.SGEditData(this.sg1,4,[0,1,2],["Kode DRK","All",""]);
	this.gridLib.SGEditData(this.sg1,5,[0,1,2],["Output Laporan","=","Layar"]);
	this.gridLib.SGEditData(this.sg1,6,[0,1,2],["Versi Laporan","=","Versi 2(GL)"]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	
};
window.app_saku3_report_yakes21_budget_flBudgetMonitor.extend(window.childForm);
window.app_saku3_report_yakes21_budget_flBudgetMonitor.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			var filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and");
				...
				if (this.app._userStatus=="A") {
					var sql = "select a.kode_pp,a.nama from pp a where a.tipe ='Posting' "+filter;
					var sql2 = "select count(*) from pp a where a.tipe ='Posting' "+filter;
				}
				else {
					var sql = "select a.kode_pp,a.nama from pp a inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.nik='"+this.app._userLog+"' where a.tipe ='Posting' "+filter;
					var sql2 = "select count(*) from pp a inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.nik='"+this.app._userLog+"' where a.tipe ='Posting' "+filter;
				}
	...
	doSelectCell: function(sender, col, row){
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6,7],["3","3","123","13","123","3","3","3"]);
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Versi 2(GL)","Versi 2(KPA)"));//,"Versi 1"
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report="server_report_saku3_yakes21_budget_rptBudgetMonitor";
				if (this.sg1.getCell(2,6)=="Versi 2(GL)") {
					this.nama_report="server_report_saku3_yakes21_budget_rptBudgetMonitor2";
				}
				if (this.sg1.getCell(2,6)=="Versi 2(KPA)") {
					this.nama_report="server_report_saku3_yakes21_budget_rptBudgetMonitor2gar";
				}
				this.dbLib.execQuerySync(this.sql);	
