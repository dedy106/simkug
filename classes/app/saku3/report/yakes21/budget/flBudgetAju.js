window.app_saku3_report_yakes21_budget_flBudgetAju = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_budget_flBudgetAju.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_budget_flBudgetAju";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,2,[0,1,2],["Jenis Anggaran","All",""]);
...
window.app_saku3_report_yakes21_budget_flBudgetAju.extend(window.portalui_childForm);
window.app_saku3_report_yakes21_budget_flBudgetAju.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 3)
		{
			this.filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					...
					this.filterRep.filterStr("a.sts_pdrk",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
			...
			this.filterRep.ListDataSGFilter(this, "Data Redis",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,["a.no_pdrk","a.keterangan"],"and",["No Anggaran","Keterangan"]);
		}
	...
	doSelectCell: function(sender, col, row){
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["13","123","13","123"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3],[2,0,0,2]);
		}else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["3","123","13","123"]);
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct(sts_pdrk) as periode from rra_pdrk_m "+
									   "where kode_lokasi='"+this.app._lokasi+"' order by sts_pdrk",[this.sg1.columns.get(2).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			this.filter=this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
					this.filterRep.filterStr("a.sts_pdrk",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
					this.filterRep.filterStr("a.no_pdrk",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
			this.nama_report="server_report_saku3_yakes21_budget_rptBudgetAju";	
