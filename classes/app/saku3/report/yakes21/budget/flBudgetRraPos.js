window.app_saku3_report_yakes21_budget_flBudgetRraPos = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_budget_flBudgetRraPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_budget_flBudgetRraPos";
		this.maximize();
	...
	if (this.app._lokasi == "99" && this.app._userStatus=="A") this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Lokasi","All",""]);
	else this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Lokasi","=",this.app._lokasi]);
...
window.app_saku3_report_yakes21_budget_flBudgetRraPos.extend(window.childForm);
window.app_saku3_report_yakes21_budget_flBudgetRraPos.implement({
	doEllipseClick: function(sender, col, row){
		if (row ==0)
		{
			if (this.app._lokasi == "99" && this.app._userStatus=="A") {
		...
		if (row == 2)
		{
			...
			var sql1="select a.no_pdrk,a.keterangan from rra_pdrk_m a "+this.filter;
			var sql2="select count(a.no_pdrk) from rra_pdrk_m a "+this.filter;
			this.filterRep.ListDataSGFilter(this, "Data RRA Anggaran",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,["a.no_pdrk","a.keterangan"],"and",["No Anggaran","Keterangan"]);
		}
		
	},
	doSelectCell: function(sender, col, row){
		if (this.app._lokasi == "99" && this.app._userStatus=="A") {
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","123","123","13","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,2,0,0]);
		}
		else  {
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["3","123","123","13","3"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[0,0,2,0,0]);
		}
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("RRLC","RRRMULTI","RRA"));
		}
		...
		try
		{
			...
			else
			{
				...
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_pdrk",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
				...
				this.nama_report="server_report_saku3_yakes21_budget_rptBudgetRraPos";
				this.dbLib.execQuerySync(this.sql);	
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter2 = "";
		filter=" where a.no_pdrk='"+no_bukti+"'";
		nama_report="server_report_saku3_yakes21_budget_rptBudgetRra";
