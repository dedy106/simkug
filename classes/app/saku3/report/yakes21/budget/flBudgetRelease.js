window.app_saku3_report_yakes21_budget_flBudgetRelease = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_budget_flBudgetRelease.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_budget_flBudgetRelease";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,1,[0,1,2],["PP / Unit","All",""]);
...
window.app_saku3_report_yakes21_budget_flBudgetRelease.extend(window.childForm);
window.app_saku3_report_yakes21_budget_flBudgetRelease.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row ==1)
		{
			this.filterRep.ListDataSGFilter(this, "Data PP/Unit",this.sg1, this.sg1.row, this.sg1.col,
											"select kode_pp, nama from pp where kode_lokasi='"+this.sg1.cells(2,0)+"' ",
											"select count(*) from pp where kode_lokasi='"+this.sg1.cells(2,0)+"' ",
											new Array("kode_lokasi","nama"),"where",new Array("kode","nama"));
		}
		
	},
	doSelectCell: function(sender, col, row){
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2],["123","123","123","3"]);
		...
		if (row == 2)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct(substring(periode,1,4)) as periode from anggaran_d "+
									   "where kode_lokasi='"+this.app._lokasi+"' order by substring(periode,1,4) desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2 = this.sg1.getCell(2,2)+"/"+this.sg1.getCell(3,2);
				this.nama_report="server_report_saku3_yakes21_budget_rptBudgetRelease";
				this.dbLib.execQuerySync(this.sql);	
