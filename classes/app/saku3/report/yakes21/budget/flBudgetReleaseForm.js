window.app_saku3_report_yakes21_budget_flBudgetReleaseForm = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_budget_flBudgetReleaseForm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_budget_flBudgetReleaseForm";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(tahun) as periode from anggaran_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_yakes21_budget_flBudgetReleaseForm.extend(window.childForm);
window.app_saku3_report_yakes21_budget_flBudgetReleaseForm.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 2)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Release",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,["a.no_agg","a.keterangan"],"and",["No Anggaran","Keterangan"]);
		}
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct(tahun) as periode from anggaran_m "+
									   "where kode_lokasi='"+this.app._lokasi+"' order by tahun desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report="server_report_saku3_yakes21_budget_rptBudgetReleaseForm";
				this.dbLib.execQuerySync(this.sql);	
