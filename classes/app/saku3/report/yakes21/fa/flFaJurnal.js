window.app_saku3_report_yakes21_fa_flFaJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_fa_flFaJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_fa_flFaJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from fasusut_m where kode_lokasi='"+this.app._lokasi+"' ");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Format Jurnal","=","Daftar Jurnal"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_yakes21_fa_flFaJurnal.extend(window.childForm);
window.app_saku3_report_yakes21_fa_flFaJurnal.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 2)
			{
				...
				var sql1="select distinct a.no_fasusut,a.keterangan from fasusut_m a "+filter; 
				var sql2="select count(distinct a.no_fasusut) from fasusut_m a "+filter;
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
												new Array("a.no_fasusut","a.keterangan"),"and",new Array("No Bukti","Keterangan"),"and");
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from fasusut_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_fasusut",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				
				this.filter2=this.sg1.getCell(2,1)+"/";
				this.nama_report="server_report_saku3_yakes21_fa_rptFaJurnal";
				if (this.sg1.getCell(2,3)=="Memo Jurnal")
				{
					this.nama_report="server_report_saku3_yakes21_fa_rptFaBuktiJurnal";
				}
