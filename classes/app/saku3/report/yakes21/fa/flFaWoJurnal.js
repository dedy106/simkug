window.app_saku3_report_yakes21_fa_flFaWoJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_fa_flFaWoJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_fa_flFaWoJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from fa_woapp_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_yakes21_fa_flFaWoJurnal.extend(window.childForm);
window.app_saku3_report_yakes21_fa_flFaWoJurnal.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 2)
			{
				...
				var sql1="select a.no_woapp,a.keterangan from fa_woapp_m a "+filter; 
				var sql2="select count( a.no_woapp) from fa_woapp_m a "+filter;
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
												new Array("a.no_woapp","a.keterangan"),"and",new Array("No Bukti","Keterangan"),"and");
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report="server_report_saku3_yakes21_fa_rptFaWoJurnal";
				if (this.sg1.getCell(2,3)=="Memo Jurnal")
				{
					this.nama_report="server_report_saku3_yakes21_fa_rptFaWoBuktiJurnal";
				}
