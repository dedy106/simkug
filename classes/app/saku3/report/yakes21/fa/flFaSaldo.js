window.app_saku3_report_yakes21_fa_flFaSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_fa_flFaSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_fa_flFaSaldo";
		this.maximize();
	...
	var sql="select max(a.periode) as periode from (select max(periode) as periode from fasusut_d where kode_lokasi='"+this.app._lokasi+"')a";
	...
	this.gridLib.SGEditData(this.sg1,4,[0,1,2], ["Kelompok Akun","All",""]);

	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_yakes21_fa_flFaSaldo.extend(window.childForm);
window.app_saku3_report_yakes21_fa_flFaSaldo.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 3)
			{
				...
				var sql1="select distinct a.kode_pp, a.nama from pp a inner join fasusut_d b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+filter; 
				var sql2="select count(distinct a.kode_pp) from pp a inner join fasusut_d b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+filter; 
				// alert(sql1);

				this.filterRep.ListDataSGFilter(this, "Data PP",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","236","236","123","123"]);
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_klpakun",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"where");
							// this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							//this.filterRep.filterStr("a.kode_klpakun",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				if(this.sg1.getCell(1,3) == "Range"){
					this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,3)+"/"+" and a.kode_pp between '"+this.sg1.getCell(2,3)+"' and '"+this.sg1.getCell(3,3)+"' ";
				}else if (this.sg1.getCell(1,3) == "="){
					this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,3)+"/"+" and a.kode_pp = '"+this.sg1.getCell(2,3)+"' ";
				}else{
					this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2);
				}
				
				this.nama_report="server_report_saku3_yakes21_fa_rptFaSaldo";
	...
	doOpenAsset: function(kode_akun,kode_lokasi,periode,kode_pp){
		...
		if (this.kode_pp != ""){
			filter=" where a.kode_klpakun='"+kode_akun+"' and a.kode_lokasi='"+kode_lokasi+"' and a.kode_pp='"+kode_pp+"' ";
		}else{
