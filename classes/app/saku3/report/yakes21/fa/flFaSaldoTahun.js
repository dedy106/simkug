window.app_saku3_report_yakes21_fa_flFaSaldoTahun = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_fa_flFaSaldoTahun.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_fa_flFaSaldoTahun";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Tahun","All",""));
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kelompok Asset","All",""));
	this.gridLib.SGEditData(this.sg1,3,[0,1,2], ["Periode Penyusutan","<=",this.periode2]);
	this.gridLib.SGEditData(this.sg1,4,[0,1,2], ["Periode Perolehan","<=",this.periode]);
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Jenis","=","A"));
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_yakes21_fa_flFaSaldoTahun.extend(window.childForm);
window.app_saku3_report_yakes21_fa_flFaSaldoTahun.implement({
	...
	{		
		...
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,0,2,0,0,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct substring(a.periode,1,4) "+
									"from (select distinct periode from fa_asset where kode_lokasi='"+this.app._lokasi+"' "+
									"	  union "+
									"	  select periode from periode where kode_lokasi='"+this.app._lokasi+"' "+
									"	 )a "+
									"order by substring(a.periode,1,4) desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct jenis from fa_klp where kode_lokasi='"+this.app._lokasi+"' order by jenis asc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenAsset(this.kode_akun,this.kode_lokasi,this.tahun);
				}
			}
			else
			{
				...
				this.filter3= this.filterRep.filterStr("a.jenis",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"where");	
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,3)+"/"+this.sg1.getCell(2,1)+"/"+this.filter3;
				this.nama_report="server_report_saku3_yakes21_fa_rptFaSaldoTahun";
	...
	doOpenAsset: function(kode_akun,kode_lokasi,tahun){
		this.kode_akun=kode_akun;
		this.kode_lokasi=kode_lokasi;
		this.tahun=tahun;
		this.link="1";
		filter2 = kode_lokasi+"/"+this.sg1.getCell(2,3)+"/"+this.sg1.getCell(2,3)+"/Layar/Trail";
		filter=" where a.kode_klpakun='"+kode_akun+"' and a.kode_lokasi='"+kode_lokasi+"' and substring(a.periode,1,4)='"+tahun+"'";
		nama_report="server_report_saku3_yakes21_fa_rptFaAsset";
	...
	doOpenBbAsset: function(no_fa,kode_lokasi){
		...
		this.kode_lokasi=kode_lokasi;
		this.link="2";
		filter2 = kode_lokasi+"/"+this.sg1.getCell(2,1);
