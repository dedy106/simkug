window.app_saku3_report_yakes21_pbh_flKbTransfer = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_pbh_flKbTransfer.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_pbh_flKbTransfer";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from pbh_nota where kode_lokasi='"+this.app._lokasi+"'  ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("No Nota","All",""));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Output Laporan","=","Layar"));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Jenis Laporan","=","Nota"));
...
window.app_saku3_report_yakes21_pbh_flKbTransfer.extend(window.childForm);
window.app_saku3_report_yakes21_pbh_flKbTransfer.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_nota, a.keterangan from pbh_nota a "+this.filter,
												"select count(*) from pbh_nota a "+this.filter,
												new Array("a.no_nota","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from pbh_nota where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row === 3)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Nota","Mandiri","Non Mandiri"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_nota",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,3);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_yakes21_pbh_rptKbNota";
				if (this.sg1.getCell(2,4)=="Mandiri")
				{
					this.nama_report = "server_report_saku3_yakes21_pbh_rptKbTransfer";
				}
				if (this.sg1.getCell(2,4)=="Non Mandiri")
				{
					this.nama_report = "server_report_saku3_yakes21_pbh_rptKbTransferNon";
				}
			...
			systemAPI.alert("[app_saku3_report_yakes21_pbh_flKbTransfer]::mainButtonClick:"+e);
		}
	},
