window.app_saku3_report_yakes21_pbh_flPbPos = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_pbh_flPbPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_pbh_flPbPos";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from pbh_pb_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_yakes21_pbh_flPbPos.extend(window.childForm);
window.app_saku3_report_yakes21_pbh_flPbPos.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from pbh_pb_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_yakes21_pbh_rptPbPos";
			...
			systemAPI.alert("[app_saku3_report_yakes21_pbh_flPbPos]::mainButtonClick:"+e);
		}
	},
	...
	doOpenVer: function(no_bukti,kode_lokasi){
		...
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = "where a.no_ver='"+no_bukti+"'  ";
		filter2="";
	...
	doOpenSpb: function(no_bukti,kode_lokasi){
		...
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = "where a.no_spb='"+no_bukti+"'  ";
		filter2="";
	...
	doOpenKas: function(no_bukti,kode_lokasi){
		...
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = "where a.no_kas='"+no_bukti+"'  ";
		filter2="";
