window.app_saku3_report_yakes21_pbh_flVer = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_pbh_flVer.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_pbh_flVer";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from pbh_ver_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_yakes21_pbh_flVer.extend(window.childForm);
window.app_saku3_report_yakes21_pbh_flVer.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Verifikasi",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_ver, a.keterangan from pbh_ver_m a "+filter,
												"select count(a.no_ver) from pbh_ver_m a "+filter,
												new Array("a.no_ver","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from pbh_ver_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_yakes21_pbh_rptVer";
			...
			systemAPI.alert("[app_saku3_report_yakes21_pbh_flVer]::mainButtonClick:"+e);
		}
	},
