window.app_saku3_report_yakes21_piutang_flPeserta = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_piutang_flPeserta.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_piutang_flPeserta";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_peserta_m where kode_lokasi='"+this.app._lokasi+"'  ");
	this.no_load=this.dbLib.getPeriodeFromSQL("select max(no_load) as periode from yk_peserta_m where kode_lokasi='"+this.app._lokasi+"'  ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("No Bukti","=",this.no_load));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Tampilan Laporan","=","Layar"));
...
window.app_saku3_report_yakes21_piutang_flPeserta.extend(window.childForm);
window.app_saku3_report_yakes21_piutang_flPeserta.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Peserta",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_load, a.keterangan from yk_peserta_m a "+filter,
												"select count(a.no_load) from yk_peserta_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from yk_peserta_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_load",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,3)+"/"+this.sg1.getCell(2,2);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_yakes21_piutang_rptPeserta";
			...
			systemAPI.alert("[app_saku3_report_yakes21_piutang_flPeserta]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBayar: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter = "where a.no_load='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
