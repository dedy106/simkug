window.app_saku3_report_yakes21_piutang_flBillingBast = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_piutang_flBillingBast.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_piutang_flBillingBast";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_bast_m where  kode_lokasi='"+this.app._lokasi+"'");
	this.bill=this.dbLib.getPeriodeFromSQL("select max(no_selesai) as periode from yk_bast_m where  kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("No Bukti","=",this.bill));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Jenis","=","BPCCGR"));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Tampilan Laporan","=","Excell"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	this.doSelectCell(this.sg1,2,4);
};
window.app_saku3_report_yakes21_piutang_flBillingBast.extend(window.childForm);
window.app_saku3_report_yakes21_piutang_flBillingBast.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data BAST",this.sg1, this.sg1.row, this.sg1.col,
												"select no_selesai,keterangan from yk_bast_m "+this.filter,
												"select count(no_selesai) from yk_bast_m "+this.filter,
												new Array("no_selesai","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["3","3","13","3","3","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,0,2,0,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct a.periode from yk_bast_m a "+
		...
		try
		{
			...
			}else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_selesai",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report="server_report_saku3_yakes21_piutang_rptBillingBast";
				
				this.filter2 = this.app._nikUser+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,3)+"/"+this.sg1.getCell(2,4);
