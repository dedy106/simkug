window.app_saku3_report_yakes21_piutang_flHutangKesVendor = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_piutang_flHutangKesVendor.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_piutang_flHutangKesVendor";
		this.maximize();
...
window.app_saku3_report_yakes21_piutang_flHutangKesVendor.extend(window.childForm);
window.app_saku3_report_yakes21_piutang_flHutangKesVendor.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from yk_hutang_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenSaldo(this.kode_vendor,this.kode_lokasi);					
			...
			else
			{
				...
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0)+"/"+this.app._nikUser;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_yakes21_piutang_rptHutangKesVendor";
			...
			systemAPI.alert("[app_saku3_report_yakes21_piutang_flHutangKesVendor]::mainButtonClick:"+e);
		}
	},
	...
	doOpenSaldo: function(kode_vendor,kode_lokasi){
		
		this.kode_vendor=kode_vendor;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = "where b.kode_vendor='"+kode_vendor+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
		
		nama_report="server_report_saku3_piutang_rptHutangKesSaldo";
	...
	doOpenHutang: function(no_hutang,kode_lokasi){
		
		this.no_hutang=no_hutang;
		this.kode_lokasi=kode_lokasi;
		this.link="2";
		filter = "where a.no_hutang='"+no_hutang+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
		
		nama_report="server_report_saku3_piutang_rptTagihan";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
