window.app_saku3_report_yakes21_panjar_flPjAjuForm = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_panjar_flPjAjuForm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_panjar_flPjAjuForm";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from panjar2_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_yakes21_panjar_flPjAjuForm.extend(window.childForm);
window.app_saku3_report_yakes21_panjar_flPjAjuForm.implement({
		...
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Panjar",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_panjar, a.keterangan from panjar2_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter,
												"select count(*) from panjar2_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from panjar2_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_yakes21_panjar_rptPjAjuForm";
			...
			systemAPI.alert("[app_saku3_report_yakes21_panjar_flPjAjuForm]::mainButtonClick:"+e);
		}
	},
