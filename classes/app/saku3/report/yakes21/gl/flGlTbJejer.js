window.app_saku3_report_yakes21_gl_flGlTbJejer = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_gl_flGlTbJejer.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_gl_flGlTbJejer";
		this.maximize();
...
window.app_saku3_report_yakes21_gl_flGlTbJejer.extend(window.childForm);
window.app_saku3_report_yakes21_gl_flGlTbJejer.implement({
	...
	{		
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["3","23","123","123","3"]);
		...
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					...
					{
						this.doOpenBbTahun(this.kode_akun,this.kode_lokasi,this.periode);
					}
				}
			...
			else
			{
				...
				this.nama_report="server_report_saku3_yakes21_gl_rptGlTbJejer";
	...
	doOpenBb: function(kode_akun,kode_lokasi,periode){
		...
		filter2=kode_lokasi+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser+"/"+"Tidak/"+var_periode;
