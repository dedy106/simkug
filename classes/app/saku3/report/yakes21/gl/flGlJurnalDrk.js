// JavaScript Document
window.app_saku3_report_yakes21_gl_flGlJurnalDrk = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_gl_flGlJurnalDrk.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_gl_flGlJurnalDrk";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1), new Array("Modul","All"));
	...
	this.gridLib.SGEditData(this.sg1,6,new Array(0,1,2), new Array("Sum Jurnal","=","Ya"));
...
window.app_saku3_report_yakes21_gl_flGlJurnalDrk.extend(window.portalui_childForm);
window.app_saku3_report_yakes21_gl_flGlJurnalDrk.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 4)
		{
			...
			var sql1="select distinct a.no_bukti,a.keterangan from gldt a "+filter; 
			var sql2="select count(a.no_ju) from gldt a "+filter;
			this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
			                                new Array("a.no_bukti","a.keterangan"),"and",new Array("No Bukti","Keterangan"),"and");
		
		}
	},
	doSelectCell: function(sender, col, row){
		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,new Array(0,1,2,3,4,5,6),new  Array("3","123","123","123","123","3","3"));
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,new Array(0,1,2,3,4,5,6),new  Array(3,0,2,0,2,0,0));
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct modul from gldt where kode_lokasi='"+this.app._lokasi+"' order by modul",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row === 5)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.filterRep.filterStr("a.modul",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
						this.filterRep.filterStr("no_bukti",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
			this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(1,1)+"/"+this.sg1.getCell(3,1)+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,5)+"/"+this.sg1.getCell(2,6);
			this.nama_report="server_report_saku3_yakes21_gl_rptGlJurnalDrk";
			if (this.sg1.getCell(2,6)=="Tidak")
			{
				this.nama_report="server_report_saku3_yakes21_gl_rptGlJurnalDrk2";
			}
