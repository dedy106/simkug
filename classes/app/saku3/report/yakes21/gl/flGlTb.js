window.app_saku3_report_yakes21_gl_flGlTb = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_gl_flGlTb.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_gl_flGlTb";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Jenis","=","Bulanan"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_yakes21_gl_flGlTb.extend(window.childForm);
window.app_saku3_report_yakes21_gl_flGlTb.implement({
			...
			if (row ==0)
			{
				...
				if (this.app._lokasi=="99") {
					this.filterRep.ListDataSGFilter(this, "Data Lokasi",this.sg1, this.sg1.row, this.sg1.col,
													"select kode_lokasi, nama from lokasi where flag_konsol='1' union select kode_lokasi, nama from lokasi where kode_lokasi='99' ",
	...
	{		
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["3","3","123","123","3"]);
		...
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Bulanan","Tahunan"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser+"/"+this.sg1.getCell(2,3)+"//"+this.sg1.getCell(3,1);
				this.nama_report="server_report_saku3_yakes21_gl_rptGlTb";
	...
	doOpenBb: function(kode_akun,kode_lokasi,periode){
		...
		{
			var var_periode=" and a.periode between '"+this.sg1.getCell(2,1)+"' and '"+this.sg1.getCell(3,1)+"'";
		}
		...
		nama_report="server_report_saku3_yakes21_gl_rptGlBukuBesar";
	...
	doOpenBbTahun: function(kode_akun,kode_lokasi,periode){
		this.kode_akun=kode_akun;
		this.kode_lokasi=kode_lokasi;
		this.periode=periode;
		this.link="1";
		this.periode_awal=periode.substr(0,4)+"01";
		var var_periode=" and a.periode between '"+this.periode_awal+"' and '"+periode+"'";
		
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser+"/"+"Tidak/"+var_periode;
		filter="where a.kode_lokasi='"+kode_lokasi+"' and a.kode_akun='"+kode_akun+"' ";
		nama_report="server_report_saku3_gl_rptGlBukuBesarTahun";
	...
	doOpenJurnal: function(no_bukti,kode_lokasi){
		...
		this.kode_lokasi=kode_lokasi;
		this.link="2";
		filter = "where a.no_bukti='"+no_bukti+"'  ";
		filter2="";
		nama_report="server_report_saku3_yakes21_gl_rptGlJurnalDrk";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
