window.app_saku3_report_yakes21_spj_flPdPos = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_spj_flPdPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_spj_flPdPos";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from pdss_aju_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_yakes21_spj_flPdPos.extend(window.childForm);
window.app_saku3_report_yakes21_spj_flPdPos.implement({
		...
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data SPJ",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_aju, a.keterangan from pdss_aju_m a "+
												"inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter,
												"select count(*) from pdss_aju_m a "+
												"inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter,
												new Array("a.no_aju","a.keterangan"),"and",new Array("No SPB","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from pdss_aju_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_yakes21_spj_rptPdPos";
			...
			systemAPI.alert("[app_saku3_report_yakes21_spj_flPdPos]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter2 = "";
		filter=" where a.no_spj='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"'";
		nama_report="server_report_saku3_spj_rptSpjForm";
	...
	doOpenPb: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_yakes21_pbh_rptPb";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
