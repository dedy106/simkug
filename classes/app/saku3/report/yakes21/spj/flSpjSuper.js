window.app_saku3_report_yakes21_spj_flSpjSuper = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_yakes21_spj_flSpjSuper.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_yakes21_spj_flSpjSuper";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_spj_m where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("No SPPD","=",""));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Jenis SPPD","=","Surat Perintah"));
...
window.app_saku3_report_yakes21_spj_flSpjSuper.extend(window.childForm);
window.app_saku3_report_yakes21_spj_flSpjSuper.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data SPJ",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_spj, a.keterangan from yk_spj_m a inner join pdss_aju_m x on a.no_spj=x.no_aju "+
												"inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter + "  and a.modul='PD21' and x.progress = '1' ", //sudah approve budget
												
												"select count(*) from yk_spj_m a inner join pdss_aju_m x on a.no_spj=x.no_aju "+
												"inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter+"  and a.modul='PD21' and x.progress = '1' ",												
												new Array("a.no_spj","a.keterangan"),"and",new Array("No PD","Keterangan"));
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from yk_spj_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 5)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Surat Perintah","Surat Perintah Manager"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode PP","All",""));
				this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Nik","All",""));
				this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("No IF","All",""));
			}else{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.nik_buat",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.no_spj",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							"  and a.modul='PD21'  ";
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();
				
				if (this.sg1.getCell(2,5)=="Surat Perintah")
				{
					this.nama_report = "server_report_saku3_yakes21_spj_rptSpjSp";
				}
				if (this.sg1.getCell(2,5)=="Surat Perintah Manager")
				{
					this.nama_report = "server_report_saku3_yakes21_spj_rptSpjSpMgr";
				}
			...
			systemAPI.alert("[app_saku3_report_yakes21_spj_flSpjSuper]::mainButtonClick:"+e);
		}
	},
