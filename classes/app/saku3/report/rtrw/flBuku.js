// JavaScript Document
window.app_saku3_report_rtrw_flBuku = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_rtrw_flBuku.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_rtrw_flBuku";
		this.maximize();
	...
	this.akun=this.dbLib.getPeriodeFromSQL("select akun_kas as periode from pp where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"'");
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Referensi","All",""));
...
window.app_saku3_report_rtrw_flBuku.extend(window.portalui_childForm);
window.app_saku3_report_rtrw_flBuku.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row ==4)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Referensi",this.sg1, this.sg1.row, this.sg1.col,
											"select kode_ref, nama from trans_ref where kode_lokasi='"+this.app._lokasi+"' ",
											"select count(*) from trans_ref where kode_lokasi='"+this.app._lokasi+"' ",
											new Array("kode_ref","nama"),"and",new Array("kode","nama"));
		}
	},
	doSelectCell : function(sender, col, row){
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","3","23","123","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,2,0,2,2]);	
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from trans_m where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			...
			else
			{
				...
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							...
							this.filterRep.filterStr("b.param1",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4)," and ");
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.app._nikUser+"/"+this.filterRep.filterStr("a.kode_akun",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3)," and ");
				this.nama_report="server_report_saku3_rtrw_rptBuku";
				this.dbLib.execQuerySync(this.sql);	
