// JavaScript Document
window.app_saku3_report_rtrw_flSaldoLain = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_rtrw_flSaldoLain.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_rtrw_flSaldoLain";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from trans_m where kode_lokasi='"+this.app._lokasi+"'");
	this.jenis=this.dbLib.getPeriodeFromSQL("select top 1 kode_jenis as periode from rt_iuran_jenis where kode_lokasi='"+this.app._lokasi+"' and jenis='INSIDENTIL' ");
	this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("RT","=",this.app._kodePP));
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Tahun","=",this.periode));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Rumah","All",""));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Jenis Iuran","=",this.jenis));
...
window.app_saku3_report_rtrw_flSaldoLain.extend(window.portalui_childForm);
window.app_saku3_report_rtrw_flSaldoLain.implement({
		...
		if (row == 1)
		{
				var filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"and");
					...
					var sql2="select count(a.kode_pp) "+
							...
							"inner join karyawan_pp c on a.kode_lokasi=c.kode_lokasi and a.kode_pp=c.kode_pp "+
							"where a.tipe='Posting' and c.nik='"+this.app._userLog+"' "+filter;
					this.filterRep.ListDataSGFilter(this, "Data RT",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,
													new Array("a.kode_pp","a.nama"),"and",new Array("kode pp","nama"));
				
		}
		if (row == 3)
		{
			var filter=this.filterRep.filterStr("a.rw",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
					this.filterRep.filterStr("a.rt",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1)," and ");
			var sql="select a.kode_rumah,b.nama "+
					"from rt_rumah a "+
					"inner join rt_warga b on a.kode_penghuni=b.nik "+filter;
			alert(sql);
			var sql2="select count(a.kode_rumah) "+
					"from rt_rumah a "+
					"inner join rt_warga b on a.kode_penghuni=b.nik "+filter;
			this.filterRep.ListDataSGFilter(this, "Data Rumah",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,
											new Array("a.kode_rumah","a.nama"),"and",new Array("kode","nama"));
		}
		
	},
	doSelectCell : function(sender, col, row){
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4],["123","3","3","123","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4],[2,2,0,2,0]);	
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from trans_m where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"' order by substring(periode,1,4) desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select kode_jenis as periode from rt_iuran_jenis where kode_lokasi='"+this.app._lokasi+"' and jenis='INSIDENTIL'",[this.sg1.columns.get(2).pickList]);
		}
	},
		...
		try
		{
			...
			else
			{
				...
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							...
							this.filterRep.filterStr("a.kode_rumah",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3)," and ");
				this.filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.sg1.getCell(2,3)+"/"+this.sg1.getCell(2,4);
				this.nama_report="server_report_saku3_rtrw_rptSaldoLain";
				this.dbLib.execQuerySync(this.sql);	
	...
	doOpenBuku: function(kode_rumah,kode_lokasi,periode,kode_pp){
		this.kode_rumah=kode_rumah;
		this.kode_lokasi=kode_lokasi;
		this.periode=periode;
		this.kode_pp=kode_pp;
		this.link="1";
		filter = "where a.kode_rumah='"+kode_rumah+"' and a.kode_lokasi='"+kode_lokasi+"' and a.rt='"+kode_pp+"' ";
		filter2=kode_lokasi+"/"+kode_pp+"/"+periode+"/"+kode_rumah+"/"+this.sg1.getCell(2,4);
		nama_report="server_report_saku3_rtrw_rptKartuLain";
