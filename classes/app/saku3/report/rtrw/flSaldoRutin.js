// JavaScript Document
window.app_saku3_report_rtrw_flSaldoRutin = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_rtrw_flSaldoRutin.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_rtrw_flSaldoRutin";
		this.maximize();
	...
	this.jenis=this.dbLib.getPeriodeFromSQL("select top 1 kode_jenis as periode from rt_iuran_jenis where kode_lokasi='"+this.app._lokasi+"' and jenis='REGULER' ");
...
window.app_saku3_report_rtrw_flSaldoRutin.extend(window.portalui_childForm);
window.app_saku3_report_rtrw_flSaldoRutin.implement({
	...
	doSelectCell : function(sender, col, row){
		...
		if (row == 2)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from rt_bill_d where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"' order by substring(periode,1,4) desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select kode_jenis as periode from rt_iuran_jenis where kode_lokasi='"+this.app._lokasi+"' and jenis='REGULER'",[this.sg1.columns.get(2).pickList]);
		}
	},
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report="server_report_saku3_rtrw_rptSaldoRutinTahun";
				this.dbLib.execQuerySync(this.sql);	
	...
	doOpenBuku: function(kode_rumah,kode_lokasi,periode,kode_pp){
		...
		filter2=kode_lokasi+"/"+kode_pp+"/"+periode+"/"+this.app._nikUser;
		nama_report="server_report_saku3_rtrw_rptKartuRutin";
