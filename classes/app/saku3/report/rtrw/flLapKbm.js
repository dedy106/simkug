window.app_saku3_report_rtrw_flLapKbm = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_rtrw_flLapKbm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_rtrw_flLapKbm";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Kode Kendaraan","All",""));
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_rtrw_flLapKbm.extend(window.childForm);
window.app_saku3_report_rtrw_flLapKbm.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==1)
			{
				this.filterRep.ListDataSGFilter(this, "Data Kendaraan",this.sg1, this.sg1.row, this.sg1.col,
														  "select a.no_polisi, a.pemilik from rt_kbm a  "+
														   this.filterRep.filterStr("a.no_polisi",this.sg1.getCell(2,0)," where "),
														  "select count(a.no_polisi) from rt_kbm a "+
														  this.filterRep.filterStr("a.no_polisi",this.sg1.getCell(2,0)," where "),
														  ["a.no_polisi","a.nama"],"and",["Kode","Nama"]);
			}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  this.filterRep.filterStr("no_polisi",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_rtrw_rptLapKbm";
