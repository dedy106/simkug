window.app_saku3_report_sppd_flSurTgs = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sppd_flSurTgs.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sppd_flSurTgs";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sp_stugas_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_sppd_flSurTgs.extend(window.childForm);
window.app_saku3_report_sppd_flSurTgs.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_stugas, a.keterangan from sp_stugas_m a "+this.filter,
												"select count(*) from sp_stugas_m a "+this.filter,
												new Array("a.no_stugas","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from sp_stugas_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_stugas",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
			...
			systemAPI.alert("[app_saku3_report_sppd_flSurTgs]::mainButtonClick:"+e);
		}
	},
