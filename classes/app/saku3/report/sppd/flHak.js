window.app_saku3_report_sppd_flHak = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sppd_flHak.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sppd_flHak";
		this.maximize();
...
window.app_saku3_report_sppd_flHak.extend(window.childForm);
window.app_saku3_report_sppd_flHak.implement({
		...
		try{			
			...
			if (row ==	1)
			{
				this.filterRep.ListDataSGFilter(this, "Data NIK",this.sg1, this.sg1.row, this.sg1.col,
												"select a.nik, a.nama from hakakses a where a.kode_lokasi='"+this.app._lokasi+"'  ",
												"select count(*) from hakakses a where a.kode_lokasi='"+this.app._lokasi+"' ",
												new Array("a.nik","a.nama"),"where",new Array("a.nik","a.nama"));
			}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_sppd_rptHak";
			...
			systemAPI.alert("[app_saku3_report_sppd_flHak]::mainButtonClick:"+e);
		}
	},
