window.app_saku3_report_sppd_flUhar = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sppd_flUhar.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sppd_flUhar";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Uang Harian", 2);
...
window.app_saku3_report_sppd_flUhar.extend(window.childForm);
window.app_saku3_report_sppd_flUhar.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			if (row ==0)
			{
				this.filterRep.ListDataSGFilter(this, "Data Lokasi",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_lokasi, kode_band from sp_uhar",
												"select count(*) from sp_uhar",
												new Array("kode_lokasi","kode_band"),"where",new Array("Kode","Kode Band"));
			}
			if (row ==1)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Status",this.sg1, this.sg1.row, this.sg1.col,
									"select sts_spj, nama from sp_status "+this.filter,
									"select count(sts_spj) from sp_status "+this.filter,
									new Array("sts_spj","nama"),"and",new Array("Kode","Nama"));
			}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_sppd_rptUhar";
