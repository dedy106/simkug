window.app_saku3_report_sppd_flTrfTrans = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sppd_flTrfTrans.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sppd_flTrfTrans";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Tarif Transport", 2);
	...
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Kode Jenis","All",""));
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Kode Trans","All",""));
	
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_sppd_flTrfTrans.extend(window.childForm);
window.app_saku3_report_sppd_flTrfTrans.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			if (row ==0)
			{
				this.filterRep.ListDataSGFilter(this, "Data Lokasi",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_lokasi, nama from sp_trans",
												"select count(*) from sp_trans",
			...
			if (row ==1)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Kode Jenis",this.sg1, this.sg1.row, this.sg1.col,
									"select a.kode_jenis, a.nama from sp_jenis a "+this.filter,
									"select count(*) from sp_jenis a "+this.filter,
									new Array("a.kode_jenis","a.nama"),"and",new Array("Kode Jenis","Nama"));
			...
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
				this.filterRep.filterStr("a.kode_jenis",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
				this.filterRep.ListDataSGFilter(this, "Data Kode Transportasi",this.sg1, this.sg1.row, this.sg1.col,
									"select a.kode_trans, a.nama from sp_trans a "+this.filter,
									"select count(*) from sp_trans a "+this.filter,
									new Array("a.kode_trans","a.nama"),"and",new Array("Kode Trans","Nama"));
			}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			{
				this.gridLib.SGEditData(this.sg1,0,new Array(0,1,2), new Array("Lokasi","=",this.app._lokasi));
				this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Kode Trans","All",""));
			}else{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  this.filterRep.filterStr("a.kode_jenis",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							  this.filterRep.filterStr("a.kode_trans",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
							  

							  this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_sppd_rptTrfTrans";
