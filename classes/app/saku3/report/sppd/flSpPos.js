window.app_saku3_report_sppd_flSpPos = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sppd_flSpPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sppd_flSpPos";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sp_perintah_m where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_sppd_flSpPos.extend(window.childForm);
window.app_saku3_report_sppd_flSpPos.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			if (row ==0)
			{
				this.filterRep.ListDataSGFilter(this, "Data Lokasi",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_lokasi, keterangan from sp_perintah_m where kode_lokasi='"+this.app._lokasi+"'",
												"select count(*) from sp_perintah_m where kode_lokasi='"+this.app._lokasi+"'",
												new Array("kode_lokasi","keterangan"),"where",new Array("kode","keterangan"));
			}
			...
			if (row == 3)
			{
				...
				this.showFilter = this.filterRep.showFilter(this.sg1);
  				...
  				this.filterRep.ListDataSGFilter(this, "Data NIK",this.sg1, this.sg1.row, this.sg1.col,
					  "select distinct a.nik, a.nama from karyawan a "+this.filter,
			...
			if (row == 4)
			{
				...
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  ...
							  this.showFilter = this.filterRep.showFilter(this.sg1);
				this.filterRep.ListDataSGFilter(this, "Data Surat Perintah",this.sg1, this.sg1.row, this.sg1.col,
									"select a.no_perintah, a.keterangan from sp_perintah_m a "+this.filter,
									"select count(*) from sp_perintah_m a "+this.filter,
									new Array("a.no_perintah","a.keterangan"),"and",new Array("Kode","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from sp_perintah_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("b.nik_spj",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.no_perintah",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_sppd_rptSpPos";
