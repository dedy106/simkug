window.app_saku3_report_sppd_flApp = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sppd_flApp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sppd_flApp";
		this.maximize();
...
window.app_saku3_report_sppd_flApp.extend(window.childForm);
window.app_saku3_report_sppd_flApp.implement({
		...
		try{			
			...
			if (row ==	1)
			{
				this.filterRep.ListDataSGFilter(this, "Data App",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_spj, b.keterangan from sp_spj_m a inner join sp_perintah_m b on a.no_perintah = b.no_perintah and a.kode_lokasi=b.kode_lokasi where a.nik_app1='"+this.app._userLog+"' and a.progress in ('1','R','Z') ",
												"select count(*) from sp_spj_m a where a.nik_app1='"+this.app._userLog+"' and a.progress in ('1','R','Z') ",
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_sppd_flApp]::mainButtonClick:"+e);
		}
	},
