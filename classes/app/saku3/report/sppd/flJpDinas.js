window.app_saku3_report_sppd_flJpDinas = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sppd_flJpDinas.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sppd_flJpDinas";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Jenis Perjalanan Dinas", 2);
	...
	this.gridLib.SGEditData(this.sg1,1,new Array(0,1,2), new Array("Sts Spj","All",""));
	
	
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_sppd_flJpDinas.extend(window.childForm);
window.app_saku3_report_sppd_flJpDinas.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			if (row ==0)
			{
				this.filterRep.ListDataSGFilter(this, "Data Lokasi",this.sg1, this.sg1.row, this.sg1.col,
												"select kode_lokasi, nama from sp_status",
												"select count(*) from sp_status",
			...
			if (row ==1)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Sts Spj",this.sg1, this.sg1.row, this.sg1.col,
									"select a.sts_spj, a.nama from sp_status a "+this.filter,
									"select count(a.sts_spj) from sp_status a "+this.filter,
									new Array("a.sts_spj","a.nama"),"and",new Array("Jenis PD","Nama"));
			}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  this.filterRep.filterStr("a.sts_spj",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
							  this.showFilter = this.filterRep.showFilter(this.sg1);
				this.nama_report = "server_report_saku3_sppd_rptJpDinas";
