window.app_saku3_report_sppd_flSppd = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_sppd_flSppd.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_sppd_flSppd";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from sp_spj_m where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("No SPPD","All",""));
...
window.app_saku3_report_sppd_flSppd.extend(window.childForm);
window.app_saku3_report_sppd_flSppd.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Karyawan",this.sg1, this.sg1.row, this.sg1.col,
									"select a.nik, nama from karyawan a "+this.filter,
									"select count(*) from karyawan a "+this.filter,
									new Array("a.nik","nama"),"and",new Array("Nik","Nama"));
			...
			if (row ==4)
			{
				...
				this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.nik_spj",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filterRep.ListDataSGFilter(this, "Data SPPD",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_spj, a.nama_spj from sp_spj_m a "+this.filter,
												"select count(*) from sp_spj_m a "+this.filter,
												new Array("a.no_spj","a.nama_spj"),"and",new Array("No Bukti","Keterangan"));
			...
			// 				" and b.kode_bidang='"+this.bidang+"' ";
			// 	this.filterRep.ListDataSGFilter(this, "Data SPJ",this.sg1, this.sg1.row, this.sg1.col,
			// 									"select a.no_spj, a.keterangan from sp_spj_m a "+
			// 									"inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter,
												
			// 									"select count(*) from sp_spj_m a "+
			// 									"inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter,
			// 									new Array("a.no_spj","a.keterangan"),"and",new Array("No SPB","Keterangan"));
			// }
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from sp_spj_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		// if (row == 5)
		// {
		// 	this.sg1.columns.get(2).pickList.clear();
		// 	this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Biaya"));
		// }
	},
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_sppd_flSppd]::mainButtonClick:"+e);
		}
	},
