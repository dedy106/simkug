window.app_saku3_report_bangtel_flSaldoBdd = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bangtel_flSaldoBdd.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bangtel_flSaldoBdd";
		this.maximize();
	...
	var sql="select max(periode) as periode from yk_pb_m ";
	this.periode=this.dbLib.getPeriodeFromSQL(sql);
	this.tahun = this.periode.substr(0,4);
	...
	// this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Tahun","=",this.tahun));
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("Periode","=",this.periode));
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Kode Proyek","All",""));
...
window.app_saku3_report_bangtel_flSaldoBdd.extend(window.childForm);
window.app_saku3_report_bangtel_flSaldoBdd.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Proyek",this.sg1, this.sg1.row, this.sg1.col,
												"select a.kode_proyek, a.nama from spm_proyek a "+filter,
												"select count(a.kode_proyek) from spm_proyek a "+filter,
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Bukti",this.sg1, this.sg1.row, this.sg1.col,
									"select a.no_pb, a.keterangan from yk_pb_m a "+filter,
									"select count(a.no_pb) from yk_pb_m a "+filter,
									new Array("a.no_pb","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			...
			if (row ==5)
			{
				...
				var sql="select a.kode_akun,b.nama "+
						...
						"from yk_pb_j a "+filter+" and a.modul='PBPR' and a.jenis='BEBAN' "+
				...
				this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,
												new Array("a.kode_akun","b.nama"),"and",new Array("Kode Akun","Nama"));
			}

			if (row == 6)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where");
				var sql="select distinct a.kode_jenis,b.nama from spm_proyek a "+
				"inner join spm_proyek_jenis b on a.kode_jenis=b.kode_jenis and a.kode_lokasi=b.kode_lokasi "+filter;
				var sql2="select count(distinct a.kode_jenis) from spm_proyek a "+
				"inner join spm_proyek_jenis b on a.kode_jenis=b.kode_jenis and a.kode_lokasi=b.kode_lokasi "+filter;
				this.filterRep.ListDataSGFilter(this, "Data Jenis",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,
												new Array("a.kode_jenis","b.nama"),"and",new Array("Kode Jenis","Nama"));
			}
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123","123","23","123","3","123","123"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[2,2,0,2,2,2,2]);	
		// if (row == 2)
		// {
		// 	this.sg1.columns.get(2).pickList.clear();
		// 	var sql="select distinct substring(periode,1,4) as tahun from yk_pb_m where kode_lokasi='"+this.app._lokasi+"' order by substring(periode,1,4) desc ";
		// 	this.dbLib.setItemsFromSQL(sql,[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		// }
		if (row == 2)
		{
			this.sg1.columns.get(2).pickList.clear();
			var sql="select distinct periode from yk_pb_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc ";
			this.dbLib.setItemsFromSQL(sql,[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							// this.filterRep.filterStr("substring(a.periode,1,4)",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.kode_proyek",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.no_pb",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and")+
							this.filterRep.filterStr("e.kode_akun",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and")+
							this.filterRep.filterStr("c.kode_jenis",this.sg1.getCell(1,6),this.sg1.getCell(2,6),this.sg1.getCell(3,6),"and");

				var nama_cab="Semua Cabang";
				if (this.sg1.getCell(1,1)!="All")
				{
					nama_cab=this.dbLib.getPeriodeFromSQL("select max(nama) as periode from pp where  kode_pp='"+this.sg1.getCell(2,1)+"'");
				}
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_bangtel_rptSaldoBdd";
			...
			systemAPI.alert("[app_saku3_report_bangtel_flSaldoBdd]::mainButtonClick:"+e);
		}
	},
