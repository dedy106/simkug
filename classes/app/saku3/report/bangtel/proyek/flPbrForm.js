window.app_saku3_report_bangtel_proyek_flPbrForm = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bangtel_proyek_flPbrForm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bangtel_proyek_flPbrForm";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_pb_m where modul in ('PBPR','REVPB') and kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_bangtel_proyek_flPbrForm.extend(window.childForm);
window.app_saku3_report_bangtel_proyek_flPbrForm.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from yk_pb_m where modul in ('PBPR','REVPB') and kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_bangtel_proyek_flPbrForm]::mainButtonClick:"+e);
		}
	},
