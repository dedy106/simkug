window.app_saku3_report_bangtel_flKartuProyek = function (owner) {
	if (owner) {
		window.app_saku3_report_bangtel_flKartuProyek.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_report_bangtel_flKartuProyek";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1, 2, new Array(0, 1, 2), new Array("Periode", "All", ""));
	...
	this.gridLib.SGEditData(this.sg1, 5, new Array(0, 1, 2), new Array("Kode Proyek", "All", ""));
...
window.app_saku3_report_bangtel_flKartuProyek.extend(window.childForm);
window.app_saku3_report_bangtel_flKartuProyek.implement({
	...
	doSelectCell: function (sender, col, row) {
		this.filterRep.setSGFilterRowTipe(this.sg1, row, [0, 1, 2, 3, 4, 5, 6], ["123", "123", "13", "123", "123", "123", "3"]);
		...
		if (row == 2) {
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct '20'+substring(kode_proyek,4,4) as periode from spm_proyek where kode_lokasi='" + this.app._lokasi + "' order by periode", [this.sg1.columns.get(2).pickList, this.sg1.columns.get(3).pickList]);
		}
	},
		...
		try {
			if (sender === this.app._mainForm.bClear2) {
				...
				this.filter = this.filterRep.filterStr("a.kode_lokasi", this.sg1.getCell(1, 0), this.sg1.getCell(2, 0), this.sg1.getCell(3, 0), "where") +
					this.filterRep.filterStr("a.kode_pp", this.sg1.getCell(1, 1), this.sg1.getCell(2, 1), this.sg1.getCell(3, 1), "and") +
					this.filterRep.filterStr("'20'+SUBSTRING(a.kode_proyek,4,4)", this.sg1.getCell(1, 2), this.sg1.getCell(2, 2), this.sg1.getCell(3, 2), "and") +
				...
				this.nama_report = "server_report_saku3_bangtel_rptKartuProyek";
			...
			systemAPI.alert("[app_saku3_report_bangtel_flKartuProyek]::mainButtonClick:" + e);
		}
	},
	...
	doOpenJurnal: function (no_bukti, kode_lokasi) {
		this.link = "2";
		if(no_bukti.substr(3,3) == "RBD"){
			filter = "where a.no_reklas='" + no_bukti + "' and a.kode_lokasi='" + kode_lokasi + "' ";
			filter2 = this.app._namaUser;
			nama_report = "server_report_saku3_bangtel_rptJurnalReklasBdd";
		}else{

			filter = "where a.no_bukti='" + no_bukti + "' and a.kode_lokasi='" + kode_lokasi + "' ";
			filter2 = this.app._namaUser;
			nama_report = "server_report_saku3_bangtel_rptKbJurnalBukti";
		}
		this.viewer.useIframe(this.report.previewWithHeader(nama_report, filter, this.page, this.pager, this.showFilter, this.lokasi, filter2) + "&resource=" + this.resourceId + "&fullId=" + this.getFullId());
