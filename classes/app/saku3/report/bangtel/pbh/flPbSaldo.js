window.app_saku3_report_bangtel_pbh_flPbSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bangtel_pbh_flPbSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bangtel_pbh_flPbSaldo";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_pb_m where kode_lokasi='"+this.app._lokasi+"' ");
	...
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("No Bukti PB","All",""));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Status","=","Belum"));
...
window.app_saku3_report_bangtel_pbh_flPbSaldo.extend(window.childForm);
window.app_saku3_report_bangtel_pbh_flPbSaldo.implement({
		...
		try{			
			...
			if (row == 3)
			{
				...
				var sql="select a.kode_akun, a.nama "+
						...
						filter+" and b.kode_flag = '004' ";
				var sql2="select count(a.kode_akun) "+
						"from masakun a "+
						"inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
						filter+" and b.kode_flag = '004' ";
			
				this.filterRep.ListDataSGFilter(this, "Data Akun Hutang",this.sg1, this.sg1.row, this.sg1.col,sql,sql2,
			...
			if (row ==4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_pb, a.keterangan from yk_pb_m a "+this.filter,
												"select count(*) from yk_pb_m a "+this.filter,
	...
	{		
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5,6],["123","1236","123","123","123","13","3"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5,6],[2,0,2,2,2,0,0]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from yk_pb_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}

		if (row == 5)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Lunas","Belum"));
		}
		if (row == 6)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,5)+"/"+this.sg1.getCell(2,6);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_bangtel_pbh_rptPbSaldo";
			...
			systemAPI.alert("[app_saku3_report_bangtel_pbh_flPbSaldo]::mainButtonClick:"+e);
		}
	},
	...
	doOpenPb: function(no_pb,kode_lokasi){
		
		this.no_pb=no_pb;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter="where a.no_pb='"+no_pb+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
		nama_report="server_report_saku3_spm_rptPbJurnal";
	...
	doOpenKas: function(no_kas,kode_lokasi){
		...
		nama_report="server_report_saku3_kb_rptKbJurnalBuktiPp";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenVendor: function(kode_vendor,kode_lokasi){
		
		this.kode_vendor=kode_vendor;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter="where a.kode_vendor='"+kode_vendor+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		nama_report="server_report_saku3_spm_rptVendor";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
