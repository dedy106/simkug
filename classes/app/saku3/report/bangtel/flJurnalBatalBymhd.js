// JavaScript Document
window.app_saku3_report_bangtel_flJurnalBatalBymhd = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bangtel_flJurnalBatalBymhd.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bangtel_flJurnalBatalBymhd";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from hutang_m where modul='REV' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_bangtel_flJurnalBatalBymhd.extend(window.portalui_childForm);
window.app_saku3_report_bangtel_flJurnalBatalBymhd.implement({
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from hutang_m where modul='REV' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.nama_report="server_report_saku3_bangtel_rptJurnalBatalBymhd";
