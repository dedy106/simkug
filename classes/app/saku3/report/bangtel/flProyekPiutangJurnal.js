window.app_saku3_report_bangtel_flProyekPiutangJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bangtel_flProyekPiutangJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bangtel_flProyekPiutangJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from spm_piutang_m  WHERE modul='BILL' ");
...
window.app_saku3_report_bangtel_flProyekPiutangJurnal.extend(window.childForm);
window.app_saku3_report_bangtel_flProyekPiutangJurnal.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from spm_piutang_m where modul='BILL' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_bangtel_rptProyekPiutangJurnal";
			...
			systemAPI.alert("[app_saku3_report_bangtel_flProyekPiutangJurnal]::mainButtonClick:"+e);
		}
	},
