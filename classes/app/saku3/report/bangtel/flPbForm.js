window.app_saku3_report_bangtel_flPbForm = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bangtel_flPbForm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bangtel_flPbForm";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from yk_pb_m where modul='PBBAU' and kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_bangtel_flPbForm.extend(window.childForm);
window.app_saku3_report_bangtel_flPbForm.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from yk_pb_m where modul='PBBAU' and kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_bangtel_flPbForm]::mainButtonClick:"+e);
		}
	},
