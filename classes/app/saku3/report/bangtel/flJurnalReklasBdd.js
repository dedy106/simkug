// JavaScript Document
window.app_saku3_report_bangtel_flJurnalReklasBdd = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bangtel_flJurnalReklasBdd.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bangtel_flJurnalReklasBdd";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from spm_proyek_reklas_m where modul='REKLAS' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_report_bangtel_flJurnalReklasBdd.extend(window.portalui_childForm);
window.app_saku3_report_bangtel_flJurnalReklasBdd.implement({
	doEllipseClick: function(sender, col, row){
		...
		if (row == 2)
		{
			...
			var sql1="select a.no_reklas,a.keterangan from spm_proyek_reklas_m a "+filter; 
			var sql2="select count(a.no_reklas) from spm_proyek_reklas_m a "+filter;
			this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
			                                new Array("a.no_reklas","a.keterangan"),"and",new Array("No Bukti","Keterangan"),"and");
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from spm_proyek_reklas_m where modul='REKLAS' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			this.nama_report="server_report_saku3_bangtel_rptJurnalReklasBdd";
