window.app_saku3_report_bangtel_flTagihanProyek2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bangtel_flTagihanProyek2.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bangtel_flTagihanProyek2";
		this.maximize();
	...
	var sql="select max(a.periode) as periode from ( "+
			"select periode from spm_piutang_m "+
			"where kode_lokasi='"+this.app._lokasi+"' "+
			"union all "+
			"select periode from piutang_m "+
			"where kode_lokasi='"+this.app._lokasi+"' "+
			") a ";
	// alert(sql);
...
window.app_saku3_report_bangtel_flTagihanProyek2.extend(window.childForm);
window.app_saku3_report_bangtel_flTagihanProyek2.implement({
		...
		try{			
			...
			if (row == 4)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
												...
												" select count(a.no_piutang) from ( select a.no_piutang from spm_piutang_m a "+filter+
												" union all "+
												" select a.no_piutang from piutang_m a "+filter+
												") a ",
	...
	{		
		...
		if (row == 1)
		{
			...
			var sql="select distinct periode from (select distinct periode from spm_piutang_m where kode_lokasi='"+this.app._lokasi+"' union all select distinct periode from piutang_m where kode_lokasi='"+this.app._lokasi+"' "+
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_bangtel_rptTagihanProyek2";
			...
			systemAPI.alert("[app_saku3_report_bangtel_flTagihanProyek2]::mainButtonClick:"+e);
		}
	},
