window.app_saku3_report_bangtel_flProyekKasJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bangtel_flProyekKasJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bangtel_flProyekKasJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBBILL'");
...
window.app_saku3_report_bangtel_flProyekKasJurnal.extend(window.childForm);
window.app_saku3_report_bangtel_flProyekKasJurnal.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"' and modul='KBBILL'  order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_bangtel_rptProyekKasJurnal";
			...
			systemAPI.alert("[app_saku3_report_bangtel_flProyekKasJurnal]::mainButtonClick:"+e);
		}
	},
