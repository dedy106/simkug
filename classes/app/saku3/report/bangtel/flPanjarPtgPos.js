window.app_saku3_report_bangtel_flPanjarPtgPos = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bangtel_flPanjarPtgPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bangtel_flPanjarPtgPos";
		this.maximize();
...
window.app_saku3_report_bangtel_flPanjarPtgPos.extend(window.childForm);
window.app_saku3_report_bangtel_flPanjarPtgPos.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_ptg, a.keterangan from panjarptg2_m a "+this.filter,
												"select count(*) from panjarptg2_m a "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from panjarptg2_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_bangtel_rptPanjarPtgPos";
			...
			systemAPI.alert("[app_saku3_report_bangtel_flPanjarPtgPos]::mainButtonClick:"+e);
		}
	},
