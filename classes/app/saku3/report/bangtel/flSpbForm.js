window.app_saku3_report_bangtel_flSpbForm = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bangtel_flSpbForm.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bangtel_flSpbForm";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from spm_app_m where kode_lokasi='"+this.app._lokasi+"' and form='APPVER' ");
...
window.app_saku3_report_bangtel_flSpbForm.extend(window.childForm);
window.app_saku3_report_bangtel_flSpbForm.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_app, a.catatan from spm_app_m a "+this.filter,
												"select count(*) from spm_app_m a "+this.filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from spm_app_m where kode_lokasi='"+this.app._lokasi+"' and form='APPVER' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_bangtel_flSpbForm]::mainButtonClick:"+e);
		}
	},
