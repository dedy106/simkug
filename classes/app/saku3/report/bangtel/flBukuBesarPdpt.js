window.app_saku3_report_bangtel_flBukuBesarPdpt = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bangtel_flBukuBesarPdpt.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bangtel_flBukuBesarPdpt";
		this.maximize();
	...
	this.pusat=this.dbLib.getPeriodeFromSQL("select flag_pusat as periode from lokasi where kode_lokasi='"+this.app._lokasi+"'");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Jenis Laporan","=","Akun"));
	this.gridLib.SGEditData(this.sg1,4,new Array(0,1,2), new Array("Kode Customer","All",""));
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Kode PP","All",""));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_saku3_report_bangtel_flBukuBesarPdpt.extend(window.childForm);
window.app_saku3_report_bangtel_flBukuBesarPdpt.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			if (row ==0)
			{
				if (this.app._userStatus=="A" && this.pusat=="1" )
				{
			...
			if (row == 2)
			{
				var filter=this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
							" and substring(kode_akun,1,1)='4' " ;
				this.filterRep.ListDataSGFilter(this, "Data Akun",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_akun, nama from masakun  "+filter ,
														  "select count(kode_akun) from masakun "+filter,
														  new Array("kode_akun","nama"),"and",new Array("Kode Akun","Nama"));
			}
			if (row == 4)
			{
				var filter=this.filterRep.filterStr("kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
				...
				this.filterRep.ListDataSGFilter(this, "Data Customer",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_cust, nama from cust  "+filter ,
														  "select count(no_ju) from cust "+filter,
			...
			if (row == 5)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data PP",this.sg1, this.sg1.row, this.sg1.col,
														  "select distinct a.kode_pp,b.nama from spm_piutang_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi  "+filter ,
														  "select count(distinct no_ju) from spm_piutang_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+filter,
														  new Array("kode_pp","nama"),"and",new Array("Kode","Nama"));
			}
	...
	{		
		if (this.app._userStatus=="A" && this.pusat=="1" )
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["13","3","123","123","123","13"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,0,2,0,2,2]);	
		}
		else
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3,4,5],["3","3","123","123","123","13"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[3,0,2,0,2,2]);	
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select periode from periode where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Akun","Akun Customer","Detail"));
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenAkunCust(this.kode_akun,this.kode_cust,this.kode_lokasi);					
			...
			else
			{
				...
				if (this.sg1.getCell(2,3)=="Akun")
				{
					...
					this.nama_report="server_report_saku3_bangtel_rptPdpt";
				}
				if (this.sg1.getCell(2,3)=="Akun Customer")
				{
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
								this.filterRep.filterStr("a.kode_akun",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2)," and ")+
								this.filterRep.filterStr("a.kode_cust",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4)," and ");
					this.nama_report="server_report_saku3_bangtel_rptPdptCust";
				}
				if (this.sg1.getCell(2,3)=="Detail")
				{
					this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0)," where ")+
								...
								this.filterRep.filterStr("b.kode_cust",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4)," and ")+
								this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5)," and ");
					this.nama_report="server_report_saku3_bangtel_rptPdptDetail";
				}
	...
	doOpenAkun: function(kode_akun,kode_lokasi){
		this.kode_akun=kode_akun;
		this.kode_lokasi=kode_lokasi;
		this.link="1";
		filter = "where a.kode_akun='"+kode_akun+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
		nama_report="server_report_saku3_bangtel_rptPdptCust";
	...
	doOpenAkunCust: function(kode_akun,kode_cust,kode_lokasi){
		
		this.kode_akun=kode_akun;
		this.kode_lokasi=kode_lokasi;
		this.kode_cust=kode_cust;
		this.link="2";
		filter = "where a.kode_lokasi='"+kode_lokasi+"' and a.kode_akun='"+kode_akun+"' and b.kode_cust='"+kode_cust+"' ";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);
		
		nama_report="server_report_saku3_bangtel_rptPdptDetail";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());

	},
	doOpenNilaii: function(no_akun,kode_lokasi,periode){
		
		this.no_akun=no_akun;
		this.kode_lokasi=kode_lokasi;
		this.periode=periode;
		this.link="2";
		filter = "where a.kode_lokasi='"+kode_lokasi+"' and a.kode_akun='"+no_akun+"' and a.periode='"+periode+"' ";
		filter2="";
		
		nama_report="server_report_saku3_bangtel_rptBukuBesarPdpt";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
