window.app_saku3_report_bangtel_flPanjarPos = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bangtel_flPanjarPos.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bangtel_flPanjarPos";
		this.maximize();
...
window.app_saku3_report_bangtel_flPanjarPos.extend(window.childForm);
window.app_saku3_report_bangtel_flPanjarPos.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_panjar, a.keterangan from panjar2_m a "+this.filter,
												"select count(*) from panjar2_m a "+this.filter,
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_panjar",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_bangtel_rptPanjarPos";
			...
			systemAPI.alert("[app_saku3_report_bangtel_flPanjarPos]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBukti: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_bangtel_rptPanjarForm";
	...
	doOpenSpb: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_bangtel_rptSpbForm";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
