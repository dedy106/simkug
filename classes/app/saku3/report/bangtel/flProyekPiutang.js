window.app_saku3_report_bangtel_flProyekPiutang = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_bangtel_flProyekPiutang.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_bangtel_flProyekPiutang";
		this.maximize();
...
window.app_saku3_report_bangtel_flProyekPiutang.extend(window.childForm);
window.app_saku3_report_bangtel_flProyekPiutang.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Bukti",this.sg1, this.sg1.row, this.sg1.col,
												...
												"select count(b.no_piutang) from spm_piutang_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from spm_piutang_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.nama_report = "server_report_saku3_bangtel_rptProyekPiutang";
			...
			systemAPI.alert("[app_saku3_report_bangtel_flProyekPiutang]::mainButtonClick:"+e);
		}
	},
