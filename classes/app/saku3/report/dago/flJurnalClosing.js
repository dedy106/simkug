window.app_saku3_report_dago_flJurnalClosing = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_dago_flJurnalClosing.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_dago_flJurnalClosing";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from haj_akru_m  ");
...
window.app_saku3_report_dago_flJurnalClosing.extend(window.childForm);
window.app_saku3_report_dago_flJurnalClosing.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pemesanan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_akru, a.keterangan from haj_akru_m a "+filter,
												"select count(a.no_akru) from haj_akru_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from haj_akru_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_dago_rptJurnalClosing";
			...
			systemAPI.alert("[app_saku3_report_dago_flJurnalClosing]::mainButtonClick:"+e);
		}
	},
