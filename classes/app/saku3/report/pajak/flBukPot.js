window.app_saku3_report_pajak_flBukPot = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_pajak_flBukPot.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_pajak_flBukPot";
		this.maximize();
...
window.app_saku3_report_pajak_flBukPot.extend(window.childForm);
window.app_saku3_report_pajak_flBukPot.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from it_aju_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row === 5)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			...
			else
			{
				...
				*/
				...
				this.nama_report = "server_report_saku3_pajak_rptBukPot";
			...
			systemAPI.alert("[app_saku3_report_pajak_flBukPot]::mainButtonClick:"+e);
		}
	},
