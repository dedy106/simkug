window.app_saku3_report_droping_flDropingTerima = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_droping_flDropingTerima.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_droping_flDropingTerima";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Bentuk Laporan","=","Droping Terima"));
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
	this.doSelectCell(this.sg1,2,3);
};
window.app_saku3_report_droping_flDropingTerima.extend(window.childForm);
window.app_saku3_report_droping_flDropingTerima.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where modul='KBDROPTRM' and kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,new Array("Droping Terima"));
		}
	},
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				if (this.sg1.getCell(2,3)=="Droping Terima")
				{
					this.nama_report = "server_report_saku2_kb_rptKbDropingTerima";
			...
			systemAPI.alert("[app_saku3_report_droping_flDropingTerima]::mainButtonClick:"+e);
		}
	},
