window.app_saku3_report_droping_flDropingApp = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_droping_flDropingApp.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_droping_flDropingApp";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from spb_m where kode_lokasi='"+this.app._lokasi+"' and modul='DROPING' ");
...
window.app_saku3_report_droping_flDropingApp.extend(window.childForm);
window.app_saku3_report_droping_flDropingApp.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from spb_m where kode_lokasi='"+this.app._lokasi+"'  nd modul='KBAJU' order by periode desc",[this.sg1.columns.get(2).pickList]);
		}
		if (row == 3)
		{
			this.sg1.columns.get(2).pickList.clear();
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				this.nama_report = "server_report_saku3_droping_rptDropingApp";
			...
			systemAPI.alert("[app_saku3_report_droping_flDropingApp]::mainButtonClick:"+e);
		}
	},
