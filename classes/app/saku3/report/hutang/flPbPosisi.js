window.app_saku3_report_hutang_flPbPosisi = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_hutang_flPbPosisi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_hutang_flPbPosisi";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("No SPB","All",""));
...
window.app_saku3_report_hutang_flPbPosisi.extend(window.childForm);
window.app_saku3_report_hutang_flPbPosisi.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==5)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Pengajuan Pembayaran",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_pb, a.keterangan from yk_pb_m a "+
												"inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter,
												"select count(*) from yk_pb_m a "+
												"inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+this.filter,
	...
	{		
		...
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,0,2,2,0,2]);	
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from yk_pb_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 4)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct modul from yk_pb_m where kode_lokasi='"+this.app._lokasi+"' order by modul desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter =  this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_pb",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_hutang_rptPbPosisi";
			...
			systemAPI.alert("[app_saku3_report_hutang_flPbPosisi]::mainButtonClick:"+e);
		}
	},
	...
	doOpenSpb: function(no_bukti,kode_lokasi){
		...
		nama_report="server_report_saku3_hutang_rptSpbForm";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
