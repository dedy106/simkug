window.app_saku3_report_inves2_flDepTutup = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_inves2_flDepTutup.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_inves2_flDepTutup";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_depotutup_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_inves2_flDepTutup.extend(window.childForm);
window.app_saku3_report_inves2_flDepTutup.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data ",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_tutup,a.keterangan from inv_depotutup_m a "+this.filter,
												"select count(a.no_tutup) from inv_depotutup_m a "+this.filter,
												new Array("a.no_tutup","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}		
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from inv_depotutup_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							  this.filterRep.filterStr("a.no_tutup",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				
				this.nama_report = "server_report_saku3_inves2_rptDepTutup";
