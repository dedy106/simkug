window.app_saku3_report_inves2_flRdJualInv = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_inves2_flRdJualInv.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_inves2_flRdJualInv";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_rdjual_m ");
...
window.app_saku3_report_inves2_flRdJualInv.extend(window.childForm);
window.app_saku3_report_inves2_flRdJualInv.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Penjualan",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_rdjual, a.keterangan from inv_rdjual_m a "+filter,
												"select count(a.no_rdjual) from inv_rdjual_m a "+filter,
												new Array("a.no_rdjual","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from inv_rdjual_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row === 4)
		{
			this.sg1.columns.get(2).pickList.clear();
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.no_rdjual",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_inves2_rptRdJualInv";
			...
			systemAPI.alert("[app_saku3_report_inves2_flRdJualInv]::mainButtonClick:"+e);
		}
	},
	...
	doOpenBayar: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter = "where a.no_rdjual='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
