window.app_saku3_report_inves2_flRoiRd = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_inves2_flRoiRd.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_inves2_flRoiRd";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_rd_roi ");
...
window.app_saku3_report_inves2_flRoiRd.extend(window.childForm);
window.app_saku3_report_inves2_flRoiRd.implement({
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from inv_rd_roi  order by periode desc",[this.sg1.columns.get(2).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"where")+
							  this.filterRep.filterStr("a.kode_rdklp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.sg1.getCell(2,2)+"/"+this.app._nikUser;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				
				this.nama_report = "server_report_saku3_inves2_rptRoiRd";
