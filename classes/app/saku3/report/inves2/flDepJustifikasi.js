window.app_saku3_report_inves2_flDepJustifikasi = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_inves2_flDepJustifikasi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_inves2_flDepJustifikasi";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from inv_depoplan_m where kode_lokasi='"+this.app._lokasi+"' ");
...
window.app_saku3_report_inves2_flDepJustifikasi.extend(window.childForm);
window.app_saku3_report_inves2_flDepJustifikasi.implement({
		...
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data ",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_plan,a.keterangan from inv_depoplan_m a "+this.filter,
												"select count(a.no_plan) from inv_depoplan_m a "+this.filter,
												new Array("a.no_plan","a.keterangan"),"and",new Array("No Bukti","Keterangan"));
			}		
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from inv_depoplan_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							  this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							  this.filterRep.filterStr("a.no_plan",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/";
				this.showFilter = this.filterRep.showFilter(this.sg1);
				
				this.nama_report = "server_report_saku3_inves2_rptDepJustifikasi";
