window.app_saku3_report_kantintu_flKontrolTenan = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_kantintu_flKontrolTenan.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_kantintu_flKontrolTenan";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from ktu_ba  ");
	...
	this.gridLib.SGEditData(this.sg1,2,new Array(0,1,2), new Array("No Berita Acara","All",""));
...
window.app_saku3_report_kantintu_flKontrolTenan.extend(window.childForm);
window.app_saku3_report_kantintu_flKontrolTenan.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==2)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Berita Acara",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_ba from ktu_ba a "+filter,
												"select count(a.no_ba) from ktu_ba a "+filter,
												new Array("a.no_ba"),"and",new Array("No Berita Acara"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from ktu_ba where kode_lokasi='"+this.app._lokasi+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			if (sender == this.app._mainForm.bBack)
			{
				...
				if (this.link=="2")
				{
					this.doOpenKartu(this.no_reg,this.kode_lokasi);
				}
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.no_ba",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and");
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.sg1.getCell(3,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_kantintu_rptKontrolTenan";
			...
			systemAPI.alert("[app_saku3_report_kantintu_flKontrolTenan]::mainButtonClick:"+e);
		}
	},
	...
	doOpenKartu: function(no_reg,kode_lokasi){
		...
		nama_report="server_report_saku3_dago_rptKartu";
	...
	doOpenKas: function(no_kas,kode_lokasi){
		...
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1);;
		nama_report="server_report_saku3_dago_rptJurnalKas";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
