window.app_saku3_report_kredit_flLhi = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_kredit_flLhi.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_kredit_flLhi";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kre_lhi_m ");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Collector","All",""));
...
window.app_saku3_report_kredit_flLhi.extend(window.childForm);
window.app_saku3_report_kredit_flLhi.implement({
		...
		try{			
			...
			if (row ==3)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						...
						"and a.status='COLLECTOR' ";
				this.filterRep.ListDataSGFilter(this, "Data Collector",this.sg1, this.sg1.row, this.sg1.col,
												"select a.nik, a.nama from karyawan a "+filter,
												"select count(a.nik) from karyawan a "+filter,
												new Array("a.nik","a.nama"),"and",new Array("Kode","Nama"));
			}
			if (row ==5)
			{
				filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						...
						this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
						this.filterRep.filterStr("a.nik_kolek",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
				...
				this.filterRep.ListDataSGFilter(this, "Data Billing",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_lhi, a.nik_kolek from kre_lhi_m a "+filter,
												"select count(a.no_lhi) from kre_lhi_m a "+filter,
												new Array("a.no_lhi","a.nik_kolek"),"and",new Array("No Bukti","Collector"));
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		else
		{
			...
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3,4,5],[2,0,3,2,1,2]);	
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from kre_lhi_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.nik_kolek",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
							this.filterRep.filterStr("a.tanggal",this.sg1.getCell(1,4),this.sg1.getCellDateValue(2,4),this.sg1.getCellDateValue(3,4),"and")+
							this.filterRep.filterStr("a.no_lhi",this.sg1.getCell(1,5),this.sg1.getCell(2,5),this.sg1.getCell(3,5),"and");
			...
			systemAPI.alert("[app_saku3_report_kredit_flLhi]::mainButtonClick:"+e);
		}
	},
