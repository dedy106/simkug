window.app_saku3_report_kredit_flBrgKeluar = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_kredit_flBrgKeluar.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_kredit_flBrgKeluar";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kre_jual_m ");
...
window.app_saku3_report_kredit_flBrgKeluar.extend(window.childForm);
window.app_saku3_report_kredit_flBrgKeluar.implement({
		...
		try{			
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Barang Masuk",this.sg1, this.sg1.row, this.sg1.col,
												"select a.no_jual, a.keterangan from kre_jual_m a "+filter,
												"select count(a.no_jual) from kre_jual_m a "+filter,
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kre_jual_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_kredit_flBrgKeluar]::mainButtonClick:"+e);
		}
	},
