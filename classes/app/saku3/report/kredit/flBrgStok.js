window.app_saku3_report_kredit_flBrgStok = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_kredit_flBrgStok.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_kredit_flBrgStok";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kre_beli_m ");
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1,2), new Array("Kode Barang","All",""));
...
window.app_saku3_report_kredit_flBrgStok.extend(window.childForm);
window.app_saku3_report_kredit_flBrgStok.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row ==2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Unit Bisnis",this.sg1, this.sg1.row, this.sg1.col,
			...
			if (row ==3)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Barang ",this.sg1, this.sg1.row, this.sg1.col,
												"select a.kode_brg, a.nama from kre_brg a "+filter,
												"select count(a.kode_brg) from kre_brg a "+filter,
												new Array("a.kode_brg","a.nama"),"and",new Array("Kode Barang","Nama"));
			}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct periode from kre_beli_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							this.filterRep.filterStr("a.kode_brg",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser;
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_kredit_rptBrgStok";
			...
			systemAPI.alert("[app_saku3_report_kredit_flBrgStok]::mainButtonClick:"+e);
		}
	},
	...
	doOpenKartu: function(kode_brg,periode,kode_pp){
		
		this.kode_brg=kode_brg;
		this.periode=periode;
		this.kode_pp=kode_pp;
		this.link="1";
		filter = "where a.kode_brg='"+kode_brg+"' and b.kode_pp='"+kode_pp+"' ";
		filter2=this.sg1.getCell(2,0)+"/"+this.sg1.getCell(2,1)+"/"+this.app._nikUser;
		
		nama_report="server_report_saku3_kredit_rptBrgKartu";
