window.app_saku3_report_kredit_flBayar = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_kredit_flBayar.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_kredit_flBayar";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from kas_m ");
...
window.app_saku3_report_kredit_flBayar.extend(window.childForm);
window.app_saku3_report_kredit_flBayar.implement({
	...
	{		
		...
		{
			this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["123","123","3","!23"]);
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3],[2,0,3,2]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			...
			systemAPI.alert("[app_saku3_report_kredit_flBayar]::mainButtonClick:"+e);
		}
	},
