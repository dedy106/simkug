window.app_saku3_report_kredit_flSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_report_kredit_flSaldo.prototype.parent.constructor.call(this,owner);
		this.className = "app_saku3_report_kredit_flSaldo";
		this.maximize();
...
window.app_saku3_report_kredit_flSaldo.extend(window.childForm);
window.app_saku3_report_kredit_flSaldo.implement({
	...
	{		
		...
		else
		{
			...
			this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3],[2,0,3,2]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from kre_ttb2_m where kode_lokasi='"+this.app._lokasi+"' order by periode",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		...
		try
		{
			...
			else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_pp",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
							this.filterRep.filterStr("a.no_ttb",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.filter2 = this.sg1.getCell(2,1);
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new arrayMap();		
				this.nama_report = "server_report_saku3_kredit_rptSaldo";
			...
			systemAPI.alert("[app_saku3_report_kredit_flSaldo]::mainButtonClick:"+e);
		}
	},
	...
	doOpenKartu: function(no_bukti,kode_lokasi){
		...
		this.link="1";
		filter = "where a.no_ttb='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2="";
		
		nama_report="server_report_saku3_kredit_rptKartu";
	...
	doOpenKas: function(no_bukti,kode_lokasi,periode){
		this.periode=periode;
		this.link="2";
		filter = "where a.no_kas='"+no_bukti+"' and a.kode_lokasi='"+kode_lokasi+"' ";
		filter2=this.app._namaUser;
		nama_report="server_report_saku3_kredit_rptBayar";
		this.viewer.useIframe(this.report.previewWithHeader(nama_report,filter, this.page,  this.pager, this.showFilter, this.lokasi,filter2)+"&resource="+ this.resourceId+"&fullId="+this.getFullId());
