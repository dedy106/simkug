window.app_saku3_transaksi_sju16_fDraft = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sju16_fDraft.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sju16_fDraft";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Setting Draf Dokumen", 0);	
		
		uses("saiCBBL;saiEdit;saiGrid;tinymceCtrl");
		this.cb_tipe = new portalui_saiCBBL(this,{bound:[20,13,220,20],caption:"COB",tag:2,multiSelection:false,change:[this,"doChange2"]});		
		...
		this.c_jenis = new saiCB(this,{bound:[20,12,200,20],caption:"Jenis Form", items:["QUOTATION","PLACING"]});
...
window.app_saku3_transaksi_sju16_fDraft.extend(window.childForm);
window.app_saku3_transaksi_sju16_fDraft.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into sju_draft(no_draft,kode_lokasi,nama,keterangan,jenis,kode_tipe) values "+
						    "('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+urlencode(this.mDesk.getCode())+"','"+this.c_jenis.getText()+"','"+this.cb_tipe.getText()+"')");					
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update sju_draft set nama='"+this.e_nama.getText()+"',keterangan='"+urlencode(this.mDesk.getCode())+"',jenis='"+this.c_jenis.getText()+"',kode_tipe='"+this.cb_tipe.getText()+"' "+
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				...
				var data = this.dbLib.getMultiDataProvider(new server_util_arrayList({items:[strSQL,"select kunci, nama, jenis from sju_draft_d where  no_draft ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' order by no_urut"]}),true);
