window.app_saku3_transaksi_sju16_fPbNoted2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sju16_fPbNoted2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sju16_fPbNoted2";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,315], childPage:["Item Pengajuan","Atensi & Otorisasi","Jurnal++","Data Nominatif","File Dok","Cat. Approval"]});						
		...
		this.c_jenis = new saiCB(this.pc1.childPage[3],{bound:[20,11,200,20],caption:"Jenis DN",items:["ENTERTN","PROMOSI","NON-DN"], readOnly:true,tag:2,change:[this,"doChange"]});		
		this.cb_dn = new saiCBBL(this.pc1.childPage[3],{bound:[20,10,220,20],caption:"No DN", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});								
		this.e_nilaidn = new saiLabelEdit(this.pc1.childPage[3],{bound:[20,15,200,20],caption:"Nilai DN", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		...
		this.sgctt = new saiGrid(this.pc1.childPage[5],{bound:[1,5,this.pc2.width-12,this.pc2.height-15],colCount:1,tag:9, 
					colTitle:["Catatan"],
					colWidth:[[0],[100]],					
					readOnly:true,autoAppend:false,defaultRow:1});
...
window.app_saku3_transaksi_sju16_fPbNoted2.extend(window.childForm);
window.app_saku3_transaksi_sju16_fPbNoted2.implement({	
					...
					if (this.sg4.getRowValidCount() > 0){
						for (var i=0;i < this.sg4.getRowCount();i++){
							if (this.sg4.rowValid(i)){			
								...
								sql.add("insert into sju_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai_curr,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs,nilai) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg4.cells(0,i)+"','"+this.sg4.cells(3,i)+"','"+this.sg4.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg4.cells(4,i))+",'"+this.sg4.cells(5,i)+"','-','"+this.app._lokasi+"','PBPROSES','BEBAN2','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+","+nilaiIDR+")");										
							}
						}
					}
