window.app_saku3_transaksi_sju16_fPbNoted3 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sju16_fPbNoted3.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sju16_fPbNoted3";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan Pembayaran [Proses]", 0);	
		...
		this.e_dok = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,20,450,20],caption:"Pesan", maxLength:200});								
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,315], childPage:["Item Pengajuan","Atensi & Otorisasi","Data Nominatif","File Dok","Cat. Approval"]});						
		...
		this.cb_vendor = new saiCBBL(this.pc1.childPage[1],{bound:[20,16,220,20],caption:"Atensi/Mitra", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});								
		this.e_bank = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,17,450,20],caption:"Bank", readOnly:true});				
		this.e_norek = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,18,450,20],caption:"No Rekening", readOnly:true});				
		this.e_namarek = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,19,450,20],caption:"Nama Rekening", readOnly:true});				
		this.e_spasi1 = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,99,200,20],caption:"", tag:1, readOnly:true, visible:false, tag:7, text:""});					

		this.cb_pp = new saiCBBL(this.pc1.childPage[1],{bound:[20,10,220,20],caption:"PP / Unit", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});								
		this.cb_buat = new saiCBBL(this.pc1.childPage[1],{bound:[20,11,220,20],caption:"NIK Pengaju", multiSelection:false, maxLength:10, tag:2});						
		this.cb_atasan = new saiCBBL(this.pc1.childPage[1],{bound:[20,15,220,20],caption:"NIK Atasan", multiSelection:false, maxLength:10, tag:2});								
		this.e_spasi = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,99,200,20],caption:"", tag:1, readOnly:true, visible:false, tag:7, text:""});				

		this.cb_app1 = new saiCBBL(this.pc1.childPage[1],{bound:[20,12,220,20],caption:"NIK VP Finance", readOnly:true, tag:9});						
		this.cb_app2 = new saiCBBL(this.pc1.childPage[1],{bound:[20,13,220,20],caption:"NIK DIRKUG", readOnly:true, tag:2});						
		this.cb_app3 = new saiCBBL(this.pc1.childPage[1],{bound:[20,14,220,20],caption:"NIK DIRUT", readOnly:true, tag:2});						
	
		this.c_jenis = new saiCB(this.pc1.childPage[2],{bound:[20,11,200,20],caption:"Jenis DN",items:["ENTERTN","PROMOSI","NON-DN"], readOnly:true,tag:2,change:[this,"doChange"]});		
		this.cb_dn = new saiCBBL(this.pc1.childPage[2],{bound:[20,10,220,20],caption:"No DN", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});								
		this.e_nilaidn = new saiLabelEdit(this.pc1.childPage[2],{bound:[20,15,200,20],caption:"Nilai DN", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		...
		this.sgctt = new saiGrid(this.pc1.childPage[4],{bound:[1,5,this.pc2.width-12,this.pc2.height-15],colCount:1,tag:9, 
					colTitle:["Catatan"],
					colWidth:[[0],[100]],					
					readOnly:true,autoAppend:false,defaultRow:1});
		...
		try {
			...
			sql.add("select a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '061' where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'");									
			...
			this.cb_vendor.setSQL("select kode_vendor, nama from sju_vendor_pb where kode_lokasi='"+this.app._lokasi+"'",["kode_vendor","nama"],false,["Kode","Nama"],"and","Data Atensi/Mitra",true);			
...
window.app_saku3_transaksi_sju16_fPbNoted3.extend(window.childForm);
window.app_saku3_transaksi_sju16_fPbNoted3.implement({	
		...
		try{			
			if (sender == this.sgUpld) {				
				if (col == 0){
					...
					"select kode_jenis, nama  from dok_jenis_pb where kode_lokasi='"+this.app._lokasi+"' ", 
					"select count(*) from dok_jenis_pb where kode_lokasi='"+this.app._lokasi+"' ", 
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from sju_pb_his where no_pb = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from sju_pb_rek where no_pb = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						...
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){			
								...
								sql.add("insert into sju_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai_curr,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs,nilai) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(2,i)+"','D',"+nilaiToFloat(this.sg1.cells(3,i))+",'"+this.sg1.cells(4,i)+"','-','"+this.app._lokasi+"','PBPROSES','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+","+nilaiIDR+")");										

								sql.add("insert into sju_pb_his(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai_curr,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs,nilai) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(2,i)+"','D',"+nilaiToFloat(this.sg1.cells(3,i))+",'"+this.sg1.cells(4,i)+"','-','"+this.app._lokasi+"','PBPROSES','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+","+nilaiIDR+")");												
							}
						}
					}
					...
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i) && this.sgUpld.cells(2,i)!="-") {							
							...
							sql.add("insert into pbh_dok(no_bukti,no_gambar,nu,kode_jenis,kode_lokasi,modul,no_ref) values ('"+this.e_nb.getText()+"','"+this.sgUpld.cells(3,i).tmpfile+"','"+i+"','"+this.sgUpld.cells(0,i)+"','"+this.app._lokasi+"','PBPROSES','"+this.e_nb.getText()+"')");															
						}	
					}	
	...
	doChange:function(sender){
		...
		if (sender == this.cb_vendor && this.cb_vendor.getText()!="") {
			var strSQL = "select bank,no_rek,nama_rek from sju_vendor_pb where kode_vendor='"+this.cb_vendor.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";										
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){													
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							...
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_sju16_rptBayarAju3";
	...
	doLoad3:function(sender){		
		if (this.cb_pp.getText()!="") {																		
			...
			//0 = input; A=return Atasan; V=verifikasi P=vp K=dirkug U=dirut
			var strSQL = "select x.no_pb,convert(varchar,x.tanggal,103) as tgl,x.modul,x.no_dokumen,x.keterangan,x.progress,x.nilai_curr,x.kode_curr "+												
						...
						"where x.no_kas='-' and x.periode<='"+this.e_periode.getText()+"' and x.kode_lokasi='"+this.app._lokasi+"' and x.modul = 'PBPROSES' and x.progress in ('0','A','V','P','K','U') ";
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.keterangan,a.no_dokumen,a.modul,a.tanggal,a.nik_buat,a.nik_atasan,a.nik_app1,a.nik_app2,a.nik_app3,a.nik_app4,a.kode_curr,a.kurs,a.kode_pp,a.due_date , a.jenis_dn,a.no_dn, b.kode_vendor,c.nama as nama_vendor, b.bank,b.no_rek,b.nama_rek "+
							 "from sju_pb_m a inner join sju_pb_rek b on a.no_pb=b.no_pb and a.kode_lokasi=b.kode_lokasi "+								 
							 "				  inner join sju_vendor_pb c on b.kode_vendor=c.kode_vendor and b.kode_lokasi=c.kode_lokasi "+
				...
				var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai_curr,a.kode_pp,c.nama as nama_pp "+
							...
							"where a.jenis in ('BEBAN','BEBAN2') and a.no_pb = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg1.clear();
					for (var i in data.rs.rows){
