window.app_saku3_transaksi_sju16_fApp = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_sju16_fApp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sju16_fApp";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Approval", 0);	
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:12,tag:0,
		            colTitle:["Detail","No PB","Status","Tgl PB","Kode PP","Deskripsi","Kurs","Nilai","No Apprv","Kode Curr","Due Date","Nilai Curr"],
...
window.app_saku3_transaksi_sju16_fApp.extend(window.childForm);
window.app_saku3_transaksi_sju16_fApp.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into sju_ver_m (no_ver,kode_lokasi,tanggal,periode,tgl_input,nik_user,status,modul,no_dokumen,no_verseb) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_periode.getText()+"',getdate(),'"+this.app._userLog+"','"+vStatus+"','ATASAN','"+this.e_npb.getText()+"','-')");
					...
					//---------------- flag bukti									
					sql.add("update sju_pb_m set progress='"+vStatus+"', no_atasan='"+this.e_nb.getText()+"' where no_pb='"+this.e_npb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");	
	...
	doSelectDate: function(sender, y,m,d){
		...
		if (this.stsSimpan == 1) {
			this.doClick();
			this.doLoad();
			var strSQL = "select a.no_pb,a.keterangan from sju_pb_m a where a.periode <='"+this.e_periode.getText()+"' and a.nik_atasan='"+this.app._userLog+"' and a.modul='PBPROSES' and a.no_atasan = '-' and a.kode_lokasi='"+this.app._lokasi+"' ";						 
	...
	doChange:function(sender){								
		if (sender == this.cb_nb && this.cb_nb.getText() != "") {										
			var strSQL = "select a.due_date,a.no_pb,'INPROG' as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as due_date,a.kurs,a.kode_curr,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,a.nilai_curr,c.nik+' - '+c.nama as pembuat,a.no_atasan,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
	...
	doClick:function(sender){		
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"sju_ver_m","no_ver",this.app._lokasi+"-AAT"+this.e_periode.getText().substr(2,4)+".","0000"));												
		this.e_npb.setFocus();								
	},		
	...
	doLoad:function(sender){	
		try {					
			var strSQL = "select case when no_atasan ='-' then 'INPROG' else 'APPROVE' end as status, no_pb, convert(varchar,tanggal,103) as tgl,kode_pp,keterangan, nilai,kode_curr,kurs,no_atasan,nilai_curr,due_date, progress "+
						 "from sju_pb_m "+
						 "where periode <='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and progress='0' and no_atasan='-' and modul='PBPROSES' and no_kas='-' and nik_atasan='"+this.app._userLog+"' ";						 						 
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU = data;
				this.sgn.setTotalPage(Math.ceil(data.rs.rows.length/20));
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData(["Detail",line.no_pb,line.status.toUpperCase(),line.tgl,line.kode_pp,line.keterangan,line.kurs,floatToNilai(line.nilai),line.no_atasan,line.kode_curr,line.due_date,floatToNilai(line.nilai_curr)]); 
		}
