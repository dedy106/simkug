window.app_saku3_transaksi_sju16_fDisBDD = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sju16_fDisBDD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sju16_fDisBDD";
		...
		this.c_jenis = new saiCBBL(this.pc3.childPage[0],{bound:[20,11,222,20],caption:"Jenis Dok", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});				
		...
		this.pc1 = new pageControl(this.pc3.childPage[0],{bound:[1,12,995,327], childPage:["Otorisasi","Daftar BDD"]});
		this.cb_buat = new saiCBBL(this.pc1.childPage[0],{bound:[20,11,220,20],caption:"Dibuat Oleh", multiSelection:false, maxLength:10, tag:2});
		this.cb_periksa = new saiCBBL(this.pc1.childPage[0],{bound:[20,12,220,20],caption:"Diperiksa Oleh", multiSelection:false, maxLength:10, tag:2});
		
		this.sg1 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:9,		            
				colTitle:["Status","No Kartu","Keterangan","Periode","Akru Ke-","Akun Beban","Akun BDD","PP/Unit","Nilai"],
				colWidth:[[8,7,6,5,4,3,2,1,0],[100,100,100,100,60,50,250,100,80]],
		...
		try{			
			...
			this.c_jenis.setSQL("select distinct a.no_dokumen, a.nama "+
								"from sju_dokumen a "+
								"   inner join sju_dokumen_form c on a.no_dokumen=c.no_dokumen and c.kode_form = 'KU012' "+
...
window.app_saku3_transaksi_sju16_fDisBDD.extend(window.portalui_childForm);
window.app_saku3_transaksi_sju16_fDisBDD.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,2])){
				try{													
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("update sju_bdd_sch set no_bukti='-' where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
						sql.add("delete from sju_bdd_d  where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}
					...
					for (var i=0; i<this.sg1.getRowCount(); i++){
						if (this.sg1.rowValid(i) && this.sg1.cells(0,i) == "APP"){
							...
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(5,i)+"','D',"+parseNilai(this.sg1.cells(8,i))+","+
										parseNilai(this.sg1.cells(8,i))+",'"+this.sg1.cells(2,i)+"','MI','BEBAN','IDR',1,'"+this.sg1.cells(7,i)+"','-','-','-','-','-','"+this.sg1.cells(1,i)+"','-','-')");
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sg1.cells(6,i)+"','C',"+parseNilai(this.sg1.cells(8,i))+","+
										parseNilai(this.sg1.cells(8,i))+",'"+this.sg1.cells(2,i)+"','MI','BDD','IDR',1,'"+this.sg1.cells(7,i)+"','-','-','-','-','-','"+this.sg1.cells(1,i)+"','-','-')");
							sql.add("insert into sju_bdd_d (no_bukti,kode_lokasi,periode,periode_bdd, no_bdd,akun_beban,akun_bdd,kode_pp,nilai,dc,ref1) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.sg1.cells(3,i)+"', '"+this.sg1.cells(1,i)+"','"+this.sg1.cells(5,i)+"','"+this.sg1.cells(6,i)+"','"+this.sg1.cells(7,i)+"',"+nilaiToFloat(this.sg1.cells(8,i))+",'D','-')");
							
							sql.add("update sju_bdd_sch set no_bukti='"+this.e_nb.getText()+"' where no_bdd='"+this.sg1.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"' and periode='"+this.sg1.cells(3,i)+"' ");
						}
					}					
	...
	doTampilClick: function(sender){
		try{			
			if (this.e_periode.getText() != "") {								
				this.sg1.clear(1);	
				var strSQL = "select a.no_bdd,a.keterangan,b.periode,b.nu,a.akun_beban,a.akun_bdd,a.kode_pp,b.nilai "+
							 "from sju_bdd_m a "+
							 "inner join sju_bdd_sch b on a.no_bdd=b.no_bdd and a.kode_lokasi=b.kode_lokasi and b.periode<='"+this.e_periode.getText()+"' "+
							 "where b.no_bukti = '-' and b.kode_lokasi ='"+this.app._lokasi+"'  "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];									
						this.sg1.appendData(["NONAPP",line.no_bdd,line.keterangan,line.periode,line.nu,line.akun_beban,line.akun_bdd,line.kode_pp,floatToNilai(line.nilai)]);
					}
	...
	doDoubleClick1: function(sender, col , row) {
		if (sender.cells(0,row) == "NONAPP") sender.cells(0,row,"APP");
		else sender.cells(0,row,"NONAPP");
	},			
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.no_bdd,a.keterangan,b.periode,b.nu,a.akun_beban,a.akun_bdd,a.kode_pp,b.nilai "+
							 "from sju_bdd_m a "+
							 "inner join sju_bdd_sch b on a.no_bdd=b.no_bdd and a.kode_lokasi=b.kode_lokasi "+
							 "inner join sju_bdd_d c on b.no_bdd=c.no_bdd and b.kode_lokasi=c.kode_lokasi and b.periode=c.periode_bdd  "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];								
						this.sg1.appendData(["APP",line.no_bdd,line.keterangan,line.periode,line.nu,line.akun_beban,line.akun_bdd,line.kode_pp,floatToNilai(line.nilai)]);
					}
				} else this.sg1.clear(1);							
				this.sg1.validasi();
