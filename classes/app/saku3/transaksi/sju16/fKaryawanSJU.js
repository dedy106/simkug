window.app_saku3_transaksi_sju16_fKaryawanSJU = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sju16_fKaryawanSJU.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sju16_fKaryawanSJU";
		...
		this.cb_band = new saiCBBL(this.pc1.childPage[1],{bound:[20,12,200,20],caption:"Band",multiSelection:false,muxLength:10,tag:2});		
		...
		this.iFoto = new image(this.pc1.childPage[1], {bound:[620, this.cb_band.top, 100,100]});
		...
		try {
			...
			// this.cb_band.items.clear();
			// var data = this.dbLib.getDataProvider("select distinct kode_jab,nama as band from karyawan order by grade",true);
			// if (typeof data == "object" && data.rs.rows[0] != undefined){
			// 	var line;
			// 	for (var i in data.rs.rows){
			// 		line = data.rs.rows[i];							
			// 		this.cb_band.addItem(i,line.band);
			// 	}
			// }
			...
			this.cb_band.setSQL("select kode_jab,nama from hr_jab where flag_aktif='1' and kode_lokasi='"+this.app._lokasi+"' ",["kode_jab","nama"],false,["Kode","Nama"],"where","Data Jabatan",true);
...
window.app_saku3_transaksi_sju16_fKaryawanSJU.extend(window.childForm);
window.app_saku3_transaksi_sju16_fKaryawanSJU.implement({
	doLoadPP : function() {
	...
	doChange: function(sender){
		try{						
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select nik, kode_lokasi, nama, alamat, jabatan, no_telp, email, kode_pp, npwp, bank, cabang, no_rek, nama_rek, status, kode_jab, kota, kode_pos, no_hp, flag_aktif, foto, ttd "+
				             "from karyawan "+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){						
						...
						this.cb_band.setText(line.kode_jab);
	...
	doCari:function(sender){								
		...
		var strSQL = "select a.nik,a.nama,a.kode_jab as band,a.jabatan,a.status,a.no_telp,a.no_hp,a.email,a.alamat,a.kode_pp+' - '+b.nama as pp "+
