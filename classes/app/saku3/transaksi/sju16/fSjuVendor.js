window.app_saku3_transaksi_sju16_fSjuVendor = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sju16_fSjuVendor.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sju16_fSjuVendor";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Penanggung Asuransi", 0);	
		
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		uses("saiGrid",true);		
		this.pc1 = new pageControl(this,{bound:[20,12,1000,450], childPage:["List Penanggung","Entry Data","Filter Cari"]});				
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,6,this.pc1.width-6,this.pc1.height-35],colCount:6,tag:9,
		            colTitle:["Kode","Nama","Akun Rekon","NPWP","Alamat","PIC"],
					colWidth:[[5,4,3,2,1,0],[200,350,80,200,250,80]],
					readOnly:true,
		...
		this.l_tgl1 = new portalui_label(this.pc1.childPage[1],{bound:[20,11,100,18],caption:"Tgl Aktif", underline:true});
...
window.app_saku3_transaksi_sju16_fSjuVendor.extend(window.childForm);
window.app_saku3_transaksi_sju16_fSjuVendor.implement({
	doCari: function() {
		...
		var strSQL = "select kode_vendor,nama,akun_hutang,npwp,alamat,pic "+
		             "from sju_vendor "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();					
					sql.add("delete from sju_vendor where kode_vendor = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("insert into sju_vendor(kode_vendor,kode_lokasi,nama,akun_hutang, alamat,kota,no_tel,no_fax,npwp,pic,departemen,email,no_hp,no_fax2,alamat2, inisial, tgl_aktif) values "+
							"	('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.cb_hutang.getText()+"', '"+this.e_alamat.getText()+"','"+this.e_kota.getText()+"','"+this.e_tel.getText()+"','"+this.e_fax.getText()+"','"+this.e_npwp.getText()+"','"+this.e_pic.getText()+"','"+this.e_dep.getText()+"','"+this.e_email.getText()+"','"+this.e_nohp.getText()+"','"+this.e_fax2.getText()+"','"+this.e_alamat.getText()+"','"+this.e_nama2.getText()+"','"+this.dp_d1.getDateString()+"')");					
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select * from sju_vendor where kode_vendor ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
				var data = this.dbLib.getDataProvider(strSQL,true);
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_vendor,line.nama,line.akun_hutang,line.npwp,line.alamat,line.pic]); 
		}
