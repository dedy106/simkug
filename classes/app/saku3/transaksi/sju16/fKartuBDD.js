window.app_saku3_transaksi_sju16_fKartuBDD = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sju16_fKartuBDD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sju16_fKartuBDD";
		...
		this.c_jenis = new saiCBBL(this.pc2.childPage[0],{bound:[20,17,220,20],caption:"Jenis Dok", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});				
		...
		this.cb_beban = new saiCBBL(this.pc2.childPage[0],{bound:[20,13,220,20],caption:"Akun Beban", multiSelection:false, maxLength:10, tag:2});				
...
window.app_saku3_transaksi_sju16_fKartuBDD.extend(window.portalui_childForm);
window.app_saku3_transaksi_sju16_fKartuBDD.implement({
		...
		try{					
			...
			if (this.standarLib.checkEmptyByTag(this, [0,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						sql.add("delete from sju_bdd_m where no_bdd = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from sju_bdd_sch where no_bdd = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}
					var thnBln = this.dp_d1.getDateString().substr(0,4) + this.dp_d1.getDateString().substr(5,2);
					sql.add("insert into sju_bdd_m (no_bdd,kode_lokasi,periode,nik_user,tgl_input,  tanggal,no_bukti,akun_bdd,akun_beban,nilai,keterangan,jk,kode_pp) values "+
					...
					for (var i=0; i < this.sg.rows.getLength(); i++){
					  j = i+1;					
					  sql.add("insert into sju_bdd_sch(no_bdd,kode_lokasi,nu,periode,nilai,no_bukti) values "+
	...
	doChange: function(sender){
		try{	
			if (sender == this.c_jenis && this.c_jenis.getText()!="") {
				this.cb_bukti.setSQL("select no_bukti,keterangan from trans_m where param3='"+this.c_jenis.getText()+"' and posted ='T' and kode_lokasi = '"+this.app._lokasi+"'",["no_bukti","keterangan"],false,["No Bukti","Deskripsi"],"and","Data Bukti",true);						
			}		
	...
	doClick:function(sender){
		try {			
			...
			this.stsSimpan = 1;
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"sju_bdd_m","no_bdd",this.app._lokasi+"-BDD"+thnBln+".","0000"));		
		    this.dp_d1.setFocus();
	...
	doLoad1:function(sender){																		
		var strSQL = "select no_bdd,no_bukti,keterangan,nilai from sju_bdd_m "+
	...
	doDoubleClick1: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {
				...
				var strSQL = "select param3 from trans_m where no_bukti = '"+this.cb_bukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){				
