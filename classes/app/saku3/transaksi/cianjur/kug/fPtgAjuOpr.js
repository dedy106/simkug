window.app_saku3_transaksi_cianjur_kug_fPtgAjuOpr = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_cianjur_kug_fPtgAjuOpr.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_cianjur_kug_fPtgAjuOpr";
		...
		this.pc2 = new pageControl(this,{bound:[10,10,1000,470], childPage:["Data Pertanggungan","List Pertanggungan"]});		
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,990,226], childPage:["Item Mata Anggaran"]});
...
window.app_saku3_transaksi_cianjur_kug_fPtgAjuOpr.extend(window.childForm);
window.app_saku3_transaksi_cianjur_kug_fPtgAjuOpr.implement({
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){								
								...
								sql.add("insert into pr_beban_d(no_bukti,kode_akun,keterangan,dc,nilai,kode_pp,kode_proyek,kode_lokasi,modul) values "+
										"('"+this.e_nb.getText()+"','"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i)+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','-','"+this.app._lokasi+"','PJPTG')");
							}
						}
					}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah"	  :	
				...
				this.dataOR = {rs:{rows:[]}};
				var data = this.dbLib.getDataProvider("select kode_proyek from pr_proyek where kode_lokasi='"+this.app._lokasi+"'",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					this.dataOR = data;
                }
	...
	doChange:function(sender){
		...
		if (sender == this.cb_pj && this.cb_pj.getText()!="" && this.stsSimpan == 1) {
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData([line.kode_akun,line.nama_akun,line.dc,line.keterangan,this.e_nilaipj.getText(),line.kode_pp,line.nama_pp]);
				}
