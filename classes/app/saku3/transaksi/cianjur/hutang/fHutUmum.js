window.app_saku3_transaksi_cianjur_hutang_fHutUmum = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_cianjur_hutang_fHutUmum.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_cianjur_hutang_fHutUmum";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Hutang Project", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,12,990,232], childPage:["Data Item Jurnal"]});
...
window.app_saku3_transaksi_cianjur_hutang_fHutUmum.extend(window.childForm);
window.app_saku3_transaksi_cianjur_hutang_fHutUmum.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','AP','HUTUMUM','F','-','-','"+this.cb_pp.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+","+parseNilai(this.e_ppn.getText())+",0,'"+this.cb_app.getText()+"','-','-', '-','-','-', '-','"+this.cb_vendor.getText()+"','"+this.cb_akun.getText()+"')");
					...
					sql.add("insert into hutang_m(no_hutang,kode_lokasi,no_dokumen,tanggal,keterangan,kode_project,kode_vendor,kode_curr,kurs,nik_app,kode_pp,nilai,periode,nik_user,tgl_input,akun_hutang,posted,nilai_ppn) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','-','"+this.cb_vendor.getText()+"','IDR',1,'"+this.cb_app.getText()+"','"+this.cb_pp.getText()+"',"+parseNilai(this.e_nilai.getText())+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.cb_akun.getText()+"','X',"+parseNilai(this.e_ppn.getText())+")");
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){															
								var j = i+1;
								sql.add("insert into hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i)+"','IDR',1,"+parseNilai(this.sg.cells(4,i))+","+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','-','"+this.app._lokasi+"','HUTUMUM','BBN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");								
							}
						}
					}
	...
	doLoad3:function(sender){																		
		var strSQL = "select distinct a.no_hutang,a.no_dokumen,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,a.nilai from hutang_m a "+ 
					 "inner join trans_m c on a.no_hutang=c.no_bukti and a.kode_lokasi=c.kode_lokasi "+
					 "left join (select no_hutang,kode_lokasi,sum(nilai) as nilai_pb from pb_m where kode_lokasi='"+this.app._lokasi+"' group by no_hutang,kode_lokasi) b  "+
					 "on a.no_hutang=b.no_hutang and a.kode_lokasi=b.kode_lokasi "+				
					 "where c.posted='F' and c.form='HUTUMUM' and a.kode_project ='-' and (a.nilai+a.nilai_ppn-isnull(b.nilai_pb,0)) > 0 and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";						
