window.app_saku3_transaksi_yakes21_pbh_fFiatPb = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_yakes21_pbh_fFiatPb.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_pbh_fFiatPb";
...
window.app_saku3_transaksi_yakes21_pbh_fFiatPb.extend(window.childForm);
window.app_saku3_transaksi_yakes21_pbh_fFiatPb.implement({					
		...
		try{															
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.c_status.getText() == "APPROVE") var vStatus = "2"; else var vStatus = "F";
					
					sql.add("update pbh_ver_m set no_flag='"+this.e_nb.getText()+"' where no_bukti='"+this.e_nobukti.getText()+"' and no_flag='-' and form='FIATSPB' and modul='SPB' and kode_lokasi='"+this.app._lokasi+"'");					
					sql.add("insert into pbh_ver_m (no_ver,kode_lokasi,tanggal,periode,tgl_input,nik_user,status,modul,form,no_bukti,catatan,no_flag,nik_bdh,nik_fiat) values "+
                            "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_periode.getText()+"',getdate(),'"+this.app._userLog+"','"+vStatus+"','SPB','FIATSPB','"+this.e_nobukti.getText()+"','"+this.e_memo.getText()+"','-','X','X')");
                    													
				    sql.add("update spb_m set no_fiat='"+this.e_nb.getText()+"',progress='"+vStatus+"' where no_spb='"+this.e_nobukti.getText()+"' ");							 
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "hapus" :	
				...
				sql.add("update spb_m set no_fiat='-',progress='1' where no_spb='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");				
				setTipeButton(tbAllFalse);					
				this.dbLib.execArraySQL(sql);				
				break;					
		}
	...
	doSelectDate: function(sender, y,m,d){
		...
		if (this.stsSimpan == 1) {
			...
			this.cb_nb.setSQL("select a.no_spb, a.keterangan from spb_m a "+
							  ...
							  "where b.no_spb is null and a.nik_fiat='"+this.app._userLog+"' and a.periode<='"+this.e_periode.getText()+"' and a.progress in ('2','F') ",["no_spb","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar SPB",true);				
		}
	},	
	doChange:function(sender){										
		if (sender == this.cb_nb && this.cb_nb.getText() != "") {
			var strSQL = "select a.no_spb as no_bukti,case a.progress when '2' then 'APPROVE' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.tanggal,103) as tgl2,a.no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_fiat as no_ver1,convert(varchar,a.tgl_input,120) as tglinput "+
						 "from spb_m a "+
						 "      inner join karyawan c on a.nik_user=c.nik "+
						 "where a.progress in ('2','F') and a.no_spb='"+this.cb_nb.getText()+"' ";
	...
	doClick:function(sender){		
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"pbh_ver_m","no_ver",this.app._lokasi+"-FIA"+this.e_periode.getText().substr(2,4)+".","0000"));												
	...
	doLoad:function(sender){			
		strSQL = "select a.no_spb as no_bukti,'INPROG' as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,no_dokumen,a.keterangan,a.nilai,a.nik_user as pembuat,a.no_fiat as no_ver1,convert(varchar,a.tgl_input,120) as tglinput "+
				 "from spb_m a "+
				 "where a.progress='1' and a.nik_fiat='"+this.app._userLog+"' order by no_spb";                                
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
			this.sgn.setTotalPage(Math.ceil(data.rs.rows.length/20));
