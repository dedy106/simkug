window.app_saku3_transaksi_yakes21_pbh_fKbSpb = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_pbh_fKbSpb.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_pbh_fKbSpb";
		...
		this.sg5 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:8,tag:0,
		            colTitle:["Status","No SPB","Bank Transfer","No Dokumen","Tanggal","Deskripsi","Nilai","Rekening"],
					colWidth:[[7,6,5,4,3,2,1,0],[70,100,300,80,140,80,100,80]],
					colFormat:[[6,7],[cfNilai,cfButton]], colAlign:[[7],[alCenter]],
					click:[this,"doSgBtnClick5"], colAlign:[[7],[alCenter]],									
...
window.app_saku3_transaksi_yakes21_pbh_fKbSpb.extend(window.childForm);
window.app_saku3_transaksi_yakes21_pbh_fKbSpb.implement({	
				...
				try{														
					...
					if (this.stsSimpan == 0) {
						sql.add("update a set a.no_kas='-' "+
								...
								"where c.no_kas='"+this.e_nb.getText()+"'  ");		
						...
						sql.add("update a set a.no_kas='-',progress='0' "+
								...
								"inner join spb_m c on b.no_spb=c.no_spb "+
								"where c.no_kas='"+this.e_nb.getText()+"' and a.status_dana<>'PERPANJANG' ");

						sql.add("delete from kas_m where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from kas_j where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("update spb_m set progress='2', no_kas='-' where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");																	
					}			
					...
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',234,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_nilaikb.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBSPB','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.e_nilaikb.getText())+")");
										
					for (var i=0;i < this.dataJU5.rs.rows.length;i++){
						line = this.dataJU5.rs.rows[i];
						if (line.status.toUpperCase() == "BAYAR") {
							sql.add("update spb_m set progress='3', no_kas='"+this.e_nb.getText()+"' where no_spb = '"+line.no_spb+"' and kode_lokasi='"+this.app._lokasi+"'");						
					...
					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){
								var k = 1000+i;
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+k+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"','"+this.sg1.cells(7,i)+"','-','-','"+this.app._lokasi+"','KBSPB','NONKB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.sg1.cells(4,i))+")");										
							}
						}
					}					
					...
					sql.add("update a set a.no_kas='"+this.e_nb.getText()+"',progress='1' "+
							...
							"inner join pbh_pb_m b on a.no_shop=b.no_pb "+
							"inner join spb_m c on b.no_spb=c.no_spb "+
							"where c.no_spb='"+line.no_spb+"' and a.status_dana<>'PERPANJANG' ");		
	...
	doLoad:function(sender){		
		this.dataJU5 = {rs:{rows:[]}};
        var strSQL = "select 'INPROG' as status,no_spb,bank_trans,no_dokumen,convert(varchar,tanggal,103) as tgl,keterangan,nilai "+
		             "from spb_m "+
					 "where progress='2' and no_kas='-' and kode_lokasi='"+this.app._lokasi+"' and modul='SPB'";                    
	...
	doTampilData5: function(page) {		
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU5.rs.rows[i];									
			this.sg5.appendData([line.status.toUpperCase(),line.no_spb,line.bank_trans,line.no_dokumen,line.tgl,line.keterangan,floatToNilai(line.nilai),"Rekening"]);
		}
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.jenis,a.no_dokumen,a.keterangan,a.nilai "+
					 "from kas_m a "+						 
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'KBSPB' and a.posted ='F'";		
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				this.dataJU5 = {rs:{rows:[]}};							 
                var strSQL = "select 'BAYAR' as status,no_spb,bank_trans,no_dokumen,convert(varchar,tanggal,103) as tgl,keterangan,nilai "+
							 "from spb_m a "+
							 "where progress='3' and no_kas='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";								 
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					this.dataJU5 = data;
					this.sgn5.setTotalPage(Math.ceil(data.rs.rows.length/20));
