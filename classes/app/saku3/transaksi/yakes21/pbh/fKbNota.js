window.app_saku3_transaksi_yakes21_pbh_fKbNota = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_pbh_fKbNota.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_pbh_fKbNota";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","KB Nota Transfer", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,996,315], childPage:["Daftar PB","Rekening","Jurnal Tambahan"]});		
		this.sg5 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:0,
		            colTitle:["Status","No SPB","Bank Transfer","No PB","Tanggal","Deskripsi","Nilai","Modul","Rekening"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[70,70,100,300,80,100,80,100,80]],
					colFormat:[[6,8],[cfNilai,cfButton]], colAlign:[[8],[alCenter]],
					click:[this,"doSgBtnClick5"], colAlign:[[8],[alCenter]],									
		...
		this.sg1 = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:9,
					...
					columnReadOnly:[true,[1,6,7],[0,2,3,4,5,8]],
...
window.app_saku3_transaksi_yakes21_pbh_fKbNota.extend(window.childForm);
window.app_saku3_transaksi_yakes21_pbh_fKbNota.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("update a set a.no_kas='-' "+
								"from if_reim_m a "+
								"inner join pbh_pb_m b on a.no_reim=b.no_pb "+
								"where b.no_kas='"+this.e_nb.getText()+"'  ");		
								
						sql.add("update a set a.no_kas='-',progress='1' "+
								"from panjar2_m a "+
								"inner join pbh_pb_m b on a.no_panjar=b.no_pb "+
								"where b.no_kas='"+this.e_nb.getText()+"'  ");

						sql.add("update a set a.no_kas='-',progress='1' "+
								"from yk_hutang_d a "+
								"inner join pbh_pb_m b on a.no_pb=b.no_pb "+
								"where b.no_kas='"+this.e_nb.getText()+"'  ");

						sql.add("update a set a.no_kas='-' "+
								"from inv_oblibeli_m a "+
								"inner join pbh_pb_m b on a.no_beli=b.no_pb "+								
								"where b.no_kas='"+this.e_nb.getText()+"'  ");				

						sql.add("update a set a.no_kas='-' "+
								"from inv_shmbeli_m a "+
								"inner join pbh_pb_m b on a.no_shmbeli=b.no_pb "+								
								"where b.no_kas='"+this.e_nb.getText()+"'  ");		

						sql.add("update a set a.no_kas='-' "+
								"from inv_rdbeli_m a "+
								"inner join pbh_pb_m b on a.no_rdbeli=b.no_pb "+								
								"where b.no_kas='"+this.e_nb.getText()+"'  ");		

						sql.add("update a set a.no_kas='-' "+
								"from inv_spbeli_m a "+
								"inner join pbh_pb_m b on a.no_spbeli=b.no_pb "+								
								"where b.no_kas='"+this.e_nb.getText()+"'  ");		

						sql.add("update a set a.no_kas='-',progress='0' "+
								"from inv_depo2_m a "+
								"inner join pbh_pb_m b on a.no_shop=b.no_pb "+								
								"where b.no_kas='"+this.e_nb.getText()+"' and a.status_dana<>'PERPANJANG' ");

						sql.add("delete from kas_m where no_kas = '"+this.e_nb.getText()+"'");
						sql.add("delete from kas_j where no_kas = '"+this.e_nb.getText()+"'");						
						sql.add("update pbh_pb_m set progress='4', no_kas='-' where no_kas='"+this.e_nb.getText()+"' ");
					}			


					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBPB','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilaikb.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','-','-','-')");					
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',234,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_nilaikb.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBPB','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.e_nilaikb.getText())+")");
										
					for (var i=0;i < this.dataJU5.rs.rows.length;i++){
						line = this.dataJU5.rs.rows[i];
						if (line.status.toUpperCase() == "BAYAR") {		
							sql.add("update pbh_pb_m set progress='5', no_kas='"+this.e_nb.getText()+"' where no_pb='"+line.no_pb+"' ");
							...
							sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) "+
									"select '"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,'-',no_dokumen,kode_lokasi,'KBPB',jenis,'"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',nilai "+
									"from pbh_pb_j "+
									"where no_pb='"+line.no_pb+"' and jenis in ('BEBAN','TAMBAH','PAJAK','PANJAR','TUJUAN','PENSIUN','PEGAWAI') ");										
							
							//flag modul		
							if (line.modul == "IFREIM") {
								sql.add("update a set a.no_kas='"+this.e_nb.getText()+"' "+
										"from if_reim_m a "+
										"inner join pbh_pb_m b on a.no_reim=b.no_pb "+
										"where b.no_pb='"+line.no_pb+"' ");	
							} 		
							...
							if (line.modul == "DEPOSWA") {
								sql.add("update a set a.no_kas='"+this.e_nb.getText()+"',progress='1' "+
										"from inv_depo2_m a "+
										"inner join pbh_pb_m b on a.no_depo=b.no_pb "+										
										"where b.no_pb='"+line.no_pb+"' and a.status_dana<>'PERPANJANG' ");
							}
					...
					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){
								var k = 1000+i;
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+k+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"','"+this.sg1.cells(7,i)+"','-','-','"+this.app._lokasi+"','KBPB','NONKB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.sg1.cells(4,i))+")");										
							}
						}
					}					
	...
	doLoad:function(sender){		
		this.dataJU5 = {rs:{rows:[]}};
        var strSQL = "select 'INPROG' as status,no_spb,bank_trans,no_pb,convert(varchar,tanggal,103) as tgl,keterangan,nilai,modul "+
		             "from pbh_pb_m a "+
					 "where progress='4' and no_kas = '-' ";    					 
	...
	doTampilData5: function(page) {		
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU5.rs.rows[i];									
			this.sg5.appendData([line.status.toUpperCase(),line.no_spb,line.bank_trans,line.no_pb,line.tgl,line.keterangan,floatToNilai(line.nilai),line.modul,"Rekening"]);
		}
	...
	doDoubleClick5: function(sender, col , row) {
		...
		if (noPB != "") {					
            var strSQL = "select a.bank,a.nama,a.no_rek,a.nama_rek,a.bruto,a.pajak,a.nilai "+
						 "from pbh_rek a inner join pbh_pb_m b on a.no_bukti=b.no_pb and a.modul<>'PINBUK-C' "+
		...
		else system.alert(this,"Data tidak valid.","Tidak ada PB yang berstatus BAYAR.");			
	},
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.jenis,a.no_dokumen,a.keterangan,a.nilai "+
					 "from kas_m a "+						 
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'KBPB' and a.posted ='F'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				this.dataJU5 = {rs:{rows:[]}};							                 
				var strSQL = "select 'BAYAR' as status,no_spb,bank_trans,no_pb,convert(varchar,tanggal,103) as tgl,keterangan,nilai,modul "+
		             		"from pbh_pb_m "+
					 		"where progress='5' and no_kas = '"+this.e_nb.getText()+"' ";							 
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					this.dataJU5 = data;
					this.sgn5.setTotalPage(Math.ceil(data.rs.rows.length/20));
