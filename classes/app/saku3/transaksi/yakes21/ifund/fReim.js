window.app_saku3_transaksi_yakes21_ifund_fReim = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_ifund_fReim.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_ifund_fReim";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Reimburse IF", 0);	
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:9,
		            colTitle:["Status","No Pengajuan","Tanggal","Uraian","Nilai","Akun/Nikes","PP/Nama","DRK/AhliWaris","File Dok"],
		...
		this.sg4 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:9,
					colTitle:["Status","No Pengajuan","Tanggal","Uraian","Nilai","Akun","PP","DRK","File Dok"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[70,150,150,150,100,250,70,100,70]],
					columnReadOnly:[true,[0,1,2,3,4,5,6,7],[]],
					buttonStyle:[[0],[bsAuto]],colFormat:[[4,8],[cfNilai,cfButton]],
					colAlign:[[8],[alCenter]],	
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('SAPHIF','SAPHDAKEM')",true);			
...
window.app_saku3_transaksi_yakes21_ifund_fReim.extend(window.childForm);
window.app_saku3_transaksi_yakes21_ifund_fReim.implement({	
	...
	isiCBnik: function() {
		try{			
			//sesuai login pemegang IF
			this.cb_nik.setSQL("select a.nik, a.nama from karyawan a inner join if_nik b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi and b.jenis='OPERASIONAL' "+
			...
			var strSQL = "select nik,no_kas from if_nik where flag_aktif='1' and nik='"+this.app._userLog+"' and kode_lokasi='"+this.app._lokasi+"' and jenis='OPERASIONAL'";							  						
			...
			var strSQL = "select a.kode_pp,a.nik_app,a.nilai - isnull(b.pakai,0) as saldo, a.bank_trans,a.bank,a.no_rek,a.nama_rek,a.akun_if,a.nilai as nilai_if  "+
						...
						"			 where b.no_reim is null and a.nik_if='"+this.app._userLog+"' and a.no_kasopen='"+this.noKasOpen+"' and a.kode_lokasi='"+this.app._lokasi+"' "+
						"			 group by a.nik_if,a.kode_lokasi "+					
						"		) b on a.nik = b.nik_if and a.kode_lokasi=b.kode_lokasi "+

						"where a.jenis='OPERASIONAL' and a.nik ='"+this.app._userLog+"' and a.no_kas='"+this.noKasOpen+"' and a.kode_lokasi='"+this.app._lokasi+"'";						   
	...
	doLoad:function(sender){		
		try {
			if (this.cb_nik.getText() != "" && this.e_periode.getText() != "") {
				var data = this.dbLib.getDataProvider(
						"select 'PROSES' as status,a.no_aju,convert(varchar,a.tanggal,103) as tgl,d.kode_pp+' | '+d.nama as pp,b.kode_akun+' | '+b.nama as akun,c.kode_drk+' | '+c.nama as drk,a.keterangan,a.nilai-a.npajak as nilai "+
						"from if_aju_m a "+
						...
						"where a.no_kasopen ='"+this.noKasOpen+"' and a.nik_if ='"+this.cb_nik.getText()+"' and a.posted='T' and a.progress = '1'  and a.no_reim='-' and a.kode_lokasi='"+this.app._lokasi+"' and a.periode<='"+this.e_periode.getText()+"' "+
						
						"union all "+
						"select 'PROSES' as status,a.no_aju,convert(varchar,a.tanggal,103) as tgl,isnull(f.nama_nikes,a.kode_pp) as pp,isnull(f.nikkes,a.kode_akun) as akun,isnull(f.namaaw,a.kode_drk) as drk, a.keterangan,a.nilai-a.npajak as nilai "+
						"from if_aju_m a "+						
						"	 inner join if_nik e on a.nik_if=e.nik and a.no_kasopen=e.no_kas and e.flag_aktif='1' "+
						"	 inner join yk_dakem_d f on a.no_aju=f.kdtrans "+
						"where a.no_kasopen ='"+this.noKasOpen+"' and a.nik_if ='"+this.cb_nik.getText()+"' and a.posted='T' and a.progress = '1'  and a.no_reim='-' and a.kode_lokasi='"+this.app._lokasi+"' and a.periode<='"+this.e_periode.getText()+"'"
				...
				var data = this.dbLib.getDataProvider(
						"select case progress when '1' then 'ONPRO-PST' else 'ONPRO-APP' end as status,a.no_aju,convert(varchar,a.tanggal,103) as tgl,d.kode_pp+' | '+d.nama as pp,b.kode_akun+' | '+b.nama as akun,c.kode_drk+' | '+c.nama as drk,a.keterangan,a.nilai-a.npajak as nilai "+
						"from if_aju_m a "+
						"	 inner join if_nik e on a.nik_if=e.nik and a.no_kasopen=e.no_kas and e.flag_aktif='1' "+
						"	 inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
						"    inner join pp d on a.kode_pp=d.kode_pp and a.kode_lokasi=d.kode_lokasi "+	
						"	 left join drk c on a.kode_drk=c.kode_drk and a.kode_lokasi=c.kode_lokasi and substring(a.periode,1,4)=c.tahun "+
						"where a.no_kasopen ='"+this.noKasOpen+"' and a.nik_if ='"+this.cb_nik.getText()+"' and a.posted<>'T' and a.no_reim='-' and a.kode_lokasi='"+this.app._lokasi+"' and a.periode<='"+this.e_periode.getText()+"' "+
						
						"union all "+
						"select case progress when '1' then 'ONPRO-PST' else 'ONPRO-APP' end as status,a.no_aju,convert(varchar,a.tanggal,103) as tgl,isnull(f.nama_nikes,a.kode_pp) as pp,isnull(f.nikkes,a.kode_akun) as akun,isnull(f.namaaw,a.kode_drk) as drk, a.keterangan,a.nilai-a.npajak as nilai "+
						"from if_aju_m a "+						
						"	 inner join if_nik e on a.nik_if=e.nik and a.no_kasopen=e.no_kas and e.flag_aktif='1' "+
						"	 inner join yk_dakem_d f on a.no_aju=f.kdtrans "+
						"where a.no_kasopen ='"+this.noKasOpen+"' and a.nik_if ='"+this.cb_nik.getText()+"' and a.posted<>'T' and a.no_reim='-' and a.kode_lokasi='"+this.app._lokasi+"' and a.periode<='"+this.e_periode.getText()+"'"
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];																	
						this.sg4.appendData([line.status.toUpperCase(),line.no_aju,line.tgl,line.keterangan,floatToNilai(line.nilai),line.akun,line.pp,line.drk,"File..."]);
					}
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into pbh_rek(kode_vendor,nu,no_bukti,kode_lokasi,modul,nama_rek,no_rek,bank,nama,bruto,pajak,nilai,bank_trans) values "+
							"('"+this.cb_nik.getText()+"',1,'"+this.e_nb.getText()+"','"+this.app._lokasi+"','IFREIM','"+this.e_namarek.getText()+"','"+this.e_norek.getText()+"','"+this.e_bank.getText()+"','"+this.cb_nik.rightLabelCaption+"',"+niFinal+",0,"+niFinal+",'"+this.e_banktrans.getText()+"')");
					
					// dimatikan krn akun hutang if dakem beda
					...
					sql.add("insert into pbh_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) "+
							"select '"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',999,b.kode_akun,'"+this.e_ket.getText()+"','D',sum(b.nilai),'"+this.app._kodePP+"','-','"+this.app._lokasi+"','IFREIM','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1 "+
							"from if_aju_m a inner join if_aju_j b on a.no_aju=b.no_aju and b.jenis='HUTIF' "+
							"where a.no_reim='"+this.e_nb.getText()+"' "+
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.keterangan,a.tanggal,a.kode_pp,a.nik_buat,a.modul,b.nik_app,b.nik_ver,a.no_dokumen as no_kasopen "+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){								
						...
						this.cb_nik.setSQL("select a.nik, a.nama from karyawan a where a.nik='"+this.app._userLog+"' and a.kode_lokasi='"+this.app._lokasi+"'",["a.nik","a.nama"],false,["Kode","Nama"],"and","Data Pemegang",true);
				...
				var strSQL = "select a.kode_pp,a.nik_app,a.nilai - isnull(b.pakai,0) + isnull(c.pakai_current,0) as saldo, a.bank_trans,a.bank,a.no_rek,a.nama_rek,a.akun_if,a.nilai as nilai_if  "+
							"from if_nik a "+

							"		left join  ("+						
							"			 select a.nik_if,a.kode_lokasi,sum(a.nilai-a.npajak) as pakai_current "+
							"			 from if_aju_m a "+							
							"			 where a.no_reim = '"+this.e_nb.getText()+"' and a.nik_if='"+this.app._userLog+"' and a.no_kasopen='"+this.noKasOpen+"' and a.kode_lokasi='"+this.app._lokasi+"' "+
							"			 group by a.nik_if,a.kode_lokasi "+					
							"		) c on a.nik = c.nik_if and a.kode_lokasi=c.kode_lokasi "+
