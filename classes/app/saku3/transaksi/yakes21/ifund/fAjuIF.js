window.app_saku3_transaksi_yakes21_ifund_fAjuIF = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_ifund_fAjuIF.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_ifund_fAjuIF";
...
window.app_saku3_transaksi_yakes21_ifund_fAjuIF.extend(window.childForm);
window.app_saku3_transaksi_yakes21_ifund_fAjuIF.implement({	
	isiCBnik: function() {
		try {
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into if_aju_m(no_aju,kode_lokasi,periode,tgl_input,user_input,tanggal,tgl_kuitansi,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,sts_pajak,npajak,no_app,progress,nik_app,no_reim,no_kasopen,posted,nik_setuju, nilai_dpp,persen, nik_if,form) values "+
							...
							this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",'"+this.c_status.getText()+"',"+nilaiToFloat(this.e_npajak.getText())+",'-','0','"+this.cb_app.getText()+"','-','"+this.noKasOpen+"','X','"+this.cb_app.getText()+"', "+nilaiToFloat(this.e_ndpp.getText())+","+nilaiToFloat(this.e_npersen.getText())+",'"+this.cb_if.getText()+"','REGULER')");
	...
	doChange:function(sender){
		try {
			if (sender == this.cb_pp && this.cb_pp.getText()!= "") {
				this.cb_akun.setSQL(
					"select distinct a.kode_akun, a.nama from masakun a inner join angg_r b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and substring(b.periode1,1,4) = '"+this.e_periode.getText().substr(0,4)+"' and b.kode_pp='"+this.cb_pp.getText()+"' where a.block='0' and a.kode_lokasi='"+this.app._lokasi+"' and b.modul='RELEASE' and a.modul<>'A' "+
					"union all "+
					"select a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '010' where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'"
	...
	doLoad3:function(sender){																								
		...
		var strSQL = "select a.no_aju,convert(varchar,a.tanggal,103) as tgl,modul,a.keterangan,a.nilai "+
					 ...
					 "where a.form='REGULER' and a.posted<>'T' and a.modul='IFAJU' "+bidang+" and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress in ('0','R') order by a.no_aju";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.keterangan,a.tgl_kuitansi,a.tanggal,a.nik_app, "+
							 "       a.kode_akun,a.kode_pp,a.kode_drk,a.nilai,a.sts_pajak,a.npajak,a.nik_setuju, a.nilai_dpp,a.persen,a.nik_if,a.no_kasopen "+
							 "from if_aju_m a "+							 
