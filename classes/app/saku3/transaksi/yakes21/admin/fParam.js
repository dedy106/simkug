window.app_saku3_transaksi_yakes21_admin_fParam = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_admin_fParam.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_admin_fParam";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Parameter Sistem", 0);	
		...
		this.e_modul = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,12,180,20],caption:"Modul", maxLength:10, tag:1});	
...
window.app_saku3_transaksi_yakes21_admin_fParam.extend(window.childForm);
window.app_saku3_transaksi_yakes21_admin_fParam.implement({
	...
	doLoad:function(sender){						
		var strSQL = "select kode_spro,nama,modul,flag,value1,value2,keterangan "+
		             "from spro where kode_lokasi='"+this.app._lokasi+"' order by kode_spro";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
			this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length/20));
