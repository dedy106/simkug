window.app_saku3_transaksi_yakes21_gl_fJP = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_gl_fJP.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_yakes21_gl_fJP";
		...
		this.ed_debet = new portalui_saiLabelEdit(this,{bound:[700,10,220,20],caption:"Total Debet",tipeText:ttNilai, alignment:alRight,readOnly:true,text:"0"});
		...
		this.bTampil = new portalui_button(this,{bound:[580,13,100,20],caption:"Tampil",hint:"Load",image:"icon/"+system.getThemes()+"/tabCont2.png",click:[this,"doHitungTB"]});
		...
		this.ed_desc.setText("Jurnal Penutup periode "+this.ed_period.getText());
		...
		sql.add("select cast(value1 as varchar) as value1 from spro where kode_spro = 'MAXPRD'");
		this.dbLib.getMultiDataProviderA(sql);				
		setTipeButton(tbSimpan);
	}
};
window.app_saku3_transaksi_yakes21_gl_fJP.extend(window.portalui_childForm);
window.app_saku3_transaksi_yakes21_gl_fJP.implement({
	doHitungTB: function(){		
		try{			
					...
					try{						
						if (this.standarLib.checkEmptyByTag(this, new Array("0","1","2"))){
							...
							sql.add("insert into jp(no_bukti, tanggal, periode, keterangan, nik_pembuat, kode_lokasi, nilai, tgl_input, nik_user )values"+
									"('"+this.ed_nb.getText()+"','"+tgl+"','"+this.ed_period.getText()+"','Jurnal JP periode "+this.ed_period.getText()+"', "+
									"'"+this.app._userLog+"','"+this.app._lokasi+"',"+parseNilai(this.ed_jp.getText())+",getdate(), '"+this.app._userLog+"')");
							
							var line; var jp2=jp3=nilai = 0; var dc = "";							
							for (var i=0;i < this.dataJU.rs.rows.length;i++){
								line = this.dataJU.rs.rows[i];
								
								if (line.kode_akun.substr(0,1) == "4" || line.kode_akun.substr(0,1) == "5") {
									jp2 += parseFloat(line.so_akhir);
								}
								
								if (line.kode_akun.substr(0,1) == "6") {
							...
							if (jp2 > 0) dc = "D"; else dc = "C";							
							jp2 = Math.abs(jp2);
							sql.add("insert into gldt (no_bukti,no_urut,kode_lokasi,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,modul,jenis,periode,kode_curr,kurs,nilai_curr,tgl_input,nik_user) values "+
									"('"+this.ed_nb.getText()+"',99998,'"+this.app._lokasi+"','-','"+tgl+"','31010101','"+dc+"',"+jp2+",'Jurnal Penutup Periode "+this.ed_period.getText()+"','-','-','-','-','-','-','-','-','JP','JP','"+this.ed_period.getText()+"','IDR',1,"+jp2+",getdate(),'"+this.app._userLog+"')");
							
							
							if (jp3 > 0) dc = "D"; else dc = "C";							
							jp3 = Math.abs(jp3);							
							sql.add("insert into gldt (no_bukti,no_urut,kode_lokasi,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,modul,jenis,periode,kode_curr,kurs,nilai_curr,tgl_input,nik_user) values "+
									"('"+this.ed_nb.getText()+"',99999,'"+this.app._lokasi+"','-','"+tgl+"','31020101','"+dc+"',"+jp3+",'Jurnal Penutup Periode "+this.ed_period.getText()+"','-','-','-','-','-','-','-','-','JP','JP','"+this.ed_period.getText()+"','IDR',1,"+jp3+",getdate(),'"+this.app._userLog+"')");
