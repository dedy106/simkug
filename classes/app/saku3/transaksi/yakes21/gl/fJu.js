window.app_saku3_transaksi_yakes21_gl_fJu = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_gl_fJu.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_gl_fJu";		
...
window.app_saku3_transaksi_yakes21_gl_fJu.extend(window.childForm);
window.app_saku3_transaksi_yakes21_gl_fJu.implement({		
						...
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){
								sql.add("insert into ju_j (no_ju, no_dokumen, tanggal, no_urut, kode_akun, keterangan, dc, nilai, kode_pp, kode_drk, kode_lokasi, modul, jenis, periode, kode_curr, kurs, nik_user, tgl_input) values "+
										"('"+this.e_nb.getText()+"', '"+this.e_dok.getText()+"', '"+this.dp_d1.getDateString()+"', "+i+", '"+this.sg1.cells(0,i)+"', '"+this.sg1.cells(3,i)+"', '"+this.sg1.cells(2,i).toUpperCase()+"', "+nilaiToFloat(this.sg1.cells(4,i))+", '"+this.sg1.cells(5,i)+"', '"+this.sg1.cells(7,i)+"', '"+this.app._lokasi+"', 'JU', 'UMUM', '"+this.e_periode.getText()+"', 'IDR', 1, '"+this.app._userLog+"', getdate())");
							}
						}
					}
