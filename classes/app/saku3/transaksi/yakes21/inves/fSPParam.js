window.app_saku3_transaksi_yakes21_inves_fSPParam = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_inves_fSPParam.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_inves_fSPParam";
...
window.app_saku3_transaksi_yakes21_inves_fSPParam.extend(window.childForm);
window.app_saku3_transaksi_yakes21_inves_fSPParam.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("delete from inv_sp_param where kode_plan='"+this.cb_plan.getText()+"'");

					for (var i = 0; i < this.sg3.rows.getLength();i++){
						if (this.sg3.rowValid(i)) {
							sql.add("insert into inv_sp_param(kode_plan,kode_param,flag) values "+
		...
		try{
			if (sender == this.cb_plan && this.cb_plan.getText() != ""){
				var strSQL = "select a.kode_spro, a.nama, isnull(b.flag,'-') as flag  "+
							 "from spro a "+
							 "left join inv_sp_param b on a.kode_spro=b.kode_param and b.kode_plan='"+this.cb_plan.getText()+"'  "+
							 "where a.kode_lokasi='"+this.app._lokasi+"' and a.kode_spro in ( "+							 
							
							 "	'PPINV','DRKSPB','SPHUT','SPDRKSPI','SPNT','DRKSPJ','SPPIU','SPPIUGL','SPGL','SPDEV','SPPPHM' "+
