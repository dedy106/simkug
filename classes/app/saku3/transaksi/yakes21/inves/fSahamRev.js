window.app_saku3_transaksi_yakes21_inves_fSahamRev = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_inves_fSahamRev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_inves_fSahamRev";
...
window.app_saku3_transaksi_yakes21_inves_fSahamRev.extend(window.childForm);
window.app_saku3_transaksi_yakes21_inves_fSahamRev.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from inv_dok where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='SHMREV'");								
					}
					...
					sql.add("insert into inv_shmjual_m(no_shmjual,kode_lokasi,periode,tanggal,nik_user,tgl_input,posted,no_kasjual,nik_buat,no_dokumen,keterangan,kode_drk,kode_kelola,tgl_set,nilai_komisi,nilai_ppn,nilai_levy,nilai_pph,akun_piutang,akun_piugl,nilai_piutang,nilai_piugl,modul,kode_pp,no_app1,progress,kode_plan) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.app._userLog+"',getdate(),'X','-','"+this.app._userLog+"',  '-','"+this.e_ket.getText()+"','-','"+this.cb_saham.getText()+"','"+this.dp_d1.getDateString()+"',"+nilaiToFloat(this.e_bagi.getText())+",0,0,0,'-','-',0,0,'SHMREV','"+this.kodepp+"','-','0','"+this.cb_plan.getText()+"')");
					...
					for (var i = 0; i < this.sg.rows.getLength();i++){
						if (this.sg.rowValid(i) && this.sg.cells(7,i) != "" && this.sg.cells(7,i) != "0"){							
							sql.add("insert into inv_shmjual_d (no_shmjual,kode_kelola,kode_saham,h_oleh,h_buku,h_jual,jumlah,n_jual,gainlos, komisi,vat,levi,pph,kode_broker,kode_plan) values "+
								    "('"+this.e_nb.getText()+"','"+this.sg.cells(0,i)+"','"+this.sg.cells(2,i)+"',"+nilaiToFloat(this.sg.cells(4,i))+",0,0,"+nilaiToFloat(this.sg.cells(7,i))+",0,0, 0,0,0,0,'-','"+this.cb_plan.getText()+"')");
						}						
					}
					...
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i) && this.sgUpld.cells(2,i)!="-") {							
							...
							sql.add("insert into inv_dok(no_bukti,no_gambar,nu,kode_jenis,kode_lokasi,modul,no_ref) values ('"+this.e_nb.getText()+"','"+this.sgUpld.cells(3,i).tmpfile+"','"+ix+"','"+this.sgUpld.cells(0,i)+"','"+this.app._lokasi+"','SHMREV','"+this.e_nb.getText()+"')");															
						}	
					}
		...
		try{
			...
			if (this.sg3.cells(0,baris) != "") {			
				...
				var strSQL = "select a.kode_kelola,b.nama as nama_kelola,a.kode_saham,c.nama as nama_saham,a.jumlah,a.h_oleh,a.h_buku,a.jumlah - round(a.jumlah * "+nilaiToFloat(this.e_bagi.getText())+",0) as kurang, round(a.jumlah * "+nilaiToFloat(this.e_bagi.getText())+",0) as lbr_akhir "+
							 ...
							 "                     inner join inv_saham c on a.kode_saham=c.kode_saham "+
							 "					   inner join inv_shmjual_d d on a.kode_kelola=d.kode_kelola and a.kode_saham=d.kode_saham and a.kode_plan=d.kode_plan "+
							 "where a.kode_plan='"+this.cb_plan.getText()+"' and d.no_shmjual='"+this.e_nb.getText()+"' and a.nik_user='"+this.nik_user+"' order by a.kode_kelola";
				var data1 = this.dbLib.getDataProvider(strSQL,true);	
				if (typeof data1 == "object" && data1.rs.rows[0] != undefined){
					var line1;
					this.sg.clear();
