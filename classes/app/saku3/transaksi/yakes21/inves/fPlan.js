window.app_saku3_transaksi_yakes21_inves_fPlan = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_inves_fPlan.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_inves_fPlan";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Plan Asset", 0);	
		
		uses("saiCB;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		this.pc2 = new pageControl(this,{bound:[10,10,1000,450], childPage:["List Data","Data Plan"]});		
...
window.app_saku3_transaksi_yakes21_inves_fPlan.extend(window.childForm);
window.app_saku3_transaksi_yakes21_inves_fPlan.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into inv_plan(kode_plan,nama) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update inv_plan "+
							"set nama='"+this.e_nama.getText()+"' "+
							"where kode_plan='"+this.cb_kode.getText()+"'");			
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
	...
	doLoad3:function(sender){														
		var strSQL = "select kode_plan,nama from inv_plan order by kode_plan";
		...
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			...
			for (var i=0;i<this.dataJU3.rs.rows.length;i++){
				...
				this.sg3.appendData([line.kode_plan,line.nama,"Pilih"]); 
			}			
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select kode_plan,nama "+
				             "from inv_plan "+
						     "where kode_plan ='"+this.cb_kode.getText()+"' ";						   
				var data = this.dbLib.getDataProvider(strSQL,true);
