window.app_saku3_transaksi_yakes21_inves_fObliParam = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_inves_fObliParam.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_inves_fObliParam";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Parameter Jurnal Obligasi", 0);	
...
window.app_saku3_transaksi_yakes21_inves_fObliParam.extend(window.childForm);
window.app_saku3_transaksi_yakes21_inves_fObliParam.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("delete from inv_obli_param where kode_plan='"+this.cb_plan.getText()+"'");

					for (var i = 0; i < this.sg3.rows.getLength();i++){
						if (this.sg3.rowValid(i)) {
							sql.add("insert into inv_obli_param(kode_plan,kode_param,flag) values "+
		...
		try{
			if (sender == this.cb_plan && this.cb_plan.getText() != ""){
				var strSQL = "select a.kode_spro, a.nama, isnull(b.flag,'-') as flag  "+
							 "from spro a "+
							 "left join inv_obli_param b on a.kode_spro=b.kode_param and b.kode_plan='"+this.cb_plan.getText()+"'  "+
							 "where a.kode_lokasi='"+this.app._lokasi+"' and a.kode_spro in ( "+							 
							
							 "	'PPINV','AKUNOBLI','PIUJUAL','PIUKUPON','AKUNKUPON','PDPTAMOR','AKUNSPI','AKUNGL', "+
							 "  'OBLIHUT','DRKOBLIB','OBLINT','DRKOBLSPI','OBLAMOR'  "+
