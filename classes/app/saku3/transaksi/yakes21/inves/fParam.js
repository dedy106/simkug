window.app_saku3_transaksi_yakes21_inves_fParam = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_inves_fParam.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_inves_fParam";
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:8,tag:9,
		            colTitle:["Kode","Nama","Modul","Flag","Value1","Value2","Keterangan","Pilih"],
					colWidth:[[7,6,5,4,3,2,1,0],[70,200,100,100,100,50,250,50]],					
					colFormat:[[7],[cfButton]],readOnly:true,
					click:[this,"doSgBtnClick1"], colAlign:[[7],[alCenter]],
		...
		this.e_modul = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,12,200,20],caption:"Modul", maxLength:10, tag:2,text:"INV19",readOnly:true});	
...
window.app_saku3_transaksi_yakes21_inves_fParam.extend(window.childForm);
window.app_saku3_transaksi_yakes21_inves_fParam.implement({
	...
	doSgBtnClick1: function(sender, col, row){
		try{
			if (col == 7) this.doDoubleClick(this.sg1,0,row); 				
		}catch(e){
			alert(e);
		}
	},
	...
	doCari:function(sender){								
		...
		var strSQL = "select kode_spro,nama,modul,flag,value1,value2,keterangan "+
		             "from spro "+
					 "where "+filter+" kode_lokasi='"+this.app._lokasi+"' and modul='INV19' order by kode_spro";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
	...
	doLoad:function(sender){						
		var strSQL = "select kode_spro,nama,modul,flag,value1,value2,keterangan "+
		             "from spro where kode_lokasi='"+this.app._lokasi+"' and modul='INV19' order by kode_spro";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
			this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length/20));
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_spro,line.nama,line.modul,line.flag,floatToNilai(line.value1),floatToNilai(line.value2),line.keterangan,"Pilih"]); 
		}
