window.app_saku3_transaksi_yakes21_inves_fRDSPI = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_yakes21_inves_fRDSPI.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_inves_fRDSPI";
...
window.app_saku3_transaksi_yakes21_inves_fRDSPI.extend(window.childForm);
window.app_saku3_transaksi_yakes21_inves_fRDSPI.implement({
	doHitung: function() {
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into inv_rdspi_m(no_spi,no_dokumen,tanggal,keterangan,kode_curr,kurs,nilai,kode_pp,kode_drk,posted,modul,nik_buat,nik_setuju,kode_lokasi,periode,no_del,no_link,nik_user,tgl_input,jenis,no_spikug,no_app1,progress,kode_plan) values "+
						    "('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.kodepp+"','"+this.cb_drk.getText()+"','F','RDSPI','"+this.app._userLog+"','-','"+this.app._lokasi+"','"+this.e_periode.getText()+"','-','-','"+this.app._userLog+"',getdate(),'"+this.c_jenis.getText()+"','-','-','0','"+this.cb_plan.getText()+"')");
