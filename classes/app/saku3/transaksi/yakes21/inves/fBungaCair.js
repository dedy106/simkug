window.app_saku3_transaksi_yakes21_inves_fBungaCair = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_inves_fBungaCair.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_inves_fBungaCair";
...
window.app_saku3_transaksi_yakes21_inves_fBungaCair.extend(window.childForm);
window.app_saku3_transaksi_yakes21_inves_fBungaCair.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into inv_depocair_m(no_cair,no_depo,tanggal,keterangan,kode_curr,kurs,nilai_rev,nilai,kode_pp,kode_drk,posted,modul,nik_buat,nik_setuju,kode_lokasi,periode,no_del,no_link,nik_user,tgl_input, no_app,progress) values "+
						    "('"+this.e_nb.getText()+"','"+this.cb_depo.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','IDR',1,"+nilaiToFloat(this.e_akru.getText())+","+nilaiToFloat(this.e_nilai.getText())+",'"+this.kodePP+"','"+this.cb_drk.getText()+"','F','DOCCAIR','"+this.app._userLog+"','-','"+this.app._lokasi+"','"+this.e_periode.getText()+"','-','"+this.cb_kelola.getText()+"','"+this.app._userLog+"',getdate(), '-','0')");
