window.app_saku3_transaksi_yakes21_inves_fSahamMI = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_inves_fSahamMI.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_inves_fSahamMI";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Manager Investasi Saham", 0);	
		...
		this.c_jenis = new saiCB(this.pc2.childPage[1],{bound:[20,12,200,20],caption:"Jenis",items:["MI","SWAKELOLA"], readOnly:true,tag:2});		
		this.c_aktif = new saiCB(this.pc2.childPage[1],{bound:[20,15,200,20],caption:"Status Aktif",items:["1. YA","0. TIDAK"], readOnly:true,tag:2});		
...
window.app_saku3_transaksi_yakes21_inves_fSahamMI.extend(window.childForm);
window.app_saku3_transaksi_yakes21_inves_fSahamMI.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into inv_kelola(kode_kelola,nama,alias,jenis,flag_aktif,gambar) values "+
						    "('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.e_alias.getText()+"','"+this.c_jenis.getText()+"','"+this.c_aktif.getText().substr(0,1)+"','"+this.e_foto.getText()+"')");
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update inv_kelola set nama='"+this.e_nama.getText()+"',alias='"+this.e_alias.getText()+"',jenis='"+this.c_jenis.getText()+"',flag_aktif='"+this.c_aktif.getText().substr(0,1)+"',gambar='"+this.e_foto.getText()+"' "+
	...
	doLoad3:function(sender){														
		var strSQL = "select kode_kelola,nama,case flag_aktif when '0' then 'T' else 'Y' end as aktif,flag_aktif from inv_kelola order by flag_aktif desc,kode_kelola";
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.kode_kelola,line.nama,line.aktif.toUpperCase(),"Pilih"]); 
		}
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select * from inv_kelola where kode_kelola ='"+this.cb_kode.getText()+"'";
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
