window.app_saku3_transaksi_yakes21_inves_fRDBonus = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_inves_fRDBonus.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_inves_fRDBonus";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Unit RD Bonus", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,22,996,305], childPage:["Data RD","Rekap Data","File Dok"]});		
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:6,tag:0,				
				colTitle:["Kd RD","Nama RD","Lbr Unit","Harga Oleh","Harga Buku","Lbr Bonus"],
				colWidth:[[5,4,3,2,1,0],[120,100,100,120,200,80]],
		...
		try {
			...
			this.cb_kelola.setSQL("select kode_rdkelola, nama from inv_rdkelola where flag_uni='1' and flag_aktif='1'",["kode_rdkelola","nama"],false,["Kode","Nama"],"and","Daftar Pengelola",true);			
...
window.app_saku3_transaksi_yakes21_inves_fRDBonus.extend(window.childForm);
window.app_saku3_transaksi_yakes21_inves_fRDBonus.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from inv_dok where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='RDBONUS'");			
					}
					...
					for (var i = 0; i < this.sg2.rows.getLength();i++){
						if (this.sg2.rowValid(i)) {
							...
							if (typeof data0 == "object"){
								...
								if (line0 == undefined){									
									sql.add("insert into inv_rd_d(kode_plan,kode_rd) values "+
											"('"+this.cb_plan.getText()+"','"+this.sg2.cells(0,i)+"')");
								}
							}					
						}
					}
					...
					sql.add("insert into inv_rdbeli_m(no_rdbeli,kode_lokasi,periode,tanggal,nik_user,tgl_input,posted,no_kas,progress,nik_buat,  no_dokumen,keterangan,kode_drk,kode_rdkelola,tgl_set,nilai_komisi,nilai_ppn,nilai_levy,nilai_pph,no_app1,no_spb,kode_pp,modul,nilai, nik_app,kode_plan) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.app._userLog+"',getdate(),'X','RDBONUS','0','"+this.app._userLog+"',  '-','"+this.e_ket.getText()+"','-','"+this.cb_kelola.getText()+"','"+this.dp_d1.getDateString()+"',0,0,0,0,'-','-','"+this.kodepp+"','RDBONUS',0,'-','"+this.cb_plan.getText()+"')");										
					
					for (var i = 0; i < this.sg.rows.getLength();i++){
						if (this.sg.rowValid(i) && this.sg.cells(5,i) != "" && this.sg.cells(5,i) != "0"){
							sql.add("insert into inv_rdbeli_d (no_rdbeli,kode_rdkelola,kode_rd,h_oleh,h_buku,harga,jumlah,n_beli,  komisi,vat,levi,pph,kode_plan) values "+
									"('"+this.e_nb.getText()+"','"+this.cb_kelola.getText()+"','"+this.sg.cells(0,i)+"',"+nilaiToFloat(this.sg.cells(3,i))+","+nilaiToFloat(this.sg.cells(4,i))+",0,"+nilaiToFloat(this.sg.cells(5,i))+",0,0,0,0,0,'"+this.cb_plan.getText()+"')");
						}						
					}
					...
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i) && this.sgUpld.cells(2,i)!="-") {							
							...
							sql.add("insert into inv_dok(no_bukti,no_gambar,nu,kode_jenis,kode_lokasi,modul,no_ref) values ('"+this.e_nb.getText()+"','"+this.sgUpld.cells(3,i).tmpfile+"','"+ix+"','"+this.sgUpld.cells(0,i)+"','"+this.app._lokasi+"','RDBONUS','"+this.e_nb.getText()+"')");															
						}	
					}	
	...
	doChange:function(sender){		
		if (sender == this.cb_plan && this.cb_plan.getText()!="") {		
			var data = this.dbLib.getDataProvider("select kode_param,flag from inv_rd_param where kode_plan = '"+this.cb_plan.getText()+"' and kode_param in ('PPINV')",true);						
			if (typeof data == "object"){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];																	
		...
		if (sender == this.cb_kelola && this.cb_kelola.getText()!="") {
			var sql = new server_util_arrayList();						
			sql.add("select kode_rd, nama from inv_rd where kode_rdkelola='"+this.cb_kelola.getText()+"' and flag_aktif='1'");				
	...
	doClick:function(sender){
		if (sender == this.i_gen) {			
			...
			setTipeButton(tbSimpan);
			this.stsSimpan = 1;			
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"inv_rdbeli_m","no_rdbeli",this.app._lokasi+"-BNS"+this.e_periode.getText().substr(2,4)+".","0000"));
			this.e_ket.setFocus();
		}		
	},		
		...
		if (col == 0 && this.sg.cells(0,row)!="") {
			if (this.sg.cells(0,row) != "") {
				...
				if (rd) sender.cells(1,row,rd);
					...
					if (trim(sender.cells(0,row)) != "") system.alert(this,"Kode RD "+sender.cells(0,row)+" tidak ditemukan","Inputkan kode lainnya.","checkSaham");                
					sender.cells(0,row,"");
					sender.cells(1,row,"");
	...
	doLoad3:function(sender){														
		var strSQL = "select a.no_rdbeli,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.kode_kelola "+
		             "from inv_rdbeli_m a "+
					 "where a.progress in ('0','M') and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='RDBONUS'";
		var data = this.dbLib.getDataProvider(strSQL,true);
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_rdbeli,line.tgl,line.keterangan,line.kode_kelola,"Pilih"]); 
		}
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			...
			if (this.sg3.cells(0,baris) != "") {			
				...
				strSQL = "select a.kode_rd,b.nama,c.jumlah as jml_seb,a.h_oleh,a.h_buku,a.jumlah "+
				...
				if (typeof data1 == "object" && data1.rs.rows[0] != undefined){
					...
					for (var i in data1.rs.rows){
						...
						this.sg.appendData([line1.kode_rd,line1.nama,floatToNilai(line1.jml_seb),parseFloat(line1.h_oleh),parseFloat(line1.h_buku),parseFloat(line1.jumlah)]);
					}
