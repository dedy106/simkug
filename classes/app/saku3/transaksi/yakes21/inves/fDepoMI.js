window.app_saku3_transaksi_yakes21_inves_fDepoMI = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_inves_fDepoMI.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_inves_fDepoMI";
...
window.app_saku3_transaksi_yakes21_inves_fDepoMI.extend(window.childForm);
window.app_saku3_transaksi_yakes21_inves_fDepoMI.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					//jurnal di kasbank (cashbasis) --> krn yg pegang uangnya juga di MI 
					sql.add("insert into inv_shop_m (no_shop,periode,nik_user,tgl_input,kode_lokasi,tanggal,keterangan,nik_app,tgl_awal,tgl_akhir,jml_hari,jml_bulan,progress,nilai,no_app,no_spb,bsumber,  nodin,kepada1,dari1,lamp1,hal1,nikttd1,jab1,  noins,nikttd2,jab2,nikttd3,jab3,just,kode_pp,modul,posted,nik_sap,kode_plan) values "+
							"('"+this.e_nb.getText()+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.app._lokasi+"','"+this.dp_d2.getDateString()+"','"+this.e_ket.getText()+"','-','"+this.dp_d2.getDateString()+"','"+this.dp_d2.getDateString()+"',"+jmlHari+","+jmlBulan+",'0',"+nilaiToFloat(this.e_nilai.getText())+",'-','-','-', '-','-','-','-','-','-','-','-','-','-','-','-','-','"+this.app._kodePP+"','MI','X','-','"+this.cb_plan.getText()+"')");					
	...
	doLoad3:function(sender){																									
		var strSQL = "select a.no_depo,convert(varchar,a.tgl_mulai,103) as mulai,convert(varchar,a.tgl_selesai,103) as selesai,a.keterangan,a.bdepo,b.nama as nama_bank,a.nilai,a.no_bilyet "+
					 ...
					 "where a.no_kas='-' and c.modul='MI' and a.progress = '0' and a.kode_lokasi = '"+this.app._lokasi+"' order by a.no_depo desc";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.sg3.clear();
			this.dataJU3 = data;
