window.app_saku3_transaksi_yakes21_inves_fKbSahamJual = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_inves_fKbSahamJual.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_inves_fKbSahamJual";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,22,995,282], childPage:["Data Saham"]});		
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:10,tag:9,				
				...
				colWidth:[[9,8,7,6,5,4,3,2,1,0],[80,100,100,80,80,150,250,80,100,60]],
...
window.app_saku3_transaksi_yakes21_inves_fKbSahamJual.extend(window.childForm);
window.app_saku3_transaksi_yakes21_inves_fKbSahamJual.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBSHMJUAL','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.app._userLog+"','-',getdate(),'"+this.app._userLog+"','F','-','-','"+this.cb_plan.getText()+"','0')");
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i) && this.sg.cells(0,i) == "APP"){							    																							
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.sg.cells(7,i))+",'"+this.app._kodePP+"','-','10','-','"+this.app._lokasi+"','KBSHMJUAL','KBSHM','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'"+this.cb_akun.getText()+"')");
								
								if (nilaiToFloat(this.sg.cells(8,i)) > 0) {
									sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
											"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',1,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.sg.cells(8,i))+",'"+this.app._kodePP+"','-','09','-','"+this.app._lokasi+"','KBSHMJUAL','KBGL','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'"+this.cb_akun.getText()+"')");
									sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
	...
	doLoad:function(sender){
		if (this.e_periode.getText() != "" && this.cb_plan.getText()!="") {			
			var strSQL = "select a.no_shmjual,convert(varchar,a.tanggal,103) as tgl,a.keterangan,c.nama as kelola,a.nilai_piutang,0 as nilai_pph,a.nilai_piugl,a.akun_piutang,a.akun_piugl "+ //a.nilai_pph dijurnal saat input penjualan
						 ...
						 "where a.kode_plan='"+this.cb_plan.getText()+"' and a.progress='1' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' order by a.no_shmjual";						
	...
	doChange:function(sender){
		try{
			if (sender == this.cb_plan && this.cb_plan.getText()!="") {		
				var data = this.dbLib.getDataProvider("select kode_param,flag from inv_saham_param where kode_plan = '"+this.cb_plan.getText()+"' and kode_param in ('SHMPPH')",true);						
				if (typeof data == "object"){
					var line;
					for (var i in data.rs.rows){
						line = data.rs.rows[i];																	
	...
	doLoad3:function(sender){																											
		var strSQL = "select distinct a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
		             "from kas_m a  "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.posted ='F' and a.modul = 'KBSHMJUAL' "+
					 "order by a.no_kas";
