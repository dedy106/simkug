window.app_saku3_transaksi_yakes21_simlog_fNego = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_simlog_fNego.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_simlog_fNego";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Klarifikasi dan Negosiasi", 0);	
		...
		this.cb_buat = new saiCBBL(this.pc2.childPage[0],{bound:[20,16,220,20],caption:"NIK Logistik", multiSelection:false, maxLength:10, tag:2});				
		this.e_nilai = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,16,200,20],caption:"Nilai Disepakati", tag:1, readOnly:true, tipeText:ttNilai, text:"0",change:[this,"doChange"]});				
		
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,290], childPage:["Vendor","Item Barang","File Dok"]});				
		this.e_nospph = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,15,200,20],caption:"No SPPH", readOnly:true});										
		this.e_ketspph = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,17,450,20],caption:"Deskripsi", readOnly:true});										
		...
		this.e_pic = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,16,450,20],caption:"Pihak Vendor", maxLength:50});				
		
		this.sg4 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag: 0,
		            colTitle:["ID","Item Barang","Merk","Tipe","Spesifikasi","Jumlah","Hrg Tawar","Hrg Nego","Total"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[100,80,80,60,170,170,170,170,30]],
					colFormat:[[5,6,7,8],[cfNilai,cfNilai,cfNilai,cfNilai]],
					columnReadOnly:[true,[0,5,6,8],[1,2,3,4,7]],
...
window.app_saku3_transaksi_yakes21_simlog_fNego.extend(window.childForm);
window.app_saku3_transaksi_yakes21_simlog_fNego.implement({	
	isiCBsph: function() {
		this.cb_sph.setSQL("select no_sph, keterangan from log_sph_m where modul in ('PL','TL') and periode ='"+this.e_periode.getText()+"' and no_nego='-' and kode_lokasi='"+this.app._lokasi+"'",["no_sph","keterangan"],false,["No SPH","Deskripsi"],"and","Data SPH",true);			
	},
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into log_nego_m(no_nego,no_tap,kode_lokasi,tgl_input,nik_user,periode,tanggal,nik_nego,nama_pic,keterangan,no_sph,kode_vendor,nilai,ppn,no_dokumen) values "+
							"('"+this.e_nb.getText()+"','-','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_buat.getText()+"','"+this.e_pic.getText()+"','"+this.e_ket.getText()+"','"+this.cb_sph.getText()+"','"+this.kodeVendor+"',"+nilaiToFloat(this.e_nilai.getText())+",0,'"+this.e_dok.getText()+"')");					
					...
					if (this.sg4.getRowValidCount() > 0){
						for (var i=0;i < this.sg4.getRowCount();i++){
							if (this.sg4.rowValid(i)){
								sql.add("update log_sph_d set nilai_nego="+nilaiToFloat(this.sg4.cells(7,i))+" where no_sph='"+this.cb_sph.getText()+"' and no_urut="+this.sg4.cells(0,i));
							}
						}
					}
					...
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i) && this.sgUpld.cells(2,i)!="-") {							
							...
							sql.add("insert into pbh_dok(no_bukti,no_gambar,nu,kode_jenis,kode_lokasi,modul,no_ref) values ('"+this.e_nb.getText()+"','"+this.sgUpld.cells(3,i).tmpfile+"','"+ix+"','"+this.sgUpld.cells(0,i)+"','"+this.app._lokasi+"','NEGO','"+this.e_nb.getText()+"')");															
						}	
					}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :		
				this.preView = "1";					
				//cek budget gak boleh melebihi yg di keep										
				...
				if (typeof data == "object"){
					...
					if (line != undefined){													
						if (parseFloat(line.nilai) < nilaiToFloat(this.e_nilai.getText())) {
							system.alert(this,"Transaksi tidak valid.","Total Nilai disepakati melebihi Anggaran Justifikasi Kebutuhan.");
							return false;
						}
					}
				}																							
	...
	doChange:function(sender){
		...
		if (sender == this.cb_sph && this.cb_sph.getText()!="") {			
			var strSQL = "select a.kode_vendor,a.nama,a.alamat,d.no_spph, d.keterangan "+
						 "from vendor a "+
						 "inner join log_sph_m b on a.kode_vendor=b.kode_vendor and a.kode_lokasi=b.kode_lokasi "+
						 "inner join log_spph_vendor c on a.kode_vendor=c.kode_vendor and a.kode_lokasi=c.kode_lokasi and c.no_sph=b.no_sph "+
						 "inner join log_spph_m d on c.no_spph=d.no_spph and c.kode_lokasi=d.kode_lokasi "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){													
					...
					this.e_nospph.setText(line.no_spph);
					this.e_ketspph.setText(line.keterangan);
				}
			}

			if (this.stsSimpan ==  1) {							
				var strSQL = "select a.no_pesan,a.no_urut,a.item,a.merk,a.tipe,a.catatan,a.jumlah,a.nilai,a.jumlah * a.nilai as total "+
							 "from log_sph_d a "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						...
						this.sg4.appendData([line2.no_urut,line2.item,line2.merk,line2.tipe,line2.catatan,floatToNilai(line2.jumlah),floatToNilai(line2.nilai),floatToNilai(line2.nilai),floatToNilai(line2.total)]);
					}
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "") {
			...
			this.stsSimpan = 1;
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"log_nego_m","no_nego",this.app._lokasi+"-NG"+this.e_periode.getText().substr(2,4)+".","000"));						
			this.e_dok.setFocus();
	...
	doNilaiChange4: function(){
		try{
			...
			for (var i = 0; i < this.sg4.rows.getLength();i++){
				if (this.sg4.rowValid(i) && this.sg4.cells(5,i) != "" && this.sg4.cells(6,i) != "" && this.sg4.cells(7,i) != ""){					
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_nego,convert(varchar,a.tanggal,103) as tgl,a.keterangan,b.nama,a.nilai+a.ppn as nilai "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_tap='-' and d.modul in ('TL','PL') ";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.keterangan,a.tanggal,a.nik_nego,a.nama_pic,a.kode_vendor,a.no_sph,a.no_dokumen "+
							 "from log_nego_m a "+							 
				...
				var strSQL = "select a.no_pesan,a.no_urut,a.item,a.merk,a.tipe,a.catatan,a.jumlah,a.nilai,a.nilai_nego,a.jumlah * a.nilai_nego as total "+
							 "from log_sph_d a "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line2 = data.rs.rows[i];												
						this.sg4.appendData([line2.no_urut,line2.item,line2.merk,line2.tipe,line2.catatan,floatToNilai(line2.jumlah),floatToNilai(line2.nilai),floatToNilai(line2.nilai_nego),floatToNilai(line2.total)]);
					}
				} else this.sg4.clear(1);	
