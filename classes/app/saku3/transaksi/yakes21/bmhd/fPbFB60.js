window.app_saku3_transaksi_yakes21_bmhd_fPbFB60 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_bmhd_fPbFB60.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_bmhd_fPbFB60";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Permintaan Bayar FB60", 0);	
		...
		this.sg3 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:6,tag:9,
					...
					colWidth:[[5,4,3,2,1,0],[70,100,250,150,80,100]],
        ...
        this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,996,328], childPage:["Atensi","Item Jurnal","Item Pajak","Budget","Otorisasi","File Dok","Cattn Verf."]});    
        this.cb_akunhut = new saiCBBL(this.pc1.childPage[0],{bound:[20,19,220,20],caption:"Akun Hutang", multiSelection:false, maxLength:10, tag:2});					
		this.cb_vendor = new saiCBBL(this.pc1.childPage[0],{bound:[20,22,220,20],caption:"Mitra", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});				
		...
		this.e_banktrans = new saiCB(this.pc1.childPage[0],{bound:[20,21,200,20],caption:"Bank Transfer",items:["MANDIRI","X-MANDIRI","BNI","BRI"], readOnly:true,tag:2});	
		this.e_totalRek = new saiLabelEdit(this.pc1.childPage[0],{bound:[790,21,200,20],caption:"Total Rekening", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});					
		this.sgRek = new saiGrid(this.pc1.childPage[0],{bound:[1,5,990,193],colCount:5,tag:0,
		...
		this.sgnRek = new portalui_sgNavigator(this.pc1.childPage[0],{bound:[0,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:2,grid:this.sgRek});	

		// this.cb_rek = new saiCBBL(this.pc1.childPage[0],{bound:[20,14,220,20],caption:"ID Rekening", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});
		// this.e_nama = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,17,400,20],caption:"Nama", maxLength:150, tag:2,readOnly:true});	
        // this.e_bank = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,18,400,20],caption:"Bank", maxLength:150, tag:2,readOnly:true});	
        // this.e_norek = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,19,400,20],caption:"No Rekening", maxLength:150, tag:2,readOnly:true});
        // this.e_nmrek = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,20,400,20],caption:"Nama Rekening", maxLength:150, tag:2,readOnly:true});		
		// this.e_banktrans = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,21,400,20],caption:"Bank Transfer", maxLength:150, tag:2,readOnly:true});		
		...
		this.cb_buat = new saiCBBL(this.pc1.childPage[4],{bound:[20,16,220,20],caption:"Dibuat Oleh", multiSelection:false, maxLength:10, tag:2});						
		this.cb_app = new saiCBBL(this.pc1.childPage[4],{bound:[20,15,220,20],caption:"NIK Approval", multiSelection:false, maxLength:10, tag:2});								
		...
		try {
			...
			this.cb_vendor.setSQL("select kode_vendor, nama from vendor where kode_lokasi in ('"+this.app._lokasi+"','99')",["kode_vendor","nama"],false,["Kode","Nama"],"and","Data Atensi",true);
            this.cb_akunhut.setSQL("select a.kode_akun, a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
...
window.app_saku3_transaksi_yakes21_bmhd_fPbFB60.extend(window.childForm);
window.app_saku3_transaksi_yakes21_bmhd_fPbFB60.implement({		
	...
	doAfterPaste: function(sender,totalRow){
		try {			
			for (var i=0;i < this.sg1.rows.getLength();i++){						
				...
				this.doChangeCell1(this.sg1,7,i);						
			}			
		} catch(e) {alert(e);}
	},	
	doAfterPaste4: function(sender,totalRow){
		try {			
			for (var i=0;i < this.sg4.rows.getLength();i++){						
				...
				this.doChangeCell4(this.sg4,5,i);						
			}			
		} catch(e) {alert(e);}
	},	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into hutang_m (no_hutang, kode_lokasi, no_dokumen, tanggal, keterangan, kode_vendor, kode_curr, kurs, nik_app, kode_pp, nilai, periode, nik_user, tgl_input, akun_hutang, posted, nilai_ppn, modul, no_ref, dc) values "+
							"('"+this.e_nb.getText()+"', '"+this.app._lokasi+"', '"+this.e_dok.getText()+"', '"+this.dp_d1.getDateString()+"', '"+this.e_ket.getText()+"', '"+this.cb_vendor.getText()+"', 'IDR', 1, '"+this.cb_app.getText()+"', '"+this.app._kodePP+"', "+this.nilaiHutang+", '"+this.e_periode.getText()+"', '"+this.app._userLog+"', getdate(), '"+this.cb_akunhut.getText()+"', 'X', 0, 'PBAKRU', '-','D')")					
					sql.add("insert into hutang_j (no_hutang, no_dokumen, tanggal, no_urut, kode_akun, keterangan, dc, kode_curr, kurs, nilai_curr, nilai, kode_pp, kode_drk, kode_lokasi, modul, jenis, periode, nik_user, tgl_input) values "+
							"('"+this.e_nb.getText()+"', '"+this.e_dok.getText()+"', '"+this.dp_d1.getDateString()+"', 999, '"+this.cb_akunhut.getText()+"', '"+this.e_ket.getText()+"', 'C', 'IDR', 1, "+this.nilaiHutang+", "+this.nilaiHutang+", '"+this.app._kodePP+"', '-', '"+this.app._lokasi+"', 'PBAKRU', 'HUTANG', '"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");

					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){								
								sql.add("insert into hutang_j (no_hutang, no_dokumen, tanggal, no_urut, kode_akun, keterangan, dc, kode_curr, kurs, nilai_curr, nilai, kode_pp, kode_drk, kode_lokasi, modul, jenis, periode, nik_user, tgl_input) values "+
										"('"+this.e_nb.getText()+"', '"+this.e_dok.getText()+"', '"+this.dp_d1.getDateString()+"', "+i+", '"+this.sg1.cells(0,i)+"', '"+this.sg1.cells(3,i)+"', '"+this.sg1.cells(2,i).toUpperCase()+"', 'IDR', 1, "+nilaiToFloat(this.sg1.cells(4,i))+", "+nilaiToFloat(this.sg1.cells(4,i))+", '"+this.sg1.cells(5,i)+"', '"+this.sg1.cells(7,i)+"', '"+this.app._lokasi+"', 'PBAKRU', 'BEBAN', '"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");			
							}
						}
					}			
					...
					sql.add("insert into hutang_bayar (no_bayar,no_hutang,kode_lokasi,akun_hutang,keterangan,kode_pp,modul,periode,kode_curr,kurs,nilai,dc,kode_vendor,sts_pajak,pajak) values  "+
							"('"+this.e_nb.getText()+"','"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_akunhut.getText()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','PBAKRU','"+this.e_periode.getText()+"','IDR',1,"+nilaiToFloat(this.e_totalDC.getText())+",'D','-','-',0)");		

					sql.add("insert into pbh_pb_m (no_pb,no_dokumen,kode_lokasi,periode,nik_user,tgl_input,tanggal,due_date,keterangan,nilai,modul,progress,kode_pp,nik_app,nik_buat,no_hutang,no_spb,no_ver,akun_hutang,nik_ver,bank_trans,no_nota,no_kas,no_app) values  "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.dp_d1.getDateString()+"','"+this.dp_d2.getDateString()+"','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_totalDC.getText())+",'PBAKRU','X','"+this.app._kodePP+"','"+this.cb_app.getText()+"','"+this.cb_buat.getText()+"','"+this.e_nb.getText()+"','-','-','"+this.cb_akunhut.getText()+"','-','"+this.e_banktrans.getText()+"','-','-','-')");

					// sql.add("insert into pbh_rek(kode_vendor,nu,no_bukti,kode_lokasi,modul,nama_rek,no_rek,bank,nama,bruto,pajak,nilai) values "+
					// 		"('"+this.cb_vendor.getText()+"','"+this.cb_rek.getText()+"','"+this.e_nb.getText()+"','"+this.app._lokasi+"','PBAKRU','"+this.e_nmrek.getText()+"','"+this.e_norek.getText()+"','"+this.e_bank.getText()+"','"+this.e_nama.getText()+"',"+this.nilaiHutang+","+this.nilaiPajak+","+nilaiToFloat(this.e_totalDC.getText())+")");
							...
							if (this.sgRek.rowValid(i)){		
								var k = 1000+i;								
								 sql.add("insert into pbh_rek(kode_vendor,nu,no_bukti,kode_lokasi,modul,nama_rek,no_rek,bank,nama,bruto,pajak,nilai,bank_trans) values "+
										"('"+this.cb_vendor.getText()+"','"+this.cb_rek.getText()+"','"+this.e_nb.getText()+"','"+this.app._lokasi+"','PBAKRU','"+this.sgRek.cells(3,i)+"','"+this.sgRek.cells(2,i)+"','"+this.sgRek.cells(1,i)+"','"+this.sgRek.cells(0,i)+"',"+nilaiToFloat(this.sgRek.cells(4,i))+",0,"+nilaiToFloat(this.sgRek.cells(4,i))+",'"+this.e_banktrans.getText()+"')");										
							}
						}
					}
					...
					sql.add("insert into pbh_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
					 		"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.cb_akunhut.getText()+"','"+this.e_ket.getText()+"','D',"+this.nilaiHutang+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBAKRU','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");										
					...
					if (this.sg4.getRowValidCount() > 0){
						for (var i=0;i < this.sg4.getRowCount();i++){
							if (this.sg4.rowValid(i)){		
								var k = 1000+i;								
								sql.add("insert into pbh_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+k+",'"+this.sg4.cells(0,i)+"','"+this.sg4.cells(3,i)+"','"+this.sg4.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg4.cells(4,i))+",'"+this.sg4.cells(5,i)+"','-','"+this.app._lokasi+"','PBAKRU','PAJAK','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");										
							}
						}
					}
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"('"+this.e_nb.getText()+"','PBAKRU','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(6,i))+","+nilai+")");
							}
						}
					}
				...
				for (var i=0;i < this.sg2.getRowCount();i++){
					for (var j=0;j<this.dataAkunGar.rs.rows.length;j++) {
						...
						if (line.kode_akun == this.sg2.cells(0,i)) {		
							if (nilaiToFloat(this.sg2.cells(7,i))>0 && nilaiToFloat(this.sg2.cells(6,i)) < nilaiToFloat(this.sg2.cells(7,i))) { //(nilaiToFloat(this.sg2.cells(3,i))>0 && nilaiToFloat(this.sg2.cells(2,i)) < nilaiToFloat(this.sg2.cells(3,i))) {
	...
	doChange:function(sender){
		try {
			...
			if (sender == this.cb_vendor && this.cb_vendor.getText()!="") {
				var strSQL = "select a.nama,b.nu "+
							"from vendor a inner join vendor_rek b on a.kode_vendor=b.kode_vendor and a.kode_lokasi=b.kode_lokasi and b.status='1' "+								 
							"where a.kode_vendor = '"+this.cb_vendor.getText()+"'";										
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){				
						this.nama_atensi = line.nama;																
						
						if (this.stsSimpan==1) {
							this.cb_rek.setSQL("select nu, bank from vendor_rek where kode_vendor='"+this.cb_vendor.getText()+"' ",["nu","bank"],false,["Kode","Bank"],"and","Data Rekening",true);
			...
			if (sender == this.cb_rek && this.cb_rek.getText()!="" && this.cb_vendor.getText()!="" && this.stsSimpan==1) {
				var data = this.dbLib.getDataProvider("select * from vendor_rek where nu="+this.cb_rek.getText()+" and kode_vendor='"+this.cb_vendor.getText()+"' ",true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){								
						...
						this.sgRek.clear();
						this.sgRek.appendData([this.nama_atensi,line.bank,line.no_rek,line.nama_rek,"0"]);
					}
				}
			}			
		}
	...
	doNilaiChange: function(){		
		try{
			...
			this.nilaiPajak = this.nilaiHutang - totDC;
	...
	doLoad3:function(sender){																				
		...
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai "+
					 ...
					 "and a.modul = 'PBAKRU' and a.progress in ('X','R','V') "+
					 "order by a.no_pb";						 	 
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select isnull(a.bank_trans,'X-MANDIRI') as bank_trans,a.keterangan,a.no_dokumen,a.modul,a.due_date,a.tanggal,a.nik_buat,a.nik_app,a.akun_hutang,b.kode_vendor,b.nu as kode_rek "+
							 "from pbh_pb_m a inner join pbh_rek b on a.no_pb=b.no_bukti "+								 
