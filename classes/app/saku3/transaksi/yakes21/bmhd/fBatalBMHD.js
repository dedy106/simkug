window.app_saku3_transaksi_yakes21_bmhd_fBatalBMHD = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_bmhd_fBatalBMHD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_bmhd_fBatalBMHD";
		...
		this.cb_bmhd = new saiCBBL(this.pc1.childPage[0],{bound:[20,11,220,20],caption:"Data BMHD", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});
		this.e_vendor = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,12,450,20],caption:"Data Mitra", readOnly:true});			
		...
		this.cb_drk = new saiCBBL(this.pc1.childPage[0],{bound:[20,17,220,20],caption:"Kode DRK", multiSelection:false, maxLength:10, tag:1});			
		...
		this.e_nilai = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,16,200,20],caption:"Ni. Penyelesaian", tag:1, tipeText:ttNilai, text:"0",change:[this,"doChange"],readOnly:true});
...
window.app_saku3_transaksi_yakes21_bmhd_fBatalBMHD.extend(window.childForm);
window.app_saku3_transaksi_yakes21_bmhd_fBatalBMHD.implement({	
	isiBMHD: function() {
		//hanya modul bmhd dari akru (kasus akhir tahun)
		this.cb_bmhd.setSQL(
				"select a.no_hutang,a.keterangan "+
				"from hutang_m a "+
				"		left join ("+
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,ref1,tgl_input,nik_user) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.cb_pp.getText()+"','REVBMHD','BTLBMHD','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"','F','"+this.cb_bmhd.getText()+"',getdate(),'"+this.app._userLog+"')");
					...
					//DC=D -->anggaran balik lagi		
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
							"	('"+this.e_nb.getText()+"','REVBMHD','"+this.app._lokasi+"','"+this.cb_akun.getText()+"','"+this.cb_pp.getText()+"','"+this.cb_drk.getText()+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','D',0,"+nilaiToFloat(this.e_nilai.getText())+")");
	...
	doChange: function(sender){
		...
		if (sender == this.cb_bmhd && this.cb_bmhd.getText()!="") {
			var strSQL =  "select c.nama,a.akun_hutang,a.nilai-isnull(b.bayar,0) as saldo "+
						  "from hutang_m a "+
						  "inner join vendor c on a.kode_vendor=c.kode_vendor and a.kode_lokasi=c.kode_lokasi "+
						  "left join ("+
						  "         	select no_hutang,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar  "+
						  "		    	from hutang_bayar "+
						  "				where kode_lokasi='"+this.app._lokasi+"' and no_bayar<>'"+this.e_nb.getText()+"' "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){									
					...
					//load ref jurnal sebelumnya
					var data = this.dbLib.getDataProvider(
							"select top 1 kode_akun,kode_pp,kode_drk,substring(periode,1,4) as tahun "+
							"from hutang_j "+
							"where no_hutang='"+this.cb_bmhd.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and dc='D' ",true);
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
						if (line != undefined){	
							this.cb_akun.setText(line.kode_akun);
							this.cb_pp.setText(line.kode_pp);
							if (line.tahun == this.e_periode.getText().substr(0,4)) this.cb_drk.setText(line.kode_drk);
							else this.cb_drk.setText("","");
						}
					}
		...
		if ((sender == this.cb_akun || sender == this.cb_pp) && this.cb_akun.getText()!="" && this.cb_pp.getText()!="")	{
			var strSQL = "select distinct a.kode_drk, a.nama from drk a inner join angg_r b on a.kode_drk=b.kode_drk where a.tahun=substring(b.periode1,1,4) and b.periode1 like '"+this.e_periode.getText().substr(0,4)+"%' and b.kode_akun='"+this.cb_akun.getText()+"' and b.kode_pp = '"+this.cb_pp.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' union select '-','-' ";
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.*,b.kode_akun,b.kode_drk from ju_m a inner join ju_j b on a.no_ju=b.no_ju and a.kode_lokasi=b.kode_lokasi and b.jenis='PDPT' where a.no_ju = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";										
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){													
