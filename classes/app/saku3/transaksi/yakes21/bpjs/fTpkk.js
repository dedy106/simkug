window.app_saku3_transaksi_yakes21_bpjs_fTpkk = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_bpjs_fTpkk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_bpjs_fTpkk";
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,430], childPage:["Daftar TPKK","Data TPKK"]});				
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:4,tag:9,
					...
					colWidth:[[3,2,1,0],[70,100,350,100]],
					readOnly:true,
		...
		this.c_jenis = new saiCB(this.pc1.childPage[1],{bound:[20,13,200,20],caption:"Jenis",items:["TPKK","TPKU"], readOnly:true,tag:2});
...
window.app_saku3_transaksi_yakes21_bpjs_fTpkk.extend(window.childForm);
window.app_saku3_transaksi_yakes21_bpjs_fTpkk.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into yk_tpkk(kode_tpkk,kode_lokasi,nama,jenis) values "+
						    "('"+this.cb_kode.getText()+"','"+this.cb_lokasi.getText()+"','"+this.e_nama.getText()+"','"+this.c_jenis.getText()+"')");
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select * from yk_tpkk where kode_tpkk ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.cb_lokasi.getText()+"' ";						   
	...
	doLoad:function(sender){								
		var strSQL = "select kode_tpkk, nama, jenis from yk_tpkk where kode_lokasi='"+this.cb_lokasi.getText()+"' ";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_tpkk,line.nama,line.jenis,"Pilih"]); 
		}
