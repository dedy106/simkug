window.app_saku3_transaksi_yakes21_aktap_fFaWO = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_aktap_fFaWO.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_aktap_fFaWO";		
		...
		this.cb_akun = new saiCBBL(this.pc2.childPage[1],{bound:[20,16,220,20],caption:"Akun Beban", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});						
		this.cb_drk = new saiCBBL(this.pc2.childPage[1],{bound:[20,18,220,20],caption:"DRK Penghapusan", multiSelection:false, maxLength:10, tag:2});						
		this.e_noaju = new saiLabelEdit(this.pc2.childPage[1],{bound:[20,19,200,20],caption:"No Pengajuan", readOnly:true,});						
		this.e_status = new saiLabelEdit(this.pc2.childPage[1],{bound:[790,19,200,20],caption:"Status", tag:1, readOnly:true });								
		this.cb_lokasi = new saiCBBL(this.pc2.childPage[1],{bound:[20,16,220,20],caption:"Lok. Aktap", readOnly:true});								
		this.e_total = new saiLabelEdit(this.pc2.childPage[1],{bound:[790,16,200,20],caption:"Nilai Penghapusan", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});								
		
		this.pc1 = new pageControl(this.pc2.childPage[1],{bound:[1,12,995,236], childPage:["Item Aktap","Otorisasi","File Dok"]});
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:7,tag:9,
		            colTitle:["ID Aktap","Nama","Tgl Perolehan","Nilai Aktap","Nilai WO","Catatan","Kode PP"],
					colWidth:[[6,5,4,3,2,1,0],[80,150,100,100,80,280,150]],					
					readOnly:true,					
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag,keterangan from spro where kode_spro in ('BRUSAK','BHILANG') ",true);			
			if (typeof data == "object"){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];									
					if (line.kode_spro == "BRUSAK") this.akunRusak = line.flag;
					if (line.kode_spro == "BHILANG") this.akunHilang = line.flag;
				}
			}
...
window.app_saku3_transaksi_yakes21_aktap_fFaWO.extend(window.childForm);
window.app_saku3_transaksi_yakes21_aktap_fFaWO.implement({			
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						sql.add("update fa_woaju_m set progress='1',no_ba='-' where no_ba='"+this.e_nb.getText()+"' and kode_lokasi='"+this.cb_lokasi.getText()+"'");						
						...
						sql.add("delete from angg_r where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.cb_lokasi.getText()+"'");																		
					}					
					
					sql.add("update fa_woaju_m set progress='2',no_ba='"+this.e_nb.getText()+"' where no_aju='"+this.e_noaju.getText()+"' and kode_lokasi='"+this.cb_lokasi.getText()+"'");
					sql.add("insert into ju_m (no_ju, kode_lokasi, no_dokumen, tanggal, keterangan, kode_pp, modul, jenis, periode, kode_curr, kurs, nilai, nik_buat, nik_setuju, tgl_input, nik_user, posted, ref1, atensi) values "+
							"('"+this.e_nb.getText()+"', '"+this.cb_lokasi.getText()+"', '"+this.e_dok.getText()+"', '"+this.dp_d1.getDateString()+"', '"+this.e_ket.getText()+"', '"+this.app._kodePP+"', 'WOBAST', 'BAST', '"+this.e_periode.getText()+"', 'IDR', 1, "+nilaiToFloat(this.e_total.getText())+", '"+this.cb_buat.getText()+"', '"+this.cb_app.getText()+"', getdate(), '"+this.app._userLog+"', 'F', '"+this.cb_akun.getText()+"','"+this.cb_drk.getText()+"')")
					...
					sql.add("insert into ju_j (no_ju, no_dokumen, tanggal, no_urut, kode_akun, keterangan, dc, nilai, kode_pp, kode_drk, kode_lokasi, modul, jenis, periode, kode_curr, kurs, nik_user, tgl_input) "+
							"select '"+this.e_nb.getText()+"', '"+this.e_dok.getText()+"', '"+this.dp_d1.getDateString()+"', 1, '"+this.cb_akun.getText()+"', '"+this.e_ket.getText()+"', 'D', sum(zz.nilai-isnull(d.akum_susut,0)), b.kode_pp, '"+this.cb_drk.getText()+"', b.kode_lokasi, 'WOBAST', 'BEBAN', '"+this.e_periode.getText()+"', 'IDR', 1, '"+this.app._userLog+"', getdate() "+
							...
							"where a.no_aju='"+this.e_noaju.getText()+"' and a.kode_lokasi='"+this.cb_lokasi.getText()+"' "+
					...
					sql.add("insert into ju_j (no_ju, no_dokumen, tanggal, no_urut, kode_akun, keterangan, dc, nilai, kode_pp, kode_drk, kode_lokasi, modul, jenis, periode, kode_curr, kurs, nik_user, tgl_input) values "+
							"('"+this.e_nb.getText()+"', '"+this.e_dok.getText()+"', '"+this.dp_d1.getDateString()+"', 999, '"+this.akunAPP+"', '"+this.e_ket.getText()+"', 'C', "+nilaiToFloat(this.e_total.getText())+", '"+this.app._kodePP+"', '-', '"+this.cb_lokasi.getText()+"', 'WOBAST', 'AKTAPLAIN', '"+this.e_periode.getText()+"', 'IDR', 1, '"+this.app._userLog+"', getdate())");

					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) "+
							"select no_ju,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode,periode,'C',0,sum(nilai) "+
							"from ju_j "+
							"where dc ='D' and no_ju='"+this.e_nb.getText()+"' and kode_lokasi='"+this.cb_lokasi.getText()+"' "+
							"group by no_ju,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode,periode");
	...
	doSelectDate: function(sender, y,m,d){
		...
		if (this.stsSimpan == 1) {
			this.doLoad3();		
			this.cb_nb.setSQL("select no_ju, keterangan from ju_m where modul ='WOBAST' and periode='"+this.e_periode.getText()+"' and posted ='F' ",["no_ju","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Approve",true);											
		}
	},
	doChange:function(sender){
		try {
			...
			if (sender == this.cb_akun && this.cb_akun.getText()!="") {
				this.cb_drk.setSQL("select distinct a.kode_drk, a.nama from drk a inner join angg_r b on a.kode_drk=b.kode_drk where a.tahun=substring(b.periode1,1,4) and b.periode1 like '"+this.e_periode.getText().substr(0,4)+"%' and b.kode_akun='"+this.cb_akun.getText()+"' and a.kode_lokasi='"+this.cb_lokasi.getText()+"' and b.modul='RELEASE'",["a.kode_drk","a.nama"],false,["Kode","Nama"],"and","Daftar DRK",true);											
			}
			...
			if (sender == this.cb_nb && this.cb_nb.getText() != "") {
				...
				var strSQL = "select a.no_aju,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai,b.nama as lokasi,a.status "+
							...
							"where a.no_ba='"+this.cb_nb.getText()+"' ";		
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "" && this.cb_lokasi.getText()!="") {
			...
			this.stsSimpan = 1;
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"ju_m","no_ju",this.cb_lokasi.getText()+"-BW"+this.e_periode.getText().substr(2,4)+".","0000"));						
			this.e_dok.setFocus();
	...
	doLoad3:function(sender){	
		try {																	
			var strSQL = "select a.no_aju,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai,b.nama as lokasi,a.status "+
						 "from fa_woaju_m a inner join lokasi b on a.kode_lokasi=b.kode_lokasi "+				
						 "where a.periode<='"+this.e_periode.getText()+"' and a.progress ='1'";		
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU3 = data;
				this.sgn3.setTotalPage(Math.ceil(data.rs.rows.length/20));
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(1,row) != "") {
				...
				var strSQL = "select a.*,b.ref1 as akun_approve from fa_woaju_m a inner join ju_m b on a.no_app=b.no_ju and b.modul='WOAPP' where a.no_aju = '"+this.e_noaju.getText()+"'";
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					...
					if (line != undefined){								
						...
						this.akunAPP = line.akun_approve;	
						if (this.e_status.getText() == "RUSAK") this.cb_akun.setText(this.akunRusak);															
						else this.cb_akun.setText(this.akunHilang);															
					}
				}			
				...
				if (this.stsSimpan == 1) this.doClick();	
					...
					if (typeof data == "object"){
						...
						if (line != undefined){				
							...
							this.cb_drk.setText(line.atensi);
						}
					}	
				}
				
				var data = this.dbLib.getDataProvider(
							"select a.no_fa,b.kode_pp, b.nama, convert(varchar,tgl_perolehan,103)  as tgl_oleh, zz.nilai,zz.nilai - isnull(d.akum_susut,0) as nilai_wo, a.keterangan "+
							"from fa_woaju_d a "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];												
						this.sg1.appendData([line.no_fa,line.nama,line.tgl_oleh,floatToNilai(line.nilai),floatToNilai(line.nilai_wo),line.keterangan,line.kode_pp]);
					}
