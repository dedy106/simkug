window.app_saku3_transaksi_yakes21_aktap_fFasusutCepat = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_aktap_fFasusutCepat.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_aktap_fFasusutCepat";
		...
		this.cb_drk = new saiCBBL(this,{bound:[20,18,220,20],caption:"DRK Penyusutan", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"] });
		this.cb_app = new saiCBBL(this,{bound:[20,17,220,20],caption:"NIK Approve", multiSelection:false, maxLength:10, tag:2});		
		this.cb_fa = new saiCBBL(this,{bound:[20,18,250,20],caption:"No Aktiva Tetap", multiSelection:false, maxLength:20, tag:2, change:[this,"doChange"]});		
		...
		this.e_gar = new saiLabelEdit(this,{bound:[250,24,200,20],caption:"Saldo Budget", tag:3, readOnly: true, tipeText:ttNilai, text:"0"});				
		...
		try {
			...
			this.cb_fa.setSQL("select a.no_fa, a.nama "+
							  "from fa_asset a "+
							  ...
							  "  where a.jenis='A' and a.progress in ('2','L') and zz.nilai-a.nilai_residu > isnull(tot_susut,0) and a.kode_lokasi='"+this.app._lokasi+"'",["a.no_fa","a.nama"],false,["No Aktap","Nama"],"and","Daftar Aktiva Tetap",true);
...
window.app_saku3_transaksi_yakes21_aktap_fFasusutCepat.extend(window.childForm);
window.app_saku3_transaksi_yakes21_aktap_fFasusutCepat.implement({
	...
	simpan: function(){			
		try{						
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"fasusut_m","no_fasusut",this.app._lokasi+"-RSC"+this.e_periode.getText().substr(2,4)+".","0000"));
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,3])){
				try{													
					...
					sql.add("insert into fasusut_m(no_fasusut,no_dokumen,tanggal,keterangan,kode_curr,kurs,nilai,kode_pp,kode_drk,posted,modul,nik_buat,nik_setuju,kode_lokasi,periode,no_del,no_link,nik_user,tgl_input) values "+
						    "('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'-','"+this.cb_drk.getText()+"','F','FACPT','"+this.app._userLog+"','"+this.cb_app.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','-','-','"+this.app._userLog+"',getdate())");					
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "simpan" :	
				...
				// 	system.alert(this,"Transaksi tidak valid.","Nilai penyusutan tidak boleh melebihi saldo anggaran.");
				// 	return false;						
				// }				
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!= "") this.cb_drk.setSQL("select kode_drk, nama from drk where tahun = '"+this.e_periode.getText().substr(0,4)+"' and tipe = 'posting' and kode_lokasi='"+this.app._lokasi+"'",["kode_drk","nama"],false,["Kode","Nama"],"and","Daftar DRK",true);
		...
		if (sender == this.cb_fa && this.cb_fa.getText()!="") {
			if (this.cb_drk.getText()!="") {
				var strSQL = "select a.no_seri,a.merk,a.tipe,zz.nilai,a.nilai_residu,isnull(d.tot_susut,0) as tot_susut,(zz.nilai-a.nilai_residu-isnull(d.tot_susut,0)) as nilai_buku, "+
							 ...
							 "   			select no_fa,sum(case dc when 'D' then nilai else -nilai end) as tot_susut "+
							 "	  			from fasusut_d "+
							 "				group by no_fa) d on a.no_fa=d.no_fa "+						 

							 "where a.umur <> 0 and a.no_fa='"+this.cb_fa.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
				...
				if (typeof data == "object"){
					...
					if (line != undefined){	
						...
						var data = this.dbLib.getDataProvider("select fn_saldoRilis('"+this.app._lokasi+"','"+line.akun_bp+"','"+line.kode_pp_susut+"','"+this.cb_drk.getText()+"','"+this.e_periode.getText()+"','"+this.e_nb.getText()+"') as saldo ",true);
				...
				system.alert(this,"DRK tidak valid.","DRK harap diisi.");
			}
		}
		if (sender == this.cb_drk && this.cb_drk.getText()!="" && this.pp!="" && this.akunbp!="") {
			var data = this.dbLib.getDataProvider("select fn_saldoRilis('"+this.app._lokasi+"','"+this.akunbp+"','"+this.pp+"','"+this.cb_drk.getText()+"','"+this.e_periode.getText()+"','"+this.e_nb.getText()+"') as saldo ",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line = data.rs.rows[0];					
				this.e_gar.setText(floatToNilai(line.saldo));							
			}
