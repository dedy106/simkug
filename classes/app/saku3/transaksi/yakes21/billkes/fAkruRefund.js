window.app_saku3_transaksi_yakes21_billkes_fAkruRefund = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_billkes_fAkruRefund.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_billkes_fAkruRefund";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Jurnal Billing REFUND", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,20,995,313], childPage:["Data Billing","Jurnal Akru","Daftar Rekening"]});		
		...
		try {
			...
			this.kodepp = "";
			var data = this.dbLib.getDataProvider("select kode_spro,flag,keterangan from spro where kode_spro in ('PPBPCC','DRKBPCC','PIURFD') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku3_transaksi_yakes21_billkes_fAkruRefund.extend(window.childForm);
window.app_saku3_transaksi_yakes21_billkes_fAkruRefund.implement({				
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i) && this.sg2.cells(4,i) != "0"){
								sql.add("insert into yk_hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg2.cells(0,i)+"','"+this.sg2.cells(3,i)+"','"+this.sg2.cells(2,i)+"',"+parseNilai(this.sg2.cells(4,i))+",'"+this.kodepp+"','"+this.sg2.cells(6,i)+"','"+this.app._lokasi+"','REFUND','"+this.sg2.cells(5,i)+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");								
							}
						}
					}										
					var nobukti = ""; 
					...
					//tidak perlu approve ..langsung posting, no dokumen untuk nokas refund nya
					//progress=1 
					sql.add("insert into yk_hutang_m(no_hutang,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_app,posted,tgl_input,nik_user,kode_loktuj,no_app,progress) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','-','"+this.e_ket.getText()+"','"+this.kodepp+"','REFUND','BILL','IDR',1,"+totHut+",'"+this.app._userLog+"','"+this.cb_app.getText()+"','F',getdate(),'"+this.app._userLog+"','"+this.app._lokasi+"','-','1')");
					...
					sql.add("insert into yk_hutang_d(no_hutang,kode_lokasi,no_inv,periode,kode_vendor,bank,cabang,no_rek,nama_rek,nilai_bp,nilai_cc,no_rekon,bank_trans,no_app,no_pb,no_kas,progress,kode_loktuj,pajak) "+
							...
							"       b.kode_vendor,e.bank,e.cabang,e.no_rek,e.nama_rek,isnull(sum(case when f.jenis <> 'PENSIUN' then a.nilai - a.pph else 0 end),0),isnull(sum(case when f.jenis = 'PENSIUN' then a.nilai - a.pph else 0 end),0),'-',e.bank_trans,'-','REFUND','REFUND','0','"+this.app._lokasi+"',0 "+
							" from yk_bill_d a "+
			...
			case "ubah" :	
				if (this.stsSimpan == 0) {
					...
					if (typeof data == "object"){
						...
						if (line != undefined){																				
							system.alert(this,"Transaksi tidak valid.","No Pengajuan AKRU sudah diprogress lain [HB06].");
							return false;							
						}
					}
				}								
	...
	doTampil:function(sender){		
		if (this.e_periode.getText() != "") {
			var strSQL = "select a.no_bill,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,a.nilai,a.kunj,a.cs,a.jenis "+
			             "from yk_bill_m a "+
						 "where a.jenis = 'REFUND' and a.progress = '0' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"'";
						 //"where a.jenis = 'REFUND' and a.progress = '0' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"'";
	...
	doJurnal:function(sender){		
		try {
			if (this.cb_drk.getText()!="") {
				...
				nobukti = nobukti.substr(1);
				if (nobukti == "") nobukti = "''";			
				//jurnal di balik.. piutanglain2 pd BP/CC
				var strSQL = 
							...
							"select "+
							" 'REFUND BIAYA PENGOBATAN'  as ket, "+
							" '"+this.akunPiuLain+"' as kode_akun,c.nama as nama_akun,'D' as dc, abs(sum(a.nilai)) as nilai,  "+ 
							" 'RFUND' as jenis,'-' as kode_drk,'-' as nama_drk  "+
							"from yk_bill_d a "+
							"			inner join masakun c on '"+this.akunPiuLain+"'=c.kode_akun and '"+this.app._lokasi+"'=c.kode_lokasi  "+
							"			inner join yk_bill_m d on a.no_bill=d.no_bill and a.kode_lokasi=d.kode_lokasi  "+							
	...
	doLoad4:function(sender){																				
		var strSQL = "select a.no_hutang,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai,a.kode_loktuj+' - '+c.nama as lokbayar,a.nik_app+' - '+b.nama as nama "+
					 ...
					 "where a.modul = 'REFUND' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress = '1' and a.posted='F' ";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU4 = data;
