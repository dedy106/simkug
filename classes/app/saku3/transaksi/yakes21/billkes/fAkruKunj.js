window.app_saku3_transaksi_yakes21_billkes_fAkruKunj = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_billkes_fAkruKunj.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_billkes_fAkruKunj";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Jurnal Kunjungan", 0);	
		...
		this.c_jenis = new saiCB(this.pc2.childPage[0],{bound:[790,12,200,20],caption:"Jenis Data",items:["PDPT","PDPTREV"], readOnly:true, tag:2,change:[this,"doChange"]});								
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,20,995,327], childPage:["Data Billing","Item Jurnal"]});		
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:7,tag:0,
				...
				colWidth:[[6,5,4,3,2,1,0],[100,100,100,300,80,150,80]],
		...
		this.sg2 = new saiGrid(this.pc1.childPage[1],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:8,tag:0,
					...
					colWidth:[[7,6,5,4,3,2,1,0],[150,80,80,100,250,50,200,100]],
...
window.app_saku3_transaksi_yakes21_billkes_fAkruKunj.extend(window.childForm);
window.app_saku3_transaksi_yakes21_billkes_fAkruKunj.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					//langsung bisa posting krn tidak ada budget (terima uang)
					sql.add("insert into yk_hutang_m(no_hutang,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_app,posted,tgl_input,nik_user,kode_loktuj,no_app,progress) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.kodepp+"','"+this.c_jenis.getText()+"','BILL','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.app._userLog+"','-','F',getdate(),'"+this.app._userLog+"','"+this.app._lokasi+"','"+this.e_nb.getText()+"','1')");
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i) && this.sg2.cells(4,i) != "0"){								                     
								sql.add("insert into yk_hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg2.cells(0,i)+"','"+this.sg2.cells(3,i)+"','"+this.sg2.cells(2,i)+"',"+parseNilai(this.sg2.cells(4,i))+",'"+this.kodepp+"','"+this.kodeDRK+"','"+this.app._lokasi+"','"+this.c_jenis.getText()+"','"+this.sg2.cells(5,i)+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");								
							}
						}
					}	
	...
	doTampil:function(sender){		
		if (this.c_jenis.getText()!="") {
			if (this.e_periode.getText() != "") {
				var strSQL = "select a.no_bill,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,b.total_kunj,b.total_cs,a.jenis "+
							"from yk_bill_m a "+
							"inner join yk_billkunj_m b on a.no_bill=b.no_bill and a.kode_lokasi=b.kode_lokasi "+
							"where a.jenis= '"+this.c_jenis.getText()+"' and a.progress = '0' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"'";			
							//"where a.jenis= '"+this.c_jenis.getText()+"' and a.progress = '0' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"'";			
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg.appendData(["INPROG",line.no_bill,line.tanggal,line.keterangan,floatToNilai(line.total_kunj),floatToNilai(line.total_cs),line.jenis]);
					}
		...
		else system.alert(this,"Jenis Data tidak valid.","Jenis Data harus dipilih.");
	},
	doJurnal:function(sender){		
		try {
			...
			var strSQL ="select 'PIU KUNJ '+c.nama as ket,(case b.jenis when 'PEGAWAI' then c.akun_pku else c.akun_pap end) as kode_akun,d.nama as nama_akun,case when sum(a.umum+a.gigi+a.kbia+a.matkes)>0 then 'D' else 'C' end as dc, "+
						"abs(sum(a.umum+a.gigi+a.kbia+a.matkes)) as nilai,'PIUKUNJUM' as jenis, e.kode_drk,e.nama as nama_drk "+
						...
						"		   inner join drk e on '"+this.kodeDRK+"'=e.kode_drk and e.kode_lokasi='"+this.app._lokasi+"' and e.tahun=substring(a.periode,1,4) "+
						"where abs(a.umum+a.gigi+a.kbia+a.matkes)<> 0 and b.jenis <> 'PENSIUN' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_bill in ("+nobukti+") "+
						"group by  c.nama,case b.jenis when 'PEGAWAI' then c.akun_pku else c.akun_pap end,d.nama,e.kode_drk,e.nama "+												
						
						"union all "+												
						"select 'KUNJ '+c.nama as ket,c.akun_ku as kode_akun,d.nama as nama_akun,case when sum(a.umum+a.gigi+a.kbia)>0 then 'C' else 'D' end as dc, "+
						"abs(sum(a.umum+a.gigi+a.kbia)) as nilai,'PDPTKUNJUM' as jenis, e.kode_drk,e.nama as nama_drk "+
						"from yk_billkunj_d a "+
						"          inner join yk_loker bb on a.loker=bb.loker "+
						"          inner join cust b on bb.kode_cust=b.kode_cust "+
						"		   inner join yk_kunj c on a.kode_produk=c.kode_produk "+
						...
						"		   inner join drk e on '"+this.kodeDRK+"'=e.kode_drk and e.kode_lokasi='"+this.app._lokasi+"' and e.tahun=substring(a.periode,1,4) "+
						"where abs(a.umum+a.gigi+a.kbia)<> 0 and b.jenis <> 'PENSIUN' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_bill in ("+nobukti+") "+
						"group by  c.nama,c.akun_ku,d.nama,e.kode_drk,e.nama "+												

						"union all "+						
						"select 'KUNJ '+c.nama as ket,c.akun_ob as kode_akun,d.nama as nama_akun,case when sum(a.matkes)>0 then 'C' else 'D' end as dc, "+
						"abs(sum(a.matkes)) as nilai,'PDPTKUNJUM' as jenis, e.kode_drk,e.nama as nama_drk "+
						"from yk_billkunj_d a "+
						"          inner join yk_loker bb on a.loker=bb.loker "+
						"          inner join cust b on bb.kode_cust=b.kode_cust "+
						"		   inner join yk_kunj c on a.kode_produk=c.kode_produk "+
						"		   inner join masakun d on c.akun_ob=d.kode_akun and d.kode_lokasi='"+this.app._lokasi+"' "+
						"		   inner join drk e on '"+this.kodeDRK+"'=e.kode_drk and e.kode_lokasi='"+this.app._lokasi+"' and e.tahun=substring(a.periode,1,4) "+
						"where abs(a.matkes)<> 0 and b.jenis <> 'PENSIUN' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_bill in ("+nobukti+") "+
						"group by  c.nama,c.akun_ob,d.nama,e.kode_drk,e.nama "+												
						
						//CS						
						"union all "+
						"select 'TTP CS '+d.nama as ket,c.akun_hutcs as kode_akun,d.nama as nama_akun,case when sum(a.cs)>0 then 'D' else 'C' end as dc, "+
						"abs(sum(a.cs)) as nilai,'HUTCS' as jenis, e.kode_drk,e.nama as nama_drk "+
						"from yk_billkunj_d a "+
						"          inner join yk_loker bb on a.loker=bb.loker "+
						"          inner join cust b on bb.kode_cust=b.kode_cust "+
						"		   inner join yk_kunj c on a.kode_produk=c.kode_produk "+
						...
						"group by  d.nama,c.akun_hutcs,d.nama,e.kode_drk,e.nama "+

						"union all "+						
						"select 'PIU CS '+d.nama as ket,c.akun_piucs as kode_akun,d.nama as nama_akun,case when sum(a.cs)>0 then 'C' else 'D' end as dc, "+
						"abs(sum(a.cs)) as nilai,'PIUCS' as jenis,e.kode_drk,e.nama as nama_drk "+
						"from yk_billkunj_d a "+
						"          inner join yk_loker bb on a.loker=bb.loker "+
						"          inner join cust b on bb.kode_cust=b.kode_cust "+
						"		   inner join yk_kunj c on a.kode_produk=c.kode_produk "+
						...
						"		   inner join drk e on '"+this.kodeDRK+"'=e.kode_drk and e.kode_lokasi='"+this.app._lokasi+"' and e.tahun=substring(a.periode,1,4) "+
						"where abs(a.cs)<> 0 and b.jenis <> 'PENSIUN' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_bill in ("+nobukti+") "+
						"group by  d.nama,c.akun_piucs,d.nama,e.kode_drk,e.nama "+
	...
	doLoad4:function(sender){																				
		var strSQL = "select a.no_hutang,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
		             "from yk_hutang_m a "+					 
					 "where a.modul in ('PDPT','PDPTREV') and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress='1' and a.posted='F' ";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU4 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg4.cells(0,row) != "") {
				...
				var strSQL = "select no_dokumen,keterangan,tanggal,modul "+
							 "from yk_hutang_m "+							 
				...
				var strSQL = "select a.no_bill,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,b.total_kunj,b.total_cs,a.jenis "+
							 "from yk_bill_m a inner join yk_billkunj_m b on a.no_bill=b.no_bill and a.kode_lokasi=b.kode_lokasi "+
