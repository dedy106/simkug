window.app_saku3_transaksi_yakes21_billkes_fAkruBill = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_billkes_fAkruBill.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_billkes_fAkruBill";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Jurnal Billing MITRA-RESTITUSI", 0);	
		...
		this.cb_lokasi = new saiCBBL(this.pc2.childPage[0],{bound:[20,11,220,20],caption:"Lokasi Bayar", multiSelection:false, maxLength:10, tag:2});				
		this.cb_drk = new saiCBBL(this.pc2.childPage[0],{bound:[20,13,220,20],caption:"DRK", multiSelection:false, maxLength:10, tag:2});						
		this.e_debet = new saiLabelEdit(this.pc2.childPage[0],{bound:[780,13,200,20],caption:"Total Debet", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});					
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,20,995,305], childPage:["Data Billing","Jurnal Akru","Daftar Rekening","Otorisasi","Cattn. Ver"]});		
		...
		this.cb_tahu = new saiCBBL(this.pc1.childPage[3],{bound:[20,10,220,20],caption:"Diketahui Oleh", multiSelection:false, maxLength:10, tag:2});				
		this.cb_app = new saiCBBL(this.pc1.childPage[3],{bound:[20,11,220,20],caption:"Disetujui Oleh", multiSelection:false, maxLength:10, tag:2});				
...
window.app_saku3_transaksi_yakes21_billkes_fAkruBill.extend(window.childForm);
window.app_saku3_transaksi_yakes21_billkes_fAkruBill.implement({				
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into yk_hutang_m(no_hutang,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_app,posted,tgl_input,nik_user,kode_loktuj,no_app,progress,nik_tahu) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','-','"+this.e_ket.getText()+"','"+this.kodepp+"','HUTKES','BILL','IDR',1,"+totHut+",'"+this.app._userLog+"','"+this.cb_app.getText()+"','F',getdate(),'"+this.app._userLog+"','"+this.cb_lokasi.getText()+"','X','1','"+this.cb_tahu.getText()+"')");
							
					nobukti = nobukti.substr(1);	
					//nilai_bp+nilai_cc sudah dikurangi pajak, pajak di jurnal saat bayar				
					sql.add("insert into yk_hutang_d(no_hutang,kode_lokasi,no_inv,periode,kode_vendor,bank,cabang,no_rek,nama_rek,nilai_bp,nilai_cc,no_rekon,bank_trans,no_app,no_pb,no_kas,progress,kode_loktuj,pajak,pajak_bp) "+
							"select '"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_nb.getText()+".'+b.kode_vendor,'"+this.e_periode.getText()+"', "+
							"       b.kode_vendor,e.bank,e.cabang,e.no_rek,e.nama_rek,isnull(sum(case when f.jenis <> 'PENSIUN' then a.nilai - a.pph else 0 end),0),isnull(sum(case when f.jenis = 'PENSIUN' then a.nilai - a.pph else 0 end),0),'-',e.bank_trans,'-','-','-','0','"+this.cb_lokasi.getText()+"',isnull(sum( case when f.jenis = 'PENSIUN' then a.pph else 0 end),0), isnull(sum( case when f.jenis <> 'PENSIUN' then a.pph else 0 end),0) "+
							" from yk_bill_d a "+
		...
		try {
			if (this.cb_drk.getText()!="") {
				...
				var strSQL = 
							...
							"sum(a.umum+a.gigi+a.kbia+a.matkes) as nilai,'PIUKUNJ' as jenis, e.kode_drk,e.nama as nama_drk "+
							...
							"		 	 inner join drk e on '"+this.cb_drk.getText()+"'=e.kode_drk and e.kode_lokasi='"+this.app._lokasi+"' and e.tahun='"+this.e_periode.getText().substr(0,4)+"' "+
							...
							"sum(a.umum+a.gigi+a.kbia) as nilai,'PDPTKUNJUM' as jenis, e.kode_drk,e.nama as nama_drk "+
							"from yk_billkunj_d a "+
							"                     inner join yk_loker bb on a.loker=bb.loker "+
							"				      inner join cust b on bb.kode_cust=b.kode_cust "+
							"		 	          inner join yk_kunj c on a.kode_produk=c.kode_produk "+
							...
							"		 	          inner join drk e on '"+this.cb_drk.getText()+"'=e.kode_drk and e.kode_lokasi='"+this.app._lokasi+"' and e.tahun='"+this.e_periode.getText().substr(0,4)+"' "+
							"where a.umum+a.gigi+a.kbia<> 0 and b.jenis <> 'PENSIUN' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_bill in ("+nobukti+") "+
							"group by  c.nama,c.akun_ku,d.nama,e.kode_drk,e.nama "+												
							"union all "+						
							
							"select 'PDPT KUNJ '+c.nama as ket,c.akun_ob as kode_akun,d.nama as nama_akun,'C', "+
							"sum(a.matkes) as nilai,'PDPTKUNJUM' as jenis, e.kode_drk,e.nama as nama_drk "+
							"from yk_billkunj_d a "+
							"                     inner join yk_loker bb on a.loker=bb.loker "+
							"				      inner join cust b on bb.kode_cust=b.kode_cust "+
							"		 	          inner join yk_kunj c on a.kode_produk=c.kode_produk "+
							...
							"sum(a.cs) as nilai,'HUTCS' as jenis, e.kode_drk,e.nama as nama_drk "+
							"from yk_billkunj_d a "+
							"                     inner join yk_loker bb on a.loker=bb.loker "+
							"					  inner join cust b on bb.kode_cust=b.kode_cust "+
							"		 	          inner join yk_kunj c on a.kode_produk=c.kode_produk "+
							...
							"group by  c.nama,c.akun_hutcs,d.nama,e.kode_drk,e.nama "+
							"union all "+						
							
							"select 'PIU CS '+c.nama as ket,c.akun_piucs as kode_akun,d.nama as nama_akun,'C', "+
							"sum(a.cs) as nilai,'PIUCS' as jenis, e.kode_drk,e.nama as nama_drk "+
							"from yk_billkunj_d a "+
							"                     inner join yk_loker bb on a.loker=bb.loker "+
							"                     inner join cust b on bb.kode_cust=b.kode_cust "+
							"		 	          inner join yk_kunj c on a.kode_produk=c.kode_produk "+
							...
							"		 	          inner join drk e on '"+this.cb_drk.getText()+"'=e.kode_drk and e.kode_lokasi='"+this.app._lokasi+"' and e.tahun='"+this.e_periode.getText().substr(0,4)+"' "+
							"where a.cs<> 0 and b.jenis <> 'PENSIUN' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_bill in ("+nobukti+") "+
							"group by  c.nama,c.akun_piucs,d.nama,e.kode_drk,e.nama "+
	...
	doLoad4:function(sender){																				
		var strSQL = "select a.no_hutang,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai,a.kode_loktuj+' - '+c.nama as lokbayar,a.nik_app+' - '+b.nama as nama "+
					 ...
					 "where a.modul = 'HUTKES' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress in ('1','R') and a.posted='F' ";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU4 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg4.cells(0,row) != "") {
				...
				var strSQL = "select keterangan,tanggal,kode_loktuj,nik_app,nik_tahu "+
							 "from yk_hutang_m "+							 
