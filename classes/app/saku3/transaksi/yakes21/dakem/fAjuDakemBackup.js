window.app_saku3_transaksi_yakes21_dakem_fAjuDakem = function(owner)
{
	if (owner)
	{
		...
		this.cb_if = new portalui_saiCBBL(this.pc1.childPage[0],{bound:[20,10,220,20],caption:"Pemegang IF",tag:2,readOnly:true,change:[this,"doChange"]}); 				
	...
	isiCBnik: function() {
		try {			
			this.cb_if.setSQL("select a.nik, a.nama from karyawan a "+
							"inner join if_nik b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi and b.flag_aktif='1' and b.jenis='DAKEM' "+								
		...
		try {			
			if (this.stsSimpan==1) {				
				if (sender == this.e_nik || this.e_nik.getText()!="") {
					...
					var self = this;
					var datap = [];
					this.app.services.getPesertaSIKAByNIK(this.niksika, function(res) {	
						if(res.status == 'success'){
							if(typeof res.data == 'object' && res.data.length > 0){
								...
								for (j in datap){
									var line = datap[j];							
									if (line != undefined){					
										self.nikes = line.NIKES;					
									}
									var datap2 = [];
									var self2 = self;
									self.app.services.getPesertaSIKA(self.nikes, function(res2) {	
										if(res2.status == 'success'){
											if(typeof res2.data == 'object' && res2.data.length > 0){
												datap2 = res2.data;
												if (typeof datap2 == "object"){
													var line = datap2[0];							
													if (line != undefined){					
														self2.jenis = line.ID_JNS_PESERTA;	
													}else self2.jenis = "00";
												}
												...
												if (self.jenis == "21" || self.jenis == "22" || self.jenis == "23" || self.jenis == "24") {	
													 ...
													 //buat object Date
													 var tanggal1 = new Date(line.TGL_MULAI_KERJA);
													 var tanggal2 = new Date(line.TGL_HENTI_KERJA);
													 // Date.parse akan menghasilkan nilai bernilai integer dalam bentuk milisecond
													 var tglPertama = Date.parse(tanggal1);
													 var tglKedua = Date.parse(tanggal2);
													 var mkerja = (tglKedua - tglPertama) / miliday;
	
													self2.e_nama.setText(line.NM_PESERTA);					
													self2.e_mkerja.setText(mkerja);					
													self2.e_alamat.setText(line.ALMT_KYW2);			
													if (self2.stsSimpan == 1) {
														...
														if (typeof datap2 == "object" && datap2.length > 0){
															...
															for (var i in datap2){
																line2 = datap2[i];										
																self2.sg.appendData(["INPROG",line2.NIKES,line2.NM_PESERTA,line2.TGL_HENTI_KERJA,'-',0,"BELUM",line2.TGL_LAHIR]);
															}
														} 
														else {
															self2.sg.clear(1);	
											...
											system.alert(this,'NIKES Tidak ditemukan ('+self.nikes+')', res2.message);
											return false;
										}
									});	
								}
			...
			if (sender == this.cb_if && this.cb_if.getText()!="") {
				var strSQL = "select a.nik_app,a.nilai - isnull(b.pakai,0) as saldo "+
							"from if_nik a "+
							...
							"where a.jenis='DAKEM' and a.no_kas='"+this.noKasOpen+"' and a.nik ='"+this.cb_if.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.flag_aktif='1'";						   
