window.app_saku3_transaksi_yakes21_sdm_fPbGaji = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_sdm_fPbGaji.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_sdm_fPbGaji";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Permintaan Bayar SDM", 0);	
        ...
        this.c_status = new saiCB(this.pc1.childPage[0],{bound:[20,20,200,20],caption:"Status Organik",items:["ORGANIK","NON"], readOnly:true,tag:2});						
		this.cb_vendor = new saiCBBL(this.pc1.childPage[0],{bound:[20,22,220,20],caption:"Atensi", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});				
		...
		this.sgRek = new saiGrid(this.pc1.childPage[0],{bound:[1,5,990,218],colCount:5,tag:0,
		...
		// this.e_nama = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,17,400,20],caption:"Nama", maxLength:150, tag:2});	
        // this.e_bank = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,18,400,20],caption:"Bank/Cabang", maxLength:150, tag:2});	
        // this.e_norek = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,19,400,20],caption:"No Rekening", maxLength:150, tag:2});
        // this.e_nmrek = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,20,400,20],caption:"Nama Rekening", maxLength:150, tag:2});		
		...
		this.cb_ver = new saiCBBL(this.pc1.childPage[4],{bound:[20,14,220,20],caption:"NIK Verifikator", multiSelection:false, maxLength:10, tag:2});						
		...
		try {
			...
			this.cb_vendor.setSQL("select kode_vendor, nama from vendor where kode_lokasi='"+this.app._lokasi+"'",["kode_vendor","nama"],false,["Kode","Nama"],"and","Data Atensi",true);
			...
			var sql = new server_util_arrayList();
				sql.add("select a.kode_akun,a.nama from masakun a "+
						"inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag in ('065') "+						
...
window.app_saku3_transaksi_yakes21_sdm_fPbGaji.extend(window.childForm);
window.app_saku3_transaksi_yakes21_sdm_fPbGaji.implement({		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into pbh_pb_m (no_pb,no_dokumen,kode_lokasi,periode,nik_user,tgl_input,tanggal,due_date,keterangan,nilai,modul,progress,kode_pp,nik_app,nik_buat,no_hutang,no_spb,no_ver,akun_hutang,nik_ver,bank_trans,no_nota,no_kas,no_app) values  "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.dp_d1.getDateString()+"','"+this.dp_d2.getDateString()+"','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_totalDC.getText())+",'PBSDM','X','"+this.app._kodePP+"','"+this.cb_app.getText()+"','"+this.cb_buat.getText()+"','"+this.cb_vendor.getText()+"','-','-','"+this.c_status.getText()+"','"+this.cb_ver.getText()+"','"+this.e_banktrans.getText()+"','-','-','-')");
						...
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){								
								sql.add("insert into pbh_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"','"+this.sg1.cells(7,i)+"','"+this.sg1.cells(5,i).substr(0,2)+"','PBSDM','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");										
							}
						}
					}		
					...
					if (this.sg4.getRowValidCount() > 0){
						for (var i=0;i < this.sg4.getRowCount();i++){
							if (this.sg4.rowValid(i)){		
								var k = 1000+i;								
								sql.add("insert into pbh_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+k+",'"+this.sg4.cells(0,i)+"','"+this.sg4.cells(3,i)+"','"+this.sg4.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg4.cells(4,i))+",'"+this.sg4.cells(5,i)+"','-','"+this.sg4.cells(5,i).substr(0,2)+"','PBSDM','PAJAK','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");										
							}
						}
					}
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"('"+this.e_nb.getText()+"','PBSDM','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(6,i))+","+nilai+")");
							}
						}
					}

					// sql.add("insert into pbh_rek(kode_vendor,nu,no_bukti,kode_lokasi,modul,nama_rek,no_rek,bank,nama,bruto,pajak,nilai) values "+
					// 		"('"+this.cb_vendor.getText()+"',1,'"+this.e_nb.getText()+"','"+this.app._lokasi+"','PBSDM','"+this.e_nmrek.getText()+"','"+this.e_norek.getText()+"','"+this.e_bank.getText()+"','"+this.e_nama.getText()+"',"+nilaiToFloat(this.e_totalDC.getText())+",0,"+nilaiToFloat(this.e_totalDC.getText())+")");
							...
							if (this.sgRek.rowValid(i)){		
								var k = 1000+i;								
								 sql.add("insert into pbh_rek(kode_vendor,nu,no_bukti,kode_lokasi,modul,nama_rek,no_rek,bank,nama,bruto,pajak,nilai,bank_trans) values "+
										"('"+this.cb_vendor.getText()+"','1','"+this.e_nb.getText()+"','"+this.app._lokasi+"','PBSDM','"+this.sgRek.cells(3,i)+"','"+this.sgRek.cells(2,i)+"','"+this.sgRek.cells(1,i)+"','"+this.sgRek.cells(0,i)+"',"+nilaiToFloat(this.sgRek.cells(4,i))+",0,"+nilaiToFloat(this.sgRek.cells(4,i))+",'"+this.e_banktrans.getText()+"')");										
							}
						}
					}
	...
	doChange:function(sender){
		try {
			...
			if (sender == this.cb_vendor && this.cb_vendor.getText()!="" && this.stsSimpan==1) {
				var strSQL = "select b.bank_trans,a.nama,b.bank as bank,b.no_rek,b.nama_rek "+
							"from vendor a inner join vendor_rek b on a.kode_vendor=b.kode_vendor and a.kode_lokasi=b.kode_lokasi and b.status='1' "+								 
							"where a.kode_vendor = '"+this.cb_vendor.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";										
				...
				if (typeof data == "object"){
					...
					if (line != undefined){													
						...
						this.sgRek.clear();
						this.sgRek.appendData([line.nama,line.bank,line.no_rek,line.nama_rek,"0"]);
					}
				}
			}	
		}
	...
	doLoad3:function(sender){																				
		...
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai "+
					 ...
					 "and a.modul = 'PBSDM' and a.progress in ('X','R','V') "+
					 "order by a.no_pb";						 	 
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.keterangan,a.no_dokumen,a.modul,a.due_date,a.tanggal,a.nik_buat,a.nik_app,a.nik_ver,a.akun_hutang as status "+
							 "from pbh_pb_m a "+								 
				...
				if (typeof data == "object"){
					...
					if (line != undefined){													
						...
						this.cb_vendor.setText(line.no_hutang);									
					}
				}								
				...
				var data = this.dbLib.getDataProvider(
							...
							"where a.jenis='BEBAN' and a.no_pb = '"+this.e_nb.getText()+"'  order by a.no_urut",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg1.clear();
					for (var i in data.rs.rows){
