window.app_saku3_transaksi_yakes21_sdm_fKaryawan = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_sdm_fKaryawan.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_sdm_fKaryawan";
		...
		this.cb_lokdata = new saiCBBL(this,{bound:[20,10,200,20],caption:"Lokasi", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});
		
		this.pc1 = new pageControl(this,{bound:[20,12,1000,430], childPage:["Daftar Karyawan","Data Karyawan","Filter Cari"]});				
		...
		this.pc2 = new pageControl(this.pc1.childPage[1],{bound:[1,12,996,258], childPage:["Lokasi Kerja","PP Lainnya","Data Personal","View Foto"]});						
		this.cb_lokasi = new saiCBBL(this.pc2.childPage[0],{bound:[20,10,220,20],caption:"Lokasi Aktif", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});
		this.cb_pp = new saiCBBL(this.pc2.childPage[0],{bound:[20,15,220,20],caption:"PP Def. Aktif", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});		
		this.cb_lokbaru = new saiCBBL(this.pc2.childPage[0],{bound:[20,10,220,20],caption:"Lokasi Baru", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});
		this.cb_ppbaru = new saiCBBL(this.pc2.childPage[0],{bound:[20,16,220,20],caption:"PP Baru Default", multiSelection:false, maxLength:10, tag:2});
		
		this.sg = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:2,tag:9,
				colTitle:["Kode PP","Nama PP"],
				colWidth:[[1,0],[180,80]],					
		...
		this.e_tel = new saiLabelEdit(this.pc2.childPage[2],{bound:[20,15,400,20],caption:"No Telepon", maxLength:50, tag:1});	
		this.e_hp = new saiLabelEdit(this.pc2.childPage[2],{bound:[20,16,400,20],caption:"No HandPhone", maxLength:50, tag:1});	
		this.e_email = new saiLabelEdit(this.pc2.childPage[2],{bound:[20,17,400,20],caption:"Email", maxLength:50, tag:1});	
		this.e_alamat = new saiLabelEdit(this.pc2.childPage[2],{bound:[20,18,400,20],caption:"Alamat", maxLength:50, tag:1});	
		this.e_kota = new saiLabelEdit(this.pc2.childPage[2],{bound:[20,19,400,20],caption:"Kota", maxLength:50, tag:1});	
		this.e_pos = new saiLabelEdit(this.pc2.childPage[2],{bound:[20,20,200,20],caption:"KodePos", maxLength:5, tag:1});	
		this.e_npwp = new saiLabelEdit(this.pc2.childPage[2],{bound:[20,21,400,20],caption:"NPWP", maxLength:50, tag:1});			
		this.e_bank = new saiLabelEdit(this.pc2.childPage[2],{bound:[20,22,400,20],caption:"Bank", maxLength:50, tag:1});	
		this.e_cabang = new saiLabelEdit(this.pc2.childPage[2],{bound:[520,22,400,20],caption:"Cabang", maxLength:50, tag:1});	
		this.e_norek = new saiLabelEdit(this.pc2.childPage[2],{bound:[20,24,400,20],caption:"No Rekening", maxLength:50, tag:1});	
		this.e_namarek = new saiLabelEdit(this.pc2.childPage[2],{bound:[520,24,400,20],caption:"Nama Rekening", maxLength:50, tag:1});					
		
		this.e_foto = new saiLabelEdit(this.pc2.childPage[3], {bound:[20,11,300,20], caption:"Foto"});
		this.u_foto = new uploader(this.pc2.childPage[3], {bound:[330,11,80,20],param3: "object", param4 :"server/media/",param2 :"server/tmp/", param1 : "uploadTo",
		...
		this.iFoto = new image(this.pc2.childPage[3], {bound:[120, 50, 180,180]});
		...
		try {
			...
			this.cb_lokdata.setSQL("select kode_lokasi,nama from lokasi where flag_konsol='0' ",["kode_lokasi","nama"],false,["Kode","Nama"],"and","Data Lokasi",true);			
			...
			this.cb_lokbaru.setSQL("select kode_lokasi,nama from lokasi where flag_konsol='0' ",["kode_lokasi","nama"],false,["Kode","Nama"],"and","Data Lokasi",true);			
			...
			sql.add("select kode_pp, nama from pp where kode_lokasi<>'00' and flag_aktif='1'");
...
window.app_saku3_transaksi_yakes21_sdm_fKaryawan.extend(window.childForm);
window.app_saku3_transaksi_yakes21_sdm_fKaryawan.implement({
		...
		if (col == 0) {
			if (sender.cells(0,row) != "") {
				...
				if (pp) sender.cells(1,row,pp);
					...
					if (trim(sender.cells(0,row)) != "") system.alert(this,"Kode PP "+sender.cells(5,row)+" tidak ditemukan","Inputkan kode lainnya.","checkPP");                
					sender.cells(0,row,"");
					sender.cells(1,row,"");
				}
			}
			...
			if (sender == this.sg) {				
				if (col == 0){
					this.standarLib.showListData(this, "Daftar PP/Unit",sender,undefined, 
												  "select kode_pp, nama  from pp where kode_lokasi <> '00' and flag_aktif ='1'",
												  "select count(kode_pp) from pp where kode_lokasi <> '00' and flag_aktif ='1'",
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into karyawan(nik,kode_lokasi,nama,alamat,jabatan,no_telp,email,kode_pp,npwp,bank,cabang,no_rek,nama_rek,status,grade,kota,kode_pos,no_hp,flag_aktif, foto) values "+
						    "('"+this.cb_kode.getText()+"','"+this.cb_lokasi.getText()+"','"+this.e_nama.getText()+"','"+this.e_alamat.getText()+"','"+this.e_jabatan.getText()+"','"+this.e_tel.getText()+"','"+this.e_email.getText()+"','"+this.cb_pp.getText()+"','"+this.e_npwp.getText()+"','"+this.e_bank.getText()+"','"+this.e_cabang.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"','"+this.c_jenis.getText()+"','"+this.c_band.getText()+"','"+this.e_kota.getText()+"','"+this.e_pos.getText()+"','"+this.e_hp.getText()+"','"+this.c_status.getText().substr(0,1)+"','"+this.e_foto.getText()+"')");					
					sql.add("insert into karyawan_pp(nik,kode_lokasi,kode_pp) values ('"+this.cb_kode.getText()+"','"+this.cb_lokasi.getText()+"','"+this.cb_pp.getText()+"')");
							...
							if (this.sg.rowValid(i)){	
								if (this.sg.cells(0,i) != this.cb_pp.getText())	{
									sql.add("insert into karyawan_pp (nik,kode_lokasi,kode_pp) values "+
											"('"+this.cb_kode.getText()+"','"+this.cb_lokasi.getText()+"','"+this.sg.cells(0,i)+"')");											
								}
							}
						}
					}
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					...
					sql.add("insert into karyawan(nik,kode_lokasi,nama,alamat,jabatan,no_telp,email,kode_pp,npwp,bank,cabang,no_rek,nama_rek,status,grade,kota,kode_pos,no_hp,flag_aktif,foto) values "+
						    "('"+this.cb_kode.getText()+"','"+this.cb_lokbaru.getText()+"','"+this.e_nama.getText()+"','"+this.e_alamat.getText()+"','"+this.e_jabatan.getText()+"','"+this.e_tel.getText()+"','"+this.e_email.getText()+"','"+this.cb_ppbaru.getText()+"','"+this.e_npwp.getText()+"','"+this.e_bank.getText()+"','"+this.e_cabang.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"','"+this.c_jenis.getText()+"','"+this.c_band.getText()+"','"+this.e_kota.getText()+"','"+this.e_pos.getText()+"','"+this.e_hp.getText()+"','"+this.c_status.getText().substr(0,1)+"','"+this.e_foto.getText()+"')");
					sql.add("insert into karyawan_pp(nik,kode_lokasi,kode_pp) values ('"+this.cb_kode.getText()+"','"+this.cb_lokbaru.getText()+"','"+this.cb_ppbaru.getText()+"')");

					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){		
								if (this.sg.cells(0,i) != this.cb_ppbaru.getText())	{																													
									sql.add("insert into karyawan_pp (nik,kode_lokasi,kode_pp) values "+
											"('"+this.cb_kode.getText()+"','"+this.cb_lokasi.getText()+"','"+this.sg.cells(0,i)+"')");											
								}
							}
	...
	doChange: function(sender){
		try{			
			...
			if (sender == this.cb_lokdata && this.cb_lokdata.getText() != "") {	
				this.doLoad();
			}
			...
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				...
				if (typeof data == "object"){
					...
					if (line != undefined){						
						...
						var data = this.dbLib.getDataProvider("select a.kode_pp,a.nama from pp a inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and a.kode_pp<>'"+line.kode_pp+"' where b.nik='"+this.cb_kode.getText()+"'",true);
						if (typeof data == "object" && data.rs.rows[0] != undefined){
							var line;
							this.sg.clear();
							for (var i in data.rs.rows){
	...
	doLoad:function(sender){						
		var strSQL = "select a.nik,a.nama,a.grade as band,a.jabatan,a.status,a.no_telp,a.no_hp,a.email,a.alamat,a.kode_pp+' - '+b.nama as pp "+
		             "from karyawan a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi  "+
					 "where a.status<>'CLIENT' and a.kode_lokasi='"+this.cb_lokdata.getText()+"' order by a.flag_aktif desc,a.nik";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
			this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length/20));
