window.app_saku3_transaksi_yakes21_spj_fSpjJenis = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes21_spj_fSpjJenis.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes21_spj_fSpjJenis";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Jenis Parameter", 0);	
...
window.app_saku3_transaksi_yakes21_spj_fSpjJenis.extend(window.childForm);
window.app_saku3_transaksi_yakes21_spj_fSpjJenis.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update yk_spj_jenis set nama='"+this.e_nama.getText()+"'"+
	...
	doCari:function(sender){								
		try {
			...
			var strSQL = "select kode_jenis,nama "+
						 "from yk_spj_jenis where "+filter+" order by kode_jenis";								
			var data = this.dbLib.getDataProvider(strSQL,true);		
	...
	doLoad:function(sender){								
		try {
			var strSQL = "select kode_jenis,nama "+
						 "from yk_spj_jenis order by kode_jenis";				
			var data = this.dbLib.getDataProvider(strSQL,true);		
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU = data;
				this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length/20));
