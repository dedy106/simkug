window.app_saku3_transaksi_ypkp_kug_fKbin = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypkp_kug_fKbin.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypkp_kug_fKbin";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank : Penerimaan", 0);	
...
window.app_saku3_transaksi_ypkp_kug_fKbin.extend(window.childForm);
window.app_saku3_transaksi_ypkp_kug_fKbin.implement({	
	...
	simpan: function(){			
		try{						
			if (this.stsSimpan == 1) {				
				if (this.stsSimpan == 1) this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti",this.app._lokasi+"-BM"+this.e_periode.getText().substr(2,4)+".","0000"));						
			}
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','KB','KBIN','F','-','-','"+this.cb_pp.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok2.getText()+"','"+this.e_atensi.getText()+"','IDR',1,"+
					...
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_dok2.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.cb_kas.getText()+"','D',"+parseNilai(this.e_nilai.getText())+","+
	...
	doCari:function(sender){																									
		...
		var strSQL = "select distinct a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.no_ref1,a.keterangan,a.nilai1,a.tanggal "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='KB' and a.form = 'KBIN' and a.posted ='F' "+filter+
					 "order by a.tanggal";
	...
	doLoad3:function(sender){		
		try {																							
			var strSQL = "select distinct a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.no_ref1,a.keterangan,a.nilai1,a.tanggal "+
						"from trans_m a "+			
						"inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.nik='"+this.app._userLog+"' "+ 					 
						"where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='KB' and a.form = 'KBIN' and a.posted ='F'  "+
						"order by a.tanggal";					 
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU3 = data;
