window.app_saku3_transaksi_apparindo_fClosingThn = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_apparindo_fClosingThn.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_apparindo_fClosingThn";
...
window.app_saku3_transaksi_apparindo_fClosingThn.extend(window.childForm);
window.app_saku3_transaksi_apparindo_fClosingThn.implement({
				...
				try{														
					...
					if (this.isPostedAll(this.app._periode)){						
						sql.add("insert into glma(kode_akun, kode_lokasi, periode, so_akhir, tgl_input,nik_user)  "+
								"select kode_akun,kode_lokasi,'"+closePeriode(this.app._periode,this.maxPeriode)+"',so_akhir,getdate(),'"+this.app._userLog+"' "+
	...
	isPostedAll: function(periode){
		...
		if (ck != "") { 
			...
			if (data[1] != "0"){
				result = result && false;
				msg+= "Data Modul PIUTANG [PIU] masih ada yang belum diposting.("+data[1]+" data)\n";
			}
		}
		...
		if (ck != "") { 
			...
			if (data[1] != "0"){
				result = result && false;
				msg+= "Data Modul TR masih ada yang belum diposting.("+data[1]+" data)\n";
			}
		}	
