window.app_saku3_transaksi_apparindo_fRekonPiu = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_apparindo_fRekonPiu.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_apparindo_fRekonPiu";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Rekonsilisai Piutang Umum", 0);	
		...
		this.e_total = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,16,200,20],caption:"Nilai Pelunasan", tag:1, tipeText:ttNilai, text:"0",change:[this,"doChange"]});					
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,20,990,305], childPage:["Bukti KasBank", "Data Invoice","Jurnal Sisa"]});		
		this.c_perKas = new saiCB(this.pc1.childPage[0],{bound:[20,17,200,20],caption:"Periode KasBank",readOnly:true,tag:2,change:[this,"doChange"]});	
		this.cb_kasbank = new saiCBBL(this.pc1.childPage[0],{bound:[20,11,220,20],caption:"No KasBank", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});							
		this.cb_akun = new saiCBBL(this.pc1.childPage[0],{bound:[20,15,220,20],caption:"Akun Pelunasan", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});							
		this.e_saldokas = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,18,200,20],caption:"Saldo Jurnal", tag:1, readOnly:true, tipeText:ttNilai, text:"0",change:[this,"doChange"]});					

		this.sg1 = new saiGrid(this.pc1.childPage[1],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:6,tag:0,
				colTitle:["Status","No Invoice","Tanggal","Keterangan","Akun Piutang","Tagihan"],
				colWidth:[[5,4,3,2,1,0],[100,100,400,80,150,80]],
		...
		this.sg = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:7,tag:9,
					...
					colWidth:[[6,5,4,3,2,1,0],[150,80,100,250,50,200,80]],										
		...
		try {
			...
			this.c_perKas.items.clear();
			var data = this.dbLib.getDataProvider("select distinct periode from kas_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];													
					this.c_perKas.addItem(i,line.periode);
				}
			} 
			this.c_perKas.setText("");
		}catch(e){
			systemAPI.alert(e);
		}
	}
};
window.app_saku3_transaksi_apparindo_fRekonPiu.extend(window.childForm);
window.app_saku3_transaksi_apparindo_fRekonPiu.implement({
	doLoad : function() {
		this.pc2.setActivePage(this.pc2.childPage[0]);	
		this.pc1.setActivePage(this.pc1.childPage[1]);																		
		if (this.stsSimpan==1 && this.cb_cust.getText()!="") {		
			var strSQL = "select a.no_piutang,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.akun_piutang,a.nilai+a.nilai_ppn - isnull(b.bayar,0) as saldo "+
						 "from piutang_m a "+

						 "    left join ( "+
						 "             select no_piutang,sum(case dc when 'D' then nilai else -nilai end) as bayar "+
						 "             from piubayar_d where kode_lokasi='"+this.app._lokasi+"' "+
						 "			   group by no_piutang "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg1.appendData(["INPROG",line.no_piutang,line.tgl,line.keterangan,line.akun_piutang,floatToNilai(line.saldo)]);					
				}
			} else this.sg1.clear(1);
			this.sg1.validasi();
		}	
		else system.alert(this,"Customer harus diisi.","");
	},
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_kasbank.getText()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','MIREKON','"+this.c_jenis.getText()+"','IDR',1,"+parseNilai(this.e_total.getText())+",'"+this.app._userLog+"','-','F','-','"+this.cb_akun.getText()+"','"+this.cb_cust.getText()+"',getdate(),'"+this.app._userLog+"')");
					...
					sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_kasbank.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','D',"+parseNilai(this.e_total.getText())+",'"+this.app._kodePP+"','-','-','-','-','-','-','-','"+this.app._lokasi+"','MIREKON','TITIP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','-')");
					
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && this.sg1.cells(0,i) == "BAYAR") {																				

							sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
									"('"+this.e_nb.getText()+"','"+this.sg1.cells(1,i)+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(4,i)+"','Pemby atas INV : "+this.sg1.cells(1,i)+"','C',"+nilaiToFloat(this.sg1.cells(5,i))+",'"+this.app._kodePP+"','-','-','-','-','-','-','-','"+this.app._lokasi+"','MIREKON','PIU','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','-')");
							...
							sql.add("insert into piubayar_d(no_bukti,no_piutang,kode_lokasi,modul,periode,nik_user,tgl_input,dc,nilai,no_ref1,no_ref2) values "+
									"('"+this.e_nb.getText()+"','"+this.sg1.cells(1,i)+"','"+this.app._lokasi+"','MIREKON','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'D',"+parseNilai(this.sg1.cells(5,i))+",'"+this.cb_kasbank.getText()+"','"+this.cb_akun.getText()+"')");					
						}
					}
					...
					if (this.sg.getRowValidCount() > 0) {
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								var k = i+5000;				
								sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
										"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+k+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg.cells(4,i))+",'"+this.app._kodePP+"','-','-','-','-','-','-','-','"+this.app._lokasi+"','MIREKON','SLS','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','-')");				
							}
						}
					}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :														
				...
				if (nilaiToFloat(this.e_total.getText()) > nilaiToFloat(this.e_saldokas.getText())) {
					system.alert(this,"Transaksi tidak valid.","Nilai Pelunasan tidak boleh melebihi saldo jurnal.");
					return false;						
				}
	...
	doChange:function(sender){		
		...
		if (sender == this.c_perKas && this.c_perKas.getText()!="") {
			this.cb_kasbank.setSQL("select no_kas, keterangan from kas_m where jenis = 'BM' and periode = '"+this.c_perKas.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kas","keterangan"],false,["No Bukti","Deskripsi"],"and","Data KasBank",true);						
		}
		if (sender == this.cb_kasbank && this.cb_kasbank.getText()!="") {
			this.cb_akun.setSQL("select a.kode_akun, a.nama from masakun a inner join kas_j b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi where b.no_kas = '"+this.cb_kasbank.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and b.dc = 'C'",
								["kode_akun","nama"],false,["Kode","Nama"],"and","Data Akun",true);
		}
		if ((sender == this.cb_kasbank || sender == this.cb_akun) && this.cb_kasbank.getText()!="" && this.cb_akun.getText()!="") {			
			var strSQL = "select sum(case dc when 'C' then nilai else -nilai end) as nilai "+			             
						 "from kas_j a "+						 
						 "where a.no_kas='"+this.cb_kasbank.getText()+"' and a.kode_akun='"+this.cb_akun.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){		
					this.e_saldokas.setText(floatToNilai(line.nilai));
				} 
			}

			var totpakai = 0;
			var strSQL = "select isnull(sum(case dc when 'D' then nilai else -nilai end),0) as totpakai "+
						 "from piubayar_d a "+						 
						 "where no_bukti<>'"+this.e_nb.getText()+"' and a.no_ref1='"+this.cb_kasbank.getText()+"' and a.no_ref2='"+this.cb_akun.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){		
					totpakai = parseFloat(line.totpakai);					
				} 				
			}
			
			var saldokas = nilaiToFloat(this.e_saldokas.getText()) - totpakai;
			this.e_saldokas.setText(floatToNilai(saldokas));
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "" && this.c_jenis.getText()!= "") {
			...
			this.stsSimpan = 1;			
			this.cb_cust.setSQL("select kode_cust, nama from cust2 where kode_lokasi='"+this.app._lokasi+"'",["kode_cust","nama"],false,["Kode","Nama"],"and","Data Cust",true);
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_ju,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
		             "from ju_m a "+					 
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'MIREKON' and a.posted ='F'";					 
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_ju,line.tgl,line.keterangan,nilaiToFloat(line.nilai)]); 
		}
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){															
						...
						this.cb_cust.setSQL("select kode_cust, nama from cust2 where kode_cust='"+line.ref1+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_cust","nama"],false,["Kode","Nama"],"and","Data Customer",true);				
						this.cb_cust.setText(line.ref1);	
						
						var noKasBank = line.no_dokumen;
						var akunKas = line.no_link;
					}
				}			
				
				var strSQL = "select periode from kas_m where no_kas = '"+noKasBank+"' and kode_lokasi='"+this.app._lokasi+"'";						
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){		
						this.c_perKas.setText(line.periode);	
						this.cb_kasbank.setText(noKasBank);	
						this.cb_akun.setText(akunKas);	
					}
				}	
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData(["BAYAR",line.no_piutang,line.tgl,line.keterangan,line.akun_piutang,floatToNilai(line.saldo)]);					
					}
				} else this.sg1.clear(1);
