window.app_saku3_transaksi_apparindo_fJab = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_apparindo_fJab.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_apparindo_fJab";
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,450], childPage:["Daftar Jabatan","Data Jabatan"]});
...
window.app_saku3_transaksi_apparindo_fJab.extend(window.childForm);
window.app_saku3_transaksi_apparindo_fJab.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					var sql = new server_util_arrayList();					
						sql.add("insert into ape_jab(kode_jab,kode_lokasi,nama) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();					
						sql.add("delete from ape_jab where kode_jab = '"+this.e_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");			
						sql.add("insert into ape_jab(kode_jab,kode_lokasi,nama) values "+
								"('"+this.e_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"')");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
	...
	doChange: function(sender){
		try{
			if (sender == this.e_kode && this.e_kode.getText() != ""){
				var strSQL = "select * from ape_jab where kode_jab ='"+this.e_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   				
