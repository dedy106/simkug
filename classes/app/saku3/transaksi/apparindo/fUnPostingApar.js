window.app_saku3_transaksi_apparindo_fUnPostingApar = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_apparindo_fUnPostingApar.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_apparindo_fUnPostingApar";
		...
		this.c_proses = new saiCB(this,{bound:[20,21,200,20],caption:"Jenis Proses",items:["FRONTEND"], readOnly:true,tag:2});		
		this.c_modul = new saiCB(this,{bound:[20,22,200,20],caption:"Modul",items:["JU","KB","AR","TR"], readOnly:true,tag:2,change:[this,"doChange"]});
...
window.app_saku3_transaksi_apparindo_fUnPostingApar.extend(window.childForm);
window.app_saku3_transaksi_apparindo_fUnPostingApar.implement({
			...
			if (this.c_modul.getText() != "") {
				...
				switch(this.c_modul.getText()){
					...
					case "KB" :
						...
						break;																						
					case "AR" :
								strSQL = "select 'INPROG' as status,no_piutang as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'PIU' as jenis "+
										 "from piutang_m "+
										 "where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";
								this.cb_bukti.setSQL("select no_piutang as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'PIU' as jenis from piutang_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ",
