window.app_saku3_transaksi_dkm_fMustahiq = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_dkm_fMustahiq.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_dkm_fMustahiq";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Mustahiq", 0);	
		
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		uses("saiGrid",true);		
		
		this.pc1 = new pageControl(this,{bound:[20,12,1000,450], childPage:["Daftar Mustahiq","Data Mustahiq","Filter Cari"]});
		...
		this.e_nilai = new saiLabelEdit(this.pc1.childPage[1],{bound:[320,22,200,20],caption:"Nilai", tag:1, tipeText:ttNilai, text:"0",readOnly:true});	
		...
		try {
			...
			this.doLoad();
			var strSQL = "select kode_salur,nama,0 as nilai from dkm_salur where kode_lokasi='"+this.app._lokasi+"' order by kode_salur";				
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg2.appendData([line.kode_salur,line.nama,floatToNilai(line.nilai)]);
				}
...
window.app_saku3_transaksi_dkm_fMustahiq.extend(window.childForm);
window.app_saku3_transaksi_dkm_fMustahiq.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into dkm_mustahiq(kode_mustahiq,kode_lokasi,nama,alamat,no_tel,email,bank,cabang,no_rek,nama_rek,nilai,jenis) values "+
					...
					for (var i = 0;i < this.sg2.getRowCount();i++){
						if (this.sg2.rowValid(i) && this.sg2.cells(2,i) != ""){
							sql.add("insert into dkm_mustahiq_d(kode_mustahiq,kode_salur,kode_lokasi,nilai) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from dkm_mustahiq where kode_mustahiq='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
					sql.add("delete from dkm_mustahiq_d where kode_mustahiq='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				...
				var strSQL = "select kode_mustahiq,nama,alamat,no_tel,email,bank,cabang,no_rek,nama_rek,nilai,jenis "+
				             "from dkm_mustahiq "+
						     "where kode_mustahiq ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					...
					if (line != undefined){					
						...
						var strSQL = "select a.kode_salur,a.nama,isnull(b.nilai,0) as nilai from dkm_salur a left join dkm_mustahiq_d b on a.kode_salur=b.kode_salur and a.kode_lokasi=b.kode_lokasi and b.kode_mustahiq='"+this.cb_kode.getText()+"' "+
	...
	doBtnClick: function(sender, event){
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Mustahiq",sender,undefined, 
											  "select kode_mustahiq, nama  from dkm_mustahiq where kode_lokasi='"+this.app._lokasi+"'",
											  "select count(kode_mustahiq) from dkm_mustahiq where kode_lokasi='"+this.app._lokasi+"'",
											  ["kode_mustahiq","nama"],"and",["Kode","Nama"],false);				
			}
		}catch(e){
			systemAPI.alert(e);
		}
	...
	doCari:function(sender){								
		try {
			if (this.e_kode2.getText() != "") var filter = " a.kode_mustahiq like '%"+this.e_kode2.getText()+"%' and ";
			else var filter = " a.nama like '%"+this.e_nama2.getText()+"%' and ";
			
			var strSQL = "select a.kode_mustahiq,a.nama,a.alamat,a.jenis "+
						 "from dkm_mustahiq a "+					 
						 "where "+filter+" a.kode_lokasi='"+this.app._lokasi+"' order by a.kode_mustahiq";				
			var data = this.dbLib.getDataProvider(strSQL,true);		
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU = data;
	...
	doLoad:function(sender){								
		try {
			var strSQL = "select a.kode_mustahiq,a.nama,a.alamat,a.jenis "+
						 "from dkm_mustahiq a "+					 
						 "where a.kode_lokasi='"+this.app._lokasi+"' order by a.kode_mustahiq";				
			var data = this.dbLib.getDataProvider(strSQL,true);		
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU = data;
				this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length/20));
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_mustahiq,line.nama,line.alamat,line.jenis]); 
		}
