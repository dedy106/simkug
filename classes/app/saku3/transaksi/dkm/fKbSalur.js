window.app_saku3_transaksi_dkm_fKbSalur = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_dkm_fKbSalur.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_dkm_fKbSalur";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Penyaluran", 0);	
		...
		this.cb_agg = new saiCBBL(this.pc2.childPage[0],{bound:[20,13,220,20],caption:"Mustahiq", multiSelection:false, maxLength:10, tag:1, change:[this,"doChange"]});				
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,20,990,325], childPage:["Item Penyaluran"]});		
		...
		try {
			...
			this.cb_agg.setSQL("select kode_mustahiq, nama from dkm_mustahiq where kode_lokasi='"+this.app._lokasi+"'",["kode_mustahiq","nama"],false,["Kode","Nama"],"and","Data Mustahiq",true);						
			this.c_jenis.setText("BK");
			
			var strSQL = "select kode_salur,nama,kode_akun from dkm_salur where kode_lokasi='"+this.app._lokasi+"' order by kode_salur";				
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg1.appendData([line.kode_salur,line.nama,line.kode_akun,"0"]);
				}
...
window.app_saku3_transaksi_dkm_fKbSalur.extend(window.childForm);
window.app_saku3_transaksi_dkm_fKbSalur.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from dkm_salur_d where no_kas='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}
					
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_agg.getText()+"','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','DKMSALUR','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','-','-','-')");				
					
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',999,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','DKMSALUR','KREDIT','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.e_nilai.getText())+")");										
							
					for (var i = 0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && this.sg1.cells(2,i) != "" && this.sg1.cells(3,i) != "0"){
							sql.add("insert into dkm_salur_d(no_kas,kode_mustahiq,kode_salur,kode_lokasi,nilai,modul,dc,kode_akun) values "+
									"('"+this.e_nb.getText()+"','"+this.cb_agg.getText()+"','"+this.sg1.cells(0,i)+"','"+this.app._lokasi+"',"+nilaiToFloat(this.sg1.cells(3,i))+",'SALUR','D','"+this.sg1.cells(2,i)+"')");
							
							sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
									"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(2,i)+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.sg1.cells(3,i))+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','DKMSALUR','DEBET','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.sg1.cells(3,i))+")");					
	...
	doChange:function(sender){
		try {
			...
			if (sender == this.cb_agg && this.cb_agg.getText()!="" && this.stsSimpan == 1) {
				this.e_ket.setText("Penyaluran a.n "+this.cb_agg.rightLabelCaption);
				var strSQL = "select a.kode_salur,a.nama,a.kode_akun,isnull(b.nilai,0) as nilai "+
				             "from dkm_salur a left join dkm_mustahiq_d b on a.kode_salur=b.kode_salur and a.kode_lokasi=b.kode_lokasi and b.kode_mustahiq='"+this.cb_agg.getText()+"' "+
							 "where a.kode_lokasi='"+this.app._lokasi+"' order by a.kode_salur";				
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg1.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData([line.kode_salur,line.nama,line.kode_akun,floatToNilai(line.nilai)]);
					}
				} else this.sg1.clear(1);
			}
		}
	...
	doCari:function(sender){																									
		...
		var strSQL = "select distinct a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai,a.tanggal "+
		             "from kas_m a "+			 					 
					 "where no_del='-' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'DKMSALUR' and a.posted ='F' "+filter+
					 "order by a.tanggal";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doLoad3:function(sender){																									
		var strSQL = "select distinct a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai,a.tanggal "+
		             "from kas_m a "+			 					 
					 "where no_del='-' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'DKMSALUR' and a.posted ='F' "+
					 "order by a.tanggal";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {		
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.kode_salur,a.nama,a.kode_akun,isnull(b.nilai,0) as nilai "+
				             "from dkm_salur a left join dkm_salur_d b on a.kode_salur=b.kode_salur and a.kode_lokasi=b.kode_lokasi "+
							 "where b.no_kas='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.kode_salur";				
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg1.clear();
