window.app_saku3_transaksi_dkm_fPostingDkm = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_dkm_fPostingDkm.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_dkm_fPostingDkm";
...
window.app_saku3_transaksi_dkm_fPostingDkm.extend(window.childForm);
window.app_saku3_transaksi_dkm_fPostingDkm.implement({
		...
		switch (event){
			...
			case "simpan" :	
				...
				var strSQL = "select no_bukti+' - '+periode as bukper from ("+
						 ...
						 "where no_del<>'-' and a.no_kas in ("+this.nobukti+") and a.kode_lokasi='"+this.app._lokasi+"' group by a.no_kas,a.periode "+						 
			...
			if (this.sg1.rowValid(i) && this.sg1.cells(0,i)=="TRUE") {
				...
				if (this.sg1.cells(1,i) == "KB") 
					strSQL += "union all "+
							  ...
							  "from kas_m where no_del<>'-' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";				
				/*
