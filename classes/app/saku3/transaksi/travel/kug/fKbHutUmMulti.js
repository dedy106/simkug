window.app_saku3_transaksi_travel_kug_fKbHutUmMulti = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_travel_kug_fKbHutUmMulti.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_travel_kug_fKbHutUmMulti";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Pembayaran Hutang [Multi]", 0);	
...
window.app_saku3_transaksi_travel_kug_fKbHutUmMulti.extend(window.childForm);
window.app_saku3_transaksi_travel_kug_fKbHutUmMulti.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','KB','KBHUTMT','F','-','-','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','IDR',1,"+
					...
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','C',"+parseNilai(this.e_total.getText())+","+
					...
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && this.sg1.cells(0,i) == "BAYAR"){
							...
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.sg1.cells(2,i)+"','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sg1.cells(6,i)+"','D',"+nilaiHut+","+
									nilaiHut+",'"+this.e_ket.getText()+"','KB','HUTANG','IDR',1,'"+this.sg1.cells(8,i)+"','-','-','"+this.cb_vendor.getText()+"','-','-','"+this.sg1.cells(1,i)+"','-','-')");											
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_vendor && this.cb_vendor.getText()!="" && this.stsSimpan==1 ) {
			var strSQL = "select a.no_hutang,a.no_dokumen,a.no_fp,CONVERT(varchar,a.due_date,103) as due_date,a.keterangan,a.akun_hutang,a.nilai-ISNULL(b.bayar,0) as saldo,a.kode_pp "+
						 ...
						 "			from hutbayar_d where kode_lokasi ='"+this.app._lokasi+"' "+
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_travel_rptHutangKasJurnal";
								this.filter = " where a.kode_lokasi='"+this.app._lokasi+"' and a.no_bukti='"+this.e_nb.getText()+"' and a.form in ('MIHUT','KBHUTMT') ";
	...
	doLoad3:function(sender){		
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,b.kode_vendor+' - '+ b.nama as vendor, a.nilai1 "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.form = 'KBHUTMT' and a.posted ='F'";
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						this.cb_vendor.setSQL("select kode_vendor, nama from vendor where kode_vendor='"+line.param1+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_vendor","nama"],false,["Kode","Nama"],"and","Data Vendor",true);									
					}
				}
				
				var strSQL = "select a.no_hutang,a.no_dokumen,a.no_fp,CONVERT(varchar,a.due_date,103) as due_date,a.keterangan,a.akun_hutang,a.nilai-ISNULL(b.bayar,0) as saldo, a.kode_pp "+
							 "from hutang_d a  "+
							 "	    inner join hutbayar_d c on a.no_hutang=c.no_hutang and a.kode_lokasi=c.kode_lokasi "+		 
							 
							 "		left join ("+
							 "				select no_hutang,kode_lokasi,SUM(case dc when 'D' then nilai else -nilai end) as bayar "+
							 "				from hutbayar_d where kode_lokasi ='"+this.app._lokasi+"' and no_bukti <>'"+this.e_nb.getText()+"' "+
							 "				group by no_hutang,kode_lokasi "+
							 "		) b on a.no_hutang=b.no_hutang and a.kode_lokasi=b.kode_lokasi "+
