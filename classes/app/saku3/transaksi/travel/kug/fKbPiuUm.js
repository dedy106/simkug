window.app_saku3_transaksi_travel_kug_fKbPiuUm = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_travel_kug_fKbPiuUm.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_travel_kug_fKbPiuUm";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penyelesaian Piutang Umum", 0);	
...
window.app_saku3_transaksi_travel_kug_fKbPiuUm.extend(window.childForm);
window.app_saku3_transaksi_travel_kug_fKbPiuUm.implement({
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){														
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										...
										parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(3,i)+"','"+this.modul+"','KB','IDR',1,'"+this.sg.cells(5,i)+"','-','-','-','-','-','-','-','-')");											
							}
						}
					}
