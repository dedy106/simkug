window.app_saku3_transaksi_travel_kug_fCust = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_travel_kug_fCust.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_travel_kug_fCust";
		...
		this.cb_akun = new saiCBBL(this.pc1.childPage[1],{bound:[20,13,220,20],caption:"Akun Piutang", multiSelection:false, maxLength:10, tag:2});								
...
window.app_saku3_transaksi_travel_kug_fCust.extend(window.childForm);
window.app_saku3_transaksi_travel_kug_fCust.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into cust(kode_cust,kode_lokasi,nama,alamat,no_tel,email,npwp,pic,alamat2,no_fax,akun_piutang) values "+
						    "('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.e_alamat.getText()+"','"+this.e_tel.getText()+"','"+this.e_mail.getText()+"','"+this.e_npwp.getText()+"','"+this.e_pic.getText()+"','"+this.e_alamat2.getText()+"','"+this.e_fax.getText()+"','"+this.cb_akun.getText()+"')");
					setTipeButton(tbAllFalse);
