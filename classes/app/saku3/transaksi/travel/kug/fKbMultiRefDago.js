window.app_saku3_transaksi_travel_kug_fKbMultiRefDago = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_travel_kug_fKbMultiRefDago.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_travel_kug_fKbMultiRefDago";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Umum Dual Akun : Pengeluaran", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,12,990,260], childPage:["Data Jurnal"]});
		...
		try {
			...
			sql.add("select kode_ref,nama from trans_ref where kode_lokasi = '"+this.app._lokasi+"' and jenis='PENGELUARAN'");												
...
window.app_saku3_transaksi_travel_kug_fKbMultiRefDago.extend(window.childForm);
window.app_saku3_transaksi_travel_kug_fKbMultiRefDago.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','KB','KBMULTIOUT','F','-','-','"+this.cb_pp.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok2.getText()+"','"+this.e_atensi.getText()+"','IDR',1,"+
					...
					for (var i=0;i < this.sg.getRowCount();i++) {
						if (this.sg.rowValid(i)){			
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_dok2.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(4,i)+"','D',"+parseNilai(this.sg.cells(3,i))+","+
									parseNilai(this.sg.cells(3,i))+",'"+this.sg.cells(2,i)+"','KB','BEBAN','IDR',1,'"+this.cb_pp.getText()+"','-','-','-','-','-','"+this.sg.cells(0,i)+"','-','-')");							
						}
					}
	    ...
	    if (col == 0) {
			if (sender.cells(0,row) != "") {				
				...
				if (namaref) {
					...
					var data = this.dbLib.getDataProvider("select akun_debet from trans_ref where kode_ref='"+sender.cells(0,row)+"' and kode_lokasi='"+this.app._lokasi+"'",true);
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
						if (line != undefined){							
							sender.cells(4,row,line.akun_debet);
						}
					}				
				}
				else {                                    
			...
			if (sender == this.sg) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Referensi",sender,undefined, 
						    "select a.kode_ref,a.nama from trans_ref a where a.kode_lokasi = '"+this.app._lokasi+"' and jenis='PENGELUARAN' ",
							"select count(*) from trans_ref a where a.kode_lokasi = '"+this.app._lokasi+"' and jenis='PENGELUARAN' ",
	...
	doCari:function(sender){																									
		...
		var strSQL = "select distinct a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.no_ref1,a.keterangan,a.nilai1,a.tanggal "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='KB' and a.form = 'KBMULTIOUT' and a.posted ='F' "+filter+
					 "order by a.tanggal";
	...
	doLoad3:function(sender){																									
		var strSQL = "select distinct a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.no_ref1,a.keterangan,a.nilai1,a.tanggal "+
		             "from trans_m a "+			
								 "inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.nik='"+this.app._userLog+"' "+ 					 
								 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='KB' and a.form = 'KBMULTIOUT' and a.posted ='F'  "+
								 "order by a.tanggal";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select b.kode_ref,b.nama,a.keterangan,a.nilai,a.kode_akun "+
							 ...
							 "where a.jenis='BEBAN' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_bukti='"+this.e_nb.getText()+"'";
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
