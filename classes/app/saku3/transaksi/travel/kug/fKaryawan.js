window.app_saku3_transaksi_travel_kug_fKaryawan = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_travel_kug_fKaryawan.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_travel_kug_fKaryawan";
		...
		this.cb_jenis = new saiCBBL(this.pc1.childPage[1],{bound:[20,24,220,20],caption:"Status", multiSelection:false, maxLength:10, tag:2});
		...
		this.e_fm = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,27,200,20],caption:"FM", maxLength:30, tag:1});			
		...
		try {
			...
			this.cb_jenis.setSQL("select kode_sts,nama from karyawan_sts where kode_lokasi='"+this.app._lokasi+"' ",["kode_sts","nama"],false,["Kode","Nama"],"where","Data Status Karyawan",true);
...
window.app_saku3_transaksi_travel_kug_fKaryawan.extend(window.childForm);
window.app_saku3_transaksi_travel_kug_fKaryawan.implement({
	doLoadPP : function() {
	...
	doChange: function(sender){
		try{						
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select nik, kode_lokasi, nama, alamat, jabatan, no_telp, email, kode_pp, npwp, bank, cabang, no_rek, nama_rek, status, grade, kota, kode_pos, no_hp, flag_aktif, foto, ttd,fm "+
				             "from karyawan "+
