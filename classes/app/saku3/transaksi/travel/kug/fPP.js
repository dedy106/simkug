window.app_saku3_transaksi_travel_kug_fPP = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_travel_kug_fPP.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_travel_kug_fPP";
...
window.app_saku3_transaksi_travel_kug_fPP.extend(window.childForm);
window.app_saku3_transaksi_travel_kug_fPP.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into pp(kode_pp,kode_lokasi,nama,kode_bidang,jenis,flag_aktif,tipe,nik, kota,bank,norek) values "+
							"('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','-','-','"+this.c_status.getText().substr(0,1)+"','Posting','"+this.cb_nik.getText()+"','-','-','-')");					
					setTipeButton(tbAllFalse);
	...
	doLoad:function(sender){						
		var strSQL = "select a.kode_pp,a.nama,case when a.flag_aktif='1' then '1. AKTIF' else '0. NON' end as status "+
		             "from pp a "+
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_pp,line.nama,line.status.toUpperCase()]); 
		}
