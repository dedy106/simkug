window.app_saku3_transaksi_travel_kug_fKbHutUm = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_travel_kug_fKbHutUm.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_travel_kug_fKbHutUm";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penyelesaian Hutang Umum", 0);	
		...
		this.c_jenis = new saiCB(this.pc2.childPage[0],{bound:[20,11,200,20],caption:"Jenis",items:["MI"], readOnly:true,tag:2});
...
window.app_saku3_transaksi_travel_kug_fKbHutUm.extend(window.childForm);
window.app_saku3_transaksi_travel_kug_fKbHutUm.implement({
	doLoadRev: function() {
		if (this.cb_hutang.getText()!="") {
			strSQL = "select a.kode_akun,b.nama as nama_akun,case a.dc when 'D' then 'C' else 'D' end as dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp  "+
					...
					"where a.jenis <> 'HUTANG' and a.no_hutang = '"+this.cb_hutang.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut";			
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','MI','MIHUT','F','-','-','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','IDR',1,"+
					...
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							...
							parseNilai(this.e_nilai.getText())+",'"+this.e_ket.getText()+"','MI','HUTANG','IDR',1,'"+this.app._kodePP+"','-','-','"+this.cb_vendor.getText()+"','-','-','-','-','-')");										
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){																															
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										...
										parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(3,i)+"','MI','BEBAN','IDR',1,'"+this.sg.cells(5,i)+"','-','-','-','-','-','-','-','-')");											
							}
						}
					}

					sql.add("insert into hutbayar_d(no_bukti,kode_lokasi,no_hutang,periode,dc,modul,nilai,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_hutang.getText()+"','"+this.e_periode.getText()+"','D','MIHUT',"+parseNilai(this.e_nilai.getText())+",'"+this.app._userLog+"',getdate())");
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.param3 as jenis,a.no_dokumen,a.keterangan,a.nilai1 "+
		             "from trans_m a "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.form = 'MIHUT' and a.posted ='F'";						
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
