window.app_saku3_transaksi_travel_kug_fClosingBulan = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_travel_kug_fClosingBulan.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_travel_kug_fClosingBulan";
		...
		try {
			...
			if (this.maxPeriode.toString() == this.e_peraktif.getText().substr(4,2)) {
				this.bTampil.hide();
				system.alert(this,"Periode tidak valid.","Closing Akhir Tahun di KU47.");				
			}
			else {
				this.bTampil.show();
			}
...
window.app_saku3_transaksi_travel_kug_fClosingBulan.extend(window.childForm);
window.app_saku3_transaksi_travel_kug_fClosingBulan.implement({
	doClose: function() {
