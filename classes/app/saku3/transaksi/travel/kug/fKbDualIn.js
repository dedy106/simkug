window.app_saku3_transaksi_travel_kug_fKbDualIn = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_travel_kug_fKbDualIn.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_travel_kug_fKbDualIn";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Referensi : Penerimaan", 0);	
...
window.app_saku3_transaksi_travel_kug_fKbDualIn.extend(window.childForm);
window.app_saku3_transaksi_travel_kug_fKbDualIn.implement({	
	...
	simpan: function(){			
		try{						
			if (this.stsSimpan == 1) {				
				this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti","BM/"+this.e_periode.getText().substr(2,4)+"/","0000"));
			}
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','KB','KBDUALIN','F','-','-','"+this.cb_pp.getText()+"','"+this.dp_d1.getDateString()+"','-','"+this.e_ket.getText()+"','IDR',1,"+
							parseNilai(this.e_nilai.getText())+",0,0,'"+this.app._userLog+"','-','-','"+this.e_atensi.getText()+"','-','-','"+this.cb_jurnal.getText()+"','TUNAI','"+this.jenis+"')");
	...
	doChange:function(sender){
		try {
			if (sender == this.cb_kas && this.cb_kas.getText()!="") {
				this.akunDebet = this.cb_kas.getText();			
			}
			...
			if (sender == this.c_jenis && this.c_jenis.getText()!="") {
				...
				if (this.c_jenis.getText() == "PENERIMAAN") this.jenis = "BM";						
			}

			if (sender == this.cb_jurnal && this.cb_jurnal.getText()!="" && this.stsSimpan == 1) {				
				var data = this.dbLib.getDataProvider("select a.nama,a.akun_debet,a.akun_kredit,a.kode_pp "+
							"from trans_ref a "+							
				...
				if (typeof data == "object"){
					...
					if (line != undefined){										
						...
						this.cb_kas.setText(line.akun_debet);
					} 
				}
				this.e_ket.setFocus();															
			}
	...
	doCari:function(sender){																									
		...
		var strSQL = "select distinct a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.param1,a.keterangan,a.nilai1,a.tanggal "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='KB' and a.form = 'KBDUALIN' and a.posted ='F' "+filter+
					 "order by a.tanggal";
	...
	doLoad3:function(sender){																									
		var strSQL = "select distinct a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.param1,a.keterangan,a.nilai1,a.tanggal "+
		             "from trans_m a "+			
					 "inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.nik='"+this.app._userLog+"' "+ 					 
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='KB' and a.form = 'KBDUALIN' and a.posted ='F'  "+
					 "order by a.tanggal";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {		
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider("select a.kode_akun from trans_j a where a.dc = 'D' and a.no_bukti='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){																
				...
				var data = this.dbLib.getDataProvider("select a.kode_akun from trans_j a where a.dc = 'C' and a.no_bukti='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){																
