window.app_saku3_transaksi_travel_kug_fKbpln = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_travel_kug_fKbpln.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_travel_kug_fKbpln";
		...
		this.sg3 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:6,tag:9,
		            colTitle:["No Bukti","Tanggal","Status","No Dokumen","Deskripsi","Nilai"],
		...
		this.c_jenis = new saiCB(this.pc2.childPage[0],{bound:[20,17,200,20],caption:"Jenis",items:["BK","KK","BM","KM"], readOnly:true,tag:2,change:[this,"doChange"]});
		this.e_nb = new portalui_saiLabelEdit(this.pc2.childPage[0],{bound:[20,12,200,20],caption:"No Bukti",maxLength:30,readOnly:true});
		this.i_gen = new portalui_imageButton(this.pc2.childPage[0],{bound:[225,12,20,20],hint:"Generate",image:"icon/"+system.getThemes()+"/tabCont2.png",click:[this,"doClick"]});		
		this.c_status = new saiCB(this.pc2.childPage[0],{bound:[20,11,202,20],caption:"Status",items:["TUNAI","MCM","TRANSFER","CHEQUE","GIRO"], readOnly:true,tag:2});
		this.e_dok2 = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,14,450,20],caption:"No Dokumen",maxLength:50});				
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,12,990,280], childPage:["Data Item Jurnal"]});
...
window.app_saku3_transaksi_travel_kug_fKbpln.extend(window.childForm);
window.app_saku3_transaksi_travel_kug_fKbpln.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							...
							parseNilai(this.e_debet.getText())+",0,0,'"+this.app._userLog+"','-','-','-','-','-','-','"+this.c_status.getText()+"','"+this.c_jenis.getText()+"')");
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :						
				...
				if (this.c_jenis.getText().substr(0,1) == "B")
					var strsql = "select kode_akun from flag_relasi where kode_flag in ('009') and kode_lokasi='"+this.app._lokasi+"'";
				else {
					if (this.c_jenis.getText().substr(0,1) == "K") 
						var strsql = "select kode_akun from flag_relasi where kode_flag in ('001') and kode_lokasi='"+this.app._lokasi+"'";
				}
					...
					if (this.sg.rowValid(j)){
						for (var i=0;i<this.dataJU.rs.rows.length;i++){
							...
							if (line.kode_akun == this.sg.cells(0,j)) {
								akunKB = true;	
								if (this.c_jenis.getText().substr(1,1) == "K" && this.sg.cells(2,j) == "C") posisiDC = true;
								if (this.c_jenis.getText().substr(1,1) == "M" && this.sg.cells(2,j) == "D") posisiDC = true;
							}
						}
					}
				}
	...
	doChange:function(sender){
		if (sender == this.c_jenis && this.c_jenis.getText()!="") {				
			...
			if (this.c_jenis.getText().substr(0,1) == "B") var filterFlag = " and b.kode_flag in ('009','035') ";
			else var filterFlag = " and b.kode_flag in ('001','035') ";

			sql.add("select a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+filterFlag+" where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'");									
			...
			if (sender == this.sg) {
				if (col == 0){
					...
					this.standarLib.showListData(this, "Daftar Akun",sender,undefined, 
						    "select a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+filterFlag+" where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'",
							"select count(a.kode_akun) from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+filterFlag+" where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'",
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.param2 as status,a.no_dokumen,a.keterangan,a.nilai1 "+
		             "from trans_m a inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.nik='"+this.app._userLog+"'  "+					 					 
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_bukti,line.tgl,line.status,line.no_dokumen,line.keterangan,floatToNilai(line.nilai1)]); 
		}
