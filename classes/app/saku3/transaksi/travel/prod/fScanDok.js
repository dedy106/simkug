window.app_saku3_transaksi_travel_prod_fScanDok = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_travel_prod_fScanDok.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_travel_prod_fScanDok";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Scanning Dokumen", 0);	
				
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		
		this.cb_peserta = new saiCBBL(this,{bound:[20,11,220,20],caption:"No Jamaah", maxLength:20, multiSelection:false, tag:2,change:[this,"doLoad"]});								
		...
		this.sg = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:5,tag:9,				
					colTitle:["No Reg","Alamat","Paket","Jadwal","Pilih"],
					colWidth:[[4,3,2,1,0],[70,100,300,300,100]],
		...
		this.sgUpld = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,283],colCount:5, 
...
window.app_saku3_transaksi_travel_prod_fScanDok.extend(window.childForm);
window.app_saku3_transaksi_travel_prod_fScanDok.implement({
	...
	doLoad:function(sender){		
		if (this.cb_peserta.getText() != "") {			
			var strSQL = "select a.no_reg,b.alamat,c.nama as nama_paket,convert(varchar,d.tgl_berangkat,103) as tgl "+
						 ...
						 "where a.no_peserta='"+this.cb_peserta.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_reg desc";
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData([line.no_reg,line.alamat,line.nama_paket,line.tgl,"Pilih"]);
				}				
			}
