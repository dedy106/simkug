window.app_saku3_transaksi_travel_barang_fReturBeli = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_travel_barang_fReturBeli.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_travel_barang_fReturBeli";
		this.itemsValue = new portalui_arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Retur Pembelian", 0);	
		...
		this.cb_vendor = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,17,220,20],caption:"Vendor",multiSelection:false,tag:2});
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,12,990,281], childPage:["Data Barang Keluar","Item Jurnal"]});		
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:7,tag:9,
		            colTitle:["Kode","Nama Barang","Satuan","Stok","Jml Retur","Harga","Subtotal"],
					colWidth:[[6,5,4,3,2,1,0],[100,100,100,100,100,330,100]],					
...
window.app_saku3_transaksi_travel_barang_fReturBeli.extend(window.portalui_childForm);
window.app_saku3_transaksi_travel_barang_fReturBeli.implement({
		...
		if (this.cb_vendor.getText() != "") {
			...
			for (var i=0;i < this.sg.getRowCount();i++){
				if (this.sg.rowValid(i)) {
					nilai = nilaiToFloat(this.sg.cells(4,i)) * (nilaiToFloat(this.sg.cells(5,i))) ;
					var isAda = false;
					var idx = total = 0;

					this.sg2.appendData([this.akunHutang,"D","Hutang Vendor Pembelian",floatToNilai(nilai),this.app._kodePP,"HUTANG"]);
					...
					if (!isAda) {							
						this.sg2.appendData([akun,"C","Persediaan Barang",floatToNilai(nilai),this.app._kodePP,"RETURBRG"]);
					} 
					else { 
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{																																							
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','IV','BRGRETBELI','F','-','-','"+vKodePP+"','"+this.dp_d1.getDateString()+"','-','Retur Pembelian No: "+this.e_nb.getText()+"','IDR',1,"+parseNilai(this.e_total.getText())+",0,0,'-','-','-','-','-','-','"+this.cb_gudang.getText()+"','"+this.cb_vendor.getText()+"','-')");
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){		
								...
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg2.cells(0,i)+"','"+this.sg2.cells(1,i)+"',"+parseNilai(this.sg2.cells(3,i))+","+parseNilai(this.sg2.cells(3,i))+",'"+this.sg2.cells(2,i)+"','BRGRETBELI','"+this.sg2.cells(5,i)+"','IDR',1,'"+this.sg2.cells(4,i)+"','-','-','"+kodeVendor+"','-','-','-','-','-')");
							}
						}						
					}
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){																																										
								sql.add("insert into brg_trans_d (no_bukti,kode_lokasi,periode,modul,form,nu,kode_gudang,kode_barang,no_batch,tgl_ed,satuan,dc,stok,jumlah,bonus,harga,hpp,p_disk,diskon,tot_diskon,total) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','BRGRETBELI','BRGRETBELI',"+i+",'"+this.cb_gudang.getText()+"','"+this.sg.cells(0,i)+"','-',getdate(),'"+this.sg.cells(2,i)+"','C',"+nilaiToFloat(this.sg.cells(3,i))+","+
										nilaiToFloat(this.sg.cells(4,i))+",0,"+nilaiToFloat(this.sg.cells(5,i))+",0,0,0,0,0)");								
							}
						}						
					}
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk) {
					...
					this.sg.clear(1);  this.sg3.clear(1); this.sg3.clear(1); 
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "") {
			...
			this.stsSimpan = 1;
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti","RB/"+this.e_periode.getText().substr(2,4)+"/","0000"));			
			this.e_dok.setFocus();
		...
		try{
			if ( col == 0 && this.sg.cells(0,row)!="" &&  this.cb_gudang.getText()!="") {												
				if (this.stsSimpan == 1) {
					...
					var strSQL = "select a.nama,a.sat_kecil,b.stok+isnull(c.jumlah+c.bonus,0) as stok "+
								 "from brg_barang a inner join brg_stok b on a.kode_barang=b.kode_barang and a.kode_lokasi=b.kode_lokasi and b.kode_gudang='"+this.cb_gudang.getText()+"' and b.nik_user='"+this.app._userLog+"' "+
								 "                left join brg_tukar_d c on b.kode_barang = c.kode_barang and b.kode_lokasi=c.kode_lokasi and c.no_tukar='"+this.e_nb.getText()+"' "+
								 "where a.kode_barang='"+this.sg.cells(0,row)+"' and a.flag_aktif='1' and a.kode_lokasi='"+this.app._lokasi+"' ";
				}			
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
			...
			if (col == 4 || col == 5) {
				if (this.sg.cells(4, row) != "" && this.sg.cells(5, row) != "") {
					this.sg.cells(6, row, Math.round(nilaiToFloat(this.sg.cells(5, row)) * nilaiToFloat(this.sg.cells(4, row))));
				}
			}
			this.sg.validasi();	
		} catch(e) {
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai1 "+
					 "from trans_m a "+
				     "inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.nik='"+this.app._userLog+"' "+				 					 			 					 				 					 
					 "where a.modul='IV' and a.form='BRGRETBELI' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.posted ='F' "
					 "order by a.no_bukti";			
		var data = this.dbLib.getDataProvider(strSQL,true);
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.kode_barang,b.nama,a.satuan,a.jumlah,b.pabrik,c.stok+a.jumlah as stok,a.harga,a.jumlah*a.harga as total "+
				...
				if (typeof data1 == "object" && data1.rs.rows[0] != undefined){
					...
					for (var i in data1.rs.rows){
						...
						this.sg.appendData([line1.kode_barang,line1.nama,line1.satuan,parseFloat(line1.stok),parseFloat(line1.jumlah),parseFloat(line1.harga),parseFloat(line1.total)]);
					}
