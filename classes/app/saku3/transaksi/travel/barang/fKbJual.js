window.app_saku3_transaksi_travel_barang_fKbJual = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_travel_barang_fKbJual.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_travel_barang_fKbJual";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pelunasan Piutang Penjualan", 0);	
		...
		this.c_jenis = new saiCB(this.pc2.childPage[0],{bound:[20,17,200,20],caption:"Jenis",items:["BM","MI"], readOnly:true,tag:2,change:[this,"doChange"]});	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,20,990,281], childPage:["Data Penjualan","Jurnal Sisa"]});		
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-33],colCount:7,tag:0,
				colTitle:["Status","No Referensi","Tanggal","Keterangan","Kode PP","Akun Piutang","Tagihan"],
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:9,tag:9,
		            colTitle:["Kode Akun","Nama Akun","DC","Keterangan","Nilai","Kode PP","Nama PP","Kode Cust","Nama Cust"],
...
window.app_saku3_transaksi_travel_barang_fKbJual.extend(window.childForm);
window.app_saku3_transaksi_travel_barang_fKbJual.implement({
	doLoad : function() {
		this.pc2.setActivePage(this.pc2.childPage[0]);	
		this.pc1.setActivePage(this.pc1.childPage[0]);																		
		if (this.stsSimpan==1 && this.cb_cust.getText()!="") {				
			var strSQL = "select a.no_jual,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.kode_pp,a.akun_piutang,a.nilai-isnull(b.bayar,0) as saldo  "+						  
						 "from brg_jualpiu_d a "+

						 "     left join ("+
						 "          select no_jual,sum(case dc when 'D' then nilai else -nilai end) as bayar  "+
						 "          from brg_jualbayar_d where kode_cust='"+this.cb_cust.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
						 "			group by no_jual "+
						 ") b on a.no_jual=b.no_jual "+
						 
						 "where a.kode_cust='"+this.cb_cust.getText()+"' and (a.nilai-isnull(b.bayar,0)) > 0 and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_jual";						 
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg1.appendData(["INPROG",line.no_jual,line.tgl,line.keterangan,line.kode_pp,line.akun_piutang,floatToNilai(line.saldo)]);					
				}
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.modul+"','KBJUAL','F','-','-','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','-','"+this.e_ket.getText()+"','IDR',1,"+
							parseNilai(this.e_total.getText())+",0,0,'-','-','-','-','-','-','"+this.cb_akun.getText()+"','"+this.cb_cust.getText()+"','"+this.c_jenis.getText()+"')");
					
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',999,'"+this.cb_akun.getText()+"','D',"+nilaiToFloat(this.e_total.getText())+","+nilaiToFloat(this.e_total.getText())+",'"+this.e_ket.getText()+"','"+
							this.modul+"','KB','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");
					
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && this.sg1.cells(0,i) == "BAYAR"){
							sql.add("insert into brg_jualbayar_d(no_bukti,no_jual,kode_cust,kode_lokasi,modul,periode,nik_user,tgl_input,dc,nilai) values "+
									"('"+this.e_nb.getText()+"','"+this.sg1.cells(1,i)+"','"+this.cb_cust.getText()+"','"+this.app._lokasi+"','KBJUAL','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'D',"+nilaiToFloat(this.sg1.cells(6,i))+")");					

							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(5,i)+"','C',"+nilaiToFloat(this.sg1.cells(6,i))+","+
									nilaiToFloat(this.sg1.cells(6,i))+",'Pembayaran atas "+this.cb_cust.rightLabelCaption+" No Ref "+this.sg1.cells(1,i)+"','"+this.modul+"','PIUTANG','IDR',1,'"+this.app._kodePP+"','-','"+this.cb_cust.getText()+"','-','-','"+this.sg1.cells(1,i)+"','-','-','-')");
						}
					}
					...
					if (this.sg.getRowValidCount() > 0) {
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								...
								//create piutang baru (sisa yg dipiutangkan/masuk ke akun piutang cust--> like f-44 SAP)
								if (this.sg.cells(7,i) == "-") {									
									sql.add("insert into brg_jualpiu_d(no_jual,kode_lokasi,tanggal,keterangan,kode_cust,kode_curr,kurs,kode_pp,nilai,periode,nik_user,tgl_input,akun_piutang,nilai_ppn,nilai_pph,no_fp,diskon,kode_gudang) values  "+
											"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(7,i)+"','IDR',1,'"+this.sg.cells(5,i)+"',"+nilaiToFloat(this.sg.cells(4,i))+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.sg.cells(0,i)+"',0,0,'-',0,'-')");										
								}
							}
						}
					}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :														
				this.preView = "1";
				this.sg.validasi();								
				var data = this.dbLib.getDataProvider("select kode_akun from flag_relasi where kode_flag = '003' and kode_lokasi='"+this.app._lokasi+"'",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					this.dataAkunPiu = data;
				}				
				for (var i=0;i < this.sg.getRowCount();i++) {
					for (var j=0;j<this.dataAkunPiu.rs.rows.length;j++) {
						var line = this.dataAkunPiu.rs.rows[j];
						if (line.kode_akun == this.sg.cells(0,i)) {		
							if (this.sg.cells(7,i) == "-") {
								var k =i+1;
								system.alert(this,"Transaksi tidak valid.","Cust tidak terdaftar. [Baris : "+k+"]");
								return false;						
							}							
						}
					}
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "" && this.c_jenis.getText()!= "") {
			...
			this.stsSimpan = 1;			
			this.cb_cust.setSQL("select kode_cust, nama from cust where kode_lokasi='"+this.app._lokasi+"'",["kode_cust","nama"],false,["Kode","Nama"],"and","Data Cust",true);
	...
	doChangeCell: function(sender, col, row){
		...
		if (col == 7) {
			if (sender.cells(7,row) != "") {
				var data = this.dbLib.getDataProvider("select kode_akun from flag_relasi where kode_flag='003' and kode_akun='"+sender.cells(0,row)+"' and kode_lokasi='"+this.app._lokasi+"'",true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined) {						
						var data = this.dbLib.getDataProvider("select nama from cust where kode_cust='"+sender.cells(7,row)+"' and kode_lokasi='"+this.app._lokasi+"'",true);
						if (typeof data == "object"){
							var line = data.rs.rows[0];							
							if (line != undefined) {
								...
								system.alert(this,"Kode Cust "+sender.cells(7,row)+" tidak ditemukan","Inputkan kode lainnya.","");                
								sender.cells(7,row,"");
								sender.cells(8,row,"");
							}
						}
						...
						if (sender.cells(7,row) == "-")	sender.cells(8,row,"-");
							...
							system.alert(this,"Kode Cust "+sender.cells(7,row)+" tidak diperkenankan","Inputkan kode '-'.","");                
							sender.cells(7,row,"-");
							sender.cells(8,row,"-");
						}
					}
			...
			if (sender == this.sg) {
				...
				if (col == 7 && sender.cells(0,row) != "") {					
					...
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
						if (line != undefined) {
							var strSQL1 = "select kode_cust, nama from cust where kode_lokasi = '"+this.app._lokasi+"'";
							var strSQL2 = "select count(*) from cust where kode_lokasi = '"+this.app._lokasi+"'";
						} 
						else {
							var strSQL1 = "select '-' as kode_cust, '-' as nama ";
							var strSQL2 = "select count(*) from cust a where kode_cust='-'";
						}
					}										
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai1 "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.form = 'KBJUAL' and a.posted ='F' order by a.no_bukti";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						this.cb_cust.setSQL("select kode_cust, nama from cust where kode_cust='"+line.param2+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_cust","nama"],false,["Kode","Nama"],"and","Data Customer",true);				
						this.cb_cust.setText(line.param2);										
					}
				}				
				...
				var strSQL = "select a.no_jual,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.kode_pp,a.akun_piutang,a.nilai-isnull(b.bayar,0) as saldo  "+						  
							 "from brg_jualpiu_d a "+
							 "     inner join brg_jualbayar_d c on a.no_jual=c.no_jual and c.kode_lokasi='"+this.app._lokasi+"' "+
							 "     left join ("+
							 "          select no_jual,sum(case dc when 'C' then nilai else -nilai end) as bayar  "+
							 "          from brg_jualbayar_d where no_bukti <> '"+this.e_nb.getText()+"' and kode_cust='"+this.cb_cust.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
							 "			group by no_jual "+
							 ") b on a.no_jual=b.no_jual "+
							
							 "where a.kode_lokasi='"+this.app._lokasi+"' and c.no_bukti='"+this.e_nb.getText()+"' order by a.no_jual";						 
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData(["BAYAR",line.no_jual,line.tgl,line.keterangan,line.kode_pp,line.akun_piutang,floatToNilai(line.saldo)]);					
					}
				} else this.sg1.clear(1);
				
				var strSQL = "select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_cust,isnull(d.nama,'-') as nama_cust "+
							 "from trans_j a "+
							 "inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+							 
							 "inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+							 
							 "left join cust d on a.kode_cust=d.kode_cust and a.kode_lokasi=d.kode_lokasi "+							 
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];		
						this.sg.appendData([line.kode_akun,line.nama_akun,line.dc,line.keterangan,floatToNilai(line.nilai),line.kode_pp,line.nama_pp,line.kode_cust,line.nama_cust]);						
					}
				}
				this.sg.validasi();	
