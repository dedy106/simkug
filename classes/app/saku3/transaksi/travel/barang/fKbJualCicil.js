window.app_saku3_transaksi_travel_barang_fKbJualCicil = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_travel_barang_fKbJualCicil.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_travel_barang_fKbJualCicil";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pelunasan Piutang Penjualan Cicilan", 0);	
		...
		this.e_total = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,19,200,20],caption:"Total Pelunasan", tag:1, tipeText:ttNilai, text:"0",readOnly:true});			

		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,20,990,281], childPage:["Data Penjualan"]});		
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-33],colCount:7,tag:0,
				colTitle:["No Referensi","Tanggal","Keterangan","Kode PP","Akun Piutang","Tagihan","Pelunasan"],
...
window.app_saku3_transaksi_travel_barang_fKbJualCicil.extend(window.childForm);
window.app_saku3_transaksi_travel_barang_fKbJualCicil.implement({
	doLoad : function() {
		...
		if (this.stsSimpan==1 && this.cb_cust.getText()!="") {				
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg1.appendData([line.no_jual,line.tgl,line.keterangan,line.kode_pp,line.akun_piutang,floatToNilai(line.saldo),"0"]);					
				}
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.modul+"','KBJUALCCL','F','-','-','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','-','"+this.e_ket.getText()+"','IDR',1,"+
					...
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && nilaiToFloat(this.sg1.cells(6,i)) > 0){
							sql.add("insert into brg_jualbayar_d(no_bukti,no_jual,kode_cust,kode_lokasi,modul,periode,nik_user,tgl_input,dc,nilai) values "+
									"('"+this.e_nb.getText()+"','"+this.sg1.cells(0,i)+"','"+this.cb_cust.getText()+"','"+this.app._lokasi+"','KBJUAL','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'D',"+nilaiToFloat(this.sg1.cells(6,i))+")");					

							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(4,i)+"','C',"+nilaiToFloat(this.sg1.cells(6,i))+","+
									nilaiToFloat(this.sg1.cells(6,i))+",'Pembayaran atas "+this.cb_cust.rightLabelCaption+" No Ref "+this.sg1.cells(0,i)+"','"+this.modul+"','PIUTANG','IDR',1,'"+this.sg1.cells(3,i)+"','-','"+this.cb_cust.getText()+"','-','-','"+this.sg1.cells(0,i)+"','-','-','-')");
						}
					}
		...
		switch (event){
			...
			case "ubah" :														
				...
				if (nilaiToFloat(this.e_saldo.getText()) < nilaiToFloat(this.e_total.getText())) {
					system.alert(this,"Transaksi tidak valid.","Total Pelunasan melebihi Saldo Tagihan.");
					return false;						
				}
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai1 "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.form = 'KBJUALCCL' and a.posted ='F' order by a.no_bukti";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.no_jual,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.kode_pp,a.akun_piutang,a.nilai-isnull(b.bayar,0) as saldo,c.nilai as nilai_bayar "+						  
							 "from brg_jualpiu_d a "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData([line.no_jual,line.tgl,line.keterangan,line.kode_pp,line.akun_piutang,floatToNilai(line.saldo),floatToNilai(line.nilai_bayar)]);					
					}
				} else this.sg1.clear(1);
