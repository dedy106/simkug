window.app_saku3_transaksi_travel_barang_fKirim = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_travel_barang_fKirim.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_travel_barang_fKirim";
		this.itemsValue = new portalui_arrayList();
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Mutasi Kirim Barang", 0);	
		...
		this.cb_asal = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,16,220,20],caption:"Gudang Asal",multiSelection:false,tag:2,change:[this,"doChange"]});
		this.cb_tujuan = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,17,220,20],caption:"Gudang Tujuan",multiSelection:false,tag:1});
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:5,tag:0,
		            colTitle:["Kode","Nama","Satuan","Stok","Jumlah"],					
					colWidth:[[4,3,2,1,0],[100,100,100,400,100]],					
		...
		try{			
			...
			this.cb_asal.setSQL("select a.kode_gudang, a.nama from brg_gudang a "+
			...
			if (typeof data == "object"){
				...
				this.cb_asal.setText(line.kode_gudang); 										
			}
...
window.app_saku3_transaksi_travel_barang_fKirim.extend(window.portalui_childForm);
window.app_saku3_transaksi_travel_barang_fKirim.implement({
		...
		try{					
			...
			for (var i=0;i < this.sg.getRowCount();i++){
				if (this.sg.rowValid(i)){
					if (this.stsSimpan == 1)  {
						var data = this.dbLib.getDataProvider("select stok from brg_stok where kode_barang='"+this.sg.cells(1,i)+"' and kode_gudang='"+this.cb_asal.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and nik_user='"+this.nik_user+"'",true);
					}
					else {
						var strSQL = "select a.stok+isnull(b.jumlah,0) as stok "+
								 	 "from brg_stok a left join brg_trans_d b on a.kode_barang = b.kode_barang and a.kode_lokasi=b.kode_lokasi and b.no_bukti='"+this.e_nb.getText()+"' and b.dc='C' and b.modul='BRGKIRIM' "+
									 "where a.kode_barang='"+this.sg.cells(0,i)+"' and a.kode_gudang='"+this.cb_asal.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.nik_user='"+this.nik_user+"'";
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{																																							
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','IV','BRGKIRIM','X','0','0','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','IDR',1,0,0,0,'-','-','-','-','-','-','"+this.cb_asal.getText()+"','"+this.cb_tujuan.getText()+"','-')");
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){																																																		
								sql.add("insert into brg_trans_d (no_bukti,kode_lokasi,periode,modul,form,nu,kode_gudang,kode_barang,no_batch,tgl_ed,satuan,dc,stok,jumlah,bonus,harga,hpp,p_disk,diskon,tot_diskon,total) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','BRGKIRIM','BRGKIRIM',"+i+",'"+this.cb_asal.getText()+"','"+this.sg.cells(0,i)+"','-',getdate(),'"+this.sg.cells(2,i)+"','C',"+nilaiToFloat(this.sg.cells(3,i))+","+
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "") {
			...
			this.stsSimpan = 1;
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti","MK/"+this.e_periode.getText().substr(2,4)+"/","0000"));						
		...
		try{			
			if (col == 0 && this.cb_asal.getText()!=""){
				this.standarLib.showListData(this, "Daftar Item Barang",sender,undefined, 
											  "select kode_barang,nama from brg_barang where kode_lokasi='"+this.app._lokasi+"'",
											  "select count(kode_barang) from brg_barang where kode_lokasi='"+this.app._lokasi+"'",
		...
		if ((col == 0) && this.sg.cells(0,row)!="" ) {												
			if (this.stsSimpan == 1) {
				var strSQL = "select a.nama,a.sat_kecil,b.stok "+
							 "from brg_barang a inner join brg_stok b on a.kode_barang=b.kode_barang and a.kode_lokasi=b.kode_lokasi and b.kode_gudang='"+this.cb_asal.getText()+"' and b.nik_user='"+this.app._userLog+"' "+
							 "where a.kode_barang='"+this.sg.cells(0,row)+"' and a.kode_lokasi='"+this.app._lokasi+"'";						 			
			}
			else {
				var strSQL = "select a.nama,a.sat_kecil,b.stok+isnull(c.jumlah,0) as stok "+
							 "from brg_barang a inner join brg_stok b on a.kode_barang=b.kode_barang and a.kode_lokasi=b.kode_lokasi and b.kode_gudang='"+this.cb_asal.getText()+"' and b.nik_user='"+this.app._userLog+"' "+
							 "                left join brg_trans_d c on b.kode_barang = c.kode_barang and b.kode_lokasi=c.kode_lokasi and c.no_bukti='"+this.e_nb.getText()+"' and c.dc='C' and c.modul='BRGKIRIM' "+
							 "where a.kode_barang='"+this.sg.cells(0,row)+"' and a.kode_lokasi='"+this.app._lokasi+"'";
			}			
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object"){
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_asal && this.cb_asal.getText()!="") {
			this.cb_tujuan.setSQL("select kode_gudang, nama from brg_gudang where kode_gudang <> '"+this.cb_asal.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'",["kode_gudang","nama"],false,["Kode","Nama"],"and","Data Gudang Tujuan",true);
		}		
	},		
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan "+
		             "from trans_m a "+	
					 "inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.nik='"+this.app._userLog+"' "+				 					 
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and no_ref1='-' and form='BRGKIRIM'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						this.cb_asal.setSQL("select kode_gudang, nama from brg_gudang where kode_gudang='"+line.param1+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_gudang","nama"],false,["Kode","Nama"],"and","Data Gudang Asal",true);									
						this.cb_tujuan.setSQL("select kode_gudang, nama from brg_gudang where kode_gudang='"+line.param2+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_gudang","nama"],false,["Kode","Nama"],"and","Data Gudang Tujuan",true);														
						this.cb_asal.setText(line.param1);						
						this.cb_tujuan.setText(line.param2);												
					}
				}												
				...
				var strSQL = "select a.kode_barang,b.nama,a.satuan,a.jumlah,c.stok+a.jumlah as stok "+
