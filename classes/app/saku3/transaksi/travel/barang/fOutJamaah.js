window.app_saku3_transaksi_travel_barang_fOutJamaah = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_travel_barang_fOutJamaah.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_travel_barang_fOutJamaah";
		this.itemsValue = new portalui_arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Keluar Barang Jamaah", 0);	
		...
		this.cb_regis = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,15,220,20],caption:"No Registrasi",multiSelection:false,tag:2});
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,12,990,280], childPage:["Data Barang Keluar","Data Barang Masuk"]});		
...
window.app_saku3_transaksi_travel_barang_fOutJamaah.extend(window.portalui_childForm);
window.app_saku3_transaksi_travel_barang_fOutJamaah.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{																																							
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from dgw_barang_d where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}										
					
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','IV','IOJAMAAH','X','0','0','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','IDR',1,0,0,0,'-','-','-','-','-','-','"+this.cb_gudang.getText()+"','"+this.cb_regis.getText()+"','-')");
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){	
								sql.add("insert into brg_trans_d (no_bukti,kode_lokasi,periode,modul,form,nu,kode_gudang,kode_barang,no_batch,tgl_ed,satuan,dc,stok,jumlah,bonus,harga,hpp,p_disk,diskon,tot_diskon,total) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','BRGIO','IOJAMAAH',"+i+",'"+this.cb_gudang.getText()+"','"+this.sg.cells(0,i)+"','-',getdate(),'"+this.sg.cells(2,i)+"','C',"+nilaiToFloat(this.sg.cells(3,i))+","+
										nilaiToFloat(this.sg.cells(4,i))+",0,0,0,0,0,0,0)");

								//histori utk jamaah		
								sql.add("insert into dgw_barang_d (no_bukti,kode_lokasi,no_reg,kode_gudang,kode_barang,satuan,dc,jumlah) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_regis.getText()+"','"+this.cb_gudang.getText()+"','"+this.sg.cells(0,i)+"','"+this.sg.cells(2,i)+"','C',"+nilaiToFloat(this.sg.cells(4,i))+")");										
							}
						}						
					}
					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){	
								var j = i+1000;																																																	
								sql.add("insert into brg_trans_d (no_bukti,kode_lokasi,periode,modul,form,nu,kode_gudang,kode_barang,no_batch,tgl_ed,satuan,dc,stok,jumlah,bonus,harga,hpp,p_disk,diskon,tot_diskon,total) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','BRGIO','IOJAMAAH',"+j+",'"+this.cb_gudang.getText()+"','"+this.sg1.cells(0,i)+"','-',getdate(),'"+this.sg1.cells(2,i)+"','D',0,"+
										nilaiToFloat(this.sg1.cells(3,i))+",0,0,0,0,0,0,0)");

								//histori utk jamaah (barang masuk jika ada penukaran)		
								sql.add("insert into dgw_barang_d (no_bukti,kode_lokasi,no_reg,kode_gudang,kode_barang,satuan,dc,jumlah) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_regis.getText()+"','"+this.cb_gudang.getText()+"','"+this.sg1.cells(0,i)+"','"+this.sg1.cells(2,i)+"','D',"+nilaiToFloat(this.sg1.cells(3,i))+")");										
							}
						}						
					}		
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "") {
			...
			this.stsSimpan = 1;
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti","IOJ/"+this.e_periode.getText().substr(2,4)+"/","0000"));			
			this.e_dok.setFocus();
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan "+
		             "from trans_m a "+	
					 "inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.nik='"+this.app._userLog+"' "+				 					 			 					 				 					 
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.form='IOJAMAAH' ";		
		var data = this.dbLib.getDataProvider(strSQL,true);
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						this.cb_regis.setSQL("select a.no_reg, b.nama "+
								 "from dgw_reg a inner join dgw_peserta b on a.no_peserta=b.no_peserta and a.kode_lokasi=b.kode_lokasi "+
								 "where a.kode_lokasi='"+this.app._lokasi+"' and a.no_reg='"+line.param2+"' ",
								 ["a.no_reg","a.nama"],false,["No Registrasi","Nama"],"and","Data Registrasi",true);
						this.cb_regis.setText(line.param2);												
					}
				}												
