window.app_saku3_transaksi_siaga_fAppRRADirnik = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fAppRRADirnik.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fAppRRADirnik";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Approval RRA Direksi", 0);	
		...
		this.sg2 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,320],colCount:5,
...
window.app_saku3_transaksi_siaga_fAppRRADirnik.extend(window.childForm);
window.app_saku3_transaksi_siaga_fAppRRADirnik.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{				
					...
					sql.add("insert into rra_app_m(no_app, kode_lokasi,tanggal,keterangan,modul,periode,no_del,nik_buat,nik_app,nik_user,tgl_input,jenis_form) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','PUSAT','"+this.e_periode.getText()+"','-','"+this.app._userLog+"','"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'APPDIR')");
					sql.add("insert into rra_app_d(no_app,modul,kode_lokasi,no_bukti,kode_lokbukti,sts_pdrk,catatan,status) values "+
							"('"+this.e_nb.getText()+"','PUSAT','"+this.app._lokasi+"','"+this.e_nopdrk.getText()+"','"+this.app._lokasi+"','RRD','-','"+this.c_status.getText()+"')");
	...
	doClick:function(sender){
		if (sender == this.i_gen) {
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"rra_app_m","no_app",this.app._lokasi+"-RRD"+this.e_periode.getText().substr(2,4)+".","0000"));
			this.e_ket.setFocus();
		}		
	},	
	doLoad:function(sender){		
		var strSQL = "select a.kode_lokasi,a.kode_pp+' - '+c.nama as pp,a.no_pdrk as no_bukti,b.no_dokumen,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,a.sts_pdrk as jenis "+
								...
								"where a.modul = 'MULTI' and a.progress='1' and a.periode<='"+this.e_periode.getText()+"' and a.nik_app4='"+this.app._userLog+"'";							

					 //approve2 ==9 dibatalin "where a.modul = 'MULTI' and a.progress='9' and a.periode<='"+this.e_periode.getText()+"' ";							
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){														
							...
							this.filter2 = this.app._lokasi+"/"+this.app._userLog+"/"+this.e_nb.getText()+"/"+this.c_status.getText()+"/Approval Direksi";
							this.viewer.prepare();
