window.app_saku3_transaksi_siaga_fa_fFasusutRev = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fa_fFasusutRev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fa_fFasusutRev";
		...
		this.cb_fa = new saiCBBL(this,{bound:[20,18,220,20],caption:"No Aktiva Tetap", multiSelection:false, maxLength:10, tag:2, change:[this,"doChange"]});		
		this.e_seri = new portalui_saiLabelEdit(this,{bound:[20,19,432,20],caption:"Nomor Seri",readOnly:true});
		this.e_tipe = new portalui_saiLabelEdit(this,{bound:[500,19,502,20],caption:"Tipe",readOnly:true});
		this.e_merk = new portalui_saiLabelEdit(this,{bound:[20,17,432,20],caption:"Merk",readOnly:true});
		this.e_pp = new portalui_saiLabelEdit(this,{bound:[500,17,502,20],caption:"PP",readOnly:true});
		...
		this.e_bp = new portalui_saiLabelEdit(this,{bound:[500,18,502,20],caption:"Akun Beban",readOnly:true, tag:3});
		...
		this.e_ap = new portalui_saiLabelEdit(this,{bound:[500,20,502,20],caption:"Akun Akumulasi", tag:3, readOnly:true});
		...
		try {
			...
			this.cb_fa.setSQL("select a.no_fa, a.nama from fa_asset a "+
			        ...
			        "                where b.tot_susut>0 and a.jenis='A' and a.progress ='2' and a.kode_lokasi='"+this.app._lokasi+"'",["a.no_fa","a.nama"],false,["No Aktap","Nama"],"and","Daftar Aktiva Tetap",true);
...
window.app_saku3_transaksi_siaga_fa_fFasusutRev.extend(window.childForm);
window.app_saku3_transaksi_siaga_fa_fFasusutRev.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,3])){
				try{													
					...
					sql.add("insert into fasusut_m(no_fasusut,no_dokumen,tanggal,keterangan,kode_curr,kurs,nilai,kode_pp,kode_drk,posted,modul,nik_buat,nik_setuju,kode_lokasi,periode,no_del,no_link,nik_user,tgl_input) values "+
						    "('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'-','-','F','FAREV','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','-','-','"+this.app._userLog+"',getdate())");					
					...
					sql.add("insert into fasusut_j(no_fasusut,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.akunap+"','"+this.e_ket.getText()+"','D',"+parseNilai(this.e_nilai.getText())+",'"+this.pp+"','-','"+this.app._lokasi+"','FAREV','AP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");
					sql.add("insert into fasusut_j(no_fasusut,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',1,'"+this.akunbp+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_nilai.getText())+",'"+this.pp+"','-','"+this.app._lokasi+"','FAREV','BP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");
	...
	doChange:function(sender){
		...
		if (sender == this.cb_fa && this.cb_fa.getText()!="") {
			var strSQL = "select a.no_seri,a.merk,a.tipe,zz.nilai,a.nilai_residu,isnull(d.tot_susut,0) as tot_susut,(zz.nilai-a.nilai_residu-isnull(d.tot_susut,0)) as nilai_buku, "+
						 "  ceiling(zz.nilai/a.umur) as nilai_susut, "+
