window.app_saku3_transaksi_siaga_fa_fFaEditM = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fa_fFaEditM.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fa_fFaEditM";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Edit Data Aktiva Tetap [Migrasi]", 0);	
		...
		this.dp_d2 = new portalui_datePicker(this,{bound:[120,11,98,18]}); 
...
window.app_saku3_transaksi_siaga_fa_fFaEditM.extend(window.childForm);
window.app_saku3_transaksi_siaga_fa_fFaEditM.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("update fa_asset set kode_lokfa='"+this.cb_lokasi.getText()+"',nik_pnj='"+this.cb_buat.getText()+"',kode_klpfa='"+this.cb_klp.getText()+"',kode_klpakun='"+this.cb_klpakun.getText()+"',kode_akun='"+this.kodeakun+"',umur="+parseNilai(this.e_umur.getText())+",persen="+parseNilai(this.e_persen.getText())+",nama='"+this.e_nama.getText()+"',merk='"+this.e_merk.getText()+"',tipe='"+this.e_tipe.getText()+"',no_seri='"+this.e_seri.getText()+"',nilai="+parseNilai(this.e_nilai.getText())+",nilai_residu="+parseNilai(this.e_residu.getText())+",kode_pp='"+this.cb_pp1.getText()+"',kode_pp_susut='"+this.cb_pp2.getText()+"',tgl_perolehan='"+this.dp_d2.getDateString()+"',tgl_susut='"+this.dp_d3.getDateString()+"',periode='"+periode+"',periode_susut='"+periodeSusut+"',nik_user='"+this.app._userLog+"',tgl_input=getdate() "+
							"where no_fa='"+this.cb_fa.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("update fa_nilai set nilai="+nilaiToFloat(this.e_nilai.getText())+" where no_fa='"+this.cb_fa.getText()+"' and no_bukti='"+this.cb_fa.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");		
	...
	doChange:function(sender){	
		if (sender == this.cb_fa && this.cb_fa.getText() != ""){
			...
			var strSQL = "select a.nama,a.no_seri,a.merk,a.tipe,a.nilai,a.nilai_residu,isnull(d.tot_susut,0) as tot_susut,(a.nilai-a.nilai_residu-isnull(d.tot_susut,0)) as nilai_buku,convert(varchar,a.tgl_perolehan,103) as tgl_perolehan,convert(varchar,a.tgl_susut,103) as tgl_susut,a.kode_klpakun,a.kode_klpfa, "+
						 ...
						 "where a.no_fa='"+this.cb_fa.getText()+"' and a.progress in ('1') and a.kode_lokasi='"+this.app._lokasi+"'";
		...
		try{
			if (sender == this.cb_fa) {   
			    this.standarLib.showListData(this, "Daftar Aktiva Tetap",sender,undefined, 
											  "select no_fa, nama  from fa_asset where progress in ('1') and kode_lokasi='"+this.app._lokasi+"' and no_fa=no_baps",
											  "select count(no_fa) from fa_asset where progress in ('1') and kode_lokasi='"+this.app._lokasi+"' and no_fa=no_baps",
