window.app_saku3_transaksi_siaga_fa_fFasusutCepat = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fa_fFasusutCepat.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fa_fFasusutCepat";
		...
		this.e_bp = new portalui_saiLabelEdit(this,{bound:[500,18,500,20],caption:"Akun Beban",readOnly:true, tag:3});
...
window.app_saku3_transaksi_siaga_fa_fFasusutCepat.extend(window.childForm);
window.app_saku3_transaksi_siaga_fa_fFasusutCepat.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,3])){
				try{													
					...
					sql.add("insert into fasusut_m(no_fasusut,no_dokumen,tanggal,keterangan,kode_curr,kurs,nilai,kode_pp,kode_drk,posted,modul,nik_buat,nik_setuju,kode_lokasi,periode,no_del,no_link,nik_user,tgl_input) values "+
						    "('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'-','-','F','FACPT','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','-','-','"+this.app._userLog+"',getdate())");					
					...
					sql.add("insert into fasusut_j(no_fasusut,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.akunbp+"','"+this.e_ket.getText()+"','D',"+parseNilai(this.e_nilai.getText())+",'"+this.pp+"','-','"+this.app._lokasi+"','FACPT','BP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");
					sql.add("insert into fasusut_j(no_fasusut,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',1,'"+this.akunap+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_nilai.getText())+",'"+this.pp+"','-','"+this.app._lokasi+"','FACPT','AP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");
					...
					sql.add("insert into fasusut_d(no_fasusut,no_fa,periode,nilai,kode_lokasi,akun_bp,akun_ap,kode_akun,kode_pp,kode_drk,dc,no_del,nilai_aset,umur) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_fa.getText()+"','"+this.e_periode.getText()+"',"+parseNilai(this.e_nilai.getText())+",'"+this.app._lokasi+"','"+this.akunbp+"','"+this.akunap+"','"+this.kodeakun+"','"+this.pp+"','-','D','-',"+parseNilai(this.e_hp.getText())+","+this.umur+")");
	...
	doChange:function(sender){
		...
		if (sender == this.cb_fa && this.cb_fa.getText()!="") {
			var strSQL = "select a.no_seri,a.merk,a.tipe,zz.nilai,a.nilai_residu,isnull(d.tot_susut,0) as tot_susut,(zz.nilai-a.nilai_residu-isnull(d.tot_susut,0)) as nilai_buku, "+
						 ...
						 "	  			from fasusut_d "+
						 "	  			where no_fa='"+this.cb_fa.getText()+"' group by no_fa,kode_lokasi "+
		...
		if (sender == this.i_gen) {
			if (this.e_periode.getText()!= "" ) {				
				...
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){
						if (line.no_fasusut == "0") this.e_nb.setText("FASC001"+AddFormat);
						else {
							...
							this.e_nb.setText("FASC"+nu+AddFormat);
						}
					} 
				}
				this.e_ket.setFocus();
