window.app_saku3_transaksi_siaga_fa_fFasusut = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fa_fFasusut.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fa_fFasusut";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penyusutan Aktiva Tetap : Generate", 0);	
		...
		this.e_nilai = new saiLabelEdit(this,{bound:[820,17,200,20],caption:"Total Penyusutan", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		this.bTampil = new portalui_button(this,{bound:[700,17,80,18],caption:"Tampil Data",click:[this,"doLoadData"]});		
	
		this.p1 = new panel(this,{bound:[20,23,1000,350],caption:"Item Jurnal"});
		this.sg = new saiGrid(this.p1,{bound:[1,20,this.p1.width-5,this.p1.height-51],colCount:7,tag:9,
		            colTitle:["Akun BP","Nama Akun","Akun Deprs","Nama Akun","Kode PP","Nama PP","Nilai"],
					colWidth:[[6,5,4,3,2,1,0],[100,150,80,200,100,200,100]],
...
window.app_saku3_transaksi_siaga_fa_fFasusut.extend(window.childForm);
window.app_saku3_transaksi_siaga_fa_fFasusut.implement({
				...
				try{													
					...
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						...
						sql.add("insert into fasusut_j(no_fasusut,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
								"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+line.akun_bp+"','"+this.e_ket.getText()+"','D',"+line.nilai_susut+",'"+line.kode_pp_susut+"','-','"+this.app._lokasi+"','FA','BP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");
						sql.add("insert into fasusut_j(no_fasusut,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
					...
					sql.add("insert into fasusut_d(no_fasusut,no_fa,periode,nilai,kode_lokasi,akun_bp,akun_ap,kode_akun,kode_pp,kode_drk,dc,no_del,nilai_aset,umur) "+
							...
							"		  else (zz.nilai-a.nilai_residu-isnull(d.tot_susut,0)) end as nilai_susut, "+
							"		  a.kode_lokasi,c.akun_bp,c.akun_deprs,c.kode_akun,a.kode_pp_susut,'-','D','-',zz.nilai,a.umur "+
							"  from fa_asset a "+
							"	 inner join fa_klpakun c on a.kode_klpakun=c.kode_klpakun and a.kode_lokasi=c.kode_lokasi and c.umur<>0 "+
							...
							"where a.jenis='A' and a.progress = '2' and a.kode_lokasi='"+this.app._lokasi+"' and (zz.nilai-a.nilai_residu) > isnull(d.tot_susut,0) and a.periode_susut <= '"+this.e_periode.getText()+"' ");
		...
		if (sender == this.i_gen) {
			if (this.e_periode.getText()!= "" ) {				
				...
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){
						if (line.no_fasusut == "0") this.e_nb.setText("FASR001"+AddFormat);
						else {
							...
							this.e_nb.setText("FASR"+nu+AddFormat);
						}
					} 
				}
				this.e_ket.setFocus();
	...
	doLoadData: function(sender){
		...
		var strSQL = "select b.akun_bp,x.nama as nama_bp,b.akun_deprs,y.nama as nama_deprs,a.kode_pp_susut,c.nama as nama_pp, "+
					 ...
					 "sum( case when (zz.nilai-a.nilai_residu-isnull(d.tot_susut,0)) > ceiling(zz.nilai/a.umur) "+
					 "		    then ceiling(zz.nilai/a.umur)  "+
					 "		    else ceiling(zz.nilai-a.nilai_residu-isnull(d.tot_susut,0)) end) as nilai_susut "+

					 "from fa_asset a  "+		
					 "inner join fa_klpakun b on a.kode_klpakun=b.kode_klpakun and a.kode_lokasi=b.kode_lokasi and b.umur<>0 "+
