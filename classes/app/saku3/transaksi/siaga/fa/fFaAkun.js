window.app_saku3_transaksi_siaga_fa_fFaAkun = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fa_fFaAkun.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fa_fFaAkun";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Akun Aktiva Tetap", 0);	
		
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		uses("saiGrid",true);		
				
		this.pc1 = new pageControl(this,{bound:[20,12,1000,400], childPage:["Daftar Kelompok","Data Kelompok","Filter Cari"]});
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:7,tag:9,
					...
					colWidth:[[6,5,4,3,2,1,0],[200,200,200,80,80,200,80]],
		...
		this.cb_akun = new saiCBBL(this.pc1.childPage[1],{bound:[20,10,220,20],caption:"Kode Akun", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});		
		this.cb_kode = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,10,200,20],caption:"Kode",maxLength:10,change:[this,"doChange"],visible:false});		
		...
		this.e_persen = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,13,200,20],caption:"Persentase", readOnly:true, tag:1, tipeText:ttNilai});													
		this.cb_bp = new saiCBBL(this.pc1.childPage[1],{bound:[20,15,220,20],caption:"Akun BP", multiSelection:false, maxLength:10, tag:2});		
...
window.app_saku3_transaksi_siaga_fa_fFaAkun.extend(window.childForm);
window.app_saku3_transaksi_siaga_fa_fFaAkun.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into fa_klpakun(kode_klpakun,kode_lokasi,nama,umur,persen,kode_akun,akun_bp,akun_deprs) values "+
							"('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"',"+umurBln+","+nilaiToFloat(this.e_persen.getText())+",'"+this.cb_akun.getText()+"','"+this.cb_bp.getText()+"','"+this.cb_deprs.getText()+"')");									
					setTipeButton(tbAllFalse);
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			case "clear" :
				...
				break;
			case "simpan" :	
				if (this.e_umur.getText()!="0" && (this.cb_bp.getText()=="-" || this.cb_deprs.getText()=="-")) {
					system.alert(this,"Transaksi tidak valid.","Akun BP atau Akumulasi harus terisi untuk yang umurnya tidak nol.");
					return false;
				}
				else this.simpan();
				break;				
	...
	doChange: function(sender){
		try{
			if (sender == this.e_umur && this.e_umur.getText() != ""){
				...
				else this.e_persen.setText("0");
			}
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select kode_klpakun,nama,umur,persen,kode_akun,akun_bp,akun_deprs "+
			...
			if (sender == this.cb_akun && this.cb_akun.getText() != ""){
				this.cb_kode.setText(this.cb_akun.getText());
			}		
		}catch(e){
			systemAPI.alert(e);
		}
	...
	doCari:function(sender){								
		try {
			if (this.e_kode2.getText() != "") var filter = " a.kode_klpakun like '%"+this.e_kode2.getText()+"%' and a.kode_lokasi='"+this.app._lokasi+"' ";
			else var filter = " a.nama like '%"+this.e_nama2.getText()+"%' and a.kode_lokasi='"+this.app._lokasi+"' ";			
			var strSQL = "select a.kode_klpakun,a.nama,a.umur /12 as umur,a.persen,a.kode_akun+' - '+ b.nama as akun,a.akun_bp+' - '+isnull(c.nama,'-') as bp,a.akun_deprs+' - '+isnull(d.nama,'-') as deprs "+
						 "from fa_klpakun a inner join masakun b on a.kode_akun=b.kode_akun and b.kode_lokasi ='"+this.app._lokasi+"' "+
						 "                  left join masakun c on a.akun_bp=c.kode_akun and c.kode_lokasi ='"+this.app._lokasi+"' "+
						 "                  left join masakun d on a.akun_deprs=d.kode_akun and d.kode_lokasi ='"+this.app._lokasi+"'  "+
						 "where "+filter;
			var data = this.dbLib.getDataProvider(strSQL,true);		
	...
	doLoad:function(sender){								
		try {			
			var strSQL = "select a.kode_klpakun,a.nama,a.umur /12 as umur,a.persen,a.kode_akun+' - '+ b.nama as akun,a.akun_bp+' - '+isnull(c.nama,'-') as bp,a.akun_deprs+' - '+isnull(d.nama,'-') as deprs "+
						 ...
						 "                  left join masakun c on a.akun_bp=c.kode_akun and c.kode_lokasi ='"+this.app._lokasi+"' "+
						 "                  left join masakun d on a.akun_deprs=d.kode_akun and d.kode_lokasi ='"+this.app._lokasi+"'  "+						 
						 "where a.kode_lokasi='"+this.app._lokasi+"' order by a.kode_klpakun";								
			var data = this.dbLib.getDataProvider(strSQL,true);		
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU = data;
				this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length/20));
