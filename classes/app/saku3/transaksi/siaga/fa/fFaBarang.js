window.app_saku3_transaksi_siaga_fa_fFaBarang = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fa_fFaBarang.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fa_fFaBarang";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Barang Aktiva Tetap", 0);	
		...
		this.cb_akun = new saiCBBL(this.pc1.childPage[1],{bound:[20,14,220,20],caption:"Kode Akun", multiSelection:false, maxLength:10, tag:2});		
		...
		try {
			...
			this.cb_akun.setSQL("select a.kode_klpakun,a.nama from fa_klpakun a union select '-','-' ",["kode_klpakun","nama"],false,["Kode","Nama"],"where","Data Klp Akun",true);			
...
window.app_saku3_transaksi_siaga_fa_fFaBarang.extend(window.childForm);
window.app_saku3_transaksi_siaga_fa_fFaBarang.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into fa_klp(kode_klpfa,kode_lokasi,nama,kode_klpakun,jenis) values "+
							"('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.cb_akun.getText()+"','"+this.c_jenis.getText().substr(0,1)+"')");
					setTipeButton(tbAllFalse);
	...
	doCari:function(sender){								
		try {
			if (this.e_kode2.getText() != "") var filter = " a.kode_klpfa like '%"+this.e_kode2.getText()+"%' and a.kode_lokasi='"+this.app._lokasi+"'  ";
