window.app_saku3_transaksi_siaga_fa_fFaInputM = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fa_fFaInputM.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fa_fFaInputM";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Input Data Aktiva Tetap [Migrasi]", 0);	
		...
		this.cb_lokasi = new saiCBBL(this,{bound:[20,19,220,20],caption:"Lokasi", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});
		...
		this.e_fa = new saiLabelEdit(this,{bound:[20,10,202,20],caption:"Format ID",readOnly:true});
		...
		this.e_residu = new saiLabelEdit(this,{bound:[252,18,200,20],caption:"Nilai Residu", tag:1, tipeText:ttNilai, text:"0"});		
		...
		this.e_nb = new portalui_saiLabelEdit(this,{bound:[20,18,432,20],caption:"No Ref",maxLength:50});
...
window.app_saku3_transaksi_siaga_fa_fFaInputM.extend(window.childForm);
window.app_saku3_transaksi_siaga_fa_fFaInputM.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					var nuAkhir = 0;					
					var strSQL = "select isnull(max(no_fa),0) as no_fa from fa_asset where no_fa like '"+this.e_fa.getText()+"____%'";
	...
	doClick:function(sender){		
		if (this.e_periode.getText()!= "" ) {			
			var formatID = this.e_periode.getText().substr(2,2)+"-"+this.cb_lokasi.getText() + "-"+ this.cb_klp.getText()+"-"; 			
			this.e_fa.setText(formatID);
			this.e_jml.setFocus();
		}		
	},
