window.app_saku3_transaksi_siaga_fAju2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fAju2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fAju2";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Permintaan Bayar", 0);	
		...
		this.cb_app = new saiCBBL(this.pc2.childPage[0],{bound:[20,16,220,20],caption:"NIK Mgr Unit", multiSelection:false, maxLength:10, tag:2, change:[this,"doChangeJab"]});		
		this.c_jab2 = new saiCB(this.pc2.childPage[0],{bound:[700,16,220,20],caption:"Jab Mgr Unit", readOnly:true,tag:0});    //,items:["President Director","Director","Vice President","Manager","Assistant Manager","Staff"]
		
		this.cb_sah = new saiCBBL(this.pc2.childPage[0],{bound:[20,17,220,20],caption:"NIK VP Unit", multiSelection:false, maxLength:10, tag:2, change:[this,"doChangeJab"]});		
		this.c_jab3 = new saiCB(this.pc2.childPage[0],{bound:[700,17,220,20],caption:"Jab VP Unit", readOnly:true,tag:0});	//items:["President Director","Director","Vice President","Manager","Assistant Manager","Staff"],
		
		this.cb_ver = new saiCBBL(this.pc2.childPage[0],{bound:[20,16,220,20],caption:"NIK Ver Anggaran", multiSelection:false, maxLength:10, tag:2, change:[this,"doChangeJab"]});		
		this.c_jab4 = new saiCB(this.pc2.childPage[0],{bound:[700,16,220,20],caption:"Jab Ver Anggaran", readOnly:true,tag:0}); //items:["President Director","Director","Vice President","Manager","Assistant Manager","Staff"],	
		
		this.cb_dir = new saiCBBL(this.pc2.childPage[0],{bound:[20,17,220,20],caption:"NIK Dir Unit ", multiSelection:false, maxLength:10, tag:2, change:[this,"doChangeJab"]});		
		this.c_jab5 = new saiCB(this.pc2.childPage[0],{bound:[700,17,220,20],caption:"Jab Dir Unit", readOnly:true,tag:0}); //items:["President Director","Director","Vice President","Manager","Assistant Manager","Staff"],	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,203], childPage:["Item Mata Anggaran","KPA Anggaran","Lampiran","Rincian Barang","Verifikasi","File Dok"]});
		...
		try {
			...
			if (this.app._userStatus == "A") {
				...
				sql.add("select distinct a.kode_akun,a.nama "+
						"from masakun a "+
			...
			/*
			var data = this.dbLib.getDataProvider("select nik,nama from karyawan where kode_lokasi='"+this.app._lokasi+"'",true);
...
window.app_saku3_transaksi_siaga_fAju2.extend(window.childForm);
window.app_saku3_transaksi_siaga_fAju2.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from gr_pb_dok where no_pb = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
						sql.add("delete from angg_r where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='AJU'");
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','AJU','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.cb_pp.getText()+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(2,i))+","+nilai+")");
							}
						}
					}
					...
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i) && this.sgUpld.cells(2,i)!="-") {							
							...
							sql.add("insert into gr_pb_dok(no_pb,no_gambar,nu,kode_jenis,kode_lokasi,no_ref) values ('"+this.e_nb.getText()+"','"+this.sgUpld.cells(3,i).tmpfile+"','"+i+"','"+this.sgUpld.cells(0,i)+"','"+this.app._lokasi+"','"+this.e_nb.getText()+"')");								
						}	
					}	
                    ...
                    "where a.kode_lokasi='"+this.app._lokasi+"' and b.kode_role='R01' "+
                    "order by b.no_urut ");
					...
					*/
					...
					"select '"+this.no_pesan+"' as no_bukti,a.kode_lokasi,'Beban','Pengajuan Beban','Unit '+c.nama+' dengan no transaksi '+a.no_bukti+' mengajukan beban senilai '+convert(varchar,b.nilai)+', menunggu approval dari anda',a.nik,getdate(),'-',a.no_bukti,'Beban','-',0,0 "+
					"from apv_flow a "+
	...
	doChange:function(sender){		
		if (sender == this.cb_pp && this.cb_pp.getText()!="")
			this.cb_buat.setSQL("select distinct a.nik, a.nama from karyawan a inner join karyawan_pp b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi and b.kode_pp='"+this.cb_pp.getText()+"'  where a.kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Data NIK Pembuat",true);			
	...
	doChangeJab:function(sender){
		if(this.stsSimpan == 1){
			...
			if (sender == this.cb_dir && this.cb_dir.getText()!="") {
				// var strSQL = "select b.nama from karyawan a inner join kug_jab b on a.kode_jab=b.kode_jab and a.kode_lokasi=b.kode_lokasi where a.nik='"+this.cb_dir.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";			
				// var data = this.dbLib.getDataProvider(strSQL,true);
				// if (typeof data == "object"){
				// 	var line = data.rs.rows[0];							
				// 	if (line != undefined) {
				// 		this.c_jab5.setText(line.nama);
				// 	}				
				// }
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_siaga_rptBebanJustEmail";
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai_curr "+
		             "from gr_pb_m a "+				 
					 "where nik_user = '"+this.app._userLog+"' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'AJU2' and a.progress in ('A','0','R','B')";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider(
						   "select b.kode_jenis,b.nama,a.no_gambar from gr_pb_dok a inner join dok_jenis b on a.kode_jenis=b.kode_jenis and a.kode_lokasi=b.kode_lokasi "+
