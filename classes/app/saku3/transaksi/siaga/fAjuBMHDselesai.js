window.app_saku3_transaksi_siaga_fAjuBMHDselesai = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fAjuBMHDselesai.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fAjuBMHDselesai";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Permintaan Bayar Ref BMHD", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,233], childPage:["Data BYMHD","Data Pengajuan","KPA Anggaran","Lampiran","Rincian Barang","Verifikasi"]}); 
		this.c_periode = new saiCB(this.pc1.childPage[0],{bound:[20,10,200,20],caption:"Periode",readOnly:true,tag:0,change:[this,"doChange"]});	
		...
		this.cb_akun = new saiCBBL(this.pc1.childPage[0],{bound:[20,12,220,20],caption:"Akun BMHD", readOnly:true});		
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select distinct periode from gr_bmhd_d",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];													
...
window.app_saku3_transaksi_siaga_fAjuBMHDselesai.extend(window.childForm);
window.app_saku3_transaksi_siaga_fAjuBMHDselesai.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from angg_r where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='BYRBMHD'");
					...
					sql.add("insert into gr_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','D','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+","+parseNilai(this.e_bmhdcurr.getText())+","+parseNilai(this.e_bmhd.getText())+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','BYRBMHD','BMHDREV','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
					
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){								
								sql.add("insert into gr_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i)+"','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+","+parseNilai(this.sg.cells(4,i))+","+parseNilai(this.sg.cells(5,i))+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','BYRBMHD','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
							}
						}
					}
					...
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
							"('"+this.e_nb.getText()+"','BYRBMHD','"+this.app._lokasi+"','"+this.cb_akun.getText()+"','"+this.cb_pp.getText()+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','D',0,"+parseNilai(this.e_bmhd.getText())+")");
					if (this.sg2.getRowValidCount() > 0){
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','BYRBMHD','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.cb_pp.getText()+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(2,i))+","+nilai+")");
							}
						}
					}
					...
					sql.add("insert into gr_bmhd_d(no_bukti,no_bmhd,no_dokumen,akun_hutang,keterangan,dc,nilai,kode_pp,kode_lokasi,modul,jenis,periode,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_bmhd.getText()+"','"+this.e_dok.getText()+"','"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_bmhd.getText())+",'"+this.app._kodePP+"','"+this.app._lokasi+"','BYRBMHD','AJUSLS','"+this.e_periode.getText()+"','IDR',1)");
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :		
				...
				if (nilaiToFloat(this.e_saldo.getText()) < nilaiToFloat(this.e_bmhd.getText())) {
					system.alert(this,"Transaksi tidak valid.","Nilai BMHD melebihi Saldo BMHD.");
					return false;						
				}
	...
	doChange:function(sender){		
		...
		if (sender == this.c_periode && this.c_periode.getText()!="" && this.stsSimpan==1) {
			this.cb_bmhd.setSQL("select distinct a.no_bmhd, a.keterangan from gr_bmhd_d a "+									
					...
					"where a.periode='"+this.c_periode.getText()+"' and a.jenis='AKRU' and b.saldo > 0 and a.kode_lokasi='"+this.app._lokasi+"' ",["a.no_bmhd","a.keterangan"],false,["No Bukti","Deskripsi"],"and","Data BMHD",true);			
		}
	},	
	...
	doClick:function(sender){
		try {
			if (this.e_periode.getText()!= "") {
				...
				this.stsSimpan = 1;
				this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"gr_pb_m","no_pb",this.app._lokasi+"-PBBD"+this.e_periode.getText().substr(2,2)+this.Aperiode+".","0000"));						
				this.e_dok.setFocus();
				setTipeButton(tbSimpan);			
			}		
		}
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_siaga_rptAjuBMHD";
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai_curr "+
		             "from gr_pb_m a "+				 
					 "where nik_user = '"+this.app._userLog+"' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'BYRBMHD' and a.progress in ('0','R','V')";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
