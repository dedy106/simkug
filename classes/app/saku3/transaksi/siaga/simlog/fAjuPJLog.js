window.app_saku3_transaksi_siaga_simlog_fAjuPJLog = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_simlog_fAjuPJLog.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_simlog_fAjuPJLog";
		...
		this.c_jab1 = new saiCB(this.pc2.childPage[0],{bound:[700,17,220,20],caption:"Jabatan Pembuat", readOnly:true,tag:2});		//items:["President Director","Director","Vice President","Manager","Assistant Manager","Staff"],		
		...
		this.c_jab2 = new saiCB(this.pc2.childPage[0],{bound:[700,16,220,20],caption:"Jabatan Pemeriksa", readOnly:true,tag:2});		//items:["President Director","Director","Vice President","Manager","Assistant Manager","Staff"],		
		...
		this.c_jab3 = new saiCB(this.pc2.childPage[0],{bound:[700,17,220,20],caption:"Jab Pengesahan", readOnly:true,tag:2});		//items:["President Director","Director","Vice President","Manager","Assistant Manager","Staff"],		
		...
		this.c_jab4 = new saiCB(this.pc2.childPage[0],{bound:[700,16,220,20],caption:"Jabatan Verifikasi", readOnly:true,tag:2});		//items:["President Director","Director","Vice President","Manager","Assistant Manager","Staff"],
		...
		this.c_curr = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,20,160,20],caption:"Mt Uang - Kurs",readOnly:true,tag:2,change:[this,"doChange"]});
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,210], childPage:["Data Request","KPA Anggaran","Lampiran","Rincian Barang","Verifikasi"]});		
		this.cb_pesan = new saiCBBL(this.pc1.childPage[0],{bound:[20,10,220,20],caption:"No Request", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});				
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-65],colCount:6,tag:9,
...
window.app_saku3_transaksi_siaga_simlog_fAjuPJLog.extend(window.childForm);
window.app_saku3_transaksi_siaga_simlog_fAjuPJLog.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from angg_r where no_bukti = '"+this.noReq+"' and kode_lokasi='"+this.app._lokasi+"' and modul='R-LOGREQ'");												
					...
					//loncat gak perlu approve budget, krn sudah di approve saat pengajuan barang					
					...
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) "+
							"select no_bukti,'R-LOGREQ',kode_lokasi,kode_akun,kode_pp,kode_drk,'"+this.e_periode.getText()+"',periode2,case dc when 'D' then 'C' else 'D' end,saldo,nilai "+
							"from angg_r "+
							"where no_bukti='"+this.cb_pesan.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					
					//insert budget
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) "+
							"select '"+this.e_nb.getText()+"','AJU',kode_lokasi,kode_akun,kode_pp,kode_drk,'"+this.e_periode.getText()+"',periode2,dc,saldo,nilai "+
							"from angg_r "+
							"where no_bukti='"+this.cb_pesan.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='LOGREQ'");
			...
			case "clear" :
				if (modalResult == mrOk)
					...
					this.cb_pesan.setSQL("select a.no_pesan,a.keterangan from log_pesan_m a "+
										 ...
										 "where a.periode<='"+this.e_periode.getText()+"' and a.progress='2' and a.lok_proses = '"+this.app._lokasi+"' and c.jenis in ('PJR')",["no_pesan","keterangan"],false,["No Request","Deskripsi"],"and","Data Request",true);			
				break;
			case "simpan" :	
			case "ubah" :		
				this.preView = "1";							
		...
		*/
		...
		if (sender == this.cb_pesan && this.cb_pesan.getText()!="") {
			var data = this.dbLib.getDataProvider(
						"select a.kode_akun,b.nama as nama_akun,c.dc,a.keterangan,a.nilai_curr,a.nilai,c.saldo,a.kode_curr,a.kurs "+
						"from log_pesan_m a inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+						
						"					inner join angg_r c on a.no_pesan=c.no_bukti and a.kode_lokasi=c.kode_lokasi and c.modul='LOGREQ' "+
						"where a.no_pesan = '"+this.cb_pesan.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.kode_akun"
						,true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
				...
				for (var i in data.rs.rows){
					...
					var sak = parseFloat(line.saldo) - parseFloat(line.nilai);
					this.sg2.appendData([line.kode_akun,line.nama_akun,floatToNilai(line.saldo),floatToNilai(line.nilai),floatToNilai(sak)]);
				}
				if (this.stsSimpan == 1) {
			...
			var strSQL = "select item,'-' as sat,jumlah,nilai,(jumlah * nilai) as total "+
						 "from log_pesan_d "+
						 "where no_pesan='"+this.cb_pesan.getText()+"' and kode_lokasi ='"+this.app._lokasi+"'";
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg4.clear();
				for (var i in data.rs.rows){
					line = data.rs.rows[i];												
					this.sg4.appendData([line.item,line.sat,floatToNilai(line.jumlah),floatToNilai(line.nilai),floatToNilai(line.total)]);										
				}
			} 
		}
	},	
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_siaga_simlog_rptPanjarAju";
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai_curr "+
		             "from gr_pb_m a "+					 					 
					 "where nik_user = '"+this.app._userLog+"' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'PJAJULOG' and a.progress in ('1','V')";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
				...
				if (typeof data == "object"){
					...
					if (line != undefined){
						...
						this.cb_pesan.setSQL("select a.no_pesan,a.keterangan from log_pesan_m a "+
											"inner join log_justerima_m c on a.no_pesan=c.no_pesan "+
											"inner join app_m d on c.no_terima=d.no_app and d.no_appseb='-' "+
											" where a.no_pesan='"+line.atensi+"' and a.lok_proses = '"+this.app._lokasi+"' and c.jenis in ('PJR')",["no_pesan","keterangan"],false,["No Request","Deskripsi"],"and","Data Request",true);			
						this.cb_pesan.setText(line.atensi);
						this.noReq = line.atensi;
