window.app_saku3_transaksi_siaga_simlog_fPks = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_simlog_fPks.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_simlog_fPks";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form PKS", 0);	
		...
		this.e_dok = new saiLabelEdit(this.pc1.childPage[2],{bound:[20,11,450,20],caption:"No Dokumen", maxLength:50,tag:9});						
...
window.app_saku3_transaksi_siaga_simlog_fPks.extend(window.childForm);
window.app_saku3_transaksi_siaga_simlog_fPks.implement({	
			...
			case "clear" :
				if (modalResult == mrOk)
					...
					this.cb_spk.setSQL("select a.no_spk, a.keterangan "+
							   ...
							   "where a.modul = 'TL' and a.periode<='"+this.e_periode.getText()+"' and a.no_pks='-' and a.kode_lokasi='"+this.app._lokasi+"'",["no_spk","keterangan"],false,["No SPK","Deskripsi"],"and","Data SPK",true);			
				break;
			case "simpan" :															
			case "ubah" :																											
				this.preView = "1";												
