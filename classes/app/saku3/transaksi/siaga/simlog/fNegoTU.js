window.app_saku3_transaksi_siaga_simlog_fNegoTU = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_simlog_fNegoTU.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_simlog_fNegoTU";
		...
		this.e_ket = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,13,450,20],caption:"Perihal", maxLength:150,tag:0});						
		...
		this.e_vendor = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,12,450,20],caption:"Mitra", readOnly:true,tag:0});										
		this.e_alamat = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,14,450,20],caption:"Alamat", readOnly:true,tag:0});										
		this.e_pic = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,16,450,20],caption:"Pihak Mitra", maxLength:50,tag:0});								
		...
		this.cb_buat = new saiCBBL(this.pc2.childPage[0],{bound:[20,17,220,20],caption:"NIK Logistik", multiSelection:false, maxLength:10, tag:0});				
...
window.app_saku3_transaksi_siaga_simlog_fNegoTU.extend(window.childForm);
window.app_saku3_transaksi_siaga_simlog_fNegoTU.implement({	
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					setTipeButton(tbAllFalse);			
					this.cb_sph.setSQL("select no_sph, keterangan from log_sph_m where modul = 'TL' and periode<='"+this.e_periode.getText()+"' and no_nego='-' and kode_lokasi='"+this.app._lokasi+"'",["no_sph","keterangan"],false,["No SPH","Deskripsi"],"and","Data SPH",true);			
				break;
			case "simpan" :															
			case "ubah" :												
				/*
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_nego,convert(varchar,a.tanggal,103) as tgl,a.keterangan,b.nama,a.nilai+a.ppn as nilai "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_tap='-' and d.modul='TL' ";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
