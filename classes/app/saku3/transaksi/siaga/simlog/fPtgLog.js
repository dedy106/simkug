window.app_saku3_transaksi_siaga_simlog_fPtgLog = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_simlog_fPtgLog.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_simlog_fPtgLog";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Input Pertanggungan Logistik: Input", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,10,990,350], childPage:["Data Panjar","Item Mata Anggaran","KPA Anggaran","Cat. Approval"]});
		...
		this.e_dok = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,16,200,20],caption:"No Request", readOnly:true});		
		...
		try {
			...
			this.cb_pp.setSQL("select kode_pp, nama from pp where kode_bidang='"+this.app._kodeBidang+"' and kode_lokasi='"+this.app._lokasi+"' and tipe='posting' and flag_aktif ='1' ",["kode_pp","nama"],false,["Kode","Nama"],"and","Data PP",true);			
...
window.app_saku3_transaksi_siaga_simlog_fPtgLog.extend(window.childForm);
window.app_saku3_transaksi_siaga_simlog_fPtgLog.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from angg_r where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul ='PTGLOG'");					
						sql.add("delete from angg_r where no_bukti = '"+this.e_dok.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul ='PTGLOG'");					
					...
					sql.add("insert into panjarptg2_m (no_ptg,no_panjar,no_final,no_app,no_ver,no_dokumen,tanggal,keterangan,akun_panjar,nik_buat,nik_app,kode_lokasi,kode_pp,nilai_pj,nilai,nilai_kas,progress,periode,nik_user,tgl_input,modul) values "+
						    "('"+this.e_nb.getText()+"','"+this.cb_pj.getText()+"','-','"+this.noAppLama+"','"+this.noVerLama+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.e_akunpj.getText()+"','"+this.app._userLog+"','"+this.cb_setuju.getText()+"','"+this.app._lokasi+"','"+this.cb_pp.getText()+"',"+parseNilai(this.e_nilaipj.getText())+","+parseNilai(this.e_total.getText())+","+parseNilai(this.e_sls.getText())+",'"+vProg+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'PTGLOG')");
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){								
								sql.add("insert into panjarptg2_j(no_ptg,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i)+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','"+this.app._lokasi+"','PTGLOG','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
							}
						}
					}
					sql.add("insert into panjarptg2_j(no_ptg,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',99,'"+this.e_akunpj.getText()+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_nilaipj.getText())+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','PTGLOG','PANJAR','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");					
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','PTGLOG','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(4,i)+"','"+this.periodePR+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(6,i))+","+nilai+")");
							}
						}
					}				
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) "+
							"select no_bukti,'PTGLOG',kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,'"+this.e_periode.getText()+"',case dc when 'D' then 'C' else 'D' end,0,nilai from angg_r where no_bukti='"+this.e_dok.getText()+"' and modul='LOGREQ'");
		...
		if (sender == this.e_periode || sender == this.cb_pp) {
			if (this.e_periode.getText()!="" && this.cb_pp.getText()!="" && this.stsSimpan == 1) {				
				this.cb_pj.setSQL("select a.no_panjar, a.keterangan from panjar2_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
				                  "where a.modul='PJLOG' and b.kode_bidang='"+this.app._kodeBidang+"' and  a.no_kas<>'-' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",["no_panjar","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true); 				
			}
		}
		if (sender == this.cb_pj && this.cb_pj.getText()!="" && this.stsSimpan == 1) {
			var strSQL = "select c.periode,a.nilai,a.akun_panjar,a.nik_buat+' - '+b.nama as pemegang,c.no_pesan "+
			             "from panjar2_m a "+
			             "inner join karyawan b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi "+
						 "inner join log_pesan_m c on a.no_panjar=c.no_spph and a.kode_lokasi=c.kode_lokasi "+
			             "where a.no_panjar='"+this.cb_pj.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){						
					this.e_dok.setText(line.no_pesan);
	...
	doHitungGar: function(){
		...
		var sls = 0;
		for (var i=0;i < this.sg2.getRowCount();i++){			
			if (this.stsSimpan == 1) var data = this.dbLib.getDataProvider("select fn_cekagg3('"+this.sg2.cells(2,i)+"','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(4,i)+"','"+this.periodePR+"','"+this.e_dok.getText()+"') as gar ",true);
			else var data = this.dbLib.getDataProvider("select fn_cekagg3('"+this.sg2.cells(2,i)+"','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(4,i)+"','"+this.periodePR+"','"+this.e_nb.getText()+"') as gar ",true);
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){							
							this.nama_report="server_report_saku3_panjar_rptPjPtgForm";
	...
	doLoad3:function(sender){																		
		var strSQL = "select c.no_ptg,a.no_panjar,convert(varchar,c.tanggal,103) as tgl,case c.progress when '0' then 'AJU' when 'R' then 'REVISI' when 'V' then 'REV.VER' end as jenis,c.no_dokumen,c.keterangan,a.nik_buat+' - '+b.nama as nama,c.nilai "+
					 ...
					 "where c.periode='"+this.e_periode.getText()+"' and c.kode_lokasi='"+this.app._lokasi+"' and c.progress in ('0','R','V')";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.nilai,a.akun_panjar,a.no_panjar,a.keterangan,a.nik_buat+' - '+e.nama as pemegang,b.progress, "+
							 " b.no_dokumen,b.keterangan as ket,b.nik_app,d.nama as nama_app,b.tanggal,b.no_app as no_applama,b.no_ver as no_verlama,cc.periode as periodepr "+
							 ...
							 "				   inner join log_pesan_m cc on a.no_panjar=cc.no_spph and a.kode_lokasi=cc.kode_lokasi "+
