window.app_saku3_transaksi_siaga_simlog_fSpkTL = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_simlog_fSpkTL.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_simlog_fSpkTL";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form SPK Penunjukan Langsung", 0);	
		...
		this.cb_buat = new saiCBBL(this.pc2.childPage[0],{bound:[20,17,220,20],caption:"NIK TTD", multiSelection:false, maxLength:10, tag:1});				
...
window.app_saku3_transaksi_siaga_simlog_fSpkTL.extend(window.childForm);
window.app_saku3_transaksi_siaga_simlog_fSpkTL.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into log_spk_m(no_spk,no_pks,kode_lokasi,tgl_input,nik_user,periode,tanggal,nik_buat,keterangan,no_tap,kode_vendor,total,no_dokumen,modul,no_sanggup) values "+
							"('"+this.e_nb.getText()+"','-','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_buat.getText()+"','"+this.e_ket.getText()+"','"+this.e_nb.getText()+"','"+this.kodeVendor+"',"+nilaiToFloat(this.e_total.getText())+",'"+this.e_dok.getText()+"','TL','-')");					
					...
					sql.add("insert into log_tap_m(no_tap,no_spk,kode_lokasi,tgl_input,nik_user,periode,tanggal,nik_buat,keterangan,no_spph,alasan,no_dokumen,modul,no_sanggup) values "+
							"('"+this.e_nb.getText()+"','"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_buat.getText()+"','"+this.e_ket.getText()+"','"+this.cb_spph.getText()+"','-','"+this.e_dok.getText()+"','TL','-')");					
	...
	doChange:function(sender){
		...
		if (sender == this.cb_spph && this.cb_spph.getText()!="") {			
			var strSQL = "select no_pesan "+
			             "from log_pesan_m "+
						 "where no_spph='"+this.cb_spph.getText()+"'";
			...
			if (this.stsSimpan == 1) {				
				var strSQL = "select a.no_urut,a.item,a.merk,a.tipe,a.catatan,a.jumlah,a.nilai_nego,ppn_nego as ppn,a.jumlah*(a.nilai_nego+a.ppn_nego) as total, e.kode_klpfa, e.nama "+
							 ...
							 "where d.no_spph='"+this.cb_spph.getText()+"' order by a.no_urut";
				var data = this.dbLib.getDataProvider(strSQL,true);
	...
	doLoad3:function(sender){																				
		var strSQL = "select a.no_spk,convert(varchar,a.tanggal,103) as tgl,a.keterangan,b.nama,a.total as nilai "+
					 ...
					 "where a.no_sanggup='-' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_pks='-' and a.modul='TL'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				this.cb_spph.setSQL("select no_spph, keterangan from log_spph_m where no_sanggup='-' and no_tap='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_spph","keterangan"],false,["No SPPH","Deskripsi"],"and","Data SPPH",true);			
				
				var strSQL = "select a.keterangan,a.tanggal,a.nik_buat,b.no_gambar,a.kode_vendor,c.no_spph,a.no_dokumen "+
