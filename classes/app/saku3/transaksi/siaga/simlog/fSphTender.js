window.app_saku3_transaksi_siaga_simlog_fSphTender = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_simlog_fSphTender.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_simlog_fSphTender";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form SPH Tender", 0);	
		...
		this.cb_spph = new saiCBBL(this.pc2.childPage[0],{bound:[20,17,220,20],caption:"No Penjelasan", multiSelection:false, maxLength:10, tag:0,change:[this,"doChange"]});						
...
window.app_saku3_transaksi_siaga_simlog_fSphTender.extend(window.childForm);
window.app_saku3_transaksi_siaga_simlog_fSphTender.implement({	
	...
	doSelectDate: function(sender, y,m,d){
		...
		if (this.stsSimpan == 1) {
			this.cb_spph.setSQL("select no_spph, keterangan from log_spph_m where periode <= '"+this.e_periode.getText()+"' and modul='TD' and no_tap='-' and kode_lokasi='"+this.app._lokasi+"'",["no_spph","keterangan"],false,["No SPPH","Deskripsi"],"and","Data SPPH",true);			
			this.doClick();				
		}
	},
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_sph,convert(varchar,a.tanggal,103) as tgl,a.keterangan,b.nama,a.nilai+a.ppn as nilai "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_nego='-' and a.modul='TD'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
