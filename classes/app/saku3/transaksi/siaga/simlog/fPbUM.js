window.app_saku3_transaksi_siaga_simlog_fPbUM = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_simlog_fPbUM.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_simlog_fPbUM";
		...
		this.cb_spk = new saiCBBL(this.pc1.childPage[0],{bound:[20,11,220,20],caption:"Data PO / SPK", multiSelection:false, maxLength:20, tag:1,change:[this,"doChange"]});						
		...
		this.e_saldocurr = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,13,200,20],caption:"Sisa PO/SPK+", tag:4, readOnly:true, tipeText:ttNilai, text:"0"});				
		
		this.c_curr = new saiLabelEdit(this.pc1.childPage[0],{bound:[250,13,160,20],caption:"Mt Uang - Kurs",readOnly:true,tag:2,change:[this,"doChange"]});
		...
		this.e_nilaium = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,16,200,20],caption:"Nilai PB Curr", tag:4, tipeText:ttNilai, text:"0", change:[this,"doChange"]});				
		this.e_ppn = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,14,200,20],caption:"Nilai PPN Curr", tag:4, tipeText:ttNilai, text:"0",readOnly:true, change:[this,"doChange"]});				
		this.e_hutang = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,12,200,20],caption:"Total PB Curr", tag:4, tipeText:ttNilai, text:"0",readOnly:true, change:[this,"doChange"]});				
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('PPNM','LOGADK','HUTPBLOG','JUAKUNKB','GARFREE') and kode_lokasi = '"+this.app._lokasi+"'",true);
			if (typeof data == "object"){
				...
				for (var i in data.rs.rows){
					...
					if (line.kode_spro == "LOGADK") this.akunUM = line.flag;		
...
window.app_saku3_transaksi_siaga_simlog_fPbUM.extend(window.childForm);
window.app_saku3_transaksi_siaga_simlog_fPbUM.implement({
	isiCBSpk: function() {
		//spk belum pernah bast rekon dan nilai spk > nilai total um
		this.cb_spk.setSQL("select a.no_spk, a.keterangan "+
						   "from log_spk_m a "+						   
						   "	left join ("+
						   "			  select no_spk,kode_lokasi,sum(case dc when 'D' then nilai_curr+ppn_curr else -(nilai_curr+ppn_curr) end) tot_um "+
					       "    		  from log_pbadk_d where kode_lokasi='"+this.app._lokasi+"' group by no_spk,kode_lokasi "+
						   "			  ) e on a.no_spk=e.no_spk and a.kode_lokasi=e.kode_lokasi "+						   
						   "where a.total > isnull(e.tot_um,0) and a.no_pks ='-' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,4])){
				try{														
					...
					sql.add("insert into gr_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.akunHutang+"','"+this.e_ket.getText()+"','D','"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+","+parseNilai(this.e_hutang.getText())+","+hutIDR+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','PBADK','HUTANG','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");		 
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){								
								sql.add("insert into gr_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i)+"','"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+","+parseNilai(this.sg.cells(4,i))+","+parseNilai(this.sg.cells(5,i))+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','PBADK','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
							}
						}
					}
					...
					//jurnal
					var umIDR = Math.round(nilaiToFloat(this.e_nilaium.getText()) * nilaiToFloat(this.e_kurs.getText()));
					...
					sql.add("insert into hutang_m(no_hutang,kode_lokasi,no_dokumen,tanggal,keterangan,kode_project,kode_vendor,kode_curr,kurs,nik_app,kode_pp,nilai,periode,nik_user,tgl_input,akun_hutang,posted,nilai_ppn,modul,no_ref,nilai_curr) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','-','"+this.e_vendor.getText()+"','"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+",'"+this.app._userLog+"','"+this.app._kodePP+"',"+hutIDR+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.akunUM+"','F',0,'LOGUM','"+this.cb_spk.getText()+"','"+nilaiToFloat(this.e_hutang.getText())+"')");
					...
					sql.add("insert into hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_spk.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.akunUM+"','"+this.e_ket.getText()+"','D','"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+","+parseNilai(this.e_nilaium.getText())+","+umIDR+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','LOGUM','UM','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
					sql.add("insert into hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_spk.getText()+"','"+this.dp_d1.getDateString()+"',1,'"+this.akunPPN+"','"+this.e_ket.getText()+"','D','"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+","+parseNilai(this.e_ppn.getText())+","+ppnIDR+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','LOGUM','PPN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
					sql.add("insert into hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_spk.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.akunHutang+"','"+this.e_ket.getText()+"','C','"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+","+parseNilai(this.e_hutang.getText())+","+hutIDR+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','LOGUM','HUT','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
			
					sql.add("insert into log_pbadk_d(no_pb,no_spk,kode_lokasi,periode,dc,nilai,ppn,no_bast,akun_adk,kode_curr,kurs,nilai_curr,ppn_curr) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_spk.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','D',"+umIDR+","+ppnIDR+",'-','"+this.akunUM+"','"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+","+nilaiToFloat(this.e_nilaium.getText())+","+nilaiToFloat(this.e_ppn.getText())+")");
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_spk && this.cb_spk.getText() != "") {
			var strSQL = "select b.kode_vendor,b.nama,b.bank,b.cabang,b.no_rek,b.nama_rek,a.kode_curr, yy.jenis, a.ppn,  "+						 
						 "a.total - isnull(d.tot_um,0) as saldo_um_curr "+
						 "from log_spk_m a "+
						 ...
						 "	left join ("+
						 "			  select no_spk,kode_lokasi,sum(case dc when 'D' then nilai_curr+ppn_curr else -(nilai_curr+ppn_curr) end) tot_um "+
						 "    		  from log_pbadk_d where no_spk='"+this.cb_spk.getText()+"' and no_pb<>'"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' group by no_spk,kode_lokasi "+
						 "			  ) d on a.no_spk=d.no_spk and a.kode_lokasi=d.kode_lokasi "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){				
					...
					this.e_saldocurr.setText(floatToNilai(line.saldo_um_curr));

					this.c_curr.setText(line.kode_curr);	
					
					//status spk/po ada ppn ato tidak
					if (parseFloat(line.ppn) == 0) this.stsPPN = "NON";
					else this.stsPPN = "PPN";

				}
			}
		}	

		if (sender == this.e_nilaium && this.e_nilaium.getText()!="") {
			var ppn = 0;
			if (this.stsPPN == "PPN") {
				ppn = 0.1 * nilaiToFloat(this.e_nilaium.getText());
				this.e_ppn.setText(floatToNilai(ppn));
			}
			else this.e_ppn.setText("0");

			var total = nilaiToFloat(this.e_nilaium.getText()) + ppn;
			this.e_hutang.setText(floatToNilai(total));
		}
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_siaga_rptPermintaanByr";
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider(
						   "select a.*,c.nilai_curr as nilai_um_curr,c.ppn_curr,   isnull(b.no_app,'-') as no_app,isnull(b.catatan,'-') as catatan "+						  
						   "from gr_pb_m a "+	
						   "		inner join log_pbadk_d c on a.no_pb=c.no_pb and a.kode_lokasi=c.kode_lokasi "+					   
						   "		left join ("+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){
						...
						this.e_nilaium.setText(floatToNilai(line.nilai_um_curr));
						this.e_ppn.setText(floatToNilai(line.ppn_curr));
