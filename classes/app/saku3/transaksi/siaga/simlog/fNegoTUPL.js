window.app_saku3_transaksi_siaga_simlog_fNegoTUPL = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_simlog_fNegoTUPL.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_simlog_fNegoTUPL";
...
window.app_saku3_transaksi_siaga_simlog_fNegoTUPL.extend(window.childForm);
window.app_saku3_transaksi_siaga_simlog_fNegoTUPL.implement({	
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					setTipeButton(tbAllFalse);			
					this.cb_sph.setSQL("select a.no_sph, a.keterangan from log_sph_m a inner join log_spph_m b on a.no_spph=b.no_spph and b.no_tap='-' where a.modul = 'PL' and a.periode<='"+this.e_periode.getText()+"' and a.no_nego='-' and a.kode_lokasi='"+this.app._lokasi+"'",["no_sph","keterangan"],false,["No SPH","Deskripsi"],"and","Data SPH",true);			
				break;
			case "simpan" :															
			case "ubah" :												
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_nego,convert(varchar,a.tanggal,103) as tgl,a.keterangan,b.nama,a.nilai+a.ppn as nilai "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_tap='-' and d.modul='PL'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
