window.app_saku3_transaksi_siaga_simlog_fSanggupTD = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_simlog_fSanggupTD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_simlog_fSanggupTD";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Kesanggupan Tender", 0);	
...
window.app_saku3_transaksi_siaga_simlog_fSanggupTD.extend(window.childForm);
window.app_saku3_transaksi_siaga_simlog_fSanggupTD.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into log_sanggup_m(no_sanggup,no_spk,kode_lokasi,tgl_input,nik_user,periode,tanggal,nik_buat,keterangan,no_tap,kode_vendor,total,no_dokumen,modul) values "+
							"('"+this.e_nb.getText()+"','-','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_buat.getText()+"','"+this.e_ket.getText()+"','"+this.cb_spph.getText()+"','"+this.kodeVendor+"',"+nilaiToFloat(this.e_total.getText())+",'"+this.e_dok.getText()+"','TD')");					
			...
			case "clear" :
				if (modalResult == mrOk)
					...
					this.cb_spk.setSQL("select a.no_spk, a.keterangan from log_spk_m a "+			                   
									   "where a.no_sanggup='-' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='TD'",["no_spk","keterangan"],false,["No SPK","Deskripsi"],"and","Data SPK",true);						
				break;
			case "simpan" :															
			case "ubah" :																											
				this.preView = "1";												
	...
	doLoad3:function(sender){																				
		var strSQL = "select a.no_sanggup,convert(varchar,a.tanggal,103) as tgl,a.keterangan,b.nama,a.total as nilai "+
					 ...
					 "where d.no_pks='-' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='TD'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
