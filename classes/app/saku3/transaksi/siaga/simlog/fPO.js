window.app_saku3_transaksi_siaga_simlog_fPO = function (owner) {
	if (owner) {
		window.app_saku3_transaksi_siaga_simlog_fPO.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_siaga_simlog_fPO";
		...
		this.e_nilai = new saiLabelEdit(this.pc2.childPage[0], { bound: [790, 18, 200, 20], caption: "Nilai PO", tag: 1, readOnly: true, tipeText: ttNilai, text: "0" });
		...
		this.e_total = new saiLabelEdit(this.pc2.childPage[0], { bound: [790, 14, 200, 20], caption: "Total PO", tag: 1, readOnly: true, tipeText: ttNilai, text: "0" });
		...
		this.pc1 = new pageControl(this.pc2.childPage[0], { bound: [1, 12, 995, 268], childPage: ["Item Barang", "Ketentuan", "Termin Tagihan", "File Dok", "Otorisasi"] });
		...
		this.e_denda = new saiMemo(this.pc1.childPage[1], { bound: [10, 12, 450, 70], caption: "Denda", tag: 9, visible: false });
		this.e_spek = new saiMemo(this.pc1.childPage[1], { bound: [530, 12, 450, 70], caption: "Spesifikasi Barang", tag: 9, visible: false });
...
window.app_saku3_transaksi_siaga_simlog_fPO.extend(window.childForm);
window.app_saku3_transaksi_siaga_simlog_fPO.implement({
		...
		try {
			...
			if (this.standarLib.checkEmptyByTag(this, [0, 1, 2])) {
				try {
					...
					sql.add("insert into log_spk_m(no_spk,no_pks,kode_lokasi,tgl_input,nik_user,periode,tanggal,nik_buat,keterangan,no_tap,kode_vendor,total,no_dokumen,modul,no_sanggup,ket_harga,ket_bayar,ket_kirim,ket_garansi,ket_denda,ket_spek,tgl_garansi, kode_curr, kurs, nik_ttd1,nik_ttd2,nik_ttd3,  jab_ttd1,jab_ttd2,jab_ttd3,ppn) values " +
						"('" + this.e_nb.getText() + "','-','" + this.app._lokasi + "',getdate(),'" + this.app._userLog + "','" + this.e_periode.getText() + "','" + this.dp_d1.getDateString() + "','-','" + this.e_ket.getText() + "','" + this.cb_tap.getText() + "','" + this.kodeVendor + "'," + nilaiToFloat(this.e_total.getText()) + ",'" + this.e_dok.getText() + "','PO','-',  '" + this.e_harga.getText() + "','" + this.e_bayar.getText() + "','" + this.e_kirim.getText() + "','" + this.e_garansi.getText() + "','" + this.e_denda.getText() + "','" + this.e_spek.getText() + "','" + this.dp_d2.getDateString() + "' ,'" + this.e_curr.getText() + "'," + nilaiToFloat(this.e_kurs.getText()) + ",'" + this.cb_ttd1.getText() + "','-','-','" + this.c_jabttd1.getText() + "','-','-'," + nilaiToFloat(this.e_ppn.getText()) + " )");
					//sql.add("insert into log_pesan_dok(no_pesan,no_gambar,nu,kode_jenis,kode_lokasi) values('" + this.e_nb.getText() + "','" + this.e_file.getText() + "',0,'PO','" + this.app._lokasi + "')");
		...
		switch (event) {
			case "clear":
				...
				this.cb_tap.setSQL("select a.no_tap, a.keterangan from log_tap_m a " +
					...
					"where a.no_spk='-' and a.periode<='" + this.e_periode.getText() + "' and a.kode_lokasi='" + this.app._lokasi + "' and a.modul='PO'", ["no_tap", "keterangan"], false, ["No TAP", "Deskripsi"], "and", "Data Penetapan", true);
				break;
			case "simpan":
			case "ubah":
	...
	doLoad3: function (sender) {
		var strSQL = "select a.no_spk,convert(varchar,a.tanggal,103) as tgl,a.keterangan,b.nama,a.total as nilai " +
			...
			"where c.no_po is null and a.periode='" + this.e_periode.getText() + "' and a.kode_lokasi='" + this.app._lokasi + "' and a.modul='PO'";
		var data = this.dbLib.getDataProvider(strSQL, true);
		if (typeof data == "object" && data.rs.rows[0] != undefined) {
			this.dataJU3 = data;
	...
	doDoubleClick3: function (sender, col, row) {
		try {
			if (this.sg3.cells(0, row) != "") {
				...
				this.cb_tap.setSQL("select no_tap, keterangan from log_tap_m where no_spk='" + this.e_nb.getText() + "' and kode_lokasi='" + this.app._lokasi + "' and modul='PO'", ["no_tap", "keterangan"], false, ["No TAP", "Deskripsi"], "and", "Data Penetapan", true);
				...
				var strSQL = "select a.keterangan,a.tanggal,a.nik_buat,a.kode_vendor,c.no_tap,a.no_dokumen, a.tgl_garansi,a.ket_harga,a.ket_bayar,a.ket_kirim,a.ket_garansi,a.ket_denda,a.ket_spek, " +
					...
					"from log_spk_m a " + //inner join log_pesan_dok b on a.no_spk=b.no_pesan and a.kode_lokasi=b.kode_lokasi and b.kode_jenis='PO' 
