window.app_saku3_transaksi_siaga_simlog_fSanggup = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_simlog_fSanggup.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_simlog_fSanggup";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Kesanggupan Pemilihan Langsung", 0);	
		...
		this.cb_spph = new saiCBBL(this.pc2.childPage[0],{bound:[20,18,220,20],caption:"No SPPH", readOnly:true, tag:1,change:[this,"doChange"]});						
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,248], childPage:["Item Barang","File Dok"]});						
...
window.app_saku3_transaksi_siaga_simlog_fSanggup.extend(window.childForm);
window.app_saku3_transaksi_siaga_simlog_fSanggup.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("update log_tap_m set no_sanggup='-' where no_spk='"+this.cb_spk.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
					}					
					...
					sql.add("insert into log_sanggup_m(no_sanggup,no_spk,kode_lokasi,tgl_input,nik_user,periode,tanggal,nik_buat,keterangan,no_tap,kode_vendor,total,no_dokumen,modul) values "+
							"('"+this.e_nb.getText()+"','-','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_buat.getText()+"','"+this.e_ket.getText()+"','"+this.cb_spph.getText()+"','"+this.kodeVendor+"',"+nilaiToFloat(this.e_total.getText())+",'"+this.e_dok.getText()+"','PL')");					
			...
			case "clear" :
				if (modalResult == mrOk)
					...
					this.cb_spk.setSQL("select a.no_spk, a.keterangan from log_spk_m a "+			                   
									   "where a.no_sanggup='-' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='PL'",["no_spk","keterangan"],false,["No SPK","Deskripsi"],"and","Data SPK",true);						
				break;
			case "simpan" :															
			case "ubah" :																											
				this.preView = "1";												
	...
	doChange:function(sender){
		...
		if (sender == this.cb_spk && this.cb_spk.getText()!="") {			
			var strSQL = "select a.kode_vendor,a.nama,a.alamat,b.nilai+b.ppn as total,d.no_spph,d.keterangan as ket_spph "+
	...
	doLoad3:function(sender){																				
		var strSQL = "select a.no_sanggup,convert(varchar,a.tanggal,103) as tgl,a.keterangan,b.nama,a.total as nilai "+
					 ...
					 "where d.no_pks='-' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='PL'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select cc.no_spk,a.keterangan,a.tanggal,a.nik_buat,isnull(b.no_gambar,'-') as no_gambar,a.kode_vendor,a.no_sanggup,a.no_dokumen,c.no_tap,c.no_spph,c.keterangan as ket_spph  "+
							 "from log_sanggup_m a "+
