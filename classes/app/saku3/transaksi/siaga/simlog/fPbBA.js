window.app_saku3_transaksi_siaga_simlog_fPbBA = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_simlog_fPbBA.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_simlog_fPbBA";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Permintaan Bayar - ADK / Uang Muka", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,233], childPage:["Data Akru","Data Permohonan", "Jurnal+","KPA Anggaran","Lampiran","Ref Data Akru","Verifikasi"]});		
		this.sgh = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:7,tag:0,
				colTitle:["No Hutang","Tanggal","Keterangan","Akun AP","Nilai","Saldo","Mitra"],
				colWidth:[[6,5,4,3,2,1,0],[200,100,100,70,290,70,100]],					
				dblClick:[this,"doDoubleClickh"],readOnly:true,colFormat:[[4,5],[cfNilai,cfNilai]],autoAppend:false,defaultRow:1});
		this.sgnh = new portalui_sgNavigator(this.pc1.childPage[0],{bound:[1,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:3,grid:this.sgh});		
		...
		this.e_saldo = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,11,220,20],caption:"Saldo Hutang", tag:4, readOnly:true, tipeText:ttNilai, text:"0"});				
		this.e_hutang = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,12,220,20],caption:"Nilai Bayar", tag:4, tipeText:ttNilai, text:"0",change:[this,"doChange"]});				
				

		this.sg = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:6,tag:9,
		...
		this.e_latar = new saiMemo(this.pc1.childPage[4],{bound:[20,12,500,50],caption:"Latar Belakang",tag:2});
		this.e_lain = new saiMemo(this.pc1.childPage[4],{bound:[540,12,450,50],caption:"Aspek Lain",tag:2});		
		this.e_strategis = new saiMemo(this.pc1.childPage[4],{bound:[20,13,500,50],caption:"Aspek Strategis",tag:2});
		this.e_bisnis = new saiMemo(this.pc1.childPage[4],{bound:[20,14,500,50],caption:"Aspek Bisnis",tag:2});
		this.e_teknis = new saiMemo(this.pc1.childPage[4],{bound:[20,15,500,50],caption:"Spesifikasi Teknis",tag:2});
		
		this.sg5 = new saiGrid(this.pc1.childPage[5],{bound:[1,5,this.pc1.width-5,this.pc1.height-40],colCount:9,tag:9,
		...
		this.sgn5 = new portalui_sgNavigator(this.pc1.childPage[5],{bound:[1,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:bsAll,grid:this.sg5});		


		this.e_noapp = new saiLabelEdit(this.pc1.childPage[6],{bound:[20,15,450,20],caption:"No Verifikasi", tag:9, readOnly:true});
		this.e_memo = new saiMemo(this.pc1.childPage[6],{bound:[20,12,450,150],caption:"Catatan",tag:9, readOnly:true});
...
window.app_saku3_transaksi_siaga_simlog_fPbBA.extend(window.childForm);
window.app_saku3_transaksi_siaga_simlog_fPbBA.implement({
	dataHutang:function(sender){				
		if (this.e_periode.getText()!="") {
			if (this.app._userStatus == "A") var vPP = " "; else var vPP = "z.kode_bidang ='"+this.app._kodeBidang+"' and ";
			var strSQL = "select a.no_hutang,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.akun_hutang,a.nilai,a.nilai-isnull(pb,0) as saldo,b.kode_vendor+' - '+b.nama as vendor "+
					 ...
					 "                left join (select kode_lokasi,ref1,sum(nilai_ref) as pb "+
					 "                           from gr_pb_m where kode_lokasi='"+this.app._lokasi+"' and modul='PBAKRU' "+
					 "                           group by kode_lokasi,ref1) c on a.no_hutang=c.ref1 and a.kode_lokasi=c.kode_lokasi "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sgh.appendData([line.no_hutang,line.tgl,line.keterangan,line.akun_hutang,floatToNilai(line.nilai),floatToNilai(line.saldo),line.vendor]);
				}
			} else this.sgh.clear(1);							
		}
	},
	doDoubleClickh: function(sender, col , row) {		
		if (this.sgh.cells(0,row) != "") {
			this.pc1.setActivePage(this.pc1.childPage[1]);																		
			this.e_nohutang.setText(this.sgh.cells(0,row));
			this.e_tanggal.setText(this.sgh.cells(1,row));
			this.e_kethut.setText(this.sgh.cells(2,row));
			this.e_akun.setText(this.sgh.cells(3,row));			
			this.e_saldo.setText(this.sgh.cells(5,row));
			if (this.stsSimpan == 1) this.e_hutang.setText(this.sgh.cells(5,row));
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,4])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from angg_r where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='PBAKRU'");						
					}	
		 			...
		 			sql.add("insert into gr_pb_m (no_pb,kode_lokasi,periode,nik_user,tgl_input,tanggal,due_date,no_dokumen,keterangan,atensi,kode_curr,kurs,nilai_curr,nilai,modul,kode_pp,nik_buat,nik_tahu,progress,no_ver,no_spb,no_kas,ref1,ref2,no_app,  latar,strategis,bisnis,teknis,lain,nik_sah,nik_ver,jenis,jab1,jab2,jab3,jab4, nilai_ref) values "+
		 					"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.dp_d1.getDateString()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.e_atensi.getText()+"','"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+","+nilaiToFloat(this.e_nilaiCurr.getText())+","+nilaiToFloat(this.e_nilai.getText())+",'PBAKRU','"+this.cb_pp.getText()+"','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','0','-','-','-','"+this.e_nohutang.getText()+"','-','-','"+this.e_latar.getText()+"','"+this.e_strategis.getText()+"','"+this.e_bisnis.getText()+"','"+this.e_teknis.getText()+"','"+this.e_lain.getText()+"','"+this.cb_sah.getText()+"','"+this.cb_ver.getText()+"','"+this.c_jenis.getText()+"','"+this.c_jab1.getText()+"','"+this.c_jab2.getText()+"','"+this.c_jab3.getText()+"','"+this.c_jab4.getText()+"',  "+nilaiToFloat(this.e_hutang.getText())+")");		
					...
					sql.add("insert into gr_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
							 "('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.e_akun.getText()+"','"+this.e_ket.getText()+"','D','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+","+parseNilai(this.e_hutang.getText())+","+hutIDR+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','PBAKRU','HUTANG','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");		 

					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){								
								sql.add("insert into gr_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i)+"','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+","+parseNilai(this.sg.cells(4,i))+","+parseNilai(this.sg.cells(5,i))+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','PBAKRU','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
							}
						}
					}
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','PBAKRU','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.cb_pp.getText()+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(2,i))+","+nilai+")");
							}
						}
					}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :		
				...
				if (nilaiToFloat(this.e_hutang.getText()) > nilaiToFloat(this.e_saldo.getText())) {
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai_curr "+
		             "from gr_pb_m a "+				 
					 "where nik_user = '"+this.app._userLog+"' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'PBAKRU' and a.progress in ('0','R','V')";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg.appendData([line.no_hutang,line.tgl,line.keterangan,line.akun_hutang,floatToNilai(line.nilai),floatToNilai(line.saldo),line.vendor]);
					}
					this.doDoubleClickh(this.sgh,0,0);
				} else this.sg.clear(1);						
				var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk "+
							"from hutang_j a inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
							"                inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+													
							"                left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and substring(a.periode,1,4)=d.tahun "+													
							"where a.no_hutang = '"+this.noHut+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg5.clear();
					for (var i in data.rs.rows){
