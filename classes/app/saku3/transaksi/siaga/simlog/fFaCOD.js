window.app_saku3_transaksi_siaga_simlog_fFaCOD = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_simlog_fFaCOD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_simlog_fFaCOD";
		this.maximize();		
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penerimaan Barang via COD", 0);	
		
		uses("saiCBBL;saiEdit;datePicker;");	
		
		this.pc2 = new pageControl(this,{bound:[10,10,1000,480], childPage:["Data Penerimaan","Cari Data"]});				
		...
		this.cb_pesan = new saiCBBL(this.pc2.childPage[0],{bound:[20,12,220,20],caption:"No Request", multiSelection:false, maxLength:10, tag:1, change:[this,"doChange"]});
		this.e_npesan = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,16,200,20],caption:"Sisa Nilai (IDR)", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,377], childPage:["Data Penerimaan","Daftar Barang Req"]});				
		this.cb_lokasi = new saiCBBL(this.pc1.childPage[0],{bound:[20,13,220,20],caption:"Lokasi Barang", multiSelection:false, maxLength:10, tag:1, change:[this,"doChange"]});
		this.cb_klp = new saiCBBL(this.pc1.childPage[0],{bound:[20,14,220,20],caption:"Jenis Barang", multiSelection:false, maxLength:10, tag:1, change:[this,"doChange"]});		
		...
		this.e_seri = new portalui_saiLabelEdit(this.pc1.childPage[0],{bound:[520,18,432,20],caption:"Nomor Seri",maxLength:50, tag:1});
		this.e_merk = new portalui_saiLabelEdit(this.pc1.childPage[0],{bound:[20,17,432,20],caption:"Merk",maxLength:100, tag:1});
		this.e_tipe = new portalui_saiLabelEdit(this.pc1.childPage[0],{bound:[520,17,432,20],caption:"Tipe",maxLength:100, tag:1});
		this.c_curr = new saiCB(this.pc1.childPage[0],{bound:[20,20,200,20],caption:"Mt Uang",readOnly:true,tag:2});		
		this.e_kurs = new saiLabelEdit(this.pc1.childPage[0],{bound:[250,20,200,20],caption:"Kurs", tag:1, tipeText:ttNilai, text:"0"});		
		...
		this.cb_pp1 = new saiCBBL(this.pc1.childPage[0],{bound:[20,18,220,20],caption:"PP Aktap", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});
		...
		this.cb_vendor = new saiCBBL(this.pc1.childPage[0],{bound:[20,13,220,20],caption:"Mitra", multiSelection:false, maxLength:10, tag:1});
		this.cb_klpakun = new saiCBBL(this.pc1.childPage[0],{bound:[20,14,220,20],caption:"Kelompok Akun", readOnly:true, tag:1});
		...
		this.e_persen = new saiLabelEdit(this.pc1.childPage[0],{bound:[272,16,180,20],caption:"% Susut [Tahun]", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		...
		this.cb_pesan2 = new saiCBBL(this.pc2.childPage[1],{bound:[20,12,220,20],caption:"No Request", multiSelection:false, maxLength:10, tag:9, change:[this,"doChange"]});
		...
		try {
			...
			var strSQL = "select a.no_pesan,a.keterangan "+ 
						 ...
						 "where a.kode_lokasi = '"+this.app._lokasi+"' and a.progress ='2' and b.jenis='COD'"; 
			this.cb_pesan2.setSQL(strSQL,["no_pesan","keterangan"],false,["No Request","Deskripsi"],"and","Data Request",true);
...
window.app_saku3_transaksi_siaga_simlog_fFaCOD.extend(window.childForm);
window.app_saku3_transaksi_siaga_simlog_fFaCOD.implement({
	doLoad : function() {			
		if (this.cb_fa2.getText()!="" && this.cb_pesan2.getText()!="") {			
			...
			var strSQL = "select a.no_pesan,a.keterangan "+ 
						 "from log_pesan_m a "+
						 "where a.kode_lokasi = '"+this.app._lokasi+"' and a.no_pesan='"+this.cb_pesan2.getText()+"'"; 
			this.cb_pesan.setSQL(strSQL,["no_pesan","keterangan"],false,["No Request","Deskripsi"],"and","Data Request",true);
			this.cb_pesan.setText(this.cb_pesan2.getText());
			...
			if (typeof data == "object"){
				...
				if (line != undefined){
					...
					this.e_total.setText(floatToNilai(line.nilai_curr));
			...
			var strSQL = "select a.nilai - isnull(c.n_aset,0) as sisa, a.kode_akun "+
						 "from log_pesan_m a "+
						 "					   left join (select kode_lokasi,no_bukti,sum(nilai) as n_aset "+
						 "								 from fa_nilai where no_bukti<>'"+this.cb_pesan.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
						 "								 group by kode_lokasi,no_bukti) c on a.kode_lokasi=c.kode_lokasi and a.no_pesan = c.no_bukti "+
						 "where a.no_pesan='"+this.cb_pesan.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){
					var sisa = parseFloat(line.sisa);
					this.e_npesan.setText(floatToNilai(sisa));					
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 1) {
						...
						if (nilaiToFloat(this.e_umur.getText()) == 0) var nsusut = 0;
						else var nsusut = Math.round(nilaiToFloat(this.e_nilai.getText()) / nilaiToFloat(this.e_umur.getText()));
						...
						for (var i=0;i < jml;i++){								
							...
							nbfa2 = formatID + nu;		
							var nilaiIDRtmp = nilaiToFloat(this.e_kurs.getText()) * nilaiToFloat(this.e_nilai.getText());	
							sql.add("insert into fa_asset(no_fa,barcode,kode_lokasi,kode_lokfa,kode_klpfa,kode_klpakun,kode_akun,umur,persen,nama,merk,tipe,no_seri,nilai,nilai_residu,kode_pp,kode_pp_susut,tgl_perolehan,tgl_susut,periode,periode_susut,progress,nik_user,tgl_input,catatan,nik_pnj,nilai_susut,jenis,no_baps,kode_vendor,no_po,  kode_curr,kurs,nilai_curr) values "+
									"('"+nbfa2+"','"+nbfa2+"','"+this.app._lokasi+"','"+this.cb_lokasi.getText()+"','"+this.cb_klp.getText()+"','"+this.cb_klpakun.getText()+"','"+this.kodeakun+"',"+parseNilai(this.e_umur.getText())+","+parseNilai(this.e_persen.getText())+",'"+this.e_nama.getText()+"','"+this.e_merk.getText()+"','"+this.e_tipe.getText()+"','"+this.e_seri.getText()+"',"+nilaiIDRtmp+","+parseNilai(this.e_residu.getText())+",'"+this.cb_pp1.getText()+"','"+this.cb_pp2.getText()+"','"+this.dp_d2.getDateString()+"','"+this.dp_d3.getDateString()+"','"+periode+"','"+periodeSusut+"','1','"+this.app._userLog+"',getdate(),'-','-',"+nsusut+",'"+this.jenis+"','"+this.cb_pesan.getText()+"','"+this.cb_vendor.getText()+"','"+this.cb_pesan.getText()+"', '"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+","+nilaiToFloat(this.e_nilai.getText())+")");														
							sql.add("insert into fa_nilai(no_fa,kode_lokasi,no_bukti,dc,nilai,periode, kode_curr,kurs,nilai_curr) values "+
									"('"+nbfa2+"','"+this.app._lokasi+"','"+this.cb_pesan.getText()+"','D',"+nilaiIDRtmp+",'"+this.e_periode.getText()+"', '"+this.c_curr.getText()+"',1,"+nilaiToFloat(this.e_nilai.getText())+")");						
						}
					}
					else {
						var data = this.dbLib.getDataProvider(
									...
									"			from fa_nilai where kode_lokasi='"+this.app._lokasi+"' group by no_fa "+
						...
						sql.add("insert into fa_asset(no_fa,barcode,kode_lokasi,kode_lokfa,kode_klpfa,kode_klpakun,kode_akun,umur,persen,nama,merk,tipe,no_seri,nilai,nilai_residu,kode_pp,kode_pp_susut,tgl_perolehan,tgl_susut,periode,periode_susut,progress,nik_user,tgl_input,catatan,nik_pnj,nilai_susut,jenis,no_baps,kode_vendor,no_po, kode_curr,kurs,nilai_curr) values "+
								"('"+nbfa2+"','"+nbfa2+"','"+this.app._lokasi+"','"+this.cb_lokasi.getText()+"','"+this.cb_klp.getText()+"','"+this.cb_klpakun.getText()+"','"+this.kodeakun+"',"+parseNilai(this.e_umur.getText())+","+parseNilai(this.e_persen.getText())+",'"+this.e_nama.getText()+"','"+this.e_merk.getText()+"','"+this.e_tipe.getText()+"','"+this.e_seri.getText()+"',"+nilaiIDRtmp+","+parseNilai(this.e_residu.getText())+",'"+this.cb_pp1.getText()+"','"+this.cb_pp2.getText()+"','"+this.dp_d2.getDateString()+"','"+this.dp_d3.getDateString()+"','"+periode+"','"+periodeSusut+"','1','"+this.app._userLog+"',getdate(),'-','-',"+nsusut+",'"+this.jenis+"','"+this.cb_pesan.getText()+"','"+this.cb_vendor.getText()+"','"+this.cb_pesan.getText()+"',  '"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+","+nilaiToFloat(this.e_nilai.getText())+" )");
						sql.add("insert into fa_nilai(no_fa,kode_lokasi,no_bukti,dc,nilai,periode, kode_curr,kurs,nilai_curr) values "+
								"('"+nbfa2+"','"+this.app._lokasi+"','"+this.cb_pesan.getText()+"','D',"+nilaiIDRtmp+",'"+this.e_periode.getText()+"', '"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+","+nilaiToFloat(this.e_nilai.getText())+")");						
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					this.stsSimpan=1;
					this.e_jml.setReadOnly(false);
					this.e_fa.setText("");
					
					var strSQL = "select a.no_pesan,a.keterangan "+ 
								"from log_pesan_m a "+
								"		inner join log_justerima_m b on a.no_terima=b.no_terima and a.kode_lokasi=b.kode_lokasi "+
								"		left join (select kode_lokasi,no_bukti,sum(nilai) as n_aset from fa_nilai "+
								"				  where kode_lokasi='"+this.app._lokasi+"' "+
								"				  group by kode_lokasi,no_bukti) c on a.kode_lokasi=c.kode_lokasi and a.no_pesan = c.no_bukti "+
								"where a.nilai > isnull(c.n_aset,0) and a.kode_lokasi = '"+this.app._lokasi+"' and a.progress ='2' and b.jenis='COD'"; 
			...
			case "ubah" :	
				this.preView="1";								
				if (nilaiToFloat(this.e_npesan.getText()) < (nilaiToFloat(this.e_total.getText()) * nilaiToFloat(this.e_kurs.getText())) ){
					system.alert(this,"Transaksi tidak valid.","Total Perolehan melebihi Sisa Nilai Pesan.");
					return false;
				}
	    			...
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {
								...
								this.nama_report="server_report_saku3_siaga_simlog_rptCod";
								this.filter2 = "where a.kode_lokasi='"+this.app._lokasi+"' and a.no_bukti='"+this.cb_pesan.getText()+"' ";
								...
								system.info(this,"Transaksi telah sukses tereksekusi (No PR : "+ this.cb_pesan.getText()+")","");							
								this.clearLayar();
							} 
	...
	doChange:function(sender){	
		if (sender == this.cb_pesan) {			
			var data = this.dbLib.getDataProvider("select a.kode_curr,a.kurs, a.nilai - isnull(c.n_aset,0) as sisa, a.kode_akun "+
			                      "from log_pesan_m a "+
								  "					   left join (select kode_lokasi,no_bukti,sum(nilai) as n_aset "+
								  "								 from fa_nilai where kode_lokasi='"+this.app._lokasi+"' "+
								  "								 group by kode_lokasi,no_bukti) c on a.kode_lokasi=c.kode_lokasi and a.no_pesan = c.no_bukti "+
								  "where a.nilai > isnull(c.n_aset,0) and a.no_pesan='"+this.cb_pesan.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){		
					this.c_curr.setText(line.kode_curr);
					this.e_kurs.setText(floatToNilai(line.kurs));
					if (this.stsSimpan==1) this.e_npesan.setText(floatToNilai(line.sisa));					
					this.akunAktap = line.kode_akun;
			...
			var strSQL = "select no_urut,item,merk,tipe,catatan,jumlah,nilai,jumlah*nilai as total from log_pesan_d where no_pesan='"+this.cb_pesan.getText()+"' and kode_lokasi='"+this.app._lokasi+"' order by no_urut";							
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line2;
				this.sg.clear();
		...
		if (sender == this.cb_pesan2 && this.cb_pesan2.getText()!="") {
			this.cb_fa2.setSQL("select no_fa, nama from fa_asset where progress='1' and no_baps='"+this.cb_pesan2.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_fa","nama"],false,["No FA","Nama"],"where","Data Aset",true);			
		}
	}
