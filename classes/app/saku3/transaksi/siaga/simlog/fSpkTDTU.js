window.app_saku3_transaksi_siaga_simlog_fSpkTDTU = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_simlog_fSpkTDTU.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_simlog_fSpkTDTU";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form SPK Pemilihan Langsung", 0);	
		...
		this.cb_tap = new saiCBBL(this.pc2.childPage[0],{bound:[20,18,220,20],caption:"No Tap", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});				
		this.e_vendor = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,12,450,20],caption:"Mitra", readOnly:true});								
		this.e_gar = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,12,200,20],caption:"Saldo Budget", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,270], childPage:["Item Barang","File Dok"]});						
...
window.app_saku3_transaksi_siaga_simlog_fSpkTDTU.extend(window.childForm);
window.app_saku3_transaksi_siaga_simlog_fSpkTDTU.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into log_spk_m(no_spk,no_pks,kode_lokasi,tgl_input,nik_user,periode,tanggal,nik_buat,keterangan,no_tap,kode_vendor,total,no_dokumen,modul,no_sanggup) values "+
							"('"+this.e_nb.getText()+"','-','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_buat.getText()+"','"+this.e_ket.getText()+"','"+this.cb_tap.getText()+"','"+this.kodeVendor+"',"+nilaiToFloat(this.e_total.getText())+",'"+this.e_dok.getText()+"','TD','-')");					
					...
					for (var i=0;i < this.sg4.getRowCount();i++){
						if (this.sg4.rowValid(i)){
							sql.add("insert into log_spk_d(no_spk,kode_lokasi,no_urut,item,merk,tipe,catatan,jumlah,harga,ppn,kode_klpfa) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',"+this.sg4.cells(0,i)+",'"+this.sg4.cells(1,i)+"','"+this.sg4.cells(2,i)+"','"+this.sg4.cells(3,i)+"','"+this.sg4.cells(4,i)+"',"+nilaiToFloat(this.sg4.cells(5,i))+","+nilaiToFloat(this.sg4.cells(6,i))+","+nilaiToFloat(this.sg4.cells(7,i))+",'"+this.sg4.cells(9,i)+"')");
						}
					}	
					...
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) "+
							"select '"+this.e_nb.getText()+"','LOGSPK',kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,'D',0,"+nilaiToFloat(this.e_total.getText())+" "+
							"from angg_r where no_bukti='"+this.noPesan +"'");
			...
			case "clear" :
				if (modalResult == mrOk)
					...
					this.cb_tap.setSQL("select a.no_tap, a.keterangan from log_tap_m a "+
			                   ...
			                   "where a.no_spk='-' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='TD'",["no_tap","keterangan"],false,["No TAP","Deskripsi"],"and","Data Penetapan",true);					
				break;
			case "simpan" :															
			case "ubah" :																											
				this.preView = "1";												
				...
				*/
				...
				if (nilaiToFloat(this.e_total.getText()) > nilaiToFloat(this.e_gar.getText())) {
					system.alert(this,"Transaksi tidak valid.","Total SPK melebihi Saldo Budget");
					return false;
				}
				if (parseFloat(this.app._periode) < parseFloat(this.e_periode.getText())){
	...
	//kontrol budget tidak dari justifikasi, tp dari saldo buget setahun
	doHitungGar: function(){				
		var sisa = 0;		
		if (this.stsSimpan == 1) 		
			var data = this.dbLib.getDataProvider("select fn_cekagg2Thn('"+this.ppPesan+"','"+this.app._lokasi+"','"+this.akunPesan+"','"+this.drkPesan+"','"+this.periodePesan+"') as gar ",true);
		else var data = this.dbLib.getDataProvider("select fn_cekagg3Thn('"+this.ppPesan+"','"+this.app._lokasi+"','"+this.akunPesan+"','"+this.drkPesan+"','"+this.periodePesan+"','"+this.e_nb.getText()+"') as gar ",true);
		
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			var line = data.rs.rows[0];
			data = line.gar.split(";");
			sisa = parseFloat(data[0]) - parseFloat(data[1]) + this.nilaiPesan; // tambahkan budget keep saat pesan
	...
	doChange:function(sender){
		...
		if (sender == this.cb_tap && this.cb_tap.getText()!="") {			
			var strSQL = "select no_spph "+
			             "from log_tap_m  "+
						 "where no_tap='"+this.cb_tap.getText()+"'";
			...
			var strSQL = "select no_pesan,  jenis,kode_akun,kode_pp,kode_drk,periode, nilai  "+
			             "from log_pesan_m "+
						 "where no_spph='"+this.noSPPH+"'";
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){				
					...
					this.jenisGar = line.jenis;

					this.akunPesan = line.kode_akun;
					this.ppPesan = line.kode_pp;
					this.drkPesan = line.kode_drk;
					this.periodePesan = line.periode;
					this.nilaiPesan = parseFloat(line.nilai);

					this.doHitungGar();
				}
			}
		
			var strSQL = "select a.kode_vendor,a.nama,a.alamat,b.nilai+b.ppn as total from vendor a inner join log_nego_m b on a.kode_vendor=b.kode_vendor and a.kode_lokasi=b.kode_lokasi "+
			...
			if (this.stsSimpan == 1) {				
				var strSQL = "select a.no_urut,a.item,a.merk,a.tipe,a.catatan,a.jumlah,a.nilai_nego,ppn_nego as ppn,a.jumlah*(a.nilai_nego+a.ppn_nego) as total, e.kode_klpfa, e.nama "+
	    			...
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								...
								this.filter2 = " where a.kode_lokasi='"+this.app._lokasi+"' and k.no_spk='"+this.e_nb.getText()+"' ";
	...
	doLoad3:function(sender){																				
		var strSQL = "select a.no_spk,convert(varchar,a.tanggal,103) as tgl,a.keterangan,b.nama,a.total as nilai "+
					 ...
					 "where a.no_sanggup='-' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_pks='-' and a.modul='TD'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				this.cb_tap.setSQL("select no_tap, keterangan from log_tap_m where no_spk='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='TD'",["no_tap","keterangan"],false,["No TAP","Deskripsi"],"and","Data Penetapan",true);			
				
				var strSQL = "select a.keterangan,a.tanggal,a.nik_buat,b.no_gambar,a.kode_vendor,c.no_tap,a.no_dokumen "+
				...
				var strSQL = "select a.no_urut,a.item,a.merk,a.tipe,a.catatan,a.jumlah,a.harga,a.ppn,a.jumlah*(a.harga+a.ppn) as total,a.kode_klpfa,b.nama "+
							 "from log_spk_d a "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line2 = data.rs.rows[i];												
						this.sg4.appendData([line2.no_urut,line2.item,line2.merk,line2.tipe,line2.catatan,floatToNilai(line2.jumlah),floatToNilai(line2.harga),floatToNilai(line2.ppn),floatToNilai(line2.total),line2.kode_klpfa,line2.nama]);
					}
				} else this.sg4.clear(1);											
			}									
		} catch(e) {alert(e);}
