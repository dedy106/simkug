window.app_saku3_transaksi_siaga_simlog_fSph = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_simlog_fSph.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_simlog_fSph";
		...
		this.cb_vendor = new saiCBBL(this.pc2.childPage[0],{bound:[20,13,220,20],caption:"Mitra", readOnly:true, change:[this,"doChange"],tag:1}); //multiSelection:false, maxLength:10, tag:0,
		...
		this.cb_buat = new saiCBBL(this.pc2.childPage[0],{bound:[20,17,220,20],caption:"Yang Menerima", multiSelection:false, maxLength:10, tag:1});				
...
window.app_saku3_transaksi_siaga_simlog_fSph.extend(window.childForm);
window.app_saku3_transaksi_siaga_simlog_fSph.implement({	
	...
	doSelectDate: function(sender, y,m,d){
		...
		if (this.stsSimpan == 1) {
			...
			this.cb_spph.setSQL("select no_spph, keterangan from log_spph_m where periode<='"+this.e_periode.getText()+"' and modul='TL' and no_tap='-' and kode_lokasi='"+this.app._lokasi+"'",["no_spph","keterangan"],false,["No SPPH","Deskripsi"],"and","Data SPPH",true);			
			this.doClick();				
		}
	},
	doChange:function(sender){
		...
		if (sender == this.cb_spph && this.cb_spph.getText()!="") {
			if (this.stsSimpan == 1) {				
				...
				var strSQL = "select kode_vendor from log_spph_vendor where no_spph='"+this.cb_spph.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_sph,convert(varchar,a.tanggal,103) as tgl,a.keterangan,b.nama,a.nilai+a.ppn as nilai "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_nego='-' and a.modul='TL'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
