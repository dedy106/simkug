window.app_saku3_transaksi_siaga_simlog_fSpph = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_simlog_fSpph.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_simlog_fSpph";
		...
		this.e_ket = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,17,450,20],caption:"Perihal", maxLength:150,tag:1});						
		...
		this.cb_buat = new saiCBBL(this.pc2.childPage[0],{bound:[20,13,220,20],caption:"Yang Membuat", multiSelection:false, maxLength:10, tag:1});						
		this.cb_app = new saiCBBL(this.pc2.childPage[0],{bound:[20,15,220,20],caption:"Yang Menyetujui", multiSelection:false, maxLength:10, tag:1});												
		this.e_dok2 = new saiLabelEdit(this.pc2.childPage[0],{bound:[680,15,300,20],caption:"No Dokumen", readOnly:true,tag:1});						
		...
		this.e_dok = new saiLabelEdit(this.pc1.childPage[3],{bound:[20,11,450,20],caption:"No Dokumen", maxLength:50,tag:1});						
...
window.app_saku3_transaksi_siaga_simlog_fSpph.extend(window.childForm);
window.app_saku3_transaksi_siaga_simlog_fSpph.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into log_spph_m(no_spph,kode_lokasi,tgl_input,nik_user,periode,tanggal,nik_buat,nik_app,keterangan,no_tap,no_dokumen,due_date,modul,no_sanggup) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.e_ket.getText()+"','-','"+this.e_dok.getText()+"','"+this.dp_d2.getDateString()+"','TL','-')");					
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					this.cb_pesan.setSQL("select a.no_pesan,a.keterangan from log_pesan_m a "+
										 ...
										 "where a.progress='2' and a.lok_proses = '"+this.app._lokasi+"' and c.jenis = 'TL' and a.periode<='"+this.e_periode.getText()+"' ",
			...
			case "ubah" :	
				...
				if (jmlVendor != 1){
					system.alert(this,"Transaksi tidak valid.","Mitra untuk penunjukan langsung harus 1.");
					return false;						
				}		
	...
	doChange:function(sender){
		...
		if (sender == this.cb_pesan && this.stsSimpan ==1) {
			...
			if (line2.jenis == "TL") this.e_jenis.setText("TL - PENUNJUKAN LANGSUNG");			
	...
	doLoad3:function(sender){																				
		var strSQL = "select distinct a.no_spph,convert(varchar,a.tanggal,103) as tgl,a.keterangan "+
					 ...
					 "where b.no_sph is null and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_tap='-' and a.modul='TL' ";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
