window.app_saku3_transaksi_siaga_gar_fAppRRADirMulti = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_gar_fAppRRADirMulti.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_gar_fAppRRADirMulti";
...
window.app_saku3_transaksi_siaga_gar_fAppRRADirMulti.extend(window.childForm);
window.app_saku3_transaksi_siaga_gar_fAppRRADirMulti.implement({
	doApp: function() {
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{				
					...
					sql.add("insert into rra_app_m(no_app, kode_lokasi,tanggal,keterangan,modul,periode,no_del,nik_buat,nik_app,nik_user,tgl_input,jenis_form) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','PUSAT','"+this.e_periode.getText()+"','-','"+this.app._userLog+"','"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'APPDIRMUL')");
					...
					for (var i=0;i < this.sg.getRowCount();i++){
						if (this.sg.rowValid(i) && this.sg.cells(0,i) == "APP") {							
							sql.add("insert into anggaran_d (no_agg,kode_lokasi,no_urut,kode_pp,kode_akun,kode_drk,volume,periode,nilai_sat,nilai,dc,satuan,nik_user,tgl_input,modul)  "+		
									...
									"where a.no_pdrk = '"+this.sg.cells(4,i)+"' and a.dc ='D'"); 

							sql.add("insert into rra_app_d(no_app,modul,kode_lokasi,no_bukti,kode_lokbukti,sts_pdrk,catatan,status) values "+
									"('"+this.e_nb.getText()+"','DIRMULTI','"+this.app._lokasi+"','"+this.sg.cells(4,i)+"','"+this.app._lokasi+"','RRD','-','APPROVE')");
