window.app_saku3_transaksi_siaga_gar_fAppRRAmulti = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_gar_fAppRRAmulti.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_gar_fAppRRAmulti";
...
window.app_saku3_transaksi_siaga_gar_fAppRRAmulti.extend(window.childForm);
window.app_saku3_transaksi_siaga_gar_fAppRRAmulti.implement({
	doApp: function() {
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{				
					...
					sql.add("insert into rra_app_m(no_app, kode_lokasi,tanggal,keterangan,modul,periode,no_del,nik_buat,nik_app,nik_user,tgl_input,jenis_form) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','PUSAT','"+this.e_periode.getText()+"','-','"+this.app._userLog+"','"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'APPMULTI')");
					...
					for (var i=0;i < this.sg.getRowCount();i++){
						if (this.sg.rowValid(i) && this.sg.cells(0,i) == "APP") {							
							sql.add("insert into rra_app_d(no_app,modul,kode_lokasi,no_bukti,kode_lokbukti,sts_pdrk,catatan,status) values "+
									"('"+this.e_nb.getText()+"','APPMULTI','"+this.app._lokasi+"','"+this.sg.cells(4,i)+"','"+this.app._lokasi+"','RRR','-','APPROVE')");
	...
	doLoad:function(sender){		
		var strSQL = "select 'USUL' as status,a.kode_lokasi,a.kode_pp+' - '+c.nama as pp,a.no_pdrk as no_bukti,b.no_dokumen,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,a.sts_pdrk as jenis "+
					...
					"where a.modul = 'MULTI' and a.progress in ('0','Z') and a.periode<='"+this.e_periode.getText()+"' and a.no_pdrk like '01-RRA%'";							
