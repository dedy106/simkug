window.app_saku3_transaksi_siaga_fAjuPtg = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fAjuPtg.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fAjuPtg";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,190], childPage:["Item Mata Anggaran","KPA Anggaran"]});
...
window.app_saku3_transaksi_siaga_fAjuPtg.extend(window.childForm);
window.app_saku3_transaksi_siaga_fAjuPtg.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{				
					...
					var nilaiPJidr = nilaiToFloat(this.e_nilaiPJ.getText()) * nilaiToFloat(this.e_kurs.getText());
					sql.add("insert into gr_panjarptg_j(no_ptg,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',99,'"+this.akunPJ+"','"+this.e_ket.getText()+"','C','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+","+parseNilai(this.e_nilaiPJ.getText())+","+nilaiPJidr+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','PJPTG','PANJAR','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");					
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_buat && this.cb_buat.getText()!="" && this.stsSimpan == 1)
			this.cb_panjar.setSQL("select a.no_pb, a.keterangan from gr_pb_m a inner join gr_spb2_m b on a.no_spb=b.no_spb and a.kode_lokasi=b.kode_lokasi and b.no_kas <>'-' "+
								  "where a.ref2='-' and a.modul in ('PJAJU','PJAJULOG') and a.nik_buat='"+this.cb_buat.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",["no_pb","keterangan"],false,["Bukti","Keterangan"],"and","Bukti Panjar",true);			
		
		
		if (sender == this.cb_panjar && this.cb_panjar.getText()!="") {
			var data = this.dbLib.getDataProvider(
					   "select a.periode,a.kode_curr,c.kurs,a.nilai_curr,a.ref1 as akun_panjar,a.no_ver "+
					   "from gr_pb_m a "+
					   "		inner join gr_spb2_m b on a.no_spb=b.no_spb and a.kode_lokasi=b.kode_lokasi "+
			...
			if (this.stsSimpan == 1) {
				var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.nilai_curr "+
							...
							"where a.no_pb = '"+this.cb_panjar.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
					for (var i in data.rs.rows){
	...
	doLoad3:function(sender){																				
		var strSQL = "select distinct a.no_ptg,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai_curr "+
					 "from gr_panjarptg_m a inner join gr_pb_m b on a.no_panjar=b.no_pb and b.modul<>'PJAJUBMHD' "+
					 "inner join karyawan_pp c on a.nik_user=c.nik and a.kode_lokasi=c.kode_lokasi and c.nik='"+this.app._userLog+"' "+
					 "where a.progress in ('0','R','V') and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
