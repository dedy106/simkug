window.app_saku3_transaksi_siaga_fAppDir = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fAppDir.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fAppDir";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Approve Direksi", 0);	
...
window.app_saku3_transaksi_siaga_fAppDir.extend(window.childForm);
window.app_saku3_transaksi_siaga_fAppDir.implement({
	...
	doLoad:function(sender){	
		...
		var strSQL = "select a.tanggal as due_date,a.no_pb as no_bukti,'INPROG' as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,'PBAJU' as modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai_curr as nilai,c.nik+' - '+c.nama as pembuat,a.no_app as no_app,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
					 ...
					 "where a.modul in ('AJU','PJAJU','PJAJUBMHD','BYRBMHD','PBADK','PBCOD','GAJI','SPJ','KES','KLAIM') and a.periode<='"+this.e_periode.getText()+"' and a.progress in ('B') and a.kode_lokasi='"+this.app._lokasi+"' "+
					 ...
					 "from gr_pb_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
					 "               inner join karyawan c on a.nik_buat=c.nik and a.kode_lokasi=c.kode_lokasi "+
					 "where a.modul = 'BMHD' and a.periode<='"+this.e_periode.getText()+"' and a.progress in ('B') and a.kode_lokasi='"+this.app._lokasi+"' "+
					 ...
					 "               		inner join karyawan c on a.nik_buat=c.nik and a.kode_lokasi=c.kode_lokasi "+
					 "where a.periode<='"+this.e_periode.getText()+"' and a.progress in ('B') and a.kode_lokasi='"+this.app._lokasi+"' "+
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.e_modul.getText() == "PBAJU" || this.e_modul.getText() == "BMHD")
						sql.add("update gr_pb_m set no_appdir='"+this.e_nb.getText()+"',progress='"+vStatus+"' where no_pb='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					if (this.e_modul.getText() == "PJPTG") 
						sql.add("update gr_panjarptg_m set no_appdir='"+this.e_nb.getText()+"',progress='"+vStatus+"' where no_ptg='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
