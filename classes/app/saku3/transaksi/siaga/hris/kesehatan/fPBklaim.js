window.app_saku3_transaksi_siaga_hris_kesehatan_fPBklaim = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_hris_kesehatan_fPBklaim.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_hris_kesehatan_fPBklaim";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Permintaan Bayar Ref Klaim", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,233], childPage:["Data Klaim","Data Pengajuan","KPA Anggaran","Lampiran","Verifikasi"]});
		this.cb_klaim = new saiCBBL(this.pc1.childPage[0],{bound:[20,11,220,20],caption:"No Ref Klaim", multiSelection:false, maxLength:10, tag:1, change:[this,"doChange"], rightLabelVisible:false });		
...
window.app_saku3_transaksi_siaga_hris_kesehatan_fPBklaim.extend(window.childForm);
window.app_saku3_transaksi_siaga_hris_kesehatan_fPBklaim.implement({
	isiCBklaim: function(sender){
		this.cb_klaim.setSQL("select no_klaim,keterangan from gr_klaim_m where periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and no_pb='-' and progress='2'",["no_klaim","keterangan"],false,["No Bukti","Deskripsi"],"and","Data Klaim",true);			
	},
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("update gr_klaim_m set progress='2',no_pb='-' where no_pb = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
					}	
					
					sql.add("update gr_klaim_m set progress='3',no_pb='"+this.e_nb.getText()+"' where no_klaim = '"+this.cb_klaim.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','KLAIM','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.cb_pp.getText()+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(2,i))+","+nilai+")");
							}
						}
					}
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_klaim && this.cb_klaim.getText()!=""){
			this.e_ket2.setText(this.cb_klaim.rightLabelCaption);
			this.e_dok.setText(this.cb_klaim.getText());

			var strSQL = "select a.keterangan, a.nilai "+
						 "from gr_klaim_m a "+
						 "where a.no_klaim ='"+this.cb_klaim.getText()+"' and a.nilai > 0 and a.kode_lokasi='"+this.app._lokasi+"' ";
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData(["","","D",line.keterangan,floatToNilai(line.nilai),floatToNilai(line.nilai)]);					
				}
			}
		}		
	},	
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai_curr "+
		             "from gr_pb_m a "+				 
					 "where nik_user = '"+this.app._userLog+"' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'KLAIM' and a.progress in ('0','R','V')";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider(
						   "select a.*,  isnull(b.no_app,'-') as no_app,isnull(b.catatan,'-') as catatan,  c.no_klaim "+						  
						   "from gr_pb_m a inner join gr_klaim_m c on a.no_pb=c.no_pb and a.kode_lokasi=c.kode_lokasi "+
						   "		left join ("+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){
						...
						this.cb_klaim.setSQL("select no_klaim, keterangan from gr_klaim_m "+
											"where no_klaim='"+line.no_klaim+"' and kode_lokasi='"+this.app._lokasi+"' ",
											["no_klaim","keterangan"],false,["No Bukti","Deskripsi"],"and","Data SPPD",true);			
