window.app_saku3_transaksi_siaga_hris_kesehatan_fPBbenefit = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_hris_kesehatan_fPBbenefit.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_hris_kesehatan_fPBbenefit";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Permintaan Bayar Ref Benefit", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,233], childPage:["Data Benefit","Data Pengajuan","KPA Anggaran","Lampiran","Verifikasi"]});
		this.cb_benefit = new saiCBBL(this.pc1.childPage[0],{bound:[20,11,220,20],caption:"No Ref Benefit", multiSelection:false, maxLength:10, tag:1, change:[this,"doChange"], rightLabelVisible:false });		
...
window.app_saku3_transaksi_siaga_hris_kesehatan_fPBbenefit.extend(window.childForm);
window.app_saku3_transaksi_siaga_hris_kesehatan_fPBbenefit.implement({
	isiCBbenefit: function(sender){
		this.cb_benefit.setSQL("select no_kes,keterangan from gr_kes_m where periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and no_pb='-' and progress='2'",["no_kes","keterangan"],false,["No Bukti","Deskripsi"],"and","Data Benefit",true);			
	},
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("update gr_kes_m set progress='2',no_pb='-' where no_pb = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
					}	
					
					sql.add("update gr_kes_m set progress='3',no_pb='"+this.e_nb.getText()+"' where no_kes = '"+this.cb_benefit.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','KES','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.cb_pp.getText()+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(2,i))+","+nilai+")");
							}
						}
					}
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_benefit && this.cb_benefit.getText()!=""){
			this.e_ket2.setText(this.cb_benefit.rightLabelCaption);
			this.e_dok.setText(this.cb_benefit.getText());

			var strSQL = "select b.nama, a.nilai "+
						 "from gr_kes_d a inner join gr_kes_jenis b on a.kode_jenis=b.kode_jenis and a.kode_lokasi=b.kode_lokasi "+
						 "where a.no_kes ='"+this.cb_benefit.getText()+"' and a.nilai > 0 and a.kode_lokasi='"+this.app._lokasi+"' ";
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData(["","","D",line.nama,floatToNilai(line.nilai),floatToNilai(line.nilai)]);					
				}
			}
		}		
	},	
	...
	doClick:function(sender){
		try {
			if (this.e_periode.getText()!= "") {
				...
				this.stsSimpan = 1;
				this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"gr_pb_m","no_pb",this.app._lokasi+"-PBBF"+this.e_periode.getText().substr(2,2)+this.Aperiode+".","0000"));						
				this.e_dok.setFocus();
				setTipeButton(tbSimpan);			
			}		
		}
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_siaga_hris_rptPbBenefit";
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai_curr "+
		             "from gr_pb_m a "+				 
					 "where nik_user = '"+this.app._userLog+"' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'KES' and a.progress in ('0','R','V')";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider(
						   "select a.*,  isnull(b.no_app,'-') as no_app,isnull(b.catatan,'-') as catatan,  c.no_kes "+						  
						   "from gr_pb_m a inner join gr_kes_m c on a.no_pb=c.no_pb and a.kode_lokasi=c.kode_lokasi "+
						   "		left join ("+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){
						...
						this.cb_benefit.setSQL("select no_kes, keterangan from gr_kes_m "+
											"where no_kes='"+line.no_kes+"' and kode_lokasi='"+this.app._lokasi+"' ",
											["no_kes","keterangan"],false,["No Bukti","Deskripsi"],"and","Data SPPD",true);			
						
						this.cb_benefit.setText(line.no_kes);
					} 
				}		
