window.app_saku3_transaksi_siaga_hris_kesehatan_fFinalReimb = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_hris_kesehatan_fFinalReimb.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_hris_kesehatan_fFinalReimb";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Finalisasi Reimburse Biaya Kesehatan", 0);	
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,440], childPage:["Data Approve Reimburse","Daftar Approve Reimburse"]});				
		...
		this.cb_app = new saiCBBL(this.pc1.childPage[0],{bound:[10,18,220,20],caption:"NIK Verifikator", multiSelection:false, maxLength:10, tag:2,readOnly:true});		
		this.bTampil = new button(this.pc1.childPage[0],{bound:[878,18,80,18],caption:"Tampil Data",click:[this,"doTampilClick"]});			
		...
		this.sg = new saiGrid(this.pc2.childPage[0],{bound:[0,5,this.pc2.width-5,this.pc2.height-35],colCount:10,tag:9,
				colTitle:["Status","Catatan","Tanggal Transfer","No Reimburse","Tanggal","Keterangan","Loker","Karyawan","Tgl Kuitansi","Nilai"],
				colWidth:[[9,8,7,6,5,4,3,2,1,0],[100,100,200,200,220,70,120,100,150,80]],
		...
		try {
			...
			this.cb_app.setSQL("select a.nik, a.nama from gr_karyawan a "+
							...
							"where a.kode_lokasi='"+this.app._lokasi+"' and b.sts_oto='APPREIMB' ",["a.nik","a.nama"],false,["NIK","Nama"],"and","Data NIK Approval",true);			
...
window.app_saku3_transaksi_siaga_hris_kesehatan_fFinalReimb.extend(window.childForm);
window.app_saku3_transaksi_siaga_hris_kesehatan_fFinalReimb.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into gr_ver_m(no_ver,kode_lokasi,periode,tanggal,modul,nik_buat,nik_app,keterangan,tgl_input,nik_user,no_del) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','FINALREIMB','-','"+this.cb_app.getText()+"','"+this.e_ket.getText()+"',getdate(),'"+this.app._userLog+"','-')");
							...
							if (this.sg.rowValid(i)){
								if (this.sg.cells(0,i).toUpperCase() != "INPROG") {
									...
									sql.add("insert into gr_ver_d(no_ver,kode_lokasi,modul,no_bukti,status,catatan,no_del) values "+
											"	('"+this.e_nb.getText()+"','"+this.app._lokasi+"','FINALREIMB','"+this.sg.cells(3,i)+"','"+this.sg.cells(0,i)+"','"+this.sg.cells(1,i)+"','-')");									

									sql.add("update gr_kes_m set tgl_transfer='"+this.sg.getCellDateValue(2,i)+"', progress='"+vProgress+"' where no_kes='"+this.sg.cells(3,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try {									
					...
					sql.add("update a set a.progress='1',a.tgl_transfer=null "+
	...
	doTampilClick:function(sender){		
		if (this.e_periode.getText() != "") {
			var data = this.dbLib.getDataProvider("select a.no_kes,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,b.kode_klpjab+' - '+b.nama as loker,a.nik_buat+' - '+c.nama as karyawan,convert(varchar,a.tgl_kuitansi,103) as tgl_kuitansi,isnull(x.nilai,0) as nilai  "+
				"from gr_kes_m a "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData(["INPROG","-",this.dp_d1.getText(),line.no_kes,line.tanggal,line.keterangan,line.loker,line.karyawan,line.tgl_kuitansi,floatToNilai(line.nilai)]);
				}
	...
	doChange:function(sender){
		if (sender == this.e_nb && this.e_nb.getText()!="" && this.stsSimpan == 0) {
			...
			var data = this.dbLib.getDataProvider("select a.no_kes,convert(varchar,a.tgl_transfer,103) as tgl_transfer,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,b.kode_klpjab+' - '+b.nama as loker,a.nik_buat+' - '+c.nama as karyawan,convert(varchar,a.tgl_kuitansi,103) as tgl_kuitansi,isnull(x.nilai,0) as nilai ,z.status,z.catatan   "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData([line.status,line.catatan,line.tgl_transfer,line.no_kes,line.tanggal,line.keterangan,line.loker,line.karyawan,line.tgl_kuitansi,floatToNilai(line.nilai)]);
				}
			} else this.sg.clear(1);
			this.pc1.setActivePage(this.pc1.childPage[0]);
		}
	...
	doDoubleClick: function(sender, col , row) {
		if (this.sg.cells(3,row) != "") {
			var data = this.dbLib.getDataProvider(
				...
				"where a.no_kes='"+this.sg.cells(3,row)+"' and a.kode_lokasi='"+this.app._lokasi+"' ",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg2.clear();
				for (var i in data.rs.rows){
	...
	doLoad:function(sender){						
		var strSQL = "select a.no_ver,a.keterangan,a.nik_app "+
					 "from gr_ver_m a "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.modul='FINALREIMB' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_ver desc";		
