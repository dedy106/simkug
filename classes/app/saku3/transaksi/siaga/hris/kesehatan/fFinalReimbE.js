window.app_saku3_transaksi_siaga_hris_kesehatan_fFinalReimbE = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_hris_kesehatan_fFinalReimbE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_hris_kesehatan_fFinalReimbE";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Finalisasi Reimburse Biaya Kesehatan: Edit", 0);	
		...
		this.e_nb = new saiCBBL(this,{bound:[20,12,223,20],caption:"No Finalisasi", multiSelection:false, maxLength:10, tag:1, readOnly:true,change:[this,"doChange"]});		
		this.e_ket = new saiLabelEdit(this,{bound:[20,14,450,20],caption:"Keterangan", maxLength:150});		
		this.cb_app = new saiCBBL(this,{bound:[20,18,200,20],caption:"NIK Verifikator", multiSelection:false, maxLength:10, tag:2});
		
		this.pc1 = new pageControl(this,{bound:[20,20,900,320], childPage:["Data Pengajuan","Detail Data Reimburse"]});		
		...
		try {
			...
			this.e_nb.setSQL("select no_ver, keterangan from gr_ver_m where modul = 'FINALREIMB' and kode_lokasi='"+this.app._lokasi+"'",["no_ver","keterangan"],false,["No Approve","Keterangan"],"and","Data Finalisasi",true);
		}catch(e){
			systemAPI.alert(e);
		}
	}
};
window.app_saku3_transaksi_siaga_hris_kesehatan_fFinalReimbE.extend(window.childForm);
window.app_saku3_transaksi_siaga_hris_kesehatan_fFinalReimbE.implement({
	...
	doChange:function(sender){
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			...
			var data = this.dbLib.getDataProvider("select a.no_kes,convert(varchar,a.tgl_transfer,103) as tgl_transfer,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,b.kode_loker+' - '+b.nama as loker,a.nik_buat+' - '+c.nama as karyawan,convert(varchar,a.tgl_kuitansi,103) as tgl_kuitansi,isnull(x.nilai,0) as nilai ,z.status,z.catatan   "+
	...
	doDoubleClick: function(sender, col , row) {
		if (this.sg.cells(3,row) != "") {
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];																
					this.sg2.appendData([line.nik,line.nama_kar,line.kode_jenis,line.nama_jenis,floatToNilai(line.saldo),floatToNilai(line.nilai),line.pasien]);
				}
