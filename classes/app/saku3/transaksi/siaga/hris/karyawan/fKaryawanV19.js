window.app_saku3_transaksi_siaga_hris_karyawan_fKaryawanV19 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_hris_karyawan_fKaryawanV19.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_hris_karyawan_fKaryawanV19";
		...
		try {
			...
			this.cb_kode = new saiCBBL(this,{bound:[20,10,220,20],caption:"ID",maxLength:10,rightLabelVisible:false,readOnly:true,multiSelection:false,change:[this,"doChange"]});
			...
			this.e_univ = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,26,400,20],caption:"Perguruan Tinggi", maxLength:50, tag:9});		
...
window.app_saku3_transaksi_siaga_hris_karyawan_fKaryawanV19.extend(window.childForm);
window.app_saku3_transaksi_siaga_hris_karyawan_fKaryawanV19.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into gr_keluarga(nik,no_urut,nama,tempat,tgl_lahir,sts_kel,sex,institusi,kode_lokasi,no_telp,nik2,alamat,kota,kodepos,kode_prov,flag_hidup,flag_kerja,flag_tanggung,flag_anak,ayah,ibu) values "+  
							"('"+this.cb_kode.getText()+"','99','"+this.e_nama.getText()+"','"+this.e_tempat.getText()+"','"+this.dp_d1.getDateString()+"','01','"+this.c_sex.getText()+"','-','"+this.app._lokasi+"','-','"+this.e_ktp.getText()+"','-','-','-','-','Y','Y','Y','T','"+this.e_ayah.getText()+"','"+this.e_ibu.getText()+"')");		
		...
		try{					
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){				
				...
				var data = this.dbLib.getDataProvider("select a.nama,a.tempat,convert(varchar,a.tgl_lahir,103) as tgl_lahir,a.sts_kel,b.nama as nama_sts,a.sex, "+
													  ...
													  "where a.sts_kel<>'01' and  a.nik = '"+this.cb_kode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' ",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
					for (var i in data.rs.rows){
