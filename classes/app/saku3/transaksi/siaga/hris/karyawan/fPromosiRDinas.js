window.app_saku3_transaksi_siaga_hris_karyawan_fPromosiRDinas = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_hris_karyawan_fPromosiRDinas.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_hris_karyawan_fPromosiRDinas";
...
window.app_saku3_transaksi_siaga_hris_karyawan_fPromosiRDinas.extend(window.childForm);
window.app_saku3_transaksi_siaga_hris_karyawan_fPromosiRDinas.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("update gr_dinas set tgl_akhir='"+tglAkhir+"',flag_seb='"+id+"' where nik='"+this.cb_nik.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and flag_aktif='1'");																				
					...
					sql.add("insert into gr_dinas(nik,no_sk,kode_lokasi,kode_loker,kode_grade,kode_vendor,kode_dir,kode_dept,kode_jab,kode_pb,sts_sdm,mk_tahun,mk_bulan,nik_user,tgl_input,flag_aktif,flag_form,  tgl_awal,tgl_akhir,kode_kota,kode_tipe,kode_reason,kode_so,flag_seb, no_dok,kode_kelas, tgl_surat,gambar,nik_tg) values "+
							"('"+this.cb_nik.getText()+"','"+id+"','"+this.app._lokasi+"','"+this.cb_unit.getText()+"','"+this.cb_grade.getText()+"','-','"+this.cb_dir.getText()+"','"+this.cb_dept.getText()+"','"+this.cb_jab.getText()+"','-','"+this.cb_sts.getText()+"',0,0,'"+this.app._userLog+"',getdate(),'Z','RDINAS', '"+this.dp_d1.getDateString()+"','2099-12-31','"+this.cb_kota.getText()+"','"+this.cb_tipe.getText()+"','"+this.cb_reason.getText()+"','-','-','"+this.e_dok.getText()+"','"+this.cb_kelas.getText()+"', '"+this.dp_d2.getDateString()+"','"+this.e_file.getText()+"','"+this.e_nik2.getText()+"')");					
	...
	doLoad: function(sender){			
		var strSQL = "select a.no_dok,a.kode_kelas,a.no_sk,a.tgl_awal,convert(varchar,a.tgl_awal,103) as tgl1,convert(varchar,a.tgl_akhir,103) as tgl2,a.flag_aktif,case when a.gambar = '' then '-' else a.gambar end as gambar "+
					 ...
					 "where a.flag_form='RDINAS' and a.nik='"+this.cb_nik.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by tgl_awal desc";		
