window.app_saku3_transaksi_siaga_hris_karyawan_fPromosi = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_hris_karyawan_fPromosi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_hris_karyawan_fPromosi";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Riwayat Kedinasan Karyawan", 0);	
...
window.app_saku3_transaksi_siaga_hris_karyawan_fPromosi.extend(window.childForm);
window.app_saku3_transaksi_siaga_hris_karyawan_fPromosi.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into gr_dinas(nik,no_sk,kode_lokasi,kode_loker,kode_grade,kode_vendor,kode_dir,kode_dept,kode_jab,kode_pb,sts_sdm,mk_tahun,mk_bulan,nik_user,tgl_input,flag_aktif,flag_form,  tgl_awal,tgl_akhir,kode_kota,kode_tipe,kode_reason,kode_so,flag_seb, no_dok,kode_kelas, tgl_surat,gambar,nik_tg) values "+
							"('"+this.cb_nik.getText()+"','"+id+"','"+this.app._lokasi+"','"+this.cb_unit.getText()+"','"+this.cb_grade.getText()+"','-','"+this.cb_dir.getText()+"','"+this.cb_dept.getText()+"','"+this.cb_jab.getText()+"','-','"+this.cb_sts.getText()+"',0,0,'"+this.app._userLog+"',getdate(),'1','RDINAS', '"+this.dp_d1.getDateString()+"','2099-12-31','"+this.cb_kota.getText()+"','"+this.cb_tipe.getText()+"','"+this.cb_reason.getText()+"','-','-','"+this.e_dok.getText()+"','"+this.cb_kelas.getText()+"', '"+this.dp_d2.getDateString()+"','"+this.e_file.getText()+"','"+this.e_nik2.getText()+"')");					
	...
	doDoubleClick: function(sender, col , row) {
		try{			
			if (this.sg1.cells(0,row) != "" ) {		//&& this.sg1.cells(10,row) != "0"
				setTipeButton(tbUbahHapus);
				
				// jika FA = 1 --> update ke grkaraywan else tidak update
				this.flagAktif = this.sg1.cells(10,row);
