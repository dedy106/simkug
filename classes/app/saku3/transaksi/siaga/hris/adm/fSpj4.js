window.app_saku3_transaksi_siaga_hris_adm_fSpj4 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_hris_adm_fSpj4.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_hris_adm_fSpj4";
		...
		this.sg2 = new saiGrid(this.pc2.childPage[1],{bound:[0,5,this.pc2.width-5,this.pc2.height-33],colCount:7,tag:0,
					...
					columnReadOnly:[true,[0,1,4,6],[2,3,5]],
...
window.app_saku3_transaksi_siaga_hris_adm_fSpj4.extend(window.childForm);
window.app_saku3_transaksi_siaga_hris_adm_fSpj4.implement({
	...
	doChangeCell2: function(sender, col, row) {
		if (col == 2 || col == 3 || col == 0) {
			if (col == 0) {
				var data = this.dbLib.getDataProvider("select nilai from gr_spj_harian where kode_grade = '"+this.kodegrade.substr(0,1)+"' and sts_spj = '"+this.sg2.cells(0,row)+"' and kode_lokasi='"+this.app._lokasi+"' and kode_klpjab='"+this.kodeklpjab+"' ",true); //and kode_so='"+this.kodeso+"' 
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){
