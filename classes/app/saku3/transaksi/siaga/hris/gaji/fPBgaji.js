window.app_saku3_transaksi_siaga_hris_gaji_fPBgaji = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_hris_gaji_fPBgaji.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_hris_gaji_fPBgaji";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Permintaan Bayar Ref GAJI", 0);	
		...
		this.c_jenis = new saiCB(this.pc2.childPage[0],{bound:[700,12,220,20],caption:"Jenis Anggaran",items:["OPEX"], readOnly:true,tag:0});	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,233], childPage:["Data Gaji","Data Pengajuan","KPA Anggaran","Lampiran","Verifikasi"]});
		this.cb_klp = new saiCBBL(this.pc1.childPage[0],{bound:[20,10,220,20],caption:"Klp Parameter", multiSelection:false, maxLength:10, tag:2,  change:[this,"doChange"]});		
		this.cb_gaji = new saiCBBL(this.pc1.childPage[0],{bound:[20,11,220,20],caption:"No Ref", multiSelection:false, maxLength:10, tag:1, change:[this,"doChange"], rightLabelVisible:false });		
...
window.app_saku3_transaksi_siaga_hris_gaji_fPBgaji.extend(window.childForm);
window.app_saku3_transaksi_siaga_hris_gaji_fPBgaji.implement({
	isiCBGaji: function(sender){
		//and no_pb='-' no gaji bisa bbrpa kali
		this.cb_gaji.setSQL("select no_gaji,keterangan from gr_gaji_m where periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ",["no_gaji","keterangan"],false,["No Bukti","Deskripsi"],"and","Data Gaji",true);			
	},
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						//bisa lebih dari satu kali --tidak jadi di flag 
						//sql.add("update gr_gaji_m set no_pb='-' where no_pb = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
					}	
					
					//bisa lebih dari satu kali --tidak jadi di flag 
					//sql.add("update gr_gaji_m set no_pb='"+this.e_nb.getText()+"' where no_gaji = '"+this.cb_gaji.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','GAJI','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.cb_pp.getText()+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(2,i))+","+nilai+")");
							}
						}
					}
	...
	doChange:function(sender){		
		...
		if ((sender == this.cb_gaji || sender == this.cb_klp) && this.cb_gaji.getText()!="" && this.stsSimpan==1){
			if (this.cb_klp.getText()!="") {
				this.e_ket2.setText(this.cb_gaji.rightLabelCaption);
				this.e_dok.setText(this.cb_gaji.getText());

				var strSQL = "select x.akun_gaji,sum(x.nilai) as nilai "+
							 "from ( "+

							 "select case when d.kode_klp='GAJI' then b.akun_gaji "+
							 "			  when d.kode_klp='PPH' then b.akun_pph "+
							 "			  when d.kode_klp='POT' then b.akun_pot "+
							 " end as akun_gaji "+

							 ",abs(sum(nilai)) -   (case when d.kode_klp='GAJI' then (case when c.flag_gaji = 'HARIAN' then isnull(f.rem_harian,0) else 0 end) else 0 end) as nilai "+
							 
							 "from gr_gaji_d a "+
							 "inner join gr_gaji_akun b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi "+
							 "inner join gr_karyawan c on a.nik=c.nik and a.kode_lokasi=c.kode_lokasi "+
							 "inner join gr_gaji_param d on a.kode_param=d.kode_param and a.kode_lokasi=d.kode_lokasi and d.kode_klp='"+this.cb_klp.getText()+"' "+
							 ...
							 "left join ( "+
									...
									"where a.kode_lokasi='"+this.app._lokasi+"' and b.flag_rutin = '1' and a.no_gaji = '"+this.cb_gaji.getText()+"' "+
									"group by a.nik) f on a.nik=f.nik "+
										
							 "where a.no_gaji ='"+this.cb_gaji.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' "+
							 "group by isnull(f.rem_harian,0),c.flag_gaji,d.kode_klp, "+
							 "case when d.kode_klp='GAJI' then b.akun_gaji "+
							 "			  when d.kode_klp='PPH' then b.akun_pph "+
							 "			  when d.kode_klp='POT' then b.akun_pot "+
							 " end "+
							 
							 ") x group by x.akun_gaji ";		
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg.appendData([line.akun_gaji,"-","D","-",floatToNilai(line.nilai),floatToNilai(line.nilai)]);
						this.doChangeCell(this.sg,0,i);						
						this.sg.cells(3,i,this.sg.cells(1,i));					
					}
				}
			}
			else system.alert(this,"Kelompok Parameter harus terpilih.","");
		}		
	},	
	...
	doClick:function(sender){
		try {
			if (this.e_periode.getText()!= "") {
				...
				this.stsSimpan = 1;
				this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"gr_pb_m","no_pb",this.app._lokasi+"-PBGJ"+this.e_periode.getText().substr(2,2)+this.Aperiode+".","0000"));						
				this.e_dok.setFocus();
				setTipeButton(tbSimpan);			
			}		
		}
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai_curr "+
		             "from gr_pb_m a "+				 
					 "where nik_user = '"+this.app._userLog+"' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'GAJI' and a.progress in ('0','R','V')";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider(
						   "select a.*,  isnull(b.no_app,'-') as no_app,isnull(b.catatan,'-') as catatan,  c.no_gaji,a.ref1,c.keterangan as ket_gaji "+						  
						   "from gr_pb_m a inner join gr_gaji_m c on a.ref2=c.no_gaji and a.kode_lokasi=c.kode_lokasi "+
						   "		left join ("+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){
						...
						this.cb_gaji.setSQL("select no_gaji, keterangan from gr_gaji_m "+
											"where no_gaji='"+line.no_gaji+"' and kode_lokasi='"+this.app._lokasi+"' ",
											["no_gaji","keterangan"],false,["No Bukti","Deskripsi"],"and","Data Gaji",true);			
						
						this.cb_gaji.setText(line.no_gaji);
						this.cb_klp.setText(line.ref1);
						this.e_ket2.setText(line.ket_gaji);
					} 
				}		
