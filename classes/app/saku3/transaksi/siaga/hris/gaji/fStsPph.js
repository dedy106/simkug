window.app_saku3_transaksi_siaga_hris_gaji_fStsPph = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_hris_gaji_fStsPph.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_hris_gaji_fStsPph";
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select bawah,atas,persen,kurang_seb,nilai_seb from gr_pph21 where kode_lokasi='"+this.app._lokasi+"' order by bawah",true);
...
window.app_saku3_transaksi_siaga_hris_gaji_fStsPph.extend(window.childForm);
window.app_saku3_transaksi_siaga_hris_gaji_fStsPph.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from gr_pph21 where kode_lokasi='"+this.app._lokasi+"'");			
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into gr_pph21(bawah,atas,persen,kurang_seb,nilai_seb,kode_lokasi) values "+
