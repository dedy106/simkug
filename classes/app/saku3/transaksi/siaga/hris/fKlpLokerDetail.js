window.app_saku3_transaksi_siaga_hris_fKlpLokerDetail = function(owner,modul)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_hris_fKlpLokerDetail.prototype.parent.constructor.call(this, owner);
		window.app_saku3_transaksi_siaga_hris_fKlpLokerDetail.prototype.parent.setWidth.call(this, 300);
		window.app_saku3_transaksi_siaga_hris_fKlpLokerDetail.prototype.parent.setHeight.call(this, 400);
		
		this.centerize();
		
		this.className = "app_saku3_transaksi_siaga_hris_fKlpLokerDetail";
		
		this.mouseX = 0;
		this.mouseY = 0;			
		
		this.p1 = new panel(this,{bound:[0,10,this.width - 40,this.height - 100], borderStyle:2});						
		...
		this.e5 = new saiCB(this.p1,{bound:[20,16,220,20], caption:"Jenis",items:["-"]});				
...
window.app_saku3_transaksi_siaga_hris_fKlpLokerDetail.extend(window.commonForm);
window.app_saku3_transaksi_siaga_hris_fKlpLokerDetail.implement({
	...
	doSysMouseDown: function(x, y, button, buttonState){	
		window.app_saku3_transaksi_siaga_hris_fKlpLokerDetail.prototype.parent.doSysMouseDown.call(this,x, y, button, buttonState);
	},
	doSysMouseUp: function(x, y, button, buttonState){	
		window.app_saku3_transaksi_siaga_hris_fKlpLokerDetail.prototype.parent.doSysMouseUp.call(this,x, y, button, buttonState);
	},
	doSysMouseMove: function(x, y, button, buttonState){
		window.app_saku3_transaksi_siaga_hris_fKlpLokerDetail.prototype.parent.doSysMouseMove.call(this,x, y, button, buttonState);
		if (this.isClick)
		{		
			...
			try{
				if (this.e3.getText().toLowerCase() != "posting"){
					if (this.event === "Edit" &&  this.itemParent && this.itemParent.data2 && this.itemParent.data2.getLength() != 0){
						system.alert(this,"Kode ini masih terelasi dengan item terpilih. tidak boleh bertipe selain Posting","");
						return;
					}
				}			
				if (this.e3.getText().toLowerCase() == "posting"){
					if (this.event === "Edit" &&  this.itemParent && this.itemParent.isHasChild() != 0){
						system.alert(this,"Kode  ini mempunyai item dibawahnya. tidak boleh bertipe posting.","");
						return;
					}
				}
			}catch(e){
