window.app_saku3_transaksi_siaga_fAjuPJBMHD = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fAjuPJBMHD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fAjuPJBMHD";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Permintaan Bayar Panjar Referensi BMHD", 0);	
		...
		this.cb_akun = new saiCBBL(this.pc2.childPage[0],{bound:[20,15,220,20],caption:"Akun Panjar", multiSelection:false, maxLength:10, tag:0});
		this.c_jenis = new saiCB(this.pc2.childPage[0],{bound:[700,15,220,20],caption:"Jenis Anggaran",items:["OPEX","CAPEX"], readOnly:true,tag:0});	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,210], childPage:["Data BYMHD","","","Lampiran","Rincian Barang","Verifikasi"]}); //7-1-21 rudi hide : "Item Mata Anggaran","KPA Anggaran"

		this.cb_bmhd = new saiCBBL(this.pc1.childPage[0],{bound:[20,11,220,20],caption:"No Ref BMHD", multiSelection:false, maxLength:10, tag:1, change:[this,"doChange"], rightLabelVisible:false });		
		this.cb_akunhut = new saiCBBL(this.pc1.childPage[0],{bound:[20,12,220,20],caption:"Akun BMHD", readOnly:true});		
		this.e_ket2 = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,13,450,20],caption:"Deskripsi", maxLength:150, readOnly:true});				
		this.e_saldo = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,14,200,20],caption:"Saldo BMHD IDR", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});						
		this.e_bmhdcurr = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,15,200,20],caption:"Nilai BMHD Curr", tag:1, tipeText:ttNilai, text:"0",change:[this,"doChange"]});						
		this.e_bmhd = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,16,200,20],caption:"Nilai BMHD IDR", tag:1, tipeText:ttNilai, text:"0",change:[this,"doChange"],readOnly:true});						
		
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:6,tag:9,visible:false,
		...
		this.sg2 = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:5,tag:9,visible:false,
		...
		this.e_latar = new saiMemo(this.pc1.childPage[3],{bound:[20,12,500,50],caption:"Latar Belakang",tag:2});
		this.e_lain = new saiMemo(this.pc1.childPage[3],{bound:[540,12,450,50],caption:"Aspek Lain",tag:2});		
		this.e_strategis = new saiMemo(this.pc1.childPage[3],{bound:[20,13,500,50],caption:"Aspek Strategis",tag:2});
		this.e_bisnis = new saiMemo(this.pc1.childPage[3],{bound:[20,14,500,50],caption:"Aspek Bisnis",tag:2});
		this.e_teknis = new saiMemo(this.pc1.childPage[3],{bound:[20,15,500,50],caption:"Spesifikasi Teknis",tag:2});
		
		this.sg4 = new saiGrid(this.pc1.childPage[4],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:5,tag:9,
		...
		this.e_noapp = new saiLabelEdit(this.pc1.childPage[5],{bound:[20,15,450,20],caption:"No Verifikasi", tag:9, readOnly:true});
		this.e_memo = new saiMemo(this.pc1.childPage[5],{bound:[20,12,450,150],caption:"Catatan",tag:9, readOnly:true});
...
window.app_saku3_transaksi_siaga_fAjuPJBMHD.extend(window.childForm);
window.app_saku3_transaksi_siaga_fAjuPJBMHD.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						//7/1/21-rudi sql.add("delete from angg_r where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='PJAJU'");
					...
					//7/1/21-rudi 
					/*
					//bmhd juga masukin ke angg_r
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
							"('"+this.e_nb.getText()+"','PJAJUBMHD','"+this.app._lokasi+"','"+this.cb_akunhut.getText()+"','"+this.cb_pp.getText()+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','D',0,"+parseNilai(this.e_bmhd.getText())+")");
					...
					if (this.sg2.getRowValidCount() > 0){
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','PJAJUBMHD','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.cb_pp.getText()+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(2,i))+","+nilai+")");
							}
						}
					}	
					*/
					...
					sql.add("insert into gr_bmhd_d(no_bukti,no_bmhd,no_dokumen,akun_hutang,keterangan,dc,nilai,kode_pp,kode_lokasi,modul,jenis,periode,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_bmhd.getText()+"','"+this.e_dok.getText()+"','"+this.cb_akunhut.getText()+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_bmhd.getText())+",'"+this.app._kodePP+"','"+this.app._lokasi+"','PJAJUBMHD','AJUSLS','"+this.e_periode.getText()+"','IDR',1)");
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					this.cb_bmhd.setSQL("select distinct a.no_bmhd, a.keterangan from gr_bmhd_d a "+									
									"inner join ( "+
									
									"select no_bmhd,sum(case dc when 'D' then nilai else -nilai end) as saldo "+
									"from gr_bmhd_d where kode_lokasi='"+this.app._lokasi+"' "+
									"group by no_bmhd "+
									
									" ) b on a.no_bmhd=b.no_bmhd "+

									"where a.jenis='AKRU' and  b.saldo > 0 and a.kode_lokasi='"+this.app._lokasi+"' ",["a.no_bmhd","a.keterangan"],false,["No Bukti","Deskripsi"],"and","Data BMHD",true);			
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_bmhd && this.cb_bmhd.getText()!=""){
			this.e_ket2.setText(this.cb_bmhd.rightLabelCaption);

			var strSQL = "select akun_hutang,sum(case dc when 'D' then nilai else -nilai end) as saldo "+
						 "from gr_bmhd_d where no_bukti <> '"+this.e_nb.getText()+"' and no_bmhd='"+this.cb_bmhd.getText()+"' and kode_lokasi='"+this.app._lokasi+
						 "' group by akun_hutang";			
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined) {
	...
	doClick:function(sender){
		try {
			if (this.e_periode.getText()!= "") {
				...
				this.stsSimpan = 1;
				this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"gr_pb_m","no_pb",this.app._lokasi+"-PJBD"+this.e_periode.getText().substr(2,2)+this.Aperiode+".","0000"));						
				this.e_dok.setFocus();
				setTipeButton(tbSimpan);			
			}		
		}
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_siaga_rptPanjarAju";
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai_curr "+
		             "from gr_pb_m a "+					 					 
					 "where nik_user = '"+this.app._userLog+"' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'PJAJUBMHD' and a.progress in ('0','R','V')";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider(
						   "select a.*,  isnull(b.no_app,'-') as no_app,isnull(b.catatan,'-') as catatan, c.no_bmhd  "+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){
						...
						this.cb_bmhd.setSQL("select no_bmhd, keterangan from gr_bmhd_d "+
											"where jenis='AKRU' and no_bukti='"+line.no_bmhd+"' and kode_lokasi='"+this.app._lokasi+"' ",
				...
				var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.nilai_curr,a.jenis "+
