window.app_saku3_transaksi_siaga_fAjuPJ2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fAjuPJ2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fAjuPJ2";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Permintaan Bayar Panjar", 0);	
		...
		this.pc2 = new pageControl(this,{bound:[10,10,1000,445], childPage:["Data Pengajuan","List Pengajuan"]});				
		...
		this.cb_buat = new saiCBBL(this.pc2.childPage[0],{bound:[20,17,220,20],caption:"NIK Admin Unit", multiSelection:false, maxLength:10, tag:2, change:[this,"doChangeJab"]});			
		this.c_jab1 = new saiCB(this.pc2.childPage[0],{bound:[700,17,220,20],caption:"Jab Admin Unit",readOnly:true,tag:0});		//items:["President Director","Director","Vice President","Manager","Assistant Manager","Staff"], 
		
		this.cb_app = new saiCBBL(this.pc2.childPage[0],{bound:[20,18,220,20],caption:"NIK Mgr Unit", multiSelection:false, maxLength:10, tag:2, change:[this,"doChangeJab"]});		
		this.c_jab2 = new saiCB(this.pc2.childPage[0],{bound:[700,18,220,20],caption:"Jab Mgr Unit", readOnly:true,tag:0});    //,items:["President Director","Director","Vice President","Manager","Assistant Manager","Staff"]
		
		this.cb_sah = new saiCBBL(this.pc2.childPage[0],{bound:[20,19,220,20],caption:"NIK VP Unit", multiSelection:false, maxLength:10, tag:2, change:[this,"doChangeJab"]});		
		this.c_jab3 = new saiCB(this.pc2.childPage[0],{bound:[700,19,220,20],caption:"Jab VP Unit", readOnly:true,tag:0});	//items:["President Director","Director","Vice President","Manager","Assistant Manager","Staff"],
		
		this.cb_ver = new saiCBBL(this.pc2.childPage[0],{bound:[20,20,220,20],caption:"NIK Ver Anggaran", multiSelection:false, maxLength:10, tag:2, change:[this,"doChangeJab"]});		
		this.c_jab4 = new saiCB(this.pc2.childPage[0],{bound:[700,20,220,20],caption:"Jab Ver Anggaran", readOnly:true,tag:0}); //items:["President Director","Director","Vice President","Manager","Assistant Manager","Staff"],	
		
		this.cb_dir = new saiCBBL(this.pc2.childPage[0],{bound:[20,21,220,20],caption:"NIK Dir Unit ", multiSelection:false, maxLength:10, tag:2, change:[this,"doChangeJab"]});		
		this.c_jab5 = new saiCB(this.pc2.childPage[0],{bound:[700,21,220,20],caption:"Jab Dir Unit", readOnly:true,tag:0}); //items:["President Director","Director","Vice President","Manager","Assistant Manager","Staff"],	

		this.cb_akun = new saiCBBL(this.pc2.childPage[0],{bound:[20,22,220,20],caption:"Akun Panjar", multiSelection:false, maxLength:10, tag:0});
		this.c_jenis = new saiCB(this.pc2.childPage[0],{bound:[700,22,220,20],caption:"Jenis Anggaran",items:["OPEX","CAPEX"], readOnly:true,tag:0});	
		...
		this.e_nilaiCurr = new saiLabelEdit(this.pc2.childPage[0],{bound:[700,12,220,20],caption:"Nilai Curr", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});						
		this.e_ket = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,14,450,20],caption:"Desk. Peruntukan", maxLength:150});				
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,160], childPage:["Item Mata Anggaran","KPA Anggaran","Lampiran","Rincian Barang","Verifikasi"]});
		...
		this.e_latar = new saiMemo(this.pc1.childPage[2],{bound:[20,12,450,50],caption:"Latar Belakang",tag:2});
		this.e_lain = new saiMemo(this.pc1.childPage[2],{bound:[500,12,450,50],caption:"Aspek Lain",tag:2});		
		this.e_strategis = new saiMemo(this.pc1.childPage[2],{bound:[20,13,450,50],caption:"Aspek Strategis",tag:2});
		this.e_bisnis = new saiMemo(this.pc1.childPage[2],{bound:[20,14,450,50],caption:"Aspek Bisnis",tag:2});
		this.e_teknis = new saiMemo(this.pc1.childPage[2],{bound:[20,15,450,50],caption:"Spesifikasi Teknis",tag:2});
		...
		this.e_memo = new saiMemo(this.pc1.childPage[4],{bound:[20,12,450,120],caption:"Catatan",tag:9, readOnly:true});
		...
		try {
			...
			this.cb_dir.setSQL("select nik, nama from karyawan where kode_lokasi='"+this.app._lokasi+"' and flag_aktif='1' ",["nik","nama"],false,["NIK","Nama"],"and","Data Karyawan",true);		
			...
			/*
			this.c_jab1.setText("Staff");
			this.c_jab2.setText("Staff");
			this.c_jab3.setText("Staff");
			this.c_jab4.setText("Staff");
			*/
			...
			this.c_jab5.items.clear();
			var data = this.dbLib.getDataProvider("select nama from kug_jab where kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];													
					this.c_jab5.addItem(i,line.nama);
				}
			} 
...
window.app_saku3_transaksi_siaga_fAjuPJ2.extend(window.childForm);
window.app_saku3_transaksi_siaga_fAjuPJ2.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from angg_r where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='PJAJU'");
					...
					//ref1=akunpanjar, ref2=noptg
		 			sql.add("insert into gr_pb_m (no_pb,kode_lokasi,periode,nik_user,tgl_input,tanggal,due_date,no_dokumen,keterangan,atensi,kode_curr,kurs,nilai_curr,nilai,modul,kode_pp,nik_buat,nik_tahu,progress,no_ver,no_spb,no_kas,ref1,ref2,no_app, latar,strategis,bisnis,teknis,lain,nik_sah,nik_ver,nik_dir,jenis,jab1,jab2,jab3,jab4,jab5) values "+
		 					"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.dp_d1.getDateString()+"','"+this.dp_d2.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','-','"+this.c_curr.getText()+"',"+nilaiToFloat(this.e_kurs.getText())+","+nilaiToFloat(this.e_nilaiCurr.getText())+","+nilaiToFloat(this.e_nilai.getText())+",'PJAJU','"+this.cb_pp.getText()+"','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','0','-','-','-','"+this.cb_akun.getText()+"','-','-','"+this.e_latar.getText()+"','"+this.e_strategis.getText()+"','"+this.e_bisnis.getText()+"','"+this.e_teknis.getText()+"','"+this.e_lain.getText()+"','"+this.cb_sah.getText()+"','"+this.cb_ver.getText()+"','"+this.cb_dir.getText()+"','"+this.c_jenis.getText()+"','"+this.c_jab1.getText()+"','"+this.c_jab2.getText()+"','"+this.c_jab3.getText()+"','"+this.c_jab4.getText()+"','"+this.c_jab5.getText()+"')");	
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','PJAJU','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.cb_pp.getText()+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(2,i))+","+nilai+")");
							}
						}
					}	
					...
					// untuk approval di esaku
					sql.add("insert into apv_flow (no_bukti,kode_lokasi,kode_role,kode_jab,no_urut,status,tgl_app,sts_ver,nik) "+
					"select a.no_pb as no_bukti,a.kode_lokasi,'-' as kode_role,b.kode_jab,1 as no_urut,1 as status,NULL as tgl_app,0 as sts_ver,a.nik_tahu as nik "+
					"from gr_pb_m a "+
					"inner join karyawan b on a.nik_tahu=b.nik and a.kode_lokasi=b.kode_lokasi "+
					"where a.no_pb='"+this.e_nb.getText()+"' "+
					"union all "+
					"select a.no_pb as no_bukti,a.kode_lokasi,'-' as kode_role,b.kode_jab,2 as no_urut,0 as status,NULL as tgl_app,0 as sts_ver,a.nik_sah as nik "+
					"from gr_pb_m a "+
					"inner join karyawan b on a.nik_sah=b.nik and a.kode_lokasi=b.kode_lokasi "+
					"where a.no_pb='"+this.e_nb.getText()+"' "+
					"union all "+
					"select a.no_pb as no_bukti,a.kode_lokasi,'-' as kode_role,b.kode_jab,3 as no_urut,0 as status,NULL as tgl_app,0 as sts_ver,a.nik_ver as nik "+
					"from gr_pb_m a "+
					"inner join karyawan b on a.nik_ver=b.nik and a.kode_lokasi=b.kode_lokasi "+
					"where a.no_pb='"+this.e_nb.getText()+"' "+
					"union all "+
					"select a.no_pb as no_bukti,a.kode_lokasi,'-' as kode_role,b.kode_jab,4 as no_urut,0 as status,NULL as tgl_app,0 as sts_ver,a.nik_dir as nik "+
					"from gr_pb_m a "+
					"left join karyawan b on a.nik_dir=b.nik and a.kode_lokasi=b.kode_lokasi "+
					"where a.no_pb='"+this.e_nb.getText()+"' ");
					...
					"select '"+this.no_pesan+"' as no_bukti,a.kode_lokasi,'Panjar','Pengajuan Panjar','Unit '+c.nama+' dengan no transaksi '+a.no_bukti+' mengajukan beban senilai '+convert(varchar,b.nilai)+', menunggu approval dari anda',a.nik,getdate(),'-',a.no_bukti,'Panjar','-',0,0 "+
					"from apv_flow a "+
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_siaga_rptPanjarAjuEmail";
								this.filter = " where a.kode_lokasi='"+this.app._lokasi+"' and a.no_pb='"+this.e_nb.getText()+"' ";
								this.filter2 = this.app._lokasi + "/" + this.app._periode + "/" + this.app._kodePP + "/" + this.app._userLog + "/" + this.app._userPwd + "/" + this.e_nb.getText() +"/"+"/"+this.no_pesan;
								this.viewer.prepare();
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai_curr "+
		             "from gr_pb_m a "+					 					 
					 "where nik_user = '"+this.app._userLog+"' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'PJAJU' and a.progress in ('0','R','V')";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
