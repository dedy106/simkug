window.app_saku3_transaksi_siaga_fSpb = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fSpb.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fSpb";
		...
		this.cb_ver = new saiCBBL(this.pc2.childPage[0],{bound:[20,12,220,20],caption:"Pemberi SPB/Dir", multiSelection:false, maxLength:10, tag:2});							
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,30,990,313], childPage:["Peruntukan","Data Pengajuan"]});
		...
		this.e_jtran = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,15,450,20],caption:"Jenis Transaksi", maxLength:150,tag:1});										
		this.e_bank = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,16,450,20],caption:"Bank", maxLength:150,tag:1});								
		this.e_norek = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,17,450,20],caption:"No Rekening ", maxLength:150,tag:1});								
		this.e_alrek = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,18,450,20],caption:"Alamat Rekening", maxLength:150,tag:1});								
...
window.app_saku3_transaksi_siaga_fSpb.extend(window.childForm);
window.app_saku3_transaksi_siaga_fSpb.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into gr_spb2_m (no_spb,kode_lokasi,nik_user,tgl_input,periode,tanggal,nama,alamat,keterangan,nik_bdh,nik_ver,nik_fiat,nilai,no_kas,kode_curr,  cat_pajak,jtran,bank,norek,alrek,no_po,tgl_po,no_dok,tgl_dok,cat_bdh) values  "+
							...
							+this.e_nopo.getText()+"','"+this.dp_dpo.getDateString()+"','"+this.e_nodok.getText()+"','"+this.dp_ddok.getDateString()+"','"+this.e_bdh.getText()+"')");					
			...
			case "clear" :
				if (modalResult == mrOk)
					...
					sql.add("select no_beban,keterangan "+
							"from gr_beban_m "+
							"where modul not in ('PBAJU2','PJPTG','PJPTGLOG') and no_spb='-' and kode_curr='"+this.c_curr.getText()+"' and periode<='"+this.e_periode.getText()+"' and progress = '0' and kode_lokasi='"+this.app._lokasi+"'");			
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_siaga_rptSpbForm";
