window.app_saku3_transaksi_siaga_fSO = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fSO.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_siaga_fSO";
...
window.app_saku3_transaksi_siaga_fSO.extend(window.portalui_childForm);
window.app_saku3_transaksi_siaga_fSO.implement({
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
						...
						try
						{
							...
							var sql2,sql1 = "delete from neraca where modul = '"+modul+"'  and kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"'";					// kode_lokasi,
							var sql = new server_util_arrayList();
							sql.add("delete from relakun  where kode_lokasi = '"+this.elokasi.getText()+"' and kode_fs = '"+this.eKlp.getText()+"' "+
								"and kode_neraca in (select kode_neraca from neraca where modul = '"+modul+"'  and kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"')");																		
							sql.add(sql1);							
							//if mysql langsung														
		...
		{ 
			...
			{*/
				if (sender == this.addBtn || sender == this.editBtn){
					uses("app_saku3_transaksi_siaga_fSODetail");
					if (this.entryMenu == undefined)
						this.entryMenu = new app_saku3_transaksi_siaga_fSODetail(this.app);
