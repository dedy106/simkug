window.app_saku3_transaksi_siaga_fAjuSch = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fAjuSch.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fAjuSch";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,233], childPage:["Data Kartu", "Item Jurnal","KPA Anggaran","Lampiran","Rincian Barang","Verifikasi"]});
		this.cb_kartu = new saiCBBL(this.pc1.childPage[0],{bound:[10,11,220,20],caption:"Data Kartu", multiSelection:false, maxLength:10, tag:2, change:[this,"doChange"]});							
		this.e_reverse = new saiLabelEdit(this.pc1.childPage[0],{bound:[700,11,220,20],caption:"Tot Reverse", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				

		this.sgk = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-63],colCount:4,tag:9,
					colTitle:["Status","Periode","No Bukti","Nilai"],
					colWidth:[[3,2,1,0],[100,100,150,100]],					
					readOnly:true,
					buttonStyle:[[0],[bsAuto]], 
					colFormat:[[3],[cfNilai]],picklist:[[0],[new portalui_arrayMap({items:["INPROG","REVERSE"]})]],checkItem:true,	
					change:[this,"doChangeCellk"],nilaiChange:[this,"doNilaiChangek"],				
					autoAppend:false,defaultRow:1});
		this.sgnk = new portalui_sgNavigator(this.pc1.childPage[0],{bound:[1,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:2,grid:this.sgk});			
		...
		try {
			...
			*/
			...
			this.cb_kartu.setSQL("select no_kartu, keterangan from gr_kartu_m where kode_lokasi='"+this.app._lokasi+"' ",["no_kartu","keterangan"],false,["No Kartu","Deskripsi"],"and","Data Kartu",true);			
...
window.app_saku3_transaksi_siaga_fAjuSch.extend(window.childForm);
window.app_saku3_transaksi_siaga_fAjuSch.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					//------------------------------------------------------------  jurnal reverse-nya  ------------------------------------------------------------  
					sql.add("insert into gr_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',998,'"+this.akunHutang+"','"+this.e_ket.getText()+"','D','IDR',1,"+parseNilai(this.e_reverse.getText())+","+parseNilai(this.e_reverse.getText())+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','AJU','REVHUT','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
					sql.add("insert into gr_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.akunBeban+"','"+this.e_ket.getText()+"','C','IDR',1,"+parseNilai(this.e_reverse.getText())+","+parseNilai(this.e_reverse.getText())+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','AJU','REVBBN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");		

					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
							"('"+this.e_nb.getText()+"','AJU','"+this.app._lokasi+"','"+this.akunBeban+"','"+this.cb_pp.getText()+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','C',0,"+parseNilai(this.e_reverse.getText())+")");

					if (this.sgk.getRowValidCount() > 0){
						for (var i=0;i < this.sgk.getRowCount();i++){
							if (this.sgk.rowValid(i) && this.sgk.cells(0,i) == "REVERSE") {								
								sql.add("update gr_kartu_sch set no_rev='"+this.e_nb.getText()+"',periode_rev='"+this.e_periode.getText()+"' "+
										"where no_bill='"+this.sgk.cells(2,i)+"' and no_kartu='"+this.cb_kartu.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");
							}
						}
					}
					//------------------------------------------------------------  jurnal reverse-nya  ------------------------------------------------------------  
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					this.sg.clear(1); this.sg2.clear(1);  this.sg3.clear(1);   this.sgk.clear(1);  
			...
			case "ubah" :		
				...
				if (nilaiToFloat(this.e_reverse.getText()) <= 0 ) {
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_kartu && this.cb_kartu.getText()!="") {
			var strSQL = "select b.nama,a.akun_beban,a.akun_hutang from gr_kartu_m a inner join vendor b on a.kode_vendor=b.kode_vendor and a.kode_lokasi=b.kode_lokasi where a.no_kartu='"+this.cb_kartu.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";			
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined) {
			...
			if (this.stsSimpan == 1) {
				var data = this.dbLib.getDataProvider(
							"select * from gr_kartu_sch where no_kartu='"+this.cb_kartu.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' and no_bill<>'-' and no_rev='-'",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sgk.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sgk.appendData(["INPROG",line.periode,line.no_bill,floatToNilai(line.nilai)]);
					}
				} else this.sgk.clear(1);	
			}
		}
	},	
	...
	doChangeCellk: function(sender, col, row){		
		if (col == 0) {			
			this.sgk.validasi();			
		}		
	},
	doNilaiChangek: function(){
		try{
			var  tot = 0;
			for (var i = 0; i < this.sgk.rows.getLength();i++){
				if (this.sgk.rowValid(i) && this.sgk.cells(3,i) != "" && this.sgk.cells(0,i) == "REVERSE") {					
					tot += nilaiToFloat(this.sgk.cells(3,i));														
				}
			}						
			this.e_reverse.setText(floatToNilai(tot));
		}catch(e)
		{
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai_curr "+
		             "from gr_pb_m a "+				 
					 "where nik_user = '"+this.app._userLog+"' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'AJU' and a.progress in ('0','R','V')";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider("select * from gr_kartu_sch where kode_lokasi ='"+this.app._lokasi+"' and no_bill<>'-' and no_rev='"+this.e_nb.getText()+"' order by periode",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sgk.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sgk.appendData(["REVERSE",line.periode,line.no_bill,floatToNilai(line.nilai)]);
					}
					this.cb_kartu.setSQL("select no_kartu, keterangan from gr_kartu_m where no_kartu='"+line.no_kartu+"' and kode_lokasi='"+this.app._lokasi+"' ",["no_kartu","keterangan"],false,["No Kartu","Deskripsi"],"and","Data Kartu",true);			
					this.cb_kartu.setText(line.no_kartu);
				} else this.sgk.clear(1);
