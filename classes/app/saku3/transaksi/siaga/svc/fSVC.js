window.app_saku3_transaksi_siaga_svc_fSVC = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_svc_fSVC.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_svc_fSVC";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Parameter SVC", 0);	
...
window.app_saku3_transaksi_siaga_svc_fSVC.extend(window.childForm);
window.app_saku3_transaksi_siaga_svc_fSVC.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into gr_svc(kode_cabang,akun_ar,akun_pend,akun_pot_pend,akun_ppn,akun_biaya,akun_materai,akun_oi,akun_oe) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update gr_svc set akun_ar='"+this.cb_akunar.getText()+"',akun_pend='"+this.cb_akunpdpt.getText()+"',akun_pot_pend='"+this.cb_akundiskon.getText()+"',akun_ppn='"+this.cb_akunppn.getText()+"',akun_biaya='"+this.cb_akunbiaya.getText()+"',akun_materai='"+this.cb_akunmaterai.getText()+"',akun_oi='"+this.cb_oi.getText()+"',akun_oe='"+this.cb_oe.getText()+"' "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from gr_svc where kode_cabang = '"+this.cb_kode.getText()+"'");			
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
		...
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select  b.kode_cabang,b.nama as nama_cabang,a.akun_ar,a.akun_pend,a.akun_pot_pend,a.akun_ppn,a.akun_biaya,a.akun_materai,a.akun_oi,a.akun_oe,  "+
							 ...
							 "from gr_svc a inner join gr_cabang b on a.kode_cabang=b.kode_cabang "+
