window.app_saku3_transaksi_siaga_svc_fKbSVCmultiE = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_svc_fKbSVCmultiE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_svc_fKbSVCmultiE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Penerimaan SVC: Edit", 0);	
		...
		this.cb_akun = new saiCBBL(this,{bound:[20,11,220,20],caption:"Akun KasBank", multiSelection:false, maxLength:10, tag:2});									
		...
		this.pc1 = new pageControl(this,{bound:[20,12,980,230], childPage:["Data Pelunasan","Data Anggaran","NoBukti Baru"]});				
...
window.app_saku3_transaksi_siaga_svc_fKbSVCmultiE.extend(window.childForm);
window.app_saku3_transaksi_siaga_svc_fKbSVCmultiE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from gr_svcbayar_d where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.nb+"','"+this.app._lokasi+"','-','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_cabang.getText()+"','KBSVCM','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR','1',"+parseNilai(this.e_total.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_buat.getText()+"',getdate(),'"+this.app._userLog+"','F','-','-','"+this.e_atensi.getText()+"','"+this.cb_bank.getText()+"')");
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.nb+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_total.getText())+",'"+this.app._kodePP+"','-','-','"+this.e_atensi.getText()+"','"+this.app._lokasi+"','KBSVCM','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'"+this.cb_bank.getText()+"',"+nilaiToFloat(this.e_total.getText())+")");					
					...
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i)){	
							...
							sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
									"('"+this.nb+"','"+this.sg1.cells(0,i)+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(2,i)+"','"+this.e_ket.getText()+"','C',"+nilai+",'"+this.app._kodePP+"','-','-','"+this.sg1.cells(1,i)+"','"+this.app._lokasi+"','KBSVCM','AR','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilai+")");
					...
					if (nilaiToFloat(this.e_beban.getText()) > 0) {
						sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
								"('"+this.nb+"','-','"+this.dp_d1.getDateString()+"',98,'"+this.akunOE+"','"+this.e_ket.getText()+"','D',"+parseNilai(this.e_beban.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBSVCM','OE','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+parseNilai(this.e_beban.getText())+")");										
					}					
					if (this.nilaiOI != 0) {
						sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
								"('"+this.nb+"','-','"+this.dp_d1.getDateString()+"',99,'"+this.akunOI+"','"+this.e_ket.getText()+"','C',"+this.nilaiOI+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBSVCM','OI','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+this.nilaiOI+")");										
					}
	...
	doChange:function(sender){
		if (sender == this.e_periode || sender == this.cb_cabang) {			
			if (this.e_periode.getText()!="" && this.cb_cabang.getText()!="") {
				this.e_nb.setSQL("select no_kas, keterangan from kas_m where modul = 'KBSVCM' and kode_pp ='"+this.cb_cabang.getText()+"' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kas","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);
			}			
		}		
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			...
			var data = this.dbLib.getDataProvider("select a.no_invoice,a.nama_cust,a.total-isnull(c.bayar,0) as total,a.akun_piutang,x.nilai_kas,case when x.nilai_lain > 0 then x.nilai_lain else 0 end as nilai_lain "+
			           "from gr_svc_d a inner join gr_svcbayar_d x on a.no_invoice=x.no_invoice and a.kode_lokasi=x.kode_lokasi "+
					   "                 inner join masakun b on a.akun_piutang=b.kode_akun and b.kode_lokasi = a.kode_lokasi "+
					   "                 left join (select no_invoice,kode_lokasi,sum(case dc when 'D' then (nilai_kas+nilai_lain) else -(nilai_kas+nilai_lain) end) as bayar from gr_svcbayar_d where no_bukti <> '"+this.e_nb.getText()+"' group by no_invoice,kode_lokasi ) c on a.no_invoice=c.no_invoice and a.kode_lokasi=c.kode_lokasi "+
