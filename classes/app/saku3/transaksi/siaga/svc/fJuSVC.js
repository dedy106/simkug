window.app_saku3_transaksi_siaga_svc_fJuSVC = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_svc_fJuSVC.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_svc_fJuSVC";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pembatalan Tagihan SVC: Input", 0);	
...
window.app_saku3_transaksi_siaga_svc_fJuSVC.extend(window.childForm);
window.app_saku3_transaksi_siaga_svc_fJuSVC.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_inv.getText()+"','"+this.e_ket.getText()+"','-','JUSVC','JU','IDR',1,"+parseNilai(this.e_saldo.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','F','-','-','-',getdate(),'"+this.app._userLog+"')");
					...
					sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref)  "+
							"select '"+this.e_nb.getText()+"','"+this.cb_inv.getText()+"','"+this.dp_d1.getDateString()+"',0,c.akun_ar,'"+this.e_ket.getText()+"','C',a.total,b.kode_pp,b.kode_drk,'-','-','-','-','-','-',a.kode_lokasi,'SVC','PIUTANG','"+this.e_periode.getText()+"',b.kode_curr,b.kurs,'"+this.app._userLog+"',getdate(),'-','-' "+
							"from gr_svc_d a "+
							"inner join gr_svc_m b on a.no_svc=b.no_svc "+
							"inner join gr_svc c on b.kode_cabang=c.kode_cabang "+
							"where a.total <> 0 and a.no_invoice='"+this.cb_inv.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'");
					
					sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref)  "+
							"select '"+this.e_nb.getText()+"','"+this.cb_inv.getText()+"','"+this.dp_d1.getDateString()+"',1,c.akun_pot_pend,'"+this.e_ket.getText()+"','C',a.diskon,b.kode_pp,b.kode_drk,'-','-','-','-','-','-',a.kode_lokasi,'SVC','DISKON','"+this.e_periode.getText()+"',b.kode_curr,b.kurs,'"+this.app._userLog+"',getdate(),'-','-' "+
							"from gr_svc_d a "+
							"inner join gr_svc_m b on a.no_svc=b.no_svc "+
							"inner join gr_svc c on b.kode_cabang=c.kode_cabang "+
					...
					sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref)  "+
							"select '"+this.e_nb.getText()+"','"+this.cb_inv.getText()+"','"+this.dp_d1.getDateString()+"',2,c.akun_pend,'"+this.e_ket.getText()+"','D',a.nilai,b.kode_pp,b.kode_drk,'-','-','-','-','-','-',a.kode_lokasi,'SVC','PEND','"+this.e_periode.getText()+"',b.kode_curr,b.kurs,'"+this.app._userLog+"',getdate(),'-','-' "+
							"from gr_svc_d a "+
							"inner join gr_svc_m b on a.no_svc=b.no_svc "+
							"inner join gr_svc c on b.kode_cabang=c.kode_cabang "+
					...
					sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref)  "+
							"select '"+this.e_nb.getText()+"','"+this.cb_inv.getText()+"','"+this.dp_d1.getDateString()+"',3,c.akun_biaya,'"+this.e_ket.getText()+"','D',a.biaya,b.kode_pp,b.kode_drk,'-','-','-','-','-','-',a.kode_lokasi,'SVC','BIAYA','"+this.e_periode.getText()+"',b.kode_curr,b.kurs,'"+this.app._userLog+"',getdate(),'-','-' "+
							"from gr_svc_d a "+
							"inner join gr_svc_m b on a.no_svc=b.no_svc "+
							"inner join gr_svc c on b.kode_cabang=c.kode_cabang "+
					...
					sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref)  "+
							"select '"+this.e_nb.getText()+"','"+this.cb_inv.getText()+"','"+this.dp_d1.getDateString()+"',4,c.akun_materai,'"+this.e_ket.getText()+"','D',a.materai,b.kode_pp,b.kode_drk,'-','-','-','-','-','-',a.kode_lokasi,'SVC','MATERAI','"+this.e_periode.getText()+"',b.kode_curr,b.kurs,'"+this.app._userLog+"',getdate(),'-','-' "+
							"from gr_svc_d a "+
							"inner join gr_svc_m b on a.no_svc=b.no_svc "+
							"inner join gr_svc c on b.kode_cabang=c.kode_cabang "+
					...
					sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref)  "+
							"select '"+this.e_nb.getText()+"','"+this.cb_inv.getText()+"','"+this.dp_d1.getDateString()+"',5,c.akun_ppn,'"+this.e_ket.getText()+"','D',a.ppn,b.kode_pp,b.kode_drk,'-','-','-','-','-','-',a.kode_lokasi,'SVC','PPN','"+this.e_periode.getText()+"',b.kode_curr,b.kurs,'"+this.app._userLog+"',getdate(),'-','-' "+
							"from gr_svc_d a "+
							"inner join gr_svc_m b on a.no_svc=b.no_svc "+
							"inner join gr_svc c on b.kode_cabang=c.kode_cabang "+
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					this.stsSimpan = 1;			
					setTipeButton(tbSimpan);
					this.cb_inv.setSQL("select a.no_invoice, a.nama_cust from gr_svc_d a "+
	...
	doChange:function(sender){
		...
		if (sender == this.cb_inv && this.cb_inv.getText()!="") {		
			var data = this.dbLib.getDataProvider("select a.nama_cust,a.total-isnull(c.bayar,0) as total,a.akun_piutang,a.akun_piutang + ' - '+b.nama as nama "+
					   ...
					   "                            from gr_svcbayar_d where no_bukti<>'"+this.e_nb.getText()+"' group by no_invoice,kode_lokasi ) c on a.no_invoice=c.no_invoice and a.kode_lokasi=c.kode_lokasi "+
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_ju,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai "+
		             "from ju_m a "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'JUSVC' and a.posted ='F'";						
