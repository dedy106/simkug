window.app_saku3_transaksi_siaga_fVer2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fVer2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fVer2";
		...
		this.pc1 = new pageControl(this.pc2.childPage[1],{bound:[1,12,995,308], childPage:["Data Pengajuan","Item Mata Anggaran","KPA Anggaran","BYMHD  "]});		
		...
		this.c_modul2 = new saiCB(this.pc2.childPage[2],{bound:[20,11,200,20],caption:"Modul",items:["PBAJU","PJPTG"], readOnly:true,tag:9,change:[this,"doChange"]});
		this.cb_nb = new saiCBBL(this.pc2.childPage[2],{bound:[20,12,220,20],caption:"No Bukti", multiSelection:false, maxLength:20, tag:9,change:[this,"doChange"]});		
...
window.app_saku3_transaksi_siaga_fVer2.extend(window.childForm);
window.app_saku3_transaksi_siaga_fVer2.implement({
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {			
				...
				if (this.e_modul.getText() == "PBAJU") var strSQL = "select * from gr_pb_m where no_pb='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";		
				if (this.e_modul.getText() == "PJPTG") var strSQL = "select a.periode,a.tanggal,a.no_dokumen,a.keterangan,'-' as atensi,a.kode_pp,a.nik_buat,a.nik_app as nik_tahu,a.kode_curr,a.kurs,a.no_ver "+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){
						...
						if (this.e_modul.getText() == "PBAJU") {
							//dari PJAJU atau bukan
							this.modulForm = line.modul;
					...
					if (this.c_status.getText() == "APPROVE") {
						...
						if (this.cb_bymhd.getText() != "") {
							...
							sql.add("insert into ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user) values "+
									"('"+this.e_ju.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_nb.getText()+"','"+this.e_ket.getText()+"','-','BYMHD','BYMHD','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+","+parseNilai(this.e_nilaiCurr.getText())+",'"+this.cb_buat2.getText()+"','"+this.cb_app2.getText()+"','F','-','-','"+this.cb_bymhd.getText()+"',getdate(),'"+this.app._userLog+"')");
							...
							sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
									"('"+this.e_ju.getText()+"','"+this.e_nb.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.cb_bymhd.getText()+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','-','-','-','-','-','-','"+this.app._lokasi+"','BYMHD','BYMHD','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','-')");
									...
									if (this.sg.rowValid(i)){		
										if (this.cb_bymhd.getText() == "") {											
											sql.add("insert into gr_beban_j(no_beban,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
													"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i)+"','"+this.c_curr.getText()+"',"+parseNilai(this.e_kurs.getText())+","+parseNilai(this.sg.cells(4,i))+","+parseNilai(this.sg.cells(5,i))+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','PBAJU','BEBAN','"+this.periodeAju+"','"+this.app._userLog+"',getdate())");																	
										}
										else {																			
											sql.add("insert into gr_beban_j(no_beban,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
							...
							for (var i=0;i < this.sg2.getRowCount();i++){
								if (this.sg2.rowValid(i)){
									...
									sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
											"	('"+this.e_nb.getText()+"','VER','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.cb_pp.getText()+"','-','"+this.periodeAju+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(2,i))+","+nilai+")");
								}
							}
						}					
					}
	...
	doChange:function(sender){		
		...
		if (sender == this.c_modul2) {
			if (this.c_modul2.getText() == "PBAJU") 
				this.cb_nb.setSQL("select no_pb, keterangan from gr_pb_m where periode<='"+this.e_periode.getText()+"' and progress in ('2') and kode_lokasi='"+this.app._lokasi+"'",["no_pb","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);								
			
			if (this.c_modul2.getText() == "PJPTG") 
				this.cb_nb.setSQL("select no_ptg, keterangan from gr_panjarptg_m where periode<='"+this.e_periode.getText()+"' and progress in ('2') and kode_lokasi='"+this.app._lokasi+"'",["no_ptg","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);				
		}
		if (sender == this.cb_nb && this.cb_nb.getText() != "") {
			if (this.c_modul2.getText() == "PBAJU") {
				var strSQL = "select a.tanggal as due_date,a.no_pb as no_bukti,case a.progress when '2' then 'APPROVE' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,'PBAJU' as modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai_curr as nilai,c.nik+' - '+c.nama as pembuat,a.no_ver as no_app,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp,a.modul as jenis "+
							 "from gr_pb_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
							 "               inner join karyawan c on a.nik_buat=c.nik and a.kode_lokasi=c.kode_lokasi "+
							 "				 inner join gr_beban_m d on a.no_ver=d.no_beban and a.kode_lokasi=d.kode_lokasi "+
							 "where d.modul='PBAJU' and a.no_pb='"+this.cb_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' ";				
			}
			if (this.c_modul2.getText() == "PJPTG") {
