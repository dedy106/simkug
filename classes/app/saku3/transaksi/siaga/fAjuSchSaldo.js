window.app_saku3_transaksi_siaga_fAjuSchSaldo = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fAjuSchSaldo.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fAjuSchSaldo";
		...
		this.cb_bmhd = new saiCBBL(this.pc1.childPage[0],{bound:[20,11,220,20],caption:"No Kartu", multiSelection:false, maxLength:10, tag:1, change:[this,"doChange"], rightLabelVisible:false });		
...
window.app_saku3_transaksi_siaga_fAjuSchSaldo.extend(window.childForm);
window.app_saku3_transaksi_siaga_fAjuSchSaldo.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from gr_kartu_rev where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}	
					...
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
							"('"+this.e_nb.getText()+"','AJU','"+this.app._lokasi+"','"+this.cb_akun.getText()+"','"+this.cb_pp.getText()+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','D',0,"+parseNilai(this.e_bmhd.getText())+")");
					...
					sql.add("insert into gr_kartu_rev (no_bukti,kode_lokasi,no_kartu,periode,dc,nilai,kode_akun,modul) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_bmhd.getText()+"','"+this.e_periode.getText()+"','D',"+nilaiToFloat(this.e_bmhd.getText())+",'"+this.cb_akun.getText()+"','AJUSCH')");
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					var strSQL = "select c.no_kartu,c.keterangan "+
								 "from "+
								 "gr_kartu_m c "+
								 "inner join  "+
								 "( "+
								 "select no_kartu,sum(nilai) as tot_bmhd "+
								 "from gr_kartu_sch "+
								 "where kode_lokasi ='"+this.app._lokasi+"' and no_bill<>'-' "+
								 "group by no_kartu "+
								 ") a on c.no_kartu=a.no_kartu "+
								 "left join "+
								 "( "+
								 "select no_kartu,sum(nilai) as tot_rev "+
								 "from gr_kartu_rev "+
								 "where kode_lokasi ='"+this.app._lokasi+"' "+
								 "group by no_kartu "+
								 ")b on a.no_kartu=b.no_kartu "+
								 "where c.kode_lokasi ='"+this.app._lokasi+"' and a.tot_bmhd-isnull(b.tot_rev,0) > 0 ";

					this.cb_bmhd.setSQL(strSQL,["no_kartu","keterangan"],false,["No Bukti","Deskripsi"],"and","Data BMHD",true);			
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_bmhd && this.cb_bmhd.getText()!=""){						 
			var strSQL = "select a.no_kartu,c.keterangan,c.akun_beban,c.akun_hutang, "+ 
						 "a.tot_bmhd-isnull(b.tot_rev,0) as saldo_bmhd "+
						 "from "+
						 "gr_kartu_m c "+
						 "inner join  "+
						 "( "+
						 "select no_kartu,sum(nilai) as tot_bmhd "+
						 "from gr_kartu_sch "+
						 "where kode_lokasi ='"+this.app._lokasi+"' and no_bill<>'-' and no_kartu='"+this.cb_bmhd.getText()+"' "+
						 "group by no_kartu "+
						 ") a on c.no_kartu=a.no_kartu "+
						 "left join "+
						 "( "+
						 "select no_kartu,sum(nilai) as tot_rev "+
						 "from gr_kartu_rev "+
						 "where kode_lokasi ='"+this.app._lokasi+"' and no_bukti <> '"+this.e_nb.getText()+"' and no_kartu='"+this.cb_bmhd.getText()+"'"+
						 "group by no_kartu "+
						 ")b on a.no_kartu=b.no_kartu "+
						 "where c.kode_lokasi ='"+this.app._lokasi+"' and c.no_kartu='"+this.cb_bmhd.getText()+"'"; 
			...
			if (typeof data == "object"){
				...
				if (line != undefined) {
					...
					this.e_saldo.setText(floatToNilai(line.saldo_bmhd));
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider(
						   "select a.*,  isnull(b.no_app,'-') as no_app,isnull(b.catatan,'-') as catatan,  c.no_kartu as no_bmhd "+						  
						   "from gr_pb_m a inner join gr_kartu_rev c on a.no_pb=c.no_bukti and a.kode_lokasi=c.kode_lokasi "+
						   "		left join ("+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){
						...
						this.cb_bmhd.setSQL("select no_kartu, keterangan from gr_kartu_m "+
											"where no_kartu='"+line.no_bmhd+"' and kode_lokasi='"+this.app._lokasi+"' ",
											["no_kartu","keterangan"],false,["No Bukti","Deskripsi"],"and","Data BMHD",true);			
