window.app_saku3_transaksi_siaga_fJu = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fJu.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fJu";
		...
		this.e_kredit = new saiLabelEdit(this.pc2.childPage[0],{bound:[780,18,210,20],caption:"Total Kredit (IDR)", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		this.cb_ref = new saiCBBL(this.pc2.childPage[0],{bound:[20,17,220,20],caption:"Jurnal Ref", multiSelection:false, maxLength:10, tag:9,change:[this,"doChange"]});		
		this.e_sls = new saiLabelEdit(this.pc2.childPage[0],{bound:[780,17,210,20],caption:"Selisih (IDR)", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:5,tag:0,
					...
					colWidth:[[4,3,2,1,0],[100,400,40,250,80]],					
		...
		try {
			...
			this.cb_ref.setSQL("select no_ref, keterangan from gr_juref_m where kode_lokasi='"+this.app._lokasi+"'",["no_ref","keterangan"],false,["No Ref","Keterangan"],"and","Data Referensi",true);
...
window.app_saku3_transaksi_siaga_fJu.extend(window.childForm);
window.app_saku3_transaksi_siaga_fJu.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','-','"+this.e_ket.getText()+"','-','JU','JU','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','F','-','-','-',getdate(),'"+this.app._userLog+"')");
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
										"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.app._kodePP+"','-','-','-','-','-','-','-','"+this.app._lokasi+"','JU','JU','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','-')");
							}
						}
					}
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider("select a.tanggal,a.periode,a.keterangan,a.nik_buat,b.nama as nama_buat,a.nik_setuju,b.nama as nama_setuju "+
						   "from ju_m a "+
