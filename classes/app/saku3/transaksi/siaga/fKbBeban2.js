window.app_saku3_transaksi_siaga_fKbBeban2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fKbBeban2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fKbBeban2";
...
window.app_saku3_transaksi_siaga_fKbBeban2.extend(window.childForm);
window.app_saku3_transaksi_siaga_fKbBeban2.implement({
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!="" && this.stsSimpan == 1) {
			this.cb_spb.setSQL("select no_spb, keterangan from gr_spb2_m where sts_spb2 = 'SPBv2' and no_kas='-' and progress='S' and periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_spb","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar SPB",true);
		}
	...
	doClick:function(sender){
		if (this.stsSimpan == 0) {
			this.sg.clear(1); this.sg2.clear(1); this.sg3.clear(1); this.sg4.clear(1); this.sg5.clear(1); 
			this.cb_spb.setSQL("select no_spb, keterangan from gr_spb2_m where no_kas='-' and progress='S' and periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_spb","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar SPB",true);
		}
		this.stsSimpan = 1;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg5.cells(0,row) != "") {
				...
				this.cb_spb.setSQL("select no_spb, keterangan from gr_spb2_m where no_kas='"+this.e_nb.getText()+"' and periode<='"+this.e_periode.getText()+"' and progress='S' and kode_lokasi='"+this.app._lokasi+"'",["no_spb","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar SPB",true);
