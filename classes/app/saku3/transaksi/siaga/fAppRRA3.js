window.app_saku3_transaksi_siaga_fAppRRA3 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fAppRRA3.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fAppRRA3";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Approval Dir User", 0);	
...
window.app_saku3_transaksi_siaga_fAppRRA3.extend(window.childForm);
window.app_saku3_transaksi_siaga_fAppRRA3.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{				
					...
					sql.add("insert into rra_app_m(no_app, kode_lokasi,tanggal,keterangan,modul,periode,no_del,nik_buat,nik_app,nik_user,tgl_input,jenis_form) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','PUSAT','"+this.e_periode.getText()+"','-','"+this.app._userLog+"','"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'APPDIRUS')");
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){														
							...
							this.filter2 = this.app._lokasi+"/"+this.app._userLog+"/"+this.e_nb.getText()+"/"+this.c_status.getText()+"/Approval Dir Unit"+"/"+this.app._kodePP+"/"+this.e_nopdrk.getText();
							this.viewer.prepare();
