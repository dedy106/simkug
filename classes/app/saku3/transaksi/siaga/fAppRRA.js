window.app_saku3_transaksi_siaga_fAppRRA = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fAppRRA.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fAppRRA";
...
window.app_saku3_transaksi_siaga_fAppRRA.extend(window.childForm);
window.app_saku3_transaksi_siaga_fAppRRA.implement({
	...
	doLoad:function(sender){		
		var strSQL = "select a.kode_lokasi,a.kode_pp+' - '+c.nama as pp,a.no_pdrk as no_bukti,b.no_dokumen,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,a.sts_pdrk as jenis "+
					 ...
					 "where a.modul = 'MULTI' and a.progress in ('9') and a.periode<='"+this.e_periode.getText()+"' and a.no_pdrk like '01-RRA%'";	 //ubah 5-3-22 -->and a.progress in ('0','Z')
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){														
							...
							this.filter2 = this.app._lokasi+"/"+this.app._userLog+"/"+this.e_nb.getText()+"/"+this.c_status.getText()+"/Approval RRA Anggaran"+"/"+this.app._kodePP+"/"+this.e_nopdrk.getText();
							this.viewer.prepare();
