window.app_saku3_transaksi_siaga_fStruktur = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fStruktur.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_siaga_fStruktur";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Setting Anggaran", 0);	
		
		this.maximize();
		uses("portalui_saiCBBL;datePicker;portalui_checkBox");
		...
		//this.eKlp.setSQL("select kode_fs, nama from fsproduk where kode_lokasi = '"+this.app._lokasi+"' ",["kode_fs","nama"],undefined,["Kode FS","Nama"],"and","Data Versi Neraca");	
	}
};
window.app_saku3_transaksi_siaga_fStruktur.extend(window.portalui_childForm);
window.app_saku3_transaksi_siaga_fStruktur.implement({
			...
			switch (event)
			{
				...
				case "Add" :
				   if ( modalResult == mrOk)
				   {
						...
						{   
							...
							//data.set("modul","B");	
							data.set("tgl_awal",valArray[4])
							data.set("tgl_akhir",valArray[5])
							node.data = data;
						}catch(e)
						{
							alert(e);
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
						...
						{
							var modul = "L";//
							var sql2,sql1 = "delete from gr_so where  kode_lokasi = '"+this.elokasi.getText()+"'";					// kode_lokasi,
							var sql = new server_util_arrayList();
							//sql.add("delete from relakunproduk  where kode_lokasi = '"+this.elokasi.getText()+"' and kode_fs = '"+this.eKlp.getText()+"' "+
							//	"and kode_so in (select kode_so from neracaproduk where  kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"')");																		
							sql.add(sql1);							
							//if mysql langsung
							...
							if (driver.search("mssql") != -1){
								...
								if (value.length!=0){
									...
									for (var i in data){
										sql2 = "insert into gr_so(kode_so, kode_lokasi, nama, level_spasi, level_lap,tipe, sum_header, kode_induk, rowindex,tgl_awal,tgl_akhir) values ";
										sql2 += " "+ data[i] +" ";
										sql.add(sql2);
									}	
								}
	...
	loadMenu: function(daftarNrc){
		try
		{
			...
			var item = undefined;
			var node = undefined;
			var sql="select kode_neraca, kode_akun from relakunproduk where kode_lokasi ='"+this.elokasi.getText()+"' order by kode_akun"
			var line, ix = -1, nrcRls, lineData, nrc_d = this.dblib.loadQuery(sql);												
	...
	doClick: function(sender){
		var nrc = this.dblib.runSQL("select * from gr_so where kode_lokasi = '"+this.elokasi.getText()+"'  order by rowindex",0,0);
		...
		{ 
		 	if (sender == this.relBtn)
			{
				...
				if (this.selectedItem.data.get("tipe").toLowerCase() == "posting"){
					/*
					uses("app_saku2_master_gl_fRelakunProduk",true);
					if (this.relakun != undefined)
						this.relakun.free();
					this.relakun = new app_saku2_master_gl_fRelakunProduk(this.app);
			...
			}else
			{
				if (sender == this.addBtn || sender == this.editBtn){
					uses("app_saku3_transaksi_siaga_fStrukturDetail");
					if (this.entryMenu == undefined)
						this.entryMenu = new app_saku3_transaksi_siaga_fStrukturDetail(this.app);
				...
				if (sender == this.addBtn){
					...
					this.entryMenu.dp_d1.setText(item.data.get("tgl_awal"));
					this.entryMenu.dp_d2.setText(item.data.get("tgl_akhir"));
					this.entryMenu.showModal();		
	...
	copyChilds: function(from, to){
		...
		{			
			...
			chld.data.set("kode_so",temp.getKode());					
	...
	doEditChange: function(sender){
		if (sender == this.elokasi){			
			this.eKlp.setText("");
			this.treev.clear();
			this.eKlp.setSQL("select kode_fs, nama from fsproduk where kode_lokasi = '"+sender.getText()+"' ",["kode_fs","nama"],undefined,["Kode FS","Nama"],"and","Data Versi Neraca");	
	...
	switchItem: function(from, to){
		...
		child.data.set("kode_so",item.getKode());					
