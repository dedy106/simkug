window.app_saku3_transaksi_siaga_fVer5 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fVer5.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fVer5";
		...
		this.c_modul2 = new saiCB(this.pc2.childPage[2],{bound:[20,11,200,20],caption:"Modul",items:["PBAJU2","PJPTG","BMHD"], readOnly:true,tag:9,change:[this,"doChange"]});
...
window.app_saku3_transaksi_siaga_fVer5.extend(window.childForm);
window.app_saku3_transaksi_siaga_fVer5.implement({
	...
	doLoad:function(sender){			
		var strSQL = "select a.tanggal as due_date,a.no_pb as no_bukti,'INPROG' as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,'PBAJU2' as modul,b.kode_pp+' - '+b.nama as pp,"+
					 ...
					 "where a.modul in ('AJU2') and periode >= '201601' and a.periode<='"+this.e_periode.getText()+"' and a.progress='1' and a.kode_lokasi='"+this.app._lokasi+"'  "+
					 //,'PJAJU','PJAJUBMHD', 'BYRBMHD','PBADK','PBCOD','PJAJULOG','GAJI','SPJ','KES','KLAIM'
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {			
				...
				if (this.e_modul.getText() == "PBAJU2" || this.e_modul.getText() == "BMHD") 
				...
				if (typeof data == "object"){
					...
					if (line != undefined){
						...
						if (this.e_modul.getText() == "PBAJU2" || this.e_modul.getText() == "BMHD" || this.e_modul.getText() == "PJPTG") {
							//untuk yg NON PANJAR -- (bukan PJAJU,PJLOG,PJBMHD)							
							this.modulForm = line.modul.toUpperCase();
							if (this.modulForm == "AJU2") this.modulForm = "PBAJU2";
							
							//utk yg pbaju dari modul bayarbmhd---> ganti jadi PBAJU
							if (this.modulForm == "BYRBMHD") this.modulForm = "PBAJU2";
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.c_status.getText() == "RETURN") {
						...
						var vStatus2 = "3";
                        if (this.e_modul.getText() == "PBAJU2" || this.e_modul.getText() == "BMHD"){
                            sql.add("update apv_flow set status = 1 where no_bukti='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and no_urut=(select max(no_urut) as no_urut from apv_flow where kode_lokasi='"+this.app._lokasi+"' and no_bukti='"+this.e_nobukti.getText()+"') ");

							this.no_pesan = this.standarLib.noBuktiOtomatis(this.dbLib,"app_notif_m","no_bukti","PSN"+this.e_periode.getText().substr(2,2)+".","000000");
							...
							"select '"+this.no_pesan+"' as no_bukti,a.kode_lokasi,'Beban','Pengajuan Beban','Pengajuan Beban dengan No Transaksi '+a.no_bukti+' telah direturn oleh "+this.app._userLog+" , menunggu approval anda.',a.nik,getdate(),'-',a.no_bukti,'Beban','-',0,0 "+
							"from apv_flow a "+
							"where a.status=1 and a.no_bukti='"+this.e_nobukti.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'");
                        }	
					...
					// insert catatan approval utk esaku
					sql.add("insert into apv_pesan (no_bukti,kode_lokasi,keterangan,tanggal,no_urut,status,modul,no_ref) values ('"+this.e_nobukti.getText()+"','"+this.app._lokasi+"','"+this.e_memo.getText()+"','"+this.dp_d1.getDateString()+"',10,'"+vStatus2+"','AJU','"+this.e_nb.getText()+"') ");
					...
					if (this.c_status.getText() == "APPROVE") {			
						...
						if (this.modulForm == "PBAJU2" || this.modulForm == "PBADK" || this.modulForm == "PJPTG" || this.modulForm == "KES" || this.modulForm == "KLAIM" || this.modulForm == "SPJ" || this.modulForm == "GAJI") {
		...
		switch (event){
			...
			case "hapus" :	
				...
				if (parseFloat(this.app._periode) > parseFloat(this.e_periode.getText())){
					...
					sql.add("delete from apv_pesan where no_ref='"+this.noVerLama+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doChange:function(sender){		
		...
		if (sender == this.c_modul2) {
			if (this.c_modul2.getText() == "PBAJU2" || this.c_modul2.getText() == "BMHD" || this.c_modul2.getText() == "BYRBMHD") 
				this.cb_nb.setSQL("select no_pb, keterangan from gr_pb_m where modul <> 'AJU' and periode<='"+this.e_periode.getText()+"' and progress in ('2','V','1') and kode_lokasi='"+this.app._lokasi+"'",["no_pb","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);								
		...
		if (sender == this.cb_nb && this.cb_nb.getText() != "") {
			if (this.c_modul2.getText() == "PBAJU2" || this.c_modul2.getText() == "BMHD") {
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_siaga_rptBuktiBebanEmail";
								this.filter = " where a.kode_lokasi='"+this.app._lokasi+"' and a.no_beban='"+this.e_nb.getText()+"' ";
								this.filter2 = this.app._namaUser + "/" +this.app._userLog + "/" + this.app._userPwd + "//"+this.no_pesan+"/"+this.e_nobukti.getText();
								this.viewer.prepare();
