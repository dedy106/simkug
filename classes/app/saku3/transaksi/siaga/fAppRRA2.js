window.app_saku3_transaksi_siaga_fAppRRA2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fAppRRA2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fAppRRA2";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Approval VP User", 0);	
...
window.app_saku3_transaksi_siaga_fAppRRA2.extend(window.childForm);
window.app_saku3_transaksi_siaga_fAppRRA2.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{				
					...
					sql.add("insert into rra_app_m(no_app, kode_lokasi,tanggal,keterangan,modul,periode,no_del,nik_buat,nik_app,nik_user,tgl_input,jenis_form) values "+
						      "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','PUSAT','"+this.e_periode.getText()+"','-','"+this.app._userLog+"','"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'APPVP')");
	...
	doLoad:function(sender){		
		var strSQL = "select a.kode_lokasi,a.kode_pp+' - '+c.nama as pp,a.no_pdrk as no_bukti,b.no_dokumen,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,a.sts_pdrk as jenis "+
								 ...
								 "where a.modul = 'MULTI' and a.progress in ('8') and a.periode<='"+this.e_periode.getText()+"' and a.nik_app1='"+this.app._userLog+"'";							
