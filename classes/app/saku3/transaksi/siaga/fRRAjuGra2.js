window.app_saku3_transaksi_siaga_fRRAjuGra2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siaga_fRRAjuGra2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siaga_fRRAjuGra2";
		...
		this.pc1 = new pageControl(this.pc2.childPage[1],{bound:[1,5,995,327], childPage:["Penerima","Pemberi","Cek Anggaran","Otorisasi"]});				
		...
		this.e_terima = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,18,200,20],caption:"Nilai Penerima", tag:9, tipeText:ttNilai, text:"0",change:[this,"doChange"]});
		...
		this.cb_app = new saiCBBL(this.pc1.childPage[3],{bound:[20,12,220,20],caption:"Approve VP", multiSelection:false, maxLength:10, tag:2});						
		this.cb_app2 = new saiCBBL(this.pc1.childPage[3],{bound:[20,13,220,20],caption:"Approve Dir Unit", multiSelection:false, maxLength:10, tag:2});						
		this.cb_app3 = new saiCBBL(this.pc1.childPage[3],{bound:[20,14,220,20],caption:"Approve Budget", multiSelection:false, maxLength:10, tag:2});						
		this.cb_app4 = new saiCBBL(this.pc1.childPage[3],{bound:[20,15,220,20],caption:"Approve Direksi", multiSelection:false, maxLength:10, tag:2});						
		...
		try {
			...
			this.cb_app2.setSQL("select nik, nama from karyawan where kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Data NIK Approval",true);
			this.cb_app3.setSQL("select nik, nama from karyawan where kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Data NIK Approval",true);
			this.cb_app4.setSQL("select nik, nama from karyawan where kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Data NIK Approval",true);

			this.kodePP = this.app._kodePP;
			this.getOtorisasi();
		}catch(e){
			systemAPI.alert(e);
		}
...
window.app_saku3_transaksi_siaga_fRRAjuGra2.extend(window.childForm);
window.app_saku3_transaksi_siaga_fRRAjuGra2.implement({
	getOtorisasi:function() {
		var str = "select kode_app,nik_app from app_otorisasi where kode_pp='"+this.kodePP+"' and kode_lokasi='"+this.app._lokasi+"' and kode_app='APP1' and "+nilaiToFloat(this.e_terima.getText())+" between nilai_min and nilai_max "+
							"union all "+
							"select kode_app,nik_app from app_otorisasi where kode_pp='"+this.kodePP+"' and kode_lokasi='"+this.app._lokasi+"' and kode_app='APP2' and "+nilaiToFloat(this.e_terima.getText())+" between nilai_min and nilai_max "+
							"union all "+
							"select kode_app,nik_app from app_otorisasi where kode_pp='"+this.kodePP+"' and kode_lokasi='"+this.app._lokasi+"' and kode_app='APP3' and "+nilaiToFloat(this.e_terima.getText())+" between nilai_min and nilai_max "+
							"union all "+
							"select kode_app,nik_app from app_otorisasi where kode_pp='"+this.kodePP+"' and kode_lokasi='"+this.app._lokasi+"' and kode_app='APP4' and "+nilaiToFloat(this.e_terima.getText())+" between nilai_min and nilai_max ";						  							
		var data = this.dbLib.getDataProvider(str,true); 
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			var line;			
			for (var i in data.rs.rows){
				line = data.rs.rows[i];												
				if (line.kode_app=="APP1") this.cb_app.setText(line.nik_app);
				if (line.kode_app=="APP2") this.cb_app2.setText(line.nik_app);
				if (line.kode_app=="APP3") this.cb_app3.setText(line.nik_app);
				if (line.kode_app=="APP4") this.cb_app4.setText(line.nik_app);
			}
		} 
	},
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					//progress panjang mulai dr 7-8/M-9/V-0/D-X
					//koreksi 5-9-18 progress panjang mulai dr 8/9/V-0/D-X
					//koreksi 5-3-22 progress : 8->9/V->1/X
					...
					sql.add("insert into rra_pdrk_m(no_pdrk,kode_lokasi,keterangan,kode_pp,kode_bidang,jenis_agg,tanggal,periode,nik_buat,nik_app1,nik_app2,nik_app3,nik_app4,sts_pdrk,justifikasi, nik_user, tgl_input,progress,modul) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_periode.getText()+"','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.cb_app2.getText()+"','"+this.cb_app3.getText()+"','"+this.cb_app4.getText()+"','RRR','-','"+this.app._userLog+"',getdate(),'8','MULTI')");
	...
	doChange:function(sender){																		
		try{
			if (sender == this.c_bulanT || sender == this.cb_akunT) {							
				var data = this.dbLib.getDataProvider("select distinct kode_pp from relakun_pp where kode_lokasi='"+this.app._lokasi+"' and kode_akun='"+this.cb_akunT.getText()+"'",true);
				...
				if (sender == this.e_terima && this.e_terima.getText()!="") {
					getOtorisasi();
				}
			}		
		}
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){							
							if (this.preView == "1") {								
								this.nama_report = "server_report_saku3_siaga_rptAggPdrk2";
								this.filter = " where a.kode_lokasi='" + this.app._lokasi + "' and a.no_pdrk='" + this.e_nb.getText() + "' ";
								this.filter2 = this.app._lokasi+"/"+this.app._userLog+"/"+this.e_nb.getText()+"/Pengajuan/Pengajuan RRA";
								this.viewer.prepare();
	...
	doLoad3:function(sender){																							
		var strSQL = "select a.no_pdrk,convert(varchar,a.tanggal,103) as tgl,b.no_dokumen,a.keterangan,a.progress "+
		             "from rra_pdrk_m a inner join anggaran_m b on a.no_pdrk=b.no_agg and a.kode_lokasi=b.kode_lokasi "+					 					 
					 			 "where a.modul = 'MULTI' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress in ('8','V','X') order by a.tanggal";		//ubah  5/3/22 --> and a.progress in ('8','V','D','X')
