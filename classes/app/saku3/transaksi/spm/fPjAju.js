window.app_saku3_transaksi_spm_fPjAju = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_fPjAju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fPjAju";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Input Permohonan Panjar", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,12,990,350], childPage:["Pemegang & Approval","Catatan","Panjar Open","File Dok"]});		
		...
		this.sg6 = new saiGrid(this.pc1.childPage[2],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:6,tag:9,
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag,value1 from spro where kode_spro in ('MAXPJR','GARFREE','DOKFREE') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku3_transaksi_spm_fPjAju.extend(window.childForm);
window.app_saku3_transaksi_spm_fPjAju.implement({
	...
	doChange:function(sender){		
		if (sender == this.cb_buat && this.cb_buat.getText() !="" && this.stsSimpan == 1) {
			...
			var str = "select a.no_panjar,a.keterangan,convert(varchar,a.tanggal,103) as tgl,a.nilai,convert(varchar,a.due_date,103) as tgl_pakai,DATEDIFF(DAY,a.tanggal,getdate()) as umur "+
					  ...
					  "where b.no_pj is null and a.nik_buat='"+this.cb_buat.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_panjar,convert(varchar,a.tanggal,103) as tgl,case a.progress when '0' then 'AJU' when 'C' then 'REVISI' when 'V' then 'REV.VER' end as jenis,a.no_dokumen,a.keterangan,a.nik_buat+' - '+b.nama as nama,a.nilai, "+
					 ...
					 "where a.kode_pp='"+this.app._kodePP+"' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress in ('0','C','V','S') and a.modul='PJAJU'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
