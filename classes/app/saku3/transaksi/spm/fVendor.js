window.app_saku3_transaksi_spm_fVendor = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_fVendor.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fVendor";
		...
		this.cb_pp = new saiCBBL(this.pc1.childPage[1],{bound:[20,12,220,20],caption:"Cabang/PP", multiSelection:false, readOnly:false, tag:0});
		this.cb_klp = new saiCBBL(this.pc1.childPage[1],{bound:[20,13,220,20],caption:"Kelompok", multiSelection:false, maxLength:10, tag:2});	
		...
		try {
			...
			this.cb_klp.setSQL("select kode_klpvendor, nama from vendor_klp where kode_lokasi='"+this.app._lokasi+"'",["kode_klpvendor","nama"],false,["Kode","Nama"],"and","Daftar Kelompok",true);			
...
window.app_saku3_transaksi_spm_fVendor.extend(window.childForm);
window.app_saku3_transaksi_spm_fVendor.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into vendor(kode_vendor,kode_lokasi,nama,alamat,no_tel,email,npwp,pic,alamat2,bank,cabang,no_rek,nama_rek,no_fax,no_pictel,spek,kode_klpvendor,penilaian,bank_trans,akun_pph,kode_pp,flag_aktif) values "+
						    "('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.e_alamat.getText()+"','"+this.e_tel.getText()+"','"+this.e_mail.getText()+"','"+this.e_npwp.getText()+"','"+this.e_pic.getText()+"','"+this.e_alamat2.getText()+"','"+this.e_bank.getText()+"','"+this.e_cabang.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"','"+this.e_fax.getText()+"','"+this.e_pictel.getText()+"','-','"+this.cb_klp.getText()+"','-','-','-','"+this.cb_pp.getText()+"','"+this.c_status.getText().substr(0,1)+"')");
					setTipeButton(tbAllFalse);
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select kode_vendor,nama,alamat,no_tel,email,npwp,alamat2,pic,bank,cabang,no_rek,nama_rek,no_fax,no_pictel,kode_pp,kode_klpvendor,flag_aktif "+
				             "from vendor "+
						     "where kode_vendor ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   				
