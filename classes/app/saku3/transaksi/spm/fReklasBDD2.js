window.app_saku3_transaksi_spm_fReklasBDD2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_fReklasBDD2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fReklasBDD2";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penyelesaian BDD", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,12,990,265], childPage:["Data BDD","Data Anggaran"]});
...
window.app_saku3_transaksi_spm_fReklasBDD2.extend(window.childForm);
window.app_saku3_transaksi_spm_fReklasBDD2.implement({		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,4])){
				try{														
					...
					sql.add("insert into spm_proyek_reklas_m(no_reklas,kode_lokasi,no_dokumen,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_setuju,tgl_input,nik_user,posted,ref1 ) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','REKLAS','BDD','"+this.e_periode.getText()+"','IDR',1,"+nilaiToFloat(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','"+this.cb_proyek.getText()+"')");
