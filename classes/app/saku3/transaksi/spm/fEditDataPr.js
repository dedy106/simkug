window.app_saku3_transaksi_spm_fEditDataPr = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_fEditDataPr.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fEditDataPr";
		...
		this.c_status = new saiCB(this,{bound:[20,22,200,20],caption:"Jenis",items:["PBR","HUTANG","REKLAS","PIUTANG"], readOnly:true,tag:2, change:[this,"doChange"]});
...
window.app_saku3_transaksi_spm_fEditDataPr.extend(window.childForm);
window.app_saku3_transaksi_spm_fEditDataPr.implement({
			...
			if (sender == this.cb_bukti && this.cb_bukti.getText() != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){						
						...
						this.cb_prbaru.setSQL("select kode_proyek,nama from spm_proyek where kode_pp='"+this.kodepp+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_proyek","nama"],false,["Kode","Nama"],"and","Data Proyek",true);
					}
				}
			}
