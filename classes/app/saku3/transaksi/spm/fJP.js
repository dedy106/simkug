window.app_saku3_transaksi_spm_fJP = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_fJP.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_spm_fJP";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Jurnal Penutup Akhir Tahun [PP]", 0);	
		...
		this.p1 = new portalui_panel(this,{bound:[20,189,900,344],caption:"Data Saldo Akun PP"});
		this.sg1 = new saiGrid(this.p1,{bound:[0,20,this.p1.width-5,this.p1.height-40],colCount:6,tag:0,
		            colTitle:["Kode Akun","Kode PP","Saldo Awal","Debet","Kredit","Saldo Akhir"],
					colWidth:[[5,4,3,2,1,0],[120,120,120,120,100,100]], colFormat:[[5,4,3,2],[cfNilai,cfNilai,cfNilai,cfNilai]],
...
window.app_saku3_transaksi_spm_fJP.extend(window.portalui_childForm);
window.app_saku3_transaksi_spm_fJP.implement({
	doUploadTB: function(){		
		try{			
			...
			var sql = "call sp_glmapp ('"+this.app._lokasi+"','"+this.ed_period.getText()+"','"+this.nik_user+"')";		
			this.dbLib.execQuerySync(sql);			
			
			sql = "select a.kode_akun,a.kode_pp,a.sawal,a.debet,a.kredit,a.sakhir "+
				  "from glmapp_tmp a inner join masakun b on b.kode_akun = a.kode_akun and b.kode_lokasi = a.kode_lokasi and b.modul = 'L' "+
				  "where a.kode_lokasi ='"+this.app._lokasi+"' and a.nik_user = '"+this.nik_user+"' and a.sakhir <> 0 order by a.kode_akun,a.kode_pp";
			...
			for (var i=0;i < this.dataJU.rs.rows.length;i++){
				...
				if (line.sakhir > 0) totD += parseFloat(line.sakhir);
				if (line.sakhir < 0) totC += parseFloat(line.sakhir);
			}
			...
			case "simpan" :
					...
					try{						
						if (this.standarLib.checkEmptyByTag(this, new Array("0","1","2"))){
							...
							for (var i=0;i < this.dataJU.rs.rows.length;i++){
								line = this.dataJU.rs.rows[i];
								if (parseFloat(line.sakhir) > 0) dc = "C"; else dc = "D";
								nilai = Math.abs(parseFloat(line.sakhir));
								sql.add("insert into jp_d (no_bukti,kode_lokasi,kode_akun,dc,nilai,kode_pp) values ('"+this.ed_nb.getText()+"','"+this.app._lokasi+"','"+line.kode_akun+"','"+dc+"',"+nilai+",'"+line.kode_pp+"')");
								sql.add("insert into gldt (no_bukti,no_urut,kode_lokasi,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,modul,jenis,periode,kode_curr,kurs,nilai_curr,tgl_input,nik_user) values "+
										"('"+this.ed_nb.getText()+"',"+i+",'"+this.app._lokasi+"','-','"+tgl+"','"+line.kode_akun+"','"+dc+"',"+nilai+",'Jurnal Penutup Periode "+this.ed_period.getText()+"','"+line.kode_pp+"','-','-','-','-','-','-','-','JP','AKUN','"+this.ed_period.getText()+"','IDR',1,"+nilai+",getdate(),'"+this.app._userLog+"')");
							}
							if (nilaiToFloat(this.ed_jp.getText()) > 0) dc = "D"; else dc = "C";
							nilai = Math.abs(nilaiToFloat(this.ed_jp.getText()));
							sql.add("insert into gldt (no_bukti,no_urut,kode_lokasi,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,modul,jenis,periode,kode_curr,kurs,nilai_curr,tgl_input,nik_user) values "+
									"('"+this.ed_nb.getText()+"',99999,'"+this.app._lokasi+"','-','"+tgl+"','"+this.akunJP+"','"+dc+"',"+nilai+",'Jurnal Penutup Periode "+this.ed_period.getText()+"','"+this.app._kodePP+"','-','-','-','-','-','-','-','JP','JP','"+this.ed_period.getText()+"','IDR',1,"+nilai+",getdate(),'"+this.app._userLog+"')");
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU.rs.rows[i];							
			this.sg1.appendData([line.kode_akun,line.kode_pp,floatToNilai(line.sawal),floatToNilai(line.debet),floatToNilai(line.kredit),floatToNilai(line.sakhir)]);
		}
