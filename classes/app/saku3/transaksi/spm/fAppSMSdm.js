window.app_saku3_transaksi_spm_fAppSMSdm = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_spm_fAppSMSdm.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fAppSMSdm";
...
window.app_saku3_transaksi_spm_fAppSMSdm.extend(window.childForm);
window.app_saku3_transaksi_spm_fAppSMSdm.implement({	
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "hapus" :	
				...
				if (this.e_modul.getText() == "PBBAU") sql.add("update yk_pb_m set no_app2='-',progress='1' where no_pb='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
		...
		if (this.cb_pp.getText() != "") {
			if (this.c_modul3.getText() == "PBBAU")	{
				strSQL = "select a.due_date,a.no_pb as no_bukti,'INPROG' as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_app2,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
						 "from yk_pb_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
						 "               inner join karyawan c on a.nik_user=c.nik and a.kode_lokasi=c.kode_lokasi "+
						 "where a.progress='1' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='PBBAU' and a.kode_pp='"+this.cb_pp.getText()+"'";
			} 
		}				 					
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
