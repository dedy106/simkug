window.app_saku3_transaksi_spm_fAkunPP = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_fAkunPP.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fAkunPP";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Relasi PP - Akun", 0);	
...
window.app_saku3_transaksi_spm_fAkunPP.extend(window.childForm);
window.app_saku3_transaksi_spm_fAkunPP.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("delete from relakun_pp2 where kode_pp = '"+this.cb_pp.getText()+"'");								
																		
					for (var i=0;i < this.sg.getRowCount();i++){
						if (this.sg.rowValid(i)) {
							sql.add("insert into relakun_pp2 (kode_pp,kode_lokasi,kode_akun) values ('"+this.cb_pp.getText()+"','"+this.app._lokasi+"','"+this.sg.cells(0,i)+"')");
						}
					}							
	...
	doChange: function(sender){
		try{
			if (this.cb_pp.getText() != ""){						
				var strSQL = "select a.kode_akun,a.nama from masakun a inner join relakun_pp2 b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+						 
