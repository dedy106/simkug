window.app_saku3_transaksi_spm_fPtgAju = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_fPtgAju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fPtgAju";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,10,990,347], childPage:["Data Panjar","Item Mata Anggaran","KPA Anggaran","Catatan","File Dok"]});
...
window.app_saku3_transaksi_spm_fPtgAju.extend(window.childForm);
window.app_saku3_transaksi_spm_fPtgAju.implement({
	...
	doLoad3:function(sender){																				
		var strSQL = "select c.no_ptg,a.no_panjar,convert(varchar,c.tanggal,103) as tgl,case c.progress when '0' then 'AJU' when 'C' then 'REVISI' when 'V' then 'REV.VER' end as jenis,c.no_dokumen,c.keterangan,a.nik_buat+' - '+b.nama as nama,c.nilai, "+
					 ...
					 "where c.modul = 'PJPTG' and c.periode<='"+this.e_periode.getText()+"' and c.kode_lokasi='"+this.app._lokasi+"' and c.progress in ('0','C','V','S') and d.kode_pp='"+this.app._kodePP+"'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
