window.app_saku3_transaksi_spm_fBillDummyRev = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_fBillDummyRev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fBillDummyRev";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,280], childPage:["Data Akru"]});
...
window.app_saku3_transaksi_spm_fBillDummyRev.extend(window.childForm);
window.app_saku3_transaksi_spm_fBillDummyRev.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into spm_piutang_m(no_piutang,kode_lokasi,no_dokumen,tanggal,keterangan,kode_proyek,kode_cust,kode_curr,kurs,nik_app,kode_pp,nilai,periode,nik_user,tgl_input,akun_piutang,posted,nilai_ppn,modul) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_bill.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_proyek.getText()+"','"+this.cb_cust.getText()+"','IDR',1,'"+this.app._userLog+"','"+this.app._kodePP+"',"+parseNilai(this.e_total.getText())+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.akunPiu+"','F',0,'REVBILL')");
	...
	doLoad3:function(sender){																								
		var strSQL = "select a.no_piutang,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
					 ...
					 "where a.kode_pp='"+this.app._kodePP+"' and a.posted='F' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'  and a.modul='REVBILL'";					 
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
