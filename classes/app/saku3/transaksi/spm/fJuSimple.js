window.app_saku3_transaksi_spm_fJuSimple = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_fJuSimple.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fJuSimple";
		...
		this.pc1 = new pageControl(this.pc2.childPage[1],{bound:[5,12,990,320], childPage:["Data Item Jurnal","Data Anggaran","Referensi Jurnal","Otorisasi"]});
		...
		this.cb_app = new saiCBBL(this.pc1.childPage[3],{bound:[20,17,200,20],caption:"Diperiksa Oleh", multiSelection:false, maxLength:10, tag:2});
...
window.app_saku3_transaksi_spm_fJuSimple.extend(window.childForm);
window.app_saku3_transaksi_spm_fJuSimple.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','JU','UMUM','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.app._userLog+"','"+this.cb_app.getText()+"','F','-','-','-',getdate(),'"+this.app._userLog+"')");
