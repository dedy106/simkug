window.app_saku3_transaksi_spm_fBill = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_fBill.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fBill";
		...
		this.cb_pph = new saiCBBL(this.pc2.childPage[0],{bound:[20,19,220,20],caption:"Akun PPh", multiSelection:false, maxLength:10, tag:2});								
		this.e_nilai = new saiLabelEdit(this.pc2.childPage[0],{bound:[750,19,210,20],caption:"Nilai", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});										
		...
		this.e_ppn = new saiLabelEdit(this.pc2.childPage[0],{bound:[750,19,210,20],caption:"PPN", tag:1, tipeText:ttNilai, text:"0",change:[this,"doChange"]});	//fee jtk bisa diubah	
		this.i_ppn = new portalui_imageButton(this.pc2.childPage[0],{bound:[970,19,20,20],hint:"Hit PPN",image:"icon/"+system.getThemes()+"/tabCont2.png",click:[this,"doPPN"]});				
		
		this.e_fp = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,17,250,20],caption:"Faktur Pajak", maxLength:50});
		this.e_total = new saiLabelEdit(this.pc2.childPage[0],{bound:[750,17,210,20],caption:"Total", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,234], childPage:["Data Billing"]});
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:4,tag:0,
		            colTitle:["Kode PP","Nama PP","Nilai UnBill","Nilai Bill"],
					colWidth:[[3,2,1,0],[100,100,400,100]],										
...
window.app_saku3_transaksi_spm_fBill.extend(window.childForm);
window.app_saku3_transaksi_spm_fBill.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into spm_piutang_m(no_piutang,kode_lokasi,no_dokumen,tanggal,keterangan,kode_proyek,kode_cust,kode_curr,kurs,nik_app,kode_pp,nilai,periode,nik_user,tgl_input,akun_piutang,posted,nilai_ppn,modul,nilai_pph,akun_pph,no_fpajak) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_proyek.getText()+"','"+this.cb_cust.getText()+"','IDR',1,'"+this.app._userLog+"','"+this.app._kodePP+"',"+parseNilai(this.e_nilai.getText())+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.akunPiu+"','F',"+parseNilai(this.e_ppn.getText())+",'BILL',"+parseNilai(this.e_pph.getText())+",'"+this.cb_pph.getText()+"','"+this.e_fp.getText()+"')");									
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){															
								...
								sql.add("insert into spm_piutang_d(no_piutang,kode_lokasi,periode,kode_proyek,kode_pp,nilai,dc,no_ref) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.sg.cells(0,i)+"',"+nilaiToFloat(this.sg.cells(3,i))+",'D','-')");
								sql.add("insert into spm_piutang_j(no_piutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+j+",'"+this.akunPdpt+"','"+this.e_ket.getText()+"','C','IDR',1,"+parseNilai(this.sg.cells(3,i))+","+parseNilai(this.sg.cells(3,i))+",'"+this.sg.cells(0,i)+"','-','"+this.app._lokasi+"','PIUUMUM','PDPT','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");								
							}
						}
					}
					setTipeButton(tbAllFalse);					
				...
				for (var i=0;i < this.sg.getRowCount();i++){
					if (this.sg.rowValid(i)){															
						...
						if (nilaiToFloat(this.sg.cells(2,i)) <  nilaiToFloat(this.sg.cells(3,i))) {
							system.alert(this,"Transaksi tidak valid.","Nilai Bill melebihi saldo UnBill. (Baris "+j+")");
							return false;
						}
					}
				}
	...
	doChange:function(sender){	
		if (sender == this.cb_cust && this.cb_cust.getText()!="") {
			var strSQL = "select wapu from cust "+
		...
		if (sender == this.cb_proyek && this.cb_proyek.getText()!="") {
			...
			if (this.stsSimpan == 1) {
				var data = this.dbLib.getDataProvider("select a.kode_pp,b.nama,a.nilai-isnull(c.nilai_tagih,0) as saldo "+
							"from spm_proyek_pdpt a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
							"			left join (select kode_proyek,kode_pp,sum(case dc when 'D' then nilai else -nilai end) as nilai_tagih "+
							"                      from spm_piutang_d where kode_lokasi='"+this.app._lokasi+"' group by kode_proyek,kode_pp) c on a.kode_proyek=c.kode_proyek and a.kode_pp=c.kode_pp "+
							"where a.kode_proyek = '"+this.cb_proyek.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.kode_pp",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];												
						this.sg.appendData([line.kode_pp,line.nama,floatToNilai(line.saldo),"0"]);
					}
				} else this.sg.clear(1);							
			}
		}		
		if ((sender == this.e_ppn || this.e_pph) && this.e_ppn.getText()!="" && this.e_pph.getText()!="") {			
				...
				switch(methodName){
					case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_tm_rptPiutangJurnalPry";
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider("select a.kode_pp,b.nama,a.nilai-isnull(c.nilai_tagih,0) as saldo,d.nilai "+
							"from spm_proyek_pdpt a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
							"			inner join spm_piutang_d d on a.kode_proyek=d.kode_proyek and a.kode_pp=d.kode_pp and a.kode_lokasi=d.kode_lokasi "+
							"			left join (select kode_proyek,kode_pp,sum(case dc when 'D' then nilai else -nilai end) as nilai_tagih "+
							"                      from spm_piutang_d where no_piutang <>'"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' group by kode_proyek,kode_pp) c on a.kode_proyek=c.kode_proyek and a.kode_pp=c.kode_pp "+
							"where a.kode_proyek = '"+this.cb_proyek.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.kode_pp",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];												
						this.sg.appendData([line.kode_pp,line.nama,floatToNilai(line.saldo),floatToNilai(line.nilai)]);
					}
				} else this.sg.clear(1);							
			}									
		} catch(e) {alert(e);}
