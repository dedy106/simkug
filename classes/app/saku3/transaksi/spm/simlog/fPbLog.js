window.app_saku3_transaksi_spm_simlog_fPbLog = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_simlog_fPbLog.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_simlog_fPbLog";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Permohonan Bayar via BAST Logistik", 0);	
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:10,tag:0,
		            colTitle:["No Hutang","Tanggal","Keterangan","Akun AP","Nilai","Saldo","Mitra","Proyek","Deskripsi","No PO"],
					colWidth:[[9,8,7,6,5,4,3,2,1,0],[150,150,80,200,100,100,70,290,70,100]],					
		...
		this.e_nopo = new saiLabelEdit(this.pc1.childPage[2],{bound:[750,15,220,20],caption:"No PO", readOnly:true,tag:4});						
...
window.app_saku3_transaksi_spm_simlog_fPbLog.extend(window.childForm);
window.app_saku3_transaksi_spm_simlog_fPbLog.implement({		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,4])){
				try{														
					...
					sql.add("insert into spm_rek(no_bukti,kode_lokasi,modul,nama_rek,no_rek,bank,cabang,bruto,pajak,nilai) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','PBLOG','"+this.e_namarek.getText()+"','"+this.e_norek.getText()+"','"+this.e_bank.getText()+"','"+this.e_cabang.getText()+"',"+nilaiToFloat(this.e_total.getText())+",0,"+nilaiToFloat(this.e_total.getText())+")");
	...
	doDoubleClick: function(sender, col , row) {		
		if (this.sg.cells(0,row) != "") {
			...
			this.e_nopo.setText(this.sg.cells(9,row));					
	...
	dataHutang:function(sender){				
		if (this.e_periode.getText()!="") {
			//if (this.app._userStatus == "A") var vPP = " "; else 
			var vPP = "z.kode_pp ='"+this.app._kodePP+"' and ";
			var strSQL = "select a.no_hutang,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.akun_hutang,a.nilai,a.nilai-isnull(pb,0) as saldo,b.kode_vendor+' - '+b.nama as vendor,a.kode_proyek,isnull(e.nama,'-') as nama_proyek,a.no_ref as no_po  "+
					 ...
					 "                           from yk_pb_m where kode_lokasi='"+this.app._lokasi+"' and modul='PBLOG' "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData([line.no_hutang,line.tgl,line.keterangan,line.akun_hutang,floatToNilai(line.nilai),floatToNilai(line.saldo),line.vendor,line.kode_proyek,line.nama_proyek,line.no_po]);
				}
	...
	doLoad3:function(sender){																						
		...
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.modul,a.no_dokumen,a.keterangan,a.progress,a.nilai "+
		             "from yk_pb_m a "+					 					 
					 "where "+pp+" a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'PBLOG' and a.progress in ('0','R','V') and posted='F'";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.no_hutang,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.akun_hutang,a.nilai,a.nilai-isnull(pb,0) as saldo,b.kode_vendor+' - '+b.nama as vendor,a.kode_proyek,isnull(e.nama,'-') as nama_proyek,a.no_ref as no_po "+
							 ...
							 "                left join spm_proyek e on a.kode_proyek=e.kode_proyek and a.kode_lokasi=e.kode_lokasi "+
							 "                left join (select kode_lokasi,no_hutang,sum(nilai_final) as pb "+
							 "                           from yk_pb_m where kode_lokasi='"+this.app._lokasi+"' and modul='PBLOG' and no_pb<>'"+this.e_nb.getText()+"' "+
							 "                           group by kode_lokasi,no_hutang) c on a.no_hutang=c.no_hutang and a.kode_lokasi=c.kode_lokasi "+
