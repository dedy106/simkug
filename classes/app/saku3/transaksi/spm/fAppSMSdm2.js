window.app_saku3_transaksi_spm_fAppSMSdm2 = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_spm_fAppSMSdm2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fAppSMSdm2";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Approval SM SDM", 0);	
		...
		this.c_modul3 = new saiCB(this,{bound:[20,10,200,20],caption:"Modul",items:["PBBAU"], tag:9,change:[this,"doChange"]});
		...
		this.sg1 = new saiGrid(this.pc1.childPage[1],{bound:[1,16,this.pc1.width-5,212],colCount:7,tag:0,
		...
		this.c_modul2 = new saiCB(this.pc1.childPage[3],{bound:[20,11,200,20],caption:"Modul",items:["PBBAU"], readOnly:true,tag:9,change:[this,"doChange"]});
...
window.app_saku3_transaksi_spm_fAppSMSdm2.extend(window.childForm);
window.app_saku3_transaksi_spm_fAppSMSdm2.implement({	
		...
		try{														
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					//---------------- flag bukti									
					if (this.e_modul.getText() == "PBBAU") sql.add("update yk_pb_m set no_app2='"+this.e_nb.getText()+"',progress='"+vStatus+"' where no_pb='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");																	
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "hapus" :	
				...
				if (this.e_modul.getText() == "PBBAU") sql.add("update yk_pb_m set no_app2='-',progress='9' where no_pb='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doChange:function(sender){				
		...
		if (sender == this.c_modul2) {
			if (this.c_modul2.getText() == "PBBAU") this.cb_nb.setSQL("select no_pb, keterangan from yk_pb_m where modul = 'PBBAU' and periode<='"+this.e_periode.getText()+"' and progress in ('2','S') and kode_lokasi='"+this.app._lokasi+"'",["no_pb","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);					
		}
	...
	doLoad:function(sender){															
		if (this.c_modul3.getText() == "" && this.cb_pp.getText() == "")	{																
			var strSQL = "select a.due_date,a.no_pb as no_bukti,'INPROG' as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_app2,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
						 "from yk_pb_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
						 "               inner join karyawan c on a.nik_user=c.nik and a.kode_lokasi=c.kode_lokasi "+
						 "where a.progress='9' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='PBBAU' "+					 
		...
		if (this.cb_pp.getText() != "") {
			if (this.c_modul3.getText() == "PBBAU")	{
				strSQL = "select a.due_date,a.no_pb as no_bukti,'INPROG' as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_app2,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
						 "from yk_pb_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
						 "               inner join karyawan c on a.nik_user=c.nik and a.kode_lokasi=c.kode_lokasi "+
						 "where a.progress='9' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='PBBAU' and a.kode_pp='"+this.cb_pp.getText()+"'";
			} 
		}				 					
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
