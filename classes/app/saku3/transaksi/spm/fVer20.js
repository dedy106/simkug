window.app_saku3_transaksi_spm_fVer20 = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_spm_fVer20.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fVer20";
		...
		this.pc1 = new pageControl(this,{bound:[10,18,1000,380], childPage:["Daftar Bukti","Verifikasi","Item Verifikasi","Jurnal & Budget","File Dok","Filter Data"]});		
		...
		this.sg1 = new saiGrid(this.pc1.childPage[1],{bound:[1,16,this.pc1.width-5,160],colCount:7,tag:9,
		...
		this.cb_bdh = new saiCBBL(this.pc1.childPage[2],{bound:[20,11,220,20],caption:"Bendahara", multiSelection:false, maxLength:10, tag:2});							
		this.cb_fiat = new saiCBBL(this.pc1.childPage[2],{bound:[20,13,220,20],caption:"Fiatur", multiSelection:false, maxLength:10, tag:2});							
		...
		this.sg3 = new saiGrid(this.pc1.childPage[3],{bound:[1,5,this.pc1.width-5,200],colCount:9,tag:9,
		...
		this.c_modul2 = new saiCB(this.pc1.childPage[5],{bound:[20,11,200,20],caption:"Modul",items:["IFAJU","IFREIM","PBBAU","PBPR","PBBDD","PBLOG","PJAJU","PJPTG","SPPD"], readOnly:true,tag:9,change:[this,"doChange"]});
		...
		try {			
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('NIKBDH','NIKFIAT') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku3_transaksi_spm_fVer20.extend(window.childForm);
window.app_saku3_transaksi_spm_fVer20.implement({	
		...
		try{														
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.c_status.getText() == "RETURN") var vStatus = "V"; else var vStatus = "3";										
					sql.add("update spm_app_m set no_flag='"+this.e_nb.getText()+"' where no_bukti='"+this.e_nobukti.getText()+"' and no_flag='-' and form='APPVER' and modul='"+this.e_modul.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add("insert into spm_app_m (no_app,kode_lokasi,tanggal,periode,tgl_input,nik_user,status,modul,form,no_bukti,catatan,no_flag,nik_bdh,nik_fiat) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_periode.getText()+"',getdate(),'"+this.app._userLog+"','"+vStatus+"','"+this.e_modul.getText()+"','APPVER','"+this.e_nobukti.getText()+"','"+this.e_memo.getText()+"','-','"+this.cb_bdh.getText()+"','"+this.cb_fiat.getText()+"')");
												
					//---------------- flag bukti					
					if (this.e_modul.getText() == "IFAJU") {
						sql.add("update spm_if_m set no_ver='"+this.e_nb.getText()+"',progress='"+vStatus+"'  where no_if='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
						if (vStatus == "3") {
							sql.add("insert into spm_app_j (no_app,no_bukti,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,nilai_curr,kode_pp,kode_drk,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) "+
									"select '"+this.e_nb.getText()+"',no_if,no_dokumen,tanggal,0,akun_if,keterangan,'D',nilai,nilai,kode_pp,'-','-',kode_lokasi,'"+this.e_modul.getText()+"','IF',periode,'IDR',1,nik_user,tgl_input "+
									"from spm_if_m where no_if = '"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
						}
					}
					if (this.e_modul.getText() == "IFREIM") {
						sql.add("update spm_ifreim_m set no_ver='"+this.e_nb.getText()+"',progress='"+vStatus+"' where no_reim='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
						if (vStatus == "3") {
							sql.add("insert into spm_app_j (no_app,no_bukti,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,nilai_curr,kode_pp,kode_drk,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) "+
									"select '"+this.e_nb.getText()+"',no_reim,'-',tanggal,0,kode_akun,keterangan,'D',nilai,nilai,kode_pp,kode_drk,'-',kode_lokasi,modul,jenis,periode,'IDR',1,nik_user,tgl_input "+
									"from spm_ifreim_j where jenis='HUTIF' and no_reim = '"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
							sql.add("insert into spm_app_j (no_app,no_bukti,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,nilai_curr,kode_pp,kode_drk,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) "+
									"select '"+this.e_nb.getText()+"',c.no_reim,'-',c.tanggal,0,a.kode_akun,a.keterangan,'D',a.nilai,a.nilai,a.kode_pp,a.kode_drk,'-',a.kode_lokasi,a.modul,a.jenis,c.periode,'IDR',1,nik_user,a.tgl_input "+
									"from spm_ifganti_j a "+
									"inner join spm_ifganti_m b on a.no_ganti=b.no_ganti and a.kode_lokasi=b.kode_lokasi "+
									"inner join spm_ifreim_m c on b.no_reim=c.no_reim and a.kode_lokasi=c.kode_lokasi "+
									"where a.jenis='HUTIF' and c.no_reim = '"+this.e_nobukti.getText()+"' and c.kode_lokasi='"+this.app._lokasi+"'");					
						}
					}
					if (this.e_modul.getText() == "PBBAU") {
						sql.add("update yk_pb_m set no_ver='"+this.e_nb.getText()+"',progress='"+vStatus+"' where no_pb='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
						if (vStatus == "3") {
							sql.add("insert into spm_app_j (no_app,no_bukti,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,nilai_curr,kode_pp,kode_drk,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) "+
									"select '"+this.e_nb.getText()+"',no_pb,'-',tanggal,0,kode_akun,keterangan,'D',nilai,nilai,kode_pp,kode_drk,'-',kode_lokasi,modul,jenis,periode,'IDR',1,nik_user,tgl_input "+
									"from yk_pb_j where jenis='BMHD' and no_pb = '"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						}
					}
					...
					if (this.e_modul.getText() == "PJAJU") {
						sql.add("update panjar2_m set no_ver='"+this.e_nb.getText()+"',progress='"+vStatus+"' where no_panjar='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
						if (vStatus == "3") {
							sql.add("insert into spm_app_j (no_app,no_bukti,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,nilai_curr,kode_pp,kode_drk,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) "+
									"select '"+this.e_nb.getText()+"',no_panjar,'-',tanggal,0,akun_panjar,keterangan,'D',nilai,nilai,kode_pp,'-','-',kode_lokasi,modul,'PJ',periode,'IDR',1,nik_user,tgl_input "+
									"from panjar2_m where no_panjar = '"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						}
					}
					if (this.e_modul.getText() == "PJPTG" || this.e_modul.getText() == "PRPTG") {
						sql.add("update panjarptg2_m set no_ver='"+this.e_nb.getText()+"',progress='"+vStatus+"' where no_ptg='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
					}
					
					if (this.e_modul.getText() != "PJPTG" && this.e_modul.getText() != "PRPTG") {
					
						//---------------- jurnal													
						var tglTrans = this.e_tgl.getText().substr(6,4)+"-"+this.e_tgl.getText().substr(3,2)+"-"+this.e_tgl.getText().substr(0,2);
						var period  = this.e_tgl.getText().substr(6,4)+this.e_tgl.getText().substr(3,2);						
						for (var i=0;i < this.sgv.getRowCount();i++){
							if (this.sgv.rowValid(i)){
								var nu = 1000+i;
								sql.add("insert into spm_app_j (no_app,no_bukti,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,nilai_curr,kode_pp,kode_drk,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input)  values "+
										"('"+this.e_nb.getText()+"','"+this.e_nobukti.getText()+"','-','"+tglTrans+"',"+nu+",'"+this.sgv.cells(0,i)+"','"+this.sgv.cells(3,i)+"','"+this.sgv.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sgv.cells(4,i))+","+nilaiToFloat(this.sgv.cells(4,i))+",'"+this.sgv.cells(5,i)+"','-','-','"+this.app._lokasi+"','"+this.e_modul.getText()+"','VER','"+period+"','IDR',1,'"+this.app._userLog+"',getdate())");
							}
						}
					
						//---------------- rekening
						sql.add("delete from spm_rek where no_bukti='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){
								var neto = nilaiToFloat(this.sg1.cells(4,i)) - nilaiToFloat(this.sg1.cells(5,i));
								sql.add("insert into spm_rek(no_bukti,kode_lokasi,modul,nama_rek,no_rek,bank,cabang,bruto,pajak,nilai) values "+
										"('"+this.e_nobukti.getText()+"','"+this.app._lokasi+"','"+this.e_modul.getText()+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i)+"','"+this.sg1.cells(0,i)+"','"+this.sg1.cells(1,i)+"',"+nilaiToFloat(this.sg1.cells(4,i))+","+nilaiToFloat(this.sg1.cells(5,i))+","+neto+")");
							}
						}
					...
					if (this.e_modul.getText() == "SPPD") {
						sql.add("update pd_spj_m set no_ver='"+this.e_nb.getText()+"',progress='"+vStatus+"' where no_spj='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
						if (vStatus == "3") {
							sql.add("insert into spm_app_j (no_app,no_bukti,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,nilai_curr,kode_pp,kode_drk,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) "+
									"select '"+this.e_nb.getText()+"',a.no_spj,'-',a.tanggal,0,aa.kode_akun,aa.tujuan as keterangan,'D',dd.nilai,dd.nilai,aa.kode_pp,'-' as kode_drk,'-',a.kode_lokasi,'SPPD','BEBAN',a.periode,'IDR',1,'"+this.app._userLog+"',getdate() "+
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					this.sg1.clear(1); this.sg2.clear(1); this.sg3.clear(1); this.sgv.clear(1); this.sg1mp2.clear(1);
					this.doClick();
					this.doLoad();					
			...
			case "hapus" :	
				...
				uses("server_util_arrayList");
				var sql = new server_util_arrayList();									
				sql.add("delete from spm_app_m where no_app='"+this.noVerLama+"' and kode_lokasi='"+this.app._lokasi+"'");
				sql.add("delete from spm_app_j where no_app='"+this.noVerLama+"' and kode_lokasi='"+this.app._lokasi+"'");
				
				if (this.e_modul.getText() == "IFAJU") sql.add("update spm_if_m set no_ver='-',progress='1' where no_if='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
				if (this.e_modul.getText() == "IFREIM") sql.add("update spm_ifreim_m set no_ver='-',progress='1' where no_reim='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
				if (this.e_modul.getText() == "PBBAU") sql.add("update yk_pb_m set no_ver='-',progress='2' where no_pb='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
				if (this.e_modul.getText() == "PBPR") sql.add("update yk_pb_m set no_ver='-',progress='2' where no_pb='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
				if (this.e_modul.getText() == "PBBDD") sql.add("update yk_pb_m set no_ver='-',progress='2' where no_pb='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
				if (this.e_modul.getText() == "PBLOG") sql.add("update yk_pb_m set no_ver='-',progress='2' where no_pb='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
				if (this.e_modul.getText() == "PJAJU") sql.add("update panjar2_m set no_ver='-',progress='2' where no_panjar='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
				if (this.e_modul.getText() == "PJPTG" || this.e_modul.getText() == "PRPTG") sql.add("update panjarptg2_m set no_ver='-',progress='2' where no_ptg='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");

				if (this.e_modul.getText() == "SPPD") sql.add("update pd_spj_m set no_ver='-',progress='9' where no_spj='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doChange:function(sender){				
		...
		if (sender == this.c_modul2) {
			if (this.c_modul2.getText() == "IFAJU") this.cb_nb.setSQL("select no_if, keterangan from spm_if_m where periode<='"+this.e_periode.getText()+"' and progress in ('3','V') and kode_lokasi='"+this.app._lokasi+"'",["no_if","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);		
			if (this.c_modul2.getText() == "IFREIM") this.cb_nb.setSQL("select no_reim, keterangan from spm_ifreim_m where periode<='"+this.e_periode.getText()+"' and progress in ('3','V') and kode_lokasi='"+this.app._lokasi+"'",["no_reim","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);					
			if (this.c_modul2.getText() == "PBBAU") this.cb_nb.setSQL("select no_pb, keterangan from yk_pb_m where periode<='"+this.e_periode.getText()+"' and progress in ('3','V') and kode_lokasi='"+this.app._lokasi+"'",["no_pb","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);					
			if (this.c_modul2.getText() == "PBPR") this.cb_nb.setSQL("select no_pb, keterangan from yk_pb_m where periode<='"+this.e_periode.getText()+"' and progress in ('3','V') and kode_lokasi='"+this.app._lokasi+"'",["no_pb","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);					
			if (this.c_modul2.getText() == "PBBDD") this.cb_nb.setSQL("select no_pb, keterangan from yk_pb_m where periode<='"+this.e_periode.getText()+"' and progress in ('3','V') and kode_lokasi='"+this.app._lokasi+"'",["no_pb","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);					
			if (this.c_modul2.getText() == "PBLOG") this.cb_nb.setSQL("select no_pb, keterangan from yk_pb_m where periode<='"+this.e_periode.getText()+"' and progress in ('3','V') and kode_lokasi='"+this.app._lokasi+"'",["no_pb","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);								
			if (this.c_modul2.getText() == "PJAJU") this.cb_nb.setSQL("select no_panjar, keterangan from panjar2_m where periode<='"+this.e_periode.getText()+"' and progress in ('3','V') and kode_lokasi='"+this.app._lokasi+"'",["no_panjar","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);					
			if (this.c_modul2.getText() == "PJPTG" || this.c_modul2.getText() == "PRPTG") this.cb_nb.setSQL("select no_ptg, keterangan from panjarptg2_m where periode<='"+this.e_periode.getText()+"' and progress in ('3','V') and kode_lokasi='"+this.app._lokasi+"'",["no_ptg","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);					
			if (this.c_modul2.getText() == "SPPD") {
				this.cb_nb.setSQL("select b.no_spj, b.no_dokumen from pd_spj_m b inner join pd_aju_m a on a.no_aju=b.no_aju and a.kode_lokasi=b.kode_lokasi "+
								  "where b.periode<='"+this.e_periode.getText()+"' and b.progress in ('3','V') and a.kode_lokasi='"+this.app._lokasi+"'",["no_spj","no_dokumen"],false,["No Bukti","No Dokumen"],"and","Daftar Bukti",true);					
			}
		}
		if (sender == this.cb_nb && this.cb_nb.getText() != "") {
			if (this.c_modul2.getText() == "IFAJU") {
				var strSQL = "select a.tanggal as due_date,a.no_if as no_bukti,case a.progress when '3' then 'APPROVE' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.tanggal,103) as tgl2,'IFAJU' as modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
			...
			if (this.c_modul2.getText() == "PJAJU") {					
				var strSQL = "select a.due_date,a.no_panjar as no_bukti,case a.progress when '3' then 'APPROVE' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
			...
			if (this.c_modul2.getText() == "PJPTG" || this.c_modul2.getText() == "PRPTG") {					
				var strSQL = "select a.tanggal as due_date,a.no_panjar as no_bukti,case a.progress when '3' then 'APPROVE' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.tanggal,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
	...
	doClick:function(sender){		
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"spm_app_m","no_app",this.app._lokasi+"-SPB"+this.e_periode.getText().substr(2,4)+".","0000"));												
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg.cells(0,row) != "") {			
				...
				this.noVerLama = this.sg.cells(10,row);						
	...
	doLoadJurnal:function(){
		...
		var strSQL3 = "select b.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp "+
					  ...
					  "                 inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+																					  
					  "where a.jenis='VER' and a.no_app = '"+this.noVerLama+"' and a.kode_lokasi='"+this.app._lokasi+"' ";
		var data = this.dbLib.getDataProvider(strSQL3,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			var line;
			this.sgv.clear();
	...
	doLoad:function(sender){	
		try {					
			...
			var strSQL = "select a.tanggal as due_date,a.no_if as no_bukti,case a.progress when '1' then 'INPROG' when '9' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.tanggal,103) as tgl2,'IFAJU' as modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
					"from spm_if_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
					"                inner join karyawan c on a.nik=c.nik and a.kode_lokasi=c.kode_lokasi "+
					"where a.progress in ('9','1','K','F') and a.kode_lokasi='"+this.app._lokasi+"'  "+					 
					"union all "+
				
					"select a.tanggal as due_date,a.no_reim as no_bukti,case a.progress when '1' then 'INPROG' when '9' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.tanggal,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
					"from spm_ifreim_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
					"                    inner join karyawan c on a.nik=c.nik and a.kode_lokasi=c.kode_lokasi "+
					"where a.progress in ('9','1','K','F') and a.kode_lokasi='"+this.app._lokasi+"'  "+					 
					...
					"select a.due_date,a.no_pb as no_bukti,case a.progress when '2' then 'INPROG' when '9' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
					"from yk_pb_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
					"               inner join karyawan c on a.nik_user=c.nik and a.kode_lokasi=c.kode_lokasi "+
					"where a.progress in ('9','2','K','F') and a.kode_lokasi='"+this.app._lokasi+"'  "+					 
				
					"union all "+
				
					"select a.due_date,a.no_panjar as no_bukti,case a.progress when '2' then 'INPROG' when '9' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
					"from panjar2_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
					"                inner join karyawan c on a.nik_buat=c.nik and a.kode_lokasi=c.kode_lokasi "+
					"where a.progress in ('9','2','K','F') and a.kode_lokasi='"+this.app._lokasi+"' "+					 
					...
					"select a.tanggal as due_date,a.no_ptg as no_bukti,case a.progress when '2' then 'INPROG' when '9' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.tanggal,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
					"from panjarptg2_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
					"                inner join karyawan c on a.nik_buat=c.nik and a.kode_lokasi=c.kode_lokasi "+
					"where a.progress in ('9','2','K','F') and a.kode_lokasi='"+this.app._lokasi+"' "+	
					...
					"select aa.tanggal as due_date,a.no_spj as no_bukti,case a.progress when '9' then 'INPROG' else 'RETURN' end as status,convert(varchar,aa.tanggal,103) as tgl,convert(varchar,aa.tanggal,103) as tgl2,'SPPD' as modul,aa.kode_pp+' - '+b.nama as pp,a.no_dokumen,aa.tujuan as keterangan,dd.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,aa.kode_pp "+
					"from pd_spj_m a inner join pd_aju_m aa on a.no_aju=aa.no_aju and a.kode_lokasi=aa.kode_lokasi "+
					"				  inner join pp b on aa.kode_pp=b.kode_pp and aa.kode_lokasi=b.kode_lokasi "+
					"				  inner join ( "+
					"						select no_spj,sum(total) as nilai "+
					...
					" 				 ) dd on a.no_spj=dd.no_spj "+
					"                inner join karyawan c on aa.nik_buat=c.nik and aa.kode_lokasi=c.kode_lokasi "+
					"where a.progress in ('9','K','F') and a.kode_lokasi='"+this.app._lokasi+"' "+
				
					"order by no_bukti ";	
			 
			if (this.cb_pp.getText() != "") {
				if (this.c_modul3.getText() == "IFAJU")	{
					strSQL = "select a.tanggal as due_date,a.no_if as no_bukti,case a.progress when '1' then 'INPROG' when '9' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.tanggal,103) as tgl2,'IFAJU' as modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
							"from spm_if_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
							"                inner join karyawan c on a.nik=c.nik and a.kode_lokasi=c.kode_lokasi and a.kode_pp='"+this.cb_pp.getText()+"' "+
							"where a.progress in ('9','1','K','F') and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_if";
				}	
				if (this.c_modul3.getText() == "IFREIM")	{
					strSQL =  "select a.tanggal as due_date,a.no_reim as no_bukti,case a.progress when '1' then 'INPROG' when '9' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.tanggal,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
							"from spm_ifreim_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
							"                    inner join karyawan c on a.nik=c.nik and a.kode_lokasi=c.kode_lokasi and a.kode_pp='"+this.cb_pp.getText()+"' "+
							"where a.progress in ('9','1','K','F') and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_reim";
				}	 
				if (this.c_modul3.getText() == "PBBAU")	{
					strSQL =  "select a.due_date,a.no_pb as no_bukti,case a.progress when '2' then 'INPROG' when '9' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
							"from yk_pb_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
							"               inner join karyawan c on a.nik_user=c.nik and a.kode_lokasi=c.kode_lokasi "+
							"where a.progress in ('9','2','K','F') and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='PBBAU' and a.kode_pp='"+this.cb_pp.getText()+"' order by a.no_pb";
				}	
				if (this.c_modul3.getText() == "PBPR")	{
					strSQL =  "select a.due_date,a.no_pb as no_bukti,case a.progress when '2' then 'INPROG' when '9' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
							"from yk_pb_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
							"               inner join karyawan c on a.nik_user=c.nik and a.kode_lokasi=c.kode_lokasi "+
							"where a.progress in ('9','2','K','F') and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='PBPR' and a.kode_pp='"+this.cb_pp.getText()+"' order by a.no_pb";
				} 
				if (this.c_modul3.getText() == "PBBDD")	{
					strSQL =  "select a.due_date,a.no_pb as no_bukti,case a.progress when '2' then 'INPROG' when '9' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
							"from yk_pb_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
							"               inner join karyawan c on a.nik_user=c.nik and a.kode_lokasi=c.kode_lokasi "+
							"where a.progress in ('9','2','K','F') and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='PBBDD' and a.kode_pp='"+this.cb_pp.getText()+"' order by a.no_pb";
				}
				if (this.c_modul3.getText() == "PBLOG")	{
					strSQL =  "select a.due_date,a.no_pb as no_bukti,case a.progress when '2' then 'INPROG' when '9' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
							"from yk_pb_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
							"               inner join karyawan c on a.nik_user=c.nik and a.kode_lokasi=c.kode_lokasi "+
							"where a.progress in ('9','2','K','F') and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='PBLOG' and a.kode_pp='"+this.cb_pp.getText()+"' order by a.no_pb";
				} 
				if (this.c_modul3.getText() == "PJAJU")	{
					strSQL =  "select a.due_date,a.no_panjar as no_bukti,case a.progress when '2' then 'INPROG' when '9' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
							"from panjar2_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
							"                inner join karyawan c on a.nik_buat=c.nik and a.kode_lokasi=c.kode_lokasi "+
							"where a.progress in ('9','2','K','F') and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_pp='"+this.cb_pp.getText()+"' order by a.no_panjar";
				} 
				if (this.c_modul3.getText() == "PJPTG")	{
					strSQL =  "select a.tanggal as due_date,a.no_ptg as no_bukti,case a.progress when '2' then 'INPROG' when '9' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.tanggal,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
							"from panjarptg2_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
							"                inner join karyawan c on a.nik_buat=c.nik and a.kode_lokasi=c.kode_lokasi "+
							"where a.progress in ('9','2','K','F') and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_pp='"+this.cb_pp.getText()+"' order by a.no_ptg";
				} 
				if (this.c_modul3.getText() == "SPPD") {					
					var strSQL = "select aa.tanggal as due_date,a.no_spj as no_bukti,case a.progress when '9' then 'INPROG' else 'RETURN' end as status,convert(varchar,aa.tanggal,103) as tgl,convert(varchar,aa.tanggal,103) as tgl2,'SPPD' as modul,aa.kode_pp+' - '+b.nama as pp,a.no_dokumen,aa.tujuan as keterangan,dd.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,aa.kode_pp "+
									"from pd_spj_m a inner join pd_aju_m aa on a.no_aju=aa.no_aju and a.kode_lokasi=aa.kode_lokasi "+
									"				  inner join pp b on aa.kode_pp=b.kode_pp and aa.kode_lokasi=b.kode_lokasi "+
									"				  inner join ( "+
									"						select no_spj,sum(total) as nilai "+
									...
									" 				 ) dd on a.no_spj=dd.no_spj "+
									"                inner join karyawan c on aa.nik_buat=c.nik and aa.kode_lokasi=c.kode_lokasi "+
									"where a.progress in ('9','K','F') and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_spj";					
				}
			}	
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData([line.modul.toUpperCase(),line.no_bukti,line.status.toUpperCase(),line.tgl,line.tgl2,line.pp,line.no_dokumen,line.keterangan,floatToNilai(line.nilai),line.pembuat,line.no_ver,line.tglinput,line.kode_pp]); 
		}
