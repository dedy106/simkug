window.app_saku3_transaksi_spm_fPbProyek2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_fPbProyek2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fPbProyek2";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,338], childPage:["Data Proyek","Atensi Pembayaran","Detail Transaksi","Jurnal Tambahan/Pajak","Catatan","File Dok"]});
		...
		this.sg1 = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:2,tag:0,
		...
		this.sgUpld = new saiGrid(this.pc1.childPage[5],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:5,tag:1,
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag,value1 from spro where kode_spro in ('GARFREE','DOKFREE','PRBATAS') and kode_lokasi = '"+this.app._lokasi+"'",true);			
			...
			sql.add("select kode_vendor,nama from vendor where kode_vendor not in ('-','00.NURUL') and kode_lokasi = '"+this.app._lokasi+"' and flag_aktif='1' ");						
			sql.add("select a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '053' "+					
...
window.app_saku3_transaksi_spm_fPbProyek2.extend(window.childForm);
window.app_saku3_transaksi_spm_fPbProyek2.implement({	
				...
				try{														
						...
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){								
								sql.add("insert into spm_proyek_bdd(no_bukti,modul,kode_proyek,nu,keterangan,dc,nilai,kode_lokasi,periode) values "+
										"('"+this.e_nb.getText()+"','PBPR','"+this.cb_proyek.getText()+"',"+i+",'"+this.sg1.cells(0,i)+"','D',"+nilaiToFloat(this.sg1.cells(1,i))+",'"+this.app._lokasi+"','"+this.e_periode.getText()+"')");										
								
								sql.add("insert into yk_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.akunBDD+"','"+this.sg1.cells(0,i)+"','D',"+nilaiToFloat(this.sg1.cells(1,i))+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBPR','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");										
							}
						}
					}														
					...
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i) && this.sgUpld.cells(2,i)!="-"){
							ix++;
							sql.add("insert into yk_pb_dok(no_pb,no_gambar,nu,kode_jenis,kode_lokasi,no_ref)values('"+this.e_nb.getText()+"','"+this.sgUpld.cells(3,i).tmpfile+"','"+ix+"','"+this.sgUpld.cells(0,i)+"','"+this.app._lokasi+"','"+this.e_nb.getText()+"')");								
						}	
					}														
					setTipeButton(tbAllFalse);					
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					this.sg.clear(1); this.sg1.clear(1); this.sg3.clear(1); this.sgUpld.clear(1);
			...
			case "ubah" :															
				...
				if ((nilaiToFloat(this.e_pakai.getText()) + nilaiToFloat(this.e_totalRek.getText()) - this.totTambah ) > nilaiToFloat(this.e_max.getText())  ) {
	...
	doClick:function(sender){
		try {
			if (this.e_periode.getText()!= "") {
				...
				this.noVerLama = "-";
				this.stsSimpan = 1;
				this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"yk_pb_m","no_pb",this.app._lokasi+"-PBR"+this.e_periode.getText().substr(2,4)+".","0000"));						
				this.e_dok.setFocus();
	...
	doChangeCell1: function(sender, col, row){
		if (col == 1 && sender.cells(1,row) != "") this.sg.validasi();			
	},
			...
			if (sender == this.sg) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Mitra",sender,undefined, 
						    "select kode_vendor,nama   from vendor where kode_vendor not in ('-','00.NURUL') and flag_aktif='1'  and kode_lokasi = '"+this.app._lokasi+"'",
							"select count(kode_vendor) from vendor where kode_vendor not in ('-','00.NURUL') and flag_aktif='1'  and kode_lokasi = '"+this.app._lokasi+"'",
			...
			if (sender == this.sg2) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Akun",sender,undefined, 
						    "select a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '053' "+							
							"where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'",
							"select count(a.kode_akun) from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '053' where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'",
	...
	doLoad3:function(sender){																				
		...
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.modul,a.no_dokumen,a.keterangan,a.progress,a.nilai, "+
					 ...
					 "where a.posted='F' and a.kode_pp = '"+this.app._kodePP+"' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'PBPR' and a.progress in ('0','C','F','S','V','X') ";
