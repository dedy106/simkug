window.app_saku3_transaksi_spm_fJuBayarBill = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_fJuBayarBill.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fJuBayarBill";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penyelesaian Piutang Proyek NonCash", 0);	
		...
		this.c_jenis = new saiCB(this.pc2.childPage[0],{bound:[20,12,200,20],caption:"Jenis",items:["NKP"], readOnly:true,tag:2, visible:false});
...
window.app_saku3_transaksi_spm_fJuBayarBill.extend(window.childForm);
window.app_saku3_transaksi_spm_fJuBayarBill.implement({
	doLoadRev: function() {
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','JUREVBIL','"+this.c_jenis.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.cb_app.getText()+"','F','-','-','"+this.cb_piutang.getText()+"',getdate(),'"+this.app._userLog+"')");
					...
					sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.akunPiutang+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_nilai.getText())+",'"+this.kodePPInvoice+"','-','-','-','-','-','-','-','"+this.app._lokasi+"','JUREVBIL','PIU','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','-')");

					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){																															
								sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','-','-','-','-','-','-','"+this.app._lokasi+"','JUREVBIL','REV','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','-')");
							}
						}
					}
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_ju,convert(varchar,a.tanggal,103) as tgl,a.jenis,a.no_dokumen,a.keterangan,a.nilai "+
		             "from ju_m a "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'JUREVBIL' and a.posted ='F'";						
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select (a.nilai+a.nilai_ppn-a.nilai_pph) -  isnull(b.bayar,0) as saldo, a.kode_pp, a.akun_piutang,a.kode_cust,"+							 
							 ...
							 "from spm_piutang_m a inner join ju_m e on e.ref1 = a.no_piutang and a.kode_lokasi=e.kode_lokasi "+
							 "                 left join ("+
							 "                   select no_piutang,kode_lokasi,sum(case dc when 'D' then nilai_kas else -nilai_kas end) as bayar "+
							 "                   from spm_billbayar_d where no_bukti <> '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' group by no_piutang,kode_lokasi) b on a.no_piutang=b.no_piutang and a.kode_lokasi=b.kode_lokasi "+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){		
						...
						this.cb_piutang.setSQL("select no_piutang,keterangan from spm_piutang_m where kode_lokasi='"+this.app._lokasi+"' and no_piutang='"+line.ref1+"'",["no_piutang","keterangan"],false,["No Bukti","Keterangan"],"and","Data Piutang",true);											
