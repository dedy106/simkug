window.app_saku3_transaksi_spm_fPP = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_fPP.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fPP";
		...
		this.e_kota = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,13,400,20],caption:"Kota", maxLength:50, tag:1});	
...
window.app_saku3_transaksi_spm_fPP.extend(window.childForm);
window.app_saku3_transaksi_spm_fPP.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into pp(kode_pp, kode_lokasi, nama, initial, level_spasi, tipe, sum_header, kode_induk, rowindex, kode_bidang, flag_aktif, kota) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.cb_lokasi.getText()+"','"+this.e_nama.getText()+"',  '-', '0', 'Posting', '00', '00', 1, '"+this.cb_bidang.getText()+"', '"+this.c_status.getText().substr(0,1)+"', '"+this.e_kota.getText()+"')");
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update pp set kota='"+this.e_kota.getText()+"',nama='"+this.e_nama.getText()+"', flag_aktif ='"+this.c_status.getText().substr(0,1)+"', kode_bidang='"+this.cb_bidang.getText()+"' "+
	...
	doChange: function(sender){
		try{			
			...
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select kode_pp,nama,flag_aktif,kode_bidang,kota from pp "+
						     "where kode_pp ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.cb_lokasi.getText()+"'";						   
	...
	doLoad:function(sender){						
		var strSQL = "select a.kode_pp,a.nama,a.kode_bidang+' - '+b.nama as bidang "+
		             "from pp a left join bidang b on a.kode_bidang=b.kode_bidang  "+
