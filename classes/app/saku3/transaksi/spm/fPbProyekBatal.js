window.app_saku3_transaksi_spm_fPbProyekBatal = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_fPbProyekBatal.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fPbProyekBatal";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pembatalan Permintaan Bayar Proyek", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,310], childPage:["Data Proyek","Atensi Pembayaran","Detail Transaksi","Jurnal Tambahan","Catatan"]});
		...
		try {
			...
			//falg aktif di spm_proyek dibuka,request elis digorup spm HO 08/08/2020
			this.cekRek = true;						
...
window.app_saku3_transaksi_spm_fPbProyekBatal.extend(window.childForm);
window.app_saku3_transaksi_spm_fPbProyekBatal.implement({	
	isiNoPB: function() {
		try{
			var strSQL = "select a.no_pb,a.keterangan "+
						"from yk_pb_m a "+
						"where a.kode_pp = '"+this.app._kodePP+"' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'PBPR' and a.progress not in ('0','C','S','V','6') ";							
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into yk_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','"+this.e_nbpb.getText()+"','"+this.dp_d1.getDateString()+"',99,'"+this.akunBMHD+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_totalDC.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','REVPB','BMHD','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");
					...
					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){								
								...
								sql.add("insert into yk_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','"+this.e_nbpb.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.akunBDD+"','"+this.sg1.cells(0,i)+"','C',"+nilaiToFloat(this.sg1.cells(1,i))+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','REVPB','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");										
							}
						}
					}																			
	...
	doLoad3:function(sender){																				
		...
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.modul,a.no_dokumen,a.keterangan,a.progress,a.nilai, "+
					 ...
					 "where a.posted='F' and a.kode_pp = '"+this.app._kodePP+"' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'REVPB' ";
