window.app_saku3_transaksi_spm_fHutangRevBDD = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_fHutangRevBDD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fHutangRevBDD";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,12,990,338], childPage:["Data Akru","Data Jurnal","Referensi Akru"]});				
		...
		this.sg5 = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-40],colCount:7,tag:0,
...
window.app_saku3_transaksi_spm_fHutangRevBDD.extend(window.childForm);
window.app_saku3_transaksi_spm_fHutangRevBDD.implement({		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,4])){
				try{														
					...
					sql.add("insert into hutang_m(no_hutang,kode_lokasi,no_dokumen,tanggal,keterangan,kode_project,kode_vendor,kode_curr,kurs,nik_app,kode_pp,nilai,periode,nik_user,tgl_input,akun_hutang,posted,nilai_ppn,modul,no_ref) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','-','"+this.e_vendor.getText()+"','IDR',1,'"+this.app._userLog+"','"+this.kodePP+"',"+parseNilai(this.e_nilai.getText())+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.e_akun.getText()+"','F',0,'REV','"+this.e_nohutang.getText()+"')");	
