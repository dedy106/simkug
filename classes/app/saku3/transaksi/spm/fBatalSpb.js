window.app_saku3_transaksi_spm_fBatalSpb = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_fBatalSpb.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fBatalSpb";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pembatalan PB : Input/Edit", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,335], childPage:["Daftar PB"]});		
		...
		this.e_nilai2 = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,17,200,20],caption:"Nilai", readOnly:true});				
...
window.app_saku3_transaksi_spm_fBatalSpb.extend(window.childForm);
window.app_saku3_transaksi_spm_fBatalSpb.implement({	
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("update yk_pb_m set prog_seb=progress,progress='6', no_kas='"+this.e_nb.getText()+"' where no_pb = '"+this.cb_pb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
					...
					sql.add("insert into ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','JUSPB','UMUM','IDR',1,0,'"+this.app._userLog+"','"+this.app._userLog+"','F','-','-','"+this.cb_pb.getText()+"',getdate(),'"+this.app._userLog+"')");
					...
					sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref)  "+
							"select '"+this.e_nb.getText()+"',b.no_pb,'"+this.dp_d1.getDateString()+"',a.no_urut,a.kode_akun,'"+this.e_ket.getText()+"',case a.dc when 'D' then 'C' else 'D' end,a.nilai,a.kode_pp,a.kode_drk,'-','-','-','-','-','-',a.kode_lokasi,'JUSPB',jenis,'"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','-' "+
					...
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) "+
							"select '"+this.e_nb.getText()+"','JUSPB',a.kode_lokasi,a.kode_akun,a.kode_pp,a.kode_drk,'"+this.e_periode.getText()+"','"+this.e_periode.getText()+"',case a.dc when 'D' then 'C' else 'D' end,0,a.nilai "+
							"from yk_pb_j a inner join yk_pb_m b on a.no_pb=b.no_pb and a.kode_lokasi=b.kode_lokasi "+
							"where b.no_pb='"+this.cb_pb.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"'");		
	...
	doSelectDate: function(sender, y,m,d){
		...
		if (this.stsSimpan == 1) {
			...
			var strSQL = "select no_pb, keterangan from yk_pb_m  "+
						 "where no_kas='-' and modul in ('PBBDD','PBBAU') and kode_lokasi='"+this.app._lokasi+"' ";	
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_ju,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan "+
		             "from ju_m a "+					 					 
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'JUSPB' and a.posted ='F'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
