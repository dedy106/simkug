window.app_saku3_transaksi_spm_fKbPiu = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_fKbPiu.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fKbPiu";
...
window.app_saku3_transaksi_spm_fKbPiu.extend(window.childForm);
window.app_saku3_transaksi_spm_fKbPiu.implement({
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_piutang && this.cb_piutang.getText()!= "") {
			var strSQL = "select (a.nilai+a.nilai_ppn-a.nilai_pph) -  isnull(b.bayar,0) as saldo, a.akun_piutang "+			             
						 "from piutang_m a "+						 
						 "     left join ("+
						 "              select no_piutang,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar "+
						 "              from piubayar_d group by no_piutang,kode_lokasi) b on a.no_piutang=b.no_piutang and a.kode_lokasi=b.kode_lokasi "+
