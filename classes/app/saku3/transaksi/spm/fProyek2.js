window.app_saku3_transaksi_spm_fProyek2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_fProyek2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fProyek2";
		...
		this.e_nama = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,11,430,20],caption:"Deskripsi", maxLength:50, tag:1});	
		...
		this.e_nilai = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,14,200,20],caption:"Nilai Kontrak", tag:1, tipeText:ttNilai, text:"0"});				
		this.e_nilaior = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,16,200,20],caption:"Nilai RAB", tag:1, tipeText:ttNilai, text:"0"});						
...
window.app_saku3_transaksi_spm_fProyek2.extend(window.childForm);
window.app_saku3_transaksi_spm_fProyek2.implement({	
				...
				try{									
					...
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i) && this.sgUpld.cells(2,i)!="-"){
							ix++;
							sql.add("insert into spm_proyek_dok(no_bukti,no_gambar,nu,kode_jenis,kode_lokasi,no_ref) values ('"+this.cb_kode.getText()+"','"+this.sgUpld.cells(3,i).tmpfile+"','"+ix+"','"+this.sgUpld.cells(0,i)+"','"+this.app._lokasi+"','"+this.cb_kode.getText()+"')");								
						}	
					}			
					setTipeButton(tbAllFalse);
			...
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				...
				if (typeof data == "object"){
					...
					if (line != undefined){			
						...
						this.c_status.addItem(0,"1. OGP");
						this.c_status.addItem(1,"0. CLOSE");
						this.c_status.addItem(2,"2. BATAL");
						this.c_status.addItem(3,"3. MASALALU");
						...
						if (line.flag_aktif == "0") this.c_status.setText("0. CLOSE");
						if (line.flag_aktif == "1") this.c_status.setText("1. OGP");
						if (line.flag_aktif == "2") this.c_status.setText("2. BATAL");
						if (line.flag_aktif == "3") this.c_status.setText("3. MASALALU");
	...
	doBtnClick: function(sender, event){
		try{
			if (sender == this.cb_kode && this.cb_pp.getText()!="") {   
			    this.standarLib.showListData(this, "Daftar Proyek",sender,undefined, 
											  "select kode_proyek, nama  from spm_proyek where kode_pp='"+this.cb_pp.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",
											  "select count(kode_proyek) from spm_proyek where kode_pp='"+this.cb_pp.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",
