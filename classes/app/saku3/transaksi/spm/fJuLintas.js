window.app_saku3_transaksi_spm_fJuLintas = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_fJuLintas.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fJuLintas";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Jurnal Lintas Periode (SPM): Input", 0);	
...
window.app_saku3_transaksi_spm_fJuLintas.extend(window.childForm);
window.app_saku3_transaksi_spm_fJuLintas.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					var j = nilaiToFloat(this.e_periode.getText().substr(4,2)); 
					var k = nilaiToFloat(this.app._periode.substr(4,2)); 
					
					for (var i=j;i <= k;i++){	
						var periodeIn = this.e_periode.getText().substr(0,4) + (i<10?"0":"")+i;
						sql.add("call sp_exs_proses '"+this.app._lokasi+"','"+periodeIn+"','FS1' ");
					}	
