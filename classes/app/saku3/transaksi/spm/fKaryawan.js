window.app_saku3_transaksi_spm_fKaryawan = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spm_fKaryawan.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spm_fKaryawan";
		...
		this.e_foto = new saiLabelEdit(this.pc1.childPage[1], {bound:[520,11,210,20], caption:"Foto"});
		this.u_foto = new uploader(this.pc1.childPage[1], {bound:[735,11,80,20],param3: "object", param4 :"server/media/",param2 :"server/tmp/", param1 : "uploadTo",
				autoSubmit:true, afterUpload: [this, "doUploadFinish"], caption:"Browse"} );
		this.c_band = new saiCB(this.pc1.childPage[1],{bound:[20,12,200,20],caption:"Band",tag:2, mustCheck:true});		
		this.e_jabatan = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,13,400,20],caption:"Jabatan", maxLength:50, tag:1});			
		this.c_jenis = new saiCB(this.pc1.childPage[1],{bound:[20,24,200,20],caption:"Status Pegawai",items:["ORGANIK","NON","PENSIUN"], readOnly:true,tag:2,change:[this,"doChange"]});
		...
		this.cb_pp = new saiCBBL(this.pc1.childPage[1],{bound:[20,15,220,20],caption:"PP Eksisting", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});
		...
		this.c_status = new saiCB(this.pc1.childPage[1],{bound:[20,26,200,20],caption:"Status Aktif",items:["1. AKTIF","0. NONAKTIF"], readOnly:true,tag:2});
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_band from band order by kode_band",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.c_band.addItem(i,line.kode_band);
				}
			}
...
window.app_saku3_transaksi_spm_fKaryawan.extend(window.childForm);
window.app_saku3_transaksi_spm_fKaryawan.implement({
	...
	doChange: function(sender){
		try{
			if (sender == this.c_jenis && this.c_jenis.getText() != "") {	
				if (this.c_jenis.getText()=="PENSIUN") this.c_status.setText("0. NONAKTIF");
