window.app_saku3_transaksi_kredit_fBeli = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_kredit_fBeli.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_kredit_fBeli";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pembelian", 0);	
		...
		this.cb_pp = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,15,220,20],caption:"Area Bisnis",multiSelection:false,tag:1});
		this.e_diskon = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,15,200,20],caption:"Diskon", tag:1, tipeText:ttNilai, text:"0",change:[this,"doChange"]});		
		this.cb_vendor = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,16,220,20],caption:"Vendor",multiSelection:false,tag:1});
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:0,
					...
					colWidth:[[8,7,6,5,4,3,2,1,0],[80,60,60,60,60,50,250,250,80]],					
		...
		try{			
			...
			this.cb_pp.setSQL("select kode_pp, nama from pp where flag_aktif='1' and kode_lokasi = '"+this.app._lokasi+"'",["kode_pp","nama"],false,["Kode","Nama"],"and","Data Area",true);
			this.cb_vendor.setSQL("select kode_vendor, nama from kre_vendor where kode_lokasi = '"+this.app._lokasi+"'",["kode_vendor","nama"],false,["Kode","Nama"],"and","Data Vendor",true);
...
window.app_saku3_transaksi_kredit_fBeli.extend(window.portalui_childForm);
window.app_saku3_transaksi_kredit_fBeli.implement({	
		...
		try{								
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					if (this.stsSimpan == 0) {
						sql.add("delete from kre_beli_m where no_beli = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from kre_beli_j where no_beli = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from kre_beli_d where no_beli = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}
					sql.add("insert into kre_beli_m(no_beli,kode_lokasi,tanggal,no_dokumen,keterangan,kode_pp,periode,nik_user,tgl_input,nilai,nilai_ppn,nilai_diskon,nilai_mat,kode_vendor,no_kas,posted,modul,akun_hutang) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.cb_pp.getText()+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),"+nilaiToFloat(this.e_total.getText())+",0,"+nilaiToFloat(this.e_diskon.getText())+",0,'"+this.cb_vendor.getText()+"','-','F','NON','"+this.akunHutang+"')");
							
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){															
								var diskonBrg = nilaiToFloat(this.sg.cells(7,i)) / nilaiToFloat(this.sg.cells(5,i));
								sql.add("insert into kre_beli_d (no_beli,kode_lokasi,periode,nu,kode_brg,kode_pp,satuan,jumlah,bonus,harga,diskon,tot_diskon,total) values "+  
									   "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.cb_pp.getText()+"','"+this.sg.cells(3,i)+"',"+nilaiToFloat(this.sg.cells(5,i))+","+nilaiToFloat(this.sg.cells(6,i))+","+nilaiToFloat(this.sg.cells(4,i))+","+diskonBrg+","+nilaiToFloat(this.sg.cells(7,i))+","+nilaiToFloat(this.sg.cells(8,i))+")");
							}
						}						
					}
										
					sql.add("insert into kre_beli_j(no_beli,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
					...
					sql.add("insert into kre_beli_j(no_beli,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',1,'"+this.akunPPN+"','"+this.e_ket.getText()+"','D',0,'"+this.app._kodePP+"','-','"+this.app._lokasi+"','BRGBELI','PPN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");					
					sql.add("insert into kre_beli_j(no_beli,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',5,'"+this.akunMat+"','"+this.e_ket.getText()+"','D',0,'"+this.app._kodePP+"','-','"+this.app._lokasi+"','BRGBELI','BEAMAT','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");
					...
					sql.add("insert into kre_beli_j(no_beli,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) "+
							"select "+
							...
							"    from kre_beli_j where no_beli='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "") {
			...
			this.stsSimpan = 1;
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"kre_beli_m","no_beli",this.app._lokasi+"-PO"+this.e_periode.getText().substr(2,4)+".","0000"));
			this.e_dok.setFocus();
	...
	doChangeCell: function(sender, col, row){
		if (col == 0 && this.sg.cells(0,row)!="") {
			...
			var strSQL = "select merk+' - '+tipe as pabrik,satuan from kre_brg where kode_brg='"+this.sg.cells(0,row)+"' and kode_lokasi='"+this.app._lokasi+"'";
	...
	doNilaiChange: function(){
		try{
			...
			this.e_total.setText(floatToNilai(tot-nilaiToFloat(this.e_diskon.getText())));	
		}catch(e)
		{
	...
	doChange:function(sender){
		if (sender == this.e_nilai || sender == this.e_diskon) {
			if (this.e_nilai.getText()!="" && this.e_diskon.getText()!="") {
				this.e_total.setText(floatToNilai(nilaiToFloat(this.e_nilai.getText())-nilaiToFloat(this.e_diskon.getText())));
			}
		}
	},
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						this.cb_pp.setSQL("select kode_pp, nama from pp where kode_pp='"+line.kode_pp+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_pp","nama"],false,["Kode","Nama"],"and","Data Area",true);																				
						this.cb_vendor.setSQL("select kode_vendor, nama from kre_vendor where kode_vendor='"+line.kode_vendor+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_vendor","nama"],false,["Kode","Nama"],"and","Data Vendor",true);									
						this.cb_pp.setText(line.kode_pp);			
						this.cb_vendor.setText(line.kode_vendor);										
					}
				}												
				var data = this.dbLib.getDataProvider("select b.kode_brg,b.nama,b.merk+' - '+tipe as pabrik,a.satuan,a.jumlah,a.bonus,a.harga,a.tot_diskon,a.total "+
							"from kre_beli_d a inner join kre_brg b on a.kode_brg=b.kode_brg and a.kode_lokasi=b.kode_lokasi "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];												
						this.sg.appendData([line.kode_brg,line.nama,line.pabrik,line.satuan,floatToNilai(line.harga),floatToNilai(line.jumlah),floatToNilai(line.bonus),floatToNilai(line.tot_diskon),floatToNilai(line.total)]);
					}
