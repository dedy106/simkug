window.app_saku3_transaksi_kredit_fVendor = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_kredit_fVendor.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_kredit_fVendor";
...
window.app_saku3_transaksi_kredit_fVendor.extend(window.childForm);
window.app_saku3_transaksi_kredit_fVendor.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into kre_vendor(kode_vendor,kode_lokasi,nama,flag_aktif,alamat,alamat2,no_tel,no_tel2,no_fax,email,npwp,pic) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','1','"+this.e_alamat.getText()+"','-','"+this.e_tel.getText()+"','"+this.e_hp.getText()+"','"+this.e_fax.getText()+"','-','-','"+this.e_pic.getText()+"')");
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();					
					sql.add("delete from kre_vendor where kode_vendor = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");			
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select nama from kre_vendor where kode_vendor ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {			
				...
				var strSQL = "select nama,flag_aktif,alamat,alamat2,no_tel,no_tel2,no_fax,email,npwp,pic "+
							 "from kre_vendor where kode_vendor = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){													
