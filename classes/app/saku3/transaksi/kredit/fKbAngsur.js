window.app_saku3_transaksi_kredit_fKbAngsur = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_kredit_fKbAngsur.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_kredit_fKbAngsur";
		...
		this.e_nilai = new saiLabelEdit(this.pc2.childPage[0],{bound:[780,14,200,20],caption:"Tot Angsuran", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,347], childPage:["Data TTB","Daftar Rincian"]});		
		...
		this.e_angsur = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,15,200,20],caption:"Nilai Angsuran", tag:9, tipeText:ttNilai, text:"0",change:[this,"doChange"]});				
		this.e_sisa = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,16,200,20],caption:"Sisa Tagihan", tag:9, readOnly:true, tipeText:ttNilai, text:"0",change:[this,"doChange"]});				
		this.l_tgl2 = new portalui_label(this.pc1.childPage[0],{bound:[20,11,100,18],caption:"Jadwal Sisa", underline:true,visible:false});
		this.dp_d2 = new portalui_datePicker(this.pc1.childPage[0],{bound:[120,11,100,18],visible:false}); 
		this.bOk = new button(this.pc1.childPage[0],{bound:[120,17,100,18],caption:"Angsur",click:[this,"doBayar"]});			
		
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-45],colCount:6,tag:0,
		            colTitle:["No TTB","Koordinator","Angs-Ke","Nilai","Saldo","Jadwal Sisa"],
...
window.app_saku3_transaksi_kredit_fKbAngsur.extend(window.childForm);
window.app_saku3_transaksi_kredit_fKbAngsur.implement({
	doBayar: function(sender) {
		try {
			if (nilaiToFloat(this.e_angsur.getText()) > nilaiToFloat(this.e_tagih.getText()) || nilaiToFloat(this.e_angsur.getText()) < 0) {
				system.alert(this,"Nilai Angsuran tidak valid.","Nilai tidak boleh melebihi tagihan dan atau kurang dari nol.");
				return false;
			}
			else {
				var temu = false;
				...
				if (!temu) {
					this.sg.appendData([this.cb_ttb.getText(),this.cb_agg.rightLabelCaption,this.cb_ttb.rightLabelCaption,this.e_angsur.getText(),this.e_sisa.getText(),this.dp_d2.getDateString()]);
					this.sg.validasi();
				}
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','-','-','-','"+this.dp_d1.getDateString()+"','Angsuran Collector a.n "+this.cb_kolek.rightLabelCaption+"','"+this.cb_pp.getText()+"','ANGSUR','BM','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','"+this.cb_kolek.getText()+"','-','-')");
					
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){																							
								sql.add("insert into kre_angsur_d (no_bukti,periode,kode_lokasi,no_ttb,cicilan_ke,no_bill,nilai,sisa,dc,tgl_lunas) values "+
										"('"+this.e_nb.getText()+"','"+this.e_periode.getText()+"','"+this.app._lokasi+"','"+this.sg.cells(0,i)+"',"+this.sg.cells(2,i)+",'-',"+nilaiToFloat(this.sg.cells(3,i))+","+nilaiToFloat(this.sg.cells(4,i))+",'D','"+this.sg.cells(5,i)+"')"); 
							}
						}
					}
					setTipeButton(tbAllFalse);					
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_ttb && this.cb_ttb.getText() != "") {	
			...
			var strSQL = "select a.npokok-isnull(c.angsur,0) as saldo "+						 
						 "from kre_ttb2_sch a "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){		
					...
					this.e_angsur.setText(floatToNilai(line.saldo));						
				} 
			}
		}	
		
		if (sender == this.e_angsur && this.e_angsur.getText() != "") {
			var sisa = nilaiToFloat(this.e_tagih.getText()) - nilaiToFloat(this.e_angsur.getText()); 
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				strSQL = "select a.no_ttb,a.cicilan_ke,a.nilai,a.sisa,substring(convert(varchar,a.tgl_lunas,121),1,10) as tgl_lunas,c.nama "+
						 "from kre_angsur_d a "+
				         "inner join kre_ttb2_m b on a.no_ttb=b.no_ttb and a.kode_lokasi=b.kode_lokasi "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg.appendData([line.no_ttb,line.nama,floatToNilai(line.cicilan_ke),floatToNilai(line.nilai),floatToNilai(line.sisa),line.tgl_lunas]);
					}
