window.app_saku3_transaksi_bpr_fPjptgSimple = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bpr_fPjptgSimple.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bpr_fPjptgSimple";
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:10,tag:9,
		            colTitle:["ID Proyek","Deskripsi","Akun Beban","PP","Ket. Peruntukan","Nilai","PPN Masukan","Pot. PPh23","Pot. PPh21","Total"],					
...
window.app_saku3_transaksi_bpr_fPjptgSimple.extend(window.childForm);
window.app_saku3_transaksi_bpr_fPjptgSimple.implement({
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg.appendData([line.kode_proyek,line.nama,line.akun_beban,line.kode_pp,line.keterangan,floatToNilai(line.nilai),floatToNilai(line.ppnm),floatToNilai(line.pph21),floatToNilai(line.pph23),floatToNilai(line.total)]);
					}
