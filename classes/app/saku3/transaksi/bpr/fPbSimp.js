window.app_saku3_transaksi_bpr_fPbSimp = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bpr_fPbSimp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bpr_fPbSimp";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Permintaan Bayar Penarikan Simpanan", 0);	
		...
		this.cb_agg = new saiCBBL(this.pc1.childPage[1],{bound:[20,14,220,20],caption:"Anggota", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});								
		...
		this.e_saldo = new saiLabelEdit(this.pc1.childPage[1],{bound:[770,18,200,20],caption:"Saldo Simpanan", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});										
		this.e_namarek = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,19,550,20],caption:"Nama Rekening", maxLength:150});		
		this.e_nilai = new saiLabelEdit(this.pc1.childPage[1],{bound:[770,19,200,20],caption:"Total Penarikan", tag:1,  tipeText:ttNilai, text:"0",change:[this,"doChange"]});				

		this.sg5 = new saiGrid(this.pc1.childPage[1],{bound:[0,5,this.pc1.width-5,this.pc1.height-155],colCount:6,tag:0,
		...
		this.sgn5 = new portalui_sgNavigator(this.pc1.childPage[1],{bound:[0,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:3,grid:this.sg5});
...
window.app_saku3_transaksi_bpr_fPbSimp.extend(window.childForm);
window.app_saku3_transaksi_bpr_fPbSimp.implement({			
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,due_date,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','AP','PBSIM','F','0','0','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','"+this.dp_d2.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','IDR',1,"+nilaiToFloat(this.e_totalDC.getText())+","+nilaiToFloat(this.e_nilai.getText())+",0,'"+this.cb_app.getText()+"','"+this.cb_tahu.getText()+"','-','-','-','-','"+this.cb_agg.getText()+"','-','"+this.cb_akun.getText()+"')");					
					
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.cb_akun.getText()+"','C',"+nilaiToFloat(this.e_totalDC.getText())+","+nilaiToFloat(this.e_totalDC.getText())+",'"+this.e_ket.getText()+"','PBSIM','BMHD','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");																		
					
					for (var i=0;i < this.sg5.getRowCount();i++){
						if (this.sg5.rowValid(i) && this.sg5.cells(5,i) != "0"){
							var nilaiAmbil = nilaiToFloat(this.sg5.cells(5,i));							
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.sg5.cells(0,i)+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg5.cells(3,i)+"','D',"+nilaiAmbil+","+nilaiAmbil+",'Pengambilan Simpanan "+this.sg5.cells(0,i)+"','KBAMBIL','SIMP','IDR',1,'"+this.app._kodePP+"','-','-','"+this.cb_agg.getText()+"','-','-','-','-','-')");
					
							if (this.sg5.cells(1,i) != "BS") var jenis = "SIMP";
							...
							sql.add("insert into kop_simpangs_d (no_angs,no_simp,no_bill,akun_piutang,nilai,kode_lokasi,dc,periode,modul,no_agg,jenis) values "+
									"('"+this.e_nb.getText()+"','"+this.sg5.cells(0,i)+"','-','"+this.sg5.cells(3,i)+"',"+nilaiAmbil+",'"+this.app._lokasi+"','C','"+this.e_periode.getText()+"','SIMPAMBIL','"+this.cb_agg.getText()+"','"+jenis+"')");							
						}
					}

					sql.add("insert into pb_rek(no_bukti,kode_lokasi,modul,nama_rek,no_rek,bank,cabang,bruto,pajak,nilai, kode_vendor) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','PBSIM','"+this.e_namarek.getText()+"','"+this.e_norek.getText()+"','"+this.e_bank.getText()+"','"+this.e_cab.getText()+"',"+nilaiToFloat(this.e_totalDC.getText())+",0,"+nilaiToFloat(this.e_totalDC.getText())+", '"+this.cb_agg.getText()+"')");
					...
					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){		
								var k = i+1000;
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+k+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg1.cells(4,i))+","+nilaiToFloat(this.sg1.cells(4,i))+",'"+this.sg1.cells(3,i)+"','PBSIM','BEBAN','IDR',1,'"+this.sg1.cells(5,i)+"','-','-','-','-','-','-','-','-')");														
							}
						}
					}	
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_gar,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai,gar_thn,gar_tw,gar_bulan,no_ref,param1,param2) values "+
										"('"+this.e_nb.getText()+"','PBSIM','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(1,i)+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(2,i))+","+nilai+",0,0,0,'-','-','-')");
							}
						}
					}
					setTipeButton(tbAllFalse);					
		...
		switch (event){
			...
			case "ubah" :															
				...
				if (nilaiToFloat(this.e_nilai.getText()) > nilaiToFloat(this.e_saldo.getText())) {
					system.alert(this,"Transaksi tidak valid.","Nilai Penarikan melebihi Saldo Simpanan.");
					return false;						
				}
	...
	doChange:function(sender){
		...
		if (sender == this.cb_agg && this.cb_agg.getText()!= "" && this.stsSimpan == 1) {						
			var strSQL = "select bank,cabang,no_rek,nama_rek "+
						 "from kop_agg "+
						 "where no_agg ='"+this.cb_agg.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){												
	...
	doClick:function(sender){
		try {
			if (this.e_periode.getText()!= "") {
				...
				this.noVerLama = "-";
				this.stsSimpan = 1;
				this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti",this.app._lokasi+"-PBS"+this.e_periode.getText().substr(2,4)+".","0000"));						
				this.e_dok.setFocus();
	...
	LoadDataSimp : function(sender) {
		...
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			...
			for (var i in data.rs.rows){
			line = data.rs.rows[i];												
			this.sg5.appendData([line.no_simp,line.jenis.toUpperCase(),line.nama,line.akun_titip,floatToNilai(line.saldo),"0"]);
			}
	...
	doNilaiChange5: function(){
		try{			
			var ambil = saldo = 0;			
			for (var i = 0; i < this.sg5.getRowCount();i++) {
				if (this.sg5.rowValid(i) && this.sg5.cells(4,i) != "" && this.sg5.cells(5,i) != "") {
					saldo += nilaiToFloat(this.sg5.cells(4,i));
					ambil += nilaiToFloat(this.sg5.cells(5,i));
				}
			}			
			this.e_saldo.setText(floatToNilai(Math.round(saldo * 100)/100));			
			this.e_nilai.setText(floatToNilai(Math.round(ambil * 100)/100));			
		}catch(e)
		{
			alert("[app_saku_gl_transaksi_fJu2]::doNilaiChange:"+e);
		}
	...
	doDoubleClick5: function(sender, col , row) {
		if (this.sg5.cells(5,row) == "0") this.sg5.cells(5,row,this.sg5.cells(4,row));
		else this.sg5.cells(5,row,"0");
	},
	...
	doLoad3:function(sender){																				
		...
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.form as modul,a.no_dokumen,a.keterangan,a.progress,a.nilai1, "+
					 ...
					 "where a.posted='F' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.form = 'PBSIM' and a.progress in ('0','V') ";					 
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];												
						this.sg5.appendData([line.no_simp,line.jenis,line.nama,line.akun_titip,floatToNilai(line.saldo),floatToNilai(line.nilai)]);
					}
				} else this.sg5.clear(1);
				this.sg5.validasi();
