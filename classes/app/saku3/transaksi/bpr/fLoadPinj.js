window.app_saku3_transaksi_bpr_fLoadPinj = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bpr_fLoadPinj.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bpr_fLoadPinj";
		...
		this.c_status = new saiCB(this,{bound:[820,17,202,20],caption:"Status Rekon",items:["LOADPINJ"], readOnly:true,tag:8,visible:false});
		...
		this.e_nilai = new saiLabelEdit(this,{bound:[820,14,200,20],caption:"Total Rekon", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		this.cb_app = new saiCBBL(this,{bound:[20,18,220,20],caption:"NIK Approve", multiSelection:false, maxLength:10, tag:2});						
		this.e_cd = new saiLabelEdit(this,{bound:[820,18,200,20],caption:"Total Sisa (CD)", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,340], childPage:["Data Tagihan","Data Pembayaran","Error Msg"]});		
		...
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[1],{bound:[1,5,450,this.pc1.height-35],colCount:4,
					colTitle:["No Anggota","Nilai Bayar","Nilai Rekon","Cek NoAgg"],
		...
		this.sg2 = new portalui_saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:1,tag:9,
		...
		this.e_totBayar = new saiLabelEdit(this.pc1.childPage[1],{bound:[500,14,200,20],caption:"Total Bayar", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		...
		try {
			...
			this.c_status.setText("LOADPINJ");
...
window.app_saku3_transaksi_bpr_fLoadPinj.extend(window.childForm);
window.app_saku3_transaksi_bpr_fLoadPinj.implement({			
	doAfterPaste: function(sender,totalRow){
		try {
			...
			this.inValid = false;
			var strSQL = "select no_agg from kop_agg where flag_aktif='1' and kode_lokasi='"+this.app._lokasi+"'";			
			...
			for (var i=0; i < this.sg1.getRowCount();i++){
				totBayar += nilaiToFloat(this.sg1.cells(1,i));
				this.sg1.cells(3,i,"INVALID");
				
				if (this.dataAgg.rs.rows.length > 0) {
					for (var j=0;j < this.dataAgg.rs.rows.length;j++){				
						if (this.sg1.cells(0,i) == this.dataAgg.rs.rows[j].no_agg) {
	...
	doRekon:	function(sender){				
		try {
			if (this.inValid) {
				system.alert(this,"Data Pembayaran tidak valid.","Terdapat No Anggota Tidak Valid.");
				return false;						
			}
	...
	simpan: function(){			
		try{						
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti",this.app._lokasi+"-PAP"+this.e_periode.getText().substr(2,4)+".","0000"));
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','KP','LOADPINJ','F','0','0','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','-','"+this.e_ket.getText()+
							"','IDR',1,"+nilaiToFloat(this.e_nilai.getText())+",0,0,'"+this.cb_app.getText()+"','-','-','"+this.cb_titip.getText()+"','-','-','-','-','-')");
																
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.cb_titip.getText()+"','D',"+nilaiToFloat(this.e_totBayar.getText())+","+nilaiToFloat(this.e_totBayar.getText())+",'"+this.e_ket.getText()+"','LOADPINJ','TTP','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");
					
					if (this.e_cd.getText() != "0") {
						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',77,'"+this.akunCD+"','C',"+nilaiToFloat(this.e_cd.getText())+","+nilaiToFloat(this.e_cd.getText())+",'"+this.e_ket.getText()+"','LOADPINJ','CDIN','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");
					}
					...
					for (var i in this.gridAR.objList){
						...
						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+line.get("kode_akun")+"','C',"+parseFloat(line.get("nilai"))+","+parseFloat(line.get("nilai"))+",'"+this.e_ket.getText()+"','"+this.c_status.getText()+"','PIUT','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");
					}											
					...
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						...
						if (parseFloat(line.lunas) != 0){
							sql.add("insert into kop_pinjangs_d (no_angs,no_pinj,no_bill,akun_piutang,akun_pjasa,npokok,nbunga,kode_lokasi,dc,periode,cicilan_ke,modul,no_agg) values "+
									"('"+this.e_nb.getText()+"','"+line.no_pinj+"','"+line.no_bill+"','"+line.akun_piutang+"','"+line.akun_pjasa+"',"+line.npokok+","+line.nbunga+",'"+this.app._lokasi+"','D','"+this.e_periode.getText()+"',"+line.cicilan_ke+",'LOADPINJ','"+line.no_agg+"')");
						}
					}					
					
					var cd = 0;
					for (var i=0; i < this.sg1.getRowCount();i++){
						cd = nilaiToFloat(this.sg1.cells(1,i)) - nilaiToFloat(this.sg1.cells(2,i));
						if (cd > 0) {
							sql.add("insert into kop_cd_d (no_bukti,kode_lokasi,no_agg,periode,nilai,kode_akun,dc,modul,no_ref1) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.sg1.cells(0,i)+"','"+this.e_periode.getText()+"',"+cd+",'"+this.akunCD+"','D','LOADPINJ','-')");
						}
					}
