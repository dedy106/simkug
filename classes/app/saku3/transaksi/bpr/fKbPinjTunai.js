window.app_saku3_transaksi_bpr_fKbPinjTunai = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bpr_fKbPinjTunai.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bpr_fKbPinjTunai";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penerimaan Angsuran Per Anggota", 0);	
		...
		this.c_jenis = new saiCB(this.pc2.childPage[0],{bound:[20,16,210,20],caption:"Status",items:["BM","MI"], readOnly:true,tag:2,visible:false});
		...
		this.e_total = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,18,200,20],caption:"Total Angsuran", tag:1, tipeText:ttNilai, text:"0",readOnly:true, change:[this,"doChange"]});				
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,20,990,303], childPage:["Data Akru","Data Angsuran","Jurnal Sisa"]});		
		...
		this.cb_akun = new saiCBBL(this.pc1.childPage[1],{bound:[20,15,220,20],caption:"Akun KasBank", multiSelection:false, maxLength:10, tag:9,change:[this,"doChange"]});							
		this.e_kas = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,12,200,20],caption:"Nilai KasBank", tag:1, tipeText:ttNilai, text:"0", readOnly:true, change:[this,"doChange"]});					
		this.e_saldoCD = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,11,200,20],caption:"Saldo CD", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});	
		this.e_cd = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,12,200,20],caption:"CD Dipakai", tag:1, tipeText:ttNilai, text:"0", change:[this,"doChange"]});					
		...
		this.bCD = new portalui_imageButton(this.sgn,{bound:[740,2,22,22],image:"icon/"+system.getThemes()+"/reload.png", hint:"Hitung CD",click:[this,"doHitCD"]});						
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('KOPCD') and kode_lokasi = '"+this.app._lokasi+"'",true);			
			if (typeof data == "object"){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];					
					if (line.kode_spro == "KOPCD") this.akunCD = line.flag;						
				}
			}
...
window.app_saku3_transaksi_bpr_fKbPinjTunai.extend(window.childForm);
window.app_saku3_transaksi_bpr_fKbPinjTunai.implement({
	doHitCD : function() {
		if (nilaiToFloat(this.e_beban.getText()) > 0) {
			this.sg.clear();
			this.sg.appendData([this.akunCD,"-","C","Sisa kelebihan angsuran No.Agg "+this.cb_agg.getText(),this.e_beban.getText(),this.app._kodePP,this.app._namaPP]);
			this.doChangeCell(this.sg,0,0);
		}
		else system.alert(this,"Data tidak valid.","Nilai Sisa utk CD harus lebih besar dari nol.");
	},
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from kop_cd_d where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					} 

					var akunKB = this.cb_akun.getText();
					if (akunKB == "") akunKB = "-"; 	

					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','KP','PJANGS','F','0','0','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+
							"','IDR',1,"+nilaiToFloat(this.e_total.getText())+","+nilaiToFloat(this.e_kas.getText())+","+nilaiToFloat(this.e_cd.getText())+",'-','-','-','"+akunKB+"','-','-','"+this.cb_agg.getText()+"','"+this.c_jenis.getText()+"','-')");
										
					if (this.e_kas.getText() != "0") {								
						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',98,'"+this.cb_akun.getText()+"','D',"+nilaiToFloat(this.e_kas.getText())+","+nilaiToFloat(this.e_kas.getText())+",'"+this.e_ket.getText()+"','PJANGS','KB','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");
					}
					if (this.e_cd.getText() != "0") {	
						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',99,'"+this.akunCD+"','D',"+nilaiToFloat(this.e_cd.getText())+","+nilaiToFloat(this.e_cd.getText())+",'"+this.e_ket.getText()+"','PJANGS','CD','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");
						sql.add("insert into kop_cd_d (no_bukti,kode_lokasi,no_agg,periode,nilai,kode_akun,dc,modul,no_ref1) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_agg.getText()+"','"+this.e_periode.getText()+"',"+nilaiToFloat(this.e_cd.getText())+",'"+this.akunCD+"','C','PJANGS','-')");								
					}	
					...
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && this.sg1.cells(0,i) == "BAYAR"){
							...
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(4,i)+"','C',"+nilaiPiuPokok+","+nilaiPiuPokok+",'Pelunasan Pokok atas "+this.sg1.cells(2,i)+"','PJANGS','POKOK','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");
					
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sg1.cells(5,i)+"','C',"+nilaiPiuJasa+","+nilaiPiuJasa+",'Pelunasan Jasa atas "+this.sg1.cells(2,i)+"','PJANGS','JASA','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");
					...
					if (this.sg.getRowValidCount() > 0) {
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){								
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg.cells(4,i))+","+nilaiToFloat(this.sg.cells(4,i))+",'"+this.sg.cells(3,i)+"','PJANGS','SLS','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");

								if (this.akunCD == this.sg.cells(0,i) && this.sg.cells(2,i).toUpperCase() == "C") {
									sql.add("insert into kop_cd_d (no_bukti,kode_lokasi,no_agg,periode,nilai,kode_akun,dc,modul,no_ref1) values "+
											"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_agg.getText()+"','"+this.e_periode.getText()+"',"+nilaiToFloat(this.sg.cells(4,i))+",'"+this.akunCD+"','D','PJANGS','-')");								
								}
							}
						}
					}	
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :														
				...
				if (this.e_kas.getText()!="0" && this.cb_akun.getText() == "") {
					system.alert(this,"Transaksi tidak valid.","Akun KasBank harus terisi untuk nilai kasbank yang terisi.");
					return false;						
				}
				if (this.e_kas.getText()=="0" && this.cb_akun.getText() != "") {
					system.alert(this,"Transaksi tidak valid.","Nilai KasBank harus terisi untuk akun KasBank yang terisi.");
					return false;						
				}
				if (nilaiToFloat(this.e_cd.getText()) > nilaiToFloat(this.e_saldoCD.getText())) {
					system.alert(this,"Transaksi tidak valid.","CD Dipakai melebihi Saldo CD.");
					return false;						
				}
	...
	doChange:function(sender){		
		try	{
			if (sender == this.cb_akun) {
				if (this.stsSimpan == 1) this.doClick();
				if (this.cb_akun.getText() == "") this.e_kas.setReadOnly(true);
				else this.e_kas.setReadOnly(false);
			}
			...
			if (sender == this.cb_agg && this.cb_agg.getText()!="") {							
				if (this.stsSimpan==1) {
					var strSQL = "select b.no_bill,a.no_pinj,c.keterangan,a.akun_piutang,a.akun_pjasa,b.periode,b.cicilan_ke,b.npokok, "+
								"case when (b.no_bill = '-' or substring(b.no_bill,4,2) = 'BM' or substring(b.no_bill,4,3) = 'KOM') then 0 else b.nbunga end as nbunga,"+
								"(b.npokok + case when (b.no_bill = '-' or substring(b.no_bill,4,2) = 'BM' or substring(b.no_bill,4,3) = 'KOM') then 0 else b.nbunga end) "+
								" - isnull(d.bayar,0) as saldo "+
								...
								"where  a.no_agg = '"+this.cb_agg.getText()+"' and (b.npokok +  (case when (b.no_bill = '-' or substring(b.no_bill,4,2) = 'BM' or substring(b.no_bill,4,3) = 'KOM') then 0 else b.nbunga end)   )-isnull(d.bayar,0)>0 and a.kode_lokasi= '"+this.app._lokasi+"' order by a.no_pinj,b.periode"; 
				...
				var strSQL ="select sum(case dc when 'D' then nilai else -nilai end) as saldo_cd from kop_cd_d where no_bukti <> '"+this.e_nb.getText()+"' and no_agg='"+this.cb_agg.getText()+"' and  kode_lokasi='"+this.app._lokasi+"'";
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line = data.rs.rows[0];												
					this.e_saldoCD.setText(floatToNilai(line.saldo_cd));
				}				
			}	
			...
			if ((sender == this.e_kas || sender == this.e_cd) && this.e_kas.getText() != "" && this.e_cd.getText() != "") {
				var total = nilaiToFloat(this.e_kas.getText()) + nilaiToFloat(this.e_cd.getText());
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "" && this.c_jenis.getText()!= "") {
			...
			if (this.cb_akun.getText() == "") this.c_jenis.setText("MI");
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						if (line.no_ref1 != "-") this.cb_akun.setText(line.no_ref1);				
						else  this.cb_akun.setText("");				

						this.e_total.setText(floatToNilai(line.nilai1));
						this.e_kas.setText(floatToNilai(line.nilai2));
						this.e_cd.setText(floatToNilai(line.nilai3));
				...
				var strSQL = "select b.no_bill,a.no_pinj,c.keterangan,a.akun_piutang,a.akun_pjasa,b.periode,b.cicilan_ke,b.npokok,b.nbunga,(b.npokok+b.nbunga)-isnull(d.bayar,0) as saldo "+
							 ...
							 "              (select y.no_pinj, y.no_bill, y.kode_lokasi, sum(case dc when 'D' then y.npokok+nbunga else -(y.npokok+nbunga) end) as bayar "+
							 "               from kop_pinjangs_d y inner join trans_m x on y.no_angs=x.no_bukti and y.kode_lokasi=x.kode_lokasi "+
							 "               where y.no_agg = '"+this.cb_agg.getText()+"' and y.kode_lokasi='"+this.app._lokasi+"' and y.no_angs<>'"+this.e_nb.getText()+"' "+
							 "               group by y.no_pinj, y.no_bill, y.kode_lokasi) d on b.no_pinj=d.no_pinj and b.no_bill=d.no_bill and b.kode_lokasi=d.kode_lokasi "+
