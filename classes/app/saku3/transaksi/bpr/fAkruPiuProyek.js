window.app_saku3_transaksi_bpr_fAkruPiuProyek = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bpr_fAkruPiuProyek.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bpr_fAkruPiuProyek";
		...
		this.pc2 = new pageControl(this,{bound:[10,10,1000,450], childPage:["Data Akru Bill","List Akru Bill"]});		
		...
		this.e_nilai = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,16,200,20],caption:"Nilai Tagihan", tag:1, tipeText:ttNilai, text:"0", change:[this,"doChange"]});
		...
		this.e_total = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,16,200,20],caption:"Total Tagihan", tag:1, tipeText:ttNilai, text:"0",readOnly:true});
...
window.app_saku3_transaksi_bpr_fAkruPiuProyek.extend(window.childForm);
window.app_saku3_transaksi_bpr_fAkruPiuProyek.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+												
							"('"+this.e_nb.getText()+"','AKRUPIU','"+this.app._lokasi+"','"+this.cb_akunpdpt.getText()+"','"+this.cb_pp.getText()+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','C',0,"+parseNilai(this.e_nilai.getText())+")"); //C = realisasi
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){							
							if (this.preView == "1") {
								this.nama_report="server_report_saku3_produk_rptProyekPiutangJurnal";
	...
	doLoad3:function(sender){																								
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai1+a.nilai2 as nilai "+
		             "from trans_m a "+
					 " left join ("+
					 "         select distinct no_selesai from trans_j where kode_lokasi='"+this.app._lokasi+"' and jenis='PIUTANG' and modul in ('KB','MI') "+
					 ") b on a.no_bukti=b.no_selesai "+
		             "where b.no_selesai is null and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and posted='F' and modul ='PR' and form='AKRUPIU'";	
		var data = this.dbLib.getDataProvider(strSQL,true);
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				this.cb_proyek.setSQL("select a.kode_proyek, a.nama "+
									  "from pr_proyek a inner join trans_m b on a.kode_proyek =b.no_dokumen and a.kode_lokasi=b.kode_lokasi and b.modul='PR' and b.form='AKRUPIU' "+
				...
				var strSQL = "select b.akun_piutang,b.akun_pdpt,a.nilai-isnull(c.nilai_piu,0) as sisa_akru,d.nilai1, d.nilai2,a.kode_cust,a.kode_drkp,nik1 "+
							 "from pr_proyek a "+
							 "inner join pr_jenis b on a.kode_jenis=b.kode_jenis and a.kode_lokasi=b.kode_lokasi "+
							 "inner join trans_m d on a.kode_proyek=d.no_dokumen and a.kode_lokasi=d.kode_lokasi and d.modul='PR' and d.form='AKRUPIU' "+
							 "left join ( "+						 		
									"select no_dokumen,kode_lokasi,isnull(sum(case dc when 'D' then nilai else -nilai end),0) as nilai_piu "+
									"from trans_j "+
