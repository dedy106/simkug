window.app_saku3_transaksi_bpr_fVer = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_bpr_fVer.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bpr_fVer";
		...
		this.cb_pp = new saiCBBL(this,{bound:[20,18,220,20],caption:"Unit/Cabang", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});								
		this.e_periode = new portalui_saiLabelEdit(this,{bound:[20,11,200,20],caption:"Periode",tag:2,readOnly:true,visible:false});
		this.l_tgl1 = new portalui_label(this,{bound:[20,11,100,18],caption:"Tanggal", underline:true});
		this.dp_d1 = new portalui_datePicker(this,{bound:[120,11,98,18],selectDate:[this,"doSelectDate"]});		
		
		this.pc1 = new pageControl(this,{bound:[10,18,1000,410], childPage:["Daftar Bukti","Data Bukti","Item Verifikasi","Jurnal & Budget","Filter Data"]});		
		...
		this.sg1 = new saiGrid(this.pc1.childPage[1],{bound:[1,16,this.pc1.width-5,192],colCount:7,tag:9,
		...
		this.sg3 = new saiGrid(this.pc1.childPage[3],{bound:[1,5,this.pc1.width-5,190],colCount:9,tag:9,
		...
		this.sg2 = new saiGrid(this.pc1.childPage[3],{bound:[1,210,this.pc1.width-5,190],colCount:5,tag:9,
		...
		this.c_modul2 = new saiCB(this.pc1.childPage[4],{bound:[20,11,200,20],caption:"Modul",items:["IFREIM","PBDIR","PBHUT","PBSIM","PBPIN","PJPTG"], readOnly:true,tag:9,change:[this,"doChange"]});		
		...
		try {			
			...
			sql.add("select a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '049' "+					
...
window.app_saku3_transaksi_bpr_fVer.extend(window.childForm);
window.app_saku3_transaksi_bpr_fVer.implement({	
		...
		try{														
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.c_status.getText() == "RETURN") var vStatus = "V"; else var vStatus = "1";										
					
					sql.add("update pb_app_m set no_flag='"+this.e_nb.getText()+"' where no_bukti='"+this.e_nobukti.getText()+"' and no_flag='-' and form='APPVER' and modul='"+this.e_modul.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("insert into pb_app_m (no_app,kode_lokasi,tanggal,periode,tgl_input,nik_user,status,modul,form,no_bukti,catatan,no_flag,nik_bdh,nik_fiat) values "+
					...
					if (this.e_modul.getText() == "PBPIN" || this.e_modul.getText() == "PBSIM" || this.e_modul.getText() == "PBHUT" || this.e_modul.getText() == "PBDIR" || this.e_modul.getText() == "IFREIM") {
						sql.add("update trans_m set no_ref2='"+this.e_nb.getText()+"',progress='"+vStatus+"' where no_bukti='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
						if (vStatus == "1") {
							sql.add("insert into pb_app_j (no_app,no_bukti,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,nilai_curr,kode_pp,kode_drk,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) "+
									"select '"+this.e_nb.getText()+"',no_bukti,'-',tanggal,0,kode_akun,keterangan,'D',nilai,nilai,kode_pp,kode_drk,'-',kode_lokasi,'"+this.e_modul.getText()+"',jenis,periode,'IDR',1,'"+this.app._userLog+"',getdate() "+
									"from trans_j "+
									"where jenis='BMHD' and no_bukti = '"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						}
					}
					...
					if (this.e_modul.getText() != "PJPTG" && this.e_modul.getText() != "PRPTG") {					
						...
						for (var i=0;i < this.sgv.getRowCount();i++){
							if (this.sgv.rowValid(i)){
								var nu = 1000+i;
								sql.add("insert into pb_app_j (no_app,no_bukti,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,nilai_curr,kode_pp,kode_drk,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input)  values "+
						...
						//---------------- rekening
						sql.add("delete from pb_rek where no_bukti='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){
								var neto = nilaiToFloat(this.sg1.cells(4,i)) - nilaiToFloat(this.sg1.cells(5,i));
								sql.add("insert into pb_rek(no_bukti,kode_lokasi,modul,nama_rek,no_rek,bank,cabang,bruto,pajak,nilai) values "+
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "hapus" :	
				...
				uses("server_util_arrayList");
				var sql = new server_util_arrayList();									
				sql.add("delete from pb_app_m where no_app='"+this.noVerLama+"' and kode_lokasi='"+this.app._lokasi+"'");
				sql.add("delete from pb_app_j where no_app='"+this.noVerLama+"' and kode_lokasi='"+this.app._lokasi+"'");
				
				// ------------------------------------------------ unFLAG ------------------------------------------------
				if (this.e_modul.getText() == "PBPIN" || this.e_modul.getText() == "PBSIM" || this.e_modul.getText() == "PBHUT" || this.e_modul.getText() == "PBDIR" || this.e_modul.getText() == "IFREIM") 
					sql.add("update trans_m set no_ref2='-',progress='0' where no_bukti='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
				
				if (this.e_modul.getText() == "PJPTG" || this.e_modul.getText() == "PRPTG") 
					sql.add("update panjarptg2_m set no_ver='-',progress='2' where no_ptg='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doChange:function(sender){				
		...
		if (sender == this.c_modul2) {			
			if (this.c_modul2.getText() == "PBPIN" || this.c_modul2.getText() == "PBSIM" || this.c_modul2.getText() == "PBHUT" || this.c_modul2.getText() == "PBDIR" || this.c_modul2.getText() == "IFREIM") 
				this.cb_nb.setSQL("select no_bukti, keterangan from trans_m where form ='"+this.c_modul2.getText()+"' and periode='"+this.e_periode.getText()+"' and progress in ('1','V') and kode_lokasi='"+this.app._lokasi+"'",["no_bukti","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);					
			if (this.c_modul2.getText() == "PJPTG" || this.c_modul2.getText() == "PRPTG") 
				this.cb_nb.setSQL("select no_ptg, keterangan from panjarptg2_m where periode='"+this.e_periode.getText()+"' and progress in ('1','V') and kode_lokasi='"+this.app._lokasi+"'",["no_ptg","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);					
		}
		if (sender == this.cb_nb && this.cb_nb.getText() != "") {			
			if (this.c_modul2.getText() == "PBPIN" || this.c_modul2.getText() == "PBSIM" || this.c_modul2.getText() == "PBHUT" || this.c_modul2.getText() == "PBDIR" || this.c_modul2.getText() == "IFREIM") {				
				var strSQL = "select a.due_date,a.no_bukti,case a.progress when '1' then 'APPROVE' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.form as modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai1 as nilai,c.nik+' - '+c.nama as pembuat,a.no_ref2 as no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
							 ...
							 "where a.no_bukti='"+this.cb_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' ";
	...
	doClick:function(sender){		
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"pb_app_m","no_app",this.app._lokasi+"-SPB"+this.e_periode.getText().substr(2,4)+".","0000"));												
	...
	doLoadRek:function(){
		var strSQL1 = "select bank,cabang,no_rek,nama_rek,bruto,pajak,'"+this.e_ket.getText()+"' as keterangan "+
					  "from pb_rek where no_bukti ='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";
	...
	doLoadGar:function(){						
		if (this.e_modul.getText() == "PJPTG" || this.e_modul.getText() == "PRPTG" || this.e_modul.getText() == "IFREIM" ||
			this.e_modul.getText() == "PBDIR" || this.e_modul.getText() == "PBHUT" || this.e_modul.getText() == "PBSIM"  || 
			this.e_modul.getText() == "PBPIN" ) {
			var strSQL2 = "select a.kode_akun,a.kode_pp,a.saldo,a.nilai,a.saldo-a.nilai as sakhir "+
						  "from angg_r a "+
		...
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			...
			for (var i in data.rs.rows){
				line = data.rs.rows[i];													
				this.sg2.appendData([line.kode_akun,line.kode_pp,floatToNilai(line.saldo),floatToNilai(line.nilai),floatToNilai(line.sakhir)]);
			}
		} else this.sg2.clear(1);
	},
	doLoadJurnal:function(){		
		if (this.e_modul.getText() == "PBPIN" || this.e_modul.getText() == "PBSIM" || this.e_modul.getText() == "PBHUT" || this.e_modul.getText() == "PBDIR" || this.e_modul.getText() == "IFREIM" ) {			  
			var strSQL3 = "select b.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,no_selesai as kode_proyek,'-' as nama_proyek "+
					  	  "from trans_j a inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
					  	  "               inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+												 
					  	  "where a.no_bukti = '"+this.e_nobukti.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.dc desc";
		}
		...
		var strSQLv = "select b.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp "+
					  "from pb_app_j a inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
					  "                inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+																					  
					  "where a.jenis='VER' and a.no_app = '"+this.noVerLama+"' and a.kode_lokasi='"+this.app._lokasi+"' ";
		var data = this.dbLib.getDataProvider(strSQLv,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			var line;
			this.sgv.clear();
			for (var i in data.rs.rows){
	...
	doLoad:function(sender){															
		if (this.cb_pp.getText() == "") var filter = " ";
		else var filter  = " and b.kode_pp='"+this.cb_pp.getText()+"' ";

		var strSQL = "select a.due_date,a.no_bukti,case a.progress when '0' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.form as modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai1 as nilai,c.nik+' - '+c.nama as pembuat,a.no_ref2 as no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
					 "from trans_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+filter+
					 "               inner join karyawan c on a.nik1=c.nik and a.kode_lokasi=c.kode_lokasi "+
					 "where a.progress in ('0') and a.kode_lokasi='"+this.app._lokasi+"' and a.form in ('PBPIN','PBSIM','PBHUT','PBDIR','IFREIM') "+					 
					 
					 "union all "+ 
					 
					 "select a.tanggal as due_date,a.no_ptg as no_bukti,case a.progress when '0' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.tanggal,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
					 "from panjarptg2_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+filter+
					 "                    inner join karyawan c on a.nik_buat=c.nik and a.kode_lokasi=c.kode_lokasi "+
					 "where a.progress in ('0') and a.kode_lokasi='"+this.app._lokasi+"' "+	
	...
	doNilaiChange: function(){		
		try{
			...
			//untuk pertanggungan Panjar dan PJ Proyek, nilai rek diisi nilai pertanggungannya
		...
		try{			
			if (col == 0){
				this.standarLib.showListData(this, "Daftar Akun",sender,undefined, 
						"select a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '049' "+							
						"where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'",
						"select count(a.kode_akun) from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '049' where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'",
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){							
							if (this.preView == "1") {
								this.nama_report="server_report_saku3_bpr_rptVerif";									                  
