window.app_saku3_transaksi_bpr_fAkruPiuUnbill = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bpr_fAkruPiuUnbill.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bpr_fAkruPiuUnbill";
...
window.app_saku3_transaksi_bpr_fAkruPiuUnbill.extend(window.childForm);
window.app_saku3_transaksi_bpr_fAkruPiuUnbill.implement({
	doLoadCBproyek: function() {			
			var strSQL = "select a.kode_proyek,a.nama as keterangan "+
						 ...
						 "	from trans_j					"+
						 "	where modul ='PR' and jenis in ('PIUTANG','HPPN','UNBILL','REVUNBILL') and kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.cb_pp.getText()+"' "+
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {						
						...
						sql.add("delete from pr_unbill_d where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");	
					}					
					...
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akunpiu.getText()+"','D',"+parseNilai(this.e_nilai.getText())+","+parseNilai(this.e_nilai.getText())+",'"+this.e_ket.getText()+"','PR','UNBILL','IDR',1,'"+this.cb_pp.getText()+"','-','"+this.cb_cust.getText()+"','-','-','-','-','-','-')");
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',1,'"+this.cb_akunpdpt.getText()+"','C',"+parseNilai(this.e_nilai.getText())+","+parseNilai(this.e_nilai.getText())+",'"+this.e_ket.getText()+"','PR','PDPT','IDR',1,'"+this.cb_pp.getText()+"','-','-','-','-','-','-','-','-')");					
					
					
					sql.add("insert into pr_unbill_d (no_bukti,kode_lokasi,tgl_input,nik_user,periode,kode_proyek,kode_cust,akun_piutang,akun_pdpt,kode_drk,nilai,no_reverse) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.cb_cust.getText()+"','"+this.cb_akunpiu.getText()+"','"+this.cb_akunpdpt.getText()+"','-',"+nilaiToFloat(this.e_nilai.getText())+",'-')");
		...
		if (sender == this.cb_proyek && this.cb_proyek.getText()!="" && this.stsSimpan==1) {
			...
			var strSQL = "select b.akun_piutang,b.akun_pdpt,a.nilai-isnull(c.nilai_piu,0) as sisa_akru,a.kode_cust,a.kode_drkp "+
						 ...
						 "left join ( "+						 		
						 		...
						 		"from trans_j "+
						 		"where modul ='PR' and jenis in ('PIUTANG','HPPN','UNBILL','REVUNBILL') and no_dokumen='"+this.cb_proyek.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
						 		"group by no_dokumen,kode_lokasi "+										 				 		
						 "		) c "+
	...
	doLoad3:function(sender){																								
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai1+a.nilai2 as nilai "+
		             "from trans_m a inner join pr_unbill_d b on a.no_bukti=b.no_bukti and a.kode_lokasi=b.kode_lokasi and b.no_reverse='-' "+
		             "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and posted='F' and modul ='PR' and form='UNBILL'";	
		var data = this.dbLib.getDataProvider(strSQL,true);
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select b.akun_piutang,b.akun_pdpt,a.nilai-isnull(c.nilai_piu,0) as sisa_akru,d.nilai1,a.kode_cust,a.kode_drkp,nik1 "+
							 "from pr_proyek a "+
							 "inner join pr_jenis b on a.kode_jenis=b.kode_jenis and a.kode_lokasi=b.kode_lokasi "+
							 ...
							 "left join ( "+						 		
									"select no_dokumen,kode_lokasi,isnull(sum(case dc when 'D' then nilai else -nilai end),0) as nilai_piu "+
									"from trans_j "+
									"where modul='PR' and jenis in ('PIUTANG','HPPN','UNBILL','REVUNBILL') and no_bukti <> '"+this.e_nb.getText()+"' and no_dokumen='"+this.cb_proyek.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
									"group by no_dokumen,kode_lokasi "+						 											
							 "		) c "+
							 "		on  a.kode_proyek=c.no_dokumen and a.kode_lokasi=c.kode_lokasi "+
