window.app_saku3_transaksi_bpr_fMasakunRelasiGar = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bpr_fMasakunRelasiGar.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bpr_fMasakunRelasiGar";
		...
		this.cb_gar = new saiCBBL(this.pc2.childPage[3],{bound:[20,11,220,20],caption:"Akun Budget", readOnly:true, maxLength:10, tag:2});
...
window.app_saku3_transaksi_bpr_fMasakunRelasiGar.extend(window.childForm);
window.app_saku3_transaksi_bpr_fMasakunRelasiGar.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("delete from masgar where kode_gar = '"+this.cb_kode.getText()+"'");
					...
					if (typeof data == "object" && data.rs.rows[0] != undefined){
						...
						for (var i in data.rs.rows){
							...
							//copy ke masgar sbg akun budget
							sql.add("insert into masgar(kode_gar,kode_lokasi,nama,status_gar) values "+
									"('"+this.cb_kode.getText()+"','"+line.kode_lokasi+"','"+this.e_nama.getText()+"','"+this.c_gar.getText().substring(0,1)+"')");					
								...
								for (var k=0;k < this.sg.getRowCount();k++){
									if (this.sg.rowValid(k)){								
										sql.add("insert into flag_relasi(kode_akun,kode_lokasi,kode_flag) values "+
												"('"+this.cb_kode.getText()+"','"+line.kode_lokasi+"','"+this.sg.cells(0,k)+"')");
									}
								}
							}
							
							if (this.sg3.getRowValidCount() > 0){
								for (var l=0;l < this.sg3.getRowCount();l++){
									if (this.sg3.rowValid(l)){								
										sql.add("insert into relakun (kode_neraca,kode_fs,kode_akun,kode_lokasi) values ('"+this.sg3.cells(2,l)+"','"+this.sg3.cells(0,l)+"','"+this.cb_kode.getText()+"','"+line.kode_lokasi+"')");
									}
								}
							}

							if (this.sg4.getRowValidCount() > 0){
								for (var m=0;m < this.sg4.getRowCount();m++){
									if (this.sg4.rowValid(m)){								
										sql.add("insert into relakun_pp (kode_pp,kode_akun,kode_lokasi) values ('"+this.sg4.cells(0,m)+"','"+this.cb_kode.getText()+"','"+line.kode_lokasi+"')");
									}
								}
							}
				...
				if (this.cb_kode.getText() != "" ){
					...
					if (typeof data == "object"){
						...
						if (line != undefined){
							...
							var data = this.dbLib.getDataProvider("select b.kode_pp,b.nama from relakun_pp a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi where a.kode_akun = '"+this.cb_kode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by b.kode_pp",true);
							if (typeof data == "object" && data.rs.rows[0] != undefined){
								var line;
								this.sg4.clear();
								for (var i in data.rs.rows){
							...
							this.cb_gar.setText(this.cb_kode.getText());
			...
			if (sender == this.sg) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Flag Akun",sender,undefined, 
												  ...
												  ["kode_flag","nama"],"where",["Kode","Nama"],false);				
				}						
			}			
		}catch(e){
			systemAPI.alert(e);
