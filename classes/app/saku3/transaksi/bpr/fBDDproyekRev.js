window.app_saku3_transaksi_bpr_fBDDproyekRev = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bpr_fBDDproyekRev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bpr_fBDDproyekRev";
		...
		this.cb_akun = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,18,220,20],caption:"Akun Beban",tag:1,readOnly:true,change:[this,"doChange"]}); 										
...
window.app_saku3_transaksi_bpr_fBDDproyekRev.extend(window.childForm);
window.app_saku3_transaksi_bpr_fBDDproyekRev.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into pr_bdd_dist (no_bukti,kode_lokasi,periode,kode_proyek,kode_akun,dc,nilai,jenis,kode_pp,kode_drk) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.cb_akun.getText()+"','D',"+nilaiToFloat(this.e_nilai.getText())+",'BEBAN','"+this.cb_pp.getText()+"','-')");								
					
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_gar,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+												
							"('"+this.e_nb.getText()+"','BDDBEBAN','"+this.app._lokasi+"','"+this.cb_akun.getText()+"','"+this.cb_pp.getText()+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','C',"+parseNilai(this.e_saldogar.getText())+","+parseNilai(this.e_nilai.getText())+")");
