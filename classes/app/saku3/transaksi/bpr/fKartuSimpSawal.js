window.app_saku3_transaksi_bpr_fKartuSimpSawal = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bpr_fKartuSimpSawal.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bpr_fKartuSimpSawal";
		...
		this.e_saldo = new portalui_saiLabelEdit(this.pc1.childPage[0],{bound:[20,17,200,20],caption:"Saldo Simpanan",maxLength:15,tipeText:ttNilai,text:"0"});		
		this.e_saldobunga = new portalui_saiLabelEdit(this.pc1.childPage[0],{bound:[20,18,200,20],caption:"Saldo Bunga",maxLength:15,tipeText:ttNilai,text:"0"});		
		...
		try{			
			...
			var data = this.dbLib.getDataProvider("select a.kode_spro,a.flag from spro a where a.kode_spro in ('BSIMP') and a.kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku3_transaksi_bpr_fKartuSimpSawal.extend(window.portalui_childForm);
window.app_saku3_transaksi_bpr_fKartuSimpSawal.implement({
		...
		try{					
			...
			if (this.standarLib.checkEmptyByTag(this, [0,2])){
				try{														
					...
					if (this.e_jenis.getText() == "SP") var vPeriode = "999999";
					else var vPeriode = thnBln;
					...
					sql.add("insert into kop_simp_m (no_simp,kode_lokasi,no_agg,kode_param,jenis,nilai,p_bunga,tgl_tagih,status_bayar,periode_gen,flag_aktif,nik_user,tgl_input,periode_bunga) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_agg.getText()+"','"+this.cb_simp.getText()+"','"+this.e_jenis.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+","+nilaiToFloat(this.e_bunga.getText())+",'"+this.dp_d1.getDateString()+"','"+this.cb_status.getText()+"','"+vPeriode+"','"+this.c_flag.getText().substr(0,1)+"','"+this.app._userLog+"',getdate(),'"+vPeriode+"')");
					...
					//		"('"+this.e_nb.getText()+"','-','-','"+this.dp_d1.getDateString()+"',"+nilaiToFloat(this.e_saldo.getText())+",'"+thnBln+"','"+this.app._kodePP+"','"+this.app._lokasi+"','"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'X','IDR',1,'SAWAL')");					
					sql.add("insert into kop_simp_d (no_simp,no_bill,kode_lokasi,periode,nilai,akun_piutang,akun_titip,dc,modul,no_agg) values "+
							"('"+this.e_nb.getText()+"','"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+thnBln+"',"+nilaiToFloat(this.e_saldo.getText())+",'"+this.cb_piutang.getText()+"','"+this.cb_titip.getText()+"','D','GENBILL','"+this.cb_agg.getText()+"')");
					...
					//		"('"+this.e_nb.getText()+"','-','-','"+this.dp_d1.getDateString()+"',"+nilaiToFloat(this.e_saldo.getText())+",0,0,'KBSIMP','"+thnBln+"','"+this.app._lokasi+"','X','"+this.app._kodePP+"','"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.e_nb.getText()+"')");
					
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+thnBln+"','KOP','KBSIMP','X','0','0','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','-','-','IDR',1,"+nilaiToFloat(this.e_saldo.getText())+",0,0,'-','-','-','-','-','-','-','-','-')");
					...
					sql.add("insert into kop_simpangs_d (no_angs,no_simp,no_bill,akun_piutang,nilai,kode_lokasi,dc,periode,modul,no_agg,jenis) values "+
							"('"+this.e_nb.getText()+"','"+this.e_nb.getText()+"','"+this.e_nb.getText()+"','"+this.cb_piutang.getText()+"',"+nilaiToFloat(this.e_saldo.getText())+",'"+this.app._lokasi+"','D','"+thnBln+"','SIMPTUNAI','"+this.cb_agg.getText()+"','SIMP')");																							
					if (nilaiToFloat(this.e_saldobunga.getText()) != 0) {
						sql.add("insert into kop_simpangs_d (no_angs,no_simp,no_bill,akun_piutang,nilai,kode_lokasi,dc,periode,modul,no_agg,jenis) values "+
								"('"+this.e_nb.getText()+"','"+this.e_nb.getText()+"','"+this.e_nb.getText()+"','"+this.akunBunga+"',"+nilaiToFloat(this.e_saldobunga.getText())+",'"+this.app._lokasi+"','D','"+thnBln+"','SIMPTUNAI','"+this.cb_agg.getText()+"','BSIMP')");																							
					}						
					setTipeButton(tbAllFalse);					
	...
	doClick:function(sender){
		try {			
			...
			this.stsSimpan = 1;
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti",this.app._lokasi+"-"+this.e_jenis.getText()+this.cb_agg.getText()+".","0000"));		
		    this.dp_d1.setFocus();
	...
	doDoubleClick1: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {
				...
				var strSQL = "select a.*, b.nilai as saldo,isnull(c.nilai,0) as bunga "+
							 "from kop_simp_m a inner join kop_simpangs_d b on a.no_simp=b.no_angs and a.kode_lokasi=b.kode_lokasi and b.jenis='SIMP' "+	
							 "					left join kop_simpangs_d c on a.no_simp=c.no_angs and a.kode_lokasi=c.kode_lokasi and c.jenis='BSIMP' "+							 
							 "where a.no_simp = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";						
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){								
						...
						this.e_saldobunga.setText(floatToNilai(line.bunga));
