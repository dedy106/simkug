window.app_saku3_transaksi_bpr_fBebanProyekDummyRev = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bpr_fBebanProyekDummyRev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bpr_fBebanProyekDummyRev";
		...
		this.sg1 = new saiGrid(this.pc2.childPage[0],{bound:[0,5,this.pc2.width-5,293],colCount:8,tag:0,
				colTitle:["Status","No Referensi","Tanggal","Keterangan","Akun Beban","Akun Hutang","Nilai Akru","Vendor"],
				colWidth:[[7,6,5,4,3,2,1,0],[80,100,80,80,320,80,120,80]],
...
window.app_saku3_transaksi_bpr_fBebanProyekDummyRev.extend(window.childForm);
window.app_saku3_transaksi_bpr_fBebanProyekDummyRev.implement({
						...
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i) && this.sg1.cells(0,i) == "REVERSE"){
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.sg1.cells(5,i)+"','D',"+parseNilai(this.sg1.cells(6,i))+","+parseNilai(this.sg1.cells(6,i))+",'"+this.e_ket.getText()+"','PR','HUTDUMMY','IDR',1,'"+this.cb_pp.getText()+"','-','-','-','-','-','-','-','-')");								
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',1,'"+this.sg1.cells(4,i)+"','C',"+parseNilai(this.sg1.cells(6,i))+","+parseNilai(this.sg1.cells(6,i))+",'"+this.e_ket.getText()+"','PR','BBNDUMMY','IDR',1,'"+this.cb_pp.getText()+"','-','-','-','-','-','-','-','-')");					
								...
								sql.add("insert into pr_or_d (no_bukti,kode_lokasi,periode,kode_proyek,kode_akun,dc,nilai,jenis,kode_pp,kode_drk,no_reverse) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.sg1.cells(4,i)+"','C',"+nilaiToFloat(this.sg1.cells(6,i))+",'REVDUMMY','"+this.cb_pp.getText()+"','-','NONREV')");								
							}
						}
					}
			...
			if (this.stsSimpan==1) {
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData(["INPROG",line.no_bukti,line.tgl,line.keterangan,line.kode_akun,line.akun_hutang,floatToNilai(line.nilai),line.kode_vendor]);
					}
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData(["REVERSE",line.no_bukti,line.tgl,line.keterangan,line.kode_akun,line.akun_hutang,floatToNilai(line.nilai),line.kode_vendor]);
					}
				} else this.sg1.clear(1);
