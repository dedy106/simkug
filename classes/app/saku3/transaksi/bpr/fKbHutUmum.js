window.app_saku3_transaksi_bpr_fKbHutUmum = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bpr_fKbHutUmum.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bpr_fKbHutUmum";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pembayaran Hutang Umum", 0);	
		...
		this.c_jenis = new saiCB(this.pc2.childPage[0],{bound:[20,22,200,20],caption:"Jenis",items:["MI"], readOnly:true,tag:2,change:[this,"doChange"]}); //"BK", ---> harus lewat modul PBH
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,234], childPage:["Item Jurnal Pelunasan"]});		
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:7,tag:0,
		            colTitle:["Kode Akun","Nama Akun","DC","Keterangan","Nilai","Kode PP","Nama PP"],
					colWidth:[[6,5,4,3,2,1,0],[150,80,100,250,50,200,100]],					
		...
		this.bLoad = new portalui_imageButton(this.sgn,{bound:[this.sgn.width - 25,2,22,22],image:"icon/"+system.getThemes()+"/reload.png", hint:"Load Jurnal Ref",click:[this,"doLoad"]});		
...
window.app_saku3_transaksi_bpr_fKbHutUmum.extend(window.childForm);
window.app_saku3_transaksi_bpr_fKbHutUmum.implement({
	doLoad : function(sender) {
		if (this.cb_hutang.getText()!="") {
			strSQL = "select a.kode_akun,b.nama as nama_akun,case a.dc when 'D' then 'C' else 'D' end as dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp "+
					...
					"where a.jenis = 'BBN' and a.no_bukti = '"+this.cb_hutang.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.nu";
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','KB','KBHUT','F','0','0','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+
							"','IDR',1,"+parseNilai(this.e_nilai.getText())+",0,0,'"+this.cb_app.getText()+"','-','-','"+this.cb_hutang.getText()+"','-','-','"+this.cb_vendor.getText()+"','"+this.c_jenis.getText()+"','-')");
					
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_hutang.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.akunHutang+"','C',"+nilaiToFloat(this.e_nilai.getText())+","+nilaiToFloat(this.e_nilai.getText())+",'"+this.e_ket.getText()+"','KBHUT','PIU','IDR',1,'"+this.app._kodePP+"','-','"+this.cb_vendor.getText()+"','-','-','-','-','-','-')");
					
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){																															
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(2,i)+"',"+parseNilai(this.sg.cells(4,i))+","+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(3,i)+"','KBHUT','KB','IDR',1,'"+this.sg.cells(5,i)+"','-','-','-','-','-','-','-','-')");
								
								if (this.sg.cells(2,i) == "D") var DC = "C"; else var DC = "D";								
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_gar,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai,gar_thn,gar_tw,gar_bulan,no_ref,param1,param2) values "+
										"('"+this.e_nb.getText()+"','AP','"+this.app._lokasi+"','"+this.sg.cells(0,i)+"','"+this.sg.cells(5,i)+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',0,"+parseNilai(this.sg.cells(4,i))+",0,0,0,'"+this.cb_hutang.getText()+"','-','-')");		
							}
						}
					}
					sql.add("insert into hu_bayar_d(no_bukti,no_hutang,kode_lokasi,modul,periode,nik_user,tgl_input,dc,nilai, kode_vendor) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_hutang.getText()+"','"+this.app._lokasi+"','KBHUT','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'D',"+parseNilai(this.e_nilai.getText())+", '"+this.cb_hutang.getText()+"')");					
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :	
				...
				if (this.c_jenis.getText() == "MI") var vModul = "MI"; else var vModul = "KB";
				if (this.standarLib.doCekPeriode(this.dbLib,vModul,this.app._lokasi,this.app._userStatus,this.e_periode.getText()) == "0") {
					system.alert(this,"Periode transaksi modul tidak valid ("+vModul+" - LOCKED).","Hubungi Administrator Sistem.");
					return false;
				} 
				else this.simpan();
				break;				
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_hutang && this.cb_hutang.getText()!= "") {
			var strSQL = "select (a.nilai+a.nilai_ppn) -  isnull(b.bayar,0) as saldo, a.akun_hutang,  "+			             
						 "from hu_bill_d a "+						 
		...
		if (sender == this.c_jenis && this.c_jenis.getText()!="") {
			if (this.stsSimpan == 1) this.doClick();
			if (this.c_jenis.getText() == "BK") var vFlagAkun = " in ('001','009') ";	
			else var vFlagAkun = " in ('034') ";	

			var sql = new server_util_arrayList();			
			sql.add("select a.kode_akun,a.nama "+
					"from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag "+vFlagAkun+" "+					
			...
			if (sender == this.sg) {
				if (col == 0){
					...
					this.standarLib.showListData(this, "Daftar Akun",sender,undefined, 
												  "select a.kode_akun,a.nama    from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag "+vFlagAkun+" where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'",
												  "select count(*)  from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag "+vFlagAkun+" where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'",
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.param2,a.no_dokumen,a.keterangan,a.nilai1 "+
		             "from trans_m a "+
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_bukti,line.tgl,line.param2,line.no_dokumen,line.keterangan,floatToNilai(line.nilai1)]); 
		}
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.akun_hutang,a.kode_vendor,e.param2,e.periode,e.no_dokumen,e.tanggal,e.keterangan as ket_kas,e.nik1,e.no_ref1 "+
							 "from hu_bill_d a inner join trans_m e on e.no_ref1 = a.no_hutang and a.kode_lokasi=e.kode_lokasi "+							 
				...
				if (typeof data == "object"){
					...
					if (line != undefined){		
						...
						var strSQL = "select a.no_hutang,a.keterangan "+
									 "from hu_bill_d a "+										
									 "where a.no_hutang='"+line.no_ref1+"' and a.kode_lokasi='"+this.app._lokasi+"'";
						this.cb_hutang.setSQL(strSQL,["a.no_hutang","a.keterangan"],false,["No Bukti","Keterangan"],"and","Data Hutang",true);											
						
						this.cb_vendor.setText(line.kode_vendor);
						this.cb_hutang.setText(line.no_ref1);						
					} 
				}		
