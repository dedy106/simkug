window.app_saku3_transaksi_bpr_fAkruJasaPinj = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bpr_fAkruJasaPinj.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bpr_fAkruJasaPinj";
		...
		this.sg2 = new portalui_saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-10],colCount:7,			    
		...
		this.sg1 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:12,tag:9,		            
...
window.app_saku3_transaksi_bpr_fAkruJasaPinj.extend(window.portalui_childForm);
window.app_saku3_transaksi_bpr_fAkruJasaPinj.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,2])){
				try{													
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','KP','BILPINJ','F','0','0','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','-','"+this.e_desc.getText()+
							"','IDR',1,"+nilaiToFloat(this.e_total.getText())+",0,0,'-','-','-','-','-','-','-','-','-')");
					...
					for (var i=0; i<this.sg2.getRowCount(); i++){
						if (this.sg2.rowValid(i)){
							...
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg2.cells(2,i)+"','D',"+nilaiToFloat(this.sg2.cells(6,i))+","+nilaiToFloat(this.sg2.cells(6,i))+",'"+this.e_desc.getText()+"','BILPINJ','PIUJASA','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");
					
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sg2.cells(4,i)+"','C',"+nilaiToFloat(this.sg2.cells(6,i))+","+nilaiToFloat(this.sg2.cells(6,i))+",'"+this.e_desc.getText()+"','BILPINJ','PDPT','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");
						}
					}					
	...
	doClick:function(sender){
		...
		this.stsSimpan = 1;
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti",this.app._lokasi+"-BPJ"+this.e_periode.getText().substr(2,4)+".","0000"));
		...
		try{			
			if (this.e_periode.getText() != "") {
				...
				var strSQL = "select c.kode_param,c.nama,a.akun_pjasa,d.nama as nama_piu,c.akun_jasa,e.nama as nama_pdpt,sum(b.nbunga) as tbunga "+
							 ...
							 //link ke pb nya sudah dicairkan
							 "inner join trans_m g on a.no_kas=g.no_bukti and a.kode_lokasi=g.kode_lokasi and g.form='PBPIN' and g.no_ref3<>'-' "+
	...
	doLoad3:function(sender){																				
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai1 "+
		             "from trans_m a "+
					 "where a.kode_lokasi='"+this.app._lokasi+"' and a.periode='"+this.e_periode.getText()+"' and a.posted='F' and a.form='BILPINJ'";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
