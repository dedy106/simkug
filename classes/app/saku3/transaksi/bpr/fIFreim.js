window.app_saku3_transaksi_bpr_fIFreim = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bpr_fIFreim.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bpr_fIFreim";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan Reimburse", 0);	
		...
		this.c_status = new saiCB(this.pc2.childPage[0],{bound:[790,12,200,20],caption:"Status",items:["IFREIM","TUTUP"], readOnly:true,tag:2});
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,305], childPage:["Data Reimburse","Controlling","Data Rekening"]});
		...
		this.e_bank = new saiLabelEdit(this.pc1.childPage[2],{bound:[20,16,550,20],caption:"Bank", maxLength:150});									
		this.e_cab = new saiLabelEdit(this.pc1.childPage[2],{bound:[20,17,550,20],caption:"Cabang", maxLength:150});									
		this.e_norek = new saiLabelEdit(this.pc1.childPage[2],{bound:[20,18,550,20],caption:"No Rekening", maxLength:150});									
		this.e_namarek = new saiLabelEdit(this.pc1.childPage[2],{bound:[20,19,550,20],caption:"Nama Rekening", maxLength:150});									
		...
		try {
			...
			this.cb_nik.setSQL("select a.nik, a.nama from karyawan a inner join if_m b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi and b.tahun='"+this.dp_d1.year+"' where b.flag_aktif='1' and a.nik='"+this.app._userLog+"' and a.kode_lokasi='"+this.app._lokasi+"'",["a.nik","a.nama"],false,["Kode","Nama"],"and","Data Pemegang",true);
...
window.app_saku3_transaksi_bpr_fIFreim.extend(window.childForm);
window.app_saku3_transaksi_bpr_fIFreim.implement({		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {						
						...
						//if status = "TUTUP"
						sql.add("update if_m set flag_aktif='1' where nik='"+this.cb_nik.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.e_periode.getText().substr(0,4)+"'");											
					}					

					if (this.c_status.getText() == "TUTUP") {
						sql.add("update if_m set flag_aktif='0' where nik='"+this.cb_nik.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.e_periode.getText().substr(0,4)+"'");					
					}
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,due_date,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','KB','IFREIM','F','0','0','"+this.cb_pp.getText()+"','"+this.dp_d1.getDateString()+"','"+this.dp_d1.getDateString()+"','-','"+this.e_ket.getText()+"','IDR',1,"+nilaiToFloat(this.e_nilai.getText())+",0,0,'"+this.cb_nik.getText()+"','-','-','-','-','-','"+this.c_status.getText()+"','"+this.akunHutIF+"','-')");
					
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',999,'"+this.akunHutIF+"','C',"+nilaiToFloat(this.e_nilai.getText())+","+nilaiToFloat(this.e_nilai.getText())+",'"+this.e_ket.getText()+"','IFREIM','BMHD','IDR',1,'"+this.app._kodePP+"','-','-','"+this.cb_nik.getText()+"','-','-','-','-','-')");	
					
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg.cells(4,i))+","+nilaiToFloat(this.sg.cells(4,i))+",'"+this.sg.cells(3,i)+"','IFREIM','BEBAN','IDR',1,'"+this.sg.cells(5,i)+"','-','-','-','-','-','-','-','-')");																						
							}
						}
					}	
					...
					sql.add("insert into pb_rek(no_bukti,kode_lokasi,modul,nama_rek,no_rek,bank,cabang,bruto,pajak,nilai) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','IFREIM','"+this.e_namarek.getText()+"','"+this.e_norek.getText()+"','"+this.e_bank.getText()+"','"+this.e_cab.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",0,"+nilaiToFloat(this.e_nilai.getText())+")");
		...
		switch (event){
			...
			case "ubah" :					
				...
				if (nilaiToFloat(this.e_nilai.getText()) > nilaiToFloat(this.e_saldo.getText())) {
					system.alert(this,"Nilai transaksi tidak valid.","Nilai tidak boleh melebihi Saldo IF.");
					return false;
				}
				...
				if (this.c_status.getText() != "TUTUP") {
					if (nilaiToFloat(this.e_nilai.getText()) == 0) {
						system.alert(this,"Nilai transaksi tidak valid.","Nilai tidak boleh nol.");
						return false;
					}
				}
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "") {
			...
			this.cb_nik.setSQL("select a.nik, a.nama from karyawan a inner join if_m b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi and b.tahun='"+this.e_periode.getText().substr(0,4)+"' where b.flag_aktif='1' and a.nik='"+this.app._userLog+"' and a.kode_lokasi='"+this.app._lokasi+"'",["a.nik","a.nama"],false,["Kode","Nama"],"and","Data Pemegang",true);

			this.stsSimpan = 1; 
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti",this.app._lokasi+"-IFR"+this.e_periode.getText().substr(2,4)+".","0000"));
	...
	doChange:function(sender){		
		if (sender == this.cb_nik && this.cb_nik.getText()!="") {
			var strSQL = "select a.kode_pp,akun_hutif,a.nilai-isnull(b.reim,0) as saldo, "+
			 			 " 		 c.bank,c.cabang,c.no_rek,c.nama_rek "+
						 "from if_m a "+
						 "		inner join karyawan c on a.nik=c.nik and a.kode_lokasi=c.kode_lokasi "+

						 "		left join (  "+						 
						 "			select nik1,sum(nilai1) as reim from trans_m "+
						 "			where form='IFREIM' and kode_lokasi='"+this.app._lokasi+"' and nik1='"+this.cb_nik.getText()+"' and no_ref3='-' and no_bukti <>'"+this.e_nb.getText()+"' "+
						 "			group by nik1"+
						 "		) b on a.nik=b.nik1  "+

						 "where a.flag_aktif='1' and a.nik='"+this.cb_nik.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.tahun='"+this.e_periode.getText().substr(0,4)+"'";						   
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){							
					this.cb_pp.setText(line.kode_pp);
					this.akunHutIF = line.akun_hutif;	
			...
			if (sender == this.sg) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Akun",sender,undefined, 
							...
							"		inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '034' "+							
							"	where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"') a where a.kode_akun like '%' ",
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){							
							if (this.preView == "1") {
								this.nama_report="server_report_saku3_bpr_rptIfPtg";
	...
	doLoad3:function(sender){																								
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai1,a.progress "+
		             "from trans_m a inner join if_m b on a.nik1=b.nik and a.kode_lokasi=b.kode_lokasi and b.tahun=substring(a.periode,1,4) and b.flag_aktif='1' "+					 					 
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_bukti,line.tgl,line.keterangan,floatToNilai(line.nilai1),line.progress]); 
		}
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.*, c.bank,c.cabang,c.no_rek,c.nama_rek  "+
							 "from trans_m a inner join karyawan c on a.nik1=c.nik and a.kode_lokasi=c.kode_lokasi "+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){								
						...
						this.cb_nik.setSQL("select a.nik, a.nama from karyawan a inner join if_m b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi and b.tahun='"+this.e_periode.getText().substr(0,4)+"' where a.nik='"+line.nik1+"' and a.kode_lokasi='"+this.app._lokasi+"'",["a.nik","a.nama"],false,["Kode","Nama"],"and","Data Pemegang",true);
						this.cb_nik.setText(line.nik1);
						this.cb_pp.setText(line.kode_pp);
						this.e_ket.setText(line.keterangan);																				
						this.c_status.setText(line.param1);		
				...
				var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp "+
							...
							"where a.jenis<>'BMHD' and a.no_bukti = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.nu",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
