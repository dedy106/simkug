window.app_saku3_transaksi_piutang_fLoadPeserta = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_piutang_fLoadPeserta.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_piutang_fLoadPeserta";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Load Data Peserta (+Master)", 0);	
...
window.app_saku3_transaksi_piutang_fLoadPeserta.extend(window.portalui_childForm);
window.app_saku3_transaksi_piutang_fLoadPeserta.implement({
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "simpan" :		
					...
					this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'yk_peserta_m','no_load',this.app._lokasi+"-LPS"+this.e_periode.getText().substr(2,4)+".",'0000'));
					if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
						try{
							...
							sql.add("insert into dbsimkug.dbo.yk_peserta_m(no_load,kode_lokasi,periode,tanggal,keterangan,nik_user,tgl_input) values "+ 
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._userLog+"',getdate())");
							...
							for (var i=0; i < this.dataUpload.rows.length;i++){
								...
								sql.add("insert into yk_peserta_d(no_load,kode_lokasi,nik,nama,band,loker) values "+
										"	('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+line.nik+"','"+line.nama.substr(0,50)+"','"+line.band+"','"+line.loker+"')");
