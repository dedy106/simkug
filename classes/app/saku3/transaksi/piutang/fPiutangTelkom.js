window.app_saku3_transaksi_piutang_fPiutangTelkom = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_piutang_fPiutangTelkom.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_piutang_fPiutangTelkom";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penyelesain Piutang Telkom: Input", 0);	
...
window.app_saku3_transaksi_piutang_fPiutangTelkom.extend(window.childForm);
window.app_saku3_transaksi_piutang_fPiutangTelkom.implement({
	...
	doLoadData:function(sender){						  
		var data = this.dbLib.getDataProvider(
				   ...
				   "where b.jenis<>'AKRU' and b.no_piutang='-' and b.no_selesai <> '-' and c.jenis = 'PEGAWAI' and a.periode <= '"+this.e_periode.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' and a.modul in ('BILTERIMA','TMTERIMA') "+ 
				   ...
				   "                   inner join yk_loker bb on b.loker=bb.loker "+
				   "                   inner join cust c on bb.kode_cust=c.kode_cust and c.kode_lokasi='"+this.app._lokasi+"' "+ 
				   "where b.jenis<>'AKRU' and b.no_piutang='-' and b.no_selesai <> '-' and c.jenis = 'PEGAWAI' and a.periode <= '"+this.e_periode.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' and a.modul in ('HUTKES','BAREV') "+
				   ...
				   "                   inner join yk_loker bb on b.loker=bb.loker "+
				   "                   inner join cust c on bb.kode_cust=c.kode_cust and c.kode_lokasi='"+this.app._lokasi+"' "+ 
				   "where b.jenis<>'AKRU' and b.no_piutang='-' and b.no_selesai <> '-' and c.jenis = 'PEGAWAI' and a.periode <= '"+this.e_periode.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' and a.modul in ('HUTKES','PDPT','PDPTREV','BAREV','KAPITASI') "+
	...
	doJurnal:function(sender){				
		...
		var strSQL = "select a.kode_akun,b.nama as nama_akun,a.dc,'"+ket+"' as ket,a.nilai,a.jenis "+
					...
					"( "+					
							...
							"where a.nilai>0 and b.jenis = 'PEGAWAI' and a.no_hutang+a.loker+a.no_selesai in ("+nobukti+") and a.kode_lokasi='"+this.app._lokasi+"' "+							
							...
							"where b.jenis = 'PEGAWAI' and (a.umum+a.gigi+a.kbia+a.matkes) > 0 and a.no_hutang+a.loker+a.no_selesai in ("+nokunj+") and a.kode_lokasi='"+this.app._lokasi+"' "+							
