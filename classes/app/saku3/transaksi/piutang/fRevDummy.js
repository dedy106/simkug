window.app_saku3_transaksi_piutang_fRevDummy = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_piutang_fRevDummy.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_piutang_fRevDummy";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Reverse Akru Hutang Dummy: Input", 0);	
...
window.app_saku3_transaksi_piutang_fRevDummy.extend(window.childForm);
window.app_saku3_transaksi_piutang_fRevDummy.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from dbtm.dbo.yk_bill_d where no_bill='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
						sql.add("delete from angg_r where no_bukti ='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
						sql.add("delete from dbtm.dbo.angg_r where no_bukti ='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
					...
					sql.add("insert into yk_rekon_m(no_rekon,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,tgl_input,nik_user) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','-','REKONDUMMY','HUT','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"','F',getdate(),'"+this.app._userLog+"')");	
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){								
								sql.add("insert into yk_rekon_j(no_rekon,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg2.cells(0,i)+"','"+this.sg2.cells(3,i)+"','"+this.sg2.cells(2,i)+"',"+parseNilai(this.sg2.cells(4,i))+",'"+this.kodepp+"','"+this.sg2.cells(6,i)+"','"+this.app._lokasi+"','REKONDUMMY','"+this.sg2.cells(5,i)+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");
					...
					//mirror ke dbtm
					sql.add("insert into dbtm.dbo.angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) "+
							"select a.no_bukti,a.modul,a.kode_lokasi,b.akun_tm,a.kode_pp,a.kode_drk,a.periode1,a.periode2,a.dc,a.saldo,a.nilai "+
							"from angg_r a inner join xakuntm b on a.kode_akun=b.akun_yakes "+
					...
					/*
					sql.add("insert into yk_billkunj_d(no_bill,no_urut,kode_lokasi,no_ref,nik,nama,loker,tgl_masuk,band,nikkes,pasien,dokter,kode_produk,kode_keg,no_rujuk,umum,gigi,kbia,matkes,cs,jenis,periode,no_kas,no_piutang,no_tak,no_selesai,no_hutang) "+
							"select '"+this.e_nb.getText()+"',no_urut,kode_lokasi,no_ref,nik,nama,loker,tgl_masuk,band,nikkes,pasien,dokter,kode_produk,kode_keg,no_rujuk,-umum,-gigi,-kbia,-matkes,-cs,jenis,'"+this.e_periode.getText()+"',no_kas,no_piutang,no_tak,no_selesai,no_hutang "+
							"from yk_billkunj_d where no_hutang in ("+nobukti+")  and kode_lokasi ='"+this.app._lokasi+"'");
					*/		
					setTipeButton(tbAllFalse);					
	...
	doTampilClick:function(sender){		
		if (this.e_periode.getText() != "") {			
			...
			var data = this.dbLib.getDataProvider("select a.no_hutang,c.kode_vendor,c.nama as nama_vendor,convert(varchar,b.tanggal,103) as tgl,b.keterangan,sum(a.nilai_bp) as nilai_bp,sum(a.nilai_cc) as nilai_cc,sum(a.nilai_bp+a.nilai_cc) as total "+
			           ...
			           "where b.progress='1' and b.modul='HUTDUMMY' and a.no_rekon='-' and b.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' "+
	...
	doLoad4:function(sender){																				
		var strSQL = "select a.no_rekon,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
		             "from yk_rekon_m a "+					 
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.posted='F' and modul='REKONDUMMY' ";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU4 = data;
