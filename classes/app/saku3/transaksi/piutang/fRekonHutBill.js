window.app_saku3_transaksi_piutang_fRekonHutBill = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_piutang_fRekonHutBill.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_piutang_fRekonHutBill";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Rekon Penyelesaian Hutang: Input", 0);	
...
window.app_saku3_transaksi_piutang_fRekonHutBill.extend(window.childForm);
window.app_saku3_transaksi_piutang_fRekonHutBill.implement({
	...
	doJurnalClick:function(sender){		
		...
		var strSQL = "select c.flag as kode_akun,d.nama as nama_akun,'C' as dc,'MUTASI KEWAJIBAN' as ket,sum(a.nilai_bp+a.nilai_cc) as nilai,'TAK' as jenis "+
					 ...
					 "union "+					 
					 "select c.bp_hut as kode_akun,d.nama as nama_akun,'D' as dc,'HUTANG PEGAWAI' as ket,sum(a.nilai_bp) as nilai,'PEGAWAI' as jenis  "+
					 "from yk_hutang_d a "+					 
					 ...
					 "where a.no_rekon='-' and a.no_hutang+a.bank_trans+a.no_kas in ("+nobukti+") and a.nilai_bp > 0 "+
					 "group by c.bp_hut,d.nama "+
					 "union "+					 
					 "select c.cc_hut as kode_akun,d.nama as nama_akun,'D' as dc,'HUTANG PENSIUN' as ket,sum(a.nilai_cc) as nilai,'PENSIUN' as jenis  "+
					 "from yk_hutang_d a "+					 
					 "     inner join vendor b on a.kode_vendor = b.kode_vendor and a.kode_lokasi=b.kode_lokasi "+
					 "     inner join vendor_klp c on b.kode_klpvendor = c.kode_klpvendor and c.kode_lokasi=b.kode_lokasi "+
					 "     inner join masakun d on c.cc_hut = d.kode_akun and c.kode_lokasi=d.kode_lokasi "+
					 "where a.no_rekon='-' and a.no_hutang+a.bank_trans+a.no_kas in ("+nobukti+") and a.nilai_cc > 0 "+
					 "group by c.cc_hut,d.nama "+					 
					"order by dc desc";
