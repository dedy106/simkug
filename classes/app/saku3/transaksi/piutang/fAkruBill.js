window.app_saku3_transaksi_piutang_fAkruBill = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_piutang_fAkruBill.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_piutang_fAkruBill";
...
window.app_saku3_transaksi_piutang_fAkruBill.extend(window.childForm);
window.app_saku3_transaksi_piutang_fAkruBill.implement({
	...
	doJurnal:function(sender){		
		try {
			...
			var strSQL = "select d.nama as ket,"+
						...
						"sum(a.umum+a.gigi+a.kbia) as nilai,'PDPTKUNJUM' as jenis, c.drk_kunj as kode_drk,e.nama as nama_drk "+
						"from yk_billkunj_d a "+
						"                     inner join yk_loker bb on a.loker=bb.loker "+
						"				      inner join cust b on bb.kode_cust=b.kode_cust "+
						"		 	          inner join yk_kunj c on a.kode_produk=c.kode_produk "+
						...
						"sum(a.matkes) as nilai,'PDPTKUNJUM' as jenis, c.drk_kunj as kode_drk,e.nama as nama_drk "+
						"from yk_billkunj_d a "+
						"                     inner join yk_loker bb on a.loker=bb.loker "+
						"				      inner join cust b on bb.kode_cust=b.kode_cust "+
						"		 	          inner join yk_kunj c on a.kode_produk=c.kode_produk "+
