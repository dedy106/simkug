window.app_saku3_transaksi_spj_fSpjDokApp = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spj_fSpjDokApp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spj_fSpjDokApp";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan SPPD: Input/Edit", 0);	
...
window.app_saku3_transaksi_spj_fSpjDokApp.extend(window.childForm);
window.app_saku3_transaksi_spj_fSpjDokApp.implement({		
		...
		if (typeof data2 == "object" && data2.rs.rows[0] != undefined){
			...
			if (this.jenisApp1.toUpperCase() != "LOS") {
				...
				if (this.jenisApp1.toUpperCase() == "BID") {		
					...
					this.cb_atas1.setSQL("select nik, nama from karyawan where nik in ('622579','660179','670241','710365') ",["nik","nama"],false,["NIK","Nama"],"and","Data Karyawan",true);			
	...
	doChange:function(sender){
		if (sender == this.cb_atas1 && this.cb_atas1.getText() != "") {
			var data2 = this.dbLib.getDataProvider("select nik,nama,jabatan from karyawan where nik='"+this.cb_atas1.getText()+"'",true);
			if (typeof data2 == "object" && data2.rs.rows[0] != undefined){
				var line2 = data2.rs.rows[0];	
				this.e_jabatas1.setText(line2.jabatan);
		...
		if (sender == this.cb_atas2 && this.cb_atas2.getText() != "") {
			var data2 = this.dbLib.getDataProvider("select nik,nama,jabatan from karyawan where nik='"+this.cb_atas2.getText()+"'",true);
			if (typeof data2 == "object" && data2.rs.rows[0] != undefined){
				var line2 = data2.rs.rows[0];			
				this.e_jabatas2.setText(line2.jabatan);
		...
		if (sender == this.cb_nik && this.stsSimpan ==1) {
			...
			if (this.cb_nik.getText() != "" && this.stsSimpan == 1) {				
				if (this.cb_nik.getText().substr(0,5) == "99999" || this.cb_nik.getText().substr(0,1) == "L") this.e_nama.setReadOnly(false);
	...
	doLoad3:function(sender){																				
		...
		var strSQL = "select a.no_spj,convert(varchar,a.tanggal,103) as tgl,modul,a.no_dokumen,a.keterangan,a.progress,a.transport+a.harian as nilai "+
					 ...
					 "where a.no_spb ='-' and "+bidang+" a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'PDLOCAL' and a.progress in ('0','R','V','P')";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
