window.app_saku3_transaksi_spj_fSpjVerPD = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_spj_fSpjVerPD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spj_fSpjVerPD";
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:9,tag:0,
		            colTitle:["No PD","Status","Dasar PD","Maksud/Tujuan","NIK SPJ","Tgl Input","Nilai","No SPB","PP User"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[80,100,100,110,150,200,200,80,100]],
					colFormat:[[6],[cfNilai]],
		...
		this.e_pp = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,11,450,20],caption:"PP User", readOnly:true});		
		this.e_nb = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,12,200,20],caption:"No Verifikasi", readOnly:true});						
		this.e_nobukti = new saiLabelEdit(this.pc1.childPage[1],{bound:[520,12,230,20],caption:"No PD", readOnly:true});		
		...
		this.pc2 = new pageControl(this.pc1.childPage[1],{bound:[1,12,995,275], childPage:["Data Transport","Uang Harian"]});
		...
		this.sg2 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:8,tag:0,
...
window.app_saku3_transaksi_spj_fSpjVerPD.extend(window.childForm);
window.app_saku3_transaksi_spj_fSpjVerPD.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						//tidak di pakai --->sql.add("update yk_spj_app set progress='1',no_ver='-',no_spb='-' where no_spb='"+this.e_nb2.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add("update yk_spj_m set progress='2',no_ver='"+this.e_nb.getText()+"',no_spb='"+this.e_nb2.getText()+"' where no_spj='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					//tidak di pakai --->sql.add("update yk_spj_app set progress='2',no_ver='"+this.e_nb.getText()+"',no_spb='"+this.e_nb2.getText()+"' where no_app='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add("insert into spb_m(no_spb,no_ver,no_bukti,kode_lokasi,periode,nik_user,tgl_input,tanggal,due_date,keterangan,nik_buat,nik_sah,nik_fiat,nik_bdh,lok_bayar,no_fiat,no_kas,nilai,modul,progress,kode_ppasal) values "+
							"('"+this.e_nb2.getText()+"','"+this.e_nb.getText()+"','"+this.e_nobukti.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.dp_d1.getDateString()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._userLog+"','"+this.cb_sah.getText()+"','"+this.cb_fiat.getText()+"','"+this.cb_bdh.getText()+"','"+this.app._lokasi+"','-','-',"+nilaiToFloat(this.e_total.getText())+",'PD_SPB','0','"+this.kodePPAsal+"')");										
					sql.add("insert into spb_d(no_spb,kode_lokasi,kode_vendor,bank,cabang,no_rek,nama_rek,nilai,keterangan,kode_lokvendor) "+
							...
							"from yk_spj_m a inner join karyawan b on a.nik_buat=b.nik "+
					...
					sql.add("insert into spb_j(no_spb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) "+
							...
							"from yk_spj_m where no_spj='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg.cells(0,row) != "") {			
				...
				this.noSPB = this.sg.cells(7,row);
				this.kodePPAsal = this.sg.cells(8,row);
				...
				this.doFiat();					
				var data = this.dbLib.getDataProvider("select kode_pp+' - '+nama as pp from pp where kode_pp='"+this.kodePPAsal+"' and kode_lokasi='"+this.app._lokasi+"'",true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){													
				...
				var strSQL = "select a.kode_trans,a.asal+'-'+a.tujuan as nama,a.nilai,a.asal,a.tujuan,a.kode_jenis,a.tarif,a.jumlah,b.nama as nama_jenis,c.no_ver "+
				...
				if (this.stsProg == "APPROVE" || this.stsProg == "REVISI") this.e_nb.setText(line.no_ver);					
	...
	doLoad:function(sender){										
		var strSQL = "select d.no_spj as no_bukti,'INPROG' as status,b.dasar,b.maksud,d.nama_spj as pembuat,convert(varchar,a.tgl_input,120) as tglinput,d.no_spb,d.transport+d.harian as nilai,d.kode_pp "+
		             "from app_m a inner join yk_spj_app b on a.no_app=b.no_app and a.kode_lokasi=b.kode_lokasi "+
					 "             inner join yk_spj_m d on b.no_app=d.no_app and d.kode_lokasi=b.kode_lokasi "+					 
					 "             inner join karyawan c on d.nik_buat=c.nik "+
					 "where d.progress='1' and d.kode_lokasi='"+this.app._lokasi+"' ";					 
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData([line.no_bukti,line.status.toUpperCase(),line.dasar,line.maksud,line.pembuat,line.tglinput,floatToNilai(line.nilai),line.no_spb,line.kode_pp]); 
		}
	...
	doCari:function(sender){										
		var filter = "";
		if (this.c_status2.getText() == "INPROG") filter = " and d.progress = '1' "; 
		if (this.c_status2.getText() == "APPROVE") filter = " and d.progress = '2' "; 
		if (this.c_status2.getText() == "REVISI") filter = " and d.progress = 'V'  ";
		if (this.e_ket2.getText()!="") filter = " and a.keterangan like '%"+this.e_ket2.getText()+"%' ";		
		
		var strSQL = "select d.no_spj as no_bukti,'"+this.c_status2.getText()+"' as status,b.dasar,b.maksud,d.nama_spj as pembuat,convert(varchar,a.tgl_input,120) as tglinput,d.no_spb,d.transport+d.harian as nilai,d.kode_pp "+
		             "from app_m a inner join yk_spj_app b on a.no_app=b.no_app and a.kode_lokasi=b.kode_lokasi "+
					 "             inner join yk_spj_m d on b.no_app=d.no_app and d.kode_lokasi=b.kode_lokasi "+					 
					 "             inner join karyawan c on d.nik_buat=c.nik "+
					 "where d.kode_lokasi='"+this.app._lokasi+"' "+filter+" order by d.no_spj";					 
