window.app_saku3_transaksi_ginas_barang_fStok = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ginas_barang_fStok.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ginas_barang_fStok";
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:7,tag:0,
		            colTitle:["Kode Barang","Nama","Satuan","Stok Sistem","Jml Fisik","Selisih","Barcode"],					
					colWidth:[[6,5,4,3,2,1,0],[100,100,100,100,100,350,100]],			
		...
		try{			
			...
			this.isiCBGudang();		
			// var strSQL = "select top 1 kode_gudang from brg_gudang where kode_pp ='"+this.app._kodePP+"' and kode_lokasi = '"+this.app._lokasi+"'";

			var strSQL = "select top 1 kode_gudang from brg_gudang where kode_lokasi = '"+this.app._lokasi+"'";
...
window.app_saku3_transaksi_ginas_barang_fStok.extend(window.portalui_childForm);
window.app_saku3_transaksi_ginas_barang_fStok.implement({
		...
		try {
			...
			for (var i=0;i < this.sg1.getRowCount();i++){
				for (var j=0;j < this.dataJU.rs.rows.length;j++){
					if ((this.dataJU.rs.rows[j].kode_barang == this.sg1.cells(0,i)) || (this.dataJU.rs.rows[j].barcode == this.sg1.cells(0,i))) {						
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "") {
			...
			this.stsSimpan = 1;
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti","OP/"+this.e_periode.getText().substr(2,4)+"/","0000"));			
			this.e_ket.setFocus();
	...
	doLoadData:	function(sender){
		if (this.cb_gudang.getText()!="") {				
			...
			var strSQL = "select a.kode_barang,a.nama,a.sat_kecil as satuan,isnull(b.stok,0) as stok, 0 as jumlah, 0 as selisih, a.barcode "+
			             "from brg_barang a "+
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData([line.kode_barang,line.nama,line.satuan,floatToNilai(line.stok),floatToNilai(line.jumlah),floatToNilai(line.selisih),line.barcode]); 
		}
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.kode_barang,b.barcode, b.nama,a.satuan,a.jumlah,b.pabrik,c.stok, case dc when 'D' then a.stok+a.jumlah else a.stok-a.jumlah end as jumlah, case dc when 'D' then -a.jumlah else a.jumlah end as selisih "+
							 ...
							 "where a.no_bukti='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.kode_barang";										
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					this.dataJU = data;							
					this.sgn.setTotalPage(Math.ceil(data.rs.rows.length/this.app._pageRow));
