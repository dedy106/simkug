window.app_saku3_transaksi_ginas_barang_fKbSetor3 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ginas_barang_fKbSetor3.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ginas_barang_fKbSetor3";
		...
		this.cb_terima = new saiCBBL(this.pc2.childPage[0],{bound:[20,17,220,20],caption:"NIK Terima", multiSelection:false, maxLength:10, tag:2});							
        this.e_total = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,17,200,20],caption:"Nilai Setoran", tag:1, tipeText:ttNilai, text:"0",readOnly:true});
        this.bTampil = new portalui_button(this.pc2.childPage[0],{bound:[650,17,80,18],caption:"Load Data",click:[this,"doLoadData"]});

        this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,18,996,213], childPage:["Data Closing Penjualan"]});			
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:5,tag:0,
		            colTitle:["No Closing","Tanggal","Saldo Closing","Nilai Setor","Copy"],					
					colWidth:[[4,3,2,1,0],[70,100,100,80,120]],
					columnReadOnly:[true,[0,1,4],[3]],
					colFormat:[[2,3,4],[cfNilai,cfNilai,cfButton]],
					click:[this,"doSgBtnClick1"], colAlign:[[3],[alCenter]],													 
					change:[this,"doChangeCells1"],dblClick:[this,"doDoubleClick1"],nilaiChange:[this,"doNilaiChange1"],
...
window.app_saku3_transaksi_ginas_barang_fKbSetor3.extend(window.childForm);
window.app_saku3_transaksi_ginas_barang_fKbSetor3.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from brg_close_bayar where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("update brg_jualpiu_d set no_str=NULL where no_str='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");
					...
					sql.add("insert into brg_setor_d (no_bukti,kode_lokasi,periode,tgl_setor,nik_setor,nik_terima,dc,keterangan,akun_kas,akun_titip,nilai,no_ref1) values "+
                            "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_nik.getText()+"','"+this.cb_terima.getText()+"','D','"+this.e_ket.getText()+"','"+this.cb_akun.getText()+"','"+this.cb_titip.getText()+"',"+nilaiToFloat(this.e_total.getText())+",'-')");	
                    				
					for (var i = 0; i < this.sg1.getRowCount();i++) {
						if (this.sg1.rowValid(i)) {
							if (nilaiToFloat(this.sg1.cells(3,i)) != 0) {
								sql.add("insert into brg_close_bayar (no_bukti,kode_lokasi,no_close,nilai) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.sg1.cells(0,i)+"',"+nilaiToFloat(this.sg1.cells(3,i))+")");
								sql.add("update brg_jualpiu_d set no_str='"+this.e_nb.getText()+"' where no_close='"+this.sg1.cells(0,i)+"' and kode_lokasi='"+this.app._lokasi+"' ");		
							}
						}
					}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :														
				...
				this.sg1.validasi();				
				for (var i = 0; i < this.sg1.getRowCount();i++){
					if (this.sg1.rowValid(i) && this.sg1.cells(3,i) != "0"){										
						if (nilaiToFloat(this.sg1.cells(2,i)) < nilaiToFloat(this.sg1.cells(3,i))) {
							var j = i+1;
							system.alert(this,"Transaksi tidak valid.","Nilai Setoran tidak boleh melebihi Saldo Closing.(Baris: "+j+")");
							return false;						
						}						
					}
				}
    ...
    doLoadData:function(sender){		
        this.sg1.clear(1);
        var strSQL =  "select a.no_close,convert(varchar,e.tanggal,103) as tgl,sum(case when a.no_jual like '04-PNJ%' then a.nilai else -a.nilai end) - isnull(b.setor,0)  as sisa "+
					  "from brg_jualpiu_d a inner join trans_m e on a.no_close=e.no_bukti and a.kode_lokasi=e.kode_lokasi "+
					  "left join (select no_close,kode_lokasi,sum(nilai) as setor "+
					  "			  from brg_close_bayar "+
					  "			  where kode_lokasi='"+this.app._lokasi+"' "+
					  "			  group by no_close,kode_lokasi) b on a.no_close = b.no_close and a.kode_lokasi=b.kode_lokasi "+
					  "where a.no_close <> '-' and a.kode_lokasi='"+this.app._lokasi+"'  "+
					  "group by a.no_close,convert(varchar,e.tanggal,103),isnull(b.setor,0) "+
					  "having  sum(a.nilai) - isnull(b.setor,0) > 0 "+
					  "order by a.no_close";		
		...
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			...
			for (var i in data.rs.rows){
				line = data.rs.rows[i];															
				this.sg1.appendData([line.no_close,line.tgl,floatToNilai(line.sisa),"0","Copy"]);
			}
		} else this.sg1.clear(1);
	},
    doChangeCells1: function(sender, col , row) {
		if (col == 2) {
			if (this.sg1.cells(2,row) != "0" && this.sg1.cells(3,row) != "") this.sg1.validasi();          
		}
		if (col == 3) {
			if (this.sg1.cells(2,row) != "0" && this.sg1.cells(3,row) != "") this.sg1.validasi();          
		}
		sender.onChange.set(this,"doChangeCells1");	
	},
	...
	doDoubleClick1: function(sender, col , row) {		
		this.sg1.cells(3,row,this.sg1.cells(2,row));
	},
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				this.sg1.clear(1);
				var strSQL =  "select a.no_close,convert(varchar,e.tanggal,103) as tgl,sum(a.nilai) - isnull(b.setor,0)  as sisa, c.nilai "+
							  "from brg_jualpiu_d a "+
							  "inner join trans_m e on a.no_close=e.no_bukti and a.kode_lokasi=e.kode_lokasi "+
							  "inner join brg_close_bayar c on a.no_close=c.no_close and a.kode_lokasi=c.kode_lokasi "+
							  "left join (select no_close,kode_lokasi,sum(nilai) as setor "+
							  "			  from brg_close_bayar "+
							  "			  where kode_lokasi='"+this.app._lokasi+"' and no_bukti <> '"+this.e_nb.getText()+"' "+
							  "			  group by no_close,kode_lokasi) b on a.no_close = b.no_close and a.kode_lokasi=b.kode_lokasi "+
							  "where a.kode_lokasi='"+this.app._lokasi+"' and c.no_bukti='"+this.e_nb.getText()+"' and c.nilai <> 0 "+
							  "group by a.no_close,convert(varchar,e.tanggal,103),isnull(b.setor,0),c.nilai "+							  
							  "order by a.no_close";	
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];															
						this.sg1.appendData([line.no_close,line.tgl,floatToNilai(line.sisa),floatToNilai(line.nilai),"Copy"]);
					}
				} else this.sg1.clear(1);
