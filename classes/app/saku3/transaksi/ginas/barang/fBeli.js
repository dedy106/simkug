window.app_saku3_transaksi_ginas_barang_fBeli = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ginas_barang_fBeli.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ginas_barang_fBeli";
		...
		try{			
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('PPNM','BELIDIS') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku3_transaksi_ginas_barang_fBeli.extend(window.portalui_childForm);
window.app_saku3_transaksi_ginas_barang_fBeli.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into brg_belihut_d(no_beli,kode_lokasi,tanggal,keterangan,kode_vendor,kode_curr,kurs,kode_pp,nilai,periode,nik_user,tgl_input,akun_hutang,nilai_ppn,no_fp,due_date, nilai_pph, diskon, modul,kode_gudang) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"', 'Pembelian Persediaan Gudang : "+this.cb_gudang.getText()+"','"+this.cb_vendor.getText()+"','IDR',1,'"+vKodePP+"',"+parseNilai(this.e_total.getText())+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.akunHutang+"',"+nilaiToFloat(this.e_ppn.getText())+",'-','"+this.dp_d1.getDateString()+"',0,"+nilaiToFloat(this.e_diskon.getText())+", 'BELI','"+this.cb_gudang.getText()+"')");
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "") {
			...
			this.stsSimpan = 1;
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti","PO/"+this.e_periode.getText().substr(2,4)+"/","0000"));
		...
		try{			
			if (col == 0){
				this.standarLib.showListData(this, "Daftar Item Barang",sender,undefined, 
											  "select kode_barang,nama,barcode from brg_barang where flag_aktif='1' and kode_lokasi='"+this.app._lokasi+"'",
											  "select count(kode_barang) from brg_barang where flag_aktif='1' and kode_lokasi='"+this.app._lokasi+"'",
											  ["kode_barang","nama","barcode"],"and",["Kode","Nama","Barcode"],false);				
			}
