window.app_saku3_transaksi_ginas_barang_fKbBeliCicil = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ginas_barang_fKbBeliCicil.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ginas_barang_fKbBeliCicil";
		...
		this.cb_akun = new saiCBBL(this.pc2.childPage[0],{bound:[20,19,220,20],caption:"Akun Pelunasan", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});									
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,20,996,305], childPage:["Data Pembelian"]});		
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-33],colCount:8,tag:0,
				colTitle:["No Referensi","Tanggal","Keterangan","Kode PP","Akun Hutang","Tagihan","Nilai Bayar","Vendor"],
				colWidth:[[7,6,5,4,3,2,1,0],[200,100,100,80,80,300,80,150]],
...
window.app_saku3_transaksi_ginas_barang_fKbBeliCicil.extend(window.childForm);
window.app_saku3_transaksi_ginas_barang_fKbBeliCicil.implement({
	doLoad : function() {
		this.pc2.setActivePage(this.pc2.childPage[0]);	
		this.pc1.setActivePage(this.pc1.childPage[0]);																		
		if (this.stsSimpan==1 ) {				
			var strSQL = "select a.no_beli,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.kode_pp,a.akun_hutang,a.nilai+a.nilai_ppn-a.diskon-isnull(b.bayar,0) as saldo, c.nama,a.tanggal "+						  
						 ...
						 "          from brg_belibayar_d where kode_lokasi='"+this.app._lokasi+"' "+
						 "			group by no_beli "+
						 ") b on a.no_beli=b.no_beli "+
						 
						 "where (a.nilai-isnull(b.bayar,0)) > 0 and a.kode_lokasi='"+this.app._lokasi+"' order by a.tanggal";						 
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg1.appendData([line.no_beli,line.tgl,line.keterangan,line.kode_pp,line.akun_hutang,floatToNilai(line.saldo),"0",line.nama]);					
				}
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',999,'"+this.cb_akun.getText()+"','C',"+nilaiToFloat(this.e_total.getText())+","+nilaiToFloat(this.e_total.getText())+",'"+this.e_ket.getText()+"','"+
					...
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && nilaiToFloat(this.sg1.cells(6,i)) > 0){							
							sql.add("insert into brg_belibayar_d(no_bukti,kode_lokasi,no_beli,kode_vendor,periode,dc,modul,nilai,nik_user,tgl_input) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.sg1.cells(0,i)+"','-', '"+this.e_periode.getText()+"','D','KBBELICCL',"+nilaiToFloat(this.sg1.cells(6,i))+",'"+this.app._userLog+"',getdate())");
							...
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
									...
									nilaiToFloat(this.sg1.cells(6,i))+",'Pembayaran atas No Ref "+this.sg1.cells(0,i)+"','"+this.modul+"','HUTANG','IDR',1,'"+this.sg1.cells(3,i)+"','-','-','-','-','"+this.sg1.cells(0,i)+"','-','-','-')");
						}
					}
	...
	doChange:function(sender){		
		if ((sender == this.e_periode || sender == this.c_jenis) && this.stsSimpan==1) {
			...
			if (this.c_jenis.getText() == "BK") {
				...
				this.modul = "MI";
				this.cb_akun.setSQL("select a.kode_akun, a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi where b.kode_flag in ('002') and a.kode_lokasi='"+this.app._lokasi+"'",
									["kode_akun","nama"],false,["Kode","Nama"],"and","Data Akun",true);									
			}
		}							
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.no_beli,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.kode_pp,a.akun_hutang,a.nilai+a.nilai_ppn-a.diskon-isnull(b.bayar,0) as saldo,c.nilai as nilai_bayar,d.nama  "+						  
							 "from brg_belihut_d a "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData([line.no_beli,line.tgl,line.keterangan,line.kode_pp,line.akun_hutang,floatToNilai(line.saldo),floatToNilai(line.nilai_bayar),line.nama]);					
					}
				} else this.sg1.clear(1);
			}						
		} catch(e) {alert(e);}
