window.app_saku3_transaksi_ginas_barang_fKbSetor = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ginas_barang_fKbSetor.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ginas_barang_fKbSetor";
		...
		this.cb_titip = new saiCBBL(this.pc2.childPage[0],{bound:[20,17,220,20],caption:"Akun Titipan", maxLength:10, tag:2});							
		...
		this.e_total = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,17,200,20],caption:"Nilai Setoran", tag:1, tipeText:ttNilai, text:"0"});			
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('TTPSTR') and kode_lokasi = '"+this.app._lokasi+"'",true);			
			if (typeof data == "object"){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];	
					if (line.kode_spro == "TTPSTR") this.cb_titip.setText(line.flag);
				}
			}
...
window.app_saku3_transaksi_ginas_barang_fKbSetor.extend(window.childForm);
window.app_saku3_transaksi_ginas_barang_fKbSetor.implement({	
