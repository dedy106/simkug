window.app_saku3_transaksi_spro_fNeracaKasLokasi = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spro_fNeracaKasLokasi.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_spro_fNeracaKasLokasi";
...
window.app_saku3_transaksi_spro_fNeracaKasLokasi.extend(window.portalui_childForm);
window.app_saku3_transaksi_spro_fNeracaKasLokasi.implement({
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
						...
						try
						{
							...
							var sql2,sql1 = "delete from neracakas where modul = '"+modul+"'  and kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";					// kode_lokasi,
							var sql = new server_util_arrayList();
							sql.add("delete from relakunkas  where  kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
								"and kode_neraca in (select distinct kode_neraca from neracakas where modul = '"+modul+"'  and kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi='"+this.app._lokasi+"')");
							sql.add(sql1);							
							//if mysql langsung														
		...
		{ 
		 	if (sender == this.relBtn)
			{
				...
				if (this.selectedItem.data.get("tipe").toLowerCase() == "posting"){
					uses("app_saku3_transaksi_spro_fRelakunKasLokasi",true);
					if (this.relakun != undefined)
						this.relakun.free();
					this.relakun = new app_saku3_transaksi_spro_fRelakunKasLokasi(this.app);
