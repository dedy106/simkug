window.app_saku3_transaksi_spro_fNeracaKas = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spro_fNeracaKas.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_spro_fNeracaKas";
		...
		this.eKlp = new portalui_saiCBBL(this,{bound:[20, 11, 200, 20], caption:"Versi Arus Kas", multiSelection:false, 
			sql:["select kode_fs, nama from fskas where kode_lokasi = '"+this.app._lokasi+"' ", ["kode_fs","nama"], false, ["Kode fskas","Nama"],"and","Daftar Versi Laporan",true]
		});					
		this.e0 = new portalui_saiCB(this,{bound:[20,12,200,20], caption:"Tipe Arus Kas", items:["CF"]});						
...
window.app_saku3_transaksi_spro_fNeracaKas.extend(window.portalui_childForm);
window.app_saku3_transaksi_spro_fNeracaKas.implement({
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
						...
						try
						{
							...
							var sql2,sql1 = "delete from neracakas where modul = '"+modul+"'  and kode_fs = '"+this.eKlp.getText()+"' ";					// kode_lokasi,
							var sql = new server_util_arrayList();
							sql.add("delete from relakunkas  where  kode_fs = '"+this.eKlp.getText()+"' "+
								"and kode_neraca in (select distinct kode_neraca from neracakas where modul = '"+modul+"'  and kode_fs = '"+this.eKlp.getText()+"' )");
							sql.add(sql1);							
							//if mysql langsung														
							var data = value.split("#");
							for (var i in data){
								sql2 = "insert into neracakas(kode_neraca, kode_lokasi, kode_fs, nama, level_spasi, level_lap, tipe, sum_header, jenis_akun, kode_induk, rowindex, modul) values ";
								sql2 += " "+ data[i] +" ";
								sql.add(sql2);
							}	
							data = this.value2.split("#");
							for (var i in data){
								sql3 = "insert into relakunkas(kode_neraca, kode_fs, kode_akun, kode_lokasi) values ";
								if (data[i]) {
									sql3 += " "+ data[i] +" ;";
									sql.add(sql3);					
								}
							...
							sql.add("insert into neracakas(kode_neraca, kode_lokasi, kode_fs, nama, level_spasi, level_lap, tipe, sum_header, jenis_akun, kode_induk, rowindex, modul) "+
								...
								" from neracakas a cross join lokasi b  where a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_lokasi <> '"+this.app._lokasi+"' and a.modul = '"+modul+"' ");
							
							sql.add("insert into relakunkas(kode_neraca, kode_fs, kode_akun, kode_lokasi) "+
								" select distinct a.kode_neraca, a.kode_fs, a.kode_akun, b.kode_lokasi "+
								" from relakunkas a cross join lokasi b  where a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_lokasi <> '"+this.app._lokasi+"' "+
								"and kode_neraca in (select distinct kode_neraca from neracakas where modul = '"+modul+"'  and kode_fs = '"+this.eKlp.getText()+"' )");
	...
	loadMenu: function(daftarNrc){
		try
		{
			...
			var item = undefined;
			var node = undefined;
			var line, ix = -1, nrcRls, lineData, nrc_d = this.dblib.loadQuery("select kode_neraca, kode_akun from relakunkas where kode_lokasi ='"+this.elokasi.getText()+"' and kode_fs = '"+this.eKlp.getText()+"' order by kode_neraca");															
	...
	doClick: function(sender){
		if (this.e0.getText() != "")
		{
			...
			//var sql="select * from neracakas where Modul = '"+modul+"' and kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"'  order by rowindex";
			//alert(sql);
			var nrc = this.dblib.runSQL("select * from neracakas where Modul = '"+modul+"' and kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"'  order by rowindex",0,0);
		...
		{ 
		 	if (sender == this.relBtn)
			{
				...
				if (this.selectedItem.data.get("tipe").toLowerCase() == "posting"){
					uses("app_saku3_transaksi_spro_fRelakunKas",true);
					if (this.relakun != undefined)
						this.relakun.free();
					this.relakun = new app_saku3_transaksi_spro_fRelakunKas(this.app);
			...
			}else
			{
				if (sender == this.addBtn || sender == this.editBtn){
					uses("app_saku2_transaksi_kopeg_sju_fNeracaDetailKas");
					if (this.entryMenu == undefined)
						this.entryMenu = new app_saku2_transaksi_kopeg_sju_fNeracaDetailKas(this.app,this.e0.getText());
	...
	doFindKlp: function(sender){	
		this.standarLib.showListDataFromItems(this, "Data Versi neracakas",this.eKlp, 
												  "select kode_fs, nama from fskas where kode_lokasi = '"+this.elokasi.getText()+"' ",
												  "select count(*) from fskas where kode_lokasi = '"+this.elokasi.getText()+"' ",
											  ["kode_fs","nama"],"and",["Kode fskas","Nama"]);	
	},
