window.app_saku3_transaksi_spro_fFaAkun = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spro_fFaAkun.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spro_fFaAkun";
		...
		try {
			...
			this.cb_deprs.setSQL("select a.kode_akun,a.nama from masakun a where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'",["kode_akun","nama"],false,["Kode","Nama"],"and","Data Akun",true);			
...
window.app_saku3_transaksi_spro_fFaAkun.extend(window.childForm);
window.app_saku3_transaksi_spro_fFaAkun.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into fa_klpakun(kode_klpakun,nama,umur,persen,kode_akun,akun_bp,akun_deprs,kode_agg,tahun,kode_drk) values "+
							"('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"',"+umurBln+","+nilaiToFloat(this.e_persen.getText())+",'"+this.cb_akun.getText()+"','"+this.cb_bp.getText()+"','"+this.cb_deprs.getText()+"','-','-','-')");									
					setTipeButton(tbAllFalse);
	...
	doChange: function(sender){
		try{
			...
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select kode_klpakun,nama,umur,persen,kode_akun,akun_bp,akun_deprs,kode_agg,tahun,kode_drk "+
				             "from fa_klpakun where kode_klpakun ='"+this.cb_kode.getText()+"' ";						   				
