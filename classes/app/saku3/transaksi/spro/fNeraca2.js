window.app_saku3_transaksi_spro_fNeraca2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spro_fNeraca2.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_spro_fNeraca2";
...
window.app_saku3_transaksi_spro_fNeraca2.extend(window.portalui_childForm);
window.app_saku3_transaksi_spro_fNeraca2.implement({
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
						...
						try
						{
							...
							var sql2,sql1 = "delete from neraca2 where modul = '"+modul+"'  and kode_fs = '"+this.eKlp.getText()+"' ";					// kode_lokasi,
							var sql = new server_util_arrayList();
							sql.add("delete from relakun2  where  kode_fs = '"+this.eKlp.getText()+"' "+
								"and kode_neraca in (select distinct kode_neraca from neraca2 where modul = '"+modul+"'  and kode_fs = '"+this.eKlp.getText()+"' )");
							sql.add(sql1);							
							//if mysql langsung														
							var data = value.split("#");
							for (var i in data){
								sql2 = "insert into neraca2(kode_neraca, kode_lokasi, kode_fs, nama, level_spasi, level_lap, tipe, sum_header, jenis_akun, kode_induk, rowindex, modul) values ";
								sql2 += " "+ data[i] +" ";
								sql.add(sql2);
							}	
							data = this.value2.split("#");
							for (var i in data){
								sql3 = "insert into relakun2(kode_neraca, kode_fs, kode_akun, kode_lokasi) values ";
								if (data[i]) {
									sql3 += " "+ data[i] +" ;";
									sql.add(sql3);					
								}
							...
							sql.add("insert into neraca2(kode_neraca, kode_lokasi, kode_fs, nama, level_spasi, level_lap, tipe, sum_header, jenis_akun, kode_induk, rowindex, modul) "+
								...
								" from neraca2 a cross join lokasi b  where a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_lokasi <> '"+this.app._lokasi+"' and a.modul = '"+modul+"' and a.kode_fs = '"+this.eKlp.getText()+"' ");
							
							sql.add("insert into relakun2(kode_neraca, kode_fs, kode_akun, kode_lokasi) "+
								" select distinct a.kode_neraca, a.kode_fs, a.kode_akun, b.kode_lokasi "+
								" from relakun2 a cross join lokasi b  where a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_lokasi <> '"+this.app._lokasi+"' and kode_fs = '"+this.eKlp.getText()+"' "+
	...
	loadMenu: function(daftarNrc){
		try
		{
			...
			var item = undefined;
			var node = undefined;
			var line, ix = -1, nrcRls, lineData, nrc_d = this.dblib.loadQuery("select kode_neraca, kode_akun from relakun2 where kode_lokasi ='"+this.elokasi.getText()+"' and kode_fs = '"+this.eKlp.getText()+"' order by kode_neraca");															
	...
	doClick: function(sender){
		if (this.e0.getText() != "")
		{
			...
			var nrc = this.dblib.runSQL("select * from neraca2 where Modul = '"+modul+"' and kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"'  order by rowindex",0,0);
			this.loadMenu(nrc);
		}	
	},
		...
		{ 
		 	if (sender == this.relBtn)
			{
				...
				if (this.selectedItem.data.get("tipe").toLowerCase() == "posting"){
					uses("app_saku_gl_master_fRelakun2",true);
					if (this.relakun != undefined)
						this.relakun.free();
					this.relakun = new app_saku_gl_master_fRelakun2(this.app);
			...
			}else
			{
				if (sender == this.addBtn || sender == this.editBtn){
					uses("app_saku3_transaksi_spro_fNeraca2Detail");
					if (this.entryMenu == undefined)
						this.entryMenu = new app_saku3_transaksi_spro_fNeraca2Detail(this.app,this.e0.getText());
