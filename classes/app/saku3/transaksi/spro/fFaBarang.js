window.app_saku3_transaksi_spro_fFaBarang = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spro_fFaBarang.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spro_fFaBarang";
...
window.app_saku3_transaksi_spro_fFaBarang.extend(window.childForm);
window.app_saku3_transaksi_spro_fFaBarang.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into fa_klp(kode_klpfa,nama,kode_klpakun,jenis) values "+
							"('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.cb_akun.getText()+"','"+this.c_jenis.getText().substr(0,1)+"')");
					setTipeButton(tbAllFalse);
		...
		try{
			...
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select kode_klpfa,nama,kode_klpakun,jenis "+
				             "from fa_klp where kode_klpfa ='"+this.cb_kode.getText()+"' ";						   				
