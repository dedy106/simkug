window.app_saku3_transaksi_spro_fNeracaGar = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spro_fNeracaGar.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_spro_fNeracaGar";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Setting Anggaran", 0);	
		...
		this.eKlp = new portalui_saiCBBL(this,{bound:[20,21,200,20],caption:"Versi ", multiSelection:false,change:[this,"doEditChange"]});										
		//this.e0 = new portalui_saiCB(this,{bound:[20,22,200,20],caption:"Tipe Neraca",items:["AKTIVA","PASIVA","LABARUGI","MODAL"]});		
		this.btn = new portalui_imageButton(this,{bound:[220,23,22,22], hint:"Reload",image:"icon/"+system.getThemes()+"/reload.png", click:[this,"doClick"]});
		...
		this.relBtn = new portalui_imageButton(this,{bound:[87,23,22,22],chint:"Relasi",image:"icon/"+system.getThemes()+"/relakun.png",click:[this,"entriesClick"]});		
		...
		this.rootBtn = new portalui_imageButton(this,{bound:[187,23,22,22],chint:"Deselect",image:"icon/"+system.getThemes()+"/imgSelect.png",caption:"R",click:[this,"entriesClick"]});		
		...
		this.eKlp.setSQL("select kode_fs, nama from fsgar where kode_lokasi = '"+this.app._lokasi+"' ",["kode_fs","nama"],undefined,["Kode FS","Nama"],"and","Data Versi Neraca");	
	}
};
window.app_saku3_transaksi_spro_fNeracaGar.extend(window.portalui_childForm);
window.app_saku3_transaksi_spro_fNeracaGar.implement({
			...
			switch (event)
			{
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
						...
						{
							//var modul = "B";//
							var sql2,sql1 = "delete from neracagar where kode_fs = '"+this.eKlp.getText()+"' ";					// kode_lokasi,
							var sql = new server_util_arrayList();
							sql.add("delete from relakungar  where  kode_fs = '"+this.eKlp.getText()+"' "+
								"and kode_neraca in (select distinct kode_neraca from neracagar where kode_fs = '"+this.eKlp.getText()+"' )");	
							sql.add(sql1);							
							//if mysql langsung
													
							var data = value.split("#");
							for (var i in data){
								sql2 = "insert into neracagar(kode_neraca, kode_lokasi, kode_fs, nama, level_spasi, level_lap, tipe, sum_header, jenis_akun, kode_induk, rowindex, modul) values ";
								sql2 += " "+ data[i] +" ";
								sql.add(sql2);
							}	
							data = this.value2.split("#");
							for (var i in data){
								sql3 = "insert into relakungar(kode_neraca, kode_fs, kode_akun, kode_lokasi) values ";
								if (data[i]) {
									sql3 += " "+ data[i] +" ;";
									sql.add(sql3);					
								}
							...
							sql.add("insert into neracagar(kode_neraca, kode_lokasi, kode_fs, nama, level_spasi, level_lap, tipe, sum_header, jenis_akun, kode_induk, rowindex, modul) "+
								...
								" from neracagar a cross join lokasi b  where a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_lokasi <> '"+this.app._lokasi+"' ");
							
							sql.add("insert into relakungar(kode_neraca, kode_fs, kode_akun, kode_lokasi) "+
								" select distinct a.kode_neraca, a.kode_fs, a.kode_akun, b.kode_lokasi "+
								" from relakungar a cross join lokasi b  where a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_lokasi <> '"+this.app._lokasi+"' "+
								"and kode_neraca in (select distinct kode_neraca from neracagar where kode_fs = '"+this.eKlp.getText()+"' )");
	...
	loadMenu: function(daftarNrc){
		try
		{
			...
			var item = undefined;
			var node = undefined;
			var line, ix = -1, nrcRls, lineData, nrc_d = this.dblib.loadQuery("select kode_neraca, kode_akun from relakungar where kode_lokasi ='"+this.elokasi.getText()+"' and kode_fs = '"+this.eKlp.getText()+"' order by kode_neraca");												
	...
	doClick: function(sender){
		var nrc = this.dblib.runSQL("select * from neracagar where kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"'  order by rowindex",0,0);
		this.loadMenu(nrc);
		...
		{ 
		 	if (sender == this.relBtn)
			{
				...
				if (this.selectedItem.data.get("tipe").toLowerCase() == "posting"){
					uses("app_saku3_transaksi_spro_fRelakunGar",true);
					if (this.relakun != undefined)
						this.relakun.free();
					this.relakun = new app_saku3_transaksi_spro_fRelakunGar(this.app);
			...
			}else
			{
				if (sender == this.addBtn || sender == this.editBtn){
					uses("app_saku3_transaksi_spro_fNeracaGarDetail");
					if (this.entryMenu == undefined)
						this.entryMenu = new app_saku3_transaksi_spro_fNeracaGarDetail(this.app);
	...
	doEditChange: function(sender){
		if (sender == this.elokasi){			
			this.eKlp.setText("");
			this.treev.clear();
			this.eKlp.setSQL("select kode_fs, nama from fsgar where kode_lokasi = '"+sender.getText()+"' ",["kode_fs","nama"],undefined,["Kode FS","Nama"],"and","Data Versi Neraca");	
