window.app_saku3_transaksi_spro_fRefJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spro_fRefJurnal.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spro_fRefJurnal";
...
window.app_saku3_transaksi_spro_fRefJurnal.extend(window.childForm);
window.app_saku3_transaksi_spro_fRefJurnal.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into refju_m(kode_ref,kode_lokasi,nama,debet,kredit,jenis) values ('"+
							this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.cb_debet.getText()+"','"+this.cb_kredit.getText()+"','PDPT')");
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update refju_m set nama='"+this.e_nama.getText()+"',debet='"+this.cb_debet.getText()+"',kredit='"+this.cb_kredit.getText()+"' where kode_ref = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
