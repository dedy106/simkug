window.app_saku3_transaksi_spro_fMasakunRelasiFS2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spro_fMasakunRelasiFS2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spro_fMasakunRelasiFS2";
...
window.app_saku3_transaksi_spro_fMasakunRelasiFS2.extend(window.childForm);
window.app_saku3_transaksi_spro_fMasakunRelasiFS2.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("delete from relakun2 where kode_akun = '"+this.cb_kode.getText()+"'");
							...
							if (this.sg3.getRowValidCount() > 0){
								for (var i=0;i < this.sg3.getRowCount();i++){
									if (this.sg3.rowValid(i)){								
										...
										sql.add("insert into relakun2 (kode_neraca,kode_fs,kode_akun,kode_lokasi) values ('"+this.sg3.cells(2,i)+"','"+this.sg3.cells(0,i)+"','"+this.cb_kode.getText()+"','"+line.kode_lokasi+"')");
									}
								}
							}
					...
					if (typeof data == "object"){
						...
						if (line != undefined){
							...
							var data = this.dbLib.getDataProvider("select b.kode_fs,b.nama as nama_fs,c.kode_neraca,c.nama as nama_lap "+
									   ...
									   "inner join neraca2 c on a.kode_neraca=c.kode_neraca and a.kode_fs=c.kode_fs and a.kode_lokasi=c.kode_lokasi "+
		...
		try{			
			...
			if (col == 2){
					this.standarLib.showListData(this, "Daftar Format Lap Keuangan",sender,undefined, 
												  "select kode_neraca, nama from neraca2 where kode_fs='"+this.sg3.cells(0,row)+"' and tipe = 'posting' and kode_lokasi='"+this.app._lokasi+"'",
												  "select count(kode_neraca) from neraca2 where kode_fs='"+this.sg3.cells(0,row)+"' and tipe = 'posting' and kode_lokasi='"+this.app._lokasi+"'",
