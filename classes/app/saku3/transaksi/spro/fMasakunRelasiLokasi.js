window.app_saku3_transaksi_spro_fMasakunRelasiLokasi = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spro_fMasakunRelasiLokasi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spro_fMasakunRelasiLokasi";
		...
		this.pc2 = new pageControl(this.pc1.childPage[1],{bound:[5,12,990,180], childPage:["Flag Akun","Lap Keuangan","Lap Anggaran"]});		
...
window.app_saku3_transaksi_spro_fMasakunRelasiLokasi.extend(window.childForm);
window.app_saku3_transaksi_spro_fMasakunRelasiLokasi.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("delete from masakun where kode_akun = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from relakun where kode_akun = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from relakungar where kode_akun = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from flag_relasi where kode_akun = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
											
							sql.add("insert into masakun (kode_akun,kode_lokasi,nama,modul,jenis,kode_curr,block,status_gar,normal) values "+
									"('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.c_modul.getText().substr(0,1)+"','"+this.c_jenis.getText()+"','"+this.c_curr.getText()+"','0','"+this.c_gar.getText().substring(0,1)+"','"+this.c_normal.getText().substring(0,1)+"')");
							...
							if (this.sg3.getRowValidCount() > 0){
								for (var i=0;i < this.sg3.getRowCount();i++){
									if (this.sg3.rowValid(i)){								
										sql.add("insert into relakun (kode_neraca,kode_fs,kode_akun,kode_lokasi) values ('"+this.sg3.cells(2,i)+"','"+this.sg3.cells(0,i)+"','"+this.cb_kode.getText()+"','"+this.app._lokasi+"')");
									}
								}
							}
							if (this.sg4.getRowValidCount() > 0){
								for (var i=0;i < this.sg4.getRowCount();i++){
									if (this.sg4.rowValid(i)){								
										sql.add("insert into relakungar (kode_neraca,kode_fs,kode_akun,kode_lokasi) values ('"+this.sg4.cells(2,i)+"','"+this.sg4.cells(0,i)+"','"+this.cb_kode.getText()+"','"+this.app._lokasi+"')");
									}
								}
							}
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					...
					sql.add("delete from relakungar where kode_akun = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from flag_relasi where kode_akun = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from konsol_relasi where kode_akun = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
