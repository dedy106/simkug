window.app_saku3_transaksi_spro_fRefBeban2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spro_fRefBeban2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spro_fRefBeban2";
		...
		this.c_jenis = new saiCB(this.pc1.childPage[1],{bound:[20,12,220,20],caption:"Jenis",items:["BEBAN"], readOnly:true,tag:2,change:[this,"doChange"]});		
...
window.app_saku3_transaksi_spro_fRefBeban2.extend(window.childForm);
window.app_saku3_transaksi_spro_fRefBeban2.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into refju_dual(kode_jurnal, kode_lokasi, nama, keterangan, akun_debet, akun_kredit, nilai, kode_pp, kode_drk, modul, jenis) "+
							"select a.kode_ref+'-'+b.kode_pp,a.kode_lokasi,a.nama,a.nama,a.debet,a.kredit,0,b.kode_pp,'-','KB','"+this.c_jenis.getText()+"' "+
							"from refju_m a "+
							"cross join pp b "+
							"where a.kode_lokasi='"+this.app._lokasi+"' and a.kode_ref='"+this.cb_kode.getText()+"'  and b.kode_lokasi='"+this.app._lokasi+"' ");
					setTipeButton(tbAllFalse);
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select kode_ref,nama,debet,kredit from refju_m where kode_ref ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and jenis='BEBAN' ";						   
