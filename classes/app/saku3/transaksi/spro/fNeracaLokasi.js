window.app_saku3_transaksi_spro_fNeracaLokasi = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spro_fNeracaLokasi.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_spro_fNeracaLokasi";
...
window.app_saku3_transaksi_spro_fNeracaLokasi.extend(window.portalui_childForm);
window.app_saku3_transaksi_spro_fNeracaLokasi.implement({
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
						...
						try
						{
							...
							var sql2,sql1 = "delete from neraca where modul = '"+modul+"'  and kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";					// kode_lokasi,
							var sql = new server_util_arrayList();
							sql.add("delete from relakun  where  kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
								"and kode_neraca in (select distinct kode_neraca from neraca where modul = '"+modul+"'  and kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi='"+this.app._lokasi+"')");
							sql.add(sql1);							
							//if mysql langsung														
		...
		{ 
			...
			}else
			{
				if (sender == this.addBtn || sender == this.editBtn){
					uses("app_saku3_transaksi_spro_fNeracaLokasiDetail");
					if (this.entryMenu == undefined)
						this.entryMenu = new app_saku3_transaksi_spro_fNeracaLokasiDetail(this.app,this.e0.getText());
