window.app_saku3_transaksi_spro_fBankRek = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spro_fBankRek.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spro_fBankRek";
		...
		this.cb_lokasi = new saiCBBL(this.pc1.childPage[1],{bound:[20,15,220,20],caption:"Lokasi", multiSelection:false, maxLength:10, tag:2});		
		this.c_jenis = new saiCB(this.pc1.childPage[1],{bound:[20,11,200,20],caption:"Jenis",items:["OPERASIONAL","RESTITUSI","DAKEM"], readOnly:true,tag:2});		
...
window.app_saku3_transaksi_spro_fBankRek.extend(window.childForm);
window.app_saku3_transaksi_spro_fBankRek.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into bank_rek(kode_rek,kode_lokasi,kode_akun,nama,no_rek,cabang,kode_bank,jenis) values "+
						    "('"+this.cb_kode.getText()+"','"+this.cb_lokasi.getText()+"','"+this.cb_akun.getText()+"','"+this.e_nama.getText()+"','"+this.e_norek.getText()+"','"+this.e_cabang.getText()+"','"+this.cb_bank.getText()+"','"+this.c_jenis.getText()+"')");
					setTipeButton(tbAllFalse);
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select kode_rek,kode_lokasi,kode_akun,nama,no_rek,cabang,kode_bank,jenis "+
