window.app_saku3_transaksi_spro_fRelkonsol = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spro_fRelkonsol.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spro_fRelkonsol";
		this.setTop(60);
		this.setWidth(1280);
		this.setHeight(550);
...
window.app_saku3_transaksi_spro_fRelkonsol.extend(window.portalui_childForm);
window.app_saku3_transaksi_spro_fRelkonsol.prototype.mainButtonClick = function(sender){
...
window.app_saku3_transaksi_spro_fRelkonsol.prototype.doModalResult = function(event, modalResult)
{
	switch (event)
	{
		case "clear" :
...
window.app_saku3_transaksi_spro_fRelkonsol.prototype.doFindBtnClick = function(sender, col, row){
	try{
		switch(col)
		{
			case 0 : 
		...
		systemAPI.alert("[app_saku3_transaksi_spro_fRelkonsol] : doFindBtnClick : " + e);
	}	
};
window.app_saku3_transaksi_spro_fRelkonsol.prototype.genClick = function(sender){
	try 
	{
		this.sg1.clear(); 
		setTipeButton(tbSimpan);
...
window.app_saku3_transaksi_spro_fRelkonsol.prototype.FindBtnClick = function(sender, event){
	try{
...
window.app_saku3_transaksi_spro_fRelkonsol.prototype.doRequestReady = function(sender, methodName, result){
