window.app_saku3_transaksi_spro_fKaryawanYSPT = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spro_fKaryawanYSPT.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spro_fKaryawanYSPT";
		...
		this.cb_pp = new saiCBBL(this,{bound:[20,10,200,20],caption:"Sekolah", multiSelection:false, tag:2,change:[this,"doChange"]});	
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:10,tag:9,
		            colTitle:["NIK","Nama","Grade","Jabatan","PP","Status","No Telpon","No HP","Email","Alamat"],
		...
		this.e_inis = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,13,400,20],caption:"Inisal", maxLength:10, tag:1});			
		this.e_nama = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,11,400,20],caption:"Nama", maxLength:50, tag:1});			
		this.e_foto = new saiLabelEdit(this.pc1.childPage[1], {bound:[520,11,320,20], caption:"File Foto",readOnly:true});
		this.u_foto = new uploader(this.pc1.childPage[1], {bound:[842,11,80,20],param3: "object", param4 :"server/media/",param2 :"server/tmp/", param1 : "uploadTo",
				autoSubmit:true, afterUpload: [this, "doUploadFinish"], caption:"Pilih File"} )
		this.c_band = new saiCB(this.pc1.childPage[1],{bound:[20,12,200,20],caption:"Grade",tag:2,mustCheck:false});		
		this.e_jabatan = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,13,400,20],caption:"Jabatan", maxLength:50, tag:1});			
		this.c_jenis = new saiCB(this.pc1.childPage[1],{bound:[20,24,200,20],caption:"Status",items:["PEGAWAI","GURU"], readOnly:true,tag:2});
		this.e_tel = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,15,400,20],caption:"No Telepon", maxLength:50, tag:1});	
		this.e_hp = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,16,300,20],caption:"No HandPhone", maxLength:50, tag:1});	
		this.e_email = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,17,300,20],caption:"Email", maxLength:50, tag:1});	
		...
		this.cb_ppbaru = new saiCBBL(this.pc1.childPage[1],{bound:[20,15,220,20],caption:"PP Baru", multiSelection:false, maxLength:10, tag:2});
...
window.app_saku3_transaksi_spro_fKaryawanYSPT.extend(window.childForm);
window.app_saku3_transaksi_spro_fKaryawanYSPT.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into karyawan(nik,kode_lokasi,nama,alamat,jabatan,no_telp,email,kode_pp,npwp,bank,cabang,no_rek,nama_rek,status,grade,kota,kode_pos,no_hp,flag_aktif, foto, inisial) values "+
						    "('"+this.cb_kode.getText()+"','"+this.cb_lokasi.getText()+"','"+this.e_nama.getText()+"','"+this.e_alamat.getText()+"','"+this.e_jabatan.getText()+"','"+this.e_tel.getText()+"','"+this.e_email.getText()+"','"+this.cb_pp.getText()+"','"+this.e_npwp.getText()+"','"+this.e_bank.getText()+"','"+this.e_cabang.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"','"+this.c_jenis.getText()+"','"+this.c_band.getText()+"','"+this.e_kota.getText()+"','"+this.e_pos.getText()+"','"+this.e_hp.getText()+"','"+this.c_status.getText().substr(0,1)+"','"+this.e_foto.getText()+"', '"+this.e_inis.getText()+"' )");					
		...
		try{			
			...
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select * from karyawan "+
						     "where nik ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.cb_pp.getText()+"'";						   				
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){						
						this.e_inis.setText(line.inisial);
	...
	doCari:function(sender){								
		...
		var strSQL = "select a.nik,a.nama,a.grade as band,a.jabatan,a.status,a.no_telp,a.no_hp,a.email,a.alamat,a.kode_pp+' - '+b.nama as pp "+
					 ...
					 "where "+filter+" and a.kode_pp='"+this.cb_pp.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'  order by a.nik";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
	...
	doLoad:function(sender){						
		var strSQL = "select a.nik,a.nama,a.grade as band,a.jabatan,a.status,a.no_telp,a.no_hp,a.email,a.alamat,a.kode_pp+' - '+b.nama as pp "+
		             "from karyawan a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi  "+
					 "where a.kode_pp='"+this.cb_pp.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.nik";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
			this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length/20));
