window.app_saku3_transaksi_spro_fKendaliPp = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spro_fKendaliPp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spro_fKendaliPp";
		...
		try {
			...
			this.cb_bidang.setSQL("select kode_bidang,nama from bidang where kode_lokasi='"+this.app._lokasi+"'",["kode_bidang","nama"],false,["Kode","Nama"],"where","Data Bidang",true);
...
window.app_saku3_transaksi_spro_fKendaliPp.extend(window.childForm);
window.app_saku3_transaksi_spro_fKendaliPp.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into pp(kode_pp, kode_lokasi, nama, initial, level_spasi, tipe, sum_header, kode_induk, rowindex, kode_bidang, flag_aktif) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.cb_lokasi.getText()+"','"+this.e_nama.getText()+"',  '-', '0', 'Posting', '00', '00', 1, '"+this.cb_bidang.getText()+"', '"+this.c_status.getText().substr(0,1)+"')");
					setTipeButton(tbAllFalse);
