window.app_saku3_transaksi_spro_fNeraca = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spro_fNeraca.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_spro_fNeraca";
...
window.app_saku3_transaksi_spro_fNeraca.extend(window.portalui_childForm);
window.app_saku3_transaksi_spro_fNeraca.implement({
		...
		{ 
			...
			}else
			{
				if (sender == this.addBtn || sender == this.editBtn){
					uses("app_saku3_transaksi_spro_fNeracaDetail");
					if (this.entryMenu == undefined)
						this.entryMenu = new app_saku3_transaksi_spro_fNeracaDetail(this.app,this.e0.getText());
