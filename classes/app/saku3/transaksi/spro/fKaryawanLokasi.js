window.app_saku3_transaksi_spro_fKaryawanLokasi = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spro_fKaryawanLokasi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spro_fKaryawanLokasi";
		...
		try {
			...
			this.cb_lokbaru.setSQL("select kode_lokasi,nama from lokasi where kode_lokasi='"+this.app._lokasi+"'",["kode_lokasi","nama"],false,["Kode","Nama"],"and","Data Lokasi",true);			
			this.c_band.items.clear();
			var data = this.dbLib.getDataProvider("select distinct grade as band from karyawan where kode_lokasi='"+this.app._lokasi+"' order by grade",true);
...
window.app_saku3_transaksi_spro_fKaryawanLokasi.extend(window.childForm);
window.app_saku3_transaksi_spro_fKaryawanLokasi.implement({
	...
	doCari:function(sender){								
		...
		var strSQL = "select a.nik,a.nama,a.grade as band,a.jabatan,a.status,a.no_telp,a.no_hp,a.email,a.alamat,a.kode_pp+' - '+b.nama as pp "+
					 ...
					 "where "+filter+" and a.kode_lokasi='"+this.app._lokasi+"'  order by a.nik";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
	...
	doLoad:function(sender){						
		var strSQL = "select a.nik,a.nama,a.grade as band,a.jabatan,a.status,a.no_telp,a.no_hp,a.email,a.alamat,a.kode_pp+' - '+b.nama as pp "+
		             "from karyawan a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi  "+
					 "where a.kode_lokasi='"+this.cb_lokasi.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.nik";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
			this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length/20));
