window.app_saku3_transaksi_spro_fBank = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_spro_fBank.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_spro_fBank";
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,400], childPage:["Daftar bank","Data bank","Filter Cari"]});
...
window.app_saku3_transaksi_spro_fBank.extend(window.childForm);
window.app_saku3_transaksi_spro_fBank.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into bank(kode_bank,nama) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update bank set nama='"+this.e_nama.getText()+"'"+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from bank where kode_bank = '"+this.cb_kode.getText()+"'");			
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select kode_bank,nama "+
				             "from bank "+
						     "where kode_bank ='"+this.cb_kode.getText()+"'";						   
	...
	doCari:function(sender){								
		try {
			if (this.e_kode2.getText() != "") var filter = " kode_bank like '%"+this.e_kode2.getText()+"%' ";
			else var filter = " nama like '%"+this.e_nama2.getText()+"%' ";
			
			var strSQL = "select kode_bank,nama "+
						 "from bank where "+filter+" order by kode_bank";								
			var data = this.dbLib.getDataProvider(strSQL,true);		
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_bank,line.nama]); 
		}
