window.app_saku3_transaksi_tak_fTakTerima = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_tak_fTakTerima.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tak_fTakTerima";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form TAK Terima: Input", 0);	
		...
		this.c_status = new saiCB(this.pc1.childPage[1],{bound:[20,10,202,20],caption:"Status",items:["FINAL","PB"], readOnly:true,tag:2,change:[this,"doChange"]});
		...
		this.sg1 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,205],colCount:9,tag:9,
		            colTitle:["Kode Mitra","Nama","Bank","Cabang","No Rekening","Nama Rekening","N Transfer","Pajak","Keterangan"],
		...
		this.sgt = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,130],colCount:9,tag:0,
		...
		this.sgnt = new portalui_sgNavigator(this.pc1.childPage[2],{bound:[1,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:3,grid:this.sgt});		
		
		this.sgv = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,260],colCount:9,tag:0,
		            colTitle:["Kode Akun","Nama Akun","DC","Keterangan","Nilai","Kode PP","Nama PP","Kode DRK","Nama DRK"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[150,80,150,80,100,270,50,200,80]],					
		...
		this.sg2 = new saiGrid(this.pc1.childPage[3],{bound:[1,6,this.pc1.width-5,this.pc1.height-30],colCount:9,tag:9,
		...
		this.c_status2 = new saiCB(this.pc1.childPage[4],{bound:[20,10,200,20],caption:"Status",items:["INPROG","FINAL","PB"], readOnly:true,tag:9});
		...
		try {			
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('HUTSPB','GARFREE','DOKFREE') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku3_transaksi_tak_fTakTerima.extend(window.childForm);
window.app_saku3_transaksi_tak_fTakTerima.implement({	
		...
		try{		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.sgv.getRowValidCount() > 0){
						for (var i=0;i < this.sgv.getRowCount();i++){
							if (this.sgv.rowValid(i)){
								var j=i+100;
								sql.add("insert into takterima_j(no_terima,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sgv.cells(0,i)+"','"+this.sgv.cells(3,i)+"','"+this.sgv.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sgv.cells(4,i))+",'"+this.sgv.cells(5,i)+"','"+this.sgv.cells(7,i)+"','"+this.app._lokasi+"','"+this.e_modul.getText()+"','LAWAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");										
							}
						}
					}
					...
					if (this.c_status.getText() == "PB") {						
						var duedate = this.e_duedate.getText().substr(6,4)+"-"+this.e_duedate.getText().substr(3,2)+"-"+this.e_duedate.getText().substr(0,2);
						sql.add("insert into yk_pb_m (no_pb,no_dokumen,kode_lokasi,periode,nik_user,tgl_input,tanggal,due_date,keterangan,nilai,modul,progress,kode_pp,nik_app,nik_tahu,no_hutang,no_app,no_spb,no_ver,kode_bidang,kode_loktuj) values  "+
								"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.dp_d1.getDateString()+"','"+duedate+"','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",'PBTAK','0','"+this.app._kodePP+"','"+this.cb_app.getText()+"','"+this.app._userLog+"','-','-','-','-','"+this.app._kodeBidang+"','"+this.app._lokasi+"')");
						if (this.sgv.getRowValidCount() > 0){
							for (var i=0;i < this.sgv.getRowCount();i++){
								if (this.sgv.rowValid(i)){
									if (this.sgv.cells(2,i).toUpperCase() == "C") var dc = "D"; else var dc = "C"; 
									sql.add("insert into yk_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
											"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sgv.cells(0,i)+"','"+this.sgv.cells(3,i)+"','"+dc+"',"+nilaiToFloat(this.sgv.cells(4,i))+",'"+this.sgv.cells(5,i)+"','"+this.sgv.cells(7,i)+"','"+this.app._lokasi+"','PBTAK','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");
								}
							}
						}																			
							...
							for (var i=0;i < this.sg1.getRowCount();i++){
								if (this.sg1.rowValid(i)){
									sql.add("insert into yk_pb_d(no_pb,kode_lokasi,kode_lokvendor,kode_vendor,bank,cabang,no_rek,nama_rek,nilai,pajak,keterangan) values "+
											"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.kodeLokAsal+"','"+this.sg1.cells(0,i)+"','"+this.sg1.cells(2,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(4,i)+"','"+this.sg1.cells(5,i)+"',"+nilaiToFloat(this.sg1.cells(6,i))+","+nilaiToFloat(this.sg1.cells(7,i))+",'"+this.sg1.cells(8,i)+"')");
								}
							}
						}					
					}					
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					this.sg1.clear(1); this.sg2.clear(1); this.sgt.clear(1); this.sgv.clear(1); 
					this.doClick();
					this.doLoad();					
			...
			case "simpan" :							
			case "ubah" :										
				if (this.c_status.getText() == "PB") this.sg1.setTag("0");
				else this.sg1.setTag("9");
				...
				for (var i=0;i < this.sgv.getRowCount();i++){					
					if (!this.sgv.rowValid(i)){
						...
						for (var j=0;j<this.dataAkunGar.rs.rows.length;j++){
							...
							if (line.kode_akun == this.sgv.cells(0,i) && this.sgv.cells(7,i) == "-") {		
								var k = i+1;
	...
	doChange:function(sender){				
		if (sender == this.c_status && this.c_status.getText() != "") {
			...
			if (this.c_status.getText() == "PB") {
				this.sgv.clear();
				this.sgv.appendData([this.akunHutSPB,"-","C",this.e_ket.getText(),this.e_nilai.getText(),this.app._kodePP,this.app._namaPP,"-","-"]);
				this.doChangeCell(this.sgv,0,0);
			}
		}
	},
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg.cells(0,row) != "") {							
				...
				var strSQL1 = "select a.kode_vendor,b.nama,a.bank,a.cabang,a.no_rek,a.nama_rek,a.nilai,a.pajak,a.keterangan "+
							  "from yk_takkirim_d a inner join vendor b on a.kode_vendor=b.kode_vendor and a.kode_lokasi=b.kode_lokasi "+
							  "where a.no_kirim='"+this.e_nobukti.getText()+"' and a.kode_lokasi='"+this.kodeLokAsal+"'";										
				...
				if (this.e_modul.getText() == "TAKSPB") {
					var strSQLTak = "select a.kode_akun,b.nama as nama_akun,case a.dc when 'D' then 'C' else 'D' end as dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk "+
									"from takkirim_j a inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
									"                  inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+													
									"                  left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and substring(a.periode,1,4)=d.tahun "+													
									"where a.jenis='TAK' and a.no_kirim = '"+this.e_nobukti.getText()+"' and a.kode_lokasi='"+this.kodeLokAsal+"' order by a.no_urut";
				}
	...
	doLoad:function(sender){																		
		var strSQL = "select a.no_kirim as no_bukti,'INPROG' as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_terima,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput "+
					 ...
					 "where a.progress='0' and a.kode_loktuj='"+this.app._lokasi+"' and a.modul<> 'BILKIRIM' and a.periode<='"+this.e_periode.getText()+"' "+
					 "union "+
					 "select a.no_kirim as no_bukti,'PB' as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_terima,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput "+
					 "from takkirim_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
					 "                  inner join karyawan c on a.nik_user=c.nik "+					 
					 "                  inner join yk_pb_m d on a.no_terima=d.no_pb and a.kode_loktuj=d.kode_lokasi and d.progress in ('R','V') and d.modul='PBTAK' "+
					 "                  inner join takterima_m e on a.no_terima=e.no_terima and a.kode_loktuj=e.kode_lokasi and e.posted='F' "+
					 "where a.progress='1' and a.kode_loktuj='"+this.app._lokasi+"' and a.periode<='"+this.e_periode.getText()+"' ";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
	...
	doCari:function(sender){						
		...
		if (this.c_status2.getText() == "PB") filter = " and a.progress = '1'  "; 					
		...
		if (this.c_status2.getText() == "PB") {
			var strSQL = "select a.no_kirim as no_bukti,'"+this.c_status2.getText()+"' as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_terima,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput "+
						 "from takkirim_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
						 "                  inner join karyawan c on a.nik_user=c.nik "+					 
						 "                  inner join yk_pb_m d on a.no_terima=d.no_pb and a.kode_loktuj=d.kode_lokasi and d.progress in ('0','R','V') and d.modul='PBTAK' "+
						 "                  inner join takterima_m e on a.no_terima=e.no_terima and a.kode_loktuj=e.kode_lokasi and e.posted='F' "+
						 "where a.kode_loktuj='"+this.app._lokasi+"' "+filter;
		}		
		var data = this.dbLib.getDataProvider(strSQL,true);
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){							
							if (this.preView == "1") {
								if (this.c_status.getText() == "PB" || this.c_status.getText() == "FINAL") {
	...
	doHitungGar: function(){
		...
		var nilai = total = 0;
		for (var i=0;i < this.sgv.getRowCount();i++){
			if (this.sgv.rowValid(i) && this.sgv.cells(5,i) != "-" && this.sgv.cells(7,i)!= "-"){
				...
				for (var j=0;j < this.sg2.getRowCount();j++){
					if (this.sgv.cells(0,i) == this.sg2.cells(0,j) && this.sgv.cells(5,i) == this.sg2.cells(2,j) && this.sgv.cells(7,i) == this.sg2.cells(4,j)) {
						isAda = true;
						idx = j;
						break;
					}
				...
				if (!isAda) {
					this.sg2.appendData([this.sgv.cells(0,i),this.sgv.cells(1,i),this.sgv.cells(5,i),this.sgv.cells(6,i),this.sgv.cells(7,i),this.sgv.cells(8,i),"0",floatToNilai(nilai),"0"]);
				} 
				else { 
		...
		var sls = 0;
		for (var i=0;i < this.sg2.getRowCount();i++){
			if (this.stsProg == "INPROG") var data = this.dbLib.getDataProvider("select fn_cekagg2('"+this.sg2.cells(2,i)+"','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(4,i)+"','"+this.e_periode.getText()+"') as gar ",true);
