window.app_saku3_transaksi_tak_fTakKirim = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tak_fTakKirim.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tak_fTakKirim";
		...
		this.cb_bidang = new saiCBBL(this.pc2.childPage[0],{bound:[20,15,220,20],caption:"Bidang", multiSelection:false, maxLength:10, tag:2});										
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,12,990,302], childPage:["Data Item Jurnal","Data Anggaran"]});
...
window.app_saku3_transaksi_tak_fTakKirim.extend(window.childForm);
window.app_saku3_transaksi_tak_fTakKirim.implement({	
		...
		switch (event){
			...
			case "ubah" :															
				...
				if (this.flagDokFree == "1") {				
					var data = this.dbLib.getDataProvider("select no_kirim from kirim_m where no_kirim <>'"+this.e_nb.getText()+"' and no_dokumen='"+this.e_dok.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
					if (typeof data == "object"){
			...
			if (sender == this.sg) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Akun",sender,undefined, 
						    "select a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag in ('034','016') where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'",
							"select count(a.kode_akun) from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag in ('034','016') where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'",
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						this.cb_bidang.setText(line.ref1);						
					}
				}								
