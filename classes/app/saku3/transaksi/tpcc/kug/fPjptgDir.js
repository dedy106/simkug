window.app_saku3_transaksi_tpcc_kug_fPjptgDir = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tpcc_kug_fPjptgDir.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tpcc_kug_fPjptgDir";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,293], childPage:["Data Panjar","Item Pertanggungan","Controlling"]});
		...
		this.e_nilaikb = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,16,200,20],caption:"Nilai KasBank", tag:9, tipeText:ttNilai, text:"0", readOnly:true, change:[this,"doChange"]});				
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:9,tag:9,
				...
				colWidth:[[8,7,6,5,4,3,2,1,0],[150,80,150,80,100,250,50,180,80]],
...
window.app_saku3_transaksi_tpcc_kug_fPjptgDir.extend(window.childForm);
window.app_saku3_transaksi_tpcc_kug_fPjptgDir.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					var sql = new server_util_arrayList();								
					if (this.stsSimpan == 0) {
						sql.add("delete from trans_m where no_bukti = '"+this.noJurnalLama+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from trans_j where no_bukti = '"+this.noJurnalLama+"' and kode_lokasi='"+this.app._lokasi+"'");						
						...
						sql.add("delete from angg_r where no_bukti = '"+this.noJurnalLama+"' and kode_lokasi='"+this.app._lokasi+"'");						
					}
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										...
										parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(3,i)+"','KB','BEBAN','IDR',1,'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','-','-','-','-','-','-','-')");										
							}
						}
					}
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.nbJurnal+"','KB','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(1,i)+"','"+this.sg2.cells(2,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(3,i))+","+nilai+")");
							}
						}
					}
	...
	doChange:function(sender){
		try {
			if (sender == this.cb_buat && this.cb_buat.getText()!="" && this.stsSimpan==1) 
				this.cb_pj.setSQL("select no_pj, keterangan from panjar_m where progress = '2' and modul='PJDIR' and nik_pengaju='"+this.cb_buat.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_pj","keterangan"],false,["No Panjar","Keterangan"],"and","Data Panjar",true);
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_ref1 as no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan "+
		             "from trans_m a "+					 					 
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider(
						   ...
						   " 			 g.param3 as jenis,g.no_ref2 as no_kas,g.param1 as akun_kb "+
						   "from ptg_m a "+
						   ...
						   "where g.no_ref1='"+this.e_nb.getText()+"' and g.kode_lokasi='"+this.app._lokasi+"'",true);
							 

				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){	
						if (line.no_kas == "-" || line.no_kas == "") this.noJurnalLama = this.e_nb.getText();
						else this.noJurnalLama = line.no_kas;
				...
				var data = this.dbLib.getDataProvider(
							...
							"where a.jenis = 'BEBAN' and a.no_bukti in ('"+this.e_nb.getText()+"','"+this.e_kb.getText()+"') and a.kode_lokasi='"+this.app._lokasi+"' order by a.nu",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
