window.app_saku3_transaksi_tpcc_kug_fMI = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tpcc_kug_fMI.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tpcc_kug_fMI";
		...
		this.sg3 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:6,tag:9,
		            colTitle:["No Bukti","Tanggal","Jenis","Deskripsi","Nilai","Pilih"],
					colWidth:[[5,4,3,2,1,0],[70,100,300,100,100,100]],									
		...
		this.c_status = new saiCB(this.pc2.childPage[0],{bound:[790,12,200,20],caption:"Status",items:["POSTING","PARKING"], readOnly:true,tag:2});
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,996,305], childPage:["Item Jurnal","Controlling"]});
...
window.app_saku3_transaksi_tpcc_kug_fMI.extend(window.childForm);
window.app_saku3_transaksi_tpcc_kug_fMI.implement({			
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','MI','MI','"+vPosted+"','-','-','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','"+this.e_dok2.getText()+"','"+this.e_ket.getText()+"','IDR',1,"+
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','MI','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(1,i)+"','"+this.sg2.cells(2,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(3,i))+","+nilai+")");
							}
						}
					}
	...
	doCari:function(sender){	
		...
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.param3 as jenis,a.keterangan,a.nilai1 "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'MI' and a.posted ='"+vPosted+"' and a.form='MI' " +filter;				
	...
	doLoad3:function(sender){				
		...
		var strSQL = "select distinct a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.param3 as jenis,a.keterangan,a.nilai1 "+
		             "from trans_m a inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.nik='"+this.app._userLog+"' "+					 					 
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'MI' and a.posted ='"+vPosted+"' and a.form='MI'";							 
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_bukti,line.tgl,line.jenis,line.keterangan,line.nilai1,"Pilih"]); 
		}
