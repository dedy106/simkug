window.app_saku3_transaksi_tpcc_kug_fGarRRA = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tpcc_kug_fGarRRA.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tpcc_kug_fGarRRA";
		...
		this.sgG = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:7,tag:9,
		            colTitle:["Kode Akun","Kode PP","Kode DRK","Bulan","Saldo Awal","Nilai","Saldo Akhir"],
		...
		try {
			...
			this.cb_akunT.setSQL("select a.kode_akun,a.nama from masakun a where a.kode_lokasi='"+this.app._lokasi+"' and a.status_gar='0' ",["a.kode_akun","a.nama"],false,["Kode","Nama"],"and","Data Akun",true);					
...
window.app_saku3_transaksi_tpcc_kug_fGarRRA.extend(window.childForm);
window.app_saku3_transaksi_tpcc_kug_fGarRRA.implement({
	...
	simpan: function(){			
		try{						
			...
			for (var i=0;i < this.sgG.getRowCount();i++){
				if (nilaiToFloat(this.sgG.cells(5,i))>0 && nilaiToFloat(this.sgG.cells(4,i)) < nilaiToFloat(this.sgG.cells(5,i))) {
					var k =i+1;
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into rra_pdrk_d(no_pdrk,kode_lokasi,no_urut,kode_akun,kode_pp,kode_drk,periode,saldo,nilai,dc,target) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',999,'"+this.cb_akunT.getText()+"','"+this.cb_ppT.getText()+"','"+this.cb_drkT.getText()+"','"+periode+"',0,"+nilaiToFloat(this.e_terima.getText())+",'D','-')");
	...
	doSelectDate: function(sender, y,m,d){
		...
		if (this.stsSimpan == 1) {
			...
			var sql = new server_util_arrayList(); 
			sql.add("select a.kode_akun,a.nama from masakun a inner join anggaran_d b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.periode like '"+this.e_periode.getText().substr(0,4)+"%' where a.status_gar='0' and a.block='0' and a.kode_lokasi='"+this.app._lokasi+"'");
	...
	doChange:function(sender){																		
		try{
			if (sender == this.c_bulanT || sender == this.cb_ppT || sender == this.cb_akunT ||  sender == this.cb_drkT ) {				
				this.e_saldo.setText("0");
				if (this.cb_akunT.getText()!="" && this.cb_ppT.getText()!="" && this.c_bulanT.getText()!="") {					
					var data = this.dbLib.getDataProvider("select fn_garakunppdrk('"+this.cb_akunT.getText()+"','"+this.cb_ppT.getText()+"','"+this.cb_drkT.getText()+"','"+this.e_periode.getText().substr(0,4)+this.c_bulanT.getText()+"','"+this.app._lokasi+"','"+this.e_nb.getText()+"') as gar ",true);
					if (typeof data == "object" && data.rs.rows[0] != undefined){
						var line = data.rs.rows[0];
						data = line.gar.split(";");
	...
	doHitungGar: function(){
		...
		for (var i=0;i < this.sg.getRowCount();i++){
			if (this.sg.rowValid(i)){
				...
				for (var j=0;j < this.sgG.getRowCount();j++){
					if (this.sg.cells(0,i) == this.sgG.cells(0,j) && this.sg.cells(2,i) == this.sgG.cells(1,j) && this.sg.cells(4,i) == this.sgG.cells(2,j) && this.sg.cells(6,i) == this.sgG.cells(3,j) ) {
						isAda = true;
						idx = j;
						break;
					}
				...
				if (!isAda) {
					this.sgG.appendData([this.sg.cells(0,i),this.sg.cells(2,i),this.sg.cells(4,i),this.sg.cells(6,i),"0",floatToNilai(nilai),"0"]);
				} 
				else { 
					total = nilaiToFloat(this.sgG.cells(5,idx));
					total = total + nilai;
					this.sgG.setCell(5,idx,total);
				}
			}
		}
		...
		for (var i=0;i < this.sgG.getRowCount();i++){					
			if (this.sgG.rowValid(i)){
				var data = this.dbLib.getDataProvider("select fn_garakunppdrk('"+this.sgG.cells(0,i)+"','"+this.sgG.cells(1,i)+"','"+this.sgG.cells(2,i)+"','"+this.e_periode.getText().substr(0,4)+this.sgG.cells(3,i)+"','"+this.app._lokasi+"','"+this.e_nb.getText()+"') as gar ",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line = data.rs.rows[0];
					data = line.gar.split(";");
					sls = parseFloat(data[0]) - parseFloat(data[1]);
					this.sgG.cells(4,i,floatToNilai(sls));
					sls = sls - nilaiToFloat(this.sgG.cells(5,i));
	...
	doChangeCell: function(sender, col, row){
		...
		if (col == 0 || col == 2 || col == 4 || col == 6) {
			if (sender.cells(0,row) != "" && sender.cells(2,row) != "" && sender.cells(4,row) != "" && sender.cells(6,row) != "") {
				...
				var data = this.dbLib.getDataProvider("select fn_garakunppdrk('"+this.sg.cells(0,row)+"','"+this.sg.cells(2,row)+"','"+this.sg.cells(4,row)+"','"+this.e_periode.getText().substr(0,4)+this.sg.cells(6,row)+"','"+this.app._lokasi+"','"+this.e_nb.getText()+"') as gar ",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line = data.rs.rows[0];
					data = line.gar.split(";");
					sls = parseFloat(data[0]) - parseFloat(data[1]);
			...
			if (sender == this.sg) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Mata Anggaran",sender,undefined, 
												  "select distinct a.kode_akun,a.nama from masakun a inner join anggaran_d b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.periode like '"+this.e_periode.getText().substr(0,4)+"%' where a.status_gar ='0' and a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'",
												  "select count(*) from (select distinct a.kode_akun,a.nama from masakun a inner join anggaran_d b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.periode like '"+this.e_periode.getText().substr(0,4)+"%' where a.status_gar ='0' and a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"') a",
	...
	doLoad3:function(sender){																						
		var strSQL = "select a.no_pdrk,convert(varchar,a.tanggal,103) as tgl,b.no_dokumen,a.keterangan,a.progress "+
					 ...
					 "where b.tahun='"+this.e_periode.getText().substr(0,4)+"' and a.modul = 'MULTI' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress in ('0','R') order by a.tanggal";		
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select substring(a.periode,5,2) as bulan,a.kode_pp,b.nama as nama_pp,a.kode_akun,c.nama as nama_akun,a.kode_drk,d.nama as nama_drk,a.nilai "+
