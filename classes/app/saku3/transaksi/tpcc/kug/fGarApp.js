window.app_saku3_transaksi_tpcc_kug_fGarApp = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tpcc_kug_fGarApp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tpcc_kug_fGarApp";
		...
		this.sg2 = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:9,
					colTitle:["Bulan","Kode Akun","Nama Akun","Kode PP","Nama PP","Kode DRK","Nama DRK","Jenis","Nilai"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[100,60,200,100,150,60,200,80,60]],
...
window.app_saku3_transaksi_tpcc_kug_fGarApp.extend(window.childForm);
window.app_saku3_transaksi_tpcc_kug_fGarApp.implement({
	...
	doDoubleClick: function(sender, col , row) {
		if (this.sg.cells(1,row) != "") {
			...
			this.sg2.clear();
			var strSQL = "select substring(a.periode,5,2) as bulan,a.kode_pp,b.nama as nama_pp,a.kode_akun,c.nama as nama_akun,a.kode_drk,d.nama as nama_drk, "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg2.appendData([line.bulan,line.kode_akun,line.nama_akun,line.kode_pp,line.nama_pp,line.kode_drk,line.nama_drk,line.dc.toUpperCase(),floatToNilai(line.nilai)]);
				}
