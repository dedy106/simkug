window.app_saku3_transaksi_tpcc_bill_fCust = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tpcc_bill_fCust.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tpcc_bill_fCust";
		...
		this.e_div = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,19,500,20],caption:"Divisi", maxLength:50, tag:1});			
...
window.app_saku3_transaksi_tpcc_bill_fCust.extend(window.childForm);
window.app_saku3_transaksi_tpcc_bill_fCust.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into cust(kode_cust,kode_lokasi,nama,alamat,no_tel,email,npwp,pic,alamat2,no_fax,akun_piutang,divisi) values "+
						    "('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.e_alamat.getText()+"','"+this.e_tel.getText()+"','"+this.e_mail.getText()+"','"+this.e_npwp.getText()+"','"+this.e_pic.getText()+"','"+this.e_alamat2.getText()+"','"+this.e_fax.getText()+"','-','"+this.e_div.getText()+"')");
					setTipeButton(tbAllFalse);
