window.app_saku3_transaksi_tpcc_bill_fUnBill2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tpcc_bill_fUnBill2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tpcc_bill_fUnBill2";
		...
		this.e_nilai = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,12,200,20],caption:"Nilai", tag:1, tipeText:ttNilai, text:"0",change:[this,"doChange"], readOnly:true});
		this.cb_pp = new saiCBBL(this.pc2.childPage[0],{bound:[20,14,220,20],caption:"PP", multiSelection:false, maxLength:10, tag:2});				
		this.e_ppn = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,14,200,20],caption:"PPN", tag:1, tipeText:ttNilai, text:"0",readOnly:true,change:[this,"doChange"]});				
		this.cb_cust2 = new saiCBBL(this.pc2.childPage[0],{bound:[20,17,220,20],caption:"Cutomer", multiSelection:false, maxLength:10, tag:2});	        
        ...
        this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,15,996,328], childPage:["Data PKS","Detail Akru","Dokumen Upload"]});	        
		...
		//this.e_tgl1 = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,16,200,20],caption:"Tanggal PKS", readOnly:true});		
		//this.e_tgl2 = new saiLabelEdit(this.pc1.childPage[0],{bound:[240,16,100,20],labelWidth:0,caption:"", readOnly:true});		
		//this.e_saldo = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,17,200,20],caption:"Saldo PKS", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});
		
		this.sg2 = new portalui_saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:9,tag:9,
		            colTitle:["Kode Akun","Nama Akun","DC","Deskripsi","Nilai","Kode PP","Nama PP","Kode DRK","Nama DRK"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[150,80,150,80,100,250,60,200,80]],
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select a.flag from spro a where a.kode_spro='HUTPPN' and a.kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line = data.rs.rows[0];											
				this.akunPPN = line.flag;
			} 
			else this.akunPPN = "HUTPPN";
...
window.app_saku3_transaksi_tpcc_bill_fUnBill2.extend(window.childForm);
window.app_saku3_transaksi_tpcc_bill_fUnBill2.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into bill_m(no_bill,no_dokumen,no_ba,no_faktur,tanggal,keterangan,kode_curr,kurs,nilai,nilai_ppn,kode_cust,no_kontrak,kode_pp,nik_buat,nik_app,jabatan,kode_lokasi,periode,nik_user,tgl_input,bank,cabang,no_rek,nama_rek,draft,nama_bill,kode_rek,no_kuitansi,no_piutang,progress,no_app,modul,akun_piutang) values "+
                            "('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','IDR',1,"+nilaiToFloat(this.e_nilai.getText())+","+parseNilai(this.e_ppn.getText())+",'"+this.cb_cust2.getText()+"','-','"+this.app._kodePP+"','"+this.app._userLog+"','-','-','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'-','-','-','-','-','-','-','-','"+this.e_nb.getText()+"','X','-','UNBILL','"+this.cb_akun.getText()+"')");		
					sql.add("insert into piutang_d(no_piutang,kode_lokasi,no_dokumen,tanggal,keterangan,kode_project,kode_cust,kode_curr,kurs,kode_pp,nilai,periode,nik_user,tgl_input,akun_piutang,nilai_ppn,nilai_pph,no_fp,modul) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','-','"+this.cb_cust2.getText()+"','IDR',1,'"+this.cb_pp.getText()+"',"+parseNilai(this.e_nilai.getText())+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.cb_akun.getText()+"',"+parseNilai(this.e_ppn.getText())+",0,'-','UNBILL')");		
		
					//7/1/21-rudi --> posted X (asalnya F)
                    sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','AR','UNBILL','F','-','-','"+this.cb_pp.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','IDR',1,"+
                            parseNilai(this.e_nilai.getText())+","+parseNilai(this.e_ppn.getText())+",0,'-','-','-','-','-','-','"+this.cb_cust2.getText()+"','"+this.cb_akun.getText()+"','-')");                    
                    sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							...
							parseNilai(this.e_total.getText())+",'"+this.e_ket.getText()+"','AR','PIUTANG','IDR',1,'"+this.cb_pp.getText()+"','-','"+this.cb_cust2.getText()+"','-','-','-','-','-','-')");					
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){	
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
                                        ...
                                        parseNilai(this.sg2.cells(4,i))+",'"+this.sg2.cells(3,i)+"','AR','PDPT','IDR',1,'"+this.sg2.cells(5,i)+"','"+this.sg2.cells(7,i)+"','-','-','-','-','-','-','-')");	                                
							}
						}
					}
					...
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.akunPPN+"','C',"+parseNilai(this.e_ppn.getText())+","+
							parseNilai(this.e_ppn.getText())+",'"+this.e_ket.getText()+"','AR','HUTPPN','IDR',1,'"+this.cb_pp.getText()+"','-','"+this.cb_cust2.getText()+"','-','-','-','-','-','-')");					
	...
	doChange:function(sender){
		...
		/*
		if (sender == this.cb_cust2 && this.cb_cust2.getText() != "") {
			this.cb_kontrak.setSQL("select a.no_kontrak, a.no_dok "+
								   "from kontrak_m a "+
								   "	 left join ("+
								   "     	select kode_lokasi,no_kontrak,sum(nilai+nilai_ppn) as bill "+
								   ...
								   "where (a.nilai-isnull(b.bill,0)) > 0 and a.kode_cust='"+this.cb_cust2.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"'",["no_kontrak","no_dok"],false,["ID","No Dokumen"],"and","Data PKS",true);														
		}

		if (sender == this.cb_kontrak && this.cb_kontrak.getText() != "") {			
			var strSQL = "select a.kode_produk,a.keterangan,convert(varchar,a.tgl_awal,103) as tgl_awal,convert(varchar,a.tgl_akhir,103) as tgl_akhir,(a.nilai-isnull(b.bill,0)) as saldo "+
						 "from kontrak_m a "+
						 "			left join ("+
						 ...
						 "				group by kode_lokasi,no_kontrak "+
						 "			) b on a.no_kontrak=b.no_kontrak and a.kode_lokasi=b.kode_lokasi "+
						 "where (a.nilai-isnull(b.bill,0)) > 0 and a.kode_cust='"+this.cb_cust2.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_kontrak='"+this.cb_kontrak.getText()+"'";
	...
	doLoad3:function(sender){			
		//7/1/21-rudi --> posted X (asalnya F)																	
		var strSQL = "select a.no_bill,convert(varchar,a.tanggal,103) as tgl,b.nama as cust,'-' as no_dok,a.keterangan,a.nilai+a.nilai_ppn as total "+
		             "from bill_m a inner join cust b on a.kode_cust=b.kode_cust and a.kode_lokasi=b.kode_lokasi "+					 
					 "				inner join trans_m d on a.no_bill=d.no_bukti and a.kode_lokasi=d.kode_lokasi and d.posted='F' "+
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.kode_akun,a.kode_pp,b.nama as nama_akun,a.dc,c.nama as nama_pp,a.keterangan,a.nilai,a.kode_drk,isnull(d.nama,'-') as nama_drk  "+
                             "from trans_j a "+
