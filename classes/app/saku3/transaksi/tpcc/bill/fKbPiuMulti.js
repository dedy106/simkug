window.app_saku3_transaksi_tpcc_bill_fKbPiuMulti = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tpcc_bill_fKbPiuMulti.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tpcc_bill_fKbPiuMulti";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pelunasan Piutang Billing [Multi]", 0);	
		...
		this.sg3 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:7,tag:9,
					...
					colWidth:[[6,5,4,3,2,1,0],[70,100,200,250,150,80,100]],
					readOnly:true,
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,20,996,292], childPage:["Data Invoice","Jurnal Sisa"]});		
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:9,tag:0,
				colTitle:["Status","No Piutang","No Bill","No FP","Tanggal","Deskripsi","Akun AR","Tagihan+","PP"],
				colWidth:[[8,7,6,5,4,3,2,1,0],[80,100,80,400,80,120,150,120,80]],
				colHide:[[3,6,8],[true,true,true]],
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:9,tag:9,
					...
					colWidth:[[8,7,6,5,4,3,2,1,0],[150,60,150,60,100,300,50,200,80]],										
...
window.app_saku3_transaksi_tpcc_bill_fKbPiuMulti.extend(window.childForm);
window.app_saku3_transaksi_tpcc_bill_fKbPiuMulti.implement({
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								...
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										...
										nilaiSlsIDR+",'"+this.sg.cells(3,i)+"','KB','SELISIH','IDR',1,'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','-','-','-','-','-','-','-')");											
							}
						}
					}					
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_cust && this.cb_cust.getText()!="" && this.stsSimpan==1 ) {			
			var strSQL = "select a.no_piutang,a.no_dokumen,a.no_fp,CONVERT(varchar,a.tanggal,103) as tanggal,a.keterangan,a.akun_piutang,a.nilai+a.nilai_ppn-ISNULL(b.bayar,0) as saldo,a.kode_pp "+
						 ...
						 "where a.modul='PIUBILL' and a.kode_cust ='"+this.cb_cust.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' and a.nilai+a.nilai_ppn-ISNULL(b.bayar,0) >0 ";
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg.appendData([line.kode_akun,line.nama_akun,line.dc,line.keterangan,floatToNilai(line.nilai_curr),line.kode_pp,line.nama_pp,line.kode_drk,line.nama_drk]);
					}
				}
				this.sg.validasi();								
			}						
