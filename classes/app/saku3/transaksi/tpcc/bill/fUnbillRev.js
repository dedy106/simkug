window.app_saku3_transaksi_tpcc_bill_fUnbillRev = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tpcc_bill_fUnbillRev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tpcc_bill_fUnbillRev";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Reverse Akru Unbill", 0);	
		...
		this.c_status = new saiCB(this.pc2.childPage[0],{bound:[780,17,200,20],caption:"Status",items:["PARKING","POSTING"], readOnly:true,tag:2});
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:8,tag:0,
					colTitle:["Status","No Bukti","Tanggal","Kontrak","Deskripsi","Akun Piutang","Ni. UnBill","Ni PPN"],
					colWidth:[[7, 6,5,4,3,2,1,0],[80,80,80,300,200,70,100,70]],					
					readOnly:true,
					buttonStyle:[[0],[bsAuto]], 
					colFormat:[[6,7],[cfNilai,cfNilai]],picklist:[[0],[new portalui_arrayMap({items:["REVERSE","UNBILL"]})]],checkItem:true,
...
window.app_saku3_transaksi_tpcc_bill_fUnbillRev.extend(window.childForm);
window.app_saku3_transaksi_tpcc_bill_fUnbillRev.implement({
	loadData: function() {		
		//bill yg belm pernah direverse
		strSQL = "select a.no_bill,convert(varchar,a.tanggal,103) as tgl,a.no_kontrak,a.keterangan,a.akun_piutang,a.nilai , a.nilai_ppn "+
				 "from bill_m a "+		
				 "	left join bill_m b on a.no_bill=b.no_dokumen and a.kode_lokasi=b.kode_lokasi and b.modul='REVUBILL' "+		 				 
				 "where a.kode_lokasi='"+this.app._lokasi+"' and a.modul ='UNBILL' and b.no_bill is null ";				 				 
		...
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			...
			for (var i in data.rs.rows){
				...
				this.sg.appendData(["UNBILL",line2.no_bill,line2.tgl,line2.no_kontrak,line2.keterangan,line2.akun_piutang,floatToNilai(line2.nilai), floatToNilai(line2.nilai_ppn)]);
			}
				...
				try{														
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i) && this.sg.cells(0,i) == "REVERSE") {															
								...
								sql.add("insert into bill_m(no_bill,no_dokumen,no_ba,no_faktur,tanggal,keterangan,kode_curr,kurs,nilai,nilai_ppn,kode_cust,no_kontrak,kode_pp,nik_buat,nik_app,jabatan,kode_lokasi,periode,nik_user,tgl_input,bank,cabang,no_rek,nama_rek,draft,nama_bill,kode_rek,no_kuitansi,no_piutang,progress,no_app,modul,akun_piutang)  "+
										"select '"+this.e_nb.getText()+"',no_bill,'-','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"',kode_curr,kurs,-nilai,-nilai_ppn,kode_cust,no_kontrak,kode_pp,'"+this.app._userLog+"','"+this.app._userLog+"','-',kode_lokasi,'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),bank,cabang,no_rek,nama_rek,draft,nama_bill,kode_rek,no_kuitansi,'"+this.e_nb.getText()+"',progress,no_app,'REVUBILL',akun_piutang "+
										"from bill_m where no_bill='"+this.sg.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"'");		
                                ...
                                sql.add("insert into piutang_d(no_piutang,kode_lokasi,no_dokumen,tanggal,keterangan,kode_project,kode_cust,kode_curr,kurs,kode_pp,nilai,periode,nik_user,tgl_input,akun_piutang,nilai_ppn,nilai_pph,no_fp,modul) "+
										"select '"+this.e_nb.getText()+"',kode_lokasi,'"+this.sg.cells(1,i)+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"',kode_project,kode_cust,kode_curr,kurs,kode_pp,-nilai,'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),akun_piutang,-nilai_ppn,0,'-','REVUBILL' "+
                                        "from piutang_d where no_piutang='"+this.sg.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
								...
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) "+
										"select '"+this.e_nb.getText()+"',kode_lokasi,getdate(),nik_user,'"+this.e_periode.getText()+"','"+this.sg.cells(1,i)+"','"+this.dp_d1.getDateString()+"',nu,kode_akun,case dc when 'D' then 'C' else 'D' end,nilai,nilai_curr,'"+this.e_ket.getText()+"','REVUBILL',jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3 "+
										"from trans_j where no_bukti='"+this.sg.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"'");											
							}
						}
					}
	...
	doDoubleClick: function(sender, col , row) {
		if (sender.cells(0,row) == "UNBILL") sender.cells(0,row,"REVERSE");
		else sender.cells(0,row,"UNBILL");
	},
	...
	doNilaiChange: function(){
		try{			
			...
			for (var i = 0; i < this.sg.rows.getLength();i++){
				if (this.sg.rowValid(i) && this.sg.cells(6,i) != "" && this.sg.cells(0,i) == "REVERSE"){										
					tot += nilaiToFloat(this.sg.cells(6,i))+nilaiToFloat(this.sg.cells(7,i));					
				}
			}						
	...
	doLoad3:function(sender){				
		...
		var strSQL = "select no_bukti,convert(varchar,tanggal,103) as tgl,keterangan,nilai1 from trans_m "+
					 "where posted='"+vPosted+"' and form='REVUBILL' and kode_lokasi='"+this.app._lokasi+"' order by no_bukti";						
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
                ...
                strSQL = "select a.no_bill,convert(varchar,a.tanggal,103) as tgl,a.no_kontrak,a.keterangan,a.akun_piutang,a.nilai,a.nilai_ppn "+
						"from bill_m a "+		
						"	  inner join bill_m b on a.no_bill=b.no_dokumen and a.kode_lokasi=b.kode_lokasi and b.modul='REVUBILL' "+		 				 
						"where a.kode_lokasi='"+this.app._lokasi+"' and a.modul ='UNBILL' and b.no_bill='"+this.e_nb.getText()+"' ";				 				 
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line2;
					this.sg.clear();
					for (var i in data.rs.rows){
						line2 = data.rs.rows[i];							
						this.sg.appendData(["REVERSE",line2.no_bill,line2.tgl,line2.no_kontrak,line2.keterangan,line2.akun_piutang,floatToNilai(line2.nilai),floatToNilai(line2.nilai_ppn)]);
					}
				} else this.sg.clear(1);
