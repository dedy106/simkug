window.app_saku3_transaksi_tpcc_bill_fUnBill = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tpcc_bill_fUnBill.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tpcc_bill_fUnBill";
		...
		this.c_status = new saiCB(this.pc2.childPage[0],{bound:[790,14,200,20],caption:"Status",items:["PARKING","POSTING"], readOnly:true,tag:2});
...
window.app_saku3_transaksi_tpcc_bill_fUnBill.extend(window.childForm);
window.app_saku3_transaksi_tpcc_bill_fUnBill.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into bill_m(no_bill,no_dokumen,no_ba,no_faktur,tanggal,keterangan,kode_curr,kurs,nilai,nilai_ppn,kode_cust,no_kontrak,kode_pp,nik_buat,nik_app,jabatan,kode_lokasi,periode,nik_user,tgl_input,bank,cabang,no_rek,nama_rek,draft,nama_bill,kode_rek,no_kuitansi,no_piutang,progress,no_app,modul,akun_piutang) values "+
                            "('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','IDR',1,"+nilaiToFloat(this.e_nilai.getText())+",0,'"+this.cb_cust2.getText()+"','"+this.cb_kontrak.getText()+"','"+this.app._kodePP+"','"+this.app._userLog+"','-','-','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'-','-','-','-','-','-','-','-','"+this.e_nb.getText()+"','X','-','UNBILL','"+this.cb_akun.getText()+"')");		
					sql.add("insert into piutang_d(no_piutang,kode_lokasi,no_dokumen,tanggal,keterangan,kode_project,kode_cust,kode_curr,kurs,kode_pp,nilai,periode,nik_user,tgl_input,akun_piutang,nilai_ppn,nilai_pph,no_fp,modul) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_kontrak.getText()+"','"+this.cb_cust2.getText()+"','IDR',1,'"+this.cb_pp.getText()+"',"+parseNilai(this.e_nilai.getText())+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.cb_akun.getText()+"',0,0,'-','UNBILL')");		
		
					if (this.c_status.getText() == "PARKING") var vPosted = "X";
					else var vPosted = "F";		
                    ...
                    for (var i=0;i < this.sgUpld.getRowCount();i++){
                        if (this.sgUpld.rowValid(i)){
                            ix++;											
                            sql.add("insert into bill_dok(no_bukti,no_gambar,nu,kode_jenis,kode_lokasi,modul,no_kontrak) values ('"+this.e_nb.getText()+"','"+this.sgUpld.cells(3,i).tmpfile+"','"+ix+"','"+this.sgUpld.cells(0,i)+"','"+this.app._lokasi+"','UNBILL','"+this.cb_kontrak.getText()+"')");
                        }	
                    }	
		...
		try{
			if (this.sg3.cells(0,row) != "") {
                ...
                var strSQL= "select b.kode_jenis,b.nama,a.no_gambar "+
                			...
                			"where a.no_kontrak = '"+this.cb_kontrak.getText()+"' and a.no_bukti='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.nu";
