window.app_saku3_transaksi_haji_fPeserta = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_haji_fPeserta.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_haji_fPeserta";
		...
		this.cb_goldar = new saiCB(this.pc1.childPage[1],{bound:[340,23,180,20],caption:"Golongan Darah",items:["A","B","O","AB"], readOnly:true,tag:2});
		...
		this.e_bank = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,13,300,20],caption:"Bank", maxLength:50, tag:1});			
		this.e_cabang = new saiLabelEdit(this.pc1.childPage[1],{bound:[530,13,300,20],caption:"Cabang", maxLength:50, tag:1});			
		this.e_norek = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,15,300,20],caption:"No Rekening", maxLength:50, tag:1});					
		this.e_namarek = new saiLabelEdit(this.pc1.childPage[1],{bound:[530,15,300,20],caption:"Nama Rekening", maxLength:50, tag:1});							
		...
		try {
			...
			this.doLoad();
			this.cb_mitra.setSQL("select kode_mitra, nama from haj_mitra where flag_aktif='1' and kode_lokasi='"+this.app._lokasi+"'",["kode_mitra","nama"],false,["Kode","Nama"],"and","Data Mitra",true);
			...
			var data = this.dbLib.getDataProvider("select distinct jenis_id as jenis from haj_peserta where kode_lokasi='"+this.app._lokasi+"' order by jenis_id",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
...
window.app_saku3_transaksi_haji_fPeserta.extend(window.childForm);
window.app_saku3_transaksi_haji_fPeserta.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into haj_peserta(no_peserta,kode_lokasi,jenis_id,id_peserta,kode_mitra,nama,ayah,ibu,tempat,tgl_lahir,jk,kode_agama,kode_goldar,alamat,no_hp,no_tel,email,kerja,foto,bank,cabang,no_rek,nama_rek) values "+
						    "('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.c_jenis.getText()+"','"+this.e_id.getText()+"','"+this.cb_mitra.getText()+"','"+this.e_nama.getText()+"','"+this.e_ayah.getText()+"','"+this.e_ibu.getText()+"','"+this.e_tempat.getText()+"','"+this.dp_d1.getDateString()+"','"+this.c_jk.getText()+"','-','"+this.cb_goldar.getText()+"','"+this.e_alamat.getText()+"','"+this.e_hp.getText()+"','"+this.e_tel.getText()+"','"+this.e_email.getText()+"','"+this.e_kerja.getText()+"','"+this.e_foto.getText()+"','"+this.e_bank.getText()+"','"+this.e_cabang.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"')");
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();					
					sql.add("delete from haj_peserta where no_peserta = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");								
					sql.add("insert into haj_peserta(no_peserta,kode_lokasi,jenis_id,id_peserta,kode_mitra,nama,ayah,ibu,tempat,tgl_lahir,jk,kode_agama,kode_goldar,alamat,no_hp,no_tel,email,kerja,foto,bank,cabang,no_rek,nama_rek) values "+
						    "('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.c_jenis.getText()+"','"+this.e_id.getText()+"','"+this.cb_mitra.getText()+"','"+this.e_nama.getText()+"','"+this.e_ayah.getText()+"','"+this.e_ibu.getText()+"','"+this.e_tempat.getText()+"','"+this.dp_d1.getDateString()+"','"+this.c_jk.getText()+"','-','"+this.cb_goldar.getText()+"','"+this.e_alamat.getText()+"','"+this.e_hp.getText()+"','"+this.e_tel.getText()+"','"+this.e_email.getText()+"','"+this.e_kerja.getText()+"','"+this.e_foto.getText()+"','"+this.e_bank.getText()+"','"+this.e_cabang.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"')");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
	...
	doClick:function(sender){
		if (sender == this.i_gen) {
			this.cb_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"haj_peserta","no_peserta",this.app._lokasi+"-"+this.tahun+".","00000"));
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select nama from haj_peserta where no_peserta ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {			
				...
				var strSQL = "select jenis_id,kode_mitra,email,foto,bank,cabang,no_rek,nama_rek from haj_peserta where no_peserta='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";			
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){							
