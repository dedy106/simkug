window.app_saku3_transaksi_investasi_fDOCtutupE = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_fDOCtutupE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_fDOCtutupE";
...
window.app_saku3_transaksi_investasi_fDOCtutupE.extend(window.childForm);
window.app_saku3_transaksi_investasi_fDOCtutupE.implement({
	...
	doChange:function(sender){		
		if (sender == this.e_periode && this.e_periode.getText() != "") {
			this.e_nb.setSQL("select no_tutup, keterangan from inv_depotutup_m where posted <> 'T' and (no_kas=no_tutup or no_kas='-') and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_tutup","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true); //((no_kas ='-' and jenis='TUTUP') or (no_kas=no_tutup and jenis='PERPANJANG'))
		}
