window.app_saku3_transaksi_investasi_fDOCcair = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_fDOCcair.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_fDOCcair";
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('BUDEPDRK') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku3_transaksi_investasi_fDOCcair.extend(window.childForm);
window.app_saku3_transaksi_investasi_fDOCcair.implement({
