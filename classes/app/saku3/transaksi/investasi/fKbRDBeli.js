window.app_saku3_transaksi_investasi_fKbRDBeli = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_fKbRDBeli.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_fKbRDBeli";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Pembelian Reksadana: Input", 0);	
		...
		this.bTampil = new button(this,{bound:[600,15,80,18],caption:"Data RD",click:[this,"doLoad"]});			
...
window.app_saku3_transaksi_investasi_fKbRDBeli.extend(window.childForm);
window.app_saku3_transaksi_investasi_fKbRDBeli.implement({
	...
	doLoad:function(sender){
		if (this.e_periode.getText() != "") {			
			var strSQL = "select a.no_rdbeli,convert(varchar,a.tanggal,103) as tgl,a.keterangan,c.nama as kelola,x.total+a.nilai_komisi+a.nilai_ppn+a.nilai_levy as nilai,a.nilai_pph,b.kode_akun as akun_hutang "+
						 ...
						 "                    inner join (select no_rdbeli,round(sum(harga*jumlah),0) as total from inv_rdbeli_d group by no_rdbeli) x on a.no_rdbeli=x.no_rdbeli "+
						 "where a.progress ='0' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' order by a.no_rdbeli";						
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData(["INPROG",line.no_rdbeli,line.tgl,line.keterangan,line.kelola,line.akun_hutang,floatToNilai(line.nilai),floatToNilai(line.nilai_pph)]);
				}
