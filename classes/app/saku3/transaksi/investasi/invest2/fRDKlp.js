window.app_saku3_transaksi_investasi_invest2_fRDKlp = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_invest2_fRDKlp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_invest2_fRDKlp";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Kelompok Reksadana", 0);	
		
		uses("saiCB;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		this.pc2 = new pageControl(this,{bound:[10,10,1000,440], childPage:["List Kelompok","Data Kelompok"]});		
		...
		this.cb_nt = new saiCBBL(this.pc2.childPage[1],{bound:[20,15,220,20],caption:"Akun Naik Turun", multiSelection:false, maxLength:10, tag:2});						
...
window.app_saku3_transaksi_investasi_invest2_fRDKlp.extend(window.childForm);
window.app_saku3_transaksi_investasi_invest2_fRDKlp.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into inv_rdklp(kode_rdklp,nama,akun_rd,akun_spi,akun_gl,akun_nt,akun_dev,status_dana) values "+					
							"('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.cb_rd.getText()+"','"+this.cb_spi.getText()+"','"+this.cb_gl.getText()+"','"+this.cb_nt.getText()+"','"+this.cb_dev.getText()+"','"+this.c_status.getText()+"')");					
					setTipeButton(tbAllFalse);
	...
	doLoad3:function(sender){														
		var strSQL = "select kode_rdklp,nama from inv_rdklp order by kode_rdklp";
		...
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			...
			for (var i=0;i<this.dataJU3.rs.rows.length;i++){
				...
				this.sg3.appendData([line.kode_rdklp,line.nama]); 
			}			
