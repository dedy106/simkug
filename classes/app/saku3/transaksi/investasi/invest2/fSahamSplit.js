window.app_saku3_transaksi_investasi_invest2_fSahamSplit = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_invest2_fSahamSplit.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_invest2_fSahamSplit";
...
window.app_saku3_transaksi_investasi_invest2_fSahamSplit.extend(window.childForm);
window.app_saku3_transaksi_investasi_invest2_fSahamSplit.implement({
	doTampil : function() {		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into inv_shmbeli_m(no_shmbeli,kode_lokasi,periode,tanggal,nik_user,tgl_input,posted,no_kas,progress,nik_buat,  no_dokumen,keterangan,kode_drk,kode_kelola,tgl_set,nilai_komisi,nilai_ppn,nilai_levy,nilai_pph,modul,nilai,no_app1,no_spb,kode_pp) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.app._userLog+"',getdate(),'X','SHMSPLIT','0','"+this.app._userLog+"','-','"+this.e_ket.getText()+"','-','"+this.cb_saham.getText()+"','"+this.dp_d1.getDateString()+"',"+nilaiToFloat(this.e_kali.getText())+",0,0,0,'SHMSPLIT',0,'-','-','"+this.kodepp+"')");
					...
					for (var i = 0; i < this.sg.rows.getLength();i++){
						if (this.sg.rowValid(i) && this.sg.cells(7,i) != "" && this.sg.cells(7,i) != "0"){							
							sql.add("insert into inv_shmbeli_d (no_shmbeli,kode_kelola,kode_saham,h_oleh,h_buku,harga,jumlah,n_beli,  komisi,vat,levi,pph,kode_broker,h_buku2) values "+
								    "('"+this.e_nb.getText()+"','"+this.sg.cells(0,i)+"','"+this.sg.cells(2,i)+"',"+nilaiToFloat(this.sg.cells(4,i))+","+nilaiToFloat(this.sg.cells(5,i))+",0,"+nilaiToFloat(this.sg.cells(7,i))+",0, 0,0,0,0,'-',0)");
						}						
					}
		...
		try{
			...
			if (this.sg3.cells(0,baris) != "") {			
				...
				var strSQL = "select a.kode_kelola,b.nama as nama_kelola,a.kode_saham,c.nama as nama_saham,a.jumlah,a.h_oleh,a.h_buku,round(a.jumlah * "+nilaiToFloat(this.e_kali.getText())+",0) - a.jumlah as tambah, round(a.jumlah * "+nilaiToFloat(this.e_kali.getText())+",0) as lbr_akhir "+
							 ...
							 "                     inner join inv_saham c on a.kode_saham=c.kode_saham "+
							 "					   inner join inv_shmbeli_d d on a.kode_kelola=d.kode_kelola and a.kode_saham=d.kode_saham "+
							 "where d.no_shmbeli='"+this.e_nb.getText()+"' and a.nik_user='"+this.nik_user+"' order by a.kode_kelola";
				var data1 = this.dbLib.getDataProvider(strSQL,true);	
				if (typeof data1 == "object" && data1.rs.rows[0] != undefined){
					var line1;
					this.sg.clear();
