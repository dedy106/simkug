window.app_saku3_transaksi_investasi_invest2_fVerSP = function(owner)
{
	...
	{		
		...
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Verifikasi Saham Penempatan", 0);	
		...
		this.pc2 = new pageControl(this.pc1.childPage[1],{bound:[1,12,995,295], childPage:["Data Saham Penyertaan"]});				
		this.sg1 = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:4,tag:0,				
				colTitle:["Kd Mitra","Nama Mitra","Persentase","Nilai"],
...
window.app_saku3_transaksi_investasi_invest2_fVerSP.implement({	
		...
		try{		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,8])){
				try{														
					...
					if (this.stsSimpan == 0) {
						sql.add("update inv_pstempat_m set progress='1',no_spb='-' where no_spb='"+this.e_nb2.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
					...
					sql.add("update inv_pstempat_m set progress='2',no_spb='"+this.e_nb2.getText()+"' where no_ps='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add("insert into spb_d(no_spb,kode_lokasi,kode_vendor,bank,cabang,no_rek,nama_rek,nilai,keterangan,kode_lokvendor) "+
							"select '"+this.e_nb2.getText()+"','"+this.app._lokasi+"',a.kode_mitra,'-','-','-',b.nama,a.nilai,a.keterangan,a.kode_lokasi "+
							"from inv_pstempat_m a inner join inv_mitra b on a.kode_mitra=b.kode_mitra "+
							"where a.no_ps='"+this.e_nobukti.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add("insert into spb_j(no_spb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) "+
							"select '"+this.e_nb2.getText()+"',no_ps,'"+this.dp_d1.getDateString()+"',0,kode_akun,keterangan,'D',nilai,kode_pp,'-',kode_lokasi,modul,jenis,'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1 "+
							"from inv_pstempat_j where jenis='HUT' and no_ps='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doDoubleClick: function(sender, col , row) {
		try{
			...
			if (this.sg.cells(0,baris) != "") {			
				...
				if (this.modul == "PSBELI") {
					strSQL1 = "select a.kode_mitra,b.nama,a.p_bunga,a.nilai "+
							 "from inv_pstempat_m a inner join inv_mitra b on a.kode_mitra=b.kode_mitra "+					 							 
							 "where a.no_ps='"+this.e_nobukti.getText()+"' ";					
					...
					if (typeof data1 == "object" && data1.rs.rows[0] != undefined){
						...
						for (var i in data1.rs.rows){
							...
							this.sg1.appendData([line1.kode_mitra,line1.nama,parseFloat(line1.p_bunga),parseFloat(line1.nilai)]);
						}
	...
	doLoad:function(sender){										
		var strSQL = "select a.modul,a.no_ps as no_bukti,'INPROG' as status,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai,a.nik_buat+' - '+d.nama as nikuser,a.no_spb as no_flag,a.tgl_input "+
					 "from inv_pstempat_m a inner join karyawan d on a.nik_buat=d.nik "+
					 "where a.periode<='"+this.e_periode.getText()+"' and a.progress = '1' and a.kode_lokasi ='"+this.app._lokasi+"' and not no_app1 is null order by a.no_ps";
		var data = this.dbLib.getDataProvider(strSQL,true);
	...
	doCari:function(sender){										
		...
		var strSQL = "select a.modul,a.no_ps as no_bukti,case when a.progress = '2' then 'APPROVE' else 'REVISI' end as status,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai,a.nik_buat+' - '+d.nama as nikuser,a.no_spb as no_flag,a.tgl_input "+
					 "from inv_pstempat_m a inner join karyawan d on a.nik_buat=d.nik "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi ='"+this.app._lokasi+"' "+filter+" order by a.no_ps";							 
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
			this.sgn.setTotalPage(Math.ceil(data.rs.rows.length/20));
