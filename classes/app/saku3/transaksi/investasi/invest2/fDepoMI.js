window.app_saku3_transaksi_investasi_invest2_fDepoMI = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_invest2_fDepoMI.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_invest2_fDepoMI";
...
window.app_saku3_transaksi_investasi_invest2_fDepoMI.extend(window.childForm);
window.app_saku3_transaksi_investasi_invest2_fDepoMI.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into inv_shop_m (no_shop,periode,nik_user,tgl_input,kode_lokasi,tanggal,keterangan,nik_app,tgl_awal,tgl_akhir,jml_hari,jml_bulan,progress,nilai,no_app,no_spb,bsumber,  nodin,kepada1,dari1,lamp1,hal1,nikttd1,jab1,  noins,nikttd2,jab2,nikttd3,jab3,just,kode_pp,modul) values "+
							"('"+this.e_nb.getText()+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.app._lokasi+"','"+this.dp_d2.getDateString()+"','"+this.e_ket.getText()+"','-','"+this.dp_d2.getDateString()+"','"+this.dp_d2.getDateString()+"',"+jmlHari+","+jmlBulan+",'0',"+nilaiToFloat(this.e_nilai.getText())+",'-','-','-', '-','-','-','-','-','-','-','-','-','-','-','-','-','"+this.app._kodePP+"','MI')");					
	...
	doDoubleClick3: function(sender, col , row) {		
		try{
			...
			if (this.sg3.cells(0,baris) != "") {
				...
				var strSQL = "select * from inv_depo2_m where no_depo= '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";
				var data = this.dbLib.getDataProvider(strSQL,true);
