window.app_saku3_transaksi_investasi_invest2_fVerSaham = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_investasi_invest2_fVerSaham.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_invest2_fVerSaham";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Verifikasi Pembelian Saham: Input", 0);	
		...
		this.pc2 = new pageControl(this.pc1.childPage[1],{bound:[1,12,995,295], childPage:["Data Saham"]});				
		this.sg1 = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:11,tag:0,				
				colTitle:["Kd Saham","Nama Saham","Harga Beli","Lbr Beli","Nilai Beli","Komisi","VAT","Levi","PPh","Broker","Nama"],
		...
		try {			
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('SHMPPH','BDH1','FIAT1','FIAT2') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku3_transaksi_investasi_invest2_fVerSaham.extend(window.childForm);
window.app_saku3_transaksi_investasi_invest2_fVerSaham.implement({	
		...
		try{		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,8])){
				try{														
					...
					if (this.stsSimpan == 0) {
						sql.add("update inv_shmbeli_m set progress='1',no_spb='-' where no_spb='"+this.e_nb2.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
					...
					sql.add("update inv_shmbeli_m set progress='2',no_spb='"+this.e_nb2.getText()+"' where no_shmbeli='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add("insert into spb_d(no_spb,kode_lokasi,kode_vendor,bank,cabang,no_rek,nama_rek,nilai,keterangan,kode_lokvendor) "+
							"select '"+this.e_nb2.getText()+"','"+this.app._lokasi+"',a.kode_kelola,'-','-','-',b.nama,a.nilai,a.keterangan,a.kode_lokasi "+
							"from inv_shmbeli_m a inner join inv_kelola b on a.kode_kelola=b.kode_kelola "+
							"where a.no_shmbeli='"+this.e_nobukti.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add("insert into spb_j(no_spb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) "+
							"select '"+this.e_nb2.getText()+"',no_shmbeli,'"+this.dp_d1.getDateString()+"',0,kode_akun,keterangan,'D',nilai,kode_pp,'-',kode_lokasi,modul,jenis,'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1 "+
							"from inv_shmbeli_j where jenis='HUT' and no_shmbeli='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					
					sql.add("insert into spb_j(no_spb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) "+
							"select '"+this.e_nb2.getText()+"',a.no_shmbeli,'"+this.dp_d1.getDateString()+"',99,'"+this.akunpph+"','PPH '+substring(b.nama,1,140) as keterangan,'C',a.pph,'"+this.app._kodePP+"','-','"+this.app._lokasi+"','SHMBELI','PPH','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1 "+
							"from inv_shmbeli_d a inner join inv_broker b on a.kode_broker=b.kode_broker "+
		...
		try{
			...
			if (this.sg.cells(0,baris) != "") {			
				...
				if (this.modul == "SHMBELI") {
					strSQL1 = "select a.kode_saham,b.nama,a.harga,a.jumlah,a.n_beli,a.komisi,a.vat,a.levi,a.pph,a.kode_broker,d.nama as nama_broker "+
	...
	doLoad:function(sender){										
		var strSQL = "select a.modul,a.no_shmbeli as no_bukti,'INPROG' as status,convert(varchar,a.tgl_set,103) as tgl,a.keterangan,a.nilai,a.nik_buat+' - '+d.nama as nikuser,a.no_spb as no_flag,a.tgl_input "+
					 "from inv_shmbeli_m a inner join karyawan d on a.nik_buat=d.nik "+
					 "where a.modul<>'SHMBONUS' and a.periode<='"+this.e_periode.getText()+"' and a.progress = '1' and a.kode_lokasi ='"+this.app._lokasi+"' and not no_app1 is null order by a.no_shmbeli";
		var data = this.dbLib.getDataProvider(strSQL,true);
	...
	doCari:function(sender){										
		...
		var strSQL = "select a.modul,a.no_shmbeli as no_bukti,case when a.progress = '2' then 'APPROVE' else 'REVISI' end as status,convert(varchar,a.tgl_set,103) as tgl,a.keterangan,a.nilai,a.nik_buat+' - '+d.nama as nikuser,a.no_spb as no_flag,a.tgl_input "+
					 "from inv_shmbeli_m a inner join karyawan d on a.nik_buat=d.nik "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi ='"+this.app._lokasi+"' "+filter+" order by a.no_shmbeli";							 
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
			this.sgn.setTotalPage(Math.ceil(data.rs.rows.length/20));
