window.app_saku3_transaksi_investasi_invest2_fReksadana = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_invest2_fReksadana.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_invest2_fReksadana";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Reksadana", 0);	
		
		uses("saiCB;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		this.pc2 = new pageControl(this,{bound:[10,10,1000,440], childPage:["List Reksadana","Data Reksadana"]});		
		...
		this.cb_kelola = new saiCBBL(this.pc2.childPage[1],{bound:[20,12,220,20],caption:"Manajer Investasi", multiSelection:false, maxLength:10, tag:2});				
...
window.app_saku3_transaksi_investasi_invest2_fReksadana.extend(window.childForm);
window.app_saku3_transaksi_investasi_invest2_fReksadana.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into inv_rd(kode_rd,nama,kode_rdklp,kode_rdkelola,flag_aktif) values "+
						    "('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.cb_klp.getText()+"','"+this.cb_kelola.getText()+"','"+this.c_aktif.getText().substr(0,1)+"')");					
					setTipeButton(tbAllFalse);
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					...
					sql.add("update inv_rd "+
							"set nama='"+this.e_nama.getText()+"',kode_rdklp='"+this.cb_klp.getText()+"',kode_rdkelola='"+this.cb_kelola.getText()+"',flag_aktif='"+this.c_aktif.getText().substr(0,1)+"'  "+
	...
	doLoad3:function(sender){														
		...
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			...
			for (var i=0;i<this.dataJU3.rs.rows.length;i++){
				...
				this.sg3.appendData([line.kode_rd,line.nama,line.aktif.toUpperCase()]); 
			}			
