window.app_saku3_transaksi_investasi_invest2_fAppRD = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_investasi_invest2_fAppRD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_invest2_fAppRD";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Approval Reksadana", 0);	
		...
		this.sg1 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,212],colCount:9,tag:0,				
				colTitle:["Kd RD","Nama RD","Harga","Jml Lbr","Nilai","Komisi","VAT","Levi","PPh"],
				colWidth:[[8,7,6,5,4,3,2,1,0],[80,80,80,80,110,60,100,240,80]],
		...
		this.c_modul2 = new saiCB(this.pc1.childPage[3],{bound:[20,11,200,20],caption:"Modul",items:["RDBELI","RDJUAL","RDDEV","RDSPI"], readOnly:true,tag:9,change:[this,"doChange"]});
...
window.app_saku3_transaksi_investasi_invest2_fAppRD.extend(window.childForm);
window.app_saku3_transaksi_investasi_invest2_fAppRD.implement({	
		...
		try{														
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					//---------------- flag bukti					
					if (this.e_modul.getText() == "RDBELI") sql.add("update inv_rdbeli_m set no_app1='"+this.e_nb.getText()+"',progress='"+vStatus+"' where no_rdbeli='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					if (this.e_modul.getText() == "RDSPI") sql.add("update inv_rdspi_m set no_app1='"+this.e_nb.getText()+"',progress='"+vStatus+"' where no_spi='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "hapus" :	
				...
				if (this.e_modul.getText() == "RDBELI") sql.add("update inv_rdbeli_m set no_app1='-',progress='0' where no_rdbeli='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");				
				...
				if (this.e_modul.getText() == "RDSPI") sql.add("update inv_rdspi_m set no_app1='-',progress='0' where no_spi='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doChange:function(sender){				
		if (sender == this.c_modul2 && this.e_periode.getText() != "") {
			if (this.c_modul2.getText() == "RDBELI") this.cb_nb.setSQL("select no_rdbeli, keterangan from inv_rdbeli_m where modul='RDBELI' and periode='"+this.e_periode.getText()+"' and progress in ('1','M') and kode_lokasi='"+this.app._lokasi+"'",["no_rdbeli","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);				
			...
			if (this.c_modul2.getText() == "RDSPI") this.cb_nb.setSQL("select no_spi, keterangan from inv_rdspi_m where modul='RDSPI' and periode='"+this.e_periode.getText()+"' and progress in ('1','M') and kode_lokasi='"+this.app._lokasi+"'",["no_spi","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);							
		}
		if (sender == this.cb_nb && this.cb_nb.getText() != "") {
			if (this.c_modul2.getText() == "RDBELI") {					
				var strSQL = "select a.tgl_set as due_date,a.no_rdbeli as no_bukti,case a.progress when '1' then 'APPROVE' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.tgl_set,103) as tgl2,a.modul,a.keterangan,a.nilai,a.no_app1,convert(varchar,a.tgl_input,120) as tglinput,b.nik+' - '+b.nama as buat "+
							 "from inv_rdbeli_m a inner join karyawan b on a.nik_buat=b.nik "+					 
							 "where a.no_rdbeli='"+this.cb_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' ";
			...
			if (this.c_modul2.getText() == "RDDEV") {					
				var strSQL = "select a.tanggal as due_date,a.no_rddev as no_bukti,case a.progress when '1' then 'APPROVE' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.tanggal,103) as tgl2,a.modul,a.keterangan,a.nilai,a.no_app1,convert(varchar,a.tgl_input,120) as tglinput,b.nik+' - '+b.nama as buat "+
							 ...
							 "where a.no_rddev='"+this.cb_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' ";			 
	...
	doClick:function(sender){		
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"inv_app2_m","no_app",this.app._lokasi+"-RDM"+this.e_periode.getText().substr(2,4)+".","0000"));												
	...
	doLoadData:function(){
		...
		if (this.e_modul.getText() == "RDSPI") {
			var strSQL1 = "select d.kode_rd,b.nama,(d.h_wajar-d.h_buku) * d.jumlah as nilai "+
						 "from inv_rdspi_d d "+						 
						 "     inner join inv_rd b on d.kode_rd=b.kode_rd "+						 
	...
	doLoadGar:function(){			
		if (this.e_modul.getText() == "RDBELI" || this.e_modul.getText() == "RDJUAL" || this.e_modul.getText() == "RDDEV") {
	...
	doLoad:function(sender){																		
		var strSQL = "select a.tgl_set as due_date,a.no_rdbeli as no_bukti,'INPROG' as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.tgl_set,103) as tgl2,a.modul,a.keterangan,a.nilai,a.no_app1,convert(varchar,a.tgl_input,120) as tglinput,b.nik+' - '+b.nama as buat "+
		             "from inv_rdbeli_m a inner join karyawan b on a.nik_buat=b.nik "+					 
