window.app_saku3_transaksi_investasi_invest2_fKbDepoMI = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_invest2_fKbDepoMI.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_invest2_fKbDepoMI";
		...
		this.c_jenis = new saiCB(this.pc2.childPage[0],{bound:[20,12,202,20],caption:"Jenis",items:["BK"], readOnly:true,tag:2,visible:false});
		...
		this.cb_bank = new saiCBBL(this.pc2.childPage[0],{bound:[20,18,222,20],caption:"Bank Sumber", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});				
		...
		this.sg = new saiGrid(this.p1,{bound:[1,20,this.p1.width-5,this.p1.height-50],colCount:9,tag:0,				
				colTitle:["Status","No Deposito","Tanggal","Keterangan","Bank Depo","Akun Deposito","Nilai","Jenis","Status"],
				colWidth:[[8,7,6,5,4,3,2,1,0],[80,80,100,80,80,250,80,100,80]],
...
window.app_saku3_transaksi_investasi_invest2_fKbDepoMI.extend(window.childForm);
window.app_saku3_transaksi_investasi_invest2_fKbDepoMI.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','-','-','"+this.akunKB+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBDOC2','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.nikMan+"',getdate(),'"+this.app._userLog+"','F','-','"+this.cb_bank.getText()+"','-','"+this.kodeRek+"')");					
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.akunKB+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBDOC2','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'"+this.kodeRek+"')");
	...
	doChange:function(sender){		
		if (sender == this.cb_bank && this.cb_bank.getText()!="") {
			var data = this.dbLib.getDataProvider("select kode_rek,kode_akun from inv_bank where kode_bank='"+this.cb_bank.getText()+"'",true);
			if (typeof data == "object"){
	...
	doLoad:function(sender){
		if (this.e_periode.getText() != "" && this.cb_bank.getText()!="") {
			var strSQL = "select a.no_depo,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.bdepo,a.akun_depo,a.nilai,a.jenis,a.status_dana "+
			             "from inv_depo2_m a inner join inv_shop_m b on a.no_depo=b.no_shop "+
						 "where b.modul='MI' and a.bsumber='"+this.cb_bank.getText()+"' and a.progress ='0' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' order by a.jenis,a.no_depo";
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData(["INPROG",line.no_depo,line.tgl,line.keterangan,line.bdepo,line.akun_depo,floatToNilai(line.nilai),line.jenis,line.status_dana]);
				}
			...
			if (this.sg3.cells(0,baris) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						this.cb_bank.setText(line.no_link);						
					}
				}
				
				var strSQL = "select no_depo,convert(varchar,tanggal,103) as tgl,keterangan,bdepo,akun_depo,nilai,jenis,status_dana "+
							 "from inv_depo2_m "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg.appendData(["APP",line.no_depo,line.tgl,line.keterangan,line.bdepo,line.akun_depo,floatToNilai(line.nilai),line.jenis,line.status_dana]);
					}
				} else this.sg.clear(1);			
