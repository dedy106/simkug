window.app_saku3_transaksi_investasi_invest2_fSahamBonus = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_invest2_fSahamBonus.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_invest2_fSahamBonus";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,22,995,325], childPage:["Data Saham","Rekap Data"]});		
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:6,tag:0,				
				colTitle:["Kd Saham","Nama Saham","Lbr Unit","Harga Oleh","Harga Buku","Lbr Bonus"],
				colWidth:[[5,4,3,2,1,0],[100,100,100,60,140,80]],
...
window.app_saku3_transaksi_investasi_invest2_fSahamBonus.extend(window.childForm);
window.app_saku3_transaksi_investasi_invest2_fSahamBonus.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into inv_shmbeli_m(no_shmbeli,kode_lokasi,periode,tanggal,nik_user,tgl_input,posted,no_kas,progress,nik_buat,  no_dokumen,keterangan,kode_drk,kode_kelola,tgl_set,nilai_komisi,nilai_ppn,nilai_levy,nilai_pph,modul,nilai,no_app1,no_spb,kode_pp) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.app._userLog+"',getdate(),'X','SHMBONUS','1','"+this.app._userLog+"',  '-','"+this.e_ket.getText()+"','-','"+this.cb_kelola.getText()+"','"+this.dp_d1.getDateString()+"',0,0,0,0,'SHMBONUS',0,'SAP','-','"+this.kodepp+"')");
					...
					for (var i = 0; i < this.sg.rows.getLength();i++){
						if (this.sg.rowValid(i) && this.sg.cells(5,i) != "" && this.sg.cells(5,i) != "0"){
							sql.add("insert into inv_shmbeli_d (no_shmbeli,kode_kelola,kode_saham,h_oleh,h_buku,harga,jumlah,n_beli,  komisi,vat,levi,pph,kode_broker,h_buku2) values "+
								    "('"+this.e_nb.getText()+"','"+this.cb_kelola.getText()+"','"+this.sg.cells(0,i)+"',"+nilaiToFloat(this.sg.cells(3,i))+","+nilaiToFloat(this.sg.cells(4,i))+",0,"+nilaiToFloat(this.sg.cells(5,i))+",0, 0,0,0,0,'-',0)");
						}						
					}
	...
	doLoad3:function(sender){														
		var strSQL = "select a.no_shmbeli,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.progress,b.catatan "+
					 ...
					 "where a.progress in ('0','M') and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='SHMBONUS'";
		var data = this.dbLib.getDataProvider(strSQL,true);
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			...
			if (this.sg3.cells(0,baris) != "") {			
				...
				strSQL = "select a.kode_saham,b.nama,c.jumlah as jml_seb,a.h_oleh,a.h_buku,a.jumlah "+
				...
				if (typeof data1 == "object" && data1.rs.rows[0] != undefined){
					...
					for (var i in data1.rs.rows){
						...
						this.sg.appendData([line1.kode_saham,line1.nama,floatToNilai(line1.jml_seb),parseFloat(line1.h_oleh),parseFloat(line1.h_buku),parseFloat(line1.jumlah)]);
					}
