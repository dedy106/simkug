window.app_saku3_transaksi_investasi_invest2_fRDKelola = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_invest2_fRDKelola.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_invest2_fRDKelola";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Manager Investasi Reksadana", 0);	
		
		uses("saiCB;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		this.pc2 = new pageControl(this,{bound:[10,10,1000,440], childPage:["List MI","Data MI"]});		
...
window.app_saku3_transaksi_investasi_invest2_fRDKelola.extend(window.childForm);
window.app_saku3_transaksi_investasi_invest2_fRDKelola.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into inv_rdkelola(kode_rdkelola,nama,flag_aktif) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update inv_rdkelola set nama='"+this.e_nama.getText()+"',flag_aktif='"+this.c_aktif.getText().substr(0,1)+"'  "+
	...
	doLoad3:function(sender){														
		var strSQL = "select kode_rdkelola,nama,case flag_aktif when '0' then 'T' else 'Y' end as aktif,flag_aktif from inv_rdkelola order by flag_aktif desc,kode_rdkelola";
		...
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			...
			for (var i=0;i<this.dataJU3.rs.rows.length;i++){
				...
				this.sg3.appendData([line.kode_rdkelola,line.nama,line.aktif.toUpperCase()]); 
			}			
