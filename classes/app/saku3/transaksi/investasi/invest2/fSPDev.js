window.app_saku3_transaksi_investasi_invest2_fSPDev = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_invest2_fSPDev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_invest2_fSPDev";
...
window.app_saku3_transaksi_investasi_invest2_fSPDev.extend(window.childForm);
window.app_saku3_transaksi_investasi_invest2_fSPDev.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into inv_spdev_m(no_spdev,kode_lokasi,no_dokumen,tanggal,keterangan,kode_pp,kode_drk,kode_mitra,periode,nilai,nik_buat,tgl_input,nik_user,no_kas,progress,no_app1,modul,posted) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.kodepp+"','"+this.cb_drk.getText()+"','"+this.cb_mitra.getText()+"','"+this.e_periode.getText()+"',"+nilaiToFloat(this.e_total.getText())+",'"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','-','0','-','SPDEV','F')");								
