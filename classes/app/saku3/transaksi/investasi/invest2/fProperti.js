window.app_saku3_transaksi_investasi_invest2_fProperti = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_invest2_fProperti.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_invest2_fProperti";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Mitra", 0);	
		
		uses("saiCB;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		this.pc2 = new pageControl(this,{bound:[10,10,1000,440], childPage:["List Properti","Data Properti"]});		
		...
		this.cb_spi = new saiCBBL(this.pc2.childPage[1],{bound:[20,21,220,20],caption:"Akun Properti", multiSelection:false, maxLength:10, tag:2});
...
window.app_saku3_transaksi_investasi_invest2_fProperti.extend(window.childForm);
window.app_saku3_transaksi_investasi_invest2_fProperti.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into inv_properti(kode_properti,nama,akun_spi,flag_aktif) values "+
	...
	doLoad3:function(sender){														
		var strSQL = "select kode_properti,nama from inv_properti order by kode_properti";
		...
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			...
			for (var i=0;i<this.dataJU3.rs.rows.length;i++){
				...
				this.sg3.appendData([line.kode_properti,line.nama]); 
			}			
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){				
				var strSQL = "select * from inv_properti where kode_properti ='"+this.cb_kode.getText()+"'";						   
				var data = this.dbLib.getDataProvider(strSQL,true);
