window.app_saku3_transaksi_investasi_invest2_fRDDev = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_invest2_fRDDev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_invest2_fRDDev";
...
window.app_saku3_transaksi_investasi_invest2_fRDDev.extend(window.childForm);
window.app_saku3_transaksi_investasi_invest2_fRDDev.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into inv_rddev_m(no_rddev,kode_lokasi,no_dokumen,tanggal,keterangan,kode_pp,kode_drk,kode_rdkelola,periode,nilai,nik_buat,tgl_input,nik_user,no_kas,progress,no_app1,modul,posted) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.kodepp+"','"+this.cb_drk.getText()+"','"+this.cb_kelola.getText()+"','"+this.e_periode.getText()+"',"+nilaiToFloat(this.e_total.getText())+",'"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','-','0','-','RDDEV','F')");								
