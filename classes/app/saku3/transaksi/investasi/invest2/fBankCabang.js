window.app_saku3_transaksi_investasi_invest2_fBankCabang = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_invest2_fBankCabang.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_invest2_fBankCabang";
		...
		this.sg3 = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-30],colCount:4,tag:9,
		            colTitle:["Kode","Nama","Alamat","Kode Bank"],
					colWidth:[[3,2,1,0],[80,400,200,100]],
...
window.app_saku3_transaksi_investasi_invest2_fBankCabang.extend(window.childForm);
window.app_saku3_transaksi_investasi_invest2_fBankCabang.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					if (this.c_status.getText().substr(0,1) == "1") sql.add("update inv_bank set flag_default='0' where kode_bankklp='"+this.cb_klp.getText()+"'");					
					if (this.c_bdepo.getText().substr(0,1) == "1") sql.add("update inv_bank set flag_bdepo='0' where kode_bankklp='"+this.cb_klp.getText()+"'");					
	...
	doLoad3:function(sender){														
		...
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			...
			for (var i=0;i<this.dataJU3.rs.rows.length;i++){
				...
				this.sg3.appendData([line.kode_bank,line.nama,line.alamat,line.kode_bankklp]); 
			}			
