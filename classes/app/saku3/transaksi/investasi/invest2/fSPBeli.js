window.app_saku3_transaksi_investasi_invest2_fSPBeli = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_invest2_fSPBeli.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_invest2_fSPBeli";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,22,995,300], childPage:["Data Penyertaan","Rekap Penyertaan"]});			
...
window.app_saku3_transaksi_investasi_invest2_fSPBeli.extend(window.childForm);
window.app_saku3_transaksi_investasi_invest2_fSPBeli.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into inv_spbeli_m(no_spbeli,kode_lokasi,periode,tanggal,nik_user,tgl_input,posted,no_kas,progress,nik_buat,  no_dokumen,keterangan,kode_drk,kode_spkelola,tgl_set,nilai_komisi,nilai_ppn,nilai_levy,nilai_pph,no_app1,no_spb,kode_pp,modul,nilai) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.app._userLog+"',getdate(),'F','-','0','"+this.app._userLog+"',  '-','"+this.e_ket.getText()+"','"+this.cb_drk.getText()+"','-','"+this.dp_d2.getDateString()+"',0,0,0,0,'-','-','"+this.kodepp+"','SPBELI',"+nilaiToFloat(this.e_total.getText())+")");
					...
					sql.add("insert into inv_spbeli_j(no_spbeli,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',99,'"+this.akunHutang+"','"+this.e_ket.getText()+"','C',"+nilaiHutang+",'"+this.kodepp+"','-','"+this.app._lokasi+"','SPBELI','HUT','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");
