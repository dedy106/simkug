window.app_saku3_transaksi_investasi_fKbDOCtutup = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_fKbDOCtutup.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_fKbDOCtutup";
...
window.app_saku3_transaksi_investasi_fKbDOCtutup.extend(window.childForm);
window.app_saku3_transaksi_investasi_fKbDOCtutup.implement({
	...
	doLoad:function(sender){
		if (this.e_periode.getText() != "") {
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData(["INPROG",line.no_depo,line.tgl,line.keterangan,line.kode_bank,line.akun_doc,floatToNilai(line.nilai),floatToNilai(line.nilai_pinalti),floatToNilai(line.nilai_adm),line.no_tutup,line.modul]);
				}
