window.app_saku3_transaksi_investasi_fKbpinbuk = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_fKbpinbuk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_fKbpinbuk";
...
window.app_saku3_transaksi_investasi_fKbpinbuk.extend(window.childForm);
window.app_saku3_transaksi_investasi_fKbpinbuk.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.akunKB+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','"+this.cb_cf.getText()+"','-','"+this.app._lokasi+"','KBPB','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'"+this.kodeRek+"')");
					...
					if (this.sg2.getRowValidCount() > 0){
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
										"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sg2.cells(3,i)+"','"+this.sg2.cells(4,i)+"','D',"+nilaiToFloat(this.sg2.cells(5,i))+",'"+this.app._kodePP+"','-','"+this.cb_cf.getText()+"','-','"+this.app._lokasi+"','KBPB','PB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'"+kodeRekTuj+"')");
	...
	doChange:function(sender){
		if (sender == this.cb_bank && this.cb_bank.getText()!="") {
			var data = this.dbLib.getDataProvider("select kode_akun,kode_rek from inv_bank where kode_bank='"+this.cb_bank.getText()+"'",true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){
