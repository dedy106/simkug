window.app_saku3_transaksi_investasi_fSahamDev = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_fSahamDev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_fSahamDev";
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('DRKSHMJ','PPINV','SHMDEVMI','SHMDEVSW','SHMPPHM') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku3_transaksi_investasi_fSahamDev.extend(window.childForm);
window.app_saku3_transaksi_investasi_fSahamDev.implement({
	...
	doChange:function(sender){		
		if (sender == this.cb_saham && this.cb_saham.getText()!="") {			
			var strSQL = "select a.kode_kelola, a.nama from inv_kelola a inner join inv_saham_d b on a.kode_kelola=b.kode_kelola and b.kode_saham='"+this.cb_saham.getText()+"'";			
	...
	doClick:function(sender){
		if (sender == this.i_gen) {
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"inv_shmdev_m","no_shmdev",this.app._lokasi+"-SDEV"+this.e_periode.getText().substr(2,4)+".","000"));
			this.e_dok.setFocus();
		}		
	},		
