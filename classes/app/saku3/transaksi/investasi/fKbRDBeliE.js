window.app_saku3_transaksi_investasi_fKbRDBeliE = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_fKbRDBeliE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_fKbRDBeliE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Pembelian Reksadana: Edit", 0);	
		...
		this.p1 = new panel(this,{bound:[10,23,900,300],caption:"Daftar Pembelian Reksadana"});
		this.sg = new saiGrid(this.p1,{bound:[0,20,this.p1.width-5,this.p1.height-50],colCount:8,tag:9,				
				colTitle:["Status","No Beli","Tanggal","Keterangan","Broker","Akun Hutang","Nilai","Nilai PPh"],
...
window.app_saku3_transaksi_investasi_fKbRDBeliE.extend(window.childForm);
window.app_saku3_transaksi_investasi_fKbRDBeliE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("update inv_rdbeli_m set progress='0', no_kas ='-' where no_kas='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBRDBELI','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"',getdate(),'"+this.app._userLog+"','F','-','-','"+this.cb_cf.getText()+"','"+this.cb_rek.getText()+"')");					
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','"+this.cb_cf.getText()+"','-','"+this.app._lokasi+"','KBRDBELI','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'"+this.cb_rek.getText()+"')");																	
					if (this.sg.getRowValidCount() > 0){
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i) && this.sg.cells(0,i) == "APP"){
							    j = i+1; k = i+2;
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(3,i)+"','D',"+nilaiToFloat(this.sg.cells(6,i))+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBRDBELI','HUT','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+k+",'"+this.akunpph+"','"+this.sg.cells(3,i)+"','C',"+nilaiToFloat(this.sg.cells(7,i))+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBRDBELI','HUTPPH','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");								
								sql.add("update inv_rdbeli_m set progress='1', no_kas ='"+this.e_nb.getText()+"' where no_rdbeli='"+this.sg.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
							}
						}
					}																				
	...
	doChange:function(sender){
		...
		if (sender == this.e_periode && this.e_periode.getText()!="") {			
			this.e_nb.setSQL("select no_kas, keterangan from kas_m where no_del='-' and modul = 'KBRDBELI' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kas","keterangan"],false,["No KB","Deskripsi"],"and","Daftar Bukti KB",true);
		}
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			var data = this.dbLib.getDataProvider("select jenis,tanggal,periode,no_dokumen,keterangan,akun_kb,nik_buat,nik_app,ref1,kode_bank "+
			           "from kas_m "+			           					   					   
			...
			var strSQL = "select a.no_rdbeli,convert(varchar,a.tanggal,103) as tgl,a.keterangan,c.nama as kelola,x.total+a.nilai_komisi+a.nilai_ppn+a.nilai_levy as nilai,a.nilai_pph,b.kode_akun as akun_hutang "+
			             "from inv_rdbeli_m a inner join inv_rdbeli_j b on a.no_rdbeli=b.no_rdbeli and a.kode_lokasi=b.kode_lokasi and b.jenis='HUT' "+
						 "                     inner join inv_rdkelola c on a.kode_rdkelola=c.kode_rdkelola "+
						 "                     inner join (select no_rdbeli,sum(harga*jumlah) as total from inv_rdbeli_d group by no_rdbeli) x on a.no_rdbeli=x.no_rdbeli "+
						 "where a.no_kas='"+this.e_nb.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' order by a.no_rdbeli";						
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData(["APP",line.no_rdbeli,line.tgl,line.keterangan,line.kelola,line.akun_hutang,floatToNilai(line.nilai),floatToNilai(line.nilai_pph)]);
				}
