window.app_saku3_transaksi_investasi_fKbRDDevE = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_fKbRDDevE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_fKbRDDevE";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Reksadana Deviden: Input", 0);	
		...
		this.p1 = new panel(this,{bound:[10,23,900,300],caption:"Daftar Reksadana Deviden"});
...
window.app_saku3_transaksi_investasi_fKbRDDevE.extend(window.childForm);
window.app_saku3_transaksi_investasi_fKbRDDevE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("update inv_rddev_m set no_kas ='-' where no_kas='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
										
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBRDDEV','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"',getdate(),'"+this.app._userLog+"','F','-','-','-','-')");										
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','09','-','"+this.app._lokasi+"','KBRDDEV','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");
					...
					if (this.sg.getRowValidCount() > 0){						
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i) && this.sg.cells(0,i) == "APP"){
								...
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) "+
										"(select '"+this.e_nb.getText()+"',a.no_rddev,'"+this.dp_d1.getDateString()+"',1,a.akun_dev,'"+this.sg.cells(3,i)+"','C',sum(a.nilai_dev),b.kode_pp,b.kode_drk,'-','-',a.kode_lokasi,'KBRDDEV','DEVIDEN','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-' "+
										" from inv_rddev_d a inner join inv_rddev_m b on a.no_rddev=b.no_rddev and a.kode_lokasi=b.kode_lokasi "+
										" where a.no_rddev='"+this.sg.cells(1,i)+"' and a.kode_lokasi='"+this.app._lokasi+"' "+
										" group by a.no_rddev,a.akun_dev,b.kode_pp,b.kode_drk,a.kode_lokasi)");
								sql.add("update inv_rddev_m set no_kas='"+this.e_nb.getText()+"' where no_rddev='"+this.sg.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
							}
						}
					}																				
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!="") {			
			this.e_nb.setSQL("select no_kas, keterangan from kas_m where no_del='-' and modul = 'KBRDDEV' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_kas","keterangan"],false,["No KB","Deskripsi"],"and","Daftar Bukti KB",true);
		}
