window.app_saku3_transaksi_investasi_fKbDOCaju = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_fKbDOCaju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_fKbDOCaju";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Penempatan DOC/Deposito: Input", 0);	
		...
		this.bTampil = new button(this,{bound:[600,15,80,18],caption:"DOC / DEPO",click:[this,"doLoad"]});			
...
window.app_saku3_transaksi_investasi_fKbDOCaju.extend(window.childForm);
window.app_saku3_transaksi_investasi_fKbDOCaju.implement({
	...
	doLoad:function(sender){
		if (this.e_periode.getText() != "" && this.cb_bank.getText()!="") {
			var strSQL = "select no_depo,convert(varchar,tanggal,103) as tgl,keterangan,kode_bank,akun_doc,nilai,modul from inv_depo_m where progress ='0' and periode<='"+this.e_periode.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' and kode_bank='"+this.cb_bank.getText()+"' order by modul,no_depo";
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData(["INPROG",line.no_depo,line.tgl,line.keterangan,line.kode_bank,line.akun_doc,floatToNilai(line.nilai),line.modul]);
				}
