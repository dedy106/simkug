window.app_saku3_transaksi_investasi_fRDDev = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_investasi_fRDDev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_investasi_fRDDev";
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('DRKRDJ','PPINV','RDDEVMI','RDDEVSW','RDDEVDKM') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku3_transaksi_investasi_fRDDev.extend(window.childForm);
window.app_saku3_transaksi_investasi_fRDDev.implement({
