window.app_saku3_transaksi_tm_fIFreim = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tm_fIFreim.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tm_fIFreim";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,305], childPage:["Data Reimburse","Data Anggaran"]});
...
window.app_saku3_transaksi_tm_fIFreim.extend(window.childForm);
window.app_saku3_transaksi_tm_fIFreim.implement({		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into if_reim_m (no_reim,no_ver,no_spb,no_kas,no_dokumen,tanggal,keterangan,kode_curr,kurs,akun_hutang,nik_buat,kode_lokasi,kode_pp,modul,nilai,progress,posted,periode,nik_user,tgl_input) values "+
					        "('"+this.e_nb.getText()+"','-','-','-','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','IDR',1,'"+this.akunHutIF+"','"+this.cb_nik.getText()+"','"+this.app._lokasi+"','"+this.cb_pp.getText()+"','"+this.c_status.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",'1','F','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())")					
					sql.add("insert into if_reim_j(no_reim,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',999,'"+this.akunHutIF+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_nilai.getText())+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','"+this.c_status.getText()+"','HUTANG','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");					
					
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into if_reim_j(no_reim,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','-','"+this.app._lokasi+"','IFREIM','BEBAN','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");								
							}
						}
					}		
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :					
				...
				var strSQL = "select no_reim from if_reim_m  "+
							 "where no_kas ='-' and nik_buat = '"+this.cb_nik.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){								
						system.alert(this,"Transaksi tidak valid.","Terdapat Reimburse yang sedah proses.("+line.no_reim+")");
						return false;						
					}
				}	
	...
	doChange:function(sender){		
		if (sender == this.cb_nik && this.cb_nik.getText()!="") {
			var strSQL = "select a.kode_pp,akun_hutif,a.nilai-isnull(b.reim,0) as saldo "+
						 ...
						 "		left join (  "+						 
						 "			select nik_buat,sum(nilai) as reim from if_reim_m "+
						 "			where kode_lokasi='"+this.app._lokasi+"' and nik_buat='"+this.cb_nik.getText()+"' and no_kas='-' and no_reim <>'"+this.e_nb.getText()+"' "+
						 "			group by nik_buat"+
						 "		) b on a.nik=b.nik_buat  "+
	...
	doLoad3:function(sender){																								
		var strSQL = "select a.no_reim,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai,a.progress "+
		             "from if_reim_m a inner join if_m b on a.nik_buat=b.nik and a.kode_lokasi=b.kode_lokasi and b.tahun=substring(a.periode,1,4) and b.flag_aktif='1' "+					 					 
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.posted='F' and a.progress ='1'";	
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){								
						...
						this.cb_nik.setSQL("select a.nik, a.nama from karyawan a inner join if_m b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi and b.tahun='"+this.e_periode.getText().substr(0,4)+"' where a.nik='"+line.nik_buat+"' and a.kode_lokasi='"+this.app._lokasi+"'",["a.nik","a.nama"],false,["Kode","Nama"],"and","Data Pemegang",true);
				...
				var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp "+
							...
							"where a.jenis<>'HUTANG' and a.no_reim = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
