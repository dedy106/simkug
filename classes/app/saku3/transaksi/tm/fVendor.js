window.app_saku3_transaksi_tm_fVendor = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tm_fVendor.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tm_fVendor";
		...
		this.cb_kode = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,10,220,20],caption:"Kode",maxLength:20,change:[this,"doChange"]});		
		...
		this.e_sap = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,13,200,20],caption:"Kode SAP", maxLength:50, tag:1});	
		...
		this.cb_pp = new portalui_saiCBBL(this.pc1.childPage[1],{bound:[20,10,220,20],caption:"PP/Unit",multiSelection:false,tag:2});
...
window.app_saku3_transaksi_tm_fVendor.extend(window.childForm);
window.app_saku3_transaksi_tm_fVendor.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into vendor(kode_vendor,kode_sap,kode_lokasi,nama,alamat,no_tel,email,npwp,pic,alamat2,bank,cabang,no_rek,nama_rek,no_fax,no_pictel,spek,kode_klpvendor,penilaian,bank_trans,akun_pph,kode_pp) values "+
						    "('"+this.cb_kode.getText()+"','"+this.e_sap.getText()+"', '"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.e_alamat.getText()+"','"+this.e_tel.getText()+"','"+this.e_mail.getText()+"','"+this.e_npwp.getText()+"','"+this.e_pic.getText()+"','"+this.e_alamat2.getText()+"','"+this.e_bank.getText()+"','"+this.e_cabang.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"','"+this.e_fax.getText()+"','"+this.e_pictel.getText()+"','-','-','-','-','-','"+this.cb_pp.getText()+"')");
					setTipeButton(tbAllFalse);
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select kode_sap,kode_vendor,nama,alamat,no_tel,email,npwp,alamat2,pic,bank,cabang,no_rek,nama_rek,no_fax,no_pictel,kode_pp "+
				             "from vendor "+
						     "where kode_vendor ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";						   				
				...
				if (typeof data == "object"){
					...
					if (line != undefined){						
						...
						this.e_sap.setText(line.kode_sap);
		...
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Mitra",sender,undefined, 
											  "select kode_vendor, nama  from vendor where kode_pp='"+this.app._kodePP+"' and kode_lokasi='"+this.app._lokasi+"'",
											  "select count(kode_vendor) from vendor where kode_pp='"+this.app._kodePP+"' and kode_lokasi='"+this.app._lokasi+"'",
	...
	doCari:function(sender){								
		try {
			...
			var strSQL = "select a.kode_vendor,a.nama,a.alamat "+
						 "from vendor a "+					 
						 "where "+filter+" a.kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"' order by a.kode_vendor";				
			var data = this.dbLib.getDataProvider(strSQL,true);		
