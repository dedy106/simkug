window.app_saku3_transaksi_tm_fIFtutup = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tm_fIFtutup.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tm_fIFtutup";
...
window.app_saku3_transaksi_tm_fIFtutup.extend(window.childForm);
window.app_saku3_transaksi_tm_fIFtutup.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {						
						...
						sql.add("update if_reim_m set progress='1',no_kas='-' where no_reim='"+this.e_noptg.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
					}
					
					sql.add("update if_reim_m set progress='2',no_kas='"+this.e_nb.getText()+"' where no_reim='"+this.e_noptg.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");											
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_noptg.getText()+"','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBIFTTP','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilaikb.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','"+this.e_noptg.getText()+"','-','"+this.cb_akun.getText()+"')");
					

					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','"+this.e_noptg.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','D',"+Math.abs(nilaiToFloat(this.e_nilaikb.getText()))+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBIFTTP','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'"+this.cb_akun.getText()+"')");																				
					//akun hutangif		
					...
					//akun if
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) "+
							"select '"+this.e_nb.getText()+"','"+this.e_noptg.getText()+"','"+this.dp_d1.getDateString()+"',999,a.kode_akun,c.keterangan,'C',a.nilai,c.kode_pp,'-','-','-',c.kode_lokasi,'KBIFTTP','IF','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),a.kode_akun "+
							"from if_m a inner join if_reim_m c on a.nik=c.nik_buat and a.kode_lokasi=c.kode_lokasi and a.tahun=substring(c.periode,1,4) "+
	...
	doDoubleClick: function(sender, col , row) {
		if (this.sg.cells(0,row) != "") {			
			...
			var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,'D' as dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp "+
							"from if_reim_j a "+
							"			   inner join if_reim_m x on a.no_reim=x.no_reim and a.kode_lokasi=x.kode_lokasi "+
							...
							"from if_m a "+
							"			 inner join if_reim_m b on a.nik=b.nik_buat and a.kode_lokasi=b.kode_lokasi and a.tahun=substring(b.periode,1,4) "+							
	...
	doLoad:function(sender){						
		var strSQL = "select a.no_reim,c.nik+' - '+c.nama as pemohon,convert(varchar,a.tanggal,103) as tanggal,a.no_dokumen,a.keterangan,aa.nilai as nilai_if, "+
					 "a.nilai,aa.nilai-a.nilai as nilai_kas,aa.kode_akun as akun_if,b.kode_pp+' - '+b.nama as pp "+
					 "from if_reim_m a "+
					 "    inner join if_m aa on a.nik_buat=aa.nik and a.kode_lokasi=aa.kode_lokasi and aa.tahun=substring(a.periode,1,4) "+					 					 
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData([line.no_reim,line.pemohon,line.tanggal,line.no_dokumen,line.keterangan,floatToNilai(line.nilai_if),floatToNilai(line.nilai),floatToNilai(line.nilai_kas),line.akun_if,line.pp]);
		}
	...
	doDoubleClickL: function(sender, col , row) {
		try{
			if (this.sgL.cells(0,row) != "") {
				...
				var strSQL = "select a.no_reim,c.nik+' - '+c.nama as pemohon,convert(varchar,a.tanggal,103) as tanggal,a.no_dokumen,a.keterangan,aa.nilai as nilai_if,a.nilai,aa.nilai-a.nilai as nilai_kas,aa.kode_akun as akun_if,b.kode_pp+' - '+b.nama as pp "+
					 "from if_reim_m a "+
					 "		inner join if_m aa on a.nik_buat=aa.nik and a.kode_lokasi=aa.kode_lokasi and aa.tahun=substring(a.periode,1,4) "+					 					 
		             "		inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
					 "		inner join karyawan c on aa.nik=c.nik and aa.kode_lokasi=c.kode_lokasi "+
