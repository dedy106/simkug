window.app_saku3_transaksi_tm_fTakKirimSusut = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tm_fTakKirimSusut.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tm_fTakKirimSusut";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form TAK Kirim Beban Penyusutan", 0);	
		...
		this.cb_bukti = new saiCBBL(this.pc2.childPage[0],{bound:[20,15,220,20],caption:"Bukti Susut", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});										
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,12,990,300], childPage:["Data Item Jurnal","Data Anggaran"]});
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('TAKSUSUT','GARFREE','DOKFREE') and kode_lokasi = '"+this.app._lokasi+"'",true);			
			if (typeof data == "object"){
				...
				for (var i in data.rs.rows){
					...
					if (line.kode_spro == "TAKSUSUT") this.akunTAK = line.flag;			
				}
			}			
...
window.app_saku3_transaksi_tm_fTakKirimSusut.extend(window.childForm);
window.app_saku3_transaksi_tm_fTakKirimSusut.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("update fasusut_m set no_del='-' where no_del='"+this.e_nb.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
					}					
					sql.add("update fasusut_m set no_del='"+this.e_nb.getText()+"' where no_fasusut='"+this.cb_bukti.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
					...
					//		"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.akunTAK+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_total.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','TAKSUSUT','TAK','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");																					
					
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){															           
								sql.add("insert into takkirim_j(no_kirim,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','"+this.app._lokasi+"','TAKSUSUT','JURNAL','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");
							}
						}
					}
					...
					sql.add("insert into takkirim_j(no_kirim,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) "+
							"select '"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.akunTAK+"',a.nama,'D',a.nilai,'"+this.app._kodePP+"','-','"+this.app._lokasi+"','TAKSUSUT','TAK','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1 "+
							"from ( "+
							
							"select b.nama,sum(case a.dc when 'D' then a.nilai else -a.nilai end) as nilai "+
							"from fasusut_j a inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
							"where substring(a.kode_akun,1,1) <> '1' and a.no_fasusut = '"+this.cb_bukti.getText()+"' and a.kode_lokasi='"+this.app._lokasi+" '"+ 					
							"group by b.nama "+
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"('"+this.e_nb.getText()+"','TAKSUSUT','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(6,i))+","+nilai+")");
							}
						}
					}
					setTipeButton(tbAllFalse);					
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					this.stsSimpan = 1;
					this.cb_bukti.setSQL("select no_fasusut,keterangan from fasusut_m where periode<='"+this.e_periode.getText()+"' and no_del='-' and kode_lokasi = '"+this.app._lokasi+"'",["no_fasusut","keterangan"],false,["No Bukti","Keterangan"],"and","Data Penyusutan",true);			
				break;
			case "simpan" :															
			case "ubah" :															
				this.preView = "1";
				...
				this.nilaiTAK = nilaiToFloat(this.e_total.getText());
	...
	doChange:function(sender){
		...
		if (sender == this.cb_bukti && this.cb_bukti.getText() != "" && this.stsSimpan == 1) {		    
		    var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,case a.dc when 'D' then 'C' else 'D' end as dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk "+
						...
						"where substring(a.kode_akun,1,1) <> '1' and a.no_fasusut = '"+this.cb_bukti.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
				for (var i in data.rs.rows){
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_kirim,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.kode_loktuj+' - '+b.nama as lokasi,a.nilai "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'TAKSUSUT' and a.posted ='F' and a.no_terima='-' and progress='0' ";		
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						this.cb_bukti.setSQL("select no_fasusut,keterangan from fasusut_m where no_fasusut='"+line.ref1+"' and kode_lokasi = '"+this.app._lokasi+"'",["no_fasusut","keterangan"],false,["No Bukti","Keterangan"],"and","Data Penyusutan",true);			
						this.cb_bukti.setText(line.ref1);												
					}
				}								
				var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk "+
							...
							"                  inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+													
							"                  left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and substring(a.periode,1,4)=d.tahun "+													
							"where a.jenis<>'TAK' and a.no_kirim = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
					for (var i in data.rs.rows){
