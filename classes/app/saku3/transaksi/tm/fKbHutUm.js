window.app_saku3_transaksi_tm_fKbHutUm = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tm_fKbHutUm.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tm_fKbHutUm";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Pembayaran Hutang", 0);	
		...
		this.cb_vendor = new saiCBBL(this.pc2.childPage[0],{bound:[20,19,220,20],caption:"Mitra", multiSelection:false, maxLength:10, tag:0,change:[this,"doChange"]});							
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:0,
				colTitle:["Status","No Hutang","No Tagihan","No FP","Jth Tempo","Deskripsi","Akun AP","Tagihan+","PP"],
				colWidth:[[8,7,6,5,4,3,2,1,0],[80,80,80,300,70,100,120,100,80]],
...
window.app_saku3_transaksi_tm_fKbHutUm.extend(window.childForm);
window.app_saku3_transaksi_tm_fKbHutUm.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBHUTUM','BK','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_total.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','"+this.cb_vendor.getText()+"','-','-')");
															
					var kasIDR = nilaiToFloat(this.e_total.getText());
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','C',"+kasIDR+",'"+this.app._kodePP+"','-','-','"+this.cb_vendor.getText()+"','"+this.app._lokasi+"','KBHUTUM','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+kasIDR+")");
										
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && this.sg1.cells(0,i) == "BAYAR"){
							var j=i+1000;
							var nilaiHut = nilaiToFloat(this.sg1.cells(7,i));
							sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
									"('"+this.e_nb.getText()+"','"+this.sg1.cells(2,i)+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(6,i)+"','"+this.e_ket.getText()+"','D',"+nilaiHut+",'"+this.sg1.cells(8,i)+"','-','-','-','"+this.app._lokasi+"','KBHUTUM','AP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiHut+")");				
							sql.add("insert into tm_hutbayar_d(no_bukti,kode_lokasi,no_hutang,periode,dc,modul,nilai,nik_user,tgl_input) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.sg1.cells(1,i)+"','"+this.e_periode.getText()+"','D','KBHUTUM',"+nilaiHut+",'"+this.app._userLog+"',getdate())");
					...
					if (this.sg.getRowValidCount() > 0) {
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								...
								var k = i+5000;
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+k+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+nilaiSlsIDR+",'"+this.sg.cells(5,i)+"','-','-','-','"+this.app._lokasi+"','KBHUTUM','SLS','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','"+nilaiToFloat(this.sg.cells(4,i))+"')");
							}
						}
					}					
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_vendor && this.cb_vendor.getText()!="" && this.stsSimpan==1 ) {
			var strSQL = "select a.no_hutang,a.no_dokumen,a.no_fp,CONVERT(varchar,a.due_date,103) as due_date,a.keterangan,a.akun_hutang,a.nilai-ISNULL(b.bayar,0) as saldo,a.kode_pp "+
						 "from tm_hutang_m a "+
						 " 	   left join "+
						 
						 "	   ("+
						 "			select no_hutang,kode_lokasi,SUM(case dc when 'D' then nilai else -nilai end) as bayar "+
						 "			from tm_hutbayar_d where kode_lokasi ='"+this.app._lokasi+"' "+
						 "			group by no_hutang,kode_lokasi "+
						 "		) b on a.no_hutang=b.no_hutang and a.kode_lokasi=b.kode_lokasi "+

						 "where a.kode_vendor ='"+this.cb_vendor.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' and a.nilai-ISNULL(b.bayar,0) >0 ";
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg1.appendData(["INPROG",line.no_hutang,line.no_dokumen,line.no_fp,line.due_date,line.keterangan,line.akun_hutang,floatToNilai(line.saldo),line.kode_pp]);
				}
	...
	doLoad3:function(sender){		
		var strSQL = "select a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,b.kode_vendor+' - '+ b.nama as vendor, a.nilai "+
		             "from kas_m a "+
					 "inner join vendor b on a.no_link=b.kode_vendor and a.kode_lokasi=b.kode_lokasi "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'KBHUTUM' and a.posted ='F'";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_kas,line.tgl,line.no_dokumen,line.keterangan,line.vendor,floatToNilai(line.nilai)]); 
		}
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						this.cb_vendor.setSQL("select kode_vendor, nama from vendor where kode_vendor='"+line.no_link+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_vendor","nama"],false,["Kode","Nama"],"and","Data Vendor",true);									
					}
				}
				
				var strSQL = "select a.no_hutang,a.no_dokumen,a.no_fp,CONVERT(varchar,a.due_date,103) as due_date,a.keterangan,a.akun_hutang,a.nilai-ISNULL(b.bayar,0) as saldo, a.kode_pp "+
							 "from tm_hutang_m a  "+
							 "	    inner join tm_hutbayar_d c on a.no_hutang=c.no_hutang and a.kode_lokasi=c.kode_lokasi "+		 
							 
							 "		left join ("+
							 "				select no_hutang,kode_lokasi,SUM(case dc when 'D' then nilai else -nilai end) as bayar "+
							 "				from tm_hutbayar_d where kode_lokasi ='"+this.app._lokasi+"' and no_bukti <>'"+this.e_nb.getText()+"' "+
							 "				group by no_hutang,kode_lokasi "+
							 "		) b on a.no_hutang=b.no_hutang and a.kode_lokasi=b.kode_lokasi "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData(["BAYAR",line.no_hutang,line.no_dokumen,line.no_fp,line.due_date,line.keterangan,line.akun_hutang,floatToNilai(line.saldo),line.kode_pp]);
					}
				} else this.sg1.clear(1);
				this.sg1.validasi();
