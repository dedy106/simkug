window.app_saku3_transaksi_tm_fTakPdptTerima = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_tm_fTakPdptTerima.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tm_fTakPdptTerima";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form TAK Terima Pendapatan", 0);	
		...
		try {			
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('TMID','TMMC','GARFREE','DOKFREE') and kode_lokasi = '"+this.app._lokasi+"'",true);			
			if (typeof data == "object"){
				...
				for (var i in data.rs.rows){
					...
					if (line.kode_spro == "TMID") this.akunID = line.flag;
					if (line.kode_spro == "TMMC") this.akunMC = line.flag;												
				}
			}
...
window.app_saku3_transaksi_tm_fTakPdptTerima.extend(window.childForm);
window.app_saku3_transaksi_tm_fTakPdptTerima.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsProg != "INPROG") {						
						...
						sql.add("update tm_takpdpt_d set no_terima='-' where no_kirim ='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.kodeLokAsal+"'");
					}					
					
					sql.add("update tm_takpdpt_d set no_terima='"+this.e_nb.getText()+"' where no_kirim ='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.kodeLokAsal+"'");										
		...
		try{
			if (this.sg.cells(0,row) != "") {							
				...
				if (this.stsProg == "INPROG") {
					setTipeButton(tbSimpan); 
					this.doClick();				
					var data = this.dbLib.getDataProvider("select (case when c.kode_induk = 'I' then '"+this.akunID+"' else '"+this.akunMC+"' end) as kode_akun,b.nama as nama_akun,case a.dc when 'D' then 'C' else 'D' end as dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp "+
								"from tm_takpdpt_d a "+							
								"  inner join pp c on a.kode_pp=c.kode_pp and a.kode_loktuj=c.kode_lokasi "+	
								"  inner join masakun b on (case when c.kode_induk = 'I' then '"+this.akunID+"' else '"+this.akunMC+"' end) =b.kode_akun and a.kode_lokasi=b.kode_lokasi  "+                   
								"where a.no_selesai='-' and a.no_kirim = '"+this.e_nobukti.getText()+"' and a.kode_loktuj='"+this.app._lokasi+"' order by a.nu",true);
					if (typeof data == "object" && data.rs.rows[0] != undefined){
						var line;
						this.sgv.clear();
						for (var i in data.rs.rows){
	...
	doLoad:function(sender){																		
		var strSQL = "select a.no_kirim as no_bukti,'INPROG' as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_terima,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput "+
					 ...
					 "where a.progress='0' and a.kode_loktuj='"+this.app._lokasi+"' and a.modul= 'TAKPDPT' and a.periode<='"+this.e_periode.getText()+"' ";
