window.app_saku3_transaksi_tm_fKbSPB = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tm_fKbSPB.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tm_fKbSPB";
		...
		this.c_jenis = new saiCB(this.pc2.childPage[0],{bound:[20,11,200,20],caption:"Jenis",items:["BK"], readOnly:true,tag:2,change:[this,"doChange"]});		
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,325], childPage:["Daftar SPB","Daftar Transfer","Jurnal KasBank","Jurnal Tambahan","Data Anggaran"]});		
		this.sg5 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:9,tag:0,
		            colTitle:["Status","No SPB","No Fiat","Tanggal","Due Date","Deskripsi","Nilai","Catatan Fiatur","Modul"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[80,160,100,210,70,70,120,120,80]],colFormat:[[6],[cfNilai]],										
		...
		this.sg = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:6,tag:0,
		            colTitle:["Kode Rek","Nama Rek","Kode Akun","DC","Keterangan","Nilai"],
					colWidth:[[5,4,3,2,1,0],[100,350,50,100,250,80]],					
					columnReadOnly:[true,[1,2],[0,3,4,5]],
		...
		this.sg1 = new saiGrid(this.pc1.childPage[3],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:8,tag:9,
		...
		this.sg2 = new saiGrid(this.pc1.childPage[4],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:7,tag:9,
					...
					colWidth:[[6,5,4,3,2,1,0],[100,100,100,150,80,150,80]],
...
window.app_saku3_transaksi_tm_fKbSPB.extend(window.childForm);
window.app_saku3_transaksi_tm_fKbSPB.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("update yk_kasdrop_d set no_kas='-',progress='0' where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						
						sql.add("delete from takkirim_m where no_kirim = '"+this.e_nb.getText()+"'");
						...
						sql.add("update hutang_m set no_ref='-' where no_ref = '"+this.e_nb.getText()+"'");
						
					}					
					for (var i=0;i < this.dataJU5.rs.rows.length;i++){
						line = this.dataJU5.rs.rows[i];
						if (line.status.toUpperCase() == "BAYAR") {
							if (line.modul == "DROPING") sql.add("update yk_kasdrop_d set no_kas='"+this.e_nb.getText()+"',progress='1' where no_spb = '"+line.no_spb+"' and kode_lokasi='"+this.app._lokasi+"'");							
							
							if (line.modul == "HUTTAK") {
								var strSQL2 = "select kode_lokasi,no_hutang,kode_pp,akun_hutang,nilai from hutang_m where no_spb='"+line.no_spb+"' ";						
								var data2 = this.dbLib.getDataProvider(strSQL2,true);
								if (typeof data2 == "object"){
									var line2 = data2.rs.rows[0];							
									if (line2 != undefined){								
										var lokTujuan = line2.kode_lokasi;
										var noHutang = line2.no_hutang;	
										var ppTujuan = line2.kode_pp;	
										var akunHutang = line2.akun_hutang;		
										var nilaiTAK = parseFloat(line2.nilai);	
									}
								}				
								sql.add("insert into takkirim_m(no_kirim,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user,kode_loktuj,progress,no_terima,due_date) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','HUTTAK','KIRIM','IDR',1,"+nilaiTAK+",'"+this.app._userLog+"','"+this.app._userLog+"','X','-','-','"+line.no_spb+"',getdate(),'"+this.app._userLog+"','"+lokTujuan+"','1','"+this.e_nb.getText()+"','"+this.dp_d1.getDateString()+"')");
								sql.add("insert into takterima_m (no_terima,kode_lokasi,no_dokumen,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_setuju,tgl_input,nik_user,posted,no_del,no_link,kode_lokkirim,no_kirim) values "+
										"('"+this.e_nb.getText()+"','"+lokTujuan+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','-','"+ppTujuan+"','HUTTAK','TAKTERIMA','"+this.e_periode.getText()+"','IDR',1,"+nilaiTAK+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','-','"+this.app._lokasi+"','"+this.e_nb.getText()+"')");
								...
								sql.add("insert into takterima_j(no_terima,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+akunHutang+"','"+this.e_ket.getText()+"','D',"+nilaiTAK+",'"+ppTujuan+"','-','"+lokTujuan+"','HUTTAK','HUTANG','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");										
								sql.add("insert into takterima_j(no_terima,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) "+
										"select '"+this.e_nb.getText()+"',no_dokumen,'"+this.dp_d1.getDateString()+"',no_urut,kode_akun,keterangan,'C',"+nilaiTAK+",'"+ppTujuan+"','-','"+lokTujuan+"','HUTTAK','TAK','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1 "+
										"from spb_j where no_spb='"+line.no_spb+"' and kode_lokasi='"+this.app._lokasi+"'");												
							
								sql.add("update hutang_m set no_ref='"+this.e_nb.getText()+"' where no_spb = '"+line.no_spb+"'");							
							}
							...
							sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) "+
									"select '"+this.e_nb.getText()+"',no_dokumen,'"+this.dp_d1.getDateString()+"',no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,'-','-','"+this.app._lokasi+"','KBSPB',jenis,'"+this.e_periode.getText()+"',kode_curr,kurs,'"+this.app._userLog+"',getdate(),'-',nilai "+
									"from spb_j where no_spb='"+line.no_spb+"' and kode_lokasi='"+this.app._lokasi+"'");
						}
					}
					
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBSPB','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','-','-','-')");																
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(2,i)+"','"+this.sg.cells(4,i)+"','"+this.sg.cells(3,i).toUpperCase()+"',"+nilaiToFloat(this.sg.cells(5,i))+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBSPB','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'"+this.sg.cells(0,i)+"',"+nilaiToFloat(this.sg.cells(5,i))+")");
							}
						}
					}
					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"','-','-','-','"+this.app._lokasi+"','KBSPB','NONKB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.sg1.cells(4,i))+")");										
							}
						}
					}					
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"('"+this.e_nb.getText()+"','KBSPB','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(4,i))+","+nilai+")");
							}
						}
					}					
					setTipeButton(tbAllFalse);					
	...
	doLoad:function(sender){		
		this.dataJU5 = {rs:{rows:[]}};
		var strSQL = "select 'INPROG' as status,a.no_spb,a.no_fiat,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as duedate,a.keterangan,a.nilai,case b.catatan when '' then '-' else b.catatan end as catatan,a.modul "+
	...
	doTampilData5: function(page) {		
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU5.rs.rows[i];									
			this.sg5.appendData([line.status.toUpperCase(),line.no_spb,line.no_fiat,line.tgl,line.duedate,line.keterangan,floatToNilai(line.nilai),line.catatan,line.modul]);
		}
	...
	doDoubleClick5: function(sender, col , row) {
		...
		if (noSPB != "") {
			var strSQL = "select a.kode_loktuj as kode_vendor,c.nama,b.kode_bank as bank,b.cabang,b.no_rek,b.nama as nama_rek,a.nilai,a.keterangan "+
						 ...
						 "select a.kode_vendor,b.nama,b.bank,b.cabang,b.no_rek,b.nama as nama_rek,a.nilai,a.keterangan "+
					     "from hutang_m a inner join vendor b on a.kode_vendor=b.kode_vendor and a.kode_lokasi=b.kode_lokasi "+
					     "where a.no_spb in ("+noSPB+") "+
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select keterangan,no_dokumen,jenis,tanggal "+
							 "from kas_m "+							 
				...
				var data = this.dbLib.getDataProvider("select b.kode_rek,b.nama as nama_rek,a.kode_akun,a.dc,a.keterangan,a.nilai "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];												
						this.sg.appendData([line.kode_rek,line.nama_rek,line.kode_akun,line.dc,line.keterangan,floatToNilai(line.nilai)]);
					}
