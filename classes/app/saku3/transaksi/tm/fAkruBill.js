window.app_saku3_transaksi_tm_fAkruBill = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tm_fAkruBill.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tm_fAkruBill";
...
window.app_saku3_transaksi_tm_fAkruBill.extend(window.childForm);
window.app_saku3_transaksi_tm_fAkruBill.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.sg2.getRowValidCount() > 0){
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i) && this.sg2.cells(4,i) != "0"){
								...
								if (this.sg2.cells(5,i) == "MITRA") totHut += nilaiToFloat(this.sg2.cells(4,i)); 
							}
						}
					}										
					var nobukti = ""; 					
					...
					sql.add("insert into yk_hutang_m(no_hutang,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,tgl_input,nik_user,kode_loktuj,no_app,progress) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','-','"+this.e_ket.getText()+"','"+this.kodepp+"','HUTKES','BILL','IDR',1,"+totHut+",'"+this.app._userLog+"','"+this.cb_app.getText()+"','F',getdate(),'"+this.app._userLog+"','"+this.cb_lokasi.getText()+"','-','0')");
							
					nobukti = nobukti.substr(1);					
					sql.add("insert into yk_hutang_d(no_hutang,kode_lokasi,no_inv,periode,kode_vendor,bank,cabang,no_rek,nama_rek,nilai_bp,nilai_cc,no_rekon,bank_trans,no_app,no_ver,no_spb,no_kas,progress,kode_loktuj,flag) "+
							...
							"       case when b.flag = '1' then '-' else '"+this.e_nb.getText()+"' end as no_app,"+
							"       case when b.flag = '1' then '-' else '"+this.e_nb.getText()+"' end as no_ver,"+
							"       case when b.flag = '1' then '-' else '"+this.e_nb.getText()+"' end as no_spb,"+
							"       case when b.flag = '1' then '-' else '"+this.e_nb.getText()+"' end as no_kas,"+
							"       case when b.flag = '1' then '0' else 'X' end as progress,"+
							"       '"+this.cb_lokasi.getText()+"',b.flag "+
							" from yk_bill_d a "+
							"    inner join yk_loker ff on a.loker=ff.loker "+							
							"    inner join cust f on ff.kode_cust=f.kode_cust "+							
							"    inner join vendor b on a.kode_vendor=b.kode_vendor and a.kode_lokasi=b.kode_lokasi "+//and b.flag<>'0' 
							"    inner join vendor_klp bb on b.kode_klpvendor=bb.kode_klpvendor and bb.kode_lokasi=b.kode_lokasi "+
							"    inner join yk_bill_m d on a.no_bill=d.no_bill and a.kode_lokasi=d.kode_lokasi "+
							" where a.no_bill in ("+nobukti+") and d.periode<='"+this.e_periode.getText()+"' and d.kode_lokasi='"+this.app._lokasi+"' and bb.jenis='MITRA' "+
							" group by b.flag,b.nama_rek,b.no_rek,b.bank,b.cabang,b.kode_vendor,b.nama,b.bank_trans");
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "simpancek" : this.simpan();			
				break;
			case "hapus" :	
				var data = this.dbLib.getDataProvider("select distinct progress from yk_hutang_d where progress not in ('0','R','X') and no_hutang='"+this.e_nb.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'",true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){																				
						system.alert(this,"Transaksi tidak valid.","No Pengajuan HUTKES sudah diprogress lain (yk_hutang_d).");
	...
	doTampil:function(sender){		
		if (this.e_periode.getText() != "") {
			var strSQL = "select a.no_bill,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,a.nilai,a.kunj,a.cs,a.jenis "+
			             "from yk_bill_m a "+
						 "where a.jenis in ('MITRA','RESTITUSI') and a.progress = '0' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"'";
	...
	doJurnal:function(sender){		
		try {
			...
			var strSQL = "select d.nama as ket,"+
						...
						"  			inner join vendor xx on a.kode_vendor=xx.kode_vendor and xx.flag <> '0' and a.kode_lokasi=xx.kode_lokasi "+												
						...
						"select "+
						"case bb.jenis when 'MITRA'     then 'HUTANG MITRA' "+
						"              when 'INTERNAL'  then 'PENDAPATAN' end as ket, "+
						"bb.bp_hut as kode_akun,c.nama as nama_akun,'C' as dc, sum(a.nilai - a.pph) as nilai,  "+						
						"bb.jenis,'-' as kode_drk,'-' as nama_drk "+
						"from yk_bill_d a "+
						"			inner join vendor b on a.kode_vendor=b.kode_vendor and a.kode_lokasi=b.kode_lokasi and b.flag <> '0' "+
						"			inner join vendor_klp bb on bb.kode_klpvendor=b.kode_klpvendor and bb.kode_lokasi=b.kode_lokasi  "+
						"			inner join masakun c on bb.bp_hut=c.kode_akun and bb.kode_lokasi=c.kode_lokasi  "+
						"			inner join yk_bill_m d on a.no_bill=d.no_bill and a.kode_lokasi=d.kode_lokasi  "+
						"where d.no_bill in ("+nobukti+") and d.kode_lokasi='"+this.app._lokasi+"' "+
						"group by bb.jenis,bb.bp_hut,c.nama, "+
						"         case bb.jenis when 'MITRA'     then 'HUTANG MITRA' "+
						"                       when 'INTERNAL'  then 'PENDAPATAN' end "+						
						"union all "+						
						...
						//KUNJUNGAN	& PDPT					
						"select 'PIU KUNJ '+d.nama as ket,c.akun_pku as kode_akun,d.nama as nama_akun,'D' as dc, "+
						...
						"where a.umum+a.gigi+a.kbia+a.matkes<> 0 and b.jenis = 'TMMC' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_bill in ("+nobukti+") "+
						"group by d.nama,c.akun_pku,d.nama,c.drk_kunj,e.nama "+
						"union all "+									
						
						"select 'PDPT KUNJ '+d.nama as ket,c.akun_ku as kode_akun,d.nama as nama_akun,'C' as dc, "+
						...
						"		 	          inner join drk e on c.drk_kunj=e.kode_drk and e.kode_lokasi='"+this.app._lokasi+"' and e.tahun=c.tahun "+
						"where a.umum+a.gigi+a.kbia+a.matkes<> 0 and b.jenis = 'TMMC' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_bill in ("+nobukti+") "+
						"group by d.nama,c.akun_ku,d.nama,c.drk_kunj,e.nama "+																								
						"union all "+						
						
						//CS						
						"select 'TTP CS '+d.nama as ket,c.akun_hutcs as kode_akun,d.nama as nama_akun,'D' as dc, "+
						...
						"group by  d.nama,c.akun_hutcs,d.nama,c.drk_kunj,e.nama "+
						"union all "+						
						"select 'PIU CS '+d.nama as ket,c.akun_piucs as kode_akun,d.nama as nama_akun,'C' as dc, "+
						...
						"		 	          inner join drk e on c.drk_kunj=e.kode_drk and e.kode_lokasi='"+this.app._lokasi+"' and e.tahun=c.tahun "+
						"where a.cs<> 0 and b.jenis <> 'PENSIUN' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_bill in ("+nobukti+") "+
						"group by  d.nama,c.akun_piucs,d.nama,c.drk_kunj,e.nama "+
	...
	doLoad4:function(sender){																				
		var strSQL = "select distinct a.no_hutang,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai,a.kode_loktuj+' - '+c.nama as lokbayar,a.nik_setuju+' - '+b.nama as nama "+
		             "from yk_hutang_m a inner join karyawan b on a.nik_setuju=b.nik "+
					 "                   inner join lokasi c on a.kode_loktuj=c.kode_lokasi "+					 
					 "                   left join yk_bill_m d on a.no_hutang=d.no_hutang and a.kode_lokasi=d.kode_lokasi and no_batch <> '-' "+
					 "where d.no_hutang is null and a.modul = 'HUTKES' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress in ('0','R') and a.posted='F' ";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU4 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg4.cells(0,row) != "") {
				...
				var strSQL = "select keterangan,tanggal,nik_setuju,kode_loktuj "+
							 "from yk_hutang_m "+							 
