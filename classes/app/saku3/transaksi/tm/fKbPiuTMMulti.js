window.app_saku3_transaksi_tm_fKbPiuTMMulti = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tm_fKbPiuTMMulti.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tm_fKbPiuTMMulti";
...
window.app_saku3_transaksi_tm_fKbPiuTMMulti.extend(window.childForm);
window.app_saku3_transaksi_tm_fKbPiuTMMulti.implement({
	...
	doChange:function(sender){		
		...
		if ((sender == this.cb_cust) && this.cb_cust.getText()!="" && this.stsSimpan==1 ) {			
			var strSQL = "select a.no_piutang,a.no_dokumen,a.no_fp,CONVERT(varchar,a.tanggal,103) as tgl,a.keterangan,a.akun_piutang,a.nilai+a.nilai_ppn-ISNULL(b.bayar,0) as saldo,a.kode_pp "+
						 "from piutang_m a "+						 
						 "     left join ("+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg1.appendData(["INPROG",line.no_piutang,line.no_dokumen,line.no_fp,line.tgl,line.keterangan,line.akun_piutang,floatToNilai(line.saldo),line.kode_pp]);
				}
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData(["BAYAR",line.no_piutang,line.no_dokumen,line.no_fp,line.tgl,line.keterangan,line.akun_piutang,floatToNilai(line.saldo),line.kode_pp]);
					}
				} else this.sg1.clear(1);
