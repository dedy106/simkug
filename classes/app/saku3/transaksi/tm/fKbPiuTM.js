window.app_saku3_transaksi_tm_fKbPiuTM = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tm_fKbPiuTM.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tm_fKbPiuTM";
...
window.app_saku3_transaksi_tm_fKbPiuTM.extend(window.childForm);
window.app_saku3_transaksi_tm_fKbPiuTM.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.akunPiutang+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_nilai.getText())+",'"+this.ppPiutang+"','-','-','-','"+this.app._lokasi+"','KBPIU','PIU','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");					
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){																							
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i)+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','-','-','-','"+this.app._lokasi+"','KBPIU','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");					
							}
						}
					}
	...
	doChange:function(sender){		
		if ((sender == this.e_periode || sender == this.cb_cust) && (this.e_periode.getText()!= "" && this.cb_cust.getText()!= "" && this.stsSimpan==1)) {			
			var strSQL = "select a.no_piutang,a.keterangan from piutang_m a "+
						 "    left join ( "+
		...
		if (sender == this.cb_piutang && this.cb_piutang.getText()!= "") {
			var strSQL = "select (a.nilai+a.nilai_ppn-a.nilai_pph) -  isnull(b.bayar,0) as saldo, a.akun_piutang, a.kode_pp "+			             
						 "from piutang_m a "+						 
						 "     left join ("+
						 "              select no_piutang,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar "+
						 "              from piubayar_d  where no_bukti <> '"+this.e_nb.getText()+"' group by no_piutang,kode_lokasi) b on a.no_piutang=b.no_piutang and a.kode_lokasi=b.kode_lokasi "+
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select (a.nilai+a.nilai_ppn-a.nilai_pph) -  isnull(b.bayar,0) as saldo, a.akun_piutang,a.kode_cust,"+							 
							 "e.jenis,e.periode,e.no_dokumen,e.tanggal,e.keterangan as ket_kas,e.nik_app,e.no_link,a.kode_pp as pp_piutang "+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){		
						...
						this.ppPiutang = line.pp_piutang;
