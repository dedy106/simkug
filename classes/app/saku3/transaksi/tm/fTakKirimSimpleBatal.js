window.app_saku3_transaksi_tm_fTakKirimSimpleBatal = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_tm_fTakKirimSimpleBatal.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tm_fTakKirimSimpleBatal";
...
window.app_saku3_transaksi_tm_fTakKirimSimpleBatal.extend(window.childForm);
window.app_saku3_transaksi_tm_fTakKirimSimpleBatal.implement({	
			...
			if (this.sg.cells(0,row) != "") {							
				...
				if (this.e_modul.getText() == "TAKUMUM") {
					...
					if (this.stsProg == "INPROG") { //load data awal jurnal kirim
						var strSQLlawanSeb = "select a.kode_akun,b.nama as nama_akun,case a.dc when 'D' then 'C' else 'D' end as dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp "+
									"from takkirim_j a inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
									"                  inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+										
