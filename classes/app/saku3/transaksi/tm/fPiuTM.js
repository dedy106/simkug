window.app_saku3_transaksi_tm_fPiuTM = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_tm_fPiuTM.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tm_fPiuTM";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Piutang via Bill: Input", 0);	
		...
		this.c_stsppn = new saiCB(this.pc2.childPage[0],{bound:[780,17,200,20],caption:"Status PPN",items:["PPN","NON"], readOnly:true,tag:9,change:[this,"doChange"]});
		this.e_ket = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,18,450,20],caption:"Deskripsi", maxLength:150});								
		this.e_unbill = new saiLabelEdit(this.pc2.childPage[0],{bound:[780,18,210,20],caption:"Nilai Unbill", tag:1, tipeText:ttNilai, text:"0",readOnly:true});
		this.cb_pp = new saiCBBL(this.pc2.childPage[0],{bound:[20,16,220,20],caption:"PP YanKes", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});								
		this.e_nilai = new saiLabelEdit(this.pc2.childPage[0],{bound:[780,16,210,20],caption:"Nilai Bill", tag:1, tipeText:ttNilai, text:"0"});
		...
		this.e_ppn = new saiLabelEdit(this.pc2.childPage[0],{bound:[780,13,210,20],caption:"PPN", tag:1, tipeText:ttNilai, text:"0", readOnly:true});		
		...
		this.e_total = new saiLabelEdit(this.pc2.childPage[0],{bound:[780,15,210,20],caption:"Total Bill", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,10,995,265], childPage:["Data Item Jurnal","Jurnal Sisa"]});
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:8,tag:0,
		            colTitle:["Status","No Bukti","Keterangan","Kode Lokasi","Kode PP","Nama PP","Kode Akun","Nilai"],
					colWidth:[[7,6,5,4,3,2,1,0],[100,80,150,80,80,250,100,80]],					
					columnReadOnly:[true,[0,1,2,3,4,5,6,7],[]],			
					buttonStyle:[[0],[bsAuto]], 
					colFormat:[[7],[cfNilai]],picklist:[[0],[new portalui_arrayMap({items:["APP","INPROG"]})]],checkItem:true,
		...
		this.e_sls = new saiLabelEdit(this.sgn1,{bound:[780,1,200,20],caption:"Tot Jurnal Sisa", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});	
		...
		try {
			...
			this.cb_akun.setSQL("select kode_akun, nama from masakun where kode_lokasi='"+this.app._lokasi+"'",["kode_akun","nama"],false,["Kode","Nama"],"and","Data Akun Piutang",true);						
			...
			this.flagGarFree = "0"; 
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('BIDYANKES','GARFREE','HUTPPN') and kode_lokasi = '"+this.app._lokasi+"'",true);			
			...
			this.cb_pp.setSQL("select kode_pp, nama from pp where kode_bidang='"+this.bidYankes+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_pp","nama"],false,["Kode","Nama"],"and","Data PP",true);						
...
window.app_saku3_transaksi_tm_fPiuTM.extend(window.childForm);
window.app_saku3_transaksi_tm_fPiuTM.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into piutang_m(no_piutang,kode_lokasi,no_dokumen,tanggal,keterangan,kode_project,kode_cust,kode_curr,kurs,nik_app,kode_pp,nilai,periode,nik_user,tgl_input,akun_piutang,posted,nilai_ppn) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','-','"+this.cb_cust.getText()+"','IDR',1,'"+this.cb_app.getText()+"','"+this.cb_pp.getText()+"',"+parseNilai(this.e_nilai.getText())+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.cb_akun.getText()+"','F',"+parseNilai(this.e_ppn.getText())+")");								
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){															
								var j = i+1;
								sql.add("insert into piutang_j(no_piutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i)+"','IDR',1,"+parseNilai(this.sg.cells(4,i))+","+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','"+this.app._lokasi+"','PIUUMUM','BBN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");								
							}
						}
					}
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','PIUUM','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(6,i))+","+nilai+")");
							}
						}
					}
					setTipeButton(tbAllFalse);					
	...
	doChange:function(sender){
		...
		if (sender == this.cb_pp && this.cb_pp.getText() != "") {
			//colTitle:["Status","No Bukti","Keterangan","Kode Lokasi","Kode PP","Nama PP","Kode Akun","Nilai"],
			var strSQL = "select "+
			             "from tm_takpdpt_d a inner join takkirim_m b on a.no_kirim=b.no_kirim and a.kode_lokasi=b.kode_lokasi "+
						 "where a.kode_pp='"+this.cb_pp.getText()+"' and a.kode_loktuj ='"+this.app._lokasi+"'";
	...
	doLoad3:function(sender){																				
		var strSQL = "select distinct a.no_piutang,convert(varchar,a.tanggal,103) as tgl,a.kode_cust+'-'+b.nama as cust,a.no_dokumen,a.keterangan,a.nilai+a.nilai_ppn as total "+
		             "from piutang_m a inner join cust2 b on a.kode_cust=b.kode_cust and a.kode_lokasi=b.kode_lokasi "+
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select d.no_bill,a.tanggal,a.periode,a.no_dokumen,a.keterangan,a.kode_pp,a.akun_piutang,a.nik_app,a.kode_cust,c.nama as nama_cust,nilai_ppn "+						 
							 "from piutang_m a inner join cust2 c on a.kode_cust=c.kode_cust and a.kode_lokasi=c.kode_lokasi "+
							 "                 inner join bill_m d on a.no_piutang=d.no_piutang and a.kode_lokasi=d.kode_lokasi "+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){		
						...
						if (this.e_ppn.getText() != "0") this.c_stsppn.setText("PPN");
