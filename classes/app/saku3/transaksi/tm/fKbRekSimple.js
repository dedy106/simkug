window.app_saku3_transaksi_tm_fKbRekSimple = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tm_fKbRekSimple.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tm_fKbRekSimple";
		...
		this.cb_akun = new saiCBBL(this.pc2.childPage[0],{bound:[20,18,220,20],caption:"Akun KasBank", multiSelection:false, maxLength:10, tag:9,change:[this,"doChange"]});				
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,300], childPage:["Jurnal KasBank","Jurnal NonKB","Data Anggaran"]});
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:6,tag:0,
					...
					colWidth:[[5,4,3,2,1,0],[100,270,50,80,200,80]],					
...
window.app_saku3_transaksi_tm_fKbRekSimple.extend(window.childForm);
window.app_saku3_transaksi_tm_fKbRekSimple.implement({	
					...
					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"','-','-','-','"+this.app._lokasi+"','KBUMUM','NONKB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.sg1.cells(4,i))+")");										
							}
						}
					}					
			...
			if (sender == this.sg) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Rekening Bank",sender,undefined, 
						    "select kode_rek,nama   from bank_rek where kode_lokasi = '"+this.app._lokasi+"' and kode_akun like '"+this.cb_akun.getText()+"%'",
							"select count(kode_rek) from bank_rek where kode_lokasi = '"+this.app._lokasi+"' and kode_akun like '"+this.cb_akun.getText()+"%'",
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.jenis,a.no_dokumen,a.keterangan,a.nilai "+
		             "from kas_m a "+					 					 
					 "where a.no_del='-' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'KBUMUM' and a.posted ='F'";		
