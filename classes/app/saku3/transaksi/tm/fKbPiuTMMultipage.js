window.app_saku3_transaksi_tm_fKbPiuTMMultipage = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tm_fKbPiuTMMultipage.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tm_fKbPiuTMMultipage";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Penerimaan Piutang Umum", 0);	
		...
		this.pc2 = new pageControl(this,{bound:[10,10,1000,430], childPage:["Data Bayar","List Bayar"]});		
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,20,990,282], childPage:["Data Invoice","Jurnal Sisa"]});		
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:0,
				...
				colWidth:[[8,7,6,5,4,3,2,1,0],[80,80,80,300,70,120,120,120,80]],
...
window.app_saku3_transaksi_tm_fKbPiuTMMultipage.extend(window.childForm);
window.app_saku3_transaksi_tm_fKbPiuTMMultipage.implement({		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBPIUM','BM','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_total.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','"+this.cb_cust.getText()+"','-','-')");
															
					var kasIDR = nilaiToFloat(this.e_total.getText());
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','D',"+kasIDR+",'"+this.app._kodePP+"','-','-','"+this.cb_cust.getText()+"','"+this.app._lokasi+"','KBPIUM','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+kasIDR+")");
					...
					// 		sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
					// 				"('"+this.e_nb.getText()+"','"+this.sg1.cells(2,i)+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(6,i)+"','"+this.e_ket.getText()+"','C',"+nilaiPiu+",'"+this.sg1.cells(8,i)+"','-','-','-','"+this.app._lokasi+"','KBPIUM','AR','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiPiu+")");				

					// 		sql.add("insert into piubayar_d(no_bukti,no_piutang,kode_lokasi,modul,periode,nik_user,tgl_input,dc,nilai) values "+
					// 				"('"+this.e_nb.getText()+"','"+this.sg1.cells(1,i)+"','"+this.app._lokasi+"','KBPIU','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'D',"+nilaiPiu+")");																					
					// 	}
					// }		
					...
					for (var i=0;i < this.dataJU1.rs.rows.length;i++){
						...
						if (line.status.toUpperCase() == "BAYAR"){
							...
							var j=i+1000;
							var nilaiPiu = parseFloat(line.saldo);
							
							sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
									"('"+this.e_nb.getText()+"','"+line.no_dokumen+"','"+this.dp_d1.getDateString()+"',"+j+",'"+line.akun_piutang+"','"+line.keterangan+"','C',"+nilaiPiu+",'"+line.kode_pp+"','-','-','-','"+this.app._lokasi+"','KBPIUM','AR','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiPiu+")");				

							sql.add("insert into piubayar_d(no_bukti,no_piutang,kode_lokasi,modul,periode,nik_user,tgl_input,dc,nilai) values "+
									"('"+this.e_nb.getText()+"','"+line.no_piutang+"','"+this.app._lokasi+"','KBPIU','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'D',"+nilaiPiu+")");																					
					...
					if (this.sg.getRowValidCount() > 0) {
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								...
								var k = i+5000;
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+k+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+nilaiSlsIDR+",'"+this.sg.cells(5,i)+"','-','-','-','"+this.app._lokasi+"','KBPIUM','SLS','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','"+nilaiToFloat(this.sg.cells(4,i))+"')");
							}
						}
					}					
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_cust && this.cb_cust.getText()!="" && this.stsSimpan==1 ) {	
			this.dataJU1 = {rs:{rows:[]}};					
			var strSQL = "select 'INPROG' as status,a.no_piutang,a.no_dokumen,a.no_fp,CONVERT(varchar,a.tanggal,103) as tgl,a.keterangan,a.akun_piutang,a.nilai+a.nilai_ppn-ISNULL(b.bayar,0) as saldo,a.kode_pp "+
						 "from piutang_m a "+						 
						 "     left join ("+
						 ...
						 "where a.kode_cust ='"+this.cb_cust.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' and a.nilai+a.nilai_ppn-ISNULL(b.bayar,0) >0 order by a.tanggal";			
	...
	doTampilData1: function(page) {
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU1.rs.rows[i];																
			this.sg1.appendData([line.status.toUpperCase(),line.no_piutang,line.no_dokumen,line.no_fp,line.tgl,line.keterangan,line.akun_piutang,floatToNilai(line.saldo),line.kode_pp]);			
		}
	...
	doChangeCell1: function(sender, col, row){						
		...
		if (col == 0) {
			this.dataJU1.rs.rows[((this.page1 - 1)*20) + row].status = this.sg1.cells(0,row);
			this.sg1.validasi();
		}
	},		
	doDoubleClick1: function(sender, col , row) {
		...
		if (this.dataJU1.rs.rows[((this.page1 - 1)*20) + row].status == "INPROG") {
			this.sg1.cells(0,row,"BAYAR");
			this.dataJU1.rs.rows[((this.page1 - 1)*20) + row].status = "BAYAR";			
		}
		else {
			this.sg1.cells(0,row,"INPROG");
			this.dataJU1.rs.rows[((this.page1 - 1)*20) + row].status = "INPROG";			
		}
	},
	...
	doLoad3:function(sender){		
		var strSQL = "select a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,b.kode_cust+' - '+ b.nama as cust, a.nilai "+
		             "from kas_m a "+
					 "inner join cust b on a.no_link=b.kode_cust and a.kode_lokasi=b.kode_lokasi "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'KBPIUM' and a.posted ='F'";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_kas,line.tgl,line.no_dokumen,line.keterangan,line.cust,floatToNilai(line.nilai)]); 
		}
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						this.cb_cust.setSQL("select kode_cust, nama from cust where kode_cust='"+line.no_link+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_cust","nama"],false,["Kode","Nama"],"and","Data Cust",true);									
					}
				}
				
				var strSQL = "select 'BAYAR' as status,a.periode,a.no_piutang,a.no_dokumen,a.no_fp,CONVERT(varchar,a.tanggal,103) as tgl,a.keterangan,a.akun_piutang,a.nilai+a.nilai_ppn-ISNULL(b.bayar,0) as saldo, a.kode_pp "+
							 "from piutang_m a  "+
				...
				// 	for (var i in data.rs.rows){
				// 		line = data.rs.rows[i];							
				// 		this.sg1.appendData([line.status.toUpperCase(),line.no_piutang,line.no_dokumen,line.no_fp,line.tanggal,line.keterangan,line.akun_piutang,floatToNilai(line.saldo),line.kode_pp]);
				// 	}
				// } else this.sg1.clear(1);
				this.dataJU1 = {rs:{rows:[]}};	
				var data = this.dbLib.getDataProvider(strSQL,true);
