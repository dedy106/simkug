window.app_saku3_transaksi_kopeg_fAkruBatal = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_kopeg_fAkruBatal.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_kopeg_fAkruBatal";
		...
		this.pc1 = new pageControl(this.pc3.childPage[0],{bound:[1,12,995,300], childPage:["Daftar Akru"]});
...
window.app_saku3_transaksi_kopeg_fAkruBatal.extend(window.portalui_childForm);
window.app_saku3_transaksi_kopeg_fAkruBatal.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{																	
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from kop_simpangs_j where no_angs = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
					...
					sql.add("insert into kop_simpangs_m(no_angs,no_dokumen,keterangan,tanggal,nilai,nilai_lain,nilai_sls,modul,periode,kode_lokasi,posted,kode_pp,nik_app,nik_user,tgl_input,no_kas) values  "+
							"('"+this.e_nb.getText()+"','-','"+this.e_desc.getText()+"','"+this.dp_d1.getDateString()+"',"+nilaiToFloat(this.e_tot.getText())+",0,0,'BTLBILL','"+this.e_periode.getText()+"','"+this.app._lokasi+"','F','"+this.app._kodePP+"','"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.e_nb.getText()+"')");
					...
					for (var i=0;i < this.sg.getRowCount();i++){
						if (this.sg.rowValid(i) && this.sg.cells(0,i) == "BATAL"){							
							...
							sql.add("insert into kop_simpangs_j (no_angs,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+	
									"('"+this.e_nb.getText()+"','"+this.sg.cells(1,i)+"','"+this.dp_d1.getDateString()+"',"+idx+",'"+this.sg.cells(7,i)+"','"+this.e_desc.getText()+"','D',"+nilai+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','BTLBILL','APSIMP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");							
							idx++;
							sql.add("insert into kop_simpangs_j (no_angs,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+	
									"('"+this.e_nb.getText()+"','"+this.sg.cells(1,i)+"','"+this.dp_d1.getDateString()+"',"+idx+",'"+this.sg.cells(6,i)+"','"+this.e_desc.getText()+"','C',"+nilai+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','BTLBILL','ARSIMP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");							
							idx++;
	...
	doLoad3:function(sender){																				
		var strSQL = "select a.no_angs,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
		             "from kop_simpangs_m a "+
					 "where a.kode_lokasi='"+this.app._lokasi+"' and a.periode='"+this.e_periode.getText()+"' and a.posted='F' and a.modul='BTLBILL'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_angs,line.tgl,line.keterangan,floatToNilai(line.nilai)]); 
		}
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.tanggal,a.keterangan, b.no_simp,c.no_agg "+
							 ...
							 "where a.no_angs = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";						
				var data = this.dbLib.getDataProvider(strSQL,true);
