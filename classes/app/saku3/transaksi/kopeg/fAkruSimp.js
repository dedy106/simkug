window.app_saku3_transaksi_kopeg_fAkruSimp = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_kopeg_fAkruSimp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_kopeg_fAkruSimp";
		...
		this.sg2 = new portalui_saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:8,			    
...
window.app_saku3_transaksi_kopeg_fAkruSimp.extend(window.portalui_childForm);
window.app_saku3_transaksi_kopeg_fAkruSimp.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,2])){
				try{													
					...
					sql.add("insert into kop_simpangs_m(no_angs,no_dokumen,keterangan,tanggal,nilai,nilai_lain,nilai_sls,modul,periode,kode_lokasi,posted,kode_pp,nik_app,nik_user,tgl_input,no_kas) values  "+
							"('"+this.e_nb.getText()+"','-','"+this.e_desc.getText()+"','"+this.dp_d1.getDateString()+"',0,0,0,'GENBILL','"+this.e_periode.getText()+"','"+this.app._lokasi+"','X','"+this.app._kodePP+"','"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.e_nb.getText()+"')");
					...
					for (var i=0; i<this.sg2.getRowCount(); i++){
						if (this.sg2.rowValid(i)){
							...
							sql.add("insert into kop_simpbill_j(no_bill,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
									"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg2.cells(3,i)+"','"+this.e_desc.getText()+"','D',"+nilaiToFloat(this.sg2.cells(7,i))+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','GENBILL','PIUTANG','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");							
							sql.add("insert into kop_simpbill_j(no_bill,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
									"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sg2.cells(5,i)+"','"+this.e_desc.getText()+"','C',"+nilaiToFloat(this.sg2.cells(7,i))+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','GENBILL','SIMP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");
						}
					}					
	...
	doClick:function(sender){
		...
		this.stsSimpan = 1;
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"kop_simpbill_m","no_bill",this.app._lokasi+"-BSM"+this.e_periode.getText().substr(2,4)+".","0000"));
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select * from kop_simpbill_m "+							 
