window.app_saku3_transaksi_kb_fKbTitip = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_kb_fKbTitip.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_kb_fKbTitip";
		...
		this.sg3 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:9,tag:9,
		            colTitle:["No Bukti","Tanggal","Jenis","No Dokumen","Akun Titipan","Kode Vendor","Nama Vendor","Deskripsi","Nilai"],
		...
		this.cb_vendor = new saiCBBL(this.pc2.childPage[0],{bound:[20,13,220,20],caption:"Kode Vendor", multiSelection:false, maxLength:10, tag:2});				
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,235], childPage:["Item Jurnal"]});
...
window.app_saku3_transaksi_kb_fKbTitip.extend(window.childForm);
window.app_saku3_transaksi_kb_fKbTitip.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_titip (no_kas,kode_lokasi,akun_titip,keterangan,kode_pp,modul,periode,kode_curr,kurs,nilai,dc,kode_vendor, no_agenda) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_titip.getText()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBTITIP','"+this.e_periode.getText()+"','IDR',1,"+nilaiToFloat(this.e_debet.getText())+",'D','"+this.cb_vendor.getText()+"', '"+this.cb_agenda.getText()+"')");
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.jenis,a.no_dokumen,a.keterangan,a.nilai,b.kode_vendor,b.akun_titip,c.nama as nama_vendor "+
		             "from kas_m a "+				
					 "inner join kas_titip b on a.no_kas=b.no_kas and a.kode_lokasi=b.kode_lokasi "+
					 "inner join it_vendor c on b.kode_vendor=c.kode_vendor and b.kode_lokasi=c.kode_lokasi "+
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.keterangan,a.no_dokumen,a.jenis,a.tanggal,a.akun_kb,b.kode_vendor,b.no_agenda "+
							 "from kas_m a "+			
							 "inner join kas_titip b on a.no_kas=b.no_kas and a.kode_lokasi=b.kode_lokasi "+
