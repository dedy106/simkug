window.app_saku3_transaksi_kb_fKbSPB = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_kb_fKbSPB.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_kb_fKbSPB";
		...
		this.sg5 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:8,tag:0,
		            colTitle:["Status","No SPB","No Fiat","Tanggal","Due Date","Deskripsi","Nilai","Catatan Fiatur"],
					colWidth:[[7,6,5,4,3,2,1,0],[160,100,210,70,70,120,120,80]],colFormat:[[6],[cfNilai]],										
...
window.app_saku3_transaksi_kb_fKbSPB.extend(window.childForm);
window.app_saku3_transaksi_kb_fKbSPB.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("update panjar2_m set no_kas='-' where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("update if_reim_m set no_kas='-',progress='2' where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("update inv_depo2_m set no_kas='-',progress='0' where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					for (var i=0;i < this.dataJU5.rs.rows.length;i++){
						...
						if (line.status.toUpperCase() == "BAYAR") {
							...
							sql.add("update panjar2_m set no_kas='"+this.e_nb.getText()+"' where no_spb = '"+line.no_spb+"' and kode_lokasi='"+this.app._lokasi+"'");
							sql.add("update if_reim_m set no_kas='"+this.e_nb.getText()+"',progress='3' where no_spb = '"+line.no_spb+"' and kode_lokasi='"+this.app._lokasi+"'");							
							sql.add("update a set a.no_kas='"+this.e_nb.getText()+"', progress='1' "+
									"from inv_depo2_m a inner join inv_shop_m b on a.no_shop=b.no_shop and a.kode_lokasi=b.kode_lokasi "+
							...
							//jika droping kirim
							sql.add("update yk_kasdrop_d set no_kas='"+this.e_nb.getText()+"',progress='1' where no_spb = '"+line.no_spb+"' and kode_lokasi='"+this.app._lokasi+"'");							
						}
					}
					...
					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"','"+this.sg1.cells(7,i)+"','-','-','"+this.app._lokasi+"','KBSPB','NONKB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.sg1.cells(4,i))+")");										
							}
						}
					}					
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"('"+this.e_nb.getText()+"','KBSPB','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(6,i))+","+nilai+")");
							}
						}
					}					
					setTipeButton(tbAllFalse);					
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "simpan" :															
			case "ubah" :															
				var strSQL = "select progress from panjar2_m where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){								
						if (line.progress != "2") {
							system.alert(this,"Transaksi tidak valid.","Progress Panjar sudah pertanggungan.");
							return false;						
						}
					}
				}
	...
	doLoad:function(sender){		
		this.dataJU5 = {rs:{rows:[]}};
		var strSQL = "select 'INPROG' as status,a.no_spb,a.no_fiat,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as duedate,a.keterangan,a.nilai,case b.catatan when '' then '-' else b.catatan end as catatan "+
		             "from spb_m a inner join app_d b on a.no_spb=b.no_bukti and a.kode_lokasi=b.kode_lokasi "+
					 "             inner join app_m c on b.no_app=c.no_app and c.kode_lokasi=b.kode_lokasi and c.no_appseb='-'  and c.modul='FIAT' "+
	...
	doTampilData5: function(page) {		
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU5.rs.rows[i];									
			this.sg5.appendData([line.status.toUpperCase(),line.no_spb,line.no_fiat,line.tgl,line.duedate,line.keterangan,floatToNilai(line.nilai),line.catatan]);
		}
	...
	doDoubleClick5: function(sender, col , row) {
		...
		if (noSPB != "") {
			var strSQL = "select a.kode_vendor,b.nama,a.bank,a.cabang,a.no_rek,a.nama_rek,a.nilai,a.keterangan "+
						 ...
						 "from spb_d a inner join karyawan b on a.kode_vendor=b.nik  "+ 
						 "where a.no_spb in ("+noSPB+") and a.kode_lokasi='"+this.app._lokasi+"' "+
						 "union "+
					     ...
					     "where a.no_spb in ("+noSPB+") and a.kode_lokasi='"+this.app._lokasi+"' "+
						 "union "+
						 "select '-' as kode_vendor,b.aw as nama,a.bank,a.cabang,a.no_rek,a.nama_rek,a.nilai,b.keterangan "+
						 "from spb_d a inner join yk_dakem_m b on a.no_spb=b.no_spb and a.kode_lokvendor=b.kode_lokasi "+
						 "where a.no_spb in ("+noSPB+") and a.kode_lokasi='"+this.app._lokasi+"' "+
						 "union "+
						 "select a.kode_vendor,b.nama,a.bank,a.cabang,a.no_rek,a.nama_rek,a.nilai,a.keterangan "+
						 "from spb_d a inner join inv_bank b on a.kode_vendor=b.kode_bank "+
						 "where a.no_spb in ("+noSPB+") and a.kode_lokasi='"+this.app._lokasi+"' "+						 
						 " ";						 
