window.app_saku3_transaksi_kb_fKbResTM = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_kb_fKbResTM.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_kb_fKbResTM";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,325], childPage:["Daftar Billing","Jurnal KasBank","Jurnal Tambahan","Data Anggaran"]});		
		this.sg5 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:8,tag:0,
		            colTitle:["Status","No Bill","Tanggal","Jenis","Deskripsi","Nilai BP","Nilai CC","Lok Asal"],
					colWidth:[[7,6,5,4,3,2,1,0],[60,100,100,300,70,70,100,80]],colFormat:[[5,6],[cfNilai,cfNilai]],										
					readOnly:true,buttonStyle:[[0],[bsAuto]],picklist:[[0],[new portalui_arrayMap({items:["BAYAR","INPROG"]})]],
					change:[this,"doChangeCell5"],autoAppend:false,defaultRow:1});		
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:8,tag:0,
		...
		this.sg1 = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:9,tag:9,
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('GARFREE','DOKFREE','HUTBP','HUTCC') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku3_transaksi_kb_fKbResTM.extend(window.childForm);
window.app_saku3_transaksi_kb_fKbResTM.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from angg_r where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='KBRESTM'");
						sql.add("update dbtm.dbo.yk_bill_m set no_kasres='-' where no_kasres = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}					
					for (var i=0;i < this.dataJU5.rs.rows.length;i++){
						line = this.dataJU5.rs.rows[i];
						if (line.status.toUpperCase() == "BAYAR") {
							var k = i +100;
							sql.add("update dbtm.dbo.yk_bill_m set no_kasres='"+this.e_nb.getText()+"' where no_bill = '"+line.no_bill+"' and kode_lokasi='"+this.app._lokasi+"'");							
							sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
									"('"+this.e_nb.getText()+"','"+line.no_bill+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.akunHutBP+"','"+line.keterangan+"','D',"+parseFloat(line.bp)+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBRESTM','HUTBP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+parseFloat(line.bp)+")");
							sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
									"('"+this.e_nb.getText()+"','"+line.no_bill+"','"+this.dp_d1.getDateString()+"',"+k+",'"+this.akunHutCC+"','"+line.keterangan+"','D',"+parseFloat(line.cc)+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBRESTM','HUTCC','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+parseFloat(line.cc)+")");											
						}
					}
					
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBRESTM','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','-','-','"+this.cb_rek.getText()+"')");
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(2,i)+"','"+this.sg.cells(4,i)+"','"+this.sg.cells(3,i).toUpperCase()+"',"+nilaiToFloat(this.sg.cells(5,i))+",'"+this.app._kodePP+"','-','"+this.sg.cells(6,i)+"','-','"+this.app._lokasi+"','KBRESTM','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'"+this.sg.cells(0,i)+"',"+nilaiToFloat(this.sg.cells(5,i))+")");
							}
						}
					}
					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"','"+this.sg1.cells(7,i)+"','-','-','"+this.app._lokasi+"','KBRESTM','NONKB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.sg1.cells(4,i))+")");										
							}
						}
					}					
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"('"+this.e_nb.getText()+"','KBRESTM','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(6,i))+","+nilai+")");
							}
						}
					}					
					setTipeButton(tbAllFalse);					
	...
	doLoad:function(sender){				
		this.dataJU5 = {rs:{rows:[]}};
		var strSQL = "select 'INPROG' as status,a.no_bill,convert(varchar,a.tanggal,103) as tgl,a.jenis,a.keterangan,a.kode_lokasi,ISNULL(b.bp,0) as bp,ISNULL(c.cc,0) as cc "+
					 "from dbtm.dbo.yk_bill_m a "+
					 "left join ( "+
					 "	select a.no_bill,a.kode_lokasi,SUM(a.nilai) as bp "+
					 "	from dbtm.dbo.yk_bill_d a inner join dbtm.dbo.yk_loker b on a.loker=b.loker "+
					 "					 		  inner join dbtm.dbo.cust c on b.kode_cust=c.kode_cust and c.jenis<>'PENSIUN' "+
					 "	where a.jenis='RESYKT' and a.kode_lokasi='"+this.app._lokasi+"' "+
					 "	group by a.no_bill,a.kode_lokasi "+
					 ") b on a.no_bill=b.no_bill and a.kode_lokasi=b.kode_lokasi "+
					 "left join ( "+
					 "	select a.no_bill,a.kode_lokasi,SUM(a.nilai) as cc "+
					 "	from dbtm.dbo.yk_bill_d a inner join dbtm.dbo.yk_loker b on a.loker=b.loker "+
					 "					          inner join dbtm.dbo.cust c on b.kode_cust=c.kode_cust and c.jenis='PENSIUN' "+
					 "	where a.jenis='RESYKT' and a.kode_lokasi='"+this.app._lokasi+"' "+
					 "	group by a.no_bill,a.kode_lokasi "+
					 ") c on a.no_bill=c.no_bill and a.kode_lokasi=c.kode_lokasi "+
					 "where a.kode_lokasi='"+this.app._lokasi+"' and a.no_kasres='-' and a.jenis='RESYKT' and a.periode <='"+this.e_periode.getText()+"' order by a.periode";						
	...
	doTampilData5: function(page) {		
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU5.rs.rows[i];									
			this.sg5.appendData([line.status.toUpperCase(),line.no_bill,line.tgl,line.jenis,line.keterangan,floatToNilai(line.bp),floatToNilai(line.cc),line.kode_lokasi]);
		}
		...
		try {
			if (this.cb_rek.getText() != "" && this.e_ket.getText()!="") {
				...
				for (var i=0;i < this.dataJU5.rs.rows.length;i++){
					line = this.dataJU5.rs.rows[i];
					if (line.status.toUpperCase() == "BAYAR") {
						totBP += parseFloat(line.bp);
						totCC += parseFloat(line.cc);
					}
				}
				
				this.sg.clear();
				this.sg.appendData([this.cb_rek.getText(),this.cb_rek.rightLabelCaption,"-","C",this.e_ket.getText(),floatToNilai(totBP),"13","-"]);
				this.sg.appendData([this.cb_rek.getText(),this.cb_rek.rightLabelCaption,"-","C",this.e_ket.getText(),floatToNilai(totCC),"17","-"]);
				for (var i=0;i < this.sg.getRowCount();i++){					
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.jenis,a.no_dokumen,a.keterangan,a.nilai "+
		             "from kas_m a "+					 					 
					 "where a.no_del='-' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'KBRESTM' and a.posted ='F'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				this.dataJU5 = {rs:{rows:[]}};
				var strSQL = "select 'BAYAR' as status,a.no_bill,convert(varchar,a.tanggal,103) as tgl,a.jenis,a.keterangan,a.kode_lokasi,ISNULL(b.bp,0) as bp,ISNULL(c.cc,0) as cc "+
					 "from dbtm.dbo.yk_bill_m a "+
					 "left join ( "+
					 "	select a.no_bill,a.kode_lokasi,SUM(a.nilai) as bp "+
					 "	from dbtm.dbo.yk_bill_d a inner join dbtm.dbo.yk_loker b on a.loker=b.loker "+
					 ...
					 "	group by a.no_bill,a.kode_lokasi "+
					 ") c on a.no_bill=c.no_bill and a.kode_lokasi=c.kode_lokasi "+
					 "where a.kode_lokasi='"+this.app._lokasi+"' and a.no_kasres='"+this.e_nb.getText()+"' and a.jenis='RESYKT'";						
				...
				var data = this.dbLib.getDataProvider(
							...
							"where a.no_bukti = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='KBRESTM' order by a.kode_akun",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg2.clear();
					for (var i in data.rs.rows){
