window.app_saku3_transaksi_kb_fKbMasukTkDrk = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_kb_fKbMasukTkDrk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_kb_fKbMasukTkDrk";
		...
		try {
			...
			var sql="select a.kode_ref, a.nama from refju_m a inner join refju_pp b on a.kode_ref=b.kode_ref and a.kode_lokasi=b.kode_lokasi and b.kode_pp='"+this.app._kodePP+"' where a.kode_lokasi='"+this.app._lokasi+"' and a.jenis='PDPT'  ";
...
window.app_saku3_transaksi_kb_fKbMasukTkDrk.extend(window.childForm);
window.app_saku3_transaksi_kb_fKbMasukTkDrk.implement({	
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',1,'"+this.akunKredit+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_nilai.getText())+",'"+this.app._kodePP+"','"+this.cb_drk.getText()+"','-','-','"+this.app._lokasi+"','KBDUAL','KREDIT','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.e_nilai.getText())+")");					
					...
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
							"	('"+this.e_nb.getText()+"','KBDUAL','"+this.app._lokasi+"','"+this.akunKredit+"','"+this.app._kodePP+"','"+this.cb_drk.getText()+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','C',0,"+nilaiToFloat(this.e_nilai.getText())+")");							
		...
		switch (event){
			...
			case "ubah" :					
				...
				if (this.c_jenis.getText() == "PENDAPATAN") {
					...
					// alert("select kode_flag from flag_relasi where kode_akun='"+this.akunDebet+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doChange:function(sender){
		try {
			...
			if (sender == this.cb_jurnal && this.cb_jurnal.getText()!="" && this.stsSimpan == 1) {
				...
				// alert("select nama,debet,kredit,0 as nilai from refju_m where kode_ref='"+this.cb_jurnal.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");
