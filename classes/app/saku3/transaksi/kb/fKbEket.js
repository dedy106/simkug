window.app_saku3_transaksi_kb_fKbEKet = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_kb_fKbEKet.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_kb_fKbEKet";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Edit Keterangan KasBank Umum", 0);	
		...
		this.pc2 = new pageControl(this,{bound:[10,10,1000,410], childPage:["List Jurnal","Data Jurnal"]});		
		...
		this.c_jenis = new portalui_saiLabelEdit(this.pc2.childPage[1],{bound:[20,11,200,20],caption:"Jenis",readOnly:true,tag:2});		
		...
		this.pc1 = new pageControl(this.pc2.childPage[1],{bound:[5,12,990,280], childPage:["Data Item Jurnal"]});
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:7,tag:0,
					...
					columnReadOnly:[true,[0,1,2,3,4,5,6],[3]],
					colFormat:[[4],[cfNilai]],
...
window.app_saku3_transaksi_kb_fKbEKet.extend(window.childForm);
window.app_saku3_transaksi_kb_fKbEKet.implement({		
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from kas_m where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.cb_lokasi.getText()+"' and periode='"+this.e_periode.getText()+"'");
					sql.add("delete from kas_j where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.cb_lokasi.getText()+"' and periode='"+this.e_periode.getText()+"'");
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.cb_lokasi.getText()+"','"+this.e_dok.getText()+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','"+this.vModul+"','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','T','-','-','-','-')");																
					
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','-','-','-','"+this.cb_lokasi.getText()+"','"+this.vModul+"','UMUM','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.sg.cells(4,i))+")");										
							}
						}
					}										
					...
					sql.add("insert into gldt_h(no_bukti,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,kode_curr,kurs,nilai_curr,tgl_input,nik_user,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik) "+
							"select no_kas,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,kode_curr,kurs,nilai_curr,tgl_input,nik_user,'-','-','-','-','-','-' "+
							"from kas_j where nilai <> 0 and kode_lokasi='"+this.cb_lokasi.getText()+"' and no_kas='"+this.e_nb.getText()+"' and periode='"+this.e_periode.getText()+"'");
	    			...
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								...
								this.filter2 = " where a.kode_lokasi='"+this.cb_lokasi.getText()+"' and a.no_kas='"+this.e_nb.getText()+"' ";
	...
	doLoad3:function(sender){																		
		if (this.cb_lokasi.getText() !="" && this.e_periode.getText() != "") {
			var strSQL = "select a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.jenis,a.no_dokumen,a.keterangan "+
						 "from kas_m a "+					 					 
						 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.cb_lokasi.getText()+"' and a.posted ='T'";		
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select keterangan,no_dokumen,jenis,tanggal,modul "+
							 "from kas_m "+							 
							 "where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.cb_lokasi.getText()+"'";						
				var data = this.dbLib.getDataProvider(strSQL,true);
				...
				var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp "+
							...
							"where a.no_kas = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.cb_lokasi.getText()+"' order by a.no_urut",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
					for (var i in data.rs.rows){
