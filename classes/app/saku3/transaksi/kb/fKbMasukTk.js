window.app_saku3_transaksi_kb_fKbMasukTk = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_kb_fKbMasukTk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_kb_fKbMasukTk";
		...
		try {
			...
			//var sql="select a.kode_jurnal, a.nama from refju_dual a where a.kode_pp='"+this.app._kodePP+"' and a.modul='KB' and a.kode_lokasi='"+this.app._lokasi+"' and a.jenis='PDPT'  "; 
			...
			this.cb_jurnal.setSQL(sql,["a.kode_ref","a.nama"],false,["Kode","Nama"],"and","Data Referensi",true);						
...
window.app_saku3_transaksi_kb_fKbMasukTk.extend(window.childForm);
window.app_saku3_transaksi_kb_fKbMasukTk.implement({	
	...
	doChange:function(sender){
		try {
			...
			if (sender == this.cb_jurnal && this.cb_jurnal.getText()!="" && this.stsSimpan == 1) {
				...
				var data = this.dbLib.getDataProvider("select nama,debet,kredit,0 as nilai from refju_m where kode_ref='"+this.cb_jurnal.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ",true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){										
						this.e_ket.setText(line.nama);
						this.e_nilai.setText(floatToNilai(line.nilai));
						this.akunDebet = line.debet;
						this.akunKredit = line.kredit;					
					} 
				}															
			}
		}
