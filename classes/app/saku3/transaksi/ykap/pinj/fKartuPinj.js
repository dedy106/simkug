window.app_saku3_transaksi_ykap_pinj_fKartuPinj = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ykap_pinj_fKartuPinj.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ykap_pinj_fKartuPinj";
		...
		this.e_nb = new portalui_saiLabelEdit(this.pc1.childPage[0],{bound:[20,12,220,20],caption:"No Kartu",maxLength:30,readOnly:true});
		this.i_gen = new portalui_imageButton(this.pc1.childPage[0],{bound:[245,12,20,20],hint:"Generate",image:"icon/"+system.getThemes()+"/tabCont2.png",click:[this,"doClick"]});						
		this.l_tgl = new portalui_label(this.pc1.childPage[0],{bound:[20,13,100,18],caption:"Tgl Mulai Tagihan", underline:true});
		this.dp_d1 = new portalui_datePicker(this.pc1.childPage[0],{bound:[120,13,98,18],date:new Date().getDateStr(),selectDate:[this,"doSelectDate"]});
...
window.app_saku3_transaksi_ykap_pinj_fKartuPinj.extend(window.portalui_childForm);
window.app_saku3_transaksi_ykap_pinj_fKartuPinj.implement({
	...
	simpan: function(){			
		try{					
			if (this.stsSimpan == 1) {
				...
				if (this.cb_pinj.getText() == "Y-Housing") {
					var strSQL = "select * "+
								"from kop_pinj_m "+							 
								"where kode_param='"+this.cb_pinj.getText()+"' and no_agg = '"+this.cb_agg.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						
					var data = this.dbLib.getDataProvider(strSQL,true);
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
						if (line != undefined){								
							system.alert(this,"Sudah pernah mengajukan Pinjaman Y-Housing.","Transaksi tidak dapat diproses.");
							return false;
						}
					}	
				}
	...
	doChange: function(sender){
		try{			
			if (sender == this.cb_pinj && this.cb_pinj.getText()!="") {
				var strSQL = "select akun_piutang,akun_pjasa,p_bunga from kop_pinj_param "+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						if (this.stsSimpan==1) this.e_bunga.setText(floatToNilai(line.p_bunga));
					}
				}
			}
	...
	doLoad1:function(sender){																		
		...
		var strSQL = "select a.no_pinj,a.no_agg,c.nama,a.jenis_angs,a.nilai "+
					 ...
					 "where "+filter+" a.kode_lokasi='"+this.app._lokasi+"' and a.no_kas='-' and a.no_kasasur='-'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
