window.app_saku3_transaksi_ykap_pinj_fKbPinjTunaiCB = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ykap_pinj_fKbPinjTunaiCB.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ykap_pinj_fKbPinjTunaiCB";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Penerimaan Angsuran Tunai [Cash Basis]", 0);	
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-33],colCount:11,tag:0,
				colTitle:["Status","No Akru","No Kartu","Deskripsi","Akun Pokok","Akun Jasa","Periode","Cil Ke-","N Pokok","N Jasa","Tagihan"],
...
window.app_saku3_transaksi_ykap_pinj_fKbPinjTunaiCB.extend(window.childForm);
window.app_saku3_transaksi_ykap_pinj_fKbPinjTunaiCB.implement({
	...
	doChange:function(sender){		
		try	{
			...
			if (sender == this.cb_agg && this.cb_agg.getText()!="" && this.stsSimpan==1) {			
				var strSQL = "select b.no_bill,a.no_pinj,c.keterangan,a.akun_piutang,e.akun_jasa,b.periode,b.cicilan_ke,b.npokok,b.nbunga,(b.npokok+b.nbunga)-isnull(d.bayar,0) as saldo "+
							 ...
							 "					 inner join kop_pinj_param e on a.kode_param=e.kode_param and a.kode_lokasi=e.kode_lokasi "+
							 "      left outer join "+  
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];												
						this.sg1.appendData(["INPROG",line.no_bill,line.no_pinj,line.keterangan,line.akun_piutang,line.akun_jasa,line.periode,line.cicilan_ke,floatToNilai(line.npokok),floatToNilai(line.nbunga),floatToNilai(line.saldo)]);
					}
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select b.no_bill,a.no_pinj,c.keterangan,a.akun_piutang,f.akun_jasa,b.periode,b.cicilan_ke,b.npokok,b.nbunga,(b.npokok+b.nbunga)-isnull(d.bayar,0) as saldo "+
							 "from  kop_pinj_m a inner join kop_pinj_sch b on a.no_pinj=b.no_pinj  and a.kode_lokasi=b.kode_lokasi "+
							 "                   inner join kop_pinjbill_m c on b.no_bill=c.no_bill and b.kode_lokasi=c.kode_lokasi "+							 
							 "					 inner join kop_pinjangs_d e on b.no_bill=e.no_bill and b.no_pinj=e.no_pinj and b.kode_lokasi=e.kode_lokasi "+
							 "					 inner join kop_pinj_param f on a.kode_param=f.kode_param and a.kode_lokasi=f.kode_lokasi "+
							 "      left outer join "+  
							 "              (select y.no_pinj, y.no_bill, y.kode_lokasi, sum(case dc when 'D' then y.npokok+nbunga else -(y.npokok+nbunga) end) as bayar "+
							 "               from kop_pinjangs_d y inner join kop_pinjangs_m x on y.no_angs=x.no_angs and y.kode_lokasi=x.kode_lokasi "+
							 "               where y.no_agg = '"+this.cb_agg.getText()+"' and y.kode_lokasi='"+this.app._lokasi+"' "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];												
						this.sg1.appendData(["BAYAR",line.no_bill,line.no_pinj,line.keterangan,line.akun_piutang,line.akun_jasa,line.periode,line.cicilan_ke,floatToNilai(line.npokok),floatToNilai(line.nbunga),floatToNilai(line.saldo)]);
					}
				} else this.sg1.clear(1);
				this.sg1.validasi();
