window.app_saku3_transaksi_ykap_pinj_fKbPinjCairKomp = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ykap_pinj_fKbPinjCairKomp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ykap_pinj_fKbPinjCairKomp";
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-33],colCount:10,tag:0,
				colTitle:["Status","No Agg","Nama","No Kartu","Jenis","Akun AR","Nilai Pinj","Biaya","Kompensasi","Nilai Cair"],
				colWidth:[[9,8,7,6,5,4,3,2,1,0],[80,80,80,80,80,80,120,250,80,80]],
...
window.app_saku3_transaksi_ykap_pinj_fKbPinjCairKomp.extend(window.childForm);
window.app_saku3_transaksi_ykap_pinj_fKbPinjCairKomp.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("update a set a.no_ref1='-' "+
								"from kop_pinj_m a inner join kop_pinj_m b on a.no_ref1=b.no_pinj and a.kode_lokasi=b.kode_lokasi and b.no_kas='"+this.e_nb.getText()+"' "+
					...
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && this.sg1.cells(0,i) == "CAIR"){
							...
							if (nilaiToFloat(this.sg1.cells(8,i)) != 0) {
								var j = 400+i;								
								sql.add("update kop_pinj_m set no_ref1='"+this.sg1.cells(3,i)+"' where no_agg='"+this.sg1.cells(1,i)+"' and status_aktif='K' and no_ref1='-' and kode_lokasi='"+this.app._lokasi+"'");		
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3)  "+		
										
										"select '"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.sg1.cells(3,i)+"','"+this.dp_d1.getDateString()+"',"+j+",a.akun_piutang,'C',a.komp,a.komp,'Kompensasi '+a.no_pinj,'PINJCAIR','AR','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-' "+
										"from  "+
										"			(select a.no_pinj,c.akun_piutang,sum(a.npokok+a.nbunga) as komp "+
										"			from kop_pinjangs_d a "+
										"				inner join kop_pinjangs_m b  on a.no_angs=b.no_angs and a.kode_lokasi=b.kode_lokasi and b.modul='KOMP' "+
										"				inner join kop_pinj_m c on a.no_pinj=c.no_pinj and a.kode_lokasi=c.kode_lokasi and c.status_aktif='K' and c.no_ref1='"+this.sg1.cells(3,i)+"' "+
										"			where a.kode_lokasi='"+this.app._lokasi+"' "+
										"			group by a.no_pinj,c.akun_piutang) a "+
										"  ");
							}
						}
					}														
	...
	doTampilClick: function(sender){
		try{			
			if (this.e_periode.getText() != "") {
				this.sg1.clear(1);				
				var strSQL = "select a.no_agg,b.nama,a.no_pinj,a.kode_param,a.akun_piutang,a.nilai,nilai_mat+nilai_prov+nilai_asur as biaya,isnull(c.komp,0) as komp,nilai-(nilai_mat+nilai_prov+nilai_asur)-isnull(c.komp,0) nilai_cair "+
							 "from  kop_pinj_m a inner join kop_agg b on a.no_agg=b.no_agg and a.kode_lokasi=b.kode_lokasi "+	

							 "		left join (select a.no_agg,a.kode_lokasi,sum(a.npokok+a.nbunga) as komp "+
							 "				   from kop_pinjangs_d a "+
							 "								inner join kop_pinj_m b on a.no_pinj=b.no_pinj and a.kode_lokasi=b.kode_lokasi and b.status_aktif='K' and b.no_ref1='-' "+
							 "								inner join kop_pinjangs_m c on a.no_angs=c.no_angs and a.kode_lokasi=c.kode_lokasi and c.modul='KOMP' "+
							 "				   where a.kode_lokasi='"+this.app._lokasi+"'	group by a.no_agg,a.kode_lokasi) c on a.no_agg=c.no_agg and a.kode_lokasi=c.kode_lokasi "+						 
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];												
						this.sg1.appendData(["INPROG",line.no_agg,line.nama,line.no_pinj,line.kode_param,line.akun_piutang,floatToNilai(line.nilai),floatToNilai(line.biaya),floatToNilai(line.komp),floatToNilai(line.nilai_cair)]);
					}
	...
	doNilaiChange1: function(){
		try{			
			var saldo = 0;			
			for (var i = 0; i < this.sg1.getRowCount();i++) {
				if (this.sg1.rowValid(i) && this.sg1.cells(0,i)=="CAIR" && this.sg1.cells(9,i) != "") {
					saldo += nilaiToFloat(this.sg1.cells(9,i));
				}
			}			
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.no_agg,b.nama,a.no_pinj,a.kode_param,a.akun_piutang,a.nilai,nilai_mat+nilai_prov+nilai_asur as biaya,isnull(c.komp,0) as komp,nilai-(nilai_mat+nilai_prov+nilai_asur)-isnull(c.komp,0)  nilai_cair "+
							 ...
							 "		left join (select b.no_ref1,a.no_agg,a.kode_lokasi,sum(a.npokok+a.nbunga) as komp "+
							 "				   from kop_pinjangs_d a "+
							 "								inner join kop_pinj_m b on a.no_pinj=b.no_pinj and a.kode_lokasi=b.kode_lokasi and b.status_aktif='K' and b.no_ref1<>'-' "+
							 "								inner join kop_pinjangs_m c on a.no_angs=c.no_angs and a.kode_lokasi=c.kode_lokasi and c.modul='KOMP' "+
							 "				   where a.kode_lokasi='"+this.app._lokasi+"' group by b.no_ref1,a.no_agg,a.kode_lokasi) c on a.no_pinj=c.no_ref1 and a.no_agg=c.no_agg and a.kode_lokasi=c.kode_lokasi "+						 
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];												
						this.sg1.appendData(["CAIR",line.no_agg,line.nama,line.no_pinj,line.kode_param,line.akun_piutang,floatToNilai(line.nilai),floatToNilai(line.biaya),floatToNilai(line.komp),floatToNilai(line.nilai_cair)]);
					}
				} else this.sg1.clear(1);
				this.sg1.validasi();
