window.app_saku3_transaksi_ykap_kug_fKb = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ykap_kug_fKb.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ykap_kug_fKb";
		...
		this.pc2 = new pageControl(this,{bound:[10,10,1000,430], childPage:["Entry Data","List Jurnal","Filter Cari"]});		
		...
		this.c_jenis = new saiCB(this.pc2.childPage[0],{bound:[20,17,200,20],caption:"Jenis",items:["BK","BM"], readOnly:true,tag:2,change:[this,"doChange"]});
		this.e_nb = new portalui_saiLabelEdit(this.pc2.childPage[0],{bound:[20,12,200,20],caption:"No Bukti",maxLength:30,readOnly:true});
		this.i_gen = new portalui_imageButton(this.pc2.childPage[0],{bound:[225,12,20,20],hint:"Generate",image:"icon/"+system.getThemes()+"/tabCont2.png",click:[this,"doClick"]});		
		this.e_dok2 = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,14,320,20],caption:"No Dokumen",maxLength:50});				
		this.c_jtran = new saiCB(this.pc2.childPage[0],{bound:[20,11,200,20],caption:"Jenis Transaksi",items:["KESEHATAN","THT"], readOnly:true,tag:2,change:[this,"doChange"]});
		this.c_status = new saiCB(this.pc2.childPage[0],{bound:[790,11,200,20],caption:"Status",items:["TUNAI","MCM","TRANSFER","CHEQUE","GIRO"], readOnly:true,tag:2});		
		...
		this.e_debet = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,14,200,20],caption:"Total Debet", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,12,990,257], childPage:["Data Item Jurnal","Approval"]});
		...
		this.cb_buat = new saiCBBL(this.pc1.childPage[1],{bound:[20,10,220,20],caption:"Dibuat Oleh",  maxLength:10, tag:2, readOnly:true});	//multiSelection:false,	
		this.cb_periksa = new saiCBBL(this.pc1.childPage[1],{bound:[20,11,220,20],caption:"Diperiksa Oleh", multiSelection:false, maxLength:10, tag:2});		
		...
		try {
			...
			this.cb_buat.setSQL("select nik, nama from karyawan where kode_lokasi='"+this.app._lokasi+"' and flag_aktif='1'",["nik","nama"],false,["NIK","Nama"],"where","Daftar Karyawan",true);			
			this.cb_buat.setText(this.app._userLog,this.app._namaUser);
			this.cb_periksa.setSQL("select nik, nama from karyawan where kode_lokasi='"+this.app._lokasi+"' and flag_aktif='1'",["nik","nama"],false,["NIK","Nama"],"where","Daftar Karyawan",true);			
							
			this.c_jenis.setText("BK");
			this.c_status.setText("");
			
			this.c_jtran.setText("");
			this.c_jtran.setText("THT");
...
window.app_saku3_transaksi_ykap_kug_fKb.extend(window.childForm);
window.app_saku3_transaksi_ykap_kug_fKb.implement({
	...
	simpan: function(){			
		try{						
			...
			if (this.e_periode.getText().substr(2,2) != this.e_nb.getText().substr(2,2)) {
				system.alert(this,"Transaksi tidak valid.","No Bukti tidak sesuai Tahun-nya.");
				return false;
			}			
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','KB','KB','F','-','-','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','"+this.e_dok2.getText()+"','"+this.e_ket.getText()+"','IDR',1,"+
							parseNilai(this.e_debet.getText())+",0,0,'"+this.cb_buat.getText()+"','"+this.cb_periksa.getText()+"','-','"+this.e_bank.getText()+"','"+this.e_dok.getText()+"','-','"+this.c_jtran.getText()+"','"+this.c_status.getText()+"','"+this.c_jenis.getText()+"')");
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										...
										parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(3,i)+"','KB','"+this.c_jenis.getText()+"','IDR',1,'"+this.sg.cells(5,i)+"','-','-','-','-','-','-','-','-')");
							}
						}
					}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "simpan" :																	
			case "ubah" :				
				var strSQL = "select no_bukti from trans_m where modul='KB' and no_dokumen='"+this.e_dok2.getText()+"' and no_bukti <> '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){							
						system.alert(this,"Transaksi tidak valid.","No Dokumen telah terpakai utk No Bukti : "+line.no_bukti);
						return false;
					}
				}			
				this.preView = "1";		
					...
					if (this.sg.rowValid(j)){
						for (var i=0;i<this.dataJU.rs.rows.length;i++){
							...
							if (line.kode_akun == this.sg.cells(0,j)) {
								akunKB = true;	
								if (this.c_jenis.getText() == "BK" && this.sg.cells(2,j) == "C") posisiDC = true;
								if (this.c_jenis.getText() == "BM" && this.sg.cells(2,j) == "D") posisiDC = true;
							}
						}
					}
				}
				...
				if (!posisiDC) {
					system.alert(this,"Transaksi tidak valid.","Akun KasBank tidak sesuai posisi dengan jenis transaksi.(DC)");
					return false;						
				}
	...
	doChange:function(sender){
		if (sender == this.c_jtran && this.c_jtran.getText()!="") {				
			this.sg.clear(1);

			var sql = new server_util_arrayList();
			if (this.c_jtran.getText() == "KESEHATAN") 
				sql.add("select a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '051' where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'");									
			if (this.c_jtran.getText() == "THT") 
				sql.add("select a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '050' where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'");									
	...
	doEllipsClick: function(sender, col, row){
		try{			
			if (sender == this.sg) {
				if (col == 0){
					if (this.c_jtran.getText() == "KESEHATAN") {
						str1 = "select a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '051' where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'";
						str2 = "select count(a.kode_akun) from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '051' where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'";
					}
					if (this.c_jtran.getText() == "THT") {
						str1 = "select a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '050' where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'";
						str2 = "select count(a.kode_akun) from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '050' where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'";
					}
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_yakap_rptKbJurnalBukti";
	...
	doCari:function(sender){																		
		...
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.param2 as status,a.no_ref1+' - '+a.no_ref2 as no_ref,a.keterangan,a.nilai1 "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'KB' and a.posted ='F' and a.form='KB' "+filter;				
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						this.c_jtran.setText(line.param1);	
