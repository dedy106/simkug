window.app_saku3_transaksi_ykap_kug_fJP = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ykap_kug_fJP.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ykap_kug_fJP";
		...
		try {
			...
			/*
			akun JP pakai kodeflag 051 dan 050 krn mapping beda tiap akun
...
window.app_saku3_transaksi_ykap_kug_fJP.extend(window.childForm);
window.app_saku3_transaksi_ykap_kug_fJP.implement({	
	...
	simpan: function(){			
		try{			
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"jp","no_bukti",this.app._lokasi+"-JP"+this.e_periode.getText().substr(2,4)+".","000"));
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.isPostedAll(this.app._periode)) {	
						...
						var totD = totC = 0;
						var sql2 = "select a.kode_akun,a.kode_pp,a.so_awal,a.debet,a.kredit,a.so_akhir "+
								   ...
								   //akun SPI
								   "select a.kode_akun,a.kode_pp,a.so_awal,a.debet,a.kredit,a.so_akhir "+
								   "from glma_pp_tmp a "+
								   "inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag in ('052','053') "+
								   "inner join masakun c on c.kode_akun = a.kode_akun and c.kode_lokasi = a.kode_lokasi and c.modul <> 'L' "+
								   "where a.kode_lokasi ='"+this.app._lokasi+"' and a.nik_user = '"+this.nik_user+"' and round(a.so_akhir,0) <> 0 ";
						...
						//---- jurnal akun JP -------------- flag relasi di 050/051 		  
						var sql2 = "select b.kode_flag,sum(a.so_akhir) as so_akhir "+
									"from glma_pp_tmp a "+
									"inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag in ('050','051') "+
									"inner join masakun c on c.kode_akun = a.kode_akun and c.kode_lokasi = a.kode_lokasi and c.modul = 'L' "+
									"where a.kode_lokasi ='"+this.app._lokasi+"' and a.nik_user = '"+this.nik_user+"' and round(a.so_akhir,0) <> 0 "+
									"group by b.kode_flag";										
					   	var data = this.dbLib.getDataProvider(sql2,true);
					   	if (typeof data == "object" && data.rs.rows[0] != undefined) {
						   var line;
						   var nilai = 0;							
						   for (var i in data.rs.rows){
							   line = data.rs.rows[i];							
							  	if (line.kode_flag == "050") {
									var akunJP = "321.1001.1";
									if (parseFloat(line.so_akhir) > 0) var dc = "D"; else var dc = "C";
									nilai = Math.abs(parseFloat(line.so_akhir));
									sqlJp.add("insert into gldt (no_bukti, kode_lokasi, tgl_input, nik_user, periode, no_dokumen, tanggal, nu, kode_akun, dc, nilai, nilai_curr, keterangan, modul, jenis, kode_curr, kurs, kode_pp, kode_drk, kode_cust, kode_vendor, no_fa, no_selesai, no_ref1, no_ref2, no_ref3) values "+
											"('"+this.e_nb.getText()+"', '"+this.app._lokasi+"', getdate(), '"+this.app._userLog+"', '"+this.e_periode.getText()+"', '-', '"+this.dp_d1.getDateString()+"', 99998, '"+akunJP+"', '"+dc+"', "+nilai+", "+nilai+", '"+this.e_ket.getText()+"', 'JP', 'JP', 'IDR', 1, '"+this.app._kodePP+"', '-', '-', '-', '-', '-', '-', '-', '-')");	

								}
								if (line.kode_flag == "051") {
									var akunJP = "321.1001.2";
									if (parseFloat(line.so_akhir) > 0) var dc = "D"; else var dc = "C";
									nilai = Math.abs(parseFloat(line.so_akhir));
									sqlJp.add("insert into gldt (no_bukti, kode_lokasi, tgl_input, nik_user, periode, no_dokumen, tanggal, nu, kode_akun, dc, nilai, nilai_curr, keterangan, modul, jenis, kode_curr, kurs, kode_pp, kode_drk, kode_cust, kode_vendor, no_fa, no_selesai, no_ref1, no_ref2, no_ref3) values "+
											"('"+this.e_nb.getText()+"', '"+this.app._lokasi+"', getdate(), '"+this.app._userLog+"', '"+this.e_periode.getText()+"', '-', '"+this.dp_d1.getDateString()+"', 99999, '"+akunJP+"', '"+dc+"', "+nilai+", "+nilai+", '"+this.e_ket.getText()+"', 'JP', 'JP', 'IDR', 1, '"+this.app._kodePP+"', '-', '-', '-', '-', '-', '-', '-', '-')");	
						...
						//---- jurnal akun JP SPI -------------- flag relasi di 052/053, dijurnal dgn keterangan per nama akun						  
						var sql2 = "select b.kode_flag,a.kode_akun,c.nama,sum(a.so_akhir) as so_akhir "+
									"from glma_pp_tmp a "+
									"inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag in ('052','053') "+
									"inner join masakun c on c.kode_akun = a.kode_akun and c.kode_lokasi = a.kode_lokasi and c.modul <> 'L' "+
									"where a.kode_lokasi ='"+this.app._lokasi+"' and a.nik_user = '"+this.nik_user+"' and round(a.so_akhir,0) <> 0 "+
									"group by b.kode_flag,a.kode_akun,c.nama";										
					   	var data = this.dbLib.getDataProvider(sql2,true);
					   	if (typeof data == "object" && data.rs.rows[0] != undefined) {
						   var line;
						   var nilai = 0;							
						   for (var i in data.rs.rows){
							   line = data.rs.rows[i];							
							  	if (line.kode_flag == "052") {
									var akunJP = "321.1001.1";
									if (parseFloat(line.so_akhir) > 0) var dc = "D"; else var dc = "C";
									nilai = Math.abs(parseFloat(line.so_akhir));
									sqlJp.add("insert into gldt (no_bukti, kode_lokasi, tgl_input, nik_user, periode, no_dokumen, tanggal, nu, kode_akun, dc, nilai, nilai_curr, keterangan, modul, jenis, kode_curr, kurs, kode_pp, kode_drk, kode_cust, kode_vendor, no_fa, no_selesai, no_ref1, no_ref2, no_ref3) values "+
											"('"+this.e_nb.getText()+"', '"+this.app._lokasi+"', getdate(), '"+this.app._userLog+"', '"+this.e_periode.getText()+"', '-', '"+this.dp_d1.getDateString()+"', 99931, '"+akunJP+"', '"+dc+"', "+nilai+", "+nilai+", '"+line.nama+"', 'JP', 'JPSPI', 'IDR', 1, '"+this.app._kodePP+"', '-', '-', '-', '-', '-', '-', '-', '-')");	

								}
								if (line.kode_flag == "053") {
									var akunJP = "321.1001.2";
									if (parseFloat(line.so_akhir) > 0) var dc = "D"; else var dc = "C";
									nilai = Math.abs(parseFloat(line.so_akhir));
									sqlJp.add("insert into gldt (no_bukti, kode_lokasi, tgl_input, nik_user, periode, no_dokumen, tanggal, nu, kode_akun, dc, nilai, nilai_curr, keterangan, modul, jenis, kode_curr, kurs, kode_pp, kode_drk, kode_cust, kode_vendor, no_fa, no_selesai, no_ref1, no_ref2, no_ref3) values "+
											"('"+this.e_nb.getText()+"', '"+this.app._lokasi+"', getdate(), '"+this.app._userLog+"', '"+this.e_periode.getText()+"', '-', '"+this.dp_d1.getDateString()+"', 99932, '"+akunJP+"', '"+dc+"', "+nilai+", "+nilai+", '"+line.nama+"', 'JP', 'JPSPI', 'IDR', 1, '"+this.app._kodePP+"', '-', '-', '-', '-', '-', '-', '-', '-')");	
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "simpan" :					
				...
				var totSaldo = 0;				
				sql = "select round( sum(a.so_akhir) ,0) as tot_sakhir "+
					  "from glma_pp_tmp a where a.kode_lokasi ='"+this.app._lokasi+"' and a.nik_user = '"+this.nik_user+"' and a.so_akhir <> 0";
				var data = this.dbLib.getDataProvider(sql,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					this.dataJU = data;
					var line = this.dataJU.rs.rows[0];
					totSaldo += parseFloat(line.tot_sakhir);								
				} 

				if (totSaldo != 0) {
					system.alert(this,"Closing tidak valid.","Neraca Lajur PP tidak balance.");
					return false;
				}
				else this.simpan();
				break;		
