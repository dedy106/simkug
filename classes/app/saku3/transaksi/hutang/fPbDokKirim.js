window.app_saku3_transaksi_hutang_fPbDokKirim = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_hutang_fPbDokKirim.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_hutang_fPbDokKirim";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Permohonan Bayar via Dok Kirim: Input/Edit", 0);	
		...
		this.e_totalRek = new saiLabelEdit(this.pc2.childPage[0],{bound:[770,13,220,20],caption:"Tot Rek + Pajak", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,348], childPage:["Data dan Otorisasi","Atensi Pembayaran","Jurnal","Data Anggaran","Data Approval"]});
		...
		this.sg4 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-90],colCount:8,tag:9,
		            colTitle:["No Bukti","Tanggal","Due Date","No Dokumen","Deskripsi","Bidang","Lok Asal","Nilai"],
					colWidth:[[7,6,5,4,3,2,1,0],[100,60,120,300,100,80,80,100]],
					readOnly:true,colFormat:[[7],[cfNilai]],
					dblClick:[this,"doDoubleClick4"],autoAppend:false,defaultRow:1});		
		this.sgn4 = new portalui_sgNavigator(this.pc1.childPage[0],{bound:[1,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:bsAll,grid:this.sg4,pager:[this,"doPager4"]});				
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:0,
					...
					columnReadOnly:[true,[],[0,1,2,3,4,5,6,8]],					
					colFormat:[[6,7],[cfNilai,cfNilai]],checkItem: true,					
					nilaiChange:[this,"doNilaiChange"],autoPaging:true,rowPerPage:100,
		...
		this.sg1 = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:0,
		...
		try {
			...
			sql.add("select kode_pp, nama  from pp where kode_bidang='"+this.app._kodeBidang+"' and kode_lokasi = '"+this.app._lokasi+"' and tipe='posting' and flag_aktif ='1'");	
...
window.app_saku3_transaksi_hutang_fPbDokKirim.extend(window.childForm);
window.app_saku3_transaksi_hutang_fPbDokKirim.implement({		
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					var sql = new server_util_arrayList();
					if (this.stsSimpan == 0) {
						sql.add("update dokkirim_m set no_terima='-',progress='0' where no_kirim = '"+this.nodok+"' and kode_lokasi='"+this.kodeLokAsal+"'");
					...
					sql.add("update dokkirim_m set no_terima='"+this.e_nb.getText()+"',progress='1' where no_kirim = '"+this.nodok+"' and kode_lokasi='"+this.kodeLokAsal+"'");
						...
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){
								sql.add("insert into yk_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"','"+this.sg1.cells(7,i)+"','"+this.app._lokasi+"','PBGAR','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");										
							}
						}
					}														
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into yk_pb_d(no_pb,kode_lokasi,kode_lokvendor,kode_vendor,bank,cabang,no_rek,nama_rek,nilai,pajak,keterangan) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.kodeLokAsal+"','"+this.sg.cells(0,i)+"','"+this.sg.cells(2,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(4,i)+"','"+this.sg.cells(5,i)+"',"+nilaiToFloat(this.sg.cells(6,i))+","+nilaiToFloat(this.sg.cells(7,i))+",'"+this.sg.cells(8,i)+"')");
							}
						}
					}					
		...
		try{
			...
			for (var i = 0; i < this.sg.getRowCount();i++){
				...
				if (this.sg.rowValid(i) && this.sg.cells(7,i) != ""){										
					totRek += nilaiToFloat(this.sg.cells(7,i));					
				}
			}
			for (var i = 0; i < this.sg1.getRowCount();i++){
			...
			if (sender == this.sg1) {
				...
				if (col == 5){
					this.standarLib.showListData(this, "Daftar PP/Unit",sender,undefined, 
							"select kode_pp, nama  from pp where kode_bidang='"+this.app._kodeBidang+"' and kode_lokasi = '"+this.app._lokasi+"' and tipe='posting' and flag_aktif ='1'",
							"select count(kode_pp) from pp where kode_bidang='"+this.app._kodeBidang+"' and kode_lokasi = '"+this.app._lokasi+"' and tipe='posting' and flag_aktif ='1'",
	...
	doLoad3:function(sender){																				
		...
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.modul,a.no_dokumen,a.keterangan,a.progress,a.nilai "+
		             "from yk_pb_m a "+					 					 
					 "where "+pp+" a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'PBDOK' and a.progress in ('0','R','V') ";
	...
	doLoad:function(sender){						
		if (this.app._userStatus == "A") var bidang = ""; else var bidang = " a.kode_bidang = '"+this.app._kodeBidang+"' and "; 
		var strSQL = "select a.no_kirim,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as duedate,a.no_dokumen,a.keterangan,a.kode_bidang+' - '+b.nama as bidang,a.kode_lokasi,a.nilai "+
		             "from dokkirim_m a inner join bidang b on a.kode_bidang=b.kode_bidang "+
					 "where "+bidang+" a.no_terima = '-' and a.progress='0' and a.periode<='"+this.e_periode.getText()+"' and a.kode_loktuj='"+this.app._lokasi+"' and a.modul = 'PBNONGAR' ";					 
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU4 = data;
	...
	doTampilData4: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg4.appendData([line.no_kirim,line.tgl,line.duedate,line.no_dokumen,line.keterangan,line.bidang,line.kode_lokasi,floatToNilai(line.nilai)]); 
		}
	...
	doDoubleClick4: function(sender, col , row) {
		if (this.sg4.cells(0,row) != "") {
			this.nodok = this.sg4.cells(0,row);
			this.kodeLokAsal = this.sg4.cells(6,row);
			var data = this.dbLib.getDataProvider("select a.kode_vendor,b.nama,a.bank,a.cabang,a.no_rek,a.nama_rek,a.nilai,a.pajak,a.keterangan "+
												  ...
												  "where a.no_kirim='"+this.nodok+"' and a.kode_lokasi='"+this.kodeLokAsal+"'",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
				for (var i in data.rs.rows){
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider("select no_kirim,kode_lokasi from dokkirim_m where no_terima='"+this.e_nb.getText()+"' and kode_loktuj='"+this.app._lokasi+"'",true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){													
						this.kodeLokAsal = line.kode_lokasi;
						this.nodok = line.no_kirim;
					}
				}				
				
				if (this.progSeb == "V") {
					var modulApp = "PBDOK_SPB"; 
				...
				var strSQL = "select a.no_ver as no_verlama,a.no_app as no_applama,a.keterangan,a.no_dokumen,a.modul,a.due_date,a.tanggal,a.nik_tahu,a.nik_app,a.progress,isnull(b.no_app,'-') as no_app,isnull(b.catatan,'-') as catatan "+
				...
				var data = this.dbLib.getDataProvider(
							...
							"where a.no_bukti = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='PBDOK' order by a.kode_akun",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg2.clear();
					for (var i in data.rs.rows){
				...
				var strSQL = "select a.no_kirim,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as duedate,a.no_dokumen,a.keterangan,a.kode_bidang+' - '+b.nama as bidang,a.kode_lokasi,a.nilai "+
							 "from dokkirim_m a inner join bidang b on a.kode_bidang=b.kode_bidang "+
							 "where a.no_terima = '"+this.e_nb.getText()+"' and a.kode_loktuj='"+this.app._lokasi+"' and a.progress='1' ";					 
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					this.dataJU4 = data;
					this.sgn4.setTotalPage(Math.ceil(data.rs.rows.length/20));
