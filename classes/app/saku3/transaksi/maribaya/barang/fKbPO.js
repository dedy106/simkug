window.app_saku3_transaksi_maribaya_barang_fKbPO = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_maribaya_barang_fKbPO.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_maribaya_barang_fKbPO";
		...
		this.bLoad = new button(this.pc2.childPage[0],{bound:[690,19,80,18],caption:"Tampil  Data",click:[this,"doLoad"]});
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-33],colCount:7,tag:0,
				colTitle:["No PO","Tanggal","Keterangan","Kode PP","Akun Hutang","Tagihan","Nilai Bayar"],
				colWidth:[[6,5,4,3,2,1,0],[100,100,80,80,300,80,120]],
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('HUTDUM','HUTBELI','UMBELI') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku3_transaksi_maribaya_barang_fKbPO.extend(window.childForm);
window.app_saku3_transaksi_maribaya_barang_fKbPO.implement({
	doLoad : function() {
		this.pc2.setActivePage(this.pc2.childPage[0]);	
		this.pc1.setActivePage(this.pc1.childPage[0]);																		
		if (this.stsSimpan==1 && this.cb_vendor.getText()!="") {				
			var strSQL = "select a.no_po,convert(varchar,a.tanggal,103) as tgl,a.keterangan,c.kode_pp, "+
									 "case when a.no_ref2='-' then '"+this.hutdum+"' else '"+this.hutbeli+"' end as akun_hutang, "+
									 "a.nilai1-isnull(b.bayar,0) as saldo  "+	
									 "from mb_po_m a "+
									
									 "		 inner join mb_pesan_m c on a.no_ref1=c.no_pesan and a.kode_lokasi=c.kode_lokasi "+
									 "     left join ("+
									 ...
									 "			group by no_beli "+
									 ") b on a.no_po=b.no_beli "+
									
									 "where a.param1='"+this.cb_vendor.getText()+"' and (a.nilai1 - isnull(b.bayar,0)) > 0 and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_po";						 
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg1.appendData([line.no_po,line.tgl,line.keterangan,line.kode_pp,line.akun_hutang,floatToNilai(line.saldo),"0"]);					
				}
				...
				try{														
					...
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && nilaiToFloat(this.sg1.cells(6,i)) > 0){							
							sql.add("insert into brg_belibayar_d(no_bukti,kode_lokasi,no_beli,kode_vendor,periode,dc,modul,nilai,nik_user,tgl_input,no_ref1) values "+
