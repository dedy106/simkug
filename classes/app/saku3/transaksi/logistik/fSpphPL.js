window.app_saku3_transaksi_logistik_fSpphPL = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_logistik_fSpphPL.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_logistik_fSpphPL";
...
window.app_saku3_transaksi_logistik_fSpphPL.extend(window.childForm);
window.app_saku3_transaksi_logistik_fSpphPL.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into log_spph_m(no_spph,kode_lokasi,tgl_input,nik_user,periode,tanggal,nik_buat,nik_app,keterangan,no_tap,no_dokumen,due_date,modul,no_sanggup) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.e_ket.getText()+"','-','"+this.e_dok.getText()+"','"+this.dp_d2.getDateString()+"','PL','-')");					
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					this.cb_pesan.setSQL("select a.no_pesan,a.keterangan from log_pesan_m a "+
								 ...
								 "where a.progress='2' and a.lok_proses = '"+this.app._lokasi+"' and c.jenis = 'PL' and a.periode<='"+this.e_periode.getText()+"' ",
			...
			case "ubah" :	
				...
				if (jmlVendor <= 1 && this.e_jenis.getText().substr(0,2) == "PL"){
					system.alert(this,"Transaksi tidak valid.","Mitra untuk pemilihan langsung harus lebih dari 1.");
					return false;						
				}
	...
	doChange:function(sender){
		...
		if (sender == this.cb_pesan && this.stsSimpan ==1) {
			...
			if (line2.jenis == "PL") this.e_jenis.setText("PL - PEMILIHAN LANGSUNG");			
	...
	doLoad3:function(sender){																		
		var strSQL = "select distinct a.no_spph,convert(varchar,a.tanggal,103) as tgl,a.keterangan "+
					 ...
					 "where b.no_sph is null and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_tap='-' and a.modul='PL' ";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
