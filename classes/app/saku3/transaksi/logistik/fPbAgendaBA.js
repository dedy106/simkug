window.app_saku3_transaksi_logistik_fPbAgendaBA = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_logistik_fPbAgendaBA.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_logistik_fPbAgendaBA";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,325], childPage:["Otorisasi","Data Akru","Data Permohonan","Referensi Akru","Data Approval","Data Anggaran"]});
		...
		this.e_nilai = new saiLabelEdit(this.pc1.childPage[2],{bound:[20,12,220,20],caption:"Nilai Final", tag:4, tipeText:ttNilai, text:"0",change:[this,"doChange"]});				
		this.e_total = new saiLabelEdit(this.pc1.childPage[2],{bound:[750,12,220,20],caption:"Total PB", tag:4, tipeText:ttNilai, text:"0"});				
				
		this.sg1 = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,97],colCount:9,tag:9,
		...
		this.sg5 = new saiGrid(this.pc1.childPage[3],{bound:[1,5,this.pc1.width-5,this.pc1.height-40],colCount:9,tag:9,
		            colTitle:["Kode Akun","Nama Akun","DC","Keterangan","Nilai","Kode PP","Nama PP","Kode DRK","Nama DRK"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[150,80,150,80,100,270,50,200,80]],					
		...
		this.sg2 = new saiGrid(this.pc1.childPage[5],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:9,
		...
		try {
			...
			sql.add("select kode_pp, nama  from pp where kode_lokasi = '"+this.app._lokasi+"' and tipe='posting' and flag_aktif ='1' and kode_bidang='"+this.app._kodeBidang+"'");						
...
window.app_saku3_transaksi_logistik_fPbAgendaBA.extend(window.childForm);
window.app_saku3_transaksi_logistik_fPbAgendaBA.implement({		
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,4])){
				try{														
					...
					sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,user_input,sts_pajak,form) values "+
							"('"+this.e_agenda.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','LOG','"+this.e_akun.getText()+"','"+this.app._kodePP+"','-','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_total.getText())+",getdate(),'"+this.app._userLog+"','-','-','-','A','-','-','"+this.app._namaUser.substr(0,50)+"','NON','LOGBA')");						
					sql.add("insert into it_aju_rek(no_aju,kode_lokasi,bank,no_rek,nama_rek,bank_trans,nilai,keterangan) values "+
							"('"+this.e_agenda.getText()+"','"+this.app._lokasi+"','"+this.e_bank.getText() +" - "+ this.e_cabang.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"','-',"+nilaiToFloat(this.e_total.getText())+",'-')");							
	...
	doLoad3:function(sender){																						
		...
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.modul,a.no_dokumen,a.keterangan,a.progress,a.nilai "+
		             "from yk_pb_m a "+					 					 
					 "where "+pp+" a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'PBAKRU' and a.progress in ('0','R','V') ";					 
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
