window.app_saku3_transaksi_logistik_fPjLog = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_logistik_fPjLog.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_logistik_fPjLog";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Input Permohonan Panjar Logistik: Input", 0);	
...
window.app_saku3_transaksi_logistik_fPjLog.extend(window.childForm);
window.app_saku3_transaksi_logistik_fPjLog.implement({
		...
		try{									
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					//tidak masuk ke dummy spph,sph,nego,tap
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into panjar2_d(no_panjar,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.cb_pesan.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i)+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','"+this.app._lokasi+"','SPB','BBN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
							}
						}
					}																			
					setTipeButton(tbAllFalse);					
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					setTipeButton(tbAllFalse);					
					this.cb_pesan.setSQL("select a.no_pesan,a.keterangan from log_pesan_m a inner join log_justerima_m c on a.no_pesan=c.no_pesan where a.periode<='"+this.e_periode.getText()+"' and a.progress='2' and a.lok_proses = '"+this.app._lokasi+"' and c.jenis in ('CC')",["no_pesan","keterangan"],false,["No Request","Deskripsi"],"and","Data Request",true);			
				break;
			case "simpan" :	
			case "ubah" :								
				this.preView = "1";
	...
	doChange:function(sender){		
		if (sender == this.cb_pesan && this.cb_pesan.getText()!="") {
			var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk,a.nilai "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];												
					this.sg.appendData([line.kode_akun,line.nama_akun,line.dc,this.cb_pesan.rightLabelCaption,floatToNilai(line.nilai),line.kode_pp,line.nama_pp,line.kode_drk,line.nama_drk]);
				}
