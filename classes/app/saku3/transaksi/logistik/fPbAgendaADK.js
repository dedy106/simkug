window.app_saku3_transaksi_logistik_fPbAgendaADK = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_logistik_fPbAgendaADK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_logistik_fPbAgendaADK";
		...
		try {
			...
			if (typeof data == "object"){
				...
				for (var i in data.rs.rows){
					...
					//if (line.kode_spro == "HUTLOG") this.akunHutang = line.flag; //ambil dari vendor (akun hutang)							
				}
			}						
...
window.app_saku3_transaksi_logistik_fPbAgendaADK.extend(window.childForm);
window.app_saku3_transaksi_logistik_fPbAgendaADK.implement({		
	...
	simpan: function(){			
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,4])){
				try{														
					...
					sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,user_input,sts_pajak,form) values "+
							"('"+this.e_agenda.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','LOG','"+this.akunHutang+"','"+this.app._kodePP+"','-','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",getdate(),'"+this.app._userLog+"','-','-','-','A','-','-','"+this.app._namaUser.substr(0,50)+"','NON','LOGADK')");						
					sql.add("insert into it_aju_rek(no_aju,kode_lokasi,bank,no_rek,nama_rek,bank_trans,nilai,keterangan) values "+
							"('"+this.e_agenda.getText()+"','"+this.app._lokasi+"','"+this.e_bank.getText() +" - "+ this.e_cabang.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"','-',"+nilaiToFloat(this.e_nilai.getText())+",'-')");												
			...
			//posted di buat T dulu, dibalikan normal ke posted = F		
	...
	doChange:function(sender){
		...
		if (sender == this.cb_spk && this.cb_spk.getText() != "") {
			var strSQL = "select b.kode_vendor,b.nama,b.bank,b.cabang,b.no_rek,b.nama_rek,a.total - isnull(c.adk,0) as saldo, b.akun_hutang "+
			             "from log_spk_m a "+
