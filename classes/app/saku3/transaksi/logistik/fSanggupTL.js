window.app_saku3_transaksi_logistik_fSanggupTL = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_logistik_fSanggupTL.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_logistik_fSanggupTL";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Kesanggupan Penunjukan Langsung", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,240], childPage:["Item Barang","File Dok"]});						
...
window.app_saku3_transaksi_logistik_fSanggupTL.extend(window.childForm);
window.app_saku3_transaksi_logistik_fSanggupTL.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into log_sanggup_m(no_sanggup,no_spk,kode_lokasi,tgl_input,nik_user,periode,tanggal,nik_buat,keterangan,no_tap,kode_vendor,total,no_dokumen,modul) values "+
							"('"+this.e_nb.getText()+"','-','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_buat.getText()+"','"+this.e_ket.getText()+"','"+this.cb_spph.getText()+"','"+this.kodeVendor+"',"+nilaiToFloat(this.e_total.getText())+",'"+this.e_dok.getText()+"','TL')");					
	...
	doChange:function(sender){
		...
		if (sender == this.cb_spk && this.cb_spk.getText()!="") {			
			var strSQL = "select a.no_spph,a.keterangan "+
			             "from log_spph_m a inner join log_tap_m b on a.no_spph=b.no_spph "+
						 "where b.no_spk='"+this.cb_spk.getText()+"'";
		...
		if (sender == this.cb_spph && this.cb_spph.getText()!="") {			
			...
			if (this.stsSimpan == 1) {				
				var strSQL = "select a.no_urut,a.item,a.merk,a.tipe,a.catatan,a.jumlah,a.nilai_nego,a.ppn_nego as ppn,a.jumlah*(a.nilai_nego+a.ppn_nego) as total, e.kode_klpfa, e.nama "+
							 ...
							 "                 inner join fa_klp e on d.kode_klpfa=e.kode_klpfa and g.kode_akun=e.kode_klpakun and a.kode_lokasi=e.kode_lokasi "+							 
		...
		if (this.e_periode.getText()!= "") {
			if (this.stsSimpan == 0) {					
				...
				this.cb_spk.setSQL("select d.no_spk, d.keterangan "+
								...
								"inner join log_tap_m d on a.no_tap=d.no_tap "+
								"inner join log_spk_m e on d.no_spk=e.no_spk "+
								"where e.periode<='"+this.e_periode.getText()+"' and d.no_sanggup='-' and d.kode_lokasi='"+this.app._lokasi+"'",["no_spk","keterangan"],false,["No SPK","Deskripsi SPK"],"and","Data SPK",true);											
			}
			this.stsSimpan = 1;
	...
	doLoad3:function(sender){																				
		var strSQL = "select a.no_sanggup,convert(varchar,a.tanggal,103) as tgl,a.keterangan,b.nama,a.total as nilai "+
					 ...
					 "where d.no_pks='-' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='TL'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
