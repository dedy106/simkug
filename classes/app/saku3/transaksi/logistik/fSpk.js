window.app_saku3_transaksi_logistik_fSpk = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_logistik_fSpk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_logistik_fSpk";
...
window.app_saku3_transaksi_logistik_fSpk.extend(window.childForm);
window.app_saku3_transaksi_logistik_fSpk.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into log_spk_m(no_spk,no_pks,kode_lokasi,tgl_input,nik_user,periode,tanggal,nik_buat,keterangan,no_tap,kode_vendor,total,no_dokumen,modul,no_sanggup) values "+
							"('"+this.e_nb.getText()+"','-','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_buat.getText()+"','"+this.e_ket.getText()+"','"+this.cb_tap.getText()+"','"+this.kodeVendor+"',"+nilaiToFloat(this.e_total.getText())+",'"+this.e_dok.getText()+"','PL','-')");					
			...
			case "clear" :
				if (modalResult == mrOk)
					...
					this.cb_tap.setSQL("select a.no_tap, a.keterangan from log_tap_m a "+
			                   ...
			                   "where a.no_spk='-' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='PL'",["no_tap","keterangan"],false,["No TAP","Deskripsi"],"and","Data Penetapan",true);			
				break;
			case "simpan" :															
			case "ubah" :																											
				this.preView = "1";												
	...
	doLoad3:function(sender){																				
		var strSQL = "select a.no_spk,convert(varchar,a.tanggal,103) as tgl,a.keterangan,b.nama,a.total as nilai "+
					 ...
					 "where a.no_sanggup='-' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_pks='-' and a.modul='PL'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				this.cb_tap.setSQL("select no_tap, keterangan from log_tap_m where no_spk='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='PL'",["no_tap","keterangan"],false,["No TAP","Deskripsi"],"and","Data Penetapan",true);			
