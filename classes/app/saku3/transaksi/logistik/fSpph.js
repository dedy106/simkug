window.app_saku3_transaksi_logistik_fSpph = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_logistik_fSpph.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_logistik_fSpph";
...
window.app_saku3_transaksi_logistik_fSpph.extend(window.childForm);
window.app_saku3_transaksi_logistik_fSpph.implement({	
	...
	doChange:function(sender){
		...
		if (sender == this.cb_pesan && this.stsSimpan ==1) {
			...
			sql.add("select kode_klpfa,nama from fa_klp where kode_klpakun = '"+this.akunAktap+"' and kode_lokasi='"+this.app._lokasi+"' union select '-','-' ");			
			...
			if (sender == this.sg5) {				
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Kelompok Barang",sender,undefined, 
							"select kode_klpfa, nama  from fa_klp where kode_klpfa<>'-' and kode_klpakun = '"+this.akunAktap+"' and kode_lokasi='"+this.app._lokasi+"'",
							"select count(kode_klpfa) from fa_klp where kode_klpfa<>'-' and kode_klpakun = '"+this.akunAktap+"' and kode_lokasi='"+this.app._lokasi+"'",
