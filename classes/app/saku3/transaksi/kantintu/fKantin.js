window.app_saku3_transaksi_kantintu_fKantin = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_kantintu_fKantin.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_kantintu_fKantin";
		...
		this.cb_kode = new saiCBBL(this.pc1.childPage[1],{bound:[20,12,220,20],caption:"Kantin/PP", maxLength:50,readOnly:true, tag:0, multiSelection:false,change:[this,"doChange"]});		
		this.e_nama = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,11,250,20],caption:"Nama", maxLength:50, tag:1});
		this.e_telp = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,13,250,20],caption:" No. Telp", maxLength:50, tag:1});
		this.e_alamat = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,12,360,20],caption:"Alamat", maxLength:50, tag:1});
		...
		try {
			...
			this.cb_kode.setSQL("select kode_pp, nama from pp where kode_lokasi='"+this.app._lokasi+"' ",["kode_pp","nama"],false,["Kode","Nama"],"and","Data PP",true);
...
window.app_saku3_transaksi_kantintu_fKantin.extend(window.childForm);
window.app_saku3_transaksi_kantintu_fKantin.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into ktu_kantin(kode_kantin,kode_lokasi,nama,pic,telp,alamat,akun_piutang,akun_titipan,akun_pdpt,akun_bymhd,akun_beban) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.cb_pic.getText()+"','"+this.e_telp.getText()+"','"+this.e_alamat.getText()+"','"+this.cb_piutang.getText()+"','"+this.cb_titipan.getText()+"','"+this.cb_pdpt.getText()+"','"+this.cb_bymhd.getText()+"','"+this.cb_beban.getText()+"')");
					
					sql.add("insert into kli_pp(kode_pp,kode_lokasi,flag_aktif,nama) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','1','"+this.e_nama.getText()+"')");
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select nama,pic,telp,alamat,akun_piutang,akun_titipan,akun_pdpt,akun_bymhd,akun_beban from ktu_kantin where kode_kantin ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
