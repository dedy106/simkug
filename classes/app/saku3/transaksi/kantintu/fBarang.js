window.app_saku3_transaksi_kantintu_fBarang = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_kantintu_fBarang.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_kantintu_fBarang";
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:5,tag:9,
		            colTitle:["Kode","Nama","Sat Kecil","Pabrik","HNA"],
		...
		this.cb_kode = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,10,300,20],caption:"Kode",maxLength:20,change:[this,"doChange"]});		
		...
		this.e_satkecil = new saiCB(this.pc1.childPage[1],{bound:[20,21,180,20],caption:"Sat Kecil",tag:2,items:["btl","pcs","ktk"],mustCheck:false});				
		this.e_hna = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,15,180,20],caption:"HNA", tag:1,  tipeText:ttNilai, text:"0", maxLength:10});			
...
window.app_saku3_transaksi_kantintu_fBarang.extend(window.childForm);
window.app_saku3_transaksi_kantintu_fBarang.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					...
					sql.add("insert into kli_obat(kode_obat,nama,kode_lokasi,sat_kecil,sat_besar,jml_sat,hna,pabrik,flag_gen,flag_aktif,ss,sm1,sm2,mm1,mm2,fm1,fm2) values "+
						    "('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.e_satkecil.getText()+"','-',1,"+nilaiToFloat(this.e_hna.getText())+",'"+this.e_pabrik.getText()+"','"+this.c_gen.getText()+"','1',"+nilaiToFloat(this.e_ss.getText())+","+nilaiToFloat(this.e_sm1.getText())+","+nilaiToFloat(this.e_sm2.getText())+","+nilaiToFloat(this.e_mm1.getText())+","+nilaiToFloat(this.e_mm2.getText())+","+nilaiToFloat(this.e_fm1.getText())+","+nilaiToFloat(this.e_fm2.getText())+")");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
		...
		switch (event){
			...
			case "simpan" :	
				...
				if (typeof data == "object"){
					...
					if (line != undefined){						
						system.alert(this,"Nama Barang Duplikasi.","Duplikasi dengan Kode  ["+line.kode_obat+"]");
						return false;
					}					
				}
				this.simpan();
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select * from kli_obat where kode_obat ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){						
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {			
				...
				this.e_pabrik.setText(this.sg1.cells(3,row));					
				this.e_satkecil.setText(this.sg1.cells(2,row));				
				this.e_hna.setText(this.sg1.cells(4,row));					
	...
	doLoad:function(sender){						
		var strSQL = "select kode_obat,nama,sat_kecil,hna,pabrik "+
		             "from kli_obat "+
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_obat,line.nama,line.sat_kecil,line.pabrik,floatToNilai(line.hna)]); 
		}
	...
	doCari:function(sender){								
		try {
			...
			var strSQL = "select kode_obat,nama,sat_kecil,,hna,pabrik,flag_gen "+
						"from kli_obat "+
