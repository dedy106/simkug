window.app_saku3_transaksi_kantintu_fCust = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_kantintu_fCust.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_kantintu_fCust";
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:6,tag:9,
		            colTitle:["Kode","Nama","Alamat","cp","No HP","Email"],
					colWidth:[[4,3,2,1,0],[150,80,80,300,200,60]],
					readOnly:true,
...
window.app_saku3_transaksi_kantintu_fCust.extend(window.childForm);
window.app_saku3_transaksi_kantintu_fCust.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into ktu_cust(kode_cust,kode_lokasi,nama,alamat,telp,hp,fax,email,cp) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.e_alamat.getText()+"','"+this.e_tel.getText()+"','"+this.e_hp.getText()+"','"+this.e_fax.getText()+"','"+this.e_email.getText()+"','"+this.e_cp.getText()+"')");
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					...
					sql.add("update ktu_cust set nama='"+this.e_nama.getText()+"',alamat='"+this.e_alamat.getText()+"',telp='"+this.e_tel.getText()+"',fax='"+this.e_fax.getText()+"',email='"+this.e_email.getText()+"',cp='"+this.e_cp.getText()+"' "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from ktu_cust where kode_cust = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");			
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select nama from ktu_cust where kode_cust ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
				...
				if (typeof data == "object"){
					...
					if (line != undefined){						
						...
						/*
						var strSQL = "select nama,flag_aktif,alamat,alamat2,no_tel,no_tel2,no_fax,email,npwp,cp,max_piu,max_hari "+
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {			
				...
				var strSQL = "select nama,alamat,telp,fax,email,cp "+
							 "from ktu_cust where kode_cust = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){													
