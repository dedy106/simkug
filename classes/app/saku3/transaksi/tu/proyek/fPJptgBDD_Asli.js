			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,user_input,sts_pajak,form) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.c_jenis.getText()+"','"+this.cb_akun.getText()+"','"+this.cb_pp.getText()+"','-','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",getdate(),'"+this.app._userLog+"','-','-','-','A','"+this.cb_nik.getText()+"','"+this.cb_panjar.getText()+"','"+this.e_user.getText()+"','NON','PRPJPTG')");
		...
		switch (event){
			...
			case "ubah"   :		
				...
				if (nilaiToFloat(this.e_saldoBMHD.getText()) > 0) {
					system.alert(this,"Transaksi tidak valid.","Saldo BYMHD masih ada.");
					return false;
				}
	...
	doChange:function(sender){
		if (sender == this.cb_nik && this.cb_nik.getText()!="" && this.stsSimpan==1) {	
			this.cb_panjar.setSQL("select a.no_aju,a.keterangan from it_aju_m a "+
								  "inner join tu_prbdd_d c on a.no_aju=c.no_bukti and a.kode_lokasi=c.kode_lokasi and c.modul='NONBDD' "+
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.progress,a.modul,a.tanggal,a.periode,a.keterangan,a.nilai,a.kode_pp,a.nik_panjar,isnull(x.no_ver,'-') as no_ver,isnull(x.catatan,'-') as catatan,a.kode_akun,c.no_aju as no_pj,c.keterangan as ket_pj,c.kode_akun as akun_pj,  "+
						     ...
						     "      inner join tu_prbdd_d cc on c.no_aju=cc.no_bukti and c.kode_lokasi=cc.kode_lokasi and cc.modul='NONBDD' "+
