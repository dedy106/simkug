window.app_saku3_transaksi_tu_proyek_fInisialBDD = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyek_fInisialBDD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyek_fInisialBDD";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan BDD", 0);	
		...
		this.cb_id = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,19,220,20],caption:"ID Kegiatan",tag:1,multiSelection:false,change:[this,"doChange"]}); 				
...
window.app_saku3_transaksi_tu_proyek_fInisialBDD.extend(window.childForm);
window.app_saku3_transaksi_tu_proyek_fInisialBDD.implement({	
	...
	doChange:function(sender){
		...
		if (sender==this.cb_id && this.cb_id.getText()!="") {
			var strSQL = "select a.nama,b.akun_bdd,a.nilai_or - isnull(c.bdd,0) as saldo "+
			             ...
			             "		from tu_prbdd_d where kode_lokasi='"+this.app._lokasi+"' and no_bukti <> '"+this.e_nb.getText()+"' and modul in ('AJUBDD','NONBDD') group by kode_proyek,kode_lokasi "+
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){							
							if (this.preView == "1") {
								this.nama_report="server_report_saku3_tu_proyek_rptBdd";
	...
	doLoad3:function(sender){		
		var strSQL = "select a.no_aju,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
		             ...
		             "where  a.form = 'INISIAL' and a.modul='UMUM' and a.progress in ('0','A','R') and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and b.nik='"+this.app._userLog+"'";	
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
