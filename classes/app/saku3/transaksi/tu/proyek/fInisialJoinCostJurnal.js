window.app_saku3_transaksi_tu_proyek_fInisialJoinCostJurnal = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyek_fInisialJoinCostJurnal.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyek_fInisialJoinCostJurnal";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Join Cost Inisiasi + Jurnal", 0);	
		...
		this.cb_drkKeg = new saiCBBL(this.pc1.childPage[0],{bound:[20,11,220,20],caption:"DRK Kegiatan",  tag:1,readOnly:true});
		...
		this.cb_proyek = new saiCBBL(this.pc1.childPage[0],{bound:[20,19,220,20],caption:"ID Proyek", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});
		this.c_periodeSch = new saiCB(this.pc1.childPage[0],{bound:[20,10,200,20],caption:"Periode Akru", readOnly:true,tag:1});
		
		this.sg2 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-10,180],colCount:8,tag:1,
		            colTitle:["ID Project","Nama Project","No SPK/PKS","Saldo OR","Nilai Sharing","PP","Periode PR","Kode DRK"],
					colWidth:[[7,6,5,4,3,2,1,0],[80,80,80,100,100,300,200,120]],					
					columnReadOnly:[true,[0,1,2,3,5,6,7],[4]],
					colHide:[[5,6,7],[true]],
					//buttonStyle:[[0],[bsEllips]], ----> 1 ke 1 (gara2 periode di cek utk jurnal reklas drk)
		...
		this.sgn2 = new portalui_sgNavigator(this.pc1.childPage[0],{bound:[1,this.pc1.height-25,this.pc1.width-6,25],buttonStyle:3,grid:this.sg2});		
...
window.app_saku3_transaksi_tu_proyek_fInisialJoinCostJurnal.extend(window.childForm);
window.app_saku3_transaksi_tu_proyek_fInisialJoinCostJurnal.implement({
		...
		try{		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					//16-09-17 ---> kt rudi posting semua gara bisa beda pp sama drk 
					/*				
					//cek periode inisiasi berbeda dgn periode proyek ??? jika beda maka ada jurnal yg harus diposting
					var posted = "X";
					for (var i=0;i < this.sg2.getRowCount();i++){
						if (this.periodKeg == this.sg2.cells(6,i)) {
							posted = "F";
						}
					}
					*/
					...
					sql.add("insert into tu_prbeban_m (no_bukti,no_dokumen,keterangan,tanggal,nilai,periode,kode_pp,kode_drk,kode_lokasi,nik_app,nik_user,tgl_input,posted,kode_curr,kurs,modul) values  "+
							"('"+this.e_nb.getText()+"','"+this.cb_id.getText()+"','"+this.e_uraian.getText()+"','"+this.dp_d1.getDateString()+"',"+nilaiToFloat(this.e_totalcost.getText())+",'"+this.e_periode.getText()+"','"+this.cb_pp.getText()+"','"+this.drkBeban+"','"+this.app._lokasi+"','-','"+this.app._userLog+"',getdate(),'"+posted+"','IDR',1,'JCOSTINI')");																		
					...
					sql.add("insert into tu_prbeban_j(no_bukti,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,periode_beban,kode_curr,kurs,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_id.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.akunBeban+"','"+this.e_uraian.getText()+"','C',"+nilaiToFloat(this.e_totalcost.getText())+",'"+this.cb_pp.getText()+"','"+this.cb_drkKeg.getText()+"','"+this.app._lokasi+"','JCOSTINI','BEBAN','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");																										

					//di proyek baru... nilai bdd ditambah, saldo bdd gak berubah karena langsung di bebankan
					//nilai bebannya bertambah
					if (this.sg2.getRowValidCount() > 0){
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){																
								sql.add("insert into tu_prbdd_d(no_bukti,kode_lokasi,periode,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1) values "+
						    			"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.akunBeban+"','"+this.sg2.cells(5,i)+"','"+this.sg2.cells(7,i)+"','"+this.e_uraian.getText()+"','D',"+nilaiToFloat(this.sg2.cells(4,i))+",getdate(),'"+this.sg2.cells(0,i)+"','NONBDD','"+this.cb_id.getText()+"')");				
								
								sql.add("insert into tu_prbeban_j(no_bukti,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,periode_beban,kode_curr,kurs,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.sg2.cells(0,i)+"','"+this.dp_d1.getDateString()+"',2,'"+this.akunBeban+"','"+this.sg2.cells(1,i)+"','D',"+nilaiToFloat(this.sg2.cells(4,i))+",'"+this.sg2.cells(5,i)+"','"+this.sg2.cells(7,i)+"','"+this.app._lokasi+"','JCOSTINI','BEBAN','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");																																		
							}
						}
					}									
		...
		switch (event){
			...
			case "ubah" :	
				...
				if (this.sg2.getRowValidCount() > 0){
					for (var i=0;i < this.sg2.getRowCount();i++){
						if (this.sg2.rowValid(i)){																
							if (nilaiToFloat(this.sg2.cells(3,i)) < nilaiToFloat(this.sg2.cells(4,i)) ) {
								var k = i+1;
								system.alert(this,"Transaksi Sharing tidak valid.","Nilai Sharing melebihi Saldo OR. Baris "+k);
								return false;									
							}
						}
					}
				...
				if (parseFloat(this.e_periode.getText()) != parseFloat(this.c_periodeSch.getText())){
					system.alert(this,"Periode transaksi tidak valid.","Periode transaksi harus sama dengan Periode Akru");
					return false;
				}
	...
	doSelectDate: function(sender, y,m,d){
		...
		if (this.stsSimpan == 1) {
			this.cb_drkKeg.setSQL("select a.kode_drk, a.nama from drk a inner join tu_proyek_drk b on a.kode_drk=b.kode_drk and a.tahun=b.tahun and a.kode_lokasi=b.kode_lokasi and b.status='BEBAN' where a.tahun='"+this.e_periode.getText().substr(0,4)+"' and a.kode_lokasi='"+this.app._lokasi+"'",["kode_drk","nama"],false,["Kode","Nama"],"and","Data DRK Beban",true);																													 				
			this.doClick()	
		}	
	},		
	...
	doChange: function(sender){	
		try {
			...
			if (sender==this.cb_id && this.cb_id.getText()!="") {			
				var strSQL = "select xx.periode, xx.kode_proyek,xx.nama,xx.akun_beban,xx.akun_bdd,xx.kode_drkb, isnull(dd.saldo_beban,0) as saldo_beban  "+
						    "from ( "+
				
							"select b.kode_proyek,b.nama,c.akun_beban,c.akun_bdd, b.kode_drkb, "+
							"substring(convert(varchar,tgl_mulai,112),1,6) as periode "+
							"from tu_proyek b "+
			...
			if (sender == this.cb_proyek && this.cb_proyek.getText()!="" && this.stsSimpan==1) {
				...
				var data = this.dbLib.getDataProvider("select distinct periode as periode from tu_proyek_d where kode_proyek='"+this.cb_proyek.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
				...
				this.sg2.clear(1);
				this.sg2.cells(0,0,this.cb_proyek.getText());
				this.doChangeCell2();
			}
	...
	doChangeCell2: function(sender, col, row){
		if (col == 0 && this.stsSimpan == 1) {		
			var strSQL = "select a.nama,a.kode_pp,a.no_pks,a.nilai_or - isnull(c.bdd,0) as saldo ,"+
						 "substring(convert(varchar,tgl_mulai,112),1,6) as periode_proyek,a.kode_drkb "+
			             "from tu_proyek a "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line = data.rs.rows[0];		
				this.sg2.cells(1,row,line.nama);		
				...
				this.sg2.cells(6,row,line.periode_proyek);
				this.sg2.cells(7,row,line.kode_drkb);
			}			
		}
		if (col == 4) {			
	...
	/*
	karena ada jurnal utk yg periode yg sama (inisiasi dgn project) ... maka project harus satu( 1 ke 1),,
	...
	*/
	...
	doLoad3:function(sender){				
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai "+
		             "from tu_prbeban_m a "+					 					 
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'JCOSTINI' and a.posted ='F'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.*, b.kode_proyek "+
							 "from tu_prbeban_m a "+
							 "inner join ( "+
							 "			     select no_bukti,kode_proyek from tu_prbdd_d  "+
							 "				 where dc = 'D' and no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg2.appendData([line.kode_proyek,line.nama,line.no_pks,floatToNilai(line.saldo),floatToNilai(line.nilai),line.kode_pp,line.periode_proyek,line.kode_drkb]);
					}
