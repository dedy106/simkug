window.app_saku3_transaksi_tu_proyek_fAgendaBDD = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyek_fAgendaBDD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyek_fAgendaBDD";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan BDD proyek", 0);	
...
window.app_saku3_transaksi_tu_proyek_fAgendaBDD.extend(window.childForm);
window.app_saku3_transaksi_tu_proyek_fAgendaBDD.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,user_input,form,sts_pajak,npajak) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.c_jenis.getText()+"','"+akunAJU+"','"+this.cb_pp.getText()+"','-','"+this.cb_id.getText()+' | '+this.e_ket.getText()+"',"+nilaiToFloat(this.e_netto.getText())+",getdate(),'"+this.app._userLog+"','-','-','-','A','-','-','"+this.e_user.getText()+"','PRAGENDA','"+this.c_status.getText()+"',"+nilaiToFloat(this.e_npajak.getText())+")");				
					...
					if (nilaiBDD != 0) {
						sql.add("insert into it_aju_d (no_aju, no_dokumen, tanggal, no_urut, kode_akun, keterangan, dc, kode_curr, kurs, nilai_curr, nilai, kode_pp, kode_drk, kode_lokasi, modul, jenis, periode, nik_user, tgl_input, akun_seb) values "+
								"('"+this.e_nb.getText()+"', '-', '"+this.dp_d1.getDateString()+"', 0, '"+this.akunBDD+"', '"+this.cb_id.getText()+' | '+this.e_ket.getText()+"', 'D', 'IDR', 1, "+nilaiBDD+", "+nilaiBDD+", '"+this.cb_pp.getText()+"', '-', '"+this.app._lokasi+"', 'PRAGENDA', 'BDD', '"+this.e_periode.getText()+"', '"+this.app._userLog+"', getdate(), '-')");		
						sql.add("insert into tu_prbdd_d(no_bukti,kode_lokasi,periode,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.akunBDD+"','"+this.cb_pp.getText()+"','-','"+this.cb_id.getText()+' | '+this.e_ket.getText()+"','D',"+nilaiBDD +",getdate(),'"+this.cb_id.getText()+"','AJUBDD','-')");						
					}
					//bmhd
					if (nilaiBMHD != 0) {						
						sql.add("insert into it_aju_d (no_aju, no_dokumen, tanggal, no_urut, kode_akun, keterangan, dc, kode_curr, kurs, nilai_curr, nilai, kode_pp, kode_drk, kode_lokasi, modul, jenis, periode, nik_user, tgl_input, akun_seb) values "+
								"('"+this.e_nb.getText()+"', '-', '"+this.dp_d1.getDateString()+"', 0, '"+this.akunBMHD+"', '"+this.cb_id.getText()+' | '+this.e_ket.getText()+"', 'D', 'IDR', 1, "+nilaiBMHD+", "+nilaiBMHD+", '"+this.cb_pp.getText()+"', '-', '"+this.app._lokasi+"', 'PRAGENDA', 'BMHD', '"+this.e_periode.getText()+"', '"+this.app._userLog+"', getdate(), '-')");		
						sql.add("insert into tu_prbdd_d(no_bukti,kode_lokasi,periode,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.akunBMHD+"','"+this.cb_pp.getText()+"','-','"+this.cb_id.getText()+' | '+this.e_ket.getText()+"','C',"+nilaiBMHD+",getdate(),'"+this.cb_id.getText()+"','REVBMHD','PRAGENDA')");					
					}
