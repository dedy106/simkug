window.app_saku3_transaksi_tu_proyek_fJuBill = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyek_fJuBill.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyek_fJuBill";
		...
		this.e_saldo = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,12,200,20],caption:"Nilai Jurnal", tag:1, readOnly:true, tipeText:ttNilai, text:"0",change:[this,"doChange"]});					
		...
		this.e_total = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,15,200,20],caption:"Total Rekon", tag:1, tipeText:ttNilai, text:"0",change:[this,"doChange"]});					
		...
		this.e_sisa = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,19,200,20],caption:"Nilai Sisa", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});	
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:7,tag:0,
				colTitle:["No Bill","No Proyek","Deskripsi","Akun AR","Saldo Tagihan","N Rekon","PP / Unit"],
				colWidth:[[6,5,4,3,2,1,0],[80,100,100,60,320,150,100]],
		...
		this.e_pph = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,15,200,20],caption:"Nilai PPh", tag:1, tipeText:ttNilai, text:"0", readOnly:true});					
...
window.app_saku3_transaksi_tu_proyek_fJuBill.extend(window.childForm);
window.app_saku3_transaksi_tu_proyek_fJuBill.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					var tot = nilaiToFloat(this.e_saldo.getText()); 					
					sql.add("insert into ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_bukti.getText()+"','"+this.e_ket.getText()+"','-','PIUPRO','REKON','IDR',1,"+tot+",'"+this.app._userLog+"','"+this.app._userLog+"','F','"+this.cb_cust.getText()+"','"+this.c_periode.getText()+"','"+this.cb_akun.getText()+"',getdate(),'"+this.app._userLog+"')");					
					...
					sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_bukti.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','D',"+tot+",'"+this.app._kodePP+"','-','"+this.cb_cust.getText()+"','-','-','-','-','-','"+this.app._lokasi+"','PIUPRO','TITIP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','-')");
					
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && this.sg1.cells(5,i) != "0"){	
							var noProyek = this.sg1.cells(1,i);
							sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
									"('"+this.e_nb.getText()+"','"+this.sg1.cells(0,i)+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(3,i)+"','Pelunasan atas Proyek "+this.sg1.cells(1,i)+"','C',"+nilaiToFloat(this.sg1.cells(5,i))+",'"+this.app._kodePP+"','-','-','-','-','-','-','-','"+this.app._lokasi+"','PIUPRO','PIU','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'"+this.cb_cust.getText()+"','-')");
						}
					}
					...
					if (this.e_pph.getText() != "0") {
						...
						sql.add("insert into tu_prbdd_d(no_bukti,kode_lokasi,periode,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_pph.getText()+"','"+this.app._kodePP+"','-','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_pph.getText())+",getdate(),'"+noProyek+"','NONBDD','-')");					
					}
					...
					if (this.cb_lokasi.getText() != "") {
						...
						sql.add("insert into takkirim_j(no_kirim,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
								"('"+this.e_nbtak.getText()+"','"+this.cb_bukti.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.cb_tak.getText()+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_tak.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PIUPRO','TAK','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");
					
						sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
								"('"+this.e_nb.getText()+"','"+this.cb_bukti.getText()+"','"+this.dp_d1.getDateString()+"',998,'"+this.cb_tak.getText()+"','TAK atas bukti "+this.cb_bukti.getText()+"','C',"+nilaiToFloat(this.e_tak.getText())+",'"+this.app._kodePP+"','-','-','-','-','-','-','-','"+this.app._lokasi+"','PIUPRO','TAK','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','-')");
					}								
					setTipeButton(tbAllFalse);					
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :														
				...
				if (temuBayar != 1) {					
					system.alert(this,"Transaksi tidak valid.","Rekon hanya untuk satu Proyek");
					return false;						
				}
	...
	doChange:function(sender){				
		...
		if (sender == this.cb_cust && this.cb_cust.getText()!="" && this.stsSimpan==1 ) {			
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];												
					this.sg1.appendData([line.no_bill,line.kode_proyek,line.keterangan,line.akun_piutang,floatToNilai(line.saldo),"0",line.kode_pp]);
				}
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.kode_loktuj,a.nilai,b.kode_akun from takkirim_m a inner join takkirim_j b on a.no_kirim=b.no_kirim and a.kode_lokasi=b.kode_lokasi and b.jenis='TAK' "+
				...
				var strSQL = "select a.no_bill,a.kode_proyek,a.akun_piutang,a.keterangan,round(a.total-isnull(b.bayar,0),2) as saldo,a.kode_pp, c.nilai "+
							 "from ( "+						 						 
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];												
						this.sg1.appendData([line.no_bill,line.kode_proyek,line.keterangan,line.akun_piutang,floatToNilai(line.saldo),floatToNilai(line.nilai),line.kode_pp]);
					}
				} else this.sg1.clear(1);
				this.sg1.validasi();
