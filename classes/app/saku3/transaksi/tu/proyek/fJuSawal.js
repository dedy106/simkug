window.app_saku3_transaksi_tu_proyek_fJuSawal = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyek_fJuSawal.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyek_fJuSawal";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Rekon Pelunasan Billing Saldo Awal", 0);	
...
window.app_saku3_transaksi_tu_proyek_fJuSawal.extend(window.childForm);
window.app_saku3_transaksi_tu_proyek_fJuSawal.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_bukti.getText()+"','"+this.e_ket.getText()+"','-','PIUPRO','REKON','IDR',1,"+tot+",'"+this.app._userLog+"','"+this.app._userLog+"','F','-','"+this.c_periode.getText()+"','"+this.cb_akun.getText()+"',getdate(),'"+this.app._userLog+"')");					
					...
					sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_bukti.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','D',"+tot+",'"+this.app._kodePP+"','-','-','-','-','-','-','-','"+this.app._lokasi+"','PIUPRO','TITIP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','-')");
					
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && this.sg1.cells(5,i) != "0"){							
							sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
									"('"+this.e_nb.getText()+"','"+this.sg1.cells(0,i)+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(3,i)+"','Pelunasan atas Bill "+this.sg1.cells(0,i)+"','C',"+nilaiToFloat(this.sg1.cells(5,i))+",'"+this.app._kodePP+"','-','-','-','-','-','-','-','"+this.app._lokasi+"','PIUPRO','PIU','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','-')");
						}
					}
					if (this.e_pph.getText() != "0") {
						sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_bukti.getText()+"','"+this.dp_d1.getDateString()+"',777,'"+this.cb_pph.getText()+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_pph.getText())+",'"+this.app._kodePP+"','-','-','-','-','-','-','-','"+this.app._lokasi+"','PIUPRO','PPH','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','-')");
					}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :		
				...
				if (pph != nilaiToFloat(this.e_pph.getText())) {
					system.alert(this,"Transaksi tidak valid.","Total tidak balance.");
					return false;
				}
