window.app_saku3_transaksi_tu_proyek_fInisialPjBDD = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyek_fInisialPjBDD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyek_fInisialPjBDD";
		...
		this.cb_id = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,13,222,20],caption:"ID Kegiatan",tag:1,multiSelection:false,change:[this,"doChange"]}); 						
		this.cb_akun = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,19,222,20],caption:"Akun BDD",tag:1, readOnly:true}); 		
		this.e_uraian = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,16,550,20],caption:"Deskripsi", readOnly:true});				
		this.e_nsaldo = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,19,202,20],caption:"Saldo Budget", tag:1, tipeText:ttNilai, text:"0",readOnly:true});
...
window.app_saku3_transaksi_tu_proyek_fInisialPjBDD.extend(window.childForm);
window.app_saku3_transaksi_tu_proyek_fInisialPjBDD.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,user_input,form,sts_pajak) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.c_jenis.getText()+"','"+this.akunPJ+"','"+this.cb_pp.getText()+"','-','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",getdate(),'"+this.app._userLog+"','-','-','-','A','"+this.cb_nik.getText()+"','-','"+this.e_user.getText()+"','PJINISIAL','NON')");					
	...
	doChange:function(sender){
		if (sender==this.cb_pp && this.cb_pp.getText()!="" && this.stsSimpan == 1) {			
			this.cb_id.setText("","");
			this.cb_id.setSQL("select kode_proyek, nama from tu_proyek where no_app='INISIAL' and kode_pp='"+this.cb_pp.getText()+"' and flag_aktif='1' and kode_lokasi='"+this.app._lokasi+"'",["kode_proyek","nama"],false,["ID Kegiatan","Keterangan"],"and","Data Proyek",true);						
		...
		if (sender==this.cb_id && this.cb_id.getText()!="") {
			var strSQL = "select a.nama,b.akun_bdd,a.nilai_or - isnull(c.bdd,0) as saldo "+
			             ...
			             "		from tu_prbdd_d where  modul='AJUBDD' and kode_lokasi='"+this.app._lokasi+"' and no_bukti <> '"+this.e_nb.getText()+"' group by kode_proyek,kode_lokasi "+
	...
	doLoad3:function(sender){		
		var strSQL = "select a.no_aju,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
		             ...
		             "where a.form='PJINISIAL' and a.modul='PANJAR' and a.progress in ('0','A','R') and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and b.nik='"+this.app._userLog+"'";	
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
