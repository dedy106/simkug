window.app_saku3_transaksi_tu_proyek_fBDDdis = function(owner)
{
	if (owner)
	{
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:8,tag:9,		            
				colTitle:["ID Proyek","Periode","Keterangan","Akun Beban","Akun BDD","Sisa Akru Beban","PP","DRK"],
		...
		this.sg2 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:8,tag:9,		            
				colTitle:["ID Proyek","Keterangan","Beban Schedule","Saldo BDD","Bbn Dummy(Bdd-BbnSch)","PP","Akun BDD","Akun BMHD"],
				colWidth:[[7,6,5,4,3,2,1,0],[80,80,80,150,100,100,200,120]],
...
window.app_saku3_transaksi_tu_proyek_fBDDdis.implement({
				...
				try{													
					...
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						...
						sql.add("insert into tu_prbeban_j(no_bukti,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,periode_beban,kode_curr,kurs,nik_user,tgl_input) values "+
								"('"+this.e_nb.getText()+"','"+line.kode_proyek+"','"+this.dp_d1.getDateString()+"',1,'"+line.akun_beban+"','"+this.e_ket.getText()+"','D',"+parseFloat(line.sisa_akru_beban)+",'"+line.kode_pp+"','"+line.kode_drkb+"','"+this.app._lokasi+"','GENBBN','BEBAN','"+this.e_periode.getText()+"','"+line.periode+"','IDR',1,'"+this.app._userLog+"',getdate())");							
						sql.add("insert into tu_prbeban_j(no_bukti,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,periode_beban,kode_curr,kurs,nik_user,tgl_input) values "+
								"('"+this.e_nb.getText()+"','"+line.kode_proyek+"','"+this.dp_d1.getDateString()+"',2,'"+line.akun_bdd+"','"+this.e_ket.getText()+"','C',"+parseFloat(line.sisa_akru_beban)+",'"+line.kode_pp+"','-','"+this.app._lokasi+"','GENBBN','BDD','"+this.e_periode.getText()+"','"+line.periode+"','IDR',1,'"+this.app._userLog+"',getdate())");																			
				
					}		
					
					for (var i=0;i < this.sg2.getRowCount();i++){
						if (this.sg2.rowValid(i) && nilaiToFloat(this.sg2.cells(4,i)) != 0) {
							//"ID Proyek","Keterangan","Beban Schedule","Saldo BDD","Bbn Dummy(Bdd-BbnSch)","PP","Akun BDD","Akun BMHD"
							sql.add("insert into tu_prbeban_j(no_bukti,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,periode_beban,kode_curr,kurs,nik_user,tgl_input) values "+
	...
	doTampilClick: function(sender){
		try{			
			if (this.e_periode.getText() != "") {
				this.sg1.clear(1);
				var strSQL = "select a.kode_proyek,a.periode,b.nama,c.akun_bdd,c.akun_beban,c.akun_bmhd,b.kode_pp, "+
							 "( a.nilai_beban - isnull(d.nilai_beban,0) - isnull(e.nilai_rekon_ju,0) ) as sisa_akru_beban, "+
							 "b.kode_drkb "+
							 ...
							 "  from tu_prbeban_j "+
							 "  where ((jenis = 'BEBAN') or (modul+jenis = 'JCOSTINIBDD')) "+
							 ...
							 "	select a.kode_proyek,a.periode,a.kode_lokasi,sum(case a.dc when 'D' then a.nilai else -a.nilai end) nilai_rekon_ju "+
							 "  from tu_prbdd_d a inner join ju_m b on a.no_bukti=b.no_ju and a.kode_lokasi=b.kode_lokasi and b.modul='PIUPRO' and b.jenis='REKON' "+
							 "  where a.modul = 'NONBDD' "+
							 "  group by a.kode_proyek,a.periode,a.kode_lokasi "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						...
						tot = tot + parseFloat(line.sisa_akru_beban);																	
					}
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU.rs.rows[i];
			this.sg1.appendData([line.kode_proyek,line.periode,line.nama,line.akun_beban,line.akun_bdd,floatToNilai(line.sisa_akru_beban),line.kode_pp,line.kode_drkb]);
		}
	...
	doRekapBeban : function() {
		try {
			...
			for (var i=0;i < this.dataJU.rs.rows.length;i++){
				...
				beban = parseFloat(line.sisa_akru_beban);
				var isAda = false;
				...
				if (!isAda) {
					this.sg2.appendData([line.kode_proyek,line.nama,floatToNilai(beban),"0","0",line.kode_pp,line.akun_bdd,line.akun_bmhd]);
				} 
				else { 
					totalBeban = nilaiToFloat(this.sg2.cells(2,idx));
					totalBeban = totalBeban + beban;
					this.sg2.setCell(2,idx,totalBeban);					
				}											
			}	
			...
			for (var i=0;i < this.sg2.getRowCount();i++){
				...
				var strSQL = "select sum(x.nilai) as bdd "+
							 ...
							 "where a.periode<='"+this.e_periode.getText()+"' and a.kode_proyek='"+this.sg2.cells(0,i)+"' and a.kode_lokasi ='"+this.app._lokasi+
							 "'  and a.modul = 'AJUBDD' and a.no_bukti <>'"+this.e_nb.getText()+"'  "+
							 
							 "union all "+
							 "select sum(case a.dc when 'D' then a.nilai else -a.nilai end) as nilai "+
							 "from tu_prbdd_d a inner join ju_m b on a.no_bukti=b.no_ju and a.kode_lokasi=b.kode_lokasi and a.kode_proyek=b.no_dokumen "+
							 "where a.periode<='"+this.e_periode.getText()+"' and a.kode_proyek='"+this.sg2.cells(0,i)+"' and a.kode_lokasi ='"+this.app._lokasi+
							 "'  and a.modul = 'AJUBDD' and a.no_bukti <>'"+this.e_nb.getText()+"'  "+

							 "union all "+
							 "select sum(case a.dc when 'D' then a.nilai else -a.nilai end) as nilai "+
							 "from tu_prbdd_d a inner join ptg_m b on a.no_bukti=b.no_ptg and a.kode_lokasi=b.kode_lokasi  "+
							 "where a.periode<='"+this.e_periode.getText()+"' and a.kode_proyek='"+this.sg2.cells(0,i)+"' and a.kode_lokasi ='"+this.app._lokasi+
							 "'  and a.modul = 'AJUBDD' and a.no_bukti <>'"+this.e_nb.getText()+"'  "+
							 ...
							 "union all "+
							 "select sum(case a.dc when 'D' then a.nilai else -a.nilai end) as nilai "+
							 "from tu_prbdd_d a inner join tu_prbeban_m b on a.no_ref1=b.no_dokumen and a.kode_lokasi=b.kode_lokasi and a.no_bukti=b.no_bukti "+
							 "where  a.periode<='"+this.e_periode.getText()+"' and a.kode_proyek='"+this.sg2.cells(0,i)+"' and a.kode_lokasi ='"+this.app._lokasi+
							 "'  and a.modul = 'AJUBDD' and a.no_bukti <>'"+this.e_nb.getText()+"'  "+
				...
				//ambil BDD yang sudah dibebankan (Credit - Debet)
				var strSQL = "select isnull(sum(case dc when 'C' then nilai else -nilai end),0) as beban_bdd "+
							 "from tu_prbeban_j "+
							 "where periode<='"+this.e_periode.getText()+"' and no_dokumen='"+this.sg2.cells(0,i)+"' and jenis='BDD' and kode_lokasi ='"+this.app._lokasi+"' and no_bukti <>'"+this.e_nb.getText()+"'";							 							 

				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){	
						totBeban = parseFloat(line.beban_bdd);																					
					}					
				}
			
				saldoBDD = saldoBDD - totBeban;				
				this.sg2.cells(3,i,saldoBDD);


				if (nilaiToFloat(this.sg2.cells(3,i)) < nilaiToFloat(this.sg2.cells(2,i))) {
					var nilaiBMHD = nilaiToFloat(this.sg2.cells(2,i)) - nilaiToFloat(this.sg2.cells(3,i));
		...
		try{
			if (this.sg2.cells(0,row) != "") {
				...
				var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.dc,a.nilai "+							
							 ...
							 //versi 1 : "and a.kode_lokasi ='"+this.app._lokasi+"'  and a.modul <> 'REVBDD' and a.no_bukti <>'"+this.e_nb.getText()+"'  and a.no_ref1='-' ";
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){							
							if (this.preView == "1") {															
								this.nama_report="server_report_saku3_tu_rptProyekDistJurnal";
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				this.sg1.clear(1);				
				var strSQL = "select a.kode_proyek,a.periode,b.nama,c.akun_bdd,c.akun_beban,c.akun_bmhd,b.kode_pp,(a.nilai_beban - isnull(d.nilai_beban,0)) as sisa_akru_beban, b.kode_drkb "+
							 "from tu_proyek_d a "+
							/*
							 "inner join (		 "+
