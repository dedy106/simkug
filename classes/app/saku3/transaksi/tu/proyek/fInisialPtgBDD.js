window.app_saku3_transaksi_tu_proyek_fInisialPtgBDD = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyek_fInisialPtgBDD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyek_fInisialPtgBDD";
		...
		this.cb_id = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,19,220,20],caption:"ID Kegiatan",tag:1,readOnly:true}); 				
		this.e_uraian = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,16,550,20],caption:"Deskripsi", readOnly:true});				
		this.e_nsaldo = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,19,200,20],caption:"Saldo Budget", tag:1, tipeText:ttNilai, text:"0",readOnly:true});		
...
window.app_saku3_transaksi_tu_proyek_fInisialPtgBDD.extend(window.childForm);
window.app_saku3_transaksi_tu_proyek_fInisialPtgBDD.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,user_input,sts_pajak,form) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.c_jenis.getText()+"','"+this.cb_akun.getText()+"','"+this.cb_pp.getText()+"','-','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",getdate(),'"+this.app._userLog+"','-','-','-','A','"+this.cb_nik.getText()+"','"+this.cb_panjar.getText()+"','"+this.e_user.getText()+"','NON','INPJPTG')");
					...
					sql.add("insert into tu_prbdd_d(no_bukti,kode_lokasi,periode,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.akunBDD+"','"+this.cb_pp.getText()+"','-','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_nilaipj.getText()) +",getdate(),'"+this.cb_id.getText()+"','AJUBDD','-')");
					sql.add("insert into tu_prbdd_d(no_bukti,kode_lokasi,periode,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_akun.getText()+"','"+this.cb_pp.getText()+"','-','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_nilai.getText()) +",getdate(),'"+this.cb_id.getText()+"','AJUBDD','-')");
	...
	doChange:function(sender){
		if (sender == this.cb_nik && this.cb_nik.getText()!="" && this.stsSimpan==1) {	
			this.cb_panjar.setSQL("select a.no_aju,a.keterangan from it_aju_m a "+
								  "inner join tu_prbdd_d c on a.no_aju=c.no_bukti and a.kode_lokasi=c.kode_lokasi and c.modul='AJUBDD' "+
			                      "where a.progress='3' and a.nik_panjar ='"+this.cb_nik.getText()+"' and a.form='PJINISIAL' and a.modul = 'PANJAR' and a.no_ptg='-' and a.kode_lokasi='"+this.app._lokasi+"'",
			                      ["a.no_aju","a.keterangan"],false,["No Bukti","Keterangan"],"and","Data Panjar",true);
		}
		if (sender == this.cb_panjar && this.cb_panjar.getText()!="") {	
			var strSQL = "select a.nilai,a.kode_pp,b.kode_proyek,a.kode_akun,c.nama,b.kode_akun as akun_bdd "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){			
					this.cb_akun.setSQL("select kode_akun, nama from masakun where kode_akun='"+line.akun_bdd+"' and kode_lokasi='"+this.app._lokasi+"' union select '-','-' ",["kode_akun","nama"],false,["Kode","Nama"],"and","Data Akun",true);									
			...
			var strSQL = "select a.nama,b.akun_bdd,a.nilai_or - isnull(c.bdd,0) as saldo "+
			             "from tu_proyek a "+
			             ...
			             "		from tu_prbdd_d where  modul='AJUBDD' and kode_lokasi='"+this.app._lokasi+"' and no_bukti not in ('"+this.cb_panjar.getText()+"','"+this.e_nb.getText()+"') group by kode_proyek,kode_lokasi "+
	...
	doLoad3:function(sender){	
		var strSQL = "select a.no_aju,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
		             ...
		             "where a.form='INPJPTG' and a.modul='PJPTG' and a.progress in ('0','A','R') and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and b.nik='"+this.app._userLog+"'";	
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.progress,a.modul,a.tanggal,a.periode,a.keterangan,a.nilai,a.kode_pp,a.nik_panjar,isnull(x.no_ver,'-') as no_ver,isnull(x.catatan,'-') as catatan,a.kode_akun,c.no_aju as no_pj,c.keterangan as ket_pj,c.kode_akun as akun_pj,  "+
							 "d.kode_proyek,d.nama as nama_proyek "+
						     
						     "from it_aju_m a inner join it_aju_m c on a.no_aju=c.no_ptg and a.kode_lokasi=c.kode_lokasi "+					   
						     "      inner join tu_prbdd_d cc on c.no_aju=cc.no_bukti and c.kode_lokasi=cc.kode_lokasi and cc.modul='AJUBDD' "+
