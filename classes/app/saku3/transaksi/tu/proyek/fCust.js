window.app_saku3_transaksi_tu_proyek_fCust = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyek_fCust.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyek_fCust";
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:5,tag:9,
		            colTitle:["Kode","Nama","Alamat","PIC","Jabatan"],
					colWidth:[[4,3,2,1,0],[150,150,400,300,80]],
					readOnly:true,
		...
		this.c_status = new saiCB(this.pc1.childPage[1],{bound:[20,11,200,20],caption:"Status WAPU",items:["WAPU","NON"], readOnly:true,tag:2});
...
window.app_saku3_transaksi_tu_proyek_fCust.extend(window.childForm);
window.app_saku3_transaksi_tu_proyek_fCust.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into cust(kode_cust,kode_lokasi,nama,alamat,no_tel,email,npwp,pic,alamat2,no_fax,kode_pp,wapu,jabatan) values "+
						    "('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.e_alamat.getText()+"','"+this.e_tel.getText()+"','"+this.e_mail.getText()+"','"+this.e_npwp.getText()+"','"+this.e_pic.getText()+"','"+this.e_alamat2.getText()+"','"+this.e_fax.getText()+"','-','"+this.c_status.getText()+"','"+this.e_jabatan.getText()+"')");
					setTipeButton(tbAllFalse);
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select kode_cust,nama,alamat,no_tel,email,npwp,alamat2,pic,no_fax,jabatan,wapu "+
				             "from cust "+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){						
						...
						this.c_status.setText(line.wapu);	
	...
	doLoad:function(sender){								
		try {
			var strSQL = "select a.kode_cust,a.nama,a.alamat,a.pic,a.jabatan "+
						 "from cust a "+					 
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_cust,line.nama,line.alamat,line.pic,line.jabatan]); 
		}
