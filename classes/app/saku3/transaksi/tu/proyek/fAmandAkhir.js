window.app_saku3_transaksi_tu_proyek_fAmandAkhir = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyek_fAmandAkhir.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyek_fAmandAkhir";
...
window.app_saku3_transaksi_tu_proyek_fAmandAkhir.extend(window.childForm);
window.app_saku3_transaksi_tu_proyek_fAmandAkhir.implement({	
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :	
				this.sg.validasi();
				var data = this.dbLib.getDataProvider("select a.periode ,isnull(b.no_bukti,'-') as no_pyt,a.nilai_pend as pdpt,  isnull(c.no_bukti,'-') as no_beban,a.nilai_beban as beban "+
							"from tu_proyek_d a "+
							"		left join tu_prpyt_d b on a.kode_proyek=b.kode_proyek and a.periode=b.periode_dis and a.kode_lokasi=b.kode_lokasi and b.dc ='D' "+
							"		left join tu_prbdd_d c on a.kode_proyek=c.kode_proyek and a.periode=c.periode and a.kode_lokasi=c.kode_lokasi and c.dc ='D' "+
							"where a.kode_proyek = '"+this.cb_kode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by nu",true);
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						...
						var temuPdptSalah = false;		

						if (line.no_pyt != "-") var temuPeriodePYT = false;	
						else var temuPeriodePYT = true;	
						...
						for (var j=0;j < this.sg.getRowCount();j++) {
							if (this.sg.rowValid(j)) {			
								...
								//periode ada, nilai berubah, sudah pernah jurnal pyt---> tidak boleh di edit					
								if (line.periode == this.sg.cells(0,j) && parseFloat(line.pdpt) != nilaiToFloat(this.sg.cells(3,j)) && line.no_pyt != "-") {
									temuPdptSalah = true;
								}
								//jika periode yg sudah pyt ketemu... berarti gak dihapus
				...
				if (temuPdptSalah){
					system.alert(this,"Transaksi tidak valid.","Terdapat perubahan pada nilai pdpt yang sudah distribusi.");
					return false;
				}
				if (!temuPeriodePYT){
					system.alert(this,"Transaksi tidak valid.","Tidak ditemukan Periode distribusi pdpt yang sudah jurnal.");
					return false;
				}
