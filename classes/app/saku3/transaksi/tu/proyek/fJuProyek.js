window.app_saku3_transaksi_tu_proyek_fJuProyek = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyek_fJuProyek.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyek_fJuProyek";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Jurnal Umum Proyek", 0);	
		...
		this.cb_bdd = new saiCBBL(this.pc2.childPage[0],{bound:[20,16,220,20],caption:"Akun BDD", maxLength:10, tag:1,readOnly:true});						
		this.e_saldo = new saiLabelEdit(this.pc2.childPage[0],{bound:[770,16,220,20],caption:"Saldo OR", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		this.e_ket = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,17,450,20],caption:"Deskripsi", maxLength:150});				
		this.e_bdd = new saiLabelEdit(this.pc2.childPage[0],{bound:[770,17,220,20],caption:"Total BDD", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		...
		try {
			...
			this.cb_bdd.setSQL("select kode_akun, nama from masakun where  kode_lokasi='"+this.app._lokasi+"'",["kode_akun","nama"],false,["Kode","Deskripsi"],"and","Data Akun",true);			
		}catch(e){
			systemAPI.alert(e);
		}
	}
};
window.app_saku3_transaksi_tu_proyek_fJuProyek.extend(window.childForm);
window.app_saku3_transaksi_tu_proyek_fJuProyek.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_proyek.getText()+"','"+this.e_ket.getText()+"','-','JUPROYEK','UMUM','IDR',1,"+parseNilai(this.e_bdd.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"','F','-','-','-',getdate(),'"+this.app._userLog+"')");
					...
					sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.cb_bdd.getText()+"','"+this.e_ket.getText()+"','D',"+parseNilai(this.e_bdd.getText())+",'"+this.ppProyek+"','-','-','-','-','-','-','-','"+this.app._lokasi+"','JUPROYEK','BDD','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','-')");

					sql.add("insert into tu_prbdd_d(no_bukti,kode_lokasi,periode,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_bdd.getText()+"','"+this.ppProyek+"','-','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_bdd.getText())+",getdate(),'"+this.cb_proyek.getText()+"','AJUBDD')");
														
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
										"	('"+this.e_nb.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','-','-','-','-','-','-','"+this.app._lokasi+"','JUPROYEK','UMUM','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','-')");
							}
						}
					}			
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"('"+this.e_nb.getText()+"','JUPROYEK','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(6,i))+","+nilai+")");
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :															
				...
				if ((nilaiToFloat(this.e_bdd.getText()) > nilaiToFloat(this.e_saldo.getText()) ) ||  nilaiToFloat(this.e_bdd.getText()) == 0) {
					system.alert(this,"Transaksi tidak valid.","Nilai BDD melebihi Saldo/Nilai BDD tidak boleh nol atau kurang.");
					return false;						
				}				
				if (parseFloat(this.app._periode) > parseFloat(this.e_periode.getText())){
					system.alert(this,"Nilai BDD transaksi tidak valid.","Transaksi melebihi saldo dan tidak boleh nol.");
					return false;
				}
				if (parseFloat(this.app._periode) < parseFloat(this.e_periode.getText())){
	...
	doChange:function(sender){
		if (sender == this.cb_proyek && this.cb_proyek.getText()!="" ) {
			this.cb_bdd.setText("");
			this.ppProyek = "";
			var data = this.dbLib.getDataProvider("select a.akun_bdd,b.kode_pp from tu_proyek_jenis a inner join tu_proyek b on "+
			...
			var strSQL = "select a.nama,b.akun_bdd,a.nilai,a.nilai_or,a.nilai_or - isnull(c.bdd,0) as saldo "+
			             ...
			             "		from tu_prbdd_d where kode_lokasi='"+this.app._lokasi+"' and no_bukti <> '"+this.e_nb.getText()+"' and modul='AJUBDD' group by kode_proyek,kode_lokasi "+
	...
	doNilaiChange: function(){
		try{
			...
			this.e_bdd.setText(floatToNilai(tot));
		}catch(e)
		{
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_ju,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai "+
		             "from ju_m a "+					 					 
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'JUPROYEK' and a.posted ='F'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk "+
							...
							"where a.jenis <> 'BDD' and a.no_ju = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
					for (var i in data.rs.rows){
