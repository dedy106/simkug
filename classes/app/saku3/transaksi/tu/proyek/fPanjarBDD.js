window.app_saku3_transaksi_tu_proyek_fPanjarBDD = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyek_fPanjarBDD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyek_fPanjarBDD";
...
window.app_saku3_transaksi_tu_proyek_fPanjarBDD.extend(window.childForm);
window.app_saku3_transaksi_tu_proyek_fPanjarBDD.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into tu_prbdd_d(no_bukti,kode_lokasi,periode,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_akun.getText()+"','"+this.cb_pp.getText()+"','-','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_nilai.getText())+",getdate(),'"+this.cb_id.getText()+"','NONBDD')");
	...
	doChange:function(sender){
		...
		if (sender==this.cb_id && this.cb_id.getText()!="") {
			var strSQL = "select a.nama,b.akun_bdd,a.nilai,a.nilai_or,a.nilai_or - isnull(c.bdd,0) as saldo, convert(varchar,a.tgl_selesai,103) as tgl_selesai  "+
			             "from tu_proyek a "+
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.nik_panjar,aa.kode_proyek,a.progress,a.modul,a.tanggal,a.periode,a.keterangan,a.nilai,a.kode_pp,a.kode_akun,a.kode_drk,isnull(x.no_ver,'-') as no_ver,isnull(x.catatan,'-') as catatan,a.user_input,a.kode_drk, "+
						     ...
						     "from it_aju_m a "+
						     "				inner join tu_prbdd_d aa on a.no_aju=aa.no_bukti and a.kode_lokasi=aa.kode_lokasi and aa.modul='NONBDD' "+
