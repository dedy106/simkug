window.app_saku3_transaksi_tu_kb_fKbTitipMRekNonAgenda = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_kb_fKbTitipMRekNonAgenda.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_kb_fKbTitipMRekNonAgenda";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Penerimaan Titipan Non Agenda", 0);	
		...
		this.e_kb = new saiLabelEdit(this.pc2.childPage[0],{bound:[770,12,220,20],caption:"Nilai Titip", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});								
		this.cb_titip = new saiCBBL(this.pc2.childPage[0],{bound:[20,15,220,20],caption:"Akun Titip", multiSelection:false, maxLength:10, tag:2});			
		this.e_debet = new saiLabelEdit(this.pc2.childPage[0],{bound:[770,15,220,20],caption:"Total Debet", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,305], childPage:["Item Jurnal"]});		
...
window.app_saku3_transaksi_tu_kb_fKbTitipMRekNonAgenda.extend(window.childForm);
window.app_saku3_transaksi_tu_kb_fKbTitipMRekNonAgenda.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_titip (no_kas,kode_lokasi,akun_titip,keterangan,kode_pp,modul,periode,kode_curr,kurs,nilai,dc,kode_vendor, no_agenda) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_titip.getText()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBTITIP','"+this.e_periode.getText()+"','IDR',1,"+nilaiToFloat(this.e_debet.getText())+",'D','-', 'NONAGENDA')");
												
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','NONAGENDA','-','"+this.cb_titip.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBTITIP','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','-','-','-')");
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','NONAGENDA','"+this.dp_d1.getDateString()+"',999,'"+this.cb_titip.getText()+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_kb.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBTITIP','TITIP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");										
					
					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
										"('"+this.e_nb.getText()+"','NONAGENDA','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"','-','-','-','"+this.app._lokasi+"','KBTITIP','UMUM','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");
							}
						}
					}
