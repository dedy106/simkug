window.app_saku3_transaksi_tu_kb_fDosen = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_kb_fDosen.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_kb_fDosen";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Dosen - Pajak", 0);	
		...
		this.c_status = new saiCB(this.pc1.childPage[1],{bound:[20,21,200,20],caption:"Status WNI",items:["WNI","WNA"], readOnly:true,tag:2});						
		this.e_nikkk = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,22,500,20],caption:"NIK KK", maxLength:50, tag:1});
		this.cb_pajak = new saiCBBL(this.pc1.childPage[1],{bound:[20,14,220,20],caption:"Status Pajak", multiSelection:false, maxLength:10, tag:2});							
		...
		try {
			...
			this.cb_pajak.setSQL("select kode_pajak , nama from it_stspajak a where kode_lokasi='"+this.app._lokasi+"'",["kode_pajak","nama"],false,["Kode","Nama"],"and","Data Sts Pajak",true);											
...
window.app_saku3_transaksi_tu_kb_fDosen.extend(window.childForm);
window.app_saku3_transaksi_tu_kb_fDosen.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into it_dosen(kode_dosen,kode_lokasi,nama,gelar,alamat,no_tel,email,npwp,bank,cabang,no_rek,nama_rek, sts_wni,nik_kk,kode_pajak) values "+
						    "('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.e_gelar.getText()+"','"+this.e_alamat.getText()+"','"+this.e_tel.getText()+"','"+this.e_mail.getText()+"','"+this.e_npwp.getText()+"','"+this.e_bank.getText()+"','"+this.e_cabang.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"', '"+this.c_status.getText()+"','"+this.e_nikkk.getText()+"','"+this.cb_pajak.getText()+"')");
					setTipeButton(tbAllFalse);
			...
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){				
				...
				if (typeof data == "object"){
					...
					if (line != undefined){												
						...
						this.c_status.setText(line.sts_wni);
						this.e_nikkk.setText(line.nik_kk);
