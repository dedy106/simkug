window.app_saku3_transaksi_tu_fLatihUser = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_fLatihUser.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_fLatihUser";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan Biaya Pelatihan", 0);	
		...
		this.cb_pp = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,11,220,20],caption:"PP Pengaju",tag:2,change:[this,"doChange"]}); 		
		this.e_ket = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,16,550,20],caption:"Uraian", maxLength:150});				
		this.e_nilai = new saiLabelEdit(this.pc2.childPage[0],{bound:[780,17,200,20],caption:"Nilai Bruto", tag:1, tipeText:ttNilai, readOnly:true, text:"0"});
		this.cb_nik = new saiCBBL(this.pc2.childPage[0],{bound:[20,17,220,20],caption:"NIP", multiSelection:false, maxLength:10, tag:2});				
...
window.app_saku3_transaksi_tu_fLatihUser.extend(window.childForm);
window.app_saku3_transaksi_tu_fLatihUser.implement({	
	doLoadParam: function(){		
		var data = this.dbLib.getDataProvider("select kode_param,nama,sat,tarif from tu_param_latih where kode_lokasi='"+this.app._lokasi+"'",true);
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						sql.add("delete from tu_latih_m where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");	
						sql.add("delete from tu_latih_d where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");									
						sql.add("delete from tu_latih_rek where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");																			
					}
					
					sql.add("insert into tu_latih_m(no_bukti,kode_lokasi,periode,tanggal,kode_pp,nik,keterangan,nilai,tgl_input,nik_user,no_aju) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_pp.getText()+"','"+this.cb_nik.getText()+"','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",getdate(),'"+this.app._userLog+"','-')");					
					
					sql.add("insert into tu_latih_rek(no_bukti,kode_lokasi,bank,no_rek,nama_rek,bank_trans,nilai,keterangan) values "+
					         "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_bank.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"','-',"+nilaiToFloat(this.e_nilai.getText())+",'-')");										
					
					if (this.sg.getRowValidCount() > 0) {
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){								                           
								sql.add("insert into tu_latih_d (no_bukti,kode_ppaju,kode_pp,nik,kode_lokasi,kode_param,tarif) values "+
										"('"+this.e_nb.getText()+"','"+this.cb_ppaju.getText()+"','"+this.cb_pp.getText()+"','"+this.cb_nik.getText()+"','"+this.app._lokasi+"','"+this.sg.cells(0,i)+"',"+nilaiToFloat(this.sg.cells(3,i))+")");
							}
						}
					}
	...
	doClick:function(sender){
		...
		this.stsSimpan = 1;			
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"tu_latih_m","no_bukti",this.app._lokasi+"-LTH"+this.e_periode.getText().substr(2,4)+".","000"));
		this.cb_pp.setFocus();
		setTipeButton(tbSimpan);
	},	
	doChange:function(sender){
		if (sender == this.cb_pp && this.cb_pp.getText() != "") {
			this.cb_nik.setSQL("select kode_dosen, nama from tu_dosen where kode_pp ='"+this.cb_pp.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_dosen","nama"],false,["Kode","Nama"],"and","Data Dosen",true);	
		}
	},
	...
	doLoad3:function(sender){																
		var strSQL = "select no_bukti, convert(varchar,tanggal,103) as tgl, keterangan, nilai "+
					 "from tu_latih_m "+
					 "where kode_pp='"+this.cb_pp.getText()+"' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider("select a.tanggal,a.periode,a.keterangan,a.nilai,a.kode_pp,a.nik, r.bank,r.no_rek,r.nama_rek,a.user_input   "+
						   "from tu_latih_m a inner join tu_latih_rek r on a.no_bukti=r.no_bukti and a.kode_lokasi=r.kode_lokasi "+
