window.app_saku3_transaksi_tu_tak_fTakTerima = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_tu_tak_fTakTerima.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_tak_fTakTerima";
		...
		this.sgt = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,230],colCount:9,tag:0,
		...
		this.sgv = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:9,tag:0,
		            colTitle:["Kode Akun","Nama Akun","DC","Keterangan","Nilai","Kode PP","Nama PP","Kode DRK","Nama DRK"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[150,80,150,80,100,270,50,200,80]],					
		...
		this.e_defisit = new saiLabelEdit(this.pc1.childPage[3],{bound:[20,11,200,20],caption:"Total Defisit", readOnly:true, tipeText:ttNilai, text:"0"});		
		this.bRRA = new button(this.pc1.childPage[3],{bound:[320,11,80,18],caption:"RRA",click:[this,"doRRA"]});			
		this.sg2 = new saiGrid(this.pc1.childPage[3],{bound:[1,5,this.pc1.width-5,this.pc1.height-65],colCount:9,tag:9,
...
window.app_saku3_transaksi_tu_tak_fTakTerima.extend(window.childForm);
window.app_saku3_transaksi_tu_tak_fTakTerima.implement({	
	doRRA: function() {
		if (nilaiToFloat(this.e_defisit.getText()) < 0) {
			
			uses("server_util_arrayList");
			var sql = new server_util_arrayList();		
					
			var nilairra = Math.abs(nilaiToFloat(this.e_defisit.getText())); 
			var noRRA = this.standarLib.noBuktiOtomatis(this.dbLib,"anggaran_m","no_agg",this.app._lokasi+"-RRA"+this.e_periode.getText().substr(2,4)+".","0000");
			this.e_nb.setText(noRRA); //utk pesan saat simpan muncul no bukti rra
			sql.add("insert into anggaran_m (no_agg,kode_lokasi,no_dokumen,tanggal,keterangan,tahun,kode_curr,nilai,tgl_input,nik_user,posted,no_del,nik_buat,nik_setuju,jenis) values  "+
					"('"+noRRA+"','"+this.app._lokasi+"','"+this.e_nobukti.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.e_periode.getText().substr(0,4)+"','IDR',"+nilairra+",getdate(),'"+this.app._userLog+"','T','-','-','-','RR')");
			...
			sql.add("insert into rra_pdrk_m(no_pdrk,kode_lokasi,keterangan,kode_pp,kode_bidang,jenis_agg,tanggal,periode,nik_buat,nik_app1,nik_app2,nik_app3,sts_pdrk,justifikasi, nik_user, tgl_input,progress,modul) values "+
					"('"+noRRA+"','"+this.app._lokasi+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_periode.getText()+"','"+this.app._userLog+"','-','-','-','RRR','-','"+this.app._userLog+"',now(),'A','MULTI')");						

			if (this.sg2.getRowValidCount() > 0){
				for (var i=0;i < this.sg2.getRowCount();i++){
					if (this.sg2.rowValid(i) && nilaiToFloat(this.sg2.cells(8,i))<0) {												
						sql.add("insert into rra_pdrk_d(no_pdrk,kode_lokasi,no_urut,kode_akun,kode_pp,kode_drk,periode,saldo,nilai,dc,target) values "+
								"('"+noRRA+"','"+this.app._lokasi+"',"+i+",'"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(4,i)+"','"+this.e_periode.getText()+"',0,"+Math.abs(parseNilai(this.sg2.cells(8,i)))+",'D','-')");
					}
				}
			}
		...
		else system.alert(this,"Transaksi harus defisit budget.","");
	},
			...
			case "ubah" :														
				...
				for (var i=0;i < this.sgv.getRowCount();i++){					
					if (!this.sgv.rowValid(i)){
						...
						if (nilaiToFloat(this.sgv.cells(4,i)) < 0) {
							var k = i+1;
							system.alert(this,"Transaksi tidak valid.","Nilai harus lebih besar dari nol.[Baris : "+k+"]");
							return false;
						}
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg.cells(0,row) != "") {							
				...
				if (this.e_modul.getText() == "TAKUMUM" || this.e_modul.getText() == "TAKBEA" || this.e_modul.getText() == "TAKBILL" || this.e_modul.getText() == "TAKSUSUT" || this.e_modul.getText() == "TAKREV" || this.e_modul.getText() == "TAKPDREV" || this.e_modul.getText() == "PIUPRO" || this.e_modul.getText() == "TAKPJPTG") {
	...
	doHitungGar: function(){
		...
		for (var i=0;i < this.sg2.getRowCount();i++){
			...
			if (nilaiToFloat(this.sg2.cells(8,i)) < 0) {
				defisit += nilaiToFloat(this.sg2.cells(8,i));
			}			
		}
		this.e_defisit.setText(floatToNilai(defisit));
	},


	doCellEnterd: function(sender, col, row){
		switch(col){
			case 2 : 
					if (this.sgd.cells(2,row) == ""){
						this.sgd.setCell(2,row,this.app._kodePP);						
					}
				break;			
		}
	},
	doEllipsClickd: function(sender, col, row){
		try{			
			if (sender == this.sgd) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Akun",sender,undefined, 
				...
				if (col == 4){					
					...
					this.standarLib.showListData(this, "Daftar DRK",sender,undefined, 
													  "select distinct a.kode_drk, a.nama from drk a inner join anggaran_d b on a.kode_drk=b.kode_drk where a.tahun=substring(b.periode,1,4) and b.periode like '"+this.e_periode.getText().substr(0,4)+"%' and b.kode_akun='"+this.sgd.cells(0,row)+"' and b.kode_pp = '"+this.sgd.cells(2,row)+"' and a.kode_lokasi='"+this.app._lokasi+"'",
													  "select count(distinct a.kode_drk)  from drk a inner join anggaran_d b on a.kode_drk=b.kode_drk where a.tahun=substring(b.periode,1,4) and b.periode like '"+this.e_periode.getText().substr(0,4)+"%' and b.kode_akun='"+this.sgd.cells(0,row)+"' and b.kode_pp = '"+this.sgd.cells(2,row)+"' and a.kode_lokasi='"+this.app._lokasi+"'",
	...
	doChangeCelld: function(sender, col, row){
		if ((col == 6 || col == 8) && (this.sgd.cells(8,row) != "")) this.sgd.validasi();
		sender.onChange.set(undefined,undefined);
	    if (col == 0) {
			if (this.sgd.cells(0,row) != "") {
				var akun = this.dataAkun.get(sender.cells(0,row));
				if (akun) sender.cells(1,row,akun);
				else {                                    
					if (trim(sender.cells(0,row)) != "") system.alert(this,"Kode Akun "+sender.cells(0,row)+" tidak ditemukan","Inputkan kode lainnya.","checkAkun");                
		...
		if (col == 4) {
			if (this.sgd.cells(4,row) != "") {
				...
				var data = this.dbLib.getDataProvider("select distinct a.nama from drk a inner join anggaran_d b on a.kode_drk=b.kode_drk where a.tahun=substring(b.periode,1,4) and b.periode like '"+this.e_periode.getText().substr(0,4)+"%' and b.kode_akun='"+this.sgd.cells(0,row)+"' and b.kode_pp = '"+this.sgd.cells(2,row)+"' and b.kode_drk = '"+this.sgd.cells(4,row)+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined) this.sgd.cells(5,row,line.nama);
					else {
						if (!isAda) this.sgd.cells(4,row,"-");
						else {
							this.sgd.cells(4,row,"");
							this.sgd.cells(5,row,"");
						}
					}
				}
			}
		...
		if (col == 0 || col == 2 || col == 4 || col == 6) {
			if (this.sgd.cells(0,row) !="") {
				var data = this.dbLib.getDataProvider("select fn_cekaggBulan('"+this.sgd.cells(2,row)+"','"+this.app._lokasi+"','"+this.sgd.cells(0,row)+"','"+this.sgd.cells(4,row)+"','"+this.e_periode.getText().substr(0,4)+this.sgd.cells(6,row)+"','"+this.e_nb.getText()+"') as gar ",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line = data.rs.rows[0];
					data = line.gar.split(";");
					sls = parseFloat(data[0]) - parseFloat(data[1]);
					this.sgd.cells(7,row,floatToNilai(sls));				
				}						
			}
		}		
		sender.onChange.set(this,"doChangeCelld");		
	},
		...
		try{
			...
			for (var i = 0; i < this.sgd.rows.getLength();i++){
				if (this.sgd.rowValid(i) && this.sgd.cells(8,i) != ""){					
					totC += nilaiToFloat(this.sgd.cells(8,i));
				}
			}
