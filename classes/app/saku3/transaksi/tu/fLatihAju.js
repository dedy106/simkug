window.app_saku3_transaksi_tu_fLatihAju = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_fLatihAju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_fLatihAju";
		...
		this.cb_ppaju = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,11,220,20],caption:"PP Pengaju",tag:2,multiSelection:false,change:[this,"doChange"]}); 		
		
		this.cb_pp = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,13,220,20],caption:"PP Budget",tag:2,change:[this,"doChange"]}); 		
		...
		this.cb_nik = new saiCBBL(this.pc2.childPage[0],{bound:[20,15,220,20],caption:"NIP", multiSelection:false, maxLength:10, tag:2});				
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:4,tag:9,
		            colTitle:["Kode Biaya","Deskripsi","Satuan","Nilai"],
		...
		try {
			...
			this.cb_ppaju.setSQL("select a.kode_pp, a.nama from pp a inner join karyawan_pp d on a.kode_pp = d.kode_pp and a.kode_lokasi=d.kode_lokasi and d.nik='"+this.app._userLog+"' "+
			...
			var data = this.dbLib.getDataProvider("select flag from spro where kode_spro='PPSDM' and kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line = data.rs.rows[0];							
				this.ppSDM = line.flag;
			} else this.ppSDM = "-";
			
			this.cb_pp.setSQL("select a.kode_pp, a.nama from pp a where a.kode_pp='"+this.ppSDM+"' and a.kode_lokasi='"+this.app._lokasi+"'",["a.kode_pp","a.nama"],false,["Kode","Nama"],"and","Data Prodi/Unit",true);			
			this.cb_pp.setText(this.ppSDM);
...
window.app_saku3_transaksi_tu_fLatihAju.extend(window.childForm);
window.app_saku3_transaksi_tu_fLatihAju.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from tu_latih_aju where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");										
					...
					sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_kpa,no_app,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,user_input,form,sts_pajak,npajak) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.c_jenis.getText()+"','"+this.cb_akun.getText()+"','"+this.cb_pp.getText()+"','"+this.cb_drk.getText()+"','"+this.e_ket.getText()+"',"+netto+",getdate(),'"+this.app._userLog+"','-','-','-','-','-','A','-','-','"+this.e_user.getText()+"','LATIH','NON',0)");					
					...
					if (this.sg.getRowValidCount() > 0) {
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){								                           
								sql.add("insert into tu_latih_aju (no_bukti,kode_ppaju,kode_pp,nik,kode_lokasi,kode_param,tarif) values "+
	...
	doChange:function(sender){
		if (sender == this.cb_ppaju && this.cb_ppaju.getText() != "") {
			this.cb_nik.setSQL("select nik, nama from karyawan where kode_pp ='"+this.cb_ppaju.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Data Karyawan",true);	
	...
	doLoad3:function(sender){																
		var strSQL = "select no_aju, convert(varchar,tanggal,103) as tgl, keterangan, nilai "+
					 "from it_aju_m "+
					 "where kode_pp='"+this.cb_pp.getText()+"' and form = 'LATIH' and modul='UMUM' and progress in ('A','K','R') and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select b.nik,a.nilai+a.pajak as bruto,a.pajak,a.nilai,b.nama,b.gelar,b.bank+' - '+b.cabang+' - '+b.no_rek+' - '+b.nama_rek as rek,a.berita "+
				...
				var data = this.dbLib.getDataProvider("select a.kode_param,a.nama,a.sat,b.tarif,b.nik "+
													  "from tu_param_latih a inner join tu_latih_aju b on a.kode_param=b.kode_param and a.kode_lokasi=b.kode_lokasi "+
