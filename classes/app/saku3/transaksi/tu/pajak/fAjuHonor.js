window.app_saku3_transaksi_tu_pajak_fAjuHonor = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_pajak_fAjuHonor.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_pajak_fAjuHonor";
		...
		this.c_bayar = new saiCB(this.pc2.childPage[0],{bound:[790,12,200,20],caption:"Jenis Bayar",items:["TRANSFER"], readOnly:true,tag:2,change:[this,"doChange"]});
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:0,
				colTitle:["Kd Dosen","Bruto","Pot. Pajak","Netto","Berita","Nama Dosen","Gelar","Rekening","NPWP"],
...
window.app_saku3_transaksi_tu_pajak_fAjuHonor.extend(window.childForm);
window.app_saku3_transaksi_tu_pajak_fAjuHonor.implement({	
		...
		try {						
			...
			for (var i=0;i < this.sg1.getRowCount();i++){			
				if (this.sg1.rowValid(i)){
					var data = this.dbLib.getDataProvider("select nama,gelar,bank+' - '+cabang+' - '+no_rek+' - '+nama_rek as rekening,npwp from it_dosen where kode_dosen='"+this.sg1.cells(0,i)+"' and kode_lokasi='"+this.app._lokasi+"'",true);
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
						if (line != undefined) {
							
							//if (line.npwp == "-") var persen = 0.06;
							//else var persen = 0.05;
							//this.sg1.cells(2,i,nilaiToFloat(this.sg1.cells(1,i)) * 0.5 * persen);

							//var strSQL2 = "select (case when '"+line.npwp+"' <> '-' then persen/100 else persen2/100 end) * "+nilaiToFloat(this.sg1.cells(1,i))+" * 0.5 as nilai_pph "+
						...
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){
								...
								sql.add("insert into it_aju_rek(no_aju,kode_lokasi,bank,no_rek,nama_rek,bank_trans,nilai,keterangan,pajak,berita,kode_pajak) values "+
								        "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','-','-','-','-',"+nilaiToFloat(this.sg1.cells(3,i))+",'"+this.sg1.cells(0,i)+"',"+nilaiToFloat(this.sg1.cells(2,i))+",'"+this.sg1.cells(4,i)+"','"+kodePajak+"')");
							}
						}
					}		
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_aju, convert(varchar,a.tanggal,103) as tgl, a.keterangan, a.nilai "+
					 ...
					 "where a.form = 'HONOR1' and a.modul='UMUM' and a.progress in ('A','K','R') and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select b.kode_dosen,a.nilai+a.pajak as bruto,a.pajak,a.nilai,b.nama,b.gelar,b.bank+' - '+b.cabang+' - '+b.no_rek+' - '+b.nama_rek as rek,a.berita,b.npwp "+
				             "from it_aju_rek a inner join it_dosen b on a.keterangan=b.kode_dosen and a.kode_lokasi=b.kode_lokasi "+
							 "where a.no_aju='"+this.e_nb.getText()+"' and a.kode_lokasi ='"+this.app._lokasi+"'";		
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
