window.app_saku3_transaksi_tu_pajak_fPegawai = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_pajak_fPegawai.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_pajak_fPegawai";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Pegawai", 0);	
		
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		uses("saiGrid",true);		
				
		this.pc1 = new pageControl(this,{bound:[20,12,1000,400], childPage:["Daftar Pegawai","Data Pegawai","Filter Cari"]});
		...
		this.c_status = new saiCB(this.pc1.childPage[1],{bound:[20,11,200,20],caption:"Status",items:["AKTIF","NON"], readOnly:true,tag:2});		
...
window.app_saku3_transaksi_tu_pajak_fPegawai.extend(window.childForm);
window.app_saku3_transaksi_tu_pajak_fPegawai.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into it_pegawai(nik,kode_lokasi,nama,gelar,alamat,no_tel,email,npwp,bank,cabang,no_rek,nama_rek,no_ktp,flag_aktif) values "+
						    "('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.e_gelar.getText()+"','"+this.e_alamat.getText()+"','"+this.e_tel.getText()+"','"+this.e_mail.getText()+"','"+this.e_npwp.getText()+"','"+this.e_bank.getText()+"','"+this.e_cabang.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"','"+this.e_ktp.getText()+"','"+this.c_status.getText()+"')");
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from it_pegawai where nik = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");			
					sql.add("insert into it_pegawai(nik,kode_lokasi,nama,gelar,alamat,no_tel,email,npwp,bank,cabang,no_rek,nama_rek,no_ktp,flag_aktif) values "+
						    "('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.e_gelar.getText()+"','"+this.e_alamat.getText()+"','"+this.e_tel.getText()+"','"+this.e_mail.getText()+"','"+this.e_npwp.getText()+"','"+this.e_bank.getText()+"','"+this.e_cabang.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"','"+this.e_ktp.getText()+"','"+this.c_status.getText()+"')");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
	...
	doCari:function(sender){								
		try {
			...
			var strSQL = "select nik,nama,alamat,kode_lokasi "+
						 "from it_pegawai where kode_lokasi='"+this.app._lokasi+"' and "+filter+" order by nik";								
			var data = this.dbLib.getDataProvider(strSQL,true);		
	...
	doLoad:function(sender){								
		try {
			var strSQL = "select nik,nama,alamat "+
						 "from it_pegawai where kode_lokasi='"+this.app._lokasi+"' order by nik";				
			var data = this.dbLib.getDataProvider(strSQL,true);		
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU = data;
				this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length/20));
