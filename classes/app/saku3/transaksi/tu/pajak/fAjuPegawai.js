window.app_saku3_transaksi_tu_pajak_fAjuPegawai = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_pajak_fAjuPegawai.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_pajak_fAjuPegawai";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan Pegawai", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,20,990,237], childPage:["Detail Gaji","Error Data"]});		
		...
		try {
			...
			this.cb_akun.setSQL("select a.kode_akun, a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+								
								"where b.kode_flag in ('041') and b.kode_akun like '2%' and a.kode_lokasi='"+this.app._lokasi+"'",["kode_akun","nama"],false,["Kode","Nama"],"and","Data Akun",true);													
...
window.app_saku3_transaksi_tu_pajak_fAjuPegawai.extend(window.childForm);
window.app_saku3_transaksi_tu_pajak_fAjuPegawai.implement({	
	...
	doLoad3:function(sender){																
		var strSQL = "select no_aju, convert(varchar,tanggal,103) as tgl, keterangan, nilai "+
					 "from it_aju_m "+
					 "where kode_pp='"+this.cb_pp.getText()+"' and form = 'PEGAWAI' and modul='UMUM' and progress in ('A','K','R') and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";
