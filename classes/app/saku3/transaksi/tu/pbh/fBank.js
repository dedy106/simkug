window.app_saku3_transaksi_tu_pbh_fBank = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_pbh_fBank.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_pbh_fBank";
		...
		this.e_pj = new saiLabelEdit(this.pc2.childPage[1],{bound:[20,12,200,20],caption:"Panjang Rekening", tipeText:ttNilai, text:"0", tag:1});	
...
window.app_saku3_transaksi_tu_pbh_fBank.extend(window.childForm);
window.app_saku3_transaksi_tu_pbh_fBank.implement({	
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into bank(kode_bank,nama,pj_rek,kode_lokasi) values "+
						    "('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"',"+this.e_pj.getText()+",'"+this.app._lokasi+"')");					
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					...
					var sql = new server_util_arrayList();
					sql.add("update bank "+
							"set nama='"+this.e_nama.getText()+"',pj_rek="+this.e_pj.getText()+" "+
	...
	doLoad3:function(sender){														
		var strSQL = "select kode_bank,nama from bank where kode_lokasi='"+this.app._lokasi+"' order by kode_bank";
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.kode_bankklp,line.nama,"Pilih"]); 
		}
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select kode_bank,nama,pj_rek "+
				             "from bank "+
						     "where kode_bank ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
				var data = this.dbLib.getDataProvider(strSQL,true);
