window.app_saku3_transaksi_tu_pbh_fVerDokFisik = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_pbh_fVerDokFisik.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_pbh_fVerDokFisik";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Verifikasi Dokumen Fisik", 0);	
		...
		this.e_nilai = new saiLabelEdit(this,{bound:[20,17,202,20],caption:"Nominal", tipeText:ttNilai, text:"0",change:[this,"doChange"]});
...
window.app_saku3_transaksi_tu_pbh_fVerDokFisik.extend(window.childForm);
window.app_saku3_transaksi_tu_pbh_fVerDokFisik.implement({	
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("update it_aju_m set progress='1', tanggal=getdate(), no_app='"+this.e_nb.getText()+"' where no_aju='"+this.e_noaju.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and progress='0'");
	...
	doClick:function(sender){
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"it_ajuapp_m","no_app",this.app._lokasi+"-VF"+this.e_periode.getText().substr(2,2)+".","00000"));
		...
		if (sender==this.e_noaju || sender==this.e_nilai) {			
			if (this.e_noaju.getText() != "" && this.e_nilai.getText() != "0") {
				var strSQL = "select a.nilai,a.kode_pp,a.keterangan,a.tanggal,a.kode_pp+' - '+b.nama as pp,a.kode_akun+' - '+c.nama as akun,a.kode_drk+' - '+isnull(d.nama,'-') as drk "+
							 ...
							 "where a.no_aju = '"+this.e_noaju.getText()+"' and a.nilai="+nilaiToFloat(this.e_nilai.getText())+" and a.kode_lokasi='"+this.app._lokasi+"' and a.progress='0' and form in ('HONDOSM')";				
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					system.alert(this,"No Agenda dan Nominal tidak valid (Progress tidak valid).","Data tidak ditemukan.");															
					this.e_ket.setText("");				
					this.e_pp.setText("");				
					this.e_akun.setText("");				
					this.e_drk.setText("");								
