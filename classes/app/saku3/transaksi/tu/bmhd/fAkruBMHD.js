window.app_saku3_transaksi_tu_bmhd_fAkruBMHD = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_bmhd_fAkruBMHD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_bmhd_fAkruBMHD";		
		...
		this.cb_vendor = new saiCBBL(this.pc2.childPage[0],{bound:[20,14,220,20],caption:"Kode Vendor", multiSelection:false, maxLength:10, tag:2});								
		this.l_tgl2 = new portalui_label(this.pc2.childPage[0],{bound:[770,14,100,18],caption:"Due Date", underline:true});
		this.dp_d2 = new portalui_datePicker(this.pc2.childPage[0],{bound:[870,14,100,18]}); 		
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,235], childPage:["Item Jurnal","Anggaran"]});
		...
		try {
			...
			this.cb_vendor.setSQL("select kode_vendor,nama from it_vendor where  kode_lokasi='"+this.app._lokasi+"'",["kode_vendor","nama"],false,["Kode","Nama"],"and","Data Vendor",true);
...
window.app_saku3_transaksi_tu_bmhd_fAkruBMHD.extend(window.childForm);
window.app_saku3_transaksi_tu_bmhd_fAkruBMHD.implement({	
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into bmhd_m(no_bmhd,kode_lokasi,no_dokumen,kode_akun,tanggal,keterangan,kode_pp,modul,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1, dc,kode_vendor, due_date) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.cb_bmhd.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_pp.getText()+"','BMHD','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.app._userLog+"','"+this.cb_app.getText()+"',getdate(),'"+this.app._userLog+"','F','-','-','-','D','"+this.cb_vendor.getText()+"','"+this.dp_d2.getDateString()+"')");
	...
	doSelectDate: function(sender, y,m,d){
		...
		if (this.stsSimpan == 1) {
			...
			if (m == "12") {
				var z = y+1;
				this.dp_d2.setText("31/03/"+z.toString());
			}
			else {
				this.dp_d2.setText("31/12/"+y);
			}
		}
	},
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "") {
			...
			this.stsSimpan = 1;
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"bmhd_m","no_bmhd",this.app._lokasi+"-HB"+this.e_periode.getText().substr(2,4)+".","0000"));						
			this.cb_pp.setFocus();
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_bmhd,convert(varchar,a.tanggal,103) as tgl,'BMHD' as jenis,a.no_dokumen,a.keterangan,a.nilai,a.kode_vendor,a.kode_akun as akun_bmhd,c.nama as nama_vendor "+
		             "from bmhd_m a "+				
					 "inner join it_vendor c on a.kode_vendor=c.kode_vendor and a.kode_lokasi=c.kode_lokasi "+
