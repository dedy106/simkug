window.app_saku3_transaksi_tu_bmhd_fAjuBMHDmrek = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_bmhd_fAjuBMHDmrek.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_bmhd_fAjuBMHDmrek";
		...
		this.cb_akun = new portalui_saiCBBL(this.pc1.childPage[1],{bound:[20,15,222,20],caption:"MTA",tag:1,readOnly:true});         				
		...
		this.e_duedate = new saiLabelEdit(this.pc1.childPage[1],{bound:[790,12,200,20],caption:"Due Date", readOnly:true});										
		this.e_nilai = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,16,200,20],caption:"Nilai (Bruto)", tag:1, tipeText:ttNilai, text:"0",readOnly:true});
		this.c_status = new saiCB(this.pc1.childPage[1],{bound:[790,16,200,20],caption:"Status Pajak",items:["NON","P21","P23"], readOnly:true,tag:2,change:[this,"doChange"]});				
		this.e_ket = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,17,550,20],caption:"Uraian", maxLength:150});										
		this.e_npajak = new saiLabelEdit(this.pc1.childPage[1],{bound:[790,17,200,20],caption:"Nilai Pajak", tag:0, tipeText:ttNilai, text:"0",readOnly:true});
		
		this.pc2 = new pageControl(this.pc1.childPage[1],{bound:[1,12,996,187], childPage:["Rekening"]});		
		this.sgRek = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc1.width-7,this.pc2.height-33],colCount:5,tag:0,
		            colTitle:["Nama Rek","No Rek","Bank-Cabang","Netto","Pajak"],
					colWidth:[[4,3,2,1,0],[100,100,300,200,200]],					
		...
		this.sgnRek = new portalui_sgNavigator(this.pc2.childPage[0],{bound:[1,this.pc2.height-25,this.pc2.width-1,25],buttonStyle:2,grid:this.sgRek,pager:[this,"doPage"]});		
...
window.app_saku3_transaksi_tu_bmhd_fAjuBMHDmrek.extend(window.childForm);
window.app_saku3_transaksi_tu_bmhd_fAjuBMHDmrek.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.sgRek.getRowValidCount() > 0){
						for (var i=0;i < this.sgRek.getRowCount();i++){
							if (this.sgRek.rowValid(i)){										
								sql.add("insert into it_aju_rek(no_aju,kode_lokasi,bank,no_rek,nama_rek,bank_trans,nilai,keterangan,pajak) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.sgRek.cells(2,i)+"','"+this.sgRek.cells(1,i)+"','"+this.sgRek.cells(0,i)+"','-',"+nilaiToFloat(this.sgRek.cells(3,i))+",'"+this.e_ket.getText()+"',"+nilaiToFloat(this.sgRek.cells(4,i))+")");																						
							}
						}
					}
					...
					sql.add("insert into bmhd_bayar (no_aju,no_bmhd,kode_lokasi,akun_bmhd,keterangan,kode_pp,modul,periode,kode_curr,kurs,nilai,dc,kode_vendor,sts_pajak,pajak) values  "+
							"('"+this.e_nb.getText()+"','"+this.cb_kb.getText()+"','"+this.app._lokasi+"','"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','AJU_M','"+this.e_periode.getText()+"','IDR',1,"+nilaiToFloat(this.e_nilai.getText())+",'D','"+this.cb_vendor.getText()+"','"+this.c_status.getText()+"',"+nilaiToFloat(this.e_npajak.getText())+")");
		...
		switch (event){
			...
			case "simpan" :		
				...
				if (nilaiToFloat(this.e_npajak.getText()) != 0 && this.c_status.getText() == "NON") {
					system.alert(this,"Transaksi tidak valid.","Status pajak tidak boleh NON.");
					return false;
				}

				if (nilaiToFloat(this.e_nilai.getText()) <= 0 || nilaiToFloat(this.e_npajak.getText()) < 0) {
					system.alert(this,"Transaksi tidak valid.","Nilai / pajak tidak boleh nol atau kurang.");
					return false;
				}
	...
	doSelectDate: function(sender, y,m,d){
		...
		// var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('TGLBMHD')",true);			
		// if (typeof data == "object"){
		// 	var line;
		// 	for (var i in data.rs.rows){
		// 		line = data.rs.rows[i];																	
		// 		if (line.kode_spro == "TGLBMHD") this.tglBatas = line.flag+"/12/"+this.e_periode.getText().substr(0,4);						
		// 	}
		// }
	...
	doChange:function(sender){
		...
		if (sender == this.cb_kb && this.cb_kb.getText() != "") {
			var strSQL =  "select c.kode_akun,c.nama,a.nilai-isnull(b.pakai,0) as saldo,convert(varchar,a.due_date,103) as due_date "+ //,d.bank,d.cabang,d.no_rek,d.nama_rek 
						  "from bmhd_m a "+
		...
		if (sender == this.e_nb && this.e_nb.getText() != "" && this.stsSimpan == 0) {
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sgRek.appendData([line.nama_rek,line.no_rek,line.bank,floatToNilai(line.nilai),floatToNilai(line.pajak)]);
				}
	...
	doAfterPaste: function(sender,totalRow){
		try {
			this.sgnRek.setTotalPage(sender.getTotalPage());
			this.sgnRek.rearrange();
			this.doNilaiChangeRek();
		} catch(e) {alert(e);}
	},
	doPage: function(sender,page){
		this.sgnRek.doSelectPage(page);
	},
		...
		try{
			...
			for (var i = 0; i < this.sgRek.getRowCount();i++){
				if (this.sgRek.rowValid(i) && this.sgRek.cells(3,i) != ""){
					bruto += nilaiToFloat(this.sgRek.cells(3,i))+nilaiToFloat(this.sgRek.cells(4,i));					
					pajak += nilaiToFloat(this.sgRek.cells(4,i));					
				}
			}						
			this.e_nilai.setText(floatToNilai(bruto));
