window.app_saku3_transaksi_tu_kantin_fTerimaKB = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_kantin_fTerimaKB.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_kantin_fTerimaKB";
		this.itemsValue = new arrayList();
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Penerimaan Setoran", 0);	
		...
		this.cb_aju = new saiCBBL(this.pc1.childPage[0],{bound:[20,15,220,20],caption:"No Agenda", multiSelection:false, maxLength:20, tag:1,change:[this,"doChange"]});		
		this.e_tot = new portalui_saiLabelEdit(this.pc1.childPage[0],{bound:[790,15,200,20],caption:"Total Pendapatan", maxLength:100, tag:1, readOnly:true, tipeText:ttNilai,text:"0"});	
		
		this.pc2 = new pageControl(this.pc1.childPage[0],{bound:[1,30,995,328], childPage:["Data Load","Data Tenan","Data Pembayaran"]});			
...
window.app_saku3_transaksi_tu_kantin_fTerimaKB.extend(window.childForm);
window.app_saku3_transaksi_tu_kantin_fTerimaKB.implement({
	isiCbAju: function() {
		this.cb_aju.setSQL("select distinct a.no_aju, a.keterangan from it_aju_m a inner join kantin_load b on a.no_aju=b.no_bast and a.kode_lokasi=b.kode_lokasi where b.kode_lokasi='"+this.app._lokasi+"' and b.no_kas='-'",["no_aju","keterangan"],false,["No Agenda","Deskripsi"],"and","Daftar Agenda",true);			
	},
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank)  values "+
					 		"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_aju.getText()+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBKANTIN','BM','"+this.e_periode.getText()+"','IDR',1,"+nilaiToFloat(this.e_tot.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','-','-','-')");										
					var line;
					for (var i=0;i < this.dataJU3.rs.rows.length;i++){
						line = this.dataJU3.rs.rows[i];						
						sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank)  values "+
								"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+line.kb+"','"+this.e_ket.getText()+"','D',"+parseFloat(line.nilai)+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBKANTIN','KAS','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");											
					}
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank)  values "+
		...
		try {
			if (this.stsSimpan == 1) {
				var strSQL = "select 'PROSES' as status,a.no_load,convert(varchar,a.tanggal,103) as tgl,a.kode_kantin+' | '+b.nama as kantin, a.total_nota,a.total_kasir,a.total_bayar "+
							...
							"where a.no_kas = '-' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_bast='"+this.cb_aju.getText()+"'";													
			...
			if (this.sg1.cells(0,row) != "") {			
				...
				if (typeof data == "object"){
					...
					if (line != undefined){	
						...
						this.cb_aju.setSQL("select  a.no_aju, a.keterangan from it_aju_m a where a.kode_lokasi='"+this.app._lokasi+"' and a.no_aju='"+line.no_dokumen+"'",["no_aju","keterangan"],false,["No Agenda","Deskripsi"],"and","Daftar Agenda",true);			
						this.cb_aju.setText(line.no_dokumen);	
