window.app_saku3_transaksi_tu_kantin_fBA = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_kantin_fBA.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_kantin_fBA";
		...
		this.cb_kantin = new saiCBBL(this.pc1.childPage[0],{bound:[20,20,220,20],caption:"Kantin", maxLength:50, tag:1 ,multiSelection:false,change:[this,"doChange"]});
		...
		this.pc2 = new pageControl(this.pc1.childPage[0],{bound:[1,11,995,280], childPage:["Data Closing Shift","Detail Nota","Detail Total"]});						
		...
		this.e_tottenan = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,17,200,20],caption:"Total Tenan", maxLength:50, tag:1,tipeText:ttNilai,text:"0",readOnly:true});
		this.e_totbrg = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,19,200,20],caption:"Total Barang", maxLength:50, tag:1,tipeText:ttNilai,text:"0",readOnly:true});
		...
		this.sg4 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:4,tag:1,
		            colTitle:["Tenan","Jml Nota","Nilai Tenan","Nilai Barang"],
					colWidth:[[3,2,1,0],[100,100,100,250]],
		...
		try {
			...
			this.e_pic.setSQL("select nik, nama from ktu_user where kode_lokasi='"+this.app._lokasi+"' ",["nik","nama"],false,["NIK","Nama"],"and","Data PIC",true);
			this.cb_kantin.setSQL("select kode_kantin,nama from ktu_kantin where kode_lokasi='"+this.app._lokasi+"' ",["kode_kantin","nama"],false,["Kode","Nama"],"and","Data Kantin",false);
...
window.app_saku3_transaksi_tu_kantin_fBA.extend(window.childForm);
window.app_saku3_transaksi_tu_kantin_fBA.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					if(this.stsSimpan == 0){
						...
						sql.add("update a set a.no_ba='-' from ktu_nota_m a "+
					...
					sql.add("update a set a.no_ba='"+this.e_nb.getText()+"' from ktu_nota_m a "+
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk) {
					...
					setTipeButton(tbAllFalse);
					this.cb_closing.setSQL("select no_closing, ket from ktu_closing_m where kode_kantin='"+this.cb_kantin.getText()+"' and no_ba ='-' and kode_lokasi='"+this.app._lokasi+"' ",["no_closing","ket"],false,["Kode","Deskripsi"],"and","Data Closing Shift",false);
					this.stsSimpan = 1;
	...
	doChange: function(sender){
		try{
			if(sender == this.cb_kantin && this.cb_kantin.getText() != ""){
					this.cb_closing.setSQL("select no_closing, ket from ktu_closing_m where kode_kantin= '"+this.cb_kantin.getText()+"' and no_ba ='-' and kode_lokasi='"+this.app._lokasi+"' ",["no_closing","ket"],false,["Kode","Deskripsi"],"and","Data Closing Shift",false);
			}
			if (sender == this.cb_closing && this.cb_closing.getText() != ""){
				...
				var strSQL  = "select  x.kode_tenan,x.nama, isnull(y.jml_nota,0) as jml_nota, "+
							"case when x.jenis='TENAN' then nilai else 0 end as tenan, "+
							"case when x.jenis='BRG' then nilai else 0 end as barang "+
							"from  "+
							"( "+
							"	select c.kode_tenan,c.nama, 'TENAN' as jenis, isnull(sum(b.nilai),0) as nilai "+
							"	from ktu_nota_m a "+
							"	inner join ktu_nota_d b on a.no_bukti = b.no_bukti and a.kode_lokasi=b.kode_lokasi "+
							"	inner join ktu_tenan c on b.kode_tenan=c.kode_tenan and b.kode_lokasi=c.kode_lokasi "+
							"	where a.no_closing = '"+this.cb_closing.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' "+ 
							"	group by c.kode_tenan,c.nama "+
							
							"	union all "+
							
							"	select 'NONTNN','NON TENAN' as nama, 'BRG' as jenis, isnull(sum(b.nontenan),0) as nilai "+
							"	from ktu_nota_m a "+
							"	inner join ( "+
							"			select no_bukti,kode_lokasi, sum(total) as nontenan "+
							"			from brg_trans_d  "+
							...
							"left join  ( "+
							"	select b.kode_tenan, count(b.no_bukti) as jml_nota "+
							"	from ktu_nota_m a  "+
							"	inner join ktu_nota_d b on a.no_bukti = b.no_bukti and a.kode_lokasi=b.kode_lokasi "+
							"	where a.no_closing = '"+this.cb_closing.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'  "+
							...
							"	select 'NONTNN' as kode_tenan, count(b.no_bukti) as jml_nota "+
							"	from ktu_nota_m a  "+
							"	inner join ( "+
							"			select distinct no_bukti as no_bukti "+
							"			from brg_trans_d  "+
							"			where kode_lokasi ='"+this.app._lokasi+"' "+
							"			group by no_bukti,kode_lokasi "+
							...
							") y on x.kode_tenan=y.kode_tenan "+
							
							"order by x.kode_tenan ";
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					var total = tenan = brg  = 0;
					var line;
					for (var i=0;i < this.dataJU4.rs.rows.length;i++){
						line = this.dataJU4.rs.rows[i];
						tenan += parseFloat(line.tenan);
						brg   += parseFloat(line.barang);
						total += parseFloat(line.tenan)+parseFloat(line.barang);
					}
					this.e_tottenan.setText(floatToNilai(tenan));
					this.e_totbrg.setText(floatToNilai(brg));
					this.e_total.setText(floatToNilai(total));
				} else this.sg4.clear(1);	



				var strSQL = "select bb.kode_bayar as status,SUM(bb.nilai - bb.n_kembali) as nilai,c.kb "+
							 "from ktu_nota_m a  "+
							 "inner join ktu_nota_bayar bb on a.no_bukti=bb.no_bukti and a.kode_lokasi=bb.kode_lokasi "+
							 "inner join ktu_jbayar c on bb.kode_bayar=c.kode_bayar and a.kode_lokasi=c.kode_lokasi "+
							 "inner join ktu_closing_m b on a.no_closing=b.no_closing and a.kode_lokasi=b.kode_lokasi "+
							 "where a.no_closing = '"+this.cb_closing.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' group by bb.kode_bayar,c.kb  ";		 							
				var data = this.dbLib.getDataProvider(strSQL,true);
	...
	doTampilData4: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg4.appendData([line2.kode_tenan.toUpperCase()+" - "+line2.nama.toUpperCase(),floatToNilai(line2.jml_nota),floatToNilai(line2.tenan),floatToNilai(line2.barang)]); 
		}
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {			
				...
				var strSQL = "select a.tanggal,a.keterangan,a.no_closing,a.nik_app,b.kode_kantin from ktu_ba a inner join ktu_closing_m b on a.no_closing=b.no_closing where a.no_ba ='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' ";						   				
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){						
