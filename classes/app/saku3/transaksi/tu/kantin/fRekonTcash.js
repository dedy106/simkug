window.app_saku3_transaksi_tu_kantin_fRekonTcash = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_kantin_fRekonTcash.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_kantin_fRekonTcash";
		...
		this.cb_kantin = new saiCBBL(this.pc1.childPage[0],{bound:[20,11,220,20],caption:"Kantin", maxLength:50, tag:1, multiSelection:false,change:[this,"doChange"]});	
		...
		this.cb_kb = new saiCBBL(this.pc1.childPage[0],{bound:[20,16,220,20],caption:"Akun Titipan", maxLength:50, tag:1, multiSelection:false});	
		...
		this.pc2 = new pageControl(this.pc1.childPage[0],{bound:[1,15,995,285], childPage:["Data Penyelesaian TCASH","Data Penerimaan TCASH"]});				
		this.sg4 = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:7,tag:1,
		            colTitle:["Status","No. BA","ID TCash","Nota","Tgl Nota","Nilai","Nilai Selisih"],
					colWidth:[[6,5,4,3,2,1,0],[100,100,100,100,150,100,80]],
		...
		try {
			...
			this.cb_kb.setSQL("select kode_akun, nama from masakun where kode_lokasi = '"+this.app._lokasi+"' ",["kode_akun","nama"],false,["Kode akun","Nama"],"and","Data Akun",true);
...
window.app_saku3_transaksi_tu_kantin_fRekonTcash.extend(window.childForm);
window.app_saku3_transaksi_tu_kantin_fRekonTcash.implement({
		...
		try {
			...
			for (var j=0;j < this.dataJU1.rs.rows.length;j++){
				for (var i=0; i < this.dataJU4.rs.rows.length;i++){
					if (this.dataJU4.rs.rows[i].id_bayar == this.dataJU1.rs.rows[j].id_tcash) {
						...
						this.sg4.cells(6,i,nilaiSelisih);
						totNS += nilaiSelisih;
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{
					...
					/*if(this.stsSimpan == 0){
						...
						sql.add("update ktu_nota_d set no_ref1 ='-' where no_ref1='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("update ktu_tcash_d set no_rekon ='-', nilai_selisih = 0 where no_rekon='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					*/							
					...
					for (var i=0;i < this.dataJU4.rs.rows.length;i++){
						...
						if (line.stsapp.toUpperCase()=="APP"){
							sql.add("update ktu_nota_bayar set no_ref1='"+this.e_nb.getText()+"' where no_bukti='"+line.no_bukti+"' and kode_bayar = 'TCASH' and kode_lokasi='"+this.app._lokasi+"'");
							
							sql.add("update ktu_tcash_d set no_rekon='"+this.e_nb.getText()+"', nilai_selisih = "+line.nilaiSelisih+" where id_tcash = '"+line.id_bayar+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							...
							nilaiToFloat(this.e_kas.getText())+",'Pembayaran Piutang TCASH','KB','KB','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");										
					...
					if (this.e_adm.getText() != "0") {
						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
								...
								nilaiToFloat(this.e_adm.getText())+",'Pembayaran Piutang TCASH','KB','ADM','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");															
					}
		...
		switch (event){
			...
			case "hapus" :
				...
				if (parseFloat(this.app._periode) > parseFloat(this.e_periode.getText())){
					...
					var sql = new server_util_arrayList();
					sql.add("delete from ktu_tcash_m where no_bukti_t= '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
					sql.add("update ktu_nota_bayar set no_ref1 ='-' where no_ref1='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("update ktu_tcash_d set no_rekon ='-', nilai_selisih = 0 where no_rekon='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from trans_m where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from trans_j where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					setTipeButton(tbAllFalse);	
	...
	doClick:function(sender){
		if (sender == this.i_gen) {
			...
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti",this.app._lokasi+"-STC"+this.e_periode.getText().substr(2,4)+".","0000"));
			this.stsSimpan=1;
			setTipeButton(tbSimpan);	
			this.e_ket.setFocus();
			setTipeButton(tbSimpan);
	...
	doCari:function(sender){	
		if (this.cb_kantin.getText()!="") {							
			this.pc1.setActivePage(this.pc1.childPage[0]);
			var strSQL = "select 'APP' as stsapp, a.no_bukti, a.id_bayar, bb.tanggal,a.nilai, bb.no_ba, c.akun_piutang, 0 as nilai_selisih "+
						"from  ktu_nota_bayar a "+
						"inner join ktu_nota_m bb on a.no_bukti = bb.no_bukti and a.kode_lokasi=bb.kode_lokasi "+
						"inner join ktu_kantin c on bb.kode_kantin = c.kode_kantin and bb.kode_lokasi=c.kode_lokasi and c.kode_kantin='"+this.cb_kantin.getText()+"' "+
						"where a.kode_bayar='TCASH' and a.no_ref1 ='-' and bb.no_ba <> '-' and a.kode_lokasi='"+this.app._lokasi+"'";	
		...
		else system.alert(this,"Kantin tidak valid.","Data kantin harus terisi.");
	},
	doTampilData4: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg4.appendData([line2.stsapp.toUpperCase(),line2.no_ba,line2.id_bayar,line2.no_bukti,line2.tanggal,floatToNilai(line2.nilai),line2.nilai_selisih]); 
		}
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {			
				...
				var strSQL = "select ket,kb,total from ktu_tcash_m where no_bukti_t ='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){						
				...
				var strSQL = "select 'APP' as stsapp, a.no_bukti, a.id_bayar, bb.tanggal,a.nilai, bb.no_ba, c.akun_piutang, 0 as nilai_selisih "+
						 	 "from  ktu_nota_bayar a "+
							 "inner join ktu_nota_m bb on a.no_bukti = bb.no_bukti and a.kode_lokasi=bb.kode_lokasi "+ 
							 "inner join ktu_kantin c on bb.kode_kantin = c.kode_kantin and bb.kode_lokasi=c.kode_lokasi "+
							 "inner join ktu_tcash_d d on a.no_ref1=d.no_rekon and a.id_bayar=d.id_tcash and a.kode_lokasi=d.kode_lokasi "+
							 "where a.no_ref1 ='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";	
