window.app_saku3_transaksi_tu_kantin_fKBtenan = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_kantin_fKBtenan.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_kantin_fKBtenan";
		...
		this.sg4 = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:8,tag:9,
		            colTitle:["Status","No. Rekon","Tanggal","Tenan","Nilai","Bank","Rekening","Pemilik"],
					colWidth:[[7,6,5,4,3,2,1,0],[100,200,200,100,300,80,100,80]],
...
window.app_saku3_transaksi_tu_kantin_fKBtenan.extend(window.childForm);
window.app_saku3_transaksi_tu_kantin_fKBtenan.implement({
				...
				try{									
					...
					*/
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank)  values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','-','-','"+this.cb_kb.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBSHARING','BK','"+this.e_periode.getText()+"','IDR',1,"+nilaiToFloat(this.e_tot.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','-','-','-')");
					
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank)  values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',999,'"+this.cb_kb.getText()+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_tot.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBSHARING','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");
					...
					for (var i=0;i < this.dataJU4.rs.rows.length;i++){
						...
						if (line.status.toUpperCase()=="APP"){
							...
							*/
							sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank)  values "+
									"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+line.akun_bymhd+"','Pembayaran Hutang : "+line.kode_tenan+"','D',"+parseFloat(line.hasil_tenan)+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBSHARING','BYMHD','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");
	...
	doCari:function(sender){								
		try {
			var strSQL = "select 'NON' as status, d.akun_bymhd, c.kode_tenan, a.no_rekon,convert(varchar,b.tanggal,103) as tanggal,c.kode_tenan+' | '+c.nama as tenan,a.hasil_tenan,c.bank+' | '+c.cabang as bank,c.norek+' | '+c.namarek as rek,c.pemilik "+
						 "from ktu_rekon_d a "+
	...
	doTampilData4: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg4.appendData([line2.status.toUpperCase(),line2.no_rekon,line2.tanggal,line2.tenan,floatToNilai(line2.hasil_tenan),line2.bank,line2.rek,line2.pemilik]);
		}
	...
	doLoad:function(sender){						
			var strSQL = "select no_kas as no_bukti, tanggal, keterangan, nilai as nilai1 from kas_m where modul = 'KBSHARING' and kode_lokasi='"+this.app._lokasi+"' and posted='F' and periode='"+this.e_periode.getText()+"' ";			
		var data = this.dbLib.getDataProvider(strSQL,true);		
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
