window.app_saku3_transaksi_tu_kantin_fKbTerimaTC = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_kantin_fKbTerimaTC.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_kantin_fKbTerimaTC";
		...
		this.e_total = new portalui_saiLabelEdit(this.pc1.childPage[0],{bound:[790,16,200,20],caption:"Total",tipeText:ttNilai,text:"0",readOnly: true});
		...
		try {
			...
			if (this.akun_titipan == "") alert("Kode Titipan TCASH Belum Ada");
		}catch(e){
			systemAPI.alert(e);
		}
	}
};
window.app_saku3_transaksi_tu_kantin_fKbTerimaTC.extend(window.childForm);
window.app_saku3_transaksi_tu_kantin_fKbTerimaTC.implement({
	...
	doNilaiChange: function(){
		try{	
			...
			for (var i = 0; i < this.sg4.rows.getLength();i++){
				if (this.sg4.rowValid(i) && this.sg4.cells(1,i) != "" ){
					totU += nilaiToFloat(this.sg4.cells(1,i));									
				}
			}
			this.e_total.setText(floatToNilai(totU));			
		}catch(e)		
		{
				...
				try{
					...
					*/
					...
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank)  "+
							"select '"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.cb_kb.getText()+"','Penerimaan Piutang TCASH','D',sum(a.nilai),'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBTCASH','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-' "+
							"from ktu_tcash_d a "+
							"where a.no_bukti='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'");
					
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank)  "+
							"select '"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',1,'"+this.akun_titipan+"','Penerimaan Piutang TCASH','C',sum(a.nilai),'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBTCASH','TTP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-' "+
							"from ktu_tcash_d a "+
							"where a.no_bukti='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank)  "+
							"select '"+this.e_nb.getText()+"','"+this.app._lokasi+"','-','-','"+this.cb_kb.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBTCASH','BM','"+this.e_periode.getText()+"','IDR',1,sum(a.nilai),'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','-','-','-' "+
							"from ktu_tcash_d a "+
							"where a.no_bukti='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'");
		...
		switch (event){
			...
			case "ubah" :
			    ...
			    if (nilaiToFloat(this.e_total.getText()) <= 0){
					system.alert(this,"Transaksi tidak valid.","Total  tidak boleh kurang dari atau sama dengan nol.");
					return false;
				}
	...
	doLoad:function(sender){						
		var strSQL = "select a.no_kas,a.keterangan from kas_m a "+
					 "where a.modul='KBTCASH' and a.kode_lokasi='"+this.app._lokasi+"' and a.posted='F' and a.periode='"+this.e_periode.getText()+"'";		
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.no_kas,line.keterangan]); 
		}
