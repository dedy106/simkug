window.app_saku3_transaksi_tu_fDosen = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_fDosen.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_fDosen";
		...
		this.e_nidn = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,22,250,20],caption:"NIDN", maxLength:20, tag:1});
		...
		this.e_inisial = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,21,200,20],caption:"Inisial", maxLength:10, tag:1});
		this.e_profesi = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,23,200,20],caption:"Profesi", maxLength:50, tag:1});
		this.cb_pp = new saiCBBL(this.pc1.childPage[1],{bound:[20,16,220,20],caption:"Prodi", multiSelection:false, maxLength:10, tag:2});													
...
window.app_saku3_transaksi_tu_fDosen.extend(window.childForm);
window.app_saku3_transaksi_tu_fDosen.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into tu_dosen(kode_dosen,kode_lokasi,nama,gelar,alamat,no_tel,email,npwp,bank,cabang,no_rek,nama_rek,inisial,nidn,kode_pp,profesi) values "+
						    "('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.e_gelar.getText()+"','"+this.e_alamat.getText()+"','"+this.e_tel.getText()+"','"+this.e_mail.getText()+"','"+this.e_npwp.getText()+"','"+this.e_bank.getText()+"','"+this.e_cabang.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"','"+this.e_inisial.getText()+"','"+this.e_nidn.getText()+"','"+this.cb_pp.getText()+"','"+this.e_profesi.getText()+"')");
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from tu_dosen where kode_dosen = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");			
					sql.add("insert into tu_dosen(kode_dosen,kode_lokasi,nama,gelar,alamat,no_tel,email,npwp,bank,cabang,no_rek,nama_rek,inisial,nidn,kode_pp,profesi) values "+
						    "('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.e_gelar.getText()+"','"+this.e_alamat.getText()+"','"+this.e_tel.getText()+"','"+this.e_mail.getText()+"','"+this.e_npwp.getText()+"','"+this.e_bank.getText()+"','"+this.e_cabang.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"','"+this.e_inisial.getText()+"','"+this.e_nidn.getText()+"','"+this.cb_pp.getText()+"','"+this.e_profesi.getText()+"')");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
			...
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){				
				...
				if (typeof data == "object"){
					...
					if (line != undefined){												
						...
						this.e_inisial.setText(line.inisial);
						this.e_nidn.setText(line.nidn);
						this.e_profesi.setText(line.profesi);
	...
	doCari:function(sender){								
		try {
			...
			var strSQL = "select kode_dosen,nama,alamat,kode_lokasi "+
						 "from tu_dosen where kode_lokasi='"+this.app._lokasi+"' and "+filter+" order by kode_dosen";								
			var data = this.dbLib.getDataProvider(strSQL,true);		
	...
	doLoad:function(sender){								
		try {
			var strSQL = "select kode_dosen,nama,alamat "+
						 "from tu_dosen where kode_lokasi='"+this.app._lokasi+"' order by kode_dosen";				
			var data = this.dbLib.getDataProvider(strSQL,true);		
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU = data;
				this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length/20));
