window.app_saku3_transaksi_tu_aka_fRekonPDDinmaba = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_aka_fRekonPDDinmaba.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_aka_fRekonPDDinmaba";
...
window.app_saku3_transaksi_tu_aka_fRekonPDDinmaba.extend(window.childForm);
window.app_saku3_transaksi_tu_aka_fRekonPDDinmaba.implement({
	...
	doValid: function(sender){						
		var strSQL = "select no_tes as nim from aka_maba where kode_lokasi='"+this.app._lokasi+"'";					
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','AR','CDINMABA','Z','-','-','-','"+this.dp_d1.getDateString()+"','"+this.cb_bukti.getText()+"','"+this.cb_akun.rightLabelCaption+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",0,0,'"+this.app._userLog+"','"+this.app._userLog+"','-','"+this.cb_akun.getText()+"','-','"+this.c_periode.getText()+"','-','-','CDIN')"); 
					...
					for (var i=0; i < this.sg1.getRowCount();i++){
						sql.add("INSERT INTO aka_cd_d (no_bukti, kode_lokasi, periode, nim, kode_akun, dc, nilai, ref1, bank, no_rek, nama_rek, tahunaka, nilai_ref, keterangan,bank_ref) VALUES "+
								"('"+this.e_nb.getText()+"', '"+this.app._lokasi+"', '"+this.e_periode.getText()+"', '"+this.sg1.cells(0,i)+"', '2141104', 'D', "+nilaiToFloat(this.sg1.cells(1,i))+", '-', '-', '-', '-', '-', 0, 'CDINMABA','"+this.c_bank.getText()+"')");
					}
	...
	doChange:function(sender){						
		...
		if (sender == this.c_periode && this.c_periode.getText()!="") {
			var strSQL = "select a.no_bukti, CONVERT (varchar, CAST (a.nilai - isnull(b.pakai,0) AS money),1) as sisa from "+
						"	( "+
						"			select no_bukti,sum(nilai) as nilai from gldt where (periode between '202112' and '"+this.c_periode.getText()+"') and kode_lokasi='"+this.app._lokasi+"' and kode_akun = '2141104' and dc='C' and modul not in ('CDMUTMULTI')  group by no_bukti "+
						"			union all "+
						"			select no_bukti,sum(nilai) as nilai from gldt_h where(periode between '202112' and '"+this.c_periode.getText()+"') and kode_lokasi='"+this.app._lokasi+"' and kode_akun = '2141104' and dc='C' and modul not in ('CDMUTMULTI')  group by no_bukti "+
						"			) a "+
						"			left join ( "+
						"			select no_dokumen,sum(nilai1) as pakai from trans_m where kode_lokasi ='"+this.app._lokasi+"' and form = 'CDINMABA' "+
						"			group by no_dokumen "+
		...
		if (sender == this.cb_bukti && this.cb_bukti.getText()!="") {
			var strSQL = "select a.no_urut,a.keterangan, CONVERT (varchar, CAST (a.nilai-isnull(b.pakai,0) AS money),1) as sisa from "+
						"( "+
						"			select no_urut,keterangan,nilai from gldt where no_bukti='"+this.cb_bukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and kode_akun = '2141104' and dc='C' and modul not in ('CDMUTMULTI')  "+
						"			union all "+
						"			select no_urut,keterangan,nilai from gldt_h where no_bukti='"+this.cb_bukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and kode_akun = '2141104' and dc='C' and modul not in ('CDMUTMULTI')  "+
						"			) a "+
						"			left join ( "+
						"			select no_ref1,sum(nilai1) as pakai from trans_m where no_dokumen='"+this.cb_bukti.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' and form = 'CDINMABA' "+
						"			group by no_ref1 "+
		...
		if (sender == this.cb_akun && this.cb_akun.getText() != "") {
			if (this.c_periode.getText() == this.app._periode) {
				var strSQL = "select nilai from gldt "+
						 	 "where kode_akun = '2141104' and no_urut ='"+this.cb_akun.getText()+"' and dc ='C' and no_bukti = '"+this.cb_bukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						
			}
			else {
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan "+
		             "from trans_m a "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.form = 'CDINMABA' and a.posted ='Z'";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
