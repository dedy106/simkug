window.app_saku3_transaksi_tu_aka_fCDmutMala = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_aka_fCDmutMala.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_aka_fCDmutMala";
		this.itemsValue = new arrayList();
		this.maximize();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pemindahan CD NIM ke NIM", 0);	
		...
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:5,
					colTitle:["NIM","Saldo PDD","Nilai Mutasi","NIM Penerima","Ver Nilai"],
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('KBTTP') and kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;			
				for (var i in data.rs.rows){
					line = data.rs.rows[i];													
					if (line.kode_spro == "KBTTP") this.akunMala = line.flag;					
				}
			}
...
window.app_saku3_transaksi_tu_aka_fCDmutMala.extend(window.childForm);
window.app_saku3_transaksi_tu_aka_fCDmutMala.implement({
	...
	doValid: function(sender){				
		var strSQL = "select a.nim,isnull(b.saldo_cd,0) as saldo_cd "+
		...
		for (var i=0; i < this.sg1.getRowCount();i++){
			...
			for (var j=0;j < this.dataReg.rs.rows.length;j++){
				if (this.sg1.cells(0,i).substr(8,30) == this.dataReg.rs.rows[j].nim) {					
		...
		if (this.inValid) {
			...
			for (var i=0; i < this.sg1.getRowCount();i++) {
				...
				if (this.sg1.cells(3,i).substr(0,7) == "INVALID") {
					var j = i+1;
					this.sg2.appendData([j,"NIMTERIMA"]);						
				}
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,due_date,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','MI','CDMUTMALA','Z','-','-','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','"+this.dp_d1.getDateString()+"', '-','"+this.e_ket.getText()+"','IDR',1,"+
									parseNilai(this.e_nilai.getText())+",0,0,'-','-','-','-','-','-','-','-','-')");

					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.akunMala+"','D',"+parseNilai(this.e_nilai.getText())+","+
									parseNilai(this.e_nilai.getText())+",'"+this.e_ket.getText()+"','CDMUTMALA','TTP','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");											
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',1,'"+this.akunMala+"','C',"+parseNilai(this.e_nilai.getText())+","+
									parseNilai(this.e_nilai.getText())+",'"+this.e_ket.getText()+"','CDMUTMALA','CD','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");											
					...
					for (var i=0; i < this.sg1.getRowCount();i++){
						sql.add("INSERT INTO aka_cd_d (no_bukti, kode_lokasi, periode, nim, kode_akun, dc, nilai, ref1, bank, no_rek, nama_rek, tahunaka, nilai_ref, keterangan) VALUES "+
								"('"+this.e_nb.getText()+"', '"+this.app._lokasi+"', '"+this.e_periode.getText()+"', '"+this.sg1.cells(0,i)+"', '"+this.akunMala+"', 'C', "+nilaiToFloat(this.sg1.cells(2,i))+", '"+this.sg1.cells(3,i)+"', '-', '-', '-', '-', 0, 'CDMUTMALA')");
						sql.add("INSERT INTO aka_cd_d (no_bukti, kode_lokasi, periode, nim, kode_akun, dc, nilai, ref1, bank, no_rek, nama_rek, tahunaka, nilai_ref, keterangan) VALUES "+
								"('"+this.e_nb.getText()+"', '"+this.app._lokasi+"', '"+this.e_periode.getText()+"', '"+this.sg1.cells(3,i)+"', '"+this.akunMala+"', 'D', "+nilaiToFloat(this.sg1.cells(2,i))+", '"+this.sg1.cells(0,i)+"', '-', '-', '-', '-', 0, 'CDMUTMALA')");
					}
	...
	doClick:function(sender){
		if (sender == this.i_gen) {		
			...
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti",this.app._lokasi+"-CML"+this.e_periode.getText().substr(2,4)+".","0000"));
			this.e_ket.setFocus();
			this.stsSimpan = 1;
		}
	},		
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai1 "+
		             "from trans_m a "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.form = 'CDMUTMALA' and a.posted ='Z'";
