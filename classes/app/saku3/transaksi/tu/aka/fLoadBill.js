window.app_saku3_transaksi_tu_aka_fLoadBill = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_aka_fLoadBill.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_tu_aka_fLoadBill";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Upload Data Tagihan ", 0);	
		...
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:13,tag:9,
				colTitle:["NIM","Thn Akademik","UP3","SDP2","BPP Pkt","BPP Non","SKS","Perpus","Denda","U Status","Asuransi","Asrama","Total"],
				colFormat:[[12,11,10,9,8,7,6,5,4,3,2],[cfNilai,cfNilai,cfNilai,cfNilai,cfNilai,cfNilai,cfNilai,cfNilai,cfNilai,cfNilai,cfNilai]],
...
window.app_saku3_transaksi_tu_aka_fLoadBill.extend(window.portalui_childForm);
window.app_saku3_transaksi_tu_aka_fLoadBill.implement({	
	doAfterPaste: function(sender,totalRow){
		try {
			...
			for (var i=0;i < this.sg1.getRowCount();i++){
				tagih = nilaiToFloat(this.sg1.cells(2,i)) + nilaiToFloat(this.sg1.cells(3,i)) + nilaiToFloat(this.sg1.cells(4,i)) + nilaiToFloat(this.sg1.cells(5,i)) + nilaiToFloat(this.sg1.cells(6,i)) + nilaiToFloat(this.sg1.cells(7,i)) + nilaiToFloat(this.sg1.cells(8,i)) + nilaiToFloat(this.sg1.cells(9,i)) + nilaiToFloat(this.sg1.cells(10,i)) + nilaiToFloat(this.sg1.cells(11,i)) ;
				this.sg1.cells(12,i,floatToNilai(tagih));			
				tot += tagih;
			}
			...
			for (var i=0;i < this.sg1.getRowCount();i++){
				...
				if (nilaiToFloat(this.sg1.cells(11,i)) != 0) {
					sql.add("insert into aka_bill_tmp (no_bill,kode_lokasi,no_inv,nim,periode,kode_produk,akun_piutang,akun_pdpt,akun_pdd,nilai,periode_susut,periode_sisih,kode_pp,dc,kode_akt,tahunaka) values "+
							"('"+this.nbTmp+"','"+this.app._lokasi+"','-','"+this.sg1.cells(0,i)+"','"+this.e_periode.getText()+"','ASRAMA','-','-','-',"+nilaiToFloat(this.sg1.cells(11,i))+",'"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','-','D','-','"+this.sg1.cells(1,i)+"')");
				}															
			}
	...
	doValid: function(sender){
		...
		//cek nim terdaftar
		var strSQL = "select distinct a.nim as nim from aka_bill_tmp a left join aka_mahasiswa b on a.nim=b.nim and a.kode_lokasi=b.kode_lokasi where a.no_bill='"+this.nbTmp+"' and a.kode_lokasi ='"+this.app._lokasi+"' and b.nim is null";		
		...
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			...
			for (var i in data.rs.rows){
				temu = true; 
				line = data.rs.rows[i];							
				this.sg2.appendData([line.nim,"NIM","NIM tidak terdaftar"]);
			}
		}
