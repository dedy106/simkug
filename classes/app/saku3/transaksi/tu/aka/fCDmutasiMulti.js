window.app_saku3_transaksi_tu_aka_fCDmutasiMulti = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_aka_fCDmutasiMulti.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_aka_fCDmutasiMulti";
		this.itemsValue = new arrayList();
		this.maximize();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pemindahan CD IDTes ke NIM", 0);	
		...
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:5,
					colTitle:["IDTes","Saldo PDD","Nilai Mutasi","NIM Penerima","Ver Nilai"],
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('AKATTP','KBTTP') and kode_lokasi='"+this.app._lokasi+"'",true);
...
window.app_saku3_transaksi_tu_aka_fCDmutasiMulti.extend(window.childForm);
window.app_saku3_transaksi_tu_aka_fCDmutasiMulti.implement({
				...
				try{													
					...
					for (var i=0; i < this.sg1.getRowCount();i++){
						sql.add("INSERT INTO aka_cd_d (no_bukti, kode_lokasi, periode, nim, kode_akun, dc, nilai, ref1, bank, no_rek, nama_rek, tahunaka, nilai_ref, keterangan) VALUES "+
								"('"+this.e_nb.getText()+"', '"+this.app._lokasi+"', '"+this.e_periode.getText()+"', '"+this.sg1.cells(0,i)+"', '"+this.akunMaba+"', 'C', "+nilaiToFloat(this.sg1.cells(2,i))+", '"+this.sg1.cells(3,i)+"', '-', '-', '-', '-', 0, 'CDMUTMULTI')");
						sql.add("INSERT INTO aka_cd_d (no_bukti, kode_lokasi, periode, nim, kode_akun, dc, nilai, ref1, bank, no_rek, nama_rek, tahunaka, nilai_ref, keterangan) VALUES "+
								"('"+this.e_nb.getText()+"', '"+this.app._lokasi+"', '"+this.e_periode.getText()+"', '"+this.sg1.cells(3,i)+"', '"+this.akunMala+"', 'D', "+nilaiToFloat(this.sg1.cells(2,i))+", '"+this.sg1.cells(0,i)+"', '-', '-', '-', '-', 0, 'CDMUTMULTI')");
					}
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {
								this.nama_report="server_report_saku3_siswa_rptSisDetSaldoCd";
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai1 "+
		             "from trans_m a "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.form = 'CDMUTMULTI' and a.posted ='F'";
