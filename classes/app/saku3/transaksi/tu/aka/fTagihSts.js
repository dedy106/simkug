window.app_saku3_transaksi_tu_aka_fTagihSts = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_aka_fTagihSts.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_tu_aka_fTagihSts";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Update Status Tertagih", 0);	
...
window.app_saku3_transaksi_tu_aka_fTagihSts.extend(window.portalui_childForm);
window.app_saku3_transaksi_tu_aka_fTagihSts.implement({	
	doAfterPaste: function(sender,totalRow){
		try {
			var stsValid = true;
			for (var i=0;i < this.sg1.getRowCount();i++){									
				this.sg1.cells(1,i,this.sg1.cells(1,i).toUpperCase());				
				if (this.sg1.cells(1,i) != "TERTAGIH" && this.sg1.cells(1,i) != "TIDAK") {
					stsValid = false;
			...
			case "simpan" :					
					if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
						try{
							...
							var id  = this.standarLib.noBuktiOtomatis(this.dbLib,"aka_mhsstatus_m","no_bukti",this.app._lokasi+"-BST"+this.e_periode.getText().substr(2,4)+".","0000");
							...
							sql.add("insert into aka_mhsstatus_m (no_bukti,kode_lokasi,tanggal,periode,tahunaka,keterangan,tgl_input,nik_user,jenis) values "+
									"('"+id+"','"+this.app._lokasi+"', '"+this.dp_d1.getDateString()+"','"+this.e_periode.getText()+"','"+this.c_tahunaka.getText()+"','"+this.e_ket.getText()+"',getdate(),'"+this.app._userLog+"','BILL-STS')");

							for (var i=0;i < this.sg1.getRowCount();i++){																	
								sql.add("update aka_bill_d set sts_tagih = '"+this.sg1.cells(1,i)+"' where nim = '"+this.sg1.cells(0,i)+"' and kode_lokasi='"+this.app._lokasi+"' and tahunaka='"+this.c_tahunaka.getText()+"'");
