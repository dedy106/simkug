window.app_saku3_transaksi_tu_proyekbaru_fRekonBill = function(owner)
{
	if (owner)
	{
		...
		this.e_nsaldo = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,19,200,20],caption:"Saldo OR", tag:1, tipeText:ttNilai, text:"0",readOnly:true});				
		this.e_saldobmhd = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,15,200,20],caption:"Saldo BMHD", tag:1, tipeText:ttNilai, text:"0",readOnly:true});				
		this.cb_beban = new saiCBBL(this.pc1.childPage[1],{bound:[20,12,220,20],caption:"Akun Beban/BMHD",readOnly:true, maxLength:10, tag:1});	
		...
		try {
			...
			this.cb_beban.setSQL("select a.kode_akun, a.nama from masakun a where a.kode_lokasi='"+this.app._lokasi+"'",["kode_akun","nama"],false,["Kode","Nama"],"and","Data Akun",true);
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into prb_prbill_bayar (no_rekon,kode_lokasi,periode,kode_pp,modul,no_bill,kode_cust,dc,nilai,nilai_pph,no_jurnal,kode_akun,no_tak) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.cb_pp.getText()+"','REKON','"+this.cb_bill.getText()+"','"+this.cb_cust.getText()+"','D',"+nilaiToFloat(this.e_nilai.getText())+","+nilaiToFloat(this.e_beban.getText())+",'"+this.cb_bukti.getText()+"','"+this.cb_akun.getText()+"','-')");
					...
					var totPiu = nilaiToFloat(this.e_nilai.getText()) + nilaiToFloat(this.e_beban.getText());
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
					...
					if (nilaiToFloat(this.e_beban.getText()) != 0) {		
						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
								...
								parseNilai(this.e_beban.getText())+",'"+this.e_ket.getText()+"','PIUPRO','BEBAN','IDR',1,'"+this.app._kodePP+"','-','"+this.cb_cust.getText()+"','-','-','-','-','-','-')");
						...
						if (nilaiToFloat(this.e_saldobmhd.getText()) == 0)	{
							sql.add("insert into prb_prbeban_d(no_bukti,kode_lokasi,periode,periode_sch,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_beban.getText()+"','"+this.cb_pp.getText()+"','-','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_beban.getText())+",getdate(),'"+this.cb_proyek.getText()+"','AJUBEBAN','-')"); //REKBILL --rudi 24/8/20
							...
							sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,user_input,form,sts_pajak,npajak,no_ref1,dasar) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','BILREKON','X','"+this.cb_pp.getText()+"','-','-',"+nilaiToFloat(this.e_beban.getText())+",getdate(),'"+this.app._userLog+"','Z','Z','"+this.e_nb.getText()+"','Z','-','-','"+this.app._userLog+"','BILREKON','Z',0,'Z','Z')");						
							sql.add("insert into kas_m(no_kas, kode_lokasi, no_dokumen, no_bg, akun_kb, tanggal, keterangan, kode_pp, modul, jenis, periode, kode_curr, kurs, nilai, nik_buat, tgl_input, nik_user, nik_app, posted, no_del, no_link, ref1, kode_bank) values "+
									"('"+this.e_nb.getText()+"', '"+this.app._lokasi+"', '-', '-', '-', '"+this.dp_d1.getDateString()+"', '-', '"+this.cb_pp.getText()+"', 'BILREKON', 'BK', '"+this.e_periode.getText()+"', 'IDR', 1, 0, '"+this.app._userLog+"', getdate(), '"+this.app._userLog+"', '"+this.app._userLog+"', 'X', '-', '-', '-', '-')");
						...
						if (nilaiToFloat(this.e_saldobmhd.getText()) != 0)	{
							sql.add("insert into prb_bmhd_d(no_bukti,kode_lokasi,periode,tanggal,kode_akun,kode_pp,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_beban.getText()+"','"+this.cb_pp.getText()+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_beban.getText())+",getdate(),'"+this.cb_proyek.getText()+"','REKBILL','-')");	
						}
					}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :														
				...
				if ((nilaiToFloat(this.e_nilai.getText()) + nilaiToFloat(this.e_beban.getText())) > nilaiToFloat(this.e_sisa.getText()) ) {
	...
	doChange:function(sender){						
		...
		if (sender == this.cb_proyek && this.cb_proyek.getText()!="") {			
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				if (nilaiToFloat(this.e_saldobmhd.getText()) == 0) this.cb_beban.setText(line.akun_beban);															
				else this.cb_beban.setText(line.akun_bmhd);
			}			
		}
	},
