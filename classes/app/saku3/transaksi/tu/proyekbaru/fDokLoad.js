window.app_saku3_transaksi_tu_proyekbaru_fDokLoad = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyekbaru_fDokLoad.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyekbaru_fDokLoad";
		...
		this.sg1mp2 = new saiGrid(this.pc1.childPage[0],{bound:[1,255,this.pc1.width-5,165],colCount:5,readOnly:true,tag:9,
...
window.app_saku3_transaksi_tu_proyekbaru_fDokLoad.extend(window.childForm);
window.app_saku3_transaksi_tu_proyekbaru_fDokLoad.implement({
					...
					for (var i=0;i < this.sg1mp2.getRowCount();i++){
						if (this.sg1mp2.rowValid(i)){
							if (this.sg1mp2.cells(4,i) == "HAPUS") {
								sql.add("delete from prb_rab_dok where no_rab='"+this.cb_rab.getText()+"' and no_gambar='"+this.sg1mp2.cells(2,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
							}
						}	
					}
