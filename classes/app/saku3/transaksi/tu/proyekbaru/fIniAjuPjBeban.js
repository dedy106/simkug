...
*/
window.app_saku3_transaksi_tu_proyekbaru_fIniAjuPjBeban = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyekbaru_fIniAjuPjBeban.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyekbaru_fIniAjuPjBeban";
...
window.app_saku3_transaksi_tu_proyekbaru_fIniAjuPjBeban.extend(window.childForm);
window.app_saku3_transaksi_tu_proyekbaru_fIniAjuPjBeban.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,user_input,form,sts_pajak,npajak) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.c_jenis.getText()+"','"+this.akunPJ+"','"+this.cb_pp.getText()+"','-','"+this.cb_id.getText()+"|"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",getdate(),'"+this.app._userLog+"','-','-','-','A','"+this.cb_nik.getText()+"','-','"+this.e_user.getText()+"','PJINISIASI','NON',0)");					
	...
	doChange:function(sender){
		...
		if (sender==this.cb_pp && this.cb_pp.getText()!="" && this.stsSimpan == 1) {			
			this.cb_id.setText("","");
			this.cb_id.setSQL("select kode_proyek, nama from prb_proyek where versi='PRO20' and modul='INISIASI' and kode_pp='"+this.cb_pp.getText()+"' and flag_aktif='1' and kode_lokasi='"+this.app._lokasi+"'",["kode_proyek","nama"],false,["ID Kegiatan","Keterangan"],"and","Data Inisiasi",true);						
		...
		if (sender==this.cb_id && this.cb_id.getText()!="") {
			...
			var strSQL = "select a.nama,a.nilai_or - isnull(c.beban,0) as saldo, convert(varchar,a.tgl_admin,103) as tgl_admin  "+
			             "from prb_proyek a "+			             
	...
	doLoad3:function(sender){		
		var strSQL = "select a.no_aju,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
		             ...
		             "where a.form='PJINISIASI' and a.modul='PANJAR' and a.progress in ('0','A','R') and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and b.nik='"+this.app._userLog+"'";	
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
