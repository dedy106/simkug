window.app_saku3_transaksi_tu_proyekbaru_fPrBebanShare = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyekbaru_fPrBebanShare.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyekbaru_fPrBebanShare";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Sharing Beban Proyek", 0);	
        ...
        this.e_nilaipro = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,16,200,20],caption:"Total Proyek", readOnly:true,tag:1, tipeText:ttNilai, text:"0"});				
		this.e_saldo = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,17,200,20],caption:"Saldo Beban", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		...
		try {
			...
			this.cb_akun.setSQL("select kode_akun, nama from masakun where kode_lokasi='"+this.app._lokasi+"' and jenis='Beban' ",["kode_akun","nama"],false,["Kode Akun","Nama"],"and","Data Akun",true);													
...
window.app_saku3_transaksi_tu_proyekbaru_fPrBebanShare.extend(window.childForm);
window.app_saku3_transaksi_tu_proyekbaru_fPrBebanShare.implement({
	...
	simpan: function(){			
		try{						
			if(this.stsSimpan == 1) this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti",this.app._lokasi+"-BBS"+this.e_periode.getText().substr(2,4)+".","0000"));
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if(this.stsSimpan == 0){
                        ...
                        sql.add("delete from prb_beban_share where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}								
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
                            "('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','AR','BBNSHARE','F','-','-','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','-','"+this.e_ket.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",0,0,'"+this.app._userLog+"','"+this.cb_app.getText()+"','-','-','"+this.cb_proyek.getText()+"','-','-','-','-')");
                    
                    sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.akunBeban+"','D',"+parseNilai(this.e_nilai.getText())+","+
                            parseNilai(this.e_nilai.getText())+",'"+this.e_ket.getText()+"','AR','BEBAN','IDR',1,'"+this.cb_pp.getText()+"','"+this.drkB+"','-','-','-','-','-','-','-')");	
                    ...
                    sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
                            ...
                            parseNilai(this.e_nilai.getText())+",'"+this.e_ket.getText()+"','AR','BEBAN','IDR',1,'"+this.cb_pp2.getText()+"','"+this.cb_drk.getText()+"','-','-','-','-','-','-','-')");	
                            
                    sql.add("insert into prb_beban_share (no_bukti,kode_lokasi,tgl_input,nik_user,periode,kode_pp,tanggal,keterangan,nilai_share,id_proyek,pp_share,akun_beban,kode_drkb) values "+
	...
	doClick:function(sender){
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti",this.app._lokasi+"-BBS"+this.e_periode.getText().substr(2,4)+".","0000"));
		this.e_ket.setFocus();
	},
	doChange:function(sender){
		if (sender == this.cb_pp && this.cb_pp.getText()!="") {
			this.cb_proyek.setSQL("select a.kode_proyek,a.nama from prb_proyek a where a.versi='PRO20' and kode_pp ='"+this.cb_pp.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress='2' ",["a.kode_proyek","a.nama"],false,["ID Proyek","Keterangan"],"and","Data Proyek",true);								
		}
		if (sender == this.cb_proyek && this.cb_proyek.getText()!="") {
			var data = this.dbLib.getDataProvider("select a.nilai, isnull(b.saldo_beban,0)-isnull(c.nilai_s,0) as saldo ,a.kode_jenis "+
			"from prb_proyek a "+
			
            "left join (select kode_proyek,kode_lokasi,sum( case dc when 'D' then nilai else -nilai end) as saldo_beban "+
            "           from prb_prbeban_d where modul = 'AJUBEBAN' "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){
                    ...
                    var data2 = this.dbLib.getDataProvider("select kode_jenis,akun_beban, kode_drkb from prb_proyek_jenis "+
                    "where kode_jenis='"+line.kode_jenis+"' and kode_lokasi='"+this.app._lokasi+"'",true);
                    if (typeof data2 == "object"){
                        var line2 = data2.rs.rows[0];	
                        
                        if (line2 != undefined){
                            this.cb_akun.setText(line2.akun_beban);
                            this.cb_drk.setText(line2.kode_drkb);	
                            this.akunBeban = line2.akun_beban;
                            this.drkB = line2.kode_drkb;
                        }
                    }
				} 
            }  
            ...
            if(this.stsSimpan == 0){
                var data3 = this.dbLib.getDataProvider("select akun_beban, kode_drkb from prb_beban_share "+
                ...
                if (typeof data3 == "object"){
                    ...
                    if (line3 != undefined){
                        this.cb_akun.setText(line3.akun_beban);
                        this.cb_drk.setText(line3.kode_drkb);	
                    }
                }
            }
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){	
							if(this.preView == "1"){
								this.nama_report="server_report_saku3_tu_proyek_rptBebanShareJurnal";
	...
	doLoad3:function(sender){																				
		var strSQL = "select no_bukti, convert(varchar,tanggal,103) as tgl, keterangan, id_proyek,nilai_share from prb_beban_share where kode_lokasi='"+this.app._lokasi+"' and nik_user='"+this.app._userLog+"' ";						
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
                ...
                var data = this.dbLib.getDataProvider("select a.keterangan,a.tanggal, a.kode_pp, a.id_proyek, a.nilai_share, a.pp_share, a.akun_beban, a.kode_drkb,b.nik2 "+
                       "from prb_beban_share a inner join trans_m b on a.no_bukti=b.no_bukti and a.kode_lokasi=b.kode_lokasi  "+
