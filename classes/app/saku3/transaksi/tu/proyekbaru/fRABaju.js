window.app_saku3_transaksi_tu_proyekbaru_fRABaju = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyekbaru_fRABaju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyekbaru_fRABaju";
		...
		this.cb_pp = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,13,220,20],caption:"Bagian / Unit",tag:2, readOnly:true, change:[this,"doChange"], multiSelection:false}); 					
		...
		this.e_norek = new saiLabelEdit(this.pc1.childPage[0],{bound:[520,15,400,20],caption:"No Rekening", maxLength:50, tag:1, readOnly:true,text:"-"});			
		this.cb_ppkelola = new portalui_saiCBBL(this.pc1.childPage[0],{bound:[20,13,220,20],caption:"Unit Pengelola",tag:1,readOnly:true}); 					
		this.e_nomemo = new saiLabelEdit(this.pc1.childPage[0],{bound:[520,13,400,20],caption:"No Memo", maxLength:50, tag:1});					
...
window.app_saku3_transaksi_tu_proyekbaru_fRABaju.extend(window.childForm);
window.app_saku3_transaksi_tu_proyekbaru_fRABaju.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into prb_rab_m(no_rab,keterangan,kode_lokasi,kode_pp,kode_cust,tgl_mulai,tgl_selesai,nilai,nilai_or,p_or,kode_jenis,nik_app,progress,pp_kelola,periode,no_dok,tanggal,nik_buat,cat_app_proyek, no_memo,ppn,pph42, sts_va,bank,nama_rek,no_rek,tgl_admin,versi) values "+
							...
							"','-','"+this.e_nomemo.getText()+"',"+nilaiToFloat(this.e_ppn.getText())+","+nilaiToFloat(this.e_pph4.getText())+", '"+this.c_stsva.getText()+"','"+this.e_bank.getText()+"','"+this.e_namarek.getText()+"','"+this.e_norek.getText()+"','"+this.dp_d4.getDateString()+"','PRO20')"); 
	...
	doLoad3:function(sender){																				
		var strSQL =  "select a.no_rab,a.keterangan,b.nama as cust,a.nilai,case a.progress when '0' then 'inprogress' when 'R' then 'return ver' when 'K' then 'return proyek' when 'N' then 'return aktifasi' end as status "+
					  ...
					  "where a.kode_lokasi='"+this.app._lokasi+"' and a.progress in ('0','R','N','K') and a.versi='PRO20' ";
		var data = this.dbLib.getDataProvider(strSQL,true);
