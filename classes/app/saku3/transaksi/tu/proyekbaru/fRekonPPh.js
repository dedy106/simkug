window.app_saku3_transaksi_tu_proyekbaru_fRekonPPh = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyekbaru_fRekonPPh.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyekbaru_fRekonPPh";
		...
		this.e_saldobmhd = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,15,200,20],caption:"Saldo BMHD", tag:1, tipeText:ttNilai, text:"0",readOnly:true});				
		this.cb_pp = new saiCBBL(this.pc1.childPage[0],{bound:[20,13,220,20],caption:"PP / Unit", readOnly:true, tag:1});						
		this.cb_pph = new saiCBBL(this.pc1.childPage[0],{bound:[20,12,220,20],caption:"Akun Beban/BMHD",readOnly:true, maxLength:10, tag:1});	
		...
		try {
			...
			this.cb_proyek.setSQL("select kode_proyek, nama from prb_proyek where versi='PRO20' and progress='2' and kode_lokasi='"+this.app._lokasi+"'",["kode_proyek","nama"],false,["Kode","Nama"],"and","Data Proyek",true);			

			this.c_stsBP.setText("BP");
		}catch(e){
			systemAPI.alert(e);
		}
	}
};
window.app_saku3_transaksi_tu_proyekbaru_fRekonPPh.extend(window.childForm);
window.app_saku3_transaksi_tu_proyekbaru_fRekonPPh.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','AR','PPHREKON','F','-','-','-','"+this.dp_d1.getDateString()+"','-','"+this.e_ket.getText()+"','IDR',1,"+parseNilai(this.e_pph.getText())+",0,0,'"+this.app._userLog+"','"+this.app._userLog+"','-','-','"+this.cb_cust.getText()+"','-','-','"+this.c_stsBP.getText()+"','PPH')"); 
					...
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							...
							parseNilai(this.e_pph.getText())+",'"+this.e_ket.getText()+"','PPHREKON','PPH','IDR',1,'"+this.app._kodePP+"','-','"+this.cb_cust.getText()+"','-','-','-','-','-','-')");			
					...
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							...
							parseNilai(this.e_pph.getText())+",'Pelunasan atas Proyek "+this.cb_proyek.getText()+"','PPHREKON','PIU','IDR',1,'"+this.app._kodePP+"','-','"+this.cb_cust.getText()+"','-','-','-','-','-','-')");
				
					//kalo pakai saldo or masih ada
					if (nilaiToFloat(this.e_saldobmhd.getText()) == 0)	{
						sql.add("insert into prb_prbeban_d(no_bukti,kode_lokasi,periode,periode_sch,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_pph.getText()+"','"+this.cb_pp.getText()+"','-','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_pph.getText())+",getdate(),'"+this.cb_proyek.getText()+"','AJUBEBAN','-')");

						//injek supaya ketarik di data schedule		
						sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,user_input,form,sts_pajak,npajak,no_ref1,dasar) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','PPHREKON','X','"+this.cb_pp.getText()+"','-','-',"+nilaiToFloat(this.e_pph.getText())+",getdate(),'"+this.app._userLog+"','Z','Z','"+this.e_nb.getText()+"','Z','-','-','"+this.app._userLog+"','PPHREKON','Z',0,'Z','Z')");						
						sql.add("insert into kas_m(no_kas, kode_lokasi, no_dokumen, no_bg, akun_kb, tanggal, keterangan, kode_pp, modul, jenis, periode, kode_curr, kurs, nilai, nik_buat, tgl_input, nik_user, nik_app, posted, no_del, no_link, ref1, kode_bank) values "+
								"('"+this.e_nb.getText()+"', '"+this.app._lokasi+"', '-', '-', '-', '"+this.dp_d1.getDateString()+"', '-', '"+this.cb_pp.getText()+"', 'PPHREKON', 'BK', '"+this.e_periode.getText()+"', 'IDR', 1, 0, '"+this.app._userLog+"', getdate(), '"+this.app._userLog+"', '"+this.app._userLog+"', 'X', '-', '-', '-', '-')");		
					}
					//kalo pakai saldo bmhd
					if (nilaiToFloat(this.e_saldobmhd.getText()) != 0)	{
						sql.add("insert into prb_bmhd_d(no_bukti,kode_lokasi,periode,tanggal,kode_akun,kode_pp,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_pph.getText()+"','"+this.cb_pp.getText()+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_pph.getText())+",getdate(),'"+this.cb_proyek.getText()+"','AJUBMHD','-')");	
					}
					setTipeButton(tbAllFalse);					
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :														
				...
				if (nilaiToFloat(this.e_saldobmhd.getText()) == 0) {
					if (nilaiToFloat(this.e_pph.getText()) > nilaiToFloat(this.e_nsaldo.getText()) ) {
						system.alert(this,"Transaksi tidak valid.","Nilai PPh tidak boleh melebihi Saldo OR");
						return false;						
					}
				}
				else {
					if (nilaiToFloat(this.e_pph.getText()) > nilaiToFloat(this.e_saldobmhd.getText()) ) {
						system.alert(this,"Transaksi tidak valid.","Nilai PPh tidak boleh melebihi Saldo BMHD");
						return false;						
					}
				}
	...
	doChange:function(sender){				
		...
		if ((sender == this.cb_bill || sender == this.c_stsBP) && this.cb_bill.getText()!="" && this.c_stsBP.getText()!="" && this.stsSimpan==1 ) {			
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				this.e_pph.setText(floatToNilai(line.saldo));								
			}		
		}	
		
		if (sender == this.cb_proyek && this.cb_proyek.getText()!="") {
			var strSQL =  "select a.kode_cust, b.akun_beban,b.akun_bmhd,(a.nilai_or  - isnull(c.beban,0)) as saldo_or,isnull(d.bmhd,0) as saldo_bmhd "+
			              "from prb_proyek a "+			             
						  ...
						  "      from prb_bmhd_d a where kode_lokasi='"+this.app._lokasi+"' and "+
						  "		no_bukti <> '"+this.e_nb.getText()+"'"+
						  "      group by kode_proyek,kode_lokasi "+                
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				if (nilaiToFloat(this.e_saldobmhd.getText()) == 0) this.cb_pph.setText(line.akun_beban);															
				else this.cb_pph.setText(line.akun_bmhd);
			}
		}
	},
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan "+
		             "from trans_m a "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.form = 'PPHREKON' and a.posted ='F'";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doLoad:function(sender){																		
		var strSQL = "select a.no_app as no_bukti,convert(varchar,a.tanggal,103) as tgl,a.no_bill as no_dokumen,b.keterangan "+
		             "from prb_bill_app a inner join trans_m b on a.no_app=b.no_bukti and a.kode_lokasi=b.kode_lokasi and param1='-' "+
		...
		try{
			if (this.sg.cells(0,row) != "") {
				...
				var strSQL = "select a.kode_proyek,a.kode_cust,a.keterangan,b.nama "+
							 "from prb_prbill_m a inner join prb_proyek b on a.kode_proyek=b.kode_proyek and a.kode_lokasi=b.kode_lokasi and b.versi='PRO20' "+
