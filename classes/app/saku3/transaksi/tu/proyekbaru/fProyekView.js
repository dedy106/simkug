window.app_saku3_transaksi_tu_proyekbaru_fProyekView = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyekbaru_fProyekView.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyekbaru_fProyekView";
		this.itemsValue = new arrayList();
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Approve Aktivasi Proyek", 0);	
...
window.app_saku3_transaksi_tu_proyekbaru_fProyekView.extend(window.childForm);
window.app_saku3_transaksi_tu_proyekbaru_fProyekView.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					if (this.c_status.getText()=="APPROVE")  {
						if (nilaiToFloat(this.e_nilai.getText()) > this.nilaiMax) var prog = "A"; 
						else var prog = "1"; 
					}
					else var prog = "N";												
					
					sql.add("update prb_proyek_app set no_appseb = '"+this.e_nb.getText()+"' where no_appseb ='-' and no_bukti='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='AKTIF_PR'");
					sql.add("insert into prb_proyek_app (no_app,tanggal,kode_lokasi,periode,nik_user,tgl_input,status,no_bukti,modul,no_appseb,catatan,no_rab,nik_app) values "+
									"('"+this.e_nb.getText()+"','"+this.dp_d3.getDateString()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.cb_app.getText()+"',getdate(),'"+this.c_status.getText()+"','"+this.cb_kode.getText()+"','AKTIF_PR','-','"+this.e_memo.getText()+"','"+this.e_rab.getText()+"','"+this.cb_app.getText()+"')");

					if (prog == "1" || prog == "A") {
						sql.add("update prb_proyek set progress='"+prog+"',no_app='"+this.e_nb.getText()+"', tgl_app='"+this.dp_d3.getDateString()+"' where kode_proyek ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "hapus" :	
				...
				sql.add("update prb_proyek set progress='0',no_app='-' where no_app='"+this.noAppLama+"' and kode_lokasi='"+this.app._lokasi+"'");				
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select a.*, x.no_rab,isnull(b.nik_user,'"+this.app._userLog+"') as nik_app "+
							 "from prb_proyek a "+
							 "inner join prb_rabapp_m x on a.kode_proyek=x.kode_proyek and a.kode_lokasi=x.kode_lokasi "+							 
							 "inner join prb_proyek_app b on x.no_rab=b.no_rab and x.kode_lokasi=b.kode_lokasi and b.no_appseb='-' and b.modul='NTF_RAB' "+
							 "where a.versi='PRO20' and a.kode_proyek ='"+this.cb_kode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";								 
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){			
	...
	doLoad3:function(sender){		
		...
		var strSQL = "select a.kode_proyek,b.nama,a.no_pks,a.nama as keterangan,a.nilai "+
					 ...
					 "where a.versi='PRO20' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress='0' order by a.kode_proyek";	
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doCari:function(sender){	
		...
		if (this.c_status2.getText() == "APPROVE") filter = " and ((a.progress = '1' and a.nilai<="+this.nilaiMax+") or (a.progress = 'A')) ";
		
		var strSQL = "select a.kode_proyek,b.nama,a.no_pks,a.nama as keterangan,a.nilai "+
					 "from prb_proyek a "+					 					 
					 "inner join prb_rabapp_m c on a.kode_proyek=c.kode_proyek and a.kode_lokasi=c.kode_lokasi "+					 					 
					 "inner join prb_rab_cust b on c.no_rab=b.no_rab and c.kode_cust=b.kode_cust and c.kode_lokasi=b.kode_lokasi "+					 					 
					 "where a.versi='PRO20' and a.kode_lokasi='"+this.app._lokasi+"' "+filter+" and a.kode_cust='"+this.cb_cust2.getText()+"' order by a.kode_proyek desc";							 
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
			this.sgn3.setTotalPage(Math.ceil(data.rs.rows.length/20));
