window.app_saku3_transaksi_tu_proyekbaru_fTagihPPN = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyekbaru_fTagihPPN.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyekbaru_fTagihPPN";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Penerimaan Faktur PPN Tagihan Proyek [Jurnal]", 0);	
		...
		this.c_wapu = new saiCB(this.pc1.childPage[0],{bound:[520,13,200,20],caption:"Status WAPU",items:["WAPU","NON"], readOnly:true,tag:2});		
		...
		this.c_status2 = new saiCB(this.pc2.childPage[2],{bound:[20,10,200,20],caption:"Status",items:["APPROVE"], readOnly:true,tag:9,change:[this,"doChange"]});
...
window.app_saku3_transaksi_tu_proyekbaru_fTagihPPN.extend(window.childForm);
window.app_saku3_transaksi_tu_proyekbaru_fTagihPPN.implement({	
	...
	simpan: function(){			
		try{		
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"prb_bill_app","no_app",this.app._lokasi+"-FP"+this.e_periode.getText().substr(2,4)+".","0000"));				
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					if (this.c_status.getText()=="APPROVE") var prog = "3"; 					
					else var prog = "X";						//---> kembalike ypt (validasi ppn ftagihvalid.js)						
					...
					if (this.c_wapu.getText() == "NON") {		
						sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','AR','PRPPN','F','-','-','"+this.ppProyek+"','"+this.dp_d3.getDateString()+"','"+this.e_bill.getText()+"','"+this.e_nama.getText()+"','IDR',1,"+nilaiToFloat(this.e_ppn.getText())+",0,0,'"+this.app._userLog+"','"+this.cb_app.getText()+"','-','"+this.cb_kode.getText()+"','-','-','-','-','-')");
						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_bill.getText()+"','"+this.dp_d3.getDateString()+"',1,'"+this.akunPiutang+"','D',"+nilaiToFloat(this.e_ppn.getText())+","+nilaiToFloat(this.e_ppn.getText())+",'"+this.e_nama.getText()+"','PRPPN','PIU','IDR',1,'"+this.ppProyek+"','-','-','-','-','-','-','-','-')");				
						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_bill.getText()+"','"+this.dp_d3.getDateString()+"',2,'"+this.akunPPN+"','C',"+nilaiToFloat(this.e_ppn.getText())+","+nilaiToFloat(this.e_ppn.getText())+",'"+this.e_nama.getText()+"','PRPPN','PPN','IDR',1,'"+this.ppProyek+"','-','-','-','-','-','-','-','-')");				
						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_bill.getText()+"','"+this.dp_d3.getDateString()+"',1,'"+this.akunPPN+"','D',"+nilaiToFloat(this.e_ppn.getText())+","+nilaiToFloat(this.e_ppn.getText())+",'"+this.e_nama.getText()+"','PRTAKPPN','PPN','IDR',1,'"+this.ppProyek+"','-','-','-','-','-','-','-','-')");				
						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_bill.getText()+"','"+this.dp_d3.getDateString()+"',2,'"+this.akunTAK+"','C',"+nilaiToFloat(this.e_ppn.getText())+","+nilaiToFloat(this.e_ppn.getText())+",'"+this.e_nama.getText()+"','PRTAKPPN','TAK','IDR',1,'"+this.ppProyek+"','-','-','-','-','-','-','-','-')");						
						...
						sql.add("insert into takkirim_m(no_kirim,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user,kode_loktuj,progress,no_terima,due_date) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d3.getDateString()+"','-','"+this.e_nama.getText()+"','"+this.app._kodePP+"','PRTAKPPN','KIRIM','IDR',1,"+nilaiToFloat(this.e_ppn.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"','X','-','-','"+this.akunTAK+"',getdate(),'"+this.app._userLog+"','"+this.lokTerima+"','0','-','"+this.dp_d3.getDateString()+"')");																
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "hapus" :	
				...
				uses("server_util_arrayList");
				var sql = new server_util_arrayList();				
				sql.add("update prb_prbill_m set progress='2',no_verppn='-' where no_bill ='"+this.e_bill.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doChange: function(sender){
		try{
			if (sender == this.e_bill && this.e_bill.getText() != ""){ 
				var strSQL = "select a.*, x.kode_pp as pprab,c.akun_piutang,c.jenis_pph42, x.no_rab,d.no_bill,convert(varchar,d.tanggal,103) as tgl_bill,d.keterangan as ket_bil,d.nilai as nilai_bil,d.diskon,d.nilai_ppn as ppn_bil,d.no_verppn,d.sts_wapu "+
							 "from prb_proyek a "+
			...
			if (sender==this.c_status2 && this.c_status.getText()!="") {
				this.getDataProyek();
			}
	...
	doLoad3:function(sender){		
		...
		var strSQL = "select c.no_bill,convert(varchar,c.tanggal,103) as tgl,c.keterangan,a.kode_proyek,b.nama,c.nilai,c.nilai_ppn "+
					 ...
					 "where c.nilai_ppn<>0 and c.kode_lokasi='"+this.app._lokasi+"' and c.progress ='2' order by c.no_bill";					 	
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	getDataProyek: function() {
		...
		var strSQL = "select a.kode_proyek,c.keterangan "+
					 ...
					 "inner join prb_rabapp_m d on a.kode_proyek=d.kode_proyek and a.kode_lokasi=d.kode_lokasi "+					 					 
					 "inner join prb_rab_cust b on d.no_rab=b.no_rab and d.kode_cust=b.kode_cust and d.kode_lokasi=b.kode_lokasi "+					 					 					 
					 "where c.nilai_ppn<>0 and c.kode_lokasi='"+this.app._lokasi+"' "+filter;	
