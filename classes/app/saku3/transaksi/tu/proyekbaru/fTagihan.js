window.app_saku3_transaksi_tu_proyekbaru_fTagihan = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyekbaru_fTagihan.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyekbaru_fTagihan";
		...
		//gak ada kontrol krn ada potongan yg tidak dicatat di kontrak
		...
		this.e_nilaipdpt = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,12,200,20],caption:"Tot Akru Pdpt", tag:1, tipeText:ttNilai, text:"0",readOnly:true,visible:false});												
		this.e_nilai = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,12,200,20],caption:"Nilai Tagihan", tag:1, tipeText:ttNilai, text:"0",readOnly:true, change:[this,"doChange"]});										
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:2,tag:9,visible:false,
		...
		try {
			...
			this.cb_proyek.setSQL("select a.kode_proyek, a.nama from prb_proyek a inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.nik='"+this.app._userLog+"' where a.versi='PRO20' and a.progress in ('1','2')  and a.flag_aktif='1' and a.kode_lokasi='"+this.app._lokasi+"'",["kode_proyek","nama"],false,["Kode","Deskripsi"],"and","Data Proyek",true);						
...
window.app_saku3_transaksi_tu_proyekbaru_fTagihan.extend(window.childForm);
window.app_saku3_transaksi_tu_proyekbaru_fTagihan.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into prb_prbill_m (no_bill,kode_lokasi,no_dokumen,tanggal,keterangan,kode_proyek,kode_cust,kode_curr,kurs,nik_app,kode_pp,nilai,periode,nik_user,tgl_input,akun_piutang,nilai_ppn,modul,pph42,no_ver,no_valid,no_verppn,progress,diskon) values "+
	...
	doChange:function(sender){	
		try {
			if (sender == this.cb_proyek && this.cb_proyek.getText()!="") {
				...
				var strSQL =  "select a.kode_pp,a.kode_cust,g.nama as nama_cust,d.wapu,a.nilai, b.akun_piutang, b.akun_bdd, b.jenis_pph42, e.no_rab,f.alamat,f.npwp,f.pic,f.jabatan "+ //a.nilai-isnull(c.bill,0) as sisa,
							"from prb_proyek a "+
	...
	doLoad3:function(sender){																													 
		var strSQL = "select a.no_bill,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai+a.nilai_ppn-a.diskon as nilai "+
					"from prb_prbill_m a "+								 			 					 
					"where  a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='BILL' and a.progress in ('0','V','P')";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
