...
*/
window.app_saku3_transaksi_tu_proyekbaru_fPrPtgMulti = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyekbaru_fPrPtgMulti.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyekbaru_fPrPtgMulti";
		...
		this.c_stsptg = new saiCB(this.pc2.childPage[0],{bound:[20,20,200,20],caption:"Status Pertggngan",items:["CLOSE","OPEN"], readOnly:true,tag:2,change:[this,"doChange"]});						
...
window.app_saku3_transaksi_tu_proyekbaru_fPrPtgMulti.extend(window.childForm);
window.app_saku3_transaksi_tu_proyekbaru_fPrPtgMulti.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from angg_r where no_bukti = '"+this.cb_panjar.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='R-PRBEBAN'");						
					}
					
					//jika close pjr tidk boleh dipilih lagi
					if (this.c_stsptg.getText() == "CLOSE") sql.add("update it_aju_m set progress='4' where no_aju='"+this.cb_panjar.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					
					var netto = nilaiToFloat(this.e_nilai.getText()) - nilaiToFloat(this.e_npajak.getText());
					sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,user_input,sts_pajak,form,npajak,no_ref1,dasar) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.c_jenis.getText()+"','"+this.cb_akun.getText()+"','"+this.ppKelola+"','"+this.cb_drk.getText()+"','"+this.cb_id.getText()+" | "+this.e_ket.getText()+"',"+netto+",getdate(),'"+this.app._userLog+"','-','-','-','A','"+this.cb_nik.getText()+"','"+this.cb_panjar.getText()+"','"+this.e_user.getText()+"','"+this.c_status.getText()+"','PRPTGMULTI',"+nilaiToFloat(this.e_npajak.getText())+",'"+this.c_stsptg.getText()+"','"+this.cb_pp.getText()+"')");
					...
					if (nilaiToFloat(this.e_nilai.getText()) > nilaiToFloat(this.e_saldosch.getText()))	{
						...
						sql.add("insert into prb_prbeban_d(no_bukti,kode_lokasi,periode,periode_sch,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.akunBDD+"','"+this.ppKelola+"','-','"+this.e_ket.getText()+"','D',"+nilaiBDD+",getdate(),'"+this.cb_id.getText()+"','BDD','-')");						
						sql.add("insert into it_aju_d (no_aju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,akun_seb) values "+
								"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',99,'"+this.akunBDD+"','"+this.e_ket.getText()+"','D','IDR',1,"+nilaiBDD+","+nilaiBDD+",'"+this.ppKelola+"','-','"+this.app._lokasi+"','PRBBN','BDD','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'-')");		
					}
					else {
					...
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) "+
							"select no_bukti,'R-PRBEBAN',kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,case dc when 'D' then 'C' else 'D' end,0,"+nRev+" "+
							"from angg_r where no_bukti='"+this.cb_panjar.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='PRBEBAN'");
					
					//beban riil saat ptg dikurangi saldo schedule (murni tidak dikurangi pajak-->lihat fIniAjuBeban.js)		
					sql.add("insert into prb_prbeban_d(no_bukti,kode_lokasi,periode,periode_sch,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_akun.getText()+"','"+this.ppKelola+"','"+this.cb_drk.getText()+"','"+this.e_ket.getText()+"','D',"+nilaiBeban+",getdate(),'"+this.cb_id.getText()+"','AJUBEBAN','-')");
					sql.add("insert into it_aju_d (no_aju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,akun_seb) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','D','IDR',1,"+nilaiBeban+","+nilaiBeban+",'"+this.ppKelola+"','"+this.cb_drk.getText()+"','"+this.app._lokasi+"','PJPR','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'-')");						

					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
							"('"+this.e_nb.getText()+"','PRBEBAN','"+this.app._lokasi+"','"+this.cb_akun.getText()+"','"+this.ppKelola+"','"+this.cb_drk.getText()+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','D',0,"+nilaiToFloat(this.e_nilai.getText())+")");
	...
	doChange:function(sender){
		...
		if (sender == this.cb_panjar && this.cb_panjar.getText()!="") {	
			var strSQL = "select convert(varchar,c.tgl_admin,103) as tgl_admin,a.nilai-isnull(e.totptg,0) as nilai,a.dasar as kode_pp,b.kode_proyek,a.kode_akun,c.nama,d.akun_beban,d.kode_drkb,d.akun_bdd "+
						 ...
						 "				   		from it_aju_m where form='PRPTGMULTI' and kode_lokasi='"+this.app._lokasi+"' and no_aju <>'"+this.e_nb.getText()+"' "+
			...
			var strSQL = "select a.kode_pp, a.nama,(a.nilai_or - a.pph42) - isnull(c.beban,0) as saldo_or "+
			             "from prb_proyek a "+			             
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){	
							if (this.preView == "1") {
								this.nama_report = "server_report_saku3_tu_proyek_rptPtgForm";
	...
	doLoad3:function(sender){	
		var strSQL = "select a.no_aju,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
					 ...
					 "where a.form='PRPTGMULTI' and a.modul='PJPTG' and a.progress in ('0','A','R') "+
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){
						...
						this.cb_drk.setText(line.kode_kode_drk);							
						this.cb_pp.setText(line.kode_pp);					
