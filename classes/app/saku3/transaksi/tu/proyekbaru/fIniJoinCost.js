window.app_saku3_transaksi_tu_proyekbaru_fIniJoinCost = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyekbaru_fIniJoinCost.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyekbaru_fIniJoinCost";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Jurnal Join Cost Inisiasi", 0);	
		...
		this.pc1 = new pageControl(this,{bound:[10,18,1000,440], childPage:["Data Transaksi","List Transaksi"]});		
		...
		this.cb_id = new saiCBBL(this.pc1.childPage[0],{bound:[20,12,220,20],caption:"ID Inisiasi", multiSelection:false, tag:1,change:[this,"doChange"]});
		this.e_uraian = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,16,550,20],caption:"Deskripsi Inisiasi", readOnly:true});				
		this.cb_akunbeban = new saiCBBL(this.pc1.childPage[0],{bound:[20,14,220,20],caption:"Akun Beban",  tag:1,readOnly:true});		
		this.cb_drkKeg = new saiCBBL(this.pc1.childPage[0],{bound:[20,11,220,20],caption:"DRK Inisiasi",  tag:1,readOnly:true});				
		
		this.cb_agenda = new saiCBBL(this.pc1.childPage[0],{bound:[20,12,220,20],caption:"No Agenda", multiSelection:false, tag:1,change:[this,"doChange"]});
		this.e_saldo = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,17,200,20],caption:"Saldo Agenda", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		
		this.cb_proyek = new saiCBBL(this.pc1.childPage[0],{bound:[20,19,220,20],caption:"ID Proyek", multiSelection:false, tag:1,change:[this,"doChange"]});		
		this.e_uraianproyek = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,16,550,20],caption:"Deskripsi Proyek", readOnly:true});				
		this.cb_ppproyek = new saiCBBL(this.pc1.childPage[0],{bound:[20,13,220,20],caption:"PP/Unit Proyek",  tag:1,readOnly:true});		
		this.cb_akun = new saiCBBL(this.pc1.childPage[0],{bound:[20,14,220,20],caption:"Akun Beban",  tag:1,readOnly:true});		
		this.cb_drk = new saiCBBL(this.pc1.childPage[0],{bound:[20,11,220,20],caption:"DRK Beban",  tag:1,readOnly:true});		
		...
		this.c_periodeSch = new saiCB(this.pc1.childPage[0],{bound:[20,10,200,20],caption:"Periode Schedule", readOnly:true,tag:1,change:[this,"doChange"]});		
		this.e_saldosch = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,17,200,20],caption:"Saldo Beban Sch", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		this.e_ket = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,16,550,20],caption:"Keterangan", maxLength:200});				
		this.e_nilai = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,18,200,20],caption:"Nilai Sharing", tag:1,  tipeText:ttNilai, text:"0"});		
		...
		try {
			...
			this.cb_akunbeban.setSQL("select kode_akun, nama from masakun where block='0' and kode_lokasi='"+this.app._lokasi+"'",["kode_akun","nama"],false,["Kode","Nama"],"and","Data Akun",true);																
...
window.app_saku3_transaksi_tu_proyekbaru_fIniJoinCost.extend(window.childForm);
window.app_saku3_transaksi_tu_proyekbaru_fIniJoinCost.implement({
	doChange: function(sender){	
		try {
			if (this.stsSimpan==1 && sender==this.cb_pp && this.cb_pp.getText()!="") {			
				this.cb_id.setSQL("select kode_proyek, nama from prb_proyek where versi='PRO20' and modul='INISIASI' and kode_pp='"+this.cb_pp.getText()+"' and flag_aktif='1' and kode_lokasi='"+this.app._lokasi+"'",["kode_proyek","nama"],false,["ID Project","Keterangan"],"and","Data Inisiasi",true);																
			...
			if (sender==this.cb_id && this.cb_id.getText()!="") {	
				var str = "select a.no_bukti,a.keterangan from prb_prbeban_d a "+
						...
						"		from prb_prbeban_d "+
						"		where no_ref1<>'-' and kode_lokasi = '"+this.app._lokasi+"' and no_bukti <> '"+this.e_nb.getText()+"' group by no_ref1,kode_lokasi "+
						") c on a.no_bukti=c.no_ref1 and a.kode_lokasi=b.kode_lokasi "+
						"where a.kode_proyek ='"+this.cb_id.getText()+"' and b.form in ('INISIASI','ISPJPTG') and dc='D' and a.kode_lokasi='"+this.app._lokasi+"' "+
						"and a.nilai > isnull(c.joincost,0) ";
				this.cb_agenda.setSQL(str,["no_bukti","keterangan"],false,["No Agenda","Deskripsi"],"and","Data Agenda",true);																

				var strSQL = "select b.akun_beban,b.kode_drkb,a.nama "+
							 "from prb_proyek a "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					this.cb_akunbeban.setText(line.akun_beban);
					this.cb_drkKeg.setText(line.kode_drkb);					
				}
			}	

			if (sender==this.cb_agenda && this.cb_agenda.getText()!="") {	
				var str = "select a.nilai - isnull(c.joincost,0) as saldo_agenda "+
						"from prb_prbeban_d a "+
						"inner join it_aju_m b on a.no_bukti=b.no_aju and a.kode_lokasi=b.kode_lokasi "+
						"left join ( "+
						"		select no_ref1,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as joincost "+
						"		from prb_prbeban_d "+
						"		where no_ref1='"+this.cb_agenda.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and no_bukti<>'"+this.e_nb.getText()+"' group by no_ref1,kode_lokasi "+
						") c on a.no_bukti=c.no_ref1 and a.kode_lokasi=b.kode_lokasi "+
						"where a.no_bukti ='"+this.cb_agenda.getText()+"' and b.form in ('INISIASI','ISPJPTG') and dc='D' and a.kode_lokasi='"+this.app._lokasi+"' ";
				var data = this.dbLib.getDataProvider(str,true);	
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line = data.rs.rows[0];									
					this.e_saldo.setText(floatToNilai(line.saldo_agenda));
				}
			}
			...
			if (sender == this.cb_proyek && this.cb_proyek.getText()!="") {
				if (this.stsSimpan == 1) {
					...
					var data = this.dbLib.getDataProvider("select distinct periode as periode from prb_proyek_d where periode ='"+this.app._periode+"' and kode_proyek='"+this.cb_proyek.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
					if (typeof data == "object" && data.rs.rows[0] != undefined){
						var line;
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','IJC','JC','F','-','-','"+this.cb_pp.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_id.getText()+"','"+this.e_ket.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",0,0,'"+this.app._userLog+"','"+this.app._userLog+"','-','"+this.cb_proyek.getText()+"','"+this.cb_agenda.getText()+"','-','-','-','INIJC')");
					
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_id.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.cb_akunbeban.getText()+"','C',"+parseNilai(this.e_nilai.getText())+","+
							parseNilai(this.e_nilai.getText())+",'"+this.e_ket.getText()+"','IJC','BBN_INI','IDR',1,'"+this.cb_ppproyek.getText()+"','"+this.cb_drkKeg.getText()+"','-','-','-','-','-','-','-')");
					...
					if (nilaiToFloat(this.e_nilai.getText()) > nilaiToFloat(this.e_saldosch.getText()))	{
						...
						sql.add("insert into prb_prbeban_d(no_bukti,kode_lokasi,periode,periode_sch,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1,jenis) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.c_periodeSch.getText()+"','"+this.dp_d1.getDateString()+"','"+this.akunBDD+"','"+this.cb_ppproyek.getText()+"','-','"+this.e_ket.getText()+"','D',"+nilaiBDD+",getdate(),'"+this.cb_proyek.getText()+"','BDD','"+this.cb_agenda.getText()+"','NONITAJU')");									
						...
						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
								...
								nilaiBDD+",'"+this.e_ket.getText()+"','IJC','BDD','IDR',1,'"+this.cb_ppproyek.getText()+"','-','-','-','-','-','-','-','-')");
					...
					if (nilaiBeban != 0) {
						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','D',"+nilaiBeban+","+
								nilaiBeban+",'"+this.e_ket.getText()+"','IJC','BBN_PR','IDR',1,'"+this.cb_ppproyek.getText()+"','-','-','-','-','-','-','-','-')");

						//noref1 diisi dgn nomor agenda utk kontrol saldo agenda terpakai		
						sql.add("insert into prb_prbeban_d(no_bukti,kode_lokasi,periode,periode_sch,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1,jenis) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.c_periodeSch.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_akun.getText()+"','"+this.cb_ppproyek.getText()+"','"+this.cb_drk.getText()+"','"+this.e_ket.getText()+"','D',"+nilaiBeban+",getdate(),'"+this.cb_proyek.getText()+"','AJUBEBAN','"+this.cb_agenda.getText()+"','NONITAJU')");		
					}

					sql.add("insert into prb_prbeban_d(no_bukti,kode_lokasi,periode,periode_sch,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1,jenis) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_akunbeban.getText()+"','"+this.cb_pp.getText()+"','"+this.cb_drkKeg.getText()+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_nilai.getText())+",getdate(),'"+this.cb_id.getText()+"','AJUBEBAN','-','NONITAJU')");
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :	
				...
				/* diloloskan---- tidka diprotek andin mei2019
				if (nilaiToFloat(this.e_nilai.getText()) > nilaiToFloat(this.e_saldo.getText())) {
					system.alert(this,"Transaksi JoinCost tidak valid.","Total Sharing melebihi Saldo Agenda.");
					return false;						
				}
				*/
	...
	doSelectDate: function(sender, y,m,d){
		...
		if (this.stsSimpan == 1) {
			this.cb_drkKeg.setSQL("select a.kode_drk, a.nama from drk a inner join prb_proyek_jenis b on a.kode_drk=b.kode_drkb and a.tahun=b.tahun and a.kode_lokasi=b.kode_lokasi where a.tahun='"+this.e_periode.getText().substr(0,4)+"' and a.kode_lokasi='"+this.app._lokasi+"'",["kode_drk","nama"],false,["Kode","Nama"],"and","Data DRK Beban",true);
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "") {
			...
			this.stsSimpan = 1;
			this.doChange(this.cb_pp);
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti",this.app._lokasi+"-IJC"+this.e_periode.getText().substr(2,4)+".","0000"));						
			this.cb_pp.setFocus();
			setTipeButton(tbSimpan);			
		}		
	},							
	...
	doLoad3:function(sender){				
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai1 "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'IJC' and a.posted ='F'";		
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){																				
						...
						this.cb_id.setSQL("select kode_proyek, nama from prb_proyek where kode_proyek='"+line.no_dokumen+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_proyek","nama"],false,["ID Project","Keterangan"],"and","Data Proyek",true);																
						...
						this.cb_agenda.setText(line.no_ref2);	
