window.app_saku3_transaksi_tu_proyekbaru_fRABView = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyekbaru_fRABView.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyekbaru_fRABView";
		...
		this.sg3 = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:7,tag:9,
		...
		this.pc1 = new pageControl(this.pc2.childPage[1],{bound:[1,20,996,358], childPage:["Data RAB","Detail PDPT", "Detail RAB","File Dokumen","Inisiasi Unit","Cattn Memo"]});
		this.c_status = new saiCB(this.pc1.childPage[0],{bound:[20,11,200,20],caption:"Status",items:["APPROVE","RETURN"], readOnly:true,tag:2});				
		this.e_memo = new saiMemo(this.pc1.childPage[0],{bound:[20,10,450,50],caption:"Catatan Verifikasi",tag:9});
		...
		this.e_nama = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,10,450,20],caption:"Deskripsi", maxLength:200, tag:1,readOnly:true});			
		this.e_bank = new saiLabelEdit(this.pc1.childPage[0],{bound:[520,10,400,20],caption:"Bank", maxLength:100, tag:1, readOnly:true});					
		this.cb_app = new portalui_saiCBBL(this.pc1.childPage[0],{bound:[20,15,220,20],caption:"Png. Jawab",tag:1,readOnly:true}); 						
		this.e_namarek = new saiLabelEdit(this.pc1.childPage[0],{bound:[520,15,400,20],caption:"Nama Rekening", maxLength:100, tag:1, readOnly:true});			
		...
		this.e_norek = new saiLabelEdit(this.pc1.childPage[0],{bound:[520,13,400,20],caption:"No Rekening", maxLength:50, tag:1, readOnly:true});			
		...
		this.e_join = new saiLabelEdit(this.pc1.childPage[0],{bound:[265,15,200,20],caption:"Ni JoinCost", tag:1, tipeText:ttNilai, text:"0",readOnly:true});			
...
window.app_saku3_transaksi_tu_proyekbaru_fRABView.extend(window.childForm);
window.app_saku3_transaksi_tu_proyekbaru_fRABView.implement({	
		...
		try{		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("update prb_rab_m set progress='"+prog+"' where no_rab ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					if(this.c_status.getText()=="APPROVE")	{						
						...
						sql.add("insert into prb_rabapp_m(no_app,no_rab,keterangan,kode_lokasi,kode_pp,kode_cust,tgl_mulai,tgl_selesai,nilai,nilai_or,p_or,kode_jenis,nik_app,progress,kode_proyek,pp_kelola,periode,no_dok, no_memo,ppn,pph42,no_appntf,no_appaktif, sts_va,bank,nama_rek,no_rek,tgl_admin) values "+	
								...
								this.cb_jenis.getText()+"','"+this.cb_app.getText()+"','"+prog+"','-','"+this.cb_ppkelola.getText()+"','"+this.periode+"','"+this.e_dok.getText()+"', '-',"+nilaiToFloat(this.e_ppn.getText())+","+nilaiToFloat(this.e_pph4.getText())+",'-','-', '"+this.c_stsva.getText()+"','"+this.e_bank.getText()+"','"+this.e_namarek.getText()+"','"+this.e_norek.getText()+"','"+this.dp_d4.getDateString()+"')"); //"+this.e_nomemo.getText()+"  
							...
							if (this.sg6.rowValid(i)) {							
								if (nilaiToFloat(this.sg6.cells(7,i)) != 0) {
									sql.add("insert into prb_prbeban_d(no_bukti,kode_lokasi,periode,periode_sch,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1,jenis) values "+
											"('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.periode+"','-','"+this.dp_d1.getDateString()+"','-','"+this.cb_pp.getText()+"','-','"+this.e_nama.getText()+"','D',"+nilaiToFloat(this.sg6.cells(7,i))+",getdate(),'"+this.cb_kode.getText()+"','AJUBEBAN','"+this.sg6.cells(0,i)+"','NONITAJU')");		
									sql.add("insert into prb_prbeban_d(no_bukti,kode_lokasi,periode,periode_sch,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1,jenis) values "+
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "hapus" :	
				...
				sql.add("update prb_rab_m set progress='0' where no_rab='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
				setTipeButton(tbAllFalse);	
				this.dbLib.execArraySQL(sql);			
				break;			
		}
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != "") {					
				if (this.noAppLama == "-") {	
					setTipeButton(tbSimpan);				
					var strSQL = "select a.* from prb_rab_m a where a.versi='PRO20' and a.no_rab ='"+this.cb_kode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";									
				}
				else {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){				
						...
						this.cb_jenis.setSQL("select kode_jenis,nama from prb_proyek_jenis where kode_pp='"+line.pp_kelola+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_jenis","nama"],false,["Kode","Nama"],"and","Data Jenis",true);								
						...
						if (this.noAppLama != "-") this.cb_jenis.setText(line.kode_jenis);					
						...
						if (this.noAppLama == "-") var data = this.dbLib.getDataProvider("select * from prb_rab_d where jenis='BEBAN' and no_rab = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and nu<>999 order by nu",true);
						...
						var strSQL = "select a.no_bukti,a.keterangan,a.kode_proyek,d.nama,a.nilai as ni_agenda, isnull(c.joincost,0) as joincost, a.nilai - isnull(c.joincost,0) as sisa_agenda,isnull(e.nilai_join,0) as nilai_join "+
									...
									"where d.kode_pp ='"+this.cb_pp.getText()+"' and b.form in ('INISIASI','ISPJPTG') and dc='D' and a.kode_lokasi='"+this.app._lokasi+"' "+
	...
	doLoad3:function(sender){		
		...
		var strSQL = "select distinct a.no_rab,b.nama,a.keterangan,a.nilai,'-' as no_app,'-' as no_apppr "+
					...
					"where a.versi='PRO20' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress = '0' "+ 				
	...
	doCari:function(sender){			
		...
		var strSQL = "select distinct a.no_rab,b.nama,a.keterangan,a.nilai,c.no_app, isnull(e.kode_proyek,'-') as no_apppr "+
					 ...
					 "where a.versi='PRO20' and a.progress in ('1','R') and a.kode_lokasi='"+this.app._lokasi+"' "+filter+" and a.kode_cust='"+this.cb_cust2.getText()+"' order by a.no_rab desc";			
