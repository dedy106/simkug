window.app_saku3_transaksi_tu_proyekbaru_fTagihValid = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_proyekbaru_fTagihValid.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_proyekbaru_fTagihValid";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Validasi Tagihan Proyek [YPT]", 0);	
		...
		this.cb_lokasi = new portalui_saiCBBL(this,{bound:[20,14,220,20],caption:"Lokasi Data",tag:2,multiSelection:false,change:[this,"doChange"]});
		
		this.pc2 = new pageControl(this,{bound:[10,10,1000,420], childPage:["List Bill","Data Bill","Filter Cari"]});		
		...
		this.pc1 = new pageControl(this.pc2.childPage[1],{bound:[1,20,996,296], childPage:["Data Proyek","Detail Tagihan","File Dokumen","Cattn Memo"]});
		...
		this.e_file = new saiLabelEdit(this.pc2.childPage[1],{bound:[520,70,300,20],caption:"File F-Pajak", readOnly:true, tag:1});		
		this.uploader = new uploader(this.pc2.childPage[1],{bound:[830,70,70,18],caption:"Browse File", param1:"uploadTo",param2:"server/media/tmp/",param3:"object",param4:"server/media/",autoSubmit:true, afterUpload:[this,"doAfterLoad"]});				
		this.bLihat = new button(this.pc2.childPage[1],{bound:[910,70,70,18],caption:"Lihat File",click:[this,"doLihat"]});			
		...
		try {
			...
			this.cb_lokasi.setSQL("select kode_lokasi,nama from lokasi where kode_lokkonsol='"+this.app._kodeLokasiKonsol+"'",["kode_lokasi","nama"],false,["Kode","Nama"],"and","Data Lokasi",true);
...
window.app_saku3_transaksi_tu_proyekbaru_fTagihValid.extend(window.childForm);
window.app_saku3_transaksi_tu_proyekbaru_fTagihValid.implement({	
	...
	simpan: function(){			
		try{		
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"prb_bill_app","no_app",this.app._lokasi+"-VD"+this.e_periode.getText().substr(2,4)+".","0000"));				
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("delete from prb_rab_dok where no_rab='"+this.noValidLama+"' and kode_lokasi='"+this.app._lokasi+"' ");								

					sql.add("update prb_bill_app set no_appseb='"+this.e_nb.getText()+"' where no_bukti ='"+this.e_bill.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and no_appseb='-' and modul='VALIDBILL'");
					sql.add("update prb_prbill_m set progress='"+prog+"',no_valid='"+this.e_nb.getText()+"' where no_bill ='"+this.e_bill.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("insert into prb_bill_app (no_app,tanggal,kode_lokasi,periode,nik_user,tgl_input,status,no_bukti,modul,no_appseb,catatan,no_bill,nik_app) values "+
									"('"+this.e_nb.getText()+"','"+this.dp_d3.getDateString()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.c_status.getText()+"','"+this.e_bill.getText()+"','VALIDBILL','-','"+this.e_memo.getText()+"','"+this.e_bill.getText()+"','"+this.cb_app.getText()+"')");

					sql.add("insert into prb_rab_dok(no_rab,no_gambar,nu,kode_jenis,kode_lokasi,modul,no_ref) values ('"+this.e_nb.getText()+"','"+this.e_file.getText()+"',888,'FPPN','"+this.app._lokasi+"','VALIDBILL','"+this.e_rab.getText()+"')");								
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :	
				this.preView = "1";	
				if (this.c_status.getText() == "APPROVE") this.e_file.setTag("1");
			...
			case "hapus" :	
				...
				uses("server_util_arrayList");
				var sql = new server_util_arrayList();				
				sql.add("update prb_prbill_m set progress='1',no_valid='-' where no_bill ='"+this.e_bill.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
				sql.add("delete from prb_bill_app where no_app='"+this.noValidLama+"' and kode_lokasi='"+this.app._lokasi+"'");
				sql.add("delete from prb_rab_dok where no_rab='"+this.noValidLama+"' and kode_lokasi='"+this.app._lokasi+"' ");								
				setTipeButton(tbAllFalse);	
				this.dbLib.execArraySQL(sql);			
				break;			
	...
	doClick:function(sender){
		...
		this.noValidLama = "-";
		this.stsSimpan = 1;
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"prb_proyek_app","no_app",this.app._lokasi+"-VD"+this.e_periode.getText().substr(2,4)+".","0000"));
	...
	doChange: function(sender){
		try{
			if (sender == this.e_bill && this.e_bill.getText() != ""){
				var strSQL = "select a.*, c.jenis_pph42, x.no_rab,d.no_bill,convert(varchar,d.tanggal,103) as tgl_bill,d.keterangan as ket_bil,d.nilai as nilai_bil,d.diskon,d.nilai_ppn as ppn_bil,d.no_valid "+
							 "from prb_proyek a "+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){					
						...
						if (this.stsSimpan == 0) this.noValidLama = line.no_valid;		
						this.doLoadCtt(this.e_bill.getText());
						...
						var data = this.dbLib.getDataProvider(
								...
								"where a.no_ref = '"+this.e_rab.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.nu<>888 order by a.nu",true);
						if (typeof data == "object" && data.rs.rows[0] != undefined){
							var line;
							this.sg1mp2.clear();
							for (var i in data.rs.rows){
					...
					var strSQL = "select catatan from prb_bill_app where no_app='"+this.noValidLama+"' and kode_lokasi='"+this.app._lokasi+"'";						   				
	...
	doLoad3:function(sender){		
		if (this.cb_lokasi.getText()!="") {
			...
			var strSQL = "select c.no_bill,convert(varchar,c.tanggal,103) as tgl,c.keterangan,a.kode_proyek,b.nama,c.nilai,c.nilai_ppn "+
						...
						"where c.kode_lokasi='"+this.cb_lokasi.getText()+"' and c.progress in ('1','X') and c.nilai_ppn<>0 order by c.no_bill";	
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU3 = data;
	...
	doCari:function(sender){	
		...
		if (this.c_status2.getText() == "APPROVE") filter = " and c.progress = '2' ";
		if (this.c_status2.getText() == "REVISI") filter = " and c.progress = 'P' "; 		
		...
		var strSQL = "select c.no_bill,convert(varchar,c.tanggal,103) as tgl,c.keterangan,a.kode_proyek,b.nama,c.nilai,c.nilai_ppn "+
					 ...
					 "inner join prb_rabapp_m d on a.kode_proyek=d.kode_proyek and a.kode_lokasi=d.kode_lokasi "+					 					 
					 "inner join prb_rab_cust b on d.no_rab=b.no_rab and d.kode_cust=b.kode_cust and d.kode_lokasi=b.kode_lokasi "+					 					 					 
					 "where c.kode_lokasi='"+this.cb_lokasi.getText()+"' and b.kode_cust='"+this.cb_cust2.getText()+"' "+filter;	
	...
	doLoadCtt: function(kode){
		try{
			var strSQL = "select distinct convert(varchar,tanggal,103) as tgl,tanggal from prb_bill_app "+
						 			 "where no_bill='"+kode+"' and kode_lokasi='"+this.app._lokasi+"' and no_app<>'"+this.noValidLama+"' "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					...
					var strSQL2 = "select catatan,no_app, convert(varchar,tanggal,103) as tgl,tanggal, convert(varchar,tgl_input,108) as jam,nik_user "+
								  "from prb_bill_app "+
								  "where no_bill='"+kode+"' and tanggal='"+line.tanggal+"' and kode_lokasi='"+this.app._lokasi+"' and no_app<>'"+this.noValidLama+"' "+
