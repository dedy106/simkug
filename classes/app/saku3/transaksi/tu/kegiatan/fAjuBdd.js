window.app_saku3_transaksi_tu_kegiatan_fAjuBdd = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_kegiatan_fAjuBdd.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_kegiatan_fAjuBdd";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan BDD Kegiatan", 0);	
		...
		this.c_jenis = new saiCB(this.pc2.childPage[0],{bound:[20,11,202,20],caption:"Kode Transaksi",items:["UMUM"], readOnly:true,tag:2, visible:false});
		...
		this.cb_keg = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,13,220,20],caption:"ID Kegiatan",tag:1,multiSelection:false,change:[this,"doChange"]});         		
		this.cb_nik = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,14,220,20],caption:"NIK PngJawab",tag:1,readOnly:true,change:[this,"doChange"]});         		
		...
		this.e_saldo = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,16,200,20],caption:"Saldo Kegiatan", tag:1, tipeText:ttNilai, text:"0", readOnly:true});				
		this.c_status = new saiCB(this.pc2.childPage[0],{bound:[20,20,200,20],caption:"Status Pajak",items:["NON","P21","P23"], readOnly:true,tag:2});				
		this.e_nilai = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,20,200,20],caption:"Total Bruto", tag:1, tipeText:ttNilai, text:"0", readOnly:true});				
		this.e_user = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,16,550,20],caption:"User input", maxLength:50});								
		this.e_npajak = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,16,200,20],caption:"Total Pajak", tag:0, tipeText:ttNilai, text:"0" , readOnly:true});		
		
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[2,12,995,213], childPage:["Daftar Rekening","List Panjar OutStanding","File Dokumen","Catatan Approval"]});
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:6,tag:0,
				...
				colWidth:[[5,4,3,2,1,0],[200,280,200,90,80,80]],
		...
		this.e_file = new saiLabelEdit(this.pc1.childPage[2],{bound:[20,15,450,20],caption:"File", readOnly:true, tag:9});		
		this.uploader = new uploader(this.pc1.childPage[2],{bound:[489,15,80,18],caption:"Browse File", param1:"uploadTo",param2:"server/media/tmp/",param3:"object",param4:"server/media/",autoSubmit:true, afterUpload:[this,"doAfterLoad"]});				
...
window.app_saku3_transaksi_tu_kegiatan_fAjuBdd.extend(window.childForm);
window.app_saku3_transaksi_tu_kegiatan_fAjuBdd.implement({	
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_kpa,no_app,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,user_input,form,sts_pajak,npajak,nik_app, no_ajukeg) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.c_jenis.getText()+"','"+this.cb_akun.getText()+"','"+this.cb_pp.getText()+"','-','"+this.e_ket.getText()+"',"+netto+",getdate(),'"+this.app._userLog+"','-','-','-','-','-','A','-','-','"+this.e_user.getText()+"','KEGBDD','"+this.c_status.getText()+"',"+nilaiToFloat(this.e_npajak.getText())+",'"+this.cb_app.getText()+"','"+this.cb_keg.getText()+"')");					
					...
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
							"('"+this.e_nb.getText()+"','ITKBAJUDRK','"+this.app._lokasi+"','"+this.cb_akun.getText()+"','"+this.cb_pp.getText()+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','D',"+nilaiToFloat(this.e_saldo.getText())+","+nilaiGar+")");
		...
		switch (event){
			...
			case "ubah" :									
				...
				if (nilaiToFloat(this.e_nilai.getText()) > nilaiToFloat(this.e_saldo.getText())) {
					system.alert(this,"Nilai transaksi tidak valid.","Nilai tidak boleh melebihi Saldo Budget Kegiatan.");
					return false;
				}
	...
	doLoad:function(sender){					
		if (this.app._userStatus == "A") 
			var strSQL = "select a.no_aju, a.keterangan, a.no_ajukeg+' | '+b.keterangan as kegiatan, a.nilai "+
						 ...
						 "where a.form = 'KEGBDD' and a.modul='UMUM' and a.progress in ('A','K','R') and a.periode<='" + this.e_periode.getText() + "' and a.kode_lokasi='"+this.app._lokasi+"'";					 			
		else 
			var strSQL = "select a.no_aju, a.keterangan, a.no_ajukeg+' | '+b.keterangan as kegiatan, a.nilai "+
						 "from it_aju_m a "+
						 "  inner join keg_aju_m b on a.no_ajukeg=b.no_aju and a.kode_lokasi=b.kode_lokasi "+
						 "  inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
						 "where a.nik_user='"+this.app._userLog+"' and c.kode_bidang ='" + this.app._kodeBidang + "' and a.form = 'KEGBDD' and a.modul='UMUM' and a.progress in ('A','K','R') and a.periode<='" + this.e_periode.getText() + "' and a.kode_lokasi='"+this.app._lokasi+"'";					 			
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg.cells(0,row) != "") {			
				...
				var data = this.dbLib.getDataProvider(
						"select a.nik_app,a.progress,a.modul,a.tanggal,a.periode,a.keterangan,a.nilai+a.npajak as bruto,a.kode_pp,a.kode_akun,a.kode_drk,isnull(x.no_kpa,'-') as no_ver,isnull(x.catatan,'-') as catatan, a.user_input,a.sts_pajak,a.npajak,isnull(c.no_gambar,'') as no_gambar ,a.no_ajukeg   "+
						"from it_aju_m a "+
