window.app_saku3_transaksi_tu_ntf21_fTagihAppDel = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_ntf21_fTagihAppDel.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_ntf21_fTagihAppDel";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Approval Pembatalan Tagihan", 0);	
		...
		this.c_status = new saiCB(this.pc2.childPage[1],{bound:[20,11,200,20],caption:"Status",items:["APPBATAL"], readOnly:true,tag:2});		
		...
		this.c_wapu = new saiLabelEdit(this.pc1.childPage[0],{bound:[520,13,200,20],caption:"Status WAPU",readOnly:true,tag:2,change:[this,"doChange"]});		
		...
		this.e_file = new saiLabelEdit(this.pc1.childPage[0],{bound:[520,19,340,20],caption:"File F-Pajak", readOnly:true, tag:9});		
...
window.app_saku3_transaksi_tu_ntf21_fTagihAppDel.extend(window.childForm);
window.app_saku3_transaksi_tu_ntf21_fTagihAppDel.implement({	
	...
	simpan: function(){			
		try{		
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"prb_bill_app","no_app",this.app._lokasi+"-ABT"+this.e_periode.getText().substr(2,4)+".","0000"));				
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					
					//status pengajuan pembatalan tagihan load yg progress = J,, setalah di app di tagiahn di delete backup ke tabel his
					//supaya saat amandemen bisa lolos
					if (this.c_status.getText() == "APPBATAL") {					
						sql.add("insert into prb_bill_app (no_app,tanggal,kode_lokasi,periode,nik_user,tgl_input,status,no_bukti,modul,no_appseb,catatan,no_bill,nik_app) values "+
								"('"+this.e_nb.getText()+"','"+this.dp_d3.getDateString()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.c_status.getText()+"','"+this.e_bill.getText()+"','APPBTLBIL','-','"+this.e_memo.getText()+"','"+this.e_bill.getText()+"','"+this.cb_app.getText()+"')");
						...
						sql.add("insert into prb_prbill_m_his (no_bill,kode_lokasi,no_dokumen,tanggal,keterangan,kode_proyek,kode_cust,kode_curr,kurs,nik_app,kode_pp,nilai,periode,nik_user,tgl_input,akun_piutang,nilai_ppn,modul,pph42,no_ver,no_valid,no_verppn,progress,diskon,no_batal,no_appbatal)  "+
								"select no_bill,kode_lokasi,no_dokumen,tanggal,keterangan,kode_proyek,kode_cust,kode_curr,kurs,nik_app,kode_pp,nilai,periode,nik_user,tgl_input,akun_piutang,nilai_ppn,modul,pph42,no_ver,no_valid,no_verppn,progress,diskon,no_batal,'"+this.e_nb.getText()+"' "+
								"from prb_prbill_m where no_bill='"+this.e_bill.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");

						sql.add("insert into prb_prbill_d_his (no_bill,kode_lokasi,nu,keterangan,harga,jumlah,total) "+		
								"select no_bill,kode_lokasi,nu,keterangan,harga,jumlah,total "+
								"from prb_prbill_d where no_bill='"+this.e_bill.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");

						sql.add("insert into prb_prbill_cust_his (no_bill,kode_lokasi,nama,alamat,npwp,pic,jabatan) "+
								"select no_bill,kode_lokasi,nama,alamat,npwp,pic,jabatan "+
								"from prb_prbill_cust where no_bill='"+this.e_bill.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");

						sql.add("delete from prb_prbill_m where no_bill='"+this.e_bill.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from prb_prbill_d where no_bill='"+this.e_bill.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from prb_prbill_cust where no_bill='"+this.e_bill.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						

						//jika sudah ada jurnal
						if (this.no_verppn != "-") {
							sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','AR','BTLBILL','F','-','-','"+this.ppProyek+"','"+this.dp_d3.getDateString()+"','"+this.e_bill.getText()+"','"+this.e_ketbil.getText()+"','IDR',1,"+nilaiToFloat(this.e_nilai.getText())+","+nilaiToFloat(this.e_ppn.getText())+",0,'"+this.app._userLog+"','"+this.app._userLog+"','-','"+this.cb_kode.getText()+"','-','-','-','-','-')");
											
							if (this.c_wapu.getText() == "NON" && nilaiToFloat(this.e_ppn.getText()) != 0) {
								var nilaiPiu = nilaiToFloat(this.e_total.getText());		
							}
							else {
								var nilaiPiu = nilaiToFloat(this.e_nilai.getText());		
							}

							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_bill.getText()+"','"+this.dp_d3.getDateString()+"',0,'"+this.akunPiutang+"','C',"+nilaiPiu+","+
									nilaiToFloat(this.e_total.getText())+",'"+this.e_ketbil.getText()+"','BTLBILL','PIU','IDR',1,'"+this.ppProyek+"','-','-','-','-','-','-','-','-')");				
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_bill.getText()+"','"+this.dp_d3.getDateString()+"',1,'"+akunLawan+"','D',"+nilaiToFloat(this.e_nilai.getText())+","+
									nilaiToFloat(this.e_nilai.getText())+",'"+this.e_ketbil.getText()+"','BTLBILL','LAWAN','IDR',1,'"+this.ppProyek+"','"+kodeDRK+"','-','-','-','-','-','-','-')");				
														
							if (this.c_wapu.getText() == "NON" && nilaiToFloat(this.e_ppn.getText()) != 0) {		
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_bill.getText()+"','"+this.dp_d3.getDateString()+"',2,'"+this.akunPPN+"','D',"+nilaiToFloat(this.e_ppn.getText())+","+nilaiToFloat(this.e_ppn.getText())+",'"+this.e_ketbil.getText()+"','PRPPN','PPN','IDR',1,'"+this.ppProyek+"','-','-','-','-','-','-','-','-')");				
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_bill.getText()+"','"+this.dp_d3.getDateString()+"',3,'"+this.akunPPN+"','C',"+nilaiToFloat(this.e_ppn.getText())+","+nilaiToFloat(this.e_ppn.getText())+",'"+this.e_ketbil.getText()+"','PRTAKPPN','PPN','IDR',1,'"+this.ppProyek+"','-','-','-','-','-','-','-','-')");				
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_bill.getText()+"','"+this.dp_d3.getDateString()+"',4,'"+this.akunTAK+"','D',"+nilaiToFloat(this.e_ppn.getText())+","+nilaiToFloat(this.e_ppn.getText())+",'"+this.e_ketbil.getText()+"','PRTAKPPN','TAK','IDR',1,'"+this.ppProyek+"','-','-','-','-','-','-','-','-')");						
								...
								sql.add("insert into takkirim_j(no_kirim,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','-','"+this.dp_d3.getDateString()+"',999,'"+this.akunTAK+"','"+this.e_ketbil.getText()+"','D',"+nilaiToFloat(this.e_ppn.getText())+",'"+this.ppProyek+"','-','"+this.app._lokasi+"','PRTAKPPN','TAK','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");						
							}

							//jika pyt dan sudah di distribusi
							sql.add("insert into prb_prpyt_d(no_bukti,kode_lokasi,kode_proyek,periode,periode_dis,akun_pyt,akun_pdpt,kode_pp,kode_drk,nilai,dc,modul) "+
									"select '"+this.e_nb.getText()+"',kode_lokasi,kode_proyek,'"+this.e_periode.getText()+"',periode_dis,akun_pyt,akun_pdpt,kode_pp,'"+this.kode_drkp+"',abs(sum(case dc when 'D' then nilai else -nilai end)),case when sum(case dc when 'D' then nilai else -nilai end) > 0 then 'C' else 'D' end,'BTLBILL' "+
									"from prb_prpyt_d "+
							...
							//jika ada beban yg sudah finalisasi dr bdd .. balikin ke bdd
							sql.add("insert into prb_beban_reklas (no_bukti,kode_lokasi,periode,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,nik_user,kode_proyek,form) "+
									"select '"+this.e_nb.getText()+"',kode_lokasi,'"+this.e_periode.getText()+"','"+this.dp_d3.getDateString()+"',kode_akun,'"+this.ppProyek+"','"+kodeDRK+"','"+this.e_ketbil.getText()+"',case when sum(case dc when 'D' then nilai else -nilai end) > 0 then 'C' else 'D' end,abs(sum(case dc when 'D' then nilai else -nilai end)),getdate(),'"+this.app._userLog+"',kode_proyek,'BTLBILL' "+
									"from prb_beban_reklas "+
									"where kode_proyek='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
									"group by kode_lokasi,kode_akun,kode_proyek");
							...
							sql.add("insert into prb_prbdd_d(no_bukti,kode_lokasi,periode,tanggal,kode_akun,kode_pp,keterangan,dc,nilai,tgl_input,nik_user,kode_proyek,form)  "+
									"select '"+this.e_nb.getText()+"',kode_lokasi,periode,tanggal,'"+this.akunBDD+"',kode_pp,keterangan,case dc when 'C' then 'D' else 'C' end,nilai,tgl_input,nik_user,kode_proyek,form "+
									"from prb_beban_reklas "+
									"where no_bukti='"+this.e_nb.getText()+"' and kode_proyek='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");
							...
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) "+
									"select no_bukti,kode_lokasi,getdate(),'"+this.app._userLog+"',periode,kode_proyek,'"+this.dp_d3.getDateString()+"',88,akun_pyt,dc,nilai,nilai,'"+this.e_ketbil.getText()+"','BTLBILL','PYT','IDR',1,kode_pp,'-','-','-','-','-','-','-','-' "+
									"from prb_prpyt_d where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");
							
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) "+
									"select no_bukti,kode_lokasi,getdate(),'"+this.app._userLog+"',periode,kode_proyek,'"+this.dp_d3.getDateString()+"',89,akun_pdpt,case dc when 'C' then 'D' else 'C' end,nilai,nilai,'"+this.e_ketbil.getText()+"','BTLBILL','PDPT','IDR',1,kode_pp,kode_drk,'-','-','-','-','-','-','-' "+
									"from prb_prpyt_d where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");

							//reverse beban		
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) "+
									"select no_bukti,kode_lokasi,tgl_input,nik_user,periode,kode_proyek,tanggal,0,kode_akun,dc,nilai,nilai,keterangan,'BTLBILL','BEBAN','IDR',1,kode_pp,kode_drk,'-','-','-','-','-','-','-' "+
									"from prb_beban_reklas where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");
									
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) "+
									"select no_bukti,kode_lokasi,tgl_input,nik_user,periode,kode_proyek,tanggal,1,kode_akun,dc,nilai,nilai,keterangan,'BTLBILL','BDD','IDR',1,kode_pp,'-','-','-','-','-','-','-','-' "+
									"from prb_prbdd_d where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");

							//budget		
							sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai)  "+
									"select no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode,periode,dc,0,nilai "+
									"from trans_j where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and jenis in ('BEBAN','PDPT')");

						}

						sql.add("update trans_m set no_ref2='"+this.e_nb.getText()+"' where form in ('PRPIU','BTLBILL') and no_ref2='-' and no_ref1='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}
	...
	doChange: function(sender){
		try{
			if (sender == this.e_bill && this.e_bill.getText() != ""){ 
				var strSQL = "select a.*, a.kode_pp as pp_kelola,d.akun_piutang,c.kelompok,c.akun_bdd,c.akun_pyt,c.akun_pdpt,c.kode_drkp,c.jenis_pph42,x.no_rab,d.no_bill,convert(varchar,d.tanggal,103) as tgl_bill,d.keterangan as ket_bil,d.nilai as nilai_bil,d.diskon,d.nilai_ppn as ppn_bil,d.no_verppn,d.sts_wapu "+
							 "from prb_proyek a "+
	...
	doLoad3:function(sender){		
		...
		var strSQL = "select c.no_bill,convert(varchar,c.tanggal,103) as tgl,c.keterangan,a.kode_proyek,b.nama,c.nilai,c.nilai_ppn "+
					 ...
					 "where c.progress='J' and c.kode_lokasi='"+this.app._lokasi+"'  order by c.no_bill";					 	
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
