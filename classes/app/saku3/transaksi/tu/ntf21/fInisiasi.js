window.app_saku3_transaksi_tu_ntf21_fInisiasi = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_ntf21_fInisiasi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_ntf21_fInisiasi";		
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Inisiasi Proyek", 0);	
		
		uses("saiCBBL;saiEdit;saiGrid;sgNavigator");
		
		this.pc2 = new pageControl(this,{bound:[10,10,1000,450], childPage:["Data Inisiasi","List Inisiasi"]});		
		this.sg3 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:4,tag:9,
				colTitle:["ID Inisiasi","Deskripsi","Nilai","Pilih"],
				colWidth:[[3,2,1,0],[70,200,300,100]],
				colFormat:[[2,3],[cfNilai,cfButton]],readOnly:true,
		...
		this.e_dok = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,14,500,20],caption:"No Dokumen", maxLength:100, tag:1});			
		this.e_nama = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,11,500,20],caption:"Deskripsi", maxLength:200, tag:1});			
		this.cb_jenis = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,19,220,20],caption:"Jenis",tag:2,multiSelection:false,change:[this,"doChange"]}); 								
		...
		this.e_nilaior = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,18,200,20],caption:"Nilai Alokasi", tag:1, tipeText:ttNilai, text:"0"});						
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,20,996,163], childPage:["File Dokumen"]});
		this.sgUpld = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:5,
		...
		try {
			...
			this.cb_buat.setSQL("select nik, nama from karyawan a where kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Karyawan",true);	
			this.cb_app.setSQL("select nik, nama from karyawan a where kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Karyawan",true);												

			this.cb_pp.setText(this.app._kodePP);	
			this.dp_d3.setText("31/12/"+this.dp_d1.year);
...
window.app_saku3_transaksi_tu_ntf21_fInisiasi.extend(window.childForm);
window.app_saku3_transaksi_tu_ntf21_fInisiasi.implement({	
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into prb_proyek(kode_proyek,nama,flag_aktif,kode_lokasi,no_pks,kode_pp,kode_cust,tgl_mulai,tgl_selesai,nilai,nilai_or,p_or,kode_jenis,nilai_ppn,jumlah,  nik_app,progress,no_app, tgl_app,nik_buat,modul,tgl_admin,versi) values "+
						    "('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','1','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.cb_pp.getText()+"','-','"+this.dp_d2.getDateString()+"','"+this.dp_d3.getDateString()+"',0,"+nilaiToFloat(this.e_nilaior.getText())+",0,'"+this.cb_jenis.getText()+"',0,0,'"+this.cb_app.getText()+"','X','-','"+this.dp_d1.getDateString()+"','"+this.cb_buat.getText()+"','INISIASI','"+this.dp_d3.getDateString()+"','NTF21')");
					...
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i) && this.sgUpld.cells(2,i)!="-") {							
							...
							sql.add("insert into prb_rab_dok(no_rab,no_gambar,nu,kode_jenis,kode_lokasi,modul,no_ref) values ('"+this.cb_kode.getText()+"','"+this.sgUpld.cells(3,i).tmpfile+"','"+ix+"','"+this.sgUpld.cells(0,i)+"','"+this.app._lokasi+"','INISIASI','"+this.cb_kode.getText()+"')");																				
						}	
					}			
					setTipeButton(tbAllFalse);
	...
	doClick:function(sender){		
		try {
			...
			this.cb_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"prb_proyek","kode_proyek",this.cb_pp.getText()+"-IS"+this.periode.substr(2,4)+".","000"));						
			this.e_dok.setFocus();	
			this.stsSimpan = 1;		
		}
		catch(e) {
	    			...
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){									
							if (this.preView == "1") {
								...
								this.nama_report="server_report_saku3_tu_proyek_rptInisiasi";
	...
	doLoad3:function(sender){																				
		var strSQL = "select a.kode_proyek,a.nama as keterangan,a.nilai_or as nilai "+
		             "from prb_proyek a where  a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'INISIASI' and a.versi='NTF21' ";		
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.kode_proyek,line.keterangan,floatToNilai(line.nilai),"Pilih"]); 
		}
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select * from prb_proyek where kode_proyek ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";						   
				var data = this.dbLib.getDataProvider(strSQL,true);
