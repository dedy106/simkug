window.app_saku3_transaksi_tu_ntf21_fPrAjuPanjar = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_ntf21_fPrAjuPanjar.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_ntf21_fPrAjuPanjar";
...
window.app_saku3_transaksi_tu_ntf21_fPrAjuPanjar.extend(window.childForm);
window.app_saku3_transaksi_tu_ntf21_fPrAjuPanjar.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,user_input,form,sts_pajak, npajak,dasar) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.c_jenis.getText()+"','"+this.akunPJ+"','"+this.ppKelola+"','-','"+this.cb_id.getText()+" | "+this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",getdate(),'"+this.app._userLog+"','-','-','-','A','"+this.cb_nik.getText()+"','-','"+this.e_user.getText()+"','NTF20','NON',0,'"+this.cb_pp.getText()+"')");					
					
					//pengajuan panjar sudah dianggap sebagai realisasi beban (AJUBEBAN).. 					
					...
					sql.add("insert into it_aju_d (no_aju, no_dokumen, tanggal, no_urut, kode_akun, keterangan, dc, kode_curr, kurs, nilai_curr, nilai, kode_pp, kode_drk, kode_lokasi, modul, jenis, periode, nik_user, tgl_input, akun_seb ) values "+
							"('"+this.e_nb.getText()+"', '"+this.cb_id.getText()+"', '"+this.dp_d1.getDateString()+"', 0, '"+this.akunPJ+"', '"+this.e_ket.getText()+"', 'D', 'IDR', 1, "+nilaiToFloat(this.e_nilai.getText())+", "+nilaiToFloat(this.e_nilai.getText())+", '"+this.ppKelola+"', '-', '"+this.app._lokasi+"', 'NTF20', 'PJ', '"+this.e_periode.getText()+"', '"+this.app._userLog+"', getdate(), '-' )");
		
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
							"('"+this.e_nb.getText()+"','NTF20','"+this.app._lokasi+"','"+this.akunPJ+"','"+this.ppKelola+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','D',0,"+nilaiToFloat(this.e_nilai.getText())+")");
	...
	doChange:function(sender){
		...
		if (sender==this.cb_id && this.cb_id.getText()!="") {
			var strSQL = "select a.kode_pp,convert(varchar,a.tgl_admin,103) as tgl_admin,a.nama,a.nilai_or - isnull(c.pakai_or,0) as saldo "+
			             "from prb_proyek a "+			             
	...
	doLoad3:function(sender){		
		var strSQL = "select a.no_aju,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
		             ...
		             "where a.form='NTF20' and a.modul='PANJAR' and a.progress in ('0','A','R') and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and b.nik='"+this.app._userLog+"'";	
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.nik_panjar,aa.kode_proyek,a.progress,a.modul,a.tanggal,a.periode,substring(a.keterangan,18,200) as keterangan,a.nilai,a.dasar as kode_pp,a.user_input, "+
						     "r.bank,r.no_rek,r.nama_rek , isnull(o.no_gambar,'') as no_gambar "+
						     "from it_aju_m a "+
