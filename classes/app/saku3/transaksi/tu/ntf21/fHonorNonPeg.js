window.app_saku3_transaksi_tu_ntf21_fHonorNonPeg = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_ntf21_fHonorNonPeg.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_ntf21_fHonorNonPeg";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan Honor Non Pegawai", 0);	
		...
		this.c_bayar = new saiCB(this.pc2.childPage[0],{bound:[790,19,200,20],caption:"Jenis Bayar",items:["TRANSFER","TUNAI"], readOnly:true,tag:2,change:[this,"doChange"]});
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,20,990,212], childPage:["Detail Daftar","Error Daftar"]});		
...
window.app_saku3_transaksi_tu_ntf21_fHonorNonPeg.extend(window.childForm);
window.app_saku3_transaksi_tu_ntf21_fHonorNonPeg.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_kpa,no_app,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,user_input,form,sts_pajak,npajak,nik_app) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.c_jenis.getText()+"','"+this.cb_akun.getText()+"','"+this.cb_pp.getText()+"','-','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_netto.getText())+",getdate(),'"+this.app._userLog+"','-','-','-','-','-','A','-','-','"+this.e_user.getText()+"','NTF30','P21',"+vPajak+",'"+this.cb_app.getText()+"')");					
					...
					sql.add("insert into it_aju_d (no_aju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,akun_seb) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','"+this.cb_id.getText()+" | "+this.e_ket.getText()+"','D','IDR',1,"+nilaiToFloat(this.e_bruto.getText())+","+nilaiToFloat(this.e_bruto.getText())+",'"+this.ppKelola+"','-','"+this.app._lokasi+"','NTF30','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'-')");		
					sql.add("insert into it_aju_d (no_aju, no_dokumen, tanggal, no_urut, kode_akun, keterangan, dc, kode_curr, kurs, nilai_curr, nilai, kode_pp, kode_drk, kode_lokasi, modul, jenis, periode, nik_user, tgl_input, akun_seb ) values "+
							"('"+this.e_nb.getText()+"', '"+this.cb_id.getText()+"', '"+this.dp_d1.getDateString()+"', 1, '"+this.akunPPH21+"', '"+this.e_ket.getText()+"', 'C', 'IDR', 1, "+vPajak+", "+vPajak+", '"+this.ppKelola+"', '-', '"+this.app._lokasi+"', 'NTF30', 'P21', '"+this.e_periode.getText()+"', '"+this.app._userLog+"', getdate(), '-' )");												
					...
					sql.add("insert into prb_prbdd_d(no_bukti,kode_lokasi,periode,tanggal,kode_akun,kode_pp,keterangan,dc,nilai,tgl_input,nik_user,kode_proyek,form) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_akun.getText()+"','"+this.ppKelola+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_bruto.getText())+",getdate(),'"+this.app._userLog+"','"+this.cb_id.getText()+"','NTF30')");
					...
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
							"('"+this.e_nb.getText()+"','NTF30','"+this.app._lokasi+"','"+this.cb_akun.getText()+"','"+this.ppKelola+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','D',0,"+nilaiToFloat(this.e_bruto.getText())+")");
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :			
				...
				if (this.stsGar == "1") {					
					if (nilaiToFloat(this.e_bruto.getText()) > nilaiToFloat(this.e_nsaldo.getText())) {
						system.alert(this,"Transaksi tidak valid.","Nilai Bruto transaksi melebihi saldo OR.");
						return false;						
					}					
				}									
	...
	doLoad3:function(sender){																
		var strSQL = "select no_aju, convert(varchar,tanggal,103) as tgl, keterangan, nilai "+
					 "from it_aju_m "+
					 "where kode_pp='"+this.cb_pp.getText()+"' and form = 'NTF30' and modul='UMUM' and progress in ('0','A','R') and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider("select a.nik_app,a.progress,a.modul,a.tanggal,a.periode,a.keterangan,a.nilai,a.kode_pp,a.kode_akun,isnull(x.no_kpa,'-') as no_ver,isnull(x.catatan,'-') as catatan, r.bank,r.no_rek,r.nama_rek,a.user_input,a.sts_pajak,a.npajak,isnull(c.no_gambar,'') as no_gambar ,y.kode_proyek  "+
						   "from it_aju_m a "+
