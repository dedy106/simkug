...
*/
window.app_saku3_transaksi_tu_ntf21_fPrPtgFF = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_ntf21_fPrPtgFF.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_ntf21_fPrPtgFF";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pertanggungan Panjar Proyek Multi", 0);	
		...
		this.e_nilaipj = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,20,200,20],caption:"Nilai Panjar", tipeText:ttNilai, text:"0", readOnly:true,change:[this,"doChange"]});		
		...
		this.c_stsptg = new saiCB(this.pc2.childPage[0],{bound:[790,12,200,20],caption:"Status Pertggngan",items:["CLOSE","OPEN"], readOnly:true,tag:2,change:[this,"doChange"]});						
		...
		this.e_nilaikb = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,17,200,20],caption:"Nilai Selisih Kas", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});
		
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,33,996,213], childPage:["Data Proyek","User & Verifikasi"]});			
		this.cb_id = new portalui_saiCBBL(this.pc1.childPage[0],{bound:[20,19,220,20],caption:"ID Proyek",tag:1,readOnly:true,change:[this,"doChange"]}); 				
		this.e_uraian = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,16,550,20],caption:"Deskripsi", readOnly:true});				
		this.e_nsaldo = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,19,200,20],caption:"Saldo OR", tag:1, tipeText:ttNilai, text:"0",readOnly:true});		
		this.e_tgladm = new saiLabelEdit(this.pc1.childPage[0],{bound:[370,19,200,20],caption:"TglMax Adminstrsi", tag:1, readOnly:true});				
		this.cb_akun = new portalui_saiCBBL(this.pc1.childPage[0],{bound:[20,11,220,20],caption:"Akun BDD",change:[this,"doChange"],readOnly:true});		
...
window.app_saku3_transaksi_tu_ntf21_fPrPtgFF.extend(window.childForm);
window.app_saku3_transaksi_tu_ntf21_fPrPtgFF.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from angg_r where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='NTF20'");
						sql.add("delete from angg_r where no_bukti = '"+this.cb_panjar.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='NTF21'");						
					}
					
					//jika close pjr tidk boleh dipilih lagi
					if (this.c_stsptg.getText() == "CLOSE") 
						sql.add("update it_aju_m set progress='4' where no_aju='"+this.cb_panjar.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					
					var netto = nilaiToFloat(this.e_nilai.getText()) - nilaiToFloat(this.e_npajak.getText());
					sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,user_input,sts_pajak,form,npajak,no_ref1,dasar,status_if) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.c_jenis.getText()+"','"+this.cb_akun.getText()+"','"+this.ppKelola+"','-','"+this.cb_id.getText()+" | "+this.e_ket.getText()+"',"+netto+",getdate(),'"+this.app._userLog+"','-','-','-','A','"+this.cb_nik.getText()+"','"+this.cb_panjar.getText()+"','"+this.e_user.getText()+"','"+this.c_status.getText()+"','NTF21',"+nilaiToFloat(this.e_npajak.getText())+",'-','"+this.cb_pp.getText()+"','"+this.c_stsptg.getText()+"')");

					var nilaiBeban = nilaiToFloat(this.e_nilai.getText());											
					//reverse beban2 saat ambil pj		
					if (this.c_stsptg.getText() == "CLOSE") var nRev = nilaiToFloat(this.e_nilaipj.getText());
					else var nRev = nilaiToFloat(this.e_nilai.getText());
					sql.add("insert into prb_prbeban_d(no_bukti,kode_lokasi,periode,periode_sch,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.akunPJ+"','"+this.ppKelola+"','-','"+this.e_ket.getText()+"','C',"+nRev+",getdate(),'"+this.cb_id.getText()+"','AJUBEBAN','-')");
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) "+
							"select no_bukti,'NTF21',kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,case dc when 'D' then 'C' else 'D' end,0,"+nRev+" "+
							"from angg_r where no_bukti='"+this.cb_panjar.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='NTF20'");
					...
					sql.add("insert into it_aju_d (no_aju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,akun_seb) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','D','IDR',1,"+nilaiBeban+","+nilaiBeban+",'"+this.ppKelola+"','-','"+this.app._lokasi+"','NTF21','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'-')");											
					sql.add("insert into prb_prbeban_d(no_bukti,kode_lokasi,periode,periode_sch,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_akun.getText()+"','"+this.ppKelola+"','-','"+this.e_ket.getText()+"','D',"+nilaiBeban+",getdate(),'"+this.cb_id.getText()+"','AJUBEBAN','-')");
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
							"('"+this.e_nb.getText()+"','NTF21','"+this.app._lokasi+"','"+this.cb_akun.getText()+"','"+this.ppKelola+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','D',0,"+nilaiBeban+")");
					sql.add("insert into prb_prbdd_d(no_bukti,kode_lokasi,periode,tanggal,kode_akun,kode_pp,keterangan,dc,nilai,tgl_input,nik_user,kode_proyek,form) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_akun.getText()+"','"+this.ppKelola+"','"+this.e_ket.getText()+"','D',"+nilaiBeban+",getdate(),'"+this.app._userLog+"','"+this.cb_id.getText()+"','NTF21')");
											
					if (this.c_status.getText() != "NON") {
						if (this.c_status.getText() == "P21") var akunPajak = this.akunPPH21;
						else var akunPajak = this.akunPPH23;
						sql.add("insert into it_aju_d (no_aju, no_dokumen, tanggal, no_urut, kode_akun, keterangan, dc, kode_curr, kurs, nilai_curr, nilai, kode_pp, kode_drk, kode_lokasi, modul, jenis, periode, nik_user, tgl_input, akun_seb ) values "+
								"('"+this.e_nb.getText()+"', '"+this.cb_id.getText()+"', '"+this.dp_d1.getDateString()+"', 1, '"+akunPajak+"', '"+this.e_ket.getText()+"', 'C', 'IDR', 1, "+nilaiToFloat(this.e_npajak.getText())+", "+nilaiToFloat(this.e_npajak.getText())+", '"+this.ppKelola+"', '-', '"+this.app._lokasi+"', 'NTF21', '"+this.c_status.getText()+"', '"+this.e_periode.getText()+"', '"+this.app._userLog+"', getdate(), '-' )");												
					}
	...
	doChange:function(sender){
		if (sender == this.cb_nik && this.cb_nik.getText()!="" && this.stsSimpan==1) {	
			this.cb_panjar.setSQL("select a.no_aju,a.keterangan from it_aju_m a "+
			                      ...
			                      "where a.progress='3' and a.nik_panjar ='"+this.cb_nik.getText()+"' and a.form='NTF20' and a.modul = 'PANJAR' and a.no_ptg='-' and a.kode_lokasi='"+this.app._lokasi+"'",
			                      ["a.no_aju","a.keterangan"],false,["No Bukti","Keterangan"],"and","Data Panjar",true);
		}
		if (sender == this.cb_panjar && this.cb_panjar.getText()!="") {	
			var strSQL = "select convert(varchar,c.tgl_admin,103) as tgl_admin,a.nilai-isnull(e.totptg,0) as nilai,a.dasar as kode_pp,b.kode_proyek,a.kode_akun,c.nama,d.akun_bdd "+
						 "from it_aju_m a inner join prb_prbeban_d b on a.no_aju=b.no_bukti and a.kode_lokasi=b.kode_lokasi "+
						 "                inner join prb_proyek c on b.kode_proyek=c.kode_proyek and b.kode_lokasi=c.kode_lokasi and c.versi = 'NTF21'  "+						
						 ...
						 "				   		from it_aju_m where form='NTF21' and kode_lokasi='"+this.app._lokasi+"' and no_aju <>'"+this.e_nb.getText()+"' "+
						 "				   		group by	no_ptg,kode_lokasi "+
						 "                ) e on a.no_aju=e.no_pj and a.kode_lokasi=e.kode_lokasi "+					
			...
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){			
					this.cb_akun.setSQL("select kode_akun, nama from masakun where kode_akun='"+line.akun_bdd+"' and kode_lokasi='"+this.app._lokasi+"' ",["kode_akun","nama"],false,["Kode","Nama"],"and","Data Akun",true);									
			...
			var strSQL = "select a.kode_pp, a.nama,a.nilai_or - isnull(c.beban,0) as saldo_or "+
			             "from prb_proyek a "+			             
						 ...
						 "where a.versi = 'NTF21' and a.kode_proyek = '"+this.cb_id.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
		...
		if (sender == this.e_nilaipj || sender == this.e_nilai || sender == this.e_npajak || sender == this.c_stsptg) {
			if (this.e_nilaipj.getText()!="" && this.e_nilai.getText()!="" && this.e_npajak.getText()!="") {
				...
				if (this.c_stsptg.getText()=="CLOSE") this.e_nilaikb.setText(floatToNilai(nilaiToFloat(this.e_nilaipj.getText()) - nilaiToFloat(this.e_nilai.getText()) + nilaiToFloat(this.e_npajak.getText()) ));
				else this.e_nilaikb.setText("0");
			}
		}	
	...
	doLoad3:function(sender){	
		var strSQL = "select a.no_aju,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
					 ...
					 "where a.form='NTF21' and a.modul='PJPTG' and a.progress in ('0','A','R') "+
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.progress,a.modul,a.tanggal,a.periode,a.keterangan,a.nilai+a.npajak as nilai,a.dasar as kode_pp,a.nik_panjar,a.kode_akun,c.no_aju as no_pj,c.keterangan as ket_pj,c.kode_akun as akun_pj,a.kode_drk,  "+
							 "d.kode_proyek,d.nama as nama_proyek, a.sts_pajak,a.npajak,a.status_if "+						     
							 "from it_aju_m a "+
							 "		inner join it_aju_m c on c.no_aju=a.no_ptg and a.kode_lokasi=c.kode_lokasi "+
							 "      inner join prb_prbeban_d cc on c.no_aju=cc.no_bukti and c.kode_lokasi=cc.kode_lokasi and cc.modul='AJUBEBAN' "+
						     "   	inner join prb_proyek d on cc.kode_proyek=d.kode_proyek and cc.kode_lokasi=d.kode_lokasi and d.versi = 'NTF21' "+						     				   
				...
				if (typeof data == "object"){
					...
					if (line != undefined){
						...
						this.c_stsptg.setText(line.status_if);
