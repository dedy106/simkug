window.app_saku3_transaksi_tu_ntf21_fProyekNonAktif = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_ntf21_fProyekNonAktif.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_ntf21_fProyekNonAktif";
		this.itemsValue = new arrayList();
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Non Aktivasi Proyek", 0);	
		...
		this.c_status = new saiCB(this.pc2.childPage[1],{bound:[20,11,200,20],caption:"Status",items:["NONAKTIF"], readOnly:true,tag:2});		
		this.cb_app = new portalui_saiCBBL(this.pc2.childPage[1],{bound:[20,14,220,20],caption:"NIK Approve",tag:2,multiSelection:false});
		this.e_memo = new saiMemo(this.pc2.childPage[1],{bound:[20,10,450,40],caption:"Catatan Aktifasi",tag:9});		
		...
		this.c_status2 = new saiCB(this.pc2.childPage[2],{bound:[20,10,200,20],caption:"Status",items:["NONAKTIF"], readOnly:true,tag:2});
...
window.app_saku3_transaksi_tu_ntf21_fProyekNonAktif.extend(window.childForm);
window.app_saku3_transaksi_tu_ntf21_fProyekNonAktif.implement({	
	...
	simpan: function(){			
		try{		
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"prb_proyek_app","no_app",this.app._lokasi+"-NON"+this.e_periode.getText().substr(2,4)+".","0000"));				
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("update prb_proyek_app set no_appseb = '"+this.e_nb.getText()+"' where no_appseb ='-' and no_bukti='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='NONAKTIF_PR'");
					sql.add("insert into prb_proyek_app (no_app,tanggal,kode_lokasi,periode,nik_user,tgl_input,status,no_bukti,modul,no_appseb,catatan,no_rab,nik_app) values "+
							"('"+this.e_nb.getText()+"','"+this.dp_d3.getDateString()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.cb_app.getText()+"',getdate(),'"+this.c_status.getText()+"','"+this.cb_kode.getText()+"','NONAKTIF_PR','-','"+this.e_memo.getText()+"','"+this.e_rab.getText()+"','"+this.cb_app.getText()+"')");
					
					sql.add("update prb_proyek set progress='"+prog+"',no_nonaktif='"+this.e_nb.getText()+"', tgl_nonapp='"+this.dp_d3.getDateString()+"' where kode_proyek ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					
					//jika ada bdd reklas ke beban semua
					//beban pd bdd
					sql.add("insert into prb_prbdd_d(no_bukti,kode_lokasi,periode,tanggal,kode_akun,kode_pp,keterangan,dc,nilai,tgl_input,nik_user,kode_proyek,form)  "+
							"select '"+this.e_nb.getText()+"',kode_lokasi,'"+this.e_periode.getText()+"','"+this.dp_d3.getDateString()+"',kode_akun,'"+this.cb_pp.getText()+"','"+this.e_memo.getText()+"',case when sum(case dc when 'D' then nilai else -nilai end) > 0 then 'C' else 'D' end,abs(sum(case dc when 'D' then nilai else -nilai end)),getdate(),'"+this.app._userLog+"',kode_proyek,'NONAKTIF_PR' "+
							"from prb_prbdd_d "+
					...
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) "+
							"select no_bukti,kode_lokasi,tgl_input,nik_user,periode,kode_proyek,tanggal,0,'"+this.akunBeban+"',case dc when 'C' then 'D' else 'C' end,nilai,nilai,keterangan,'NONAKTIF_PR','BEBAN','IDR',1,kode_pp,'"+this.drkBeban+"','-','-','-','-','-','-','-' "+
							"from prb_prbdd_d where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");

					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) "+
							"select no_bukti,kode_lokasi,tgl_input,nik_user,periode,kode_proyek,tanggal,1,kode_akun,dc,nilai,nilai,keterangan,'NONAKTIF_PR','BDD','IDR',1,kode_pp,'-','-','-','-','-','-','-','-' "+
							"from prb_prbdd_d where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3)  "+
							"select no_bukti,kode_lokasi,tgl_input,nik_user,periode,'AR',modul,'F','0','0',kode_pp,tanggal,no_dokumen,keterangan,'IDR',1,nilai,0,0,'-','-','-','-','-','-','-','-','-' "+
							"from trans_j where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and jenis ='BEBAN' ");		
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "hapus" :	
				...
				var prog = "1";
				var strSQL = "select isnull(count(*),0) as jumlah from prb_prbill_m where kode_proyek = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";										
				...
				sql.add("delete from prb_proyek_app where no_nonaktif='"+this.noAppLama+"' and kode_lokasi='"+this.app._lokasi+"'");
				sql.add("update prb_proyek set progress='"+prog+"',no_nonaktif='-' where no_nonaktif='"+this.noAppLama+"' and kode_lokasi='"+this.app._lokasi+"'");								
				setTipeButton(tbAllFalse);	
				this.dbLib.execArraySQL(sql);			
				break;			
		}
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select a.*,d.akun_beban,d.kode_drkb, x.no_rab,isnull(b.nik_user,'"+this.app._userLog+"') as nik_app "+
							 "from prb_proyek a "+
							 ...
							 "left join prb_proyek_app b on x.no_rab=b.no_rab and x.kode_lokasi=b.kode_lokasi and b.no_appseb='-' and b.modul='AKTIF_PR' "+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){			
						...
						this.noAppLama = line.no_nonaktif;		
	...
	doLoad3:function(sender){		
		...
		var strSQL = "select a.kode_proyek,b.nama,a.no_pks,a.nama as keterangan,a.nilai "+
					 ...
					 "where a.versi = 'NTF21' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress in ('1','2') order by a.kode_proyek";	
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doCari:function(sender){	
		...
		if (this.c_status2.getText() == "APPROVE") filter = " and a.progress = 'Z' ";
