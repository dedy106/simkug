window.app_saku3_transaksi_tu_ntf21_fDokJenis = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_ntf21_fDokJenis.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_ntf21_fDokJenis";
...
window.app_saku3_transaksi_tu_ntf21_fDokJenis.extend(window.childForm);
window.app_saku3_transaksi_tu_ntf21_fDokJenis.implement({
	doTambah: function() {		
		this.stsSimpan = 1;
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into prb_dok_jenis(kode_jenis,nama,kode_lokasi) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();					
					sql.add("delete from prb_dok_jenis where kode_jenis = '"+this.cb_kode.getText()+"'");
					sql.add("insert into prb_dok_jenis(kode_jenis,nama,kode_lokasi) values "+
							"('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"')");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from prb_dok_jenis where kode_jenis = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");								
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select * from prb_dok_jenis where kode_jenis ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
	...
	doCari: function (sender) {
		try {
			...
			var strSQL = "select kode_jenis, nama from prb_dok_jenis " +
