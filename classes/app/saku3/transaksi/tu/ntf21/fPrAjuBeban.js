window.app_saku3_transaksi_tu_ntf21_fPrAjuBeban = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_ntf21_fPrAjuBeban.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_ntf21_fPrAjuBeban";
		...
		this.e_tgladm = new saiLabelEdit(this.pc2.childPage[0],{bound:[370,14,200,20],caption:"TglMax Adminstrsi", tag:1, readOnly:true});				
		...
		this.e_nsaldo = new saiLabelEdit(this.pc2.childPage[0],{bound:[370,17,200,20],caption:"Saldo OR", tag:1, tipeText:ttNilai, text:"0",readOnly:true});				
		...
		this.e_netto = new saiLabelEdit(this.pc2.childPage[0],{bound:[370,12,200,20],caption:"Total Netto", tag:0, tipeText:ttNilai, text:"0" , readOnly:true});		
		this.e_ket = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,16,550,20],caption:"Uraian", maxLength:150});				

		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,33,996,213], childPage:["Rekening Penerima","User & Verifikasi"]});		
...
window.app_saku3_transaksi_tu_ntf21_fPrAjuBeban.extend(window.childForm);
window.app_saku3_transaksi_tu_ntf21_fPrAjuBeban.implement({
	...
	doChange:function(sender){				
		if ((sender==this.cb_pp || this.e_periode.getText()) && this.cb_pp.getText()!="" && this.e_periode.getText()!="" && this.stsSimpan == 1) {						
			//and substring(convert(varchar,tgl_mulai,112),1,6) <= '"+this.e_periode.getText()+"' --> rudi 17-9-21
			this.cb_id.setSQL("select kode_proyek, nama from prb_proyek where versi = 'NTF21' and progress in ('1','2') and modul='PROYEK' and pp_rab='"+this.cb_pp.getText()+"' and flag_aktif='1' and kode_lokasi='"+this.app._lokasi+"'",["kode_proyek","nama"],false,["ID Proyek","Keterangan"],"and","Data Proyek",true);												
		}
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {					
						...
						sql.add("delete from angg_r where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='NTF19'");
					}					
					...
					sql.add("insert into it_aju_d (no_aju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,akun_seb) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','"+this.cb_id.getText()+" | "+this.e_ket.getText()+"','D','IDR',1,"+nilaiBeban+","+nilaiBeban+",'"+this.ppKelola+"','-','"+this.app._lokasi+"','NTF19','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'-')");		

					if (this.c_status.getText() != "NON") {
						if (this.c_status.getText() == "P21") var akunPajak = this.akunPPH21;
						else var akunPajak = this.akunPPH23;
						sql.add("insert into it_aju_d (no_aju, no_dokumen, tanggal, no_urut, kode_akun, keterangan, dc, kode_curr, kurs, nilai_curr, nilai, kode_pp, kode_drk, kode_lokasi, modul, jenis, periode, nik_user, tgl_input, akun_seb ) values "+
								"('"+this.e_nb.getText()+"', '"+this.cb_id.getText()+"', '"+this.dp_d1.getDateString()+"', 1, '"+akunPajak+"', '"+this.e_ket.getText()+"', 'C', 'IDR', 1, "+nilaiToFloat(this.e_npajak.getText())+", "+nilaiToFloat(this.e_npajak.getText())+", '"+this.ppKelola+"', '-', '"+this.app._lokasi+"', 'NTF19', '"+this.c_status.getText()+"', '"+this.e_periode.getText()+"', '"+this.app._userLog+"', getdate(), '-' )");												
					}
					...
					sql.add("insert into prb_prbdd_d(no_bukti,kode_lokasi,periode,tanggal,kode_akun,kode_pp,keterangan,dc,nilai,tgl_input,nik_user,kode_proyek,form) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_akun.getText()+"','"+this.ppKelola+"','"+this.e_ket.getText()+"','D',"+nilaiBeban+",getdate(),'"+this.app._userLog+"','"+this.cb_id.getText()+"','NTF19')");
					sql.add("insert into prb_prbeban_d(no_bukti,kode_lokasi,periode,periode_sch,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1,jenis) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_akun.getText()+"','"+this.ppKelola+"','-','"+this.cb_id.getText()+" | "+this.e_ket.getText()+"','D',"+nilaiBeban+",getdate(),'"+this.cb_id.getText()+"','AJUBEBAN','-','ITAJU')");												
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
							"('"+this.e_nb.getText()+"','NTF19','"+this.app._lokasi+"','"+this.cb_akun.getText()+"','"+this.ppKelola+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','D',0,"+nilaiBeban+")");
	...
	doLoad3:function(sender){		
		var strSQL = "select distinct a.no_aju,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai,a.no_ref1 "+
		             ...
		             "where  a.form = 'NTF19' and a.modul='UMUM' and a.progress in ('0','A','R') and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and b.nik='"+this.app._userLog+"'";	
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select distinct aa.kode_proyek,a.progress,a.modul,a.tanggal,a.periode,substring(a.keterangan,18,200) as keterangan,a.nilai+a.npajak as bruto,a.npajak,a.sts_pajak,a.dasar as kode_pp,a.kode_akun,a.kode_drk,"+
							...
							"from it_aju_m a "+
							"				inner join prb_prbeban_d aa on a.no_aju=aa.no_bukti and a.kode_lokasi=aa.kode_lokasi and aa.modul in ('AJUBEBAN') "+						     
