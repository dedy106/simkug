window.app_saku3_transaksi_tu_ntf21_fTagihAjuDel = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_ntf21_fTagihAjuDel.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_ntf21_fTagihAjuDel";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Pembatalan Tagihan Proyek", 0);	
		
		uses("saiCBBL;saiEdit;saiGrid;sgNavigator");
		this.e_periode = new portalui_saiLabelEdit(this,{bound:[520,12,200,20],caption:"Periode",tag:2,readOnly:true,visible:false});		
		this.l_tgl3 = new portalui_label(this,{bound:[20,12,100,18],caption:"Tanggal", underline:true});
		this.dp_d3 = new portalui_datePicker(this,{bound:[120,12,98,18],selectDate:[this,"doSelectDate"]});		
		this.cb_kode = new portalui_saiCBBL(this,{bound:[20,11,220,20],caption:"ID Proyek", multiSelection:false, change:[this,"doChange"]});			

		this.pc2 = new pageControl(this,{bound:[10,10,1000,410], childPage:["Data Pembatalan","Data Bill","Filter Cari"]});				
		...
		this.e_nb = new saiLabelEdit(this.pc2.childPage[1],{bound:[20,11,200,20],caption:"No Bukti", readOnly:true,visible:false});				
		this.c_status = new saiCB(this.pc2.childPage[1],{bound:[20,11,200,20],caption:"Status",items:["BATAL"], readOnly:true,tag:2});		
		this.cb_app = new portalui_saiCBBL(this.pc2.childPage[1],{bound:[20,14,220,20],caption:"NIK Approve",tag:2,multiSelection:false});
		this.e_memo = new saiMemo(this.pc2.childPage[1],{bound:[20,10,450,40],caption:"Cat. Pembatalan",tag:9});		
		
		this.pc1 = new pageControl(this.pc2.childPage[1],{bound:[1,20,996,286], childPage:["Data Proyek","Detail Tagihan","File Dokumen","Cattn Memo"]});
		this.cb_pp = new portalui_saiCBBL(this.pc1.childPage[0],{bound:[20,13,220,20],caption:"Bagian / Unit",tag:2,readOnly:true,change:[this,"doChange"]}); 							
		//this.cb_kode = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,11,200,20],caption:"ID Proyek", readOnly:true});									
		this.e_rab = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,11,200,20],caption:"No RAB",readOnly:true});							
		...
		this.e_cust = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,13,430,20],caption:"Customer", maxLength:200, tag:1,readOnly:true});				
		...
		this.e_total = new saiLabelEdit(this.pc1.childPage[0],{bound:[250,19,200,20],caption:"Total Tagihan", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});						
		...
		this.c_status2 = new saiCB(this.pc2.childPage[2],{bound:[20,10,200,20],caption:"Status",items:["BATAL"], readOnly:true,tag:2});
		this.cb_batal = new portalui_saiCBBL(this.pc2.childPage[2],{bound:[20,12,220,20],caption:"No Bukti",tag:9,multiSelection:false}); 				
...
window.app_saku3_transaksi_tu_ntf21_fTagihAjuDel.extend(window.childForm);
window.app_saku3_transaksi_tu_ntf21_fTagihAjuDel.implement({	
	...
	simpan: function(){			
		try{		
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"prb_bill_app","no_app",this.app._lokasi+"-BTB"+this.e_periode.getText().substr(2,4)+".","0000"));
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("update prb_bill_app set no_appseb='"+this.e_nb.getText()+"' where no_bukti ='"+this.e_bill.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and no_appseb='-' and modul='BTLBILL'");
					sql.add("update prb_prbill_m set progress='"+prog+"',no_batal='"+this.e_nb.getText()+"' where no_bill ='"+this.e_bill.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("insert into prb_bill_app (no_app,tanggal,kode_lokasi,periode,nik_user,tgl_input,status,no_bukti,modul,no_appseb,catatan,no_bill,nik_app) values "+
							"('"+this.e_nb.getText()+"','"+this.dp_d3.getDateString()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.progSeb+"','"+this.e_bill.getText()+"','BTLBILL','-','"+this.e_memo.getText()+"','"+this.e_bill.getText()+"','"+this.cb_app.getText()+"')");
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "hapus" :	
				...
				sql.add("update set a.progress=b.status "+
						"from prb_prbill_m  a inner join prb_bill_app b on a.no_bill=b.no_bukti and a.kode_lokasi=b.kode_lokasi and b.modul='BTLBILL' "+
						"where a.no_bill ='"+this.e_bill.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'");
	...
	doSelectDate: function(sender, y,m,d){
		try{
			...
			this.cb_batal.setSQL("select a.no_app,a.no_bukti from prb_bill_app a inner join prb_prbill_m b on a.no_app=b.no_batal and a.kode_lokasi=b.kode_lokasi where b.kode_pp='"+this.app._kodePP+"' and a.modul = 'BTLBILL' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",["no_app","no_bukti"],false,["No Bukti","No Bill"],"and","Data Billing",true);
			if (this.stsSimpan==1) {
				this.doLoad3();
				this.doClick();					
			}
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_pp && this.cb_pp.getText()!="") {
				if (this.app._kodePP == "2103") 
					var strSQL = "select kode_proyek,nama from prb_proyek where versi = 'NTF21' and substring(convert(varchar,tgl_mulai,112),1,6) <= '"+this.periode+"' and progress = '2' and kode_lokasi='"+this.app._lokasi+"'";
				else 
					var strSQL = "select kode_proyek,nama from prb_proyek where versi = 'NTF21' and substring(convert(varchar,tgl_mulai,112),1,6) <= '"+this.periode+"' and progress = '2' and kode_lokasi='"+this.app._lokasi+"' and pp_rab='"+this.cb_pp.getText()+"'";			
			...
			if (sender == this.e_bill && this.e_bill.getText() != ""){
				var strSQL = "select a.*, c.jenis_pph42, x.no_rab,d.no_bill,convert(varchar,d.tanggal,103) as tgl_bill,d.keterangan as ket_bil,d.nilai as nilai_bil,d.diskon,d.nilai_ppn as ppn_bil,d.no_ver, d.progress "+
							 "from prb_proyek a "+
	...
	doLoad3:function(sender){		
		...
		var strSQL = "select c.no_bill,convert(varchar,c.tanggal,103) as tgl,c.keterangan,a.kode_proyek,b.nama,c.nilai,c.nilai_ppn "+
					 ...
					 "where c.kode_lokasi='"+this.app._lokasi+"' and c.kode_proyek='"+this.cb_kode.getText()+"' and c.progress <> 'J' order by c.no_bill";					 	
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doCari:function(sender){	
		...
		var strSQL = "select c.no_bill,convert(varchar,c.tanggal,103) as tgl,c.keterangan,a.kode_proyek,b.nama,c.nilai,c.nilai_ppn "+
					 ...
					 "inner join prb_bill_app f on c.no_bill=f.no_bukti and c.kode_lokasi=f.kode_lokasi "+
					 "where c.kode_lokasi='"+this.app._lokasi+"' and f.no_app='"+this.cb_batal.getText()+"' "+filter;	
