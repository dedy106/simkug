...
*/
window.app_saku3_transaksi_tu_ntf21_fIniAjuPj = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_ntf21_fIniAjuPj.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_ntf21_fIniAjuPj";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan Panjar Inisiasi", 0);	
		...
		this.cb_id = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,13,220,20],caption:"ID Inisiasi",tag:1,multiSelection:false,change:[this,"doChange"]}); 						
		...
		this.e_norek = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,18,550,20],caption:"No Rekening", maxLength:50, tipeText:ttAngka});
...
window.app_saku3_transaksi_tu_ntf21_fIniAjuPj.extend(window.childForm);
window.app_saku3_transaksi_tu_ntf21_fIniAjuPj.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,user_input,form,sts_pajak,npajak) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.c_jenis.getText()+"','"+this.akunPJ+"','"+this.cb_pp.getText()+"','-','"+this.cb_id.getText()+"|"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",getdate(),'"+this.app._userLog+"','-','-','-','A','"+this.cb_nik.getText()+"','-','"+this.e_user.getText()+"','NTF06','NON',0)");					
					...
					sql.add("insert into it_aju_d (no_aju, no_dokumen, tanggal, no_urut, kode_akun, keterangan, dc, kode_curr, kurs, nilai_curr, nilai, kode_pp, kode_drk, kode_lokasi, modul, jenis, periode, nik_user, tgl_input, akun_seb ) values "+
							"('"+this.e_nb.getText()+"', '"+this.cb_id.getText()+"', '"+this.dp_d1.getDateString()+"', 0, '"+this.akunPJ+"', '"+this.e_ket.getText()+"', 'D', 'IDR', 1, "+nilaiToFloat(this.e_nilai.getText())+", "+nilaiToFloat(this.e_nilai.getText())+", '"+this.cb_pp.getText()+"', '-', '"+this.app._lokasi+"', 'NTF06', 'PJ', '"+this.e_periode.getText()+"', '"+this.app._userLog+"', getdate(), '-' )");
					...
					sql.add("insert into prb_prbeban_d(no_bukti,kode_lokasi,periode,periode_sch,tanggal,kode_akun,kode_pp,kode_drk,keterangan,dc,nilai,tgl_input,kode_proyek,modul,no_ref1,jenis) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.akunPJ+"','"+this.cb_pp.getText()+"','-','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_nilai.getText())+",getdate(),'"+this.cb_id.getText()+"','AJUBEBAN','-','ITAJU')");
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :
				...
				if (d2 < d1) {												
					system.alert(this,"Tanggal pengajuan tidak valid.","Tanggal Awal Administrasi = '"+this.tglMulai+"'");
					return false;
				}
	...
	doChange:function(sender){
		if (sender==this.cb_nik && this.cb_nik.getText()!="" && this.stsSimpan == 1) {	
			var strSQL = "select no_rek,nama_rek,bank+' - '+cabang as bank from karyawan where nik = '"+this.cb_nik.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						 			
		...
		if (sender==this.cb_pp && this.cb_pp.getText()!="" && this.stsSimpan == 1) {			
			this.cb_id.setText("","");
			this.cb_id.setSQL("select kode_proyek, nama from prb_proyek where versi='NTF21' and modul='INISIASI' and kode_pp='"+this.cb_pp.getText()+"' and flag_aktif='1' and kode_lokasi='"+this.app._lokasi+"'",["kode_proyek","nama"],false,["ID Kegiatan","Keterangan"],"and","Data Inisiasi",true);						
			this.e_nsaldo.setText("0");							
			this.e_uraian.setText("");				
		}		
		if (sender==this.cb_id && this.cb_id.getText()!="") {
			//inisiasi semua masuk sbg beban tidak ada BDD krn tidak ada schedule
			var strSQL = "select a.nama,a.nilai_or - isnull(c.beban,0) as saldo, convert(varchar,a.tgl_mulai,103) as tgl_mulai, convert(varchar,a.tgl_admin,103) as tgl_admin  "+
						 ...
						 "				from prb_prbeban_d where modul='AJUBEBAN' and kode_lokasi='"+this.app._lokasi+"' "+
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){	
							if (this.preView == "1") {						
								this.nama_report="server_report_saku3_tu_proyek_rptPanjar";
	...
	doLoad3:function(sender){		
		var strSQL = "select a.no_aju,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
		             ...
		             "where a.form='NTF06' and a.progress in ('0','A','R') and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and b.nik='"+this.app._userLog+"'";	
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.nik_panjar,aa.kode_proyek,a.progress,a.modul,a.tanggal,a.periode,a.keterangan,a.nilai,a.kode_pp,a.user_input, "+
