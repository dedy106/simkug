window.app_saku3_transaksi_tu_ntf21_fProyek = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tu_ntf21_fProyek.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tu_ntf21_fProyek";
		...
		this.cb_rab = new saiLabelEdit(this.pc2.childPage[0],{bound:[520,11,220,20],caption:"No RAB [Verified]",tag:9,visible:false,change:[this,"doChange"]}); 				
		
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,20,996,394], childPage:["Data RAB","Data Proyek","Distribusi Akru","File Dokumen","Detail RAB","Inisiasi Unit","Cattn Memo","Customer"]});				
		this.sg5 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:7,tag:9,
				...
				colWidth:[[6,5,4,3,2,1,0],[70,100,150,280,150,100,100]],
				readOnly:true,
		...
		this.cb_pp = new portalui_saiCBBL(this.pc1.childPage[1],{bound:[20,13,220,20],caption:"PP Proyek",tag:2, readOnly:true, change:[this,"doChange"]}); 					
		this.e_dok = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,10,430,20],caption:"No Kontrak", maxLength:50, tag:1});			
		this.e_bank = new saiLabelEdit(this.pc1.childPage[1],{bound:[520,10,430,20],caption:"Bank VA", maxLength:100, tag:1});							
		this.e_nama = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,11,430,20],caption:"Deskripsi", maxLength:200, tag:1});			
		this.e_namarek = new saiLabelEdit(this.pc1.childPage[1],{bound:[520,11,430,20],caption:"Nama Rekening", maxLength:100, tag:1});									
		...
		this.e_norek = new saiLabelEdit(this.pc1.childPage[1],{bound:[520,13,430,20],caption:"No Rekening", maxLength:50, tag:1, tipeText:ttAngka});							
		...
		this.e_join = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,13,200,20],caption:"Ni JoinCost", tag:1, tipeText:ttNilai, text:"0",readOnly:true});			
		...
		this.cb_cust = new portalui_saiCBBL(this.pc1.childPage[7],{bound:[20,14,220,20],caption:"Kode Cust",tag:2,readOnly:true});
		this.e_namacust = new saiLabelEdit(this.pc1.childPage[7],{bound:[20,11,500,20],caption:"Nama Customer", maxLength:50, tag:1});	
		this.e_alamat = new saiLabelEdit(this.pc1.childPage[7],{bound:[20,12,500,20],caption:"Alamat", maxLength:150, tag:1});					
		this.e_tel = new saiLabelEdit(this.pc1.childPage[7],{bound:[20,13,500,20],caption:"No Telpon", maxLength:50, tag:1});							
		this.e_mail = new saiLabelEdit(this.pc1.childPage[7],{bound:[20,15,500,20],caption:"Email", maxLength:50, tag:1});	
		this.e_npwp = new saiLabelEdit(this.pc1.childPage[7],{bound:[20,16,500,20],caption:"NPWP", maxLength:50, tag:1});	
		this.e_alamat2 = new saiLabelEdit(this.pc1.childPage[7],{bound:[20,17,500,20],caption:"Alamat NPWP", maxLength:150, tag:1});	
		this.e_pic = new saiLabelEdit(this.pc1.childPage[7],{bound:[20,18,500,20],caption:"P I C", maxLength:50, tag:1});			
		this.e_jabatan = new saiLabelEdit(this.pc1.childPage[7],{bound:[20,19,500,20],caption:"Jabatan", maxLength:50, tag:1});		
		this.cb_piu = new portalui_saiCBBL(this.pc1.childPage[7],{bound:[20,14,220,20],caption:"Akun Piutang",tag:2,multiSelection:false});				

		this.rearrangeChild(10, 23);
		this.pc2.childPage[0].rearrangeChild(10, 23);
		this.pc1.childPage[1].rearrangeChild(10, 23);	
		this.pc1.childPage[7].rearrangeChild(10, 23);	
		...
		try {
			...
			this.cb_jenis.setSQL("select kode_jenis,nama from prb_proyek_jenis where flag_aktif='1' and kode_lokasi='"+this.app._lokasi+"'",["kode_jenis","nama"],false,["Kode","Nama"],"and","Data Jenis",true);
...
window.app_saku3_transaksi_tu_ntf21_fProyek.extend(window.childForm);
window.app_saku3_transaksi_tu_ntf21_fProyek.implement({	
				...
				if (typeof data == "object"){
					...
					if (line != undefined){		
						...
						var strSQL = "select a.no_bukti,a.keterangan,a.kode_proyek,d.nama,a.nilai as ni_agenda, isnull(c.joincost,0) as joincost, a.nilai - isnull(c.joincost,0) as sisa_agenda,isnull(e.nilai_join,0) as nilai_join "+
									...
									"inner join prb_proyek d on a.kode_proyek=d.kode_proyek and a.kode_lokasi=d.kode_lokasi and d.versi='NTF21' "+
									...
									"where d.kode_pp ='"+this.ppRAB+"' and b.form in ('NTF05','NTF07') and dc='D' and a.kode_lokasi='"+this.app._lokasi+"' "+
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					if (this.c_status.getText() == "APPROVE") {						
						...
						if (this.noAmandemen != "-") {
							sql.add("delete from prb_proyek where kode_proyek='"+this.noAmandemen+"' and kode_lokasi='"+this.app._lokasi+"' ");
							sql.add("delete from prb_prbeban_d where no_bukti='"+this.noAmandemen+"' and kode_lokasi='"+this.app._lokasi+"' ");
							sql.add("delete from it_aju_m where no_aju='"+this.noAmandemen+"' and kode_lokasi='"+this.app._lokasi+"' ");
							sql.add("delete from kas_m where no_kas='"+this.noAmandemen+"' and kode_lokasi='"+this.app._lokasi+"' ");
						}
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try {									
					...
					//alokasi OR beban pph dihapus
					sql.add("delete from prb_prbeban_d where no_bukti='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");	
					sql.add("delete from it_aju_m where no_aju='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");	
					sql.add("delete from kas_m where no_kas='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");	

					//prb_prbdd_d tidak dihapus sebagai historis reklas inisiasi sblmnya
					//......
	...
	doClick:function(sender){		
		try {
			...
			this.e_noapp.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"prb_proyek_app","no_app",this.app._lokasi+"-AP"+this.periode.substr(2,4)+".","0000"));				
			if (this.noAmandemen == "-") this.cb_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"prb_proyek","kode_proyek",this.cb_pp.getText()+"-"+this.periode.substr(2,4)+".","0000"));						
			else this.cb_kode.setText(this.noAmandemen);
			this.e_dok.setFocus();	
			this.stsSimpan = 1;		
		}
		catch(e) {
	...
	doChange: function(sender){
		try{			
			...
			if (sender == this.cb_rab && this.cb_rab.getText()!="") {	
				...
				var strSQL = "select kode_proyek from prb_proyek "+
							 "where progress='Y' and no_rab='"+this.cb_rab.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   											 
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){		
						this.noAmandemen = line.kode_proyek;
						this.cb_kode.setText(this.noAmandemen);

						var data = this.dbLib.getDataProvider("select * from prb_proyek_d where kode_proyek = '"+this.noAmandemen+"' and kode_lokasi='"+this.app._lokasi+"' order by nu",true);
						if (typeof data == "object" && data.rs.rows[0] != undefined){
							var line;
							this.sg.clear();
							for (var i in data.rs.rows){
	...
	doLoad3:function(sender){																								
		var strSQL = "select a.no_app,convert(varchar,a.tanggal,103) as tanggal,a.no_bukti "+
					 ...
					 "left join ( "+
					 "		select x.no_appntf,x.kode_lokasi "+
					 "		from prb_rabapp_m x "+
					 "		inner join prb_proyek y on x.kode_proyek=y.kode_proyek and x.kode_lokasi=y.kode_lokasi and y.versi = 'NTF21' "+
					 "		where x.kode_lokasi='"+this.app._lokasi+"' and y.progress not in ('0','R') "+
