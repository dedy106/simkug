window.app_saku3_transaksi_rkm_fRektorat = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_rkm_fRektorat.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_rkm_fRektorat";
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,400], childPage:["Daftar Rektorat","Data Rektorat","Filter Cari"]});				
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:3,tag:9,
		            colTitle:["Kode Rektorat","Nama Rektorat","Flag Aktif"],
...
window.app_saku3_transaksi_rkm_fRektorat.extend(window.childForm);
window.app_saku3_transaksi_rkm_fRektorat.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into rkm_rektorat(kode_rek, kode_lokasi, nama, flag_aktif) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update rkm_rektorat set nama='"+this.e_nama.getText()+"', flag_aktif ='"+this.c_status.getText().substr(0,1)+"' "+
					        "where kode_rek = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from rkm_rektorat where kode_rek = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");			
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
	...
	doChange: function(sender){
		try{			
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select kode_rek,nama,flag_aktif from rkm_rektorat "+
						     "where kode_rek ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
	...
	doCari:function(sender){								
		...
		var strSQL = "select a.kode_rek,a.nama,a.flag_aktif "+
		             "from rkm_rektorat a "+
					 "where "+filter+" a.kode_lokasi='"+this.app._lokasi+"' order by a.kode_rek";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_rek,line.nama,line.flag_aktif]); 
		}
