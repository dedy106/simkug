window.app_saku3_transaksi_bangtel_kug_fGarDir = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bangtel_kug_fGarDir.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bangtel_kug_fGarDir";
...
window.app_saku3_transaksi_bangtel_kug_fGarDir.extend(window.childForm);
window.app_saku3_transaksi_bangtel_kug_fGarDir.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{				
					...
					if (this.c_status.getText() == "APPROVE") var vProg = "2"; 
					if (this.c_status.getText() == "NONAPP") var vProg = "Y"; 
					if (this.c_status.getText() == "RETURN") var vProg = "S"; 
					...
					sql.add("insert into rra_app_m(no_app, kode_lokasi,tanggal,keterangan,modul,periode,no_del,nik_buat,nik_app,nik_user,tgl_input,jenis_form) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','APPDIR','"+this.e_periode.getText()+"','-','"+this.app._userLog+"','"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'APPDIR')");
					sql.add("insert into rra_app_d(no_app,modul,kode_lokasi,no_bukti,kode_lokbukti,sts_pdrk,catatan,status) values "+
							"('"+this.e_nb.getText()+"','APPDIR','"+this.app._lokasi+"','"+this.e_nopdrk.getText()+"','"+this.app._lokasi+"','RRR','"+this.e_memo.getText()+"','"+this.c_status.getText()+"')");
	...
	doLoad:function(sender){		
		var strSQL = "select a.kode_pp+' - '+c.nama as pp,a.no_pdrk as no_bukti,b.no_dokumen,convert(varchar,a.tanggal,103) as tanggal,a.keterangan "+
					 ...
					 "where a.modul = 'MULTI' and a.progress='1' and a.kode_lokasi='"+this.app._lokasi+"'";							
