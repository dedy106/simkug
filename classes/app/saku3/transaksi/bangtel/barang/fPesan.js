window.app_saku3_transaksi_bangtel_barang_fPesan = function (owner) {
	if (owner) {
		window.app_saku3_transaksi_bangtel_barang_fPesan.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_bangtel_barang_fPesan";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0], { bound: [1, 15, 996, 384], childPage: ["Rekap Data", "Detail Item", "Otorisasi","File Dok"] });
		...
		this.cb_proyek = new saiCBBL(this.pc1.childPage[0], { bound: [20, 19, 220, 20], caption: "Proyek", multiSelection: false, maxLength: 10, tag: 2, change: [this, "doChange"] });
		...
		this.sg = new saiGrid(this.pc1.childPage[1], {
			...
			colTitle: ["Kode Brg", "Nama Barang", "Satuan", "Spesifikasi", "Jumlah", "Harga", "Total"],
			colWidth: [[6, 5, 4, 3, 2, 1, 0], [100, 100, 100, 280, 80, 200, 80]],
...
window.app_saku3_transaksi_bangtel_barang_fPesan.extend(window.childForm);
window.app_saku3_transaksi_bangtel_barang_fPesan.implement({
		...
		try {
			...
			if (this.standarLib.checkEmptyByTag(this, [0, 1, 2])) {
				try {
					...
					sql.add("insert into log_pesan_m(no_pesan,kode_lokasi,tgl_input,nik_user,periode,jenis,tanggal,no_dokumen,keterangan,kode_pp,kode_drk,kode_akun,saldo_gar,nilai,nik_buat,nik_app,kode_dana,no_app,progress,maksud,aspek,no_terima,no_spph,lok_proses,kode_ppaju, kode_proyek) values " +
						"('" + this.e_nb.getText() + "','" + this.app._lokasi + "',getdate(),'" + this.app._userLog + "','" + this.e_periode.getText() + "','-','" + this.dp_d1.getDateString() + "','" + this.e_dok.getText() + "','" + this.e_ket.getText() + "','" + this.cb_pp.getText() + "','-','" + this.akunBarang + "',0," + nilaiToFloat(this.e_total.getText()) + ",'" + this.cb_buat.getText() + "','" + this.cb_app.getText() + "','-','-','0','-','-','-','-','" + this.app._lokasi + "','" + this.cb_pp.getText() + "','"+this.cb_proyek.getText()+"')");
					...
					if (this.cb_proyek.getText() != "-") {
						sql.add("insert into spm_proyek_bdd(no_bukti,modul,kode_proyek,nu,keterangan,dc,nilai,kode_lokasi,periode) values "+
								"('"+this.e_nb.getText()+"','BRGPREQ','"+this.cb_proyek.getText()+"',0,'"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_total.getText())+",'"+this.app._lokasi+"','"+this.e_periode.getText()+"')");										
					}
	...
	doChange: function(sender) {
		try {
			if (sender == this.cb_pp && this.cb_pp.getText()!="") {
				this.cb_proyek.setSQL("select kode_proyek,nama from spm_proyek where kode_pp='"+this.cb_pp.getText()+"' and flag_aktif='1' and kode_lokasi='"+this.app._lokasi+"' union select '-','-' ",["kode_proyek","nama"],false,["Kode","Deskripsi"],"and","Data Proyek",true);								
			}
	...
	doChangeCell: function (sender, col, row) {
		try {
			...
			if (col == 0) {
				var strSQL = "select a.nama,a.sat_kecil "+
							 "from brg_barang a "+
					...
					case "execArraySQL":
						if (result.toLowerCase().search("error") == -1) {
							if (this.preView == "1") {
								...
								this.nama_report = "server_report_saku3_bangtel_barang_rptPesan";
		...
		try {
			if (this.sg3.cells(0, row) != "") {
				...
				var strSQL = "select a.no_urut,a.item,a.merk,a.tipe,a.catatan,a.jumlah,a.nilai,a.jumlah*a.nilai as total "+
							 "from log_pesan_d a "+
							 "where a.no_pesan='" + this.e_nb.getText() + "' and a.kode_lokasi='" + this.app._lokasi + "' order by a.no_urut";
				var data = this.dbLib.getDataProvider(strSQL, true);
				if (typeof data == "object" && data.rs.rows[0] != undefined) {
					var line2;
					this.sg.clear();
