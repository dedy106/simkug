window.app_saku3_transaksi_bangtel_pbh_fVer = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_bangtel_pbh_fVer.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bangtel_pbh_fVer";
		...
		this.pc1 = new pageControl(this,{bound:[10,18,1000,407], childPage:["Daftar Bukti","Verifikasi","Item Verifikasi","Jurnal & Budget","File Dok","Data Proyek"]});		
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:14,tag:0,
		      		colTitle:["Modul","No Bukti","Status","Tanggal","Due Date","PP","No Dokumen","Deskripsi","Nilai","Pembuat","No Verifikasi","Tgl Input","Kode PP","Data Proyek"],
		...
		this.sg2 = new saiGrid(this.pc1.childPage[3],{bound:[1,210,this.pc1.width-5,190],colCount:7,tag:9,
		...
		this.e_id = new saiLabelEdit(this.pc1.childPage[5],{bound:[20,14,200,20],caption:"ID Proyek", readOnly:true, tag:9});
		this.e_pks = new saiLabelEdit(this.pc1.childPage[5],{bound:[20,15,450,20],caption:"No Kontrak", readOnly:true, tag:9});			
		this.e_namaproyek = new saiLabelEdit(this.pc1.childPage[5],{bound:[20,16,450,20],caption:"Deskripsi", readOnly:true, tag:9});	
		this.e_tgl1 = new saiLabelEdit(this.pc1.childPage[5],{bound:[20,17,200,20],caption:"Tgl Mulai", readOnly:true, tag:9});					
		this.e_tgl2= new saiLabelEdit(this.pc1.childPage[5],{bound:[20,18,200,20],caption:"Tgl Selesai", readOnly:true, tag:9});					
...
window.app_saku3_transaksi_bangtel_pbh_fVer.extend(window.childForm);
window.app_saku3_transaksi_bangtel_pbh_fVer.implement({	
		...
		try{														
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.c_status.getText() == "RETURN") var vStatus = "V"; else var vStatus = "2";										
					...
					//---------------- flag bukti															
					if (this.e_modul.getText() == "PBBAU" || this.e_modul.getText() == "PBPR" || this.e_modul.getText() == "PBINV") {
					...
					if (this.e_modul.getText() == "PBLOG") {
						...
						if (vStatus == "2") {
							sql.add("insert into spm_app_j (no_app,no_bukti,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,nilai_curr,kode_pp,kode_drk,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) "+
									"select '"+this.e_nb.getText()+"',no_pb,'-',tanggal,0,kode_akun,keterangan,dc,nilai,nilai,kode_pp,kode_drk,'-',kode_lokasi,modul,jenis,periode,'IDR',1,nik_user,tgl_input "+
									"from yk_pb_j where no_pb = '"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						}
					}
	...
	doLoad:function(sender){	
		try {								
			var strSQL = "select a.due_date,a.no_pb as no_bukti,case a.progress when '1' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp,a.kode_proyek "+
					...
					"where a.progress in ('1') and a.kode_lokasi='"+this.app._lokasi+"'  "+					 
				
					"union all "+
				
					"select a.due_date,a.no_panjar as no_bukti,case a.progress when '1' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp,a.kode_proyek "+
					...
					"select a.tanggal as due_date,a.no_ptg as no_bukti,case a.progress when '1' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.tanggal,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp,d.kode_proyek "+
			...
			if (this.cb_pp.getText() != "") {				 
				if (this.c_modul3.getText() == "PBBAU" || this.c_modul3.getText() == "PBPR" || this.c_modul3.getText() == "PBINV" || this.c_modul3.getText() == "PBLOG")	{
					strSQL =  "select a.due_date,a.no_pb as no_bukti,case a.progress when '1' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp,a.kode_proyek "+
							"from yk_pb_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
							"               inner join karyawan c on a.nik_user=c.nik and a.kode_lokasi=c.kode_lokasi "+
							"where a.progress in ('1') and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = '"+this.c_modul3.getText()+"' and a.kode_pp='"+this.cb_pp.getText()+"' order by a.no_pb";
				}					
				if (this.c_modul3.getText() == "PJAJU" || this.c_modul3.getText() == "PJPR")	{
					strSQL =  "select a.due_date,a.no_panjar as no_bukti,case a.progress when '1' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp,a.kode_proyek "+
							"from panjar2_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
							"                inner join karyawan c on a.nik_buat=c.nik and a.kode_lokasi=c.kode_lokasi "+
							"where a.progress in ('1') and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = '"+this.c_modul3.getText()+"' and a.kode_pp='"+this.cb_pp.getText()+"' order by a.no_panjar";
				} 
				if (this.c_modul3.getText() == "PJPTG" || this.c_modul3.getText() == "PRPTG")	{
					strSQL =  "select a.tanggal as due_date,a.no_ptg as no_bukti,case a.progress when '1' then 'INPROG' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.tanggal,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp,d.kode_proyek "+
							"from panjarptg2_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
							"                    inner join karyawan c on a.nik_buat=c.nik and a.kode_lokasi=c.kode_lokasi "+
							"					 inner join panjar2_m d on a.no_panjar=d.no_panjar and a.kode_lokasi=d.kode_lokasi  "+
							"where a.progress in ('1') and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = '"+this.c_modul3.getText()+"' and a.kode_pp='"+this.cb_pp.getText()+"' order by a.no_ptg";
				} 				
			}	
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData([line.modul.toUpperCase(),line.no_bukti,line.status.toUpperCase(),line.tgl,line.tgl2,line.pp,line.no_dokumen,line.keterangan,floatToNilai(line.nilai),line.pembuat,line.no_ver,line.tglinput,line.kode_pp,line.kode_proyek]); 
		}
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){							
							if (this.preView == "1") {
								this.nama_report="server_report_saku3_bangtel_rptSpbForm";									                  
