window.app_saku3_transaksi_bangtel_pbh_fAppFin = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_bangtel_pbh_fAppFin.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bangtel_pbh_fAppFin";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Approval GM Finance", 0);	
...
window.app_saku3_transaksi_bangtel_pbh_fAppFin.extend(window.childForm);
window.app_saku3_transaksi_bangtel_pbh_fAppFin.implement({	
		...
		try{														
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.c_status.getText() == "RETURN") var vStatus = "K"; else var vStatus = "3";										
					...
					sql.add("insert into spm_app_m (no_app,kode_lokasi,tanggal,periode,tgl_input,nik_user,status,modul,form,no_bukti,catatan,no_flag,nik_bdh,nik_fiat) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_periode.getText()+"',getdate(),'"+this.app._userLog+"','"+vStatus+"','"+this.e_modul.getText()+"','APPFIN','"+this.e_nobukti.getText()+"','"+this.e_memo.getText()+"','-','X','X')");
	...
	doClick:function(sender){		
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"spm_app_m","no_app",this.app._lokasi+"-AFI"+this.e_periode.getText().substr(2,4)+".","0000"));												
	...
	doLoad:function(sender){	
		try {																																
			var strSQL = "select a.due_date,a.no_pb as no_bukti,'INPROG' as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_app2,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp,a.kode_proyek "+
						 ...
						 "where a.progress='C' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul in ('PBBAU','PBPR','PBINV','PBLOG') "+					 
						 "union "+
						 ...
						 "where a.progress='C' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul in ('PJAJU','PJPR') "+
						 "union "+
						 ...
						 "where a.progress='C' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul in ('PJPTG','PRPTG') "+
			...
			if (this.c_modul3.getText() != "" && this.cb_pp.getText() != "") {
				if (this.c_modul3.getText() == "PBBAU" || this.c_modul3.getText() == "PBPR" || this.c_modul3.getText() == "PBINV" || this.c_modul3.getText() == "PBLOG") {
					strSQL = "select a.due_date,a.no_pb as no_bukti,'INPROG' as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_app2,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp,a.kode_proyek "+
							"from yk_pb_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
							"               inner join karyawan c on a.nik_user=c.nik and a.kode_lokasi=c.kode_lokasi "+
							"where a.progress='C' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = '"+this.c_modul3.getText()+"' and a.kode_pp='"+this.cb_pp.getText()+"' ";				
				} 				
