window.app_saku3_transaksi_bangtel_pbh_fAppMFin = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_bangtel_pbh_fAppMFin.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bangtel_pbh_fAppMFin";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Approval Manager Finance", 0);	
...
window.app_saku3_transaksi_bangtel_pbh_fAppMFin.extend(window.childForm);
window.app_saku3_transaksi_bangtel_pbh_fAppMFin.implement({	
		...
		try{														
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.c_status.getText() == "RETURN") var vStatus = "Y"; else var vStatus = "C";										
					sql.add("update spm_app_m set no_flag='"+this.e_nb.getText()+"' where no_bukti='"+this.e_nobukti.getText()+"' and no_flag='-' and form='APPFIN' and modul='"+this.e_modul.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add("insert into spm_app_m (no_app,kode_lokasi,tanggal,periode,tgl_input,nik_user,status,modul,form,no_bukti,catatan,no_flag,nik_bdh,nik_fiat) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_periode.getText()+"',getdate(),'"+this.app._userLog+"','"+vStatus+"','"+this.e_modul.getText()+"','APPFMIN','"+this.e_nobukti.getText()+"','"+this.e_memo.getText()+"','-','X','X')");
												
					//---------------- flag bukti									
					if (this.e_modul.getText() == "PBBAU" || this.e_modul.getText() == "PBPR" || this.e_modul.getText() == "PBINV" || this.e_modul.getText() == "PBLOG") 
						sql.add("update yk_pb_m set no_app3='"+this.e_nb.getText()+"',progress='"+vStatus+"' where no_pb='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");																	
					if (this.e_modul.getText() == "PJAJU" || this.e_modul.getText() == "PJPR") 
						sql.add("update panjar2_m set no_app3='"+this.e_nb.getText()+"',progress='"+vStatus+"' where no_panjar='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");																	
					if (this.e_modul.getText() == "PJPTG" || this.e_modul.getText() == "PRPTG") 
						sql.add("update panjarptg2_m set no_app3='"+this.e_nb.getText()+"',progress='"+vStatus+"' where no_ptg='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");																	
	...
	doClick:function(sender){		
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"spm_app_m","no_app",this.app._lokasi+"-AMF"+this.e_periode.getText().substr(2,4)+".","0000"));												
	...
	doLoad:function(sender){	
		try {																																
			var strSQL = "select a.due_date,a.no_pb as no_bukti,'INPROG' as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_app2,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp,a.kode_proyek "+
						 ...
						 "where a.progress='2' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul in ('PBBAU','PBPR','PBINV','PBLOG') "+					 
						 "union "+
						 ...
						 "where a.progress='2' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul in ('PJPTG','PRPTG') "+
			...
			if (this.c_modul3.getText() != "" && this.cb_pp.getText() != "") {
				if (this.c_modul3.getText() == "PBBAU" || this.c_modul3.getText() == "PBPR" || this.c_modul3.getText() == "PBINV" || this.c_modul3.getText() == "PBLOG") {
					strSQL = "select a.due_date,a.no_pb as no_bukti,'INPROG' as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_app2,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp,a.kode_proyek "+
							"from yk_pb_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
							"               inner join karyawan c on a.nik_user=c.nik and a.kode_lokasi=c.kode_lokasi "+
							"where a.progress='2' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = '"+this.c_modul3.getText()+"' and a.kode_pp='"+this.cb_pp.getText()+"' ";				
				} 				
