window.app_saku3_transaksi_bangtel_pbh_fPtgFinal = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bangtel_pbh_fPtgFinal.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bangtel_pbh_fPtgFinal";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,18,996,348], childPage:["Data Pertanggungan","Detail Pertanggungan","Kas Bank"]});				
		...
		this.sg3 = new saiGrid(this.pc1.childPage[1],{bound:[1,10,this.pc1.width-10,235],colCount:7,tag:9,
...
window.app_saku3_transaksi_bangtel_pbh_fPtgFinal.extend(window.childForm);
window.app_saku3_transaksi_bangtel_pbh_fPtgFinal.implement({
		...
		try{		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						sql.add("update panjarptg2_m set progress='3',no_final='-' where no_ptg='"+this.e_noptg.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						...
						sql.add("delete from trans_m where form = 'KBSLSPJ' and no_dokumen = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					if (nilaiToFloat(this.e_nilaikb.getText()) == 0) {
						sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','KB','KBSLSPJ','F','0','0','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','"+this.e_nb.getText()+"','"+this.e_ket.getText()+"','IDR',1,"+Math.abs(nilaiToFloat(this.e_nilai.getText()))+",0,0,'-','-','-','"+this.e_noptg.getText()+"','"+this.e_nopj.getText()+"','-','-','-','-')");
						...
						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) "+
								"select '"+this.e_nb.getText()+"',kode_lokasi,getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_nb.getText()+"','"+this.dp_d1.getDateString()+"',no_urut,kode_akun,dc,nilai,nilai,keterangan,'KBSLSPJ',jenis,'IDR',1,kode_pp,kode_drk,'-','-','-','-','-','-','-' "+
						...
						sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
								"('"+this.e_nbkb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','KB','KBSLSPJ','F','0','0','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','"+this.e_nb.getText()+"','"+this.e_ket.getText()+"','IDR',1,"+Math.abs(nilaiToFloat(this.e_nilaikb.getText()))+",0,0,'-','-','-','"+this.e_noptg.getText()+"','"+this.e_nopj.getText()+"','-','"+this.cb_akun.getText()+"','"+this.c_jenis.getText()+"','-')");

						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
								"('"+this.e_nbkb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_nb.getText()+"','"+this.dp_d1.getDateString()+"',98,'"+this.cb_akun.getText()+"','"+DCkb+"',"+Math.abs(nilaiToFloat(this.e_nilaikb.getText()))+","+Math.abs(nilaiToFloat(this.e_nilaikb.getText()))+",'"+this.e_ket.getText()+"','KBSLSPJ','KB','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");
						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) "+
								"select '"+this.e_nbkb.getText()+"',kode_lokasi,getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_nb.getText()+"','"+this.dp_d1.getDateString()+"',no_urut,kode_akun,dc,nilai,nilai,keterangan,'KBSLSPJ',jenis,'IDR',1,kode_pp,kode_drk,'-','-','-','-','-','-','-' "+
								"from panjarptg2_j where no_ptg='"+this.e_noptg.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");		
					}
	...
	doLoad:function(sender){				
		var strSQL = "select a.no_ptg,a.no_panjar,convert(varchar,a.tanggal,103) as tanggal,a.no_dokumen,a.keterangan,a.nilai_pj,a.nilai,a.nilai_kas,a.akun_panjar,b.kode_pp+' - '+b.nama as pp,c.nik+' - '+c.nama as pemohon "+
					 ...
					 "where a.periode<='"+this.e_periode.getText()+"' and a.progress='3' and a.kode_lokasi='"+this.app._lokasi+"'";			
	...
	doLoadL:function(sender){																							 
		var strSQL = "select a.no_ptg,convert(varchar,a.tanggal,103) as tgl,a.modul,a.no_kas as no_dokumen,a.keterangan,a.nilai "+
					 "from ptg_m a "+
					 "inner join trans_m z on a.no_ptg=z.no_bukti and a.kode_lokasi=z.kode_lokasi and z.posted='F' and z.form='KBSLSPJ' "+					 					 				 					 					 
				     "where a.modul = 'SLSPJ' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";				 
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClickL: function(sender, col , row) {
		try{
			if (this.sgL.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider(
						   "select a.no_dokumen as nodok,convert(varchar,a.tanggal,103) as tanggal,a.periode,a.keterangan as ket,"+
						   "isnull(b.param2,'') as jenis,isnull(b.param1,'') as akun_kb,isnull(b.no_bukti,'') as no_kas,isnull(b.nilai1,0) as nilaikb "+
						   "from ptg_m a left join trans_m b on a.no_ptg=b.no_dokumen and a.kode_lokasi=b.kode_lokasi and b.form='KBSLSPJ' and b.no_bukti <> b.no_dokumen "+ //trans_m-nya--> kasbank					   
