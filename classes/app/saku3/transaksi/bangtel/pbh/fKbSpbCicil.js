window.app_saku3_transaksi_bangtel_pbh_fKbSpbCicil = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bangtel_pbh_fKbSpbCicil.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bangtel_pbh_fKbSpbCicil";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","From KasBank SPB Termin", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,315], childPage:["Daftar SPB","Jurnal Tambahan"]});		
		this.sg5 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:8,tag:0,
		            colTitle:["No SPB","No Bukti","Tanggal","Due Date","Deskripsi","Nilai SPB","Modul","Ni Bayar"],
					colWidth:[[7,6,5,4,3,2,1,0],[100,60,100,300,70,70,120,120]],colFormat:[[5,7],[cfNilai,cfNilai]],															
...
window.app_saku3_transaksi_bangtel_pbh_fKbSpbCicil.extend(window.childForm);
window.app_saku3_transaksi_bangtel_pbh_fKbSpbCicil.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					for (var i=0;i < this.dataJU5.rs.rows.length;i++){
						...
						if (parseFloat(line.nbayar) != 0) {
							sql.add("insert into kas_pb_d (no_kas,kode_lokasi,periode,tanggal,no_pb,no_spb,nilai) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+line.no_bukti+"','"+line.no_spb+"',"+parseFloat(line.nbayar)+")");
							...
							if (line.modul.toUpperCase() == "PBBAU" || line.modul.toUpperCase() == "PBPR" || line.modul.toUpperCase() == "PBINV") {								
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3)  "+
										"select '"+this.e_nb.getText()+"',b.kode_lokasi,getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"',b.no_pb,'"+this.dp_d1.getDateString()+"',c.no_urut,c.kode_akun,c.dc,"+parseFloat(line.nbayar)+","+parseFloat(line.nbayar)+",c.keterangan,'KBSPB',c.jenis,c.kode_curr,c.kurs,c.kode_pp,c.kode_drk,'-','-','-','-','-','-','-' "+
							...
							if (line.modul.toUpperCase() == "PJAJU" || line.modul.toUpperCase() == "PJPR") {
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3)  "+
										"select '"+this.e_nb.getText()+"',b.kode_lokasi,getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"',b.no_panjar,'"+this.dp_d1.getDateString()+"',c.no_urut,c.kode_akun,c.dc,"+parseFloat(line.nbayar)+","+parseFloat(line.nbayar)+",c.keterangan,'KBSPB',c.jenis,c.kode_curr,c.kurs,c.kode_pp,c.kode_drk,'-','-','-','-','-','-','-' "+
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					this.sg1.clear(1); this.sg5.clear(1); this.sg3.clear(1); 
	...
	doLoad:function(sender){		
		this.dataJU5 = {rs:{rows:[]}};
		var strSQL = "select b.no_ver as no_spb,b.no_pb as no_bukti,convert(varchar,b.tanggal,103) as tgl,convert(varchar,b.tanggal,103) as duedate,b.keterangan,c.nilai-isnull(d.nbayar,0) as nilai,b.modul,0 as nbayar "+
					 ...
					 "				   left join (select no_pb,sum(nilai) as nbayar "+
					 "							  from kas_pb_d where kode_lokasi='"+this.app._lokasi+"' group by no_pb) d on b.no_pb=d.no_pb "+

					 "where b.kode_lokasi='"+this.app._lokasi+"' and c.nilai-isnull(d.nbayar,0)>0 "+
					 "union all "+
					 
					 "select b.no_ver as no_spb,b.no_panjar as no_bukti,convert(varchar,b.tanggal,103) as tgl,convert(varchar,b.due_date,103) as duedate,b.keterangan,c.nilai-isnull(d.nbayar,0) as nilai,a.modul,0 as nbayar "+
		             "from spm_app_m a inner join panjar2_m b on a.no_app=b.no_ver and a.kode_lokasi=b.kode_lokasi "+
					 "                 inner join (select no_app,sum(case dc when 'D' then nilai else -nilai end) as nilai "+
					 "                             from spm_app_j where kode_lokasi='"+this.app._lokasi+"' group by no_app) c on a.no_app=c.no_app  "+

					 "				   left join (select no_pb,sum(nilai) as nbayar "+
					 "							  from kas_pb_d where kode_lokasi='"+this.app._lokasi+"' group by no_pb) d on b.no_panjar=d.no_pb "+
	...
	doTampilData5: function(page) {		
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU5.rs.rows[i];									
			this.sg5.appendData([line.no_spb,line.no_bukti,line.tgl,line.duedate,line.keterangan,floatToNilai(line.nilai),line.modul.toUpperCase(),line.nbayar]);
		}
	...
	doChangeCell5: function(sender, col , row) {
		if (col == 7) {			
			this.dataJU5.rs.rows[((this.page-1)*20) + row].nbayar = nilaiToFloat(this.sg5.cells(7,row));			
			this.sg1.validasi();
		}
	},	
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_bangtel_rptKbJurnalBuktiKbCicil";
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select b.no_ver as no_spb,b.no_pb as no_bukti,convert(varchar,b.tanggal,103) as tgl,convert(varchar,b.tanggal,103) as duedate,b.keterangan,c.nilai-isnull(d.nbayar,0) as nilai,b.modul,e.nilai as nbayar "+
							 "from spm_app_m a "+
							 "inner join yk_pb_m b on a.no_app=b.no_ver and a.kode_lokasi=b.kode_lokasi "+
							 "inner join (select no_app,sum(case dc when 'D' then nilai else -nilai end) as nilai "+
					 		 "            from spm_app_j where kode_lokasi='"+this.app._lokasi+"' group by no_app) c on a.no_app=c.no_app  "+

							 "inner join kas_pb_d e on b.no_kas=e.no_kas and b.kode_lokasi=e.kode_lokasi "+
							 "left join (select no_pb,sum(nilai) as nbayar "+
					 		 "			 from kas_pb_d where kode_lokasi='"+this.app._lokasi+"' and no_kas<>'"+this.e_nb.getText()+"' group by no_pb) d on b.no_pb=d.no_pb "+

							 "where b.progress='6' and b.no_kas='"+this.e_nb.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"' "+
							 "union all "+
							 
							 "select b.no_ver as no_spb,b.no_panjar as no_bukti,convert(varchar,b.tanggal,103) as tgl,convert(varchar,b.due_date,103) as duedate,b.keterangan,c.nilai-isnull(d.nbayar,0) as nilai,a.modul,e.nilai as nbayar "+
							 "from spm_app_m a "+
							 "inner join panjar2_m b on a.no_app=b.no_ver and a.kode_lokasi=b.kode_lokasi "+
							 "inner join (select no_app,sum(case dc when 'D' then nilai else -nilai end) as nilai "+
							 "            from spm_app_j where kode_lokasi='"+this.app._lokasi+"' group by no_app) c on a.no_app=c.no_app  "+
							  
							 "inner join kas_pb_d e on b.no_kas=e.no_kas and b.kode_lokasi=e.kode_lokasi "+
							 "left join (select no_pb,sum(nilai) as nbayar "+
					 		 "			 from kas_pb_d where kode_lokasi='"+this.app._lokasi+"' and no_kas<>'"+this.e_nb.getText()+"' group by no_pb) d on b.no_panjar=d.no_pb "+
							 "where b.progress='6' and b.no_kas='"+this.e_nb.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"' "+
