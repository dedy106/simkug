window.app_saku3_transaksi_bangtel_proyek_fReklasBDD2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bangtel_proyek_fReklasBDD2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bangtel_proyek_fReklasBDD2";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Penyelesaian BDD", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,996,259], childPage:["Data BDD"]}); //,"Data Anggaran"
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:5,tag:0,
		            colTitle:["Kode PP","Nama PP/Unit","Akun Beban","Nama Akun","Nilai"],
...
window.app_saku3_transaksi_bangtel_proyek_fReklasBDD2.extend(window.childForm);
window.app_saku3_transaksi_bangtel_proyek_fReklasBDD2.implement({		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,4])){
				try{														
					...
					sql.add("insert into spm_proyek_reklas_m(no_reklas,kode_lokasi,no_dokumen,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_setuju,tgl_input,nik_user,posted,ref1 ) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.ppProyek+"','REKLAS','BDD','"+this.e_periode.getText()+"','IDR',1,"+nilaiToFloat(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','X','"+this.cb_proyek.getText()+"')");
					
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){								
								sql.add("insert into spm_proyek_reklas_d(no_reklas,kode_lokasi,periode,kode_proyek,kode_akun,dc,nilai,kode_pp) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.sg.cells(2,i)+"','D',"+nilaiToFloat(this.sg.cells(4,i))+",'"+this.sg.cells(0,i)+"')");
					...
					sql.add("insert into spm_proyek_reklas_j(no_reklas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.cb_akunbdd.getText()+"','"+this.e_ket.getText()+"','C','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.ppProyek+"','-','"+this.app._lokasi+"','REKLAS','BDD','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
	...
	doChange:function(sender){
		...
		if (sender == this.cb_proyek && this.cb_proyek.getText()!="") {
			var strSQL = "select c.kode_pp,d.akun_bdd "+
