window.app_saku3_transaksi_bangtel_proyek_fDokLoad = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bangtel_proyek_fDokLoad.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bangtel_proyek_fDokLoad";
		...
		this.c_jenis = new saiCB(this,{bound:[20,11,200,20],caption:"Modul",items:["PB","PROYEK","PJPTG"], readOnly:true,tag:2,change:[this,"doChange"]});		
		...
		this.cb_ptg = new portalui_saiCBBL(this,{bound:[20,13,220,20],caption:"No PJPTG",tag:9,multiSelection:false,change:[this,"doChange"],visible:false}); 						
...
window.app_saku3_transaksi_bangtel_proyek_fDokLoad.extend(window.childForm);
window.app_saku3_transaksi_bangtel_proyek_fDokLoad.implement({
		...
		try{		
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.c_jenis.getText() == "PJPTG") {
						var ix=0;
						for (var i=0;i < this.sgUpld.getRowCount();i++){
							if (this.sgUpld.rowValid(i)){
								ix++;																		
								sql.add("insert into spm_panjar_dok(no_bukti,no_gambar,nu,kode_jenis,kode_lokasi,no_ref) values ('"+this.cb_ptg.getText()+"','"+this.sgUpld.cells(3,i).tmpfile+"','"+ix+"','"+this.sgUpld.cells(0,i)+"','"+this.app._lokasi+"','"+this.cb_ptg.getText()+"')");
							}	
						}				
						...
						for (var i=0;i < this.sg1mp2.getRowCount();i++){
							if (this.sg1mp2.rowValid(i)){
								if (this.sg1mp2.cells(4,i) == "HAPUS") {
									sql.add("delete from spm_panjar_dok where no_bukti='"+this.cb_ptg.getText()+"' and no_gambar='"+this.sg1mp2.cells(2,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
								}
							}	
						}
					}
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_pp && this.cb_pp.getText()!="") {
			...
			this.cb_ptg.setSQL("select no_ptg,keterangan from panjarptg2_m a where kode_pp='"+this.cb_pp.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_ptg","keterangan"],false,["No PTG","Deskripsi"],"and","Data Pertanggungan Panjar",true);
		}			
		...
		if (sender == this.cb_ptg && this.cb_ptg.getText()!="") {
			this.sg1mp2.clear();
			var data = this.dbLib.getDataProvider(
					"select b.kode_jenis,b.nama,a.no_gambar, 'PTGAJU' as modul "+
					"from spm_panjar_dok a inner join dok_jenis b on a.kode_jenis=b.kode_jenis and a.kode_lokasi=b.kode_lokasi "+
					"where a.no_ref = '"+this.cb_ptg.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.nu",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg1mp2.clear();
				for (var i in data.rs.rows){
