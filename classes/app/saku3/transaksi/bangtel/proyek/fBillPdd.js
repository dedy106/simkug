window.app_saku3_transaksi_bangtel_proyek_fBillPdd = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bangtel_proyek_fBillPdd.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bangtel_proyek_fBillPdd";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Reklas PDD-Pendapatan", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,996,282], childPage:["Data Pendapatan"]});
		...
		try {
			...
			this.cb_proyek.setSQL("select a.kode_proyek, a.nama from spm_proyek a inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.nik='"+this.app._userLog+"' where a.flag_aktif='1' and a.kode_lokasi='"+this.app._lokasi+"'",["a.kode_proyek","a.nama"],false,["Kode","Deskripsi"],"and","Data Proyek",true);			
...
window.app_saku3_transaksi_bangtel_proyek_fBillPdd.extend(window.childForm);
window.app_saku3_transaksi_bangtel_proyek_fBillPdd.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into spm_piutang_m(no_piutang,kode_lokasi,no_dokumen,tanggal,keterangan,kode_proyek,kode_cust,kode_curr,kurs,nik_app,kode_pp,nilai,periode,nik_user,tgl_input,akun_piutang,posted,nilai_ppn,modul) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_bill.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_proyek.getText()+"','"+this.cb_cust.getText()+"','IDR',1,'"+this.app._userLog+"','"+this.app._kodePP+"',"+parseNilai(this.e_total.getText())+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.akunPdd+"','X',0,'PDD')");
					sql.add("insert into spm_piutang_j(no_piutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_bill.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.akunPdd+"','"+this.e_ket.getText()+"','D','IDR',1,"+parseNilai(this.e_total.getText())+","+parseNilai(this.e_total.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PDD','PDD','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){															
								...
								sql.add("insert into spm_piutang_j(no_piutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.cb_bill.getText()+"','"+this.dp_d1.getDateString()+"',"+j+",'"+this.akunPdpt+"','"+this.e_ket.getText()+"','C','IDR',1,"+parseNilai(this.sg.cells(2,i))+","+parseNilai(this.sg.cells(2,i))+",'"+this.sg.cells(0,i)+"','-','"+this.app._lokasi+"','PDD','PDPT','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");								
							}
						}
					}
	...
	doChange:function(sender){		
		if (sender == this.cb_proyek && this.cb_proyek.getText()!="") {
			...
			var strSQL = "select a.kode_cust,b.akun_pdd,b.akun_pdpt "+
			...
			if (this.stsSimpan == 1) {
				this.cb_bill.setSQL("select a.no_piutang, a.keterangan from spm_piutang_m a "+
									...
									"where a.nilai > isnull(b.reverse,0) and a.kode_proyek='"+this.cb_proyek.getText()+"'  and a.status='PDD' and a.kode_lokasi='"+this.app._lokasi+"'",["no_piutang","keterangan"],false,["No Bukti","Deskripsi"],"and","Data Akru",true);			
			}
		}				
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "" ) {							
			...
			setTipeButton(tbSimpan);
			this.stsSimpan = 1;	
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"spm_piutang_m","no_piutang",this.app._lokasi+"-PDD"+this.e_periode.getText().substr(2,4)+".","0000"));
			this.e_ket.setFocus();
		}
	},	
	...
	doLoad3:function(sender){																								
		var strSQL = "select a.no_piutang,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
					 ...
					 "where a.kode_pp='"+this.app._kodePP+"' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'  and a.modul='PDD'";					 
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
