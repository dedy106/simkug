window.app_saku3_transaksi_bangtel_proyek_fBillDummy = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bangtel_proyek_fBillDummy.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bangtel_proyek_fBillDummy";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Akru Piutang Non Bill", 0);	
		...
		this.e_saldo = new saiLabelEdit(this.pc2.childPage[0],{bound:[770,14,210,20],caption:"Saldo Tagihan", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,996,282], childPage:["Data Billing"]});
...
window.app_saku3_transaksi_bangtel_proyek_fBillDummy.extend(window.childForm);
window.app_saku3_transaksi_bangtel_proyek_fBillDummy.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into spm_piutang_m(no_piutang,kode_lokasi,no_dokumen,tanggal,keterangan,kode_proyek,kode_cust,kode_curr,kurs,nik_app,kode_pp,nilai,periode,nik_user,tgl_input,akun_piutang,posted,nilai_ppn,modul) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_proyek.getText()+"','"+this.cb_cust.getText()+"','IDR',1,'"+this.app._userLog+"','"+this.app._kodePP+"',"+parseNilai(this.e_total.getText())+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.akunPiu+"','X',0,'NONBILL')");
	...
	doLoad3:function(sender){																								
		var strSQL = "select a.no_piutang,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
					 ...
					 "where a.kode_pp='"+this.app._kodePP+"' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'  and a.modul='NONBILL'";					 
		var data = this.dbLib.getDataProvider(strSQL,true);
