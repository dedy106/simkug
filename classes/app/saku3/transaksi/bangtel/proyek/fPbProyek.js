window.app_saku3_transaksi_bangtel_proyek_fPbProyek = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bangtel_proyek_fPbProyek.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bangtel_proyek_fPbProyek";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Permintaan Bayar Proyek", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,996,328], childPage:["Data Proyek","Atensi Pembayaran","Detail Transaksi","Jurnal Tambahan/Pajak","Catatan","File Dok"]});
...
window.app_saku3_transaksi_bangtel_proyek_fPbProyek.extend(window.childForm);
window.app_saku3_transaksi_bangtel_proyek_fPbProyek.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into yk_pb_m (no_pb,no_dokumen,kode_lokasi,periode,nik_user,tgl_input,tanggal,due_date,keterangan,nilai,modul,progress,kode_pp,nik_app,nik_tahu,no_hutang,no_app,no_spb,no_ver,kode_bidang,kode_loktuj,nilai_final,posted,kode_proyek,no_app2,no_app3,no_fiat,no_kas,akun_hutang) values  "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.dp_d1.getDateString()+"','"+this.dp_d2.getDateString()+"','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_totalDC.getText())+",'PBPR','0','"+this.ppProyek+"','"+this.cb_app.getText()+"','"+this.cb_tahu.getText()+"','-','-','-','-','"+this.ppProyek+"','"+this.app._lokasi+"',"+nilaiToFloat(this.e_totalDC.getText())+",'X','"+this.cb_proyek.getText()+"','-','-','-','-','"+this.akunBMHD+"')");
					...
					sql.add("insert into yk_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',99,'"+this.akunBMHD+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_totalDC.getText())+",'"+this.ppProyek+"','-','"+this.app._lokasi+"','PBPR','BMHD','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");
					...
					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){								
								...
								sql.add("insert into yk_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.akunBDD+"','"+this.sg1.cells(0,i)+"','D',"+nilaiToFloat(this.sg1.cells(1,i))+",'"+this.ppProyek+"','-','"+this.app._lokasi+"','PBPR','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");										
							}
						}
					}														
					...
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i) && this.sgUpld.cells(2,i)!="-") {							
							...
							sql.add("insert into yk_pb_dok(no_pb,no_gambar,nu,kode_jenis,kode_lokasi,no_ref) values ('"+this.e_nb.getText()+"','"+this.sgUpld.cells(3,i).tmpfile+"','"+i+"','"+this.sgUpld.cells(0,i)+"','"+this.app._lokasi+"','"+this.cb_proyek.getText()+"')");								
						}	
					}	
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					this.sg.clear(1); this.sg1.clear(1); this.sg3.clear(1); this.sgUpld.clear(1); this.sgFile.clear(1);
	...
	doChange:function(sender){
		...
		if (sender == this.cb_proyek && this.cb_proyek.getText() != "")  {
			var strSQL = "select a.kode_pp,b.akun_bdd,b.akun_bmhd,a.nilai_or,isnull(c.bdd,0) as bdd, round(a.nilai_or * "+this.pBatasMax+"/100,0) as batas "+
	...
	doLoad3:function(sender){																				
		...
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.modul,a.no_dokumen,a.keterangan,a.progress,a.nilai, "+
					 ...
					 "where a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'PBPR' and a.progress in ('0','S','V','K','D','F') ";
