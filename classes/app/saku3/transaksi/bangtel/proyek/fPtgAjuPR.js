window.app_saku3_transaksi_bangtel_proyek_fPtgAjuPR = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bangtel_proyek_fPtgAjuPR.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bangtel_proyek_fPtgAjuPR";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Pertanggungan PJ Proyek", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,10,996,351], childPage:["Data Panjar - Proyek","Deskripsi Pertanggungan","Catatan","File Dok"]});
		...
		this.sg1 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:2,tag:0,
		...
		try {
			...
			//rudi 7-7-20
			this.cb_pp.setSQL("select a.kode_pp, a.nama from pp a inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.nik='"+this.app._userLog+"' where a.kode_lokasi='"+this.app._lokasi+"' and a.tipe='posting' and a.flag_aktif ='1' ",["kode_pp","nama"],false,["Kode","Nama"],"and","Data PP",true);
...
window.app_saku3_transaksi_bangtel_proyek_fPtgAjuPR.extend(window.childForm);
window.app_saku3_transaksi_bangtel_proyek_fPtgAjuPR.implement({
		...
		try{									
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					//reverse bdd saat abil panjar		
					sql.add("insert into spm_proyek_bdd(no_bukti,modul,kode_proyek,nu,keterangan,dc,nilai,kode_lokasi,periode) values "+
							"('"+this.e_nb.getText()+"','PJPR','"+this.cb_proyek.getText()+"',0,'"+this.cb_pj.getText()+"','C',"+nilaiToFloat(this.e_nilaipj.getText())+",'"+this.app._lokasi+"','"+this.e_periode.getText()+"')");														
					...
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i) && this.sgUpld.cells(2,i)!="-") {							
							...
							//salah tabel ...sql.add("insert into yk_pb_dok(no_pb,no_gambar,nu,kode_jenis,kode_lokasi,no_ref) values ('"+this.e_nb.getText()+"','"+this.sgUpld.cells(3,i).tmpfile+"','"+i+"','"+this.sgUpld.cells(0,i)+"','"+this.app._lokasi+"','"+this.e_nb.getText()+"')");								
							sql.add("insert into spm_panjar_dok(no_bukti,no_gambar,nu,kode_jenis,kode_lokasi,modul,no_ref) values ('"+this.e_nb.getText()+"','"+this.sgUpld.cells(3,i).tmpfile+"','"+i+"','"+this.sgUpld.cells(0,i)+"','"+this.app._lokasi+"','PTGAJU','"+this.e_nb.getText()+"')");															
						}	
					}		
	...
	doChange:function(sender){
		if (sender == this.e_periode || sender == this.cb_pp) {
			if (this.e_periode.getText()!="" && this.cb_pp.getText()!="" && this.stsSimpan == 1) {					
				this.cb_pj.setSQL("select a.no_panjar, a.keterangan from panjar2_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
				                  "where a.progress = '6' and a.modul='PJPR' and b.kode_pp='"+this.cb_pp.getText()+"' and  a.no_kas<>'-' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",["no_panjar","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true); 				
			}
		}
		if (sender == this.cb_pj && this.cb_pj.getText()!="" && this.stsSimpan == 1) {
			var strSQL = "select a.periode,a.nilai,a.akun_panjar,a.nik_buat+' - '+b.nama as pemegang,a.kode_proyek "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){																				
					...
					//flag_aktif='1' minta diopen pa oha 13082020
					this.cb_proyek.setSQL("select kode_proyek,nama from spm_proyek where kode_proyek ='"+line.kode_proyek+"' and kode_pp='"+this.cb_pp.getText()+"'  and kode_lokasi='"+this.app._lokasi+"'",["kode_proyek","nama"],false,["Kode","Deskripsi"],"and","Data Proyek",true);			
		...
		if (sender == this.cb_proyek && this.cb_proyek.getText() != "")  {			
			var strSQL = "select b.akun_bdd,a.nilai_or,isnull(c.bdd,0) as bdd "+
						 ...
						 "                       	from spm_proyek_bdd "+
						 "							where no_bukti not in ('"+this.e_nb.getText()+"','"+this.cb_pj.getText()+"') and kode_lokasi='"+this.app._lokasi+"' group by kode_proyek "+
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "") {
			...
			this.stsSimpan = 1;
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"panjarptg2_m","no_ptg",this.app._lokasi+"-PTP"+this.e_periode.getText().substr(2,4)+".","0000"));
			this.e_ket.setFocus();
	...
	doLoad3:function(sender){		
		if (this.cb_pp.getText() != "") {																		
			var strSQL = "select c.no_ptg,a.no_panjar,convert(varchar,c.tanggal,103) as tgl,"+
						...
						"where c.modul= 'PRPTG' and c.periode<='"+this.e_periode.getText()+"' and c.kode_lokasi='"+this.app._lokasi+"' and c.progress in ('0','S','V','K') and d.kode_pp='"+this.cb_pp.getText()+"'";		
			var data = this.dbLib.getDataProvider(strSQL,true);
		...
		else system.alert(this,"Data PP harap dipilih","");		
	},
