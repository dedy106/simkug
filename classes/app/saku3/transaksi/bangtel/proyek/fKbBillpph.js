window.app_saku3_transaksi_bangtel_proyek_fKbBillpph = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bangtel_proyek_fKbBillpph.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bangtel_proyek_fKbBillpph";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","KasBank Penerimaan Invoice", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,20,996,282], childPage:["Data Invoice","Jurnal Sisa"]});		
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-33],colCount:9,tag:0,
					...
					colWidth:[[8,7,6,5,4,3,2,1,0],[70,70,150,100,60,300,150,150,80]],
...
window.app_saku3_transaksi_bangtel_proyek_fKbBillpph.extend(window.childForm);
window.app_saku3_transaksi_bangtel_proyek_fKbBillpph.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','KB','KBBILLPPH','F','0','0','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','IDR',1,"+nilaiToFloat(this.e_total.getText())+",0,0,'-','-','-','-','-','-','"+this.c_jenis.getText()+"','"+this.cb_akun.getText()+"','"+this.cb_cust.getText()+"')");
					
					var kasIDR = nilaiToFloat(this.e_total.getText());		
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3)  values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.cb_akun.getText()+"','D',"+kasIDR+","+kasIDR+",'"+this.e_ket.getText()+"','KBBILLPPH','KB','IDR',1,'"+this.e_ppkb.getText()+"','-','-','-','-','-','-','-','-')");		
					
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && this.sg1.cells(0,i) == "BAYAR"){							
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3)  values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.sg1.cells(1,i)+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(4,i)+"','C',"+nilaiToFloat(this.sg1.cells(5,i))+","+nilaiToFloat(this.sg1.cells(5,i))+",'Pelunasan atas invoice "+this.sg1.cells(1,i)+"','KBBILLPPH','PIU','IDR',1,'"+this.sg1.cells(7,i)+"','-','-','-','-','-','-','-','-')");		
					...
					if (this.sg.getRowValidCount() > 0) {
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3)  values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg.cells(4,i))+","+nilaiToFloat(this.sg.cells(4,i))+",'"+this.sg.cells(3,i)+"','KBBILLPPH','SLS','IDR',1,'"+this.sg.cells(5,i)+"','-','-','-','-','-','-','-','-')");										
							}
						}
					}										
					setTipeButton(tbAllFalse);					
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan "+
		             "from trans_m a "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.form = 'KBBILLPPH' and a.posted ='F'";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						this.cb_cust.setSQL("select kode_cust, nama from cust where kode_cust='"+line.param3+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_cust","nama"],false,["Kode","Nama"],"and","Data Customer",true);									
						this.cb_cust.setText(line.param3);										
					}
				}
				...
				var strSQL = "select a.no_piutang,a.no_dokumen,a.akun_piutang,a.keterangan,round(a.total-isnull(b.bayar,0),2) as saldo,a.pp,a.kode_pp,a.akun_pph "+
						 ...
						 "			left join spm_proyek_jenis d on c.kode_jenis=d.kode_jenis and c.kode_lokasi=d.kode_lokasi "+
						 "		where a.modul='BILL' and a.kode_cust='"+this.cb_cust.getText()+"' and a.nilai+a.nilai_ppn > 0  and a.kode_lokasi='"+this.app._lokasi+"' "+
						 "		group by a.no_piutang,a.no_dokumen,a.akun_piutang,a.keterangan,a.kode_lokasi,b.kode_pp,b.nama,d.akun_pph  "+
						 "		)a  "+						 
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];									
						this.sg1.appendData(["BAYAR",line.no_piutang,line.no_dokumen,line.keterangan,line.akun_piutang,floatToNilai(line.saldo),line.pp,line.kode_pp,line.akun_pph]);
					}
				} else this.sg1.clear(1);				
