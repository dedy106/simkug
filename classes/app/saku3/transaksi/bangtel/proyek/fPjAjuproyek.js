window.app_saku3_transaksi_bangtel_proyek_fPjAjuproyek = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bangtel_proyek_fPjAjuproyek.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bangtel_proyek_fPjAjuproyek";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Permohonan Panjar Proyek", 0);	
		...
		this.e_max = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,11,200,20],caption:"Maksimal Realisasi", tag:1, readOnly:true, tipeText:ttNilai, text:"0",visible:false});				
		...
		this.e_total = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,13,200,20],caption:"Nilai Panjar", tag:1, tipeText:ttNilai, text:"0"});						
		...
		this.sg6 = new saiGrid(this.pc1.childPage[2],{bound:[0,5,this.pc1.width-5,this.pc1.height-33],colCount:7,tag:9,
					colTitle:["No Panjar","Tanggal","Tgl Pakai","Keterangan","Nilai","Umur (Hari)","NIK Panjar"],					
					colWidth:[[6,5,4,3,2,1,0],[80,80,100,400,80,80,100]],readOnly :true,colFormat:[[4,5],[cfNilai,cfNilai]],autoAppend:false,defaultRow:1});
		...
		try {
			...
			this.pBatasMax = 0;
			var data = this.dbLib.getDataProvider("select kode_spro,flag,value1 from spro where kode_spro in ('MAXPJR','PRBATAS') and kode_lokasi = '"+this.app._lokasi+"'",true);			
			...
			this.cb_buat.setSQL("select a.nik, a.nama from karyawan a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi where b.kode_pp='"+this.app._kodePP+"' and b.kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Data Karyawan",true);			
			this.cb_setuju.setSQL("select a.nik, a.nama from karyawan a inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.nik='"+this.app._userLog+"' where a.kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Data Karyawan",true);			
...
window.app_saku3_transaksi_bangtel_proyek_fPjAjuproyek.extend(window.childForm);
window.app_saku3_transaksi_bangtel_proyek_fPjAjuproyek.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from spm_panjar_dok where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul='PJPR'");	
						sql.add("delete from spm_proyek_bdd where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}					
					
					sql.add("insert into panjar2_m(no_panjar,kode_lokasi,no_dokumen,tanggal,due_date,keterangan,nik_buat,nik_setuju,kode_pp,nilai,periode,nik_user,tgl_input,no_app,no_ver,no_spb,no_kas,progress,akun_panjar,modul,no_app2,no_app3,no_fiat,kode_proyek) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.dp_d2.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_buat.getText()+"','"+this.cb_setuju.getText()+"','"+this.cb_pp.getText()+"',"+parseNilai(this.e_total.getText())+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'-','-','-','-','0','"+this.cb_akun.getText()+"','PJPR','-','-','-','"+this.cb_proyek.getText()+"')");
					
					sql.add("insert into spm_rek(no_bukti,kode_lokasi,modul,nama_rek,no_rek,bank,cabang,bruto,pajak,nilai) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','PJPR','"+this.e_namarek.getText()+"','"+this.e_norek.getText()+"','"+this.e_bank.getText()+"','"+this.e_cabang.getText()+"',"+nilaiToFloat(this.e_total.getText())+",0,"+nilaiToFloat(this.e_total.getText())+")");
									
					sql.add("insert into spm_proyek_bdd(no_bukti,modul,kode_proyek,nu,keterangan,dc,nilai,kode_lokasi,periode) values "+
							"('"+this.e_nb.getText()+"','PJPR','"+this.cb_proyek.getText()+"',0,'"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_total.getText())+",'"+this.app._lokasi+"','"+this.e_periode.getText()+"')");												
					...
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i) && this.sgUpld.cells(2,i)!="-") {							
							...
							sql.add("insert into spm_panjar_dok(no_bukti,no_gambar,nu,kode_jenis,kode_lokasi,modul,no_ref) values ('"+this.e_nb.getText()+"','"+this.sgUpld.cells(3,i).tmpfile+"','"+ix+"','"+this.sgUpld.cells(0,i)+"','"+this.app._lokasi+"','PJPR','"+this.e_nb.getText()+"')");															
						}	
					}	
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :								
				...
				if ((nilaiToFloat(this.e_pakai.getText()) + nilaiToFloat(this.e_total.getText())) > nilaiToFloat(this.e_max.getText())  ) {
					system.alert(this,"Transaksi tidak valid.","Total Biaya melebihi Max Realisasi.");
					return false;						
				}
	...
	doClick:function(sender){		
		if (this.e_periode.getText()!= "") {
			...
			this.stsSimpan = 1;
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"panjar2_m","no_panjar",this.app._lokasi+"-PP"+this.e_periode.getText().substr(2,4)+".","0000"));
			this.e_dok.setFocus();
	...
	doChange:function(sender){	
		if (sender == this.cb_pp && this.cb_pp.getText()!="" )	{
			...
			//rudi7-7-20
			this.cb_proyek.setSQL("select a.kode_proyek,a.nama from spm_proyek a where a.kode_pp='"+this.cb_pp.getText()+"' and a.flag_aktif='1' and a.kode_lokasi='"+this.app._lokasi+"'",["kode_proyek","nama"],false,["Kode","Deskripsi"],"and","Data Proyek",true);			
		}
		if (sender == this.cb_proyek && this.cb_proyek.getText() != "")  {
			var strSQL = "select b.akun_bdd,b.akun_bmhd,a.nilai_or,isnull(c.bdd,0) as bdd, round(a.nilai_or * "+this.pBatasMax+"/100,0) as batas "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){													
					...
					this.e_nilaior.setText(floatToNilai(line.batas));
					this.e_max.setText(floatToNilai(line.batas));
					this.e_pakai.setText(floatToNilai(line.bdd));
				}
			}			
		}
		if ((sender == this.cb_buat || sender == this.cb_proyek) && this.cb_proyek.getText() !="" && this.cb_buat.getText() !="" && this.stsSimpan == 1) {
			...
			this.jumOpen = 0;
			//and a.kode_proyek='"+this.cb_proyek.getText()+"' -- diubah jd per nik bukan nik+proyek - 9/2/21 rudi
			var str = "select a.no_panjar,a.keterangan,convert(varchar,a.tanggal,103) as tgl,a.nilai,convert(varchar,a.due_date,103) as tgl_pakai,DATEDIFF(DAY,a.tanggal,getdate()) as umur, a.nik_buat "+
					  "from panjar2_m a "+
					  "left join ptg_m b on a.no_panjar=b.no_pj and a.kode_lokasi=b.kode_lokasi "+
					  "where a.modul='PJPR' and b.no_pj is null and a.kode_lokasi='"+this.app._lokasi+"' and a.nik_buat='"+this.cb_buat.getText()+"'";					  
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];												
					this.sg6.appendData([line.no_panjar,line.tgl,line.tgl_pakai,line.keterangan,floatToNilai(line.nilai),floatToNilai(line.umur), line.nik_buat]);					
				}			
	...
	doLoad3:function(sender){																		
		var strSQL = "select distinct a.no_panjar,convert(varchar,a.tanggal,103) as tgl,case a.progress when '0' then 'AJU' when 'C' then 'REVISI' when 'V' then 'REV.VER' end as jenis,a.no_dokumen,a.keterangan,a.nik_buat+' - '+b.nama as nama,a.nilai, "+
					 ...
					 "				  inner join karyawan_pp z on a.kode_pp=z.kode_pp and a.kode_lokasi=z.kode_lokasi and z.nik='"+this.app._userLog+"' "+			 					 
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress in ('0','C','V','S','K','D') and a.modul='PJPR'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.nilai,a.keterangan,a.no_dokumen,a.tanggal,a.due_date,a.akun_panjar,a.nik_buat,a.nik_setuju,a.kode_pp,a.progress, "+
							 "b.bank,b.cabang,b.no_rek,b.nama_rek, a.kode_proyek "+
							 "from panjar2_m a inner join spm_rek b on a.no_panjar=b.no_bukti and a.kode_lokasi=b.kode_lokasi and b.modul='PJPR' "+					 
