window.app_saku3_transaksi_bangtel_proyek_fKbBillCicil = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bangtel_proyek_fKbBillCicil.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bangtel_proyek_fKbBillCicil";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","KasBank Penerimaan Invoice [Bersaldo]", 0);	
		...
		this.e_ppkb = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,19,200,20],caption:"PP Akun KB", readOnly:true});						
		
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,20,996,282], childPage:["Data Invoice"]});		
...
window.app_saku3_transaksi_bangtel_proyek_fKbBillCicil.extend(window.childForm);
window.app_saku3_transaksi_bangtel_proyek_fKbBillCicil.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','KB','KBBILLCCL','F','0','0','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','IDR',1,"+nilaiToFloat(this.e_nilai.getText())+",0,0,'-','-','-','"+this.cb_bill.getText()+"','-','-','"+this.c_jenis.getText()+"','"+this.cb_akun.getText()+"','"+this.cb_cust.getText()+"')");
					
					var kasIDR = nilaiToFloat(this.e_nilai.getText());		
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3)  values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','D',"+kasIDR+","+kasIDR+",'"+this.e_ket.getText()+"','KBBILLCCL','KB','IDR',1,'"+this.e_ppkb.getText()+"','-','-','-','-','-','-','-','-')");		
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3)  values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_bill.getText()+"','"+this.dp_d1.getDateString()+"',1,'"+this.e_akunpiu.getText()+"','C',"+kasIDR+","+kasIDR+",'Pelunasan atas invoice "+this.cb_bill.getText()+"','KBBILLCCL','AR','IDR',1,'"+this.kodePP+"','-','-','-','-','-','-','-','-')");		
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan "+
		             "from trans_m a "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.form = 'KBBILLCCL' and a.posted ='F'";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						this.cb_bill.setSQL("select no_piutang, keterangan from spm_piutang_m where no_piutang='"+line.no_ref1+"' and kode_lokasi='"+this.app._lokasi+"'",["no_piutang","keterangan"],false,["No Bukti","Nama"],"and","Data Tagihan",true);									
						this.cb_bill.setText(line.no_ref1);
					}
				}										
			}						
		} catch(e) {alert(e);}
