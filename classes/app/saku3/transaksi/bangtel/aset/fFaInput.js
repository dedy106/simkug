window.app_saku3_transaksi_bangtel_aset_fFaInput = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_bangtel_aset_fFaInput.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_bangtel_aset_fFaInput";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Data Barang", 0);	
		...
		this.e_periode = new portalui_saiLabelEdit(this,{bound:[20,13,200,20],caption:"Periode Perolehan",tag:2,readOnly:true});
		...
		this.cb_pp1 = new saiCBBL(this,{bound:[20,18,220,20],caption:"PP Aktap-Brg", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});
		...
		try {
			...
			this.cb_klp.setSQL("select kode_klpfa, nama from fa_klp where jenis in ('A','I') and kode_lokasi='"+this.app._lokasi+"'",["kode_klpfa","nama"],false,["Kode","Nama"],"where","Data Kelompok Aktap",true);
...
window.app_saku3_transaksi_bangtel_aset_fFaInput.extend(window.childForm);
window.app_saku3_transaksi_bangtel_aset_fFaInput.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.e_umur.getText() == 0) var nsusut = 0								
	...
	doChange:function(sender){	
		...
		if ((sender == this.cb_klp || sender == this.e_periode) && this.cb_klp.getText() != "" && this.e_periode.getText() != "") {
			var data = this.dbLib.getDataProvider(
			           "select a.kode_klpakun,b.nama,b.kode_akun,c.nama as nama_akun,b.umur,b.persen, a.jenis "+
					   "from fa_klp a "+
