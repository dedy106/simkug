window.app_saku3_transaksi_ypt_fMasakunRelasi = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_fMasakunRelasi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_fMasakunRelasi";
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,450], childPage:["Daftar Akun","Data Akun","Filter Cari"]});				
		...
		this.pc2 = new pageControl(this.pc1.childPage[1],{bound:[5,12,990,257], childPage:["Flag Akun","Lap Keuangan","Mapping PP","Budget"]});		
		...
		this.sg3 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:4,tag:9,
					...
					readOnly:true,buttonStyle:[[2],[bsEllips]],
		...
		this.sg4 = new saiGrid(this.pc2.childPage[2],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:2,tag:9,				
				colTitle:["Kode","Nama"],
				colWidth:[[1,0],[300,80]],
		...
		this.bPP = new portalui_imageButton(this.sgn4,{bound:[this.sgn.width - 25,2,22,22],image:"icon/"+system.getThemes()+"/reload.png", hint:"Load PP",click:[this,"doLoadPP"]});

		this.c_gar = new saiCB(this.pc2.childPage[3],{bound:[20,23,200,20],caption:"Status Budget",items:["0. NON","1. BUDGET"], readOnly:true,tag:2});		
		this.cb_gar = new saiCBBL(this.pc2.childPage[3],{bound:[20,11,220,20],caption:"Akun Budget", multiSelection:false, maxLength:10, tag:2});
		...
		try {
			...
			this.cb_gar.setSQL("select kode_akun, nama from masakun where kode_lokasi='"+this.app._lokasi+"' union select '-','-' ",["kode_akun","nama"],false,["Kode","Nama"],"and","Data Mata Anggaran",true);


			var data = this.dbLib.getDataProvider("select b.kode_fs,b.nama as nama_fs "+
						"from fs b "+
						"where b.kode_lokasi='"+this.app._lokasi+"' and b.flag_status ='1'",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg3.clear();
				for (var i in data.rs.rows){
					line = data.rs.rows[i];												
					this.sg3.appendData([line.kode_fs, line.nama_fs, "-", "-"]);
				}
...
window.app_saku3_transaksi_ypt_fMasakunRelasi.extend(window.childForm);
window.app_saku3_transaksi_ypt_fMasakunRelasi.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("delete from relakun_pp where kode_akun = '"+this.cb_kode.getText()+"'");
					...
					if (typeof data == "object" && data.rs.rows[0] != undefined){
						...
						for (var i in data.rs.rows){
							line = data.rs.rows[i];							
							sql.add("insert into masakun (kode_akun,kode_lokasi,nama,modul,jenis,kode_curr,block,status_gar,normal,kode_gar) values "+
									"('"+this.cb_kode.getText()+"','"+line.kode_lokasi+"','"+this.e_nama.getText()+"','"+this.c_modul.getText().substr(0,1)+"','"+this.c_jenis.getText()+"','"+this.c_curr.getText()+"','0','"+this.c_gar.getText().substring(0,1)+"','"+this.c_normal.getText().substring(0,1)+"','"+this.cb_gar.getText()+"')");
							...
							if (this.sg4.getRowValidCount() > 0){
								for (var i=0;i < this.sg4.getRowCount();i++){
									if (this.sg4.rowValid(i)){								
										sql.add("insert into relakun_pp (kode_pp,kode_akun,kode_lokasi) values ('"+this.sg4.cells(0,i)+"','"+this.cb_kode.getText()+"','"+line.kode_lokasi+"')");
									}
								}
							}
				...
				if (this.sg3.getRowValidCount() > 0){
					for (var i=0;i < this.sg3.getRowCount();i++){
						if (this.sg3.rowValid(i) && this.sg3.cells(2,i)=="-"){								
							system.alert(this,"Relasi tidak valid.","Mapping Lap Kug Harus dilengkapi.");
							return false;						
						}
					}
				}
		...
		try{						
			if (sender == this.cb_kode) {
				if (this.cb_kode.getText() != "" ){
					var data = this.dbLib.getDataProvider("select nama,modul,jenis,block,status_gar,normal,kode_gar from masakun "+
														  "where kode_akun = '"+this.cb_kode.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ",true);
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
						if (line != undefined){
							this.e_nama.setText(line.nama);	
							this.cb_gar.setText(line.kode_gar);													
							...
							var data = this.dbLib.getDataProvider("select b.kode_flag,b.nama from flag_relasi a inner join flag_akun b on a.kode_flag=b.kode_flag where a.kode_akun = '"+this.cb_kode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.kode_flag",true);
							if (typeof data == "object" && data.rs.rows[0] != undefined){
								var line;
								this.sg.clear();
								for (var i in data.rs.rows){
							...
							var data = this.dbLib.getDataProvider("select a.kode_fs,a.nama as nama_fs,isnull(b.kode_neraca,'-') as kode_neraca,isnull(b.nama,'-') as nama_lap "+
							           "from fs a "+
							           "left join ( "+
										
									   "select b.kode_neraca,b.nama, b.kode_fs,b.kode_lokasi "+
									   "from relakun a inner join neraca b on a.kode_neraca=b.kode_neraca and a.kode_lokasi=b.kode_lokasi and a.kode_fs=b.kode_fs "+
									   "where a.kode_lokasi='"+this.app._lokasi+"' and a.kode_akun = '"+this.cb_kode.getText()+"' "+

									   ") b on a.kode_fs=b.kode_fs and a.kode_lokasi=b.kode_lokasi "+
									   "where a.flag_status='1' and a.kode_lokasi='"+this.app._lokasi+"'",true);
