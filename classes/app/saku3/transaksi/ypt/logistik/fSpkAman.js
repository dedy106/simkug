window.app_saku3_transaksi_ypt_logistik_fSpkAman = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_logistik_fSpkAman.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_logistik_fSpkAman";
		...
		this.pc2 = new pageControl(this,{bound:[10,10,1000,430], childPage:["List SPK","Data SPK"]});		
		...
		this.cb_pesan = new saiCBBL(this.pc2.childPage[1],{bound:[20,17,220,20],caption:"No Request", readOnly:true, maxLength:10, tag:1,change:[this,"doChange"]});				
		this.e_nilai = new saiLabelEdit(this.pc2.childPage[1],{bound:[790,17,200,20],caption:"Nilai SPK", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		this.cb_vendor = new saiCBBL(this.pc2.childPage[1],{bound:[20,18,220,20],caption:"Vendor", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});				
		this.e_ppn = new saiLabelEdit(this.pc2.childPage[1],{bound:[790,18,200,20],caption:"Nilai PPN", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		this.e_alamat = new saiLabelEdit(this.pc2.childPage[1],{bound:[20,14,450,20],caption:"Alamat", readOnly:true});								
		...
		this.pc1 = new pageControl(this.pc2.childPage[1],{bound:[1,12,995,292], childPage:["Item Barang","File Dok","Item Lama"]});						
		this.sg4 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:10,tag: 0,
		            colTitle:["Kode Brg","Nama","ID Pesan","Item Barang","Merk","Tipe","Spesifikasi","Jumlah","Harga","Total"],
		...
		this.e_spph = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,11,450,20],caption:"No SPPH", maxLength:50});						
		this.l_tgl2 = new portalui_label(this.pc1.childPage[1],{bound:[20,10,100,18],caption:"Tanggal SPPH", underline:true});
		this.dp_d2 = new portalui_datePicker(this.pc1.childPage[1],{bound:[120,10,98,18]}); 				
		this.e_sph = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,12,450,20],caption:"No SPH", maxLength:50});						
		this.l_tgl3 = new portalui_label(this.pc1.childPage[1],{bound:[20,11,100,18],caption:"Tanggal SPH", underline:true});
		this.dp_d3 = new portalui_datePicker(this.pc1.childPage[1],{bound:[120,11,98,18]}); 		

		this.sgUpld = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,160],colCount:5,
...
window.app_saku3_transaksi_ypt_logistik_fSpkAman.extend(window.childForm);
window.app_saku3_transaksi_ypt_logistik_fSpkAman.implement({	
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					//backup
					var strSQL = "select count(*) as ke from log_spk_m_h "+ 
					...
					sql.add("insert into log_spk_m_h(no_spk,no_pks,kode_lokasi,tgl_input,nik_user,periode,tanggal,nik_buat,keterangan,no_tap,kode_vendor,total,no_dokumen,modul,no_sanggup,nilai,ppn,no_pesan,no_spph,tgl_spph,no_sph,tgl_sph,no_ba,tgl_aman,nik_aman,ke) "+
							"select no_spk,no_pks,kode_lokasi,tgl_input,nik_user,periode,tanggal,nik_buat,keterangan,no_tap,kode_vendor,total,no_dokumen,modul,no_sanggup,nilai,ppn,no_pesan,no_spph,tgl_spph,no_sph,tgl_sph,no_ba,getdate(),'"+this.app._userLog+"',"+ke+" "+
							"from log_spk_m where no_spk='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("insert into log_spk_d_h(no_spk,kode_lokasi,no_urut,item,merk,tipe,catatan,jumlah,harga,ppn,kode_klpfa,ke)  "+
					...
					sql.add("insert into log_spk_m(no_spk,no_pks,kode_lokasi,tgl_input,nik_user,periode,tanggal,nik_buat,keterangan,no_tap,kode_vendor,total,no_dokumen,modul,no_sanggup,nilai,ppn,no_pesan,no_spph,tgl_spph,no_sph,tgl_sph,no_ba) values "+ //,flag_revisi
							"('"+this.e_nb.getText()+"','-','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.app._userLog+"','"+this.e_ket.getText()+"','-','"+this.cb_vendor.getText()+"',"+nilaiToFloat(this.e_total.getText())+",'-','TL','-',"+nilaiToFloat(this.e_nilai.getText())+","+nilaiToFloat(this.e_ppn.getText())+",'"+this.cb_pesan.getText()+"','"+this.e_spph.getText()+"','"+this.dp_d2.getDateString()+"','"+this.e_sph.getText()+"','"+this.dp_d3.getDateString()+"','-')");	//,'0'
	...
	doChange:function(sender){
		...
		if (sender == this.cb_pesan && this.cb_pesan.getText()!="") {									
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line2 = data.rs.rows[i];												
					this.sg5.appendData([line2.kode_klpfa,line2.nama,line2.no_urut,line2.item,line2.merk,line2.tipe,line2.catatan,floatToNilai(line2.jumlah),floatToNilai(line2.nilai),floatToNilai(line2.total)]);
				}
	...
	doLoad3:function(sender){																				
		var strSQL = "select a.no_spk,convert(varchar,a.tanggal,103) as tgl,a.keterangan,b.nama,a.nilai+a.ppn as total  "+
		             "from log_spk_m a "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_ba='-' ";//and a.flag_revisi='1'		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_spk,line.tgl,line.keterangan,line.nama,floatToNilai(line.total)]); 
		}
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.*,isnull(b.no_gambar,'-') as no_gambar "+
							 "from log_spk_m a left join log_pesan_dok b on a.no_spk=b.no_pesan and a.kode_lokasi=b.kode_lokasi "+							 							 							 
				...
				if (typeof data == "object"){
					...
					if (line != undefined){								
						...
						this.dp_d2.setText(line.tgl_spph);
						this.dp_d3.setText(line.tgl_sph);
