window.app_saku3_transaksi_ypt_logistik_fAppLog = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_logistik_fAppLog.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_logistik_fAppLog";
		...
		this.l_tgl2 = new portalui_label(this.pc1.childPage[1],{bound:[520,15,100,18],caption:"Tgl Fisik", underline:true});
		this.dp_d2 = new portalui_datePicker(this.pc1.childPage[1],{bound:[620,15,100,18]});		
		this.e_nopesan = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,11,450,20],caption:"No Request", readOnly:true});
		this.e_ket3 = new saiLabelEdit(this.pc1.childPage[1],{bound:[520,11,450,20],caption:"Ket. Pengadaan", maxLength:200});								
		...
		this.e_tglfisik = new saiLabelEdit(this.pc1.childPage[1],{bound:[270,13,200,20],caption:"Tgl Fisik Kebuthn.", readOnly:true});
		...
		this.sg3 = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:7,
		...
		this.sgUpld = new saiGrid(this.pc1.childPage[6],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:5, tag:9,
...
window.app_saku3_transaksi_ypt_logistik_fAppLog.extend(window.childForm);
window.app_saku3_transaksi_ypt_logistik_fAppLog.implement({	
		...
		try{		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into log_justerima_m(no_terima,no_pesan,kode_lokasi,tgl_input,nik_user,periode,tanggal,nik_serah,nik_terima,jenis,tgl_fisik,keterangan) values "+
						   "('"+this.e_nb.getText()+"','"+this.e_nopesan.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','-','-','"+this.c_jenis.getText().substr(0,2)+"','"+this.dp_d2.getDateString()+"','"+this.e_ket3.getText()+"')"); 							
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "hapus" :	
				...
				var sql = new server_util_arrayList();								
				if (this.e_modul.getText() == "LOGREQ") {
					//doloncat 4-10-21 sql.add("update log_pesan_m set progress='1',no_terima='-' where no_pesan='"+this.e_nopesan.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
					sql.add("update log_pesan_m set progress='0',no_terima='-' where no_pesan='"+this.e_nopesan.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
					var modul = "LOGREQ_TRM";
				}				
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg.cells(0,row) != "") {			
				...
				var strSQL = "select convert(varchar,a.tgl_fisik,103) as tglfisik "+
							 "from log_pesan_m a "+
				...
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){													
						this.e_tglfisik.setText(line.tglfisik);					
					}
				}
				...
				if (this.stsProg == "APPROVE" || this.stsProg == "RETURN") {
					...
					this.stsSimpan = 0;					
					var strSQL = "select nik_serah,nik_terima,jenis,isnull(tgl_fisik,getdate()) as tgl_fisik,keterangan "+
	...
	doCari:function(sender){						
		var filter = "";
		//if (this.c_status2.getText() == "INPROG") filter = " and a.progress = '1' "; diloncat 4-10-21
		...
		var strSQL = "select distinct a.tanggal as due_date, a.no_pesan as no_pb,"+
		             "case when a.progress = '0' then 'INPROG' "+ //"case when a.progress = '1' then 'INPROG' "+ --> diloncat 4-10-21
						...
						if (result.toLowerCase().search("error") == -1){	
							...
							if (this.preView == "1") {
								...
								if (this.c_status.getText() == "APPROVE") {	
									this.nama_report = "server_report_saku3_logistik_rptJustPeng";
