window.app_saku3_transaksi_ypt_logistik_fPbAgendaADK = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_logistik_fPbAgendaADK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_logistik_fPbAgendaADK";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,325], childPage:["Otorisasi","Data Permohonan","Data Approval"]});
		...
		this.e_nspk = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,12,200,20],caption:"Nilai SPK", tag:4, readOnly:true, tipeText:ttNilai, text:"0"});				
		this.e_akunhut = new saiLabelEdit(this.pc1.childPage[1],{bound:[520,12,200,20],caption:"Akun Hutang", readOnly:true,tag:4});				
		...
		this.c_jenis = new saiCB(this.pc1.childPage[1],{bound:[20,15,200,20],caption:"Jenis",items:["DP","TERMIN"], readOnly:true,tag:2,change:[this,"doChange"]});		
		this.c_persen = new saiCB(this.pc1.childPage[1],{bound:[20,16,200,20],caption:"Persentase",tag:1,mustCheck:false,  change:[this,"doChange"]});		
		this.e_bruto = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,13,200,20],caption:"Nilai PB", tag:4, tipeText:ttNilai, text:"0",change:[this,"doChange"]});				
		this.e_ppn = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,14,200,20],caption:"Nilai PPN", tag:4, tipeText:ttNilai, text:"0", readOnly:true, change:[this,"doChange"]});				
		this.e_nilai = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,12,200,20],caption:"Total PB", tag:4, tipeText:ttNilai, text:"0"});				
		...
		try {
			...
			this.flagDokFree = "0"; 
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('PPNM','LOGADK','HUTLOG','GARFREE','DOKFREE') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku3_transaksi_ypt_logistik_fPbAgendaADK.extend(window.childForm);
window.app_saku3_transaksi_ypt_logistik_fPbAgendaADK.implement({
	isiSPK : function() {
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,4])){
				try{														
					...
					sql.add("insert into it_aju_m(no_aju,kode_lokasi,periode,tanggal,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,tgl_input,nik_user,no_ver,no_fiat,no_kas,progress,nik_panjar,no_ptg,user_input,sts_pajak) values "+
							"('"+this.e_agenda.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','LOG','"+this.akunHutang+"','"+this.app._kodePP+"','-','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",getdate(),'"+this.app._userLog+"','-','-','-','A','-','-','"+this.app._namaUser.substr(0,50)+"','NON')");						
					sql.add("insert into it_aju_rek(no_aju,kode_lokasi,bank,no_rek,nama_rek,bank_trans,nilai,keterangan) values "+
							"('"+this.e_agenda.getText()+"','"+this.app._lokasi+"','"+this.e_bank.getText() +" ','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"','-',"+nilaiToFloat(this.e_nilai.getText())+",'-')");												
	...
	doChange:function(sender){
		try{
			...
			if (sender == this.cb_spk && this.cb_spk.getText() != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){													
						...
						this.e_akunhut.setText(line.akun_hutang);
			...
			if (sender == this.c_persen && this.c_persen.getText()!="") {
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.no_agenda,a.nilai_final,a.nilai-isnull(c.tambah,0) as nilai,a.no_hutang,a.no_ver as no_verlama,a.no_app as no_applama,a.keterangan,a.no_dokumen,a.modul,a.due_date,a.tanggal,a.nik_tahu,a.nik_app,a.progress,isnull(b.no_app,'-') as no_app,isnull(b.catatan,'-') as catatan "+
							 ...
							 "        	select a.kode_lokasi,a.no_app,a.catatan "+
							 "			from app_d a "+
							 "			inner join app_m b on a.no_app=b.no_app and a.kode_lokasi=b.kode_lokasi and a.no_bukti='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and b.no_appseb='-' and b.modul='"+modulApp+"') b on "+vRelasi+" a.kode_lokasi=b.kode_lokasi "+							 
							 "          left join ("+
