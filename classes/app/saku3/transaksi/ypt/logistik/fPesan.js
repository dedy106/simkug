window.app_saku3_transaksi_ypt_logistik_fPesan = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_logistik_fPesan.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_logistik_fPesan";
		...
		this.pc2 = new pageControl(this,{bound:[10,10,1000,430], childPage:["Data Request","List Request"]});		
		...
		this.e_nb = new portalui_saiLabelEdit(this.pc2.childPage[0],{bound:[20,12,232,20],caption:"No Request",maxLength:30,readOnly:true});
		this.i_gen = new portalui_imageButton(this.pc2.childPage[0],{bound:[255,12,20,20],hint:"Generate",image:"icon/"+system.getThemes()+"/tabCont2.png",click:[this,"doClick"]});
		
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,20,995,350], childPage:["Anggaran","Detail","Otorisasi","Maksud-Tujuan","Aspek Strategis","File Dok"]});			
		this.l_tgl2 = new portalui_label(this.pc1.childPage[0],{bound:[20,11,100,18],caption:"Tgl Fisik", underline:true});
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:8,
		            colTitle:["Item Barang","Merk","Tipe","Spesifikasi","Volume","Satuan","Harga Satuan","Total"],
					colWidth:[[7,6,5,4,3,2,1,0],[80,80,80,60,180,180,180,180]],															
					colFormat:[[4,6,7],[cfNilai,cfNilai,cfNilai]],
					columnReadOnly:[true,[7],[0,1,2,3,4,5]],
...
window.app_saku3_transaksi_ypt_logistik_fPesan.extend(window.childForm);
window.app_saku3_transaksi_ypt_logistik_fPesan.implement({	
	...
	simpan: function(){			
		try{						
			if (this.stsSimpan == 1) this.doGenBukti();
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into log_pesan_m(no_pesan,kode_lokasi,tgl_input,nik_user,periode,jenis,tanggal,no_dokumen,keterangan,kode_pp,kode_drk,kode_akun,saldo_gar,nilai,nik_buat,nik_app,kode_dana,no_app,progress,maksud,aspek,no_terima,no_spph,lok_proses,kode_ppaju, tgl_fisik) values "+
						   "('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.c_jenis.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.cb_pp.getText()+"','"+this.cb_drk.getText()+"','"+this.cb_akun.getText()+"',"+nilaiToFloat(this.e_gar.getText())+","+nilaiToFloat(this.e_total.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','-','-','0','"+urlencode(this.mDesk1.getCode())+"','"+urlencode(this.mDesk2.getCode())+"','-','-','"+this.cb_lokproses.getText()+"','"+this.cb_pp.getText()+"','"+this.dp_d2.getDateString()+"')"); 							
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into log_pesan_d(no_pesan,kode_lokasi,no_urut,item,merk,tipe,catatan,jumlah,nilai,harga,no_po,no_ba,kode_dana,jum_po,kode_klpfa,ppn,satuan) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(1,i)+"','"+this.sg.cells(2,i)+"','"+this.sg.cells(3,i)+"',"+nilaiToFloat(this.sg.cells(4,i))+","+nilaiToFloat(this.sg.cells(6,i))+",0,'-','-','-',0,'-',0,'"+this.sg.cells(5,i)+"')");
							}
						}
					}							
					...
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i) && this.sgUpld.cells(2,i)!="-") {							
							...
							sql.add("insert into log_pesan_dok(no_pesan,no_gambar,nu,kode_jenis,kode_lokasi,modul,no_ref) values ('"+this.e_nb.getText()+"','"+this.sgUpld.cells(3,i).tmpfile+"','"+ix+"','"+this.sgUpld.cells(0,i)+"','"+this.app._lokasi+"','PESAN','"+this.e_nb.getText()+"')");															
						}	
					}	
		...
		switch (event){
			...
			case "ubah" :	
				...
				if (this.sg.getRowValidCount() > 0){
					for (var i=0;i < this.sg.getRowCount();i++){
						if (this.sg.rowValid(i)){
							if (nilaiToFloat(this.sg.cells(6,i)) < this.minCapex && (this.c_jenis.getText() == "CAPEX" || this.cb_akun.getText().substr(0,1) == "1")) {
								var k = i+1;
	...
	doGenBukti: function() {
		var AddFormat = "___%/"+this.app._kodePP+"/"+"JKB/"+this.e_periode.getText().substr(0,4);
		var data = this.dbLib.getDataProvider("select isnull(max(no_pesan),0) as no_pesan from log_pesan_m where no_pesan like '"+AddFormat+"' and kode_lokasi='"+this.app._lokasi+"'",true);
		if (typeof data == "object"){
			var line = data.rs.rows[0];							
			if (line != undefined){
				if (line.no_pesan == "0") this.e_nb.setText("001/"+this.app._kodePP+"/"+"JKB/"+this.e_periode.getText().substr(0,4));
				else {
					var idx = parseFloat(line.no_pesan.substr(0,3)) + 1;
					idx = idx.toString();
					if (idx.length == 1) var nu = "00"+idx;
					if (idx.length == 2) var nu = "0"+idx;
					if (idx.length == 3) var nu = idx;
					this.e_nb.setText(nu+"/"+this.app._kodePP+"/"+"JKB/"+this.e_periode.getText().substr(0,4));						
				}
			} 
		}
	},
	...
	doChangeCell: function(sender, col, row){
		if (col == 4 || col == 6 ) {
			if (this.sg.cells(4,row) != "" && this.sg.cells(6,row) != "") {
				this.sg.cells(7,row,floatToNilai(nilaiToFloat(this.sg.cells(4,row)) * nilaiToFloat(this.sg.cells(6,row))));
				this.sg.validasi();		
			}
		}
	},
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){								
						...
						this.dp_d2.setText(line.tgl_fisik);
				...
				var strSQL = "select no_urut,item,merk,tipe,catatan,satuan,jumlah,nilai,jumlah*nilai as total from log_pesan_d where no_pesan='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' order by no_urut";							
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line2;
					this.sg.clear();
					for (var i in data.rs.rows){
						...
						this.sg.appendData([line2.item,line2.merk,line2.tipe,line2.catatan,floatToNilai(line2.jumlah),line2.satuan,floatToNilai(line2.nilai),floatToNilai(line2.total)]);
					}
