window.app_saku3_transaksi_ypt_logistik_fSpkFinalPP = function (owner) {
	if (owner) {
		window.app_saku3_transaksi_ypt_logistik_fSpkFinalPP.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_ypt_logistik_fSpkFinalPP";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0], { bound: [1, 12, 995, 223], childPage: ["Item Barang", "File Dok"] });
		...
		this.sgUpld = new saiGrid(this.pc1.childPage[1], {
			bound: [1, 5, this.pc1.width - 5, this.pc1.height - 33], colCount: 5, //160
...
window.app_saku3_transaksi_ypt_logistik_fSpkFinalPP.extend(window.childForm);
window.app_saku3_transaksi_ypt_logistik_fSpkFinalPP.implement({
	...
	isiPesan: function () {
        ...
        "                   inner join karyawan_pp d on a.kode_pp=d.kode_pp and a.kode_lokasi=d.kode_lokasi and d.nik='"+this.app._userLog+"' "+	
			"where a.progress='2' and a.kode_lokasi='" + this.app._lokasi + "'", ["a.no_pesan", "a.keterangan"], false, ["No Request", "Deskripsi"], "and", "Data Request", true);
	},
		...
		try {
			...
			if (this.standarLib.checkEmptyByTag(this, [0, 1, 2])) {
				try {
					...
					sql.add("insert into log_spk_m(no_spk,no_pks,kode_lokasi,tgl_input,nik_user,periode,tanggal,nik_buat,keterangan,no_tap,kode_vendor,no_dokumen,modul,no_sanggup,nilai,ppn,no_pesan,no_spph,tgl_spph,no_sph,tgl_sph,no_ba,flag_revisi, tgl_mulai,tgl_selesai,kode_akun) values " +
						"('" + this.e_nb.getText() + "','-','" + this.app._lokasi + "',getdate(),'" + this.app._userLog + "','" + this.e_periode.getText() + "','" + this.dp_d1.getDateString() + "','" + this.app._userLog + "','" + this.e_ket.getText() + "','-','" + this.cb_vendor.getText() + "','" + this.e_dok.getText() + "','TL','-'," + nilaiToFloat(this.e_nilai.getText()) + ",0,'" + this.cb_pesan.getText() + "','-','" + this.dp_d1.getDateString() + "','-','" + this.dp_d1.getDateString() + "','-','0', '"+this.dp_d2.getDateString()+"','"+this.dp_d3.getDateString()+"','"+this.akunPesan+"')");
