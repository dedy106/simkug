window.app_saku3_transaksi_ypt_logistik_fFasusutDrk = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_logistik_fFasusutDrk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_logistik_fFasusutDrk";
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:8,tag:9,
		            colTitle:["Akun BP","Nama Akun","Akun Deprs","Nama Akun","Kode PP","Nama PP","Kode DRK","Nilai"],
					colWidth:[[7,6,5,4,3,2,1,0],[100,90,150,80,180,80,180,80]],
		...
		this.sg2 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:9,tag:9,
					...
					colWidth:[[8,7,6,5,4,3,2,1,0],[90,90,90,120,80,150,80,150,80]],
...
window.app_saku3_transaksi_ypt_logistik_fFasusutDrk.extend(window.childForm);
window.app_saku3_transaksi_ypt_logistik_fFasusutDrk.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						...
						sql.add("insert into fasusut_j(no_fasusut,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_drk,kode_pp,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
								"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',1,'"+line.akun_deprs+"','"+this.e_ket.getText()+"','C',"+line.nilai_susut+",'"+line.kode_drk+"','"+line.kode_pp_susut+"','"+this.app._lokasi+"','FA_SSTREG','AP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");
					}
					...
					sql.add("insert into fasusut_d(no_fasusut,no_fa,periode,nilai,kode_lokasi,akun_bp,akun_ap,kode_akun,kode_pp,kode_drk,dc,no_del,nilai_aset,umur) "+
							...
							"		  else (zz.nilai-a.nilai_residu-a.akum_nilai-isnull(b.tot_susut,0)) end as nilai_susut, "+
	...
	doLoadData: function(sender){		
		...
		var strSQL = "select b.akun_bp,x.nama as nama_bp,b.akun_deprs,y.nama as nama_deprs,a.kode_pp_susut,c.nama as nama_pp,b.kode_drk, "+
						...
						"		 then ceiling((zz.nilai-a.akum_nilai)/a.umur)  "+
						"		 else ceiling(zz.nilai-a.nilai_residu-a.akum_nilai-isnull(d.tot_susut,0)) end) as nilai_susut "+
						"from fa_asset a "+
						"inner join (select kode_lokasi,no_fa,sum(case dc when 'D' then nilai else -nilai end) as nilai "+
						"            from fa_nilai where periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' group by kode_lokasi,no_fa) zz on a.no_fa=zz.no_fa and a.kode_lokasi=zz.kode_lokasi "+
						...
						"left join "+
						"   (select no_fa,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as tot_susut "+
						"	  from fasusut_d where kode_lokasi = '"+this.app._lokasi+"' and no_del='-' group by no_fa,kode_lokasi) d on a.no_fa=d.no_fa and a.kode_lokasi=d.kode_lokasi "+					 
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU.rs.rows[i];							
			this.sg.appendData([line.akun_bp,line.nama_bp,line.akun_deprs,line.nama_deprs,line.kode_pp_susut,line.nama_pp,line.kode_drk,floatToNilai(line.nilai_susut)]);
		}
	...
	doHitungGar: function(){
		...
		for (var i=0;i < this.dataJU.rs.rows.length;i++){
			...
			if (!isAda) {
				this.sg2.appendData([line.akun_bp,line.nama_bp,line.kode_pp_susut,line.nama_pp,line.kode_drk,'-',"0",floatToNilai(nilai),"0"]);
			} 
			else { 
