window.app_saku3_transaksi_ypt_logistik_fFaInputM = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_logistik_fFaInputM.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_logistik_fFaInputM";
		...
		this.e_fa = new saiLabelEdit(this,{bound:[20,10,200,20],caption:"Format Aktap",readOnly:true,tag:2});
		this.e_jml = new saiLabelEdit(this,{bound:[20,12,200,20],caption:"Jumlah Item", tag:1, tipeText:ttNilai, text:"1",change:[this,"doChange"]});		
		...
		this.cb_lokfa = new saiCBBL(this,{bound:[20,19,220,20],caption:"Kode Lok Asset", multiSelection:false, maxLength:10, tag:1});
		...
		try {
			...
			this.cb_lokfa.setSQL("select kode_lok, nama from fa_lokasi where kode_lokasi='"+this.app._lokasi+"'",["kode_lok","nama"],false,["Kode","Nama"],"and","Data Lokasi Asset",true);
...
window.app_saku3_transaksi_ypt_logistik_fFaInputM.extend(window.childForm);
window.app_saku3_transaksi_ypt_logistik_fFaInputM.implement({
	...
	doClick:function(sender){
		this.e_fa.setText(this.app._lokasi+"-FA"+this.e_periode.getText().substr(2,4)+".XXX");
		this.e_jml.setFocus();
	},
