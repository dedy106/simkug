window.app_saku3_transaksi_ypt_logistik_fKbBAdir = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_logistik_fKbBAdir.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_logistik_fKbBAdir";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Logistik Ref BAST", 0);	
		...
		this.pc2 = new pageControl(this,{bound:[10,10,1000,430], childPage:["Data Pembayaran","List Pembayaran"]});		
		this.sg3 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:6,tag:9,
		            colTitle:["No Bukti","Tanggal","Deskripsi","No BAST","Nilai","Pilih"],
					colWidth:[[5,4,3,2,1,0],[70,100,100,300,80,100]],
		...
		this.e_nb = new portalui_saiLabelEdit(this.pc2.childPage[0],{bound:[20,11,200,20],caption:"No KasBank",maxLength:30,readOnly:true});
		...
		this.cb_kas = new saiCBBL(this.pc2.childPage[0],{bound:[20,12,220,20],caption:"Akun KasBank", multiSelection:false, maxLength:10, tag:2});		

		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,88,996,304], childPage:["Data BAST","Detail BAST","Jurnal Tambahan","Control Budget"]});		
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:8,tag:0,
		            colTitle:["Pilih","No BAST","Tanggal","Keterangan","Akun Hutang","Nilai BA","Saldo BA","Mitra"],
					colWidth:[[7,6,5,4,3,2,1,0],[160,100,100,70,280,70,100,70]],	
					click:[this,"doSgBtnClick"], colAlign:[[0],[alCenter]],													 									
					dblClick:[this,"doDoubleClick"],readOnly:true,
					colFormat:[[0,5,6],[cfButton,cfNilai,cfNilai]],autoAppend:false,defaultRow:1});
		...
		this.e_nilai = new saiLabelEdit(this.pc1.childPage[1],{bound:[520,17,200,20],caption:"Nilai Dibayar", tag:4, tipeText:ttNilai, text:"0",change:[this,"doChange"]});				
		this.e_akun = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,16,220,20],caption:"Kode Akun", readOnly:true,tag:4});				
		this.e_namaakun = new saiLabelEdit(this.pc1.childPage[1],{bound:[250,16,220,20],labelWidth:0, caption:"", readOnly:true,tag:4});						
		this.e_ppn = new saiLabelEdit(this.pc1.childPage[1],{bound:[520,16,200,20],caption:"Nilai PPN", tag:4, readOnly:true, tipeText:ttNilai, text:"0"});				
		this.e_total = new saiLabelEdit(this.pc1.childPage[1],{bound:[770,16,200,20],caption:"Total Pembayaran", tag:4, readOnly:true, tipeText:ttNilai, text:"0"});				

		this.sg5 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,149],colCount:9,tag:9,
		...
		this.sg1 = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:9,tag:9,visible:true,//153
		            colTitle:["Kode Akun","Nama Akun","DC","Keterangan","Nilai","Kode PP","Nama PP","Kode DRK","Nama DRK"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[150,80,150,80,100,270,50,200,80]],					
		...
		this.sgn1 = new portalui_sgNavigator(this.pc1.childPage[2],{bound:[1,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:2,grid:this.sg1,visible:true});//179		
		
		this.sg2 = new saiGrid(this.pc1.childPage[3],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:9,tag:9,visible:true, //210 -- 90
		...
		this.sgn2 = new portalui_sgNavigator(this.pc1.childPage[3],{bound:[1,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:3,grid:this.sg2,visible:true});
		...
		try {
			...
			this.cb_kas.setSQL("select a.kode_akun, a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag in ('001','009') where a.kode_lokasi='"+this.app._lokasi+"'",["kode_akun","nama"],false,["Kode","Nama"],"and","Data Akun",true);

			this.flagDokFree = "0"; 
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('PPNM','DOKFREE') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku3_transaksi_ypt_logistik_fKbBAdir.extend(window.childForm);
window.app_saku3_transaksi_ypt_logistik_fKbBAdir.implement({	
	dataHutang:function(sender){				
		if (this.e_periode.getText()!="") {			
			if (this.app._userStatus == "A") var vPP = " "; 
			else  var vPP = "a.kode_pp = '"+this.app._kodePP+"' and "; 
			...
			var strSQL = "select a.no_hutang,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.akun_hutang,a.nilai,a.nilai-isnull(pb,0) as saldo,b.kode_vendor+' - '+b.nama as vendor "+
						...
						"                           group by kode_lokasi,no_hutang "+
						"				  ) c on a.no_hutang=c.no_hutang and a.kode_lokasi=c.kode_lokasi "+

						"where "+vPP+" a.modul='LOGBAST' and a.nilai-isnull(pb,0) > 0 and a.kode_lokasi='"+this.app._lokasi+"' and a.periode <='"+this.e_periode.getText()+"'";		
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,4])){
				try{														
					...
					sql.add("insert into yk_pb_m (no_pb,no_dokumen,kode_lokasi,periode,nik_user,tgl_input,tanggal,due_date,keterangan,nilai,modul,progress,kode_pp,nik_app,nik_tahu,no_hutang,no_app,no_spb,no_ver,kode_bidang,kode_loktuj,nilai_final,no_agenda) values  "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.dp_d1.getDateString()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_total.getText())+",'PBAKRU','X','"+this.app._kodePP+"','"+this.app._userLog+"','"+this.app._userLog+"','"+this.e_nohutang.getText()+"','-','-','-','-','"+this.app._lokasi+"',"+nilaiToFloat(this.e_nilai.getText())+",'-')");					
					
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_nohutang.getText()+"','-','"+this.cb_kas.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBLOGBA','BK','"+this.e_periode.getText()+"','IDR',1,"+nilaiToFloat(this.e_total.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','-','-','-')");					
					
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.e_akun.getText()+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBLOGBA','HUT','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");				
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',99,'"+this.cb_kas.getText()+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_total.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBLOGBA','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");									
					if (nilaiToFloat(this.e_ppn.getText()) != 0) {
						sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
								"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',1,'"+this.akunPPN+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_ppn.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBLOGBA','PPN','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");										
					}
					...
					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){
								var j = 1000+i;
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i)+"',"+nilaiToFloat(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"','"+this.sg1.cells(7,i)+"','-','-','"+this.app._lokasi+"','KBLOGBA','TAMBAH','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");												
							}
						}
					}
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"('"+this.e_nb.getText()+"','"+this.modul+"','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(6,i))+","+nilai+")");
							}
						}
					}
	...
	doSgBtnClick: function(sender, col, row){
		try{
			if (col == 0) this.doDoubleClick(this.sg3,0,row); 				
		}catch(e){
			alert(e);
		}
	},
	doDoubleClick: function(sender, col , row) {		
		if (this.sg.cells(1,row) != "") {
			...
			this.e_saldo.setText(this.sg.cells(6,row));
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];												
					this.sg5.appendData([line.kode_akun,line.nama_akun,line.dc,line.keterangan,floatToNilai(line.nilai),line.kode_pp,line.nama_pp,line.kode_drk,line.nama_drk]);
				}
			} else this.sg5.clear(1);							
		}
	},	
	doLoad3:function(sender){			
		if (this.app._userStatus == "A") var pp = ""; 
		else var pp = " a.kode_pp = '"+this.app._kodePP+"' and "; 

		var strSQL = "select a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai "+
		             "from kas_m a "+					 					 
					 "where "+pp+" a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'KBLOGBA' and a.posted='F' ";					 
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_kas,line.tgl,line.keterangan,line.no_dokumen,floatToNilai(line.nilai),"Pilih"]); 
		}
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				this.noHut = this.sg3.cells(3,row);
				...
				var strSQL = "select a.*,b.akun_kb from yk_pb_m a inner join kas_m b on a.no_pb=b.no_kas and a.kode_lokasi=b.kode_lokasi where a.no_pb='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){																			
				...
				var strSQL = "select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk "+
							 "from kas_j a "+
							 "inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
							 "inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
							 "left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and substring(a.periode,1,4)=d.tahun "+
							 "where a.no_kas='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.jenis='TAMBAH'";		
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg1.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData([line.kode_akun,line.nama_akun,line.dc,line.keterangan,line.kode_pp,line.nama_pp,line.kode_drk,line.nama_drk]);
					}					
	...
	doNilaiChange1: function(){		
		try{
			...
			tot = tot + nilaiToFloat(this.e_nilai.getText()) + nilaiToFloat(this.e_ppn.getText());
			this.e_total.setText(floatToNilai(tot));	
