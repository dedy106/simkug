window.app_saku3_transaksi_ypt_logistik_fTerima = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_logistik_fTerima.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_logistik_fTerima";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penerimaan Barang/Jasa", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,258], childPage:["Item SPK","Detail Barang","Dokumen Pendukung"]});
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-32],colCount:13,
		            colTitle:["ID Barang","Item Barang","Merk","Tipe","Harga","Sisa SPK","Jml Terima","Kode Lok","Nama Lokasi","Kode Akun","Kode Klp","Nama Klp","Jenis"],
		...
		this.sg2 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:11,
		            colTitle:["ID Barang","No Tag","Item Barang","Merk","Tipe","No Seri","Harga","Kode Akun","Klp Aset","Jenis","KodeLok FA"],
		...
		this.sgUpld = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-63],colCount:5, tag:9,
...
window.app_saku3_transaksi_ypt_logistik_fTerima.extend(window.childForm);
window.app_saku3_transaksi_ypt_logistik_fTerima.implement({	
				...
				try{														
					...
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i) && this.sgUpld.cells(2,i)!="-") {							
							...
							sql.add("insert into log_pesan_dok(no_pesan,no_gambar,nu,kode_jenis,kode_lokasi,modul,no_ref) values ('"+this.e_nb.getText()+"','"+this.sgUpld.cells(3,i).tmpfile+"','"+ix+"','"+this.sgUpld.cells(0,i)+"','"+this.app._lokasi+"','BATRM','"+this.e_nb.getText()+"')");															
						}	
					}	
			...
			if (sender == this.sg) {				
				if (col == 7){
					this.standarLib.showListData(this, "Daftar Lokasi",sender,undefined, 
							...
							"select count(a.kode_lok) from (select kode_lok from fa_lokasi where kode_lokasi='"+this.app._lokasi+"' ) a ", 
				...
				if (nilaiToFloat(this.sg.cells(6,i)) > nilaiToFloat(this.sg.cells(5,i))) {
					...
					for (var k=0;k < this.sg2.getRowCount();k++){
						if (this.sg2.rowValid(k)){
							//if (formatIDKunci == this.sg2.cells(1,k).substr(0,11)) 
							if (formatIDKunci == this.sg2.cells(1,k).substr(0,this.sg2.cells(1,k).length-4)) 
	...
	doChange:function(sender){	
		try {	
			if (sender == this.cb_vendor && this.cb_vendor.getText() != "") {				
				if (this.e_pic.getText()=="") this.e_pic.setText(this.cb_vendor.rightLabelCaption);
			}	
			...
			if (sender == this.cb_po && this.cb_po.getText()!="" && this.stsSimpan == 1) {
				...
				var strSQL = "select a.no_spk+'-'+cast(a.no_urut as varchar) as id,a.item,a.merk,a.tipe,a.harga+a.ppn as harga,a.jumlah-isnull(d.jml_terima,0) as jumlah,"+
							"'-' as kode_lok,'-' as nama_lok, b.kode_akun,c.kode_klpfa,c.nama as nama_klpfa,c.jenis "+
							"from log_spk_d a "+
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.no_spk+'-'+cast(a.no_urut as varchar) as id,a.item,a.merk,a.tipe,a.harga+a.ppn as harga,a.jumlah-isnull(d.jml_terima,0) as jumlah,ee.kode_lokfa as kode_lok,ee.nama as nama_lok,bb.kode_akun,c.kode_klpfa,c.nama as nama_klpfa,c.jenis,e.jum_aset "+
						 ...
						 "                 inner join (select distinct a.id_pesan,a.no_po,a.kode_lokfa,b.nama from fa_asset a inner join fa_lokasi b on a.kode_lokfa=b.kode_lok and a.kode_lokasi=b.kode_lokasi where a.no_baps='"+this.e_nb.getText()+"' and a.no_po='"+this.cb_po.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"') ee on ee.id_pesan=a.no_spk+'-'+cast(a.no_urut as varchar) and ee.no_po=a.no_spk "+						 
