window.app_saku3_transaksi_ypt_logistik_fVendor = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_logistik_fVendor.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_logistik_fVendor";		
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,450], childPage:["Daftar Vendor","Data Vendor","Histori PKS","Data Pendukung","Filter Cari"]});
		...
		this.i_gen = new portalui_imageButton(this.pc1.childPage[1],{bound:[225,10,20,20],hint:"Generate",image:"icon/"+system.getThemes()+"/tabCont2.png",click:[this,"doGen"]});		
		...
		this.c_btrans = new saiCB(this.pc1.childPage[1],{bound:[290,12,230,20],caption:"Bank Transfer",items:["MANDIRI","BNI","BCA", "BJB", "BRISYariah", "SYARIAH MANDIRI","BRI"], readOnly:true,tag:2});		
		...
		this.cb_klp = new saiCBBL(this.pc1.childPage[1],{bound:[20,13,270,20],caption:"Kelompok Vendor", multiSelection:false, maxLength:10, tag:0});						
		this.cb_akun = new saiCBBL(this.pc1.childPage[1],{bound:[20,16,270,20],caption:"Akun Hutang", multiSelection:false, maxLength:10, tag:0});						
...
window.app_saku3_transaksi_ypt_logistik_fVendor.extend(window.childForm);
window.app_saku3_transaksi_ypt_logistik_fVendor.implement({	
	doGen: function() {
		this.cb_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"vendor","kode_vendor","V-","000"));						
	},
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into vendor(kode_vendor,kode_lokasi,nama,alamat,no_tel,email,npwp,pic,alamat2,bank,cabang,no_rek,nama_rek,no_fax,no_pictel,spek,kode_klpvendor,penilaian,bank_trans,jenis, akun_hutang) values "+
						    "('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.e_alamat.getText()+"','"+this.e_tel.getText()+"','"+this.e_mail.getText()+"','"+this.e_npwp.getText()+"','"+this.e_pic.getText()+"','"+this.e_alamat2.getText()+"','"+this.e_bank.getText()+"','"+this.e_cabang.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"','"+this.e_fax.getText()+"','"+this.e_pictel.getText()+"','"+this.e_spek.getText()+"','"+this.cb_klp.getText()+"','"+this.e_nilai.getText()+"','"+this.c_btrans.getText()+"','-','"+this.cb_akun.getText()+"')"); //,'"+this.c_flag.getText().substr(0,1)+"'
					for (var i=0;i < this.sg2.getRowCount();i++){
						if (this.sg2.rowValid(i)){							
							sql.add("insert into vendor_pks(kode_vendor,kode_lokasi,tgl_mulai,tgl_selesai,no_pks) values "+
									"('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.sg2.getCellDateValue(1,i)+"','"+this.sg2.getCellDateValue(2,i)+"','"+this.sg2.cells(0,i)+"')");							
						}	
					}
					var ix=0;
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					...
					sql.add("delete from vendor_pks where kode_vendor='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					
					sql.add("update vendor set nama='"+this.e_nama.getText()+"',alamat='"+this.e_alamat.getText()+"',no_tel='"+this.e_tel.getText()+"',email='"+this.e_mail.getText()+"',npwp='"+this.e_npwp.getText()+"',pic='"+this.e_pic.getText()+"',alamat2='"+this.e_alamat2.getText()+"',bank='"+this.e_bank.getText()+"',cabang='"+this.e_cabang.getText()+"',no_rek='"+this.e_norek.getText()+"',nama_rek='"+this.e_namarek.getText()+"',no_fax='"+this.e_fax.getText()+"',no_pictel='"+this.e_pictel.getText()+"',spek='"+this.e_spek.getText()+"',kode_klpvendor='"+this.cb_klp.getText()+"',penilaian='"+this.e_nilai.getText()+"',bank_trans='"+this.c_btrans.getText()+"',jenis='-', akun_hutang='"+this.cb_akun.getText()+"' "+ //,flag='"+this.c_flag.getText().substr(0,1)+"'
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select kode_vendor,nama,alamat,no_tel,email,npwp,alamat2,pic,bank,cabang,no_rek,nama_rek,no_fax,no_pictel,spek,kode_klpvendor,penilaian,bank_trans,akun_pph,jenis,akun_hutang "+//,flag 
				             "from vendor "+
						     "where kode_vendor ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
				...
				if (typeof data == "object"){
					...
					if (line != undefined){						
						...
						var data = this.dbLib.getDataProvider("select no_pks,convert(varchar,tgl_mulai,103) as tgl1,convert(varchar,tgl_selesai,103) as tgl2 "+
						           "from vendor_pks  "+								   
								   "where kode_vendor = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' order by tgl_selesai desc",true);
						if (typeof data == "object" && data.rs.rows[0] != undefined){
							var line;
							this.sg2.clear();
							for (var i in data.rs.rows){
	...
	doCari:function(sender){								
		try {
			...
			var strSQL = "select a.kode_vendor,a.nama,a.alamat,a.kode_klpvendor+' - '+b.nama as klp,a.bank_trans,a.bank+' - '+a.cabang as bank,a.no_rek+' - '+a.nama_rek as rek,a.kode_klpvendor "+
						 ...
						 "where "+filter+" a.kode_lokasi='"+this.app._lokasi+"' order by a.kode_klpvendor,a.kode_vendor";				
