window.app_saku3_transaksi_ypt_logistik_fPesanRevisi = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_logistik_fPesanRevisi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_logistik_fPesanRevisi";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Revisi Purchase Request", 0);	
		...
		this.pc2 = new pageControl(this,{bound:[10,10,1000,430], childPage:["List Request","Data Request"]});		
		this.sg3 = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:6,tag:9,
		...
		this.c_jenis = new portalui_saiLabelEdit(this.pc2.childPage[1],{bound:[20,22,200,20],caption:"Jenis",readOnly:true,tag:2, change:[this,"doChange"]});
		this.e_nb = new portalui_saiLabelEdit(this.pc2.childPage[1],{bound:[20,12,200,20],caption:"No Request",maxLength:30,readOnly:true});
		
		this.pc1 = new pageControl(this.pc2.childPage[1],{bound:[1,20,995,350], childPage:["Anggaran","Detail","Otorisasi","Maksud-Tujuan","Aspek Strategis"]});			
...
window.app_saku3_transaksi_ypt_logistik_fPesanRevisi.extend(window.childForm);
window.app_saku3_transaksi_ypt_logistik_fPesanRevisi.implement({	
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					//backup
					sql.add("insert into log_pesan_m_h(no_pesan,kode_lokasi,tgl_input,nik_user,periode,jenis,tanggal,no_dokumen,keterangan,kode_pp,kode_drk,kode_akun,saldo_gar,nilai,nik_buat,nik_app,kode_dana,no_app,progress,maksud,aspek,no_terima,no_spph,lok_proses,kode_ppaju) "+
						    "select no_pesan,kode_lokasi,tgl_input,nik_user,periode,jenis,tanggal,no_dokumen,keterangan,kode_pp,kode_drk,kode_akun,saldo_gar,nilai,nik_buat,nik_app,kode_dana,no_app,progress,maksud,aspek,no_terima,no_spph,lok_proses,kode_ppaju "+
						    "from log_pesan_m where no_pesan='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'"); 							
					sql.add("insert into log_pesan_d_h(no_pesan,kode_lokasi,no_urut,item,merk,tipe,catatan,jumlah,nilai,harga,no_po,no_ba,kode_dana,jum_po,kode_klpfa,ppn) "+
							"select no_pesan,kode_lokasi,no_urut,item,merk,tipe,catatan,jumlah,nilai,harga,no_po,no_ba,kode_dana,jum_po,kode_klpfa,ppn "+
							"from log_pesan_d where no_pesan='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'"); 

					
					//flag untuk amandemen SPK/PO
					if (this.noSPK != "-") sql.add("update log_spk_m set flag_revisi='1' where no_spk='"+this.noSPK+"' and kode_lokasi='"+this.app._lokasi+"'");
					
					//update		
					sql.add("update log_pesan_m set tgl_input=getdate(),nik_user='"+this.app._userLog+"',no_dokumen='"+this.e_dok.getText()+"',keterangan='"+this.e_ket.getText()+"',saldo_gar="+nilaiToFloat(this.e_gar.getText())+",nilai="+nilaiToFloat(this.e_total.getText())+" "+
							"where no_pesan='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");							
	...
	doLoad3:function(sender){																				
		var strSQL = "select a.no_pesan,convert(varchar,a.tanggal,103) as tgl,jenis,a.no_dokumen,a.keterangan,a.nilai "+
		             "from log_pesan_m a "+					
					 ...
					 "where a.periode like '"+this.e_periode.getText().substr(0,4)+"%' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress in ('1','2','3') and a.kode_pp='"+this.app._kodePP+"'";		
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.*,b.no_gambar,isnull(c.catatan,'-') as catatan,isnull(d.no_spk,'-') as no_spk from log_pesan_m a "+
							 "inner join log_pesan_dok b on a.no_pesan=b.no_pesan and a.kode_lokasi=b.kode_lokasi "+
							 "left join log_spk_m d on a.no_pesan=d.no_pesan and a.kode_lokasi=d.kode_lokasi "+
