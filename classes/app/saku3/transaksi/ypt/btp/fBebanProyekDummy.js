window.app_saku3_transaksi_ypt_btp_fBebanProyekDummy = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_btp_fBebanProyekDummy.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_btp_fBebanProyekDummy";
		...
		this.pc2 = new pageControl(this,{bound:[10,10,1000,450], childPage:["Data Unbill","List Unbill"]});		
		...
		this.cb_vendor = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,15,220,20],caption:"Vendor",tag:1,multiSelection:false,readOnly:true}); 								
		this.e_ket = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,16,550,20],caption:"Uraian", maxLength:150});									
		this.cb_akun = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,12,220,20],caption:"Akun Beban",tag:0,readOnly:true}); 						
		this.cb_hutang = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,13,220,20],caption:"Akun Hutang",tag:0,readOnly:true}); 						
		this.cb_drk = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,14,220,20],caption:"DRK",tag:0,readOnly:true}); 						
		this.e_saldo = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,17,200,20],caption:"Saldo OR", tag:1, tipeText:ttNilai, text:"0",readOnly:true});
		this.e_nilai = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,16,200,20],caption:"Nilai Beban", tag:1, tipeText:ttNilai, text:"0", change:[this,"doChange"]});
		...
		try {
			...
			this.cb_hutang.setSQL("select kode_akun, nama from masakun where kode_lokasi='"+this.app._lokasi+"'",["kode_akun","nama"],false,["Kode","Nama"],"and","Data Akun",true);
...
window.app_saku3_transaksi_ypt_btp_fBebanProyekDummy.extend(window.childForm);
window.app_saku3_transaksi_ypt_btp_fBebanProyekDummy.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','PR','BBNDUMMY','F','-','-','"+this.cb_pp.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_proyek.getText()+"','"+this.e_ket.getText()+"','IDR',1,"+
							parseNilai(this.e_nilai.getText())+",0,0,'"+this.cb_app.getText()+"','-','-','-','-','-','"+this.cb_vendor.getText()+"','-','-')");
					...
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',1,'"+this.cb_hutang.getText()+"','C',"+parseNilai(this.e_nilai.getText())+","+parseNilai(this.e_nilai.getText())+",'"+this.e_ket.getText()+"','PR','HUTDUMMY','IDR',1,'"+this.cb_pp.getText()+"','-','-','"+this.cb_vendor.getText()+"','-','-','-','-','-')");					
					...
					sql.add("insert into pr_or_d (no_bukti,kode_lokasi,periode,kode_proyek,kode_akun,dc,nilai,jenis,kode_pp,kode_drk,no_reverse) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.cb_akun.getText()+"','D',"+nilaiToFloat(this.e_nilai.getText())+",'BBNDUMMY','"+this.cb_pp.getText()+"','"+this.cb_drk.getText()+"','-')");								
			...
			case "ubah" :									
				this.preView = "1";
				 if (nilaiToFloat(this.e_saldo.getText()) < nilaiToFloat(this.e_nilai.getText()) ){
					system.alert(this,"Transaksi tidak valid.","Nilai tidak boleh melebihi Saldo OR.");
					return false;
				}
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_proyek && this.cb_proyek.getText()!="") {
			if (this.stsSimpan==1) this.e_ket.setText(this.cb_proyek.rightLabelCaption);
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){							
							if (this.preView == "1") {
								this.nama_report="server_report_saku3_produk_rptProyekBebanDummy";
	...
	doLoad3:function(sender){																								
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai1 as nilai "+
		             "from trans_m a inner join pr_or_d b on a.no_bukti=b.no_bukti and a.kode_lokasi=b.kode_lokasi and b.jenis='BBNDUMMY' and b.no_reverse='-' "+
		             "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and posted='F' and modul ='PR' and form='BBNDUMMY'";	
		var data = this.dbLib.getDataProvider(strSQL,true);
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				this.cb_proyek.setSQL("select a.kode_proyek, a.nama "+
									  "from pr_proyek a inner join trans_m b on a.kode_proyek =b.no_dokumen and a.kode_lokasi=b.kode_lokasi and b.modul='PR' and b.form='BBNDUMMY' "+
