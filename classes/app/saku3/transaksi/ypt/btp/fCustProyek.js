window.app_saku3_transaksi_ypt_btp_fCustProyek = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_btp_fCustProyek.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_btp_fCustProyek";
		...
		uses("saiGrid",true);		
		this.pc1 = new pageControl(this,{bound:[20,12,1000,450], childPage:["List Cust","Entry Data","Filter Cari"]});				
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:5,tag:9,
		            colTitle:["Kode","Nama","Divisi","Alamat","PIC"],
					colWidth:[[4,3,2,1,0],[150,200,200,300,80]],
					readOnly:true,
		...
		this.cb_piutang = new saiCBBL(this.pc1.childPage[1],{bound:[20,12,220,20],caption:"Akun Piutang", multiSelection:false, maxLength:10, tag:2});		
		...
		this.e_dep = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,17,500,20],caption:"Divisi", maxLength:50, tag:1});	
		this.e_email = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,18,500,20],caption:"Email", maxLength:50, tag:1});	
		this.e_nohp = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,19,300,20],caption:"Hand Phone", maxLength:50, tag:1});	
		this.e_fax2 = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,20,300,20],caption:"Faximile PIC", maxLength:50, tag:1});	
		this.cb_klp = new saiCBBL(this.pc1.childPage[1],{bound:[20,12,220,20],caption:"Grup Cust", multiSelection:false, maxLength:10, tag:2});		
		...
		try {
			...
			this.cb_klp.setSQL("select kode_klpcust, nama from cust_klp where kode_lokasi = '"+this.app._lokasi+"'",["kode_klpcust","nama"],false,["Kode","Nama"],"and","Data Kelompok",true);
...
window.app_saku3_transaksi_ypt_btp_fCustProyek.extend(window.childForm);
window.app_saku3_transaksi_ypt_btp_fCustProyek.implement({
	doCari: function() {
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into cust(kode_cust,kode_lokasi,nama, alamat,kota,no_tel,no_fax,npwp,pic,departemen,email,no_hp,no_fax2,alamat2,kode_tagih,inisial, akun_piutang,kode_klpcust) values "+
							"('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"', '"+this.e_alamat.getText()+"','"+this.e_kota.getText()+"','"+this.e_tel.getText()+"','"+this.e_fax.getText()+"','"+this.e_npwp.getText()+"','"+this.e_pic.getText()+"','"+this.e_dep.getText()+"','"+this.e_email.getText()+"','"+this.e_nohp.getText()+"','"+this.e_fax2.getText()+"','"+this.e_alamat.getText()+"','"+this.cb_kode.getText()+"','-', '"+this.cb_piutang.getText()+"','"+this.cb_klp.getText()+"')");		
					setTipeButton(tbAllFalse);
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select * from cust where kode_cust ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
				var data = this.dbLib.getDataProvider(strSQL,true);
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_cust,line.nama,line.departemen,line.alamat,line.pic]); 
		}
