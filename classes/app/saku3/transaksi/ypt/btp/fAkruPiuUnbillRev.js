window.app_saku3_transaksi_ypt_btp_fAkruPiuUnbillRev = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_btp_fAkruPiuUnbillRev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_btp_fAkruPiuUnbillRev";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Reverse Piutang Proyek UnBill", 0);	
		...
		this.pc2 = new pageControl(this,{bound:[10,10,1000,430], childPage:["Data Reverse","List Reverse"]});		
		...
		this.sg1 = new saiGrid(this.pc2.childPage[0],{bound:[0,5,this.pc2.width-5,250],colCount:9,tag:0,
				colTitle:["Status","No Referensi","Tanggal","Keterangan","Akun Piutang","Akun Pdpt","DRK","Nilai Unbill","PPN Unbill"],
				colWidth:[[8,7,6,5,4,3,2,1,0],[100,100,80,80,80,300,80,150,80]],
...
window.app_saku3_transaksi_ypt_btp_fAkruPiuUnbillRev.extend(window.childForm);
window.app_saku3_transaksi_ypt_btp_fAkruPiuUnbillRev.implement({
	doLoadCBproyek: function() {			
			var strSQL = "select distinct a.kode_proyek,a.nama as keterangan "+
						 "from pr_proyek a inner join piutang_d b on a.kode_proyek=b.kode_project and a.kode_lokasi=b.kode_lokasi and b.no_dokumen = '-' and b.modul='UNBILL' "+
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {						
						...
						sql.add("update piutang_d set no_dokumen='-' where no_dokumen = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");																												
					}					
					
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','PR','REVUNBILL','F','-','-','"+this.cb_pp.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_proyek.getText()+"','"+this.e_ket.getText()+"','IDR',1,"+
					...
					if (this.sg1.getRowValidCount() > 0) {
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i) && this.sg1.cells(0,i) == "REVERSE"){
								var total = nilaiToFloat(this.sg1.cells(7,i))+nilaiToFloat(this.sg1.cells(8,i));
								
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.sg1.cells(5,i)+"','D',"+parseNilai(this.sg1.cells(7,i))+","+parseNilai(this.sg1.cells(7,i))+",'"+this.e_ket.getText()+"','PR','PDPT','IDR',1,'"+this.cb_pp.getText()+"','"+this.sg1.cells(6,i)+"','-','-','-','-','-','-','-')");								

								if (nilaiToFloat(this.sg1.cells(8,i)) != 0) {
									sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
											"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',1,'"+this.akunHutPPN+"','D',"+parseNilai(this.sg1.cells(8,i))+","+parseNilai(this.sg1.cells(8,i))+",'"+this.e_ket.getText()+"','PR','HPPN','IDR',1,'"+this.cb_pp.getText()+"','-','"+this.cb_cust.getText()+"','-','-','-','-','-','-')");					
								}

								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',2,'"+this.sg1.cells(4,i)+"','C',"+total+","+total+",'"+this.e_ket.getText()+"','PR','REVUNBILL','IDR',1,'"+this.cb_pp.getText()+"','-','"+this.cb_cust.getText()+"','-','-','-','-','-','-')");					
								
								sql.add("update piutang_d set no_dokumen='"+this.e_nb.getText()+"' where no_piutang = '"+this.sg1.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"'");	
								sql.add("insert into piutang_d (no_piutang,kode_lokasi,no_dokumen,no_fp,tanggal,kode_pp,akun_piutang,keterangan,kode_project,kode_cust,kode_curr,kurs,nilai,nilai_ppn,nilai_pph,periode,nik_user,tgl_input,modul,dc) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.sg1.cells(1,i)+"','-','"+this.dp_d1.getDateString()+"','"+this.cb_pp.getText()+"','"+this.sg1.cells(4,i)+"','"+this.sg1.cells(3,i)+"','"+this.cb_proyek.getText()+"','"+this.cb_cust.getText()+"','IDR',1,"+nilaiToFloat(this.sg1.cells(7,i))+","+nilaiToFloat(this.sg1.cells(8,i))+",0,'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'REVUNBILL','C')");
							}
						}
					}
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_proyek && this.cb_proyek.getText()!="") {
			if (this.stsSimpan==1) {
				...
				var strSQL = "select c.akun_piutang,e.akun_pdpt,c.nilai,c.nilai_ppn,b.kode_drkp,c.no_piutang,convert(varchar,d.tanggal,103) as tgl,d.keterangan "+
							 "from piutang_d c "+
							 "inner join trans_m d on d.no_bukti=c.no_piutang and c.kode_lokasi=d.kode_lokasi "+						
							 "inner join pr_proyek b on c.kode_project =b.kode_proyek and c.kode_lokasi=b.kode_lokasi "+
							 "inner join pr_jenis e on b.kode_jenis=e.kode_jenis and b.kode_lokasi=e.kode_lokasi "+
							 "where c.no_dokumen='-' and c.kode_project='"+this.cb_proyek.getText()+"' and c.kode_lokasi='"+this.app._lokasi+"'";					
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData(["INPROG",line.no_piutang,line.tgl,line.keterangan,line.akun_piutang,line.akun_pdpt,line.kode_drkp,floatToNilai(line.nilai),floatToNilai(line.nilai_ppn)]);
					}
		...
		try{			
			...
			for (var i = 0; i < this.sg1.rows.getLength();i++) {
				if (this.sg1.rowValid(i) && this.sg1.cells(0,i)=="REVERSE" && this.sg1.cells(7,i) != "") {
					total += nilaiToFloat(this.sg1.cells(7,i))+nilaiToFloat(this.sg1.cells(8,i));					
				}
			}			
	...
	doLoad3:function(sender){																								
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai1+a.nilai2 as nilai "+
		             "from trans_m a "+
		             "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and posted='F' and modul ='PR' and form='REVUNBILL'";	
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				this.cb_proyek.setSQL("select a.kode_proyek, a.nama "+
									  "from pr_proyek a inner join trans_m b on a.kode_proyek =b.no_dokumen and a.kode_lokasi=b.kode_lokasi and b.modul='PR' and b.form='REVUNBILL' "+
				...
				var strSQL = "select c.akun_piutang,e.akun_pdpt,c.nilai,c.nilai_ppn,b.kode_drkp,c.no_piutang,convert(varchar,d.tanggal,103) as tgl,d.keterangan "+
						 	 "from piutang_d c "+
							 "inner join trans_m d on c.no_piutang=d.no_bukti and c.kode_lokasi=d.kode_lokasi "+						
							 "inner join pr_proyek b on c.kode_project =b.kode_proyek and c.kode_lokasi=b.kode_lokasi "+
							 "inner join pr_jenis e on b.kode_jenis=e.kode_jenis and b.kode_lokasi=e.kode_lokasi "+
							 "where c.no_dokumen='"+this.e_nb.getText()+"' and c.kode_project='"+this.cb_proyek.getText()+"' and c.kode_lokasi='"+this.app._lokasi+"'";					
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg1.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData(["REVERSE",line.no_piutang,line.tgl,line.keterangan,line.akun_piutang,line.akun_pdpt,line.kode_drkp,floatToNilai(line.nilai),floatToNilai(line.nilai_ppn)]);
					}
				} else this.sg1.clear(1);
