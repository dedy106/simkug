window.app_saku3_transaksi_ypt_btp_fBDDproyekRev = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_btp_fBDDproyekRev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_btp_fBDDproyekRev";
		...
		this.e_saldogar = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,18,200,20],caption:"Saldo Budget", tag:1, tipeText:ttNilai, text:"0", readOnly:true,visible:false});
...
window.app_saku3_transaksi_ypt_btp_fBDDproyekRev.extend(window.childForm);
window.app_saku3_transaksi_ypt_btp_fBDDproyekRev.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',1,'"+this.cb_bdd.getText()+"','C',"+parseNilai(this.e_nilai.getText())+","+parseNilai(this.e_nilai.getText())+",'"+this.e_ket.getText()+"','PR','BDD','IDR',1,'"+this.cb_pp.getText()+"','-','-','-','-','-','-','-','-')");								
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_proyek && this.cb_proyek.getText()!="") {
			...
			var strSQL = "select a.bdd - isnull(b.beban,0) as saldo "+
						 ...
						 "			select z.kode_proyek,sum(z.bdd) as bdd "+
						 "			from ( "+

						 "				select x.kode_proyek,isnull(SUM(case x.dc when 'D' then x.nilai else -x.nilai end),0) as bdd "+
						 "				from  pr_or_d x "+
						 " 				inner join ( select no_hutang,sum(case dc when 'D' then nilai else -nilai end) as bayar "+
						 "					 		from hutbayar_d where kode_lokasi='"+this.app._lokasi+"' "+
						 "					 		group by no_hutang  "+
						 "							) y on x.no_bukti=y.no_hutang and y.bayar<>0 "+
						 "				where x.jenis ='BDD' and x.kode_proyek='"+this.cb_proyek.getText()+"' and x.kode_lokasi='"+this.app._lokasi+"' group by x.kode_proyek "+								

						 "				union all "+
						 "				select x.kode_proyek,isnull(SUM(case x.dc when 'D' then x.nilai else -x.nilai end),0) as bdd "+
						 "				from  pr_or_d x "+
						 " 				inner join ptg_m y on x.no_bukti=y.no_ptg  "+
						 "				where x.jenis ='BDD' and x.kode_proyek='"+this.cb_proyek.getText()+"' and x.kode_lokasi='"+this.app._lokasi+"' group by x.kode_proyek "+								

						 "				union all "+
						 "				select x.kode_proyek,isnull(SUM(case x.dc when 'D' then x.nilai else -x.nilai end),0) as bdd "+
						 "				from  pr_or_d x "+
						 " 				inner join ptg_m y on x.no_bukti=y.no_kas  "+
						 "				where x.jenis ='BDD' and x.kode_proyek='"+this.cb_proyek.getText()+"' and x.kode_lokasi='"+this.app._lokasi+"' group by x.kode_proyek "+								

						 "			) z group by z.kode_proyek "+
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){							
							if (this.preView == "1") {
								this.nama_report="server_report_saku3_tpcc_proyek_rptJurnalBddRev";
