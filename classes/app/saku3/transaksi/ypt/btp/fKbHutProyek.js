window.app_saku3_transaksi_ypt_btp_fKbHutProyek = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_btp_fKbHutProyek.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_btp_fKbHutProyek";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pelunasan Hutang Vendor Proyek", 0);	
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:8,tag:0,
				colTitle:["Status","No Referensi","Tanggal","Keterangan","Akun Hutang","Tagihan","ID Proyek","Deskripsi"],
				colWidth:[[7,6,5,4,3,2,1,0],[200,100,100,100,400,80,150,80]],
...
window.app_saku3_transaksi_ypt_btp_fKbHutProyek.extend(window.childForm);
window.app_saku3_transaksi_ypt_btp_fKbHutProyek.implement({
	doLoad: function() {
		this.pc2.setActivePage(this.pc2.childPage[0]);	
		this.pc1.setActivePage(this.pc1.childPage[0]);																		
		if (this.stsSimpan==1 && this.cb_vendor.getText()!="") {		
			var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.kode_akun as akun_hutang,a.nilai-isnull(b.bayar,0) as saldo, d.kode_proyek,d.nama as ket_proyek  "+						  
						 "from trans_j a "+
						 "	   inner join trans_m c on a.no_bukti=c.no_bukti and a.kode_lokasi=c.kode_lokasi "+
						 "	   inner join pr_proyek d on c.no_dokumen=d.kode_proyek and c.kode_lokasi=d.kode_lokasi "+
						 "     left join ("+
						 "          select no_selesai,sum(case dc when 'D' then nilai else -nilai end) as bayar  "+
						 "          from trans_j where kode_vendor='"+this.cb_vendor.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and jenis<>'HUTDUMMY' "+
						 "			group by no_selesai,kode_lokasi "+
						 ") b on a.no_bukti=b.no_selesai "+
						 
						 "where a.jenis = 'HUTANG' and a.dc='C' and a.kode_vendor='"+this.cb_vendor.getText()+"' and (a.nilai-isnull(b.bayar,0)) > 0 and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_bukti";						 
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg1.appendData(["INPROG",line.no_bukti,line.tgl,line.keterangan,line.akun_hutang,floatToNilai(line.saldo),line.kode_proyek,line.ket_proyek]);					
				}
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.modul+"','KBHUTPR','F','-','-','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','-','"+this.e_ket.getText()+"','IDR',1,"+
					...
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && this.sg1.cells(0,i) == "BAYAR"){
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(4,i)+"','D',"+nilaiToFloat(this.sg1.cells(5,i))+","+
									nilaiToFloat(this.sg1.cells(5,i))+",'Pembayaran atas "+this.cb_vendor.rightLabelCaption+" No Ref "+this.sg1.cells(1,i)+"','"+this.modul+"','HUTANG','IDR',1,'"+this.app._kodePP+"','-','-','"+this.cb_vendor.getText()+"','-','"+this.sg1.cells(1,i)+"','-','-','-')");
						}
					}
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_produk_rptProyekBebanKb";
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai1 "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.form = 'KBHUTPR' and a.posted ='F'";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.kode_akun as akun_hutang,a.nilai-isnull(b.bayar,0) as saldo , dd.kode_proyek,dd.nama as ket_proyek "+						  
							 "from trans_j a "+
							 "     inner join trans_j c on a.no_bukti=c.no_selesai and c.kode_lokasi='"+this.app._lokasi+"' "+
							 "	   inner join trans_m cc on c.no_bukti=cc.no_bukti and c.kode_lokasi=cc.kode_lokasi "+
						 	 "	   inner join pr_proyek dd on cc.no_dokumen=dd.kode_proyek and cc.kode_lokasi=dd.kode_lokasi "+
							 "     left join ("+
							 "          select no_selesai,sum(case dc when 'D' then nilai else -nilai end) as bayar  "+
							 "          from trans_j where no_bukti <> '"+this.e_nb.getText()+"' and kode_vendor='"+this.cb_vendor.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
							 "			group by no_selesai,kode_lokasi "+
							 ") b on a.no_bukti=b.no_selesai "+
							
							 "where a.dc='C' and a.kode_vendor='"+this.cb_vendor.getText()+"' and (a.nilai-isnull(b.bayar,0)) > 0 and a.kode_lokasi='"+this.app._lokasi+"' and c.no_bukti='"+this.e_nb.getText()+"' order by a.no_bukti";						 
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData(["BAYAR",line.no_bukti,line.tgl,line.keterangan,line.akun_hutang,floatToNilai(line.saldo),line.kode_proyek,line.ket_proyek]);					
					}
				} else this.sg1.clear(1);
