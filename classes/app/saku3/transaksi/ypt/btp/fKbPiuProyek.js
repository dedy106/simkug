window.app_saku3_transaksi_ypt_btp_fKbPiuProyek = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_btp_fKbPiuProyek.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_btp_fKbPiuProyek";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pelunasan Tagihan Piutang Proyek", 0);	
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:6,tag:0,
				colTitle:["Status","No Referensi","Tanggal","Keterangan","Akun Piutang","Tagihan"],
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:9,
...
window.app_saku3_transaksi_ypt_btp_fKbPiuProyek.extend(window.childForm);
window.app_saku3_transaksi_ypt_btp_fKbPiuProyek.implement({
	doLoad : function() {
		this.pc2.setActivePage(this.pc2.childPage[0]);	
		this.pc1.setActivePage(this.pc1.childPage[0]);																		
		if (this.stsSimpan==1 && this.cb_cust.getText()!="") {		
			var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.kode_akun as akun_piutang,a.nilai-isnull(b.bayar,0) as saldo  "+						  
						 "from trans_j a "+

						 "     left join ("+
						 "          select no_selesai,sum(case dc when 'C' then nilai else -nilai end) as bayar  "+
						 "          from trans_j where kode_cust='"+this.cb_cust.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
						 "			group by no_selesai,kode_lokasi "+
						 ") b on a.no_bukti=b.no_selesai "+
						 
						 "where a.jenis <> 'UNBILL' and a.dc='D' and a.kode_cust='"+this.cb_cust.getText()+"' and (a.nilai-isnull(b.bayar,0)) > 0 and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_bukti";						 
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg1.appendData(["INPROG",line.no_bukti,line.tgl,line.keterangan,line.akun_piutang,floatToNilai(line.saldo)]);					
				}
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.modul+"','KBPRPIU','F','-','-','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','-','"+this.e_ket.getText()+"','IDR',1,"+
					...
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && this.sg1.cells(0,i) == "BAYAR"){
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(4,i)+"','C',"+nilaiToFloat(this.sg1.cells(5,i))+","+
									nilaiToFloat(this.sg1.cells(5,i))+",'Pembayaran atas "+this.cb_cust.rightLabelCaption+" No Ref "+this.sg1.cells(1,i)+"','"+this.modul+"','PIUTANG','IDR',1,'"+this.app._kodePP+"','-','"+this.cb_cust.getText()+"','-','-','"+this.sg1.cells(1,i)+"','-','-','-')");

							sql.add("insert into piutang_bayar (no_bukti,kode_lokasi,periode,no_piutang,kode_akun,nilai,dc,form) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.sg1.cells(1,i)+"','"+this.cb_akun.getText()+"',"+nilaiToFloat(this.sg1.cells(5,i))+",'D','BYRPRO')");
						}
					}
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_produk_rptProyekPiutangKb";
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai1 "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.form = 'KBPRPIU' and a.posted ='F'";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.kode_akun as akun_piutang,a.nilai-isnull(b.bayar,0) as saldo  "+						  
							 "from trans_j a "+
							 "     inner join trans_j c on a.no_bukti=c.no_selesai and c.kode_lokasi='"+this.app._lokasi+"' "+
							 "     left join ("+
							 "          select no_selesai,sum(case dc when 'C' then nilai else -nilai end) as bayar  "+
							 "          from trans_j where no_bukti <> '"+this.e_nb.getText()+"' and kode_cust='"+this.cb_cust.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
							 "			group by no_selesai,kode_lokasi "+
							 ") b on a.no_bukti=b.no_selesai "+
							
							 "where a.modul='PR' and a.dc='D' and a.kode_cust='"+this.cb_cust.getText()+"' and (a.nilai-isnull(b.bayar,0)) > 0 and a.kode_lokasi='"+this.app._lokasi+"' and c.no_bukti='"+this.e_nb.getText()+"' order by a.no_bukti";						 
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData(["BAYAR",line.no_bukti,line.tgl,line.keterangan,line.akun_piutang,floatToNilai(line.saldo)]);					
					}
				} else this.sg1.clear(1);
