window.app_saku3_transaksi_ypt_btp_fPjptgProyek = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_btp_fPjptgProyek.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_btp_fPjptgProyek";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pertanggungan Panjar per Proyek", 0);	
		...
		this.cb_proyek = new portalui_saiCBBL(this.pc1.childPage[0],{bound:[20,14,220,20],caption:"ID Proyek",tag:1,readOnly:true}); 												
		this.e_namapro = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,16,450,20],caption:"Deskripsi Proyek", readOnly:true});									
		...
		this.e_akunBMHD = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,18,200,20],caption:"Akun BMHD", tag:2, readOnly:true });			
		//this.e_drkbeban = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,29,200,20],caption:"DRK Beban", tag:2, readOnly:true });			
		//this.e_saldogar = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,16,200,20],caption:"Saldo Budget", tag:2, tipeText:ttNilai, text:"0",readOnly:true,visible:false });		
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:6,tag:9,
		            colTitle:["Ket. Peruntukan","Nilai","PPN Masukan","Pot. PPh21","Pot. PPh23","Total"],					
					colWidth:[[5,4,3,2,1,0],[100,80,80,80,100,400]],
					columnReadOnly:[true,[5],[0,1,2,3,4]],					
					colFormat:[[1,2,3,4,5],[cfNilai,cfNilai,cfNilai,cfNilai,cfNilai]],checkItem:true,
...
window.app_saku3_transaksi_ypt_btp_fPjptgProyek.extend(window.childForm);
window.app_saku3_transaksi_ypt_btp_fPjptgProyek.implement({
	doHitungGar: function(){	
		var data = this.dbLib.getDataProvider("select status_gar from masakun where kode_akun='"+this.akunBMHD+"' and kode_lokasi='"+this.app._lokasi+"'",true);
		if (typeof data == "object"){
		...
		if (this.statusGar == "1") {
			var sakhir = 0;
			if (this.stsSimpan == 1) var data = this.dbLib.getDataProvider("select fn_release1('"+this.app._lokasi+"','"+this.e_akunBMHD.getText()+"','"+this.cb_pp.getText()+"','"+this.e_drkbeban.getText()+"','"+this.e_periode.getText()+"') as gar ",true);
			else var data = this.dbLib.getDataProvider("select fn_release2('"+this.app._lokasi+"','"+this.e_akunBMHD.getText()+"','"+this.cb_pp.getText()+"','"+this.e_drkbeban.getText()+"','"+this.e_periode.getText()+"','"+this.e_nb.getText()+"') as gar ",true);
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						//matiin 2-8-21 sql.add("delete from pr_or_d where no_bukti = '"+this.nobuktiLama+"' and kode_lokasi='"+this.app._lokasi+"'");						
						sql.add("delete from pr_or_d where dc ='C' and no_bukti = '"+this.cb_pj.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
						...
						sql.add("delete from angg_r where no_bukti = '"+this.nobuktiLama+"' and kode_lokasi='"+this.app._lokasi+"'");												
					}
					...
					sql.add("insert into ptg_m (no_ptg,no_pj,no_kas,no_dokumen,tanggal,keterangan,catatan,kode_curr,kurs,akun_pj,akun_kas,nik_buat,nik_setuju,kode_lokasi,kode_pp,modul,nilai,nilai_kas,kode_drk,progress,posted,periode,no_del,no_link,nik_user,tgl_input) values "+					
							"('"+this.e_nb.getText()+"','"+this.cb_pj.getText()+"','"+noKas+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','-','IDR',1,'"+this.cb_akunpj.getText()+"','"+akunKas+"','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.app._lokasi+"','"+this.cb_pp.getText()+"','PJPTG',"+parseNilai(this.e_nilaiptg.getText())+","+parseNilai(this.e_nilaikb.getText())+",'-','2','X','"+this.e_periode.getText()+"','-','-','"+this.app._userLog+"',getdate())");
					
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.nb+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.modul+"','PJPTG','F','-','-','"+this.cb_pp.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_pj.getText()+"','"+this.e_ket.getText()+"','IDR',1,"+
						...
						for (var i=0;i < this.sg.getRowCount();i++) {
							if (this.sg.rowValid(i)){
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.nb+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.akunBMHD+"','D',"+parseNilai(this.sg.cells(1,i))+","+parseNilai(this.sg.cells(1,i))+",'"+this.sg.cells(0,i)+"','"+this.modul+"','BEBAN','IDR',1,'"+this.cb_pp.getText()+"','"+this.drkBeban+"','-','-','-','-','-','-','-')");
								
								if (this.sg.cells(2,i) != "0") {
									//PPNM
									var j = 1000+i;
									sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
											"('"+this.nb+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',"+j+",'"+this.akunPPNM+"','D',"+parseNilai(this.sg.cells(2,i))+","+parseNilai(this.sg.cells(2,i))+",'"+this.e_ket.getText()+"','"+this.modul+"','PPNM','IDR',1,'"+this.cb_pp.getText()+"','-','-','-','-','-','-','-','-')");
								}
								if (this.sg.cells(3,i) != "0") {
									//PPH21
									var k = 2000+i;
									sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
											"('"+this.nb+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',"+k+",'"+this.hutPPh21+"','C',"+parseNilai(this.sg.cells(3,i))+","+parseNilai(this.sg.cells(3,i))+",'"+this.e_ket.getText()+"','"+this.modul+"','HUTPPH21','IDR',1,'"+this.cb_pp.getText()+"','-','-','-','-','-','-','-','-')");
								}
								if (this.sg.cells(4,i) != "0") {
									//PPH23
									var l = 3000+i;
									sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
											"('"+this.nb+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',"+l+",'"+this.hutPPh23+"','C',"+parseNilai(this.sg.cells(4,i))+","+parseNilai(this.sg.cells(4,i))+",'"+this.e_ket.getText()+"','"+this.modul+"','HUTPPH23','IDR',1,'"+this.cb_pp.getText()+"','-','-','-','-','-','-','-','-')");
								}
							}
						}
					}					
					...
					if (this.sg2.getRowValidCount() > 0){
						for (var i=0;i < this.sg2.getRowCount();i++) {
							var data = this.dbLib.getDataProvider("select a.akun_beban,b.kode_drkb,b.kode_pp from pr_jenis a inner join pr_proyek b on a.kode_jenis=b.kode_jenis and a.kode_lokasi=b.kode_lokasi where b.kode_proyek='"+this.sg2.cells(0,i)+"' and b.kode_lokasi='"+this.app._lokasi+"' ",true);
							if (typeof data == "object"){
								var line = data.rs.rows[0];							
								if (line != undefined){
									var bbnProyek = line.akun_beban;
									var ppProyek = line.kode_pp;
									var drkProyek = line.kode_drkb;
								} 
							}
							...
							sql.add("insert into pr_or_d (no_bukti,kode_lokasi,periode,kode_proyek,kode_akun,dc,nilai,jenis,kode_pp,kode_drk,no_reverse) values "+
									"('"+this.nb+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.sg2.cells(0,i)+"','"+bbnProyek+"','D',"+nilaiToFloat(this.sg2.cells(3,i))+",'PJBEBAN','"+ppProyek+"','"+drkProyek+"','NONREV')");								
						}
					}
					*/
					
					sql.add("insert into pr_or_d (no_bukti,kode_lokasi,periode,kode_proyek,kode_akun,dc,nilai,jenis,kode_pp,kode_drk,no_reverse) "+
							"select no_bukti,kode_lokasi,periode,kode_proyek,kode_akun,'C',nilai,jenis,kode_pp,kode_drk,no_reverse "+
							"from pr_or_d where no_bukti = '"+this.cb_pj.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");								
					
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_gar,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+												
							"('"+this.nb+"','PJPTG','"+this.app._lokasi+"','"+this.e_akunBMHD.getText()+"','"+this.cb_pp.getText()+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','C',0,"+parseNilai(this.e_nilaiptg.getText())+")");
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :	
				...
				this.doHitungGar();
				//belum dipake
				// if (this.statusGar == "1" && nilaiToFloat(this.e_nilaiptg.getText()) > nilaiToFloat(this.e_saldogar.getText())) {
				// 	system.alert(this,"Transaksi tidak valid.","Nilai Pertanggungan tidak boleh melebihi Saldo Budget");
				// 	return false;
				// }
	...
	doChange:function(sender){
		...
		if (sender == this.cb_pj && this.cb_pj.getText()!="") {
			var data = this.dbLib.getDataProvider(
					   "select convert(varchar,a.tanggal,103) as tanggal,a.akun_pj,b.nama as nama_pj,a.nilai,a.periode,a.kode_pp,c.nama as nama_pp,a.no_dokumen,e.akun_bmhd,d.kode_drkb,a.keterangan as ketpj, a.nik_setuju,d.nama as nama_pro "+
					   ...
					   "				inner join pr_jenis e on d.kode_jenis=e.kode_jenis and d.kode_lokasi=e.kode_lokasi  "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){
					...
					this.e_namapro.setText(line.nama_pro);
					
					this.e_akunBMHD.setText(line.akun_bmhd);
					//this.e_drkbeban.setText(line.kode_drkb);
					this.akunBMHD = line.akun_bmhd;
					//this.drkBeban = line.kode_drkb;
					this.drkBeban = "-";

					if (this.stsSimpan==1) {
						this.e_ket.setText(line.ketpj);
	...
	doChangeCell: function(sender, col, row){
		try {
			if (col == 1 || col == 2 || col == 3 || col == 4) {
				if (this.sg.cells(1,row) != "" && this.sg.cells(2,row) != "" && this.sg.cells(3,row) != "" && this.sg.cells(4,row) != "") {
					var total = nilaiToFloat(this.sg.cells(1,row)) + nilaiToFloat(this.sg.cells(2,row)) - nilaiToFloat(this.sg.cells(3,row)) - nilaiToFloat(this.sg.cells(4,row));
					this.sg.cells(5,row,total);
				}
				this.sg.validasi();
			}				
		}
	...
	doHitungOR: function(){		
		...
		for (var i=0;i < this.sg.getRowCount();i++){
			if (this.sg.rowValid(i)){
				...
				nilai = nilaiToFloat(this.sg.cells(1,i));
				
				var isAda = false;
				var idx = total = 0;
				for (var j=0;j < this.sg2.getRowCount();j++){
					if (this.cb_proyek.getText() == this.sg2.cells(0,j)) {
						isAda = true;
						idx = j;
						break;
					}
				...
				if (!isAda) {
					this.sg2.appendData([this.cb_proyek.getText(),this.cb_proyek.rightLabelCaption,"0",floatToNilai(nilai),"0"]);
				} 
				else { 
		...
		for (var i=0;i < this.sg2.getRowCount();i++){
			var strSQL = "select a.nilai_or-isnull(c.tot_bdd,0) as sisa_or "+
						 ...
						 "	 from pr_or_d where no_bukti not in ('"+this.e_nb.getText()+"','"+this.cb_pj.getText()+"') and kode_lokasi='"+this.app._lokasi+"' group by kode_proyek "+						 
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1) {
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_produk_rptProyekPanjarBeban";
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_ptg, convert(varchar,a.tanggal,103) as tgl, a.keterangan,a.no_kas from ptg_m a inner join trans_m b on a.no_ptg=b.no_bukti and a.kode_lokasi=b.kode_lokasi and b.form='PJPTG' where a.nilai_kas=0 and b.posted = 'F' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' "+
					 "union "+
					 "select a.no_ptg, convert(varchar,a.tanggal,103) as tgl, a.keterangan,a.no_kas from ptg_m a inner join trans_m b on a.no_kas=b.no_bukti and a.kode_lokasi=b.kode_lokasi and b.form='PJPTG' where a.nilai_kas<>0 and b.posted = 'F' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' ";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select b.kode_proyek,b.nama,c.akun_beban,b.kode_pp,b.kode_drkb,a.keterangan,a.nilai,isnull(d.nilai,0) as ppnm,isnull(e.nilai,0) as pph21,isnull(f.nilai,0) as pph23, "+
							 ...
							 " ) f on a.no_bukti=f.no_bukti and (a.nu+3000)=f.nu "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg.appendData([line.keterangan,floatToNilai(line.nilai),floatToNilai(line.ppnm),floatToNilai(line.pph21),floatToNilai(line.pph23),floatToNilai(line.total)]);
					}
