window.app_saku3_transaksi_ypt_btp_fPjptgBDDproyek = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_btp_fPjptgBDDproyek.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_btp_fPjptgBDDproyek";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pertanggungan Panjar via BDD per Proyek", 0);	
...
window.app_saku3_transaksi_ypt_btp_fPjptgBDDproyek.extend(window.childForm);
window.app_saku3_transaksi_ypt_btp_fPjptgBDDproyek.implement({
	...
	simpan: function(){			
		try{						
			...
			if (nilaiToFloat(this.e_nilaikb.getText()) != 0) 
				this.e_kb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti",this.app._lokasi+"-"+this.c_jenis.getText()+this.e_periode.getText().substr(2,4)+".","0000"));
						...
						for (var i=0;i < this.sg.getRowCount();i++) {
							if (this.sg.rowValid(i)){
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.nb+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.akunBDD+"','D',"+parseNilai(this.sg.cells(1,i))+","+parseNilai(this.sg.cells(1,i))+",'"+this.sg.cells(0,i)+"','"+this.modul+"','BEBAN','IDR',1,'"+this.cb_pp.getText()+"','-','-','-','-','-','-','-','-')");
	...
	doChange:function(sender){		
		if (sender == this.cb_buat && this.cb_buat.getText()!="" && this.stsSimpan==1) this.cb_pj.setSQL("select no_pj, keterangan from panjar_m where progress = '2' and modul='PJKB' and nik_pengaju='"+this.cb_buat.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_pj","keterangan"],false,["No Panjar","Keterangan"],"and","Data Panjar",true);
		if (sender == this.cb_pj && this.cb_pj.getText()!="") {
			var data = this.dbLib.getDataProvider(
					   "select convert(varchar,a.tanggal,103) as tanggal,a.akun_pj,b.nama as nama_pj,a.nilai,a.periode,a.kode_pp,c.nama as nama_pp, a.no_dokumen,e.akun_bdd,a.keterangan as ketpj, a.nik_setuju, d.nama as nama_pro "+
