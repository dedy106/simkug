window.app_saku3_transaksi_ypt_btp_fKbHutSimple = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_btp_fKbHutSimple.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_btp_fKbHutSimple";
		...
		this.c_jenis = new saiCB(this.pc2.childPage[0],{bound:[20,11,200,20],caption:"Jenis",items:["BK","MI"], readOnly:true,tag:2,change:[this,"doChange"]});
		...
		this.cb_vendor = new saiCBBL(this.pc2.childPage[0],{bound:[20,14,220,20],caption:"Vendor", multiSelection:false, maxLength:10, tag:1,readOnly:true});						
		this.e_dok = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,16,450,20],caption:"No Dokumen", maxLength:50});												
		this.e_ket = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,17,450,20],caption:"Deskripsi", maxLength:150});												
		this.e_saldo = new saiLabelEdit(this.pc2.childPage[0],{bound:[780,17,200,20],caption:"Saldo Hutang", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});								
		...
		this.e_nilai = new saiLabelEdit(this.pc2.childPage[0],{bound:[780,18,200,20],caption:"Nilai Pelunasan", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
...
window.app_saku3_transaksi_ypt_btp_fKbHutSimple.extend(window.childForm);
window.app_saku3_transaksi_ypt_btp_fKbHutSimple.implement({
	doLoadRev: function() {
		if (this.cb_hutang.getText()!="") {
			strSQL = "select a.kode_akun,b.nama as nama_akun,case a.dc when 'D' then 'C' else 'D' end as dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp, a.kode_drk, isnull(d.nama,'-') as nama_drk "+
					 ...
					 "where a.jenis <> 'HUTANG' and a.no_bukti = '"+this.cb_hutang.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.nu";								 
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					...
					this.sg.appendData([line2.kode_akun,line2.nama_akun,line2.dc.toUpperCase(),line2.keterangan,floatToNilai(line2.nilai),line2.kode_pp,line2.nama_pp,line2.kode_drk,line2.nama_drk]);
				}
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from hutbayar_d where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}
					
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.modul+"','KBHUT','F','-','-','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','IDR',1,"+
							parseNilai(this.e_nilai.getText())+",0,0,'"+this.cb_app.getText()+"','-','-','"+this.cb_hutang.getText()+"','-','-','"+this.cb_vendor.getText()+"','-','"+this.c_jenis.getText()+"')");

					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.akunHutang+"','D',"+parseNilai(this.e_nilai.getText())+","+
							parseNilai(this.e_nilai.getText())+",'"+this.e_ket.getText()+"','"+this.modul+"','HUTANG','IDR',1,'"+this.app._kodePP+"','-','-','"+this.cb_vendor.getText()+"','-','-','-','-','-')");										
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){																															
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										...
										parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(3,i)+"','"+this.modul+"','BEBAN','IDR',1,'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','-','-','-','-','-','-','-')");											
							}
						}
					}

					sql.add("insert into hutbayar_d(no_bukti,kode_lokasi,no_hutang,periode,dc,modul,nilai,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_hutang.getText()+"','"+this.e_periode.getText()+"','D','"+this.modul+"',"+parseNilai(this.e_nilai.getText())+",'"+this.app._userLog+"',getdate())");
	...
	doChange:function(sender){		
		if ((sender == this.e_periode ) && (this.e_periode.getText()!= "")) {	
			if (this.stsSimpan == 1) {		
				var strSQL = "select a.no_hutang,a.keterangan,a.kode_lokasi from hutang_d a "+
							"    left join ( "+
							"             select no_hutang,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar "+
							"             from hutbayar_d group by no_hutang,kode_lokasi) b on a.no_hutang=b.no_hutang and a.kode_lokasi=b.kode_lokasi "+
							"where  a.nilai>isnull(b.bayar,0) and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
		...
		if (sender == this.cb_hutang && this.cb_hutang.getText()!= "") {
			var strSQL = "select a.kode_pp,a.nilai -  isnull(b.bayar,0) as saldo, a.akun_hutang, c.kode_vendor,c.nama as nama_vendor,a.keterangan "+			             
						 "from hutang_d a "+	
						 "     inner join vendor c on a.kode_vendor=c.kode_vendor and a.kode_lokasi=c.kode_lokasi "+					 
						 "     left join ("+
						 "              select no_hutang,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar "+
						 "              from hutbayar_d where no_bukti <> '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' group by no_hutang,kode_lokasi) b on a.no_hutang=b.no_hutang and a.kode_lokasi=b.kode_lokasi "+
		...
		switch(col){
			...
			case 5 : 
					if ((this.sg.cells(5,row) == "") && (row > 0)) {
						...
						this.sg.setCell(5,row,this.kodePPInvoice);						
			...
			if (sender == this.sg) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Akun",sender,undefined, 
												  ...
												  "select count(*) from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '035' where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'",
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_tpcc_proyek_rptKbHutangJurnal";
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.param3 as jenis,a.no_dokumen,a.keterangan,a.nilai1 "+
		             "from trans_m a "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.form = 'KBHUT' and a.posted ='F'";						
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.nilai -  isnull(b.bayar,0) as saldo, a.kode_pp, a.akun_hutang,a.kode_vendor,"+							 
							 "e.param3 as jenis,e.periode,e.no_dokumen,e.tanggal,e.keterangan as ket_kas,e.nik1 as nik_app,e.no_ref1 as ref1 "+
							 "from hutang_d a inner join trans_m e on e.no_ref1 = a.no_hutang and a.kode_lokasi=e.kode_lokasi "+
							 "                   left join ("+
							 "                   select no_hutang,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar "+
							 "                   from hutbayar_d where no_bukti <> '"+this.e_nb.getText()+"' group by no_hutang,kode_lokasi) b on a.no_hutang=b.no_hutang and a.kode_lokasi=b.kode_lokasi "+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){		
						...
						this.cb_hutang.setSQL("select no_hutang,keterangan from hutang_d where kode_lokasi='"+this.app._lokasi+"' and no_hutang='"+line.ref1+"'",["no_hutang","keterangan"],false,["No Bukti","Keterangan"],"and","Data Hutang",true);											
