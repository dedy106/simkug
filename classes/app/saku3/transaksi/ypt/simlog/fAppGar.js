window.app_saku3_transaksi_ypt_simlog_fAppGar = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_simlog_fAppGar.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_simlog_fAppGar";
		...
		this.pc1 = new pageControl(this,{bound:[10,18,1000,440], childPage:["Daftar Request","Data Approval","Cari Data"]});		
		...
		this.e_memo = new saiMemo(this.pc1.childPage[1],{bound:[20,13,450,60],caption:"Catatan Approve",tag:9,readOnly:true});		
		...
		this.e_noreq = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,11,450,20],caption:"No Request", readOnly:true});		
		...
		this.e_tgl = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,13,220,20],caption:"Tgl Request", readOnly:true});
		...
		this.pc2 = new pageControl(this.pc1.childPage[1],{bound:[2,18,995,203], childPage:["Data Budget","Detail Request","Maksud-Tujuan","Aspek Strategis"]});		
...
window.app_saku3_transaksi_ypt_simlog_fAppGar.extend(window.childForm);
window.app_saku3_transaksi_ypt_simlog_fAppGar.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.e_modul.getText() == "LOGPR") {
						sql.add("update sl_pesan_m set progress='"+prog+"',no_app='"+this.e_nb.getText()+"' where no_pesan='"+this.e_noreq.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");	
						var modul = 'LOGGAR';
					}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "hapus" :	
				this.preView = "0";
				if (this.e_modul.getText() == "LOGPR") {
					var data = this.dbLib.getDataProvider("select distinct progress from sl_pesan_m where progress not in ('1','R') and no_pesan='"+this.e_noreq.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'",true);
				...
				if (this.e_modul.getText() == "LOGREQ") {
					sql.add("update sl_pesan_m set progress='0',no_app='-' where no_pesan='"+this.e_noreq.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
					var modul = "LOGGAR";
				}				
	...
	doClick:function(sender){
		if (this.e_noreq.getText()!="") {						
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"sl_app_m","no_app",this.app._lokasi+"-GAR"+this.e_periode.getText().substr(2,4)+".","0000"));									
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg.cells(0,row) != "") {			
				...
				var strSQL = "select a.nilai_curr,a.kode_curr,a.kurs, a.maksud, a.aspek, isnull(b.catatan,'-') as catatan "+
							 "from sl_pesan_m a "+
							 "left join sl_app_m b on b.no_app=a.no_app and a.kode_lokasi=b.kode_lokasi and b.modul ='LOGGAR' "+
	...
	doLoad:function(sender){												
		...
		var strSQL = "select a.tanggal,a.no_pesan,'INPROG' as status,convert(varchar,a.tanggal,103) as tgl,'LOGPR' as modul,d.kode_pp+' | '+d.nama as pp,a.no_dokumen,a.keterangan,a.nilai,c.nik+' | '+c.nama as pembuat,a.no_app "+
					 "from sl_pesan_m a "+
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData([line.no_pesan,line.status.toUpperCase(),line.tgl,line.modul.toUpperCase(),line.pp,line.no_dokumen,line.keterangan,floatToNilai(line.nilai),line.pembuat,line.no_app]); 
		}
	...
	doCari:function(sender){						
		...
		var strSQL = "select a.tanggal,a.no_pesan,"+
					 ...
					 "end as status,"+
					 "convert(varchar,a.tanggal,103) as tgl,'LOGPR' as modul,b.kode_pp+' | '+b.nama as pp,a.no_dokumen,a.keterangan,a.nilai,c.nik+' | '+c.nama as pembuat,a.no_app "+					 
		             "from sl_pesan_m a inner join pp b on a.kode_ppaju=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
					 "					inner join karyawan c on a.nik_buat=c.nik and a.kode_lokasi=c.kode_lokasi "+					 
