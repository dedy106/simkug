window.app_saku3_transaksi_ypt_simlog_fSpph = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_simlog_fSpph.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_simlog_fSpph";
		...
		this.l_tgl2 = new portalui_label(this.pc2.childPage[0],{bound:[20,11,100,18],caption:"Maksimal SPH", underline:true});
		...
		this.sg5 = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:9,tag: 0,
...
window.app_saku3_transaksi_ypt_simlog_fSpph.extend(window.childForm);
window.app_saku3_transaksi_ypt_simlog_fSpph.implement({	
	doIsiCbPesan : function() {
		this.cb_pesan.setSQL("select a.no_pesan,a.keterangan "+
							 "from sl_pesan_m a "+
							 "	   inner join sl_justerima_m c on a.no_pesan=c.no_pesan and a.kode_lokasi=c.kode_lokasi "+
							 "	   inner join sl_app_m d on c.no_terima=d.no_app and d.no_appseb='-' and d.kode_lokasi=c.kode_lokasi "+
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						sql.add("delete from sl_spph_m where no_spph = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from sl_spph_d where no_spph = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from sl_spph_vendor where no_spph = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");												
						sql.add("delete from sl_pesan_dok where no_pesan='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
						sql.add("update sl_pesan_m set progress='2',no_spph='-' where no_spph='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}					
					
					sql.add("update sl_pesan_m set progress='3',no_spph='"+this.e_nb.getText()+"' where no_pesan='"+this.cb_pesan.getText()+"'");									
					sql.add("insert into sl_spph_m(no_spph,kode_lokasi,tgl_input,nik_user,periode,tanggal,nik_buat,nik_app,keterangan,no_tap,no_dokumen,due_date,modul,no_sanggup, kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','"+this.e_ket.getText()+"','-','"+this.e_dok.getText()+"','"+this.dp_d2.getDateString()+"','"+this.e_jenis.getText()+"','-','IDR',1)");					
					
					for (var i=0;i < this.sg5.getRowCount();i++){
						if (this.sg5.rowValid(i)){
							sql.add("insert into sl_spph_d(no_spph,kode_lokasi,no_urut,item,merk,tipe,catatan,jumlah,harga,ppn,kode_klpfa) values "+
					...
					for (var i=0;i < this.sg2.getRowCount();i++){
						if (this.sg2.rowValid(i)){								
							sql.add("insert into sl_spph_vendor(no_spph,kode_lokasi,kode_vendor,no_sph) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','-')");
						}
					}					
					sql.add("insert into sl_pesan_dok(no_pesan,no_gambar,nu,kode_jenis,kode_lokasi) values('"+this.e_nb.getText()+"','"+this.e_file.getText()+"',0,'SPPH','"+this.app._lokasi+"')");					
	...
	doChange:function(sender){
		...
		if (sender == this.cb_pesan && this.stsSimpan ==1) {
			var data = this.dbLib.getDataProvider("select kode_akun from sl_pesan_m where no_pesan = '"+this.cb_pesan.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){								
			...
			sql.add("select a.kode_klpfa,a.nama from fa_klp a inner join fa_klpakun b on a.kode_klpakun=b.kode_klpakun and a.kode_lokasi=b.kode_lokasi where b.kode_akun = '"+this.akunAktap+"' and b.kode_lokasi='"+this.app._lokasi+"' union select '-','-' ");			
			...
			var strSQL = "select a.no_urut,a.item,a.merk,a.tipe,a.catatan,a.jumlah,a.nilai,a.jumlah*a.nilai as total,c.jenis,b.no_dokumen "+
			             "from sl_pesan_d a "+
						 "  inner join sl_pesan_m b on a.no_pesan=b.no_pesan and a.kode_lokasi=b.kode_lokasi "+
						 "  inner join sl_justerima_m c on a.no_pesan=c.no_pesan and a.kode_lokasi=b.kode_lokasi "+						 
						 "	inner join sl_app_m d on c.no_terima=d.no_app and d.no_appseb='-' and a.kode_lokasi=b.kode_lokasi "+
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "") {
			...
			this.stsSimpan = 1;
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"sl_spph_m","no_spph",this.app._lokasi+"-SPPH"+this.e_periode.getText().substr(2,4)+".","0000"));						
			this.e_ket.setFocus();
			...
			if (sender == this.sg5) {				
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Kelompok Barang",sender,undefined, 
							"select a.kode_klpfa, a.nama  from fa_klp a inner join fa_klpakun b on a.kode_klpakun=b.kode_klpakun and a.kode_lokasi=b.kode_lokasi where b.kode_klpakun = '"+this.akunAktap+"' and b.kode_lokasi='"+this.app._lokasi+"' ",
							"select count(*) from fa_klp a inner join fa_klpakun b on a.kode_klpakun=b.kode_klpakun and a.kode_lokasi=b.kode_lokasi where b.kode_klpakun = '"+this.akunAktap+"' and b.kode_lokasi='"+this.app._lokasi+"' ",
	...
	doLoad3:function(sender){																		
		var strSQL = "select distinct a.no_spph,convert(varchar,a.tanggal,103) as tgl,a.keterangan "+
					 ...
					 "where b.no_sph is null and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'PO-SPK' and a.no_tap='-'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select c.kode_akun,a.keterangan,a.tanggal,a.nik_buat,a.nik_app,isnull(b.no_gambar,'-')as no_gambar,a.no_dokumen,c.no_pesan,a.due_date  "+
							 "from sl_spph_m a "+
							 "   inner join sl_pesan_m c on a.no_spph=c.no_spph and a.kode_lokasi=c.kode_lokasi "+
							 "	 left join sl_pesan_dok b on a.no_spph=b.no_pesan and a.kode_lokasi=b.kode_lokasi and b.kode_jenis='SPPH' "+							 
				...
				if (typeof data == "object"){
					...
					if (line != undefined){								
						...
						this.cb_pesan.setSQL("select no_pesan,keterangan from sl_pesan_m where no_pesan='"+line.no_pesan+"'",["no_pesan","keterangan"],false,["No Request","Deskripsi"],"and","Data Request",true);			
				...
				var strSQL = "select a.no_urut,a.item,a.merk,a.tipe,a.catatan,a.jumlah,a.nilai,a.jumlah*a.nilai as total,c.jenis,b.no_dokumen "+							 
							 "from sl_pesan_d a "+
							 "  inner join sl_pesan_m b on a.no_pesan=b.no_pesan and a.kode_lokasi=b.kode_lokasi "+
							 "  inner join sl_justerima_m c on a.no_pesan=c.no_pesan and a.kode_lokasi=c.kode_lokasi and b.no_terima=c.no_terima "+						 
				...
				var strSQL = "select b.kode_klpfa,b.nama,a.item,a.merk,a.tipe,a.catatan,a.jumlah,a.harga,a.harga * a.jumlah as total "+
				             "from sl_spph_d a "+
				             "	inner join sl_pesan_m c on c.no_spph=a.no_spph and a.kode_lokasi=c.kode_lokasi "+
				...
				var data = this.dbLib.getDataProvider("select a.kode_vendor,b.nama,b.alamat from sl_spph_vendor a inner join vendor b on a.kode_vendor=b.kode_vendor and a.kode_lokasi=b.kode_lokasi where a.no_spph='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg2.clear();
					for (var i in data.rs.rows){
