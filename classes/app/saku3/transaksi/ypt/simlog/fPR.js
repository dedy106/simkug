window.app_saku3_transaksi_ypt_simlog_fPR = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_simlog_fPR.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_simlog_fPR";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,20,995,360], childPage:["Anggaran","Detail","Maksud-Tujuan","Aspek Strategis"]});			
		...
		this.cb_app = new saiCBBL(this.pc1.childPage[0],{bound:[20,18,220,20],caption:"NIK App Budget", multiSelection:false, maxLength:10, tag:2});		
		this.cb_ppaju = new saiCBBL(this.pc1.childPage[0],{bound:[20,13,220,20],caption:"PP Pengaju", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});						
		this.cb_akun = new saiCBBL(this.pc1.childPage[0],{bound:[20,14,220,20],caption:"Mata Anggaran", multiSelection:false, maxLength:10, tag:1, change:[this,"doChange"]});								
		...
		try {
			...
			this.cb_ppaju.setSQL("select distinct a.kode_pp, a.nama from pp a inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.nik='"+this.app._userLog+"' "+
								 "where a.flag_aktif='1' and a.kode_lokasi='"+this.app._lokasi+"'",["kode_pp","nama"],false,["Kode","Nama"],"and","Data PP Pengaju",true);			
...
window.app_saku3_transaksi_ypt_simlog_fPR.extend(window.childForm);
window.app_saku3_transaksi_ypt_simlog_fPR.implement({		
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						sql.add("delete from sl_pesan_m where no_pesan='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from sl_pesan_d where no_pesan='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from sl_pesan_dok where no_pesan='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from angg_r where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}
					
					sql.add("insert into sl_pesan_m(no_pesan,kode_lokasi,tgl_input,nik_user,periode,jenis,tanggal,no_dokumen,keterangan,kode_pp,kode_drk,kode_akun,saldo_gar,nilai,nik_buat,nik_app,kode_dana,no_app,progress,maksud,aspek,no_terima,no_spph,lok_proses,kode_ppaju, kode_curr,kurs,nilai_curr) values "+
						   "('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.c_jenis.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.cb_ppaju.getText()+"','"+this.cb_drk.getText()+"','"+
								this.cb_akun.getText()+"',"+nilaiToFloat(this.e_gar.getText())+","+nilaiToFloat(this.e_total.getText())+",'"+this.app._userLog+"','"+this.cb_app.getText()+"','-','-','0','"+urlencode(this.mDesk1.getCode())+"','"+urlencode(this.mDesk2.getCode())+"','-','-','"+this.app._lokasi+"','"+
								this.cb_ppaju.getText()+"','IDR',1,"+nilaiToFloat(this.e_total.getText())+")"); 							

					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into sl_pesan_d(no_pesan,kode_lokasi,no_urut,item,merk,tipe,catatan,jumlah,nilai,harga,no_po,no_ba,kode_dana,jum_po,kode_klpfa,ppn) values "+
					...
					sql.add("insert into sl_pesan_dok(no_pesan,no_gambar,nu,kode_jenis,kode_lokasi) values('"+this.e_nb.getText()+"','"+this.e_file.getText()+"',0,'LOGPR','"+this.app._lokasi+"')");					
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
							"('"+this.e_nb.getText()+"','LOGPR','"+this.app._lokasi+"','"+this.cb_akun.getText()+"','"+this.cb_ppaju.getText()+"','"+this.cb_drk.getText()+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','D',"+nilaiToFloat(this.e_gar.getText())+","+nilaiToFloat(this.e_total.getText())+")");
			...
			case "ubah" :	
				...
				if (this.stsSimpan == 1){					
					if (this.flagDokFree == "1") {				
						var data = this.dbLib.getDataProvider("select no_pesan from sl_pesan_m where no_dokumen='"+this.e_dok.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
						if (typeof data == "object"){
	...
	doClick:function(sender){		
		...
		this.stsSimpan = 1;
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"sl_pesan_m","no_pesan",this.app._lokasi+"-RQ"+this.e_periode.getText().substr(2,4)+".","0000"));
		this.e_dok.setFocus();			
	},	
	...
	doChange: function(sender){						
		if (sender == this.c_jenis) {			
			...
			if (this.c_jenis.getText() == "CAPEX") 
				this.cb_akun.setSQL("select distinct a.kode_akun,a.nama from masakun a "+
									"inner join fa_klpakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
		...
		if (sender == this.cb_ppaju || sender == this.cb_akun || sender == this.e_periode) {	
			if (this.cb_ppaju.getText()!="" && this.cb_akun.getText()!="" && this.e_periode.getText()!="") {
				this.cb_drk.setSQL("select distinct a.kode_drk, a.nama from drk a inner join anggaran_d b on a.kode_drk=b.kode_drk and a.kode_lokasi=b.kode_lokasi "+
								   "where substring(b.periode,1,4)='"+this.e_periode.getText().substr(0,4)+"' and b.kode_akun='"+this.cb_akun.getText()+"' and b.kode_pp='"+this.cb_ppaju.getText()+"' and a.tahun='"+this.e_periode.getText().substr(0,4)+"' and a.kode_lokasi='"+this.app._lokasi+"'",["a.kode_drk","a.nama"],false,["Kode","Nama"],"and","Data DRK",true);
			}
		}		
		if (sender == this.cb_ppaju || sender == this.cb_drk || sender == this.cb_akun || sender == this.e_periode) {	
			this.e_gar.setText("0");			
			this.doHitungGar();
		}
	},
	doHitungGar: function(){				
		if (this.cb_akun.getText()!="" && this.e_periode.getText()!="") {
			var sisa = 0;	
			if (this.stsSimpan == 1) var data = this.dbLib.getDataProvider("select fn_cekagg2Thn('"+this.cb_ppaju.getText()+"','"+this.app._lokasi+"','"+this.cb_akun.getText()+"','"+this.cb_drk.getText()+"','"+this.e_periode.getText()+"') as gar ",true);
			else var data = this.dbLib.getDataProvider("select fn_cekagg3Thn('"+this.cb_ppaju.getText()+"','"+this.app._lokasi+"','"+this.cb_akun.getText()+"','"+this.cb_drk.getText()+"','"+this.e_periode.getText()+"','"+this.e_nb.getText()+"') as gar ",true);
	...
	doLoad3:function(sender){																				
		var strSQL = "select a.no_pesan,convert(varchar,a.tanggal,103) as tgl,a.jenis,a.no_dokumen,a.keterangan,a.nilai "+
		             "from sl_pesan_m a inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.nik='"+this.app._userLog+"' "+					 					 
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress in ('0','R','L') order by a.no_pesan";		
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.*,b.no_gambar, case when a.no_terima ='-' then isnull(c.catatan,'-') else isnull(d.catatan,'-') end  as catatan "+
							 "from sl_pesan_m a "+
							 "	inner join sl_pesan_dok b on a.no_pesan=b.no_pesan and a.kode_lokasi=b.kode_lokasi "+
				...
				var strSQL = "select no_urut,item,merk,tipe,catatan,jumlah,nilai,jumlah*nilai as total from sl_pesan_d where no_pesan='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' order by no_urut";							
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line2;
					this.sg.clear();
