window.app_saku3_transaksi_ypt_simlog_fTap = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_simlog_fTap.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_simlog_fTap";
		...
		this.cb_buat = new saiCBBL(this.pc2.childPage[0],{bound:[20,17,240,20],caption:"NIK TTD", multiSelection:false, maxLength:10, tag:2});				
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,315], childPage:["Daftar Mitra","Item Barang","Justifikasi","File Dok"]});				
		this.sg2 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:10,tag:0,
		...
		this.sg4 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:8,tag: 9,
...
window.app_saku3_transaksi_ypt_simlog_fTap.extend(window.childForm);
window.app_saku3_transaksi_ypt_simlog_fTap.implement({	
	doIsiCbSpph: function() {		
		this.cb_spph.setSQL("select a.no_spph, a.keterangan from sl_spph_m a "+
			                "inner join sl_pesan_m b on a.no_spph=b.no_spph "+
							"inner join sl_justerima_m c on b.no_terima=c.no_terima and c.jenis = 'PO-SPK' "+
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						sql.add("delete from sl_tap_m where no_tap = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from sl_pesan_dok where no_pesan='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");										
						sql.add("update sl_nego_m set no_tap = '-' where no_tap='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
						sql.add("update sl_spph_m set no_tap='-' where no_tap='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
					}					
					
					sql.add("insert into sl_tap_m(no_tap,no_spk,kode_lokasi,tgl_input,nik_user,periode,tanggal,nik_buat,keterangan,no_spph,alasan,no_dokumen,modul,no_sanggup) values "+
							"('"+this.e_nb.getText()+"','-','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_buat.getText()+"','"+this.e_ket.getText()+"','"+this.cb_spph.getText()+"','"+urlencode(this.mDesk.getCode())+"','"+this.e_dok.getText()+"','"+this.jenisLog+"','-')");					
					sql.add("insert into sl_pesan_dok(no_pesan,no_gambar,nu,kode_jenis,kode_lokasi) values('"+this.e_nb.getText()+"','"+this.e_file.getText()+"',0,'TAP','"+this.app._lokasi+"')");					
					sql.add("update sl_spph_m set no_tap='"+this.e_nb.getText()+"' where no_spph='"+this.cb_spph.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
					
					if (this.sg2.getRowValidCount() > 0){
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								if (this.sg2.cells(0,i) == "CLOSE") {
									sql.add("update sl_nego_m set no_tap = '"+this.e_nb.getText()+"' where no_nego='"+this.sg2.cells(3,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
								}
							}
						}
					}
		...
		if (sender == this.cb_spph && this.cb_spph.getText()!="") {			
			if (this.stsSimpan == 1) {
				var strSQL = "select 'OPEN' as status,c.kode_vendor,c.nama,b.no_nego,b.keterangan,a.nilai as nilai_tawar,b.nilai as nilai_final,b.ppn as ppn_final, b.nilai + b.ppn as total_final,  a.modul,a.kode_curr "+
							 "from sl_sph_m a inner join sl_nego_m b on a.no_nego=b.no_nego and a.kode_lokasi=b.kode_lokasi "+
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "") {
			...
			this.stsSimpan = 1;
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"sl_tap_m","no_tap",this.app._lokasi+"-TAP"+this.e_periode.getText().substr(2,4)+".","0000"));						
			this.e_ket.setFocus();
	...
	doDoubleClick2: function(sender, col , row) {
		try {
			if (this.sg2.cells(3,row) != "") {				
				var strSQL = "select a.id_sph,a.no_urut,a.item,a.merk,a.tipe,a.catatan,a.jumlah,a.nilai_nego,a.jumlah*(a.nilai_nego + a.ppn_nego) as total "+
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_tap,convert(varchar,a.tanggal,103) as tgl,a.keterangan,b.nama,c.nilai+c.ppn as nilai "+
		             "from sl_tap_m a "+
					 "inner join sl_nego_m c on a.no_tap = c.no_tap and a.kode_lokasi=c.kode_lokasi "+
					 "inner join vendor b on c.kode_vendor=b.kode_vendor and c.kode_lokasi=b.kode_lokasi "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_spk='-' and a.modul = 'PO-SPK'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				this.cb_spph.setSQL("select no_spph, keterangan from sl_spph_m a where no_tap='"+this.e_nb.getText()+"'",["no_spph","keterangan"],false,["No SPPH","Deskripsi"],"and","Data SPPH",true);			
				
				var strSQL = "select a.keterangan,a.tanggal,b.no_gambar,a.no_spph,a.alasan,a.no_dokumen,a.nik_buat,a.modul "+
							 "from sl_tap_m a inner join sl_pesan_dok b on a.no_tap=b.no_pesan and a.kode_lokasi=b.kode_lokasi and b.kode_jenis='TAP' "+							 							 
