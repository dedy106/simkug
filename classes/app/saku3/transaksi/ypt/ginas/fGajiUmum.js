window.app_saku3_transaksi_ypt_ginas_fGajiUmum = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_ginas_fGajiUmum.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_ypt_ginas_fGajiUmum";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Data Penggajian Umum", 0);	
		...
		this.sg1 = new portalui_saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:10,tag:9,
				colTitle:["NIK","Nama",	
						  "GaPok","TunMK","TunJab","Lembur",
						  "Lain","Trengginas","Payroll","BPJS"],
				colWidth:[[9,8,7,6,5,4,3,2,1,0],[80,80,80,80,80,80,80,80, 200,100]],
...
window.app_saku3_transaksi_ypt_ginas_fGajiUmum.extend(window.portalui_childForm);
window.app_saku3_transaksi_ypt_ginas_fGajiUmum.implement({	
	...
	doValid: function() {
		try {
			...
			for (var i=0; i < this.sg1.getRowCount();i++){
				...
				totpdpt += nilaiToFloat(this.sg1.cells(2,i)) + nilaiToFloat(this.sg1.cells(3,i)) + 
						   nilaiToFloat(this.sg1.cells(4,i)) + nilaiToFloat(this.sg1.cells(5,i));

				totpot +=  nilaiToFloat(this.sg1.cells(6,i)) + nilaiToFloat(this.sg1.cells(7,i)) + 
						   nilaiToFloat(this.sg1.cells(8,i)) + nilaiToFloat(this.sg1.cells(9,i));						   
				...
				try{														
					...
					sql.add("insert into hr_gaji_m(no_gaji,kode_lokasi,periode,tanggal,no_dokumen,keterangan,nik_buat,nik_app,posted,tgl_transfer,tgl_input,nik_user,nilai,kode_pp,kode_drk,form) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+
								 ...
								 this.cb_pp.getText()+"','"+this.cb_drk.getText()+"','GJUMUM')");					

					for (var i=0;i < this.sg1.getRowCount();i++){	
						sql.add("insert into hr_gaji_loadumum(no_gaji,kode_lokasi,nik,gapok,tunmk,tunjab,lembur, lain,ginas,payroll,bpjs) values "+
								...
								nilaiToFloat(this.sg1.cells(9,i))+")");
								
						//pdpt
						sql.add("insert into hr_gaji_d(no_gaji,nik,kode_param,kode_lokasi,kode_akun,periode,nilai,dc) values "+
								"('"+this.e_nb.getText()+"','"+this.sg1.cells(0,i)+"','NGPOK','"+this.app._lokasi+"','X','"+this.e_periode.getText()+"',"+nilaiToFloat(this.sg1.cells(2,i))+",'X')");
						sql.add("insert into hr_gaji_d(no_gaji,nik,kode_param,kode_lokasi,kode_akun,periode,nilai,dc) values "+
								"('"+this.e_nb.getText()+"','"+this.sg1.cells(0,i)+"','NTMK','"+this.app._lokasi+"','X','"+this.e_periode.getText()+"',"+nilaiToFloat(this.sg1.cells(3,i))+",'X')");
						sql.add("insert into hr_gaji_d(no_gaji,nik,kode_param,kode_lokasi,kode_akun,periode,nilai,dc) values "+
								"('"+this.e_nb.getText()+"','"+this.sg1.cells(0,i)+"','NTJAB','"+this.app._lokasi+"','X','"+this.e_periode.getText()+"',"+nilaiToFloat(this.sg1.cells(4,i))+",'X')");		
						sql.add("insert into hr_gaji_d(no_gaji,nik,kode_param,kode_lokasi,kode_akun,periode,nilai,dc) values "+
								"('"+this.e_nb.getText()+"','"+this.sg1.cells(0,i)+"','NLBR','"+this.app._lokasi+"','X','"+this.e_periode.getText()+"',"+nilaiToFloat(this.sg1.cells(5,i))+",'X')");		

						//potongan
						sql.add("insert into hr_gaji_d(no_gaji,nik,kode_param,kode_lokasi,kode_akun,periode,nilai,dc) values "+
								"('"+this.e_nb.getText()+"','"+this.sg1.cells(0,i)+"','NLAIN','"+this.app._lokasi+"','X','"+this.e_periode.getText()+"',"+nilaiToFloat(this.sg1.cells(6,i))+",'X')");						
						sql.add("insert into hr_gaji_d(no_gaji,nik,kode_param,kode_lokasi,kode_akun,periode,nilai,dc) values "+
								"('"+this.e_nb.getText()+"','"+this.sg1.cells(0,i)+"','NGINS','"+this.app._lokasi+"','X','"+this.e_periode.getText()+"',"+nilaiToFloat(this.sg1.cells(7,i))+",'X')");
						sql.add("insert into hr_gaji_d(no_gaji,nik,kode_param,kode_lokasi,kode_akun,periode,nilai,dc) values "+
								"('"+this.e_nb.getText()+"','"+this.sg1.cells(0,i)+"','NPYROL','"+this.app._lokasi+"','X','"+this.e_periode.getText()+"',"+nilaiToFloat(this.sg1.cells(8,i))+",'X')");
						sql.add("insert into hr_gaji_d(no_gaji,nik,kode_param,kode_lokasi,kode_akun,periode,nilai,dc) values "+
								"('"+this.e_nb.getText()+"','"+this.sg1.cells(0,i)+"','NBPJS','"+this.app._lokasi+"','X','"+this.e_periode.getText()+"',"+nilaiToFloat(this.sg1.cells(9,i))+",'X')");						
					}
		...
		switch (event){
			...
			case "hapus" :	
				...
				if (parseFloat(this.app._periode) > parseFloat(this.e_periode.getText())){
					...
					sql.add("delete from hr_gaji_loadumum where no_gaji = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doLoad3:function(sender){																		
		var strSQL = "select no_gaji,convert(varchar,tanggal,103) as tgl,no_dokumen,keterangan,nilai "+
		             "from hr_gaji_m a "+					 					 
					 "where periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and posted ='F' and form ='GJUMUM'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
