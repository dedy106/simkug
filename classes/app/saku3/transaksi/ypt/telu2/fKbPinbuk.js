window.app_saku3_transaksi_ypt_telu2_fKbPinbuk = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_telu2_fKbPinbuk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_telu2_fKbPinbuk";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Pinbuk", 0);	
		...
		this.c_jenis = new saiCB(this.pc2.childPage[1],{bound:[20,11,200,20],caption:"Modul",items:["BK"], readOnly:true,tag:2,change:[this,"doChange"]});		
		...
		this.pc1 = new pageControl(this.pc2.childPage[1],{bound:[5,12,990,303], childPage:["Data Item Jurnal"]});
...
window.app_saku3_transaksi_ypt_telu2_fKbPinbuk.extend(window.childForm);
window.app_saku3_transaksi_ypt_telu2_fKbPinbuk.implement({		
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :															
				...
				if (!akunKB) {
					system.alert(this,"Transaksi tidak valid.","Akun KasBank tidak ditemukan sesuai posisinya(C).");
					return false;						
				}
				this.sg.validasi();																
				for (var i=0;i < this.sg.getRowCount();i++){					
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.jenis,a.no_dokumen,a.keterangan "+
		             "from kas_m a "+					 					 
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'KBPINBUK' and a.posted ='F'";		
