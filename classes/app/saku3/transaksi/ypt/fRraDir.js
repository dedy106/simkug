window.app_saku3_transaksi_ypt_fRraDir = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_fRraDir.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_fRraDir";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form RRA Direct", 0);	
		...
		this.pc2 = new pageControl(this,{bound:[10,10,1000,430], childPage:["Data RRA"]});		
		/*
		...
		*/
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,5,995,305], childPage:["Penerima","Pemberi","Cek Anggaran"]});		
		
		this.cb_lokasi = new saiCBBL(this.pc1.childPage[0],{bound:[20,14,220,20],caption:"Lokasi Tujuan", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});										
...
window.app_saku3_transaksi_ypt_fRraDir.extend(window.childForm);
window.app_saku3_transaksi_ypt_fRraDir.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into rra_pdrk_m(no_pdrk,kode_lokasi,keterangan,kode_pp,kode_bidang,jenis_agg,tanggal,periode,nik_buat,nik_app1,nik_app2,nik_app3,sts_pdrk,justifikasi, nik_user, tgl_input,progress,modul) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_periode.getText()+"','"+this.app._userLog+"','"+this.app._userLog+"','"+this.cb_app.getText()+"','"+this.cb_app.getText()+"','RRR','"+this.cb_lokasi.getText()+"','"+this.app._userLog+"',getdate(),'0','MULTILOK')");
					...
					sql.add("insert into rra_pdrk_d(no_pdrk,kode_lokasi,no_urut,kode_akun,kode_pp,kode_drk,periode,saldo,nilai,dc,target) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_lokasi.getText()+"',999,'"+this.cb_akunT.getText()+"','"+this.cb_ppT.getText()+"','"+this.cb_drkT.getText()+"','"+periode+"',0,"+nilaiToFloat(this.e_terima.getText())+",'D','-')");					
					sql.add("insert into anggaran_d(no_agg,kode_lokasi,no_urut,kode_pp,kode_akun,kode_drk,volume,periode,nilai_sat,nilai,dc,satuan,nik_user,tgl_input,modul) values "+		
							"('"+this.e_nb.getText()+"','"+this.cb_lokasi.getText()+"',999,'"+this.cb_ppT.getText()+"','"+this.cb_akunT.getText()+"','"+this.cb_drkT.getText()+"',999,'"+periode+"',"+nilaiToFloat(this.e_terima.getText())+","+nilaiToFloat(this.e_terima.getText())+",'D','-','"+this.app._userLog+"',getdate(),'RRA')");
	...
	doChange:function(sender){																		
		try{
			if ((sender == this.cb_lokasi || sender == this.e_periode) && this.cb_lokasi.getText()!="" && this.e_periode.getText()!="") {
				this.cb_ppT.setSQL("select a.kode_pp,a.nama from pp a where a.flag_aktif= '1' and a.kode_lokasi = '"+this.cb_lokasi.getText()+"'",["c.kode_pp","c.nama"],false,["Kode","Nama"],"and","Data PP",true);						
			...
			if (sender == this.c_bulanT || sender == this.cb_drkT || sender == this.cb_ppT || sender == this.cb_akunT) {				
				...
				if (this.cb_akunT.getText()!="" && this.cb_ppT.getText()!="" && this.cb_drkT.getText()!="" && this.c_bulanT.getText()!="") {
					var data = this.dbLib.getDataProvider("select fn_cekaggBulan('"+this.cb_ppT.getText()+"','"+this.cb_lokasi.getText()+"','"+this.cb_akunT.getText()+"','"+this.cb_drkT.getText()+"','"+this.e_periode.getText().substr(0,4)+this.c_bulanT.getText()+"','"+this.e_nb.getText()+"') as gar ",true);
					if (typeof data == "object" && data.rs.rows[0] != undefined){
						var line = data.rs.rows[0];
						data = line.gar.split(";");
	...
	/*	
	tidak boleh koreksi .. karena budget sudah langsung release (takutnya sudah terpakai)
	koreksi --> lakukan jurnal rra sebaliknya
	...
	doLoad3:function(sender){																						
		var strSQL = "select a.no_pdrk,convert(varchar,a.tanggal,103) as tgl,b.no_dokumen,a.keterangan,a.progress "+
					 ...
					 "where a.modul = 'MULTILOK' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress in ('0','R') order by a.tanggal";		
	...
	doChangeCell: function(sender, col, row){
		...
		if (col == 0 || col == 2 || col == 4 || col == 6) {
			if (sender.cells(0,row) != "" && sender.cells(2,row) != "" && sender.cells(4,row) != "" && sender.cells(6,row) != "") {
				...
				var data = this.dbLib.getDataProvider("select fn_cekaggBulan('"+this.sg.cells(2,row)+"','"+this.app._lokasi+"','"+this.sg.cells(0,row)+"','"+this.sg.cells(4,row)+"','"+this.e_periode.getText().substr(0,4)+this.sg.cells(6,row)+"','"+this.e_nb.getText()+"') as gar ",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line = data.rs.rows[0];
					data = line.gar.split(";");
					sls = parseFloat(data[0]) - parseFloat(data[1]);
