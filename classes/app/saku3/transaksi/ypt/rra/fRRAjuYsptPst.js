window.app_saku3_transaksi_ypt_rra_fRRAjuYsptPst = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_rra_fRRAjuYsptPst.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_rra_fRRAjuYsptPst";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Reprogramming - Reschedulling YSPT Pusat", 0);	
		...
		try {
			...
			this.cb_ppT.setSQL("select c.kode_pp, c.nama from pp c where  c.tipe='posting' and c.flag_aktif='1' and c.kode_lokasi='"+this.app._lokasi+"'",["c.kode_pp","c.nama"],false,["Kode","Nama"],"and","Data PP",true);									
			this.cb_ppT.setText(this.app._kodePP);
			this.cb_akunT.setSQL("select a.kode_akun,a.nama from masakun a where a.kode_lokasi='"+this.app._lokasi+"' and a.status_gar in ('0','1') ",["a.kode_akun","a.nama"],false,["Kode","Nama"],"and","Data Akun",true);		
			
			var sql = new server_util_arrayList(); 
			sql.add("select kode_akun, nama from masakun where status_gar in ('0','1') and block='0' and kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_transaksi_ypt_rra_fRRAjuYsptPst.extend(window.childForm);
window.app_saku3_transaksi_ypt_rra_fRRAjuYsptPst.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into rra_pdrk_m(no_pdrk,kode_lokasi,keterangan,kode_pp,kode_bidang,jenis_agg,tanggal,periode,nik_buat,nik_app1,nik_app2,nik_app3,sts_pdrk,justifikasi, nik_user, tgl_input,progress,modul) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_periode.getText()+"','"+this.app._userLog+"','"+this.app._userLog+"','"+this.cb_app.getText()+"','"+this.cb_app.getText()+"','RRR','-','"+this.app._userLog+"',getdate(),'0','PUSAT')");
	    			...
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){							
							if (this.preView == "1") {		
								...
								this.nama_report = "server_report_saku3_rra_rptAggDisPos";
	...
	doLoad3:function(sender){																						
		var strSQL = "select a.no_pdrk,convert(varchar,a.tanggal,103) as tgl,b.no_dokumen,a.keterangan,a.progress "+
					 ...
					 "where a.modul = 'PUSAT' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress in ('0','R') order by a.tanggal";		
			...
			if (sender == this.sg) {
				...
				if (col == 2){
					this.standarLib.showListData(this, "Daftar PP/Unit",sender,undefined, 												  												  
								"select a.kode_pp, a.nama  from pp a where a.kode_lokasi = '"+this.app._lokasi+"' and a.tipe='posting' and a.flag_aktif ='1'",
								"select count(*) from pp a where a.kode_lokasi = '"+this.app._lokasi+"' and a.tipe='posting' and a.flag_aktif ='1'",						
