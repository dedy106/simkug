window.app_saku3_transaksi_ypt_rra_fAppRRAYspt = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_rra_fAppRRAYspt.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_rra_fAppRRAYspt";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Approval RRA YSPT PUSAT", 0);	
		...
		this.e_nopdrk = new portalui_saiLabelEdit(this.pc1.childPage[1],{bound:[790,15,200,20],caption:"No Usulan",readOnly:true});
		
		this.sg2 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,295],colCount:9,
...
window.app_saku3_transaksi_ypt_rra_fAppRRAYspt.extend(window.childForm);
window.app_saku3_transaksi_ypt_rra_fAppRRAYspt.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{				
					...
					sql.add("update rra_pdrk_m set progress='"+vProg+"' where no_pdrk='"+this.e_nopdrk.getText()+"'");							
	...
	doClick:function(sender){
		if (sender == this.i_gen) {
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"rra_app_m","no_app",this.app._lokasi+"-ARR"+this.e_periode.getText().substr(2,4)+".","0000"));
			this.e_ket.setFocus();
		}		
	},	
	doLoad:function(sender){		
		var strSQL = "select a.kode_lokasi,a.kode_pp+' - '+c.nama as pp,a.no_pdrk as no_bukti,b.no_dokumen,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,a.sts_pdrk as jenis "+
					 ...
					 "where a.modul in ('UNIT','PUSAT') and a.progress='0' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' ";							
	...
	doDoubleClick: function(sender, col , row) {
		if (this.sg.cells(1,row) != "") {
			...
			var strSQL = "select c.nama_file from rra_pdrk_dok c where c.no_bukti='"+this.e_nopdrk.getText()+"' ";
