window.app_saku3_transaksi_ypt_fTakKirimDir = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ypt_fTakKirimDir.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ypt_fTakKirimDir";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form TAK Direct", 0);	
		...
		this.e_nb2 = new portalui_saiLabelEdit(this.pc2.childPage[0],{bound:[20,22,200,20],caption:"No Terima",maxLength:30,readOnly:true});
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,12,990,278], childPage:["Item Kirim","Item Terima","Budget Kirim","Budget Terima"]});
		...
		this.sg1 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:0,
					...
					pasteEnable:true,autoPaging:true,rowPerPage:200,afterPaste:[this,"doAfterPaste1"],
					autoAppend:true,defaultRow:1});
		this.sgn1 = new portalui_sgNavigator(this.pc1.childPage[1],{bound:[1,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:2,grid:this.sg1,pager:[this,"doPage1"]});		
		...
		this.sg4 = new saiGrid(this.pc1.childPage[3],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:9,
		            colTitle:["Kode Akun","Nama Akun","Kode PP","Nama PP","Kode DRK","Nama DRK","Saldo Awal","Nilai","Saldo Akhir"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[80,80,80,150,80,150,80,150,80]],
					readOnly:true,colFormat:[[6,7,8],[cfNilai,cfNilai,cfNilai]],autoAppend:true,defaultRow:1});
		this.sgn4 = new portalui_sgNavigator(this.pc1.childPage[3],{bound:[1,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:3,grid:this.sg2});
		this.i_budget4 = new portalui_imageButton(this.sgn2,{bound:[955,2,20,20],hint:"Cek Anggaran",image:"icon/"+system.getThemes()+"/tabCont2.png",click:[this,"doHitungGar4"]});
...
window.app_saku3_transaksi_ypt_fTakKirimDir.extend(window.childForm);
window.app_saku3_transaksi_ypt_fTakKirimDir.implement({	
	...
	simpan: function(){			
		try{						
			if (this.stsSimpan == 1) {
				this.doClick();
				this.e_nb2.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"takterima_m","no_terima",this.cb_lokasi.getText()+"-TT"+this.e_periode.getText().substr(2,4)+".","0000"));						
			}
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from takterima_m where no_terima ='"+this.e_nb2.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");
						sql.add("delete from takterima_j where no_terima ='"+this.e_nb2.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");
					...
					sql.add("insert into takkirim_m(no_kirim,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user,kode_loktuj,progress,no_terima,due_date) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','TAKUMUMDIR','KIRIM','IDR',1,"+nilaiToFloat(this.e_debet.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"','F','-','-','"+this.cb_tak.getText()+"',getdate(),'"+this.app._userLog+"','"+this.cb_lokasi.getText()+"','1','"+this.e_nb2.getText()+"','"+this.dp_d1.getDateString()+"')");
					...
					sql.add("insert into takkirim_j(no_kirim,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.cb_tak.getText()+"','"+this.e_ket.getText()+"','"+this.dcMutKirim+"',"+nilaiToFloat(this.e_debet.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','TAKUMUMDIR','TAK','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");

					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){															           
								sql.add("insert into takkirim_j(no_kirim,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','"+this.app._lokasi+"','TAKUMUMDIR','JURNAL','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");
							}
						}
					}
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"('"+this.e_nb.getText()+"','TAKUMUMDIR','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.sg2.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(6,i))+","+nilai+")");
							}
						}
					}
					...
					sql.add("insert into takterima_m (no_terima,kode_lokasi,no_dokumen,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_setuju,tgl_input,nik_user,posted,no_del,no_link,kode_lokkirim,no_kirim) values "+
					        "('"+this.e_nb2.getText()+"','"+this.cb_lokasi.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.ppTerimaDefault+"','TAKUMUMDIR','TAKTERIMA','"+this.e_periode.getText()+"','IDR',1,"+nilaiToFloat(this.e_debet.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','-','"+this.app._lokasi+"','"+this.e_nb.getText()+"')");
					...
					sql.add("insert into takterima_j(no_terima,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb2.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',998,'"+this.cb_tak.getText()+"','"+this.e_ket.getText()+"','"+this.dcMutTerima+"',"+nilaiToFloat(this.e_debet.getText())+",'"+this.ppTerimaDefault+"','-','"+this.cb_lokasi.getText()+"','TAKUMUMDIR','TAK','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");			

					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){	
								sql.add("insert into takterima_j(no_terima,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb2.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"','"+this.sg1.cells(7,i)+"','"+this.cb_lokasi.getText()+"','TAKUMUMDIR','JURNAL','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");
							}
						}
					}
						...
						for (var i=0;i < this.sg4.getRowCount();i++){
							if (this.sg4.rowValid(i)){
								if (nilaiToFloat(this.sg4.cells(7,i)) > 0) {
									var DC = "D"; 
									var nilai = nilaiToFloat(this.sg4.cells(7,i));
								} else {
									var DC = "C";
									var nilai = nilaiToFloat(this.sg4.cells(7,i)) * -1;
								}
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"('"+this.e_nb2.getText()+"','TAKUMUMDIR','"+this.cb_lokasi.getText()+"','"+this.sg4.cells(0,i)+"','"+this.sg4.cells(2,i)+"','"+this.sg4.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg4.cells(6,i))+","+nilai+")");
							}
						}
					}
		...
		switch (event){
			...
			case "ubah" :															
				...
				if (this.flagDokFree == "1") {				
					var data = this.dbLib.getDataProvider("select no_kirim from takkirim_m where no_kirim <>'"+this.e_nb.getText()+"' and no_dokumen='"+this.e_dok.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
						if (line != undefined){
							system.alert(this,"No Dokumen sudah terpakai.","Terpakai di no bukti : "+line.no_kirim);
							return false;
						} 
					}
				}				
				...
				if (this.flagGarFree == "0") {
					for (var i=0;i < this.sg2.getRowCount();i++){
						if (nilaiToFloat(this.sg2.cells(7,i))>0 && nilaiToFloat(this.sg2.cells(6,i)) < nilaiToFloat(this.sg2.cells(7,i))) {
							var k =i+1;
							system.alert(this,"Transaksi tidak valid (Kirim).","Saldo Anggaran tidak mencukupi. [Baris : "+k+"]");
							return false;						
						}
					}
				}
				...
				//terima
				this.dataAkunGar = {rs:{rows:[]}};
				var data = this.dbLib.getDataProvider("select kode_akun from masakun where status_gar = '1' and kode_lokasi='"+this.cb_lokasi.getText()+"'",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					this.dataAkunGar = data;
				}
				this.sg1.validasi();																
				for (var i=0;i < this.sg1.getRowCount();i++){
					if (!this.sg1.rowValid(i)){
						...
						if (!isKosong) {
							system.alert(this,"Transaksi tidak valid (Terima).","Terdapat kolom yang kosong.");
							return false;
						}
					}
					else {
						for (var j=0;j<this.dataAkunGar.rs.rows.length;j++){
							...
							if (line.kode_akun == this.sg1.cells(0,i) && this.sg1.cells(7,i) == "-") {		
								var k = i+1;
								system.alert(this,"Transaksi tidak valid (Terima).","Akun Anggaran Harus diisi DRK.[Baris : "+k+"]");
								return false;						
							}
						}
					}
				...
				if (this.flagGarFree == "0") {
					for (var i=0;i < this.sg4.getRowCount();i++){
						if (nilaiToFloat(this.sg4.cells(7,i))>0 && nilaiToFloat(this.sg4.cells(6,i)) < nilaiToFloat(this.sg4.cells(7,i))) {
							var k =i+1;
							system.alert(this,"Transaksi tidak valid (Terima).","Saldo Anggaran tidak mencukupi. [Baris : "+k+"]");
							return false;						
						}
					}
				}
				...
				this.app._periode = this.dbLib.getPeriode(this.cb_lokasi.getText());																	
				if (parseFloat(this.app._periode) > parseFloat(this.e_periode.getText())){
					system.alert(this,"Periode transaksi tidak valid (LOKASI TERIMA).","Periode transaksi tidak boleh kurang dari periode aktif sistem.["+this.app._periode+"]");
					return false;
				}
				*/
	...
	doChange:function(sender){
		...
		if (sender == this.cb_lokasi && this.cb_lokasi.getText()!="")	{
			var sql = new server_util_arrayList();
			sql.add("select distinct a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag in ('034') where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'");						
			sql.add("select kode_pp,nama from pp where flag_aktif='1' and kode_lokasi = '"+this.app._lokasi+"'");						
			sql.add("select kode_pp,nama from pp where flag_aktif='1' and kode_lokasi = '"+this.cb_lokasi.getText()+"'");						
	...
	doChangeCell: function(sender, col, row){
		...
		this.sgn.setTotalPage(Math.ceil(this.sg.getRowCount() / 20));		
	},
	doNilaiChange: function(){
		try{
			var totD = totC = totKirim = 0;
			this.dcKirim = "";
			for (var i = 0; i < this.sg.getRowCount();i++){
				if (this.sg.rowValid(i) && this.sg.cells(4,i) != ""){
					if (this.sg.cells(2,i).toUpperCase() == "D") {
						totD += nilaiToFloat(this.sg.cells(4,i));
						totKirim += nilaiToFloat(this.sg.cells(4,i));
					}
			...
			for (var i = 0; i < this.sg1.getRowCount();i++){
				if (this.sg1.rowValid(i) && this.sg1.cells(4,i) != ""){
					this.ppTerimaDefault = this.sg1.cells(5,0);
			...
			if (sender == this.sg) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Akun",sender,undefined, 
						    "select distinct a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag in ('034') where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'",
							"select count(distinct a.kode_akun) from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag in ('034') where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'",
	...
	doChangeCell1: function(sender, col, row){
		...
		if (col == 5) {
			if (this.sg1.cells(5,row) != "") {
				var pp = this.dataPP1.get(sender.cells(5,row));
				if (pp) sender.cells(6,row,pp);
				else {
					if (trim(sender.cells(5,row)) != "") system.alert(this,"Kode PP "+sender.cells(5,row)+" tidak ditemukan","Inputkan kode lainnya.","checkPP");                
					sender.cells(5,row,"");
	...
	doEllipsClick1: function(sender, col, row){
		try{			
			if (sender == this.sg1) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Akun",sender,undefined, 
						    "select distinct a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag in ('034') where a.block= '0' and a.kode_lokasi = '"+this.cb_lokasi.getText()+"'",
							"select count(distinct a.kode_akun) from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag in ('034') where a.block= '0' and a.kode_lokasi = '"+this.cb_lokasi.getText()+"'",
							["a.kode_akun","a.nama"],"and",["Kode","Nama"],false);				
				}
				if (col == 5){
					this.standarLib.showListData(this, "Daftar PP/Unit",sender,undefined, 
				...
				if (col == 7){					
					var vUnion = "";
					var data = this.dbLib.getDataProvider("select status_gar from masakun where kode_akun='"+sender.cells(0,row)+"' and kode_lokasi='"+this.cb_lokasi.getText()+"'",true);
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
						if (line != undefined){
							if (line.status_gar != "1") var vUnion = " union select '-','-' "; 
					...
					this.standarLib.showListData(this, "Daftar DRK",sender,undefined, 
							"select distinct a.kode_drk, a.nama from drk a inner join anggaran_d b on a.kode_drk=b.kode_drk where a.tahun=substring(b.periode,1,4) and b.periode like '"+this.e_periode.getText().substr(0,4)+"%' and b.kode_akun='"+sender.cells(0,row)+"' and b.kode_pp = '"+sender.cells(5,row)+"' and a.kode_lokasi='"+this.cb_lokasi.getText()+"' "+vUnion ,
							"select count(distinct a.kode_drk)  from drk a inner join anggaran_d b on a.kode_drk=b.kode_drk where a.tahun=substring(b.periode,1,4) and b.periode like '"+this.e_periode.getText().substr(0,4)+"%' and b.kode_akun='"+sender.cells(0,row)+"' and b.kode_pp = '"+sender.cells(5,row)+"' and a.kode_lokasi='"+this.cb_lokasi.getText()+"' ",
							["a.kode_drk","a.nama"],"and",["Kode DRK","Nama DRK"],false);
				}
			}
		}catch(e){
				...
				switch(methodName){
					...
					case "getMultiDataProvider":
						...
						if (typeof result != "string"){
							...
							this.dataPP1 = new portalui_arrayMap();														
							if (result.result[0]){	    			        
								var line;
							...
							if (result.result[2]){	    			        
								...
								for (var i in result.result[2].rs.rows){
									...
									this.dataPP1.set(line.kode_pp, line.nama);										
								}								
							}							
						}else throw result;
					break;
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_kirim,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.kode_loktuj+' - '+b.nama as lokasi,a.nilai "+
					 ...
					 "inner join takterima_m c on a.no_terima=c.no_terima and c.posted='F' "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'TAKUMUMDIR' and a.posted ='F' ";		
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.keterangan,a.no_dokumen,a.tanggal,a.kode_loktuj,a.ref1,a.no_terima "+
							 "from takkirim_m a inner join takterima_m b on a.no_terima=b.no_terima "+							 
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						this.e_nb2.setText(line.no_terima);	
					}
				}								
				...
				var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk "+
							...
							"where a.jenis='JURNAL' and a.no_kirim = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
					for (var i in data.rs.rows){
				...
				var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk "+
							...
							"                  inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+													
							"                  left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and substring(a.periode,1,4)=d.tahun "+													
							"where a.jenis='JURNAL' and a.no_terima = '"+this.e_nb2.getText()+"' and a.kode_lokasi='"+this.cb_lokasi.getText()+"' order by a.no_urut",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg1.clear();
					for (var i in data.rs.rows){
				...
				var data = this.dbLib.getDataProvider(
							...
							"              inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
							"              left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and d.tahun=substring(a.periode1,1,4) "+
							"where a.no_bukti = '"+this.e_nb2.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.kode_akun",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg4.clear();
					for (var i in data.rs.rows){
	...
	doHitungGar1: function(){
		...
		for (var i=0;i < this.sg1.getRowCount();i++){
			if (this.sg1.rowValid(i) && this.sg1.cells(5,i) != "-" && this.sg1.cells(7,i)!= "-"){
				...
				for (var j=0;j < this.sg4.getRowCount();j++){
					if (this.sg1.cells(0,i) == this.sg4.cells(0,j) && this.sg1.cells(5,i) == this.sg4.cells(2,j) && this.sg1.cells(7,i) == this.sg4.cells(4,j)) {
						isAda = true;
						idx = j;
						break;
					}
				...
				if (!isAda) {
					this.sg4.appendData([this.sg1.cells(0,i),this.sg1.cells(1,i),this.sg1.cells(5,i),this.sg1.cells(6,i),this.sg1.cells(7,i),this.sg1.cells(8,i),"0",floatToNilai(nilai),"0"]);
				} 
				else { 
		...
		var sls = 0;
		for (var i=0;i < this.sg4.getRowCount();i++){
			if (this.stsSimpan == 1) var data = this.dbLib.getDataProvider("select fn_cekagg2('"+this.sg4.cells(2,i)+"','"+this.cb_lokasi.getText()+"','"+this.sg4.cells(0,i)+"','"+this.sg4.cells(4,i)+"','"+this.e_periode.getText()+"') as gar ",true);
			else var data = this.dbLib.getDataProvider("select fn_cekagg3('"+this.sg4.cells(2,i)+"','"+this.cb_lokasi.getText()+"','"+this.sg4.cells(0,i)+"','"+this.sg4.cells(4,i)+"','"+this.e_periode.getText()+"','"+this.e_nb2.getText()+"') as gar ",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line = data.rs.rows[0];
				data = line.gar.split(";");
				sls = parseFloat(data[0]) - parseFloat(data[1]);
