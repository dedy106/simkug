window.app_saku3_transaksi_fitnes_fKbPending = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_fitnes_fKbPending.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_fitnes_fKbPending";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pelunasan Pendaftaran", 0);	
		...
		this.cb_mcu = new saiCBBL(this.pc2.childPage[0],{bound:[20,14,220,20],caption:"Hasil MCU", readOnly:true, maxLength:10, tag:2});
		this.e_jml = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,18,200,20],caption:"Jml Bulan", tag:1, tipeText:ttNilai, text:"0",change:[this,"doChange"],readOnly:true});		
		...
		try {
			...
			this.cb_agg.setSQL("select a.kode_agg, a.nama from fi_anggota a inner join fi_reg b on a.kode_agg=b.kode_agg "+
			                   "where b.status_bayar='PENDING' and a.kode_lokasi='"+this.app._lokasi+"'",["kode_agg","nama"],false,["Kode","Nama"],"and","Data Anggota",true);	
...
window.app_saku3_transaksi_fitnes_fKbPending.extend(window.childForm);
window.app_saku3_transaksi_fitnes_fKbPending.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						sql.add("update fi_reg set status_bayar='PENDING' where no_reg = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add("update fi_reg set status_bayar='CASH' where no_reg = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_agg && this.cb_agg.getText() != ""){
				var data = this.dbLib.getDataProvider("select b.p_diskon,b.kode_klp+' - '+b.nama as klp,b.kode_klp,c.no_reg "+
				                                      "from fi_anggota a inner join fi_peserta_klp b on a.kode_klp=b.kode_klp and a.kode_lokasi=b.kode_lokasi "+
													  "                  inner join fi_reg c on a.kode_agg=c.kode_agg and c.status_bayar='PENDING' "+
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){													
						...
						this.e_jml.setText(floatToNilai(line.jumlah));
						this.e_nilai.setText(floatToNilai(line.nilai));
						this.cb_agg.setSQL("select a.kode_agg, a.nama from fi_anggota a  "+
			                  			   "where a.kode_agg='"+line.kode_agg+"' and a.kode_lokasi='"+this.app._lokasi+"'",["kode_agg","nama"],false,["Kode","Nama"],"and","Data Anggota",true);	
