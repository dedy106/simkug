window.app_saku3_transaksi_fitnes_fParam = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_fitnes_fParam.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_fitnes_fParam";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Parameter Pengukuran", 0);	
...
window.app_saku3_transaksi_fitnes_fParam.extend(window.childForm);
window.app_saku3_transaksi_fitnes_fParam.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into fi_param(kode_param,kode_lokasi,nama,satuan) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from fi_param where kode_param='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("insert into fi_param(kode_param,kode_lokasi,nama,satuan) values "+
						    "('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.e_satuan.getText()+"')");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select kode_param,nama,satuan "+
				             "from fi_param "+
		...
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Parameter",sender,undefined, 
											  "select kode_param, nama  from fi_param where kode_lokasi='"+this.app._lokasi+"'",
											  "select count(kode_param) from fi_param where kode_lokasi='"+this.app._lokasi+"'",
		...
		try {
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i=0;i<this.dataJU.rs.rows.length;i++){
					...
					this.sg1.appendData([line.kode_param,line.nama,line.satuan]); 
				}
