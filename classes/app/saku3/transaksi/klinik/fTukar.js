window.app_saku3_transaksi_klinik_fTukar = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_klinik_fTukar.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_klinik_fTukar";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penukaran Obat: Input", 0);	
		
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		this.e_periode = new portalui_saiLabelEdit(this,{bound:[20,10,200,20],caption:"Periode",readOnly:true,tag:2});
		this.l_tgl = new portalui_label(this,{bound:[20,11,100,18],caption:"Tanggal", underline:true});
		this.dp_d1 = new portalui_datePicker(this,{bound:[120,11,98,18],selectDate:[this,"doSelectDate"]});
		this.pc2 = new pageControl(this,{bound:[10,10,1000,430], childPage:["List Transaksi","Data Transaksi"]});				
		...
		this.pc1 = new pageControl(this.pc2.childPage[1],{bound:[5,12,990,280], childPage:["Data Obat Keluar","Data Obat Masuk"]});		
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:7,tag:0,
		            colTitle:["Nama","Kode","No Batch","Exp Date","Satuan","Stok","Jml Keluar"],
		...
		this.sg1 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:6,tag:0,
		            colTitle:["Nama","Kode","No Batch","Exp Date","Satuan","Jml Masuk"],
					colWidth:[[5,4,3,2,1,0],[80,50,100,100,80,330]],					
					columnReadOnly:[true,[0,1,4],[2,3,5]],
					colFormat:[[5],[cfNilai]],
					buttonStyle:[[0,2,3],[bsEllips,bsAuto,bsDate]], 					
...
window.app_saku3_transaksi_klinik_fTukar.extend(window.portalui_childForm);
window.app_saku3_transaksi_klinik_fTukar.implement({
		...
		try{					
			...
			for (var i=0;i < this.sg.getRowCount();i++){
				if (this.sg.rowValid(i)){						
					...
					if (this.stsSimpan == 1)  var data = this.dbLib.getDataProvider("select stok from kli_stok where no_batch='"+this.sg.cells(2,i)+"' and tgl_ed='"+tglED+"' and kode_obat='"+this.sg.cells(1,i)+"' and kode_pp='"+this.cb_gudang.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and nik_user='"+this.nik_user+"'",true);
									...
									"from kli_stok a left join kli_tukar_d b on a.kode_obat = b.kode_obat and a.kode_lokasi=b.kode_lokasi and a.no_batch=b.no_batch and a.tgl_ed=b.tgl_ed and b.no_tukar='"+this.e_nb.getText()+"' "+
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{																								
					...
					if (this.stsSimpan == 0) {
						sql.add("delete from kli_tukar_m where no_tukar = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
						sql.add("delete from kli_tukar_d where no_tukar = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}										
					sql.add("insert into kli_tukar_m(no_tukar,kode_lokasi,tanggal,no_dokumen,keterangan,periode,nik_user,tgl_input,kode_pp) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.cb_gudang.getText()+"')");
							
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){																																										
								var tglED = this.sg.cells(3,i).substr(6,4)+"-"+this.sg.cells(3,i).substr(3,2)+"-"+this.sg.cells(3,i).substr(0,2);
								sql.add("insert into kli_tukar_d(no_tukar,kode_lokasi,periode,nu,kode_obat,kode_pp,satuan,stok,jumlah,dc,no_batch,tgl_ed) values "+  
									   "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"',"+i+",'"+this.sg.cells(1,i)+"','"+this.cb_gudang.getText()+"','"+this.sg.cells(4,i)+"',"+nilaiToFloat(this.sg.cells(5,i))+","+nilaiToFloat(this.sg.cells(6,i))+",'C','"+this.sg.cells(2,i)+"','"+tglED+"')");
							}
						}						
					}
					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){																																																		
								sql.add("insert into kli_tukar_d(no_tukar,kode_lokasi,periode,nu,kode_obat,kode_pp,satuan,stok,jumlah,dc,no_batch,tgl_ed) values "+  
									   "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"',"+i+",'"+this.sg1.cells(1,i)+"','"+this.cb_gudang.getText()+"','"+this.sg1.cells(4,i)+"',0,"+nilaiToFloat(this.sg1.cells(5,i))+",'D','"+this.sg1.cells(2,i)+"','"+this.sg1.getCellDateValue(3,i)+"')");
							}
						}						
					}					
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "") {
			...
			this.stsSimpan = 1;
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"kli_tukar_m","no_tukar",this.app._lokasi+"-TK"+this.e_periode.getText().substr(2,4)+".","0000"));			
			this.e_dok.setFocus();
		...
		try{			
			...
			if (col == 2 && this.cb_gudang.getText()!="") {			
				this.standarLib.showListData(this, "Daftar Batch",sender,undefined, 
											  "select no_batch,convert(varchar,tgl_ed,103) as tgl_ed from kli_stok where kode_pp='"+this.cb_gudang.getText()+"' and kode_obat='"+this.sg.cells(1,row)+"' and kode_lokasi='"+this.app._lokasi+"'",
											  "select count(no_batch) from kli_stok where kode_pp='"+this.cb_gudang.getText()+"' and kode_obat='"+this.sg.cells(1,row)+"' and kode_lokasi='"+this.app._lokasi+"'",
		...
		if ((col == 2 || col == 3) && this.sg.cells(1,row)!="" && this.sg.cells(2,row)!="" && this.sg.cells(3,row)!="" && this.cb_gudang.getText()!="") {												
			...
			if (this.stsSimpan == 1) {
				...
				var strSQL = "select a.sat_kecil,b.stok+isnull(c.jumlah+c.bonus,0) as stok "+
							 "from kli_obat a inner join kli_stok b on a.kode_obat=b.kode_obat and a.kode_lokasi=b.kode_lokasi and b.kode_pp='"+this.cb_gudang.getText()+"' and b.nik_user='"+this.app._userLog+"' and b.no_batch='"+this.sg.cells(2,row)+"' and b.tgl_ed='"+tglED+"' "+
							 "                left join kli_tukar_d c on b.kode_obat = c.kode_obat and b.kode_lokasi=c.kode_lokasi and b.no_batch=c.no_batch and b.tgl_ed=c.tgl_ed and c.no_tukar='"+this.e_nb.getText()+"' "+
							 "where a.kode_obat='"+this.sg.cells(1,row)+"' and a.kode_lokasi='"+this.app._lokasi+"'";
			}			
	...
	doChangeCell1: function(sender, col, row){
		if (col == 0 && this.sg1.cells(0,row)!="") {			
			if (this.sg1.cells(0,row) != "") {							
				var strSQL = "select sat_kecil from kli_obat where nama ='"+this.sg1.cells(0,row)+"' and kode_lokasi='"+this.app._lokasi+"'";
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){										
						this.sg1.cells(4,row,line.sat_kecil);						
						this.sg1.cells(5,row,"0");							
					} 				
				}				
			}			
		}
		if (col == 1 && this.sg1.cells(1,row)!="") {			
			var line,items = [];									
			var data = this.dbLib.getDataProvider(
				   "select no_batch from kli_sawal where kode_obat='"+this.sg1.cells(1,row)+"' and kode_lokasi='"+this.app._lokasi+"' "+
				   "union "+
				   "select no_batch from kli_beli_d where kode_obat='"+this.sg1.cells(1,row)+"' and kode_lokasi='"+this.app._lokasi+"'",true);				
			if (typeof data == "object" && data.rs.rows[0] != undefined){					
				for (var i in data.rs.rows){
					line = data.rs.rows[i];						
					items[i]=(line.no_batch);
				}									
			}									
			this.sg1.setPickList([2],[new arrayMap({items:items})]);
		}
		if (col == 2 && this.sg1.cells(2,row)!="") {
			if (this.sg1.cells(2,row) != "") {			
				var strSQL = "select top 1 convert(varchar,tgl_ed,103) as tgl_ed from ( "+
							 "select tgl_ed from kli_sawal where kode_obat='"+this.sg1.cells(1,row)+"' and no_batch='"+this.sg1.cells(2,row)+"' "+
							 "union "+
							 "select tgl_ed from kli_beli_d where kode_obat='"+this.sg1.cells(1,row)+"' and no_batch='"+this.sg1.cells(2,row)+"' "+
							 ") x";
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){										
						this.sg1.cells(3,row,line.tgl_ed);												
					} 				
				}			
			}			
		}							
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_tukar,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan "+
		             "from kli_tukar_m a "+					 					 
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_tukar,line.tgl,line.no_dokumen,line.keterangan]); 
		}
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select no_dokumen,keterangan,kode_pp "+
							 "from kli_tukar_m "+							 
							 "where no_tukar = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){							
				...
				var strSQL = "select a.kode_obat,b.nama,a.satuan,a.jumlah,b.pabrik,c.stok+a.jumlah as stok "+
							 "from kli_tukar_d a inner join kli_obat b on a.kode_obat=b.kode_obat and a.kode_lokasi=b.kode_lokasi "+
							 ...
							 "where a.dc='C' and a.no_tukar='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";										
				...
				var strSQL = "select a.kode_obat,b.nama,a.satuan,a.jumlah,a.no_batch,convert(varchar,a.tgl_ed,103) as tgl_ed  "+
							 "from kli_tukar_d a inner join kli_obat b on a.kode_obat=b.kode_obat and a.kode_lokasi=b.kode_lokasi "+
							 "                   inner join kli_stok c on a.kode_obat=c.kode_obat and a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi and c.nik_user='"+this.nik_user+"' and a.no_batch=c.no_batch and a.tgl_ed=c.tgl_ed "+
							 "where a.dc='D' and a.no_tukar='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";										
				var data1 = this.dbLib.getDataProvider(strSQL,true);	
				if (typeof data1 == "object" && data1.rs.rows[0] != undefined){
					var line1;
					this.sg1.clear();
					for (var i in data1.rs.rows){
						line1 = data1.rs.rows[i];																													
						this.sg1.appendData([line1.nama,line1.kode_obat,line1.no_batch,line1.tgl_ed,line1.satuan,parseFloat(line1.jumlah)]);
					}
