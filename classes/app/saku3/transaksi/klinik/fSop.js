window.app_saku3_transaksi_klinik_fSop = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_klinik_fSop.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_klinik_fSop";
		...
		this.pc2 = new pageControl(this,{bound:[10,10,1000,430], childPage:["List Opname","Data Transaksi"]});				
		...
		this.bTampil = new portalui_button(this.pc2.childPage[1],{bound:[790,16,80,18],caption:"Load Data",click:[this,"doLoadData"]});		
		this.bUpload = new portalui_uploader(this.pc2.childPage[1],{bound:[890,16,80,18],param3:"object",param4:"gridupload",afterUpload:[this,"doAfterUpload"],autoSubmit:true});
		
		this.pc1 = new pageControl(this.pc2.childPage[1],{bound:[5,12,990,305], childPage:["Data Item Obat"]});		
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:8,tag:0,
		            colTitle:["Kode","Nama","No Batch","Exp Date","Satuan","Stok Sistem","Jml Fisik ","Selisih"],					
					colWidth:[[7,6,5,4,3,2,1,0],[80,80,80,50,100,100,350,80]],					
...
window.app_saku3_transaksi_klinik_fSop.extend(window.portalui_childForm);
window.app_saku3_transaksi_klinik_fSop.implement({
	    ...
	    try{   			
			...
			for (var i=0; i < this.dataUpload.rows.length;i++){
				...
				for (var j=0;j < this.dataJU.rs.rows.length;j++){
					if (line.kode == this.dataJU.rs.rows[j].kode_obat && line.no_batch == this.dataJU.rs.rows[j].no_batch && line.exp_date == this.dataJU.rs.rows[j].tgl_ed) {						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{																			
					...
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						...
						var tglED = line.tgl_ed.substr(6,4)+"-"+line.tgl_ed.substr(3,2)+"-"+line.tgl_ed.substr(0,2);
						sql.add("insert into kli_sop_d(no_sop,kode_lokasi,periode,nu,kode_obat,kode_pp,satuan,harga,stok,jumlah,selisih,total,no_batch,tgl_ed) values "+  
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"',"+i+",'"+line.kode_obat+"','"+this.cb_gudang.getText()+"','"+line.satuan+"',0,"+line.stok+","+line.jumlah+","+line.selisih+",0,'"+line.no_batch+"','"+tglED+"')");
					}
	...
	doLoadData:	function(sender){
		if (this.cb_gudang.getText()!="") {	
			...
			var strSQL = "select a.kode_obat,a.nama,b.no_batch,convert(varchar,b.tgl_ed,103) as tgl_ed,a.sat_kecil as satuan,isnull(b.stok,0) as stok, 0 as jumlah,(0-isnull(b.stok,0)) as selisih "+
			             "from kli_obat a "+
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData([line.kode_obat,line.nama,line.no_batch,line.tgl_ed,line.satuan,floatToNilai(line.stok),floatToNilai(line.jumlah),floatToNilai(line.selisih)]); 
		}
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.kode_obat,b.nama,c.no_batch,convert(varchar,c.tgl_ed,103) as tgl_ed,a.satuan,a.jumlah,b.pabrik,c.stok,a.jumlah-c.stok as selisih "+
