window.app_saku3_transaksi_klinik_fBeli = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_klinik_fBeli.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_klinik_fBeli";
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pembelian Obat: Input", 0);	
		...
		this.pc2 = new pageControl(this,{bound:[10,10,1000,430], childPage:["List Pembelian","Data Transaksi"]});				
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:10,tag:0,
		            colTitle:["Nama Barang","Kode","No Batch","Exp Date","Satuan","Harga","Jumlah","Bonus","Tot Diskon","SubTtl"],					
					colWidth:[[9,8,7,6,5,4,3,2,1,0],[80,80,60,60,70,50,100,100,80,260]],					
					columnReadOnly:[true,[0,1,4,5],[2,3,6,7,8,9]],					
...
window.app_saku3_transaksi_klinik_fBeli.extend(window.portalui_childForm);
window.app_saku3_transaksi_klinik_fBeli.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into kli_beli_m(no_beli,kode_lokasi,tanggal,due_date,no_dokumen,keterangan,kode_pp,periode,nik_user,tgl_input,nilai,nilai_ppn,nilai_diskon,nilai_mat,kode_vendor,no_kas,posted,modul,akun_hutang) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.dp_d2.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_dok.getText()+"','"+this.cb_gudang.getText()+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),"+nilaiToFloat(this.e_total.getText())+","+nilaiToFloat(this.e_ppn.getText())+","+nilaiToFloat(this.e_diskon.getText())+","+nilaiToFloat(this.e_mat.getText())+",'"+this.cb_vendor.getText()+"','-','F','NON','"+this.akunHutang+"')");
							
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){															
								var diskonBrg = nilaiToFloat(this.sg.cells(8,i)) / nilaiToFloat(this.sg.cells(6,i));
								sql.add("insert into kli_beli_d (no_beli,kode_lokasi,periode,nu,kode_obat,kode_pp,satuan,jumlah,bonus,harga,diskon,tot_diskon,total,no_batch,tgl_ed) values "+  
									   "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"',"+i+",'"+this.sg.cells(1,i)+"','"+this.cb_gudang.getText()+"','"+this.sg.cells(4,i)+"',"+nilaiToFloat(this.sg.cells(6,i))+","+nilaiToFloat(this.sg.cells(7,i))+","+nilaiToFloat(this.sg.cells(5,i))+","+diskonBrg+","+nilaiToFloat(this.sg.cells(8,i))+","+nilaiToFloat(this.sg.cells(9,i))+",'"+this.sg.cells(2,i)+"','"+this.sg.getCellDateValue(3,i)+"')");
							}
						}						
					}										
					sql.add("insert into kli_beli_j(no_beli,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.akunBarang+"','Pembelian No: "+this.e_dok.getText()+"','D',"+this.barang+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','BRGBELI','BRG','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");					
					sql.add("insert into kli_beli_j(no_beli,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',1,'"+this.akunPPN+"','Pembelian No: "+this.e_dok.getText()+"','D',"+nilaiToFloat(this.e_ppn.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','BRGBELI','PPN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");					
					sql.add("insert into kli_beli_j(no_beli,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',5,'"+this.akunMat+"','Pembelian No: "+this.e_dok.getText()+"','D',"+nilaiToFloat(this.e_mat.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','BRGBELI','BEAMAT','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");
							
					var totDiskon = nilaiToFloat(this.e_diskon.getText()) + this.diskon;
					sql.add("insert into kli_beli_j(no_beli,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',2,'"+this.akunDiskon+"','Pembelian No: "+this.e_dok.getText()+"','C',"+totDiskon+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','BRGBELI','DIS','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");
					sql.add("insert into kli_beli_j(no_beli,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',3,'"+this.akunDiskon+"','Pembelian No: "+this.e_dok.getText()+"','C',"+this.bonus+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','BRGBELI','BONUS','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");									
					sql.add("insert into kli_beli_j(no_beli,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',4,'"+this.akunHutang+"','Pembelian No: "+this.e_dok.getText()+"','C',"+nilaiToFloat(this.e_total.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','BRGBELI','HUT','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");
	...
	doChangeCell: function(sender, col, row){
		if (col == 0 && this.sg.cells(0,row)!="") {			
			if (this.sg.cells(0,row) != "") {							
				var strSQL = "select sat_kecil from kli_obat where nama ='"+this.sg.cells(0,row)+"' and kode_lokasi='"+this.app._lokasi+"'";
		...
		if (col == 1 && this.sg.cells(1,row)!="") {			
			var line,items = [];									
			var data = this.dbLib.getDataProvider(
				   "select no_batch from kli_sawal where kode_obat='"+this.sg.cells(1,row)+"' and kode_lokasi='"+this.app._lokasi+"' "+
				   "union "+
				   "select no_batch from kli_beli_d where kode_obat='"+this.sg.cells(1,row)+"' and kode_lokasi='"+this.app._lokasi+"'",true);				
			...
			this.sg.setPickList([2],[new arrayMap({items:items})]);
		}
		if (col == 2 && this.sg.cells(2,row)!="") {
			if (this.sg.cells(2,row) != "") {			
				var strSQL = "select top 1 convert(varchar,tgl_ed,103) as tgl_ed from ( "+
							 "select tgl_ed from kli_sawal where kode_obat='"+this.sg.cells(1,row)+"' and no_batch='"+this.sg.cells(2,row)+"' "+
							 "union "+
							 "select tgl_ed from kli_beli_d where kode_obat='"+this.sg.cells(1,row)+"' and no_batch='"+this.sg.cells(2,row)+"' "+
							 ") x";
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){										
						this.sg.cells(3,row,line.tgl_ed);												
					} 				
				}			
			}			
		}
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider("select b.kode_obat,b.nama,a.satuan,a.jumlah,a.bonus,a.harga,a.tot_diskon,a.total,a.no_batch,convert(varchar,a.tgl_ed,103) as tgl_ed "+
