window.app_saku3_transaksi_klinik_fJual = function(owner)
{
	if (owner)
	{
		...
		this.sg3 = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-30],colCount:4,tag:9,
		            colTitle:["No Bukti","Tanggal","Faktur Pajak","Deskripsi"],
		...
		this.cb_pic = new portalui_saiCBBL(this.pc2.childPage[1],{bound:[20,15,220,20],caption:"Marketing",multiSelection:false,tag:1});
		...
		this.cb_gudang = new portalui_saiCBBL(this.pc2.childPage[1],{bound:[20,16,220,20],caption:"Gudang",multiSelection:false,tag:1,change:[this,"doChange"]});
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:10,tag:0,
		            colTitle:["Kode","Nama","Pabrikasi","Satuan","Stok","Harga","Diskon","Jumlah","Bonus","SubTtl"],					
					colWidth:[[9,8,7,6,5,4,3,2,1,0],[80,60,60,60,60,60,50,200,230,80]],					
					columnReadOnly:[true,[1,2,3,4,5,9],[0,6,7,8]],
					colFormat:[[4,5,6,7,8,9],[cfNilai,cfNilai,cfNilai,cfNilai,cfNilai,cfNilai]],
					buttonStyle:[[0],[bsEllips]], 					
					dblClick:[this,"doDoubleClick"],ellipsClick:[this,"doEllipseClick"],change:[this,"doChangeCell"],nilaiChange:[this,"doNilaiChange"],
		...
		try{			
			...
			sql.add("select kode_obat, nama from kli_obat where kode_lokasi='"+this.app._lokasi+"'");
...
window.app_saku3_transaksi_klinik_fJual.implement({
		...
		try{					
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{																			
					...
					sql.add("insert into kli_jual_m(no_jual,kode_lokasi,tanggal,no_dokumen,kode_pic,keterangan,kode_pp,periode,nik_user,tgl_input,nilai,nilai_ppn,nilai_diskon,kode_cust,no_kas,posted,modul,akun_piutang) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.cb_pic.getText()+"','"+this.e_ket.getText()+"','"+this.cb_gudang.getText()+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),"+nilaiToFloat(this.e_total.getText())+","+nilaiToFloat(this.e_ppn.getText())+","+nilaiToFloat(this.e_diskon.getText())+",'"+this.cb_cust.getText()+"','-','F','UMUM','"+this.akunPiutang+"')");
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){																																										
								sql.add("insert into kli_jual_d(no_jual,kode_lokasi,periode,nu,kode_obat,kode_pp,satuan,jumlah,bonus,harga,hpp,diskon) values "+  
									   "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.cb_gudang.getText()+"','"+this.sg.cells(3,i)+"',"+nilaiToFloat(this.sg.cells(7,i))+","+nilaiToFloat(this.sg.cells(8,i))+","+nilaiToFloat(this.sg.cells(5,i))+",0,"+nilaiToFloat(this.sg.cells(6,i))+")");
							}
						}						
					}		
					this.stsPPN = 1;
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :			
				...
				for (var i=0;i < this.sg.getRowCount();i++){
					if (this.sg.rowValid(i)){
						var data = this.dbLib.getDataProvider("select stok from kli_stok where kode_obat='"+this.sg.cells(0,i)+"'  and kode_pp='"+this.cb_gudang.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and nik_user='"+this.nik_user+"'",true);
						if (typeof data == "object"){
				...
				if (this.flagMax == 1) {
					var strSQL = "select sum(nilai) as totjual from kli_jual_m "+							 
					...
					var strSQL = "select sum(nilai_kas+nilai_lain) as totbayar from kli_jualbayar_d "+							 
								 "where kode_cust='"+this.cb_cust.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and no_jual<>'"+this.e_nb.getText()+"' ";						 
					var data = this.dbLib.getDataProvider(strSQL,true);
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
					...
					if (saldoPiu > this.nilaiMax) {
						system.alert(this,"Transaksi tidak valid.","Saldo Piutang Tidak Tertagih melebihi Maksimal Piutang.");
						return false;
					}										
					var strSQL = "select top 1 a.no_jual,a.kode_lokasi "+
						 ...
						 "where datediff(day,a.tanggal,getdate()) >= "+this.hariMax+" and a.kode_cust='"+this.cb_cust.getText()+"' and (a.nilai-isnull(b.bayar,0)) > 0 and a.kode_lokasi='"+this.app._lokasi+"' ";										
					var data = this.dbLib.getDataProvider(strSQL,true);
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
						if (line != undefined){								
	...
	doDoubleClick: function(sender, col , row) {
		if (sender.cells(5,row) != "" && sender.cells(6,row) != "") {						
			sender.cells(6,row,Math.round(nilaiToFloat(sender.cells(6,row)) * nilaiToFloat(sender.cells(5,row)) / 100));
		}
	},
	doChangeCell: function(sender, col, row){
		if (col == 0 && this.sg.cells(0,row)!="" && this.cb_gudang.getText()!="") {
			if (this.sg.cells(0,row) != "") {
				...
				if (obat) sender.cells(1,row,obat);
					...
					if (trim(sender.cells(0,row)) != "") system.alert(this,"Kode Obat "+sender.cells(0,row)+" tidak ditemukan","Inputkan kode lainnya.","checkBrg");                
					sender.cells(0,row,"");
					sender.cells(1,row,"");
			...
			var strSQL = "select a.pabrik,a.sat_kecil,a.hna,b.stok "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){					
					...
					this.sg.cells(3,row,line.sat_kecil);	
		...
		if (col == 6 || col == 7) {
			if (this.sg.cells(5,row) != "" && this.sg.cells(6,row) != "" && this.sg.cells(7,row) != "") {
				this.sg.cells(9,row,Math.round((nilaiToFloat(this.sg.cells(5,row))-nilaiToFloat(this.sg.cells(6,row))) * nilaiToFloat(this.sg.cells(7,row))));
			}
		}	
		this.sg.validasi();
	},
	...
	doChange:function(sender){		
		if (sender == this.cb_cust && this.cb_cust.getText()!="") {
			this.cb_pic.setSQL("select a.kode_pic, a.nama from kli_pic a inner join kli_pic_cust b on a.kode_pic=b.kode_pic and a.kode_lokasi=b.kode_lokasi "+
			                   "where b.kode_cust='"+this.cb_cust.getText()+"' and b.kode_lokasi = '"+this.app._lokasi+"'",["a.kode_pic","a.nama"],false,["Kode","Nama"],"and","Data Acc Marketing",true);			
			...
			if (typeof data == "object"){
				...
				if (line != undefined){					
					...
					this.hariMax = parseFloat(line.max_hari);
				} 				
			}			
		}
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_jual,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan "+
		             "from kli_jual_m a "+					 					 
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select no_dokumen,keterangan,kode_cust,kode_pp,nilai_ppn,nilai_diskon,kode_pic "+
							 "from kli_jual_m "+							 
				...
				var strSQL = "select a.kode_obat,b.nama,a.satuan,a.jumlah,a.bonus,a.harga,a.diskon,b.pabrik,round(a.jumlah * (a.harga-a.diskon),0) as total,c.stok+a.jumlah+a.bonus as stok "+
				...
				if (typeof data1 == "object" && data1.rs.rows[0] != undefined){
					...
					for (var i in data1.rs.rows){
						...
						this.sg.appendData([line1.kode_obat,line1.nama,line1.pabrik,line1.satuan,parseFloat(line1.stok),parseFloat(line1.harga),parseFloat(line1.diskon),parseFloat(line1.jumlah),parseFloat(line1.bonus),parseFloat(line1.total)]);
					}
