window.app_saku3_transaksi_klinik_fBeli = function(owner)
{
	if (owner)
	{
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:0,
		            colTitle:["Kode","Nama","Pabrikasi","Satuan","Harga","Jumlah","Bonus","Tot Diskon","SubTtl"],					
...
window.app_saku3_transaksi_klinik_fBeli.implement({	
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){															
								...
								sql.add("insert into kli_beli_d (no_beli,kode_lokasi,periode,nu,kode_obat,kode_pp,satuan,jumlah,bonus,harga,diskon,tot_diskon,total) values "+  
									   "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.cb_gudang.getText()+"','"+this.sg.cells(3,i)+"',"+nilaiToFloat(this.sg.cells(5,i))+","+nilaiToFloat(this.sg.cells(6,i))+","+nilaiToFloat(this.sg.cells(4,i))+","+diskonBrg+","+nilaiToFloat(this.sg.cells(7,i))+","+nilaiToFloat(this.sg.cells(8,i))+")");
							}
						}						
					}
	...
	doChangeCell: function(sender, col, row){
		if (col == 0 && this.sg.cells(0,row)!="") {
			...
			var strSQL = "select pabrik,sat_kecil from kli_obat where kode_obat='"+this.sg.cells(0,row)+"' and kode_lokasi='"+this.app._lokasi+"'";
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider("select b.kode_obat,b.nama,b.pabrik,a.satuan,a.jumlah,a.bonus,a.harga,a.tot_diskon,a.total "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];												
						this.sg.appendData([line.kode_obat,line.nama,line.pabrik,line.satuan,floatToNilai(line.harga),floatToNilai(line.jumlah),floatToNilai(line.bonus),floatToNilai(line.tot_diskon),floatToNilai(line.total)]);
					}
