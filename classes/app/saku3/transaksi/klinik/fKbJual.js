window.app_saku3_transaksi_klinik_fKbJual = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_klinik_fKbJual.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_klinik_fKbJual";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Pembayaran Penjualan: Input/Edit", 0);	
		...
		this.sg3 = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-30],colCount:6,tag:9,
		            colTitle:["No KasBank","Tanggal","No Dokumen","Deskripsi","Customer","Akun KasBank"],
		...
		this.cb_cust = new saiCBBL(this.pc2.childPage[1],{bound:[20,19,220,20],caption:"Customer", multiSelection:false, maxLength:10, tag:0,change:[this,"doChange"]});					
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,215],colCount:7,tag:0,
				colTitle:["No Bukti","Tanggal","Keterangan","Akun AR","Saldo Tagihan","Nilai Bayar","Sisa"],
		...
		try {
			...
			this.cb_cust.setSQL("select kode_cust, nama from kli_cust where kode_lokasi='"+this.app._lokasi+"'",["kode_cust","nama"],false,["Kode","Nama"],"and","Data Cust",true);			
...
window.app_saku3_transaksi_klinik_fKbJual.extend(window.childForm);
window.app_saku3_transaksi_klinik_fKbJual.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBJUAL','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_total.getText())+",'"+this.app._userLog+"','"+this.cb_pic.getText()+"',getdate(),'"+this.app._userLog+"','F','-','"+this.cb_cust.getText()+"','-','-')");					
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_total.getText())+",'"+this.app._kodePP+"','-','-','"+this.cb_cust.getText()+"','"+this.app._lokasi+"','KBJUAL','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.e_total.getText())+")");
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && this.sg1.cells(5,i) != "0"){							
							var nilaiPiu = nilaiToFloat(this.sg1.cells(5,i));																			
							sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
									"('"+this.e_nb.getText()+"','"+this.sg1.cells(0,i)+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(3,i)+"','Pembayaran Piu. "+this.sg1.cells(0,i)+"','C',"+nilaiPiu+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBJUAL','AR','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiPiu+")");														
							sql.add("insert into kli_jualbayar_d(no_bukti,kode_lokasi,no_jual,nu,akun_piutang,nilai_kas,nilai_lain,periode,dc,modul,kode_cust) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.sg1.cells(0,i)+"',"+i+",'"+this.sg1.cells(3,i)+"',"+nilaiToFloat(this.sg1.cells(5,i))+",0,'"+this.e_periode.getText()+"','D','KBJUAL','"+this.cb_cust.getText()+"')");							
						}
					}																																							
					setTipeButton(tbAllFalse);					
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_cust && this.cb_cust.getText()!="" && this.stsSimpan==1) {												
			var strSQL = "select a.no_jual,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.akun_piutang,a.nilai-isnull(b.bayar,0) as saldo,a.tanggal  "+						  
						 "from kli_jual_m a "+
						 "     left join ("+
						 ...
						 "where a.kode_cust='"+this.cb_cust.getText()+"' and (a.nilai-isnull(b.bayar,0)) > 0 and a.kode_lokasi='"+this.app._lokasi+"' order by a.tanggal";						 
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg1.appendData([line.no_jual,line.tgl,line.keterangan,line.akun_piutang,floatToNilai(line.saldo),"0",floatToNilai(line.saldo)]);
				}
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.no_link+' - '+b.nama as cust,a.akun_kb+' - '+c.nama as akun,a.tanggal "+
		             "from kas_m a "+
					 "inner join kli_cust b on a.no_link=b.kode_cust and a.kode_lokasi=b.kode_lokasi "+
					 "inner join masakun c on a.akun_kb=c.kode_akun and a.kode_lokasi=c.kode_lokasi "+					 
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'KBJUAL' and a.posted ='F' "+
					 "order by a.tanggal";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						this.cb_cust.setSQL("select kode_cust, nama from kli_cust where kode_cust='"+line.no_link+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_cust","nama"],false,["Kode","Nama"],"and","Data Cust",true);									
						this.cb_cust.setText(line.no_link);										
					}
				}
								
				var strSQL = "select a.no_jual,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.akun_piutang,a.nilai-isnull(b.bayar,0) as saldo,a.tanggal,c.nilai_kas,a.nilai-isnull(b.bayar,0)-c.nilai_kas as sisa  "+						  
						 "from kli_jual_m a "+
						 "     inner join kli_jualbayar_d c on a.no_jual=c.no_jual and a.kode_lokasi=c.kode_lokasi "+
						 "     left join ("+
						 "          select no_jual,kode_lokasi,sum(case dc when 'D' then (nilai_kas+nilai_lain) else -(nilai_kas+nilai_lain) end) as bayar  "+
						 "          from kli_jualbayar_d where no_bukti <> '"+this.e_nb.getText()+"' and kode_cust='"+this.cb_cust.getText()+"' and kode_lokasi='"+this.app._lokasi+"' group by no_jual,kode_lokasi "+
						 ") b on a.no_jual=b.no_jual and a.kode_lokasi=b.kode_lokasi "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData([line.no_jual,line.tgl,line.keterangan,line.akun_piutang,floatToNilai(line.saldo),floatToNilai(line.nilai_kas),floatToNilai(line.sisa)]);
					}
				} else this.sg1.clear(1);								
			}						
		} catch(e) {alert(e);}		
