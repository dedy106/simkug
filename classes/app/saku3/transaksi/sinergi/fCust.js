window.app_saku3_transaksi_sinergi_fCust = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sinergi_fCust.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sinergi_fCust";
...
window.app_saku3_transaksi_sinergi_fCust.extend(window.childForm);
window.app_saku3_transaksi_sinergi_fCust.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into si_cust(kode_cust,kode_lokasi,nama,alamat,no_tel,email,npwp,pic,alamat2) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update si_cust set nama='"+this.e_nama.getText()+"',alamat='"+this.e_alamat.getText()+"',no_tel='"+this.e_tel.getText()+"',email='"+this.e_mail.getText()+"',npwp='"+this.e_npwp.getText()+"',pic='"+this.e_pic.getText()+"',alamat2='"+this.e_alamat2.getText()+"' "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from si_cust where kode_cust = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");			
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
	...
	doChange: function(sender){
		try{								
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){				
				var strSQL = "select kode_cust,nama,alamat,no_tel,email,npwp,alamat2,pic "+
				             "from si_cust "+
						     "where kode_cust ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";						   				
	...
	doCari:function(sender){								
		try {
			...
			var strSQL = "select kode_cust,nama,alamat,kode_lokasi "+
						 "from si_cust where kode_lokasi='"+this.app._lokasi+"' and "+filter+" order by kode_cust";								
			var data = this.dbLib.getDataProvider(strSQL,true);		
	...
	doLoad:function(sender){								
		try {
			var strSQL = "select kode_cust,nama,alamat "+
						 "from si_cust where kode_lokasi='"+this.app._lokasi+"' order by kode_cust";				
			var data = this.dbLib.getDataProvider(strSQL,true);		
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU = data;
				this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length/20));
