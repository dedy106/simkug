window.app_saku3_transaksi_ppbs_fPaPp = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ppbs_fPaPp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ppbs_fPaPp";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data PP : Input/Edit", 0);	
		
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		uses("saiGrid",true);		
		this.pc1 = new pageControl(this,{bound:[20,12,1000,450], childPage:["Daftar PP","Data PP","Filter Cari","Upload Data"]});				
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:5,tag:9,
		            colTitle:["Kode","Nama","Tahun","Kode Bidang","Singkatan"],
		...
		this.e_skode = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,12,600,20],caption:"Singkatan", maxLength:150, tag:1});	
		this.cb_bidang = new portalui_saiCBBL(this.pc1.childPage[1],{bound:[20,18,202,20],caption:"Kode Bidang",tag:2,multiSelection:false});
		...
		this.c_tahun3 = new saiCB(this.pc1.childPage[3],{bound:[20,13,180,20],caption:"Tahun",readOnly:true,tag:2});
		...
		this.sg = new saiGrid(this.pc1.childPage[3],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:5,tag:9,
		            colTitle:["Kode PP","Nama","Tahun","Bidang","Singkatan"],
					colWidth:[[3,2,1,0],[50,80,300,100]],
					readOnly:true,
		...
		try {
			...
			this.cb_bidang.setSQL("select kode_bidang, nama from agg_bidang where kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.tahun+"'",["kode_bidang","nama"],false,["Kode","Nama"],"and","Data Bidang",true);
			this.doLoad();
		}catch(e){
			systemAPI.alert(e);
		}
...
window.app_saku3_transaksi_ppbs_fPaPp.extend(window.childForm);
window.app_saku3_transaksi_ppbs_fPaPp.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into agg_pp(kode_pp,nama,kode_lokasi,flag_aktif,tahun,kode_bidang,skode) values ('"+
							this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.c_status.getText().substr(0,1)+"','"+this.c_tahun.getText()+"','"+this.cb_bidang.getText()+"','"+this.e_skode.getText()+"')");
					setTipeButton(tbAllFalse);
	...
	simpan2: function(){			
		try{						
			...
			if (this.sg.getRowValidCount() > 0){
				...
				{
					sql.add("delete from agg_pp where kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun3.getText()+"'");
				}
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from agg_pp where kode_pp = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"'");
					sql.add("insert into agg_pp(kode_pp,nama,kode_lokasi,flag_aktif,tahun,kode_bidang,skode) values ('"+
							this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.c_status.getText().substr(0,1)+"','"+this.c_tahun.getText()+"','"+this.cb_bidang.getText()+"','"+this.e_skode.getText()+"')");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select kode_pp,nama,tahun,kode_bidang from agg_pp where tahun='"+this.c_tahun.getText()+"' and kode_pp ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {			
				...
				this.cb_bidang.setText(this.sg1.cells(3,row));	
				this.e_skode.setText(this.sg1.cells(4,row));					
			}
		} catch(e) {alert(e);}
	},
	doLoad:function(sender){						
		var strSQL = "select kode_pp,nama,tahun,kode_bidang,skode "+
		             "from agg_pp where kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"' and flag_aktif='1' order by kode_pp ";		
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_pp,line.nama,line.tahun,line.kode_bidang,line.skode]); 
		}
