window.app_saku3_transaksi_ppbs_fPaAkunPp = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ppbs_fPaAkunPp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ppbs_fPaAkunPp";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data PP Akun: Input/Edit", 0);	
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:4,tag:9,
		            colTitle:["Kode","Nama","Tahun","Kode Bidang"],
		...
		this.cb_bidang = new portalui_saiCBBL(this.pc1.childPage[1],{bound:[20,18,202,20],caption:"Kode Bidang",tag:2,readOnly:true,multiSelection:false});
		this.sg2 = new saiGrid(this.pc1.childPage[1],{bound:[20,20,550,280],colCount:2,tag:0,
		            colTitle:["Kode Akun","Nama Akun"],
					colWidth:[[1,0],[400,80]],
		...
		this.sg = new saiGrid(this.pc1.childPage[3],{bound:[1,5,this.pc1.width-5,this.pc1.height-65],colCount:5,tag:9,
		            colTitle:["Kode Akun","Nama Akun","Kode PP","Nama PP","Tahun"],
		...
		try {
			...
			this.cb_bidang.setSQL("select kode_bidang, nama from agg_bidang where kode_lokasi='"+this.app._lokasi+"' ",["kode_bidang","nama"],false,["Kode","Nama"],"and","Data Bidang",true);
			this.doLoad();
		}catch(e){
			systemAPI.alert(e);
		}
...
window.app_saku3_transaksi_ppbs_fPaAkunPp.extend(window.childForm);
window.app_saku3_transaksi_ppbs_fPaAkunPp.implement({
	...
	simpan2: function(){			
		try{						
			...
			if (this.sg.getRowValidCount() > 0){
				sql.add("delete from agg_akun_pp where kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"'");
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					...
					if (this.sg2.getRowValidCount() > 0){
						sql.add("delete from agg_akun_pp where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.cb_kode.getText()+"' and tahun='"+this.c_tahun.getText()+"'");
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from agg_akun_pp where kode_pp = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");			
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
		...
		try{
			...
			if (this.cb_kode.getText() != ""){
				var strSQL = "select a.kode_akun,b.nama "+
				            "from  agg_akun_pp a inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
							"where a.kode_pp='"+this.cb_kode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.tahun='"+this.c_tahun.getText()+"' order by a.kode_akun ";						   
	...
	doLoad:function(sender){						
		var strSQL = "select kode_pp,nama,tahun,kode_bidang "+
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_pp,line.nama,line.tahun,line.kode_bidang]); 
		}
