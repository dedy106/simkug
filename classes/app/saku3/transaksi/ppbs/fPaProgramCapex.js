window.app_saku3_transaksi_ppbs_fPaProgramCapex = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ppbs_fPaProgramCapex.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ppbs_fPaProgramCapex";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Program : Input/Edit", 0);	
		
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		uses("saiGrid",true);		
		this.pc1 = new pageControl(this,{bound:[20,12,1000,450], childPage:["Daftar Program","Data Program","Filter Cari","Upload Data"]});				
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:4,tag:9,
		            colTitle:["Kode","Nama","Tahun","PP"],
		...
		this.sg = new saiGrid(this.pc1.childPage[3],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:4,tag:9,
		            colTitle:["Kode Program","Nama","Tahun","PP"],
					colWidth:[[3,2,1,0],[80,80,300,100]],
					readOnly:true,
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select distinct tahun from agg_program order by tahun desc",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
...
window.app_saku3_transaksi_ppbs_fPaProgramCapex.extend(window.childForm);
window.app_saku3_transaksi_ppbs_fPaProgramCapex.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into agg_program(kode_program,nama,kode_lokasi,flag_aktif,tahun,kode_pp,jenis) values ('"+
							this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.c_status.getText().substr(0,1)+"','"+this.c_tahun.getText()+"','"+this.cb_pp.getText()+"','CAPEX')");
					setTipeButton(tbAllFalse);
	...
	simpan2: function(){			
		try{						
			if (this.sg.getRowValidCount() > 0){
				...
				{
					sql.add("delete from agg_program where jenis='CAPEX' and kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"'");
				}
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from agg_program where jenis='CAPEX' and kode_program = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"'");
					sql.add("insert into agg_program(kode_program,nama,kode_lokasi,flag_aktif,tahun,kode_pp,jenis) values ('"+
							this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.c_status.getText().substr(0,1)+"','"+this.c_tahun.getText()+"','"+this.cb_pp.getText()+"','CAPEX')");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
	...
	doChange: function(sender){
		try{
			...
			if (this.cb_kode.getText() != ""){
				var strSQL = "select kode_program,nama,tahun,kode_pp from agg_program where jenis='CAPEX' and kode_program ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"'";						   
	...
	doBtnClick: function(sender, event){
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Program",sender,undefined, 
											  "select kode_program, nama  from agg_program where jenis='CAPEX' and kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"'",
											  "select count(kode_program) from agg_program where jenis='CAPEX' and kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"'",
	...
	doLoad:function(sender){						
		var strSQL = "select kode_program,nama,tahun,kode_pp "+
		             "from agg_program where jenis='CAPEX' and kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"' order by kode_program ";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
	...
	doCari:function(sender){								
		try {
			var filter = "";
			if (this.e_kode2.getText() != "") var filter = filter+" and kode_program = '"+this.e_kode2.getText()+"' ";
			...
			var strSQL = "select kode_program,nama,tahun,kode_pp "+
		             "from agg_program where jenis='CAPEX' and kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"' "+filter+" order by kode_program ";		
			var data = this.dbLib.getDataProvider(strSQL,true);		
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU = data;
				this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length/20));
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_program,line.nama,line.tahun,line.kode_pp]); 
		}
