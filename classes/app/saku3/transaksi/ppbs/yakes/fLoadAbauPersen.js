window.app_saku3_transaksi_ppbs_yakes_fLoadAbauPersen = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ppbs_yakes_fLoadAbauPersen.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_ppbs_yakes_fLoadAbauPersen";
		...
		this.pc1 = new pageControl(this,{bound:[20,20,990,380], childPage:["Data Angg. Biaya Adm dan Umum","Pesan Kesalahan","Eval Outlook"]});		
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:12,tag:0,
		...
		this.sg2 = new portalui_saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:9,
...
window.app_saku3_transaksi_ppbs_yakes_fLoadAbauPersen.extend(window.portalui_childForm);
window.app_saku3_transaksi_ppbs_yakes_fLoadAbauPersen.implement({	
	...
	selectPage: function(sender,page){		
		if (this.stsTampil == "LOAD") {
			...
			for (var i=start; i < finish;i++){
				...
				this.sg1.appendData([line.kode_drk,line.nama_drk,line.kegiatan,line.kode_akun,line.kode_pp,line.kode_var,floatToNilai(line.tarif),floatToNilai(line.jumlah),line.volume,line.bulan,"0",line.status]);
			}
	...
	doHitung:function(sender){		
		try {
			...
			strSQL = "select a.kode_var+'-'+a.kode_akun as kunci "+
					 "from agg_abau_tmp a "+
					 "where a.tarif=0 and a.nik_user = '"+this.nik_user+"'";
			var data = this.dbLib.getDataProvider(strSQL,true);	
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;				
				for (var i in data.rs.rows){
					line = data.rs.rows[i];																													
					temu = true;
					msg+= "Tarif nol, Variabel+Akun tidak terdaftar. [kode : "+line.kunci+"]\n";
				}
			}
