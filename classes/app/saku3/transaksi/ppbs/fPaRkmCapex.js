window.app_saku3_transaksi_ppbs_fPaRkmCapex = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ppbs_fPaRkmCapex.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ppbs_fPaRkmCapex";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data RKM : Input/Edit", 0);	
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:5,tag:9,
		            colTitle:["Kode","Nama","Tahun","Program","PP"],
		...
		this.cb_program = new portalui_saiCBBL(this.pc1.childPage[1],{bound:[20,18,202,20],caption:"Kode Program",tag:2,multiSelection:false});
		...
		this.sg = new saiGrid(this.pc1.childPage[3],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:5,tag:9,
		            colTitle:["Kode RKM","Nama","Tahun","Program","Kode PP"],
		...
		try {
			...
			this.cb_program.setSQL("select kode_program, nama from agg_program where jenis='CAPEX' and kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"'",["kode_program","nama"],false,["Kode","Nama"],"and","Data Program",true);			
...
window.app_saku3_transaksi_ppbs_fPaRkmCapex.extend(window.childForm);
window.app_saku3_transaksi_ppbs_fPaRkmCapex.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into agg_rkm(kode_rkm,nama,kode_lokasi,flag_aktif,tahun,kode_program,kode_pp,jenis) values ('"+
							this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.c_status.getText().substr(0,1)+"','"+this.c_tahun.getText()+"','"+this.cb_program.getText()+"','"+this.cb_pp.getText()+"','CAPEX')");
					setTipeButton(tbAllFalse);
	...
	simpan2: function(){			
		try{						
			if (this.sg.getRowValidCount() > 0){
				...
				{
					sql.add("delete from agg_rkm where jenis='CAPEX' and kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"'");
				}
				for (var i=0;i < this.sg.getRowCount();i++){
					if (this.sg.rowValid(i)){
						sql.add("insert into agg_rkm(kode_rkm,nama,kode_lokasi,flag_aktif,tahun,kode_program,kode_pp,jenis) values ('"+
							this.sg.cells(0,i)+"','"+this.sg.cells(1,i)+"','"+this.app._lokasi+"','1','"+this.sg.cells(2,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(4,i)+"','CAPEX')");
					}
				}
				setTipeButton(tbAllFalse);
				this.dbLib.execArraySQL(sql);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from agg_rkm where jenis='CAPEX' and kode_rkm = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"'");
					sql.add("insert into agg_rkm(kode_rkm,nama,kode_lokasi,flag_aktif,tahun,kode_program,kode_pp,jenis) values ('"+
							this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.c_status.getText().substr(0,1)+"','"+this.c_tahun.getText()+"','"+this.cb_program.getText()+"','"+this.cb_pp.getText()+"','CAPEX')");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from agg_rkm where jenis='CAPEX' and kode_rkm = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");			
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
	...
	doChange: function(sender){
		try{
			...
			if (this.cb_kode.getText() != ""){
				var strSQL = "select kode_rkm,nama,tahun,kode_program,kode_pp from agg_rkm where jenis='CAPEX' and kode_rkm ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"'";						   
		...
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Flag Akun",sender,undefined, 
											  "select kode_rkm, nama  from agg_rkm where jenis='CAPEX' and kode_lokasi='"+this.app._lokasi+"'",
											  "select count(kode_rkm) from agg_rkm where jenis='CAPEX' and kode_lokasi='"+this.app._lokasi+"'",
	...
	doLoad:function(sender){						
		var strSQL = "select kode_rkm,nama,tahun,kode_program,kode_pp "+
		             "from agg_rkm where jenis='CAPEX' and kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"' order by kode_rkm ";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
	...
	doCari:function(sender){								
		try {
			...
			var strSQL = "select kode_rkm,nama,tahun,kode_program,kode_pp "+
		             "from agg_rkm where jenis='CAPEX' and kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun2.getText()+"' "+filter+" order by kode_rkm ";			
			var data = this.dbLib.getDataProvider(strSQL,true);		
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU = data;
				this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length/20));
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_rkm,line.nama,line.tahun,line.kode_program,line.kode_pp]); 
		}
