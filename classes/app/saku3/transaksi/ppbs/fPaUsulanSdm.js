window.app_saku3_transaksi_ppbs_fPaUsulanSdm = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ppbs_fPaUsulanSdm.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ppbs_fPaUsulanSdm";
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,600], childPage:["Daftar Usulan","Data Usulan","Filter Cari"]});				
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:7,tag:9,
					...
					colWidth:[[6,5,4,3,2,1,0],[200,60,60,60,250,60,100]],readOnly:true,
		...
		this.p1 = new panel(this.pc1.childPage[1],{bound:[20,23,900,350],caption:"Daftar Usulan SDM"});							
		this.sg = new saiGrid(this.p1,{bound:[0,20,this.p1.width-5,this.p1.height-45],colCount:14,tag:0,
		            colTitle:["NIK","Nama","Gaji Dasar","Tunjangan Dasar","Tunjangan Posisi/Profesi","Tunjangan Hari Tua","Tunjangan Cuti","Tunjangan PPh Pasal 21" 
							  ,"Pakaian Seragam","Insentif Pendidikan","THR Keagamaan","Fasilitas Makan","Jamsostek","SHU"],
...
window.app_saku3_transaksi_ppbs_fPaUsulanSdm.extend(window.childForm);
window.app_saku3_transaksi_ppbs_fPaUsulanSdm.implement({
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into agg_gaji_load(  no_gaji, kode_lokasi, nik, nama, n1, n2, n3, n4, n5, n6, n7, n8, n9, n10,n11,n12,nu) values "+
										...
										","+nilaiToFloat(this.sg.cells(6,i))+","+nilaiToFloat(this.sg.cells(7,i))+","+nilaiToFloat(this.sg.cells(8,i))+","+nilaiToFloat(this.sg.cells(9,i))+","+nilaiToFloat(this.sg.cells(10,i))+","+nilaiToFloat(this.sg.cells(11,i))+","+nilaiToFloat(this.sg.cells(12,i))+","+nilaiToFloat(this.sg.cells(13,i))+","+i+")");
																
							}
						}
					}
									
					sql.add("exec sp_agg_gaji_d '"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.c_tahun.getText()+"'");
					setTipeButton(tbAllFalse);
			...
			switch(col){
				case 0 :
						this.standarLib.showListDataForSG(this, "Daftar DRK",this.sg, this.sg.row, this.sg.col, 
														"select kode_drk, nama  from agg_drk where kode_lokasi='"+this.app._lokasi+"' and kode_rkm='"+this.cb_rkm.getText()+"' and tahun='"+this.c_tahun.getText()+"'",
														"select count(kode_drk) from agg_drk where kode_lokasi='"+this.app._lokasi+"' and kode_rkm='"+this.cb_rkm.getText()+"' and tahun='"+this.c_tahun.getText()+"'",
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {			
				...
				var strSQL = " select no_gaji, kode_lokasi, nik, nama, n1, n2, n3, n4, n5, n6, n7, n8, n9, n10, n11,n12  from agg_gaji_load "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg.appendData([line.nik,line.nama,floatToNilai(line.n1),floatToNilai(line.n2),floatToNilai(line.n3),floatToNilai(line.n4),floatToNilai(line.n5)
	...
	doChangeCell: function(sender, col, row){
		...
		if (col ==  2 || col ==  3  || col ==  4 || col == 5  || col == 6 || col == 7 || col == 8 || col == 9 || col == 10 || col == 11 || col == 12) {			
			...
			if (sender.cells(2,row) != "" && sender.cells(3,row) != "" && sender.cells(4,row) != "" && sender.cells(5,row) != "" && 
			   sender.cells(6,row) != "" && sender.cells(7,row) != "" && sender.cells(8,row) != "" && sender.cells(9,row) != "" && sender.cells(10,row) != "" ) {
				var total = nilaiToFloat(sender.cells(2,row)) + nilaiToFloat(sender.cells(3,row)) + nilaiToFloat(sender.cells(4,row)) + nilaiToFloat(sender.cells(5,row)) + 
							...
							nilaiToFloat(sender.cells(10,row)) +nilaiToFloat(sender.cells(11,row))+nilaiToFloat(sender.cells(12,row)) +nilaiToFloat(sender.cells(13,row)) ;
