window.app_saku3_transaksi_ppbs_fPaEvalAkunPPDRK = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ppbs_fPaEvalAkunPPDRK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ppbs_fPaEvalAkunPPDRK";
		...
		this.cb_akun = new saiCBBL(this,{bound:[20,19,200,20],caption:"Akun Anggaran", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});		
		this.cb_drk = new saiCBBL(this,{bound:[20,18,200,20],caption:"DRK", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});		
...
window.app_saku3_transaksi_ppbs_fPaEvalAkunPPDRK.extend(window.childForm);
window.app_saku3_transaksi_ppbs_fPaEvalAkunPPDRK.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("update agg_d set vol=vol+((vol * "+nilaiToFloat(this.e_persen.getText())+")/100), "+
							...
							"where kode_pp='"+this.cb_pp.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and periode like '"+this.c_tahun.getText()+"%' and kode_akun='"+this.cb_akun.getText()+"' and modul='UMUM' and kode_drk='"+this.cb_drk.getText()+"' ");						 
	...
	doChange:function(sender){		
		...
		if ((sender == this.cb_akun || sender == this.cb_pp) && this.cb_akun.getText()!="" && this.cb_pp.getText()!="") {
			this.cb_drk.setSQL("select distinct a.kode_drk, a.nama "+
			                   "from agg_drk a inner join agg_d b on a.kode_drk=b.kode_drk and a.kode_lokasi=b.kode_lokasi "+
			                   "where b.kode_akun='"+this.cb_akun.getText()+"' and b.kode_pp='"+this.cb_pp.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",["a.kode_drk","a.nama"],false,["Kode","Nama"],"and","Data DRK",true);
		}
		
		if ((sender == this.cb_akun || sender == this.cb_pp || sender == this.cb_drk) && this.cb_akun.getText()!="" && this.cb_pp.getText()!="" && this.cb_pp.getText()!="") {
			var strSQL = "select a.kode_drk,a.kode_norma,a.kode_akun,a.keterangan,a.satuan,substring(a.periode,5,2) as bulan,a.tarif,a.jumlah,a.vol,a.total, "+
						 ...
						 "where a.kode_pp='"+this.cb_pp.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.periode like '"+this.c_tahun.getText()+"%' and a.kode_akun='"+this.cb_akun.getText()+"' and a.kode_drk ='"+this.cb_drk.getText()+"'";			
