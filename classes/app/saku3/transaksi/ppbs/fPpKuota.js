window.app_saku3_transaksi_ppbs_fPpKuota = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ppbs_fPpKuota.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ppbs_fPpKuota";
...
window.app_saku3_transaksi_ppbs_fPpKuota.extend(window.childForm);
window.app_saku3_transaksi_ppbs_fPpKuota.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into agg_outlook(kode_pp,kode_akun,kode_lokasi,tahun,n_max) values ('"+this.cb_pp.getText()+"','"+this.cb_akun.getText()+"','"+this.app._lokasi+"','"+this.tahun+"',"+nilaiToFloat(this.e_nilai.getText())+")");
					setTipeButton(tbAllFalse);
	...
	simpan2: function(){			
		try{						
			if (this.sg.getRowValidCount() > 0){
				uses("server_util_arrayList");
				var sql = new server_util_arrayList();
				sql.add("delete from agg_outlook where kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.tahun+"'");
				for (var i=0;i < this.sg.getRowCount();i++){
					if (this.sg.rowValid(i)){
						sql.add("insert into agg_outlook(kode_pp,kode_akun,kode_lokasi,tahun,n_max) values ('"+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from agg_outlook where kode_akun = '"+this.cb_akun.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.cb_pp.getText()+"' and tahun='"+this.tahun+"' ");
					sql.add("insert into agg_outlook(kode_pp,kode_akun,kode_lokasi,tahun,n_max) values ('"+this.cb_pp.getText()+"','"+this.cb_akun.getText()+"','"+this.app._lokasi+"','"+this.tahun+"',"+nilaiToFloat(this.e_nilai.getText())+")");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
					this.doLoad();
	...
	doChange: function(sender){
		try{
			if (this.cb_pp.getText() != "" || this.cb_akun.getText() != ""){
				var strSQL = "select n_max from agg_outlook where kode_pp ='"+this.cb_pp.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and kode_akun='"+this.cb_akun.getText()+"' and tahun='"+this.tahun+"'";						   
				var data = this.dbLib.getDataProvider(strSQL,true);
		...
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Flag Akun",sender,undefined, 
											  "select kode_rektor, nama  from agg_rektor where kode_lokasi='"+this.app._lokasi+"'",
											  "select count(kode_rektor) from agg_rektor where kode_lokasi='"+this.app._lokasi+"'",
	...
	doLoad:function(sender){						
		var strSQL = "select a.kode_akun,a.kode_pp,b.nama as nama_akun,c.nama as nama_pp,a.n_max,a.tahun "+
					...
					"inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
					"inner join agg_pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi and a.tahun=c.tahun "+
		            "where a.kode_lokasi='"+this.app._lokasi+"' and a.kode_pp='"+this.pp+"' and a.tahun='"+this.tahun+"' order by a.kode_akun,a.kode_pp ";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
			this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length/20));
