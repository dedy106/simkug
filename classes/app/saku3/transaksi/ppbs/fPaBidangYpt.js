window.app_saku3_transaksi_ppbs_fPaBidangYpt = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ppbs_fPaBidangYpt.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ppbs_fPaBidangYpt";
		...
		this.sg = new saiGrid(this.pc1.childPage[3],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:3,tag:9,
		            colTitle:["Kode Bidang","Nama","Tahun"],
					colWidth:[[2,1,0],[80,300,100]],
					readOnly:true,
...
window.app_saku3_transaksi_ppbs_fPaBidangYpt.extend(window.childForm);
window.app_saku3_transaksi_ppbs_fPaBidangYpt.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into agg_bidang(kode_bidang,nama,kode_lokasi,flag_aktif,tahun,kode_rektor) values ('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.c_status.getText().substr(0,1)+"','"+this.c_tahun.getText()+"','-')");
					setTipeButton(tbAllFalse);
	...
	doLoad:function(sender){						
		var strSQL = "select kode_bidang,nama,tahun "+
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_bidang,line.nama,line.tahun]); 
		}
