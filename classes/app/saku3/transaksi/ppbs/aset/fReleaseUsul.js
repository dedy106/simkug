window.app_saku3_transaksi_ppbs_aset_fReleaseUsul = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ppbs_aset_fReleaseUsul.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ppbs_aset_fReleaseUsul";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Release Usulan", 0);	
		...
		this.e_catat = new saiLabelEdit(this,{bound:[20,13,450,20],caption:"Catatan", maxLength:200,tag:1});		
		
		this.pc1 = new pageControl(this,{bound:[20,12,1000,390], childPage:["Daftar Usulan","Ref Usulan","Detail Usulan","Cttn. Memo"]});				
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:9,
		             colTitle:["Status","No Bukti","Tanggal","Kode PP","Nama PP","Kode Akun","Nama Akun","Keterangan","Nilai"],
					 colWidth:[[8,7,6,5,4,3,2,1,0],[100,200,200,80,150,60,60,100,80]],readOnly:true,
					 buttonStyle:[[0],[bsAuto]], picklist:[[0],[new portalui_arrayMap({items:["RELEASE","IDLE"]})]],checkItem:true,
					 colFormat:[[8],[cfNilai]],
					 dblClick:[this,"doDoubleClick"],autoAppend:false,defaultRow:1});			
		this.sgn1 = new portalui_sgNavigator(this.pc1.childPage[0],{bound:[1,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:3,grid:this.sg1,pager:[this,"doPager"]});
		
		this.cb_rkm = new saiCBBL(this.pc1.childPage[1],{bound:[20,16,220,20],caption:"Kode RKM", readOnly:true, tag:9, change:[this,"doChange"]});
		this.cb_drk = new saiCBBL(this.pc1.childPage[1],{bound:[20,15,220,20],caption:"Kode DRK", readOnly:true, maxLength:10, tag:9});				
		this.c_tahun = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,22,200,20],caption:"Tahun",readOnly:true,tag:9});
		this.e_nobukti = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,14,200,20],caption:"No Usulan", readOnly:true,tag:9});		
		this.e_ket = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,13,450,20],caption:"Deskripsi", readOnly:true,tag:9});		
		this.cb_app = new saiCBBL(this.pc1.childPage[1],{bound:[20,17,220,20],caption:"NIK Approve", readOnly:true, tag:9});
		this.cb_pp = new saiCBBL(this.pc1.childPage[1],{bound:[20,16,220,20],caption:"Kode PP", readOnly:true,tag:9});
		this.cb_akun = new saiCBBL(this.pc1.childPage[1],{bound:[20,17,220,20],caption:"Kode MTA", readOnly:true, tag:9});
		this.cb_peruntukan = new saiCBBL(this.pc1.childPage[1],{bound:[20,11,220,20],caption:"Kode Peruntukan", readOnly:true, tag:9});
		//this.cb_lab = new saiCBBL(this.pc1.childPage[1],{bound:[20,12,220,20],caption:"Kode Lab", readOnly:true, tag:9});
		this.cb_gedung = new saiCBBL(this.pc1.childPage[1],{bound:[20,13,220,20],caption:"Gedung", readOnly:true, tag:9,change:[this,"doChange"]});
		this.cb_ruang = new saiCBBL(this.pc1.childPage[1],{bound:[20,14,220,20],caption:"Ruang", readOnly:true, tag:9});
		this.e_total = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,15,220,20],caption:"Total Usulan", tag:9, readOnly:true, tipeText:ttNilai, text:"0"});						
		
		this.sg = new saiGrid(this.pc1.childPage[2],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:9,
...
window.app_saku3_transaksi_ppbs_aset_fReleaseUsul.extend(window.childForm);
window.app_saku3_transaksi_ppbs_aset_fReleaseUsul.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into log_real_m (no_app,kode_lokasi,tgl_input,nik_user,tanggal,keterangan) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.dp_d1.getDateString()+"','"+this.e_catat.getText()+"')");

					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && this.sg1.cells(0,i) == "RELEASE") {							
							sql.add("insert into log_real_d (no_app,kode_lokasi,no_usul) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.sg1.cells(1,i)+"')");	
							sql.add("update log_usul_m set progress='6', no_app6='"+this.e_nb.getText()+"' where no_usul='"+this.sg1.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
						}
					}
										
					sql.add("insert into agg_d(no_bukti,kode_lokasi,kode_pp,kode_drk,kode_akun,kode_norma,keterangan,satuan,periode,tarif,jumlah,vol,total,modul,prioritas) "+
							"select c.no_usul,c.kode_lokasi,c.kode_pp,c.kode_drk,c.kode_akun,'-',a.keterangan,a.sat,a.periode,a.harga,1,a.vol,a.total,'LOG',prioritas "+
							"from log_usul_d a "+
							"inner join log_real_d b on a.no_usul=b.no_usul and a.kode_lokasi=b.kode_lokasi "+
							"inner join agg_usul_m c on a.no_usul=c.no_usul and a.kode_lokasi=c.kode_lokasi "+
							"where b.no_app = '"+this.e_nb.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"'");
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "simpan" :	
				...
				if (!temu){
					system.alert(this,"Transaksi tidak valid.","Tidak ada transaksi dengan status RELEASE.");
					return false;
				}
				else this.simpan();
				break;				
	...
	doClick:function(sender){
		if (sender == this.i_gen) {			
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"log_real_m","no_app",this.app._lokasi+"-RL"+this.perInput.substr(2,4)+".","0000"));			
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {											
				...
				this.e_nobukti.setText(this.sg1.cells(1,row));	
				this.doLoadCtt(this.e_nobukti.getText());
														
				var data = this.dbLib.getDataProvider(
								"select a.tanggal,a.tahun,a.keterangan,a.nik_app,a.kode_pp,a.kode_akun,a.kode_drk,c.nama as nama_drk,a.kode_rkm, b.kode_peruntukan,b.kode_lab,b.kode_gedung,b.kode_ruang "+
								"from agg_usul_m a "+
								"inner join log_usul_m b on a.no_usul=b.no_usul and a.kode_lokasi=b.kode_lokasi "+
	...
	doLoad:function(sender){	
		var strSQL = "select a.no_usul,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.kode_pp,b.nama as nama_pp,a.kode_akun,c.nama as nama_akun,isnull(d.nilai,0) as nilai "+
					 ...
					 "where a.kode_lokasi='"+this.app._lokasi+"' and a.tahun='"+this.tahun+"' and e.progress='5' order by a.no_usul ";				
		var data = this.dbLib.getDataProvider(strSQL,true);		
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData(["IDLE",line.no_usul,line.tgl,line.kode_pp,line.nama_pp,line.kode_akun,line.nama_akun,line.keterangan,floatToNilai(line.nilai)]); 
		}
