window.app_saku3_transaksi_ppbs_aset_fAjusDRK = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ppbs_aset_fAjusDRK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ppbs_aset_fAjusDRK";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Penyesuaian DRK", 0);	
		...
		this.c_tahun = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,22,202,20],caption:"Tahun",readOnly:true,tag:2});
		...
		this.cb_app = new saiCBBL(this.pc2.childPage[0],{bound:[20,17,220,20],caption:"NIK Approve", readOnly:true, tag:2});
		this.cb_pp = new saiCBBL(this.pc2.childPage[0],{bound:[20,16,220,20],caption:"Kode PP", readOnly:true});
		this.cb_akun = new saiCBBL(this.pc2.childPage[0],{bound:[20,17,220,20],caption:"Kode MTA", readOnly:true, tag:2});
		//this.cb_peruntukan = new saiCBBL(this.pc2.childPage[0],{bound:[20,11,220,20],caption:"Kode Peruntukan", readOnly:true, tag:2});
		//this.cb_lab = new saiCBBL(this.pc2.childPage[0],{bound:[20,12,220,20],caption:"Kode Lab", readOnly:true, tag:2});
		this.cb_gedung = new saiCBBL(this.pc2.childPage[0],{bound:[20,13,220,20],caption:"Gedung", readOnly:true, tag:2,change:[this,"doChange"]});
		this.cb_ruang = new saiCBBL(this.pc2.childPage[0],{bound:[20,14,220,20],caption:"Ruang", readOnly:true, tag:2});
		...
		this.sg = new saiGrid(this.pc2.childPage[1],{bound:[0,5,this.pc2.width-5,this.pc2.height-35],colCount:8,tag:0,
		            colTitle:["Uraian Barang","Spesifikasi","Vol","Sat","Harga Sat","Bulan","Total","Keterangan"],
					colWidth:[[7,6,5,4,3,2,1,0],[150,100,50,100,60,60,200,220]],										
					readOnly:true,
...
window.app_saku3_transaksi_ppbs_aset_fAjusDRK.extend(window.childForm);
window.app_saku3_transaksi_ppbs_aset_fAjusDRK.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("update log_usul_ajus set no_ref='"+this.e_nb.getText()+"' where no_usul='"+this.e_nobukti.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' and no_ref='-'");

					sql.add("insert into log_usul_ajus (no_app,kode_lokasi,tgl_input,nik_user, tahun, tanggal,keterangan,no_usul,no_ref,kode_rkmseb,kode_drkseb) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.c_tahun.getText()+"', '"+this.dp_d1.getDateString()+"','"+this.e_catat.getText()+"','"+this.e_nobukti.getText()+"','-','"+this.rkmSeb+"','"+this.drkSeb+"')");
										
					sql.add("update log_usul_m set progress='5', no_app5='"+this.e_nb.getText()+"' where no_usul='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("update agg_usul_m set kode_rkm='"+this.cb_rkm.getText()+"',kode_drk='"+this.cb_drk.getText()+"' where no_usul='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();					
					//diloncat sql.add("update log_usul_m set progress='4',no_app5='-' where no_usul='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("update log_usul_m set progress='1',no_app5='-' where no_usul='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
	...
	doClick:function(sender){
		if (sender == this.i_gen) {			
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"log_usul_ajus","no_app",this.app._lokasi+"-AJ"+this.perInput.substr(2,4)+".","0000"));			
			...
			if (this.sg1.cells(0,row) != "") {			
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg.appendData([line.uraian,line.spek,floatToNilai(line.vol),line.sat,floatToNilai(line.harga),floatToNilai(line.bulan),floatToNilai(line.total),line.keterangan]);
					}
					this.sg.validasi();
	...
	doLoad:function(sender){	
		var strSQL = "select a.no_usul,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.kode_pp,b.nama as nama_pp,a.kode_akun,c.nama as nama_akun,isnull(d.nilai,0) as nilai "+
					 ...
					 "where a.kode_lokasi='"+this.app._lokasi+"' and a.tahun='"+this.tahun+"' and e.progress='1' order by a.no_usul ";		//e.progress='4' --> diloncat
		var data = this.dbLib.getDataProvider(strSQL,true);		
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
