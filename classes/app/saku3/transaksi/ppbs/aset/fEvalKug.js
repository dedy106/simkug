window.app_saku3_transaksi_ppbs_aset_fEvalKug = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ppbs_aset_fEvalKug.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ppbs_aset_fEvalKug";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Evaluasi Usulan - Keuangan", 0);	
...
window.app_saku3_transaksi_ppbs_aset_fEvalKug.extend(window.childForm);
window.app_saku3_transaksi_ppbs_aset_fEvalKug.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into log_usul_app (no_app,kode_lokasi,tgl_input,nik_user, tahun, tanggal,status,keterangan,no_usul,no_ref,form) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.c_tahun.getText()+"', '"+this.dp_d1.getDateString()+"','"+vStatus+"','"+this.e_catat.getText()+"','"+this.e_nobukti.getText()+"','-','EVKUG')");
										
					sql.add("update log_usul_m set progress='"+vStatus+"',no_app1='"+this.e_nb.getText()+"' where no_usul='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					...
					sql.add("update log_usul_m set progress='1',no_app1='-' where no_usul='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
	...
	doClick:function(sender){
		if (sender == this.i_gen) {			
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"log_usul_app","no_app",this.app._lokasi+"-EL"+this.perInput.substr(2,4)+".","0000"));			
	...
	doLoad:function(sender){	
		var strSQL = "select a.no_usul,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.kode_pp,b.nama as nama_pp,a.kode_akun,c.nama as nama_akun,isnull(d.nilai,0) as nilai "+
					 ...
					 "where a.kode_lokasi='"+this.app._lokasi+"' and a.tahun='"+this.tahun+"' and e.progress='1' order by a.no_usul ";		
