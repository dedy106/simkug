window.app_saku3_transaksi_ppbs_fPaProgram = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ppbs_fPaProgram.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ppbs_fPaProgram";
...
window.app_saku3_transaksi_ppbs_fPaProgram.extend(window.childForm);
window.app_saku3_transaksi_ppbs_fPaProgram.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into agg_program(kode_program,nama,kode_lokasi,flag_aktif,tahun,kode_pp) values ('"+
							this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.c_status.getText().substr(0,1)+"','"+this.c_tahun.getText()+"','"+this.cb_pp.getText()+"')");
					setTipeButton(tbAllFalse);
	...
	simpan2: function(){			
		try{						
			if (this.sg.getRowValidCount() > 0){
				...
				{
					sql.add("delete from agg_program where kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"'");
				}
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from agg_program where kode_program = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"'");
					sql.add("insert into agg_program(kode_program,nama,kode_lokasi,flag_aktif,tahun,kode_pp) values ('"+
							this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.c_status.getText().substr(0,1)+"','"+this.c_tahun.getText()+"','"+this.cb_pp.getText()+"')");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
	...
	doChange: function(sender){
		try{
			...
			if (this.cb_kode.getText() != ""){
				var strSQL = "select kode_program,nama,tahun,kode_pp from agg_program where kode_program ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"'";						   
		...
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Program",sender,undefined, 
											  "select kode_program, nama  from agg_program where kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"'",
											  "select count(kode_program) from agg_program where kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"'",
	...
	doLoad:function(sender){						
		var strSQL = "select kode_program,nama,tahun,kode_pp "+
		             "from agg_program where kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"' order by kode_program ";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
	...
	doCari:function(sender){								
		try {
			...
			var strSQL = "select kode_program,nama,tahun,kode_pp "+
		             "from agg_program where kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.c_tahun.getText()+"' "+filter+" order by kode_program ";		
			var data = this.dbLib.getDataProvider(strSQL,true);		
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU = data;
				this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length/20));
