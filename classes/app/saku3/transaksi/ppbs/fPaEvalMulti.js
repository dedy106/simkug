window.app_saku3_transaksi_ppbs_fPaEvalMulti = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_ppbs_fPaEvalMulti.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_ppbs_fPaEvalMulti";
		...
		this.pc1 = new pageControl(this,{bound:[20,10,500,350], childPage:["Daftar Akun Evaluasi","Pesan Error"]});				
		this.sg = new portalui_saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-30],colCount:3,tag:0,
		            colTitle:["Kode Akun","Nama","Deviasi(+/-) %"],
					colWidth:[[2,1,0],[150,200,80]],					
...
window.app_saku3_transaksi_ppbs_fPaEvalMulti.extend(window.childForm);
window.app_saku3_transaksi_ppbs_fPaEvalMulti.implement({	
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								...
								sql.add("update agg_d set vol=vol+((vol * "+nilaiToFloat(this.sg.cells(2,i))+")/100), "+
										...
										"where kode_lokasi='"+this.app._lokasi+"' and periode like '"+this.c_tahun.getText()+"%' and kode_akun='"+this.sg.cells(0,i)+"' and modul='UMUM'");						 					
							}
						}
					}
