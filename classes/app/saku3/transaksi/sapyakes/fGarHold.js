window.app_saku3_transaksi_sapyakes_fGarHold = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sapyakes_fGarHold.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fGarHold";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Hold Data Anggaran", 0);	
...
window.app_saku3_transaksi_sapyakes_fGarHold.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fGarHold.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					var strSQL = "select b.kode_akun,c.kode_pp,periode, sum(case a.dc when 'D' then a.nilai else -a.nilai end) as nilai "+
								 ...
								 "where a.kode_pp ='KS00A00' and a.periode='"+this.e_tahun.getText()+this.c_bulan.getText()+"'  "+
					...
					if (typeof databudget == "object" && databudget.rs.rows[0] != undefined){
						...
						this.app.services.releaseSAP(data, this.e_tahun.getText(), this.e_usersap.getText(), this.e_pwdsap.getText(), function(data){ alert(data);  });
					}
