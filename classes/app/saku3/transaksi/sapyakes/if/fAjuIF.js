window.app_saku3_transaksi_sapyakes_if_fAjuIF = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sapyakes_if_fAjuIF.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_if_fAjuIF";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pemakaian IF", 0);	
		...
		this.sg3 = new saiGrid(this.pc2.childPage[2],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:7,tag:9,
		            colTitle:["No Pengajuan","Tanggal","Tgl Kuitansi","Deskripsi","Progress","Nilai","No App"],
		...
		this.e_nb = new portalui_saiLabelEdit(this.pc2.childPage[0],{bound:[20,12,200,20],caption:"No Bukti",maxLength:30,readOnly:true,visible:false,tag:0});		
		this.cb_app = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,12,220,20],caption:"Posting SAP",tag:2,readOnly:true});         						
		...
		this.cb_pp = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,11,220,20],caption:"PP",tag:2,multiSelection:false,change:[this,"doChange"]}); 		
		this.cb_akun = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,12,220,20],caption:"Akun",tag:1,multiSelection:false,change:[this,"doChange"]});         		
		this.cb_drk = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,13,220,20],caption:"DRK",tag:1,multiSelection:false});         				
		this.e_saldo = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,18,200,20],caption:"Saldo Budget", tag:1, tipeText:ttNilai, text:"0", readOnly:true});

		this.e_ket = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,16,550,20],caption:"Uraian", maxLength:150});						
		this.e_nilai = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,17,200,20],caption:"Nilai Total", tag:1, tipeText:ttNilai, text:"0",change:[this,"doChange"]});		
		this.cb_setuju = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,14,220,20],caption:"Disetujui Oleh",tag:2,multiSelection:false});         				
		...
		this.e_ndpp = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,11,200,20],caption:"Nilai DPP", tag:0, tipeText:ttNilai, text:"0",visible:false,change:[this,"doChange"]});		
		this.e_npersen = new saiCB(this.pc2.childPage[0],{bound:[20,13,200,20],caption:"Persentase", tag:0, visible:false,change:[this,"doChange"]});
		this.e_npajak = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,12,200,20],caption:"Nilai Pajak", tag:0, tipeText:ttNilai, text:"0",visible:false,change:[this,"doChange"]});		
		this.e_netto = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,15,200,20],caption:"Nilai Netto", readOnly:true, tag:0, tipeText:ttNilai, text:"0"});
		...
		try {
			...
			if (this.app._lokasi == "99") {
				this.cb_if.setSQL("select a.nik, a.nama from karyawan a "+
								"			inner join if_m b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi and b.flag_aktif='1' "+
								"			inner join pp c on b.kode_pp=c.kode_pp and b.kode_lokasi=c.kode_lokasi  "+
								"where c.kode_bidang='"+this.app._kodeBidang+"' and c.kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Data Karyawan",true);			
				var strSQL = "select a.nik from karyawan a "+
							"			inner join if_m b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi and b.flag_aktif='1' "+
							"			inner join pp c on b.kode_pp=c.kode_pp and b.kode_lokasi=c.kode_lokasi  "+
				...
				this.cb_if.setSQL("select a.nik, a.nama from karyawan a "+
								"		 inner join if_m b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi and b.flag_aktif='1' "+								
								"where b.kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Data Karyawan",true);			
				var strSQL = "select a.nik from karyawan a "+
							"			inner join if_m b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi and b.flag_aktif='1' "+							
			...
			var strSQL = "select a.nik from karyawan a inner join sap_nik_post b on a.nik=b.nik where a.flag_aktif ='1' and a.kode_lokasi='"+this.app._lokasi+"'";							  
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){		
			...
			var data = this.dbLib.getDataProvider("select sap_user,sap_pwd from hakakses where nik='"+this.cb_app.getText()+"'",true);			
			if (typeof data == "object"){
				var line = data.rs.rows[0];		
...
window.app_saku3_transaksi_sapyakes_if_fAjuIF.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_if_fAjuIF.implement({		
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					vProg = "0";
					sql.add("insert into if_aju_m(no_aju,kode_lokasi,periode,tgl_input,user_input,tanggal,tgl_kuitansi,modul,kode_akun,kode_pp,kode_drk,keterangan,nilai,sts_pajak,npajak,no_app,no_ganti,progress,nik_app,no_reim,dok_sap,posted,nik_setuju, nilai_dpp,persen, nik_if,no_postsap) values "+
							...
							this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",'"+this.c_status.getText()+"',"+nilaiToFloat(this.e_npajak.getText())+",'"+this.noAppLama+"','-','"+vProg+"','"+this.cb_app.getText()+"','-','-','F','"+this.cb_setuju.getText()+"', "+nilaiToFloat(this.e_ndpp.getText())+","+nilaiToFloat(this.e_npersen.getText())+",'"+this.cb_if.getText()+"','-')");
					...
					sql.add("insert into glsap(no_bukti,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,kode_curr,kurs,nilai_curr,tgl_input,nik_user,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,no_doksap,kode_rek,no_payment,paymetod) "+
							"select no_aju,nu,kode_lokasi,'IFAJU',jenis,no_ref,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,'IDR',1,nilai,getdate(),'"+this.app._userLog+"','-','-','-',no_ref,'-','-','-',(case when no_ref = '-' then '-' else '"+this.rekSAP+"' end),'-',(case when no_ref = '-' then '-' else 'E' end) "+
						    "from if_aju_j "+
	...
	doChange:function(sender){
		if (sender == this.cb_if && this.cb_if.getText()!="") {
			var strSQL = "select a.kode_vendor,a.rek_sap,a.nik_app,a.nilai - isnull(b.pakai,0) as saldo  "+
						 "from if_m a "+

						 "		left join  ("+						
						 "			 select nik_if,kode_lokasi,sum(nilai) as pakai "+
						 "			 from if_aju_m where no_reim='-' and nik_if='"+this.cb_if.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and no_aju<>'"+this.e_nb.getText()+"' "+
						 "			 group by nik_if,kode_lokasi "+					
						 "		) b on a.nik = b.nik_if and a.kode_lokasi=b.kode_lokasi "+

						 "where a.nik ='"+this.cb_if.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";						   
	...
	doLoad3:function(sender){																								
		...
		var strSQL = "select a.no_aju,convert(varchar,a.tanggal,103) as tgl,modul,convert(varchar,a.tgl_kuitansi,103) as tglkui,a.keterangan,a.progress,a.nilai,a.no_app "+
					 ...
					 "					select distinct no_bukti,kode_lokasi from glsap "+
					 "					where no_doksap='-' and modul='IFAJU' and kode_lokasi='"+this.app._lokasi+"' "+
					 "				  ) c on a.no_aju=c.no_bukti and a.kode_lokasi=c.kode_lokasi "+			 					 

					 "where a.modul='IFAJU' "+bidang+" and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress in ('0','C') order by a.no_aju";		
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.no_app as no_applama,a.keterangan,a.tgl_kuitansi,a.tanggal,a.nik_app,isnull(b.no_app,'-') as no_app,isnull(b.catatan,'-') as catatan, "+
							 ...
							 "		  from yks_app_m "+
							 "        where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and no_flag='-' and modul='IFAJU' and form='APPGAR' "+							 							 
