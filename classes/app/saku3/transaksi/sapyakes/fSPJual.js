window.app_saku3_transaksi_sapyakes_fSPJual = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sapyakes_fSPJual.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fSPJual";
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('DRKSPJ','PPINV','SPPIU','SPPIUGL','SPNT','SPGL') and kode_lokasi = '"+this.app._lokasi+"'",true);			
			if (typeof data == "object"){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];												
					if (line.kode_spro == "PPINV") this.kodepp = line.flag;								
					if (line.kode_spro == "SPPIU") this.akunPiutang = line.flag;	
					...
					if (line.kode_spro == "SPGL") this.akunGL = line.flag;									
				}
			}
...
window.app_saku3_transaksi_sapyakes_fSPJual.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fSPJual.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into inv_spjual_m(no_spjual,kode_lokasi,periode,tanggal,nik_user,tgl_input,posted,no_kasjual,nik_buat,no_dokumen,keterangan,kode_drk,kode_spkelola,tgl_set,nilai_komisi,nilai_ppn,nilai_levy,nilai_pph,akun_piutang,akun_piugl,nilai_piutang,nilai_piugl,modul,kode_pp,no_app1,progress, nik_app) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.app._userLog+"',getdate(),'F','-','"+this.app._userLog+"',  '-','"+this.e_ket.getText()+"','"+this.cb_drk.getText()+"','-','"+this.dp_d2.getDateString()+"',0,0,0,0,'"+this.akunPiutang+"','"+this.akunPiuGL+"',"+nilaiPiutang+","+nilaiPiuGL+",'SPJUAL','"+this.kodepp+"','-','0','"+this.cb_app.getText()+"')");					
					...
					for (var i = 0; i < this.sg.rows.getLength();i++){
						if (this.sg.rowValid(i)){
							if (nilaiToFloat(this.sg.cells(7,i)) != 0 ) {
								...
								sql.add("update inv_sp_d set jumlah=jumlah-"+nilaiToFloat(this.sg.cells(6,i))+" where kode_mitra='"+this.sg.cells(0,i)+"'");
								sql.add("insert into inv_spjual_d (no_spjual,kode_spkelola,kode_mitra,h_oleh,h_buku,h_jual,jumlah,n_jual,gainlos, komisi,vat,levi,pph) values "+
					...
					sql.add("insert into glsap(no_bukti,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,kode_curr,kurs,nilai_curr,tgl_input,nik_user,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,no_doksap,kode_rek,no_payment,paymetod)  "+
								"select no_spjual,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,'IDR',1,nilai,tgl_input,nik_user,'-','-','-','-','-','-','-','-','-','-' "+
								"from inv_spjual_j where no_spjual='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doChangeCell: function(sender, col, row){					
		if (col == 0 && this.sg.cells(0,row)!=""){			
			...
			var strSQL = "select a.jumlah,a.h_oleh,a.h_buku from inv_sp_tmp a "+
						 "where a.kode_mitra='"+this.sg.cells(0,row)+"' and a.nik_user='"+this.nik_user+"' ";
	...
	doLoad3:function(sender){														
		var strSQL = "select a.no_spjual,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.progress,b.catatan "+
		             "from inv_spjual_m a "+
		...
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			...
			for (var i=0;i<this.dataJU3.rs.rows.length;i++){
				...
				this.sg3.appendData([line.no_spjual,line.tgl,line.keterangan,line.progress,line.catatan]); 
			}			
		...
		try{
			...
			if (this.sg3.cells(0,baris) != "") {			
				...
				var strSQL = "select b.kode_mitra,b.nama,a.jumlah,d.h_oleh,d.h_buku,d.h_jual,d.jumlah as jml,d.n_jual,gainlos "+
							 "from inv_sp_tmp a inner join inv_mitra b on a.kode_mitra=b.kode_mitra "+
							 "     inner join inv_spjual_d d on a.kode_mitra=d.kode_mitra "+
							 "where d.no_spjual = '"+this.e_nb.getText()+"'";			
				 var data = this.dbLib.getDataProvider(strSQL,true);	
				 if (typeof data == "object" && data.rs.rows[0] != undefined){
					 var line;
					 this.sg.clear();
