window.app_saku3_transaksi_sapyakes_fSpjDokAppPusat = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sapyakes_fSpjDokAppPusat.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fSpjDokAppPusat";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,325], childPage:["Data SPPD","Data Transport","Uang Harian","Otorisasi"]});
		...
		this.cb_app = new saiCBBL(this.pc1.childPage[3],{bound:[20,15,220,20],caption:"NIK App Budget", multiSelection:false, maxLength:10, tag:2});						
		this.cb_tahu = new saiCBBL(this.pc1.childPage[3],{bound:[20,16,220,20],caption:"NIK Perintah", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});										
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag,keterangan from spro where kode_spro in ('VENDORSPJ','SAPHPD','BIDYAN','BIDKUG','BIDUM','BIDINV')",true);		
			if (typeof data == "object"){
				...
				for (var i in data.rs.rows){
					...
					if (line.kode_spro == "SAPHPD") this.akunHutPD= line.flag;	
					
					if (line.kode_spro == "VENDORSPJ") {
...
window.app_saku3_transaksi_sapyakes_fSpjDokAppPusat.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fSpjDokAppPusat.implement({		
		...
		if (typeof data2 == "object" && data2.rs.rows[0] != undefined){
			...
			if (this.jenisApp1.toUpperCase() != "LOS") {
				...
				if (this.jenisApp1.toUpperCase() == "BID") {		
					...
					//"select kode_spro,flag,keterangan from spro where kode_spro in ('VENDORSPJ','SAPHPD','BIDYAN','BIDKUG','BIDUM','BIDINV')"
