window.app_saku3_transaksi_sapyakes_fAkruDummy = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sapyakes_fAkruDummy.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fAkruDummy";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Akru Dummy", 0);	
...
window.app_saku3_transaksi_sapyakes_fAkruDummy.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fAkruDummy.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i) && this.sg2.cells(4,i) != "0"){
								sql.add("insert into yk_hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs,kode_rek) values "+
										"('"+this.e_nb.getText()+"','"+this.sg2.cells(8,i)+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg2.cells(0,i)+"','"+this.sg2.cells(3,i)+"','"+this.sg2.cells(2,i)+"',"+parseNilai(this.sg2.cells(4,i))+",'"+this.kodepp+"','"+this.sg2.cells(6,i)+"','"+this.app._lokasi+"','HUTKES','"+this.sg2.cells(5,i)+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1,'-')");								
							}
						}
					}										
					var nobukti = ""; 
					...
					sql.add("insert into yk_hutang_m(no_hutang,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,tgl_input,nik_user,kode_loktuj,no_app,progress) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','-','"+this.e_ket.getText()+"','"+this.kodepp+"','HUTDUMMY','BILL','IDR',1,"+totHut+",'"+this.app._userLog+"','"+this.cb_app.getText()+"','F',getdate(),'"+this.app._userLog+"','"+this.app._lokasi+"','-','0')");
					...
					sql.add("insert into glsap(no_bukti,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,kode_curr,kurs,nilai_curr,tgl_input,nik_user,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,no_doksap,kode_rek,no_payment,paymetod) "+
							...
							"'-','-',"+
							"(case when jenis in ('PEGAWAI','PENSIUN') then no_dokumen else '-' end) as kode_vendor,"+
							"'-','-','-','-','-','T' "+
							"from yk_hutang_j "+
							"where nilai <> 0 and kode_lokasi='"+this.app._lokasi+"' and no_hutang='"+this.e_nb.getText()+"'");
				
					sql.add("insert into glsap(no_bukti,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,kode_curr,kurs,nilai_curr,tgl_input,nik_user,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,no_doksap,kode_rek,no_payment,paymetod) "+
							"select no_bukti,999,kode_lokasi,modul,'TEMP',no_dokumen,tanggal,'"+this.akunTemp+"',case dc when 'D' then 'C' else 'D' end,sum(nilai),'Temporary Account',kode_pp,periode,kode_drk,kode_curr,kurs,sum(nilai_curr),getdate(),nik_user,'-',kode_proyek,'-','-',kode_lokarea,nik,no_doksap,'-','-','-' "+
							"from glsap "+
							"where nilai <> 0 and kode_lokasi='"+this.app._lokasi+"' and no_dokumen='"+this.e_nb.getText()+"' "+
							"group by "+
							"no_bukti,kode_lokasi,modul,no_dokumen,tanggal,case dc when 'D' then 'C' else 'D' end,kode_pp,periode,kode_drk,kode_curr,kurs,nik_user,kode_proyek,kode_lokarea,nik,no_doksap ");
	...
	doTampil:function(sender){		
		if (this.e_periode.getText() != "") {
			var strSQL = "select a.no_bill,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,a.nilai,a.kunj,a.cs,a.jenis "+
			             "from yk_bill_m a "+
						 "where a.no_bill not like '%RUJ%' and a.jenis = 'AKRU' and a.progress = '0' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"'";
	...
	doJurnal:function(sender){		
		try {
			...
			var strSQL = "select d.nama as ket,"+
						...
						"       sum(a.nilai) as nilai,case f.jenis when 'PENSIUN' then 'CC' else 'BP' end as jenis,g.kode_drk,g.nama as nama_drk,f.kode_sap as atensi "+
						"from yk_bill_d a  "+
						"           inner join yk_loker ff on a.loker=ff.loker "+
						"           inner join cust f on ff.kode_cust=f.kode_cust "+
						...
						"                             when 'PEGAWAI' then b.akun_bp "+
						"                             when 'GROUP' then b.akun_ap "+						
						"       end,d.nama,case f.jenis when 'PENSIUN' then 'CC' else 'BP' end,g.kode_drk,g.nama,f.kode_sap "+												
						"union all "+													
						...
						"case f.jenis when 'PENSIUN' then 'PENSIUN' else 'PEGAWAI' end as jenis,'-' as kode_drk,'-' as nama_drk,b.kode_sap as atensi "+
						"from yk_bill_d a "+
						"inner join yk_loker ff on a.loker=ff.loker "+
						"inner join cust f on ff.kode_cust=f.kode_cust "+						
						"inner join vendor b on a.kode_vendor=b.kode_vendor and a.kode_lokasi=b.kode_lokasi "+
						...
						"         case f.jenis when 'PENSIUN' then 'HUTANG PENSIUN' else 'HUTANG PEGAWAI' end, b.kode_sap "+							
						"union all "+						
						...
						"sum(a.umum+a.gigi+a.kbia+a.matkes) as nilai,'PIUKUNJ' as jenis, c.drk_kunj as kode_drk,e.nama as nama_drk, b.kode_sap as atensi "+
						...
						"sum(a.umum+a.gigi+a.kbia+a.matkes) as nilai,'PDPTKUNJUM' as jenis, c.drk_kunj as kode_drk,e.nama as nama_drk,'-' as atensi "+
						"from yk_billkunj_d a "+
						"                     inner join yk_loker bb on a.loker=bb.loker "+
						"				      inner join cust b on bb.kode_cust=b.kode_cust "+
						"		 	          inner join yk_kunj c on a.kode_produk=c.kode_produk "+
						...
						"sum(a.cs) as nilai,'HUTCS' as jenis, c.drk_kunj as kode_drk,e.nama as nama_drk,b.kode_sap as atensi "+
						"from yk_billkunj_d a "+
						"                     inner join yk_loker bb on a.loker=bb.loker "+
						"					  inner join cust b on bb.kode_cust=b.kode_cust "+
						"		 	          inner join yk_kunj c on a.kode_produk=c.kode_produk "+
						...
						"group by  c.nama,c.akun_hutcs,d.nama,c.drk_kunj,e.nama,b.kode_sap "+
						"union all "+		
										
						"select 'PIU CS '+c.nama as ket,c.akun_piucs as kode_akun,d.nama as nama_akun,'C', "+
						"sum(a.cs) as nilai,'PIUCS' as jenis, c.drk_kunj as kode_drk,e.nama as nama_drk,b.kode_sap as atensi  "+
						"from yk_billkunj_d a "+
						"                     inner join yk_loker bb on a.loker=bb.loker "+
						"                     inner join cust b on bb.kode_cust=b.kode_cust "+
						"		 	          inner join yk_kunj c on a.kode_produk=c.kode_produk "+
						...
						"		 	          inner join drk e on c.drk_kunj=e.kode_drk and e.kode_lokasi='"+this.app._lokasi+"' and e.tahun='"+this.e_periode.getText().substr(0,4)+"' "+
						"where a.cs<> 0 and b.jenis <> 'PENSIUN' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_bill in ("+nobukti+") "+
						"group by  c.nama,c.akun_piucs,d.nama,c.drk_kunj,e.nama,b.kode_sap  "+
	...
	doLoad4:function(sender){																				
		var strSQL = "select a.no_hutang,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai,a.nik_setuju+' - '+b.nama as nama "+
					 ...
					 "where a.modul= 'HUTDUMMY' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress in ('0','R') and a.posted='F' ";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU4 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg4.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.jenis,a.kode_drk,isnull(d.nama,'-') as nama_drk,a.no_dokumen "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];													
						this.sg2.appendData([line.kode_akun,line.nama_akun,line.dc,line.keterangan,floatToNilai(line.nilai),line.jenis,line.kode_drk,line.nama_drk,line.no_dokumen]);
					}
