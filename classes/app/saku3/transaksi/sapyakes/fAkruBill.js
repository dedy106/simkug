window.app_saku3_transaksi_sapyakes_fAkruBill = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sapyakes_fAkruBill.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fAkruBill";
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag,keterangan from spro where kode_spro in ('GARFREE','PPBPCC') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku3_transaksi_sapyakes_fAkruBill.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fAkruBill.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into yk_hutang_d(no_hutang,kode_lokasi,no_inv,periode,kode_vendor,bank,cabang,no_rek,nama_rek,nilai_bp,nilai_cc,no_rekon,bank_trans,no_app,no_ver,no_spb,no_kas,progress,kode_loktuj) "+
							...
							"       b.kode_vendor,b.bank,b.cabang,b.no_rek,b.nama_rek,isnull(sum(case when f.jenis <> 'PENSIUN' then a.nilai - a.pph else 0 end),0),isnull(sum(case when f.jenis = 'PENSIUN' then a.nilai - a.pph else 0 end),0),'SAPDOC',b.bank_trans,'-','-','-','-','0','"+this.cb_lokasi.getText()+"' "+
							" from yk_bill_d a "+
					...
					sql.add("insert into glsap(no_bukti,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,kode_curr,kurs,nilai_curr,tgl_input,nik_user,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,no_doksap,kode_rek,no_payment,paymetod) "+
							...
							"(case when jenis in ('BP','CC','PIUKUNJ','PIUCS') then no_dokumen else '-' end) as kode_cust, "+
							"'-','-',"+
	...
	doJurnal:function(sender){		
		try {
			...
			var strSQL = "select d.nama as ket,"+
						...
						"       sum(a.nilai) as nilai,case f.jenis when 'PENSIUN' then 'CC' else 'BP' end as jenis,g.kode_drk,g.nama as nama_drk, f.kode_sap as atensi, f.kode_rek "+
						"from yk_bill_d a  "+
						"           inner join yk_loker ff on a.loker=ff.loker "+
						"           inner join cust f on ff.kode_cust=f.kode_cust "+
						...
						"                             when 'PEGAWAI' then b.akun_bp "+
						"                             when 'GROUP' then b.akun_ap "+						
						"       end,d.nama,case f.jenis when 'PENSIUN' then 'CC' else 'BP' end,g.kode_drk,g.nama,f.kode_sap, f.kode_rek "+												
						"union all "+													
						...
						"select "+
						"'HUTANG PPH '+b.nama as ket,cc.kode_akun,cc.nama as nama_akun,'C' as dc, "+
						"sum(a.pph) as nilai,'PPH' as jenis,'-' as kode_drk,'-' as nama_drk, '-' as atensi, '-' as kode_rek  "+
						"from yk_bill_d a "+							
						"				  inner join yk_bill_m d on a.no_bill=d.no_bill and a.kode_lokasi=d.kode_lokasi "+
						"                 inner join vendor b on a.kode_vendor=b.kode_vendor and a.kode_lokasi=b.kode_lokasi "+
						"                 inner join masakun cc on b.akun_pph = cc.kode_akun and b.kode_lokasi=cc.kode_lokasi "+
						"where d.no_bill in ("+nobukti+") and d.kode_lokasi='"+this.app._lokasi+"' and a.pph <> 0 "+
						"group by cc.kode_akun,b.nama,cc.nama "+							
						"union all "+																								
						...
						"sum(a.umum+a.gigi+a.kbia+a.matkes) as nilai,'PIUKUNJ' as jenis, c.drk_kunj as kode_drk,e.nama as nama_drk, b.kode_sap as atensi, b.kode_rek "+
						...
						"group by  c.nama,case b.jenis when 'PEGAWAI' then c.akun_pku else c.akun_pap end,d.nama,c.drk_kunj,e.nama,b.kode_sap, b.kode_rek "+																							
						"union all "+												
						
						"select 'PDPT KUNJ '+c.nama as ket,c.akun_ku as kode_akun,d.nama as nama_akun,'C', "+
						"sum(a.umum+a.gigi+a.kbia) as nilai,'PDPTKUNJUM' as jenis, c.drk_kunj as kode_drk,e.nama as nama_drk,'-' as atensi,'-' as kode_rek "+
						"from yk_billkunj_d a "+
						"                     inner join yk_loker bb on a.loker=bb.loker "+
						"				      inner join cust b on bb.kode_cust=b.kode_cust "+
						"		 	          inner join yk_kunj c on a.kode_produk=c.kode_produk "+
						...
						"sum(a.matkes) as nilai,'PDPTKUNJUM' as jenis, c.drk_kunj as kode_drk,e.nama as nama_drk,'-' as atensi , '-' as kode_rek "+
						"from yk_billkunj_d a "+
						"                     inner join yk_loker bb on a.loker=bb.loker "+
						"				      inner join cust b on bb.kode_cust=b.kode_cust "+
						"		 	          inner join yk_kunj c on a.kode_produk=c.kode_produk "+
						...
						"sum(a.cs) as nilai,'HUTCS' as jenis, c.drk_kunj as kode_drk,e.nama as nama_drk,b.kode_sap as atensi, b.kode_rek "+
						"from yk_billkunj_d a "+
						"                     inner join yk_loker bb on a.loker=bb.loker "+
						"					  inner join cust b on bb.kode_cust=b.kode_cust "+
						"		 	          inner join yk_kunj c on a.kode_produk=c.kode_produk "+
						...
						"group by  c.nama,c.akun_hutcs,d.nama,c.drk_kunj,e.nama,b.kode_sap, b.kode_rek "+
						"union all "+						
						
						"select 'PIU CS '+c.nama as ket,c.akun_piucs as kode_akun,d.nama as nama_akun,'C', "+
						"sum(a.cs) as nilai,'PIUCS' as jenis, c.drk_kunj as kode_drk,e.nama as nama_drk, b.kode_sap as atensi, b.kode_rek "+
						"from yk_billkunj_d a "+
						"                     inner join yk_loker bb on a.loker=bb.loker "+
						"                     inner join cust b on bb.kode_cust=b.kode_cust "+
						"		 	          inner join yk_kunj c on a.kode_produk=c.kode_produk "+
						...
						"		 	          inner join drk e on c.drk_kunj=e.kode_drk and e.kode_lokasi='"+this.app._lokasi+"' and e.tahun='"+this.e_periode.getText().substr(0,4)+"' "+
						"where a.cs<> 0 and b.jenis <> 'PENSIUN' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_bill in ("+nobukti+") "+
						"group by  c.nama,c.akun_piucs,d.nama,c.drk_kunj,e.nama,b.kode_sap, b.kode_rek "+
	...
	doLoad4:function(sender){																				
		var strSQL = "select a.no_hutang,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai,a.kode_loktuj+' - '+c.nama as lokbayar,a.nik_setuju+' - '+b.nama as nama "+
					 ...
					 "where a.modul = 'HUTKES' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress in ('0','R') and a.posted='F' ";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU4 = data;
