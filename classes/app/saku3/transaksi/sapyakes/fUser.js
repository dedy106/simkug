window.app_saku3_transaksi_sapyakes_fUser = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sapyakes_fUser.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fUser";
		...
		this.e_sapuser = new saiLabelEdit(this,{bound:[20,14,200,20],caption:"SAP User", maxLength:20});		
		this.e_sappwd = new saiLabelEdit(this,{bound:[20,15,200,20],caption:"SAP Pwd", password:true,maxLength:20});		
...
window.app_saku3_transaksi_sapyakes_fUser.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fUser.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try{								
					...
					sql.add("insert into hakakses (nik, nama, pass, status_admin, kode_lokasi, kode_klp_menu, klp_akses, sap_user, sap_pwd) values  "+
						    "('"+this.cb_kode.getText()+"','"+this.cb_kode.rightLabelCaption+"','"+this.e_pass.getText()+"','"+sts_admin+"','"+this.kodeLokasi+"','"+this.c_menu.getText()+"','ADMIN','"+this.e_sapuser.getText()+"','"+this.e_sappwd.getText()+"')");					
					setTipeButton(tbAllFalse);
	...
	doChange: function(sender){
		try{			
			if (this.cb_kode.getText() != ""){				
				var data = this.dbLib.getDataProvider("select a.status_admin,a.kode_klp_menu,a.status_admin,a.pass,a.kode_lokasi,sap_user,sap_pwd "+
						   "from hakakses a "+
						   "inner join karyawan b on a.nik=b.nik "+ //and a.kode_lokasi=b.kode_lokasi 
						   "where a.nik ='"+this.cb_kode.getText()+"' ",true); //and a.kode_lokasi = '"+this.app._lokasi+"' 
				...
				if (typeof data == "object"){		
					...
					if (line != undefined){
						...
						this.e_sapuser.setText(line.sap_user);
						this.e_sappwd.setText(line.sap_pwd);
