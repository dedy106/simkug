window.app_saku3_transaksi_sapyakes_fSPSPI = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_sapyakes_fSPSPI.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fSPSPI";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,22,995,305], childPage:["Data Penyertaan","Rekap Data"]});		
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('SPNT','SPDRKSPI','PPINV') and kode_lokasi = '"+this.app._lokasi+"'",true);			
			if (typeof data == "object"){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];																	
					if (line.kode_spro == "PPINV") this.kodepp = line.flag;	
					if (line.kode_spro == "SPNT") this.akunNT= line.flag;	
					if (line.kode_spro == "SPDRKSPI") this.cb_drk.setText(line.flag);							
				}
			}
...
window.app_saku3_transaksi_sapyakes_fSPSPI.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fSPSPI.implement({
			...
			if (result) {								
				...
				for (var i=0; i < this.dataUpload.rows.length;i++){
					...
					for (var j=0;j < this.dataJU.rs.rows.length;j++){
						if (line.mitra == this.dataJU.rs.rows[j].kode_mitra) {							
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into inv_spspi_m(no_spi,no_dokumen,tanggal,keterangan,kode_curr,kurs,nilai,kode_pp,kode_drk,posted,modul,nik_buat,nik_setuju,kode_lokasi,periode,no_del,no_link,nik_user,tgl_input,jenis,no_spikug,no_app1,progress) values "+
						    "('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.kodepp+"','"+this.cb_drk.getText()+"','F','SPSPI','"+this.app._userLog+"','"+this.cb_app.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','-','-','"+this.app._userLog+"',getdate(),'"+this.c_jenis.getText()+"','-','-','0')");
					...
					if (this.e_periode.getText().substr(4,2) != "01") {
						sql.add("insert into inv_spspi_d(no_spi,kode_lokasi,kode_spkelola,kode_mitra,jumlah,h_oleh,h_buku,h_wajar,flag_rev,dc) "+					
								"select '"+this.e_nb.getText()+"','"+this.app._lokasi+"',kode_spkelola,kode_mitra,jumlah,h_oleh,h_buku,h_wajar,no_spi,'C' from inv_spspi_d where flag_rev='-' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("update inv_spspi_d set flag_rev='"+this.e_nb.getText()+"' where flag_rev='-' and kode_lokasi='"+this.app._lokasi+"'");					
					}
					else {
						sql.add("insert into inv_spspi_d(no_spi,kode_lokasi,kode_spkelola,kode_mitra,jumlah,h_oleh,h_buku,h_wajar,flag_rev,dc) "+					
								"select '"+this.e_nb.getText()+"','"+this.app._lokasi+"',a.kode_spkelola,a.kode_mitra,a.jumlah,a.h_oleh,a.h_buku,a.h_wajar,a.no_spi,'C' "+
					...
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						...
						sql.add("insert into inv_spspi_d(no_spi,kode_lokasi,kode_spkelola,kode_mitra,jumlah,h_oleh,h_buku,h_wajar,flag_rev,dc) values "+								
					            "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','-','"+line.kode_mitra+"',"+line.jumlah+","+h_oleh+","+h_buku+","+h_wajar+",'"+flag_rev+"','D')");
					}	
	...
	doLoadData: function(sender){
		...
		var sql = "call sp_sp_tmp ('"+this.e_periode.getText()+"','"+this.nik_user+"')";
		...
		var strSQL = "select x.akun_spi,a.kode_mitra,x.nama,a.jumlah,a.h_oleh,a.h_buku,a.h_oleh * a.jumlah as nilai_oleh,a.h_buku * a.jumlah as nilai_buku,0 as nilai_wajar,0 as nilai_sls,0 as total,isnull(b.nilai_spi,0) as total_spi "+		        
					 ...
					 "       left join ( "+
					 "       select x.kode_mitra,round(sum( (case x.dc when 'D' then (x.h_wajar-x.h_buku) else -(x.h_wajar-x.h_buku) end) * x.jumlah),0) as nilai_spi "+
					 "       from inv_spspi_d x inner join inv_spspi_m y on x.no_spi=y.no_spi and x.kode_lokasi=y.kode_lokasi "+
					 "       where y.periode<='"+this.e_periode.getText()+"' and x.flag_rev='-' group by x.kode_mitra "+
					 ") b on a.kode_mitra=b.kode_mitra "+
					 "where a.nik_user='"+this.nik_user+"' order by a.kode_mitra"; //where a.jumlah <> 0 --> yg sudah dijual di reverse meski jmlnya sudah nol		
		...
		try{
			...
			if (this.sg3.cells(0,baris) != "") {			
				...
				var strSQL = "select x.akun_spi,a.kode_mitra,x.nama,a.jumlah,a.h_oleh,a.h_buku,a.h_oleh * a.jumlah as nilai_oleh,a.h_buku * a.jumlah as nilai_buku,round(a.h_wajar * a.jumlah,0) as nilai_wajar,round((a.h_wajar-a.h_buku) * a.jumlah,0) as nilai_sls,round((a.h_wajar-a.h_buku) * a.jumlah,0) as total,isnull(b.nilai_spi,0) as total_spi "+		        
							 ...
							 "       select x.kode_mitra,round(sum( (case x.dc when 'D' then (x.h_wajar-x.h_buku) else -(x.h_wajar-x.h_buku) end) * x.jumlah),0) as nilai_spi "+
							 "       from inv_spspi_d x inner join inv_spspi_m y on x.no_spi=y.no_spi and x.kode_lokasi=y.kode_lokasi "+
							 "       where y.no_spi<>'"+this.e_nb.getText()+"' and y.periode<='"+this.e_periode.getText()+"' and x.flag_rev='"+this.e_nb.getText()+"' group by x.kode_mitra "+
							 ") b on a.kode_mitra=b.kode_mitra "+
