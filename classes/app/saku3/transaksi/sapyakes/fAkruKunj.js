window.app_saku3_transaksi_sapyakes_fAkruKunj = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sapyakes_fAkruKunj.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fAkruKunj";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,20,995,310], childPage:["Data Billing","Detail Jurnal Akru"]});		
		...
		this.sg2 = new saiGrid(this.pc1.childPage[1],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:0,
		            colTitle:["Kode Akun","Nama Akun","DC","Keterangan","Nilai","Jenis","Kode DRK","Nama DRK","Atensi"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[100,150,80,80,100,260,50,250,100]],
...
window.app_saku3_transaksi_sapyakes_fAkruKunj.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fAkruKunj.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into yk_hutang_m(no_hutang,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,tgl_input,nik_user,kode_loktuj,no_app,progress) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.kodepp+"','PDPT','BILL','IDR',1,"+parseNilai(this.e_kredit.getText())+",'"+this.app._userLog+"','"+this.cb_app.getText() +"','F',getdate(),'"+this.app._userLog+"','"+this.app._lokasi+"','"+this.e_nb.getText()+"','0')");
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i) && this.sg2.cells(4,i) != "0"){								                     
								sql.add("insert into yk_hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','"+this.sg2.cells(8,i)+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg2.cells(0,i)+"','"+this.sg2.cells(3,i)+"','"+this.sg2.cells(2,i)+"',"+parseNilai(this.sg2.cells(4,i))+",'"+this.kodepp+"','"+this.sg2.cells(6,i)+"','"+this.app._lokasi+"','PDPT','"+this.sg2.cells(5,i)+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");								
							}
						}
					}			
					var nobukti = "";
					...
					sql.add("insert into glsap(no_bukti,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,kode_curr,kurs,nilai_curr,tgl_input,nik_user,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,no_doksap,kode_rek,no_payment,paymetod) "+
							...
							"(case when jenis in ('PIUKUNJUM','HUTCS','PIUCS') then no_dokumen else '-' end) as kode_cust, "+
							"'-','-',"+
							"'-' as kode_vendor,"+
							"'-','-','-','-','-','E' "+
							"from yk_hutang_j "+
	...
	doJurnal:function(sender){		
		try {
			...
			//kalo campur -(min) sama +(plus) (pdpt dan pdpt rev)			
			var strSQL ="select 'PIU KUNJ '+c.nama as ket,(case b.jenis when 'PEGAWAI' then c.akun_pku else c.akun_pap end) as kode_akun,d.nama as nama_akun,case when sum(a.umum+a.gigi+a.kbia+a.matkes)>0 then 'D' else 'C' end as dc, "+
						"abs(sum(a.umum+a.gigi+a.kbia+a.matkes)) as nilai,'PIUKUNJUM' as jenis, c.drk_kunj as kode_drk,e.nama as nama_drk,b.kode_sap as atensi "+
						...
						"group by  c.nama,case b.jenis when 'PEGAWAI' then c.akun_pku else c.akun_pap end,d.nama,c.drk_kunj,e.nama,b.kode_sap "+												
						"union all "+												
						...
						"abs(sum(a.umum+a.gigi+a.kbia)) as nilai,'PDPTKUNJUM' as jenis, c.drk_kunj as kode_drk,e.nama as nama_drk,'-' as atensi "+
						"from yk_billkunj_d a "+
						"          inner join yk_loker bb on a.loker=bb.loker "+
						"          inner join cust b on bb.kode_cust=b.kode_cust "+
						"		   inner join yk_kunj c on a.kode_produk=c.kode_produk "+
						...
						"abs(sum(a.matkes)) as nilai,'PDPTKUNJUM' as jenis, c.drk_kunj as kode_drk,e.nama as nama_drk,'-' as atensi "+
						"from yk_billkunj_d a "+
						"          inner join yk_loker bb on a.loker=bb.loker "+
						"          inner join cust b on bb.kode_cust=b.kode_cust "+
						"		   inner join yk_kunj c on a.kode_produk=c.kode_produk "+
						...
						"group by  c.nama,c.akun_ob,d.nama,c.drk_kunj,e.nama "+												
						"union all "+
						
						//CS						
						"select 'TTP CS '+d.nama as ket,c.akun_hutcs as kode_akun,d.nama as nama_akun,case when sum(a.cs)>0 then 'D' else 'C' end as dc, "+
						"abs(sum(a.cs)) as nilai,'HUTCS' as jenis, c.drk_kunj as kode_drk,e.nama as nama_drk,b.kode_sap as atensi "+
						"from yk_billkunj_d a "+
						"          inner join yk_loker bb on a.loker=bb.loker "+
						"          inner join cust b on bb.kode_cust=b.kode_cust "+
						"		   inner join yk_kunj c on a.kode_produk=c.kode_produk "+
						...
						"group by  d.nama,c.akun_hutcs,d.nama,c.drk_kunj,e.nama,b.kode_sap "+
						"union all "+						
						"select 'PIU CS '+d.nama as ket,c.akun_piucs as kode_akun,d.nama as nama_akun,case when sum(a.cs)>0 then 'C' else 'D' end as dc, "+
						"abs(sum(a.cs)) as nilai,'PIUCS' as jenis, c.drk_kunj as kode_drk,e.nama as nama_drk,b.kode_sap as atensi "+
						"from yk_billkunj_d a "+
						"          inner join yk_loker bb on a.loker=bb.loker "+
						"          inner join cust b on bb.kode_cust=b.kode_cust "+
						"		   inner join yk_kunj c on a.kode_produk=c.kode_produk "+
						...
						"		   inner join drk e on c.drk_kunj=e.kode_drk and e.kode_lokasi='"+this.app._lokasi+"' and e.tahun=substring(a.periode,1,4) "+
						"where abs(a.cs)<> 0 and b.jenis <> 'PENSIUN' and a.kode_lokasi='"+this.app._lokasi+"' and a.no_bill in ("+nobukti+") "+
						"group by  d.nama,c.akun_piucs,d.nama,c.drk_kunj,e.nama,b.kode_sap "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg2.appendData([line.kode_akun,line.nama_akun,line.dc.toUpperCase(),line.ket.toUpperCase(),floatToNilai(line.nilai),line.jenis.toUpperCase(),line.kode_drk,line.nama_drk,line.atensi]);
				}
			}
			this.sg2.validasi();								
	...
	doLoad4:function(sender){																				
		var strSQL = "select a.no_hutang,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
		             "from yk_hutang_m a "+					 
					 "where a.modul = 'PDPT' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress='0' and a.posted='F' ";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU4 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg4.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.jenis,a.kode_drk,isnull(d.nama,'-') as nama_drk,a.no_dokumen as atensi "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						...
						this.sg2.appendData([line.kode_akun,line.nama_akun,line.dc,line.keterangan,floatToNilai(line.nilai),line.jenis,line.kode_drk,line.nama_drk,line.atensi]);
					}
