window.app_saku3_transaksi_sapyakes_fKaryawan = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sapyakes_fKaryawan.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fKaryawan";
		...
		this.e_vendorIF = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,14,200,20],caption:"Vendor IF SAP", maxLength:10, tag:1});	
...
window.app_saku3_transaksi_sapyakes_fKaryawan.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fKaryawan.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into karyawan(nik,kode_lokasi,nama,alamat,jabatan,no_telp,email,kode_pp,npwp,bank,cabang,no_rek,nama_rek,status,grade,kota,kode_pos,no_hp,flag_aktif, foto,kode_sap) values "+
						    "('"+this.cb_kode.getText()+"','"+this.cb_lokasi.getText()+"','"+this.e_nama.getText()+"','"+this.e_alamat.getText()+"','"+this.e_jabatan.getText()+"','"+this.e_tel.getText()+"','"+this.e_email.getText()+"','"+this.cb_pp.getText()+"','"+this.e_npwp.getText()+"','"+this.e_bank.getText()+"','"+this.e_cabang.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"','"+this.c_jenis.getText()+"','"+this.c_band.getText()+"','"+this.e_kota.getText()+"','"+this.e_pos.getText()+"','"+this.e_hp.getText()+"','"+this.c_status.getText().substr(0,1)+"','"+this.e_foto.getText()+"','"+this.e_vendorIF.getText()+"')");					
	...
	doChange: function(sender){
		try{			
			...
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select nik, kode_lokasi, nama, alamat, jabatan, no_telp, email, kode_pp, npwp, bank, cabang, no_rek, nama_rek, status, grade, kota, kode_pos, no_hp, flag_aktif, foto,kode_sap "+
				             "from karyawan "+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){						
						...
						this.e_vendorIF.setText(line.kode_sap);
