window.app_saku3_transaksi_sapyakes_fPDAju2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sapyakes_fPDAju2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fPDAju2";
		...
		try {
			...
			this.cb_drk.setSQL("select kode_drk, nama from drk where tahun like '"+this.e_periode.getText().substr(0,4)+"%' and kode_lokasi='"+this.app._lokasi+"' union select '-','-' ",["kode_drk","nama"],false,["Kode","Nama"],"and","Data DRK",true);	
...
window.app_saku3_transaksi_sapyakes_fPDAju2.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fPDAju2.implement({		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into yk_spj_m(no_spj,kode_lokasi,kode_loktuj,kode_bidangtuj,modul,progress,periode,tanggal,due_date,kode_pp,no_dokumen,keterangan,tempat,nik_buat,nama_spj,nik_app,nik_perintah,jabatan,akun_uhar,akun_tran,transport,harian,tgl_input,nik_user,kode_drk,   no_app,no_ver,no_spb,    band,nik_bdh,posted,no_appsap,nik_appsap,  form) values "+
							...
							"','"+this.cb_bdh.getText()+"','F','-','"+this.nikAppSAP+"','PUSAT')")							
	...
	doChange:function(sender){
		...
		if (sender == this.cb_nik && this.stsSimpan ==1) {
			...
			if (this.cb_nik.getText() != "") {				
				...
				if (this.cb_nik.getText().substr(0,5) == "99999" || this.cb_nik.getText().substr(0,1) == "L" || this.cb_nik.getText().substr(0,1) == "M") {
					this.e_nama.setReadOnly(false);					
				}
	...
	doLoad3:function(sender){																				
		...
		var strSQL = "select a.no_spj,convert(varchar,a.tanggal,103) as tgl,modul,a.no_dokumen,a.keterangan,a.progress,a.transport+a.harian as nilai "+
					 ...
					 "		and a.posted='F' and a.progress in ('0','A','B') and a.form='PUSAT' "; 	
