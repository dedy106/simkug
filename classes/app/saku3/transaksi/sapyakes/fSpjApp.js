window.app_saku3_transaksi_sapyakes_fSpjApp = function(owner)
{
	if (owner)
	{
		...
		this.cb_appsap = new saiCBBL(this.pc1.childPage[0],{bound:[20,16,220,20],caption:"NIK Post SAP", multiSelection:false, maxLength:10, tag:2});								
		...
		try {
			...
			this.cb_appsap.setSQL("select a.nik, a.nama from karyawan a inner join sap_nik_post b on a.nik=b.nik "+
...
window.app_saku3_transaksi_sapyakes_fSpjApp.implement({	
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						sql.add("update yk_spj_m set nik_appsap='-',no_app='-',progress='0' where progress='A' and no_app = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					for (var i=0;i < this.dataJU5.rs.rows.length;i++){
						...
						if (line.status.toUpperCase() == "APP") {
							sql.add("update yk_spj_m set nik_appsap='"+this.cb_appsap.getText()+"',no_app='"+this.e_nb.getText()+"',progress='A' where no_spj = '"+line.no_spj+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doLoad3:function(sender){																				
		var strSQL = "select a.no_app,convert(varchar,a.tanggal,103) as tgl,a.modul,b.dasar,b.maksud "+
					 ...
					 "             left join (select distinct no_app,kode_lokasi from yk_spj_m where progress='1' and no_spb<>'-' and kode_lokasi='"+this.app._lokasi+"') c on b.no_app=c.no_app and b.kode_lokasi=c.kode_lokasi "+
