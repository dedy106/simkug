window.app_saku3_transaksi_sapyakes_fGantiAju = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sapyakes_fGantiAju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fGantiAju";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penggantian Kuitansi", 0);	
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:10,tag:0,		            
					...
					buttonStyle:[[0],[bsAuto]],colFormat:[[4],[cfNilai]], dblClick:[this,"doDoubleClick"],
...
window.app_saku3_transaksi_sapyakes_fGantiAju.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fGantiAju.implement({	
	...
	doChange:function(sender){		
		if (sender == this.cb_nik && this.cb_nik.getText()!="") {
			...
			var strSQL = "select a.kode_pp,a.nilai-isnull(b.ganti,0)-isnull(e.reimburse,0) as saldo "+ 
						 ...
						 "	      left join (select a.nik_buat,a.kode_lokasi,sum(a.nilai) as reimburse "+
					     "                   from if_reim_m a "+
					     "						  inner join (select no_bukti,kode_lokasi from glsap "+
					     "									  where dc='C' and kode_vendor<>'-' and no_payment='-' and kode_lokasi='"+this.app._lokasi+"' and modul='IFREIM' "+ //--- RFC SAP cek no_payment
					     "									  group by no_bukti,kode_lokasi) b on a.no_reim=b.no_bukti and a.kode_lokasi=b.kode_lokasi "+						 
						 "                   where a.kode_lokasi='"+this.app._lokasi+"' and a.nik_buat='"+this.cb_nik.getText()+"' "+
						 "                   group by a.nik_buat,a.kode_lokasi) e on a.nik=e.nik_buat and a.kode_lokasi=e.kode_lokasi "+						 
	...
	doLoad:function(sender){				
		if (this.cb_nik.getText() != "" && this.e_periode.getText() != "") {						
			var data = this.dbLib.getDataProvider("select 'INPROG' as status,a.no_aju,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.tgl_kuitansi,103) as tglkui,a.kode_akun,a.kode_pp,a.kode_drk,a.keterangan,a.nilai,e.nik+' - '+e.nama as niksub,a.tanggal "+
					   "from if_aju_m a inner join karyawan e on a.user_input=e.nik and a.kode_lokasi=e.kode_lokasi "+	
					   "                inner join glsap x on a.no_aju=x.no_bukti and a.kode_lokasi=x.kode_lokasi and x.no_doksap <>'-' and dc = 'D' "+				   
					   "where a.progress='1' and a.no_app<>'-' and a.no_ganti = '-' and a.no_reim='-' and a.kode_lokasi='"+this.app._lokasi+"' and a.periode<='"+this.e_periode.getText()+"' order by a.tanggal",true);					   
	...
	doDoubleClick: function(sender, col , row) {
		if (this.sg.cells(0,row) == "INPROG") this.sg.cells(0,row,"GANTI");
