window.app_saku3_transaksi_sapyakes_fAppMan = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_sapyakes_fAppMan.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fAppMan";
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:12,tag:0,
		            colTitle:["No Aju","Status","NIK PD","Tgl Mulai","Tgl Selesai","Keg. Bidang","Kegiatan","Tempat","No Approve","Tgl Input","Kode PP","Nik App2"],
...
window.app_saku3_transaksi_sapyakes_fAppMan.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fAppMan.implement({	
		...
		try{														
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.c_status.getText() == "RETURN") var vStatus = "A"; else var vStatus = "1";										
					...
					sql.add("insert into pushmessage(  nik, pesan, status, tgl_input, token, os, appid) values "+
							"('"+this.nik_push+"','"+this.e_memo.getText()+"','0',getdate(),'-','-','SPJ')");
	...
	doSelectDate: function(sender, y,m,d){
		...
		this.cb_nb.setSQL("select no_aju, keterangan from pdss_aju_m where nik_app='"+this.app._userLog+"' and periode<='"+this.e_periode.getText()+"' and progress in ('1','A') ",["no_aju","keterangan"],false,["No Aju","Deskripsi"],"and","Daftar Bukti",true);					
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData([line.no_bukti,line.status.toUpperCase(),line.nik,line.tglawal,line.tglakhir,line.bidang,line.keterangan,line.tempat,line.no_app1,line.tglinput,line.kode_pp,line.nik_app2]); 
		}
