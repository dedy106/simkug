window.app_saku3_transaksi_sapyakes_fSPDev = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sapyakes_fSPDev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fSPDev";
		...
		this.cb_mitra = new saiCBBL(this.pc2.childPage[0],{bound:[20,12,220,20],caption:"Data Mitra", multiSelection:false, maxLength:10, tag:2});						
		this.bTampil = new button(this.pc2.childPage[0],{bound:[650,12,80,18],caption:"Tampil",click:[this,"doTampil"]});			
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,22,995,280], childPage:["Data SP"]});		
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:5,tag:0,				
				colTitle:["Kode Mitra","Nama","Nilai Deviden","Nilai PPH","Nilai KasBank"],
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('SPDEV','SPPPHM','SPPIUGL','DRKSPJ','PPINV') and kode_lokasi = '"+this.app._lokasi+"'",true);			
			if (typeof data == "object"){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];					
					if (line.kode_spro == "SPPIUGL") this.akunPiuGL = line.flag; 																		
					if (line.kode_spro == "PPINV") this.kodepp = line.flag;								
					if (line.kode_spro == "DRKSPJ") this.cb_drk.setText(line.flag);
					if (line.kode_spro == "SPPPHM") this.akunPPh = line.flag;	
					if (line.kode_spro == "SPDEV") this.akunDev = line.flag;	
				}
			}
...
window.app_saku3_transaksi_sapyakes_fSPDev.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fSPDev.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into inv_spdev_m(no_spdev,kode_lokasi,no_dokumen,tanggal,keterangan,kode_pp,kode_drk,kode_mitra,periode,nilai,nik_buat,tgl_input,nik_user,no_kas,progress,no_app1,modul,posted, nik_app) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.kodepp+"','"+this.cb_drk.getText()+"','"+this.cb_mitra.getText()+"','"+this.e_periode.getText()+"',"+nilaiToFloat(this.e_total.getText())+",'"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','-','0','-','SPDEV','F','"+this.cb_app.getText()+"')");								
					...
					for (var i=0;i < this.sg.getRowCount();i++){
						if (this.sg.rowValid(i) && this.sg.cells(2,i) != "0"){															
							sql.add("insert into inv_spdev_d(no_spdev,kode_lokasi,kode_mitra,nilai_kb,nilai_pph,nilai_dev,akun_dev,akun_pph,no_kas,akun_piutang) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.sg.cells(0,i)+"',"+nilaiToFloat(this.sg.cells(4,i))+","+nilaiToFloat(this.sg.cells(3,i))+","+nilaiToFloat(this.sg.cells(2,i))+",'"+this.akunDev+"','"+this.akunPPh+"','-','"+this.akunPiuGL+"')");
							sql.add("insert into inv_spdev_j(no_spdev,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
									"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.akunDev+"','"+this.e_ket.getText()+" - "+this.sg.cells(0,i)+"','C',"+nilaiToFloat(this.sg.cells(2,i))+",'"+this.kodepp+"','"+this.cb_drk.getText()+"','"+this.app._lokasi+"','SPDEV','DEVIDEN','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");	
	...
	doTampil:function(sender){			
		if (this.cb_mitra.getText()!="") {			
			var strSQL = "select a.kode_mitra, a.nama from inv_mitra a where a.kode_mitra='"+this.cb_mitra.getText()+"'";			
			...
			if (typeof data1 == "object" && data1.rs.rows[0] != undefined){
				...
				for (var i in data1.rs.rows){
					...
					this.sg.appendData([line1.kode_mitra,line1.nama,"0","0","0"]);
				}
	...
	doClick:function(sender){
		if (sender == this.i_gen) {			
			...
			setTipeButton(tbSimpan);
			this.stsSimpan = 1;			
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"inv_spdev_m","no_spdev",this.app._lokasi+"-SDEV"+this.e_periode.getText().substr(2,4)+".","0000"));
			this.e_dok.setFocus();
		}	
	},		
	...
	doLoad3:function(sender){														
		var strSQL = "select a.no_spdev,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.progress,b.catatan "+
		             "from inv_spdev_m a "+
		...
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			...
			for (var i=0;i<this.dataJU3.rs.rows.length;i++){
				...
				this.sg3.appendData([line.no_spdev,line.tgl,line.keterangan,line.progress,line.catatan]); 
			}			
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			...
			if (this.sg3.cells(0,baris) != "") {			
				...
				var strSQL = "select b.kode_mitra,b.nama,a.nilai_kb,a.nilai_pph,a.nilai_dev from inv_spdev_d a inner join inv_mitra b on a.kode_mitra=b.kode_mitra where a.no_spdev='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";						
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg.appendData([line.kode_mitra,line.nama,floatToNilai(line.nilai_dev),floatToNilai(line.nilai_pph),floatToNilai(line.nilai_kb)]);
					}
				} else this.sg.clear(1);						
			}
		} catch(e) {alert(e);}
