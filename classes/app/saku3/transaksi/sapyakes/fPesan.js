window.app_saku3_transaksi_sapyakes_fPesan = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sapyakes_fPesan.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fPesan";
		...
		this.cb_drk = new saiCBBL(this.pc1.childPage[0],{bound:[20,13,220,20],caption:"DRK", multiSelection:false, maxLength:10, tag:2});		
...
window.app_saku3_transaksi_sapyakes_fPesan.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fPesan.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						//biar muncul di grid budget app just pengadaan
		...
		if (sender == this.cb_pp || sender == this.cb_akun || sender == this.e_periode) {
			if (this.cb_pp.getText() != "" && this.cb_akun.getText() != "" && this.e_periode.getText() != "") {	
				...
				this.app.services.getSaldo(akun, pp, periode, function(data) {
					self.e_gar.setText(floatToNilai(data));
				});
			}
		}
	},
