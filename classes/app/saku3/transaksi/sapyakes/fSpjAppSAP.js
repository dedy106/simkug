window.app_saku3_transaksi_sapyakes_fSpjApp = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sapyakes_fSpjApp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fSpjApp";
		...
		this.c_status = new saiCB(this,{bound:[20,12,200,20],caption:"Status",items:["APPROVE","RETURN"], readOnly:true,tag:2});
		
		
		this.pc2 = new pageControl(this,{bound:[10,10,1000,400], childPage:["Data Approval","List Approval"]});		
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,320], childPage:["Data Approval","Daftar PD","Detail Transport","Detail U Harian"]});		
		...
		this.sg5 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:0,
...
window.app_saku3_transaksi_sapyakes_fSpjApp.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fSpjApp.implement({	
				...
				try{														
					...
					for (var i=0;i < this.dataJU5.rs.rows.length;i++){
						...
						if (line.status.toUpperCase() == "APP") {
							...
							sql.add("update yk_spj_m set no_app='"+this.e_nb.getText()+"',progress='"+vStatus+"',posted='"+vPosted+"' where no_spj = '"+line.no_spj+"' and kode_lokasi='"+this.app._lokasi+"'");		
						}
					}	
					
					sql.add("update sap_app_m set no_flag='"+this.e_nb.getText()+"' where no_bukti='"+this.nosppd+"' and no_flag='-' and form='APPCAB' and modul='SPPD' and kode_lokasi='"+this.app._lokasi+"'");		
					sql.add("insert into sap_app_m (no_app,kode_lokasi,tanggal,periode,tgl_input,nik_user,status,modul,form,no_bukti,catatan,no_flag,nik_bdh,nik_fiat) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_periode.getText()+"',getdate(),'"+this.app._userLog+"','"+vStatus+"','SPPD','APPCAB','"+this.nosppd+"','-','-','X','X')");
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "hapus" :	
				...
				uses("server_util_arrayList");
				var sql = new server_util_arrayList();
				sql.add("update yk_spj_m set no_app='-',progress='0',posted='F' where no_app = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
				sql.add("delete from yk_spj_app where no_app = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doLoad:function(sender){		
		...
		var strSQL = "select 'INPROG' as status,a.no_spj,a.nama_spj,convert(varchar,a.due_date,103) as tgl,a.keterangan,a.transport+a.harian as nilai,b.kode_akun+' - '+b.nama as akun, c.kode_pp+' - '+c.nama as pp,isnull(d.kode_drk,'-')+' - '+isnull(d.nama,'-') as drk "+
					 ...
					 "where "+nikApp+" a.progress='0' and a.kode_lokasi='"+this.app._lokasi+"' and a.posted='F'";				
	    			...
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								...
								//posting SAP
								if (this.c_status.getText() == "APPROVE") this.app.services.postSAP(this.nosppd,"SPPD", function(data){ alert(data); });
	...
	doLoad3:function(sender){																				
		var strSQL = "select a.no_app,convert(varchar,a.tanggal,103) as tgl,a.modul,b.dasar,b.maksud "+
		             "from sap_app_m a inner join yk_spj_app b on a.no_app=b.no_app and a.kode_lokasi=b.kode_lokasi "+					 					 
					 "             left join (select distinct no_app,kode_lokasi from yk_spj_m where posted='T' and kode_lokasi='"+this.app._lokasi+"') c on b.no_app=c.no_app and b.kode_lokasi=c.kode_lokasi "+
