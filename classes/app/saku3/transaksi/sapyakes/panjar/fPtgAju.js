window.app_saku3_transaksi_sapyakes_panjar_fPtgAju = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sapyakes_panjar_fPtgAju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_panjar_fPtgAju";
		...
		this.sg3 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-30],colCount:8,tag:9,
		            colTitle:["No Pertggn","Tanggal","Jenis","No Dokumen","Deskripsi","No Panjar","Pemegang","Nilai"],
					colWidth:[[7,6,5,4,3,2,1,0],[100,200,100,210,100,80,80,100]],
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,10,990,347], childPage:["Data Panjar","Item Mata Anggaran","Budget Pengajuan","Cat. Approval"]});
		...
		this.sg2 = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:4,tag:9,
		            colTitle:["Kode Akun","Kode PP","Nilai Aju","Nilai Ptg"],
					colWidth:[[3,2,1,0],[100,100,100,100]],
					readOnly:true,colFormat:[[2,3],[cfNilai,cfNilai]],autoAppend:true,defaultRow:1});
...
window.app_saku3_transaksi_sapyakes_panjar_fPtgAju.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_panjar_fPtgAju.implement({
				...
				try{														
					...
					if (this.stsSimpan == 0) {						
						...
						var strSQL = "select kode_akun,kode_pp,periode, sum(nilai) as nilai "+
									"from panjarptg2_j "+
									"where no_ptg='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and dc = 'D' and jenis='BEBAN' "+
					...
					//hapus dulu data keep budget saat aju panjar (jika ada sebelumnya)
					sql.add("delete from glsap where modul='PJPTG' and jenis='HAPUSKEEP' and no_bukti ='"+this.cb_pj.getText()+"-X' and kode_lokasi='"+this.app._lokasi+"'");
						
					if (this.stsSimpan == 1) {
						//balikin budget keep saat panjar
						var strSQL = "select kode_akun,kode_pp,periode, sum(nilai) as nilai "+
									 "from panjar2_d "+
									 "where no_panjar='"+this.cb_pj.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and dc = 'D' and jenis='BEBAN' "+
									 "group by kode_akun,kode_pp,periode";
						var databudget = this.dbLib.getDataProvider(strSQL,true);
						if (typeof databudget == "object" && databudget.rs.rows[0] != undefined){
							var data = databudget.rs.rows;
					...
					sql.add("insert into glsap(no_bukti,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,kode_curr,kurs,nilai_curr,tgl_input,nik_user,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,no_doksap,kode_rek,no_payment,paymetod) "+
							"select no_ptg,no_urut,kode_lokasi,'PJPTG',jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,'IDR',1,nilai,tgl_input,nik_user,'-','-','-','-','-','-','NOPOSTED','-','-','E' "+
							"from panjarptg2_j where jenis='BEBAN' and no_ptg='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
				...
				if (this.flagGarFree == "0") {
					for (var i=0;i < this.sg2.getRowCount();i++){
						if (nilaiToFloat(this.sg2.cells(2,i)) < nilaiToFloat(this.sg2.cells(3,i))) {
							var k =i+1;
							system.alert(this,"Transaksi tidak valid.","Nilai Pertanggungan melebihi Keep Budget Pengajuan. [Baris : "+k+"]");
							return false;						
						}
					}
				}																				
			...
			case "hapus" :	
				...
				//hapus dulu data keep budget saat aju panjar (sebelumnya - jika ada)
				sql.add("delete from glsap where modul='PJPTG' and jenis='HAPUSKEEP' and no_bukti ='"+this.cb_pj.getText()+"-X' and kode_lokasi='"+this.app._lokasi+"'");
				...
				//insert untuk keep budget pengajuan panjar yg telah terhaps sebelumnya (balikin lagi keepnya)
				sql.add("insert into glsap(no_bukti,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,kode_curr,kurs,nilai_curr,tgl_input,nik_user,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,no_doksap,kode_rek,no_payment,paymetod) "+
						"select no_panjar+'-X',no_urut,kode_lokasi,'PJPTG','HAPUSKEEP',no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,'IDR',1,nilai,tgl_input,nik_user,'-','-','-','-','-','-','HAPUSKEEP','-','-','E' "+
						"from panjar2_d where jenis='BEBAN' and no_panjar='"+this.cb_pj.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
		...
		if (sender == this.e_periode || sender == this.cb_pp) {
			if (this.e_periode.getText()!="" && this.cb_pp.getText()!="" && this.stsSimpan == 1) {				
				this.cb_pj.setSQL("select a.no_panjar, a.keterangan from panjar2_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
				                  "where a.progress = '2' and a.modul='PJ2' and b.kode_bidang='"+this.app._kodeBidang+"' and  a.no_kas<>'-' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",["no_panjar","keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true); 				
			}
		}
		if (sender == this.cb_pj && this.cb_pj.getText()!="" && this.stsSimpan == 1) {
			var sql = new server_util_arrayList();
			sql.add("select distinct a.kode_akun,a.nama from masakun a inner join panjar2_d b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.no_panjar='"+this.cb_pj.getText()+"' and b.jenis='BEBAN' where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'");
			sql.add("select distinct a.kode_pp,a.nama from pp a inner join panjar2_d b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.no_panjar='"+this.cb_pj.getText()+"' where a.kode_bidang='"+this.app._kodeBidang+"' and a.kode_lokasi = '"+this.app._lokasi+"' ");
			...
			var data = this.dbLib.getDataProvider(
						"select a.kode_akun,a.kode_pp,sum(a.nilai) as aju "+
						"from panjar2_d a "+
						"where a.jenis='BEBAN' and a.no_panjar = '"+this.cb_pj.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' "+
						"group by a.kode_akun,a.kode_pp order by a.kode_akun,a.kode_pp",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg2.clear();
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg2.appendData([line.kode_akun,line.kode_pp,floatToNilai(line.aju),"0"]);
				}
			...
			if (sender == this.sg) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Akun",sender,undefined, 
							...
							"select count(*) from masakun a inner join panjar2_d b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.no_panjar='"+this.cb_pj.getText()+"' where a.block= '0' and a.kode_lokasi = '"+this.app._lokasi+"'",
							["a.kode_akun","a.nama"],"and",["Kode","Nama"],false);				
				}
				if (col == 5){
					this.standarLib.showListData(this, "Daftar PP/Unit",sender,undefined, 
							"select distinct a.kode_pp, a.nama  from pp a inner join panjar2_d b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.no_panjar='"+this.cb_pj.getText()+"' and b.kode_akun='"+sender.cells(0,row)+"' where a.kode_lokasi = '"+this.app._lokasi+"' and  a.tipe='posting' and a.flag_aktif ='1'",
	...
	doHitungGar: function(){
		...
		for (var i=0;i < this.sg.getRowCount();i++){
			if (this.sg.rowValid(i) && this.sg.cells(5,i) != "-" && this.sg.cells(7,i)!= "-"){
				...
				var idx = total = 0;
				for (var j=0;j < this.sg2.getRowCount();j++){
					if (this.sg.cells(0,i) == this.sg2.cells(0,j) && this.sg.cells(5,i) == this.sg2.cells(1,j)) {
						isAda = true;
						idx = j;
						break;
					}
	    			...
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){	
							if (this.preView == "1") {						
								...
								//balikin budget aju panjar
								var noBukti = this.cb_pj.getText()+"-X";
								this.app.services.keepBudget(noBukti, function(data){ alert(data); });
	...
	doLoad3:function(sender){																				
		var strSQL = "select c.no_ptg,a.no_panjar,convert(varchar,c.tanggal,103) as tgl,case c.progress when '0' then 'AJU' when 'R' then 'REVISI' when 'V' then 'REV.VER' end as jenis,c.no_dokumen,c.keterangan,a.nik_buat+' - '+b.nama as nama,c.nilai "+
					 ...
					 "where c.periode<='"+this.e_periode.getText()+"' and c.kode_lokasi='"+this.app._lokasi+"' and c.progress in ('0','R','V') and d.kode_bidang='"+this.app._kodeBidang+"'";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_ptg,line.tgl,line.jenis.toUpperCase(),line.no_dokumen,line.keterangan,line.no_panjar,line.nama,floatToNilai(line.nilai)]); 
		}
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){													
						...
						if (this.progSeb == "V") var modulApp = "PJPTG_SPB";
						else var modulApp = "PJPTG_APP"; 	
