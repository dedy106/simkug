window.app_saku3_transaksi_sapyakes_fRDSPI = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_sapyakes_fRDSPI.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fRDSPI";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,22,995,328], childPage:["Data Reksadana","Rekap Data"]});		
...
window.app_saku3_transaksi_sapyakes_fRDSPI.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fRDSPI.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into inv_rdspi_m(no_spi,no_dokumen,tanggal,keterangan,kode_curr,kurs,nilai,kode_pp,kode_drk,posted,modul,nik_buat,nik_setuju,kode_lokasi,periode,no_del,no_link,nik_user,tgl_input,jenis,no_spikug,no_app1,progress) values "+
						    "('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.kodepp+"','"+this.cb_drk.getText()+"','F','RDSPI','"+this.app._userLog+"','"+this.cb_app.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','-','-','"+this.app._userLog+"',getdate(),'"+this.c_jenis.getText()+"','-','-','0')");
					...
					if (this.e_periode.getText().substr(4,2) != "01") {
						...
						sql.add("insert into inv_rdspi_d(no_spi,kode_lokasi,kode_rdkelola,kode_rd,jumlah,h_oleh,h_buku,h_wajar,flag_rev,dc)	"+				
								"select '"+this.e_nb.getText()+"','"+this.app._lokasi+"',a.kode_rdkelola,a.kode_rd,a.jumlah,a.h_oleh,a.h_buku,a.h_wajar,a.no_spi,'C' "+
								"from inv_rdspi_d a inner join inv_rdspi_m b on a.no_spi=b.no_spi and b.jenis = 'NBUKU' "+
								"where a.dc = 'D' and a.no_spi = (select MAX(no_spi) from inv_rdspi_m where jenis='NBUKU') ");
					}
