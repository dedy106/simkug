window.app_saku3_transaksi_sapyakes_fAjuDakem = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sapyakes_fAjuDakem.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fAjuDakem";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Klaim DAKEM", 0);	
		...
		this.cb_app = new portalui_saiCBBL(this.pc1.childPage[0],{bound:[20,10,220,20],caption:"NIK Post SAP",multiSelection:false,tag:2});		
		...
		try {
			...
			this.akunDakem = "-"; 
			var data = this.dbLib.getDataProvider("select kode_spro,flag,keterangan from spro where kode_spro in ('SAPDAKEM','SAPHDAKEM') ",true);		
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag,keterangan from spro where kode_spro in ('VENDORDKM') and kode_lokasi='"+this.app._lokasi+"'",true);	
			if (typeof data == "object"){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];																							
					if (line.kode_spro == "VENDORDKM") {
						this.vendorDKM = line.flag;
						this.rekDKM = line.keterangan;	
					}						
				}
			}
...
window.app_saku3_transaksi_sapyakes_fAjuDakem.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fAjuDakem.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from yk_dakem_j where no_dakem = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");												
						sql.add("delete from glsap where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");												
					}					
					sql.add("insert into yk_dakem_m(no_dakem,kode_lokasi,periode,nik_user,tgl_input,tanggal,no_dokumen,keterangan,nik_buat,nik,aw,status_aw,tgl_transfer,jenis_bayar, bank,cabang,no_rek,nama_rek,alamat, akun_dakem,no_app,no_ver,no_spb,progress,nik_app,kode_pp,posted) values "+
					...
					sql.add("insert into yk_dakem_j(no_dakem,no_ref,kode_lokasi,tanggal,nu,periode,kode_akun,kode_pp,kode_drk,dc,keterangan,nilai,jenis) values "+
							"('"+this.e_nb.getText()+"','-','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"',0,'"+this.e_periode.getText()+"','"+this.akunDakem+"','"+this.app._kodePP+"','-','D','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",'DAKEM')");					
					sql.add("insert into yk_dakem_j(no_dakem,no_ref,kode_lokasi,tanggal,nu,periode,kode_akun,kode_pp,kode_drk,dc,keterangan,nilai,jenis) values "+
							"('"+this.e_nb.getText()+"','-','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"',1,'"+this.e_periode.getText()+"','"+this.akunHutDakem+"','"+this.app._kodePP+"','-','C','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",'HUTDAKEM')");
					...
					sql.add("insert into glsap(no_bukti,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,kode_curr,kurs,nilai_curr,tgl_input,nik_user,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,no_doksap,kode_rek,no_payment,paymetod) "+
							"select no_dakem,nu,kode_lokasi,'DAKEM',jenis,no_ref,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,'IDR',1,nilai,getdate(),'"+this.app._userLog+"','-','-','-','-','-','-','-','-','-','-' "+
						    "from yk_dakem_j "+
							"where dc = 'D' and nilai <> 0 and kode_lokasi='"+this.app._lokasi+"' and no_dakem='"+this.e_nb.getText()+"'");
					
					sql.add("insert into glsap(no_bukti,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,kode_curr,kurs,nilai_curr,tgl_input,nik_user,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,no_doksap,kode_rek,no_payment,paymetod) "+
							"select no_dakem,nu,kode_lokasi,'DAKEM',jenis,no_ref,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,'IDR',1,nilai,getdate(),'"+this.app._userLog+"','-','-','-','"+this.vendorDKM+"','-','-','-','"+this.rekDKM+"','-','T' "+
						    "from yk_dakem_j "+
							"where dc = 'C' and nilai <> 0 and kode_lokasi='"+this.app._lokasi+"' and no_dakem='"+this.e_nb.getText()+"'");
