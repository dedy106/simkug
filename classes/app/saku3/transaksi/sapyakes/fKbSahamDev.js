window.app_saku3_transaksi_sapyakes_fKbSahamDev = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sapyakes_fKbSahamDev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fKbSahamDev";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Saham Deviden", 0);	
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:8,tag:9,				
				...
				colWidth:[[7,6,5,4,3,2,1,0],[80,100,100,150,250,80,100,60]],				
...
window.app_saku3_transaksi_sapyakes_fKbSahamDev.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fKbSahamDev.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBSHMDEV','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.cb_app.getText()+"',getdate(),'"+this.app._userLog+"','F','-','-','-','0')");										
	...
	doLoad:function(sender){
		if (this.e_periode.getText() != "") {
			var strSQL = "select a.no_shmdev,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,a.kode_saham,b.kode_kelola,b.nilai_kb,b.akun_piutang "+
						 ...
						 "where a.progress='1' and b.no_kas='-' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";						
		...
		try{
			...
			if (sender == this.cb_rek && this.cb_rek.getText()!="") {
				...
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){						
						this.cb_akun.getText() = line.kode_akun;						
					} 
				}					
			}
			*/
