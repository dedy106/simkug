window.app_saku3_transaksi_sapyakes_fDokDakem = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sapyakes_fDokDakem.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fDokDakem";
		...
		try {
			...
			this.e_nb.setSQL("select a.no_dakem, a.nik+'-'+a.keterangan as keterangan from yk_dakem_m a inner join yk_dakem_d b on a.no_dakem=b.kdtrans and a.kode_lokasi=b.kode_lokasi where a.progress in ('0','U') and a.kode_lokasi='"+this.app._lokasi+"'",["a.no_dakem","a.keterangan"],false,["No Bukti","Deskripsi"],"and","Daftar Bukti",true);			
...
window.app_saku3_transaksi_sapyakes_fDokDakem.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fDokDakem.implement({	
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					//sql.add("update yk_dakem_m set progress='U' where no_dakem='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					setTipeButton(tbAllFalse);					
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "hapus" :	
				...
				//sql.add("update yk_dakem_m set progress='0' where no_dakem='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
				setTipeButton(tbAllFalse);	
				this.dbLib.execArraySQL(sql);				
				break;					
		}
