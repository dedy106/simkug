window.app_saku3_transaksi_sapyakes_fSahamDev = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sapyakes_fSahamDev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fSahamDev";
		...
		this.cb_app = new saiCBBL(this.pc2.childPage[0],{bound:[20,12,222,20],caption:"NIK Post SAP", multiSelection:false, maxLength:10, tag:2,visible:false});								
...
window.app_saku3_transaksi_sapyakes_fSahamDev.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fSahamDev.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into inv_shmdev_m(no_shmdev,kode_lokasi,no_dokumen,tanggal,keterangan,kode_pp,kode_drk,kode_saham,periode,nilai,nik_buat,tgl_input,nik_user,progress,no_app1,modul,posted, nik_app) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.kodepp+"','"+this.cb_drk.getText()+"','"+this.cb_saham.getText()+"','"+this.e_periode.getText()+"',"+nilaiToFloat(this.e_total.getText())+",'"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','0','-','SHMDEV','F', '"+this.cb_app.getText()+"')");								
