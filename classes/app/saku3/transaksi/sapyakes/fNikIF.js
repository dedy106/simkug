window.app_saku3_transaksi_sapyakes_fNikIF = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sapyakes_fNikIF.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sapyakes_fNikIF";
		...
		this.e_vendor = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,15,200,20],caption:"Kode Vendor SAP", maxLength:20, readOnly:true,tag:2});			
		this.e_reksap = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,16,200,20],caption:"Rekening SAP", maxLength:20, readOnly:true,tag:2});			
...
window.app_saku3_transaksi_sapyakes_fNikIF.extend(window.childForm);
window.app_saku3_transaksi_sapyakes_fNikIF.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into if_m(nik,kode_lokasi,kode_pp,nilai,no_dokumen,keterangan,flag_aktif,kode_rek,kode_vendor,rek_sap) values "+
						    "('"+this.cb_nik.getText()+"','"+this.app._lokasi+"','"+this.kodePP+"',"+nilaiToFloat(this.e_nilai.getText())+",'"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.c_status.getText().substr(0,1)+"','"+this.cb_rek.getText()+"','"+this.e_vendor.getText()+"','"+this.e_reksap.getText()+"')");
					setTipeButton(tbAllFalse);
	...
	doChange: function(sender){
		try{
			if (this.cb_nik.getText() != ""){
				var strSQL = "select a.rek_sap,a.kode_rek,a.flag_aktif,a.nik,b.kode_pp,b.kode_pp+' - '+b.nama as pp,a.nilai,a.no_dokumen,a.keterangan,a.kode_vendor from if_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi where a.nik ='"+this.cb_nik.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";						   
	...
	doLoad:function(sender){								
		var strSQL = "select a.nik,a.nama,a.kode_pp+' - '+b.nama as pp,c.no_dokumen,c.keterangan,c.nilai "+
					 ...
					 "where c.flag_aktif ='1' and c.kode_lokasi='"+this.app._lokasi+"' order by a.nik";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
