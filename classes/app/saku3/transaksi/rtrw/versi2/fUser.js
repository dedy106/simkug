window.app_saku3_transaksi_rtrw_versi2_fUser = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_rtrw_versi2_fUser.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_rtrw_versi2_fUser";
		...
		this.cb_rumah = new saiCBBL(this,{bound:[20,15,220,20],caption:"Rumah", multiSelection:false, maxLength:10, tag:2});				
		...
		try {
			...
			this.cb_rumah.setSQL("select kode_rumah, alamat from rt_rumah where kode_lokasi='"+this.app._lokasi+"'",["kode_rumah","alamat"],false,["Kode","Nama"],"and","Data Rumah",true);		
		}catch(e){
			systemAPI.alert(e);
		}
	}
};
window.app_saku3_transaksi_rtrw_versi2_fUser.extend(window.childForm);
window.app_saku3_transaksi_rtrw_versi2_fUser.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0])){
				try{								
					...
					sql.add("insert into hakakses (nik, nama, pass, status_admin, kode_lokasi, kode_klp_menu, klp_akses,kode_rumah) values  "+
						    "('"+this.cb_kode.getText()+"','"+this.cb_kode.rightLabelCaption+"','"+this.e_pass.getText()+"','"+sts_admin+"','"+this.kodeLokasi+"','"+this.c_menu.getText()+"','ADMIN','"+this.cb_rumah.getText()+"')");					
					setTipeButton(tbAllFalse);
	...
	doChange: function(sender){
		try{			
			if (this.cb_kode.getText() != ""){				
				var data = this.dbLib.getDataProvider("select a.status_admin,a.kode_klp_menu,a.status_admin,a.pass,a.kode_lokasi,a.kode_rumah "+
						   "from hakakses a "+
