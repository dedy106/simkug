window.app_saku3_transaksi_rtrw_versi2_fWargaOut = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_rtrw_versi2_fWargaOut.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_rtrw_versi2_fWargaOut";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Warga Keluar", 0);	
		
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		uses("saiGrid",true);		
		
		this.pc1 = new pageControl(this,{bound:[20,12,1000,480], childPage:["Data Keluar","Daftar Keluar","Filter Cari"]});
		...
		this.l_tgl1 = new portalui_label(this.pc1.childPage[0],{bound:[20,13,100,18],caption:"Tgl Keluar", underline:true});
		this.dp_d1 = new portalui_datePicker(this.pc1.childPage[0],{bound:[120,13,100,18]}); 								
		this.c_stsout = new saiCB(this.pc1.childPage[0],{bound:[20,29,200,20],caption:"Status Masuk",items:["PINDAH","MENINGGAL"], readOnly:true,tag:2});
...
window.app_saku3_transaksi_rtrw_versi2_fWargaOut.extend(window.childForm);
window.app_saku3_transaksi_rtrw_versi2_fWargaOut.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					var nb = this.standarLib.noBuktiOtomatis(this.dbLib,"rt_warga_out","no_keluar",this.app._lokasi+"-OT"+this.tahun+".","0000");						
					
					sql.add("insert into rt_warga_out (no_keluar,tanggal,kode_lokasi,kode_rumah,nik,sts_keluar,tgl_input,nik_user) values "+
						    "('"+nb+"','"+this.dp_d1.getDateString()+"','"+this.app._lokasi+"','"+this.cb_rumah.getText()+"','"+this.cb_nik.getText()+"','"+this.c_stsout.getText()+"',getdate(),'"+this.app._userLog+"')");					
					sql.add("update rt_warga_in set no_keluar='"+nb+"' where no_masuk='"+this.noIN+"' and kode_lokasi='"+this.app._lokasi+"'");
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from rt_warga_out where no_keluar='"+this.noOUT+"' ");	
					sql.add("update rt_warga_in set no_keluar='-' where no_masuk='"+this.noIN+"' and kode_lokasi='"+this.app._lokasi+"'");

					sql.add("insert into rt_warga_out (no_keluar,tanggal,kode_lokasi,kode_rumah,nik,sts_keluar,tgl_input,nik_user) values "+
						    "('"+this.noOUT+"','"+this.dp_d1.getDateString()+"','"+this.app._lokasi+"','"+this.cb_rumah.getText()+"','"+this.cb_nik.getText()+"','"+this.c_stsout.getText()+"',getdate(),'"+this.app._userLog+"')");					
					sql.add("update rt_warga_in set no_keluar='"+this.noOUT+"' where no_masuk='"+this.noIN+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_rumah && this.cb_rumah.getText() != ""){
				...
				if (this.stsSimpan == 1) {
					this.cb_nik.setSQL("select a.nik,a.nama "+
								   	   "from rt_warga a inner join rt_warga_in b on a.nik=b.nik and b.kode_rumah='"+this.cb_rumah.getText()+
								   	   "' and b.kode_lokasi='"+this.app._lokasi+"' and b.no_keluar='-'",["nik","nama"],false,["NIK","Nama"],"where","Data Warga",true);			
				}
			}

			if (sender == this.cb_nik && this.cb_nik.getText() != ""){
				if ( this.stsSimpan==1) var filter = "and b.no_keluar='-' ";
				else var filter = " ";
				var strSQL = "select a.sts_keluarga,b.no_masuk "+
							 "from rt_warga a inner join rt_warga_in b on a.nik=b.nik and b.kode_rumah='"+this.cb_rumah.getText()+
							 "' and b.kode_lokasi='"+this.app._lokasi+"' "+filter;
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line = data.rs.rows[0];												
					this.e_status.setText(line.sts_keluarga);	
					this.noIN = line.no_masuk;
				}
			}					
		}catch(e){
			systemAPI.alert(e);
	...
	doCari:function(sender){								
		try {
			if (this.e_kode2.getText() != "") var filter = " b.nik like '%"+this.e_kode2.getText()+"%' and ";
				...
				var strSQL = "select b.nik,b.nama,a.kode_rumah,a.no_keluar,a.no_keluar "+
						 "from rt_warga_out a "+
	...
	doLoad:function(sender){								
		try {
			var strSQL = "select b.nik,b.nama,a.kode_rumah,xx.no_masuk,a.no_keluar "+
						 "from rt_warga_out a "+
						 "inner join rt_warga_in xx on xx.no_keluar=a.no_keluar "+
						 "inner join rt_warga b on a.nik=b.nik "+
						 "inner join rt_rumah c on a.kode_rumah=c.kode_rumah and a.kode_lokasi=c.kode_lokasi "+
						 "inner join karyawan_pp d on c.rt=d.kode_pp and c.kode_lokasi=d.kode_lokasi and d.nik='"+this.app._userLog+"' "+
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {			
				...
				this.noOUT = this.sg1.cells(4,row);		

				var strSQL = "select * from rt_warga_out where no_keluar='"+this.noOUT+"'";				
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line = data.rs.rows[0];												
					...
					this.cb_nik.setSQL("select a.nik,a.nama "+
								   	   "from rt_warga a inner join rt_warga_out b on a.nik=b.nik and b.no_keluar='"+this.noOUT+"' and b.kode_lokasi='"+this.app._lokasi+"' ",["nik","nama"],false,["NIK","Nama"],"where","Data Warga",true);			
					this.cb_nik.setText(line.nik);

					this.dp_d1.setText(line.tanggal);
					this.c_stsout.setText(line.sts_keluar);					
