window.app_saku3_transaksi_rtrw_fPosting = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_rtrw_fPosting.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_rtrw_fPosting";
		...
		this.c_modul = new saiCB(this.pc1.childPage[3],{bound:[20,11,202,20],caption:"Modul",items:["JU","KB","RT"], readOnly:true,tag:9,change:[this,"doChange"]});
		...
		try {
			...
			this.sg1.appendData(["TRUE","RT","RT-RW"]);			
...
window.app_saku3_transaksi_rtrw_fPosting.extend(window.childForm);
window.app_saku3_transaksi_rtrw_fPosting.implement({
		...
		switch (event){
			...
			case "simpan" :	
				...
				var strSQL = "select no_bukti+' - '+periode as bukper from ("+
						 ...
						 "from rt_bill_j a inner join rt_bill_m b on a.no_bill=b.no_bill and a.kode_lokasi=b.kode_lokasi and b.posted='F' "+
	...
	doLoad:function(sender){
		...
		for (var i=0;i < this.sg1.getRowCount();i++){
			if (this.sg1.rowValid(i) && this.sg1.cells(0,i)=="TRUE") {
				...
				if (this.sg1.cells(1,i) == "RT") 
					strSQL += "union all "+
							  "select 'INPROG' as status,no_bill as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'RT' as jenis "+
							  "from rt_bill_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";				
			}						
		}		
	...
	doChange:function(sender){
		...
		if (sender == this.c_modul && this.c_modul.getText()!="") {
			...
			if (this.c_modul.getText()=="RT") this.cb_bukti.setSQL("select no_bill as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'RT' as jenis from rt_bill_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_bukti","no_dokumen","tanggal","keterangan","jenis"],false,["No Bukti","No Dokumen","Tanggal","Keterangan","jenis"],"and","Data Bukti RT",true);			
		}		
		...
		if (this.sg.cells(1,row) != "" && this.sg.cells(5,row) != "") {
			...
			switch(this.sg.cells(5,row)){
				...
				case "KB" :
					...
					break;								
				case "RT" :
							strSQL = "select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,'-' as kode_drk,'-' as nama_drk,'-' as kode_cf,'-' as nama_cf "+
									 "from rt_bill_j a inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
									 "             inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+									 
