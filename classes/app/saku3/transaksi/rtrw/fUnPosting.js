window.app_saku3_transaksi_rtrw_fUnPosting = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_rtrw_fUnPosting.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_rtrw_fUnPosting";
...
window.app_saku3_transaksi_rtrw_fUnPosting.extend(window.childForm);
window.app_saku3_transaksi_rtrw_fUnPosting.implement({
			...
			if (this.sg1.rowValid(i) && this.sg1.cells(0,i)=="TRUE") {
				...
				if (this.sg1.cells(1,i) == "RT") 
					strSQL += "union all "+
							  ...
							  "from rt_bill_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";				
			}						
		}		
	...
	doChange:function(sender){
		...
		if (sender == this.c_modul && this.c_modul.getText()!="") {
			...
			if (this.c_modul.getText()=="RT") this.cb_bukti.setSQL("select no_bill as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'RT' as jenis from rt_bill_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_bukti","no_dokumen","tanggal","keterangan","jenis"],false,["No Bukti","No Dokumen","Tanggal","Keterangan","jenis"],"and","Data Bukti RT",true);			
		}	
