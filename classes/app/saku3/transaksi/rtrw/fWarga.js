window.app_saku3_transaksi_rtrw_fWarga = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_rtrw_fWarga.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_rtrw_fWarga";
		...
		this.e_tempat = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,12,500,20],caption:"Tempat Lahir", maxLength:150, tag:1});							
		this.l_tgl1 = new portalui_label(this.pc1.childPage[0],{bound:[20,13,100,18],caption:"Tgl Lahir", underline:true});
...
window.app_saku3_transaksi_rtrw_fWarga.extend(window.childForm);
window.app_saku3_transaksi_rtrw_fWarga.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into kop_agg(no_agg,kode_lokasi,nama,tempat,tgl_lahir,alamat,no_tel,bank,cabang,no_rek,nama_rek,flag_aktif,id_lain) values "+
						    "('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.e_tempat.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_alamat.getText()+"','"+this.e_tel.getText()+"','"+this.e_bank.getText()+"','"+this.e_cabang.getText()+"','"+this.e_norek.getText()+"','"+this.e_namarek.getText()+"','"+this.c_flag.getText().substr(0,1)+"','"+this.e_id.getText()+"')");
					setTipeButton(tbAllFalse);
