window.app_saku3_transaksi_gl_fUnPostingTarbak = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_gl_fUnPostingTarbak.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_gl_fUnPostingTarbak";
		...
		this.c_modul = new saiCB(this.pc1.childPage[3],{bound:[20,11,202,20],caption:"Modul",items:["JU","KB","AP","TAK","FA","SI"], readOnly:true,tag:9,change:[this,"doChange"]});
...
window.app_saku3_transaksi_gl_fUnPostingTarbak.extend(window.childForm);
window.app_saku3_transaksi_gl_fUnPostingTarbak.implement({
			...
			if (this.sg1.rowValid(i) && this.sg1.cells(0,i)=="TRUE") {
				...
				if (this.sg1.cells(1,i) == "TAK") 
					strSQL += "union all "+
							  ...
							  "from takkirim_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+strPP+
							  "union all "+
							  "select 'INPROG' as status,no_terima as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'TT' as jenis "+
							  "from takterima_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+strPP;							  
				...
				if (this.sg1.cells(1,i) == "SI") 
					strSQL += "union all "+
							  ...
							  "from sis_bill_m "+
							  "where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'"+strPP +
							  "union "+
	...
	doChange:function(sender){
		...
		if (sender == this.c_modul && this.c_modul.getText()!="") {
			...
			if (this.c_modul.getText()=="TAK") this.cb_bukti.setSQL("select no_kirim as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'TK' as jenis from takkirim_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+strPP+
																"union all "+
																"select no_terima as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'TT' as jenis from takterima_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+strPP,["no_bukti","no_dokumen","tanggal","keterangan","jenis"],false,["No Bukti","No Dokumen","Tanggal","Keterangan","jenis"],"and","Data Bukti TAK",true);			
			if (this.c_modul.getText()=="AP") this.cb_bukti.setSQL("select no_hutang as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'HU' as jenis from kas_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'"+strPP,["no_bukti","no_dokumen","tanggal","keterangan","jenis"],false,["No Bukti","No Dokumen","Tanggal","Keterangan","jenis"],"and","Data Bukti",true);			
			...
			if (this.c_modul.getText()=="SI") 
				this.cb_bukti.setSQL("select no_bill as no_bukti,'-' as no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'BILL' as jenis from sis_bill_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
									 "union "+
									 "select no_rekon as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'RKN' as jenis from sis_rekon_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "
