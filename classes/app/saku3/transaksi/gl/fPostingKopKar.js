window.app_saku3_transaksi_gl_fPostingKopKar = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_gl_fPostingKopKar.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_gl_fPostingKopKar";
		...
		this.c_modul = new saiCB(this.pc1.childPage[3],{bound:[20,11,202,20],caption:"Modul",items:["JU","KB","SP","AR","FA"], readOnly:true,tag:9,change:[this,"doChange"]});
		...
		try {
			...
			this.sg1.appendData(["TRUE","SP","Simpan Pinjam"]);	
...
window.app_saku3_transaksi_gl_fPostingKopKar.extend(window.childForm);
window.app_saku3_transaksi_gl_fPostingKopKar.implement({
		...
		switch (event){
			...
			case "simpan" :	
				...
				var strSQL = "select no_bukti+' - '+periode as bukper from ("+
						 ...
						 "from kop_simpbill_j a inner join kop_simpbill_m b on a.no_bill=b.no_bill and a.kode_lokasi=b.kode_lokasi and b.posted='F' "+
						 "where a.no_bill in ("+this.nobukti+") and a.kode_lokasi='"+this.app._lokasi+"' group by a.no_bill,a.periode "+						 
						
						 "union "+
						 "select a.no_angs as no_bukti,a.periode,sum(case a.dc when 'D' then a.nilai else -a.nilai end) as total  "+
						 "from kop_simpangs_j a inner join kop_simpangs_m b on a.no_angs=b.no_angs and a.kode_lokasi=b.kode_lokasi and b.posted='F' "+
						 "where a.no_angs in ("+this.nobukti+") and a.kode_lokasi='"+this.app._lokasi+"' group by a.no_angs,a.periode "+						 
						
						 "union "+
						 "select a.no_bill as no_bukti,a.periode,sum(case a.dc when 'D' then a.nilai else -a.nilai end) as total  "+
						 "from kop_pinjbill_j a inner join kop_pinjbill_m b on a.no_bill=b.no_bill and a.kode_lokasi=b.kode_lokasi and b.posted='F' "+
						 "where a.no_bill in ("+this.nobukti+") and a.kode_lokasi='"+this.app._lokasi+"' group by a.no_bill,a.periode "+						 
						
						 "union "+
						 "select a.no_angs as no_bukti,a.periode,sum(case a.dc when 'D' then a.nilai else -a.nilai end) as total  "+
						 "from kop_pinjangs_j a inner join kop_pinjangs_m b on a.no_angs=b.no_angs and a.kode_lokasi=b.kode_lokasi and b.posted='F' "+
						 "where a.no_angs in ("+this.nobukti+") and a.kode_lokasi='"+this.app._lokasi+"' group by a.no_angs,a.periode "+						 
			...
			if (this.sg1.rowValid(i) && this.sg1.cells(0,i)=="TRUE") {
				...
				if (this.sg1.cells(1,i) == "AR") 
					strSQL += "union all "+
							  ...
							  "from piutang_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";				
				
				if (this.sg1.cells(1,i) == "SP") 
					strSQL += "union all "+
							  "select 'INPROG' as status,no_bill as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'SIMPBILL' as jenis "+
							  "from kop_simpbill_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
							  "union all "+
							  "select 'INPROG' as status,no_bill as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'PINJBILL' as jenis "+
							  "from kop_pinjbill_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
							  
							  "union all "+
							  "select 'INPROG' as status,no_angs as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'SIMPANGS' as jenis "+
							  "from kop_simpangs_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
							  "union all "+
							  "select 'INPROG' as status,no_angs as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'PINJANGS' as jenis "+
							  "from kop_pinjangs_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
	...
	doChange:function(sender){
		...
		if (sender == this.c_modul && this.c_modul.getText()!="") {
			if (this.c_modul.getText()=="JU") this.cb_bukti.setSQL("select no_ju as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'JU' as jenis from ju_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_bukti","no_dokumen","tanggal","keterangan","jenis"],false,["No Bukti","No Dokumen","Tanggal","Keterangan","Jenis"],"and","Data Bukti",true);			
			if (this.c_modul.getText()=="KB") this.cb_bukti.setSQL("select no_kas as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'KB' as jenis from kas_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_bukti","no_dokumen","tanggal","keterangan","jenis"],false,["No Bukti","No Dokumen","Tanggal","Keterangan","jenis"],"and","Data Bukti",true);			
			if (this.c_modul.getText()=="AR") this.cb_bukti.setSQL("select no_piutang as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'PIUUM' as jenis from piutang_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_bukti","no_dokumen","tanggal","keterangan","jenis"],false,["No Bukti","No Dokumen","Tanggal","Keterangan","jenis"],"and","Data Bukti",true);			
			if (this.c_modul.getText()=="SP") this.cb_bukti.setSQL(
				"select no_bill as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'SIMPBILL' as jenis from kop_simpbill_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
				"union all "+
				"select no_bill as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'PINJBILL' as jenis from kop_pinjbill_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
				"union all "+
				"select no_angs as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'SIMPANGS' as jenis from kop_simpangs_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
				"union all "+
				"select no_angs as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'PINJANGS' as jenis from kop_pinjangs_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
			...
			switch(this.sg.cells(5,row)){
				...
				case "SIMPANGS" :
							strSQL = "select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk,'-' as kode_cf,'-' as nama_cf "+
									 ...
									 "             inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
									 "             left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and d.tahun=substring(a.periode,1,4) "+
									 "where a.no_angs = '"+this.sg.cells(1,row)+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.dc desc";
					break;	
				case "PINJANGS" :
							strSQL = "select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk,'-' as kode_cf,'-' as nama_cf "+
