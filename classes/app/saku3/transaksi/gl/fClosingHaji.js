window.app_saku3_transaksi_gl_fClosingHaji = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_gl_fClosingHaji.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_gl_fClosingHaji";
...
window.app_saku3_transaksi_gl_fClosingHaji.extend(window.childForm);
window.app_saku3_transaksi_gl_fClosingHaji.implement({
		...
		switch (event){
			...
			case "simpan" :	
				...
				var strSQL = "select count(*) as jml "+
							 ...
							 "left join dbtm.dbo.yk_bareg_m b on a.no_batch=b.no_bareg and a.kode_lokasi=b.kode_lokasi "+
							 "left join dbtm.dbo.yk_batm_d c on b.no_bareg=c.no_bareg "+
							 "where c.no_selesai is null and a.kode_lokasi='"+this.app._lokasi+"' and a.periode='"+this.app._periode+"'";				
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){
						if (parseFloat(line.jml) != 0) {
							system.alert(this,"Masih ada Billing belum approve di TM.","Lakukan BA Regian dan atau BA TM di SIMTM.");
							return false;
						}
					}
				}
	...
	isPostedAll: function(periode){
		...
		var sqlPR = "select count(no_akru) from haj_akru_m where posted = 'F' and periode = '"+periode+"' and kode_lokasi='"+this.app._lokasi+"'";	
		var data,ck = this.dbLib.loadQuery(sqlPR);
		if (ck != "") { 
			data = ck.split("\r\n"); 
			if (data[1] != "0"){
				result = result && false;
				msg+= "Data Modul PR masih ada yang belum diposting.("+data[1]+" data)\n";
			}
		}
