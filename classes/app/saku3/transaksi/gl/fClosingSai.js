window.app_saku3_transaksi_gl_fClosingSai = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_gl_fClosingSai.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_gl_fClosingSai";
...
window.app_saku3_transaksi_gl_fClosingSai.extend(window.childForm);
window.app_saku3_transaksi_gl_fClosingSai.implement({
	...
	isPostedAll: function(periode){
		...
		var sqlPR = "select count(no_bill) from sai_bill_m where posted = 'F' and periode = '"+periode+"' and kode_lokasi='"+this.app._lokasi+"'";	
