window.app_saku3_transaksi_gl_fPostingDikti = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_gl_fPostingDikti.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_gl_fPostingDikti";
...
window.app_saku3_transaksi_gl_fPostingDikti.extend(window.childForm);
window.app_saku3_transaksi_gl_fPostingDikti.implement({
		...
		switch (event){
			...
			case "simpan" :	
				...
				var strSQL = "select no_bukti+' - '+periode as bukper from ("+
						 ...
						 "union "+
						 "select a.no_hutang,a.periode,sum(case a.dc when 'D' then a.nilai else -a.nilai end) as total  "+
						 "from hutang_j a inner join hutang_m b on a.no_hutang=b.no_hutang and a.kode_lokasi=b.kode_lokasi and b.posted='F' and b.modul='IFREIM' "+
			...
			if (this.sg1.rowValid(i) && this.sg1.cells(0,i)=="TRUE") {
				...
				if (this.sg1.cells(1,i) == "KB") 
					strSQL += "union all "+
							  ...
							  "from hutang_m where modul='IFREIM' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";				
				...
				if (this.sg1.cells(1,i) == "DIKTI") 
					strSQL += "union all "+
							  "select 'INPROG' as status,no_bukti as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'DIKTI' as jenis "+
							  "from trans_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";				
	...
	doChange:function(sender){
		...
		if (sender == this.c_modul && this.c_modul.getText()!="") {
			...
			if (this.c_modul.getText()=="KB") this.cb_bukti.setSQL(
							"select no_kas as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'KB' as jenis from kas_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
							"union all "+
							"select no_hutang as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'HUIF' as jenis from hutang_m where modul='IFREIM' and posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "
			...
			if (this.c_modul.getText()=="DIKTI") this.cb_bukti.setSQL("select no_bukti as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'DIKTI' as jenis from trans_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_bukti","no_dokumen","tanggal","keterangan","jenis"],false,["No Bukti","No Dokumen","Tanggal","Keterangan","jenis"],"and","Data Bukti",true);			
