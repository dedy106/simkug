window.app_saku3_transaksi_gl_fUnPostingSinergi = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_gl_fUnPostingSinergi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_gl_fUnPostingSinergi";
...
window.app_saku3_transaksi_gl_fUnPostingSinergi.extend(window.childForm);
window.app_saku3_transaksi_gl_fUnPostingSinergi.implement({
			...
			if (this.sg1.rowValid(i) && this.sg1.cells(0,i)=="TRUE") {
				...
				if (this.sg1.cells(1,i) == "PR") 
					strSQL += "union all "+
							  ...
							  "from si_bill_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
							  "union all "+
							  "select 'INPROG' as status,no_spb as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'SPB' as jenis "+
							  "from si_spb_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";							  
			}						
		}		
	...
	doChange:function(sender){
		...
		if (sender == this.c_modul && this.c_modul.getText()!="") {
			...
			if (this.c_modul.getText()=="PR") this.cb_bukti.setSQL("select no_bill as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'BIL' as jenis from si_bill_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
																"union all "+
																"select no_spb as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'SPB' as jenis from si_spb_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ",["no_bukti","no_dokumen","tanggal","keterangan","jenis"],false,["No Bukti","No Dokumen","Tanggal","Keterangan","jenis"],"and","Data Bukti PR",true);			
		}	
