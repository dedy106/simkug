window.app_saku3_transaksi_gl_fClosingThnSinergi = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_gl_fClosingThnSinergi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_gl_fClosingThnSinergi";
...
window.app_saku3_transaksi_gl_fClosingThnSinergi.extend(window.childForm);
window.app_saku3_transaksi_gl_fClosingThnSinergi.implement({
	...
	isPostedAll: function(periode){
		...
		var sqlBill = "select count(no_bill) from si_bill_m where posted = 'F' and periode = '"+periode+"' and kode_lokasi='"+this.app._lokasi+"'";	
		var data,ck = this.dbLib.loadQuery(sqlBill);
		if (ck != "") { 
			data = ck.split("\r\n"); 
			if (data[1] != "0"){
				result = result && false;
				msg+= "Data Modul BILL masih ada yang belum diposting.("+data[1]+" data)\n";
			}
		}
		var sqlSpb = "select count(no_spb) from si_spb_m where posted = 'F' and periode = '"+periode+"' and kode_lokasi='"+this.app._lokasi+"'";	
		var data,ck = this.dbLib.loadQuery(sqlSpb);
		if (ck != "") { 
			data = ck.split("\r\n"); 
			if (data[1] != "0"){
				result = result && false;
				msg+= "Data Modul SPB masih ada yang belum diposting.("+data[1]+" data)\n";
			}
		}
