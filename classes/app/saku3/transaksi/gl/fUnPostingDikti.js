window.app_saku3_transaksi_gl_fUnPostingDikti = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_gl_fUnPostingDikti.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_gl_fUnPostingDikti";
		...
		this.c_modul = new saiCB(this.pc1.childPage[3],{bound:[20,11,202,20],caption:"Modul",items:["JU","KB","DIKTI","TAK","FA"], readOnly:true,tag:9,change:[this,"doChange"]});
		...
		try {
			...
			this.sg1.appendData(["TRUE","DIKTI","Pendidikan Tinggi"]);	
...
window.app_saku3_transaksi_gl_fUnPostingDikti.extend(window.childForm);
window.app_saku3_transaksi_gl_fUnPostingDikti.implement({
			...
			if (this.sg1.rowValid(i) && this.sg1.cells(0,i)=="TRUE") {
				...
				if (this.sg1.cells(1,i) == "KB") 
					strSQL += "union all "+
							  ...
							  "union all "+
							  "select 'INPROG' as status,no_hutang as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'HUIF' as jenis "+
							  "from hutang_m where modul='IFREIM' and posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";								
				...
				if (this.sg1.cells(1,i) == "DIKTI") 
					strSQL += "union all "+
							  "select 'INPROG' as status,no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'DIKTI' as jenis "+
	...
	doChange:function(sender){
		...
		if (sender == this.c_modul && this.c_modul.getText()!="") {
			...
			if (this.c_modul.getText()=="KB") this.cb_bukti.setSQL(
									"select no_kas as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'KB' as jenis from kas_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
									"union all "+
									"select no_hutang as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'HUIF' as jenis from hutang_m where modul='IFREIM' and posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "
									,["no_bukti","no_dokumen","tanggal","keterangan","jenis"],false,["No Bukti","No Dokumen","Tanggal","Keterangan","jenis"],"and","Data Bukti KB",true);			
			...
			if (this.c_modul.getText()=="DIKTI") this.cb_bukti.setSQL("select no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'DIKTI' as jenis from trans_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_bukti","no_dokumen","tanggal","keterangan","jenis"],false,["No Bukti","No Dokumen","Tanggal","Keterangan","jenis"],"and","Data Bukti",true);			
