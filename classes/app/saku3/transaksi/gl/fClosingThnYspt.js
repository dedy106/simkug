window.app_saku3_transaksi_gl_fClosingThnYspt = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_gl_fClosingThnYspt.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_gl_fClosingThnYspt";
...
window.app_saku3_transaksi_gl_fClosingThnYspt.extend(window.childForm);
window.app_saku3_transaksi_gl_fClosingThnYspt.implement({
		...
		switch (event){
			...
			case "simpan" :	
				...
				sql = "select round(a.so_akhir,2) as so_akhir "+
					  "from glma_tmp a inner join masakun b on b.kode_akun = a.kode_akun and b.kode_lokasi = a.kode_lokasi and b.modul = 'L' where a.kode_lokasi ='"+this.app._lokasi+"' and a.nik_user = '"+this.nik_user+"' and round(a.so_akhir,2) <> 0";
	...
	isPostedAll: function(periode){
		...
		var sqlTAK = "select count(no_kirim) from takkirim_m where posted = 'F' and periode = '"+periode+"' and kode_lokasi='"+this.app._lokasi+"' "+
		             "union all "+
					 "select count(no_terima) from takterima_m where posted = 'F' and periode = '"+periode+"' and kode_lokasi='"+this.app._lokasi+"' ";	
		var data,ck = this.dbLib.loadQuery(sqlTAK);
		if (ck != "") { 
			data = ck.split("\r\n"); 
			if (data[1] != "0"){
				result = result && false;
				msg+= "Data Modul TAK masih ada yang belum diposting.("+data[1]+" data)\n";
			}
		}
		var sqlAP = "select count(no_hutang) from hutang_m where posted = 'F' and periode = '"+periode+"' and kode_lokasi='"+this.app._lokasi+"' ";    
		var data,ck = this.dbLib.loadQuery(sqlAP);
		if (ck != "") { 
			data = ck.split("\r\n"); 
			if (data[1] != "0"){
				result = result && false;
				msg+= "Data Modul AP masih ada yang belum diposting.("+data[1]+" data)\n";
			}
		}
		...
		var sqlSI = "select count(x.no_bukti) from ("+
				    "  select no_bill as no_bukti from sis_bill_m where posted = 'F' and periode ='"+periode+"' and kode_lokasi='"+this.app._lokasi+"' "+
				    "  union "+
				    "  select no_rekon as no_bukti from sis_rekon_m where posted = 'F' and periode ='"+periode+"' and kode_lokasi='"+this.app._lokasi+"' "+
				    ") x";					   
		var data,ck = this.dbLib.loadQuery(sqlSI);
		if (ck != "") { 
			data = ck.split("\r\n"); 
			if (data[1] != "0"){
				result = result && false;
				msg+= "Data Modul SI [BILL,REKON] masih ada yang belum diposting.("+data[1]+" data)\n";
			}
		}
