window.app_saku3_transaksi_gl_fPostingTarbak = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_gl_fPostingTarbak.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_gl_fPostingTarbak";
...
window.app_saku3_transaksi_gl_fPostingTarbak.extend(window.childForm);
window.app_saku3_transaksi_gl_fPostingTarbak.implement({
			...
			if (this.sg1.rowValid(i) && this.sg1.cells(0,i)=="TRUE") {
				...
				if (this.sg1.cells(1,i) == "TAK") 
					strSQL += "union all "+
							  ...
							  "from takkirim_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+strPP+
							  "union all "+
							  "select 'INPROG' as status,no_terima as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'TT' as jenis "+
							  "from takterima_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+strPP;
	...
	doChange:function(sender){
		...
		if (sender == this.c_modul && this.c_modul.getText()!="") {
			...
			if (this.c_modul.getText()=="TAK") this.cb_bukti.setSQL(
							"select no_kirim as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'TK' as jenis from takkirim_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+strPP+
							"union all "+
							"select no_terima as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'TT' as jenis from takterima_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+strPP
