window.app_saku3_transaksi_gl_fUnPostingYpt = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_gl_fUnPostingYpt.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_gl_fUnPostingYpt";
...
window.app_saku3_transaksi_gl_fUnPostingYpt.extend(window.childForm);
window.app_saku3_transaksi_gl_fUnPostingYpt.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					//sql.add("call sp_exs_agg_real_bulan ('"+this.app._lokasi+"','"+this.e_periode.getText().substr(0,4)+"','"+this.app._userLog+"')");
	...
	doChange:function(sender){
		...
		if (sender == this.c_modul && this.c_modul.getText()!="") {
			...
			if (this.c_modul.getText()=="AP") this.cb_bukti.setSQL("select no_hutang as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'HU' as jenis from kas_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_bukti","no_dokumen","tanggal","keterangan","jenis"],false,["No Bukti","No Dokumen","Tanggal","Keterangan","jenis"],"and","Data Bukti",true);			
