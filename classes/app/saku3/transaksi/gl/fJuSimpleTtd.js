window.app_saku3_transaksi_gl_fJuSimpleTtd = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_gl_fJuSimpleTtd.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_gl_fJuSimpleTtd";
		...
		this.pc2 = new pageControl(this,{bound:[10,10,1000,410], childPage:["List Jurnal","Data Jurnal","Cari Data"]});		
		...
		this.cb_buat = new saiCBBL(this.pc2.childPage[1],{bound:[20,18,220,20],caption:"Dibuat Oleh", multiSelection:false, maxLength:10, tag:2});
		this.cb_app = new saiCBBL(this.pc2.childPage[1],{bound:[20,19,220,20],caption:"Disetujui Oleh", multiSelection:false, maxLength:10, tag:2});
		this.e_debet = new saiLabelEdit(this.pc2.childPage[1],{bound:[770,19,220,20],caption:"Total Debet", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		this.e_ket = new saiLabelEdit(this.pc2.childPage[1],{bound:[20,17,700,20],caption:"Deskripsi", maxLength:150});				
		...
		this.pc1 = new pageControl(this.pc2.childPage[1],{bound:[5,12,990,260], childPage:["Data Item Jurnal","Data Anggaran","Referensi Jurnal"]});
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:7,tag:0,
		            colTitle:["Kode Akun","Nama Akun","DC","Keterangan","Nilai","Kode PP","Nama PP"],
					colWidth:[[6,5,4,3,2,1,0],[150,80,100,400,50,200,80]],					
...
window.app_saku3_transaksi_gl_fJuSimpleTtd.extend(window.childForm);
window.app_saku3_transaksi_gl_fJuSimpleTtd.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','JU','UMUM','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.cb_buat.getText()+"','"+this.cb_app.getText()+"','F','-','-','-',getdate(),'"+this.app._userLog+"')");
		...
		switch (event){
			...
			case "ubah" :		
				if (this.flagDokFree == "1") {				
					var data = this.dbLib.getDataProvider("select no_ju from ju_m where no_ju <>'"+this.e_nb.getText()+"' and no_dokumen='"+this.e_dok.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
					if (typeof data == "object"){
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_gl_rptJuJurnalBuktiPp";
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select keterangan,no_dokumen,tanggal,nik_buat,nik_setuju "+
							 "from ju_m "+							 
