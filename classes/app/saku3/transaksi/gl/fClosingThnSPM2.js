window.app_saku3_transaksi_gl_fClosingThnSPM2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_gl_fClosingThnSPM2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_gl_fClosingThnSPM2";
...
window.app_saku3_transaksi_gl_fClosingThnSPM2.extend(window.childForm);
window.app_saku3_transaksi_gl_fClosingThnSPM2.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					//glma_pp
					this.nik_user=this.app._nikUser;
					var sql = "call sp_glmapp ('"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.nik_user+"')";		
					this.dbLib.execQuerySync(sql);			
					...
					if (this.isPostedAll(this.app._periode)){								
						...
						sql.add("insert into glma_pp (kode_akun, kode_pp, kode_lokasi, periode, so_akhir, tgl_input, nik_user) "+
								"select kode_akun, kode_pp, kode_lokasi, '"+closePeriode(this.app._periode,this.maxPeriode)+"', sakhir, getdate(), '"+this.app._userLog+"' "+
								"from glmapp_tmp where kode_lokasi ='"+this.app._lokasi+"' and nik_user = '"+this.nik_user+"' and sakhir <> 0");
		...
		switch (event){
			...
			case "simpan" :	
				...
				sql = "select round(sum(a.so_akhir),0) as so_akhir "+
					  "from glma_tmp a "+
					  "where a.kode_lokasi ='"+this.app._lokasi+"' and a.nik_user = '"+this.nik_user+"'";
