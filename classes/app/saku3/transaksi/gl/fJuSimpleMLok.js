window.app_saku3_transaksi_gl_fJuSimpleMLok = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_gl_fJuSimpleMLok.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_gl_fJuSimpleMLok";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Jurnal Umum Multi Lokasi", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[1],{bound:[5,12,990,330], childPage:["Data Item Jurnal","Rekap Lok PP"]});
		...
		this.cb_akun = new saiCBBL(this.pc1.childPage[1],{bound:[20,14,220,20],caption:"Akun Mutasi", multiSelection:false, maxLength:10, tag:2});										
		this.sg4 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-58],colCount:3,tag:9,
...
window.app_saku3_transaksi_gl_fJuSimpleMLok.extend(window.childForm);
window.app_saku3_transaksi_gl_fJuSimpleMLok.implement({	
		...
		try {
			...
			for (var i=0;i < this.sg.rows.getLength();i++){						
				if (this.sg.rowValid(i)) {
					if (this.sg.cells(2,i) == "D") nilai = nilaiToFloat(this.sg.cells(4,i))  * -1;
					else nilai = nilaiToFloat(this.sg.cells(4,i));
					var isAda = false;
					var idx = totalnilai = 0;
					for (var j=0;j < this.sg4.getRowCount();j++){
						if ((this.sg.cells(7,i) == this.sg4.cells(0,j)) && (this.sg.cells(5,i) == this.sg4.cells(1,j))) {
							isAda = true;
							idx = j;
							break;
						}
					...
					if (!isAda) {
						this.sg4.appendData([this.sg.cells(7,i),this.sg.cells(5,i),floatToNilai(nilai)]);
					} 
					else { 
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','-','-','-','-','-','-','-','"+this.sg.cells(7,i)+"','JT','MLOK','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','-')");
							}
						}
					}			
					...
					if (this.sg4.getRowValidCount() > 0){
						for (var i=0;i < this.sg4.getRowCount();i++){
							if (this.sg4.rowValid(i)){
								...
								sql.add("insert into ju_j(no_ju,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,no_ref,ket_ref) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+j+",'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','"+dc+"',"+Math.abs(nilaiToFloat(this.sg4.cells(2,i)))+",'"+this.sg4.cells(1,i)+"','-','-','-','-','-','-','-','"+this.sg4.cells(0,i)+"','JT','TAK','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','-')");
							}
						}
					}	
					...
					sql.add("insert into ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user)  "+
						    "select '"+this.e_nb.getText()+"',kode_lokasi,'"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','-','JT','JTAK','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"','F','-','-','"+this.cb_akun.getText()+"',getdate(),'"+this.app._userLog+"' "+
						    "from ju_j where no_ju='"+this.e_nb.getText()+"' group by kode_lokasi");
	...
	doCari:function(sender){																		
		...
		var strSQL = "select a.no_ju,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan "+
		             "from ju_m a "+					 					 
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'JT' and a.posted ='F' "+filter;		
		var data = this.dbLib.getDataProvider(strSQL,true);
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_ju,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan "+
		             "from ju_m a left join (select no_ju from ju_m where posted='T' and periode='"+this.e_periode.getText()+"') b on a.no_ju=b.no_ju "+					 					 
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'JT' and a.posted ='F' and b.no_ju is null ";	
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
			this.sgn3.setTotalPage(Math.ceil(data.rs.rows.length/20));
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_lokasi "+
							...
							"where a.no_ju = '"+this.e_nb.getText()+"' and a.jenis='MLOK' order by a.no_urut",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];												
						this.sg.appendData([line.kode_akun,line.nama_akun,line.dc,line.keterangan,floatToNilai(line.nilai),line.kode_pp,line.nama_pp,line.kode_lokasi]);
					}
