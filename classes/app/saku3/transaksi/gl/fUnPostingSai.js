window.app_saku3_transaksi_gl_fUnPostingSai = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_gl_fUnPostingSai.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_gl_fUnPostingSai";
...
window.app_saku3_transaksi_gl_fUnPostingSai.extend(window.childForm);
window.app_saku3_transaksi_gl_fUnPostingSai.implement({
			...
			if (this.sg1.rowValid(i) && this.sg1.cells(0,i)=="TRUE") {
				...
				if (this.sg1.cells(1,i) == "PR") 
					strSQL += "union all "+
							  ...
							  "from sai_bill_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";				
			}						
		}		
	...
	doChange:function(sender){
		...
		if (sender == this.c_modul && this.c_modul.getText()!="") {
			...
			if (this.c_modul.getText()=="PR") this.cb_bukti.setSQL("select no_bill as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'PR' as jenis from sai_bill_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_bukti","no_dokumen","tanggal","keterangan","jenis"],false,["No Bukti","No Dokumen","Tanggal","Keterangan","jenis"],"and","Data Bukti PR",true);			
		}	
