window.app_saku3_transaksi_gl_fPostingSinergi = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_gl_fPostingSinergi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_gl_fPostingSinergi";
		...
		this.c_modul = new saiCB(this.pc1.childPage[3],{bound:[20,11,202,20],caption:"Modul",items:["JU","KB","PR"], readOnly:true,tag:9,change:[this,"doChange"]});
		...
		try {
			...
			this.sg1.appendData(["TRUE","PR","Produksi"]);			
...
window.app_saku3_transaksi_gl_fPostingSinergi.extend(window.childForm);
window.app_saku3_transaksi_gl_fPostingSinergi.implement({
		...
		switch (event){
			...
			case "simpan" :	
				...
				var strSQL = "select no_bukti+' - '+periode as bukper from ("+
						 ...
						 "from si_bill_j a inner join si_bill_m b on a.no_bill=b.no_bill and a.kode_lokasi=b.kode_lokasi and b.posted='F' "+
						 "where a.no_bill in ("+this.nobukti+") and a.kode_lokasi='"+this.app._lokasi+"' group by a.no_bill,a.periode "+						 
						 "union "+
						 "select a.no_spb as no_bukti,a.periode,sum(case a.dc when 'D' then a.nilai else -a.nilai end) as total  "+
						 "from si_spb_j a inner join si_spb_m b on a.no_spb=b.no_spb and a.kode_lokasi=b.kode_lokasi and b.posted='F' "+
						 "where a.no_spb in ("+this.nobukti+") and a.kode_lokasi='"+this.app._lokasi+"' group by a.no_spb,a.periode "+						 
		...
		for (var i=0;i < this.sg1.getRowCount();i++){
			if (this.sg1.rowValid(i) && this.sg1.cells(0,i)=="TRUE") {
				...
				if (this.sg1.cells(1,i) == "PR") 
					strSQL += "union all "+
							  "select 'INPROG' as status,no_bill as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'BIL' as jenis "+
							  "from si_bill_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
							  "union all "+
							  "select 'INPROG' as status,no_spb as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'SPB' as jenis "+
							  "from si_spb_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";				
			}						
		}		
	...
	doChange:function(sender){
		...
		if (sender == this.c_modul && this.c_modul.getText()!="") {
			...
			if (this.c_modul.getText()=="PR") this.cb_bukti.setSQL("select no_bill as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'BIL' as jenis from si_bill_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
																   "union all "+
																   "select no_spb as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'SPB' as jenis from si_spb_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ",["no_bukti","no_dokumen","tanggal","keterangan","jenis"],false,["No Bukti","No Dokumen","Tanggal","Keterangan","jenis"],"and","Data Bukti PR",true);			
		}		
	...
	doDoubleClick: function(sender, col , row) {
		if (this.sg.cells(1,row) != "" && this.sg.cells(5,row) != "") {
			...
			switch(this.sg.cells(5,row)){
				...
				case "BIL" :
							strSQL = "select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk,'-' as kode_cf,'-' as nama_cf "+
									 "from si_bill_j a inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
									 "             inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
									 "             left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and d.tahun=substring(a.periode,1,4) "+									 
				...
				case "SPB" :
							strSQL = "select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk,'-' as kode_cf,'-' as nama_cf "+
									 "from si_spb_j a inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
									 "             inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
									 "             left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and d.tahun=substring(a.periode,1,4) "+									 
									 "where a.no_spb = '"+this.sg.cells(1,row)+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.dc desc";
					break;								
			}
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
