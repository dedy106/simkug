window.app_saku3_transaksi_gl_fPostingTM = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_gl_fPostingTM.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_gl_fPostingTM";
...
window.app_saku3_transaksi_gl_fPostingTM.extend(window.childForm);
window.app_saku3_transaksi_gl_fPostingTM.implement({
		...
		switch (event){
			...
			case "simpan" :	
				...
				var strSQL = "select no_bukti+' - '+periode as bukper from ("+
						 ...
						 "select a.no_hutang as no_bukti,a.periode,sum(case a.dc when 'D' then a.nilai else -a.nilai end) as total  "+
						 "from tm_hutang_j a inner join tm_hutang_m b on a.no_hutang=b.no_hutang and a.kode_lokasi=b.kode_lokasi and b.posted='F' "+
						 "where a.no_hutang in ("+this.nobukti+") and a.kode_lokasi='"+this.app._lokasi+"' group by a.no_hutang,a.periode "+						 
						 "union "+
			...
			if (this.sg1.rowValid(i) && this.sg1.cells(0,i)=="TRUE") {
				...
				if (this.sg1.cells(1,i) == "AP") 
					strSQL += "union all "+
							  ...
							  "from tm_hutang_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";				
	...
	doChange:function(sender){
		...
		if (sender == this.c_modul && this.c_modul.getText()!="") {
			...
			if (this.c_modul.getText()=="AP") 
				this.cb_bukti.setSQL("select no_hutang as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'HU' as jenis from hutang_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
									 "union "+
									 "select no_hutang as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'HT' as jenis from tm_hutang_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "
			...
			if (this.c_modul.getText()=="AR") 
				this.cb_bukti.setSQL("select no_piutang as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'PU' as jenis from piutang_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
									 "union "+
									 "select no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'TR' as jenis from trans_m where posted='F' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "
									,["no_bukti","no_dokumen","tanggal","keterangan","jenis"],false,["No Bukti","No Dokumen","Tanggal","Keterangan","jenis"],"and","Data Bukti",true);			
