window.app_saku3_transaksi_gl_fYptSap = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_gl_fYptSap.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_gl_fYptSap";
...
window.app_saku3_transaksi_gl_fYptSap.extend(window.childForm);
window.app_saku3_transaksi_gl_fYptSap.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into ypt_sap(no_bukti,kode_lokasi,periode,keterangan,tgl_input,nik_user) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.e_nama.getText()+"',getdate(),'"+this.app._userLog+"')");
					sql.add("exec sp_ypt_sap '"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"'");
	...
	doClick:function(sender){
		if (sender == this.i_gen) {
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"ypt_sap","no_bukti",this.app._lokasi+"-SAP"+this.e_periode.getText().substr(2,4)+".","000"));
