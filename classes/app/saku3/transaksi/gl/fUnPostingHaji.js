window.app_saku3_transaksi_gl_fUnPostingHaji = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_gl_fUnPostingHaji.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_gl_fUnPostingHaji";
...
window.app_saku3_transaksi_gl_fUnPostingHaji.extend(window.childForm);
window.app_saku3_transaksi_gl_fUnPostingHaji.implement({
			...
			if (this.sg1.rowValid(i) && this.sg1.cells(0,i)=="TRUE") {
				...
				if (this.sg1.cells(1,i) == "PR") 
					strSQL += "union all "+
							  ...
							  "from haj_akru_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";				
			}						
		}		
	...
	doChange:function(sender){
		...
		if (sender == this.c_modul && this.c_modul.getText()!="") {
			...
			if (this.c_modul.getText()=="PR") this.cb_bukti.setSQL("select no_akru as no_bukti,no_dokumen,convert(varchar,tanggal,103) as tanggal,keterangan,'PR' as jenis from haj_akru_m where posted='T' and periode='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",["no_bukti","no_dokumen","tanggal","keterangan","jenis"],false,["No Bukti","No Dokumen","Tanggal","Keterangan","jenis"],"and","Data Bukti PR",true);			
		}	
