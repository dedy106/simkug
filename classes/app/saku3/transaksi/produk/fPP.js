window.app_saku3_transaksi_produk_fPP = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_produk_fPP.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_produk_fPP";
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:4,tag:9,
		            colTitle:["Kode","Nama PP","Divisi","Status"],
					colWidth:[[3,2,1,0],[150,200,300,100]],
					readOnly:true,
		...
		this.cb_div = new saiCBBL(this.pc1.childPage[1],{bound:[20,14,220,20],caption:"Bidang", multiSelection:false, maxLength:10, tag:2});
...
window.app_saku3_transaksi_produk_fPP.extend(window.childForm);
window.app_saku3_transaksi_produk_fPP.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into pp(kode_pp,kode_lokasi,nama,kode_bidang,jenis,flag_aktif,tipe,nik, kota,bank,norek) values "+
							"('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.cb_div.getText()+"','-','"+this.c_status.getText().substr(0,1)+"','Posting','"+this.cb_nik.getText()+"', '"+
							this.e_kota.getText()+"','-','-')");					
					setTipeButton(tbAllFalse);
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_pp,line.nama,line.bidang,line.status.toUpperCase()]); 
		}
