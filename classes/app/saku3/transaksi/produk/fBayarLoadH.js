window.app_saku3_transaksi_produk_fBayarLoadH = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_produk_fBayarLoadH.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_produk_fBayarLoadH";
		this.itemsValue = new arrayList();
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Hapus Load Pelunasan Tagihan", 0);	
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select distinct periode from sis_rekon_d where kode_lokasi='"+this.app._lokasi+"'",true);
...
window.app_saku3_transaksi_produk_fBayarLoadH.extend(window.childForm);
window.app_saku3_transaksi_produk_fBayarLoadH.implement({			
			...
			case "clear" :
				if (modalResult == mrOk)
					...
					var strSQL = "select distinct a.no_rekon, b.keterangan "+
								"from sis_rekon_d a inner join trans_m b on a.no_rekon=b.no_bukti and a.kode_lokasi=b.kode_lokasi "+
	...
	doChange: function() {
		if (this.e_periode.getText() != "") {
			var strSQL = "select distinct a.no_rekon, b.keterangan "+
						 "from sis_rekon_d a inner join trans_m b on a.no_rekon=b.no_bukti and a.kode_lokasi=b.kode_lokasi and b.form in ('LOADCD','LOADBEA','KBBILL') "+
						 "where a.kode_lokasi='"+this.app._lokasi+"' and a.periode='"+this.e_periode.getText()+"' ";
			this.cb_bukti.setSQL(strSQL,["no_rekon","keterangan"],false,["No Bukti","Deskripsi"],"and","Data Pelunasan",true);			
		}
	},
