window.app_saku3_transaksi_produk_fVendor = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_produk_fVendor.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_produk_fVendor";
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:6,tag:9,
					...
					colWidth:[[5,4,3,2,1,0],[100,100,150,300,200,60]],
					readOnly:true,
		...
		this.cb_hutang = new portalui_saiCBBL(this.pc1.childPage[1],{bound:[20,20,220,20],caption:"Akun Hutang",maxLength:20,multiSelection:false});		
		...
		try {
			...
			this.cb_hutang.setSQL("select a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '024' where  a.kode_lokasi = '"+this.app._lokasi+"' ",["a.kode_akun","a.nama"],"and",["Kode","Nama"],false);						
...
window.app_saku3_transaksi_produk_fVendor.extend(window.childForm);
window.app_saku3_transaksi_produk_fVendor.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into brg_vendor(kode_vendor,kode_lokasi,nama,flag_aktif,alamat,alamat2,no_tel,no_tel2,no_fax,email,npwp,pic,akun_hutang) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','1','"+this.e_alamat.getText()+"','-','"+this.e_tel.getText()+"','"+this.e_hp.getText()+"','"+this.e_fax.getText()+"','"+this.e_email.getText()+"','"+this.e_npwp.getText()+"','"+this.e_pic.getText()+"','"+this.cb_hutang.getText()+"')");
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					...
					sql.add("update brg_vendor set nama='"+this.e_nama.getText()+"',alamat='"+this.e_alamat.getText()+"',no_tel='"+this.e_tel.getText()+"',no_tel2='"+this.e_hp.getText()+"',no_fax='"+this.e_fax.getText()+"',email='"+this.e_email.getText()+"',npwp='"+this.e_npwp.getText()+"',pic='"+this.e_pic.getText()+"',akun_hutang='"+this.cb_hutang.getText()+"' "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from brg_vendor where kode_vendor = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");			
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select nama,alamat,no_fax,no_tel,no_tel2,npwp,email,pic,akun_hutang,flag_aktif from brg_vendor where kode_vendor ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_vendor,line.nama,line.alamat,line.pic,line.no_tel2,line.email]); 
		}
