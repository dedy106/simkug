window.app_saku3_transaksi_produk_fGarRelease = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_produk_fGarRelease.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_produk_fGarRelease";
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:0,
		            colTitle:["Kode MTA","Nama MTA","Kode PP","Nama PP","Periode", "Nilai Budget","Tot Release","Sisa Budget","Nilai Release"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[100,100,100,100,  80,150,80,150,80]],
...
window.app_saku3_transaksi_produk_fGarRelease.extend(window.portalui_childForm);
window.app_saku3_transaksi_produk_fGarRelease.implement({	
	doTampilClick: function() {
		...
		var strSQL = "select a.*,ISNULL(e.tot_release,0) as tot_release, "+
					...
					"group by "+
					"a.kode_lokasi,c.kode_gar,c.nama,a.kode_pp,d.nama,a.periode "+
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData([line.kode_gar,line.nama_gar, line.kode_pp,line.nama_pp, line.periode, floatToNilai(line.nilai_gar),floatToNilai(line.tot_release),floatToNilai(line.sisa_budget), floatToNilai(line.nilai)]);
		}
							...
							for (var i=0;i < this.dataJU.rs.rows.length;i++){
								...
								if (parseFloat(line.nilai) != 0){
									sql.add("insert into angg_release_d(no_bukti,kode_lokasi,kode_gar,kode_pp,kode_drk,periode,dc,nilai) values "+
											"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+line.kode_gar+"','"+line.kode_pp+"','-','"+line.periode+"','D',"+parseFloat(line.nilai)+")");									
								}
							}	
