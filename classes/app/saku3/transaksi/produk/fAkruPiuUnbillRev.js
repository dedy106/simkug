window.app_saku3_transaksi_produk_fAkruPiuUnbillRev = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_produk_fAkruPiuUnbillRev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_produk_fAkruPiuUnbillRev";
		...
		this.sg1 = new saiGrid(this.pc2.childPage[0],{bound:[0,5,this.pc2.width-5,270],colCount:8,tag:0,
				colTitle:["Status","No Referensi","Tanggal","Keterangan","Akun Piutang","Akun Pdpt","DRK","Nilai Unbill"],
				colWidth:[[7,6,5,4,3,2,1,0],[100,80,80,80,300,80,150,80]],
...
window.app_saku3_transaksi_produk_fAkruPiuUnbillRev.extend(window.childForm);
window.app_saku3_transaksi_produk_fAkruPiuUnbillRev.implement({
	doLoadCBproyek: function() {			
			var strSQL = "select distinct a.kode_proyek,a.nama as keterangan "+
						 "from pr_proyek a inner join pr_unbill_d b on a.kode_proyek=b.kode_proyek and a.kode_lokasi=b.kode_lokasi and b.no_reverse = '-' "+
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {						
						...
						sql.add("update pr_unbill_d set no_reverse='-' where no_reverse = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");	
					}					
					...
					if (this.sg1.getRowValidCount() > 0) {
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i) && this.sg1.cells(0,i) == "REVERSE"){
								...
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',1,'"+this.sg1.cells(4,i)+"','C',"+parseNilai(this.sg1.cells(7,i))+","+parseNilai(this.sg1.cells(7,i))+",'"+this.e_ket.getText()+"','PR','REVUNBILL','IDR',1,'"+this.cb_pp.getText()+"','-','"+this.cb_cust.getText()+"','-','-','-','-','-','-')");					
							
								sql.add("update pr_unbill_d set no_reverse='"+this.e_nb.getText()+"' where no_bukti = '"+this.sg1.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"'");	
							}
						}
					}
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_proyek && this.cb_proyek.getText()!="") {
			if (this.stsSimpan==1) {
				...
				var strSQL = "select c.akun_piutang,c.akun_pdpt,c.nilai,c.kode_drk,c.no_bukti,convert(varchar,d.tanggal,103) as tgl,d.keterangan "+
							 "from pr_unbill_d c "+
							 "inner join trans_m d on c.no_bukti=d.no_bukti and c.kode_lokasi=d.kode_lokasi "+						
							 "where c.no_reverse='-' and c.kode_proyek='"+this.cb_proyek.getText()+"' and c.kode_lokasi='"+this.app._lokasi+"'";					
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData(["INPROG",line.no_bukti,line.tgl,line.keterangan,line.akun_piutang,line.akun_pdpt,line.kode_drk,floatToNilai(line.nilai)]);
					}
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select c.akun_piutang,c.akun_pdpt,c.nilai,c.kode_drk,c.no_bukti,convert(varchar,d.tanggal,103) as tgl,d.keterangan "+
						 	 "from pr_unbill_d c "+
							 "inner join trans_m d on c.no_bukti=d.no_bukti and c.kode_lokasi=d.kode_lokasi "+						
							 "where c.no_reverse='"+this.e_nb.getText()+"' and c.kode_proyek='"+this.cb_proyek.getText()+"' and c.kode_lokasi='"+this.app._lokasi+"'";					
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg1.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData(["REVERSE",line.no_bukti,line.tgl,line.keterangan,line.akun_piutang,line.akun_pdpt,line.kode_drk,floatToNilai(line.nilai)]);
					}
				} else this.sg1.clear(1);
