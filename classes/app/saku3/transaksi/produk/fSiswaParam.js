window.app_saku3_transaksi_produk_fSiswaParam = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_produk_fSiswaParam.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_produk_fSiswaParam";
		...
		this.cb_pp = new saiCBBL(this,{bound:[20,10,230,20],caption:"Kode PP", readOnly:true, tag:2, change:[this,"doChange"],change:[this,"doChange"]});
		...
		this.pc1 = new pageControl(this,{bound:[5,12,890,405], childPage:["Per Siswa","Per Tingkat"]});	
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,318],colCount:5,tag:9,				
		...
		this.sg1 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,293],colCount:2,tag:9,				
				colTitle:["Kode","Nama"],
		...
		try {
			...
			this.cb_tingkat.setSQL("select kode_tingkat, nama from sis_tingkat  where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"'",["kode_tingkat","nama"],false,["Kode","Nama"],"and","Data Tingkat",true);
...
window.app_saku3_transaksi_produk_fSiswaParam.extend(window.childForm);
window.app_saku3_transaksi_produk_fSiswaParam.implement({
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			case "clear" :
				...
				break;
			case "simpan" :	
				this.cb_kelas.setTag("1");
				this.cb_siswa.setTag("1");
				this.simpan();
				break;				
		...
		if (this.cb_pp.getText() != "" && this.cb_jur.getText() != "" && this.cb_tingkat.getText() != "" && this.cb_akt.getText() != "" && this.c_tahun1.getText() != "" && this.c_tahun2.getText() != "" && this.c_bulan1.getText() != "" && this.c_bulan2.getText() != "" ) {			
			...
			if(listParam != ""){
				...
				sql.add("insert into sis_siswa_tarif(nis,kode_kelas,kode_param,per_awal,per_akhir,tarif,kode_lokasi,kode_pp,kode_akt) "+
						...
						"where a.kode_status='AK' and a.kode_pp='"+this.cb_pp.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and c.kode_jur='"+this.cb_jur.getText()+"' and a.kode_akt='"+this.cb_akt.getText()+"' and c.kode_tingkat='"+this.cb_tingkat.getText()+"' ");
