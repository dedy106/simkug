window.app_saku3_transaksi_produk_fRekonTenan = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_produk_fRekonTenan.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_produk_fRekonTenan";
		...
		this.sg4 = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:6,tag:9,
		            colTitle:["Kode Tenan","Tenan","Total","% Sharing","Hasil Tenan","Hasil Tel-U"],
		...
		this.sg5 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:6,tag:0,
		            colTitle:["Kode Tenan","Nama Tenan","No Nota","Tanggal","Status","Nilai"],
					colWidth:[[5,4,3,2,1,0],[100,100,100,150,300,100]],					
					readOnly:true,
					colFormat:[[5],[cfNilai]],					
					autoAppend:false,defaultRow:1});
...
window.app_saku3_transaksi_produk_fRekonTenan.extend(window.childForm);
window.app_saku3_transaksi_produk_fRekonTenan.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("update ktu_nota set no_rekon='-' where no_rekon='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");
					}					
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3, param1,param2,param3) values "+
							...
							parseNilai(this.e_total.getText())+","+parseNilai(this.e_total.getText())+",0,'-','-','-','-','-','-', '-','-','-')");
					
					for (var i=0;i < this.dataJU5.rs.rows.length;i++){
						var line = this.dataJU5.rs.rows[i];
						sql.add("update ktu_nota set no_rekon='"+this.e_nb.getText()+"' where no_bukti='"+line.no_bukti+"' and kode_lokasi='"+this.app._lokasi+"' ");
					}		
	...
	doCari:function(sender){								
		if (this.cb_kantin.getText() != "") {
			...
			var strSQL = "select a.kode_tenan, b.nama, b.persentase, isnull(sum(a.nilai),0) as tenan, round(isnull(sum(a.nilai * b.persentase / 100),0),0) as hasil  "+
						 ...
						 "where a.no_rekon =  '-' and a.no_ba <> '-' and a.kode_lokasi ='"+this.app._lokasi+"' "+
			...
			var strSQL = "select a.kode_tenan, b.nama, a.no_bukti, a.tanggal, a.status, a.nilai  "+
						 "from ktu_nota a "+ 
						 "inner join ktu_tenan b on a.kode_tenan=b.kode_tenan and a.kode_lokasi=b.kode_lokasi "+							
						 "where a.no_rekon =  '-' and a.no_ba <> '-' and a.kode_lokasi ='"+this.app._lokasi+"' "+
	...
	doTampilData5: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg5.appendData([line2.kode_tenan,line2.nama,line2.no_bukti,line2.tanggal,line2.status,floatToNilai(line2.nilai)]); 
		}
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_kantintu_rptRekon";
	...
	doLoad3:function(sender){	
		if (this.cb_kantin.getText() != "") {					
			var strSQL = "select a.no_rekon,convert(varchar,a.tanggal,103) as tanggal,a.ket,a.nilai "+
						 "from ktu_rekon_m a inner join trans_m b on a.no_rekon=b.no_bukti and a.kode_lokasi=b.kode_lokasi "+
						 "  left join ( "+
						 "      select distinct no_rekon,kode_lokasi from ktu_rekon_d where no_kas<>'-' and kode_lokasi='"+this.app._lokasi+"' "+
						 ") c on a.no_rekon=c.no_rekon and a.kode_lokasi=c.kode_lokasi "+
						 "where c.no_rekon is null and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_kantin='"+this.cb_kantin.getText()+"' and a.periode='"+this.e_periode.getText()+"' and b.posted='F' ";			
			var data = this.dbLib.getDataProvider(strSQL,true);		
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU = data;
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line4.no_rekon,line4.tanggal,line4.ket,floatToNilai(line4.nilai)]); 							
			}
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {			
				...
				var strSQL = "select * from ktu_rekon_m where no_rekon ='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){						
						this.e_ket.setText(line.ket);	
						this.cb_kantin.setSQL("select kode_kantin, nama from ktu_kantin where kode_kantin='"+line.kode_kantin+"' and  kode_lokasi = '"+this.app._lokasi+"'",["kode_kantin","nama"],false,["Kode","Nama"],"and","Data Kantin",true);
				...
				var strSQL = "select a.kode_tenan, b.nama, b.persentase, isnull(sum(a.nilai),0) as tenan, round(isnull(sum(a.nilai * b.persentase / 100),0),0) as hasil  "+
							"from ktu_nota a "+ 
							"inner join ktu_tenan b on a.kode_tenan=b.kode_tenan and a.kode_lokasi=b.kode_lokasi "+							
							"where a.no_rekon =  '"+this.e_nb.getText()+"' and a.no_ba <> '-' and a.kode_lokasi ='"+this.app._lokasi+"' "+
							"group by a.kode_tenan, b.nama,b.persentase";								 
				var data2 = this.dbLib.getDataProvider(strSQL,true);					
				if (typeof data2 == "object" && data2.rs.rows[0] != undefined){
					var line2;
