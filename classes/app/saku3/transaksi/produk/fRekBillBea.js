window.app_saku3_transaksi_produk_fRekBillBea = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_produk_fRekBillBea.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_produk_fRekBillBea";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Load Pelunasan Tagihan BeaSiswa (NONCASH)", 0);	
		...
		this.cb_titip = new saiCBBL(this,{bound:[20,18,220,20],caption:"Akun Beban", multiSelection:false, maxLength:10, tag:2 });
		...
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[1],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:5,
					colTitle:["ID Bank","NIS","Nilai Beasiswa", "Sisa Beasiswa","Validasi"],
		...
		try {
			...
			this.cb_titip.setSQL("select a.kode_akun, a.nama from masakun a "+
								 "inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag in ('020') "+
...
window.app_saku3_transaksi_produk_fRekBillBea.extend(window.childForm);
window.app_saku3_transaksi_produk_fRekBillBea.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','V','LOADBEA','X','0','0','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','-','"+this.e_ket.getText()+
							"','IDR',1,"+total+",0,0,'-','-','-','-','-','-','"+this.cb_titip.getText()+"','-','-')");
					
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.cb_titip.getText()+"','D',"+total+","+total+",'"+this.e_ket.getText()+"','V','LOADBEA','IDR',1,'"+this.cb_pp.getText()+"','-','-','-','-','-','-','-','-')");
					...
					for (var i in this.gridAR.objList){
						...
						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+line.get("kode_akun")+"','C',"+parseFloat(line.get("nilai"))+","+parseFloat(line.get("nilai"))+",'"+this.e_ket.getText()+"','V','LOADBEA','IDR',1,'"+this.cb_pp.getText()+"','-','-','-','-','-','-','-','-')");
					}		
					...
					for (var i=0; i < this.sg1.getRowCount();i++){
						if (nilaiToFloat(this.sg1.cells(2,i)) != 0) {
							sql.add("insert into sis_cd_d(no_bukti,nis,periode,nilai,kode_lokasi,akun_cd,kode_param,dc,modul,kode_pp,no_ref1) values "+
									"('"+this.e_nb.getText()+"','"+this.sg1.cells(0,i)+"','"+this.e_periode.getText()+"',"+nilaiToFloat(this.sg1.cells(2,i))+",'"+this.app._lokasi+"','"+this.akunCD+"','-','D','LOADCD','"+this.cb_pp.getText()+"','-')");							
							
							var k = i + 100000;
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',"+k+",'"+this.akunCD+"','C',"+nilaiToFloat(this.sg1.cells(2,i))+","+nilaiToFloat(this.sg1.cells(2,i))+",'"+this.e_ket.getText()+"','V','LOADBEA','IDR',1,'"+this.cb_pp.getText()+"','-','-','-','-','-','-','-','-')");					
						}
					}
	...
	doLoad: function(sender){	
		if (this.cb_pp.getText()!="") {
			...
			var strSQL = "select aa.id_bank,aa.nis,aa.nama as siswa,a.no_bill,a.periode,a.akun_piutang,sum((a.nilai-isnull(b.tot_lunas,0))) as saldo,0 as lunas,0 as sisa,a.kode_param,c.idx "+
						 "from sis_bill_d a "+
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){							
							this.nama_report="server_report_saku3_produk_rptSisJurnalBea";
