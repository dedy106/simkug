window.app_saku3_transaksi_produk_fHpp = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_produk_fHpp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_produk_fHpp";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,12,990,330], childPage:["Detail Barang"]});				
...
window.app_saku3_transaksi_produk_fHpp.extend(window.portalui_childForm);
window.app_saku3_transaksi_produk_fHpp.implement({
					...
					for (var i=0;i<this.dataJU.rs.rows.length;i++){
						...
						if(parseFloat(line.hpp) != 0){											
							...
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+line.akun_hpp+"','D',"+parseFloat(line.hpp)+","+parseFloat(line.hpp)+",'"+this.e_ket.getText()+"','IV','HPP','IDR',1,"+line.kode_pp+",'-','-','-','-','-','-','-','-')");
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+j+",'"+line.akun_pers+"','C',"+parseFloat(line.hpp)+","+parseFloat(line.hpp)+",'"+this.e_ket.getText()+"','IV','BRG','IDR',1,"+line.kode_pp+",'-','-','-','-','-','-','-','-')");			
						}
					}
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "") {
			...
			this.stsSimpan = 1;
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti",this.app._lokasi+"-HPP"+this.e_periode.getText().substr(2,4)+".","0000"));			
			this.e_dok.setFocus();
	...
	doLoadData:function(sender){		
		...
		var strSQL =  "select a.kode_barang,a.nama,a.pabrik,a.kode_gudang,a.kode_pp,a.sat_kecil,f.akun_pers,f.akun_hpp,isnull(b.sawal,0)+isnull(c.beli,0)-isnull(d.sakhir,0) as jumlah, "+
					  "isnull(round(e.h_avg,2),0) as h_avg,round((isnull(b.sawal,0)+isnull(c.beli,0)-isnull(d.sakhir,0)) * isnull(e.h_avg,0),0) as hpp "+
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select no_dokumen,keterangan from trans_m "+							 
				...
				var strSQL = "select a.kode_barang,a.nama,a.pabrik,a.sat_kecil,b.jumlah,b.h_avg,b.nilai_hpp as hpp,c.kode_pp "+
							 "from brg_barang a "+					 
