window.app_saku3_transaksi_produk_fPjptgProyek = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_produk_fPjptgProyek.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_produk_fPjptgProyek";
		...
		this.e_akunbeban = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,18,200,20],caption:"Akun Beban", tag:2, readOnly:true });			
		this.e_drkbeban = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,29,200,20],caption:"DRK Beban", tag:2, readOnly:true });			
		this.e_saldogar = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,19,200,20],caption:"Saldo Budget", tag:2, tipeText:ttNilai, text:"0",readOnly:true });		
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:6,tag:9,
		            colTitle:["Ket. Peruntukan","Nilai","PPN Masukan","Pot. PPh23","Pot. PPh21","Total"],					
...
window.app_saku3_transaksi_produk_fPjptgProyek.extend(window.childForm);
window.app_saku3_transaksi_produk_fPjptgProyek.implement({
	doHitungGar: function(){	
		var data = this.dbLib.getDataProvider("select status_gar from masakun where kode_akun='"+this.akunBeban+"' and kode_lokasi='"+this.app._lokasi+"'",true);
		if (typeof data == "object"){
		...
		if (this.statusGar == "1") {
			var sakhir = 0;
			if (this.stsSimpan == 1) var data = this.dbLib.getDataProvider("select fn_release1('"+this.app._lokasi+"','"+this.e_akunbeban.getText()+"','"+this.cb_pp.getText()+"','"+this.e_drkbeban.getText()+"','"+this.e_periode.getText()+"') as gar ",true);
			else var data = this.dbLib.getDataProvider("select fn_release2('"+this.app._lokasi+"','"+this.e_akunbeban.getText()+"','"+this.cb_pp.getText()+"','"+this.e_drkbeban.getText()+"','"+this.e_periode.getText()+"','"+this.e_nb.getText()+"') as gar ",true);
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
						...
						for (var i=0;i < this.sg.getRowCount();i++) {
							if (this.sg.rowValid(i)){
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.nb+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.akunBeban+"','D',"+parseNilai(this.sg.cells(1,i))+","+parseNilai(this.sg.cells(1,i))+",'"+this.sg.cells(0,i)+"','"+this.modul+"','BEBAN','IDR',1,'"+this.cb_pp.getText()+"','"+this.drkBeban+"','-','-','-','-','-','-','-')");
								
								if (this.sg.cells(2,i) != "0") {
									//PPNM
									var j = 1000+i;
									sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
											"('"+this.nb+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',"+j+",'"+this.akunPPNM+"','D',"+parseNilai(this.sg.cells(2,i))+","+parseNilai(this.sg.cells(2,i))+",'PPN Masukan : "+this.cb_proyek.getText()+"','"+this.modul+"','PPNM','IDR',1,'"+this.cb_pp.getText()+"','-','-','-','-','-','-','-','-')");
								}
								if (this.sg.cells(3,i) != "0") {
									//PPH21
									var k = 2000+i;
									sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
											"('"+this.nb+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',"+k+",'"+this.hutPPh21+"','C',"+parseNilai(this.sg.cells(3,i))+","+parseNilai(this.sg.cells(3,i))+",'Hutang PPh21 : "+this.cb_proyek.getText()+"','"+this.modul+"','HUTPPH21','IDR',1,'"+this.cb_pp.getText()+"','-','-','-','-','-','-','-','-')");
								}
								if (this.sg.cells(4,i) != "0") {
									//PPH23
									var l = 3000+i;
									sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
											"('"+this.nb+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.cb_proyek.getText()+"','"+this.dp_d1.getDateString()+"',"+l+",'"+this.hutPPh23+"','C',"+parseNilai(this.sg.cells(4,i))+","+parseNilai(this.sg.cells(4,i))+",'Hutang PPh23 : "+this.cb_proyek.getText()+"','"+this.modul+"','HUTPPH23','IDR',1,'"+this.cb_pp.getText()+"','-','-','-','-','-','-','-','-')");
								}
							}
						}
					}					
					...
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_gar,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+												
							"('"+this.nb+"','PJPTG','"+this.app._lokasi+"','"+this.e_akunbeban.getText()+"','"+this.cb_pp.getText()+"','"+this.e_drkbeban.getText()+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','C',"+parseNilai(this.e_saldogar.getText())+","+parseNilai(this.e_nilaiptg.getText())+")");
	...
	doChange:function(sender){
		...
		if (sender == this.cb_pj && this.cb_pj.getText()!="") {
			var data = this.dbLib.getDataProvider(
					   "select convert(varchar,a.tanggal,103) as tanggal,a.akun_pj,b.nama as nama_pj,a.nilai,a.periode,a.kode_pp,c.nama as nama_pp,a.no_dokumen,e.akun_beban,d.kode_drkb "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){
					...
					this.e_akunbeban.setText(line.akun_beban);
