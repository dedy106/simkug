window.app_saku3_transaksi_produk_fBonusBarang = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_produk_fBonusBarang.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_produk_fBonusBarang";
		this.itemsValue = new arrayList();
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Bonus Jual Barang", 0);	
	...
	//	this.e_periode = new portalui_saiLabelEdit(this,{bound:[20,11,200,20],caption:"Periode",tag:2,change:[this,"doChange"]});
		...
		this.pc1 = new pageControl(this,{bound:[20,12,900,300], childPage:["Bonus Barang","Error Msg"]});
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:9,
					colTitle:["Kode Barang","Nama Barang","Tgl Mulai","Tgl Selesai","Jumlah Min.","Brg Bonus","Nama Barang","Jml Bonus","Validasi"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[100,80,150,100,80,120,120,150,100]],
					colFormat:[[4,7],[cfNilai,cfNilai]],
					buttonStyle:[[0,2,3,5],[bsEllips,bsDate,bsDate,bsEllips]],
...
window.app_saku3_transaksi_produk_fBonusBarang.extend(window.childForm);
window.app_saku3_transaksi_produk_fBonusBarang.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					for (var i=0; i < this.sg1.getRowCount();i++){	
						sql.add("insert into brg_bonusjual_d (kode_barang,kode_lokasi,kode_pp,tgl_mulai,tgl_selesai,jml_min,kode_barang_bonus,jml_bonus,tgl_input) values "+
								"('"+this.sg1.cells(0,i)+"','"+this.app._lokasi+"','"+this.app._kodePP+"','"+this.sg1.getCellDateValue(2,i)+"','"+this.sg1.getCellDateValue(3,i)+"',"+nilaiToFloat(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"',"+nilaiToFloat(this.sg1.cells(7,i))+",getdate())");
					}
