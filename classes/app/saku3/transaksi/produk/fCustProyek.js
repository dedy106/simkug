window.app_saku3_transaksi_produk_fCustProyek = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_produk_fCustProyek.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_produk_fCustProyek";
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:5,tag:9,
		            colTitle:["Kode","Nama","Inisial","Alamat","PIC"],
					colWidth:[[4,3,2,1,0],[200,200,100,300,80]],
					readOnly:true,
		...
		this.e_nama2 = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,16,200,20],caption:"Inisial", maxLength:50, tag:1});
		this.cb_piutang = new saiCBBL(this.pc1.childPage[1],{bound:[20,12,220,20],caption:"Akun Rekonsiliasi", multiSelection:false, maxLength:10, tag:2});		
...
window.app_saku3_transaksi_produk_fCustProyek.extend(window.childForm);
window.app_saku3_transaksi_produk_fCustProyek.implement({
	doCari: function() {
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into cust(kode_cust,kode_lokasi,nama,kode_klp,jenis,  alamat,kota,no_tel,no_fax,npwp,pic,departemen,email,no_hp,no_fax2,alamat2,kode_tagih,kode_group,kode_pp,inisial, akun_piutang) values "+
							"('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','-','-', '"+this.e_alamat.getText()+"','"+this.e_kota.getText()+"','"+this.e_tel.getText()+"','"+this.e_fax.getText()+"','"+this.e_npwp.getText()+"','"+this.e_pic.getText()+"','"+this.e_dep.getText()+"','"+this.e_email.getText()+"','"+this.e_nohp.getText()+"','"+this.e_fax2.getText()+"','"+this.e_alamat.getText()+"','"+this.cb_kode.getText()+"','-','"+this.app._kodePP+"','"+this.e_nama2.getText()+"', '"+this.cb_piutang.getText()+"')");														
					setTipeButton(tbAllFalse);
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_cust,line.nama,line.inisial,line.alamat,line.pic]); 
		}
