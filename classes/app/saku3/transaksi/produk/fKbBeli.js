window.app_saku3_transaksi_produk_fKbBeli = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_produk_fKbBeli.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_produk_fKbBeli";
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:6,tag:0,
				colTitle:["Status","No Referensi","Tanggal","Keterangan","Akun Hutang","Tagihan"],
...
window.app_saku3_transaksi_produk_fKbBeli.extend(window.childForm);
window.app_saku3_transaksi_produk_fKbBeli.implement({
	doLoad : function() {
		this.pc2.setActivePage(this.pc2.childPage[0]);	
		this.pc1.setActivePage(this.pc1.childPage[0]);																		
		if (this.stsSimpan==1 && this.cb_vendor.getText()!="") {		
			var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.kode_akun as akun_hutang,a.nilai-isnull(b.bayar,0) as saldo  "+						  
						 "from trans_j a "+

						 "     left join ("+
						 "          select no_selesai,sum(case dc when 'D' then nilai else -nilai end) as bayar  "+
						 "          from trans_j where kode_vendor='"+this.cb_vendor.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
						 "			group by no_selesai,kode_lokasi "+
						 ") b on a.no_bukti=b.no_selesai "+
						 
						 "where a.dc='C' and a.kode_vendor='"+this.cb_vendor.getText()+"' and (a.nilai-isnull(b.bayar,0)) > 0 and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_bukti";						 
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg1.appendData(["INPROG",line.no_bukti,line.tgl,line.keterangan,line.akun_hutang,floatToNilai(line.saldo)]);					
				}
		...
		if (col == 7) {
			if (sender.cells(7,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined) {						
						var data = this.dbLib.getDataProvider("select nama from brg_vendor where kode_vendor='"+sender.cells(7,row)+"' and kode_lokasi='"+this.app._lokasi+"'",true);
						if (typeof data == "object"){
							var line = data.rs.rows[0];							
							if (line != undefined) {
			...
			if (sender == this.sg) {
				...
				if (col == 7 && sender.cells(0,row) != "") {					
					...
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
						if (line != undefined) {
							var strSQL1 = "select kode_vendor, nama from brg_vendor where kode_lokasi = '"+this.app._lokasi+"'";
							var strSQL2 = "select count(*) from brg_vendor where kode_lokasi = '"+this.app._lokasi+"'";
						} 
						else {
							var strSQL1 = "select '-' as kode_vendor, '-' as nama ";
							var strSQL2 = "select count(*) from brg_vendor a where kode_vendor='-'";
						}
					}										
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai1 "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.form = 'KBBELI' and a.posted ='F'";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						this.cb_vendor.setSQL("select kode_vendor, nama from brg_vendor where kode_vendor='"+line.param2+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_vendor","nama"],false,["Kode","Nama"],"and","Data Vendor",true);				
						this.cb_vendor.setText(line.param2);										
					}
				}				
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg1.appendData(["BAYAR",line.no_bukti,line.tgl,line.keterangan,line.akun_hutang,floatToNilai(line.saldo)]);					
					}
				} else this.sg1.clear(1);
				...
				var strSQL = "select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_vendor,isnull(d.nama,'-') as nama_vendor "+
							 ...
							 "left join brg_vendor d on a.kode_vendor=d.kode_vendor and a.kode_lokasi=d.kode_lokasi "+							 
