window.app_saku3_transaksi_produk_fKbJual = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_produk_fKbJual.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_produk_fKbJual";
		...
		try {
			...
			this.cb_cust.setSQL("select kode_cust, nama from brg_cust where kode_lokasi='"+this.app._lokasi+"'",["kode_cust","nama"],false,["Kode","Nama"],"and","Data Customer",true);						
...
window.app_saku3_transaksi_produk_fKbJual.extend(window.childForm);
window.app_saku3_transaksi_produk_fKbJual.implement({
	doLoad : function() {
		...
		if (this.stsSimpan==1 && this.cb_cust.getText()!="") {		
			var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.kode_akun as akun_piutang,a.nilai-isnull(b.bayar,0) as saldo  "+						  
						 ...
						 "where a.dc='D' and a.kode_cust='"+this.cb_cust.getText()+"' and (a.nilai-isnull(b.bayar,0)) > 0 and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_bukti";						 
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "" && this.c_jenis.getText()!= "") {
			...
			this.stsSimpan = 1;			
			this.cb_cust.setSQL("select kode_cust, nama from brg_cust where kode_lokasi='"+this.app._lokasi+"'",["kode_cust","nama"],false,["Kode","Nama"],"and","Data Cust",true);
		...
		if (col == 7) {
			if (sender.cells(7,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined) {						
						var data = this.dbLib.getDataProvider("select nama from brg_cust where kode_cust='"+sender.cells(7,row)+"' and kode_lokasi='"+this.app._lokasi+"'",true);
						if (typeof data == "object"){
							var line = data.rs.rows[0];							
							if (line != undefined) {
			...
			if (sender == this.sg) {
				...
				if (col == 7 && sender.cells(0,row) != "") {					
					...
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
						if (line != undefined) {
							var strSQL1 = "select kode_cust, nama from brg_cust where kode_lokasi = '"+this.app._lokasi+"'";
							var strSQL2 = "select count(*) from brg_cust where kode_lokasi = '"+this.app._lokasi+"'";
						} 
						else {
							var strSQL1 = "select '-' as kode_cust, '-' as nama ";
							var strSQL2 = "select count(*) from brg_cust a where kode_cust='-'";
						}
					}										
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai1 "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.form = 'KBJUAL' and a.posted ='F'";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						this.cb_cust.setSQL("select kode_cust, nama from brg_cust where kode_cust='"+line.param2+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_cust","nama"],false,["Kode","Nama"],"and","Data Customer",true);				
						this.cb_cust.setText(line.param2);										
					}
				}				
				...
				var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.kode_akun as akun_piutang,a.nilai-isnull(b.bayar,0) as saldo  "+						  
							 ...
							 "where a.dc='D' and a.kode_cust='"+this.cb_cust.getText()+"' and (a.nilai-isnull(b.bayar,0)) > 0 and a.kode_lokasi='"+this.app._lokasi+"' and c.no_bukti='"+this.e_nb.getText()+"' order by a.no_bukti";						 
				...
				var strSQL = "select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_cust,isnull(d.nama,'-') as nama_cust "+
							 ...
							 "left join brg_cust d on a.kode_cust=d.kode_cust and a.kode_lokasi=d.kode_lokasi "+							 
