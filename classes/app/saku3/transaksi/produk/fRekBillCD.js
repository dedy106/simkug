window.app_saku3_transaksi_produk_fRekBillCD = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_produk_fRekBillCD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_produk_fRekBillCD";
		...
		this.pc1 = new pageControl(this,{bound:[20,12,900,300], childPage:["Data Tagihan","Data PDD"]});
		...
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[1],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:2,
		...
		this.bLoadCD = new portalui_button(this.sgn1,{bound:[780,1,80,18],caption:"Data PDD",click:[this,"doLoadCD"]});		
...
window.app_saku3_transaksi_produk_fRekBillCD.extend(window.childForm);
window.app_saku3_transaksi_produk_fRekBillCD.implement({
	doLoadCD: function(sender,totalRow){
		try {
			var strSQL = "select a.nis,sum(case a.dc when 'D' then a.nilai else -a.nilai end) as saldo_cd "+
		...
		try {
			...
			*/
			...
			for (var i=0; i < this.dataJU1.rs.rows.length;i++){
				var nilaiAwal = parseFloat(this.dataJU1.rs.rows[i].saldo_cd);
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','V','LOADCD','X','0','0','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','-','"+this.e_ket.getText()+
							"','IDR',1,"+total+",0,0,'-','-','-','-','-','-','"+this.cb_titip.getText()+"','-','-')");
					
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.cb_titip.getText()+"','D',"+total+","+total+",'"+this.e_ket.getText()+"','V','LOADCD','IDR',1,'"+this.cb_pp.getText()+"','-','-','-','-','-','-','-','-')");
					...
					for (var i in this.gridAR.objList){
						...
						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+line.get("kode_akun")+"','C',"+parseFloat(line.get("nilai"))+","+parseFloat(line.get("nilai"))+",'"+this.e_ket.getText()+"','V','LOADCD','IDR',1,'"+this.cb_pp.getText()+"','-','-','-','-','-','-','-','-')");
					}		
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){							
							this.nama_report="server_report_saku3_produk_rptSisJurnalLunas";
