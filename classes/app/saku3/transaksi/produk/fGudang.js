window.app_saku3_transaksi_produk_fGudang = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_produk_fGudang.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_produk_fGudang";
...
window.app_saku3_transaksi_produk_fGudang.extend(window.childForm);
window.app_saku3_transaksi_produk_fGudang.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					...
					sql.add("delete from brg_pp where kode_pp = '"+this.cb_kode.getText()+"' ");
					sql.add("insert into brg_gudang(kode_gudang,kode_lokasi,nama,pic,telp,alamat,kode_pp) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.cb_pic.getText()+"','"+this.e_telp.getText()+"','"+this.e_alamat.getText()+"','"+this.cb_pp.getText()+"')");
					
					sql.add("insert into brg_pp(kode_pp,kode_lokasi,flag_aktif,nama) values "+
