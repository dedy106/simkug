window.app_saku3_transaksi_produk_fRRApp = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_produk_fRRApp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_produk_fRRApp";
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,430], childPage:["Data RRA","Detail RRA"]});
		...
		this.sg2 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,320],colCount:8,
					colTitle:["Bulan","Kode PP","Nama PP","Kode Akun","Nama Akun","MTA","Jenis","Nilai"],
					colWidth:[[7,6,5,4,3,2,1,0],[100,80,80,180,80,180,80,80]],
...
window.app_saku3_transaksi_produk_fRRApp.extend(window.childForm);
window.app_saku3_transaksi_produk_fRRApp.implement({
				...
				try{				
					...
					if (this.c_status.getText() == "APPROVE") {
						...
						sql.add("insert into anggaran_d (no_agg,kode_lokasi,no_urut,kode_pp,kode_akun,kode_drk,volume,periode,nilai_sat,nilai,dc,satuan,nik_user,tgl_input,modul)  "+		
								"select a.no_rra,a.kode_lokasi,a.no_urut,a.kode_pp,a.kode_akun,a.kode_drk,1,a.periode,a.nilai,a.nilai,a.dc,'-',b.nik_user,getdate(),b.modul "+
								"from angg_rra_d a inner join angg_rra_m b on a.no_rra=b.no_rra "+ 
								"where a.no_rra = '"+this.e_nopdrk.getText()+"' and a.dc ='D'"); 
					}
					else {
						var vProg = "X";
					...
					sql.add("update angg_rra_m set prog_seb=progress,progress='"+vProg+"',no_ref1='"+this.e_nb.getText()+"' where no_rra='"+this.e_nopdrk.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");							
	...
	doClick:function(sender){
		if (sender == this.i_gen) {
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"rra_app_m","no_app",this.app._lokasi+"-APV"+this.e_periode.getText().substr(2,4)+".","0000"));
			this.e_ket.setFocus();
		}		
	},	
	doLoad:function(sender){		
		var strSQL = "select a.kode_lokasi,a.kode_pp+' - '+c.nama as pp,a.no_rra as no_bukti,a.no_dokumen,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,a.modul "+
					 "from angg_rra_m a inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
					 "where a.progress='0' and a.periode<='"+this.e_periode.getText()+"' ";							
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData([line.kode_lokasi,line.pp,line.no_bukti,line.no_dokumen,line.tanggal,line.keterangan,line.modul]);
		}
	...
	doDoubleClick: function(sender, col , row) {
		if (this.sg.cells(1,row) != "") {
			...
			this.sg2.clear();
			var strSQL = "select substring(a.periode,5,2) as bulan,a.kode_pp,b.nama as nama_pp,a.kode_akun,c.nama as nama_akun,c.kode_gar,a.nilai,case a.dc when 'D' then 'PENERIMA' else 'PEMBERI' end as dc  "+
						 "from angg_rra_d a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
						 "				    inner join masakun c on a.kode_akun=c.kode_akun and a.kode_lokasi=c.kode_lokasi "+						 
						 "where a.no_rra = '"+this.e_nopdrk.getText()+"'  order by a.dc";					
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg2.appendData([line.bulan,line.kode_pp,line.nama_pp,line.kode_akun,line.nama_akun,line.kode_gar,line.dc.toUpperCase(),floatToNilai(line.nilai)]);
				}
