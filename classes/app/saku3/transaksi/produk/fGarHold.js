window.app_saku3_transaksi_produk_fGarHold = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_produk_fGarHold.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_produk_fGarHold";
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:7,tag:0,
		            colTitle:["Kode MTA","Nama MTA","Kode PP","Nama PP","Periode","Sisa Budget","Nilai Release"],
					colWidth:[[6,5,4,3,2,1,0],[100,100,  80,200,80,250,80]],
...
window.app_saku3_transaksi_produk_fGarHold.extend(window.portalui_childForm);
window.app_saku3_transaksi_produk_fGarHold.implement({	
	doTampilClick: function() {
		...
		var strSQL = "select a.*, "+
					...
					"group by "+
					"a.kode_lokasi,c.kode_gar,c.nama,a.kode_pp,d.nama,a.periode1 "+
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData([line.kode_gar,line.nama_gar, line.kode_pp,line.nama_pp, line.periode1, floatToNilai(line.sisa_gar), floatToNilai(line.nilai)]);
		}
							...
							for (var i=0;i < this.dataJU.rs.rows.length;i++){
								...
								if (parseFloat(line.nilai) != 0){
									sql.add("insert into angg_release_d(no_bukti,kode_lokasi,kode_gar,kode_pp,kode_drk,periode,dc,nilai) values "+
											"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+line.kode_gar+"','"+line.kode_pp+"','-','"+line.periode1+"','C',"+parseFloat(line.nilai)+")");									
								}
							}	
