window.app_saku3_transaksi_produk_fNilai = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_produk_fNilai.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_produk_fNilai";
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,480], childPage:["Data Penilaian","Daftar Penilaian"]});				
		...
		this.e_ke = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,18,150,20],caption:"Penilaian Ke-",tag:1, readOnly:true, tipeText:ttNilai,text:"0"});
		this.bTampil = new portalui_button(this.pc1.childPage[0],{bound:[818,18,80,18],caption:"Load Data",click:[this,"doLoadData1"]});		
		this.bRekon = new button(this.pc1.childPage[0],{bound:[913,18,80,18],caption:"Rekon Nilai", click:[this,"doRekon"]});
		
		this.pc2 = new pageControl(this.pc1.childPage[0],{bound:[5,12,990,330], childPage:["Data Siswa","Data Nilai","Error Msg"]});		
		this.sg = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:4,tag:2,
		...
		try {
			...
			this.cb_kelas.setSQL("select kode_kelas, nama from sis_kelas where kode_lokasi = '"+this.app._lokasi+"'",["kode_kelas","nama"],false,["Kode Kelas","Nama"],"and","Data Kelas",true);			
			this.cb_matpel.setSQL("select kode_matpel, nama from sis_matpel where kode_lokasi = '"+this.app._lokasi+"'",["kode_matpel","nama"],false,["Kode Matpel","Nama"],"and","Data Matpel",true);			
			this.cb_jenis.setSQL("select kode_jenis, nama from sis_jenisnilai where kode_lokasi = '"+this.app._lokasi+"'",["kode_jenis","nama"],false,["Kode Jenis Nilai","Nama"],"and","Data Jenis Nilai",true);			
...
window.app_saku3_transaksi_produk_fNilai.extend(window.childForm);
window.app_saku3_transaksi_produk_fNilai.implement({
