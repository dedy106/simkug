window.app_saku3_transaksi_produk_fReturJual = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_produk_fReturJual.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_produk_fReturJual";
...
window.app_saku3_transaksi_produk_fReturJual.extend(window.portalui_childForm);
window.app_saku3_transaksi_produk_fReturJual.implement({
	doJurnal : function() {
		if (this.cb_cust.getText() != "") {
			var data = this.dbLib.getDataProvider("select akun_piutang from brg_cust where kode_cust ='"+this.cb_cust.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'",true);			
			if (typeof data == "object"){
				var line = data.rs.rows[0];					
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "") {
			...
			this.stsSimpan = 1;
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti",this.app._lokasi+"-RJ"+this.e_periode.getText().substr(2,4)+".","0000"));			
			this.e_dok.setFocus();
