window.app_saku3_transaksi_panjar_fPjptgSimple = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_panjar_fPjptgSimple.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_panjar_fPjptgSimple";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,293], childPage:["Data Panjar","Item Jurnal Pertanggungan","Data Anggaran"]});
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:9,tag:9,
		            colTitle:["Kode Akun","Nama Akun","DC","Keterangan","Nilai","Kode PP","Nama PP","Proyek","Nama Proyek"],
...
window.app_saku3_transaksi_panjar_fPjptgSimple.extend(window.childForm);
window.app_saku3_transaksi_panjar_fPjptgSimple.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into ptg_j(no_ptg,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.sg.cells(7,i)+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','-','"+this.app._lokasi+"','PJPTG','BEBAN','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");
							}
						}
					}
					...
					if (nilaiToFloat(this.e_nilaikb.getText()) != 0) {
						...
						sql.add("insert into ptg_j(no_ptg,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
								"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',98,'"+this.cb_akunkb.getText()+"','"+this.e_ket.getText()+"','"+DC+"',"+Math.abs(parseNilai(this.e_nilaikb.getText()))+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','PJPTG','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");
					}
					sql.add("insert into ptg_j(no_ptg,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
							...
							for (var i=0;i < this.sg.getRowCount();i++){
								if (this.sg.rowValid(i)){
									sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
											"	('"+this.e_kb.getText()+"','"+this.sg.cells(7,i)+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','-','-','-','"+this.app._lokasi+"','KBPTG','BEBAN','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");
								}
							}
						}
						if (this.c_jenis.getText() == "BK") var DC = "C"; else var DC = "D"; 
						sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
								"	('"+this.e_kb.getText()+"','-','"+this.dp_d1.getDateString()+"',98,'"+this.cb_akunkb.getText()+"','"+this.e_ket.getText()+"','"+DC+"',"+Math.abs(parseNilai(this.e_nilaikb.getText()))+",'"+this.cb_pp.getText()+"','-','-','-','"+this.app._lokasi+"','KBPTG','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");
						sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.nb+"','PJPTG','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(4,i))+","+nilai+")");
							}
						}
					}					
	...
	doChange:function(sender){
		...
		if (sender == this.e_nilaiptg) {
			...
			if (this.c_jenis.getText()!="" && this.e_nilaikb.getText()!="") {
				if (nilaiToFloat(this.e_nilaikb.getText()) != 0) {
					if (this.stsSimpan == 1) this.e_kb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"kas_m","no_kas",this.app._lokasi+"-"+this.c_jenis.getText()+this.e_periode.getText().substr(2,4)+".","0000"));
				}
				else this.e_kb.setText("");
			}
		}
	...
	doChangeCell: function(sender, col, row){
		...
		if (col == 7) {
			if (sender.cells(7,row) != "") {
				var pr = this.dataPR.get(sender.cells(7,row));
			...
			if (sender == this.sg) {
				...
				if (col == 7){
					this.standarLib.showListData(this, "Daftar Proyek",sender,undefined, 
												  "select kode_proyek, nama  from spm_proyek where kode_lokasi = '"+this.app._lokasi+"' and flag_aktif ='1'  union select '-','-' ",
												  "select count(kode_proyek)  from spm_proyek where kode_lokasi = '"+this.app._lokasi+"' and flag_aktif ='1'",
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_ptg, convert(varchar,a.tanggal,103) as tgl, a.keterangan from ptg_m a where a.nilai_kas=0 and a.posted = 'F' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' "+
					 "union "+
					 "select a.no_ptg, convert(varchar,a.tanggal,103) as tgl, a.keterangan from ptg_m a inner join kas_m b on a.no_kas=b.no_kas and a.kode_lokasi=b.kode_lokasi where b.no_del='-' and a.nilai_kas<>0 and b.posted = 'F' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' ";		
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_ptg,line.tgl,line.keterangan]); 
		}
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider(
							"select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,d.kode_proyek,isnull(d.nama,'-') as nama_proyek "+
							"from ptg_j a inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
							"             inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+
							"             left join spm_proyek d on a.no_dokumen=d.kode_proyek and a.kode_lokasi=d.kode_lokasi "+
