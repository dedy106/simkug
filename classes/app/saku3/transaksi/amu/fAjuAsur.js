window.app_saku3_transaksi_amu_fAjuAsur = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_amu_fAjuAsur.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_amu_fAjuAsur";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan Asuransi Aset", 0);	
		...
		this.cb_jasur = new portalui_saiCBBL(this,{bound:[20,12,220,20],caption:"Jenis Asuransi",tag:2,multiSelection:false});         		
		this.cb_vendor = new portalui_saiCBBL(this,{bound:[20,13,220,20],caption:"Mitra Asuransi",tag:2,multiSelection:false,change:[this,"doChange"]});         		
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,247], childPage:["Data Aset","Item Beban","Data Anggaran","Data Polis"]});			
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:6,tag:0,
					colTitle:["ID Aset","Nama Aset","Merk","Type","No Seri","Nilai Perolehan"],
					colWidth:[[5,4,3,2,1,0],[100,100,200,200,250,100]],					
		...
		this.e_polis = new saiLabelEdit(this.pc1.childPage[3],{bound:[20,18,400,20],caption:"No Polis", maxLength:50});
		this.l_tgl2 = new portalui_label(this.pc1.childPage[3],{bound:[20,13,98,18],caption:"Tanggal Mulai", underline:true});
		this.dp_d2 = new portalui_datePicker(this.pc1.childPage[3],{bound:[120,13,100,18]}); 
		this.l_tgl3 = new portalui_label(this.pc1.childPage[3],{bound:[20,14,100,18],caption:"Tanggal Selesai", underline:true});
		this.dp_d3 = new portalui_datePicker(this.pc1.childPage[3],{bound:[120,14,98,18]}); 
		this.e_premi = new saiLabelEdit(this.pc1.childPage[3],{bound:[20,19,200,20],caption:"Premi", tag:1, tipeText:ttNilai, text:"0",change:[this,"doChange"]});
		this.e_diskon = new saiLabelEdit(this.pc1.childPage[3],{bound:[20,11,200,20],caption:"Discount", tag:1, tipeText:ttNilai, text:"0",change:[this,"doChange"]});
		this.e_afdis = new saiLabelEdit(this.pc1.childPage[3],{bound:[20,12,200,20],caption:"After Disc", tag:1, readOnly:true, tipeText:ttNilai, text:"0",change:[this,"doChange"]});
		this.e_admin = new saiLabelEdit(this.pc1.childPage[3],{bound:[20,13,200,20],caption:"Admin", tag:1, tipeText:ttNilai, text:"0",change:[this,"doChange"]});
		this.e_total2 = new saiLabelEdit(this.pc1.childPage[3],{bound:[20,14,200,20],caption:"Total", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});
		...
		try {
			...
			this.cb_jasur.setSQL("select id_jenis_asur,nama_jenis from amu_jenis_asuransi where kode_lokasi='"+this.app._lokasi+"'",["id_jenis_asur","nama_jenis"],false,["ID","Nama"],"and","Data Jenis",true);													
			this.cb_vendor.setSQL("select a.kode_vendor,a.nama from vendor a inner join vendor_klp b on a.kode_klpvendor=b.kode_klpvendor and a.kode_lokasi=b.kode_lokasi and b.kode_klpvendor='016' where a.kode_lokasi='"+this.app._lokasi+"'",["kode_vendor","nama"],false,["ID","Nama"],"and","Data Mitra",true);													
...
window.app_saku3_transaksi_amu_fAjuAsur.extend(window.childForm);
window.app_saku3_transaksi_amu_fAjuAsur.implement({		
	doEllipsClick: function(sender, col, row){
		try{			
			if (sender == this.sg) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Aset",sender,undefined, 
						    "select no_bukti,nama_inv from amu_asset_bergerak where kode_lokasi='"+this.app._lokasi+"'",
							"select count(*) from amu_asset_bergerak where kode_lokasi='"+this.app._lokasi+"'",
							["no_bukti","nama"],"and",["ID","Nama"],false);				
				}				
			}
		}catch(e){
			systemAPI.alert(e);
	...
	doChangeCell: function(sender, col, row){
	    ...
	    if (col == 0) {
			if (this.sg.cells(0,row) != "") {
				var data = this.dbLib.getDataProvider("select * from amu_asset_bergerak where no_bukti='"+this.sg.cells(0,row)+"' and kode_lokasi='"+this.app._lokasi+"'", true);
				if (typeof data == "object") {
					var line = data.rs.rows[0];
					if (line != undefined) {      					       
						this.sg.cells(1,row,line.nama_inv);                                                 
						this.sg.cells(2,row,line.merk);
						this.sg.cells(3,row,line.tipe);
						this.sg.cells(4,row,line.no_seri);    
						this.sg.cells(5,row,floatToNilai(line.nilai_perolehan));                       
					}
				}					
			}
		}		
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into amu_aset_asur (no_aju,periode,kode_lokasi,id_jenis_asur,id_perus,no_aset,keterangan,nilai, no_polis,tgl_awal,tgl_akhir,premi,diskon,admin) values "+
							"('"+this.e_nb.getText()+"','"+this.e_periode.getText()+"','"+this.app._lokasi+"','"+this.cb_jasur.getText()+"','"+this.cb_vendor.getText()+"','X','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_total.getText())+", '"+this.e_polis.getText()+"','"+this.dp_d2.getDateString()+"','"+this.dp_d3.getDateString()+"',"+nilaiToFloat(this.e_premi.getText())+","+nilaiToFloat(this.e_diskon.getText())+","+nilaiToFloat(this.e_admin.getText())+")");

					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){		
							sql.add("insert into amu_aset_asur_d (no_aju,kode_lokasi,no_aset) values "+
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "simpan" :					
				...
				if (nilaiToFloat(this.e_total.getText()) != nilaiToFloat(this.e_total2.getText())) {
					system.alert(this,"Transaksi tidak valid.","Total premi dan jurnal jurnal tidak sama");
					return false;
				}
	...
	doChange:function(sender){
		try {
			...
			if ((sender == this.e_premi || sender == this.e_diskon || sender == this.e_admin) && this.e_premi.getText()!="" && this.e_diskon.getText()!="" && this.e_admin.getText()!="") {
				var afdis = nilaiToFloat(this.e_premi.getText()) - nilaiToFloat(this.e_diskon.getText()) ; 
				var tot = afdis + nilaiToFloat(this.e_admin.getText()) ; 
				this.e_afdis.setText(floatToNilai(afdis));
				this.e_total2.setText(floatToNilai(tot));
			}
		}
		catch(e) {
			alert(e);
