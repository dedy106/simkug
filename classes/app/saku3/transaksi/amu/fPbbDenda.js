window.app_saku3_transaksi_amu_fPbbDenda = function(owner) {
    if (owner) {
        window.app_saku3_transaksi_amu_fPbbDenda.prototype.parent.constructor.call(this, owner);
        this.className = "app_saku3_transaksi_amu_fPbbDenda";
        this.itemsValue = new arrayList();
        this.app._mainForm.childFormConfig(this, "mainButtonClick", "Data Pajak Bumi dan Bangunan Denda", 0);
        ...
        this.e_nilai = new saiLabelEdit(this.pc1.childPage[1], { bound: [20, 21, 200, 20], caption: "Nilai Denda", tag: 1, tipeText: ttNilai, text: "0", maxLength: 20 });        
...
window.app_saku3_transaksi_amu_fPbbDenda.extend(window.childForm);
window.app_saku3_transaksi_amu_fPbbDenda.implement({
    isiCBnop: function() {
        this.cb_nop.setSQL("select b.nop,b.tahun from amu_pbb_d b  "+
                           "left join amu_pbb_denda c on b.no_bukti=c.no_sppt "+
        ...
        try {
            if (this.standarLib.checkEmptyByTag(this, [0, 1])) {
                try {
                    ...
                    sql.add("INSERT INTO amu_pbb_denda (no_denda, nop, tahun, no_pbb, no_sppt, kode_lokasi, tanggal, keterangan, nilai) VALUES "+
        ...
        try {
            if (this.standarLib.checkEmptyByTag(this, [0, 1])) {
                try {
                    uses("server_util_arrayList");
                    var sql = new server_util_arrayList();
                    sql.add("delete from amu_pbb_denda where no_denda = '" + this.cb_kode.getText() + "' and kode_lokasi='"+this.app._lokasi+"'");
                    sql.add("INSERT INTO amu_pbb_denda (no_denda, nop, tahun, no_pbb, no_sppt, kode_lokasi, tanggal, keterangan, nilai) VALUES "+
                            "('"+this.cb_kode.getText()+"', '"+this.cb_nop.getText() +"','"+this.cb_nop.rightLabelCaption+"', '"+this.e_idpbb.getText()+"', '"+this.e_sppt.getText()+"', '"+this.app._lokasi+"', '"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"', "+nilaiToFloat(this.e_nilai.getText())+")");
    ...
    doChange: function(sender) {        
        try {           
            ...
            if (sender == this.cb_kode && this.cb_kode.getText() != "") {            
                var strSQL = "select * from amu_pbb_denda where no_denda ='" + this.cb_kode.getText() + "' and kode_lokasi='"+this.app._lokasi+"'";                                
                var data = this.dbLib.getDataProvider(strSQL, true);
                if (typeof data == "object") {
                    var line = data.rs.rows[0];
                    if (line != undefined) {     
    ...
    doLoad: function(sender) {
        ...
        var strSQL = "select a.no_denda, a.no_pbb,b.nop, a.tahun " +
                     "from  amu_pbb_denda a " +
    ...
    doTampilData: function(page) {
        ...
        for (var i = start; i < finish; i++) {
            ...
            this.sg1.appendData([line.no_denda, line.no_pbb, line.nop, line.tahun, "Pilih"]);
        }
    ...
    doCari: function(sender) {
        try {
            var show = parseInt(this.c_show.getText());
            var column_array = ['a.no_denda', 'a.no_pbb', 'a.nop', 'a.tahun'];
    ...
    doSort: function(sender, col, row) {
        try {
            ...
            if (col == 4) {
                ...
                var column_array = ['a.no_denda', 'a.no_pbb', 'b.nop', 'a.tahun'];
    ...
    doClick:function(sender){
        try {            
            ...
            this.cb_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"amu_pbb_denda","no_denda",this.app._lokasi+"-DND"+this.periode.substr(2,2)+".","0000"));
            this.e_ket.setFocus();
            setTipeButton(tbSimpan);            
        }
        catch(e) {
