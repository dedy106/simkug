window.app_saku3_transaksi_amu_fKelAsset = function (owner) {
    if (owner) {
        window.app_saku3_transaksi_amu_fKelAsset.prototype.parent.constructor.call(this, owner);
        this.className = "app_saku3_transaksi_amu_fKelAsset";
        this.itemsValue = new arrayList();
        this.app._mainForm.childFormConfig(this, "mainButtonClick", "Data Kelompok Asset", 0);
        ...
        this.sg1 = new saiGrid(this.pc1.childPage[0], {
            ...
            colTitle: ["ID Kel", "Nama Kelompok", "Pilih"],
        ...
        this.e_ket = new saiLabelEdit(this.pc1.childPage[1], { bound: [20, 12, 500, 20], caption: "Keterangan", maxLength: 100, tag: 1 });
...
window.app_saku3_transaksi_amu_fKelAsset.extend(window.childForm);
window.app_saku3_transaksi_amu_fKelAsset.implement({
        ...
        try {
            if (this.standarLib.checkEmptyByTag(this, [0, 1])) {
                try {
                    ...
                    sql.add("insert into amu_klp_brg(kode_klp,nama_klp,kode_lokasi,keterangan) values " +
                            "('" + this.cb_kode.getText() + "','" + this.e_nama.getText() + "','" + this.app._lokasi + "','"+this.e_ket.getText()+"')");
        ...
        try {
            if (this.standarLib.checkEmptyByTag(this, [0, 1])) {
                try {
                    uses("server_util_arrayList");
                    var sql = new server_util_arrayList();
                    sql.add("delete from amu_klp_brg where kode_klp = '" + this.cb_kode.getText() + "' ");
                    sql.add("insert into amu_klp_brg(kode_klp,nama_klp,kode_lokasi,keterangan) values " +
                            "('" + this.cb_kode.getText() + "','" + this.e_nama.getText() + "','" + this.app._lokasi + "','"+this.e_ket.getText()+"')");
                    setTipeButton(tbAllFalse);
                    this.dbLib.execArraySQL(sql);
    ...
    doChange: function (sender) {
        try {
            if (this.cb_kode.getText() != "") {
                var strSQL = "select kode_klp,nama_klp,keterangan " +
                             "from amu_klp_brg where kode_klp ='" + this.cb_kode.getText() + "' and kode_lokasi ='" + this.app._lokasi + "'";
                var data = this.dbLib.getDataProvider(strSQL, true);
                if (typeof data == "object") {
                    var line = data.rs.rows[0];
                    if (line != undefined) {
                        this.e_nama.setText(line.nama_klp);
    ...
    doLoad: function (sender) {
        ...
        var strSQL = "select kode_klp, nama_klp from amu_klp_brg where kode_lokasi='" + this.app._lokasi + "' ";
        var data = this.dbLib.getDataProvider(strSQL, true);
    ...
    doTampilData: function (page) {
        ...
        for (var i = start; i < finish; i++) {
            ...
            this.sg1.appendData([line.kode_klp, line.nama_klp, "Pilih"]);
        }
    ...
    doCari: function (sender) {
        try {
            ...
            var show = parseInt(this.c_show.getText());
            var column_array = ['kode_klp', 'nama_klp'];
            ...
            var strSQL = "select kode_klp, nama_klp " +
                "from amu_klp_brg " +
