window.app_saku3_transaksi_amu_fKota = function (owner) {
	if (owner) {
		window.app_saku3_transaksi_amu_fKota.prototype.parent.constructor.call(this, owner);
...
window.app_saku3_transaksi_amu_fKota.extend(window.childForm);
window.app_saku3_transaksi_amu_fKota.implement({
		...
		try {
			if (this.standarLib.checkEmptyByTag(this, [0, 1])) {
				try {
					...
					sql.add("insert into amu_kota(id,nama,id_provinsi) values " +
							"('" + this.cb_kode.getText() + "','" + this.e_nama.getText() + "','" + this.cb_prov.getText() + "')");
		...
		try {
			if (this.standarLib.checkEmptyByTag(this, [0, 1])) {
				try {
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from amu_kota where id = '" + this.cb_kode.getText() + "' ");
					sql.add("insert into amu_kota(id,nama,id_provinsi) values " +
							"('" + this.cb_kode.getText() + "','" + this.e_nama.getText() + "','" + this.cb_prov.getText() + "')");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
	...
	doChange: function (sender) {
		try {
			if (sender == this.cb_kode && this.cb_kode.getText() != "") {
				var strSQL = "select id,nama,id_provinsi from amu_kota where id ='" + this.cb_kode.getText() + "'";
	...
	doLoad: function (sender) {
		...
		var strSQL = "select id, id_provinsi, nama from amu_kota ";
		var data = this.dbLib.getDataProvider(strSQL, true);
	...
	doTampilData: function (page) {
		...
		for (var i = start; i < finish; i++) {
			...
			this.sg1.appendData([line.id, line.id_provinsi, line.nama, "Pilih"]);
		}
	...
	doCari: function (sender) {
		try {
			...
			var strSQL = "select id, id_provinsi, nama from amu_kota " +"where" + filter_string;			
			var data = this.dbLib.getDataProvider(strSQL, true);
			if (typeof data == "object" && data.rs.rows[0] != undefined) {
				this.dataJU = data;
				this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length / show));
