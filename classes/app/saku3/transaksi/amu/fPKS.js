window.app_saku3_transaksi_amu_fPKS = function (owner) {
    if (owner) {
        window.app_saku3_transaksi_amu_fPKS.prototype.parent.constructor.call(this, owner);
        this.className = "app_saku3_transaksi_amu_fPKS";
        this.itemsValue = new arrayList();
        this.app._mainForm.childFormConfig(this, "mainButtonClick", "Data PKS", 0);
        ...
        this.cb_kode = new saiLabelEdit(this.pc1.childPage[1], { bound: [20, 10, 200, 20], caption: "ID PKS", maxLength: 10, change: [this, "doChange"],readOnly:true });        
        this.e_jenis = new saiLabelEdit(this.pc1.childPage[1], { bound: [20, 11, 300, 20], caption: "Jenis PKS", maxLength: 50, tag: 1 });
...
window.app_saku3_transaksi_amu_fPKS.extend(window.childForm);
window.app_saku3_transaksi_amu_fPKS.implement({
    doTambah: function() {		
		this.stsSimpan = 1;
		this.pc1.setActivePage(this.pc1.childPage[1]);	   
		this.cb_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib, "amu_pks", "id_pks",this.app._lokasi+"-PKS" + this.tahun.substr(2,2) + ".", "0000"));                    				
		this.e_jenis.setFocus();     																	
    },
            ...
            if (this.standarLib.checkEmptyByTag(this, [0, 1])) {
                try {
                    ...
                    sql.add("insert into amu_pks(id_pks,jenis,tanggal,no_pks,no_pks_mitra,kode_vendor,keterangan,tgl_mulai,tgl_selesai,kode_lokasi,id_amandemen,modul) values " +
                            "('" + this.cb_kode.getText() + "','"+this.e_jenis.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_nopks.getText()+"','"+this.e_nopks2.getText()+"','"+this.cb_vendor.getText()+"','"+this.e_ket.getText()+"','"+this.dp_d2.getDateString()+"','"+this.dp_d3.getDateString()+"', '" + this.app._lokasi + "','-','PKS')");
        ...
        try {
            if (this.standarLib.checkEmptyByTag(this, [0, 1])) {
                try {
                    ...
                    sql.add("insert into amu_pks(id_pks,jenis,tanggal,no_pks,no_pks_mitra,kode_vendor,keterangan,tgl_mulai,tgl_selesai,kode_lokasi,id_amandemen,modul) values " +
                            "('" + this.cb_kode.getText() + "','"+this.e_jenis.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_nopks.getText()+"','"+this.e_nopks2.getText()+"','"+this.cb_vendor.getText()+"','"+this.e_ket.getText()+"','"+this.dp_d2.getDateString()+"','"+this.dp_d3.getDateString()+"', '" + this.app._lokasi + "','"+this.idAmandemen+"','PKS')");
                    setTipeButton(tbAllFalse);
                    this.dbLib.execArraySQL(sql);
                }
                catch (e) {
    ...
    doModalResult: function (event, modalResult) {
        ...
        switch (event) {
            ...
            case "hapus":
                if (this.idAmandemen != "-") {
                    system.alert(this,"Transaksi tidak valid.","PKS sudah pernah di-amandemen.");
					return false;						
                }
                else this.hapus();
                break;
    ...
    doLoad: function (sender) {
        ...
        var strSQL = "select id_pks, keterangan from amu_pks where kode_lokasi='" + this.app._lokasi + "' and modul='PKS'";
        var data = this.dbLib.getDataProvider(strSQL, true);
    ...
    doCari: function (sender) {
        try {
            ...
            var strSQL = "select id_pks, keterangan " +
                "from amu_pks " +
                "where" + filter_string + "and kode_lokasi= '" + this.app._lokasi + "' and modul='PKS'";
