window.app_saku3_transaksi_amu_fAjuRawat = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_amu_fAjuRawat.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_amu_fAjuRawat";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan Perawatan Aset", 0);	
		...
		this.cb_vendor = new portalui_saiCBBL(this,{bound:[20,13,220,20],caption:"Mitra Pemeliharaan",tag:2,multiSelection:false,change:[this,"doChange"]});         		
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,247], childPage:["Data Aset","Item Beban","Data Anggaran","Asuransi"]});			
		...
		this.cb_jasur = new portalui_saiCBBL(this.pc1.childPage[3],{bound:[20,12,220,20],caption:"Jenis Asuransi",tag:2,multiSelection:false});         		
		this.e_catatan = new saiLabelEdit(this.pc1.childPage[3],{bound:[20,16,550,20],caption:"Catatan", maxLength:200});					
...
window.app_saku3_transaksi_amu_fAjuRawat.extend(window.childForm);
window.app_saku3_transaksi_amu_fAjuRawat.implement({	
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into amu_aset_beban (no_aju,periode,kode_lokasi,jenis,no_aset,keterangan,nilai,vendor,catatan,id_jenis_asur) values "+
							"('"+this.e_nb.getText()+"','"+this.e_periode.getText()+"','"+this.app._lokasi+"','RAWAT','-','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_total.getText())+",'"+this.cb_vendor.getText()+"','"+this.e_catatan.getText()+"','"+this.cb_jasur.getText()+"')");
							
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){		
							sql.add("insert into amu_aset_beban_d (no_aju,kode_lokasi,no_aset,catatan) values "+
	...
	doChange:function(sender){
		...
		// if (sender == this.cb_aset && this.cb_aset.getText()!="") {
		// 	var data = this.dbLib.getDataProvider("select * from amu_asset_bergerak where no_bukti='"+this.cb_aset.getText()+"' and kode_lokasi='"+this.app._lokasi+"'", true);
		// 	if (typeof data == "object") {
		// 		var line = data.rs.rows[0];
		// 		if (line != undefined) {      					       
		// 			this.e_nama.setText(line.nama_inv);                                                 
		// 			this.e_merk.setText(line.merk);
		// 			this.e_tipe.setText(line.tipe);
		// 			this.e_seri.setText(line.no_seri);   
		// 			this.e_noleh.setText(floatToNilai(line.nilai_perolehan));                                               
		// 		}
		// 	}
		// }
	},			
