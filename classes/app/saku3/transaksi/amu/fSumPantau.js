window.app_saku3_transaksi_amu_fSumPantau = function (owner) {
	if (owner) {
		window.app_saku3_transaksi_amu_fSumPantau.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_amu_fSumPantau";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick", "Data Sumur Pantau", 0);
		...
		this.e_pass = new saiLabelEdit(this.pc2.childPage[0], { bound: [520, 11, 200, 20], caption: "Password", maxLength: 50, tag: 1 });
		this.e_noreg = new saiLabelEdit(this.pc2.childPage[0], { bound: [20, 12, 400, 20], caption: "No Regristrasi", maxLength: 50, tag: 1 });
		this.e_tahun = new saiLabelEdit(this.pc2.childPage[0], { bound: [520, 12, 200, 20], caption: "Tahun Pembuatan", maxLength: 5, tag: 2,tipeText:ttAngka });
		this.e_noseri = new saiLabelEdit(this.pc2.childPage[0], { bound: [20, 13, 400, 20], caption: "No Serial", maxLength: 50, tag: 1 });
		this.e_merk = new saiLabelEdit(this.pc2.childPage[0], { bound: [20, 14, 400, 20], caption: "Merk", maxLength: 50, tag: 1 });
		this.e_kontak = new saiLabelEdit(this.pc2.childPage[0], { bound: [20, 15, 400, 20], caption: "Kontak Telemetri", maxLength: 50, tag: 1 });
		...
		try {
			...
			this.cb_jenis.setText("01","Sumur Pantau");
			this.doLoad();
...
window.app_saku3_transaksi_amu_fSumPantau.extend(window.childForm);
window.app_saku3_transaksi_amu_fSumPantau.implement({
	doTambah: function() {		
		this.stsSimpan = 1;
		this.pc1.setActivePage(this.pc1.childPage[1]);	   
		//this.cb_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib, "amu_sumur_pantau", "id_sumur",this.cb_jenis.getText() + this.tahun.substr(2,2) + ".", "000"));                    				
		
		var AddFormat = "__%"+"/01/"+this.tahun;
		var data = this.dbLib.getDataProvider("select isnull(max(id_sumur),0) as id_sumur from amu_sumur_pantau where id_sumur like '"+AddFormat+"' and kode_lokasi='"+this.app._lokasi+"'",true);
		if (typeof data == "object"){
			var line = data.rs.rows[0];							
			if (line != undefined){
				if (line.id_sumur == "0") this.cb_kode.setText("01"+"/01/"+this.tahun);
				else {
					var idx = parseFloat(line.id_sumur.substr(0,2)) + 1;
					idx = idx.toString();					
					if (idx.length == 1) var nu = "0"+idx;
					if (idx.length == 2) var nu = idx;
					this.cb_kode.setText(nu+"/01/"+this.tahun);						
				}
			} 
		}
		...
		try {
			if (this.standarLib.checkEmptyByTag(this, [0, 1])) {
				try {
					...
					if (this.stsSimpan == 0) {
						sql.add("delete from amu_sumur_pantau where id_sumur = '" + this.cb_kode.getText() + "' and kode_lokasi='"+this.app._lokasi+"'");						
					}

					sql.add("insert into amu_sumur_pantau (id_jenis_sumur,tahun,kode_lokasi,id_sumur,id_lahan,id_lembaga,flag_aktif,nama_sumur,koordinat,no_reg,no_seri,merk,no_kontak,dalam,jambang,posisi,pass,tgl_input,nik_user) values "+
							"('"+this.cb_jenis.getText()+"','"+this.e_tahun.getText()+"','"+this.app._lokasi+"','"+this.cb_kode.getText()+"','"+this.cb_lahan.getText()+"','"+this.cb_lembaga.getText()+"','"+this.c_aktif.getText().substr(0,1)+"','"+this.e_nama.getText()+"','"+this.e_coor.getText()+"','"+this.e_noreg.getText()+"','"+this.e_noseri.getText()+"','"+this.e_merk.getText()+"','"+this.e_kontak.getText()+"',"+nilaiToFloat(this.e_dalam.getText())+",'"+this.e_jambang.getText()+"','"+this.e_posisi.getText()+"','"+this.e_pass.getText()+"',getdate(),'"+this.app._userLog+"')");
	...
	doChange: function (sender) {
		try {
			if (sender == this.cb_kode && this.cb_kode.getText() != "") {
				var strSQL = "select * from amu_sumur_pantau where id_sumur ='" + this.cb_kode.getText() + "' and kode_lokasi='"+this.app._lokasi+"'";
				...
				if (typeof data == "object") {
					...
					if (line != undefined) {
						...
						this.e_kontak.setText(line.no_kontak);
	...
	doLoad: function (sender) {
		...
		var strSQL = "select a.id_sumur, a.nama_sumur, a.no_reg, b.nama_lahan from amu_sumur_pantau a inner join amu_lahan b on a.id_lahan=b.id_lahan where a.kode_lokasi='" + this.app._lokasi + "'";
		var data = this.dbLib.getDataProvider(strSQL, true);
	...
	doCari: function (sender) {
		try {
			...
			var strSQL = "select a.id_sumur, a.nama_sumur, b.nama_Lahan, a.no_reg " +
				"from amu_sumur_pantau a inner join amu_lahan b on a.id_lahan=b.id_lahan " +
