window.app_saku3_transaksi_amu_fAjuAsurGedung = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_amu_fAjuAsurGedung.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_amu_fAjuAsurGedung";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan Asuransi Gedung", 0);	
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:4,tag:0,
					colTitle:["ID Gedung","Nama Gedung","Alamat","Nilai Perolehan"],
					colWidth:[[3,2,1,0],[100,300,250,100]],					
...
window.app_saku3_transaksi_amu_fAjuAsurGedung.extend(window.childForm);
window.app_saku3_transaksi_amu_fAjuAsurGedung.implement({		
			...
			if (sender == this.sg) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Aset",sender,undefined, 
							...
							["id_gedung","nama_gedung"],"and",["ID","Nama"],false);				
				}				
			}
		}catch(e){
			systemAPI.alert(e);
	...
	doChangeCell: function(sender, col, row){
	    ...
	    if (col == 0) {
			if (this.sg.cells(0,row) != "") {
				var data = this.dbLib.getDataProvider("select * from amu_gedung where id_gedung='"+this.sg.cells(0,row)+"' and kode_lokasi='"+this.app._lokasi+"'", true);
				if (typeof data == "object") {
					var line = data.rs.rows[0];
					if (line != undefined) {      					       
						this.sg.cells(1,row,line.nama_gedung);                                                 
						this.sg.cells(2,row,line.alamat);
						this.sg.cells(3,row,floatToNilai(line.nilai_perolehan));                       
					}
				}					
			}
		}		
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into amu_gedung_asur (no_aju,periode,kode_lokasi,id_jenis_asur,id_perus,id_gedung,keterangan,nilai, no_polis,tgl_awal,tgl_akhir,premi,diskon,admin) values "+
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){		
							sql.add("insert into amu_gedung_asur_d (no_aju,kode_lokasi,id_gedung) values "+
