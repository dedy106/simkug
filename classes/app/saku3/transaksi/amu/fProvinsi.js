window.app_saku3_transaksi_amu_fProvinsi = function (owner) {
	if (owner) {
		window.app_saku3_transaksi_amu_fProvinsi.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_amu_fProvinsi";
		...
		uses("saiGrid", true);
		this.pc1 = new pageControl(this, { bound: [20, 12, 1000, 450], childPage: ["Daftar Provinsi", "Data Provinsi"] });		
		this.e_kode2 = new saiLabelEdit(this.pc1.childPage[0], { bound: [680, 10, 250, 20], caption: "Search [RowCount]", maxLength: 100, tag: 9, change: [this, "doCari"] });
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0], {
			...
			colTitle: ["ID Provinsi", "Nama Provinsi", "Pilih"],
...
window.app_saku3_transaksi_amu_fProvinsi.extend(window.childForm);
window.app_saku3_transaksi_amu_fProvinsi.implement({
		...
		try {
			if (this.standarLib.checkEmptyByTag(this, [0, 1])) {
				try {
					...
					sql.add("insert into amu_provinsi(id,nama) values " +
		...
		try {
			if (this.standarLib.checkEmptyByTag(this, [0, 1])) {
				try {
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from amu_provinsi where id = '" + this.cb_kode.getText() + "' ");
					sql.add("insert into amu_provinsi(id,nama) values " +
							"('" + this.cb_kode.getText() + "','" + this.e_nama.getText() + "')");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
	...
	doChange: function (sender) {
		try {
			if (this.cb_kode.getText() != "") {
				var strSQL = "select id,nama from amu_provinsi where id ='" + this.cb_kode.getText() + "'";
	...
	doLoad: function (sender) {
		...
		var strSQL = "select id, nama from amu_provinsi ";
		var data = this.dbLib.getDataProvider(strSQL, true);
	...
	doTampilData: function (page) {
		...
		for (var i = start; i < finish; i++) {
			...
			this.sg1.appendData([line.id, line.nama, "Pilih"]);
		}
	...
	doCari: function (sender) {
		try {
			...
			var strSQL = "select id, nama from amu_provinsi where" + filter_string;			
			var data = this.dbLib.getDataProvider(strSQL, true);
			if (typeof data == "object" && data.rs.rows[0] != undefined) {
				this.dataJU = data;
				this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length / show));
	...
	doSort: function (sender, col, row) {
		try {
			...
			if (col == 2) {
				...
				var strSQL = "select id, nama from amu_provinsi where " + filter_string +" order by " + column_array[col] + " " + ordertype;				
				var data = this.dbLib.getDataProvider(strSQL, true);
				if (typeof data == "object" && data.rs.rows[0] != undefined) {
					this.dataJU = data;
					this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length / show));
