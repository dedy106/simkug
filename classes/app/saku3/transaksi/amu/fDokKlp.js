window.app_saku3_transaksi_amu_fDokKlp = function (owner) {
	if (owner) {
		window.app_saku3_transaksi_amu_fDokKlp.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_amu_fDokKlp";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick", "Data Kelompok Dokumen", 0);
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0], {
			...
			colTitle: ["ID Dok", "Nama Dokumen", "Pilih"],
...
window.app_saku3_transaksi_amu_fDokKlp.extend(window.childForm);
window.app_saku3_transaksi_amu_fDokKlp.implement({
		...
		try {
			if (this.standarLib.checkEmptyByTag(this, [0, 1])) {
				try {
					...
					sql.add("insert into amu_dok_klp(kode_klp,nama,kode_lokasi) values " +
		...
		try {
			if (this.standarLib.checkEmptyByTag(this, [0, 1])) {
				try {
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from amu_dok_klp where kode_klp = '" + this.cb_kode.getText() + "' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("insert into amu_dok_klp(kode_klp,nama,kode_lokasi) values " +
							"('" + this.cb_kode.getText() + "','" + this.e_nama.getText() + "','"+this.app._lokasi+"')");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
	...
	doChange: function (sender) {
		try {
			if (this.cb_kode.getText() != "") {
				var strSQL = "select kode_klp,nama from amu_dok_klp where kode_klp ='" + this.cb_kode.getText() + "' and kode_lokasi='"+this.app._lokasi+"'";
	...
	doLoad: function (sender) {
		...
		var strSQL = "select kode_klp, nama from amu_dok_klp where kode_lokasi='"+this.app._lokasi+"' ";
		var data = this.dbLib.getDataProvider(strSQL, true);
	...
	doTampilData: function (page) {
		...
		for (var i = start; i < finish; i++) {
			...
			this.sg1.appendData([line.kode_klp, line.nama, "Pilih"]);
		}
	...
	doCari: function (sender) {
		try {
			...
			var show = parseInt(this.c_show.getText());
			var column_array = ['kode_klp', 'nama'];
			...
			var strSQL = "select kode_klp, nama from amu_dok_klp where kode_lokasi='"+this.app._lokasi+"' " + filter_string;			
			var data = this.dbLib.getDataProvider(strSQL, true);
			if (typeof data == "object" && data.rs.rows[0] != undefined) {
				this.dataJU = data;
				this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length / show));
	...
	doSort: function (sender, col, row) {
		try {
			...
			if (col == 2) {
				...
				var strSQL = "select kode_klp, nama from amu_dok_klp where kode_lokasi='"+this.app._lokasi+"' and " + filter_string +" order by " + column_array[col] + " " + ordertype;				
				var data = this.dbLib.getDataProvider(strSQL, true);
				if (typeof data == "object" && data.rs.rows[0] != undefined) {
					this.dataJU = data;
					this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length / show));
