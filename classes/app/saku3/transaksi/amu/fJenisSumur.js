window.app_saku3_transaksi_amu_fJenisSumur = function (owner) {
    if (owner) {
        window.app_saku3_transaksi_amu_fJenisSumur.prototype.parent.constructor.call(this, owner);
        this.className = "app_saku3_transaksi_amu_fJenisSumur";
        this.itemsValue = new arrayList();
        this.app._mainForm.childFormConfig(this, "mainButtonClick", "Data Jenis Sumur", 0);

        uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox;util_file");
        uses("saiGrid", true);
        this.pc1 = new pageControl(this, { bound: [20, 12, 1000, 450], childPage: ["Daftar Jenis Sumur", "Data Jenis Sumur"] });
...
window.app_saku3_transaksi_amu_fJenisSumur.extend(window.childForm);
window.app_saku3_transaksi_amu_fJenisSumur.implement({
        ...
        try {
            if (this.standarLib.checkEmptyByTag(this, [0, 1])) {
                try {
                    ...
                    sql.add("insert into amu_jenis_sumur(id_jenis_sumur,nama_jenis,kode_lokasi) values " +
        ...
        try {
            if (this.standarLib.checkEmptyByTag(this, [0, 1])) {
                try {
                    uses("server_util_arrayList");
                    var sql = new server_util_arrayList();
                    sql.add("delete from amu_jenis_sumur where id_jenis_sumur = '" + this.cb_kode.getText() + "' ");
                    sql.add("insert into amu_jenis_sumur(id_jenis_sumur,nama_jenis,kode_lokasi) values " +
                            "('" + this.cb_kode.getText() + "','" + this.e_nama.getText() + "','" + this.app._lokasi + "')");
                    setTipeButton(tbAllFalse);
                    this.dbLib.execArraySQL(sql);
    ...
    doChange: function (sender) {
        try {
            if (this.cb_kode.getText() != "") {
                var strSQL = "select id_jenis_sumur,nama_jenis " +
                             "from amu_jenis_sumur where id_jenis_sumur ='" + this.cb_kode.getText() + "'";
    ...
    doLoad: function (sender) {
        ...
        var strSQL = "select id_jenis_sumur, nama_jenis from amu_jenis_sumur where kode_lokasi='" + this.app._lokasi + "' ";
        var data = this.dbLib.getDataProvider(strSQL, true);
    ...
    doTampilData: function (page) {
        ...
        for (var i = start; i < finish; i++) {
            ...
            this.sg1.appendData([line.id_jenis_sumur, line.nama_jenis, "Pilih"]);
        }
    ...
    doCari: function (sender) {
        try {
            ...
            var show = parseInt(this.c_show.getText());
            var column_array = ['id_jenis_sumur', 'nama_jenis'];
