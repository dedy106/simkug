window.app_saku3_transaksi_tarbak_fHrLoker = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_fHrLoker.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_fHrLoker";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Lokasi Kerja : Input/Edit", 0);	
		
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		uses("saiGrid",true);		
		this.pc1 = new pageControl(this,{bound:[20,12,1000,400], childPage:["Daftar Lokasi Kerja","Data Lokasi Kerja"]});				
...
window.app_saku3_transaksi_tarbak_fHrLoker.extend(window.childForm);
window.app_saku3_transaksi_tarbak_fHrLoker.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into hr_loker(kode_loker,nama,kode_lokasi,flag_aktif) values ('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.c_status.getText().substr(0,1)+"')");
					setTipeButton(tbAllFalse);
	...
	doLoad:function(sender){						
		var strSQL = "select kode_loker,nama "+
		             "from hr_loker where kode_lokasi='"+this.app._lokasi+"' order by kode_loker ";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
