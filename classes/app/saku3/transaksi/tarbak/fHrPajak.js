window.app_saku3_transaksi_tarbak_fHrPajak = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_fHrPajak.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_fHrPajak";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Status Pajak : Input/Edit", 0);	
		
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		uses("saiGrid",true);		
		this.pc1 = new pageControl(this,{bound:[20,12,1000,400], childPage:["Daftar Status Pajak","Data Status Pajak"]});				
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:5,tag:9,
		            colTitle:["Kode","Nama","Nilai PTKP","% By Jabatan","Maks By. Jab"],
		...
		this.e_nilai = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,12,200,20],caption:"Nilai", tag:1,  tipeText:ttNilai, text:"0"});	
		this.e_biaya = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,13,200,20],caption:"BIaya Jabatan", tag:1, tipeText:ttNilai, text:"0"});	
		this.e_jab = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,4,200,20],caption:"Maksimal", tag:1,  tipeText:ttNilai, text:"0"});	
...
window.app_saku3_transaksi_tarbak_fHrPajak.extend(window.childForm);
window.app_saku3_transaksi_tarbak_fHrPajak.implement({
	...
	doLoad:function(sender){						
		var strSQL = "select kode_pajak,nama,nilai,biaya_jab,jab_max "+
		             "from hr_pajak where kode_lokasi='"+this.app._lokasi+"' order by kode_pajak ";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_pajak,line.nama,floatToNilai(line.nilai),floatToNilai(line.biaya_jab),floatToNilai(line.jab_max)]); 
		}
