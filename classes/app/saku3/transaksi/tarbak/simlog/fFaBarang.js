window.app_saku3_transaksi_tarbak_simlog_fFaBarang = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_simlog_fFaBarang.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_simlog_fFaBarang";
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:5,tag:9,
		            colTitle:["Kode","Nama","Klp Akun","Jenis","Pilih"],
					colWidth:[[4,3,2,1,0],[70,100,350,300,80]],					
					readOnly:true,
		...
		this.e_barcode = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,12,200,20],caption:"Format Barcode", maxLength:10, tag:1});			
		//this.cb_hutang = new saiCBBL(this.pc1.childPage[1],{bound:[20,16,220,20],caption:"Akun Hutang", multiSelection:false, maxLength:10, tag:2});				
		this.c_jenis = new saiCB(this.pc1.childPage[1],{bound:[20,15,200,20],caption:"Jenis",items:["AKTAP","INV","NON"], readOnly:true,tag:2,change:[this,"doChange"]});		
		this.cb_akun = new saiCBBL(this.pc1.childPage[1],{bound:[20,14,220,20],caption:"Akun", multiSelection:false, maxLength:10, tag:2});				
		...
		try {
			...
			//this.cb_hutang.setSQL("select a.kode_akun,a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag='024' where a.kode_lokasi='"+this.app._lokasi+"' ",["a.kode_akun","a.nama"],false,["Kode","Nama"],"where","Data Akun",true);			
...
window.app_saku3_transaksi_tarbak_simlog_fFaBarang.extend(window.childForm);
window.app_saku3_transaksi_tarbak_simlog_fFaBarang.implement({
	...
	simpan: function(){			
		try{						
			if (this.e_barcode.getText().length != 6) {
				system.alert(this, "Panjang Format Barcode harus 6 digit.", ".");
				return false;
			}
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into fa_klp(kode_klpfa,nama,kode_klpakun,jenis,kode_lokasi,barcode,akun_hutang) values "+
							"('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.cb_akun.getText()+"','"+this.c_jenis.getText().substr(0,1)+"','"+this.app._lokasi+"','"+this.e_barcode.getText()+"','-')");
					setTipeButton(tbAllFalse);
	...
	doChange: function(sender){
		try{
			if (sender == this.c_jenis && this.c_jenis.getText()!= "") {
				this.cb_akun.setSQL("select a.kode_klpakun,a.nama from fa_klpakun a where a.jenis='"+this.c_jenis.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' ",["kode_klpakun","nama"],false,["Kode","Nama"],"where","Data Klp Akun",true);			
			}
			...
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select kode_klpfa,nama,kode_klpakun,jenis,barcode "+
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_klpfa,line.nama,line.akun,line.jenis.toUpperCase(),"Pilih"]); 
		}
