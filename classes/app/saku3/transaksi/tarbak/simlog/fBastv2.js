window.app_saku3_transaksi_tarbak_simlog_fBastv2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_simlog_fBastv2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_simlog_fBastv2";
		...
		this.sg3 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:7,tag:9,
					colTitle:["No Bukti","Tanggal","No Dokumen","Deskripsi","Mitra","Nilai","Pilih"],
					colWidth:[[6,5,4,3,2,1,0],[70,100,200,200,180,80,100]],
		...
		this.e_adk = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,14,200,20],caption:"Nilai UM/ADK", tag:1, readOnly:true, tipeText:ttNilai, text:"0",change:[this,"doChange"]});						
		this.cb_akun = new saiCBBL(this.pc2.childPage[0],{bound:[20,17,220,20],caption:"Akun Hutang", readOnly:true, maxLength:10, tag:2});										
		this.e_total = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,17,200,20],caption:"Nilai Hutang", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,996,315], childPage:["Data Mitra","Rekap Penerimaan","Detail Penerimaan","Data Jurnal+","File Dok"]});
		...
		this.sg2 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:6,
		            colTitle:["No Terima","Tanggal","Keterangan","Kode Akun","Kode PP","Nilai"],
		...
		this.sg4 = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:11,
		...
		this.sg = new saiGrid(this.pc1.childPage[3],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:7,tag:9,
		...
		this.sgn = new portalui_sgNavigator(this.pc1.childPage[3],{bound:[1,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:2,grid:this.sg});				
...
window.app_saku3_transaksi_tarbak_simlog_fBastv2.extend(window.childForm);
window.app_saku3_transaksi_tarbak_simlog_fBastv2.implement({	
	...
	isiSPK : function() {
		...
		this.cb_spk.setSQL("select a.no_spk, a.keterangan from log_spk_m a "+
						   ...
						   "where b.no_hutang is null and a.periode<='"+this.e_periode.getText()+"' and a.no_ba='-' and a.kode_lokasi='"+this.app._lokasi+"'",["no_spk","keterangan"],false,["No SPK","Deskripsi"],"and","Data SPK",true);							   
	},
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					//if e_total minus... maka kelebihan uang(UM) dikembalikan ke bendahara
					...
					if (this.sg2.getRowValidCount() > 0){
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){								
								totAset += nilaiToFloat(this.sg2.cells(5,i));
								sql.add("insert into hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.sg2.cells(0,i)+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg2.cells(3,i)+"','"+this.sg2.cells(2,i)+"','D','IDR',1,"+parseNilai(this.sg2.cells(5,i))+","+parseNilai(this.sg2.cells(5,i))+",'"+this.sg2.cells(4,i)+"','-','"+this.app._lokasi+"','LOGBAST','AKTAP','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
								sql.add("update fa_asset set tgl_perolehan='"+this.dp_d1.getDateString()+"',tgl_susut='"+this.dp_d1.getDateString()+"',periode='"+this.e_periode.getText()+"',periode_susut='"+this.e_periode.getText()+"',progress='2',catatan='"+this.e_nb.getText()+"',kode_pp_susut='"+this.sg2.cells(4,i)+"' where no_baps='"+this.sg2.cells(0,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
							}
						}
					}
					...
					if (this.e_adk.getText() != "0") {
						if (nilaiToFloat(this.e_total.getText()) >= 0) var niADK = parseNilai(this.e_adk.getText());
						else var niADK = totAset;
						sql.add("insert into hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
								"('"+this.e_nb.getText()+"','"+this.cb_spk.getText()+"','"+this.dp_d1.getDateString()+"',777,'"+this.akunADK+"','"+this.e_ket.getText()+"','C','IDR',1,"+niADK+","+niADK+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','LOGBAST','ADK','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");						
					}
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){								
								sql.add("insert into hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
										"('"+this.e_nb.getText()+"','"+this.cb_spk.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"','IDR',1,"+parseNilai(this.sg.cells(4,i))+","+parseNilai(this.sg.cells(4,i))+",'"+this.sg.cells(5,i)+"','-','"+this.app._lokasi+"','LOGBAST','BBN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");
							}
						}
					}	
					...
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i) && this.sgUpld.cells(2,i)!="-") {							
							...
							sql.add("insert into log_pesan_dok(no_pesan,no_gambar,nu,kode_jenis,kode_lokasi,no_ref)values('"+this.e_nb.getText()+"','"+this.sgUpld.cells(3,i).tmpfile+"','"+ix+"','"+this.sgUpld.cells(0,i)+"','"+this.app._lokasi+"','"+this.cb_spk.getText()+"')");								
						}	
					}	
			...
			case "ubah" :		
				...
				for (var i=0;i < this.sg4.getRowCount();i++){
					if (this.sg4.rowValid(i)){
						var j = i+1;					
						if (nilaiToFloat(this.sg4.cells(9,i)) != nilaiToFloat(this.sg4.cells(10,i)) ) {
							system.alert(this,"Transaksi tidak valid.","Jumlah Barang SPK tidak sama dengan yang diterima.(Detail Penerimaan - Baris : "+j+")");
							return false;
						}
					}					
				}	
				...
				// boleh nol, krn uang muka dilunasin (tokopedia)
				// if (nilaiToFloat(this.e_total.getText()) <= 0) {
	...
	doChange:function(sender){
		...
		if (sender == this.cb_spk && this.cb_spk.getText() != "" ) {
			if (this.stsSimpan ==1) {
				...
				var strSQL = "select a.no_terima,convert(varchar,a.tanggal,103) as tgl,a.keterangan,b.kode_akun,b.kode_pp,b.total "+
							"from log_terima_m a inner join ( "+
							"		select no_baps,kode_akun,kode_pp,sum(nilai) as total  "+
							"		from fa_asset where catatan='-' and kode_lokasi='"+this.app._lokasi+"' group by kode_akun,kode_pp,no_baps) b on a.no_terima=b.no_baps "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						...
						this.sg2.appendData([line2.no_terima,line2.tgl,line2.keterangan,line2.kode_akun,line2.kode_pp,floatToNilai(line2.total)]);
					}
			...
			var data = this.dbLib.getDataProvider(
							...
							"where a.no_ref = '"+this.cb_spk.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.nu",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;					
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
		...
		if (sender==this.e_adk && this.e_adk.getText()!="") this.sg.validasi();
	},
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_hutang,line.tgl,line.no_dokumen,line.keterangan,line.vendor,floatToNilai(line.nilai),"Pilih"]); 
		}
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.no_terima,convert(varchar,a.tanggal,103) as tgl,a.keterangan,b.kode_akun,b.kode_pp,b.total "+
							 "from log_terima_m a inner join ( "+
							 "		select no_baps,kode_akun,kode_pp,sum(nilai) as total  "+
							 "		from fa_asset where catatan='"+this.e_nb.getText()+"' group by kode_akun,kode_pp,no_baps) b on a.no_terima=b.no_baps "+
							 "where a.no_po='"+this.cb_spk.getText()+"' ";
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line2;
