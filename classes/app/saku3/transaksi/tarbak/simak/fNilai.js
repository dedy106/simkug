window.app_saku3_transaksi_tarbak_simak_fNilai = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_simak_fNilai.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_simak_fNilai";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Data Penilaian Siswa", 0);	
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,400], childPage:["Data Penilaian","Daftar Penilaian"]});				
		this.sg1 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:7,tag:9,
		...
		this.bTampil = new portalui_button(this.pc1.childPage[0],{bound:[818,18,80,18],caption:"Data Siswa",click:[this,"doLoadData1"]});		
		this.bRekon = new button(this.pc1.childPage[0],{bound:[913,18,80,18],caption:"Rekon Nilai", click:[this,"doRekon"]});
		
		this.pc2 = new pageControl(this.pc1.childPage[0],{bound:[1,12,995,252], childPage:["Data Siswa","Data Nilai","Error Msg"]});		
		this.sg = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:4,tag:2,
		            colTitle:["NIS","Nama","Nilai","Validasi NIS"],					
					colWidth:[[3,2,1,0],[100,80,300,150]],					
		...
		try {
			...
			this.doClick();
			this.doLoad();		
			this.cb_ta.setSQL("select kode_ta, nama from sis_ta where flag_aktif='1' and kode_pp='"+this.app._kodePP+"' and kode_lokasi = '"+this.app._lokasi+"'",["kode_ta","nama"],false,["Kode TA","nama"],"and","Data TA",true);			
			this.cb_kelas.setSQL("select kode_kelas, nama from sis_kelas where flag_aktif='1' and kode_pp='"+this.app._kodePP+"' and kode_lokasi = '"+this.app._lokasi+"'",["kode_kelas","nama"],false,["Kode Kelas","Nama"],"and","Data Kelas",true);			
			this.cb_matpel.setSQL("select kode_matpel, nama from sis_matpel where flag_aktif='1' and kode_pp='"+this.app._kodePP+"' and kode_lokasi = '"+this.app._lokasi+"'",["kode_matpel","nama"],false,["Kode Matpel","Nama"],"and","Data Matpel",true);			
			this.cb_jenis.setSQL("select kode_jenis, nama from sis_jenisnilai where flag_aktif='1' and kode_pp='"+this.app._kodePP+"' and kode_lokasi = '"+this.app._lokasi+"'",["kode_jenis","nama"],false,["Kode Jenis Nilai","Nama"],"and","Data Jenis Nilai",true);			
...
window.app_saku3_transaksi_tarbak_simak_fNilai.extend(window.childForm);
window.app_saku3_transaksi_tarbak_simak_fNilai.implement({
			...
			for (var i=0;i < this.sg2.getRowCount();i++){
				for (var j=0;j < this.dataJU1.rs.rows.length;j++){
					if (this.dataJU1.rs.rows[j].nis == this.sg2.cells(0,i)) {						
						this.dataJU1.rs.rows[j].nilai = this.sg.cells(2,j,this.sg2.cells(2,i));
					}
				}																			
			}

			//cek NIS
			var strSQL = "select nis from sis_siswa where kode_kelas = '"+this.cb_kelas.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";			
		...
		try{
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					if (this.stsSimpan == 0) {
						sql.add("delete from sis_nilai where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from sis_nilai_m where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}			
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i) && this.sg.cells(3,i) != "INVALID"){
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					...
					var sql = new server_util_arrayList();
						sql.add("delete from sis_nilai where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from sis_nilai_m where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
					system.alert(this, e,"");
	...
	doLoadData1:	function(sender){
		if (this.cb_kelas.getText()!="") {
			var strSQL = "select nis,nama from sis_siswa where kode_kelas = '"+this.cb_kelas.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"' ";						
			var data = this.dbLib.getDataProvider(strSQL,true);
	...
	doTampilData1: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData([line.nis,line.nama,"0","INVALID"]); 
		}
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {			
				...
				var strSQL = "select a.nis,a.nilai,b.nama from sis_nilai a inner join sis_siswa b on a.nis=b.nis where a.no_bukti='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' ";										
				...
				if (typeof data1 == "object" && data1.rs.rows[0] != undefined){
					...
					for (var i in data1.rs.rows){
						...
						this.sg.appendData([line1.nis,line1.nama,line1.nilai,"VALID"]);
					}
				} else this.sg.clear(1);
				
				var strSQL = "select * from sis_nilai_m where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
	...
	doChange: function(sender){
		try{
			...
			if ((sender == this.cb_ta || this.c_sem || this.cb_kelas || this.cb_matpel || this.cb_jenis) && this.cb_ta.getText() != "" && this.c_sem.getText() != "" && this.cb_kelas.getText() != "" && this.cb_matpel.getText() != "" && this.cb_jenis.getText() != "") {	
				var strSQL="select COUNT(*)+1 as jumlah from sis_nilai_m where kode_ta= '"+this.cb_ta.getText()+"' and kode_sem= '"+this.c_sem.getText()+"' and kode_kelas= '"+this.cb_kelas.getText()+"' and kode_matpel= '"+this.cb_matpel.getText()+"' and kode_jenis= '"+this.cb_jenis.getText()+"' ";				
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" &&  data.rs.rows[0] != undefined){
					var line = data.rs.rows[0];
