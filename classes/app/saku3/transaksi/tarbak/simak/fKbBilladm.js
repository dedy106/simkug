window.app_saku3_transaksi_tarbak_simak_fKbBilladm = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_simak_fKbBilladm.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_simak_fKbBilladm";
		this.itemsValue = new arrayList();
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pelunasan Billing KasBank + ADM", 0);	
		...
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:5,
					colTitle:["ID Bank","Nilai Bayar","Sisa Pelunasan","Validasi","Nilai Adm"],
					colWidth:[[4,3,2,1,0],[100,100,100,100,100]],
					colFormat:[[1,2,4],[cfNilai,cfNilai,cfNilai]],
...
window.app_saku3_transaksi_tarbak_simak_fKbBilladm.extend(window.childForm);
window.app_saku3_transaksi_tarbak_simak_fKbBilladm.implement({
		...
		try {
			...
			if (this.inValid) {				
				...
				for (var i=0; i < this.sg1.getRowCount();i++){								
					...
					//sisa nilaibayar+nilai_adm untuk masuk ke cd
					nilaiBayar += nilaiToFloat(this.sg1.cells(4,i));
	...
	doLoad: function(sender){	
		if (this.cb_pp.getText()!="") {
			...
			var strSQL = "select * from sis_bill_saldo where nik_user='"+this.nik_user+"' and kode_param in ("+kodeparam+") ";			
