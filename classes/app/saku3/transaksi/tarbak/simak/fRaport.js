window.app_saku3_transaksi_tarbak_simak_fRaport = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_simak_fRaport.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_simak_fRaport";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Rapor Siswa", 0);	
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,400], childPage:["Data Raport","Daftar Raport"]});				
		...
		this.pc2 = new pageControl(this.pc1.childPage[0],{bound:[1,12,995,297], childPage:["Data Matpel","Data Nilai"]});		
		this.sg = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:3,tag:2,
		            colTitle:["Kode Matpel","Nama","Nilai"],					
		...
		this.sg2 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:3,tag:2,
		            colTitle:["Kode Matpel","Nama","Nilai"],					
					colWidth:[[2,1,0],[80,300,100]],			
		...
		try {
			...
			this.cb_kelas.setSQL("select kode_kelas, nama from sis_kelas where kode_pp='"+this.app._kodePP+"' and kode_lokasi = '"+this.app._lokasi+"'",["kode_kelas","nama"],false,["Kode Kelas","Nama"],"and","Data Kelas",true);			
...
window.app_saku3_transaksi_tarbak_simak_fRaport.extend(window.childForm);
window.app_saku3_transaksi_tarbak_simak_fRaport.implement({
				...
				try{									
					...
					for (var i=0;i < this.dataJU1.rs.rows.length;i++){
						 ...
						 sql.add("insert into sis_raport_d(no_bukti,nis,kode_matpel,nilai,kode_pp,kode_lokasi) values "+
								 "('"+this.e_nb.getText()+"','"+this.cb_nis.getText()+"','"+line.kode_matpel+"',"+line.nilai+",'"+this.app._kodePP+"','"+this.app._lokasi+"')");
					}	
	...
	doLoadData1:function(sender){
		if (this.cb_nis.getText()!="") {
			var strSQL = "select '0' as nilai, a.kode_matpel,c.nama "+
						 "from sis_nilai_m a "+
						 "inner join sis_nilai b on a.no_bukti=b.no_bukti "+
						 "inner join sis_matpel c on a.kode_matpel=c.kode_matpel "+
						 "where b.nis='"+this.cb_nis.getText()+"' and a.kode_pp='"+this.app._kodePP+"' and a.kode_lokasi='"+this.app._lokasi+"' "+
						 "group by a.kode_matpel,c.nama ";						
			var data = this.dbLib.getDataProvider(strSQL,true);
			...
			system.alert(this,"NIS harus diisi.","Pilih dari daftar");
		}			
	},
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {			
				...
				var data = this.dbLib.getDataProvider("select a.nis, a.kode_jenis, b.nama, a.nilai from sis_nilai a inner join sis_siswa b on a.nis=b.nis where a.no_bukti = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_pp='"+this.app._kodePP+"' ",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
