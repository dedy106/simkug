window.app_saku3_transaksi_tarbak_simak_fKbBillDenda = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_simak_fKbBillDenda.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_simak_fKbBillDenda";
		this.itemsValue = new arrayList();
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pelunasan Billing KasBank + Denda", 0);	
...
window.app_saku3_transaksi_tarbak_simak_fKbBillDenda.extend(window.childForm);
window.app_saku3_transaksi_tarbak_simak_fKbBillDenda.implement({
				...
				try{													
					...
					if (this.stsDenda == 1) {
						sql.add("insert into sis_bill_d (no_bill,kode_lokasi,nis,kode_kelas,kode_param,nilai,dc,periode,akun_piutang,modul,kode_pp,kode_ta,kode_sem) "+
								...
								"inner join sis_param c on b.kode_param=c.kode_param and b.kode_lokasi=c.kode_lokasi and c.sts_param='3' and '"+this.e_periode.getText()+"' between b.per_awal and b.per_akhir "+
								"inner join (select distinct no_rekon,nis,kode_pp,kode_lokasi from sis_rekon_d where no_rekon='"+this.e_nb.getText()+"' ) d on a.nis=d.nis and a.kode_lokasi=d.kode_lokasi and a.kode_pp=d.kode_pp "+
								
								"left join (select distinct a.nis,a.kode_lokasi,a.kode_pp "+
								"			from sis_bill_d a inner join sis_param c on a.kode_param=c.kode_param and a.kode_lokasi=c.kode_lokasi and c.sts_param='3' "+
								"			where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_pp='"+this.cb_pp.getText()+"' "+
								"		    ) e on a.nis=e.nis and a.kode_pp=e.kode_pp and a.kode_lokasi=e.kode_lokasi "+
								
								"where e.nis is null and d.no_rekon='"+this.e_nb.getText()+"' and d.kode_lokasi='"+this.app._lokasi+"' and d.kode_pp='"+this.cb_pp.getText()+"'");

						sql.add("insert into sis_bill_m(no_bill,kode_lokasi,periode,tanggal,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_app,posted,tgl_input,nik_user) "+
								"select '"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_pp.getText()+"','LOADSISKB','BILL','IDR',1,sum(nilai),'"+this.app._userLog+"','"+this.app._userLog+"','X',getdate(),'"+this.app._userLog+"' "+
								"from sis_bill_d "+
	...
	doSelectDate: function(sender, y,m,d){
		...
		if (d1 > d2) this.stsDenda = 1;
		else this.stsDenda = 0;
