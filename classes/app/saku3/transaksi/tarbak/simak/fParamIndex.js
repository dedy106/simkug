window.app_saku3_transaksi_tarbak_simak_fParamIndex = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_simak_fParamIndex.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_simak_fParamIndex";
		...
		this.i_gen = new portalui_imageButton(this.sgn,{bound:[575,2,20,20],hint:"Load Ref Param",image:"icon/"+system.getThemes()+"/tabCont2.png",click:[this,"doLoadRef"]});
...
window.app_saku3_transaksi_tarbak_simak_fParamIndex.extend(window.childForm);
window.app_saku3_transaksi_tarbak_simak_fParamIndex.implement({
	...
	doChange: function(sender){
		try{			
			if (sender == this.cb_pp  && this.cb_pp.getText() != "") {		
				var strSQL = "select a.kode_param,a.nama,isnull(b.idx ,0) as idx "+
							 "from sis_param a inner join sis_param_idx b on a.kode_param=b.kode_param and a.kode_lokasi=b.kode_lokasi and b.kode_pp='"+this.app._kodePP+"' "+						 	 
							 "where a.kode_lokasi = '"+this.app._lokasi+"' order by isnull(b.idx,0),a.kode_param "; 																		
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
