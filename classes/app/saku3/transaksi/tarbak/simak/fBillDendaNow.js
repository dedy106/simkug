window.app_saku3_transaksi_tarbak_simak_fBillDendaNow = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_simak_fBillDendaNow.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_simak_fBillDendaNow";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Billing Siswa Denda [CashBasis]", 0);	
...
window.app_saku3_transaksi_tarbak_simak_fBillDendaNow.extend(window.childForm);
window.app_saku3_transaksi_tarbak_simak_fBillDendaNow.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into sis_bill_m(no_bill,kode_lokasi,periode,tanggal,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_app,posted,tgl_input,nik_user) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','BILDND','BILL','IDR',1,"+nilaiToFloat(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"','X',getdate(),'"+this.app._userLog+"')");
							
					//var periodeBefore = getPrevPeriode(this.e_periode.getText());
					//periode before diassgn periode saat ini
					var periodeBefore = this.e_periode.getText();
					var tglBatas = periodeBefore.substr(0,4)+"-"+periodeBefore.substr(4,2)+"-"+this.tglDenda;						
					sql.add("insert into sis_bill_d (no_bill,kode_lokasi,nis,kode_kelas,kode_param,nilai,dc,periode,akun_piutang,modul,kode_pp,kode_ta,kode_sem) "+
							...
							"select distinct a.nis,a.no_bill,'"+this.cb_pp.getText()+"' as kode_pp "+
							"from sis_bill_saldo a "+
							"inner join sis_param b on a.kode_param=b.kode_param and b.sts_param='1' and b.kode_lokasi ='"+this.app._lokasi+"'   "+
							"where a.nik_user = '"+this.nik_user+"' and a.periode = '"+periodeBefore+"' "+
							"union "+
							"select distinct a.nis,a.no_bill,a.kode_pp from sis_rekon_d a "+
							"inner join kas_m b on a.no_rekon=b.no_kas and a.kode_lokasi=b.kode_lokasi "+
							"inner join sis_param c on a.kode_param=c.kode_param and a.kode_lokasi=c.kode_lokasi and c.sts_param = '1' "+
							"where a.periode_bill = '"+periodeBefore+"' and b.tanggal>'"+tglBatas+"' and a.kode_lokasi='"+this.app._lokasi+"' "+
							"union "+
							"select distinct a.nis,a.no_bill,a.kode_pp from sis_rekon_d a "+
							...
							") yy on a.nis=yy.nis and a.kode_pp=yy.kode_pp "+
							
							"inner join sis_siswa_tarif b on a.nis=b.nis and a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and '"+periodeBefore+"' between b.per_awal and b.per_akhir "+
							...
							"left join (select a.nis,a.kode_lokasi,a.kode_kelas,a.kode_param,a.periode,a.kode_pp,sum(case a.dc when 'D' then a.nilai else -a.nilai end) as nilai "+
						 	"           from sis_bill_d a "+
						 	"			inner join sis_param b on a.kode_param=b.kode_param and a.kode_lokasi=b.kode_lokasi "+
						 	"           where b.sts_param = '3' and a.kode_lokasi='"+this.app._lokasi+"' and a.periode='"+this.e_periode.getText()+"' and a.kode_pp='"+this.app._kodePP+"' "+
						 	"           group by a.nis,a.kode_lokasi,a.kode_kelas,a.kode_param,a.periode,a.kode_pp "+
	...
	doTampil:function(sender){		
		if (this.e_periode.getText() != "") {
			...
			var strSQL = "select zz.kode_kelas,c.kode_param,c.nama, zz.kode_tingkat,a.kode_akt,zz.kode_jur, SUM(b.tarif) as nilai "+
						 ...
						 "           from sis_bill_d a "+
						 "			 inner join sis_param b on a.kode_param=b.kode_param and a.kode_lokasi=b.kode_lokasi "+
						 "           where b.sts_param = '3' and a.kode_lokasi='"+this.app._lokasi+"' and a.periode in ('"+periodeBefore+"','"+this.e_periode.getText()+"') and a.kode_pp='"+this.app._kodePP+"' "+
						 "           group by a.nis,a.kode_lokasi,a.kode_kelas,a.kode_param,a.periode,a.kode_pp "+
						 "			 ) f on b.nis=f.nis and b.kode_pp=f.kode_pp and f.kode_param=b.kode_param and f.periode in ('"+periodeBefore+"','"+this.e_periode.getText()+"') and b.kode_lokasi=f.kode_lokasi "+						
	...
	doClick:function(sender){
		if (sender == this.i_gen) {			
			...
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"sis_bill_m","no_bill",this.app._lokasi+"-DND"+this.e_periode.getText().substr(2,4)+".","0000"));
	...
	doDblClick: function(sender, col, row){
		if (this.sg.cells(0,row) != "") {	
			...
			var strSQL = "select a.nis,a.nama,a.kode_kelas,c.kode_param,b.tarif "+
						 ...
						 "inner join sis_siswa_tarif b on a.nis=b.nis and a.kode_pp=b.kode_pp and a.kode_kelas=b.kode_kelas and '"+periodeBefore+"' between b.per_awal and b.per_akhir "+
						 "inner join sis_param c on b.kode_param=c.kode_param and b.kode_lokasi=c.kode_lokasi and c.sts_param='3' "+
						 "left join (select nis,kode_lokasi,kode_kelas,kode_param,periode,kode_pp,sum(case dc when 'D' then nilai else -nilai end) as nilai "+
						            "from sis_bill_d "+
						            "where no_bill <> '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and periode='"+this.e_periode.getText()+"' and kode_pp='"+this.app._kodePP+"' "+
	...
	doLoad4:function(sender){																				
		var strSQL = "select a.no_bill,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
					 ...
					 "where b.no_bill is null and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_pp='"+this.app._kodePP+"' and a.modul='BILDND'";						
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU4 = data;
