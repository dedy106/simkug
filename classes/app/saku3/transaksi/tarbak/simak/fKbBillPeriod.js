window.app_saku3_transaksi_tarbak_simak_fKbBillPeriod = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_simak_fKbBillPeriod.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_simak_fKbBillPeriod";
		this.itemsValue = new arrayList();
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pelunasan Billing KasBank By Periode Bill", 0);	
		...
		this.cb_titip = new saiCBBL(this,{bound:[20,17,220,20],caption:"Akun Diskon", multiSelection:false, maxLength:10, tag:2 });		
		...
		this.c_periode = new saiCB(this.pc1.childPage[0],{bound:[20,13,200,20],caption:"Periode Bill",readOnly:true,tag:2}); 
		this.sg3 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-38],colCount:3,tag:9,
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:10,tag:0,
		...
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:4,
		...
		try {
			...
			this.cb_titip.setSQL("select a.kode_akun, a.nama from masakun a "+
								 "inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag in ('100') "+
			...
			var data = this.dbLib.getDataProvider("select distinct periode as periode from sis_bill_m where kode_lokasi='"+this.app._lokasi+"' order by periode desc ",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;			
				for (var i in data.rs.rows){
					line = data.rs.rows[i];													
...
window.app_saku3_transaksi_tarbak_simak_fKbBillPeriod.extend(window.childForm);
window.app_saku3_transaksi_tarbak_simak_fKbBillPeriod.implement({
	...
	doLoad: function(sender){	
		if (this.cb_pp.getText()!="") {
			...
			var strSQL = "select * from sis_bill_saldo where periode = '"+this.c_periode.getText()+"' and nik_user='"+this.nik_user+"' and kode_param in ("+kodeparam+") order by kode_param,periode";			
