window.app_saku3_transaksi_tarbak_simak_fAkruBillOpen = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_simak_fAkruBillOpen.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_simak_fAkruBillOpen";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Akru Billing Siswa Open Period", 0);	
		...
		uses("portalui_saiGrid;util_gridLib",true);		
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,20,995,348], childPage:["Filter Billing","Jurnal Billing","Daftar Billing"]});		
		this.cb_tingkat = new portalui_saiCBBL(this.pc1.childPage[0],{bound:[20,11,220,20],caption:"Tingkat",multiSelection:false,tag:1,change:[this,"doChange"]});				
		this.c_periode = new saiCB(this.pc1.childPage[0],{bound:[20,17,200,20],caption:"Periode Bill", tag:2,change:[this,"doChange"]});		

		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[0,5,this.pc1.width-5,this.pc1.height-33],colCount:6,tag:0,
				colTitle:["PP / Unit","Kode Param","Nama Parameter","Akun Piutang","Akun Pyt","Nilai"],
		...
		this.sg1 = new saiGrid(this.pc1.childPage[2],{bound:[0,5,this.pc1.width-5,this.pc1.height-33],colCount:5,tag:9,
		...
		this.sgn1 = new portalui_sgNavigator(this.pc1.childPage[2],{bound:[0,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:3,grid:this.sg1,pager:[this,"doPager"]});
...
window.app_saku3_transaksi_tarbak_simak_fAkruBillOpen.extend(window.childForm);
window.app_saku3_transaksi_tarbak_simak_fAkruBillOpen.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into sis_bill_m(no_bill,kode_lokasi,periode,periode_bill,tanggal,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_app,posted,tgl_input,nik_user) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.c_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','PIUOPEN','BILL','IDR',1,"+nilaiToFloat(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"','F',getdate(),'"+this.app._userLog+"')");
					var j = 0;
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i) && this.sg.cells(5,i) != "0"){
								sql.add("insert into sis_bill_j(no_bill,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(3,i)+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.sg.cells(5,i))+",'"+this.sg.cells(0,i)+"','-','"+this.app._lokasi+"','PIUOPEN','PIU','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");								
								j = i+1000;
								sql.add("insert into sis_bill_j(no_bill,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sg.cells(4,i)+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.sg.cells(5,i))+",'"+this.sg.cells(0,i)+"','-','"+this.app._lokasi+"','PIUOPEN','PIU','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");								
							}
						}
					}		
					...
					sql.add("insert into sis_bill_d (no_bill,kode_lokasi,nis,kode_kelas,kode_param,nilai,dc,periode,akun_piutang,modul,kode_pp,kode_ta,kode_sem,flag_pyt) "+
							"select '"+this.e_nb.getText()+"','"+this.app._lokasi+"',a.nis,b.kode_kelas,b.kode_param,b.tarif,'D','"+this.c_periode.getText()+"',c.akun_piutang,'BILL','"+this.app._kodePP+"', '"+this.kode_ta+"','"+this.sem+"','-' "+
							"from sis_siswa a "+
							...
							"inner join sis_kelas bb on a.kode_kelas=bb.kode_kelas and a.kode_pp=bb.kode_pp and a.kode_lokasi=bb.kode_lokasi and bb.kode_tingkat='"+this.cb_tingkat.getText()+"' "+
							...
							"           from sis_bill_d "+
							"           where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.c_periode.getText()+"' and kode_pp='"+this.app._kodePP+"'"+
							"           group by nis,kode_lokasi,kode_kelas,kode_param,periode,kode_pp "+
							"			) f on b.nis=f.nis and f.kode_pp=b.kode_pp and f.kode_param=b.kode_param and f.periode='"+this.c_periode.getText()+"' and b.kode_lokasi=f.kode_lokasi "+
							...
							"	   and '"+this.c_periode.getText()+"' between b.per_awal and b.per_akhir");
	...
	doSelectDate: function(sender, y,m,d){
		...
		if (this.stsSimpan == 1) {
			...
			var data = this.dbLib.getDataProvider("select substring(convert(varchar,tgl_akhir,112),1,6) as bulanakhir "+
												  "from sis_ta "+
			...
			for (var i=1;i < 7;i++){
				ix=i;
				var bulan = "0"+ix.toString();
				if (nilaiToFloat(this.perAkhir.substr(0,4)+bulan) > nilaiToFloat(this.e_periode.getText())) 					
					this.c_periode.addItem(j,this.perAkhir.substr(0,4)+bulan);
				j++;
			}
		}
	},
	doTampil:function(sender){		
		if (this.c_periode.getText() != "" && this.cb_tingkat.getText() != "") {
			var m = this.c_periode.getText().substr(4,2);

			if(m >= 1 && m <= 6) this.sem = "GENAP";
			if(m >= 7 && m <= 12) this.sem = "GANJIL";

			var strSQL = "select a.kode_pp,c.kode_param,c.nama,c.akun_piutang,c.akun_pyt,SUM(b.tarif) as nilai "+
						 "from sis_siswa a "+
						 ...
						 "           where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.c_periode.getText()+"' and kode_pp='"+this.app._kodePP+"' "+
						 "           group by nis,kode_lokasi,kode_kelas,kode_param,periode,kode_pp "+
						 "			 ) f on b.nis=f.nis and b.kode_pp=f.kode_pp and f.kode_param=b.kode_param and f.periode='"+this.c_periode.getText()+"' and b.kode_lokasi=f.kode_lokasi "+						
						 
						 "where b.tarif<>0 and xx.flag_aktif = '1' and (f.nilai is null or f.nilai = 0) and a.kode_lokasi='"+this.app._lokasi+"' "+
						 "		and '"+this.c_periode.getText()+"' between b.per_awal and b.per_akhir and a.kode_pp='"+this.app._kodePP+"' "+
						 "group by a.kode_pp,c.kode_param,c.nama,c.akun_piutang,c.akun_pyt";
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					...
					if (parseFloat(line.nilai) > 0) {																				
						this.sg.appendData([line.kode_pp,line.kode_param,line.nama,line.akun_piutang,line.akun_pyt,floatToNilai(line.nilai)]);
					}
				}
	...
	doChange: function(sender){
		try{
			if (sender == this.c_periode || sender == this.cb_tingkat) {
				if (this.c_periode.getText()!="" && this.cb_tingkat.getText()!= "") {
					this.sg.clear(1);
				}
			}
	...
	doDblClick: function(sender, col, row){
		if (this.sg.cells(0,row) != "") {	
			var strSQL = "select a.nis,a.nama, a.kode_kelas,c.kode_param,b.tarif "+
						 ...
						 "left join (select nis,kode_lokasi,kode_kelas,kode_param,periode,kode_pp,sum(case dc when 'D' then nilai else -nilai end) as nilai "+
						            "from sis_bill_d "+
						            "where kode_lokasi='"+this.app._lokasi+"' and periode='"+this.c_periode.getText()+"' and kode_pp='"+this.sg.cells(0,row)+"' "+
						 ...
						 "where a.flag_aktif = '1' and (f.nilai is null or f.nilai = 0) and a.kode_lokasi='"+this.app._lokasi+"' and '"+this.c_periode.getText()+"' between b.per_awal and b.per_akhir and a.kode_pp='"+this.sg.cells(0,row)+"' and c.kode_param='"+this.sg.cells(1,row)+"' "+
	...
	doLoad4:function(sender){																				
		var strSQL = "select a.no_bill,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
					 ...
					 "where b.no_bill is null and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.posted='F' and a.kode_pp='"+this.app._kodePP+"' and a.modul='PIUOPEN'";						
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU4 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg4.cells(0,row) != "") {
				...
				var strSQL = "select keterangan,tanggal,periode_bill "+
							 "from sis_bill_m "+							 
				...
				if (typeof data == "object"){
					...
					if (line != undefined){								
						...
						this.c_periode.setText(line.periode_bill);						
					}					
				}					
				var strSQL = "select a.kode_pp,c.kode_param,c.nama,c.akun_piutang,c.akun_pyt,SUM(b.tarif) as nilai "+
							 "from sis_siswa a "+
							 ...
							 "            group by nis,kode_lokasi,kode_kelas,kode_param,periode,kode_pp) f on b.nis=f.nis and f.kode_kelas=b.kode_kelas and f.kode_param=b.kode_param and f.periode='"+this.c_periode.getText()+"' and b.kode_lokasi=f.kode_lokasi and b.kode_pp=f.kode_pp "+						
							 "where a.kode_lokasi='"+this.app._lokasi+"' "+
							 "group by a.kode_pp,d.kode_tingkat,c.kode_param,c.nama,c.akun_piutang,c.akun_pyt";	 
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
