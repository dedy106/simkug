window.app_saku3_transaksi_tarbak_simak_fRekonCD = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_simak_fRekonCD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_simak_fRekonCD";
		this.itemsValue = new arrayList();
		this.maximize();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Rekonsiliasi Pelunasan Tagihan via PDD", 0);	
		...
		this.e_cd = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,13,200,20],caption:"Saldo PDD", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		this.cb_titip = new saiCBBL(this.pc2.childPage[0],{bound:[20,16,220,20],caption:"Akun PDD", multiSelection:false, maxLength:10, tag:2 });
		...
		this.p1 = new panel(this.pc2.childPage[0],{bound:[1,23,995,315],caption:"Data Tagihan"});
		this.sg = new saiGrid(this.p1,{bound:[1,20,this.p1.width-5,this.p1.height-50],colCount:8,tag:0,
		            colTitle:["No Bill","Periode","Kode Param","Nama Parameter","Akun AR","Saldo Tagihan","Nilai Pelunasan","Keterangan Bill"],
					colWidth:[[7,6,5,4,3,2,1,0],[200,100,100,70,300,80,80,150]],
...
window.app_saku3_transaksi_tarbak_simak_fRekonCD.extend(window.childForm);
window.app_saku3_transaksi_tarbak_simak_fRekonCD.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into sis_rekon_m(no_rekon,no_dokumen,tanggal,keterangan,nilai,posted,modul,akun_titip,nis,nik_buat,nik_app,kode_lokasi,periode,nik_user,tgl_input,kode_pp) values "+
						    "('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"',"+parseNilai(this.e_nilai.getText())+",'F','REKONCD','"+this.cb_titip.getText()+"','"+this.cb_nim.getText()+"','"+this.app._userLog+"','"+this.app._userLog+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.cb_pp.getText()+"')");					
					...
					sql.add("insert into sis_rekon_j(no_rekon,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
							"	('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.cb_titip.getText()+"','"+this.e_ket.getText()+"','D',"+parseNilai(this.e_nilai.getText())+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','REKONCD','CD','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");							
				
					sql.add("insert into sis_cd_d(no_bukti,nis,periode,nilai,kode_lokasi,akun_cd,kode_param,dc,modul,kode_pp,no_ref1) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_nim.getText()+"','"+this.e_periode.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",'"+this.app._lokasi+"','"+this.cb_titip.getText()+"','-','C','REKONCD','"+this.cb_pp.getText()+"','-')");
					...
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i) && this.sg.cells(6,i) != "0"){
								...
								sql.add("insert into sis_rekon_j(no_rekon,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
										"	('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(4,i)+"','"+this.e_ket.getText()+"','C',"+nilaiAR+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','REKONCD','PIUT','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");
	...
	doChange: function(sender){
		if (sender == this.cb_nim && this.cb_nim.getText()!="" && this.stsSimpan==1) {
			...
			var strSQL = "select a.no_bill,a.periode,a.kode_param,a.nama,a.akun_piutang,a.tot_bill-isnull(b.tot_lunas,0) as saldo,a.ket_bill "+
						 "from ("+

						 "		select a.nis,a.no_bill,a.periode,a.akun_piutang,sum(a.nilai) as tot_bill,a.kode_param,aa.nama,c.idx,  e.keterangan as ket_bill "+
						 "		from sis_bill_d a "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData([line.no_bill,line.periode,line.kode_param,line.nama,line.akun_piutang,floatToNilai(line.saldo),"0",line.ket_bill]);
				}
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_rekon,convert(varchar,a.tanggal,103) as tgl,a.nis,a.keterangan "+
		             "from sis_rekon_m a "+
					 "where a.kode_pp='"+this.cb_pp.getText()+"' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'REKONCD' and a.posted ='F'";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				sebelum diubah -- query salah saldo jadi minus, krn bayar terquery double
				...
				*/
				...
				var strSQL = "select a.no_bill,a.periode,a.kode_param,a.nama,a.akun_piutang,a.tot_bill-isnull(b.tot_lunas,0) as saldo,dd.nilai,dd.nilai_cd,dd.id_bank, a.ket_bill  "+
							 "from ("+
	
							 "		select a.nis,a.no_bill,a.periode,a.akun_piutang,sum(a.nilai) as tot_bill,a.kode_param,aa.nama,c.idx,a.kode_lokasi,e.keterangan as ket_bill "+
							 "		from sis_bill_d a "+
							 "			inner join sis_bill_m e on a.no_bill=e.no_bill and a.kode_lokasi=e.kode_lokasi "+
							 "      	inner join sis_param aa on a.kode_param=aa.kode_param and a.kode_lokasi=aa.kode_lokasi "+
							 "      	inner join sis_kelas d on a.kode_kelas=d.kode_kelas and a.kode_lokasi=d.kode_lokasi and e.kode_pp=d.kode_pp "+						
							 "      	inner join sis_param_idx c on a.kode_param = c.kode_param and a.kode_lokasi=c.kode_lokasi and a.kode_pp=c.kode_pp and c.kode_pp='"+this.cb_pp.getText()+"' "+						
							 "		where a.nis = '"+this.cb_nim.getText()+"' and a.periode<='"+this.e_periode.getText()+"' and e.kode_pp= '"+this.cb_pp.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' "+
							 "		group by a.nis,a.no_bill,a.periode,a.akun_piutang,a.kode_param,aa.nama,c.idx,a.kode_lokasi,e.keterangan "+
							 ") a "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];		
						this.sg.appendData([line.no_bill,line.periode,line.kode_param,line.nama,line.akun_piutang,floatToNilai(line.saldo),floatToNilai(line.nilai),line.ket_bill]);
					}
				} else this.sg.clear(1);
				this.sg.validasi();
