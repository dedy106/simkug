window.app_saku3_transaksi_tarbak_simak_fGbrDepan = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_simak_fGbrDepan.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_simak_fGbrDepan";
		...
		this.pc1 = new pageControl(this,{bound:[10,12,1000,450], childPage:["Daftar Gbr Depan","Data Gambar"]});				
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:4,tag:9,
					...
					colWidth:[[3,2,1,0],[500,100,100,100]],
					readOnly:true,
					dblClick:[this,"doDoubleClick"],autoAppend:false,defaultRow:1});		
		this.sgn1 = new portalui_sgNavigator(this.pc1.childPage[0],{bound:[1,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:3,grid:this.sg1,pager:[this,"doPager"]});
		
		this.cb_pp = new saiCBBL(this.pc1.childPage[1],{bound:[20,14,220,20],caption:"Sekolah", multiSelection:false, maxLength:10, tag:2});				
		...
		this.dp_d2 = new portalui_datePicker(this.pc1.childPage[1],{bound:[120,12,98,18]}); 

		this.e_foto = new saiLabelEdit(this.pc1.childPage[1], {bound:[20,11,470,20], caption:"Gambar",readOnly:true});
		this.u_foto = new uploader(this.pc1.childPage[1], {bound:[500,11,80,20],param3: "object", param4 :"server/media/",param2 :"server/tmp/", param1 : "uploadTo",
		...
		try {
			...
			this.cb_pp.setSQL("select a.kode_pp,a.nama from pp a "+
							  "where a.kode_pp='"+this.app._kodePP+"' and a.kode_lokasi='"+this.app._lokasi+"'",["kode_pp","nama"],false,["Kode","Nama"],"and","Data Sekolah",true);						
...
window.app_saku3_transaksi_tarbak_simak_fGbrDepan.extend(window.childForm);
window.app_saku3_transaksi_tarbak_simak_fGbrDepan.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					this.nb = this.standarLib.noBuktiOtomatis(this.dbLib,"sis_gbr_depan","no_bukti",this.cb_pp.getText()+"-GBD"+this.tahun+".","00000");

					sql.add("insert into sis_gbr_depan(no_bukti,kode_lokasi,kode_pp,tgl_mulai,tgl_selesai,gambar) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();										
					sql.add("delete from sis_gbr_depan where no_bukti='"+this.nb+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("insert into sis_gbr_depan(no_bukti,kode_lokasi,kode_pp,tgl_mulai,tgl_selesai,gambar) values "+
						    "('"+this.nb+"', '"+this.app._lokasi+"','"+this.cb_pp.getText()+"','"+this.dp_d1.getDateString()+"','"+this.dp_d2.getDateString()+"','"+this.e_foto.getText()+"')");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {			
				...
				var strSQL = "select * from sis_gbr_depan where no_bukti ='"+this.nb+"' and kode_lokasi='"+this.app._lokasi+"'";						   
				var data = this.dbLib.getDataProvider(strSQL,true);
	...
	doLoad:function(sender){						
		var strSQL = "select *,convert(varchar,tgl_mulai,103) as tgl1,convert(varchar,tgl_selesai,103) as tgl2 from sis_gbr_depan where kode_pp='"+this.app._kodePP+"' and kode_lokasi='"+this.app._lokasi+"' order by tgl_mulai desc";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
