window.app_saku3_transaksi_tarbak_hutang_fPbNonAkru = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_hutang_fPbNonAkru.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_hutang_fPbNonAkru";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Permohonan Bayar Non Akru: Input/Edit", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,348], childPage:["Otorisasi","Atensi Pembayaran","Jurnal","Data Anggaran","Data Approval","Budget"]});
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:0,
		            colTitle:["Kode Mitra","Nama","Bank","Cabang","No Rekening","Nama Rekening","N Tansfer","Pajak","Keterangan"],
...
window.app_saku3_transaksi_tarbak_hutang_fPbNonAkru.extend(window.childForm);
window.app_saku3_transaksi_tarbak_hutang_fPbNonAkru.implement({	
						...
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){
								sql.add("insert into yk_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"','"+this.sg1.cells(7,i)+"','"+this.app._lokasi+"','"+this.modul+"','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");										
							}
						}
					}														
	...
	doLoad3:function(sender){																				
		...
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.modul,a.no_dokumen,a.keterangan,a.progress,a.nilai "+
		             "from yk_pb_m a "+					 					 
					 "where "+pp+" a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'PBGAR' and a.progress in ('0','R','V') "+
					 "union "+
					 "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.modul,a.no_dokumen,a.keterangan,a.progress,a.nilai "+
		             "from yk_pb_m a inner join takterima_m b on a.no_pb=b.no_terima and a.kode_lokasi=b.kode_lokasi and b.posted='T' and b.periode<'"+this.app._periode+"' "+					 					 
					 "where "+pp+" a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'PBTAK' and a.progress in ('0','R','V') ";
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				if (this.modul == "PBTAK") {
					var data = this.dbLib.getDataProvider("select kode_lokasi from takkirim_m where no_terima='"+this.e_nb.getText()+"' and kode_loktuj='"+this.app._lokasi+"'",true);
					if (typeof data == "object"){
						var line = data.rs.rows[0];							
						if (line != undefined){													
				...
				else this.kodeLokAsal = this.app._lokasi;
				
				if (this.progSeb == "V") {
					var modulApp = this.modul+"_SPB"; 
