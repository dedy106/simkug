window.app_saku3_transaksi_tarbak_hutang_fKbBmhd = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_hutang_fKbBmhd.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_hutang_fKbBmhd";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank BMHD", 0);	
		...
		this.sg5 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:8,tag:0,
		            colTitle:["No BMHD","No SPB","Tanggal","Deskripsi","Akun BYMHD","Saldo","Nilai","Rekening"],
					colWidth:[[7,6,5,4,3,2,1,0],[70,100,100,80,300,80,110,110]],
					columnReadOnly:[true,[0,1,2,3,4,5,7],[6]],
					colFormat:[[5,6,7],[cfNilai,cfNilai,cfButton]], colAlign:[[7],[alCenter]],
...
window.app_saku3_transaksi_tarbak_hutang_fKbBmhd.extend(window.childForm);
window.app_saku3_transaksi_tarbak_hutang_fKbBmhd.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from tar_hutang_bayar where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}			
					...
					for (var i=0;i < this.dataJU5.rs.rows.length;i++){
						...
						if (nilaiToFloat(line.nilai) != 0) {
							sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
									"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+line.akun_hutang+"','"+line.keterangan+"','D',"+nilaiToFloat(line.nilai)+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBBMHD','BMHD','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(line.nilai)+")");	

							sql.add("insert into tar_hutang_bayar (no_bukti,kode_lokasi,tgl_input,nik_user,tanggal,periode,no_hutang,no_spb,nilai,modul,dc) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.dp_d1.getDateString()+"','"+this.e_periode.getText()+"','"+line.no_bukti+"','"+line.no_spb+"',"+nilaiToFloat(line.nilai)+",'KBBMHD','D')");																					
						}
					}
	...
	doLoad:function(sender){				
		this.dataJU5 = {rs:{rows:[]}};
        var strSQL = "select a.no_bukti,a.no_spb,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai-isnull(b.bayar,0) as saldo,0 as nilai,a.akun_hutang "+
					 "from tar_hutang_d a "+
					 "		left join ("+
					 "    		select no_spb,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar "+
					 "			from tar_hutang_bayar where kode_lokasi='"+this.app._lokasi+"' "+
					 "			group by no_spb,kode_lokasi "+
	...
	doTampilData5: function(page) {		
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU5.rs.rows[i];									
			this.sg5.appendData([line.no_bukti,line.no_spb,line.tgl,line.keterangan,line.akun_hutang,floatToNilai(line.saldo),floatToNilai(line.nilai),"Pilih"]);
		}
	...
	doChangeCell5: function(sender, col , row) {
		if (col == 6) {			
			this.dataJU5.rs.rows[((this.page-1)*20) + row].nilai = this.sg5.cells(6,row);			
			this.sg5.validasi();
		}
	},
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				this.dataJU5 = {rs:{rows:[]}};							 
                var strSQL = "select a.no_bukti,a.no_spb,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai-isnull(b.bayar,0) as saldo,x.nilai,a.akun_hutang "+
							 "from tar_hutang_d a "+
							 "		inner join tar_hutang_bayar x on a.no_spb=x.no_spb and a.kode_lokasi=x.kode_lokasi and x.no_bukti='"+this.e_nb.getText()+"' "+
							 "		left join ("+
							 "    		select no_spb,kode_lokasi,sum(case dc when 'D' then nilai else -nilai end) as bayar "+
							 "			from tar_hutang_bayar where kode_lokasi='"+this.app._lokasi+"' and no_bukti <> '"+this.e_nb.getText()+"' "+
							 "			group by no_spb,kode_lokasi "+
							 "		) b on a.no_spb=b.no_spb and a.kode_lokasi=b.kode_lokasi "+
