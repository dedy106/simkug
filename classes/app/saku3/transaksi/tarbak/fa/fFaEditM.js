window.app_saku3_transaksi_tarbak_fa_fFaEditM = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_fa_fFaEditM.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_fa_fFaEditM";
		...
		this.e_fa = new saiCBBL(this,{bound:[20,10,220,20],caption:"ID Barang", multiSelection:false, maxLength:20, tag:1,change:[this,"doChange"],rightLabelVisible:false});
		...
		try {
			...
			this.e_fa.setSQL("select a.no_fa, a.nama from fa_asset a where a.kode_lokasi='"+this.app._lokasi+"'",["no_fa","nama"],false,["ID","Nama"],"where","Data Barang",true);

			this.cb_klp.setSQL("select kode_klpfa, nama from fa_klp where kode_lokasi='"+this.app._lokasi+"'",["kode_klpfa","nama"],false,["Kode","Nama"],"where","Data Kelompok Aktap",true);
...
window.app_saku3_transaksi_tarbak_fa_fFaEditM.extend(window.childForm);
window.app_saku3_transaksi_tarbak_fa_fFaEditM.implement({
	...
	doChange:function(sender){	
		if (sender == this.e_fa && this.e_fa.getText()!="") {
			var strSQL = "select a.*,isnull(b.susut,0) as susut,c.nilai_oleh "+
						 ...
						 ") c on a.no_fa=c.no_fa and a.kode_lokasi=c.kode_lokasi "+
