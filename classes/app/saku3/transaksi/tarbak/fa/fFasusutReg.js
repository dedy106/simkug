window.app_saku3_transaksi_tarbak_fa_fFasusutReg = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_fa_fFasusutReg.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_fa_fFasusutReg";
...
window.app_saku3_transaksi_tarbak_fa_fFasusutReg.extend(window.childForm);
window.app_saku3_transaksi_tarbak_fa_fFasusutReg.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into fasusut_d(no_fasusut,no_fa,periode,nilai,kode_lokasi,akun_bp,akun_ap,kode_akun,kode_pp,kode_drk,dc,no_del,nilai_aset,umur) "+
							...
							"	 case when (zz.nilai-a.nilai_residu-isnull(b.tot_susut,0)) > ceiling(zz.nilai/a.umur) "+
							...
							"  from fa_asset a "+
							"	 inner join fa_klpakun c on a.kode_klpakun=c.kode_klpakun and c.flag_susut='1' "+
							...
							"  where a.jenis='A' and a.progress = '2' and (zz.nilai-a.nilai_residu) > isnull(b.tot_susut,0) and a.kode_lokasi='"+this.app._lokasi+"' and a.periode_susut<='"+this.e_periode.getText()+"' ");
	...
	doLoadData: function(sender){
		if (this.cb_drk.getText() != "") {
			...
			var strSQL = "select b.akun_bp,x.nama as nama_bp,b.akun_deprs,y.nama as nama_deprs,a.kode_pp_susut,c.nama as nama_pp, "+
						 ...
						 "inner join (select kode_lokasi,no_fa,sum(case dc when 'D' then nilai else -nilai end) as nilai "+
						 "            from fa_nilai where periode<='"+this.e_periode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' group by kode_lokasi,no_fa) zz on a.no_fa=zz.no_fa and a.kode_lokasi=zz.kode_lokasi "+
						 "inner join fa_klpakun b on a.kode_klpakun=b.kode_klpakun and b.flag_susut='1' "+
