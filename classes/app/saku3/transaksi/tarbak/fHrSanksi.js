window.app_saku3_transaksi_tarbak_fHrSanksi = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_fHrSanksi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_fHrSanksi";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Sanksi : Input/Edit", 0);	
		
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		uses("saiGrid",true);		
		this.pc1 = new pageControl(this,{bound:[20,12,1000,450], childPage:["Daftar Sanksi","Data Sanksi","Filter Data"]});				
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[0,5,this.pc1.width-5,this.pc1.height-90],colCount:3,tag:0,
		            colTitle:["Keterangan","Jenis","Tanggal"],
					colWidth:[[2,1,0],[100,200,300]],
					defaultRow:1,
...
window.app_saku3_transaksi_tarbak_fHrSanksi.extend(window.childForm);
window.app_saku3_transaksi_tarbak_fHrSanksi.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("delete from hr_sanksi where nik = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");			
					
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into hr_sanksi(nik,kode_lokasi,nu,nama,tanggal,jenis) values "+
										"('"+this.cb_kode.getText()+"','"+this.app._lokasi+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.getCellDateValue(2,i)+"','"+this.sg.cells(1,i)+"')");
							}
						}
					}			
					setTipeButton(tbAllFalse);
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select *,convert(varchar,tanggal,103) as tgl from hr_sanksi where nik='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];													
						this.sg.appendData([line.nama,line.jenis,line.tgl]);
					}
