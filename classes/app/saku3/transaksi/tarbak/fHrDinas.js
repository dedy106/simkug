window.app_saku3_transaksi_tarbak_fHrDinas = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_fHrDinas.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_fHrDinas";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Kedinasan : Input/Edit", 0);	
		
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		uses("saiGrid",true);		
		this.pc1 = new pageControl(this,{bound:[20,12,1000,450], childPage:["Daftar Kedinasan","Data Kedinasan","Filter Data"]});				
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-30],colCount:2,tag:9,
		            colTitle:["NIK","Nama","Jumlah"],
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[0,5,this.pc1.width-5,this.pc1.height-90],colCount:3,tag:0,
		            colTitle:["No SK","Tgl SK","Keterangan"],
					colWidth:[[2,1,0],[400,100,200]],
...
window.app_saku3_transaksi_tarbak_fHrDinas.extend(window.childForm);
window.app_saku3_transaksi_tarbak_fHrDinas.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("delete from hr_sk where nik = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");			
					
					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into hr_sk(nik,kode_lokasi,nu,no_sk,nama,tgl_sk) values "+
										"('"+this.cb_kode.getText()+"','"+this.app._lokasi+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(2,i)+"','"+this.sg.getCellDateValue(1,i)+"')");
							}
						}
					}			
					setTipeButton(tbAllFalse);
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select *,convert(varchar,tgl_sk,103) as tgl from hr_sk where nik='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];													
						this.sg.appendData([line.no_sk,line.tgl,line.nama]);
					}
