window.app_saku3_transaksi_tarbak_kug_fKbIFClose = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_kug_fKbIFClose.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_kug_fKbIFClose";
		...
		this.cb_reim = new saiCBBL(this.pc2.childPage[0],{bound:[20,12,220,20],caption:"No Reimburse", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});							
		this.cb_nik = new saiCBBL(this.pc2.childPage[0],{bound:[20,11,220,20],caption:"NIK IF", readOnly:true, tag:1,change:[this,"doChange"]});							
		this.cb_akunif = new saiCBBL(this.pc2.childPage[0],{bound:[20,12,220,20],caption:"Akun IF",  readOnly:true, tag:1});							
		this.e_nilai = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,17,200,20],caption:"Nilai IF", tag:1, tipeText:ttNilai, text:"0", readOnly:true});				
		...
		this.e_reim = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,13,200,20],caption:"Total Reimburse", tag:1, tipeText:ttNilai, text:"0", readOnly:true});				
		
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,996,236], childPage:["Item Reimburse"]});		
...
window.app_saku3_transaksi_tarbak_kug_fKbIFClose.extend(window.childForm);
window.app_saku3_transaksi_tarbak_kug_fKbIFClose.implement({	
	isiCBReim: function() {
		this.cb_reim.setSQL("select no_hutang, keterangan from hutang_m where periode <='"+this.e_periode.getText()+"' and kode_project='CLOSE' and modul='IFREIM' and kode_lokasi='"+this.app._lokasi+"' and no_ref='-' ",["no_hutang","keterangan"],false,["No Bukti","Deskripsi"],"and","Data Penutupan",true);
	},
	...
	simpan: function(){			
		try{						
			if (this.stsSimpan == 1) {
				if (this.e_kas.getText() == "0") this.c_jenis.setText("CL");
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("update hutang_m set no_ref='-' where modul='IFREIM' and no_ref='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");		
					}
								
					//hutang_m tidak diposting
					sql.add("update hutang_m set no_ref='"+this.e_nb.getText()+"' where modul='IFREIM' and no_hutang='"+this.cb_reim.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");		
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_reim.getText()+"','-','"+this.cb_akunkb.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.kodePP+"','KBIFCLOSE','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_kas.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','-','-','-')");									

					if (this.e_kas.getText() != "0") {
						sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
								"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',777,'"+this.cb_akunkb.getText()+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_kas.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBIFCLOSE','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");					
					}
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) "+
							"select '"+this.e_nb.getText()+"',no_hutang,'"+this.dp_d1.getDateString()+"',no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,'-','-',kode_lokasi,modul,jenis,'"+this.e_periode.getText()+"',kode_curr,kurs,'"+this.app._userLog+"',getdate(),'-' "+
							"from hutang_j where no_hutang='"+this.cb_reim.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doChange:function(sender){
		try {										
			...
			if (sender == this.cb_reim && this.cb_reim.getText()!="") {
				var strSQL = "select b.nik,b.akun_if,b.nilai,a.akun_hutang "+
							 "from hutang_m a "+
							 "inner join if_nik b on a.no_dokumen=b.no_kas and a.kode_lokasi=b.kode_lokasi "+
							 "where a.no_hutang='"+this.cb_reim.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
				...
				var data = this.dbLib.getDataProvider(
							...
							"where a.jenis='BEBAN' and a.no_hutang = '"+this.cb_reim.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
	...
	doNilaiChange1: function(){		
		try{
			...
			this.e_reim.setText(floatToNilai(totD - totC));
			this.e_kas.setText(floatToNilai(nilaiToFloat(this.e_nilai.getText()) - totD - totC));
		}catch(e)
		{
	...
	doLoad3:function(sender){																									
		var strSQL = "select distinct a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai,a.tanggal "+
		             "from kas_m a "+			 					 
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='KBIFCLOSE' and a.posted ='F' "+
					 "order by a.tanggal";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {		
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.tanggal,a.jenis,a.keterangan,a.akun_kb,b.no_hutang "+
							 "from kas_m a inner join hutang_m b on a.no_kas=b.no_ref and a.kode_lokasi=b.kode_lokasi "+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){		
						...
						this.cb_reim.setSQL("select no_hutang, keterangan from hutang_m where no_hutang='"+line.no_hutang+"' and kode_lokasi='"+this.app._lokasi+"' and no_ref='"+this.e_nb.getText()+"' ",["no_hutang","keterangan"],false,["No Bukti","Deskripsi"],"and","Data Penutupan",true);
						this.cb_reim.setText(line.no_hutang);
					}
				}				
			}
		} catch(e) {alert(e);}		
