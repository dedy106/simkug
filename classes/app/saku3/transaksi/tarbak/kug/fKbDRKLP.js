window.app_saku3_transaksi_tarbak_kug_fKbDRKLP = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_kug_fKbDRKLP.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_kug_fKbDRKLP";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Umum Lintas Periode", 0);	
		
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		this.e_periode = new portalui_saiLabelEdit(this,{bound:[810,11,200,20],caption:"Periode",tag:2,readOnly:true,change:[this,"doChange"]});
		...
		this.pc1 = new pageControl(this.pc2.childPage[1],{bound:[1,12,996,305], childPage:["Data Item Jurnal","Referensi Jurnal"]});
...
window.app_saku3_transaksi_tarbak_kug_fKbDRKLP.extend(window.childForm);
window.app_saku3_transaksi_tarbak_kug_fKbDRKLP.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBUMUM','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_debet.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','T','-','-','-','-')");																
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								...
								sql.add("insert into gldt_h(no_bukti,no_urut,kode_lokasi,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,modul,jenis,periode,kode_curr,kurs,nilai_curr,tgl_input,nik_user) values "+
										"('"+this.e_nb.getText()+"',"+i+",'"+this.app._lokasi+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.sg.cells(0,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+Math.abs(nilaiToFloat(this.sg.cells(4,i)))+",'"+this.sg.cells(3,i)+"','"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','-','-','-','-','-','"+this.app._userLog+"','KBUMUM','UMUM','"+this.e_periode.getText()+"','IDR',1,"+Math.abs(nilaiToFloat(this.sg.cells(4,i)))+",getdate(),'"+this.app._userLog+"')");
							}
						}
					}					
		...
		switch (event){
			...
			case "ubah" :																			
				...
				if (parseFloat(this.app._periode) <= parseFloat(this.e_periode.getText())){
					system.alert(this,"Periode transaksi tidak valid.","Periode transaksi harus lebih rendah dari periode sistem.");
					return false;
				}
				else 				
				this.simpan();
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_kas,line.tgl,line.jenis,line.no_dokumen,line.keterangan,floatToNilai(line.nilai),"Pilih"]); 
		}
