window.app_saku3_transaksi_tarbak_kug_fPtgAju = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_kug_fPtgAju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_kug_fPtgAju";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Pengajuan Pertanggungan PJ", 0);	
		...
		this.sg3 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:6,tag:9,
		            colTitle:["No Pertanggungan","Tanggal","Deskripsi","Nilai","Progress","Pilih"],
					colWidth:[[5,4,3,2,1,0],[70,150,100,350,80,100]],
		...
		this.cb_nopj = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,13,220,20],caption:"No Panjar",tag:1,multiSelection:false,change:[this,"doChange"]}); 						
		this.cb_nik = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,15,220,20],caption:"NIK Panjar",tag:1,readOnly:true}); 						
		this.e_saldo = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,14,200,20],caption:"Nilai Panjar", tag:1, tipeText:ttNilai, text:"0",readOnly:true});		
		this.cb_pp = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,13,220,20],caption:"Bagian / Unit",tag:2,readOnly:true}); 										
		this.e_kas = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,13,200,20],caption:"Nilai Kas", tag:1, tipeText:ttNilai, text:"0",readOnly:true});
		this.e_ket = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,16,550,20],caption:"Uraian", maxLength:150});					
		this.e_total = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,16,200,20],caption:"Total Pertggn.", tag:1, tipeText:ttNilai, text:"0",readOnly:true});
		
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,996,259], childPage:["Item Ptg","Budget","File Dok","Otorisasi","Cattn Verf."]});		
...
window.app_saku3_transaksi_tarbak_kug_fPtgAju.extend(window.childForm);
window.app_saku3_transaksi_tarbak_kug_fPtgAju.implement({	
	isiCBnopj: function() {
		this.cb_nopj.setSQL("select distinct b.no_pj, b.keterangan from panjar_m b "+
						   ...
						   "where b.nik_pengaju='"+this.app._userLog+"' and b.kode_lokasi='"+this.app._lokasi+"' and b.progress='2'",["b.no_pj","b.keterangan"],false,["No Bukti","Deskripsi"],"and","Data Panjar",true);				
	},
	...
	doHitungGar: function(){
		try {
			...
			for (var i=0;i < this.sg2.getRowCount();i++){			
				if (this.stsSimpan == 1) 
					var data = this.dbLib.getDataProvider("select fn_saldotw('"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(1,i)+"','"+this.e_periode.getText()+"','"+this.cb_nopj.getText()+"') as gar ",true);
				else var data = this.dbLib.getDataProvider("select fn_saldotw('"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(1,i)+"','"+this.e_periode.getText()+"','"+this.e_nb.getText()+"') as gar ",true);
	...
	simpan: function(){			
		try{						
			if (this.stsSimpan == 1) this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"ptg_m","no_ptg",this.app._lokasi+"-PP"+this.e_periode.getText().substr(2,4)+".","0000"));
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from angg_r where no_bukti ='"+this.cb_nopj.getText()+"' and modul='PJPTG' and kode_lokasi='"+this.app._lokasi+"'");
					}
					...
					sql.add("insert into ptg_m(no_ptg, no_pj, no_kas, no_dokumen, tanggal, keterangan, catatan, kode_curr, kurs, akun_pj, akun_kas, nik_buat, nik_setuju, kode_lokasi, kode_pp, modul, nilai, nilai_kas, kode_drk, progress, posted, periode, no_del, no_link, nik_user, tgl_input) values  "+
							"('"+this.e_nb.getText()+"', '"+this.cb_nopj.getText()+"', '-', '-', '"+this.dp_d1.getDateString()+"', '"+this.e_ket.getText()+"', '-', 'IDR', 1, '"+this.akunPJ+"', '-', '"+this.cb_nik.getText()+"', '"+this.cb_app.getText()+"','"+this.app._lokasi+"', '"+this.kodePP+"', 'PJPTG', "+nilaiToFloat(this.e_total.getText())+", "+nilaiToFloat(this.e_kas.getText())+", '-', '0', 'X', '"+this.e_periode.getText()+"', '-', '-', '"+this.app._userLog+"', getdate())");									
					sql.add("insert into ptg_j(no_ptg, no_dokumen, tanggal, no_urut, kode_akun, keterangan, dc, nilai, kode_pp, kode_drk, kode_lokasi, modul, jenis, periode, kode_curr, kurs, nik_user, tgl_input, no_link) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',999,'"+this.akunPJ+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_saldo.getText())+",'"+this.kodePP+"','-','"+this.app._lokasi+"','PTGPJ','PJ','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");					
					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){								
								sql.add("insert into ptg_j(no_ptg, no_dokumen, tanggal, no_urut, kode_akun, keterangan, dc, nilai, kode_pp, kode_drk, kode_lokasi, modul, jenis, periode, kode_curr, kurs, nik_user, tgl_input, no_link) values "+
										"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"','-','"+this.app._lokasi+"','PTGPJ','BEBAN','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");					
							}
						}
					}
					...
					sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) "+
							...
							"from angg_r where modul='PJAJU' and no_bukti = '"+this.cb_nopj.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					sql.add("insert into pbh_pb_m (no_pb,no_dokumen,kode_lokasi,periode,nik_user,tgl_input,tanggal,due_date,keterangan,nilai,modul,progress,kode_pp,nik_app,nik_tahu,no_hutang,no_app,no_spb,no_ver,kode_bidang,kode_loktuj,nilai_final,posted,kode_proyek,no_app2,no_app3,no_fiat,no_kas,akun_hutang) values  "+
							"('"+this.e_nb.getText()+"','"+this.cb_nopj.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.dp_d1.getDateString()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_total.getText())+",'PJPTG','0','"+this.kodePP+"','"+this.cb_app.getText()+"','"+this.cb_ver.getText()+"','-','-','-','-','"+this.kodePP+"','"+this.app._lokasi+"',"+nilaiToFloat(this.e_total.getText())+",'X','-','-','-','-','-','-')");
					sql.add("insert into pbh_rek(no_bukti,kode_lokasi,modul,nama_rek,no_rek,bank,nama,bruto,pajak,nilai) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','PJPTG','"+this.e_namarek.getText()+"','"+this.e_norek.getText()+"','"+this.e_bank.getText()+"','"+this.e_namarek.getText()+"',"+nilaiToFloat(this.e_total.getText())+",0,"+nilaiToFloat(this.e_total.getText())+")");		
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"('"+this.e_nb.getText()+"','PJPTG','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(1,i)+"','"+this.sg2.cells(2,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(2,i))+","+nilai+")");
							}
						}
					}
		...
		switch (event){
			...
			case "ubah" :
				...
				if (nilaiToFloat(this.e_saldo.getText()) < nilaiToFloat(this.e_total.getText())) {
					system.alert(this,"Nilai transaksi tidak valid.","Nilai Pertanggungan tidak boleh melebihi Nilai Panjar.");
					return false;
				}
	...
	doClick:function(sender){
		...
		this.stsSimpan=1;
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"ptg_m","no_ptg",this.app._lokasi+"-PP"+this.e_periode.getText().substr(2,4)+".","0000"));
		this.cb_nopj.setFocus();
		setTipeButton(tbSimpan);
	},		
	doChange:function(sender){				
		if (sender == this.cb_nopj && this.cb_nopj.getText()!="" && this.stsSimpan==1) {
			var strSQL = "select a.no_pj, a.keterangan, a.nilai, a.nik_pengaju, b.nama, a.akun_pj, a.kode_pp, b.bank+' - '+b.cabang as bank, b.no_rek,b.nama_rek "+
			...
			if (typeof data == "object"){
				...
				if (line != undefined){							
					...
					this.cb_nik.setText(line.nik_pengaju,line.nama);					
			...
			var data = this.dbLib.getDataProvider(
						...
						"where a.jenis='BEBAN' and a.no_pj = '"+this.cb_nopj.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg1.clear();
				for (var i in data.rs.rows){
	...
	doNilaiChange1: function(){		
		try{
			...
			this.e_kas.setText(floatToNilai(nilaiToFloat(this.e_saldo.getText()) - totD - totC));
		}catch(e)
		{
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1) {	
							...
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_pbh_ypt_rptPanjarForm";
	...
	doLoad3:function(sender){																				
		this.app._periode = this.dbLib.getPeriode(this.app._lokasi);																			
		var strSQL = "select a.no_ptg,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai, "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.posted in ('F','X') and b.no_pb is null "+
					 "and a.modul = 'PJPTG' and a.kode_pp in (select kode_pp from karyawan_pp where nik='"+this.app._userLog+"' and kode_lokasi='"+this.app._lokasi+"') "+
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_ptg,line.tgl,line.keterangan,floatToNilai(line.nilai),line.status.toUpperCase(),"Pilih"]); 
		}
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.keterangan,a.tanggal,b.nik_app,b.nik_tahu,a.no_pj "+
				...
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){			
						this.cb_nopj.setSQL("select a.no_pj, a.keterangan from panjar_m a where a.no_pj='"+line.no_pj+"' and a.kode_lokasi='"+this.app._lokasi+"'",["a.no_pj","a.keterangan"],false,["No Bukti","Deskripsi"],"and","Data Panjar",true);
						this.cb_nopj.setText(line.no_pj);			
						this.e_ket.setText(line.keterangan);						
						this.dp_d1.setText(line.tanggal);						
						this.cb_app.setText(line.nik_app);
						this.cb_ver.setText(line.nik_tahu);													
					}
				}		
