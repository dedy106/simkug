window.app_saku3_transaksi_tarbak_kug_fJuLintasDRK = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_kug_fJuLintasDRK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_kug_fJuLintasDRK";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Jurnal Lintas Periode DRK", 0);	
		
		uses("saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		this.e_periode = new portalui_saiLabelEdit(this,{bound:[810,11,200,20],caption:"Periode",tag:2,readOnly:true});
		this.l_tgl1 = new portalui_label(this,{bound:[20,11,100,18],caption:"Tanggal", underline:true});
		this.dp_d1 = new portalui_datePicker(this,{bound:[120,11,100,18],selectDate:[this,"doSelectDate"]}); 		
		this.c_jenis = new saiCB(this,{bound:[810,12,200,20],caption:"Jenis",items:["ADJ"], readOnly:true,tag:2});
		...
		this.e_debet = new saiLabelEdit(this,{bound:[810,16,200,20],caption:"Total Debet", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		...
		this.pc1 = new pageControl(this,{bound:[20,12,990,320], childPage:["Data Item Jurnal","Data Anggaran"]});
...
window.app_saku3_transaksi_tarbak_kug_fJuLintasDRK.extend(window.childForm);
window.app_saku3_transaksi_tarbak_kug_fJuLintasDRK.implement({
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){	
								...
								sql.add("insert into gldt_h(no_bukti,no_urut,kode_lokasi,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,kode_drk,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,modul,jenis,periode,kode_curr,kurs,nilai_curr,tgl_input,nik_user) values "+
										"('"+this.e_nb.getText()+"',"+(nu)+",'"+this.app._lokasi+"','"+this.e_nb.getText()+"','"+this.dp_d1.getDateString()+"','"+this.sg.cells(0,i)+"','"+this.sg.cells(2,i)+"',"+Math.abs(nilaiToFloat(this.sg.cells(4,i)))+",'"+this.sg.cells(3,i)+"','"+this.sg.cells(5,i)+"','"+this.sg.cells(7,i)+"','-','-','-','-','-','"+this.cb_buat.getText()+"','JU','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+Math.abs(nilaiToFloat(this.sg.cells(4,i)))+",getdate(),'"+this.app._userLog+"')");
	...
	doHitungGar: function(){
		...
		for (var i=0;i < this.sg2.getRowCount();i++){
			var data = this.dbLib.getDataProvider("select fn_cekagg2('"+this.sg2.cells(1,i)+"','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.e_periode.getText()+"') as gar ",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line = data.rs.rows[0];
				data = line.gar.split(";");
