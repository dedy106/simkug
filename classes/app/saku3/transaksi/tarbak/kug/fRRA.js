window.app_saku3_transaksi_tarbak_kug_fRRA = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_kug_fRRA.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_kug_fRRA";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","RRA Anggaran", 0);	
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[0,5,this.pc1.width-5,this.pc1.height-33],colCount:7,tag:0,
		            colTitle:["Kode MTA","Nama MTA","Kode PP","Nama PP","Bulan","Saldo","Nilai"],
					colWidth:[[6,5,4,3,2,1,0],[100,80,80,200,80,200,80]],					
					columnReadOnly:[true,[1,3,5],[0,2,4,6]],
					colFormat:[[5,6],[cfNilai,cfNilai]],
					buttonStyle:[[0,2,4],[bsEllips,bsEllips,bsAuto]], 
					picklist:[[4],[new portalui_arrayMap({items:["01","02","03","04","05","06","07","08","09","10","11","12"]})]], 					
		...
		this.sgG = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:8,tag:9,
		            colTitle:["Kode Akun","Nama Akun","Kode PP","Nama PP","Saldo Awal","Nilai","Saldo Akhir","Bulan"],
					colWidth:[[7,6,5,4,3,2,1,0],[80,80,80,80,150,80,150,80]],
...
window.app_saku3_transaksi_tarbak_kug_fRRA.extend(window.childForm);
window.app_saku3_transaksi_tarbak_kug_fRRA.implement({	
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){																
								...
								sql.add("insert into anggaran_d(no_agg,kode_lokasi,no_urut,kode_pp,kode_akun,kode_drk,volume,periode,nilai_sat,nilai,dc,satuan,nik_user,tgl_input,modul) values "+		
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',"+i+",'"+this.sg.cells(2,i)+"','"+this.sg.cells(0,i)+"','-',1,'"+periode+"',"+parseNilai(this.sg.cells(6,i))+","+parseNilai(this.sg.cells(6,i))+",'C','-','"+this.app._userLog+"',getdate(),'RRPUSAT')");
							}
						}
					}
					...
					sql.add("insert into rra_pdrk_d(no_pdrk,kode_lokasi,no_urut,kode_akun,kode_pp,kode_drk,periode,saldo,nilai,dc,target) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',999,'"+this.cb_akunT.getText()+"','"+this.cb_ppT.getText()+"','-','"+periode+"',"+nilaiToFloat(this.e_saldo.getText())+","+nilaiToFloat(this.e_terima.getText())+",'D','-')");
					
					//langsung nambah --> tidak pakai approve		
					sql.add("insert into anggaran_d(no_agg,kode_lokasi,no_urut,kode_pp,kode_akun,kode_drk,volume,periode,nilai_sat,nilai,dc,satuan,nik_user,tgl_input,modul) values "+		
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',"+i+",'"+this.cb_ppT.getText()+"','"+this.cb_akunT.getText()+"','-',1,'"+periode+"',"+nilaiToFloat(this.e_terima.getText())+","+nilaiToFloat(this.e_terima.getText())+",'D','-','"+this.app._userLog+"',getdate(),'RRPUSAT')");
		...
		try{			
			if (sender == this.c_bulanT || sender == this.cb_ppT || sender == this.cb_akunT) {				
				...
				if (this.cb_akunT.getText()!="" && this.cb_ppT.getText()!="" && this.c_bulanT.getText()!="") {					
					var data = this.dbLib.getDataProvider("select fn_saldotw('"+this.app._lokasi+"','"+this.cb_akunT.getText()+"','"+this.cb_ppT.getText()+"','"+this.e_periode.getText().substr(0,4)+this.c_bulanT.getText()+"','"+this.e_nb.getText()+"') as gar ",true);
	...
	doHitungGarTW: function(){
		...
		for (var i=0;i < this.sg.getRowCount();i++){
			if (this.sg.rowValid(i)){
				...
				for (var j=0;j < this.sgG.getRowCount();j++){
					if (this.sg.cells(0,i) == this.sgG.cells(0,j) && this.sg.cells(2,i) == this.sgG.cells(2,j) && this.sg.cells(4,i) == this.sgG.cells(7,j)) {
						isAda = true;
						idx = j;
						break;
					}
				...
				if (!isAda) {
					this.sgG.appendData([this.sg.cells(0,i),this.sg.cells(1,i),this.sg.cells(2,i),this.sg.cells(3,i),"0",floatToNilai(nilai),"0",this.sg.cells(4,i)]);
				} 
				else { 
		...
		for (var i=0;i < this.sgG.getRowCount();i++){					
			if (this.sgG.rowValid(i)){				
				var data = this.dbLib.getDataProvider("select fn_saldotw('"+this.app._lokasi+"','"+this.sgG.cells(0,i)+"','"+this.sgG.cells(2,i)+"','"+this.e_periode.getText().substr(0,4)+this.sgG.cells(7,i)+"','"+this.e_nb.getText()+"') as gar ",true);				
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line = data.rs.rows[0];
					data = line.gar.split(";");
					sls = parseFloat(data[0]) - parseFloat(data[1]);
	...
	doChangeCell: function(sender, col, row){
		...
		if (col == 0 || col == 2 || col == 4) {
			if (sender.cells(0,row) != "" && sender.cells(2,row) != "" && sender.cells(4,row) != "") {
				...
				var data = this.dbLib.getDataProvider("select fn_saldotw('"+this.app._lokasi+"','"+this.sg.cells(0,row)+"','"+this.sg.cells(2,row)+"','"+this.e_periode.getText().substr(0,4)+this.sg.cells(4,row)+"','"+this.e_nb.getText()+"') as gar ",true);								
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line = data.rs.rows[0];
					data = line.gar.split(";");
					sls = parseFloat(data[0]) - parseFloat(data[1]);
