window.app_saku3_transaksi_tarbak_kug_fPjAju = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_kug_fPjAju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_kug_fPjAju";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Pengajuan Panjar", 0);	
		...
		this.sg3 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:7,tag:9,
		            colTitle:["No Panjar","Tanggal","N I K","Deskripsi","Nilai","Progress","Pilih"],
					colWidth:[[6,5,4,3,2,1,0],[70,150,100,300,150,80,100]],
		...
		this.e_nopj = new portalui_saiLabelEdit(this.pc2.childPage[0],{bound:[20,13,200,20],caption:"No Panjar",maxLength:30,readOnly:true,visible:true});
		...
		this.l_tgl2 = new portalui_label(this.pc2.childPage[0],{bound:[790,11,100,18],caption:"Tgl Pertanggungan", underline:true});
		this.dp_d2 = new portalui_datePicker(this.pc2.childPage[0],{bound:[890,11,98,18]}); 		
		...
		this.e_nilai = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,12,200,20],caption:"Nilai Pengajuan", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,22,996,305], childPage:["Item Peruntukan","Budget","File Dok","Otorisasi & Bank","Cattn Verf.","Daftar Panjar"]});		
		...
		this.sg2 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:5,tag:9,
					colTitle:["Kode Akun","Kode PP","Saldo Awal","Nilai","Saldo Akhir"],
		...
		this.cb_app = new saiCBBL(this.pc1.childPage[3],{bound:[20,15,220,20],caption:"NIK Approve", multiSelection:false, maxLength:10, tag:2});						
		this.cb_ver = new saiCBBL(this.pc1.childPage[3],{bound:[20,16,220,20],caption:"NIK Verifikator", multiSelection:false, maxLength:10, tag:2});						
		...
		this.sg4 = new saiGrid(this.pc1.childPage[5],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:6,tag:9,
					colTitle:["No Panjar","Tanggal","Due Date","Keterangan","Nilai","Umur (Hari)"],					
					colWidth:[[5,4,3,2,1,0],[80,100,400,80,80,100]],readOnly :true,colFormat:[[4,5],[cfNilai,cfNilai]],autoAppend:false,defaultRow:1});
		this.sgn4 = new portalui_sgNavigator(this.pc1.childPage[5],{bound:[1,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:3,grid:this.sg4});		
		...
		try {
			...
			this.cb_nik.setSQL("select a.nik, a.nama from karyawan a inner join nik_panjar b on a.nik=b.nik and a.kode_lokasi=b.kode_lokasi where a.flag_aktif='1' and a.kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Data Karyawan",true);
			...
			sql.add("select distinct a.kode_akun,a.nama from masakun a "+
					"	inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag in ('051','004') "+	
			...
			this.doLoadCtt(this.e_nopj.getText());
...
window.app_saku3_transaksi_tarbak_kug_fPjAju.extend(window.childForm);
window.app_saku3_transaksi_tarbak_kug_fPjAju.implement({	
	...
	doHitungGar: function(){
		try {
			...
			for (var i=0;i < this.sg1.getRowCount();i++){
				if (this.sg1.rowValid(i) && this.sg1.cells(4,i) != "0"){				
					...
					for (var j=0;j < this.sg2.getRowCount();j++){
						if (this.sg1.cells(0,i) == this.sg2.cells(0,j) && this.sg1.cells(5,i) == this.sg2.cells(1,j)) {
							isAda = true;
							idx = j;
							break;
						}
			...
			for (var i=0;i < this.sg2.getRowCount();i++){
				var data = this.dbLib.getDataProvider("select fn_saldotw('"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(1,i)+"','"+this.e_periode.getText()+"','"+this.e_nopj.getText()+"') as gar ",true);
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from pbh_pb_m where no_pb ='"+this.e_nopj.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");
						sql.add("delete from pbh_rek where no_bukti ='"+this.e_nopj.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");
					}
					...
					sql.add("insert into pbh_pb_m (no_pb,no_dokumen,kode_lokasi,periode,nik_user,tgl_input,tanggal,due_date,keterangan,nilai,modul,progress,kode_pp,nik_app,nik_tahu,no_hutang,no_app,no_spb,no_ver,kode_bidang,kode_loktuj,nilai_final,posted,kode_proyek,no_app2,no_app3,no_fiat,no_kas,akun_hutang) values  "+
							"('"+this.e_nopj.getText()+"','"+this.cb_nik.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.dp_d1.getDateString()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",'PJAJU','0','"+this.kodePP+"','"+this.cb_app.getText()+"','"+this.cb_ver.getText()+"','-','-','-','-','"+this.kodePP+"','"+this.app._lokasi+"',"+nilaiToFloat(this.e_nilai.getText())+",'X','-','-','-','-','-','-')");
					sql.add("insert into pbh_rek(no_bukti,kode_lokasi,modul,nama_rek,no_rek,bank,nama,bruto,pajak,nilai) values "+
							"('"+this.e_nopj.getText()+"','"+this.app._lokasi+"','PJAJU','"+this.e_namarek.getText()+"','"+this.e_norek.getText()+"','"+this.e_bank.getText()+"','"+this.e_namarek.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+",0,"+nilaiToFloat(this.e_nilai.getText())+")");		
					...
					sql.add("insert into panjar_m(no_pj, no_kas, no_dokumen, tanggal, due_date, keterangan, catatan, kode_curr, kurs, akun_pj, nik_pengaju, nik_setuju, kode_lokasi, kode_pp, modul, nilai, nilai_pot, kode_drk, progress, periode, no_del, no_link, nik_user, tgl_input) values "+
						    "('"+this.e_nopj.getText()+"', '-', '-', '"+this.dp_d1.getDateString()+"', '"+this.dp_d1.getDateString()+"', '"+this.e_ket.getText()+"', '-', 'IDR', 1, '"+this.cb_akunpj.getText()+"', '"+this.cb_nik.getText()+"', '"+this.cb_app.getText()+"', '"+this.app._lokasi+"', '"+this.kodePP+"', 'PJAJU', "+nilaiToFloat(this.e_nilai.getText())+", 0, '-', '0', '"+this.e_periode.getText()+"', '-', '"+this.cb_ver.getText()+"', '"+this.app._userLog+"', getdate())");						
						...
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){								
								sql.add("insert into panjar_j(no_pj, no_dokumen, tanggal, no_urut, kode_akun, keterangan, dc, nilai, kode_pp, kode_drk, kode_lokasi, modul, jenis, periode, kode_curr, kurs, nik_user, tgl_input) values "+
										"('"+this.e_nopj.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"','-','"+this.app._lokasi+"','PJAJU','BEBAN','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");					
							}
						}
					}	
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nopj.getText()+"','PJAJU','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(1,i)+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(2,i))+","+nilai+")");
							}
						}
					}
					...
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i) && this.sgUpld.cells(2,i)!="-") {							
							...
							sql.add("insert into pbh_dok(no_bukti,no_gambar,nu,kode_jenis,kode_lokasi,modul,no_ref) values ('"+this.e_nopj.getText()+"','"+this.sgUpld.cells(3,i).tmpfile+"','"+ix+"','"+this.sgUpld.cells(0,i)+"','"+this.app._lokasi+"','PJAJU','"+this.e_nopj.getText()+"')");															
						}	
					}	
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					this.standarLib.clearByTag(this, new Array("0","1"),this.e_nopj);										
					setTipeButton(tbSimpan);
					this.doLoad3();
				break;
			case "simpan" :					
			case "ubah" :					
				...
				if (d1 > d2) {							
					var k = i+1;
					system.alert(this,"Tanggal tidak valid.","Tanggal Pertanggungan tidak boleh lebih awal dari tanggal Pengajuan");
					return false;
				}

				var data = this.dbLib.getDataProvider(
							"select count(a.no_pj) as jml from panjar_m a left join ptg_m b on a.no_pj=b.no_pj and a.kode_lokasi=b.kode_lokasi and b.progress= '2' "+
							"where b.no_pj is null and a.no_pj <> '"+this.e_nopj.getText()+"' and a.nik_pengaju ='"+this.cb_nik.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line = data.rs.rows[0];												
					if (parseFloat(line.jml) + 1 >  this.maxPJ) {
						system.alert(this,"Transaksi tidak valid.","Panjar Outstanding sudah maksimal.(Maksimal : "+this.maxPJ+")");
						return false;						
					}
				} 
	...
	doChange:function(sender){
		try {
			if (sender == this.cb_nik && this.cb_nik.getText()!="") {			
				var data = this.dbLib.getDataProvider("select kode_pp,bank+' - '+cabang as bank, no_rek,nama_rek from karyawan where nik='"+this.cb_nik.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){										
				...
				var data = this.dbLib.getDataProvider(
						"select a.no_pj,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as due_date,a.keterangan,a.nilai,DATEDIFF(DAY,a.tanggal,getdate()) as umur "+
						"from panjar_m a left join ptg_m b on a.no_pj=b.no_pj and a.kode_lokasi=b.kode_lokasi and b.progress= '2' "+
						"where b.no_pj is null and a.nik_pengaju ='"+this.cb_nik.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg4.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];												
						this.sg4.appendData([line.no_pj,line.tgl,line.due_date,line.keterangan,floatToNilai(line.nilai),floatToNilai(line.umur)]);
					}
	    			...
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (result.toLowerCase().search("error") == -1){
								...
								if (this.preView == "1") {								
									this.nama_report="server_report_saku3_pbh_ypt_rptPanjarAju";
									this.filter = " where a.kode_lokasi='"+this.app._lokasi+"' and a.no_pj='"+this.e_nopj.getText()+"' ";
									this.filter2 = "";
									this.viewer.prepare();
									...
									system.info(this,"Transaksi telah sukses tereksekusi (No Bukti : "+ this.e_nopj.getText()+")","");							
									this.clearLayar();
								} 
	...
	doLoad3:function(sender){																									
		var strSQL = "select a.no_pj,convert(varchar,a.tanggal,103) as tgl,a.nik_pengaju,a.keterangan,a.nilai,a.tanggal,  "+
					 ...
					 "where a.kode_pp='"+this.app._kodePP+"' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul='PJAJU' and a.no_kas ='-' "+
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_pj,line.tgl,line.nik_pengaju,line.keterangan,floatToNilai(line.nilai),line.status.toUpperCase(),"Pilih"]); 
		}
	...
	doDoubleClick3: function(sender, col , row) {		
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				this.stsSimpan = 0;								
				this.e_nopj.setText(this.sg3.cells(0,row));	
				this.doLoadCtt(this.e_nopj.getText());						
								
				var strSQL = "select * from panjar_m where no_pj = '"+this.e_nopj.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";										
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){							
						...
						this.cb_ver.setText(line.no_link);						
						this.cb_akunpj.setText(line.akun_pj);												
					}
				}	
				
				var strSQL = "select * from pbh_rek a where a.no_bukti = '"+this.e_nopj.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";										
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){																			
				...
				var data = this.dbLib.getDataProvider(
							 ...
							 "where a.no_bukti = '"+this.e_nopj.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.nu",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;					
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
