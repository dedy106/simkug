window.app_saku3_transaksi_tarbak_kug_fReimIFDRK = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_tarbak_kug_fReimIFDRK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_tarbak_kug_fReimIFDRK";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Reimburse Imprest Fund DRK", 0);	
		...
		this.sg3 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:5,tag:9,
		            colTitle:["No Reimburse","Tanggal","Deskripsi","Nilai","Pilih"],
					colWidth:[[4,3,2,1,0],[70,100,350,80,100]],
					colFormat:[[3,4],[cfNilai,cfButton]],			
					readOnly:true, click:[this,"doSgBtnClick3"], colAlign:[[4],[alCenter]],
		...
		this.e_debet = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,14,200,20],caption:"Debet", tag:1, tipeText:ttNilai, text:"0",readOnly:true});		
		this.cb_pp = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,13,220,20],caption:"Bagian / Unit",tag:2,readOnly:true}); 										
		this.e_kredit = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,13,200,20],caption:"Kredit", tag:1, tipeText:ttNilai, text:"0",readOnly:true});				
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,996,259], childPage:["Item Reimburse","Budget","File Dok","Otorisasi & Bank","Cattn Verf."]});		
		...
		try {
			...
			/*
			memakai akun if-nya
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('HUTIF') and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku3_transaksi_tarbak_kug_fReimIFDRK.extend(window.childForm);
window.app_saku3_transaksi_tarbak_kug_fReimIFDRK.implement({	
	isiCBnik: function() {
	...
	doHitungGar: function(){
		try {
			...
			for (var i=0;i < this.sg2.getRowCount();i++){
				var data = this.dbLib.getDataProvider("select fn_saldotwdrk('"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(1,i)+"','"+this.sg2.cells(2,i)+"','"+this.e_periode.getText()+"','"+this.e_nb.getText()+"') as gar ",true);
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					//kas_m_j hanya untuk report krn masuk langsung ke akun kas kecil tidak ke hutangif
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.noIFCair+"','-','"+this.akunHutIF+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','IFREIM','REIM','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_total.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','Z','-','-','-','-')");					
						...
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){								
								...
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
										"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"','"+this.sg1.cells(7,i)+"','-','-','"+this.app._lokasi+"','IFREIM','BEBAN','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+parseNilai(this.sg1.cells(4,i))+")");
							}
						}
					}
					...
					if (this.c_status.getText() == "REIMBURSE") {
						...
						sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+		
								"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',999,'"+this.akunHutIF+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_total.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','IFREIM','HUT','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+parseNilai(this.e_total.getText())+")");
					}
					else {
						...
						sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+		
								 "('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',888,'"+this.akunIF+"','"+this.e_ket.getText()+"','C',"+this.nilaiIF+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','IFREIM','R-IF','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+this.nilaiIF+")");
					}
		...
		if (sender == this.cb_nik && this.cb_nik.getText()!="" && this.stsSimpan==1) {
			...
			if (typeof data == "object"){
				...
				if (line != undefined){							
					...
					//akun hutang if memakai akun if-nya
					this.akunHutIF = line.akun_if;
					
					// alert(this.akunHutIF);
	...
	doLoad3:function(sender){																				
		...
		var strSQL = "select a.no_hutang,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
					 "from hutang_m a left join pbh_pb_m b on a.no_hutang=b.no_pb and a.kode_lokasi=b.kode_lokasi and b.progress not in ('0','R','K','V') "+					 					 
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_hutang,line.tgl,line.keterangan,floatToNilai(line.nilai),"Pilih"]); 
		}
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.keterangan,a.tanggal,isnull(b.nik_tahu,a.nik_app) as nik_tahu,isnull(b.nik_app,a.nik_app) as nik_app, c.no_kas,c.nik,a.kode_project "+
							 "from hutang_m a inner join if_nik c on a.no_dokumen = c.no_kas and a.kode_lokasi=c.kode_lokasi "+							 
							 "				  left join pbh_pb_m b on b.no_pb=a.no_hutang and a.kode_lokasi=b.kode_lokasi and b.modul='IFCLOSE' "+
