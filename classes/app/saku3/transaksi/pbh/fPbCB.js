window.app_saku3_transaksi_pbh_fPbCB = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_pbh_fPbCB.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_pbh_fPbCB";
        ...
        this.e_nama = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,17,400,20],caption:"Nama Atensi", maxLength:150, tag:2});	
		...
		this.sg1 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:7,tag:0,
		...
		this.sg2 = new saiGrid(this.pc1.childPage[5],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:5,tag:9,
		...
		try {
			...
			this.cb_ver.setSQL("select nik, nama from karyawan where kode_pp ='"+this.app._kodePP+"' and flag_aktif='1' and kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Data Karyawan",true);
			...
			var strSQL = "select * from pp a where a.kode_pp = '"+this.app._kodePP+"' and a.kode_lokasi='"+this.app._lokasi+"'";										
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){													
...
window.app_saku3_transaksi_pbh_fPbCB.extend(window.childForm);
window.app_saku3_transaksi_pbh_fPbCB.implement({		
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','PBBAU','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(1,i)+"','-','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(2,i))+","+nilai+")");
							}
						}
					}
	...
	doLoad3:function(sender){																				
		...
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai "+
					 ...
					 "and a.modul = 'PBBAU' and a.progress in ('0','V') and a.kode_pp in (select kode_pp from karyawan_pp where nik='"+this.app._userLog+"' and kode_lokasi='"+this.app._lokasi+"') "+
					 "order by a.no_pb";						 	 
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
