window.app_saku3_transaksi_pbh_ypt_fPbBMHD = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_pbh_ypt_fPbBMHD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_pbh_ypt_fPbBMHD";
        ...
        this.e_totalDC = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,13,200,20],caption:"Total PB", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});					
		this.e_ket = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,17,450,20],caption:"Deskripsi", maxLength:150});				
		this.e_totalRek = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,17,200,20],caption:"TotNet Rekening", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});					
		
        this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,996,338], childPage:["Data BMHD","Atensi Pembayaran","Jurnal++","Otorisasi","Cattn Verf.","File Dok","Budget"]});            
		...
		this.sgRek = new saiGrid(this.pc1.childPage[1],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:7,tag:0,
					colTitle:["Atensi","Bank-Cabang","Nama Rekening","No Rekening","Bruto","Potongan","Netto"],
					colWidth:[[6,5,4,3,2,1,0],[100,80,100,120,200,150,200]],
					columnReadOnly:[true,[6],[0,1,2,3,4,5]],				
					colFormat:[[4,5,6],[cfNilai,cfNilai,cfNilai]],																	
					nilaiChange:[this,"doNilaiChange"],change:[this,"doChangeCellR"],
...
window.app_saku3_transaksi_pbh_ypt_fPbBMHD.extend(window.childForm);
window.app_saku3_transaksi_pbh_ypt_fPbBMHD.implement({	
	isiBMHD: function() {
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into pbh_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.akunBMHD+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBBMHD','BMHD','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");										
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','PBBMHD','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(1,i)+"','"+this.sg2.cells(2,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(3,i))+","+nilai+")");
							}
						}
					}
						...
						for (var i=0;i < this.sgRek.getRowCount();i++){
							if (this.sgRek.rowValid(i)){								
								sql.add("insert into pbh_rek(no_bukti,kode_lokasi,modul,nama_rek,no_rek,bank,nama,bruto,pajak,nilai) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','PBBMHD','"+this.sgRek.cells(2,i)+"','"+this.sgRek.cells(3,i)+"','"+this.sgRek.cells(1,i)+"','"+this.sgRek.cells(0,i)+"',"+nilaiToFloat(this.sgRek.cells(4,i))+","+nilaiToFloat(this.sgRek.cells(5,i))+","+nilaiToFloat(this.sgRek.cells(6,i))+")");
							}
						}
					}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :															
				...
				if (nilaiToFloat(this.e_totalDC.getText()) != nilaiToFloat(this.e_totalRek.getText())) {
					system.alert(this,"Transaksi tidak valid.","Total Jurnal dan TotalNet Rekening tidak sama.");
					return false;
				}
	...
	doChangeCellR: function(sender, col, row){
		if ((col == 4 || col == 5) && sender.cells(4,row) != "" && sender.cells(5,row) != "") {
			var subtot = nilaiToFloat(this.sgRek.cells(4,row)) - nilaiToFloat(this.sgRek.cells(5,row));
			this.sgRek.cells(6,row,floatToNilai(subtot));
			this.sgRek.validasi();
		}		
	},
	...
	doNilaiChange: function(){		
		try{
			...
			var totRek = 0;
			for (var i = 0; i < this.sgRek.getRowCount();i++){
				if (this.sgRek.rowValid(i) && this.sgRek.cells(6,i) != ""){
					totRek += nilaiToFloat(this.sgRek.cells(6,i));					
				}
			}
	...
	doLoad3:function(sender){																				
		...
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai, "+
					 ...
					 "and a.modul = 'PBBMHD' and a.progress in ('0','R','K','V') and a.kode_pp in (select kode_pp from karyawan_pp where nik='"+this.app._userLog+"' and kode_lokasi='"+this.app._lokasi+"') "+
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select b.nilai,a.keterangan,a.no_dokumen,a.modul,a.due_date,a.tanggal,a.nik_tahu,a.nik_app,a.nik_ver,a.no_hutang "+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];												
						this.sgRek.appendData([line.nama,line.bank,line.nama_rek,line.no_rek,floatToNilai(line.bruto),floatToNilai(line.pajak),floatToNilai(line.nilai)]);
					}
