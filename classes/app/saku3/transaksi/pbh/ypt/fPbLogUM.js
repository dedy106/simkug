window.app_saku3_transaksi_pbh_ypt_fPbLogUM = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_pbh_ypt_fPbLogUM.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_pbh_ypt_fPbLogUM";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Permintaan Bayar Uang Muka", 0);	
        ...
        this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,996,338], childPage:["Data SPK","Atensi Pembayaran","","Otorisasi","Cattn Verf.","File Dok",""]}); //jurnal++ dan budget di hide             
		...
		this.c_jenis = new saiCB(this.pc1.childPage[0],{bound:[20,15,200,20],caption:"Jenis",items:["DP","TERMIN"], readOnly:true,tag:2,change:[this,"doChange"]});		
		this.c_persen = new saiCB(this.pc1.childPage[0],{bound:[20,16,200,20],caption:"Persentase",tag:2,mustCheck:false,  change:[this,"doChange"]});		
		this.e_bruto = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,13,200,20],caption:"Ni. DP/Termin", tag:4, tipeText:ttNilai, text:"0",change:[this,"doChange"]});				
		this.e_ppn = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,14,200,20],caption:"Nilai PPN", tag:4, tipeText:ttNilai, text:"0", readOnly:true, change:[this,"doChange"]});				
		
		this.e_vendor = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,16,200,20],caption:"Kode Mitra", readOnly:true,tag:4});				
		this.e_namavendor = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,17,450,20],caption:"Nama Mitra", readOnly:true,tag:4});				
		this.e_bank = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,18,450,20],caption:"Bank",  maxLength:100,tag:4});				
		this.e_norek = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,19,450,20],caption:"No Rekening",  maxLength:50,tag:4});				
		this.e_namarek = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,21,450,20],caption:"Nama Rekening",  maxLength:100,tag:4});								
		...
		this.sg2 = new saiGrid(this.pc1.childPage[6],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:6,tag:9,
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('NIKVER','LOGADK','PPNM') and kode_lokasi = '"+this.app._lokasi+"'",true);			
			if (typeof data == "object"){
				var line;
			...
			sql.add("select distinct a.kode_akun,a.nama from masakun a "+
					...
					"	inner join karyawan_pp d on c.kode_pp=d.kode_pp and c.kode_lokasi=d.kode_lokasi and d.nik='"+this.app._userLog+"' " +
...
window.app_saku3_transaksi_pbh_ypt_fPbLogUM.extend(window.childForm);
window.app_saku3_transaksi_pbh_ypt_fPbLogUM.implement({	
	isiSPK: function() {
		this.cb_spk.setSQL("select a.no_spk, a.keterangan "+
						   "from log_spk_m a "+
						   "     inner join log_pesan_m d on a.no_pesan=d.no_pesan and a.kode_lokasi=d.kode_lokasi and d.jenis in ('CAPEX','OPEX') "+
						   "	 inner join log_wenang x on d.kode_pp=x.pp_asal and d.kode_lokasi=x.kode_lokasi and d.nilai between x.awal and x.akhir and x.pp_proses='"+this.app._kodePP+"' "+ 					 			 
						   "     left join ("+
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into hutang_m(no_hutang,kode_lokasi,no_dokumen,tanggal,keterangan,kode_project,kode_vendor,kode_curr,kurs,nik_app,kode_pp,nilai,periode,nik_user,tgl_input,akun_hutang,posted,nilai_ppn,modul,no_ref) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','-','"+this.e_vendor.getText()+"','IDR',1,'"+this.app._userLog+"','"+this.app._kodePP+"',"+parseNilai(this.e_bruto.getText())+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.akunHutang+"','F',0,'LOGADK','"+this.cb_spk.getText()+"')");

					var total = nilaiToFloat(this.e_bruto.getText()) + nilaiToFloat(this.e_ppn.getText());
					...
					sql.add("insert into hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_spk.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.akunHutang+"','"+this.e_ket.getText()+"','C','IDR',1,"+total+","+total+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','LOGADK','HUT','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");					
					...
					sql.add("insert into log_pbadk_d(no_pb,no_spk,kode_lokasi,periode,dc,nilai,no_bast,akun_adk,jenis,persen) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_spk.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','D',"+nilaiToFloat(this.e_bruto.getText())+",'-','"+this.akunADK+"','"+this.c_jenis.getText()+"',"+nilaiToFloat(this.c_persen.getText())+")");
					...
					sql.add("insert into pbh_pb_m (no_pb,no_dokumen,kode_lokasi,periode,nik_user,tgl_input,tanggal,due_date,keterangan,nilai,modul,progress,kode_pp,nik_app,nik_tahu,no_hutang,no_app,no_spb,no_ver,kode_bidang,kode_loktuj,nilai_final,posted,kode_proyek,no_app2,no_app3,no_fiat,no_kas,akun_hutang,nik_ver) values  "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.dp_d1.getDateString()+"','"+this.dp_d2.getDateString()+"','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_totalDC.getText())+",'PBADK','0','"+this.app._kodePP+"','"+this.cb_app.getText()+"','"+this.cb_tahu.getText()+"','"+this.cb_spk.getText()+"','-','-','-','"+this.app._kodePP+"','"+this.app._lokasi+"',"+nilaiToFloat(this.e_bruto.getText())+",'X','-','-','-','-','-','-','"+this.cb_ver.getText()+"')");
					sql.add("insert into pbh_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.akunHutang+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_totalDC.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBADK','HUTANG','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");															

					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){																		
								sql.add("insert into pbh_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"','"+this.sg1.cells(7,i)+"','"+this.app._lokasi+"','PBADK','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");										
							}
						}
					}

					sql.add("insert into pbh_rek(no_bukti,kode_lokasi,modul,nama_rek,no_rek,bank,nama,bruto,pajak,nilai) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','PBADK','"+this.e_namarek.getText()+"','"+this.e_norek.getText()+"','"+this.e_bank.getText()+"','"+this.e_namavendor.getText()+"',"+nilaiToFloat(this.e_totalDC.getText())+",0,"+nilaiToFloat(this.e_totalDC.getText())+")");
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','PBADK','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(1,i)+"','"+this.sg2.cells(2,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(3,i))+","+nilai+")");
							}
						}
					}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :															
				...
				if ((nilaiToFloat(this.c_persen.getText()) < 0) && (nilaiToFloat(this.c_persen.getText()) > 100)) {
					system.alert(this,"Transaksi tidak valid.","Persentase diantara 0-100.");
					return false;
				}

				if (nilaiToFloat(this.e_bruto.getText()) > nilaiToFloat(this.e_saldo.getText())) {
					system.alert(this,"Transaksi tidak valid.","Nilai DP/Termin melebihi saldo.");
					return false;
				}
				if (nilaiToFloat(this.e_bruto.getText()) <= 0) {
					system.alert(this,"Transaksi tidak valid.","Nilai DP/Termin harus lebih besar nol.");
					return false;
				}
	...
	doChange: function(sender){
		...
		if (sender == this.cb_spk && this.cb_spk.getText() != "") {
			var strSQL = "select b.akun_hutang,b.kode_vendor,b.nama,b.bank,b.cabang,b.no_rek,b.nama_rek,a.nilai - isnull(c.adk,0) as saldo,a.nilai "+
						 "from log_spk_m a "+
		...
		if (sender == this.c_jenis && this.c_jenis.getText()!="") {
			this.c_persen.setText("");
			this.c_persen.items.clear();
			var data = this.dbLib.getDataProvider("select persen from log_termin where jenis='"+this.c_jenis.getText()+"' and kode_lokasi='"+this.app._lokasi+"' order by persen",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];													
					this.c_persen.addItem(i,line.persen);
				}
			} 
		}
		if (sender == this.c_persen && this.c_persen.getText()!="" && this.stsSimpan==1) {
			var nilai = nilaiToFloat(this.e_nspk.getText()) * (nilaiToFloat(this.c_persen.getText()) / 100); 
			this.e_bruto.setText(floatToNilai(Math.round(nilai)));			
			//var ppn = nilaiToFloat(this.e_bruto.getText()) * 0.1; 
			// this.e_ppn.setText(floatToNilai(Math.round(ppn)));
			this.e_ppn.setText("0");
			this.sg1.validasi();
		}
		if (sender == this.e_bruto && this.e_bruto.getText()!="") {
			//var ppn = nilaiToFloat(this.e_bruto.getText()) * 0.1; 
			// this.e_ppn.setText(floatToNilai(Math.round(ppn)));
			this.e_ppn.setText("0");
			this.sg1.validasi();
	...
	doNilaiChange: function(){		
		try{
			...
			totDC = totDC + nilaiToFloat(this.e_bruto.getText()) + nilaiToFloat(this.e_ppn.getText());			
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							...
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_ypt_logistik_rptPbLog";
	...
	doLoad3:function(sender){																				
		...
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai, "+
					 ...
					 "and a.modul = 'PBADK' and a.progress in ('0','R','K','V') and a.kode_pp in (select kode_pp from karyawan_pp where nik='"+this.app._userLog+"' and kode_lokasi='"+this.app._lokasi+"') "+
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select b.jenis,b.persen,b.nilai,a.keterangan,a.no_dokumen,a.modul,a.due_date,a.tanggal,a.nik_tahu,a.nik_app,a.nik_ver,a.no_hutang "+
							 "from pbh_pb_m a "+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){													
						...
						this.cb_spk.setSQL("select no_spk,keterangan from log_spk_m where no_spk='"+line.no_hutang+"' and kode_lokasi='"+this.app._lokasi+"'",["no_spk","keterangan"],false,["No SPK","Deskripsi"],"and","Data SPK",true);		
						this.cb_spk.setText(line.no_hutang);
						this.c_jenis.setText(line.jenis);
						this.e_bruto.setText(floatToNilai(line.nilai));						
						this.c_persen.setText(floatToNilai(line.persen));
				...
				var data = this.dbLib.getDataProvider("select * from pbh_rek where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and modul<>'PINBUK-C'",true);											
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){													
						this.e_bank.setText(line.bank);
