window.app_saku3_transaksi_pbh_ypt_fPbpinbuk = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_pbh_ypt_fPbpinbuk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_pbh_ypt_fPbpinbuk";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Permintaan Bayar Pindah Buku", 0);	
        ...
        this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,996,338], childPage:["Sumber Dana","Rekening Tujuan","Otorisasi","Cattn Verf.","File Dok"]});    		
		...
		try {
			...
			this.cb_akun.setSQL("select a.kode_akun, a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag='009' where a.block='0' and a.kode_lokasi='"+this.app._lokasi+"'",["a.kode_akun","a.nama"],false,["Kode","Nama"],"and","Data Akun",true);			
			...
			sql.add("select distinct a.kode_akun,a.nama from masakun a "+
					"inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag = '009' "+	
...
window.app_saku3_transaksi_pbh_ypt_fPbpinbuk.extend(window.childForm);
window.app_saku3_transaksi_pbh_ypt_fPbpinbuk.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into pbh_pb_m (no_pb,no_dokumen,kode_lokasi,periode,nik_user,tgl_input,tanggal,due_date,keterangan,nilai,modul,progress,kode_pp,nik_app,nik_tahu,no_hutang,no_app,no_spb,no_ver,kode_bidang,kode_loktuj,nilai_final,posted,kode_proyek,no_app2,no_app3,no_fiat,no_kas,akun_hutang,nik_ver) values  "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.dp_d1.getDateString()+"','"+this.dp_d2.getDateString()+"','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_total.getText())+",'PINBUK','1','"+this.app._kodePP+"','"+this.cb_app.getText()+"','"+this.cb_tahu.getText()+"','-','-','-','-','"+this.app._kodePP+"','"+this.app._lokasi+"',"+nilaiToFloat(this.e_total.getText())+",'X','-','-','-','-','-','"+this.cb_akun.getText()+"','"+this.cb_ver.getText()+"')");

					//rek sumber
					sql.add("insert into pbh_rek(no_bukti,kode_lokasi,modul,nama_rek,no_rek,bank,nama,bruto,pajak,nilai,nu) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','PINBUK-C','"+this.e_nmrek.getText()+"','"+this.e_norek.getText()+"','"+this.e_bank.getText()+"','"+this.e_nama.getText()+"',"+nilaiToFloat(this.e_total.getText())+",0,"+nilaiToFloat(this.e_total.getText())+",99999)");		
					...
					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){			
								...
								sql.add("insert into pbh_rek(no_bukti,kode_lokasi,modul,nama_rek,no_rek,bank,nama,bruto,pajak,nilai,nu) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','PINBUK-D','"+this.sg1.cells(4,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i)+"','"+this.e_nama.getText()+"',"+nilaiToFloat(this.sg1.cells(5,i))+",0,"+nilaiToFloat(this.sg1.cells(5,i))+","+i+")");
							}
						}
					}		
	...
	doLoad3:function(sender){																				
		...
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai, "+
					 ...
					 "and a.modul = 'PINBUK' and a.progress in ('1') and a.kode_pp in (select kode_pp from karyawan_pp where nik='"+this.app._userLog+"' and kode_lokasi='"+this.app._lokasi+"') "+
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.keterangan,a.no_dokumen,a.modul,a.due_date,a.tanggal,a.nik_tahu,a.nik_app,a.nik_ver,a.akun_hutang "+
							 "from pbh_pb_m a "+								 
				...
				var data = this.dbLib.getDataProvider(
							"select a.kode_akun,b.nama as nama_akun,c.bank,c.no_rek,c.nama_rek,a.nilai "+
