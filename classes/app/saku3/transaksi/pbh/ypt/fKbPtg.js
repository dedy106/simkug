window.app_saku3_transaksi_pbh_ypt_fKbPtg = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_pbh_ypt_fKbPtg.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_pbh_ypt_fKbPtg";
...
window.app_saku3_transaksi_pbh_ypt_fKbPtg.extend(window.childForm);
window.app_saku3_transaksi_pbh_ypt_fKbPtg.implement({	
	isiCBReim: function() {
		...
		try {										
			...
			if (sender == this.cb_ptg && this.cb_ptg.getText()!="") {
				...
				var data = this.dbLib.getDataProvider(
							...
							"where a.jenis in ('BEBAN','PAJAK') and a.no_ptg = '"+this.cb_ptg.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.no_urut",true);
