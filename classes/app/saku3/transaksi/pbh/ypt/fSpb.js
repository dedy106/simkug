window.app_saku3_transaksi_pbh_ypt_fSpb = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_pbh_ypt_fSpb.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_pbh_ypt_fSpb";
		...
		this.e_dok = new saiLabelEdit(this.pc2.childPage[0],{bound:[10,16,450,20],caption:"No Dokumen SPB", maxLength:150});				
		this.e_ket = new saiLabelEdit(this.pc2.childPage[0],{bound:[10,17,450,20],caption:"Deskripsi", maxLength:150});				
		this.e_nilaispb = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,17,200,20],caption:"Total SPB", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});					
		
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,996,338], childPage:["Daftar PB","Daftar Transfer","Otorisasi","Tambah PB"]});		
		...
		this.sg4 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:8,tag:9,
				colTitle:["Bank","Cabang","No Rekening","Nama Rekening","Bruto","Potongan","Netto","Jenis"],
				colWidth:[[7,6,5,4,3,2,1,0],[80,100,80,100,200,100,200,100]],
		...
		this.cb_bdh = new saiCBBL(this.pc1.childPage[2],{bound:[20,15,220,20],caption:"NIK Bendahara", multiSelection:false, maxLength:10, tag:2});						
		this.cb_fiat = new saiCBBL(this.pc1.childPage[2],{bound:[20,16,220,20],caption:"NIK Fiatur", multiSelection:false, maxLength:10, tag:2});						
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('NIKBDH','NIKFIA') and kode_lokasi = '"+this.app._lokasi+"'",true);			
			if (typeof data == "object"){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];																	
					if (line.kode_spro == "NIKFIA") this.cb_fiat.setText(line.flag);							
...
window.app_saku3_transaksi_pbh_ypt_fSpb.extend(window.childForm);
window.app_saku3_transaksi_pbh_ypt_fSpb.implement({	
	doAdd: function() {	
		try {
			var temu = false;
			...
			if (!temu) {			
				var strSQL = "select 'SPB' as status,no_pb,convert(varchar,tanggal,103) as tgl,keterangan,nilai "+
							"from pbh_pb_m "+
							"where no_pb = '"+this.cb_pbAdd.getText()+"' and progress='1' and no_spb='-' and kode_lokasi='"+this.app._lokasi+"' and modul not in ('IFCLOSE','PJPTG')";					
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into spb_m (no_spb,no_dokumen,no_ver,no_bukti,kode_lokasi,periode,nik_user,tgl_input,tanggal,due_date,keterangan,nik_buat,nik_sah,nik_fiat,nik_bdh,lok_bayar,no_kas,nilai,modul,no_fiat,progress,kode_ppasal) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','-','-','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.dp_d1.getDateString()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._userLog+"','-','"+this.cb_fiat.getText()+"','"+this.cb_bdh.getText()+"','"+this.app._lokasi+"','-',"+nilaiToFloat(this.e_nilaispb.getText())+",'SPB','-','0','-')");
					...
					for (var i = 0; i < this.sg5.rows.getLength();i++){
						if (this.sg5.rowValid(i) && this.sg5.cells(0,i) == "SPB"){							
							...
							sql.add("insert into spb_j (no_spb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs)  "+
									...
									"from pbh_pb_j where no_pb='"+this.sg5.cells(1,i)+"' and jenis in ('BEBAN','HUTIF','PJ','TAK','TUJUAN','PAJAK') and kode_lokasi='"+this.app._lokasi+"'");							
						}
					}
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
                ...
                var strSQL = "select 'SPB' as status,b.no_pb,convert(varchar,b.tanggal,103) as tgl,b.keterangan,b.nilai "+
							 "from pbh_pb_m b "+
							 "where b.progress='2' and b.no_spb='"+this.e_nb.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"' and modul not in ('IFCLOSE','PJPTG')";					 
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg5.clear();
