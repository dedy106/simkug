window.app_saku3_transaksi_pbh_ypt_fKbTerima = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_pbh_ypt_fKbTerima.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_pbh_ypt_fKbTerima";
		...
		this.cb_app = new saiCBBL(this.pc2.childPage[0],{bound:[20,17,220,20],caption:"NIK Mengetahui", multiSelection:false, maxLength:10, tag:2});
		this.bTampil = new button(this.pc2.childPage[0],{bound:[658,17,80,18],caption:"Tampil Data",click:[this,"doTampilClick"]});			
		this.e_nilai = new saiLabelEdit(this.pc2.childPage[0],{bound:[780,17,202,20],caption:"Total Penerimaan", readOnly:true,tipeText:ttNilai, text:"0"});		
		
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,257], childPage:["Data Pengiriman"]});
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:0,
		            colTitle:["Status","No Bukti","No Dokumen","Lokasi Kirim","Akun TAK","Keterangan","Nilai","Tgl Kirim","ID"],
...
window.app_saku3_transaksi_pbh_ypt_fKbTerima.extend(window.childForm);
window.app_saku3_transaksi_pbh_ypt_fKbTerima.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',1,'"+this.cb_kas.getText()+"','"+this.e_ket.getText()+"','D',"+parseNilai(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBDROPTRM','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");							
	...
	doTampilClick:function(sender){		
		if (this.e_periode.getText() != "") {
			var data = this.dbLib.getDataProvider(
			              "select a.no_kas,a.no_dokumen,a.kode_lokasi,a.akun_tak,a.keterangan,a.nilai,convert(varchar,b.tanggal,103) as tanggal,a.nu "+
			              "from yk_kasdrop_d a inner join kas_m b on a.no_kas=b.no_kas and a.kode_lokasi=b.kode_lokasi where a.kode_loktuj='"+this.app._lokasi+"' and a.progress = '0' and a.periode<='"+this.e_periode.getText()+"'",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData(["INPROG",line.no_kas,line.no_dokumen,line.kode_lokasi,line.akun_tak,line.keterangan,floatToNilai(line.nilai),line.tanggal,line.nu]);
				}
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.jenis,a.no_dokumen,a.keterangan,a.nilai "+
		             "from kas_m a "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'KBDROPTRM' and a.posted ='F'";						
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider(
							  "select a.no_kas,a.no_dokumen,a.kode_lokasi,a.akun_tak,a.keterangan,a.nilai,convert(varchar,b.tanggal,103) as tanggal,a.nu "+
							  "from yk_kasdrop_d a inner join kas_m b on a.no_kas=b.no_kas and a.kode_lokasi=b.kode_lokasi where a.no_kasterima='"+this.e_nb.getText()+"' and a.kode_loktuj='"+this.app._lokasi+"'",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg.appendData(["APP",line.no_kas,line.no_dokumen,line.kode_lokasi,line.akun_tak,line.keterangan,floatToNilai(line.nilai),line.tanggal,line.nu]);
					}
				} else this.sg.clear(1);									
			}									
		} catch(e) {alert(e);}
