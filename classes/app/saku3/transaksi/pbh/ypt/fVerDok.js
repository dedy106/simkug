window.app_saku3_transaksi_pbh_ypt_fVerDok = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_pbh_ypt_fVerDok.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_pbh_ypt_fVerDok";
		...
		this.sgUpld = new saiGrid(this.pc2.childPage[3],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:4, tag:9,
		...
		this.sg4 = new saiGrid(this.pc2.childPage[5],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:4,tag:9, 
...
window.app_saku3_transaksi_pbh_ypt_fVerDok.extend(window.childForm);
window.app_saku3_transaksi_pbh_ypt_fVerDok.implement({	
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "hapus" :	
				...
				sql.add("update pbh_pb_m set no_verdok='-',progress='S' where no_pb='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");				
				sql.add("update panjar_m set progress='S' where no_pj='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");				
				this.sg1.validasi();
				setTipeButton(tbAllFalse);					
				this.dbLib.execArraySQL(sql);				
				break;					
	...
	doChange:function(sender){	
		...
		if (sender == this.cb_nb && this.cb_nb.getText() != "") {
			var strSQL = "select a.due_date,a.no_pb as no_bukti,case a.progress when 'D' then 'APPROVE' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.tanggal,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_verdok as no_ver1,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
						 ...
						 "where a.progress in ('D','R') and a.no_pb='"+this.cb_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul in ('PBBAU', 'PBBMHD', 'PBBA', 'PBADK','IFREIM','IFCLOSE','PJAJU','PJPTG')  ";
	...
	doLoad: function(sender){			
		strSQL = "select a.due_date,a.no_pb as no_bukti,'INPROG' as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,a.nik_user as pembuat,a.no_ver as no_ver1,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
				 "from pbh_pb_m a "+
				 "		inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+				 
				 "where a.progress='S' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul in ('PBBAU','PBBMHD','PBBA','PBADK','IFREIM','IFCLOSE','PJAJU','PJPTG') order by no_pb";                                
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
			this.sgn.setTotalPage(Math.ceil(data.rs.rows.length/20));
