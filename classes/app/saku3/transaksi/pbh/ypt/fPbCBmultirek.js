window.app_saku3_transaksi_pbh_ypt_fPbCBmultirek = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_pbh_ypt_fPbCBmultirek.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_pbh_ypt_fPbCBmultirek";
		...
		this.sgRek = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:7,tag:0,
		...
		this.sg2 = new saiGrid(this.pc1.childPage[5],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:6,tag:9,
...
window.app_saku3_transaksi_pbh_ypt_fPbCBmultirek.extend(window.childForm);
window.app_saku3_transaksi_pbh_ypt_fPbCBmultirek.implement({	
						...
						for (var i=0;i < this.sgRek.getRowCount();i++){
							if (this.sgRek.rowValid(i)){								
								sql.add("insert into pbh_rek(no_bukti,kode_lokasi,modul,nama_rek,no_rek,bank,nama,bruto,pajak,nilai) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','PBBAU','"+this.sgRek.cells(2,i)+"','"+this.sgRek.cells(3,i)+"','"+this.sgRek.cells(1,i)+"','"+this.sgRek.cells(0,i)+"',"+nilaiToFloat(this.sgRek.cells(4,i))+","+nilaiToFloat(this.sgRek.cells(5,i))+","+nilaiToFloat(this.sgRek.cells(6,i))+")");
							}
						}
					}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :															
				this.preView = "1";								
				this.sg1.validasi();	
				if (nilaiToFloat(this.e_totalDC.getText()) != (nilaiToFloat(this.e_totalRek.getText()) + this.totPotRek) ) {
					system.alert(this,"Transaksi tidak valid.","Total Jurnal dan Total Bruto Rekening tidak sesuai.");
					return false;
				}
	...
	doNilaiChange: function(){		
		try{
			...
			for (var i = 0; i < this.sgRek.getRowCount();i++){
				...
				if (this.sgRek.rowValid(i) && this.sgRek.cells(5,i) != ""){
					this.totPotRek += nilaiToFloat(this.sgRek.cells(5,i));					
				}				
			}
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider("select * from pbh_rek where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);							
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sgRek.clear();
					for (var i in data.rs.rows){
