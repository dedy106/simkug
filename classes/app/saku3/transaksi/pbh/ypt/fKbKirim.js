window.app_saku3_transaksi_pbh_ypt_fKbKirim = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_pbh_ypt_fKbKirim.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_pbh_ypt_fKbKirim";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Droping Kirim Non Pengajuan", 0);	
		...
		this.e_nilai = new saiLabelEdit(this.pc2.childPage[0],{bound:[780,17,200,20],caption:"Total Pengiriman", readOnly:true,tipeText:ttNilai, text:"0"});		
				
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,255], childPage:["Data Atensi"]});
		...
		try {
			...
			this.cb_kas.setSQL("select a.kode_akun, a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
			                   "where b.kode_flag in ('009') and a.kode_lokasi='"+this.app._lokasi+"'",["a.kode_akun","a.nama"],false,["Kode","Nama"],"and","Daftar Akun KasBank",true);
			...
			sql.add("select kode_lokasi, nama from lokasi where kode_lokasi not in ('"+this.app._kodeLokasiKonsol+"','"+this.app._lokasi+"')");
...
window.app_saku3_transaksi_pbh_ypt_fKbKirim.extend(window.childForm);
window.app_saku3_transaksi_pbh_ypt_fKbKirim.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.cb_kas.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBDROP','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.cb_app.getText()+"',getdate(),'"+this.app._userLog+"','F','-','-','"+this.cb_tak.getText()+"','-')");					
					...
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',1,'"+this.cb_kas.getText()+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBDROP','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");							
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								sql.add("insert into yk_kasdrop_d(no_spb,nu,no_kas,no_dokumen,kode_lokasi,periode,kode_loktuj,kode_rek,keterangan,nilai,progress,akun_tak,no_kasterima) values "+
										"('"+this.e_nb.getText()+"',"+i+",'"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.sg.cells(0,i)+"','-','"+this.sg.cells(2,i)+"','"+parseNilai(this.sg.cells(3,i))+"','0','"+this.cb_tak.getText()+"','-')");
							}
						}
					}
					setTipeButton(tbAllFalse);					
			...
			if (sender == this.sg) {
				if (col == 0){
					this.standarLib.showListData(this, "Daftar Lokasi Tujuan",sender,undefined, 
												  "select kode_lokasi, nama from lokasi where kode_lokasi not in ('"+this.app._kodeLokasiKonsol+"','"+this.app._lokasi+"')",
												  "select count(*) from lokasi where kode_lokasi not in ('"+this.app._kodeLokasiKonsol+"','"+this.app._lokasi+"')",
	...
	doLoad3:function(sender){																		
		var strSQL = "select distinct a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.jenis,a.no_dokumen,a.keterangan,a.nilai "+
					 ...
					 "where b.no_kasterima is null and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'KBDROP' and a.posted ='F'";				
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider("select a.kode_loktuj,b.nama,a.keterangan,a.nilai "+
							"from yk_kasdrop_d a inner join lokasi b on a.kode_loktuj=b.kode_lokasi "+
							"where a.no_kas = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.nu",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];												
						this.sg.appendData([line.kode_loktuj,line.nama,line.keterangan,floatToNilai(line.nilai)]);
					}
