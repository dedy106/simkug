window.app_saku3_transaksi_pbh_ypt_fVerPajak = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_pbh_ypt_fVerPajak.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_pbh_ypt_fVerPajak";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Verifikasi Pajak", 0);	
		...
		this.pc2 = new pageControl(this.pc1.childPage[1],{bound:[1,18,996,252], childPage:["Rekening","Item Jurnal","Cattn Verf.","File Dok","Item Pajak"]});			
		...
		this.sg4 = new saiGrid(this.pc2.childPage[4],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:9,tag:9,
					colTitle:["Kode Akun","Nama Akun","DC","Keterangan","Nilai","Kode PP","Nama PP","Kode DRK","Nama DRK"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[150,80,150,80,100,290,50,200,80]],								
					...
					autoAppend:true,defaultRow:1});		
		this.sgn4 = new portalui_sgNavigator(this.pc2.childPage[4],{bound:[1,this.pc2.height-25,this.pc2.width-1,25],buttonStyle:2,grid:this.sg4});					
...
window.app_saku3_transaksi_pbh_ypt_fVerPajak.extend(window.childForm);
window.app_saku3_transaksi_pbh_ypt_fVerPajak.implement({		
		...
		try{															
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					var sql = new server_util_arrayList();																								
					if (this.c_status.getText() == "APPROVE") var vStatus = "P"; else var vStatus = "K";										
					
					sql.add("update pbh_ver_m set no_flag='"+this.e_nb.getText()+"' where no_bukti='"+this.e_nobukti.getText()+"' and no_flag='-' and form='VERPJK' and modul='"+this.e_modul.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
					sql.add("insert into pbh_ver_m (no_ver,kode_lokasi,tanggal,periode,tgl_input,nik_user,status,modul,form,no_bukti,catatan,no_flag,nik_bdh,nik_fiat) values "+
                            "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.dp_d1.getDateString()+"','"+this.e_periode.getText()+"',getdate(),'"+this.app._userLog+"','"+vStatus+"','"+this.e_modul.getText()+"','VERPJK','"+this.e_nobukti.getText()+"','"+this.e_memo.getText()+"','-','X','X')");
                    													
				    sql.add("update pbh_pb_m set no_pajak='"+this.e_nb.getText()+"',progress='"+vStatus+"',nilai="+nilaiToFloat(this.e_nilai.getText())+",nilai_final="+nilaiToFloat(this.e_nilai.getText())+" "+
					...
					//------------------------------------------------------------------------------------------------------------------------------------------
					//modul PB-cashbasis
					if (this.e_modul.getText() == "PBBAU" || this.e_modul.getText() == "PBBMHD" || this.e_modul.getText() == "PBADK" || this.e_modul.getText() == "PBBA") {
						...
						if (this.sg4.getRowValidCount() > 0){
							for (var i=0;i < this.sg4.getRowCount();i++){
								if (this.sg4.rowValid(i)){										
									sql.add("insert into pbh_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
											"('"+this.e_nobukti.getText()+"','"+this.e_dok.getText()+"','"+this.tglSQL+"',"+i+",'"+this.sg4.cells(0,i)+"','"+this.sg4.cells(3,i)+"','"+this.sg4.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg4.cells(4,i))+",'"+this.sg4.cells(5,i)+"','"+this.sg4.cells(7,i)+"','"+this.app._lokasi+"','"+this.e_modul.getText()+"','PAJAK','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");										
								}
							}
						}
					}
					...
					if (this.e_modul.getText() == "IFREIM") {
						...
						sql.add("delete from hutang_j where jenis in ('BEBAN','PAJAK') and no_hutang='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from angg_r where no_bukti='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						if (this.sg3.getRowValidCount() > 0){
							for (var i=0;i < this.sg3.getRowCount();i++){
								if (this.sg3.rowValid(i)){										
						...
						if (this.sg4.getRowValidCount() > 0){
							for (var i=0;i < this.sg4.getRowCount();i++){
								if (this.sg4.rowValid(i)){										
									sql.add("insert into hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
											"('"+this.e_nobukti.getText()+"','-','"+tglInput+"',"+i+",'"+this.sg4.cells(0,i)+"','"+this.sg4.cells(3,i)+"','"+this.sg4.cells(2,i).toUpperCase()+"','IDR',1,"+parseNilai(this.sg4.cells(4,i))+","+parseNilai(this.sg4.cells(4,i))+",'"+this.sg4.cells(5,i)+"','"+this.sg4.cells(7,i)+"','"+this.app._lokasi+"','IFREIM','PAJAK','"+periodeInput+"','"+this.app._userLog+"',getdate())");					
								}
							}
						}
					...
					if (this.e_modul.getText() == "PJAJU") {
						...
						sql.add("delete from panjar_j where jenis in ('BEBAN','PAJAK') and no_pj='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from angg_r where no_bukti='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						if (this.sg3.getRowValidCount() > 0){
							for (var i=0;i < this.sg3.getRowCount();i++){
								if (this.sg3.rowValid(i)){										
						...
						if (this.sg4.getRowValidCount() > 0){
							for (var i=0;i < this.sg4.getRowCount();i++){
								if (this.sg4.rowValid(i)){										
									sql.add("insert into panjar_j(no_pj, no_dokumen, tanggal, no_urut, kode_akun, keterangan, dc, nilai, kode_pp, kode_drk, kode_lokasi, modul, jenis, periode, kode_curr, kurs, nik_user, tgl_input) values "+
											"('"+this.e_nobukti.getText()+"','-','"+tglInput+"',"+i+",'"+this.sg4.cells(0,i)+"','"+this.sg4.cells(3,i)+"','"+this.sg4.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg4.cells(4,i))+",'"+this.sg4.cells(5,i)+"','"+this.sg4.cells(7,i)+"','"+this.app._lokasi+"','PJAJU','PAJAK','"+periodeInput+"','IDR',1,'"+this.app._userLog+"',getdate())");					
								}
							}
						}
						sql.add("insert into pbh_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) "+
					...
					if (this.e_modul.getText() == "PJPTG") {
						...
						sql.add("delete from ptg_j where jenis in ('BEBAN','PAJAK') and no_ptg='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from angg_r where no_bukti='"+this.e_nobukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						if (this.sg3.getRowValidCount() > 0){
							for (var i=0;i < this.sg3.getRowCount();i++){
								if (this.sg3.rowValid(i)){										
						...
						if (this.sg4.getRowValidCount() > 0){
							for (var i=0;i < this.sg4.getRowCount();i++){
								if (this.sg4.rowValid(i)){										
									sql.add("insert into ptg_j(no_ptg, no_dokumen, tanggal, no_urut, kode_akun, keterangan, dc, nilai, kode_pp, kode_drk, kode_lokasi, modul, jenis, periode, kode_curr, kurs, nik_user, tgl_input, no_link) values "+
											"('"+this.e_nobukti.getText()+"','-','"+tglInput+"',"+i+",'"+this.sg4.cells(0,i)+"','"+this.sg4.cells(3,i)+"','"+this.sg4.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg4.cells(4,i))+",'"+this.sg4.cells(5,i)+"','"+this.sg4.cells(7,i)+"','"+this.app._lokasi+"','PTGPJ','PAJAK','"+periodeInput+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");					
								}
							}
						}
					}
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk)
					...
					this.sg1.clear(1); this.sg3.clear(1); this.sg4.clear(1); 
					this.doClick();
					this.doLoad();					
					...
					if (!this.sg4.rowValid(i)){
						...
						if (!isKosong) {
							...
							for (var j=0;j<this.dataAkunGar.rs.rows.length;j++){
								...
								if (line.kode_akun == this.sg4.cells(0,i) && this.sg4.cells(7,i) == "-") {		
									var k = i+1;
	...
	doSelectDate: function(sender, y,m,d){
		...
		if (this.stsSimpan == 1) {
			this.doClick();		
			this.cb_nb.setSQL("select no_pb, keterangan, case progress when 'P' then 'APPROVE' else 'RETURN' end as status from pbh_pb_m where periode<='"+this.e_periode.getText()+"' and progress in ('P','K') and kode_lokasi='"+this.app._lokasi+"'",["no_pb","keterangan","status"],false,["No Bukti","Deskripsi","Status"],"and","Daftar Bukti",true);				
		}
	},	
	doChange:function(sender){	
		if (sender == this.c_modul3) this.doLoad();								
		if (sender == this.cb_nb && this.cb_nb.getText() != "") {
			var strSQL = "select a.due_date,a.no_pb as no_bukti,case a.progress when 'P' then 'APPROVE' else 'RETURN' end as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.tanggal,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,c.nik+' - '+c.nama as pembuat,a.no_ver as no_ver1,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
						 "from pbh_pb_m a "+
						 "		inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
						 "      inner join karyawan c on a.nik_user=c.nik and a.kode_lokasi=c.kode_lokasi "+
						 "where a.progress in ('P','K') and a.no_pb='"+this.cb_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul in ('PBBAU','PBBMHD', 'PBBA', 'PBADK','IFREIM','IFCLOSE','PJAJU','PJPTG') ";
	...
	doClick:function(sender){		
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"pbh_ver_m","no_ver",this.app._lokasi+"-PJK"+this.e_periode.getText().substr(2,4)+".","0000"));												
	...
	doLoadJurnal:function(){		
		if (this.e_modul.getText() == "PBBAU" || this.e_modul.getText() == "PBBMHD" || this.e_modul.getText() == "PBADK" || this.e_modul.getText() == "PBBA") {			  
			var strSQL3 = "select b.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk "+
						...
						"where a.jenis in ('BEBAN','BMHD','HUTANG') and a.no_pb = '"+this.e_nobukti.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";		
			var strSQL4 = "select b.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk "+
						"from pbh_pb_j a "+
						"	  inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
						"     inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+	
						"     left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and substring(a.periode,1,4)=d.tahun "+																															  
						"where a.jenis = 'PAJAK' and a.no_pb = '"+this.e_nobukti.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";		
		}
		...
		if (this.e_modul.getText() == "IFREIM" || this.e_modul.getText() == "IFCLOSE") {			  
			...
			var strSQL4 = "select b.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk "+
						...
						"     inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+	
						"     left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and substring(a.periode,1,4)=d.tahun "+																															  
						"where a.jenis = 'PAJAK' and a.no_hutang = '"+this.e_nobukti.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";		
		}
		...
		if (this.e_modul.getText() == "PJAJU") {			  
			...
			var strSQL4 = "select b.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk "+
						...
						"     inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+	
						"     left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and substring(a.periode,1,4)=d.tahun "+																															  
						"where a.jenis = 'PAJAK' and a.no_pj = '"+this.e_nobukti.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";		
		}
		...
		if (this.e_modul.getText() == "PJPTG") {			  
			...
			var strSQL4 = "select b.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk "+
						...
						"     inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+	
						"     left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and substring(a.periode,1,4)=d.tahun "+																															  
						"where a.jenis = 'PAJAK' and a.no_ptg = '"+this.e_nobukti.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";		
		}
		...
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			...
			for (var i in data.rs.rows){
				line = data.rs.rows[i];												
				this.sg4.appendData([line.kode_akun,line.nama_akun,line.dc.toUpperCase(),line.keterangan,floatToNilai(line.nilai),line.kode_pp,line.nama_pp,line.kode_drk,line.nama_drk]);
			}
	...
	doLoad:function(sender){			
		strSQL = "select a.due_date,a.no_pb as no_bukti,'INPROG' as status,convert(varchar,a.tanggal,103) as tgl,convert(varchar,a.due_date,103) as tgl2,a.modul,b.kode_pp+' - '+b.nama as pp,'-' as no_dokumen,a.keterangan,a.nilai,a.nik_user as pembuat,a.no_ver as no_ver1,a.kode_lokasi,convert(varchar,a.tgl_input,120) as tglinput,b.kode_pp "+
				 "from pbh_pb_m a "+
				 "		inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+				 
				 "where a.progress='D' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul in ('PBBAU','PBBMHD','PBBA','PBADK','IFREIM','IFCLOSE','PJAJU','PJPTG') order by no_pb";                                
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
			this.sgn.setTotalPage(Math.ceil(data.rs.rows.length/20));
