window.app_saku3_transaksi_pbh_ypt_fPbLogBA = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_pbh_ypt_fPbLogBA.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_pbh_ypt_fPbLogBA";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Permintaan Bayar (BAST)", 0);	
        ...
        this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,996,328], childPage:["Data BAST","Atensi Pembayaran","","Otorisasi","Cattn Verf.","File Dok",""]});          //jurnal++ dan budget di hide  
		this.cb_ba = new saiCBBL(this.pc1.childPage[0],{bound:[20,11,220,20],caption:"Data BAST", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});						
		this.e_nospk = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,17,200,20],caption:"No SPK", readOnly:true,tag:4});				
		this.e_nbast = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,12,200,20],caption:"Nilai BAST", tag:4, readOnly:true, tipeText:ttNilai, text:"0"});						
		this.e_saldo = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,11,200,20],caption:"Saldo BAST", tag:4, readOnly:true, tipeText:ttNilai, text:"0"});				
		this.e_nilai = new saiLabelEdit(this.pc1.childPage[0],{bound:[20,13,200,20],caption:"Ni. Pembayaran", tag:4, tipeText:ttNilai, text:"0",change:[this,"doChange"]});				
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro in ('NIKVER','PPNM') and kode_lokasi = '"+this.app._lokasi+"'",true);			
			if (typeof data == "object"){
				var line;
...
window.app_saku3_transaksi_pbh_ypt_fPbLogBA.extend(window.childForm);
window.app_saku3_transaksi_pbh_ypt_fPbLogBA.implement({	
	isiBAST: function() {
		var strSQL = "select a.no_hutang,a.keterangan,a.no_dokumen "+
					 "from hutang_m a "+		
					 
					 "inner join log_spk_m w on a.no_dokumen=w.no_spk and a.kode_lokasi=w.kode_lokasi "+
					 "inner join log_pesan_m y on w.no_pesan=y.no_pesan and w.kode_lokasi=y.kode_lokasi "+
					 "inner join log_wenang x on y.kode_pp=x.pp_asal and y.kode_lokasi=x.kode_lokasi and y.nilai between x.awal and x.akhir and x.pp_proses='"+this.app._kodePP+"' "+ 					 			 
		...
		this.cb_ba.setSQL(strSQL,["no_hutang","keterangan","no_dokumen"],false,["No BAST","Deskripsi","No SPK"],"and","Data BAST",true);		
	},
				...
				try{														
					...
					if (nilaiToFloat(this.e_ppn.getText()) != 0) {					
						sql.add("insert into hutang_m(no_hutang,kode_lokasi,no_dokumen,tanggal,keterangan,kode_project,kode_vendor,kode_curr,kurs,nik_app,kode_pp,nilai,periode,nik_user,tgl_input,akun_hutang,posted,nilai_ppn,modul,no_ref) values  "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','-','-','IDR',1,'"+this.app._userLog+"','"+this.app._kodePP+"',"+parseNilai(this.e_ppn.getText())+",'"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.akunHutang+"','F',0,'PBBAPPN','"+this.cb_ba.getText()+"')");
						...
						sql.add("insert into hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input) values "+
								"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',1,'"+this.akunHutang+"','"+this.e_ket.getText()+"','C','IDR',1,"+parseNilai(this.e_ppn.getText())+","+parseNilai(this.e_ppn.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBBAPPN','HUT','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate())");					
					}
					...
					sql.add("insert into pbh_pb_m (no_pb,no_dokumen,kode_lokasi,periode,nik_user,tgl_input,tanggal,due_date,keterangan,nilai,modul,progress,kode_pp,nik_app,nik_tahu,no_hutang,no_app,no_spb,no_ver,kode_bidang,kode_loktuj,nilai_final,posted,kode_proyek,no_app2,no_app3,no_fiat,no_kas,akun_hutang,nik_ver) values  "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.dp_d1.getDateString()+"','"+this.dp_d2.getDateString()+"','"+this.e_ket.getText()+"',"+nilaiToFloat(this.e_totalDC.getText())+",'PBBA','0','"+this.app._kodePP+"','"+this.cb_app.getText()+"','"+this.cb_tahu.getText()+"','"+this.cb_ba.getText()+"','-','-','-','"+this.app._kodePP+"','"+this.app._lokasi+"',"+nilaiToFloat(this.e_nilai.getText())+",'X','-','-','-','-','-','-','"+this.cb_ver.getText()+"')");
					sql.add("insert into pbh_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.akunHutang+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_totalDC.getText())+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','PBBA','HUTANG','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");															

					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){																		
								sql.add("insert into pbh_pb_j(no_pb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"','"+this.sg1.cells(7,i)+"','"+this.app._lokasi+"','PBBA','BEBAN','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");										
							}
						}
					}

					sql.add("insert into pbh_rek(no_bukti,kode_lokasi,modul,nama_rek,no_rek,bank,nama,bruto,pajak,nilai) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','PBBA','"+this.e_namarek.getText()+"','"+this.e_norek.getText()+"','"+this.e_bank.getText()+"','"+this.e_namavendor.getText()+"',"+nilaiToFloat(this.e_totalDC.getText())+",0,"+nilaiToFloat(this.e_totalDC.getText())+")");
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"	('"+this.e_nb.getText()+"','PBBA','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(1,i)+"','"+this.sg2.cells(2,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(3,i))+","+nilai+")");
							}
						}
					}
		...
		switch (event){
			...
			case "ubah" :															
				...
				if (nilaiToFloat(this.e_nilai.getText()) <= 0) {
					system.alert(this,"Transaksi tidak valid.","Nilai Pembayaran harus lebih besar nol.");
					return false;
				}
	...
	doChange: function(sender){
		...
		if (sender == this.cb_ba && this.cb_ba.getText() != "") {
			var strSQL = "select a.no_hutang,a.keterangan,a.no_dokumen,a.akun_hutang,a.nilai,a.nilai-isnull(pb,0) as saldo,b.kode_vendor,b.nama as vendor,b.bank,b.no_rek,b.nama_rek "+
						...
						"     left join (select kode_lokasi,no_hutang,sum(nilai_final) as pb "+
					 	...
					 	"                group by kode_lokasi,no_hutang) c on a.no_hutang=c.no_hutang and a.kode_lokasi=c.kode_lokasi "+
						"where a.no_hutang='"+this.cb_ba.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";						

			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object"){
				var line = data.rs.rows[0];							
				if (line != undefined){						
					this.e_nospk.setText(line.no_dokumen);								
					this.e_vendor.setText(line.kode_vendor);					
					this.e_namavendor.setText(line.vendor);					

					this.akunHutang = line.akun_hutang;					
					this.e_nbast.setText(floatToNilai(line.nilai));
	...
	doNilaiChange: function(){		
		try{
			...
			totDC = totDC + nilaiToFloat(this.e_nilai.getText()) + nilaiToFloat(this.e_ppn.getText());			
	...
	doLoad3:function(sender){																				
		...
		var strSQL = "select a.no_pb,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai, "+
					 ...
					 "and a.modul = 'PBBA' and a.progress in ('0','R','K','V') and a.kode_pp in (select kode_pp from karyawan_pp where nik='"+this.app._userLog+"' and kode_lokasi='"+this.app._lokasi+"') "+
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){													
						...
						this.cb_ba.setSQL("select no_hutang,keterangan from hutang_m where no_hutang='"+line.no_hutang+"' and kode_lokasi='"+this.app._lokasi+"'",["no_hutang","keterangan"],false,["No BAST","Deskripsi"],"and","Data BAST",true);		
