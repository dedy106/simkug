window.app_saku3_transaksi_pbh_ypt_fKbPb = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_pbh_ypt_fKbPb.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_pbh_ypt_fKbPb";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank PB", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,996,315], childPage:["Daftar PB","Daftar Transfer","Jurnal Tambahan"]});		
		this.sg5 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:6,tag:0,
		            colTitle:["Status","No PB","Tanggal","Deskripsi","Nilai","Rekening"],
...
window.app_saku3_transaksi_pbh_ypt_fKbPb.extend(window.childForm);
window.app_saku3_transaksi_pbh_ypt_fKbPb.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("update pbh_pb_m set progress='0', no_kas='-' where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");											
					...
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',999,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.e_nilaikb.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBPB','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.e_nilaikb.getText())+")");
										
					for (var i=0;i < this.dataJU5.rs.rows.length;i++){
						line = this.dataJU5.rs.rows[i];
						if (line.status.toUpperCase() == "BAYAR") {
							//dari progress 0-->3 (motong proses)
							sql.add("update pbh_pb_m set progress='3', no_kas='"+this.e_nb.getText()+"' where no_pb = '"+line.no_pb+"' and kode_lokasi='"+this.app._lokasi+"'");						
							...
							sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) "+
									"select '"+this.e_nb.getText()+"',no_dokumen,'"+this.dp_d1.getDateString()+"',no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,'-','-','"+this.app._lokasi+"','KBPB',jenis,'"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',nilai "+
									"from pbh_pb_j "+									
									"where no_pb='"+line.no_pb+"' and jenis in ('HUTANG','BEBAN','HUTIF','PJ','TAK','TUJUAN','PAJAK') and kode_lokasi='"+this.app._lokasi+"'");												
...
//-------------------------------------------------------- 	update status modul ---------------------------------------------------------------
							...
							sql.add("update a set a.progress='2', a.no_kas='"+this.e_nb.getText()+"' "+
									...
									"where b.no_pb='"+line.no_spb+"' and b.kode_lokasi='"+this.app._lokasi+"' ");																									
							//DROPING		
							sql.add("update a set a.progress='2', a.no_kas='"+this.e_nb.getText()+"' "+
									"from ys_minta_m a "+
							...
							sql.add("insert into yk_kasdrop_d(no_spb,nu,no_kas,no_dokumen,kode_lokasi,periode,kode_loktuj,kode_rek,keterangan,nilai,progress,akun_tak,no_kasterima) "+
									"select a.no_pb,0,'"+this.e_nb.getText()+"',a.no_pb,'"+this.app._lokasi+"','"+this.e_periode.getText()+"',a.kode_loktuj,'-',a.keterangan,a.nilai,'0',a.akun_hutang,'-' "+
									"from pbh_pb_m a "+									
									"where a.no_pb = '"+line.no_spb+"'");
					...
					if (this.sg1.getRowValidCount() > 0){
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){
								var k = 1000+i;
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+k+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i).toUpperCase()+"',"+nilaiToFloat(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"','-','-','-','"+this.app._lokasi+"','KBPB','NONKB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.sg1.cells(4,i))+")");										
							}
						}
					}					
			...
			case "hapus" :	
				...
				if (parseFloat(this.app._periode) > parseFloat(this.e_periode.getText())){
					...
					sql.add("update a set a.progress='1', a.no_kas='-' "+
							"from panjar_m a "+
							"inner join pbh_pb_m b on a.no_pj=b.no_pb and a.kode_lokasi=b.kode_lokasi "+														
							"where b.no_kas='"+this.e_nb.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"' ");
	...
	doLoad:function(sender){		
        ...
        var strSQL = "select 'INPROG' as status,no_pb,convert(varchar,tanggal,103) as tgl,keterangan,nilai "+
		             "from pbh_pb_m "+
					 "where progress='0' and no_kas='-' and kode_lokasi='"+this.app._lokasi+"' and modul in ('PBADK','PBBA') ";                    
	...
	doTampilData5: function(page) {		
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU5.rs.rows[i];									
			this.sg5.appendData([line.status.toUpperCase(),line.no_pb,line.tgl,line.keterangan,floatToNilai(line.nilai),"Pilih"]);
		}
	...
	doDoubleClick5: function(sender, col , row) {
		...
		if (noPB != "") {					
            var strSQL = "select a.bank,a.nama,a.no_rek,a.nama_rek,a.bruto,a.pajak,a.nilai "+
						 "from pbh_rek a "+
						 "inner join pbh_pb_m b on a.no_bukti=b.no_pb and a.kode_lokasi=b.kode_lokasi and a.modul<>'PINBUK-C' "+
	...
	doLoad3:function(sender){																		
		var strSQL = "select a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.jenis,a.no_dokumen,a.keterangan,a.nilai "+
					 ...
					 "where b.no_kas is null and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'KBPB' and a.posted ='F'";		
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
                ...
                var strSQL = "select 'BAYAR' as status,no_pb,convert(varchar,tanggal,103) as tgl,keterangan,nilai "+
							 "from pbh_pb_m a "+
							 "where no_kas='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";							 
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
