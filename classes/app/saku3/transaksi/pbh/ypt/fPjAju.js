window.app_saku3_transaksi_pbh_ypt_fPjAju = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_pbh_ypt_fPjAju.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_pbh_ypt_fPjAju";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan Panjar", 0);	
		...
		this.sg3 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:7,tag:9,
					...
					colWidth:[[6,5,4,3,2,1,0],[70,150,100,350,150,80,100]],
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,22,996,305], childPage:["Item Peruntukan","Budget","File Dok","Otorisasi & Bank","Cattn Verf."]});		
...
window.app_saku3_transaksi_pbh_ypt_fPjAju.extend(window.childForm);
window.app_saku3_transaksi_pbh_ypt_fPjAju.implement({	
	...
	doHitungGar: function(){
		try {
			...
			for (var i=0;i < this.sg2.getRowCount();i++){				
				var data = this.dbLib.getDataProvider("select fn_cekagg3('"+this.sg2.cells(1,i)+"','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(2,i)+"','"+this.e_periode.getText()+"','"+this.e_nopj.getText()+"') as gar ",true);			
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into panjar_m(no_pj, no_kas, no_dokumen, tanggal, due_date, keterangan, catatan, kode_curr, kurs, akun_pj, nik_pengaju, nik_setuju, kode_lokasi, kode_pp, modul, nilai, nilai_pot, kode_drk, progress, periode, no_del, no_link, nik_user, tgl_input) values "+
						    "('"+this.e_nopj.getText()+"', '-', '-', '"+this.dp_d1.getDateString()+"', '"+this.dp_d1.getDateString()+"', '"+this.e_ket.getText()+"', '-', 'IDR', 1, '"+this.cb_akunpj.getText()+"', '"+this.cb_nik.getText()+"', '"+this.cb_app.getText()+"', '"+this.app._lokasi+"', '"+this.kodePP+"', 'PJAJU', "+nilaiToFloat(this.e_nilai.getText())+", 0, '-', '0', '"+this.e_periode.getText()+"', '-', '"+this.cb_tahu.getText()+"', '"+this.app._userLog+"', getdate())");						
						...
						for (var i=0;i < this.sg1.getRowCount();i++){
							if (this.sg1.rowValid(i)){								
								sql.add("insert into panjar_j(no_pj, no_dokumen, tanggal, no_urut, kode_akun, keterangan, dc, nilai, kode_pp, kode_drk, kode_lokasi, modul, jenis, periode, kode_curr, kurs, nik_user, tgl_input) values "+
										"('"+this.e_nopj.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(0,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(2,i).toUpperCase()+"',"+parseNilai(this.sg1.cells(4,i))+",'"+this.sg1.cells(5,i)+"','"+this.sg1.cells(7,i)+"','"+this.app._lokasi+"','PJAJU','BEBAN','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");					
							}
						}
					}	
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
										"('"+this.e_nopj.getText()+"','PJAJU','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"','"+this.sg2.cells(1,i)+"','"+this.sg2.cells(2,i)+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','"+DC+"',"+parseNilai(this.sg2.cells(3,i))+","+nilai+")");
							}
						}
					}
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						this.cb_tahu.setText(line.no_link);						
