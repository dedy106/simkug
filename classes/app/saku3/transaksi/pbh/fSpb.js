window.app_saku3_transaksi_pbh_fSpb = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_pbh_fSpb.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_pbh_fSpb";
		...
		this.e_cek = new saiLabelEdit(this.pc2.childPage[0],{bound:[10,18,450,20],caption:"No Cheque", maxLength:50});				
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,996,315], childPage:["Daftar PB","Daftar Transfer","Tambah PB","Otorisasi"]});		
		...
		this.cb_man = new saiCBBL(this.pc1.childPage[3],{bound:[20,15,220,20],caption:"Manager Keuangan", multiSelection:false, maxLength:10, tag:2});								
		this.cb_gm = new saiCBBL(this.pc1.childPage[3],{bound:[20,14,220,20],caption:"General Manager", multiSelection:false, maxLength:10, tag:2});						
		this.cb_bdh = new saiCBBL(this.pc1.childPage[3],{bound:[20,16,220,20],caption:"Bendahara Umum", multiSelection:false, maxLength:10, tag:2});						
		this.cb_dir = new saiCBBL(this.pc1.childPage[3],{bound:[20,17,220,20],caption:"Ketua Umum", multiSelection:false, maxLength:10, tag:2});								
		...
		try {
			...
			this.cb_man.setSQL("select nik, nama from karyawan where flag_aktif='1' and kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Data Karyawan",true);
			this.cb_gm.setSQL("select nik, nama from karyawan where flag_aktif='1' and kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Data Karyawan",true);
			...
			this.cb_dir.setSQL("select nik, nama from karyawan where flag_aktif='1' and kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Data Karyawan",true);
			
			var data = this.dbLib.getDataProvider("select a.kode_spro,a.flag,b.nama from spro a inner join karyawan b on a.flag=b.nik and a.kode_lokasi=b.kode_lokasi where a.kode_spro in ('MANKUG','GMKUG','BDH','DIR') and a.kode_lokasi = '"+this.app._lokasi+"'",true);			
			if (typeof data == "object"){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];																							
					if (line.kode_spro == "MANKUG") this.cb_man.setText(line.flag,line.nama);	
					if (line.kode_spro == "GMKUG") this.cb_gm.setText(line.flag,line.nama);								
					if (line.kode_spro == "BDH") this.cb_bdh.setText(line.flag,line.nama);								
					if (line.kode_spro == "DIR") this.cb_dir.setText(line.flag,line.nama);								
				}
			}			
...
window.app_saku3_transaksi_pbh_fSpb.extend(window.childForm);
window.app_saku3_transaksi_pbh_fSpb.implement({	
	doAdd: function() {	
		try {
			var temu = false;
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into spb_m (no_spb,no_dokumen,no_ver,no_bukti,kode_lokasi,periode,nik_user,tgl_input,tanggal,due_date,keterangan,nik_buat,nik_sah,nik_fiat,nik_bdh,lok_bayar,no_kas,nilai,modul,no_fiat,progress,kode_ppasal,nik_gm,no_cek) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','-','-','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.dp_d1.getDateString()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_buat.getText()+"','"+this.cb_man.getText()+"','"+this.cb_dir.getText()+"','"+this.cb_bdh.getText()+"','"+this.app._lokasi+"','-',"+nilaiToFloat(this.e_nilaispb.getText())+",'SPB','-','0','-','"+this.cb_gm.getText()+"','"+this.e_cek.getText()+"')");
					...
					for (var i = 0; i < this.sg5.rows.getLength();i++){
						if (this.sg5.rowValid(i) && this.sg5.cells(0,i) == "DPC"){							
							...
							sql.add("insert into spb_j (no_spb,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs)  "+
									...
									"from pbh_pb_j where no_pb='"+this.sg5.cells(1,i)+"' and jenis in ('BEBAN','HUTIF') and kode_lokasi='"+this.app._lokasi+"'");							
						}
					}
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_pbh_rptMcm";
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){								
						...
						this.cb_man.setText(line.nik_sah);
						this.cb_gm.setText(line.nik_gm);
						this.cb_dir.setText(line.nik_fiat);
					}
				}								
