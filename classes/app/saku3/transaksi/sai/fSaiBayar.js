window.app_saku3_transaksi_sai_fSaiBayar = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sai_fSaiBayar.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sai_fSaiBayar";
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:7,tag:0,
				colTitle:["Status","No Bill","Periode","No Invoice","Deskripsi","Akun AR","Tagihan"],
				colWidth:[[6,5,4,3,2,1,0],[100,80,250,200,60,150,80]],
...
window.app_saku3_transaksi_sai_fSaiBayar.extend(window.childForm);
window.app_saku3_transaksi_sai_fSaiBayar.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("delete from sai_billbayar_d where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("update sai_bill_d set no_kas='-' where no_kas='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					} 
					
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBPIUT','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_total.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','"+this.cb_cust.getText()+"','-','-')");
															
					var kasIDR = nilaiToFloat(this.e_total.getText());
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','D',"+kasIDR+",'"+this.app._kodePP+"','-','-','"+this.cb_cust.getText()+"','"+this.app._lokasi+"','KBPIUT','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiToFloat(this.e_total.getText())+")");
										
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && this.sg1.cells(0,i) == "BAYAR"){
							var nilaiPiu = nilaiToFloat(this.sg1.cells(6,i));							
							sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
									"('"+this.e_nb.getText()+"','"+this.sg1.cells(1,i)+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(5,i)+"','Pelunasan atas invoice "+this.sg1.cells(1,i)+"','C',"+nilaiPiu+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBPIUT','AR','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+nilaiPiu+")");
														
							sql.add("insert into sai_billbayar_d(no_bukti,kode_lokasi,no_bill,no_inv,akun_piutang,nilai_kas,nilai_lain,nilai_sk,periode,dc,modul,kode_curr,kurs,kode_cust)  values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.sg1.cells(1,i)+"','"+this.sg1.cells(3,i)+"','"+this.sg1.cells(5,i)+"',"+nilaiToFloat(this.sg1.cells(6,i))+",0,0,'"+this.e_periode.getText()+"','D','KB','IDR',1,'"+this.cb_cust.getText()+"')");																							
							
							sql.add("update sai_bill_d set no_kas='"+this.e_nb.getText()+"' where no_inv='"+this.sg1.cells(3,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
						}
					}																																		
					if (this.sg.getRowValidCount() > 0) {
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								...
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
										"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+nilaiSlsIDR+",'"+this.sg.cells(5,i)+"','-','-','-','"+this.app._lokasi+"','KBPIUT','SLS','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-','"+nilaiToFloat(this.sg.cells(4,i))+"')");
							}
						}
					}					
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_cust && this.cb_cust.getText()!="" && this.stsSimpan==1) {			
			var strSQL = "select a.no_bill,a.periode,a.no_inv,'Kontrak : '+a.no_kontrak as ket,a.akun_piutang,round(a.total-isnull(b.bayar,0),2) as saldo "+
			             "from ( "+						 						 
						 "select a.kode_cust,a.no_bill,a.no_inv,a.no_kontrak,c.akun_piutang,a.kode_lokasi,a.periode, "+
						 "sum(a.nilai+a.nilai_ppn) as total  "+ 
						 "from sai_bill_d a inner join sai_kontrak_m b on a.no_kontrak=b.no_kontrak and a.kode_lokasi=b.kode_lokasi "+
						 "                  inner join sai_produk c on b.kode_produk=c.kode_produk and c.kode_lokasi=b.kode_lokasi "+
						 "where a.kode_cust='"+this.cb_cust.getText()+"' and a.nilai+a.nilai_ppn > 0  and a.kode_lokasi='"+this.app._lokasi+"' "+
						 "group by a.kode_cust,a.no_bill,a.no_inv,a.no_kontrak,c.akun_piutang,a.kode_lokasi,periode "+
						 ")a  "+						 						 
						 "left join ( "+
						 "    select no_inv,kode_lokasi,sum(case dc when 'D' then nilai_kas else -nilai_kas end) as bayar  "+
						 "    from sai_billbayar_d where kode_cust='"+this.cb_cust.getText()+"' and kode_lokasi='"+this.app._lokasi+"' group by no_inv,kode_lokasi"+
						 ") b "+						 
						 "on a.no_inv=b.no_inv and a.kode_lokasi=b.kode_lokasi "+						 						 
						 "where a.kode_cust='"+this.cb_cust.getText()+"' and a.total>isnull(b.bayar,0) and a.kode_lokasi ='"+this.app._lokasi+"' order by a.no_inv";			
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];												
					this.sg1.appendData(["INPROG",line.no_bill,line.periode,line.no_inv,line.ket.toUpperCase(),line.akun_piutang,floatToNilai(line.saldo)]);
				}
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan "+
		             "from kas_m a "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'KBPIUT' and a.posted ='F'";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){							
						...
						this.cb_cust.setSQL("select kode_cust, nama from sai_cust where kode_cust='"+line.no_link+"' and kode_lokasi='"+this.app._lokasi+"'",["kode_cust","nama"],false,["Kode","Nama"],"and","Data Customer",true);									
					}
				}
				var strSQL = "select a.no_bill,a.periode,a.no_inv,'Kontrak : '+a.no_kontrak as ket,a.akun_piutang,round(a.total-isnull(b.bayar,0),2) as saldo "+
							 "from ( "+						 						 
							 ...
							 "group by a.kode_cust,a.no_bill,a.no_inv,a.no_kontrak,c.akun_piutang,a.kode_lokasi,periode "+
							 ")a  "+						 						 
							 "inner join sai_billbayar_d c on a.no_inv=c.no_inv and a.kode_lokasi=c.kode_lokasi "+
							 "left join ( "+
							 "    select no_inv,kode_lokasi,sum(case dc when 'D' then nilai_kas else -nilai_kas end) as bayar  "+
							 "    from sai_billbayar_d where no_bukti<>'"+this.e_nb.getText()+"' and kode_cust='"+this.cb_cust.getText()+"' and kode_lokasi='"+this.app._lokasi+"' group by no_inv,kode_lokasi"+
							 ") b "+						 
							 "on a.no_inv=b.no_inv and a.kode_lokasi=b.kode_lokasi "+						 						 
							 "where c.no_bukti='"+this.e_nb.getText()+"' and c.kode_lokasi ='"+this.app._lokasi+"' order by a.no_inv";			
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg1.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];												
						this.sg1.appendData(["BAYAR",line.no_bill,line.periode,line.no_inv,line.ket.toUpperCase(),line.akun_piutang,floatToNilai(line.saldo)]);
					}
				} else this.sg1.clear(1);
