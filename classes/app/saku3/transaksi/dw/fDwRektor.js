window.app_saku3_transaksi_dw_fDwRektor = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_dw_fDwRektor.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_dw_fDwRektor";
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select distinct tahun from exs_rektor order by tahun desc",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
...
window.app_saku3_transaksi_dw_fDwRektor.extend(window.childForm);
window.app_saku3_transaksi_dw_fDwRektor.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into exs_rektor(kode_rektor,nama,kode_lokasi,flag_aktif,tahun) values ('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.c_status.getText().substr(0,1)+"','"+this.c_tahun.getText()+"')");
					setTipeButton(tbAllFalse);
	...
	simpan2: function(){			
		try{						
			if (this.sg.getRowValidCount() > 0){
				uses("server_util_arrayList");
				var sql = new server_util_arrayList();
				sql.add("delete from exs_rektor where kode_lokasi='"+this.app._lokasi+"'");
				for (var i=0;i < this.sg.getRowCount();i++){
					if (this.sg.rowValid(i)){
						sql.add("insert into exs_rektor(kode_rektor,nama,kode_lokasi,flag_aktif,tahun) values ('"+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from exs_rektor where kode_rektor = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("insert into exs_rektor(kode_rektor,nama,kode_lokasi,flag_aktif,tahun) values ('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.c_status.getText().substr(0,1)+"','"+this.c_tahun.getText()+"')");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select kode_rektor,nama,tahun from exs_rektor where kode_rektor ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
		...
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Flag Akun",sender,undefined, 
											  "select kode_rektor, nama  from exs_rektor where kode_lokasi='"+this.app._lokasi+"'",
											  "select count(kode_rektor) from exs_rektor where kode_lokasi='"+this.app._lokasi+"'",
	...
	doLoad:function(sender){						
		var strSQL = "select kode_rektor,nama,tahun "+
		             "from exs_rektor where kode_lokasi='"+this.app._lokasi+"' order by kode_rektor ";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
