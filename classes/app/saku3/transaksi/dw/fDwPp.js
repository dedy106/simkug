window.app_saku3_transaksi_dw_fDwPp = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_dw_fDwPp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_dw_fDwPp";
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select distinct tahun from exs_pp order by tahun desc",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
			...
			this.cb_bidang.setSQL("select kode_bidang, nama from exs_bidang where kode_lokasi='"+this.app._lokasi+"' ",["kode_bidang","nama"],false,["Kode","Nama"],"and","Data Bidang",true);
			this.doLoad();
		}catch(e){
			systemAPI.alert(e);
		}
...
window.app_saku3_transaksi_dw_fDwPp.extend(window.childForm);
window.app_saku3_transaksi_dw_fDwPp.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into exs_pp(kode_pp,nama,kode_lokasi,flag_aktif,tahun,kode_bidang,skode) values ('"+
	...
	simpan2: function(){			
		try{						
			...
			if (this.sg.getRowValidCount() > 0){
				...
				{
					sql.add("delete from exs_pp where kode_lokasi='"+this.app._lokasi+"'");
				}
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from exs_pp where kode_pp = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("insert into exs_pp(kode_pp,nama,kode_lokasi,flag_aktif,tahun,kode_bidang,skode) values ('"+
							this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.c_status.getText().substr(0,1)+"','"+this.c_tahun.getText()+"','"+this.cb_bidang.getText()+"','"+this.e_skode.getText()+"')");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select kode_pp,nama,tahun,kode_bidang from exs_pp where kode_pp ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
		...
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Flag Akun",sender,undefined, 
											  "select kode_pp, nama  from exs_pp where kode_lokasi='"+this.app._lokasi+"'",
											  "select count(kode_pp) from exs_pp where kode_lokasi='"+this.app._lokasi+"'",
	...
	doLoad:function(sender){						
		var strSQL = "select kode_pp,nama,tahun,kode_bidang,skode "+
		             "from exs_pp where kode_lokasi='"+this.app._lokasi+"' order by kode_pp ";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
