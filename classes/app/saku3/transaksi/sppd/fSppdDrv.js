window.app_saku3_transaksi_sppd_fSppdDrv = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sppd_fSppdDrv.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sppd_fSppdDrv";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form SPPD Driver", 0);	
		...
		this.cb_pp = new saiCBBL(this.pc2.childPage[0],{bound:[520,13,220,20],caption:"PP", maxLength:10, tag:7, readOnly:true,visible:false, multiSelection:false});
		this.cb_unit = new saiCBBL(this.pc2.childPage[0],{bound:[20,13,220,20],caption:"Unit", maxLength:10, tag:7, readOnly:true});		
		this.e_nilaiut = new saiLabelEdit(this.pc2.childPage[0],{bound:[780,13,200,20],caption:"Nilai Transport", tag:9, readOnly:true, tipeText:ttNilai, text:"0",change:[this,"doChange"],visible:false});			
		this.e_jmlaju = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,18,200,20],caption:"Jml Driver", maxLength:50, tipeText:ttNilai, readOnly:true, text:"0"});													
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,292], childPage:["Data SPPD","Uang Harian","Approval","Referensi"]});
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:9,tag:9,visible:false,		            
		...
		this.sgn = new portalui_sgNavigator(this.pc1.childPage[1],{bound:[0,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:2,grid:this.sg,visible:false});


		this.sg2 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:8,tag:0,
		...
		this.cb_atas1 = new saiCBBL(this.pc1.childPage[2],{bound:[20,10,220,20],caption:"NIP Atasan 1", multiSelection:false, readOnly:true, tag:1});	
		this.e_jabatas1 =  new saiLabelEdit(this.pc1.childPage[2],{bound:[20,32,450,20],caption:"Jabatan", maxLength:80, readOnly:true});				
		this.cb_atas2 = new saiCBBL(this.pc1.childPage[2],{bound:[20,54,220,20],caption:"NIP Atasan 2", multiSelection:false, readOnly:true, tag:1});	
		this.e_jabatas2 =  new saiLabelEdit(this.pc1.childPage[2],{bound:[20,76,450,20],caption:"Jabatan", maxLength:80, readOnly:true});	

		this.cb_asal = new saiCBBL(this.pc1.childPage[3],{bound:[20,10,220,20],caption:"Kota Asal", multiSelection:false, tag:9, change:[this,"doChange"]});	
		this.cb_kota = new saiCBBL(this.pc1.childPage[3],{bound:[20,11,220,20],caption:"Kota Tujuan", multiSelection:false, tag:9, change:[this,"doChange"]});	
		...
		try {
			...
			this.cb_nik.setSQL("select nik, nama from karyawan where kode_jab='DRIVER' and flag_sppd='1'",["nik","nama"],false,["NIK","Nama"],"and","Data Karyawan",true);
...
window.app_saku3_transaksi_sppd_fSppdDrv.extend(window.childForm);
window.app_saku3_transaksi_sppd_fSppdDrv.implement({	
	isiCBPerintah: function(sender) {	
		this.cb_perintah.setSQL("select distinct a.no_perintah, a.keterangan, convert(varchar,a.tanggal,103) as tgl "+
							  ...
							  "where a.no_batch='-' and ((a.kode_kategori<>'8' and d.no_perintah is null) or (a.kode_kategori='8')) and b.kode_lokasi='"+this.app._lokasi+"' and (b.jum_driver-isnull(c.pakai,0)) > 0 ",
		...
		var strSQL = "select top 1 a.no_perintah "+
				  ...
				  "where a.no_batch='-' and ((a.kode_kategori<>'8' and d.no_perintah is null) or (a.kode_kategori='8')) and b.kode_lokasi='"+
				  this.app._lokasi+"' and (b.jum_driver-isnull(c.pakai,0)) > 0 order by a.no_perintah";
		...
		if (this.stsSimpan == 1) {
			...
			if (typeof data2 == "object" && data2.rs.rows[0] != undefined){
				...
				if (this.jenisApp1.toUpperCase() != "LOS") {
					...
					this.cb_atas1.setSQL("select nik, nama from karyawan where flag_aktif='1' ",["nik","nama"],false,["NIP","Nama"],"and","Data Karyawan",true);			
					
					this.cb_atas2.setBtnVisible(true);
					this.cb_atas2.setSQL("select nik, nama from karyawan where flag_aktif='1' ",["nik","nama"],false,["NIP","Nama"],"and","Data Karyawan",true);							
				}			
			}	
		}
	},
	...
	doChange:function(sender){
		try {
			if (sender == this.cb_perintah && this.cb_perintah.getText()!="") {				
				var data2 = this.dbLib.getDataProvider("select c.kode_pp, a.kode_unit, dd.nama as asal,d.nama as tempat,a.jum_driver as jumlah, isnull(b.pakai,0) as jml_input,  convert(varchar,c.tgl_mulai,103) + ' - '+ convert(varchar,c.tgl_selesai,103) as tgl_kegiatan, convert(varchar,c.tgl_mulai,103) as tglmulai,convert(varchar,c.tgl_selesai,103) as tglselesai,  a.jum_driver-isnull(b.pakai,0) as sisa "+
							...
							"				where a.kode_lokasi='"+this.app._lokasi+"' and a.no_spj<>'"+this.e_nb.getText()+"' and a.progress <>'Z' "+
							" 				group by a.no_perintah,a.kode_unit,a.kode_lokasi "+
							") b on b.no_perintah=a.no_perintah and a.kode_lokasi=b.kode_lokasi and a.kode_unit=b.kode_unit "+
							"where a.kode_lokasi='"+this.app._lokasi+"' and a.no_perintah='"+this.cb_perintah.getText()+"'",true);
		...
		try {
			...
			if (col == 2 || col == 3 || col == 0) {			 
				if ( this.e_jab1.getText() != "" && this.e_jab2.getText() != "") {
					...
					system.alert(this,"Transaksi tidak valid.","Harap pilih NIP SPPD terlebih dahulu (Jabatan harus terisi).");
					return false;
				}	
			}		
	...
	doLoad3:function(sender){			
		var strSQL = "select a.no_spj,convert(varchar,a.tanggal,103) as tgl,a.nama_spj, b.keterangan, a.progress, a.nilai_trans+a.nilai_uhar as total "+
					 ...
					 "where a.form = 'PDDRV' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and "+
