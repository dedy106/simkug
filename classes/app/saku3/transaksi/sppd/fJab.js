window.app_saku3_transaksi_sppd_fJab = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sppd_fJab.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sppd_fJab";
...
window.app_saku3_transaksi_sppd_fJab.extend(window.childForm);
window.app_saku3_transaksi_sppd_fJab.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into sp_jab(kode_jab,nama,kode_lokasi) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();					
					sql.add("delete from sp_jab where kode_jab = '"+this.cb_kode.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");			
					sql.add("insert into sp_jab(kode_jab,nama,kode_lokasi) values "+
						    "('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"')");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from sp_jab where kode_jab = '"+this.cb_kode.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");sql.add("delete from sp_jab where kode_jab = '"+this.cb_kode.getText()+"'");				
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			case "clear" :
				...
				break;
			case "simpan" :	
				var strSQL = "select kode_jab from sp_jab where nama ='"+this.e_nama.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";						   
			...
			case "simpancek" : this.simpan();			
				break;
			case "ubah" :
				var strSQL = "select kode_jab from sp_jab where kode_jab <> '"+this.cb_kode.getText()+"' and nama ='"+this.e_nama.getText()+"'";						   
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){						
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select * from sp_jab where kode_jab ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";						   
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){						
	...
	doLoad:function(sender){						
		var strSQL = "select * from sp_jab where kode_lokasi='"+this.app._lokasi+"'";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
