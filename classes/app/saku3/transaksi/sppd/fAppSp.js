window.app_saku3_transaksi_sppd_fAppSp = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sppd_fAppSp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sppd_fAppSp";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Approval SPPD", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,370], childPage:["Daftar PD","Detail Transport","Detail U Harian"]});		
		...
		this.sg4 = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:8,tag:9,
					...
					autoAppend:false});
		this.sgn4 = new portalui_sgNavigator(this.pc1.childPage[2],{bound:[0,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:3,grid:this.sg4});
...
window.app_saku3_transaksi_sppd_fAppSp.extend(window.childForm);
window.app_saku3_transaksi_sppd_fAppSp.implement({	
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						sql.add("update sp_spj_m set nik_appsap='-',no_app='-',progress='0' where progress='A' and no_app = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from sp_app_m where no_app = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from sp_app_d where no_app = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from sp_spj_app where no_app = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}					
					sql.add("insert into sp_app_m (no_app,tanggal,kode_lokasi,periode,nik_user,tgl_input,status,modul,no_appseb) values "+
					...
					// sql.add("insert into sp_spj_app(no_app,kode_lokasi,maksud,dasar,nik_app,jabatan,no_dokumen,no_ver,no_spb,progress,periode,tanggal) values "+
					...
					for (var i=0;i < this.dataJU5.rs.rows.length;i++){
						...
						if (line.status.toUpperCase() == "APP") {
							sql.add("update sp_spj_m set no_app='"+this.e_nb.getText()+"',progress='A' where no_spj = '"+line.no_spj+"' and kode_lokasi='"+this.app._lokasi+"'");
							sql.add("insert into sp_app_d (no_app,status,modul,no_bukti,kode_lokasi,catatan) values "+
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "") {
			...
			this.stsSimpan = 1;
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"sp_app_m","no_app",this.app._lokasi+"-PDA"+this.e_periode.getText().substr(2,4)+".","0000"));						
			this.e_dok.setFocus();
	...
	doLoad:function(sender){		
		...
		this.dataJU5 = {rs:{rows:[]}};
		var strSQL = "select distinct 'INPROG' as status,a.progress,a.no_spj,a.nama_spj,convert(varchar,a.due_date,103) as tgl, "+
                        "a.keterangan,a.transport+a.harian as nilai,b.kode_akun+' - '+b.nama as akun, "+
                        "c.kode_pp+' - '+c.nama as pp,isnull(a.kode_drk,'-')+' - '+isnull(d.nama,'-') as drk "+ 
                        "from sp_spj_m a "+
                        "inner join masakun b on a.akun_uhar=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+				 
                        "inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi 	"+			 					 
                        "left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi "+
                        "where "+nikApp+" a.progress = 'P' and a.kode_lokasi='"+this.app._lokasi+"'";			
	...
	doLoad3:function(sender){																				
		var strSQL = "select a.no_app,convert(varchar,a.tanggal,103) as tgl,a.modul,b.dasar,b.maksud "+
		             "from sp_app_m a inner join sp_spj_app b on a.no_app=b.no_app and a.kode_lokasi=b.kode_lokasi "+					 					 
					 "             left join (select distinct no_app,kode_lokasi from sp_spj_m where progress='1' and no_spb<>'-' and kode_lokasi='"+this.app._lokasi+"') c on b.no_app=c.no_app and b.kode_lokasi=c.kode_lokasi "+
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				this.dataJU5 = {rs:{rows:[]}};
				var strSQL = "select 'APP' as status,a.no_spj,a.nama_spj,convert(varchar,a.due_date,103) as tgl,a.keterangan,a.transport+a.harian as nilai,b.kode_akun+' - '+b.nama as akun, c.kode_pp+' - '+c.nama as pp,isnull(d.kode_drk,'-')+' - '+isnull(d.nama,'-') as drk,a.nik_appsap "+
					 "from sp_spj_m a inner join masakun b on a.akun_uhar=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+					 
					 "                inner join pp c on a.kode_pp=c.kode_pp and a.kode_lokasi=c.kode_lokasi "+					 					 
					 "                left join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi "+					 
					 "where a.no_app='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress='A'";		
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					this.dataJU5 = data;
					this.sgn5.setTotalPage(Math.ceil(data.rs.rows.length/20));
