window.app_saku3_transaksi_gar_fRRAjuLocal = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_gar_fRRAjuLocal.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_gar_fRRAjuLocal";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Reprogramming Approve Local: Input", 0);	
		...
		this.c_bulanD = new saiCB(this.pc1.childPage[0],{bound:[20,19,200,20],caption:"Bulan Donor", tag:9,change:[this,"doChange"]}); 
		...
		this.c_bulanT = new saiCB(this.pc1.childPage[0],{bound:[20,19,200,20],caption:"Bulan Penerima", tag:9}); 
		...
		try {
			...
			if (this.app._lokasi == this.app._kodeLokasiPusat) var bidang = " and c.kode_bidang ="+this.app._kodeBidang;
			else {
				var bidang = " ";				
			}
			this.cb_akunD.setSQL("select distinct a.kode_akun, a.nama from masakun a inner join anggaran_d b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
			                     ...
			                     "where b.periode like '"+this.e_periode.getText().substr(0,4)+"%' and b.kode_lokasi='"+this.app._lokasi+"' "+bidang,["a.kode_akun","a.nama"],false,["Kode","Nama"],"and","Data Akun Donor",true);
			
			this.cb_ppD.setSQL("select c.kode_pp, c.nama from pp c where c.kode_lokasi='"+this.app._lokasi+"' "+bidang+" ",["c.kode_pp","c.nama"],false,["Kode","Nama"],"and","Data PP",true);
			this.cb_ppT.setSQL("select c.kode_pp, c.nama from pp c where c.kode_lokasi='"+this.app._lokasi+"' "+bidang+" ",["c.kode_pp","c.nama"],false,["Kode","Nama"],"and","Data PP",true);						
			
			this.cb_app.setSQL("select a.nik, a.nama from karyawan a where kode_lokasi='"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Data Karyawan",true);						
...
window.app_saku3_transaksi_gar_fRRAjuLocal.extend(window.childForm);
window.app_saku3_transaksi_gar_fRRAjuLocal.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into rra_pdrk_m(no_pdrk,kode_lokasi,keterangan,kode_pp,kode_bidang,jenis_agg,tanggal,periode,nik_buat,nik_app1,nik_app2,nik_app3,sts_pdrk,justifikasi, nik_user, tgl_input,progress,modul) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_periode.getText()+"','"+this.app._userLog+"','"+this.app._userLog+"','"+this.cb_app.getText()+"','"+this.cb_app.getText()+"','RRR','-','"+this.app._userLog+"',getdate(),'0','LOCAL')");							
					var periode ="";
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								...
								sql.add("insert into anggaran_d(no_agg,kode_lokasi,no_urut,kode_pp,kode_akun,kode_drk,volume,periode,nilai_sat,nilai,dc,satuan,nik_user,tgl_input,modul) values "+		
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',"+i+",'"+this.sg.cells(2,i)+"','"+this.sg.cells(0,i)+"','"+this.sg.cells(4,i)+"',1,'"+periode+"',"+parseNilai(this.sg.cells(8,i))+","+parseNilai(this.sg.cells(8,i))+",'C','-','"+this.app._userLog+"',getdate(),'RRLOCAL')");
							}
						}
					}
			...
			case "ubah" :	
				...
				for (var i=0;i < this.sg.getRowCount();i++){					
					if (!this.sg.rowValid(i)){
						...
						if ((this.e_periode.getText().substr(4,2) == "10" || this.e_periode.getText().substr(4,2) == "11" || this.e_periode.getText().substr(4,2) == "12" || this.e_periode.getText().substr(4,2) == "13") && this.sg.cells(6,i)!="10" && this.sg.cells(6,i)!="11" && this.sg.cells(6,i)!="12") {
							system.alert(this,"Transaksi tidak valid.","Periode donor tidak dalam satu triwulan dengan periode transaksi.");
							return false;
						}
					}
				...
				for (var i=0;i < this.sg2.getRowCount();i++){					
					if (!this.sg2.rowValid(i)){
						...
						if ((this.e_periode.getText().substr(4,2) == "10" || this.e_periode.getText().substr(4,2) == "11" || this.e_periode.getText().substr(4,2) == "12" || this.e_periode.getText().substr(4,2) == "13") && this.sg2.cells(6,i)!="10" && this.sg2.cells(6,i)!="11" && this.sg2.cells(6,i)!="12") {
							system.alert(this,"Transaksi tidak valid.","Periode penerima tidak dalam satu triwulan dengan periode transaksi.");
							return false;
						}						
					}
	...
	doSelectDate: function(sender, y,m,d){
		...
		if (this.e_periode.getText().substr(4,2) == "10" || this.e_periode.getText().substr(4,2) == "11" || this.e_periode.getText().substr(4,2) == "12" || this.e_periode.getText().substr(4,2) == "13") {
	...
	doChange:function(sender){				
		...
		if (sender == this.c_bulanD || sender == this.cb_drkD || sender == this.cb_ppD || sender == this.cb_akunD) {
			...
			if (this.cb_akunD.getText()!="" && this.cb_ppD.getText()!="" && this.cb_drkD.getText()!="" && this.c_bulanD.getText()!="") {				
				...
				var data = this.dbLib.getDataProvider("select fn_cekaggBulan('"+this.cb_ppD.getText()+"','"+this.app._lokasi+"','"+this.cb_akunD.getText()+"','"+this.cb_drkD.getText()+"','"+this.e_periode.getText().substr(0,4)+this.c_bulanD.getText()+"','"+this.e_nb.getText()+"') as gar ",true);
	...
	doClick:function(sender){
		...
		this.stsSimpan = 1;
		this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"anggaran_m","no_agg",this.app._lokasi+"-RRL"+this.e_periode.getText().substr(2,4)+".","0000"));
		this.e_dok.setFocus();
	...
	doLoad3:function(sender){																						
		var strSQL = "select a.no_pdrk,convert(varchar,a.tanggal,103) as tgl,b.no_dokumen,a.keterangan,a.progress "+
					 ...
					 "where a.modul = 'LOCAL' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress in ('0','R') order by a.tanggal";		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select substring(a.periode,5,2) as bulan,a.kode_pp,b.nama as nama_pp,a.kode_drk,d.nama as nama_drk,a.kode_akun,c.nama as nama_akun,a.nilai "+
							 ...
							 "where a.no_pdrk='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.dc ='C' ";					
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear(1);
				...
				var strSQL = "select substring(a.periode,5,2) as bulan,a.kode_pp,b.nama as nama_pp,a.kode_drk,d.nama as nama_drk,a.kode_akun,c.nama as nama_akun,a.nilai "+
							 ...
							 "				    inner join masakun c on a.kode_akun=c.kode_akun and a.kode_lokasi=c.kode_lokasi "+
							 "				    inner join drk d on a.kode_drk=d.kode_drk and a.kode_lokasi=d.kode_lokasi and d.tahun=substring(a.periode,1,4)  "+
							 "where a.no_pdrk='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.dc ='D' ";					
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg2.clear(1);
	...
	doHitungGar: function(){
		...
		for (var i=0;i < this.sgG.getRowCount();i++){						
			var data = this.dbLib.getDataProvider("select fn_cekaggBulan('"+this.sgG.cells(2,i)+"','"+this.app._lokasi+"','"+this.sgG.cells(0,i)+"','"+this.sgG.cells(4,i)+"','"+this.e_periode.getText().substr(0,4)+this.sgG.cells(9,i)+"','"+this.e_nb.getText()+"') as gar ",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line = data.rs.rows[0];
				data = line.gar.split(";");
