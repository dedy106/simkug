window.app_saku3_transaksi_optik_produksi_fNotaBatal = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_optik_produksi_fNotaBatal.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_optik_produksi_fNotaBatal";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pembatalan Nota", 0);	
		...
		this.pc2 = new pageControl(this,{bound:[10,10,1000,440], childPage:["Data Resep","List Resep"]});		
		...
		this.cb_akun = new saiCBBL(this.pc2.childPage[0],{bound:[20,15,220,20],caption:"Revers U. Muka", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});							
		...
		this.e_umuka = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,18,200,20],caption:"Uang Muka", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});			
		this.e_total = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,19,200,20],caption:"Total", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});			
		...
		try {
			...
			this.cb_akun.setSQL("select a.kode_akun, a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi where b.kode_flag in ('001','009','034') and a.kode_lokasi='"+this.app._lokasi+"'",["kode_akun","nama"],false,["Kode","Nama"],"and","Data Akun KasBank",true);
...
window.app_saku3_transaksi_optik_produksi_fNotaBatal.extend(window.childForm);
window.app_saku3_transaksi_optik_produksi_fNotaBatal.implement({
	isiCBresep: function() {
		var strSQL = "select a.no_bukti,a.no_dokumen from optik_pesan_m a "+					
					 "where a.periode<='"+this.e_periode.getText()+"' and progress in ('0','1') and umuka<>0 and a.kode_lokasi='"+this.app._lokasi+"'";
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','MI','NOTAREV','F','-','-','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','IDR',1,"+
							parseNilai(this.e_total.getText())+","+parseNilai(this.e_umuka.getText())+",0,'-','-','-','-','-','-','"+this.cb_resep.getText()+"','"+this.cb_akun.getText()+"','-')");
					...
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) "+
							"select '"+this.e_nb.getText()+"',kode_lokasi,getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"',no_dokumen,'"+this.dp_d1.getDateString()+"',nu,"+
							"case when jenis <> 'KB' then kode_akun else '"+this.cb_akun.getText()+"' end,"+
							"case dc when 'D' then 'C' else 'D' end,nilai,nilai_curr,'"+this.e_ket.getText()+"','MI',jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,'"+this.cb_resep.getText()+"',no_ref2,no_ref3 "+
							"from trans_j where no_bukti='"+this.cb_resep.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
							
					sql.add("insert into optik_pesan_bayar(no_bukti,no_resep,kode_lokasi,modul,periode,nik_user,tgl_input,dc,nilai) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_resep.getText()+"','"+this.app._lokasi+"','NOTAREV','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'D',"+this.pelunasan+")");																					
	...
	doChange:function(sender){		
		...
		if (sender == this.cb_resep || this.cb_resep.getText()!="") {
			var strSQL = "select a.no_bukti,a.no_dokumen,a.nik,a.pasien,a.umuka,(a.nilai_lensa+nilai_frame) as total,a.alamat,(a.nilai_lensa + nilai_frame - a.umuka) as sisa "+
						 ...
						 "where a.no_bukti='"+this.cb_resep.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
			...
			if (typeof data == "object"){
				...
				if (line != undefined){							
					...
					this.pelunasan = parseFloat(line.sisa);	
				}
			}
	...
	doLoad3:function(sender){		
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan, a.nilai1 "+
		             "from trans_m a "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.form = 'NOTAREV' and a.posted ='F'";
		var data = this.dbLib.getDataProvider(strSQL,true);
