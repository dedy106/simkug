window.app_saku3_transaksi_optik_produksi_fJualUmum = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_optik_produksi_fJualUmum.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_optik_produksi_fJualUmum";
		this.itemsValue = new portalui_arrayList();
		this.maximize();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pesanan Umum", 0);	
		...
		this.sg3 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:4,tag:9,
		            colTitle:["No Resep","Tanggal","No Nota","Total"],
					colWidth:[[3,2,1,0],[100,100,100,120]],
					readOnly:true,
					colFormat:[[3],[cfNilai]],
		...
		this.e_dok = new portalui_saiLabelEdit(this.pc2.childPage[0],{bound:[20,16,200,20],caption:"No Resep",maxLength:6,change:[this,"doChange"]});
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:6,tag:0,
		...
		try{			
			...
			this.sg.cells(0,0,"OD");
			this.sg.cells(0,1,"OS");
...
window.app_saku3_transaksi_optik_produksi_fJualUmum.extend(window.portalui_childForm);
window.app_saku3_transaksi_optik_produksi_fJualUmum.implement({		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{																									
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','AR','JUMUM','F','-','-','"+this.cb_pp.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_dok.getText()+"','IDR',1,"+
					...
					sql.add("insert into optik_pesan_m (no_bukti,tanggal,kode_lokasi,periode,kode_pp,no_dokumen,no_dok2,jenis,kode_mitra,nik,pasien,umur,alamat,no_hp,loker,band,lensa,frame,nilai_lensa,nilai_frame,bantu_len,bantu_fr,by_sendiri,umuka,progress,no_app,app_blen,app_bfr, no_tagih,n_tagih,akun_kas,,nik_user,tgl_input,no_ver,catatan) values "+
							"('"+this.e_nb.getText()+"','"+this.dp_d1.getDateString()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.cb_pp.getText()+"','"+this.e_dok.getText()+"','"+this.e_dok2.getText()+"','UMUM','-','-','"+this.e_pasien.getText()+"',"+parseNilai(this.e_umur.getText())+",'"+this.e_alamat.getText()+"','"+this.e_hp.getText()+"','-','-','"+this.e_lensa.getText()+"','"+this.e_frame.getText()+"',"+parseNilai(this.e_nlensa.getText())+","+parseNilai(this.e_nframe.getText())+",  0,0, "+parseNilai(this.e_bsendiri.getText())+","+parseNilai(this.e_umuka.getText())+", '0', '-',0,0,'-',"+parseNilai(this.e_sisa.getText())+",'"+this.cb_kas.getText()+"','"+this.app._userLog+"',getdate(),'-','"+this.e_catatan.getText()+"')");		

					if (this.sg.getRowValidCount() > 0){
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){																																										
								sql.add("insert into optik_pesan_d (no_bukti,kode_lokasi,jenis,sph,cyl,axis,rabun,pd) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.sg.cells(0,i)+"','"+this.sg.cells(1,i)+"','"+this.sg.cells(2,i)+"','"+this.sg.cells(3,i)+"','"+this.sg.cells(4,i)+"','"+this.sg.cells(5,i)+"')");
							}
						}						
					}	
		...
		switch (event){
			case "clear" :
				if (modalResult == mrOk) {
					...
					this.sg.clear(2); this.sg3.clear();
					this.sg.cells(0,0,"OD");
					this.sg.cells(0,1,"OS");
		...
		try {			
			...
			*/
			if ((sender == this.e_total) && this.e_total.getText()!="") {
				var tagih = nilaiToFloat(this.e_total.getText());
	...
	doLoad3:function(sender){			
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.nilai1,a.param1 as nik "+
					 "from trans_m a "+				     
					 "where a.modul='AR' and a.form='JUMUM' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.posted ='F' and a.kode_pp='"+this.cb_pp.getText()+"' "+
					 "order by a.no_bukti";			
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_bukti,line.tgl,line.no_dokumen,floatToNilai(line.nilai1)]); 
		}
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data1 == "object" && data1.rs.rows[0] != undefined){
					...
					for (var i in data1.rs.rows){
						...
						this.sg.appendData([line1.jenis,line1.sph,line1.cyl,line1.axis,line1.rabun,line1.pd]);
					}
