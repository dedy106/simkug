window.app_saku3_transaksi_sla_fRateJenis = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sla_fRateJenis.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sla_fRateJenis";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Jenis Rate", 0);	
...
window.app_saku3_transaksi_sla_fRateJenis.extend(window.childForm);
window.app_saku3_transaksi_sla_fRateJenis.implement({
	doTambah: function() {		
		this.stsSimpan = 1;
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into sla_rate_jenis(kode_rate,nama,kode_curr) values "+
							"('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.c_curr.getText()+"')");					
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();					
					sql.add("delete from sla_rate_jenis where kode_rate = '"+this.cb_kode.getText()+"' ");
					sql.add("insert into sla_rate_jenis(kode_rate,nama,kode_curr) values "+
							"('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.c_curr.getText()+"')");					
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select * from sla_rate_jenis where kode_rate ='"+this.cb_kode.getText()+"' ";						   
	...
	doLoad:function(sender){						
		var strSQL = "select kode_rate,nama "+
		             "from sla_rate_jenis "+
					 "order by kode_rate";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_rate,line.nama,"Pilih"]); 
		}
	...
	doCari: function (sender) {
		try {
			...
			var show = parseInt(this.c_show.getText());
			var column_array = ['kode_rate', 'nama'];
			...
			var strSQL = "select kode_rate, nama from sla_rate_jenis " +
						 "where " + filter_string + "  ";
