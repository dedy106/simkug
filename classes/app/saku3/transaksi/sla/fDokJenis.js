window.app_saku3_transaksi_sla_fDokJenis = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_sla_fDokJenis.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_sla_fDokJenis";
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-88],colCount:3,tag:9,
...
window.app_saku3_transaksi_sla_fDokJenis.extend(window.childForm);
window.app_saku3_transaksi_sla_fDokJenis.implement({
	doTambah: function() {		
		this.stsSimpan = 1;
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into sla_dok_jenis(kode_jenis,nama,kode_cocd) values "+
							"('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.cb_cocd.getText()+"')");					
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();					
					sql.add("delete from sla_dok_jenis where kode_jenis = '"+this.cb_kode.getText()+"'");
					sql.add("insert into sla_dok_jenis(kode_jenis,nama,kode_cocd) values "+
							"('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.cb_cocd.getText()+"')");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from sla_dok_jenis where kode_jenis = '"+this.cb_kode.getText()+"' and kode_cocd='"+this.cb_cocd.getText()+"'");								
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select * from sla_dok_jenis where kode_jenis ='"+this.cb_kode.getText()+"' and kode_cocd='"+this.cb_cocd.getText()+"'";						   
	...
	doLoad:function(sender){						
		if (this.cb_cocd.getText()!="") {
			var strSQL = "select kode_jenis,nama "+
						"from sla_dok_jenis where kode_cocd='"+this.cb_cocd.getText()+"' "+
						"order by kode_jenis";		
			var data = this.dbLib.getDataProvider(strSQL,true);		
	...
	doCari: function (sender) {
		try {
			...
			var strSQL = "select kode_jenis, nama from sla_dok_jenis " +
						 "where " + filter_string + " and kode_cocd= '" + this.cb_cocd.getText() + "' ";
