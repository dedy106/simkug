window.app_saku3_transaksi_dago_fReg = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_dago_fReg.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_dago_fReg";
		...
		this.e_total = new saiLabelEdit(this.pc1.childPage[1],{bound:[410,51,200,20],caption:"Total Biaya", tag:1,tipeText:ttNilai, text:"0",readOnly:true});
		...
		this.e_nopass = new saiLabelEdit(this.pc1.childPage[2],{bound:[20,36,260,20],caption:"No Passpord", tag:3});
		...
		this.cb_kerja = new portalui_saiCBBL(this.pc1.childPage[2],{bound:[20,16,280,20],caption:"Pekerjaan",rightLabelVisible:false,multiSelection:false,tag:3,change:[this,"doChange"]});		
		...
		this.pc2 = new pageControl(this.pc1.childPage[1],{bound:[5,14,990,255], childPage:["Biaya Tambahan"]});				
		this.sg4 = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:5,tag:9,
		            colTitle:["Kode","Nama Biaya","Tarif","Jumlah","Total"],
					colWidth:[[4,3,2,1,0],[100,80,100,200,100]],
					columnReadOnly:[true,[0,1,2,4]],
...
window.app_saku3_transaksi_dago_fReg.extend(window.childForm);
window.app_saku3_transaksi_dago_fReg.implement({
	doLoadParam:function(sender){
		var strSQL = "select kode_biaya, nama, nilai from dgw_biaya where kode_lokasi='"+this.app._lokasi+"'";					
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,3,4,5])){
				try{
					...
					sql.add("insert into dgw_reg(no_reg,tgl_input,no_peserta,no_paket,no_jadwal,no_agen,no_type,harga_room,info,kode_lokasi,no_quota,harga,uk_pakaian, no_marketing,kode_harga,periode, jenis,no_fee) values "+
							"('"+this.cb_kode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_peserta.getText()+"','"+this.cb_paket.getText()+"','"+this.cb_jadwal.rightLabelCaption+"','"+this.c_agen.getText()+"','"+this.c_room.getText()+"','"+this.e_harga_room.getText()+"','"+this.c_info.getText()+"','"+this.app._lokasi+"',"+this.e_quota.getText()+","+nilaiToFloat(this.e_harga.getText())+",'"+this.c_pakaian.getText()+"', '"+this.e_marketing.getText()+"','"+this.cb_jpaket.getText()+"','"+this.e_periode.getText()+"','"+this.c_jenis.getText()+"','"+noFee+"')");
						...
						for (var i=0;i < this.sg4.getRowCount();i++){
							if (this.sg4.rowValid(i)){
								sql.add("insert into dgw_reg_biaya(no_reg,kode_biaya,tarif,jml,nilai,kode_lokasi) values "+
										"('"+this.cb_kode.getText()+"','"+this.sg4.cells(0,i)+"',"+nilaiToFloat(this.sg4.cells(2,i))+",'"+this.sg4.cells(3,i)+"',"+nilaiToFloat(this.sg4.cells(4,i))+",'"+this.app._lokasi+"')");
							}
						}						
					}	
	...
	doClick:function(sender){
		if (sender == this.i_gen) {			
			this.cb_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"dgw_reg","no_reg",this.app._lokasi+"-RG"+this.e_periode.getText().substr(2,4)+".","0000"));
		...
		if (sender == this.i_gen2) {
			...
			this.cb_peserta.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"dgw_peserta","no_peserta",this.app._lokasi+"-"+this.tahun+".","00000"));
	...
	doChange: function(sender){		
		try{
			...
			if((sender == this.cb_paket || sender == this.cb_jpaket || sender == this.c_jenis) && this.cb_paket.getText() != "" && this.cb_jpaket.getText() != "" && this.c_jenis.getText() != "" ){
			...
			if ((sender == this.cb_jadwal || this.cb_paket ) && this.cb_jadwal.getText() != "" && this.cb_paket.getText() != "" ) {													
				this.doLoadPaket(); 
			...
			if (sender == this.c_room  && this.c_room.getText() != ""  && this.c_curr.getText() != ""){
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {			
				...
				var strSQL="select a.kode_harga,a.harga, a.harga_room,a.no_paket, a.no_jadwal, convert(varchar,b.tgl_berangkat,103) as tgl_jadwal, a.no_type, b.tgl_berangkat, b.lama_hari, a.uk_pakaian, a.no_peserta, a.no_agen,a.no_jadwal,a.no_paket, a.no_marketing, a.info, a.jenis  "+ 
						   "from dgw_reg a "+
				...
				var strSQL = "select a.kode_biaya,a.tarif,a.jml,a.nilai,b.nama "+
							 ...
							 "where a.no_reg = '"+this.cb_kode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.kode_biaya";					
