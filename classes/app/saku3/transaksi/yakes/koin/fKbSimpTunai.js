window.app_saku3_transaksi_yakes_koin_fKbSimpTunai = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes_koin_fKbSimpTunai.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes_koin_fKbSimpTunai";
		...
		this.e_total = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,18,200,20],caption:"Total Angsuran", tag:1, tipeText:ttNilai, readOnly:true, text:"0",change:[this,"doChange"]});						
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,20,990,305], childPage:["Data Akru","Data Angsuran","Jurnal Sisa"]});		
...
window.app_saku3_transaksi_yakes_koin_fKbSimpTunai.extend(window.childForm);
window.app_saku3_transaksi_yakes_koin_fKbSimpTunai.implement({
	doHitCD : function() {
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','KP','KBSIMP','F','0','0','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','-','"+this.e_ket.getText()+
							"','IDR',1,"+parseNilai(this.e_total.getText())+","+parseNilai(this.e_kas.getText())+","+parseNilai(this.e_cd.getText())+",'-','-','-','"+akunKB+"','-','-','"+this.cb_agg.getText()+"','"+this.c_jenis.getText()+"','-')");
						
					if (this.e_kas.getText() != "0") {					
						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',98,'"+this.cb_akun.getText()+"','D',"+nilaiToFloat(this.e_kas.getText())+","+nilaiToFloat(this.e_kas.getText())+",'"+this.e_ket.getText()+"','KBSIMP','KB','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");
					}
					if (this.e_cd.getText() != "0") {					
						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',99,'"+this.akunCD+"','D',"+nilaiToFloat(this.e_cd.getText())+","+nilaiToFloat(this.e_cd.getText())+",'"+this.e_ket.getText()+"','KBSIMP','CD','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");
						sql.add("insert into kop_cd_d (no_bukti,kode_lokasi,no_agg,periode,nilai,kode_akun,dc,modul,no_ref1) values "+
								"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_agg.getText()+"','"+this.e_periode.getText()+"',"+nilaiToFloat(this.e_cd.getText())+",'"+this.akunCD+"','C','KBSIMP','-')");								
					}
					for (var i=0;i < this.sg1.getRowCount();i++){
						if (this.sg1.rowValid(i) && this.sg1.cells(0,i) == "BAYAR"){
							var nilaiPiu = nilaiToFloat(this.sg1.cells(7,i));							
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.sg1.cells(2,i)+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg1.cells(5,i)+"','C',"+nilaiPiu+","+nilaiPiu+",'Pelunasan atas "+this.sg1.cells(2,i)+"','KBSIMP','AR','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");
							...
							sql.add("insert into kop_simpangs_d (no_angs,no_simp,no_bill,akun_piutang,nilai,kode_lokasi,dc,periode,modul,no_agg,jenis) values "+
									"('"+this.e_nb.getText()+"','"+this.sg1.cells(2,i)+"','"+this.sg1.cells(1,i)+"','"+this.sg1.cells(5,i)+"',"+nilaiPiu+",'"+this.app._lokasi+"','D','"+this.e_periode.getText()+"','SIMPTUNAI','"+this.cb_agg.getText()+"','SIMP')");																							
						}
					}	
					...
					if (this.sg.getRowValidCount() > 0) {
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								...
								sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+i+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(2,i).toUpperCase()+"',"+nilaiSlsIDR+","+nilaiSlsIDR+",'"+this.sg.cells(3,i)+"','KBSIMP','SLS','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");
								if (this.akunCD == this.sg.cells(0,i) && this.sg.cells(2,i).toUpperCase() == "C") {
									sql.add("insert into kop_cd_d (no_bukti,kode_lokasi,no_agg,periode,nilai,kode_akun,dc,modul,no_ref1) values "+
											"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_agg.getText()+"','"+this.e_periode.getText()+"',"+nilaiToFloat(this.sg.cells(4,i))+",'"+this.akunCD+"','D','KBSIMP','-')");								
								}							
							}
						}
					}										
	...
	doChange:function(sender){		
		try	{
			...
			if (sender == this.cb_agg && this.cb_agg.getText()!="") {		
				...
				var strSQL ="select isnull(sum(case dc when 'D' then nilai else -nilai end),0) as saldo_cd from kop_cd_d where no_bukti <> '"+this.e_nb.getText()+"' and no_agg='"+this.cb_agg.getText()+"' and  kode_lokasi='"+this.app._lokasi+"'";
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
	...
	doLoad3:function(sender){		
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan "+
		             "from trans_m a "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.form = 'KBSIMP' and a.posted ='F'";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
