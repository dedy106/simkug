window.app_saku3_transaksi_yakes_inves_fObligor = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes_inves_fObligor.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes_inves_fObligor";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Emiten", 0);	
		
		uses("saiCB;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		this.pc2 = new pageControl(this,{bound:[10,10,1000,450], childPage:["List Emiten","Data Emiten","Filter Cari"]});		
		this.sg3 = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:4,tag:9,
		            colTitle:["Kode","Nama","Universe","Pilih"],
		...
		this.c_jenis = new saiCB(this.pc2.childPage[1],{bound:[20,12,200,20],caption:"Jenis",items:["PEMERINTAH","KORPORASI"], readOnly:true,tag:2});		
		this.c_status = new saiCB(this.pc2.childPage[1],{bound:[20,14,200,20],caption:"Status",items:["KONVESIONAL","SYARIAH"], readOnly:true,tag:2});		
		this.c_flag = new saiCB(this.pc2.childPage[1],{bound:[20,15,200,20],caption:"Status Universe",items:["1. YA","0. TIDAK"], readOnly:true,tag:2});		
...
window.app_saku3_transaksi_yakes_inves_fObligor.extend(window.childForm);
window.app_saku3_transaksi_yakes_inves_fObligor.implement({	
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into inv_obligor(kode_obligor,nama,jenis,status,flag_uni,flag_aktif,gambar) values "+
						    "('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.c_jenis.getText()+"','"+this.c_status.getText()+"','"+this.c_flag.getText().substr(0,1)+"','"+this.c_aktif.getText().substr(0,1)+"','"+this.e_foto.getText()+"')");	
					setTipeButton(tbAllFalse);
	...
	doCari:function(sender){														
		var strSQL = "select kode_obligor,nama,case flag_uni when '0' then 'T' else 'Y' end as uni,flag_uni from inv_obligor where nama like '%"+this.e_nama2.getText()+"%' order by flag_uni desc,kode_obligor";
	...
	doLoad3:function(sender){														
		var strSQL = "select kode_obligor,nama,case flag_uni when '0' then 'T' else 'Y' end as uni,flag_uni from inv_obligor order by flag_uni desc,kode_obligor";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.page = 1;
			this.dataJU3 = data;
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.kode_obligor,line.nama,line.uni.toUpperCase(),"Pilih"]); 
		}
