window.app_saku3_transaksi_yakes_inves_fRDBonus = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes_inves_fRDBonus.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes_inves_fRDBonus";
...
window.app_saku3_transaksi_yakes_inves_fRDBonus.extend(window.childForm);
window.app_saku3_transaksi_yakes_inves_fRDBonus.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into inv_rdbeli_m(no_rdbeli,kode_lokasi,periode,tanggal,nik_user,tgl_input,posted,no_kas,progress,nik_buat,  no_dokumen,keterangan,kode_drk,kode_rdkelola,tgl_set,nilai_komisi,nilai_ppn,nilai_levy,nilai_pph,no_app1,no_spb,kode_pp,modul,nilai, nik_app,kode_plan) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.app._userLog+"',getdate(),'X','RDBONUS','1','"+this.app._userLog+"',  '-','"+this.e_ket.getText()+"','-','"+this.cb_kelola.getText()+"','"+this.dp_d1.getDateString()+"',0,0,0,0,'SAP','-','"+this.kodepp+"','RDBONUS',0,'-','"+this.cb_plan.getText()+"')");										
