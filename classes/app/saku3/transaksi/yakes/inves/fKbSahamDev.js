window.app_saku3_transaksi_yakes_inves_fKbSahamDev = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes_inves_fKbSahamDev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes_inves_fKbSahamDev";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Penerimaan Deviden Saham", 0);	
		...
		this.cb_akun = new saiCBBL(this.pc2.childPage[0],{bound:[20,19,220,20],caption:"Akun KasBank", multiSelection:false, maxLength:10, tag:2});						
		this.e_nilai = new saiLabelEdit(this.pc2.childPage[0],{bound:[770,19,200,20],caption:"Total Nilai", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
		this.bTampil = new button(this.pc2.childPage[0],{bound:[650,19,80,18],caption:"Data Deviden",click:[this,"doLoad"]});			
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:8,tag:9,				
					colTitle:["Status","No Deviden","Tanggal","Keterangan","Saham","MI","Nilai KB","Akun Piutang"],
					colWidth:[[7,6,5,4,3,2,1,0],[80,100,100,150,250,80,120,80]],				
...
window.app_saku3_transaksi_yakes_inves_fKbSahamDev.extend(window.childForm);
window.app_saku3_transaksi_yakes_inves_fKbSahamDev.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBSHMDEV','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.cb_sap.getText()+"',getdate(),'"+this.app._userLog+"','F','-','-','-','0')");										
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','09','-','"+this.app._lokasi+"','KBSHMDEV','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i) && this.sg.cells(0,i) == "APP"){
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) "+
										...
										" where a.kode_kelola='"+this.sg.cells(5,i)+"' and a.no_shmdev='"+this.sg.cells(1,i)+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.nilai_pph>0 "+
										" group by a.no_shmdev,a.akun_pph,b.kode_drk,a.kode_lokasi)");
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) "+
										"(select '"+this.e_nb.getText()+"',a.no_shmdev,'"+this.dp_d1.getDateString()+"',1,a.akun_piutang,'"+this.sg.cells(3,i)+"','C',sum(a.nilai_dev),b.kode_pp,b.kode_drk,'-','-',a.kode_lokasi,'KBSHMDEV','PIUTANG','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-' "+
										" from inv_shmdev_d a inner join inv_shmdev_m b on a.no_shmdev=b.no_shmdev and a.kode_lokasi=b.kode_lokasi "+
										" where a.kode_kelola='"+this.sg.cells(5,i)+"' and a.no_shmdev='"+this.sg.cells(1,i)+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.nilai_dev>0 "+
										" group by a.no_shmdev,a.akun_piutang,b.kode_pp,b.kode_drk,a.kode_lokasi)");
	...
	doLoad:function(sender){
		if (this.e_periode.getText() != "") {
			var strSQL = "select a.no_shmdev,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,a.kode_saham,b.kode_kelola,b.nilai_kb,b.akun_piutang "+
			             "from inv_shmdev_m a inner join inv_shmdev_d b on a.no_shmdev=b.no_shmdev and a.kode_lokasi=b.kode_lokasi "+
						 "where b.no_kas='-' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";												 
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData(["INPROG",line.no_shmdev,line.tanggal,line.keterangan,line.kode_saham,line.kode_kelola,floatToNilai(line.nilai_kb),line.akun_piutang]);
				}
	...
	doLoad3:function(sender){																											
		var strSQL = "select distinct a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.posted ='F' and a.modul = 'KBSHMDEV' "+
					 "order by a.no_kas";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.sg3.clear();
	...
	doDoubleClick3: function(sender, col , row) {		
		try{
			...
			if (this.sg3.cells(0,baris) != "") {
				...
				var strSQL = "select a.no_shmdev,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,a.kode_saham,b.kode_kelola,b.nilai_kb as nilai,b.akun_piutang "+						 
							 "from inv_shmdev_m a inner join inv_shmdev_d b on a.no_shmdev = b.no_shmdev "+
							 "where b.no_kas='"+this.e_nb.getText()+"' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";						
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg.appendData(["APP",line.no_shmdev,line.tanggal,line.keterangan,line.kode_saham,line.kode_kelola,floatToNilai(line.nilai),line.akun_piutang]);
					}
				} else this.sg.clear(1);			
				this.sg.validasi();
			}
