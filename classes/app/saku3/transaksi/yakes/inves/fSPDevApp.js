window.app_saku3_transaksi_yakes_inves_fSPDevApp = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes_inves_fSPDevApp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes_inves_fSPDevApp";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Approval Deviden Penyertaan -Akru", 0);	
		...
		this.cb_mitra = new saiCBBL(this.pc2.childPage[0],{bound:[20,12,230,20],caption:"Manajer Investasi", maxLength:10, tag:2,readOnly:true});						
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:7,tag:0,				
				colTitle:["Kode Mitra","Nama","Jumlah","Dev / Lbr","Nilai Deviden","Nilai PPH","Nilai KasBank"],
				colWidth:[[6,5,4,3,2,1,0],[100,100,100,150,150,240,80]],				
...
window.app_saku3_transaksi_yakes_inves_fSPDevApp.extend(window.childForm);
window.app_saku3_transaksi_yakes_inves_fSPDevApp.implement({
	isiCBAkru: function() {
		this.cb_akru.setSQL("select no_spdev, keterangan from inv_spdev_m where periode<='"+this.e_periode.getText()+"' and no_final='-' and kode_lokasi='"+this.app._lokasi+"'",["no_spdev","keterangan"],false,["No Bukti","Deskripsi"],"and","Data Akru",true);								
	},
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
					  sql.add("update inv_spdev_m set no_final='-' where no_spdev='"+this.cb_akru.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");									
					...
					//jurnal balik
					sql.add("update inv_spdev_m set no_final='"+this.e_nb.getText()+"' where no_spdev='"+this.cb_akru.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");									
					
					sql.add("insert into inv_spdev_d(no_spdev,kode_lokasi,kode_mitra,nilai_kb,nilai_pph,nilai_dev,akun_dev,akun_pph,no_kas,akun_piutang,jum_lbr,dev_lbr,kode_plan) "+
							"select '"+this.e_nb.getText()+"',kode_lokasi,kode_mitra,-nilai_kb,-nilai_pph,-nilai_dev,akun_dev,akun_pph,'X',akun_piutang,-jum_lbr,dev_lbr,kode_plan "+
							"from inv_spdev_d where no_spdev='"+this.cb_akru.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("insert into inv_spdev_j(no_spdev,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) "+
							...
							"from inv_spdev_j where no_spdev='"+this.cb_akru.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					for (var i=0;i < this.sg.getRowCount();i++){
						if (this.sg.rowValid(i) && this.sg.cells(2,i) != "0"){															
							sql.add("insert into inv_spdev_d(no_spdev,kode_lokasi,kode_mitra,nilai_kb,nilai_pph,nilai_dev,akun_dev,akun_pph,no_kas,akun_piutang,kode_plan,jum_lbr,dev_lbr) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.sg.cells(0,i)+"',"+nilaiToFloat(this.sg.cells(6,i))+","+nilaiToFloat(this.sg.cells(5,i))+","+nilaiToFloat(this.sg.cells(4,i))+",'"+this.akunDev+"','"+this.akunPPh+"','-','"+this.akunPiuGL+"','"+this.cb_plan.getText()+"',"+nilaiToFloat(this.sg.cells(2,i))+","+nilaiToFloat(this.sg.cells(3,i))+")");
							sql.add("insert into inv_spdev_j(no_spdev,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
									"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+this.akunDev+"','"+this.e_ket.getText()+" - "+this.sg.cells(0,i)+"','C',"+nilaiToFloat(this.sg.cells(4,i))+",'"+this.kodepp+"','"+this.cb_drk.getText()+"','"+this.app._lokasi+"','APPDEV','DEVIDEN','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");	
	...
	doClick:function(sender){
		if (sender == this.i_gen) {			
			...
			setTipeButton(tbSimpan);
			this.stsSimpan = 1;			
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"inv_spdev_m","no_spdev",this.app._lokasi+"-APDV"+this.e_periode.getText().substr(2,4)+".","0000"));
			this.e_dok.setFocus();
		}	
	},		
	doChange: function(sender) {
		...
		if (sender == this.cb_akru && this.cb_akru.getText()!="" && this.stsSimpan==1) {			
			var strSQL = "select * from inv_spdev_m where no_spdev= '"+this.cb_akru.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";			
			...
			var strSQL = "select b.kode_mitra,b.nama,a.nilai_kb,a.nilai_pph,a.nilai_dev,a.jum_lbr,a.dev_lbr "+
						 ...
						 "where a.no_spdev='"+this.cb_akru.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";						
			var data = this.dbLib.getDataProvider(strSQL,true);
		...
		try{
			...
			if (this.sg3.cells(0,baris) != "") {			
				...
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){		
						this.cb_akru.setSQL("select no_spdev, keterangan from inv_spdev_m where no_spdev='"+line.no_final+"' and kode_lokasi='"+this.app._lokasi+"'",["no_spdev","keterangan"],false,["No Bukti","Deskripsi"],"and","Data Akru",true);								
				...
				var strSQL = "select b.kode_mitra,b.nama,a.nilai_kb,a.nilai_pph,a.nilai_dev,a.jum_lbr,a.dev_lbr "+
							 "from inv_spdev_d a inner join inv_mitra b on a.kode_mitra=b.kode_mitra "+
							 "where a.no_spdev='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.jum_lbr>0";						
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
