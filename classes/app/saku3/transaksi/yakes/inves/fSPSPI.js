window.app_saku3_transaksi_yakes_inves_fSPSPI = function(owner)
{
	if (owner)
	{		
		window.app_saku3_transaksi_yakes_inves_fSPSPI.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes_inves_fSPSPI";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Perhitungan SPI Saham Penyertaan", 0);	
		...
		this.cb_sap = new saiCBBL(this.pc2.childPage[0],{bound:[20,18,220,20],caption:"NIK Post SAP", multiSelection:false, maxLength:10, tag:2, visible:false});		
		this.cb_drk = new saiCBBL(this.pc2.childPage[0],{bound:[20,18,220,20],caption:"DRK SPI", multiSelection:false, maxLength:10, tag:2,visible:false });		
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,22,996,305], childPage:["Data Penyertaan","Rekap Data","Data Closing"]});		
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:10,tag:9,
		      		colTitle:["Kode Mitra","Nama","Akun SPI","Jumlah","Nilai Oleh","Nilai Buku","Nilai Wajar","Selisih","Total","Rev SPI"],
					colWidth:[[9,8,7,6,5,4,3,2,1,0],[100,100,100,100,100,100,100,60,180,80]],
...
window.app_saku3_transaksi_yakes_inves_fSPSPI.extend(window.childForm);
window.app_saku3_transaksi_yakes_inves_fSPSPI.implement({
	doHitung: function() {
		var strSQL = "select a.kode_mitra,a.h_wajar from inv_sp_harga a inner join inv_mitra b on a.kode_mitra=b.kode_mitra where a.tanggal ='"+this.dp_d2.getDateString()+"' order by a.kode_mitra";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			...
			for (var i=0; i < this.dataJU4.rs.rows.length;i++){
				...
				for (var j=0;j < this.dataJU.rs.rows.length;j++){
					if (line.kode_mitra == this.dataJU.rs.rows[j].kode_mitra) {							
	...
	doTampilData4: function(page) {		
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU4.rs.rows[i];													
			this.sg4.appendData([line.kode_mitra,floatToNilai(line.h_wajar)]); 
		}
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					if (this.stsSimpan == 0) {
						sql.add("delete from inv_spspi_m where no_spi='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from inv_spspi_j where no_spi='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from inv_spspi_d where no_spi='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("update inv_spspi_d set flag_rev = '-' where flag_rev='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
						sql.add("delete from glsap where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");		
					}
					
					sql.add("insert into inv_spspi_m(no_spi,no_dokumen,tanggal,keterangan,kode_curr,kurs,nilai,kode_pp,kode_drk,posted,modul,nik_buat,nik_setuju,kode_lokasi,periode,no_del,no_link,nik_user,tgl_input,jenis,no_spikug,no_app1,progress,kode_plan) values "+
						    "('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.kodepp+"','"+this.cb_drk.getText()+"','F','SPSPI','"+this.app._userLog+"','"+this.cb_sap.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','-','-','"+this.app._userLog+"',getdate(),'"+this.c_jenis.getText()+"','-','-','0','"+this.cb_plan.getText()+"')");
					...
					if (this.sg2.getRowValidCount() > 0){
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i)){
								...
								var k = 2+n;
								if (nilai != 0) {
									sql.add("insert into inv_spspi_j(no_spi,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
											"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+j+",'"+akunDr+"','"+this.e_ket.getText()+"','D',"+nilai+",'"+this.kodepp+"','"+this.cb_drk.getText()+"','"+this.app._lokasi+"','SPSPI','"+jenis1+"','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");					
									sql.add("insert into inv_spspi_j(no_spi,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
											"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+k+",'"+akunCr+"','"+this.e_ket.getText()+"','C',"+nilai+",'"+this.kodepp+"','"+this.cb_drk.getText()+"','"+this.app._lokasi+"','SPSPI','"+jenis2+"','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");								
								}
					...
					if (this.e_periode.getText().substr(4,2) != "01") {
						sql.add("insert into inv_spspi_d(no_spi,kode_lokasi,kode_spkelola,kode_mitra,jumlah,h_oleh,h_buku,h_wajar,flag_rev,dc,kode_plan) "+					
								"select '"+this.e_nb.getText()+"','"+this.app._lokasi+"',kode_spkelola,kode_mitra,jumlah,h_oleh,h_buku,h_wajar,no_spi,'C',kode_plan from inv_spspi_d where flag_rev='-' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("update inv_spspi_d set flag_rev='"+this.e_nb.getText()+"' where flag_rev='-' and kode_lokasi='"+this.app._lokasi+"' and kode_plan='"+this.cb_plan.getText()+"'");					
					}
					else {
						sql.add("insert into inv_spspi_d(no_spi,kode_lokasi,kode_spkelola,kode_mitra,jumlah,h_oleh,h_buku,h_wajar,flag_rev,dc,kode_plan) "+					
								"select '"+this.e_nb.getText()+"','"+this.app._lokasi+"',a.kode_spkelola,a.kode_mitra,a.jumlah,a.h_oleh,a.h_buku,a.h_wajar,a.no_spi,'C',a.kode_plan "+
								"from inv_spspi_d a inner join inv_spspi_m b on a.no_spi=b.no_spi and b.jenis = 'NBUKU' "+
								"where a.dc = 'D' and a.no_spi = (select MAX(no_spi) from inv_spspi_m where jenis='NBUKU') ");
					}
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						line = this.dataJU.rs.rows[i];						
						...
						sql.add("insert into inv_spspi_d(no_spi,kode_lokasi,kode_spkelola,kode_mitra,jumlah,h_oleh,h_buku,h_wajar,flag_rev,dc,kode_plan) values "+								
					            "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','-','"+line.kode_mitra+"',"+line.jumlah+","+h_oleh+","+h_buku+","+h_wajar+",'"+flag_rev+"','D','"+this.cb_plan.getText()+"')");
					}	
	...
	doChange:function(sender){
		if (sender == this.cb_plan && this.cb_plan.getText()!="") {
			var data = this.dbLib.getDataProvider("select kode_param,flag from inv_sp_param where kode_plan = '"+this.cb_plan.getText()+"' and kode_param in ('SPNT','SPDRKSPI','PPINV')",true);			
			if (typeof data == "object"){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];	
					...
					if (line.kode_param == "SPDRKSPI") this.cb_drk.setText(line.flag);							
				}
			}			
		}
	...
	doClick:function(sender){
		if (sender == this.i_gen) {			
			...
			setTipeButton(tbSimpan);
			this.stsSimpan = 1;			
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"inv_spspi_m","no_spi",this.app._lokasi+"-SPSPI"+this.e_periode.getText().substr(2,4)+".","000"));
			this.e_ket.setFocus();
		}
	},
	doLoadData: function(sender){
		...
		// var sql = "call sp_get_hspmitra ('"+this.cb_plan.getText()+"','"+this.e_periode.getText()+"','"+this.nik_user+"','"+this.e_nb.getText()+"')";								
		// this.dbLib.execQuerySync(sql);	
		
		this.e_nilai.setText("0");		
		var strSQL = "select x.akun_spi,a.kode_mitra,x.nama,a.jumlah,a.h_oleh,a.h_buku,a.h_oleh * a.jumlah as nilai_oleh,a.h_buku * a.jumlah as nilai_buku,0 as nilai_wajar,0 as nilai_sls,0 as total,isnull(b.nilai_spi,0) as total_spi "+		        
					// "from inv_sp_tmp a inner join inv_mitra x on a.kode_mitra=x.kode_mitra "+
					"from inv_sp_kkp a inner join inv_mitra x on a.kode_mitra=x.kode_mitra "+
					 "       left join ( "+
					 "       select x.kode_plan,x.kode_mitra,round(sum( (case x.dc when 'D' then (x.h_wajar-x.h_buku) else -(x.h_wajar-x.h_buku) end) * x.jumlah),0) as nilai_spi "+
					 "       from inv_spspi_d x inner join inv_spspi_m y on x.no_spi=y.no_spi and x.kode_lokasi=y.kode_lokasi "+
					 "       where y.periode<='"+this.e_periode.getText()+"' and x.flag_rev='-' group by x.kode_mitra,x.kode_plan "+
					 ") b on a.kode_mitra=b.kode_mitra and a.kode_plan=b.kode_plan "+
					 ...
					 "order by a.kode_mitra"; //where a.jumlah <> 0 --> yg sudah dijual di reverse meski jmlnya sudah nol		
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
			var line;
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData([line.kode_mitra,line.nama,line.akun_spi,floatToNilai(jml),floatToNilai(noleh),floatToNilai(nbuku),floatToNilai(line.nilai_wajar),floatToNilai(line.nilai_sls),floatToNilai(line.total),floatToNilai(line.total_spi)]);								   
		}
	...
	doLoad3:function(sender){														
		var strSQL = "select a.no_spi,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
		             "from inv_spspi_m a "+					 
					 "     left join (select distinct no_spi from inv_spspi_d where dc ='D' and flag_rev<>'-' and kode_lokasi='"+this.app._lokasi+"') c on a.no_spi=c.no_spi "+
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			...
			if (this.sg3.cells(0,baris) != "") {			
				...
				var strSQL = "select * from inv_spspi_m where no_spi= '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){																			
				...
				var strSQL = "select x.akun_spi,a.kode_mitra,x.nama,a.jumlah,a.h_oleh,a.h_buku,a.h_oleh * a.jumlah as nilai_oleh,a.h_buku * a.jumlah as nilai_buku,round(a.h_wajar * a.jumlah,0) as nilai_wajar,round((a.h_wajar-a.h_buku) * a.jumlah,0) as nilai_sls,round((a.h_wajar-a.h_buku) * a.jumlah,0) as total,isnull(b.nilai_spi,0) as total_spi "+		        
						 	 "from inv_spspi_d a inner join inv_mitra x on a.kode_mitra=x.kode_mitra "+
							 "       left join ( "+
							 "       select x.kode_plan,x.kode_mitra,round(sum( (case x.dc when 'D' then (x.h_wajar-x.h_buku) else -(x.h_wajar-x.h_buku) end) * x.jumlah),0) as nilai_spi "+
							 "       from inv_spspi_d x inner join inv_spspi_m y on x.no_spi=y.no_spi and x.kode_lokasi=y.kode_lokasi "+
							 "       where y.no_spi<>'"+this.e_nb.getText()+"' and y.periode<='"+this.e_periode.getText()+"' and x.flag_rev='"+this.e_nb.getText()+"' group by x.kode_plan,x.kode_mitra "+
							 ") b on a.kode_mitra=b.kode_mitra and a.kode_plan=b.kode_plan "+
					 		 "where a.dc='D' and a.no_spi='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.kode_mitra";
