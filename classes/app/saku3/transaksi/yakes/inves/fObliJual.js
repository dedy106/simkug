window.app_saku3_transaksi_yakes_inves_fObliJual = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes_inves_fObliJual.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes_inves_fObliJual";
		...
		this.sg4 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:7,tag:9,
					colTitle:["No Seri","ISIN","Nama Seri","Tgl Settlmt","No Pembelian","Nominal","Pilih"],
					colWidth:[[6,5,4,3,2,1,0],[70,100,100,80,350,100,100]],
					colFormat:[[5,6],[cfNilai,cfButton]],readOnly:true,
					click:[this,"doSgBtnClick4"], colAlign:[[6],[alCenter]],
		...
		this.e_dpp = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,22,200,20],caption:"Taxbl Cpt Gain", tipeText:ttNilai, text:"0",change:[this,"doChange"]});				
		this.e_pajak = new saiLabelEdit(this.pc1.childPage[1],{bound:[270,22,200,20],caption:"Tax On Cpt Gain", tipeText:ttNilai, text:"0",change:[this,"doChange"]});			
...
window.app_saku3_transaksi_yakes_inves_fObliJual.extend(window.childForm);
window.app_saku3_transaksi_yakes_inves_fObliJual.implement({	
	doSgBtnClick4: function(sender, col, row){
		try{
			if (col == 6) this.doDoubleClick4(this.sg4,0,row); 				
		}catch(e){
			alert(e);
		}
	},
	doDoubleClick4: function(sender, col , row) {
		...
		var strSQL = "select a.*,a.nilai_buku+isnull(d.amor,0) as nilai_buku,b.nama as nama_seri,convert(varchar,b.tgl_mulai,103) as tglmulai,convert(varchar,b.tgl_selesai,103) as tglselesai,c.nama as obligor "+
					 ...
					 "where a.no_beli='"+this.noBeli+"' and a.status='TRADING'";
		...
		if (typeof data == "object"){
			...
			if (line != undefined){		
				...
				if (this.stsSimpan==1) {
					this.e_pharga2.setText(floatToNilai(line.p_price)); //default
					this.niPiuKupon = 0; // dijual sblm kupon saat beli didapat
					this.niGainLos = 0;  // gainlos dr selisih njual - noleh(afs)/nbuku(htm)
				}
	...
	doLoadSeri: function() {
		var strSQL = "select b.kode_jenis,b.isin,b.nama,convert(varchar,c.tgl_settl,103) as tgl_set, a.no_beli,a.nilai "+
					 "from inv_obli_d a "+
					 "inner join inv_oblijenis b on a.kode_jenis=b.kode_jenis "+
		...
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			...
			for (var i in data.rs.rows){
				line = data.rs.rows[i];							
				this.sg4.appendData([line.kode_jenis,line.isin,line.nama,line.tgl_set,line.no_beli,floatToNilai(line.nilai),"Pilih"]);						
			}
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into inv_oblijual_m(no_oblijual,kode_lokasi,periode,tanggal,nik_user,tgl_input,posted,no_kasjual,nik_buat,no_dokumen,keterangan,kode_drk,kode_jenis,akun_piutang,akun_piukupon,kode_plan,tgl_settl,kode_rdkelola) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.app._userLog+"',getdate(),'X','-','"+this.app._userLog+"','-','"+this.e_ket.getText()+"','-','"+this.cb_seri.getText()+"','-','-','"+this.cb_plan.getText()+"','"+this.dp_d2.getDateString()+"','"+this.cb_kelola.getText()+"')");					

					sql.add("insert into inv_oblijual_d (no_oblijual,kode_jenis,no_beli,n_oleh,n_buku,n_piukupon,n_jual,gainlos,n_kupon,tgl_kupon,p_price,p_price2,dpp,pajak,kode_broker) values "+
					 		"('"+this.e_nb.getText()+"','"+this.cb_seri.getText()+"','"+this.noBeli+"',"+this.niOleh+","+this.niBuku+","+this.niPiuKupon+","+nilaiToFloat(this.e_nilaijual.getText())+","+this.niGainLos+","+nilaiToFloat(this.e_piukupon.getText())+",'"+this.dp_d3.getDateString()+"',"+nilaiToFloat(this.e_pharga1.getText())+","+nilaiToFloat(this.e_pharga2.getText())+","+nilaiToFloat(this.e_dpp.getText())+","+nilaiToFloat(this.e_pajak.getText())+",'"+this.cb_broker.getText()+"')");
	...
	doChange:function(sender){		
		...
		if (sender == this.e_pharga1 || sender == this.e_pharga2 || sender == this.e_nilai) {
			if (this.e_pharga1.getText()!="" && this.e_pharga2.getText()!="" && this.e_nilai.getText()!="") {
				if (nilaiToFloat(this.e_pharga2.getText()) > nilaiToFloat(this.e_pharga1.getText())) {
					var dpp = ((nilaiToFloat(this.e_pharga2.getText()) - nilaiToFloat(this.e_pharga1.getText())) /100) * nilaiToFloat(this.e_nilai.getText());
					this.e_dpp.setText(floatToNilai(Math.round(dpp)));
					this.e_dpp.setReadOnly(false);
					this.e_pajak.setReadOnly(false);
				}	
				else {
