window.app_saku3_transaksi_yakes_inves_fKbRDDev = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes_inves_fKbRDDev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes_inves_fKbRDDev";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Deviden Final", 0);	
		...
		this.cb_sap = new saiCBBL(this.pc2.childPage[0],{bound:[20,15,220,20],caption:"NIK Post SAP", multiSelection:false, maxLength:10, tag:2,visible:false});			
		...
		this.bTampil = new button(this.pc2.childPage[0],{bound:[670,15,80,18],caption:"Data Deviden",click:[this,"doLoad"]});			
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:6,tag:9,				
					...
					colWidth:[[5,4,3,2,1,0],[100,250,300,80,120,80]],				
...
window.app_saku3_transaksi_yakes_inves_fKbRDDev.extend(window.childForm);
window.app_saku3_transaksi_yakes_inves_fKbRDDev.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBRDDEV','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.cb_sap.getText()+"',getdate(),'"+this.app._userLog+"','F','-','-','-','0')");																	
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i) && this.sg.cells(0,i) == "APP"){								
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) "+
										...
										" where a.nilai_pph<>0 and a.no_rddev='"+this.sg.cells(1,i)+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.nilai_pph >0  "+
								...
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) "+
										...
										" from inv_rddev_d a inner join inv_rddev_m b on a.no_rddev=b.no_rddev and a.kode_lokasi=b.kode_lokasi "+
										" where a.no_rddev='"+this.sg.cells(1,i)+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.nilai_dev >0 "+
	...
	doLoad3:function(sender){																											
		var strSQL = "select distinct a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
					 ...
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.posted ='F' and a.modul = 'KBRDDEV' "+
					 "order by a.no_kas";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.sg3.clear();
