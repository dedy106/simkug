window.app_saku3_transaksi_yakes_inves_fSahamPindahMI = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes_inves_fSahamPindahMI.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes_inves_fSahamPindahMI";
		...
		this.e_nsaham = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,18,200,20],caption:"Nilai Saham", tag:1,  tipeText:ttNilai, text:"0",readOnly:true});		 
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,22,996,282], childPage:["Data Saham","File Dok"]});		
...
window.app_saku3_transaksi_yakes_inves_fSahamPindahMI.extend(window.childForm);
window.app_saku3_transaksi_yakes_inves_fSahamPindahMI.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{																			
					...
					sql.add("insert into inv_shmbeli_m(no_shmbeli,kode_lokasi,periode,tanggal,nik_user,tgl_input,posted,no_kas,progress,nik_buat,  no_dokumen,keterangan,kode_drk,kode_kelola,tgl_set,nilai_komisi,nilai_ppn,nilai_levy,nilai_pph,modul,nilai,no_app1,no_spb,kode_pp,kode_plan) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.app._userLog+"',getdate(),'X','-','X','"+this.app._userLog+"',  '"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','-','-','"+this.dp_d1.getDateString()+"',0,0,0,0,'BPINDAH',"+nilaiToFloat(this.e_nsaham.getText())+",'SAP','-','"+this.kodepp+"','"+this.cb_plan.getText()+"')");
							
					for (var i = 0; i < this.sg.rows.getLength();i++){
						if (this.sg.rowValid(i)){
							if (nilaiToFloat(this.sg.cells(5,i)) != 0 ) {
								var strSQL = "select kode_saham from inv_saham_d where kode_plan='"+this.cb_plan.getText()+"' and kode_saham='"+this.sg.cells(0,i)+"' and kode_kelola='"+this.sg.cells(6,i)+"'";
								var data = this.dbLib.getDataProvider(strSQL,true);
								if (typeof data == "object" && data.rs.rows[0] == undefined){
									sql.add("insert into inv_saham_d (kode_plan,kode_saham,kode_kelola) values "+
											"('"+this.cb_plan.getText()+"','"+this.sg.cells(0,i)+"','"+this.sg.cells(6,i)+"')");
								}						

								//harga jual yg pakai adalah harga perolehan rata2 dr yg barangnya keluar(jual)
								//harga oleh/buu asal masuk ke harga oleh/buku baru
								...
								sql.add("insert into inv_shmjual_d (no_shmjual,kode_kelola,kode_saham,h_oleh,h_buku,h_jual,jumlah,n_jual,gainlos, komisi,vat,levi,pph,kode_broker,kode_plan) values "+
								        "('"+this.e_nb.getText()+"','"+this.cb_kelola.getText()+"','"+this.sg.cells(0,i)+"',"+nilaiToFloat(this.sg.cells(3,i))+","+nilaiToFloat(this.sg.cells(4,i))+","+nilaiToFloat(this.sg.cells(3,i))+","+nilaiToFloat(this.sg.cells(5,i))+","+njual+",0,0,0,0,0,'"+this.sg.cells(8,i)+"','"+this.cb_plan.getText()+"')");																
								...
								sql.add("insert into inv_shmbeli_d (no_shmbeli,kode_kelola,kode_saham,h_oleh,h_buku,harga,jumlah,n_beli,  komisi,vat,levi,pph,kode_broker,h_buku2,kode_plan) values "+
								        "('"+this.e_nb.getText()+"','"+this.sg.cells(6,i)+"','"+this.sg.cells(0,i)+"',"+nilaiToFloat(this.sg.cells(3,i))+","+nilaiToFloat(this.sg.cells(4,i))+","+nilaiToFloat(this.sg.cells(3,i))+","+nilaiToFloat(this.sg.cells(5,i))+","+nbeli+",0,0,0,0,'"+this.sg.cells(8,i)+"',"+nilaiToFloat(this.sg.cells(4,i))+",'"+this.cb_plan.getText()+"')");																
								...
								sql.add("insert into inv_shmpindah_d (no_bukti,kode_lokasi,kode_kelola_asal,kode_kelola_tuj,kode_saham,jumlah,kode_broker,kode_plan) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.cb_kelola.getText()+"','"+this.sg.cells(6,i)+"','"+this.sg.cells(0,i)+"',"+nilaiToFloat(this.sg.cells(5,i))+",'"+this.sg.cells(8,i)+"','"+this.cb_plan.getText()+"')");
