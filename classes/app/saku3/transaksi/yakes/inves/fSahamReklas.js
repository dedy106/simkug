window.app_saku3_transaksi_yakes_inves_fSahamReklas = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes_inves_fSahamReklas.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes_inves_fSahamReklas";
...
window.app_saku3_transaksi_yakes_inves_fSahamReklas.extend(window.childForm);
window.app_saku3_transaksi_yakes_inves_fSahamReklas.implement({
	doLoadData: function() {
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into inv_shmreklas_m(no_shmreklas,kode_lokasi,periode,tanggal,nik_user,tgl_input,posted,no_dokumen,keterangan,kode_drk,kode_kelola,nilai,nilai_spi,modul,kode_pp,no_app1,progress,nik_app,kode_plan) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.app._userLog+"',getdate(),'F','-','"+this.e_ket.getText()+"','"+this.cb_drk.getText()+"','"+this.cb_kelola.getText()+"',"+nilaiToFloat(this.e_nilai.getText())+","+nilaiToFloat(this.e_spi.getText())+",'SHMREKLAS','"+this.kodepp+"','-','0','"+this.cb_sap.getText()+"','"+this.cb_plan.getText()+"')");
					...
					sql.add("insert into glsap(no_bukti,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,kode_curr,kurs,nilai_curr,tgl_input,nik_user,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,no_doksap,kode_rek,no_payment,paymetod)  "+
							"select no_shmreklas,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,'IDR',1,nilai,tgl_input,nik_user,'-','-','-','-','-','-','-','-','-','-' "+
							"from inv_shmreklas_j where no_shmreklas='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
