window.app_saku3_transaksi_yakes_inves_fRoiData = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes_inves_fRoiData.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes_inves_fRoiData";
...
window.app_saku3_transaksi_yakes_inves_fRoiData.extend(window.childForm);
window.app_saku3_transaksi_yakes_inves_fRoiData.implement({
	getData: function() {	
				...
				try{														
					...
					for (var i=0;i < this.sg.getRowCount();i++){
						if (this.sg.rowValid(i)) {		
							var tanggal = this.sg.cells(0,i).substr(0,4)+"-"+this.sg.cells(0,i).substr(4,2)+"-"+this.sg.cells(0,i).substr(6,2);														
							var jenis = this.sg.cells(1,i);
							if (jenis == "51030101" || jenis == "51030102" || jenis == "51030103" || jenis == "51030105") jenis = "BINVES";

							sql.add("insert into inv_cashout_tgl (periode,tanggal,kode_akun,jenis,nilai) values "+
									"('"+this.c_periode.getText()+"','"+tanggal+"','"+this.sg.cells(1,i)+"','"+jenis+"',"+nilaiToFloat(this.sg.cells(2,i))+")");
						}	
					}
