window.app_saku3_transaksi_yakes_inves_fObliBeli = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes_inves_fObliBeli.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes_inves_fObliBeli";
...
window.app_saku3_transaksi_yakes_inves_fObliBeli.extend(window.childForm);
window.app_saku3_transaksi_yakes_inves_fObliBeli.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into inv_oblibeli_m(no_beli,kode_lokasi,periode,tanggal,nik_user,tgl_input,posted,no_kas,progress,nik_buat,no_dokumen,keterangan,kode_pp,kode_drk,kode_obligor,kode_rdkelola,kode_plan,tgl_settl,market,tgl_kupon_last) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.app._userLog+"',getdate(),'F','"+this.e_nb.getText()+"','0','"+this.app._userLog+"','-','"+this.e_ket.getText()+"','"+this.app._kodePP+"', '-','"+this.kodeObligor+"','"+this.cb_kelola.getText()+"','"+this.cb_plan.getText()+"','"+this.dp_d2.getDateString()+"','"+this.c_market.getText()+"','"+this.dp_d3.getDateString()+"')");					
					...
					var nilai = nilaiToFloat(this.e_total.getText()) + nilaiToFloat(this.e_pjkkupon.getText());
					sql.add("insert into inv_oblibeli_j(no_beli,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,kode_curr,kurs) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',2,'"+this.akunHutang+"','"+this.e_ket.getText()+"','C',"+nilai+",'"+this.kodepp+"','-','"+this.app._lokasi+"','OBLBELI','HUTANG','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'IDR',1)");												
					...
					sql.add("insert into glsap(no_bukti,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,kode_curr,kurs,nilai_curr,tgl_input,nik_user,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,no_doksap,kode_rek,no_payment,paymetod) "+
							"select no_beli,no_urut,kode_lokasi,'OBLIBELI',jenis,'-',tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,'IDR',1,nilai,getdate(),'"+this.app._userLog+"','-','-','-',no_dokumen,'-','-','-',case when no_dokumen = '-' then '-' else '"+this.rekSAP+"' end,'-',case when no_dokumen = '-' then '-' else 'T' end "+
						    "from inv_oblibeli_j "+
							"where nilai <> 0 and kode_lokasi='"+this.app._lokasi+"' and no_beli='"+this.e_nb.getText()+"'");				
