window.app_saku3_transaksi_yakes_inves_fSahamRev = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes_inves_fSahamRev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes_inves_fSahamRev";
...
window.app_saku3_transaksi_yakes_inves_fSahamRev.extend(window.childForm);
window.app_saku3_transaksi_yakes_inves_fSahamRev.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into inv_shmjual_m(no_shmjual,kode_lokasi,periode,tanggal,nik_user,tgl_input,posted,no_kasjual,nik_buat,no_dokumen,keterangan,kode_drk,kode_kelola,tgl_set,nilai_komisi,nilai_ppn,nilai_levy,nilai_pph,akun_piutang,akun_piugl,nilai_piutang,nilai_piugl,modul,kode_pp,no_app1,progress,kode_plan) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.app._userLog+"',getdate(),'F','-','"+this.app._userLog+"',  '-','"+this.e_ket.getText()+"','-','"+this.cb_saham.getText()+"','"+this.dp_d1.getDateString()+"',"+nilaiToFloat(this.e_bagi.getText())+",0,0,0,'-','-',0,0,'SHMREV','"+this.kodepp+"','SAP','1','"+this.cb_plan.getText()+"')");
