window.app_saku3_transaksi_yakes_inves_fSahamDevApp = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes_inves_fSahamDevApp.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes_inves_fSahamDevApp";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Approval Deviden Saham -Akru", 0);	
		...
		this.cb_sap = new saiCBBL(this.pc2.childPage[0],{bound:[320,13,230,20],caption:"NIK Post SAP", multiSelection:false, maxLength:10, tag:2,visible:false});								
		this.cb_drk = new saiCBBL(this.pc2.childPage[0],{bound:[20,13,230,20],caption:"Kode DRK", multiSelection:false, maxLength:10, tag:2,visible:false});								
		...
		this.cb_saham = new saiCBBL(this.pc2.childPage[0],{bound:[20,12,230,20],caption:"Saham", readOnly:true, maxLength:10, tag:1});										
		...
		this.e_ndev = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,11,210,20],caption:"Ni Dev/Lembar", tipeText:ttNilai, text:"0"});						
...
window.app_saku3_transaksi_yakes_inves_fSahamDevApp.extend(window.childForm);
window.app_saku3_transaksi_yakes_inves_fSahamDevApp.implement({
	isiCBAkru: function() {
		this.cb_akru.setSQL("select no_shmdev, keterangan from inv_shmdev_m where periode<='"+this.e_periode.getText()+"' and no_final='-' and kode_lokasi='"+this.app._lokasi+"'",["no_shmdev","keterangan"],false,["No Bukti","Deskripsi"],"and","Data Akru",true);								
	},
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						sql.add("update inv_shmdev_m set no_final='-' where no_shmdev='"+this.cb_akru.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");									
					...
					//jurnal balik
					sql.add("update inv_shmdev_m set no_final='"+this.e_nb.getText()+"' where no_shmdev='"+this.cb_akru.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");									
					sql.add("insert into inv_shmdev_d(no_shmdev,kode_lokasi,kode_kelola,nilai_kb,nilai_pph,nilai_dev,akun_dev,akun_pph,no_kas,akun_piutang,jum_lbr,dev_lbr,kode_plan) "+
							"select '"+this.e_nb.getText()+"',kode_lokasi,kode_kelola,-nilai_kb,-nilai_pph,-nilai_dev,akun_dev,akun_pph,'X',akun_piutang,-jum_lbr,dev_lbr,kode_plan "+
							"from inv_shmdev_d where no_shmdev='"+this.cb_akru.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("insert into inv_shmdev_j(no_shmdev,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) "+
							"select '"+this.e_nb.getText()+"',no_dokumen,'"+this.dp_d1.getDateString()+"',no_urut,kode_akun,keterangan,case dc when 'D' then 'C' else 'D' end,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,'"+this.e_periode.getText()+"',kode_curr,kurs,nik_user,getdate() "+
							"from inv_shmdev_j where no_shmdev='"+this.cb_akru.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					...
					for (var i=0;i < this.sg.getRowCount();i++){
						if (this.sg.rowValid(i) && this.sg.cells(4,i) != "0"){
							...
							sql.add("insert into inv_shmdev_d(no_shmdev,kode_lokasi,kode_kelola,nilai_kb,nilai_pph,nilai_dev,akun_dev,akun_pph,no_kas,akun_piutang,jum_lbr,dev_lbr,kode_plan) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.sg.cells(0,i)+"',"+nilaiToFloat(this.sg.cells(6,i))+","+nilaiToFloat(this.sg.cells(5,i))+","+nilaiToFloat(this.sg.cells(4,i))+",'"+akunDev+"','"+this.akunPPh+"','-','"+this.akunPiuGL+"',"+nilaiToFloat(this.sg.cells(2,i))+","+nilaiToFloat(this.sg.cells(3,i))+",'"+this.cb_plan.getText()+"')");						
							
							sql.add("insert into inv_shmdev_j(no_shmdev,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
									"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+i+",'"+akunDev+"','"+this.e_ket.getText()+" - "+this.sg.cells(0,i)+"','C',"+nilaiToFloat(this.sg.cells(4,i))+",'"+this.kodepp+"','"+this.cb_drk.getText()+"','"+this.app._lokasi+"','APPDEV','DEVIDEN','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");											
						}
					}
					...
					sql.add("insert into glsap(no_bukti,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,kode_curr,kurs,nilai_curr,tgl_input,nik_user,kode_cust,kode_proyek,kode_task,kode_vendor,kode_lokarea,nik,no_doksap,kode_rek,no_payment,paymetod)  "+
							"select no_shmdev,no_urut,kode_lokasi,modul,jenis,no_dokumen,tanggal,kode_akun,dc,nilai,keterangan,kode_pp,periode,kode_drk,'IDR',1,nilai,tgl_input,nik_user,'-','-','-','-','-','-','-','-','-','-' "+
							"from inv_shmdev_j where no_shmdev='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doClick:function(sender){			
		if (sender == this.i_gen) {			
			...
			setTipeButton(tbSimpan);
			this.stsSimpan = 1;			
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"inv_shmdev_m","no_shmdev",this.app._lokasi+"-ASDV"+this.e_periode.getText().substr(2,4)+".","0000"));
			this.e_dok.setFocus();
		}
	},		
	doChange: function(sender) {
		...
		if (sender == this.cb_akru && this.cb_akru.getText()!="" && this.stsSimpan==1) {			
			var strSQL = "select * from inv_shmdev_m where no_shmdev= '"+this.cb_akru.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";			
			...
			var strSQL = "select b.kode_kelola,b.nama,a.nilai_kb,a.nilai_pph,a.nilai_dev,a.jum_lbr,a.dev_lbr "+
						 ...
						 "where a.no_shmdev='"+this.cb_akru.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";						
			var data = this.dbLib.getDataProvider(strSQL,true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
	...
	doChangeCell: function(sender, col, row){				
		if (col == 3 || col == 4) {						
			...
			//2021 tidak ada pph var nilaiPPh = Math.round(nilaiToFloat(this.sg.cells(4,row)) * 15/100);

			var nilaiPPh = 0;
			if (nilaiToFloat(this.sg.cells(3,row)) != 0) {
			...
			this.sg.cells(5,row,floatToNilai(nilaiPPh));
	...
	doLoad3:function(sender){														
		var strSQL = "select distinct a.no_shmdev,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
		             "from inv_shmdev_m a left join inv_shmdev_d b on a.no_shmdev=b.no_shmdev and a.kode_lokasi=b.kode_lokasi and b.no_kas<>'-' and b.jum_lbr > 0 "+
					 "where a.modul = 'APPDEV' and b.no_shmdev is null and a.posted='F' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			...
			if (this.sg3.cells(0,baris) != "") {			
				...
				var strSQL = "select a.* from inv_shmdev_m a where a.no_shmdev= '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){									
						this.cb_akru.setSQL("select no_shmdev, keterangan from inv_shmdev_m where no_shmdev='"+line.no_final+"' and kode_lokasi='"+this.app._lokasi+"'",["no_shmdev","keterangan"],false,["No Bukti","Deskripsi"],"and","Data Akru",true);								
				...
				var strSQL = "select b.kode_kelola,b.nama,a.nilai_kb,a.nilai_pph,a.nilai_dev,a.jum_lbr,a.dev_lbr "+
							 "from inv_shmdev_d a inner join inv_kelola b on a.kode_kelola=b.kode_kelola "+
							 "where a.no_shmdev='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.jum_lbr>0";						
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
