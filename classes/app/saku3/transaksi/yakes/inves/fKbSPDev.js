window.app_saku3_transaksi_yakes_inves_fKbSPDev = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes_inves_fKbSPDev.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes_inves_fKbSPDev";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form KasBank Deviden Saham Penyertaan ", 0);	
		...
		this.bTampil = new button(this.pc2.childPage[0],{bound:[650,15,80,18],caption:"Data Deviden",click:[this,"doLoad"]});			
				
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,22,996,305], childPage:["Data Penyertaan"]});	
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:6,tag:9,				
					...
					colWidth:[[5,4,3,2,1,0],[100,250,250,80,100,80]],				
...
window.app_saku3_transaksi_yakes_inves_fKbSPDev.extend(window.childForm);
window.app_saku3_transaksi_yakes_inves_fKbSPDev.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						...
						sql.add("update inv_spdev_m set no_kas ='-' where no_kas='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBSPDEV','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.cb_sap.getText()+"',getdate(),'"+this.app._userLog+"','F','-','-','-','0')");															
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','D',"+nilaiToFloat(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','09','-','"+this.app._lokasi+"','KBSPDEV','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i) && this.sg.cells(0,i) == "APP"){								
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) "+
										"select '"+this.e_nb.getText()+"',a.no_spdev,'"+this.dp_d1.getDateString()+"',0,a.akun_pph,'"+this.sg.cells(3,i)+"','D',sum(a.nilai_pph),'"+this.app._kodePP+"',b.kode_drk,'-','-',a.kode_lokasi,'KBSPDEV','PPHM','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-' "+
										" from inv_spdev_d a inner join inv_spdev_m b on a.no_spdev=b.no_spdev and a.kode_lokasi=b.kode_lokasi "+
										" where a.nilai_pph<>0 and a.no_spdev='"+this.sg.cells(1,i)+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.nilai_pph>0 "+
										" group by a.no_spdev,a.akun_pph,b.kode_drk,a.kode_lokasi");
								sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) "+
										"select '"+this.e_nb.getText()+"',a.no_spdev,'"+this.dp_d1.getDateString()+"',1,a.akun_piutang,'"+this.sg.cells(3,i)+"','C',sum(a.nilai_dev),b.kode_pp,b.kode_drk,'-','-',a.kode_lokasi,'KBSPDEV','DEVIDEN','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-' "+
										" from inv_spdev_d a inner join inv_spdev_m b on a.no_spdev=b.no_spdev and a.kode_lokasi=b.kode_lokasi and a.nilai_dev>0 "+
										" where a.no_spdev='"+this.sg.cells(1,i)+"' and a.kode_lokasi='"+this.app._lokasi+"' "+
										" group by a.no_spdev,a.akun_dev,b.kode_pp,b.kode_drk,a.kode_lokasi,a.akun_piutang");
								
								sql.add("update inv_spdev_m set no_kas='"+this.e_nb.getText()+"' where no_spdev='"+this.sg.cells(1,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
							}
						}
					}	
		...
		switch (event){
			...
			case "hapus" :	
				...
				if (parseFloat(this.app._periode) > parseFloat(this.e_periode.getText())){
					...
					sql.add("delete from kas_m where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from kas_j where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
					sql.add("update inv_spdev_m set progress='1',no_kas ='-' where no_kas='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
					sql.add("delete from glsap where no_bukti='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					setTipeButton(tbAllFalse);	
					this.dbLib.execArraySQL(sql);
				}
	...
	doLoad:function(sender){
		if (this.e_periode.getText() != "") {
			var strSQL = "select a.no_spdev,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,a.kode_mitra + ' - '+b.nama as mi,a.nilai "+						 
			             "from inv_spdev_m a inner join inv_mitra b on a.kode_mitra = b.kode_mitra "+
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData(["INPROG",line.no_spdev,line.tanggal,line.keterangan,line.mi,floatToNilai(line.nilai)]);
				}
	...
	doLoad3:function(sender){																											
		var strSQL = "select a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
		             "from kas_m a "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.posted ='F' and modul = 'KBSPDEV' "+
					 "order by a.no_kas";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.sg3.clear();
	...
	doDoubleClick3: function(sender, col , row) {		
		try{
			...
			if (this.sg3.cells(0,baris) != "") {
				...
				var strSQL = "select a.no_spdev,convert(varchar,a.tanggal,103) as tanggal,a.keterangan,a.kode_mitra + ' - '+b.nama as mi,a.nilai "+						 
							 "from inv_spdev_m a inner join inv_mitra b on a.kode_mitra = b.kode_mitra where a.no_kas='"+this.e_nb.getText()+"' and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";						
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					var line;
					this.sg.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg.appendData(["APP",line.no_spdev,line.tanggal,line.keterangan,line.mi,floatToNilai(line.nilai)]);
					}
				} else this.sg.clear(1);	
				this.sg.validasi();
			}
