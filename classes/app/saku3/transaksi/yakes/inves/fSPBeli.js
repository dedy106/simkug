window.app_saku3_transaksi_yakes_inves_fSPBeli = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yakes_inves_fSPBeli.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yakes_inves_fSPBeli";
		...
		this.cb_sap = new saiCBBL(this.pc2.childPage[0],{bound:[20,10,220,20],caption:"NIK Post SAP", multiSelection:false, maxLength:10, tag:2, visible:false});						
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,22,996,305], childPage:["Data Penyertaan","Rekap Penyertaan","File Dok"]});			
...
window.app_saku3_transaksi_yakes_inves_fSPBeli.extend(window.childForm);
window.app_saku3_transaksi_yakes_inves_fSPBeli.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into inv_spbeli_m(no_spbeli,kode_lokasi,periode,tanggal,nik_user,tgl_input,posted,no_kas,progress,nik_buat,  no_dokumen,keterangan,kode_drk,kode_spkelola,tgl_set,nilai_komisi,nilai_ppn,nilai_levy,nilai_pph,no_app1,no_spb,kode_pp,modul,nilai,nik_app,kode_plan) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.app._userLog+"',getdate(),'F','"+this.e_nb.getText()+"','0','"+this.app._userLog+"',  '-','"+this.e_ket.getText()+"','"+this.cb_drk.getText()+"','-','"+this.dp_d2.getDateString()+"',0,0,0,0,'-','-','"+this.kodepp+"','SPBELI',"+nilaiToFloat(this.e_total.getText())+",'"+this.cb_sap.getText()+"','"+this.cb_plan.getText()+"')");
