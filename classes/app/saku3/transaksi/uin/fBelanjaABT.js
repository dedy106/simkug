window.app_saku3_transaksi_uin_fBelanjaABT = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_uin_fBelanjaABT.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_uin_fBelanjaABT";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form ABT Anggaran Belanja", 0);	
		...
		this.e_alokasi = new saiLabelEdit(this.pc1.childPage[1],{bound:[790,11,200,20],caption:"Nilai Alokasi", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});											
		...
		this.e_saldo = new saiLabelEdit(this.pc1.childPage[1],{bound:[790,12,200,20],caption:"Saldo Alokasi", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});									
		this.e_ket = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,17,450,20],caption:"Deskripsi", maxLength:200});				
		this.e_total = new saiLabelEdit(this.pc1.childPage[1],{bound:[790,17,200,20],caption:"Total Usulan", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});					

		this.pc2 = new pageControl(this.pc1.childPage[1],{bound:[1,12,996,318], childPage:["Parameter","Detail"]});				
		this.cb_giat = new saiCBBL(this.pc2.childPage[0],{bound:[20,10,220,20],caption:"Kegiatan", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});
		this.cb_out = new saiCBBL(this.pc2.childPage[0],{bound:[20,11,220,20],caption:"Output", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});
		this.cb_dout = new saiCBBL(this.pc2.childPage[0],{bound:[20,12,220,20],caption:"Detail Output", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});
		this.cb_komp = new saiCBBL(this.pc2.childPage[0],{bound:[20,13,220,20],caption:"Komponen", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});
		this.cb_dkomp = new saiCBBL(this.pc2.childPage[0],{bound:[20,14,220,20],caption:"Detail Komponen", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});
		...
		this.cb_idBukti = new saiCBBL(this.pc2.childPage[1],{bound:[20,15,220,20],caption:"IDitem", multiSelection:false, maxLength:10, tag:8,change:[this,"doChange"]});
		this.bCopy = new button(this.pc2.childPage[1],{bound:[555,15,80,18],caption:"Copy IDitem",click:[this,"doCopy"]});						

		this.sg = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-63],colCount:9,tag:0,
		            colTitle:["Item Deskripsi","Kd Norma","Norma","Satuan","Harga","Vol","Jumlah","IDitem","Idx"],
...
window.app_saku3_transaksi_uin_fBelanjaABT.extend(window.childForm);
window.app_saku3_transaksi_uin_fBelanjaABT.implement({
	cekSaldoAlok: function() {
		var strSQL = 	"select a.belanja, a.belanja - isnull(b.totusul,0) as saldo "+
						"from uin_alokasi_m a "+ 
						"left join ( "+
						"	select kode_pp,kode_lokasi,sum(total) as totusul "+
						"	from uin_usul_m where form='BELANJA' and no_usul <> '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.cb_pp.getText()+"' and tahun='"+this.c_tahun.getText()+"' "+
		...
		if (typeof data == "object"){
			var line = data.rs.rows[0];							
			if (line != undefined){													
				this.e_alokasi.setText(floatToNilai(line.belanja));															
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into uin_usul_m(no_usul,kode_lokasi,tahun,tanggal,keterangan,kode_pp,nik_app,no_close,tgl_input,nik_user,total,form,status,kode_pp2) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+tahun+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_pp.getText()+"','-','"+this.e_nb.getText()+"',getdate(),'"+this.app._userLog+"',"+nilaiToFloat(this.e_total.getText())+",'BLJABT','ABT','-')");
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)) {
								if (this.sg.cells(7,i) == "-") {
									...
									var idbukti = this.sg.cells(7,i); //idref
									var nu = this.sg.cells(8,i);
								}
								sql.add("insert into uin_usul_d(no_usul,kode_lokasi,nu,kode_norma,keterangan,satuan,tarif,vol,total,tahun,dc, kdsatker,kdprogram,kdgiat,kddept,kdunit, kdoutput,kdsoutput,kdkmpnen,kdskmpnen,kode_akun,kode_pp,idbukti,no_park) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',"+nu+",'"+this.sg.cells(1,i)+"','"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"',"+nilaiToFloat(this.sg.cells(4,i))+","+nilaiToFloat(this.sg.cells(5,i))+","+nilaiToFloat(this.sg.cells(6,i))+",'"+this.c_tahun.getText()+"','D','"+this.kdsatker+"','"+this.kdprogram+"','"+this.cb_giat.getText()+"','"+this.kddept+"','"+this.kdunit+"','"+this.cb_out.getText()+"','"+this.cb_dout.getText()+"','"+this.cb_komp.getText()+"','"+this.cb_dkomp.getText()+"','"+this.cb_akun.getText()+"','"+this.cb_pp.getText()+"','"+idbukti+"','-')");
							}
						}
					}
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "ubah" :
				this.cekCBBL();
				if (!this.statusCB) {
					system.alert(this,"Transaksi tidak valid.","Data Referensi tidak konsisten (Cek kembali Kegiatan s/d Akun).");
					return false;
				}
				if (nilaiToFloat(this.e_total.getText()) > nilaiToFloat(this.e_saldo.getText())) {
					system.alert(this,"Transaksi tidak valid.","Total Pengajuan melebihi Saldo Alokasi.");
					return false;
				}
				else 
				this.simpan();
	...
	doClick:function(sender){
		if (sender == this.i_gen) {			
			...
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"uin_usul_m","no_usul",this.app._lokasi+"-ATB"+this.c_tahun.getText().substr(2,2)+".","00000"));			
	...
	doChangeCell: function(sender, col, row){
		try {
			...
			if ((col == 5 || col == 4) && (this.sg.cells(5,row) != "")) {
		...
		if (this.cb_idBukti.getText()!="") {
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				for (var i in data.rs.rows){
					line = data.rs.rows[i];	
					this.sg.appendData([line.keterangan,line.kode_norma,line.nama,line.satuan,floatToNilai(line.tarif),"0","0",this.cb_idBukti.getText(),this.cb_idBukti.rightLabelCaption]);
				}				
	...
	doLoad:function(sender){	
		try{		
			var strSQL = "select a.no_usul,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.total "+
						 "from uin_usul_m a "+
						 "where a.form='BLJABT' and a.kode_lokasi='"+this.app._lokasi+"' and a.tahun='"+this.c_tahun.getText()+"' and a.kode_pp='"+this.cb_pp.getText()+"' and a.status='ABT' order by a.no_usul desc";													
			var data = this.dbLib.getDataProvider(strSQL,true);		
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU = data;
			...
			if (this.sg1.cells(0,row) != "") {			
				...
				if (typeof data == "object"){
					...
					if (line != undefined){			
						...
						this.cb_out.setText(line.kdoutput);
						this.cb_dout.setText(line.kdsoutput);
						this.cb_komp.setText(line.kdkmpnen);
						this.cb_dkomp.setText(line.kdskmpnen);
						this.cb_akun.setText(line.kode_akun);												
					} 
				}			
				var strSQL = "select a.*,b.nama "+
							 "from uin_usul_d a inner join uin_norma b on a.kode_norma=b.kode_norma and a.kode_lokasi=b.kode_lokasi and a.tahun=b.tahun "+
