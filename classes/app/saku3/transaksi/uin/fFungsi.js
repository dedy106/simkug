window.app_saku3_transaksi_uin_fFungsi = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_uin_fFungsi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_uin_fFungsi";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Fungsi dan SubFungsi", 0);	
		
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		uses("saiGrid",true);		

	
		this.pc1 = new pageControl(this,{bound:[20,10,1000,405], childPage:["Jenis Belanja","Bagan Akun"]});						
...
window.app_saku3_transaksi_uin_fFungsi.extend(window.childForm);
window.app_saku3_transaksi_uin_fFungsi.implement({
	...
	doDoubleClick: function(sender, col , row) {
		try{
			...
			if (this.sg1.cells(0,row) != "") {		
				...
				var sql = "select kdsfung as kode,nmsfung as nama from uin_sfung where kdfungsi='"+this.sg1.cells(0,row)+"' "+
				"order by kdsfung ";
	...
	doLoad:function(sender){
		...
		var sql = "select kdfungsi as kode,nmfungsi as nama from uin_fungsi "+
				"order by kdfungsi ";
