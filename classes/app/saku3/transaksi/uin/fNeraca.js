window.app_saku3_transaksi_uin_fNeraca = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_uin_fNeraca.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_uin_fNeraca";
		...
		this.eKlp = new portalui_saiCBBL(this,{bound:[20, 11, 200, 20], caption:"Versi Neraca", multiSelection:false, 
			sql:["select kode_fs, nama from uin_fs where kode_lokasi = '"+this.app._lokasi+"' ", ["kode_fs","nama"], false, ["Kode FS","Nama"],"and","Daftar Versi Laporan",true]
		});					
		...
		this.e0.setText("LABARUGI");
		this.eKlp.setText("FS1");
		
	}
};
window.app_saku3_transaksi_uin_fNeraca.extend(window.portalui_childForm);
window.app_saku3_transaksi_uin_fNeraca.implement({
				...
				case "simpan" :
					if (modalResult == mrOk)
					{
						...
						try
						{
							...
							var sql2,sql1 = "delete from uin_neraca where modul = '"+modul+"'  and kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";					// kode_lokasi,
							var sql = new server_util_arrayList();
							sql.add("delete from uin_relakun  where  kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi='"+this.app._lokasi+"' "+
								"and kode_neraca in (select distinct kode_neraca from uin_neraca where modul = '"+modul+"'  and kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi='"+this.app._lokasi+"')");
							sql.add(sql1);							
							//if mysql langsung														
							var data = value.split("#");
							for (var i in data){
								sql2 = "insert into uin_neraca(kode_neraca, kode_lokasi, kode_fs, nama, level_spasi, level_lap, tipe, sum_header, jenis_akun, kode_induk, rowindex, modul) values ";
								sql2 += " "+ data[i] +" ";
								sql.add(sql2);
							}	
							data = this.value2.split("#");
							for (var i in data){
								sql3 = "insert into uin_relakun(kode_neraca, kode_fs, kode_akun, kode_lokasi) values ";
								if (data[i]) {
									sql3 += " "+ data[i] +" ;";
									sql.add(sql3);					
								}
	...
	loadMenu: function(daftarNrc){
		try
		{
			...
			var item = undefined;
			var node = undefined;
			var line, ix = -1, nrcRls, lineData, nrc_d = this.dblib.loadQuery("select kode_neraca, kode_akun from uin_relakun where kode_lokasi ='"+this.elokasi.getText()+"' and kode_fs = '"+this.eKlp.getText()+"' order by kode_neraca");															
	...
	doClick: function(sender){
		if (this.e0.getText() != "")
		{
			...
			var nrc = this.dblib.runSQL("select * from uin_neraca where Modul = '"+modul+"' and kode_fs = '"+this.eKlp.getText()+"' and kode_lokasi = '"+this.elokasi.getText()+"'  order by rowindex",0,0);
			this.loadMenu(nrc);
		}	
	},
		...
		{ 
		 	if (sender == this.relBtn)
			{
				...
				if (this.selectedItem.data.get("tipe").toLowerCase() == "posting"){
					...
					uses("app_saku3_transaksi_uin_fRelakun",true);
					if (this.relakun != undefined)
						this.relakun.free();
					this.relakun = new app_saku3_transaksi_uin_fRelakun(this.app);
			...
			}else
			{
				if (sender == this.addBtn || sender == this.editBtn){
					uses("app_saku3_transaksi_uin_fNeracaDetail");
					if (this.entryMenu == undefined)
						this.entryMenu = new app_saku3_transaksi_uin_fNeracaDetail(this.app,this.e0.getText());
