window.app_saku3_transaksi_uin_fPdptABT = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_uin_fPdptABT.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_uin_fPdptABT";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Anggaran Tambahan Pendapatan", 0);	
		...
		this.cb_giat = new saiCBBL(this.pc1.childPage[1],{bound:[20,11,220,20],caption:"Kegiatan", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});
		this.e_alokasi = new saiLabelEdit(this.pc1.childPage[1],{bound:[790,11,200,20],caption:"Target Alokasi", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});									
		this.cb_akun = new saiCBBL(this.pc1.childPage[1],{bound:[20,10,220,20],caption:"Akun", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});		
		...
		this.cb_idBukti = new saiCBBL(this.pc1.childPage[1],{bound:[20,15,220,20],caption:"IDitem", multiSelection:false, maxLength:10, tag:8,change:[this,"doChange"]});
		this.bCopy = new button(this.pc1.childPage[1],{bound:[555,15,80,18],caption:"Copy IDitem",click:[this,"doCopy"]});						
		...
		this.p1 = new panel(this.pc1.childPage[1],{bound:[1,23,997,270],caption:"Daftar Usulan"});							
		this.sg = new saiGrid(this.p1,{bound:[0,20,this.p1.width-5,this.p1.height-47],colCount:9,tag:0,
...
window.app_saku3_transaksi_uin_fPdptABT.extend(window.childForm);
window.app_saku3_transaksi_uin_fPdptABT.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into uin_usul_m(no_usul,kode_lokasi,tahun,tanggal,keterangan,kode_pp,nik_app,no_close,tgl_input,nik_user,total,form,status,kode_pp2) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+tahun+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_pp.getText()+"','-','-',getdate(),'"+this.app._userLog+"',"+nilaiToFloat(this.e_total.getText())+",'PDPTABT','ABT','-')");
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)) {
								...
								sql.add("insert into uin_usul_d(no_usul,kode_lokasi,nu,kode_norma,keterangan,satuan,tarif,vol,total,tahun,dc,kdsatker,kdprogram,kdgiat,kddept,kdunit,kdoutput,kdsoutput,kdkmpnen,kdskmpnen,kode_akun,kode_pp,idbukti,no_park) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',"+nu+",'"+this.sg.cells(1,i)+"','"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"',"+nilaiToFloat(this.sg.cells(4,i))+","+nilaiToFloat(this.sg.cells(5,i))+","+nilaiToFloat(this.sg.cells(6,i))+",'"+this.c_tahun.getText()+"','D','"+this.kdsatker+"','"+this.kdprogram+"','"+this.cb_giat.getText()+"','"+this.kddept+"','"+this.kdunit+"','-','-','-','-','"+this.cb_akun.getText()+"','"+this.cb_pp.getText()+"','"+idbukti+"','-')");
							}
						}
					}
	...
	doClick:function(sender){
		if (sender == this.i_gen) {			
			...
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"uin_usul_m","no_usul",this.app._lokasi+"-ATP"+this.c_tahun.getText().substr(2,2)+".","00000"));			
	...
	doChange: function(sender){
		try{			
			...
			if (sender==this.cb_giat || sender==this.cb_akun) {
				if (this.cb_giat.getText()!="" && this.cb_akun.getText()!="") {					
					this.cb_idBukti.setSQL("select distinct a.idbukti,a.nu,a.keterangan from uin_usul_d a "+
											"where a.tahun='"+this.c_tahun.getText()+"' and a.kdsatker='"+this.kdsatker+"' and a.kddept='"+this.kddept+"' and a.kdunit ='"+this.kdunit+"' and a.kdprogram = '"+this.kdprogram+"' and a.kdgiat='"+this.cb_giat.getText()+"' and a.kode_akun='"+this.cb_akun.getText()+"' "											
	...
	doLoad:function(sender){	
		try{		
			var strSQL = "select a.no_usul,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.total "+
						 "from uin_usul_m a "+
						 "where a.form='PDPTABT' and a.kode_lokasi='"+this.app._lokasi+"' and a.tahun='"+this.c_tahun.getText()+"' and a.kode_pp='"+this.cb_pp.getText()+"' and a.status='ABT' order by a.no_usul desc";																 
			var data = this.dbLib.getDataProvider(strSQL,true);		
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU = data;
