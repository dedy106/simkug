window.app_saku3_transaksi_uin_fBelanja = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_uin_fBelanja.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_uin_fBelanja";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Usulan Belanja", 0);	
		...
		this.cb_akun = new saiCBBL(this.pc2.childPage[0],{bound:[20,15,220,20],caption:"Akun", multiSelection:false, maxLength:10, tag:1});

		this.sg = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:7,tag:0,
...
window.app_saku3_transaksi_uin_fBelanja.extend(window.childForm);
window.app_saku3_transaksi_uin_fBelanja.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into uin_usul_m(no_usul,kode_lokasi,tahun,tanggal,keterangan,kode_pp,nik_app,no_close,tgl_input,nik_user,total,form,status,kode_pp2) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+tahun+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_pp.getText()+"','-','-',getdate(),'"+this.app._userLog+"',"+nilaiToFloat(this.e_total.getText())+",'BELANJA','ORGI','-')");
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)) {
								sql.add("insert into uin_usul_d(no_usul,kode_lokasi,nu,kode_norma,keterangan,satuan,tarif,vol,total,tahun,dc, kdsatker,kdprogram,kdgiat,kddept,kdunit, kdoutput,kdsoutput,kdkmpnen,kdskmpnen,kode_akun,kode_pp,idbukti,no_park) values "+
										"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',"+i+",'"+this.sg.cells(1,i)+"','"+this.sg.cells(0,i)+"','"+this.sg.cells(3,i)+"',"+nilaiToFloat(this.sg.cells(4,i))+","+nilaiToFloat(this.sg.cells(5,i))+","+nilaiToFloat(this.sg.cells(6,i))+",'"+this.c_tahun.getText()+"','D','"+this.kdsatker+"','"+this.kdprogram+"','"+this.cb_giat.getText()+"','"+this.kddept+"','"+this.kdunit+"','"+this.cb_out.getText()+"','"+this.cb_dout.getText()+"','"+this.cb_komp.getText()+"','"+this.cb_dkomp.getText()+"','"+this.cb_akun.getText()+"','"+this.cb_pp.getText()+"','"+this.e_nb.getText()+"','-')");
							}
						}
					}
	...
	doClick:function(sender){
		if (sender == this.i_gen) {			
			...
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"uin_usul_m","no_usul",this.app._lokasi+"-UBL"+this.c_tahun.getText().substr(2,2)+".","00000"));			
	...
	doLoad:function(sender){	
		try{		
			var strSQL = "select a.no_usul,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.total "+
						 "from uin_usul_m a "+
						 "where a.no_close='-' and a.form='BELANJA' and a.kode_lokasi='"+this.app._lokasi+"' and a.tahun='"+this.c_tahun.getText()+"' and a.kode_pp='"+this.cb_pp.getText()+"' and a.status='ORGI' order by a.no_usul desc";													
			var data = this.dbLib.getDataProvider(strSQL,true);		
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU = data;
