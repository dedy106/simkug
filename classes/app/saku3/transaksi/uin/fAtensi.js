window.app_saku3_transaksi_uin_fAtensi = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_uin_fAtensi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_uin_fAtensi";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Penerima", 0);	
		
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		uses("saiGrid",true);		
		
		this.pc1 = new pageControl(this,{bound:[20,12,1000,450], childPage:["Daftar Penerima","Data Penerima","Filter Cari"]});
...
window.app_saku3_transaksi_uin_fAtensi.extend(window.childForm);
window.app_saku3_transaksi_uin_fAtensi.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into uin_atensi(kode_atensi,kode_lokasi,nama,alamat,no_tel,email,npwp,pic,alamat2,bank,cabang,no_rek,nama_rek,no_fax,no_pictel) values "+
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from uin_atensi where kode_atensi = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");								
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select kode_atensi,nama,alamat,no_tel,email,npwp,alamat2,pic,bank,cabang,no_rek,nama_rek,no_fax,no_pictel "+
				             "from uin_atensi "+
						     "where kode_atensi ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";						   				
	...
	doBtnClick: function(sender, event){
		try{
			if (sender == this.cb_kode) {   
			    this.standarLib.showListData(this, "Daftar Atensi",sender,undefined, 
											  "select kode_atensi, nama  from uin_atensi where kode_lokasi='"+this.app._lokasi+"'",
											  "select count(kode_atensi) from uin_atensi where kode_lokasi='"+this.app._lokasi+"'",
											  ["kode_atensi","nama"],"and",["Kode","Nama"],false);				
			}
		}catch(e){
			systemAPI.alert(e);
		}
	...
	doCari:function(sender){								
		try {
			if (this.e_kode2.getText() != "") var filter = " a.kode_atensi like '%"+this.e_kode2.getText()+"%' and ";
			else var filter = " a.nama like '%"+this.e_nama2.getText()+"%' and ";
			
			var strSQL = "select a.kode_atensi,a.nama,a.alamat "+
						 "from uin_atensi a "+					 
						 "where "+filter+" a.kode_lokasi='"+this.app._lokasi+"' order by a.kode_atensi";				
			var data = this.dbLib.getDataProvider(strSQL,true);		
	...
	doLoad:function(sender){								
		try {
			var strSQL = "select a.kode_atensi,a.nama,a.alamat "+
						 "from uin_atensi a "+					 
						 "where a.kode_lokasi='"+this.app._lokasi+"' order by a.kode_atensi";				
			var data = this.dbLib.getDataProvider(strSQL,true);		
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU = data;
				this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length/20));
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_atensi,line.nama,line.alamat,"Pilih"]); 
		}
