window.app_saku3_transaksi_fa_fFasusutCepatYpt = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_fa_fFasusutCepatYpt.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_fa_fFasusutCepatYpt";
		...
		this.e_gar = new saiLabelEdit(this,{bound:[250,22,202,20],caption:"Saldo Gar", tag:3, readOnly: true, tipeText:ttNilai, text:"0"});				
		...
		this.e_nilai = new saiLabelEdit(this,{bound:[250,23,202,20],caption:"Nilai Penyusutan", tag:3,  tipeText:ttNilai, text:"0"});		
		...
		try {
			...
			this.cb_fa.setSQL("select a.no_fa, a.nama from fa_asset a "+
							  ...
							  "  where a.progress in ('2','L') and zz.nilai-a.nilai_residu > isnull(tot_susut,0) and a.kode_lokasi='"+this.app._lokasi+"'",["a.no_fa","a.nama"],false,["No Aktap","Nama"],"and","Daftar Aktiva Tetap",true);
...
window.app_saku3_transaksi_fa_fFasusutCepatYpt.extend(window.childForm);
window.app_saku3_transaksi_fa_fFasusutCepatYpt.implement({
	...
	doChange:function(sender){
		...
		if (sender == this.cb_fa && this.cb_fa.getText()!="") {
			if (this.cb_drk.getText()!="") {
				var strSQL = "select a.no_seri,a.merk,a.tipe,(zz.nilai-a.akum_nilai) as nilai,a.nilai_residu,isnull(d.tot_susut,0) as tot_susut,(zz.nilai-a.akum_nilai-a.nilai_residu-isnull(d.tot_susut,0)) as nilai_buku, "+
							 "  case when (zz.nilai-a.akum_nilai-a.nilai_residu-isnull(d.tot_susut,0)) > ceiling((zz.nilai-a.akum_nilai)/a.umur) "+
							 "		 then ceiling((zz.nilai-a.akum_nilai)/a.umur)  "+
							 "		 else ceiling(zz.nilai-a.akum_nilai-a.nilai_residu-isnull(d.tot_susut,0)) end as nilai_susut, "+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){	
						...
						this.umur = line.umur;						
						var data2 = this.dbLib.getDataProvider("select fn_cekagg2('"+line.kode_pp_susut+"','"+this.app._lokasi+"','"+line.akun_bp+"','"+this.cb_drk.getText()+"','"+this.e_periode.getText()+"') as gar ",true);
						if (typeof data2 == "object" && data2.rs.rows[0] != undefined){
							var line2 = data2.rs.rows[0];
							data2 = line2.gar.split(";");
							var sls = parseFloat(data2[0]) - parseFloat(data2[1]);
