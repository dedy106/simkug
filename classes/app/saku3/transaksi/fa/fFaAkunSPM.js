window.app_saku3_transaksi_fa_fFaAkunSPM = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_fa_fFaAkunSPM.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_fa_fFaAkunSPM";
		...
		this.c_jenis = new saiCB(this.pc1.childPage[1],{bound:[20,13,200,20],caption:"Jenis",items:["A.ASET","I.INVENTARIS","B.HABISPAKAI"], readOnly:true,tag:2});		
...
window.app_saku3_transaksi_fa_fFaAkunSPM.extend(window.childForm);
window.app_saku3_transaksi_fa_fFaAkunSPM.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into fa_klpakun(kode_klpakun,nama,umur,persen,kode_akun,akun_bp,akun_deprs,kode_agg,tahun,kode_drk,kode_lokasi, flag_susut,jenis) values "+
							"('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"',"+umurBln+","+nilaiToFloat(this.e_persen.getText())+",'"+this.cb_akun.getText()+"','"+this.cb_bp.getText()+"','"+this.cb_deprs.getText()+"','-','-','-','"+this.app._lokasi+"','"+this.c_status.getText().substr(0,1)+"','"+this.c_jenis.getText().substr(0,1)+"')");									
					setTipeButton(tbAllFalse);
	...
	doChange: function(sender){
		try{
			...
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select kode_klpakun,nama,umur,persen,kode_akun,akun_bp,akun_deprs,kode_agg,tahun,kode_drk,flag_susut,jenis "+
				...
				if (typeof data == "object"){
					...
					if (line != undefined){						
						...
						if (line.jenis=="A") this.c_jenis.setText("A.ASET");
						if (line.jenis=="I") this.c_jenis.setText("I.INVENTARIS");
						if (line.jenis=="B") this.c_jenis.setText("B.HABISPAKAI");
