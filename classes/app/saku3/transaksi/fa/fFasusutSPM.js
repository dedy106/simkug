window.app_saku3_transaksi_fa_fFasusutSPM = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_fa_fFasusutSPM.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_fa_fFasusutSPM";
		...
		this.pc1 = new pageControl(this,{bound:[20,12,995,333], childPage:["Item Jurnal","Data Anggaran","Detail FA"]});
		...
		this.sg3 = new saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:8,tag:9,
				colTitle:["No FA","Deskripsi","Tgl Perolehan","Umur","Kode Akun","Kode PP","Nilai Perolehan","Nilai Susut"],
				colWidth:[[7,6,5,4,3,2,1,0],[100,100,100,100,80,80,300,120]],
				readOnly:true,colFormat:[[3,6,7],[cfNilai,cfNilai,cfNilai]],autoAppend:true,defaultRow:1});
		...
		this.e_tot = new saiLabelEdit(this.sgn3,{bound:[790,2,200,20],caption:"Tot Nilai Susut", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});				
...
window.app_saku3_transaksi_fa_fFasusutSPM.extend(window.childForm);
window.app_saku3_transaksi_fa_fFasusutSPM.implement({
	...
	doDoubleClick1: function(sender, col , row) {
		...
		var strSQL = "select a.no_fa,a.nama,convert(varchar,a.tgl_perolehan,103) as tgl_oleh,a.umur/12 as umur, c.kode_akun,a.kode_pp,zz.nilai, "+
				"case when (abs(zz.nilai)-abs(a.nilai_residu)-abs(isnull(b.tot_susut,0))) > abs(a.nilai_susut) "+
				"then a.nilai_susut "+
				"else (zz.nilai-a.nilai_residu-isnull(b.tot_susut,0)) end as nsusut "+
				...
				"from fa_nilai "+
				"where periode<='201810' and kode_lokasi='"+this.app._lokasi+"' "+
				...
				"where c.akun_bp='"+this.sg.cells(0,row)+"' and c.akun_deprs='"+this.sg.cells(2,row)+"' and a.kode_pp='"+this.sg.cells(4,row)+"' and progress = '2' and (abs(zz.nilai)-abs(a.nilai_residu)) > abs(isnull(b.tot_susut,0)) and a.kode_lokasi='"+this.app._lokasi+"' and a.periode_susut<='"+this.e_periode.getText()+"' ";
		...
		if (typeof data1 == "object" && data1.rs.rows[0] != undefined){
			...
			for (var i in data1.rs.rows){
				...
				tot+=parseFloat(line1.nsusut);
				this.sg3.appendData([line1.no_fa,line1.nama,line1.tgl_oleh,line1.umur,line1.kode_akun,line1.kode_pp,floatToNilai(line1.nilai),floatToNilai(line1.nsusut)]);
			}
