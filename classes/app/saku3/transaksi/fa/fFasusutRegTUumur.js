window.app_saku3_transaksi_fa_fFasusutRegTUumur = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_fa_fFasusutRegTUumur.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_fa_fFasusutRegTUumur";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Penyusutan Aktiva Tetap By Umur", 0);	
		//by umur fa_aset
...
window.app_saku3_transaksi_fa_fFasusutRegTUumur.extend(window.childForm);
window.app_saku3_transaksi_fa_fFasusutRegTUumur.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into fasusut_d(no_fasusut,no_fa,periode,nilai,kode_lokasi,akun_bp,akun_ap,kode_akun,kode_pp,kode_drk,dc,no_del,nilai_aset,umur) "+
							...
							"	 	  then a.nilai_susut * (case when a.umur >= ((datediff(month,substring(a.periode_susut,1,4)+'-'+substring(a.periode_susut,5,2)+'-15',substring('"+this.e_periode.getText()+"',1,4)+'-'+substring('"+this.e_periode.getText()+"',5,2)+'-15'))+1) then ((datediff(month,substring(a.periode_susut,1,4)+'-'+substring(a.periode_susut,5,2)+'-15',substring('"+this.e_periode.getText()+"',1,4)+'-'+substring('"+this.e_periode.getText()+"',5,2)+'-15'))+1) else a.umur end )  "+
