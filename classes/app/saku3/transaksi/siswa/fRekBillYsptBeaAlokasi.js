window.app_saku3_transaksi_siswa_fRekBillYsptBeaAlokasi = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siswa_fRekBillYsptBeaAlokasi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siswa_fRekBillYsptBeaAlokasi";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Load Pelunasan Tagihan BeaSiswa per Alokasi", 0);	
		...
		this.e_piutang = new saiLabelEdit(this,{bound:[720,12,200,20],caption:"Total Tagihan", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});			
		...
		this.e_sisa = new saiLabelEdit(this,{bound:[720,17,200,20],caption:"Tot Sisa Beasiswa", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		...
		this.pc1 = new pageControl(this,{bound:[20,12,900,330], childPage:["Alokasi","Data Tagihan","Data Pelunasan"]});
		...
		this.sg = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:10,tag:0,
					...
					colWidth:[[9,8,7,6,5,4,3,2,1,0],[80,80,80,100,100,80,80,150,150,100]],					
		...
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[2],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:4,
					colTitle:["NIS","Nilai Beasiswa", "Sisa Beasiswa","Validasi"],
					colWidth:[[3,2,1,0],[100,100,100,100]],
					colFormat:[[1,2],[cfNilai,cfNilai]],
					pasteEnable:true,autoPaging:true,rowPerPage:20,afterPaste:[this,"doAfterPaste"], 
					readOnly:true, defaultRow:1
					});							
		this.sgn1 = new portalui_sgNavigator(this.pc1.childPage[2],{bound:[0,this.pc1.height-25,this.pc1.width-1,25],buttonStyle:bsAll, grid:this.sg1, pager:[this,"doPager1"]});		
		...
		try {
			...
			this.cb_ppkirim.setSQL("select kode_pp, nama from pp where kode_lokasi='"+this.app._lokasi+"'",["kode_pp","nama"],false,["Kode","Nama"],"and","Data PP Pengirim",true);
...
window.app_saku3_transaksi_siswa_fRekBillYsptBeaAlokasi.extend(window.childForm);
window.app_saku3_transaksi_siswa_fRekBillYsptBeaAlokasi.implement({	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into sis_rekon_m(no_rekon,no_dokumen,tanggal,keterangan,nilai,posted,modul,akun_titip,nis,nik_buat,nik_app,kode_lokasi,periode,nik_user,tgl_input,kode_pp) values "+
						    "('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"',"+total+",'F','LOADBEA','"+this.cb_titip.getText()+"','-','"+this.app._userLog+"','"+this.app._userLog+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.cb_pp.getText()+"')");
					...
					sql.add("insert into sis_rekon_j(no_rekon,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.cb_titip.getText()+"','"+this.e_ket.getText()+"','D',"+total+",'"+this.cb_ppkirim.getText()+"','-','"+this.app._lokasi+"','LOADBEA','TTP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");					
					...
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						...
						if (parseFloat(line.lunas) != 0){							
							sql.add("insert into sis_rekon_d(no_rekon,nis,no_bill,periode,nilai,kode_lokasi,akun_titip,akun_piutang,kode_param,dc,modul,id_bank,kode_pp, nilai_cd,periode_bill) values "+
									"	('"+this.e_nb.getText()+"','"+line.nis+"','"+line.no_bill+"','"+this.e_periode.getText()+"',"+parseFloat(line.lunas)+",'"+this.app._lokasi+"','"+this.cb_titip.getText()+"','"+line.akun_piutang+"','"+line.kode_param+"','D','LOADBEA','"+line.id_bank+"','"+this.cb_pp.getText()+"',"+parseFloat(line.sisa)+",'"+line.periode+"')");							
						}						
					}
					...
					for (var i=0; i < this.sg1.getRowCount();i++){
						if (nilaiToFloat(this.sg1.cells(2,i)) != 0) {
							...
							var k = i + 100000;
							sql.add("insert into sis_rekon_j(no_rekon,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
									"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',"+k+",'"+this.akunCD+"','"+this.e_ket.getText()+"','C',"+nilaiToFloat(this.sg1.cells(2,i))+",'"+this.app._kodePP+"','-','"+this.app._lokasi+"','LOADBEA','CD','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");
						}
					}
					
					sql.add("update sis_beagar_d set no_ref = '"+this.e_nb.getText()+"' where no_gar='"+this.cb_bukti.getText()+"' and kode_ppterima='"+this.cb_pp.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "simpan" :	
				...
				if ( total != nilaiToFloat(this.e_nilaialok.getText())) {
					system.alert(this,"Transaksi tidak valid.","Nilai Pelunasan tidak sama dengan Nilai Alokasi.");
					return false;						
				}				
	...
	doSelectDate: function(sender, y,m,d){
		...
		var strSQL = "select a.no_gar,a.keterangan "+
					 "from sis_beagar_m a inner join ( "+
					 "    select distinct no_gar,kode_lokasi from sis_beagar_d where kode_ppterima ='"+this.app._kodePP+"' and  no_ref = '-' and kode_lokasi='"+this.app._lokasi+"' "+
	...
	doChange:function(sender){
		if (sender == this.cb_bukti && this.cb_bukti.getText()!="") {
			var strSQL = "select a.*,b.nilai "+
						"from sis_beagar_m a inner join sis_beagar_d b on a.no_gar=b.no_gar and a.kode_lokasi=b.kode_lokasi "+							 
	...
	doLoad: function(sender){	
		if (this.cb_pp.getText()!="") {
			...
			var strSQL = "select aa.nis,aa.nama as siswa,a.no_bill,a.periode,a.akun_piutang,sum((a.nilai-isnull(b.tot_lunas,0))) as saldo,0 as lunas, '-' as id_bank,0 as sisa,a.kode_param,c.idx "+
						 "from sis_bill_d a "+
						 ...
						 "group by aa.nis,aa.nama,a.no_bill,a.periode,a.akun_piutang,a.kode_param,c.idx order by aa.nis,c.idx ";
