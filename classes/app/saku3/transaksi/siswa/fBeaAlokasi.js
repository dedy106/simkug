window.app_saku3_transaksi_siswa_fBeaAlokasi = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siswa_fBeaAlokasi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siswa_fBeaAlokasi";
		...
		this.e_total = new saiLabelEdit(this.pc2.childPage[0],{bound:[770,13,220,20],caption:"Total", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});						
...
window.app_saku3_transaksi_siswa_fBeaAlokasi.extend(window.childForm);
window.app_saku3_transaksi_siswa_fBeaAlokasi.implement({	
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into sis_beagar_m (no_gar,kode_lokasi,nik_user,tgl_input,periode,tanggal,no_dokumen,keterangan,kode_akun,kode_pp,nilai, kode_drk) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.app._userLog+"',getdate(),'"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.cb_akun.getText()+"','"+this.app._kodePP+"',"+nilaiToFloat(this.e_total.getText())+",'"+this.cb_drk.getText()+"')");
