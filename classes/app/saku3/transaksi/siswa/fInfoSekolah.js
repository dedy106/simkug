window.app_saku3_transaksi_siswa_fInfoSekolah = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siswa_fInfoSekolah.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siswa_fInfoSekolah";
		...
		this.dp_d1 = new portalui_datePicker(this,{bound:[120,16,100,18],selectDate:[this,"doSelectDate"]}); 		
		...
		this.pc1 = new pageControl(this,{bound:[20,22,1000,410], childPage:["Upload Dokumen"]});				
		this.sgUpld = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:2,
...
window.app_saku3_transaksi_siswa_fInfoSekolah.extend(window.childForm);
window.app_saku3_transaksi_siswa_fInfoSekolah.implement({	
				...
				try{									
					...
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i)){							
							sql.add("insert into sis_info(no_bukti,kode_pp,kode_lokasi,nik_user,tanggal,foto,tgl_input) values "+
						    		"('"+this.e_nb.getText()+"','"+this.app._kodePP+"','"+this.app._lokasi+"','"+this.app._userLog+"','"+this.dp_d1.getDateString()+"', '"+this.sgUpld.cells(1,i).tmpfile+"',getdate())");														
						}	
					}	
					setTipeButton(tbAllFalse);
