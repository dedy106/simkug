window.app_saku3_transaksi_siswa_fCDmutasi = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siswa_fCDmutasi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siswa_fCDmutasi";
...
window.app_saku3_transaksi_siswa_fCDmutasi.extend(window.childForm);
window.app_saku3_transaksi_siswa_fCDmutasi.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into sis_rekon_m(no_rekon,no_dokumen,tanggal,keterangan,nilai,posted,modul,akun_titip,nis,nik_buat,nik_app,kode_lokasi,periode,nik_user,tgl_input,kode_pp) values "+
						    "('"+this.e_nb.getText()+"','"+this.cb_nim2.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"',"+parseNilai(this.e_nilai.getText())+",'F','CDMUT','"+this.akunCD+"','"+this.cb_nim.getText()+"','"+this.app._userLog+"','"+this.app._userLog+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.cb_pp.getText()+"')");					
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){										
						...
						this.cb_nim2.setSQL("select nis, nama from sis_siswa where nis ='"+line.no_dokumen+"' and kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"'",["nis","nama"],false,["NIS","Nama"],"and","Daftar Siswa",true);		
