window.app_saku3_transaksi_siswa_fNilaiReg = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siswa_fNilaiReg.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siswa_fNilaiReg";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Hasil Penilaian Calon Siswa", 0);	
		
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		uses("saiGrid",true);	
	
		this.pc1 = new pageControl(this,{bound:[20,14,1000,430], childPage:["Data Penilaian","Daftar Penilaian"]});

		this.cb_ta = new portalui_saiCBBL(this.pc1.childPage[0],{bound:[20,10,220,20],caption:"Tahun Ajaran",multiSelection:false,tag:2,change:[this,"doChange"]});			
		...
		this.bRekon = new button(this.pc1.childPage[0],{bound:[740,13,100,20],caption:"Validasi", click:[this,"doRekon"]});

		this.pc2 = new pageControl(this.pc1.childPage[0],{bound:[5,12,990,325], childPage:["Data Nilai  Calon Siswa","Error Msg"]});							
		this.sg2 = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:5,tag:9,
		            colTitle:["ID Registrasi","Nama","Nilai","Keterangan","Val-IDReg"],					
					colWidth:[[4,3,2,1,0],[80,150,80,350,150]],		
					colFormat:[[2],[cfNilai]],
		...
		this.sg1 = new saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:3,tag:9,
		            colTitle:["No. Bukti","Tahun Ajaran","Sekolah/PP"],
...
window.app_saku3_transaksi_siswa_fNilaiReg.extend(window.childForm);
window.app_saku3_transaksi_siswa_fNilaiReg.implement({
	...
	doRekon: function(sender){				
		try {
			var strSQL = "select no_reg from sis_siswareg where kode_lokasi='"+this.app._lokasi+"' and kode_pp = '"+this.app._kodePP+"'";			
		...
		try{
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					if (this.stsSimpan == 0) {
						sql.add("delete from sis_nilai_reg where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and kode_pp = '"+this.app._kodePP+"'");
					}			

					if (this.sg2.getRowValidCount() > 0){
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i) && this.sg2.cells(4,i) == "VALID"){
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {			
				...
				var strSQL = "select a.no_reg,a.nilai,a.keterangan from sis_nilai_reg a inner join sis_siswareg b on a.no_reg=b.no_reg where a.no_bukti='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_pp = '"+this.app._kodePP+"' ";										
				...
				if (typeof data1 == "object" && data1.rs.rows[0] != undefined){
					...
					for (var i in data1.rs.rows){
						...
						this.sg2.appendData([line1.no_reg,line1.nama,line1.nilai,line1.keterangan,"VALID"]);
					}
