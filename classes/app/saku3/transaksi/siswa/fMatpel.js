window.app_saku3_transaksi_siswa_fMatpel = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siswa_fMatpel.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siswa_fMatpel";
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,430], childPage:["Daftar Pelajaran","Data Pelajaran"]});				
		...
		this.e_ket = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,13,550,20],caption:"Keterangan", maxLength:50, tag:1});
...
window.app_saku3_transaksi_siswa_fMatpel.extend(window.childForm);
window.app_saku3_transaksi_siswa_fMatpel.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into sis_matpel(kode_matpel,kode_lokasi,nama,flag_aktif,kode_pp,keterangan,sifat, kode_jenis, kode_jur) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+vSts+"','"+this.app._kodePP+"','"+this.e_ket.getText()+"','"+this.c_sifat.getText()+"', '"+this.cb_jenis.getText()+"','-')");
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					...
					var sql = new server_util_arrayList();					
					if (this.c_status.getText() == "1.AKTIF") vSts = "1"; else vSts = "0";					
					sql.add("update sis_matpel set nama='"+this.e_nama.getText()+"',flag_aktif='"+vSts+"',keterangan = '"+this.e_ket.getText()+"',sifat = '"+this.c_sifat.getText()+"',kode_jenis='"+this.cb_jenis.getText()+"' "+
	...
	doLoad:function(sender){								
		var strSQL = "select kode_matpel, nama from sis_matpel where kode_jenis <> 'PRO' and kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"' ";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
