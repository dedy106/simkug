window.app_saku3_transaksi_siswa_fKalAkad = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siswa_fKalAkad.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siswa_fKalAkad";
		...
		this.pc1 = new pageControl(this,{bound:[20,13,1000,430], childPage:["Data Kegiatan"]});				
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:2,tag:0,
		            colTitle:["Tanggal","Agenda"],
		            pasteEnable:true,autoPaging:true,rowPerPage:20,afterPaste:[this,"doAfterPaste"], 							
					colWidth:[[1,0],[350,150]],				
...
window.app_saku3_transaksi_siswa_fKalAkad.extend(window.childForm);
window.app_saku3_transaksi_siswa_fKalAkad.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1,9])){
				try{									
					...
					sql.add("delete from sis_kalender_akad where kode_ta = '"+this.cb_ta.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"' and kode_sem='"+this.c_sem.getText()+"' ");		
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from sis_kalender_akad where kode_ta = '"+this.cb_ta.getText()+"' and kode_sem = '"+this.c_sem.getText()+"'  and kode_lokasi='"+this.app._lokasi+"'");						
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
