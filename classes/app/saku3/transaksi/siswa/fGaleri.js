window.app_saku3_transaksi_siswa_fGaleri = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siswa_fGaleri.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siswa_fGaleri";
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,480], childPage:["Form Input Galeri","List Galeri"]});				
		...
		this.dp_d1 = new portalui_datePicker(this.pc1.childPage[0],{bound:[120,16,100,18],selectDate:[this,"doSelectDate"]}); 
		this.cb_pp = new portalui_saiCBBL(this.pc1.childPage[0],{bound:[20,11,220,20],caption:"Sekolah",multiSelection:false,tag:1,change:[this,"doChange"]});	
		...
		this.pc2 = new pageControl(this.pc1.childPage[0],{bound:[1,18,995,330], childPage:["File Dokumen"]});
		...
		try {
			...
			this.cb_kelas.setSQL("select kode_kelas, nama from sis_kelas where flag_aktif='1' and kode_pp='"+this.app._kodePP+"' and kode_lokasi = '"+this.app._lokasi+"' union select '-','-' ",["kode_kelas","nama"],false,["Kode Kelas","Nama"],"and","Data Kelas",true);			
...
window.app_saku3_transaksi_siswa_fGaleri.extend(window.childForm);
window.app_saku3_transaksi_siswa_fGaleri.implement({	
				...
				try{									
					...
					for (var i=0;i < this.sgUpld.getRowCount();i++){
						if (this.sgUpld.rowValid(i)){							
							sql.add("insert into sis_galeri(no_bukti,kode_pp,kode_lokasi,kode_kelas,tanggal,keterangan,foto,tgl_input,nik_user) values "+
						    		"('"+this.e_nb.getText()+"','"+this.cb_pp.getText()+"','"+this.app._lokasi+"','"+this.cb_kelas.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.sgUpld.cells(1,i).tmpfile+"',getdate(),'"+this.app._userLog+"')");
						}	
					}	
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from sis_galeri where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and kode_pp = '"+this.app._kodePP+"'");
	...
	doLoad:function(sender){								
		var strSQL = "select distinct no_bukti,tanggal,keterangan from sis_galeri where kode_lokasi='"+this.app._lokasi+"' and kode_pp = '"+this.app._kodePP+"' group by no_bukti,tanggal,keterangan";		
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {		
				...
				var data = this.dbLib.getDataProvider("select keterangan,tanggal,kode_pp,kode_kelas,foto from sis_galeri where no_bukti = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and kode_pp = '"+this.app._kodePP+"' ",true);
				this.deleteFiles = [];
