window.app_saku3_transaksi_siswa_fAkruBillYpt = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siswa_fAkruBillYpt.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siswa_fAkruBillYpt";
...
window.app_saku3_transaksi_siswa_fAkruBillYpt.extend(window.childForm);
window.app_saku3_transaksi_siswa_fAkruBillYpt.implement({
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into sis_bill_d (no_bill,kode_lokasi,nis,kode_kelas,kode_param,nilai,dc,periode,akun_piutang,modul,kode_pp) "+							
							"select '"+this.e_nb.getText()+"','"+this.app._lokasi+"',a.nis,b.kode_kelas,b.kode_param,b.tarif,'D','"+this.e_periode.getText()+"',c.akun_piutang,'BILL','"+this.app._kodePP+"' "+
							"from sis_siswa a "+
							"inner join sis_siswa_tarif b on a.nis=b.nis and a.kode_pp=b.kode_pp and a.kode_kelas=b.kode_kelas and a.kode_lokasi=b.kode_lokasi "+
							...
							"where b.tarif<>0 and a.flag_aktif = '1' and a.kode_pp='"+this.app._kodePP+"' and (f.nilai is null or f.nilai = 0) and a.kode_lokasi='"+this.app._lokasi+"' and '"+this.e_periode.getText()+"' between b.per_awal and b.per_akhir");
	...
	doTampil:function(sender){		
		if (this.e_periode.getText() != "") {
			var strSQL = "select a.kode_pp,c.kode_param,c.nama,c.akun_piutang,c.akun_pdpt,SUM(b.tarif) as nilai "+
						 ...
						 "where b.tarif<>0 and a.flag_aktif = '1' and (f.nilai is null or f.nilai = 0) and a.kode_lokasi='"+this.app._lokasi+"' and '"+this.e_periode.getText()+"' between b.per_awal and b.per_akhir and a.kode_pp='"+this.app._kodePP+"' "+
	...
	doLoad4:function(sender){																				
		var strSQL = "select a.no_bill,convert(varchar,a.tanggal,103) as tgl,a.keterangan,a.nilai "+
					 ...
					 "where b.no_bill is null and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.posted='F' and a.kode_pp='"+this.app._kodePP+"'";						
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU4 = data;
