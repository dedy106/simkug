window.app_saku3_transaksi_siswa_fCDinReg = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siswa_fCDinReg.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siswa_fCDinReg";
		...
		this.pc2 = new pageControl(this.pc1.childPage[0],{bound:[5,12,990,305], childPage:["Data Nilai  Calon Siswa","Error Msg"]});							
...
window.app_saku3_transaksi_siswa_fCDinReg.extend(window.childForm);
window.app_saku3_transaksi_siswa_fCDinReg.implement({
	...
	doRekon: function(sender){				
		try {
			var strSQL = "select a.no_reg from sis_siswareg a "+ //,b.keterangan
						 //"inner join sis_nilai_reg b on a.no_reg=b.no_reg  "+
						 "where a.kode_lokasi='"+this.app._lokasi+"'";			//b.keterangan='LULUS' and 
		...
		try{
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					if (this.stsSimpan == 0) {
						sql.add("delete from kas_m where no_kasno_kas='"+this.e_nb.getText()+"' and kode_lokasi ='"+this.app._lokasi+"'");
						sql.add("delete from kas_j where no_kasno_kas='"+this.e_nb.getText()+"' and kode_lokasi ='"+this.app._lokasi+"'");						
