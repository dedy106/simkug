window.app_saku3_transaksi_siswa_fPresensi = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siswa_fPresensi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siswa_fPresensi";
		...
		this.pc2 = new pageControl(this.pc1.childPage[0],{bound:[5,12,990,370], childPage:["Data Siswa","Data Presensi","Error Msg"]});		
		this.sg = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:4,tag:9,
		            colTitle:["NIS","Status","Keterangan","Val. Status"],
		            buttonStyle:[[1],[bsAuto]],	
		            picklist:[[1],[new portalui_arrayMap({items:["HADIR","ALFA","IZIN","SAKIT"]})]],				
					colWidth:[[3,2,1,0],[100,300,80,150]],		   			
...
window.app_saku3_transaksi_siswa_fPresensi.extend(window.childForm);
window.app_saku3_transaksi_siswa_fPresensi.implement({
	...
	doRekon: function(sender){				
		try {
			...
			for (var i=0;i < this.sg.getRowCount();i++){
				...
				if (this.sg.cells(1,i) == "HADIR" || this.sg.cells(1,i) == "IZIN" || this.sg.cells(1,i) == "ALFA" || this.sg.cells(1,i) == "SAKIT") {					
	...
	*/
	...
	doLoadData1:	function(sender){
		if (this.cb_kelas.getText()!="") {
			var strSQL = "select 'HADIR' as stsapp,'-' as ket, nis from sis_siswa where flag_aktif='01' and kode_kelas = '"+this.cb_kelas.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"' ";						
			var data = this.dbLib.getDataProvider(strSQL,true);
	...
	doTampilData1: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg.appendData([line.nis,line.stsapp.toUpperCase(),line.ket,"INVALID"]); 
		}
