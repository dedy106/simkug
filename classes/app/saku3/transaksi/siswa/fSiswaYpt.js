window.app_saku3_transaksi_siswa_fSiswaYpt = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siswa_fSiswaYpt.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siswa_fSiswaYpt";
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,430], childPage:["Daftar Siswa","Data Siswa","Filter Cari"]});
...
window.app_saku3_transaksi_siswa_fSiswaYpt.extend(window.childForm);
window.app_saku3_transaksi_siswa_fSiswaYpt.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into sis_siswa(nis,kode_lokasi,nama,flag_aktif,kode_kelas,kode_pp,kode_akt,id_bank,tgl_lulus) values "+
						    "('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.c_flag.getText().substr(0,1)+"','"+this.cb_kelas.getText()+"','"+this.app._kodePP+"','"+this.cb_akt.getText()+"','"+this.e_id.getText()+"','"+this.e_tlulus.getText()+"')");
					setTipeButton(tbAllFalse);
	...
	doCari:function(sender){								
		try {
			...
			var strSQL = "select a.nis,a.nama,a.kode_pp,a.kode_kelas "+
						 "from sis_siswa a "+					 
	...
	doLoad:function(sender){								
		try {
			var strSQL = "select a.nis,a.nama,a.kode_kelas,a.kode_akt,a.kode_pp "+
						 "from sis_siswa a "+					 
						 "where a.kode_lokasi='"+this.app._lokasi+"' and a.kode_pp='"+this.app._kodePP+"' order by a.nis";				
			var data = this.dbLib.getDataProvider(strSQL,true);		
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				this.dataJU = data;
				this.sgn1.setTotalPage(Math.ceil(data.rs.rows.length/20));
