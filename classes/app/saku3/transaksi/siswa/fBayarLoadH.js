window.app_saku3_transaksi_siswa_fBayarLoadH = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siswa_fBayarLoadH.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siswa_fBayarLoadH";
...
window.app_saku3_transaksi_siswa_fBayarLoadH.extend(window.childForm);
window.app_saku3_transaksi_siswa_fBayarLoadH.implement({			
			...
			case "clear" :
				if (modalResult == mrOk)
					...
					var strSQL = "select distinct a.no_rekon, b.keterangan "+
								"from sis_rekon_d a inner join kas_m b on a.no_rekon=b.no_kas and a.kode_lokasi=b.kode_lokasi "+
								"where a.kode_lokasi='"+this.app._lokasi+"' and a.periode='"+this.e_periode.getText()+"' "+
								"union all "+
								"select distinct a.no_rekon, b.keterangan "+
								"from sis_rekon_d a inner join sis_rekon_m b on a.no_rekon=b.no_rekon and a.kode_lokasi=b.kode_lokasi "+
								"where a.kode_lokasi='"+this.app._lokasi+"' and a.periode='"+this.e_periode.getText()+"' ";
