window.app_saku3_transaksi_siswa_fCDoutNonCash = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siswa_fCDoutNonCash.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siswa_fCDoutNonCash";
		this.itemsValue = new arrayList();
		this.maximize();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengeluaran CD per Siswa Non Cash (Pdpt)", 0);	
...
window.app_saku3_transaksi_siswa_fCDoutNonCash.extend(window.childForm);
window.app_saku3_transaksi_siswa_fCDoutNonCash.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into ju_m(no_ju,kode_lokasi,periode,tanggal,no_dokumen,keterangan,kode_pp,modul,jenis,kode_curr,kurs,nilai,nik_buat,nik_setuju,posted,no_del,no_link,ref1,tgl_input,nik_user) values "+
						    "('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.dp_d1.getDateString()+"','"+this.cb_nim.getText()+"','"+this.e_ket.getText()+"','"+this.cb_pp.getText()+"','JUCDOUT','CDOUT','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"','F','-','-','"+this.cb_akun.getText()+"',getdate(),'"+this.app._userLog+"')");
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_ju,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen as nis,a.keterangan "+
		             "from ju_m a "+
					 "where a.kode_pp='"+this.cb_pp.getText()+"' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'JUCDOUT' and a.posted ='F'";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
