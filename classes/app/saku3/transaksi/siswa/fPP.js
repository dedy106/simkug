window.app_saku3_transaksi_siswa_fPP = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siswa_fPP.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siswa_fPP";
		...
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-35],colCount:3,tag:9,
		            colTitle:["Kode PP","Nama PP","Jenjang"],
		...
		this.e_nama = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,11,400,20],caption:"Nama PP / Sekolah", maxLength:50, tag:1});	
		this.cb_bidang = new saiCBBL(this.pc1.childPage[1],{bound:[20,15,220,20],caption:"Jenjang Sekolah", multiSelection:false, maxLength:10, tag:2});
		...
		try {
			...
			this.cb_bidang.setSQL("select kode_bidang,nama from bidang",["kode_bidang","nama"],false,["Kode","Nama"],"where","Data Jenjang",true);
			this.doLoad();
		}catch(e){
			systemAPI.alert(e);
		}
...
window.app_saku3_transaksi_siswa_fPP.extend(window.childForm);
window.app_saku3_transaksi_siswa_fPP.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into pp(kode_pp, kode_lokasi, nama, initial, level_spasi, tipe, sum_header, kode_induk, rowindex, kode_bidang, flag_aktif) values "+
						    "	('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"',  '-', '0', 'Posting', '00', '00', 1, '"+this.cb_bidang.getText()+"', '"+this.c_status.getText().substr(0,1)+"')");
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("update pp set nama='"+this.e_nama.getText()+"', flag_aktif ='"+this.c_status.getText().substr(0,1)+"', kode_bidang='"+this.cb_bidang.getText()+"' "+
	...
	doTampilData: function(page) {		
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_pp,line.nama,line.bidang]); 
		}
