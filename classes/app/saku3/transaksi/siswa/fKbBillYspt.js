window.app_saku3_transaksi_siswa_fKbBillYspt = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_siswa_fKbBillYspt.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_siswa_fKbBillYspt";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Load Pelunasan Tagihan", 0);	
		...
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[1],{bound:[0,5,this.pc1.width-5,this.pc1.height-35],colCount:3,
					colTitle:["NIS","Bayar", "ID Bank"],
...
window.app_saku3_transaksi_siswa_fKbBillYspt.extend(window.childForm);
window.app_saku3_transaksi_siswa_fKbBillYspt.implement({
	doAfterUpload: function(sender, result, data){		
	    try{   			
			...
			for (var i=0; i < this.dataUpload.rows.length;i++){
				...
				var nilaiAwal = parseFloat(line.bayar);				
				var nilaiBayar = parseFloat(line.bayar);				
				
				for (var j=0;j < this.dataJU.rs.rows.length;j++){
					if (line.nis == this.dataJU.rs.rows[j].nis) {
