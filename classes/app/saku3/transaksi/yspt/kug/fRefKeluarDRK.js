window.app_saku3_transaksi_yspt_kug_fRefKeluarDRK = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_kug_fRefKeluarDRK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_kug_fRefKeluarDRK";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Template Pengeluaran DRK", 0);	
...
window.app_saku3_transaksi_yspt_kug_fRefKeluarDRK.extend(window.childForm);
window.app_saku3_transaksi_yspt_kug_fRefKeluarDRK.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into refju_dual(kode_jurnal, kode_lokasi, nama, keterangan, akun_debet, akun_kredit, nilai, kode_pp, kode_drk, modul, jenis,tahun) "+
							"select a.kode_ref+'-'+b.kode_ba,a.kode_lokasi,a.nama,a.nama,a.debet,a.kredit,0,b.kode_pp,a.kode_drk,'KB','"+this.c_jenis.getText()+"',a.tahun "+
							"from refju_m a "+
							"cross join pp b "+
	...
	doChange: function(sender){
		try{
			...
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select kode_ref,nama,debet,kredit,kode_drk,tahun from refju_m where kode_ref ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and jenis='BEBAN' ";						   
	...
	doLoad:function(sender){						
		var strSQL = "select kode_ref,nama,debet,kredit,jenis "+
		             "from refju_m where kode_lokasi='"+this.app._lokasi+"' and jenis='BEBAN' order by kode_ref ";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
