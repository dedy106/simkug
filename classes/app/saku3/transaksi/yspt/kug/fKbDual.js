window.app_saku3_transaksi_yspt_kug_fKbDual = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_kug_fKbDual.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_kug_fKbDual";
		...
		this.sg3 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-30],colCount:5,tag:9,
					...
					colWidth:[[4,3,2,1,0],[100,350,150,80,150]],
					colFormat:[[4],[cfNilai]],			
					readOnly:true,
		...
		this.c_jenis = new saiCB(this.pc2.childPage[0],{bound:[20,12,220,20],caption:"Jenis",items:["PENGELUARAN","PENERIMAAN"], readOnly:true,tag:2,change:[this,"doChange"]});
...
window.app_saku3_transaksi_yspt_kug_fKbDual.extend(window.childForm);
window.app_saku3_transaksi_yspt_kug_fKbDual.implement({	
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					var modul="BM";
					if (this.c_jenis.getText()=="PENGELUARAN")
					{
						modul="BK";
					}
	...
	doClick:function(sender){
		if (this.e_periode.getText()!= "" && this.c_jenis.getText() != "") {
			...
			var sql="select kode_jurnal, nama from refju_dual where kode_pp='"+this.app._kodePP+"' and modul='KB' and kode_lokasi='"+this.app._lokasi+"' and jenis='"+jenis+"' ";
	...
	doLoad3:function(sender){																									
		var strSQL = "select distinct a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.keterangan,a.nilai,a.tanggal "+
		             "from kas_m a "+			 					 
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'KBDUAL' and a.posted ='F' and kode_pp='"+this.app._kodePP+"' "+
					 "order by a.tanggal";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
