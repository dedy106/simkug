window.app_saku3_transaksi_yspt_kug_fFaWO = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_kug_fFaWO.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_kug_fFaWO";
		...
		this.cb_drk = new saiCBBL(this,{bound:[20,18,220,20],caption:"DRK Penghapusan", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});
		this.cb_beban = new saiCBBL(this,{bound:[20,15,220,20],caption:"Beban Penghapusan", multiSelection:false, maxLength:10, tag:2 });
		this.cb_app = new saiCBBL(this,{bound:[20,17,220,20],caption:"NIK Approve", multiSelection:false, maxLength:10, tag:2});		
		this.cb_fa = new saiCBBL(this,{bound:[20,18,250,20],caption:"No Aktiva Tetap", multiSelection:false, maxLength:10, change:[this,"doChange"]});		
		...
		this.e_akun = new portalui_saiLabelEdit(this,{bound:[460,18,452,20],caption:"Akun Aktap",readOnly:true, tag:3});
...
window.app_saku3_transaksi_yspt_kug_fFaWO.extend(window.childForm);
window.app_saku3_transaksi_yspt_kug_fFaWO.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2,3])){
				try{													
					...
					sql.add("insert into fawoapp_m(no_woapp,no_wo,no_dokumen,tanggal,keterangan,kode_curr,kurs,nilai,nilai_ap,kode_pp,kode_drk,posted,nik_buat,nik_setuju,kode_lokasi,periode,no_del,no_link,nik_user,tgl_input) values "+
						    "('"+this.e_nb.getText()+"','-','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','IDR',1,"+parseNilai(this.e_hp.getText())+","+parseNilai(this.e_totSusut.getText())+",'"+this.pp+"','"+this.cb_drk.getText()+"','F','"+this.app._userLog+"','"+this.cb_app.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','-','-','"+this.app._userLog+"',getdate())");					
	...
	doChange:function(sender){
		...
		if (sender == this.cb_drk && this.cb_drk.getText()!="") {
			this.cb_beban.setText("","");			
			this.cb_beban.setSQL("select a.kode_akun, a.nama from masakun a where a.kode_lokasi='"+this.app._lokasi+"'",["a.kode_akun","a.nama"],false,["Kode","Nama"],"and","Daftar Akun",true);			
		}
		if (sender == this.cb_fa && this.cb_fa.getText()!="") {
			var strSQL = "select a.no_seri,a.merk,a.tipe,zz.nilai,a.nilai_residu,isnull(d.tot_susut,0) as tot_susut,(zz.nilai-a.nilai_residu-isnull(d.tot_susut,0)) as nilai_buku, "+
						 ...
						 "where a.no_fa='"+this.cb_fa.getText()+"' and a.progress in ('2','L') and a.kode_lokasi='"+this.app._lokasi+"'";
