window.app_saku3_transaksi_yspt_kug_fKbMintaTAK = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_kug_fKbMintaTAK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_kug_fKbMintaTAK";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Droping via TAK [Direct]", 0);	
		...
		this.e_nb = new portalui_saiLabelEdit(this.pc2.childPage[0],{bound:[20,12,200,20],caption:"No KB Kirim",maxLength:30,readOnly:true});
		...
		this.cb_kas = new saiCBBL(this.pc2.childPage[0],{bound:[20,21,220,20],caption:"Akun KB Pengirim", multiSelection:false, maxLength:10, tag:2});
		...
		this.cb_lokasi = new saiCBBL(this.pc2.childPage[0],{bound:[20,12,220,20],caption:"Lokasi Penerima", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,255], childPage:["Penerima Droping"]});
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:7,tag:0,
		            colTitle:["Rek Penerima","Nama Akun","Nilai Disetujui","No Pengajuan","Sekolah Pengaju","Keterangan","Kode PP"],
					colWidth:[[6,5,4,3,2,1,0],[0,280,200,100,100,180,100]],
...
window.app_saku3_transaksi_yspt_kug_fKbMintaTAK.extend(window.childForm);
window.app_saku3_transaksi_yspt_kug_fKbMintaTAK.implement({
	doTampil: function(sender){	
		var strSQL = "select '-' as kode_akun,'-' as nama_akun, isnull(d.nilai,0) as nilai,a.no_minta,a.kode_pp + ' - ' +b.nama as pp,a.keterangan,a.kode_pp "+
					 ...
					 "left join (select a.no_minta,a.kode_lokasi,sum(a.nilai_app) as nilai "+
							"from ys_minta_d a "+							
							"group by a.no_minta,a.kode_lokasi) d on a.no_minta=d.no_minta and a.kode_lokasi=d.kode_lokasi "+
					 "where a.progress='1' and a.periode<='"+this.e_periode.getText()+"' ";
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_dok.getText()+"','-','"+this.cb_kas.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBDROPTAK','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','-','"+this.e_nb2.getText()+"','"+this.cb_tak.getText()+"')");							
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',99,'"+this.cb_kas.getText()+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBDROPTAK','KIRIM','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");							
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',98,'"+this.cb_tak.getText()+"','"+this.e_ket.getText()+"','D',"+parseNilai(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBDROPTAK','TAK','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");							
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){
								if (this.sg.cells(0,i) != "-" && this.sg.cells(0,i) != "") {
									var kodePPDefault = this.sg.cells(6,i);
									var j = 1000+i;
									sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
											"('"+this.e_nb2.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',"+j+",'"+this.sg.cells(0,i)+"','"+this.sg.cells(5,i)+"','D',"+parseNilai(this.sg.cells(2,i))+",'"+this.sg.cells(6,i)+"','-','-','-','"+this.cb_lokasi.getText()+"','KBDROPTAK','TERIMA','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");							
					
									sql.add("update ys_minta_m set akun_terima='"+this.sg.cells(0,i)+"',progress='2',no_kas='"+this.e_nb2.getText()+"' where no_minta = '"+this.sg.cells(3,i)+"' and kode_lokasi='"+this.app._lokasi+"'");
								}
							}
						}
					}
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb2.getText()+"','"+this.e_dok.getText()+"','"+this.dp_d1.getDateString()+"',1999,'"+this.cb_tak.getText()+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_nilai.getText())+",'"+kodePPDefault+"','-','-','-','"+this.cb_lokasi.getText()+"','KBDROPTAK','TAK','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-')");							

					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb2.getText()+"','"+this.cb_lokasi.getText()+"','"+this.e_dok.getText()+"','-','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+kodePPDefault+"','KBDROPTAK','BM','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','-','"+this.e_nb.getText()+"','-')");							
					//------------------------------------------------
	...
	doLoad3:function(sender){																		
		var strSQL = "select distinct a.no_kas,convert(varchar,a.tanggal,103) as tgl,a.jenis,a.no_dokumen,a.keterangan,a.nilai,a.ref1 "+
		             "from kas_m a  "+
					 "  		inner join kas_m b on a.no_kas=b.ref1 and b.posted='F' and b.modul = 'KBDROPTAK' "+
					 "where a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'KBDROPTAK' and a.posted ='F'";				
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select keterangan,no_dokumen,jenis,tanggal,akun_kb,ref1,nik_app,ref1,kode_bank "+
							 "from kas_m "+							 
				...
				var strSQL = "select c.kode_akun,c.nama as nama_akun, isnull(d.nilai,0) as nilai,a.no_minta,a.kode_pp + ' - ' +b.nama as pp,a.keterangan,a.kode_pp "+
							 ...
							 "group by a.no_minta,a.kode_lokasi) d on a.no_minta=d.no_minta and a.kode_lokasi=d.kode_lokasi "+
							 "where a.no_kas='"+this.e_nb2.getText()+"' and a.kode_lokasi='"+this.cb_lokasi.getText()+"'";
