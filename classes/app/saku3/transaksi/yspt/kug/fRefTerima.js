window.app_saku3_transaksi_yspt_kug_fRefTerima = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_kug_fRefTerima.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_kug_fRefTerima";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Template Penerimaan", 0);	
...
window.app_saku3_transaksi_yspt_kug_fRefTerima.extend(window.childForm);
window.app_saku3_transaksi_yspt_kug_fRefTerima.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into refju_m(kode_ref,kode_lokasi,nama,debet,kredit,jenis) values ('"+
							this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.cb_debet.getText()+"','"+this.cb_kredit.getText()+"','"+this.c_jenis.getText()+"')");
	...
	doChange: function(sender){
		try{
			if (this.cb_kode.getText() != ""){
				var strSQL = "select kode_ref,nama,debet,kredit from refju_m where kode_ref ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and jenis='PDPT' ";						   
