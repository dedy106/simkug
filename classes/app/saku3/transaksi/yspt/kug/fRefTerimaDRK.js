window.app_saku3_transaksi_yspt_kug_fRefTerimaDRK = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_kug_fRefTerimaDRK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_kug_fRefTerimaDRK";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Template Penerimaan DRK", 0);	
		
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		uses("saiGrid",true);		
		this.pc1 = new pageControl(this,{bound:[20,12,1000,450], childPage:["Daftar Template ","Data Template "]});				
		...
		this.c_jenis = new portalui_saiLabelEdit(this.pc1.childPage[1],{bound:[20,12,200,20],caption:"Jenis Transaksi",readOnly:true,tag:2,change:[this,"doChange"]});
		this.cb_kode = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,10,200,20],caption:"Kode Template",change:[this,"doChange"],readOnly:true});		
		...
		this.e_tahun = new portalui_saiLabelEdit(this.pc1.childPage[1],{bound:[20,17,200,20],caption:"Tahun",tag:2,change:[this,"doChange"]});
		this.cb_drk = new saiCBBL(this.pc1.childPage[1],{bound:[20,18,220,20],caption:"DRK", multiSelection:false, maxLength:10, tag:2});				
...
window.app_saku3_transaksi_yspt_kug_fRefTerimaDRK.extend(window.childForm);
window.app_saku3_transaksi_yspt_kug_fRefTerimaDRK.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into refju_dual(kode_jurnal, kode_lokasi, nama, keterangan, akun_debet, akun_kredit, nilai, kode_pp, kode_drk, modul, jenis, tahun) "+
							"select a.kode_ref+'-'+b.kode_ba,a.kode_lokasi,a.nama,a.nama,a.debet,a.kredit,0,b.kode_pp,'"+this.cb_drk.getText()+"','KB','"+this.c_jenis.getText()+"', '"+this.e_tahun.getText()+"' "+
							"from refju_m a "+
							"cross join pp b "+
	...
	doChange: function(sender){
		try{
			if (sender == this.e_tahun && this.e_tahun.getText() != ""){
				this.cb_drk.setSQL("select kode_drk, nama from drk where kode_lokasi='"+this.app._lokasi+"' and tahun='"+this.e_tahun.getText()+"'",["kode_drk","nama"],false,["Kode","Nama"],"and","Data DRK",true);
			}

			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var strSQL = "select kode_ref,nama,debet,kredit,tahun,kode_drk from refju_m where kode_ref ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and jenis='PDPT' ";						   
