window.app_saku3_transaksi_yspt_simak_fNilaiReg = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_simak_fNilaiReg.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_simak_fNilaiReg";
...
window.app_saku3_transaksi_yspt_simak_fNilaiReg.extend(window.childForm);
window.app_saku3_transaksi_yspt_simak_fNilaiReg.implement({
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i) && this.sg2.cells(4,i) == "VALID" && this.sg2.cells(5,i) == "VALID"){								
								...
								sql.add("insert into sis_nilai_reg(no_bukti,kode_ta,kode_pp,kode_lokasi,no_reg,nilai,keterangan,tgl_input) values "+
								 		"('"+this.e_nb.getText()+"','"+this.cb_ta.getText()+"','"+this.cb_pp.getText()+"','"+this.app._lokasi+"','"+this.sg2.cells(0,i)+"',"+parseNilai(this.sg2.cells(2,i))+",'"+this.sg2.cells(3,i)+"',getdate() )");
							}
						}
					}	
	...
	doLoad:function(sender){								
		var strSQL = "select no_bukti,kode_ta,kode_pp from sis_nilai_reg where kode_pp='"+this.app._kodePP+"' and kode_lokasi='"+this.app._lokasi+"'  group by no_bukti,kode_pp,kode_ta ";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
