window.app_saku3_transaksi_yspt_simak_fPresensi = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_simak_fPresensi.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_simak_fPresensi";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Presensi Siswa", 0);	
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,420], childPage:["Data Presensi","Daftar Presensi"]});				
		...
		this.pc2 = new pageControl(this.pc1.childPage[0],{bound:[1,12,995,341], childPage:["Data Siswa","Data Presensi"]});		
		this.sg = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:3,tag:9,
...
window.app_saku3_transaksi_yspt_simak_fPresensi.extend(window.childForm);
window.app_saku3_transaksi_yspt_simak_fPresensi.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					if (this.stsSimpan == 0) {
						sql.add("delete from sis_presensi where kode_kelas = '"+this.cb_kelas.getText()+"' and tanggal = '"+this.dp_d1.getText()+"' and kode_ta = '"+this.cb_ta.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and jenis_absen='HARIAN'");
					}			

					for (var i=0;i < this.dataJU1.rs.rows.length;i++){
						 var line = this.dataJU1.rs.rows[i];
						 sql.add("insert into sis_presensi(nis,kode_kelas,kode_ta,tgl_input,status,kode_lokasi,kode_pp,keterangan,tanggal,jenis_absen) values "+
								 "('"+line.nis+"','"+this.cb_kelas.getText()+"','"+this.cb_ta.getText()+"',getdate(),'"+line.stsapp+"','"+this.app._lokasi+"','"+this.app._kodePP+"','"+line.ket+"','"+this.dp_d1.getDateString()+"','HARIAN')");
					}	
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();
					sql.add("delete from sis_presensi where kode_kelas = '"+this.cb_kelas.getText()+"' and tanggal = '"+this.dp_d1.getText()+"' and kode_ta = '"+this.cb_ta.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and kode_pp = '"+this.app._kodePP+"' and jenis_absen='HARIAN'");
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
				}
				catch(e){
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {			
				...
				var data = this.dbLib.getDataProvider("select a.nis, a.status,a.keterangan from sis_presensi a  where a.kode_kelas = '"+this.cb_kelas.getText()+"' and a.kode_ta = '"+this.cb_ta.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_pp='"+this.app._kodePP+"'and a.jenis_absen='HARIAN' ",true);
				if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
	...
	doLoad:function(sender){								
		var strSQL = "select a.kode_kelas, a.kode_ta, b.nama,a.tanggal from sis_presensi a inner join sis_kelas b on a.kode_kelas=b.kode_kelas where a.jenis_absen='HARIAN' and a.kode_lokasi='"+this.app._lokasi+"' and a.kode_pp='"+this.app._kodePP+"' and a.kode_ta='"+this.cb_ta.getText()+"' group by a.kode_kelas,b.nama,a.kode_ta,a.tanggal";		
		var data = this.dbLib.getDataProvider(strSQL,true);		
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU = data;
