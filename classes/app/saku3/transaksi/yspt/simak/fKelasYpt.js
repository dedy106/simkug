window.app_saku3_transaksi_yspt_simak_fKelasYpt = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_simak_fKelasYpt.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_simak_fKelasYpt";
...
window.app_saku3_transaksi_yspt_simak_fKelasYpt.extend(window.childForm);
window.app_saku3_transaksi_yspt_simak_fKelasYpt.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into sis_kelas(kode_kelas,nama,kode_lokasi,kode_pp,kode_tingkat,kode_jur,flag_aktif) values "+
						    "('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.cb_pp.getText()+"','"+this.cb_tingkat.getText()+"','"+this.cb_jur.getText()+"','"+vSts+"')");										
					setTipeButton(tbAllFalse);
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					...
					sql.add("delete from sis_kelas where kode_pp='"+this.cb_pp.getText()+"' and kode_kelas='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");			
					sql.add("insert into sis_kelas(kode_kelas,nama,kode_lokasi,kode_pp,kode_tingkat,kode_jur,flag_aktif) values "+
						    "('"+this.cb_kode.getText()+"','"+this.e_nama.getText()+"','"+this.app._lokasi+"','"+this.cb_pp.getText()+"','"+this.cb_tingkat.getText()+"','"+this.cb_jur.getText()+"','"+vSts+"')");					
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
	...
	doChange: function(sender){
		try{
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				var data = this.dbLib.getDataProvider("select nama,kode_pp,kode_tingkat,kode_jur,flag_aktif from sis_kelas where kode_pp='"+this.app._kodePP+"' and kode_kelas='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
