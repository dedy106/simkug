window.app_saku3_transaksi_yspt_simak_fDok = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_simak_fDok.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_simak_fDok";
		...
		this.e_foto = new saiLabelEdit(this.pc1.childPage[1], {bound:[20,15,300,20], caption:"Dokumen"});
...
window.app_saku3_transaksi_yspt_simak_fDok.extend(window.childForm);
window.app_saku3_transaksi_yspt_simak_fDok.implement({
		...
		try{	
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
                    ...
                    if (this.stsSimpan == 0){
                        sql.add("delete from sis_konten_dok where no_konten = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ");
                    }

					sql.add("insert into sis_konten_dok(no_konten,kode_lokasi,kode_pp,judul,keterangan,tanggal,flag_aktif, file_dok,nik_user,tgl_input) values "+
	...
	doClick:function(sender){
		if (sender == this.i_gen) {
			...
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"sis_konten_dok","no_konten",this.app._lokasi+"-KDK"+this.e_periode.getText().substr(2,4)+".","0000"));
	...
	doLoad:function(sender){						
        var strSQL = "select a.no_konten, convert(varchar,a.tanggal,103) as tgl, a.judul,a.keterangan, case when a.flag_aktif ='1' then 'AKTIF' else 'NON AKTIF' end as status from sis_konten_dok a where a.kode_lokasi='"+this.app._lokasi+"'  order by a.no_konten ";	
