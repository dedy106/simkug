window.app_saku3_transaksi_yspt_simak_fCaSiswaLoadYpt = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_simak_fCaSiswaLoadYpt.prototype.parent.constructor.call(this, owner);
		this.className = "app_saku3_transaksi_yspt_simak_fCaSiswaLoadYpt";
		...
		this.cb_ta = new portalui_saiCBBL(this,{bound:[20,10,200,20],caption:"Tahun Ajaran",multiSelection:false,tag:2,change:[this,"doChange"]});			
...
window.app_saku3_transaksi_yspt_simak_fCaSiswaLoadYpt.extend(window.portalui_childForm);
window.app_saku3_transaksi_yspt_simak_fCaSiswaLoadYpt.implement({	
	...
	doValid: function() {
		try {
			...
			//8/1/2021 var strSQL = "select no_reg,id_bank,'REG' as jenis from sis_siswareg where kode_pp='"+this.app._kodePP+"' and kode_lokasi='"+this.app._lokasi+"' union select '-','-' ";			
			var strSQL = "select no_reg,id_bank,'REG' as jenis from sis_siswareg where kode_pp='"+this.app._kodePP+"' and kode_lokasi='"+this.app._lokasi+"' "+
						 "union all select '-','-','REG' "+
						 "union all select nis as no_reg,id_bank,'SIS' as jenis from sis_siswa where kode_pp='"+this.app._kodePP+"' and kode_lokasi='"+this.app._lokasi+"' ";
				...
				for (var i=0; i < this.sg1.getRowCount();i++){
					...
					if (this.dataREG.rs.rows.length > 0) {
						for (var j=0;j < this.dataREG.rs.rows.length;j++){				
							if (this.sg1.cells(0,i) == this.dataREG.rs.rows[j].no_reg && this.dataREG.rs.rows[j].jenis=="REG") {
								this.sg1.cells(3,i,"VALID");				
							}
							// dianggap valid ....if (this.sg1.cells(2,i) == this.dataREG.rs.rows[j].id_bank) {
