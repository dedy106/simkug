window.app_saku3_transaksi_yspt_simak_fBayarLoadH = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_simak_fBayarLoadH.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_simak_fBayarLoadH";
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select distinct periode from sis_rekon_d where kode_pp='"+this.app._kodePP+"' and  kode_lokasi='"+this.app._lokasi+"' order by periode desc",true);
...
window.app_saku3_transaksi_yspt_simak_fBayarLoadH.extend(window.childForm);
window.app_saku3_transaksi_yspt_simak_fBayarLoadH.implement({			
