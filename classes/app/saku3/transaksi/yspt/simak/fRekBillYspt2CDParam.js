window.app_saku3_transaksi_yspt_simak_fRekBillYspt2CDParam = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_simak_fRekBillYspt2CDParam.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_simak_fRekBillYspt2CDParam";
		this.itemsValue = new arrayList();
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Load Pelunasan Tagihan CD (NONCASH) By Parameter", 0);	
		...
		this.pc1 = new pageControl(this,{bound:[5,12,1000,330], childPage:["Parameter","Data Tagihan","Data PDD"]});
		...
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:2,
...
window.app_saku3_transaksi_yspt_simak_fRekBillYspt2CDParam.extend(window.childForm);
window.app_saku3_transaksi_yspt_simak_fRekBillYspt2CDParam.implement({
		...
		try {
			...
			for (var i=0; i < this.dataJU1.rs.rows.length;i++){				
				...
				for (var j=0;j < this.dataJU.rs.rows.length;j++){
					if (this.dataJU1.rs.rows[i].nis.toUpperCase() == this.dataJU.rs.rows[j].nis.toUpperCase()) {
	...
	doLoad: function(sender){	
		if (this.cb_pp.getText()!="") {
			...
			var strSQL = "select a.nis,a.siswa,a.no_bill,a.periode,a.akun_piutang,a.tot_bill-isnull(b.tot_lunas,0) as saldo,0 as lunas,'-' as id_bank, 0 as sisa,a.kode_param,a.idx "+
						 ...
						 "		where  a.periode<='"+this.e_periode.getText()+"' and e.kode_pp= '"+this.cb_pp.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' and a.kode_param in ("+kodeparam+") "+
						 ...
						 "      from sis_rekon_d "+
						 "		where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"' and kode_param in ("+kodeparam+") "+
