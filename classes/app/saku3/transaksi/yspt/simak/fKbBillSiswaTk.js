window.app_saku3_transaksi_yspt_simak_fKbBillSiswaTk = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_simak_fKbBillSiswaTk.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_simak_fKbBillSiswaTk";
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,295], childPage:["Data Tagihan"]});		
		...
		try {
			...
			this.cb_titip.setSQL("select a.kode_akun, a.nama from masakun a "+
								 "inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi and b.kode_flag in ('091') "+
...
window.app_saku3_transaksi_yspt_simak_fKbBillSiswaTk.extend(window.childForm);
window.app_saku3_transaksi_yspt_simak_fKbBillSiswaTk.implement({
	isiCBsiswaAktif: function() {
	this.cb_nim.setSQL("select a.nis, a.nama from sis_siswa a "+
						"inner join sis_siswa_status b on a.flag_aktif=b.kode_ss and a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.flag_aktif='1' "+
						"where a.kode_lokasi='"+this.app._lokasi+"' and a.kode_pp='"+this.cb_pp.getText()+"'",["nis","nama"],false,["NIS","Nama"],"and","Daftar Siswa",true);	
	},
	...
	simpan: function(){			
		try{						
			if (this.stsSimpan == 1) this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"kas_m","no_kas",this.app._lokasi+"-BM"+this.e_periode.getText().substr(2,4)+".","0000"));
				...
				if (this.sg.getRowValidCount() > 0){
					for (var i=0;i < this.sg.getRowCount();i++){
						if (this.sg.rowValid(i)){
							...
							else tot_sum += nilaiToFloat(this.sg.cells(6,i));
						}						
					}
				}
						
				if (total != tot_sum){
					system.alert(this,"Transaksi tidak valid.","Total pelunasan dan jumlah nilai pelunasan tidak sama");
					return false;	
				}

				if (nilaiToFloat(this.e_nilai.getText()) <= 0) {
					system.alert(this,"Transaksi tidak valid.","Nilai pelunasan tidak boleh kurang/sama dengan nol.");
					return false;						
				}
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.no_bill,a.periode,a.kode_param,a.nama,a.akun_piutang,a.tot_bill-isnull(b.tot_lunas,0) as saldo,dd.nilai as nilai,dd.id_bank  "+
							 "from ("+
