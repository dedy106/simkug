window.app_saku3_transaksi_yspt_simak_fCDinRegBank = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_simak_fCDinRegBank.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_simak_fCDinRegBank";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Penerimaan Biaya Registrasi Siswa Baru", 0);	
		...
		this.pc2 = new pageControl(this.pc1.childPage[0],{bound:[1,12,995,307], childPage:["Data Pembayaran","Error Msg"]});							
		this.sg2 = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:4,tag:9,
		            colTitle:["ID Bank","ID Registrasi","Nilai Bayar","Val-IDBank"],					
					colWidth:[[3,2,1,0],[100,100,150,150]],		
					colFormat:[[2],[cfNilai]],
...
window.app_saku3_transaksi_yspt_simak_fCDinRegBank.extend(window.childForm);
window.app_saku3_transaksi_yspt_simak_fCDinRegBank.implement({
	...
	doRekon: function(sender){				
		try {
			var strSQL = "select a.id_bank,a.no_reg from sis_siswareg a "+
						 "inner join sis_nilai_reg b on a.no_reg=b.no_reg and b.keterangan='LULUS' and a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi  "+
			...
			for (var i=0; i < this.sg2.getRowCount();i++){
				...
				for (var j=0;j < this.dataNo.rs.rows.length;j++){					
					if (this.sg2.cells(0,i) == this.dataNo.rs.rows[j].id_bank) {						
						this.sg2.cells(1,i,this.dataNo.rs.rows[j].no_reg);
						this.sg2.cells(3,i,"VALID");						
					}					
				}	
				if (this.sg2.cells(3,i) == "INVALID") this.inValid = true;
				if (this.sg2.cells(3,i) == "VALID") tot += nilaiToFloat(this.sg2.cells(2,i));									
			}
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{									
					...
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','-','-','"+this.cb_titip.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.cb_pp.getText()+"','CDINREGIB','BM','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','-','-','-')");																
					
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.cb_titip.getText()+"','"+this.e_ket.getText()+"','D',"+parseNilai(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','CDINREGIB','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+parseNilai(this.e_nilai.getText())+")");
					
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank,nilai_curr) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',1,'"+this.akunCD+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_nilai.getText())+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','CDINREGIB','TITIP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-',"+parseNilai(this.e_nilai.getText())+")");
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i) && this.sg2.cells(3,i) == "VALID"){
								...
								sql.add("insert into sis_cd_d(no_bukti,nis,periode,nilai,kode_lokasi,akun_cd,kode_param,dc,modul,kode_pp,no_ref1) values "+
										"('"+this.e_nb.getText()+"','"+this.sg2.cells(1,i)+"','"+this.e_periode.getText()+"',"+parseNilai(this.sg2.cells(2,i))+",'"+this.app._lokasi+"','"+this.akunCD+"','-','D','CDINREGIB','"+this.cb_pp.getText()+"','-')");
							}
						}
					}	
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {			
				...
				var strSQL = "select b.id_bank,a.nis,a.nilai "+
							 "from sis_cd_d a inner join sis_siswareg b on a.nis=b.no_reg and a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
				...
				if (typeof data1 == "object" && data1.rs.rows[0] != undefined){
					...
					for (var i in data1.rs.rows){
						...
						this.sg2.appendData([line1.id_bank,line1.nis,line1.nilai,"VALID"]);
					}
	...
	doLoad:function(sender){								
		var strSQL = "select a.no_kas, convert(varchar,a.tanggal,103) as tgl, a.keterangan "+
					 "from kas_m a "+
					 "where a.kode_pp='"+this.cb_pp.getText()+"' and a.modul='CDINREGIB' and a.kode_lokasi='"+this.app._lokasi+"' and a.posted='F' ";
		var data = this.dbLib.getDataProvider(strSQL,true);		
