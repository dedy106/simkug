window.app_saku3_transaksi_yspt_simak_fJadwalUjian = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_simak_fJadwalUjian.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_simak_fJadwalUjian";
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,340], childPage:["Data Jadwal Ujian"]});				
		...
		try {
			...
			this.cb_jenis.setSQL("select kode_jenis, nama from sis_jenisnilai where flag_aktif='1' and kode_lokasi = '"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"'",["kode_jenis","nama"],false,["Kode Jenis","Nama"],"and","Data Jenis",true);		
...
window.app_saku3_transaksi_yspt_simak_fJadwalUjian.extend(window.childForm);
window.app_saku3_transaksi_yspt_simak_fJadwalUjian.implement({
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i)){																				
									sql.add("insert into sis_jadwal_ujian(kode_pp,kode_lokasi,kode_ta,kode_sem,kode_tingkat,kode_jenis,tanggal,jam,kode_matpel) values "+
						    				"('"+this.app._kodePP+"','"+this.app._lokasi+"','"+this.cb_ta.getText()+"','"+this.c_sem.getText()+"','"+this.cb_tingkat.getText()+"','"+this.cb_jenis.getText()+"','"+this.sg.getCellDateValue(0,i)+"','"+this.sg.cells(1,i)+"','"+this.sg.cells(2,i)+"')");
							}
						}						
					}
	...
	doChange: function(sender){
		try{
			if ((sender == this.cb_ta || sender == this.cb_tingkat || sender == this.c_sem || sender == this.cb_jenis) && this.cb_ta.getText()!="" && this.cb_tingkat.getText()!="" && this.c_sem.getText()!="" && this.cb_jenis.getText()!="") {				
