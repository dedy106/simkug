window.app_saku3_transaksi_yspt_simak_fSiswaYpt2 = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_simak_fSiswaYpt2.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_simak_fSiswaYpt2";
		...
		this.e_nists = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,31,220,20],caption:"NIS TS", maxLength:20, tag:1});								
...
window.app_saku3_transaksi_yspt_simak_fSiswaYpt2.extend(window.childForm);
window.app_saku3_transaksi_yspt_simak_fSiswaYpt2.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					var sql = new server_util_arrayList();
					sql.add("insert into sis_siswa(nis,kode_lokasi,nama,flag_aktif,kode_kelas,kode_pp,kode_akt,id_bank,tgl_lulus,nis_ts) values "+
							"('"+this.cb_kode.getText()+"','"+this.app._lokasi+"','"+this.e_nama.getText()+"','"+this.c_flag.getText()+"','"+this.cb_kelas.getText()+"','"+this.app._kodePP+"','"+this.cb_akt.getText()+"','"+this.e_id.getText()+"','"+this.e_tlulus.getText()+"','"+this.e_nists.getText()+"')");
		...
		if (this.cb_akt.getText() != "" && this.cb_jur.getText() != "" && this.cb_tingkat.getText() != "") {
			if (this.stsSimpan == 1) {
				var strSQL = "select a.kode_param,a.nama,isnull(b.tarif ,0) as tarif,isnull(b.bulan1 ,'-') as bulan1, isnull(b.bulan2 ,'-') as bulan2 "+
							...
							"where a.kode_lokasi = '"+this.app._lokasi+"' and b.tarif <> 0  "+ // deny-26/4/19 -->and b.tarif <> 0 
							"order by a.idx "; 					
			}
			else {
				if (this.cb_kode.getText() != "") {
					//yg tampil hanya yg ada isinya (deny-26-4-19)
					var strSQL = "select a.kode_param,a.nama,isnull(b.tarif ,0) as tarif,isnull(b.per_awal ,'-') as bulan1, isnull(b.per_akhir ,'-') as bulan2 "+
								"from sis_param a "+	
								"inner join sis_siswa_tarif b on a.kode_param=b.kode_param and a.kode_lokasi=b.kode_lokasi and b.nis='"+this.cb_kode.getText()+"' and b.kode_pp='"+this.cb_pp.getText()+"' "+
	...
	doChange: function(sender){
		try{
			...
			if (sender == this.cb_kelas && this.cb_kelas.getText() != "" && this.cb_pp.getText() != "") {
				var data = this.dbLib.getDataProvider("select kode_tingkat,kode_jur from sis_kelas where kode_pp='"+this.cb_pp.getText()+"' and kode_kelas='"+this.cb_kelas.getText()+"' and kode_lokasi='"+this.app._lokasi+"'",true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){					
			...
			if (sender == this.cb_kode && this.cb_kode.getText() != ""){
				//cek data registrasi siswa
				var data = this.dbLib.getDataProvider("select nama from sis_siswareg where no_reg='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"'",true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){		
						this.cb_kode.setText("");
						system.alert(this,"NIS tidak valid.","NIS duplikasi dgn nomor REG ("+line.nama+").");
						return false;
					}					
				}	
				...
				if (typeof data == "object"){
					...
					if (line != undefined){		
						...
						this.e_nists.setText(line.nis_ts);
			...
			if (sender == this.sg) {				
				if (col == 0){
					...
					this.standarLib.showListData(this, "Daftar Parameter",sender,undefined, 
							...
							"select count(*) from sis_param where kode_lokasi = '"+this.app._lokasi+"' "+filter,
