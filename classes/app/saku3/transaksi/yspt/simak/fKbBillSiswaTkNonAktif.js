window.app_saku3_transaksi_yspt_simak_fKbBillSiswaTkNonAktif = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_simak_fKbBillSiswaTkNonAktif.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_simak_fKbBillSiswaTkNonAktif";
		...
		this.sg = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:8,tag:0,
					colTitle:["No Invoice","Periode","Kode Param","Nama Parameter","Akun AR","Saldo Tagihan","Nilai Pelunasan","Nilai CD"],
					colWidth:[[7,6,5,4,3,2,1,0],[80,100,100,70,300,80,80,150]],
...
window.app_saku3_transaksi_yspt_simak_fKbBillSiswaTkNonAktif.extend(window.childForm);
window.app_saku3_transaksi_yspt_simak_fKbBillSiswaTkNonAktif.implement({
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.no_bill,a.periode,a.kode_param,a.nama,a.akun_piutang,a.tot_bill-isnull(b.tot_lunas,0) as saldo,dd.nilai+dd.nilai_cd as nilai,dd.nilai_cd,dd.id_bank  "+
							 "from ("+
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];							
						this.sg.appendData([line.no_bill,line.periode,line.kode_param,line.nama,line.akun_piutang,floatToNilai(line.saldo),floatToNilai(line.nilai),floatToNilai(line.nilai_cd)]);
					}
				} else this.sg.clear(1);
				this.sg.validasi();
