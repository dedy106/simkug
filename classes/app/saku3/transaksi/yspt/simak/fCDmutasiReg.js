window.app_saku3_transaksi_yspt_simak_fCDmutasiReg = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_simak_fCDmutasiReg.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_simak_fCDmutasiReg";
		this.itemsValue = new arrayList();
		this.maximize();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pemindahan PDD Siswa", 0);	
		...
		this.cb_nim2 = new saiCBBL(this.pc2.childPage[0],{bound:[20,18,220,20],caption:"ID Reg Penerima", multiSelection:false, maxLength:10, tag:1});				
		this.cb_nim = new saiCBBL(this.pc2.childPage[0],{bound:[20,19,220,20],caption:"ID Reg Pengirim", multiSelection:false, maxLength:10, tag:1, change:[this,"doChange"]});				
		...
		this.e_nilai = new saiLabelEdit(this.pc2.childPage[0],{bound:[20,16,200,20],caption:"Nilai Transfer", tag:1, tipeText:ttNilai, text:"0"});				
		...
		try {
			...
			this.cb_nim.setSQL("select no_reg, nama from sis_siswareg where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"'",["no_reg","nama"],false,["ID","Nama"],"and","Daftar Calon Siswa",true);
			this.cb_nim2.setSQL("select no_reg, nama from sis_siswareg where kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"'",["no_reg","nama"],false,["ID","Nama"],"and","Daftar Calon Siswa",true);
			...
			if (this.akunCD == "") {
				system.alert(this,"SPRO PDD (SISCD) tidak ditemukan.","");
			}
...
window.app_saku3_transaksi_yspt_simak_fCDmutasiReg.extend(window.childForm);
window.app_saku3_transaksi_yspt_simak_fCDmutasiReg.implement({
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into sis_rekon_m(no_rekon,no_dokumen,tanggal,keterangan,nilai,posted,modul,akun_titip,nis,nik_buat,nik_app,kode_lokasi,periode,nik_user,tgl_input,kode_pp) values "+
						    "('"+this.e_nb.getText()+"','"+this.cb_nim2.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"',"+parseNilai(this.e_nilai.getText())+",'F','CDMUTREG','"+this.akunCD+"','"+this.cb_nim.getText()+"','"+this.app._userLog+"','"+this.app._userLog+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.cb_pp.getText()+"')");					
					...
					sql.add("insert into sis_cd_d(no_bukti,nis,periode,nilai,kode_lokasi,akun_cd,kode_param,dc,modul,kode_pp,no_ref1) values "+
							"('"+this.e_nb.getText()+"','"+this.cb_nim.getText()+"','"+this.e_periode.getText()+"',"+parseNilai(this.e_nilai.getText())+",'"+this.app._lokasi+"','"+this.akunCD+"','-','C','CDMUTREG','"+this.cb_pp.getText()+"','-')");
					sql.add("insert into sis_cd_d(no_bukti,nis,periode,nilai,kode_lokasi,akun_cd,kode_param,dc,modul,kode_pp,no_ref1) values "+
	...
	doChange: function(sender){
		if (sender == this.cb_nim && this.cb_nim.getText()!="") {
			var strSQL = "select isnull(sum(case a.dc when 'D' then a.nilai else -a.nilai end) ,0) as saldo_cd "+
						 "from sis_cd_d a inner join sis_siswareg b on a.nis=b.no_reg and a.kode_lokasi=b.kode_lokasi and b.kode_pp='"+this.cb_pp.getText()+"' "+
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_rekon,convert(varchar,a.tanggal,103) as tgl,a.nis,a.keterangan "+
		             "from sis_rekon_m a "+
					 "where a.kode_pp='"+this.cb_pp.getText()+"' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'CDMUTREG' and a.posted ='F'";
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			this.dataJU3 = data;
			...
			if (this.sg3.cells(0,row) != "") {
				...
				if (typeof data == "object"){
					...
					if (line != undefined){										
						...
						this.cb_nim.setSQL("select no_reg, nama from sis_siswareg where no_reg ='"+line.nis+"' and kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"'",["no_reg","nama"],false,["ID","Nama"],"and","Daftar Siswa",true);		
						this.cb_nim.setText(line.nis);											
						this.cb_nim2.setSQL("select no_reg, nama from sis_siswareg where no_reg ='"+line.no_dokumen+"' and kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.app._kodePP+"'",["no_reg","nama"],false,["ID","Nama"],"and","Daftar Siswa",true);		
						this.cb_nim2.setText(line.no_dokumen);											
					}
				}				
			}						
		} catch(e) {alert(e);}
