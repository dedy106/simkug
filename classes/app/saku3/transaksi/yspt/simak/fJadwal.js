window.app_saku3_transaksi_yspt_simak_fJadwal = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_simak_fJadwal.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_simak_fJadwal";
		...
		this.cb_pp = new portalui_saiCBBL(this,{bound:[20,11,220,20],caption:"Sekolah", readOnly:true, tag:2});
		...
		this.pc1 = new pageControl(this,{bound:[20,12,1000,410], childPage:["Data Jadwal","Daftar Jadwal"]});				
		...
		this.pc2 = new pageControl(this.pc1.childPage[0],{bound:[1,12,995,308], childPage:["Detail Jadwal"]});		
		this.sg = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:9,tag:0,
		...
		try {
			...
			this.cb_guru.setSQL("select nik, nama from karyawan where status='GURU' and flag_aktif='1' and kode_lokasi = '"+this.app._lokasi+"'",["nik","nama"],false,["NIK","Nama"],"and","Data Pegawai",true);			
...
window.app_saku3_transaksi_yspt_simak_fJadwal.extend(window.childForm);
window.app_saku3_transaksi_yspt_simak_fJadwal.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("delete from sis_jadwal where kode_ta = '"+this.cb_ta.getText()+"' and nik = '"+this.cb_guru.getText()+"' "+
							...
							if (this.sg.rowValid(i)){		
								for (var j=2;j < 9;j++){
									if(this.sg.cells(j,i) == "ISI"){
										var kodeHari = j-1;
										sql.add("insert into sis_jadwal(kode_slot,kode_lokasi,kode_pp,kode_kelas,kode_hari,kode_ta,nik,kode_matpel) values "+
		...
		try{
			if(this.cb_guru.getText() != ""){
				this.cb_matpel.setSQL("select a.kode_matpel, b.nama from sis_guru_matpel a inner join sis_matpel b on a.kode_matpel=b.kode_matpel and a.kode_lokasi=b.kode_lokasi "+
									  "where a.nik='"+this.cb_guru.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"'",["kode_matpel","nama"],false,["Kode Matpel","Nama"],"and","Data Matpel",true);	
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	doCari:function(sender){								
		...
		var strSQL = "select a.kode_slot,a.nama "+
					 "from  sis_slot a "+
					 "where a.kode_pp='"+this.app._kodePP+"' and a.kode_lokasi='"+this.app._lokasi+"' order by a.kode_slot ";	
		var data = this.dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object" && data.rs.rows[0] != undefined){
			...
			for (var i in data.rs.rows){
				...
				var strSQL2 = "select kode_hari,kode_matpel,nik from sis_jadwal where kode_slot='"+line2.kode_slot+"' and kode_ta='"+this.cb_ta.getText()+"' and kode_kelas='"+this.cb_kelas.getText()+"' and kode_pp='"+this.app._kodePP+"' "; 
				...
				if (typeof dataJadwal == "object" && dataJadwal.rs.rows[0] != undefined){
					...
					for (var j in dataJadwal.rs.rows){
						...
						if (lineJ.kode_hari == "1") {
							if (lineJ.kode_matpel == this.cb_matpel.getText() && lineJ.nik == this.cb_guru.getText()) senin  = "ISI";
							else senin  = "TERPAKAI";
						}
						if (lineJ.kode_hari == "2") {
							if (lineJ.kode_matpel == this.cb_matpel.getText() && lineJ.nik == this.cb_guru.getText()) selasa  = "ISI";
							else selasa  = "TERPAKAI";
						}
						if (lineJ.kode_hari == "3") {
							if (lineJ.kode_matpel == this.cb_matpel.getText() && lineJ.nik == this.cb_guru.getText()) rabu  = "ISI";
							else rabu  = "TERPAKAI";
						}
						if (lineJ.kode_hari == "4") {
							if (lineJ.kode_matpel == this.cb_matpel.getText() && lineJ.nik == this.cb_guru.getText()) kamis  = "ISI";
							else kamis  = "TERPAKAI";
						}
						if (lineJ.kode_hari == "5") {
							if (lineJ.kode_matpel == this.cb_matpel.getText() && lineJ.nik == this.cb_guru.getText()) jumat  = "ISI";
							else jumat  = "TERPAKAI";
						}
						if (lineJ.kode_hari == "6") {
							if (lineJ.kode_matpel == this.cb_matpel.getText() && lineJ.nik == this.cb_guru.getText()) sabtu  = "ISI";
							else sabtu  = "TERPAKAI";
						}
						if (lineJ.kode_hari == "7") {
							if (lineJ.kode_matpel == this.cb_matpel.getText() && lineJ.nik == this.cb_guru.getText()) minggu  = "ISI";
	...
	doLoad:function(sender){								
		var strSQL = "select distinct kode_ta,kode_matpel,nik,kode_kelas from sis_jadwal "+
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line.kode_ta,line.nik,line.kode_matpel,line.kode_kelas]); 
		}
