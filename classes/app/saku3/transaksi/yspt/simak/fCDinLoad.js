window.app_saku3_transaksi_yspt_simak_fCDinLoad = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_simak_fCDinLoad.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_simak_fCDinLoad";
		...
		this.sg2 = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:4,tag:9,
		            colTitle:["IDBank","NIS","Nilai Bayar","Val-IDReg"],					
...
window.app_saku3_transaksi_yspt_simak_fCDinLoad.extend(window.childForm);
window.app_saku3_transaksi_yspt_simak_fCDinLoad.implement({
	...
	doRekon: function(sender){				
		try {
			var strSQL = "select a.id_bank,a.nis from sis_siswa a where a.kode_lokasi='"+this.app._lokasi+"' and a.kode_pp='"+this.cb_pp.getText()+"'";		
			...
			for (var i=0; i < this.sg2.getRowCount();i++){
				...
				for (var j=0;j < this.dataNo.rs.rows.length;j++){					
					if (this.sg2.cells(0,i) == this.dataNo.rs.rows[j].id_bank) {						
						this.sg2.cells(1,i,this.dataNo.rs.rows[j].nis);
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i) && this.sg2.cells(3,i) == "VALID"){								
								sql.add("insert into sis_cd_d(no_bukti,nis,periode,nilai,kode_lokasi,akun_cd,kode_param,dc,modul,kode_pp,no_ref1) values "+
										"('"+this.e_nb.getText()+"','"+this.sg2.cells(1,i)+"','"+this.e_periode.getText()+"',"+parseNilai(this.sg2.cells(2,i))+",'"+this.app._lokasi+"','"+this.akunCD+"','-','D','CDINLOAD','"+this.cb_pp.getText()+"','-')");
							}
						}
					}	
	...
	doDoubleClick: function(sender, col , row) {
		try{
			if (this.sg1.cells(0,row) != "") {			
				...
				var strSQL = "select a.nis,a.nilai,b.id_bank from sis_cd_d a inner join sis_siswa b on a.nis=b.nis and a.kode_lokasi=b.kode_lokasi where a.no_bukti='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' ";										
