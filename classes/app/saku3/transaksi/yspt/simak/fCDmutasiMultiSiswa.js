window.app_saku3_transaksi_yspt_simak_fCDmutasiMultiSiswa = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_simak_fCDmutasiMultiSiswa.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_simak_fCDmutasiMultiSiswa";
		this.itemsValue = new arrayList();
		this.maximize();	
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pemindahan CD NIS ke NIS", 0);	
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,340], childPage:["Data PDD","Error Msg","Rekap Saldo"]});
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[0],{bound:[0,5,this.pc1.width-5,315],colCount:5,
					colTitle:["NIS Pemberi","Nilai Mutasi","NIS Penerima","Val-NIS","Val-NIS"],
					colWidth:[[4,3,2,1,0],[80,80,200,100,200]],
					colFormat:[[1],[cfNilai]],
		...
		this.sg4 = new portalui_saiGrid(this.pc1.childPage[2],{bound:[1,5,this.pc1.width-5,this.pc1.height-10],colCount:3,tag:9,
			colTitle:["NIS Pemberi","Saldo","Tot Mutasi"],
			colFormat:[[1,2],[cfNilai,cfNilai]],
			colWidth:[[2,1,0],[100,100,200]],autoAppend:false,
			readOnly:true, defaultRow:1
		});
...
window.app_saku3_transaksi_yspt_simak_fCDmutasiMultiSiswa.extend(window.childForm);
window.app_saku3_transaksi_yspt_simak_fCDmutasiMultiSiswa.implement({
	...
	doValid: function(sender){					
		...
		for (var i=0; i < this.sg1.getRowCount();i++){
			...
			for (var j=0;j < this.dataNIS.rs.rows.length;j++){
				if (this.sg1.cells(2,i) == this.dataNIS.rs.rows[j].nis) {					
		...
		if (this.inValid) {
			...
			for (var i=0; i < this.sg1.getRowCount();i++) {
				if (this.sg1.cells(3,i) == "INVALID") {
					var j = i+1;
					this.sg2.appendData([j,"NIS PEMBERI"]);						
				}
				if (this.sg1.cells(4,i) == "INVALID") {
					var j = i+1;
					this.sg2.appendData([j,"NIS PENERIMA"]);						
				}
			}
		}			
		else {
			...
			this.sg1.validasi();
			this.sg4.clear();
			var totmutasi = nmutasi = 0;
			for (var i=0;i < this.sg1.rows.getLength();i++){						
				if (this.sg1.rowValid(i)) {
					var isAda = false;
					var idx = totmutasi = 0;					
					nmutasi = nilaiToFloat(this.sg1.cells(1,i));

					for (var j=0;j < this.sg4.getRowCount();j++){
						if (this.sg1.cells(0,i) == this.sg4.cells(0,j)) {
							isAda = true;
							idx = j;
							break;
						}
					...
					if (!isAda) {
						this.sg4.appendData([this.sg1.cells(0,i),"0",floatToNilai(nmutasi)]);
					} 
					else { 
						totmutasi = nilaiToFloat(this.sg4.cells(2,idx));
						totmutasi = totmutasi + nmutasi;
						this.sg4.cells(2,idx,totmutasi);						
					}								
				}
			}
			this.doHitungSaldo();
	...
	doHitungSaldo: function() {
		for (var j=0;j < this.sg4.getRowCount();j++) {
			if (this.sg4.rowValid(i)) {			
				var strSQL = "select sum(case dc when 'D' then nilai else -nilai end) as saldo_cd "+
							" from sis_cd_d "+
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into sis_rekon_m(no_rekon,no_dokumen,tanggal,keterangan,nilai,posted,modul,akun_titip,nis,nik_buat,nik_app,kode_lokasi,periode,nik_user,tgl_input,kode_pp) values "+
						    "('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"',"+parseNilai(this.e_nilai.getText())+",'F','CDMUTSIS','"+this.akunCD+"','-','"+this.app._userLog+"','"+this.app._userLog+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+this.app._userLog+"',getdate(),'"+this.cb_pp.getText()+"')");					
					...
					sql.add("insert into sis_rekon_j(no_rekon,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',0,'"+this.akunCD+"','"+this.e_ket.getText()+"','D',"+parseNilai(this.e_nilai.getText())+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','CDMUTSIS','TTP','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");							
					sql.add("insert into sis_rekon_j(no_rekon,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) values "+
							"('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"',1,'"+this.akunCD+"','"+this.e_ket.getText()+"','C',"+parseNilai(this.e_nilai.getText())+",'"+this.cb_pp.getText()+"','-','"+this.app._lokasi+"','CDMUTSIS','CD','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate())");								
					...
					for (var i=0; i < this.sg1.getRowCount();i++){
						if (nilaiToFloat(this.sg1.cells(2,i)) != 0) {
							sql.add("insert into sis_cd_d(no_bukti,nis,periode,nilai,kode_lokasi,akun_cd,kode_param,dc,modul,kode_pp,no_ref1) values "+
									"('"+this.e_nb.getText()+"','"+this.sg1.cells(0,i)+"','"+this.e_periode.getText()+"',"+parseNilai(this.sg1.cells(1,i))+",'"+this.app._lokasi+"','"+this.akunCD+"','-','C','CDMUTSIS','"+this.cb_pp.getText()+"','"+this.sg1.cells(2,i)+"')");
							sql.add("insert into sis_cd_d(no_bukti,nis,periode,nilai,kode_lokasi,akun_cd,kode_param,dc,modul,kode_pp,no_ref1) values "+
									"('"+this.e_nb.getText()+"','"+this.sg1.cells(2,i)+"','"+this.e_periode.getText()+"',"+parseNilai(this.sg1.cells(1,i))+",'"+this.app._lokasi+"','"+this.akunCD+"','-','D','CDMUTSIS','"+this.cb_pp.getText()+"','"+this.sg1.cells(0,i)+"')");										
						}
					}
		...
		switch (event){
			...
			case "ubah" :	
				...
				for (var i=0; i < this.sg4.getRowCount();i++){
					if (nilaiToFloat(this.sg4.cells(2,i)) > nilaiToFloat(this.sg4.cells(1,i))) {
						var j= i+1;
						system.alert(this,"Transaksi tidak valid.","Nilai tidak boleh melebihi Saldo. (Baris Rekap :"+j+")");
						return false;							
					}
				}
	...
	doClick:function(sender){
		if (sender == this.i_gen) {		
			...
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"sis_rekon_m","no_rekon",this.app._lokasi+"-CDS"+this.e_periode.getText().substr(2,4)+".","0000"));
			this.e_ket.setFocus();
			this.stsSimpan = 1;
		}
	},		
	...
	doLoad3:function(sender){																
		var strSQL = "select a.no_rekon,convert(varchar,a.tanggal,103) as tgl,a.keterangan "+
		             "from sis_rekon_m a "+
					 "where a.kode_pp='"+this.cb_pp.getText()+"' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.modul = 'CDMUTSIS' and a.posted ='F'";
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_rekon,line.tgl,line.keterangan]); 
		}
	...
	doDoubleClick3: function(sender, col , row) {
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select  a.* from sis_cd_d a "+							
				...
				if (typeof data == "object" && data.rs.rows[0] != undefined){
					...
					for (var i in data.rs.rows){
						line = data.rs.rows[i];				
						this.sg1.appendData([line.nis,floatToNilai(line.nilai),line.no_ref1,"VALID","VALID"]);
					}
