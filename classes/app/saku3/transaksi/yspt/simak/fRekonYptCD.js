window.app_saku3_transaksi_yspt_simak_fRekonYptCD = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_simak_fRekonYptCD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_simak_fRekonYptCD";
...
window.app_saku3_transaksi_yspt_simak_fRekonYptCD.extend(window.childForm);
window.app_saku3_transaksi_yspt_simak_fRekonYptCD.implement({
						...
						for (var i=0;i < this.sg.getRowCount();i++){
							if (this.sg.rowValid(i) && this.sg.cells(6,i) != "0"){
								...
								sql.add("insert into sis_rekon_d(no_rekon,nis,no_bill,periode,nilai,kode_lokasi,akun_titip,akun_piutang,kode_param,dc,modul,id_bank,kode_pp, nilai_cd,periode_bill) values "+
										"	('"+this.e_nb.getText()+"','"+this.cb_nim.getText()+"','"+this.sg.cells(0,i)+"','"+this.e_periode.getText()+"',"+nilaiAR+",'"+this.app._lokasi+"','"+this.cb_titip.getText()+"','"+this.sg.cells(4,i)+"','"+this.sg.cells(2,i)+"','D','REKONCD','-','"+this.cb_pp.getText()+"', 0,'"+this.sg.cells(1,i)+"')");
	...
	doChange: function(sender){
		if (sender == this.cb_nim && this.cb_nim.getText()!="" && this.stsSimpan==1) {
			...
			var strSQL = "select a.no_bill,a.periode,a.kode_param,a.nama,a.akun_piutang,a.tot_bill-isnull(b.tot_lunas,0) as saldo "+
						 "from ("+
			...
			var strSQL = "select isnull(sum(case dc when 'D' then nilai else -nilai end),0) as saldo from sis_cd_d where nis='"+this.cb_nim.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and kode_pp='"+this.cb_pp.getText()+"'";						
			var data = this.dbLib.getDataProvider(strSQL,true);
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				*/
				...
				var strSQL = "select a.no_bill,a.periode,a.kode_param,a.nama,a.akun_piutang,a.tot_bill-isnull(b.tot_lunas,0) as saldo,dd.nilai,dd.nilai_cd,dd.id_bank  "+
							 "from ("+
	
							 "		select a.nis,a.no_bill,a.periode,a.akun_piutang,sum(a.nilai) as tot_bill,a.kode_param,aa.nama,c.idx,a.kode_lokasi "+
							 "		from sis_bill_d a "+
							 "			inner join sis_bill_m e on a.no_bill=e.no_bill and a.kode_lokasi=e.kode_lokasi "+
							 "      	inner join sis_param aa on a.kode_param=aa.kode_param and a.kode_lokasi=aa.kode_lokasi "+
							 "      	inner join sis_kelas d on a.kode_kelas=d.kode_kelas and a.kode_lokasi=d.kode_lokasi and e.kode_pp=d.kode_pp "+						
							 "      	inner join sis_param_idx c on a.kode_param = c.kode_param and a.kode_lokasi=c.kode_lokasi and a.kode_pp=c.kode_pp and c.kode_pp='"+this.cb_pp.getText()+"' "+						
							 "		where a.nis = '"+this.cb_nim.getText()+"' and a.periode<='"+this.e_periode.getText()+"' and e.kode_pp= '"+this.cb_pp.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' "+
							 "		group by a.nis,a.no_bill,a.periode,a.akun_piutang,a.kode_param,aa.nama,c.idx,a.kode_lokasi "+
							 ") a "+
				...
				var strSQL = "select isnull(sum(case dc when 'D' then nilai else -nilai end),0) as saldo from sis_cd_d where nis='"+this.cb_nim.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and no_bukti<>'"+this.e_nb.getText()+"' and kode_pp='"+this.cb_pp.getText()+"'";						
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){										
