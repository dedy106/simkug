window.app_saku3_transaksi_yspt_simak_fCDinReg = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_simak_fCDinReg.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_simak_fCDinReg";
		...
		this.sg2 = new saiGrid(this.pc2.childPage[0],{bound:[1,5,this.pc2.width-5,this.pc2.height-35],colCount:3,tag:9,
		            colTitle:["ID Registrasi","Nilai Bayar","Val-IDReg"],					
...
window.app_saku3_transaksi_yspt_simak_fCDinReg.extend(window.childForm);
window.app_saku3_transaksi_yspt_simak_fCDinReg.implement({
	...
	doRekon: function(sender){				
		try {
			var strSQL = "select a.no_reg from sis_siswareg a "+
			...
			for (var i=0; i < this.sg2.getRowCount();i++){
				...
				if (this.sg2.cells(2,i) == "VALID") tot += nilaiToFloat(this.sg2.cells(1,i));									
			}
						...
						for (var i=0;i < this.sg2.getRowCount();i++){
							if (this.sg2.rowValid(i) && this.sg2.cells(2,i) == "VALID"){
								...
								sql.add("insert into sis_cd_d(no_bukti,nis,periode,nilai,kode_lokasi,akun_cd,kode_param,dc,modul,kode_pp,no_ref1) values "+
										"('"+this.e_nb.getText()+"','"+this.sg2.cells(0,i)+"','"+this.e_periode.getText()+"',"+parseNilai(this.sg2.cells(1,i))+",'"+this.app._lokasi+"','"+this.akunCD+"','-','D','CDINREG','"+this.cb_pp.getText()+"','-')");
							}
						}
					}	
