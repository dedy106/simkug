window.app_saku3_transaksi_yspt_sistem_fUserAkses = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_sistem_fUserAkses.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_sistem_fUserAkses";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","User HakAkses", 0);	
		
		uses("saiCBBL;saiEdit;saiGrid;sgNavigator;checkBox");		
		this.cb_kode = new portalui_saiCBBL(this,{bound:[20,10,220,20],caption:"NIK User",tag:2,multiSelection:false,change:[this,"doChange"]}); 						
		...
		this.e_pass2 = new saiEdit(this,{bound:[350,12,100,20],caption:"Password", password:false,maxLength:10,visible:false,readOnly:true});	
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select distinct kode_klp from menu where kode_klp like 'MOB%'",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
...
window.app_saku3_transaksi_yspt_sistem_fUserAkses.extend(window.childForm);
window.app_saku3_transaksi_yspt_sistem_fUserAkses.implement({
