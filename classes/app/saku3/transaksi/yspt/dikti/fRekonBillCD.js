window.app_saku3_transaksi_yspt_dikti_fRekonBillCD = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_dikti_fRekonBillCD.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_dikti_fRekonBillCD";
		this.itemsValue = new arrayList();
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pelunasan Billing via CD", 0);	
		...
		this.cb_titip = new saiCBBL(this,{bound:[20,17,220,20],caption:"Akun Pelunasan", multiSelection:false, maxLength:10, tag:2 });		
		this.bTampil = new button(this,{bound:[650,17,100,20],caption:"Data Bill",click:[this,"doLoad"]});			
		
		this.pc1 = new pageControl(this,{bound:[5,12,1000,380], childPage:["Data Tagihan","Data Pelunasan","Hapus Bukti"]});
		...
		this.sg1 = new portalui_saiGrid(this.pc1.childPage[1],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:2,
			  colTitle:["NIM","Saldo PDD"],
		...
		this.c_periode = new saiCB(this.pc1.childPage[2],{bound:[20,22,200,20],caption:"Periode",readOnly:true,tag:9, change:[this,"doChange"]});
		this.cb_bukti = new saiCBBL(this.pc1.childPage[2],{bound:[20,16,220,20],caption:"No Bukti", multiSelection:false, maxLength:10, tag:9, change:[this,"doChange"]});
		this.e_nilai2 = new saiLabelEdit(this.pc1.childPage[2],{bound:[20,18,200,20],caption:"Nilai", tag:9, readOnly:true, tipeText:ttNilai, text:"0"});						
		this.bDel = new button(this.pc1.childPage[2],{bound:[120,16,80,18],caption:"Hapus Bukti",click:[this,"doDelete"]});
		...
		this.bLoadCD = new button(this.pc1.childPage[1],{bound:[420,10,100,20],caption:"Load Saldo PDD",click:[this,"doLoadCD"]});			
		this.bRekon = new button(this.pc1.childPage[1],{bound:[530,10,100,20],caption:"Rekon Pelunasan", click:[this,"doRekon"]});
		...
		try {
			...
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				...
				this.cb_titip.setText(line.flag);
			} 
			
			this.c_periode.items.clear();
			var data = this.dbLib.getDataProvider("select distinct periode from trans_m where form = 'REKONCD' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
...
window.app_saku3_transaksi_yspt_dikti_fRekonBillCD.extend(window.childForm);
window.app_saku3_transaksi_yspt_dikti_fRekonBillCD.implement({	
	doDelete: function() {		
		if (this.cb_bukti.getText() != "") {
			var sql = new server_util_arrayList();
			sql.add("delete from trans_m where no_bukti='"+this.cb_bukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
			sql.add("delete from trans_j where no_bukti='"+this.cb_bukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
	...
	doLoadCD: function(sender,totalRow){
		try {
			var strSQL = "select * from  ("+
						 ...
						 "	select a.nim,sum(case a.dc when 'D' then a.nilai else -a.nilai end) as saldo_cd "+
						 "	from dikti_cd a inner join dikti_mhs b on a.nim=b.nim and a.kode_lokasi=b.kode_lokasi "+
						 "	where a.nilai <> 0 and a.periode <= '"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' "+
						 "	group by a.nim "+
	...
	doTampilData1: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1.appendData([line1.nim,floatToNilai(line1.saldo_cd)]);		
		}
		...
		try {
			...
			for (var i=0; i < this.dataJU1.rs.rows.length;i++){				
				...
				for (var j=0;j < this.dataJU.rs.rows.length;j++){
					if (this.dataJU1.rs.rows[i].nim == this.dataJU.rs.rows[j].nim) {
	...
	simpan: function(){			
		try{						
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"trans_m","no_bukti",this.app._lokasi+"-RCD"+this.e_periode.getText().substr(2,4)+".","0000"));
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,due_date,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','MI','REKONCD','F','-','-','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','"+this.dp_d1.getDateString()+"', '-','"+this.e_ket.getText()+"','IDR',1,"+
							parseNilai(this.e_nilai.getText())+",0,0,'-','-','-','-','-','-','"+this.cb_titip.getText()+"','-','-')");
					...
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							...
							parseNilai(this.e_nilai.getText())+",'"+this.e_ket.getText()+"','REKONCD','TTP','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");											
					...
					for (var i in this.gridAR.objList){
						...
						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							...
							parseFloat(line.get("nilai"))+",'"+this.e_ket.getText()+"','REKONCD','PIUT','IDR',1,'"+line.get("kode_pp")+"','-','-','-','-','-','-','-','-')");											
					}												
					...
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						...
						if (parseFloat(line.lunas) != 0){									
							sql.add("insert into dikti_bill_rekon(no_rekon,kode_lokasi,periode,no_bill,nim,kode_param,akun_titip,akun_piutang,nilai,nilai_cd,dc,modul) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+line.no_bill+"','"+line.nim+"','"+line.kode_param+"','"+this.cb_titip.getText()+"','"+line.akun_piutang+"',"+parseFloat(line.lunas)+",0,'D','REKONCD')");

							sql.add("insert into dikti_cd(no_bukti,nim,periode,nilai,kode_lokasi,akun_cd,kode_param,dc,modul,no_ref1) values "+
									"('"+this.e_nb.getText()+"','"+line.nim+"','"+this.e_periode.getText()+"',"+parseFloat(line.lunas)+",'"+this.app._lokasi+"','"+this.cb_titip.getText()+"','-','C','REKONCD','-')");
	...
	doChange: function(sender) {
		if (sender == this.c_periode && this.c_periode.getText()!="") {
			this.cb_bukti.setSQL("select no_bukti,keterangan from trans_m where posted='F' and kode_lokasi='"+this.app._lokasi+"' and form='REKONCD' and periode='"+this.c_periode.getText()+"'",["no_bukti","keterangan"],false,["Bukti","Deskripsi"],"and","Daftar Pelunasan",true);	
		}
		if (sender == this.cb_bukti && this.cb_bukti.getText()!="") {
			var strSQL = "select nilai1 from trans_m where no_bukti='"+this.cb_bukti.getText()+"' and kode_lokasi='"+this.app._lokasi+"'";
	...
	doLoad: function(sender){			
		...
		var strSQL = "SELECT a.nim,a.nama,a.no_bill,a.akun_piutang,a.tot_bill-isnull(b.tot_lunas,0) as saldo,0 as lunas,0 as sisa,a.kode_param,a.idx,a.kode_pp "+
						...
						"("+
						"		select "+
						"		a.nim,c.nama,a.no_bill,a.akun_piutang,a.kode_param,d.idx,a.kode_pp,sum(a.nilai) as tot_bill "+
						"		from dikti_bill_d a "+
