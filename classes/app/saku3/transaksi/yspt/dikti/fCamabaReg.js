window.app_saku3_transaksi_yspt_dikti_fCamabaReg = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_dikti_fCamabaReg.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_dikti_fCamabaReg";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pendaftaran Calon Mahasiswa Baru", 0);	
		
		uses("portalui_uploader;saiCBBL;saiEdit;datePicker;saiGrid;sgNavigator;pageControl;checkBox");
		uses("saiGrid",true);		

		this.cb_ta = new portalui_saiCBBL(this,{bound:[20,10,220,20],caption:"Periode Akademik",multiSelection:false,readOnly:true,tag:2,change:[this,"doChange"]});					
		this.l_tgl = new portalui_label(this,{bound:[20,11,100,18],caption:"Tanggal", underline:true});
		this.dp_d1 = new portalui_datePicker(this,{bound:[120,11,100,18],selectDate:[this,"doSelectDate"],date:new Date().getDateStr()}); 		
		
		this.pc1 = new pageControl(this,{bound:[1,12,1000,410], childPage:["Daftar Calon MHS","Data Calon MHS","Data Orgtua/Wali","Riwayat Kesehatan"]});				
		this.sg1 = new saiGrid(this.pc1.childPage[0],{bound:[1,5,this.pc1.width-5,this.pc1.height-33],colCount:2,tag:2,
		...
		this.e_tlpsiswa = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,18,200,20],caption:"Telp Mhs", maxLength:50, tag:1});								
		this.e_hpsiswa = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,20,200,20],caption:"HP Mhs", maxLength:50, tag:1});
		this.e_alamat = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,16,390,20],caption:"Alamat Mhs", maxLength:200, tag:1});
		this.e_email = new saiLabelEdit(this.pc1.childPage[1],{bound:[20,19,390,20],caption:"Email Mhs", maxLength:50, tag:1});	
		...
		this.e_hub = new saiLabelEdit(this.pc1.childPage[2],{bound:[20,28,390,20],caption:"Hub. dgn Mhs", maxLength:50, tag:2});	
...
window.app_saku3_transaksi_yspt_dikti_fCamabaReg.extend(window.childForm);
window.app_saku3_transaksi_yspt_dikti_fCamabaReg.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try {									
					uses("server_util_arrayList");
					var sql = new server_util_arrayList();					
					sql.add("delete from dikti_camaba where no_reg = '"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");			
					sql.add("insert into dikti_camaba(no_reg,kode_ta,tanggal,nama,agama,jk,tmp_lahir,tgl_lahir,kwn,foto,alamat_siswa,tlp_siswa,hp_siswa,email,asal_sek,no_ijazah,nilai_un,nama_ayah,nama_ibu,alamat_ayah,alamat_ibu,kerja_ayah,kerja_ibu,pdd_ayah,pdd_ibu,hasil_ayah,hasil_ibu,hp_ayah,hp_ibu,email_ayah,email_ibu,nama_wali,alamat_wali,kerja_wali,hp_wali,email_wali,hub_wali,gol_darah,kond_kes,kond_obat,dokter_klg,telp_dokter,kode_lokasi) values "+
						      "('"+this.cb_kode.getText()+"','"+this.cb_ta.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_nama.getText()+"','"+this.c_agama.getText()+"','"+this.c_jk.getText()+"','"+this.e_tmplahir.getText()+"','"+this.dp_d2.getDateString()+"','"+this.c_kwn.getText()+"','"+this.e_foto.getText()+"','"+this.e_alamat.getText()+"','"+this.e_tlpsiswa.getText()+"','"+this.e_hpsiswa.getText()+"','"+this.e_email.getText()+"','"+this.e_asal.getText()+"','"+this.e_ijazah.getText()+"',"+nilaiToFloat(this.e_un.getText())+",'"+this.e_namaayah.getText()+"','"+this.e_namaibu.getText()+"','"+this.e_alamatayah.getText()+"','"+this.e_alamatibu.getText()+"','"+this.c_kerjaayah.getText()+"','"+this.c_kerjaibu.getText()+"','"+this.c_pddayah.getText()+"','"+this.c_pddibu.getText()+"',"+nilaiToFloat(this.e_hasilayah.getText())+","+nilaiToFloat(this.e_hasilibu.getText())+",'"+this.e_hpayah.getText()+"','"+this.e_hpibu.getText()+"','"+this.e_emailayah.getText()+"','"+this.e_emailibu.getText()+"','"+this.e_namawali.getText()+"','"+this.e_alamatwali.getText()+"','"+this.c_kerjawali.getText()+"','"+this.e_hpwali.getText()+"','"+this.e_emailwali.getText()+"','"+this.e_hub.getText()+"','"+this.c_darah.getText()+"','"+this.e_hismed.getText()+"','"+this.e_hisobat.getText()+"','"+this.e_dokter.getText()+"','"+this.e_tlpdokter.getText()+"','"+this.app._lokasi+"')");					
					setTipeButton(tbAllFalse);
					this.dbLib.execArraySQL(sql);
	...
	doClick:function(sender){	
			this.cb_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"dikti_camaba","no_reg",this.app._lokasi+"-REG"+this.period.substr(2,4)+".","0000"));
	...
	doChange: function(sender){
		try{
			if ((sender == this.cb_ta || this.cb_pp) && this.cb_ta.getText() != "" && this.cb_pp.getText() != "") {	
				var strSQL = "select no_reg, nama from dikti_camaba where kode_lokasi='"+this.app._lokasi+"' and kode_ta='"+this.cb_ta.getText()+"' ";
			...
			if (this.cb_kode.getText() != ""){
				var strSQL = "select * from dikti_camaba where no_reg ='"+this.cb_kode.getText()+"' and kode_lokasi='"+this.app._lokasi+"' ";						   
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){
