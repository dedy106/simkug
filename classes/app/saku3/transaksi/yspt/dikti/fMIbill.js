window.app_saku3_transaksi_yspt_dikti_fMIbill = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_yspt_dikti_fMIbill.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_yspt_dikti_fMIbill";
		this.itemsValue = new arrayList();
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pelunasan Billing NonCash", 0);	
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select distinct periode from trans_m where form='MHSNONKAS' and kode_lokasi='"+this.app._lokasi+"' order by periode desc",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
...
window.app_saku3_transaksi_yspt_dikti_fMIbill.extend(window.childForm);
window.app_saku3_transaksi_yspt_dikti_fMIbill.implement({
	doDelete: function() {		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,due_date,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"',getdate(),'"+this.app._userLog+"','"+this.e_periode.getText()+"','MI','MHSNONKAS','F','-','-','"+this.app._kodePP+"','"+this.dp_d1.getDateString()+"','"+this.dp_d1.getDateString()+"', '-','"+this.e_ket.getText()+"','IDR',1,"+
					...
					sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
							...
							total+",'"+this.e_ket.getText()+"','MHSNONKAS','TITIP','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");											
					...
					for (var i in this.gridAR.objList){
						...
						sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
								...
								parseFloat(line.get("nilai"))+",'"+this.e_ket.getText()+"','MHSNONKAS','PIUT','IDR',1,'"+line.get("kode_pp")+"','-','-','-','-','-','-','-','-')");											
					}		
					...
					for (var i=0;i < this.dataJU.rs.rows.length;i++){
						...
						if (parseFloat(line.lunas) != 0){									
							sql.add("insert into dikti_bill_rekon(no_rekon,kode_lokasi,periode,no_bill,nim,kode_param,akun_titip,akun_piutang,nilai,nilai_cd,dc,modul) values "+
									"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','"+line.no_bill+"','"+line.nim+"','"+line.kode_param+"','"+this.cb_titip.getText()+"','"+line.akun_piutang+"',"+parseFloat(line.lunas)+",0,'D','MHSNONKAS')");
						}						
					}
					...
					for (var i=0; i < this.sg1.getRowCount();i++){
						if (nilaiToFloat(this.sg1.cells(2,i)) != 0) {				
							sql.add("insert into dikti_cd(no_bukti,nim,periode,nilai,kode_lokasi,akun_cd,kode_param,dc,modul,no_ref1) values "+
									"('"+this.e_nb.getText()+"','"+this.sg1.cells(0,i)+"','"+this.e_periode.getText()+"',"+nilaiToFloat(this.sg1.cells(2,i))+",'"+this.app._lokasi+"','"+this.akunCD+"','-','D','MHSNONKAS','-')");
							...
							sql.add("insert into trans_j (no_bukti,kode_lokasi,tgl_input,nik_user,periode,no_dokumen,tanggal,nu,kode_akun,dc,nilai,nilai_curr,keterangan,modul,jenis,kode_curr,kurs,kode_pp,kode_drk,kode_cust,kode_vendor,no_fa,no_selesai,no_ref1,no_ref2,no_ref3) values "+
									...
									nilaiToFloat(this.sg1.cells(2,i))+",'"+this.e_ket.getText()+"','MHSNONKAS','CD','IDR',1,'"+this.app._kodePP+"','-','-','-','-','-','-','-','-')");
						}
					}	
	...
	doChange: function(sender) {
		if (sender == this.c_periode && this.c_periode.getText()!="") {
			this.cb_bukti.setSQL("select no_bukti,keterangan from trans_m where posted='F' and kode_lokasi='"+this.app._lokasi+"' and form='MHSNONKAS' and periode='"+this.c_periode.getText()+"'",["no_bukti","keterangan"],false,["Bukti","Deskripsi"],"and","Daftar Bukti",true);	
		}
	...
	doLoad: function(sender){			
		...
		var strSQL = "SELECT a.nim,a.nama,a.no_bill,a.akun_piutang,a.tot_bill-isnull(b.tot_lunas,0) as saldo,0 as lunas,0 as sisa,a.kode_param,a.idx,a.kode_pp "+
						...
						"		group by a.nim,c.nama,a.no_bill,a.akun_piutang,a.kode_param,a.kode_pp,d.idx "+
						") a "+						 
						"LEFT JOIN "+						 
						"( "+
