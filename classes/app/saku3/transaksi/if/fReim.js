window.app_saku3_transaksi_if_fReim = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_if_fReim.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_if_fReim";
		this.itemsValue = new arrayList();
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Pengajuan Reimburse I/F: Input", 0);	
		...
		this.c_status = new saiCB(this.pc2.childPage[0],{bound:[790,13,200,20],caption:"Status",items:["IFREIM","TUTUP"], readOnly:true,tag:2});
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,12,995,322], childPage:["Data Reimburse","Cat. Approval"]});
		...
		try {
			...
			var data = this.dbLib.getDataProvider("select kode_spro,flag from spro where kode_spro = 'HUTIF' and kode_lokasi = '"+this.app._lokasi+"'",true);			
...
window.app_saku3_transaksi_if_fReim.extend(window.childForm);
window.app_saku3_transaksi_if_fReim.implement({	
		...
		try{						
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into if_reim_j(no_reim,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input) "+
							"select b.no_reim,a.no_aju,'"+this.dp_d1.getDateString()+"',a.nu,a.kode_akun,a.keterangan,a.dc,a.nilai,a.kode_pp,a.kode_drk,a.kode_lokasi,'"+this.c_status.getText()+"',a.jenis,'"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate() "+
	...
	doChange:function(sender){		
		if (sender == this.cb_nik && this.cb_nik.getText()!="") {
			var strSQL = "select a.kode_pp from if_m a where a.nik='"+this.cb_nik.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";						   
	...
	doLoad:function(sender){		
		if (this.cb_nik.getText() != "" && this.e_periode.getText() != "") {
			var data = this.dbLib.getDataProvider("select 'INPROG' as status,a.no_aju,convert(varchar,a.tanggal,103) as tgl,b.kode_akun+' - '+b.nama as akun,c.kode_drk+' - '+c.nama as drk,a.keterangan,a.nilai "+
