window.app_saku3_transaksi_if_fTutupIF = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_if_fTutupIF.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_if_fTutupIF";
		...
		this.cb_rek = new saiCBBL(this.pc2.childPage[0],{bound:[20,15,220,20],caption:"Rek KasBank", multiSelection:false, maxLength:10, tag:2,change:[this,"doChange"]});				
		this.cb_akun = new portalui_saiLabelEdit(this.pc2.childPage[0],{bound:[20,16,200,20],caption:"Kode Akun",maxLength:30,readOnly:true});
		this.e_nilaikb = new saiLabelEdit(this.pc2.childPage[0],{bound:[790,16,200,20],caption:"Nilai KasBank", readOnly:true,tipeText:ttNilai,text:"0"});						
		
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[1,18,995,300], childPage:["Data Pertanggungan","Detail Pertanggungan"]});				
		...
		this.sg3 = new saiGrid(this.pc1.childPage[1],{bound:[10,10,this.pc1.width-20,195],colCount:9,tag:9,
...
window.app_saku3_transaksi_if_fTutupIF.extend(window.childForm);
window.app_saku3_transaksi_if_fTutupIF.implement({
		...
		try{		
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					if (this.stsSimpan == 0) {
						sql.add("update if_reim_m set progress='2',no_kas='-' where no_reim='"+this.e_noptg.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");						
						sql.add("delete from kas_m where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
						sql.add("delete from kas_j where no_kas = '"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					}
					
					sql.add("update if_reim_m set progress='3',no_kas='"+this.e_nb.getText()+"' where no_reim='"+this.e_noptg.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");											
					sql.add("insert into kas_m (no_kas,kode_lokasi,no_dokumen,no_bg,akun_kb,tanggal,keterangan,kode_pp,modul,jenis,periode,kode_curr,kurs,nilai,nik_buat,nik_app,tgl_input,nik_user,posted,no_del,no_link,ref1,kode_bank) values  "+
							"('"+this.e_nb.getText()+"','"+this.app._lokasi+"','"+this.e_noptg.getText()+"','-','"+this.cb_akun.getText()+"','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','"+this.app._kodePP+"','KBIFTTP','"+this.c_jenis.getText()+"','"+this.e_periode.getText()+"','IDR',1,"+parseNilai(this.e_nilaikb.getText())+",'"+this.app._userLog+"','"+this.app._userLog+"',getdate(),'"+this.app._userLog+"','F','-','"+this.e_noptg.getText()+"','-','"+this.cb_rek.getText()+"')");
					
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) values "+
							"('"+this.e_nb.getText()+"','"+this.e_noptg.getText()+"','"+this.dp_d1.getDateString()+"',0,'"+this.cb_akun.getText()+"','"+this.e_ket.getText()+"','D',"+Math.abs(nilaiToFloat(this.e_nilaikb.getText()))+",'"+this.app._kodePP+"','-','-','-','"+this.app._lokasi+"','KBIFTTP','KB','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'"+this.cb_rek.getText()+"')");										
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) "+
							"select '"+this.e_nb.getText()+"','"+this.e_noptg.getText()+"','"+this.dp_d1.getDateString()+"',997,kode_akun,keterangan,'D',nilai,kode_pp,kode_drk,'-','-',kode_lokasi,'KBIFTTP',jenis,'"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),'-' "+
							"from if_reim_j where jenis ='HUTANG' and no_reim='"+this.e_noptg.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("insert into kas_j(no_kas,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,nilai,kode_pp,kode_drk,kode_cf,ref1,kode_lokasi,modul,jenis,periode,kode_curr,kurs,nik_user,tgl_input,kode_bank) "+
							"select '"+this.e_nb.getText()+"','"+this.e_noptg.getText()+"','"+this.dp_d1.getDateString()+"',999,b.flag,c.keterangan,'C',a.nilai,c.kode_pp,'-','-','-',c.kode_lokasi,'KBIFTTP','IF','"+this.e_periode.getText()+"','IDR',1,'"+this.app._userLog+"',getdate(),a.kode_rek "+
							"from if_m a inner join spro b on a.kode_lokasi=b.kode_lokasi and b.kode_spro='AKUNIF' "+
							"            inner join if_reim_m c on a.nik=c.nik_buat and a.kode_lokasi=c.kode_lokasi "+
	...
	doDoubleClick: function(sender, col , row) {
		if (this.sg.cells(0,row) != "") {			
			...
			var data = this.dbLib.getDataProvider("select a.kode_akun,b.nama as nama_akun,a.dc,a.keterangan,a.nilai,a.kode_pp,c.nama as nama_pp,a.kode_drk,isnull(d.nama,'-') as nama_drk "+
							...
							"where b.no_reim = '"+this.e_noptg.getText()+"' and b.kode_lokasi='"+this.app._lokasi+"' ",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg3.clear();
				for (var i in data.rs.rows){
	...
	doLoad:function(sender){						
		var strSQL = "select a.no_reim,c.nik+' - '+c.nama as pemohon,convert(varchar,a.tanggal,103) as tanggal,a.no_dokumen,a.keterangan,aa.nilai as nilai_if,a.nilai,aa.nilai-a.nilai as nilai_kas,d.flag as akun_panjar,b.kode_pp+' - '+b.nama as pp "+
		             "from if_reim_m a inner join if_m aa on a.nik_buat=aa.nik and a.kode_lokasi=aa.kode_lokasi "+					 
					 "inner join spro d on a.kode_lokasi=d.kode_lokasi and d.kode_spro = 'AKUNIF' "+
		             "inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi "+
					 "inner join karyawan c on aa.nik=c.nik and aa.kode_lokasi=c.kode_lokasi "+
					 "where a.modul='TUTUP' and a.periode<='"+this.e_periode.getText()+"' and a.progress='2' and a.kode_lokasi='"+this.app._lokasi+"'";			
	...
	doDoubleClickL: function(sender, col , row) {
		try{
			if (this.sgL.cells(0,row) != "") {
				...
				var data = this.dbLib.getDataProvider("select no_dokumen,tanggal,periode,keterangan,kode_bank,jenis,nilai "+						   
						   "from kas_m "+					   
