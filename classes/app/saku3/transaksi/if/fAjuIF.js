window.app_saku3_transaksi_if_fAjuIF = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_if_fAjuIF.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_if_fAjuIF";
		...
		this.cb_akun = new portalui_saiCBBL(this.pc2.childPage[0],{bound:[20,12,222,20],caption:"Akun",tag:2,multiSelection:false,change:[this,"doChange"]});         		
...
window.app_saku3_transaksi_if_fAjuIF.extend(window.childForm);
window.app_saku3_transaksi_if_fAjuIF.implement({		
				...
				try{														
					...
					if (this.stsGar == "1") {						
						sql.add("insert into angg_r(no_bukti,modul,kode_lokasi,kode_akun,kode_pp,kode_drk,periode1,periode2,dc,saldo,nilai) values "+
								"('"+this.e_nb.getText()+"','IFAJU','"+this.app._lokasi+"','"+this.cb_akun.getText()+"','"+this.cb_pp.getText()+"','"+this.cb_drk.getText()+"','"+this.e_periode.getText()+"','"+this.e_periode.getText()+"','D',"+nilaiToFloat(this.e_saldo.getText())+","+nilaiGar+")");
					}							
	...
	doChange:function(sender){
		...
		if ((sender == this.cb_pp ||sender == this.e_periode)  && this.cb_pp.getText() !="" && this.e_periode.getText() !="") {
				this.cb_akun.setSQL("select distinct a.kode_akun, a.nama from masakun a inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi "+
									"       inner join anggaran_d c on a.kode_akun=c.kode_akun and a.kode_lokasi=c.kode_lokasi and c.kode_pp='"+this.cb_pp.getText()+"' and c.periode = '"+this.e_periode.getText()+"' "+ 
									"where b.kode_flag in ('031') and a.kode_lokasi='"+this.app._lokasi+"'",["kode_akun","nama"],false,["Kode","Nama"],"and","Data Akun",true);													
		}
	...
	doLoad3:function(sender){																						
		...
		var strSQL = "select a.no_aju,convert(varchar,a.tanggal,103) as tgl,modul,convert(varchar,a.tgl_kuitansi,103) as tglkui,a.keterangan,a.progress,a.nilai,a.no_app "+
					 ...
					 "where a.modul='IFAJU' "+bidang+" and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"' and a.progress in ('0','R')";		
		...
		try{
			if (this.sg3.cells(0,row) != "") {
				...
				var strSQL = "select a.no_app as no_applama,a.keterangan,a.tgl_kuitansi,a.tanggal,a.nik_app,isnull(b.no_app,'-') as no_app,isnull(b.catatan,'-') as catatan, "+
							 "       a.kode_akun,a.kode_pp,a.kode_drk,a.nilai,a.sts_pajak,a.npajak "+
