window.app_saku3_transaksi_if_fNikIF = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_if_fNikIF.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_if_fNikIF";
		...
		this.cb_rek = new portalui_saiCBBL(this.pc1.childPage[1],{bound:[20,18,220,20],caption:"Rekening",tag:0,multiSelection:false});				
...
window.app_saku3_transaksi_if_fNikIF.extend(window.childForm);
window.app_saku3_transaksi_if_fNikIF.implement({
		...
		try{						
			if (this.standarLib.checkEmptyByTag(this, [0,1])){
				try{									
					...
					sql.add("insert into if_m(nik,kode_lokasi,kode_pp,nilai,no_dokumen,keterangan,flag_aktif,kode_rek) values "+
						    "('"+this.cb_nik.getText()+"','"+this.app._lokasi+"','"+this.kodePP+"',"+nilaiToFloat(this.e_nilai.getText())+",'"+this.e_dok.getText()+"','"+this.e_ket.getText()+"','"+this.c_status.getText().substr(0,1)+"','"+this.cb_rek.getText()+"')");
					setTipeButton(tbAllFalse);
	...
	doChange: function(sender){
		try{
			if (this.cb_nik.getText() != ""){
				var strSQL = "select a.kode_rek,a.flag_aktif,a.nik,b.kode_pp,b.kode_pp+' - '+b.nama as pp,a.nilai,a.no_dokumen,a.keterangan from if_m a inner join pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi where a.nik ='"+this.cb_nik.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'";						   
