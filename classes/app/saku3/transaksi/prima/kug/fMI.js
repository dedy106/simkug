window.app_saku3_transaksi_prima_kug_fMI = function(owner)
{
	if (owner)
	{
		window.app_saku3_transaksi_prima_kug_fMI.prototype.parent.constructor.call(this,owner);
		this.className  = "app_saku3_transaksi_prima_kug_fMI";
		...
		this.sg3 = new saiGrid(this.pc2.childPage[1],{bound:[1,5,this.pc2.width-5,this.pc2.height-33],colCount:6,tag:9,
					...
					colWidth:[[5,4,3,2,1,0],[70,100,300,200,100,100]],					
		...
		this.pc1 = new pageControl(this.pc2.childPage[0],{bound:[5,12,990,302], childPage:["Item Jurnal","Otorisasi"]});
		...
		this.cb_periksa = new saiCBBL(this.pc1.childPage[1],{bound:[20,11,220,20],caption:"Pemeriksa", multiSelection:false, maxLength:10, tag:2});									
		...
		try {
			...
			this.cb_periksa.setSQL("select nik, nama from karyawan where kode_lokasi='"+this.app._lokasi+"' and flag_aktif='1' ",["nik","nama"],false,["Kode","Nama"],"and","Daftar Karyawan",true);			
...
window.app_saku3_transaksi_prima_kug_fMI.extend(window.childForm);
window.app_saku3_transaksi_prima_kug_fMI.implement({				
			...
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("insert into trans_m (no_bukti,kode_lokasi,tgl_input,nik_user,periode,modul,form,posted,prog_seb,progress,kode_pp,tanggal,no_dokumen,keterangan,kode_curr,kurs,nilai1,nilai2,nilai3,nik1,nik2,nik3,no_ref1,no_ref2,no_ref3,param1,param2,param3) values "+
							...
							parseNilai(this.e_debet.getText())+",0,0,'"+this.app._userLog+"','"+this.cb_periksa.getText()+"','-','-','-','-','-','-','"+this.c_jenis.getText()+"')");
	...
	doModalResult: function(event, modalResult){
		...
		switch (event){
			...
			case "simpan" :															
			case "ubah" :			
				var strSQL = "select no_bukti from trans_m where no_dokumen = '"+this.e_dok2.getText()+"' and kode_lokasi='"+this.app._lokasi+"' and no_bukti<>'"+this.e_nb.getText()+"' ";						
				var data = this.dbLib.getDataProvider(strSQL,true);
				if (typeof data == "object"){
					var line = data.rs.rows[0];							
					if (line != undefined){							
						system.alert(this,"Transaksi tidak valid.","No Dokumen '"+this.e_dok2.getText()+"' sudah terpakai di No Bukti '"+line.no_bukti+"'.");
							return false;
					}
				}	
				...
				switch(methodName){
	    			case "execArraySQL" :	    				
						if (result.toLowerCase().search("error") == -1){
							if (this.preView == "1") {								
								this.nama_report="server_report_saku3_prima_rptJuJurnalBukti"; 
	...
	doLoad3:function(sender){																			
		var strSQL = "select a.no_bukti,convert(varchar,a.tanggal,103) as tgl,a.no_dokumen,a.param3 as jenis,a.keterangan,a.nilai1 "+
		             "from trans_m a inner join karyawan_pp b on a.kode_pp=b.kode_pp and a.kode_lokasi=b.kode_lokasi and b.nik='"+this.app._userLog+"' "+					 					 
	...
	doTampilData3: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg3.appendData([line.no_bukti,line.tgl,line.no_dokumen,line.keterangan,line.nilai1,"Pilih"]); 
		}
