...
//* 			SAI, PT											
//***********************************************************************************************
window.app_saku3_app = function(owner){
	window.app_saku3_app.prototype.parent.constructor.call(this, owner);
	this.className = "app_saku3_app";
	this.onTerminate.set(this,"doTerminate");
	this.form = undefined;
	this.setTitle("SIMKUG");
	system.setThemes("sai");
	this.showForm();
};
window.app_saku3_app.extend(window.application);
window.app_saku3_app.implement({
	showForm: function(){
		try
		{
			//uses("util_dbLib;form;childForm");	//;
			uses("util_dbLib;form;childForm;util_EventEmitter;util_JSONRPC");	//;
			...
			this.services.on("error", function(message){
                console.log("Error SAPLib "+ message);
			...
			this._mainForm = new app_saku3_fMain(this);	
			//alert("fmain");
	...
	doKeyDown: function(charCode, buttonState, keyCode, event){
		try{			
			window.app_saku3_app.prototype.parent.doKeyDown.call(this, charCode, buttonState, keyCode, event);
