...
//* 			SAI, PT											
//***********************************************************************************************
window.app_inv_app = function(owner){
	window.app_inv_app.prototype.parent.constructor.call(this, owner);
	this.className = "app_inv_app";
	this.onTerminate.set(this,"doTerminate");
	this.form = undefined;
	this.setTitle("roojax apps");
	this.showForm();
};
window.app_inv_app.extend(window.application);
window.app_inv_app.implement({
	showForm: function(){
		try
		{
			...
			this._mainForm = new app_inv_fMain(this);	
	...
	doKeyDown: function(charCode, buttonState, keyCode, event){
		try{			
			window.app_inv_app.prototype.parent.doKeyDown.call(this, charCode, buttonState, keyCode, event);
