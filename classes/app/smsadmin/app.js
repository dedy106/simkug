...
//* 			SAI, PT											
//***********************************************************************************************
window.app_smsadmin_app = function(owner){
	window.app_smsadmin_app.prototype.parent.constructor.call(this, owner);
	this.className = "app_smsadmin_app";
	this.onTerminate.set(this,"doTerminate");
	this.form = undefined;
	this.setTitle("jamboo e-Saku");
	this.showForm();
};
window.app_smsadmin_app.extend(window.portalui_application);
window.app_smsadmin_app.implement({
	showForm: function(){
		try
		{
			var step = "load";			
			uses("util_dbLib;portalui_form;portalui_childForm");
			uses("app_smsadmin_fMain");							
			step = "create";		
			this._mainForm = new app_smsadmin_fMain(this);	
	...
	doKeyDown: function(charCode, buttonState, keyCode, event){
		try{			
			window.app_smsadmin_app.prototype.parent.doKeyDown.call(this, charCode, buttonState, keyCode, event);
