window.app_smsadmin_report_flDataPinjaman = function(owner)
{
	if (owner)
	{
		window.app_smsadmin_report_flDataPinjaman.prototype.parent.constructor.call(this,owner);
		this.className = "app_smsadmin_report_flDataPinjaman";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Data Saldo", 2);
...
window.app_smsadmin_report_flDataPinjaman.extend(window.controls_childForm);


window.app_smsadmin_report_flDataPinjaman.prototype.doEllipseClick= function(sender, col, row)
{
		if (row == 0)
		{				
		...
		if (row == 3)
		{
			this.standar.ListDataSGFilter2(this, "Data Pinjaman",this.sg1, this.sg1.row, this.sg1.col,
													"select a.no_pinj, a.saldo, b.nama from sms_cust  b inner join sms_saldo a on a.kode_cust = a.kode_cust and a.kode_lokasi = a.kode_lokasi "+
													...
													"select count(*) from sms_cust from sms_cust  b inner join sms_saldo a on a.kode_cust = a.kode_cust and a.kode_lokasi = a.kode_lokasi"+
														this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
														this.filterRep.filterStr("b.kode_klpcust",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
														this.filterRep.filterStr("b.kode_cust",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and"),
													["no_pinj","saldo"],"and",["No Pinjaman","Saldo","Nama Customer"]);
		}
}

window.app_smsadmin_report_flDataPinjaman.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
	...
	{
		this.filterRep.setSGFilterRowTipe(this.sg1, row,[0,1,2,3],["3","23","23","23"]);
		this.filterRep.setSGFilterRowButtonStyle(this.sg1, row,[0,1,2,3],[3,2,2,2]);
	}	
}

window.app_smsadmin_report_flDataPinjaman.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
	    	...
	    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
						...
						this.filterRep.filterStr("c.no_pinj",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");			
	    	this.showFilter = this.filterRep.showFilter(this.sg1);
	    	var isnull = this.app._dbEng == "mysqlt" ? "ifnull" :"isnull";
			var result  = new controls_arrayMap();			
			var sql = "select a.kode_cust, a.nama, a.kode_klpcust, b.nama as nmgroup, c.no_pinj, c.saldo "+
					"from sms_cust a inner join sms_klpcust b on b.kode_klpcust = a.kode_klpcust and b.kode_lokasi = a.kode_lokasi "+
					"	inner join sms_saldo c on c.kode_cust = a.kode_cust and c.kode_lokasi = a.kode_lokasi "+
					this.filter;
			...
			title.add("Kode");title.add("Nama Customer");title.add("Group");title.add("Nama Group");title.add("No Pinjaman");title.add("Saldo");
			var width = new server_util_arrayList();			
			width.add(80);width.add(280);width.add(80);width.add(230);width.add(150);width.add(120);
	...
	{
		alert("[app_smsadmin_report_flDataPinjaman]::mainButtonClick:"+e);
	}
}

window.app_smsadmin_report_flDataPinjaman.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_smsadmin_report_flDataPinjaman.prototype.doSelectedPage = function(sender, page)
{	
	this.dbLib.sqlToHtmlA(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false,this.groupBy);			
	//this.previewReport(dthtml);			
}
window.app_smsadmin_report_flDataPinjaman.prototype.previewReport = function(dthtml)
{
	var html = "<div align='center'><br><br>"+
				"<div class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'>DAFTAR SALDO PINJAMAN</div><br>";			
...
window.app_smsadmin_report_flDataPinjaman.prototype.doCloseReportClick = function(sender)
{
...
window.app_smsadmin_report_flDataPinjaman.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_smsadmin_report_flDataPinjaman.prototype.doRowPerPageChange = function(sender, rowperpage)
{
