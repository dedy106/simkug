window.app_smsadmin_report_flGroup = function(owner)
{
	if (owner)
	{
		window.app_smsadmin_report_flGroup.prototype.parent.constructor.call(this,owner);
		this.className = "app_smsadmin_report_flGroup";
		this.maximize();
		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Daftar Group Customer", 2);
...
window.app_smsadmin_report_flGroup.extend(window.controls_childForm);


window.app_smsadmin_report_flGroup.prototype.doEllipseClick= function(sender, col, row)
{
		if (row == 0)
		{				
...
window.app_smsadmin_report_flGroup.prototype.doSelectCell = function(sender, col, row)
{
	if (this.userStatus=="A")
	{
...
window.app_smsadmin_report_flGroup.prototype.mainButtonClick = function(sender)
{
	try
	{
		...
		else
		{
			...
			var sql = "select kode_klpcust, fn_spasi(nama, level_spasi) as nama, tipe "+
					"from sms_klpcust a "+
					this.filter+
					"order by a.rowindex";
	...
	{
		alert("[app_smsadmin_report_flGroup]::mainButtonClick:"+e);
	}
}

window.app_smsadmin_report_flGroup.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_smsadmin_report_flGroup.prototype.doSelectedPage = function(sender, page)
{	
	this.dbLib.sqlToHtmlA(this.sqlScript,page,this.pager, this.title, this.widthTable, this.fieldType,false,this.groupBy);			
	//this.previewReport(dthtml);			
}
window.app_smsadmin_report_flGroup.prototype.previewReport = function(dthtml)
{
	var html = "<div align='center'><br><br>"+
				"<div class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'>DAFTAR GROUP CUSTOMER</div><br>";			
...
window.app_smsadmin_report_flGroup.prototype.doCloseReportClick = function(sender)
{
...
window.app_smsadmin_report_flGroup.prototype.sg1onChange = function(sender, col , row)
{
    if (col==1)
	{
...
window.app_smsadmin_report_flGroup.prototype.doRowPerPageChange = function(sender, rowperpage)
{
