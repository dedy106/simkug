window.app_smsadmin_parameter_fGroup = function(owner)
{
	if (owner)
	{
		window.app_smsadmin_parameter_fGroup.prototype.parent.constructor.call(this, owner);
		this.className = "app_smsadmin_parameter_fGroup";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Customer", 0);	
...
window.app_smsadmin_parameter_fGroup.extend(window.portalui_childForm);
window.app_smsadmin_parameter_fGroup.prototype.mainButtonClick = function(sender)
{
...
window.app_smsadmin_parameter_fGroup.prototype.doRequestReady = function(sender, methodName, result)
{
...
window.app_smsadmin_parameter_fGroup.prototype.doModalResult = function(event, modalResult, value)
{
	try
	{
		switch (event)
		{
			case "Add" :
			   if ( modalResult == mrOk)
			   {
					...
					{   
						...
						data.set("kode_klpcust",valArray[0]);
						...
						data.set("nik_user",this.app._nikUser);
						node.data = data;
					}catch(e)
					{
						alert(e);
			...
			case "simpan" :
				if (modalResult == mrOk)
				{
					...
					try
					{						
						var sql2,sql1 = "delete from sms_klpcust where kode_lokasi = '"+this.elokasi.getText()+"'";					// kode_lokasi,
						...
						if (driver.search("mssql") != -1){
							...
							for (var i in data){
								sql2 = "insert into sms_klpcust(kode_klpcust, kode_lokasi,  nama, level_spasi, level_lap, tipe, sum_header, kode_induk, rowindex, tgl_input, nik_user) values ";
								sql2 += " "+ data[i] +" ";
								sql.add(sql2);
							}								
...
window.app_smsadmin_parameter_fGroup.prototype.loadMenu = function(daftarNrc)
{
	try
	{		
		this.isStr = true;
...
//-------------------------------------
window.app_smsadmin_parameter_fGroup.prototype.doClick = function(sender)
{	
	this.dblib.loadQueryA("select kode_klpcust, kode_lokasi, nama, level_spasi, level_lap, tipe, sum_header, kode_induk, rowindex, tgl_input, nik_user from sms_klpcust where kode_lokasi = '"+this.elokasi.getText()+"'  order by rowindex",0,0);			
};

window.app_smsadmin_parameter_fGroup.prototype.entriesClick = function(sender)
{
	try
	{ 
	 	{
...
window.app_smsadmin_parameter_fGroup.prototype.treeClick = function(item)
{
	
	
};

window.app_smsadmin_parameter_fGroup.prototype.getTreeData = function(node)
{
	var result = "";
	var child = undefined;
...
window.app_smsadmin_parameter_fGroup.prototype.getDataAkun = function(node)
{
	var result = "";
	var child = undefined;
	if (node instanceof portalui_treeView)
...
window.app_smsadmin_parameter_fGroup.prototype.getSummary = function(node)
{
	var result = "";
	var child = undefined;
	if (node instanceof portalui_treeView)
	...
	}else
	{
		...
		if (result != "Summary")
			result = "";
		else result = ";" + value.get("kode_klpcust");
...
window.app_smsadmin_parameter_fGroup.prototype.doFindLokasi = function(sender)
{
...
window.app_smsadmin_parameter_fGroup.prototype.copyChilds = function(from, to)
{
	var temp = undefined;
	...
	{			
		...
		chld.data.set("kode_klpcust",temp.getKode());					
...
window.app_smsadmin_parameter_fGroup.prototype.doEditChange = function(sender)
{
	this.eKlp.setText("");
	this.treev.clear();
};
window.app_smsadmin_parameter_fGroup.prototype.switchItem = function(from, to)
{
	...
	child.data.set("kode_klpcust",item.getKode());					
...
window.app_smsadmin_parameter_fGroup.prototype.moveItem = function(item, owner)
{	
