window.app_mede_master_fVendor = function(owner){
	if (owner)
	{
		window.app_mede_master_fVendor.prototype.parent.constructor.call(this,owner);
		this.className  = "app_mede_master_fVendor";
		this.maximize();
...
window.app_mede_master_fVendor.extend(window.portalui_childForm);
window.app_mede_master_fVendor.prototype.mainButtonClick = function(sender)
{
...
window.app_mede_master_fVendor.prototype.doModalResult = function(event, modalResult)
{			
	switch (event)
	{
		case "clear" :
		...
		case "simpan" :
			if (modalResult == mrOk)
			{
				...
				{					
					...
					sql.add("insert into doc_vendor (kode_vendor,nama,alamat,kota,kode_pos,no_telp,no_fax,npwp,pic,kode_lokasi,bank,cabang,no_rek,nama_rek,nik_user, tgl_input)  values "+
		...
		case "hapus" :
		   if (modalResult == mrOk)
		   {
				uses("server_util_arrayList");					
					var sql = new server_util_arrayList();
					sql.add("delete from doc_vendor where kode_vendor='"+this.ed_kode.getText()+"' and kode_lokasi='"+this.cb_lokasi.getText()+"'");
					this.dbLib.execArraySQL(sql);	
		   }
			break;
	}
};
window.app_mede_master_fVendor.prototype.showClick = function(sender){
...
window.app_mede_master_fVendor.prototype.FindBtnClick = function(sender, event)
{
	try
	{
		if (sender == this.ed_kode) 
		{
			this.standarLib.showListData(this, "Daftar Vendor",this.ed_kode,this.ed_nama, 
										  "select kode_vendor, nama  from doc_vendor where kode_lokasi='"+this.cb_lokasi.getText()+"'",
										  "select count(kode_vendor) from doc_vendor where kode_lokasi='"+this.cb_lokasi.getText()+"'",
...
window.app_mede_master_fVendor.prototype.doRequestReady = function(sender, methodName, result)
{
