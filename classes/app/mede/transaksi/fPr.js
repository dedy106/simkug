window.app_mede_transaksi_fPr = function(owner)
{
	if (owner)
	{
		window.app_mede_transaksi_fPr.prototype.parent.constructor.call(this,owner);
		this.className  = "app_mede_transaksi_fPr";
		this.maximize();
		...
		this.sg1 = new portalui_saiGrid(this.p1,{bound:[1,20,895,210],colCount:7,colTitle:["Kode Akun","Nama Akun","Keterangan","Satuan","Qty","Nilai","SubTotal"],
			buttonStyle:[[0],[bsEllips]],colReadOnly:[[1,3],[],true],colFormat:[[4,5,6],[cfNilai, cfNilai, cfNilai]],colWidth:[[6,5,4,3,2,1,0],[100,100,100,100,250,250,100]]});
...
window.app_mede_transaksi_fPr.extend(window.portalui_childForm);
window.app_mede_transaksi_fPr.implement({
	...
	simpan: function(){
		...
		if (this.standarLib.checkEmptyByTag(this, [0]))
		{
			try
			{
				...
				sql.add("insert into doc_pr_m (no_pr,no_po,kode_lokasi,no_dokumen,kode_drk,tanggal,keterangan,kode_pp,progress,"+
				...
				{			
					var j = i+1;
					sql.add("insert into doc_pr_d (no_pr,no_urut,kode_akun,kode_brg,keterangan,kode_lokasi,kode_sat,jumlah,nilai) values "+	
							"('"+this.ed_nb.getText()+"',"+j+",'"+this.sg1.getCell(0,i)+"','"+j+"','"+this.sg1.getCell(2,i)+"','"+this.app._lokasi+"','"+this.sg1.getCell(3,i)+"','"+this.sg1.getCell(4,i)+"',"+parseNilai(this.sg1.getCell(5,i))+")");
				}
	...
	doCellExit : function(sender, col, row){
		try
		{
			...
			alert("[app_mede_transaksi_fPr] : doFindBtnClick : " + e);
		}
	},
	...
	doNilaiChange: function(){
		...
		{
			alert("[app_mede_transaksi_fPr]::doNilaiChange:"+e);
		}
	},
	hitungGar : function(){
