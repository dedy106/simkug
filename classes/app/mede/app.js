showSplash("classes/app/mede/image/tsel.png",350,94);
uses("util_dbLib;portalui_form;portalui_sysForm;portalui_childForm;portalui_saiCB;portalui_slider;portalui_image;portalui_imageButton;portalui_button;portalui_timer;portalui_label;portalui_saiLabelEdit;portalui_saiCBBL;portalui_panel;portalui_reportNavigator;portalui_treeView;portalui_treeNode;portalui_statusBar;portalui_marquee;util_addOnLib;util_standar;portalui_imageButton;portalui_chart");
...
window.app_mede_app = function(owner){
	window.app_mede_app.prototype.parent.constructor.call(this, owner);
	this.className = "app_mede_app";
	this.splash = "classes/app/mede/image/tsel.png";			
	this.onTerminate.set(this,"doTerminate");
	this.form = undefined;
	this.setTitle("Menejemen ");
	this.showForm();
};
window.app_mede_app.extend(window.portalui_application);
window.app_mede_app.implement({
	showForm: function(){
		try		
		{			
			this._mainForm= new app_mede_fMain(this);
	...
	doKeyDown: function(charCode, buttonState, keyCode, event){
		try{			
			window.app_mede_app.prototype.parent.doKeyDown.call(this, charCode, buttonState, keyCode, event);
			...
			if (this.activeForm.activeChildForm !== undefined) {
				...
				if ((keyCode == 40 || keyCode == 33 || keyCode == 34 || keyCode == 35 || keyCode == 39) && (target.id.search("inplaceEdit") == -1 && target.id.toLowerCase().search("edit") == -1 && (target.id.search("input") == -1 || target.id.search("textarea") == -1)))
					return false;
				return true;			
			}
		}catch(e){
...
window.app_mede_fMain = function(owner)
{
	if (owner)
	{	
		try
		{			
			window.app_mede_fMain.prototype.parent.constructor.call(this, owner);
			window.app_mede_fMain.prototype.parent.setWidth.call(this, 600);
			window.app_mede_fMain.prototype.parent.setHeight.call(this, 450);
			this._mainButtonClick = new portalui_eventHandler();
			this.title = "Manajemen Dokumen";
			document.title = "Manajemen Dokumen";
			this._isLogedIn	= false;
			this.app._kodeMenu = "DOCMENU";
			this._userLog = "";
			this._lokasi = "";
			this._pp = "";
			this.alwaysLoad = true;
			this._openAccess = true;
			this._checkLisence = false;
			...
			this.setTop(60);						
			this.maximize();	
			//this.progress = new portalui_progressForm(this,{bound:[this.width /2 - 118,-28,230,30]});
...
//------------------------------------ panel transaksi -------------------------
			this.pTrans = new portalui_panel(this.pButton,{bound:[2,1,550,26],border:0,color:system.getConfig("app.color.panel")});						
			...
			this.bEdit = new portalui_button(this.pTrans,{bound:[90,3,80,18],caption:"<u>E</u>dit",click:[this,"mainButtonClick"],icon:"url(icon/"+system.getThemes()+"/edit.png)",name:"bEdit",hint:"Simpan Edit data Transaksi"});			
			this.bHapus = new portalui_button(this.pTrans,{bound:[175,3,80,18],caption:"<u>H</u>apus",click:[this,"mainButtonClick"],icon:"url(icon/"+system.getThemes()+"/delete.png)",hint:"Hapus data Transaksi",name:"bHapus"});		
			this.bClear = new portalui_button(this.pTrans,{bound:[300,3,80,18],caption:"<u>C</u>lear",click:[this,"mainButtonClick"],icon:"url(icon/"+system.getThemes()+"/refresh.png)",hint:"Clear Form",name:"bClear"});
//--------------- panel login -------------------------		
			this.pLogin = new portalui_panel(this.pButton,{bound:[2,1,550,26],border:0,color:system.getConfig("app.color.panel"),visible:false});
...
//------------------------------------ panel report -------------------------		
			this.pReport = new portalui_panel(this.pButton,{bound:[2,1,550,26],visible:false,border:0,color:system.getConfig("app.color.panel")});
...
//------------------------------------ panel Editor-------------------------		
			this.pEditor = new portalui_panel(this.pButton,{bound:[2,1,550,26],visible:false,border:0,color:system.getConfig("app.color.panel")});
			...
			this.bSave = new portalui_button(this.pEditor,{bound:[90,3,80,18],caption:"Save",click:[this,"mainButtonClick"],hint:"Save Form",icon:"url(icon/"+system.getThemes()+"/save.png)"});
			this.bOpen = new portalui_button(this.pEditor,{bound:[175,3,80,18],caption:"Open",click:[this,"mainButtonClick"],icon:"url(icon/"+system.getThemes()+"/preview.png)",hint:"Open Form"});		
			this.bRun = new portalui_button(this.pEditor,{bound:[260,3,80,18],caption:"Run",click:[this,"mainButtonClick"],icon:"url(icon/"+system.getThemes()+"/Execute.png)",hint:"Test Form"});			
//------------------------------------ panel Editor-------------------------		
			this.pQuery = new portalui_panel(this.pButton,{bound:[2,1,550,26],visible:false,border:0,color:system.getConfig("app.color.panel")});
...
//------------------------------------ panel Message-------------------------		
			this.pMsg = new portalui_panel(this.pButton,{bound:[2,1,550,26],visible:false,border:0,color:system.getConfig("app.color.panel")});
...
window.app_mede_fMain.extend(window.portalui_form);
window.app_mede_fMain.implement({
	gotoLogin: function(){		
		...
		this.form.show();
		this.form.maximize();
		this.form.setImage("classes/app/mede/image/Blue.jpg",true,this.form.width,this.form.height);
		this.setCaption("Login to Flow Document");
		this.setFormCaption("Login to Flow Document");
		this.hideMainComp();		
	},	
	...
	bTutupClick: function(sender){	
		try{
			...
			this.marquee.show();
			this.setFormCaption("rooJax menu access");
	...
	treeClick: function(item){		
		try{				
			...
			this.showProgress("load "+kodeForm);
			switch(kodeForm){
				case "U01I" :				
					window.location = ".";
					break;			
	...
	doClose: function(sender){
		if (window.desktop !== undefined) this.getApplication().terminate();
		else this.app.restart();
	},
...
//------------------------------------------------------------------- locking transaction		
			...
			if (sender == this.timerAlert){
				if (this.alertLisence && this._checkLisence){
	...
	isValidLisence: function(form){
		if (!this._checkLisence) return true;
		var result = false;
	...
	doRecall: function(sender){	    
        ...
        if (this.form !== undefined){
            if (this.form.className == "app_saku_fLogin"){
               this.gotoLogin();
            }else{
