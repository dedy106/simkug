window.app_dmt_dashboard = function(owner,options)
{
	if (owner)
	{
		window.app_dmt_dashboard.prototype.parent.constructor.call(this,owner,options);
		this.className  = "app_dmt_dashboard";
		...
		try {
			...
			this.dataProvider = new app_dmt_remote_dataProvider(this.app._dbSetting);
...
window.app_dmt_dashboard.extend(window.panel);
window.app_dmt_dashboard.implement({	
	...
	doInformasi: function(event,no_konten){
		try{
			...
			var nama_report="server_report_dmt_konten_rptInformasi";
	...
	doViewItem: function(event,no_bukti){
		try{
			...
			if (no_bukti.substr(3,2)=="SO")
			{
				...
				nama_report="server_report_dmt_rptKontrak";
			}
			...
			if (no_bukti.substr(3,2)=="AK")
			{
				...
				nama_report="server_report_dmt_rptBilling";
			}
			if (no_bukti.substr(3,2)=="IN")
			{
				this.filter=" where a.no_ar='"+no_bukti+"'";
				nama_report="server_report_dmt_rptFormInvoice1";
			}
			if (no_bukti.substr(3,2)=="AM")
			{
				this.filter=" where a.no_akru='"+no_bukti+"'";
				nama_report="server_report_dmt_rptAmor";
			}
			this.report = new server_report_report();
			this.showFilter = "";
			this.lokasi = "";
