window.app_dmt_report_flTerimaPph = function(owner)
{
	if (owner)
	{
		window.app_dmt_report_flTerimaPph.prototype.parent.constructor.call(this,owner);
		this.className = "app_dmt_report_flTerimaPph";
		this.maximize();		
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from dmt_arpph_m where kode_lokasi ='"+this.app._lokasi+"'");
...
window.app_dmt_report_flTerimaPph.extend(window.portalui_childForm);
window.app_dmt_report_flTerimaPph.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row == 3)
		{
			this.filterRep.ListDataSGFilter(this, "Data Kontrak",this.sg1, this.sg1.row, this.sg1.col,
										  "select no_kontrak, keterangan from dmt_arpph_m "+
	...
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			this.dbLib.setItemsFromSQL("select distinct periode from dmt_arpph_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 5)
		{
	...
	mainButtonClick: function(sender)
	{
		try
		{
			...
			}else
			{
				...
				this.nama_report="server_report_dmt_rptTerimaPph";
		...
		{
			alert("[app_dmt_report_flTerimaPph]::mainButtonClick:"+e);
		}
	},
