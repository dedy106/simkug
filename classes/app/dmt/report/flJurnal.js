window.app_dmt_report_flJurnal = function(owner)
{
	if (owner)
	{
		window.app_dmt_report_flJurnal.prototype.parent.constructor.call(this,owner);
		this.className = "app_dmt_report_flJurnal";
		this.maximize();
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(a.periode) as periode "+
											"from (select max(periode) as periode from dmt_akru_j "+
											"	  union "+
											"	  select max(periode) as periode from dmt_ar_j "+
											"	  union "+
											"	  select max(periode) as periode from kas_j "+
											"	  )a");
...
window.app_dmt_report_flJurnal.extend(window.childForm);
window.app_dmt_report_flJurnal.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			...
			if (row == 4)
			{
				...
				var sql1="select distinct a.no_ar,a.no_dokumen from dmt_ar_j a "+filter; 
				var sql2="select count(distinct a.no_ar) from dmt_ar_j a "+filter;
				this.filterRep.ListDataSGFilter(this, "Data Jurnal",this.sg1, this.sg1.row, this.sg1.col,sql1,sql2,
												new Array("a.no_ar","a.no_dokumen"),"and",new Array("No Bukti","No Dokumen"),"and");
			}			
		}catch(e){
			systemAPI.alert(e);
		}
	...
	{		
		...
		if (row == 1)
		{
			...
			this.dbLib.setItemsFromSQL("select a.periode "+
											"from (select periode from dmt_akru_j "+
											"	  union "+
											"	  select periode from dmt_ar_j "+
											"	  union "+
											"	  select periode from kas_j "+
											"	  )a order by periode desc ",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row === 3)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.gridLib.SGIsiItemsFromArray(this.sg1.columns.get(2).pickList,["Billing","Invoice","Penerimaan"]);
		}
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2)
			{
				...
				var modul="";
				if (this.sg1.getCell(2,3)=="Billing")
				{
					modul=" and a.modul='AR' ";
				}
				if (this.sg1.getCell(2,3)=="Invoice")
				{
					modul=" and a.modul='ARINV' ";
				}
				if (this.sg1.getCell(2,3)=="Penerimaan")
				{
					modul=" and a.modul='KBAR' ";
				}
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
							this.filterRep.filterStr("a.kode_akun",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+modul;
				this.filter2=this.sg1.getCell(2,1)+"/"+this.filterRep.filterStr("a.no_bukti",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"where");
				
				this.nama_report="server_report_dmt_rptJurnal";
