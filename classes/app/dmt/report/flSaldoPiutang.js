window.app_dmt_report_flSaldoPiutang = function(owner)
{
	if (owner)
	{
		window.app_dmt_report_flSaldoPiutang.prototype.parent.constructor.call(this,owner);
		this.className = "app_dmt_report_flSaldoPiutang";
		this.maximize();		
...
window.app_dmt_report_flSaldoPiutang.extend(window.portalui_childForm);
window.app_dmt_report_flSaldoPiutang.implement({
	...
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			this.dbLib.setItemsFromSQL("select a.periode "+
									...
									"	 )a where a.kode_lokasi = '"+this.sg1.getCell(2,0)+"' order by periode desc",[this.sg1.columns.get(2).pickList]);
		}
	...
	mainButtonClick: function(sender)
	{
		try
		{
			...
			}else
			{
				...
				this.nama_report="server_report_dmt_rptSaldoPiutang";
		...
		{
			alert("[app_dmt_report_flSaldoPiutang]::mainButtonClick:"+e);
		}
	},
