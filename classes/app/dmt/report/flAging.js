window.app_dmt_report_flAging = function(owner)
{
	if (owner)
	{
		window.app_dmt_report_flAging.prototype.parent.constructor.call(this,owner);
		this.className = "app_dmt_report_flAging";
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan", 2);
	...
	this.gridLib.SGEditData(this.sg1,5,new Array(0,1,2), new Array("Jenis Laporan","=","Detail"));
	this.pager = 1;
};
window.app_dmt_report_flAging.extend(window.portalui_childForm);
window.app_dmt_report_flAging.implement({
	...
	{
		this.filterRep.setSGFilterRowTipe(this.sg1,row,[0,1,2,3,4,5],["3","123","123","123","3","3"]);
	...
	mainButtonClick: function(sender)
	{
		try
		{
			...
			}else
			{
				...
				this.nama_report="server_report_dmt_rptAging";
				if (this.sg1.getCell(2,5)=="Summary")
				{
					this.nama_report="server_report_dmt_rptAgingSum";
				}
				
				var tgl_aging=this.sg1.getCellDateValue(2,4);
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+"/"+tgl_aging;
		...
		{
			alert("[app_dmt_report_flAging]::mainButtonClick:"+e);
		}
	},
