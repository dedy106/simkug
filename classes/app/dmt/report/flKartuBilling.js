window.app_dmt_report_flKartuBilling = function(owner)
{
	if (owner)
	{
		window.app_dmt_report_flKartuBilling.prototype.parent.constructor.call(this,owner);
		this.className = "app_dmt_report_flKartuBilling";
		this.maximize();		
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select distinct periode from dmt_akru_m where kode_lokasi ='"+this.app._lokasi+"'");
...
window.app_dmt_report_flKartuBilling.extend(window.portalui_childForm);
window.app_dmt_report_flKartuBilling.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row == 3)
		{
			this.filterRep.ListDataSGFilter(this, "Data Kontrak",this.sg1, this.sg1.row, this.sg1.col,
										  "select no_kontrak, keterangan from dmt_kontrak_m "+
										  ...
										  "select count(*) from dmt_kontrak_m "+
										  this.filterRep.filterStr("kode_cust",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"where")+
										  this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+
										  " and kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",
	...
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			this.dbLib.setItemsFromSQL("select distinct periode from dmt_akru_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	mainButtonClick: function(sender)
	{
		try
		{
			...
			}else
			{
				...
				this.nama_report="server_report_dmt_rptKartuBilling";
				this.filter2 = this.sg1.getCell(2,1)+"/"+this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and");
		...
		{
			alert("[app_dmt_report_flKartuBilling]::mainButtonClick:"+e);
		}
	},
