window.app_dmt_report_flBilling = function(owner)
{
	if (owner)
	{
		window.app_dmt_report_flBilling.prototype.parent.constructor.call(this,owner);
		this.className = "app_dmt_report_flBilling";
		this.maximize();		
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from dmt_akru_m where kode_lokasi ='"+this.app._lokasi+"' and modul='AR'");
...
window.app_dmt_report_flBilling.extend(window.portalui_childForm);
window.app_dmt_report_flBilling.implement({
	doEllipseClick: function(sender, col, row)
	{
		...
		if (row == 4)
		{
			this.standar.ListDataSGFilter2(this, "Data Invoice",this.sg1, this.sg1.row, this.sg1.col,
										  "select no_ar, keterangan  from dmt_akru_m "+
										  this.filterRep.filterStr("kode_cust",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"where")+
										  this.filterRep.filterStr("no_kontrak",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
											this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+	
										  " and kode_lokasi = '"+this.sg1.getCell(2,0)+"' and modul='AR'",
										  "select count(*) from dmt_akru_m "+
										  this.filterRep.filterStr("kode_cust",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"where")+
										  this.filterRep.filterStr("no_kontrak",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and")+
											this.filterRep.filterStr("periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and")+	
										  " and kode_lokasi = '"+this.sg1.getCell(2,0)+"' and modul='AR'",
	...
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			this.dbLib.setItemsFromSQL("select distinct periode from dmt_akru_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' and modul='AR'",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 5)
		{
	...
	mainButtonClick: function(sender)
	{
		try
		{
			...
			}else
			{
		    	...
		    	this.filter = this.filterRep.filterStr("a.kode_lokasi",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where")+
							...
							"and a.modul='AR' ";
				this.nama_report="server_report_dmt_rptAmor";
		...
		{
			alert("[app_dmt_report_flBilling]::mainButtonClick:"+e);
		}
	},
