window.app_dmt_report_flKontrak = function(owner)
{
	if (owner)
	{
		window.app_dmt_report_flKontrak.prototype.parent.constructor.call(this,owner);
		this.className = "app_dmt_report_flKontrak";
		this.maximize();		
	...
	this.periode=this.dbLib.getPeriodeFromSQL("select max(periode) as periode from dmt_kontrak_m where kode_lokasi ='"+this.app._lokasi+"'");
...
window.app_dmt_report_flKontrak.extend(window.portalui_childForm);
window.app_dmt_report_flKontrak.implement({
	...
	doSelectCell: function(sender, col, row)
	{
		...
		if (row === 1){
			this.dbLib.setItemsFromSQL("select distinct periode from dmt_kontrak_m where kode_lokasi = '"+this.sg1.getCell(2,0)+"' ",[this.sg1.columns.get(2).pickList]);
		}
		if (row === 5)
		{
	...
	mainButtonClick: function(sender)
	{
		try
		{
			...
			}else
			{
				...
				this.nama_report="server_report_dmt_rptKontrak";
		...
		{
			alert("[app_dmt_report_flKontrak]::mainButtonClick:"+e);
		}
	},
