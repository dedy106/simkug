window.app_dmt_transaksi_fInvoiceE = function(owner)
{
	if (owner)
	{
		window.app_dmt_transaksi_fInvoiceE.prototype.parent.constructor.call(this,owner);
		this.className  = "app_dmt_transaksi_fInvoiceE";
		...
		this.e_nb = new saiCBBL(this,{bound:[20,12,223,20],caption:"No Invoice", multiSelection:false, maxLength:10, tag:1, readOnly:true,change:[this,"doChange"]});		
		...
		this.cb_cust = new saiCBBL(this,{bound:[20,15,220,20],caption:"Customer", readOnly:true,tag:2});
		this.e_total = new saiLabelEdit(this,{bound:[720,15,200,20],caption:"Total", tag:1, readOnly:true, tipeText:ttNilai, text:"0"});		
		this.cb_kontrak = new saiCBBL(this,{bound:[20,17,220,20],caption:"Kontrak",readOnly:true,tag:2});						
...
window.app_dmt_transaksi_fInvoiceE.extend(window.childForm);
window.app_dmt_transaksi_fInvoiceE.implement({
		...
		try{									
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{														
					...
					sql.add("delete from dmt_ar_m where no_ar ='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("delete from dmt_ar_j where no_ar ='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("update dmt_bill_d set no_akru='-' where no_ar='"+this.e_nb.getText()+"' and no_akru='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");
					sql.add("update dmt_bill_d set no_ar='-' where no_ar='"+this.e_nb.getText()+"' and kode_lokasi='"+this.app._lokasi+"'");					
	...
	doChange:function(sender){
		if (sender == this.e_periode && this.e_periode.getText()!="") {			
			this.e_nb.setSQL("select a.no_ar, a.keterangan from dmt_ar_m a left join "+
							"( select distinct no_arref as no_ar,kode_lokasi from dmt_ar_m where dc='C' "+
							"  union "+
							"  select distinct no_ar,kode_lokasi from dmt_kaspiutang_d where jenis = 'AR' "+
							") b on a.no_ar=b.no_ar and a.kode_lokasi=b.kode_lokasi "+
							"where b.no_ar is null and a.posted='F' and a.periode='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",["a.no_ar","a.keterangan"],false,["No Inv","Deskripsi"],"and","Daftar Bukti",true);
		}
		if (sender == this.e_nb && this.e_nb.getText()!="") {
			var data = this.dbLib.getDataProvider(					   
					   "select a.periode,a.tanggal,a.no_dokumen,a.no_fp,a.keterangan,a.kode_cust,a.no_kontrak,a.akun_ar,a.due_date,a.nik_app,b.nama as nama_cust,c.keterangan as ket,d.nama as nama_app "+
					   "from dmt_ar_m a "+
					   "       inner join cust b on a.kode_cust=b.kode_cust and a.kode_lokasi=b.kode_lokasi "+
					   "       inner join dmt_kontrak_m c on a.no_kontrak=c.no_kontrak and a.kode_lokasi=c.kode_lokasi "+
					   "       inner join karyawan d on a.nik_app=d.nik and a.kode_lokasi=d.kode_lokasi "+
					   "where a.no_ar='"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object"){
				...
				if (line != undefined){
					...
					var data = this.dbLib.getDataProvider("select a.akun_ar,a.akun_pdpt,a.akun_unbill,a.akun_pdd, "+
						...
						"where a.no_kontrak='"+line.no_kontrak+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
					if (typeof data == "object" && data.rs.rows[0] != undefined){
						var line = data.rs.rows[0];											
			...
			var data = this.dbLib.getDataProvider("select a.no_site,a.periode,case when a.no_akru=a.no_ar then '-' else a.no_akru end as no_akru,(a.rawat+a.fee+a.sewa) as nilai,a.nilai_ar,b.nama as nama_site,c.site_cust "+
						"from dmt_bill_d a inner join dmt_site b on a.no_site=b.no_site and a.kode_lokasi=b.kode_lokasi "+
						"                  inner join dmt_kontrak_d c on a.no_kontrak=c.no_kontrak and a.no_site=c.no_site and a.kode_lokasi=c.kode_lokasi "+
						"where a.no_ar = '"+this.e_nb.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line;
				this.sg.clear();
				for (var i in data.rs.rows){
					line = data.rs.rows[i];							
					this.sg.appendData([line.no_site,line.nama_site,line.site_cust,line.periode,line.no_akru,floatToNilai(line.nilai),floatToNilai(line.nilai_ar)]);
				}
