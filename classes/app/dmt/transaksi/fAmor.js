window.app_dmt_transaksi_fAmor = function(owner)
{
	if (owner)
	{
		window.app_dmt_transaksi_fAmor.prototype.parent.constructor.call(this,owner);
		this.className  = "app_dmt_transaksi_fAmor";
		this.itemsValue = new arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Amortisasi: Input", 0);	
...
window.app_dmt_transaksi_fAmor.extend(window.childForm);
window.app_dmt_transaksi_fAmor.implement({
	...
	simpan: function(){			
		try{						
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"dmt_akru_m","no_akru",this.app._lokasi+"-AMR"+this.e_periode.getText().substr(2,4)+".","000"));
			if (this.standarLib.checkEmptyByTag(this, [0,1,2])){
				try{													
					...
					sql.add("insert into dmt_akru_m(no_akru,no_dokumen,tanggal,keterangan,kode_curr,kurs,nilai,kode_pp,kode_drk,posted,modul,nik_buat,nik_setuju,kode_lokasi,periode,no_del,no_link,nik_user,tgl_input) values "+
						    "('"+this.e_nb.getText()+"','-','"+this.dp_d1.getDateString()+"','"+this.e_ket.getText()+"','IDR',1,"+parseNilai(this.e_nilai.getText())+",'-','-','F','AMOR','"+this.app._userLog+"','"+this.app._userLog+"','"+this.app._lokasi+"','"+this.e_periode.getText()+"','-','-','"+this.app._userLog+"',getdate())");					
	...
	doLoadData: function(sender){
		...
		var strSQL = "select aa.akun_pdd,x.nama as nama_pdd,a.akun_pdpt,y.nama as nama_pdpt,'"+this.app._kodePP+"' as kode_pp,c.nama as nama_pp, "+
	...
	doTampilData: function(page) {
		...
		for (var i=start;i<finish;i++){
			line = this.dataJU.rs.rows[i];							
			this.sg.appendData([line.akun_pdd,line.nama_pdd,line.akun_pdpt,line.nama_pdpt,line.kode_pp,line.nama_pp,floatToNilai(line.nilai)]);
		}
