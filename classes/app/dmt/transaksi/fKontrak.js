window.app_dmt_transaksi_fKontrak = function(owner)
{
	if (owner)
	{
		window.app_dmt_transaksi_fKontrak.prototype.parent.constructor.call(this,owner);
		this.className  = "app_dmt_transaksi_fKontrak";
...
window.app_dmt_transaksi_fKontrak.extend(window.childForm);
window.app_dmt_transaksi_fKontrak.implement({
	...
	simpan: function(){			
		try{						
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"dmt_kontrak_m","no_kontrak",this.app._lokasi+"-SO"+this.e_periode.getText().substr(2,4)+".","000"));
