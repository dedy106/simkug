window.app_dmt_transaksi_fKbPiutang = function(owner)
{
	if (owner)
	{
		window.app_dmt_transaksi_fKbPiutang.prototype.parent.constructor.call(this,owner);
		this.className  = "app_dmt_transaksi_fKbPiutang";
		...
		this.cb_piutang = new saiCBBL(this,{bound:[20,17,220,20],caption:"No Invoice", multiSelection:false, maxLength:10, tag:1,change:[this,"doChange"]});						
...
window.app_dmt_transaksi_fKbPiutang.extend(window.childForm);
window.app_dmt_transaksi_fKbPiutang.implement({
	...
	doChange:function(sender){				
		if (sender == this.cb_cust && this.cb_cust.getText()!="") {			
			...
			this.cb_piutang.setSQL("select a.no_ar,(a.no_dokumen+'-'+a.keterangan) as keterangan from dmt_ar_m a "+
								   ...
								   "            union "+
								   "            select no_ar,kode_lokasi,sum(nilai) as bayar from dmt_kaspiutang_d where jenis = 'AR' group by no_ar,kode_lokasi) b on a.no_ar=b.no_ar and a.kode_lokasi=b.kode_lokasi "+
								   "where (a.nilai+a.ppn) > isnull(b.bayar,0) and a.periode<='"+this.e_periode.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",
		...
		if (sender == this.cb_piutang && this.cb_piutang.getText()!="") {
			var data = this.dbLib.getDataProvider("select a.akun_ar,a.nilai,a.ppn,(a.nilai+a.ppn) - isnull(b.bayar,0) as saldo from dmt_ar_m a "+
								   " left join (select no_arref as no_ar,kode_lokasi,sum(nilai+ppn) as bayar from dmt_ar_m where dc='C' group by no_arref,kode_lokasi "+
								   "            union "+
								   "            select no_ar,kode_lokasi,sum(nilai) as bayar from dmt_kaspiutang_d where jenis = 'AR' group by no_ar,kode_lokasi) b on a.no_ar=b.no_ar and a.kode_lokasi=b.kode_lokasi "+
								   "where (a.nilai+a.ppn) > isnull(b.bayar,0) and a.no_ar='"+this.cb_piutang.getText()+"' and a.kode_lokasi='"+this.app._lokasi+"'",true);
			if (typeof data == "object" && data.rs.rows[0] != undefined){
				var line = data.rs.rows[0];							
