...
//* 			SAI, PT											
//***********************************************************************************************
window.app_tanbang_fMain = function(owner)
{
	if (owner)
	{	
		try
		{
			window.app_tanbang_fMain.prototype.parent.constructor.call(this, owner);
			window.app_tanbang_fMain.prototype.parent.setWidth.call(this, 600);
			window.app_tanbang_fMain.prototype.parent.setHeight.call(this, 450);
			this._mainButtonClick = new portalui_eventHandler();			
			this.title = "Asset Management Unit";
			document.title = "TANBANG";
			...
			this.setTop(60);						
			this.maximize();										
			this.pTool = new panel(this,{bound:[0,0,this.width,30],color:system.getConfig("app.color.panel"),shadow:true});							
...
////------------------------------------ panel button -------------------------		
			this.pButton = new panel(this,{bound:[0,55,this.width,31],border:1,color:system.getConfig("app.color.panel"),shadow:true});
...
//------------------------------------ panel transaksi -------------------------
			this.pTrans = new panel(this.pButton,{bound:[2,1,500,26],border:0,color:system.getConfig("app.color.panel")});						
			this.bTutup = new button(this.pButton,{bound:[this.width-90,4,80,20],caption:"Tutup",icon:"url(icon/"+system.getThemes()+"/exit.png)",hint:"Exit Modul",click:[this,"bTutupClick"]});
			this.bTutup.hide();			
...
//--------------- panel login -------------------------		
			this.pLogin = new panel(this.pButton,{bound:[2,1,500,26],border:0,color:system.getConfig("app.color.panel"),visible:false});
...
//------------------------------------ panel report -------------------------		
			this.pReport = new panel(this.pButton,{bound:[2,1,500,26],visible:false,border:0,color:system.getConfig("app.color.panel")});
...
//------------------------------------ panel Editor-------------------------		
			this.pEditor = new panel(this.pButton,{bound:[2,1,500,26],visible:false,border:0,color:system.getConfig("app.color.panel")});
...
//------------------------------------ panel Editor-------------------------		
			this.pQuery = new panel(this.pButton,{bound:[2,1,500,26],visible:false,border:0,color:system.getConfig("app.color.panel")});
...
//------------------------------------ panel Message-------------------------		
			this.pMsg = new panel(this.pButton,{bound:[2,1,500,26],visible:false,border:0,color:system.getConfig("app.color.panel")});
...
//----------------------------------------------- report Navigator									
			this.reportNavigator = new reportNavigator(this,{bound:[0,55,this.width,31],visible:false,color:system.getConfig("app.color.panel"),border:1,shadow:true});			
//----------------------------------------------------------------- tree View-----------------------------------						
...
//--------------------------------------------------
			...
			this.setImage("url(image/themes/"+system.getThemes()+"/telkom2.png)");			
			...
			this.infoUnit = new control(this,{bound:[this.treev.width + 30,this.treev.top + this.treev.height - 30,this.width - this.treev.width - 40,60]});
			...
			this.infoPerusahaan.getCanvas().innerHTML = "<table border=0 cellspacing=0 cellpadding=0 width='100%'>"+
					...
					"<tr><td align='center'>"+
					"<span style='font-size:40pt;font-weight:bold;color:#299bcf;font-family:Arial, Helvetica, Sans-serif;'> TANBANG</span>"+
					"</td></tr>"+
					"<tr><td align='center'>"+
					"<span style='font-size:14pt;font-weight:bold;color:#ff9900;font-family:Arial, Helvetica, Sans-serif;'>(Informasi Tanah dan Bangunan)</span>"+
					"</td></tr>"+					
					" </table>";
...
window.app_tanbang_fMain.extend(window.form);
window.app_tanbang_fMain.implement({
	gotoLogin: function(){
		uses("app_tanbang_fLogin");
		this.form = new app_tanbang_fLogin(this);
	...
	doAfterLogin: function(){
		try{		    
			...
			this.msgBoard = new control(this,{bound:[this.width - 270,85,260,200]});//this.treev.height - 70
			...
			this.msgBoard.scrollbarV.style.cssText = "position:absolute;left:250;top:0;width:10;height:100%;z-index:2;overflow:hidden";
			this.msgBoard.scroll = document.createElement("div");
			this.msgBoard.scroll.style.cssText = "background:url(image/scrollbarV.png);position:absolute;left:0;top:0;width:10;height:100%;z-index:2";
			...
			this.cnvInfo.style.cssText = "position:absolute;left:0;top:0;width:250;height:100%;overflow:hidden;";
			...
			//eventOn(this.msgBoard.scroll,"mouseup","$$(" + this.resourceId + ").eventMouseScrollUp(event);");
			//eventOn(this.cnvInfo,"mouseup","$$(" + this.resourceId + ").eventMouseScrollUp(event);");
			//eventOn(this.cnvInfo,"mousemove","$$(" + this.resourceId + ").eventMouseScrollMove(event);");
			
			this.msgBoard.ajax = new util_ajaxCaller("server/serverApp.php");
			this.msgBoard.ajax.setFrequency(5000);
			...
			this.indexInfo=0;
			this.addInfoItem("a",{total:7, caption:"testing 1"}, new arrayList({items:[
				{jenis:"", no_bukti:"", keterangan:"testing juga"},
				{jenis:"", no_bukti:"", keterangan:"testing juga"},
				{jenis:"", no_bukti:"", keterangan:"testing juga"},
				{jenis:"", no_bukti:"", keterangan:"testing juga"},
				{jenis:"", no_bukti:"", keterangan:"testing juga"},
			...
			this.addInfoItem("b",{total:7, caption:"testing 1"}, new arrayList({items:[
				{jenis:"", no_bukti:"", keterangan:"testing juga"},
				{jenis:"", no_bukti:"", keterangan:"testing juga"},
				{jenis:"", no_bukti:"", keterangan:"testing juga"},
				{jenis:"", no_bukti:"", keterangan:"testing juga"},
	...
	addInfoItem: function(id, data, detail){
		try{			
			...
			html +=" <div style='position:absolute;top:0;left:0;width:100%;height:100%;background:url(image/whitegradsmall.png)0 bottom repeat-x;filter:alpha(opacity:100);opacity:0.9;moz-opacity:0.9'></div>";
			html += "<div id='"+id+"_h' style='position:relative;height:40;top:0;left:0;width:100%;background:url(image/themes/dynpro/roundpanelBg.png)0 0 repeat-x;'><img width='30' height='25' style='position:absolute;left:5;top:10;width:30;height:25;' src='image/infodialog2.png'/><span style='font-weight:bold;position:absolute;left:35;top:15;width:190;height:25'>"+data.total+" "+data.caption+"</span><img id='"+id+"_btn' width='25' height='25' src='image/navigate-left-icon24.png' style='position:absolute;left:220;top:10;width:25;height:25;cursor:pointer' onclick='$$("+this.resourceId +").doItemClick(event,\""+id+"\");'/></div>";
		...
		try{
			...
			if (node) {
				if (node.style.display == "none"){
					target.src = "image/navigate-down-icon24.png";
					node.style.display = "";
				}else{
					 target.src = "image/navigate-left-icon24.png";
	...
	bTutupClick: function(sender){	
		try{			
			if ((sender == this.bTutup)||(sender == this.bExit)){
				...
				if (this.msgBoard) this.msgBoard.show();
			}
	...
	doRequestReady: function(sender, methodName, result, errCode, connection){
		...
		if (sender == this.dbLib2 && connection == this.msgBoard.ajax){					
			if (methodName == "getDataProvider"){
				if (errCode == 200){
					var beda = true;
					try{
					...
					if (beda){
						if (this.form != undefined && this.timer) this.timer.setEnabled(true);
					}
				}
			}
		}
	...
	doBtnList: function(sender){
		if (sender == this.btnList){
			this.msgBoard.setVisible(!this.msgBoard.visible);
...
//------------------------------------------------------------------- locking transaction		
	...
	doSendMsg: function(sender){		
		this.dbLib.execArraySQL(new server_util_arrayList({items:["insert into amu_msg(msg,nik_user, tgl_input)values('"+this.sendMsgPanel.editor.getText()+"','"+this.app._userLog+"',sysdate)"]}));
	}
});
