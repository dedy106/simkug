...
//* 			SAI, PT											
//***********************************************************************************************
window.app_tanbang_app = function(owner){
	window.app_tanbang_app.prototype.parent.constructor.call(this, owner);
	this.className = "app_tanbang_app";
	this.onTerminate.set(this,"doTerminate");
	this.form = undefined;
	this.setTitle("T A N B A N G");
	this.showForm();
};
window.app_tanbang_app.extend(window.application);
window.app_tanbang_app.implement({
	showForm: function(){
		try
		{
			uses("util_dbLib;form;childForm");		
			this._dbSetting = "oraveat";	
			this.dbLib = new util_dbLib(undefined, "oraveat");
			this._mainForm = new app_tanbang_fMain(this);	
	...
	doKeyDown: function(charCode, buttonState, keyCode, event){
		try{			
			window.app_tanbang_app.prototype.parent.doKeyDown.call(this, charCode, buttonState, keyCode, event);
