window.app_eclaim2_master_fLokasi = function(owner)
{
	if (owner){
		window.app_eclaim2_master_fLokasi.prototype.parent.constructor.call(this, owner);
		this.className = "app_eclaim2_master_fLokasi";
...
window.app_eclaim2_master_fLokasi.extend(window.childForm);
window.app_eclaim2_master_fLokasi.implement({
			...
			switch (event){
				...
				case "simpan" :
					...
					try{
						...
						sql.add("delete from tlk_lokasi where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg = '"+this.cb_ttg.getText()+"' ");						
						var data = value.replace(/#/g,",");
						sql2 = "insert into tlk_lokasi(kode_lok, kode_ttg, kode_lokasi,nama, level_lap,level_spasi, tipe, sum_header, kode_induk, rowindex, nik_user, tgl_input) values ";
						sql2 += " "+ data +" ";
						sql.add(sql2);						
	...
	doClick: function(sender){	
		var nrc = this.dblib.runSQL("select kode_lok, kode_ttg, kode_lokasi,nama, level_lap,level_spasi, tipe, sum_header, kode_induk, rowindex, nik_user, tgl_input from tlk_lokasi where  kode_lokasi = '"+this.app._lokasi+"' order by rowindex",0,0);	
		this.loadMenu(nrc);	
	},	
	entriesClick: function(sender){
		...
		{ 
			if (sender == this.reloadBtn){				
				var nrc = this.dblib.getDataProvider("select kode_lok, kode_ttg, kode_lokasi,nama, level_lap,level_spasi, tipe, sum_header, kode_induk, rowindex, nik_user, tgl_input from tlk_lokasi where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg = '"+this.cb_ttg.getText()+"' order by rowindex",true);
				this.loadMenu(nrc);		
...
window.app_eclaimg_master_fFrameAddLks.extend(window.commonForm);
