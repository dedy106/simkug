window.app_eclaim2_master_fKota = function(owner)
{
	if (owner){
		window.app_eclaim2_master_fKota.prototype.parent.constructor.call(this, owner);
		this.className = "app_eclaim2_master_fKota";
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Kota", 0);	
		
		this.maximize();
		uses("saiCBBL");
		this.cb_ttg = new saiCBBL(this,{bound:[20,10,200,20],caption:"Tertanggung",btnClick:[this,"FindBtnClick"]});
		
		this.addBtn = new imageButton(this,{bound:[27,20,22,22],image:"icon/"+system.getThemes()+"/createentries.png",hint:"Add",click:[this,"entriesClick"]});		
		this.editBtn = new imageButton(this,{bound:[47,20,22,22],image:"icon/"+system.getThemes()+"/editentries.png",hint:"Edit",click:[this,"entriesClick"]});
		this.delBtn = new imageButton(this,{bound:[67,20,22,22],image:"icon/"+system.getThemes()+"/delentries.png",hint:"Delete",click:[this,"entriesClick"]});		
		this.relBtn = new imageButton(this,{bound:[87,20,22,22],image:"icon/"+system.getThemes()+"/relakun.png",hint:"Deselect",click:[this,"entriesClick"]});						
		this.downBtn = new imageButton(this,{bound:[107,20,22,22],image:"icon/"+system.getThemes()+"/down.png",hint:"Geser ke Bawah",click:[this,"entriesClick"]});								
		this.upBtn = new imageButton(this,{bound:[127,20,22,22],image:"icon/"+system.getThemes()+"/up.png",hint:"Geser ke Atas",click:[this,"entriesClick"]});								
		this.leftBtn = new imageButton(this,{bound:[147,20,22,22],image:"icon/"+system.getThemes()+"/bleft.png",hint:"Geser Kiri",click:[this,"entriesClick"]});				
		this.rightBtn = new imageButton(this,{bound:[167,20,22,22],image:"icon/"+system.getThemes()+"/bright.png",hint:"Geser Kanan",click:[this,"entriesClick"]});				
		this.reloadBtn = new imageButton(this,{bound:[197,20,22,22],image:"icon/"+system.getThemes()+"/reload.png",hint:"Refresh",click:[this,"entriesClick"]});								
		this.treev = new treeView(this,{bound:[20,50,700,this.getHeight() - 130],childLength:700,dblClick:[this,"treeClick"]});
...
window.app_eclaim2_master_fKota.extend(window.childForm);
window.app_eclaim2_master_fKota.implement({
			...
			switch (event){
				...
				case "simpan" :
					...
					try{
						...
						sql.add("delete from eclaim_kota where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg = '"+this.cb_ttg.getText()+"' ");						
						var data = value.replace(/#/g,",");
						sql2 = "insert into eclaim_kota(kode_kota, kode_ttg, kode_lokasi,nama, level_lap,level_spasi, tipe, sum_header, kode_induk, rowindex, nik_user, tgl_input) values ";
						sql2 += " "+ data +" ";
						sql.add(sql2);						
	...
	doClick: function(sender){	
		var nrc = this.dblib.runSQL("select kode_kota, kode_ttg, kode_lokasi,nama, level_lap,level_spasi, tipe, sum_header, kode_induk, rowindex, nik_user, tgl_input from eclaim_kota where  kode_lokasi = '"+this.app._lokasi+"' order by rowindex",0,0);	
		this.loadMenu(nrc);	
	},	
	entriesClick: function(sender){
		...
		{ 
			if (sender == this.reloadBtn){				
				var nrc = this.dblib.getDataProvider("select kode_kota, kode_ttg, kode_lokasi,nama, level_lap,level_spasi, tipe, sum_header, kode_induk, rowindex, nik_user, tgl_input from eclaim_kota where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg = '"+this.cb_ttg.getText()+"' order by rowindex",true);
				this.loadMenu(nrc);		
				...
				if (sender == this.addBtn || sender == this.editBtn){
					if (this.frameAdd == undefined){
						this.frameAdd = new app_eclaimg_master_fFrameAdd(this.app, this);						
					}
				}
	...
	copyChilds : function(from, to){
		...
		{			
			...
			chld.data.kode_kota = temp.getKode();					
	...
	switchItem : function(from, to){
		...
		child.data.kode_kota = item.getKode();
...
//------------------------------------- Frame
window.app_eclaimg_master_fFrameAdd = function(owner, requester){
	if (owner){
		window.app_eclaimg_master_fFrameAdd.prototype.parent.constructor.call(this, owner);
		this.setHeight(150);
		this.setWidth(400);
		this.className = "app_eclaimg_master_fFrameAdd";
		this.centerize();		
		this.e0 = new saiLabelEdit(this,{bound:[20,20,220,20],caption:"Kode Objek"});		
		this.e1 = new saiLabelEdit(this,{bound:[20,43,350,20],caption:"Nama"});						
		this.e2 = new saiCB(this,{bound:[20,66,220,20],caption:"Tipe",items:["Header","Posting"]});		
		
		this.b1 = new imageButton(this,{bound:[20,90,22,22],image:"icon/"+system.getThemes()+"/bOk.png",hint:"Ok",click:"doClick"});
		this.b2 = new imageButton(this,{bound:[42,90,22,22],image:"icon/"+system.getThemes()+"/bCancel.png",hint:"Cancel",click:"doClick"});				
		system.addMouseListener(this);
		this.formRequester = requester;
	}
};
window.app_eclaimg_master_fFrameAdd.extend(window.commonForm);
window.app_eclaimg_master_fFrameAdd.implement({
	...
	doSysMouseMove: function(x, y, button, buttonState){
		window.app_eclaimg_master_fFrameAdd.prototype.parent.doSysMouseMove.call(this,x, y, button, buttonState);
		if (this.isClick)
		{		
