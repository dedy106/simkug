window.app_eclaim2_master_fPenanggung = function(owner) {
	if (owner){
		window.app_eclaim2_master_fPenanggung.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_master_fPenanggung";		
...
window.app_eclaim2_master_fPenanggung.extend(window.childForm);
window.app_eclaim2_master_fPenanggung.implement({
	...
	doModalResult: function(event, result){				
		try{			
			...
			switch(event){
				case "simpan" :
					sql.add("insert into tlk_png (kode_png, nama, alamat, kota, kodepos, no_telp, no_fax, pic, email, web, npwp, skode, status,kode_lokasi, tgl_input, nik_user)values"+
				...
				break;
				case "ubah" :
					sql.add("update tlk_png set nama = '"+this.ed_nama.getText()+"', skode='"+this.ed_skode.getText()+"' "+
				...
				break;
				case "hapus" :
					sql.add("delete from tlk_png where kode_lokasi = '"+this.app._lokasi+"' and kode_png ='"+this.ed_kode.getText()+"'  ");
				break;
			}
	...
	doChange: function(sender){
		...
		this.ed_skode.clear();
		if (sender.getText() != ""){	
			var kode = this.dbLib.getDataProvider("select nama, alamat, kota, kodepos, no_telp, no_fax, pic, email, web, npwp, skode from tlk_png where kode_lokasi = '"+this.app._lokasi+"' and kode_png = '"+sender.getText()+"' ",true);
	...
	doClick: function(sender){
		this.ed_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'tlk_png','kode_png',"PG",'000'));
	},
