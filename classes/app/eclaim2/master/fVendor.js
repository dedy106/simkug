window.app_eclaim2_master_fVendor = function(owner) {
	if (owner){
		window.app_eclaim2_master_fVendor.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_master_fVendor";		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Vendor", 0);	
		
		uses("checkBox");
		this.ed_kode = new saiCBBL(this,{bound:[20,1,200,20], caption:"Kode Vendor", rightLabelVisible:false, btnClick:[this,"doFindBtnClick"],change:[this,"doChange"]});
...
window.app_eclaim2_master_fVendor.extend(window.childForm);
window.app_eclaim2_master_fVendor.implement({
	...
	doModalResult: function(event, result){				
		try{			
			...
			switch(event){
				case "simpan" :
					sql.add("insert into tlk_vendor (kode_vendor, nama, alamat, kota, kodepos, no_telp, no_fax, pic, email, web, npwp, skode, status,kode_lokasi, tgl_input, nik_user)values"+
				...
				break;
				case "ubah" :
					sql.add("update tlk_vendor set nama = '"+this.ed_nama.getText()+"', skode='"+this.ed_skode.getText()+"' "+
					...
					" where kode_lokasi = '"+this.app._lokasi+"' and kode_vendor ='"+this.ed_kode.getText()+"'  ");
				break;
				case "hapus" :
					sql.add("delete from tlk_vendor where kode_lokasi = '"+this.app._lokasi+"' and kode_vendor ='"+this.ed_kode.getText()+"'  ");
				break;
			}
	...
	doFindBtnClick: function(sender){
		this.standarLib.showListData(this, "Data Tertanggung",this.ed_kode,this.ed_nama, 
										  ...
										  ["kode_vendor","nama","skode"],"and",["Kode Tertanggung","Deskripsi"],false); 
	},
	doChange: function(sender){
		this.ed_nama.clear();
		this.ed_skode.clear();
		if (sender.getText() != ""){	
			var kode = this.dbLib.getDataProvider("select nama, alamat, kota, kodepos, no_telp, no_fax, pic, email, web, npwp, skode from tlk_vendor where kode_lokasi = '"+this.app._lokasi+"' and kode_vendor = '"+sender.getText()+"' ",true);
	...
	doClick: function(sender){
		this.ed_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'tlk_vendor','kode_vendor',"MT",'000'));
	},
