window.app_eclaim2_master_fLostAd = function(owner) {
	if (owner){
		window.app_eclaim2_master_fLostAd.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_master_fLostAd";		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Loss Adjuster", 0);	
		
		uses("checkBox");
		this.ed_kode = new saiCBBL(this,{bound:[20,1,200,20], caption:"Kode", btnClick:[this,"doFindBtnClick"],rightLabelVisible:false,change:[this,"doChange"]});
...
window.app_eclaim2_master_fLostAd.extend(window.childForm);
window.app_eclaim2_master_fLostAd.implement({
	...
	doModalResult: function(event, result){				
		try{			
			...
			switch(event){
				...
				break;
				case "simpan" :
					sql.add("insert into tlk_la (kode_la, nama, alamat, kota, kodepos, no_telp, no_fax, pic, email, web, npwp, skode, status,kode_lokasi, tgl_input, nik_user)values"+
				...
				break;
				case "ubah" :
					sql.add("update tlk_la set nama = '"+this.ed_nama.getText()+"', skode='"+this.ed_skode.getText()+"' "+
				...
				break;
				case "delete" :
					sql.add("delete from tlk_la where kode_lokasi = '"+this.app._lokasi+"' and kode_la ='"+this.ed_kode.getText()+"'  ");
				break;
			}
	...
	doChange: function(sender){
		...
		this.ed_skode.clear();
		if (sender.getText() != ""){	
			var kode = this.dbLib.getDataProvider("select nama, alamat, kota, kodepos, no_telp, no_fax, pic, email, web, npwp, skode from tlk_la where kode_lokasi = '"+this.app._lokasi+"' and kode_la = '"+sender.getText()+"' ",true);
	...
	doClick: function(sender){
		this.ed_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'tlk_la','kode_la',"LA",'000'));
	},
