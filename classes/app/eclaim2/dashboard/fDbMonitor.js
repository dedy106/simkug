window.app_eclaim2_dashboard_fDbMonitor = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_dashboard_fDbMonitor.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim2_dashboard_fDbMonitor";
		...
		this.pc1 = new pageControl(this,{bound:[15,12,980,500], childPage:["Filter","Data Monitoring","Rekapitulasi Klaim","Data Klaim","Data Riwayat Klaim"]});
		/*
...
window.app_eclaim2_dashboard_fDbMonitor.extend(window.childForm);
window.app_eclaim2_dashboard_fDbMonitor.implement({
	...
	doViewItem: function(event,no_bukti){
		try{
			...
			var sql="select a.no_klaim, a.no_dokumen, date_format(a.tanggal,'%d-%m-%Y') as tanggal, b.nama as nama_lok, c.nama as nama_obyek, d.nama as nama_sebab, a.nilai, e.nilai as nilai_adjust, e.nilai_ddct as nilai_bayar,a.alamat "+
					...
					" left join tlk_adjust e on e.kode_lokasi = a.kode_lokasi and e.no_klaim = a.no_klaim and e.status = '1' "+this.filter+tmp+
	...
	doSgClick: function(sender, col , row) 
	{
		try{
			...
			lokasi = "";
			filter2 = "";
			this.docViewer.src = this.report.previewWithHeader("server_report_eclaim2_rptRwyKlaimSum",filter, 1,1, this.showFilter, lokasi,filter2);
	...
	doRekapKlaim: function(filter,filter2) 
	{
		try{
			...
			this.docViewer2.src = this.report.previewWithHeader("server_report_eclaim2_rptRekapKlaim2",this.filter, 1,1, this.showFilter, this.lokasi,this.filter2);
