window.app_eclaim2_transaksi_fAdjust = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fAdjust.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fAdjust";		
		...
		this.l_tgl = new label(this,{bound:[20,1,100,20],caption:"Tanggal Adjustment",underline:true});
		this.dp_tgl = new datePicker(this,{bound:[120,1,100,18], selectDate:[this,"doSelectedDate"]});
		this.ed_kode = new saiLabelEdit(this,{bound:[20,2,200,20],caption:"No Adjustment",readOnly:true});
		...
		this.cb_survey = new saiCBBL(this,{bound:[20,4,220,20],caption:"No Survey", btnClick:[this,"doFindBtnClick"]});
...
window.app_eclaim2_transaksi_fAdjust.extend(window.childForm);
window.app_eclaim2_transaksi_fAdjust.implement({
	...
	doFindBtnClick: function(sender){
		...
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, alamat from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('4') and flag6='0' and status='1' ",
										  "select count(no_klaim) from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('4')  and flag6='0' and status='1' ",
	...
	doChange: function(sender){		
		try{
			if (sender == this.cb_klaim){
				var sql = new server_util_arrayList();
				sql.add("select a.nilai,b.no_survey from tlk_klaim a "+
						"inner join tlk_survey b on a.no_klaim=b.no_klaim "+
						"where a.no_klaim = '"+sender.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' ");
				...
				if (typeof data != "String" && data.result[0].rs.rows[0] != undefined){
					...
					this.cb_survey.setText(line.no_survey);
