window.app_eclaim2_transaksi_fAdjustV = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_transaksi_fAdjustV.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim2_transaksi_fAdjustV";
		...
		this.tahun=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from tlk_adjust");
		...
		this.sg1mp = new saiGrid(this.pc1.childPage[1],{bound:[1,20,this.pc1.width - 4,this.pc1.height - 180],colCount:10,
					colTitle:["No. Adjustment","No. Klaim","Tanggal","No. Dokumen","Keterangan","Nilai Estimasi","Nilai Adjust","Nilai DDCT","File Dok","Hist Adjust"],
...
window.app_eclaim2_transaksi_fAdjustV.extend(window.childForm);
window.app_eclaim2_transaksi_fAdjustV.implement({
	...
	doEllipseClick:function(sender, col, row)
	{
		if (row == 2)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Klaim",this.sg1, this.sg1.row, this.sg1.col,
												...
												"from tlk_klaim a "+
												"inner join tlk_adjust b on a.no_klaim=b.no_klaim "+filter,
	...
	doSelectCell:function(sender, col, row)
	{
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from tlk_adjust order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from tlk_adjust order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
		...
		try
		{
			...
			else
			{
				...
				var sql="select a.no_adjust,a.no_klaim,c.nama as nama_ttg,a.no_survey,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
						...
						"inner join tlk_survey d on a.no_survey=d.no_survey "+this.filter+user_lok+
	...
	doSgClick:function(sender, col, row){	
		...
		if (col==8){
			try{
				var data = this.dbLib.getDataProvider("select nama as nm from tlk_adjust_dok "+
		...
		if (col==9){
			try{
				var sql="select a.no_adjust,a.no_klaim,c.nama as nama_ttg,a.no_survey,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
						...
						"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
						"inner join tlk_ttg c on b.kode_ttg=c.kode_ttg "+
						"inner join tlk_survey d on a.no_survey=d.no_survey "+
