window.app_eclaim2_transaksi_fLAk = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fLAk.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fLAk";		
...
window.app_eclaim2_transaksi_fLAk.extend(window.childForm);
window.app_eclaim2_transaksi_fLAk.implement({
	...
	doModalResult: function(event, result){				
		try{
			...
			switch(event){
					...
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						*/
						...
						sql.add("insert into tlk_klaim (no_klaim, kode_ttg, periode, tanggal, alamat, kode_curr, kurs,nik_buat,kode_obyek, kode_sebab, kode_asuransi, pic, no_telp, "+
							" no_fax, penyebab,  tindakan, progress,kode_lok, no_dokumen, no_polis, tgl_dokumen,nilai , kode_lokasi, tgl_input, nik_user, host, ip, keterangan,status)values"+
							...
							" '"+this.app._lokasi+"',now(),'"+this.app._userLog+"','"+this.app._hostname+"','"+this.app._iphost+"','-','1') ");
						/*
						if (files.length > 0){
				...
				break;									
				case "hapus" :
					sql.add("delete from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and no_klaim ='"+this.ed_kode.getText()+"' and progress='0' and status='1' ");
					sql.add("delete from tlk_klaim_dok where no_klaim ='"+this.ed_kode.getText()+"' ");
					this.dbLib.execArraySQL(sql);
				break;
			}
		...
		switch(sender){
			...
			*/
			...
			case this.ed_kode : this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
											  "select no_klaim, alamat,nilai from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress = '0' and nik_buat = '"+this.app._userLog+"' ",
											  "select count(*) from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress = '0' and nik_buat = '"+this.app._userLog+"' ",
											  ["no_klaim","alamat","nilai"],"and",["No Klaim","Lokasi","Nilai"],false);
								break;
		}		
	},
