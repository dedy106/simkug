window.app_eclaim2_transaksi_fTestingK = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fTestingK.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fTestingK";		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Testing Hasil Survey: Koreksi", 0);	
		...
		this.ed_kode = new saiCBBL(this,{bound:[20,2,220,20],caption:"No Testing",readOnly:true, multiSelection:false,change:[this,"doChange"]});	
		...
		this.separator = "/";
		this.ed_kode.setSQL("select a.no_testing, a.keterangan, a.no_klaim "+
							"from tlk_testing a "+
							"inner join tlk_klaim b on b.no_klaim = a.no_klaim and b.kode_lokasi = a.kode_lokasi "+
							"where a.kode_lokasi = '"+this.app._lokasi+"' and kode_ttg = '"+this.app._kodeTtg+"' and b.progress='4' "
							,["no_testing","keterangan","no_klaim"],false,["No Ver","Keterangan","No Klaim"],"and","Data Verifikasi Klaim",true);

		this.onClose.set(this,"doClose");
	}
};
window.app_eclaim2_transaksi_fTestingK.extend(window.childForm);
window.app_eclaim2_transaksi_fTestingK.implement({
			...
			switch(event){
				...
				case "ubah" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){						
						...
						sql.add("delete from tlk_testing_dok where no_testing = '"+this.ed_kode.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
						sql.add("update tlk_testing set periode='"+this.ed_Periode.getText()+"', tanggal='"+this.dp_tgl.getDateString()+"', no_dokumen='"+this.ed_dok.getText()+"', tgl_input=now(), nik_user='"+this.app._userLog+"', keterangan = '"+this.ed_ket.getText()+"' "+
							" where no_testing = '"+this.ed_kode.getText()+"' and kode_lokasi =  '"+this.app._lokasi+"' ");
						sql.add("update tlk_klaim set progress = '4',flag4='1' where no_klaim = '"+this.cb_klaim.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");
						if (files.length > 0){
							var scan = "insert into tlk_testing_dok (no_testing,kode_lokasi,no_file,nama,nu) values ";
							var first = true;
							var noUrut=1;
							for (var i in files){
								if (!first) scan +=",";
				...
				case "hapus":
					...
					sql.add("update tlk_klaim set progress = '3',flag4='0' where no_klaim = '"+this.cb_klaim.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");
					sql.add("delete from tlk_testing_dok where no_testing = '"+this.ed_kode.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
					sql.add("delete from tlk_testing where no_testing = '"+this.ed_kode.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
					this.dbLib.execArraySQL(sql);
				break;
			}			
		}catch(e){
	...
	doFindBtnClick: function(sender){
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, alamat from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"'  and progress='4' and status='1' ",
										  "select count(no_klaim) from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"'  and progress='4' and status='1' ",
										  ["no_klaim","alamat"],"and",["No Klaim","Lokasi"],false); 
		}		
	},	
	doClick: function(sender){
		this.ed_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'tlk_testing','no_testing',"TST/"+this.ed_Periode.getText().substring(2)+"/",'0000'));
	},
	...
	doChange: function(sender){
		try{
			if (sender == this.ed_kode){
				var sql = new server_util_arrayList();
				sql.add("select periode, no_klaim,date_format(tanggal, '%d-%m-%Y') as tgl, no_dokumen, keterangan from tlk_testing where no_testing = '"+sender.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
				sql.add("select no_file, nu from tlk_testing_dok where no_testing = '"+sender.getText()+"' and kode_lokasi = '"+this.app._lokasi +"' ");
