window.app_eclaim2_transaksi_fBASTParsialK = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fBASTParsialK.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fBASTParsialK";		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Berita Acara Serah Terima Parsial: Koreksi", 0);	
		...
		this.sg1 = new saiGrid(this.p1.childPage[0],{bound:[1,5,597,195],colCount:5,colTitle:"No BAUT,Tanggal,Keterangan, No Dokumen,Kode Vendor,Nama", colWidth:[[4,3,2,1,0],[150,100,200,80,100]],
...
window.app_eclaim2_transaksi_fBASTParsialK.extend(window.childForm);
window.app_eclaim2_transaksi_fBASTParsialK.implement({
			...
			switch(event){
				...
				case "ubah" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){						
						...
						sql.add("delete from tlk_bast_dok where no_bast = '"+this.ed_kode.getText()+"' ");
						sql.add("delete from tlk_bast_d where no_bast = '"+this.ed_kode.getText()+"' ");
						sql.add("update tlk_bast set periode='"+this.ed_Periode.getText()+"', tanggal='"+this.dp_tgl.getDateString()+"' "+
							" , no_dokumen='"+this.ed_dok.getText()+"', keterangan='"+this.ed_ket.getText()+"' "+
						...
						sql.add("update tlk_klaim set progress = '11',flag11='1' where no_klaim = '"+this.cb_klaim.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");
						if (files.length > 0){
				...
				case "hapus":
					...
					sql.add("delete from tlk_bast_dok where no_bast = '"+this.ed_kode.getText()+"' ");
					sql.add("delete from tlk_bast_d where no_bast = '"+this.ed_kode.getText()+"' ");
					sql.add("delete from tlk_bast where no_bast = '"+this.ed_kode.getText()+"' ");
					sql.add("update tlk_klaim set progress = '10',flag11='0' where no_klaim = '"+this.cb_klaim.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");
					this.dbLib.execArraySQL(sql);
				break;		
			}			
		}catch(e){
	...
	doFindBtnClick: function(sender){
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, penyebab from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('11')and flag11='1' and status='1' ",
										  "select count(no_klaim) from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('11') and flag11='1' and status='1' ",
	...
	doChange: function (sender){		
		try{
			...
			if (sender == this.cb_klaim){
				this.ed_kode.setSQL("select no_bast, keterangan from tlk_bast where kode_lokasi = '"+this.app._lokasi+"' and no_klaim='"+sender.getText()+"' and jenis = 'PAR'",["no_bast","keterangan"],false, ["No BAST","Keterangan"],"and","Daftar BAST", true);
