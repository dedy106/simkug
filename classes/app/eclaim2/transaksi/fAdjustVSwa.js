window.app_eclaim2_transaksi_fAdjustVSwa = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_transaksi_fAdjustVSwa.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim2_transaksi_fAdjustVSwa";
		...
		this.pc1 = new pageControl(this,{bound:[15,12,1000,500], childPage:["Filter","Data Konfirmasi","Data Klaim","Data Riwayat Klaim","History Konfirmasi"]});
		...
		this.tahun=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from tlk_adjust_swa");
		...
		this.gridLib.SGEditData(this.sg1,4,new Array(0,1), new Array("No Konfirmasi","All"));
		
		this.sg1mp = new saiGrid(this.pc1.childPage[1],{bound:[1,20,this.pc1.width - 4,this.pc1.height - 180],colCount:10,
					colTitle:["No. Konfirmasi","No. Klaim","Tanggal","No. Dokumen","Keterangan","Nilai Estimasi","Nilai Adjust","Nilai DDCT","File Dok","Hist Adjust"],
					colWidth:[[9,8,7,6,5,4,3,2,1,0],[60,60,80,80,80,200,100,60,100,100]],
		...
		this.sg3mp = new saiGrid(this.pc1.childPage[4],{bound:[1,20,this.pc1.width - 4,this.pc1.height - 180],colCount:9,
					colTitle:["No. Konfirmasi","No. Klaim","Tanggal","No. Dokumen","No Survey","Keterangan","Nilai Estimasi","Nilai Adjust","File Dok"],
...
window.app_eclaim2_transaksi_fAdjustVSwa.extend(window.childForm);
window.app_eclaim2_transaksi_fAdjustVSwa.implement({
	...
	doEllipseClick:function(sender, col, row)
	{
		if (row == 2)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Klaim",this.sg1, this.sg1.row, this.sg1.col,
												...
												"from tlk_klaim a "+
												"inner join tlk_adjust_swa b on a.no_klaim=b.no_klaim "+filter,
		...
		if (row == 4)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Verifikasi",this.sg1, this.sg1.row, this.sg1.col,
													  "select a.no_adjust,a.keterangan from tlk_adjust_swa a "+
													  "inner join tlk_klaim b on a.no_klaim=b.no_klaim "+filter,
													  "select count(a.no_adjust) from tlk_adjust_swa a "+
													  "inner join tlk_klaim b on a.no_klaim=b.no_klaim "+filter,
	...
	doSelectCell:function(sender, col, row)
	{
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from tlk_adjust_swa order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from tlk_adjust_swa order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
		...
		try
		{
			...
			else
			{
				...
				var sql="select a.no_adjust,a.no_klaim,c.nama as nama_ttg,a.no_survey,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
						...
						"from tlk_adjust_swa a "+
						"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
						"inner join tlk_ttg c on b.kode_ttg=c.kode_ttg "+this.filter+user_lok+
	...
	doSgClick:function(sender, col, row){	
		...
		if (col==8){
			try{
				var data = this.dbLib.getDataProvider("select nama as nm from tlk_adjust_dok_swa "+
						"where no_adjust='"+this.sg1mp.getCell(0,row)+"' and kode_lokasi = '"+this.app._lokasi+"'  ",true);
				this.sg1mp2.clear();
		...
		if (col==9){
			try{
				var sql="select a.no_adjust,a.no_klaim,c.nama as nama_ttg,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
