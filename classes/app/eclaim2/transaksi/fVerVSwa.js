window.app_eclaim2_transaksi_fVerVSwa = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_transaksi_fVerVSwa.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim2_transaksi_fVerVSwa";
		...
		this.tahun=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from tlk_ver_swa");
...
window.app_eclaim2_transaksi_fVerVSwa.extend(window.childForm);
window.app_eclaim2_transaksi_fVerVSwa.implement({
	...
	doEllipseClick:function(sender, col, row)
	{
		if (row == 2)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Klaim",this.sg1, this.sg1.row, this.sg1.col,
												...
												"from tlk_klaim a "+
												"inner join tlk_ver_swa b on a.no_klaim=b.no_klaim "+filter,
		...
		if (row == 4)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Verifikasi",this.sg1, this.sg1.row, this.sg1.col,
													  "select a.no_ver,a.keterangan from tlk_ver_swa a "+
													  "inner join tlk_klaim b on a.no_klaim=b.no_klaim "+filter,
													  "select count(a.no_ver) from tlk_ver_swa a "+
													  "inner join tlk_klaim b on a.no_klaim=b.no_klaim "+filter,
	...
	doSelectCell:function(sender, col, row)
	{
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from tlk_ver_swa order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from tlk_ver_swa order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	...
	doSgClick:function(sender, col, row){	
		...
		if (col==5){
			try{
				var data = this.dbLib.getDataProvider("select no_file as nm from tlk_ver_dok_swa "+
