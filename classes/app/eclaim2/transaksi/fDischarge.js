window.app_eclaim2_transaksi_fDischarge = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fDischarge.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fDischarge";		
		...
		this.ed_kode = new saiLabelEdit(this,{bound:[20,2,200,20],caption:"No Discharge",readOnly:true});
		...
		this.cb_bast = new saiCBBL(this,{bound:[20,4,200,20],caption:"No BAST", btnClick:[this,"doFindBtnClick"]});	
...
window.app_eclaim2_transaksi_fDischarge.extend(window.childForm);
window.app_eclaim2_transaksi_fDischarge.implement({
			...
			switch(event){
				...
				case "simpan" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						sql.add("insert into tlk_df (no_df, no_bast, no_klaim, periode, tanggal, no_dokumen,keterangan, kode_lokasi, tgl_input, nik_user)values"+
							"('"+this.ed_kode.getText()+"','"+this.cb_bast.getText()+"','"+this.cb_klaim.getText()+"','"+this.ed_Periode.getText()+"','"+this.dp_tgl.getDateString()+"','"+this.ed_dok.getText()+"',"+
							"'"+this.ed_ket.getText()+"','"+this.app._lokasi+"',now(),'"+this.app._userLog+"') ");
	...
	doClick: function(sender){
		this.ed_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'tlk_df','no_df',"DF/"+this.ed_Periode.getText().substring(2)+"/",'0000'));
	},
