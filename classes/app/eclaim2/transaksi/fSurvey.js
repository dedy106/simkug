window.app_eclaim2_transaksi_fSurvey = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fSurvey.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fSurvey";		
		...
		this.l_tgl = new label(this,{bound:[20,1,100,20],caption:"Tanggal Survey",underline:true});
		this.dp_tgl = new datePicker(this,{bound:[120,1,100,18], selectDate:[this,"doSelectedDate"]});
		this.ed_kode = new saiLabelEdit(this,{bound:[20,2,200,20],caption:"No Survey",readOnly:true});
		...
		this.cb_la = new saiCBBL(this,{bound:[20,7,200,20],caption:"Loss Adjuster", btnClick:[this,"doFindBtnClick"]});		
		this.cStatus = new saiCB(this,{bound:[20,8,200,20],caption:"Status",items:["LIABLE","NOT LIABLE"],tag:2});
...
window.app_eclaim2_transaksi_fSurvey.extend(window.childForm);
window.app_eclaim2_transaksi_fSurvey.implement({
			...
			switch(event){
				...
				case "simpan" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						//sql.add("update tlk_klaim set progress = '3',status='"+status2+"', flag3='1' where no_klaim = '"+this.cb_klaim.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");
	...
	doFindBtnClick: function(sender){
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, alamat from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('1','2')  and status='1' ",
										  "select count(no_klaim) from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('1','2')  and status='1' ",
