window.app_eclaim2_transaksi_fDokumenSwa = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fDokumenSwa.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fDokumenSwa";		
		...
		this.ed_kode = new saiLabelEdit(this,{bound:[20,2,200,20],caption:"No Dokumen",readOnly:true, maxLength:45});
...
window.app_eclaim2_transaksi_fDokumenSwa.extend(window.childForm);
window.app_eclaim2_transaksi_fDokumenSwa.implement({
	...
	doFindBtnClick: function(sender){
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, penyebab from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress='3' and status='0' ",//
										  "select count(no_klaim) from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress='3' and status='0' ",//"+this.app._userLog+"
