window.app_eclaim2_transaksi_fAdjustSwa = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fAdjustSwa.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fAdjustSwa";		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Konfirmasi", 0);	
		...
		this.l_tgl = new label(this,{bound:[20,1,100,20],caption:"Tanggal Konfirmasi",underline:true});
		this.dp_tgl = new datePicker(this,{bound:[120,1,100,18], selectDate:[this,"doSelectedDate"]});
		this.ed_kode = new saiLabelEdit(this,{bound:[20,2,200,20],caption:"No Konfirmasi",readOnly:true});
		...
		this.ed_nilaiEst = new saiLabelEdit(this,{bound:[20,8,200,20],caption:"Nilai Estimasi",tipeText:ttNilai});		
		this.ed_dok = new saiLabelEdit(this,{bound:[20,6,350,20],caption:"No Dokumen", maxLength:45});		
		//this.ed_dp = new saiLabelEdit(this,{bound:[20,7,200,20],caption:"Nilai DP",tipeText:ttNilai});	
		this.ed_nilai = new saiLabelEdit(this,{bound:[20,8,200,20],caption:"Nilai Adjust",tipeText:ttNilai});		
		this.ed_nilaiDdct = new saiLabelEdit(this,{bound:[20,11,200,20],caption:"Nilai Deductable",tipeText:ttNilai});		
		this.ed_ket = new saiMemo(this,{bound:[20,9,500,50],caption:"Keterangan", maxLength:150});		
		...
		this.cb_aktif = new checkBox(this,{bound:[530,this.ed_ket.top,100,20],caption:"Status Aktif", selected:true});		
		...
		//this.cb_klaim.onChange.set(this,"doEditChange");
	}
};
window.app_eclaim2_transaksi_fAdjustSwa.extend(window.childForm);
window.app_eclaim2_transaksi_fAdjustSwa.implement({
			...
			switch(event){
				...
				case "simpan" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						//	sql.add("update tlk_adjust_swa set status = '0' where kode_lokasi = '"+this.app._lokasi+"' and  no_survey = '"+this.cb_survey.getText()+"' and no_klaim ='"+this.cb_klaim.getText()+"' ");
							
						sql.add("insert into tlk_adjust_swa (no_adjust,  no_klaim, periode, tanggal, no_dokumen,nilai,nilai_dp, keterangan, nilai_ddct,kode_lokasi, tgl_input, nik_user,status)values"+
						...
						sql.add("update tlk_klaim set progress = '5', flag5='1',nilai="+parseNilai(this.ed_nilaiEst.getText())+" where no_klaim = '"+this.cb_klaim.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");
						if (files.length > 0){
							var scan = "insert into tlk_adjust_dok_swa (no_adjust,kode_lokasi,no_file,nama,nu) values ";						
							var first = true;
							var noUrut=1;
							for (var i in files){
								if (!first) scan +=",";
	...
	doEditChange: function(sender){
		if (sender == this.cb_klaim){
			var sql="select a.nilai,b.no_survey from tlk_klaim a "+
					"inner join tlk_survey b on a.no_klaim=b.no_klaim "+
					"where a.no_klaim = '"+sender.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' "
			alert(sql);
			var data = this.dbLib.getDataProvider(sql,true);
			if (data.rs.rows[0]){
				this.ed_nilaiEst.setText(floatToNilai(data.rs.rows[0].nilai));
				this.cb_survey.setText(data.rs.rows[1].nilai));
			}
		}
	},*/
	...
	doFindBtnClick: function(sender){
		/*if (sender == this.cb_survey){
			this.standarLib.showListData(this, "Data Survey",sender,undefined, 
										  "select no_survey, keterangan from tlk_survey where kode_lokasi = '"+this.app._lokasi+"' and no_klaim='"+this.cb_klaim.getText()+"'  ",
		...
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, penyebab from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and status='0' and progress='4' ",
										  "select count(no_klaim) from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and status='0' and progress='4' ",
	...
	doChange: function(sender){		
		try{
			if (sender == this.cb_klaim){
				var sql = new server_util_arrayList();
				sql.add("select a.nilai from tlk_klaim a "+
						"where a.no_klaim = '"+sender.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' ");
				...
				if (typeof data != "String" && data.result[0].rs.rows[0] != undefined){
					...
					this.ed_nilaiEst.setText(floatToNilai(line.nilai));
	...
	doClick: function(sender){
		this.ed_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'tlk_adjust_swa','no_adjust',"ADJ/"+this.ed_Periode.getText().substring(2)+"/",'0000'));
	},
