window.app_eclaim2_transaksi_fProgress = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fProgress.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fProgress";		
		...
		this.ed_kode = new saiLabelEdit(this,{bound:[20,2,200,20],caption:"No Progress",readOnly:true});
...
window.app_eclaim2_transaksi_fProgress.extend(window.childForm);
window.app_eclaim2_transaksi_fProgress.implement({
			...
			switch(event){
				...
				case "simpan" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						sql.add("insert into tlk_progresspk (no_progress, no_klaim, periode, tanggal, no_dokumen, kode_lokasi, tgl_input, nik_user,keterangan, no_pelaksanaan, kode_vendor) values"+
							"('"+this.ed_kode.getText()+"','"+this.cb_klaim.getText()+"','"+this.ed_Periode.getText()+"','"+this.dp_tgl.getDateString()+"','-',"+
