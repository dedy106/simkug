...
//* 			SAI, PT											
//***********************************************************************************************
window.app_eclaim2_transaksi_fPenunjukanV = function(owner,options){
	try{
		if (owner)
		{
			window.app_eclaim2_transaksi_fPenunjukanV.prototype.parent.constructor.call(this, owner,options);
			this.className = "app_eclaim2_transaksi_fPenunjukanV";
			this.maximize();
	...
	{
		alert("[app_eclaim2_transaksi_fPenunjukanV]::contruct:"+e,"");
	}
};
window.app_eclaim2_transaksi_fPenunjukanV.extend(window.childForm);
window.app_eclaim2_transaksi_fPenunjukanV.implement({
    ...
    initComponent: function(){
		try{
			...
			this.p1mp = new panel(this,{bound:[10,35,this.width - 25,250],border:3,caption:"Daftar Penunjukan"});
			this.sg1mp = new saiGrid(this.p1mp,{bound:[1,20,this.p1mp.width - 4,this.p1mp.height - 50],colCount:7,colTitle:["No. Penunjukan","No. Klaim","Tertanggung","Tgl Verifikasi","No. Dokumen","Keterangan","Preview History"],
			...
			this.p1mp3 = new panel(this,{bound:[440,300,500,180],border:3,caption:"Vendor"});
			this.sg1mp3 = new saiGrid(this.p1mp3,{bound:[1,20,this.p1mp3.width - 4,128],colCount:4,readOnly:true,colTitle:["Kode Vendor","Nama Vendor","no_spk","tgl_spk"],
			...
			this.pager = 50;
			var sql = "select count(a.no_penunjukan)	   "+                         
			...
			this.sql =  "select a.no_penunjukan,a.no_klaim,c.nama as nama_ttg,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
			...
			this.sgFilter.editData(3,["No Penunjukan","All","",""]);			
    ...
    doClick: function(sender){
        try{
            if (sender == this.bApply){
			    ...
			    this.filterRep.filterStr("a.no_penunjukan",this.sgFilter.getCell(1,3),this.sgFilter.getCell(2,3),this.sgFilter.getCell(3,3),"and");
			        ...
			        if (this.senderPager == this.sgn){
						...
						if (brg.rs.rows[0]!=undefined)
						{
							...
							for (var i in brg.rs.rows){
								line = brg.rs.rows[i];
								this.gridDataTmp.set(line.no_penunjukan,line);
								this.sg1mp.appendData([line.no_penunjukan,line.no_klaim, line.nama_ttg, line.tanggal, line.no_dokumen, line.keterangan,"Preview"]);
						...
						if (brg.rs.rows[0]!=undefined){
							...
							for (var i in brg.rs.rows){
								line = brg.rs.rows[i];								
								this.sgHist.appendData([(line.status == "1" ? "<img width=18 height=18 src='image/ok.png' />." :"<img width=18 height=18 src='image/error.png' />." ),line.no_penunjukan,line.no_penunjukan, line.tanggal, line.no_dokumen, line.keterangan, floatToNilai(line.nilai), floatToNilai(line.nilai_adjust)]);								
							}					
	...
	doSgClick:function(sender, col, row){		
		if (sender == this.sg1mp){
			try{
				var data = this.dbLib.getDataProvider("select nama as nm from tlk_penunjukan_dok "+
						"where no_penunjukan='"+this.sg1mp.getCell(0,row)+"' and kode_lokasi = '"+this.app._lokasi+"'  ",true);
				this.sg1mp2.clear();
				...
				this.sgn2.rearrange();						
				data = this.dbLib.getDataProvider("select a.kode_vendor, b.nama, a.no_spk, date_format(a.tgl_spk,'%d-%m-%Y') as tgl from tlk_penunjukan_d a inner join tlk_vendor b on b.kode_vendor = a.kode_vendor and b.kode_lokasi = a.kode_lokasi "+
						"where no_penunjukan='"+this.sg1mp.getCell(0,row)+"' and a.kode_lokasi = '"+this.app._lokasi+"'  ",true);
				this.sg1mp3.clear();
				if (typeof(data) != "string")
				{  
					if (data.rs.rows[0] != undefined)
					{
						this.sg1mp3.showLoading();						
						for (var k in data.rs.rows){
							var line=data.rs.rows[k];
							this.sg1mp3.appendData([line.kode_vendor, line.nama, line.no_spk, line.tgl]);							
						}
						this.sg1mp3.hideLoading();
					}
				}				
			...
			if (col == 10){
				var sql =  "select a.status, a.no_penunjukan,a.no_klaim,c.nama as nama_ttg,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
					"a.no_dokumen,a.keterangan   "+                         
					"from tlk_penunjukan a "+
					"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
					"inner join tlk_ttg c on b.kode_ttg=c.kode_ttg "+					
					"where a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_ttg='"+this.app._kodeTtg+"' and a.no_penunjukan = '"+this.sg1mp.cells(3,row)+"' ";
					
				var sqlCount =  "select count(a.no_penunjukan) "+                         
					"from tlk_penunjukan a "+
					"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
					"inner join tlk_ttg c on b.kode_ttg=c.kode_ttg "+					
					"where a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_ttg='"+this.app._kodeTtg+"' and a.no_penunjukan = '"+this.sg1mp.cells(3,row)+"'  ";
	...
	doEllipsFilterClick: function(sender, col, row){
		...
		if (row == 3)
		{
			this.filterRep.ListDataSGFilter(this, "Data Penunjukan",sender, sender.row, sender.col,
													  "select a.no_penunjukan,a.no_dokumen,date_format(a.tanggal,'%d/%m/%Y') as tanggal from tlk_penunjukan a "+
													  ...
													  "select count(no_penunjukan) from tlk_penunjukan a "+
													  "	inner join tlk_klaim b on b.no_klaim = a.no_klaim and b.kode_lokasi = a.kode_lokasi "+
													  " where a.kode_lokasi='"+this.app._lokasi+"' "+
													  this.filterRep.filterStr("a.periode",this.sgFilter.getCell(1,0),this.sgFilter.getCell(2,0),this.sgFilter.getCell(3,0),"and") +			    
													  this.filterRep.filterStr("b.kode_ttg",this.sgFilter.getCell(1,1),this.sgFilter.getCell(2,1),this.sgFilter.getCell(3,1),"and")+
													  this.filterRep.filterStr("a.no_klaim",this.sgFilter.getCell(1,2),this.sgFilter.getCell(2,2),this.sgFilter.getCell(3,2),"and"),
													  ["no_penunjukan","no_dokumen","tanggal"],"and",["No Penunjukan","no dokumen","tanggal"]);
		}		
	},
