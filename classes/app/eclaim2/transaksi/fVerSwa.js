window.app_eclaim2_transaksi_fVerSwa = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fVerSwa.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fVerSwa";		
...
window.app_eclaim2_transaksi_fVerSwa.extend(window.childForm);
window.app_eclaim2_transaksi_fVerSwa.implement({
			...
			switch(event){
				...
				case "simpan" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						sql.add("insert into tlk_ver_swa (no_ver, no_klaim, periode, tanggal, no_dokumen, kode_lokasi, tgl_input, nik_user, status,keterangan,host,ip,nik_ver,tgl_png) values"+
						...
						if (files.length > 0){
							var scan = "insert into tlk_ver_swa_dok (no_ver,kode_lokasi,no_file,nama,nu) values ";						
							var first = true;
							var noUrut=1;
							for (var i in files){
								if (!first) scan +=",";
	...
	doClick: function(sender){
		this.ed_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'tlk_ver_swa','no_ver',"VER/BDG/"+this.ed_Periode.getText().substring(2)+"/",'000'));
	},
