window.app_eclaim2_transaksi_fPelaksanaan = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fPelaksanaan.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fPelaksanaan";		
		...
		this.ed_kode = new saiLabelEdit(this,{bound:[20,2,200,20],caption:"No Pelaksanaan",readOnly:true});
		...
		this.cb_penunjukan = new saiCBBL(this,{bound:[20,4,200,20],caption:"No Penunjukan", btnClick:[this,"doFindBtnClick"]});
		this.cb_vendor = new saiCBBL(this,{bound:[20,5,200,20],caption:"Vendor", btnClick:[this,"doFindBtnClick"]});
...
window.app_eclaim2_transaksi_fPelaksanaan.extend(window.childForm);
window.app_eclaim2_transaksi_fPelaksanaan.implement({
			...
			switch(event){
				...
				case "simpan" :
					if (this.colData == undefined){
						system.alert(this,"Tolong Generate Jadwal dulu","");
						return;
					}
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						sql.add("insert into tlk_pelaksanaan (no_pelaksanaan, no_klaim, periode, tanggal, no_dokumen, kode_lokasi, tgl_input, nik_user, keterangan, no_penunjukan, kode_vendor, bobot, tgl_mulai, tgl_selesai) values"+
							"('"+this.ed_kode.getText()+"','"+this.cb_klaim.getText()+"','"+this.ed_Periode.getText()+"','"+this.dp_tgl.getDateString()+"','"+this.ed_dok.getText()+"',"+
							" '"+this.app._lokasi+"',now(),'"+this.app._userLog+"','"+this.ed_ket.getText()+"','"+this.cb_penunjukan.getText()+"','"+this.cb_vendor.getText()+"','"+parseNilai(this.ed_bobot.getText())+"','"+this.dp_tgl2.getDateString()+"','"+this.dp_tgl3.getDateString()+"') ");
	...
	doFindBtnClick: function(sender){
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, penyebab from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('8')and flag9='0' and status='1' ",
										  "select count(no_klaim) from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('8') and flag9='0' and status='1' ",
