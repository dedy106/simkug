window.app_eclaim2_transaksi_fBantek = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fBantek.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fBantek";		
		...
		this.ed_kode = new saiLabelEdit(this,{bound:[20,2,200,20],caption:"No BANTEK",readOnly:true});
...
window.app_eclaim2_transaksi_fBantek.extend(window.childForm);
window.app_eclaim2_transaksi_fBantek.implement({
			...
			switch(event){
				...
				case "simpan" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						sql.add("insert into tlk_bantek (no_bantek, no_klaim, kode_vendor, periode, tanggal, no_dokumen, kode_lokasi, tgl_input, nik_user, status,keterangan) values"+
	...
	doFindBtnClick: function(sender){
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, penyebab from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('1') and status='1'",
