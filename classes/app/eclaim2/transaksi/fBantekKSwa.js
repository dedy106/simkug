window.app_eclaim2_transaksi_fBantekKSwa = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fBantekKSwa.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fBantekKSwa";		
		...
		this.ed_kode = new saiCBBL(this,{bound:[20,2,220,20],caption:"No Pelaksanaan",
...
window.app_eclaim2_transaksi_fBantekKSwa.extend(window.childForm);
window.app_eclaim2_transaksi_fBantekKSwa.implement({
			...
			switch(event){
				...
				case "ubah" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){						
						...
						sql.add("delete from tlk_bantek_dok_swa where no_bantek= '"+this.ed_kode.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");						
						sql.add("update tlk_bantek_swa set kode_vendor='"+this.cb_vendor.getText()+"' , periode ='"+this.ed_Periode.getText()+"', tanggal='"+this.dp_tgl.getDateString()+"' "+
						...
						if (files.length > 0){
							var scan = "insert into tlk_bantek_dok_swa (no_bantek,kode_lokasi,no_file,nama,nu) values ";						
							var first = true;
							var noUrut=1;
							for (var i in files){
								if (!first) scan +=",";
				...
				case "hapus":		
					var sql = new server_util_arrayList();
					sql.add("delete from tlk_bantek_dok_swa where no_bantek= '"+this.ed_kode.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");						
					sql.add("delete from tlk_bantek_swa where no_bantek= '"+this.ed_kode.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");						
	...
	doFindBtnClick: function(sender){
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, penyebab from tlk_klaim where status='0' and kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('1') and status='0'",
										  "select count(no_klaim) from tlk_klaim where status='0' and kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('1') and status='0' ",
	...
	doClick: function(sender){
		this.ed_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'tlk_bantek_swa','no_bantek',"BNT/"+this.ed_Periode.getText().substring(2)+"/",'0000'));
	},
	doChange: function(sender){
		if (sender == this.ed_kode){
			var data = this.dbLib.getMultiDataProvider(new server_util_arrayList({items:[
				"select a.no_klaim,a.keterangan, date_format(a.tanggal,'%d-%m-%Y') as tgl, a.status, a.kode_vendor, a.no_dokumen from tlk_bantek_swa a "+
				" where a.no_bantek = '"+sender.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'" ,
				"select no_file, nu from tlk_bantek_dok_swa a  "+
