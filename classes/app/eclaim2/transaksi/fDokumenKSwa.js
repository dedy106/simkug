window.app_eclaim2_transaksi_fDokumenKSwa = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fDokumenKSwa.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fDokumenKSwa";		
...
window.app_eclaim2_transaksi_fDokumenKSwa.extend(window.portalui_childForm);
window.app_eclaim2_transaksi_fDokumenKSwa.implement({
			...
			switch(event){
				...
				case "ubah" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						sql.add("delete from tlk_dok_m_swa where kode_lokasi = '"+this.app._lokasi+"' and no_dok = '"+this.ed_kode.getText()+"' ");
						sql.add("delete from tlk_dok_d_swa where kode_lokasi = '"+this.app._lokasi+"' and no_dok = '"+this.ed_kode.getText()+"' ");
						sql.add("insert into tlk_dok_m_swa (no_dok,no_klaim,kode_lokasi,tanggal,keterangan,periode,nik_user,tgl_input)values "+
						...
						if (files.length > 0){
							var scan = "insert into tlk_dok_d_swa (no_dok,kode_lokasi,kode_ref,no_file,nu,nik_user,tgl_input,ket_dok) values ";						
							var first = true;
							var noUrut=1;
							for (var i in files){
								if (!first) scan +=",";
	...
	doFindBtnClick: function(sender){
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, penyebab from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and status='0' and progress in ('4') and status='0' ",
										  "select count(no_klaim) from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and status='0' and progress in ('4') and status='0' ",
										  ["no_klaim","penyebab"],"and",["No Klaim","Keterangan"],false); 
		}
		if (sender == this.ed_kode){
			this.standarLib.showListData(this, "Data Dokumen ",sender,undefined, 
										  "select a.no_dok, a.keterangan from tlk_dok_m_swa a inner join tlk_klaim b on b.no_klaim = a.no_klaim and b.kode_lokasi = a.kode_lokasi "+
										  " where a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_ttg='"+this.app._kodeTtg+"' and b.progress='4'  ",
										  "select count(a.no_dok) from tlk_dok_m_swa a inner join tlk_klaim b on b.no_klaim = a.no_klaim and b.kode_lokasi = a.kode_lokasi "+
										  " where a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_ttg='"+this.app._kodeTtg+"' and b.progress='4'  ",
	...
	doChange: function(sender){	
		try{
			if (sender == this.ed_kode){
				if (sender.getText() != ""){	
					var sql = new server_util_arrayList();
					sql.add("select a.*, b.penyebab from tlk_dok_m_swa a "+
					...
					sql.add("select a.nu, a.kode_ref, b.nama as nm_ref, a.no_file as nama,a.ket_dok from tlk_dok_d_swa a "+
	...
	doClick: function(sender){
		this.ed_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'tlk_dok_m_swa','no_dok',"DOK/"+this.ed_periode.getText().substring(2)+"/",'0000'));
	},
