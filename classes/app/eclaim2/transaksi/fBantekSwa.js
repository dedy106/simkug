window.app_eclaim2_transaksi_fBantekSwa = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fBantekSwa.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fBantekSwa";		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Bantuan Teknis : Input", 0);	
...
window.app_eclaim2_transaksi_fBantekSwa.extend(window.childForm);
window.app_eclaim2_transaksi_fBantekSwa.implement({
			...
			switch(event){
				...
				case "simpan" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						sql.add("insert into tlk_bantek_swa (no_bantek, no_klaim, kode_vendor, periode, tanggal, no_dokumen, kode_lokasi, tgl_input, nik_user, status,keterangan) values"+
							"('"+this.ed_kode.getText()+"','"+this.cb_klaim.getText()+"','"+this.cb_vendor.getText()+"','"+this.ed_Periode.getText()+"','"+this.dp_tgl.getDateString()+"','"+this.ed_dok.getText()+"',"+
	...
	doFindBtnClick: function(sender){
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, penyebab from tlk_klaim where status='0' and kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('1') and flag2='0' and status='0'",
										  "select count(no_klaim) from tlk_klaim where status='0' and kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('1') and flag2='0' and status='0' ",
