window.app_eclaim2_transaksi_fAdjustKSwa = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fAdjustKSwa.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fAdjustKSwa";		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Konfirmasi : Koreksi", 0);	
		...
		this.l_tgl = new portalui_label(this,{bound:[20,1,100,20],caption:"Tanggal Konfirmasi",underline:true});
		this.dp_tgl = new portalui_datePicker(this,{bound:[120,1,100,18], selectDate:[this,"doSelectedDate"]});
		this.ed_kode = new portalui_saiCBBL(this,{bound:[20,2,220,20],caption:"No Konfirmasi",readOnly:true,change:[this,"doChange"], btnClick:[this,"doFindBtnClick"], rightLabelVisible:false});		
...
window.app_eclaim2_transaksi_fAdjustKSwa.extend(window.portalui_childForm);
window.app_eclaim2_transaksi_fAdjustKSwa.implement({
			...
			switch(event){
				...
				case "ubah" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						sql.add("delete from tlk_adjust_swa where kode_lokasi = '"+this.app._lokasi+"' and no_adjust = '"+this.ed_kode.getText()+"' ");
						sql.add("delete from tlk_adjust_dok_swa where kode_lokasi = '"+this.app._lokasi+"' and no_adjust = '"+this.ed_kode.getText()+"' ");
						if (this.cb_aktif.isSelected())
							sql.add("update tlk_adjust_swa set status = '0' where kode_lokasi = '"+this.app._lokasi+"' and no_klaim ='"+this.cb_klaim.getText()+"' ");
				...
				case "hapus" :
					sql.add("delete from tlk_adjust_swa where kode_lokasi = '"+this.app._lokasi+"' and no_adjust = '"+this.ed_kode.getText()+"' ");
					sql.add("delete from tlk_adjust_dok_swa where kode_lokasi = '"+this.app._lokasi+"' and no_adjust = '"+this.ed_kode.getText()+"' ");
					sql.add("update tlk_klaim a left outer join tlk_adjust_swa b on b.no_klaim = a.no_klaim and b.kode_lokasi = a.kode_lokasi "+
						" set a.progress = '4',flag4='0' "+						
	...
	doFindBtnClick: function(sender){
		...
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, penyebab from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and status='0' and progress='5' ",
										  "select count(no_klaim) from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and status='0' and progress='5'  ",
		...
		if (sender == this.ed_kode){
			this.standarLib.showListData(this, "Data Adjustment",sender,undefined, 
										  ...
										  " where a.kode_lokasi = '"+this.app._lokasi+"'  and b.progress='5'  ",
		...
		try{			
			if (sender == this.ed_kode){
				if (sender.getText() != ""){	
					this.sgUpld.clear();
					var sql = new server_util_arrayList();
					sql.add("select a.*, b.penyebab,b.nilai as nilai_est from tlk_adjust_swa a "+
						" inner join tlk_klaim b on b.no_klaim = a.no_klaim and b.kode_lokasi = a.kode_lokasi "+
							" where a.kode_lokasi = '"+this.app._lokasi+"' and a.no_adjust = '"+sender.getText()+"' ");
					sql.add("select nama, nu from tlk_adjust_dok_swa where kode_lokasi =  '"+this.app._lokasi+"' and no_adjust = '"+sender.getText()+"' order by nu");
