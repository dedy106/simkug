window.app_eclaim_transaksi_fBayarK = function(owner) {
	if (owner){
		...
		this.ed_kode = new portalui_saiCBBL(this,{bound:[20,2,200,20],caption:"No Bayar",readOnly:true, rightLabelVisible:false, change:[this,"doChange"], btnClick:[this,"doFindBtnClick"]});		
		this.cb_klaim = new portalui_saiCBBL(this,{bound:[20,3,200,20],caption:"No Klaim", btnClick:[this,"doFindBtnClick"]});
		this.cb_adjust = new portalui_saiCBBL(this,{bound:[20,4,200,20],caption:"No Adjust", btnClick:[this,"doFindBtnClick"]});		
			...
			switch(event){
				...
				case "ubah" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						sql.add("delete from eclaim_bayar where kode_lokasi = '"+this.app._lokasi+"' and no_bayar= '"+this.ed_kode.getText()+"'");
						sql.add("delete from eclaim_bayar_dok where kode_lokasi = '"+this.app._lokasi+"' and no_bayar= '"+this.ed_kode.getText()+"'");
						var param = this.sgUpld.columns.get(1).param2.split("tmp/");
						sql.add("insert into eclaim_bayar (no_bayar, no_adjust, no_klaim, periode, tanggal, no_dokumen,nilai,keterangan, kode_lokasi, tgl_input, nik_user)values"+
						...
						sql.add("update eclaim_klaim set progress = '12',flag13='0', status='1' where no_klaim = '"+this.cb_klaim.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");
						if (files.length > 0){
							var scan = "insert into eclaim_bayar_dok (no_bayar,kode_lokasi,no_file,nama,nu) values ";						
							var first = true;
							var noUrut=1;
							for (var i in files){
								if (!first) scan +=",";
				...
				case "hapus" :
					sql.add("delete from eclaim_bayar where kode_lokasi = '"+this.app._lokasi+"' and no_bayar= '"+this.ed_kode.getText()+"'");
					sql.add("delete from eclaim_bayar_dok where kode_lokasi = '"+this.app._lokasi+"' and no_bayar= '"+this.ed_kode.getText()+"'");
					sql.add("update eclaim_klaim set progress = '12',flag13='0', status='0' where no_klaim = '"+this.cb_klaim.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");
					this.dbLib.execArraySQL(sql);
				break;
			}			
		}catch(e){
