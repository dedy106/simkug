window.app_eclaim2_transaksi_fVerK = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fVerK.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fVerK";		
		...
		this.ed_kode.setSQL("select a.no_ver, a.keterangan, a.no_klaim "+
							...
							"where a.kode_lokasi = '"+this.app._lokasi+"' and kode_ttg = '"+this.app._kodeTtg+"' and b.progress='1' and b.status='1' "
							,["no_ver","keterangan","no_klaim"],false,["No Ver","Keterangan","No Klaim"],"and","Data Verifikasi Klaim",true);

	}
};
window.app_eclaim2_transaksi_fVerK.extend(window.childForm);
window.app_eclaim2_transaksi_fVerK.implement({
			...
			switch(event){
				...
				case "hapus" :
					if (this.progress == 1){
						...
						sql.add("delete from tlk_ver where no_ver = '"+this.ed_kode.getText()+"' and kode_lokasi = '"+this.app._lokasi +"' ");
						sql.add("delete from tlk_ver_dok where no_ver = '"+this.ed_kode.getText()+"' and kode_lokasi = '"+this.app._lokasi +"' ");
				...
				case "ubah" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){						
						...
						sql.add("update tlk_ver set no_klaim ='"+this.cb_klaim.getText()+"', tanggal='"+this.dp_tgl.getDateString()+"', no_dokumen = '"+this.ed_dok.getText()+
						...
						sql.add("delete from tlk_ver_dok where no_ver = '"+this.ed_kode.getText()+"' and kode_lokasi = '"+this.app._lokasi +"' ");
						if (files.length > 0){
							var scan = "insert into tlk_ver_dok (no_ver,kode_lokasi,no_file,nama,nu) values ";						
							var first = true;
							var noUrut=1;
							for (var i in files){
								if (!first) scan +=",";
	...
	doFindBtnClick: function(sender){
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, lokasi from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('1') and status='1' ",
										  "select count(no_klaim) from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('1') and status='1' ",
										  ["no_klaim","lokasi"],"and",["No Klaim","Lokasi"],false); 
		}		
	...
	doClick: function(sender){
		this.ed_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'tlk_ver','no_ver',"VER/"+this.ed_Periode.getText().substring(2)+"/",'0000'));
	},
		...
		try{
			if (sender == this.ed_kode){
				var data = this.dbLib.getDataProvider("select date_format(a.tanggal,'%d/%m/%Y')as tgl,date_format(a.tgl_png,'%d/%m/%Y')as tgl_png, a.no_dokumen, a.keterangan, a.no_klaim, b.penyebab,b.progress,  ifnull(c.no_file,'-') as no_file, nu,a.nik_ver,d.nama as nama_ver "+
					"from tlk_ver a "+
					"inner join tlk_klaim b on b.no_klaim = a.no_klaim and b.kode_lokasi = a.kode_lokasi  "+
					" left outer join tlk_ver_dok c on c.no_ver = a.no_ver and c.kode_lokasi = a.kode_lokasi "+
	...
	doMail: function(no_ver){
		try{
			this.nama_report="server_report_eclaim2_rptVerifikasiMail";
