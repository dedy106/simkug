window.app_eclaim2_transaksi_fVer = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fVer.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fVer";		
...
window.app_eclaim2_transaksi_fVer.extend(window.childForm);
window.app_eclaim2_transaksi_fVer.implement({
			...
			switch(event){
				...
				case "simpan" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						sql.add("insert into tlk_ver (no_ver, no_klaim, periode, tanggal, no_dokumen, kode_lokasi, tgl_input, nik_user, status,keterangan,host,ip,nik_ver,tgl_png) values"+
							"('"+this.ed_kode.getText()+"','"+this.cb_klaim.getText()+"','"+this.ed_Periode.getText()+"','"+this.dp_tgl.getDateString()+"','"+this.ed_dok.getText()+"',"+
							" '"+this.app._lokasi+"',now(),'"+this.app._userLog+"','0','"+this.ed_ket.getText()+"','"+this.app._hostname+"','"+this.app._iphost+"','"+this.cb_buat.getText()+"','"+this.dp_tglpng.getDateString()+"') ");
						sql.add("update tlk_klaim set progress = '1', flag1='1' where no_klaim = '"+this.cb_klaim.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");
						if (files.length > 0){
	...
	doFindBtnClick: function(sender){
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, alamat from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('0') and status='1' ",
										  "select count(no_klaim) from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('0') and status='1' ",
	...
	doClick: function(sender){
		this.ed_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'tlk_ver','no_ver',"VER/BDG/"+this.ed_Periode.getText().substring(2)+"/",'000'));
	},
