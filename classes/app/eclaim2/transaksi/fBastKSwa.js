window.app_eclaim2_transaksi_fBastKSwa = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fBastKSwa.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fBastKSwa";		
		...
		this.ed_kode = new saiCBBL(this,{bound:[20,3,220,20],caption:"No BAST",readOnly:true, multiSelection:false,change:[this, "doChange"]});		
		...
		//data awal
		this.ed_kode.setSQL("select a.no_bast, a.keterangan, a.no_klaim from tlk_bast_swa a inner join tlk_klaim b on b.no_klaim = a.no_klaim and b.kode_lokasi = a.kode_lokasi  where a.kode_lokasi = '"+this.app._lokasi+"' and kode_ttg = '"+this.app._kodeTtg+"' "
							,["no_bast","keterangan","no_klaim"],false,["No Ver","Keterangan","No Klaim"],"and","Data Verifikasi Klaim",true);

	}
};
window.app_eclaim2_transaksi_fBastKSwa.extend(window.childForm);
window.app_eclaim2_transaksi_fBastKSwa.implement({
			...
			switch(event){
				...
				case "hapus" :
					if (this.progress == 1){
						...
						sql.add("delete from tlk_bast_swa where no_bast = '"+this.ed_kode.getText()+"' and kode_lokasi = '"+this.app._lokasi +"' ");
						sql.add("delete from tlk_bast_dok_swa where no_bast = '"+this.ed_kode.getText()+"' and kode_lokasi = '"+this.app._lokasi +"' ");
						sql.add("update tlk_klaim set progress = '2', flag3='0' where no_klaim = '"+this.cb_klaim.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");
				...
				case "ubah" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){						
						...
						sql.add("update tlk_bast_swa set no_klaim ='"+this.cb_klaim.getText()+"', tanggal='"+this.dp_tgl.getDateString()+"', no_dokumen = '"+this.ed_dok.getText()+
								"',keterangan='"+this.ed_ket.getText()+"',tgl_input=now() "+
								" where no_bast = '"+this.ed_kode.getText()+"' and kode_lokasi = '"+ this.app._lokasi +"'");
						sql.add("delete from tlk_bast_dok_swa where no_bast = '"+this.ed_kode.getText()+"' and kode_lokasi = '"+this.app._lokasi +"' ");
						if (files.length > 0){
							var scan = "insert into tlk_bast_dok_swa (no_bast,kode_lokasi,no_file,nama,nu) values ";						
							var first = true;
							var noUrut=1;
							for (var i in files){
								if (!first) scan +=",";
	...
	doClick: function(sender){
		this.ed_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'tlk_bast_swa','no_bast',"VER/"+this.ed_Periode.getText().substring(2)+"/",'0000'));
	},
    ...
    doChange: function(sender){
		try{
			if (sender == this.ed_kode){
				var data = this.dbLib.getDataProvider("select date_format(a.tanggal,'%d/%m/%Y')as tgl, a.no_dokumen, a.keterangan, a.no_klaim, b.penyebab,b.progress,  ifnull(c.no_file,'-') as no_file, nu "+
					"from tlk_bast_swa a "+
					"inner join tlk_klaim b on b.no_klaim = a.no_klaim and b.kode_lokasi = a.kode_lokasi  "+
					" left outer join tlk_bast_dok_swa c on c.no_bast = a.no_bast and c.kode_lokasi = a.kode_lokasi "+
					" where a.no_bast = '"+sender.getText()+"' and a.kode_lokasi = '"+this.app._lokasi+"' "+
					" order by nu",true);
	...
	doMail: function(no_bast){
		try{
			this.nama_report="server_report_eclaim2_rptVerifikasi";
			this.filter = " where h.no_bast='"+no_bast+"' ";
			this.filter2 = "";
			this.viewer.prepare();
