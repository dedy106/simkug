window.app_eclaim2_transaksi_fBayar = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fBayar.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fBayar";		
		...
		this.ed_kode = new saiLabelEdit(this,{bound:[20,2,200,20],caption:"No Bayar",readOnly:true});
		...
		this.cb_adjust = new saiCBBL(this,{bound:[20,4,200,20],caption:"No Adjust", btnClick:[this,"doFindBtnClick"]});		
...
window.app_eclaim2_transaksi_fBayar.extend(window.childForm);
window.app_eclaim2_transaksi_fBayar.implement({
			...
			switch(event){
				...
				case "simpan" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						sql.add("insert into tlk_bayar (no_bayar, no_adjust, no_klaim, periode, tanggal, no_dokumen,nilai,keterangan, kode_lokasi, tgl_input, nik_user)values"+
							...
							parseNilai(this.ed_nilai.getText())+",'"+this.ed_ket.getText()+"','"+this.app._lokasi+"',now(),'"+this.app._userLog+"') ");
						sql.add("update tlk_klaim set progress = '13',flag13='1', status='1' where no_klaim = '"+this.cb_klaim.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ");
						if (files.length > 0){
							var scan = "insert into tlk_bayar_dok (no_bayar,kode_lokasi,no_file,nama,nu) values ";						
							var first = true;
							var noUrut=1;
							for (var i in files){
								if (!first) scan +=",";
	...
	doFindBtnClick: function(sender){
		if (sender == this.cb_adjust){
			this.standarLib.showListData(this, "Data Adjustment",sender,undefined, 
										  "select no_adjust, keterangan from tlk_adjust where kode_lokasi = '"+this.app._lokasi+"' and no_klaim='"+this.cb_klaim.getText()+"'  and nik_user = '"+this.app._userLog+"'  ",
										  "select count(no_adjust) from tlk_adjust where kode_lokasi = '"+this.app._lokasi+"' and no_klaim='"+this.cb_klaim.getText()+"' and nik_user = '"+this.app._userLog+"'  ",
		...
		if (sender == this.cb_klaim){			
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, penyebab from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress='12'  ",
										  "select count(no_klaim) from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress='12' ",
	...
	doChange: function(sender){
		...
		this.ed_skode.clear();
		if (sender.getText() != ""){	
			var data = this.dbLib.getDataProvider("select * from tlk_bayar where kode_lokasi = '"+this.app._lokasi+"' and no_survey = '"+sender.getText()+"' ",true);
			if (typeof data != "string" && data.rs.rows[0] !== undefined){
				...
				this.ed_file.setText(data.file_dok);
	...
	doClick: function(sender){
		this.ed_kode.setText(this.standarLib.noBuktiOtomatis(this.dbLib,'tlk_bayar','no_bayar',"BYR/"+this.ed_Periode.getText().substring(2)+"/",'0000'));
	},
