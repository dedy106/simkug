window.app_eclaim2_transaksi_fTestingV = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_transaksi_fTestingV.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim2_transaksi_fTestingV";
		...
		this.pc1 = new pageControl(this,{bound:[15,12,1000,500], childPage:["Filter","Data Testing","Data Klaim","Data Riwayat Klaim"]});
		...
		this.tahun=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from tlk_testing");
		...
		this.gridLib.SGEditData(this.sg1,4,new Array(0,1), new Array("No Testing","All"));
		
		this.sg1mp = new saiGrid(this.pc1.childPage[1],{bound:[1,20,this.pc1.width - 4,this.pc1.height - 180],colCount:6,
					colTitle:["No. Testing","No. Klaim","Tgl Testing","No. Dokumen","Keterangan","View Dokumen"],
...
window.app_eclaim2_transaksi_fTestingV.extend(window.childForm);
window.app_eclaim2_transaksi_fTestingV.implement({
	...
	doEllipseClick:function(sender, col, row)
	{
		if (row == 2)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Klaim",this.sg1, this.sg1.row, this.sg1.col,
												...
												"from tlk_klaim a "+
												"inner join tlk_testing b on a.no_klaim=b.no_klaim "+filter,
		...
		if (row == 4)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Verifikasi",this.sg1, this.sg1.row, this.sg1.col,
													  "select a.no_testing,a.keterangan from tlk_testing a "+
													  "inner join tlk_klaim b on a.no_klaim=b.no_klaim "+filter,
													  "select count(a.no_testing) from tlk_testing a "+
													  "inner join tlk_klaim b on a.no_klaim=b.no_klaim "+filter,
													  new Array("a.no_testing","a.keterangan"),"and",new Array("kode","nama"));
		}
	...
	doSelectCell:function(sender, col, row)
	{
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from tlk_testing order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from tlk_testing order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
		...
		try
		{
			...
			else
			{
				this.filter = this.filterRep.filterStr("a.kode_lokasi","=",this.app._lokasi,this.app._lokasi,"where")+
					  ...
					  this.filterRep.filterStr("a.periode",this.sg1.getCell(1,1),this.sg1.getCell(2,1),this.sg1.getCell(3,1),"and") +
					  this.filterRep.filterStr("a.no_klaim",this.sg1.getCell(1,2),this.sg1.getCell(2,2),this.sg1.getCell(3,2),"and")+
					  this.filterRep.filterStr("a.no_testing",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				...
				var sql="select a.no_testing,a.no_klaim,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
	...
	doTampilData2: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1mp.appendData([line.no_testing,line.no_klaim,line.tanggal, line.no_dokumen, line.keterangan,"Preview"]);
	...
	doSgClick:function(sender, col, row){	
		...
		if (col==5){
			try{
				var data = this.dbLib.getDataProvider("select nama as nm from tlk_testing_dok "+
						"where no_testing='"+this.sg1mp.getCell(0,row)+"' and kode_lokasi = '"+this.app._lokasi+"'  ",true);
				this.sg1mp2.clear();
