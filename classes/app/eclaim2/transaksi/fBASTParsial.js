window.app_eclaim2_transaksi_fBASTParsial = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fBASTParsial.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fBASTParsial";		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Berita Acara Serah Terima Parsial: Input", 0);	
		...
		this.ed_kode = new saiLabelEdit(this,{bound:[20,2,200,20],caption:"No BAST",readOnly:true});
...
window.app_eclaim2_transaksi_fBASTParsial.extend(window.childForm);
window.app_eclaim2_transaksi_fBASTParsial.implement({
			...
			switch(event){
				...
				case "simpan" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						sql.add("insert into tlk_bast (no_bast, no_klaim, periode, tanggal, no_dokumen, kode_lokasi, tgl_input, nik_user, status,keterangan, jenis) values"+
							"('"+this.ed_kode.getText()+"','"+this.cb_klaim.getText()+"','"+this.ed_Periode.getText()+"','"+this.dp_tgl.getDateString()+"','"+this.ed_dok.getText()+"',"+
							" '"+this.app._lokasi+"',now(),'"+this.app._userLog+"','0','"+this.ed_ket.getText()+"','PAR') ");
	...
	doFindBtnClick: function(sender){
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, penyebab from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('10')and flag11='0' and status='1' ",
										  "select count(no_klaim) from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('10') and flag11='0' and status='1' ",
