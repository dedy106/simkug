window.app_eclaim2_transaksi_fLAV = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_transaksi_fLAV.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim2_transaksi_fLAV";
...
window.app_eclaim2_transaksi_fLAV.extend(window.childForm);
window.app_eclaim2_transaksi_fLAV.implement({
		...
		try
		{
			...
			else
			{
				...
				var sql="select a.no_klaim, a.no_dokumen, date_format(a.tanggal,'%d-%m-%Y') as tanggal, b.nama as nama_lok, c.nama as nama_obyek, d.nama as nama_sebab, a.nilai, e.nilai as nilai_adjust, e.nilai_ddct as nilai_bayar,a.alamat "+
					...
					" left join tlk_adjust e on e.kode_lokasi = a.kode_lokasi and e.no_klaim = a.no_klaim and e.status = '1' "+this.filter+user_lok+order;
