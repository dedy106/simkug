window.app_eclaim2_transaksi_fBAST = function(owner) {
	if (owner){
		window.app_eclaim2_transaksi_fBAST.prototype.parent.constructor.call(this,owner);
		this.maximize();
		this.className  = "app_eclaim2_transaksi_fBAST";		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Berita Acara Serah Terima: Input", 0);	
...
window.app_eclaim2_transaksi_fBAST.extend(window.childForm);
window.app_eclaim2_transaksi_fBAST.implement({
				...
				case "simpan" :
					if (this.standarLib.checkEmptyByTag(this,[0,1,2])){
						...
						sql.add("insert into tlk_bast (no_bast, no_klaim, periode, tanggal, no_dokumen, kode_lokasi, tgl_input, nik_user, status,keterangan, jenis) values"+
							...
							" '"+this.app._lokasi+"',now(),'"+this.app._userLog+"','0','"+this.ed_ket.getText()+"', 'FUL') ");
	...
	doFindBtnClick: function(sender){
		...
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, alamat from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('8') and status='1' ",
										  "select count(no_klaim) from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and progress in ('8')  and status='1' ",
		...
		if (sender == this.cb_klaim){
			this.standarLib.showListData(this, "Data Klaim",sender,undefined, 
										  "select no_klaim, alamat from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and flag12='0' and status='0' ",
										  "select count(no_klaim) from tlk_klaim where kode_lokasi = '"+this.app._lokasi+"' and kode_ttg='"+this.app._kodeTtg+"' and flag12='0' and status='0' ",
										  ["no_klaim","alamat"],"and",["No Klaim","Lokasi"],false); 
		}
		*/
	},	
