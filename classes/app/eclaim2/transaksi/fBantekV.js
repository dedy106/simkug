window.app_eclaim2_transaksi_fBantekV = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_transaksi_fBantekV.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim2_transaksi_fBantekV";
		...
		this.tahun=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from tlk_bantek");
...
window.app_eclaim2_transaksi_fBantekV.extend(window.childForm);
window.app_eclaim2_transaksi_fBantekV.implement({
	...
	doEllipseClick:function(sender, col, row)
	{
		if (row == 2)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Klaim",this.sg1, this.sg1.row, this.sg1.col,
												...
												"from tlk_klaim a "+
												"inner join tlk_bantek b on a.no_klaim=b.no_klaim "+filter,
		...
		if (row == 4)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Verifikasi",this.sg1, this.sg1.row, this.sg1.col,
													  "select a.no_bantek,a.keterangan from tlk_bantek a "+
													  "inner join tlk_klaim b on a.no_klaim=b.no_klaim "+filter,
													  "select count(a.no_bantek) from tlk_bantek a "+
													  "inner join tlk_klaim b on a.no_klaim=b.no_klaim "+filter,
	...
	doSelectCell:function(sender, col, row)
	{
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from tlk_bantek order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from tlk_bantek order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	...
	doSgClick:function(sender, col, row){	
		...
		if (col==6){
			try{
				var data = this.dbLib.getDataProvider("select nama as nm from tlk_bantek_dok "+
