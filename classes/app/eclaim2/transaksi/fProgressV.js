...
//* 			SAI, PT											
//***********************************************************************************************
window.app_eclaim2_transaksi_fProgressV = function(owner,options){
	try{
		if (owner)
		{
			window.app_eclaim2_transaksi_fProgressV.prototype.parent.constructor.call(this, owner,options);
			this.className = "app_eclaim2_transaksi_fProgressV";
			this.maximize();
	...
	{
		alert("[app_eclaim2_transaksi_fProgressV]::contruct:"+e,"");
	}
};
window.app_eclaim2_transaksi_fProgressV.extend(window.childForm);
window.app_eclaim2_transaksi_fProgressV.implement({
    ...
    initComponent: function(){
		try{
			...
			this.p1mp = new panel(this,{bound:[10,35,this.width - 25,250],border:3,caption:"Daftar Progress"});
			this.sg1mp = new saiGrid(this.p1mp,{bound:[1,20,this.p1mp.width - 4,this.p1mp.height - 50],colCount:8,colTitle:["No. Progress","No. Klaim","Tertanggung","Tanggal","Vendor","No. Dokumen","Keterangan","Preview History"],
			...
			this.sg1mp3 = new saiGrid(this.p1mp3,{bound:[1,20,this.p1mp3.width - 4,128],colCount:3,readOnly:true,colTitle:["Pekerjaan","Progress","Realisasi"],
                colWidth:[[2,1,0],[100,100,250]]});
			...
			this.pager = 50;
			var sql = "select count(a.no_progress)	   "+                         
			...
			this.sql =  "select a.no_progress,a.no_klaim,c.nama as nama_ttg,date_format(a.tanggal,'%d/%m/%Y') as tgl,concat(a.kode_vendor,'-',d.nama) as vendor , "+
						"a.no_dokumen,a.keterangan    "+
						"from tlk_progresspk a "+
						"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
						"inner join tlk_ttg c on b.kode_ttg=c.kode_ttg "+			
						" inner join tlk_vendor d on d.kode_vendor = a.kode_vendor and d.kode_lokasi = a.kode_lokasi "+
						"where a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_ttg='"+this.app._kodeTtg+"'  "+this.userLogin;	
			...
			this.sgFilter.editData(3,["No Progress","All","",""]);			
    ...
    doClick: function(sender){
        try{
            if (sender == this.bApply){
			    ...
			    this.filterRep.filterStr("a.no_progress",this.sgFilter.getCell(1,3),this.sgFilter.getCell(2,3),this.sgFilter.getCell(3,3),"and");
			        ...
			        if (this.senderPager == this.sgn){
						...
						if (brg.rs.rows[0]!=undefined)
						{
							...
							for (var i in brg.rs.rows){
								line = brg.rs.rows[i];
								this.gridDataTmp.set(line.no_progress,line);
								this.sg1mp.appendData([line.no_progress,line.no_klaim, line.nama_ttg, line.tanggal, line.vendor, line.no_dokumen, line.keterangan,"Preview"]);
						...
						if (brg.rs.rows[0]!=undefined){
							...
							for (var i in brg.rs.rows){
								line = brg.rs.rows[i];								
								this.sgHist.appendData([(line.status == "1" ? "<img width=18 height=18 src='image/ok.png' />." :"<img width=18 height=18 src='image/error.png' />." ),line.no_progress,line.no_progress, line.tanggal, line.no_dokumen, line.keterangan, floatToNilai(line.nilai), floatToNilai(line.nilai_adjust)]);								
							}					
	...
	doSgClick:function(sender, col, row){		
		if (sender == this.sg1mp){
			try{
				var data = this.dbLib.getDataProvider("select nama as nm from tlk_progresspk_dok "+
						"where no_progress='"+this.sg1mp.getCell(0,row)+"' and kode_lokasi = '"+this.app._lokasi+"'  ",true);
				this.sg1mp2.clear();
				...
				this.sgn2.rearrange();						
				data = this.dbLib.getDataProvider("select a.nm_pekerjaan, a.realisasi, sum(d.realisasi * b.bobot/100) as prog from tlk_progresspk_d a "+
						...
						" left outer join (select b.no_pelaksanaan, a.nm_pekerjaan, b.tanggal, a.realisasi from tlk_progresspk_d a inner join tlk_progresspk b on b.no_progress = a.no_progress and b.no_klaim = '"+ this.sg1mp.getCell(1,row)+"' and b.no_progress <> '"+this.sg1mp.getCell(0,row)+"' order by b.tanggal) d on d.tanggal <=  a.tgl_mulai and d.nm_pekerjaan = a.nm_pekerjaan and d.no_pelaksanaan = b.no_pelaksanaan "+
						"	where a.no_progress = '"+this.sg1mp.getCell(0,row)+"' group by a.nm_pekerjaan",true);
				this.sg1mp3.clear();
				if (typeof(data) != "string")
				{  					
					this.sg1mp3.clear();
					for (var i in data.rs.rows){
						line = data.rs.rows[i];
						this.sg1mp3.appendData([line.nm_pekerjaan, line.prog, line.realisasi]);					
					}
				}				
			...
			if (col == 10){
				var sql =  "select a.status, a.no_progress,a.no_klaim,c.nama as nama_ttg,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
					"a.no_dokumen,a.keterangan   "+                         
					"from tlk_progresspk a "+
					"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
					"inner join tlk_ttg c on b.kode_ttg=c.kode_ttg "+					
					"where a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_ttg='"+this.app._kodeTtg+"' and a.no_progress = '"+this.sg1mp.cells(3,row)+"' ";
					
				var sqlCount =  "select count(a.no_progress) "+                         
					"from tlk_progresspk a "+
					"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
					"inner join tlk_ttg c on b.kode_ttg=c.kode_ttg "+					
					"where a.kode_lokasi = '"+this.app._lokasi+"' and b.kode_ttg='"+this.app._kodeTtg+"' and a.no_progress = '"+this.sg1mp.cells(3,row)+"'  ";
	...
	doEllipsFilterClick: function(sender, col, row){
		...
		if (row == 3)
		{
			this.filterRep.ListDataSGFilter(this, "Data Progress",sender, sender.row, sender.col,
													  "select a.no_progress,a.no_dokumen,date_format(a.tanggal,'%d/%m/%Y') as tanggal from tlk_progresspk a "+
													  ...
													  "select count(no_progress) from tlk_progresspk a "+
													  "	inner join tlk_klaim b on b.no_klaim = a.no_klaim and b.kode_lokasi = a.kode_lokasi "+
													  " where a.kode_lokasi='"+this.app._lokasi+"' "+
													  this.filterRep.filterStr("a.periode",this.sgFilter.getCell(1,0),this.sgFilter.getCell(2,0),this.sgFilter.getCell(3,0),"and") +			    
													  this.filterRep.filterStr("b.kode_ttg",this.sgFilter.getCell(1,1),this.sgFilter.getCell(2,1),this.sgFilter.getCell(3,1),"and")+
													  this.filterRep.filterStr("a.no_klaim",this.sgFilter.getCell(1,2),this.sgFilter.getCell(2,2),this.sgFilter.getCell(3,2),"and"),
													  ["no_progress","no_dokumen","tanggal"],"and",["No Progress","no dokumen","tanggal"]);
		}		
	},
