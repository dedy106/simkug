window.app_eclaim2_transaksi_fSurveyV = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_transaksi_fSurveyV.prototype.parent.constructor.call(this,owner);
		this.className  = "app_eclaim2_transaksi_fSurveyV";
		...
		this.pc1 = new pageControl(this,{bound:[15,12,1000,500], childPage:["Filter","Data Verifikasi","Data Klaim","Data Riwayat Klaim"]});
		...
		this.tahun=this.dbLib.getPeriodeFromSQL("select max(substring(periode,1,4)) as periode from tlk_survey");
		...
		this.sg1mp = new saiGrid(this.pc1.childPage[1],{bound:[1,20,this.pc1.width - 4,this.pc1.height - 180],colCount:9,
					colTitle:["No. Survey","No. Klaim","Loss Adjuster","Tgl Survey","No. Dokumen","Keterangan","Status","Nilai Estimasi","Preview Dok"],
					colWidth:[[8,7,6,5,4,3,2,1,0],[70,100,70,120,100,60,200,100,100]],
					selectCell:[this,"doSgClick"], 
					colAlign:[[6,7,8],[alCenter,alRight,alCenter]],
					colFormat:[[7,8],[cfNilai, cfButton]],readOnly:true});
...
window.app_eclaim2_transaksi_fSurveyV.extend(window.childForm);
window.app_eclaim2_transaksi_fSurveyV.implement({
	...
	doEllipseClick:function(sender, col, row)
	{
		if (row == 2)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Klaim",this.sg1, this.sg1.row, this.sg1.col,
												...
												"from tlk_klaim a "+
												"inner join tlk_survey b on a.no_klaim=b.no_klaim "+filter,
		...
		if (row == 4)
		{
			...
			this.filterRep.ListDataSGFilter(this, "Data Verifikasi",this.sg1, this.sg1.row, this.sg1.col,
													  "select a.no_survey,a.keterangan from tlk_survey a "+
													  "inner join tlk_klaim b on a.no_klaim=b.no_klaim "+filter,
													  "select count(a.no_survey) from tlk_survey a "+
													  "inner join tlk_klaim b on a.no_klaim=b.no_klaim "+filter,
													  new Array("a.no_survey","a.keterangan"),"and",new Array("kode","nama"));
		}
	...
	doSelectCell:function(sender, col, row)
	{
		...
		if (row == 0)
		{
			...
			this.dbLib.setItemsFromSQL("select distinct substring(periode,1,4) from tlk_survey order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
		if (row == 1)
		{
			this.sg1.columns.get(2).pickList.clear();
			this.dbLib.setItemsFromSQL("select distinct periode from tlk_survey order by periode desc",[this.sg1.columns.get(2).pickList,this.sg1.columns.get(3).pickList]);
		}
	},
	mainButtonClick:function(sender)
	{
		try
		{
			...
			else
			{
				...
				var sql="select a.no_survey,a.no_klaim,c.nama as nama_ttg,d.nama as nama_la,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
						...
						"inner join tlk_la d on a.kode_la=d.kode_la "+this.filter+user_lok+
	...
	doTampilData2: function(page) {
		...
		for (var i=start;i<finish;i++){
			...
			this.sg1mp.appendData([line.no_survey,line.no_klaim,line.nama_la,line.tanggal,line.no_dokumen, line.keterangan, line.sts_survey, floatToNilai(line.nilai),"Preview"]);
	...
	doSgClick:function(sender, col, row){	
		...
		if (col==8){
			try{
				var data = this.dbLib.getDataProvider("select nama as nm from tlk_survey_dok "+
						"where no_survey='"+this.sg1mp.getCell(0,row)+"' and kode_lokasi = '"+this.app._lokasi+"'  ",true);
				this.sg1mp2.clear();
