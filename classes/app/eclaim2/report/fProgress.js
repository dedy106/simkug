window.app_eclaim2_report_fProgress = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_report_fProgress.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim2_report_fProgress";
		this.maximize();
		this.app._mainForm.childFormConfig(this,"mainButtonClick","Laporan Progress",2);
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1), new Array("No Progress","All"));
...
window.app_eclaim2_report_fProgress.extend(window.portalui_childForm);
window.app_eclaim2_report_fProgress.implement({
	doEllipseClick: function(sender, col, row){
		if (row == 1)
			...
			if (row == 3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Progress",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_progress,no_dokumen,date_format(tanggal,'%d/%m/%Y') as tanggal from tlk_progresspk where kode_lokasi='"+this.app._lokasi+"'",
														  "select count(no_progress) from tlk_progresspk  where kode_lokasi='"+this.app._lokasi+"'",
														  new Array("no_progress","no_dokumen","tanggal"),"and",new Array("No Progres","no dokumen","tanggal"));
			}
	},
	doSelectCell: function(sender, col, row){
			...
			if (row == 0)
			{
				...
				var rs = this.dbLib.runSQL("select distinct periode from tlk_progresspk where kode_lokasi='"+this.app._lokasi+"' ");			
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter = this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					  ...
					  this.filterRep.filterStr("a.no_progress",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.sql =  "select a.no_progress,a.no_klaim,c.nama as nama_ttg,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
							"a.keterangan,a.no_pelaksanaan, d.kode_vendor, d.nama as nmvendor, e.nm_pekerjaan, e.realisasi	   "+                         
							"from tlk_progresspk a "+
							"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
							"inner join tlk_ttg c on b.kode_ttg=c.kode_ttg "+
							"inner join tlk_vendor d on d.kode_vendor = a.kode_vendor "+
							"inner join tlk_progresspk_d e on e.no_progress = a.no_progress "+
							this.filter+" order by a.no_progress, e.nu ";
				this.scriptSqlCount =   "select count(a.no_progress)	   "+                         
							"from tlk_progresspk a "+
							"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
							"inner join tlk_ttg c on b.kode_ttg=c.kode_ttg "+
							"inner join tlk_vendor d on d.kode_vendor = a.kode_vendor "+
				...
				title.add("No Progress");width.add(100);fieldType.add("S");		
				...
				title.add("No Pelaksanaan");width.add(100);fieldType.add("S");	
				title.add("Kode Vendor");width.add(100);fieldType.add("S");	
				title.add("Nama Vendor");width.add(300);fieldType.add("S");	
				title.add("Item Pekerjaan");width.add(300);fieldType.add("S");	
				title.add("Realisasi");width.add(80);fieldType.add("N");	
				
				this.groupBy = new server_util_arrayList({items:["no_progress","no_klaim","nama_ttg","tanggal","keterangan","no_pelaksanaan","kode_vendor","nmvendor"]});		
				this.groupHeader = new server_util_arrayList({items:["no_progress"]});		
		...
		{
			alert("[app_eclaim2_report_fProgress]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN PROGRESS<br>";			
		var d = new Date();
