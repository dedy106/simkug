window.app_eclaim2_report_flLostAd = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_report_flLostAd.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim2_report_flLostAd";
		this.maximize();
...
window.app_eclaim2_report_flLostAd.extend(window.portalui_childForm);
window.app_eclaim2_report_flLostAd.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.sql =  "select kode_la,  nama, alamat, kota, kodepos,pic, no_telp, no_fax,  email, web "+
							"from tlk_la "+this.filter+
							" order by kode_la ";
				this.scriptSqlCount =  "select count(kode_la) from tlk_la "+this.filter;
		...
		{
			alert("[app_eclaim2_report_flLostAd]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN LOSS ADJUSTER<br>";			
		var d = new Date();
