...
window.app_eclaim2_report_flBAST = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_report_flBAST.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim2_report_flBAST";
		this.maximize();
		this.app._mainForm.childFormConfig(this,"mainButtonClick","Laporan BAST",2);
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1), new Array("No BAST","All"));
...
window.app_eclaim2_report_flBAST.extend(window.portalui_childForm);
window.app_eclaim2_report_flBAST.implement({
		...
		if (row == 1)
			...
			if (row == 3)
			{
				this.filterRep.ListDataSGFilter(this, "Data BAST Parsial",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_bast,no_dokumen,date_format(tanggal,'%d/%m/%Y') as tanggal from tlk_bast where kode_lokasi='"+this.app._lokasi+"' and jenis ='FUL' ",
														  "select count(no_bast) from tlk_bast  where kode_lokasi='"+this.app._lokasi+"' and jenis = 'FUL'",
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.sql =  "select a.no_bast,a.no_klaim,c.nama as nama_ttg,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
							...
							this.filter+" and a.jenis = 'FUL' order by a.no_bast ";
		...
		{
			alert("[app_eclaim2_report_flBAST]::mainButtonClick:"+e);
		}
	},
		...
		try{
			if (sender === sender.owner.bConfirm){
				...
				if (to !== ""){
					sender.owner.free();
					var d = new Date();
					var subject = "Laporan BAST "+d.toLocaleString();
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN BAST<br>";			
		var d = new Date();
