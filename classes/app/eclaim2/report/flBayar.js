window.app_eclaim2_report_flBayar = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_report_flBayar.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim2_report_flBayar";
		this.maximize();
...
window.app_eclaim2_report_flBayar.extend(window.portalui_childForm);
window.app_eclaim2_report_flBayar.implement({
		...
		if (row == 1)
			...
			if (row == 4)
			{
				this.filterRep.ListDataSGFilter(this, "Data Pembayaran",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_bayar,no_dokumen,date_format(tanggal,'%d/%m/%Y') as tanggal from tlk_bayar where kode_lokasi='"+this.app._lokasi+"'",
														  "select count(no_bayar) from tlk_bayar  where kode_lokasi='"+this.app._lokasi+"'",
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 0)
		{
			...
			var rs = this.dbLib.runSQL("select distinct periode from tlk_bayar where kode_lokasi='"+this.app._lokasi+"' ");			
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.sql =  "select a.no_bayar,a.no_klaim,c.nama as nama_ttg,a.no_adjust,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
							...
							"inner join tlk_adjust d on a.no_adjust=d.no_adjust "+
							this.filter+" order by a.no_adjust ";
				this.scriptSqlCount =   "select count(a.no_bayar)	   "+                         
							"from tlk_bayar a "+
							"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
							"inner join tlk_ttg c on b.kode_ttg=c.kode_ttg "+
							"inner join tlk_adjust d on a.no_adjust=d.no_adjust "+this.filter;
		...
		{
			alert("[app_eclaim2_report_flBayar]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN PEMBAYARAN<br>";			
		var d = new Date();
