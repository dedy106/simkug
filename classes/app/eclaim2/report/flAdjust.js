window.app_eclaim2_report_flAdjust = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_report_flAdjust.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim2_report_flAdjust";
		this.maximize();
...
window.app_eclaim2_report_flAdjust.extend(window.portalui_childForm);
window.app_eclaim2_report_flAdjust.implement({
		...
		if (row == 1)
			...
			if (row == 4)
			{
				this.filterRep.ListDataSGFilter(this, "Data Adjustment",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_adjust,no_dokumen,date_format(tanggal,'%d/%m/%Y') as tanggal from tlk_adjust where kode_lokasi='"+this.app._lokasi+"'",
														  "select count(no_adjust) from tlk_adjust  where kode_lokasi='"+this.app._lokasi+"'",
	...
	doSelectCell: function(sender, col, row){
			...
			if (row == 0)
			{
				...
				var rs = this.dbLib.runSQL("select distinct periode from tlk_adjust where kode_lokasi='"+this.app._lokasi+"' ");			
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.scriptSqlCount =   "select count(a.no_adjust)	   "+                         
							...
							"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
							"inner join tlk_ttg c on b.kode_ttg=c.kode_ttg "+
							"inner join tlk_survey d on a.no_survey=d.no_survey "+this.filter;
		...
		{
			alert("[app_eclaim2_report_flAdjust]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN ADJUSTMENT<br>";			
		var d = new Date();
