window.app_eclaim2_report_flPenunjukan = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_report_flPenunjukan.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim2_report_flPenunjukan";
		this.maximize();
		this.app._mainForm.childFormConfig(this,"mainButtonClick","Laporan Penunjukan Vendor",2);
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1), new Array("No Penunjukan","All"));
...
window.app_eclaim2_report_flPenunjukan.extend(window.portalui_childForm);
window.app_eclaim2_report_flPenunjukan.implement({
	doEllipseClick: function(sender, col, row){
		if (row == 1)
			...
			if (row == 3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Penunjukan",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_penunjukan,no_dokumen,date_format(tanggal,'%d/%m/%Y') as tanggal from tlk_penunjukan where kode_lokasi='"+this.app._lokasi+"'",
														  "select count(no_penunjukan) from tlk_penunjukan  where kode_lokasi='"+this.app._lokasi+"'",
														  new Array("no_penunjukan","no_dokumen","tanggal"),"and",new Array("no dokumen","no dokumen","tanggal"));
			}
	},
	doSelectCell: function(sender, col, row){
			...
			if (row == 0)
			{
				...
				var rs = this.dbLib.runSQL("select distinct periode from tlk_penunjukan where kode_lokasi='"+this.app._lokasi+"' ");			
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter = this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					  ...
					  this.filterRep.filterStr("a.no_penunjukan",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.sql =  "select a.no_penunjukan,a.no_klaim,c.nama as nama_ttg,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
							"a.keterangan,d.kode_vendor, e.nama, date_format(d.tgl_spk,'%d/%m/%Y') as tgl_spk, d.no_spk	   "+                         
							"from tlk_penunjukan a "+
							"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
							"inner join tlk_ttg c on b.kode_ttg=c.kode_ttg "+
							"inner join tlk_penunjukan_d d on d.no_penunjukan = a.no_penunjukan  "+
							"inner join tlk_vendor e on e.kode_vendor = d.kode_vendor  "+
							this.filter+" order by a.no_penunjukan ";
				this.scriptSqlCount =   "select count(a.no_penunjukan)	   "+                         
							"from tlk_penunjukan a "+
							"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
							"inner join tlk_ttg c on b.kode_ttg=c.kode_ttg "+
							"inner join tlk_penunjukan_d d on d.no_penunjukan = a.no_penunjukan  "+
				...
				title.add("No Penunjukan");width.add(100);fieldType.add("S");		
				title.add("No Klaim");width.add(100);fieldType.add("S");		
				title.add("Tertanggung");width.add(150);fieldType.add("S");		
				title.add("Tgl Dok ");width.add(60);fieldType.add("D");	
				title.add("Keterangan");width.add(150);fieldType.add("S");	
				title.add("Kode Vendor");width.add(100);fieldType.add("S");	
				title.add("Nama Vendor");width.add(100);fieldType.add("S");		
				title.add("Tgl SPK");width.add(100);fieldType.add("S");	
				title.add("No SPK");width.add(100);fieldType.add("S");					
				
				this.groupBy = new server_util_arrayList({items:["no_penunjukan","no_klaim","nama_ttg","tanggal","keterangan"]});		
				this.groupHeader = new server_util_arrayList({items:["no_penunjukan"]});		
				var dthtml = this.dbLib.sqlToHtml(this.sql,1,this.pager, title, width,fieldType,false,this.groupBy, this.summary, this.groupHeader);
		...
		{
			alert("[app_eclaim2_report_flPenunjukan]::mainButtonClick:"+e);
		}
	},
		...
		try{
			if (sender === sender.owner.bConfirm){
				...
				if (to !== ""){
					sender.owner.free();
					var d = new Date();
					var subject = "Laporan Kelengkapan Dokumen "+d.toLocaleString();
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN PENUNJUKAN VENDOR KLAIM<br>";			
		var d = new Date();
