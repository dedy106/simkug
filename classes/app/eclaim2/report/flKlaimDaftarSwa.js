window.app_eclaim2_report_flKlaimDaftarSwa = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_report_flKlaimDaftarSwa.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim2_report_flKlaimDaftarSwa";
		this.maximize();
...
window.app_eclaim2_report_flKlaimDaftarSwa.extend(window.portalui_childForm);
window.app_eclaim2_report_flKlaimDaftarSwa.implement({
	...
	mainButtonClick: function(sender){
			...
			else
			{
				...
				if (this.sg1.getCell(2,12)=="Long")
				{
					this.sql =  "select a.no_polis,concat(date_format(j.periode_awal,'%d/%m/%Y'),' - ',date_format(j.periode_akhir,'%d/%m/%Y')) as periode_polis,f.nama as nama_asuransi, date_format(a.tanggal,'%d/%m/%Y') as tanggal "+
							...
							"i.no_ver,i.no_dokumen as dok_ver,date_format(i.tanggal,'%d/%m/%Y') as tgl_ver "+
							",date_format(h.tanggal,'%d/%m/%Y') as tgl_adjust,ifnull(h.nilai,0) as nilai_adjust, "+
							...
							" when a.progress='6' then 'Adjustment' "+
							...
							" when a.progress='14' then 'Discharge Form' end as status "+
							"from tlk_klaim a "+
				...
				{
					this.sql =  "select a.no_polis,f.nama as nama_asuransi, date_format(a.tanggal,'%d/%m/%Y') as tanggal,date_format(i.tanggal,'%d/%m/%Y') as tgl_ver "+
							",c.nama as nama_obyek,d.nama as nama_sebab,e.nama as nama_lok,a.alamat,a.no_klaim,a.nilai, "+
							"ifnull(h.nilai,0) as nilai_adjust, "+
							"case  when a.progress='0' then 'Laporan Awal' "+
							" when a.progress='1' then 'Verifikasi' "+
							" when a.progress='2' then 'Bantek' "+ 
							" when a.progress='3' then 'Survey' "+
		...
		{
			alert("[app_eclaim2_report_flKlaimDaftarSwa]::mainButtonClick:"+e);
		}
	},
