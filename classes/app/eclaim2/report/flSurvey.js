window.app_eclaim2_report_flSurvey = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_report_flSurvey.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim2_report_flSurvey";
		this.maximize();
...
window.app_eclaim2_report_flSurvey.extend(window.portalui_childForm);
window.app_eclaim2_report_flSurvey.implement({
		...
		if (row == 1)
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Loss Adjuster",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_la,nama from tlk_la where tipe='Posting' and kode_lokasi='"+this.app._lokasi+"'",
														  "select count(kode_la) from tlk_la  where tipe='Posting' and kode_lokasi='"+this.app._lokasi+"'",
			...
			if (row == 4)
			{
				this.filterRep.ListDataSGFilter(this, "Data Survey",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_survey,no_dokumen,date_format(tanggal,'%d/%m/%Y') as tanggal from tlk_survey where kode_lokasi='"+this.app._lokasi+"'",
														  "select count(no_survey) from tlk_survey  where kode_lokasi='"+this.app._lokasi+"'",
	...
	doSelectCell: function(sender, col, row){
		...
		if (row == 0)
		{
			...
			var rs = this.dbLib.runSQL("select distinct periode from tlk_survey where kode_lokasi='"+this.app._lokasi+"' ");			
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter = this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					  ...
					  this.filterRep.filterStr("a.no_survey",this.sg1.getCell(1,4),this.sg1.getCell(2,4),this.sg1.getCell(3,4),"and");
				...
				this.sql =  "select a.no_survey,a.no_klaim,c.nama as nama_ttg,d.nama as nama_la,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
							"a.no_dokumen,a.keterangan,case a.status when '0' then 'Not Liable' else 'Liable' end as sts_survey,b.nilai	   "+                         
							"from tlk_survey a "+
							"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
							"inner join tlk_ttg c on b.kode_ttg=c.kode_ttg "+
							"inner join tlk_la d on a.kode_la=d.kode_la "+
							this.filter+" order by a.no_survey ";
				this.scriptSqlCount =   "select count(a.no_survey)	   "+                         
							"from tlk_survey a "+
							"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
							"inner join tlk_ttg c on b.kode_ttg=c.kode_ttg "+
							"inner join tlk_la d on a.kode_la=d.kode_la "+this.filter;
		...
		{
			alert("[app_eclaim2_report_flSurvey]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN SURVEY<br>";			
		var d = new Date();
