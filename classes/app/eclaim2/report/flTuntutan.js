window.app_eclaim2_report_flTuntutan = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_report_flTuntutan.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim2_report_flTuntutan";
		this.maximize();
		this.app._mainForm.childFormConfig(this,"mainButtonClick","Laporan Tuntutan",2);
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1), new Array("No Tuntutan","All"));
...
window.app_eclaim2_report_flTuntutan.extend(window.portalui_childForm);
window.app_eclaim2_report_flTuntutan.implement({
	doEllipseClick: function(sender, col, row){
		if (row == 1)
			...
			if (row == 3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Tuntutan",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_tuntutan,no_dokumen,date_format(tanggal,'%d/%m/%Y') as tanggal from tlk_tuntutan where kode_lokasi='"+this.app._lokasi+"'",
														  "select count(no_tuntutan) from tlk_tuntutan  where kode_lokasi='"+this.app._lokasi+"'",
														  new Array("no_tuntutan","no_dokumen","tanggal"),"and",new Array("No Tuntutan","No Dokumen","Tanggal"));
			}
	},
	doSelectCell: function(sender, col, row){
			...
			if (row == 0)
			{
				...
				var rs = this.dbLib.runSQL("select distinct periode from tlk_tuntutan where kode_lokasi='"+this.app._lokasi+"' ");			
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter = this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					  ...
					  this.filterRep.filterStr("a.no_tuntutan",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.sql =  "select a.no_tuntutan,a.no_dokumen,a.no_klaim,c.nama as nama_ttg,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
							"a.keterangan	   "+                         
							"from tlk_tuntutan a "+
							"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
							"inner join tlk_ttg c on b.kode_ttg=c.kode_ttg "+
							this.filter+" order by a.no_tuntutan ";
				this.scriptSqlCount =   "select count(a.no_tuntutan)	   "+                         
							"from tlk_tuntutan a "+
							"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
							"inner join tlk_ttg c on b.kode_ttg=c.kode_ttg "+this.filter;
				var title = new server_util_arrayList();
				var width = new server_util_arrayList();
				var fieldType = new server_util_arrayList();
				title.add("No Tuntutan");width.add(100);fieldType.add("S");		
				...
				title.add("Tgl Tuntutan ");width.add(60);fieldType.add("D");	
		...
		{
			alert("[app_eclaim2_report_flTuntutan]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN TUNTUTAN KLAIM<br>";			
		var d = new Date();
