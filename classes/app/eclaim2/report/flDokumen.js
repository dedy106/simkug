window.app_eclaim2_report_flDokumen = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_report_flDokumen.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim2_report_flDokumen";
		this.maximize();
		this.app._mainForm.childFormConfig(this,"mainButtonClick","Laporan Kelengkapan Dokumen",2);
...
window.app_eclaim2_report_flDokumen.extend(window.portalui_childForm);
window.app_eclaim2_report_flDokumen.implement({
		...
		if (row == 1)
			...
			if (row == 3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Dokumen",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_dok,no_dokumen,date_format(tanggal,'%d/%m/%Y') as tanggal from tlk_dok_m where kode_lokasi='"+this.app._lokasi+"'",
														  "select count(no_dok) from tlk_dok_m  where kode_lokasi='"+this.app._lokasi+"'",
	...
	doSelectCell: function(sender, col, row){
			...
			if (row == 0)
			{
				...
				var rs = this.dbLib.runSQL("select distinct periode from tlk_dok_m where kode_lokasi='"+this.app._lokasi+"' ");			
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.sql =  "select a.no_dok,a.no_klaim,c.nama as nama_ttg,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
							"a.keterangan,b.nilai	   "+                         
							"from tlk_dok_m a "+
							"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
							"inner join tlk_ttg c on b.kode_ttg=c.kode_ttg "+
							this.filter+" order by a.no_dok ";
				this.scriptSqlCount =   "select count(a.no_dok)	   "+                         
							"from tlk_dok_m a "+
							"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
				...
				title.add("No Dok");width.add(100);fieldType.add("S");		
		...
		{
			alert("[app_eclaim2_report_flDokumen]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN KELENGKAPAN DOKUMEN<br>";			
		var d = new Date();
