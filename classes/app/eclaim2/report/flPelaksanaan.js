window.app_eclaim2_report_flPelaksanaan = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_report_flPelaksanaan.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim2_report_flPelaksanaan";
		this.maximize();
		this.app._mainForm.childFormConfig(this,"mainButtonClick","Laporan Pelaksanaan",2);
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1), new Array("No Pelaksanaan","All"));
...
window.app_eclaim2_report_flPelaksanaan.extend(window.portalui_childForm);
window.app_eclaim2_report_flPelaksanaan.implement({
	doEllipseClick: function(sender, col, row){
		if (row == 1)
			...
			if (row == 3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Pelaksanaan",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_pelaksanaan,no_dokumen,date_format(tanggal,'%d/%m/%Y') as tanggal from tlk_pelaksanaan where kode_lokasi='"+this.app._lokasi+"'",
														  "select count(no_pelaksanaan) from tlk_pelaksanaan  where kode_lokasi='"+this.app._lokasi+"'",
														  new Array("no_pelaksanaan","no_dokumen","tanggal"),"and",new Array("No Pelaksanaan","No Dokumen","Tanggal"));
			}
	},
	doSelectCell: function(sender, col, row){
			...
			if (row == 0)
			{
				...
				var rs = this.dbLib.runSQL("select distinct periode from tlk_pelaksanaan where kode_lokasi='"+this.app._lokasi+"' ");			
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter = this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					  ...
					  this.filterRep.filterStr("a.no_pelaksanaan",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.sql =  "select a.no_pelaksanaan,a.no_klaim,c.nama as nama_ttg,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
							"a.keterangan,d.kode_vendor, d.nama as nmvdr	   "+                         
							...
							this.filter+" order by a.no_pelaksanaan ";
				this.scriptSqlCount =   "select count(a.no_pelaksanaan)	   "+                         
							"from tlk_pelaksanaan a "+
							"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
							"inner join tlk_ttg c on b.kode_ttg=c.kode_ttg "+
							"inner join tlk_vendor d on d.kode_vendor=a.kode_vendor "+
		...
		{
			alert("[app_eclaim2_report_flPelaksanaan]::mainButtonClick:"+e);
		}
	},
		...
		try{
			if (sender === sender.owner.bConfirm){
				...
				if (to !== ""){
					sender.owner.free();
					var d = new Date();
					var subject = "Laporan Pelaksanaan Pekerjaan "+d.toLocaleString();
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN PELAKSANAAN<br>";			
		var d = new Date();
