window.app_eclaim2_report_flPenanggung = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_report_flPenanggung.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim2_report_flPenanggung";
		this.maximize();
...
window.app_eclaim2_report_flPenanggung.extend(window.childForm);
window.app_eclaim2_report_flPenanggung.implement({
		...
		if (row == 0)
			{
				...
				this.filterRep.ListDataSGFilter(this, "Data Penanggung",this.sg1, this.sg1.row, this.sg1.col,
														  ...
														  "select count(kode_png) from tlk_png where kode_lokasi='"+this.app._lokasi+"'",
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.sql =  "select kode_png,  nama, alamat, kota, kodepos,pic, no_telp, no_fax,  email, web "+
							"from tlk_png "+this.filter+
							" order by kode_png ";
				this.scriptSqlCount =  "select count(kode_png) from tlk_png "+this.filter;
		...
		{
			alert("[app_eclaim2_report_flPenanggung]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN PENANGGUNG<br>";			
		var d = new Date();
		...
		case "pdfBtn" :      
			try{			
				...
				downloadFile(this.dbLib.getPdf(this.sqlScript, 
					new server_util_arrayList({items:[this.app._namalokasi.toUpperCase(), "LAPORAN PENANGGUNG"]}),
					new server_util_arrayList({items:[5, 6]}),
					new server_util_arrayList({items:[8, 12]}),
					'L','A4',8,8,
					this.title,8,8,
					new server_util_arrayList({items:[10, 60, 80, 15, 15, 40, 15, 15, 15, 15]}),
					"penanggung.pdf", 5
				));
			}catch(e){
				alert(e);
