window.app_eclaim2_report_flKlaimPosisi = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_report_flKlaimPosisi.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim2_report_flKlaimPosisi";
		this.maximize();
...
window.app_eclaim2_report_flKlaimPosisi.extend(window.portalui_childForm);
window.app_eclaim2_report_flKlaimPosisi.implement({
		...
		if (row == 1)
			...
			if (row == 2)
			{
				this.filterRep.ListDataSGFilter(this, "Data Penyebab Kerugian",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_sebab,nama from tlk_sebab where tipe='Posting' and kode_lokasi='"+this.app._lokasi+"'",
														  "select count(kode_sebab) from tlk_sebab  where tipe='Posting' and kode_lokasi='"+this.app._lokasi+"'",
			...
			if (row == 3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Obyek Kerugian",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_obyek,nama from tlk_obyek where tipe='Posting' and kode_lokasi='"+this.app._lokasi+"'",
														  "select count(kode_obyek) from tlk_obyek  where tipe='Posting' and kode_lokasi='"+this.app._lokasi+"'",
			...
			if (row == 4)
			{
				this.filterRep.ListDataSGFilter(this, "Data Lokasi Kejadian",this.sg1, this.sg1.row, this.sg1.col,
														  "select kode_lok,nama from tlk_lokasi where tipe='Posting' and kode_lokasi='"+this.app._lokasi+"'",
														  "select count(kode_lok) from tlk_lokasi  where tipe='Posting' and kode_lokasi='"+this.app._lokasi+"'",
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.sql =  "select a.no_klaim, b.nama as nama_ttg,f.nama as nama_asuransi, date_format(a.tanggal,'%d/%m/%Y') as tanggal "+
							"	   ,c.nama as nama_obyek,d.nama as nama_sebab,e.nama as nama_lok,a.nilai, "+
							...
							"left join tlk_bayar i on a.no_klaim=i.no_klaim "+
				...
				this.scriptSqlCount =   "select count(a.no_klaim) "+                         
							...
							"inner join tlk_asuransi f on a.kode_asuransi=f.kode_asuransi "+ 
							"left join tlk_survey g on a.no_klaim=g.no_klaim "+
							"left join tlk_adjust h on a.no_klaim=h.no_klaim "+
							"left join tlk_bayar i on a.no_klaim=i.no_klaim "+this.filter;
		...
		{
			alert("[app_eclaim2_report_flKlaimPosisi]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN POSISI KLAIM<br>";			
		var d = new Date();
