window.app_eclaim2_report_flTesting = function(owner)
{
	if (owner)
	{
		window.app_eclaim2_report_flTesting.prototype.parent.constructor.call(this,owner);
		this.className = "app_eclaim2_report_flTesting";
		this.maximize();
		this.app._mainForm.childFormConfig(this,"mainButtonClick","Laporan Testing",2);
	...
	this.gridLib.SGEditData(this.sg1,3,new Array(0,1), new Array("No Testing","All"));
...
window.app_eclaim2_report_flTesting.extend(window.portalui_childForm);
window.app_eclaim2_report_flTesting.implement({
	doEllipseClick: function(sender, col, row){
		if (row == 1)
			...
			if (row == 3)
			{
				this.filterRep.ListDataSGFilter(this, "Data Testing",this.sg1, this.sg1.row, this.sg1.col,
														  "select no_testing,no_dokumen,date_format(tanggal,'%d/%m/%Y') as tanggal from tlk_testing where kode_lokasi='"+this.app._lokasi+"'",
														  "select count(no_testing) from tlk_testing  where kode_lokasi='"+this.app._lokasi+"'",
														  new Array("no_testing","no_dokumen","tanggal"),"and",new Array("no dokumen","no dokumen","tanggal"));
			}
	},
	doSelectCell: function(sender, col, row){
			...
			if (row == 0)
			{
				...
				var rs = this.dbLib.runSQL("select distinct periode from tlk_testing where kode_lokasi='"+this.app._lokasi+"' ");			
	...
	mainButtonClick: function(sender){
		try
		{
			...
			else
			{
				...
				this.filter = this.filterRep.filterStr("a.periode",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where") +
					  ...
					  this.filterRep.filterStr("a.no_testing",this.sg1.getCell(1,3),this.sg1.getCell(2,3),this.sg1.getCell(3,3),"and");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				this.sql =  "select a.no_testing,a.no_klaim,c.nama as nama_ttg,date_format(a.tanggal,'%d/%m/%Y') as tanggal, "+
							"a.keterangan "+
							"from tlk_testing a "+
							"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
							"inner join tlk_ttg c on b.kode_ttg=c.kode_ttg "+
							this.filter+" order by a.no_testing ";
				this.scriptSqlCount =   "select count(a.no_testing)	   "+                         
							"from tlk_testing a "+
							"inner join tlk_klaim b on a.no_klaim=b.no_klaim "+
				...
				title.add("No Testing");width.add(100);fieldType.add("S");		
				title.add("No Klaim");width.add(100);fieldType.add("S");		
				title.add("Tertanggung");width.add(150);fieldType.add("S");		
				title.add("Tgl Testing");width.add(60);fieldType.add("D");	
		...
		{
			alert("[app_eclaim2_report_flTesting]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>LAPORAN TESTING<br>";			
		var d = new Date();
