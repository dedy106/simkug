...
//* 			SAI, PT											
//***********************************************************************************************
window.app_frontend_alpa_fProfil = function(owner,options){
	try{
		if (owner)
		{
			window.app_frontend_alpa_fProfil.prototype.parent.constructor.call(this, owner,options);
			this.setBorder(0);		
			this.setColor("");		
			this.className = "app_frontend_alpa_fProfil";											
			this.initComponent();
			this.getClientCanvas().style.overflow = "auto";
		}
	}catch(e)
	{
		alert("[app_frontend_alpa_fProfil]::contruct:"+e,"");
	}
};
window.app_frontend_alpa_fProfil.extend(window.portalui_panel);
window.app_frontend_alpa_fProfil.implement({
	initComponent: function(){
		try{
			this.standarLib = new util_standar();
			this.dbLib = new util_dbUtility();
			this.dbLib.addListener(this);
			this.swf1 = new portalui_flashObj(this,{bound:[0,0,this.width,150],flashId:this.getFullId()+"_swf1",flashFile:"classes/app/frontend/alpa/swf/profil.swf"});   								
			...
			this.dbLib.getDataProviderA("select judul,kode_konten from portal_konten where kode_klp='K02' and kode_lokasi='"+this.app._lokasi+"' ");
		}catch(e){
			alert(e);
		}
	},
