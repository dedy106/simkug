...
//* 			SAI, PT											
//***********************************************************************************************
window.app_frontend_alpa_fForum = function(owner,options){
	try{
		if (owner)
		{
			window.app_frontend_alpa_fForum.prototype.parent.constructor.call(this, owner,options);
			this.setBorder(0);
			this.setColor("");
			this.setScroll(true);
			this.className = "app_frontend_alpa_fForum";
			this.initComponent();		
			this.title = "Unit Pinjaman";			
		}
	}catch(e)
	{
		alert("[app_frontend_alpa_fForum]::contruct:"+e,"");
	}
};
window.app_frontend_alpa_fForum.extend(window.portalui_panel);
window.app_frontend_alpa_fForum.implement({
	initComponent: function(){
		try{			
			uses("portalui_treeView;portalui_treeNode;portalui_saiCB;portalui_image;portalui_pager;server_util_mail;util_file;portalui_label");
			this.standarLib = new util_standar();
			this.dbLib = new util_dbUtility();
			this.dbLib.addListener(this);
			this.swf1 = new portalui_flashObj(this,{bound:[0,0,this.width,150],flashId:this.getFullId()+"_swf1",flashFile:"classes/app/frontend/alpa/swf/forum.swf"});   								            			
			this.pSearch = new portalui_roundPanel(this,{bound:[10,160,260,130],background:"image/themes/dynpro/roundpanelBgCenter.png",caption:"Pencarian",icon:"image/themes/dynpro/iconpanel.png",color:"#efefef",titleBg:"#b5c6d5",view :0, itemClick:[this,"doChildItemsClick"]});			                        
			this.cbKategori = new portalui_saiCB(this.pSearch,{bound:[5,10,200,20],caption:"Kategori"});
			this.cbKategori.setItemHeight(10);
			this.cbKategori.addItem(0,"All");			
			this.search = new portalui_saiLabelEdit(this.pSearch,{bound:[5,35,200,20],caption:"Cari",keyDown:[this,"keyDown"]});			
			this.btnCari = new portalui_button(this.pSearch,{bound:[130,65,80,18],icon:"url(icon/"+system.getThemes()+"/find2.png)",caption:"Search",click:[this,"doSearch"]});			
            this.tbmenu = new portalui_roundPanel(this,{bound:[10,300,260,this.height - 320],background:"image/themes/dynpro/roundpanelBgCenter.png",caption:"Menu",icon:"image/themes/dynpro/iconpanel.png",color:"#efefef",titleBg:"#b5c6d5",view :0, itemClick:[this,"doChildItemsClick"]});			
			this.treev = new portalui_treeView(this.tbmenu,{bound:[1,1,this.tbmenu.width -2,this.tbmenu.height - 22], dblClick:[this, "treeClick"],styled:false});
			this.treev.childLength = 260;						
			this.pDetail = new portalui_panel(this,{bound:[280, 160,this.width - 310,this.height - 200],border:0,color:""});			
			this.pDetail.getClientCanvas().style.overflow ="auto";
            this.childBlock = new portalui_panel(this,{bound:[280,160,this.width - 310,this.height - 180],border:0,color:"#9cc1db"});
			this.childBlock.getCanvas().style.zIndex = 999999;
			this.childBlock.hide();
			this.load = new portalui_image(this.childBlock,{bound:[this.childBlock.width / 2 - 15,this.childBlock.height / 2 - 30,31,31],image:"image/gridload.gif"});
			this.childBlock.setOpacity(80);
			
			this.pgr = new portalui_pager(this,{bound:[280,this.height - 36,this.width - 310,18]});
			this.pgr.onPager.set(this,"doPager");
			this.pgr.hide();
			this.path = [];
			this.smileCode=[];
			this.smileGbr=[];
			var data = this.dbLib.getDataProvider("select * from portal_icon",true);
			...
			systemAPI.alert("Forum Initialization",e);
		}
	},
	doChildItemsClick: function(sender,id,item){
		try{			
			...
			alert("fUsaha::doChildItemsClick : "+e);
		}
	},
	showForum: function(){
		try{
			...
			this.panel = new portalui_panel(this.pDetail,{bound:[0,0,this.pDetail.width-3,this.pDetail.height - 3],border:0,color:""});
			this.panel.setScroll(true);			
			var bglefttop = new portalui_image(this.panel,{bound:[0,0,5,25],image:"image/themes/dynpro/bglefttop.png"});
	...
	doClickForum: function(sender){
		try{		
			...
			this.panel.free();			
			this.panel = new portalui_panel(this.pDetail,{bound:[0,0,this.pDetail.width,this.pDetail.height],border:0,color:""});
			this.panel.setScroll(true);
			var newtopik = new portalui_button(this.panel,{bound:[1,1,100,18],icon:"url(icon/"+system.getThemes()+"/newtopik.png)",caption:"NEWTOPIK",tag:"NEWTOPIK",click:[this,"doBtnClick"]});
			...
			if (typeof(data)=="object"){
				if (data.rs.rows[0] != undefined){
					...
					{						
						...
						post.setCaption("by <a href='#' onclick='return window.app_frontend_alpa_transaksi_fTabForum.userklik=true;' style='color:#aa0000;'>"+line.nik_buat+"</a> >> "+line.tgljam);
						post.fnt.setSize(8);
						...
						last.setCaption("by <a href='#' onclick='return window.app_frontend_alpa_transaksi_fTabForum.userklik=true;' style='color:#aa0000;'>"+line.nik_buat+"</a><br>"+line.lastpost);
						last.fnt.setSize(8);
						last.setColor("#655d9f");
	...
	doUserClick: function(gettag){
		...
		{
			system.alert(this,"app_frontend_alpa_transaksi_fTabForum::doUserClick : "+e,"");
		}
	},
	showStatistik: function(data1,data2,data3){
		try{
			this.pgr.hide();
			this.panel.free();
			this.panel = new portalui_panel(this.pDetail,{bound:[1,1,this.pDetail.width-3,this.pDetail.height],border:3,color:"#cadceb",caption:"User Statistics"});
			this.panel.setScroll(true);
	...
	doPager: function(sender,pageid){
		try{
			...
			alert("app_frontend_alpa_transaksi_fTabForum.prototype.doPager"+e);
		}
	},
	...
	showPost: function(data,pageid){
		...
		{			
			...
			if (typeof data == "object"){
				if (data.rs.rows[0] != undefined){										
					...
					for (var i in data.rs.rows){
						...
						post2.setCaption("<a href='#' onclick='return window.app_frontend_alpa_transaksi_fTabForum.userklik=true;' style='color:#aa0000;'>"+line.nik_buat+"</a>");
						post2.fnt.setSize(8);
			...
			system.alert(this,"app_frontend_alpa_transaksi_fTabForum::showPost : "+e,"");
		}
	},
	...
	postingForm: function(sender,resbj,file,msg){
		try
		{
			...
			this.childBlock.hide();
			this.panel.free();
			this.panel = new portalui_panel(this.pDetail,{bound:[1,1,this.pDetail.width-15,this.pDetail.height - 20],border:3});			
	...
	doSmilyClick: function(sender){
		try{
			...
			if (sender.getTag() === "icon1")
				this.pesan.insertNodeAtSelection(img);
			else if (sender.getTag() === "icon2")
				this.pesanf.insertNodeAtSelection(img);
		}catch(e){
			alert("doSmilyClick : "+e,"");
		}
	},
	doBtnEdit : function(sender){
		...
		{
			system.alert(this,"app_frontend_alpa_transaksi_fTabForum::doBtnEdit : "+e,"");
		}
	},
		...
		try{
			...
			if (sender.getTag() == "POSTREPLY")
			{
				if (this.app.ceklogin){
					...
					alert("Silakan login terlebih dahulu sebagai sales atau customer.","");
				}
				this.execcek=false;
			}
			...
			if (sender.getCaption() == "Kirim Pesan")
			{
				...
				if (data instanceof portalui_arrayMap)
				{
					if (data.get(0) != undefined)
					{
						...
						this.childBlock.show();
						var p9 = new portalui_panel(this.pDetail,{bound:[1,1,this.pDetail.width - 4,this.pDetail.height - 3],caption:"Kirim Pesan",border:3});
						...
						this.subjekf.setWidth(this.pDetail.width-40);
			...
			if (sender.getTag() === "sendmsg")
			{
				...
				this.confirm = new app_frontend_alpa_fConfirmPswd(this);
				this.confirm.setBound(this.pDetail.left+(this.panel.width/2-125),this.pDetail.top+(this.panel.height/2-60),250,120);
				this.confirm.setCaption("Confirm Password");
				this.confirm.setBorder(3);
			}
		}catch(e)
		{
			system.alert(this,"app_frontend_alpa_transaksi_fTabForum::doBtnClick : "+e,"");
		}
	},
	...
	sendMail: function(){
		...
		sql.add("insert into portal_pesan (no_pesan,kode_lokasi,tanggal,dari, kepada,subyek,isi_pesan,flag_email,no_file_dok,tgl_input,flag_read,periode,nik_user,modul) values  "+
				"('"+id+"','"+this.app._lokasi+"',now(),'"+this.app.userlog+"','"+this.kpdf.getText()+"','"+this.subjekf.getText()+"','"+this.pesanf.getText(2)+"','0','"+this.attfilef.getText()+"',now(),'0','"+this.getForm().getPeriodeNow()+"','"+this.app.userlog+"','FORUM') ");
		...
		this.mail.send(dari,kepada,this.subjekf.getText(),this.pesanf.getText(2));
		this.dbLib.execArraySQL(sql);
		this.saveData = true;
		this.attfilef.setText("-");
	},
	treeClick: function(item){
		try{
			...
			alert("treeClick : " + e,"Error Class ::"+temp);
			this.childBlock.hide();
		}
	},
					...
					if (result.toLowerCase().search("error") == -1)
					{
						if (this.execcek)
						{
							...
							if  (this.btnSubmit == "submit") {
								...
								system.confirm(this, "Pemesanan","Proccess Complete....","");
	...
	addPath: function(path, id, index){		
		...
		lblPath.setBound(left,145,(path.length * 7),20);
