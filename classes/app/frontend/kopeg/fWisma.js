...
//* 			SAI, PT											
//***********************************************************************************************
window.app_frontend_kopeg_fWisma = function(owner){
	try{
		if (owner)
		{
			window.app_frontend_kopeg_fWisma.prototype.parent.constructor.call(this, owner);
			window.app_frontend_kopeg_fWisma.prototype.parent.setBorder.call(this, 0);
			window.app_frontend_kopeg_fWisma.prototype.parent.setColor.call(this, "");
			this.className = "app_frontend_kopeg_fWisma";											
			this.initComponent();		
			this.title = "Unit Wisma Asri";
		}
	}catch(e)
	{
		systemAPI.alert("[app_frontend_kopeg_fWisma]::contruct:"+e,"");
	}
};
window.app_frontend_kopeg_fWisma.extend(window.portalui_panel);
window.app_frontend_kopeg_fWisma.implement({
	initComponent: function(){
		try{			
			...
			this.profile.addPage([" UNIT WISMA ASRI "]);						
			...
			this.dbLib.getDataProviderA("select judul,kode_konten from portal_konten where kode_klp='K11' and kode_lokasi='"+this.app._kodeLokasi+"' ");
		}catch(e){
			systemAPI.alert(e);
		}
	},
