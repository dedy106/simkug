...
//* 			SAI, PT											
//***********************************************************************************************
window.app_frontend_kopeg_fDownload = function(owner){
	try{
		if (owner)
		{
			window.app_frontend_kopeg_fDownload.prototype.parent.constructor.call(this, owner);
			window.app_frontend_kopeg_fDownload.prototype.parent.setBorder.call(this, 0);		
			window.app_frontend_kopeg_fDownload.prototype.parent.setColor.call(this, "");		
			this.className = "app_frontend_kopeg_fDownload";											
			this.initComponent();		
			this.title = "Download";
		}
	}catch(e)
	{
		systemAPI.alert("[app_frontend_kopeg_fDownload]::contruct:"+e,"");
	}
};
window.app_frontend_kopeg_fDownload.extend(window.portalui_panel);
window.app_frontend_kopeg_fDownload.implement({
	initComponent: function(){
		try{			
			...
			this.profile.addPage([" UNIT UPKP "]);						
			...
			this.dbLib.getDataProviderA("select judul,kode_konten from portal_konten where kode_klp='K07' and kode_lokasi='"+this.app._kodeLokasi+"' ");
		}catch(e){
			systemAPI.alert(e);
		}
	},
