...
//* 			SAI, PT											
//***********************************************************************************************
window.app_frontend_kopeg_fUpkp = function(owner){
	try{
		if (owner)
		{
			window.app_frontend_kopeg_fUpkp.prototype.parent.constructor.call(this, owner);
			window.app_frontend_kopeg_fUpkp.prototype.parent.setBorder.call(this, 0);		
			window.app_frontend_kopeg_fUpkp.prototype.parent.setColor.call(this, "");		
			this.className = "app_frontend_kopeg_fUpkp";											
			this.initComponent();		
			this.title = "Unit UPKP";
		}
	}catch(e)
	{
		systemAPI.alert("[app_frontend_kopeg_fUpkp]::contruct:"+e,"");
	}
};
window.app_frontend_kopeg_fUpkp.extend(window.portalui_panel);
window.app_frontend_kopeg_fUpkp.implement({
	initComponent: function(){
		try{			
			...
			this.dbLib.getDataProviderA("select judul,kode_konten from portal_konten where kode_klp='K12' and kode_lokasi='"+this.app._kodeLokasi+"' ");
		}catch(e){
			systemAPI.alert(e);
		}
	},
