...
//* 			SAI, PT											
//***********************************************************************************************
window.app_frontend_kopeg_fMain = function(owner){
	try
	{
		if (owner)
		{
			window.app_frontend_kopeg_fMain.prototype.parent.constructor.call(this, owner);
			this.className = "app_frontend_kopeg_fMain";
			this.isClick = false;
			this.mouseX = 0;
			this.mouseY = 0;
			this.maximize();
	...
	{
		systemAPI.alert("[app_frontend_kopeg_fMain]::contruct:"+e,"");
	}
};
window.app_frontend_kopeg_fMain.extend(window.portalui_commonForm);
window.app_frontend_kopeg_fMain.implement({
	...
	initComponent:function(){
	    try{
			...
			this.tab.childPage[1].frameClass = "app_frontend_kopeg_fProfil";	
			this.tab.childPage[2].frameClass = "app_frontend_kopeg_fToko";						
			this.tab.childPage[4].frameClass = "app_frontend_kopeg_fUsaha";			
			this.tab.childPage[5].frameClass = "app_frontend_kopeg_fPinjaman";			
			this.tab.childPage[6].frameClass = "app_frontend_kopeg_fUpkp";			
			this.tab.childPage[7].frameClass = "app_frontend_kopeg_fWisma";			
			this.tab.childPage[8].frameClass = "app_frontend_kopeg_fKarir";			
			this.tab.childPage[9].frameClass = "app_frontend_kopeg_fForum";										
			this.tab.childPage[10].frameClass = "app_frontend_kopeg_fDownload";					
	...
	createListData: function(){
		try{		
			uses("app_frontend_kopeg_fListData");
			this.listDataForm = new app_frontend_kopeg_fListData(this.app);		
					...
					if (this.frame.get(sender.childPage[id].frameClass) === undefined || !this.app.ceklogin){						
						if (this.app.ceklogin){
							if (this.app.statuslog == "Customer")
								sender.childPage[id].frameClass = "app_frontend_kopeg_fCustMenu";
							else sender.childPage[id].frameClass = "app_frontend_kopeg_fSalesMenu";
							this.frame.set(sender.childPage[id].frameClass,true);			
						}else{
							this.frame.set("app_frontend_kopeg_fCustMenu",undefined);			
							this.frame.set("app_frontend_kopeg_fSalesMenu",undefined);			
							sender.childPage[id].frameClass = "app_frontend_kopeg_fLogin";							
						}							
