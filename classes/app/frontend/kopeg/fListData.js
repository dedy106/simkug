...
//* 			SAI, PT											
//***********************************************************************************************
window.app_frontend_kopeg_fListData = function(owner){
	try{
		if (owner){
			window.app_frontend_kopeg_fListData.prototype.parent.constructor.call(this, owner);
			this.className = "app_frontend_kopeg_fListData";
			this.items = [];
			this.items2 = [];						
			this.isClick = false;
			this.mouseX = 0;
...
window.app_frontend_kopeg_fListData.extend(window.portalui_commonForm);
window.app_frontend_kopeg_fListData.implement({
	...
	setLabels: function(data){
		...
		for (var i in data){			
			...
			script += "this.e"+i+".onKeyDown.set(this, 'doKeyDown');";
			eval(script);
			top += 23;
		}		
	...
	doSysMouseMove: function(x, y, button, buttonState){
		window.app_frontend_kopeg_fListData.prototype.parent.doSysMouseMove.call(this,x, y, button, buttonState);
		if (this.isClick)
		{		
	...
	doClick: function(sender){
		try{
			...
			if (sender == this.b1)
			{
				...
				if ((this.requester.className == "portalui_saiSG" || this.requester.className == "portalui_saiGrid") && (!this.isFilter)){
	...
	doKeyDown: function(sender, keyCode, buttonState){
		if (keyCode == 13)
			this.b3.click();
	},
	...
	doFind: function(sender){
		try{
			...
			this.scriptSql = this.basicScript +" "+this.filter +" order by "+this.fields[0];
			this.scriptSqlCount = this.basicCountScript+" "+this.filter;
			this.sg.clear();
			this.dbLib.listDataObjA(this.scriptSql, 1,this.pager);							
	...
	doSelectedPage: function(sender, page){
		this.sg.clear();
		this.dbLib.listDataObjA(this.scriptSql, page,this.pager);					
