...
window.app_frontend_alpa_fMemberEBayar = function(owner,options){
	try{
		if (owner)
		{
			...
			this.setCaption("Edit Pembayaran");
		}
	}catch(e)
	{
	...
	initComponent: function(){		
		try{
			...
			this.sg1 = new portalui_saiGrid(this.p1,{bound:[1,20,this.p1.width - 4,125],colCount:3,colTitle:["No. Order","Keterangan","Nilai Order"],colWidth:[[2,1,0],[125,375,100]],
                buttonStyle:[[0],[bsEllips]],colFormat:[[2],[cfNilai]],change:[this,"sg1onChange"],ellipsClick:[this,"doFindBtnClick"], rowCount:1});
		...
		try{
			if (sender == this.bGen){
				...
				if (this.sg1.allRowValid){														
					...
					sql.add("delete from portal_bayar_d where no_bayar ='"+this.e0.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
					sql.add("delete from portal_bayar_m where no_bayar ='"+this.e0.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
					...
					for (var i in this.dataDetail)
					{
					   sql.add("update portal_order_m set status_bayar='0' where no_order='"+this.dataDetail[i].no_order+"' and kode_lokasi = '"+this.app._lokasi+"' ");
					}
                    for (var k=0; k < this.sg1.rows.getLength(); k++)
					{						
						sql.add("update portal_order_m set status_bayar='1' where no_order='"+this.sg1.getCell(0,k)+"' and kode_lokasi = '"+this.app._lokasi+"' ");
	...
	FindBtnClick : function(sender){
		if (sender == this.e0)
			this.standarLib.showListData(this.getForm(), "Data Pembayaran",sender,undefined, 
									  ...
									  ["no_bayar","Keterangan"],"and",["No Bukti","Keterangan"]);
	},
			    ...
			    case 0:
					if (sender == this.sg1){
						this.standarLib.ListDataSGFilter(this.getForm(),"Data Order",sender, 
							  ...
							  "select no_order,keterangan from portal_order_m where sales='"+this.app.userlog+"' and status_bayar='0' and status in ('S') and kode_lokasi='"+this.app._lokasi+"' and periode='"+this.ePrd.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ",
						      "select count(*) from portal_order_m where sales='"+this.app.userlog+"' and status_bayar='0' and status in ('S') and kode_lokasi='"+this.app._lokasi+"' and periode='"+this.ePrd.getText()+"' and kode_lokasi ='"+this.app._lokasi+"' ",
			...
			if (col == 0){
				try
				{
					var data = this.dbLib.getDataProvider("select a.no_order,a.keterangan,sum(b.jumlah*b.harga) as total "+
						"from portal_order_m a inner join portal_order_d b on a.no_order=b.no_order and b.kode_lokasi = a.kode_lokasi "+
			...
			if (typeof(data) !== "string"){
				if (data.rs.rows[0] !== undefined){
					...
					var brg = this.dbLib.getDataProvider("select a.no_order,b.keterangan, sum(c.jumlah*c.harga) as total "+
						"from portal_bayar_d a inner join portal_order_m b on a.no_order=b.no_order and b.kode_lokasi = a.kode_lokasi "+
						"inner join portal_order_d c on b.no_order=c.no_order and c.kode_lokasi = a.kode_lokasi "+
						"where a.no_bayar = '"+this.e0.getText()+"' and a.kode_lokasi ='"+this.app._lokasi+"' "+
						"group by b.no_order ",true);
                    if (typeof(brg) !== "string")
					{
						if (brg.rs.rows[0]!==undefined)
						{
						    this.dataDetail = brg.rs.rows;
                            this.sg1.clear();
