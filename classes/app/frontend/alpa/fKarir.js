	...
	doChildItemsClick: function(sender,id,item){
		try{						
			...
			if (sender == this.detail){
				...
				this.p_tmp = new portalui_panel(this.detail,{bound:[1,1,this.detail.width-10,this.detail.height - 65],border:3});
				var desk = new portalui_panel(this.p_tmp,{bound:[20,10,this.p_tmp.width-10,this.p_tmp.height - 65],border:0,color:""});
				var bInput = new portalui_button(this.p_tmp,{bound:[this.p_tmp.width-100,2,80,18],click:[this,"doBtnClick"]});
				if (this.app.karirlogin){					
				...
				if (typeof(data) != "string"){
					if (data.rs.rows[0] != undefined){
						...
						desk.getClientCanvas().style.overflow="auto";
