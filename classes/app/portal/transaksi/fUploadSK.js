...
* 			SAI, PT											
***********************************************************************************************/
window.app_portal_transaksi_fUploadSK = function(owner)
{
	if (owner)
	{
		window.app_portal_transaksi_fUploadSK.prototype.parent.constructor.call(this,owner);
		this.className  = "app_portal_transaksi_fUploadSK";
		this.setTop(60);		
		this.maximize();		
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Upload Jenis Harga", 0);				
		...
		this.p1 = new portalui_panel(this,{bound:[20,23,600,260],caption:"Daftar Barang"});
    	this.sg1 = new portalui_saiGrid(this.p1,{bound:[1,20,598,210],colCount:4,
                        colTitle:["Kode Produk","Nama Produk","Harga","Diskon"],
                        colWidth:[[3,2,1,0],[100,100,250,100]],readOnly:true,colFormat:[[2,3],[cfNilai,cfNilai]]});		
...
window.app_portal_transaksi_fUploadSK.extend(window.portalui_childForm);
window.app_portal_transaksi_fUploadSK.implement({
			...
			case "simpan" :
				if (modalResult == mrOk && (this.standarLib.checkEmptyByTag(this, new Array("0","1"))))
				{
					try
					{
						...
						sql.add("insert into portal_sk_m(no_sk, periode, kode_lokasi, keterangan, flag_aktif,nik_user, tanggal)values('"+this.ed_sk.getText()+"','"+this.ed_periode.getText()+"','"+this.app._lokasi+"','"+this.ed_ket.getText()+"','"+flag+"','"+this.app._userLog+"','"+this.dp_tgl.getDateString()+"')");
						var line,script = "";						
						for (var i in this.dataUpload.rows){
							line = this.dataUpload.rows[i];
							if (script != "") script +=",";
							script += "('"+this.ed_sk.getText()+"','"+this.app._lokasi+"','"+line.kode_produk+"','"+parseFloat(line.harga)+"','"+parseFloat(line.diskon)+"')";
						}
						if (script !="") sql.add("insert into portal_sk_d(no_sk, kode_lokasi, kode_produk, harga, diskon)values " + script);
			...
			case "ubah" :
				if (modalResult == mrOk && (this.standarLib.checkEmptyByTag(this, new Array("0","1"))))
				{
					try
					{
						uses("server_util_arrayList",true);
						sql = new server_util_arrayList();
						sql.add("delete from portal_sk_d where no_sk ='"+this.ed_sk.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
						sql.add("update portal_sk_m set keterangan = '"+this.ed_ket.getText()+"',tanggal='"+this.dp_tgl.getDateString()+"',periode='"+this.ed_periode.getText()+"', flag_aktif = '"+flag+"' where no_sk ='"+this.ed_sk.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
						var line,script = "";						
						for (var i in this.dataUpload.rows){
							line = this.dataUpload.rows[i];
							if (script != "") script +=",";
			...
			case "hapus" :
			   if (modalResult == mrOk && (this.standarLib.checkEmptyByTag(this, new Array("0","1"))))
			   {
					try
					{
						uses("server_util_arrayList",true);
						sql = new server_util_arrayList();
						sql.add("delete from  portal_sk_m where no_sk = '"+this.ed_sk.getText()+"' and kode_lokasi = '"+this.app._lokasi+"'");
						sql.add("delete from  portal_sk_d where no_sk = '"+this.ed_sk.getText()+"' and kode_lokasi = '"+this.app._lokasi+"' ");
						this.dbLib.execArraySQL(sql);						
					}
					catch(e)
	...
	doFindBtnClick: function(sender){
		if (sender == this.ed_sk){
			this.standarLib.showListData(this, "Data Jenis Harga",sender,undefined,
										  ...
										  ["no_sk","keterangan"],"where",["No Jenis Harga","Keterangan"]);
		}
	},
	...
	doGen: function(sender){	
		this.ed_sk.setText(this.standarLib.noBuktiOtomatis(this.dbLib, "portal_sk_m", "no_sk", "SK"+this.ed_periode.getText(),"000000"));	
	},	
        ...
        try{
            ...
            if (typeof data != "string"){
                ...
                for(var i in data.rows){
                    ...
                    this.sg1.appendData([line.kode_produk,line.nama, floatToNilai(line.harga),floatToNilai(line.diskon)]);
                } 
	...
	doChange: function(sender){
		if (sender == this.ed_sk){
			var data = this.dbLib.getDataProvider("select a.periode, a.keterangan,a.tanggal, a.flag_aktif, b.kode_produk, c.nama,b.harga, b.diskon "+
				" from portal_sk_m a inner join portal_sk_d b on b.no_sk = a.no_sk and b.kode_lokasi = a.kode_lokasi "+
				" left outer join portal_produk c on c.kode_produk = b.kode_produk and c.kode_lokasi = a.kode_lokasi "+
				" where a.kode_lokasi = '"+this.app._lokasi+"' and a.no_sk = '"+sender.getText()+"' ",true);
			if (typeof data != "string"){
