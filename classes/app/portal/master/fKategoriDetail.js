window.app_portal_master_fKategoriDetail = function(owner)
{
	if (owner)
	{
		window.app_portal_master_fKategoriDetail.prototype.parent.constructor.call(this, owner);
		window.app_portal_master_fKategoriDetail.prototype.parent.setWidth.call(this, 300);
		window.app_portal_master_fKategoriDetail.prototype.parent.setHeight.call(this, 400);
		this.centerize();
		this.className = "app_portal_master_fKategoriDetail";
		this.mouseX = 0;
		this.mouseY = 0;			
...
window.app_portal_master_fKategoriDetail.extend(window.portalui_commonForm);
window.app_portal_master_fKategoriDetail.prototype.doDraw = function(canvas)
{
...
window.app_portal_master_fKategoriDetail.prototype.doAfterResize = function(event)
{
	this.p1.setWidth(this.width - 40);
...
window.app_portal_master_fKategoriDetail.prototype.eventMouseDown = function(event)
{
	this.mouseX = event.clientX;
    this.mouseY = event.clientY;	
	this.isClick = true;
};
window.app_portal_master_fKategoriDetail.prototype.eventMouseUp = function(event)
{
	this.isClick = false;
};
window.app_portal_master_fKategoriDetail.prototype.eventMouseMove = function(event)
{
	if (this.isClick)
	{
		var x = event.clientX;
...
window.app_portal_master_fKategoriDetail.prototype.doClick = function(sender)
{
	if (sender == this.b1)
...
window.app_portal_master_fKategoriDetail.prototype.doRequestReady = function(sender, methodName, result)
{
};
window.app_portal_master_fKategoriDetail.prototype.findBtnClick = function(sender)
{
};
window.app_portal_master_fKategoriDetail.prototype.setCaption = function(data)
{
	var caption = $(this.getFullId() + "_header");
	if (caption != undefined)
		caption.innerHTML = "<div style='{positon:absolute; left: 2; top : 4; background:url(icon/"+system.getThemes()+"/sai.png) no-repeat;width:16;height:16;}'> </div>"+"<span style='{align:center;position:absolute;left:20; top: 4;"+
			"width:100%; height:100%;color:#ffffff; }'>"+data+"</span>";
};
window.app_portal_master_fKategoriDetail.prototype.setItemParent = function(data)
{
	this.itemParent = data;
};
window.app_portal_master_fKategoriDetail.prototype.setSummaryItems = function(data)
{
	this.e4.clearItem();
	for (var i in data)
	{
