window.app_portal_master_fCustKontak = function(owner)
{
  if (owner)
	{
		window.app_portal_master_fCustKontak.prototype.parent.constructor.call(this,owner);
		this.className  = "app_portal_master_fCustKontak";
		this.setTop(60);
		this.setWidth(1280);
		this.setHeight(550);
		this.itemsValue = new portalui_arrayList();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Data Customer Kontak Portal : Input/Koreksi", 0);	
		...
		this.e0.setCaption("Kode Kontak");		
		...
		{
			alert("[app_portal_master_fCustKontak]->constructor : "+e);
		}
	}
};
window.app_portal_master_fCustKontak.extend(window.portalui_childForm);
window.app_portal_master_fCustKontak.prototype.mainButtonClick = function(sender)
{
	...
	if (sender == this.app._mainForm.bSimpan)
	{
		...
		else if (this.eMail.getText().search("@") == -1) 
...
window.app_portal_master_fCustKontak.prototype.doModalResult = function(event, modalResult)
{
	switch (event)
	{
		case "clear" :
		...
		case "simpan" :
			if (modalResult == mrOk &&(this.standarLib.checkEmptyByTag(this, new Array("0"))))
			{
				try
				{
					...
					sql.add("insert into portal_cust_kontak (kode_cust_kontak, kode_cust, nama, tgl_lahir, email, no_telp, no_fax, alamat, kota, kode_pos, perusahaan, tgl_input) values  "+
							"('"+this.e0.getText()+"','"+this.eKlp.getText()+"','"+this.e1.getText()+"','"+this.dp_lahir.getDateString()+"', "+
							"	'"+this.eEmail.getText()+"','"+this.eNoTelp.getText()+"','"+this.eNoFax.getText()+"','"+this.eAlamat.getText()+"' , "+
							"	'"+this.eKota.getText()+"','"+this.eKodePos.getText()+"','"+this.ePerusahaan.getText()+"','"+(new Date).getDateStr()+"') ");
		...
		case "ubah" :
			...
			{				
					...
					sql.add("update portal_cust_kontak set  "+
							" kode_klp_cust = '"+this.eKlp.getText()+"' "+
							"	, nama = '"+this.ePwd.getText()+"' "+
							...
							" where kode_cust_kontak = '"+this.e0.getText()+"' ");
					this.dbLib.execArraySQL(sql);	
			}
		break;
		case "hapus" :
		   ...
		   {			  
				  uses("server_util_arrayList",true);					
					var sql = new server_util_arrayList();
					sql.add("delete from portal_cust_kontak where kode_cust_kontak='"+this.e0.getText()+"'");
					this.dbLib.execArraySQL(sql);	
					this.standarLib.clearByTag(this, new Array("0"),this.e0);
		   }
		break;
...
window.app_portal_master_fCustKontak.prototype.keyPress = function(sender, charCode, buttonState )
{
};
window.app_portal_master_fCustKontak.prototype.doEditChange = function(sender)
{
	if (this.e0.getText() != "")
	{
		try
		{
			setTipeButton(tbSimpan);
			var data = this.dbLib.runSQL("select a.*, b.nama as nmKlp from portal_cust_kontak a inner join portal_cust b on b.kode_cust = a.kode_cust where a.kode_cust = '"+this.e0.getText()+"'");
			if (data instanceof portalui_arrayMap){
				if (data.get(0) != undefined){
					data = data.get(0);
					this.eKlp.setText(data.get("kode_cust"));
...
window.app_portal_master_fCustKontak.prototype.FindBtnClick = function(sender, event)
{
	try
	{
		if (sender == this.e0)
			this.standarLib.showListData(this, "Data Customer Kontak",this.e0,this.e1, 
										  "select kode_cust_kontak, nama from portal_cust_kontak","select count(*) from portal_cust_kontak",										  
										  ["kode_cust_kontak","nama"],"where",["Kode","Nama"]);
		else if (sender == this.eKlp)
			this.standarLib.showListData(this, "Data Customer",sender,undefined, 
										  "select kode_cust, nama from portal_cust","select count(*) from portal_cust",										  
...
window.app_portal_master_fCustKontak.prototype.doRequestReady = function(sender, methodName, result)
{
