...
//* 			SAI, PT											
//***********************************************************************************************
window.app_portal_fLogin = function(owner){
	if (owner){
		try{		
			window.app_portal_fLogin.prototype.parent.constructor.call(this, owner);
			this.className  = "app_portal_fLogin";			
			this.setBound(0,60,400,200);			
			owner.childFormConfig(this, "mainButtonClick","Portal Login", 1);			
			//------------------------ login data ------------------------	
			...
			systemAPI.alert("[app_portal_fLogin]::oncreate lib : ",e);
		}
	}
};
window.app_portal_fLogin.extend(window.portalui_childForm);
window.app_portal_fLogin.implement({
