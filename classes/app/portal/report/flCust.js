window.app_portal_report_flCust = function(owner)
{
	if (owner)
	{
		window.app_portal_report_flCust.prototype.parent.constructor.call(this,owner);
		this.className = "app_portal_report_flCust";
		this.maximize();
	...
	this.gridLib.SGEditData(this.sg1,0,[0,1,2],["Kode Customer","All",""]);
	this.pager = parseInt(this.app._mainForm.reportNavigator.cb.getText());
};
window.app_portal_report_flCust.extend(window.portalui_childForm);
window.app_portal_report_flCust.implement({
	doEllipseClick: function(sender, col, row){
		try{			
			if (row === 0)
			{
				this.filterRep.ListDataSGFilter(this, "Data Customer",this.sg1, this.sg1.row, this.sg1.col,
									  "select kode_cust,nama from portal_cust where kode_lokasi='"+this.lokasi+"' ",
									  "select count(*) from portal_cust where kode_lokasi='"+this.lokasi+"' ",
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2){
		    	...
		    	this.filter = this.filterRep.filterStr("kode_cust",this.sg1.getCell(1,0),this.sg1.getCell(2,0),this.sg1.getCell(3,0),"where");
				this.showFilter = this.filterRep.showFilter(this.sg1);
				var result  = new portalui_arrayMap();
				var sql = "select kode_cust,nama,date_format(tgl_lahir,'%e/%m/%Y'),email,no_telp,no_fax,alamat,kota,kode_pos,perusahaan "+
						"from portal_cust "+this.filter+
						" order by kode_cust ";
			...
			systemAPI.alert("[app_portal_report_flCust]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>DATA CUSTOMER<br>";
		var d = new Date();
	  ...
	  switch(sender.getName())
	  {
	    ...
	    case "pdfBtn" :      
		  var html = new server_util_arrayList();
			...
			html.add("pdf");			
			html.add(new Date().valueOf()+"_Customer");				
