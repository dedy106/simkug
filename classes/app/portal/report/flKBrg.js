window.app_portal_report_flKBrg = function(owner)
{
	if (owner)
	{
		window.app_portal_report_flKBrg.prototype.parent.constructor.call(this,owner);
		this.className = "app_portal_report_flKBrg";
		this.maximize();
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Laporan Kategori Barang", 2);
...
window.app_portal_report_flKBrg.extend(window.portalui_childForm);
window.app_portal_report_flKBrg.implement({
	...
	mainButtonClick: function(sender){
		try
		{
			if (sender === this.app._mainForm.bClear2){
				...
				var sql = "select kode_kategori,fn_spasi(nama,level_spasi) as nmkat,tipe,kode_induk "+
						"from portal_kategori "+this.filter+
						" order by rowindex ";
				this.scriptSqlCount = "select count(*) "+
						"from portal_kategori "+this.filter;
				this.title = new server_util_arrayList({items:["Kode","Nama","Tipe","Induk"]});
			...
			systemAPI.alert("[app_portal_report_flKBrg]::mainButtonClick:"+e);
		}
	},
	...
	previewReport: function(dthtml){
		var html = "<div align='center' class='judul_laporan' style='{width:100%;font-size:14;font-weight:bold;}'><br><br>"+this.app._namalokasi.toUpperCase()+"<br>DATA KATEGORI BARANG<br>";
		var d = new Date();
