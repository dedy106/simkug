...
//* 			SAI, PT											
//***********************************************************************************************
window.system_fListData2 = function(owner){
	try{
		if (owner){
			window.system_fListData2.prototype.parent.constructor.call(this, owner);
			this.className = "system_fListData2";
			this.items = [];
			this.items2 = [];						
			this.isClick = false;
			this.mouseX = 0;
...
window.system_fListData2.extend(window.commonForm);
window.system_fListData2.implement({
	...
	setLabels: function(data){
		...
		if (data == undefined)
			data = this.fields;		
		var script, top = 20;
		this.e0 = new saiLabelEdit(this);
		this.e0.setBound(10,20,this.width - 20, 20);
		this.e0.setCaption("Search");
		this.e0.setWidth(this.width - 48);
		this.e0.setPlaceHolder(true);
		this.e0.setText('');
		this.e0.onKeyPress.set(this, 'editKeyPress');
		this.e0.onKeyDown.set(this, 'doEditKeyDown');
		top += 23;
		this.sg.setColTitle(data);
		this.sg.refreshLayout();		
		this.pButton.setTop(this.e0.top + this.e0.height + 10);
	...
	show: function(withBlank){
		try{			
			...
			this.page = 1;
			this.pageCount = tmp;
			this.dbLib.getDataProviderPageA(this.basicScript +" order by "+this.fields[0], 1,this.pager);														
	...
	doSysMouseUp: function(x, y, button, buttonState){
		window.system_fListData2.prototype.parent.doSysMouseUp.call(this,x, y, button, buttonState);
		this.isClick = false;
		this.blockElm.style.display = "none";
		this.frameElm.style.display = "";
	},
	doSysMouseMove: function(x, y, button, buttonState){
		window.system_fListData2.prototype.parent.doSysMouseMove.call(this,x, y, button, buttonState);
		if (this.isClick)
		{					
	...
	doClick: function(sender){
		try{
			...
			if (sender == this.b1)
			{
				...
				if (this.requester.className == "saiSearch"){								
					this.requester.setSelectedId(data[0]);
					this.requester.setText(data[1]);
					this.requester.setFocus();
				} else 
				if (this.requester.className == "saiCBBL"){								
				...
				if (this.requester.className == "saiCBB"){								
					this.requester.setText(data[0]);					
					if (this.editNama !== undefined) this.editNama.setText(data[1]);
					this.requester.setFocus();
				} else
				if ((this.requester.className == "saiGrid") && (!this.isFilter)){
				...
				if ((this.requester.className == "saiLabelEdit")||(this.requester.className == "saiEdit")){
					this.requester.setText(data[0]);
					this.requester.setFocus();
					if (this.editNama !== undefined) this.editNama.setText(data[1]);
				}				
				...
				this.requester.setFocus();
				if ((this.requester.className == "saiGrid")){
					this.requester.setFocusCell(this.requester.col,this.requester.row);
				}
            }
	...
	init: function(){
		try{		    
			uses("saiLabelEdit;saiGrid;sgNavigator");
			var step = "init library";		
			step = "create Find btn";	
			this.b3 = new imageButton(this);
			...
			step = "Create Ok btn";	
			this.b1 = new imageButton(this.pButton);
			...
			step = "Create Cancel";	
			this.b2 = new imageButton(this.pButton);
			...
			this.sgNav =  new sgNavigator(this.pButton);
	...
	doKeyDown: function(charCode, buttonState,keyCode,event){   
	    try{
    		if (keyCode == 13 && this.activeControl.className == "saiGrid"){ 
    			this.b1.click();
    			return false;
    		}if (keyCode == 27){
                 this.b2.click();
            ...
            return window.system_fListData2.prototype.parent.doKeyDown.call(this, charCode, buttonState, keyCode,event);
        }catch(e){
            alert(e);
        }
	},
	...
	doFind: function(sender){
		try{
			var child;
			var text = this.e0.getText(); 
			
			var script = "";
			var filter = "";
			for (var i in this.fields){								
				if ( i > 0)
					script += "filter += \"or( lower(\" + this.fields["+i+"] +\") like '%\" + text.toLowerCase() +\"%')\"; ";			
				else
					script += "filter += \"( lower(\" + this.fields["+i+"] +\") like '%\" + text.toLowerCase() +\"%')\";";
			}						
			eval(script);						
			...
			this.sg.clear();
			this.dbLib.getDataProviderPageA(this.scriptSql +" order by "+this.fields[0], 1,this.pager);							
	...
	doSelectedPage: function(sender, page){
		this.sg.clear();
		this.dbLib.getDataProviderPageA(this.scriptSql +" order by "+this.fields[0], page,this.pager);					
