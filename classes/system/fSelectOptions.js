 ...
 * 
 */
window.system_fSelectOptions.extend(window.portalui_commonForm);
window.system_fSelectOptions.implement({
	...
	setLabels: function(data){
		...
		for (var i in data){			
			script = "this.e"+i+" = new portalui_saiLabelEdit(this.p2);";
	...
	showForm : function(withBlank){
		try{			
			...
			}else if (this.requester.className == "portalui_saiCBB" || this.requester.className == "portalui_saiCBBL"){
				var line,data;				
	...
	init: function(){
		try{		    
			...
			this.pSelection = new portalui_panel(this,{bound:[0,0,this.width, this.height - 25]});
			this.p1 = new portalui_panel(this.pSelection,{bound:[0,5,449,45],border:1,caption:"Selection"});					
			this.pButton = new portalui_panel(this.pSelection,{bound:[0,0,449,24],border:0,});									
			this.sg = new portalui_saiGrid(this.pSelection,{bound:[1,27,447,413],colCount:3,colTitle:["Type","Value1","Value2"],colWidth:[[2,1,0],[150,150,80]],
				rowCount:1, buttonStyle:[[0,1,2],[bsAuto, bsEllips, bsEllips]], ellipsClick:[this,"doEllipseClick"],selectCell:[this,"doSelectCell"]});
			this.sg.columns.get(0).setPicklist(new portalui_arrayMap({items:["=","Range","like"]}));//"<=","<",">",">=",
			this.b1 = new portalui_imageButton(this.pButton,{bound:[10,2,22,22],click:[this,"doClick"],hint:"Ok",image:"icon/"+system.getThemes()+"/bOk.png"});
			this.b2 = new portalui_imageButton(this.pButton,{bound:[31,2,22,22],click:[this,"doClick"],hint:"Cancel",image:"icon/"+system.getThemes()+"/bCancel.png"});
			this.sgNav =  new portalui_sgNavigator(this.pButton,{bound:[53,0,this.pButton.width - 55,24],grid:this.sg,border:0,pager:[this,"doSelectedPage"], buttonStyle:bsTrans});			
		    this.pSelection.setTabChildIndex();	
			
			uses("portalui_saiLabelEdit;portalui_saiGrid;portalui_sgNavigator");			
			this.pListData = new portalui_panel(this,{bound:[0,0,this.width, this.height - 25],visible:false});
			this.p2 = new portalui_panel(this.pListData,{bound:[0,5,449,45],border:1,caption:"Search"});							     	
			this.b32 = new portalui_imageButton(this.p2,{bound:[413,20,22,22],click:[this,"doFind"],image:"icon/"+system.getThemes()+"/bOk.png",hint:"Klik untuk cari data"});												
			this.pButton2 = new portalui_panel(this.pListData,{bound:[0,75,449,24],border:0});						
			this.sg2 = new portalui_saiGrid(this.pListData,{bound:[1,100,447,320],dblClick:[this,"doDblClick"],selectCell:[this,"doSelectCell"],
				readOnly:true,rowSelect:true});						
			this.sg2.setAllowAutoAppend(false);			
			this.b12 = new portalui_imageButton(this.pButton2,{bound:[10,2,22,22],click:[this,"doClick"],hint:"Select Row",image:"icon/"+system.getThemes()+"/bOk.png"});						
			this.b22 = new portalui_imageButton(this.pButton2,{bound:[31,2,22,22],click:[this,"doClick"],hint:"Cancel",image:"icon/"+system.getThemes()+"/bCancel.png"});						
			this.sgNav2 =  new portalui_sgNavigator(this.pButton2,{bound:[53,0,297,24],grid:this.sg2,border:0,pager:[this,"doSelectedPage"],buttonStyle:3});			
			this.pStatus2 = new portalui_panel(this.pListData,{bound:[0,this.height - 50,449,25]});							    
			this.eStatus2 = new portalui_label(this.pStatus2,{bound:[20,2,300,19],caption:"Page: ;TotalPage: ;"});		    
	...
	setRequester: function(formReq, req, row, col, isFilter, editNama){
		try {
			...
			if (req.className == "portalui_saiGrid"){
	...
	doRequestReady: function(sender, methodName, result, callObj, connection){		
		try{	
			...
			systemAPI.alert(this,"[doRequestReady]"+e);
		}
	},
	...
	getFilterStr: function(dataSelection){
		var data,tmp,filter = new portalui_arrayMap();
		filter.set("=",[]);
		filter.set("range",new portalui_arrayMap());
		filter.set("like",new portalui_arrayMap());					
