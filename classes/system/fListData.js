...
window.system_fListData.implement({
	...
	setLabels: function(data){
		...
		for (var i in this.p1.childs.objList) 
			if (!(system.getResource(i) instanceof imageButton))
				system.getResource(i).free();
		for (var i in data){			
			script = "this.e"+i+" = new saiLabelEdit(this.p1);";
	...
	init: function(){
		try{		    
			...
			this.b3 = new imageButton(this.p1);
	...
	doKeyDown: function(charCode, buttonState,keyCode,event){   
	    try{
            ...
            return window.system_fListData.prototype.parent.doKeyDown.call(this, charCode, buttonState, keyCode,event);
        }catch(e){
            alert(e);
        }
	},
	...
	doFind: function(sender){
		try{
			...
			for (var i in this.p1.childs.objList){
				...
				if (child !== undefined && child instanceof saiLabelEdit && child.getText() != ""){
					text = child.getText();
					break;
				}
			}
