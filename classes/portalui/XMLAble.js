...
//* 			SAI, PT											
//***********************************************************************************************
window.portalui_XMLAble = function(){
    this.className = "portalui_XMLAble";    
    this.serialXML = "";
};
window.portalui_XMLAble.extend(window.Function);
window.XMLAble = window.portalui_XMLAble;
window.portalui_XMLAble.implement({
	...
	serialize: function(name, defaultType){
	    try{
	        ...
	        alert("[portalui_XMLAble] :: serialize : " + e);
	    }
	},
...
window.xmlToObj = function(xml){	
    try{
		...
		if ((type != undefined) && (type != ""))
		{
	        uses(type);		
	        eval("try { result = new " + type + "();} catch (e) { result = undefined; }");
	        if (result instanceof portalui_XMLAble)
