...
//* 			SAI, PT											
//***********************************************************************************************
window.portalui_fileExplorer = function(owner,options){	
    if (owner){
		try{
			this.className = "portalui_fileExplorer";			
			window.portalui_fileExplorer.prototype.parent.constructor.call(this, owner,options);        
			...
			this.showFile = false;
			uses("portalui_fileExplorerItem");
...
window.portalui_fileExplorer.extend(window.portalui_containerControl);
window.fileExplorer = window.portalui_fileExplorer;
window.portalui_fileExplorer.implement({
	...
	doSelectItem: function(item){
	    ...
	    if (oldSelected instanceof portalui_fileExplorerItem)	
	...
	setWidth: function(data){
		try{
		    window.portalui_fileExplorer.prototype.parent.setWidth.call(this, data);
	...
	setHeight: function(data){
	    window.portalui_fileExplorer.prototype.parent.setHeight.call(this, data);
	...
	getMaxExpandLevel: function(node){
		var child = undefined;
		if (node instanceof portalui_fileExplorer)
			this.maxExpandLevel = 0;
		for (var i in node.childs.objList){
			child = window.system.getResource(node.childs.objList[i]);
			if (child instanceof window.portalui_fileExplorerItem)
			{
				if (!child.isHasChild())
				{
	...
	setPath: function(data, caption){
		try{
			...
			this.root = new portalui_fileExplorerItem(this);
				...
				case "listFolder": 
					...
					for (var i in this.usrFolder){			
						...
						if (file != "" && file != "." && file != ".." && file != ".svn"){				
							if (tipe == "d") {
								node = new portalui_fileExplorerItem(this.root);
