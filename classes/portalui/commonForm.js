...
//* 			SAI, PT											
//***********************************************************************************************
window.portalui_commonForm = function(owner){
    if (owner)
    {
        window.portalui_commonForm.prototype.parent.constructor.call(this, owner);
		...
		this.onActivate = new portalui_eventHandler();
    }
};
window.portalui_commonForm.extend(window.portalui_containerControl);
window.commonForm = window.portalui_commonForm;
//---------------------------- Function ----------------------------------------
window.portalui_commonForm.implement({
	doDraw: function(canvas){		
		canvas.style.display = "none";
		canvas.innerHTML = "<div id='" + this.getFullId() + "_form' style='position: absolute; left: 0; top: 0; width: 100%; height: 100%' ></div>"+
	...
	show: function(){
		try
		{
			...
			if (form != this){			
				if (form instanceof portalui_commonForm)
			...
			if (this.activeControl == undefined)
			{
				...
				for (var i in this.childs.objList)
				{
					...
					if ((ctrl instanceof portalui_control) && (ctrl.isTabStop()))
					{
	...
	block: function(message){
	    ...
	    if (node != undefined){
	        ...
	        node.style.display = "";
	        node.innerHTML = "<span style='position:absolute;left:0;top:0;width:auto;height:auto;color:#ffff00;font-style:italic'>"+message+"</span>";	        
	        var textNode = node.firstChild;
	        textNode.style.left = this.width / 2 - parseInt(textNode.offsetWidth) / 2;
	        textNode.style.top = this.height / 2 - parseInt(textNode.offsetHeight) / 2;
        }
		this.blocked = true;
	},
	unblock: function(){
		...
		try{
			...
			{				
				...
				for (var i in this.app.childs.objList){													
					...
					if (this != child && child instanceof portalui_commonForm)
						child.hide();					
				}
			}
		}catch(e){
	...
	doKeyDown: function(charCode, buttonState, keyCode,event){					
		...
		}else if (this.activeControl instanceof portalui_control)
		...
		try
		{
			if (control != this.activeControl)
			{
				...
				if (this.activeControl instanceof window.portalui_control)
					...
					if (this.activeChildForm.activeControl instanceof window.portalui_control)
