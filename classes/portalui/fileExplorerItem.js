...
//* 			SAI, PT											
//***********************************************************************************************
window.portalui_fileExplorerItem = function(owner){
    if (owner){
        this.owner = owner;
		if (owner instanceof portalui_fileExplorer)		
			this.level = 1;
		else if (owner instanceof portalui_fileExplorerItem)
			this.level = owner.getLevel() + 1;
		this.leftFrame = 0;
		this.childIndex = owner.childs.getLength() + 1;
		this.className = "portalui_fileExplorerItem";
		window.portalui_fileExplorerItem.prototype.parent.constructor.call(this, owner);        
...
window.portalui_fileExplorerItem.extend(window.portalui_containerControl);
window.fileExplorerItem = window.portalui_fileExplorerItem;
window.portalui_fileExplorerItem.implement({
	...
	addChild: function(child){
	    if (child instanceof portalui_fileExplorerItem){
	        window.portalui_fileExplorerItem.prototype.parent.addChild.call(this, child);        
	...
	delChild: function(child){
	    if (child instanceof portalui_fileExplorerItem){
	        window.portalui_fileExplorerItem.prototype.parent.delChild.call(this, child);
	        var node = $(this.getFullId() + "_collapse");        			
	        if (node != undefined){
	...
	eventMouseDown: function(event){
	    try{			
			...
			if (this.app!= undefined && tree instanceof portalui_control) this.app.activeForm.setActiveControl(tree);
			if (this.isRightClick){
				window.portalui_fileExplorerItem.prototype.parent.eventMouseUp.call(this, event);	
			}
		}catch(e){
			alert(e);
		}
	...
	eventMouseUp: function(event){
		...
		if (this.clicked){
			...
			if ((selNode.getKode() != this.getKode())){			
				if (selNode.owner instanceof portalui_fileExplorerItem)
	...
	getTree: function(){
	    ...
	    while ((result != undefined) && !(result instanceof portalui_fileExplorer)){
	...
	getTreeView: function(){
		...
		while (!(parent instanceof portalui_fileExplorer))	
			parent = parent.owner;
		return parent;
	},
	...
	getRealPath: function(){	
		...
		while ((result != undefined) && !(result instanceof portalui_fileExplorer) && (result.getFolderName() !== undefined))
		{
			...
			if (sender == this.file && this == callObj){
				switch(methodName){
					...
					case "listFolder": 									    
						...
						for (var i in folder){
							file = folder[i];
							node = new portalui_fileExplorerItem(this);
