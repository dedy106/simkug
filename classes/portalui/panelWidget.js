...
//* 			SAI, PT											
//***********************************************************************************************
window.portalui_panelWidget = function(owner, options){
    if (owner)
    {
		this.caption = "";
		window.portalui_panelWidget.prototype.parent.constructor.call(this, owner, options);		
		this.className = "portalui_panelWidget";
		this.owner = owner;
...
window.portalui_panelWidget.extend(window.portalui_containerControl);
window.panelWidget= window.portalui_panelWidget;
//---------------------------- Function ----------------------------------------
window.portalui_panelWidget.implement({
	draw: function(canvas){
		window.portalui_panelWidget.prototype.parent.draw.call(this, canvas);
	...
	rearrangeWidgets: function(){
		try{
			var child, maxColumn = this.maxColumn;
			var childLength = this.childs.getLength();
			if (childLength <= maxColumn) {
				maxColumn = childLength;
				var childHeight = this.height - 30;
			}else var childHeight = Math.round((this.height - 40) / 2);
			var childWidth = Math.round(this.width / maxColumn) - 10;		
			var left = 5, top = 20, width = 5;			
			for (var i in this.childs.objList){			
				...
				left += childWidth + 10;
				if (width + childWidth + 10 <= this.width) 
					width += childWidth + 10;
				else {
					top += childHeight + 10;
					left = 5;
	...
	setCaption: function(data){
		...
		if (this.caption != ""){
			...
			if (n != undefined){				
				...
				//n.style.left = 10;
				//n.style.width = wdth;
				n.innerHTML = "<div style='position:absolute;top:3;width:100%; height:100%;text-align:center'><bold>"+data+" </bold></div>";
			}
	...
	setHeight: function(data){
		window.portalui_panelWidget.prototype.parent.setHeight.call(this, data);		
	},
	unblock: function(){
	    var node = $(this.getFullId() + "_block");
	    if (node != undefined)
