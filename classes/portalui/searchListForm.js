...
//* 			SAI, PT											
//***********************************************************************************************
window.portalui_searchListForm= function(owner)
{
	if (owner)
	{
		this.itemWidth = 200;
		window.portalui_searchListForm.prototype.parent.constructor.call(this, owner);
		this.className = "portalui_searchList";		
		this.color = "#F0F9EF";
		this.className = "searchListForm";
...
window.portalui_searchListForm.extend(window.portalui_commonForm);
window.searchListForm = window.portalui_searchListForm;
window.portalui_searchListForm.implement({
	...
	setItems: function(items){
		try{
			...
			for (var i in items.objList)
			{
				...
				item = new Array(resId, i, realItem);
				...
				node.innerHTML =    "<div id='" + n + "_caption' style='position: absolute; left: 2; top: 3; width:100%; height: 100%;white-space: nowrap'>" + i +":"+realItem + "</div>" +
									"<div style='cursor: pointer; background: rgba(255,255,255); position: absolute; left: 0; top: 0; width: 100%; height: 100%;' " +
	...
	eventMouseDown: function(event, itemId){
	    ...
	    if (item != undefined){
	        ...
	        this.onSelect.call(this, item[1], item[2],item[0]);
	    }
	},
	eventDblClick: function(event, itemId){
		...
		if (this.linkCtrl) {
			this.linkCtrl.setFocus();
			if (this.linkCtrl.className == 'saiSearch'){
				this.linkCtrl.setSelectedId(this.selectedItem[1]);
				this.linkCtrl.setText(this.selectedItem[2]);
			}
		}
	},
	setWidth: function(data){
		try{		
			...
			alert("[portalui_searchList]::setWidth:"+e+"\r\n"+step);
		}
	},
	setHeight: function(data){
		window.portalui_searchListForm.prototype.parent.setHeight.call(this, data);
	 //   if (document.all)
	    {
	...
	doLostFocus: function(){
		window.portalui_searchListForm.prototype.parent.doLostFocus.call(this);
