...
//* 			SAI, PT											
//***********************************************************************************************
window.portalui_application = function(owner,options){
    if (owner)
    {				
        window.portalui_application.prototype.parent.constructor.call(this, owner);        
        this.oneInstance = false;        
        this.className = "portalui_application";	
    	...
    	this.onTerminate = new portalui_eventHandler();
    	uses("portalui_commonForm");
...
window.portalui_application.extend(window.portalui_containerComponent);
window.application = window.portalui_application;
//---------------------------- Function ----------------------------------------
window.portalui_application.implement({
	addChild: function(child){
		try{			
			if (child instanceof portalui_commonForm){
	...
	terminate: function(){        
	    try{
    		if (window.desktop !== undefined && this.className !== "desktop_app") window.desktop.removeTask(this);
	...
	run: function(){
		try{
			...
			{
				var form = $$(firstForm);				
				if (form instanceof portalui_commonForm || form.className == "app_builder_component_controls_commonForm")
				{
	...
	confirm: function(requester,event, msg, info){
	    ...
	    try
		{
			...
			var r = confirm(msg);			
			if (requester instanceof portalui_childForm)
				requester.doModalResult(event, ( r ? 1 : 2));				
			else if (requester instanceof portalui_commonForm && requester.doModalResult)
					requester.doModalResult(event, ( r ? 1 : 2));
		}	
	...
	block: function(modalForm){
		try{
			...
			for (var i in this.childs.objList){
				...
				if ((form instanceof portalui_commonForm) && (form != modalForm)) form.block();
			}
		}catch(e){
	...
	unblock: function(){
		try{
			...
			for (var i in this.childs.objList){
				form = $$(this.childs.objList[i]);		
				if (form instanceof portalui_commonForm) form.unblock();
			}
		}catch(e){
	...
	doThemesChange: function(themeName){
	    ...
	    for (var i in this.childs.objList){
	        ...
	        if (child instanceof portalui_control) child.doThemesChange(themeName);	    
		}
	},
	...
	doKeyDown: function(charCode, buttonState, keyCode, event){	
		...
		{	if (event == undefined) return false;		
			if ((this.activeForm instanceof portalui_commonForm))// 
			{		
                ...
                if (keyCode == 32){  
                    if (this.activeForm.activeControl && (this.activeForm.activeControl.className == "portalui_checkBox" || this.activeForm.activeControl.className == "portalui_radioButton" || this.activeForm.activeControl.className == "portalui_saiGrid")){                        
	...
	doKeyUp: function(keyCode, buttonState){		
		...
		else if (this.activeForm instanceof portalui_commonForm)
	...
	doActivate: function(oldApplication){
		try{		
			this.active = true;		
			if (this.activeForm instanceof portalui_commonForm && this.activeForm.className != "portalui_MenuForm"){
	...
	setActiveForm: function(form){
		try{		
			if (form != this.activeForm && form.className != "portalui_MenuForm")
			{    
				var oldForm = this.activeForm;
				this.activeForm = form;			
				if (oldForm instanceof portalui_commonForm)
	...
	doSystemPaste: function(str){
		...
		else if (this.activeForm instanceof portalui_commonForm && this.activeForm.activeChildForm.activeControl)
