...
//* 			SAI, PT											
//***********************************************************************************************
window.desktop_app = function(owner){
	window.desktop_app.prototype.parent.constructor.call(this, owner);
	this.className = "desktop_app";
	this.setTitle("roojax Desk");
	this.onTerminate.set(this,"doTerminate");
	this.form = undefined;
	this.showForm();
};
window.desktop_app.extend(window.portalui_application);
window.desktop_app.implement({
	showForm: function(){
		try
		{
			var step = "load";			
			uses("desktop_fMain",true);							
			this._mainForm = new desktop_fMain(this);	
