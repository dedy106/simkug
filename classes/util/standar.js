...
window.util_standar.implement({
	...
	doCekPeriode : function(dbLib, modul, lokasi, status, periode) {
		var perValid = 0;
		if ( status == "A") var strSQL = "select modul from periode_aktif where kode_lokasi ='"+lokasi+"'  and modul ='"+modul+"' and '"+periode+"' between per_awal2 and per_akhir2";
		else var strSQL = "select modul from periode_aktif where kode_lokasi ='"+lokasi+"'  and modul ='"+modul+"' and '"+periode+"' between per_awal1 and per_akhir1";
		
		var data = dbLib.getDataProvider(strSQL,true);
		if (typeof data == "object"){
			var line = data.rs.rows[0];							
			if (line != undefined) perValid = 1;													
		}
		return perValid;				
	},	
	...
	strToArray: function(str){
		try{
			...
			var result = new portalui_arrayList();
			if (str != "")
			{
				var lines = str.split("\r\n");
				var line = undefined;	
		...
		try{
			...
			for (var i in container.childs.objList)
			{
				...
				for (var j = 0; j < tag.length;j++)
				{
					...
					{					  
						if ((comp.className == "portalui_saiLabelEdit") || (comp.className == "portalui_saiEdit") || (comp.className == "portalui_saiCBBL") || (comp.className == "portalui_saiCB") || (comp.className == "portalui_saiMemo") || (comp.className == "portalui_saiCBB")) 
						{
							comp.setText("");
							if (comp.className == "portalui_saiCBBL")
								comp.setRightLabelCaption("");
							if (comp == focusControl) comp.setFocus();		
							if ((comp.className == "portalui_saiLabelEdit") || (comp.className == "portalui_saiEdit") || (comp.className == "portalui_saiCBB"))
							{ 
						...
						}else if ((comp.className == "portalui_saiSG")|| (comp.className == "portalui_saiGrid"))
						{
							comp.clear();
						}else if ( comp.className == "portalui_tinymceCtrl"){
							comp.setCode("");
						}
						break;
					}
				}
				if ((comp instanceof portalui_containerControl))
				{
					...
					{					  
						if ((comp.className == "portalui_saiLabelEdit") || (comp.className == "portalui_saiEdit") || (comp.className == "portalui_saiCBBL") || (comp.className == "portalui_saiCB") || (comp.className == "portalui_saiMemo")  || (comp.className == "portalui_saiCBB"))
						{
				            ...
				            {															
				              ...
				              comp.setFocus();
				              if (!(comp.className == "portalui_saiEdit"))                
						...
						}else if ((comp.className == "portalui_saiSG")||(comp.className == "portalui_saiGrid")||(comp.className == "portalui_treeGrid")||(comp.className == "portalui_stringGrid")||(comp.className == "portalui_mdGrid"))
						{						  
				            /*
