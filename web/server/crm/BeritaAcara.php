<?php    
    ...
    function getDatatable(){
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'],$_SESSION['userPwd'])){
            ...
            if($jenis == null OR $jenis == 'new'){
                $query .= "select d.no_bukti,d.no_ref, convert(varchar(10),d.tanggal,105) as tanggal, d.keterangan, b.nama as nama_cust, c.nama as nama_produk, d.nilai,
                k.nama as nama_karyawan,d.inves,d.margin 
                from sai_proses01 a
                inner join sai_proses04 d on a.no_bukti=d.no_ref
                ...
                where progress='SA04' and d.status=2 and a.kode_lokasi = '".$kode_lokasi."'";
                $jenis = 'new';
                ...
                $query .= "select b.no_bukti, convert(varchar(10),b.tanggal,105) as tanggal,b.no_ref, b.keterangan, c.nama as nama_cust, d.nama as nama_produk, b.nilai,k.nama as nama_karyawan,b.inves,b.margin 
                from sai_proses01 a
                inner join sai_proses05 b on a.no_bukti=b.no_ref and a.kode_lokasi=b.kode_lokasi
                ...
                where a.progress='SA05' and b.status=0  and b.flag_keep !='0' and a.kode_lokasi ='".$kode_lokasi."'";
                $jenis = 'keep';
                ...
                inner join sai_produk d on a.kode_produk=d.kode_produk 
                inner join sai_karyawan k on b.nik=k.nik
                where a.progress='SA05' and b.status=2 and a.kode_lokasi = '".$kode_lokasi."'";
                $jenis = 'finish';
                $column_array = array('b.no_bukti','b.no_ref','convert(varchar(10),b.tanggal,105)','b.keterangan','b.nilai', 'c.nama','d.nama','k.nama','b.inves','b.margin');
                $order_column = 'order by b.no_bukti '.$data['order'][0]['dir'];
                $default_order = 'order by b.no_bukti ';
    ...
    function getJenisDok(){
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'],$_SESSION['userPwd'])){
            ...
            $sql = "select kode_dok,nama from sai_dok where kode_proses='SA05' and kode_lokasi='11' ";
            $rs = execute($sql);
    ...
    function insertBeritaAcara() {
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'],$_SESSION['userPwd'])){
            ...
            $exec_array=array();
            if($data["no_bukti"] == ""){
                $no_bukti = qstr(generateKode("sai_proses05", "no_bukti", "S05-", "0001"));
                $edit = false;
                if($data['status']=='0'){
                    $flag_keep='1';
                }else{
                ...
                if($data['status']=='0'){
                    ...
                    $upd1 = "0";
                    $no_bukti = qstr(generateKode("sai_proses05", "no_bukti", "S05-", "0001"));
                    $update1 = "update sai_proses05 set flag_keep ='$upd1' where no_bukti='".$data['no_bukti']."' and kode_lokasi=$kode_lokasi ";
            ...
            if(arrayKeyCheck(array('kode_dok'), $data) AND arrayKeyCheck(array('file'), $_FILES) AND !empty($_FILES['file']['tmp_name'])){
                ...
                if($status == '1'){
                    $update1 = "update sai_proses01 set progress='SA04' where no_bukti='".$data['no_ref']."' and kode_lokasi =$kode_lokasi ";
                    array_push($exec_array,$update1);

                    $update2 = "update sai_proses04 set status='0' where no_bukti='".$data['no_ref']."' and kode_lokasi =$kode_lokasi ";
                    array_push($exec_array,$update2);
                }
                
                if($edit){
                    $del1 = "DELETE FROM sai_proses05 where no_bukti=$no_bukti and kode_lokasi=$kode_lokasi "; 
                    array_push($exec_array,$del1);
                }
                
                $master = "insert into sai_proses05 (no_bukti,kode_lokasi,status,tanggal,keterangan,nik,nilai,flag_keep,no_ref,tgl_input,nik_input,inves,margin) values ($no_bukti,$kode_lokasi,'$status','".reverseDate($data["tanggal"])."','".$data["keterangan"]."','".$data["nik"]."',".joinNum($data["nilai"]).",'$flag_keep','".$data["no_ref"]."',getdate(),'".$_SESSION['userLog']."',".joinNum($data["inves"]).",".joinNum($data["margin"]).") ";
                array_push($exec_array,$master);
                
                if(arrayKeyCheck(array('kode_dok'), $data) AND arrayKeyCheck(array('file'), $_FILES) AND !empty($_FILES['file']['tmp_name'])){
                    $del2 = "DELETE FROM sai_proses_dok where no_bukti=$no_bukti and kode_proses = 'SA05' and kode_lokasi=$kode_lokasi ";
                    array_push($exec_array,$del2);
                    
                    for($i=0; $i<count($data['kode_dok']);$i++){
                        $insdet[$i] = "insert into sai_proses_dok (kode_lokasi,no_bukti,kode_proses,kode_dok,path_file) values ($kode_lokasi,$no_bukti,'SA05','".$data['kode_dok'][$i]."','".$arr_nama[$i]."')"; 
                ...
                if($status != '1'){
                    $update3 = "update sai_proses01 set progress='SA05' where no_bukti='".$data['no_ref']."' and kode_lokasi =$kode_lokasi ";
