<?php
    ...
    function getData(){
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'])){            
            ...
            $query .= "select a.id_lahan,a.nama_lahan,a.alamat,isnull(b.jum_dok,'0') as jum_upload 
            from amu_lahan a 
            left join (select no_bukti,kode_lokasi,count(no_bukti) as jum_dok
                        from amu_lahan_dok
                        group by no_bukti,kode_lokasi 
						) b on a.id_lahan=b.no_bukti and a.kode_lokasi=b.kode_lokasi 
            where a.kode_lokasi= '$kode_lokasi' ";
            
            $column_array = array('a.id_lahan','a.nama_lahan','a.alamat',"isnull(b.jum_dok,'0')");
            $order_column = 'ORDER BY a.id_lahan '.$data['order'][0]['dir'];
            ...
            {
                $sub_array = array();
                $sub_array[] = $row->id_lahan;
                $sub_array[] = $row->nama_lahan;
    ...
    function simpanDok(){
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'])){            
            ...
            try{
                ...
                $url2 = "https://api.simkug.com/api/aset/upload-dok-lahan";
    ...
    function getJenis() {
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'])){             
            ...
            from amu_dok_jenis 
            where kode_lokasi='$kode_lokasi' and kode_klp='K01' ";
            $res = dbResultArray($sql);
            $response["daftar"] = $res;
            $response["message"] = "Success";
        } else{
    ...
    function hapusDok() {
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'])){             
            ...
            $url = "https://api.simkug.com/api/aset/login";
            $url2 = "https://api.simkug.com/api/aset/delete-dok-lahan/$no_bukti/$no_urut";
