<?php
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'])){             
            ...
            if($uploadOk){
                ...
                for ($i=2; $i<=$jumlah_baris; $i++){
                    ...
                    $tgl_invoice   = $data->val($i, 4);
                    $no_invoice   = $data->val($i, 5);
                    $no_fpajak   = $data->val($i, 6);
                    $tgl_fpajak   = $data->val($i, 7);
                    $kredit   = floatval($data->val($i, 8));
                    $harga   = floatval($data->val($i, 9));
                    $diskon   = floatval($data->val($i, 10));
                    $hrg_diskon   = floatval($data->val($i, 11));
                    $ppn   = floatval($data->val($i, 12));
                    $harga_ppn   = floatval($data->val($i, 13));
                    $tgl_jth_tempo  = $data->val($i, 14);
                    $jenis_trans   = $data->val($i, 15);
                    ...
                    if(!cekKodePP($kode_pp,$kode_lokasi)){
                    ...
                    else if(!cekTglInvoice($tgl_invoice,$periode)){
                        $err_ins[] = array("no_invoice" =>"Invalid tgl invoice","err_msg"=>"Error Tanggal $tgl_invoice tidak sesuai periode yg dipilih ");
                    }
                        ...
                        // if($no_invoice != $data->val($i-1, 5)){
                            $sql[$i] = "INSERT into  tm_hutang_tmp (kode_lokasi,periode,kode_pbf,nama,kode_pp,tgl_invoice,no_invoice,no_fpajak,tgl_pajak,kredit,harga,diskon,hrg_diskon,ppn,harga_ppn,tgl_jth_tempo,jenis_trans) values ('$kode_lokasi','$periode','$kode_pbf','$nama','$kode_pp','$tgl_invoice','$no_invoice','$no_fpajak','$tgl_fpajak',$kredit,$harga,$diskon,$hrg_diskon,$ppn,$harga_ppn,'$tgl_jth_tempo','$jenis_trans')";
    ...
    function getHutTmp(){
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'])){            
            ...
            $kode_lokasi = $data['kode_lokasi'];
            $query .= "select kode_pbf,nama,kode_pp,tgl_invoice,no_invoice,no_fpajak,tgl_pajak,kredit,harga,diskon,hrg_diskon,ppn,harga_ppn,tgl_jth_tempo,jenis_trans from tm_hutang_tmp
    ...
    function simpanHut() {
        ...
        if($_SESSION['isLogedIn'] AND cekAuth($_SESSION['userLog'])){             
            ...
            $row = dbResultArray("select * from tm_hutang_tmp where kode_lokasi='$kode_lokasi' and periode='$periode' ");
                ...
                if(floatval($row[$i]["harga"]) - floatval($row[$i]['diskon']) == floatval($row[$i]['hrg_diskon'])){
                    ...
                    if(floatval($row[$i]["hrg_diskon"]) + floatval($row[$i]["ppn"]) == floatval($row[$i]['harga_ppn'])){
                        ...
                        //if(!cekNoInvoice($row[$i]["no_invoice"],$kode_lokasi)){
                            ...
                            $sql = "insert into tm_hutang_m(no_hutang,kode_lokasi,no_dokumen,tanggal,keterangan,kode_jenis,kode_vendor,kode_curr,kurs,nik_app,kode_pp,nilai,periode,nik_user,tgl_input,akun_hutang,posted,nilai_ppn,modul,no_ref,no_fp,no_spb, bruto,diskon,due_date, nilai_pph,tgl_fp,lama,no_load) values ('$no_bukti','$kode_lokasi','".$no_bukti."','".$row[$i]["tgl_invoice"]."','".$row[$i]["no_fpajak"]."','".$row[$i]["jenis_trans"]."','".$row[$i]["kode_pbf"]."','IDR',1,'".$nik."','".$row[$i]["kode_pp"]."',".$row[$i]["harga_ppn"].",'".$periode."','".$nik."',getdate(),'".$akunKredit."','F',".$row[$i]["ppn"].",'HUTUM','-','".$row[$i]["no_fpajak"]."','-',".$row[$i]["harga"].",".$row[$i]["diskon"].",'".$row[$i]["tgl_jth_tempo"]."',0,'".$row[$i]["tgl_pajak"]."',".$row[$i]["kredit"].",'".$idDok."')";
                            
                            array_push($exec,$sql);
                            
                            $sql2 = "insert into tm_hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,no_load) values ('".$no_bukti."','".$no_bukti."','".$row[$i]["tgl_invoice"]."',0,'".$akunDebet."','".$row[$i]["no_fpajak"]."' ,'D','IDR',1,".$row[$i]["hrg_diskon"].",".joinNUm($row[$i]["hrg_diskon"]).",'".$row[$i]["kode_pp"]."','-','".$kode_lokasi."','HUTUM','BEBAN','".$periode."','".$nik."',getdate(),'".$idDok."')";
                           
                            array_push($exec,$sql2);
                            
                            $sql3 = "insert into tm_hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,no_load) values ('".$no_bukti."','".$no_bukti."','".$row[$i]["tgl_invoice"]."',2,'".$akunKredit."','".$row[$i]["no_fpajak"]."','C','IDR',1,".$row[$i]["harga_ppn"].",".$row[$i]["harga_ppn"].",'".$row[$i]["kode_pp"]."','-','".$kode_lokasi."','HUTUM','HUT','".$periode."','".$nik."',getdate(),'".$idDok."')";
                           
                            array_push($exec,$sql3);
                            
                            $sql4 = "insert into tm_hutang_j(no_hutang,no_dokumen,tanggal,no_urut,kode_akun,keterangan,dc,kode_curr,kurs,nilai_curr,nilai,kode_pp,kode_drk,kode_lokasi,modul,jenis,periode,nik_user,tgl_input,no_load) values ('".$no_bukti."','".$no_bukti."','".$row[$i]["tgl_invoice"]."',1,'".$akunPPN."','".$row[$i]["no_fpajak"]."','D','IDR',1,".$row[$i]["ppn"].",".$row[$i]["ppn"].",'".$row[$i]["kode_pp"]."','-','".$kode_lokasi."','HUTUM','PPN','".$periode."','".$nik."',getdate(),'".$idDok."')";
