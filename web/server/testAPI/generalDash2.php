<?php
    ...
    //         // if(authKey2($data["api_key"], 'testAPI', $data['username'])){ 
    //             $sql = "select top 1 * from db_grafik_m ";
    //             $rs = execute($sql);
    //             $response ['hasil']= $rs->RecordCount();
    ...
    function getKas() {
        ...
        if(arrayKeyCheck(array('api_key', 'username', 'kode_lokasi','periode'), $data)){
            if(authKey($data["api_key"], 'testAPI', $data['username'])){ 
                ...
                $myquery="INSERT INTO warga_token (token,username,kode_lokasi) VALUES ('$newToken','$username','99')";
                $res=execute($myquery);
                // $baris = $res->FetchNextObject($toupper=false);
    ...
    function getRecent() {
        ...
        if(arrayKeyCheck(array('api_key', 'username', 'kode_lokasi','periode'), $data)){
            if(authKey($data["api_key"], 'testAPI', $data['username'])){ 
                ...
                from trans_j where kode_lokasi = '32' and periode like '$tahun%' order by tgl_input desc";
                $recent = execute($sql);
                $response["daftar"] = array();
    ...
    // function getDetailBK(){
    //     $data=$_GET;
    //     if(arrayKeyCheck(array('api_key', 'username', 'kode_lokasi'), $_data)){
    //         if(authKey($data["api_key"], 'testAPI', $data['username'])){
    //             $kode_lokasi = $data['kode_lokasi'];  
    //             $periode = $data['periode'];
    //             $kode_akun = $data['kode_akun'];
    ...
    //             case when a.dc='D' then a.nilai else 0 end as debet,case when a.dc='C' then a.nilai else 0 end as kredit,a.kode_drk,a.no_dokumen
    //             from gldt a
    //             where a.kode_lokasi='$kode_lokasi' and a.periode='$periode' and a.kode_akun='$kode_akun' $filtertgl 
    //             order by a.tanggal,a.no_bukti,a.dc";
    //             $rs1 = execute($sql);
    //             $response["daftar2"] = array();
    //             while($row = $rs1->FetchNextObject($toupper = false)){
    ...
    function getArusKas(){
        ...
        if(arrayKeyCheck(array('api_key', 'username', 'kode_lokasi','periode'), $data)){
            if(authKey($data["api_key"], 'testAPI', $data['username'])){ 
                ...
                where a.kode_lokasi='32' and a.periode='201903' and b.kode_flag in ('001','009')
                group by a.kode_lokasi";
                $rs = execute($sql);
                ...
                from (select a.no_bukti,a.kode_lokasi
                    from trans_j a
                    inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
                    where a.kode_lokasi='32' and  b.kode_flag in ('001','009') and a.periode='201903' and a.dc='D'
                ...
                from (select a.no_bukti,a.kode_lokasi
                    from trans_j a
                    inner join flag_relasi b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
                    where a.kode_lokasi='32' and  b.kode_flag in ('001','009') and a.periode='201903'  and a.dc='C'
                    group by a.no_bukti,a.kode_lokasi
                    ) a
                inner join trans_j b on a.no_bukti=b.no_bukti and a.kode_lokasi=b.kode_lokasi 
                inner join flag_relasi c on b.kode_akun=c.kode_akun and b.kode_lokasi=c.kode_lokasi
        ...
        if(arrayKeyCheck(array('api_key', 'username', 'kode_lokasi','periode'), $data)){
            if(authKey($data["api_key"], 'testAPI', $data['username'])){
                ...
                $data = "select b.kode_akun, b.nama,
                               ...
                               where b.jenis = '".$data['jenis']."' and a.periode like '".$data['periode']."' and a.kode_lokasi = '".$data['kode_lokasi']."'
    ...
    function getRealisasiTahunanPen() {
        ...
        $tahunan = "select sum(a.so_akhir*-1) 'so_akhir', sum(isnull(c.nilai,0)) 'nilai', (sum(a.so_akhir*-1)/sum(isnull(c.nilai,0))) 'rasio' 
                   ...
                   inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
                   left join anggaran_d c on a.kode_akun=c.kode_akun and a.kode_lokasi=c.kode_lokasi  
                   where b.jenis = '".$data['jenis']."' and a.periode like '".substr($data['periode'],0,4)."' and a.kode_lokasi = '".$data['kode_lokasi']."'";
        $exe     = execute($tahunan);
        ...
        if(arrayKeyCheck(array('api_key', 'username', 'kode_lokasi','periode'), $data)){
            if(authKey($data["api_key"], 'testAPI', $data['username'])){
                $bulanan = "select isnull(sum(a.so_akhir),0) 'so_akhir', isnull(sum(isnull(c.nilai,0)),0) 'nilai', 
                      ...
                      inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
                      left join anggaran_d c on a.kode_akun=c.kode_akun and a.kode_lokasi=c.kode_lokasi  
                      where b.jenis = '".$data['jenis']."' and a.periode like '".date('Y').date('M')."' and a.kode_lokasi = '".$data['kode_lokasi']."'";
                $exe = execute($bulanan);
            ...
            if(authKey($data["api_key"], 'testAPI', $data['username'])){
                if(!empty($data['per'])){
                    ...
                    $getDataper = "select b.kode_akun, b.nama,
                      ...
                      inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
                      left join anggaran_d c on a.kode_akun=c.kode_akun and a.kode_lokasi=c.kode_lokasi  
                      where b.jenis = '".$data['jenis']."' and a.periode = '".$data['per']."' and a.kode_lokasi = '".$data['kode_lokasi']."'
                      group by b.kode_akun, b.nama";
                    ...
                    $getNilaiper = "select isnull(sum(a.so_akhir*-1),0) 'so_akhir', isnull(sum(isnull(c.nilai,0)),0) 'nilai', 
                      ...
                      inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
                      left join anggaran_d c on a.kode_akun=c.kode_akun and a.kode_lokasi=c.kode_lokasi  
                      where b.jenis = 'Pendapatan' and a.periode = '".$data['per']."' and a.kode_lokasi = '".$data['kode_lokasi']."'";
                ...
                while($row3 = $exe3->FetchNextObject($toupper = false)){
                    ...
                    $response['real_bln_per'] = $row4->so_akhir;
                    $response['angg_bln_per'] = $row4->nilai;
                    $response['persen_bln_per'] = number_format($row4->rasio*100);
                    $persen = number_format($row4->rasio*100);
                    }elseif($data['jenis'] == "Beban"){
    ...
    function getChartLaba() {
         ...
         if(arrayKeyCheck(array('api_key', 'username', 'kode_lokasi','periode'), $data)){
            if(authKey($data["api_key"], 'testAPI', $data['username'])){
                ...
                from exs_neraca a
                inner join db_grafik_d b on a.kode_neraca=b.kode_neraca and a.kode_lokasi=b.kode_lokasi
                where a.kode_lokasi='".$data['kode_lokasi']."' and substring(a.periode,1,4)='".substr($data['periode'],0,4)."' and b.kode_grafik in ('DB02')
                group by a.kode_lokasi";
                ...
                from exs_neraca a
                inner join db_grafik_d b on a.kode_neraca=b.kode_neraca and a.kode_lokasi=b.kode_lokasi
                where a.kode_lokasi='".$data['kode_lokasi']."' and substring(a.periode,1,4)='".substr($data['periode'],0,4)."' and b.kode_grafik in ('DB03')
                group by a.kode_lokasi";
                ...
                from exs_neraca a
                inner join db_grafik_d b on a.kode_neraca=b.kode_neraca and a.kode_lokasi=b.kode_lokasi
                where a.kode_lokasi='".$data['kode_lokasi']."' and substring(a.periode,1,4)='".substr($data['periode'],0,4)."' and b.kode_grafik in ('DB04')
                group by a.kode_lokasi";
         ...
         if(arrayKeyCheck(array('api_key', 'username', 'kode_lokasi','periode'), $data)){
            if(authKey($data["api_key"], 'testAPI', $data['username'])){
                ...
                $beban = "select b.kode_akun, b.nama,
                      ...
                      from exs_glma a 
                      inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
                      where b.jenis = 'Beban' and a.periode = '".$data['per']."' and a.kode_lokasi = '".$data['kode_lokasi']."'
                      group by b.kode_akun, b.nama";
                $exe2 = execute($beban);
                $total_beban = 0;
                while($row2 = $exe2->FetchNextObject($toupper = false)){
                    if($row2->so_akhir !=0){
                        ...
                        $total_beban = $total_beban + $row1->so_akhir;
                    }
                }
                ...
                $pendapatan = "select b.kode_akun, b.nama,
                      ...
                      from exs_glma a 
                      inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
                      where b.jenis = 'Pendapatan' and a.periode = '".$data['periode']."' and a.kode_lokasi = '".$data['kode_lokasi']."'
                      group by b.kode_akun, b.nama";
                $exe1 = execute($pendapatan);
                $total_pendapatan = 0;
                while($row1 = $exe1->FetchNextObject($toupper = false)){
                ...
                $beban = "select b.kode_akun, b.nama,
                      ...
                      from exs_glma a 
                      inner join masakun b on a.kode_akun=b.kode_akun and a.kode_lokasi=b.kode_lokasi
                      where b.jenis = 'Beban' and a.periode = '".$data['periode']."' and a.kode_lokasi = '".$data['kode_lokasi']."'
                      group by b.kode_akun, b.nama";
                $exe2 = execute($beban);
                $total_beban = 0;
                while($row2 = $exe2->FetchNextObject($toupper = false)){
