<?php 
	...
	echo $header;
?>
<div class="row" style='border:none;margin-top: 65px;'>
	<div class="col-md-12 col-sm-12 col-xs-12">
	<h4 style="font-weight: bold; padding-left: 1rem;">Pendapatan</h4>
	<select id="periode" name="periode" class="form-control" style=" margin-left: 1rem; width: 36rem; border-top: none; border-left: none; border-right: none;">
	
	</select>
	<h6 style="padding-left: 1rem; font-weight: bold;">Target Tahunan</h6>
	<div id="progress-tahunan" class="progress" style="width: 37rem; margin-left: 1rem;"></div>
	...
	<div id="progress-bulanan" class="progress" style="width: 37rem; margin-left: 1rem;"></div>
 ...
 <div class="row">
 	...
 	<table style="padding-left: 1rem;" id="tabel-pendapatan">
 		<tbody id="pendapatan-row">
...
$(document).ready(function(){
	$('#periode').on('change', function(){
		var per = $('#periode').val();
		var jenis = 'Pendapatan';
		$.ajax({
			type: 'GET',
			url: '<?=$root_ser?>/generalDash.php?fx=getDataPer',
			dataType: 'json',
			data: {'kode_lokasi':'<?=$kode_lokasi?>','periode':'<?=$periode?>','per':per,'jenis':jenis},
			success: function(response){
				...
				if(response.status){
					...
					$('table#tabel-pendapatan tr.data-pendapatan').remove();
					var html = "";
					...
					html += "<tr class='data-pendapatan'>"+
						    ...
						    "</td>"+
						 	...
						 	"<h5 style='text-align: right; margin-top: -0.5rem;'>"
...
function loadService(index,method,url,param=null) {
	...
	$.ajax({
		...
		url: url,
		dataType: 'json',
		data: {'kode_lokasi':<?=$kode_lokasi?>,'periode':'<?=$periode?>','jenis':jenis},
		success: function(response) {
			if(response.status) {
				switch(index){
					case "nmperus":
...
function initDash() {
	 ...
	 loadService('tahunan','GET','<?=$root_ser?>/generalDash.php?fx=getRealisasiTahunanPen');
	 loadService('bulanan','GET','<?=$root_ser?>/generalDash.php?fx=getRealisasiBulananPen');
}
initDash();
