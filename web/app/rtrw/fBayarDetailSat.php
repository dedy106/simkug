...
if ($version[1] == "iPhone" || $version[1] == "Android" || $version[1] == "Blackberry" || $version[1] == "Blazer" ||$version[1] == "Elaine" || $version[1] == "Hiptop" || $version[1] == "iPod" || $version[1] == "Kindle" ||$version[1] == "Midp" || $version[1] == "Mobile" || $version[1] == "O2" || $version[1] == "Opera Mini" ||$version[1] == "Mobile" || $version[1] == "Smartphone"){
    $back1="";
    $backto="fMainMobile.php?hal=app/rtrw/fBayarIuranSat.php&blok=$blok";
    $mobile=true;
    include('back.php');
    $padding="padding-top:50px;border:0px solid grey;box-shadow: 0 0 0 white;";
}else{
    $back1="<div class='panel-heading'><a href='fMainMobile.php?hal=app/rtrw/fBayarIuranSat.php&blok=$blok' class='small-box-footer' > Back <i class='fa fa-arrow-circle-left'></i></a></div>";
    $padding="";
    $mobile=false;
}
...
<script>
    ...
    function getDetailBayar(){
        $.ajax({
            type: 'POST',
            url: 'include_lib.php?hal=server/rtrw/dash_rtrw.php&fx=getDetailBayarSat',
            dataType: 'json',
    ...
    $(document).ready(function(){
        ...
        $('.form-iuran').on('click','#btnSubmit',function(e){
            e.preventDefault();
            ...
            if(total != 0 && akun != ""){
                $.ajax({
                    type: 'POST',
                    url: 'include_lib.php?hal=server/rtrw/dash_rtrw.php&fx=simpanIuranSat',
                    dataType: 'json',
                    data: myForm,
                    cache: false,
                    success:function(result){
                        ...
                        if(result.status){
                            // location.reload();
                            window.location.href='fMainMobile.php?hal=app/rtrw/fBuktiBayarRwDok.php&no_bukti='+result.no_bukti+'&blok=<?=$blok?>&kode_rumah=<?=$kode_rumah?>&file_back=fBayarDetailSat';
                        }
                    }
                });
                $(this).html("<i class='fa fa-plus-circle'></i> Save").removeAttr('disabled');
            }else{
