...
if ($version[1] == "iPhone" || $version[1] == "Android" || $version[1] == "Blackberry" || $version[1] == "Blazer" ||$version[1] == "Elaine" || $version[1] == "Hiptop" || $version[1] == "iPod" || $version[1] == "Kindle" ||$version[1] == "Midp" || $version[1] == "Mobile" || $version[1] == "O2" || $version[1] == "Opera Mini" ||$version[1] == "Mobile" || $version[1] == "Smartphone"){
    $back1="";
    $backto="fMainMobile.php?hal=app/rtrw/fProsesDok.php&blok=$blok&periode=$periode&kode_akun=$kode_akun&kode_pp=$kode_pp";
    $mobile=true;
    include('back.php');
    $padding="padding-top:50px;border:0px solid grey;box-shadow: 0 0 0 white;";
}else{
    $back1="<div class='panel-heading'><a href='fMainMobile.php?hal=app/rtrw/fProsesDok.php&blok=$blok&periode=$periode&kode_akun=$kode_akun&kode_pp=$kode_pp' class='small-box-footer' > Back <i class='fa fa-arrow-circle-left'></i></a></div>";
    $padding="";
    $mobile=false;
}
                ...
                <div class='row'>
                    <div class='col-xs-12'>
                        ...
                        <div class='box-body'>
                            ...
                            <div class="form-group">
                                <label>No Bukti Upload</label>
                                <input type='text' name='no_bukti_dok' class='form-control' readonly value='<?=$no_bukti?>' >
                            </div>
...
<script>
    ...
    function getDetailBayar(){
        $.ajax({
            ...
            success:function(result){   
                ...
                hitungSaldo();
                if("<?=$periode?>" != ""){
                    $('.sai-grid-row > td > :input[value=<?=$periode?>]').closest('tr').find('.toggle-handle').click();
                }
            }
        });
    }
    ...
    $(document).ready(function(){
        ...
        $('#btnSubmit').click(function(e){
            ...
            var bukti = $('#no_bukti_dok').val();
            if(bukti == ""){
                alert('No Bukti Upload tidak boleh kosong !');
                return false;
            }
            if(total != 0 && akun != ""){
                $.ajax({
                    type: 'POST',
                    url: 'include_lib.php?hal=server/rtrw/dash_rtrw.php&fx=simpanIuranRwDok',
                    dataType: 'json',
                    data: myForm,
                    cache: false,
                    success:function(result){
                        ...
                        if(result.status){
                            window.location.href='fMainMobile.php?hal=app/rtrw/fBuktiBayarRwDok.php&no_bukti='+result.no_bukti+'&blok=<?=$blok?>&kode_rumah=<?=$kode_rumah?>&file_back=fBayarDetailRwBukti';
                        }
                    }
                });
            }else{
