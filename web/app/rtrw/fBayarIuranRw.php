                    ...
                    <div class='col-md-3'>
                        <select class='form-control input-sm selectize' id='dash_blok' style='margin-bottom:5px;'>
                            ...
                            <?php
                                ...
                                $filter = "";
                                $sql = "select blok from rt_blok where kode_lokasi='$kode_lokasi' order by blok ";
                                $res = execute($sql);
...
<script>
    ...
    function getKartuIuran(blok =null){
        $.ajax({
            type: 'POST',
            url: 'include_lib.php?hal=server/rtrw/dash_rtrw.php&fx=getBayarIuranRw',
            dataType: 'json',
            data: {'blok':blok},
            success:function(result){    
                var html=``;
                ...
                if(result.status){
                    ...
                    for (var i=0;i<result.daftar.length;i++)
                    {
                        ...
                        html+=`<tr>
                            ...
                            <td width='2%'><a class='btn btn-sm btn-primary' href='fMainMobile.php?hal=app/rtrw/fBayarDetailRw.php&kode_rumah=`+line.kode_rumah+`&blok=`+blok+`' style='cursor:pointer;'>Bayar</td>
                        </tr>`;
                        x++;
                    }
                }
