...
<script>
    ...
    function getImage(){
        ...
        $.ajax({
            type: 'POST',
            url: 'include_lib.php?hal=server/cms/EditLogo.php&fx=getLogo',
            dataType: 'json',
            data: {'kode_lokasi':'<?php echo $kode_lokasi ?>'},
            success:function(result){    
                if(result.status){
                    if(typeof result.daftar !== 'undefined' && result.daftar.length>0){
                        var urls = path+"image/"+result.daftar[0].logo;
                        var html = "<center>"+
                                    "<img src='"+urls+"' style='width:25%; height:25%; min-width:200px; min-height:200px; display:block; margin-left: auto; margin-right: auto;'>"+
                                    "<br><br>"+
                                    "<center><b>Url:</b> <i>"+urls+"</i></center>"+
                                "</center>";
                        $('#img-logo').html(html)   
                    }
                }
    ...
    $(document).ready(function(){
        ...
        e.preventDefault();
            ...
            $.ajax({
                type: 'POST',
                url: 'include_lib.php?hal=server/cms/EditLogo.php&fx=editLogo',
                dataType: 'json',
                data: formData,
                contentType: false,
                cache: false,
