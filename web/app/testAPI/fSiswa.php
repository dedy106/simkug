...
function getJurusan(){
    $.ajax({
        ...
        //     xhr.setRequestHeader('Authorization', 'Basic ' + btoa('<?=$us?>:<?=$pw?>'));
        // },
...
function getInit(){
    $.ajax({
        type: 'POST',
        url: '<?=$root_ser?>/siswa.php?fx=getAkses',
        dataType: 'json',
        data: {'kode_lokasi':'<?=$kode_lokasi?>','url':'<?=$root_ser?>/siswa.php?fx=get_siswa2'},
        success:function(result){   
                if(typeof result.daftar !== 'undefined' && result.daftar.length>0){
                    var html = "";
                    for(i=0;i<result.daftar.length;i++){
                         html += "<h5>"+result.daftar[i].nim+"</h5><br>";
                    }
                    $('.test').html(html);
                    // alert(html);
                }
...
getInit();
  ...
  $('#saiweb_container').on('click', '.web_datatable_edit', function(){
     ...
     $.ajax({
        ...
        dataType: 'json',
        data: {'kode_lokasi':<?=$kode_lokasi?>,'username':'<?=$nik?>','api_key':'<?=$_SESSION['api_key']?>','nim':kode},
  ...
  $('#saiweb_container').on('submit', '#web_form_edit', function(e){
    ...
    // formData.push({name: "kode_lokasi", value: kode_lok});
    // formData.push({name: "username", value: username});
    // formData.push({name: "api_key", value: api_key});
   ...
   $('#saiweb_container').on('click', '.web_datatable_del', function(){
        if(confirm('Apakah anda ingin menghapus data ini?')){
            ...
            $.ajax({
                ...
                url: '<?=$root_ser?>/siswa.php',
                dataType: 'json',
                data: {'nim':kode, 'kode_lokasi':<?=$kode_lokasi?>,'username':'<?=$nik?>','api_key':'<?=$_SESSION['api_key']?>'},
                success:function(result){
