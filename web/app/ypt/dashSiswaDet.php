			...
			<div class='panel-body'>
                ...
                switch($kunci){
                    ...
                    ";
                        break;
                        case "keu" :
                        // $periodeNow = date('Y').date('m');
                        $sql="select max(a.periode) as periode from ( select max(periode) as periode from periode where kode_lokasi='$kode_lokasi'
                        union all
                        select max(periode) as periode from sis_cd_d where kode_lokasi='$kode_lokasi' and nis='$nik2'
                        union all
                        select max(periode) as periode from sis_bill_d where kode_lokasi='$kode_lokasi' and nis='$nik2' 
                        union all
                        select max(periode) as periode from sis_rekon_d where kode_lokasi='$kode_lokasi' and nis='$nik2'
                        ) a";
                        $p = execute($sql);
                        $periodeNow = $p->fields[0];
                        ...
                        $sql = "select a.nis,a.nama,a.kode_lokasi,a.kode_pp,a.kode_akt,isnull(b.total,0)-isnull(d.total,0)+isnull(c.total,0)-isnull(e.total,0) as sak_total,
                        ...
                        left join (select y.nis,y.kode_lokasi,  
                                    ...
                                    from sis_bill_d x 			
                                    inner join sis_siswa y on x.nis=y.nis and x.kode_lokasi=y.kode_lokasi and x.kode_pp=y.kode_pp
                                    where(x.kode_lokasi = '$kode_lokasi')and(x.periode < '$periodeNow') and x.kode_pp='$kode_pp'			
                                    group by y.nis,y.kode_lokasi 			
                                    )b on a.nis=b.nis and a.kode_lokasi=b.kode_lokasi
                        left join (select y.nis,y.kode_lokasi,  
                                        sum(case when x.dc='D' then x.nilai else -x.nilai end) as total		
                        ...
                        where a.kode_pp='$kode_pp' and a.kode_lokasi='$kode_lokasi' and a.nis='$nik2'
                        order by a.kode_kelas,a.nis";
                        // echo $sql;
                        $rsTung = execute($sql);  
