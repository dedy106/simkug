<?php
    ...
    $root_app=$_SERVER['REQUEST_SCHEME']."://".$_SERVER['SERVER_NAME']."/web/app/yakes";
    $root_ser=$_SERVER['REQUEST_SCHEME']."://".$_SERVER['SERVER_NAME']."/web/server/yakes";
	$root2=$_SERVER['REQUEST_SCHEME']."://".$_SERVER['SERVER_NAME'];
	
    //$root=$_SERVER["DOCUMENT_ROOT"];
    //$root_app="http://".$_SERVER['SERVER_NAME']."/web/app/yakes";
    //$root_ser="http://".$_SERVER['SERVER_NAME']."/web/server/yakes";
	//$root2="http://".$_SERVER['SERVER_NAME'];
	
	$folder_css=$root2."/web/css";
	$folder_js=$root2."/web/js";
    $folder_img=$root2."/web/img";
	$folder_fonts=$root2."/web/fonts";
    
   
    session_start();
    	
    if(!$_SESSION['isLogedIn']){
        echo "<script>alert('Harap login terlebih dahulu !'); window.location='$root_app/mobile';</script>";
    }
...
<head>
    ...
    <title>Mobile<?php echo $app_nama?></title>
   ...
   <!-- AdminLTE App -->
    ...
    <script src="<?=$folder_js?>/jquery.easing.1.3"></script>
    
    <meta name="theme-color" content="#ffffff">
    <style type="text/css">
        @font-face {
				font-family: 'Font SF';
				src: url('<?=$folder_fonts?>/SF-Pro-Text-Regular.otf');
            }
...
<body style="font-family: 'Roboto', sans-serif;" class="skin-red fixed sidebar-mini sidebar-mini-expand-feature sidebar-collapse">
    <div class="wrapper" style='background-color:#ffff;'>
        ...
        <?php
            ...
            if (count($daftar_menu) > 0){
                for($i=0; $i<count($daftar_menu); $i++){
                    ...
                    $forms = explode("/",$forms);
                    $this_link = "$root_app/mainmobile/".$forms[2];
                    $formAktif = str_replace(".php","",$_GET['hal']);
                    $link_aktif = "$root_app/mainmobile/".$formAktif;                    
        ...
        ?>
                    ...
                    <span><i class='<?php if ($daftar_menu[$i]['icon'] == ""){ 
                            echo $icon[$i]; 
                        }else{ 
                            echo $daftar_menu[$i]['icon']; } 
                        ?>' style='text-align: center;'></i></span><br><span style='text-align: center;font-size: 14px;'><?php echo $daftar_menu[$i]["nama"]; ?></span></a>
