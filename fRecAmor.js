window.app_saku2_transaksi_gl_fRecAmor = function(owner)
{
	if (owner)
	{
		...
		this.app._mainForm.childFormConfig(this, "mainButtonClick","Form Transaksi Recurring", 0);	
...
window.app_saku2_transaksi_gl_fRecAmor.implement({
	...
	simpan: function(){			
		try{						
			this.e_nb.setText(this.standarLib.noBuktiOtomatis(this.dbLib,"ju_m","no_ju",this.app._lokasi+"-REC"+this.e_periode.getText().substr(2,4)+".","000"));
	...
	doLoadData: function(sender){
		var strSQL = "select a.no_kas,b.no_dokumen,convert(varchar,b.tanggal,103) as tanggal,a.keterangan,a.per_awal,a.per_akhir,a.nilai_bdd,"+
		             "      a.nilai_rec,a.nilai_bdd - isnull(c.nilai_beban,0) as saldo "+  //case when a.nilai_bdd - isnull(c.nilai_beban,0)<a.nilai_rec then a.nilai_bdd - isnull(c.nilai_beban,0) else a.nilai_rec end as nilai_rec
